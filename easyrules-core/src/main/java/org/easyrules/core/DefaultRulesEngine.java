...
class DefaultRulesEngine implements RulesEngine {
  ...
  @Override public RulesEngineParameters getParameters() {
    return parameters;
  }

  @Override public void registerRule(final Object rule) {
    rules.add(asRule(rule));
  }

  @Override public void unregisterRule(final Object rule) {
    rules.remove(asRule(rule));
  }

  @Override public Set<Rule> getRules() {
    return rules;
  }

  @Override public List<RuleListener> getRuleListeners() {
  ...
  @Override public void fireRules() {
    if (rules.isEmpty()) {
  ...
  @Override public Map<Rule, Boolean> checkRules() {
  ...
  @Override public void unregisterRuleByName(final String ruleName) {
  ...
  private void applyRules() {
    ...
    for (Rule rule : rules) {
      ...
      if (priority > parameters.getPriorityThreshold()) {
        LOGGER.log(Level.INFO, "Rule priority threshold ({0}) exceeded at rule \'\'{1}\'\' with priority={2}, next rules will be skipped", new Object[] { parameters.getPriorityThreshold(), name, priority });
        break;
      }
      if (!shouldBeEvaluated(rule)) {
  ...
  private void logRegisteredRules() {
    ...
    for (Rule rule : rules) {
      LOGGER.log(Level.INFO, format("Rule { name = \'%s\', description = \'%s\', priority = \'%s\'}", rule.getName(), rule.getDescription(), rule.getPriority()));
    }
  }
