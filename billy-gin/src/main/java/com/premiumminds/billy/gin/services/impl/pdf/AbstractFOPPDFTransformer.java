...
public abstract class AbstractFOPPDFTransformer<T extends GenericInvoiceData> extends FOPPDFTransformer implements BillyExportTransformer<T, OutputStream> {
  ...
  public AbstractFOPPDFTransformer(Class<T> transformableClass, MathContext mc, String logoImagePath, InputStream xsltFileStream) {
  ...
  @Override public Class<T> getTransformableClass() {
    return this.transformableClass;
  }

  @Override public void transform(T document, OutputStream targetStream) throws ExportServiceException {
    this.transformToStream(this.xsltFileStream, this.mapDocumentToParamsTree(document), targetStream);
  }
  ...
  protected void setHeader(ParamsTree<String, String> params, T document) {
    ...
    if (document.getPayments() != null) {
      for (PaymentData payment : document.getPayments()) {
        params.getRoot().addChild(ParamKeys.INVOICE_PAYMETHOD, this.getPaymentMechanismTranslation(payment.getPaymentMethod()));
      }
    }
    params.getRoot().addChild(ParamKeys.EMISSION_DATE, AbstractFOPPDFTransformer.DATE_FORMAT.format(document.getDate()));
    if (null != document.getSettlementDate()) {
      params.getRoot().addChild(ParamKeys.DUE_DATE, AbstractFOPPDFTransformer.DATE_FORMAT.format(document.getSettlementDate()));
    }
  }
  ...
  protected void setTaxDetails(TaxTotals taxTotals, ParamsTree<String, String> params) {
    ...
    for (TaxTotals.TaxTotalEntry taxDetail : taxTotals.getEntries()) {
      ...
      taxDetailNode.addChild(ParamKeys.TAX_DETAIL_TAX, taxDetail.getTaxValue().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString() + (taxDetail.isPercentage() ? "%" : "&#8364;"));
      taxDetailNode.addChild(ParamKeys.TAX_DETAIL_NET_VALUE, taxDetail.getNetValue().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
      taxDetailNode.addChild(ParamKeys.TAX_DETAIL_VALUE, taxDetail.getAppliedTaxValue().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
  ...
  protected void setEntries(TaxTotals taxTotals, ParamsTree<String, String> params, T document) {
    ...
    for (InvoiceEntryData entry : entriesList) {
      ...
      entryNode.addChild(ParamKeys.ENTRY_QUANTITY, entry.getQuantity().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
      entryNode.addChild(ParamKeys.ENTRY_UNIT_PRICE, entry.getUnitAmountWithTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
      entryNode.addChild(ParamKeys.ENTRY_TOTAL, entry.getAmountWithTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
      Collection<TaxData> taxList = entry.getTaxes();
      for (TaxData tax : taxList) {
        entryNode.addChild(ParamKeys.ENTRY_TAX, tax.getValue().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString() + (tax.getTaxRateType() == TaxRateType.PERCENTAGE ? "%" : "&#8364;"));
        taxTotals.add((tax.getTaxRateType() == TaxRateType.PERCENTAGE ? true : false), tax.getValue(), entry.getAmountWithoutTax(), entry.getTaxAmount(), tax.getUID().getValue(), tax.getDesignation(), tax.getDescription());
      }
    }
  }
  ...
  protected void setBusiness(ParamsTree<String, String> params, T document) {
    ...
    businessInfo.addChild(ParamKeys.BUSINESS_LOGO, this.logoImagePath);
    ...
    businessAddress.addChild(ParamKeys.BUSINESS_ADDRESS_COUNTRY, document.getBusiness().getAddress().getISOCountry());
    businessAddress.addChild(ParamKeys.BUSINESS_ADDRESS_DETAILS, document.getBusiness().getAddress().getDetails());
    businessAddress.addChild(ParamKeys.BUSINESS_ADDRESS_CITY, document.getBusiness().getAddress().getCity());
    businessAddress.addChild(ParamKeys.BUSINESS_ADDRESS_REGION, document.getBusiness().getAddress().getRegion());
    businessAddress.addChild(ParamKeys.BUSINESS_ADDRESS_POSTAL_CODE, document.getBusiness().getAddress().getPostalCode());
    Node<String, String> businessContacts = businessInfo.addChild(ParamKeys.BUSINESS_CONTACTS);
    businessContacts.addChild(ParamKeys.BUSINESS_PHONE, document.getBusiness().getMainContact().getTelephone());
    businessContacts.addChild(ParamKeys.BUSINESS_FAX, document.getBusiness().getMainContact().getFax());
    businessContacts.addChild(ParamKeys.BUSINESS_EMAIL, document.getBusiness().getMainContact().getEmail());
    return;
  }
  ...
  protected void setCustomer(ParamsTree<String, String> params, T document) {
    ...
    if (document.getCustomer().getBillingAddress() != null) {
      ...
      customerAddress.addChild(ParamKeys.CUSTOMER_BILLING_ADDRESS_COUNTRY, document.getCustomer().getBillingAddress().getISOCountry());
      customerAddress.addChild(ParamKeys.CUSTOMER_BILLING_ADDRESS_DETAILS, document.getCustomer().getBillingAddress().getDetails());
      customerAddress.addChild(ParamKeys.CUSTOMER_BILLING_ADDRESS_CITY, document.getCustomer().getBillingAddress().getCity());
      customerAddress.addChild(ParamKeys.CUSTOMER_BILLING_ADDRESS_REGION, document.getCustomer().getBillingAddress().getRegion());
      customerAddress.addChild(ParamKeys.CUSTOMER_BILLING_ADDRESS_POSTAL_CODE, document.getCustomer().getBillingAddress().getPostalCode());
    }
    return;
  }

  protected void setTaxValues(ParamsTree<String, String> params, T document) {
    params.getRoot().addChild(ParamKeys.TOTAL_BEFORE_TAX, document.getAmountWithoutTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
    params.getRoot().addChild(ParamKeys.TOTAL_TAX, document.getTaxAmount().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
    params.getRoot().addChild(ParamKeys.TOTAL, document.getAmountWithTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString());
    Validate.isTrue(document.getAmountWithoutTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).add(document.getTaxAmount().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode())).compareTo(document.getAmountWithTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode())) == 0);
  }
  ...
  protected class TaxTotals {
    ...
    private class TaxTotalEntry {
      ...
      public TaxTotalEntry(boolean perc, BigDecimal taxValue, BigDecimal baseValue, BigDecimal appliedTaxValue, String designation, String description) {
      ...
      public void addBaseValue(BigDecimal val) {
        this.baseValue = this.baseValue.add(val, AbstractFOPPDFTransformer.this.mc);
      }

      public void addAppliedTaxValue(BigDecimal val) {
        this.appliedTaxValue = this.appliedTaxValue.add(val, AbstractFOPPDFTransformer.this.mc);
      }

      public BigDecimal getAppliedTaxValue() {
        return this.appliedTaxValue;
      }
    }
    ...
    public void add(boolean isPercentage, BigDecimal taxValue, BigDecimal baseValue, BigDecimal taxAmount, String taxUid, String designation, String description) {
      TaxTotalEntry currentEntry = new TaxTotalEntry(isPercentage, taxValue, baseValue, taxAmount, designation, description);
