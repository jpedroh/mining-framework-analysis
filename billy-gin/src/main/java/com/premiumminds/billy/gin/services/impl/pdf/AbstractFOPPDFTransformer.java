  ...
  protected void setEntries(TaxTotals taxTotals, ParamsTree<String, String> params, T document) {
    ...
    for (InvoiceEntryData entry : entriesList) {
      ...
      for (TaxData tax : taxList) {
        entryNode.addChild(ParamKeys.ENTRY_TAX, tax.getValue().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).toPlainString() + (shouldPrintPercent(tax.getTaxRateType()) ? "%" : "&#8364;"));
        taxTotals.add(shouldPrintPercent(tax.getTaxRateType()), tax.getValue(), entry.getAmountWithoutTax(), entry.getTaxAmount(), tax.getUID().getValue(), tax.getDesignation(), tax.getDescription());
      }
    }
  }
