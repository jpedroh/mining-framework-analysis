 ...
 * Created by bpolster.
 */
@Service(value = "blStaticAssetService") public class StaticAssetServiceImpl implements StaticAssetService {
  ...
  @Resource(name = "blImageArtifactProcessor") protected ImageArtifactProcessor imageArtifactProcessor;

  @Value(value = "${asset.use.filesystem.storage}") protected boolean storeAssetsOnFileSystem = false;

  @Resource(name = "blStaticAssetDao") protected StaticAssetDao staticAssetDao;

  @Resource(name = "blStaticAssetStorageService") protected StaticAssetStorageService staticAssetStorageService;

  @Resource(name = "blStaticAssetPathService") protected StaticAssetPathService staticAssetPathService;

  @Resource(name = "blStaticAssetMultiTenantExtensionManager") protected StaticAssetMultiTenantExtensionManager staticAssetExtensionManager;

  @Value(value = "${should.accept.non.image.asset:true}") protected boolean shouldAcceptNonImageAsset;

  @Value(value = "${disabled.file.extensions}") protected String disabledFileExtensions;

  private final Random random = new Random();

  private final String FILE_NAME_CHARS = "0123456789abcdef";

  @Value(value = "${static.asset.invalid.chars.in.filename}") protected char[] notAllowedCharsInFileName;

  @Value(value = "${static.asset.exception.on.invalid.char.in.filename:false}") protected boolean exceptionOnInvalidChar = false;

  @Value(value = "${static.asset.invalid.chars.replacement}") protected String replacementString;
  ...
  @Override public Long findTotalStaticAssetCount() {
  ...
  @Override @Transactional(value = TransactionUtils.DEFAULT_TRANSACTION_MANAGER) public StaticAsset createStaticAssetFromFile(MultipartFile file, Map<String, String> properties) {
    try {
  ...
  @Override @Transactional(value = TransactionUtils.DEFAULT_TRANSACTION_MANAGER) public StaticAsset createStaticAsset(InputStream inputStream, String fileName, long fileSize, Map<String, String> properties) {
    if (properties == null) {
  ...
  @Override public StaticAsset findStaticAssetByFullUrl(String fullUrl) {
    try {
  ...
  @Override @Transactional(value = TransactionUtils.DEFAULT_TRANSACTION_MANAGER) public StaticAsset addStaticAsset(StaticAsset staticAsset) {
    StaticAsset newAsset = staticAssetDao.addOrUpdateStaticAsset(staticAsset, true);
    return newAsset;
  }

  @Override @Transactional(value = TransactionUtils.DEFAULT_TRANSACTION_MANAGER) public StaticAsset updateStaticAsset(StaticAsset staticAsset) {
    return staticAssetDao.addOrUpdateStaticAsset(staticAsset, true);
  }

  @Override @Transactional(value = TransactionUtils.DEFAULT_TRANSACTION_MANAGER) public void deleteStaticAsset(StaticAsset staticAsset) {
  ...
  @Override public String getPrefixedStaticAssetUrl(String assetUrl) {
