 ...
 */
@Service(value = "blFormBuilderService") public class FormBuilderServiceImpl implements FormBuilderService {
  ...
  @Resource(name = "blAdminEntityService") protected AdminEntityService adminEntityService;

  @Resource(name = "blAdminNavigationService") protected AdminNavigationService navigationService;

  @Resource(name = "blFormBuilderExtensionManager") protected FormBuilderExtensionManager extensionManager;

  @Resource(name = "blEntityConfiguration") protected EntityConfiguration entityConfiguration;

  @Resource(name = "blAdminSecurityRemoteService") protected SecurityVerifier adminRemoteSecurityService;

  @Resource(name = "blRowLevelSecurityService") protected RowLevelSecurityService rowLevelSecurityService;

  @Resource(name = "blMediaBuilderService") protected MediaBuilderService mediaBuilderService;

  @Resource(name = "blListGridErrorMessageExtensionManager") protected ListGridErrorMessageExtensionManager listGridErrorExtensionManager;

  @Resource(name = "blAdminNavigationService") protected AdminNavigationService adminNavigationService;
  ...
  @Resource(name = "blTranslationService") protected TranslationService translationService;

  @Value(value = "${use.translation.search:false}") protected boolean useTranslationSearch;

  @Resource(name = "blExploitProtectionService") ExploitProtectionService exploitProtectionService;

  protected static final VisibilityEnum[] FORM_HIDDEN_VISIBILITIES = new VisibilityEnum[] { VisibilityEnum.HIDDEN_ALL, VisibilityEnum.FORM_HIDDEN };

  protected static final VisibilityEnum[] GRID_HIDDEN_VISIBILITIES = new VisibilityEnum[] { VisibilityEnum.HIDDEN_ALL, VisibilityEnum.GRID_HIDDEN };

  @Override public ListGrid buildMainListGrid(DynamicResultSet drs, ClassMetadata cmd, String sectionKey, List<SectionCrumb> sectionCrumbs) throws ServiceException {
  ...
  protected FieldDTO constructFieldDTOFromFieldData(Field field, BasicFieldMetadata fmd) {
    ...
    if (field.getFieldType().equals("STRING")) {
      ...
      if (field.getFieldType().equals("DATE")) {
        ...
        if (field.getFieldType().equals("NUMBER") || field.getFieldType().equals("MONEY") || field.getFieldType().equals("DECIMAL")) {
          ...
          if (field.getFieldType().equals("BOOLEAN")) {
            ...
            if (field.getFieldType().equals("BROADLEAF_ENUMERATION")) {
              ...
              if (field.getFieldType().equals("ADDITIONAL_FOREIGN_KEY")) {
  ...
  @Override public ListGrid buildCollectionListGrid(String containingEntityId, DynamicResultSet drs, Property field, String sectionKey, List<SectionCrumb> sectionCrumbs) throws ServiceException {
    ...
    if (fmd instanceof BasicFieldMetadata) {
      ...
      if (fmd instanceof BasicCollectionMetadata) {
        ...
        if (fmd instanceof AdornedTargetCollectionMetadata) {
          modalSingleSelectable = true;
    ...
    if (CollectionUtils.isEmpty(headerFields)) {
      ...
      if (selectize && (type == ListGrid.Type.ADORNED || type == ListGrid.Type.ADORNED_WITH_FORM)) {
        ...
        if (type == ListGrid.Type.ADORNED || type == ListGrid.Type.ADORNED_WITH_FORM) {
          ...
          if (selectize && type == ListGrid.Type.BASIC) {
  ...
  @Override public Map<String, Object> buildSelectizeCollectionInfo(String containingEntityId, DynamicResultSet drs, Property field, String sectionKey, List<SectionCrumb> sectionCrumbs) throws ServiceException {
    FieldMetadata fmd = field.getMetadata();
    PersistencePackageRequest ppr = PersistencePackageRequest.fromMetadata(fmd, sectionCrumbs);
    if (field != null) {
      ppr.setSectionEntityField(field.getName());
  ...
  @Override public Map<String, Object> constructSelectizeOptionMap(DynamicResultSet drs, ClassMetadata cmd) {
    ...
    for (Entity e : drs.getRecords()) {
      ...
      if (e.findProperty("id") != null) {
        ...
        if (e.findProperty("localeCode") != null) {
          selectizeOption.put("id", e.findProperty("localeCode").getValue());
        } else {
          if (e.findProperty("currencyCode") != null) {
      ...
      if (property.getMetadata() instanceof BasicFieldMetadata) {
        ...
        if (!ArrayUtils.contains(getFormHiddenVisibilities(), fmd.getVisibility())) {
          ...
          if (fieldType.equals(SupportedFieldType.BROADLEAF_ENUMERATION.toString()) || fieldType.equals(SupportedFieldType.EXPLICIT_ENUMERATION.toString()) || fieldType.equals(SupportedFieldType.DATA_DRIVEN_ENUMERATION.toString()) || fieldType.equals(SupportedFieldType.EMPTY_ENUMERATION.toString())) {
            ...
            if (fieldType.equals(SupportedFieldType.CODE.toString())) {
              f = new CodeField();
            } else {
              if (fieldType.equals(SupportedFieldType.RULE_SIMPLE.toString()) || fieldType.equals(SupportedFieldType.RULE_SIMPLE_TIME.toString()) || fieldType.equals(SupportedFieldType.RULE_WITH_QUANTITY.toString())) {
                ...
                if (fieldType.equals(SupportedFieldType.RULE_SIMPLE.toString())) {
                  ...
                  if (fieldType.equals(SupportedFieldType.RULE_WITH_QUANTITY.toString())) {
                    ((RuleBuilderField) f).setRuleType("rule-builder-with-quantity");
                  } else {
                    if (fieldType.equals(SupportedFieldType.RULE_SIMPLE_TIME.toString())) {
                ...
                if (LookupType.DROPDOWN.equals(fmd.getLookupType())) {
                  ...
                  if (fieldType.equals(SupportedFieldType.MEDIA.toString())) {
                    f = new MediaField();
                  } else {
                    f = new Field();
                  }
  ...
  @Override public String extractDefaultValueFromFieldData(String fieldType, BasicFieldMetadata fmd) {
    String defaultValue = fmd.getDefaultValue();
    if (fieldType.equals(SupportedFieldType.RULE_SIMPLE.toString()) || fieldType.equals(SupportedFieldType.RULE_SIMPLE_TIME.toString()) || fieldType.equals(SupportedFieldType.RULE_WITH_QUANTITY.toString())) {
      return null;
    } else {
      if (fieldType.equals(SupportedFieldType.INTEGER.toString())) {
        try {
        ...
        if (fieldType.equals(SupportedFieldType.DECIMAL.toString()) || fieldType.equals(SupportedFieldType.MONEY.toString())) {
          try {
            DecimalFormat numberFormat = (DecimalFormat) NumberFormat.getInstance(BroadleafRequestContext.getBroadleafRequestContext().getJavaLocale());
          ...
          if (fieldType.equals(SupportedFieldType.BOOLEAN.toString())) {
            if (!defaultValue.toLowerCase().equals("true") && !defaultValue.toLowerCase().equals("false") && !defaultValue.toUpperCase().equals("Y") && !defaultValue.toUpperCase().equals("N")) {
              String msg = buildMsgForDefValException(SupportedFieldType.BOOLEAN.toString(), fmd, defaultValue);
              LOG.debug(msg);
              return null;
            }
          } else {
            if (fieldType.equals(SupportedFieldType.DATE.toString())) {
  ...
  protected String buildMsgForDefValException(String type, BasicFieldMetadata fmd, String defaultValue) {
    return StringUtil.sanitize(fmd.getTargetClass()) + " : " + StringUtil.sanitize(fmd.getName()) + " - Failed to parse " + StringUtil.sanitize(type) + " from DefaultValue [ " + StringUtil.sanitize(defaultValue) + " ]";
  }

  @Override public void removeNonApplicableFields(ClassMetadata cmd, EntityForm entityForm, String entityType) {
    for (Property p : cmd.getProperties()) {
  ...
  @Override public EntityForm createEntityForm(ClassMetadata cmd, List<SectionCrumb> sectionCrumbs) throws ServiceException {
  ...
  @Override public void populateEntityForm(ClassMetadata cmd, EntityForm ef, List<SectionCrumb> sectionCrumbs) throws ServiceException {
  ...
  @Override public EntityForm createEntityForm(ClassMetadata cmd, Entity entity, List<SectionCrumb> sectionCrumbs) throws ServiceException {
    EntityForm ef = createStandardEntityForm();
  ...
  @Override public void populateEntityForm(ClassMetadata cmd, Entity entity, EntityForm ef, List<SectionCrumb> sectionCrumbs) throws ServiceException {
    populateEntityForm(cmd, ef, sectionCrumbs);
  ...
  @Override public void populateEntityFormFieldValues(ClassMetadata cmd, Entity entity, EntityForm ef) {
    for (Property p : cmd.getProperties()) {
      if (p.getMetadata() instanceof BasicFieldMetadata) {
          ...
          if (entityProp == null && !SupportedFieldType.PASSWORD_CONFIRM.equals(basicFM.getExplicitFieldType())) {
            ...
            if (field != null) {
              ...
              if (basicFM.getFieldType() == SupportedFieldType.MEDIA) {
                ...
                if (!SupportedFieldType.PASSWORD_CONFIRM.equals(basicFM.getExplicitFieldType())) {
  ...
  @Override public EntityForm createEntityForm(ClassMetadata cmd, Entity entity, Map<String, DynamicResultSet> collectionRecords, List<SectionCrumb> sectionCrumbs) throws ServiceException {
    EntityForm ef = createStandardEntityForm();
  ...
  @Override public void populateEntityForm(ClassMetadata cmd, Entity entity, Map<String, DynamicResultSet> collectionRecords, EntityForm ef, List<SectionCrumb> sectionCrumbs) throws ServiceException {
    populateEntityForm(cmd, entity, ef, sectionCrumbs);
    for (Property p : cmd.getProperties()) {
      if (p.getMetadata() instanceof BasicFieldMetadata) {
        continue;
  ...
  protected boolean isDeletionAllowed(final EntityForm entityForm, final ClassMetadata cmd, final Entity entity) {
    ...
    return rowLevelSecurityService.canUpdate(persistentAdminUser, entity) && rowLevelSecurityService.canRemove(persistentAdminUser, entity);
  }
  ...
  protected boolean isDuplicationAllowed(final EntityForm entityForm, final ClassMetadata cmd) {
    ...
    return duplicator.validate(entityClass, Long.valueOf(entityForm.getId())) && rowLevelSecurityService.canAdd(adminRemoteSecurityService.getPersistentAdminUser(), securityClassname, cmd);
  }
  ...
  @Override public void populateEntityFormFields(EntityForm ef, Entity entity) {
    populateEntityFormFields(ef, entity, true, true);
  }

  @Override public void populateEntityFormFields(EntityForm ef, Entity entity, boolean populateType, boolean populateId) {
    if (populateId) {
  ...
  @Override public void populateAdornedEntityFormFields(EntityForm ef, Entity entity, AdornedTargetList adornedList) {
  ...
  @Override public void populateMapEntityFormFields(EntityForm ef, Entity entity) {
  ...
  @Override public EntityForm buildAdornedListForm(AdornedTargetCollectionMetadata adornedMd, AdornedTargetList adornedList, String parentId, boolean isViewCollectionItem, List<SectionCrumb> sectionCrumbs, boolean isAdd) throws ServiceException {
  ...
  @Override public EntityForm buildAdornedListForm(AdornedTargetCollectionMetadata adornedMd, AdornedTargetList adornedList, String parentId, boolean isViewCollectionItem, EntityForm ef, List<SectionCrumb> sectionCrumbs, boolean isAdd) throws ServiceException {
  ...
  @Override public EntityForm buildMapForm(MapMetadata mapMd, final MapStructure mapStructure, ClassMetadata cmd, String parentId) throws ServiceException {
    EntityForm ef = createStandardEntityForm();
    return buildMapForm(mapMd, mapStructure, cmd, parentId, ef);
  }

  @Override public EntityForm buildMapForm(MapMetadata mapMd, final MapStructure mapStructure, ClassMetadata cmd, String parentId, EntityForm ef) throws ServiceException {
