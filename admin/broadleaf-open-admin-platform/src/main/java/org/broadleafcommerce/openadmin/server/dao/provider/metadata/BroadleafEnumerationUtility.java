 ...
 */
@Component(value = "blBroadleafEnumerationUtility") public class BroadleafEnumerationUtility {
  @SuppressWarnings(value = { "rawtypes" }) public List<Tuple<String, String>> getEnumerationValues(String broadleafEnumerationClass, DynamicEntityDao dynamicEntityDao) {
    try {
  ...
  protected Map<Object, ?> getTypesMap(final Field types, @SuppressWarnings(value = { "unused" }) final Class<?> broadleafEnumeration) throws IllegalAccessException {
  ...
  protected Collection<Object> getSortedEnumValues(final Map<Object, ?> typesMap) {
    ...
    return broadleafEnumerationTypes.stream().map((objectComparable) -> (Object) objectComparable).collect(Collectors.toList());
  }
}