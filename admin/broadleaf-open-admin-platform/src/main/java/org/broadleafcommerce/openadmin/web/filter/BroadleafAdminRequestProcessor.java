 ...
 */
@Component(value = "blAdminRequestProcessor") public class BroadleafAdminRequestProcessor extends AbstractBroadleafWebRequestProcessor {
  ...
  @Resource(name = "blSiteResolver") protected BroadleafSiteResolver siteResolver;

  @Resource(name = "messageSource") protected MessageSource messageSource;

  @Resource(name = "blLocaleResolver") protected BroadleafLocaleResolver localeResolver;

  @Resource(name = "blAdminTimeZoneResolver") protected BroadleafTimeZoneResolver broadleafTimeZoneResolver;

  @Resource(name = "blCurrencyResolver") protected BroadleafCurrencyResolver currencyResolver;

  @Resource(name = "blSandBoxService") protected SandBoxService sandBoxService;

  @Resource(name = "blSiteService") protected SiteService siteService;
  ...
  @Resource(name = "blDeployBehaviorUtil") protected DeployBehaviorUtil deployBehaviorUtil;

  @Value(value = "${" + ADMIN_STRICT_VALIDATE_PRODUCTION_CHANGES_KEY + ":true}") protected boolean adminStrictValidateProductionChanges = true;

  @Resource(name = "blEntityExtensionManagers") protected Map<String, ExtensionManager<?>> entityExtensionManagers;

  @Resource(name = "blAdminRequestProcessorExtensionManager") protected AdminRequestProcessorExtensionManager extensionManager;

  @Resource(name = "blStaleStateProtectionService") protected StaleStateProtectionService staleStateProtectionService;
  ...
  protected void prepareProfile(WebRequest request, BroadleafRequestContext brc) {
    ...
    if (adminUser != null || ((ServletWebRequest) request).getRequest().getRequestURL().toString().contains("sendResetPassword")) {
      ...
      if (profile == null) {
        ...
        if (BLCRequestUtils.isOKtoUseSession(request)) {
          previouslySetProfileId = (Long) request.getAttribute(PROFILE_REQ_PARAM, WebRequest.SCOPE_SESSION);
        }
  ...
  protected void prepareCatalog(WebRequest request, BroadleafRequestContext brc) {
    ...
    if (adminUser == null) {
      ...
      if (StringUtils.isNotBlank(request.getParameter(CATALOG_REQ_PARAM))) {
        ...
        if (StringUtils.isNotBlank(request.getParameter("catalogEntityCatalogDiscriminatorId"))) {
      ...
      if (catalog == null) {
        ...
        if (BLCRequestUtils.isOKtoUseSession(request)) {
          previouslySetCatalogId = (Long) request.getAttribute(CATALOG_REQ_PARAM, WebRequest.SCOPE_SESSION);
        }
