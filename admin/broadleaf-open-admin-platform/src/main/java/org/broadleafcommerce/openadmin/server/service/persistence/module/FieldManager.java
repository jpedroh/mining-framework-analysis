...
public class FieldManager {
  ...
  public Field getField(Class<?> clazz, String fieldName) throws IllegalStateException {
    ...
    BLCFieldUtils fieldUtils = new BLCFieldUtils(true, dynamicEntityDao.useCache(), entityConfiguration, dynamicEntityDao.getDynamicDaoHelper());
        ...
        if (myValue != null) {
          ...
          try {
            ...
            if (!ArrayUtils.isEmpty(entities)) {
              ...
              componentClass = newEntity.getClass();
              value = newEntity;
              LOG.debug("Unable to find a reference to (" + field.getType().getName() + ") in the EntityConfigurationManager. " + "Using the type of this class.");
            }
          }
        }
      }
  ...
  protected Object handleMapFieldExtraction(Object bean, String fieldName, Class<?> componentClass, Object value, String fieldNamePart, String mapKey) throws IllegalAccessException, FieldNotAvailableException {
    ...
    if (value != null && !(value instanceof Map)) {
      ...
      throw new IllegalArgumentException(String.format("A field containing a map field separator was requested " + "(%s), but no Map type field or method returning a Map was found using the following tests (%s)", fieldName, combined));
    }
    if (value != null) {
  ...
  private class SortableValue implements Comparable<SortableValue> {
    ...
    @Override public int compareTo(SortableValue o) {
