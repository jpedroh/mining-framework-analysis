 ...
 * @author Jeff Fischer
 */
@Component(value = "blMapFieldsFieldMetadataProvider") @Scope(value = "prototype") public class MapFieldsFieldMetadataProvider extends DefaultFieldMetadataProvider {
  private static final Log LOG = LogFactory.getLog(MapFieldsFieldMetadataProvider.class);

  @Override protected boolean canHandleFieldForConfiguredMetadata(AddFieldMetadataRequest addMetadataRequest, Map<String, FieldMetadata> metadata) {
  ...
  @Override public MetadataProviderResponse addMetadata(AddFieldMetadataRequest addMetadataRequest, Map<String, FieldMetadata> metadata) {
    ...
    for (AdminPresentationMapField mapField : annot.mapDisplayFields()) {
      ...
      if (annotMap != null && !StringUtils.isEmpty(annotMap.toOneTargetProperty())) {
        ...
        if (!StringUtils.isEmpty(annot.toOneTargetProperty())) {
      ...
      if (annotMap != null && !StringUtils.isEmpty(annotMap.toOneParentProperty())) {
        ...
        if (!StringUtils.isEmpty(annot.toOneParentProperty())) {
