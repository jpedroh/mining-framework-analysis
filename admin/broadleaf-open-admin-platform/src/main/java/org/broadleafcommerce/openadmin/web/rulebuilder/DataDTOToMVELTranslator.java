...
public class DataDTOToMVELTranslator {
    ...
    switch (operator) {
      ...
      case WITHIN_DAYS:
      {
        try {
          ...
          throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_INTEGER_VALUE, "Cannot format value for the field (" + field + ") based on field type. The type of field is Integer, " + "and you entered: (" + value[0] + ")");
        }
        sb.append("(");
        buildExpression(sb, entityKey, field, new Object[] { "MvelHelper.subtractFromCurrentTime(" + value[0] + ")" }, type, SupportedFieldType.WITHIN_DAYS, GREATER_THAN_OPERATOR, false, false, false, false, true);
        sb.append("&&");
        buildExpression(sb, entityKey, field, new Object[] { "MvelHelper.currentTime()" }, type, SupportedFieldType.WITHIN_DAYS, LESS_THAN_OPERATOR, false, false, false, false, true);
        sb.append(")");
        break;
      }
    }
  ...
  protected void buildExpression(StringBuffer sb, String entityKey, String field, Object[] value, SupportedFieldType type, SupportedFieldType secondaryType, String operator, boolean includeParenthesis, boolean isFieldComparison, boolean ignoreCase, boolean isNegation, boolean ignoreQuotes) throws MVELTranslationException {
    if (operator.equals(EQUALS_OPERATOR) && !isFieldComparison && value.length > 1) {
      ...
      if (CONTAINS_OPERATOR.equals(operator) || STARTS_WITH_OPERATOR.equals(operator) || ENDS_WITH_OPERATOR.equals(operator)) {
        if (isNegation) {
          sb.append("!");
        }
        sb.append(operator);
      ...
      for (int j = 0; j < value.length; j++) {
        ...
        switch (type) {
          ...
          try {
            ...
            throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_DECIMAL_VALUE, "Cannot format value for the field (" + fieldName + ") based on field type. The type of field is Decimal, " + "and you entered: (" + parsableVal + ")");
          }
          ...
          try {
            ...
            throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_INTEGER_VALUE, "Cannot format value for the field (" + fieldName + ") based on field type. The type of field is Integer, " + "and you entered: (" + parsableVal + ")");
          }
          response.append(parsableVal);
          break;
          case MONEY:
          try {
            ...
            throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_DECIMAL_VALUE, "Cannot format value for the field (" + fieldName + ") based on field type. The type of field is Money, " + "and you entered: (" + parsableVal + ")");
          }
          response.append(parsableVal);
          break;
          case DATE:
          if (SupportedFieldType.WITHIN_DAYS.equals(secondaryType)) {
            ...
            try {
              ...
              throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_DATE_VALUE, "Cannot format value for the field (" + fieldName + ") based on field type. The type of field is Date, " + "and you entered: (" + parsableVal + "). Dates must be in the format MM/dd/yyyy HH:mm.");
            }
