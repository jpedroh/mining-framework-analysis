...
@Component(value = "blBasicFieldMetadataProvider") @Scope(value = "prototype") public class BasicFieldMetadataProvider extends FieldMetadataProviderAdapter {
  ...
  @Override public MetadataProviderResponse addMetadata(AddFieldMetadataRequest addMetadataRequest, Map<String, FieldMetadata> metadata) {
  ...
  @Override public MetadataProviderResponse overrideViaAnnotation(OverrideViaAnnotationRequest overrideViaAnnotationRequest, Map<String, FieldMetadata> metadata) {
  ...
  @Override public MetadataProviderResponse overrideViaXml(OverrideViaXmlRequest overrideViaXmlRequest, Map<String, FieldMetadata> metadata) {
  ...
  protected FieldMetadataOverride overrideMergeMetadata(AdminPresentationMergeOverride merge) {
    ...
    for (Map.Entry<String, AdminPresentationMergeEntry> entry : overrideValues.entrySet()) {
      ...
      if (entry.getKey().equals(PropertyType.AdminPresentation.FRIENDLYNAME)) {
        ...
        if (entry.getKey().equals(PropertyType.AdminPresentation.ADDFRIENDLYNAME)) {
          fieldMetadataOverride.setAddFriendlyName(stringValue);
        } else {
          if (entry.getKey().equals(PropertyType.AdminPresentation.SECURITYLEVEL)) {
            fieldMetadataOverride.setSecurityLevel(stringValue);
          } else {
            if (entry.getKey().equals(PropertyType.AdminPresentation.GROUP)) {
              fieldMetadataOverride.setGroup(stringValue);
            } else {
              if (entry.getKey().equals(PropertyType.AdminPresentation.TAB)) {
                fieldMetadataOverride.setTab(stringValue);
              } else {
                if (entry.getKey().equals(PropertyType.AdminPresentation.COLUMNWIDTH)) {
                  fieldMetadataOverride.setColumnWidth(stringValue);
                } else {
                  if (entry.getKey().equals(PropertyType.AdminPresentation.BROADLEAFENUMERATION)) {
                    fieldMetadataOverride.setBroadleafEnumeration(stringValue);
                  } else {
                    if (entry.getKey().equals(PropertyType.AdminPresentation.HIDEENUMERATIONIFEMPTY)) {
                      ...
                      if (entry.getKey().equals(PropertyType.AdminPresentation.FIELDCOMPONENTRENDERER)) {
                        fieldMetadataOverride.setFieldComponentRenderer(SupportedFieldType.valueOf(stringValue));
                      } else {
                        if (entry.getKey().equals(PropertyType.AdminPresentation.FIELDCOMPONENTRENDERERTEMPLATE)) {
                          fieldMetadataOverride.setFieldComponentRendererTemplate(stringValue);
                        } else {
                          if (entry.getKey().equals(PropertyType.AdminPresentation.GRIDFIELDCOMPONENTRENDERER)) {
                            fieldMetadataOverride.setGridFieldComponentRenderer(SupportedFieldType.valueOf(stringValue));
                          } else {
                            if (entry.getKey().equals(PropertyType.AdminPresentation.GRIDFIELDCOMPONENTRENDERERTEMPLATE)) {
                              fieldMetadataOverride.setGridFieldComponentRendererTemplate(stringValue);
                            } else {
                              if (entry.getKey().equals(PropertyType.AdminPresentation.TOOLTIP)) {
                                fieldMetadataOverride.setTooltip(stringValue);
                              } else {
                                if (entry.getKey().equals(PropertyType.AdminPresentation.HELPTEXT)) {
                                  fieldMetadataOverride.setHelpText(stringValue);
                                } else {
                                  if (entry.getKey().equals(PropertyType.AdminPresentation.HINT)) {
                                    fieldMetadataOverride.setHint(stringValue);
                                  } else {
                                    if (entry.getKey().equals(PropertyType.AdminPresentation.SHOWIFPROPERTY)) {
                                      fieldMetadataOverride.setShowIfProperty(stringValue);
                                    } else {
                                      if (entry.getKey().equals(PropertyType.AdminPresentation.SHOWIFFIELDEQUALS)) {
                                        processShowIfFieldEqualsAnnotations(entry.getValue().showIfFieldEquals(), fieldMetadataOverride);
                                      } else {
                                        if (entry.getKey().equals(PropertyType.AdminPresentation.CURRENCYCODEFIELD)) {
                                          fieldMetadataOverride.setCurrencyCodeField(stringValue);
                                        } else {
                                          if (entry.getKey().equals(PropertyType.AdminPresentation.RULEIDENTIFIER)) {
                                            fieldMetadataOverride.setRuleIdentifier(stringValue);
                                          } else {
                                            if (entry.getKey().equals(PropertyType.AdminPresentation.ORDER)) {
                                              ...
                                              if (entry.getKey().equals(PropertyType.AdminPresentation.GRIDORDER)) {
                                                ...
                                                if (entry.getKey().equals(PropertyType.AdminPresentation.VISIBILITY)) {
                                                  fieldMetadataOverride.setVisibility(VisibilityEnum.valueOf(stringValue));
                                                } else {
                                                  if (entry.getKey().equals(PropertyType.AdminPresentation.FIELDTYPE)) {
                                                    fieldMetadataOverride.setFieldType(SupportedFieldType.valueOf(stringValue));
                                                  } else {
                                                    if (entry.getKey().equals(PropertyType.AdminPresentation.GROUPORDER)) {
                                                      ...
                                                      if (entry.getKey().equals(PropertyType.AdminPresentation.GROUPCOLLAPSED)) {
                                                        ...
                                                        if (entry.getKey().equals(PropertyType.AdminPresentation.TABORDER)) {
                                                          ...
                                                          if (entry.getKey().equals(PropertyType.AdminPresentation.LARGEENTRY)) {
                                                            ...
                                                            if (entry.getKey().equals(PropertyType.AdminPresentation.PROMINENT)) {
                                                              ...
                                                              if (entry.getKey().equals(PropertyType.AdminPresentation.READONLY)) {
                                                                ...
                                                                if (entry.getKey().equals(PropertyType.AdminPresentation.REQUIREDOVERRIDE)) {
                                                                  ...
                                                                  if (entry.getKey().equals(PropertyType.AdminPresentation.EXCLUDED)) {
                                                                    ...
                                                                    if (entry.getKey().equals(PropertyType.AdminPresentation.DEFAULTVALUE)) {
                                                                      fieldMetadataOverride.setDefaultValue(stringValue);
                                                                    } else {
                                                                      if (entry.getKey().equals(PropertyType.AdminPresentation.VALIDATIONCONFIGURATIONS)) {
                                                                        processValidationAnnotations(entry.getValue().validationConfigurations(), fieldMetadataOverride);
                                                                      } else {
                                                                        if (entry.getKey().equals(PropertyType.AdminPresentation.ASSOCIATEDFIELDNAME)) {
                                                                          fieldMetadataOverride.setAssociatedFieldName(stringValue);
                                                                        } else {
                                                                          if (entry.getKey().equals(PropertyType.AdminPresentationToOneLookup.LOOKUPDISPLAYPROPERTY)) {
                                                                            ...
                                                                            if (entry.getKey().equals(PropertyType.AdminPresentationToOneLookup.FORCEPOPULATECHILDPROPERTIES)) {
                                                                              ...
                                                                              if (entry.getKey().equals(PropertyType.AdminPresentationToOneLookup.ENABLETYPEAHEADLOOKUP)) {
                                                                                ...
                                                                                if (entry.getKey().equals(PropertyType.AdminPresentationToOneLookup.USESERVERSIDEINSPECTIONCACHE)) {
                                                                                  ...
                                                                                  if (entry.getKey().equals(PropertyType.AdminPresentationToOneLookup.LOOKUPTYPE)) {
                                                                                    fieldMetadataOverride.setLookupType(LookupType.valueOf(stringValue));
                                                                                  } else {
                                                                                    if (entry.getKey().equals(PropertyType.AdminPresentationToOneLookup.CUSTOMCRITERIA)) {
                                                                                      fieldMetadataOverride.setCustomCriteria(entry.getValue().stringArrayOverrideValue());
                                                                                    } else {
                                                                                      if (entry.getKey().equals(PropertyType.AdminPresentationDataDrivenEnumeration.OPTIONLISTENTITY)) {
                                                                                        fieldMetadataOverride.setOptionListEntity(stringValue);
                                                                                      } else {
                                                                                        if (entry.getKey().equals(PropertyType.AdminPresentationDataDrivenEnumeration.OPTIONVALUEFIELDNAME)) {
                                                                                          fieldMetadataOverride.setOptionValueFieldName(stringValue);
                                                                                        } else {
                                                                                          if (entry.getKey().equals(PropertyType.AdminPresentationDataDrivenEnumeration.OPTIONDISPLAYFIELDNAME)) {
                                                                                            fieldMetadataOverride.setOptionDisplayFieldName(stringValue);
                                                                                          } else {
                                                                                            if (entry.getKey().equals(PropertyType.AdminPresentationDataDrivenEnumeration.OPTIONHIDEIFEMPTY)) {
                                                                                              fieldMetadataOverride.setHideEnumerationIfEmpty(StringUtils.isEmpty(stringValue) ? entry.getValue().booleanOverrideValue() : Boolean.parseBoolean(stringValue));
                                                                                            } else {
                                                                                              if (entry.getKey().equals(PropertyType.AdminPresentationDataDrivenEnumeration.OPTIONCANEDITVALUES)) {
                                                                                                ...
                                                                                                if (entry.getKey().equals(PropertyType.AdminPresentationDataDrivenEnumeration.OPTIONFILTERPARAMS)) {
                                                                                                  ...
                                                                                                  if (entry.getKey().equals(PropertyType.AdminPresentation.CANLINKTOEXTERNALENTITY)) {
                                                                                                    ...
                                                                                                    if (entry.getKey().equals(PropertyType.AdminPresentation.TRANSLATABLE)) {
