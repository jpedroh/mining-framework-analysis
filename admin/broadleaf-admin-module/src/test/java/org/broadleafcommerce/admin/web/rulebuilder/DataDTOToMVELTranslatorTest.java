...
public class DataDTOToMVELTranslatorTest extends TestCase {
  ...
  public void testWithinDaysMVEL() throws MVELTranslationException {
    ...
    String d1Mvel = "(MvelHelper.convertField(\"DATE\",?customer.?getCustomerAttributes()[\"invoice_date\"])" + ">MvelHelper.convertField(\"DATE\",MvelHelper.subtractFromCurrentTime(12))" + "&&(MvelHelper.convertField(\"DATE\",?customer.?getCustomerAttributes()[\"invoice_date\"])" + "<MvelHelper.convertField(\"DATE\",MvelHelper.currentTime())))";
    customerFieldService.init();
    assert (d1Mvel.equals(d1Translated));
  }
}