 ...
 */
@Controller(value = "blAdminProductController") @RequestMapping(value = "/" + AdminProductController.SECTION_KEY) public class AdminProductController extends AdminBasicEntityController {
  ...
  @Override protected void modifyAddEntityForm(EntityForm ef, Map<String, String> pathVars) {
  ...
  @Override @RequestMapping(value = "/selectize", method = RequestMethod.GET) public @ResponseBody Map<String, Object> viewEntityListSelectize(HttpServletRequest request, HttpServletResponse response, Model model, @PathVariable Map<String, String> pathVars, @RequestParam MultiValueMap<String, String> requestParams) throws Exception {
  ...
  @Override protected String showViewUpdateCollection(HttpServletRequest request, Model model, Map<String, String> pathVars, String id, String collectionField, String collectionItemId, String alternateId, String modalHeaderType, EntityForm entityForm, Entity entity) throws ServiceException {
    try {
      if ("additionalSkus".equals(collectionField)) {
        return showUpdateAdditionalSku(request, model, id, collectionItemId, pathVars, entityForm);
      } else {
        return super.showViewUpdateCollection(request, model, pathVars, id, collectionField, collectionItemId, alternateId, modalHeaderType, entityForm, entity);
      }
    } catch (Exception e) {
  ...
  @Override @RequestMapping(value = "/{id}", method = RequestMethod.GET) public String viewEntityForm(HttpServletRequest request, HttpServletResponse response, Model model, @PathVariable Map<String, String> pathVars, @PathVariable(value = "id") String id) throws Exception {
  ...
  @Override protected void modifyCriteria(Map<String, FilterAndSortCriteria> fasMap) {
  ...
  @Override public String addCollectionItem(HttpServletRequest request, HttpServletResponse response, Model model, @PathVariable Map<String, String> pathVars, @PathVariable(value = "id") String id, @PathVariable(value = "collectionField") String collectionField, @ModelAttribute(value = "entityForm") EntityForm entityForm, BindingResult result) throws Exception {
    if (PRODUCT_OPTIONS_COLLECTION_FIELD.equals(collectionField)) {
      ...
      try {
        ...
        isAttributeNameExist = product.getProductOptionXrefs().stream().map(ProductOptionXref::getProductOption).map(ProductOption::getAttributeName).anyMatch((attributeName) -> attributeName.equals(addProductOption.getAttributeName()));
      } catch (Exception e) {
        LOG.error(e);
      }
      if (isAttributeNameExist) {
        return new JsonResponse(response).with("status", "error").with("message", String.format("Product Option with attribute name: \'%s\' is already assigned", addAttributeName)).done();
      }
    }
