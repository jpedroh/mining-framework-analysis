...
@Component(value = "blProductCustomPersistenceHandler") public class ProductCustomPersistenceHandler extends CustomPersistenceHandlerAdapter {
  ...
  @Resource(name = "blProductCustomPersistenceHandlerExtensionManager") protected ProductCustomPersistenceHandlerExtensionManager extensionManager;

  @Resource(name = "blParentCategoryLegacyModeService") protected ParentCategoryLegacyModeService parentCategoryLegacyModeService;

  @Resource(name = "blSandBoxHelper") protected SandBoxHelper sandBoxHelper;

  @Value(value = "${product.query.limit:500}") protected long queryLimit;

  @Value(value = "${product.eager.fetch.associations.admin:true}") protected boolean eagerFetchAssociations = true;
  ...
  @Override public DynamicResultSet inspect(PersistencePackage persistencePackage, DynamicEntityDao dynamicEntityDao, InspectHelper helper) throws ServiceException {
  ...
  @Override public DynamicResultSet fetch(PersistencePackage persistencePackage, CriteriaTransferObject cto, DynamicEntityDao dynamicEntityDao, RecordHelper helper) throws ServiceException {
    ...
    if (!legacy) {
      ...
      if (fsc != null) {
        ...
        cto.getNonCountAdditionalFilterMappings().add(new FilterMapping().withFieldPath(new FieldPath().withAssociationPath(Arrays.asList("allParentCategoryXrefs", "category")).withTargetPropertyPieces(Arrays.asList("name"))).withDirectFilterValues(new EmptyFilterValues()).withRestriction(new Restriction().withPredicateProvider(new PredicateProvider() {
          @Override public Predicate buildPredicate(CriteriaBuilder builder, FieldPathBuilder fieldPathBuilder, From root, String ceilingEntity, String fullPropertyName, Path explicitPath, List directValues) {
  ...
  @Override public Entity add(PersistencePackage persistencePackage, DynamicEntityDao dynamicEntityDao, RecordHelper helper) throws ServiceException {
  ...
  @Override public Entity update(PersistencePackage persistencePackage, DynamicEntityDao dynamicEntityDao, RecordHelper helper) throws ServiceException {
    Entity entity = persistencePackage.getEntity();
    try {
      PersistencePerspective persistencePerspective = persistencePackage.getPersistencePerspective();
      Map<String, FieldMetadata> adminProperties = helper.getSimpleMergedProperties(Product.class.getName(), persistencePerspective);
  ...
  @Override public void remove(PersistencePackage persistencePackage, DynamicEntityDao dynamicEntityDao, RecordHelper helper) throws ServiceException {
    Entity entity = persistencePackage.getEntity();
    try {
  ...
  protected boolean isRecursiveProductSelection(PersistencePackage persistencePackage) {
    ...
    return Stream.of("upsaleProduct", "crossSaleProduct", "requestingField=addOnProduct").anyMatch(customCriteria::contains);
  }
  ...
  protected FilterMapping createFilterMappingForProperty(String targetPropertyName, PredicateProvider predicateProvider) {
    ...
    return new FilterMapping().withFieldPath(fieldPath).withDirectFilterValues(directFilterValues).withRestriction(newRestriction);
  }
  ...
  protected DynamicResultSet getFilteredDynamicResultSet(PersistencePackage persistencePackage, CriteriaTransferObject cto, RecordHelper helper) throws ServiceException {
    ...
    if (sectionCrumbs != null && sectionCrumbs.length > 0) {
      FilterMapping defaultCategoryMapping = this.createFilterMappingForProperty(ID_PROPERTY, (builder, fieldPathBuilder, root, ceilingEntity, fullPropertyName, explicitPath, directValues) -> builder.and(builder.notEqual(explicitPath, sectionCrumbs[0].getSectionId())));
