...
@Component(value = "blOfferCustomPersistenceHandler") public class OfferCustomPersistenceHandler extends ClassCustomPersistenceHandlerAdapter {
  ...
  @Value(value = "${admin.offer.isactive.filter:false}") protected boolean isActiveFilter = false;

  @Resource(name = "blOfferCustomServiceExtensionManager") protected OfferCustomServiceExtensionManager extensionManager;
  ...
  @Override public DynamicResultSet fetch(PersistencePackage persistencePackage, CriteriaTransferObject cto, DynamicEntityDao dynamicEntityDao, RecordHelper helper) throws ServiceException {
    ...
    for (Entity entity : resultSet.getRecords()) {
      ...
      if (discountType == null || StringUtils.isBlank(discountType.getValue())) {
        ...
        switch (discountType.getValue()) {
          ...
          if (value.contains(decimalSeparator)) {
            value = value.replaceAll("0*$", "").replaceAll("\\" + decimalSeparator + "$", "");
          }
      ...
      if (!"listGridView".equals(customCriteria)) {
        ...
        setValue = setValue.replaceAll(Pattern.quote(moneyPrefix), "").replaceAll(Pattern.quote(moneySuffix), "").replaceAll("\\%", "").replaceAll("\\" + groupingSeparator, "");
        if (!setValue.isEmpty()) {
  ...
  protected void addIsActiveFiltering(CriteriaTransferObject cto) {
    if (isActiveFilter && cto.getCriteriaMap().containsKey(IS_ACTIVE)) {
      ...
      FilterMapping filterMapping = new FilterMapping().withFieldPath(new FieldPath().withTargetProperty("id")).withDirectFilterValues(new EmptyFilterValues()).withRestriction(new Restriction().withPredicateProvider((builder, fieldPathBuilder, root, ceilingEntity, fullPropertyName, explicitPath, directValues) -> {
