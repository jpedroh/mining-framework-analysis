...
@Component(value = "blChildCategoriesCustomPersistenceHandler") public class ChildCategoriesCustomPersistenceHandler extends CustomPersistenceHandlerAdapter {
  ...
  protected void validateSelfLink(final Entity entity) throws ValidationException {
    ...
    if (categoryIdProperty != null && categoryIdProperty.getValue() != null && subCategoryIdProperty != null) {
  ...
  protected void validateDuplicateChild(final Entity entity) throws ValidationException {
    final Property categoryIdProperty = entity.findProperty(CATEGORY_ID);
    final Property subCategoryIdProperty = entity.findProperty(SUB_CATEGORY_ID);
    if (categoryIdProperty != null && categoryIdProperty.getValue() != null && subCategoryIdProperty != null && subCategoryIdProperty.getValue() != null) {
      final String categoryId = categoryIdProperty.getValue();
      final String subCategoryId = subCategoryIdProperty.getValue();
      ...
      final List<Long> childCategoryIds = category.getChildCategoryXrefs().stream().map((categoryXref) -> categoryXref.getSubCategory().getId()).collect(Collectors.toList());
  ...
  protected void validateChildCategories(final Entity entity, final Category category, final Long id, final StringBuilder categoryLinks) throws ValidationException {
    if (category != null) {
      for (CategoryXref categoryXref : category.getChildCategoryXrefs()) {
        ...
        if (subCategory != null) {
          ...
          if (id.equals(subCategory.getId()) || id.equals(originalId)) {
            ...
            final String errorMessage = BLCMessageUtils.getMessage("validateCategoryRecursiveRelationship", newCategoryLinks);
