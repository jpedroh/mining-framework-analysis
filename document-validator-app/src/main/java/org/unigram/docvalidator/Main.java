...
public final class Main {
  public static void main(String[] args) throws DocumentValidatorException {
    ...
    Document document = DocumentGenerator.generate(inputFileNames, conf, inputFormat);
    if (document == null) {
      LOG.error("Failed to create a Document object");
      System.exit(-1);
    }
    DocumentValidator validator = new DocumentValidator.Builder().setResource(conf).setResultDistributor(new DefaultResultDistributor(System.out)).build();
