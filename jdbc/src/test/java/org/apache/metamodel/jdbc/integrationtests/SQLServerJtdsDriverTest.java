...
public class SQLServerJtdsDriverTest extends AbstractJdbIntegrationTest {
  ...
  @Test public void testCreateInsertAndUpdate() throws Exception {
    if (!isConfigured()) {
      return;
    }
  ...
  @Test public void ignoreTestTimestampValueInsertSelect() throws Exception {
    if (!isConfigured()) {
      return;
    }
  ...
  @Test public void testCreateTableInUpdateScript() throws Exception {
    if (!isConfigured()) {
      return;
    }
    ...
    dc.executeUpdate(new UpdateScript() {
      @Override public void run(UpdateCallback callback) {
        Table table = callback.createTable(schema, tableName).withColumn("GroupID").withColumn("RecordID_1").withColumn("RecordID_2").withColumn("SimilarityScore").ofType(ColumnType.VARCHAR).execute();
        assertNotNull(table);
      }
    });
  ...
  @Test public void testCompositePrimaryKeyCreation() throws Exception {
    if (!isConfigured()) {
      return;
    }
  ...
  @Test public void testWorkingWithDates() throws Exception {
    if (!isConfigured()) {
      return;
    }
    final Connection connection = getConnection();
  ...
  @Test public void testAutomaticConversionWhenInsertingString() throws Exception {
    if (!isConfigured()) {
      return;
    }
    final Connection connection = getConnection();
    ...
    dc.executeUpdate(new UpdateScript() {
      @Override public void run(UpdateCallback cb) {
        Table table = cb.createTable(schema, "test_table").withColumn("id").asPrimaryKey().ofType(ColumnType.INTEGER).withColumn("birthdate").ofType(ColumnType.DATE).execute();
    ...
    assertEquals("[Column[name=id,columnNumber=0,type=INTEGER,nullable=false,nativeType=int,columnSize=10], " + "Column[name=birthdate,columnNumber=1,type=VARCHAR,nullable=true,nativeType=date,columnSize=10]]", Arrays.toString(table.getColumns().toArray()));
  ...
  @Test public void testQueryUsingExpressions() throws Exception {
    if (!isConfigured()) {
      return;
    }
    JdbcDataContext strategy = new JdbcDataContext(getConnection(), new TableType[] { TableType.TABLE, TableType.VIEW }, DATABASE_NAME);
    Query q = new Query().from(strategy.getTableByQualifiedLabel("Production.Product")).select("Name").where("COlor IS NOT NULL").setMaxRows(5);
  ...
  @Test public void testGetSchemaNormalTableTypes() throws Exception {
    if (!isConfigured()) {
      return;
    }
    JdbcDataContext dc = new JdbcDataContext(getConnection(), new TableType[] { TableType.TABLE, TableType.VIEW }, DATABASE_NAME);
  ...
  @Test public void testGetSchemaAllTableTypes() throws Exception {
    if (!isConfigured()) {
      return;
    }
    JdbcDataContext strategy = new JdbcDataContext(getConnection(), new TableType[] { TableType.OTHER, TableType.GLOBAL_TEMPORARY }, DATABASE_NAME);
    Schema schema = strategy.getDefaultSchema();
    assertEquals("dbo", schema.getName());
    final List<String> expectedSchemaNames = Arrays.asList("Sales", "HumanResources", "dbo", "Purchasing", "sys", "Production", "INFORMATION_SCHEMA", "Person");
    assertTrue(strategy.getSchemaNames().containsAll(expectedSchemaNames));
  }

  @Test public void testQueryRewriterQuoteAliases() throws Exception {
    if (!isConfigured()) {
      return;
    }
    ...
    Query q = new Query().from(customersTable, "cus-tomers").select(new SelectItem(customersTable.getColumnByName("AccountNumber")).setAlias("c|o|d|e"));
    q.setMaxRows(5);
    ...
    assertEquals("SELECT TOP 5 \"cus-tomers\".\"AccountNumber\" AS \"c|o|d|e\" FROM Sales.\"Customer\" \"cus-tomers\"", queryString);
  ...
  @Test public void testQuotedString() throws Exception {
    if (!isConfigured()) {
      return;
    }
    JdbcDataContext dc = new JdbcDataContext(getConnection(), TableType.DEFAULT_TABLE_TYPES, DATABASE_NAME);
    ...
    assertFalse(ds.next());
    ds.close();
    assertEquals("SELECT Production.\"Product\".\"Name\" FROM Production.\"Product\" WHERE Production.\"Product\".\"Color\" = \'R\'\'ed\'", queryRewriter.rewriteQuery(q));
  }

  @Test public void testMaxAndOffset() throws Exception {
    if (!isConfigured()) {
      return;
    }
    final JdbcDataContext context = getDataContext();
    final List<Row> onlyMaxRows = context.query().from("Person", "Person").select("BusinessEntityID").maxRows(10).execute().toRows();
    assertEquals("Should limit size even without offset or order by", 10, onlyMaxRows.size());
    final List<Row> onlyOffset = context.query().from("Person", "Person").select("BusinessEntityID").orderBy("BusinessEntityID").firstRow(5).execute().toRows();
    ...
    final List<Row> maxRowsAndOffset = context.query().from("Person", "Person").select("BusinessEntityID").maxRows(20).orderBy("BusinessEntityID").firstRow(20).execute().toRows();
