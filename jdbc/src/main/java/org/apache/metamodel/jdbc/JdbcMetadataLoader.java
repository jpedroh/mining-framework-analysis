...
final class JdbcMetadataLoader implements MetadataLoader {
  ...
  @Override public void loadTables(JdbcSchema schema) {
  ...
  @Override public void loadTables(JdbcSchema schema, Connection connection) {
    try {
  ...
  private void loadTables(JdbcSchema schema, DatabaseMetaData metaData, String[] types) {
    try (ResultSet rs = metaData.getTables(getCatalogName(schema), getJdbcSchemaName(schema), null, types)) {
      logger.debug("Querying for table types {}, in catalog: {}, schema: {}", types, _dataContext.getCatalogName(), schema.getName());
      schema.clearTables();
      int tableNumber = -1;
      while (rs.next()) {
        tableNumber++;
  ...
  @Override public void loadIndexes(JdbcTable jdbcTable) {
  ...
  @Override public void loadIndexes(JdbcTable table, Connection connection) {
  ...
  @Override public void loadPrimaryKeys(JdbcTable jdbcTable) {
    final int identity = System.identityHashCode(jdbcTable);
  ...
  @Override public void loadPrimaryKeys(JdbcTable table, Connection connection) {
    final int identity = System.identityHashCode(table);
    if (_loadedPrimaryKeys.contains(identity)) {
      return;
    }
  ...
  @Override public void loadColumns(JdbcTable jdbcTable) {
    final int identity = System.identityHashCode(jdbcTable);
  ...
  /**
     ...
     * @param table
     */
  @Override public void loadColumns(JdbcTable table, Connection connection) {
    final int identity = System.identityHashCode(table);
    if (_loadedColumns.contains(identity)) {
      return;
    }
    ...
    try (ResultSet rs = metaData.getColumns(getCatalogName(schema), getJdbcSchemaName(schema), table.getName(), null)) {
      ...
      while (rs.next()) {
        ...
        if (jdbcNullable == DatabaseMetaData.columnNullable) {
          ...
          if (jdbcNullable == DatabaseMetaData.columnNoNulls) {
            nullable = false;
          } else {
            nullable = null;
          }
      ...
      if (columnsReturned == 0) {
        logger.info("No column metadata records returned for table \'{}\' in schema \'{}\'", table.getName(), schema.getName());
      } else {
        logger.debug("Returned {} column metadata records for table \'{}\' in schema \'{}\'", columnsReturned, table.getName(), schema.getName());
      }
    } catch (SQLException e) {
  ...
  @Override public void loadRelations(JdbcSchema jdbcSchema) {
  ...
  @Override public void loadRelations(JdbcSchema schema, Connection connection) {
  ...
  private void loadRelations(ResultSet rs, Schema schema) throws SQLException {
    ...
    while (rs.next()) {
      ...
      if (pkColumn == null || fkColumn == null) {
        logger.error("Could not find relation columns: pkTableName={},pkColumnName={},fkTableName={},fkColumnName={}", pkTableName, pkColumnName, fkTableName, fkColumnName);
...
<<<<<<< /usr/src/app/output/apache/metamodel/c4acebd5d3911bc208c3fd919a8cf7894b371c00/jdbc/src/main/java/org/apache/metamodel/jdbc/JdbcMetadataLoader.java/left.java
        createRelationship(pkColumn, fkColumn)
=======
        getFkCols().add(fkColumn)
>>>>>>> /usr/src/app/output/apache/metamodel/c4acebd5d3911bc208c3fd919a8cf7894b371c00/jdbc/src/main/java/org/apache/metamodel/jdbc/JdbcMetadataLoader.java/right.java
    ...
    relations.values().stream().flatMap((map) -> map.values().stream()).forEach((ct) -> MutableRelationship.createRelationship(ct.getPkCols().toArray(new Column[0]), ct.getFkCols().toArray(new Column[0])));
  }
