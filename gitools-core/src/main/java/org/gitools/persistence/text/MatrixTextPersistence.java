...
public abstract class MatrixTextPersistence<T extends BaseMatrix> extends BaseMatrixPersistence<T> {
  ...
  public void readMetadata(File file, T matrix, IProgressMonitor monitor) throws PersistenceException {
    ...
    monitor.info(matrix.getColumns().cardinality() + " columns and " + matrix.getRows().cardinality() + " rows");
    monitor.end();
  }
