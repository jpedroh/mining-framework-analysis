 ...
 */
@Component public class ManagementBusInvocationPluginSoapHttp extends IManagementBusInvocationPluginService {
  ...
  @Inject public ManagementBusInvocationPluginSoapHttp(CsarStorageService storage) {
  ...
  @Override public Exchange invoke(Exchange exchange) {
    ...
    if (params instanceof HashMap) {
      ...
      @SuppressWarnings(value = { "unchecked" }) final HashMap<String, String> paramsMap = (HashMap<String, String>) params;
    ...
    LOG.info("Invoking the web service with headers:\n{}\nand content:\n{}", headers.keySet().stream().map((key) -> key + "=" + headers.get(key)).collect(Collectors.joining(", ", "{", "}")), document != null ? MBUtils.docToString(document) : "");
  ...
  @Override public List<String> getSupportedTypes() {
