...
public class RequestProcessor implements Processor {
  ...
  public RequestProcessor(CsarStorageService csarStorage, ContainerEngine containerEngine, IManagementBusService managementBusService, ChoreographyHandler choreoHandler, PlanInstanceRepository planInstanceRepository) {
  ...
  @Override public void process(final Exchange exchange) throws Exception {
    ...
    @SuppressWarnings(value = { "unchecked" }) final List<SoapHeader> soapHeaders = (List<SoapHeader>) exchange.getIn().getHeader(Header.HEADER_LIST);
    Element elementx;
    if (soapHeaders != null) {
    ...
    if (exchange.getIn().getBody() instanceof InvokeOperationAsync) {
      ...
      if (nodeTemplateID != null) {
        ...
        if (Types.openStackTrainNodeType.getLocalPart().equals(nodeTemplate.getType().getLocalPart()) || Types.openStackTrainNodeType_legacy.getLocalPart().equals(nodeTemplate.getType().getLocalPart())) {
          final List<TNodeTemplate> relatedSourceNodeTemplate = ToscaEngine.getRelatedSourceNodeTemplate(serviceTemplate, nodeTemplate, Types.hostedOnRelationType, Types.deployedOnRelationType, Types.dependsOnRelationType);
          for (final TNodeTemplate nodeTemplate1 : relatedSourceNodeTemplate) {
            if (nodeTemplate1.getName().startsWith("Ubuntu")) {
              ...
              for (TNodeTypeImplementation nodeTypeImpl : nodeTypeImpls) {
                if (nodeTypeImpl.getNodeType().equals(nodeType.getQName()) && nodeTypeImpl.getDeploymentArtifacts() != null) {
                  final ResolvedArtifacts resolvedArtifacts = this.containerEngine.resolvedDeploymentArtifactsOfNodeTypeImpl(csar, nodeTypeImpl);
        ...
        final ResolvedArtifacts resolvedArtifacts = this.containerEngine.resolvedDeploymentArtifacts(csar, nodeTemplate);
        resolvedDAs.addAll(resolvedArtifacts.getDeploymentArtifacts());
      }
      ...
      for (final ResolvedDeploymentArtifact resolvedDeploymentArtifact : resolvedDAs) {
        ...
        for (final String s : resolvedDeploymentArtifact.getReferences()) {
          ...
          final String url = serviceInstanceIDUrl.getProtocol() + "://" + serviceInstanceIDUrl.getHost() + ":" + serviceInstanceIDUrl.getPort() + "/csars/" + csarIDString + "/content/";
        ...
        if (exchange.getIn().getBody() instanceof InvokePlan) {
          ...
          if (exchange.getIn().getBody() instanceof NotifyPartners) {
            ...
            if (exchange.getIn().getBody() instanceof NotifyPartner) {
              ...
              final PlanInstance planInstance = planInstanceRepository.findByCorrelationId(notifyPartnerRequest.getPlanCorrelationID());
              ...
              if (exchange.getIn().getBody() instanceof ReceiveNotifyPartner) {
                ...
                final String receivingPartner = getParamByName(receiveNotifyRequest.getParams(), Constants.RECEIVING_PARTNER_PARAM);
                ...
                final Csar choreoCsar = this.choreoHandler.getChoreographyCsar(appChoreoId, this.csarStorage.findAll(), receivingPartner);
                if (choreoCsar == null) {
                  LOG.warn("Received NotifyPartners message but found no participating CSAR, message: \u00a0{}", receiveNotifyRequest);
                  return;
                }
                ...
                final String planCorrelationId = planInstanceRepository.findByChoreographyCorrelationIdAndTemplateId(receiveNotifyRequest.getPlanChorCorrelation(), planID).getCorrelationId();
                ...
                exchange.getIn().setHeader(MBHeader.PLANCHORCORRELATIONID_STRING.toString(), receiveNotifyRequest.getPlanChorCorrelation());
                ...
                if (exchange.getIn().getBody() instanceof ReceiveNotifyPartners) {
                  ...
                  final TServiceTemplate choreoServiceTemplate = choreoCsar.entryServiceTemplate();
                  final QName serviceTemplateID = new QName(choreoServiceTemplate.getTargetNamespace(), choreoServiceTemplate.getId());
                  final QName planID = MBUtils.findPlanByOperation(choreoCsar, OpenTOSCA_LifecycleInterface, OpenTOSCA_BuildPlanOperation);
                  exchange.getIn().setBody(createRequestBody(choreoCsar.id().csarName(), serviceTemplateID, receiveNotifyRequest.getPlanCorrelationID()));
                  exchange.getIn().setHeader(MBHeader.PLANCHORCORRELATIONID_STRING.toString(), receiveNotifyRequest.getPlanChorCorrelation());
                  exchange.getIn().setHeader(MBHeader.SERVICETEMPLATEID_QNAME.toString(), choreoServiceTemplate.getId());
                  exchange.getIn().setHeader(MBHeader.CSARID.toString(), choreoCsar.id().csarName());
                  exchange.getIn().setHeader(MBHeader.PLANID_QNAME.toString(), planID);
                  ...
                  exchange.getIn().setHeader(CxfConstants.OPERATION_NAME, "invokePlan");
                  final String partner = receiveNotifyRequest.getParams().getParam().stream().filter((param) -> param.getKey().equals("SendingPartner")).findFirst().map(ParamsMapItemType::getValue).orElse(null);
  ...
  private Map<String, String> createRequestBody(final String csarID, final QName serviceTemplateID, final String planCorrelationID) {
    String str = Settings.CONTAINER_INSTANCEDATA_API.replace("{csarid}", csarID);
    str = str.replace("{servicetemplateid}", URLEncoder.encode(URLEncoder.encode(serviceTemplateID.getLocalPart(), StandardCharsets.UTF_8), StandardCharsets.UTF_8));
