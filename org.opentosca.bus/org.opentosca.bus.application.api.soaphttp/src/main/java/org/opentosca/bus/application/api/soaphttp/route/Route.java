 ...
 */
@Component @Named(value = "application-bus-soaphttp-route") public class Route extends RouteBuilder {
  ...
  @Override public void configure() throws Exception {
    ...
    final String SOAP_ENDPOINT = "cxf:" + ENDPOINT + "?wsdlURL=" + wsdlURL.toString() + "&serviceName={http://opentosca.org/appinvoker/}AppInvokerSoapWebServiceService&portName=" + PORT + "&dataFormat=PAYLOAD&loggingFeatureEnabled=true";
    ...
    from(SOAP_ENDPOINT).unmarshal(jaxb).process(requestProcessor).to("direct-vm:org.opentosca.bus.application.service").to("direct:handleResponse");
    from("direct:handleException").throwException(new ApplicationBusInternalException("It seems like the Application Bus is not running.")).to("direct:handleResponse");
