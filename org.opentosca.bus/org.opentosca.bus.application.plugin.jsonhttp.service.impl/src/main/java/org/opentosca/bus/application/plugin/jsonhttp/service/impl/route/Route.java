...
public class Route extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:polling").setHeader(Exchange.HTTP_METHOD, constant("GET")).to(DUMMY_ENDPOINT).convertBodyTo(java.lang.String.class).choice().when(PENDING).delay(5000).to("direct:polling").endChoice().when(RESULT_RECEIVED).process(responseProcessor).endChoice().otherwise().to("direct:throwException");
        from("direct:throwException").process(( exchange) -> exchange.getIn().setBody(new ApplicationBusExternalException(exchange.getIn().getBody(.class))));
    }
}