 ...
 */
@Service public class RequestReceiver {
  ...
  @Inject public RequestReceiver(CollaborationContext context, DeploymentDistributionDecisionMaker decisionMaker, ICoreEndpointService endpointService, PluginRegistry pluginRegistry) {
  ...
  public void invokeInstanceDataMatching(final Exchange exchange) {
    ...
    if (deploymentLocation != null) {
      ...
      LOG.debug("Broker: {} Topic: {} Correlation: {}", headers.get(MBHeader.MQTTBROKERHOSTNAME_STRING.toString()), headers.get(MBHeader.MQTTTOPIC_STRING.toString()), headers.get(MBHeader.CORRELATIONID_STRING.toString()));
  ...
  public void invokeIADeployment(Exchange exchange) {
    ...
    synchronized (ManagementBusServiceImpl.getLockForString(identifier)) {
      ...
      final List<Endpoint> endpoints = endpointService.getEndpointsForNTImplAndIAName(triggeringContainer, deploymentLocation, typeImplementationID, implementationArtifactName);
      if (endpoints != null && endpoints.size() > 0) {
        endpointURI = endpoints.get(0).getUri();
        LOG.warn("IA is already deployed. Storing only one endpoint at the remote side. Endpoint URI: {}", endpointURI);
      } else {
        ...
        if (deploymentPlugin != null) {
          ...
          final Endpoint endpoint = new Endpoint(endpointURI, triggeringContainer, deploymentLocation, csarID, serviceTemplateInstanceID, new HashMap<>(), portType, typeImplementationID, implementationArtifactName, null);
  ...
  public void invokeIAUndeployment(Exchange exchange) {
    ...
    final String artifactType = message.getHeader(MBHeader.ARTIFACTTYPEID_STRING.toString(), String.class);
    final String deploymentLocation = Settings.OPENTOSCA_CONTAINER_HOSTNAME;
    LOG.debug("Undeployment of IA: Triggering Container: {}, Deployment location: {}, NodeTypeImplementation ID: {}, IA name: {}, Type: {}", triggeringContainer, deploymentLocation, typeImplementationID, implementationArtifactName, artifactType);
    synchronized (ManagementBusServiceImpl.getLockForString(identifier)) {
  ...
  public void invokeIAOperation(final Exchange exchange) {
    ...
    if (responseBody instanceof HashMap) {
      ...
      @SuppressWarnings(value = { "unchecked" }) final HashMap<String, String> paramsMap = (HashMap<String, String>) responseBody;
  ...
  private boolean isDestinationLocal(final Message message) {
    final String deploymentLocation = message.getHeader(MBHeader.DEPLOYMENTLOCATION_STRING.toString(), String.class);
  ...
  private String getUniqueSynchronizationString(final Message message) {
    ...
    return ManagementBusServiceImpl.getUniqueSynchronizationString(triggeringContainer, deploymentLocation, typeImplementationID, implementationArtifactName, serviceInstanceId);
  }
}