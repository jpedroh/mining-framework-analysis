 ...
 */
@Service @Named(value = "managementBusService") public class ManagementBusServiceImpl implements IManagementBusService {
  ...
  /**
     ...
     * missing
     */
  public static String getUniqueSynchronizationString(final String triggeringContainer, final String deploymentLocation, final QName typeImpl, final String iaName, final String serviceInstanceId) {
    if (Objects.isNull(triggeringContainer) || Objects.isNull(deploymentLocation) || Objects.isNull(typeImpl) || Objects.isNull(iaName) || Objects.isNull(serviceInstanceId)) {
      return null;
    }
  ...
  @Override public void invokeIA(final Exchange exchange) {
    ...
    if (!serviceTemplateInstanceID.equals(Long.MIN_VALUE)) {
      final IAInvocationArguments arguments = new IAInvocationArguments(csarID, serviceInstanceID, serviceTemplateID, serviceTemplateInstanceID, nodeTemplateID, relationship, neededInterface, neededOperation);
    ...
    if (Objects.nonNull(correlationID)) {
      ...
      if (Objects.nonNull(plan)) {
        ...
        event.setMessage("Finished execution of IA for NodeTemplate \'" + nodeTemplateID + "\' interface \'" + neededInterface + "\' and operation \'" + neededOperation + "\' after " + duration + "ms");
  ...
  private PlanInstanceEvent internalInvokeIA(IAInvocationArguments arguments, Exchange exchange) {
    ...
    if (Objects.nonNull(arguments.nodeTemplateId)) {
      ...
      if (Objects.nonNull(arguments.relationshipTemplateId)) {
    ...
    if (message.getBody() instanceof HashMap) {
      @SuppressWarnings(value = { "unchecked" }) Map<String, String> inputParams = (Map<String, String>) message.getBody();
  ...
  /**
     ...
     */
  private boolean invokeIAOperation(Exchange exchange, final Csar csar, final Long serviceTemplateInstanceID, final TEntityType type, final NodeTemplateInstance nodeTemplateInstance, final TEntityTypeImplementation typeImplementation, final TImplementationArtifact ia, final String neededInterface, final String neededOperation) {
    ...
    final String invocationType = this.pluginHandler.getSupportedInvocationType(artifactTypeQName, artifactTemplate);
    if (Objects.isNull(invocationType)) {
      LOG.debug("No invocation plug-in found which supports the invocation of ArtifactType {} and ArtifactTemplate {}", artifactTypeQName, artifactTemplate.getId());
      return false;
    }
    ...
    synchronized (getLockForString(identifier)) {
      ...
      final List<Endpoint> endpoints = this.endpointService.getEndpointsForNTImplAndIAName(triggeringContainer, deploymentLocation, typeImplementation.getQName(), ia.getName());
      if (Objects.nonNull(endpoints) && !endpoints.isEmpty()) {
        ...
        final Optional<Endpoint> currentEndpoint = endpoints.stream().filter((endpoint) -> endpoint.getServiceTemplateInstanceID().equals(serviceTemplateInstanceID)).findFirst();
        if (!currentEndpoint.isPresent()) {
          final Endpoint endpoint = new Endpoint(endpointURI, triggeringContainer, deploymentLocation, csar.id(), serviceTemplateInstanceID, new HashMap<>(), portType, typeImplementation.getQName(), ia.getName(), null);
        ...
        handleResponse(this.pluginHandler.callMatchingInvocationPlugin(exchange, invocationType, deploymentLocation));
        return true;
      }
      ...
      final IManagementBusDeploymentPluginService deploymentPlugin = this.pluginRegistry.getDeploymentPluginServices().get(deploymentType);
      ...
      final List<TArtifactReference> artifacts = Optional.ofNullable(artifactTemplate.getArtifactReferences()).orElse(Collections.emptyList());
      ...
      for (final TArtifactReference artifact : artifacts) {
        ...
        final String absoluteArtifactReference = Settings.OPENTOSCA_CONTAINER_CONTENT_API_ARTIFACTREFERENCE.replace("{csarid}", csar.id().csarName()).replace("{artifactreference}", artifact.getReference().trim().replaceAll(" ", "%20"));
      ...
      if (endpointURI.toString().contains(Constants.PLACEHOLDER_START) && endpointURI.toString().contains(Constants.PLACEHOLDER_END)) {
        ...
        LOG.debug("URI = {}, portType = {}, triggeringContainer = {}, managingContainer = {}, csar = {}, serviceTemplateInstanceID = {}, planId = {}, nodeTypeImplementation = {}, iaName = {}, metadata = {}", endpointURI, portType, triggeringContainer, deploymentLocation, csar.id(), serviceTemplateInstanceID, null, typeImplementation.getQName(), ia.getName(), new HashMap<>());
        this.endpointService.storeEndpoint(endpoint);
      }
  ...
  @Override public void invokePlan(Exchange exchange) {
  ...
  @Override public void notifyPartner(final Exchange exchange) {
    final Message message = exchange.getIn();
    final String chorCorrelationID = message.getHeader(MBHeader.PLANCHORCORRELATIONID_STRING.toString(), String.class);
    final CsarId csarID = message.getHeader(MBHeader.CSARID.toString(), CsarId.class);
    final QName serviceTemplateID = message.getHeader(MBHeader.SERVICETEMPLATEID_QNAME.toString(), QName.class);
    ...
    if (!(exchange.getIn().getBody() instanceof HashMap)) {
      LOG.error("Message to notify partner with Correlation ID {}, CSARID {} and ServiceTemplate ID {} contains no parameters. Aborting!", chorCorrelationID, csarID, serviceTemplateID);
      return;
    }
    ...
    @SuppressWarnings(value = { "unchecked" }) final HashMap<String, String> params = (HashMap<String, String>) exchange.getIn().getBody();
    ...
    LOG.debug("Notifying partner {} for connectsTo with ID {} for choreography with correlation ID {}, CsarID {}, and ServiceTemplateID {}", receivingPartner, connectingRelationshipTemplate, chorCorrelationID, csarID, serviceTemplateID);
    ...
    final Optional<TTag> endpointTagOptional = this.choreographyHandler.getPartnerEndpoints(serviceTemplate).stream().filter((tag) -> tag.getName().equals(receivingPartner)).findFirst();
    ...
    final Document inputDoc = MBUtils.mapToDoc(Constants.BUS_WSDL_NAMESPACE, Constants.RECEIVE_NOTIFY_PARTNER_OPERATION, inputMap);
  ...
  @Override public void notifyPartners(final Exchange exchange) {
    final Message message = exchange.getIn();
    ...
    LOG.debug("Notifying partners to start their plans for choreography with correlation ID {}, CsarID {}, and ServiceTemplateID {}", correlationID, csarID, serviceTemplateID);
    final TServiceTemplate serviceTemplate = this.storage.findById(csarID).entryServiceTemplate();
    if (Objects.isNull(serviceTemplate)) {
      LOG.error("Unable to retrieve ServiceTemplate for the notification request.");
      return;
    ...
    for (final TTag endpointTag : partnerTags) {
      ...
      final Document inputDoc = MBUtils.mapToDoc(Constants.BUS_WSDL_NAMESPACE, Constants.RECEIVE_NOTIFY_PARTNERS_OPERATION, input);
      final Element root = inputDoc.getDocumentElement();
      final Element paramsWrapper = inputDoc.createElement(Constants.PARAMS_PARAM);
      root.appendChild(paramsWrapper);
      for (final Entry<String, String> entry : params.entrySet()) {
  ...
  private void internalInvokePlan(PlanInvocationArguments arguments, Exchange exchange) {
    ...
    synchronized (this) {
      if (!isReceiveNotify && arguments.chorCorrelationId != null && planInstanceRepository.findByChoreographyCorrelationIdAndTemplateId(arguments.chorCorrelationId, arguments.planId) != null) {
        LOG.warn("Skipping the plan invocation of choreography build plan with choreography id {}", arguments.chorCorrelationId);
        return;
      }
      if (isReceiveNotify) {
        plan = planInstanceRepository.findByChoreographyCorrelationIdAndTemplateId(arguments.chorCorrelationId, arguments.planId);
      } else {
        try {
          plan = planInstanceHandler.createPlanInstance(arguments.csar, arguments.serviceTemplateId, arguments.serviceTemplateInstanceId, arguments.planId, arguments.operationName, arguments.correlationId, arguments.chorCorrelationId, arguments.chorPartners, exchange.getIn().getBody());
    ...
    final List<Endpoint> endpoints = this.endpointService.getEndpointsForPlanId(Settings.OPENTOSCA_CONTAINER_HOSTNAME, arguments.csar.id(), plan.getTemplateId());
    Endpoint endpoint = null;
    if (Objects.isNull(callbackInvocation) || !callbackInvocation) {
      endpoint = endpoints.stream().filter((e) -> e.getPortType() == null || !e.getPortType().equals(Constants.CALLBACK_PORT_TYPE)).findFirst().orElse(null);
    } else {
      LOG.debug("Invokation using callback.");
      endpoint = endpoints.stream().filter((e) -> e.getPortType().equals(Constants.CALLBACK_PORT_TYPE)).findFirst().orElse(null);
    }
    if (endpoint != null) {
      ...
      if (plan.getLanguage().equals(PlanLanguage.BPMN)) {
        exchange = this.pluginHandler.callMatchingInvocationPlugin(exchange, "REST", Settings.OPENTOSCA_CONTAINER_HOSTNAME);
      } else {
        exchange = this.pluginHandler.callMatchingInvocationPlugin(exchange, "SOAP/HTTP", Settings.OPENTOSCA_CONTAINER_HOSTNAME);
      }
      ...
      LOG.warn("No endpoint found for specified plan: {} of csar: {}. Invocation aborted!", plan.getTemplateId(), arguments.csar.id());
    }
    ...
    event.setEndTimestamp(new Date());
    final long duration = event.getEndTimestamp().getTime() - event.getStartTimestamp().getTime();
    event.setMessage("Finished plan execution with correlation id " + arguments.correlationId + " after " + duration + "ms");
    ...
    final long currentPlanWCET = Long.valueOf(currentPlan.getOtherAttributes().getOrDefault(new QName("http://opentosca.org", "WCET"), String.valueOf(0)));
  ...
  private void deleteEndpointsForServiceInstance(final CsarId csarID, final ServiceTemplateInstance serviceInstance) {
    ...
    final Csar csar = this.storage.findById(csarID);
    final List<Endpoint> serviceEndpoints = this.endpointService.getEndpointsForSTID(Settings.OPENTOSCA_CONTAINER_HOSTNAME, instanceID);
    ...
    for (final Endpoint serviceEndpoint : serviceEndpoints) {
      ...
      LOG.debug("Deleting endpoint: Triggering Container: {}; " + "Managing Container: {}; NodeTypeImplementation: {}; IA name: {}", triggeringContainer, deploymentLocation, typeImpl, iaName);
      final String identifier = getUniqueSynchronizationString(triggeringContainer, deploymentLocation, typeImpl, iaName, instanceID.toString());
      synchronized (getLockForString(identifier)) {
        final int count = this.endpointService.getEndpointsForNTImplAndIAName(triggeringContainer, deploymentLocation, typeImpl, iaName).size();
        if (count == 1) {
          LOG.debug("Undeploying corresponding IA...");
          final TImplementationArtifact ia;
          try {
            final TEntityTypeImplementation typeImplementation = ToscaEngine.resolveTypeImplementation(csar, typeImpl);
  ...
  private URI replacePlaceholderWithInstanceData(URI endpoint, final NodeTemplateInstance nodeTemplateInstance) {
    ...
    final String placeholder = endpoint.toString().substring(endpoint.toString().lastIndexOf(Constants.PLACEHOLDER_START), endpoint.toString().lastIndexOf(Constants.PLACEHOLDER_END) + Constants.PLACEHOLDER_END.length());
    LOG.debug("Placeholder: {} detected in Endpoint: {}", placeholder, endpoint);
    final String[] placeholderProperties = placeholder.replace(Constants.PLACEHOLDER_START, "").replace(Constants.PLACEHOLDER_END, "").split("_");
  ...
  @Override public synchronized void addPartnerToReadyList(final String correlationID, final String partnerID) {
    activePartners.putIfAbsent(correlationID, new LinkedList<String>());
    activePartners.get(correlationID).add(partnerID);
  }

  @Override public synchronized boolean isPartnerAvailable(final String correlationID, final String partnerID) {
  ...
  private static class PlanInvocationArguments {
    ...
    public PlanInvocationArguments(Csar csar, QName serviceTemplateID, Long serviceTemplateInstanceID, QName planID, String operationName, String correlationID, String chorCorrelationId, String chorPartners) {
      this.csar = csar;
