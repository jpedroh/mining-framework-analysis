...
public class SSStrU {
  ...
  public static String addTrailingSlash(final String string) {
    if (string == null || string.lastIndexOf(slash) == string.length() - 1) {
      return string;
    }
    return string + slash;
  }
  ...
  public static String removeTrailingSlash(final String string) {
    if (isEmpty(string) || string.lastIndexOf(slash) != string.length() - 1) {
      return string;
    }
  ...
  public static String removeTrailingString(final String string, final String trail) {
    if (isEmpty(string) || string.lastIndexOf(trail) != string.length() - trail.length()) {
      return string;
    }
  ...
  public static String addAtBegin(final String string, final String add) {
    if (SSObjU.isNull(string) || isEmpty(add) || string.startsWith(add)) {
      return string;
    }
    return add + string;
  }
  ...
  public static String addAtEnd(final String string, final String add) {
    if (SSObjU.isNull(string) || isEmpty(add) || string.substring(string.length() - 1, string.length()).equals(add)) {
      return string;
    }
    return string + add;
  }
  ...
  public static String removeStringFromBegin(final String string, final String begin) {
    if (isEmpty(string) || isEmpty(begin) || !string.startsWith(begin)) {
      return string;
    }
  ...
  public static boolean hasLength(final List<String> list, final int desiredLength) throws Exception {
    if (SSObjU.isNull(list) || list.size() != desiredLength) {
      return false;
    }
    return true;
  }

  public static boolean hasNotLengthAtLeast(final List<String> list, final int desiredLength) throws Exception {
    return !hasLengthAtLeast(list, desiredLength);
  }

  public static boolean hasLengthAtLeast(final List<String> list, final int desiredLength) throws Exception {
    if (SSObjU.isNull(list) || list.size() < desiredLength) {
      return false;
    }
    return true;
  ...
  public static String getHashFromString(final String string) throws NoSuchAlgorithmException {
  ...
  public static boolean containsNot(final List<String> list, final String value) {
    return !contains(list, value);
  }

  public static boolean containsNot(final List<Integer> list, final Integer integer) {
    return !contains(list, integer);
  }

  public static boolean containsNot(final String string1, final String string2) {
    return !contains(string1, string2);
  }

  public static boolean contains(final List<String> list, final String value) {
    if (SSObjU.isNull(list) || SSObjU.isNull(value)) {
      return false;
    }
    return list.contains(value);
  }

  public static boolean contains(final String string1, final String string2) {
    if (SSObjU.isNull(string1) || SSObjU.isNull(string2)) {
      return false;
    }
    return string1.contains(string2);
  }

  public static boolean contains(final List<Integer> list, final Integer integer) {
    if (SSObjU.isNull(list) || SSObjU.isNull(integer)) {
      return false;
    }
  ...
  public static String surroundWithDoubleQuotes(final String string) {
    ...
    if (string.length() > 1 && string.substring(0, 1).equals("\"") && string.substring(string.length() - 1, string.length()).equals("\"")) {
      return string;
    }
    return "\"" + string + "\"";
  }
  ...
  public static List<String> distinct(final List<String> strings) {
    final List<String> result = new ArrayList<String>();
    if (strings == null) {
      return result;
    }
    for (String string : strings) {
      if (!isEmpty(string) && !contains(result, string)) {
        result.add(string);
      }
    }
    return result;
  ...
  public static List<String> splitDistinct(final String toSplit, final String splitter) throws Exception {
    return new ArrayList<String>(new LinkedHashSet<String>(split(toSplit, splitter)));
  }

  public static List<String> split(final String toSplit, final String splitter) throws Exception {
    if (isEmpty(splitter)) {
  ...
  public static boolean notEquals(final String string1, final String string2) {
  ...
  public static String subString(final String string, final int from, final int to) {
    if (isEmpty(string) || SSNumberU.isLess(from, 0) || SSNumberU.isLess(string.length(), to)) {
      return null;
    }
  ...
  public static String filterNumbers(final String string) {
    if (SSObjU.isNull(string)) {
      return null;
    }
  ...
  public static List<String> removeAll(final List<String> list, final List<String> toRemove) {
  ...
  public static List<String> retainAll(final List<String> list, final List<String> toRetain) {
    if (list == null || toRetain == null) {
      return new ArrayList<String>();
    }
