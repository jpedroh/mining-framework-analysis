...
<<<<<<< /usr/src/app/output/mpusher/mpush/684f038dad1d5b95b6f3f60674e12e4ef0b47922/mpush-tools/src/main/java/com/mpush/tools/config/CC.java/left.java
      ws_server_port = cfg.getInt("ws-server-port")
=======
      gateway_client_num = cfg.getInt("gateway-client-num")
>>>>>>> /usr/src/app/output/mpusher/mpush/684f038dad1d5b95b6f3f60674e12e4ef0b47922/mpush-tools/src/main/java/com/mpush/tools/config/CC.java/right.java
    ...
    interface redis {
      ...
      List<RedisNode> nodes = cfg.getList("nodes").stream().map((v) -> RedisNode.from(v.unwrapped().toString())).collect(toCollection(ArrayList::new));

      static boolean isCluster() {
        return "cluster".equals(clusterModel);
      }

      static <T extends java.lang.Object> T getPoolConfig(Class<T> clazz) {
    ...
    interface http {
      ...
      static Map<String, List<DnsMapping>> loadMapping() {
        ...
        cfg.getObject("dns-mapping").forEach((s, v) -> map.put(s, ConfigList.class.cast(v).stream().map((cv) -> DnsMapping.parse((String) cv.unwrapped())).collect(toCollection(ArrayList::new))));
        return map;
      }
    }
