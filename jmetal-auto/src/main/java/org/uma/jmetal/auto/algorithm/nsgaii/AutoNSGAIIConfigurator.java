...
public class AutoNSGAIIConfigurator {
  ...
  @Option(names = { "--algorithmResult" }, description = "Algorithm result - population vs archive (default: ${DEFAULT-VALUE})") private AlgorithmResult algorithmResult = AlgorithmResult.population;

  @Option(names = { "--populationSize" }, description = "Population Size (default: ${DEFAULT-VALUE})") private int populationSize = 100;

  @Option(names = { "--populationSizeWithArchive" }, description = "Population Size when an archive is used (default: ${DEFAULT-VALUE})") private int populationSizeWithArchive = 40;

  @Option(names = { "-p", "--problemName" }, required = true, description = "problem name})") private String problemName;

  @Option(names = { "-rf", "--referenceFront" }, required = true, description = "reference front") private String referenceParetoFront;
  ...
  @Option(names = { "--crossover" }, required = true, description = "Crossover: ${COMPLETION-CANDIDATES}") private CrossoverType crossoverType;

  @Option(names = { "--crossoverProbability" }, description = "Crossover probability (default: ${DEFAULT-VALUE})") private double crossoverProbability = 0.9;

  @Option(names = { "--sbxCrossoverDistributionIndex" }, description = "SBX crossover distribution index (default: ${DEFAULT-VALUE})") private double sbxCrossoverDistributionIndex = 0.20;

  @Option(names = { "--blxAlphaCrossoverAlphaValue" }, description = "BLX-alpha crossover Alpha value (default: ${DEFAULT-VALUE})") private double blxAlphaCrossoverAlphaValue = 0.5;

  @Option(names = { "--crossoverRepairStrategy" }, description = "Crossover repair strategy (default: ${DEFAULT-VALUE})") private RepairStrategyType crossoverRepairStrategy = RepairStrategyType.random;

  CrossoverOperator<DoubleSolution> crossover;

  @Option(names = { "--mutation" }, required = true, description = "Mutation: ${COMPLETION-CANDIDATES}") private MutationType mutationType;

  @Option(names = { "--mutationProbability" }, description = "Mutation probability (default: ${DEFAULT-VALUE})") private double mutationProbability = 0.01;

  @Option(names = { "--polynomialMutationDistributionIndex" }, description = "Polynomial Mutation Distribution Index (default: ${DEFAULT-VALUE})") private double polynomialMutationDistributionIndex = 0.20;

  @Option(names = { "--uniformMutationPerturbation" }, description = "Uniform Mutation Perturbation (default: ${DEFAULT-VALUE})") private double uniformMutationPerturbation = 0.5;

  @Option(names = { "--mutationRepairStrategy" }, description = "Mutation repair strategy (default: ${DEFAULT-VALUE})") private RepairStrategyType mutationRepairStrategy = RepairStrategyType.random;

  MutationOperator<DoubleSolution> mutation;

  @Option(names = { "--offspringPopulationSize" }, description = "offspring population size (default: ${DEFAULT-VALUE})") private int offspringPopulationSize = populationSize;

  @Option(names = { "--createInitialSolutions" }, required = true, description = "Create initial population: ${COMPLETION-CANDIDATES}") private CreateInitialSolutionsStrategyType createInitialSolutionsType = CreateInitialSolutionsStrategyType.random;

  InitialSolutionsCreation<DoubleSolution> createInitialSolutions;

  @Option(names = { "--variation" }, required = true, description = "Variation: ${COMPLETION-CANDIDATES}") private VariationType variationType;

  @Option(names = { "--differentialEvolutionFValue" }, description = "Differential evolution F  value (default: ${DEFAULT-VALUE})") private double f = 0.5;

  @Option(names = { "--differentialEvolutionCRValue" }, description = "Differential evolution CR  value (default: ${DEFAULT-VALUE})") private double cr = 0.5;

  Variation<DoubleSolution> variation;

  @Option(names = { "--selection" }, required = true, description = "Selection: ${COMPLETION-CANDIDATES}") private SelectionType selectionType;

  MatingPoolSelection<DoubleSolution> selection;

  @Option(names = { "--selectionTournamentSize" }, description = "Selection: number of selected population") private int selectionTournamentSize = 2;
  ...
  public EvolutionaryAlgorithm<DoubleSolution> configureAndGetAlgorithm() {
    ...
    class NSGAII extends EvolutionaryAlgorithm<DoubleSolution> {
      ...
      public NSGAII(String name, Evaluation<DoubleSolution> evaluation, InitialSolutionsCreation<DoubleSolution> createInitialSolutionList, Termination termination, MatingPoolSelection<DoubleSolution> selection, Variation<DoubleSolution> variation, Replacement<DoubleSolution> replacement, ExternalArchiveObserver<DoubleSolution> archiveObserver) {
        super(name, evaluation, createInitialSolutionList, termination, selection, variation, replacement);
    ...
    NSGAII nsgaii = new NSGAII("NSGAII", evaluation, createInitialSolutions, termination, selection, variation, replacement, boundedArchiveObserver);
    return nsgaii;
  }
  ...
  MatingPoolSelection<DoubleSolution> getSelection() {
    switch (selectionType) {
      ...
      case tournament:
      return new NaryTournamentMatingPoolSelection<>(selectionTournamentSize, variation.getMatingPoolSize(), rankingAndCrowdingComparator);
  ...
  CrossoverOperator<DoubleSolution> getCrossover() {
    switch (crossoverType) {
      case SBX:
      return new SBXCrossover(crossoverProbability, sbxCrossoverDistributionIndex, getRepairDoubleSolutionStrategy(crossoverRepairStrategy));
      case BLX_ALPHA:
      return new BLXAlphaCrossover(crossoverProbability, blxAlphaCrossoverAlphaValue, getRepairDoubleSolutionStrategy(crossoverRepairStrategy));
      default:
  ...
  MutationOperator<DoubleSolution> getMutation() {
    switch (mutationType) {
      case polynomial:
      return new PolynomialMutation(mutationProbability, polynomialMutationDistributionIndex, getRepairDoubleSolutionStrategy(mutationRepairStrategy));
      case uniform:
      return new UniformMutation(mutationProbability, uniformMutationPerturbation, getRepairDoubleSolutionStrategy(mutationRepairStrategy));
      case random:
  ...
  RepairDoubleSolution getRepairDoubleSolutionStrategy(RepairStrategyType repairStrategyType) {
    switch (repairStrategyType) {
      ...
      default:
      throw new RuntimeException(repairStrategyType + " is not a valid repair double solution strategy");
    }
  }
