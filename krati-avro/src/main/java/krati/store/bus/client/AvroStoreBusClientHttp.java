 ...
 * @since 10/04, 2011
 */
public class AvroStoreBusClientHttp<K extends java.lang.Object> extends StoreBusClientHttp<K, GenericRecord> {
  ...
  private class LazyAvroGenericRecordSerializer implements Serializer<GenericRecord> {
    ...
    @Override public GenericRecord deserialize(byte[] bytes) throws SerializationException {
      checkSchema();
      return _delegate.deserialize(bytes);
    }

    @Override public byte[] serialize(GenericRecord record) throws SerializationException {
      checkSchema();
