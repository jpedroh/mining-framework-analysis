...
public interface ConfigContext extends MantaMBeanable {
  ...
  /** {@inheritDoc} */
  @Override default DynamicMBean toMBean() {
  ...
  static void validate(final ConfigContext config) {
    ...
    if (StringUtils.isBlank(config.getMantaURL())) {
      ...
      try {
        ...
        final String msg = String.format("%s - invalid Manta URL: %s", e.getMessage(), config.getMantaURL());
    ...
    if (config.getMetricReporterMode() != null) {
      if (MetricReporterMode.SLF4J.equals(config.getMetricReporterMode())) {
        if (config.getMetricReporterOutputInterval() == null) {
          ...
          if (config.getMetricReporterOutputInterval() < 1) {
    ...
    if (!failureMessages.isEmpty()) {
      ...
      e.setContextValue(MapConfigContext.MANTA_CLIENT_ENCRYPTION_ENABLED_KEY, config.isClientEncryptionEnabled());
      e.setContextValue(MapConfigContext.MANTA_CONTENT_TYPE_DETECTION_ENABLED_KEY, config.isContentTypeDetectionEnabled());
      if (BooleanUtils.isTrue(config.isClientEncryptionEnabled())) {
        e.setContextValue(MapConfigContext.MANTA_ENCRYPTION_KEY_ID_KEY, config.getEncryptionKeyId());
        e.setContextValue(MapConfigContext.MANTA_PERMIT_UNENCRYPTED_DOWNLOADS_KEY, config.permitUnencryptedDownloads());
        e.setContextValue(MapConfigContext.MANTA_ENCRYPTION_ALGORITHM_KEY, config.getEncryptionAlgorithm());
        e.setContextValue(MapConfigContext.MANTA_ENCRYPTION_PRIVATE_KEY_PATH_KEY, config.getEncryptionPrivateKeyPath());
        e.setContextValue(MapConfigContext.MANTA_ENCRYPTION_PRIVATE_KEY_BYTES_KEY + "_size", ArrayUtils.getLength(config.getEncryptionPrivateKeyBytes()));
      }
      throw e;
    }
  }
