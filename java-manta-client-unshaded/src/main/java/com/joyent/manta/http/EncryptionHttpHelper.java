...
public class EncryptionHttpHelper extends StandardHttpHelper {
  ...
  /**
     ...
     */
  private static final String CIPHERTEXT_CONTENT_TYPE = ContentType.APPLICATION_OCTET_STREAM.toString();
  ...
  /**
     ...
     */
  @Deprecated public EncryptionHttpHelper(final MantaConnectionContext connectionContext, final MantaConnectionFactory connectionFactory, final ConfigContext config) {
  ...
  /**
     ...
     * @param config configuration context object
     */
  EncryptionHttpHelper(final MantaConnectionContext connectionContext, final ConfigContext config) {
  ...
  /**
     ...
     * @param config configuration context object
     */
  public EncryptionHttpHelper(final MantaConnectionContext connectionContext, final MantaHttpRequestFactory requestFactory, final ConfigContext config) {
    super(connectionContext, requestFactory, ObjectUtils.firstNonNull(config.verifyUploads(), DefaultsConfigContext.DEFAULT_VERIFY_UPLOADS), ObjectUtils.firstNonNull(config.downloadContinuations(), DefaultsConfigContext.DEFAULT_DOWNLOAD_CONTINUATIONS));
    this.encryptionKeyId = ObjectUtils.firstNonNull(config.getEncryptionKeyId(), "unknown-key");
    this.permitUnencryptedDownloads = ObjectUtils.firstNonNull(config.permitUnencryptedDownloads(), DefaultsConfigContext.DEFAULT_PERMIT_UNENCRYPTED_DOWNLOADS);
    this.encryptionAuthenticationMode = ObjectUtils.firstNonNull(config.getEncryptionAuthenticationMode(), EncryptionAuthenticationMode.DEFAULT_MODE);
    this.cipherDetails = ObjectUtils.firstNonNull(SupportedCiphersLookupMap.INSTANCE.getWithCaseInsensitiveKey(config.getEncryptionAlgorithm()), DefaultsConfigContext.DEFAULT_CIPHER);
  ...
  @Override public HttpResponse httpHead(final String path) throws IOException {
    HttpResponse response = super.httpHead(path);
    attachMetadata(response);
    return response;
  }

  @Override public HttpResponse httpGet(final String path) throws IOException {
    HttpResponse response = super.httpGet(path);
    attachMetadata(response);
    return response;
  }

  @Override public MantaObjectResponse httpPut(final String path, final MantaHttpHeaders headers, final HttpEntity originalEntity, final MantaMetadata originalMetadata) throws IOException {
    ...
    EncryptingEntity encryptingEntity = new EncryptingEntity(secretKey, cipherDetails, originalEntity);
  ...
  @Override @SuppressWarnings(value = { "MagicNumber" }) public MantaObjectInputStream httpRequestAsInputStream(final HttpUriRequest request, final MantaHttpHeaders requestHeaders) throws IOException {
    ...
    if (hasRangeRequest) {
      PlaintextByteRangePosition rangeProperties = calculateSkipBytesAndPlaintextLength(request, requestHeaders);
    ...
    @SuppressWarnings(value = { "unchecked" }) final HttpResponse response = (HttpResponse) rawStream.getHttpResponse();
    ...
    if (metadataCiphertextBase64 != null) {
      Map<String, String> encryptedMetadata = buildEncryptedMetadata(encryptionType, metadataIvBase64, metadataCiphertextBase64, hmacId, metadataHmacBase64, request, response);
    ...
    if (hasRangeRequest) {
      ...
      return new MantaEncryptedObjectInputStream(rawStream, this.cipherDetails, secretKey, false, initialSkipBytes, plaintextRangeLength, unboundedEnd);
    } else {
      final boolean authenticateCiphertext = !encryptionAuthenticationMode.equals(EncryptionAuthenticationMode.VerificationDisabled);
      return new MantaEncryptedObjectInputStream(rawStream, this.cipherDetails, secretKey, authenticateCiphertext);
    }
  }
  ...
  /**
     ...
     */
  private PlaintextByteRangePosition calculateSkipBytesAndPlaintextLength(final HttpUriRequest request, final MantaHttpHeaders requestHeaders) throws IOException {
    final Long initialSkipBytes;
    ...
    if (plaintextStart == 0 && negativeEndRequest) {
      ...
      request.setHeader(HttpHeaders.IF_UNMODIFIED_SINCE, objectResponse.getHeaderAsString(HttpHeaders.LAST_MODIFIED));
      ...
      final ByteRangeConversion computedRanges = this.cipherDetails.translateByteRange(initialSkipBytes, fullPlaintextSize - 1);
      ...
      final ByteRangeConversion computedRanges = this.cipherDetails.translateByteRange(plaintextStart, scaledPlaintextEnd);
      binaryStartPositionInclusive = computedRanges.getCiphertextStartPositionInclusive();
  ...
  @Override public MantaObjectResponse httpPutMetadata(final String path, final MantaHttpHeaders headers, final MantaMetadata metadata) throws IOException {
    ...
    if (actualContentType != null) {
      metadata.putIfAbsent(MantaHttpHeaders.ENCRYPTED_CONTENT_TYPE, actualContentType.getValue());
    }
  ...
  private void attachMetadata(final HttpResponse response) {
    ...
    if (encryptedContentType != null) {
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug("Encrypted content-type [{}] overwriting returned content-type [{}]", encryptedContentType, response.getFirstHeader(HttpHeaders.CONTENT_TYPE));
      }
  ...
  /**
     ...
     */
  private Map<String, String> extractEncryptionHeadersFromResponse(final HttpResponse response) {
    ...
    return buildEncryptedMetadata(encryptionType, metadataIvBase64, metadataCiphertextBase64, hmacId, metadataHmacBase64, null, response);
  }
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "ParameterNumber" }) private Map<String, String> buildEncryptedMetadata(final String encryptionType, final String metadataIvBase64, final String metadataCiphertextBase64, final String hmacId, final String metadataHmacBase64, final HttpRequest request, final HttpResponse response) {
    try {
  ...
  public void attachEncryptionCipherHeaders(final MantaMetadata metadata) {
    metadata.put(MantaHttpHeaders.ENCRYPTION_KEY_ID, encryptionKeyId);
  ...
  /**
     ...
     */
  public void attachEncryptedEntityHeaders(final MantaMetadata metadata, final Cipher cipher) {
    ...
    if (cipherDetails.isAEADCipher()) {
      metadata.put(MantaHttpHeaders.ENCRYPTION_AEAD_TAG_LENGTH, String.valueOf(cipherDetails.getAuthenticationTagOrHmacLengthInBytes()));
  ...
  /**
     ...
     */
  public void attachEncryptionPlaintextLengthHeader(final MantaMetadata metadata, final long length) {
    if (length > EncryptingEntity.UNKNOWN_LENGTH) {
      String originalLength = String.valueOf(length);
      metadata.put(MantaHttpHeaders.ENCRYPTION_PLAINTEXT_CONTENT_LENGTH, originalLength);
  ...
  /**
     ...
     */
  public void attachEncryptionPlaintextLengthHeader(final MantaMetadata metadata, final EncryptingEntity encryptingEntity) {
  ...
  public void attachEncryptedMetadata(final MantaMetadata metadata) {
    ...
    if (!this.cipherDetails.isAEADCipher()) {
      ...
      metadata.put(MantaHttpHeaders.ENCRYPTION_METADATA_AEAD_TAG_LENGTH, String.valueOf(this.cipherDetails.getAuthenticationTagOrHmacLengthInBytes()));
    }
  }
  ...
  /**
     ...
     */
  private String findOriginalContentType(final HttpEntity originalEntity, final MantaHttpHeaders httpHeaders) {
    ...
    return ObjectUtils.firstNonNull(httpHeaders.getContentType(), entityContentType);
  }
  ...
  /**
     ...
     */
  private void appendPlaintextContentLength(final String path, final EncryptingEntity encryptingEntity, final MantaMetadata metadata, final MantaObjectResponse response) throws IOException {
    ...
    metadata.put(MantaHttpHeaders.ENCRYPTION_PLAINTEXT_CONTENT_LENGTH, String.valueOf(encryptingEntity.getOriginalLength()));
  ...
  /**
     ...
     */
  private void enforceCipherAndMode(final String cipherId, final HttpRequest request, final HttpResponse response) {
  ...
  /**
     ...
     */
  static long[] byteRangeAsNullSafe(final Long[] ranges, final SupportedCipherDetails cipherDetails) {
