...
public class JsonSerializer implements ODataSerializer {
  ...
  @Override public <T extends java.lang.Object> void write(final Writer writer, final T obj) throws ODataSerializerException {
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override public <T extends java.lang.Object> void write(final Writer writer, final ResWrap<T> container) throws ODataSerializerException {
    final T obj = container == null ? null : container.getPayload();
    try (JsonGenerator json = new JsonFactory().createGenerator(writer)) {
      if (obj instanceof EntityCollection) {
        new JsonEntitySetSerializer(serverMode, contentType).doContainerSerialize((ResWrap<EntityCollection>) container, json);
      } else {
        if (obj instanceof Entity) {
  ...
  protected void links(final Linked linked, final JsonGenerator jgen) throws IOException, EdmPrimitiveTypeException {
    if (serverMode) {
  ...
  protected void clientLinks(final Linked linked, final JsonGenerator jgen) throws IOException, EdmPrimitiveTypeException {
  ...
  protected void serverLinks(final Linked linked, final JsonGenerator jgen) throws IOException, EdmPrimitiveTypeException {
  ...
  private void collection(final JsonGenerator jgen, final EdmTypeInfo typeInfo, final ValueType valueType, final List<?> value) throws IOException, EdmPrimitiveTypeException {
  ...
  protected void primitiveValue(final JsonGenerator jgen, final EdmTypeInfo typeInfo, final Object value) throws IOException, EdmPrimitiveTypeException {
    final EdmPrimitiveTypeKind kind = typeInfo == null ? EdmTypeInfo.determineTypeKind(value) : typeInfo.getPrimitiveTypeKind();
    if (value == null) {
      jgen.writeNull();
    } else {
      if (kind == EdmPrimitiveTypeKind.Boolean) {
        ...
        if (kind == null) {
          ...
          final String serialized = EdmPrimitiveTypeFactory.getInstance(kind).valueToString(value, null, null, Constants.DEFAULT_PRECISION, Constants.DEFAULT_SCALE, null);
          if (isIEEE754Compatible && (kind == EdmPrimitiveTypeKind.Int64 || kind == EdmPrimitiveTypeKind.Decimal) || !NUMBER_TYPES.contains(kind)) {
  ...
  private void complexValue(final JsonGenerator jgen, final EdmTypeInfo typeInfo, final List<Property> value, final Linked linked) throws IOException, EdmPrimitiveTypeException {
    jgen.writeStartObject();
  ...
  private void value(final JsonGenerator jgen, final String type, final Valuable value) throws IOException, EdmPrimitiveTypeException {
  ...
  protected void valuable(final JsonGenerator jgen, final Valuable valuable, final String name) throws IOException, EdmPrimitiveTypeException {
    if (!Constants.VALUE.equals(name) && !(valuable instanceof Annotation) && !(valuable.isComplex() && !valuable.isCollection())) {
      ...
      if (type != null && !type.isEmpty() && isODataMetadataFull) {
        jgen.writeStringField(name + Constants.JSON_TYPE, new EdmTypeInfo.Builder().setTypeExpression(type).build().external());
      }
    }
  ...
  private boolean isODataMetadataFull() {
    return contentType.isCompatible(ContentType.APPLICATION_JSON) && ContentType.VALUE_ODATA_METADATA_FULL.equalsIgnoreCase(contentType.getParameter(ContentType.PARAMETER_ODATA_METADATA));
  }
}