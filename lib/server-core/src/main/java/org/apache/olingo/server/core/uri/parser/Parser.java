...
public class Parser {
  ...
  public UriInfo parseUri(final String path, final String query, final String fragment) throws UriParserException, UriValidationException {
        ...
        if (firstSegment.equals("$metadata")) {
          ...
          if (firstSegment.equals("$all")) {
            ...
            if (firstSegment.equals("$entity")) {
              ...
              if (firstSegment.startsWith("$crossjoin")) {
                ensureLastSegment(firstSegment, 1, numberOfSegments);
                ...
                for (final String pathSegment : pathSegmentsDecoded) {
                  ...
                  if (segment != null) {
                    if (segment instanceof UriResourceCount || segment instanceof UriResourceRef || segment instanceof UriResourceValue) {
                      ...
                      if (segment instanceof UriResourceAction || segment instanceof UriResourceFunction && !((UriResourceFunction) segment).getFunction().isComposable()) {
                        ...
                        if (segment instanceof UriResourceStartingTypeFilterImpl) {
                          throw new UriParserSemanticException("First resource-path segment must not be namespace-qualified.", UriParserSemanticException.MessageKeys.NAMESPACE_NOT_ALLOWED_AT_FIRST_ELEMENT);
                        } else {
                          lastSegment = segment;
                        }
                      }
    ...
    for (final QueryOption option : options) {
      ...
      if (optionName.startsWith("$")) {
        ...
        if (optionName.equals(SystemQueryOptionKind.FILTER.toString())) {
          ...
          systemOption = new FilterParser(edm, odata).parse(filterTokenizer, contextTypes.peek(), contextUriInfo.getEntitySetNames());
          checkOptionEOF(filterTokenizer, optionName, optionValue);
        } else {
          if (optionName.equals(SystemQueryOptionKind.FORMAT.toString())) {
            ...
            if (optionName.equals(SystemQueryOptionKind.EXPAND.toString())) {
              if (contextTypes.peek() instanceof EdmStructuredType || !contextUriInfo.getEntitySetNames().isEmpty() || contextUriInfo.getKind() == UriInfoKind.entityId) {
                UriTokenizer expandTokenizer = new UriTokenizer(optionValue);
                systemOption = new ExpandParser(edm, odata).parse(expandTokenizer, contextTypes.peek() instanceof EdmStructuredType ? (EdmStructuredType) contextTypes.peek() : null);
                checkOptionEOF(expandTokenizer, optionName, optionValue);
              } else {
                throw new UriValidationException("Expand is only allowed on structured types!", UriValidationException.MessageKeys.SYSTEM_QUERY_OPTION_NOT_ALLOWED, optionName);
              }
            } else {
              if (optionName.equals(SystemQueryOptionKind.ID.toString())) {
                ...
                if (optionName.equals(SystemQueryOptionKind.LEVELS.toString())) {
                  throw new UriParserSyntaxException("System query option \'$levels\' is allowed only inside \'$expand\'!", UriParserSyntaxException.MessageKeys.SYSTEM_QUERY_OPTION_LEVELS_NOT_ALLOWED_HERE);
                } else {
                  if (optionName.equals(SystemQueryOptionKind.ORDERBY.toString())) {
                    UriTokenizer orderByTokenizer = new UriTokenizer(optionValue);
                    systemOption = new OrderByParser(edm, odata).parse(orderByTokenizer, contextTypes.peek() instanceof EdmStructuredType ? (EdmStructuredType) contextTypes.peek() : null, contextUriInfo.getEntitySetNames());
                    checkOptionEOF(orderByTokenizer, optionName, optionValue);
                  } else {
                    if (optionName.equals(SystemQueryOptionKind.SEARCH.toString())) {
                      systemOption = new SearchParser().parse(optionValue);
                    } else {
                      if (optionName.equals(SystemQueryOptionKind.SELECT.toString())) {
                        UriTokenizer selectTokenizer = new UriTokenizer(optionValue);
                        systemOption = new SelectParser(edm).parse(selectTokenizer, contextTypes.peek() instanceof EdmStructuredType ? (EdmStructuredType) contextTypes.peek() : null, contextIsCollection);
                        checkOptionEOF(selectTokenizer, optionName, optionValue);
                      } else {
                        if (optionName.equals(SystemQueryOptionKind.SKIP.toString())) {
                          ...
                          if (optionName.equals(SystemQueryOptionKind.SKIPTOKEN.toString())) {
                            ...
                            if (optionName.equals(SystemQueryOptionKind.TOP.toString())) {
                              ...
                              if (optionName.equals(SystemQueryOptionKind.COUNT.toString())) {
        ...
        try {
          ...
          throw new UriParserSyntaxException("Double system query option!", e, UriParserSyntaxException.MessageKeys.DOUBLE_SYSTEM_QUERY_OPTION, optionName);
        }
      } else {
          ...
          if (contextUriInfo.getAlias(optionName) == null) {
            ...
            if (aliasTokenizer.next(TokenKind.jsonArrayOrObject)) {
              if (!aliasTokenizer.next(TokenKind.EOF)) {
                throw new UriParserSyntaxException("Illegal value for alias \'" + optionName + "\'.", UriParserSyntaxException.MessageKeys.SYNTAX);
              }
            } else {
              UriTokenizer aliasValueTokenizer = new UriTokenizer(optionValue);
              expression = new ExpressionParser(edm, odata).parse(aliasValueTokenizer, null, contextUriInfo.getEntitySetNames());
              if (!aliasValueTokenizer.next(TokenKind.EOF)) {
                throw new UriParserSyntaxException("Illegal value for alias \'" + optionName + "\'.", UriParserSyntaxException.MessageKeys.SYNTAX);
              }
            }
            contextUriInfo.addAlias((AliasQueryOption) new AliasQueryOptionImpl().setAliasValue(expression).setName(optionName).setText(NULL.equals(optionValue) ? null : optionValue));
          } else {
            throw new UriParserSyntaxException("Alias already specified! Name: " + optionName, UriParserSyntaxException.MessageKeys.DUPLICATED_ALIAS, optionName);
          }
        } else {
  ...
  private void ensureLastSegment(final String segment, final int pos, final int size) throws UriParserSyntaxException {
    if (pos < size) {
  ...
  private void checkOptionEOF(UriTokenizer tokenizer, final String optionName, final String optionValue) throws UriParserException {
