...
public class Parser {
  ...
  public UriInfo parseUri(final String path, final String query, final String fragment, final Edm edm) throws UriParserException {
    UriContext context = new UriContext();
    UriParseTreeVisitor uriParseTreeVisitor = new UriParseTreeVisitor(edm, context);
    try {
      final RawUri uri = UriDecoder.decodeUri(path, query, fragment, 0);
      ...
      if (firstSegment.isEmpty()) {
        ...
        if (firstSegment.startsWith("$batch")) {
          ...
          if (firstSegment.startsWith("$metadata")) {
            ensureLastSegment(firstSegment, 1, uri.pathSegmentListDecoded.size());
            ...
            if (firstSegment.startsWith("$entity")) {
              ...
              if (firstSegment.startsWith("$all")) {
                ensureLastSegment(firstSegment, 1, uri.pathSegmentListDecoded.size());
                ...
                if (firstSegment.startsWith("$crossjoin")) {
                  ...
                  if (lastSegment instanceof UriResourceCount || lastSegment instanceof UriResourceRef || lastSegment instanceof UriResourceValue) {
        ...
        if (option.name.startsWith("$")) {
          ...
          if (option.name.equals(SystemQueryOptionKind.FILTER.toString())) {
            ...
            if (option.name.equals(SystemQueryOptionKind.FORMAT.toString())) {
              ...
              if (option.value.equalsIgnoreCase(JSON) || option.value.equalsIgnoreCase(XML) || option.value.equalsIgnoreCase(ATOM) || isFormatSyntaxValid(option.value)) {
                formatOption.setFormat(option.value);
              } else {
                throw new UriParserSyntaxException("Illegal value of $format option!", UriParserSyntaxException.MessageKeys.WRONG_VALUE_FOR_SYSTEM_QUERY_OPTION_FORMAT, option.value);
              }
                        ...
                        if (option.name.equals(SystemQueryOptionKind.SELECT.toString())) {
                          ...
                          if (option.name.equals(SystemQueryOptionKind.SKIP.toString())) {
                            ...
                            try {
                              ...
                              throw new UriParserSyntaxException("Illegal value of $skip option!", e, UriParserSyntaxException.MessageKeys.WRONG_VALUE_FOR_SYSTEM_QUERY_OPTION, option.name, option.value);
                            }
                            ...
                            if (option.name.equals(SystemQueryOptionKind.SKIPTOKEN.toString())) {
                              ...
                              if (option.name.equals(SystemQueryOptionKind.TOP.toString())) {
                                ...
                                try {
                                  ...
                                  throw new UriParserSyntaxException("Illegal value of $top option!", e, UriParserSyntaxException.MessageKeys.WRONG_VALUE_FOR_SYSTEM_QUERY_OPTION, option.name, option.value);
                                }
                                ...
                                if (option.name.equals(SystemQueryOptionKind.COUNT.toString())) {
                                  ...
                                  if (option.value.equals("true") || option.value.equals("false")) {
                                    ...
                                    throw new UriParserSyntaxException("Illegal value of $count option!", UriParserSyntaxException.MessageKeys.WRONG_VALUE_FOR_SYSTEM_QUERY_OPTION, option.name, option.value);
                                  }
                                  systemOption = inlineCountOption;
                                } else {
                                  throw new UriParserSyntaxException("Unknown system query option!", UriParserSyntaxException.MessageKeys.UNKNOWN_SYSTEM_QUERY_OPTION, option.name);
                                }
                              }
                            }
                          }
          ...
          try {
            ...
            throw new UriParserSyntaxException("Double system query option!", e, UriParserSyntaxException.MessageKeys.DOUBLE_SYSTEM_QUERY_OPTION, option.name);
          }
        } else {
          if (option.name.startsWith(AT)) {
            if (context.contextUriInfo.getAlias(option.name) == null) {
              final FilterExpressionEOFContext filterExpCtx = (FilterExpressionEOFContext) parseRule(option.value, ParserEntryRules.FilterExpression);
              final Expression expression = ((FilterOption) uriParseTreeVisitor.visitFilterExpressionEOF(filterExpCtx)).getExpression();
              context.contextUriInfo.addAlias((AliasQueryOption) new AliasQueryOptionImpl().setAliasValue(expression).setName(option.name).setText(NULL.equals(option.value) ? null : option.value));
            } else {
              throw new UriParserSyntaxException("Alias already specified! Name: " + option.name, UriParserSyntaxException.MessageKeys.DUPLICATED_ALIAS, option.name);
            }
          } else {
            context.contextUriInfo.addCustomQueryOption((CustomQueryOption) new CustomQueryOptionImpl().setName(option.name).setText(option.value));
          }
        }
      }
      return context.contextUriInfo;
    } catch (ParseCancellationException e) {
      throw e.getCause() instanceof UriParserException ? (UriParserException) e.getCause() : new UriParserSyntaxException("Syntax error", e, UriParserSyntaxException.MessageKeys.SYNTAX);
    }
  }
  ...
  private ParserRuleContext parseRule(final String input, final ParserEntryRules entryPoint) throws UriParserSyntaxException {
    ...
    try {
      ...
      try {
        ...
        throw new UriParserSyntaxException("Error in syntax", weakException, UriParserSyntaxException.MessageKeys.SYNTAX);
      }
    } catch (final RecognitionException hardException) {
      throw new UriParserSyntaxException("Error in syntax", hardException, UriParserSyntaxException.MessageKeys.SYNTAX);
    }
    return ret;
  }
