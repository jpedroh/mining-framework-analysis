...
public class SearchParserAndTokenizerTest {
  ...
  @Test public void basicParsing() throws Exception {
  ...
  @Test public void mixedParsing() throws Exception {
    assertQuery("a AND b OR c").resultsIn("{{\'a\' AND \'b\'} OR \'c\'}");
    assertQuery("a OR b AND c").resultsIn("{\'a\' OR {\'b\' AND \'c\'}}");
  }

  @Test public void notParsing() throws Exception {
    assertQuery("NOT a AND b OR c").resultsIn("{{{NOT \'a\'} AND \'b\'} OR \'c\'}");
    assertQuery("a OR b AND NOT c").resultsIn("{\'a\' OR {\'b\' AND {NOT \'c\'}}}");
  }

  @Test public void parenthesesParsing() throws Exception {
    assertQuery("a AND (b OR c)").resultsIn("{\'a\' AND {\'b\' OR \'c\'}}");
    assertQuery("(a OR b) AND NOT c").resultsIn("{{\'a\' OR \'b\'} AND {NOT \'c\'}}");
    assertQuery("(a OR B) AND (c OR d AND NOT e OR (f))").resultsIn("{{\'a\' OR \'B\'} AND {{\'c\' OR {\'d\' AND {NOT \'e\'}}} OR \'f\'}}");
    assertQuery("(a OR B) (c OR d NOT e OR (f))").resultsIn("{{\'a\' OR \'B\'} AND {{\'c\' OR {\'d\' AND {NOT \'e\'}}} OR \'f\'}}");
    assertQuery("((((a))))").resultsIn("\'a\'");
  ...
  @Test public void parseImplicitAnd() throws Exception {
  ...
  @Test public void invalidSearchQuery() throws Exception {
   ...
   * @throws Exception
   */
  @Test public void searchQueryPhraseAbnfTestcases() throws Exception {
    ...
    assertQuery("foo AND bar OR foo AND baz OR that AND bar OR that AND baz").resultsIn("{{{{\'foo\' AND \'bar\'} OR {\'foo\' AND \'baz\'}} OR {\'that\' AND \'bar\'}} OR {\'that\' AND \'baz\'}}");
...
>>>>>>> /usr/src/app/output/apache/olingo-odata4/010642c506d593c83cfb136cd9f5ddb815bf8ba7/lib/server-core/src/test/java/org/apache/olingo/server/core/uri/parser/search/SearchParserAndTokenizerTest.java/right.java
    ...
    private void resultsIn(final SearchParserException.MessageKey key) throws SearchTokenizerException {
      try {
        ...
        Assert.assertEquals("SearchParserException with unexpected message \'" + e.getMessage() + "\' was thrown.", key, e.getMessageKey());
        return;
      }
    ...
    private void resultsIn(final String expectedSearchExpression) throws SearchTokenizerException, SearchParserException {
