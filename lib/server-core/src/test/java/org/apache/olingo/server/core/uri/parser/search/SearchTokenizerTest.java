...
public class SearchTokenizerTest {
  @Test public void parseBasics() throws Exception {
  ...
  @Test public void parseWords() throws Exception {
  ...
  @Test public void parsePhrase() throws Exception {
  ...
  @Test public void parseNot() throws Exception {
  ...
  @Test public void parseOr() throws Exception {
  ...
  @Test public void parseImplicitAnd() throws SearchTokenizerException {
  ...
  @Test public void parseAnd() throws Exception {
  ...
  @Test public void parseAndOr() throws Exception {
  ...
  @Test public void parseCombinations() throws Exception {
    ...
    assertQuery("foo AND bar OR foo AND baz OR that AND bar OR that AND baz").addExpected(WORD, "foo").addExpected(AND).addExpected(WORD, "bar").addExpected(OR).addExpected(WORD, "foo").addExpected(AND).addExpected(WORD, "baz").addExpected(OR).addExpected(WORD, "that").addExpected(AND).addExpected(WORD, "bar").addExpected(OR).addExpected(WORD, "that").addExpected(AND).addExpected(WORD, "baz").validate();
    assertQuery("(foo OR that) AND (bar OR baz)").addExpected(OPEN).addExpected(WORD, "foo").addExpected(OR).addExpected(WORD, "that").addExpected(CLOSE).addExpected(AND).addExpected(OPEN).addExpected(WORD, "bar").addExpected(OR).addExpected(WORD, "baz").addExpected(CLOSE).validate();
  }
  ...
  @Test public void unicodeInWords() throws Exception {
    assertQuery("abc OR Ll\u01e3Lm\u02b5Lo\u00aaLt\u01f2Lu\u03d3Nl\u216f").resultsIn(WORD, OR, WORD);
  }
   ...
   * @throws Exception
   */
  @Test public void characterInPhrase() throws Exception {
    ...
    assertQuery("\"100\'Olingo\"").resultsIn(new Validator.Tuple(PHRASE, "\"100\'Olingo\""));
    assertQuery("\"\\\"123\" OR \"\\\\abc\"").resultsIn(new Validator.Tuple(PHRASE, "\"\"123\""), new Validator.Tuple(OR), new Validator.Tuple(PHRASE, "\"\\abc\""));
  ...
  @Test public void moreMixedTests() throws SearchTokenizerException {
    validate("abc");
    validate("NOT abc");
  ...
  @Test public void tokenizeInvalid() throws SearchTokenizerException {
  ...
  private static class Validator {
    ...
    private void resultsIn(final SearchTokenizerException.MessageKey key) throws SearchTokenizerException {
      try {
        validate();
        ...
        if (log) {
          System.out.println("Caught SearchTokenizerException with message key " + e.getMessageKey() + " and message " + e.getMessage());
        }
        return;
      }
