 ...
 */
@SuppressWarnings(value = { "serial" }) public class FAME<S extends DoubleSolution> extends SteadyStateNSGAII<S> {
  ...
  /** Constructor */
  public FAME(Problem<S> problem, int populationSize, int archiveSize, int maxEvaluations, SelectionOperator<List<S>, S> selectionOperator, SolutionListEvaluator<S> evaluator) {
    super(problem, maxEvaluations, populationSize, null, null, selectionOperator, new DominanceWithConstraintsComparator<>(), evaluator);
  ...
  private void loadFIS() {
    ...
    probability.addTerm(new Triangle("LOW", -0.4, 0.0, 0.4));
    probability.addTerm(new Triangle("MID", 0.1, 0.5, 0.9));
    probability.addTerm(new Triangle("HIGH", 0.6, 1.0, 1.4));
  ...
  @Override protected List<S> reproduction(List<S> population) {
    ...
    CR = 1.0;
    F = 0.5;
    DifferentialEvolutionCrossover crossoverOperator_DE = new DifferentialEvolutionCrossover(CR, F, DifferentialEvolutionCrossover.DE_VARIANT.RAND_1_BIN);
    double crossoverProbability, crossoverDistributionIndex;
    crossoverProbability = 1.0;
    crossoverDistributionIndex = 20.0;
    SBXCrossover crossoverOperator_SBX = new SBXCrossover(crossoverProbability, crossoverDistributionIndex);
