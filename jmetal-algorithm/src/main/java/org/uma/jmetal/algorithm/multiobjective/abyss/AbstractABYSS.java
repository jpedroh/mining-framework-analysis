...
public abstract class AbstractABYSS<S extends Solution> implements Algorithm<List<? extends Solution>> {
  ...
  public AbstractABYSS(int numberOfSubranges, int solutionSetSize, int refSet1Size, int refSet2Size, int archiveSize, int maxEvaluations, Archive archive, CrossoverOperator crossoverOperator, MutationLocalSearch improvementOperator, DoubleProblem problem) {
      ...
      for (int i = 0; i < subSet.size(); i++) {
        ...
        if (refSet1Test(individual)) {
          for (int indSet2 = 0; indSet2 < refSet2.size(); indSet2++) {
            double aux = SolutionUtils.distanceBetweenSolutions(individual, refSet2.get(indSet2));
