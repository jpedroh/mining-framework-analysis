 ...
 */
@SuppressWarnings(value = { "serial" }) public class MOSA<S extends Solution<?>> extends AbstractEvolutionStrategy<S, List<S>> {
   ...
   * Constructor
   */
  public MOSA(Problem<S> problem, int maxEvaluations, BoundedArchive<S> archive, MutationOperator<S> mutationOperator, double initialTemperature, CoolingScheme coolingScheme) {
    super(problem);
    setProblem(problem);
  ...
  public MOSA(Problem<S> problem, int maxEvaluations, int archiveSize, int biSections, MutationOperator<S> mutationOperator, double initialTemperature, CoolingScheme coolingScheme) {
    this(problem, maxEvaluations, new GenericBoundedArchive<>(archiveSize, new GridDensityEstimator<>(biSections, problem.getNumberOfObjectives())), mutationOperator, initialTemperature, coolingScheme);
  }
  ...
  protected double compute_acceptance_probability(S currentSolution, S mutatedSolution, double temperature) {
    ...
    for (int i = 0; i < currentSolution.objectives().length; i++) {
      value += (mutatedSolution.objectives()[i] - currentSolution.objectives()[i]) / Math.max(temperature, minimumTemperature);
    }
