 ...
 */
@SuppressWarnings(value = { "serial" }) public class IBEA<S extends Solution<?>> implements Algorithm<List<S>> {
  ...
  public void computeIndicatorValuesHD(List<S> solutionSet, double[] maximumValues, double[] minimumValues) {
    ...
    for (int j = 0; j < solutionSet.size(); j++) {
      ...
      for (S solution : solutionSet) {
        ...
        double value;
        if (flag == -1) {
          value = -calculateHypervolumeIndicator(A.get(0), B.get(0), problem.getNumberOfObjectives(), maximumValues, minimumValues);
        } else {
          value = calculateHypervolumeIndicator(B.get(0), A.get(0), problem.getNumberOfObjectives(), maximumValues, minimumValues);
        }
