...
@SuppressWarnings(value = { "serial" }) public class DMOPSO implements Algorithm<List<DoubleSolution>> {
  ...
  protected void updateVelocity(int i) {
    ...
    for (int var = 0; var < particle.variables().size(); var++) {
      speed[i][var] = velocityConstriction(constrictionCoefficient(C1, C2) * (inertiaWeight(iterations, maxIterations, this.weightMax, this.weightMin) * speed[i][var] + C1 * r1 * (bestParticle.variables().get(var) - particle.variables().get(var)) + C2 * r2 * (bestGlobal.variables().get(var) - particle.variables().get(var))), deltaMax, deltaMin, var, i);
    }
  }
  ...
  protected void initializeUniformWeight() {
    if ((problem.getNumberOfObjectives() == 2) && (swarmSize <= 300)) {
      ...
      String dataFileName;
      dataFileName = "W" + problem.getNumberOfObjectives() + "D_" + swarmSize + ".dat";
      try {
        ...
        throw new JMetalException("initializeUniformWeight: failed when reading for file: " + dataDirectory + "/" + dataFileName, e);
      }
    }
  }
...
>>>>>>> /usr/src/app/output/jmetal/jmetal/163ad814eccdd268a4305e4524a0fae9aaed5978/jmetal-algorithm/src/main/java/org/uma/jmetal/algorithm/multiobjective/dmopso/DMOPSO.java/right.java
  ...
  private double fitnessFunction(DoubleSolution sol, double[] lambda) {
    ...
    if (functionType == FunctionType.TCHE) {
      ...
      if (functionType == FunctionType.AGG) {
        double sum = 0.0;
        for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
        ...
        if (functionType == FunctionType.PBI) {
          double d1, d2, nl;
          double theta = 5.0;
          d1 = d2 = nl = 0.0;
          for (int i = 0; i < problem.getNumberOfObjectives(); i++) {
