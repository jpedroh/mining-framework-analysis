...
@SuppressWarnings(value = { "serial" }) public class OMOPSO extends AbstractParticleSwarmOptimization<DoubleSolution, List<DoubleSolution>> {
  ...
  /** Constructor */
  public OMOPSO(DoubleProblem problem, SolutionListEvaluator<DoubleSolution> evaluator, int swarmSize, int maxIterations, int archiveSize, double eta, UniformMutation uniformMutation, NonUniformMutation nonUniformMutation) {
  ...
  @Override protected void updateVelocity(List<DoubleSolution> swarm) {
    ...
    for (int i = 0; i < swarmSize; i++) {
      ...
      for (int var = 0; var < particle.variables().size(); var++) {
        speed[i][var] = W * speed[i][var] + C1 * r1 * (bestParticle.variables().get(var) - particle.variables().get(var)) + C2 * r2 * (bestGlobal.variables().get(var) - particle.variables().get(var));
      }
    }
  }
