...
public class Generators {
  ...
  private static abstract class AbstractGenerator extends QueryGenerator {
    ...
    @Override public int currentIteration() {
      return iteration;
    }
  ...
  public static final QueryGenerator.Builder 
<<<<<<< /usr/src/app/output/datastax/java-driver/69f03b57eecc274842347dd017d23c7abd64eeeb/driver-examples/stress/src/main/java/com/datastax/driver/stress/Generators.java/left.java
  CASSANDRA_INSERTER = new QueryGenerator.Builder() {
    public String name() {
      return "insert";
    }
    ...
    public QueryGenerator create(final int id, final int iterations, final OptionSet options, final Session session) {
...
=======
  INSERTER = new QueryGenerator.Builder() {
    public String name() {
      return "insert";
    }

    public OptionParser addOptions(OptionParser parser) {
      String msg = "Simple insertion of CQL3 rows (using prepared statements unless the --no-prepare option is used). " + "The inserted rows have a fixed set of columns but no clustering columns.";
      parser.formatHelpWith(Stress.Help.formatFor(name(), msg));
    ...
    public QueryGenerator create(int id, int iterations, OptionSet options, Session session) {
      return options.has("no-prepare") ? createRegular(id, iterations, options) : createPrepared(id, iterations, options, session);
    }
    ...
    public QueryGenerator createRegular(int id, int iterations, OptionSet options) {
      ...
      return new AbstractGenerator(iterations) {
        @Override public QueryGenerator.Request next() {
          StringBuilder sb = new StringBuilder();
          sb.append("UPDATE standard1 SET ");
          for (int i = 0; i < columnsPerRow; ++i) {
            if (i > 0) {
...
>>>>>>> /usr/src/app/output/datastax/java-driver/69f03b57eecc274842347dd017d23c7abd64eeeb/driver-examples/stress/src/main/java/com/datastax/driver/stress/Generators.java/right.java
  ;
  ...
  public static final QueryGenerator.Builder READER = new QueryGenerator.Builder() {
    ...
    public QueryGenerator create(int id, int iterations, OptionSet options, Session session) {
      return options.has("no-prepare") ? createRegular(id, iterations) : createPrepared(id, iterations, session);
    }
