...
public class HttpClientChannelSink extends AbstractChannelSink {
  ...
  @Override public ChannelFuture execute(ChannelPipeline httpPipeline, Runnable task) {
    if (transport != null) {
  ...
  @Override protected void setInterestOpsRequested(ChannelPipeline pipeline, ChannelStateEvent evt) throws Exception {
  ...
  @Override protected void bindRequested(ChannelPipeline pipeline, ChannelStateEvent evt) throws Exception {
  ...
  @Override protected void writeRequested(ChannelPipeline pipeline, MessageEvent e) throws Exception {
    ...
    switch (httpClientChannel.state()) {
      ...
      if (isContentLengthSet(httpRequest)) {
        ...
        if (isTransferEncodingChunked(httpRequest)) {
          ...
          if (httpRequest.headers().contains(Names.UPGRADE)) {
            httpRequest.setContent(httpContent);
            ChannelFuture future = transport.write(httpRequest);
  ...
  @Override protected void abortRequested(ChannelPipeline pipeline, final AbortEvent evt) throws Exception {
      ...
      case REQUEST:
      {
        ...
        if (isContentLengthSet(httpRequest)) {
          ...
          if (isTransferEncodingChunked(httpRequest)) {
            ...
            if (httpRequestHeaders.contains(Names.UPGRADE)) {
              ChannelFuture future = transport.write(httpRequest);
              httpClientChannel.state(UPGRADEABLE);
              chainFutures(future, httpFuture);
            } else {
