...
abstract class TapListener extends RunListener {
  ...
  protected void generateTapPerMethod(Result result) {
    for (final JUnitTestData testMethod : testMethodsList) {
      final TestResult tapTestResult = TapJUnitUtil.generateTAPTestResult(testMethod, 1, isYaml());
      final TestSet testSet = new TestSet();
      testSet.setPlan(new Plan(1));
      testSet.addTestResult(tapTestResult);
      final String className = TapJUnitUtil.extractClassName(testMethod.getDescription());
      final String methodName = TapJUnitUtil.extractMethodName(testMethod.getDescription());
      final File output = new File(System.getProperty("tap.junit.results", "target/"), className + ":" + methodName + ".tap");
  ...
  protected void generateTapPerClass(Result result) {
    ...
    for (JUnitTestData testMethod : testMethodsList) {
      ...
      testsByClass.computeIfAbsent(className, (k) -> new ArrayList<>()).add(testMethod);
    }
    testsByClass.forEach((className, testMethods) -> {
      final TestSet testSet = new TestSet();
      testSet.setPlan(new Plan(testMethods.size()));
      testMethods.forEach((testMethod) -> {
        TestResult tapTestResult = TapJUnitUtil.generateTAPTestResult(testMethod, 1, isYaml());
        testSet.addTestResult(tapTestResult);
      });
