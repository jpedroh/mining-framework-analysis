...
public class LdContextFactory {
  ...
  private void addEnumTerms(Map<String, Object> termsMap, Expose expose, String name, Enum value) throws NoSuchFieldException {
    if (value != null) {
      ...
      if (enumValueExpose != null) {
        ...
        final String camelCaseEnumValue = WordUtils.capitalizeFully(value.toString(), new char[] { '_' }).replaceAll("_", "");
  ...
  public String vocabFromClass(Class<?> clazz, String defaultVocab) {
    final Vocab packageVocab = getAnnotation(clazz.getPackage(), Vocab.class);
    final Vocab classVocab = getAnnotation(clazz, Vocab.class);
    String vocab;
    if (classVocab != null) {
      vocab = classVocab.value();
    } else {
      if (packageVocab != null) {
  ...
  public Map<String, Object> termsFromClass(Class<?> clazz) {
    Map<String, Object> termsMap = getAnnotatedTerms(clazz.getPackage(), clazz.getPackage().getName());
