...
public class CShareableResource implements ChocoView {
  ...
  @Override public boolean inject(Parameters ps, ReconfigurationProblem p) throws SchedulerException {
    this.rp = p;
  ...
  /**
     ...
     */
  @Override public boolean beforeSolve(ReconfigurationProblem p) throws SchedulerException {
  ...
  @Override public boolean insertActions(ReconfigurationProblem r, Solution s, ReconfigurationPlan p) {
    Mapping srcMapping = r.getSourceModel().getMapping();
    Map<VM, Action> actions = new HashMap<>();
    p.getActions().stream().filter(RunningVMPlacement.class::isInstance).map((a) -> (RunningVMPlacement) a).filter((a) -> changing.contains(a.getVM()) || clones.containsValue(a.getVM())).forEach((a) -> actions.put(destVM(a.getVM()), (Action) a));
    for (VM vm : changing) {
...
<<<<<<< /usr/src/app/output/btrplace/scheduler/8a939c1deed03406d77b6bcc81aae4dc91b2c9c6/choco/src/main/java/org/btrplace/scheduler/choco/view/CShareableResource.java/left.java
        var
=======
        rp.fixed(m, "vmAllocation(\'", getResourceIdentifier(), "\', \'", vm, "\'")
>>>>>>> /usr/src/app/output/btrplace/scheduler/8a939c1deed03406d77b6bcc81aae4dc91b2c9c6/choco/src/main/java/org/btrplace/scheduler/choco/view/CShareableResource.java/right.java
    ...
    for (SatConstraint c : i.getSatConstraints()) {
      ...
      if (c instanceof Preserve) {
        ...
        if (c instanceof Overbook) {
          ...
          if (c instanceof ResourceCapacity && c.getInvolvedNodes().size() == 1) {
            Node n = c.getInvolvedNodes().iterator().next();
  ...
  /**
     ...
     */
  private boolean overloaded(final TObjectDoubleMap<Node> wantedRatios, final IntMap wantedAmount, final IntMap wantedCapacity, final Collection<VM> vms, Node n) {
  ...
  @Override public boolean cloneVM(VM vm, VM clone) {
