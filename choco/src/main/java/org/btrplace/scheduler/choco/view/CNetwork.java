...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  @Override public boolean beforeSolve(ReconfigurationProblem rp) throws SchedulerException {
    ...
    for (VMTransition migration : rp.getVMActions()) {
      ...
      if (durationMin > hotDirtyDuration) {
        ...
        durationTotal = durationMin + (((hotDirtySize / hotDirtyDuration) * durationMin) / (bandwidth_octet - (hotDirtySize / hotDirtyDuration)));
      }
      try {
...
>>>>>>> /usr/src/app/output/btrplace/scheduler/d42500d47b1f8f88aada61e71e4880f680cee32f/choco/src/main/java/org/btrplace/scheduler/choco/view/CNetwork.java/right.java
      ...
      if (!tasksListUp.isEmpty()) {
        solver.post(ICF.cumulative(tasksListUp.toArray(new Task[tasksListUp.size()]), heightsListUp.toArray(new IntVar[heightsListUp.size()]), VF.fixed(l.getCapacity(), solver), true));
        tasksListUp.clear();
        heightsListUp.clear();
      }
      if (!tasksListDown.isEmpty()) {
        solver.post(ICF.cumulative(tasksListDown.toArray(new Task[tasksListDown.size()]), heightsListDown.toArray(new IntVar[heightsListDown.size()]), VF.fixed(l.getCapacity(), solver), true));
        tasksListDown.clear();
    ...
    for (Switch sw : net.getSwitches()) {
      if (sw.getCapacity() > 0) {
        ...
        if (!tasksListDown.isEmpty()) {
          solver.post(ICF.cumulative(tasksList.toArray(new Task[tasksList.size()]), heightsList.toArray(new IntVar[heightsList.size()]), VF.fixed(sw.getCapacity(), solver), true));
          tasksList.clear();
          heightsList.clear();
        }
      }
