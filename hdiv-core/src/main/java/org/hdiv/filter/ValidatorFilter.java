...
public class ValidatorFilter extends OncePerRequestFilter {
  ...
  @Override protected void doFilterInternal(final HttpServletRequest request, final HttpServletResponse response, final FilterChain filterChain) throws ServletException, IOException {
    ...
    try {
      ...
      ValidationContext context = validationContextFactory.newInstance(multipartProcessedRequest, response, validationHelper, hdivConfig.isUrlObfuscation());
      ...
      if (hdivException instanceof HDIVException) {
        ...
        if (!hdivConfig.isDebugMode()) {
          List<ValidatorError> errors = Collections.singletonList(new ValidatorError(hdivException.getMessage(), request.getRequestURI()));
          errorHandler.handleValidatorError(multipartProcessedRequest, responseWrapper, errors);
        }
      } else {
  ...
  /**
	 ...
	 */
  protected void processRequest(final HttpServletRequest requestWrapper, final ResponseWrapper responseWrapper, final FilterChain filterChain, final String obfuscated) throws IOException, ServletException {
