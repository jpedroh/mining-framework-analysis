...
public class SimpleViewGenerator {
  ...
  @edu.umd.cs.findbugs.annotations.SuppressWarnings(value = "VA_FORMAT_STRING_EXTRA_ARGUMENTS_PASSED") private String resolveTypeDiscriminator(final Class<?> persistentType) {
  ...
  /**
	 ...
	 * @return a Map with generated views.
	 */
  public Map<String, DesignDocument.View> generateViewsFromPersistentType(final Class<?> persistentType) {
  ...
  private void addView(Map<String, DesignDocument.View> views, View input, Class<?> repositoryClass) {
    if (input.file().length() > 0) {
      ...
      if (shouldLoadFunctionFromClassPath(input.map()) || shouldLoadFunctionFromClassPath(input.reduce())) {
  ...
  protected DesignDocument.View loadViewFromFile(View input, Class<?> repositoryClass) {
  ...
  protected String loadResourceFromClasspath(Class<?> repositoryClass, String path) {
    try {
  ...
  protected DesignDocument.View loadViewFromFile(Map<String, DesignDocument.View> views, View input, Class<?> repositoryClass) {
    try {
      String json = loadResourceFromClasspath(repositoryClass, input.file());
      return mapper().readValue(json.replaceAll(LINE_ENDING, ""), DesignDocument.View.class);
    } catch (Exception e) {
      throw Exceptions.propagate(e);
    }
  }
  ...
  protected void generateView(Map<String, org.ektorp.support.DesignDocument.View> views, Method me) {
  ...
  private void generateSetBasedDocRefView(Map<String, org.ektorp.support.DesignDocument.View> views, Member me, DocumentReferences referenceMetaData) {
    ...
    if (orderBy.length() > 0) {
      views.put(viewName, generateDocRefsAsSetWithOrderByView(backRef, fieldName, orderBy, typeDiscriminator));
    } else {
  ...
  private String resolveTypeDiscriminatorForBackReference(Member m) {
    Method me = ReflectionUtils.findMethod(m.getDeclaringClass(), "get" + firstCharToUpperCase(m.getName()));
    if (me != null) {
