...
public class StdCouchDbInstance implements CouchDbInstance {
  ...
  @Override public boolean checkIfDbExists(String path) {
    return checkIfDbExists(DbPath.fromString(path));
  }

  @Override public boolean checkIfDbExists(DbPath db) {
  ...
  public List<String> getAllDatabases() {
    return restTemplate.get("/_all_dbs", new StdResponseHandler<List<String>>() {
      @Override public List<String> success(HttpResponse hr) throws Exception {
  ...
  public ReplicationStatus replicate(ReplicationCommand cmd) {
    try {
      return restTemplate.post("/_replicate", objectMapper.writeValueAsString(cmd), new StdResponseHandler<ReplicationStatus>() {
        @Override public ReplicationStatus success(HttpResponse hr) throws Exception {
  ...
  @Override public CouchDbConnector getReplicatorConnector() {
    return createConnector("_replicator", false);
  }

  @Override public <T extends java.lang.Object> T getConfiguration(final Class<T> c) {
    return getConfiguration(c, null, null);
  }

  @Override public <T extends java.lang.Object> T getConfiguration(final Class<T> c, String section) {
    return getConfiguration(c, section, null);
  }

  @Override public <T extends java.lang.Object> T getConfiguration(final Class<T> c, String section, String key) {
    ...
    return restTemplate.get(url, new StdResponseHandler<T>() {
  ...
  @Override public String getConfiguration(String section, String key) {
    return getConfiguration(String.class, section, key);
  }

  @Override public String setConfiguration(String section, String key, String value) {
    Assert.notNull(section, "Section may not be null");
    Assert.notNull(key, "Key may not be null");
    String url = "/_config/" + section + "/" + key;
    return restTemplate.put(url, jsonSerializer.toJson(value), new StdResponseHandler<String>() {
      @Override public String success(HttpResponse hr) throws Exception {
  ...
  @Override public String deleteConfiguration(String section, String key) {
    Assert.notNull(section, "Section may not be null");
    Assert.notNull(key, "Key may not be null");
    String url = "/_config/" + section + "/" + key;
    return restTemplate.delete(url, new StdResponseHandler<String>() {
      @Override public String success(HttpResponse hr) throws Exception {
        String s = objectMapper.readValue(hr.getContent(), String.class);
        return s;
      }
  ...
  @Override public Collection<ActiveTask> getActiveTasks() {
    String url = "/_active_tasks";
    List<StdActiveTask> tasks = restTemplate.get(url, new StdResponseHandler<List<StdActiveTask>>() {
      @Override public List<StdActiveTask> success(HttpResponse hr) throws Exception {
