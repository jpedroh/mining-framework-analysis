...
public class StdCouchDbConnector implements CouchDbConnector {
  ...
  @Override public AttachmentInputStream getAttachment(final String id, final String attachmentId) {
    ...
    HttpResponse r = restTemplate.get(dbURI.append(id).append(attachmentId).toString());
  ...
  @Override public <T extends java.lang.Object> Page<T> queryForPage(ViewQuery query, PageRequest pr, Class<T> type) {
    ...
    return query.hasMultipleKeys() ? restTemplate.post(query.buildQuery(), query.getKeysAsJson(), ph) : restTemplate.get(query.buildQuery(), ph);
  }

  @Override public ViewResult queryView(ViewQuery query) {
  ...
  @Override public InputStream queryForStream(ViewQuery query) {
    Assert.notNull(query, "query cannot be null");
    query.dbPath(dbURI.toString());
    return query.hasMultipleKeys() ? restTemplate.postUncached(query.buildQuery(), query.getKeysAsJson()).getContent() : restTemplate.getUncached(query.buildQuery()).getContent();
  }
