...
public class StdCouchDbConnector implements CouchDbConnector {
  ...
  @Override public void create(final Object o) {
  ...
  @Override public void create(String id, Object node) {
  ...
  @Override public boolean contains(String id) {
    return restTemplate.head(URIWithDocId(id), new ResponseCallback<Boolean>() {
      @Override public Boolean error(HttpResponse hr) {
      ...
      @Override public Boolean success(HttpResponse hr) throws Exception {
        return Boolean.TRUE;
      }
    });
  }

  @Override public String createAttachment(String docId, AttachmentInputStream data) {
    return createAttachment(docId, null, data);
  }

  @Override public String createAttachment(String docId, String revision, AttachmentInputStream data) {
  ...
  @Override public AttachmentInputStream getAttachment(final String id, final String attachmentId) {
    assertDocIdHasValue(id);
  ...
  @Override public AttachmentInputStream getAttachment(String id, String attachmentId, String revision) {
    assertDocIdHasValue(id);
    Assert.hasText(attachmentId, "attachmentId may not be null or empty");
  ...
  @Override public String delete(Object o) {
    Assert.notNull(o, "document may not be null");
    return delete(Documents.getId(o), Documents.getRevision(o));
  }

  @Override public PurgeResult purge(Map<String, List<String>> revisionsToPurge) {
    return restTemplate.post(dbURI.append("_purge").toString(), jsonSerializer.toJson(revisionsToPurge), new StdResponseHandler<PurgeResult>() {
      @Override public PurgeResult success(HttpResponse hr) throws Exception {
        return objectMapper.readValue(hr.getContent(), PurgeResult.class);
      }
    });
  }

  @Override public <T extends java.lang.Object> T get(final Class<T> c, String id) {
    return get(c, id, EMPTY_OPTIONS);
  }

  @Override public <T extends java.lang.Object> T get(final Class<T> c, String id, Options options) {
    Assert.notNull(c, "Class may not be null");
    assertDocIdHasValue(id);
    URI uri = dbURI.append(id);
    applyOptions(options, uri);
    return restTemplate.get(uri.toString(), new StdResponseHandler<T>() {
      @Override public T success(HttpResponse hr) throws Exception {
        return objectMapper.readValue(hr.getContent(), c);
      }
    });
  }

  @Override public <T extends java.lang.Object> T get(final Class<T> c, String id, String rev) {
    Assert.notNull(c, "Class may not be null");
    assertDocIdHasValue(id);
  ...
  @Override public <T extends java.lang.Object> T getWithConflicts(final Class<T> c, String id) {
    Assert.notNull(c, "Class may not be null");
    assertDocIdHasValue(id);
    return get(c, id, new Options().includeConflicts());
  }

  @Override public <T extends java.lang.Object> T find(Class<T> c, String id) {
    return find(c, id, EMPTY_OPTIONS);
  }

  @Override public <T extends java.lang.Object> T find(final Class<T> c, String id, Options options) {
    Assert.notNull(c, "Class may not be null");
    assertDocIdHasValue(id);
    URI uri = dbURI.append(id);
    applyOptions(options, uri);
    return restTemplate.get(uri.toString(), new StdResponseHandler<T>() {
      ...
      @Override public T error(HttpResponse hr) {
  ...
  @Override public List<Revision> getRevisions(String id) {
    assertDocIdHasValue(id);
    return restTemplate.get(dbURI.append(id).param("revs_info", "true").toString(), new StdResponseHandler<List<Revision>>() {
      @Override public List<Revision> success(HttpResponse hr) throws Exception {
      ...
      @Override public List<Revision> error(HttpResponse hr) {
        if (hr.getCode() == HttpStatus.NOT_FOUND) {
  ...
  @Override public InputStream getAsStream(String id) {
    assertDocIdHasValue(id);
    return getAsStream(id, EMPTY_OPTIONS);
  }

  @Override public InputStream getAsStream(String id, Options options) {
    URI uri = dbURI.append(id);
    applyOptions(options, uri);
    HttpResponse r = restTemplate.get(uri.toString());
    return r.getContent();
  ...
  @Override public InputStream getAsStream(String id, String rev) {
    assertDocIdHasValue(id);
    Assert.hasText(rev, "Revision may not be null or empty");
    return getAsStream(id, new Options().revision(rev));
  }

  @Override public void update(final Object o) {
    Assert.notNull(o, "Document cannot be null");
    final String id = Documents.getId(o);
    assertDocIdHasValue(id);
    restTemplate.put(dbURI.append(id).toString(), jsonSerializer.toJson(o), new StdResponseHandler<Void>() {
      @Override public Void success(HttpResponse hr) throws Exception {
        JsonNode n = objectMapper.readValue(hr.getContent(), JsonNode.class);
        Documents.setRevision(o, n.get("rev").getTextValue());
        return null;
      }

      @Override public Void error(HttpResponse hr) {
  ...
  @Override public String delete(String id, String revision) {
    assertDocIdHasValue(id);
    return restTemplate.delete(dbURI.append(id).param("rev", revision).toString(), revisionHandler).getRevision();
  }

  @Override public List<String> getAllDocIds() {
    return restTemplate.get(dbURI.append("_all_docs").toString(), docIdResponseHandler);
  }

  @Override public void createDatabaseIfNotExists() {
  ...
  @Override public <T extends java.lang.Object> List<T> queryView(final ViewQuery query, final Class<T> type) {
    Assert.notNull(query, "query may not be null");
    query.dbPath(dbURI.toString());
    EmbeddedDocViewResponseHandler<T> rh = new EmbeddedDocViewResponseHandler<T>(type, objectMapper, query.isIgnoreNotFound());
    return executeQuery(query, rh);
  }

  private <T extends java.lang.Object> T executeQuery(final ViewQuery query, ResponseCallback<T> rh) {
    if (!query.isCacheOk()) {
      return query.hasMultipleKeys() ? restTemplate.postUncached(query.buildQuery(), query.getKeysAsJson(), rh) : restTemplate.getUncached(query.buildQuery(), rh);
    }
    return query.hasMultipleKeys() ? restTemplate.post(query.buildQuery(), query.getKeysAsJson(), rh) : restTemplate.get(query.buildQuery(), rh);
  }

  @Override public <T extends java.lang.Object> Page<T> queryForPage(ViewQuery query, PageRequest pr, Class<T> type) {
    Assert.notNull(query, "query may not be null");
  ...
  @Override public ViewResult queryView(final ViewQuery query) {
    Assert.notNull(query, "query cannot be null");
    query.dbPath(dbURI.toString());
    ResponseCallback<ViewResult> rh = new StdResponseHandler<ViewResult>() {
      @Override public ViewResult success(HttpResponse hr) throws Exception {
  ...
  @Override public StreamingViewResult queryForStreamingView(ViewQuery query) {
    return new StreamingViewResult(objectMapper, queryForHttpResponse(query), query.isIgnoreNotFound());
  }

  @Override public InputStream queryForStream(ViewQuery query) {
  ...
  private HttpResponse queryForHttpResponse(ViewQuery query) {
    Assert.notNull(query, "query cannot be null");
    query.dbPath(dbURI.toString());
    return query.hasMultipleKeys() ? restTemplate.postUncached(query.buildQuery(), query.getKeysAsJson()) : restTemplate.getUncached(query.buildQuery());
  }

  @Override public String deleteAttachment(String docId, String revision, String attachmentId) {
    return restTemplate.delete(dbURI.append(docId).append(attachmentId).param("rev", revision).toString(), revisionHandler).getRevision();
  }
  ...
  @Override public HttpClient getConnection() {
    return dbInstance.getConnection();
  }

  @Override public DbInfo getDbInfo() {
    return restTemplate.get(dbURI.toString(), new StdResponseHandler<DbInfo>() {
      @Override public DbInfo success(HttpResponse hr) throws Exception {
        return objectMapper.readValue(hr.getContent(), DbInfo.class);
      }
    });
  }

  @Override public DesignDocInfo getDesignDocInfo(String designDocId) {
    Assert.hasText(designDocId, "designDocId may not be null or empty");
    String uri = dbURI.append("_design").append(designDocId).append("_info").toString();
    return restTemplate.get(uri, new StdResponseHandler<DesignDocInfo>() {
      @Override public DesignDocInfo success(HttpResponse hr) throws Exception {
        return objectMapper.readValue(hr.getContent(), DesignDocInfo.class);
      }
    });
  }

  @Override public void compact() {
    restTemplate.post(dbURI.append("_compact").toString(), "not_used", VOID_RESPONSE_HANDLER);
  }

  @Override public void cleanupViews() {
    restTemplate.post(dbURI.append("_view_cleanup").toString(), "not_used", VOID_RESPONSE_HANDLER);
  }

  @Override public ReplicationStatus replicateFrom(String source) {
    ReplicationCommand cmd = new ReplicationCommand.Builder().target(dbName).source(source).build();
    return dbInstance.replicate(cmd);
  }

  @Override public ReplicationStatus replicateFrom(String source, Collection<String> docIds) {
    ReplicationCommand cmd = new ReplicationCommand.Builder().target(dbName).source(source).docIds(docIds).build();
    return dbInstance.replicate(cmd);
  }

  @Override public ReplicationStatus replicateTo(String target) {
    ReplicationCommand cmd = new ReplicationCommand.Builder().target(target).source(dbName).build();
    return dbInstance.replicate(cmd);
  }

  @Override public ReplicationStatus replicateTo(String target, Collection<String> docIds) {
    ReplicationCommand cmd = new ReplicationCommand.Builder().target(target).source(dbName).docIds(docIds).build();
    return dbInstance.replicate(cmd);
  }

  @Override public void compactViews(String designDocumentId) {
    Assert.hasText(designDocumentId, "designDocumentId may not be null or empty");
    restTemplate.post(dbURI.append("_compact").append(designDocumentId).toString(), "not_used", VOID_RESPONSE_HANDLER);
  }

  @Override public List<DocumentOperationResult> executeAllOrNothing(InputStream inputStream) {
    return executeBulk(inputStream, true);
  }

  @Override public List<DocumentOperationResult> executeBulk(InputStream inputStream) {
  ...
  private List<DocumentOperationResult> executeBulk(InputStream inputStream, boolean allOrNothing) {
    ...
    return restTemplate.post(dbURI.append("_bulk_docs").toString(), writer.createInputStreamWrapper(allOrNothing, inputStream), new BulkOperationResponseHandler(objectMapper));
  }

  @Override public List<DocumentOperationResult> executeAllOrNothing(Collection<?> objects) {
    return executeBulk(objects, true);
  }

  @Override public List<DocumentOperationResult> executeBulk(Collection<?> objects) {
    return executeBulk(objects, false);
  }

  @Override public void addToBulkBuffer(Object o) {
    bulkBufferManager.add(o);
    LOG.debug("{} added to bulk buffer", o);
  }

  @Override public void clearBulkBuffer() {
    bulkBufferManager.clear();
    LOG.debug("bulk buffer cleared");
  }

  @Override public List<DocumentOperationResult> flushBulkBuffer() {
    try {
  ...
  private List<DocumentOperationResult> executeBulk(Collection<?> objects, boolean allOrNothing) {
    BulkOperation op = jsonSerializer.createBulkOperation(objects, allOrNothing);
    try {
      List<DocumentOperationResult> result = restTemplate.post(dbURI.append("_bulk_docs").toString(), op.getData(), new BulkOperationResponseHandler(objects, objectMapper));
      op.awaitCompletion();
      return result;
    }  finally {
      op.close();
  ...
  @Override public int getRevisionLimit() {
    return restTemplate.get(dbURI.append("_revs_limit").toString(), new StdResponseHandler<Integer>() {
      @Override public Integer success(HttpResponse hr) throws Exception {
  ...
  @Override public void setRevisionLimit(int limit) {
    restTemplate.put(dbURI.append("_revs_limit").toString(), Integer.toString(limit), VOID_RESPONSE_HANDLER);
  }

  private InputStream fetchChangesAsStream(ChangesCommand cmd) {
    HttpResponse r = restTemplate.get(dbURI.append(cmd.toString()).toString());
    return r.getContent();
  }

  @Override public List<DocumentChange> changes(ChangesCommand cmd) {
    if (cmd.continuous) {
  ...
  @Override public StreamingChangesResult changesAsStream(ChangesCommand cmd) {
    if (cmd.continuous) {
      throw new IllegalArgumentException("ChangesCommand may not declare continous = true while calling changes");
    }
    ChangesCommand actualCmd = new ChangesCommand.Builder().merge(cmd).continuous(false).build();
    HttpResponse response = restTemplate.get(dbURI.append(actualCmd.toString()).toString());
    return new StreamingChangesResult(objectMapper, response);
  }

  @Override public ChangesFeed changesFeed(ChangesCommand cmd) {
    int heartbeat = cmd.heartbeat > 0 ? cmd.heartbeat : DEFAULT_HEARTBEAT_INTERVAL;
  ...
  @Override public String callUpdateHandler(String designDocID, String function, String docID) {
    return callUpdateHandler(designDocID, function, docID, null);
  }

  @Override public String callUpdateHandler(String designDocID, String function, String docID, Map<String, String> params) {
    Assert.hasText(designDocID, "designDocID may not be null or empty");
    ...
    req.dbPath(dbURI.toString()).designDocId(designDocID).functionName(function).docId(docID).params(params).buildRequestUri();
  ...
  @Override public String callUpdateHandler(final UpdateHandlerRequest req) {
    ...
    return restTemplate.put(req.buildRequestUri(), serializeUpdateHandlerRequestBody(req.getBody()), new StdResponseHandler<String>() {
      @Override public String success(HttpResponse hr) throws JsonProcessingException, IOException {
        return IOUtils.toString(hr.getContent(), "UTF-8");
      }
    });
  }

  @Override public <T extends java.lang.Object> T callUpdateHandler(final UpdateHandlerRequest req, final Class<T> c) {
    Assert.hasText(req.getDesignDocId(), "designDocID may not be null or empty");
    Assert.hasText(req.getFunctionName(), "functionName may not be null or empty");
    Assert.hasText(req.getDocId(), "docId may not be null or empty");
    req.dbPath(dbURI.toString());
    return restTemplate.put(req.buildRequestUri(), serializeUpdateHandlerRequestBody(req.getBody()), new StdResponseHandler<T>() {
      @Override public T success(HttpResponse hr) throws Exception {
        return objectMapper.readValue(hr.getContent(), c);
      }
  ...
  @Override public void ensureFullCommit() {
