...
public class ProcessServiceImpl implements ProcessService {
  ...
  @Autowired private ProcessDAO processDAO;
  ...
  @Autowired private MetadataFieldService metadataFieldService;

  @Autowired private EPersonService ePersonService;

  @Override public Process create(Context context, EPerson ePerson, String scriptName, List<DSpaceCommandLineParameter> parameters, final Set<Group> specialGroups) throws SQLException {
    ...
    Optional.ofNullable(specialGroups).ifPresent((sg) -> {
    ...
    log.info(LogHelper.getHeader(context, "process_create", "Process has been created for eperson with email " + ePerson.getEmail() + " with ID " + createdProcess.getID() + " and scriptName " + scriptName + " and parameters " + parameters));
    return createdProcess;
  }

  @Override public Process find(Context context, int processId) throws SQLException {
    return processDAO.findByID(context, Process.class, processId);
  }

  @Override public List<Process> findAll(Context context) throws SQLException {
    return processDAO.findAll(context, Process.class);
  }

  @Override public List<Process> findAll(Context context, int limit, int offset) throws SQLException {
    return processDAO.findAll(context, limit, offset);
  }

  @Override public List<Process> findAllSortByScript(Context context) throws SQLException {
    return processDAO.findAllSortByScript(context);
  }

  @Override public List<Process> findAllSortByStartTime(Context context) throws SQLException {
  ...
  @Override public void start(Context context, Process process) throws SQLException {
  ...
  @Override public void fail(Context context, Process process) throws SQLException {
  ...
  @Override public void complete(Context context, Process process) throws SQLException {
  ...
  @Override public void appendFile(Context context, Process process, InputStream is, String type, String fileName) throws IOException, SQLException, AuthorizeException {
    ...
    MetadataField dspaceProcessFileTypeField = metadataFieldService.findByString(context, Process.BITSTREAM_TYPE_METADATAFIELD, '.');
  ...
  @Override public void delete(Context context, Process process) throws SQLException, IOException, AuthorizeException {
  ...
  @Override public void update(Context context, Process process) throws SQLException {
    processDAO.save(context, process);
  }

  @Override public List<DSpaceCommandLineParameter> getParameters(Process process) {
  ...
  @Override public Bitstream getBitstreamByName(Context context, Process process, String bitstreamName) {
  ...
  @Override public Bitstream getBitstream(Context context, Process process, String type) {
    ...
    if (type == null) {
      ...
      if (allBitstreams != null) {
        for (Bitstream bitstream : allBitstreams) {
          if (StringUtils.equals(bitstreamService.getMetadata(bitstream, Process.BITSTREAM_TYPE_METADATAFIELD), type)) {
            return bitstream;
          }
        }
      }
  ...
  @Override public List<Bitstream> getBitstreams(Context context, Process process) {
  ...
  @Override public List<String> getFileTypesForProcessBitstreams(Context context, Process process) {
    List<Bitstream> list = getBitstreams(context, process);
    Set<String> fileTypesSet = new HashSet<>();
    for (Bitstream bitstream : list) {
      List<MetadataValue> metadata = bitstreamService.getMetadata(bitstream, Process.BITSTREAM_TYPE_METADATAFIELD, Item.ANY);
  ...
  @Override public List<Process> search(Context context, ProcessQueryParameterContainer processQueryParameterContainer, int limit, int offset) throws SQLException {
    return processDAO.search(context, processQueryParameterContainer, limit, offset);
  }

  @Override public int countSearch(Context context, ProcessQueryParameterContainer processQueryParameterContainer) throws SQLException {
    return processDAO.countTotalWithParameters(context, processQueryParameterContainer);
  }

  @Override public void appendLog(int processId, String scriptName, String output, ProcessLogLevel processLogLevel) throws IOException {
  ...
  @Override public void createLogBitstream(Context context, Process process) throws IOException, SQLException, AuthorizeException {
    File tmpDir = FileUtils.getTempDirectory();
  ...
  @Override public List<Process> findByStatusAndCreationTimeOlderThan(Context context, List<ProcessStatus> statuses, Date date) throws SQLException {
