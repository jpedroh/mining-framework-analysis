...
public class METSManifest {
  ...
  public List getMdFiles() throws MetadataValidationException {
    if (mdFiles == null) {
      XPathExpression<Element> xpath = XPathFactory.instance().compile("descendant::mets:mdRef", Filters.element(), null, metsNS);
  ...
  public String getOriginalFilePath(Element file) {
    ...
    XPathExpression<Element> xpath = XPathFactory.instance().compile("mets:fileSec/mets:fileGrp[@USE=\"CONTENT\"]/mets:file[@GROUPID=\"" + groupID + "\"]", Filters.element(), null, metsNS);
  ...
  public InputStream getMdContentAsStream(Element mdSec, Mdref callback) throws MetadataValidationException, PackageValidationException, IOException, SQLException, AuthorizeException {
    ...
    if (mdWrap != null) {
      ...
      if (xmlData == null) {
        ...
        return new ByteArrayInputStream(outputPretty.outputString(xmlData.getChildren()).getBytes(StandardCharsets.UTF_8));
      }
    } else {
      mdRef = mdSec.getChild("mdRef", metsNS);
      if (mdRef != null) {
  ...
  protected Element getElementByXPath(String path, boolean nullOk) throws MetadataValidationException {
    XPathExpression<Element> xpath = XPathFactory.instance().compile(path, Filters.element(), null, metsNS, xlinkNS);
  ...
  public InputStream getMetsAsStream() {
    XMLOutputter outputPretty = new XMLOutputter(Format.getPrettyFormat());
    return new ByteArrayInputStream(outputPretty.outputString(mets).getBytes(StandardCharsets.UTF_8));
  }
}