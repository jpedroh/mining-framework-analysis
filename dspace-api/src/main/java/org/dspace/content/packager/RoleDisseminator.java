...
public class RoleDisseminator implements PackageDisseminator {
  ...
  @Override public void disseminate(Context context, DSpaceObject object, PackageParameters params, File pkgFile) throws PackageException, CrosswalkException, AuthorizeException, SQLException, IOException {
  ...
  protected class Serializer implements Runnable {
    ...
    @SuppressWarnings(value = { "unused" }) private Serializer() {
    }
  ...
  protected void writeToStream(Context context, DSpaceObject object, OutputStream stream, boolean emitPasswords) throws PackageException {
    try {
      ...
      if ((groups != null && groups.size() > 0) || (people != null && people.size() > 0)) {
  ...
  protected List<Group> findAssociatedGroups(Context context, DSpaceObject object) throws SQLException {
    if (object.getType() == Constants.SITE) {
      ...
      if (object.getType() == Constants.COMMUNITY) {
