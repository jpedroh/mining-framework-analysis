...
public class CitationDocumentServiceImpl implements CitationDocumentService, InitializingBean {
  ...
  @Override public void afterPropertiesSet() throws Exception {
    ...
    if (pageformatCfg != null) {
      if (pageformatCfg.equalsIgnoreCase("A4")) {
        ...
        if (!pageformatCfg.equalsIgnoreCase("LETTER")) {
  ...
  @Override public Boolean isCitationEnabledForBitstream(Bitstream bitstream, Context context) throws SQLException {
  ...
  @Override public boolean canGenerateCitationVersion(Context context, Bitstream bitstream) throws SQLException {
    return VALID_TYPES.contains(bitstream.getFormat(context).getMIMEType());
  }

  @Override public Pair<byte[], Long> makeCitedDocument(Context context, Bitstream bitstream) throws IOException, SQLException, AuthorizeException {
...
>>>>>>> /usr/src/app/output/dspace/dspace/c65314db9d4f1df5539b4785a5b234ee3ab8a2a5/dspace-api/src/main/java/org/dspace/disseminate/CitationDocumentServiceImpl.java/right.java
    ...
    try {
      ...
      for (String field : fields) {
        ...
        if (field.contains("title")) {
          ...
          if (field.contains("creator") || field.contains("contributor")) {
            fontSize = 16;
          }
        }
        if (field.equals("_line_")) {
          ...
          if (StringUtils.isNotEmpty(itemService.getMetadata(item, field))) {
  ...
  @Override public int drawStringWordWrap(PDPage page, PDPageContentStream contentStream, String text, int startX, int startY, PDFont pdfFont, float fontSize) throws IOException {
  ...
  @Override public String getOwningCommunity(Context context, Item item) {
    try {
  ...
  @Override public String getOwningCollection(Item item) {
    return item.getOwningCollection().getName();
  }

  @Override public String getAllMetadataSeparated(Item item, String metadataKey) {
  ...
  @Override public void drawTable(PDPage page, PDPageContentStream contentStream, float y, float margin, String[][] content, PDFont font, int fontSize, boolean cellBorders) throws IOException {
