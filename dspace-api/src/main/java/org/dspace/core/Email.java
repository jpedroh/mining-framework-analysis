...
public class Email {
  ...
  public void addAttachment(InputStream is, String name, String mimetype) {
    if (null == mimetype) {
      LOG.error("Null MIME type replaced with \'" + DEFAULT_ATTACHMENT_TYPE + "\' for attachment \'" + name + "\'");
      mimetype = DEFAULT_ATTACHMENT_TYPE;
    } else {
      try {
        new ContentType(mimetype);
      } catch (ParseException ex) {
        LOG.error("Bad MIME type \'" + mimetype + "\' replaced with \'" + DEFAULT_ATTACHMENT_TYPE + "\' for attachment \'" + name + "\'", ex);
        mimetype = DEFAULT_ATTACHMENT_TYPE;
      }
    }
  ...
  public void send() throws MessagingException, IOException {
    ...
    while (i.hasNext()) {
      message.addRecipient(Message.RecipientType.TO, new InternetAddress(i.next()));
    }
    ...
    if (null == template) {
      ...
      StringResourceRepository repo = (StringResourceRepository) templateEngine.getApplicationAttribute(RESOURCE_REPOSITORY_NAME);
    ...
    if (attachments.isEmpty() && moreAttachments.isEmpty()) {
      ...
      for (InputStreamAttachment attachment : moreAttachments) {
        messageBodyPart = new MimeBodyPart();
        messageBodyPart.setDataHandler(new DataHandler(new InputStreamDataSource(attachment.name, attachment.mimetype, attachment.is)));
        messageBodyPart.setFileName(attachment.name);
        multipart.addBodyPart(messageBodyPart);
      }
  ...
  public static Email getEmail(String emailFile) throws IOException {
    ...
    try (InputStream is = new FileInputStream(emailFile); InputStreamReader ir = new InputStreamReader(is, "UTF-8"); BufferedReader reader = new BufferedReader(ir)) {
      boolean more = true;
      while (more) {
