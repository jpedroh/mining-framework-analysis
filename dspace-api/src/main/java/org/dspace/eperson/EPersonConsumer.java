...
public class EPersonConsumer implements Consumer {
  ...
  /**
     ...
     * @throws Exception if error
     */
  @Override public void consume(Context context, Event event) throws Exception {
      ...
      if (et == Event.CREATE) {
        ...
        if (!notifyRecipient.equals("")) {
          try {
            ...
            log.warn(LogHelper.getHeader(context, "error_emailing_administrator", ""), me);
          }
        }
        if (configurationService.getBooleanProperty("mail.welcome.enabled", false)) {
          ...
          if (StringUtils.isNotBlank(addressee)) {
            ...
            try {
              ...
              log.warn("Welcome message not sent to {}:  {}", addressee, ex.getMessage());
            }
          } else {
            log.warn("Welcome message not sent to EPerson {} because it has no email address.", eperson.getID().toString());
          }
        }
      } else {
        if (et == Event.DELETE) {
  ...
  /**
     ...
     * @throws Exception if error
     */
  @Override public void end(Context ctx) throws Exception {
  }
  ...
  /**
     ...
     * @param ctx The relevant DSpace Context.
     */
  @Override public void finish(Context ctx) {
  }
}