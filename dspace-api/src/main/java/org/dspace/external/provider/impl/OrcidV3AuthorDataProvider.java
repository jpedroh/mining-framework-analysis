...
public class OrcidV3AuthorDataProvider extends AbstractExternalDataProvider {
  ...
  @Override public String getSourceIdentifier() {
  ...
  public void init() throws IOException {
    if (StringUtils.isNotBlank(clientSecret) && StringUtils.isNotBlank(clientId) && StringUtils.isNotBlank(OAUTHUrl)) {
      ...
      try (InputStream is = getResponse.getEntity().getContent(); BufferedReader streamReader = new BufferedReader(new InputStreamReader(is, "UTF-8"))) {
        String inputStr;
  ...
  @Override public Optional<ExternalDataObject> getExternalDataObject(String id) {
  ...
  protected ExternalDataObject convertToExternalDataObject(Person person) {
    ...
    if (person.getName() != null) {
      ...
      externalDataObject.addMetadata(new MetadataValueDTO("dc", "identifier", "uri", null, orcidUrl + "/" + person.getName().getPath()));
  ...
  @Override public List<ExternalDataObject> searchExternalDataObjects(String query, int start, int limit) {
    if (limit > 100) {
  ...
  @Override public int getNumberOfResults(String query) {
  ...
  /**
     ...
     */
  @Autowired(required = true) public void setOrcidUrl(String orcidUrl) {
