...
public class SHERPAService {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unused" }) @PostConstruct private void init() {
    endpoint = configurationService.getProperty("sherpa.romeo.url", "https://v2.sherpa.ac.uk/cgi/retrieve");
  ...
  /**
     ...
     */
  @Cacheable(key = "#query", cacheNames = "sherpa.searchByJournalISSN") public SHERPAResponse searchByJournalISSN(String query) {
  ...
  public SHERPAPublisherResponse performPublisherRequest(String type, String field, String predicate, String value, int start, int limit) {
    ...
    while (numberOfTries < maxNumberOfTries && sherpaResponse == null) {
      numberOfTries++;
      log.debug(String.format("Trying to contact SHERPA/RoMEO - attempt %d of %d; timeout is %d; sleep between timeouts is %d", numberOfTries, maxNumberOfTries, timeout, sleepBetweenTimeouts));
      try {
