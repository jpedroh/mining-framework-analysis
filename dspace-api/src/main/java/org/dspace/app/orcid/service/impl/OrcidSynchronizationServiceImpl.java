...
public class OrcidSynchronizationServiceImpl implements OrcidSynchronizationService {
  ...
  @Override public void linkProfile(Context context, Item profile, OrcidTokenResponseDTO token) throws SQLException {
  ...
  @Override public void unlinkProfile(Context context, Item profile) throws SQLException {
  ...
  @Override public boolean setEntityPreference(Context context, Item profile, OrcidEntityType type, OrcidEntitySyncPreference value) throws SQLException {
  ...
  @Override public boolean setProfilePreference(Context context, Item profile, List<OrcidProfileSyncPreference> values) throws SQLException {
    List<String> valuesAsString = values.stream().map(OrcidProfileSyncPreference::name).collect(Collectors.toList());
    return updatePreferenceForSynchronizingWithOrcid(context, profile, "sync-profile", valuesAsString);
  }

  @Override public boolean setSynchronizationMode(Context context, Item profile, OrcidSynchronizationMode value) throws SQLException {
  ...
  @Override public boolean isSynchronizationEnabled(Item profile, Item item) {
    String entityType = itemService.getEntityType(item);
    if (entityType == null) {
      return false;
    }
    if (OrcidEntityType.isValidEntityType(entityType)) {
      return getEntityPreference(profile, OrcidEntityType.fromEntityType(entityType)).filter((pref) -> pref != DISABLED).isPresent();
    }
  ...
  @Override public Optional<OrcidSynchronizationMode> getSynchronizationMode(Item item) {
    return getMetadataValue(item, "dspace.orcid.sync-mode").map((metadataValue) -> metadataValue.getValue()).filter((value) -> isValidEnum(OrcidSynchronizationMode.class, value)).map((value) -> OrcidSynchronizationMode.valueOf(value));
  }

  @Override public Optional<OrcidEntitySyncPreference> getEntityPreference(Item item, OrcidEntityType entityType) {
    return getMetadataValue(item, "dspace.orcid.sync-" + entityType.name().toLowerCase() + "s").map((metadataValue) -> metadataValue.getValue()).filter((value) -> isValidEnum(OrcidEntitySyncPreference.class, value)).map((value) -> OrcidEntitySyncPreference.valueOf(value));
  }

  @Override public List<OrcidProfileSyncPreference> getProfilePreferences(Item item) {
    return getMetadataValues(item, "dspace.orcid.sync-profile").map(MetadataValue::getValue).filter((value) -> isValidEnum(OrcidProfileSyncPreference.class, value)).map((value) -> OrcidProfileSyncPreference.valueOf(value)).collect(Collectors.toList());
  }

  @Override public boolean isLinkedToOrcid(Item item) {
    return getOrcidAccessToken(item).isPresent() && getOrcid(item).isPresent();
  }

  @Override public OrcidProfileDisconnectionMode getDisconnectionMode() {
  ...
  private boolean updatePreferenceForSynchronizingWithOrcid(Context context, Item profile, String metadataQualifier, List<String> values) throws SQLException {
    ...
    List<String> oldValues = itemService.getMetadata(profile, "dspace", "orcid", metadataQualifier, ANY).stream().map((metadataValue) -> metadataValue.getValue()).collect(Collectors.toList());
  ...
  private Optional<String> getOrcidAccessToken(Item item) {
    return getMetadataValue(item, "dspace.orcid.access-token").map((metadataValue) -> metadataValue.getValue());
  }

  public Optional<String> getOrcid(Item item) {
    return getMetadataValue(item, "person.identifier.orcid").map((metadataValue) -> metadataValue.getValue());
  }
