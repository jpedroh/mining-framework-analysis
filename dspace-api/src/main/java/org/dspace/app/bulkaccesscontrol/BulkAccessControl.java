...
public class BulkAccessControl extends DSpaceRunnable<BulkAccessControlScriptConfiguration<BulkAccessControl>> {
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public void setup() throws ParseException {
    ...
    this.bulkAccessConditionConfigurationService = new DSpace().getServiceManager().getServiceByName("bulkAccessConditionConfigurationService", BulkAccessConditionConfigurationService.class);
    this.dSpaceObjectUtils = new DSpace().getServiceManager().getServiceByName(DSpaceObjectUtilsImpl.class.getName(), DSpaceObjectUtilsImpl.class);
    BulkAccessConditionConfiguration bulkAccessConditionConfiguration = bulkAccessConditionConfigurationService.getBulkAccessConditionConfiguration("default");
    itemAccessConditions = bulkAccessConditionConfiguration.getItemAccessConditionOptions().stream().collect(Collectors.toMap(AccessConditionOption::getName, Function.identity()));
    uploadAccessConditions = bulkAccessConditionConfiguration.getBitstreamAccessConditionOptions().stream().collect(Collectors.toMap(AccessConditionOption::getName, Function.identity()));
    filename = commandLine.getOptionValue('f');
    uuids = commandLine.hasOption('u') ? Arrays.asList(commandLine.getOptionValues('u')) : null;
  }

  @Override public void internalRun() throws Exception {
  ...
  private void validateItemNode(AccessConditionItem item) {
    ...
    if (StringUtils.isEmpty(mode)) {
      ...
      if (!(StringUtils.equalsAny(mode, ADD_MODE, REPLACE_MODE))) {
        handler.logError("wrong value for item mode<" + mode + ">");
        throw new BulkAccessControlException("wrong value for item mode<" + mode + ">");
      } else {
        if (ADD_MODE.equals(mode) && CollectionUtils.isEmpty(accessConditions)) {
          handler.logError("accessConditions of item must be provided with mode<" + ADD_MODE + ">");
          throw new BulkAccessControlException("accessConditions of item must be provided with mode<" + ADD_MODE + ">");
        }
      }
    }
    ...
    if (StringUtils.isEmpty(mode)) {
      ...
      if (!(StringUtils.equalsAny(mode, ADD_MODE, REPLACE_MODE))) {
        ...
        if (ADD_MODE.equals(mode) && CollectionUtils.isEmpty(accessConditions)) {
          ...
          throw new BulkAccessControlException("accessConditions of bitstream must be provided with mode<" + ADD_MODE + ">");
        }
      }
    }
...
<<<<<<< /usr/src/app/output/dspace/dspace/fd388620c086ab7dbfe9a30fc3e6ef4d0cb55ba1/dspace-api/src/main/java/org/dspace/app/bulkaccesscontrol/BulkAccessControl.java/left.java
      if (uuids.length == 1 && containsConstraints(bitstream)) {
        DSpaceObject dso = dSpaceObjectUtils.findDSpaceObject(context, UUID.fromString(uuids[0]));
...
=======
      {
        DSpaceObject dso = dSpaceObjectUtils.findDSpaceObject(context, UUID.fromString(uuids.get(0)));
        if (Objects.nonNull(dso) && dso.getType() != Constants.ITEM) {
          handler.logError("constraint is not supported when uuid isn\'t an Item");
          throw new BulkAccessControlException("constraint is not supported when uuid isn\'t an Item");
        }
...
>>>>>>> /usr/src/app/output/dspace/dspace/fd388620c086ab7dbfe9a30fc3e6ef4d0cb55ba1/dspace-api/src/main/java/org/dspace/app/bulkaccesscontrol/BulkAccessControl.java/right.java
  ...
  private void validateAccessCondition(AccessCondition accessCondition) {
    ...
    try {
      itemAccessConditions.get(accessCondition.getName()).validateResourcePolicy(context, accessCondition.getName(), accessCondition.getStartDate(), accessCondition.getEndDate());
    } catch (Exception e) {
  ...
  public void updateItemsAndBitstreamsPolices(AccessControl accessControl) throws SQLException, SearchServiceException, AuthorizeException {
    int counter = 0;
    int start = 0;
    int limit = 20;
  ...
  private Iterator<Item> findItems(String query, int start, int limit) throws SearchServiceException {
    ...
    return searchService.search(context, discoverQuery).getIndexableObjects().stream().map((indexableObject) -> ((IndexableItem) indexableObject).getIndexedObject()).collect(Collectors.toList()).iterator();
  }
  ...
  private void setItemPolicies(Item item, AccessControl accessControl) throws SQLException, AuthorizeException {
    ...
    accessControl.getItem().getAccessConditions().forEach((accessCondition) -> createResourcePolicy(item, accessCondition, itemAccessConditions.get(accessCondition.getName())));
...
<<<<<<< /usr/src/app/output/dspace/dspace/fd388620c086ab7dbfe9a30fc3e6ef4d0cb55ba1/dspace-api/src/main/java/org/dspace/app/bulkaccesscontrol/BulkAccessControl.java/left.java
    if (containsConstraints(accessControl.getBitstream())) {
      findMatchedBitstreams(item, accessControl.getBitstream().getConstraints().getUuid()).forEach((bitstream) -> updateBitstreamPolicies(bitstream, item, accessControl));
    } else {
      findAllBitstreams(item).forEach((bitstream) -> updateBitstreamPolicies(bitstream, item, accessControl));
    }
=======
    item.getBundles(Constants.CONTENT_BUNDLE_NAME).stream().flatMap((bundle) -> bundle.getBitstreams().stream()).filter((bitstream) -> uuids == null || uuids.size() == 0 || uuids.contains(bitstream.getID().toString())).forEach((bitstream) -> updateBitstreamPolicies(bitstream, item, accessControl));
>>>>>>> /usr/src/app/output/dspace/dspace/fd388620c086ab7dbfe9a30fc3e6ef4d0cb55ba1/dspace-api/src/main/java/org/dspace/app/bulkaccesscontrol/BulkAccessControl.java/right.java
  }

  private boolean containsConstraints(AccessConditionBitstream bitstream) {
    return Objects.nonNull(bitstream) && Objects.nonNull(bitstream.getConstraints()) && isNotEmpty(bitstream.getConstraints().getUuid());
  }
  ...
  private void setBitstreamPolicies(Bitstream bitstream, Item item, AccessControl accessControl) throws SQLException, AuthorizeException {
    AccessConditionBitstream bitstreamControl = accessControl.getBitstream();
    accessControl.getBitstream().getAccessConditions().forEach((accessCondition) -> createResourcePolicy(bitstream, accessCondition, uploadAccessConditions.get(accessCondition.getName())));
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public BulkAccessControlScriptConfiguration<BulkAccessControl> getScriptConfiguration() {
    return new DSpace().getServiceManager().getServiceByName("bulk-access-control", BulkAccessControlScriptConfiguration.class);
  }
}