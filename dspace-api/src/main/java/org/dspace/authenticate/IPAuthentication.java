  ...
  protected void addMatchers(String groupName, String[] ipRanges) {
    for (String entry : ipRanges) {
      try {
        ...
        log.warn("Malformed IP range specified for group " + groupName, ipme);
      }
    }
  }
  ...
  @Override public List<Group> getSpecialGroups(Context context, HttpServletRequest request) throws SQLException {
    ...
    for (IPMatcher ipm : ipMatchers) {
      try {
        ...
        log.warn(LogHelper.getHeader(context, "configuration_error", "bad_ip=" + addr), ipme);
      }
    }
    ...
    if (log.isDebugEnabled()) {
      ...
      log.debug(LogHelper.getHeader(context, "authenticated", "special_groups=" + gsb.toString() + " (by IP=" + addr + ", useProxies=" + useProxies.toString() + ")"));
    }
    return groups;
  }
