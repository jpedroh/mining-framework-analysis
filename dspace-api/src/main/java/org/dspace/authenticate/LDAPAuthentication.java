    ...
    try {
      ...
      if (!context.getCurrentUser().getNetid().equals("")) {
        ...
        if ((groupName != null) && (!groupName.trim().equals(""))) {
          ...
          if (ldapGroup == null) {
            log.warn(LogManager.getHeader(context, "ldap_specialgroup", "Group defined in login.specialgroup does not exist"));
            return Collections.EMPTY_LIST;
          } else {
      ...
      if (ldap.ldapAuthenticate(dn, password, context)) {
        ...
        if (StringUtils.isEmpty(email)) {
          if (configurationService.hasProperty("authentication-ldap.netid_email_domain")) {
            ...
            log.warn(LogManager.getHeader(context, "autoregister", "Unable to locate email address for account \'" + netid + "\', so" + " it has been set to \'" + netid + "\'. " + "Please check the LDAP \'email_field\' OR consider " + "configuring \'netid_email_domain\'."));
            email = netid;
          }
        }
    ...
    protected String getDNOfUser(String adminUser, String adminPassword, Context context, String netid) {
      ...
      try {
        ...
        if (ldap_search_scope != null) {
          log.warn(LogManager.getHeader(context, "ldap_authentication", "invalid search scope: " + ldap_search_scope));
        }
      }
        ...
        if (cmp) {
          try {
            ...
            if (ldapGroup != null) {
              ...
              log.warn(LogManager.getHeader(context, "ldap_assignGroupsBasedOnLdapDn", "Group defined in authentication-ldap.login.groupmap." + i + " does not exist :: " + dspaceGroupName));
            }
