...
public class ShibAuthentication implements AuthenticationMethod {
  ...
  @Override public int authenticate(Context context, String username, String password, String realm, HttpServletRequest request) throws SQLException {
    boolean swordCompatibility = configurationService.getBooleanProperty("authentication-shibboleth.sword.compatibility", true);
    ...
    if (log.isDebugEnabled()) {
      ...
      @SuppressWarnings(value = { "unchecked" }) Enumeration<String> headerNames = request.getHeaderNames();
      while (headerNames.hasMoreElements()) {
        String headerName = headerNames.nextElement();
        @SuppressWarnings(value = { "unchecked" }) Enumeration<String> headerValues = request.getHeaders(headerName);
      ...
      if (affiliations != null) {
        for (String affiliation : affiliations) {
          ...
          if (groupNames == null || groupNames.length == 0) {
            groupNames = configurationService.getArrayProperty("authentication-shibboleth.role." + affiliation.toLowerCase());
          }
          if (groupNames == null) {
          ...
          for (int i = 0; i < groupNames.length; i++) {
            try {
              ...
              log.error("Exception thrown while trying to lookup affiliation role for group name: \'" + groupNames[i].trim() + "\'", sqle);
            }
          }
        }
      }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  /**
     ...
     * @throws SQLException if database error
     */
  @Override public boolean allowSetPassword(Context context, HttpServletRequest request, String email) throws SQLException {
    return false;
  }
  ...
  @Override public String loginPageURL(Context context, HttpServletRequest request, HttpServletResponse response) {
    ...
    if (lazySession) {
      ...
      if (request.getHeader("Referer") != null && StringUtils.isNotBlank(request.getHeader("Referer"))) {
        ...
        if (request.getHeader("X-Requested-With") != null && StringUtils.isNotBlank(request.getHeader("X-Requested-With"))) {
          redirectUrl = request.getHeader("X-Requested-With");
        }
      }
      String returnURL = configurationService.getProperty("dspace.server.url") + "/api/authn/shibboleth" + ((redirectUrl != null) ? "?redirectUrl=" + redirectUrl : "");
      try {
  ...
  protected EPerson findEPerson(Context context, HttpServletRequest request) throws SQLException, AuthorizeException {
    boolean isUsingTomcatUser = configurationService.getBooleanProperty("authentication-shibboleth.email-use-tomcat-remote-user");
  ...
  protected void updateEPerson(Context context, HttpServletRequest request, EPerson eperson) throws SQLException, AuthorizeException {
    ...
    for (String header : metadataHeaderMap.keySet()) {
      ...
      if (value == null) {
        log.warn("Unable to update the eperson\'s \'{}\' metadata" + " because the header \'{}\' does not exist.", field, header);
        continue;
      } else {
        if ("phone".equals(field) && value.length() > PHONE_MAX_SIZE) {
          ...
          if (value.length() > METADATA_MAX_SIZE) {
            log.warn("Truncating eperson {} metadata because it is longer than {}: \'{}\'", field, METADATA_MAX_SIZE, value);
            value = value.substring(0, METADATA_MAX_SIZE);
          }
        }
      }
      String[] nameParts = MetadataFieldName.parse(field);
      ePersonService.setMetadataSingleValue(context, eperson, nameParts[0], nameParts[1], nameParts[2], value, null);
      log.debug("Updated the eperson\'s \'{}\' metadata using header: \'{}\' = \'{}\'.", field, header, value);
    }
    ePersonService.update(context, eperson);
    context.dispatchEvents();
    context.restoreAuthSystemState();
  ...
  protected synchronized void initialize(Context context) throws SQLException {
    ...
    boolean autoCreate = configurationService.getBooleanProperty("authentication-shibboleth.eperson.metadata.autocreate", true);
    ...
    for (String metadataString : mappingString) {
      ...
      if (valid) {
        ...
        log.error("Skipping the additional eperson metadata mapping for: \'{}\' = \'{}\'" + " because the field is not supported by the current configuration.", header, name);
      }
    }
    metadataHeaderMap = map;
  }
  ...
  protected synchronized boolean checkIfEpersonMetadataFieldExists(Context context, String metadataName) throws SQLException {
    ...
    MetadataField metadataField = metadataFieldService.findByElement(context, MetadataSchemaEnum.EPERSON.getName(), metadataName, null);
  ...
  protected String findAttribute(HttpServletRequest request, String name) {
    ...
    boolean reconvertAttributes = configurationService.getBooleanProperty("authentication-shibboleth.reconvert.attributes", false);
    if (!StringUtils.isEmpty(value) && reconvertAttributes) {
      try {
        value = new String(value.getBytes("ISO-8859-1"), "UTF-8");
      } catch (UnsupportedEncodingException ex) {
        log.warn("Failed to reconvert shibboleth attribute (" + name + ").", ex);
      }
    }
    return value;
  }
  ...
  private String getShibURL(HttpServletRequest request) {
    ...
    boolean forceHTTPS = configurationService.getBooleanProperty("authentication-shibboleth.lazysession.secure", true);
  ...
  @Override public boolean isUsed(final Context context, final HttpServletRequest request) {
    if (request != null && context.getCurrentUser() != null && request.getAttribute("shib.authenticated") != null) {
      return true;
    }
    return false;
  }
