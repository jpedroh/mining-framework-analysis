...
public class StatisticsDataVisits extends StatisticsData {
  ...
  @Override public Dataset createDataset(Context context, int facetMinCount) throws SQLException, SolrServerException, ParseException, IOException {
    if (getDataset() != null) {
      return getDataset();
    }
    ...
    if (dateFacet != null && dateFacet.getActualStartDate() != null && dateFacet.getActualEndDate() != null) {
      ...
      if (dateFacet != null && dateFacet.getStartDate() != null && dateFacet.getEndDate() != null) {
        StatisticsSolrDateFilter dateFilter = new StatisticsSolrDateFilter();
    ...
    if (dateFacet != null) {
      for (int i = 0; i < datasetQueries.size(); i++) {
        ...
        if (dataSetQuery.getQueries().size() != 1) {
          ...
          if (dataSetQuery.getMax() == -1) {
            ObjectCount[] results = solrLoggerService.queryFacetDate(query, filterQuery, dataSetQuery.getMax(), dateFacet.getDateType(), dateFacet.getStartDate(), dateFacet.getEndDate(), showTotal, context, facetMinCount);
            ...
            ObjectCount[] maxObjectCounts = solrLoggerService.queryFacetField(query, filterQuery, dataSetQuery.getFacetField(), dataSetQuery.getMax(), false, null, facetMinCount);
            for (int j = 0; j < maxObjectCounts.length; j++) {
              ...
              ObjectCount[] maxDateFacetCounts = solrLoggerService.queryFacetDate(newQuery, filterQuery, dataSetQuery.getMax(), dateFacet.getDateType(), dateFacet.getStartDate(), dateFacet.getEndDate(), showTotal, context, facetMinCount);
              if (dataset == null) {
      ...
      ObjectCount[] topCounts1 = queryFacetField(firsDataset, firsDataset.getQueries().get(0).getQuery(), filterQuery, facetMinCount);
      if (datasetQueries.size() == 2) {
        DatasetQuery secondDataSet = datasetQueries.get(1);
        ObjectCount[] topCounts2 = queryFacetField(secondDataSet, secondDataSet.getQueries().get(0).getQuery(), filterQuery, facetMinCount);
        ...
        for (int i = 0; i < topCounts1.length; i++) {
          ...
          Map<String, Integer> facetResult = solrLoggerService.queryFacetQuery(query, filterQuery, facetQueries, facetMinCount);
  ...
  /**
     ...
     *
     * @param value        UUID (or legacy record ID) of the DSO in question.
     * @param datasetQuery FIXME: PLEASE DOCUMENT.
  ...
  protected ObjectCount[] queryFacetField(DatasetQuery dataset, String query, String filterQuery, int facetMinCount) throws SolrServerException, IOException {
    ...
    return solrLoggerService.queryFacetField(query, filterQuery, facetType, dataset.getMax(), false, null, facetMinCount);
  }
    ...
    public String getQuery() {
      ...
      if (owningDso != null && currentDso != null) {
        ...
        if (currentDso instanceof DSpaceObjectLegacySupport) {
          owningStr = "(" + owningStr + ":" + currentDso.getID() + " OR " + owningStr + ":" + ((DSpaceObjectLegacySupport) currentDso).getLegacyId() + ")";
        } else {
