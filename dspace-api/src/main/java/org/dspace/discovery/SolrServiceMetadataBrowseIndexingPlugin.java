...
public class SolrServiceMetadataBrowseIndexingPlugin implements SolrServiceIndexPlugin {
  ...
  @Override public void additionalIndex(Context context, IndexableObject indexableObject, SolrInputDocument document) {
        ...
        if (item.isArchived() || item.isWithdrawn()) {
          for (int mdIdx = 0; mdIdx < bi.getMetadataCount(); mdIdx++) {
            ...
            if (values != null && values.size() > 0) {
              ...
              boolean ignoreAuthority = DSpaceServicesFactory.getInstance().getConfigurationService().getPropertyAsType("discovery.browse.authority.ignore." + bi.getName(), DSpaceServicesFactory.getInstance().getConfigurationService().getPropertyAsType("discovery.browse.authority.ignore", Boolean.FALSE), true);
              for (int x = 0; x < values.size(); x++) {
                if (StringUtils.isEmpty(values.get(x).getValue())) {
                  ...
                  if ((ignoreAuthority && !bi.isAuthorityIndex()) || (values.get(x).getAuthority() != null && values.get(x).getConfidence() >= minConfidence)) {
                    ...
                    boolean ignorePrefered = DSpaceServicesFactory.getInstance().getConfigurationService().getPropertyAsType("discovery.browse.authority.ignore-prefered." + bi.getName(), DSpaceServicesFactory.getInstance().getConfigurationService().getPropertyAsType("discovery.browse.authority.ignore-prefered", Boolean.FALSE), true);
                    if (!ignorePrefered) {
                      try {
                        preferedLabel = choiceAuthorityService.getLabel(values.get(x), collection, values.get(x).getLanguage());
                      } catch (Exception e) {
                        log.warn("Failed to get preferred label for " + values.get(x).getMetadataField().toString('.'), e);
                      }
                    }
                    List<String> variants = null;
                    boolean ignoreVariants = DSpaceServicesFactory.getInstance().getConfigurationService().getPropertyAsType("discovery.browse.authority.ignore-variants." + bi.getName(), DSpaceServicesFactory.getInstance().getConfigurationService().getPropertyAsType("discovery.browse.authority.ignore-variants", Boolean.FALSE), true);
                    if (!ignoreVariants) {
                      try {
                        variants = choiceAuthorityService.getVariants(values.get(x), collection);
                      } catch (Exception e) {
                        log.warn("Failed to get variants for " + values.get(x).getMetadataField().toString(), e);
                      }
                    }
                    if (StringUtils.isNotBlank(preferedLabel)) {
                      String nLabel = OrderFormat.makeSortString(preferedLabel, values.get(x).getLanguage(), bi.getDataType());
                      distFValues.add(nLabel + SearchUtils.FILTER_SEPARATOR + preferedLabel + SearchUtils.AUTHORITY_SEPARATOR + values.get(x).getAuthority());
                      distValuesForAC.add(preferedLabel);
                    }
                    if (variants != null) {
                      for (String var : variants) {
                        String nVal = OrderFormat.makeSortString(var, values.get(x).getLanguage(), bi.getDataType());
                        distFValues.add(nVal + SearchUtils.FILTER_SEPARATOR + var + SearchUtils.AUTHORITY_SEPARATOR + values.get(x).getAuthority());
                    ...
                    distFValues.add(nVal + SearchUtils.FILTER_SEPARATOR + values.get(x).getValue());
