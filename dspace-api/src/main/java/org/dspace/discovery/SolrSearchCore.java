...
public class SolrSearchCore {
  ...
  @Autowired @Named(value = "solrHttpConnectionPoolService") protected HttpConnectionPoolService httpConnectionPoolService;
  ...
  protected void initSolr() {
    if (solr == null) {
      ...
      if (urlValidator.isValid(solrService) || configurationService.getBooleanProperty("discovery.solr.url.validation.enabled", true)) {
        try {
          ...
          SolrQuery solrQuery = new SolrQuery().setQuery(SearchUtils.RESOURCE_TYPE_FIELD + ":" + IndexableItem.TYPE + " AND " + SearchUtils.RESOURCE_ID_FIELD + ":1");
          ...
          log.error("Error while initializing solr server {}", solrService, e);
