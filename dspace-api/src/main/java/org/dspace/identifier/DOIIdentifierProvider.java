...
public class DOIIdentifierProvider extends FilteredIdentifierProvider {
  ...
  protected String getPrefix() {
    if (null == this.PREFIX) {
      ...
      if (null == this.PREFIX) {
        ...
        throw new RuntimeException("Unable to load DOI prefix from " + "configuration. Cannot find property " + CFG_PREFIX + ".");
      }
    }
    return this.PREFIX;
  }
  ...
  /**
     ...
     */
  @Override public void setFilterService(Filter filterService) {
  ...
  /**
     ...
     */
  @Override public String register(Context context, DSpaceObject dso) throws IdentifierException {
  ...
  /**
     ...
     * @throws IdentifierException
     */
  @Override public String register(Context context, DSpaceObject dso, boolean skipFilter) throws IdentifierException {
  ...
  /**
     ...
     * @throws IdentifierException
     */
  @Override public void register(Context context, DSpaceObject dso, String identifier, boolean skipFilter) throws IdentifierException {
    if (!(dso instanceof Item)) {
      return;
    }
  ...
  /**
     ...
     */
  @Override public void reserve(Context context, DSpaceObject dso, String identifier) throws IdentifierException, IllegalArgumentException {
  ...
  /**
     ...
     */
  @Override public void reserve(Context context, DSpaceObject dso, String identifier, boolean skipFilter) throws IdentifierException, IllegalArgumentException {
    String doi = doiService.formatIdentifier(identifier);
    DOI doiRow = null;
    try {
      doiRow = loadOrCreateDOI(context, dso, doi, skipFilter);
  ...
  public void reserveOnline(Context context, DSpaceObject dso, String identifier, boolean skipFilter) throws IdentifierException, IllegalArgumentException, SQLException {
    ...
    if (DELETED.equals(doiRow.getStatus()) || TO_BE_DELETED.equals(doiRow.getStatus())) {
      throw new DOIIdentifierException("You tried to reserve a DOI that " + "is marked as DELETED.", DOIIdentifierException.DOI_IS_DELETED);
    }
  ...
  public void updateMetadata(Context context, DSpaceObject dso, String identifier) throws IdentifierException, IllegalArgumentException, SQLException {
    ...
    if (IS_REGISTERED.equals(doiRow.getStatus())) {
      ...
      if (TO_BE_REGISTERED.equals(doiRow.getStatus())) {
        doiRow.setStatus(UPDATE_BEFORE_REGISTRATION);
      } else {
        if (IS_RESERVED.equals(doiRow.getStatus())) {
  ...
  public void updateMetadataOnline(Context context, DSpaceObject dso, String identifier) throws IdentifierException, SQLException {
    ...
    if (!Objects.equals(doiRow.getDSpaceObject(), dso)) {
      log.error("Refuse to update metadata of DOI {} with the metadata of " + " an object ({}/{}) the DOI is not dedicated to.", doi, contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso), dso.getID().toString());
      throw new DOIIdentifierException("Cannot update DOI metadata: " + "DOI and DSpaceObject does not match!", DOIIdentifierException.MISMATCH);
    }
    if (DELETED.equals(doiRow.getStatus()) || TO_BE_DELETED.equals(doiRow.getStatus())) {
      throw new DOIIdentifierException("You tried to update the metadata" + " of a DOI that is marked as DELETED.", DOIIdentifierException.DOI_IS_DELETED);
    }
    connector.updateMetadata(context, dso, doi);
    if (UPDATE_REGISTERED.equals(doiRow.getStatus())) {
      doiRow.setStatus(IS_REGISTERED);
    } else {
      if (UPDATE_BEFORE_REGISTRATION.equals(doiRow.getStatus())) {
        doiRow.setStatus(TO_BE_REGISTERED);
      } else {
        if (UPDATE_RESERVED.equals(doiRow.getStatus())) {
          doiRow.setStatus(IS_RESERVED);
        }
      }
    }
  ...
  /**
     ...
     * @throws IdentifierException
     */
  @Override public String mint(Context context, DSpaceObject dso, boolean skipFilter) throws IdentifierException {
    String doi = null;
    try {
      doi = getDOIByObject(context, dso);
    } catch (SQLException e) {
      log.error("Error while attemping to retrieve information about a DOI for " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + ".");
      throw new RuntimeException("Error while attempting to retrieve " + "information about a DOI for " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + ".", e);
    }
    if (null == doi) {
      try {
        ...
        throw new RuntimeException("Error while attempting to create a " + "new DOI for " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + ".", e);
      }
    }
    return doi;
  }
  ...
  /**
     ...
     */
  @Override public DSpaceObject resolve(Context context, String identifier, String... attributes) throws IdentifierNotFoundException, IdentifierNotResolvableException {
    String doi = null;
    try {
  ...
  @Override public String lookup(Context context, DSpaceObject dso) throws IdentifierNotFoundException, IdentifierNotResolvableException {
    ...
    if (null == doi) {
      throw new IdentifierNotFoundException("No DOI for DSpaceObject of type " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + " found.");
    }
    return doi;
  }
  ...
  @Override public void delete(Context context, DSpaceObject dso) throws IdentifierException {
    ...
    try {
      ...
      log.error("Error while removing a DOI out of the metadata of an " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + ".", ex);
      throw new RuntimeException("Error while removing a DOI out of the metadata of an " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + ".", ex);
    } catch (SQLException ex) {
      log.error("Error while removing a DOI out of the metadata of an " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + ".", ex);
      throw new RuntimeException("Error while removing a DOI out of the " + "metadata of an " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID() + ".", ex);
    }
  ...
  @Override public void delete(Context context, DSpaceObject dso, String identifier) throws IdentifierException {
    ...
    if (null != doiRow) {
      if (!Objects.equals(dso, doiRow.getDSpaceObject())) {
        throw new DOIIdentifierException("Trying to delete a DOI out of " + "an object that is not addressed by the DOI.", DOIIdentifierException.MISMATCH);
      }
    }
    try {
      ...
      throw new DOIIdentifierException("Not authorized to delete DOI.", ex, DOIIdentifierException.UNAUTHORIZED_METADATA_MANIPULATION);
    } catch (SQLException ex) {
  ...
  public void deleteOnline(Context context, String identifier) throws DOIIdentifierException {
    ...
    if (null == doiRow) {
      throw new DOIIdentifierException("This identifier: " + identifier + " isn\'t in our database", DOIIdentifierException.DOI_DOES_NOT_EXIST);
    }
    if (!TO_BE_DELETED.equals(doiRow.getStatus())) {
      log.error("This identifier: {} couldn\'t be deleted. Delete it first from metadata.", DOI.SCHEME + doiRow.getDoi());
      throw new IllegalArgumentException("Couldn\'t delete this identifier:" + DOI.SCHEME + doiRow.getDoi() + ". Delete it first from metadata.");
    }
  ...
  public String getDOIByObject(Context context, DSpaceObject dso) throws SQLException {
    ...
    if (doiRow.getDoi() == null) {
      log.error("A DOI with an empty doi column was found in the database. DSO-Type: " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + ", ID: " + dso.getID() + ".");
      throw new IllegalStateException("A DOI with an empty doi column was found in the database. DSO-Type: " + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + ", ID: " + dso.getID() + ".");
    }
    ...
    if (null != doiIdentifier) {
      ...
      if (null != doi) {
        if (doi.getDSpaceObject() == null) {
          if (doi.getResourceTypeId() != null && doi.getResourceTypeId() != dso.getType()) {
            throw new DOIIdentifierException("Cannot reassign" + " previously deleted DOI " + doiIdentifier + " as the resource types of the object it was" + " previously assigned to and the object it" + " shall be assigned to now differ (was: " + Constants.typeText[doi.getResourceTypeId()] + ", trying to assign to " + Constants.typeText[dso.getType()] + ").", DOIIdentifierException.DOI_IS_DELETED);
          } else {
          }
        } else {
          if (dso.getID().equals(doi.getDSpaceObject().getID())) {
            ...
            throw new DOIIdentifierException("Trying to create a DOI " + "that is already reserved for another object.", DOIIdentifierException.DOI_ALREADY_EXISTS);
          }
        }
      }
      if (skipFilter) {
      ...
      if (!doiIdentifier.startsWith(this.getPrefix() + "/")) {
        throw new DOIIdentifierException("Trying to create a DOI " + "that\'s not part of our Namespace!", DOIIdentifierException.FOREIGN_DOI);
      }
      if (doi == null) {
  ...
  /**
     ...
     */
  @Override public void checkMintable(Context context, DSpaceObject dso) throws DOIIdentifierNotApplicableException {
    if (this.filterService != null && contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso).equals("ITEM")) {
      try {
