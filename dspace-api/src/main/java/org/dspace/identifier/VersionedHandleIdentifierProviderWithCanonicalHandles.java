...
@Component public class VersionedHandleIdentifierProviderWithCanonicalHandles extends IdentifierProvider {
  /**
     * log4j category
     */
  private static final Logger log = org.apache.logging.log4j.LogManager.getLogger(VersionedHandleIdentifierProviderWithCanonicalHandles.class);
  ...
  @Autowired(required = true) private VersioningService versionService;

  @Autowired(required = true) private VersionHistoryService versionHistoryService;

  @Autowired(required = true) private HandleService handleService;

  @Autowired(required = true) private ItemService itemService;

  @Override public boolean supports(Class<? extends Identifier> identifier) {
  ...
  @Override public String register(Context context, DSpaceObject dso) {
  ...
  @Override public void register(Context context, DSpaceObject dso, String identifier) {
    try {
      Item item = (Item) dso;
  ...
  @Override public void reserve(Context context, DSpaceObject dso, String identifier) {
    try {
  ...
  /**
     ...
     */
  @Override public String mint(Context context, DSpaceObject dso) {
  ...
  @Override public DSpaceObject resolve(Context context, String identifier, String... attributes) {
    try {
      return handleService.resolveToObject(context, identifier);
    } catch (IllegalStateException | SQLException e) {
      log.error(LogHelper.getHeader(context, "Error while resolving handle to item", "handle: " + identifier), e);
    }
    return null;
  }

  @Override public String lookup(Context context, DSpaceObject dso) throws IdentifierNotFoundException, IdentifierNotResolvableException {
    try {
  ...
  @Override public void delete(Context context, DSpaceObject dso) throws IdentifierException {
    try {
      if (dso instanceof Item) {
        Item item = (Item) dso;
        VersionHistory history = versionHistoryService.findByItem(context, item);
        if (history != null && versionHistoryService.getLatestVersion(context, history).getItem().equals(item) && versionService.getVersionsByHistory(context, history).size() > 1) {
          Item previous;
          try {
  ...
  /**
     ...
     */
  protected void modifyHandleMetadata(Context context, Item item, String handle) throws SQLException, AuthorizeException {
    String handleref = handleService.getCanonicalForm(handle);
    List<MetadataValue> identifiers = itemService.getMetadata(item, MetadataSchemaEnum.DC.getName(), "identifier", "uri", Item.ANY);
    ...
    if (!StringUtils.isEmpty(handleref)) {
      itemService.addMetadata(context, item, MetadataSchemaEnum.DC.getName(), "identifier", "uri", null, handleref);
    }
