...
public class AbstractIntegrationTestWithDatabase extends AbstractDSpaceIntegrationTest {
  ...
  /**
     ...
     */
  @BeforeClass public static void initDatabase() {
    try {
      DatabaseUtils.updateDatabase();
    } catch (SQLException se) {
      log.error("Error initializing database", se);
      fail("Error initializing database: " + se.getMessage() + (se.getCause() == null ? "" : ": " + se.getCause().getMessage()));
    }
  }
  ...
  @After public void destroy() throws Exception {
    try {
      ...
      MockSolrSearchCore searchService = serviceManager.getServiceByName(null, MockSolrSearchCore.class);
      searchService.reset();
      serviceManager.getServiceByName(SolrStatisticsCore.class.getName(), MockSolrStatisticsCore.class).reset();
      MockSolrLoggerServiceImpl statisticsService = serviceManager.getServiceByName("solrLoggerService", MockSolrLoggerServiceImpl.class);
      statisticsService.reset();
      MockAuthoritySolrServiceImpl authorityService = serviceManager.getServiceByName(AuthoritySearchService.class.getName(), MockAuthoritySolrServiceImpl.class);
