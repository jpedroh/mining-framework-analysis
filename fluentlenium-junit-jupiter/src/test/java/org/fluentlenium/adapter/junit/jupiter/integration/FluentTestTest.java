...
@NotThreadSafe public class FluentTestTest {
  ...
  public static class InternalTest extends FluentTest {
    @Override public WebDriver newWebDriver() {
  ...
  @FluentConfiguration(driverLifecycle = DriverLifecycle.CLASS) @ExtendWith(value = MockitoExtension.class) public static class InternalTestSharedClass extends FluentTest {
    @Override public WebDriver newWebDriver() {
      WebDriver webDriver = Mockito.mock(WebDriver.class);
  ...
  @FluentConfiguration(driverLifecycle = DriverLifecycle.JVM) public static class InternalTestSharedOnce extends FluentTest {
    @Override public WebDriver newWebDriver() {
      WebDriver webDriver = Mockito.mock(WebDriver.class);
  ...
  @FluentConfiguration(driverLifecycle = DriverLifecycle.CLASS, deleteCookies = BooleanValue.TRUE) public static class ShouldDeleteCookiesTest extends FluentTest {
    @Override public WebDriver newWebDriver() {
      WebDriver webDriver = Mockito.mock(WebDriver.class);
  ...
  @Test public void testFluentTest() {
    LauncherDiscoveryRequest request = LauncherDiscoveryRequestBuilder.request().selectors(selectClass(InternalTest.class)).build();
    class Listener implements TestExecutionListener {
      public List<Throwable> failures = new ArrayList<>();

      @Override public void executionFinished(TestIdentifier identifier, TestExecutionResult result) {
  ...
  @Test public void testInternalTestSharedClass() {
  ...
  @Test public void testInternalTestSharedOnce() {
  ...
  @Test public void testShouldDeleteCookiesTest() {
  ...
  @Test public void testAutomaticScreenShotTest() throws IOException {
    SummaryGeneratingListener summaryGeneratingListener = getSummaryGeneratingListener(AutomaticScreenShotTest.class);
    assertAll("summary", () -> assertThat(summaryGeneratingListener.getSummary().getFailures()).hasSize(1), () -> assertThat(summaryGeneratingListener.getSummary().getFailures().get(0).getException().getMessage()).isEqualTo("Failing Test"));
