...
public class MoldUDP64Client implements Closeable {
  ...
  public MoldUDP64Client(DatagramChannel channel, SocketAddress requestAddress, MessageListener listener, MoldUDP64ClientStatusListener statusListener) {
    this(SystemClock.INSTANCE, channel, channel, requestAddress, listener, statusListener, 1);
  }
  ...
  public MoldUDP64Client(DatagramChannel channel, SocketAddress requestAddress, MessageListener listener, MoldUDP64ClientStatusListener statusListener, long requestedSequenceNumber) {
    this(SystemClock.INSTANCE, channel, channel, requestAddress, listener, statusListener, requestedSequenceNumber);
  }
  ...
  public MoldUDP64Client(DatagramChannel channel, DatagramChannel requestChannel, SocketAddress requestAddress, MessageListener listener, MoldUDP64ClientStatusListener statusListener) {
    this(SystemClock.INSTANCE, channel, requestChannel, requestAddress, listener, statusListener, 1);
  }
  ...
  public MoldUDP64Client(DatagramChannel channel, DatagramChannel requestChannel, SocketAddress requestAddress, MessageListener listener, MoldUDP64ClientStatusListener statusListener, long requestedSequenceNumber) {
    this(SystemClock.INSTANCE, channel, requestChannel, requestAddress, listener, statusListener, requestedSequenceNumber);
  }
    ...
    if (sequenceNumber > nextExpectedSequenceNumber) {
      ...
      if (requestUntilSequenceNumber != 0) {
        if (requestUntilSequenceNumber == REQUEST_UNTIL_SEQUENCE_NUMBER_UNKNOWN) {
          ...
          if (requestUntilSequenceNumber == nextSequenceNumber) {
            requestUntilSequenceNumber = 0;
            state(SYNCHRONIZED);
          } else {
  ...
  private void request(long requestFromSequenceNumber, long requestUntilSequenceNumber) throws IOException {
    int requestedMessageCount = (int) Math.min(requestUntilSequenceNumber - requestFromSequenceNumber, MAX_MESSAGE_COUNT);
    txBuffer.clear();
