...
public class GenericTest {
  ...
  @Before public void setUpClass() {
    GenericTest.injector = Guice.createInjector(new CoreJPADependencyModule(), new CoreJPATestPersistenceDependencyModule());
  ...
  @Test public void test1() {
    ...
    Context parent = contextBuilder.setName("parent name").setDescription("the parent description").setParentContextUID(null).build();
    parent = daoContext.create((ContextEntity) parent);
    contextBuilder.clear();
    Context child = contextBuilder.setName("child name").setDescription("the child description").setParentContextUID(parent.getUID()).build();
    child = daoContext.create((ContextEntity) child);
    builder.setOperationalContextUID(child.getUID()).setName("name").setCommercialName("commercial name").addContact(contactBuilder.setName("name").setEmail("email").setFax("fax").setMobile("mobile").setTelephone("phone").setWebsite("website"), true).setAddress(addressBuilder.setBuilding("building").setCity("city").setDetails("details").setISOCountry("PT").setNumber("number").setPostalCode("2345").setRegion("region").setStreetName("street")).setBillingAddress(addressBuilder2.setBuilding("building").setCity("city").setDetails("details").setISOCountry("PT").setNumber("number").setPostalCode("2345").setRegion("region").setStreetName("street")).addApplication(applicationBuilder.setDeveloperCompanyName("name").setDeveloperCompanyTaxIdentifier("taxid").setName("application name").setVersion("1.0").addContact(contactBuilder2.setName("name").setEmail("email").setFax("fax").setMobile("mobile").setTelephone("phone").setWebsite("website"))).setFinancialID("financial id", null);
    GenericTest.injector.getInstance(DAOBusiness.class).create((BusinessEntity) builder.build());
    taxBuilder.setCode("PT").setContextUID(child.getUID()).setCurrency(Currency.getInstance("EUR")).setDescription("description").setDesignation("designation").setTaxRate(TaxRateType.PERCENTAGE, BigDecimal.TEN).setValidFrom(new Date()).setValidTo(new Date()).setValue(BigDecimal.TEN);
