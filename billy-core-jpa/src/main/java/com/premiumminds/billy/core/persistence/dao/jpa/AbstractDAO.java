...
public abstract class AbstractDAO<TInterface extends BaseEntity, TEntity extends JPABaseEntity & BaseEntity> implements DAO<TInterface> {
  ...
  @SuppressWarnings(value = { "unchecked" }) protected <T2 extends BaseEntity> T2 checkEntity(Object candidate, Class<T2> entityClass) {
    if (candidate == null) {
      return null;
    }
  ...
  @SuppressWarnings(value = { "unchecked" }) protected <T2 extends BaseEntity> List<T2> checkEntityList(List<?> candidates, Class<T2> entityClass) {
    if (candidates == null) {
      return null;
    }
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public TInterface get(UID uid) throws NoResultException {
  ...
  protected TEntity getEntity(UID uid) throws NoResultException {
    ...
    try {
      result = this.getEntityManager().createQuery("select e from " + entityClass.getCanonicalName() + " e " + "where e.uid=:uid and e.active=true " + "order by e.entityVersion desc", entityClass).setParameter("uid", uid.toString()).setMaxResults(1).getSingleResult();
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public TInterface create(final TInterface entity) throws PersistenceException {
    if (!entity.isNew()) {
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public final synchronized TInterface update(final TInterface entity) throws PersistenceException {
    if (entity.isNew()) {
  ...
  @Override public boolean exists(UID uid) {
    Class<? extends TEntity> entityClass = this.getEntityClass();
    TEntity entity = null;
    try {
      entity = this.getEntityManager().createQuery("select e from " + entityClass.getCanonicalName() + " e " + "where e.uid=:uid", entityClass).setParameter("uid", uid.toString()).getSingleResult();
    } catch (NoResultException e) {
      return false;
    }
    return entity != null;
