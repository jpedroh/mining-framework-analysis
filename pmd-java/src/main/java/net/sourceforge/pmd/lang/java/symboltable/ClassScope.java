...
public class ClassScope extends AbstractJavaScope {
    ...
    if (occurrence.isMethodOrConstructorInvocation()) {
      for (MethodNameDeclaration mnd : methodDeclarations.keySet()) {
        if (mnd.getImage().equals(occurrence.getImage())) {
          ...
          if (!mnd.isVarargs() && occurrence.getArgumentCount() == mnd.getParameterCount() && (!getEnclosingScope(SourceFileScope.class).hasAuxclasspath() || parameterTypes.equals(argumentTypes))) {
            return mnd;
          } else {
            if (mnd.isVarargs()) {
              int varArgIndex = parameterTypes.size() - 1;
              TypedNameDeclaration varArgType = parameterTypes.get(varArgIndex);
              if ((varArgIndex == 0 || argumentTypes.size() >= varArgIndex) && (!getEnclosingScope(SourceFileScope.class).hasAuxclasspath() || parameterTypes.subList(0, varArgIndex).equals(argumentTypes.subList(0, varArgIndex)))) {
  ...
  private String qualifyTypeName(String typeImage) {
    ...
    for (String qualified : this.getEnclosingScope(SourceFileScope.class).getQualifiedTypeNames().keySet()) {
      ...
      if (qualified.endsWith(typeImage) && (fullLength == nameLength || qualified.substring(0, fullLength - nameLength).endsWith("."))) {
        return qualified;
      }
    }
      ...
      for (int i = 0; i < arguments.jjtGetNumChildren(); i++) {
        ...
        if (child instanceof ASTName) {
          ...
          if (child instanceof ASTLiteral) {
            ...
            if (child instanceof ASTAllocationExpression && child.jjtGetChild(0) instanceof ASTClassOrInterfaceType) {
