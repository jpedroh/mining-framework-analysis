 ...
 */@Deprecated public enum SignatureAlgorithm {
  ...
  private static final List<SignatureAlgorithm> PREFERRED_EC_ALGS = Collections.unmodifiableList(Arrays.asList(SignatureAlgorithm.ES512, SignatureAlgorithm.ES384, SignatureAlgorithm.ES256));
  ...
  /**
     ...
     */
  @Deprecated private final String pkcs12Name;

  SignatureAlgorithm(String value, String description, String familyName, String jcaName, boolean jdkStandard, int digestLength, int minKeyLength) {
    this(value, description, familyName, jcaName, jdkStandard, digestLength, minKeyLength, jcaName);
  }

  SignatureAlgorithm(String value, String description, String familyName, String jcaName, boolean jdkStandard, int digestLength, int minKeyLength, String pkcs12Name) {
    this.value = value;
    ...
    if (this == NONE) {
      ...
      if (isHmac()) {
        ...
        if (!HS256.jcaName.equalsIgnoreCase(alg) && !HS384.jcaName.equalsIgnoreCase(alg) && !HS512.jcaName.equalsIgnoreCase(alg) && !HS256.pkcs12Name.equals(alg) && !HS384.pkcs12Name.equals(alg) && !HS512.pkcs12Name.equals(alg)) {
          throw new InvalidKeyException("The " + keyType(signing) + " key\'s algorithm \'" + alg + "\' does not equal a valid HmacSHA* algorithm name and cannot be used with " + name() + ".");
        }
