...
=======
  ...
  private LinkedHashMap<Link, Boolean> getFirstPhysicalPath(LinkedHashMap<Link, Boolean> currentPath, Switch sw, Node dst) {
    for (Link l : net.getConnectedLinks(sw)) {
      ...
      List<Link> tmpListToRemove = new ArrayList<>(currentPath.keySet());
      currentPath.remove(tmpListToRemove.get(tmpListToRemove.size() - 1));
    }
    return new LinkedHashMap<>();
  }
>>>>>>> /usr/src/app/output/btrplace/scheduler/d42500d47b1f8f88aada61e71e4880f680cee32f/api/src/main/java/org/btrplace/model/view/network/StaticRouting.java/right.java
