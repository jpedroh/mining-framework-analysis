...
public class XmlExtractorFactoryTest extends BaseExtractorFactoryTest<StreamConnector, XmlExtractorFactory> {
  ...
  @Override protected XmlExtractorFactory createExtractorFactory() {
  ...
  @Override public void setUpExtractorFactory() {
    super.setUpExtractorFactory();
    try {
      when(getConnectorMock().getInputStream(anyMap())).thenReturn(new ByteArrayInputStream(XML.getBytes()));
    } catch (IOException e) {
  ...
  @Test(expected = IllegalArgumentException.class) public void testCreateExtractorWithEmptyXPathExpression() {
