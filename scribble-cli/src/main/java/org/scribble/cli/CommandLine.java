...
public class CommandLine {
  ...
  protected MainContext newMainContext() throws ScribParserException, ScribbleException {
    ...
    if (this.args.containsKey(CLArgFlag.INLINE_MAIN_MOD)) {
      return new MainContext(debug, locator, this.args.get(CLArgFlag.INLINE_MAIN_MOD)[0], useOldWF, noLiveness, minEfsm, fair, noLocalChoiceSubjectCheck, noAcceptCorrelationCheck, noValidation);
    } else {
  ...
  public void run() throws CommandLineException, AntlrSourceException {
    try {
      try {
        runBody();
  ...
  protected void doValidationTasks(Job job) throws AntlrSourceException, ScribParserException, CommandLineException {
    {
