...
public class CommandLine {
  protected Map<CLArgFlag, String[]> args;
  ...
  protected MainContext newMainContext() throws ScribParserException, ScribbleException {
    ...
    List<Path> impaths = this.args.containsKey(CLArgFlag.IMPORT_PATH) ? CommandLine.parseImportPaths(this.args.get(CLArgFlag.IMPORT_PATH)[0]) : Collections.emptyList();
    ResourceLocator locator = new DirectoryResourceLocator(impaths);
    if (this.args.containsKey(CLArgFlag.INLINE_MAIN_MOD)) {
      return new MainContext(debug, locator, this.args.get(CLArgFlag.INLINE_MAIN_MOD)[0], useOldWF, noLiveness, minEfsm, fair, noLocalChoiceSubjectCheck, noAcceptCorrelationCheck, noValidation, f17);
    } else {
      Path mainpath = CommandLine.parseMainPath(this.args.get(CLArgFlag.MAIN_MOD)[0]);
      return new MainContext(debug, locator, mainpath, useOldWF, noLiveness, minEfsm, fair, noLocalChoiceSubjectCheck, noAcceptCorrelationCheck, noValidation, f17);
    }
  }
  ...
  public void run() throws CommandLineException, ScribbleException {
    try {
      try {
        tryRun();
...
>>>>>>> /usr/src/app/output/scribble/scribble-java/39cd920f8b8bf4a99d9c47a8ec198fba0ba0c29e/scribble-cli/src/main/java/org/scribble/cli/CommandLine.java/right.java
  ...
  protected void tryOutputTask(Job job) throws CommandLineException, ScribbleException {
    ...
    if (this.args.containsKey(CLArgFlag.SGRAPH) || this.args.containsKey(CLArgFlag.SGRAPH_PNG) || this.args.containsKey(CLArgFlag.UNFAIR_SGRAPH) || this.args.containsKey(CLArgFlag.UNFAIR_SGRAPH_PNG)) {
      if (job.useOldWf) {
  ...
  private EGraph getEGraph(Job job, GProtocolName fullname, Role role, boolean forUser, boolean fair) throws ScribbleException, CommandLineException {
    ...
    EGraph graph;
    if (forUser) {
      graph = this.args.containsKey(CLArgFlag.LTSCONVERT_MIN) ? jcontext.getMinimisedEGraph(fullname, role) : jcontext.getEGraph(fullname, role);
    } else {
      graph = !fair ? jcontext.getUnfairEGraph(fullname, role) : jcontext.getEGraph(fullname, role);
    }
    if (graph == null) {
  ...
  private void outputEndpointApi(Job job) throws ScribbleException, CommandLineException {
    ...
    JEndpointApiGenerator jgen = new JEndpointApiGenerator(job);
    for (int i = 0; i < args.length; i += 2) {
      GProtocolName fullname = checkGlobalProtocolArg(jcontext, args[i]);
  ...
  protected void outputClasses(Map<String, String> classes) throws ScribbleException {
    ...
    if (this.args.containsKey(CLArgFlag.API_OUTPUT)) {
      ...
      f = (path) -> {
        ScribUtil.handleLambdaScribbleException(() -> {
          String tmp = dir + "/" + path;
