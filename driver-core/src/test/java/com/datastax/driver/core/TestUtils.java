...
public abstract class TestUtils {
  ...
  @SuppressWarnings(value = { "unchecked", "rawtypes" }) public static BoundStatement setBoundValue(BoundStatement bs, String name, DataType type, Object value) {
    switch (type.getName()) {
      case ASCII:
  ...
  @SuppressWarnings(value = { "serial" }) public static Object getFixedValue(final DataType type) {
    try {
      switch (type.getName()) {
        case ASCII:
  ...
  @SuppressWarnings(value = { "serial" }) public static Object getFixedValue2(final DataType type) {
    try {
      switch (type.getName()) {
        case ASCII:
  ...
  public static int numberOfLocalCoreConnections(Cluster cluster) {
    ...
    return (protocolVersion.compareTo(ProtocolVersion.V3) < 0) ? configuration.getPoolingOptions().getCoreConnectionsPerHost(HostDistance.LOCAL) : 1;
  }
