...
public abstract class TokenIntegrationTest {
  ...
  @AfterClass(groups = "short", alwaysRun = true) public void teardown() {
    if (cluster != null) {
      cluster.close();
    }
    if (ccm != null) {
  ...
  /**
     ...
     * </ol>
     */
  @Test(groups = "short") @CassandraVersion(major = 2) public void should_get_token_from_row_and_set_token_in_query_with_binding_and_aliasing() {
  ...
  @Test(groups = "short") public void should_expose_tokens_per_host() {
    for (Host host : cluster.getMetadata().allHosts()) {
      ...
      Row row = (host.listenAddress == null) ? session.execute("select tokens from system.local").one() : session.execute("select tokens from system.peers where peer = \'" + host.listenAddress.getHostAddress() + "\'").one();
  ...
  @Test(groups = "short") public void should_expose_token_and_range_creation_methods() {
    Metadata metadata = cluster.getMetadata();
