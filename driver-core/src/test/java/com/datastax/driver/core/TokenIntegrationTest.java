...
public abstract class TokenIntegrationTest {
  List<String> schema = Lists.newArrayList("CREATE KEYSPACE IF NOT EXISTS test WITH replication = {\'class\': \'SimpleStrategy\', \'replication_factor\': 1}", "CREATE KEYSPACE IF NOT EXISTS test2 WITH replication = {\'class\': \'SimpleStrategy\', \'replication_factor\': 2}", "USE test", "CREATE TABLE IF NOT EXISTS foo(i int primary key)", "INSERT INTO foo (i) VALUES (1)", "INSERT INTO foo (i) VALUES (2)", "INSERT INTO foo (i) VALUES (3)", "CREATE TABLE IF NOT EXISTS foo2(\"caseSensitiveKey\" int primary key)", "INSERT INTO foo2 (\"caseSensitiveKey\") VALUES (1)");
  ...
  @BeforeClass(groups = "short") public void setup() {
    ccm = CCMBridge.create("test", 3, ccmOptions);
    LoadBalancingPolicy lbp = new WhiteListPolicy(new RoundRobinPolicy(), Lists.newArrayList(new InetSocketAddress(CCMBridge.ipOfNode(1), 9042)));
    cluster = Cluster.builder().addContactPoints(CCMBridge.ipOfNode(1)).withLoadBalancingPolicy(lbp).build();
    cluster.init();
  ...
  @AfterClass(groups = "short") public void teardown() {
    if (cluster != null) {
      cluster.close();
    }
    if (ccm != null) {
  ...
  /**
     ...
     * @since 2.0.10, 2.1.5
     */
  @Test(groups = "short") public void should_expose_token_ranges() throws Exception {
  ...
  private List<Row> rangeQuery(String query, TokenRange range) {
    ...
    for (TokenRange subRange : range.unwrap()) {
      rows.addAll(session.execute(query, subRange.getStart(), subRange.getEnd()).all());
    }
    return rows;
  }
  ...
  /**
     ...
     * @since 2.0.10, 2.1.5
     */
  @Test(groups = "short") public void should_get_token_from_row_and_set_token_in_query() {
    ...
    assertThat(session.execute("SELECT token(i) AS t FROM test.foo WHERE i = 1").one().getToken("t")).isEqualTo(token);
  ...
  /**
     ...
     * @since 2.0.10, 2.1.5
     */
  @Test(groups = "short", expectedExceptions = InvalidTypeException.class) public void should_raise_exception_when_get_token_on_non_token() {
  ...
  /**
     ...
     * @since 2.0.10, 2.1.5
     */
  @Test(groups = "short") public void should_expose_token_ranges_per_host() {
  ...
  /**
     ...
     * @since 2.0.10, 2.1.5
     */
  @Test(groups = "short") public void should_expose_tokens_per_host() {
    for (Host host : cluster.getMetadata().allHosts()) {
      assertThat(host.getTokens()).hasSize(numTokens);
      Row row = (host.listenAddress == null) ? session.execute("select tokens from system.local").one() : session.execute("select tokens from system.peers where peer = ?", host.listenAddress).one();
  ...
  /**
     ...
     * @since 2.0.10, 2.1.5
     */
  @Test(groups = "short") public void should_only_unwrap_one_range_for_all_ranges() {
    Set<TokenRange> ranges = cluster.getMetadata().getTokenRanges();
    assertOnlyOneWrapped(ranges);
    Iterable<TokenRange> splitRanges = Iterables.concat(Iterables.transform(ranges, new Function<TokenRange, Iterable<TokenRange>>() {
