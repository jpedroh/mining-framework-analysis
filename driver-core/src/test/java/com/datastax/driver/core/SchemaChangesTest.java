...
public class SchemaChangesTest extends CCMBridge.PerClassSingleNodeCluster {
  ...
  @Test(groups = "short") public void should_notify_of_udt_creation() {
    session.execute("CREATE TYPE ks.type1(i int)");
    for (Metadata m : metadatas) {
      assertThat(m.getKeyspace("ks").getUserType("type1")).isNotNull();
    }
  }
  ...
  @Test(groups = "short") public void should_notify_of_udt_update() {
    session.execute("CREATE TYPE ks.type1(i int)");
    session.execute("ALTER TYPE ks.type1 ADD j int");
    for (Metadata m : metadatas) {
      assertThat(m.getKeyspace("ks").getUserType("type1").getFieldType("j")).isNotNull();
    }
  }

  @Test(groups = "short") public void should_notify_of_udt_drop() {
    session.execute("CREATE TYPE ks.type1(i int)");
