...
public class QueryBuilderTest {
  @Test(groups = "unit") public void selectTest() throws Exception {
    String query;
    Statement select;
    ...
    select = select("a", "b", quote("C")).from("foo").where(in("a", InetAddress.getByName("127.0.0.1"), InetAddress.getByName("127.0.0.3"))).and(eq(quote("C"), "foo")).orderBy(asc("a"), desc("b")).limit(42);
    assertEquals(select.toString(), query);
  ...
  @Test(groups = "unit") @SuppressWarnings(value = { "serial" }) public void insertTest() throws Exception {
    String query;
    Statement insert;
    query = "INSERT INTO foo(a,b,\"C\",d) VALUES (123,\'127.0.0.1\',\'foo\'\'bar\',{\'x\':3,\'y\':2}) USING TIMESTAMP 42 AND TTL 24;";
    insert = insertInto("foo").value("a", 123).value("b", InetAddress.getByName("127.0.0.1")).value(quote("C"), "foo\'bar").value("d", new TreeMap<String, Integer>() {
      {
        put("x", 3);
        put("y", 2);
      }
    ...
    insert = insertInto("foo", "bar").values(new String[] { "a", "b" }, new Object[] { new TreeSet<Integer>() {
      {
        add(2);
        add(3);
        add(4);
      }
    }, 3.4 }).using(ttl(bindMarker())).and(timestamp(bindMarker()));
    assertEquals(insert.toString(), query);
    query = "INSERT INTO foo.bar(a,b,c) VALUES ({2,3,4},3.4,123) USING TIMESTAMP 42;";
    insert = insertInto("foo", "bar").using(timestamp(42)).values(new String[] { "a", "b" }, new Object[] { new TreeSet<Integer>() {
      {
        add(2);
        add(3);
        add(4);
    ...
    insert = insertInto("foo").using(timestamp(42)).value("c", 123).values(new String[] { "a", "b" }, new Object[] { new TreeSet<Integer>() {
      {
        add(2);
        add(3);
        add(4);
  ...
  @Test(groups = "unit") @SuppressWarnings(value = { "serial" }) public void updateTest() throws Exception {
    String query;
    Statement update;
  ...
  @Test(groups = "unit") public void deleteTest() throws Exception {
    String query;
    Statement delete;
    ...
    assertEquals(delete.toString(), query);
    query = 
<<<<<<< /usr/src/app/output/datastax/java-driver/e08ae2c0181b4ce6f03eaf258d01394b77e11b4f/driver-core/src/test/java/com/datastax/driver/core/querybuilder/QueryBuilderTest.java/left.java
    "DELETE FROM foo.bar WHERE k1=1 AND k2=1;"
=======
    "DELETE  FROM foo.bar WHERE k1=\'foo\' AND k2=1;"
>>>>>>> /usr/src/app/output/datastax/java-driver/e08ae2c0181b4ce6f03eaf258d01394b77e11b4f/driver-core/src/test/java/com/datastax/driver/core/querybuilder/QueryBuilderTest.java/right.java
    ;
    delete = delete().from("foo", "bar").where(eq("k1", 
<<<<<<< /usr/src/app/output/datastax/java-driver/e08ae2c0181b4ce6f03eaf258d01394b77e11b4f/driver-core/src/test/java/com/datastax/driver/core/querybuilder/QueryBuilderTest.java/left.java
    1
=======
    "foo"
>>>>>>> /usr/src/app/output/datastax/java-driver/e08ae2c0181b4ce6f03eaf258d01394b77e11b4f/driver-core/src/test/java/com/datastax/driver/core/querybuilder/QueryBuilderTest.java/right.java
  ...
  @Test(groups = "unit") @SuppressWarnings(value = { "serial" }) public void batchTest() throws Exception {
    String query;
    Statement batch;
    ...
    query += "DELETE a[3],b[\'foo\'],c FROM foo WHERE k=1;";
    query += "APPLY BATCH;";
    batch = batch().add(insertInto("foo").values(new String[] { "a", "b" }, new Object[] { new TreeSet<Integer>() {
      {
        add(2);
        add(3);
        add(4);
      }
    }, 3.4 })).add(update("foo").with(setIdx("a", 2, "foo")).and(prependAll("b", Arrays.asList(3, 2, 1))).and(remove("c", "a")).where(eq("k", 2))).add(delete().listElt("a", 3).mapElt("b", "foo").column("c").from("foo").where(eq("k", 1))).using(timestamp(42));
  ...
  @Test(groups = "unit") public void batchCounterTest() throws Exception {
    String query;
    Statement batch;
    ...
    query += "APPLY BATCH;";
    batch = batch().add(update("foo").with(incr("a", 1))).add(update("foo").with(incr("b", 2))).add(update("foo").with(incr("c", 3))).using(timestamp(42));
    assertEquals(batch.toString(), query);
    query = "BEGIN COUNTER BATCH USING TIMESTAMP 42 ";
    query += "UPDATE foo SET a=a+1;";
    ...
    query += "APPLY BATCH;";
    batch = batch().add(update("foo").with(incr("a"))).add(update("foo").with(incr("b"))).add(update("foo").with(incr("c"))).using(timestamp(42));
    assertEquals(batch.toString(), query);
    query = "BEGIN COUNTER BATCH USING TIMESTAMP 42 ";
    query += "UPDATE foo SET a=a-1;";
    query += "UPDATE foo SET b=b-2;";
    query += "UPDATE foo SET c=c-3;";
    query += "APPLY BATCH;";
    batch = batch().add(update("foo").with(decr("a", 1))).add(update("foo").with(decr("b", 2))).add(update("foo").with(decr("c", 3))).using(timestamp(42));
    assertEquals(batch.toString(), query);
    query = "BEGIN COUNTER BATCH USING TIMESTAMP 42 ";
    query += "UPDATE foo SET a=a-1;";
    query += "UPDATE foo SET b=b-1;";
    query += "UPDATE foo SET c=c-1;";
    query += "APPLY BATCH;";
    batch = batch().add(update("foo").with(decr("a"))).add(update("foo").with(decr("b"))).add(update("foo").with(decr("c"))).using(timestamp(42));
    assertEquals(batch.toString(), query);
    query = "BEGIN COUNTER BATCH USING TIMESTAMP 42 ";
    query += "UPDATE foo SET a=a+1;";
    query += "UPDATE foo SET b=b+-2;";
    query += "UPDATE foo SET c=c-3;";
    query += "APPLY BATCH;";
    batch = batch().add(update("foo").with(decr("a", -1))).add(update("foo").with(incr("b", -2))).add(update("foo").with(decr("c", 3))).using(timestamp(42));
    assertEquals(batch.toString(), query);
  }

  @Test(groups = "unit", expectedExceptions = { IllegalArgumentException.class }) public void batchMixedCounterTest() throws Exception {
    String query;
    Statement batch;
    batch = batch().add(update("foo").with(incr("a", 1))).add(update("foo").with(set("b", 2))).add(update("foo").with(incr("c", 3))).using(timestamp(42));
  }

  @Test(groups = "unit") public void markerTest() throws Exception {
    String query;
    Statement insert;
  ...
  @Test(groups = "unit") public void rawEscapingTest() throws Exception {
    String query;
    Statement select;
  ...
  @Test(groups = "unit") public void selectInjectionTests() throws Exception {
    String query;
    Statement select;
  ...
  @Test(groups = "unit") @SuppressWarnings(value = { "serial" }) public void insertInjectionTest() throws Exception {
    String query;
    Statement insert;
  ...
  @Test(groups = "unit") public void updateInjectionTest() throws Exception {
    String query;
    Statement update;
  ...
  @Test(groups = "unit") public void deleteInjectionTests() throws Exception {
    String query;
    Statement delete;
    ...
    delete = delete("a", "b").from("foo").where(in("a", "b", "c\'); --comment"));
    assertEquals(delete.toString(), query);
  ...
  @Test(groups = "unit") public void statementForwardingTest() throws Exception {
  ...
  @Test(groups = "unit") public void rejectUnknownValueTest() throws Exception {
    try {
  ...
  @Test(groups = "unit") public void truncateTest() throws Exception {
    assertEquals(truncate("foo").toString(), "TRUNCATE foo;");
    assertEquals(truncate("foo", quote("Bar")).toString(), "TRUNCATE foo.\"Bar\";");
  }

  @Test(groups = "unit") public void quotingTest() {
    assertEquals(QueryBuilder.select().from("Metrics", "epochs").getQueryString(), "SELECT * FROM Metrics.epochs;");
    assertEquals(QueryBuilder.select().from("Metrics", quote("epochs")).getQueryString(), "SELECT * FROM Metrics.\"epochs\";");
    assertEquals(QueryBuilder.select().from(quote("Metrics"), "epochs").getQueryString(), "SELECT * FROM \"Metrics\".epochs;");
    assertEquals(QueryBuilder.select().from(quote("Metrics"), quote("epochs")).getQueryString(), "SELECT * FROM \"Metrics\".\"epochs\";");
    assertEquals(QueryBuilder.insertInto("Metrics", "epochs").getQueryString(), "INSERT INTO Metrics.epochs() VALUES ();");
    assertEquals(QueryBuilder.insertInto("Metrics", quote("epochs")).getQueryString(), "INSERT INTO Metrics.\"epochs\"() VALUES ();");
    assertEquals(QueryBuilder.insertInto(quote("Metrics"), "epochs").getQueryString(), "INSERT INTO \"Metrics\".epochs() VALUES ();");
    assertEquals(QueryBuilder.insertInto(quote("Metrics"), quote("epochs")).getQueryString(), "INSERT INTO \"Metrics\".\"epochs\"() VALUES ();");
  }

  @Test(groups = "unit") public void compoundWhereClauseTest() throws Exception {
    String query;
    Statement select;
    ...
    select = select().all().from("foo").where(eq("k", 4)).and(gte(Arrays.asList("c1", "c2"), Arrays.<Object>asList("a", 2))).and(lt(Arrays.asList("c1", "c2"), Arrays.<Object>asList("b", 0)));
    assertEquals(select.toString(), query);
    query = "SELECT * FROM foo WHERE k=4 AND (c1,c2)<=(\'a\',2);";
