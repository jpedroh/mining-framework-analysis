...
public abstract class DataType {
  ...
  static DataType decode(ChannelBuffer buffer) {
    ...
    switch (name) {
      ...
      return CassandraTypeParser.isUserType(className) || CassandraTypeParser.isTupleType(className) ? CassandraTypeParser.parseOne(className) : custom(className);
      case LIST:
  ...
  /**
     ...
     */
  @Deprecated public ByteBuffer serialize(Object value) {
  ...
  /**
     ...
     */
  @Deprecated public Object deserialize(ByteBuffer bytes) {
  ...
  /**
     ...
     */
  @Deprecated public static ByteBuffer serializeValue(Object value) {
  ...
  private static class Native extends DataType {
    ...
    @Override boolean canBeDeserializedAs(TypeToken typeToken) {
      return typeToken.isAssignableFrom(getName().javaType);
    }

    @Override TypeCodec<Object> codec(ProtocolVersion protocolVersion) {
    ...
    @Override public final boolean equals(Object o) {
      ...
      return name == that.name || name == Name.VARCHAR && that.name == Name.TEXT || name == Name.TEXT && that.name == Name.VARCHAR;
    }
  ...
  private static class Collection extends DataType {
    ...
    @Override @SuppressWarnings(value = { "unchecked" }) boolean canBeDeserializedAs(TypeToken typeToken) {
      switch (name) {
        case LIST:
        return typeToken.getRawType().isAssignableFrom(List.class) && typeArguments.get(0).canBeDeserializedAs(typeToken.resolveType(typeToken.getRawType().getTypeParameters()[0]));
        case SET:
        return typeToken.getRawType().isAssignableFrom(Set.class) && typeArguments.get(0).canBeDeserializedAs(typeToken.resolveType(typeToken.getRawType().getTypeParameters()[0]));
        case MAP:
        return typeToken.getRawType().isAssignableFrom(Map.class) && typeArguments.get(0).canBeDeserializedAs(typeToken.resolveType(typeToken.getRawType().getTypeParameters()[0])) && typeArguments.get(1).canBeDeserializedAs(typeToken.resolveType(typeToken.getRawType().getTypeParameters()[1]));
      }
      throw new AssertionError();
    }

    @SuppressWarnings(value = { "unchecked" }) @Override TypeCodec<Object> codec(ProtocolVersion protocolVersion) {
      switch (name) {
        case LIST:
    ...
    @Override public List<DataType> getTypeArguments() {
  ...
  private static class Custom extends DataType {
    ...
    @Override public String getCustomTypeClassName() {
