...
public abstract class Token implements Comparable<Token> {
  ...
  static class M3PToken extends Token {
    ...
    private static class M3PTokenFactory extends Factory {
      ...
      @SuppressWarnings(value = { "fallthrough" }) private long murmur(ByteBuffer data) {
      ...
      @Override M3PToken fromString(String tokenStr) {
        return new M3PToken(Long.parseLong(tokenStr));
      }

      @Override DataType getTokenType() {
        return DataType.bigint();
      }

      @Override Token deserialize(ByteBuffer buffer, ProtocolVersion protocolVersion) {
        return new M3PToken((Long) getTokenType().deserialize(buffer, protocolVersion));
      }

      @Override Token minToken() {
        return MIN_TOKEN;
      }

      @Override M3PToken hash(ByteBuffer partitionKey) {
        long v = murmur(partitionKey);
        return new M3PToken(v == Long.MIN_VALUE ? Long.MAX_VALUE : v);
      }

      @Override List<Token> split(Token startToken, Token endToken, int numberOfSplits) {
  ...
  static class OPPToken extends Token {
    ...
    private static class OPPTokenFactory extends Factory {
      ...
      @Override public OPPToken fromString(String tokenStr) {
      ...
      @Override OPPToken hash(ByteBuffer partitionKey) {
    ...
    @VisibleForTesting OPPToken(ByteBuffer value) {
    ...
    @Override public int compareTo(Token other) {
      ...
      return UnsignedBytes.lexicographicalComparator().compare(Bytes.getArray(value), Bytes.getArray(((OPPToken) other).value));
    }
  ...
  static class RPToken extends Token {
    ...
    private static class RPTokenFactory extends Factory {
      ...
      @Override RPToken fromString(String tokenStr) {
      ...
      @Override RPToken hash(ByteBuffer partitionKey) {
