        ...
        if (logger.isDebugEnabled()) {
          ...
          for (Map.Entry<Host, TimestampedAverage> entry : currentLatencies.entrySet()) {
            ...
            if (latency.average > ((long) (exclusionThreshold * currentMin))) {
              ...
              if (!excludedAtLastTick.contains(host)) {
                logger.debug(String.format("Host %s has an average latency score of %.3fms, more than %f times more than the minimum %.3fms: will be avoided temporarily.", host, inMS(latency.average), exclusionThreshold, inMS(currentMin)));
              }
              continue;
            }
            if (excludedAtLastTick.contains(host)) {
