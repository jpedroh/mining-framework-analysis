...
class Requests {
  ...
  public static class QueryProtocolOptions {
    ...
    public int encodedSize(ProtocolVersion version) {
      switch (version) {
        case V1:
        return CBUtil.sizeOfValueList(values) + CBUtil.sizeOfConsistencyLevel(consistency);
        case V2:
        case V3:
        int size = 0;
  ...
  public static class BatchProtocolOptions {
    ...
    @Override public String toString() {
      return String.format("[cl=%s, serialCl=%s, defaultTs=%d]", consistency, serialConsistency, defaultTimestamp);
    }
  }
