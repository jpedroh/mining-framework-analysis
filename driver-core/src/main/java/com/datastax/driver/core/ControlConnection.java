...
class ControlConnection implements Host.StateListener {
  ...
  private Connection tryConnect(Host host) throws ConnectionException, ExecutionException, InterruptedException, UnsupportedProtocolVersionException {
    ...
    try {
      ...
      List<ProtocolEvent.Type> evs = Arrays.asList(new ProtocolEvent.Type[] { ProtocolEvent.Type.TOPOLOGY_CHANGE, ProtocolEvent.Type.STATUS_CHANGE, ProtocolEvent.Type.SCHEMA_CHANGE });
  ...
  static void refreshSchema(Connection connection, String keyspace, String table, Cluster.Manager cluster) throws ConnectionException, BusyConnectionException, ExecutionException, InterruptedException {
    ...
    DefaultResultSetFuture ksFuture = table == null ? new DefaultResultSetFuture(null, new Requests.Query(SELECT_KEYSPACES + whereClause)) : null;
  ...
  public void refreshNodeInfo(Host host) {
    ...
    try {
      DefaultResultSetFuture future = c.address.equals(host.getAddress()) ? new DefaultResultSetFuture(null, new Requests.Query(SELECT_LOCAL)) : new DefaultResultSetFuture(null, new Requests.Query(SELECT_PEERS + " WHERE peer=\'" + host.getAddress().getHostAddress() + '\''));
      c.write(future);
