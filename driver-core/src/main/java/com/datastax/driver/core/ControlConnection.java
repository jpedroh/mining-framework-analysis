...
class ControlConnection implements Host.StateListener {
  ...
  private void backgroundReconnect(long initialDelayMs) {
    ...
    new AbstractReconnectionHandler(cluster.reconnectionExecutor, cluster.reconnectionPolicy().newSchedule(), reconnectionAttempt, initialDelayMs) {
      @Override protected Connection tryReconnect() throws ConnectionException {
        try {
  ...
  public void refreshSchema(String keyspace, String table, String udt) throws InterruptedException {
    logger.debug("[Control connection] Refreshing schema for {}{}", keyspace == null ? "everything" : keyspace, table == null ? (udt == null ? "" : '.' + udt + " (UDT)") : '.' + table + " (table)");
    try {
  ...
  static void refreshSchema(Connection connection, String keyspace, String table, String udt, Cluster.Manager cluster, boolean isInitialConnection) throws ConnectionException, BusyConnectionException, ExecutionException, InterruptedException {
    ...
    if (host == null || host.getCassandraVersion() == null) {
      ...
      logger.warn("Cannot find Cassandra version for host {} to parse the schema, using {} based on protocol version in use. " + "If parsing the schema fails, this could be the cause", connection.address, cassandraVersion);
    } else {
    ...
    DefaultResultSetFuture ksFuture = (table == null && udt == null) ? new DefaultResultSetFuture(null, cluster.protocolVersion(), new Requests.Query(SELECT_KEYSPACES + whereClause)) : null;
    DefaultResultSetFuture udtFuture = table == null && (cassandraVersion.getMajor() > 2 || (cassandraVersion.getMajor() == 2 && cassandraVersion.getMinor() >= 1)) ? new DefaultResultSetFuture(null, cluster.protocolVersion(), new Requests.Query(SELECT_USERTYPES + whereClause)) : null;
    DefaultResultSetFuture cfFuture = udt == null ? new DefaultResultSetFuture(null, cluster.protocolVersion(), new Requests.Query(SELECT_COLUMN_FAMILIES + whereClause)) : null;
    DefaultResultSetFuture colsFuture = udt == null ? new DefaultResultSetFuture(null, cluster.protocolVersion(), new Requests.Query(SELECT_COLUMNS + whereClause)) : null;
    if (ksFuture != null) {
    ...
    try {
      cluster.metadata.rebuildSchema(keyspace, table, udt, ksFuture == null ? null : ksFuture.get(), udtFuture == null ? null : udtFuture.get(), cfFuture == null ? null : cfFuture.get(), colsFuture == null ? null : colsFuture.get(), cassandraVersion);
    } catch (RuntimeException e) {
  ...
  private Row fetchNodeInfo(Host host, Connection c) throws ConnectionException, BusyConnectionException, ExecutionException, InterruptedException {
    ...
    if (isConnectedHost || host.listenAddress != null) {
      DefaultResultSetFuture future = isConnectedHost ? new DefaultResultSetFuture(null, cluster.protocolVersion(), new Requests.Query(SELECT_LOCAL)) : new DefaultResultSetFuture(null, cluster.protocolVersion(), new Requests.Query(SELECT_PEERS + " WHERE peer=\'" + host.listenAddress.getHostAddress() + '\''));
      c.write(future);
