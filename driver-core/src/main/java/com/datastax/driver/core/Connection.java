...
class Connection extends com.datastax.cassandra.transport.Connection {
  ...
  private static final Connection.Tracker EMPTY_TRACKER = new Connection.Tracker() {
    @Override public void addConnection(Channel ch, com.datastax.cassandra.transport.Connection connection) {
    }
  ...
  private static String extractMessage(Throwable t) {
    String msg = t == null || t.getMessage() == null || t.getMessage().isEmpty() ? t.toString() : t.getMessage();
    return " (" + msg + ")";
  }
  ...
  public boolean close(long timeout, TimeUnit unit) throws InterruptedException {
    ...
    boolean closed = channel == null ? true : channel.close().await(timeout - Cluster.timeSince(start, unit), unit);
  ...
  public static class Factory {
    ...
    public boolean shutdown(long timeout, TimeUnit unit) throws InterruptedException {
      ...
      timer.stop();
      return future.await(timeout, unit) && bossExecutor.awaitTermination(timeout - Cluster.timeSince(start, unit), unit) && workerExecutor.awaitTermination(timeout - Cluster.timeSince(start, unit), unit);
    }
  }
    ...
    @Override public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) {
      if (!(e.getMessage() instanceof com.datastax.cassandra.transport.Message.Response)) {
        ...
        if (handler == null) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug("[{}] Response received on stream {} but no handler set anymore (either the request has " + "timeouted or it was closed due to another error). Received message is {}", name, streamId, asDebugString(response));
          }
          return;
        }
        handler.cancelTimeout();
  ...
  static class Future extends SimpleFuture<com.datastax.cassandra.transport.Message.Response> implements RequestHandler.Callback {
    ...
    @Override public com.datastax.cassandra.transport.Message.Request request() {
      return request;
    }

    @Override public void onSet(Connection connection, com.datastax.cassandra.transport.Message.Response response, ExecutionInfo info, long latency) {
      onSet(connection, response, latency);
    }

    @Override public void onSet(Connection connection, com.datastax.cassandra.transport.Message.Response response, long latency) {
  ...
  private static class PipelineFactory implements ChannelPipelineFactory {
    ...
    public PipelineFactory(final Connection connection) {
      ...
      this.cfactory = new com.datastax.cassandra.transport.Connection.Factory() {
        @Override public Connection newConnection(Connection.Tracker tracker) {
          return connection;
        }
      };
    }
