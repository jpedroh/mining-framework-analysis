    ...
    @Override public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) {
      if (!(e.getMessage() instanceof Message.Response)) {
        ...
        if (handler == null) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug("[{}] Response received on stream {} but no handler set anymore (either the request has " + "timed out or it was closed due to another error). Received message is {}", name, streamId, asDebugString(response));
          }
          return;
        }
        handler.cancelTimeout();
