...
public class AndroidHttpClient implements HttpClient {
  ...
  private final static Logger LOG = LoggerFactory.getLogger(AndroidHttpClient.class);
  ...
  @Override public HttpResponse delete(String uri) {
    return executeRequest(new HttpDelete(uri));
  }

  @Override public HttpResponse get(String uri) {
    return executeRequest(new HttpGet(uri));
  }

  @Override public HttpResponse get(String uri, Map<String, String> headers) {
    return executeRequest(new HttpGet(uri), headers);
  }

  @Override public HttpResponse getUncached(String uri) {
    return executeRequest(new HttpGet(uri), true);
  }

  @Override public HttpResponse head(String uri) {
    return executeRequest(new HttpHead(uri));
  }

  @Override public HttpResponse post(String uri, String content) {
    return executePutPost(new HttpPost(uri), content, false);
  }

  @Override public HttpResponse post(String uri, InputStream content) {
    InputStreamEntity e = new InputStreamEntity(content, -1);
    e.setContentType("application/json");
    return post(uri, e);
  }

  @Override public HttpResponse post(String uri, HttpEntity httpEntity) {
    HttpPost post = new HttpPost(uri);
    post.setEntity(httpEntity);
    return executeRequest(post, true);
  }

  @Override public HttpResponse postUncached(String uri, String content) {
    return executePutPost(new HttpPost(uri), content, true);
  }

  @Override public HttpResponse put(String uri) {
    return executeRequest(new HttpPut(uri));
  }

  @Override public HttpResponse put(String uri, String content) {
    return executePutPost(new HttpPut(uri), content, false);
  }

  @Override public HttpResponse put(String uri, InputStream data, String contentType, long contentLength) {
    InputStreamEntity e = new InputStreamEntity(data, contentLength);
    e.setContentType(contentType);
    return put(uri, e);
  }

  @Override public HttpResponse put(String uri, HttpEntity httpEntity) {
    HttpPut hp = new HttpPut(uri);
    hp.setEntity(httpEntity);
    return executeRequest(hp);
  }

  @Override public HttpResponse copy(String sourceUri, String destination) {
    return executeRequest(new HttpCopyRequest(sourceUri, destination), true);
  }

  private HttpResponse executePutPost(HttpEntityEnclosingRequestBase request, String content, boolean useBackend) {
    try {
  ...
  public static class Builder {
    ...
    public org.apache.http.client.HttpClient configureClient() {
      ...
      if (username != null && password != null) {
        client.getCredentialsProvider().setCredentials(new AuthScope(host, port, AuthScope.ANY_REALM), new UsernamePasswordCredentials(username, password));
