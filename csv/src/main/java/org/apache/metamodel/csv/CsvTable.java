...
final class CsvTable extends AbstractTable {
  ...
  @Override public List<Column> getColumns() {
    if (_columns == null) {
      synchronized (this) {
        if (_columns == null) {
  ...
  private List<Column> buildColumns(final List<String> columnNames) {
    ...
    try (ColumnNamingSession namingSession = columnNamingStrategy.startColumnNamingSession()) {
      for (int i = 0; i < columnNames.size(); i++) {
        final String intrinsicColumnName = columnNameLineNumber == CsvConfiguration.NO_COLUMN_NAME_LINE ? null : columnNames.get(i);
        final String columnName = namingSession.getNextColumnName(new ColumnNamingContextImpl(this, intrinsicColumnName, i));
        final Column column = new MutableColumn(columnName, ColumnType.STRING, this, i, null, null, nullable, null, false, null);
        columns.add(column);
      }
    }
    return columns;
  ...
  @Override public TableType getType() {
    return TableType.TABLE;
  }

  @Override public Collection<Relationship> getRelationships() {
