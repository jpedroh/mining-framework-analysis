...
public class SSTagImpl extends SSServImplWithDBA implements SSTagClientI, SSTagServerI, SSEntityHandlerImplI {
  ...
  @Override public void removeDirectlyAdjoinedEntitiesForUser(final SSEntityEnum entityType, final SSEntityUserDirectlyAdjoinedEntitiesRemovePar par) throws Exception {
    try {
      ...
      SSServCaller.tagsUserRemove(par.user, par.entityUri, null, null, false);
  ...
  @Override public SSEntityDescA getDescForEntity(final SSEntityEnum entityType, final SSUri userUri, final SSUri entityUri, final SSLabelStr label, final Long creationTime, final List<SSTag> tags, final SSRatingOverall overallRating, final List<SSUri> discUris, final SSUri author) throws Exception {
    try {
      ...
      return SSTagDesc.get(entityUri, label, creationTime, author);
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
      return null;
    }
  ...
  /****** SSTagClientI ******/
  @Override public void tagAdd(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSTagAddRet.get(tagAdd(par), par.op));
  }

  @Override public void tagsUserRemove(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSTagsUserRemoveRet.get(tagsUserRemove(par), par.op));
  }

  @Override public void tagUserFrequsGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSTagUserFrequsGetRet.get(tagUserFrequsGet(par), par.op));
  }

  /****** SSTagServerI ******/
  @Override public Boolean tagAdd(final SSServPar parA) throws Exception {
    try {
      ...
      SSServCaller.entityAdd(par.user, tagUri, SSLabelStr.get(SSTagLabel.toStr(par.tagString)), SSEntityEnum.tag, false);
      SSServCaller.entityAdd(par.user, par.resource, SSLabelStr.get(SSUri.toStr(par.resource)), SSEntityEnum.entity, false);
  ...
  @Override public Boolean tagAddAtCreationTime(final SSServPar parA) throws Exception {
    try {
      final SSTagAddAtCreationTimePar par = new SSTagAddAtCreationTimePar(parA);
      final Boolean existsTag = sqlFct.existsTagLabel(par.tagString);
      final SSUri tagUri = sqlFct.getOrCreateTagUri(existsTag, par.tagString);
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAddAtCreationTime(par.user, tagUri, SSLabelStr.get(SSStrU.toString(par.tagString)), par.creationTime, SSEntityEnum.tag, false);
      SSServCaller.entityAdd(par.user, par.resource, SSLabelStr.get(par.resource.toString()), SSEntityEnum.entity, false);
      sqlFct.addTagAss(tagUri, par.user, par.resource, par.space);
      dbSQL.commit(par.shouldCommit);
      return true;
    } catch (SSSQLDeadLockErr deadLockErr) {
  ...
  @Override public Boolean tagsAdd(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public Boolean tagsAddAtCreationTime(final SSServPar parA) throws Exception {
    try {
      final SSTagsAddAtCreationTimePar par = new SSTagsAddAtCreationTimePar(parA);
      dbSQL.startTrans(par.shouldCommit);
      for (SSTagLabel tagString : par.tagStrings) {
        SSServCaller.tagAddAtCreationTime(par.user, par.resource, tagString, par.space, par.creationTime, false);
      }
      dbSQL.commit(par.shouldCommit);
      return true;
    } catch (SSSQLDeadLockErr deadLockErr) {
  ...
  @Override public Boolean tagsRemove(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public Boolean tagsUserRemove(final SSServPar parA) throws Exception {
    try {
      final SSTagsUserRemovePar par = new SSTagsUserRemovePar(parA);
      if (par.user == null) {
        throw new Exception("user null");
      }
      if (par.space == null && par.resource == null) {
        dbSQL.startTrans(par.shouldCommit);
  ...
  @Override public List<SSUri> tagUserEntitiesForTagGet(final SSServPar parA) throws Exception {
    ...
    try {
      ...
      if (par.space == null) {
        ...
        entityUris.addAll(sqlFct.getEntitiesForTagLabel(par.user, par.tagString, SSSpaceEnum.privateSpace));
        entityUris.addAll(sqlFct.getEntitiesForTagLabel(null, par.tagString, SSSpaceEnum.sharedSpace));
        return SSUri.distinct(entityUris);
      }
      if (SSSpaceEnum.isShared(par.space)) {
        return sqlFct.getEntitiesForTagLabel(null, par.tagString, par.space);
      }
      if (SSSpaceEnum.isPrivate(par.space)) {
        return sqlFct.getEntitiesForTagLabel(par.user, par.tagString, par.space);
      }
      throw new Exception("reached not reachable code");
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  @Override public List<SSTag> tagsUserGet(final SSServPar parA) throws Exception {
  ...
  @Override public List<SSTagFrequ> tagUserFrequsGet(final SSServPar parA) throws Exception {
