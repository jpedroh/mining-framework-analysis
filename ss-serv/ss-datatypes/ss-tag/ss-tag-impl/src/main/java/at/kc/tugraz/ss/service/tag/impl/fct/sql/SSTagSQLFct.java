...
public class SSTagSQLFct extends SSDBSQLFct {
  ...
  public SSUri getOrCreateTagURI(final Boolean exsitsTag, final SSTagLabel tagString) throws Exception {
    ResultSet resultSet = null;
    try {
      if (!exsitsTag) {
        return createTagURI();
  ...
  public void addTagAssIfNotExists(final SSUri tagUri, final SSUri userUri, final SSUri entityUri, final SSSpaceE space) throws Exception {
    try {
  ...
  public Boolean existsTagAss(final SSUri userUri, final SSUri entityUri, final SSUri tagUri, final SSSpaceE space) throws Exception {
    ResultSet resultSet = null;
    try {
      final Map<String, String> wheres = new HashMap<String, String>();
  ...
  public void removeTagAsss(final SSUri userUri, final SSUri entityUri, final SSTagLabel tagString, final SSSpaceE space) throws Exception {
    ResultSet resultSet = null;
    try {
      final Map<String, String> wheres = new HashMap<String, String>();
  ...
  public List<SSTag> getTagAsss(final SSUri userUri, final SSUri entity, final SSTagLabel tagString, final SSSpaceE tagSpace) throws Exception {
    ResultSet resultSet = null;
    try {
      final Map<String, String> wheres = new HashMap<String, String>();
      ...
      while (resultSet.next()) {
        tagAsss.add(SSTag.get(bindingStrToUri(resultSet, SSSQLVarU.tagId), bindingStrToUri(resultSet, SSSQLVarU.entityId), bindingStrToUri(resultSet, SSSQLVarU.userId), bindingStrToSpace(resultSet, SSSQLVarU.tagSpace), SSTagLabel.get(bindingStr(resultSet, SSSQLVarU.label))));
      }
      return tagAsss;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  public List<SSUri> getEntitiesForTagLabel(final SSUri userUri, final SSTagLabel tagString, final SSSpaceE tagSpace) throws Exception {
    ResultSet resultSet = null;
    try {
