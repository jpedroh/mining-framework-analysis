...
public class SSLocationImpl extends SSServImplWithDBA implements SSLocationClientI, SSLocationServerI, SSEntityHandlerImplI {
  ...
  @Override public void removeDirectlyAdjoinedEntitiesForUser(final SSEntityEnum entityType, final SSEntityUserDirectlyAdjoinedEntitiesRemovePar par) throws Exception {
    ...
    try {
      SSServCaller.locationsUserRemove(par.user, par.entityUri, false);
  ...
  @Override public SSEntityDescA getDescForEntity(final SSEntityEnum entityType, final SSUri userUri, final SSUri entityUri, final SSLabelStr label, final Long creationTime, final List<SSTag> tags, final SSRatingOverall overallRating, final List<SSUri> discUris, final SSUri author) throws Exception {
    ...
    return SSLocationDesc.get(entityUri, label, creationTime, author);
  }

  /****** SSLocationClientI ******/
  @Override public void locationAdd(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLocationAddRet.get(locationAdd(par), par.op));
  }

  @Override public void locationsGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLocationsGetRet.get(locationsGet(par), par.op));
  }

  /****** SSLabelServerI ******/
  @Override public Boolean locationsUserRemove(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public SSUri locationAdd(final SSServPar parA) throws Exception {
    try {
      ...
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, locationUri, SSLabelStr.get(par.location), SSEntityEnum.location, false);
      SSServCaller.entityAdd(par.user, par.entityUri, SSLabelStr.get(SSStrU.empty), SSEntityEnum.entity, false);
      if (!existsLocation) {
  ...
  @Override public List<String> locationsGet(SSServPar parI) throws Exception {
