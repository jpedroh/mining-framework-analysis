...
public class SSUserAllRet extends SSServRetI {
  ...
  public static SSUserAllRet get(final List<SSUri> users, final SSMethU op) {
    return new SSUserAllRet(users, op);
  }

  private SSUserAllRet(final List<SSUri> users, final SSMethU op) {
    super(op);
    if (users != null) {
  ...
  @Override public Map<String, Object> jsonLDDesc() {
