...
public class SSRatingImpl extends SSServImplWithDBA implements SSRatingClientI, SSRatingServerI, SSEntityHandlerImplI {
  ...
  @Override public void removeDirectlyAdjoinedEntitiesForUser(final SSEntityEnum entityType, final SSEntityUserDirectlyAdjoinedEntitiesRemovePar par) throws Exception {
    ...
    try {
      SSServCaller.ratingsUserRemove(par.user, par.entityUri, false);
  ...
  @Override public SSEntityDescA getDescForEntity(final SSEntityEnum entityType, final SSUri userUri, final SSUri entityUri, final SSLabelStr label, final Long creationTime, final List<SSTag> tags, final SSRatingOverall overallRating, final List<SSUri> discUris, final SSUri author) throws Exception {
    ...
    return SSRatingDesc.get(entityUri, label, creationTime, author);
  }

  /****** SSRatingClientI ******/
  @Override public void ratingUserSet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSRatingUserSetRet.get(ratingUserSet(par), par.op));
  }

  @Override public void ratingUserGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSRatingUserGetRet.get(ratingUserGet(par), par.op));
  }

  @Override public void ratingOverallGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSRatingOverallGetRet.get(ratingOverallGet(par), par.op));
  }

  /****** SSRatingServerI ******/
  @Override public Boolean ratingsUserRemove(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public Boolean ratingUserSet(SSServPar parA) throws Exception {
    try {
      ...
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, ratingUri, SSLabelStr.get(ratingUri.toString()), SSEntityEnum.rating, false);
  ...
  @Override public Integer ratingUserGet(SSServPar parI) throws Exception {
  ...
  @Override public SSRatingOverall ratingOverallGet(SSServPar parI) throws Exception {
