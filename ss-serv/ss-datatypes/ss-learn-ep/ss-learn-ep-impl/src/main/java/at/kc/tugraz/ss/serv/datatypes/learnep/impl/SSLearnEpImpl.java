...
public class SSLearnEpImpl extends SSServImplWithDBA implements SSLearnEpClientI, SSLearnEpServerI, SSEntityHandlerImplI {
  ...
  @Override public Boolean shareUserEntity(final SSUri userUri, final List<SSUri> userUrisToShareWith, final SSUri entityUri, final SSUri entityCircleUri, final SSEntityEnum entityType) throws Exception {
    return false;
  }
  ...
  @Override public SSEntityDescA getDescForEntity(final SSEntityEnum entityType, final SSUri userUri, final SSUri entityUri, final SSLabelStr label, final Long creationTime, final List<SSTag> tags, final SSRatingOverall overallRating, final List<SSUri> discUris, final SSUri author) throws Exception {
    if (SSEntityEnum.equals(entityType, SSEntityEnum.learnEp)) {
      return SSLearnEpDesc.get(entityUri, label, creationTime, author);
    }
    if (SSEntityEnum.equals(entityType, SSEntityEnum.learnEpVersion)) {
      return SSLearnEpVersionDesc.get(entityUri, label, creationTime, author);
    }
    if (SSEntityEnum.equals(entityType, SSEntityEnum.learnEpCircle)) {
      return SSLearnEpCircleDesc.get(entityUri, label, creationTime, author);
    }
    if (SSEntityEnum.equals(entityType, SSEntityEnum.learnEpEntity)) {
      return SSLearnEpEntityDesc.get(entityUri, label, creationTime, author);
    }
    if (SSEntityEnum.equals(entityType, SSEntityEnum.learnEpTimelineState)) {
      return SSLearnEpTimelineStateDesc.get(entityUri, label, creationTime, author);
    }
    return SSEntityDesc.get(entityUri, label, creationTime, tags, overallRating, discUris, author);
  }

  /****** SSLearnEpClientI ******/
  @Override public void learnEpsGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpsGetRet.get(learnEpsGet(par), par.op));
  }

  @Override public void learnEpVersionsGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionsGetRet.get(learnEpVersionsGet(par), par.op));
  }

  @Override public void learnEpVersionGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionGetRet.get(learnEpVersionGet(par), par.op));
  }

  @Override public void learnEpVersionCreate(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionCreateRet.get(learnEpVersionCreate(par), par.op));
  }

  @Override public void learnEpVersionAddCircle(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionAddCircleRet.get(learnEpVersionAddCircle(par), par.op));
  }

  @Override public void learnEpVersionAddEntity(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionAddEntityRet.get(learnEpVersionAddEntity(par), par.op));
  }

  @Override public void learnEpCreate(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpCreateRet.get(learnEpCreate(par), par.op));
  }

  @Override public void learnEpVersionUpdateCircle(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionUpdateCircleRet.get(learnEpVersionUpdateCircle(par), par.op));
  }

  @Override public void learnEpVersionUpdateEntity(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionUpdateEntityRet.get(learnEpVersionUpdateEntity(par), par.op));
  }

  @Override public void learnEpVersionRemoveCircle(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionRemoveCircleRet.get(learnEpVersionRemoveCircle(par), par.op));
  }

  @Override public void learnEpVersionRemoveEntity(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionRemoveEntityRet.get(learnEpVersionRemoveEntity(par), par.op));
  }

  @Override public void learnEpVersionSetTimelineState(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionSetTimelineStateRet.get(learnEpVersionSetTimelineState(par), par.op));
  }

  @Override public void learnEpVersionGetTimelineState(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionGetTimelineStateRet.get(learnEpVersionGetTimelineState(par), par.op));
  }

  @Override public void learnEpVersionCurrentGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionCurrentGetRet.get(learnEpVersionCurrentGet(par), par.op));
  }

  @Override public void learnEpVersionCurrentSet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSLearnEpVersionCurrentSetRet.get(learnEpVersionCurrentSet(par), par.op));
  }

  /****** SSLearnEpServerI *********************************************/
  @Override public SSUri learnEpVersionCurrentSet(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public SSUri learnEpVersionSetTimelineState(final SSServPar parA) throws Exception {
    try {
      ...
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, learnEpTimelineStateUri, SSLabelStr.get(learnEpTimelineStateUri.toString()), SSEntityEnum.learnEpTimelineState, false);
  ...
  @Override public Boolean learnEpVersionRemoveCircle(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public Boolean learnEpVersionRemoveEntity(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public Boolean learnEpVersionUpdateCircle(final SSServPar parA) throws Exception {
    try {
      final SSLearnEpVersionUpdateCirclePar par = new SSLearnEpVersionUpdateCirclePar(parA);
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityLabelSet(par.learnEpCircleUri, par.label, false);
  ...
  @Override public Boolean learnEpVersionUpdateEntity(SSServPar parA) throws Exception {
    try {
      final SSLearnEpVersionUpdateEntityPar par = new SSLearnEpVersionUpdateEntityPar(parA);
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, par.entityUri, SSLabelStr.get(SSStrU.toString(par.entityUri)), SSEntityEnum.entity, false);
  ...
  @Override public SSUri learnEpCreate(final SSServPar parA) throws Exception {
    try {
      final SSLearnEpCreatePar par = new SSLearnEpCreatePar(parA);
      final SSUri learnEpUri = sqlFct.createLearnEpUri();
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, learnEpUri, par.label, SSEntityEnum.learnEp, false);
  ...
  @Override public SSUri learnEpVersionCreate(final SSServPar parA) throws Exception {
    try {
      ...
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, learnEpVersionUri, SSLabelStr.get(learnEpVersionUri.toString()), SSEntityEnum.learnEpVersion, false);
  ...
  @Override public SSUri learnEpVersionAddCircle(final SSServPar parA) throws Exception {
    try {
      ...
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, circleUri, par.label, SSEntityEnum.learnEpCircle, false);
  ...
  @Override public SSUri learnEpVersionAddEntity(SSServPar parA) throws Exception {
    try {
      ...
      dbSQL.startTrans(par.shouldCommit);
      SSServCaller.entityAdd(par.user, learnEpEntityUri, SSLabelStr.get(learnEpEntityUri.toString()), SSEntityEnum.learnEpEntity, false);
      SSServCaller.entityAdd(par.user, par.entityUri, SSLabelStr.get(par.entityUri.toString()), SSEntityEnum.entity, false);
  ...
  @Override public SSLearnEpVersion learnEpVersionGet(SSServPar parI) throws Exception {
  ...
  @Override public List<SSLearnEpVersion> learnEpVersionsGet(SSServPar parI) throws Exception {
  ...
  @Override public List<SSLearnEp> learnEpsGet(SSServPar parI) throws Exception {
  ...
  @Override public SSLearnEpVersion learnEpVersionCurrentGet(SSServPar parI) throws Exception {
  ...
  @Override public SSLearnEpTimelineState learnEpVersionGetTimelineState(SSServPar parI) throws Exception {
