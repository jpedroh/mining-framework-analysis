...
public class SSCollImpl extends SSServImplWithDBA implements SSCollClientI, SSCollServerI, SSEntityHandlerImplI {
  ...
  @Override public Boolean setUserEntityPublic(final SSUri userUri, final SSUri entityUri, final SSEntityEnum entityType, final SSUri publicCircleUri) throws Exception {
    if (!SSEntityEnum.equals(entityType, SSEntityEnum.coll)) {
      return false;
    }
    try {
      SSServCaller.collUserSetPublic(userUri, entityUri, publicCircleUri, false);
      return true;
  ...
  @Override public Boolean shareUserEntity(final SSUri userUri, final List<SSUri> userUrisToShareWith, final SSUri entityUri, final SSUri circleUri, final SSEntityEnum entityType) throws Exception {
    if (!SSEntityEnum.equals(entityType, SSEntityEnum.coll)) {
      return false;
    }
    try {
      for (SSUri userUriToShareWith : userUrisToShareWith) {
        SSServCaller.collUserShareWithUser(userUri, userUriToShareWith, entityUri, circleUri, false);
      }
      return true;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  @Override public Boolean addEntityToCircle(final SSUri userUri, final SSUri circleUri, final SSUri entityUri, final SSEntityEnum entityType) throws Exception {
    if (!SSEntityEnum.equals(entityType, SSEntityEnum.coll)) {
      return false;
    }
    try {
      SSServCaller.collToCircleAdd(userUri, circleUri, entityUri, false);
      return true;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
      return null;
  ...
  @Override public void removeDirectlyAdjoinedEntitiesForUser(final SSEntityEnum entityType, final SSEntityUserDirectlyAdjoinedEntitiesRemovePar par) throws Exception {
    if (!par.removeFromUserColls) {
      return;
    }
    try {
      for (SSColl coll : SSServCaller.collsUserEntityIsInGet(par.user, par.entityUri)) {
        SSServCaller.collUserEntryDelete(par.user, par.entityUri, coll.uri, true, false);
      }
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
    }
  ...
  @Override public SSEntityDescA getDescForEntity(final SSEntityEnum entityType, final SSUri userUri, final SSUri entityUri, final SSLabelStr label, final Long creationTime, final List<SSTag> tags, final SSRatingOverall overallRating, final List<SSUri> discUris, final SSUri author) throws Exception {
    if (!SSEntityEnum.equals(entityType, SSEntityEnum.coll)) {
      return SSEntityDesc.get(entityUri, label, creationTime, tags, overallRating, discUris, author);
    }
    return SSCollDesc.get(entityUri, label, creationTime, tags, overallRating, discUris, author);
  }

  @Override public void collUserParentGet(SSSocketCon sSCon, SSServPar parA) throws Exception {
    SSServCaller.checkKey(parA);
    SSColl collParent = collUserParentGet(parA);
    sSCon.writeRetFullToClient(SSCollUserParentGetRet.get(collParent, parA.op));
  }

  @Override public void collUserRootGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserRootGetRet.get(collUserRootGet(par), par.op));
  }

  @Override public void collUserEntryDelete(final SSSocketCon sSCon, final SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserEntryDeleteRet.get(collUserEntryDelete(par), par.op));
  }

  @Override public void collsUserCouldSubscribeGet(final SSSocketCon sSCon, final SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollsUserCouldSubscribeGetRet.get(collsUserCouldSubscribeGet(par), par.op));
  }

  @Override public void collUserEntriesDelete(final SSSocketCon sSCon, final SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserEntriesDeleteRet.get(collUserEntriesDelete(par), par.op));
  }

  @Override public void collUserEntryAdd(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserEntryAddRet.get(collUserEntryAdd(par), par.op));
  }

  @Override public void collUserEntriesAdd(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserEntriesAddRet.get(collUserEntriesAdd(par), par.op));
  }

  @Override public void collUserEntryChangePos(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserEntryChangePosRet.get(collUserEntryChangePos(par), par.op));
  }

  @Override public void collUserWithEntries(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserWithEntriesRet.get(collUserWithEntries(par), par.op));
  }

  @Override public void collsUserWithEntries(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollsUserWithEntriesRet.get(collsUserWithEntries(par), par.op));
  }

  @Override public void collUserHierarchyGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSCollUserHierarchyGetRet.get(collUserHierarchyGet(par), par.op));
  }

  @Override public void collUserCumulatedTagsGet(final SSSocketCon sSCon, final SSServPar parA) throws Exception {
    SSServCaller.checkKey(parA);
    sSCon.writeRetFullToClient(SSCollUserCummulatedTagsGetRet.get(collUserCumulatedTagsGet(parA), parA.op));
  }

  @Override public void collsUserEntityIsInGet(final SSSocketCon sSCon, final SSServPar parA) throws Exception {
    SSServCaller.checkKey(parA);
    sSCon.writeRetFullToClient(SSCollsUserEntityIsInGetRet.get(collsUserEntityIsInGet(parA), parA.op));
  }

  @Override public Boolean collUserRootAdd(final SSServPar parA) throws Exception {
    ...
    try {
      ...
      SSServCaller.entityAdd(par.user, rootCollUri, SSLabelStr.get(SSStrU.valueRoot), SSEntityEnum.coll, false);
      sqlFct.createColl(rootCollUri);
      SSServCaller.entityCircleCreate(par.user, rootCollUri, new ArrayList<SSUri>(), SSEntityCircleTypeE.priv, SSLabelStr.get(SSUri.toStr(par.user) + SSStrU.underline + SSStrU.valueRoot), SSUri.get(SSUserGlobals.systemUserURI), false);
      SSServCaller.entityUsersToCircleAdd(par.user, SSServCaller.entityCircleURIPublicGet(), par.user, false);
  ...
  @Override public SSUri collUserEntryAdd(final SSServPar parA) throws Exception {
  ...
  @Override public Boolean collUserEntriesAdd(final SSServPar parA) throws Exception {
    final SSCollUserEntriesAddPar par = new SSCollUserEntriesAddPar(parA);
    try {
      dbSQL.startTrans(par.shouldCommit);
      for (int counter = 0; counter < par.entries.size(); counter++) {
        SSServCaller.collUserEntryAdd(par.user, par.coll, par.entries.get(counter), par.entryLabels.get(counter), false, par.saveUE, false);
      }
      dbSQL.commit(par.shouldCommit);
      return true;
    } catch (SSSQLDeadLockErr deadLockErr) {
  ...
  @Override public Boolean collUserEntryDelete(final SSServPar parA) throws Exception {
  ...
  @Override public Boolean collUserEntriesDelete(final SSServPar parA) throws Exception {
  ...
  @Override public SSUri collUserShareWithUser(final SSServPar parA) throws Exception {
    try {
      ...
      if (sqlFct.containsEntry(rootCollUri, par.collUri) || sqlFct.ownsUserColl(par.userUriToShareWith, par.collUri)) {
      ...
      SSCollMiscFct.addCollAndSubCollsWithEntriesToCircle(sqlFct, par.user, sqlFct.getCollWithEntries(par.collUri, new ArrayList<SSEntityCircleTypeE>()), par.collCircleUri);
  ...
  @Override public SSUri collUserSetPublic(final SSServPar parA) throws Exception {
    ...
    try {
      ...
      dbSQL.startTrans(par.shouldCommit);
      SSCollMiscFct.addCollAndSubCollsWithEntriesToCircle(sqlFct, par.user, sqlFct.getCollWithEntries(par.collUri, new ArrayList<SSEntityCircleTypeE>()), par.publicCircleUri);
      dbSQL.commit(par.shouldCommit);
      return par.collUri;
    } catch (SSSQLDeadLockErr deadLockErr) {
      if (dbSQL.rollBack(parA)) {
  ...
  @Override public Boolean collUserEntryChangePos(SSServPar parA) throws Exception {
  ...
  @Override public SSColl collUserWithEntries(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public List<SSColl> collsUserWithEntries(SSServPar parA) throws Exception {
    try {
      ...
      for (String collUri : sqlFct.getAllUserCollURIs(par.user)) {
        userCollsWithEntries.add(SSCollMiscFct.getCollWithEntriesWithCircleTypes(sqlFct, par.user, SSUri.get(collUri)));
      }
  ...
  @Override public SSColl collUserRootGet(final SSServPar parA) throws Exception {
    final SSCollUserRootGetPar par = new SSCollUserRootGetPar(parA);
    try {
      return SSCollMiscFct.getCollWithEntriesWithCircleTypes(sqlFct, par.user, sqlFct.getUserRootCollURI(par.user));
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
      return null;
    }
  ...
  @Override public SSColl collUserParentGet(SSServPar parA) throws Exception {
    final SSCollUserParentGetPar par = new SSCollUserParentGetPar(parA);
    try {
      if (!SSServCaller.entityUserCanRead(par.user, par.coll)) {
        throw new Exception("user cannot change the order of entities in this coll");
      }
      return SSCollMiscFct.getCollWithEntriesWithCircleTypes(sqlFct, par.user, sqlFct.getUserDirectParentCollURI(par.user, par.coll));
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
      return null;
    }
  ...
  @Override public List<SSColl> collUserHierarchyGet(final SSServPar parA) throws Exception {
    try {
      ...
      for (SSUri collUri : hierarchy) {
        colls.add(SSColl.get(collUri, null, null, SSStrU.toString(SSServCaller.entityLabelGet(collUri)), null));
      }
      return colls;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  @Override public List<SSTagFrequ> collUserCumulatedTagsGet(final SSServPar parA) throws Exception {
    try {
      ...
      for (Map.Entry<String, Integer> entry : tagLabelFrequs.entrySet()) {
        tagFrequs.add(SSTagFrequ.get(SSTagLabel.get(entry.getKey()), null, entry.getValue()));
      }
      return tagFrequs;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  @Override public List<SSColl> collsUserEntityIsInGet(final SSServPar parA) throws Exception {
  ...
  @Override public List<SSColl> collsUserCouldSubscribeGet(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public SSUri collToCircleAdd(final SSServPar parA) throws Exception {
    try {
