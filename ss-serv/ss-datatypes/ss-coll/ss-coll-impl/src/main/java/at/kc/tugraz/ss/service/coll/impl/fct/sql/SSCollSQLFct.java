...
public class SSCollSQLFct extends SSDBSQLFct {
  ...
  public void addEntryToColl(final SSUri collParent, final SSUri collEntry, final SSLabelStr entryLabel) throws Exception {
  ...
  public List<SSColl> getAllPublicColls() throws Exception {
    ...
    try {
      ...
      resultSet = dbSQL.selectCertainDistinctWhere(tableNames, columnNames, where, whereFixed);
      while (resultSet.next()) {
        publicColls.add(SSColl.get(bindingStrToUri(resultSet, SSSQLVarU.id), null, bindingStrToUri(resultSet, SSSQLVarU.author), bindingStr(resultSet, SSSQLVarU.label), null));
      }
      return publicColls;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  public Boolean ownsUserColl(final SSUri userUri, final SSUri collUri) throws Exception {
    ResultSet resultSet = null;
    try {
      final Map<String, String> where = new HashMap<String, String>();
  ...
  public Boolean ownsUserASubColl(final SSUri userUri, final SSUri collUri) throws Exception {
  ...
  public void addCollToUserColl(final SSUri user, final SSUri collParent, final SSUri collChild, final Boolean createdCollIsInSharedOrPublicCircle, final Boolean addedCollIsSharedOrPublic) throws Exception {
  ...
  public void unlinkUserCollAndSubColls(final SSUri userUri, final SSUri parentCollUri, final SSUri collUri) throws Exception {
  ...
  public void getAllChildCollURIs(final String startCollUri, final String currentCollUri, final List<String> subCollUris) throws Exception {
  ...
  public void getAllParentCollURIs(final String startCollUri, final String currentCollUri, final List<String> parentCollUris) throws Exception {
  ...
  public void removeUserPrivateCollAndUnlinkSubColls(final SSUri userUri, final SSUri collUri) throws Exception {
    try {
      if (SSObjU.isNull(userUri, collUri)) {
  ...
  public void removeEntryFromColl(final SSUri collUri, final SSUri collEntryUri) throws Exception {
  ...
  public void changeCollEntriesPos(final SSUri collUri, final List<SSUri> collEntries, final List<Integer> order) throws Exception {
    if (SSObjU.isNull(collUri, collEntries, order) || collEntries.size() != order.size()) {
      SSServErrReg.regErrThrow(new Exception("pars not okay"));
      return;
    }
  ...
  private SSColl getColl(final SSUri collUri, final List<SSEntityCircleTypeE> circleTypes) throws Exception {
    ...
    try {
      ...
      resultSet = dbSQL.selectCertainWhere(tableNames, columnNames, whereParNamesWithValues, SSSQLVarU.collId + SSStrU.equal + SSSQLVarU.id);
      resultSet.first();
      return SSColl.get(collUri, null, bindingStrToUri(resultSet, SSSQLVarU.author), bindingStr(resultSet, SSSQLVarU.label), circleTypes);
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
      return null;
    } finally {
  ...
  public SSColl getCollWithEntries(final SSUri collUri, final List<SSEntityCircleTypeE> circleTypes) throws Exception {
    if (SSObjU.isNull(collUri, circleTypes)) {
      SSServErrReg.regErrThrow(new Exception("pars null"));
      return null;
    }
    ...
    try {
      ...
      resultSet = dbSQL.selectCertainWhereOrderBy(tableNames, columnNames, whereParNamesWithValues, SSSQLVarU.entryId + SSStrU.equal + SSSQLVarU.id, SSSQLVarU.pos, "ASC");
      while (resultSet.next()) {
        collEntry = SSCollEntry.get(bindingStrToUri(resultSet, SSSQLVarU.entryId), bindingStr(resultSet, SSSQLVarU.label), new ArrayList<SSEntityCircleTypeE>(), bindingStrToInteger(resultSet, SSSQLVarU.pos), bindingStrToEntityType(resultSet, SSSQLVarU.type));
  ...
  public Boolean containsEntry(final SSUri collUri, final SSUri collEntryUri) throws Exception {
    if (SSObjU.isNull(collUri, collEntryUri)) {
  ...
  public Boolean existsUserRootColl(final SSUri userUri) throws Exception {
    if (SSObjU.isNull(userUri)) {
      SSServErrReg.regErrThrow(new Exception("parameter(s) null"));
      return null;
    }
  ...
  public List<SSUri> getCollUsers(final SSUri collUri) throws Exception {
