...
public class SSDiscSQLFct extends SSDBSQLFct {
  ...
  public SSDisc getDiscWithoutEntries(final SSUri discUri) throws Exception {
    ...
    try {
      ...
      resultSet = dbSQL.selectCertainWhere(tableNames, columNames, whereParNamesWithValues, SSSQLVarU.discId + SSStrU.equal + SSSQLVarU.id);
      resultSet.first();
      return SSDisc.get(discUri, bindingStrToLabel(resultSet, SSSQLVarU.label), bindingStrToUri(resultSet, SSSQLVarU.author), bindingStrToUri(resultSet, SSSQLVarU.target), null);
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
      return null;
    } finally {
  ...
  public SSDisc getDiscWithEntries(final SSUri discUri) throws Exception {
    if (SSObjU.isNull(discUri)) {
      SSServErrReg.regErrThrow(new Exception("pars null"));
      return null;
    }
    ...
    try {
      ...
      resultSet = dbSQL.selectCertainDistinctWhere(tableNames, columNames, where, whereFixed);
      while (resultSet.next()) {
        discEntries.add(SSDiscEntry.get(bindingStrToUri(resultSet, SSSQLVarU.discEntryId), bindingStrToInteger(resultSet, SSSQLVarU.pos), SSDiscEntryContent.get(bindingStr(resultSet, SSSQLVarU.discEntryContent)), bindingStrToUri(resultSet, SSSQLVarU.author), bindingStrToLong(resultSet, SSSQLVarU.creationTime)));
      }
