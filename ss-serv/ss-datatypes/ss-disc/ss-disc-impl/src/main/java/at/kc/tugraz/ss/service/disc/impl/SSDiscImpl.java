...
public class SSDiscImpl extends SSServImplWithDBA implements SSDiscClientI, SSDiscServerI, SSEntityHandlerImplI {
  ...
  @Override public SSEntityDescA getDescForEntity(final SSEntityEnum entityType, final SSUri userUri, final SSUri entityUri, final SSLabelStr label, final Long creationTime, final List<SSTag> tags, final SSRatingOverall overallRating, final List<SSUri> discUris, final SSUri author) throws Exception {
    if (SSEntityEnum.equals(entityType, SSEntityEnum.disc)) {
      return SSDiscDesc.get(entityUri, label, creationTime, tags, overallRating, author);
    }
    if (SSEntityEnum.equals(entityType, SSEntityEnum.discEntry)) {
      return SSDiscEntryDesc.get(entityUri, label, creationTime, author);
    }
    return SSEntityDesc.get(entityUri, label, creationTime, tags, overallRating, discUris, author);
  }

  @Override public void discUserEntryAdd(final SSSocketCon sSCon, final SSServPar parA) throws Exception {
    SSServCaller.checkKey(parA);
    sSCon.writeRetFullToClient(discUserEntryAdd(parA));
  }

  @Override public void discsUserAllGet(final SSSocketCon sSCon, final SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSDiscsUserAllGetRet.get(discsUserAllGet(par), par.op));
  }

  @Override public void discUserWithEntriesGet(final SSSocketCon sSCon, final SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSDiscUserWithEntriesRet.get(discUserWithEntriesGet(par), par.op));
  }

  @Override public List<SSUri> discUrisUserForTargetGet(final SSServPar parA) throws Exception {
  ...
  @Override public SSDiscUserEntryAddRet discUserEntryAdd(final SSServPar parA) throws Exception {
    ...
    try {
      discUri = par.disc;
      if (!par.addNewDisc && par.content == null) {
        throw new Exception("pars not valid");
      }
      dbSQL.startTrans(par.shouldCommit);
      if (par.addNewDisc) {
        discUri = sqlFct.createDiscUri();
        SSServCaller.entityAdd(par.user, par.target, SSLabelStr.get(par.target.toString()), SSEntityEnum.entity, false);
        discLabel = SSServCaller.entityLabelGet(par.target);
        SSServCaller.entityAdd(par.user, discUri, discLabel, SSEntityEnum.disc, false);
      }
      if (par.content != null) {
        discEntryUri = sqlFct.createDiscEntryUri();
        SSServCaller.entityAdd(par.user, discEntryUri, SSLabelStr.get(discEntryUri.toString()), SSEntityEnum.discEntry, false);
      }
      if (par.addNewDisc) {
  ...
  @Override public List<SSDisc> discsUserWithEntriesGet(final SSServPar parA) throws Exception {
    ...
    try {
      for (SSDisc disc : SSServCaller.discsUserAllGet(par.user)) {
        discsWithEntries.add(SSServCaller.discUserWithEntriesGet(par.user, disc.uri, par.maxDiscEntries));
      }
