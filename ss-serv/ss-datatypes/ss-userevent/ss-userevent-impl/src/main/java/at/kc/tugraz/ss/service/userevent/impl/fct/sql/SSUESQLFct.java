...
public class SSUESQLFct extends SSDBSQLFct {
  ...
  public SSUE getUE(SSUri ueUri) throws Exception {
    ...
    try {
      ...
      resultSet.first();
      ue = SSUE.get(ueUri, bindingStrToUri(resultSet, SSSQLVarU.userId), SSUEEnum.get(bindingStr(resultSet, SSSQLVarU.eventType)), bindingStrToUri(resultSet, SSSQLVarU.entityId), bindingStr(resultSet, SSSQLVarU.content), null);
  ...
  public List<SSUE> getUEs(final SSUri forUserUri, final SSUri entityUri, final SSUEEnum eventType, final Long startTime, final Long endTime) throws Exception {
    ...
    try {
      resultSet = dbSQL.selectCertainWhere(tableNames, columnNames, whereParNamesWithValues, SSSQLVarU.userEventId + SSStrU.equal + SSSQLVarU.id);
      while (resultSet.next()) {
        ...
        ues.add(SSUE.get(bindingStrToUri(resultSet, SSSQLVarU.userEventId), bindingStrToUri(resultSet, SSSQLVarU.userId), eventTypeFromDB, bindingStrToUri(resultSet, SSSQLVarU.entityId), bindingStr(resultSet, SSSQLVarU.content), timestamp));
      }
      return ues;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  public void addUE(final SSUri ueUri, final SSUri userUri, final SSUri entityUri, final SSUEEnum eventType, final String content) throws Exception {
