...
public class SSLOMExtractorImpl extends SSServImplMiscA implements SSLOMExtractorClientI, SSLOMExtractorServerI {
  ...
  @Override public boolean lomExtractFromDir(SSServPar par) throws Exception {
  ...
  private void extractFromDir(Map<String, SSLOMResource> resources, File[] files, SSLOMExtractorStats stat) throws Exception {
    for (File file : files) {
      if (file.isDirectory()) {
        if (SSStrU.notEquals(file.getName(), SSStrU.valueOEORGANICEPRINTS) && ((SSLOMExtractorConf) conf).useEdunet) {
          extractFromDir(resources, file.listFiles(), stat);
        }
        if (SSStrU.equals(file.getName(), SSStrU.valueOEORGANICEPRINTS) && ((SSLOMExtractorConf) conf).useEPrints) {
          extractFromDir(resources, file.listFiles(), stat);
        }
      } else {
