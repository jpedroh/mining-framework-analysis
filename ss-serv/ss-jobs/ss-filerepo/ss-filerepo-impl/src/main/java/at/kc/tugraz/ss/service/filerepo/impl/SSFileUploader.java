...
public class SSFileUploader extends SSServImplStartA {
  ...
  public SSFileUploader(final SSFileRepoConf fileRepoConf, final SSSocketCon sSCon, final SSServPar par) throws Exception {
    super(fileRepoConf);
    this.sSCon = sSCon;
  ...
  private void uploadFileToWebDav() throws Exception {
    try {
      ...
      SardineFactory.begin(((SSFileRepoConf) conf).user, ((SSFileRepoConf) conf).password).put(((SSFileRepoConf) conf).getPath() + fileId, fileInputStream);
  ...
  private void addFileEntity() throws Exception {
    SSServCaller.entityAdd(par.user, uri, par.fileName, SSEntityEnum.file, true);
  }
}