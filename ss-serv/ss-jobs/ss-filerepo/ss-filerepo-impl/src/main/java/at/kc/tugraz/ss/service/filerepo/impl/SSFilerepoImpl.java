...
public class SSFilerepoImpl extends SSServImplMiscA implements SSFileRepoClientI, SSFileRepoServerI, SSEntityHandlerImplI {
  ...
  public SSFilerepoImpl(final SSFileRepoConf conf, final Map<String, SSFileRepoFileAccessProperty> fileAccessProps) throws Exception {
    super(conf);
  ...
  @Override public SSEntityDescA getDescForEntity(final SSEntityEnum entityType, final SSUri userUri, final SSUri entityUri, final SSLabelStr label, final Long creationTime, final List<SSTag> tags, final SSRatingOverall overallRating, final List<SSUri> discUris, final SSUri author) throws Exception {
    ...
    return SSFileDesc.get(entityUri, label, creationTime, tags, overallRating, discUris, author);
  }
   ...
   * SSFileRepoClientI *****
   */
  @Override public void fileCanWrite(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(fileCanWrite(par));
  }

  @Override public void fileSetReaderOrWriter(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(fileSetReaderOrWriter(par));
  }

  @Override public void fileRemoveReaderOrWriter(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(fileRemoveReaderOrWriter(par));
  }

  @Override public void fileWritingMinutesLeft(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(fileWritingMinutesLeft(par));
  }

  @Override public void fileUserFileWrites(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(fileUserFileWrites(par));
  }

  @Override public void fileDownload(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    new Thread(new SSFileDownloader((SSFileRepoConf) conf, sSCon, par)).start();
  }

  @Override public void fileReplace(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    new Thread(new SSFileReplacer((SSFileRepoConf) conf, sSCon, par, fileAccessProps)).start();
  }

  @Override public void fileUpload(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    new Thread(new SSFileUploader((SSFileRepoConf) conf, sSCon, par)).start();
  }

  @Override public void fileThumbGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    new Thread(new SSFileThumbGetter((SSFileRepoConf) conf, sSCon, par)).start();
  }

  @Override public void fileExtGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
   ...
   * SSFileRepoServerI 
   */
  @Override public SSUri fileCreateUri(SSServPar parA) throws Exception {
    SSFileCreateUriPar par = new SSFileCreateUriPar(parA);
    return fct.createFileUri(par.fileExtension);
  }

  @Override public String fileExtGet(final SSServPar parA) throws Exception {
  ...
  @Override public void fileUpdateWritingMinutes(SSServPar parI) {
  ...
  @Override public SSFileGetEditingFilesRet fileUserFileWrites(SSServPar parI) throws Exception {
  ...
  @Override public SSFileCanWriteRet fileCanWrite(SSServPar parI) throws Exception {
  ...
  @Override public SSFileSetReaderOrWriterRet fileSetReaderOrWriter(SSServPar parI) throws Exception {
  ...
  @Override public SSFileRemoveReaderOrWriterRet fileRemoveReaderOrWriter(SSServPar parI) throws Exception {
  ...
  @Override public SSFileWritingMinutesLeftRet fileWritingMinutesLeft(SSServPar parI) throws Exception {
  ...
  @Override public SSUri fileUriFromID(SSServPar parA) throws Exception {
  ...
  @Override public String fileIDFromURI(final SSServPar parA) throws Exception {
