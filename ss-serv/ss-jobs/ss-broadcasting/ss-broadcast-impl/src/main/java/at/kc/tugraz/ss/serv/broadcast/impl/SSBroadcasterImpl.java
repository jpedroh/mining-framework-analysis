...
public class SSBroadcasterImpl extends SSServImplMiscA implements SSBroadcasterClientI, SSBroadcasterServerI {
  ...
  @Override public void broadcastUpdate(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSBroadcastUpdateRet.get(broadcastUpdate(par), par.op));
  }

  @Override public void broadcastUpdateTimeGet(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSBroadcastUpdateTimeGetRet.get(broadcastUpdateTimeGet(par), par.op));
  }

  @Override public void broadcastServerTime(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSBroadcastServerTimeRet.get(broadcastServerTime(par), par.op));
  }

  /****** SSBroadcasterServerI ******/
  @Override public boolean broadcastUpdate(SSServPar parI) throws Exception {
    SSBroadcastUpdatePar par = new SSBroadcastUpdatePar(parI);
    updateList.add(SSBroadcast.get(par.type, par.resource, par.user));
    return true;
  }

  @Override public List<SSBroadcast> broadcastUpdateTimeGet(SSServPar par) throws Exception {
  ...
  @Override public Long broadcastServerTime(SSServPar par) throws Exception {
