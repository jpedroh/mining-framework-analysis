...
public class SSI5CloudImpl extends SSServImplMiscA implements SSI5CloudClientI, SSI5CloudServerI {
  ...
  @Override public Map<String, String> i5CloudAuth(final SSServPar parA) throws Exception {
  ...
  @Override public Boolean i5CloudFileUpload(final SSServPar parA) throws Exception {
    ...
    try {
      ...
      SSFileU.readFileBytes(i5CloudCon.getOutputStream(), SSFileU.openFileForRead(SSFileU.dirWorkingTmp() + par.fileName));
  ...
  @Override public Boolean i5CloudFileDownload(final SSServPar parA) throws Exception {
    final SSI5CloudFileDownloadPar par = new SSI5CloudFileDownloadPar(parA);
    final HttpURLConnection i5CloudCon;
    try {
      i5CloudCon = (HttpURLConnection) new URL(SSFileU.correctDirPath(((SSI5CloudConf) conf).uri) + "storage/" + par.space + SSStrU.slash + par.fileName).openConnection();
      i5CloudCon.setRequestProperty(SSHTMLU.xAuthToken, par.xAuthToken);
      SSFileU.writeFileBytes(SSFileU.openOrCreateFileWithPathForWrite(SSFileU.dirWorkingTmp() + par.fileName), i5CloudCon.getInputStream());
      return true;
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
      return null;
