...
public class SSDataImportEvernoteHelper {
  ...
  public void handleNotebooks(SSDataImportEvernotePar par) throws Exception {
    ...
    for (Notebook notebook : SSServCaller.getEvernoteNotebooks(evernoteInfo.noteStore)) {
      ...
      SSServCaller.entityAddAtCreationTime(userUri, notebookUri, notebookLabel, notebook.getServiceCreated(), SSEntityEnum.evernoteNotebook, false);
      ...
      handleEvernoteNotes(userUri, evernoteInfo, notebook, isSharedNotebook);
    }
  }
  ...
  public void handleLinkedNotebooks() throws Exception {
    ...
    for (LinkedNotebook linkedNotebook : SSServCaller.getEvernoteLinkedNotebooks(evernoteInfo.noteStore)) {
      ...
      timeCounter++;
      SSServCaller.entityAddAtCreationTime(userUri, notebookUri, notebookLabel, creationTimeForLinkedNotebook, SSEntityEnum.evernoteNotebook, false);
      evernoteHelper.ueHelper.addUEsForLinkedNotebook(userUri, notebookUri, creationTimeForLinkedNotebook);
    }
  }

  private void handleEvernoteNotes(SSUri userUri, SSEvernoteInfo evernoteInfo, Notebook notebook, Boolean isSharedNotebook) throws Exception {
    SSUri noteUri;
    SSLabelStr noteLabel;
    for (Note note : SSServCaller.getEvernoteNotes(evernoteInfo.noteStore, notebook.getGuid())) {
      ...
      SSServCaller.entityAddAtCreationTime(userUri, noteUri, noteLabel, note.getCreated(), SSEntityEnum.evernoteNote, false);
  ...
  private void handleEvernoteResources(SSUri userUri, Notebook notebook, SSEvernoteInfo evernoteInfo, Note note, Boolean isSharedNotebook, List<SharedNotebook> sharedNotebooks) throws Exception {
    SSUri resourceUri;
    SSLabelStr resourceLabel;
    if (note == null || note.getResources() == null) {
      return;
    }
    for (Resource resource : note.getResources()) {
      ...
      SSServCaller.entityAddAtCreationTime(userUri, resourceUri, resourceLabel, note.getUpdated(), SSEntityEnum.evernoteResource, false);
  ...
  private void handleEvernoteNoteContent(Note note) throws Exception {
    ...
    try {
      ...
      evernoteNoteWritePNG(pdfFilePath, pngFilePath);
  ...
  private void evernoteNoteWritePNG(String pdfFilePath, String pngFilePath) throws Exception {
    ...
    try {
      ...
      pdfToImgDecoder.openPdfFile(pdfFilePath);
      pdfToImgDecoder.setExtractionMode(0, 1f);
