...
public class SSAuthImpl extends SSServImplWithDBA implements SSAuthClientI, SSAuthServerI {
  ...
  @Override public void authUsersFromCSVFileAdd(final SSServPar parA) throws Exception {
    try {
      ...
      for (Map.Entry<String, String> passwordForUser : passwordsForUsersFromCSVFile.entrySet()) {
        SSServCaller.authRegisterUser(SSUserGlobals.systemUser, SSLabel.get(passwordForUser.getKey()), passwordForUser.getValue(), false);
      }
  ...
  @Override public SSUri authRegisterUser(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public SSAuthCheckCredRet authCheckCred(final SSServPar parA) throws Exception {
  ...
  @Override public void authLoadKeys(final SSServPar parA) throws Exception {
    try {
  ...
  @Override public void authCheckKey(final SSServPar parA) throws Exception {
    try {
      switch (((SSAuthConf) conf).authType) {
        ...
        case csvFileAuth:
        {
          if (parA.key.equals(noAuthKey) || !keys.contains(parA.key)) {
            throw new Exception("login key wrong");
          }
        }
      }
