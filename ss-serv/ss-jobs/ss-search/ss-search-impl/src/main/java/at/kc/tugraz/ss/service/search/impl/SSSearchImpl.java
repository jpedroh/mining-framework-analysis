...
public class SSSearchImpl extends SSServImplMiscA implements SSSearchClientI, SSSearchServerI {
  ...
  /****** SSSearchClientI ******/
  @Override public void searchTags(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSSearchTagsRet.get(searchTags(par), par.op));
  }

  @Override public void searchMIs(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    List<SSSearchResult> searchResults = searchMIs(par);
    sSCon.writeRetFullToClient(SSSearchMIsRet.get(searchResults, par.op));
  }

  @Override public void searchSolr(SSSocketCon sSCon, SSServPar par) throws Exception {
    SSServCaller.checkKey(par);
    sSCon.writeRetFullToClient(SSSearchSolrRet.get(searchSolr(par), par.op));
  }

  /****** SSSearchServerI ******/
  @Override public List<SSSearchResult> searchTags(final SSServPar parA) throws Exception {
    ...
    try {
      for (SSTagLabel tagString : par.tags) {
        ...
        for (SSUri foundEntity : SSServCaller.tagUserEntitiesForTagGet(par.user, tagString, SSSpaceEnum.sharedSpace)) {
          searchResultsForTagOneTag.add(new SSSearchResult(foundEntity, SSSpaceEnum.sharedSpace));
        }
        for (SSUri foundEntity : SSServCaller.tagUserEntitiesForTagGet(par.user, tagString, SSSpaceEnum.privateSpace)) {
          searchResultsForTagOneTag.add(new SSSearchResult(foundEntity, SSSpaceEnum.privateSpace));
        }
  ...
  @Override public List<SSSearchResult> searchSolr(final SSServPar parA) throws Exception {
    ...
    try {
      for (SSSolrKeywordLabel keyword : par.keywords) {
        ...
        for (String entityId : SSServCaller.solrSearch(keyword, 20)) {
          SSSearchMiscFct.getPublicAndPrivateResults(par.user, SSServCaller.fileUriFromID(par.user, entityId), searchResultsForOneKeyword);
        }
  ...
  @Override public List<SSSearchResult> searchMIs(final SSServPar parA) throws Exception {
    ...
    try {
      for (SSModelUEMILabel mi : par.mIs) {
        ...
        for (SSUri entityUri : SSServCaller.modelUEEntitiesForMiGet(par.user, mi)) {
          SSSearchMiscFct.getPublicAndPrivateResults(par.user, entityUri, searchResultsForOneKeyword);
        }
