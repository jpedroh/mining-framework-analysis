...
public class SSModelUEPersonPropertySetter {
  ...
  public SSModelUEPersonPropertySetter(final SSServImplA serv, final List<SSUE> sortedEventsSinceLastUpdate, final Map<String, SSModelUEResource> resources) {
  ...
  public void setPersonIndependentProperties(SSModelUEResource resource) throws Exception {
  ...
  private void addTopicUsingEvent(SSModelUEResource user, SSUE event) {
    List<SSUE> events;
    if (user.personsUsingTopicEvents.containsKey(event.content) == false) {
      events = new ArrayList<SSUE>();
      events.add(event);
      user.personsUsingTopicEvents.put(event.content, events);
    } else {
  ...
  private void addTopicCreationEvent(SSModelUEResource user, SSUE event) {
    List<SSUE> events;
    if (user.personsCreatedTopicEvents.containsKey(event.content) == false) {
      events = new ArrayList<SSUE>();
      events.add(event);
      user.personsCreatedTopicEvents.put(event.content, events);
    } else {
  ...
  private void setPersonsTopics(SSModelUEResource resource) throws Exception {
    ...
    for (Map.Entry<String, Integer> topicAndFrequency : resource.personsTopicFrequencies.entrySet()) {
      topicScore = new SSModelUETopicScore(SSTagLabel.get(topicAndFrequency.getKey()), -1, topicAndFrequency.getValue());
      if (topicAndFrequency.getValue() > thirdThreshold * 2) {
        topicScore.level = 1;
      } else {
        if (topicAndFrequency.getValue() > thirdThreshold) {
          topicScore.level = 2;
        } else {
          topicScore.level = 3;
        }
  ...
  private void setPersonsRelatedPersons(SSModelUEResource resource) {
    ...
    for (SSUri relatedResourceUrl : resource.personsRelatedResources) {
      for (SSUri personUrl : resources.get(SSStrU.toString(relatedResourceUrl)).relatedPersons) {
        if (!SSUri.isSame(personUrl, resource.resourceUrl) && SSUri.containsNot(resource.personsRelatedPersons, personUrl)) {
  ...
  private void setRecentProperties(SSModelUEResource resource, SSUE event, List<Long> recentTimeStamps) throws Exception {
    ...
    if (SSUEEnum.contains(SSModelUEU.useTopicEventTypes, event.type) && event.timestamp > recentTimeStamps.get(1)) {
      resource.personsRecentTopic = SSTagLabel.get(event.content);
      recentTimeStamps.set(1, event.timestamp);
    }
  }

  private void increaseCountersDiscussionOf(SSModelUEResource resource, SSUE event) {
    if (SSUEEnum.isSame(event.type, SSUEEnum.addDiscussionComment) && SSUri.contains(resource.personsDiscussions, event.resource)) {
      resource.personsDiscussions.add(event.resource);
    }
  }

  private void increaseCountersRelateResource(SSModelUEResource resource, SSUE event) {
    if (SSUEEnum.contains(SSModelUEU.relateResourceEventTypes, event.type) && SSUri.containsNot(resource.personsRelatedResources, event.resource)) {
      resource.counters.put(SSModelUEResourceCounterEnum.counterPersonsRelatedResources.toString(), resource.counters.get(SSModelUEResourceCounterEnum.counterPersonsRelatedResources.toString()) + 1);
