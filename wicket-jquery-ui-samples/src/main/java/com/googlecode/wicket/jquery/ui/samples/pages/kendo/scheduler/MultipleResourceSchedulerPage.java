...
public class MultipleResourceSchedulerPage extends AbstractSchedulerPage {
  ...
  public MultipleResourceSchedulerPage() {
    ...
    final Scheduler scheduler = new Scheduler("scheduler", newSchedulerModel(), options) {
      ...
      @Override public void onCreate(AjaxRequestTarget target, SchedulerEvent event) {
        EmployeeEventsDAO.get().create(event);
        this.info("Created: " + event);
        target.add(feedback);
      }

      @Override public void onUpdate(AjaxRequestTarget target, SchedulerEvent event) {
        EmployeeEventsDAO.get().update(event);
        this.info("Updated: " + event);
        target.add(feedback);
      }

      @Override public void onDelete(AjaxRequestTarget target, SchedulerEvent event) {
  ...
  static SchedulerModel newSchedulerModel() {
    return new SchedulerModel() {
      ...
      @Override protected List<SchedulerEvent> load() {
  ...
  static ResourceList newRoomList() {
    ResourceList list = new ResourceList("Room", "roomId", "Rooms");
  ...
  static ResourceList newEmployeeList() {
    ResourceList list = new ResourceList("Employee", "employeeId", true);
    list.add(new Resource(EmployeeEventsDAO.EMPLOYEE_1, "Patrick", "#339966"));
