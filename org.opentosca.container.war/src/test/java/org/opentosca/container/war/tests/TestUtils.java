...
public class TestUtils {
  ...
  private Path getRepositoryPath(String testRemoteRepositoryUrl) {
    ...
    repositoryPath = Paths.get(System.getProperty("java.io.tmpdir")).resolve("opentosca-test-repository-" + repoSuffix);
    LOGGER.info("Using repository path \'{}\'", repositoryPath);
    return repositoryPath;
  }

  public Csar loadCSARFromRepositoryIntoStorage(RepositoryConfigurationObject.RepositoryProvider provider, QName serviceTemplateId, CsarStorageService storage, Path repositoryInputPath, String remoteUrl) throws Exception {
  ...
  private IRepository fetchRepository(RepositoryConfigurationObject.RepositoryProvider provider, Path repositoryInputPath, String remoteUrl) throws GitAPIException, IOException {
    ...
    if (!Files.exists(repositoryPath.resolve(".git")) && remoteUrl != null && !Files.exists(repositoryPath.resolve(DEFAULT_LOCAL_REPO_NAME).resolve(".git"))) {
      ...
      if (remoteUrl == null) {
        ...
        try {
          isCorrectRepository = Git.open(repositoryPath.toFile()).remoteList().call().stream().anyMatch((remote) -> remote.getURIs().stream().anyMatch((uri) -> uri.toASCIIString().equals(remoteUrl)));
        } catch (Exception e) {
          try {
            isCorrectRepository = Git.open(repositoryPath.resolve(DEFAULT_LOCAL_REPO_NAME).toFile()).remoteList().call().stream().anyMatch((remote) -> remote.getURIs().stream().anyMatch((uri) -> uri.toASCIIString().equals(remoteUrl)));
          } catch (Exception e1) {
    ...
    if (!isInitializedRepo) {
      RepositoryFactory.reconfigure(new GitBasedRepositoryConfiguration(false, new FileBasedRepositoryConfiguration(repositoryPath, provider)));
    } else {
  ...
  private void cloneRepo(Path repositoryPath, String remoteUrl) throws IOException, GitAPIException {
    ...
    Git.cloneRepository().setURI(remoteUrl).setBare(false).setCloneAllBranches(true).setDirectory(repositoryPath.toFile()).call();
  }
  ...
  public Collection<Endpoint> getDeployedPlans(ICoreEndpointService endpointService) {
    ...
    return endpoints.stream().filter((endpoint) -> endpoint.getMetadata() != null && endpoint.getMetadata().containsKey("PlanType") && endpoint.getMetadata().containsKey("EndpointType") && endpoint.getMetadata().get("EndpointType").equals("Invoke")).collect(Collectors.toList());
  }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  public TPlan getBuildPlan(List<TPlan> plans) {
    for (TPlan plan : plans) {
      if (PlanType.fromString(plan.getPlanType()).equals(PlanType.BUILD) && !plan.getId().toLowerCase().contains(OpenTOSCA_DefrostPlanOperation) && plan.getId().toLowerCase().contains("buildplan")) {
        return plan;
      }
    }
    return null;
  ...
  public TPlan getTerminationPlan(List<TPlan> plans) {
    for (TPlan plan : plans) {
      if (PlanType.fromString(plan.getPlanType()).equals(PlanType.TERMINATION) && !plan.getId().toLowerCase().contains(OpenTOSCA_FreezePlanOperation) && plan.getId().toLowerCase().contains("terminationplan")) {
        return plan;
      }
    }
    return null;
