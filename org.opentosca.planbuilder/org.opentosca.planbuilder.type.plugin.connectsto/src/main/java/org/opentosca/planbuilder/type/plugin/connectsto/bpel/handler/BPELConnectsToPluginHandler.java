...
public class BPELConnectsToPluginHandler implements ConnectsToPluginHandler<BPELPlanContext> {
  ...
  @Override public boolean handle(final BPELPlanContext templateContext) {
    ...
    if (hasOperation(targetNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_CONNECTTO, templateContext.getCsar())) {
      if (!ModelUtils.hasInterface(targetNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_NON_INTERRUPTIVE, templateContext.getCsar()) && startAndStopAvailable(targetNodeTemplate, templateContext.getCsar())) {
        final String ifaceName = getInterface(targetNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_STOP, templateContext.getCsar());
        templateContext.executeOperation(targetNodeTemplate, ifaceName, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_STOP, null);
      }
      executeConnectsTo(templateContext, targetNodeTemplate, sourceNodeTemplate, targetNodeTemplate);
      if (!ModelUtils.hasInterface(targetNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_NON_INTERRUPTIVE, templateContext.getCsar()) && startAndStopAvailable(targetNodeTemplate, templateContext.getCsar())) {
        templateContext.executeOperation(targetNodeTemplate, getInterface(targetNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_START, templateContext.getCsar()), Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_START, null);
      }
    }
    if (hasOperation(sourceNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_CONNECTTO, templateContext.getCsar())) {
      if (!ModelUtils.hasInterface(sourceNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_NON_INTERRUPTIVE, templateContext.getCsar()) && startAndStopAvailable(sourceNodeTemplate, templateContext.getCsar())) {
        templateContext.executeOperation(sourceNodeTemplate, getInterface(sourceNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_STOP, templateContext.getCsar()), Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_STOP, null);
      }
      executeConnectsTo(templateContext, sourceNodeTemplate, sourceNodeTemplate, targetNodeTemplate);
      if (!ModelUtils.hasInterface(sourceNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_NON_INTERRUPTIVE, templateContext.getCsar()) && startAndStopAvailable(sourceNodeTemplate, templateContext.getCsar())) {
        templateContext.executeOperation(sourceNodeTemplate, getInterface(sourceNodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_START, templateContext.getCsar()), Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_START, null);
      }
    }
    return true;
  }
  ...
  /**
     ...
     */
  private boolean executeConnectsTo(final BPELPlanContext templateContext, final TNodeTemplate connectToNode, final TNodeTemplate sourceParameterNode, final TNodeTemplate targetParameterNode) {
    ...
    for (final TInterface iface : ModelUtils.findNodeType(connectToNode, templateContext.getCsar()).getInterfaces()) {
      for (final TOperation op : iface.getOperations()) {
        if (op.getName().equals(Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_CONNECTTO)) {
          ...
          param2propertyMapping = findInputParameters(templateContext, op, connectToNode, sourceParameterNode, targetParameterNode);
          if (param2propertyMapping.size() != op.getInputParameters().size() && !allRequiredParamsAreMatched(op.getInputParameters(), param2propertyMapping)) {
    ...
    final Boolean result = templateContext.executeOperation(connectToNode, connectsToIface.getName(), connectsToOp.getName(), param2propertyMapping);
  ...
  /**
     ...
     */
  private boolean findInputParameter(final BPELPlanContext templateContext, final TNodeTemplate connectToNode, final TNodeTemplate sourceParameterNode, final TNodeTemplate targetParameterNode, Map<TParameter, Variable> param2propertyMapping, TParameter param) {
    final Variable var = templateContext.getPropertyVariable(templateContext.getRelationshipTemplate(), param.getName());
    if (var != null) {
  ...
  /**
     ...
     */
  private boolean findInputPrefixedParameter(final BPELPlanContext templateContext, final TNodeTemplate sourceParameterNode, final TNodeTemplate targetParameterNode, Map<TParameter, Variable> param2propertyMapping, TParameter param) {
  ...
  /**
     ...
     */
  private Map<TParameter, Variable> findInputParameters(final BPELPlanContext templateContext, final TOperation connectsToOp, final TNodeTemplate connectToNode, final TNodeTemplate sourceParameterNode, final TNodeTemplate targetParameterNode) {
  ...
  private boolean searchBasedOnDefinedOnSource(final boolean definedOnSource, final BPELPlanContext templateContext, final String paramName, final TParameter param, final Map<TParameter, Variable> param2propertyMapping, final TNodeTemplate sourceParameterNode, final TNodeTemplate targetParameterNode) {
    if (definedOnSource) {
  ...
  private boolean searchAndAddIfFound(final BPELPlanContext templateContext, final String paramName, final TParameter param, final Map<TParameter, Variable> param2propertyMapping, final TNodeTemplate... parametersRootNodes) {
  ...
  private boolean searchAndAddIfFound(final BPELPlanContext templateContext, final TNodeTemplate parametersRootNode, final String paramName, final TParameter param, final Map<TParameter, Variable> param2propertyMapping) {
  ...
  /**
     ...
     */
  private Variable searchPropertyInStack(final PlanContext templateContext, TNodeTemplate currentNode, final String propName) {
  ...
  private TNodeTemplate fetchNodeConnectedWithHostedOn(final TNodeTemplate nodeTemplate, Csar csar) {
    for (final TRelationshipTemplate relation : ModelUtils.getOutgoingRelations(nodeTemplate, csar)) {
      if (ModelUtils.getRelationshipTypeHierarchy(ModelUtils.findRelationshipType(relation, csar), csar).contains(Types.hostedOnRelationType)) {
  ...
  private boolean startAndStopAvailable(final TNodeTemplate nodeTemplate, Csar csar) {
    return hasOperation(nodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_STOP, csar) & hasOperation(nodeTemplate, Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_LIFECYCLE_START, csar);
  }
  ...
  private boolean hasOperation(final TNodeTemplate nodeTemplate, final String operationName, Csar csar) {
    ...
    if (Objects.isNull(nodeType.getInterfaces())) {
      ...
      return nodeType.getInterfaces().stream().flatMap((inter) -> inter.getOperations().stream()).filter((op) -> op.getName().equals(operationName)).findFirst().isPresent();
    }
  }
  ...
  /**
     ...
     */
  private boolean allRequiredParamsAreMatched(final List<TParameter> inputParameters, final Map<TParameter, Variable> param2propertyMapping) {
