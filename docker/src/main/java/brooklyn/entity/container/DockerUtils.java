...
public class DockerUtils {
  ...
  public static final Set<String> URL_SENSOR_NAMES = ImmutableSet.<String>of(WebAppServiceConstants.ROOT_URL.getName(), DatastoreMixins.DATASTORE_URL.getName(), CouchbaseNode.COUCHBASE_WEB_ADMIN_URL.getName(), MessageBroker.BROKER_URL.getName());

  public static final Set<String> BLACKLIST_URL_SENSOR_NAMES = ImmutableSet.<String>of(SoftwareProcess.DOWNLOAD_URL.getName(), CouchbaseCluster.COUCHBASE_CLUSTER_CONNECTION_URL.getName());
  ...
  public static final CharMatcher DOCKERFILE_CHARACTERS = CharMatcher.anyOf("_").or(CharMatcher.inRange('a', 'z')).or(CharMatcher.inRange('0', '9'));
  ...
  public static <T extends java.lang.Object> AttributeSensor<T> mappedSensor(AttributeSensor<?> source) {
  ...
  public static void configureEnrichers(SubnetTier subnetTier, Entity entity) {
    for (AttributeSensor sensor : Iterables.filter(entity.getEntityType().getSensors(), AttributeSensor.class)) {
      if ((DockerUtils.URL_SENSOR_NAMES.contains(sensor.getName()) || sensor.getName().endsWith(".url") || URI.class.isAssignableFrom(sensor.getType())) && !DockerUtils.BLACKLIST_URL_SENSOR_NAMES.contains(sensor.getName())) {
        ...
        entity.addEnricher(subnetTier.uriTransformingEnricher(EntityAndAttribute.create(entity, sensor), target));
        ...
        if (sensor.getName().matches("docker\\.port\\.[0-9]+") || PortAttributeSensorAndConfigKey.class.isAssignableFrom(sensor.getClass())) {
          AttributeSensor<String> target = DockerUtils.mappedPortSensor(sensor);
          entity.addEnricher(subnetTier.hostAndPortTransformingEnricher(EntityAndAttribute.create(entity, sensor), target));
  ...
  public static Optional<String> getContainerName(Entity target) {
    return Optional.fromNullable(target.sensors().get(DockerContainer.DOCKER_CONTAINER_NAME)).or(Optional.fromNullable(target.config().get(DockerContainer.DOCKER_CONTAINER_NAME))).or(Optional.fromNullable(getContainerNameFromPlan(target))).transform(DockerUtils.ALLOWED);
  }
  ...
  public static void addExtraPublicKeys(Entity entity, SshMachineLocation location) {
    ...
    if (extraPublicKey != null) {
      ...
      ProcessTaskWrapper<Integer> task = SshTasks.newSshExecTaskFactory(location, cmd).summary("Add public key").requiringExitCodeZero().newTask();
