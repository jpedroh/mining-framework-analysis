...
public class DockerHostSshDriver extends AbstractSoftwareProcessSshDriver implements DockerHostDriver {
  ...
  /** {@inheritDoc} */
  @Override public String buildImage(String dockerfile, Optional<String> entrypoint, Optional<String> contextArchive, String name, boolean useSsh, Map<String, Object> substitutions) {
    String imageId;
    ...
    if (!ArchiveType.UNKNOWN.equals(ArchiveType.of(dockerfile)) || Urls.isDirectory(dockerfile)) {
      ...
      if (contextArchive.isPresent()) {
        ...
        ProcessTaskWrapper<Integer> task = SshEffectorTasks.ssh(format("mkdir -p %s", imageDir)).machine(getMachine()).newTask();
  ...
  /** {@inheritDoc} */
  @Override public String layerSshableImageOn(String fullyQualifiedImageName) {
    checkNotNull(fullyQualifiedImageName, "fullyQualifiedImageName");
    copyTemplate(DockerUtils.SSHD_DOCKERFILE, Os.mergePaths(fullyQualifiedImageName, "Sshd" + DockerUtils.DOCKERFILE), true, ImmutableMap.<String, Object>of("fullyQualifiedImageName", fullyQualifiedImageName));
  ...
  @Override public void install() {
    ...
    if (osDetails.isMac()) {
      commands.add(alternatives(ifExecutableElse1("boot2docker", "boot2docker status || boot2docker init"), fail("Mac OSX install requires Boot2Docker preinstalled", 1)));
    }
    if (osDetails.isLinux()) {
  ...
  private void executeKernelInstallation(List<String> commands) {
    ...
    Task<Boolean> sshable = TaskBuilder.<Boolean>builder().displayName("Waiting until host is SSHable").body(new Callable<Boolean>() {
      @Override public Boolean call() throws Exception {
        return Repeater.create().every(Duration.TEN_SECONDS).until(new Callable<Boolean>() {
          public Boolean call() {
            return getLocation().isSshable();
          }
        }).limitTimeTo(Duration.minutes(15)).run();
      }
    }).build();
    Boolean result = DynamicTasks.queueIfPossible(sshable).orSubmitAndBlock().andWaitForSuccess();
    if (!result) {
      throw new IllegalStateException(String.format("The entity %s is not sshable after reboot (waited %s)", entity, Time.makeTimeStringRounded(stopwatchForReboot)));
    }
  }
  ...
  private String useYum(String osVersion, String arch, String epelRelease) {
    ...
    return chainGroup(alternatives(sudo("rpm -qa | grep epel-release"), sudo(format("rpm -Uvh http://dl.fedoraproject.org/pub/epel/%s/%s/epel-release-%s.noarch.rpm", osMajorVersion, arch, epelRelease))));
  }
  ...
  private String installDockerOnUbuntu() {
    ...
    return chainGroup(installPackage("apt-transport-https"), "echo \'deb https://get.docker.com/ubuntu docker main\' | " + sudo("tee -a /etc/apt/sources.list.d/docker.list"), sudo("apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 36A1D7869245C8950F966E92D8576A8BA88D21E9"), installPackage("lxc-docker-" + version));
  }
  ...
  @Override public void customize() {
    ...
    if (entity.config().get(DockerInfrastructure.DOCKER_GENERATE_TLS_CERTIFICATES)) {
      newScript(ImmutableMap.of(NON_STANDARD_LAYOUT, "true"), CUSTOMIZING).body.append(String.format("cp ca-cert.pem %s/ca.pem", getRunDir()), String.format("cp server-cert.pem %s/cert.pem", getRunDir()), String.format("cp server-key.pem %s/key.pem", getRunDir())).failOnNonZeroResultCode().execute();
    }
    ...
    newScript(CUSTOMIZING).body.append(chainGroup(sudo("mkdir -p " + repoCAPath), sudo("cp ca.pem " + repoCAPath + "/ca.crt"))).failOnNonZeroResultCode().execute();
    newScript(CUSTOMIZING + "-upstart").body.append(chain(sudo("mkdir -p /etc/default"), format("echo \'DOCKER_OPTS=\"-H tcp://0.0.0.0:%d -H unix:///var/run/docker.sock %s --tlsverify --tls --tlscert=%s/cert.pem --tlskey=%<s/key.pem --tlscacert=%<s/ca.pem\"\' | ", getDockerPort(), getStorageOpts(), getRunDir()) + sudo("tee -a /etc/default/docker"), sudo("groupadd -f docker"), sudo(format("gpasswd -a %s docker", getMachine().getUser())), sudo("newgrp docker"))).failOnNonZeroResultCode().execute();
    newScript(CUSTOMIZING + "-centos").body.append(chain(sudo("mkdir -p /etc/sysconfig"), format("echo \'other_args=\"--selinux-enabled -H tcp://0.0.0.0:%d -H unix:///var/run/docker.sock -e lxc %s --tlsverify --tls --tlscert=%s/cert.pem --tlskey=%<s/key.pem --tlscacert=%<s/ca.pem\"\' | ", getDockerPort(), getStorageOpts(), getRunDir()) + sudo("tee -a /etc/sysconfig/docker"))).failOnNonZeroResultCode().execute();
    String systemDConfigInstallPath = Os.mergePaths(getInstallDir(), "docker.service");
    copyTemplate("classpath://brooklyn/entity/container/docker/SystemDTemplate", systemDConfigInstallPath, true, ImmutableMap.of("args", format("-H tcp://0.0.0.0:%d -H unix:///var/run/docker.sock %s --tlsverify --tls --tlscert=%s/cert.pem --tlskey=%<s/key.pem --tlscacert=%<s/ca.pem", getDockerPort(), getStorageOpts(), getRunDir())));
    newScript(CUSTOMIZING + "-systemd").body.append(chain(sudo("mkdir -p /etc/systemd/system"), sudo(String.format("cp %s %s", systemDConfigInstallPath, "/etc/systemd/system/docker.service")), ifExecutableElse0("systemctl", sudo("systemctl daemon-reload")))).failOnNonZeroResultCode().execute();
  ...
  @Override public boolean isRunning() {
    ScriptHelper helper = newScript(CHECK_RUNNING).body.append(alternatives(ifExecutableElse1("boot2docker", "boot2docker status"), ifExecutableElse1("service", sudo("service docker status")))).noExtraOutput().gatherOutput();
    helper.execute();
    return helper.getResultStdout().contains("running");
  }

  @Override public void stop() {
    newScript(STOPPING).body.append(alternatives(ifExecutableElse1("boot2docker", "boot2docker down"), ifExecutableElse1("service", sudo("service docker stop")))).failOnNonZeroResultCode().execute();
  }

  @Override public void launch() {
    newScript(LAUNCHING).body.append(alternatives(ifExecutableElse1("boot2docker", "boot2docker up"), ifExecutableElse1("service", sudo("service docker start")))).failOnNonZeroResultCode().uniqueSshConnection().execute();
  }

  @Override public Map<String, String> getShellEnvironment() {
    ImmutableMap.Builder<String, String> builder = ImmutableMap.<String, String>builder().putAll(super.getShellEnvironment());
