...
public class DockerHostImpl extends MachineEntityImpl implements DockerHost {
  ...
  @Override public void init() {
    ...
    EntitySpec<?> dockerContainerSpec = EntitySpec.create(config().get(DOCKER_CONTAINER_SPEC)).configure(DockerContainer.DOCKER_HOST, this).configure(DockerContainer.DOCKER_INFRASTRUCTURE, getInfrastructure());
    if (config().get(DockerInfrastructure.HA_POLICY_ENABLE)) {
      dockerContainerSpec.policy(PolicySpec.create(ServiceRestarter.class).configure(ServiceRestarter.FAILURE_SENSOR_TO_MONITOR, ServiceFailureDetector.ENTITY_FAILED));
    }
    ...
    if (config().get(DockerInfrastructure.HA_POLICY_ENABLE)) {
      containers.addPolicy(PolicySpec.create(ServiceReplacer.class).configure(ServiceReplacer.FAILURE_SENSOR_TO_MONITOR, ServiceRestarter.ENTITY_RESTART_FAILED));
    }
    sensors().set(DOCKER_CONTAINER_CLUSTER, containers);
    if (Entities.isManaged(this)) {
      Entities.manage(containers);
    }
    addEnricher(Enrichers.builder().propagating(ImmutableMap.of(DynamicCluster.GROUP_SIZE, DockerAttributes.DOCKER_CONTAINER_COUNT)).from(containers).build());
  }
  ...
  @Override protected Collection<Integer> getRequiredOpenPorts() {
    ...
    if (config().get(DockerInfrastructure.SDN_ENABLE)) {
      Entity sdn = sensors().get(DockerHost.DOCKER_INFRASTRUCTURE).sensors().get(DockerInfrastructure.SDN_PROVIDER);
  ...
  @Override protected Map<String, Object> obtainProvisioningFlags(MachineProvisioningLocation location) {
    ...
    if (location instanceof JcloudsLocation) {
      Set<ConfigKey<?>> imageChoiceToRespect = ImmutableSet.<ConfigKey<?>>of(JcloudsLocationConfig.TEMPLATE_BUILDER, JcloudsLocationConfig.IMAGE_CHOOSER, JcloudsLocationConfig.IMAGE_ID, JcloudsLocationConfig.IMAGE_NAME_REGEX, JcloudsLocationConfig.IMAGE_DESCRIPTION_REGEX, JcloudsLocationConfig.OS_FAMILY, JcloudsLocationConfig.OS_VERSION_REGEX, JcloudsLocationConfig.OS_64_BIT);
      Set<ConfigKey<?>> hardwareChoiceToRespect = ImmutableSet.<ConfigKey<?>>of(JcloudsLocationConfig.HARDWARE_ID, JcloudsLocationConfig.MIN_RAM, JcloudsLocationConfig.MIN_CORES, JcloudsLocationConfig.MIN_DISK);
      ...
      if (Strings.isNonBlank(securityGroup)) {
        ...
        flags.put(JcloudsLocationConfig.JCLOUDS_LOCATION_CUSTOMIZERS.getName(), ImmutableList.of(JcloudsLocationSecurityGroupCustomizer.getInstance(getApplicationId())));
      }
      if (isJcloudsLocation(location, SoftLayerConstants.SOFTLAYER_PROVIDER_NAME)) {
        if (template == null) {
          template = new PortableTemplateBuilder();
        ...
        if (DockerUtils.isSdnProvider(this, "SdnVeNetwork")) {
          ...
          options.diskType(Volume.Type.LOCAL.name());
        }
  ...
  @Override public Integer resize(Integer desiredSize) {
  ...
  @Override public Integer getCurrentSize() {
  ...
  @Override public DockerHostDriver getDriver() {
    return (DockerHostDriver) super.getDriver();
  }

  @Override public Integer getDockerPort() {
    return sensors().get(DOCKER_SSL_PORT);
  }

  @Override public String getDockerHostName() {
    return sensors().get(DOCKER_HOST_NAME);
  }

  @Override public List<Entity> getDockerContainerList() {
    return ImmutableList.copyOf(getDockerContainerCluster().getMembers());
  }

  @Override public DockerInfrastructure getInfrastructure() {
  ...
  /** {@inheritDoc} */
  @Override public String buildImage(String dockerFile, @Nullable String entrypoint, @Nullable String contextArchive, String name, boolean useSsh, Map<String, Object> substitutions) {
  ...
  @Override public String layerSshableImageOnFullyQualified(String fullyQualifiedName) {
  ...
  @Override public String layerSshableImageOn(String baseImage, String tag) {
  ...
  /** {@inheritDoc} */
  @Override public String runDockerCommand(String command) {
    return runDockerCommandTimeout(command, Duration.FIVE_MINUTES);
  }

  /** {@inheritDoc} */
  @Override public String runDockerCommandTimeout(String command, Duration timeout) {
  ...
  /** {@inheritDoc} */
  @Override public String deployArchive(String url) {
  ...
  @Override public DockerHostLocation getDynamicLocation() {
    return (DockerHostLocation) sensors().get(DYNAMIC_LOCATION);
  }

  @Override public boolean isLocationAvailable() {
    return getDynamicLocation() != null;
  }

  @Override public DynamicCluster getDockerContainerCluster() {
    return sensors().get(DOCKER_CONTAINER_CLUSTER);
  }

  @Override public JcloudsLocation getJcloudsLocation() {
    return sensors().get(JCLOUDS_DOCKER_LOCATION);
  }

  @Override public SubnetTier getSubnetTier() {
    return sensors().get(DOCKER_HOST_SUBNET_TIER);
  }

  @Override public int execCommandStatus(String command) {
    return execCommandStatusTimeout(command, Duration.seconds(15));
  }

  @Override public int execCommandStatusTimeout(String command, Duration timeout) {
    ProcessTaskWrapper<Object> task = SshEffectorTasks.ssh(command).environmentVariables(((AbstractSoftwareProcessSshDriver) getDriver()).getShellEnvironment()).returning(ScriptReturnType.EXIT_CODE).allowingNonZeroExitCode().machine(getMachine()).summary(command).newTask();
    try {
      Object result = DynamicTasks.queueIfPossible(task).executionContext(this).orSubmitAsync().asTask().get(timeout);
  ...
  @Override public Optional<String> getImageNamed(String name) {
    return getImageNamed(name, "latest");
  }

  @Override public Optional<String> getImageNamed(String name, String tag) {
  ...
  /**
     ...
     */
  @Override public DockerHostLocation createLocation(Map<String, ?> flags) {
  ...
  @Override public void deleteLocation() {
  ...
  @Override public void configureSecurityGroups() {
    Collection<IpPermission> permissions = getIpPermissions();
    addIpPermissions(permissions);
  }

  @Override public void removeIpPermissions(Collection<IpPermission> permissions) {
  ...
  @Override public void addIpPermissions(Collection<IpPermission> permissions) {
    Location location = getDriver().getLocation();
    String securityGroup = config().get(DockerInfrastructure.SECURITY_GROUP);
    if (Strings.isBlank(securityGroup)) {
      if (!(location instanceof JcloudsSshMachineLocation)) {
  ...
  protected Collection<IpPermission> getIpPermissions() {
    ...
    if (config().get(DockerInfrastructure.DOCKER_SHOULD_START_REGISTRY) && sensors().get(DynamicGroup.FIRST_MEMBER)) {
      IpPermission dockerRegistryPort = IpPermission.builder().ipProtocol(IpProtocol.TCP).fromPort(config().get(DockerRegistry.DOCKER_REGISTRY_PORT)).toPort(config().get(DockerRegistry.DOCKER_REGISTRY_PORT)).cidrBlock(Cidr.UNIVERSAL.toString()).build();
  ...
  private List<IpPermission> getClockerPermisionsForCIDR(String cidr) {
    List<IpPermission> permissions = MutableList.of();
    IpPermission dockerPort = IpPermission.builder().ipProtocol(IpProtocol.TCP).fromPort(sensors().get(DockerHost.DOCKER_PORT)).toPort(sensors().get(DockerHost.DOCKER_PORT)).cidrBlock(cidr).build();
    IpPermission dockerSslPort = IpPermission.builder().ipProtocol(IpProtocol.TCP).fromPort(sensors().get(DockerHost.DOCKER_SSL_PORT)).toPort(sensors().get(DockerHost.DOCKER_SSL_PORT)).cidrBlock(cidr).build();
  ...
  @Override protected void preStart() {
    ...
    if (DockerUtils.isSdnProvider(this, "CalicoNetwork")) {
      dockerPort = sensors().get(DockerHost.DOCKER_INFRASTRUCTURE).sensors().get(DockerInfrastructure.SDN_PROVIDER).config().get(CalicoNode.POWERSTRIP_PORT);
      tlsEnabled = false;
    }
    if (DockerUtils.isSdnProvider(this, "WeaveNetwork")) {
      dockerPort = sensors().get(DockerHost.DOCKER_INFRASTRUCTURE).sensors().get(DockerInfrastructure.SDN_PROVIDER).config().get(WeaveContainer.WEAVE_PROXY_PORT);
      tlsEnabled = true;
    }
    Maybe<SshMachineLocation> found = Machines.findUniqueSshMachineLocation(getLocations());
    String dockerLocationSpec = String.format("jclouds:docker:%s://%s:%s", tlsEnabled ? "https" : "http", found.get().getSshHostAndPort().getHostText(), dockerPort);
    ...
    if (config().get(DockerInfrastructure.DOCKER_GENERATE_TLS_CERTIFICATES)) {
      ...
      getMachine().execCommands("createCertificates", ImmutableList.of("chmod 755 create-certs.sh", "./create-certs.sh " + sensors().get(ADDRESS)));
    ...
    JcloudsLocation jcloudsLocation = (JcloudsLocation) getManagementContext().getLocationRegistry().resolve(dockerLocationSpec, MutableMap.builder().put("identity", certPath).put("credential", keyPath).put(ComputeServiceProperties.IMAGE_LOGIN_USER, "root:" + getPassword()).build());
    ...
    SubnetTier subnetTier = addChild(EntitySpec.create(SubnetTier.class, SubnetTierImpl.class).configure(SubnetTier.PORT_FORWARDER, portForwarder).configure(SubnetTier.SUBNET_CIDR, Cidr.UNIVERSAL));
    Entities.manage(subnetTier);
    subnetTier.start(ImmutableList.of(found.get()));
    sensors().set(DOCKER_HOST_SUBNET_TIER, subnetTier);
    Map<String, ?> flags = MutableMap.<String, Object>builder().putAll(config().get(LOCATION_FLAGS)).put("machine", found.get()).put("jcloudsLocation", jcloudsLocation).put("portForwarder", portForwarder).build();
  ...
  private FunctionFeed scanner() {
    ...
    return FunctionFeed.builder().entity(this).poll(new FunctionPollConfig<Object, Void>(SCAN).period(interval).description("Scan Containers").callable(new Callable<Void>() {
      @Override public Void call() throws Exception {
        scanContainers();
        return null;
      }
    }).onFailureOrException(Functions.<Void>constant(null))).build();
  }
  ...
  @Override public void preStop() {
    ...
    if (agent != null && Entities.isManaged(agent)) {
      boolean agentStopped = Entities.invokeEffector(this, agent, Startable.STOP).blockUntilEnded(Duration.TEN_SECONDS);
      if (!agentStopped) {
    ...
    try {
      ...
      if (ps.size() > 1) {
        for (int i = 1; i < ps.size(); i++) {
          ...
          Optional<Entity> container = Iterables.tryFind(getDockerContainerCluster().getMembers(), Predicates.compose(StringPredicates.startsWith(id), EntityFunctions.attribute(DockerContainer.CONTAINER_ID)));
          ...
          EntitySpec<DockerContainer> containerSpec = EntitySpec.create(config().get(DOCKER_CONTAINER_SPEC)).configure(SoftwareProcess.ENTITY_STARTED, Boolean.TRUE).configure(DockerContainer.DOCKER_HOST, this).configure(DockerContainer.DOCKER_INFRASTRUCTURE, getInfrastructure()).configure(DockerContainer.DOCKER_IMAGE_ID, imageId).configure(DockerContainer.DOCKER_IMAGE_NAME, imageName).configure(DockerContainer.LOCATION_FLAGS, MutableMap.of("container", getMachine()));
  ...
  @Override protected void connectServiceUpIsRunning() {
    serviceUpIsRunningFeed = SshFeed.builder().entity(this).period(Duration.FIVE_SECONDS).machine(this.getMachine()).poll(new SshPollConfig<Boolean>(SERVICE_UP).command("docker ps").onSuccess(Functions.constant(true)).onFailureOrException(Functions.constant(false))).build();
  }

  @Override protected void disconnectServiceUpIsRunning() {
    if (serviceUpIsRunningFeed != null) {
      serviceUpIsRunningFeed.stop();
    }
  }
