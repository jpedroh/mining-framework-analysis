...
public class DockerContainerImpl extends BasicStartableImpl implements DockerContainer {
  ...
  private Optional<String> getContainerName(Entity target) {
    return Optional.fromNullable(target.sensors().get(DOCKER_CONTAINER_NAME)).or(Optional.fromNullable(target.config().get(DOCKER_CONTAINER_NAME))).or(Optional.fromNullable(target.config().get(BrooklynCampConstants.PLAN_ID))).transform(DockerUtils.ALLOWED);
  }
