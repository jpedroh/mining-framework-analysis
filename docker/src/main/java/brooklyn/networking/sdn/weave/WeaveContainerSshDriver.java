...
public class WeaveContainerSshDriver extends AbstractSoftwareProcessSshDriver implements WeaveContainerDriver {
  ...
  @Override public void launch() {
    ...
    newScript(MutableMap.of(USE_PID_FILE, false), LAUNCHING).body.append(chain(BashCommands.sudo(String.format("%s launch-router -iprange %s %s ", getWeaveCommand(), entity.config().get(SdnProvider.CONTAINER_NETWORK_CIDR), Boolean.TRUE.equals(firstMember) ? "" : first.sensors().get(Attributes.SUBNET_ADDRESS))), BashCommands.sudo(String.format("%s launch-proxy -H tcp://[::]:%d --tlsverify --tls --tlscert=%s/cert.pem --tlskey=%<s/key.pem --tlscacert=%<s/ca.pem", getWeaveCommand(), entity.config().get(DockerHost.DOCKER_SSL_PORT).iterator().next(), getRunDir())))).failOnNonZeroResultCode().uniqueSshConnection().execute();
  }

  @Override public boolean isRunning() {
    getEntity().sensors().get(SdnAgent.DOCKER_HOST).getDynamicLocation().getLock().lock();
    try {
      return newScript(MutableMap.of(USE_PID_FILE, false), CHECK_RUNNING).body.append(BashCommands.sudo(getWeaveCommand() + " status")).execute() == 0;
    }  finally {
  ...
  @Override public void stop() {
    newScript(MutableMap.of(USE_PID_FILE, false), STOPPING).body.append(BashCommands.sudo(getWeaveCommand() + " stop")).execute();
  }

  @Override public void createSubnet(String svirtualNetworkId, String subnetId, Cidr subnetCidr) {
    LOG.debug("Nothing to do for Weave subnet creation");
  }

  @Override public InetAddress attachNetwork(String containerId, String subnetId) {
    ...
    try {
      ...
      ((WeaveContainer) getEntity()).getDockerHost().execCommand(BashCommands.sudo(String.format("%s attach %s/%d %s", getWeaveCommand(), address.getHostAddress(), cidr.getLength(), containerId)));
      return address;
    }  finally {
