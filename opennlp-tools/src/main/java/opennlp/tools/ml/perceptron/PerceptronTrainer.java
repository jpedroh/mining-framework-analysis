...
public class PerceptronTrainer extends AbstractEventTrainer {
  ...
  public AbstractModel doTrain(DataIndexer indexer) throws IOException {
    ...
    double tolerance = parameters.getDoubleParam("Tolerance", PerceptronTrainer.TOLERANCE_DEFAULT);
  ...
  private MutableContext[] findParameters(int iterations, boolean useAverage) {
    ...
    for (int i = 1; i <= iterations; i++) {
      ...
      if (Math.abs(prevAccuracy1 - trainingAccuracy) < tolerance && Math.abs(prevAccuracy2 - trainingAccuracy) < tolerance && Math.abs(prevAccuracy3 - trainingAccuracy) < tolerance) {
