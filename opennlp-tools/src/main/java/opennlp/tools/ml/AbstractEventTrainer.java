...
public abstract class AbstractEventTrainer extends AbstractTrainer implements EventTrainer {
  ...
  @Override public boolean isValid() {
    ...
    String dataIndexer = parameters.getStringParam(DATA_INDEXER_PARAM, DATA_INDEXER_TWO_PASS_VALUE);
    if (dataIndexer != null) {
      if (!(DATA_INDEXER_ONE_PASS_VALUE.equals(dataIndexer) || DATA_INDEXER_TWO_PASS_VALUE.equals(dataIndexer))) {
        return false;
      }
    }
    return true;
  ...
  public DataIndexer getDataIndexer(ObjectStream<Event> events) throws IOException {
    String dataIndexerName = parameters.getStringParam(DATA_INDEXER_PARAM, DATA_INDEXER_TWO_PASS_VALUE);
    ...
    if (DATA_INDEXER_ONE_PASS_VALUE.equals(dataIndexerName)) {
      ...
      if (DATA_INDEXER_TWO_PASS_VALUE.equals(dataIndexerName)) {
