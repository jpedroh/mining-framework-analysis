      ...
      for (BratAnnotation ann : sample.getAnnotations()) {
        if (isSpanAnnotation(ann)) {
          ...
          for (int i = 1; i < mappedFragments.size(); i++) {
            if (mappedFragments.get(i - 1).getEnd() == mappedFragments.get(i).getStart()) {
              mappedFragments.set(i, new Span(mappedFragments.get(i - 1).getStart(), mappedFragments.get(i).getEnd(), mappedFragments.get(i).getType()));
