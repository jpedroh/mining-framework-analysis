...
public class FixedWidthDataContext extends QueryPostprocessDataContext {
  ...
  @Override protected Schema getMainSchema() throws MetaModelException {
    ...
    try {
      final boolean hasColumnHeader = _configuration.getColumnNameLineNumber() != FixedWidthConfiguration.NO_COLUMN_NAME_LINE;
      if (hasColumnHeader) {
      ...
      if (columnNames != null) {
        try (ColumnNamingSession columnNamingSession = columnNamingStrategy.startColumnNamingSession()) {
          for (int i = 0; i < columnNames.length; i++) {
            ...
            columnNames[i] = columnNamingSession.getNextColumnName(new ColumnNamingContextImpl(table, intrinsicColumnName, i));
          }
        }
      }
    }  finally {
  ...
  private FixedWidthReader createReader() {
    ...
    if (_configuration instanceof EbcdicConfiguration) {
      reader = new EbcdicReader((BufferedInputStream) inputStream, _configuration.getEncoding(), _configuration.getValueWidths(), _configuration.isFailOnInconsistentLineWidth(), ((EbcdicConfiguration) _configuration).isSkipEbcdicHeader(), ((EbcdicConfiguration) _configuration).isEolPresent());
    } else {
      if (_configuration.isConstantValueWidth()) {
        reader = new FixedWidthReader(inputStream, _configuration.getEncoding(), _configuration.getFixedValueWidth(), _configuration.isFailOnInconsistentLineWidth());
      } else {
        reader = new FixedWidthReader(inputStream, _configuration.getEncoding(), _configuration.getValueWidths(), _configuration.isFailOnInconsistentLineWidth());
      }
    }
    return reader;
  }
