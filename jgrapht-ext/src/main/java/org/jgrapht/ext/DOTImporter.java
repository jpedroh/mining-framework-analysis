 ...
 * @author Wil Selwood
 */
public class DOTImporter<V extends java.lang.Object, E extends java.lang.Object> implements GraphImporter<V, E> {
  ...
  /**
     ...
     * @throws ImportException if there is a problem parsing the file.
     */
  @Deprecated public void read(String input, AbstractBaseGraph<V, E> graph) throws ImportException {
  ...
  private int processHeader(String input, int position, StringBuilder sectionBuffer, Graph<V, E> graph) throws ImportException {
    ...
    if (current == '{') {
      ...
      int i = 0;
      if (graph instanceof AbstractBaseGraph && ((AbstractBaseGraph<V, E>) graph).isAllowingMultipleEdges() && headerParts[i].equals("strict")) {
  ...
  private int findNextQuote(String input, int start) {
    ...
    } while((input.charAt(result - 1) == '\\') && !((input.charAt(result - 1) == '\\') && (input.charAt(result - 2) == '\\')));
    return result;
  }
}