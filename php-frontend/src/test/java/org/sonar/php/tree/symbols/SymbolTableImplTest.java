...
public class SymbolTableImplTest extends ParsingTestUtils {
  ...
  @Test public void case_sensitivity() {
    ...
    assertThat(constUp.usages()).hasSize(0);
  ...
  @Test public void symbols_filtering() {
  ...
  @Test public void test_class_fields() {
  ...
  @Test public void test_global_constant() {
  ...
  @Test public void global_variable() {
  ...
  @Test public void retrieve_symbol_by_tree() {
    ExpressionTree dollarAUsage = ((AssignmentExpressionTree) ((ExpressionStatementTree) ((FunctionDeclarationTree) cut.script().statements().get(5)).body().statements().get(3)).expression()).variable();
    Symbol symbol = SYMBOL_MODEL.getSymbol(dollarAUsage);
    assertThat(symbol).isNotNull();
    assertThat(symbol.name()).isEqualTo("$a");
  }

  @Test public void built_in_variables() {
  ...
  @Test public void qualified_name_for_classes() {
  ...
  @Test public void qn_class_symbol_usages() {
  ...
  @Test public void use_statements() {
  ...
  @Test public void use_statements_aliased_name() {
    SymbolTableImpl symbolTable = symbolTableFor("<?php \n" + "namespace N1\\N2 { class A {} }\n" + "use N1\\N2;\n" + "$a = new N2\\A();");
    assertSymbolUsages(symbolTable, "N1\\N2\\A", 4);
  }

  @Test public void global_and_alias_usage() {
    SymbolTableImpl symbolTable = symbolTableFor("<?php \n" + "class A {} \n" + "namespace N { class A {} }\n" + "use N\\A;\n" + "$a = new A();\n" + "$a = new \\A();");
  ...
  @Test public void use_statements_group() {
  ...
  @Test public void usage_before_declaration() {
  ...
  @Test public void function_usage_before_declaration() {
    CompilationUnitTree cut = parseSource("<?php namespace N {\n" + "f();\n" + "function f() {}" + "}");
    SymbolTableImpl symbolTable = SymbolTableImpl.create(cut);
  ...
  @Test public void nested_function() {
  ...
  @Test public void nested_function_inside_method() {
  ...
  @Test public void undeclared_class_usage() {
  ...
  @Test public void undeclared_class_usage_with_fully_qualified_name() {
    SymbolTableImpl symbolTable = symbolTableFor("<?php $dbh = new \\PDO(\'odbc:sample\', \'db2inst1\', \'ibmdb2\');");
    Symbol symbol = symbolTable.getSymbol("pdo");
    assertThat(symbol).isInstanceOf(UndeclaredSymbol.class);
    SyntaxToken usage = Iterables.getOnlyElement(symbol.usages());
  ...
  @Test public void undeclared_class_usage_in_namespace() {
  ...
  @Test public void undeclared_function_usage() {
  ...
  @Test public void test_type_symbol() {
  ...
  @Test public void test_superclass_with_qualified_name() {
  ...
  @Test public void test_class_symbol_with_interfaces() {
  ...
  @Test public void test_anonymous_class() {
  ...
  @Test public void test_class_symbol_members() {
    ...
    assertThat(classA.members()).extracting((m) -> m.qualifiedName().toString()).containsExactly("n\\a::a", "n\\a::foo");
    ...
    assertThat(classB.members()).extracting((m) -> m.qualifiedName().toString()).containsExactly("b::bar");
    assertThat(classB.members()).extracting(MemberSymbol::owner).allMatch(classB::equals);
  }

  @Test public void test_class_symbol_members_case_insensitive() {
  ...
  @Test public void static_invocation() {
  ...
  @Test public void new_expression_wo_brackets() {
  ...
  @Test public void anonymous_class() {
  ...
  @Test public void global_statement_with_compound_variable() {
    SymbolTableImpl symbolTable = symbolTableFor("<?php global ${foo()};");
    assertThat(symbolTable.getSymbol("foo")).isNotNull();
  }

  @Test public void lexical_vars_tree_symbol_association() {
  ...
  private static ListAssert<String> assertClassSymbols(SymbolTableImpl symbolTable, String... fullyQualifiedNames) {
    return assertThat(symbolTable.getSymbols(Kind.CLASS)).extracting((s) -> s.qualifiedName().toString()).containsExactly(fullyQualifiedNames);
  }

  private static ListAssert<String> assertFunctionSymbols(SymbolTableImpl symbolTable, String... fullyQualifiedNames) {
    return assertThat(symbolTable.getSymbols(Kind.FUNCTION)).extracting((s) -> s.qualifiedName().toString()).containsExactly(fullyQualifiedNames);
  }
