...
public class TestESGenericInvoiceBuilder extends ESAbstractTest {
  ...
  @Test public void doTest() {
    MockESGenericInvoiceEntity mock = this.createMockEntity(MockESGenericInvoiceEntity.class, TestESGenericInvoiceBuilder.ES_GENERIC_INVOICE_YML);
    MockESCustomerEntity mockCustomerEntity = this.createMockEntity(MockESCustomerEntity.class, TestESGenericInvoiceBuilder.ESCUSTOMER_YML);
    mock.setCurrency(Currency.getInstance("EUR"));
    Mockito.when(this.getInstance(DAOESGenericInvoice.class).getEntityInstance()).thenReturn(new MockESGenericInvoiceEntity());
    Mockito.when(this.getInstance(DAOESCustomer.class).get(Matchers.any(UID.class))).thenReturn(mockCustomerEntity);
    MockESGenericInvoiceEntryEntity entryMock = this.createMockEntity(MockESGenericInvoiceEntryEntity.class, TestESGenericInvoiceBuilder.ES_GENERIC_INVOICE_ENTRY_YML);
    Mockito.when(this.getInstance(DAOESGenericInvoiceEntry.class).get(Matchers.any(UID.class))).thenReturn(entryMock);
    ...
    MockESPaymentEntity mockPayment = this.createMockEntity(MockESPaymentEntity.class, TestESGenericInvoiceBuilder.ES_PAYMENT_YML);
