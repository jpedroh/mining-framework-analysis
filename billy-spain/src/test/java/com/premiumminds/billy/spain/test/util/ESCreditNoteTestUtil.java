...
public class ESCreditNoteTestUtil {
  ...
  public ESCreditNote.Builder getCreditNoteBuilder(ESInvoiceEntity reference) {
    ...
    return creditNoteBuilder.setBilled(ESCreditNoteTestUtil.BILLED).setCancelled(ESCreditNoteTestUtil.CANCELLED).setSelfBilled(ESCreditNoteTestUtil.SELFBILL).setDate(new Date()).setSourceId(ESCreditNoteTestUtil.SOURCEID).addEntry(creditNoteEntryBuilder).setBusinessUID(reference.getBusiness().getUID()).setCustomerUID(reference.getCustomer().getUID()).addPayment(this.payment.getPaymentBuilder());
  }
}