...
public class TestESCreditReceiptBuilder extends ESAbstractTest {
  private static final String ES_CREDIT_RECEIPT_YML = AbstractTest.YML_CONFIGS_DIR + "ESCreditReceipt.yml";
  ...
  @Test public void doTest() {
    MockESCreditReceiptEntity mock = this.createMockEntity(MockESCreditReceiptEntity.class, TestESCreditReceiptBuilder.ES_CREDIT_RECEIPT_YML);
    mock.setCurrency(Currency.getInstance("EUR"));
    MockESCustomerEntity mockCustomerEntity = this.createMockEntity(MockESCustomerEntity.class, TestESCreditReceiptBuilder.ES_CUSTOMER_YML);
    ...
    Mockito.when(this.getInstance(DAOESCreditReceipt.class).getEntityInstance()).thenReturn(new MockESCreditReceiptEntity());
    MockESCreditReceiptEntryEntity entryMock = this.createMockEntity(MockESCreditReceiptEntryEntity.class, TestESCreditReceiptBuilder.ES_CREDIT_RECEIPT_ENTRY_YML);
    Mockito.when(this.getInstance(DAOESCreditReceiptEntry.class).get(Matchers.any(UID.class))).thenReturn(entryMock);
    mock.getEntries().add(entryMock);
    ArrayList<ESCreditReceiptEntry> creditNodeEntries = (ArrayList<ESCreditReceiptEntry>) mock.getEntries();
    ...
    MockESPaymentEntity mockPayment = this.createMockEntity(MockESPaymentEntity.class, TestESCreditReceiptBuilder.ES_PAYMENT_YML);
