...
public class TestESCreditNoteBuilder extends ESAbstractTest {
  ...
  @Test public void doTest() {
    MockESCreditNoteEntity mock = this.createMockEntity(MockESCreditNoteEntity.class, TestESCreditNoteBuilder.ES_CREDIT_NOTE_YML);
    mock.setCurrency(Currency.getInstance("EUR"));
    MockESCustomerEntity mockCustomerEntity = this.createMockEntity(MockESCustomerEntity.class, TestESCreditNoteBuilder.ESCUSTOMER_YML);
    Mockito.when(this.getInstance(DAOESCustomer.class).get(Matchers.any(UID.class))).thenReturn(mockCustomerEntity);
    Mockito.when(this.getInstance(DAOESSimpleInvoice.class).getEntityInstance()).thenReturn(new MockESSimpleInvoiceEntity());
    Mockito.when(this.getInstance(DAOESCreditNote.class).getEntityInstance()).thenReturn(new MockESCreditNoteEntity());
    MockESCreditNoteEntryEntity entryMock = this.createMockEntity(MockESCreditNoteEntryEntity.class, TestESCreditNoteBuilder.ES_CREDIT_NOTE_ENTRY_YML);
    ...
    MockESPaymentEntity mockPayment = this.createMockEntity(MockESPaymentEntity.class, TestESCreditNoteBuilder.ES_PAYMENT_YML);
