...
public class TestESCreditNoteEntryBuilder extends ESAbstractTest {
  ...
  @Test public void doTest() {
    MockESCreditNoteEntryEntity mock = this.createMockEntity(MockESCreditNoteEntryEntity.class, TestESCreditNoteEntryBuilder.ES_CREDIT_NOTE_ENTRY_YML);
    mock.setCurrency(Currency.getInstance("EUR"));
    Mockito.when(this.getInstance(DAOESCreditNoteEntry.class).getEntityInstance()).thenReturn(new MockESCreditNoteEntryEntity());
    MockESInvoiceEntity mockInvoiceEntity = this.createMockEntity(MockESInvoiceEntity.class, TestESCreditNoteEntryBuilder.ES_INVOICE_YML);
    Mockito.when(this.getInstance(DAOESInvoice.class).get(Matchers.any(UID.class))).thenReturn(mockInvoiceEntity);
