...
public class TestDAOESInvoice extends ESPersistencyAbstractTest {
  ...
  @Test public void testLastInvoiceNumberWithDifferentBusiness() {
    ...
    ESInvoiceEntity resultInvoice1 = this.getInstance(DAOESInvoice.class).getLatestInvoiceFromSeries(inv1.getSeries(), inv1.getBusiness().getUID().toString());
    ESInvoiceEntity resultInvoice2 = this.getInstance(DAOESInvoice.class).getLatestInvoiceFromSeries(inv2.getSeries(), inv2.getBusiness().getUID().toString());
  ...
  @Test(expected = BillyRuntimeException.class) public void testWithNoInvoice() {
    this.getInstance(DAOESInvoice.class).getLatestInvoiceFromSeries("NON EXISTING SERIES", (new UID().toString()));
  }

  @Test public void testInvoiceFromBusiness() {
    ESInvoiceEntity inv1 = this.getNewIssuedInvoice("B1");
    ESGenericInvoiceEntity res = this.getInstance(DAOESInvoice.class).findByNumber(inv1.getBusiness().getUID(), inv1.getNumber());
    Assert.assertEquals(inv1.getUID(), res.getUID());
    Assert.assertNull(this.getInstance(DAOESInvoice.class).findByNumber(UID.fromString("INEXISTENT_BUSINESS"), inv1.getNumber()));
  }

  @Test public void testFindCreditNote() {
    ESInvoiceEntity inv1 = this.getNewIssuedInvoice("B1");
    ESCreditNote cc1 = this.getNewIssuedCreditnote(inv1);
    List<ESCreditNote> cn1 = this.getInstance(DAOESCreditNote.class).findByReferencedDocument(cc1.getBusiness().getUID(), inv1.getUID());
    List<ESCreditNote> cn0 = this.getInstance(DAOESCreditNote.class).findByReferencedDocument(UID.fromString("B1"), UID.fromString("INEXISTENT_CN"));
