...
public class ESCreditReceiptEntryTestUtil {
  ...
  public ESCreditReceiptEntry.Builder getCreditReceiptEntryBuilder(ESReceiptEntity reference) {
    ESCreditReceiptEntry.Builder creditReceiptEntryBuilder = this.injector.getInstance(ESCreditReceiptEntry.Builder.class);
    ESProductEntity newProduct = (ESProductEntity) this.injector.getInstance(DAOESProduct.class).create(new ESProductTestUtil(this.injector).getProductEntity());
    this.context = this.contexts.spain().allRegions();
    creditReceiptEntryBuilder.setUnitAmount(AmountType.WITH_TAX, ESCreditReceiptEntryTestUtil.AMOUNT).setTaxPointDate(new Date()).setDescription(newProduct.getDescription()).setQuantity(ESCreditReceiptEntryTestUtil.QUANTITY).setUnitOfMeasure(newProduct.getUnitOfMeasure()).setProductUID(newProduct.getUID()).setContextUID(this.context.getUID()).setReason(ESCreditReceiptEntryTestUtil.REASON).setReferenceUID(reference.getUID()).setCurrency(Currency.getInstance("EUR"));
