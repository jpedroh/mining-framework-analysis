...
public class ESSimpleInvoiceTestUtil {
  ...
  public ESSimpleInvoiceEntity getSimpleInvoiceEntity(CLIENTTYPE clientType) {
    ESSimpleInvoiceEntity invoice = (ESSimpleInvoiceEntity) this.getSimpleInvoiceBuilder(this.business.getBusinessEntity(), clientType).build();
    return invoice;
  }
  ...
  public ESSimpleInvoice.Builder getSimpleInvoiceBuilder(ESBusinessEntity businessEntity, CLIENTTYPE clientType) {
    ...
    return invoiceBuilder.setBilled(ESInvoiceTestUtil.BILLED).setCancelled(ESInvoiceTestUtil.CANCELLED).setSelfBilled(ESInvoiceTestUtil.SELFBILL).setDate(new Date()).setSourceId(ESInvoiceTestUtil.SOURCE_ID).setCustomerUID(customerUID).setBusinessUID(businessEntity.getUID()).addPayment(this.payment.getPaymentBuilder()).setClientType(clientType);
  }
}