...
public class TestESTaxBuilder extends ESAbstractTest {
  ...
  @Test public void doTestFlat() {
    ...
    BigDecimal amount = (mockTax.getTaxRateType() == TaxRateType.FLAT) ? mockTax.getFlatRateAmount() : mockTax.getPercentageRateValue();
    builder.setCode(mockTax.getCode()).setContextUID(mockTax.getContext().getUID()).setCurrency(mockTax.getCurrency()).setDescription(mockTax.getDescription()).setDesignation(mockTax.getDesignation()).setValidFrom(mockTax.getValidFrom()).setValidTo(mockTax.getValidTo()).setTaxRate(mockTax.getTaxRateType(), amount).setValue(mockTax.getValue());
    ...
    if (mockTax.getTaxRateType() == ESTax.TaxRateType.FLAT) {
      ...
      Assert.assertThat(mockTax.getPercentageRateValue(), CoreMatchers.is(CoreMatchers.not(tax.getPercentageRateValue())));
    } else {
  ...
  public MockESTaxEntity loadFixture(Class<MockESTaxEntity> clazz, String path) {
    ...
    MockESRegionContextEntity mockContext = this.createMockEntity(MockESRegionContextEntity.class, TestESTaxBuilder.REGIONCONTEXT_YML);
    mockContext.uid = new UID("uid_region_context");
    Mockito.when(this.getInstance(DAOESRegionContext.class).get(Matchers.any(UID.class))).thenReturn(mockContext);
