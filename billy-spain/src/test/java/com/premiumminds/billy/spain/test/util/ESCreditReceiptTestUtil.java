...
public class ESCreditReceiptTestUtil {
  ...
  public ESCreditReceipt.Builder getCreditReceiptBuilder(ESReceiptEntity reference) {
    ...
    ESCreditReceiptEntry.Builder creditReceiptEntryBuilder = this.creditReceiptEntry.getCreditReceiptEntryBuilder(reference);
    return creditReceiptBuilder.setBilled(ESCreditReceiptTestUtil.BILLED).setCancelled(ESCreditReceiptTestUtil.CANCELLED).setSelfBilled(ESCreditReceiptTestUtil.SELFBILL).setDate(new Date()).setSourceId(ESCreditReceiptTestUtil.SOURCEID).addEntry(creditReceiptEntryBuilder).setBusinessUID(reference.getBusiness().getUID()).addPayment(this.payment.getPaymentBuilder());
  }
}