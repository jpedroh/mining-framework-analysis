...
public class TestESBusinessBuilder extends ESAbstractTest {
  ...
  @Test public void doTest() {
    MockESBusinessEntity mockBusiness = this.createMockEntity(MockESBusinessEntity.class, TestESBusinessBuilder.ESBUSINESS_YML);
    Mockito.when(this.getInstance(DAOESBusiness.class).getEntityInstance()).thenReturn(new MockESBusinessEntity());
    Mockito.when(this.getInstance(DAOESRegionContext.class).get(Matchers.any(UID.class))).thenReturn((ESRegionContextEntity) mockBusiness.getOperationalContext());
    ...
    Mockito.when(mockApplicationBuilder.build()).thenReturn((ESApplicationEntity) mockBusiness.getApplications().get(0));
    ...
    Mockito.when(mockShippingAddressBuilder.build()).thenReturn((ESAddressEntity) mockBusiness.getShippingAddress());
