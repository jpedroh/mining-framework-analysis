...
public class TestESReceiptEntryBuilder extends ESAbstractTest {
	...
	@Test
	public void doTest() {
		MockESReceiptEntryEntity mockEntry = this.createMockEntity(MockESReceiptEntryEntity.class, TestESReceiptEntryBuilder.ES_RECEIPT_ENTRY_YML);
		mockEntry.setCurrency(Currency.getInstance("EUR"));
		Mockito.when(this.getInstance(DAOESReceiptEntry.class).getEntityInstance()).thenReturn(new MockESReceiptEntryEntity());
		MockESReceiptEntity mockReceipt = this.createMockEntity(MockESReceiptEntity.class, TestESReceiptEntryBuilder.ES_RECEIPT_YML);
		Mockito.when(this.getInstance(DAOESReceipt.class).get(Matchers.any(UID.class))).thenReturn(mockReceipt);
		Mockito.when(this.getInstance(DAOESProduct.class).get(Matchers.any(UID.class))).thenReturn(((ESProductEntity) (mockEntry.getProduct())));
		Mockito.when(this.getInstance(DAOESRegionContext.class).isSubContext(Matchers.any(Context.class), Matchers.any(Context.class))).thenReturn(true);
		...
		builder.setDescription(mockEntry.getDescription()).addDocumentReferenceUID(mockEntry.getDocumentReferences().get(0).getUID()).setQuantity(mockEntry.getQuantity()).setUnitAmount(AmountType.WITH_TAX, mockEntry.getUnitAmountWithTax()).setUnitOfMeasure(mockEntry.getUnitOfMeasure()).setProductUID(mockEntry.getProduct().getUID()).setTaxPointDate(mockEntry.getTaxPointDate()).setCurrency(Currency.getInstance("EUR"));
