...
public class ESReceiptEntryTestUtil {
  ...
  public ESReceiptEntry.Builder getReceiptEntryBuilder(ESProductEntity product) {
    ...
    receiptEntryBuilder.setUnitAmount(AmountType.WITH_TAX, ESReceiptEntryTestUtil.AMOUNT).setTaxPointDate(new Date()).setDescription(product.getDescription()).setQuantity(ESReceiptEntryTestUtil.QUANTITY).setUnitOfMeasure(product.getUnitOfMeasure()).setProductUID(product.getUID()).setContextUID(this.context.getUID()).setCurrency(ESReceiptEntryTestUtil.CURRENCY);
  ...
  public ESReceiptEntry.Builder getReceiptEntryBuilder() {
    ...
    return this.getReceiptEntryBuilder((ESProductEntity) this.injector.getInstance(DAOESProduct.class).create(newProduct));
  }
}