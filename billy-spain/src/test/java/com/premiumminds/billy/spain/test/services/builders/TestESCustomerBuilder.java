...
public class TestESCustomerBuilder extends ESAbstractTest {
  ...
  @Test public void doTest() {
    MockESCustomerEntity mockCustomer = this.createMockEntity(MockESCustomerEntity.class, TestESCustomerBuilder.ESCUSTOMER_YML);
    Mockito.when(this.getInstance(DAOESCustomer.class).getEntityInstance()).thenReturn(new MockESCustomerEntity());
    Mockito.when(this.getInstance(DAOESContact.class).get(Matchers.any(UID.class))).thenReturn((ESContactEntity) mockCustomer.getMainContact());
    ...
    Mockito.when(mockShippingAddressBuilder.build()).thenReturn((ESAddressEntity) mockCustomer.getShippingAddress());
