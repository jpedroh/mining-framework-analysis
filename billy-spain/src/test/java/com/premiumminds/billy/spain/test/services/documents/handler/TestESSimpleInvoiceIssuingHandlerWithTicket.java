...
public class TestESSimpleInvoiceIssuingHandlerWithTicket extends ESDocumentAbstractTest {
  ...
  @Before public void setUpNewSimpleInvoice() {
    try {
      ...
      ESSimpleInvoice.Builder simpleInvoiceBuilder = new ESSimpleInvoiceTestUtil(ESAbstractTest.injector).getSimpleInvoiceBuilder(business, CLIENTTYPE.CUSTOMER);
  ...
  @Test public void testIssuedInvoiceSimpleWithTicket() throws DocumentIssuingException {
    ...
    ESSimpleInvoicePersistenceService service = ESAbstractTest.injector.getInstance(ESSimpleInvoicePersistenceService.class);
  ...
  @Test public void testIssueWithUsedTicket() {
    ...
    this.parameters.setInvoiceSeries(this.DEFAULT_SERIES);
    ESBusinessEntity business = new ESBusinessTestUtil(ESAbstractTest.injector).getBusinessEntity("business");
    ESSimpleInvoice.Builder builder = new ESSimpleInvoiceTestUtil(ESAbstractTest.injector).getSimpleInvoiceBuilder(business, CLIENTTYPE.CUSTOMER);
    try {
      entity = (ESSimpleInvoiceEntity) services.issueDocument(builder, this.parameters, this.issuedInvoiceUID.getValue());
    } catch (InvalidTicketException e) {
    } catch (DocumentIssuingException e) {
      e.printStackTrace();
    }
  ...
  @Test public void testOpenCloseConnections() {
    Services services = new Services(ESAbstractTest.injector);
    ESSimpleInvoicePersistenceService persistenceService = ESAbstractTest.injector.getInstance(ESSimpleInvoicePersistenceService.class);
    ESBusinessEntity business = new ESBusinessTestUtil(ESAbstractTest.injector).getBusinessEntity("business");
    ESSimpleInvoice.Builder testinvoice = new ESSimpleInvoiceTestUtil(ESAbstractTest.injector).getSimpleInvoiceBuilder(business, CLIENTTYPE.CUSTOMER);
