...
public class TestESInvoiceEntryBuilder extends ESAbstractTest {
  ...
  @Test public void doTest() {
    MockESInvoiceEntryEntity mock = this.createMockEntity(MockESInvoiceEntryEntity.class, TestESInvoiceEntryBuilder.ES_INVOICE_ENTRY_YML);
    mock.currency = Currency.getInstance("EUR");
    Mockito.when(this.getInstance(DAOESInvoiceEntry.class).getEntityInstance()).thenReturn(new MockESInvoiceEntryEntity());
    MockESInvoiceEntity mockInvoice = this.createMockEntity(MockESInvoiceEntity.class, TestESInvoiceEntryBuilder.ES_INVOICE_YML);
