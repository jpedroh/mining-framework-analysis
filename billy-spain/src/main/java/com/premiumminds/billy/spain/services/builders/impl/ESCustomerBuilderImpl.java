...
public class ESCustomerBuilderImpl<TBuilder extends ESCustomerBuilderImpl<TBuilder, TCustomer>, TCustomer extends ESCustomer> extends CustomerBuilderImpl<TBuilder, TCustomer> implements ESCustomerBuilder<TBuilder, TCustomer> {
  protected static final Localizer LOCALIZER = new Localizer("com/premiumminds/billy/core/i18n/FieldNames");

  @Inject protected ESCustomerBuilderImpl(DAOESCustomer daoESCustomer, DAOESContact daoESContact) {
    super(daoESCustomer, daoESContact);
  }

  @Override @NotOnUpdate public TBuilder setTaxRegistrationNumber(String number, String countryCode) throws InvalidTaxIdentificationNumberException {
  ...
  @Override protected ESCustomerEntity getTypeInstance() {
  ...
  @Override public <T extends Address> TBuilder setBillingAddress(Builder<T> addressBuilder) {
    BillyValidator.notNull(addressBuilder, ESCustomerBuilderImpl.LOCALIZER.getString("field.customer_billing_address"));
  ...
  @Override public TBuilder setHasSelfBillingAgreement(boolean selfBiling) {
    BillyValidator.notNull(selfBiling, ESCustomerBuilderImpl.LOCALIZER.getString("field.customer_self_billing_agreement"));
    this.getTypeInstance().setHasSelfBillingAgreement(selfBiling);
    return this.getBuilder();
  }

  @Override public TBuilder setReferralName(String referralName) {
  ...
  @Override protected void validateInstance() throws BillyValidationException {
    ...
    BillyValidator.mandatory(c.getTaxRegistrationNumber(), ESCustomerBuilderImpl.LOCALIZER.getString("field.customer_tax_number"));
    BillyValidator.mandatory(c.getMainAddress(), ESCustomerBuilderImpl.LOCALIZER.getString("field.customer_main_address"));
    BillyValidator.mandatory(c.getBillingAddress(), ESCustomerBuilderImpl.LOCALIZER.getString("field.customer_billing_address"));
    BillyValidator.mandatory(c.hasSelfBillingAgreement(), ESCustomerBuilderImpl.LOCALIZER.getString("field.customer_self_billing_agreement"));
