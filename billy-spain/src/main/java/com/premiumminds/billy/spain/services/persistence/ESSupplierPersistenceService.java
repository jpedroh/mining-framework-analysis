...
public class ESSupplierPersistenceService implements PersistenceService<ESSupplier> {
  ...
  @Inject public ESSupplierPersistenceService(DAOESSupplier daoSupplier) {
    this.daoSupplier = daoSupplier;
  }

  @Override public ESSupplier create(final Builder<ESSupplier> builder) {
    try {
      return new TransactionWrapper<ESSupplier>(this.daoSupplier) {
        @Override public ESSupplier runTransaction() throws Exception {
  ...
  @Override public ESSupplier update(final Builder<ESSupplier> builder) {
    try {
      return new TransactionWrapper<ESSupplier>(this.daoSupplier) {
        @Override public ESSupplier runTransaction() throws Exception {
          ESSupplierEntity entity = (ESSupplierEntity) builder.build();
  ...
  @Override public ESSupplier get(final UID uid) {
    try {
      return new TransactionWrapper<ESSupplier>(this.daoSupplier) {
        @Override public ESSupplier runTransaction() throws Exception {
