...
public class ESApplicationPersistenceService implements PersistenceService<ESApplication> {
  ...
  @Inject public ESApplicationPersistenceService(DAOESApplication daoApplication) {
    this.daoApplication = daoApplication;
  }

  @Override public ESApplication create(final Builder<ESApplication> builder) {
    try {
      return new TransactionWrapper<ESApplication>(this.daoApplication) {
        @Override public ESApplication runTransaction() throws Exception {
  ...
  @Override public ESApplication update(final Builder<ESApplication> builder) {
    try {
      return new TransactionWrapper<ESApplication>(this.daoApplication) {
        @Override public ESApplication runTransaction() throws Exception {
          ESApplicationEntity entity = (ESApplicationEntity) builder.build();
  ...
  @Override public ESApplication get(final UID uid) {
    try {
      return new TransactionWrapper<ESApplication>(this.daoApplication) {
        @Override public ESApplication runTransaction() throws Exception {
