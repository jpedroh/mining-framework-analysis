...
public class ESManualCreditNoteEntryBuilderImpl<TBuilder extends ESManualCreditNoteEntryBuilderImpl<TBuilder, TEntry>, TEntry extends ESCreditNoteEntry> extends ESManualEntryBuilderImpl<TBuilder, TEntry, DAOESCreditNoteEntry, DAOESInvoice> implements ESManualCreditNoteEntryBuilder<TBuilder, TEntry> {
  public ESManualCreditNoteEntryBuilderImpl(DAOESCreditNoteEntry daoESCreditNoteEntry, DAOESInvoice daoESInvoice, DAOESTax daoESTax, DAOESProduct daoESProduct, DAOESRegionContext daoESRegionContext) {
    super(daoESCreditNoteEntry, daoESInvoice, daoESTax, daoESProduct, daoESRegionContext);
  }

  @Override @NotOnUpdate public TBuilder setReferenceUID(UID referenceUID) {
    BillyValidator.notNull(referenceUID, ESCreditNoteEntryBuilderImpl.LOCALIZER.getString("field.invoice_reference"));
  ...
  @Override @NotOnUpdate public TBuilder setReason(String reason) {
  ...
  @Override protected ESCreditNoteEntryEntity getTypeInstance() {
  ...
  @Override protected void validateInstance() throws BillyValidationException {
    ...
    BillyValidator.mandatory(cn.getQuantity(), ESGenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.quantity"));
    BillyValidator.mandatory(cn.getUnitOfMeasure(), ESGenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.unit"));
    BillyValidator.mandatory(cn.getProduct(), ESGenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.product"));
    ...
    BillyValidator.mandatory(cn.getTaxPointDate(), ESGenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.tax_point_date"));
    BillyValidator.mandatory(cn.getReference(), ESCreditNoteEntryBuilderImpl.LOCALIZER.getString("field.invoice_reference"));
