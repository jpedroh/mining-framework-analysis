 ...
 *
 */
@Singleton public class BillySpain {
  ...
  public BillySpain(String persistenceUnitId) {
    this.injector = Guice.createInjector(new SpainDependencyModule(), new SpainPersistenceDependencyModule(persistenceUnitId));
    this.injector.getInstance(PersistService.class).start();
  }

  @Inject public BillySpain(Injector injector) {
