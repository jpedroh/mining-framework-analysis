...
public class ESCreditReceiptPersistenceService implements PersistenceService<ESCreditReceipt> {
  ...
  @Inject public ESCreditReceiptPersistenceService(DAOESCreditReceipt daoCreditReceipt, DAOTicket daoTicket) {
    this.daoCreditReceipt = daoCreditReceipt;
    this.daoTicket = daoTicket;
  }

  @Override @NotImplemented public ESCreditReceipt create(final Builder<ESCreditReceipt> builder) {
    return null;
  }

  @Override public ESCreditReceipt update(final Builder<ESCreditReceipt> builder) {
    try {
      return new TransactionWrapper<ESCreditReceipt>(this.daoCreditReceipt) {
        @Override public ESCreditReceipt runTransaction() throws Exception {
  ...
  @Override public ESCreditReceipt get(final UID uid) {
    try {
      return new TransactionWrapper<ESCreditReceipt>(this.daoCreditReceipt) {
        @Override public ESCreditReceipt runTransaction() throws Exception {
  ...
  public ESCreditReceipt getWithTicket(final UID ticketUID) throws NoResultException, BillyRuntimeException {
    try {
      return new TransactionWrapper<ESCreditReceipt>(this.daoCreditReceipt) {
        @Override public ESCreditReceipt runTransaction() throws Exception {
          UID objectUID = ESCreditReceiptPersistenceService.this.daoTicket.getObjectEntityUID(ticketUID.getValue());
  ...
  public List<ESCreditReceipt> findByReferencedDocument(final UID uidCompany, final UID uidInvoice) {
    try {
      return new TransactionWrapper<List<ESCreditReceipt>>(this.daoCreditReceipt) {
        @Override public List<ESCreditReceipt> runTransaction() throws Exception {
          return ESCreditReceiptPersistenceService.this.daoCreditReceipt.findByReferencedDocument(uidCompany, uidInvoice);
        }
      }.execute();
    } catch (NoResultException e) {
      throw e;
