...
public class ApexCRUDViolationRule extends AbstractApexRule {
  private static final Pattern SELECT_FROM_PATTERN = Pattern.compile("[\\S|\\s]+?FROM[\\s]+?(\\w+)", Pattern.CASE_INSENSITIVE);
  ...
  private boolean hasAccessLevelArgument(ASTMethodCallExpression node) {
    for (int i = 0; i < node.getNumChildren(); i++) {
      ...
      if (argument instanceof ASTVariableExpression && argument.getFirstChildOfType(ASTReferenceExpression.class) != null) {
  ...
  private boolean isLastMethodName(final ASTMethodCallExpression methodNode, final String className, final String methodName) {
    ...
    if (reference != null && !reference.getNames().isEmpty()) {
      if (reference.getNames().get(reference.getNames().size() - 1).equalsIgnoreCase(className) && Helper.isMethodName(methodNode, methodName)) {
        return true;
      }
    }
    return false;
  ...
  private boolean isWithSecurityEnforced(final ApexNode<?> node) {
    return node instanceof ASTSoqlExpression && WITH_SECURITY_ENFORCED.matcher(((ASTSoqlExpression) node).getQuery()).matches();
  }

  private boolean isWithUserMode(final ApexNode<?> node) {
    return node instanceof ASTSoqlExpression && WITH_USER_MODE.matcher(((ASTSoqlExpression) node).getQuery()).matches();
  }

  private boolean isWithSystemMode(final ApexNode<?> node) {
    return node instanceof ASTSoqlExpression && WITH_SYSTEM_MODE.matcher(((ASTSoqlExpression) node).getQuery()).matches();
  }

  private String getType(final ASTMethodCallExpression methodNode) {
    final ASTReferenceExpression reference = methodNode.getFirstChildOfType(ASTReferenceExpression.class);
    if (!reference.getNames().isEmpty()) {
      return new StringBuilder().append(reference.getDefiningType()).append(":").append(reference.getNames().get(0)).toString();
    }
    return "";
  }

  private void extractObjectAndFields(final List<String> listIdentifiers, final String method, final String definingType) {
  ...
  private void recursivelyEvaluateCRUDMethodCalls(final ApexNode<?> self, final Set<ASTMethodCallExpression> innerMethodCalls, final ASTBlockStatement blockStatement) {
  ...
  private void mapCallToMethodDecl(final ApexNode<?> self, final Set<ASTMethodCallExpression> innerMethodCalls, final List<ASTMethodCallExpression> nodes) {
  ...
  private boolean validateCRUDCheckPresent(final ApexNode<?> node, final Object data, final String crudMethod, final String typeCheck) {
    boolean missingKey = !typeToDMLOperationMapping.containsKey(typeCheck);
    boolean isImproperDMLCheck = !isProperESAPICheckForDML(typeCheck, crudMethod) && !isProperAuthPatternBasedCheckForDML(typeCheck, crudMethod);
    ...
    if (forEachStatement != null) {
      if (typesFromSOQL.isEmpty()) {
        ...
        if (variableDeclFor != null) {
          ...
          type = getSimpleType(type);
          StringBuilder typeCheck = new StringBuilder().append(variableDeclFor.getDefiningType()).append(":").append(type);
  ...
  private String getReturnType(final ASTMethod method) {
    return new StringBuilder().append(method.getDefiningType()).append(":").append(method.getReturnType()).toString();
  }
  ...
  private static PropertyDescriptor<String> authMethodPatternProperty(String operation) {
    ...
    return stringProperty(propertyName).desc("A regular expression for one or more custom " + operation + " authorization method name patterns.").defaultValue("").build();
  }
  ...
  private static PropertyDescriptor<Integer> authMethodTypeParamIndexProperty(String operation) {
    ...
    return intProperty(propertyName).desc("The 0-based index of the " + S_OBJECT_TYPE + " parameter for the custom " + operation + " authorization method. Defaults to 0.").defaultValue(0).build();
  }
        ...
        if (sobjectTypeParameterCandidate != null && S_OBJECT_TYPE.equalsIgnoreCase(sobjectTypeParameterCandidate.getImage())) {
          ...
          if (objectTypeCandidate != null) {
            ...
            if (StringUtils.isNotBlank(objectType)) {
              final StringBuilder checkedTypeBuilder = new StringBuilder().append(methodNode.getDefiningType()).append(":").append(objectType);
