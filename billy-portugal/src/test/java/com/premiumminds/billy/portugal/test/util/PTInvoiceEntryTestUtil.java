...
public class PTInvoiceEntryTestUtil {
  ...
  public PTInvoiceEntry.Builder getInvoiceEntryBuilder(PTProductEntity product) {
    ...
    invoiceEntryBuilder.setUnitAmount(AmountType.WITH_TAX, PTInvoiceEntryTestUtil.AMOUNT).setTaxPointDate(new Date()).setDescription(product.getDescription()).setQuantity(PTInvoiceEntryTestUtil.QUANTITY).setUnitOfMeasure(product.getUnitOfMeasure()).setProductUID(product.getUID()).setContextUID(this.context.getUID()).setShippingOrigin(originBuilder).setCurrency(PTInvoiceEntryTestUtil.CURRENCY);
    return invoiceEntryBuilder;
  }

  private ManualBuilder getManualInvoiceEntryBuilder(PTProductEntity product) {
    PTInvoiceEntry.ManualBuilder invoiceEntryBuilder = this.injector.getInstance(PTInvoiceEntry.ManualBuilder.class);
    PTShippingPoint.Builder originBuilder = this.shippingPoint.getShippingPointBuilder();
    this.context = this.contexts.portugal().allRegions();
    invoiceEntryBuilder.clear();
    invoiceEntryBuilder.setUnitAmount(AmountType.WITH_TAX, PTInvoiceEntryTestUtil.AMOUNT).setTaxPointDate(new Date()).setDescription(product.getDescription()).setQuantity(PTInvoiceEntryTestUtil.QUANTITY).setUnitOfMeasure(product.getUnitOfMeasure()).setProductUID(product.getUID()).setContextUID(this.context.getUID()).setShippingOrigin(originBuilder).setCurrency(PTInvoiceEntryTestUtil.CURRENCY);
  ...
  public PTInvoiceEntry.Builder getInvoiceEntryBuilder() {
    ...
    return this.getInvoiceEntryBuilder((PTProductEntity) this.injector.getInstance(DAOPTProduct.class).create(newProduct));
  }

  public ManualBuilder getManualInvoiceEntryBuilder() {
    PTProductEntity newProduct = this.product.getProductEntity();
    return this.getManualInvoiceEntryBuilder((PTProductEntity) this.injector.getInstance(DAOPTProduct.class).create(newProduct));
  }
  ...
  public PTInvoiceEntry.Builder getInvoiceOtherRegionsEntryBuilder(String region) {
    ...
    if (region.equals("PT-20")) {
      product = (PTProductEntity) this.injector.getInstance(DAOPTProduct.class).create(this.product.getOtherRegionProductEntity(region));
    ...
    PTShippingPoint.Builder originBuilder = this.shippingPoint.getShippingPointBuilder();
    invoiceEntryBuilder.clear();
    invoiceEntryBuilder.setUnitAmount(AmountType.WITHOUT_TAX, PTInvoiceEntryTestUtil.AMOUNT).setTaxPointDate(new Date()).setDescription(product.getDescription()).setQuantity(PTInvoiceEntryTestUtil.QUANTITY).setUnitOfMeasure(product.getUnitOfMeasure()).setProductUID(product.getUID()).setContextUID(this.context.getUID()).setShippingOrigin(originBuilder).setCurrency(Currency.getInstance("EUR"));
    return invoiceEntryBuilder;
  }
}