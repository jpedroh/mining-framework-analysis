...
public class PTProductTestUtil {
  ...
  public PTProduct.Builder getProductBuilder(String productCode, String unitMesure, String numberCode, String group, String description, ProductType type) {
    PTProduct.Builder productBuilder = this.injector.getInstance(PTProduct.Builder.class);
    return productBuilder.addTaxUID(this.tax.getUID()).setNumberCode(numberCode).setUnitOfMeasure(unitMesure).setProductCode(productCode).setDescription(description).setType(type).setProductGroup(group);
  }

  public PTProduct.Builder getProductBuilder() {
    return this.getProductBuilder(PTProductTestUtil.PRODUCT_CODE, PTProductTestUtil.UNIT_OF_MEASURE, PTProductTestUtil.NUMBER_CODE, PTProductTestUtil.GROUP, PTProductTestUtil.DESCRIPTION, PTProductTestUtil.TYPE);
  }

  public PTProductEntity getProductEntity(String productCode, String unitMesure, String numberCode, String group, ProductType type) {
    return (PTProductEntity) this.getProductBuilder(productCode, unitMesure, numberCode, group, PTProductTestUtil.DESCRIPTION, type).build();
  }
  ...
  public PTProductEntity getOtherRegionProductEntity(String region) {
    ...
    productBuilder.addTaxUID(taxRegion.getUID()).setNumberCode(PTProductTestUtil.NUMBER_CODE).setUnitOfMeasure(PTProductTestUtil.UNIT_OF_MEASURE).setProductCode(PTProductTestUtil.PRODUCT_CODE).setDescription(PTProductTestUtil.DESCRIPTION).setType(PTProductTestUtil.TYPE).setProductGroup(PTProductTestUtil.GROUP);
