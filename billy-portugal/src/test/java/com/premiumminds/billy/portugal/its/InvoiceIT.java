...
public class InvoiceIT {
  ...
  @Test public void test() throws Exception {
    injector = Guice.createInjector(new PortugalDependencyModule(), new PortugalPersistenceDependencyModule("BillyPortugalTestPersistenceUnit"));
    ...
    PTCreditNote creditNote = createPtCreditNote(dateFormat, billyPortugal, business, creditNoteParameters, customer, product, invoice);
    Date startDate = dateFormat.parse("01-01-2013");
    Date endDate = dateFormat.parse("01-01-2014");
    try (InputStream inputStream = billyPortugal.saft().export(application.getUID(), business.getUID(), startDate, endDate, PTSAFTFileGenerator.SAFTVersion.CURRENT, true)) {
  ...
  private PTTax createFlatTax(BillyPortugal billyPortugal) {
    ...
    taxBuilder.setTaxRate(Tax.TaxRateType.FLAT, new BigDecimal("3.14")).setContextUID(UID.fromString("cff9a2f4-cd1d-4786-95c4-78b167a00d2b")).setCode("code1").setDescription("description 1").setValidFrom(new Date(0)).setCurrency(Currency.getInstance("EUR")).setValue(new BigDecimal("5.55"));
  ...
  private PTApplication.Builder getPTApplicationBuilder(BillyPortugal billyPortugal) throws MalformedURLException {
    ...
    applicationBuilder.setDeveloperCompanyName("Developer Company Name").setDeveloperCompanyTaxIdentifier("1000000001").setSoftwareCertificationNumber(SOFTWARE_CERTIFICATION).setName("Billy").setVersion("1.0").setApplicationKeysPath(new URL("http://www.keys.path")).addContact(contactBuilder).setMainContact(contactBuilder);
  ...
  private PTBusiness createPtBusiness(BillyPortugal billyPortugal, PTApplication.Builder applicationBuilder) {
    ...
    addressBuilder.setStreetName("Street name 2").setNumber("2").setPostalCode("1000-100").setCity("Lisbon").setISOCountry("PT").setDetails("Av. 5 de Outubro N\u00ba 2 1000-100 Lisboa");
    PTBusiness.Builder businessBuilder = billyPortugal.businesses().builder();
    businessBuilder.addApplication(applicationBuilder).addContact(contactBuilder, true).setMainContactUID(contactBuilder.build().getUID()).setName("Business 1").setCommercialName("Business, INC").setFinancialID("500003564", "PT").setAddress(addressBuilder).setBillingAddress(addressBuilder);
    return billyPortugal.businesses().persistence().create(businessBuilder);
  }

  private PTInvoice createPtInvoice(SimpleDateFormat dateFormat, BillyPortugal billyPortugal, PTBusiness business, PTIssuingParams invoiceParameters, PTCustomer customer, PTProduct product, PTProduct productExempt, PTProduct productTax) throws ParseException, DocumentIssuingException {
    PTInvoice.Builder invoiceBuilder = billyPortugal.invoices().builder();
    Date invoiceDate = dateFormat.parse("01-03-2013");
    invoiceBuilder.setSelfBilled(false).setCancelled(false).setBilled(false).setDate(invoiceDate).setSourceId("User 1").setSourceBilling(PTGenericInvoice.SourceBilling.P).setBusinessUID(business.getUID()).setCustomerUID(customer.getUID());
    PTInvoiceEntry.Builder entryBuilder = billyPortugal.invoices().entryBuilder();
    entryBuilder.setCurrency(Currency.getInstance("EUR")).setQuantity(new BigDecimal("10")).setTaxPointDate(dateFormat.parse("01-02-2013")).setUnitAmount(GenericInvoiceEntryBuilder.AmountType.WITH_TAX, new BigDecimal("100")).setContextUID(billyPortugal.contexts().continent().allContinentRegions().getUID()).setProductUID(product.getUID()).setDescription(product.getDescription()).setUnitOfMeasure(product.getUnitOfMeasure());
    invoiceBuilder.addEntry(entryBuilder);
    entryBuilder = billyPortugal.invoices().entryBuilder();
    entryBuilder.setCurrency(Currency.getInstance("EUR")).setQuantity(new BigDecimal("10.0")).setTaxPointDate(dateFormat.parse("01-02-2013")).setUnitAmount(GenericInvoiceEntryBuilder.AmountType.WITH_TAX, new BigDecimal("100")).setContextUID(billyPortugal.contexts().continent().allContinentRegions().getUID()).setProductUID(productExempt.getUID()).setDescription(productExempt.getDescription()).setUnitOfMeasure(productExempt.getUnitOfMeasure()).setTaxExemptionCode("M99").setTaxExemptionReason("reason 1");
    invoiceBuilder.addEntry(entryBuilder);
    entryBuilder = billyPortugal.invoices().entryBuilder();
    entryBuilder.setCurrency(Currency.getInstance("EUR")).setQuantity(new BigDecimal("10.0")).setTaxPointDate(dateFormat.parse("01-02-2013")).setUnitAmount(GenericInvoiceEntryBuilder.AmountType.WITH_TAX, new BigDecimal("100")).setContextUID(billyPortugal.contexts().continent().allContinentRegions().getUID()).setProductUID(productTax.getUID()).setDescription(productTax.getDescription()).setUnitOfMeasure(productTax.getUnitOfMeasure()).setTaxExemptionCode("M99").setTaxExemptionReason("reason 1");
    invoiceBuilder.addEntry(entryBuilder);
    return billyPortugal.invoices().issue(invoiceBuilder, invoiceParameters);
  }

  private PTCreditNote createPtCreditNote(SimpleDateFormat dateFormat, BillyPortugal billyPortugal, PTBusiness business, PTIssuingParams invoiceParameters, PTCustomer customer, PTProduct product, PTInvoice invoice) throws ParseException, DocumentIssuingException {
    PTCreditNote.Builder creditNoteBuilder = billyPortugal.creditNotes().builder();
    Date creditNoteDate = dateFormat.parse("01-03-2013");
    creditNoteBuilder.setSelfBilled(false).setCancelled(false).setBilled(false).setDate(creditNoteDate).setSourceId("User 2").setSourceBilling(PTGenericInvoice.SourceBilling.P).setBusinessUID(business.getUID()).setCustomerUID(customer.getUID());
    PTCreditNoteEntry.Builder entryBuilder = billyPortugal.creditNotes().entryBuilder();
    entryBuilder.setAmountType(GenericInvoiceEntryBuilder.AmountType.WITH_TAX).setCurrency(Currency.getInstance("EUR")).setContextUID(billyPortugal.contexts().portugal().allRegions().getUID()).setQuantity(new BigDecimal("10")).setTaxPointDate(dateFormat.parse("01-02-2013")).setUnitAmount(GenericInvoiceEntryBuilder.AmountType.WITH_TAX, new BigDecimal("100")).setContextUID(billyPortugal.contexts().continent().allContinentRegions().getUID()).setProductUID(product.getUID()).setDescription(product.getDescription()).setUnitOfMeasure(product.getUnitOfMeasure()).setReferenceUID(invoice.getUID()).setReason("some reason 1");
  ...
  private PTProduct createPtProduct(BillyPortugal billyPortugal) {
    ...
    productBuilder.setDescription("product 1").setNumberCode("1").setProductCode("1").setType(Product.ProductType.GOODS).setUnitOfMeasure("kg").setProductGroup("group 1").addTaxUID(billyPortugal.taxes().continent().normal().getUID());
  ...
  private PTProduct createPtProductExempt(BillyPortugal billyPortugal) {
    PTProduct.Builder productBuilder = billyPortugal.products().builder();
    productBuilder.setDescription("product 2").setNumberCode("2").setProductCode("2").setType(Product.ProductType.GOODS).setUnitOfMeasure("kg").setProductGroup("group 1").addTaxUID(billyPortugal.taxes().exempt().getUID());
    return billyPortugal.products().persistence().create(productBuilder);
  }

  private PTProduct createPtProductFlat(BillyPortugal billyPortugal, PTTax flatTax) {
    PTProduct.Builder productBuilder = billyPortugal.products().builder();
    productBuilder.setDescription("product 3").setNumberCode("3").setProductCode("3").setType(Product.ProductType.GOODS).setUnitOfMeasure("kg").setProductGroup("group 1").addTaxUID(flatTax.getUID());
    return billyPortugal.products().persistence().create(productBuilder);
  }
  ...
  private PTCustomer createPtCustomer(BillyPortugal billyPortugal) {
    ...
    PTAddress.Builder addressBuilder = billyPortugal.addresses().builder();
    addressBuilder.setStreetName("Customer 1 street name 1").setNumber("2").setPostalCode("1000-100").setCity("Lisbon").setISOCountry("PT").setDetails("Av. 5 de Outubro N\u00ba 2 1000-100 Lisboa");
    PTCustomer.Builder customerBuilder = billyPortugal.customers().builder();
    customerBuilder.setName("Customer name 1").setTaxRegistrationNumber("123456789", "PT").addAddress(addressBuilder, true).setBillingAddress(addressBuilder).setShippingAddress(addressBuilder).setHasSelfBillingAgreement(false).addContact(contactBuilder);
