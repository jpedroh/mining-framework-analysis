...
public class TestPTCustomerBuilder extends PTAbstractTest {
  ...
  @Test public void doTest() {
    MockPTCustomerEntity mockCustomer = this.createMockEntity(MockPTCustomerEntity.class, TestPTCustomerBuilder.PTCUSTOMER_YML);
    Mockito.when(this.getInstance(DAOPTCustomer.class).getEntityInstance()).thenReturn(new MockPTCustomerEntity());
    Mockito.when(this.getInstance(DAOPTContact.class).get(Matchers.any(UID.class))).thenReturn((PTContactEntity) mockCustomer.getMainContact());
    ...
    Mockito.when(mockShippingAddressBuilder.build()).thenReturn((PTAddressEntity) mockCustomer.getShippingAddress());
    ...
    builder.addAddress(mockMainAddressBuilder, true).addContact(mockMainContactBuilder).addContact(mockContactBuilder1).addContact(mockContactBuilder2).setBillingAddress(mockBillingAddressBuilder).setHasSelfBillingAgreement(mockCustomer.hasSelfBillingAgreement()).setName(mockCustomer.getName()).setShippingAddress(mockShippingAddressBuilder).setTaxRegistrationNumber(mockCustomer.getTaxRegistrationNumber(), PTAbstractTest.PT_COUNTRY_CODE).setMainContactUID(mockCustomer.getMainContact().getUID());
