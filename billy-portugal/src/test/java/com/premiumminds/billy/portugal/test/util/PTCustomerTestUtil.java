...
public class PTCustomerTestUtil {
  ...
  public PTCustomer.Builder getCustomerBuilder() {
    ...
    return this.getCustomerBuilder(PTCustomerTestUtil.NAME, PTCustomerTestUtil.TAX_NUMBER, PTCustomerTestUtil.SELF_BILLING_AGREE, addressBuilder, contactBuilder);
  }

  public PTCustomer.Builder getCustomerBuilder(String name, String taxNumber, Boolean selfBilling, PTAddress.Builder addressBuilder, PTContact.Builder contactBuilder) {
    PTCustomer.Builder customerBuilder = this.injector.getInstance(PTCustomer.Builder.class);
    return customerBuilder.addAddress(addressBuilder, true).addContact(contactBuilder).setBillingAddress(addressBuilder).setName(name).setHasSelfBillingAgreement(selfBilling).setTaxRegistrationNumber(taxNumber, PTCustomerTestUtil.PT_COUNTRY_CODE);
  }

  public PTCustomerEntity getCustomerEntity(String name, String taxNumber, boolean selfBilling, PTAddress.Builder addressBuilder, PTContact.Builder contactBuilder) {
    PTCustomerEntity customer = (PTCustomerEntity) this.getCustomerBuilder().build();
    return customer;
  }
}