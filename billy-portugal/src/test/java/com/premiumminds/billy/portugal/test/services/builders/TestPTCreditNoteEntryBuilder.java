...
public class TestPTCreditNoteEntryBuilder extends PTAbstractTest {
  ...
  @Test public void doTest() {
    MockPTCreditNoteEntryEntity mock = this.createMockEntity(MockPTCreditNoteEntryEntity.class, TestPTCreditNoteEntryBuilder.PT_CREDIT_NOTE_ENTRY_YML);
    ...
    MockPTInvoiceEntity mockInvoiceEntity = this.createMockEntity(MockPTInvoiceEntity.class, TestPTCreditNoteEntryBuilder.PT_INVOICE_YML);
    ...
    Mockito.when(this.getInstance(DAOPTRegionContext.class).isSubContext(Matchers.any(Context.class), Matchers.any(Context.class))).thenReturn(true);
