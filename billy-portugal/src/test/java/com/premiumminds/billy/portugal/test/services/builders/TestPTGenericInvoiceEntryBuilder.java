...
public class TestPTGenericInvoiceEntryBuilder extends PTAbstractTest {
  ...
  @Test public void doTest() {
    ...
    Mockito.when(this.getInstance(DAOPTProduct.class).get(Matchers.any(UID.class))).thenReturn((PTProductEntity) mock.getProduct());
    Mockito.when(this.getInstance(DAOPTRegionContext.class).isSubContext(Matchers.any(PTRegionContext.class), Matchers.any(PTRegionContext.class))).thenReturn(true);
