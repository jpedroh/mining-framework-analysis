...
public class TestPTGenericInvoiceEntryBuilder extends PTAbstractTest {
  private static final String PT_GENERIC_INVOICE_ENTRY_YML = AbstractTest.YML_CONFIGS_DIR + "PTGenericInvoiceEntry.yml";

  private static final String PT_GENERIC_INVOICE_ENTRY_WITHOUT_TAXES_YML = AbstractTest.YML_CONFIGS_DIR + "PTGenericInvoiceEntryWithoutTaxes.yml";
  ...
  @Test public void doTest() {
    MockPTGenericInvoiceEntryEntity mock = this.createMockEntity(MockPTGenericInvoiceEntryEntity.class, TestPTGenericInvoiceEntryBuilder.PT_GENERIC_INVOICE_ENTRY_YML);
    mock.currency = Currency.getInstance("EUR");
    Mockito.when(this.getInstance(DAOPTGenericInvoiceEntry.class).getEntityInstance()).thenReturn(new MockPTGenericInvoiceEntryEntity());
    MockPTGenericInvoiceEntity mockInvoice = this.createMockEntity(MockPTGenericInvoiceEntity.class, TestPTGenericInvoiceEntryBuilder.PT_GENERIC_INVOICE_YML);
    Mockito.when(this.getInstance(DAOPTGenericInvoice.class).get(Mockito.any(UID.class))).thenReturn(mockInvoice);
    Mockito.when(this.getInstance(DAOPTProduct.class).get(Mockito.any(UID.class))).thenReturn((PTProductEntity) mock.getProduct());
    Mockito.when(this.getInstance(DAOPTRegionContext.class).isSameOrSubContext(Mockito.any(), Mockito.any(PTRegionContext.class))).thenReturn(true);
  ...
  @Test public void doTestFailWithInvalidTaxExemptionCode() {
    MockPTGenericInvoiceEntryEntity mock = this.createMockEntity(MockPTGenericInvoiceEntryEntity.class, TestPTGenericInvoiceEntryBuilder.PT_GENERIC_INVOICE_ENTRY_YML);
    mock.currency = Currency.getInstance("EUR");
    Mockito.when(this.getInstance(DAOPTGenericInvoiceEntry.class).getEntityInstance()).thenReturn(new MockPTGenericInvoiceEntryEntity());
    MockPTGenericInvoiceEntity mockInvoice = this.createMockEntity(MockPTGenericInvoiceEntity.class, TestPTGenericInvoiceEntryBuilder.PT_GENERIC_INVOICE_YML);
    ...
    mock.getDocumentReferences().add(mockInvoice);
    PTGenericInvoiceEntry.Builder builder = this.getInstance(PTGenericInvoiceEntry.Builder.class);
    builder.setDescription(mock.getDescription()).addDocumentReferenceUID(mock.getDocumentReferences().get(0).getUID()).setQuantity(mock.getQuantity()).setShippingCostsAmount(mock.getShippingCostsAmount()).setUnitAmount(AmountType.WITH_TAX, mock.getUnitAmountWithTax()).setUnitOfMeasure(mock.getUnitOfMeasure()).setProductUID(mock.getProduct().getUID()).setTaxPointDate(mock.getTaxPointDate()).setCurrency(Currency.getInstance("EUR")).setTaxExemptionCode("M 9 9");
    try {
      builder.build();
      Assertions.fail();
  ...
  @Test public void doTestFailWithMissingTaxExemptionCode() {
    MockPTGenericInvoiceEntryEntity mock = this.createMockEntity(MockPTGenericInvoiceEntryEntity.class, TestPTGenericInvoiceEntryBuilder.PT_GENERIC_INVOICE_ENTRY_WITHOUT_TAXES_YML);
    mock.currency = Currency.getInstance("EUR");
    Mockito.when(this.getInstance(DAOPTGenericInvoiceEntry.class).getEntityInstance()).thenReturn(new MockPTGenericInvoiceEntryEntity());
    MockPTGenericInvoiceEntity mockInvoice = this.createMockEntity(MockPTGenericInvoiceEntity.class, TestPTGenericInvoiceEntryBuilder.PT_GENERIC_INVOICE_WITHOUT_TAXES_YML);
    Mockito.when(this.getInstance(DAOPTGenericInvoice.class).get(Mockito.any(UID.class))).thenReturn(mockInvoice);
    Mockito.when(this.getInstance(DAOPTProduct.class).get(Mockito.any(UID.class))).thenReturn((PTProductEntity) mock.getProduct());
    Mockito.when(this.getInstance(DAOPTRegionContext.class).isSameOrSubContext(Mockito.any(), Mockito.any(PTRegionContext.class))).thenReturn(true);
    mock.getDocumentReferences().add(mockInvoice);
