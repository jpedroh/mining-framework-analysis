...
public class TestPTInvoiceIssuingHandlerWithTicket extends PTDocumentAbstractTest {
  ...
  @Before public void setUpNewInvoice() {
    try {
      ...
      PTInvoice.Builder invoiceBuilder = new PTInvoiceTestUtil(PTAbstractTest.injector).getInvoiceBuilder(business, TestPTInvoiceIssuingHandlerWithTicket.SOURCE_BILLING);
  ...
  @Test public void testIssuedInvoiceSimpleWithTicket() throws DocumentIssuingException {
    ...
    String formatedNumber = TestPTInvoiceIssuingHandlerWithTicket.DEFAULT_TYPE + " " + PTPersistencyAbstractTest.DEFAULT_SERIES + "/1";
  ...
  @Test public void testIssueWithUsedTicket() {
    ...
    this.parameters.setInvoiceSeries(PTPersistencyAbstractTest.DEFAULT_SERIES);
    PTBusinessEntity business = new PTBusinessTestUtil(PTAbstractTest.injector).getBusinessEntity("business");
    PTInvoice.Builder builder = new PTInvoiceTestUtil(PTAbstractTest.injector).getInvoiceBuilder(business, TestPTInvoiceIssuingHandlerWithTicket.SOURCE_BILLING);
    try {
      entity = (PTInvoiceEntity) services.issueDocument(builder, this.parameters, this.issuedInvoiceUID.getValue());
    } catch (InvalidTicketException e) {
    } catch (DocumentIssuingException e) {
      e.printStackTrace();
    }
  ...
  @Test public void testOpenCloseConnections() {
    Services services = new Services(PTAbstractTest.injector);
    PTInvoicePersistenceService persistenceService = PTAbstractTest.injector.getInstance(PTInvoicePersistenceService.class);
    PTBusinessEntity business = new PTBusinessTestUtil(PTAbstractTest.injector).getBusinessEntity("business");
    PTInvoice.Builder testinvoice = new PTInvoiceTestUtil(PTAbstractTest.injector).getInvoiceBuilder(business, TestPTInvoiceIssuingHandlerWithTicket.SOURCE_BILLING);
