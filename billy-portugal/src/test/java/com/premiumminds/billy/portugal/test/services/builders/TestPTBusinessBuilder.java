...
public class TestPTBusinessBuilder extends PTAbstractTest {
  ...
  @Test public void doTest() {
    MockPTBusinessEntity mockBusiness = this.createMockEntity(MockPTBusinessEntity.class, TestPTBusinessBuilder.PTBUSINESS_YML);
    Mockito.when(this.getInstance(DAOPTBusiness.class).getEntityInstance()).thenReturn(new MockPTBusinessEntity());
    Mockito.when(this.getInstance(DAOPTRegionContext.class).get(Matchers.any(UID.class))).thenReturn((PTRegionContextEntity) mockBusiness.getOperationalContext());
    ...
    Mockito.when(mockApplicationBuilder.build()).thenReturn((PTApplicationEntity) mockBusiness.getApplications().get(0));
    ...
    Mockito.when(mockShippingAddressBuilder.build()).thenReturn((PTAddressEntity) mockBusiness.getShippingAddress());
    ...
    builder.setFinancialID(mockBusiness.getFinancialID(), PTAbstractTest.PT_COUNTRY_CODE).setName(mockBusiness.getName()).setAddress(mockAddressBuilder).setBillingAddress(mockBillingAddressBuilder).setShippingAddress(mockShippingAddressBuilder).addApplication(mockApplicationBuilder).addContact(mockMainContactBuilder, true).setWebsite(mockBusiness.getWebsiteAddress()).setOperationalContextUID(mockBusiness.getOperationalContext().getUID()).setCommercialName(mockBusiness.getCommercialName());
