...
public class TestPTReceiptInvoiceIssuingHandlerWithTicket extends PTDocumentAbstractTest {
  ...
  @BeforeEach public void setUpNewReceiptInvoice() {
    try {
      ...
      PTReceiptInvoice.Builder receiptInvoiceBuilder = new PTReceiptInvoiceTestUtil(PTAbstractTest.injector).getReceiptInvoiceBuilder(business, TestPTReceiptInvoiceIssuingHandlerWithTicket.SOURCE_BILLING);
  ...
  @Test public void testIssuedInvoiceReceiptWithTicket() throws DocumentIssuingException {
    PTReceiptInvoice issuedInvoice = this.getInstance(DAOPTReceiptInvoice.class).get(this.issuedInvoiceUID);
    PTReceiptInvoicePersistenceService service = PTAbstractTest.injector.getInstance(PTReceiptInvoicePersistenceService.class);
    ...
    String formatedNumber = TestPTReceiptInvoiceIssuingHandlerWithTicket.DEFAULT_TYPE + " " + PTPersistencyAbstractTest.DEFAULT_SERIES + "/1";
    Assertions.assertEquals(formatedNumber, issuedInvoice.getNumber());
    Assertions.assertEquals(TestPTReceiptInvoiceIssuingHandlerWithTicket.SOURCE_BILLING, issuedInvoice.getSourceBilling());
  ...
  @Test public void testIssueWithUsedTicket() {
    ...
    PTReceiptInvoice.Builder builder = new PTReceiptInvoiceTestUtil(PTAbstractTest.injector).getReceiptInvoiceBuilder(business, TestPTReceiptInvoiceIssuingHandlerWithTicket.SOURCE_BILLING);
    try {
      entity = (PTReceiptInvoiceEntity) services.issueDocument(builder, this.parameters, this.issuedInvoiceUID.getValue());
    } catch (InvalidTicketException e) {
    } catch (DocumentIssuingException e) {
      e.printStackTrace();
    }
  ...
  @Test public void testOpenCloseConnections() {
    Services services = new Services(PTAbstractTest.injector);
    PTReceiptInvoicePersistenceService persistenceService = PTAbstractTest.injector.getInstance(PTReceiptInvoicePersistenceService.class);
    PTBusinessEntity business = new PTBusinessTestUtil(PTAbstractTest.injector).getBusinessEntity("business");
    PTReceiptInvoice.Builder testinvoice = new PTReceiptInvoiceTestUtil(PTAbstractTest.injector).getReceiptInvoiceBuilder(business, TestPTReceiptInvoiceIssuingHandlerWithTicket.SOURCE_BILLING);
