...
public class PTInvoiceTestUtil {
  ...
  public PTInvoiceEntity getInvoiceEntity(SourceBilling billing) {
    ...
    switch (billing) {
      case M:
      invoice = (PTInvoiceEntity) this.getManualInvoiceBuilder(this.business.getBusinessEntity(), billing).build();
      break;
      case P:
      default:
  ...
  public PTInvoice.Builder getInvoiceBuilder(PTBusinessEntity business, SourceBilling billing) {
    ...
    return invoiceBuilder.setBilled(PTInvoiceTestUtil.BILLED).setCancelled(PTInvoiceTestUtil.CANCELLED).setSelfBilled(PTInvoiceTestUtil.SELFBILL).setDate(new Date()).setSourceId(PTInvoiceTestUtil.SOURCE_ID).setCustomerUID(customerUID).setSourceBilling(billing).setBusinessUID(business.getUID()).addPayment(this.payment.getPaymentBuilder());
  }
  ...
  public PTInvoice.ManualBuilder getManualInvoiceBuilder(PTBusinessEntity business, SourceBilling billing) {
    ...
    PTCustomerEntity customerEntity = this.customer.getCustomerEntity();
    UID customerUID = daoPTCustomer.create(customerEntity).getUID();
    PTInvoiceEntry.ManualBuilder invoiceEntryBuilder = this.invoiceEntry.getManualInvoiceEntryBuilder().setUnitAmount(AmountType.WITH_TAX, price).setUnitAmount(AmountType.WITHOUT_TAX, price.subtract(tax)).setUnitTaxAmount(tax).setAmount(AmountType.WITH_TAX, price).setAmount(AmountType.WITHOUT_TAX, price.subtract(tax)).setTaxAmount(tax);
    invoiceBuilder.addEntry(invoiceEntryBuilder);
    return invoiceBuilder.setBilled(PTInvoiceTestUtil.BILLED).setCancelled(PTInvoiceTestUtil.CANCELLED).setSelfBilled(PTInvoiceTestUtil.SELFBILL).setDate(new Date()).setSourceId(PTInvoiceTestUtil.SOURCE_ID).setCustomerUID(customerUID).setSourceBilling(billing).setBusinessUID(business.getUID()).setAmount(AmountType.WITH_TAX, price).setAmount(AmountType.WITHOUT_TAX, price.subtract(tax)).setTaxAmount(tax).addPayment(this.payment.getPaymentBuilder());
  }
  ...
  public PTInvoiceEntity getDiferentRegionsInvoice() {
    ...
    invoiceBuilder.setBilled(PTInvoiceTestUtil.BILLED).setCancelled(PTInvoiceTestUtil.CANCELLED).setSelfBilled(PTInvoiceTestUtil.SELFBILL).setDate(new Date()).setSourceId(PTInvoiceTestUtil.SOURCE_ID).setCustomerUID(customerUID).setSourceBilling(SourceBilling.P).setBusinessUID(this.business.getBusinessEntity().getUID()).addPayment(this.payment.getPaymentBuilder());
