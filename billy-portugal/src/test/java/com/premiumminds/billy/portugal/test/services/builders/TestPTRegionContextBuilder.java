...
public class TestPTRegionContextBuilder extends PTAbstractTest {
  ...
  @Test public void testRegionCode() {
    MockPTRegionContextEntity mockRegionContextEntity = this.createMockEntity(MockPTRegionContextEntity.class, TestPTRegionContextBuilder.PTCONTEXT_YML);
    Mockito.when(this.getInstance(DAOPTRegionContext.class).getEntityInstance()).thenReturn(new MockPTRegionContextEntity());
    Mockito.when(this.getInstance(DAOPTRegionContext.class).get(Matchers.any(UID.class))).thenReturn((ContextEntity) mockRegionContextEntity.getParentContext());
    PTRegionContext.Builder builder = this.getInstance(PTRegionContext.Builder.class);
    builder.setDescription(mockRegionContextEntity.getDescription()).setName(mockRegionContextEntity.getName()).setRegionCode(mockRegionContextEntity.getRegionCode()).setParentContextUID(mockRegionContextEntity.getParentContext().getUID());
