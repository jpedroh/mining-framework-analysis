...
public class TestPTCreditNoteBuilder extends PTAbstractTest {
  ...
  @Test public void doTest() {
    MockPTCreditNoteEntity mock = this.createMockEntity(MockPTCreditNoteEntity.class, TestPTCreditNoteBuilder.PT_CREDIT_NOTE_YML);
    mock.setCurrency(Currency.getInstance("EUR"));
    MockPTCustomerEntity mockCustomerEntity = this.createMockEntity(MockPTCustomerEntity.class, TestPTCreditNoteBuilder.PTCUSTOMER_YML);
    Mockito.when(this.getInstance(DAOPTCustomer.class).get(Matchers.any(UID.class))).thenReturn(mockCustomerEntity);
    Mockito.when(this.getInstance(DAOPTSimpleInvoice.class).getEntityInstance()).thenReturn(new MockPTSimpleInvoiceEntity());
    Mockito.when(this.getInstance(DAOPTCreditNote.class).getEntityInstance()).thenReturn(new MockPTCreditNoteEntity());
    MockPTCreditNoteEntryEntity entryMock = this.createMockEntity(MockPTCreditNoteEntryEntity.class, TestPTCreditNoteBuilder.PT_CREDIT_NOTE_ENTRY_YML);
    ...
    MockPTPaymentEntity mockPayment = this.createMockEntity(MockPTPaymentEntity.class, TestPTCreditNoteBuilder.PT_PAYMENT_YML);
