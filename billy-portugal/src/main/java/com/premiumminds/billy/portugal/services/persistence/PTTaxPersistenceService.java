...
public class PTTaxPersistenceService implements PersistenceService<PTTax> {
  ...
  @Inject public PTTaxPersistenceService(DAOPTTax daoTax) {
    this.daoTax = daoTax;
  }

  @Override public PTTax create(final Builder<PTTax> builder) {
    try {
      return new TransactionWrapper<PTTax>(this.daoTax) {
        @Override public PTTax runTransaction() throws Exception {
  ...
  @Override public PTTax update(final Builder<PTTax> builder) {
    try {
      return new TransactionWrapper<PTTax>(this.daoTax) {
        @Override public PTTax runTransaction() throws Exception {
          PTTaxEntity entity = (PTTaxEntity) builder.build();
  ...
  @Override public PTTax get(final UID uid) {
    try {
      return new TransactionWrapper<PTTax>(this.daoTax) {
        @Override public PTTax runTransaction() throws Exception {
