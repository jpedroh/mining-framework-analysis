 ...
 *
 */
@Singleton public class BillyPortugal {
  ...
  public BillyPortugal(String persistenceUnitId) {
    this.injector = Guice.createInjector(new PortugalDependencyModule(), new PortugalPersistenceDependencyModule(persistenceUnitId));
    this.injector.getInstance(PersistService.class).start();
  }

  @Inject public BillyPortugal(Injector injector) {
