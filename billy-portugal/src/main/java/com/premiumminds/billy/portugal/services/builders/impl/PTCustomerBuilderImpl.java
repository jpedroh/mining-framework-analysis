...
public class PTCustomerBuilderImpl<TBuilder extends PTCustomerBuilderImpl<TBuilder, TCustomer>, TCustomer extends PTCustomer> extends CustomerBuilderImpl<TBuilder, TCustomer> implements PTCustomerBuilder<TBuilder, TCustomer> {
  protected static final Localizer LOCALIZER = new Localizer("com/premiumminds/billy/core/i18n/FieldNames");

  @Inject protected PTCustomerBuilderImpl(DAOPTCustomer daoPTCustomer, DAOPTContact daoPTContact) {
  ...
  @Override protected PTCustomerEntity getTypeInstance() {
  ...
  @Override public <T extends Address> TBuilder setBillingAddress(Builder<T> addressBuilder) {
    BillyValidator.notNull(addressBuilder, PTCustomerBuilderImpl.LOCALIZER.getString("field.customer_billing_address"));
  ...
  @Override public TBuilder setHasSelfBillingAgreement(boolean selfBiling) {
    BillyValidator.notNull(selfBiling, PTCustomerBuilderImpl.LOCALIZER.getString("field.customer_self_billing_agreement"));
  ...
  @Override protected void validateInstance() throws BillyValidationException {
    ...
    BillyValidator.mandatory(c.getTaxRegistrationNumber(), PTCustomerBuilderImpl.LOCALIZER.getString("field.customer_tax_number"));
    BillyValidator.mandatory(c.getMainAddress(), PTCustomerBuilderImpl.LOCALIZER.getString("field.customer_main_address"));
    BillyValidator.mandatory(c.getBillingAddress(), PTCustomerBuilderImpl.LOCALIZER.getString("field.customer_billing_address"));
    BillyValidator.mandatory(c.hasSelfBillingAgreement(), PTCustomerBuilderImpl.LOCALIZER.getString("field.customer_self_billing_agreement"));
