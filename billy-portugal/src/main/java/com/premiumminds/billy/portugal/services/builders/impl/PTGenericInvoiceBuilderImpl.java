...
public class PTGenericInvoiceBuilderImpl<TBuilder extends PTGenericInvoiceBuilderImpl<TBuilder, TEntry, TDocument>, TEntry extends PTGenericInvoiceEntry, TDocument extends PTGenericInvoice> extends GenericInvoiceBuilderImpl<TBuilder, TEntry, TDocument> implements PTGenericInvoiceBuilder<TBuilder, TEntry, TDocument> {
  protected static final Localizer LOCALIZER = new Localizer("com/premiumminds/billy/core/i18n/FieldNames");

  public <TDAO extends AbstractDAOPTGenericInvoice<? extends TDocument>> PTGenericInvoiceBuilderImpl(TDAO daoPTGenericInvoice, DAOPTBusiness daoPTBusiness, DAOPTCustomer daoPTCustomer, DAOPTSupplier daoPTSupplier) {
  ...
  @Override protected PTGenericInvoiceEntity getTypeInstance() {
    return (PTGenericInvoiceEntity) super.getTypeInstance();
  }

  @Override public TBuilder setCancelled(boolean cancelled) {
  ...
  @Override public TBuilder setBilled(boolean billed) {
    if (this.getTypeInstance().isCancelled()) {
      throw new BillyUpdateException("Invoice is allready cancelled!");
    }
  ...
  @Override @NotOnUpdate public TBuilder setChangeReason(String reason) {
    this.getTypeInstance().setChangeReason(reason);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setSourceBilling(SourceBilling sourceBilling) {
  ...
  @Override protected void validateInstance() throws BillyValidationException {
    ...
    BillyValidator.mandatory(i.getSourceBilling(), PTGenericInvoiceBuilderImpl.LOCALIZER.getString("field.source_billing"));
  ...
  private void validatePTInstance(PTGenericInvoiceEntity i) {
    ...
    if (i.isSelfBilled() != null) {
      BillyValidator.mandatory(i.isSelfBilled(), PTGenericInvoiceBuilderImpl.LOCALIZER.getString("field.self_billed"));
    } else {
    ...
    BillyValidator.notEmpty(i.getPayments(), PTGenericInvoiceBuilderImpl.LOCALIZER.getString("field.payment_mechanism"));
  }
}