...
public class PTCreditNotePersistenceService implements PersistenceService<PTCreditNote> {
  ...
  @Inject public PTCreditNotePersistenceService(DAOPTCreditNote daoCreditNote, DAOTicket daoTicket) {
    this.daoCreditNote = daoCreditNote;
    this.daoTicket = daoTicket;
  }

  @Override @NotImplemented public PTCreditNote create(final Builder<PTCreditNote> builder) {
    return null;
  }

  @Override public PTCreditNote update(final Builder<PTCreditNote> builder) {
    try {
      return new TransactionWrapper<PTCreditNote>(this.daoCreditNote) {
        @Override public PTCreditNote runTransaction() throws Exception {
  ...
  @Override public PTCreditNote get(final UID uid) {
    try {
      return new TransactionWrapper<PTCreditNote>(this.daoCreditNote) {
        @Override public PTCreditNote runTransaction() throws Exception {
  ...
  public PTCreditNote getWithTicket(final UID ticketUID) throws NoResultException, BillyRuntimeException {
    try {
      return new TransactionWrapper<PTCreditNote>(this.daoCreditNote) {
        @Override public PTCreditNote runTransaction() throws Exception {
          UID objectUID = PTCreditNotePersistenceService.this.daoTicket.getObjectEntityUID(ticketUID.getValue());
  ...
  public List<PTCreditNote> findByReferencedDocument(final UID uidCompany, final UID uidInvoice) {
    try {
      return new TransactionWrapper<List<PTCreditNote>>(this.daoCreditNote) {
        @Override public List<PTCreditNote> runTransaction() throws Exception {
          return PTCreditNotePersistenceService.this.daoCreditNote.findByReferencedDocument(uidCompany, uidInvoice);
        }
      }.execute();
    } catch (NoResultException e) {
      throw e;
