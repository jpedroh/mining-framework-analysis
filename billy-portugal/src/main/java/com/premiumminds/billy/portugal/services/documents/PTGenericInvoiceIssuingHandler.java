...
public abstract class PTGenericInvoiceIssuingHandler<T extends PTGenericInvoiceEntity, P extends PTIssuingParams> implements DocumentIssuingHandler<T, P> {
  ...
  protected <D extends AbstractDAOPTGenericInvoice<T>> T issue(final T document, final PTIssuingParams parametersPT, final D daoInvoice, final TYPE invoiceType) throws DocumentIssuingException {
    ...
    final String atcud = invoiceSeriesEntity.getSeriesUniqueCode().map((s) -> new StringBuilder().append(s).append("-").append(seriesNumber.toString())).orElse(new StringBuilder().append("0")).toString();
