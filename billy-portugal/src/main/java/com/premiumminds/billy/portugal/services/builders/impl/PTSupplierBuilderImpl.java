...
public class PTSupplierBuilderImpl<TBuilder extends PTSupplierBuilderImpl<TBuilder, TSupplier>, TSupplier extends PTSupplier> extends SupplierBuilderImpl<TBuilder, TSupplier> implements PTSupplierBuilder<TBuilder, TSupplier> {
  protected static final Localizer LOCALIZER = new Localizer("com/premiumminds/billy/core/i18n/FieldNames");

  @Inject public PTSupplierBuilderImpl(DAOPTSupplier daoPTSupplier) {
    super(daoPTSupplier);
  }

  @Override protected PTSupplierEntity getTypeInstance() {
  ...
  @Override public <T extends Address> TBuilder setBillingAddress(Builder<T> addressBuilder) {
    BillyValidator.mandatory(addressBuilder, PTSupplierBuilderImpl.LOCALIZER.getString("field.supplier_billing_address"));
  ...
  @Override public TBuilder setSelfBillingAgreement(boolean selfBilling) {
    BillyValidator.mandatory(selfBilling, PTSupplierBuilderImpl.LOCALIZER.getString("field.supplier_self_billing_agreement"));
  ...
  @Override protected void validateInstance() throws BillyValidationException {
    ...
    BillyValidator.mandatory(s.getTaxRegistrationNumber(), PTSupplierBuilderImpl.LOCALIZER.getString("field.supplier_tax_number"));
    BillyValidator.mandatory(s.getBillingAddress(), PTSupplierBuilderImpl.LOCALIZER.getString("field.supplier_billing_address"));
    BillyValidator.mandatory(s.hasSelfBillingAgreement(), PTSupplierBuilderImpl.LOCALIZER.getString("field.supplier_self_billing_agreement"));
  }
}