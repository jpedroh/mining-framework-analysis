...
public class PTSupplierPersistenceService implements PersistenceService<PTSupplier> {
  ...
  @Inject public PTSupplierPersistenceService(DAOPTSupplier daoSupplier) {
    this.daoSupplier = daoSupplier;
  }

  @Override public PTSupplier create(final Builder<PTSupplier> builder) {
    try {
      return new TransactionWrapper<PTSupplier>(this.daoSupplier) {
        @Override public PTSupplier runTransaction() throws Exception {
  ...
  @Override public PTSupplier update(final Builder<PTSupplier> builder) {
    try {
      return new TransactionWrapper<PTSupplier>(this.daoSupplier) {
        @Override public PTSupplier runTransaction() throws Exception {
          PTSupplierEntity entity = (PTSupplierEntity) builder.build();
  ...
  @Override public PTSupplier get(final UID uid) {
    try {
      return new TransactionWrapper<PTSupplier>(this.daoSupplier) {
        @Override public PTSupplier runTransaction() throws Exception {
