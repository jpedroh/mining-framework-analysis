...
public class RedPen extends Validator<Document> {
  ...
  static Type getParameterizedClass(Object obj) {
    ...
    try {
      parameterizedType = ParameterizedType.class.cast(genericInterface);
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) private void loadValidators() throws RedPenException {
  ...
  private List<ValidationError> runDocumentValidators(DocumentCollection documentCollection, List<ValidationError> errors) {
  ...
  private List<ValidationError> runSectionValidators(DocumentCollection documentCollection, List<ValidationError> errors) {
    for (Document document : documentCollection) {
  ...
  private List<ValidationError> runSentenceValidators(DocumentCollection documentCollection, List<ValidationError> errors) {
  ...
  private void runSentencePreProcessorsToDocumentCollection(DocumentCollection documentCollection, List<ValidationError> errors) {
    for (Document document : documentCollection) {
      for (Section section : document) {
  ...
  private void preprocessSentences(List<Sentence> sentences) {
    for (Validator<Sentence> sentenceValidator : sentenceValidators) {
      ...
      if (clazz.getInterfaces().length > 0 && clazz.getInterfaces()[0].equals(cc.redpen.validator.PreProcessor.class)) {
  ...
  private void runSentenceValidatorsToDocumentCollection(DocumentCollection documentCollection, List<ValidationError> errors) {
    for (Document document : documentCollection) {
      for (Section section : document) {
        List<ValidationError> newErrors = applySentenceValidationsToSection(document, section);
        errors.addAll(newErrors);
      }
    }
  }

  private List<ValidationError> applySentenceValidationsToSection(Document document, Section section) {
  ...
  @Override public boolean equals(Object o) {
    ...
    if (distributor != null ? !distributor.equals(redPen.distributor) : redPen.distributor != null) {
      return false;
    }
  ...
  @Override public String toString() {
    return "RedPen{" + "validators=" + validators + ", sectionValidators=" + sectionValidators + ", sentenceValidators=" + sentenceValidators + ", distributor=" + distributor + '}';
  }
