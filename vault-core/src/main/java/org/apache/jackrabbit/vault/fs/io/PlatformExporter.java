...
public class PlatformExporter extends AbstractExporter {
  ...
  public void close() throws IOException {
    if (pruneMissing) {
      exportInfo.getEntries().values().stream().filter((e) -> e.type == ExportInfo.Type.RMDIR).map((e) -> e.path).sorted(Comparator.comparingInt(String::length)).forEachOrdered((path) -> {
      ...
      exportInfo.getEntries().values().stream().filter((e) -> e.type == ExportInfo.Type.DELETE).map((e) -> e.path).forEach((path) -> {
        File file = new File(path);
        if (file.exists()) {
          FileUtils.deleteQuietly(file);
          track("D", PathUtil.getRelativePath(localParent.getAbsolutePath(), path));
  ...
  public void writeFile(VaultFile file, String relPath) throws RepositoryException, IOException {
    ...
    switch (a.getPreferredAccess()) {
      ...
      break;
      case STREAM:
      try (InputStream in = a.getInputStream(); OutputStream out = new FileOutputStream(local)) {
        IOUtils.copy(in, out);
      }
      break;
    }
