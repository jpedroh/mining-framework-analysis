...
public class JsonExtractorFactory implements IExtractorFactory<StreamConnector> {
  ...
  @Override public Extractor createExtractor(StreamConnector connector, ExtractorModel model) {
    return new JsonExtractor(jacksonService, connector, mapToJsonAttributes(model.getAttributes()), getRootQuery(model));
  }
