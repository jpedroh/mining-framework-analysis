...
@WithPlugins(value = { "sonar", "filesystem_scm", "plain-credentials" }) public class SonarPluginTest extends AbstractJUnitTest {
...
<<<<<<< /usr/src/app/output/sonarsource/jenkins-sonar-plugin/b784e8f5f5d9658f4e644526211c2806df5b73ff/its/src/test/java/com/sonar/it/jenkins/SonarPluginTest.java/left.java
  SONAR_PUBLIC_BASE_URL = "http://sonar_public_domain"
=======
  MS_BUILD_RECENT_VERSION = "4.7.1.2311"
>>>>>>> /usr/src/app/output/sonarsource/jenkins-sonar-plugin/b784e8f5f5d9658f4e644526211c2806df5b73ff/its/src/test/java/com/sonar/it/jenkins/SonarPluginTest.java/right.java
  ;

  private final static String SONAR_PUBLIC_BASE_URL_PROPERTY = "sonar.core.serverBaseURL";

  @ClassRule public static final Orchestrator ORCHESTRATOR = Orchestrator.builderEnv().setSonarVersion(requireNonNull(System.getProperty("sonar.runtimeVersion"), "Please set system property sonar.runtimeVersion")).addPlugin(MavenLocation.of("org.sonarsource.java", "sonar-java-plugin", "5.14.0.18788")).addPlugin(MavenLocation.of("org.sonarsource.javascript", "sonar-javascript-plugin", "5.2.1.7778")).addPlugin(MavenLocation.of("org.sonarsource.dotnet", "sonar-csharp-plugin", "7.17.0.9346")).restoreProfileAtStartup(FileLocation.ofClasspath("/com/sonar/it/jenkins/SonarPluginTest/sonar-way-it-profile_java.xml")).build();
  ...
  @BeforeClass public static void setUpJenkins() {
    DEFAULT_QUALITY_GATE = qgClient().list().defaultGate().id();
    wsClient = WsClientFactories.getDefault().newClient(HttpConnector.newBuilder().url(ORCHESTRATOR.getServer().getUrl()).credentials(Server.ADMIN_LOGIN, Server.ADMIN_PASSWORD).build());
  }
  ...
  @Test public void testFreestyleJobWithSonarQubeScanner_use_sq_scanner_3_3() {
    ...
    Build result = jenkinsOrch.newFreestyleJobWithSQScanner(jobName, "-Duseless=Y", jsFolder, null, "sonar.projectKey", projectKey, "sonar.projectVersion", "1.0", "sonar.projectName", "Abacus", "sonar.sources", "src").executeJob(jobName);
  ...
  @Test @WithOS(os = WithOS.OS.WINDOWS) @WithPlugins(value = { "msbuild" }) public void testFreestyleJobWithScannerForMsBuild() throws FailedExecutionException {
    MSBuildScannerInstallation.install(jenkins, MS_BUILD_RECENT_VERSION, false);
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR).configureMSBuild(ORCHESTRATOR);
    String jobName = "csharp";
    String projectKey = "csharp";
    assertThat(getProject(projectKey)).isNull();
    jenkinsOrch.newFreestyleJobWithScannerForMsBuild(jobName, null, consoleApp1Folder, projectKey, "CSharp", "1.0", MS_BUILD_RECENT_VERSION, "ConsoleApplication1.sln", false).executeJob(jobName);
    waitForComputationOnSQServer();
    assertThat(getProject(projectKey)).isNotNull();
    assertSonarUrlOnJob(jobName, projectKey);
  }

  @Test public void testFreestyleJobWithScannerForMsBuild_NetCore() {
    ...
    assertThat(getProject(projectKey)).isNull();
    jenkinsOrch.newFreestyleJobWithScannerForMsBuild(jobName, null, consoleNetCoreFolder, projectKey, "CSharp NetCore", "1.0", MS_BUILD_RECENT_VERSION, "NetCoreConsoleApp.sln", true).executeJob(jobName);
    waitForComputationOnSQServer();
    assertThat(getProject(projectKey)).isNotNull();
    assertSonarUrlOnJob(jobName, projectKey);
  }

  @Test @WithOS(os = WithOS.OS.WINDOWS) @WithPlugins(value = { "msbuild" }) public void testFreestyleJobWithScannerForMsBuild_3_0() {
    ...
    assertThat(getProject(projectKey)).isNull();
    Build result = jenkinsOrch.newFreestyleJobWithScannerForMsBuild(jobName, null, jsFolder, projectKey, "JS with space", "1.0", "3.0.0.629", null, false).executeJobQuietly(jobName);
    assertThat(result.getConsole()).contains("tools" + File.separator + "hudson.plugins.sonar.MsBuildSQRunnerInstallation" + File.separator + "Scanner_for_MSBuild_3.0.0.629" + File.separator + "MSBuild.SonarQube.Runner.exe begin /k:" + projectKey + " \"/n:JS with space\" /v:1.0 /d:sonar.host.url=" + ORCHESTRATOR.getServer().getUrl());
  }

  @Test @WithOS(os = WithOS.OS.WINDOWS) @WithPlugins(value = { "msbuild" }) public void testFreestyleJobWithScannerForMsBuild_2_3_2() {
    MSBuildScannerInstallation.install(jenkins, "2.3.2.573", false);
    MSBuildScannerInstallation.install(jenkins, "3.0.0.629", false);
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR).configureMSBuild(ORCHESTRATOR);
    ...
    assertThat(getProject(projectKey)).isNull();
    Build result = jenkinsOrch.newFreestyleJobWithScannerForMsBuild(jobName, null, jsFolder, projectKey, "JS with space", "1.0", "2.3.2.573", null, false).executeJobQuietly(jobName);
    assertThat(result.getConsole()).contains("tools" + File.separator + "hudson.plugins.sonar.MsBuildSQRunnerInstallation" + File.separator + "Scanner_for_MSBuild_2.3.2.573" + File.separator + "MSBuild.SonarQube.Runner.exe begin /k:" + projectKey + " \"/n:JS with space\" /v:1.0 /d:sonar.host.url=" + ORCHESTRATOR.getServer().getUrl());
  }

  @Test @Since(value = "2.0") public void testNoSonarPublisher() {
    String jobName = "noSonarPublisher";
    jenkinsOrch.assertNoSonarPublisher(jobName, new File("projects", "noPublisher"));
  }

  @Test @WithPlugins(value = { "maven-plugin" }) public void testMavenJob() {
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR).configureMaven(ORCHESTRATOR);
    String jobName = "abacus-maven";
    String projectKey = "org.codehaus.sonar-plugins:sonar-abacus-plugin";
    assertThat(getProject(projectKey)).isNull();
    jenkinsOrch.newMavenJobWithSonar(jobName, new File("projects", "abacus"), null).executeJob(jobName);
    waitForComputationOnSQServer();
    assertThat(getProject(projectKey)).isNotNull();
    assertSonarUrlOnJob(jobName, projectKey);
    jenkinsOrch.assertQGOnProjectPage(jobName);
  ...
  @Test @WithPlugins(value = { "maven-plugin" }) public void testVariableInjection() throws JenkinsUtils.FailedExecutionException {
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR).configureMaven(ORCHESTRATOR);
    String jobName = "abacus-freestyle-vars";
    String projectKey = "org.codehaus.sonar-plugins:sonar-abacus-plugin";
    assertThat(getProject(projectKey)).isNull();
    jenkinsOrch.enableInjectionVars(true).newFreestyleJobWithMaven(jobName, new File("projects", "abacus"), null, ORCHESTRATOR).executeJob(jobName);
    waitForComputationOnSQServer();
    assertThat(getProject(projectKey)).isNotNull();
    assertSonarUrlOnJob(jobName, projectKey);
    jenkinsOrch.assertQGOnProjectPage(jobName);
  ...
  @Test @WithPlugins(value = { "maven-plugin" }) public void testFreestyleJobWithSonarMaven() {
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR).configureMaven(ORCHESTRATOR);
    String jobName = "abacus-freestyle";
    String projectKey = "org.codehaus.sonar-plugins:sonar-abacus-plugin";
    assertThat(getProject(projectKey)).isNull();
    jenkinsOrch.newFreestyleJobWithSonar(jobName, new File("projects", "abacus"), null).executeJob(jobName);
    waitForComputationOnSQServer();
    assertThat(getProject(projectKey)).isNotNull();
    assertSonarUrlOnJob(jobName, projectKey);
    jenkinsOrch.assertQGOnProjectPage(jobName);
  ...
  @Test @WithPlugins(value = "workflow-aggregator") public void no_sq_vars_without_env_wrapper() throws JenkinsUtils.FailedExecutionException {
  ...
  @Test @WithPlugins(value = "workflow-aggregator") public void env_wrapper_without_params_should_inject_sq_vars() {
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR);
    String script = "withSonarQubeEnv { " + DUMP_ENV_VARS_PIPELINE_CMD + " }";
    runAndVerifyEnvVarsExist("withSonarQubeEnv-parameterless", script);
  }

  @Test @WithPlugins(value = "workflow-aggregator") public void env_wrapper_with_specific_sq_should_inject_sq_vars() {
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR);
  ...
  @Test(expected = JenkinsUtils.FailedExecutionException.class) @WithPlugins(value = "workflow-aggregator") public void env_wrapper_with_nonexistent_sq_should_fail() {
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR);
  ...
  @Test @WithPlugins(value = { "workflow-aggregator", "msbuild" }) @WithOS(os = WithOS.OS.WINDOWS) public void msbuild_pipeline() {
    MSBuildScannerInstallation.install(jenkins, "3.0.0.629", false);
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR);
  ...
  @Test @WithPlugins(value = "workflow-aggregator") public void qualitygate_pipeline_ok() {
    SonarScannerInstallation.install(jenkins, JENKINS_VERSION);
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR);
  ...
  @Test @WithPlugins(value = "workflow-aggregator") public void qualitygate_pipeline_ko() {
    SonarScannerInstallation.install(jenkins, JENKINS_VERSION);
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR);
  ...
  @Test @WithPlugins(value = "workflow-aggregator") public void qualitygate_pipeline_failed_with_unknown_webhook_secret_id() {
    SonarScannerInstallation.install(jenkins, JENKINS_VERSION);
    jenkinsOrch.configureSonarInstallation(ORCHESTRATOR);
    StringBuilder script = new StringBuilder();
    script.append("withSonarQubeEnv(\'" + DEFAULT_SONARQUBE_INSTALLATION + "\') {\n");
  ...
  @Test @WithPlugins(value = "workflow-aggregator") public void qualitygate_with_wrong_webhook_secret_fails_pipeline() {
  ...
  private String enableWebhook() {
    ...
    if (ORCHESTRATOR.getServer().version().isGreaterThanOrEquals(7, 1)) {
      Webhooks.CreateWsResponse response = wsClient.webhooks().create(new CreateRequest().setName("Jenkins").setUrl(url).setSecret(SECRET));
  ...
  private void setWebhookSecret(String secret) {
    String url = StringUtils.removeEnd(jenkins.getCurrentUrl(), "/") + "/sonarqube-webhook/";
    if (ORCHESTRATOR.getServer().version().isGreaterThanOrEquals(7, 9)) {
      wsClient.webhooks().update(new UpdateRequest().setName("Jenkins").setUrl(url).setWebhook(webhookKey).setSecret(secret));
    } else {
  ...
  @CheckForNull static Component getProject(String componentKey) {
    try {
