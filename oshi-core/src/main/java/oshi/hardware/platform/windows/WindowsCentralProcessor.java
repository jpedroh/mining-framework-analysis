...
public class WindowsCentralProcessor extends AbstractCentralProcessor {
  ...
  private void initVars() {
    ...
    if (processorIds.length > 0) {
      ...
      setVendor(Advapi32Util.registryGetStringValue(WinReg.HKEY_LOCAL_MACHINE, cpuRegistryPath, "VendorIdentifier"));
      setName(Advapi32Util.registryGetStringValue(WinReg.HKEY_LOCAL_MACHINE, cpuRegistryPath, "ProcessorNameString"));
      setIdentifier(Advapi32Util.registryGetStringValue(WinReg.HKEY_LOCAL_MACHINE, cpuRegistryPath, "Identifier"));
    }
    ...
    if (sysinfo.processorArchitecture.pi.wProcessorArchitecture.intValue() == 9 || sysinfo.processorArchitecture.pi.wProcessorArchitecture.intValue() == 6) {
      ...
      if (sysinfo.processorArchitecture.pi.wProcessorArchitecture.intValue() == 0) {
        setCpu64(false);
      }
    }
  ...
  private void initPdhCounters() {
    ...
    if (!enumeration) {
      ...
      processorTickCountQuery = new WmiQuery<>("Win32_PerfRawData_PerfOS_Processor WHERE NOT Name=\"_Total\"", ProcessorTickCountProperty.class);
      systemTickCountQuery = new WmiQuery<>("Win32_PerfRawData_PerfOS_Processor WHERE Name=\"_Total\"", SystemTickCountProperty.class);
      interruptsQuery = new WmiQuery<>("Win32_PerfRawData_PerfOS_Processor WHERE Name=\"_Total\"", InterruptsProperty.class);
    }
    processorTickCountQuery = new WmiQuery<>("Win32_PerfRawData_PerfOS_Processor WHERE NOT Name=\"_Total\"", ProcessorTickCountProperty.class);
  }
  ...
  /**
     ...
     */
  @Override protected void calculateProcessorCounts() {
    SYSTEM_INFO sysinfo = new SYSTEM_INFO();
  ...
  @Override public long[] getSystemCpuLoadTicks() {
    ...
    if (this.systemTickCountQuery == null) {
      ...
      if (result.getResultCount() > 0) {
        ticks[TickType.IRQ.getIndex()] = WmiUtil.getUint64(result, SystemTickCountProperty.PERCENTINTERRUPTTIME, 0) / 10000L;
        ticks[TickType.SOFTIRQ.getIndex()] = WmiUtil.getUint64(result, SystemTickCountProperty.PERCENTDPCTIME, 0) / 10000L;
      }
    }
    ticks[TickType.IDLE.getIndex()] = lpIdleTime.toDWordLong().longValue() / 10000L;
    ticks[TickType.SYSTEM.getIndex()] = lpKernelTime.toDWordLong().longValue() / 10000L - ticks[TickType.IDLE.getIndex()];
  ...
  @Override public long[][] getProcessorCpuLoadTicks() {
    ...
    if (this.processorTickCountQuery == null) {
      ...
      for (int cpu = 0; cpu < result.getResultCount() && cpu < this.logicalProcessorCount; cpu++) {
        ticks[cpu][TickType.SYSTEM.getIndex()] = WmiUtil.getUint64(result, ProcessorTickCountProperty.PERCENTPRIVILEGEDTIME, cpu);
        ticks[cpu][TickType.USER.getIndex()] = WmiUtil.getUint64(result, ProcessorTickCountProperty.PERCENTUSERTIME, cpu);
        ticks[cpu][TickType.IRQ.getIndex()] = WmiUtil.getUint64(result, ProcessorTickCountProperty.PERCENTINTERRUPTTIME, cpu);
        ticks[cpu][TickType.SOFTIRQ.getIndex()] = WmiUtil.getUint64(result, ProcessorTickCountProperty.PERCENTDPCTIME, cpu);
        ticks[cpu][TickType.IDLE.getIndex()] = WmiUtil.getUint64(result, ProcessorTickCountProperty.PERCENTPROCESSORTIME, cpu);
      }
    }
    for (int cpu = 0; cpu < this.logicalProcessorCount; cpu++) {
      ticks[cpu][TickType.IDLE.getIndex()] -= ticks[cpu][TickType.SYSTEM.getIndex()] + ticks[cpu][TickType.USER.getIndex()];
      ticks[cpu][TickType.SYSTEM.getIndex()] -= ticks[cpu][TickType.IRQ.getIndex()] + ticks[cpu][TickType.SOFTIRQ.getIndex()];
  ...
  /**
     * {@inheritDoc}
     */
  @Override @Deprecated public String getSystemSerialNumber() {
