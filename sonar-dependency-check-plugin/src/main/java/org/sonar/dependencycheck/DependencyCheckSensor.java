...
public class DependencyCheckSensor implements Sensor {
  ...
  private void addIssue(SensorContext context, Dependency dependency, Vulnerability vulnerability) {
    ...
    Severity severity = DependencyCheckUtils.cvssToSonarQubeSeverity(vulnerability.getCvssScore(), severityCritical, severityMajor, context.settings().getDouble(DependencyCheckConstants.SEVERITY_MINOR));
    context.newIssue().forRule(RuleKey.of(DependencyCheckPlugin.REPOSITORY_KEY, DependencyCheckPlugin.RULE_KEY)).at(new DefaultIssueLocation().on(context.module()).message(formatDescription(dependency, vulnerability))).overrideSeverity(severity).save();
  ...
  private void addIssues(SensorContext context, Analysis analysis) {
    ...
    for (Dependency dependency : analysis.getDependencies()) {
      InputFile testFile = fileSystem.inputFile(fileSystem.predicates().hasPath(escapeReservedPathChars(dependency.getFilePath())));
  ...
  private void saveMeasures(SensorContext context) {
    ...
    context.<Integer>newMeasure().forMetric(DependencyCheckMetrics.INHERITED_RISK_SCORE).on(context.module()).withValue(DependencyCheckMetrics.inheritedRiskScore(criticalIssuesCount, majorIssuesCount, minorIssuesCount)).save();
    context.<Double>newMeasure().forMetric(DependencyCheckMetrics.VULNERABLE_COMPONENT_RATIO).on(context.module()).withValue(DependencyCheckMetrics.vulnerableComponentRatio(vulnerabilityCount, vulnerableDependencies)).save();
  ...
  @Override public void describe(SensorDescriptor sensorDescriptor) {
    sensorDescriptor.name(SENSOR_NAME);
  }

  @Override public void execute(SensorContext sensorContext) {
