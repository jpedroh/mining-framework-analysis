...
public final class DependencyCheckUtils {
  ...
  public static Severity cvssToSonarQubeSeverity(Float cvssScore, Float blocker, Float critical, Float major, Float minor) {
    if (blocker.floatValue() >= 0 && cvssScore.floatValue() >= blocker.doubleValue()) {
      ...
      if (critical.floatValue() >= 0 && cvssScore.floatValue() >= critical.floatValue()) {
        return Severity.CRITICAL;
      } else {
        if (major.floatValue() >= 0 && cvssScore.floatValue() >= major.floatValue()) {
          return Severity.MAJOR;
        } else {
          if (minor.floatValue() >= 0 && cvssScore.floatValue() >= minor.floatValue()) {
  ...
  public static String getRuleKey(Configuration config) {
    return config.getBoolean(DependencyCheckConstants.SECURITY_HOTSPOT).orElse(DependencyCheckConstants.SECURITY_HOTSPOT_DEFAULT) ? DependencyCheckConstants.RULE_KEY_WITH_SECURITY_HOTSPOT : DependencyCheckConstants.RULE_KEY;
  }
  ...
  public static Float severityToScore(String severity, Float blocker, Float critical, Float major, Float minor) {
    if (blocker >= 0 && StringUtils.equalsAnyIgnoreCase(severity, CRITICAL)) {
      ...
      if (critical >= 0 && StringUtils.equalsAnyIgnoreCase(severity, CRITICAL, HIGH)) {
        return critical;
      } else {
        if (major >= 0 && StringUtils.equalsAnyIgnoreCase(severity, CRITICAL, HIGH, MEDIUM, MODERATE)) {
          return major;
        } else {
          if (minor >= 0 && StringUtils.equalsAnyIgnoreCase(severity, CRITICAL, HIGH, MEDIUM, MODERATE, LOW)) {
            return minor;
          } else {
            return 0.0f;
          }
  ...
  public static String formatDescription(Dependency dependency, Vulnerability vulnerability, Configuration config) {
    ...
    if (config.getBoolean(DependencyCheckConstants.USE_FILEPATH).orElse(DependencyCheckConstants.USE_FILEPATH_DEFAULT)) {
