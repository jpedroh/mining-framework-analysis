...
public class DiscoveryWriter extends FunctionWithThreadPool<List<List<IMetric>>, Void> {
  ...
  private static final Meter locatorsWritten = Metrics.meter(DiscoveryWriter.class, "Locators Written to Discovery");
  ...
  public DiscoveryWriter(ThreadPoolExecutor threadPool) {
    ...
    maxNewLocatorsPerMinute = Configuration.getInstance().getIntegerProperty(CoreConfig.DISCOVERY_MAX_NEW_LOCATORS_PER_MINUTE);
    maxNewLocatorsPerMinutePerTenant = Configuration.getInstance().getIntegerProperty(CoreConfig.DISCOVERY_MAX_NEW_LOCATORS_PER_MINUTE_PER_TENANT);
    newLocatorsThrottle = CacheBuilder.newBuilder().concurrencyLevel(16).expireAfterWrite(1, TimeUnit.MINUTES).build();
    newLocatorsThrottlePerTenant = CacheBuilder.newBuilder().concurrencyLevel(16).expireAfterAccess(1, TimeUnit.MINUTES).build(new CacheLoader<String, Cache<Locator, Locator>>() {
      @Override public Cache<Locator, Locator> load(String key) {
        return CacheBuilder.newBuilder().concurrencyLevel(16).expireAfterWrite(1, TimeUnit.MINUTES).build();
      }
    });
  }
  ...
  public void registerIO(DiscoveryIO io) {
    discoveryIOs.add(io);
    writeErrorMeters.put(io.getClass(), Metrics.meter(io.getClass(), "DiscoveryWriter Write Errors"));
  }
  ...
  private List<IMetric> condense(List<List<IMetric>> input) throws ExecutionException {
    ...
    Set<String> tenants = input.stream().flatMap(Collection::stream).map(IMetric::getLocator).map(Locator::getTenantId).collect(Collectors.toSet());
  ...
  @Override public Void apply(List<List<IMetric>> input) {
    if (canIndex) {
