 ...
 */
@Test(groups = { "expensive" }, retryAnalyzer = ThreeTriesRetryAnalyzer.class) public class MantaClientJobIT {
  ...
  @Test(groups = { "expensive" }) public void createJob() throws IOException {
  ...
  @Test(groups = { "expensive" }, dependsOnMethods = { "createJob" }) public void getJob() throws IOException {
    MantaJob job = buildJob();
    UUID jobId = createJobAndSkipIfUnsupported(job);
    MantaJob jobResponse = mantaClient.getJob(jobId);
    Assert.assertEquals(jobResponse.getName(), job.getName());
    MantaJobPhase mapPhaseRequest = job.getMapPhases().get(0);
    MantaJobPhase mapPhaseResponse = jobResponse.getMapPhases().get(0);
    Assert.assertEquals(mapPhaseResponse.getType(), mapPhaseRequest.getType());
    Assert.assertEquals(mapPhaseResponse.getExec(), mapPhaseRequest.getExec());
  ...
  @Test(dependsOnMethods = { "createJob", "getJob" }) public void cancelJob() throws IOException {
    MantaJob job = buildJob();
    UUID jobId = createJobAndSkipIfUnsupported(job);
    mantaClient.cancelJob(jobId);
    MantaJob jobResponse = mantaClient.getJob(jobId);
  ...
  @Test(dependsOnMethods = { "createJob" }, successPercentage = 66) public void canAddAndGetInputsFromIterator() throws IOException {
    MantaJob job = buildJob();
    UUID jobId = createJobAndSkipIfUnsupported(job);
    ...
    try {
      ...
      List<String> inputsResponse = mantaClient.getJobInputs(jobId).collect(Collectors.toList());
  ...
  @Test(dependsOnMethods = { "createJob" }, successPercentage = 66) public void canAddAndGetInputsFromStream() throws IOException {
    MantaJob job = buildJob();
    UUID jobId = createJobAndSkipIfUnsupported(job);
    List<String> inputs = new ArrayList<>();
    String objPath = String.format("%s%s", testPathPrefix, UUID.randomUUID());
  ...
  @Test(dependsOnMethods = { "createJob", "getJob" }, groups = { "expensive" }) public void canListAllJobIDs() throws IOException, InterruptedException {
    ...
    try (Stream<UUID> jobs = mantaClient.getAllJobIds()) {
      List<UUID> found = jobs.filter((id) -> id.equals(job1id) || id.equals(job2id)).collect(Collectors.toList());
  ...
  @Test(dependsOnMethods = { "createJob", "getJob" }, groups = { "expensive" }) public void canListAllJobs() throws IOException, InterruptedException {
    final MantaJob job1 = buildJob();
    final UUID job1id = createJobAndSkipIfUnsupported(job1);
    final MantaJob job2 = buildJob();
    final UUID job2id = createJobAndSkipIfUnsupported(job2);
    ...
    try (Stream<MantaJob> jobs = mantaClient.getAllJobs(); Stream<MantaJob> filtered = jobs.filter(findByJobIdFilter)) {
  ...
  @Test(dependsOnMethods = { "createJob", "getJob" }, groups = { "expensive" }) public void canListAllRunningJobIDs() throws IOException, InterruptedException {
    final MantaJob job1 = buildJob();
    final UUID job1id = createJobAndSkipIfUnsupported(job1);
    final MantaJob job2 = buildJob();
    final UUID job2id = createJobAndSkipIfUnsupported(job2);
    ...
    try (Stream<UUID> jobs = mantaClient.getJobIdsByState("running")) {
      List<UUID> found = jobs.filter(searchIds::contains).collect(Collectors.toList());
      Assert.assertEquals(found.size(), 2, "We should have found both jobs");
    } catch (AssertionError e) {
      String msg = "Couldn\'t find job in job list, retry test a few times to verify";
      LOG.error(msg, e);
  ...
  @Test(dependsOnMethods = { "createJob", "getJob" }, groups = { "expensive" }) public void canListAllRunningJobs() throws IOException, InterruptedException {
    final MantaJob job1 = buildJob();
    final UUID job1id = createJobAndSkipIfUnsupported(job1);
    final MantaJob job2 = buildJob();
    final UUID job2id = createJobAndSkipIfUnsupported(job2);
    ...
    try (Stream<MantaJob> jobs = mantaClient.getJobsByState("running")) {
      List<MantaJob> found = jobs.filter((j) -> searchIds.contains(j.getId())).collect(Collectors.toList());
      Assert.assertEquals(found.size(), 2, "We should have found both jobs");
    } catch (AssertionError e) {
      String msg = "Couldn\'t find job in job list, retry test a few times to verify";
      LOG.error(msg, e);
  ...
  @Test(dependsOnMethods = { "createJob", "getJob" }) public void canListJobIdsByName() throws IOException {
  ...
  @Test public void canListOutputsForJobWithNoInputs() throws IOException, InterruptedException {
    ...
    List<String> outputs = mantaClient.getJobOutputs(jobId).collect(Collectors.toList());
    Assert.assertEquals(outputs.size(), 0);
  }

  @Test public void canListOutputsForJobWithOneInput() throws IOException, InterruptedException {
    ...
    Assert.assertEquals(actual, TEST_DATA, "Output wasn\'t the same as input");
  }
  ...
  @Test public void canListOutputsForJobAsStrings() throws IOException, InterruptedException {
    String path1 = String.format("%s%s", testPathPrefix, UUID.randomUUID());
    mantaClient.put(path1, TEST_DATA);
    String path2 = String.format("%s%s", testPathPrefix, UUID.randomUUID());
    mantaClient.put(path2, TEST_DATA);
  ...
  @Test public void canListFailedJobs() throws IOException, InterruptedException {
    String path = String.format("%s%s", testPathPrefix, UUID.randomUUID());
    mantaClient.put(path, TEST_DATA);
    ...
    Assert.assertTrue(mantaClient.endJobInput(jobId));
    awaitJobCompletion(jobId);
    List<String> failures = mantaClient.getJobFailures(jobId).collect(Collectors.toList());
    Assert.assertEquals(failures.size(), 1, "There should only be a single failure");
  }

  @Test public void canGetJobErrors() throws IOException, InterruptedException {
    String path1 = String.format("%s%s", testPathPrefix, UUID.randomUUID());
    mantaClient.put(path1, TEST_DATA);
    String path2 = String.format("%s%s", testPathPrefix, UUID.randomUUID());
    mantaClient.put(path2, TEST_DATA);
    ...
    Assert.assertTrue(mantaClient.endJobInput(jobId));
    awaitJobCompletion(jobId);
    List<MantaJobError> errors = mantaClient.getJobErrors(jobId).collect(Collectors.toList());
  ...
  private MantaJob buildJob(final String name, final String exec) {
    ...
    MantaJobPhase map = new MantaJobPhase().setType("map").setExec(exec);
    phases.add(map);
