 ...
 */
@Test(groups = { "directory" }) public class MantaClientDirectoriesIT {
  private static final Logger LOG = LoggerFactory.getLogger(MantaClientDirectoriesIT.class);

  private static final RandomStringGenerator STRING_GENERATOR = new RandomStringGenerator.Builder().withinRange((int) 'a', (int) 'z').build();
  ...
  @Test public void canCreateDirectory() throws IOException {
  ...
  @Test public void willReturnFalseWhenWeOverwriteDirectory() throws IOException {
    mantaClient.putDirectory(testPathPrefix);
    String dir = String.format("%s/%s", testPathPrefix, UUID.randomUUID());
    Assert.assertTrue(mantaClient.putDirectory(dir), "We were unable to create the initial directory");
    boolean result = mantaClient.putDirectory(dir);
    Assert.assertFalse(result, "Expected a false value because we " + "didn\'t create a new directory");
  }

  @Test(dependsOnMethods = { "canCreateDirectory" }) public void canDeleteDirectory() throws IOException {
    String dir = String.format("%s/%s", testPathPrefix, UUID.randomUUID());
    mantaClient.putDirectory(dir);
    MantaObject response = mantaClient.head(dir);
    Assert.assertEquals(dir, response.getPath());
    mantaClient.delete(dir);
    MantaAssert.assertResponseFailureStatusCode(404, RESOURCE_NOT_FOUND_ERROR, (MantaFunction<Object>) () -> mantaClient.get(dir));
  }

  @Test(dependsOnMethods = { "canCreateDirectory" }) public void wontErrorWhenWeCreateOverAnExistingDirectory() throws IOException {
    String dir = String.format("%s/%s", testPathPrefix, UUID.randomUUID());
    mantaClient.putDirectory(dir);
    mantaClient.putDirectory(dir);
    mantaClient.putDirectory(dir);
  ...
  /**
     ...
     */
  @Test(dependsOnMethods = { "canCreateDirectory" }) public void noErrorWhenWeOverwriteAnExistingFile() throws IOException {
    String dir = String.format("%s/%s", testPathPrefix, UUID.randomUUID());
    mantaClient.putDirectory(dir);
  ...
  @Test(dependsOnMethods = { "canCreateDirectory" }) public void directoryIsMarkedAsSuch() throws IOException {
    MantaObject dir = mantaClient.head(testPathPrefix);
    Assert.assertTrue(dir.isDirectory(), String.format("Directory should be marked as such [%s]. " + "\nResponse: %s", testPathPrefix, dir));
  }

  @Test(dependsOnMethods = { "wontErrorWhenWeCreateOverAnExistingDirectory" }) public void canRecursivelyCreateDirectory() throws IOException {
    String dir = String.format("%s/%s/%s/%s/%s/%s", testPathPrefix, UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID());
    mantaClient.putDirectory(dir, true);
    MantaObject response = mantaClient.head(dir);
    Assert.assertTrue(response.isDirectory(), String.format("Directory should be marked as such [%s]", testPathPrefix));
    Assert.assertEquals(dir, response.getPath());
  }

  @Test public void canCreateDirectoriesNormallyWhenNewDirectoryDepthLessThanSkipDepth() throws IOException {
    ...
    final long operations = RecursiveDirectoryCreationStrategy.createWithSkipDepth(mantaClient, childDirectory, null, config.getSkipDirectoryDepth());
    Assert.assertEquals(operations, childDirectoryDepth);
  }

  @Test public void canSkipAlreadyCreatedDirectoriesWhenDepthSetCorrectly() throws IOException {
    directorySkipping(true);
  }

  @Test public void canAddAtMostOneFailedOperationWhenDepthSetIncorrectly() throws IOException {
  ...
  private void directorySkipping(final boolean settingCorrectDepth) throws IOException {
    ...
    final int childAddedDepth = RandomUtils.nextInt(2, 5);
    ...
    for (int i = 0; i < childAddedDepth; i++) {
      childDirBuilder.append(SEPARATOR).append(STRING_GENERATOR.generate(3));
    }
    ...
    final long operations = RecursiveDirectoryCreationStrategy.createWithSkipDepth(mantaClient, childDir, null, config.getSkipDirectoryDepth());
    final MantaObject nestedResponse = mantaClient.head(childDir);
    Assert.assertTrue(nestedResponse.isDirectory(), String.format("Nested directory should be marked as such [%s]", testPathPrefix));
  ...
  /**
     ...
     * @throws IOException
     */
  @Test public void pruneParentDirectoriesFull() throws IOException {
  ...
  @Test public void pruneParentDirectoryZero() throws IOException {
    final String parentDir = createRandomDirectory(testPathPrefix, 1);
  ...
  @Test public void pruneParentDirectoryOne() throws IOException {
    final String parentDir = createRandomDirectory(testPathPrefix, 1);
  ...
  /**
     ...
     * @throws IOException
     */
  @Test(expectedExceptions = IllegalArgumentException.class) public void pruneParentDirectoriesInvalid() throws IOException {
    final String parentDir = createRandomDirectory(testPathPrefix, 1);
    final String childDir = createRandomDirectory(parentDir, 5);
  ...
  /**
     ...
     */
  @Test public void pruneParentDirectoriesWithFile() throws IOException {
    final String parentDir = createRandomDirectory(testPathPrefix, 1);
    final String childDir = createRandomDirectory(parentDir, 5);
    LOG.debug("CHILD DIR  : " + childDir);
    LOG.debug("Parent DIR : " + parentDir);
  ...
  /**
     ...
     * @throws IOException - when there is an error that is not accounted for.
     */
  @Test public void pruneParentDirectoriesFailingWithFile() throws IOException {
    final String parentDir = createRandomDirectory(testPathPrefix, 1);
    final String childDir = createRandomDirectory(parentDir, 5);
    LOG.debug("CHILD DIR  : " + childDir);
    LOG.debug("Parent DIR : " + parentDir);
  ...
  @Test public void pruneParentDirectoriesFailingGreaterThanDirPath() throws IOException {
    final String parentDir = createRandomDirectory(testPathPrefix, 1);
    final String childDir = createRandomDirectory(parentDir, 2);
