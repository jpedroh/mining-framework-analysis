...
public abstract class AbstractGemMojo extends AbstractJRubyMojo {
  ...
  private File setupNativeSupport() throws MojoExecutionException {
    ...
    if (!jrubyDir.exists()) {
      Artifact dist = manager.createArtifact("org.jruby", "jruby-dist", getJrubyVersion().toString(), "bin", "zip");
      try {
        manager.resolve(dist, localRepository, project.getRemoteArtifactRepositories());
      } catch (final GemException e) {
        throw new MojoExecutionException("could not setup jruby distribution for native support", e);
      }
      if (jrubyVerbose) {
  ...
  @Override protected void executeJRuby() throws MojoExecutionException, MojoFailureException, IOException, ScriptException {
    ...
    try {
      ...
      if (hasGems) {
        String base = this.gemsConfig.getGemHome() != null ? this.gemsConfig.getGemHome().getAbsolutePath() : (project.getBuild().getDirectory() + "/rubygems");
    ...
    if (this.includeRubygemsInTestResources) {
      for (File path : this.gemsConfig.getGemPath()) {
        if (jrubyVerbose) {
          getLog().info("add gems to test-classpath from: " + path.getAbsolutePath());
        }
    ...
    if (this.includeRubygemsInResources) {
      for (File path : this.gemsConfig.getGemPath()) {
        if (jrubyVerbose) {
          getLog().info("add gems to classpath from: " + path.getAbsolutePath());
        }
        Resource resource = new Resource();
        resource.setDirectory(path.getAbsolutePath());
        resource.addInclude("gems/**");
  ...
  abstract protected void executeWithGems() throws MojoExecutionException, ScriptException, GemException, IOException, MojoFailureException;
