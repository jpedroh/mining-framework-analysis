...
final class StorageImpl extends BaseService<StorageOptions> implements Storage {
  private static final Interceptor EXCEPTION_HANDLER_INTERCEPTOR = new Interceptor() {
    ...
    @Override public RetryResult afterEval(Exception exception, RetryResult retryResult) {
      return Interceptor.RetryResult.CONTINUE_EVALUATION;
    }

    @Override public RetryResult beforeEval(Exception exception) {
  ...
  static final ExceptionHandler EXCEPTION_HANDLER = ExceptionHandler.builder().abortOn(RuntimeException.class).interceptor(EXCEPTION_HANDLER_INTERCEPTOR).build();
  ...
  @Override public BucketInfo create(BucketInfo bucketInfo, BucketTargetOption... options) {
    ...
    return BucketInfo.fromPb(runWithRetries(new Callable<com.google.api.services.storage.model.Bucket>() {
  ...
  @Override public BlobInfo create(BlobInfo blobInfo, BlobTargetOption... options) {
    return create(blobInfo, new ByteArrayInputStream(EMPTY_BYTE_ARRAY), options);
  }

  @Override public BlobInfo create(BlobInfo blobInfo, final byte[] content, BlobTargetOption... options) {
    return create(blobInfo, new ByteArrayInputStream(firstNonNull(content, EMPTY_BYTE_ARRAY)), options);
  }

  @Override public BlobInfo create(BlobInfo blobInfo, final InputStream content, BlobTargetOption... options) {
  ...
  @Override public BucketInfo get(String bucket, BucketSourceOption... options) {
  ...
  @Override public BlobInfo get(String bucket, String blob, BlobSourceOption... options) {
  ...
  private static abstract class BasePageFetcher<T extends Serializable> implements BaseListResult.NextPageFetcher<T> {
  ...
  private static class BucketPageFetcher extends BasePageFetcher<BucketInfo> {
    ...
    @Override public ListResult<BucketInfo> nextPage() {
  ...
  private static class BlobPageFetcher extends BasePageFetcher<BlobInfo> {
    ...
    @Override public ListResult<BlobInfo> nextPage() {
      return listBlobs(bucket, serviceOptions, requestOptions);
    }
  }

  @Override public ListResult<BucketInfo> list(BucketListOption... options) {
    return listBuckets(options(), optionMap(options));
  }

  private static ListResult<BucketInfo> listBuckets(final StorageOptions serviceOptions, final Map<StorageRpc.Option, ?> optionsMap) {
    Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> result = runWithRetries(new Callable<Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>>>() {
      @Override public Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> call() {
        return serviceOptions.storageRpc().list(optionsMap);
      }
    }, serviceOptions.retryParams(), EXCEPTION_HANDLER);
    String cursor = result.x();
    Iterable<BucketInfo> buckets = result.y() == null ? ImmutableList.<BucketInfo>of() : Iterables.transform(result.y(), new Function<com.google.api.services.storage.model.Bucket, BucketInfo>() {
      @Override public BucketInfo apply(com.google.api.services.storage.model.Bucket bucketPb) {
        return BucketInfo.fromPb(bucketPb);
      }
    });
    return new BaseListResult<>(new BucketPageFetcher(serviceOptions, cursor, optionsMap), cursor, buckets);
  }

  @Override public ListResult<BlobInfo> list(final String bucket, BlobListOption... options) {
    return listBlobs(bucket, options(), optionMap(options));
  }

  private static ListResult<BlobInfo> listBlobs(final String bucket, final StorageOptions serviceOptions, final Map<StorageRpc.Option, ?> optionsMap) {
    Tuple<String, Iterable<StorageObject>> result = runWithRetries(new Callable<Tuple<String, Iterable<StorageObject>>>() {
      @Override public Tuple<String, Iterable<StorageObject>> call() {
        return serviceOptions.storageRpc().list(bucket, optionsMap);
      }
    }, serviceOptions.retryParams(), EXCEPTION_HANDLER);
    String cursor = result.x();
    Iterable<BlobInfo> blobs = result.y() == null ? ImmutableList.<BlobInfo>of() : Iterables.transform(result.y(), new Function<StorageObject, BlobInfo>() {
      @Override public BlobInfo apply(StorageObject storageObject) {
        return BlobInfo.fromPb(storageObject);
      }
    });
    return new BaseListResult<>(new BlobPageFetcher(bucket, serviceOptions, cursor, optionsMap), cursor, blobs);
  }

  @Override public BucketInfo update(BucketInfo bucketInfo, BucketTargetOption... options) {
    final com.google.api.services.storage.model.Bucket bucketPb = bucketInfo.toPb();
    final Map<StorageRpc.Option, ?> optionsMap = optionMap(bucketInfo, options);
    return BucketInfo.fromPb(runWithRetries(new Callable<com.google.api.services.storage.model.Bucket>() {
      @Override public com.google.api.services.storage.model.Bucket call() {
  ...
  @Override public BlobInfo update(BlobInfo blobInfo, BlobTargetOption... options) {
  ...
  @Override public boolean delete(String bucket, BucketSourceOption... options) {
    final com.google.api.services.storage.model.Bucket bucketPb = BucketInfo.of(bucket).toPb();
    final Map<StorageRpc.Option, ?> optionsMap = optionMap(options);
  ...
  @Override public boolean delete(String bucket, String blob, BlobSourceOption... options) {
  ...
  @Override public BlobInfo compose(final ComposeRequest composeRequest) {
    ...
    for (ComposeRequest.SourceBlob sourceBlob : composeRequest.sourceBlobs()) {
      sources.add(BlobInfo.builder(composeRequest.target().bucket(), sourceBlob.name()).generation(sourceBlob.generation()).build().toPb());
    }
  ...
  @Override public BlobInfo copy(CopyRequest copyRequest) {
    final StorageObject source = BlobInfo.of(copyRequest.sourceBucket(), copyRequest.sourceBlob()).toPb();
    copyRequest.sourceOptions();
    final Map<StorageRpc.Option, ?> sourceOptions = optionMap(null, null, copyRequest.sourceOptions(), true);
  ...
  @Override public byte[] readAllBytes(String bucket, String blob, BlobSourceOption... options) {
    final StorageObject storageObject = BlobInfo.of(bucket, blob).toPb();
    final Map<StorageRpc.Option, ?> optionsMap = optionMap(options);
  ...
  @Override public BatchResponse apply(BatchRequest batchRequest) {
    ...
    for (Map.Entry<BlobInfo, Iterable<BlobSourceOption>> entry : batchRequest.toDelete().entrySet()) {
      ...
      Map<StorageRpc.Option, ?> optionsMap = optionMap(blobInfo.generation(), blobInfo.metageneration(), entry.getValue());
    ...
    List<BatchResponse.Result<BlobInfo>> updates = transformBatchResult(toUpdate, response.updates, BlobInfo.FROM_PB_FUNCTION);
    List<BatchResponse.Result<BlobInfo>> gets = transformBatchResult(toGet, response.gets, BlobInfo.FROM_PB_FUNCTION, HTTP_NOT_FOUND);
    return new BatchResponse(deletes, updates, gets);
  }

  private <I extends java.lang.Object, O extends Serializable> List<BatchResponse.Result<O>> transformBatchResult(Iterable<Tuple<StorageObject, Map<StorageRpc.Option, ?>>> request, Map<StorageObject, Tuple<I, StorageException>> results, Function<I, O> transform, int... nullOnErrorCodes) {
  ...
  @Override public BlobReadChannel reader(String bucket, String blob, BlobSourceOption... options) {
  ...
  @Override public BlobWriteChannel writer(BlobInfo blobInfo, BlobTargetOption... options) {
    final Map<StorageRpc.Option, ?> optionsMap = optionMap(blobInfo, options);
    return new BlobWriteChannelImpl(options(), blobInfo, optionsMap);
  }

  @Override public URL signUrl(BlobInfo blobInfo, long expiration, SignUrlOption... options) {
    ...
    ServiceAccountAuthCredentials cred = (ServiceAccountAuthCredentials) optionMap.get(SignUrlOption.Option.SERVICE_ACCOUNT_CRED);
    if (cred == null) {
      checkArgument(options().authCredentials() instanceof ServiceAccountAuthCredentials, "Signing key was not provided and could not be derived");
  ...
  private static <T extends java.lang.Object> void addToOptionMap(StorageRpc.Option option, T defaultValue, Map<StorageRpc.Option, Object> map) {
    addToOptionMap(option, option, defaultValue, map);
  }

  private static <T extends java.lang.Object> void addToOptionMap(StorageRpc.Option getOption, StorageRpc.Option putOption, T defaultValue, Map<StorageRpc.Option, Object> map) {
    if (map.containsKey(getOption)) {
      @SuppressWarnings(value = { "unchecked" }) T value = (T) map.remove(getOption);
      checkArgument(value != null || defaultValue != null, "Option " + getOption.value() + " is missing a value");
