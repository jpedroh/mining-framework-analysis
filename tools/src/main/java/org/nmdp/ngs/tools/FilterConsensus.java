...
public final class FilterConsensus implements Runnable {
  ...
  /**
     ...
     */
  public FilterConsensus(final File inputBamFile, final File inputGenomicFile, final File outputFile, final String gene, final boolean cdna, final boolean removeGaps, final double minimumBreadth, final int expectedPloidy) {
  ...
  @Override public void run() {
    ...
    try {
      ...
      for (SAMRecord record : new SAMFileReader(inputBamFile)) {
        ...
        Allele contig = Allele.builder().withContig("chr6").withStart(start).withEnd(end).withSequence(sequence).build();
      ...
      if (cdna) {
        ...
        for (String contig : contigs.keySet()) {
          ...
          for (int index : contigs.get(contig).keySet()) {
            ...
            Collections.sort(list, new Comparator<Allele>() {
              @Override public int compare(final Allele first, final Allele second) {
        ...
        Collections.sort(list, new Comparator<Map.Entry<String, String>>() {
          @Override public int compare(final Map.Entry<String, String> first, final Map.Entry<String, String> second) {
    ...
    try {
      ...
      while (reader.ready()) {
        ...
        try {
          ...
          exons.put(exon, Allele.builder().withContig(locus.getContig()).withStart(locus.getMin()).withEnd(locus.getMax() + 1).build());
  ...
  public static void main(final String[] args) {
    ...
    try {
      ...
      new FilterConsensus(inputBamFile.getValue(), inputGenomicFile.getValue(), outputFile.getValue(), gene.getValue(), cdna.wasFound(), removeGaps.wasFound(), minimumBreadth.getValue(DEFAULT_MINIMUM_BREADTH), expectedPloidy.getValue(DEFAULT_EXPECTED_PLOIDY)).run();
