...
public class NSGAIIIRunner {
  ...
  public static void main(String[] args) throws JMetalException {
    ...
    crossover = new SBXCrossover.Builder().setDistributionIndex(30.0).setProbability(0.9).build();
    mutation = new PolynomialMutation.Builder().setDistributionIndex(20.0).setProbability(1.0 / problem.getNumberOfVariables()).build();
    selection = new BinaryTournamentSelection.Builder().build();
    algorithm = new BuilderNSGAIII(problem).setCrossoverOperator(crossover).setMutationOperator(mutation).setSelectionOperator(selection).setMaxIterations(250).build();
