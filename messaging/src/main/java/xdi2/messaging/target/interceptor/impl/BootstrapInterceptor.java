...
public class BootstrapInterceptor extends AbstractInterceptor implements Prototype<BootstrapInterceptor> {
  ...
  @Override public BootstrapInterceptor instanceFor(PrototypingContext prototypingContext) {
    ...
    if (prototypingContext.getOwnerPeerRoot() != null) {
      ...
      for (int i = 0; i < bootstrapOwnerSynonyms.length; i++) {
        bootstrapOwnerSynonyms[i] = ownerSynonymPeerRoots[i].getXriOfPeerRoot();
      }
    }
  ...
  @Override public void init(MessagingTarget messagingTarget) throws Exception {
    ...
    if (log.isDebugEnabled()) {
      log.debug("bootstrapOwner=" + this.getBootstrapOwner() + ", bootstrapOwnerSynonyms=" + Arrays.asList(this.getBootstrapOwnerSynonyms()) + ", bootstrapLinkContract=" + this.getBootstrapRootLinkContract() + ", bootstrapPublicLinkContract=" + this.getBootstrapPublicLinkContract() + ", bootstrapGraph=" + (this.getBootstrapGraph() != null) + ", bootstrapMessageEnvelope=" + (this.getBootstrapMessageEnvelope() != null));
    }
    if (XdiLocalRoot.findLocalRoot(graph).getSelfPeerRoot() != null) {
      return;
    }
    ...
    if (this.getBootstrapOwner() != null) {
      if (log.isDebugEnabled()) {
        log.debug("Creating bootstrap owner: " + this.getBootstrapOwner());
      }
      ...
      if (this.getBootstrapOwnerSynonyms() != null) {
        if (log.isDebugEnabled()) {
          log.debug("Creating bootstrap owner synonyms: " + Arrays.asList(this.getBootstrapOwnerSynonyms()));
        }
    ...
    if (this.getBootstrapPublicLinkContract()) {
      ...
      XDI3Statement 
<<<<<<< /usr/src/app/output/projectdanube/xdi2/41c3558dfc19ea135b2b6d3e8c482e75a5350d10/messaging/src/main/java/xdi2/messaging/target/interceptor/impl/BootstrapInterceptor.java/left.java
      bootstrapOwnerSynonymIsRefStatement = XDI3Statement.fromRelationComponents(this.getBootstrapOwner(), XDIDictionaryConstants.XRI_S_IS_REF, XDIConstants.XRI_S_VARIABLE)
=======
      bootstrapOwnerSynonymsIsRefStatement = XDI3Statement.fromRelationComponents(this.getBootstrapOwner(), XDIDictionaryConstants.XRI_S_IS_REF, XDIConstants.XRI_S_VARIABLE)
>>>>>>> /usr/src/app/output/projectdanube/xdi2/41c3558dfc19ea135b2b6d3e8c482e75a5350d10/messaging/src/main/java/xdi2/messaging/target/interceptor/impl/BootstrapInterceptor.java/right.java
      ;
      bootstrapPublicLinkContract.setPermissionTargetStatement(XDILinkContractConstants.XRI_S_GET, 
<<<<<<< /usr/src/app/output/projectdanube/xdi2/41c3558dfc19ea135b2b6d3e8c482e75a5350d10/messaging/src/main/java/xdi2/messaging/target/interceptor/impl/BootstrapInterceptor.java/left.java
      bootstrapOwnerSynonymIsRefStatement
=======
      bootstrapOwnerSynonymsIsRefStatement
>>>>>>> /usr/src/app/output/projectdanube/xdi2/41c3558dfc19ea135b2b6d3e8c482e75a5350d10/messaging/src/main/java/xdi2/messaging/target/interceptor/impl/BootstrapInterceptor.java/right.java
      );
      for (XDI3Segment bootstrapOwnerSynonym : this.getBootstrapOwnerSynonyms()) {
    ...
    if (this.getBootstrapGraph() != null) {
      ...
      if (log.isDebugEnabled()) {
        log.debug("Creating bootstrap graph: " + bootstrapGraph.toString());
      }
    ...
    if (this.getBootstrapMessageEnvelope() != null) {
      ...
      if (log.isDebugEnabled()) {
        log.debug("Executing bootstrap message envelope: " + bootstrapMessageEnvelope.getGraph().toString());
      }
      ...
      try {
        ...
        if (toInterceptor != null) {
          toInterceptor.setEnabled(false);
        }
        ...
        if (refInterceptor != null) {
          refInterceptor.setEnabled(false);
        }
        ...
        if (toInterceptor != null && toInterceptorEnabled != null) {
          toInterceptor.setEnabled(toInterceptorEnabled.booleanValue());
        }
        if (refInterceptor != null && refInterceptorEnabled != null) {
          refInterceptor.setEnabled(refInterceptorEnabled.booleanValue());
        }
        if (linkContractInterceptor != null && linkContractInterceptorEnabled != null) {
          linkContractInterceptor.setEnabled(linkContractInterceptorEnabled.booleanValue());
        }
      }
    }
  }

  @Override public void shutdown(MessagingTarget messagingTarget) throws Exception {
    super.shutdown(messagingTarget);
  }

  private final CopyStrategy replaceSelfVariableCopyStrategy = new CopyStrategy() {
    @Override public ContextNode replaceContextNode(ContextNode contextNode) {
      XDI3Segment contextNodeXri = contextNode.getXri();
      XDI3SubSegment contextNodeArcXri = contextNode.getArcXri();
      XDI3Segment replacedContextNodeXri = XDI3Util.concatXris(XDI3Util.parentXri(contextNodeXri, -1), XDI3Util.replaceXri(XDI3Segment.fromComponent(contextNodeArcXri), XRI_SS_SELF, BootstrapInterceptor.this.getBootstrapOwner(), true, true, true));
      if (log.isDebugEnabled()) {
        log.debug("Replaced " + contextNodeXri + " with " + replacedContextNodeXri);
      }
      if (contextNodeXri.equals(replacedContextNodeXri)) {
    ...
    @Override public Relation replaceRelation(Relation relation) {
      ...
      XDI3Segment replacedTargetContextNodeXri = XDI3Util.replaceXri(targetContextNodeXri, XRI_SS_SELF, BootstrapInterceptor.this.getBootstrapOwner(), true, true, true);
      if (log.isDebugEnabled()) {
        log.debug("Replaced " + targetContextNodeXri + " with " + replacedTargetContextNodeXri);
      }
      if (targetContextNodeXri.equals(replacedTargetContextNodeXri)) {
        return super.replaceRelation(relation);
      }
  ...
  public void setBootstrapOwnerSynonyms(String[] bootstrapOwnerSynonyms) {
    ...
    for (int i = 0; i < this.bootstrapOwnerSynonyms.length; i++) {
      this.bootstrapOwnerSynonyms[i] = XDI3Segment.create(bootstrapOwnerSynonyms[i]);
    }
  }
