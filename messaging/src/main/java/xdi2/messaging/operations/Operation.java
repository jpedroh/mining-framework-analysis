...
public abstract class Operation implements Serializable, Comparable<Operation> {
  ...
  public static boolean isValid(Relation relation) {
    return GetOperation.isValid(relation) || SetOperation.isValid(relation) || DelOperation.isValid(relation) || DoOperation.isValid(relation) || ConnectOperation.isValid(relation) || SendOperation.isValid(relation) || PushOperation.isValid(relation);
  }
  ...
  public static Operation fromMessageAndRelation(Message message, Relation relation) {
    ...
    if (ConnectOperation.isValid(relation)) {
      return new ConnectOperation(message, relation);
    }
    if (SendOperation.isValid(relation)) {
      return new SendOperation(message, relation);
    }
    if (PushOperation.isValid(relation)) {
  ...
  public XdiInnerRoot getTargetXdiInnerRoot() {
    ...
    if (!xdiInnerRoot.getSubjectOfInnerRoot().equals(this.getRelation().getContextNode().getXDIAddress())) {
      return null;
    }
    if (!xdiInnerRoot.getPredicateOfInnerRoot().equals(this.getRelation().getXDIAddress())) {
      return null;
    }
    return xdiInnerRoot;
  }
  ...
  public Iterator<XDIStatement> getTargetXDIStatements() {
    XdiInnerRoot targetInnerRoot = this.getTargetXdiInnerRoot();
    if (targetInnerRoot != null) {
      return new MappingAbsoluteToRelativeXDIStatementIterator(targetInnerRoot, new MappingXDIStatementIterator(new SelectingNotImpliedStatementIterator(targetInnerRoot.getContextNode().getAllStatements())));
    } else {
      return null;
    }
  }
  ...
  public boolean isReadOnlyOperation() {
    if (this instanceof GetOperation) {
      return true;
    }
    return false;
  }
  ...
  public Map<XDIArc, XDIAddress> getVariableValues() {
    ...
    if (variableValuesXdiEntity == null) {
    ...
    for (XdiVariable<?> xdiVariable : xdiVariablesIterator) {
      ...
      if (log.isDebugEnabled()) {
        log.debug("Variable value for " + this.getOperationXDIAddress() + " operation: " + variableValueXDIArc + " --> " + variableValueXDIAddress);
      }
      if (variableValueXDIArc == null || variableValueXDIAddress == null) {
        continue;
      }
  ...
  @Override public int compareTo(Operation other) {
