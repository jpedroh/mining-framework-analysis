...
public class ObjectDefinitionProcessor implements StructNodeProcessor {
  ...
  private void addMultiObjectsToSpec(StructNode objectNode, String objectName, Locator locator, List<String> groupsForThisObject) {
    ...
    for (int index = 1; index <= count; index++) {
      addObjectToSpec(objectNode, objectName.replace("*", Integer.toString(index)), new Locator(locator.getLocatorType(), locator.getLocatorValue(), index).withParent(locator.getParent()).withCorrections(locator.getCorrections()), groupsForThisObject);
    }
  }
  ...
  private String expectCorrectionsOrId(StructNode structNode, StringCharReader reader, String objectName) {
    ...
    if (word.isEmpty()) {
      throw new SyntaxException(structNode.getPlace(), format("Missing locator for object \"%s\"", objectName));
    }
    return word;
  }
