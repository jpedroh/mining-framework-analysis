...
public class GalenActionCheck extends GalenAction {
  ...
  @Override public void execute() throws IOException {
    ...
    for (String pageSpecPath : checkArguments.getPaths()) {
      ...
      test.setPageTests(asList(new GalenPageTest().withTitle("Simple check").withUrl(checkArguments.getUrl()).withSize(checkArguments.getScreenSize()).withBrowserFactory(new SeleniumBrowserFactory()).withActions(asList((GalenPageAction) new GalenPageActionCheck().withSpec(pageSpecPath).withIncludedTags(checkArguments.getIncludedTags()).withExcludedTags(checkArguments.getExcludedTags()).withOriginalCommand(originalCommand(arguments))))));
      galenTests.add(test);
    }
