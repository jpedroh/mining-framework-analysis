...
public final class ReactorServiceInstancesV3Test extends AbstractClientApiTest {
  ...
  @Test public void share() {
    ...
    this.serviceInstances.share(ShareServiceInstanceRequest.builder().serviceInstanceId("test-service-instance-id").data(Relationship.builder().id("space-guid-1").build()).data(Relationship.builder().id("space-guid-2").build()).build()).as(StepVerifier::create).expectNext(ShareServiceInstanceResponse.builder().data(Relationship.builder().id("68d54d31-9b3a-463b-ba94-e8e4c32edbac").build()).data(Relationship.builder().id("b19f6525-cbd3-4155-b156-dc0c2a431b4c").build()).link("self", Link.builder().href("https://api.example.org/v3/service_instances/bdeg4371-cbd3-4155-b156-dc0c2a431b4c/relationships/shared_spaces").build()).build()).expectComplete().verify(Duration.ofSeconds(5));
  }

  @Test public void unshare() {
    mockRequest(InteractionContext.builder().request(TestRequest.builder().method(DELETE).path("/service_instances/test-service-instance-id/relationships/shared_spaces/test-space-id").build()).response(TestResponse.builder().status(NO_CONTENT).build()).build());
    this.serviceInstances.unshare(UnshareServiceInstanceRequest.builder().serviceInstanceId("test-service-instance-id").spaceId("test-space-id").build()).as(StepVerifier::create).expectComplete().verify(Duration.ofSeconds(5));
  }

  @Test public void update() {
    mockRequest(InteractionContext.builder().request(TestRequest.builder().method(PATCH).path("/service_instances/68d54d31-9b3a-463b-ba94-e8e4c32edbac").payload("fixtures/client/v3/serviceinstances/PATCH_request.json").build()).response(TestResponse.builder().status(OK).payload("fixtures/client/v3/serviceinstances/PATCH_response.json").build()).build());
    this.serviceInstances.update(UpdateServiceInstanceRequest.builder().serviceInstanceId("68d54d31-9b3a-463b-ba94-e8e4c32edbac").metadata(Metadata.builder().annotation("note", "detailed information").label("key", "value").build()).build()).as(StepVerifier::create).expectNext(UpdateServiceInstanceResponse.builder().createdAt("2017-11-17T13:54:21Z").id("85ccdcad-d725-4109-bca4-fd6ba062b5c8").link("space", Link.builder().href("https://api.example.org/v3/spaces/ae0031f9-dd49-461c-a945-df40e77c39cb").build()).metadata(Metadata.builder().annotation("note", "detailed information").label("key", "value").build()).name("my_service_instance").relationships(ServiceInstanceRelationships.builder().space(ToOneRelationship.builder().data(Relationship.builder().id("ae0031f9-dd49-461c-a945-df40e77c39cb").build()).build()).build()).updatedAt("2017-11-17T13:54:21Z").build()).expectComplete().verify(Duration.ofSeconds(5));
  }
}