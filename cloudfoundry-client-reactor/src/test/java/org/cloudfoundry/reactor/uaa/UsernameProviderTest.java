...
public final class UsernameProviderTest {
  ...
  @SuppressWarnings(value = { "unchecked" }) @Test public void getInvalidToken() throws NoSuchAlgorithmException {
    ...
    this.usernameProvider.get().as(StepVerifier::create).expectNext("test-username").expectComplete().verify(Duration.ofSeconds(1));
    verify(this.tokenProvider).invalidate(this.connectionContext);
  }

  @Test public void getValidToken() throws NoSuchAlgorithmException {
    KeyPair keyPair = getKeyPair();
    when(this.signingKeyResolver.resolveSigningKey(any(JwsHeader.class), any(Claims.class))).thenReturn(keyPair.getPublic());
  ...
  @SuppressWarnings(value = { "unchecked" }) private static String getToken(PrivateKey privateKey, Instant expiration) {
    return Jwts.builder().setHeader((Map<String, Object>) new DefaultJwsHeader().setKeyId("test-key")).signWith(SignatureAlgorithm.RS256, privateKey).claim("user_name", "test-username").setExpiration(Date.from(expiration)).compact();
  }
