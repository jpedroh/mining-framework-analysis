...
public final class ErrorPayloadMappersTest {
  ...
  @Test public void clientV2NoError() {
    ...
    Flux.just(responseWithBody).transform(ErrorPayloadMappers.clientV2(this.objectMapper)).as(StepVerifier::create).expectNext(responseWithBody).expectComplete().verify(Duration.ofSeconds(1));
  }
  ...
  @Test public void clientV3ClientError() throws IOException {
    ...
    Flux.just(responseWithBody).transform(ErrorPayloadMappers.clientV3(this.objectMapper)).as(StepVerifier::create).consumeErrorWith((t) -> {
  ...
  @Test public void clientV3NoError() {
    when(this.response.status()).thenReturn(OK);
    HttpClientResponseWithBody responseWithBody = buildResponseWithBody();
    Flux.just(responseWithBody).transform(ErrorPayloadMappers.clientV3(this.objectMapper)).as(StepVerifier::create).expectNext(responseWithBody).expectComplete().verify(Duration.ofSeconds(1));
  }
  ...
  @Test public void uaaNoError() {
    when(this.response.status()).thenReturn(OK);
    HttpClientResponseWithBody responseWithBody = buildResponseWithBody();
    Flux.just(responseWithBody).transform(ErrorPayloadMappers.uaa(this.objectMapper)).as(StepVerifier::create).expectNext(responseWithBody).expectComplete().verify(Duration.ofSeconds(1));
  }
