...
<<<<<<< /usr/src/app/output/cloudfoundry/cf-java-client/be097d58609b1a40124d79fe34cb11947357c419/cloudfoundry-client-reactor/src/main/java/org/cloudfoundry/reactor/_DefaultConnectionContext.java/left.java
      loopResources(getThreadPool())
=======
      option(ALLOCATOR, getAllocator())
>>>>>>> /usr/src/app/output/cloudfoundry/cf-java-client/be097d58609b1a40124d79fe34cb11947357c419/cloudfoundry-client-reactor/src/main/java/org/cloudfoundry/reactor/_DefaultConnectionContext.java/right.java
      .option(SO_SNDBUF, SEND_BUFFER_SIZE).option(SO_RCVBUF, RECEIVE_BUFFER_SIZE).disablePool();
  ...
  @Override @Value.Default public ObjectMapper getObjectMapper() {
    ObjectMapper objectMapper = new ObjectMapper().disable(FAIL_ON_UNKNOWN_PROPERTIES).enable(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY).registerModule(new Jdk8Module()).setSerializationInclusion(NON_NULL);
  ...
  @Value.Derived ByteBufAllocator getAllocator() {
  ...
  /**
     ...
     */
  @Nullable abstract String getApiHost();
  ...
  @Value.Derived Optional<PoolResources> getConnectionPool() {
    return Optional.ofNullable(getConnectionPoolSize()).map((connectionPoolSize) -> PoolResources.fixed("cloudfoundry-client", connectionPoolSize));
  }
