 ...
 */
@Value.Immutable abstract class _DefaultConnectionContext implements ConnectionContext {
  ...
  @Override public abstract Optional<Duration> getCacheDuration();
  ...
  /**
     ...
     */
  @Nullable @Value.Default public Integer getConnectionPoolSize() {
    return ConnectionProvider.DEFAULT_POOL_MAX_CONNECTIONS;
  }

  @Override @Value.Default public HttpClient getHttpClient() {
    HttpClient client = createHttpClient().compress(true).secure(this::configureSsl).tcpConfiguration(this::configureTcpClient);
    return getAdditionalHttpClientConfiguration().map((configuration) -> configuration.apply(client)).orElse(client);
  }

  @Override @Value.Default public Long getInvalidTokenRetries() {
    return 5L;
  }

  @Override @Value.Default public ObjectMapper getObjectMapper() {
    ObjectMapper objectMapper = new ObjectMapper().disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES).enable(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY).registerModule(new Jdk8Module()).setSerializationInclusion(NON_NULL);
    getProblemHandlers().forEach(objectMapper::addHandler);
    return objectMapper;
  }

  @Override @Value.Default public RootProvider getRootProvider() {
  ...
  /**
     ...
     */
  @Value.Default public Integer getThreadPoolSize() {
    return LoopResources.DEFAULT_IO_WORKER_COUNT;
  }

  @Override public Mono<Void> trust(String host, int port) {
    return getSslCertificateTruster().map((t) -> t.trust(host, port, Duration.ofSeconds(30))).orElse(Mono.empty());
  }
  ...
  @Value.Derived Optional<ConnectionProvider> getConnectionProvider() {
    ConnectionProvider.Builder builder = ConnectionProvider.builder("cloudfoundry-client");
    return Optional.ofNullable(getConnectionPoolSize()).map((connectionPoolSize) -> builder.maxConnections(connectionPoolSize).pendingAcquireMaxCount(-1).build());
  }
  ...
  @Value.Derived Optional<SslCertificateTruster> getSslCertificateTruster() {
  ...
  @Value.Derived LoopResources getThreadPool() {
    return LoopResources.create("cloudfoundry-client", getThreadPoolSize(), true);
  }

  @PostConstruct void monitorByteBufAllocator() {
    try {
      ObjectName name = getByteBufAllocatorObjectName();
      if (ManagementFactory.getPlatformMBeanServer().isRegistered(name)) {
  ...
  private TcpClient configureConnectTimeout(TcpClient tcpClient) {
    return getConnectTimeout().map((connectTimeout) -> tcpClient.option(CONNECT_TIMEOUT_MILLIS, (int) connectTimeout.toMillis())).orElse(tcpClient);
  }

  private TcpClient configureKeepAlive(TcpClient tcpClient) {
    return getKeepAlive().map((keepAlive) -> tcpClient.option(SO_KEEPALIVE, keepAlive)).orElse(tcpClient);
  }

  private TcpClient configureProxy(TcpClient tcpClient) {
    return getProxyConfiguration().map((proxyConfiguration) -> proxyConfiguration.configure(tcpClient)).orElse(tcpClient);
  }
  ...
  private TcpClient configureTcpClient(TcpClient tcpClient) {
    ...
    tcpClient = tcpClient.runOn(getThreadPool()).option(SO_SNDBUF, SEND_RECEIVE_BUFFER_SIZE).option(SO_RCVBUF, SEND_RECEIVE_BUFFER_SIZE);
  ...
  private HttpClient createHttpClient() {
    return getConnectionProvider().map(HttpClient::create).orElse(HttpClient.create());
  }

  private SslContextBuilder createSslContextBuilder() {
    SslContextBuilder sslContextBuilder = SslContextBuilder.forClient();
    getSslCertificateTruster().map(this::createTrustManagerFactory).ifPresent(sslContextBuilder::trustManager);
