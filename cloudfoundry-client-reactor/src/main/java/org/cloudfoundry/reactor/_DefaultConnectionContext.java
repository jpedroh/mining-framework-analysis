...
@Value.Immutable abstract class _DefaultConnectionContext implements ConnectionContext {
  ...
  @Override @Value.Default public HttpClient getHttpClient() {
    HttpClient client = createHttpClient().compress(true).secure(this::configureSsl);
    return getAdditionalHttpClientConfiguration().map((configuration) -> configuration.apply(configureHttpClient(client))).orElse(client);
  }
  ...
  private HttpClient configureConnectTimeout(HttpClient client) {
    return getConnectTimeout().map((connectTimeout) -> client.option(CONNECT_TIMEOUT_MILLIS, (int) connectTimeout.toMillis())).orElse(client);
  }

  private HttpClient configureHttpClient(HttpClient client) {
    client = configureProxy(client);
    client = client.runOn(getThreadPool()).option(SO_SNDBUF, SEND_RECEIVE_BUFFER_SIZE).option(SO_RCVBUF, SEND_RECEIVE_BUFFER_SIZE);
  ...
  private HttpClient configureKeepAlive(HttpClient client) {
    return getKeepAlive().map((keepAlive) -> client.option(SO_KEEPALIVE, keepAlive)).orElse(client);
  }

  private HttpClient configureProxy(HttpClient client) {
    return getProxyConfiguration().map((proxyConfiguration) -> proxyConfiguration.configure(client)).orElse(client);
  }
