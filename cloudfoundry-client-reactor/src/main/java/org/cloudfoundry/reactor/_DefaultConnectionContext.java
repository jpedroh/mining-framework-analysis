...
@Value.Immutable abstract class _DefaultConnectionContext implements ConnectionContext {
  ...
  @Override @Value.Default public HttpClient getHttpClient() {
    return HttpClient.create((options) -> {
      options.compression(true).loopResources(getThreadPool()).option(ALLOCATOR, getAllocator()).option(SO_SNDBUF, SEND_BUFFER_SIZE).option(SO_RCVBUF, RECEIVE_BUFFER_SIZE).disablePool();
  ...
  @Value.Derived ByteBufAllocator getAllocator() {
    ...
    try {
      ManagementFactory.getPlatformMBeanServer().registerMBean(new ByteBufAllocatorMetricProviderWrapper(byteBufAllocator), ObjectName.getInstance("org.cloudfoundry.reactor:type=ByteBufAllocator"));
    } catch (JMException e) {
