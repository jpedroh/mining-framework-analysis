...
public abstract class AbstractUaaTokenProvider implements TokenProvider {
  ...
  /**
     ...
     */
  @Value.Default public String getClientSecret() {
    return "";
  }
  ...
  @Override public final Mono<String> getToken(ConnectionContext connectionContext) {
    return this.accessTokens.computeIfAbsent(connectionContext, this::token);
  }

  @Override public void invalidate(ConnectionContext connectionContext) {
  ...
  private Mono<String> refreshToken(ConnectionContext connectionContext, String refreshToken) {
    return requestToken(connectionContext, refreshTokenGrantTokenRequestTransformer(refreshToken), tokensExtractor(connectionContext)).onErrorResume((t) -> t instanceof UaaException && ((UaaException) t).getStatusCode() == HttpResponseStatus.UNAUTHORIZED.code(), (t) -> Mono.empty());
  }

  private BiConsumer<HttpClientRequest, HttpClientForm> refreshTokenGrantTokenRequestTransformer(String refreshToken) {
    return (request, form) -> form.multipart(false).attr("client_id", getClientId()).attr("client_secret", getClientSecret()).attr("grant_type", "refresh_token").attr("refresh_token", refreshToken);
  }
  ...
  private Mono<String> requestToken(ConnectionContext connectionContext, BiConsumer<HttpClientRequest, HttpClientForm> tokenRequestTransformer, Function<ByteBufFlux, Mono<String>> tokenExtractor) {
    return connectionContext.getRootProvider().getRoot(AUTHORIZATION_ENDPOINT, connectionContext).map((root) -> createOperator(connectionContext, root)).flatMap((operator) -> operator.headers(this::addHeaders).post().uri(tokenUriTransformer(getIdentityZoneSubdomain())).sendForm(tokenRequestTransformer).response().parseBodyToToken((responseWithBody) -> tokenExtractor.apply(responseWithBody.getBody())));
  }
  ...
  private Mono<String> token(ConnectionContext connectionContext) {
    Mono<String> cached = this.refreshTokens.getOrDefault(connectionContext, Mono.empty()).flatMap((refreshToken) -> refreshToken(connectionContext, refreshToken).doOnSubscribe((s) -> LOGGER.debug("Negotiating using refresh token"))).switchIfEmpty(primaryToken(connectionContext).doOnSubscribe((s) -> LOGGER.debug("Negotiating using token provider")));
    return connectionContext.getCacheDuration().map(cached::cache).orElseGet(cached::cache).checkpoint();
  }

  @SuppressWarnings(value = { "unchecked" }) private Function<ByteBufFlux, Mono<String>> tokensExtractor(ConnectionContext connectionContext) {
    return (body) -> JsonCodec.decode(connectionContext.getObjectMapper(), body, Map.class).map((payload) -> (Map<String, String>) payload).doOnNext(extractRefreshToken(connectionContext)).map(AbstractUaaTokenProvider::extractAccessToken);
  }
