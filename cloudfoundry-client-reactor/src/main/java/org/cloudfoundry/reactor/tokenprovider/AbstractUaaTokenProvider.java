...
public abstract class AbstractUaaTokenProvider implements TokenProvider {
  ...
  private static HttpClientRequest addContentType(HttpClientRequest request) {
    return request.header(CONTENT_TYPE, APPLICATION_X_WWW_FORM_URLENCODED);
  }
  ...
  private static String getTokenUri(String root, String identityZoneId) {
    ...
    return builder.pathSegment("oauth", "token").build().encode().toUriString();
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private Function<Mono<HttpClientResponse>, Mono<String>> extractTokens(ConnectionContext connectionContext) {
    return (inbound) -> inbound.transform(JsonCodec.decode(connectionContext.getObjectMapper(), Map.class)).map((payload) -> (Map<String, String>) payload).doOnNext(extractRefreshToken(connectionContext)).map(AbstractUaaTokenProvider::extractAccessToken);
  }
  ...
  private Mono<HttpClientResponse> refreshToken(ConnectionContext connectionContext, String refreshToken) {
    return requestToken(connectionContext, refreshTokenGrantTokenRequestTransformer(refreshToken)).onErrorResume((t) -> t instanceof UaaException && ((UaaException) t).getStatusCode() == UNAUTHORIZED.code(), (t) -> Mono.empty());
  }

  private Function<Mono<HttpClientRequest>, Mono<Void>> refreshTokenGrantTokenRequestTransformer(String refreshToken) {
    return (outbound) -> outbound.flatMap((request) -> request.sendForm((form) -> form.multipart(false).attr("client_id", getClientId()).attr("client_secret", getClientSecret()).attr("grant_type", "refresh_token").attr("refresh_token", refreshToken)).then());
  }
