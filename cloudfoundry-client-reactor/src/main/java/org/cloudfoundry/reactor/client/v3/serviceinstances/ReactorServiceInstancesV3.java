...
public final class ReactorServiceInstancesV3 extends AbstractClientV3Operations implements ServiceInstancesV3 {
  ...
  @Override public Mono<ListServiceInstancesResponse> list(ListServiceInstancesRequest request) {
    return get(request, ListServiceInstancesResponse.class, (builder) -> builder.pathSegment("service_instances")).checkpoint();
  }

  @Override public Mono<ListSharedSpacesRelationshipResponse> listSharedSpacesRelationship(ListSharedSpacesRelationshipRequest request) {
    return get(request, ListSharedSpacesRelationshipResponse.class, (builder) -> builder.pathSegment("service_instances", request.getServiceInstanceId(), "relationships", "shared_spaces")).checkpoint();
  }

  @Override public Mono<ShareServiceInstanceResponse> share(ShareServiceInstanceRequest request) {
    return post(request, ShareServiceInstanceResponse.class, (builder) -> builder.pathSegment("service_instances", request.getServiceInstanceId(), "relationships", "shared_spaces")).checkpoint();
  }

  @Override public Mono<Void> unshare(UnshareServiceInstanceRequest request) {
    return delete(request, Void.class, (builder) -> builder.pathSegment("service_instances", request.getServiceInstanceId(), "relationships", "shared_spaces", request.getSpaceId())).checkpoint();
  }

  @Override public Mono<UpdateServiceInstanceResponse> update(UpdateServiceInstanceRequest request) {
