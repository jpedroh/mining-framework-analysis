...
public final class ErrorPayloadMapper {
  @SuppressWarnings(value = { "unchecked" }) public static Function<Mono<HttpClientResponse>, Mono<HttpClientResponse>> clientV2(ObjectMapper objectMapper) {
  ...
  @SuppressWarnings(value = { "unchecked" }) public static Function<Mono<HttpClientResponse>, Mono<HttpClientResponse>> clientV3(ObjectMapper objectMapper) {
    return (inbound) -> inbound.flatMap(mapToError((statusCode, payload) -> {
  ...
  public static Function<Mono<HttpClientResponse>, Mono<HttpClientResponse>> fallback() {
    return (inbound) -> inbound.flatMap((response) -> {
      ...
      return response.receive().aggregate().asString().flatMap((payload) -> Mono.error(new UnknownCloudFoundryException(response.status().code(), payload)));
    });
  }

  @SuppressWarnings(value = { "unchecked" }) public static Function<Mono<HttpClientResponse>, Mono<HttpClientResponse>> uaa(ObjectMapper objectMapper) {
    return (inbound) -> inbound.flatMap(mapToError((statusCode, payload) -> {
      Map<String, Object> map = objectMapper.readValue(payload, Map.class);
  ...
  private static Function<HttpClientResponse, Mono<HttpClientResponse>> mapToError(ExceptionGenerator exceptionGenerator) {
    return (response) -> {
      ...
      return response.receive().aggregate().asString().switchIfEmpty(Mono.error(new UnknownCloudFoundryException(response.status().code()))).flatMap((payload) -> {
        try {
  ...
  @FunctionalInterface private interface ExceptionGenerator {
