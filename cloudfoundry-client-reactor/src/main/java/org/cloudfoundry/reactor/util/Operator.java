...
public class Operator extends OperatorContextAware {
  ...
  public static class ResponseReceiver extends OperatorContextAware {
    ...
    public Mono<HttpClientResponse> get() {
      return this.responseReceiver.response((resp, body) -> Mono.just(HttpClientResponseWithBody.of(body, resp))).transform(this::processResponse).map(HttpClientResponseWithBody::getResponse).singleOrEmpty();
    }

    public <T extends java.lang.Object> Mono<T> parseBody(Class<T> bodyType) {
    ...
    public <T extends java.lang.Object> Flux<T> parseBodyToFlux(Function<HttpClientResponseWithBody, Publisher<T>> responseTransformer) {
      ...
      }).transform(this::processResponse).flatMap(responseTransformer);
    }

    public <T extends java.lang.Object> Mono<T> parseBodyToMono(Function<HttpClientResponseWithBody, Publisher<T>> responseTransformer) {
    ...
    private <T extends java.lang.Object> Mono<T> deserialized(ByteBufFlux body, Class<T> bodyType) {
    ...
    private Flux<HttpClientResponseWithBody> processResponse(Flux<HttpClientResponseWithBody> inbound) {
      return inbound.transform(this::invalidateToken).retry(this.context.getConnectionContext().getInvalidTokenRetries(), (t) -> t instanceof InvalidTokenException).transform(this.context.getErrorPayloadMapper().orElse(ErrorPayloadMappers.fallback()));
    }
