...
public class Operator extends OperatorContextAware {
  ...
  public static class ResponseReceiver extends OperatorContextAware {
    ...
    private Flux<HttpClientResponseWithConnection> processResponse(Flux<HttpClientResponseWithConnection> inbound) {
      return inbound.transform(this::invalidateToken).retryWhen(Retry.max(this.context.getConnectionContext().getInvalidTokenRetries()).filter(InvalidTokenException.class::isInstance)).transform(this.context.getErrorPayloadMapper().orElse(ErrorPayloadMappers.fallback()));
    }
