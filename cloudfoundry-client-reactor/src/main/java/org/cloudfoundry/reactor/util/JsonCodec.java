...
public final class JsonCodec {
  ...
  public static HttpClientRequest addDecodeHeaders(HttpClientRequest request) {
    return request.header(HttpHeaderNames.ACCEPT, HttpHeaderValues.APPLICATION_JSON);
  }

  public static <T extends java.lang.Object> Function<Mono<HttpClientResponse>, Flux<T>> decode(ObjectMapper objectMapper, Class<T> responseType) {
    return (inbound) -> inbound.flatMapMany((response) -> response.addHandler(new JsonObjectDecoder(MAX_PAYLOAD_SIZE)).receive().asByteArray().map((payload) -> {
      try {
  ...
  static Function<Mono<HttpClientRequest>, Publisher<Void>> encode(ObjectMapper objectMapper, Object requestPayload) {
    ...
    return (outbound) -> outbound.flatMapMany((request) -> {
      try {
        ...
        return request.header(HttpHeaderNames.CONTENT_TYPE, HttpHeaderValues.APPLICATION_JSON).header(HttpHeaderNames.CONTENT_LENGTH, String.valueOf(bytes.length)).sendByteArray(Mono.just(bytes));
