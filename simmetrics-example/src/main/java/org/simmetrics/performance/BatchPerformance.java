...
public class BatchPerformance {
  ...
  private static void testCompareUncached() {
    StringMetric metric = with(new SimonWhite<String>()).tokenize(new Whitespace()).tokenize(new QGram(2)).build();
    Stopwatch sw = Stopwatch.createStarted();
    for (int n = 0; n < TEST_REPEATS; n++) {
      @SuppressWarnings(value = { "unused" }) final float[] results = compare(metric, name, names);
    }
    sw.stop();
  ...
  private static void testCompareCached() {
    StringMetric metric = with(new SimonWhite<String>()).tokenize(new Whitespace()).tokenize(new QGram(2)).tokenizerCache().build();
    Stopwatch sw = Stopwatch.createStarted();
    for (int n = 0; n < TEST_REPEATS; n++) {
      @SuppressWarnings(value = { "unused" }) final float[] results = compare(metric, name, names);
    }
