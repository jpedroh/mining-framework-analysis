...
public class PythonExecutorTest {
    ...
    public void testMultithreadedExecWithDependencies() throws InterruptedException {
        ...
        int executionsNum = 5;
        final CountDownLatch latch = new CountDownLatch(executionsNum);
        final String[] dependencies = new java.lang.String[]{ new File(getClass().getClassLoader().getResource(".m2/repository/python/math2/mult/1.0/mult-1.0.zip").getFile()).getAbsolutePath(), new File(getClass().getClassLoader().getResource(".m2/repository/python/math2/sum/2.1/sum-2.1.zip").getFile()).getAbsolutePath(), new File(getClass().getClassLoader().getResource(".m2/repository/python/math3/mult/1.2/mult-1.2.zip").getFile()).getAbsolutePath(), new File(getClass().getClassLoader().getResource(".m2/repository/python/math3/sum/4.1/sum-4.1.zip").getFile()).getAbsolutePath() };
        for (int i = 0; i < executionsNum; i++) {
            final String executioId = String.valueOf(i);
