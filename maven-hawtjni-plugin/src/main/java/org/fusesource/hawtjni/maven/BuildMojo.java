    ...
    if ("detect".equals(tool)) {
      ...
      if ("Windows7.1SDK".equals(toolset)) {
        ...
        if (vcinstalldir != null) {
          if (vcinstalldir.contains("Microsoft Visual Studio 10") || vcinstalldir.contains("Microsoft Visual Studio 11") || vcinstalldir.contains("Microsoft Visual Studio 12")) {
            useMSBuild = true;
          }
        }
      }
    } else {
      if ("msbuild".equals(tool)) {
        useMSBuild = true;
      } else {
        if ("vcbuild".equals(tool)) {
          useMSBuild = false;
        } else {
