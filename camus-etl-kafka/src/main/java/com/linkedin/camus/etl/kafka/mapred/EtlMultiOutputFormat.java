...
public class EtlMultiOutputFormat extends FileOutputFormat<EtlKey, Object> {
  ...
  public final static String ETL_RECORD_WRITER_PROVIDER_CLASS = "etl.record.writer.provider.class";
  ...
  @Override public RecordWriter<EtlKey, Object> getRecordWriter(TaskAttemptContext context) throws IOException, InterruptedException {
    if (committer == null) {
  ...
  private RecordWriter<IEtlKey, CamusWrapper> getDataRecordWriter(TaskAttemptContext context, String fileName, CamusWrapper value) throws IOException, InterruptedException {
  ...
  @Override public synchronized OutputCommitter getOutputCommitter(TaskAttemptContext context) throws IOException {
    if (committer == null) {
      committer = new EtlMultiOutputCommitter(getOutputPath(context), context);
    }
  ...
  public static Class<RecordWriterProvider> getRecordWriterProviderClass(JobContext job) {
    return (Class<RecordWriterProvider>) job.getConfiguration().getClass(ETL_RECORD_WRITER_PROVIDER_CLASS, AvroRecordWriterProvider.class);
  }
  ...
  class MultiEtlRecordWriter extends RecordWriter<EtlKey, Object> {
    ...
    @Override public void write(EtlKey key, Object val) throws IOException, InterruptedException {
      if (val instanceof CamusWrapper<?>) {
        ...
        if (val instanceof ExceptionWritable) {
          committer.addOffset(key);
  ...
  public class EtlMultiOutputCommitter extends FileOutputCommitter {
    ...
    public void addCounts(EtlKey key) throws IOException {
      ...
      if (!counts.containsKey(workingFileName)) {
        counts.put(workingFileName, new EtlCounts(key.getTopic(), granularityMs));
      }
      ...
      if (isRunMoveData(context)) {
        ...
        for (FileStatus f : fs.listStatus(workPath)) {
          ...
          if (file.startsWith("data")) {
            ...
            String partitionedFile = getPartitionedPath(context, file, count.getEventCount(), count.getLastKey().getOffset());
            ...
            if (isRunTrackingPost(context)) {
              count.writeCountsToHDFS(allCountObject, fs, new Path(workPath, COUNTS_PREFIX + "." + dest.getName().replace(recordWriterProvider.getFilenameExtension(), "")));
            }
          }
        }
    ...
    public String getPartitionedPath(JobContext context, String file, int count, long offset) throws IOException {
      ...
      String partitionedPath = getPartitioner(context, topic).generatePartitionedPath(context, topic, Integer.parseInt(leaderId), Integer.parseInt(partition), encodedPartition);
      return partitionedPath + "/" + topic + "." + leaderId + "." + partition + "." + count + "." + offset + recordWriterProvider.getFilenameExtension();
    }
  }
}