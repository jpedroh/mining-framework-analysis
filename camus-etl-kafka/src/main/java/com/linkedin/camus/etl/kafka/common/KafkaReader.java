...
public class KafkaReader {
	...
	public KafkaReader(TaskAttemptContext context, EtlRequest request, int clientTimeout, int fetchBufferSize) throws Exception {
		...
		log.info((((("Connected to leader " + uri) + " beginning reading at offset ") + beginOffset) + " latest offset=") + lastOffset);
		fetch();
	}
	...
	public boolean getNext(EtlKey key, BytesWritable payload, BytesWritable pKey) throws IOException {
		if (hasNext()) {
			...
			key.clear();
			key.set(kafkaRequest.getTopic(), kafkaRequest.getLeaderId(), kafkaRequest.getPartition(), currentOffset, msgAndOffset.offset() + 1, message.checksum());
			key.setMessageSize(msgAndOffset.message().size());
			// increase offset
			currentOffset = msgAndOffset.offset() + 1;
