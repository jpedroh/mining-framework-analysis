...
public class EtlRecordReader extends RecordReader<EtlKey, CamusWrapper> {
  ...
  @SuppressWarnings(value = { "rawtypes", "unchecked" }) @Override public void initialize(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  @Override public EtlKey getCurrentKey() throws IOException, InterruptedException {
    return key;
  }

  @Override public CamusWrapper getCurrentValue() throws IOException, InterruptedException {
    return value;
  }
...
>>>>>>> /usr/src/app/output/linkedin/camus/afe16bd0816776894fd39f6605651d8b566312b0/camus-etl-kafka/src/main/java/com/linkedin/camus/etl/kafka/mapred/EtlRecordReader.java/right.java
           + request.getTopic() + " partition:" + request.getPartition() + " beginOffset:" + request.getOffset() + " estimatedLastOffset:" + request.getLastOffset());
          ...
          reader = new KafkaReader(context, request, CamusJob.getKafkaTimeoutValue(mapperContext), CamusJob.getKafkaBufferSize(mapperContext));
        ...
        while (reader.getNext(key, msgValue, msgKey)) {
          ...
          if (checksum != message.checksum()) {
            throw new ChecksumException("Invalid message checksum " + message.checksum() + ". Expected " + key.getChecksum(), key.getOffset());
          }
          ...
          if (timeStamp < beginTimeStamp) {
            ...
            if (endTimeStamp == 0) {
              ...
              if (timeStamp > endTimeStamp || System.currentTimeMillis() > maxPullTime) {
                ...
                context.setStatus(statusMsg);
                log.info(key.getTopic() + " max read at " + new DateTime(timeStamp).toString());
