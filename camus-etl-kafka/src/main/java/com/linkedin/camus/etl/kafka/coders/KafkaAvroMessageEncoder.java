...
public class KafkaAvroMessageEncoder extends MessageEncoder<IndexedRecord, byte[]> {
  ...
  @SuppressWarnings(value = { "unchecked" }) public KafkaAvroMessageEncoder(String topicName, Configuration conf) {
  ...
  @Override public void init(Properties props, String topicName) {
    ...
    try {
      client = (SchemaRegistry<Schema>) Class.forName(props.getProperty(KAFKA_MESSAGE_CODER_SCHEMA_REGISTRY_CLASS)).newInstance();
      client.init(props);
    } catch (Exception e) {
...
<<<<<<< /usr/src/app/output/linkedin/camus/afe16bd0816776894fd39f6605651d8b566312b0/camus-etl-kafka/src/main/java/com/linkedin/camus/etl/kafka/coders/KafkaAvroMessageEncoder.java/left.java
