...
public class CamusJob extends Configured implements Tool {
  ...
  private static 
<<<<<<< /usr/src/app/output/linkedin/camus/afe16bd0816776894fd39f6605651d8b566312b0/camus-etl-kafka/src/main/java/com/linkedin/camus/etl/kafka/CamusJob.java/left.java
  Logger
=======
  org.apache.log4j.Logger
>>>>>>> /usr/src/app/output/linkedin/camus/afe16bd0816776894fd39f6605651d8b566312b0/camus-etl-kafka/src/main/java/com/linkedin/camus/etl/kafka/CamusJob.java/right.java
   log = Logger.getLogger(CamusJob.class);
...
<<<<<<< /usr/src/app/output/linkedin/camus/afe16bd0816776894fd39f6605651d8b566312b0/camus-etl-kafka/src/main/java/com/linkedin/camus/etl/kafka/CamusJob.java/left.java
    log.info("The blacklisted topics: " + Arrays.toString(EtlInputFormat.getKafkaBlacklistTopic(job)));
=======
>>>>>>> Unknown file: This is a bug in JDime.


<<<<<<< /usr/src/app/output/linkedin/camus/afe16bd0816776894fd39f6605651d8b566312b0/camus-etl-kafka/src/main/java/com/linkedin/camus/etl/kafka/CamusJob.java/left.java
    log.info("The whitelisted topics: " + Arrays.toString(EtlInputFormat.getKafkaWhitelistTopic(job)));
=======
>>>>>>> Unknown file: This is a bug in JDime.
...
<<<<<<< /usr/src/app/output/linkedin/camus/afe16bd0816776894fd39f6605651d8b566312b0/camus-etl-kafka/src/main/java/com/linkedin/camus/etl/kafka/CamusJob.java/left.java
    if (EtlMultiOutputFormat.isRunTrackingPost(job)) {
      ...
      for (FileStatus f : fs.listStatus(newExecutionOutput, new PrefixFilter(EtlMultiOutputFormat.COUNTS_PREFIX))) {
        String topic = f.getPath().getName().split("\\.")[1];
        if (!countsMap.containsKey(topic)) {
          countsMap.put(topic, new EtlCounts(job.getConfiguration(), topic, EtlMultiOutputFormat.getMonitorTimeGranularityMins(job) * 60 * 1000L));
        }
        ...
        if (props.getProperty(ETL_KEEP_COUNT_FILES, "false").equals("true")) {
          fs.rename(f.getPath(), new Path(props.getProperty(ETL_COUNTS_PATH), f.getPath().getName()));
        } else {
      ...
      List<URI> brokerURI;
      try {
        EtlZkClient zkClient = new EtlZkClient(props.getProperty(ZK_AUDIT_HOSTS), EtlInputFormat.getZkSessionTimeout(job), EtlInputFormat.getZkConnectionTimeout(job), EtlInputFormat.getZkTopicPath(job), EtlInputFormat.getZkBrokerPath(job));
