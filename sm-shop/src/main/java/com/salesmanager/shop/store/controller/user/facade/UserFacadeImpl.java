...
@Service(value = "userFacade") public class UserFacadeImpl implements UserFacade {
  ...
  @Inject private PersistableUserPopulator persistableUserPopulator;

  @Inject private SecurityFacade securityFacade;

  private static final Logger LOGGER = LoggerFactory.getLogger(UserFacadeImpl.class);

  @Override public ReadableUser findByUserName(String userName, String storeCode, Language lang) {
  ...
  private User converPersistabletUserToUser(MerchantStore store, Language lang, User userModel, PersistableUser user) {
    try {
  ...
  @Override public List<ReadablePermission> findPermissionsByGroups(List<Integer> ids) {
    return getPermissionsByIds(ids).stream().map((permission) -> convertPermissionToReadablePermission(permission)).collect(Collectors.toList());
  }
  ...
  @Override public boolean authorizedStore(String userName, String merchantStoreCode) {
    try {
  ...
  @Override public void authorizedGroup(String userName, List<String> groupName) {
    ReadableUser readableUser = findByUserName(userName, languageService.defaultLanguage());
    for (ReadableGroup group : readableUser.getGroups()) {
  ...
  @Override public String authenticatedUser() {
    Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
  ...
  @Override public ReadableUserList getByCriteria(Language language, String drawParam, Criteria criteria) {
    try {
  ...
  @Override public void delete(Long id, String merchant) {
  ...
  @Override public ReadableUser update(Long id, String authenticatedUser, String storeCode, PersistableUser user) {
  ...
  @Override public ReadableUser findById(Long id, String storeCode, Language lang) {
  ...
  @Override public void changePassword(Long userId, String authenticatedUser, UserPassword changePassword) {
  ...
  @Override public ReadableUserList listByCriteria(Criteria criteria, int page, int count, Language language) {
    try {
      ReadableUserList readableUserList = new ReadableUserList();
      Page<User> userList = userService.listByCriteria(criteria, page, count);
      List<ReadableUser> readableUsers = userList.getContent().stream().map((user) -> convertUserToReadableUser(language, user)).collect(Collectors.toList());
  ...
  @Override public void authorizedGroups(String authenticatedUser, PersistableUser user) {
  ...
  @Override public boolean userInRoles(String userName, List<String> groupNames) {
    Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
    List<String> roles = authentication.getAuthorities().stream().filter((x) -> groupNames.contains(x.getAuthority())).map((r) -> r.getAuthority()).collect(Collectors.toList());
    return roles.size() > 0;
  }
}