...
@Service(value = "categoryFacade") public class CategoryFacadeImpl implements CategoryFacade {
  ...
  @Inject private ReadableCategoryMapper readableCategoryMapper;
  ...
  @Override public ReadableCategoryList getCategoryHierarchy(MerchantStore store, ListCriteria criteria, int depth, Language language, List<String> filter, int page, int count) {
    ...
    try {
      ...
      if (filter != null && filter.contains(VISIBLE_CATEGORY)) {
        readableCategories = categories.stream().filter(Category::isVisible).map((cat) -> readableCategoryMapper.convert(cat, store, language)).collect(Collectors.toList());
      } else {
        readableCategories = categories.stream().map((cat) -> readableCategoryMapper.convert(cat, store, language)).collect(Collectors.toList());
      }
  ...
  @Override public boolean existByCode(MerchantStore store, String code) {
    try {
  ...
  @Override public PersistableCategory saveCategory(MerchantStore store, PersistableCategory category) {
    try {
  ...
  @Override public ReadableCategory getById(MerchantStore store, Long id, Language language) {
    ...
    ReadableCategory readableCategory = readableCategoryMapper.convert(categoryModel, store, language);
    List<Category> children = getListByLineage(store, lineage.toString());
    List<ReadableCategory> childrenCats = children.stream().map((cat) -> readableCategoryMapper.convert(cat, store, language)).collect(Collectors.toList());
  ...
  private Category getCategoryById(Long id, Language language) {
    return Optional.ofNullable(categoryService.getOneByLanguage(id, language)).orElseThrow(() -> new ResourceNotFoundException("Category id [" + id + "] not found"));
  }

  @Override public void deleteCategory(Category category) {
    try {
  ...
  @Override public ReadableCategory getByCode(MerchantStore store, String code, Language language) throws Exception {
  ...
  @Override public ReadableCategory getCategoryByFriendlyUrl(MerchantStore store, String friendlyUrl, Language language) throws Exception {
  ...
  @Override public void deleteCategory(Long categoryId, MerchantStore store) {
    Category category = getOne(categoryId, store.getId());
    deleteCategory(category);
  }

  private Category getOne(Long categoryId, int storeId) {
    return Optional.ofNullable(categoryService.getById(categoryId)).orElseThrow(() -> new ResourceNotFoundException(String.format("No Category found for ID : %s", categoryId)));
  }

  @Override public List<ReadableProductVariant> categoryProductVariants(Long categoryId, MerchantStore store, Language language) {
    ...
    try {
      ...
      while (it.hasNext()) {
        @SuppressWarnings(value = { "rawtypes" }) Map.Entry pair = (Map.Entry) it.next();
  ...
  @Override public void move(Long child, Long parent, MerchantStore store) {
  ...
  @Override public Category getByCode(String code, MerchantStore store) {
    try {
  ...
  @Override public void setVisible(PersistableCategory category, MerchantStore store) {
  ...
  @Override public ReadableCategoryList listByProduct(MerchantStore store, Long product, Language language) {
