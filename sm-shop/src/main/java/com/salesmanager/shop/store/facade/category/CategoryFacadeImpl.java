...
>>>>>>> /usr/src/app/output/shopizer-ecommerce/shopizer/e992f347e0b4f6502966d46fffa4c60cb4c89211/sm-shop/src/main/java/com/salesmanager/shop/store/facade/category/CategoryFacadeImpl.java/right.java
    ...
    List<ReadableCategory> filteredList = readableCategoryMap.values().stream().filter((cat) -> cat.getDepth() == 0).sorted(Comparator.comparing(ReadableCategory::getSortOrder)).collect(Collectors.toList());
  ...
  @Override public PersistableCategory saveCategory(MerchantStore store, PersistableCategory category) {
    try {
      ...
      Category target = Optional.ofNullable(categoryId).filter((id) -> id > 0).map(categoryService::findById).orElse(new Category());
  ...
  private void saveCategory(MerchantStore store, Category category, Category parent) throws ServiceException {
    if (parent != null) {
      ...
      category.setLineage(new StringBuilder().append(lineage).append(Constants.SLASH).toString());
    }
  ...
  private Category getCategoryById(Long id, Language language) {
    return Optional.ofNullable(categoryService.getOneByLanguage(id, language)).orElseThrow(() -> new ResourceNotFoundException("Category id not found"));
  }
  ...
  @Override public void deleteCategory(Long categoryId) {
