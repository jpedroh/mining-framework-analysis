 ...
 *
 */
@Controller @RequestMapping(value = Constants.SHOP_URI + "/order") public class ShoppingOrderController extends AbstractController {
  private static final Logger LOGGER = LoggerFactory.getLogger(ShoppingOrderController.class);

  @Value(value = "${config.googleMapsKey}") private String googleMapsKey;
  ...
  @Inject private OrderProductDownloadService orderProdctDownloadService;

  @SuppressWarnings(value = { "unused" }) @RequestMapping(value = "/checkout.html") public String displayCheckout(@CookieValue(value = "cart") String cookie, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
  ...
  @RequestMapping(value = "/commitPreAuthorized.html") public String commitPreAuthorizedOrder(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
    MerchantStore store = (MerchantStore) request.getAttribute(Constants.MERCHANT_STORE);
    Language language = (Language) request.getAttribute("LANGUAGE");
    ...
    @SuppressWarnings(value = { "unchecked" }) Map<String, Object> configs = (Map<String, Object>) request.getAttribute(Constants.REQUEST_CONFIGS);
  ...
  @SuppressWarnings(value = { "unchecked" }) @RequestMapping(value = "/commitOrder.html") public String commitOrder(@CookieValue(value = "cart") String cookie, @Valid @ModelAttribute(value = "order") ShopOrder order, BindingResult bindingResult, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
    MerchantStore store = (MerchantStore) request.getAttribute(Constants.MERCHANT_STORE);
    Language language = (Language) request.getAttribute("LANGUAGE");
    model.addAttribute("googleMapsKey", googleMapsKey);
    if (!StringUtils.isBlank(googleMapsKey)) {
    ...
    try {
      ...
      @SuppressWarnings(value = { "unused" }) Order modelOrder = commitOrder(order, request, locale);
      ...
      if (se.getExceptionType() == ServiceException.EXCEPTION_VALIDATION) {
        ...
        if (se.getExceptionType() == ServiceException.EXCEPTION_PAYMENT_DECLINED) {
  ...
  /**
	 ...
	 * @throws Exception
	 */
  @SuppressWarnings(value = { "unchecked" }) @RequestMapping(value = { "/shippingQuotes.json" }, method = RequestMethod.POST) public @ResponseBody ReadableShopOrder calculateShipping(@ModelAttribute(value = "order") ShopOrder order, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
    Language language = (Language) request.getAttribute("LANGUAGE");
    MerchantStore store = (MerchantStore) request.getAttribute(Constants.MERCHANT_STORE);
  ...
  /**
	 ...
	 * @throws Exception
	 */
  @RequestMapping(value = { "/calculateOrderTotal.json" }, method = RequestMethod.POST) public @ResponseBody ReadableShopOrder calculateOrderTotal(@ModelAttribute(value = "order") ShopOrder order, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
    Language language = (Language) request.getAttribute("LANGUAGE");
    MerchantStore store = (MerchantStore) request.getAttribute(Constants.MERCHANT_STORE);
    String shoppingCartCode = getSessionAttribute(Constants.SHOPPING_CART, request);
    Validate.notNull(shoppingCartCode, "shoppingCartCode does not exist in the session");
    ...
    try {
      ...
      if (order.getSelectedShippingOption() != null) {
        ShippingSummary summary = (ShippingSummary) request.getSession().getAttribute(Constants.SHIPPING_SUMMARY);
        @SuppressWarnings(value = { "unchecked" }) List<ShippingOption> options = (List<ShippingOption>) request.getSession().getAttribute(Constants.SHIPPING_OPTIONS);
        order.setShippingSummary(summary);
        ReadableShippingSummary readableSummary = new ReadableShippingSummary();
        ReadableShippingSummaryPopulator readableSummaryPopulator = new ReadableShippingSummaryPopulator();
        readableSummaryPopulator.setPricingService(pricingService);
        ...
        if (!CollectionUtils.isEmpty(options)) {
          ...
          summary.setShipping(quoteOption.getOptionPrice());
          order.setShippingSummary(summary);
          @SuppressWarnings(value = { "unchecked" }) Map<String, String> informations = (Map<String, String>) request.getSession().getAttribute("SHIPPING_INFORMATIONS");
