...
public class ReadableProductPopulator extends AbstractDataPopulator<Product, ReadableProduct> {
  ...
  @Override public ReadableProduct populate(Product source, ReadableProduct target, MerchantStore store, Language language) throws ConversionException {
    ...
    try {
      ...
      if (source.getType() != null) {
        ...
        type.setName(source.getType().getCode());
        target.setType(type);
      }
  ...
  private ReadableProductOption createOption(ProductAttribute productAttribute, Language language) {
    ...
    option.setId(productAttribute.getProductOption().getId());
  ...
  @Override protected ReadableProduct createTarget() {
    return null;
  }
