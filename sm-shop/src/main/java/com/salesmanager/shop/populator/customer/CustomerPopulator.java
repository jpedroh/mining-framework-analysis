...
@Component public class CustomerPopulator extends AbstractDataPopulator<PersistableCustomer, Customer> {
  ...
  /**
	 ...
	 */
  @Override public Customer populate(PersistableCustomer source, Customer target, MerchantStore store, Language language) throws ConversionException {
    try {
      ...
      if (target.getDefaultLanguage() == null) {
        Language lang = source.getLanguage() == null ? null : languageService.getByCode(source.getLanguage());
  ...
  @Override protected Customer createTarget() {
    return new Customer();
  }
}