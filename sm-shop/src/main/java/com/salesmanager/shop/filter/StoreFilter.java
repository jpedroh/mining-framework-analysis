...
public class StoreFilter extends HandlerInterceptorAdapter {
  ...
  @Inject private MerchantStoreService merchantService;
  ...
  @Inject private WebApplicationCacheUtils webApplicationCache;
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
    ...
    try {
      ...
      @SuppressWarnings(value = { "unchecked" }) Map<String, ContentDescription> contents = (Map<String, ContentDescription>) request.getAttribute(Constants.REQUEST_CONTENT_OBJECTS);
      if (contents != null) {
  ...
  @SuppressWarnings(value = { "unchecked" }) private void getMerchantConfigurations(MerchantStore store, HttpServletRequest request) throws Exception {
  ...
  @SuppressWarnings(value = { "unchecked" }) private void getContentPageNames(MerchantStore store, Language language, HttpServletRequest request) throws Exception {
    StringBuilder contentKey = new StringBuilder();
    contentKey.append(store.getId()).append("_").append(Constants.CONTENT_PAGE_CACHE_KEY).append("-").append(language.getCode());
  ...
  @SuppressWarnings(value = { "unchecked" }) private void getContentObjects(MerchantStore store, Language language, HttpServletRequest request) throws Exception {
    StringBuilder contentKey = new StringBuilder();
  ...
  @SuppressWarnings(value = { "unchecked" }) private void setTopCategories(MerchantStore store, Language language, HttpServletRequest request) throws Exception {
    ...
    if (store.isUseCache()) {
      ...
      if (objects == null) {
        ReadableCategoryList categoryList = categoryFacade.getCategoryHierarchy(store, null, 0, language, null, 0, 200);
  ...
  @SuppressWarnings(value = { "unused" }) private Map<String, Object> getConfigurations(MerchantStore store) {
    ...
    try {
      ...
      if (merchantConfig != null) {
        ...
        @SuppressWarnings(value = { "unchecked" }) Map<String, Object> props = m.convertValue(merchantConfig, Map.class);
        ...
        if (language.getCode().equals(breadCrumb.getLanguage().getCode())) {
          ...
          for (BreadcrumbItem item : breadCrumb.getBreadCrumbs()) {
            if (item.getItemType().name().equals(BreadcrumbItemType.HOME)) {
              ...
              if (item.getItemType().name().equals(BreadcrumbItemType.PRODUCT)) {
                ...
                if (item.getItemType().name().equals(BreadcrumbItemType.CATEGORY)) {
                  ...
                  if (item.getItemType().name().equals(BreadcrumbItemType.PAGE)) {
