...
public class MongoResponse extends BsonResponse {
  ...
  /**
     ...
     * @param t
     */
  @Override public void setInError(int code, String message, Throwable t) {
    setStatusCode(code);
    String httpStatusText = HttpStatus.getStatusText(code);
    setInError(true);
    setContent(getErrorContent(code, httpStatusText, message, t, false));
  }
  ...
  public void rollback(MongoClient mclient) throws Exception {
    ...
    MongoCollection<BsonDocument> coll = mdb.getCollection(request.getCollectionName(), BsonDocument.class);
    BsonDocument oldData = getDbOperationResult().getOldData();
    Object newEtag = getDbOperationResult().getEtag();
    if (oldData != null) {
      restoreDocument(request.getClientSession(), coll, oldData.get("_id"), request.getShardKey(), oldData, newEtag, "_etag");
      if (oldData.get("$set") != null && oldData.get("$set").isDocument() && oldData.get("$set").asDocument().get("_etag") != null) {
        response.getHeaders().put(Headers.ETAG, oldData.get("$set").asDocument().get("_etag").asObjectId().getValue().toString());
      } else {
  ...
  private static boolean restoreDocument(final ClientSession cs, final MongoCollection<BsonDocument> coll, final Object documentId, final BsonDocument shardKeys, final BsonDocument data, final Object etag, final String etagLocation) {
    ...
    UpdateResult result = cs == null ? coll.replaceOne(query, data, R_NOT_UPSERT_OPS) : coll.replaceOne(cs, query, data, R_NOT_UPSERT_OPS);
