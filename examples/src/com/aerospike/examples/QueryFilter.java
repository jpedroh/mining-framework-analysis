...
public class QueryFilter extends Example {
  ...
  private void writeRecords(IAerospikeClient client, Parameters params, String keyPrefix, String binName) throws Exception {
  ...
  private void writeRecord(IAerospikeClient client, Parameters params, String userKey, String name, String password) throws Exception {
    Key key = new Key(params.namespace, params.set, userKey);
    Bin bin1 = new Bin("name", name);
    Bin bin2 = new Bin("password", password);
    console.info("Put: ns=%s set=%s key=%s bin=%s value=%s", key.namespace, key.setName, key.userKey, bin1.name, bin1.value);
    client.put(params.writePolicy, key, bin1, bin2);
  }

  @SuppressWarnings(value = { "unchecked" }) private void runQuery(IAerospikeClient client, Parameters params, String indexName, String binName) throws Exception {
