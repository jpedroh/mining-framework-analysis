...
public class UserDefinedFunction extends Example {
  ...
  private void writeBlobUsingUdf(IAerospikeClient client, Parameters params) throws Exception {
    ...
    if (Arrays.equals(blob, received)) {
      console.info("Blob data matched: namespace=%s set=%s key=%s bin=%s value=%s", key.namespace, key.setName, key.userKey, binName, Arrays.toString(received));
    } else {
      throw new Exception(String.format("Mismatch: expected=%s received=%s", Arrays.toString(blob), Arrays.toString(received)));
    }
  }
}