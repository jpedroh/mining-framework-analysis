...
public class BatchOperate extends Example {
  ...
  @Override public void runExample(IAerospikeClient client, Parameters params) {
  ...
  private void writeRecords(IAerospikeClient client, Parameters params) {
  ...
  private void batchListReadOperate(IAerospikeClient client, Parameters params) {
    ...
    Record[] records = client.get(null, keys, ListOperation.size(BinName3), ListOperation.getByIndex(BinName3, -1, ListReturnType.VALUE));
    for (int i = 0; i < records.length; i++) {
      Record record = records[i];
      if (record != null) {
  ...
  private void batchListWriteOperate(IAerospikeClient client, Parameters params) {
    ...
    BatchResults bresults = client.operate(null, null, keys, ListOperation.append(ListPolicy.Default, BinName3, Value.get(999)), ListOperation.size(BinName3), ListOperation.getByIndex(BinName3, -1, ListReturnType.VALUE));
  ...
  private void batchWriteOperateComplex(IAerospikeClient client, Parameters params) {
    ...
    Operation[] ops1 = Operation.array(Operation.put(new Bin(BinName4, 100)), ExpOperation.read(ResultName1, rexp1, ExpReadFlags.DEFAULT));
    ...
    Operation[] ops4 = Operation.array(ExpOperation.write(BinName1, wexp1, ExpWriteFlags.DEFAULT), ExpOperation.read(ResultName1, rexp3, ExpReadFlags.DEFAULT));
    Operation[] ops5 = Operation.array(ExpOperation.read(ResultName1, rexp2, ExpReadFlags.DEFAULT), ExpOperation.read(ResultName2, rexp3, ExpReadFlags.DEFAULT));
