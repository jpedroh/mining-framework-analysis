...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private void validateBin(Key key, Bin bin, Record record) {
    ...
    if (received != null && received.equals(expected)) {
      console.info("Bin matched: namespace=%s set=%s key=%s bin=%s value=%s generation=%d expiration=%d", key.namespace, key.setName, key.userKey, bin.name, received, record.generation, record.expiration);
    } else {
  ...
  private void runGetHeaderTest(IAerospikeClient client, Parameters params) throws Exception {
    ...
    if (record.generation == 0) {
      throw new Exception(String.format("Invalid record header: generation=%d expiration=%d", record.generation, record.expiration));
    }
