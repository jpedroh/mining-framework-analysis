...
public class Operate extends Example {
  ...
  private void addWriteGet(IAerospikeClient client, WritePolicy policy, Key key, String bin1, String bin2) throws Exception {
  ...
  private void touchReadMultipleBins(IAerospikeClient client, WritePolicy policy, Key key, String bin1, String bin2, String bin3) throws Exception {
    Record record = client.operate(policy, key, Operation.touch(), Operation.get(bin1), Operation.get(bin2), Operation.get(bin3));
    if (record == null) {
      throw new Exception(String.format("Failed to get: namespace=%s set=%s key=%s", key.namespace, key.setName, key.userKey));
    }
  ...
  private void validateBin(Key key, Record record, String binName, Object expected, Object received) {
    if (received != null && received.equals(expected)) {
      console.info("Bin matched: namespace=%s set=%s key=%s bin=%s value=%s generation=%s expiration=%s", key.namespace, key.setName, key.userKey, binName, received, record.generation, record.expiration);
    } else {
