...
public class IdentifyParser {
  ...
  @SuppressWarnings(value = { "unchecked" }) public Identify parse() {
    try {
      ...
      if (reader.current(theEndOfDocument())) {
        ...
        if (reader.current(elementName(localPart(equalTo("description"))))) {
  ...
  private XmlReader.IslandParser<Description> descriptionParser() {
    return new XmlReader.IslandParser<Description>() {
      @Override public Description parse(XmlReader reader) throws XmlReaderException {
