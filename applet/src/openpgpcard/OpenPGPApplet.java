...
public class OpenPGPApplet extends Applet implements ISO7816 {
  ...
  private static final byte[] HISTORICAL = { 0x00, 0x73, 0x00, 0x00, (byte) 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

  private static final byte[] VERSION = { 0x01, 0x00, 0x12 };

  private static final short 
<<<<<<< /usr/src/app/output/yubico/ykneo-openpgp/2766bd74444ce9ff9205bf215c0cd05757331503/applet/src/openpgpcard/OpenPGPApplet.java/left.java
  SW_AUTHENTICATION_BLOCKED = 0x6983
=======
  SW_REFERENCED_DATA_NOT_FOUND = 0x6A88
>>>>>>> /usr/src/app/output/yubico/ykneo-openpgp/2766bd74444ce9ff9205bf215c0cd05757331503/applet/src/openpgpcard/OpenPGPApplet.java/right.java
  ;

  private static final byte[] EXTENDED_CAP = { (byte) 0xF8, 0x00, 0x00, (byte) 0xFF, 0x04, (byte) 0xC0, 0x00, (byte) 0xFF, 0x00, (byte) 0xFF };
  ...
  private static final byte[] PW3_DEFAULT = { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38 };
  ...
  private short getData(short tag) {
    ...
    switch (tag) {
      ...
      buffer[offset++] = (short) 60;
      offset = Util.arrayCopyNonAtomic(ca1_fp, _0, buffer, offset, FP_LENGTH);
      offset = Util.arrayCopyNonAtomic(ca2_fp, _0, buffer, offset, FP_LENGTH);
      offset = Util.arrayCopyNonAtomic(ca3_fp, _0, buffer, offset, FP_LENGTH);
  ...
  private void putData(short tag) {
    ...
    switch (tag) {
      ...
      JCSystem.beginTransaction();
      loginData_length = Util.arrayCopy(buffer, _0, loginData, _0, in_received);
      JCSystem.commitTransaction();
      break;
      case (short) 0x5F2D:
      ...
      if (buffer[0] != (byte) 0x31 && buffer[0] != (byte) 0x32 && buffer[0] != (byte) 0x39) {
        ISOException.throwIt(SW_WRONG_DATA);
      }
      sex = buffer[0];
      break;
      ...
      if (in_received == 0) {
        ...
        if (in_received >= RC_MIN_LENGTH && in_received <= RC_MAX_LENGTH) {
          JCSystem.beginTransaction();
  ...
  private short getLength(byte[] data, short offset) {
    ...
    if ((data[offset] & (byte) 0x80) == (byte) 0x00) {
      ...
      if ((data[offset] & (byte) 0x7F) == (byte) 0x01) {
