...
public class ApplicationControllerTest extends NinjaApiTest {
  @Test public void testThatRedirectWorks() {
    ...
    String result = NinjaApiTestHelper.makeRequest(getServerAddress() + "/redirect", headers);
  ...
  @Test public void testHtmlEscapingInTeamplateWorks() {
    ...
    Map<String, String> headers = Maps.newHashMap();
    String result = NinjaApiTestHelper.makeRequest(getServerAddress() + "htmlEscaping", headers);
    assertTrue(result.contains(expectedContent));
  }

  @Test public void makeSureSessionsGetSentToClient() {
    Map<String, String> headers = Maps.newHashMap();
    HttpResponse httpResponse = NinjaApiTestHelper.makeRequestAndGetResponse(getServerAddress() + "session", headers);
