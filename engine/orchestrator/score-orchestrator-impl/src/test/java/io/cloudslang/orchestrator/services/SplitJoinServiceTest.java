...
@RunWith(value = SpringJUnit4ClassRunner.class) @ContextConfiguration public class SplitJoinServiceTest {
  @InjectMocks private SplitJoinService splitJoinService = new SplitJoinServiceImpl();

  @Mock private SuspendedExecutionsRepository suspendedExecutionsRepository;

  @Mock private FinishedBranchRepository finishedBranchRepository;

  @Mock private ExecutionQueueRepository executionQueueRepository;

  @Mock private QueueDispatcherService queueDispatcherService;

  @Mock private FastEventBus fastEventBus;

  @Mock private ExecutionMessageConverter converter;

  @Captor private ArgumentCaptor<List<ExecutionMessage>> queueDispatcherDispatchCaptor;

  @Captor private ArgumentCaptor<List<SuspendedExecution>> suspendedExecutionsSaveCaptor;

  @Captor private ArgumentCaptor<SuspendedExecution> suspendedExecutionsSingleSaveCaptor;

  @Captor private ArgumentCaptor<Execution> converterCaptor;
  ...
  @Test public void triggerChildrenSplitTest() {
  ...
  @Test public void suspendParentSplitTest() {
    String splitId = UUID.randomUUID().toString();
    SplitMessage splitMessage = createSplitMessage(splitId, "MULTI_INSTANCE");
    splitJoinService.split(Arrays.asList(splitMessage));
  ...
  @Test(expected = IllegalArgumentException.class) public void nullParamSplitTest() {
    splitJoinService.split(null);
  }

  @Test public void missingSuspendedExecutionEndBranchTest() {
  ...
  @Test(expected = IllegalArgumentException.class) public void nullParamEndBranchTest() {
    splitJoinService.endBranch(null);
  }

  @Test public void triggerParentJoinFinishedSplitsTest() {
    String splitId = UUID.randomUUID().toString();
  ...
  @Test public void deleteParentJoinFinishedSplitsTest() {
    String splitId = UUID.randomUUID().toString();
    SuspendedExecution suspendedExecution = createSuspendedExecution(splitId, 1);
  ...
  @Test public void insertBranchesToParentJoinFinishedSplitsTest() {
    String splitId = UUID.randomUUID().toString();
    SuspendedExecution suspendedExecution = createSuspendedExecution(splitId, 1);
    HashMap<String, Serializable> context = new HashMap<>();
    ...
    assertThat("parent execution must contain children maps", finishedChildContexts, is(Arrays.asList(new EndBranchDataContainer(ooContexts, systemContext, null))));
  }
