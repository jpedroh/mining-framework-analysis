...
public final class SplitJoinServiceImpl implements SplitJoinService {
  ...
  @Autowired private SuspendedExecutionsRepository suspendedExecutionsRepository;

  @Autowired private FinishedBranchRepository finishedBranchRepository;

  @Autowired private QueueDispatcherService queueDispatcherService;

  @Autowired private ExecutionMessageConverter converter;

  @Autowired private ExecutionQueueRepository executionQueueRepository;

  @Autowired @Qualifier(value = "consumptionFastEventBus") private FastEventBus fastEventBus;
  ...
  private final Converter<Execution, ExecutionMessage> executionToStartExecutionMessage = new Converter<Execution, ExecutionMessage>() {
    @Override public ExecutionMessage convert(Execution execution) {
      return new ExecutionMessage(execution.getExecutionId().toString(), converter.createPayload(execution));
    }
  };
  ...
  @Override @Transactional public void split(List<SplitMessage> splitMessages) {
    ...
    for (SplitMessage splitMessage : splitMessages) {
      if (splitMessage.isExecutable()) {
        ...
        Serializable stepTypeSerializable = splitMessage.getParent().getSystemContext().get("STEP_TYPE");
        final SuspendedExecutionReason stepType = stepTypeSerializable != null ? SuspendedExecutionReason.valueOf(stepTypeSerializable.toString()) : PARALLEL_LOOP;
        suspendedParents.add(new SuspendedExecution(splitMessage.getParent().getExecutionId().toString(), splitMessage.getSplitId(), splitMessage.getTotalNumberOfBranches(), splitMessage.getParent(), stepType, false));
      } else {
  ...
  private List<ExecutionMessage> prepareExecutionMessages(List<Execution> executions, boolean active) {
    return executions.stream().map((execution) -> convertExecutionToExecutionMessage(active, execution)).collect(toList());
  }

  private ExecutionMessage convertExecutionToExecutionMessage(boolean active, Execution execution) {
    ExecutionMessage executionMessage = new ExecutionMessage(execution.getExecutionId().toString(), converter.createPayload(execution));
  ...
  private void setWorkerGroupOnCSParallelLoopBranches(Execution execution, ExecutionMessage executionMessage) {
    if (StringUtils.equals(execution.getSystemContext().getLanguageName(), "CloudSlang") && execution.getSystemContext().getWorkerGroupName() != null) {
  ...
  @Override @Transactional public void endBranch(List<Execution> executions) {
  ...
  private void processFinishedBranch(FinishedBranch finishedBranch, SuspendedExecution suspendedExecution, List<SuspendedExecution> suspendedExecutionsWithOneBranch) {
  ...
  @Override @Transactional public int joinFinishedSplits(int bulkSize) {
  ...
  @Override @Transactional public void joinFinishedSplits() {
    try {
  ...
  @Override @Transactional public int joinFinishedMiBranches(int bulkSize) {
    PageRequest pageRequest = PageRequest.of(0, bulkSize);
  ...
  private int joinMiBranchesAndSendToQueue(List<SuspendedExecution> suspendedExecutions) {
    ...
    for (SuspendedExecution se : suspendedExecutions) {
      ...
      long nrOfNewFinishedBranches = finishedBranches.stream().filter((finishedBranch) -> !finishedBranch.getBranchContexts().isBranchCancelled()).count();
  ...
  private Execution joinSplit(SuspendedExecution suspendedExecution) {
    ...
    Validate.isTrue(suspendedExecution.getNumberOfBranches().equals(finishedBranches.size()), "Expected suspended execution " + exec.getExecutionId() + " to have " + suspendedExecution.getNumberOfBranches() + "finished branches, but found " + finishedBranches.size());
    if (logger.isDebugEnabled()) {
