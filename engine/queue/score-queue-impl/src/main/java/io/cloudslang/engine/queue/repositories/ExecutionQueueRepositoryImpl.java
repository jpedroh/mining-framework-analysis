 ...
 * Time: 15:04
 */
@SuppressWarnings(value = { "FieldCanBeLocal" }) public class ExecutionQueueRepositoryImpl implements ExecutionQueueRepository {
...
<<<<<<< /usr/src/app/output/cloudslang/score/fef6f95a8c96b0224ed8ff509bd63e3ad0c23ebc/engine/queue/score-queue-impl/src/main/java/io/cloudslang/engine/queue/repositories/ExecutionQueueRepositoryImpl.java/left.java
  SELECT_FINISHED_STEPS_IDS_1 = "SELECT DISTINCT EXEC_STATE_ID FROM OO_EXECUTION_QUEUES EQ WHERE EQ.STATUS IN (6,7,8)"
=======
  SELECT_FINISHED_STEPS_IDS = "SELECT DISTINCT EXEC_STATE_ID FROM OO_EXECUTION_QUEUES EQ WHERE EQ.STATUS = " + ExecStatus.FINISHED.getNumber() + " OR EQ.STATUS = " + ExecStatus.TERMINATED.getNumber() + " OR EQ.STATUS = " + ExecStatus.FAILED.getNumber() + " OR ((EQ.EXEC_STATE_ID IN " + "(SELECT DISTINCT STATES.ID FROM OO_EXECUTION_STATES STATES JOIN OO_EXECUTION_STATE ES ON STATES.MSG_ID = CAST(ES.EXECUTION_ID AS VARCHAR(255)) " + "WHERE ((ES.STATUS = \'COMPLETED\' OR ES.STATUS = \'CANCELED\' OR ES.STATUS = \'SYSTEM_FAILURE\') ))))"
>>>>>>> /usr/src/app/output/cloudslang/score/fef6f95a8c96b0224ed8ff509bd63e3ad0c23ebc/engine/queue/score-queue-impl/src/main/java/io/cloudslang/engine/queue/repositories/ExecutionQueueRepositoryImpl.java/right.java
  ;


<<<<<<< /usr/src/app/output/cloudslang/score/fef6f95a8c96b0224ed8ff509bd63e3ad0c23ebc/engine/queue/score-queue-impl/src/main/java/io/cloudslang/engine/queue/repositories/ExecutionQueueRepositoryImpl.java/left.java
  final private String SELECT_FINISHED_STEPS_IDS_2 = "SELECT DISTINCT EXEC_STATE_ID FROM OO_EXECUTION_QUEUES EQ WHERE (EQ.EXEC_STATE_ID IN " + "(SELECT DISTINCT STATES.ID FROM OO_EXECUTION_STATES STATES JOIN OO_EXECUTION_STATE ES ON STATES.MSG_ID = CAST(ES.EXECUTION_ID AS VARCHAR(255)) " + "WHERE ES.STATUS IN(\'COMPLETED\',\'CANCELED\',\'SYSTEM_FAILURE\') ))";
=======
>>>>>>> Unknown file: This is a bug in JDime.


  final private String QUERY_DELETE_FINISHED_STEPS_FROM_QUEUES = "DELETE FROM OO_EXECUTION_QUEUES " + " WHERE EXEC_STATE_ID in (:ids)";

  final private String QUERY_DELETE_FINISHED_STEPS_FROM_STATES = "DELETE FROM OO_EXECUTION_STATES " + " WHERE ID in (:ids)";

  final private String QUERY_DELETE_EXECS_STATES_MAPPINGS = "DELETE FROM OO_EXECS_STATES_EXECS_MAPPINGS " + " WHERE EXEC_STATE_ID in (:ids)";
  ...
  final private String QUERY_COUNT_MESSAGES_WITHOUT_ACK_FOR_WORKER_SQL = "SELECT COUNT(*)  " + "  FROM  OO_EXECUTION_QUEUES  q  " + "  WHERE " + "      (q.ASSIGNED_WORKER  = ? ) AND " + "      (q.STATUS  = ? ) AND " + "     (NOT EXISTS (SELECT qq.MSG_SEQ_ID " + "                  FROM OO_EXECUTION_QUEUES qq " + "                  WHERE (qq.EXEC_STATE_ID = q.EXEC_STATE_ID) AND " + "                        qq.MSG_SEQ_ID > q.MSG_SEQ_ID " + "                 )" + "      ) AND " + "      (q.MSG_VERSION < ?)  ";
  ...
  final private String BUSY_WORKERS_SQL = "SELECT ASSIGNED_WORKER      " + " FROM  OO_EXECUTION_QUEUES q  " + " WHERE  " + "      (q.STATUS IN (:status)) AND " + " (NOT EXISTS (SELECT qq.MSG_SEQ_ID " + "              FROM OO_EXECUTION_QUEUES qq " + "              WHERE (qq.EXEC_STATE_ID = q.EXEC_STATE_ID) AND qq.MSG_SEQ_ID > q.MSG_SEQ_ID)) " + " GROUP BY ASSIGNED_WORKER";
  ...
  @Autowired private IdentityGenerator idGen;

  @Autowired private DataSource dataSource;

  @Autowired private ExecutionStateService executionStateService;
  ...
  @Override public long generateExecStateId() {
    return idGen.next();
  }

  @Override public void insertExecutionStates(final List<ExecutionMessage> stateMessages) {
  ...
  @Override public void insertExecutionQueue(final List<ExecutionMessage> messages, final long version) {
  ...
  @Override public void insertNotActiveExecutionsQueues(List<ExecutionMessage> notActiveMessages) {
  ...
  @Override public StartNewBranchPayload getFirstPendingBranch(final long executionId) {
    ...
    try {
      startNewBranchPayload = getFirstPendingBranchJdbcTemplate.queryForObject(sql, inputs, (resultSet, rowNumber) -> new StartNewBranchPayload(resultSet.getLong("EXEC_STATE_ID"), resultSet.getLong("ID")));
    } catch (EmptyResultDataAccessException ignored) {
    }
    return startNewBranchPayload;
  }

  @Override public void activatePendingExecutionStateForAnExecution(long executionId) {
  ...
  @Override public void deletePendingExecutionState(long executionStatesId) {
  ...
  @Override public List<ExecutionMessage> pollRecovery(String workerId, int maxSize, ExecStatus... statuses) {
    pollForRecoveryJdbcTemplate.setStatementBatchSize(maxSize);
    try {
      String sqlStatPrvTable = QUERY_WORKER_RECOVERY_SQL.replaceAll(":status", StringUtils.repeat("?", ",", statuses.length));
  ...
  @Override public List<ExecutionMessage> poll(String workerId, int maxSize, long workerPollingMemory, ExecStatus... statuses) {
    Object[] args = useLargeMessageQuery ? preparePollArgs(workerId, workerPollingMemory, statuses) : prepareStdPollArgs(workerId, statuses);
  ...
  private List<ExecutionMessage> executePoll(int maxSize, String sql, Object[] args) {
    ...
    try {
      RowMapper<ExecutionMessage> rowMapper = (rs, rowNum) -> new ExecutionMessage(rs.getLong("EXEC_STATE_ID"), rs.getString("ASSIGNED_WORKER"), rs.getString("EXEC_GROUP"), rs.getString("MSG_ID"), ExecStatus.find(rs.getInt("STATUS")), new Payload(rs.getBytes("PAYLOAD")), rs.getInt("MSG_SEQ_ID"), rs.getLong("CREATE_TIME"));
      List<ExecutionMessage> executionMessages = doSelectWithTemplate(pollJdbcTemplate, sql, rowMapper, args);
  ...
  @Override public void deleteFinishedSteps(Set<Long> stepIds) {
    ...
    while (itr.hasNext()) {
      ...
      logSQL(query, args);
      int deletedRows = deleteFinishedStepsJdbcTemplate.update(query, args);
      if (logger.isDebugEnabled()) {
  ...
  @Override public Set<Long> getFinishedExecStateIds() {
  ...
  @Override public Map<Long, Payload> findPayloadByExecutionIds(Long... ids) {
  ...
  @Override public List<ExecutionMessage> findByStatuses(int maxSize, ExecStatus... statuses) {
    findByStatusesJdbcTemplate.setStatementBatchSize(maxSize);
    String sqlStat = QUERY_MESSAGES_BY_STATUSES.replaceAll(":status", StringUtils.repeat("?", ",", statuses.length));
  ...
  @Override public List<String> getBusyWorkers(ExecStatus... statuses) {
    String sqlStat = BUSY_WORKERS_SQL.replaceAll(":status", StringUtils.repeat("?", ",", statuses.length));
    Object[] values = new Object[statuses.length];
    int i = 0;
    for (ExecStatus status : statuses) {
      values[i] = status.getNumber();
  ...
  @Override public List<ExecutionMessage> findOldMessages(long timestamp) {
    return findLargeJdbcTemplate.query(FIND_OLD_STATES, new Object[] { timestamp }, (rs, rowNum) -> {
      ExecutionMessage msg = new ExecutionMessage(rs.getLong("EXEC_STATE_ID"), rs.getString("ASSIGNED_WORKER"), rs.getString("EXEC_GROUP"), null, ExecStatus.find(rs.getInt("STATUS")), null, rs.getInt("MSG_SEQ_ID"), rs.getLong("CREATE_TIME"));
      return msg;
    });
  }

  @Override public Set<Long> getExecutionIdsForExecutionStateIds(Set<Long> execStateIds) {
    ...
    for (List<Long> part : Iterables.partition(execStateIds, PARTITION_SIZE)) {
      ...
      List<Long> execIds = findExecIDsJdbcTemplate.query(sqlStat, part.toArray(), (rs, rowNum) -> rs.getLong("MSG_ID"));
  ...
  private class ExecutionMessageRowMapper implements RowMapper<ExecutionMessage> {
    @Override public ExecutionMessage mapRow(ResultSet rs, int rowNum) throws SQLException {
      return new ExecutionMessage(rs.getLong("EXEC_STATE_ID"), rs.getString("ASSIGNED_WORKER"), rs.getString("EXEC_GROUP"), rs.getString("MSG_ID"), ExecStatus.find(rs.getInt("STATUS")), new Payload(rs.getBytes("PAYLOAD")), rs.getInt("MSG_SEQ_ID"), rs.getLong("CREATE_TIME"));
    }
  }

  private class ExecutionMessageWithoutPayloadRowMapper implements RowMapper<ExecutionMessage> {
    @Override public ExecutionMessage mapRow(ResultSet rs, int rowNum) throws SQLException {
      return new ExecutionMessage(rs.getLong("EXEC_STATE_ID"), rs.getString("ASSIGNED_WORKER"), rs.getString("EXEC_GROUP"), "-1", ExecStatus.find(rs.getInt("STATUS")), null, rs.getInt("MSG_SEQ_ID"), rs.getLong("CREATE_TIME"));
    }
  }

  private <T extends java.lang.Object> List<T> doSelectWithTemplate(JdbcTemplate jdbcTemplate, String sql, RowMapper<T> rowMapper, Object... params) {
    logSQL(sql, params);
    try {
      long t = System.currentTimeMillis();
