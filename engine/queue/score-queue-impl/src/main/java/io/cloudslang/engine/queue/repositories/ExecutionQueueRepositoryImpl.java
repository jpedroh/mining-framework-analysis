...
@SuppressWarnings(value = { "FieldCanBeLocal" }) public class ExecutionQueueRepositoryImpl implements ExecutionQueueRepository {
  ...
  final private String SELECT_CANCELED_STEPS_IDS = " SELECT DISTINCT EXEC_STATE_ID FROM OO_EXECUTION_QUEUES " + "WHERE EXEC_STATE_ID IN " + "(SELECT DISTINCT ESS.ID FROM OO_EXECUTION_STATES ESS JOIN OO_EXECUTION_STATE ES ON " + 
...
>>>>>>> /usr/src/app/output/cloudslang/score/caeb87d5b1f92f72cb9fb76e630e7ded4e562e1a/engine/queue/score-queue-impl/src/main/java/io/cloudslang/engine/queue/repositories/ExecutionQueueRepositoryImpl.java/right.java
  ...
  @Override public void deleteFinishedSteps(Set<Long> ids) {
  ...
  public Set<Long> getCanceledExecStateIds() {
    ...
    try {
      List<Long> result = doSelectWithTemplate(getCanceledExecStateIdsJdbcTemplate, SELECT_CANCELED_STEPS_IDS, new SingleColumnRowMapper<>(Long.class));
      return new HashSet<>(result);
    }  finally {
