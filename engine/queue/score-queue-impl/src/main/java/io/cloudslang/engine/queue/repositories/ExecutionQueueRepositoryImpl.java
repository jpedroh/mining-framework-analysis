...
@SuppressWarnings(value = { "FieldCanBeLocal" }) public class ExecutionQueueRepositoryImpl implements ExecutionQueueRepository {
  ...
  final private String SELECT_FINISHED_STEPS_IDS = " SELECT DISTINCT EXEC_STATE_ID FROM OO_EXECUTION_QUEUES " + " WHERE " + "        (STATUS = " + ExecStatus.TERMINATED.getNumber() + ") OR " + "        (STATUS = " + ExecStatus.FAILED.getNumber() + ") OR " + "        (STATUS = " + ExecStatus.FINISHED.getNumber() + ")";

  final private String SELECT_CANCELED_STEPS_IDS = " SELECT DISTINCT EXEC_STATE_ID FROM OO_EXECUTION_QUEUES " + "WHERE EXEC_STATE_ID IN " + "(SELECT DISTINCT ESS.ID FROM OO_EXECUTION_STATES ESS JOIN OO_EXECUTION_STATE ES ON " + "ESS.MSG_ID = CAST(ES.EXECUTION_ID AS VARCHAR(255)) " + "WHERE ES.STATUS = \'PENDING_CANCEL\')";

  final private String SELECT_CANCELED_STEPS_IDS_MYSQL = " SELECT DISTINCT EXEC_STATE_ID FROM OO_EXECUTION_QUEUES " + "WHERE EXEC_STATE_ID IN " + "(SELECT DISTINCT ESS.ID FROM OO_EXECUTION_STATES ESS JOIN OO_EXECUTION_STATE ES ON " + "ESS.MSG_ID = CAST(ES.EXECUTION_ID AS CHAR(255)) " + "WHERE ES.STATUS = \'PENDING_CANCEL\')";
  ...
  public Set<Long> getCanceledExecStateIds() {
    ...
    try {
      List<Long> result = doSelectWithTemplate(getCanceledExecStateIdsJdbcTemplate, cancelExecQuery, new SingleColumnRowMapper<>(Long.class));
      return new HashSet<>(result);
    }  finally {
