...
@SuppressWarnings(value = { "FieldCanBeLocal" }) public class ExecutionQueueRepositoryImpl implements ExecutionQueueRepository {
  ...
  @Override public List<ExecutionMessage> pollRecovery(String workerId, int maxSize, ExecStatus... statuses) {
    ...
    String sqlStatActiveTable = QUERY_WORKER_RECOVERY_SQL.replaceAll(":status", StringUtils.repeat("?", ",", statuses.length));
    ...
    for (ExecutionMessage executionMessage : resultPrvTable) {
      resultAsMap.put(executionMessage.getExecStateId(), executionMessage);
    }
    for (ExecutionMessage executionMessage : resultActiveTable) {
      resultAsMap.put(executionMessage.getExecStateId(), executionMessage);
    }
    return new ArrayList<>(resultAsMap.values());
  }

  @Override public List<ExecutionMessage> poll(String workerId, int maxSize, ExecStatus... statuses) {
