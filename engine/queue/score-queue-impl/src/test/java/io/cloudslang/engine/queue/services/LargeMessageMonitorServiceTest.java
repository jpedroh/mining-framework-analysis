...
@SuppressWarnings(value = { "SpringContextConfigurationInspection" }) @RunWith(value = SpringJUnit4ClassRunner.class) @ContextConfiguration(classes = LargeMessageMonitorServiceTest.Configurator.class) public class LargeMessageMonitorServiceTest {
  @Autowired private CancelExecutionService cancelExecutionService;

  @Autowired private LargeMessagesMonitorService largeMessagesMonitorService;
  ...
  @Autowired private ExecutionQueueService executionQueueService;
  ...
  @Test public void testMonitorMessageReassigned() throws InterruptedException {
    int mb = 2;
  ...
  @Test public void testMonitorCancelExecution() throws InterruptedException {
    int mb = 2;
    long workerFreeMem = QueueTestsUtils.getMB(mb - 1);
    String worker = "worker1";
    String workerGroup = "group1";
  ...
  @Configuration static class Configurator {
    ...
    @Bean public IdentityGenerator identifierGenerator() {
    ...
    @Bean LargeMessagesMonitorService largeMessagesMonitorService() {
      return new LargeMessagesMonitorServiceImpl();
    }

    @Bean CancelExecutionService cancelExecutionService() {
    ...
    @Bean public WorkerNodeService workerNodeService() {
      return Mockito.mock(WorkerNodeService.class);
    }

    @Bean public ExecutionQueueService executionQueueService() {
      return new ExecutionQueueServiceImpl();
    }

    @Bean public ExecutionMessageConverter executionMessageConverter() {
      return Mockito.mock(ExecutionMessageConverter.class);
    }

    @Bean EngineVersionService engineVersionService() {
      EngineVersionService mock = mock(EngineVersionService.class);
      when(mock.getEngineVersionId()).thenReturn("");
      return mock;
    }

    @Bean public BusyWorkersService busyWorkersService() {
      return mock(BusyWorkersService.class);
    }

    @Bean public VersionService queueVersionService() {
      return mock(VersionService.class);
    }

    @Bean public ExecutionStateService executionStateService() {
