...
public class JavaTest {
  ...
  @Test public void get_method_works() {
  ...
  @Test public void can_do_query_params_and_headers() {
    JavalinTest.test((server, client) -> {
  ...
  @Test public void post_with_json_serialization_works() {
    JavalinTest.test((server, client) -> {
  ...
  @Test public void all_common_verbs_work() {
    JavalinTest.test((server, client) -> {
  ...
  @Test public void request_method_works() {
    JavalinTest.test((server, client) -> {
  ...
  @Test public void custom_javalin_works() {
  ...
  @Test public void capture_std_out_works() {
  ...
  @Test public void testing_full_app_works() {
    JavalinTest.test(new JavaApp().app, (server, client) -> {
      assertThat(client.get("/hello").body().string()).isEqualTo("Hello, app!");
      assertThat(client.get("/hello/").body().string()).isEqualTo("Endpoint GET /hello/ not found");
    });
  }

  @Test void custom_okHttpClient_is_used() {
    Javalin app = Javalin.create().get("/hello", (ctx) -> ctx.result("Hello, " + ctx.header("X-Welcome") + "!"));
    OkHttpClient okHttpClientAddingHeader = new OkHttpClient.Builder().addInterceptor((chain) -> {
      Request userRequest = chain.request();
      return chain.proceed(userRequest.newBuilder().addHeader("X-Welcome", "Javalin").build());
    }).build();
  ...
  @Test void instantiate_JavalinTestTool() {
    javalinTest.test((server, client) -> {
      server.get("/hello", (ctx) -> ctx.result("Hello world"));
      assertThat(client.get("/hello").body().string()).isEqualTo("Hello world");
    });
  ...
  public void exceptions_in_test_code_get_rethrown() {
    assertThatExceptionOfType(Exception.class).isThrownBy(() -> JavalinTest.test((server, client) -> {
      throw new Exception("Error in test code");
    })).withMessageMatching("Error in test code");
  }

  @Test public void exceptions_in_handler_code_are_caught_by_exception_handler_and_not_thrown() {
    assertThatNoException().isThrownBy(() -> JavalinTest.test((server, client) -> {
      server.get("/hello", (ctx) -> {
  ...
  @Test public void exception_in_handler_code_is_included_in_test_logs() {
