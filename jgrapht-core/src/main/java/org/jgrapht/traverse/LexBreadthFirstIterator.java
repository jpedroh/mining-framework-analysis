 ...
 * @since March 2018
 */
public class LexBreadthFirstIterator<V extends java.lang.Object, E extends java.lang.Object> extends AbstractGraphIterator<V, E> {
  private enum Mode {
    LBFS,
    LBFS_PLUS,
    LBFS_STAR
  ...
  /**
     ...
     */
  public LexBreadthFirstIterator(Graph<V, E> graph, Ordering<V> priorityA, Ordering<V> priorityB, Ordering<V> neighborIndexA, Ordering<V> neighborIndexB, HashMap<V, Set<V>> aSets, HashMap<V, Set<V>> bSets) {
    this(graph, priorityA, priorityB, neighborIndexA, neighborIndexB, aSets, bSets, Mode.LBFS_STAR);
  }

  private LexBreadthFirstIterator(Graph<V, E> graph, Ordering<V> priorityA, Ordering<V> priorityB, Ordering<V> neighborIndexA, Ordering<V> neighborIndexB, HashMap<V, Set<V>> aSets, HashMap<V, Set<V>> bSets, Mode mode) {
    super(graph);
...
<<<<<<< /usr/src/app/output/jgrapht/jgrapht/f8492e5cee829f64689c5dd8397f5529c1a7315b/jgrapht-core/src/main/java/org/jgrapht/traverse/LexBreadthFirstIterator.java/left.java
      new HashMap<>(verticesA.size())
=======
      CollectionUtil.newHashMapWithExpectedSize(vertices.size())
>>>>>>> /usr/src/app/output/jgrapht/jgrapht/f8492e5cee829f64689c5dd8397f5529c1a7315b/jgrapht-core/src/main/java/org/jgrapht/traverse/LexBreadthFirstIterator.java/right.java
    ...
    void updateBuckets(V vertex) {
      ...
      if (mode == Mode.LBFS) {
        ...
        neighbors.removeIf((n) -> !containsBucketWith(n));
      }
      ...
      if (mode == Mode.LBFS_STAR) {
        ...
        neighborsB.removeIf((n) -> !containsBucketWith(n));
    ...
    private class Bucket {
      ...
      V poll() {
        ...
        if (neighborIndexA.getPositionOf(alpha) > priorityA.getPositionOf(alpha)) {
          ...
          if (neighborIndexB.getPositionOf(beta) > priorityB.getPositionOf(beta)) {
            result = alpha;
          } else {
            if (bSets.get(beta).isEmpty() || !aSets.get(alpha).isEmpty()) {
              result = beta;
            } else {
              if (Objects.equals(neighborIndexA.getPositionOf(bSets.get(beta).iterator().next()), priorityA.getPositionOf(alpha))) {
                result = beta;
              } else {
                result = alpha;
              }
