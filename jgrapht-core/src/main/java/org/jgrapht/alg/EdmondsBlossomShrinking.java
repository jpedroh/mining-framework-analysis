 ...
 * @since Jan 24, 2012
 */
public class EdmondsBlossomShrinking<V extends java.lang.Object, E extends java.lang.Object> implements MatchingAlgorithm<V, E> {
  ...
  private Set<E> findMatch() {
    ...
    graph.vertexSet().stream().filter((v) -> !seen.contains(v) && match.containsKey(v)).forEach((v) -> {
      seen.add(v);
    ...
    while (!q.isEmpty()) {
      ...
      for (E e : graph.edgesOf(v)) {
        ...
        if ((to.equals(root)) || ((match.containsKey(to)) && (path.containsKey(match.get(to))))) {
          ...
          graph.vertexSet().stream().filter((i) -> contracted.containsKey(i) && blossom.contains(contracted.get(i))).forEach((i) -> {
