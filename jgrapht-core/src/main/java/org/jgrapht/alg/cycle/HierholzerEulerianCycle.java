 ...
 * @since October 2016
 */
public class HierholzerEulerianCycle<V extends java.lang.Object, E extends java.lang.Object> implements EulerianCycleAlgorithm<V, E> {
  private Graph<V, E> g;
    ...
    if (graph.vertexSet().isEmpty()) {
      ...
      if (graph.edgeSet().isEmpty()) {
        ...
        if (graph.getType().isUndirected()) {
          ...
          boolean foundComponentWithEdges = false;
          for (Set<V> component : new KosarajuStrongConnectivityInspector<V, E>(graph).stronglyConnectedSets()) {
            for (V v : component) {
  ...
  /**
     ...
     */
  private void updateGraphAndInsertLocations(Pair<EdgeNode, EdgeNode> partialCycle, VertexNode partialCycleSourceVertex) {
  ...
  private void unlink(VertexNode vNode) {
    if (verticesHead == null) {
      ...
      if (!verticesHead.equals(vNode) && vNode.prev == null && vNode.next == null) {
        return;
      } else {
        if (vNode.prev != null) {
  ...
  class EdgeNode {
    ...
    EdgeNode(VertexNode sourceNode, VertexNode targetNode, EdgeNode prev, E e, EdgeNode reverse, EdgeNode next) {
