 ...
 */
public class BetweennessCentrality<V extends java.lang.Object, E extends java.lang.Object> implements VertexScoringAlgorithm<V, Double> {
  /**
     * Underlying graph
     */
  ...
  /**
     * {@inheritDoc}
     */
  @Override public Map<V, Double> getScores() {
    if (scores == null) {
      compute();
    }
  ...
  /**
     * {@inheritDoc}
     */
  @Override public Double getVertexScore(V v) {
  ...
  private void compute(V s) {
    ...
    distance.put(s, 0.0);
    MyQueue<V, Double> queue = graph.getType().isWeighted() ? new WeightedQueue() : new UnweightedQueue();
    queue.insert(s, 0.0);
    while (!queue.isEmpty()) {
      V v = queue.remove();
      stack.push(v);
    ...
    while (!stack.isEmpty()) {
      ...
      for (V v : predecessors.get(w)) {
        dependency.put(v, dependency.get(v) + (sigma.get(v).doubleValue() / sigma.get(w).doubleValue()) * (1 + dependency.get(w)));
      }
      if (!w.equals(s)) {
  ...
  private interface MyQueue<T extends java.lang.Object, D extends java.lang.Object> {
    void insert(T t, D d);
  ...
  private class WeightedQueue implements MyQueue<V, Double> {
    ...
    @Override public void insert(V t, Double d) {
      AddressableHeap.Handle<Double, V> node = delegate.insert(d, t);
      seen.put(t, node);
    }

    @Override public void update(V t, Double d) {
