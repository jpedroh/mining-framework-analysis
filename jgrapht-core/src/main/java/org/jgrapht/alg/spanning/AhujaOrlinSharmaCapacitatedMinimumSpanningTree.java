 ...
 * @since July 11, 2018
 */
public class AhujaOrlinSharmaCapacitatedMinimumSpanningTree<V extends java.lang.Object, E extends java.lang.Object> extends AbstractCapacitatedMinimumSpanningTree<V, E> {
  /**
  ...
  /**
     ...
     * @param upperLimitTabuExchanges     the upper limit of non-improving exchanges, this is the stopping criterion in the tabu search
     */
  ...
  /**
     ...
     * @param upperLimitTabuExchanges the upper limit of non-improving exchanges, this is the stopping criterion in the tabu search
     */
  public AhujaOrlinSharmaCapacitatedMinimumSpanningTree(CapacitatedSpanningTree<V, E> initialSolution, Graph<V, E> graph, V root, double capacity, Map<V, Double> demands, int lengthBound, boolean bestImprovement, boolean useVertexOperation, boolean useSubtreeOperation, boolean useTabuSearch, int tabuTime, int upperLimitTabuExchanges) {
  ...
  @Override public CapacitatedSpanningTree<V, E> getCapacitatedSpanningTree() {
    ...
    double currentCost;
    while (true) {
      partitionSpanningTrees = calculateSpanningTrees(currentSolution, partitionSpanningTrees, affected.getFirst());
      if (useSubtreeOperation) {
        subtrees = calculateSubtreesOfVertices(currentSolution, subtrees, partitionSpanningTrees, affected.getFirst());
      }
      ...
      if (useTabuSearch) {
        if (currentCost < 0) {
          affected = executeNeighborhoodOperation(currentSolution, improvementGraph.improvementGraphVertexMapping, improvementGraph.pathExchangeVertexMapping, subtrees, cycle);
          if (costDifference < 0) {
  ...
  private Map<Integer, SpanningTreeAlgorithm.SpanningTree<E>> calculateSpanningTrees(CapacitatedSpanningTreeSolutionRepresentation currentSolution, Map<Integer, SpanningTreeAlgorithm.SpanningTree<E>> partitionSpanningTrees, Set<Integer> affectedLabels) {
    for (Integer label : affectedLabels) {
      ...
      partitionSpanningTrees.put(label, new PrimMinimumSpanningTree<>(new AsSubgraph<>(graph, set)).getSpanningTree());
  ...
  private Map<V, Pair<Set<V>, Double>> calculateSubtreesOfVertices(CapacitatedSpanningTreeSolutionRepresentation currentSolution, Map<V, Pair<Set<V>, Double>> subtrees, Map<Integer, SpanningTreeAlgorithm.SpanningTree<E>> partitionSpanningTree, Set<Integer> affectedLabels) {
    for (Integer label : affectedLabels) {
      ...
      for (V v : currentSolution.getPartitionSet(label)) {
        Pair<Set<V>, Double> currentSubtree = subtree(currentSolution, modifiableSet, v, partitionSpanningTree);
  ...
  private Pair<Set<V>, Double> subtree(CapacitatedSpanningTreeSolutionRepresentation currentSolution, Set<V> modifiableSet, V v, Map<Integer, SpanningTreeAlgorithm.SpanningTree<E>> partitionSpanningTree) {
    ...
    Graph<V, E> spanningTree = new AsSubgraph<>(graph, modifiableSet, partSpanningTree.getEdges());
  ...
  private class ImprovementGraph {
    ...
    /**
         ...
         */
    public void updateImprovementGraph(CapacitatedSpanningTreeSolutionRepresentation currentSolution, Map<V, Pair<Set<V>, Double>> subtrees, Map<Integer, SpanningTreeAlgorithm.SpanningTree<E>> partitionSpanningTrees, Set<Integer> labelsToUpdate, Set<V> tabuList) {
      ...
      for (V v1 : graph.vertexSet()) {
        ...
        updateOriginNodeConnections(currentSolution, subtrees, partitionSpanningTrees, labelsToUpdate, v1, vertexOfV1Single, vertexOfV1Subtree);
        for (Integer label : currentSolution.getLabels()) {
          if (label.equals(currentSolution.getLabel(v1)) || (!labelsToUpdate.contains(currentSolution.getLabel(v1)) && !labelsToUpdate.contains(label))) {
            continue;
          }
          ...
          updateSingleNode(currentSolution, subtrees, tabuList, label, oldWeight, modifiableSet, pseudoVertex, v1, vertexOfV1Single);
          updateSubtreeNode(currentSolution, subtrees, tabuList, label, oldWeight, modifiableSet, pseudoVertex, v1, vertexOfV1Subtree);
        }
      }
    }

    /**
         * Updates the pseudo nodes corresponding to new subsets in the partition. That is, new pseudo nodes for new
         * labels in the label set are added and pseudo nodes of labels that are no more in the label set are removed.
         *
      ...
      if (!currentSolution.getLabels().equals(pseudoVertexMapping.keySet())) {
        for (Integer label : currentSolution.getLabels()) {
          if (!pseudoVertexMapping.keySet().contains(label)) {
            ...
            pathExchangeVertexMapping.put(pseudoVertex, label);
            improvementGraph.addVertex(pseudoVertex);
            DefaultWeightedEdge newEdge = improvementGraph.addEdge(pseudoVertex, origin);
    ...
    /**
         * Updates all nodes that correspond to <code>v1</code> and returns if the vertex <code>v1</code>. That is, all
         * incident edges of <code>v1</code> are removed if <code>v1</code> is in the tabu list.
         *
    ...
    private void updateOriginNodeConnections(CapacitatedSpanningTreeSolutionRepresentation currentSolution, Map<V, Pair<Set<V>, Double>> subtrees, Map<Integer, SpanningTreeAlgorithm.SpanningTree<E>> partitionSpanningTrees, Set<Integer> labelsToUpdate, V v1, Pair<Integer, ImprovementGraphVertexType> vertexOfV1Single, Pair<Integer, ImprovementGraphVertexType> vertexOfV1Subtree) {
      ...
      if (labelsToUpdate.contains(currentSolution.getLabel(v1))) {
        ...
        Set<V> partitionSetOfV1 = currentSolution.getPartitionSet(currentSolution.getLabel(v1));
    ...
    /**
         * Updates all edges from <code>vertexOfV1Single</code> to nodes in the subset represented by <code>label</code>.
         *
         * @param currentSolution  the current solution in the iteration
         * @param subtrees         the mapping from vertices to their subtree
         * @param tabuList         the tabu list of the current iteration
         ...
         * @param modifiableSet    a modifiable version of the subset of nodes represented by label inclusive the root node
      ...
      if (useVertexOperation) {
        for (V v2 : currentSolution.getPartitionSet(label)) {
          ...
          if (spanningTree.getEdges().size() == modifiableSet.size() - 1) {
            newCapacity = calculateMaximumDemandOfSubtrees(modifiableSet, spanningTree, currentSolution.getPartitionWeight(label) + demands.get(v1) - demands.get(v2));
            newWeight = spanningTree.getWeight();
          } else {
          ...
          if (useSubtreeOperation) {
            if (subtrees.get(v2).getFirst().size() > 1) {
              ...
              spanningTree = new PrimMinimumSpanningTree<>(new AsSubgraph<>(graph, modifiableSet, graph.edgeSet())).getSpanningTree();
              if (spanningTree.getEdges().size() == modifiableSet.size() - 1) {
                newCapacity = calculateMaximumDemandOfSubtrees(modifiableSet, spanningTree, currentSolution.getPartitionWeight(label) + demands.get(v1) - subtrees.get(v2).getSecond());
                newWeight = spanningTree.getWeight();
              } else {
                newCapacity = Double.NaN;
                newWeight = Double.NaN;
        ...
        spanningTree = new PrimMinimumSpanningTree<>(new AsSubgraph<>(graph, modifiableSet, graph.edgeSet())).getSpanningTree();
        if (spanningTree.getEdges().size() == modifiableSet.size() - 1) {
          newCapacity = calculateMaximumDemandOfSubtrees(modifiableSet, spanningTree, currentSolution.getPartitionWeight(label) + demands.get(v1));
          newWeight = spanningTree.getWeight();
        } else {
          newCapacity = Double.NaN;
          newWeight = Double.NaN;
    ...
    /**
         * Updates all edges from <code>vertexOfV1Single</code> to nodes in the subset represented by <code>label</code>.
         * This method does adds the subtree of v1 to <code>modifiableSet</code>.
         *
         * @param currentSolution   the current solution in the iteration
         * @param subtrees          the mapping from vertices to their subtree
         * @param tabuList          the tabu list of the current iteration
      ...
      if (useSubtreeOperation && (subtrees.get(v1).getFirst().size() > 1 || !useVertexOperation)) {
        ...
        for (V v2 : currentSolution.getPartitionSet(label)) {
          ...
          if (useVertexOperation) {
            ...
            spanningTree = new PrimMinimumSpanningTree<>(new AsSubgraph<>(graph, modifiableSet, graph.edgeSet())).getSpanningTree();
            if (spanningTree.getEdges().size() == modifiableSet.size() - 1) {
              newCapacity = calculateMaximumDemandOfSubtrees(modifiableSet, spanningTree, currentSolution.getPartitionWeight(label) + subtrees.get(v1).getSecond() - demands.get(v2));
              newWeight = spanningTree.getWeight();
            } else {
              newCapacity = Double.NaN;
              newWeight = Double.NaN;
          ...
          spanningTree = new PrimMinimumSpanningTree<>(new AsSubgraph<>(graph, modifiableSet, graph.edgeSet())).getSpanningTree();
          if (spanningTree.getEdges().size() == modifiableSet.size() - 1) {
            newCapacity = calculateMaximumDemandOfSubtrees(modifiableSet, spanningTree, currentSolution.getPartitionWeight(currentSolution.getLabel(v2)) + subtrees.get(v1).getSecond() - subtrees.get(v2).getSecond());
            newWeight = spanningTree.getWeight();
          } else {
            newCapacity = Double.NaN;
            newWeight = Double.NaN;
        ...
        spanningTree = new PrimMinimumSpanningTree<>(new AsSubgraph<>(graph, modifiableSet, graph.edgeSet())).getSpanningTree();
        if (spanningTree.getEdges().size() == modifiableSet.size() - 1) {
          newCapacity = calculateMaximumDemandOfSubtrees(modifiableSet, spanningTree, currentSolution.getPartitionWeight(label) + subtrees.get(v1).getSecond());
          newWeight = spanningTree.getWeight();
        } else {
          newCapacity = Double.NaN;
          newWeight = Double.NaN;
    ...
    /**
         * Returns the mapping that is used in the valid cycle detection algorithm, i.e. the vertex label map.
         *
      ...
      return new AbstractMap<Pair<Integer, ImprovementGraphVertexType>, Integer>() {
        ...
        @Override public boolean containsKey(Object key) {
          if (key instanceof Pair) {
            return improvementGraphVertexMapping.containsKey(((Pair) key).getFirst()) || pathExchangeVertexMapping.containsKey(key) || key.equals(origin);
          }
          return false;
        }
...
<<<<<<< /usr/src/app/output/jgrapht/jgrapht/6ddc805389528a2dbbc97aef3dc8e1df351ddb87/jgrapht-core/src/main/java/org/jgrapht/alg/spanning/AhujaOrlinSharmaCapacitatedMinimumSpanningTree.java/left.java
...
>>>>>>> /usr/src/app/output/jgrapht/jgrapht/6ddc805389528a2dbbc97aef3dc8e1df351ddb87/jgrapht-core/src/main/java/org/jgrapht/alg/spanning/AhujaOrlinSharmaCapacitatedMinimumSpanningTree.java/right.java
        ...
        @Override public Integer put(Pair<Integer, ImprovementGraphVertexType> key, Integer value) {
        ...
        @Override public void putAll(Map<? extends Pair<Integer, ImprovementGraphVertexType>, ? extends Integer> m) {
          throw new IllegalStateException();
        }
        ...
        @Override public Set<Pair<Integer, ImprovementGraphVertexType>> keySet() {
        ...
        @Override public Set<Entry<Pair<Integer, ImprovementGraphVertexType>, Integer>> entrySet() {
