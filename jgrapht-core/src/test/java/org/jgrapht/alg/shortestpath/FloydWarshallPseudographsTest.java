...
public class FloydWarshallPseudographsTest {
  @Test @Category(value = SlowTests.class) public void testRandomGraphs() {
    final int tests = 20;
    final int n = 50;
    final double p = 0.85;
    ...
    for (Supplier<Graph<Integer, DefaultWeightedEdge>> gSupplier : graphs) {
      GraphGenerator<Integer, DefaultWeightedEdge, Integer> gen = new GnpRandomGraphGenerator<>(n, p, rng, true);
      for (int i = 0; i < tests; i++) {
        ...
        FloydWarshallShortestPaths<Integer, DefaultWeightedEdge> fw = new FloydWarshallShortestPaths<>(g);
        for (Integer v : g.vertexSet()) {
          SingleSourcePaths<Integer, DefaultWeightedEdge> dTree = new DijkstraShortestPath<>(g).getPaths(v);
          ...
          for (Integer u : g.vertexSet()) {
            ...
            assertEquals(fwTree.getPath(u).getEdgeList(), fw.getPath(v, u).getEdgeList());
          }
        }
      }
    }
  ...
  @Test public void test1() {
    DirectedWeightedPseudograph<Integer, DefaultWeightedEdge> g = new DirectedWeightedPseudograph<>(DefaultWeightedEdge.class);
  ...
  @Test public void testGetPathWeight() {
    DirectedWeightedPseudograph<Integer, DefaultWeightedEdge> g = new DirectedWeightedPseudograph<>(DefaultWeightedEdge.class);
    Graphs.addAllVertices(g, Arrays.asList(1, 2, 3, 4));
    DefaultWeightedEdge e12_1 = g.addEdge(1, 2);
    g.setEdgeWeight(e12_1, -5.0);
