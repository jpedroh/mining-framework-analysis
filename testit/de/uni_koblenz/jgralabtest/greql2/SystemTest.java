...
public class SystemTest extends GenericTest {
  @Override protected Graph createGreqlTestGraph() {
    int count = 1;
    ...
    queryString.append(" add(i, mul( a, sub(b, mod( c, sub( d, add( e)))))) ");
    for (int i = 0; i < count; i++) {
  ...
  @Test public void testFunctionArgumentsEvaluation() throws Exception {
    ...
    Object result = evalTestQuery("FunctionArgumentsEvaluation", queryString);
    assertEquals(11, ((PCollection<?>) result).size());
  }

  @Test public void testFunctionAsFunctionArgumentEvaluation() throws Exception {
    ...
    Object result = evalTestQuery("FunctionAsFunctionArgumentEvaluation", queryString);
    assertEquals(5, ((PCollection<?>) result).size());
  }

  @Test public void testFunctionAsFunctionArgumentAsFuntionArgumentEvaluation() throws Exception {
    ...
    Object result = evalTestQuery("FunctionAsFunctionAsFunctionArgumentEvaluation", queryString);
    assertEquals(4, ((PCollection<?>) result).size());
  }

  @Test public void testFunctionAsArgumentInListComprehensionEvaluation() throws Exception {
    ...
    Object result = evalTestQuery("FunctionAsArgumentInListComprehension", queryString);
    assertEquals(5, ((PCollection<?>) result).size());
  }
  ...
  @Test public void test() throws Exception {
    String X1 = "X1";
    String X2 = "X2";
    String queryString = "from x:V{junctions.Crossroad}, y:V{junctions.Crossroad} " + "with x -->{connections.Street} <--{connections.Footpath} y report id(x) as \'" + X1 + "\', id(y) as \'" + X2 + "\' end";
  ...
  @Test public void testCrossroadWithUsage() throws Exception {
    String VERTEX = "Vertex";
    String IDENTIFIER = "Identifier";
    String USAGE_COUNT = "UsageCount";
    String USAGES = "Usages";
    String queryString = "from c:V{junctions.Crossroad} report c as \'" + VERTEX + "\', id(c) as \'" + IDENTIFIER + "\', " + "outDegree{connections.Way}(c) as \'" + USAGE_COUNT + "\', " + "edgesFrom(c) as \'" + USAGES + "\' end";
    Table<?> result = (Table<?>) evalTestQuery(queryString);
    @SuppressWarnings(value = { "unchecked" }) PVector<Tuple> data = (PVector<Tuple>) result.toPVector();
    ...
    for (Tuple tuple : data) {
      ...
      assertEquals(vertex.getDegree(Way.EC, EdgeDirection.OUT), usage_count);
      int n = 0;
  ...
  @Test public void testSimpleQuery() {
    ...
    new QueryImpl(query).evaluate(null, new GreqlEnvironmentAdapter(boundVars));
