...
public class SaturnExecutor {
  ...
  private SaturnExecutor(String namespace, String executorName, ClassLoader executorClassLoader, ClassLoader jobClassLoader, Object saturnApplication) {
    this.executorName = executorName;
    this.namespace = namespace;
    this.executorClassLoader = executorClassLoader;
    this.jobClassLoader = jobClassLoader;
    this.saturnApplication = saturnApplication;
    this.raiseAlarmExecutorService = Executors.newSingleThreadExecutor(new SaturnThreadFactory(executorName + "-raise-alarm-thread", false));
    this.shutdownJobsExecutorService = Executors.newCachedThreadPool(new SaturnThreadFactory(executorName + "-shutdownJobSchedulers-thread", true));
    initRestartThread();
          ...
          while (true) {
            ...
            if (needRestart) {
              try {
                ...
                needRestart = true;
                LogUtils.error(log, LogEvents.ExecutorEvent.REINIT, "Executor {} reinitialize failed, will retry again", executorName, t);
              }
            }
            Thread.sleep(1000L);
          }
  ...
  /**
	 * SaturnExecutor工厂入口
	 */
  public static SaturnExecutor buildExecutor(String namespace, String executorName, ClassLoader executorClassLoader, ClassLoader jobClassLoader, Object saturnApplication) {
    ...
    try {
      ...
      try {
        ...
        if (saturnApplicationClass.isAssignableFrom(appClass)) {
          ...
          throw new RuntimeException("the app.class " + appClassStr + " must be instance of " + SATURN_APPLICATION_CLASS);
        }
      }  finally {
    ...
    for (int i = 0; i < size; i++) {
      ...
      try {
        ...
        if (statusLine != null && statusCode.intValue() == HttpStatus.SC_OK) {
          Map<String, String> discoveryInfo = JsonUtils.getGson().fromJson(responseBody, new TypeToken<Map<String, String>>() { }.getType());
  ...
  public void execute() throws Exception {
    ...
    try {
      ...
      try {
        ...
        LogUtils.info(log, LogEvents.ExecutorEvent.INIT, "The executor {} start successfully which used {} ms", executorName, System.currentTimeMillis() - startTime);
      } catch (Throwable t) {
  ...
  private void shutdownGracefully0() throws Exception {
    shutdownLock.lockInterruptibly();
    try {
      LogUtils.info(log, LogEvents.ExecutorEvent.GRACEFUL_SHUTDOWN, "Try to stop executor {} gracefully", executorName);
      if (saturnExecutorService != null) {
        saturnExecutorService.unregisterJobsWatcher();
      }
      ...
      if (saturnApplication != null) {
        ...
        try {
          ...
          LogUtils.error(log, LogEvents.ExecutorEvent.GRACEFUL_SHUTDOWN, "SaturnApplication destroy error", t);
        } finally {
          Thread.currentThread().setContextClassLoader(oldCL);
        }
      }
      LogUtils.info(log, LogEvents.ExecutorEvent.GRACEFUL_SHUTDOWN, "executor {} is stopped gracefully", executorName);
    }  finally {
      shutdownLock.unlock();
    }
  }
