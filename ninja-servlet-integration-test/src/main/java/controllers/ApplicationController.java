...
@Singleton public class ApplicationController {
  ...
  @Timed public Result index(@Request HttpServletRequest httpServletRequest, @Response HttpServletResponse httpServletResponse, Context context) {
  ...
  @Timed public Result testPage() {
    return Results.html();
  }

  @Timed public Result userDashboard(@PathParam(value = "email") String email, @PathParam(value = "id") Integer id, Context context) {
  ...
  @Timed public Result validation(Validation validation, @Param(value = "email") @Required String email) {
  ...
  @Timed public Result redirect(Context context) {
    return Results.redirect("/");
  }

  @Timed public Result session(Session session) {
    session.put("username", "kevin");
    return Results.html();
  }

  @Timed public Result flashSuccess(FlashScope flashScope, Context context) {
  ...
  @Timed public Result flashError(Context context, FlashScope flashScope) {
    Result result = Results.html();
  ...
  @Timed public Result flashAny(Context context, FlashScope flashScope) {
    Result result = Results.html();
  ...
  @Timed public Result postForm(Context context, FormObject formObject) {
    return Results.json().render(formObject);
  }

  @Timed public Result directObjectTemplateRendering() {
  ...
  @Timed public Result testJsonP() {
    return Results.jsonp().render("object", "value");
  }

  @Timed public Result badRequest() {
    throw new BadRequestException("Just a test to make sure 400 bad request works :)");
  }

  @Timed public Result testReverseRouting() {
    return Results.html().render("email", "me@me.com").render("id", "100000");
  }

  @Timed public Result testGetContextPathWorks(Context context) {
    return Results.html().render("ninjaPropertiesGetContextPath", ninjaProperties.getContextPath()).render("contextGetContextPath", context.getContextPath());
  }
}