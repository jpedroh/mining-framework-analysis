...
public class JQMCalBox extends JQMText {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "deprecation" }) private static final double NULL_DATE = new Date(70, 0, 1).getTime();
  ...
  public void setUsePickers(Boolean usePickers) {
    ...
    if (this.usePickers != null && this.usePickers && noHeader == null) {
      noHeader = true;
    }
    refreshDataOptions();
  }
  ...
  public Date getDate() {
    ...
    if (!isReady()) {
      return delayedSetDate;
    }
    if (isInternSetDate) {
      return internDateToSet;
    }
  ...
  public String getIso8601() {
    ...
    if (!isReady()) {
      return dateAsIso8601(delayedSetDate);
    }
    if (isInternSetDate) {
      return dateAsIso8601(internDateToSet);
    }
    String s = input.getText();
    if (s == null || s.isEmpty()) {
      return null;
  ...
  @SuppressWarnings(value = { "deprecation" }) public static String dateAsIso8601(Date d) {
    if (d == null) {
      return null;
    }
  ...
  private void formatGridDateEx(int yyyy, int mm, int dd, String iso8601, boolean selectedDateVisible, JavaScriptObject result) {
    if (!(gridDateFormatter instanceof GridDateFormatterEx)) {
      ...
      String cls = ((GridDateFormatterEx) gridDateFormatter).getStyleNames(yyyy, mm, dd, iso8601, selectedDateVisible);
  ...
  private void initDisplayChange() {
    if (!calBoxHandlerAdded || !isReady()) {
      return;
    }
    initDisplayChange(input.getElement(), this);
  }

  private void fireDisplayChange(JsDate shownDate, JsDate selectedDate, String thisChange, int thisChangeAmount) {
    JQMCalBoxEvent.fire(this, new DisplayChangeData(JQMContext.jsDateToDate(shownDate), JQMContext.jsDateToDate(selectedDate), thisChange, thisChangeAmount));
  }
}