...
public class Plotter {
  ...
  private static class MyActivityRenderer extends XYLineAndShapeRenderer {
    ...
    @Override public Shape getItemShape(int seriesIndex, int itemIndex) {
    ...
    @Override public Paint getItemOutlinePaint(int seriesIndex, int itemIndex) {
      XYDataItem dataItem = seriesCollection.getSeries(seriesIndex).getDataItem(itemIndex);
      if (firstActivities.contains(dataItem)) {
        return Color.BLACK;
      }
    ...
    @Override public Paint getItemPaint(int seriesIndex, int itemIndex) {
      XYDataItem dataItem = seriesCollection.getSeries(seriesIndex).getDataItem(itemIndex);
  ...
  /**
	 ...
	 */
  @Deprecated public Plotter setShowFirstActivity(boolean show) {
    return this;
  }
  ...
  private XYItemRenderer getShipmentRenderer(XYSeriesCollection shipments) {
    XYItemRenderer shipmentsRenderer = new XYLineAndShapeRenderer(true, false);
    for (int i = 0; i < shipments.getSeriesCount(); i++) {
      shipmentsRenderer.setSeriesPaint(i, Color.DARK_GRAY);
      shipmentsRenderer.setSeriesStroke(i, new BasicStroke(1.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 1.f, new float[] { 4.0f, 4.0f }, 0.0f));
    }
  ...
  private MyActivityRenderer getProblemRenderer(final XYSeriesCollection problem) {
    ...
    problemRenderer.setBaseItemLabelGenerator(new XYItemLabelGenerator() {
      @Override public String generateLabel(XYDataset arg0, int arg1, int arg2) {
  ...
  private XYItemRenderer getRouteRenderer(XYSeriesCollection solutionColl) {
    XYItemRenderer solutionRenderer = new XYLineAndShapeRenderer(true, false);
