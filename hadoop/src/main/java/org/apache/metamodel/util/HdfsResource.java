...
public class HdfsResource extends AbstractResource implements Serializable {
  ...
  private class HdfsDirectoryInputStream extends AbstractDirectoryInputStream<FileStatus> {
    ...
    public HdfsDirectoryInputStream(final Path hadoopPath, final FileSystem fs) {
      ...
      try {
        fileStatuses = _fs.listStatus(_hadoopPath, new PathFilter() {
          @Override public boolean accept(final Path path) {
            try {
    ...
    @Override InputStream openStream(final int index) throws IOException {
  ...
  @Override public OutputStream write() throws ResourceException {
    final FileSystem fs = getHadoopFileSystem();
    try {
  ...
  @Override public OutputStream append() throws ResourceException {
    final FileSystem fs = getHadoopFileSystem();
    try {
  ...
  @Override public InputStream read() throws ResourceException {
    final FileSystem fs = getHadoopFileSystem();
  ...
  @Override public boolean equals(Object obj) {
    ...
    if (_filepath == null) {
      ...
      if (!_filepath.equals(other._filepath)) {
        return false;
      }
    }
    if (_hostname == null) {
      ...
      if (!_hostname.equals(other._hostname)) {
        return false;
      }
    }
