...
public class ITextOutputDevice extends AbstractOutputDevice implements OutputDevice {
  ...
  private void processLinkAccessible1(RenderingContext c, InlineLayoutBox parentBox, BlockBox parentBlockBox) {
    ...
    if (elem != null) {
      ...
      if (uri != null) {
        ...
        float yPos = targetArea.getBottom() + (((targetArea.getTop() - targetArea.getBottom()) / 2) * 0.4f);
  ...
  private PdfAnnotation getLinkAsAnnotation(RenderingContext c, Box box, com.itextpdf.text.Rectangle targetArea, String uri, String title) {
  ...
  private com.itextpdf.text.Rectangle createLocalTargetArea(RenderingContext c, Box box, boolean useAggregateBounds) {
    ...
    com.itextpdf.text.Rectangle result = new com.itextpdf.text.Rectangle((float) pdfCorner.getX(), (float) pdfCorner.getY(), (float) pdfCorner.getX() + getDeviceLength(bounds.width), (float) pdfCorner.getY() + getDeviceLength(bounds.height));
    return result;
  }
  ...
  public com.itextpdf.text.Rectangle createTargetArea(RenderingContext c, Box box) {
    ...
    if (inCurrentPage || box.isContainedInMarginBox()) {
      ...
      float bottom = getDeviceLength(page.getBottom() - (bounds.y + bounds.height) + page.getMarginBorderPadding(c, CalculatedStyle.BOTTOM));
      float left = getDeviceLength(page.getMarginBorderPadding(c, CalculatedStyle.LEFT) + bounds.x);
      com.itextpdf.text.Rectangle result = new com.itextpdf.text.Rectangle(left, bottom, left + getDeviceLength(bounds.width), bottom + getDeviceLength(bounds.height));
      return result;
    }
  }
  ...
  public void setColor(FSColor color) {
    if (color instanceof FSRGBColor) {
      ...
      if (color instanceof FSCMYKColor) {
...
>>>>>>> /usr/src/app/output/flyingsaucerproject/flyingsaucer/3417ec357d5b62fb2c32e59a27baf10ddc5405c0/flying-saucer-pdf-itext5/src/main/java/org/xhtmlrenderer/pdf/ITextOutputDevice.java/right.java
...
>>>>>>> /usr/src/app/output/flyingsaucerproject/flyingsaucer/3417ec357d5b62fb2c32e59a27baf10ddc5405c0/flying-saucer-pdf-itext5/src/main/java/org/xhtmlrenderer/pdf/ITextOutputDevice.java/right.java
  ...
  public void drawStringAccessible(InlineText inlineText, String s, float x, float y, JustificationInfo info) {
    ...
    if (isListItem(parentBlockBox)) {
      ...
      if (isDescriptionListItem(parentBlockBox)) {
    ...
    if (oldOk) {
      if (nStroke.getDashArray() != null) {
        if (nStroke.getDashPhase() != oStroke.getDashPhase()) {
          ...
          if (!java.util.Arrays.equals(nStroke.getDashArray(), oStroke.getDashArray())) {
            makeDash = true;
          } else {
            makeDash = false;
          }
  ...
  @Override public void drawText(RenderingContext c, InlineText inlineText) {
    ...
    if (text != null && text.length() > 0) {
      ...
      if (inlineText.getParent().getStyle().isTextJustify()) {
        ...
        if (info != null) {
          ITextTextRendererAccessible.drawStringAccessible(c.getOutputDevice(), inlineText, text, iB.getAbsX() + inlineText.getX(), iB.getAbsY() + iB.getBaseline(), info);
        } else {
          ITextTextRendererAccessible.drawStringAccessible(c.getOutputDevice(), inlineText, text, iB.getAbsX() + inlineText.getX(), iB.getAbsY() + iB.getBaseline());
        }
      } else {
        ITextTextRendererAccessible.drawStringAccessible(c.getOutputDevice(), inlineText, text, iB.getAbsX() + inlineText.getX(), iB.getAbsY() + iB.getBaseline());
      }
  ...
  /**
     * PDF/UA
     */
  @Override public void closeOpenTags() {
