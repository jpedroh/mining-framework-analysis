...
public class CircuitBreaker implements MonitoredService, ServiceWrapper {
  ...
  protected List<CircuitBreakerNotificationCallback> cbNotifyList = Collections.synchronizedList(new ArrayList<CircuitBreakerNotificationCallback>());
  ...
  /** Wrap the given service call with the {@link CircuitBreaker}
     ...
     *    execution
     */
  public <V extends java.lang.Object> V invoke(Callable<V> c) throws Exception {
    if (!byPass) {
      if (!allowRequest()) {
  ...
  /** Wrap the given service call with the {@link CircuitBreaker}
     ...
     *    execution
     */
  public <V extends java.lang.Object> V invoke(Runnable r, V result) throws Exception {
    if (!byPass) {
      if (!allowRequest()) {
        throw mapException(new CircuitBreakerException());
      }
  ...
  public ServiceStatus getServiceStatus() {
    boolean canSendProbeRequest = !isHardTrip && lastFailure.get() > 0 && allowRequest();
    if (byPass) {
      return new ServiceStatus(name, Status.DEGRADED, "Bypassed");
    }
    switch (state) {
      case OPEN:
      return (canSendProbeRequest ? new ServiceStatus(name, Status.DEGRADED, "Send Probe Request") : new ServiceStatus(name, Status.DOWN, "Open"));
      case HALF_CLOSED:
      return new ServiceStatus(name, Status.DEGRADED, "Half Closed");
      case CLOSED:
      default:
  ...
  public void setIgnore(Collection<Class<? extends Throwable>> ignore) {
    ...
    @SuppressWarnings(value = { "unchecked" }) Class<? extends Throwable>[] classes = new Class[ignore.size()];
    int i = 0;
  ...
  protected boolean allowRequest() {
    ...
    if (BreakerState.OPEN == state && clock.currentTimeMillis() - lastFailure.get() >= resetMillis.get()) {
