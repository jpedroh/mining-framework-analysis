...
public class CdiRunner extends BlockJUnit4ClassRunner {
  ...
  private void initWeld(final TestConfiguration testConfig) {
    ...
    try {
      ...
      weld = new Weld() {
        ...
        @SuppressWarnings(value = { "unused" }) protected Deployment createDeployment(ResourceLoader resourceLoader, Bootstrap bootstrap) {
          try {
  ...
  private <T extends java.lang.Object> T createTest(Class<T> testClass) {
  ...
  @Override protected Statement methodBlock(final FrameworkMethod frameworkMethod) {
    this.frameworkMethod = frameworkMethod;
    final Statement defaultStatement = super.methodBlock(frameworkMethod);
    return new Statement() {
      @Override public void evaluate() throws Throwable {
        if (startupException != null) {
          if (frameworkMethod.getAnnotation(Test.class).expected().isAssignableFrom(startupException.getClass())) {
            return;
          }
