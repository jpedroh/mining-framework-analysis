...
public class CdxLine extends BaseCdxRecord<CdxLineFormat> implements HasUnparsedData {
  ...
  @Override public <V extends java.lang.Object> Value<V> get(FieldName<V> fieldName) {
  ...
  private Value getValue(FieldName name, int fieldIndex) {
    ...
    if (data[fieldOffsets[fieldIndex]] == EMPTY_FIELD_VALUE) {
      ...
      if (name.getType() == String.class || name.getType() == Object.class) {
        result = StringValue.valueOf(data, fieldOffsets[fieldIndex], fieldOffsets[fieldIndex] + fieldLengths[fieldIndex]);
      } else {
        if (name.getType() == Number.class) {
          result = NumberValue.valueOf(data, fieldOffsets[fieldIndex], fieldOffsets[fieldIndex] + fieldLengths[fieldIndex]);
        } else {
          if (name.getType() == Uri.class) {
            result = UriValue.valueOf(data, fieldOffsets[fieldIndex], fieldOffsets[fieldIndex] + fieldLengths[fieldIndex]);
          } else {
            if (name.getType() == Boolean.class) {
              result = BooleanValue.valueOf(data, fieldOffsets[fieldIndex], fieldOffsets[fieldIndex] + fieldLengths[fieldIndex]);
            } else {
              if (name.getType() == VariablePrecisionDateTime.class) {
                result = TimestampValue.valueOf(data, fieldOffsets[fieldIndex], fieldOffsets[fieldIndex] + fieldLengths[fieldIndex]);
              } else {
                result = NullValue.NULL;
              }
            }
  ...
  @Override public Iterator<Field> iterator() {
    return new FieldIterator() {
      ...
      @Override protected Field getNext() {
  ...
  private void parseFields() {
    ...
    do {
      currIndex = ArrayUtil.indexOf(data, lastIndex);
      if (currIndex > 0) {
  ...
  static final CdxRecordKey getKeyFromLine(final char[] line) {
    ...
    if (indexOfSecondField > 0) {
      ...
      if (indexOfThirdField > 0) {
        ...
        if (line.length > indexOfSecondField + 1) {
          return new CdxLineRecordKey(line);
        }
      }
    }
    throw new IllegalArgumentException("The CDX record \'" + new String(line) + "\' cannot be parsed");
  }
