...
@RegisterPlugin(name = "graphql", description = "Service that handles GraphQL requests", secure = true, enabledByDefault = true, defaultURI = "/graphql") public class GraphQLService implements Service<GraphQLRequest, MongoResponse> {
  ...
  @Inject(value = "mclient") private MongoClient mclient;

  @Inject(value = "config") private Map<String, Object> config;

  @OnInit public void init() throws ConfigurationException, NoSuchFieldException, IllegalAccessException {
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public void handle(GraphQLRequest req, MongoResponse res) throws Exception {
    if (req.isOptions()) {
      handleOptions(req);
      return;
    }
    ...
    var inputBuilder = ExecutionInput.newExecutionInput().query(req.getQuery()).dataLoaderRegistry(dataLoaderRegistry);
  ...
  static String queryNames(Document doc) {
    var rootFields = doc.getDefinitionsOfType(OperationDefinition.class).stream().filter((d) -> d.getOperation() == Operation.QUERY).map((d) -> d.getSelectionSet().getSelectionsOfType(Field.class).stream().filter((f) -> f.getSelectionSet() != null).collect(Collectors.toList())).findFirst().orElse(new ArrayList<>());
    return rootFields.stream().filter((f) -> f.getName() != null).map((f) -> f.getName()).collect(Collectors.joining(","));
  }
}