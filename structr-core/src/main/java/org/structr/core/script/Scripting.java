...
<<<<<<< /usr/src/app/output/structr/structr/82bae61d22a72269a404e509d31b3a307f054bc3/structr-core/src/main/java/org/structr/core/script/Scripting.java/left.java
      result
=======
      PolyglotWrapper.unwrap(actionContext, context.getBindings(engineName).getMember("main").execute())
    ...
    for (int i = 0; i < length; i++) {
      ...
      switch (c) {
        ...
        if (!inTemplate && hasDollar && !inComment) {
          ...
          if (inTemplate && !inSingleQuotes && !inDoubleQuotes && !inComment) {
            level++;
          }
        }
        hasDollar = false;
  ...
  public static String formatForLogging(final Object value) {
    if (value instanceof Date) {
      ...
      if (value instanceof Iterable) {
        ...
        if (value instanceof GraphObject && !(value instanceof GraphObjectMap)) {
          final StringBuilder buf = new StringBuilder();
