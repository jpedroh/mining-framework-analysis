...
public class ImageWriter implements IFormatWriter {
  ...
  public IFormatWriter getWriter(Class<? extends IFormatWriter> c) {
    for (int i = 0; i < writers.length; i++) {
      if (writers[i].getClass().equals(c)) {
        return writers[i];
      }
    }
    return null;
  ...
  @Override public int getTileSizeX() throws FormatException {
    return getWriter().getTileSizeX();
  }

  @Override public int setTileSizeX(int tileSize) throws FormatException {
    return getWriter().setTileSizeX(tileSize);
  }

  @Override public int getTileSizeY() throws FormatException {
    return getWriter().getTileSizeY();
  }

  @Override public int setTileSizeY(int tileSize) throws FormatException {
    return getWriter().setTileSizeY(tileSize);
  }

  @Override public void setResolutions(List<Resolution> resolutions) {
    for (IFormatWriter w : writers) {
      w.setResolutions(resolutions);
    }
  }

  @Override public List<Resolution> getResolutions() {
    return getWriter().getResolutions();
  }

  @Override public Set<MetadataLevel> getSupportedMetadataLevels() {
    return getWriters()[0].getSupportedMetadataLevels();
  }

  @Override public MetadataOptions getMetadataOptions() {
   ...
   */
  @Override public void setMetadataOptions(MetadataOptions options) {
    for (IFormatWriter writer : writers) {
      writer.setMetadataOptions(options);
    }
  }

  @Override public void changeOutputFile(String id) throws FormatException, IOException {
    getWriter().changeOutputFile(id);
  }

  @Override public void saveBytes(int no, byte[] buf) throws FormatException, IOException {
    getWriter().saveBytes(no, buf);
  }

  @Override public void saveBytes(int no, byte[] buf, int x, int y, int w, int h) throws FormatException, IOException {
    getWriter().saveBytes(no, buf, x, y, w, h);
  }

  @Override public void saveBytes(int no, byte[] buf, Region tile) throws FormatException, IOException {
    getWriter().saveBytes(no, buf, tile);
  }

  @Override public void savePlane(int no, Object plane) throws FormatException, IOException {
    getWriter().savePlane(no, plane);
  }

  @Override public void savePlane(int no, Object plane, int x, int y, int w, int h) throws FormatException, IOException {
    getWriter().savePlane(no, plane, x, y, w, h);
  }

  @Override public void savePlane(int no, Object plane, Region tile) throws FormatException, IOException {
    getWriter().savePlane(no, plane, tile);
  }

  @Override public void setSeries(int series) throws FormatException {
    getWriter().setSeries(series);
  }

  @Override public int getSeries() {
    return getWriter().getSeries();
  }

  @Override public void setInterleaved(boolean interleaved) {
    for (int i = 0; i < writers.length; i++) {
      writers[i].setInterleaved(interleaved);
    }
  }

  @Override public boolean isInterleaved() {
    return writers[0].isInterleaved();
  }

  @Override public void setValidBitsPerPixel(int bits) {
    for (IFormatWriter writer : writers) {
      writer.setValidBitsPerPixel(bits);
    }
  }

  @Override public boolean canDoStacks() {
    return getWriter().canDoStacks();
  }

  @Override public void setMetadataRetrieve(MetadataRetrieve r) {
    for (int i = 0; i < writers.length; i++) {
      writers[i].setMetadataRetrieve(r);
    }
  }

  @Override public MetadataRetrieve getMetadataRetrieve() {
    return getWriter().getMetadataRetrieve();
  }

  @Override public void setColorModel(ColorModel cm) {
    for (int i = 0; i < writers.length; i++) {
      writers[i].setColorModel(cm);
    }
  }
  ...
  @Override public void setFramesPerSecond(int rate) {
    for (int i = 0; i < writers.length; i++) {
      writers[i].setFramesPerSecond(rate);
    }
  }
  ...
  @Override public String[] getCompressionTypes() {
    return getWriter().getCompressionTypes();
  }

  @Override public int[] getPixelTypes() {
    return getWriter().getPixelTypes();
  }

  @Override public int[] getPixelTypes(String codec) {
    return getWriter().getPixelTypes(codec);
  }

  @Override public boolean isSupportedType(int type) {
    return getWriter().isSupportedType(type);
  }

  @Override public void setCompression(String compress) throws FormatException {
  ...
  @Override public void setWriteSequentially(boolean sequential) {
    for (IFormatWriter writer : writers) {
      writer.setWriteSequentially(sequential);
    }
  }

  @Override public void setCodecOptions(CodecOptions options) {
    getWriter().setCodecOptions(options);
  }

  @Override public boolean isThisType(String name) {
    for (int i = 0; i < writers.length; i++) {
      if (writers[i].isThisType(name)) {
        return true;
      }
    }
    return false;
  ...
  @Override public String[] getSuffixes() {
    if (suffixes == null) {
  ...
  @Override public Class<?> getNativeDataType() {
    return getWriter().getNativeDataType();
  }

  @Override public void setId(String id) throws FormatException, IOException {
  ...
  @Override public int getResolutionCount() {
    return getWriter().getResolutionCount();
  }

  @Override public int getResolution() {
    return getWriter().getResolution();
  }

  @Override public void setResolution(int resolution) {
