...
public class ScanrReader extends FormatReader {
  ...
  public ScanrReader() {
    ...
    datasetDescription = "One .xml file, one \'data\' directory containing " + ".tif/.tiff files, and optionally two .dat files";
  }
  ...
  @Override protected void initFile(String id) throws FormatException, IOException {
    ...
    if (!checkSuffix(id, "xml") && isGroupFiles()) {
      ...
      if (!isGroupFiles() && checkSuffix(id, "tif")) {
  ...
  public boolean skipMissingWells() {
    ...
    if (options instanceof DynamicMetadataOptions) {
      return ((DynamicMetadataOptions) options).getBoolean(SKIP_MISSING_WELLS, SKIP_MISSING_WELLS_DEFAULT);
    }
      ...
      if (v.length() > 0) {
        if (qName.equals("Name")) {
          ...
          if (v.equals("subposition list")) {
            ...
            if (v.equals("format typedef")) {
              foundPlateLayout = true;
            }
          }
        } else {
          if (qName.equals("Dimsize") && foundPositions && fieldPositionX == null) {
            int nPositions = Integer.parseInt(v);
            fieldPositionX = new Length[nPositions];
            fieldPositionY = new Length[nPositions];
          } else {
            if ("Rows".equals(key) && foundPlateLayout) {
              wellRows = Integer.parseInt(v);
            } else {
              if ("Columns".equals(key) && foundPlateLayout) {
                                ...
                                if (key.equals("plate name")) {
                                  ...
                                  if (key.equals("exposure time")) {
                                    ...
                                    if (key.equals("idle") && validChannel) {
                                      ...
                                      if (key.equals("well selection table + cDNA")) {
                                        ...
                                        if (key.equals("conversion factor um/pixel")) {
