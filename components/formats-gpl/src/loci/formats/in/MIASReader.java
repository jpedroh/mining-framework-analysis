...
public class MIASReader extends FormatReader {
  ...
  private final Map<String, String> overlayFiles = new HashMap<String, String>();

  private final Map<String, Integer> overlayPlanes = new HashMap<String, Integer>();
  ...
  public MIASReader() {
    ...
    datasetDescription = "One directory per plate containing one directory " + "per well, each with one or more .tif/.tiff files";
  }
  ...
  @Override public boolean isThisType(String filename, boolean open) {
    if (!open) {
    ...
    if (checkSuffix(filename, "txt")) {
      ...
      return wellName.equals("results") || wellName.equals("Batchresults") || name.equals("Nugenesistemplate.txt") || name.startsWith("mode");
    }
  ...
  @Override public String[] getSeriesUsedFiles(boolean noPixels) {
    ...
    if (analysisFiles != null) {
      for (AnalysisFile file : analysisFiles) {
        if (file.plate <= 0 && (file.well == getSeries() || file.well < 0 || wellNumber[getSeries()] == file.well)) {
    ...
    for (String dir : list) {
      ...
      if (f.getName().startsWith("Well") || f.getName().length() == 4) {
        ...
        if (f.getName().equals("results")) {
          ...
          if (f.getName().equals("Nugenesistemplate.txt")) {
  ...
  private byte[] getTile(int well, int no, int row, int col, Region intersection) throws FormatException, IOException {
    ...
    byte[] buf = readers[well][tileIndex].openBytes(0, cachedTileBuffer, intersection.x, intersection.y, intersection.width, intersection.height);
