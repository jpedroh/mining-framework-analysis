...
public class CV7000Reader extends FormatReader {
  ...
  @Override public String[] getSeriesUsedFiles(boolean noPixels) {
    ...
    if (!noPixels && channels != null) {
      for (Channel c : channels) {
        if (c != null && c.correctionFile != null && new Location(c.correctionFile).exists()) {
  ...
  @Override protected void initFile(String id) throws FormatException, IOException {
    ...
    if (!measurementDetail.exists()) {
      ...
      channels.sort(new Comparator<Channel>() {
        @Override public int compare(Channel c1, Channel c2) {
    ...
    for (int i = 0; i < realWells * fields; i++) {
      ...
      core.get(i).imageCount = core.get(i).sizeZ * core.get(i).sizeT * (core.get(i).sizeC / reader.getSizeC());
    ...
    for (int i = 0; i < planeData.size(); i++) {
      ...
      p.series = FormatTools.positionToRaster(seriesLengths, new int[] { f.field, wellIndex });
      ...
      p.no = FormatTools.positionToRaster(planeLengths, new int[] { p.channelIndex, p.z - m.minZ, p.timepoint - m.minT });
    ...
    for (int row = 0; row < plate.getPlateRows(); row++) {
      for (int col = 0; col < plate.getPlateColumns(); col++) {
        ...
        for (int field = 0; field < fields; field++) {
          ...
          String name = "Well " + FormatTools.getWellRowName(row) + (col + 1) + ", Field " + (field + 1);
          ...
          if (p != null) {
            store.setWellSamplePositionX(FormatTools.createLength(p.xpos, UNITS.REFERENCEFRAME), 0, nextWell, field);
            store.setWellSamplePositionY(FormatTools.createLength(p.ypos, UNITS.REFERENCEFRAME), 0, nextWell, field);
          }
          nextImage++;
        }
        nextWell++;
      ...
      if (lightSources.size() > 0) {
        ...
        for (LightSource l : lightSources) {
          if ("Laser".equals(l.type)) {
            ...
            store.setLaserWavelength(new Length(l.wavelength, UNITS.NANOMETER), 0, nextLightSource);
          ...
          for (int c = 0; c < getSizeC(); c++) {
            ...
            if (channel.excitation != null && channel.lightSourceRefs != null) {
              ...
              for (int ref = 0; ref < channel.lightSourceRefs.size(); ref++) {
                ...
                if ("Laser".equals(lightSources.get(lightSource).type) && lightSources.get(lightSource).wavelength < channel.excitation) {
                  index = lightSource;
                }
              }
              if (index >= 0) {
                store.setChannelLightSourceSettingsID(MetadataTools.createLSID("LightSource", 0, index), i, c);
                store.setChannelExcitationWavelength(new Length(channel.excitation, UNITS.NANOMETER), i, c);
              }
            }
  ...
  private int getChannelIndex(Plane p) {
    ...
    for (int action = 0; action <= p.actionIndex; action++) {
      for (Channel ch : channels) {
        if (ch.timelineIndex == p.timelineIndex && ch.actionIndex == action) {
          index++;
  ...
  class MeasurementDataHandler extends BaseHandler {
    ...
    @Override public void endElement(String uri, String localName, String qName) {
      ...
      if (qName.equals("bts:MeasurementRecord") && btsType.equals("IMG") && value.trim().length() > 0) {
  ...
  class MeasurementDetailHandler extends BaseHandler {
    @Override public void startElement(String uri, String localName, String qName, Attributes attributes) {
      if (qName.equals("bts:MeasurementSamplePlate")) {
        ...
        if (qName.equals("bts:MeasurementChannel")) {
          ...
          if (qName.equals("bts:MeasurementDetail")) {
    ...
    @Override public void startElement(String uri, String localName, String qName, Attributes attributes) {
      ...
      if (qName.equals("bts:LightSource")) {
        ...
        if (qName.equals("bts:Channel")) {
          ...
          if (qName.equals("bts:Timeline")) {
            timelineIndex++;
            actionIndex = -1;
          } else {
            if (qName.startsWith("bts:ActionAcquire")) {
              actionIndex++;
            }
          }
        }
