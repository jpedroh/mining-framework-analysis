...
public class FlexReader extends FormatReader {
  ...
  public static final String[] MEASUREMENT_SUFFIXES = new String[] { MEA_SUFFIX, RES_SUFFIX };

  public static final String[] SUFFIXES = new String[] { FLEX_SUFFIX, MEA_SUFFIX, RES_SUFFIX };
  ...
  private HashMap<String, String> reverseFileMapping = new HashMap<String, String>();
  ...
  public FlexReader() {
    ...
    datasetDescription = "One directory containing one or more .flex files, " + "and an optional directory containing an .mea and .res file. The .mea " + "and .res files may also be in the same directory as the .flex file(s).";
  }

  @Override public int fileGroupOption(String id) throws FormatException, IOException {
  ...
  @Override public byte[] openBytes(int no, byte[] buf, int x, int y, int w, int h) throws FormatException, IOException {
    ...
    int bpp = 0;
    double factor = 0;
    try (RandomAccessInputStream s = new RandomAccessInputStream(getFileHandle(file.file))) {
      IFD ifd;
          ...
          for (int c = 0; c < getEffectiveSizeC(); c++) {
            ...
            if (index < cameraRefs.size()) {
              ...
              if (index < binnings.size()) {
                store.setDetectorSettingsBinning(MetadataTools.getBinning(binnings.get(index)), i, c);
              }
            }
            if (lightSources != null && c < lightSources.size()) {
              store.setChannelLightSourceSettingsID(lightSources.get(c), i, c);
            } else {
              if (c > 0 && lightSources != null && lightSources.size() == 1) {
  ...
  private void parseFlexFile(int currentWell, int wellRow, int wellCol, int field, boolean firstFile, MetadataStore store) throws FormatException, IOException {
    LOGGER.info("Parsing .flex file (well {}{}, field {})", FormatTools.getWellRowName(wellRow), wellCol + 1, field);
  ...
  private String[] findFiles(Location baseFile) throws IOException {
    ...
    if (checkSuffix(baseFile.getName(), FLEX_SUFFIX)) {
      ...
      if (checkSuffix(baseFile.getName(), MEA_SUFFIX)) {
  ...
  private void groupFiles(String[] fileList, MetadataStore store) throws FormatException, IOException {
    ...
    for (String file : fileList) {
      ...
      if (compressed == firstCompressed && barcode.equals(firstBarcode) && ifdCount <= firstIFDCount) {
      ...
      for (int col = 0; col < nCols; col++) {
        ...
        for (int field = 0; field < nFiles; field++) {
          ...
          if (compressed || firstFile) {
            LOGGER.info("Parsing IFDs for well {}{}", FormatTools.getWellRowName(row), col + 1);
            ...
            LOGGER.info("Retrieving IFD offsets for well {}{}", FormatTools.getWellRowName(row), col + 1);
  ...
  private FlexFile lookupFile(int wellRow, int wellColumn, int field) {
    for (FlexFile file : flexFiles) {
      if (file.row == wellRow && file.column == wellColumn && file.field == field) {
        return file;
      }
    }
    return null;
  ...
  public class FlexHandler extends BaseHandler {
    ...
    public FlexHandler(List<String> names, List<String> factors, MetadataStore store, boolean populateCore, int well, int thisField) {
      this.names = names;
    ...
    @Override public void endElement(String uri, String localName, String qName) {
      ...
      if (qName.equals("XSize") && "Plate".equals(parentQName)) {
        ...
        if (qName.equals("YSize") && "Plate".equals(parentQName)) {
          wellColumns = Integer.parseInt(value);
        } else {
          if ("Image".equals(parentQName)) {
            if (fieldCount == 0) {
              fieldCount = 1;
            }
            ...
            int seriesCount = 1;
            if (plateCount > 0) {
              seriesCount *= plateCount;
            }
            if (wellCount > 0) {
              seriesCount *= wellCount;
            }
            if (fieldCount > 0) {
              seriesCount *= fieldCount;
            }
      ...
      if (qName.equals("Image")) {
        ...
        if (qName.equals("PlateName")) {
          if (plateName == null) {
            plateName = value;
          }
        } else {
          if (qName.equals("Barcode")) {
            if (plateBarcode == null) {
              plateBarcode = value;
            }
            if (populateCore) {
            ...
            if (qName.equals("Wavelength") && populateCore) {
              ...
              if (qName.equals("Magnification") && populateCore) {
                store.setObjectiveCalibratedMagnification(new Double(value), 0, nextObjective);
              } else {
                if (qName.equals("NumAperture") && populateCore) {
                  store.setObjectiveLensNA(new Double(value), 0, nextObjective);
                } else {
                  if (qName.equals("Immersion") && populateCore) {
                    ...
                    if (value.equals("1.33")) {
                      ...
                      if (value.equals("1.00")) {
                        ...
                        LOGGER.warn("Unknown immersion medium: {}", value);
                      }
                    }
                    try {
                      store.setObjectiveImmersion(MetadataTools.getImmersion(immersion), 0, nextObjective);
                    } catch (FormatException e) {
                      LOGGER.warn("", e);
                    }
                  } else {
                    if (qName.equals("OffsetX") || qName.equals("OffsetY")) {
                      Double offset = new Double(Double.parseDouble(value) * 1000000);
                      if (qName.equals("OffsetX")) {
                        xPositions.add(offset);
                      } else {
                        yPositions.add(offset);
                      }
                    } else {
                      if ("Image".equals(parentQName)) {
                        if (fieldCount == 0) {
                        ...
                        if (qName.equals("CameraBinningX")) {
                          ...
                          if (qName.equals("CameraBinningY")) {
                            binY = Integer.parseInt(value);
                          } else {
                            if (qName.equals("ObjectiveRef")) {
                              ...
                              if (qName.equals("CameraRef")) {
                                int cameraIndex = cameraIDs.indexOf(value);
                                if (cameraIndex >= 0) {
                                  String detectorID = MetadataTools.createLSID("Detector", 0, cameraIndex);
                                  cameraRefs.add(detectorID);
                                }
                              } else {
                                if (qName.equals("ImageResolutionX")) {
                                  double v = Double.parseDouble(value) * 1000000;
                                  xSizes.add(new Double(v));
                                } else {
                                  if (qName.equals("ImageResolutionY")) {
                                    double v = Double.parseDouble(value) * 1000000;
                                    ySizes.add(new Double(v));
                                  } else {
                                    if (qName.equals("PositionX")) {
                                      ...
                                      if (qName.equals("PositionY")) {
                                        ...
                                        if (qName.equals("PositionZ")) {
                                          ...
                                          if (qName.equals("TimepointOffsetUsed")) {
                                            planeDeltaT.add(new Double(value));
                                          } else {
                                            if (qName.equals("CameraExposureTime")) {
                                              planeExposureTime.add(new Double(value));
                                            } else {
                                              if (qName.equals("LightSourceCombinationRef")) {
                                                lightSourceCombinationRefs.add(value);
                                              } else {
                                                if (qName.equals("FilterCombinationRef")) {
                        ...
                        if (qName.equals("FilterCombination")) {
                          nextFilterSet++;
                          nextSliderRef = 0;
                        }
                      }
        ...
        for (int i = 0; i < len; i++) {
          ...
          if (name.equals("Name")) {
            ...
            if (name.equals("Factor")) {
              factors.add(attributes.getValue(i));
            }
          }
        }
      } else {
        if (qName.equals("LightSource") && level != MetadataLevel.MINIMUM) {
          parentQName = qName;
          String type = attributes.getValue("LightSourceType");
          lightSourceIDs.add(attributes.getValue("ID"));
          nextLaser++;
        } else {
          if (qName.equals("LightSourceCombination") && level != MetadataLevel.MINIMUM) {
            lightSourceID = attributes.getValue("ID");
            lightSourceCombinationIDs.put(lightSourceID, new ArrayList<String>());
          } else {
            if (qName.equals("LightSourceRef") && level != MetadataLevel.MINIMUM) {
              ...
              if (qName.equals("Camera") && level != MetadataLevel.MINIMUM && populateCore) {
                parentQName = qName;
                String detectorID = MetadataTools.createLSID("Detector", 0, nextCamera);
                store.setDetectorID(detectorID, 0, nextCamera);
                try {
                  store.setDetectorType(MetadataTools.getDetectorType(attributes.getValue("CameraType")), 0, nextCamera);
                } catch (FormatException e) {
                  LOGGER.warn("", e);
                }
                ...
                if (qName.equals("Objective") && level != MetadataLevel.MINIMUM && populateCore) {
                  parentQName = qName;
                  nextObjective++;
                  String objectiveID = MetadataTools.createLSID("Objective", 0, nextObjective);
                  store.setObjectiveID(objectiveID, 0, nextObjective);
                  try {
                    store.setObjectiveCorrection(MetadataTools.getCorrection("Other"), 0, nextObjective);
                  } catch (FormatException e) {
                    LOGGER.warn("", e);
                  }
                  ...
                  if (qName.equals("Field")) {
                    ...
                    if (fieldNo > fieldCount && ((thisField < 0 && fieldCount < firstWellPlanes()) || fieldCount < (thisField * firstWellPlanes()))) {
                      fieldCount++;
                    }
                  } else {
                    if (qName.equals("Plane")) {
                      ...
                      if (planeNo > getSizeZ() && populateCore) {
                        core.get(0).sizeZ++;
                      }
                    } else {
                      if (qName.equals("WellShape")) {
                        parentQName = qName;
                      } else {
                        if (qName.equals("Image")) {
                          parentQName = qName;
                          ...
                          if (level != MetadataLevel.MINIMUM) {
                            ...
                            if (x != null) {
                              binX = Integer.parseInt(x);
                            }
                            if (y != null) {
                              binY = Integer.parseInt(y);
                            }
                          }
                        } else {
                          if (qName.equals("Plate")) {
                            ...
                            if (qName.equals("WellCoordinate")) {
                              ...
                              if (qName.equals("Slider") && level != MetadataLevel.MINIMUM) {
                                sliderName = attributes.getValue("Name");
                              } else {
                                if (qName.equals("Filter") && level != MetadataLevel.MINIMUM) {
                                  String id = attributes.getValue("ID");
                                  if (sliderName.endsWith("Dichro")) {
                                    String dichroicID = MetadataTools.createLSID("Dichroic", 0, nextDichroic);
                                  ...
                                  if (qName.equals("FilterCombination") && level != MetadataLevel.MINIMUM) {
                                    filterSet = "FilterSet:" + attributes.getValue("ID");
                                    filterSetMap.put(filterSet, new FilterGroup());
                                  } else {
                                    if (qName.equals("SliderRef") && level != MetadataLevel.MINIMUM) {
                                      ...
                                      if (nextSliderRef == 0 && slider.startsWith("Camera")) {
                                        ...
                                        if (nextSliderRef == 1 && slider.startsWith("Camera")) {
                                          group.excitation = filterMap.get(filterName);
                                        } else {
                                          if (slider.equals("Primary_Dichro")) {
      ...
      if (qName.equals("Host")) {
        ...
        if (hostnames != null) {
          ...
          for (int i = 0; i < hostnames.length; i++) {
            ...
            hostnames[i] = hostnames[i].replace('\\', File.separatorChar);
            LOGGER.debug("Hostname #{} was {}, is now {}", new Object[] { i, host, hostnames[i] });
          }
        }
      } else {
          ...
          if (hostnames != null) {
            ...
            for (String hostname : hostnames) {
              ...
              if (flexFile.exists()) {
                ...
                String mapName = new Location(parent, flexFile.getName()).getAbsolutePath();
