...
public class LOFReader extends LMSFileReader {
  ...
  @Override public boolean isThisType(RandomAccessInputStream stream) throws IOException {
    ...
    if (stream.readInt() != LOF_MAGIC_BYTE) {
      return false;
    }
    stream.readInt();
    ...
    if (!doc.isValid()) {
      LOGGER.error("LOF file cannot be opened due to invalid XML. " + "Please try opening the corresponding XLEF file instead, or try " + "opening and re-saving this image with a current LASX version.");
      return false;
    }
