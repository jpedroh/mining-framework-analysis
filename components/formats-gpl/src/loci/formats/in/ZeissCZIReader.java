...
public class ZeissCZIReader extends FormatReader {
  ...
  private HashMap<Coordinate, ArrayList<Integer>> indexIntoPlanes = new HashMap<Coordinate, ArrayList<Integer>>();
   ...
   */
  @Override public boolean isThisType(RandomAccessInputStream stream) throws IOException {
  ...
  @Override public byte[][] get8BitLookupTable() throws FormatException, IOException {
    if ((getPixelType() != FormatTools.INT8 && getPixelType() != FormatTools.UINT8) || previousChannel == -1 || previousChannel >= channels.size()) {
      return null;
    }
  ...
  @Override public short[][] get16BitLookupTable() throws FormatException, IOException {
    if ((getPixelType() != FormatTools.INT16 && getPixelType() != FormatTools.UINT16) || previousChannel == -1 || previousChannel >= channels.size()) {
      return null;
    }
          ...
          if ((prestitched != null && prestitched) || validScanDim) {
            ...
            if (tile.intersects(image)) {
              ...
              if (rawData.length < realX * realY * pixel) {
                ...
                if (rawData.length == (realX + 1) * (realY + 1) * pixel) {
                  realX++;
                  realY++;
                }
              }
  ...
  @Override public int getOptimalTileWidth() {
  ...
  @Override public int getOptimalTileHeight() {
    if (maxResolution > 0 && getCoreIndex() < core.size() - extraImages.size()) {
  ...
  @Override protected void initFile(String id) throws FormatException, IOException {
    ...
    if (((mosaics == seriesCount) || (positions == seriesCount)) && ((seriesCount == calculatedSeries) || (maxResolution > 0 && seriesCount * mosaics == calculatedSeries)) && prestitched != null && prestitched) {
      ...
      if (!allowAutostitching() && calculatedSeries > seriesCount) {
        ms0.sizeX = firstX;
        ms0.sizeY = firstY;
        prestitched = true;
      } else {
        if (allowAutostitching() && prestitched == null && mosaics > 1) {
          prestitched = (mosaics == seriesCount && mosaics == calculatedSeries) || (mosaics == (seriesCount / positions));
        }
      }
    }
    if (ms0.imageCount * seriesCount > planes.size() * scanDim && planes.size() > 0) {
      if (planes.size() != ms0.imageCount && planes.size() != ms0.sizeT && (planes.size() % (seriesCount * getSizeZ())) == 0) {
        ...
        if (planes.size() == ms0.sizeT || planes.size() == ms0.imageCount || (!isGroupFiles() && positions > 1)) {
          positions = 1;
          acquisitions = 1;
          mosaics = 1;
          angles = 1;
          ...
          if (seriesCount > mosaics && mosaics > 1 && prestitched != null && prestitched) {
            seriesCount /= mosaics;
            mosaics = 1;
          }
        }
    ...
    if ((prestitched != null && prestitched) && seriesCount == (mosaics * positions) && maxResolution == 0) {
      seriesCount = positions;
    }
    ...
    assignPlaneIndices();
    if (maxResolution > 0 || (mosaics > 1 && seriesCount == positions) || (mosaics == 1 && seriesCount > 1)) {
    ...
    for (int i = 0; i < getSeriesCount(); i++) {
      ...
      if (acquiredDate != null) {
        ...
        if (planes.get(0).timestamp != null) {
      ...
      for (int plane = 0; plane < getImageCount(); plane++) {
        ...
        for (Integer q : index) {
          ...
          if (currentPlane.stageX != null) {
            if (minStageX == null || currentPlane.stageX.value().doubleValue() < minStageX) {
              minStageX = currentPlane.stageX.value().doubleValue();
            }
            if (maxStageX == null || currentPlane.stageX.value().doubleValue() > maxStageX) {
              maxStageX = currentPlane.stageX.value().doubleValue();
            }
          }
          if (currentPlane.stageY != null) {
            if (minStageY == null || currentPlane.stageY.value().doubleValue() < minStageY) {
              minStageY = currentPlane.stageY.value().doubleValue();
            }
            if (maxStageY == null || currentPlane.stageY.value().doubleValue() > maxStageY) {
        ...
        if (minStageX != null && maxStageX != null) {
          ...
          if (positionsX != null && positionIndex < positionsX.length && positionsX[positionIndex] != null) {
        ...
        if (minStageY != null && maxStageY != null) {
          ...
          if (positionsY != null && positionIndex < positionsY.length && positionsY[positionIndex] != null) {
        ...
        if (p.stageZ != null) {
          ...
          if (positionsZ != null && positionIndex < positionsZ.length) {
      ...
      if (firstPlane && core.get(i).resolutionCount > 1 && hasFlattenedResolutions()) {
        positionIndex++;
      }
  ...
  public boolean allowAutostitching() {
    ...
    if (options instanceof DynamicMetadataOptions) {
      return ((DynamicMetadataOptions) options).getBoolean(ALLOW_AUTOSTITCHING_KEY, ALLOW_AUTOSTITCHING_DEFAULT);
    }
  ...
  private void assignPlaneIndices() {
      ...
      for (DimensionEntry dimension : plane.directoryEntry.dimensionEntries) {
        ...
        switch (dimension.dimension.charAt(0)) {
          ...
          if (dimension.start > prevM) {
            if (!extraDimOrder.contains('M') && mosaics <= getSeriesCount() && (prestitched == null || !prestitched || !allowAutostitching())) {
    ...
    for (int p = 0; p < planes.size(); p++) {
      ...
      if (angles > 1 && noAngle) {
        extra[extraDimOrder.indexOf('V')] = p / (getImageCount() * (getSeriesCount() / angles));
      }
      if (rotations > 0) {
    ...
    if (layers != null) {
      ...
      for (int i = 0; i < layers.getLength(); i++) {
        ...
        for (int s = 0; s < crosses.getLength(); s++, shape += 2) {
          ...
          store.setLineID(MetadataTools.createLSID("Shape", roiCount, shape), roiCount, shape);
          store.setLineID(MetadataTools.createLSID("Shape", roiCount, shape + 1), roiCount, shape + 1);
        ...
        if (ellipses != null) {
          for (int s = 0; s < ellipses.getLength(); s++, shape++) {
            ...
            store.setEllipseID(MetadataTools.createLSID("Shape", roiCount, shape), roiCount, shape);
            ...
            store.setEllipseText(getFirstNodeValue(textElements, "Text"), roiCount, shape);
          }
        }
        ...
        NodeList inOutCircles = getGrandchildren(layer, "Elements", "InOutCircle");
        if (inOutCircles != null) {
          shape = populateCircles(inOutCircles, roiCount, shape);
        }
        NodeList outInCircles = getGrandchildren(layer, "Elements", "OutInCircle");
        if (outInCircles != null) {
          shape = populateCircles(outInCircles, roiCount, shape);
        }
        NodeList pointsCircles = getGrandchildren(layer, "Elements", "PointsCircle");
        ...
        NodeList openPolylines = getGrandchildren(layer, "Elements", "OpenPolyline");
        if (openPolylines != null) {
          shape = populatePolylines(openPolylines, roiCount, shape, false);
        }
        NodeList closedPolylines = getGrandchildren(layer, "Elements", "ClosedPolyline");
        if (closedPolylines != null) {
          shape = populatePolylines(closedPolylines, roiCount, shape, true);
        }
        NodeList beziers = getGrandchildren(layer, "Elements", "Bezier");
        if (beziers != null) {
          ...
          for (int s = 0; s < events.getLength(); s++) {
            ...
            if (points != null) {
              ...
              for (String point : coords) {
                ...
                if (xy.length == 2) {
                  String pointID = MetadataTools.createLSID("Shape", roiCount, shape);
                  store.setPointID(pointID, roiCount, shape);
                  store.setPointX(DataTools.parseDouble(xy[0]), roiCount, shape);
                  store.setPointY(DataTools.parseDouble(xy[1]), roiCount, shape);
                  shape++;
                }
              }
            }
  ...
  private int populateRectangles(NodeList rectangles, int roi, int shape) {
    for (int s = 0; s < rectangles.getLength(); s++) {
      ...
      if (left != null && top != null && width != null && height != null) {
        store.setRectangleID(MetadataTools.createLSID("Shape", roi, shape), roi, shape);
  ...
  private int populatePolylines(NodeList polylines, int roi, int shape, boolean closed) {
    for (int s = 0; s < polylines.getLength(); s++, shape++) {
      ...
      if (closed) {
        ...
        store.setPolygonPoints(getFirstNodeValue(geometry, "Points"), roi, shape);
        store.setPolygonText(getFirstNodeValue(textElements, "Text"), roi, shape);
      } else {
        store.setPolylineID(shapeID, roi, shape);
        store.setPolylinePoints(getFirstNodeValue(geometry, "Points"), roi, shape);
        store.setPolylineText(getFirstNodeValue(textElements, "Text"), roi, shape);
      }
    }
    return shape;
  }
  ...
  private int populateLines(NodeList lines, int roi, int shape) {
    for (int s = 0; s < lines.getLength(); s++, shape++) {
      ...
      store.setLineID(MetadataTools.createLSID("Shape", roi, shape), roi, shape);
      if (x1 != null) {
  ...
  private int populateCircles(NodeList circles, int roi, int shape) {
    for (int s = 0; s < circles.getLength(); s++, shape++) {
      ...
      store.setEllipseID(MetadataTools.createLSID("Shape", roi, shape), roi, shape);
  ...
  private void populateOriginalMetadata(Element root, Deque<String> nameStack) {
    ...
    if (root.getChildNodes().getLength() == 1) {
      ...
      if (value != null && key.length() > 0) {
        ...
        if (key.toString().endsWith("|Rotations|")) {
          ...
          if (key.toString().endsWith("|Phases|")) {
            phaseLabels = value.split(" ");
          } else {
            if (key.toString().endsWith("|Illuminations|")) {
    ...
    for (int i = 0; i < attributes.getLength(); i++) {
      ...
      if (attrName.endsWith("|")) {
        ...
        if (attrName.length() == 0 && keyString.endsWith("|")) {
  ...
  private Segment readSegment(String filename) throws IOException {
    int skip = (ALIGNMENT - (int) (in.getFilePointer() % ALIGNMENT)) % ALIGNMENT;
    in.skipBytes(skip);
    ...
    if (segmentID.equals("ZISRAWFILE")) {
      ...
      if (segmentID.equals("ZISRAWMETADATA")) {
        ...
        if (segmentID.equals("ZISRAWSUBBLOCK")) {
  ...
  private void parseObjectives(NodeList objectives) throws FormatException {
    if (objectives != null) {
      for (int i = 0; i < objectives.getLength(); i++) {
        ...
        store.setObjectiveImmersion(MetadataTools.getImmersion(getFirstNodeValue(objective, "Immersion")), 0, i);
  ...
  class FileHeader extends Segment {
    ...
    @Override public void fillInData() throws IOException {
      super.fillInData();
      RandomAccessInputStream s = getStream();
      try {
        s.order(isLittleEndian());
  ...
  class SubBlock extends Segment {
    ...
    @Override public String toString() {
      return "coreIndex=" + coreIndex + ", planeIndex=" + planeIndex + ", resolutionIndex=" + resolutionIndex + ", x=" + x + ", y=" + y + ", row=" + row + ", col=" + col + ", metadata=" + metadata + ", attachmentSize=" + attachmentSize + ", directoryEntry=" + directoryEntry;
    }
    ...
    private void parseMetadata() throws IOException {
      ...
      Element root = null;
      try {
        ByteArrayInputStream s = new ByteArrayInputStream(metadata.getBytes(Constants.ENCODING));
        root = parser.parse(s).getDocumentElement();
        s.close();
      } catch (SAXException e) {
        metadata = null;
                  ...
                  if (tagNode.getNodeName().equals("StageYPosition")) {
                    ...
                    if (tagNode.getNodeName().equals("FocusPosition")) {
                      ...
                      if (tagNode.getNodeName().equals("AcquisitionTime")) {
                        ...
                        if (tagNode.getNodeName().equals("ExposureTime")) {
  ...
  class Attachment extends Segment {
    ...
    public Attachment(String filename, long position) throws IOException {
      ...
      try {
        ...
        dataOffset = s.getFilePointer() + 112;
      }  finally {
        if (stream == null) {
          s.close();
        }
  ...
  class DirectoryEntry {
    ...
    @Override public String toString() {
      String s = "schemaType = " + schemaType + ", pixelType = " + pixelType + ", filePosition = " + filePosition + ", filePart = " + filePart + ", compression = " + compression + ", pyramidType = " + pyramidType + ", dimensionCount = " + dimensionCount;
      if (dimensionCount > 0) {
  ...
  static class DimensionEntry {
    ...
    @Override public String toString() {
      return "dimension=" + dimension + ", start=" + start + ", size=" + size + ", startCoordinate=" + startCoordinate + ", storedSize=" + storedSize;
    }
  }
  ...
  static class AttachmentEntry {
    ...
    @Override public String toString() {
      return "schemaType = " + schemaType + ", filePosition = " + filePosition + ", filePart = " + filePart + ", contentGUID = " + contentGUID + ", contentFileType = " + contentFileType;
    }
  }
