...
public class CellWorxReader extends FormatReader {
  ...
  private HashMap<Integer, Timestamp> timestamps = new HashMap<Integer, Timestamp>();
  ...
  public CellWorxReader() {
    ...
    datasetDescription = "One .htd file plus one or more .pnl " + "files and optionally one or more .log files";
  ...
  @Override public byte[] openBytes(int no, byte[] buf, int x, int y, int w, int h) throws FormatException, IOException {
    ...
    if (lastFile == null || lastReader == null || !file.equals(lastFile) || lastReader.getCurrentFile() == null) {
      if (lastReader != null) {
        lastReader.close();
      }
      try {
    ...
    if (lastReader.getSeriesCount() == fieldCount && fieldCount > 1) {
      ...
      if (lastReader.getImageCount() == getSizeZ()) {
    ...
    LOGGER.trace("  file series = {}, planeIndex = {}", lastReader.getSeries(), planeIndex);
    ...
    for (int row = 0; row < wellFiles.length; row++) {
      for (int col = 0; col < wellFiles[row].length; col++) {
        if (wellFiles[row][col] != null) {
          ...
          if (!new Location(wellFiles[row][col][0]).exists()) {
            wellFiles[row][col] = getTiffFiles(plateName, row, col, wavelengths.length, nTimepoints, zSteps);
          }
        }
      }
    }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  protected void populateMetadata() throws FormatException, IOException {
    ...
    if (plateLogFile != null && new Location(plateLogFile).exists()) {
      ...
      for (String line : f) {
        if (line.trim().startsWith("Z Map File")) {
          ...
          if (line.trim().startsWith("Scanner SN")) {
    ...
    for (int s = 0; s < getSeriesCount(); s++) {
      ...
      if (metadataLevel != MetadataLevel.MINIMUM) {
        ...
        int plane = 0;
        while ((firstFile == null || !new Location(firstFile).exists()) && plane < getImageCount()) {
  ...
  private int getWell(int seriesIndex) {
    ...
    for (int row = 0; row < wellFiles.length; row++) {
      for (int col = 0; col < wellFiles[row].length; col++) {
        ...
        if (counter == wellIndex) {
          return row * wellFiles[row].length + col;
        }
      }
    }
    return -1;
  ...
  private String getFile(int seriesIndex, int no) {
    ...
    if (field * imageCount + no < wellFiles[row][col].length) {
      ...
      if (field < wellFiles[row][col].length) {
        return wellFiles[row][col][field];
      } else {
        if (imageCount == 0 && wellFiles[row][col].length == 1) {
          return wellFiles[row][col][0];
        }
      }
    }
...
<<<<<<< /usr/src/app/output/openmicroscopy/bioformats/6b73e46baf0dbd4692d41b00f2ff78078d5ed29e/components/formats-gpl/src/loci/formats/in/CellWorxReader.java/left.java
  private String[] getTiffFiles(String plateName, int row, int col, int channels, int nTimepoints, int zSteps) {
    String well = FormatTools.getWellName(row, col);
    ...
    if (noneExist) {
      ...
      if (noneExist) {
        ...
        if (nextFile != files.length) {
          LOGGER.warn("Well {} expected {} files; found {}", well, files.length, nextFile);
        }
      }
    }
    return files;
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  protected void initReader(IFormatReader reader, String file, boolean omexml) throws FormatException, IOException {
    if (omexml) {
      IMetadata metadata;
      try {
