...
public class MetamorphReader extends BaseTiffReader {
  ...
  @Override public boolean isThisType(String name, boolean open) {
    ...
    if (open) {
      ...
      while (baseName.indexOf('_') >= 0) {
        ...
        if (checkSuffix(name, suffixes) && (new Location(parent, baseName + ".nd").exists() || new Location(parent, baseName + ".ND").exists())) {
          return true;
        }
        if (checkSuffix(name, suffixes) && (new Location(parent, baseName + ".htd").exists() || new Location(parent, baseName + ".HTD").exists())) {
          return false;
        }
      }
    }
  ...
  @Override public byte[] openBytes(int no, byte[] buf, int x, int y, int w, int h) throws FormatException, IOException {
    ...
    stkReaders[getSeries()][ndx].setMetadataOptions(new DefaultMetadataOptions(MetadataLevel.MINIMUM));
  ...
  @Override protected void initFile(String id) throws FormatException, IOException {
                      ...
                      if (key.startsWith("StartTime")) {
                        ...
                        if (key.equals("ZStepSize")) {
                          ...
                          if (key.equals("NStagePositions")) {
                            ...
                            if (key.equals("WaveInFileName")) {
                              useWaveNames = Boolean.parseBoolean(value);
                            } else {
                              if (key.equals("DoZSeries")) {
    ...
    for (int i = 0; i < getSeriesCount(); i++) {
      ...
      if (exposureTimes.size() == 0) {
        ...
        if (exposureTimes.size() == 1 && exposureTimes.size() < getEffectiveSizeC()) {
      ...
      for (int p = 0; p < getImageCount(); p++) {
          ...
          if (file != null) {
            ...
            if (commentEntry != null) {
              if (commentEntry instanceof String) {
                ...
                if (commentEntry instanceof TiffIFDEntry) {
              ...
              for (String line : lines) {
                ...
                if (line.startsWith("<prop")) {
                  ...
                  if (key.equals("z-position")) {
                    ...
                    if (key.equals("acquisition-time-local")) {
        ...
        if (timestamps.size() > 0) {
          ...
          if (internalStamps != null && p < internalStamps.length) {
  ...
  @Override protected void initStandardMetadata() throws FormatException, IOException {
  ...
  private String makeImageName(int i) {
    ...
    if (firstSeriesChannels != null && (stageNames == null || stageNames.size() == 0 || stageNames.size() != getSeriesCount())) {
      if (name.length() > 0) {
        name.append("; ");
      }
    ...
    for (int i = 0; i < uic1count; i++) {
      ...
      switch (currentID) {
        ...
        if (valOrOffset != 0 && valOrOffset < in.length()) {
          ...
          if (valOrOffset == 0 && getSizeZ() < mmPlanes) {
            core.get(0).sizeZ = 1;
          }
        }
        break;
  ...
  private Map.Entry<Integer, Integer> getWellCoords(String label) {
    ...
    return new AbstractMap.SimpleEntry((int) (matcher.group(1).toUpperCase().charAt(0) - 'A'), Integer.parseInt(matcher.group(2)) - 1);
  }
