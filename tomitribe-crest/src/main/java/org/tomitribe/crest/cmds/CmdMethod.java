...
public class CmdMethod implements Cmd {
  ...
  private static final Join.NameCallback<String> STRING_NAME_CALLBACK = new Join.NameCallback<String>() {
    @Override public String getName(final String object) {
  ...
  @Override public String getUsage() {
    ...
    return String.format("%s %s %s", commandName, args.size() == method.getParameterTypes().length ? "" : "[options]", Join.join(" ", args)).trim();
  }
  ...
  @Override public Object exec(final String... rawArgs) {
  ...
  @Override public void help(final PrintStream out) {
    out.println();
    out.print("Usage: ");
  ...
  private <T extends java.lang.Object> List<Object> convert(final Arguments args) {
  ...
  private List<Object> convert(Arguments args, Needed needed, List<Param> parameters1) {
    ...
    for (final Param parameter : parameters1) {
      ...
      if (parameter instanceof ComplexParam) {
        ...
        if (option != null) {
          final String optionValue = OptionParam.class.isInstance(parameter) ? OptionParam.class.cast(parameter).getName() : option.value()[0];
          ...
          if (args.list.size() > 0) {
            ...
            if (Environment.class == parameter.getType()) {
  ...
  private class Arguments {
    ...
    private void getCommand(final String defaultPrefix, final String arg, final Map<String, String> defaults, final Map<String, String> supplied, final List<String> invalid, final Set<String> repeated) {
      String name;
      String value;
    ...
    private void processOption(final String prefix, final String optName, final String optValue, final Map<String, String> defaults, final Map<String, String> supplied, final List<String> invalid, final Set<String> repeated) {
      String name = optName;
  ...
  @Override public Collection<String> complete(final String buffer, final int cursorPosition) {
