 ...
 * 
 */
@DeclareRoles(value = { "org.imixs.ACCESSLEVEL.MANAGERACCESS" }) @RunAs(value = "org.imixs.ACCESSLEVEL.MANAGERACCESS") @Stateless @LocalBean public class JobHandlerRenameUser implements JobHandler {
  ...
  /**
     ...
     */
  @Override public ItemCollection run(ItemCollection adminp) throws AdminPException {
    ...
    for (ItemCollection entity : col) {
      iProcessed++;
      boolean result = ctx.getBusinessObject(JobHandlerRenameUser.class).updateWorkitemUserIds(entity, fromUserID, toUserID, replace);
      if (result == true) {
        iUpdates++;
      }
    }
    ...
    logger.info("Job " + AdminPService.JOB_RENAME_USER + " (" + adminp.getUniqueID() + ") - " + colSize + " documents processed, " + iUpdates + " updates in " + time + " sec.  (in total: " + iProcessed + " processed, " + iUpdates + " updates)");
  ...
  /**
     ...
     * @throws AccessDeniedException
     */
  @TransactionAttribute(value = TransactionAttributeType.REQUIRES_NEW) public boolean updateWorkitemUserIds(ItemCollection entity, String from, String to, boolean replace) throws AccessDeniedException {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "rawtypes", "unchecked" }) private boolean updateList(List list, String from, String to, boolean replace) {
