...
public final class Monetary {
  ...
  private final ServiceLoader<MonetaryAmountFactory> amountFactoryLoader = ServiceLoader.load(MonetaryAmountFactory.class);

  private final ServiceLoader<ExchangeRateProvider> exchangeRateProviderLoader = ServiceLoader.load(ExchangeRateProvider.class);

  private final ServiceLoader<CurrencyConverter> currencyConverterLoader = ServiceLoader.load(CurrencyConverter.class);

  private final ServiceLoader<MonetaryExtension> extensionsLoader = ServiceLoader.load(MonetaryExtension.class);
    ...
    for (MonetaryExtension t : extensionsLoader) {
      try {
        if (t.getExposedType() == null) {
          throw new IllegalArgumentException("Monetary extension of type: " + t.getClass().getName() + " does not expose a type.");
        }
        if (!t.getExposedType().isAssignableFrom(t.getClass())) {
          throw new IllegalArgumentException("Monetary extension of type: " + t.getClass().getName() + " does not implement exposed type: " + t.getExposedType().getName());
        }
  ...
  private void initDefaultNumberClass() {
    String defaultClassName = System.getProperty("javax.money.defaultNumberClass");
    if (defaultClassName != null) {
      try {
        defaultNumberClass = Class.forName(defaultClassName);
        if (!this.monetaryAmountFactories.containsKey(defaultNumberClass)) {
  ...
  /**
	 ...
	 */
  private <T extends java.lang.Object> T loadService(Class<T> serviceType) {
    ...
    for (T t : loader) {
      if (instance == null) {
        ...
        throw new IllegalStateException("javaxmoney: Implementation of " + serviceType + " is ambigous.");
      }
    }
    return instance;
  }
  ...
  /**
	 ...
	 */
  public static MonetaryAmountFactory getMonetaryAmountFactory(Class<?> numberClass) {
    MonetaryAmountFactory factory = INSTANCE.monetaryAmountFactories.get(numberClass);
    if (factory == null) {
      throw new UnsupportedOperationException("No MonetaryAmountFactory for number class \'" + numberClass.getName() + "\' loaded.");
    }
    return factory;
  }
  ...
  public static CurrencyUnitProvider getCurrencyUnitProvider() {
    if (INSTANCE.currencyUnitProvider == null) {
      throw new UnsupportedOperationException("No CurrencyUnitProvider loaded");
    }
  ...
  /**
	 ...
	 */
  public static ExchangeRateProvider getExchangeRateProvider(ConversionType type) {
    ExchangeRateProvider prov = INSTANCE.exchangeRateProviders.get(type);
    if (prov == null) {
      throw new IllegalArgumentException("No ExchangeRateProvider for the required type registered: " + type);
    }
    return prov;
  }
  ...
  public static CurrencyConverter getCurrencyConverter(ConversionType type) {
    ...
    if (prov == null) {
      throw new IllegalArgumentException("No CurrencyConverter for the required type registered: " + type);
    }
    return prov;
  }
  ...
  public static AmountFormatterFactory getAmountFormatterFactory() {
    if (INSTANCE.amountFormatterFactory == null) {
      throw new UnsupportedOperationException("No AmountFormatterFactory loaded");
    }
  ...
  public static AmountParserFactory getAmountParserFactory() {
    if (INSTANCE.amountParserFactory == null) {
      throw new UnsupportedOperationException("No AmountParserFactory loaded");
    }
  ...
  public static CurrencyFormatterFactory getCurrencyFormatterFactory() {
    if (INSTANCE.currencyFormatterFactory == null) {
      throw new UnsupportedOperationException("No CurrencyFormatterFactory loaded");
    }
  ...
  public static CurrencyParserFactory getCurrencyParserFactory() {
    if (INSTANCE.currencyParserFactory == null) {
      throw new UnsupportedOperationException("No CurrencyParserFactory loaded");
    }
  ...
  public static RoundingProvider getRoundingProvider() {
    if (INSTANCE.roundingProvider == null) {
      throw new UnsupportedOperationException("No RoundingProvider loaded");
    }
  ...
  /**
	 ...
	 */
  public static <T extends java.lang.Object> T getExtension(Class<T> extensionType) {
    @SuppressWarnings(value = { "unchecked" }) T ext = (T) INSTANCE.extensions.get(extensionType);
    if (ext == null) {
      throw new IllegalArgumentException("Unsupported monetary extension: " + extensionType);
    }
    return ext;
  }
