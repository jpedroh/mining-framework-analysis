...
public class MultiByteFont extends CIDFont implements Substitutable, Positionable {
  ...
  /** {@inheritDoc} */
  @Override public String getEmbedFontName() {
    if (isEmbeddable()) {
  ...
  /** {@inheritDoc} */
  @Override public String getEncodingName() {
    return encoding;
  }
  ...
  /** {@inheritDoc} */
  @Override public char mapChar(char c) {
    notifyMapOperation();
  ...
  /** {@inheritDoc} */
  @Override public boolean hasChar(char c) {
      ...
      if ((ch >= 0x80) && (ch < 0x00A0)) {
        ...
        if ((ch >= 0x2000) && (ch <= 0x206F)) {
          if ((ch >= 0x200B) && (ch <= 0x200F)) {
            ...
            if ((ch >= 0x2028) && (ch <= 0x202E)) {
              return true;
            } else {
              if ((ch >= 0x2066) && (ch <= 0x206F)) {
                return true;
              } else {
                return ch == 0x2060;
              }
  ...
  @Override public boolean hasFeature(int tableType, String script, String language, String feature) {
    GlyphTable table;
    if (tableType == GlyphTable.GLYPH_TABLE_TYPE_SUBSTITUTION) {
      table = getGSUB();
    } else {
      if (tableType == GlyphTable.GLYPH_TABLE_TYPE_POSITIONING) {
        table = getGPOS();
      } else {
