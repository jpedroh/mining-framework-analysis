...
@Service public class JobServiceImpl implements JobService {
        ...
        for (JobConfig jobConfig : unSystemJobs) {
          try {
            ...
            if (JobType.UNKOWN_JOB.equals(jobType)) {
              if (jobListElementVo.getJobClass() != null && jobListElementVo.getJobClass().indexOf("SaturnScriptJob") != -1) {
                jobListElementVo.setJobType(JobType.SHELL_JOB.name());
              } else {
                jobListElementVo.setJobType(JobType.JAVA_JOB.name());
              }
            }
            jobListElementVo.setStatus(getJobStatus(jobConfig.getJobName(), curatorFrameworkOp, jobConfig.getEnabled()));
  ...
  private void updateJobInfoShardingList(CuratorRepository.CuratorFrameworkOp curatorFrameworkOp, String jobName, JobListElementVo jobListElementVo) {
    if (JobStatus.STOPPED.equals(jobListElementVo.getStatus())) {
      return;
    }
  ...
  @Override public JobConfigVo getJobConfigVo(String namespace, String jobName) throws SaturnJobConsoleException {
    JobConfig4DB jobConfig4DB = currentJobConfigService.findConfigByNamespaceAndJobName(namespace, jobName);
    if (jobConfig4DB == null) {
      throw new SaturnJobConsoleException(String.format("\u8be5\u4f5c\u4e1a\uff08%s\uff09\u4e0d\u5b58\u5728", jobName));
    }
    ...
    CuratorRepository.CuratorFrameworkOp curatorFrameworkOp = registryCenterService.getCuratorFrameworkOp(namespace);
    jobConfigVo.setStatus(getJobStatus(jobConfigVo.getJobName(), curatorFrameworkOp, jobConfigVo.getEnabled()));
    return jobConfigVo;
  }
