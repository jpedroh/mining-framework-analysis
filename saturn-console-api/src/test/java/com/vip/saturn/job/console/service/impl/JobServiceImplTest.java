...
@RunWith(value = MockitoJUnitRunner.class) @FixMethodOrder(value = MethodSorters.NAME_ASCENDING) public class JobServiceImplTest {
  ...
  @Test public void testGetGroup() throws SaturnJobConsoleException {
  ...
  @Test public void testRemoveJobFailByHaveUpStream() throws SaturnJobConsoleException {
    JobConfig4DB jobConfig4DB = new JobConfig4DB();
    jobConfig4DB.setJobName(jobName);
    ...
    when(currentJobConfigService.findConfigsByNamespace(namespace)).thenReturn(Lists.newArrayList(jobConfig4DB, upStream, other));
    expectedException.expect(SaturnJobConsoleException.class);
    expectedException.expectMessage(String.format("\u4e0d\u80fd\u5220\u9664\u8be5\u4f5c\u4e1a\uff08%s\uff09\uff0c\u56e0\u4e3a\u8be5\u4f5c\u4e1a\u5b58\u5728\u4e0a\u6e38\u4f5c\u4e1a[%s]\uff0c\u8bf7\u5148\u65ad\u5f00\u4e0a\u4e0b\u6e38\u5173\u7cfb\u518d\u5220\u9664", jobName, upStream.getJobName()));
    jobService.removeJob(namespace, jobName);
  }
  ...
  @Test public void testAddJobFailByDeleting() throws SaturnJobConsoleException {
    JobConfig jobConfig = createValidJob();
    String server = "e1";
    ...
    when(curatorFrameworkOp.checkExists(eq(JobNodePath.getServerNodePath(jobConfig.getJobName())))).thenReturn(true);
    when(curatorFrameworkOp.getChildren(eq(JobNodePath.getServerNodePath(jobConfig.getJobName())))).thenReturn(Lists.newArrayList(server));
    when(curatorFrameworkOp.checkExists(eq(ExecutorNodePath.getExecutorNodePath(server, "ip")))).thenReturn(true);
    when(curatorFrameworkOp.checkExists(eq(JobNodePath.getServerStatus(jobConfig.getJobName(), server)))).thenReturn(true);
    expectedException.expect(SaturnJobConsoleException.class);
  ...
  @Test public void testGetUnSystemJob() throws SaturnJobConsoleException {
    when(currentJobConfigService.findConfigsByNamespace(namespace)).thenReturn(Lists.newArrayList(new JobConfig4DB()));
  ...
  @Test public void testGetUnSystemJobNames() throws SaturnJobConsoleException {
    JobConfig4DB jobConfig4DB = new JobConfig4DB();
    jobConfig4DB.setJobName(jobName);
    when(currentJobConfigService.findConfigsByNamespace(namespace)).thenReturn(Lists.newArrayList(jobConfig4DB));
    assertEquals(jobService.getUnSystemJobs(namespace).size(), 1);
  ...
  @Test public void testGetJobName() throws SaturnJobConsoleException {
