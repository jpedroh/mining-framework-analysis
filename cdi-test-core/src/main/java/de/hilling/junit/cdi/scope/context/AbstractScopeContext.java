...
public abstract class AbstractScopeContext implements Context, Serializable {
  ...
  @Override public <T extends java.lang.Object> T get(final Contextual<T> contextual) {
  ...
  @Override public <T extends java.lang.Object> T get(final Contextual<T> contextual, final CreationalContext<T> creationalContext) {
    Bean<T> bean = (Bean<T>) contextual;
    if (getScopeContextHolder().getBeans().containsKey(bean.getBeanClass())) {
      return getInstanceFromScope(bean);
    } else {
  ...
  @SuppressWarnings(value = { "unchecked" }) private <T extends java.lang.Object> T getInstanceFromScope(Bean<T> bean) {
    return (T) getScopeContextHolder().getBean(bean.getBeanClass()).getInstance();
  }

  private <T extends java.lang.Object> T createNewInstance(final CreationalContext<T> creationalContext, Bean<T> bean) {
