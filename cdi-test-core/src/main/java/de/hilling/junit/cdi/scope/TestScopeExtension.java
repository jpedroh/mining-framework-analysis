...
public class TestScopeExtension implements Extension, Serializable {
  ...
  public <T extends java.lang.Object> void processAnnotatedType(@Observes ProcessAnnotatedType<T> pat) {
    LOG.log(Level.FINE, "processing type " + pat);
    AnnotatedTypeAdapter<T> enhancedAnnotatedType = new AnnotationReplacementAdapter<>(pat.getAnnotatedType(), Collections.unmodifiableMap(replacementMap));
    pat.setAnnotatedType(enhancedAnnotatedType);
  }

  public <X extends java.lang.Object> void processBean(@Observes ProcessAnnotatedType<X> pat) {
    ...
    if (ReflectionsUtils.isTestClass(javaClass)) {
      ...
      if (ReflectionsUtils.shouldProxyCdiType(javaClass)) {
        AnnotatedTypeBuilder<X> builder = new AnnotatedTypeBuilder<>();
        builder.readFromType(type);
