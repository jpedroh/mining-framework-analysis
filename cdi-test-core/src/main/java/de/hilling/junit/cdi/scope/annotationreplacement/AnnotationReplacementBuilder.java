 ...
 * @param <T> bean class
 */
@SuppressWarnings(value = { "CdiManagedBeanInconsistencyInspection" }) @Alternative public class AnnotationReplacementBuilder<T extends java.lang.Object> {
  ...
  private void addAnnotations() {
    for (Map.Entry<Class<? extends Annotation>, Annotation> replacement : replacementMap.entrySet()) {
      ...
      configurator.fields().stream().filter((f) -> f.getAnnotated().isAnnotationPresent(toReplace)).forEach((f) -> f.add(replacementValue));
      configurator.methods().stream().filter((m) -> m.getAnnotated().isAnnotationPresent(toReplace)).forEach((m) -> m.add(replacementValue));
      configurator.constructors().stream().filter((c) -> c.getAnnotated().isAnnotationPresent(toReplace)).forEach((c) -> c.add(replacementValue));
    }
  }
}