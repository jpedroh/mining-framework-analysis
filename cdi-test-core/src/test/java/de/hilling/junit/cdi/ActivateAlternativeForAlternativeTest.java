 ...
 */
@ExtendWith(value = CdiTestJunitExtension.class) @ExtendWith(value = MockitoExtension.class) class ActivateAlternativeForAlternativeTest {
  @Inject private TestActivatedOverridenService testOverride;

  @Inject private OverriddenService overriddenService;

  @Inject private BackendService backendService;

  @Test void callTestActivatedService() {
    backendService.storePerson(new Person());
    backendService.storePerson(new Person());
    assertEquals(2, testOverride.getInvocationCounter());
  }

  @Test void callTestActivatedServiceIndependently() {
    backendService.storePerson(new Person());
    backendService.storePerson(new Person());
    assertEquals(2, testOverride.getInvocationCounter());
  }

  @Test void callOverridenServiceDirectly() {
  ...
  @Test void callOverridenServiceMixed() {
    overriddenService.serviceMethod();
    overriddenService.serviceMethod();
    overriddenService.serviceMethod();
    backendService.storePerson(new Person());
