      ...
      switch (expectedNodeType) {
        ...
        if (numericComparisonTolerance != null) {
          ...
          if (diff.compareTo(numericComparisonTolerance) > 0) {
            valueDifferenceFound("Different value found in node \"%s\". Expected %s, got %s, difference is %s, tolerance is %s", fieldPath, quoteTextValue(expectedNode.getNumberValue()), quoteTextValue(actualNode.getNumberValue()), diff.toString(), numericComparisonTolerance);
          }
        } else {
