...
public class HttpTransport extends AbstractTransport<HttpTransportRequest, HttpTransportResponse> {
  ...
  @Override public void execute(HttpTransportRequest request, HttpTransportResponse response) throws IOException {
    if (log.isInfoEnabled()) {
      log.info("Incoming " + request.getMethod() + " request to " + request.getRequestPath() + ". Content-Type: " + request.getContentType());
    }
    try {
      UriMessagingTargetMount uriMessagingTargetMount = this.getUriMessagingTargetRegistry().lookup(request.getRequestPath());
      if (HttpTransportRequest.METHOD_GET.equals(request.getMethod())) {
        this.processGetRequest(request, response, uriMessagingTargetMount);
      } else {
        if (HttpTransportRequest.METHOD_POST.equals(request.getMethod())) {
          this.processPostRequest(request, response, uriMessagingTargetMount);
        } else {
          if (HttpTransportRequest.METHOD_PUT.equals(request.getMethod())) {
            this.processPutRequest(request, response, uriMessagingTargetMount);
          } else {
            if (HttpTransportRequest.METHOD_DELETE.equals(request.getMethod())) {
              this.processDeleteRequest(request, response, uriMessagingTargetMount);
            } else {
              if (HttpTransportRequest.METHOD_OPTIONS.equals(request.getMethod())) {
                this.processOptionsRequest(request, response);
              } else {
                throw new Xdi2TransportException("Invalid HTTP method: " + request.getMethod());
              }
            }
          }
        }
    ...
    if (log.isDebugEnabled()) {
      log.debug("Successfully processed " + request.getMethod() + " request.");
    }
  }
  ...
  protected void processGetRequest(HttpTransportRequest request, HttpTransportResponse response, UriMessagingTargetMount uriMessagingTargetMount) throws Xdi2TransportException, IOException {
    ...
    try {
      ...
      if (messageEnvelope == null) {
        throw new Xdi2TransportException("No messaging request.");
      }
    } catch (IOException ex) {
    ...
    if (messagingResponse == null || messagingResponse.getGraph() == null) {
      throw new Xdi2TransportException("No messaging response.");
    }
  ...
  private static MessageEnvelope readFromUrl(UriMessagingTargetMount messagingTargetMount, HttpTransportRequest request, HttpTransportResponse response, XDIAddress operationAddress) throws IOException {
    ...
    while (addr.length() > 0 && addr.charAt(0) == '/') {
  ...
  private static MessageEnvelope readFromBody(HttpTransportRequest request, HttpTransportResponse response) throws IOException {
    ...
    if (xdiReader == null) {
      xdiReader = XDIReaderRegistry.getDefault();
    }
    if (log.isDebugEnabled()) {
      log.debug("Reading message in " + recvMimeType + " with reader " + xdiReader.getClass().getSimpleName() + ".");
    }
    MessageEnvelope messageEnvelope;
    try {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Message envelope received (" + messageEnvelope.getMessageCount() + " messages). Executing...");
    }
    return messageEnvelope;
  }
  ...
  private void sendOk(HttpTransportRequest request, HttpTransportResponse response, TransportMessagingResponse messagingResponse) throws IOException {
    ...
    if (HttpTransportRequest.METHOD_GET.equals(request.getMethod())) {
      headers.putAll(this.getHeadersGet());
    }
    if (HttpTransportRequest.METHOD_POST.equals(request.getMethod())) {
      headers.putAll(this.getHeadersPost());
    }
    if (HttpTransportRequest.METHOD_PUT.equals(request.getMethod())) {
      headers.putAll(this.getHeadersPut());
    }
    if (HttpTransportRequest.METHOD_DELETE.equals(request.getMethod())) {
      headers.putAll(this.getHeadersDelete());
    }
    if (HttpTransportRequest.METHOD_OPTIONS.equals(request.getMethod())) {
      headers.putAll(this.getHeadersOptions());
    }
    ...
    if (messagingResponse != null) {
      ...
      if (writer == null) {
        writer = XDIWriterRegistry.getDefault();
      }
      if (log.isDebugEnabled()) {
