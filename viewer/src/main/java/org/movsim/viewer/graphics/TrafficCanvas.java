 ...
 * 
 */
public class TrafficCanvas extends SimulationCanvasBase implements SimulationRunnable.UpdateDrawingCallback, SimulationRunnable.HandleExceptionCallback {
  ...
  @Override public void resetScaleAndOffset() {
    scale = Double.parseDouble(properties.getProperty("initialScale"));
  ...
  private void initGraphicSettings() {
    ...
    if (vehiclesInput == null) {
      logger.warn("vehiclesInput is null. cannot set vehicles\' labelColors.");
    } else {
  ...
  public void setMessageStrings(String popupString, String popupStringExitEndRoad, String trafficInflowString, String perturbationRampingFinishedString, String perturbationAppliedString, String simulationFinished) {
  ...
  /**
     ...
     * @param g
     */
  @Override protected void drawForeground(Graphics2D g) {
  ...
  /**
     ...
     * @param g
     */
  @Override protected void drawBackground(Graphics2D g) {
    if (drawSources) {
      drawSources(g);
    }
    if (drawSinks) {
  ...
  private static void drawRoadSegment(Graphics2D g, RoadMapping roadMapping) {
    final BasicStroke roadStroke = new BasicStroke((float) roadMapping.roadWidth(), BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER);
  ...
  private void drawRoadSegmentLines(Graphics2D g, RoadMapping roadMapping) {
    ...
    final Stroke lineStroke = new BasicStroke(lineWidth, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f, new float[] { lineLength, gapLength }, dashPhase);
    ...
    for (int lane = 1; lane < laneCount; ++lane) {
      ...
      if (lane == roadMapping.trafficLaneMin() || lane == roadMapping.trafficLaneMax()) {
        final Stroke exitStroke = new BasicStroke(lineWidth, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f, new float[] { 5.0f, gapLengthExit }, 5.0f);
  ...
  public static Rectangle2D trafficLightRect(RoadMapping roadMapping, TrafficLight trafficLight) {
    ...
    final Rectangle2D rect = new Rectangle2D.Double(posTheta.x - size / 2, posTheta.y - size / 2, size, size * trafficLight.lightCount());
    return rect;
  }
  ...
  private static void drawTrafficLight3(Graphics2D g, TrafficLight trafficLight, Rectangle2D trafficLightRect, double radius) {
    ...
    if (trafficLight.status() == TrafficLightStatus.GREEN_RED) {
      ...
      if (trafficLight.status() == TrafficLightStatus.RED_GREEN) {
        g.setColor(Color.ORANGE);
      } else {
  ...
  private void drawSpeedLimitsOnRoad(Graphics2D g, RoadSegment roadSegment) {
    ...
    for (final SpeedLimit speedLimit : roadSegment.speedLimits()) {
      ...
      if (speedLimitValueKmh < 150) {
        ...
        g.fillOval((int) posTheta.x - whiteRadius2, (int) posTheta.y - whiteRadius2, 2 * whiteRadius2, 2 * whiteRadius2);
        ...
        final Line2D line = new Line2D.Double((int) posTheta.x - xOnCircle, (int) posTheta.y + yOnCircle, (int) posTheta.x + xOnCircle, (int) posTheta.y - yOnCircle);
  ...
  private void drawRoadSectionIds(Graphics2D g) {
    for (final RoadSegment roadSegment : roadNetwork) {
      ...
      g.setFont(font);
      g.setColor(Color.BLACK);
      g.drawString("R" + roadSegment.userId(), (int) (posTheta.x), (int) (posTheta.y));
    }
  }
  ...
  /**
     ...
     * </p>
     */
  @Override public void updateDrawing(double simulationTime) {
    repaint();
  }
