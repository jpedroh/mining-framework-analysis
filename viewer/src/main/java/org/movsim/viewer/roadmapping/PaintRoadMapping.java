  ...
  public static void paintRoadMapping(Graphics2D g, RoadMapping roadMapping, double lateralOffset) {
    ...
    if (roadMappingClass == RoadMappingLine.class) {
      ...
      if (roadMappingClass == RoadMappingArc.class) {
        ...
        arc2D.setArcByCenter(posTheta.getScreenX() - dx, posTheta.getScreenY() + dy, radius + lateralOffset * (arc.clockwise() ? 1 : -1), Math.toDegrees(angSt), Math.toDegrees(arc.arcAngle()), Arc2D.OPEN);
        g.draw(arc2D);
        return;
      } else {
        if (roadMappingClass == RoadMappingPoly.class) {
          ...
          if (roadMappingClass == RoadMappingPolyLine.class) {
            ...
            if (roadMappingClass == RoadMappingBezier.class) {
              ...
              if (roadMappingClass == RoadMappingPolyBezier.class) {
                ...
                if (lateralOffset == 0.0) {
                  ...
                  final int radiusC = 6;
                  if (drawBezierPoints) {
                    g.fillOval((int) posTheta.getScreenX() - radius / 2, (int) posTheta.getScreenY() - radius / 2, radius, radius);
                  }
                  path.moveTo(posTheta.getScreenX(), posTheta.getScreenY());
                  posTheta = bezier.endPos(lateralOffset);
