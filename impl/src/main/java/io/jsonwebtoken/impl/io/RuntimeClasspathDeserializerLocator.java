 ...
 * @since 0.10.0
 */
public class RuntimeClasspathDeserializerLocator<T extends java.lang.Object> implements InstanceLocator<Deserializer<T>> {
  private static final AtomicReference<Deserializer> DESERIALIZER = new AtomicReference<>();

  @SuppressWarnings(value = { "unchecked" }) @Override public Deserializer<T> getInstance() {
  ...
  @SuppressWarnings(value = { "WeakerAccess" }) protected Deserializer<T> locate() {
    if (isAvailable("io.jsonwebtoken.io.JacksonDeserializer")) {
      ...
      if (isAvailable("io.jsonwebtoken.io.OrgJsonDeserializer")) {
        ...
        if (isAvailable("com.google.gson.GsonBuilder")) {
  ...
  @SuppressWarnings(value = { "WeakerAccess" }) protected boolean compareAndSet(Deserializer<T> d) {
    return DESERIALIZER.compareAndSet(null, d);
  }

  @SuppressWarnings(value = { "WeakerAccess" }) protected boolean isAvailable(String fqcn) {
