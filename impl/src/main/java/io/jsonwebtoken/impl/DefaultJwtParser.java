...
@SuppressWarnings(value = { "unchecked" }) public class DefaultJwtParser implements JwtParser {
  ...
  public static final String MISSING_JWS_ALG_MSG = "JWS header does not contain a required \'alg\' (Algorithm) header parameter.  " + "This header parameter is mandatory per the JWS Specification, Section 4.1.1. See " + "https://datatracker.ietf.org/doc/html/rfc7515#section-4.1.1 for more information.";

  public static final String MISSING_JWE_ALG_MSG = "JWE header does not contain a required \'alg\' (Algorithm) header parameter.  " + "This header parameter is mandatory per the JWE Specification, Section 4.1.1. See " + "https://datatracker.ietf.org/doc/html/rfc7516#section-4.1.1 for more information.";
  ...
  private static final String MISSING_ENC_MSG = "JWE header does not contain a required \'enc\' (Encryption Algorithm) header parameter.  " + "This header parameter is mandatory per the JWE Specification, Section 4.1.2. See " + "https://datatracker.ietf.org/doc/html/rfc7516#section-4.1.2 for more information.";

  private static final String UNSECURED_DISABLED_MSG_PREFIX = "Unsecured JWSs (those with an  " + AbstractHeader.ALGORITHM + " header value of \'" + SignatureAlgorithms.NONE.getId() + "\') are disallowed by default as mandated by " + "https://datatracker.ietf.org/doc/html/rfc7518#section-3.6. If you wish to allow them to be " + "parsed, call the JwtParserBuilder.enableUnsecuredJws() method (but please read the " + "security considerations covered in that method\'s JavaDoc before doing so). Header: ";

  @SuppressWarnings(value = { "deprecation" }) private SigningKeyResolver signingKeyResolver;

  private static final String JWE_NONE_MSG = "JWEs do not support key management " + AbstractHeader.ALGORITHM + " header value \'none\' per https://datatracker.ietf.org/doc/html/rfc7518#section-4.1";

  private static final String JWS_NONE_SIG_MISMATCH_MSG = "The JWS header references signature algorithm \'none\' yet the " + "compact JWS string contains a signature. This is not permitted per " + "https://tools.ietf.org/html/rfc7518#section-3.6.";
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "DeprecatedIsStillUsed" }) @Deprecated public DefaultJwtParser() {
  ...
  @SuppressWarnings(value = { "deprecation" }) DefaultJwtParser(Provider provider, SigningKeyResolver signingKeyResolver, boolean enableUnsecuredJws, Locator<? extends Key> keyLocator, Clock clock, long allowedClockSkewMillis, Claims expectedClaims, Decoder<String, byte[]> base64UrlDecoder, Deserializer<Map<String, ?>> deserializer, Locator<CompressionCodec> compressionCodecLocator, Collection<SignatureAlgorithm<?, ?>> extraSigAlgs, Collection<KeyAlgorithm<?, ?>> extraKeyAlgs, Collection<AeadAlgorithm> extraEncAlgs) {
  ...
  @Override public JwtParser deserializeJsonWith(Deserializer<Map<String, ?>> deserializer) {
    Assert.notNull(deserializer, "deserializer cannot be null.");
    this.deserializer = new JwtDeserializer<>(deserializer);
    return this;
  }

  @Override public JwtParser base64UrlDecodeWith(Decoder<String, byte[]> base64UrlDecoder) {
  ...
  @Override public JwtParser setClock(Clock clock) {
    Assert.notNull(clock, "Clock instance cannot be null.");
    this.clock = clock;
    return this;
  }

  @Override public JwtParser setAllowedClockSkewSeconds(long seconds) throws IllegalArgumentException {
  ...
  @Override public JwtParser setSigningKey(String base64EncodedSecretKey) {
    Assert.hasText(base64EncodedSecretKey, "signing key cannot be null or empty.");
    byte[] bytes = Decoders.BASE64.decode(base64EncodedSecretKey);
    return setSigningKey(bytes);
  }

  @Override public JwtParser setSigningKey(final Key key) {
    Assert.notNull(key, "signing key cannot be null.");
    setSigningKeyResolver(new ConstantKeyLocator(key, null));
    return this;
  }

  @SuppressWarnings(value = { "deprecation" }) @Override public JwtParser setSigningKeyResolver(SigningKeyResolver signingKeyResolver) {
    Assert.notNull(signingKeyResolver, "SigningKeyResolver cannot be null.");
    this.signingKeyResolver = signingKeyResolver;
    return this;
  }

  @SuppressWarnings(value = { "deprecation" }) @Override public JwtParser setCompressionCodecResolver(CompressionCodecResolver compressionCodecResolver) {
    Assert.notNull(compressionCodecResolver, "compressionCodecResolver cannot be null.");
    this.compressionCodecLocator = new CompressionCodecLocator(compressionCodecResolver);
    return this;
  }

  @Override public boolean isSigned(String compact) {
    if (compact == null) {
      return false;
    }
    try {
  ...
  private void verifySignature(final TokenizedJwt tokenized, final JwsHeader jwsHeader, final String alg, @SuppressWarnings(value = { "deprecation" }) SigningKeyResolver resolver, Claims claims, byte[] payload) {
    ...
    try {
      VerifySignatureRequest<Key> request = new DefaultVerifySignatureRequest<>(this.provider, null, data, key, signature);
  ...
  @Override public Jwt<?, ?> parse(String compact) throws ExpiredJwtException, MalformedJwtException, SignatureException {
    ...
    if (tokenized instanceof TokenizedJwe && Arrays.length(payload) == 0) {
    ...
    if (tokenized instanceof TokenizedJwe) {
      ...
      @SuppressWarnings(value = { "rawtypes" }) final KeyAlgorithm keyAlg = this.keyAlgorithmLocator.apply(jweHeader);
      ...
      DecryptionKeyRequest<Key> request = new DefaultDecryptionKeyRequest<>(this.provider, null, key, jweHeader, encAlg, cekBytes);
      ...
      DecryptAeadRequest decryptRequest = new DefaultAeadResult(this.provider, null, payload, cek, aad, tag, iv);
      Message result = encAlg.decrypt(decryptRequest);
      payload = result.getContent();
    } else {
      if (Strings.hasText(base64UrlDigest) && this.signingKeyResolver == null) {
    ...
    Claims claims = null;
    if (!hasContentType(header) && isLikelyJson(payload)) {
    ...
    if (Strings.hasText(base64UrlDigest) && signingKeyResolver != null) {
  ...
  private void validateExpectedClaims(Header<?> header, Claims claims) {
    for (String expectedClaimName : expectedClaims.keySet()) {
      ...
      if (actualClaimValue == null) {
        String msg = String.format(MISSING_EXPECTED_CLAIM_MESSAGE_TEMPLATE, expectedClaimName, expectedClaimValue);
        invalidClaimException = new MissingClaimException(header, claims, msg);
      } else {
        if (!expectedClaimValue.equals(actualClaimValue)) {
          String msg = String.format(INCORRECT_EXPECTED_CLAIM_MESSAGE_TEMPLATE, expectedClaimName, expectedClaimValue, actualClaimValue);
  ...
  @Override public Jwt<UnprotectedHeader, byte[]> parseContentJwt(String compact) {
    return parse(compact, new JwtHandlerAdapter<Jwt<UnprotectedHeader, byte[]>>() {
      @Override public Jwt<UnprotectedHeader, byte[]> onContentJwt(Jwt<UnprotectedHeader, byte[]> jwt) {
        return jwt;
      }
    });
  }

  @Override public Jwt<UnprotectedHeader, Claims> parseClaimsJwt(String compact) {
    return parse(compact, new JwtHandlerAdapter<Jwt<UnprotectedHeader, Claims>>() {
      @Override public Jwt<UnprotectedHeader, Claims> onClaimsJwt(Jwt<UnprotectedHeader, Claims> jwt) {
        return jwt;
      }
    });
  }

  @Override public Jws<byte[]> parseContentJws(String compact) {
    return parse(compact, new JwtHandlerAdapter<Jws<byte[]>>() {
      @Override public Jws<byte[]> onContentJws(Jws<byte[]> jws) {
        return jws;
      }
    });
  }

  @Override public Jws<Claims> parseClaimsJws(String compact) {
  ...
  @Override public Jwe<byte[]> parseContentJwe(String compact) throws JwtException {
    return parse(compact, new JwtHandlerAdapter<Jwe<byte[]>>() {
      @Override public Jwe<byte[]> onContentJwe(Jwe<byte[]> jwe) {
        return jwe;
      }
    });
  }

  @Override public Jwe<Claims> parseClaimsJwe(String compact) throws JwtException {
    return parse(compact, new JwtHandlerAdapter<Jwe<Claims>>() {
      @Override public Jwe<Claims> onClaimsJwe(Jwe<Claims> jwe) {
        return jwe;
      }
    });
  }
