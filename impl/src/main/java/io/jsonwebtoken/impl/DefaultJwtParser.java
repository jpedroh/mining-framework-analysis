...
@SuppressWarnings(value = { "unchecked" }) public class DefaultJwtParser implements JwtParser {
  ...
  private static final String JWE_NONE_MSG = "JWEs do not support key management " + DefaultHeader.ALGORITHM + " header value \'none\' per https://datatracker.ietf.org/doc/html/rfc7518#section-4.1";
  ...
  @SuppressWarnings(value = { "deprecation" }) DefaultJwtParser(Provider provider, SigningKeyResolver signingKeyResolver, boolean enableUnsecuredJws, Function<?, Key> keyLocator, Clock clock, long allowedClockSkewMillis, Claims expectedClaims, Decoder<String, byte[]> base64UrlDecoder, Deserializer<Map<String, ?>> deserializer, CompressionCodecResolver compressionCodecResolver, Collection<SignatureAlgorithm<?, ?>> extraSigAlgs, Collection<KeyAlgorithm<?, ?>> extraKeyAlgs, Collection<AeadAlgorithm> extraEncAlgs) {
  ...
  @Override public Jwt<?, ?> parse(String compact) throws ExpiredJwtException, MalformedJwtException, SignatureException {
    ...
    if (SignatureAlgorithms.NONE.getId().equalsIgnoreCase(alg)) {
      ...
      if (!Strings.hasText(tokenized.getDigest())) {
        ...
        String msg = "The " + type + " header references " + algType + " algorithm \'" + alg + "\' but the " + "compact " + type + " string is missing the required " + digestType + ".";
        throw new MalformedJwtException(msg);
      }
    }
    ...
    if (tokenized instanceof TokenizedJwe) {
      ...
      DecryptAeadRequest decryptRequest = new DefaultAeadResult(this.provider, null, bytes, cek, aad, tag, iv);
    ...
    Claims claims = null;
    if (!payload.isEmpty() && !hasContentType(header) && payload.charAt(0) == '{' && payload.charAt(payload.length() - 1) == '}') {
    ...
    if (jwt instanceof Jws) {
      ...
      try {
        ...
        String msg = 
<<<<<<< /usr/src/app/output/jwtk/jjwt/1e9b969946df0584a8942a189059cee80558a184/impl/src/main/java/io/jsonwebtoken/impl/DefaultJwtParser.java/left.java
        "The parsed JWT indicates it was signed with the " + algId + " signature " + "algorithm, but the specified verification key of type "
=======
        "The parsed JWT indicates it was signed with the \'" + algName + "\' signature " + "algorithm, but the provided " + key.getClass().getName()
>>>>>>> /usr/src/app/output/jwtk/jjwt/1e9b969946df0584a8942a189059cee80558a184/impl/src/main/java/io/jsonwebtoken/impl/DefaultJwtParser.java/right.java
         + " key may " + "not be used to verify " + algId + " signatures.  Because the verification " + "key reflects a specific and expected algorithm, and the JWT does not reflect " + "this algorithm, it is likely that the JWT was not expected and therefore should not be " + 
<<<<<<< /usr/src/app/output/jwtk/jjwt/1e9b969946df0584a8942a189059cee80558a184/impl/src/main/java/io/jsonwebtoken/impl/DefaultJwtParser.java/left.java
...
>>>>>>> /usr/src/app/output/jwtk/jjwt/1e9b969946df0584a8942a189059cee80558a184/impl/src/main/java/io/jsonwebtoken/impl/DefaultJwtParser.java/right.java
  ...
  @Override public Jwt<?, String> parsePlaintextJwt(String plaintextJwt) {
    return parse(plaintextJwt, new JwtHandlerAdapter<Jwt<?, String>>() {
      @Override public Jwt<?, String> onPlaintextJwt(Jwt<?, String> jwt) {
        return jwt;
      }
    });
  }

  @Override public Jwt<?, Claims> parseClaimsJwt(String claimsJwt) {
    return parse(claimsJwt, new JwtHandlerAdapter<Jwt<?, Claims>>() {
      @Override public Jwt<?, Claims> onClaimsJwt(Jwt<?, Claims> jwt) {
        return jwt;
      }
    });
  }
  ...
  @Override public Jwe<String> parsePlaintextJwe(String plaintextJwe) throws JwtException {
    return parse(plaintextJwe, new JwtHandlerAdapter<Jwe<String>>() {
      @Override public Jwe<String> onPlaintextJwe(Jwe<String> jwe) {
        return jwe;
      }
    });
  }

  @Override public Jwe<Claims> parseClaimsJwe(String claimsJwe) throws JwtException {
