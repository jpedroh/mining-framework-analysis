...
public class DefaultJwtBuilder implements JwtBuilder {
  ...
  @Override public JwtBuilder setProvider(Provider provider) {
    this.provider = provider;
    return this;
  }

  @Override public JwtBuilder setSecureRandom(SecureRandom secureRandom) {
    this.secureRandom = secureRandom;
    return this;
  }

  protected <I extends java.lang.Object, O extends java.lang.Object> Function<I, O> wrap(Function<I, O> fn, String fmt, Object... args) {
  ...
  protected Function<Map<String, ?>, byte[]> wrap(final Serializer<Map<String, ?>> serializer, final String which) {
    return new Function<Map<String, ?>, byte[]>() {
      @Override public byte[] apply(Map<String, ?> stringMap) {
        try {
  ...
  @Override public JwtBuilder serializeToJsonWith(final Serializer<Map<String, ?>> serializer) {
  ...
  @Override public JwtBuilder setHeader(Map<String, ?> header) {
    this.header = new DefaultUnprotectedHeader(header);
    return this;
  }

  @Override public JwtBuilder setHeader(Builder<? extends Header<?>> builder) {
  ...
  @Override public JwtBuilder setHeaderParams(Map<String, ?> params) {
  ...
  @Override public <K extends Key> JwtBuilder signWith(K key, final SignatureAlgorithm<K, ?> alg) throws InvalidKeyException {
    Assert.notNull(key, "Key argument cannot be null.");
  ...
  @SuppressWarnings(value = { "deprecation", "unchecked" }) @Override public JwtBuilder signWith(Key key, io.jsonwebtoken.SignatureAlgorithm alg) throws InvalidKeyException {
    Assert.notNull(alg, "SignatureAlgorithm cannot be null.");
    alg.assertValidSigningKey(key);
    return signWith(key, (SignatureAlgorithm<Key, ?>) SignatureAlgorithmsBridge.forId(alg.getValue()));
  }

  @SuppressWarnings(value = { "deprecation" }) @Override public JwtBuilder signWith(io.jsonwebtoken.SignatureAlgorithm alg, byte[] secretKeyBytes) throws InvalidKeyException {
    Assert.notNull(alg, "SignatureAlgorithm cannot be null.");
  ...
  @SuppressWarnings(value = { "deprecation" }) @Override public JwtBuilder signWith(io.jsonwebtoken.SignatureAlgorithm alg, String base64EncodedSecretKey) throws InvalidKeyException {
  ...
  @SuppressWarnings(value = { "deprecation" }) @Override public JwtBuilder signWith(io.jsonwebtoken.SignatureAlgorithm alg, Key key) {
    return signWith(key, alg);
  }

  @Override public JwtBuilder encryptWith(AeadAlgorithm enc, SecretKey key) {
  ...
  @Override public <K extends Key> JwtBuilder encryptWith(final AeadAlgorithm enc, final K key, final KeyAlgorithm<K, ?> keyAlg) {
    ...
    this.encFunction = wrap(new Function<AeadRequest, AeadResult>() {
      @Override public AeadResult apply(AeadRequest request) {
    ...
    this.keyAlgFunction = Functions.wrap(new Function<KeyRequest<Key>, KeyResult>() {
      @Override public KeyResult apply(KeyRequest<Key> request) {
  ...
  @Override public JwtBuilder setContent(byte[] content) {
    this.content = content;
    return this;
  }

  @Override public JwtBuilder setContent(byte[] content, String cty) {
  ...
  @Override public JwtBuilder addClaims(Map<String, ?> claims) {
  ...
  private String compact(Header<?> header, byte[] payload) {
    Assert.stateNotNull(sigAlg, "SignatureAlgorithm is required.");
  ...
  private String encrypt(JweHeader header, byte[] payload) {
    ...
    Assert.stateNotNull(keyAlg, "KeyAlgorithm is required.");
    Assert.stateNotNull(keyAlgFunction, "KeyAlgorithm function cannot be null.");
    Assert.notEmpty(payload, "JWE payload bytes cannot be empty.");
