...
public abstract class AbstractTest extends Arquillian {
  ...
  protected <T extends java.lang.Object> Bean<T> getUniqueBean(Class<T> type, Annotation... bindings) {
    Set<Bean<T>> beans = getBeans(type, bindings);
    return resolveUniqueBean(type, beans);
  }

  protected <T extends java.lang.Object> Bean<T> getUniqueBean(TypeLiteral<T> type, Annotation... bindings) {
    Set<Bean<T>> beans = getBeans(type, bindings);
    return resolveUniqueBean(type.getType(), beans);
  }

  @SuppressWarnings(value = { "unchecked", "rawtypes" }) protected <T extends java.lang.Object> Set<Bean<T>> getBeans(Class<T> type, Annotation... bindings) {
    return (Set) getCurrentManager().getBeans(type, bindings);
  }

  @SuppressWarnings(value = { "unchecked", "rawtypes" }) protected <T extends java.lang.Object> Set<Bean<T>> getBeans(TypeLiteral<T> type, Annotation... bindings) {
    return (Set) getCurrentManager().getBeans(type.getType(), bindings);
  }

  protected <T extends java.lang.Object> T getContextualReference(Class<T> beanType, Annotation... qualifiers) {
    return BeanLookupUtils.<T>getContextualReference(getCurrentManager(), beanType, qualifiers);
  }

  protected <T extends java.lang.Object> T getContextualReference(TypeLiteral<T> beanType, Annotation... qualifiers) {
    return BeanLookupUtils.<T>getContextualReference(getCurrentManager(), beanType, qualifiers);
  }

  protected <T extends java.lang.Object> T getContextualReference(String name, Class<T> beanType) {
    return BeanLookupUtils.<T>getContextualReference(getCurrentManager(), name, beanType);
  }

  protected <T extends java.lang.Object> DependentInstance<T> newDependentInstance(Class<T> beanType, Annotation... qualifiers) {
    return new DependentInstance<T>(getCurrentManager(), beanType, qualifiers);
  }

  private <T extends java.lang.Object> Bean<T> resolveUniqueBean(Type type, Set<Bean<T>> beans) {
    if (beans.size() == 0) {
