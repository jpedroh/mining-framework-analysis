...
@Stateless public class LuceneIndexService {
  ...
  public void indexDocuments(Collection<ItemCollection> documents) {
    ...
    if (updateTime > 5000) {
      logger.warning("... update index block in took " + (updateTime) + " ms ! (" + documents.size() + " documents in total)");
    } else {
      if (logger.isLoggable(Level.FINE)) {
        logger.fine("... update index block in " + (updateTime) + " ms (" + documents.size() + " documents in total)");
      }
    }
  }
