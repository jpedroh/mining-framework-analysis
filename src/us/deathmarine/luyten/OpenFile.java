...
public class OpenFile implements SyntaxConstants {
  public static final HashSet<String> WELL_KNOWN_TEXT_FILE_EXTENSIONS = new HashSet<>(Arrays.asList(".java", ".xml", ".rss", ".project", ".classpath", ".h", ".c", ".cpp", ".yaml", ".yml", ".ini", ".sql", ".js", ".php", ".php5", ".phtml", ".html", ".htm", ".xhtm", ".xhtml", ".lua", ".bat", ".pl", ".sh", ".css", ".json", ".txt", ".rb", ".make", ".mak", ".py", ".properties", ".prop"));
  ...
  public OpenFile(String name, String path, Theme theme, final MainWindow mainWindow) {
    ...
    if (name.toLowerCase().endsWith(".class") || name.toLowerCase().endsWith(".java")) {
      ...
      if (name.toLowerCase().endsWith(".xml") || name.toLowerCase().endsWith(".rss") || name.toLowerCase().endsWith(".project") || name.toLowerCase().endsWith(".classpath")) {
        textArea.setSyntaxEditingStyle(SYNTAX_STYLE_XML);
      } else {
        if (name.toLowerCase().endsWith(".h") || name.toLowerCase().endsWith(".c")) {
          ...
          if (name.toLowerCase().endsWith(".cpp")) {
            ...
            if (name.toLowerCase().endsWith(".sql")) {
              ...
              if (name.toLowerCase().endsWith(".js")) {
                ...
                if (name.toLowerCase().endsWith(".php") || name.toLowerCase().endsWith(".php5") || name.toLowerCase().endsWith(".phtml")) {
                  textArea.setSyntaxEditingStyle(SYNTAX_STYLE_PHP);
                } else {
                  if (name.toLowerCase().endsWith(".html") || name.toLowerCase().endsWith(".htm") || name.toLowerCase().endsWith(".xhtm") || name.toLowerCase().endsWith(".xhtml")) {
                    ...
                    if (name.toLowerCase().endsWith(".js")) {
                      ...
                      if (name.toLowerCase().endsWith(".lua")) {
                        ...
                        if (name.toLowerCase().endsWith(".bat")) {
                          ...
                          if (name.toLowerCase().endsWith(".pl")) {
                                  ...
                                  if (name.toLowerCase().endsWith(".ini")) {
                                    ...
                                    if (name.toLowerCase().endsWith(".yaml") || name.toLowerCase().endsWith(".yml")) {
                                      ...
                                      if (name.toLowerCase().endsWith(".rb")) {
                                        ...
                                        if (name.toLowerCase().endsWith(".make") || name.toLowerCase().endsWith(".mak")) {
    ...
    textArea.setLinkGenerator(new LinkGenerator() {
      @Override public LinkGeneratorResult isLinkAtOffset(RSyntaxTextArea textArea, final int offs) {
        ...
        if (uniqueStr != null && selectionFrom != null) {
          return new LinkGeneratorResult() {
            ...
            @Override public int getSourceOffset() {
              if (isNavigationLinksValid) {
    ...
    textArea.addMouseMotionListener(new MouseMotionAdapter() {
      ...
      @Override public synchronized void mouseMoved(MouseEvent e) {
        ...
        if (isCtrlDown && isWaitForLinksCursor) {
          ...
          if (textArea.getCursor().getType() == Cursor.WAIT_CURSOR) {
  ...
  private void onNavigationClicked(String clickedReferenceUniqueStr) {
    if (isLocallyNavigable(clickedReferenceUniqueStr)) {
      ...
      if (linkProvider.isLinkNavigable(clickedReferenceUniqueStr)) {
