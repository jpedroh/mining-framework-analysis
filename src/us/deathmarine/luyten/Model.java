...
>>>>>>> /usr/src/app/output/deathmarine/luyten/c6cd5d8c8ef78ee339561dbf844332d9aa4a5f8c/src/us/deathmarine/luyten/Model.java/right.java
  ...
  private class TreeListener extends MouseAdapter {
    @Override public void mousePressed(MouseEvent event) {
      boolean isClickCountMatches = (event.getClickCount() == 1 && luytenPrefs.isSingleClickOpenEnabled()) || (event.getClickCount() == 2 && !luytenPrefs.isSingleClickOpenEnabled());
              ...
              if (luytenPrefs.isFollowEmbeddedJarFile()) {
                if (file.getName().endsWith(".war")) {
                  ...
                  try (Stream<Path> paths = Files.walk(tempFileLocation)) {
                    paths.filter(Files::isRegularFile).filter((embeddedFile) -> embeddedFile.toString().endsWith(entryName.substring(entryName.lastIndexOf('/') + 1))).forEach((Path filePath) -> {
  ...
  @SuppressWarnings(value = { "unchecked" }) public DefaultMutableTreeNode getChild(DefaultMutableTreeNode node, TreeNodeUserObject name) {
            ...
            if (luytenPrefs.isPrepareWarFile()) {
              if (file.getName().endsWith(".war")) {
                ...
                try (Stream<Path> paths = Files.walk(tempFileLocation)) {
                  ...
                  paths.filter(Files::isRegularFile).filter((file) -> (file.toString().endsWith(".jar"))).forEach((Path filePath) -> {
  ...
  private void buildFlatTreeFromMass(List<String> mass) {
    ...
    for (String entry : mass) {
      ...
      if (!entry.startsWith("META-INF") && !entry.startsWith("WEB-INF") && packageRoot.trim().length() > 0 && entry.matches(".*\\.(class|java|prop|properties)$")) {
    ...
    for (String packagePath : packages.keySet()) {
      String packageRoot = packagePath.replaceAll("/.*$", "");
      if (!classContainingPackageRoots.contains(packageRoot) && (!packagePath.startsWith("META-INF") || !packagePath.startsWith("WEB-INF")) && packagePath.length() > 0) {
        List<String> packagePathElements = Arrays.asList(packagePath.split("/"));
        for (String entry : packages.get(packagePath)) {
          ArrayList<String> list = new ArrayList<>(packagePathElements);
          list.add(entry);
