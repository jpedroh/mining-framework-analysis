  ...
  public FindAllBox(final MainWindow mainWindow) {
    ...
    list.addMouseListener(new MouseAdapter() {
      public void mouseClicked(MouseEvent evt) {
        @SuppressWarnings(value = { "unchecked" }) JList<String> list = (JList<String>) evt.getSource();
        if (evt.getClickCount() == 2) {
          ...
          if (entryName.toLowerCase().endsWith(".class")) {
            ...
            try {
              mainWindow.getSelectedModel().extractClassToTextPane(type, array[array.length - 1], entryName, null);
            } catch (Exception e) {
            ...
            try {
              ...
              mainWindow.getSelectedModel().extractSimpleFileEntryToTextPane(jfile.getInputStream(jfile.getEntry(entryName)), array[array.length - 1], entryName);
              jfile.close();
            } catch (IOException e) {
              e.printStackTrace();
            } catch (Exception e) {
              ...
              while (ent.hasMoreElements() && findButton.getText().equals("Stop")) {
                ...
                if (locked || classname.isSelected()) {
                  ...
                  if (entry.getName().endsWith(".class")) {
                    ...
                    try (InputStreamReader inputStreamReader = new InputStreamReader(jfile.getInputStream(entry)); BufferedReader reader = new BufferedReader(inputStreamReader)) {
                      String line;
