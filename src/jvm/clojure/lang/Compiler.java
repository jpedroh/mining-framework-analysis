...
public class Compiler implements Opcodes {
  ...
  static class InstanceMethodExpr extends MethodExpr {
    ...
    public InstanceMethodExpr(String source, int line, int column, Symbol tag, IPersistentVector argTags, Expr target, String methodName, IPersistentVector args, boolean tailPosition) {
      this.source = source;
      this.line = line;
      this.column = column;
      this.args = args;
  ...
  static class StaticMethodExpr extends MethodExpr {
    ...
    public StaticMethodExpr(String source, int line, int column, Symbol tag, IPersistentVector argTags, Class c, String methodName, IPersistentVector args, boolean tailPosition) {
      this.c = c;
      this.methodName = methodName;
      this.args = args;
      this.source = source;
  ...
  static public abstract class MethodValueExpr extends FnExpr {
    ...
    private ISeq buildThunk(String name) {
      return RT.list(Symbol.intern("fn"), Symbol.intern(name), buildThunkBody(buildThunkParams()));
    }
  ...
  static public class ConstructorValueExpr extends MethodValueExpr {
    ...
    @Override Executable matchTarget(Class c, Symbol targetSymbol, IPersistentVector sig) {
      return findMatchingTarget(c.getConstructors(), c, klass.getName(), sig);
    }

    @Override IPersistentVector maybeHintReturn(PersistentVector params) {
      return params;
    }

    @Override ISeq buildThunkDispatch(IPersistentVector params) {
  ...
  static public class StaticMethodValueExpr extends MethodValueExpr {
    ...
    @Override ISeq buildThunkDispatch(IPersistentVector params) {
      return RT.listStar(Symbol.intern("."), Symbol.intern(klass.getName()), Symbol.intern(target.getName()), maybeCoerceArgs(params.seq()));
    }
  ...
  static public class InstanceMethodValueExpr extends MethodValueExpr {
    ...
    @Override ISeq buildThunkDispatch(IPersistentVector params) {
      return RT.listStar(Symbol.intern("."), params.seq().first(), Symbol.intern(target.getName()), maybeCoerceArgs(params.seq().next()));
    }
        ...
        if (op instanceof Symbol) {
          ...
          if (namesInstanceMember(sym) || namesQualifiedInstanceMember(sym)) {
            ...
            if (HostExpr.maybeClass(target, false) != null) {
              ...
              if (maybeQualifiedHint != null && target instanceof IObj) {
      ...
      targetStream = targetStream.filter((tgt) -> {
        ...
        for (int i = 0; i < targetSig.length; i++) {
          if (declaredSignature.get(i) == null) {
            ...
            if (!declaredSignature.get(i).equals(targetSig[i])) {
              return false;
            }
          }
        }
