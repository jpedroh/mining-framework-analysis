...
public class TpmPrivateKey {
  ...
  public final void loadPkcs1(ByteBuffer encoding, KeyType keyType) throws TpmPrivateKey.Error {
    if (keyType == null) {
      try {
        ...
        if (children.size() == 9 && children.get(0) instanceof DerNode.DerInteger && ((int) ((DerNode.DerInteger) children.get(0)).toVal()) == 0 && children.get(1) instanceof DerNode.DerInteger && children.get(2) instanceof DerNode.DerInteger && children.get(3) instanceof DerNode.DerInteger && children.get(4) instanceof DerNode.DerInteger && children.get(5) instanceof DerNode.DerInteger && children.get(6) instanceof DerNode.DerInteger && children.get(7) instanceof DerNode.DerInteger && children.get(8) instanceof DerNode.DerInteger) {
          keyType = KeyType.RSA;
        } else {
          keyType = KeyType.EC;
        }
    ...
    if (keyType == KeyType.EC) {
      ...
      if (keyType == KeyType.RSA) {
        pkcs8 = encodePkcs8PrivateKey(encoding, new OID(RSA_ENCRYPTION_OID), new DerNode.DerNull());
      } else {
   ...
   * @throws TpmPrivateKey.Error for errors decoding the key.
   */
  public final void loadPkcs1(ByteBuffer encoding) throws TpmPrivateKey.Error {
   ...
   * @throws TpmPrivateKey.Error for errors decoding the key.
   */
  public final void loadPkcs8(ByteBuffer encoding, KeyType keyType) throws TpmPrivateKey.Error {
    if (keyType == null) {
      ...
      if (oidString.equals(EC_ENCRYPTION_OID)) {
        ...
        if (oidString.equals(RSA_ENCRYPTION_OID)) {
          ...
          throw new TpmPrivateKey.Error("loadPkcs8: Unrecognized private key OID: " + oidString);
        }
      }
    }
    ...
    if (keyType == KeyType.EC) {
      try {
        ...
        throw new TpmPrivateKey.Error("loadPkcs8: PKCS8EncodedKeySpec is not supported for EC: " + ex);
      }
    } else {
      if (keyType == KeyType.RSA) {
        try {
          ...
          throw new TpmPrivateKey.Error("loadPkcs8: PKCS8EncodedKeySpec is not supported for RSA: " + ex);
        }
      } else {
   ...
   * @throws TpmPrivateKey.Error for errors decoding the key.
   */
  public final void loadPkcs8(ByteBuffer encoding) throws TpmPrivateKey.Error {
   ...
   */
  public final void loadEncryptedPkcs8(ByteBuffer encoding, ByteBuffer password) throws TpmPrivateKey.Error {
    String oidString;
    Object parameters;
    Blob encryptedKey;
    try {
      ...
      List algorithmIdChildren = DerNode.getSequence(encryptedPkcs8Children, 0).getChildren();
      oidString = "" + ((DerNode.DerOid) algorithmIdChildren.get(0)).toVal();
      parameters = algorithmIdChildren.get(1);
      encryptedKey = (Blob) ((DerNode.DerOctetString) encryptedPkcs8Children.get(1)).toVal();
    } catch (Throwable ex) {
      throw new TpmPrivateKey.Error("Cannot decode the PKCS #8 EncryptedPrivateKeyInfo: " + ex);
    }
    byte[] pkcs8Encoding;
    if (oidString.equals(PBES2_OID)) {
      ...
      try {
        ...
        List keyDerivationAlgorithmIdChildren = DerNode.getSequence(parametersChildren, 0).getChildren();
        keyDerivationOidString = "" + ((DerNode.DerOid) keyDerivationAlgorithmIdChildren.get(0)).toVal();
        keyDerivationParameters = keyDerivationAlgorithmIdChildren.get(1);
        List encryptionSchemeAlgorithmIdChildren = DerNode.getSequence(parametersChildren, 1).getChildren();
        encryptionSchemeOidString = "" + ((DerNode.DerOid) encryptionSchemeAlgorithmIdChildren.get(0)).toVal();
        encryptionSchemeParameters = encryptionSchemeAlgorithmIdChildren.get(1);
      } catch (Throwable ex) {
        throw new TpmPrivateKey.Error("Cannot decode the PBES2 parameters: " + ex);
      }
      ...
      if (keyDerivationOidString.equals(PBKDF2_OID)) {
        ...
        int nIterations;
        try {
          List Pbkdf2ParametersChildren = ((DerNode.DerSequence) keyDerivationParameters).getChildren();
          salt = (Blob) ((DerNode.DerOctetString) Pbkdf2ParametersChildren.get(0)).toVal();
          nIterations = (int) ((DerNode.DerInteger) Pbkdf2ParametersChildren.get(1)).toVal();
        } catch (Throwable ex) {
          throw new TpmPrivateKey.Error("Cannot decode the PBES2 parameters: " + ex);
        }
        int resultLength;
        if (encryptionSchemeOidString.equals(DES_EDE3_CBC_OID)) {
          ...
          throw new TpmPrivateKey.Error("Unrecognized PBES2 encryption scheme OID: " + encryptionSchemeOidString);
        }
        try {
          derivedKey = Common.computePbkdf2WithHmacSha1(new Blob(password, false).getImmutableArray(), salt.getImmutableArray(), nIterations, resultLength);
        } catch (Throwable ex) {
          throw new TpmPrivateKey.Error("Error computing the derived key using PBKDF2 with HMAC SHA1: " + ex);
        }
      } else {
        throw new TpmPrivateKey.Error("Unrecognized PBES2 key derivation OID: " + keyDerivationOidString);
      }
      if (encryptionSchemeOidString.equals(DES_EDE3_CBC_OID)) {
        Blob initialVector;
        try {
          initialVector = (Blob) ((DerNode.DerOctetString) encryptionSchemeParameters).toVal();
        } catch (Throwable ex) {
          throw new TpmPrivateKey.Error("Cannot decode the DES-EDE3-CBC parameters: " + ex);
        }
        try {
          DESedeKeySpec desKeySpec = new DESedeKeySpec(derivedKey);
          Cipher cipher = Cipher.getInstance("DESede/CBC/PKCS5Padding");
          cipher.init(Cipher.DECRYPT_MODE, SecretKeyFactory.getInstance("DESede").generateSecret(desKeySpec), new IvParameterSpec(initialVector.getImmutableArray()));
          pkcs8Encoding = cipher.doFinal(encryptedKey.getImmutableArray());
        } catch (Throwable ex) {
          throw new TpmPrivateKey.Error("Error decrypting PKCS #8 key with DES-EDE3-CBC: " + ex);
        }
      } else {
        throw new TpmPrivateKey.Error("Unrecognized PBES2 encryption scheme OID: " + encryptionSchemeOidString);
      }
    } else {
      throw new TpmPrivateKey.Error("Unrecognized PKCS #8 EncryptedPrivateKeyInfo OID: " + oidString);
    }
   ...
   */
  public final Blob derivePublicKey() throws TpmPrivateKey.Error {
    if (keyType_ == KeyType.EC) {
      ...
      if (keyType_ == KeyType.RSA) {
        ...
        try {
          java.security.PublicKey publicKey = KeyFactory.getInstance("RSA").generatePublic(new RSAPublicKeySpec(new BigInteger(modulus.getImmutableArray()), new BigInteger(publicExponent.getImmutableArray())));
   ...
   */
  public final Blob decrypt(ByteBuffer cipherText, EncryptAlgorithmType algorithmType) throws TpmPrivateKey.Error {
    if (keyType_ == null) {
    ...
    try {
      ...
      throw new TpmPrivateKey.Error("Error decrypting with private key: " + ex.getMessage());
    }
  }
   ...
   */
  public final Blob decrypt(ByteBuffer cipherText) throws TpmPrivateKey.Error {
   ...
   * in signing.
   */
  public final Blob sign(ByteBuffer data, DigestAlgorithm digestAlgorithm) throws TpmPrivateKey.Error {
    ...
    try {
      ...
      throw new TpmPrivateKey.Error("InvalidKeyException: " + exception.getMessage());
    }
    try {
      signature.update(data);
      return new Blob(signature.sign(), false);
    } catch (SignatureException exception) {
      throw new TpmPrivateKey.Error("SignatureException: " + exception.getMessage());
    }
  }
   ...
   */
  public final Blob toPkcs1() throws TpmPrivateKey.Error {
    if (keyType_ == null) {
   ...
   * @throws TpmPrivateKey.Error if no private key is loaded, or error encoding.
   */
  public final Blob toPkcs8() throws TpmPrivateKey.Error {
    if (keyType_ == null) {
   ...
   * @throws TpmPrivateKey.Error if no private key is loaded, or error encoding.
   */
  public final Blob toEncryptedPkcs8(ByteBuffer password) throws TpmPrivateKey.Error {
    if (keyType_ == null) {
      throw new TpmPrivateKey.Error("toEncryptedPkcs8: The private key is not loaded");
    }
    ...
    byte[] derivedKey;
    try {
      derivedKey = Common.computePbkdf2WithHmacSha1(new Blob(password, false).getImmutableArray(), salt, nIterations, DES_EDE3_KEY_LENGTH);
    } catch (Throwable ex) {
      throw new TpmPrivateKey.Error("Error computing the derived key using PBKDF2 with HMAC SHA1: " + ex);
    }
    ...
    try {
      DESedeKeySpec desKeySpec = new DESedeKeySpec(derivedKey);
      Cipher cipher = Cipher.getInstance("DESede/CBC/PKCS5Padding");
      cipher.init(Cipher.ENCRYPT_MODE, SecretKeyFactory.getInstance("DESede").generateSecret(desKeySpec), new IvParameterSpec(initialVector));
      encryptedEncoding = cipher.doFinal(privateKey_.getEncoded());
    } catch (Throwable ex) {
      throw new TpmPrivateKey.Error("Error encrypting PKCS #8 key with DES-EDE3-CBC: " + ex);
    }
    try {
      DerSequence keyDerivationParameters = new DerSequence();
      keyDerivationParameters.addChild(new DerNode.DerOctetString(ByteBuffer.wrap(salt)));
      ...
      keyDerivationAlgorithmIdentifier.addChild(new DerNode.DerOid(PBKDF2_OID));
      ...
      encryptionSchemeAlgorithmIdentifier.addChild(new DerNode.DerOid(DES_EDE3_CBC_OID));
      encryptionSchemeAlgorithmIdentifier.addChild(new DerNode.DerOctetString(ByteBuffer.wrap(initialVector)));
      ...
      encryptedKey.addChild(new DerNode.DerOctetString(ByteBuffer.wrap(encryptedEncoding)));
      return encryptedKey.encode();
    } catch (DerEncodingException ex) {
      throw new TpmPrivateKey.Error("Error encoding the encryped PKCS #8 private key: " + ex);
    }
  }
   ...
   */
  public static TpmPrivateKey generatePrivateKey(KeyParams keyParams) throws IllegalArgumentException, TpmPrivateKey.Error {
    String keyAlgorithm;
    int keySize;
    ...
    try {
      ...
      throw new TpmPrivateKey.Error("TpmPrivateKey: Could not create the key generator: " + e.getMessage());
    }
   ...
   */
  private static Blob encodePkcs8PrivateKey(ByteBuffer privateKeyDer, OID oid, DerNode parameters) throws TpmPrivateKey.Error {
    try {
