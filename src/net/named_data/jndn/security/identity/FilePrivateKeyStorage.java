...
public class FilePrivateKeyStorage extends PrivateKeyStorage {
  ...
  public FilePrivateKeyStorage() {
    keyStorePath_ = new File(new File(System.getProperty("user.home", "."), ".ndn"), "ndnsec-tpm-file");
  ...
  public final void generateKeyPair(Name keyName, KeyParams params) throws SecurityException {
    ...
    if (params.getKeyType() == KeyType.RSA) {
      ...
      if (params.getKeyType() == KeyType.ECDSA) {
        keyAlgorithm = "EC";
  ...
  public final Blob sign(ByteBuffer data, Name keyName, DigestAlgorithm digestAlgorithm) throws SecurityException {
    ...
    if (keyType[0] == KeyType.RSA) {
      ...
      if (keyType[0] == KeyType.ECDSA) {
        ...
        throw new SecurityException("FilePrivateKeyStorage: Unsupported signature key type " + keyType[0]);
      }
    }
    try {
  ...
  private void write(Name keyName, KeyClass keyClass, byte[] data) throws SecurityException {
    String extension = (String) keyTypeMap_.get(keyClass);
    try {
      BufferedWriter writer = new BufferedWriter(new FileWriter(nameTransform(keyName.toUri(), extension)));
      try {
  ...
  private byte[] read(Name keyName, KeyClass keyClass) throws SecurityException {
    ...
    try {
      BufferedReader reader = new BufferedReader(new FileReader(nameTransform(keyName.toUri(), extension)));
      try {
        String line = null;
