...
public class Tlv0_1_1WireFormat extends WireFormat {
  ...
  public Blob encodeInterest(Interest interest, int[] signedPortionBeginOffset, int[] signedPortionEndOffset) {
    ...
    encoder.writeOptionalNonNegativeIntegerTlvFromDouble(Tlv.InterestLifetime, interest.getInterestLifetimeMilliseconds());
    ...
    encodeName(interest.getName(), tempSignedPortionBeginOffset, tempSignedPortionEndOffset, encoder);
  ...
  public void decodeInterest(Interest interest, ByteBuffer input, int[] signedPortionBeginOffset, int[] signedPortionEndOffset) throws EncodingException {
    ...
    decodeName(interest.getName(), signedPortionBeginOffset, signedPortionEndOffset, decoder);
    if (decoder.peekType(Tlv.Selectors, endOffset)) {
      decodeSelectors(interest, decoder);
    }
    ByteBuffer nonce = decoder.readBlobTlv(Tlv.Nonce);
    interest.scope_ = (int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.Scope, endOffset);
  ...
  public Blob encodeControlParameters(ControlParameters controlParameters) {
    ...
    int saveLength = encoder.getLength();
    encoder.writeOptionalNonNegativeIntegerTlvFromDouble(Tlv.ControlParameters_ExpirationPeriod, controlParameters.getExpirationPeriod());
    if (controlParameters.getStrategy().size() != 0) {
      int strategySaveLength = encoder.getLength();
      encodeName(controlParameters.getStrategy(), new int[1], new int[1], encoder);
    ...
    if (!controlParameters.getUri().isEmpty()) {
      encoder.writeBlobTlv(Tlv.ControlParameters_Uri, new Blob(controlParameters.getUri()).buf());
    }
  ...
  public void decodeControlParameters(ControlParameters controlParameters, ByteBuffer input) throws EncodingException {
    ...
    TlvDecoder decoder = new TlvDecoder(input);
    int endOffset = decoder.readNestedTlvsStart(Tlv.ControlParameters_ControlParameters);
    ...
    controlParameters.setLocalControlFeature((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.ControlParameters_LocalControlFeature, endOffset));
    controlParameters.setOrigin((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.ControlParameters_Origin, endOffset));
    controlParameters.setCost((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.ControlParameters_Cost, endOffset));
    if (decoder.peekType(Tlv.ControlParameters_Flags, endOffset)) {
      ForwardingFlags flags = new ForwardingFlags();
      flags.setNfdForwardingFlags((int) decoder.readNonNegativeIntegerTlv(Tlv.ControlParameters_Flags));
  ...
  public Blob encodeLocalControlHeader(LocalControlHeader localControlHeader) {
    ...
    encoder.writeOptionalNonNegativeIntegerTlv(Tlv.LocalControlHeader_NextHopFaceId, localControlHeader.getNextHopFaceId());
    encoder.writeOptionalNonNegativeIntegerTlv(Tlv.LocalControlHeader_IncomingFaceId, localControlHeader.getIncomingFaceId());
    encoder.writeTypeAndLength(Tlv.LocalControlHeader_LocalControlHeader, encoder.getLength() - saveLength);
    return new Blob(encoder.getOutput(), false);
  }
  ...
  public void decodeLocalControlHeader(LocalControlHeader localControlHeader, ByteBuffer input) throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    int endOffset = decoder.readNestedTlvsStart(Tlv.LocalControlHeader_LocalControlHeader);
    localControlHeader.setIncomingFaceId(decoder.readOptionalNonNegativeIntegerTlv(Tlv.LocalControlHeader_IncomingFaceId, endOffset));
    localControlHeader.setNextHopFaceId(decoder.readOptionalNonNegativeIntegerTlv(Tlv.LocalControlHeader_NextHopFaceId, endOffset));
    if (decoder.peekType(Tlv.LocalControlHeader_CachingPolicy, endOffset)) {
      int cachingPolicyEndOffset = decoder.readNestedTlvsStart(Tlv.LocalControlHeader_CachingPolicy);
  ...
  public Blob encodeEncryptedContent(EncryptedContent encryptedContent) {
    TlvEncoder encoder = new TlvEncoder(256);
    int saveLength = encoder.getLength();
    encoder.writeBlobTlv(Tlv.EncryptedContent_EncryptedPayload, encryptedContent.getPayload().buf());
    encoder.writeOptionalBlobTlv(Tlv.EncryptedContent_InitialVector, encryptedContent.getInitialVector().buf());
    encoder.writeNonNegativeIntegerTlv(Tlv.EncryptedContent_EncryptionAlgorithm, encryptedContent.getAlgorithmType());
    Tlv0_1_1WireFormat.encodeKeyLocator(Tlv.KeyLocator, encryptedContent.getKeyLocator(), encoder);
    encoder.writeTypeAndLength(Tlv.EncryptedContent_EncryptedContent, encoder.getLength() - saveLength);
    return new Blob(encoder.getOutput(), false);
  }
  ...
  public void decodeEncryptedContent(EncryptedContent encryptedContent, ByteBuffer input) throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    int endOffset = decoder.readNestedTlvsStart(Tlv.EncryptedContent_EncryptedContent);
    Tlv0_1_1WireFormat.decodeKeyLocator(Tlv.KeyLocator, encryptedContent.getKeyLocator(), decoder);
    encryptedContent.setAlgorithmType((int) decoder.readNonNegativeIntegerTlv(Tlv.EncryptedContent_EncryptionAlgorithm));
    encryptedContent.setInitialVector(new Blob(decoder.readOptionalBlobTlv(Tlv.EncryptedContent_InitialVector, endOffset), true));
    encryptedContent.setPayload(new Blob(decoder.readBlobTlv(Tlv.EncryptedContent_EncryptedPayload), true));
    decoder.finishNestedTlvs(endOffset);
  }
  ...
  private static void encodeSelectors(Interest interest, TlvEncoder encoder) {
    ...
    if (interest.getKeyLocator().getType() != KeyLocatorType.NONE) {
      encodeKeyLocator(Tlv.PublisherPublicKeyLocator, interest.getKeyLocator(), encoder);
    } else {
      if (interest.getPublisherPublicKeyDigest().getPublisherPublicKeyDigest().size() > 0) {
        ...
        encoder.writeBlobTlv(Tlv.KeyLocatorDigest, interest.getPublisherPublicKeyDigest().getPublisherPublicKeyDigest().buf());
        encoder.writeTypeAndLength(Tlv.KeyLocator, encoder.getLength() - savePublisherPublicKeyDigestLength);
      }
    }
  ...
  private static void decodeSelectors(Interest interest, TlvDecoder decoder) throws EncodingException {
    ...
    interest.setMinSuffixComponents((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.MinSuffixComponents, endOffset));
    interest.setMaxSuffixComponents((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.MaxSuffixComponents, endOffset));
    interest.getPublisherPublicKeyDigest().clear();
    if (decoder.peekType(Tlv.PublisherPublicKeyLocator, endOffset)) {
      decodeKeyLocator(Tlv.PublisherPublicKeyLocator, interest.getKeyLocator(), decoder);
      if (interest.getKeyLocator().getType() == KeyLocatorType.KEY_LOCATOR_DIGEST) {
        interest.getPublisherPublicKeyDigest().setPublisherPublicKeyDigest(interest.getKeyLocator().getKeyData());
      }
    } else {
    ...
    interest.setChildSelector((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.ChildSelector, endOffset));
  ...
  private static void encodeExclude(Exclude exclude, TlvEncoder encoder) {
    ...
    for (int i = exclude.size() - 1; i >= 0; --i) {
      ...
      if (entry.getType() == Exclude.Type.ANY) {
        ...
        encoder.writeBlobTlv(Tlv.NameComponent, entry.getComponent().getValue().buf());
      }
    }
  ...
  private static void decodeExclude(Exclude exclude, TlvDecoder decoder) throws EncodingException {
    ...
    while (true) {
      if (decoder.peekType(Tlv.NameComponent, endOffset)) {
        exclude.appendComponent(new Name.Component(new Blob(decoder.readBlobTlv(Tlv.NameComponent), true)));
      } else {
        if (decoder.readBooleanTlv(Tlv.Any, endOffset)) {
          exclude.appendAny();
        } else {
          break;
        }
  ...
  private static void decodeKeyLocator(int expectedType, KeyLocator keyLocator, TlvDecoder decoder) throws EncodingException {
    ...
    if (decoder.peekType(Tlv.Name, endOffset)) {
      ...
      if (decoder.peekType(Tlv.KeyLocatorDigest, endOffset)) {
        keyLocator.setType(KeyLocatorType.KEY_LOCATOR_DIGEST);
        keyLocator.setKeyData(new Blob(decoder.readBlobTlv(Tlv.KeyLocatorDigest), true));
      } else {
  ...
  private void encodeSignatureInfo(Signature signature, TlvEncoder encoder) {
    ...
    if (signature instanceof Sha256WithRsaSignature) {
      encodeKeyLocator(Tlv.KeyLocator, ((Sha256WithRsaSignature) signature).getKeyLocator(), encoder);
      ...
      if (signature instanceof Sha256WithEcdsaSignature) {
        encodeKeyLocator(Tlv.KeyLocator, ((Sha256WithEcdsaSignature) signature).getKeyLocator(), encoder);
        encoder.writeNonNegativeIntegerTlv(Tlv.SignatureType, Tlv.SignatureType_SignatureSha256WithEcdsa);
      } else {
        if (signature instanceof DigestSha256Signature) {
          encoder.writeNonNegativeIntegerTlv(Tlv.SignatureType, Tlv.SignatureType_DigestSha256);
        } else {
  ...
  private static void decodeSignatureInfo(SignatureHolder signatureHolder, TlvDecoder decoder) throws EncodingException {
    ...
    if (signatureType == Tlv.SignatureType_SignatureSha256WithRsa) {
      ...
      if (signatureType == Tlv.SignatureType_SignatureSha256WithEcdsa) {
        ...
        if (signatureType == Tlv.SignatureType_DigestSha256) {
  ...
  private static void encodeMetaInfo(MetaInfo metaInfo, TlvEncoder encoder) {
    ...
    encoder.writeOptionalNonNegativeIntegerTlvFromDouble(Tlv.FreshnessPeriod, metaInfo.getFreshnessPeriod());
    if (!(metaInfo.getType() == ContentType.BLOB || metaInfo.getType() == ContentType.DATA)) {
      if (metaInfo.getType() == ContentType.LINK || metaInfo.getType() == ContentType.KEY) {
        encoder.writeNonNegativeIntegerTlv(Tlv.ContentType, metaInfo.getType().getNumericType());
      } else {
  ...
  private static void decodeMetaInfo(MetaInfo metaInfo, TlvDecoder decoder) throws EncodingException {
    ...
    if (decoder.peekType(Tlv.FinalBlockId, endOffset)) {
      ...
      metaInfo.setFinalBlockId(new Name.Component(new Blob(decoder.readBlobTlv(Tlv.NameComponent), true)));
