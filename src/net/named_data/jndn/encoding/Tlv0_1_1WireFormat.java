...
public class Tlv0_1_1WireFormat extends WireFormat {
  ...
  public void decodeInterest(Interest interest, ByteBuffer input, int[] signedPortionBeginOffset, int[] signedPortionEndOffset) throws EncodingException {
    ...
    interest.setSelectedDelegationIndex((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.SelectedDelegation, endOffset));
  ...
  public Blob encodeControlResponse(ControlResponse controlResponse) {
    ...
    if (controlResponse.getBodyAsControlParameters() != null) {
      encodeControlParameters(controlResponse.getBodyAsControlParameters(), encoder);
    }
  ...
  public void decodeControlResponse(ControlResponse controlResponse, ByteBuffer input) throws EncodingException {
    ...
    controlResponse.setStatusCode((int) decoder.readNonNegativeIntegerTlv(Tlv.NfdCommand_StatusCode));
    Blob statusText = new Blob(decoder.readBlobTlv(Tlv.NfdCommand_StatusText), true);
  ...
  public Blob encodeEncryptedContent(EncryptedContent encryptedContent) {
    ...
    encoder.writeNonNegativeIntegerTlv(Tlv.Encrypt_EncryptionAlgorithm, encryptedContent.getAlgorithmType().getNumericType());
  ...
  public void decodeEncryptedContent(EncryptedContent encryptedContent, ByteBuffer input) throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    int endOffset = decoder.readNestedTlvsStart(Tlv.Encrypt_EncryptedContent);
    Tlv0_1_1WireFormat.decodeKeyLocator(Tlv.KeyLocator, encryptedContent.getKeyLocator(), decoder);
    int algorithmType = (int) decoder.readNonNegativeIntegerTlv(Tlv.Encrypt_EncryptionAlgorithm);
    if (algorithmType == EncryptAlgorithmType.AesEcb.getNumericType()) {
      encryptedContent.setAlgorithmType(EncryptAlgorithmType.AesEcb);
    } else {
      if (algorithmType == EncryptAlgorithmType.AesCbc.getNumericType()) {
        encryptedContent.setAlgorithmType(EncryptAlgorithmType.AesCbc);
      } else {
        if (algorithmType == EncryptAlgorithmType.RsaPkcs.getNumericType()) {
          encryptedContent.setAlgorithmType(EncryptAlgorithmType.RsaPkcs);
        } else {
          if (algorithmType == EncryptAlgorithmType.RsaOaep.getNumericType()) {
  ...
  private static void decodeMetaInfo(MetaInfo metaInfo, TlvDecoder decoder) throws EncodingException {
    ...
    if (type == ContentType.LINK.getNumericType()) {
      ...
      if (type == ContentType.KEY.getNumericType()) {
        ...
        if (type == ContentType.NACK.getNumericType()) {
  ...
  private static void decodeControlParameters(ControlParameters controlParameters, TlvDecoder decoder) throws EncodingException {
