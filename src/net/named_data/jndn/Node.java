...
public class Node implements ElementListener {
  ...
  public final long expressInterest(Interest interest, OnData onData, OnTimeout onTimeout, WireFormat wireFormat) throws IOException {
    ...
    pendingInterestTable_.add(new PendingInterest(pendingInterestId, new Interest(interest), onData, onTimeout));
   ...
   */
  public final long registerPrefix(Name prefix, OnInterest onInterest, OnRegisterFailed onRegisterFailed, ForwardingFlags flags, WireFormat wireFormat, KeyChain commandKeyChain, Name commandCertificateName) throws IOException, SecurityException {
    long registeredPrefixId = RegisteredPrefix.getNextRegisteredPrefixId();
    if (ndndId_.size() != 0 || commandKeyChain == null) {
      if (ndndId_.size() == 0) {
        NdndIdFetcher fetcher = new NdndIdFetcher(new NdndIdFetcher.Info(this, registeredPrefixId, prefix, onInterest, onRegisterFailed, flags, wireFormat));
        expressInterest(ndndIdFetcherInterest_, fetcher, fetcher, wireFormat);
      } else {
        registerPrefixHelper(registeredPrefixId, new Name(prefix), onInterest, onRegisterFailed, flags, wireFormat);
      }
    } else {
      nfdRegisterPrefix(registeredPrefixId, new Name(prefix), onInterest, onRegisterFailed, flags, commandKeyChain, commandCertificateName, wireFormat);
    }
  ...
  public final void removeRegisteredPrefix(long registeredPrefixId) {
    for (int i = (int) registeredPrefixTable_.size() - 1; i >= 0; --i) {
      if (((RegisteredPrefix) registeredPrefixTable_.get(i)).getRegisteredPrefixId() == registeredPrefixId) {
  ...
  public final void processEvents() throws IOException, EncodingException {
    ...
    for (int i = pendingInterestTable_.size() - 1; i >= 0; --i) {
      if (((PendingInterest) pendingInterestTable_.get(i)).isTimedOut(nowMilliseconds)) {
  ...
  public final void onReceivedElement(ByteBuffer element) throws EncodingException {
    ...
    if (interest != null) {
      ...
      if (entry != null) {
        entry.getOnInterest().onInterest(entry.getPrefix(), interest, transport_, entry.getRegisteredPrefixId());
      }
    } else {
      if (data != null) {
  ...
  private static class PendingInterest {
    public PendingInterest(long pendingInterestId, Interest interest, OnData onData, OnTimeout onTimeout) {
      ...
      if (interest_.getInterestLifetimeMilliseconds() >= 0.0) {
        timeoutTimeMilliseconds_ = Common.getNowMilliseconds() + interest_.getInterestLifetimeMilliseconds();
      } else {
     ...
     */
    public boolean isTimedOut(double nowMilliseconds) {
    ...
    /**< A class variable used to get the next unique ID. */
  ...
  private static class RegisteredPrefix {
    public RegisteredPrefix(long registeredPrefixId, Name prefix, OnInterest onInterest) {
  ...
  private static class NdndIdFetcher implements OnData, OnTimeout {
    ...
    public void onData(Interest interest, Data ndndIdData) {
      ...
      info_.node_.registerPrefixHelper(info_.registeredPrefixId_, info_.prefix_, info_.onInterest_, info_.onRegisterFailed_, info_.flags_, info_.wireFormat_);
    }
    ...
    private static class Info {
       ...
       * @param wireFormat
       */
      public Info(Node node, long registeredPrefixId, Name prefix, OnInterest onInterest, OnRegisterFailed onRegisterFailed, ForwardingFlags flags, WireFormat wireFormat) {
        node_ = node;
        registeredPrefixId_ = registeredPrefixId;
  ...
  private static class RegisterResponse implements OnData, OnTimeout {
    ...
    public void onTimeout(Interest timedOutInterest) {
      if (info_.isNfdCommand_) {
        ...
        if (info_.node_.ndndId_.size() == 0) {
          NdndIdFetcher fetcher = new NdndIdFetcher(new NdndIdFetcher.Info(info_.node_, 0, info_.prefix_, info_.onInterest_, info_.onRegisterFailed_, info_.flags_, info_.wireFormat_));
          try {
            info_.node_.expressInterest(info_.node_.ndndIdFetcherInterest_, fetcher, fetcher, info_.wireFormat_);
          ...
          info_.node_.registerPrefixHelper(0, new Name(info_.prefix_), info_.onInterest_, info_.onRegisterFailed_, info_.flags_, info_.wireFormat_);
        }
      } else {
    ...
    public static class Info {
      public Info(Node node, Name prefix, OnInterest onInterest, OnRegisterFailed onRegisterFailed, ForwardingFlags flags, WireFormat wireFormat, boolean isNfdCommand) {
        node_ = node;
        prefix_ = prefix;
        onInterest_ = onInterest;
        onRegisterFailed_ = onRegisterFailed;
  ...
  private void extractEntriesForExpressedInterest(Name name, ArrayList entries) {
    for (int i = pendingInterestTable_.size() - 1; i >= 0; --i) {
      if (((PendingInterest) pendingInterestTable_.get(i)).getInterest().matchesName(name)) {
  ...
  private RegisteredPrefix getEntryForRegisteredPrefix(Name name) {
    ...
    for (int i = 0; i < registeredPrefixTable_.size(); ++i) {
      RegisteredPrefix registeredPrefix = (RegisteredPrefix) registeredPrefixTable_.get(i);
      if (registeredPrefix.getPrefix().match(name)) {
        if (iResult < 0 || registeredPrefix.getPrefix().size() > ((RegisteredPrefix) registeredPrefixTable_.get(iResult)).getPrefix().size()) {
          iResult = i;
        }
      }
    }
   ...
   * @param wireFormat
   */
  private void registerPrefixHelper(long registeredPrefixId, Name prefix, OnInterest onInterest, OnRegisterFailed onRegisterFailed, ForwardingFlags flags, WireFormat wireFormat) {
    ...
    if (registeredPrefixId != 0) {
      registeredPrefixTable_.add(new RegisteredPrefix(registeredPrefixId, prefix, onInterest));
    }
    RegisterResponse response = new RegisterResponse(new RegisterResponse.Info(this, prefix, onInterest, onRegisterFailed, flags, wireFormat, false));
    try {
   ...
   * certificateName.
   */
  private void nfdRegisterPrefix(long registeredPrefixId, Name prefix, OnInterest onInterest, OnRegisterFailed onRegisterFailed, ForwardingFlags flags, KeyChain commandKeyChain, Name commandCertificateName, WireFormat wireFormat) throws SecurityException {
    ...
    RegisterResponse response = new RegisterResponse(new RegisterResponse.Info(this, prefix, onInterest, onRegisterFailed, flags, wireFormat, true));
    try {
