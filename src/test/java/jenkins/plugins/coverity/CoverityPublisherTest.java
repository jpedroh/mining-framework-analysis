...
public @RunWith(value = PowerMockRunner.class) @PrepareForTest(value = { Jenkins.class, Executor.class, ToolLocationNodeProperty.class, SaveableListener.class }) class CoverityPublisherTest {
  @Rule public TemporaryFolder idir = new TemporaryFolder();

  @Test public void getCimStream_fromOldPre12Configuration() {
  ...
  @Test public void getInvocationAssistance_fromOldPre190Configuration() {
  ...
  @Test public void getInvocationAssistance_fromPre110Configuration_withSaOverride() {
  ...
  @Test public void checkIAConfig_withEmptyMisraConfig() {
    final String expectedOkMessage = "Pass";
    final String expectedErrorMessage = "Errors with your \"Perform Coverity build/analyze/commit\" options: \n" + "[Error] No MISRA configuration file was specified. \n";
  ...
  @Test public void deleteIntermediateDirectoryTest_DeleteIdir() {
    ...
    consoleLogger.verifyMessages("[Coverity] deleting intermediate directory: " + tempDir.getTempDir(), "[Coverity] deleting intermediate directory  \"" + tempDir.getTempDir() + "\" was successful");
    assertFalse(idir.getRoot().exists());
  }

  @Test public void deleteIntermediateDirectoryTest_PreserveIdir() {
    assertTrue(idir.getRoot().exists());
    CoverityTempDir tempDir = new CoverityTempDir(new FilePath(idir.getRoot()), false);
    CoverityPublisher publisher = new CoverityPublisherBuilder().withKeepIntDir(true).build();
    BuildListener listener = mock(BuildListener.class);
    TestableConsoleLogger consoleLogger = new TestableConsoleLogger();
    when(listener.getLogger()).thenReturn(consoleLogger.getPrintStream());
    publisher.deleteIntermediateDirectory(listener, tempDir);
    consoleLogger.verifyMessages("[Coverity] preserving intermediate directory: " + tempDir.getTempDir());
    assertTrue(idir.getRoot().exists());
  }

  @Mock private Jenkins jenkins;

  @Rule private TemporaryFolder tempJenkinsRoot = new TemporaryFolder();
  ...
  @Test public void perform_runsToolHandlerWithoutExceptions() throws IOException, InterruptedException {
