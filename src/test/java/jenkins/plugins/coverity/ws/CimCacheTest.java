...
@RunWith(value = PowerMockRunner.class) @PrepareForTest(value = { WebServiceFactory.class, Secret.class, CredentialsMatchers.class, CredentialsProvider.class }) public class CimCacheTest {
  ...
  @Test public void getProjects_returnsProjectsForInstances() throws IOException {
    ...
    assertEquals(expectedProjectNames, projects);
    cimInstance = new CIMInstanceBuilder().withName("test-instance-2").withHost("test.coverity2.").withPort(8080).withUseSSL(false).withDefaultCredentialId().build();
  ...
  @Test public void getStreams_returnsStreamsForInstances() throws IOException {
    CredentialUtil.setCredentialManager("admin", "password");
    CIMInstance cimInstance = new CIMInstanceBuilder().withName("test").withHost("test.coverity").withPort(8080).withUseSSL(false).withDefaultCredentialId().build();
    TestConfigurationService testConfigurationService = (TestConfigurationService) WebServiceFactory.getInstance().getConfigurationService(cimInstance);
