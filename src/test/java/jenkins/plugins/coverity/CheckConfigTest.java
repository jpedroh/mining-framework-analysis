...
@RunWith(value = PowerMockRunner.class) @PrepareForTest(value = { Jenkins.class, Executor.class, ToolLocationNodeProperty.class, WebServiceFactory.class, Secret.class, CredentialsMatchers.class, CredentialsProvider.class }) public class CheckConfigTest {
  @Mock private Jenkins jenkins;

  @Mock private CoverityPublisher.DescriptorImpl descriptor;
  ...
  @Mock UsernamePasswordCredentials credentials;
  ...
  @Test public void checkStreamTest_NoCIMStream() {
  ...
  @Test public void checkStreamTest_NoCIMInstance() {
  ...
  @Test public void checkStreamTest_NoStreamConfiguredForCIMStream() {
    CredentialUtil.setCredentialManager("admin", "password");
    CoverityPublisher publisher = new CoverityPublisherBuilder().build();
    CIMInstance cimInstance = new CIMInstanceBuilder().withName("test-cim-instance").withHost("test-cim-instance").withPort(8080).withUseSSL(false).withDefaultCredentialId().build();
    CIMStream cimStream = new CIMStream("test-cim-instance", StringUtils.EMPTY, StringUtils.EMPTY);
    ...
    String statusMessage = status.getStatus();
    assertNotNull(statusMessage);
    assertEquals("[Stream] test-cim-instance/null/null : " + "Could not find any Stream that matches the given configuration for this job.", statusMessage);
    assertNull(status.getVersion());
  }

  @Test public void checkStreamTest_WhenCIMInstance_DoCheckFail() throws IOException {
  ...
  @Test public void checkStreamTest_WhenStreamDoesNotExist() throws IOException {
    CoverityPublisher publisher = new CoverityPublisherBuilder().build();
    CIMInstance cimInstance = Mockito.mock(CIMInstance.class);
  ...
  @Test public void checkTaOptionBlockTest_Success() {
    TaOptionBlock taOptionBlock = new TaOptionBlockBuilder().withJavaOptionBlock(true).withJavaCoverageTool("Jacoco").withJunitFramework(true).withPolicyFile("TestPolicyFilePath").build();
  ...
  @Test public void checkTaOptionBlockTest_NoCoverityLanguageSelected() {
    TaOptionBlock taOptionBlock = new TaOptionBlockBuilder().withPolicyFile("TestPolicyFilePath").build();
    CheckConfig.Status status = CheckConfig.checkTaOptionBlock(taOptionBlock);
    assertNotNull(status);
    assertFalse(status.isValid());
    String statusMessage = status.getStatus();
  ...
  @Test public void checkScmOptionBlockTest_Success() {
  ...
  @Test public void checkScmOptionBlockTest_NoAccurevRepo() {
    ScmOptionBlock scmOptionBlock = new ScmOptionBlockBuilder().withScmSystem("accurev").build();
    CheckConfig.Status status = CheckConfig.checkScmOptionBlock(scmOptionBlock);
    assertNotNull(status);
    assertFalse(status.isValid());
    String statusMessage = status.getStatus();
  ...
  @Test public void checkNode_whenNoInstallationFound() throws IOException, InterruptedException {
  ...
  @Test public void checkNode_withInstallation() throws IOException, InterruptedException {
