...
public class CovCommitDefectsCommandTest extends CommandTestBase {
  @Test public void prepareCommandTest() throws IOException, InterruptedException {
    ...
    CIMInstance cimInstance = new CIMInstanceBuilder().withName("TestInstance").withHost("Localhost").withPort(8080).withUseSSL(false).withDefaultCredentialId().build();
  ...
  @Test public void addHttpsPortTest() throws IOException, InterruptedException {
    CredentialUtil.setCredentialManager("TestUser", "TestPassword");
    CIMStream cimStream = new CIMStream("TestInstance", "TestProject", "TestStream");
    CIMInstance cimInstance = new CIMInstanceBuilder().withName("TestInstance").withHost("Localhost").withPort(8080).withUseSSL(true).withDefaultCredentialId().build();
    InvocationAssistance invocationAssistance = new InvocationAssistanceBuilder().build();
    ...
    Command covCommitDefectsCommand = new CovCommitDefectsCommand(build, launcher, listener, publisher, StringUtils.EMPTY, envVars, cimStream, cimInstance);
    setExpectedArguments(new String[] { "cov-commit-defects", "--dir", "TestDir", "--host", "Localhost", "--ssl", "--https-port", "8080", "--on-new-cert", "trust", "--certs", "TestCertFile", "--stream", "TestStream", "--user", "TestUser" });
    covCommitDefectsCommand.runCommand();
    assertEquals("TestPassword", envVars.get("COVERITY_PASSPHRASE"));
    consoleLogger.verifyLastMessage("[Coverity] cov-commit-defects command line arguments: " + actualArguments.toString());
  }
  ...
  @Test public void doesNotExecute_WithoutInvocationAssistance() throws IOException, InterruptedException {
    CredentialUtil.setCredentialManager("TestUser", "TestPassword");
    CIMStream cimStream = new CIMStream("TestInstance", "TestProject", "TestStream");
    CIMInstance cimInstance = new CIMInstanceBuilder().withName("TestInstance").withHost("Localhost").withPort(8080).withUseSSL(false).withDefaultCredentialId().build();
