...
@RunWith(value = PowerMockRunner.class) @PrepareForTest(value = { Jenkins.class, WebServiceFactory.class, Client.class, Secret.class, CredentialsMatchers.class, CredentialsProvider.class }) public class CoverityViewResultsDescriptorTest {
  ...
  @Test public void doFillInstanceItems_returnsGlobalPublisherIntances() {
  ...
  @Test public void doCheckViews_nullValuesRequired() {
    final CoverityViewResultsDescriptor descriptor = new CoverityViewResultsDescriptor();
  ...
  @Test public void doCheckViews_emptyValuesRequired() {
    final CoverityViewResultsDescriptor descriptor = new CoverityViewResultsDescriptor();
  ...
  @Test public void doCheck_withUnknownConnectInstance() {
    final CoverityViewResultsDescriptor descriptor = new CoverityViewResultsDescriptor();
  ...
  @Test public void doCheck_withInvalidConnectInstance() {
    ...
    assertEquals(Kind.ERROR, result.kind);
    assertThat(StringEscapeUtils.unescapeHtml(result.toString()), CoreMatchers.containsString("\"" + cimInstance.getCoverityUser() + "\" does not have following permission(s): \"Access web services\""));
  }

  @Test public void doCheck_withInvalidProjectId() {
    setupConfigurationService(cimInstance);
    TestableViewsService.setupWithViewApi(StringUtils.EMPTY);
    final CoverityViewResultsDescriptor descriptor = new CoverityViewResultsDescriptor();
    FormValidation result = descriptor.doCheckViews(cimInstance.getName(), "projectId", "viewName");
    assertEquals(Kind.ERROR, result.kind);
    assertThat(StringEscapeUtils.unescapeHtml(result.toString()), CoreMatchers.containsString("Unable to find project \'projectId\' on instance \'" + cimInstance.getName() + "\'."));
  }

  @Test public void doCheck_withInvalidProjectId_suggestsKnownProjects() {
    final TestConfigurationService testConfigurationService = setupConfigurationService(cimInstance);
    ...
    assertThat(errorMessage, CoreMatchers.containsString("Unable to find project \'projectId\' on instance \'" + cimInstance.getName() + "\'."));
    assertThat(errorMessage, CoreMatchers.containsString("Available projects include: pipeline-project0, pipeline-project1"));
  }

  @Test public void doCheck_withInvalidConnectView() {
    final TestConfigurationService testConfigurationService = setupConfigurationService(cimInstance);
    ...
    assertEquals(Kind.ERROR, result.kind);
    assertThat(StringEscapeUtils.unescapeHtml(result.toString()), CoreMatchers.containsString("Unable to find the view \'viewName\' on instance \'" + cimInstance.getName() + "\'."));
  }

  @Test public void doCheck_withInvalidConnectView_suggestsKnownViews() {
    final TestConfigurationService testConfigurationService = setupConfigurationService(cimInstance);
    testConfigurationService.setupProjects("pipeline-project", 1, "s", 1);
    ...
    assertEquals(Kind.ERROR, result.kind);
    final String errorMessage = StringEscapeUtils.unescapeHtml(result.toString());
    assertThat(errorMessage, CoreMatchers.containsString("Unable to find the view \'viewName\' on instance \'" + cimInstance.getName() + "\'."));
    assertThat(errorMessage, CoreMatchers.containsString("Available views include: Custom View, All Defects"));
  }

  @Test public void doCheck_withValidConfiguration() {
    final TestConfigurationService testConfigurationService = setupConfigurationService(cimInstance);
    testConfigurationService.setupProjects("pipeline-project", 1, "s", 1);
    final HashMap<Long, String> views = new HashMap<>();
    views.put(54321L, "Custom View");
  ...
  @Test public void doCheck_withValidConfiguration_usingConnectViewId() {
    final TestConfigurationService testConfigurationService = setupConfigurationService(cimInstance);
    testConfigurationService.setupProjects("pipeline-project", 1, "s", 1);
    final HashMap<Long, String> views = new HashMap<>();
    views.put(54321L, "Custom View");
