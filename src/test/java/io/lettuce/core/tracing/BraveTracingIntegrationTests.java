 ...
 */
@EnabledOnCommand(value = "HELLO") class BraveTracingIntegrationTests extends TestSupport {
  ...
  @Test void reactivePingWithTrace() {
    ScopedSpan trace = clientTracing.tracer().startScopedSpan("foo");
    StatefulRedisConnection<String, String> connect = client.connect();
    connect.reactive().ping().subscriberContext((it) -> it.put(TraceContext.class, trace.context())).as(StepVerifier::create).expectNext("PONG").verifyComplete();
    Wait.untilNotEquals(true, spans::isEmpty).waitOrTimeout();
    trace.finish();
    List<Span> spans = new ArrayList<>(BraveTracingIntegrationTests.spans);
    assertThat(spans.get(1).name()).isEqualTo("ping");
  ...
  @Test void reactiveGetAndSetWithTrace() {
    ScopedSpan trace = clientTracing.tracer().startScopedSpan("foo");
    StatefulRedisConnection<String, String> connect = client.connect();
    connect.reactive().set("foo", "bar").then(connect.reactive().get("foo")).subscriberContext((it) -> it.put(TraceContext.class, trace.context())).as(StepVerifier::create).expectNext("bar").verifyComplete();
    Wait.untilTrue(() -> spans.size() > 2).waitOrTimeout();
    trace.finish();
    List<Span> spans = new ArrayList<>(BraveTracingIntegrationTests.spans);
    assertThat(spans.get(1).name()).isEqualTo("set");
  ...
  @Test void reactiveGetAndSetWithTraceProvider() {
    ...
    StatefulRedisConnection<String, String> connect = client.connect();
    connect.reactive().set("foo", "bar").then(connect.reactive().get("foo")).subscriberContext(io.lettuce.core.tracing.Tracing.withTraceContextProvider(() -> BraveTracing.BraveTraceContext.create(trace.context()))).as(StepVerifier::create).expectNext("bar").verifyComplete();
    Wait.untilTrue(() -> spans.size() > 2).waitOrTimeout();
    trace.finish();
    List<Span> spans = new ArrayList<>(BraveTracingIntegrationTests.spans);
    assertThat(spans.get(1).name()).isEqualTo("set");
