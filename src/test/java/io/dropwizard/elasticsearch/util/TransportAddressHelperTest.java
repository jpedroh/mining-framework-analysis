...
public class TransportAddressHelperTest {
  ...
  @Test(expected = NullPointerException.class) public void fromHostAndPortWithNullShouldFail() {
    TransportAddressHelper.fromHostAndPort(null);
  }

  @Test public void fromHostAndPortsWithNullShouldReturnEmptyArray() {
    TransportAddress[] result = TransportAddressHelper.fromHostAndPorts(null);
    assertEquals(0, result.length);
  }

  @Test public void fromHostAndPortsWithEmptyListShouldReturnEmptyArray() {
    TransportAddress[] result = TransportAddressHelper.fromHostAndPorts(Collections.<HostAndPort>emptyList());
    assertEquals(0, result.length);
  }

  @Test public void fromHostAndPortWithoutPortShouldUseDefaultPort() {
  ...
  @Test public void fromHostAndPortWithCorrectDataShouldSucceed() {
  ...
  @Test public void fromHostAndPostWithCorrectDataShouldSucceed() {
    final List<HostAndPort> hostAndPorts = ImmutableList.of(HostAndPort.fromParts("example.net", 1234), HostAndPort.fromParts("example.com", 5678), HostAndPort.fromString("example.org"));
