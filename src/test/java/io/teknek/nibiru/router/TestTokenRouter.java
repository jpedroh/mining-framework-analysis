...
public class TestTokenRouter {
  ...
  private ClusterMembership threeLiveNodes() {
    ClusterMembership mock = new ClusterMembership(null, null) {
      ...
      public List<ClusterMember> getLiveMembers() {
        return Arrays.asList(new ClusterMember("127.0.0.1", 2000, 1, "id1"), new ClusterMember("127.0.0.2", 2000, 1, "id2"), new ClusterMember("127.0.0.3", 2000, 1, "id3"));
      }
  ...
  @Test public void testWithReplicationFactor() {
    TokenRouter token = new TokenRouter();
    Keyspace keyspace = new Keyspace(new Configuration());
    KeyspaceMetaData meta = new KeyspaceMetaData();
    Map<String, Object> props = new HashMap<>();
    ...
    keyspace.setKeyspaceMetadata(meta);
    Partitioner p = new NaturalPartitioner();
    Assert.assertEquals(Arrays.asList(new Destination("id1"), new Destination("id2")), token.routesTo(null, keyspace, threeLiveNodes(), p.partition("a")));
    Assert.assertEquals(Arrays.asList(new Destination("id1"), new Destination("id2")), token.routesTo(null, keyspace, threeLiveNodes(), p.partition("aa")));
    Assert.assertEquals(Arrays.asList(new Destination("id2"), new Destination("id3")), token.routesTo(null, keyspace, threeLiveNodes(), p.partition("h")));
    Assert.assertEquals(Arrays.asList(new Destination("id3"), new Destination("id1")), token.routesTo(null, keyspace, threeLiveNodes(), p.partition("i")));
    Assert.assertEquals(Arrays.asList(new Destination("id1"), new Destination("id2")), token.routesTo(null, keyspace, threeLiveNodes(), p.partition("z")));
  }
}