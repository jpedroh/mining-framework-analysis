...
public class TransformableTest {
  @Test public void testAny() throws Exception {
    assertTrue(Xform.of(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9)).any((i) -> i > 3));
    assertTrue(Xform.of(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9)).any((i) -> i > 8));
    assertTrue(Xform.of(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9)).any((i) -> i < 2));
    assertTrue(Xform.of(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9)).any((i) -> i == 5));
    assertFalse(Xform.of(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9)).any((i) -> i == 0));
    assertFalse(Xform.of(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9)).any(Fn1.reject()));
    assertFalse(Xform.of(Collections.emptyList()).any(Fn1.accept()));
    assertTrue(Xform.of(Collections.singletonList(1)).any(Fn1.accept()));
  }
  ...
  @Test public void testToMutableSortedSet() {
    ...
    assertTrue(UnmodSortedIterable.equal(UnmodSortedIterable.castFromSortedSet(control), UnmodSortedIterable.castFromSortedSet(trans.toMutSortedSet((a, b) -> a - b))));
  }
  ...
  @Test public void testToImSortedSet() {
    ...
    assertTrue(UnmodSortedIterable.equal(UnmodSortedIterable.castFromSortedSet(control), trans.toImSortedSet(comp)));
  }
