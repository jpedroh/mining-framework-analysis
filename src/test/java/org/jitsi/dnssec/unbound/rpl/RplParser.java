      ...
      switch (state) {
        ...
        if (line.matches("\\s*trust-anchor:.*")) {
          ...
          if (line.matches("\\s*val-override-date:.*")) {
            ...
            if (line.matches("\\s*val-nsec3-keysize-iterations:.*")) {
              ...
              if (line.matches("\\s*val-digest-preference:.*")) {
                rpl.digestPreference = line.substring(line.indexOf("\"") + 1, line.length() - 1);
              } else {
                if (line.matches("\\s*harden-algo-downgrade:.*")) {
        ...
        if (line.startsWith("MATCH") || line.startsWith("ADJUST")) {
          ...
          if (line.startsWith("REPLY")) {
            ...
            if (line.startsWith("SECTION QUESTION")) {
              section = Section.QUESTION;
            } else {
              if (line.startsWith("SECTION ANSWER")) {
                section = Section.ANSWER;
              } else {
                if (line.startsWith("SECTION AUTHORITY")) {
                  section = Section.AUTHORITY;
                } else {
                  if (line.startsWith("SECTION ADDITIONAL")) {
                    ...
                    if (line.startsWith("ENTRY_END")) {
                      if (state == ParseState.ENTRY_BEGIN) {
                        ...
                        if (state == ParseState.STEP_CHECK_ANSWER) {
                          check.response = m;
    ...
    try {
      ...
      if (r.getType() == Type.RRSIG) {
        ...
        if (rr.getAlgorithm() == Algorithm.DSA && rr.getSignature().length > 41) {
          ...
          RRSIGRecord fixed = new RRSIGRecord(rr.getName(), rr.getDClass(), rr.getTTL(), rr.getTypeCovered(), rr.getAlgorithm(), rr.getOrigTTL(), rr.getExpire(), rr.getTimeSigned(), rr.getFootprint(), rr.getSigner(), signature);
