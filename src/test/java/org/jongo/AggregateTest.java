...
public class AggregateTest extends JongoTestBase {
  ...
  @Before public void setUp() throws Exception {
    ...
    nestedCollection.save(new TypeWithNested().addNested(new NestedDocument().withName("name1").withValue("value1")).addNested(new NestedDocument().withName("name2").withValue("value2")));
    nestedCollection.save(new TypeWithNested().addNested(new NestedDocument().withName("name3").withValue("value3")).addNested(new NestedDocument().withName("name4").withValue("value4")));
  }
  ...
  @Test public void canAggregateWithDefaultOptions() {
  ...
  @Test public void canAggregateWithOptions() {
  ...
  @Test public void canAggregateWithMultipleDocuments() {
  ...
  @Test public void canAggregateParameters() {
  ...
  @Test public void canAggregateWithManyMatch() {
  ...
  @Test public void canAggregateWithManyOperators() {
    articles = collection.aggregate("{$match:{tags:\'virus\'}}").and("{$limit:1}").as(Article.class);
    articles.next();
    assertThat(articles.hasNext()).isFalse();
  }

  @Test public void shouldCheckIfCommandHasErrors() {
    try {
  ...
  @Test public void shouldPopulateIds() throws IOException {
    ResultsIterator<Friend> resultsIterator = friendCollection.aggregate("{$project: {_id: \'$_id\', name: \'$name\'}}").as(Friend.class);
    List<Friend> friends = Lists.newArrayList((Iterable<Friend>) resultsIterator);
  ...
  @Test public void shouldUnmarshalNestedDocuments() throws IOException {
    ResultsIterator<NestedDocument> resultsIterator = nestedCollection.aggregate("{$unwind: \'$nested\'}").and("{$project: {name: \'$nested.name\', value: \'$nested.value\'}}").as(NestedDocument.class);
