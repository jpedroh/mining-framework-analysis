...
public class NestedPolymorphismTest extends JongoTestBase {
  ...
  @Test public void testCanSaveAndReadNestedPolymorphicField() throws Exception {
  ...
  @Test public void testCanUpdatePolymorphicField() throws Exception {
    Owner owner = new Owner();
    owner.setPet(new Cat("Sylvester", 42));
    collection.save(owner);
  ...
  @JsonTypeInfo(use = JsonTypeInfo.Id.NAME, property = "type") @JsonSubTypes(value = { @JsonSubTypes.Type(name = "cat", value = Cat.class) }) public static class Pet {
    private String name;
