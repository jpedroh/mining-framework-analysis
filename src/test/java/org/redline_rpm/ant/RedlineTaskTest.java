...
public class RedlineTaskTest extends TestBase {
  ...
  @Test public void testBadVersion() throws Exception {
    RedlineTask task = new RedlineTask();
  ...
  @Test public void testBadRelease() throws Exception {
    RedlineTask task = new RedlineTask();
    task.setName("nameRequired");
  ...
  @Test public void testBadEpoch() throws Exception {
    RedlineTask task = new RedlineTask();
    task.setName("nameRequired");
    task.setVersion("versionRequired");
    task.setGroup("groupRequired");
  ...
  @Test public void testRestrict() throws Exception {
  ...
  @Test public void testCapabilities() throws Exception {
    ...
    for (String[] def : new String[][] { { "depone", "", "1.0" }, { "deptwo", "less", "2.0" }, { "depthree", "", "" } }) {
    ...
    for (String[] def : new String[][] { { "provone", "1.1" }, { "provtwo", "2.1" }, { "provthree", "" } }) {
    ...
    for (String[] def : new String[][] { { "conone", "", "1.2" }, { "contwo", "less", "2.2" }, { "conthree", "", "" } }) {
    ...
    for (String[] def : new String[][] { { "obsone", "", "1.3" }, { "obstwo", "less", "2.3" }, { "obsthree", "", "" } }) {
  ...
  @Test public void testScripts() throws Exception {
    File dir = ensureTargetDir();
    File filename = new File(dir, "rpmtest-1.0-1.noarch.rpm");
    RedlineTask task = createBasicTask(dir);
  ...
  @Test public void testFiles() throws Exception {
    ...
    task.execute();
    Format format = getFormat(filename);
    assertArrayEquals(new String[] { "jabberwocky" }, (String[]) format.getHeader().getEntry(Header.HeaderTag.FILEUSERNAME).getValues());
    assertArrayEquals(new String[] { "vorpal" }, (String[]) format.getHeader().getEntry(Header.HeaderTag.FILEGROUPNAME).getValues());
  ...
  private Format getFormat(File filename) throws IOException {
    ...
    return scanner.run(new ReadableChannelWrapper(Channels.newChannel(new FileInputStream(filename))));
  }
  ...
  @Test public void testPackageNameLength() throws RedlineException {
    File dir = ensureTargetDir();
    RedlineTask task = new RedlineTask();
    task.setProject(createProject());
    task.setDestination(dir);
