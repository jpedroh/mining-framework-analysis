...
public class PipelineTest {
  ...
  @Test public void testEscapedExpandAll() throws Exception {
    WorkflowJob job = j.jenkins.createProject(WorkflowJob.class, "foo");
  ...
  @Test public void testUnconvertedMacro() throws Exception {
    WorkflowJob job = j.jenkins.createProject(WorkflowJob.class, "foo");
    job.setDefinition(new CpsFlowDefinition("node(\'agents\') {\n\techo \'Hello, world\'\n}", true));
    Run<?, ?> run = j.assertBuildStatusSuccess(job.scheduleBuild2(0));
    listener = StreamTaskListener.fromStdout();
  ...
  @TestExtension public static class TestMacro extends TokenMacro {
    ...
    @Override public String evaluate(Run<?, ?> run, FilePath workspace, TaskListener listener, String macroName, Map<String, String> arguments, ListMultimap<String, String> argumentMultimap) throws MacroEvaluationException, IOException, InterruptedException {
