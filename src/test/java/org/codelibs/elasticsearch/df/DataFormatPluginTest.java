...
@RunWith(value = BlockJUnit4ClassRunner.class) public class DataFormatPluginTest {
  ...
  @Test public void dumpCsvWithQuery() throws IOException {
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").body(queryWithFrom).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber - 10 + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("source", queryWithFrom).param("source_content_type", "application/json").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber - 10 + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "csv").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 1, lines.length);
  ...
  @Test public void dumpExcel() throws IOException {
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "xls").execute()) {
      try (InputStream is = curlResponse.getContentAsStream()) {
        final POIFSFileSystem fs = new POIFSFileSystem(is);
        final HSSFWorkbook book = new HSSFWorkbook(fs);
        final HSSFSheet sheet = book.getSheetAt(0);
  ...
  @Test public void dumpJson() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").param("bulk.index", "dataset02").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber * 2, lines.length);
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("q", "*:*").param("format", "json").param("from", "5").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").body(queryWithFrom).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) * 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").param("source", queryWithFrom).param("source_content_type", "application/json").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) * 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "json").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber * 2, lines.length);
      assertTrue(lines[0].startsWith("{\"index\""));
  ...
  @Test public void dumpJsonList() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "jsonlist").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 2, lines.length);
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("q", "*:*").param("format", "jsonlist").param("from", "5").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    final String queryWithFrom = "{\"query\":{\"match_all\":{}},\"from\":5,\"size\":" + String.valueOf(docNumber) + ",\"sort\":[\"bbb\"]}";
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "jsonlist").body(queryWithFrom).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) + 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "jsonlist").param("source", queryWithFrom).param("source_content_type", "application/json").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) + 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "jsonlist").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 2, lines.length);
      assertTrue(lines[0].equals("["));
  ...
  @Test public void dumpGeoJson() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").param("format", "geojson").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 2, lines.length);
  ...
  @Test public void dumpSizeLimit() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("limit", "50%").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("limit", "0").execute()) {
