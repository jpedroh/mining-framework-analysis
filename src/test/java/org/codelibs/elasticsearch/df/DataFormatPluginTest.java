...
@RunWith(value = BlockJUnit4ClassRunner.class) public class DataFormatPluginTest {
  ...
  @BeforeClass public static void setUp() throws IOException {
    ...
    }).build(newConfigs().clusterName(clusterName).numOfNode(1).pluginTypes("org.codelibs.elasticsearch.df.DataFormatPlugin"));
    runner.ensureYellow();
    indexing();
    node = runner.node();
  }
  ...
  @Before public void prepareParams() {
  ...
  @Test public void dumpCsvSimple() throws IOException {
  ...
  @Test public void dumpCsvWithDenotedFields() throws IOException {
    dumpCsvWithDenotedFields("fields_name");
  }

  @Test public void dumpCsvWithDenotedFieldsCompatible() throws IOException {
    dumpCsvWithDenotedFields("fl");
  }

  @Test public void dumpCsvWithoutHeader() throws IOException {
  ...
  @Test public void dumpCsvWithQuery() throws IOException {
    ...
    try (CurlResponse response = createRequest(node, path, paramsCsv).param("size", Integer.toString(docNumber)).execute()) {
      assertEquals(docNumber - 4, response.getContentAsString().split("\n").length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("q", "*:*").param("format", "csv").param("from", "5").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("size", Integer.toString(docNumber)).body(queryWithFrom).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber - 10 + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("source", queryWithFrom).param("source_content_type", "application/json").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber - 10 + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "csv").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 1, lines.length);
  ...
  @Test public void dumpCsvInFile() throws IOException {
  ...
  @Test public void dumpExcelSimple() throws IOException {
  ...
  @Test public void dumpExcelWithDenotedFields() throws IOException {
    dumpExcelWithDenotedFields("fields_name");
  }

  @Test public void dumpExcelWithDenotedFieldsCompatible() throws IOException {
    dumpExcelWithDenotedFields("fl");
  }

  @Test public void dumpExcelWithoutHeader() throws IOException {
    ...
    try (CurlResponse response = createRequest(node, path, paramsXls).execute()) {
      try (InputStream is = response.getContentAsStream()) {
        ...
        final HSSFWorkbook book = new HSSFWorkbook(fs);
        final HSSFSheet sheet = book.getSheetAt(0);
        assertEquals(docNumber, sheet.getLastRowNum() + 1);
      }
    }
  }

  @Test public void dumpExcel() throws IOException {
    final String query = "{\"query\":{\"bool\":{\"must\":[{\"range\":{\"bbb\":{\"from\":\"1\",\"to\":\"10\"}}}],\"must_not\":[],\"should\":[]}},\"sort\":[\"bbb\"]}";
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "xls").param("search_type", "query_then_fetch").body(query).execute()) {
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "xls").param("size", Integer.toString(docNumber)).execute()) {
      try (InputStream is = curlResponse.getContentAsStream()) {
        final POIFSFileSystem fs = new POIFSFileSystem(is);
        final HSSFWorkbook book = new HSSFWorkbook(fs);
        final HSSFSheet sheet = book.getSheetAt(0);
  ...
  @Test public void dumpXlsInFile() throws IOException {
  ...
  @Test public void dumpJson() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").param("size", Integer.toString(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").param("bulk.index", "dataset02").param("size", Integer.toString(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber * 2, lines.length);
    ...
    final String query = "{\"query\":{\"bool\":{\"must\":[{\"range\":{\"bbb\":{\"from\":\"1\",\"to\":\"10\"}}}],\"must_not\":[],\"should\":[]}},\"sort\":[\"bbb\"]}";
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").param("search_type", "query_then_fetch").body(query).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("q", "*:*").param("format", "json").param("from", "5").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").param("size", String.valueOf(docNumber)).body(queryWithFrom).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) * 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "json").param("source", queryWithFrom).param("source_content_type", "application/json").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) * 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "json").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber * 2, lines.length);
      assertTrue(lines[0].startsWith("{\"index\""));
  ...
  @Test public void dumpJsonInFile() throws IOException {
    ...
    try (CurlResponse curlResponse = createRequest(node, path, paramsJson).execute()) {
      ...
      assertTrue(lines.get(0).startsWith("{\"index\":{\"_index\":\"dataset0\","));
      assertTrue(lines.get(1).startsWith("{\"aaa\""));
    }
  }

  @Test public void dumpJsonList() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "jsonlist").param("size", Integer.toString(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 2, lines.length);
    ...
    final String query = "{\"query\":{\"bool\":{\"must\":[{\"range\":{\"bbb\":{\"from\":\"1\",\"to\":\"10\"}}}],\"must_not\":[],\"should\":[]}},\"sort\":[\"bbb\"]}";
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "jsonlist").param("search_type", "query_then_fetch").body(query).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("q", "*:*").param("format", "jsonlist").param("from", "5").param("size", Integer.toString(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber - 5 + 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("q", "*:*").param("format", "jsonlist").param("from", "5").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) + 2, lines.length);
    }
    final String queryWithFrom = "{\"query\":{\"match_all\":{}},\"from\":5,\"size\":" + String.valueOf(docNumber) + ",\"sort\":[\"bbb\"]}";
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "jsonlist").param("size", String.valueOf(docNumber)).body(queryWithFrom).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) + 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "jsonlist").param("source", queryWithFrom).param("source_content_type", "application/json").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals((docNumber - 5) + 2, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("search_type", "query_then_fetch").param("format", "jsonlist").param("size", String.valueOf(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 2, lines.length);
      assertTrue(lines[0].equals("["));
  ...
  @Test public void dumpJsonListInFile() throws IOException {
  ...
  @Test public void dumpGeoJson() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").
...
>>>>>>> /usr/src/app/output/codelibs/elasticsearch-dataformat/7f6410361b640207341ac3b31112ad48aa23d64e/src/test/java/org/codelibs/elasticsearch/df/DataFormatPluginTest.java/right.java
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").param("format", "geojson").param("geometry.lon_field", "x_lon").param("geometry.lat_field", "x_lat").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").param("format", "geojson").param("geometry.lon_field", "x_lon").param("geometry.lat_field", "x_lat").param("geometry.alt_field", "x_alt").param("keep_geometry_info", "true").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertTrue(lines[1].contains("\"x_lon\":"));
      assertTrue(lines[1].contains("\"x_lat\":"));
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").param("format", "geojson").param("geometry.lon_field", "x_lon").param("geometry.lat_field", "x_lat").param("geometry.alt_field", "iii.x_altSub").param("exclude_fields", "iii,x_alt,x_coord,x_type,x_typeArray").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertTrue(!lines[1].contains("\"x_lon\":"));
      assertTrue(!lines[1].contains("\"x_lat\":"));
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").param("format", "geojson").param("geometry.type_field", "x_type").param("geometry.coord_field", "x_coord").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
    ...
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").param("format", "geojson").param("geometry.type_field", "x_type").param("geometry.coord_field", "x_coords").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertTrue(lines[1].contains("\"coordinates\":[]"));
      assertTrue(lines[1].contains("\"x_coord\":["));
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset1/_data").header("Content-Type", "application/json").param("format", "geojson").param("geometry.type_field", "x_typeArray[1]").param("geometry.coord_field", "x_coord").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
  ...
  @Test public void dumpGeoJsonInFile() throws IOException {
  ...
  @Test public void dumpSizeLimit() throws IOException {
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("size", Integer.toString(docNumber)).execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("size", Integer.toString(docNumber)).param("limit", "50%").execute()) {
      final String content = curlResponse.getContentAsString();
      final String[] lines = content.split("\n");
      assertEquals(docNumber + 1, lines.length);
    }
    try (CurlResponse curlResponse = EcrCurl.get(node, "/dataset0/_data").header("Content-Type", "application/json").param("format", "csv").param("size", Integer.toString(docNumber)).param("limit", "0").execute()) {
  ...
  private static void indexing() {
    ...
    for (int i = 1; i <= docNumber; i++) {
      final IndexResponse indexResponse0 = runner.insert(index0, type0, String.valueOf(i), "{" + "\"aaa\":\"test " + i + "\"," + "\"bbb\":" + i + "," + "\"ccc\":\"2012-01-01:00:00.000Z\"," + "\"eee\":{\"fff\":\"TEST " + i + "\", \"ggg\":" + i + ", \"hhh\":\"2013-01-01:00:00.000Z\"}" + "}");
    ...
    for (int i = 1; i <= docNumber; i++) {
      ...
      final IndexResponse indexResponse1 = runner.insert(index1, type1, String.valueOf(i), "{" + "\"aaa\":\"test " + i + "\"," + "\"bbb\":" + i + "," + "\"ccc\":\"2012-01-01:00:00.000Z\"," + "\"eee\":{\"fff\":\"TEST " + i + "\", \"ggg\":" + i + ", \"hhh\":\"2013-01-01:00:00.000Z\"}," + "\"x_type\":\"" + geotype + "\"," + "\"x_typeArray\": [\"badtype\",\"" + geotype + "\",\"badtype\"]," + "\"x_coord\": " + geocoord + "," + "\"x_lon\": 1" + i + ".0," + "\"x_lat\": " + (i / 2) + ".0," + "\"x_alt\": " + (i / 2) + ".0," + "\"iii\":{\"x_altSub\": " + (i * 3) + "}" + "}");
  ...
  private void assertAcknowledged(CurlResponse response, File file) {
    ...
    assertEquals(file.getName(), new File(contentAsMap.get("file").toString()).getName());
  }
