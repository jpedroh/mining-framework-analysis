...
public class JDBITest {
  ...
  @Test public void testExample() throws Exception {
    Set<Something> things = jdbi.withHandle((h) -> {
      ...
      return h.query("select id, name from something").map((rs) -> new Something(rs.getInt(1), rs.getString(2))).collect(
...
>>>>>>> /usr/src/app/output/brianm/jdbi3/9f40c034b7d4b1a262f723ec81a630d1ebeb31bd/src/test/java/org/jdbi/jdbi3/JDBITest.java/right.java
  ...
  @Test public void testExample2() throws Exception {
    Set<Integer> things = jdbi.withHandle((h) -> {
      h.execute("insert into something (id, name) values (?, ?)", 1, "Brian");
      h.execute("insert into something (id, name) values (?, ?)", 2, "Steven");
      return h.query("select id, name from something").map(JDBI.extract((Integer id) -> id)).collect(Collectors.toSet());
    });
  ...
  @Test public void testExample3() throws Exception {
    Set<Something> things = jdbi.withHandle((h) -> {
      h.execute("insert into something (id, name) values (?, ?)", 1, "Brian");
      h.execute("insert into something (id, name) values (?, ?)", 2, "Steven");
      return h.query("select id, name from something").map(JDBI.extract((Integer id, String name) -> new Something(id, name))).collect(Collectors.toSet());
    });
    assertThat(things).isEqualTo(ImmutableSet.of(new Something(1, "Brian"), new Something(2, "Steven")));
  }

  @Test public void testExample4() throws Exception {
    Set<Something> things = jdbi.withHandle((h) -> {
      h.execute("insert into something (id, name) values (?, ?)", 1, "Brian");
      h.execute("insert into something (id, name) values (?, ?)", 2, "Steven");
      return h.query("select id, name from something").map(JDBI.extract(Something::new)).collect(Collectors.toSet());
    });
    assertThat(things).isEqualTo(ImmutableSet.of(new Something(1, "Brian"), new Something(2, "Steven")));
  }
  ...
  @Test public void testExample5() throws Exception {
    Set<Integer> things = jdbi.withHandle((h) -> {
      h.execute("insert into something (id, name) values (?, ?)", 1, "Brian");
      h.execute("insert into something (id, name) values (?, ?)", 2, "Steven");
      return h.query("select id from something").map(JDBI.extract1(JDBITest::succ)).map((it) -> {
