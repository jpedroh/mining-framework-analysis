...
public class JMXRemotingServer {
  ...
  public JMXRemotingServer(JMXRemotingConfig config) {
    ...
    config.port = listenerPort;
    host = config.host;
    mbeanServer = config.mbeanServer != null ? config.mbeanServer : ManagementFactory.getPlatformMBeanServer();
    saslMechanisms = Collections.unmodifiableSet(config.saslMechanisms != null ? config.saslMechanisms : Collections.emptySet());
  ...
  public void start() throws IOException {
    ...
    SaslAuthenticationFactory authFactory = SaslAuthenticationFactory.builder().setSecurityDomain(securityDomain).setMechanismConfigurationSelector((mechanismInformation) -> MechanismConfiguration.builder().addMechanismRealm(MechanismRealmConfiguration.builder().setRealmName(REALM).build()).build()).setFactory(saslServerFactory).build();
    ...
    if (mbeanServerLocator == null) {
      ...
      delegatingServer = new DelegatingRemotingConnectorServer(mbeanServerLocator, endpoint, configMap, serverMessageInterceptorFactory);
