...
public class ChannelManagerImplTest {
  ...
  @Before public void setUp() throws Exception {
    ...
    Configuration.getInstance().putProperty(Configuration.CONFIGURATION_SERVER_DOMAIN, TEST_DOMAIN);
    Configuration.getInstance().putProperty(Configuration.CONFIGURATION_SERVER_TOPICS_DOMAIN, TEST_TOPICS_DOMAIN);
  ...
  @Test @Ignore(value = "Until we can so something about the millisecond matching") public void testCreatePersonalChannel() throws Exception {
  ...
  @Test(expected = IllegalArgumentException.class) public void testCreatePersonalChannelFailsForRemoteUser() throws Exception {
    JID channelJID = new JID("testchannel@otherdomain.com");
    channelManager.createPersonalChannel(channelJID);
  }

  @Test public void testCreatePersonalWorksForExternallyValidatedDomain() throws Exception {
    Configuration.getInstance().putProperty(Configuration.CONFIGURATION_LOCAL_DOMAIN_CHECKER, Boolean.TRUE.toString());
    JID channelJID = new JID("testchannel@otherdomain.com");
    channelManager.createPersonalChannel(channelJID);
  }

  @Test @Ignore(value = "Until we can so something about the millisecond matching") public void testCreatePersonalChannelSomeNodesExist() throws Exception {
    JID channelJID = new JID("testchannel@domain.com");
  ...
  @Test public void testIsLocalNodeSuccess() throws Exception {
    assertTrue(Configuration.getInstance().isLocalNode("/user/test@domain.com/posts"));
  }

  @Test public void testIsLocalNodeFailure() throws Exception {
    assertFalse(Configuration.getInstance().isLocalNode("/user/test@otherdomain.com/posts"));
  }

  @Test(expected = IllegalArgumentException.class) public void testIsLocalNodeWithInvalidNodeThrowsException() throws Exception {
    Configuration.getInstance().isLocalNode("somerandomnodeid");
  }

  @Test public void testIsLocalJidForLocaJid() throws Exception {
    assertTrue(Configuration.getInstance().isLocalJID(new JID("user@" + TEST_DOMAIN)));
  }

  @Test public void testIsLocalJidForNonLocaJid() throws Exception {
    assertFalse(Configuration.getInstance().isLocalJID(new JID("user@server1.com")));
  }

  @Test public void testDeleteRemoteDataDeletesRemoteData() throws Exception {
  ...
  @Test public void testGetNodeDefaultAffiliationForNodeWithConf() throws Exception {
  ...
  @Test public void testGetNodeDefaultAffiliationForNodeWithoutConf() throws Exception {
    Affiliations affiliation = channelManager.getDefaultNodeAffiliation(user1);
