...
public class RepliesGetTest extends IQTestHandler {
  ...
  @Before public void setUp() throws Exception {
    ...
    Mockito.when(channelManager.getNodeItem(Mockito.anyString(), Mockito.anyString())).thenReturn(new NodeItemImpl(TEST_NODE, "1", new Date(), "payload"));
    nodeViewAcl = Mockito.mock(NodeViewAcl.class);
    repliesGet.setNodeViewAcl(nodeViewAcl);
    Mockito.doReturn(true).when(nodeViewAcl).canViewNode(Mockito.anyString(), Mockito.any(NodeMembershipImpl.class), Mockito.any(AccessModels.class), Mockito.anyBoolean());
  ...
  @Test public void testPassingRecentItemsAsElementNameReturnsTrue() {
    Assert.assertTrue(repliesGet.accept(element));
  }

  @Test public void testPassingNotRecentItemsAsElementNameReturnsFalse() {
    Element element = new BaseElement("not-replies");
    Assert.assertFalse(repliesGet.accept(element));
  }

  @Test public void testMissingNodeAttributeReturnsErrorStanza() throws Exception {
  ...
  @Test public void testMissingItemIdAttributeReturnsErrorStanza() throws Exception {
  ...
  @Test public void testNodeStoreExceptionGeneratesAnErrorStanza() throws Exception {
  ...
  @Test public void userWhoCantAccessChannelGetsPermissionErrorStanzaReply() throws Exception {
    Mockito.doReturn(false).when(nodeViewAcl).canViewNode(Mockito.anyString(), Mockito.any(NodeMembershipImpl.class), Mockito.any(AccessModels.class), Mockito.anyBoolean());
  ...
  @Test public void testIfItemDoesNotExistErrorStanzaIsReturned() throws Exception {
  ...
  @Test public void testNoRepliesReturnsEmptyStanza() throws Exception {
    Mockito.when(channelManager.getNodeItemReplies(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(), Mockito.eq(true), Mockito.anyInt())).thenReturn(new ClosableIteratorImpl<NodeItem>(new ArrayList<NodeItem>().iterator()));
    repliesGet.process(element, jid, request, null);
    Packet response = queue.poll();
  ...
  @Test public void testOutgoingStanzaFormattedAsExpected() throws Exception {
    ...
    Mockito.when(channelManager.getNodeItemReplies(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(), Mockito.eq(true), Mockito.anyInt())).thenReturn(new ClosableIteratorImpl<NodeItem>(expectedResults.iterator()));
    repliesGet.process(element, jid, request, null);
    Packet response = queue.poll();
    Element items = response.getElement().element("pubsub").element("items");
    Assert.assertEquals("/user/channeluser@example.com/posts", items.attributeValue("node"));
  ...
  @Test public void testUnparsableItemEntriesAreSimplyIgnored() throws Exception {
    ArrayList<NodeItem> expectedResults = new ArrayList<NodeItem>();
    expectedResults.add(new NodeItemImpl(TEST_NODE, "1", new Date(), "<entry>value1</entry>"));
  ...
  @Test public void testRsmElementIsAddedCorrectly() throws Exception {
    ...
    Mockito.when(channelManager.getNodeItemReplies(Mockito.anyString(), Mockito.anyString(), Mockito.eq("1"), Mockito.eq(true), Mockito.eq(4))).thenReturn(new ClosableIteratorImpl<NodeItem>(expectedResults.iterator()));
    repliesGet.process(element, jid, request, null);
    Packet response = queue.poll();
    Element items = response.getElement().element("pubsub").element("items");
    Assert.assertEquals("/user/channeluser@example.com/posts", items.attributeValue("node"));
  ...
  @Test public void providingBeforeValueInRsmElementInvokesPastItemLookup() throws Exception {
    Element rsm = request.getChildElement().addElement("set");
    rsm.addNamespace("", RepliesGet.NS_RSM);
    rsm.addElement("max").setText("4");
    rsm.addElement("after").setText("1");
    ...
    expectedResults.add(new NodeItemImpl(TEST_NODE, "3", new Date(), "<entry>value3</entry>"));
    expectedResults.add(new NodeItemImpl(TEST_NODE, "4", new Date(), "<entry>value4</entry>"));
    Mockito.when(channelManager.getNodeItemReplies(Mockito.anyString(), Mockito.anyString(), Mockito.eq("1"), Mockito.eq(false), Mockito.eq(4))).thenReturn(new ClosableIteratorImpl<NodeItem>(expectedResults.iterator()));
    repliesGet.process(element, jid, request, null);
    Packet response = queue.poll();
    Element items = response.getElement().element("pubsub").element("items");
    Assert.assertEquals(4, items.elements("item").size());
  ...
  @Test public void rsmStillAddedWhenThereAreNoResults() throws Exception {
    Element rsm = request.getChildElement().addElement("set");
    rsm.addNamespace("", RepliesGet.NS_RSM);
    rsm.addElement("max").setText("4");
    rsm.addElement("after").setText("1");
    ArrayList<NodeItem> expectedResults = new ArrayList<NodeItem>();
    Mockito.when(channelManager.getNodeItemReplies(Mockito.anyString(), Mockito.anyString(), Mockito.eq("1"), Mockito.eq(4))).thenReturn(new ClosableIteratorImpl<NodeItem>(expectedResults.iterator()));
