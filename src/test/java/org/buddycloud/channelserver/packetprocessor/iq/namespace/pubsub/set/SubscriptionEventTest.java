...
public class SubscriptionEventTest extends IQTestHandler {
  ...
  @Before public void setUp() throws Exception {
    ...
    doReturn(new ResultSetImpl<NodeSubscription>(subscribers)).when(dataStore).getNodeSubscriptionListeners(anyString());
    event.setChannelManager(dataStore);
  }

  @Test public void testPassingSubscriptionsAsElementNameReturnsTrue() {
    Element element = new BaseElement("subscriptions");
    Assert.assertTrue(event.accept(element));
  }

  @Test public void testPassingNotSubscriptionsAsElementNameReturnsFalse() {
    Element element = new BaseElement("not-subscriptions");
    Assert.assertFalse(event.accept(element));
  }

  @Test public void testNotProvidingNodeAttributeReturnsErrorStanza() throws Exception {
  ...
  @Test public void testNotProvidingSubscriptionChildNodeReturnsErrorStanza() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/subscribe/authorizationPendingGrantReply.stanza").replaceFirst("<subscription jid=\'francisco@denmark.lit\' subscription=\'subscribed\'/>", ""));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void testNotProvidingJidAttributeReturnsErrorStanza() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/subscribe/authorizationPendingGrantReply.stanza").replaceFirst("jid=\'francisco@denmark.lit\'", ""));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void testNotProvidingSubscriptionAttributeReturnsErrorStanza() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/subscribe/authorizationPendingGrantReply.stanza").replaceFirst("subscription=\'subscribed\'", ""));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @SuppressWarnings(value = { "unchecked" }) @Test public void testNodeStoreExceptionResultsInInternalServerErrorStanza() throws Exception {
    when(dataStore.nodeExists(anyString())).thenThrow(NodeStoreException.class);
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void testNonExistantNodeRetunsErrorStanza() throws Exception {
  ...
  @Test public void userWithoutSubscriptionReturnsErrorStanza() throws Exception {
    when(dataStore.getNodeMembership(anyString(), any(JID.class))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.none, Affiliations.none, null));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void userWhoIsntOwnerOrModeratorCantUpdateSubscription() throws Exception {
    when(dataStore.getNodeMembership(anyString(), any(JID.class))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.member, null));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void subscribingUserMustHaveExistingSubscriptionToUpdate() throws Exception {
    NodeMembership membership = new NodeMembershipImpl(node, new JID(subscriber), Subscriptions.none, Affiliations.owner, null);
    when(dataStore.getNodeMembership(anyString(), any(JID.class))).thenReturn(membership);
    NodeMembership inviteeMemebership = new NodeMembershipImpl(node, new JID(subscriber), Subscriptions.none, Affiliations.owner, null);
    when(dataStore.getNodeMembership(anyString(), eq(jid))).thenReturn(inviteeMemebership);
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void passingInvalidSubscriptionTypeSetsSubscriptionToNone() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/subscribe/authorizationPendingGrantReply.stanza").replaceFirst("subscription=\'subscribed\'", "subscription=\'i-can-haz-all-the-items\'"));
    ArgumentCaptor<NodeSubscription> argument = ArgumentCaptor.forClass(NodeSubscription.class);
    when(dataStore.getNodeMembership(anyString(), any(JID.class))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.owner, null));
    ArrayList<NodeSubscription> subscribers = new ArrayList<NodeSubscription>();
    doReturn(new ResultSetImpl<NodeSubscription>(subscribers)).when(dataStore).getNodeSubscriptionListeners(anyString());
    event.process(element, jid, request, null);
  ...
  @Test public void passingValidSubscriptionSendsOutExpectedNotifications() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/subscribe/authorizationPendingGrantReply.stanza").replaceFirst("subscription=\'subscribed\'", "subscription=\'subscribed\'"));
    NodeMembership membership = new NodeMembershipImpl(node, new JID(subscriber), Subscriptions.subscribed, Affiliations.moderator, null);
    when(dataStore.getNodeMembership(eq(node), any(JID.class))).thenReturn(membership);
    event.process(element, jid, request, null);
    Assert.assertEquals(5, queue.size());
    Packet notification = queue.poll();
    Assert.assertEquals(request.getFrom().toString(), notification.getTo().toString());
    ...
    Assert.assertEquals(node, notification.getElement().element("event").element("subscription").attributeValue("node"));
    Assert.assertTrue(notification.toXML().contains(JabberPubsub.NS_PUBSUB_EVENT));
    Assert.assertEquals("subscribed", notification.getElement().element("event").element("subscription").attributeValue("subscription"));
    Assert.assertEquals(subscriber, notification.getElement().element("event").element("subscription").attributeValue("jid"));
  }

  @Test public void userCanInviteAnotherUserToNode() throws Exception {
  ...
  @Test public void userCanNotInviteAnotherUserIfTheyDontHaveValidSubscription() throws Exception {
  ...
  @Test public void invitedByIsSetAsActorJid() throws Exception {
    JID invitee = new JID("francisco@denmark.lit");
    NodeMembership membership = new NodeMembershipImpl(node, invitee, Subscriptions.none, Affiliations.none, null);
    when(dataStore.getNodeMembership(eq(node), eq(invitee))).thenReturn(membership);
    IQ request = readStanzaAsIq("/iq/pubsub/subscribe/invite.stanza");
  ...
  @Test public void standardSubscribeDoesNotSetInvitedBy() throws Exception {
    NodeMembership membership = new NodeMembershipImpl(node, new JID(subscriber), Subscriptions.subscribed, Affiliations.moderator, null);
    when(dataStore.getNodeMembership(eq(node), any(JID.class))).thenReturn(membership);
    event.process(element, jid, request, null);
    IQ result = (IQ) queue.poll();
  ...
  @Test public void sendsNotificationToInvitedUserIfTheyAreLocal() throws Exception {
    JID invitee = new JID("francisco@denmark.lit");
    NodeMembership membership = new NodeMembershipImpl(node, invitee, Subscriptions.none, Affiliations.none, null);
    when(dataStore.getNodeMembership(eq(node), eq(invitee))).thenReturn(membership);
    IQ request = readStanzaAsIq("/iq/pubsub/subscribe/invite.stanza");
  ...
  @Test public void sendsNotificationToInvitedUsersServerIfTheyAreNotLocal() throws Exception {
    JID invitee = new JID("francisco@denmark.lit");
  ...
  @Test public void userCanNotModifyOwnSubscription() throws Exception {
    ...
    Assert.assertEquals(SubscriptionEvent.CAN_NOT_MODIFY_OWN_SUBSCRIPTION, error.getApplicationConditionName());
