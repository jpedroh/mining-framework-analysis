...
public class AffiliationEventTest extends IQTestHandler {
  ...
  @Before public void setUp() throws Exception {
    ...
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(jid))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.owner, null));
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(new JID("francisco@denmark.lit")))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.none, Affiliations.publisher, null));
  ...
  @Test public void testPassingAffiliationsAsElementNameReturnsTrue() {
    Element element = new BaseElement("affiliations");
    Assert.assertTrue(event.accept(element));
  }

  @Test public void testPassingNotAffiliationsAsElementNameReturnsFalse() {
  ...
  @Test public void testNotProvidingAffiliationChildNodeReturnsErrorStanza() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/affiliation/affiliationChange.stanza").replaceFirst("<affiliation jid=\'francisco@denmark.lit\' affiliation=\'member\'/>", ""));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void testNotProvidingAffiliationAttributeReturnsErrorStanza() throws Exception {
  ...
  @Test public void testNodeStoreExceptionResultsInInternalServerErrorStanza() throws Exception {
  ...
  @Test public void userWithoutAffiliationReturnsErrorStanza() throws Exception {
    Mockito.when(channelManager.nodeExists(node)).thenReturn(true);
    Mockito.when(channelManager.getNodeMembership(node, jid)).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.none, null));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void userWhoIsntOwnerOrModeratorCantUpdateAffiliation() throws Exception {
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(jid))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.publisher, null));
    event.setChannelManager(channelManager);
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
  ...
  @Test public void userMustHaveExistingAffiliationToUpdate() throws Exception {
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(jid))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.owner, null));
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(new JID("francisco@denmark.lit")))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.none, Affiliations.none, null));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void notPossibleToChangeTheAffiliationOfNodeOwner() throws Exception {
    Mockito.when(channelManager.nodeExists(node)).thenReturn(true);
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(jid))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.owner, null));
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(new JID("francisco@denmark.lit")))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.none, Affiliations.owner, null));
    event.process(element, jid, request, null);
    Packet response = queue.poll();
    PacketError error = response.getError();
    Assert.assertNotNull(error);
  ...
  @Test public void passingInvalidAffiliationTypeSetsAffiliationToNone() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/affiliation/affiliationChange.stanza").replaceFirst("affiliation=\'member\'", "affiliation=\'i-can-haz-all-the-items\'"));
  ...
  @Test public void testPassingValidAffiliationTypeUpdatesAffiliation() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/affiliation/affiliationChange.stanza").replaceFirst("affiliation=\'member\'", "affiliation=\'moderator\'"));
    Mockito.when(channelManager.getNodeSubscriptionListeners(Mockito.anyString())).thenReturn(new ResultSetImpl<NodeSubscription>(new ArrayList<NodeSubscription>()));
    Mockito.when(channelManager.nodeExists(node)).thenReturn(true);
    event.setChannelManager(channelManager);
    event.process(element, jid, request, null);
  ...
  @Test public void testPassingValidAffiliationSendsOutExpectedNotifications() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/affiliation/affiliationChange.stanza").replaceFirst("affiliation=\'member\'", "affiliation=\'moderator\'"));
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (packet.getElement().getName().equals("message") && packet.getTo().equals(new JID("romeo@shakespeare.lit"))) {
        ...
        Assert.assertEquals(Affiliations.moderator.toString(), packet.getElement().element("event").element("affiliations").element("affiliation").attributeValue("affiliation"));
  ...
  @Test public void moderatorsCantCreateOtherModerators() throws Exception {
    IQ request = toIq(readStanzaAsString("/iq/pubsub/affiliation/affiliationChange.stanza").replaceFirst("affiliation=\'member\'", "affiliation=\'moderator\'"));
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(jid))).thenReturn(new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.moderator, null));
    event.process(element, jid, request, null);
    IQ response = (IQ) queue.poll();
    Assert.assertEquals(IQ.Type.error, response.getType());
    PacketError error = response.getError();
