...
public class UnsubscribeSetTest extends IQTestHandler {
  ...
  @Before public void setUp() throws Exception {
    ...
    subscription = new NodeSubscriptionImpl(node, jid, Subscriptions.subscribed);
    affiliation = new NodeAffiliationImpl(node, jid, Affiliations.publisher, new Date());
    ...
    Mockito.when(channelManager.getUserSubscription(Mockito.anyString(), Mockito.any(JID.class))).thenReturn(subscription);
  ...
  @Test public void missingNodeAttributeReturnsError() throws Exception {
  ...
  @Test public void emptyNodeAttributeReturnsError() throws Exception {
    IQ badRequest = request.createCopy();
    badRequest.getChildElement().element("unsubscribe").attribute("node").detach();
  ...
  @Test public void makesRemoteRequest() throws Exception {
  ...
  @Test public void canNotUnsubscribeAnotherUser() throws Exception {
    IQ badRequest = request.createCopy();
  ...
  @Test public void notExistingNodeRetunsError() throws Exception {
  ...
  @Test public void nonMatchingSubscriptionToSenderReturnsError() throws Exception {
    Mockito.when(channelManager.getUserSubscription(Mockito.anyString(), Mockito.any(JID.class))).thenReturn(new NodeSubscriptionImpl(node, new JID("juliet@capulet.lit"), Subscriptions.subscribed));
    unsubscribe.process(element, jid, request, null);
    Assert.assertEquals(1, queue.size());
    IQ response = (IQ) queue.poll();
    Assert.assertEquals(IQ.Type.error, response.getType());
  ...
  @Test public void canNotUnsubscribeAsOnlyNodeOwner() throws Exception {
    affiliation = new NodeAffiliationImpl(node, jid, Affiliations.owner, new Date());
  ...
  @Test public void unsubscribesTheUser() throws Exception {
  ...
  @Test public void updatesUserAffiliationToNone() throws Exception {
    unsubscribe.process(element, jid, request, null);
  ...
  @Test public void doesNotUpdateAffiliationIfOutcast() throws Exception {
    affiliation = new NodeAffiliationImpl(node, jid, Affiliations.outcast, new Date());
    Mockito.when(channelManager.getUserAffiliation(Mockito.anyString(), Mockito.any(JID.class))).thenReturn(affiliation);
    Mockito.when(channelManager.getNodeOwners(Mockito.anyString())).thenReturn(new ArrayList<JID>());
    unsubscribe.process(element, jid, request, null);
  ...
  @Test public void sendsExpectedNotifications() throws Exception {
    JID listener = new JID("channels.example.com");
    ArrayList<NodeSubscription> listeners = new ArrayList<NodeSubscription>();
    listeners.add(new NodeSubscriptionImpl(node, jid, listener, Subscriptions.subscribed));
