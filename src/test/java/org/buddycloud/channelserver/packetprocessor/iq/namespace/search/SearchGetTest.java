...
public class SearchGetTest extends IQTestHandler {
  ...
  @Test public void testReturnsQueryChildElement() throws Exception {
    search.process(request);
  ...
  @Test public void testReturnsDataFormInstructionsElement() throws Exception {
    search.process(request);
    Assert.assertEquals(1, queue.size());
    IQ response = (IQ) queue.poll();
    Assert.assertNull(response.getError());
  ...
  @Test public void testReturnsDataFormResultsPerPageElement() throws Exception {
    search.process(request);
    Assert.assertEquals(1, queue.size());
    IQ response = (IQ) queue.poll();
    Assert.assertNull(response.getError());
