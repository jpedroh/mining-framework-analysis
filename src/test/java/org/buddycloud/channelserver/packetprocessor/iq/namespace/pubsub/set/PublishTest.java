...
public class PublishTest extends IQTestHandler {
  ...
  @Before public void setUp() throws Exception {
    ...
    Mockito.when(channelManager.getNodeSubscriptionListeners(Mockito.eq(node))).thenReturn(new ResultSetImpl<NodeSubscription>(new ArrayList<NodeSubscription>()));
    validateEntry.setPayload(request.getChildElement().element("publish").element("item").createCopy());
    Mockito.when(validateEntry.getGlobalItemId()).thenReturn(new GlobalItemIDImpl(new JID(request.getTo().toBareJID()), node, entry.elementText("id")).toString());
  ...
  @Test public void passingRetractAsElementNameReturnsTrue() {
    Element element = new BaseElement("publish");
    Assert.assertTrue(publish.accept(element));
  }

  @Test public void passingNotRetractAsElementNameReturnsFalse() {
    Element element = new BaseElement("not-publish");
    Assert.assertFalse(publish.accept(element));
  }

  @Test public void passingNoNodeResultsInErrorStanza() throws Exception {
  ...
  @Test public void nodeStoreExceptionReturnsErrorStanza() throws Exception {
  ...
  @Test public void providingNodeWhichDoesntExistReturnsError() throws Exception {
  ...
  @Test public void requestToRemoteNodeResultsInForwardedPacket() throws Exception {
  ...
  @Test public void unsubscribedUserCanNotPublish() throws Exception {
  ...
  @Test public void pendingSubscriptionCanNotPublish() throws Exception {
  ...
  @Test public void noAffiliationCanNotPublish() throws Exception {
  ...
  @Test public void memberAffiliationCanNotPublish() throws Exception {
  ...
  @Test public void outcastAffiliationCanNotPublish() throws Exception {
  ...
  @Test public void noItemElementReturnsError() throws Exception {
    IQ request = this.request.createCopy();
  ...
  @Test public void invalidEntryReturnsError() throws Exception {
  ...
  @Test public void itemIsSavedAsExpected() throws Exception {
    IQ request = this.request.createCopy();
    publish.process(element, jid, request, null);
    Mockito.verify(channelManager, Mockito.times(1)).addNodeItem(Mockito.any(NodeItemImpl.class));
  }

  @Test public void expectedSuccessResponseReceived() throws Exception {
    IQ request = this.request.createCopy();
    publish.process(element, jid, request, null);
  ...
  @Test public void sendsOutExpectedNotifications() throws Exception {
    ...
    NodeSubscription subscriber3 = new NodeSubscriptionImpl(node, new JID("faustus@marlowe.lit"), new JID("channels.marlowe.lit"), Subscriptions.subscribed, null);
  ...
  @Test public void inReplyToIdIsSavedToDatabase() throws Exception {
    ...
    Mockito.when(validateEntry.getInReplyTo()).thenReturn(GlobalItemIDImpl.toLocalId(request.getChildElement().element("publish").element("item").element("entry").element("in-reply-to").attributeValue("ref")));
    publish.process(element, jid, request, null);
  ...
  @Test public void replyUpdatesThreadParentDate() throws Exception {
    IQ request = readStanzaAsIq("/iq/pubsub/publish/reply.stanza");
    Mockito.when(validateEntry.getPayload()).thenReturn(request.getChildElement().element("publish").element("item").element("entry"));
  ...
  @Test public void doesNotUpdateParentThreadIfNotReply() throws Exception {
    IQ request = this.request.createCopy();
    publish.process(element, jid, request, null);
