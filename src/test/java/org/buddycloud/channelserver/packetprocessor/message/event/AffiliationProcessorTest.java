...
public class AffiliationProcessorTest extends IQTestHandler {
  ...
  @Test(expected = IllegalArgumentException.class) public void testInvalidAffiliationValueThrowsException() throws Exception {
  ...
  @Test public void testMissingAffiliationElementDoesNotCauseError() throws Exception {
  ...
  @Test(expected = NodeStoreException.class) public void testNodeStoreExceptionIsThrownWhenExpected() throws Exception {
    Mockito.doThrow(new NodeStoreException()).when(channelManager).setUserAffiliation(Mockito.anyString(), Mockito.any(JID.class), Mockito.any(Affiliations.class));
    affiliationProcessor.process(message);
  }
