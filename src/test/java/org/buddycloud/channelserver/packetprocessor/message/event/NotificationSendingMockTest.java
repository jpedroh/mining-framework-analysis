...
public class NotificationSendingMockTest extends IQTestHandler {
  ...
  @Test public void noNotificationSentForRemoteUser() throws Exception {
  ...
  @Test public void sendsNotificationToSpecifiedUser() throws Exception {
  ...
  @Test public void onlySendsToValidSubscribers() throws Exception {
    registerUserResponse(Subscriptions.none, Affiliations.none);
    notificationSending.process(message);
    Assert.assertEquals(0, queue.size());
  ...
  @Test public void onlySendsToOwnersAndModerators() throws Exception {
    Message message = this.message.createCopy();
