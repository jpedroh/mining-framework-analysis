...
public class ItemsProcessorTest extends IQTestHandler {
  ...
  @Test public void testLocalNodeEventDoesNotSendNotiifcations() throws Exception {
  ...
  @Test(expected = NodeStoreException.class) public void testNodeStoreExceptionIsThrown() throws Exception {
    Mockito.doThrow(new NodeStoreException()).when(channelManager).getNodeMemberships(Mockito.anyString());
    itemsProcessor.process(message);
  }

  @Test(expected = NullPointerException.class) public void testConfigurationValueNotSetThrowsException() throws Exception {
    itemsProcessor.setConfiguration(new Properties());
    itemsProcessor.process(message);
  }

  @Test public void testNotificationsAreForwarded() throws Exception {
    itemsProcessor.process(message);
    Assert.assertEquals(1, queue.size());
  }

  @Test public void testRemoteNodeIsAddedIfNotInDatastore() throws Exception {
  ...
  @Test public void testItemIsDeletedBeforeAttemptToInsertIntoDatabase() throws Exception {
    Mockito.when(channelManager.nodeExists(Mockito.anyString())).thenReturn(false);
    itemsProcessor.process(message);
    Mockito.verify(channelManager, Mockito.times(1)).deleteNodeItemById(Mockito.anyString(), Mockito.anyString());
  }

  @Test public void testItemsAreCachedToDatastore() throws Exception {
