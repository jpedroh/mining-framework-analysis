...
public class SQLSourceHelperTest {
  @SuppressWarnings(value = { "unchecked" }) List<List<Object>> queryResult = mock(ArrayList.class);
  ...
  @Test(expected = ConfigurationException.class) public void checkStatusFileNameNotSet() {
    when(context.getString("status.file.name")).thenReturn(null);
    new SQLSourceHelper(context);
  }

  @Test(expected = ConfigurationException.class) public void connectionURLNotSet() {
    when(context.getString("connection.url")).thenReturn(null);
    new SQLSourceHelper(context);
  }

  @Test(expected = ConfigurationException.class) public void tableNotSet() {
    when(context.getString("table")).thenReturn(null);
    new SQLSourceHelper(context);
  }

  @Test(expected = ConfigurationException.class) public void userNotSet() {
    when(context.getString("user")).thenReturn(null);
    new SQLSourceHelper(context);
  }

  @Test(expected = ConfigurationException.class) public void passwordNotSet() {
  ...
  @Test public void chekGetAllRowsWithEmptyParam() {
    SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context);
    assertEquals(new ArrayList<String>(), sqlSourceHelper.getAllRows(new ArrayList<List<Object>>()));
  }

  @SuppressWarnings(value = { "deprecation" }) @Test public void chekGetAllRows() {
    SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context);
  ...
  @Test public void checkStatusFileCorrectlyUpdated() throws Exception {
  ...
  @Test public void getConnectionURL() {
    SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context);
    assertEquals("jdbc:mysql://host:3306/database", sqlSourceHelper.getConnectionURL());
  }

  @SuppressWarnings(value = { "unused" }) @Test public void createDirectory() {
    SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context);
  ...
  @Test public void setCurrentIndex() {
    SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context);
    sqlSourceHelper.setCurrentIndex(10);
  ...
  @Test public void getRunQueryDelay() {
    SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context);
  ...
  @Test public void getQuery() {
    SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context);
    assertEquals("SELECT * FROM table", sqlSourceHelper.getQuery());
  }

  @Test public void getCustomQuery() {
