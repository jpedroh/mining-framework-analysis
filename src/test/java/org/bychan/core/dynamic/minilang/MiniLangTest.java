...
public class MiniLangTest {
  @Test public void test() {
    ...
    final TokenDefinitionBuilder<LaiLaiNode> rcurly = lb2.newToken().matchesString("}").named("rcurly");
    final TokenDefinitionBuilder<LaiLaiNode> lcurly = lb2.newToken().matchesString("{").named("lcurly").nud((previous, parser, lexeme) -> {
    ...
    final TokenDefinitionBuilder<LaiLaiNode> rparen = lb2.newToken().matchesString(")").named("rparen");
    TokenDefinitionBuilder<LaiLaiNode> lparen = lb2.newToken().matchesString("(").named("lparen").nud((previous, parser, lexeme) -> {
      LaiLaiNode trailingExpression = parser.subExpression(previous);
      parser.expectSingleLexeme(rparen.getName());
      return trailingExpression;
    });
    TokenDefinitionBuilder<LaiLaiNode> whitespace = lb2.newToken().matchesPattern("\\s+").named("whitespace").ignoredWhenParsing();
    TokenDefinitionBuilder<LaiLaiNode> plus = lb2.newToken().matchesString("+").named("plus").nud((previous, parser, lexeme) -> parser.subExpression(previous)).led((previous, parser, lexeme) -> new AdditionNode(previous, parser.subExpression(previous)));
    TokenDefinitionBuilder<LaiLaiNode> hat = lb2.newToken().matchesString("^").named("hat").led((previous, parser, lexeme) -> new HatNode(previous, parser.subExpression(previous)));
    TokenDefinitionBuilder<LaiLaiNode> assign = lb2.newToken().matchesString("=").named("assign").led((previous, parser, lexeme) -> {
      LaiLaiNode right = parser.subExpression(previous);
    ...
    TokenDefinitionBuilder<LaiLaiNode> variableDeclaration = lb2.newToken().matchesPattern("(?:float|int|bool) [a-z]+").named("variableDef").nud(parseAction4);
    TokenDefinitionBuilder<LaiLaiNode> variableReference = lb2.newToken().matchesPattern("[a-z]+").named("variableRef").nud((previous, parser, lexeme) -> {
      String name = lexeme.getText();
      return new VariableRefNode(name);
    });
    TokenDefinitionBuilder<LaiLaiNode> booleanLiteral = lb2.newToken().matchesPattern("true|false").named("bool").nud((previous, parser, lexeme) -> new BooleanLiteralNode(Boolean.parseBoolean(lexeme.getText())));
    TokenDefinitionBuilder<LaiLaiNode> integerLiteral = lb2.newToken().matchesPattern("[0-9]+i").named("int").nud((previous, parser, lexeme) -> {
      String text = lexeme.getText();
      return new IntegerLiteralNode(previous, Integer.parseInt(text.substring(0, text.length() - 1)));
    });
    TokenDefinitionBuilder<LaiLaiNode> floatLiteral = lb2.newToken().matchesPattern("[0-9]+f").named("float").nud((previous, parser, lexeme) -> new FloatLiteralNode(previous, Float.parseFloat(lexeme.getText())));
    TokenDefinitionBuilder<LaiLaiNode> semicolon = lb2.newToken().matchesString(";").named("statement").led((previous, parser, lexeme) -> new StatementNode(previous, parser.subExpression(previous)));
    final TokenDefinitionBuilder<LaiLaiNode> listEnd = lb2.newToken().matchesString("]").named("listEnd");
    final TokenDefinitionBuilder<LaiLaiNode> comma = lb2.newToken().matchesString(",").named("comma");
    TokenDefinitionBuilder<LaiLaiNode> listStart = lb2.newToken().matchesString("[").named("listStart").nud((previous, parser, lexeme) -> {
