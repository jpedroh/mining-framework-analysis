...
public class ApacheDS {
  ...
  private ApacheDS startDirectoryService() throws Exception {
    ...
    partition.setIndexedAttributes(Sets.<Index<?, ServerEntry>>newHashSet(new JdbmIndex<String, ServerEntry>("ou"), new JdbmIndex<String, ServerEntry>("uid"), new JdbmIndex<String, ServerEntry>("dc"), new JdbmIndex<String, ServerEntry>("objectClass")));
  ...
  @SuppressWarnings(value = { "unused" }) private ApacheDS startKerberos() throws Exception {
    Preconditions.checkState(ldapServer.isStarted());
  ...
  private ApacheDS activateNis() throws Exception {
    ...
    ModificationItem[] mods = new ModificationItem[] { new ModificationItem(DirContext.REMOVE_ATTRIBUTE, disabled), new ModificationItem(DirContext.ADD_ATTRIBUTE, disabled2) };
