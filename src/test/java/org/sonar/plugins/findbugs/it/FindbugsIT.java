...
@IntegrationTest class FindbugsIT {
  ...
  @BeforeEach public void setupProfile() {
    FindbugsTestSuite.setupProjectAndProfile(PROJECT_KEY, "Findbugs Integration Tests", "findbugs-it", "java");
  }

  @AfterEach public void deleteProject() {
    FindbugsTestSuite.deleteProject(PROJECT_KEY);
  }

  @Test void analysis() {
    MavenBuild build = MavenBuild.create(FindbugsTestSuite.projectPom("findbugs")).setCleanPackageSonarGoals().setProperty("sonar.dynamicAnalysis", "false");
   ...
   * SONAR-2325
   */
  @Test void confidence_level() {
    MavenBuild build = MavenBuild.create(FindbugsTestSuite.projectPom("findbugs")).setCleanPackageSonarGoals().setProperty("sonar.dynamicAnalysis", "false").setProperty("sonar.findbugs.confidenceLevel", "high");
    orchestrator.executeBuild(build);
    IssuesService issueClient = FindbugsTestSuite.issueClient();
    List<Issue> issues = issueClient.search(IssueQuery.create().components(FindbugsTestSuite.keyFor(PROJECT_KEY, "", "Findbugs1.java"))).getIssuesList();
    assertThat(issues).isEmpty();
   ...
   * SONARJAVA-380
   */
  @Test void should_always_use_english_locale() throws Exception {
    MavenBuild build = MavenBuild.create(FindbugsTestSuite.projectPom("findbugs")).setCleanPackageSonarGoals().setProperty("sonar.dynamicAnalysis", "false").setProperty("sonar.findbugs.confidenceLevel", "low").setProperty("sonar.violationLocale", "fr").setEnvironmentVariable("MAVEN_OPTS", "-Duser.language=fr");
    orchestrator.executeBuild(build);
    IssuesService issueClient = FindbugsTestSuite.issueClient();
   ...
   * SONARJAVA-385
   */
  @Test void inclusions_exclusions() throws Exception {
    File projectDir = FindbugsTestSuite.projectPom("findbugs").getParentFile();
    MavenBuild build = MavenBuild.create(new File(projectDir, "pom.xml")).setGoals("clean package");
    orchestrator.executeBuild(build);
  ...
  @Test void multiple_directories_with_classes() throws Exception {
    File projectDir = FindbugsTestSuite.projectPom("multiple-directories-with-classes").getParentFile();
    MavenBuild build = MavenBuild.create(new File(projectDir, "pom.xml")).setGoals("clean package");
    orchestrator.executeBuild(build);
    SonarScanner sonarScanner = SonarScanner.create().setProjectDir(projectDir).setProperty("sonar.projectKey", PROJECT_KEY).setProperty("sonar.projectName", "example").setProperty("sonar.projectVersion", "1").setProperty("sonar.sources", ".").setProperty("sonar.java.binaries", "dir1/target/classes,dir2/target/classes");
    orchestrator.executeBuild(sonarScanner);
    IssuesService issueClient = FindbugsTestSuite.issueClient();
  ...
  @Test void only_analyze() throws Exception {
    File projectDir = FindbugsTestSuite.projectPom("findbugs").getParentFile();
    MavenBuild build = MavenBuild.create(new File(projectDir, "pom.xml")).setProperty(FindbugsConstants.ONLY_ANALYZE_PROPERTY, "Findbugs1").setGoals("clean package sonar:sonar");
    orchestrator.executeBuild(build);
    String findbugsXml = Files.readString(projectDir.toPath().resolve(".scannerwork/findbugs-result.xml"), StandardCharsets.UTF_8);
