...
public class IterableAssert_usingRecursiveFieldByFieldElementComparator_Test extends IterableAssertBaseTest {
  ...
  @Test public void failed_isEqualTo_assertion_using_recursive_field_by_field_element_comparator() {
    ...
    try {
      ...
      assertThat(e).hasMessage(format("%nExpecting:%n" + " <[Foo(id=id, bar=Bar [id=1])]>%n" + "to be equal to:%n" + " <[Foo(id=id, bar=Bar [id=2])]>%n" + "when comparing elements using \'recursive field/property by field/property comparator on all fields/properties\' but was not."));
      return;
    }
  ...
  @Test public void failed_isIn_assertion_using_recursive_field_by_field_element_comparator() {
    ...
    try {
      ...
      assertThat(e).hasMessage(String.format("%nExpecting:%n" + " <[Foo(id=id, bar=Bar [id=1])]>%n" + "to be in:%n" + " <[[Foo(id=id, bar=Bar [id=2])]]>%n" + "when comparing elements using \'recursive field/property by field/property comparator on all fields/properties\'"));
      return;
    }
    failBecauseExpectedAssertionErrorWasNotThrown();
  }
  ...
  @Test public void should_be_able_to_use_a_comparator_for_specified_fields_of_elements_when_using_recursive_field_by_field_element_comparator() {
    ...
    assertThat(singletonList(actual)).usingComparatorForElementFieldsWithNames(new AlwaysEqualIntegerComparator(), "bar.id").usingRecursiveFieldByFieldElementComparator().contains(other);
  }
  ...
  @Test public void comparators_for_element_field_names_should_have_precedence_over_comparators_for_element_field_types_when_using_recursive_field_by_field_element_comparator() {
    ...
    assertThat(singletonList(actual)).usingComparatorForElementFieldsWithNames(ALWAY_EQUALS, "id").usingComparatorForElementFieldsWithType(comparator, String.class).usingRecursiveFieldByFieldElementComparator().contains(other);
  }
  ...
  @Test public void should_be_able_to_use_a_comparator_for_element_fields_with_specified_type_when_using_recursive_field_by_field_element_comparator() {
    Foo actual = new Foo("1", new Bar(1));
    Foo other = new Foo("2", new Bar(1));
    assertThat(singletonList(actual)).usingComparatorForElementFieldsWithType(ALWAY_EQUALS, String.class).usingRecursiveFieldByFieldElementComparator().contains(other);
  }
