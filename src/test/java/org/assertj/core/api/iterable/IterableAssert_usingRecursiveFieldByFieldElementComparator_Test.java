...
public class IterableAssert_usingRecursiveFieldByFieldElementComparator_Test extends IterableAssertBaseTest {
  ...
  @Test public void failed_isEqualTo_assertion_using_recursive_field_by_field_element_comparator() {
    ...
    thrown.expectAssertionError("%nExpecting:%n" + " <[Foo(id=id, bar=Bar [id=1])]>%n" + "to be equal to:%n" + " <[Foo(id=id, bar=Bar [id=2])]>%n" + "when comparing elements using \'recursive field/property by field/property comparator on all fields/properties\' but was not.");
    assertThat(list1).usingRecursiveFieldByFieldElementComparator().isEqualTo(list2);
  }
  ...
  @Test public void failed_isIn_assertion_using_recursive_field_by_field_element_comparator() {
    List<Foo> list1 = singletonList(new Foo("id", new Bar(1)));
    List<Foo> list2 = singletonList(new Foo("id", new Bar(2)));
    thrown.expectAssertionError("%nExpecting:%n" + " <[Foo(id=id, bar=Bar [id=1])]>%n" + "to be in:%n" + " <[[Foo(id=id, bar=Bar [id=2])]]>%n" + "when comparing elements using \'recursive field/property by field/property comparator on all fields/properties\'");
    assertThat(list1).usingRecursiveFieldByFieldElementComparator().isIn(singletonList(list2));
  }
