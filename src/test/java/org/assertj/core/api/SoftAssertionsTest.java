...
public class SoftAssertionsTest {
  ...
  @SuppressWarnings(value = { "unchecked" }) @Test public void should_be_able_to_catch_exceptions_thrown_by_all_proxied_methods() throws URISyntaxException {
    try {
      ...
      assertThat(errors.get(20)).isEqualTo(String.format("%nInputStreams do not have same content:" + System.getProperty("line.separator") + "line:<1>, expected:<B> but was:<A>"));
  ...
  @Test public void should_collect_all_errors_when_using_filtering() throws Exception {
    List<CartoonCharacter> characters = asList(homer, fred);
    softly.assertThat(characters).overridingErrorMessage("error 1").filteredOn("name", "Homer Simpson").containsOnly(fred);
    try {
      softly.assertAll();
      shouldHaveThrown(SoftAssertionError.class);
    } catch (SoftAssertionError e) {
