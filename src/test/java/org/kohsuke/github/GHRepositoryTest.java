...
public class GHRepositoryTest extends AbstractGitHubWireMockTest {
  ...
  @Test public void archive() throws Exception {
    snapshotNotAllowed();
  ...
  @Test public void getBranch_URLEncoded() throws Exception {
    GHRepository repo = getRepository();
    GHBranch branch = repo.getBranch("test/#UrlEncode");
    assertThat(branch.getName(), is("test/#UrlEncode"));
  }

  @Test public void getBranchNonExistentBut200Status() throws Exception {
  ...
  @Test public void subscription() throws Exception {
  ...
  @Test public void testSetPublic() throws Exception {
    kohsuke();
  ...
  @Test public void listContributors() throws IOException {
  ...
  @Test public void getPermission() throws Exception {
    kohsuke();
  ...
  @Test public void LatestRepositoryExist() {
    try {
  ...
  @Test public void LatestRepositoryNotExist() {
    try {
  ...
  @Test public void getReleaseExists() throws IOException {
  ...
  @Test public void listEmptyContributors() throws IOException {
  ...
  @Test public void searchRepositories() throws Exception {
  ...
  @Test public void testIssue162() throws Exception {
    GHRepository r = gitHub.getRepository("github-api/github-api");
  ...
  @Test public void markDown() throws Exception {
  ...
  @Test public void getMergeOptions() throws IOException {
  ...
  @Test public void setMergeOptions() throws IOException {
    GHRepository r = getTempRepository();
  ...
  @Test public void testSetTopics() throws Exception {
    GHRepository repo = getRepository(gitHub);
    List<String> topics = new ArrayList<>();
    topics.add("java");
    topics.add("api-test-dummy");
    repo.setTopics(topics);
    assertThat("Topics retain input order (are not sort when stored)", repo.listTopics(), contains("java", "api-test-dummy"));
    topics = new ArrayList<>();
    topics.add("ordered-state");
    topics.add("api-test-dummy");
    topics.add("java");
    repo.setTopics(topics);
    assertThat("Topics behave as a set and retain order from previous calls", repo.listTopics(), contains("java", "api-test-dummy", "ordered-state"));
    topics = new ArrayList<>();
    topics.add("ordered-state");
    topics.add("api-test-dummy");
    repo.setTopics(topics);
    assertThat("Topics retain order even when some are removed", repo.listTopics(), contains("api-test-dummy", "ordered-state"));
    topics = new ArrayList<>();
    repo.setTopics(topics);
