...
public class ProxyServletTest {
  ...
  @Before public void setUp() throws Exception {
    ...
    servletRunner.registerServlet("/proxyMe/*", ProxyServlet.class.getName(), params);
    ...
    params.setProperty(ProxyServlet.P_LOG, "true");
    params.setProperty(ProxyServlet.P_FORWARDEDFOR, "true");
    parameterizedServletRunner.registerServlet("/proxyParamMe/*", ProxyServlet.class.getName(), params);
    parameterizedSourceBaseUri = "http://localhost/proxyParamMe";
    psc = parameterizedServletRunner.newClient();
    psc.getClientProperties().setAutoRedirect(false);
  }
  ...
  private static String[] testUrlSuffixes = new String[] { "", "/pathInfo", "?q=v", "/p?q=v", "/p?query=note:Leitbild", "/p?id=p%20i", "/p%20i" };
  ...
  @Test public void testGetParameterized() throws Exception {
    for (String urlSuffix : testUrlSuffixes) {
  ...
  @Test public void testPostParameterized() throws Exception {
    for (String urlSuffix : testUrlSuffixes) {
      String url = parameterizedSourceBaseUri + urlSuffix;
      String params = makeParams(url);
  ...
  private String makeParams(String url) {
    ...
    params.append("hostProxyArg=").append(hostParam).append('&').append("proxyArg2=").append(portParam).append('&').append("pathProxyArg=").append(pathParam);
  ...
  @Test public void testSendFile() throws Exception {
    final PostMethodWebRequest request = new PostMethodWebRequest("http://localhost/proxyMe", true);
  ...
  @Test public void testHopByHopHeadersOnSource() throws Exception {
  ...
  private WebResponse execAndAssertParam(WebRequest request, String params, String expectedUri) throws Exception {
    ...
    assertTrue(text.startsWith("REQUESTLINE:"));
    if (expectedUri == null) {
      expectedUri = request.getURL().toString().substring(parameterizedSourceBaseUri.length()).replace(params, "");
    }
    String firstTextLine = text.substring(0, text.indexOf(System.getProperty("line.separator")));
  ...
  @SuppressWarnings(value = { "unchecked" }) private static <M extends java.lang.Object> M makeMethodRequest(final String url, Class<M> clazz) {
