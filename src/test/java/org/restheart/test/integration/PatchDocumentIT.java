...
public class PatchDocumentIT extends HttpClientAbstactIT {
  ...
  @Test public void testPatchDocument() throws Exception {
    Response resp;
    ...
    resp = adminExecutor.execute(Request.Patch(documentTmpUri).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    check("check patch tmp doc without data", resp, HttpStatus.SC_NOT_ACCEPTABLE);
    resp = adminExecutor.execute(Request.Patch(addCheckEtag(documentTmpUri)).bodyString("{a:1}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    check("check patch tmp doc without etag forcing checkEtag", resp, HttpStatus.SC_CONFLICT);
    resp = adminExecutor.execute(Request.Patch(documentTmpUri).bodyString("{a:1}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    check("check patch tmp doc without etag", resp, HttpStatus.SC_OK);
    resp = adminExecutor.execute(Request.Patch(documentTmpUri).bodyString("{a:1}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE).addHeader(Headers.IF_MATCH_STRING, "pippoetag"));
    check("check patch tmp doc with wrong etag", resp, HttpStatus.SC_PRECONDITION_FAILED);
    resp = adminExecutor.execute(Request.Get(documentTmpUri).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    ...
    resp = adminExecutor.execute(Request.Patch(documentTmpUri).bodyString("{b:2}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE).addHeader(Headers.IF_MATCH_STRING, etag));
  ...
  @Before public void createTestData() throws Exception {
    ...
    resp = Unirest.put(url(DB, COLL)).basicAuth(ADMIN_ID, ADMIN_PWD).asString();
    Assert.assertEquals("create collection " + DB.concat("/").concat(COLL), org.apache.http.HttpStatus.SC_CREATED, resp.getStatus());
  }

  @Test public void testPatchDocumentDotNotation() throws Exception {
    resp = Unirest.put(url(DB, COLL, "docid1")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'array\': [] }").asString();
    Assert.assertEquals("check response status of create test data", org.apache.http.HttpStatus.SC_CREATED, resp.getStatus());
    resp = Unirest.patch(url(DB, COLL, "docid1")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'doc.number\': 1, \'array.0\': {\'string\': \'ciao\'} }").asString();
    ...
    Assert.assertTrue("check data to have the \'array\' json array", array != null && array.isArray());
    JsonValue element = array.asArray().get(0);
    Assert.assertTrue("check array to have an object element", element != null && element.isObject());
    JsonValue string = element.asObject().get("string");
    Assert.assertTrue("check the array element to have the \'string\' property", string != null && string.isString() && string.asString().equals("ciao"));
  }

  @Test public void testPatchDocumentOperators() throws Exception {
    resp = Unirest.put(url(DB, COLL, "docid2")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ }").asString();
    Assert.assertEquals("check response status of create test data", org.apache.http.HttpStatus.SC_CREATED, resp.getStatus());
    resp = Unirest.patch(url(DB, COLL, "docid2")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'$push\': {\'array\': \'a\'}, \'$inc\': { \'count\': 100 }, \'$currentDate\': {\'timestamp\': true } }").asString();
    ...
    Assert.assertTrue("check data to be a json object", rbody != null && rbody.isObject());
    JsonValue array = rbody.asObject().get("array");
    Assert.assertTrue("check data to have the \'array\' array with one element", array != null && array.isArray() && array.asArray().size() == 1);
    JsonValue element = array.asArray().get(0);
    Assert.assertTrue("check array element to be the string \'a\'", element != null && element.isString() && element.asString().equals("a"));
    JsonValue count = rbody.asObject().get("count");
    Assert.assertTrue("check count property to be 100", count != null && count.isNumber() && count.asInt() == 100);
  ...
  /**
     ...
     * @throws Exception 
     */
  @Test public void testPatchDocumentIssue232() throws Exception {
    resp = Unirest.patch(url(DB, COLL, "issue232")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'$addToSet\':{ \'addresses\':{ \'addressType\' : \'N\', \'line2\':\'line 2\' } } }").asString();
