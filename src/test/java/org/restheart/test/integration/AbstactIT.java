...
public abstract class AbstactIT {
  ...
  protected static String url(String dbname, String... parts) {
    ...
    if (dbname != null) {
      sb.append("/").append(TEST_DB_PREFIX).append(dbname);
      if (parts != null) {
  ...
  private static void deleteTestData() {
    ...
    MongoDBClientSingleton.getInstance().getClient().listDatabaseNames().into(dbNames);
    ArrayList<String> deleted = new ArrayList<>();
    dbNames.stream().filter((db) -> db.startsWith(TEST_DB_PREFIX)).forEach((dbToDelete) -> {
    ...
    deleted.stream().forEach((db) -> {
      try {
        HttpResponse resp = Unirest.post(HTTP_HOST.toURI() + "/_logic/ic").basicAuth(ADMIN_ID, ADMIN_PWD).queryString("db", db).asJson();
