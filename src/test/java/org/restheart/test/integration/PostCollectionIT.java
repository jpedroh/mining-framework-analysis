...
public class PostCollectionIT extends HttpClientAbstactIT {
  ...
  @Test public void testPostCollection() throws Exception {
    Response response;
  ...
  @Test public void testPostDocumentDotNotation() throws Exception {
    resp = Unirest.post(url(DB, COLL)).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'_id\': \'docid1\', \'doc.number\': 1 }").asString();
  ...
  @Test public void testPostDocumentOperators() throws Exception {
    ...
    resp = Unirest.post(url(DB, COLL)).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'_id\': \'docid2\', \'$currentDate\': {\'timestamp\': true } }").asString();
