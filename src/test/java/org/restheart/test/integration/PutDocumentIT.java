...
public class PutDocumentIT extends HttpClientAbstactIT {
  ...
  @Test public void testPutDocument() throws Exception {
    ...
    response = adminExecutor.execute(Request.Put(documentTmpUri).bodyString("{a:1}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    check("check put tmp doc", response, HttpStatus.SC_CREATED);
    response = adminExecutor.execute(Request.Put(documentTmpUri).bodyString("{a:1}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    check("check put tmp doc without etag", response, HttpStatus.SC_OK);
    response = adminExecutor.execute(Request.Put(addCheckEtag(documentTmpUri)).bodyString("{a:1}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    check("check put tmp doc without etag forcing checkEtag", response, HttpStatus.SC_CONFLICT);
    response = adminExecutor.execute(Request.Put(documentTmpUri).bodyString("{a:1}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE).addHeader(Headers.IF_MATCH_STRING, "pippoetag"));
    check("check put tmp doc with wrong etag", response, HttpStatus.SC_PRECONDITION_FAILED);
    response = adminExecutor.execute(Request.Get(documentTmpUri).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE));
    ...
    response = adminExecutor.execute(Request.Put(documentTmpUri).bodyString("{b:2}", halCT).addHeader(Headers.CONTENT_TYPE_STRING, Resource.HAL_JSON_MEDIA_TYPE).addHeader(Headers.IF_MATCH_STRING, etag));
  ...
  @Test public void testPutDocumentWithNotMatchingFilter() throws Exception {
    resp = Unirest.put(url(DB, COLL, "testPutWithWrongFilter")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{\'a\':1}").asString();
    Assert.assertEquals(HttpStatus.SC_CREATED, resp.getStatus());
    resp = Unirest.put(url(DB, COLL, "testPutWithWrongFilter")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").queryString("filter", "{\'a\':2}").body("{\'modified\':true}").asString();
    Assert.assertEquals(HttpStatus.SC_NOT_FOUND, resp.getStatus());
  }

  @Test public void testPutDocumentDotNotation() throws Exception {
    resp = Unirest.put(url(DB, COLL, "docid1")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'doc.number\': 1 }").asString();
  ...
  @Test public void testPutDocumentOperators() throws Exception {
    resp = Unirest.put(url(DB, COLL, "docid2")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'$push\': {\'array\': \'a\'}, \'$inc\': { \'count\': 100 } }").asString();
    Assert.assertEquals("check response status of create test data", org.apache.http.HttpStatus.SC_BAD_REQUEST, resp.getStatus());
    resp = Unirest.put(url(DB, COLL, "docid2")).basicAuth(ADMIN_ID, ADMIN_PWD).header("content-type", "application/json").body("{ \'$currentDate\': {\'timestamp\': { \'$type\': \'date\' }}}").asString();
