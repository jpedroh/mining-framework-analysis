...
public class QueryTest {
  @BeforeClass public static void setup() throws Throwable {
    ...
    SqlClosureElf.executeUpdate("CREATE TABLE target_class1 (" + "id INTEGER NOT NULL IDENTITY PRIMARY KEY, " + "timestamp TIMESTAMP, " + "string VARCHAR(128), " + "string_from_number NUMERIC " + ")");
  }
  ...
  @Test public void testObjectFromClause() {
  ...
  @Test public void testNumberFromSql() {
  ...
  @Test public void testConverterSave() {
  ...
  @Test public void testConverterLoad() throws Exception {
    TargetClass1 target = SqlClosureElf.insertObject(new TargetClass1(null, null, "1234"));
    final int targetId = target.getId();
    target = SqlClosure.execute((connection) -> {
      PreparedStatement pstmt = connection.prepareStatement("SELECT t.id, t.timestamp, t.string, (t.string_from_number + 1) as string_from_number FROM target_class1 t where id = ?");
      return OrmElf.statementToObject(pstmt, TargetClass1.class, targetId);
    });
    assertEquals("1235", target.getStringFromNumber());
  }

  @Test public void testConversionFail() {
