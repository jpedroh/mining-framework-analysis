package org.apache.commons.collections4.queue;
import java.util.LinkedList;
import java.util.Queue;
import org.apache.commons.collections4.BulkTest;
import org.junit.jupiter.api.Disabled;
import junit.framework.Test;

/**
 * Extension of {@link AbstractQueueTest} for exercising the
 * {@link SynchronizedQueue} implementation.
 *
 * @since 4.2
 */
public class SynchronizedQueueTest<T extends java.lang.Object> extends AbstractQueueTest<T> {
  public static Test suite() {
    return BulkTest.makeSuite(SynchronizedQueueTest.class);
  }

  public SynchronizedQueueTest(final String testName) {
    super(testName);
  }

  @Override public String getCompatibilityVersion() {
    return "4.2";
  }

  @Override public Queue<T> makeObject() {
    return SynchronizedQueue.synchronizedQueue(new LinkedList<T>());
  }

  @Disabled(value = "Run once") public void testCreate() throws Exception {
    Queue<T> queue = makeObject();
    writeExternalFormToDisk((java.io.Serializable) queue, BulkTest.TEST_DATA_PATH + "SynchronizedQueue.emptyCollection.version4.2.obj");
    queue = makeFullCollection();
    writeExternalFormToDisk((java.io.Serializable) queue, BulkTest.TEST_DATA_PATH + "SynchronizedQueue.fullCollection.version4.2.obj");
  }
}