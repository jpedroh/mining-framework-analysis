...
public class JsonTypesTest {
  @Test public void testObjectInt() {
  ...
  @Test public void testObjectString() {
    JsonObject o = new JsonObject();
    o.put("key", "1");
  ...
  @Test public void testObjectNull() {
    JsonObject o = new JsonObject();
    o.put("key", null);
    assertEquals(0, o.getInt("key"));
    assertEquals(0L, o.getLong("key"));
  ...
  @Test public void testArrayNull() {
    JsonArray o = new JsonArray(Arrays.asList((String) null, null, null, null));
    o.set(3, null);
    assertEquals(0, o.getInt(3));
    assertEquals(0, o.getDouble(3), 0.0001f);
  ...
  @Test public void testJsonArrayBuilder() {
    JsonArray a = JsonArray.builder().value(true).value(1.0).value(1.0f).value(1).value(new BigInteger("1234567890")).value("hi").object().value("abc", 123).end().array().value(1).nul().end().array(JsonArray.from(1, 2, 3)).object(JsonObject.builder().nul("a").done()).done();
    assertEquals("[true,1.0,1.0,1,1234567890,\"hi\",{\"abc\":123},[1,null],[1,2,3],{\"a\":null}]", JsonWriter.string(a));
  }

  @Test public void testJsonObjectBuilder() {
    JsonObject a = JsonObject.builder().value("bool", true).value("double", 1.0).value("float", 1.0f).value("int", 1).value("bigint", new BigInteger("1234567890")).value("string", "hi").nul("null").object("object").value("abc", 123).end().array("array").value(1).nul().end().array("existingArray", JsonArray.from(1, 2, 3)).object("existingObject", JsonObject.builder().nul("a").done()).done();
    String[] bits = new String[] { "\"bigint\":1234567890", "\"int\":1", "\"string\":\"hi\"", "\"existingObject\":{\"a\":null}", "\"existingArray\":[1,2,3]", "\"object\":{\"abc\":123}", "\"bool\":true", "\"double\":1.0", "\"float\":1.0", "\"null\":null", "\"array\":[1,null]" };
  ...
  @Test(expected = JsonWriterException.class) public void testJsonArrayBuilderFailCantCloseRoot() {
    JsonArray.builder().end();
  }

  @Test(expected = JsonWriterException.class) public void testJsonArrayBuilderFailCantAddKeyToArray() {
    JsonArray.builder().value("abc", 1);
  }

  @Test(expected = JsonWriterException.class) public void testJsonArrayBuilderFailCantAddNonKeyToObject() {
    JsonObject.builder().value(1);
  }

  @Test public void testJsonKeyOrder() {
    JsonObject a = JsonObject.builder().value("key01", 1).value("key02", 2).value("key03", 3).value("key04", 4).done();
    assertArrayEquals(new String[] { "key01", "key02", "key03", "key04" }, a.keySet().toArray(new String[0]));
  }
}