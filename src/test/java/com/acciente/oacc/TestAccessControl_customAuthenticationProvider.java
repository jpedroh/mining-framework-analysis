...
public class TestAccessControl_customAuthenticationProvider extends TestAccessControlBase {
  ...
  @Before public void setUpTest() throws Exception {
    ...
    systemAccessControlContext.setDomainPermissions(adminResource, adminDomain, setOf(DomainPermissions.getInstance(DomainPermissions.SUPER_USER)));
    ...
    customAccessControlContext = SQLAccessControlContextFactory.getAccessControlContext(dataSource, TestConfigLoader.getDatabaseSchema(), sqlProfile, new CustomAuthenticationProvider(dataSource, TestConfigLoader.getDatabaseSchema(), sqlProfile.getSqlDialect()));
  }

  @After public void tearDownTest() throws Exception {
    customAccessControlContext.unauthenticate();
  }

  @Test public void authenticateSystemUser_custom_shouldFail() {
    Resource systemAuthResource = getSystemResource();
    try {
      customAccessControlContext.authenticate(systemAuthResource, PasswordCredentials.newInstance(TestConfigLoader.getOaccRootPassword()));
  ...
  @Test public void authenticate_withoutCredentials_shouldSucceed() {
  ...
  @Test public void createResource_withoutCredentials_shouldFail() {
    customAccessControlContext.authenticate(adminResource, PasswordCredentials.newInstance(ADMIN_PASSWORD));
    final Resource resource = customAccessControlContext.createResource(generateResourceClass(true, true), customAccessControlContext.getDomainNameByResource(adminResource));
    assertThat(resource, is(not(nullValue())));
  }

  @Test public void createResource_validCredentialCriteria_shouldSucceed() {
    customAccessControlContext.authenticate(adminResource, PasswordCredentials.newInstance(ADMIN_PASSWORD));
    final Resource strictAuthenticatableResource = customAccessControlContext.createResource(strictResourceClass, strictDomain, PasswordCredentials.newInstance("opensesameplease".toCharArray()));
    assertThat(strictAuthenticatableResource, is(not(nullValue())));
  }

  @Test public void createResource_invalidCredentialCriteria_shouldFail() {
    customAccessControlContext.authenticate(adminResource, PasswordCredentials.newInstance(ADMIN_PASSWORD));
    try {
      customAccessControlContext.createResource(strictResourceClass, strictDomain, PasswordCredentials.newInstance("tooshort".toCharArray()));
  ...
  @Test public void setCredentials_validCredentialCriteria_shouldSucceed() {
    customAccessControlContext.authenticate(adminResource, PasswordCredentials.newInstance(ADMIN_PASSWORD));
    final Resource strictAuthenticatableResource = customAccessControlContext.createResource(strictResourceClass, strictDomain, PasswordCredentials.newInstance("opensesameplease".toCharArray()));
  ...
  @Test public void setCredentials_invalidCredentialCriteria_shouldFail() {
    customAccessControlContext.authenticate(adminResource, PasswordCredentials.newInstance(ADMIN_PASSWORD));
    final Resource strictAuthenticatableResource = customAccessControlContext.createResource(strictResourceClass, strictDomain, PasswordCredentials.newInstance("opensesameplease".toCharArray()));
    try {
      customAccessControlContext.setCredentials(strictAuthenticatableResource, PasswordCredentials.newInstance("tooshort".toCharArray()));
  ...
  private static class CustomAuthenticationProvider extends SQLPasswordAuthenticationProvider {
    protected CustomAuthenticationProvider(Connection connection, String schemaName, SQLDialect sqlDialect) {
      super(connection, schemaName, sqlDialect, JasyptPasswordEncryptor.getPasswordEncryptor());
    }

    protected CustomAuthenticationProvider(DataSource dataSource, String schemaName, SQLDialect sqlDialect) {
    ...
    @Override public void validateCredentials(String resourceClassName, String domainName, Credentials credentials) {
      if (credentials != null) {
        if (credentials instanceof PasswordCredentials && strictResourceClass.equalsIgnoreCase(resourceClassName) && strictDomain.equalsIgnoreCase(domainName) && ((PasswordCredentials) credentials).getPassword().length < strictMinPasswordLength) {
