...
public class TestAccessControlBase {
  ...
  static {
    ...
    systemAccessControlContext = SQLAccessControlContextFactory.getAccessControlContext(dataSource, TestConfigLoader.getDatabaseSchema(), sqlProfile, TestConfigLoader.getPasswordEncryptor());
  }

  protected AccessControlContext accessControlContext;

  @Before public void setUpTest() throws Exception {
    SQLAccessControlSystemResetUtil.resetOACC(dataSource, TestConfigLoader.getDatabaseSchema(), TestConfigLoader.getOaccRootPassword(), TestConfigLoader.getPasswordEncryptor());
    accessControlContext = SQLAccessControlContextFactory.getAccessControlContext(dataSource, TestConfigLoader.getDatabaseSchema(), sqlProfile, TestConfigLoader.getPasswordEncryptor());
  }
  ...
  protected static <T extends java.lang.Object> Set<T> setOf(T... elements) {
  ...
  protected static String generateResourceClass(boolean authenticatable, boolean nonAuthenticatedCreateAllowed) {
    authenticateSystemAccessControlContext();
  ...
  protected static void authenticateSystemAccessControlContext() {
    systemAccessControlContext.authenticate(SYS_RESOURCE, PasswordCredentials.newInstance(TestConfigLoader.getOaccRootPassword()));
  }

  protected void authenticateSystemResource() {
    accessControlContext.authenticate(SYS_RESOURCE, PasswordCredentials.newInstance(TestConfigLoader.getOaccRootPassword()));
  }
  ...
  protected static Resource generateAuthenticatableResource(char[] password) {
    ...
    systemAccessControlContext.grantGlobalResourcePermissions(SYS_RESOURCE, resourceClassName, domainName, ResourcePermissions.getInstance(ResourcePermissions.QUERY));
    return systemAccessControlContext.createResource(resourceClassName, domainName, PasswordCredentials.newInstance(password));
  }
  ...
  protected static Resource generateAuthenticatableResourceWithExtId(char[] password, String externalId) {
    ...
    final String domainName = generateDomain();
    systemAccessControlContext.grantGlobalResourcePermissions(SYS_RESOURCE, resourceClassName, domainName, ResourcePermissions.getInstance(ResourcePermissions.QUERY));
    return systemAccessControlContext.createResource(resourceClassName, domainName, externalId, PasswordCredentials.newInstance(password));
  }
  ...
  protected static void grantQueryPermission(Resource accessorResource, Resource queriedResource) {
    systemAccessControlContext.grantResourcePermissions(accessorResource, queriedResource, ResourcePermissions.getInstance(ResourcePermissions.QUERY));
  }
  ...
  protected static void grantResourceCreatePermission(Resource accessorResource, String resourceClassName, String domainName, String... permissionNames) {
    authenticateSystemAccessControlContext();
    ...
    for (String permissionName : uniquePermissionNames) {
      resourceCreatePermissions.add(ResourceCreatePermissions.getInstance(ResourcePermissions.getInstance(permissionName)));
    }
    systemAccessControlContext.setResourceCreatePermissions(accessorResource, resourceClassName, domainName, resourceCreatePermissions);
  }

  protected static void grantResourceCreatePermission(Resource accessorResource, String resourceClassName, String domainName, ResourceCreatePermission firstCreatePermission, ResourceCreatePermission... otherCreatePermissions) {
    authenticateSystemAccessControlContext();
    Set<ResourceCreatePermission> resourceCreatePermissions = new HashSet<>();
