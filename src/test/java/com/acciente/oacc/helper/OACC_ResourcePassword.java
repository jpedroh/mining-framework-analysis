...
public class OACC_ResourcePassword extends DbBase {
  ...
  @Override public boolean equals(Object o) {
    ...
    if (password_plaintext != null && that.password_plaintext != null) {
      if (!password_plaintext.equals(that.password_plaintext)) {
        return false;
      }
    } else {
      if (password_plaintext == null && that.password_plaintext == null) {
      ...
      if (password_plaintext != null) {
        if (that.password == null || !__passwordEncryptor.checkPassword(PasswordUtils.computeBoundPassword(Resources.getInstance(resourceID), password_plaintext), that.password)) {
          return false;
        }
      } else {
        if (that.password_plaintext != null) {
          if (password == null || !__passwordEncryptor.checkPassword(PasswordUtils.computeBoundPassword(Resources.getInstance(that.resourceID), that.password_plaintext), password)) {
            return false;
          }
        }
      }
  ...
  @Override public String toString() {
    return "OACC_Resource{" + "resourceID=" + resourceID + ", password=\'" + password + '\'' + ", password_plaintext=\'" + password_plaintext + '\'' + '}';
  }
  ...
  public static class Finder {
    public static int getNumberOfRows(Connection con, String schemaName) throws SQLException {
      try (PreparedStatement preparedStatement = con.prepareStatement("SELECT COUNT(*) FROM (SELECT DISTINCT " + getPKColumnNames() + " FROM " + getQualifiedTableName(schemaName) + ") T")) {
    ...
    public static OACC_ResourcePassword findByID(Connection con, String dbSchema, int resourceID) throws SQLException {
      try (PreparedStatement preparedStatement = con.prepareStatement("SELECT * FROM " + getQualifiedTableName(dbSchema) + " WHERE ResourceID=?")) {
    ...
    private static OACC_ResourcePassword getDB_Resource(ResultSet resultSet) throws SQLException {
      return new Builder(getLong(resultSet, "resourceID")).password(resultSet.getString("password")).build();
    }
  }
}