...
public class TestSQLAccessControlSystemInitializer {
  ...
  @Test public void initializeOACC() throws SQLException, InterruptedException {
    SQLAccessControlSystemInitializer.initializeOACC(con, TestConfigLoader.getDatabaseSchema(), TestConfigLoader.getOaccRootPassword(), TestConfigLoader.getPasswordEncryptor());
    assertThatOACCIsInInitializedState();
  }

  @Test public void initializeOACC_invalidSchemaName_shouldFail() throws SQLException, InterruptedException {
    try {
      SQLAccessControlSystemInitializer.initializeOACC(con, "oacc.temp;drop database oaccdb;--", TestConfigLoader.getOaccRootPassword(), TestConfigLoader.getPasswordEncryptor());
  ...
  @Test public void reInitializeOACC() throws SQLException, InterruptedException {
    SQLAccessControlSystemInitializer.initializeOACC(con, TestConfigLoader.getDatabaseSchema(), TestConfigLoader.getOaccRootPassword(), TestConfigLoader.getPasswordEncryptor());
    assertThatOACCIsInInitializedState();
    SQLAccessControlSystemInitializer.initializeOACC(con, TestConfigLoader.getDatabaseSchema(), TestConfigLoader.getOaccRootPassword(), TestConfigLoader.getPasswordEncryptor());
    assertThatOACCIsInInitializedState();
  ...
  private void assertThatOACCIsInInitializedState() throws SQLException {
    OACC_Domain sysDomain = new OACC_Domain.Builder(0L).domainName(AccessControlContext.SYSTEM_DOMAIN).build();
    ...
    OACC_ResourceClass sysResourceClass = new OACC_ResourceClass.Builder(0L).resourceClassName(AccessControlContext.SYSTEM_RESOURCE_CLASS).isAuthenticatable(true).isUnauthenticatedCreateAllowed(false).build();
    ...
    OACC_Resource sysResource = new OACC_Resource.Builder(0L).resourceClassID(sysResourceClass.getResourceClassID()).domainID(sysDomain.getDomainID()).build();
    ...
    OACC_ResourcePassword sysResourcePassword = new OACC_ResourcePassword.Builder(0L).password_plaintext(TestConfigLoader.getOaccRootPassword()).build();
    ...
    OACC_Grant_DomPerm_Sys sysDomainSuperUserPermission = new OACC_Grant_DomPerm_Sys.Builder(sysResource.getResourceID(), sysDomain.getDomainID(), sysDomainPermission_SuperUser.getSystemPermissionId()).isWithGrant(true).grantorResourceID(sysResource.getResourceID()).build();
    assertThat(OACC_Grant_DomPerm_Sys.Finder.getNumberOfRows(con, TestConfigLoader.getDatabaseSchema()), is(1));
    final List<OACC_Grant_DomPerm_Sys> permissions = OACC_Grant_DomPerm_Sys.Finder.findByAccessorIDAndAccessedID(con, TestConfigLoader.getDatabaseSchema(), sysResource.getResourceID(), sysDomain.getDomainID());
    assertThat(permissions.size(), is(1));
    assertThat(permissions, hasItems(sysDomainSuperUserPermission));
    final DomainCreatePermission sysDomainCreatePermission_Create = DomainCreatePermissions.getInstance(DomainCreatePermissions.CREATE);
    ...
    OACC_Grant_DomCreatePerm_PostCreate_Sys createSysDomainSuperUserPermission = new OACC_Grant_DomCreatePerm_PostCreate_Sys.Builder(sysResource.getResourceID(), sysDomainPermission_SuperUser.getSystemPermissionId()).postCreateIsWithGrant(true).isWithGrant(true).grantorResourceID(sysResource.getResourceID()).build();
    OACC_Grant_DomCreatePerm_Sys createSysDomainCreatePermission = new OACC_Grant_DomCreatePerm_Sys.Builder(sysResource.getResourceID(), sysDomainCreatePermission_Create.getSystemPermissionId()).isWithGrant(true).grantorResourceID(sysResource.getResourceID()).build();
    OACC_Grant_DomCreatePerm_PostCreate_Sys createSysDomainCreateChildPermission = new OACC_Grant_DomCreatePerm_PostCreate_Sys.Builder(sysResource.getResourceID(), sysDomainPermission_CreateChildDomain.getSystemPermissionId()).postCreateIsWithGrant(true).isWithGrant(true).grantorResourceID(sysResource.getResourceID()).build();
    OACC_Grant_DomCreatePerm_PostCreate_Sys createSysDomainDeletePermission = new OACC_Grant_DomCreatePerm_PostCreate_Sys.Builder(sysResource.getResourceID(), sysDomainPermission_Delete.getSystemPermissionId()).postCreateIsWithGrant(true).isWithGrant(true).grantorResourceID(sysResource.getResourceID()).build();
    assertThat(OACC_Grant_DomCreatePerm_PostCreate_Sys.Finder.getNumberOfRows(con, TestConfigLoader.getDatabaseSchema()), is(3));
    final List<OACC_Grant_DomCreatePerm_PostCreate_Sys> createPostCreatePermissions = OACC_Grant_DomCreatePerm_PostCreate_Sys.Finder.findByAccessorID(con, TestConfigLoader.getDatabaseSchema(), sysResource.getResourceID());
    ...
    final List<OACC_Grant_DomCreatePerm_Sys> createSysPermissions = OACC_Grant_DomCreatePerm_Sys.Finder.findByAccessorID(con, TestConfigLoader.getDatabaseSchema(), sysResource.getResourceID());
