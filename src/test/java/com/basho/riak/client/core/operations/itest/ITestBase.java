...
public abstract class ITestBase {
  ...
  @BeforeClass public static void setUp() throws CertificateException, IOException, KeyStoreException, NoSuchAlgorithmException {
    ...
    RiakNode.Builder builder = new RiakNode.Builder().withRemoteAddress(hostname).withRemotePort(pbcPort).withMinConnections(NUMBER_OF_PARALLEL_REQUESTS);
    if (security) {
  ...
  protected static void resetAndEmptyBucket(Namespace namespace) throws InterruptedException, ExecutionException {
    ...
    ResetBucketPropsOperation.Builder resetOpBuilder = new ResetBucketPropsOperation.Builder(namespace);
  ...
  protected static void setupUsernamePasswordSecurity(RiakNode.Builder builder) throws CertificateException, IOException, KeyStoreException, NoSuchAlgorithmException {
    ...
    if (overrideCert != null) {
      ...
      in = Thread.currentThread().getContextClassLoader().getResourceAsStream("cacert.pem");
    }
  ...
  protected static void setupIndexTestData(Namespace ns, String indexName, String keyBase, String value) throws InterruptedException, ExecutionException {
    for (long i = 0; i < NUMBER_OF_TEST_VALUES; ++i) {
      RiakObject obj = new RiakObject().setValue(BinaryValue.create(value + i)).setContentType("plain/text");
