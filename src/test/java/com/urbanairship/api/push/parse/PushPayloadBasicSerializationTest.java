...
public class PushPayloadBasicSerializationTest {
  ...
  @Test public void testSmsChannel() throws Exception {
    DateTime dateTime = new DateTime(2018, 02, 17, 11, 48, DateTimeZone.UTC);
    SmsPayload smsPayload = SmsPayload.newBuilder().setAlert("sms alert").setExpiry(PushExpiry.newBuilder().setExpiryTimeStamp(dateTime).build()).build();
    PushPayload pushPayload = PushPayload.newBuilder().setNotification(Notifications.notification(smsPayload)).setAudience(SmsSelector.newBuilder().setMsisdn("15552243311").setSender("12345").build()).setDeviceTypes(DeviceTypeData.of(DeviceType.SMS)).build();
    String serializedPayload = mapper.writeValueAsString(pushPayload);
    String expected = "{\"audience\":{\"sms_id\":{\"msisdn\":\"15552243311\",\"sender\":\"12345\"}}," + "\"device_types\":[\"sms\"],\"notification\":{\"sms\":{\"alert\":\"sms alert\"," + "\"expiry\":\"2018-02-17T11:48:00\"}}}";
    JsonNode actualNode = mapper.readTree(serializedPayload);
    JsonNode expectedNode = mapper.readTree(expected);
    assertEquals(expectedNode, actualNode);
  }

  @Test public void testOpenChannel() throws Exception {
    PushPayload pushPayload = PushPayload.newBuilder().setNotification(Notifications.alert("alert")).setAudience(Selectors.open("open_channel")).setDeviceTypes(DeviceTypeData.of(DeviceType.open("sms"))).build();
    String json = "{\n" + "    \"audience\": {\n" + "        \"open_channel\": \"open_channel\"\n" + "    },\n" + "    \"device_types\": [\n" + "        \"open::sms\"\n" + "    ],\n" + "    \"notification\": {\n" + "        \"alert\": \"alert\"\n" + "    }\n" + "}";
    PushPayload secondPush = PushPayload.newBuilder().setNotification(Notifications.alert("alert")).setAudience(Selectors.open("open_channel")).setDeviceTypes(DeviceTypeData.of(DeviceType.open("email"))).build();
  ...
  @Test public void testArrayOfPushes() throws Exception {
    String json = "[" + "  { " + "    \"audience\" : \"all\"," + "    \"device_types\" : [ \"ios\" ]," + "    \"notification\" : { \"alert\" : \"wat\" }" + "  }," + "  { " + "    \"audience\" : \"all\"," + "    \"device_types\" : [ \"ios\" ]," + "    \"notification\" : { \"alert\" : \"derp\" }" + "  }" + "]";
  ...
  @Test public void testSinglePushAsList() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : [ \"ios\" ]," + "  \"notification\" : { \"alert\" : \"wat\" }" + "}";
    List<PushPayload> pushes = mapper.readValue(json, new TypeReference<List<PushPayload>>() { });
    assertNotNull(pushes);
    assertEquals(1, pushes.size());
  }

  @Test public void testNoOptions() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : [ \"ios\" ]," + "  \"notification\" : { \"alert\" : \"wat\" }" + "}";
  ...
  @Test public void testOptions() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : [ \"ios\" ]," + "  \"notification\" : { \"alert\" : \"wat\" }," + "  \"options\" : {" + "  }" + "}";
    PushPayload push = mapper.readValue(json, PushPayload.class);
    assertTrue(push.getPushOptions().isPresent());
  }

  @Test public void testDeviceTypesList() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : [\"ios\", \"android\", \"wns\", \"amazon\"]," + "  \"notification\" : { \"alert\" : \"wat\" }" + "}";
    PushPayload push = mapper.readValue(json, PushPayload.class);
  ...
  @Test(expected = APIParsingException.class) public void testDeviceTypesInvalidString() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : \"ios\"," + "  \"notification\" : { \"alert\" : \"wat\" }" + "}";
    mapper.readValue(json, PushPayload.class);
  }

  @Test(expected = JsonMappingException.class) public void testDeviceTypesAllInList() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : [\"ios\",\"all\"]," + "  \"notification\" : { \"alert\" : \"wat\" }" + "}";
    mapper.readValue(json, PushPayload.class);
  }

  @Test public void testRichPush1() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : [\"ios\"]," + "  \"message\" : { \"title\" : \"T\", \"body\" : \"B\" }" + "}";
  ...
  @Test public void testPlatformOverrides() throws Exception {
    String json = "{" + "  \"audience\" : \"all\"," + "  \"device_types\" : [ \"ios\", \"wns\", \"android\" ]," + "  \"notification\" : { " + "    \"alert\" : \"wat\"," + "    \"ios\" : {" + "      \"alert\" : \"ios\"" + "    }," + "    \"wns\" : {" + "      \"alert\" : \"wns\"" + "    }," + "    \"android\" : {" + "      \"alert\" : \"droid\"" + "    }," + "    \"amazon\" : {" + "      \"alert\" : \"phoenix\"" + "    }" + "  }" + "}";
    PushPayload push = mapper.readValue(json, PushPayload.class);
  ...
  @Test public void testInAppMessage() throws Exception {
    String json = "{" + "\"audience\": \"all\"," + "\"device_types\": [\"ios\", \"android\", \"amazon\"]," + "\"notification\": {" + "\"alert\": \"test alert test test\"" + "}," + "\"in_app\": {" + "\"alert\": \"This part appears in-app!\"," + "\"display_type\": \"banner\"," + "\"display\": {" + "\"position\": \"top\"" + "}" + "}" + "}";
    PushPayload push = mapper.readValue(json, PushPayload.class);
    assertNotNull(push);
  ...
  @Test public void testRoundTrip() throws Exception {
    PushPayload expected = PushPayload.newBuilder().setAudience(Selectors.tag("tag1")).setNotification(Notification.newBuilder().setAlert(RandomStringUtils.randomAlphabetic(10)).build()).setDeviceTypes(DeviceTypeData.newBuilder().addDeviceType(DeviceType.WNS).build()).build();
    String serial = mapper.writeValueAsString(expected);
    PushPayload parsed = mapper.readValue(serial, PushPayload.class);
    assertEquals(expected, parsed);
    expected = PushPayload.newBuilder().setAudience(Selectors.tags("this", "that")).setNotification(Notification.newBuilder().setAlert(RandomStringUtils.randomAlphabetic(10)).build()).setDeviceTypes(DeviceTypeData.newBuilder().addDeviceType(DeviceType.IOS).build()).build();
    serial = mapper.writeValueAsString(expected);
    parsed = mapper.readValue(serial, PushPayload.class);
    assertEquals(expected, parsed);
    expected = PushPayload.newBuilder().setAudience(Selectors.or(Selectors.alias("alias1"), Selectors.tag("tag1"))).setNotification(Notification.newBuilder().setAlert(RandomStringUtils.randomAlphabetic(10)).build()).setDeviceTypes(DeviceTypeData.newBuilder().addDeviceType(DeviceType.ANDROID).build()).build();
    serial = mapper.writeValueAsString(expected);
    parsed = mapper.readValue(serial, PushPayload.class);
    assertEquals(expected, parsed);
    expected = PushPayload.newBuilder().setAudience(Selectors.and(Selectors.tag("tag1"), Selectors.tag("tag2"))).setNotification(Notification.newBuilder().setAlert(RandomStringUtils.randomAlphabetic(10)).build()).setDeviceTypes(DeviceTypeData.newBuilder().addDeviceType(DeviceType.AMAZON).build()).build();
    serial = mapper.writeValueAsString(expected);
    parsed = mapper.readValue(serial, PushPayload.class);
    assertEquals(expected, parsed);
    expected = PushPayload.newBuilder().setAudience(Selectors.or(Selectors.tagWithGroup("tag1", "group1"), Selectors.tagWithGroup("tag2", "group2"))).setNotification(Notification.newBuilder().setAlert(RandomStringUtils.randomAlphabetic(10)).build()).setDeviceTypes(DeviceTypeData.newBuilder().addDeviceType(DeviceType.IOS).build()).build();
    serial = mapper.writeValueAsString(expected);
    parsed = mapper.readValue(serial, PushPayload.class);
    assertEquals(expected, parsed);
  }

  @Test(expected = APIParsingException.class) public void testInvalidDeviceIdentifiers() throws Exception {
    PushPayload payload = PushPayload.newBuilder().setAudience(Selectors.apid("apid1")).setNotification(Notification.newBuilder().setAlert(RandomStringUtils.randomAlphabetic(10)).build()).setDeviceTypes(DeviceTypeData.newBuilder().addDeviceType(DeviceType.WNS).build()).build();
    mapper.readValue(mapper.writeValueAsString(payload), PushPayload.class);
  }

  @Test public void testValidDeviceIdentifiers() throws Exception {
    PushPayload payload = PushPayload.newBuilder().setAudience(Selectors.apid("6de14dab-a4e0-fe5b-06f7-f03b090e4a25")).setNotification(Notification.newBuilder().setAlert(RandomStringUtils.randomAlphabetic(10)).build()).setDeviceTypes(DeviceTypeData.newBuilder().addDeviceType(DeviceType.WNS).build()).build();
    mapper.readValue(mapper.writeValueAsString(payload), PushPayload.class);
  }

  @Test(expected = APIParsingException.class) public void testInvalidJson() throws Exception {
  ...
  @Test(expected = APIParsingException.class) public void testEmptyArray() throws Exception {
    mapper.readValue("[]", PushPayload.class);
  }

  @Test(expected = APIParsingException.class) public void testEmptyObject() throws Exception {
    mapper.readValue("{}", PushPayload.class);
  }
  ...
  @Test(expected = APIParsingException.class) public void testArrayWithSingleObject() throws Exception {
