...
public class SitemapGeneratorTest extends TestCase {
  ...
  public void testAllUrlOptions() throws Exception {
    ...
    WebSitemapUrl url = new WebSitemapUrl.Options("http://www.example.com/index.html").changeFreq(ChangeFreq.DAILY).lastMod(new Date(0)).priority(1.0).build();
    wsg.addUrl(url);
  ...
  public void testGzip() throws Exception {
    wsg = WebSitemapGenerator.builder("http://www.example.com", dir).gzip(true).build();
    for (int i = 0; i < 9; i++) {
      wsg.addUrl("http://www.example.com/" + i);
    }
    wsg.addUrl("http://www.example.com/9");
