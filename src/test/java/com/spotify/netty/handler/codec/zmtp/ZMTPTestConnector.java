...
public abstract class ZMTPTestConnector {
  ...
  public boolean connectAndReceive(final String ip, final int port, final int serverType) {
    ...
    final Bootstrap bootstrap = new Bootstrap().group(group).channel(NioSocketChannel.class).handler(new ChannelInitializer<SocketChannel>() {
...
=======
    bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
      public ChannelPipeline getPipeline() throws Exception {
        return Channels.pipeline(new ZMTP10Codec(new ZMTPSession(ZMTPConnectionType.Addressed, "client".getBytes())), new OneToOneDecoder() {
          @Override protected Object decode(final ChannelHandlerContext ctx, final Channel channel, final Object msg) throws Exception {
            if (onMessage((ZMTPIncomingMessage) msg)) {
              receivedMessage = true;
              channel.close();
            }
...
>>>>>>> /usr/src/app/output/spotify/netty-zmtp/8d02651ebcbfc80ea17699b5547176248b2635c7/src/test/java/com/spotify/netty/handler/codec/zmtp/ZMTPTestConnector.java/right.java
