...
public class CoreTagsTest extends AbstractTest {
  ...
  @Test public void testIfWhenInvalidOrNoEndifTag() throws PebbleException, IOException {
    PebbleEngine pebble = new PebbleEngine.Builder().loader(new StringLoader()).strictVariables(false).build();
  ...
  @Test public void testForWhenInvalidOrNoEndforTag() throws PebbleException, IOException {
    PebbleEngine pebble = new PebbleEngine.Builder().loader(new StringLoader()).strictVariables(false).build();
    String source = "{% for i in \'a\'..5 %}{{i}}% endfor %}";
    try {
      pebble.getTemplate(source);
