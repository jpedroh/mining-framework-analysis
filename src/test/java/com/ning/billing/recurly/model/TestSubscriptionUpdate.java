...
public class TestSubscriptionUpdate extends TestModelBase {
  ...
  @Test(groups = "fast") public void testSerializationWithCustomFields() throws Exception {
    ...
    final String xml = xmlMapper.writeValueAsString(subscription);
    Assert.assertEquals(xml, "<subscription xmlns=\"\">" + "<timeframe>now</timeframe>" + "<unit_amount_in_cents>800</unit_amount_in_cents>" + "<quantity>1</quantity>" + "<custom_fields>" + "<custom_field>" + "<name>name1</name>" + "<value>value1</value>" + "</custom_field>" + "</custom_fields>" + "<plan_code>gold</plan_code>" + "</subscription>");
  }
