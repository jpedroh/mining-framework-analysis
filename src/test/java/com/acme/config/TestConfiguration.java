...
@Configuration @EnableTransactionManagement public class TestConfiguration {
  ...
  @Bean public SharedEntityManagerBean entityManagerBean(EntityManagerFactory entityManagerFactory) {
  ...
  @Bean public StatelessSessionFactoryBean statelessSessionFactoryBean(EntityManagerFactory entityManagerFactory) {
    StatelessSessionFactoryBean factory = new StatelessSessionFactoryBean();
    factory.setEntityManagerFactory(entityManagerFactory);
    return factory;
  }

  @Bean @Qualifier(value = "default") public BulkOperations defaultBulkOperations(EntityManager entityManager) {
    return new DefaultBulkOperations(entityManager);
  }

  @Bean @Qualifier(value = "flushing") public BulkOperations flushingBulkOperations(EntityManager entityManager) {
    return new FlushingBulkOperations(entityManager, batchSize());
  }

  @Bean @Qualifier(value = "statelessSession") public BulkOperations statelessSessionCustomerRepository(EntityManager entityManager, StatelessSession statelessSession) {
    return new StatelessSessionBulkOperations(entityManager, statelessSession);
  }

  @Bean @Qualifier(value = "jdbcTemplate") public BulkOperations jdbcTemplateBulkOperations() {
    return new JdbcTemplateBulkOperations(jdbcTemplate());
  }

  @Bean @Qualifier(value = "jdbi") public BulkOperations jdbiBulkOperations() {
