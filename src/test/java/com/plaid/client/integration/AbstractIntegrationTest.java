...
public abstract class AbstractIntegrationTest {
  ...
  void assertErrorResponse(Response response, ErrorTypeEnum expectedErrorType, String expectedErrorCode) throws Exception {
    assertFalse(response.isSuccessful());
    assertNotNull(response.errorBody());
    try {
      Gson gson = new Gson();
      Error error = gson.fromJson(response.errorBody().string(), java.lang.Error.class);
      assertNotNull(error);
      ...
      throw new Exception(String.format("Failed converting from API Response Error Body to Error %f", response.errorBody().string()));
    }
  }
