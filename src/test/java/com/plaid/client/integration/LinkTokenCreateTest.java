...
public class LinkTokenCreateTest extends AbstractItemIntegrationTest {
  @Override protected List<Products> setupItemProducts() {
    return Collections.singletonList(Products.AUTH);
  }

  @Override protected String setupItemInstitutionId() {
  ...
  @Test public void testSuccess_full() throws Exception {
    String clientUserId = Long.toString((new Date()).getTime());
  ...
  @Test public void testSuccess_depositSwitch() throws Exception {
    HashMap<String, String> userAuth = new HashMap<>();
    userAuth.put("user_id", "user_good");
    userAuth.put("auth_token", "pass_good");
    Response<ItemImportResponse> itemImportResponse = client().service().itemImport(new ItemImportRequest(Arrays.asList("identity", "auth"), userAuth).withWebhook("https://some.webhook.example.com")).execute();
    ...
    Response<AccountsGetResponse> accountsGetResponse = client().service().accountsGet(new AccountsGetRequest(accessToken)).execute();
    ...
    Response<DepositSwitchCreateResponse> depositSwitchCreateResponse = client().service().depositSwitchCreate(new DepositSwitchCreateRequest(accountId, accessToken)).execute();
    ...
    LinkTokenCreateRequest request = new LinkTokenCreateRequest(user, "very nice client name", Collections.singletonList("deposit_switch"), Collections.singletonList("US"), "en").withDepositSwitch(depositSwitch);
    Response<LinkTokenCreateResponse> response = client().service().linkTokenCreate(request).execute();
    assertSuccessResponse(response);
    assertNotNull(response.body().getLinkToken());
