...
public class NumberTest extends AbstractFakerTest {
  ...
  @Test public void testRandomDigit() {
  ...
  @Test public void testRandomDigitNotZero() {
    Set<Integer> nums = Sets.newHashSet();
    for (int i = 0; i < 1000; ++i) {
  ...
  @Test public void testRandomNumber() {
    long value = faker.number().randomNumber();
    assertThat(value, is(lessThan(Long.MAX_VALUE)));
  }

  @Test public void testRandomNumberWithSingleDigitStrict() {
    for (int i = 0; i < 100; ++i) {
  ...
  @Test public void testRandomNumberWithZeroDigitsStrict() {
    for (int i = 0; i < 100; ++i) {
  ...
  @Test public void testRandomNumberWithGivenDigitsStrict() {
    for (int i = 1; i < 9; ++i) {
      for (int x = 0; x < 100; ++x) {
  ...
  @Test public void testNumberBetween() {
    for (int i = 1; i < 100; ++i) {
  ...
  @Test @Repeat(times = 100) public void testLongNumberBetweenRepeated() {
    long low = 1;
    long high = 10;
  ...
  @Test @Repeat(times = 100) public void testIntNumberBetweenRepeated() {
    int low = 1;
    int high = 10;
  ...
  @Test public void testNumberBetweenOneAndThree() {
    Set<Integer> nums = Sets.newHashSet();
  ...
  @Test public void testLongBetweenOneAndThree() {
  ...
  @Test public void numberBetweenIntIntZeroMinMax() {
    assertThat("Calling numberBetween with min==max yields min, with 0", faker.number().numberBetween(0, 0), is(0));
    assertThat("Calling numberBetween with min==max yields min", faker.number().numberBetween(2, 2), is(2));
  }

  @Test public void numberBetweenLongLongZeroMinMax() {
    assertThat("Calling numberBetween with min==max yields min, with 0", faker.number().numberBetween(0L, 0L), is(0L));
    assertThat("Calling numberBetween with min==max yields min", faker.number().numberBetween(2L, 2L), is(2L));
  }
  ...
  @Test public void randomDoubleRandomizationQuality() {
    ...
    assertThat("Percentage of runs > 80% unique is gte 90%", percentGreaterThan80Percent, greaterThanOrEqualTo(percentRunsGtUniquePercentage));
    ...
    assertThat("Percentage of extreme runs > 80%", extremeRunUniquePercent, greaterThanOrEqualTo(individualRunGtPercentUnique));
  }
  ...
  /**
     ...
     * This isn't perfect but it ensures a pretty good degree of uniqueness in the random number generation.
     */
  @Test public void numberBetweenIntIntRandomizationQuality() {
    Function<Pair<Long, Long>, Double> minMaxRangeToUniquePercentageFunction = new Function<Pair<Long, Long>, Double>() {
      @Override public Double apply(Pair<Long, Long> minMax) {
  ...
  /**
     ...
     * This isn't perfect but it ensures a pretty good degree of uniqueness in the random number generation.
     */
  @Test public void numberBetweenLongLongRandomizationQuality() {
    Function<Pair<Long, Long>, Double> minMaxRangeToUniquePercentageFunction = new Function<Pair<Long, Long>, Double>() {
      @Override public Double apply(Pair<Long, Long> minMax) {
  ...
  /**
     ...
     */
  private double randomizationQualityTest(final double threshold, final Function<Pair<Long, Long>, Double> percentUniqueRunner) {
  ...
  /**
     ...
     */
  private <T extends java.lang.Object> double uniquePercentageOfResults(long iterations, Callable<T> callable) {
    try {
