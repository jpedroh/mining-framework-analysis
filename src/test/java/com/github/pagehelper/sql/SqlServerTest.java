...
public class SqlServerTest {
  ...
  @Test public void testSqlTestWithlock() {
    String originalSql = "select * from Agency with (NOLOCK) where status=0 order by CreateTime";
    Assert.assertEquals("SELECT TOP 10 * FROM (SELECT ROW_NUMBER() OVER (ORDER BY CreateTime) PAGE_ROW_NUMBER, * FROM (SELECT * FROM Agency WITH (NOLOCK) WHERE status = 0) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlTest() {
    String originalSql = "Select * from user o where id > 10 order by id desc , name asc";
    Assert.assertEquals("SELECT TOP 10 * FROM (SELECT ROW_NUMBER() OVER (ORDER BY id DESC, name ASC) PAGE_ROW_NUMBER, * FROM (SELECT * FROM user o WHERE id > 10) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlAlias() {
    String originalSql = "Select o.* from user o where id > 10 order by id desc , name asc";
    Assert.assertEquals("SELECT TOP 10 * FROM (SELECT ROW_NUMBER() OVER (ORDER BY id DESC, name ASC) PAGE_ROW_NUMBER, * FROM (SELECT o.* FROM user o WHERE id > 10) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlOrderByAlias() {
    String originalSql = "select py code from user order by code";
    Assert.assertEquals("SELECT TOP 10 code FROM (SELECT ROW_NUMBER() OVER (ORDER BY code) PAGE_ROW_NUMBER, code FROM (SELECT py code FROM user) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlDistinct() {
    String originalSql = "select distinct py,name from user order by py";
    Assert.assertEquals("SELECT TOP 10 py, name FROM (SELECT ROW_NUMBER() OVER (ORDER BY py) PAGE_ROW_NUMBER, py, name FROM (SELECT DISTINCT py, name FROM user) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlTableAll() {
    String originalSql = "Select user.* from user where id > 10 order by id desc , name asc";
    Assert.assertEquals("SELECT TOP 10 * FROM (SELECT ROW_NUMBER() OVER (ORDER BY id DESC, name ASC) PAGE_ROW_NUMBER, * FROM (SELECT user.* FROM user WHERE id > 10) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlAs() {
    String originalSql = "Select id as id,name name,py as code from user o where id > 10 order by id desc , name asc";
    Assert.assertEquals("SELECT TOP 10 id, name, code FROM (SELECT ROW_NUMBER() OVER (ORDER BY id DESC, name ASC) PAGE_ROW_NUMBER, id, name, code FROM (SELECT id AS id, name name, py AS code FROM user o WHERE id > 10) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSelectParameter() {
    String originalSql = "Select id as id,? name,? from user o where id > 10 order by id desc , name asc";
    Assert.assertEquals("SELECT TOP 10 id, name, ? FROM (SELECT ROW_NUMBER() OVER (ORDER BY id DESC, name ASC) PAGE_ROW_NUMBER, id, name, ? FROM (SELECT id AS id, ? name, ? FROM user o WHERE id > 10) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlWith() {
    String originalSql = "with cr as " + " (select id " + "    from user " + "   where id > 100 " + "     and id < 120) " + "select id, name, py " + "  from user " + " where id in (select * from cr) order by id";
  ...
  @Test public void testSqlUnion() {
    String originalSql = "select name,py code from user where id >170 " + "union all " + "select name,py code from user where id < 10 order by code";
    Assert.assertEquals("SELECT TOP 10 name, code FROM (SELECT ROW_NUMBER() OVER (ORDER BY RAND()) PAGE_ROW_NUMBER, name, code FROM (SELECT name, code FROM (SELECT name, py code FROM user WHERE id > 170 UNION ALL SELECT name, py code FROM user WHERE id < 10 ORDER BY code) AS WRAP_OUTER_TABLE) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlUnion2() {
    String originalSql = "select name,code from ( " + "\tselect name,py code from user where id >170 " + "\tunion all " + "\tselect name,py code from user where id < 10 " + ") as temp " + "order by code";
  ...
  @Test public void testSqlOrderByFunctionAlias() {
    String originalSql = "select py code, func() func_alias from user order by func()";
    Assert.assertEquals("SELECT TOP 10 code, func_alias FROM (SELECT ROW_NUMBER() OVER (ORDER BY func_alias) PAGE_ROW_NUMBER, code, func_alias FROM (SELECT py code, func() func_alias FROM user) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlOrderByUnknown() {
    String originalSql = "select name from user order by py";
    Assert.assertEquals("SELECT TOP 10 name FROM (SELECT ROW_NUMBER() OVER (ORDER BY ROW_ALIAS_1) PAGE_ROW_NUMBER, name FROM (SELECT name, py AS ROW_ALIAS_1 FROM user) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlOrderByTable() {
    String originalSql = "select t.py, t.name from user t order by t.py";
    Assert.assertEquals("SELECT TOP 10 py, name FROM (SELECT ROW_NUMBER() OVER (ORDER BY py) PAGE_ROW_NUMBER, py, name FROM (SELECT t.py, t.name FROM user t) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSqlStar() {
    String originalSql = "select t.*, 1 alias from user t order by t.py";
    Assert.assertEquals("SELECT TOP 10 * FROM (SELECT ROW_NUMBER() OVER (ORDER BY py) PAGE_ROW_NUMBER, * FROM (SELECT t.*, 1 alias FROM user t) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSql377() {
  ...
  @Test public void testSql354() {
    String originalSql = "SELECT ISNULL(tb.a, \'\') from table tb";
    Assert.assertEquals("SELECT TOP 10 ISNULL(tb.a, \'\') FROM (SELECT ROW_NUMBER() OVER (ORDER BY RAND()) PAGE_ROW_NUMBER, ISNULL(tb.a, \'\') FROM (SELECT ISNULL(tb.a, \'\') FROM table tb) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSql374() {
    String originalSql = "select * from table_a order by id";
    Assert.assertEquals("SELECT TOP 10 * FROM (SELECT ROW_NUMBER() OVER (ORDER BY id) PAGE_ROW_NUMBER, * FROM (SELECT * FROM table_a) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", sqlServer.convertToPageSql(originalSql, 1, 10));
  }

  @Test public void testSql345() {
  ...
  @Test public void testSql66() {
    ...
    Assert.assertEquals("SELECT count(0) FROM forum_post_info a WITH(NOLOCK) LEFT JOIN forum_carcase_tags AS b WITH(NOLOCK) ON a.id = b.carcase_id WHERE b.tag_id = 127", smartCountSql);
    String result = replaceSql.restore(pageSql);
    Assert.assertEquals("SELECT TOP 10 * FROM (SELECT ROW_NUMBER() OVER (ORDER BY RAND()) PAGE_ROW_NUMBER, * FROM (SELECT * FROM forum_post_info a WITH(NOLOCK) LEFT JOIN forum_carcase_tags AS b WITH(NOLOCK) ON a.id = b.carcase_id WHERE b.tag_id = 127) AS PAGE_TABLE_ALIAS) AS PAGE_TABLE_ALIAS WHERE PAGE_ROW_NUMBER > 1 ORDER BY PAGE_ROW_NUMBER", result);
  }

  @Test public void testSql398() {
