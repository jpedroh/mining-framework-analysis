 ...
 */
@ExtendWith(value = { WeldJunit5Extension.class }) public abstract class BitOutputTest<T extends BitOutput> {
  ...
  /**
     ...
     */
  @RepeatedTest(value = 8) public void testWriteBoolean() throws IOException {
    bitOutput.writeBoolean(current().nextBoolean());
  }

  @RepeatedTest(value = 8) public void testWriteByte() {
  ...
  @RepeatedTest(value = 8) public void testWriteShort() throws IOException {
  ...
  @RepeatedTest(value = 8) public void testWriteInt() throws IOException {
    final boolean unsigned = current().nextBoolean();
  ...
  @RepeatedTest(value = 8) public void testWriteLong() throws IOException {
    final boolean unsigned = current().nextBoolean();
  ...
  @RepeatedTest(value = 8) public void testWriteChar() throws IOException {
    final int size = current().nextInt(1, Character.SIZE + 1);
    bitOutput.writeChar(size, (char) current().nextInt());
  }

  @Test public void testAlignAssertThrowsIllegalArgumentExceptionWhenBytesIsLessThanOrEqualsToZero() {
  ...
  @Inject private Instance<BitOutput> bitOutputInstance;
