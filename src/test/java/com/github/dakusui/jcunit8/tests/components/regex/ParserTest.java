...
@RunWith(value = Parameterized.class) public class ParserTest extends PipelineTestBase {
  ...
  @Parameterized.Parameters public static String[] data() {
    return new String[] { "(A){0,1}abc(B){0,1};abc,Aabc,abcB,AabcB;(*(*A){0,1}abc(*B){0,1})", "(A){0,1}abc(B){1,2};abcB,AabcB,abcBB,AabcBB;(*(*A){0,1}abc(*B){1,2})", "A(B|C);AB,AC;(*A(+BC))", "(A)(B|C);AB,AC;(*(*A)(+BC))", "(A)(B|C){0,1};A,AB,AC;(*(*A)(+BC){0,1})", "A(B|C){0,1};A,AB,AC;(*A(+BC){0,1})", "(A(B|C));AB,AC;(*(*A(+BC)))", "(A(B|C)){0,1};,AB,AC;(*(*A(+BC)){0,1})", "A(B(C){0,1}){0,1};A,AB,ABC;(*A(*B(*C){0,1}){0,1})", "(AB){0,1};,AB;(*(*AB){0,1})", "(A)(B){0,1};A,AB;(*(*A)(*B){0,1})", "A(B);AB;(*A(*B))", "A{0,2};,A,AA;(*A{0,2})", "(git)(clone)(URL);gitcloneURL;(*(*git)(*clone)(*URL))", "git;git;(*git)", "(A|B){2,3};BBA,AA,BBB,BB,AAA,AAB;(*(+AB){2,3})", "A{1,2};A,AA;(*A{1,2})", "(A|B){0,1};,A,B;(*(+AB){0,1})", "A{0,1}B{0,1};,A,B,AB;(*A{0,1}B{0,1})", "A;A;(*A)", "(A);A;(*(*A))", "(A)(A);AA;(*(*A)(*A))", "(A|B);A,B;(*(+AB))", "(A|B|C);A,B,C;(*(+ABC))", "(A|B|(C(D{0,1})));A,B,CD,C;(*(+AB(*C(*D{0,1}))))", "((A{0,1})|B|(C{0,1}));,A,B,C;(*(+(*A{0,1})B(*C{0,1})))", "((A{0,1})|B|(C{0,1}D{0,1}));,A,B,C,CD,D;(*(+(*A{0,1})B(*C{0,1}D{0,1})))", "A|B|C;A,B,C;(+ABC)", "(A)|(B)|(C);A,B,C;(+(*A)(*B)(*C))", "git clone  URL(dir){0,1};gitcloneURL,gitcloneURLdir;(*git clone  URL(*dir){0,1})", "(A|B){0,2};,AA,BB,AB,A,B,BA;(*(+AB){0,2})", "(A|B){1,3};ABA,AA,BB,A,AAA,AB,AAB,B,BAA,BBB,BA;(*(+AB){1,3})" };
  }
  ...
  @Test public void parseTreePrintingWithId() {
  ...
  @Test public void parseTreePrintingWithName() {
    com.github.dakusui.jcunit8.core.Utils.out().println("input expression:" + input());
    new Parser().parse(input()).accept(new RegexTestUtils.ExprTreePrinter(NAME_FORMATTER));
  }

  @SuppressWarnings(value = { "unchecked" }) @Test public void printGeneratedList() {
    ...
    for (TestCase each : builtTestSuite) {
      generatedStringsFromRegex.add(StringUtils.join("", (List) each.get().get("input")));
    }
  ...
  @Test public void testInterpret() {
  ...
  @SuppressWarnings(value = { "unchecked" }) private Matcher<Set<String>> generatedStringsMatcher() {
    return CoreMatchers.allOf(CoreMatchers.is(possibleStrings(expectationForGeneratedStrings())));
  }
