...
public abstract class JoinExperimentBase {
  ...
  protected void exerciseJoin(FactorSpace rhsFactorSpace, int times) {
    List<Tuple> rhs = generateWithIpoGplus(rhsFactorSpace, strength());
    assertCoveringArray(exerciseJoin(rhs), CoveringArrayGenerationUtils.mergeFactorSpaces(lhsFactorSpace, rhsFactorSpace), strength());
    for (int i = 0; i < times; i++) {
      CoveringArrayGenerationUtils.StopWatch stopWatch = new CoveringArrayGenerationUtils.StopWatch();
      System.out.printf("%s%n", new Report(Integer.toString(lhsFactorSpace.getFactorNames().size()), Integer.toString(rhsFactorSpace.getFactorNames().size()), exerciseJoin(rhs).size(), stopWatch.get()));
    }
  }
  ...
  private List<Tuple> exerciseJoin(List<Tuple> rhs) {
    return CoveringArrayGenerationUtils.join(lhs, rhs, strength());
  }
}