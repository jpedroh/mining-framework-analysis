...
public enum CoveringArrayGenerationUtils {
  ...
  public static FactorSpace mergeFactorSpaces(FactorSpace lhsFactorSpace, FactorSpace rhsFactorSpace) {
    return FactorSpace.create(Stream.concat(lhsFactorSpace.getFactors().stream(), rhsFactorSpace.getFactors().stream()).collect(toList()), Stream.concat(lhsFactorSpace.getConstraints().stream(), rhsFactorSpace.getConstraints().stream()).collect(toList()));
  }

  public static FactorSpace createFactorSpace(String prefix, int numLevels, int numFactors) {
    return toFactorSpace(parameterSpace(IntStream.range(0, numFactors).mapToObj((i) -> String.format("%s-%02d", prefix, i)).map((name) -> p(name, IntStream.range(0, numLevels).mapToObj((i) -> i).collect(toList()).toArray())).collect(toList()), Collections.emptyList()));
  }

  public static ParameterSpace parameterSpace(List<Parameter> parameters, List<Constraint> constraints) {
    return new ParameterSpace.Builder().addAllParameters(parameters).addAllConstraints(constraints).build();
  }
  ...
  public static FactorSpace toFactorSpace(ParameterSpace parameterSpace) {
    return FactorSpace.create(parameterSpace.getParameterNames().stream().map(parameterSpace::getParameter).map((parameter) -> Factor.create(parameter.getName(), parameter.getKnownValues().toArray())).collect(toList()), parameterSpace.getConstraints());
  }

  public static List<Tuple> generateWithIpoGplus(FactorSpace factorSpace, int strength) {
    return new IpoGplus(factorSpace, new Requirement.Builder().withStrength(strength).build(), Collections.emptyList()).generate();
  }

  public static List<Tuple> join(List<Tuple> lhs, List<Tuple> rhs, int strength) {
    return new Joiner.Standard(new Requirement.Builder().withStrength(strength).build()).apply(SchemafulTupleSet.fromTuples(lhs), SchemafulTupleSet.fromTuples(rhs));
  }

  public static List<Tuple> coveredTuples(int strength, Collection<Tuple> in) {
    return Utils.unique(in.stream().flatMap((Tuple i) -> subtuplesOf(i, strength).stream()).collect(toList()));
  }

  public static <T extends java.lang.Object> List<T> subtract(Collection<T> a, Collection<T> b) {
    return a.stream().filter((eachInA) -> !b.contains(eachInA)).collect(toList());
  }

  public static Optional<Tuple> findAllowedSuperTupleFor(Tuple tuple, ParameterSpace parameterSpace) {
    List<String> allKeysUsedByConstraintsButNotInTuple = parameterSpace.getConstraints().stream().flatMap((constraint) -> constraint.involvedKeys().stream()).filter((key) -> !tuple.containsKey(key)).distinct().collect(toList());
    if (allKeysUsedByConstraintsButNotInTuple.isEmpty()) {
      return parameterSpace.getConstraints().stream().allMatch((constraint) -> constraint.test(tuple)) ? Optional.of(tuple) : Optional.empty();
    }
    return new StreamableCartesianator<>(allKeysUsedByConstraintsButNotInTuple.stream().map((key) -> parameterSpace.getParameter(key).getKnownValues()).collect(toList())).stream().map((values) -> {
      ...
      tuple.keySet().forEach((key) -> builder.put(key, tuple.get(key)));
      return builder.build();
    }).filter((t) -> parameterSpace.getConstraints().stream().allMatch((each) -> each.test(t))).findFirst();
  }
  ...
  public static List<Tuple> allPossibleTuplesInFactors(int strength, List<Factor> factors) {
    return allPossibleTuples(strength, factors.stream().map((f) -> p(f.getName(), f.getLevels().toArray())).collect(toList()));
  }
  ...
  public static List<Tuple> allPossibleTuples(int strength, Parameter... parameters) {
    return StreamSupport.stream(new Combinator<>(asList(parameters), strength).spliterator(), false).flatMap((chosenParameters) -> new StreamableCartesianator<>(convertParameters(chosenParameters)).stream().map((chosenValues) -> new Tuple.Builder() {
      {
        for (int i = 0; i < chosenParameters.size(); i++) {
          put(chosenParameters.get(i).getName(), chosenValues.get(i));
        }
      }
    }.build())).distinct().collect(toList());
  }

  private static List<List<Object>> convertParameters(List<Parameter> parameters) {
    return parameters.stream().map((Function<Parameter, List<Object>>) Parameter::getKnownValues).collect(toList());
  }
  ...
  static TestSuite buildTestSuite(int strength, List<Parameter> parameters, List<Constraint> constraints) {
    ...
    }.setStrength(strength).buildTestSuite();
  }
  ...
  static <I extends java.lang.Object> AsBoolean<? super I> failsIf(boolean condition) {
    return asBoolean(Printable.function("debugMode", (Function<I, Boolean>) ((t) -> !condition))).isTrue();
  }

  public static void assertCoveringArray(List<Tuple> coveringArray, FactorSpace factorSpace, int strength) {
    assertThat(coveringArray, asListOf(Tuple.class, Printable.function("coveredTuples", (List<Tuple> ca) -> coveredTuples(strength, ca))).containsAll(allPossibleTuplesInFactors(strength, factorSpace.getFactors())).$());
  }
  ...
  public static class TestSuiteBuilder {
    ...
    public TestSuite buildTestSuite() {
      return new Pipeline.Standard().execute(new Config.Builder(new Requirement.Builder() {
        {
          seeds.forEach(TestSuiteBuilder.this::addSeed);
        }
      }.withStrength(this.strength).build()).build(), parameterSpace(parameters, constraints));
    }
  }
