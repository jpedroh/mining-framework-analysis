...
public class StateMachineTests {
  final Enum StateA = State.A, StateB = State.B, StateC = State.C, TriggerX = Trigger.X, TriggerY = Trigger.Y;
  ...
  @Test public void CanUseReferenceTypeMarkers() {
    RunSimpleTest(new Enum[] { StateA, StateB, StateC }, new Enum[] { TriggerX, TriggerY });
  }

  @Test public void CanUseValueTypeMarkers() {
  ...
  @Test public void InitialStateIsCurrent() {
  ...
  @Test public void SubstateIsIncludedInCurrentState() {
  ...
  @Test public void WhenInSubstate_TriggerIgnoredInSuperstate_RemainsInSubstate() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
    sm.configure(State.B).substateOf(State.C);
  ...
  @Test public void PermittedTriggersIncludeSuperstatePermittedTriggers() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
  ...
  @Test public void PermittedTriggersAreDistinctValues() {
  ...
  @Test public void AcceptedTriggersRespectGuards() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
    sm.configure(State.B).permitIf(Trigger.X, State.A, new FuncBoolean() {
  ...
  @Test public void WhenDiscriminatedByGuard_ChoosesPermitedTransition() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
  ...
  @Test public void WhenTriggerIsIgnored_ActionsNotExecuted() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
    fired = false;
  ...
  @Test public void IfSelfTransitionPermited_ActionsFire() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
    fired = false;
    sm.configure(State.B).onEntry(new Action() {
      public void doIt() {
  ...
  @Test(expected = IllegalStateException.class) public void ImplicitReentryIsDisallowed() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
    sm.configure(State.B).permit(Trigger.X, State.B);
  }

  @Test(expected = IllegalStateException.class) public void TriggerParametersAreImmutableOnceSet() {
    StateMachine<State, Trigger> sm = new StateMachine<State, Trigger>(State.B);
