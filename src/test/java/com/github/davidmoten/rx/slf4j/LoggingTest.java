...
public class LoggingTest {
	...
	public void testCountEvery() {
		...
		int count = Observable
				.range(1, 6)
				// log all
				.lift(logger(logger).name("rx.Server")
						.prefix("count every test").excludeValue()
						.subscribed(Level.DEBUG).onCompleted(Level.DEBUG)
						.showCount("files").every(2).log())
				// count
				.count().toBlocking().single();
	...
	public void testLoggingTransformationErrorIsLogged() {
		...
		Observable
				.just(1)
				.lift(Logging.<Integer> logger(logger).showValue()
						.value(new Func1<Integer, Integer>() {
		...
		verify(logger).debug("onSubscribe");
		verify(logger).error(
				"the logging transformations through an exception: problem",
				exception);
		verify(logger).debug("onUnsubscribe");
		verifyNoMoreInteractions(logger);
