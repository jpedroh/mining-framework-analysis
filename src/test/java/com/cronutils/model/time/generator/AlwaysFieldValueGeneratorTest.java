...
public class AlwaysFieldValueGeneratorTest {
  ...
  @Before public void setUp() {
    fieldValueGenerator = new AlwaysFieldValueGenerator(new CronField(CronFieldName.HOUR, FieldExpression.always(), FieldConstraintsBuilder.instance().createConstraintsInstance()));
  }

  @Test public void testGenerateNextValue() throws NoSuchValueException {
    for (int j = 0; j < 10; j++) {
      assertEquals(j + 1L, fieldValueGenerator.generateNextValue(j));
    }
  }

  @Test public void testGeneratePreviousValue() throws NoSuchValueException {
    for (int j = 1; j < 10; j++) {
      assertEquals(j - 1L, fieldValueGenerator.generatePreviousValue(j));
    }
  }

  @Test public void testGenerateCandidatesNotIncludingIntervalExtremes() {
  ...
  @Test public void testMatchesFieldExpressionClass() {
  ...
  @Test(expected = IllegalArgumentException.class) public void testConstructorNotMatchesAlways() {
