...
@RunWith(value = MockitoJUnitRunner.class) public class ShutdownTaskTest {
  ...
  ShardInfo defaultShardInfo = new ShardInfo(defaultShardId, defaultConcurrencyToken, defaultParentShardIds, ExtendedSequenceNumber.LATEST);

  IRecordProcessor defaultRecordProcessor = new TestStreamlet();

  @Mock private GetRecordsCache getRecordsCache;
  ...
  @Test public final void testCallWhenApplicationDoesNotCheckpoint() {
    ...
    ShutdownTask task = new ShutdownTask(defaultShardInfo, defaultRecordProcessor, checkpointer, ShutdownReason.TERMINATE, kinesisProxy, INITIAL_POSITION_TRIM_HORIZON, cleanupLeasesOfCompletedShards, ignoreUnexpectedChildShards, leaseManager, TASK_BACKOFF_TIME_MILLIS, getRecordsCache);
  ...
  /**
     * Test method for {@link ShutdownTask#call()}.
     */
  @Test public final void testCallWhenSyncingShardsThrows() {
    RecordProcessorCheckpointer checkpointer = mock(RecordProcessorCheckpointer.class);
