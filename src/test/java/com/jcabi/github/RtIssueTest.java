...
public final class RtIssueTest {
  /**
     ...
     */
  @Rule public final transient RandomPort resource = new RandomPort();
  ...
  @Test public void patchWithJson() throws Exception {
    try (MkContainer container = new MkGrizzlyContainer().next(new MkAnswer.Simple(HttpURLConnection.HTTP_OK, "response")).start(this.resource.port())) {
  ...
  /**
     ...
     */
  @Test public void reacts() throws Exception {
    try (MkContainer container = new MkGrizzlyContainer().next(new MkAnswer.Simple(HttpURLConnection.HTTP_OK, "")).start(this.resource.port())) {
      final Repo repo = new MkGithub().randomRepo();
      final Issue issue = new RtIssue(new ApacheRequest(container.home()), repo, 10);
      issue.react(new Reaction.Simple(Reaction.HEART));
      final MkQuery query = container.take();
      MatcherAssert.assertThat("Issue was unable to react", query.method(), new IsEqual<>(Request.POST));
    }
  }
