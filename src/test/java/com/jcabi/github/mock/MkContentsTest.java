 ...
 */
@SuppressWarnings(value = { "PMD.TooManyMethods", "PMD.AvoidDuplicateLiterals" }) public final class MkContentsTest {
  /**
  ...
  /**
     ...
     * @throws Exception if some problem inside
     */
  @Test public void canFetchReadmeFromBranch() throws Exception {
  ...
  /**
     ...
     * @throws Exception if some problem inside
     */
  @Test public void canCreateFile() throws Exception {
    final String path = "file.txt";
    final Content.Smart content = new Content.Smart(this.createFile(MkContentsTest.repo(), path));
    MatcherAssert.assertThat(content.path(), Matchers.is(path));
    MatcherAssert.assertThat(content.name(), Matchers.is(path));
    MatcherAssert.assertThat(content.sha(), Matchers.not(Matchers.isEmptyOrNullString()));
  }
  ...
  /**
     ...
     * @throws Exception if some problem inside
     */
  @Test public void canCreateFileInSomeBranch() throws Exception {
    final String path = "file-in-branch.txt";
    final String branch = "branch-2";
    final String body = "some file";
    final Content.Smart content = new Content.Smart(MkContentsTest.repo().contents().create(content(path, "some file", body).add("ref", branch).build()));
    MatcherAssert.assertThat(content.path(), Matchers.is(path));
    MatcherAssert.assertThat(content.name(), Matchers.is(path));
    MatcherAssert.assertThat(content.sha(), Matchers.not(Matchers.isEmptyOrNullString()));
    MatcherAssert.assertThat(content.content(), Matchers.is(body));
  }
  ...
  /**
     ...
     * @throws Exception if some problem inside
     */
  @Test public void canRemoveFile() throws Exception {
  ...
  /**
     ...
     * @throws Exception if some problem inside
     */
  @Test public void canRemoveFileFromBranch() throws Exception {
    final String branch = "branch-1";
    final Repo repo = MkContentsTest.repo();
    final String path = "removeme.txt";
    this.createFile(repo, path);
  ...
  /**
     ...
     */
  @Test public void updatesFile() throws Exception {
    final String path = "file.txt";
  ...
  /**
     ...
     */
  @Test public void updatesFileCreateCommit() throws Exception {
  ...
  /**
     ...
     */
  @Test public void updateContent() throws Exception {
    ...
    final JsonObject jsonPatch = MkContentsTest.content(path, message, updated).add("ref", branch).build();
  ...
  /**
     ...
     * @throws Exception if any problem inside
     */
  @Test public void getContentFromDefaultBranch() throws Exception {
  ...
  /**
     ...
     */
  @Test public void canIterate() throws IOException {
    final MkStorage storage = new MkStorage.InFile();
  ...
  /**
     ...
     */
  private Content[] addContent(final Repo repo, final String... paths) throws IOException {
    final Content[] result = new Content[paths.length];
    int index = 0;
    for (final String path : paths) {
      result[index] = repo.contents().create(Json.createObjectBuilder().add("ref", "ref-1").add("path", path).add("content", path).add("message", "msg").build());
      index += 1;
    }
    return result;
  }
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  private static JsonObjectBuilder content(final String path, final String message) throws Exception {
    return Json.createObjectBuilder().add("path", path).add("message", message);
  }
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  private static JsonObjectBuilder content(final String path, final String message, final String content) throws Exception {
    return Json.createObjectBuilder().add("path", path).add("message", message).add("content", content);
  }
  ...
  private static JsonObjectBuilder committer() {
    return Json.createObjectBuilder().add("name", "joe").add("email", "joe@contents.com");
  }
  ...
  private static Repo repo(final MkStorage storage) throws IOException {
    ...
    return new MkGithub(storage, login).repos().create(Json.createObjectBuilder().add("name", login).build());
  }
}