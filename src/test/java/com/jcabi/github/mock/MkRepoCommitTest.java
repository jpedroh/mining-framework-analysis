...
public final class MkRepoCommitTest {
  /**
     * The fist test key.
     */
  private static final String SHA1 = "6dcb09b5b57875f334f61aebed695e2e4193db5e";

  /**
     * The second test key.
     */
  private static final String SHA2 = "51cabb8e759852a6a40a7a2a76ef0afd4beef96d";

  /**
     * MkRepoCommit can return repository.
     * @throws IOException If some problem inside
     */
  @Test public void getRepo() throws IOException {
  ...
  /**
     ...
     * @throws IOException If some problem inside
     */
  @Test public void getSha() throws IOException {
    final MkStorage storage = new MkStorage.InFile();
    MatcherAssert.assertThat(new MkRepoCommit(storage, this.repo(storage), SHA2).sha(), Matchers.equalTo(SHA2));
  }
  ...
  /**
     ...
     */
  @Test @Ignore public void canCompareInstances() throws Exception {
    final MkStorage storage = new MkStorage.InFile();
  ...
  /**
     ...
     */
  @Test public void compareEqual() throws Exception {
  ...
  /**
     ...
     */
  @Test @Ignore public void canGetJson() throws Exception {
    final MkStorage storage = new MkStorage.InFile();
    storage.apply(new Directives().xpath("/github").add("repos").add("repo").attr("coords", "test_login/test_repo").add("commits").add("commit").attr("sha", SHA1).set("Hello world"));
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void compareDifferent() throws Exception {
    final MkStorage storage = new MkStorage.InFile();
    final Repo repo = this.repo(storage);
  ...
  private Repo repo(final MkStorage storage) throws IOException {
    ...
    return new MkRepo(storage, login, new Coordinates.Simple(login, "test_repo"));
  }
}