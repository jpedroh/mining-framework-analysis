...
public final class IssueTest {
  ...
  /**
     ...
     */
  @Test public void fetchesProperties() throws Exception {
    final Issue issue = Mockito.mock(Issue.class);
    Mockito.doReturn(Json.createObjectBuilder().add("title", "this is some text \u20ac").add("body", "body of the issue").build()).when(issue).json();
    final Issue.Smart smart = new Issue.Smart(issue);
    MatcherAssert.assertThat(smart.title(), Matchers.notNullValue());
    MatcherAssert.assertThat(smart.body(), Matchers.notNullValue());
  }
  ...
  /**
     ...
     */
  @Test public void detectsPullRequest() throws Exception {
    final Issue issue = Mockito.mock(Issue.class);
    Mockito.doReturn(Json.createObjectBuilder().add("pull_request", Json.createObjectBuilder().add("html_url", "http://ibm.com/pulls/3")).build()).when(issue).json();
    ...
    MatcherAssert.assertThat(new Issue.Smart(issue).isPull(), Matchers.is(true));
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void detectsPullRequestAbsence() throws Exception {
    final Issue issue = Mockito.mock(Issue.class);
    Mockito.doReturn(Json.createObjectBuilder().add("pull_request", Json.createObjectBuilder().addNull("html_url")).build()).when(issue).json();
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void detectsFullPullRequestAbsence() throws Exception {
    final Issue issue = Mockito.mock(Issue.class);
    Mockito.doReturn(Json.createObjectBuilder().build()).when(issue).json();
    MatcherAssert.assertThat(new Issue.Smart(issue).isPull(), Matchers.is(false));
  }
  ...
  /**
     ...
     */
  @Test public void fetchLabelsRO() throws IOException {
