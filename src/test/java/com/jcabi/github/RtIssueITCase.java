...
public final class RtIssueITCase {
  ...
  /**
     ...
     */
  @Test public void talksInGithubProject() throws Exception {
    ...
    MatcherAssert.assertThat(issue.comments().iterate(), Matchers.<Comment>iterableWithSize(1));
    final User.Smart author = new User.Smart(new Comment.Smart(comment).author());
    final User.Smart self = new User.Smart(issue.repo().github().users().self());
    if (author.hasName() && self.hasName()) {
      MatcherAssert.assertThat(author.name(), Matchers.equalTo(self.name()));
    }
    comment.remove();
  }
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void changesTitleAndBody() throws Exception {
    final Issue issue = RtIssueITCase.issue();
    new Issue.Smart(issue).title("test one more time");
    MatcherAssert.assertThat(new Issue.Smart(issue).title(), Matchers.startsWith("test o"));
    new Issue.Smart(issue).body("some new body of the issue");
    MatcherAssert.assertThat(new Issue.Smart(issue).body(), Matchers.startsWith("some new "));
  }
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void changesIssueState() throws Exception {
    final Issue issue = RtIssueITCase.issue();
  ...
  /**
     ...
     */
  @Test public void identifyAssignee() throws Exception {
    final Issue issue = RtIssueITCase.issue();
    ...
    try {
      ...
      Assume.assumeFalse("Something wrong with your test account. Read test\'s java-doc.", true);
    }
    final User assignee = new Issue.Smart(issue).assignee();
    MatcherAssert.assertThat(assignee.login(), Matchers.equalTo(login));
  }
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void checksForPullRequest() throws Exception {
    final Issue issue = RtIssueITCase.issue();
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void listsIssueEvents() throws Exception {
    final Issue issue = RtIssueITCase.issue();
    new Issue.Smart(issue).close();
    MatcherAssert.assertThat(new Event.Smart(issue.events().iterator().next()).type(), Matchers.equalTo(Event.CLOSED));
  }
  ...
  /**
     ...
     * @throws Exception If some problem inside
     */
  @Test public void findsLatestEvent() throws Exception {
    final Issue.Smart issue = new Issue.Smart(RtIssueITCase.issue());
    issue.close();
    MatcherAssert.assertThat(new Event.Smart(new Issue.Smart(issue).latestEvent(Event.CLOSED)).author().login(), Matchers.equalTo(issue.author().login()));
  }
  ...
  /**
     ...
     */
  @Test public void issueAlwaysExistsInGithub() throws Exception {
    MatcherAssert.assertThat(new Issue.Smart(RtIssueITCase.issue()).exists(), Matchers.is(true));
  }
