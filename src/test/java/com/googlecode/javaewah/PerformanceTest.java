...
public class PerformanceTest {
  @Rule public TestRule benchmarkRun = new BenchmarkRule(CONSOLE_CONSUMER, H2_CONSUMER);

  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void bigunion() throws Exception {
  ...
  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void bigunionbuf() throws Exception {
    for (int k = 1; k < N; k += 10) {
  ...
  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void toarray() throws Exception {
    for (int k = 1; k < N * 100; ++k) {
      bogus += ewah[k % N].toArray().length;
    }
  }

  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void toarraybuf() throws Exception {
    for (int k = 1; k < N * 100; ++k) {
      bogus += ewahbuf[k % N].toArray().length;
    }
  }

  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void cardinality() throws Exception {
    for (int k = 1; k < N * 100; ++k) {
      bogus += ewah[k % N].cardinality();
    }
  }

  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void cardinalitybuf() throws Exception {
    for (int k = 1; k < N * 100; ++k) {
      bogus += ewahbuf[k % N].cardinality();
    }
  }

  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void createBitmapOrdered() {
  ...
  @BenchmarkOptions(benchmarkRounds = 3, warmupRounds = 3) @Test public void createBitmapUnordered() {
    long besttime = Long.MAX_VALUE;
    EWAHCompressedBitmap r = new EWAHCompressedBitmap();
    long bef = System.nanoTime();
