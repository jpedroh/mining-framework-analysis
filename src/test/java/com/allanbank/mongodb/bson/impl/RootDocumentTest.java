 ...
 */
@SuppressWarnings(value = { "boxing" }) public class RootDocumentTest {
  /**
     * Test method for
  ...
  /**
     ...
     */
  @Test public void testConstructEmptyDocument() {
  ...
  /**
     * Test method for {@link RootDocument#RootDocument(java.util.List)} .
     */
  @Test public void testConstructEmptyDocumentList() {
  ...
  /**
     * Test method for {@link RootDocument#RootDocument(java.util.List)} .
     */
  @Test public void testConstructEmptyDocumentListEmpty() {
  ...
  @Test public void testConstructor() {
    final List<Element> elements = Collections.singletonList((Element) new BooleanElement("1", false));
    final RootDocument element = new RootDocument(elements);
  ...
  /**
     ...
     */
  @Test public void testEqualsEmptyDocument() {
    final Document root = new RootDocument();
    final Document rootWithElement = new RootDocument(new StringElement("a", "b"));
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      for (final String elemName : Arrays.asList("1", "2", "bar", "baz")) {
        ...
        switch (type) {
          ...
          break;
          case DOCUMENT:
          builder.push(elemName).addBoolean("boolean", rand.nextBoolean());
          break;
          case DOUBLE:
          ...
          builder.addJavaScript(elemName, "function bar() {}", BuilderFactory.start().build());
          break;
          case LONG:
          ...
          builder.addMongoTimestamp(elemName, System.currentTimeMillis());
          break;
          case NULL:
  ...
  @Test public void testFindFirstWithEmptyPathNonMatchingType() {
    final RootDocument element = new RootDocument(new BooleanElement("1", false));
  ...
  @Test public void testFindFirstWithPathMatchingNonFirstSubelement() {
    ...
    final RootDocument element = new RootDocument(new BooleanElement("2", false), subElement);
  ...
  @Test public void testFindFirstWithPathMatchingNonLastSubelement() {
    final BooleanElement subElement = new BooleanElement("123", false);
    final RootDocument element = new RootDocument(subElement, new BooleanElement("2", false));
    Element found = element.findFirst(Element.class, "123");
    assertSame(subElement, found);
    found = element.findFirst(Element.class, "12.");
    assertSame(subElement, found);
  ...
  @Test public void testGetElements() {
    ...
    final RootDocument element = new RootDocument(subElement);
    assertEquals(Collections.singletonList(subElement), element.getElements());
  }

  /**
     * Test method for {@link RootDocument#get(Class,String)}.
     */
  @Test public void testGetWithType() {
    final BooleanElement subElement = new BooleanElement("1", false);
    final RootDocument element = new RootDocument(subElement);
  ...
  /**
     * Test method for {@link RootDocument#injectId()}.
     */
  @Test public void testInjectIdWhenLiedTo() {
    final Element subElement = new BooleanElement("1", false);
    final RootDocument element = new RootDocument(Collections.singletonList(subElement), true);
    assertTrue(element.contains("_id"));
    assertTrue(element.contains("1"));
    assertFalse(element.contains("2"));
    element.injectId();
  ...
  @Test public void testToString() {
    ...
    RootDocument element = new RootDocument(Arrays.asList(subElement, subElement2));
    assertEquals("{\n  \'1\' : false,\n  \'2\' : false\n}", element.toString());
    ...
    assertEquals("{\n  f : []\n}", element.toString());
    element = new RootDocument(new ArrayElement("f", new BooleanElement("0", true)));
    assertEquals("{\n  f : [ true ]\n}", element.toString());
  }
}