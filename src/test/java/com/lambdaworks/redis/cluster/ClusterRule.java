...
public class ClusterRule implements TestRule {
  ...
  public ClusterRule(RedisClusterClient clusterClient, int... ports) {
    ...
    this.ports = ports;
    for (int port : ports) {
      RedisClusterAsyncConnection<String, String> connection = clusterClient.connectToNode(new InetSocketAddress("localhost", port)).async();
  ...
  public boolean isStable() {
    RedisClusterAsyncConnection<String, String> connection = clusterClient.connectToNode(new InetSocketAddress("localhost", ports[0])).async();
    try {
      ...
      if (info != null && info.contains("cluster_state:ok")) {
        ...
        for (RedisClusterNode redisClusterNode : parse) {
          if (redisClusterNode.getFlags().contains(RedisClusterNode.NodeFlag.FAIL) || redisClusterNode.getFlags().contains(RedisClusterNode.NodeFlag.EVENTUAL_FAIL)) {
            return false;
          }
        }
        return true;
