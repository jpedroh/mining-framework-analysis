...
public class PushManagerTest extends BasePushyTest {
  ...
  @Test public void testRegisterRejectedNotificationListener() throws InterruptedException {
    ...
    for (int i = 0; i < iterations; i++) {
      if (i == iterations / 2) {
        this.getPushManager().getQueue().add(new SimpleApnsPushNotification(new byte[] {  }, "This is a deliberately malformed notification."));
      } else {
  ...
  @Test public void testRegisterFailedConnectionListener() throws Exception {
  ...
  @Test public void testUnregisterFailedConnectionListener() {
  ...
  @Test public void testDrainBeforeShutdown() throws InterruptedException {
    ...
    for (int i = 0; i < iterations; i++) {
      if (i == iterations / 2) {
        notificationsToSend.add(new SimpleApnsPushNotification(new byte[] {  }, "This is a deliberately malformed notification."));
      } else {
  ...
  @Test(expected = IllegalStateException.class) public void testDoubleStart() throws Exception {
  ...
  @Test(expected = IllegalStateException.class) public void testPrematureShutdown() throws Exception {
  ...
  @Test public void testRepeatedShutdown() throws Exception {
  ...
  @Test public void testIsStarted() throws InterruptedException {
  ...
  @Test public void testIsShutDown() throws InterruptedException {
  ...
  @Test public void testSendNotificationsWithParallelConnections() throws Exception {
  ...
  @Test public void testSendNotificationsWithParallelConnectionsAndError() throws Exception {
    final PushManagerFactory<SimpleApnsPushNotification> factory = new PushManagerFactory<SimpleApnsPushNotification>(TEST_ENVIRONMENT, SSLTestUtil.createSSLContextForTestClient());
    factory.setEventLoopGroup(this.getEventLoopGroup());
    factory.setConcurrentConnectionCount(4);
    final PushManager<SimpleApnsPushNotification> parallelPushManager = factory.buildPushManager();
    ...
    for (int i = 0; i < iterations; i++) {
      if (i == iterations / 2) {
        parallelPushManager.getQueue().add(new SimpleApnsPushNotification(new byte[] {  }, "This is a deliberately malformed notification."));
      } else {
        parallelPushManager.getQueue().add(this.createTestNotification());
      }
    }
  ...
  @Test public void testHandleDispatchThreadException() throws Exception {
