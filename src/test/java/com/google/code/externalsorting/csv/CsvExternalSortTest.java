...
public class CsvExternalSortTest {
  ...
  @Test public void testMultiLineFile() throws IOException, ClassNotFoundException {
    ...
    CsvSortOptions sortOptions = new CsvSortOptions.Builder(CsvExternalSort.DEFAULTMAXTEMPFILES, comparator, 1, CsvExternalSort.estimateAvailableMemory()).charset(Charset.defaultCharset()).distinct(false).numHeader(1).build();
    List<File> sortInBatch = CsvExternalSort.sortInBatch(file, null, sortOptions, CSVFormat.DEFAULT);
  ...
  @Test public void testNonLatin() throws Exception {
    ...
    Comparator<CSVRecord> comparator = (op1, op2) -> op1.get(0).compareTo(op2.get(0));
    CsvSortOptions sortOptions = new CsvSortOptions.Builder(CsvExternalSort.DEFAULTMAXTEMPFILES, comparator, 1, CsvExternalSort.estimateAvailableMemory()).charset(StandardCharsets.UTF_8).distinct(false).numHeader(1).build();
    List<File> sortInBatch = CsvExternalSort.sortInBatch(file, null, sortOptions, CSVFormat.DEFAULT);
    assertEquals(sortInBatch.size(), 1);
    int mergeSortedFiles = CsvExternalSort.mergeSortedFiles(sortInBatch, outputfile, sortOptions, true, CSVFormat.DEFAULT);
  ...
  @Test public void testMultiLineFileWthHeader() throws IOException, ClassNotFoundException {
    String path = this.getClass().getClassLoader().getResource(FILE_CSV).getPath();
    File file = new File(path);
    outputfile = new File("outputSort1.csv");
    Comparator<CSVRecord> comparator = (op1, op2) -> op1.get(0).compareTo(op2.get(0));
    CsvSortOptions sortOptions = new CsvSortOptions.Builder(CsvExternalSort.DEFAULTMAXTEMPFILES, comparator, 1, CsvExternalSort.estimateAvailableMemory()).charset(Charset.defaultCharset()).distinct(false).numHeader(1).skipHeader(false).build();
  ...
  @Test public void testCVSFormat() throws Exception {
