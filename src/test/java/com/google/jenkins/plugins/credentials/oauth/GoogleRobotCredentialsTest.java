...
public class GoogleRobotCredentialsTest {
  ...
  public static class NameProvider extends CredentialsNameProvider<GoogleRobotCredentials> {
    @Override public String getName(GoogleRobotCredentials credentials) {
      return NAME;
    }
  }

  @NameWith(value = NameProvider.class, priority = 100) @RequiresDomain(value = TestRequirement.class) public static class FakeGoogleCredentials extends GoogleRobotCredentials {
    ...
    @Override public GoogleCredential getGoogleCredential(GoogleOAuth2ScopeRequirement requirement) throws GeneralSecurityException {
      if (credential == null) {
    ...
    @Extension public static class DescriptorImpl extends AbstractGoogleRobotCredentialsDescriptor {
  ...
  @Test @WithoutJenkins public void testGettersNullId() throws Exception {
  ...
  @Test @WithoutJenkins public void testGetAccessToken() throws Exception {
    FakeGoogleCredentials credentials = new FakeGoogleCredentials(PROJECT_ID, fakeCredential);
    fakeCredential.setAccessToken(ACCESS_TOKEN);
    fakeCredential.setExpiresInSeconds(EXPIRATION_SECONDS);
    assertEquals(ACCESS_TOKEN, Secret.toString(credentials.getAccessToken(null)));
  }

  @Test @WithoutJenkins public void testGetAccessTokenNoCredential() throws Exception {
    FakeGoogleCredentials credentials = new FakeGoogleCredentials(PROJECT_ID, null);
    assertNull(credentials.getAccessToken(null));
  }

  @Test @WithoutJenkins public void testForRemote() throws Exception {
    FakeGoogleCredentials credentials = new FakeGoogleCredentials(PROJECT_ID, fakeCredential);
    fakeCredential.setAccessToken(ACCESS_TOKEN);
    fakeCredential.setExpiresInSeconds(EXPIRATION_SECONDS);
    GoogleOAuth2ScopeRequirement requirement = new GoogleOAuth2ScopeRequirement() {
      @Override public Collection<String> getScopes() {
  ...
  @Test public void testListBoxEmpty() throws Exception {
  ...
  @LocalData @Test public void testMigration() {
    assertNotNull(GoogleRobotCredentials.getById(MIGRATION_PROJECT_ID));
  }

  @Test public void testMultipleCredentials() {
