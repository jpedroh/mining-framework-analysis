...
public class UUIDGeneratorTest extends TestCase {
  ...
  public void testGetDummyAddress() {
    EthernetAddress ethernet_address_array[] = new EthernetAddress[SIZE_OF_TEST_ARRAY];
    ...
    for (int i = 0; i < ethernet_address_array.length; i++) {
      ...
      assertFalse("dummy EthernetAddress was null", Arrays.equals(null_ethernet_address.asByteArray(), ethernet_address));
      assertEquals("dummy EthernetAddress was not broadcast", 0x01, (ethernet_address[0] & 0x01));
    }
  }
  ...
  public void testGenerateTimeBasedUUIDWithEthernetAddress() {
    EthernetAddress ethernet_address = new EthernetAddress("87:F5:93:06:D3:0C");
  ...
  public void testGenerateNameBasedUUIDNameSpaceAndName() {
    ...
    assertTrue("expected both arrays to be equal, they were not!", Arrays.equals(uuid_array, uuid_array2));
  }
  ...
  class ReverseOrderUUIDComparator implements Comparator<UUID> {
    @Override public int compare(UUID uuid1, UUID uuid2) {
  ...
  private void checkUUIDArrayForCorrectOrdering(UUID[] uuidArray) {
    ...
    assertEquals("Cloned array length did not match", uuidArray.length, uuid_sorted_array.length);
    ReverseOrderUUIDComparator rev_order_uuid_comp = new ReverseOrderUUIDComparator();
    Arrays.sort(uuid_sorted_array, rev_order_uuid_comp);
    for (int i = 0; i < uuid_sorted_array.length; i++) {
      assertTrue("Reverse order check on uuid arrays failed on element " + i, uuidArray[i].equals(uuid_sorted_array[uuid_sorted_array.length - (1 + i)]));
    }
    Arrays.sort(uuid_sorted_array);
    for (int i = 0; i < uuid_sorted_array.length; i++) {
      assertTrue("Same order check on uuid arrays failed on element " + i, uuidArray[i].equals(uuid_sorted_array[i]));
    }
  }
  ...
  private void checkUUIDArrayForUniqueness(UUID[] uuidArray) {
    ...
    for (int i = 0; i < uuidArray.length; i++) {
      assertTrue("Uniqueness test failed on insert into HashSet: index " + i + ", value " + uuidArray[i], hash_set.add(uuidArray[i]));
      assertFalse("Paranoia Uniqueness test failed (second insert)", hash_set.add(uuidArray[i]));
    }
  }

  private void checkUUIDArrayForCorrectVariantAndVersion(UUID[] uuidArray, UUIDType expectedType) {
    for (int i = 0; i < uuidArray.length; i++) {
      UUIDType actual = UUIDUtil.typeOf(uuidArray[i]);
      if (actual != expectedType) {
        fail("Expected version (type) did not match for UUID \'" + uuidArray[i] + "\' " + i + " (of " + uuidArray.length + "); expected " + expectedType + ", got " + actual);
      }
      byte[] temp_uuid = UUIDUtil.asByteArray(uuidArray[i]);
      int type = (temp_uuid[UUIDUtil.BYTE_OFFSET_TYPE] & 0xFF) >> 4;
      assertEquals("Expected type did not match", expectedType.raw(), type);
      int variant = (temp_uuid[UUIDUtil.BYTE_OFFSET_VARIATION] & 0xFF) >> 6;
      assertEquals("Expected variant did not match", 2, variant);
    }
  }
  ...
  private void checkUUIDArrayForCorrectEthernetAddress(UUID[] uuidArray, EthernetAddress ethernetAddress) {
    for (int i = 0; i < uuidArray.length; i++) {
      ...
      assertTrue("UUID ethernet address did not equal passed ethernetAddress", Arrays.equals(ethernet_address, uuid_ethernet_address));
    }
  }
