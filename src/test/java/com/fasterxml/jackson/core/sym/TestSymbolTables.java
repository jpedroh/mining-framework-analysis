...
public class TestSymbolTables extends com.fasterxml.jackson.core.BaseTest {
  ...
  @SuppressWarnings(value = { "deprecation" }) public void testSyntheticWithBytesOld() throws IOException {
    final int SEED = 33333;
    BytesToNameCanonicalizer symbols = BytesToNameCanonicalizer.createRoot(SEED).makeChild(JsonFactory.Feature.collectDefaults());
    final int COUNT = 12000;
    for (int i = 0; i < COUNT; ++i) {
      String id = fieldNameFor(i);
  ...
  public void testSyntheticWithBytesNew() throws IOException {
    final int SEED = 33333;
    ByteQuadsCanonicalizer symbols = ByteQuadsCanonicalizer.createRoot(SEED).makeChild(JsonFactory.Feature.collectDefaults());
    final int COUNT = 12000;
    for (int i = 0; i < COUNT; ++i) {
      String id = fieldNameFor(i);
      int[] quads = calcQuads(id.getBytes("UTF-8"));
  ...
  public void testThousandsOfSymbolsWithChars() throws IOException {
    ...
    for (int doc = 0; doc < 100; ++doc) {
      CharsToNameCanonicalizer symbolsC = symbolsCRoot.makeChild(JsonFactory.Feature.collectDefaults());
      for (int i = 0; i < 250; ++i) {
  ...
  @SuppressWarnings(value = { "deprecation" }) public void testThousandsOfSymbolsWithOldBytes() throws IOException {
    final int SEED = 33333;
    BytesToNameCanonicalizer symbolsBRoot = BytesToNameCanonicalizer.createRoot(SEED);
    final Charset utf8 = Charset.forName("UTF-8");
    int exp = 0;
    for (int doc = 0; doc < 100; ++doc) {
      BytesToNameCanonicalizer symbolsB = symbolsBRoot.makeChild(JsonFactory.Feature.collectDefaults());
      for (int i = 0; i < 250; ++i) {
        String name = "f_" + doc + "_" + i;
  ...
  public void testByteBasedSymbolTable() throws Exception {
    final String JSON = aposToQuotes("{\'abc\':1, \'abc\\u0000\':2, \'\\u0000abc\':3, " + "\'abc123\':4,\'abcd1234\':5," + "\'abcd1234a\':6,\'abcd1234abcd\':7," + "\'abcd1234abcd1\':8" + "}");
  ...
  @SuppressWarnings(value = { "deprecation" }) public void testCollisionsWithBytes187() throws IOException {
    BytesToNameCanonicalizer symbols = BytesToNameCanonicalizer.createRoot(1).makeChild(JsonFactory.Feature.collectDefaults());
  ...
  public void testCollisionsWithBytesNew187a() throws IOException {
    ByteQuadsCanonicalizer symbols = ByteQuadsCanonicalizer.createRoot(1).makeChild(JsonFactory.Feature.collectDefaults());
  ...
  @SuppressWarnings(value = { "deprecation" }) public void testShortNameCollisionsDirectOld() throws IOException {
    final int COUNT = 600;
    {
      CharsToNameCanonicalizer symbols = CharsToNameCanonicalizer.createRoot(1);
      for (int i = 0; i < COUNT; ++i) {
  ...
  public void testShortNameCollisionsDirectNew() throws IOException {
    final int COUNT = 700;
    {
      ...
      assertEquals(COUNT, symbols.primaryCount() + symbols.secondaryCount() + symbols.tertiaryCount() + symbols.spilloverCount());
    }
  }
}