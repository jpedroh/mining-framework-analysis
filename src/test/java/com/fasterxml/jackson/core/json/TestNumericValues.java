 ...
 */
@SuppressWarnings(value = { "resource" }) public class TestNumericValues extends com.fasterxml.jackson.core.BaseTest {
  ...
  private void _testSimpleInt(int EXP_I, boolean useStream) throws Exception {
    String DOC = "[ " + EXP_I + " ]";
    JsonParser jp = useStream ? FACTORY.createParser(DOC) : FACTORY.createParser(DOC.getBytes("UTF-8"));
    assertToken(JsonToken.START_ARRAY, jp.nextToken());
    ...
    jp = useStream ? FACTORY.createParser(DOC) : FACTORY.createParser(DOC.getBytes("UTF-8"));
    assertToken(JsonToken.VALUE_NUMBER_INT, jp.nextToken());
  ...
  private void _testLongNumbers(JsonFactory f, String num, boolean useStream) throws Exception {
    ...
    JsonParser jp = useStream ? f.createParser(doc.getBytes("UTF-8")) : f.createParser(doc);
    assertToken(JsonToken.START_ARRAY, jp.nextToken());
    assertToken(JsonToken.VALUE_NUMBER_INT, jp.nextToken());
  ...
  private void _testIssue160LongNumbers(JsonFactory f, String doc, boolean useStream) throws Exception {
    JsonParser jp = useStream ? FACTORY.createParser(doc.getBytes("UTF-8")) : FACTORY.createParser(doc);
    assertToken(JsonToken.VALUE_NUMBER_INT, jp.nextToken());
