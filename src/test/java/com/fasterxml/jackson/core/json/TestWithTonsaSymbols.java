 ...
 */
public class TestWithTonsaSymbols extends BaseTest {
  /**
  ...
  private void _testWith(boolean useStream) throws Exception {
    JsonFactory jf = new JsonFactory();
    String doc = buildDoc(FIELD_COUNT);
    for (int x = 0; x < 3; ++x) {
      JsonParser p = useStream ? jf.createParser(new ByteArrayInputStream(doc.getBytes("UTF-8"))) : jf.createParser(new StringReader(doc));
