...
public class StringGenerationFromReaderTest extends BaseTest {
  ...
  private String generateRandom(int len) {
    StringBuilder sb = new StringBuilder(len + 1000);
  ...
  private void _testMediumStrings(int readMode, int length) throws Exception {
    ...
    JsonGenerator gen = (readMode != MODE_READER) ? FACTORY.createGenerator(ObjectWriteContext.empty(), bytes) : FACTORY.createGenerator(ObjectWriteContext.empty(), sw);
    gen.writeStartArray();
  ...
  private void _testLongerRandom(int readMode, String text) throws Exception {
  ...
  private void _testLongerRandomMulti(int readMode, String text, int round) throws Exception {
    ByteArrayOutputStream bow = new ByteArrayOutputStream(text.length());
    JsonGenerator gen = FACTORY.createGenerator(ObjectWriteContext.empty(), bow, JsonEncoding.UTF8);
    gen.writeStartArray();
    StringReader reader = new StringReader(text);
    ...
    while (offset < text.length()) {
      int shift = 1 + ((rnd.nextInt() & 0xFFFFF) % 12);
      int len = (1 << shift) + shift;
    ...
    while (p.nextToken() == JsonToken.VALUE_STRING) {
      ...
      if (!act.equals(exp)) {
        ...
        fail("String segment [" + offset + " - " + (offset + act.length()) + "[ different at offset #" + i + "; exp char 0x" + Integer.toHexString(exp.charAt(i)) + ", actual 0x" + Integer.toHexString(act.charAt(i)));
      }
      offset += act.length();
    }
