 ...
 */
@SuppressWarnings(value = { "resource" }) public class NumberParsingTest extends com.fasterxml.jackson.core.BaseTest {
  ...
  public void testLongBoundsChecks() {
    ...
    String belowMinLong = BigInteger.valueOf(Long.MIN_VALUE).subtract(BigInteger.ONE).toString().substring(1);
  ...
  public void testSimpleDouble() {
    final String[] INPUTS = new String[] { "1234.00", "2.1101567E-16", "1.0e5", "0.0", "1.0", "-1.0", "-0.5", "-12.9", "-999.0", "2.5e+5", "9e4", "-12e-3", "0.25" };
    for (int mode : ALL_MODES) {
  ...
  private void _testBigBigDecimals(int mode) throws Exception {
    ...
    for (String asText : new String[] { "50." + BASE_FRACTION, "-37." + BASE_FRACTION, "0.00" + BASE_FRACTION, "-0.012" + BASE_FRACTION, "9999998." + BASE_FRACTION, "-8888392." + BASE_FRACTION }) {
  ...
  private void _testLongNumbers(JsonFactory f, String num, boolean useStream) {
    ...
    JsonParser p = useStream ? f.createParser(ObjectReadContext.empty(), utf8Bytes(doc)) : f.createParser(ObjectReadContext.empty(), doc);
    assertToken(JsonToken.START_ARRAY, p.nextToken());
    assertToken(JsonToken.VALUE_NUMBER_INT, p.nextToken());
  ...
  private void _testIssue160LongNumbers(JsonFactory f, String doc, boolean useStream) {
    JsonParser p = useStream ? jsonFactory().createParser(ObjectReadContext.empty(), utf8Bytes(doc)) : jsonFactory().createParser(ObjectReadContext.empty(), doc);
    assertToken(JsonToken.VALUE_NUMBER_INT, p.nextToken());
  ...
  public void testParsingOfLongerSequencesWithNonNumeric() {
    JsonFactory f = streamFactoryBuilder().enable(JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS).build();
  ...
  private void _testParsingOfLongerSequencesWithNonNumeric(JsonFactory f, int mode) {
    double[] values = new double[] { 0.01, -10.5, 2.1e9, 4.0e-8, Double.NaN, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY };
    for (int i = 0; i < values.length; ++i) {
      int COUNT = 4096;
      int VCOUNT = 2 * COUNT;
  ...
  public void testLongerFloatingPoint() {
    ...
    p.close();
    p = jsonFactory().createParser(ObjectReadContext.empty(), new ByteArrayInputStream(utf8Bytes(DOC)));
    _testLongerFloat(p, DOC);
    p.close();
  }
