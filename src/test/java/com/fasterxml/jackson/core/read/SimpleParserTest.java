 ...
 */
@SuppressWarnings(value = { "resource" }) public class SimpleParserTest extends BaseTest {
  ...
  private void _testSkipping(int mode) throws Exception {
    ...
    String DOC = a2q("[ 1, 3, [ true, null ], 3, { \'a\\\\b\':\'quoted: \\\'stuff\\\'\' }, [ [ ] ], { } ]");
  ...
  private void _testHandlingOfInvalidSpaceFromResource(boolean useStream) throws Exception {
    ...
    JsonParser p = useStream ? JSON_FACTORY.createParser(in) : JSON_FACTORY.createParser(new InputStreamReader(in, "UTF-8"));
    assertToken(JsonToken.START_OBJECT, p.nextToken());
    try {
      assertToken(JsonToken.FIELD_NAME, p.nextToken());
  ...
  public void testInvalidUtf8ValidUtf16() throws IOException {
    JsonFactory factory = new JsonFactoryBuilder().disable(JsonFactory.Feature.CHARSET_DETECTION).build();
  ...
  protected void verifyJsonSpecSampleDoc(JsonParser p, boolean verifyContents) throws IOException {
    verifyJsonSpecSampleDoc(p, verifyContents, true);
  }

  protected void verifyJsonSpecSampleDoc(JsonParser p, boolean verifyContents, boolean requireNumbers) throws IOException {
