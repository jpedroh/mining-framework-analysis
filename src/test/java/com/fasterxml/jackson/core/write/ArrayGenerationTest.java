...
public class ArrayGenerationTest extends BaseTest {
  ...
  private void _testIntArray(boolean useBytes, int elements, int pre, int post) throws Exception {
    ...
    JsonGenerator gen = useBytes ? jsonFactory().createGenerator(ObjectWriteContext.empty(), bytes) : jsonFactory().createGenerator(ObjectWriteContext.empty(), sw);
    ...
    JsonParser p = useBytes ? jsonFactory().createParser(ObjectReadContext.empty(), bytes.toByteArray()) : jsonFactory().createParser(ObjectReadContext.empty(), json);
  ...
  private void _testStringArray(boolean useBytes, int elements, int pre, int post) throws Exception {
    ...
    JsonGenerator gen = useBytes ? jsonFactory().createGenerator(wctxt, bytes) : jsonFactory().createGenerator(wctxt, sw);
    gen.writeArray(values, pre, elements);
    gen.close();
    String json;
    if (useBytes) {
    ...
    JsonParser p = useBytes ? jsonFactory().createParser(rctxt, bytes.toByteArray()) : jsonFactory().createParser(rctxt, json);
    assertToken(JsonToken.START_ARRAY, p.nextToken());
    for (int i = 0; i < elements; ++i) {
      JsonToken t = p.nextToken();
