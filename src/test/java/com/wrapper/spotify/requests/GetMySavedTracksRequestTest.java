...
public class GetMySavedTracksRequestTest {
  @Test public void shouldGetSavedTracks_async() throws Exception {
    final Api api = Api.builder().accessToken("someAccessToken").build();
    final GetMySavedTracksRequest request = api.getMySavedTracks().limit(5).offset(1).setHttpManager(TestUtil.MockedHttpManager.returningJson("saved-tracks.json")).build();
    ...
    Futures.addCallback(libraryTracksFuture, new FutureCallback<Paging<LibraryTrack>>() {
      @Override public void onSuccess(Paging<LibraryTrack> libraryTracks) {
  ...
  @Test public void shouldGetSavedTracks_sync() throws Exception {
    final Api api = Api.builder().accessToken("someAccessToken").build();
    final GetMySavedTracksRequest request = api.getMySavedTracks().limit(5).offset(1).setHttpManager(TestUtil.MockedHttpManager.returningJson("saved-tracks.json")).build();
