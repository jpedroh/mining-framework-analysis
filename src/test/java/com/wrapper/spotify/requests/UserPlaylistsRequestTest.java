...
public class UserPlaylistsRequestTest {
  @Test public void shouldGetPlaylistsForUser_async() throws Exception {
    ...
    final UserPlaylistsRequest request = api.getPlaylistsForUser("wizzler").accessToken(accessToken).limit(10).offset(2).setHttpManager(TestUtil.MockedHttpManager.returningJson("user-playlists.json")).build();
  ...
  @Test public void shouldGetPlaylistsForUser_sync() throws Exception {
    ...
    final Api api = Api.builder().build();
    final UserPlaylistsRequest request = api.getPlaylistsForUser("wizzler").accessToken(accessToken).setHttpManager(TestUtil.MockedHttpManager.returningJson("user-playlists.json")).build();
