...
@RunWith(value = MockitoJUnitRunner.class) public class AlbumsRequestTest {
  @Test public void shouldGetAlbumResultForIds_async() throws Exception {
    final Api api = Api.DEFAULT_API;
    final AlbumsRequest request = api.getAlbums("2hYe61Nd2oOoM6RYCwIma1").setHttpManager(TestUtil.MockedHttpManager.returningJson("albums.json")).build();
    ...
    Futures.addCallback(albumsFuture, new FutureCallback<Album[]>() {
      @Override public void onSuccess(Album[] albums) {
  ...
  @Test public void shouldFailForNotFound_async() throws Exception {
    final Api api = Api.DEFAULT_API;
    final AlbumsRequest request = api.getAlbums("idontexist").setHttpManager(TestUtil.MockedHttpManager.returningJson("albums-none-found.json")).build();
    final CountDownLatch asyncCompleted = new CountDownLatch(1);
