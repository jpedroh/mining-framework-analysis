...
@RunWith(value = MockitoJUnitRunner.class) public class TrackSearchRequestTest {
  @Test public void shouldGetTracksResult_async() throws Exception {
    final Api api = Api.DEFAULT_API;
    final TrackSearchRequest request = api.searchTracks("tania bowra").offset(0).limit(20).setHttpManager(TestUtil.MockedHttpManager.returningJson("search-track.json")).build();
    ...
    Futures.addCallback(searchResultFuture, new FutureCallback<Paging<Track>>() {
      @Override public void onSuccess(Paging<Track> trackSearchResult) {
