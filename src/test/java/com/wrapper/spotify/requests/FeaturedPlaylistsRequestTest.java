...
public class FeaturedPlaylistsRequestTest {
  @Test public void shouldGetNewReleases_async() throws Exception {
    ...
    final FeaturedPlaylistsRequest request = api.getFeaturedPlaylists().limit(1).offset(1).country("SE").timestamp(timestamp).setHttpManager(TestUtil.MockedHttpManager.returningJson("featured-playlists.json")).build();
    ...
    Futures.addCallback(future, new FutureCallback<FeaturedPlaylists>() {
      @Override public void onSuccess(FeaturedPlaylists featuredPlaylists) {
        ...
        assertEquals("https://api.spotify.com/v1/browse/featured-playlists?timestamp=2017-12-03T01%3A40%3A49&offset=1&limit=1", playlistPage.getNext());
