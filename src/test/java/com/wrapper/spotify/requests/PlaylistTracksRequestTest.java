...
public class PlaylistTracksRequestTest {
  @Test public void shouldGetTracksResult_async() throws Exception {
    ...
    final PlaylistTracksRequest request = api.getPlaylistTracks("thelinmichael", "3ktAYNcRHpazJ9qecm3ptn").setHttpManager(TestUtil.MockedHttpManager.returningJson("playlist-tracks.json")).build();
    ...
    Futures.addCallback(playlistTracksPageFuture, new FutureCallback<Paging<PlaylistTrack>>() {
      @Override public void onSuccess(Paging<PlaylistTrack> page) {
        assertNotNull(page);
        assertEquals("https://api.spotify.com/v1/users/thelinmichael/playlists/3ktAYNcRHpazJ9qecm3ptn/tracks?offset=0&limit=100", page.getHref());
