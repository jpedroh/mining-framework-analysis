...
@RunWith(value = MockitoJUnitRunner.class) public class AlbumsForArtistsRequestTest {
  @Test public void shouldGetAlbumResultForArtistId_async() throws Exception {
    final Api api = Api.DEFAULT_API;
    final AlbumsForArtistRequest request = api.getAlbumsForArtist("1vCWHaC5f2uS3yhpwWbIA6").limit(2).types(AlbumType.SINGLE).market("US").setHttpManager(TestUtil.MockedHttpManager.returningJson("artist-album.json")).build();
    ...
    Futures.addCallback(albumsFuture, new FutureCallback<Paging<AlbumSimplified>>() {
      @Override public void onSuccess(Paging<AlbumSimplified> albumSearchResult) {
  ...
  @Test public void shouldGetAlbumResultForArtistId_sync() throws Exception {
    final Api api = Api.DEFAULT_API;
    final AlbumsForArtistRequest request = api.getAlbumsForArtist("1vCWHaC5f2uS3yhpwWbIA6").limit(2).types(AlbumType.SINGLE).market("US").setHttpManager(TestUtil.MockedHttpManager.returningJson("artist-album.json")).build();
