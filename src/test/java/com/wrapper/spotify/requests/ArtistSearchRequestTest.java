...
@RunWith(value = MockitoJUnitRunner.class) public class ArtistSearchRequestTest {
  @Test public void shouldGetArtistsResult_async() throws Exception {
    final Api api = Api.DEFAULT_API;
    final ArtistSearchRequest request = api.searchArtists("tania bowra").limit(20).offset(0).setHttpManager(TestUtil.MockedHttpManager.returningJson("search-artist.json")).build();
    ...
    Futures.addCallback(searchResultFuture, new FutureCallback<Paging<Artist>>() {
      @Override public void onSuccess(Paging<Artist> artistSearchResult) {
