...
public class NewReleasesRequestTest {
  @Test public void shouldGetNewReleases_async() throws Exception {
    ...
    final NewReleasesRequest request = api.getNewReleases().limit(1).setHttpManager(TestUtil.MockedHttpManager.returningJson("new-releases.json")).build();
    ...
    Futures.addCallback(future, new FutureCallback<NewReleases>() {
      @Override public void onSuccess(NewReleases newReleases) {
        ...
        assertEquals("https://api.spotify.com/v1/browse/new-releases?offset=0&limit=1", albums.getHref());
        assertEquals(1, albums.getLimit());
        assertEquals(0, albums.getOffset());
        assertEquals("https://api.spotify.com/v1/browse/new-releases?offset=1&limit=1", albums.getNext());
