...
public class AddTrackToPlaylistRequestTest {
  @Test public void shouldAddTracksToPlaylist_async() throws Exception {
    ...
    final AddTrackToPlaylistRequest request = api.addTracksToPlaylist(myUsername, myPlaylistId, tracksToAdd).position(insertIndex).setHttpManager(TestUtil.MockedHttpManager.returningJson("add-tracks.json")).build();
    ...
    Futures.addCallback(addTrackFuture, new FutureCallback<SnapshotResult>() {
      @Override public void onSuccess(SnapshotResult response) {
