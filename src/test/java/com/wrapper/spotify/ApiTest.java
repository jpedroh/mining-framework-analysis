...
public class ApiTest {
  ...
  @Test public void shouldCreateAGetTracksForAlbumUrl() {
    ...
    assertEquals("https://api.spotify.com:443/v1/albums/5oEljuMoe9MXH6tBIPbd5e/tracks", request.toString(false));
  }
  ...
  @Test public void shouldReplacePlaylistsTracks() {
    ...
    assertEquals("https://api.spotify.com:443/v1/users/userId/playlists/5oEljuMoe9MXH6tBIPbd5e/tracks", request.toString(false));
  }

  @Test public void shouldUnfollowPlaylist() {
    Api api = Api.DEFAULT_API;
    Request request = api.unfollowPlaylist("userId", "5oEljuMoe9MXH6tBIPbd5e").build();
    assertEquals("https://api.spotify.com:443/v1/users/userId/playlists/5oEljuMoe9MXH6tBIPbd5e/followers", request.toString(false));
  }
  ...
  @Test public void shouldHaveMultipleAlbumTypeParametersInArtistsAlbumUrl() {
    Api api = Api.DEFAULT_API;
    Request request = api.getAlbumsForArtist("4AK6F7OLvEQ5QYCBNiQWHq").types(AlbumType.ALBUM, AlbumType.SINGLE).market("SE").build();
    assertEquals("https://api.spotify.com:443/v1/artists/4AK6F7OLvEQ5QYCBNiQWHq/albums", request.toString(false));
  ...
  @Test public void shouldCreateRemoveTrackFromPlaylistUrl() {
    ...
    final String expectedJsonBody = String.format("{\"tracks\":[{\"uri\":\"%s\"},{\"uri\":\"%s\",\"positions\":[%s]}],\"snapshot_id\":\"%s\"}", track1Uri, track2Uri, String.valueOf(track2Position), snapshotId);
  ...
  @Test public void shouldCreateReplaceTracksInPlaylistUrl() {
    final String accessToken = "myVeryLongAccessToken";
    final Api api = Api.builder().accessToken(accessToken).build();
    final String myUsername = "thelinmichael";
    final String myPlaylistId = "5ieJqeLJjjI8iJWaxeBLuK";
  ...
  @Test public void shouldCreateChangePlaylistDetailsUrl() {
    ...
    final Request request = api.changePlaylistDetails(myUsername, myPlaylistId).publicAccess(isPublic).name(name).build();
    assertEquals("https://api.spotify.com:443/v1/users/thelinmichael/playlists/" + myPlaylistId, request.toString(false));
    assertHasHeader(request.toUrl(), "Authorization", "Bearer " + accessToken);
    assertHasHeader(request.toUrl(), "Content-Type", "application/json");
  ...
  @Test public void shouldCreateAGetPlaylistTracksURL() {
    ...
    final Api api = Api.builder().accessToken(accessToken).build();
    final Request request = api.getPlaylistTracks(userId, playlistId).fields("items").limit(20).offset(1).build();
  ...
  @Test public void shouldCreateAuthorizeUrlWithOptionalParameters() {
    ...
    final String[] scopes = { "user-read-private", "user-read-email" };
    final String state = "someExpectedStateString";
    String authorizeUrlString = api.createAuthorizeURL(scopes).state(state).build().toString();
    assertEquals("https://accounts.spotify.com:443/authorize?client_id=fcecfc79122e4cd299473677a17cbd4d&response_type=code&redirect_uri=http%3A%2F%2Fwww.michaelthelin.se%2Ftest-callback&scope=user-read-private+user-read-email&state=someExpectedStateString", authorizeUrlString);
  }
  ...
  @Test public void shouldCreateGetMyTracksURL() {
    final String accessToken = "myAccessToken";
    final Api api = Api.builder().accessToken(accessToken).build();
    final Request request = api.getMySavedTracks().limit(5).offset(1).build();
  ...
  @Test public void shouldCreatePutTracksURL() {
    final String accessToken = "myAccessToken";
    final Api api = Api.builder().accessToken(accessToken).build();
    final Request request = api.addToMySavedTracks(new String[] { "test", "test2" }).build();
    assertEquals("https://api.spotify.com:443/v1/me/tracks", request.toString(false));
    assertHasHeader(request.toUrl(), "Authorization", "Bearer " + accessToken);
  }
  ...
  @Test public void shouldCreateRemoveTracksURL() {
    final String accessToken = "myAccessToken";
    final Api api = Api.builder().accessToken(accessToken).build();
    final Request request = api.removeFromMySavedTracks(new String[] { "test", "test2" }).build();
    assertEquals("https://api.spotify.com:443/v1/me/tracks", request.toString(false));
    assertHasHeader(request.toUrl(), "Authorization", "Bearer " + accessToken);
  }
  ...
  @Test public void shouldCreateFeaturedPlaylistsRequest() {
    ...
    final Request request = api.getFeaturedPlaylists().country("SE").locale("es_MX").limit(5).offset(1).timestamp(timestamp).build();
