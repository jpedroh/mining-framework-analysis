...
public class ObserveOptionCancellationTest {
  ...
  @BeforeClass public static void init() throws Exception {
    ...
    testReceiver.writeMessage(reg1Request, new InetSocketAddress("localhost", COAP_SERVER_PORT));
    Thread.sleep(150);
    testReceiver.writeMessage(cancelGETrequest, new InetSocketAddress("localhost", COAP_SERVER_PORT));
    Thread.sleep(150);
    observableDummyWebService.setResourceStatus(true);
    Thread.sleep(150);
    testReceiver.writeMessage(reg2Request, new InetSocketAddress("localhost", COAP_SERVER_PORT));
    Thread.sleep(150);
    observableDummyWebService.setResourceStatus(true);
    Thread.sleep(150);
  ...
  @Test public void testReceiverReceivedRegistration1Notification1() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
  ...
  @Test public void testReceiverReceivedRegistration1ACKresponse() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
  ...
  @Test public void testReceiverReceivedRegistration2Notification1() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
    Iterator<Long> timeKeys = receivedMessages.keySet().iterator();
    timeKeys.next();
    timeKeys.next();
  ...
  @Test public void testReceiverReceivedRegistration2Notification2() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
    Iterator<Long> timeKeys = receivedMessages.keySet().iterator();
    timeKeys.next();
    timeKeys.next();
  ...
  @Test public void testReg2ObserveOptionSequenceIsSetProperly() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
    Iterator<Long> timeKeys = receivedMessages.keySet().iterator();
    timeKeys.next();
    timeKeys.next();
    ...
    String message = String.format("ObserveOption sequence in second " + "registration is not set properly (1st: %d, 2nd: %d)", observe1, observe2);
    assertTrue(message, observe1 < observe2);
  }

  @Test public void testObserveOptions() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
    Iterator<Long> timeKeys = receivedMessages.keySet().iterator();
  ...
  @Test public void testFirstNotificationHasSameMsgID() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
    CoapMessage receivedMessage = receivedMessages.get(receivedMessages.firstKey());
    String message = "First notification Msg ID does not match with request Msg ID";
  }

  @Test public void testReg2NotificationsHaveTheSameToken() {
    SortedMap<Long, CoapMessage> receivedMessages = testReceiver.getReceivedMessages();
    Iterator<Long> timeKeys = receivedMessages.keySet().iterator();
