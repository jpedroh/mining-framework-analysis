...
public class OptionalTest {
  @Test public void optionalNullIsNone() {
    assertTrue(optional(null) instanceof None);
  }

  @Test public void optionalValueIsSome() {
    assertTrue(optional("value") instanceof Some);
  }

  @Test public void iteratesOneTimeOverSome() {
    int iterations = 0;
  ...
  @Test @SuppressWarnings(value = { "unused" }) public void doesNotIterateOverNone() {
    for (Object value : none()) {
  ...
  @Test(expected = NoSuchElementException.class) public void throwsExceptionIfGettingAValueFromNone() {
    none().get();
  }

  @Test public void getTheValue() {
    assertThat(optional("value").get(), is("value"));
  }

  @Test public void mapTheValue() {
    assertThat(optional("    VALUE    ").map(lowerCased).map(trimmed).get(), is("value"));
  }

  @Test public void severalMappingsOfSomeValue() {
    assertThat(optional(" X ").map(lowerCased).map(trimmed).get(), is("x"));
  }

  @Test public void severalMappingOfNone() {
    assertThat(optional((String) null).map(lowerCased).map(trimmed).orElse("y"), is("y"));
  }

  @Test public void itIsPossibleToMapANone() {
  ...
  @Test(expected = NoSuchElementException.class) public void throwsExceptionWhenGettingAMappedNone() {
    String nonExistingString = null;
    optional(nonExistingString).map(upperCased).get();
  }

  @Test public void canAskIfValueIsPresent() {
    assertTrue(optional("value").isSome());
    assertFalse(optional(null).isSome());
  }

  @Test public void canDecideSomeOrNoneBasedOnPredicate() {
  ...
  @Test public void orElseOnNoneReturnsElse() {
    assertThat(optional((String) null).orElse("else"), is("else"));
  }

  @Test public void orElseOnSomeReturnsTheOptionalValue() {
    assertThat(optional("some").orElse("else"), is("some"));
  }

  @Test public void orNullOnNoneIsNull() {
    assertThat(optional(null).orNull(), nullValue());
  }

  @Test public void orNullOnSomeIsTheWrappedValue() {
    assertThat(optional("x").orNull(), is("x"));
  }

  @Test public void noneOrOtherOptional() {
    assertThat(optional(nonblank, "").or(optional("fallback")), contains("fallback"));
  }

  @Test public void someOrOtherOptional() {
    assertThat(optional("a").or(optional("anything")), contains("a"));
  }

  @Test(expected = UnsupportedOperationException.class) public void removingFromAnOptionalIteratorIsAbsurd() {
    optional("x").iterator().remove();
  }

  @Test public void mapperFunctionReturningNullCreatesNone() {
    Optional<?> none = optional("whatever").map(new Fn<Object, Object>() {
      @Override public Object $(Object value) {
        return null;
      }
    });
  ...
  @Test public void mappingToNoneWithFailingPredicate() {
    Optional<?> none = optional("   ").map(not(blank), trimmed);
    assertFalse(none.isSome());
  }

  @Test public void mappingNoneAlwaysGiveSameNone() {
    ...
    Optional<String> mappedNone2 = none.map(new Predicate<String>() {
      @Override public boolean $(String value) {
        return true;
      }
    }, upperCased);
  ...
  @Test public void filteringOptional() {
