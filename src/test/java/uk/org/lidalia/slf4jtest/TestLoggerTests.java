...
public class TestLoggerTests {
  ...
  @Test public void clearRemovesEvents() {
  ...
  @Test public void clearResetsLevel() {
  ...
  @Test public void traceMarkerMessageManyArgs() {
  ...
  @Test public void infoMarkerMessageManyArgs() {
  ...
  @Test public void loggerSetToOff() {
    logsIfEnabled();
  }

  @Test public void loggerSetToError() {
    logsIfEnabled(ERROR);
  }

  @Test public void loggerSetToWarn() {
    logsIfEnabled(ERROR, WARN);
  }

  @Test public void loggerSetToInfo() {
    logsIfEnabled(ERROR, WARN, INFO);
  }

  @Test public void loggerSetToDebug() {
    logsIfEnabled(ERROR, WARN, INFO, DEBUG);
  }

  @Test public void loggerSetToTrace() {
  ...
  private void logsIfEnabled(Level... shouldLog) {
    ...
    List<LoggingEvent> expectedEvents = Lists.transform(asList(shouldLog), new Function<Level, LoggingEvent>() {
      @Override public LoggingEvent apply(Level level) {
  ...
  @Test public void getLoggingEventsReturnsCopyNotView() {
    testLogger.debug(message);
  ...
  @Test(expected = UnsupportedOperationException.class) public void getLoggingEventsReturnsUnmodifiableList() {
    List<LoggingEvent> loggingEvents = testLogger.getLoggingEvents();
    loggingEvents.add(debug("hello"));
  }

  @Test public void getLoggingEventsOnlyReturnsEventsLoggedInThisThread() throws InterruptedException {
  ...
  @Test public void getAllLoggingEventsReturnsEventsLoggedInAllThreads() throws InterruptedException {
    Thread t = new Thread(new Runnable() {
      @Override public void run() {
        testLogger.info(message);
      }
  ...
  @Test public void clearOnlyClearsEventsLoggedInThisThread() throws InterruptedException {
    Thread t = new Thread(new Runnable() {
      @Override public void run() {
        testLogger.info(message);
      }
  ...
  @Test public void clearAllClearsEventsLoggedInAllThreads() throws InterruptedException {
    testLogger.info(message);
    Thread t = new Thread(new Runnable() {
      @Override public void run() {
        testLogger.info(message);
  ...
  @Test public void setEnabledLevelOnlyChangesLevelForCurrentThread() throws Exception {
  ...
  @Test public void clearOnlyChangesLevelForCurrentThread() throws Exception {
    testLogger.setEnabledLevels(Level.WARN, Level.ERROR);
    Thread t = new Thread(new Runnable() {
      @Override public void run() {
        testLogger.clear();
  ...
  @Test public void setEnabledLevelsForAllThreads() throws Exception {
    final AtomicReference<ImmutableSet<Level>> inThreadEnabledLevels = new AtomicReference<ImmutableSet<Level>>();
    Thread t = new Thread(new Runnable() {
      @Override public void run() {
  ...
  @Test public void clearAllChangesAllLevels() throws Exception {
    testLogger.setEnabledLevels(Level.WARN, Level.ERROR);
    Thread t = new Thread(new Runnable() {
      @Override public void run() {
        testLogger.clearAll();
  ...
  private void assertEnabledReturnsCorrectly(Level levelToTest) {
    ...
    assertTrue("Logger level set to " + levelToTest + " means " + levelToTest + " should be enabled", new Logger(testLogger).isEnabled(levelToTest));
    Set<Level> disabledLevels = difference(enablableValueSet(), newHashSet(levelToTest));
    for (Level disabledLevel : disabledLevels) {
      assertFalse("Logger level set to " + levelToTest + " means " + levelToTest + " should be disabled", new Logger(testLogger).isEnabled(disabledLevel));
    }
  }

  private void assertEnabledReturnsCorrectly(Level levelToTest, Marker marker) {
    testLogger.setEnabledLevels(levelToTest);
    assertTrue("Logger level set to " + levelToTest + " means " + levelToTest + " should be enabled", new Logger(testLogger).isEnabled(levelToTest, marker));
    Set<Level> disabledLevels = difference(enablableValueSet(), newHashSet(levelToTest));
    for (Level disabledLevel : disabledLevels) {
      assertFalse("Logger level set to " + levelToTest + " means " + levelToTest + " should be disabled", new Logger(testLogger).isEnabled(disabledLevel, marker));
    }
  }
}