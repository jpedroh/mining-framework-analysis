...
public class BasicFunctionalityUnitTest {
  ...
  @Test public void testAnythingBut() {
  ...
  @Test public void testSomethingBut() {
  ...
  @Test public void testStartOfLine() {
  ...
  @Test public void testStartOfLineFalse() {
    VerbalExpression testRegex = regex().startOfLine(false).then("a").build();
    assertThat(testRegex, matchesTo("ba"));
    assertThat(testRegex, matchesTo("ab"));
  }

  @Test public void testRangeWithMultiplyRanges() throws Exception {
  ...
  @Test public void testEndOfLine() {
  ...
  @Test public void testEndOfLineIsFalse() {
    VerbalExpression testRegex = regex().find("a").endOfLine(false).build();
    assertThat(testRegex, matchesTo("ba"));
    assertThat(testRegex, matchesTo("ab"));
  }

  @Test public void testMaybe() {
  ...
  @Test public void testAnySameAsAnyOf() {
  ...
  @Test public void testMultipleNoRange() {
    ...
    VerbalExpression[] testRegexesSameBehavior = { testRegexStringOnly, testRegexStringAndNull, testRegexMoreThan2Ints };
    for (VerbalExpression testRegex : testRegexesSameBehavior) {
      assertThat("abc once", testRegex, matchesTo("abc"));
      assertThat("abc more than once", testRegex, matchesTo("abcabcabc"));
      assertThat("no abc", testRegex, not(matchesTo("xyz")));
    }
  }

  @Test public void testMultipleFrom() {
    VerbalExpression testRegexFrom = new VerbalExpression.Builder().startOfLine().multiple("abc", 2).build();
    assertThat("no abc", testRegexFrom, not(matchesTo("xyz")));
    assertThat("abc less than 2 times", testRegexFrom, not(matchesTo("abc")));
    assertThat("abc exactly 2 times", testRegexFrom, matchesTo("abcabc"));
    assertThat("abc more than 2 times", testRegexFrom, matchesTo("abcabcabc"));
  }

  @Test public void testMultipleFromTo() {
    ...
    assertThat("abc less than 2 times", testRegexFromTo, not(matchesTo("abc")));
    assertThat("abc exactly 2 times", testRegexFromTo, matchesTo("abcabc"));
    assertThat("abc between 2 and 4 times", testRegexFromTo, matchesTo("abcabcabc"));
    assertThat("abc exactly 4 times", testRegexFromTo, matchesTo("abcabcabcabc"));
    assertThat("abc more than 4 times", testRegexFromTo, not(matchesExactly("abcabcabcabcabc")));
  }

  @Test public void testWithAnyCase() {
    VerbalExpression testRegex = new VerbalExpression.Builder().startOfLine().then("a").build();
  ...
  @Test public void testWithAnyCaseIsFalse() {
    VerbalExpression testRegex = regex().withAnyCase().startOfLine().then("a").withAnyCase(false).build();
    assertThat(testRegex, not(matchesTo("A")));
  }

  @Test public void testSearchOneLine() {
    VerbalExpression testRegex = regex().startOfLine().then("a").br().then("b").endOfLine().build();
  ...
  @Test public void testStartCapture() {
    String text = "aaabcd";
    VerbalExpression regex = regex().find("a").count(3).capture().find("b").anything().build();
  ...
  @Test public void captIsSameAsCapture() {
    assertThat("Capt produce defferent than capture regex", regex().capt().build().toString(), equalTo(regex().capture().build().toString()));
  }

  @Test public void shouldReturnEmptyStringWhenNoGroupFound() {
    String text = "abc";
  ...
  @Test public void testCountWithRange() {
  ...
  @Test public void testEndCapture() {
    String text = "aaabcd";
    VerbalExpression regex = regex().find("a").capture().find("b").anything().endCapture().then("cd").build();
    assertThat(regex.getText(text), equalTo("abcd"));
    assertThat("can\'t get first captured group", regex.getText(text, 1), equalTo("b"));
  }

  @Test public void testMultiplyCapture() {
    String text = "aaabcd";
    VerbalExpression regex = regex().find("a").count(1).capture().find("b").endCapture().anything().capture().find("d").build();
    assertThat("can\'t get first captured group", regex.getText(text, 1), equalTo("b"));
    assertThat("can\'t get second captured group", regex.getText(text, 2), equalTo("d"));
  }

  @Test public void testOrWithCapture() {
    VerbalExpression testRegex = regex().capture().find("abc").or("def").build();
    assertThat("Starts with abc or def", testRegex, matchesTo("defzzz"));
  ...
  @Test public void testOrWithClosedCapture() {
    VerbalExpression testRegex = regex().capture().find("abc").endCapt().or("def").build();
    assertThat("Starts with abc or def", testRegex, matchesTo("defzzz"));
    assertThat("Starts with abc or def", testRegex, matchesTo("abczzz"));
    assertThat("Doesn\'t start with abc or def", testRegex, not(matchesExactly("xyzabcefg")));
    assertThat(testRegex.getText("xxxabcdefzzz", 1), equalTo("abcnull"));
  ...
  @Test public void addRegexBuilderWrapsItWithUnsavedGroup() throws Exception {
    VerbalExpression regex = regex().add(regex().capt().find("string").count(2).endCapt().count(1).digit()).count(2).build();
    assertThat("Added regex builder don\'t wrapped with unsaved group", regex.toString(), startsWith("(?:((?:string"));
  ...
  @Test public void multiplyWith1NumProduceSameAsCountResult() throws Exception {
    VerbalExpression regex = regex().multiple("a", 1).build();
    assertThat(regex, equalToRegex(regex().find("a").count(1)));
  }

  @Test public void multiplyWith2NumProduceSameAsCountRangeResult() throws Exception {
    VerbalExpression regex = regex().multiple("a", 1, 2).build();
    assertThat(regex, equalToRegex(regex().find("a").count(1, 2)));
  }

  @Test public void atLeast1HaveSameEffectAsOneOrMore() throws Exception {
  ...
  @Test public void oneOreMoreSameAsAtLeast1() throws Exception {
  ...
  @Test public void atLeast0HaveSameEffectAsZeroOrMore() throws Exception {
  ...
  @Test public void zeroOreMoreSameAsAtLeast0() throws Exception {
  ...
  @Test public void testOneOfWithCapture() {
    VerbalExpression testRegex = regex().capture().oneOf("abc", "def").build();
    assertThat("Starts with abc or def", testRegex, matchesTo("defzzz"));
    assertThat("Starts with abc or def", testRegex, matchesTo("abczzz"));
    assertThat("Doesn\'t start with abc or def", testRegex, not(matchesExactly("xyzabcefg")));
  ...
  @Test public void testOneOfWithClosedCapture() {
    VerbalExpression testRegex = regex().capture().oneOf("abc", "def").endCapt().build();
    assertThat("Starts with abc or def", testRegex, matchesTo("defzzz"));
    assertThat("Starts with abc or def", testRegex, matchesTo("abczzz"));
    assertThat("Doesn\'t start with abc or def", testRegex, not(matchesExactly("xyzabcefg")));
    assertThat(testRegex.getText("xxxabcdefzzz", 1), equalTo("abcdef"));
  ...
  @Test public void shouldAddMaybeWithOneOfFromAnotherBuilder() {
    VerbalExpression.Builder namePrefix = regex().oneOf("Mr.", "Ms.");
    VerbalExpression name = regex().maybe(namePrefix).space().zeroOrMore().word().oneOrMore().build();
