...
public class LogstashEncoderTest {
  ...
  @Test public void basicsAreIncluded() throws Exception {
  ...
  @Test public void customDecorators() throws Exception {
    ...
    assertThat(output, is(String.format("{%n" + "  @timestamp : \"" + FastDateFormat.getInstance("yyyy-MM-dd\'T\'HH:mm:ss.SSSZZ").format(timestamp) + "\",%n" + "  @version : 1,%n" + "  message : \"My message\",%n" + "  logger_name : \"LoggerName\",%n" + "  thread_name : \"ThreadName\",%n" + "  level : \"ERROR\",%n" + "  level_value : 40000%n" + "}%n")));
  }
  ...
  @Test public void closePutsSeparatorAtTheEnd() throws Exception {
    ILoggingEvent event = mockBasicILoggingEvent(Level.ERROR);
    encoder.doEncode(event);
    encoder.close();
    closeQuietly(outputStream);
  ...
  @Test public void customTimeZone() throws Exception {
    ...
    closeQuietly(outputStream);
    JsonNode node = MAPPER.readTree(outputStream.toByteArray());
    assertThat(node.get("@timestamp").textValue(), is(FastDateFormat.getInstance("yyyy-MM-dd\'T\'HH:mm:ss.SSSZZ", TimeZone.getTimeZone("UTC")).format(timestamp)));
    assertThat(node.get("@version").intValue(), is(1));
    assertThat(node.get("logger_name").textValue(), is("LoggerName"));
    assertThat(node.get("thread_name").textValue(), is("ThreadName"));
    assertThat(node.get("message").textValue(), is("My message"));
