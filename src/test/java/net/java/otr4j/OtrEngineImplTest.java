...
public class OtrEngineImplTest extends junit.framework.TestCase {
  ...
  class DummyOtrEngineHost implements OtrEngineHost {
    ...
    public void smpAborted(SessionID sessionID) throws OtrException {
      logger.severe("SM verification has been aborted by user: " + sessionID);
    }

    public void finishedSessionMessage(SessionID sessionID, String msgText) throws OtrException {
      logger.severe("SM session was finished. You shouldn\'t send messages to: " + sessionID);
    }

    public void requireEncryptedMessage(SessionID sessionID, String msgText) throws OtrException {
      logger.severe("Message can\'t be sent while encrypted session is not established: " + sessionID);
    }
    ...
    public void unencryptedMessageReceived(SessionID sessionID, String msg) throws OtrException {
      logger.warning("Unencrypted message received: " + msg + " from " + sessionID);
    }
    ...
    public void verify(SessionID sessionID, String fingerprint, boolean approved) {
      ...
      if (!approved) {
        logger.finest("Your answer for the question was verified." + "You should ask your opponent too or check shared secret.");
      }
    }
    ...
    public void messageFromAnotherInstanceReceived(SessionID sessionID) {
      logger.warning("You received an encrypted message that was intended for another session. If you\'re logged in " + "from multiple locations, another session may have received this message");
    }

    public void multipleInstancesDetected(SessionID sessionID) {
      logger.warning("Your buddy is logged in multiple times and OTR has established multiple sessions. You can select an outgoing " + "session from the menu above");
    }
  }
