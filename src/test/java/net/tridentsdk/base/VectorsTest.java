...
@State(value = Scope.Benchmark) public class VectorsTest {
  ...
  @Test public void testSetsEqualsHash() {
  ...
  @Test public void testToPosition() {
  ...
  public static void m2() {
    Options opt = new OptionsBuilder().include(".*" + VectorsTest.class.getSimpleName() + ".*").timeUnit(TimeUnit.SECONDS).mode(Mode.Throughput).operationsPerInvocation(1).warmupIterations(20).measurementIterations(5).forks(1).threads(4).build();
  ...
  @Setup(value = Level.Iteration) public void setup() {
  ...
  @Benchmark public void addT() {
    this.vec.add(this.adder);
  }

  @TearDown(value = Level.Trial) public void teardown(Blackhole bh) {
