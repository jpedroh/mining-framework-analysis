...
public class AntTest {
  ...
  /**
     ...
     */
  @Test public void testGlobalConfigAjax() throws Exception {
    HtmlPage p = Jenkins.getVersion().toString().startsWith("2") ? r.createWebClient().goTo("configureTools") : r.createWebClient().goTo("configure");
  ...
  @Test public void testSensitiveParameters() throws Exception {
  ...
  @Test public void testParameterExpansion() throws Exception {
    if (!SystemUtils.IS_OS_WINDOWS) {
      String antName = configureDefaultAnt().getName();
      MatrixProject project = r.createProject(MatrixProject.class, "test project");
      ...
      project.getBuildersList().add(new Ant("", antName, null, null, "vNUM=$BUILD_NUMBER\nvID=$BUILD_ID\nvJOB=$JOB_NAME\nvTAG=$BUILD_TAG\nvEXEC=$EXECUTOR_NUMBER\n" + "vNODE=$NODE_NAME\nvLAB=$NODE_LABELS\nvJAV=$JAVA_HOME\nvWS=$WORKSPACE\nvHURL=$HUDSON_URL\n" + "vBURL=$BUILD_URL\nvJURL=$JOB_URL\nvHH=$HUDSON_HOME\nvJH=$JENKINS_HOME\nvFOO=$FOO\nvAX=$AX"));
  ...
  @Test public void testParameterExpansionByShell() throws Exception {
    String antName = configureDefaultAnt().getName();
    FreeStyleProject project = r.createFreeStyleProject();
    project.setScm(new ExtractResourceSCM(getClass().getResource("ant-job.zip")));
    String homeVar = Functions.isWindows() ? "%HOME%" : "$HOME";
    project.addProperty(new ParametersDefinitionProperty(new StringParameterDefinition("vFOO", homeVar, ""), new StringParameterDefinition("vBAR", "Home sweet " + homeVar + ".", "")));
    project.getBuildersList().add(new Ant("", antName, null, null, "vHOME=" + homeVar + "\nvFOOHOME=Foo " + homeVar + "\n"));
    FreeStyleBuild build = project.scheduleBuild2(0).get();
    ...
    assertTrue("Missing simple HOME parameter: " + log, log.matches("(?s).*vFOO=(?!" + homeVar + ").*"));
    assertTrue("Missing HOME parameter with other text: " + log, log.matches("(?s).*vBAR=Home sweet (?!" + homeVar + ")[^\\r\\n]*\\..*"));
    assertTrue("Missing HOME ant property: " + log, log.matches("(?s).*vHOME=(?!" + homeVar + ").*"));
    assertTrue("Missing HOME ant property with other text: " + log, log.matches("(?s).*vFOOHOME=Foo (?!" + homeVar + ").*"));
  }

  @Issue(value = "JENKINS-7108") @Test public void testEscapeXmlInParameters() throws Exception {
    String antName = configureDefaultAnt().getName();
    FreeStyleProject project = r.createFreeStyleProject();
    project.setScm(new ExtractResourceSCM(getClass().getResource("ant-job.zip")));
    project.addProperty(new ParametersDefinitionProperty(new StringParameterDefinition("vFOO", "<xml/>", "")));
  ...
  @Test public void invokeCustomTargetTest() throws Exception {
  ...
  @Test public void invokeDefaultTargetTest() throws Exception {
  ...
  @Test public void jenkinsEnvVarsFromBuildScriptTest() throws Exception {
  ...
  @Test public void optionsInTargetFieldTest() throws Exception {
  ...
  @Test public void customBuildFileTest() throws Exception {
  ...
  @Test public void unexistingCustomBuildFileTest() throws Exception {
  ...
  @Test @Issue(value = "JENKINS-33712") public void emptyParameterTest() throws Exception {
    FreeStyleProject project = createSimpleAntProject("", null, null, "property=");
    FreeStyleBuild build = project.scheduleBuild2(0).get();
    assertEquals(Result.SUCCESS, build.getResult());
  }

  @Test public void propertyReplacedByVariable() throws Exception {
    testVariableReplaced("x");
  }

  @Test @Issue(value = "JENKINS-41801") public void propertyReplacedByEmptyBuildParameter() throws Exception {
  ...
  private void testVariableReplaced(String variableValue) throws Exception {
    ...
    FreeStyleBuild build = project.scheduleBuild2(0, new Cause.UserIdCause(), new ParametersAction(new StringParameterValue("variable", variableValue))).get();
    assertEquals(Result.SUCCESS, build.getResult());
    List<String> logs = build.getLog(Integer.MAX_VALUE);
    String commandLineLine = null;
    for (int i = 0; i < logs.size(); i++) {
      String line = logs.get(i);
      if (line.contains("ant") && line.contains("build-properties.xml") && line.contains("-DtestProperty=")) {
    ...
    commandLineLine += " ";
    assertTrue("-DtestProperty param not \'" + variableValue + "\': " + commandLineLine, commandLineLine.contains("-DtestProperty=\"" + variableValue + "\" ") || commandLineLine.contains("-DtestProperty=" + variableValue + " "));
  }
