...
public class NoActivityTimeOutStrategyTest {
  ...
  public static class PollingBuilder extends Builder {
    ...
    private void log(BuildListener listener, long cur, String message) {
      String str = String.format("[%s] %s", (new SimpleDateFormat("HH:mm:ss.SSS")).format(new Date(cur)), message);
  ...
  @Test public void timeout() throws Exception {
    ...
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new NoActivityTimeOutStrategy(5), true, false));
  ...
  @Test public void noTimeout() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new NoActivityTimeOutStrategy(15), true, false));
    p.getBuildersList().add(new PollingBuilder(10 * 1000, 30 * 1000));
  ...
  @Test public void performedOnlyOnce() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    CountOperation count = new CountOperation();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new NoActivityTimeOutStrategy(3), Arrays.<BuildTimeOutOperation>asList(count), null));
  ...
  @Test public void canConfigureWithWebPage() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new NoActivityTimeOutStrategy("60"), Arrays.<BuildTimeOutOperation>asList(new AbortOperation()), null));
    p.save();
  ...
  @Test public void canConfigureWithWebPageUsingStringExpression() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new NoActivityTimeOutStrategy("${TEST}"), Arrays.<BuildTimeOutOperation>asList(new AbortOperation()), null));
    p.save();
    String projectName = p.getFullName();
    {
      NoActivityTimeOutStrategy strategy = (NoActivityTimeOutStrategy) p.getBuildWrappersList().get(BuildTimeoutWrapper.class).getStrategy();
...
<<<<<<< /usr/src/app/output/jenkinsci/build-timeout-plugin/d51178be1cd90cf9173edd1705a7bf0dc75ab16d/src/test/java/hudson/plugins/build_timeout/impl/NoActivityTimeOutStrategyTest.java/left.java
  @Test public void testConfigurationWithParameter() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    p.addProperty(new ParametersDefinitionProperty(new StringParameterDefinition("TIMEOUT", null)));
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new NoActivityTimeOutStrategy("${TIMEOUT}"), Arrays.<BuildTimeOutOperation>asList(new AbortOperation()), null));
    p.getBuildersList().add(new PollingBuilder(10 * 1000, 30 * 1000));
    j.assertBuildStatus(Result.SUCCESS, p.scheduleBuild2(0, new Cause.UserCause(), new ParametersAction(new StringParameterValue("TIMEOUT", "15"))).get());
    j.assertBuildStatus(Result.ABORTED, p.scheduleBuild2(0, new Cause.UserCause(), new ParametersAction(new StringParameterValue("TIMEOUT", "5"))).get());
  }
=======
>>>>>>> Unknown file: This is a bug in JDime.


  @Test public void configurationWithParameter() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new NoActivityTimeOutStrategy("${TIMEOUT}"), Arrays.<BuildTimeOutOperation>asList(new AbortOperation()), null));
    p.getBuildersList().add(new PollingBuilder(10 * 1000, 30 * 1000));
    j.assertBuildStatus(Result.SUCCESS, p.scheduleBuild2(0, new Cause.UserCause(), new ParametersAction(new StringParameterValue("TIMEOUT", "15"))).get());
