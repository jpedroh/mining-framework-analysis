...
public class DeadlineTimeOutStrategyTest {
  ...
  @SuppressWarnings(value = { "deprecation" }) private void testWithParam(int timeToDeadlineInSecondsFromNow, Result expectedResult) throws Exception {
    ...
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new DeadlineTimeOutStrategy("${DEADLINE}", TOLERANCE_PERIOD_IN_MINUTES), Arrays.<BuildTimeOutOperation>asList(new AbortOperation()), null));
    p.getBuildersList().add(new SleepBuilder(5000));
    j.assertBuildStatus(expectedResult, p.scheduleBuild2(0, new Cause.UserCause(), new ParametersAction(new StringParameterValue("DEADLINE", deadline))).get());
  }
