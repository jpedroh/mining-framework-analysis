...
public class BuildStepOperationTest {
  ...
  public static class TestBuilder extends Builder {
    ...
    @Override public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws InterruptedException, IOException {
      listener.getLogger().println(String.format("%s is executed: times=%d", getClass().getName(), ++executed));
      return result;
    }
  }
  ...
  @Test public void disabled() throws Exception {
    BuildStepOperation.DescriptorImpl d = (BuildStepOperation.DescriptorImpl) j.jenkins.getDescriptorOrDie(BuildStepOperation.class);
  ...
  @Test public void builder() throws Exception {
    ...
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new QuickBuildTimeOutStrategy(5000), Arrays.<BuildTimeOutOperation>asList(new BuildStepOperation(builder1, true), new BuildStepOperation(builder2, true))));
  ...
  @Test public void builderFailContinue() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    TestBuilder builder1 = new TestBuilder();
    TestBuilder builder2 = new TestBuilder();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new QuickBuildTimeOutStrategy(5000), Arrays.<BuildTimeOutOperation>asList(new BuildStepOperation(builder1, true), new BuildStepOperation(builder2, true))));
  ...
  @Test public void builderFailStop() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    TestBuilder builder1 = new TestBuilder();
    TestBuilder builder2 = new TestBuilder();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new QuickBuildTimeOutStrategy(5000), Arrays.<BuildTimeOutOperation>asList(new BuildStepOperation(builder1, false), new BuildStepOperation(builder2, true))));
    p.getBuildersList().add(new SleepBuilder(9999));
    builder1.result = false;
    assertEquals(0, builder1.executed);
    assertEquals(0, builder2.executed);
  ...
  @Test public void publisher() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    TestPublisher publisher1 = new TestPublisher();
    TestPublisher publisher2 = new TestPublisher();
    p.getBuildWrappersList().add(new BuildTimeoutWrapper(new QuickBuildTimeOutStrategy(5000), Arrays.<BuildTimeOutOperation>asList(new BuildStepOperation(publisher1, true), new BuildStepOperation(publisher2, true))));
    p.getBuildersList().add(new SleepBuilder(9999));
  ...
  @Test public void configuration() throws Exception {
    ...
    BuildTimeoutWrapper timeout = new BuildTimeoutWrapper(new AbsoluteTimeOutStrategy(3), Arrays.<BuildTimeOutOperation>asList(op));
  ...
  @Test public void configurationWithoutDbc() throws Exception {
...
<<<<<<< /usr/src/app/output/jenkinsci/build-timeout-plugin/d51178be1cd90cf9173edd1705a7bf0dc75ab16d/src/test/java/hudson/plugins/build_timeout/operations/BuildStepOperationTest.java/left.java
  @Test public void testLauncher() throws Exception {
    ...
    BuildTimeoutWrapper timeout = new BuildTimeoutWrapper(new QuickBuildTimeOutStrategy(5000), Arrays.<BuildTimeOutOperation>asList(op), null);
    p.getBuildWrappersList().add(timeout);
    p.getBuildersList().add(new SleepBuilder(9999));
    j.assertBuildStatusSuccess(p.scheduleBuild2(0, new Cause.UserCause(), new ParametersAction(new StringParameterValue("TESTSTRING", TESTSTRING))));
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  @Test public void noLauncher() throws Exception {
    FreeStyleProject p = j.createFreeStyleProject();
    String TESTSTRING = "***THIS IS OUTPUT IN TIMEOUT***";
    Builder shell = null;
    if (Functions.isWindows()) {
    ...
    p.getBuildWrappersList().add(timeout);
    p.getBuildersList().add(new SleepBuilder(9999));
    j.assertBuildStatus(Result.FAILURE, p.scheduleBuild2(0, new Cause.UserCause(), new ParametersAction(new StringParameterValue("TESTSTRING", TESTSTRING))).get());
  }
  ...
  public static class NoDataBoundConstructorBuilder extends Builder {
    ...
    @Extension public static class DescriptorImpl extends BuildStepDescriptor<Builder> {
      @Override public boolean isApplicable(@SuppressWarnings(value = { "rawtypes" }) Class<? extends AbstractProject> jobType) {
        return true;
      }
      ...
      @Override public NoDataBoundConstructorBuilder newInstance(StaplerRequest req, JSONObject formData) throws FormException {
        NoDataBoundConstructorBuilder builder = new NoDataBoundConstructorBuilder();
