 ...
 * Time: 0:57
 */
@RunWith(value = MockitoJUnitRunner.class) public class ChangingWorkflowTest {
  ...
  @Mock private JiraRestService restService;

  @Mock private JiraSession mockSession;

  @Mock private TaskListener mockTaskListener;
  ...
  @Before public void setupSpy() throws Exception {
    ...
    Set<? extends ChangeLogSet.Entry> entries = Sets.newHashSet(new MockEntry("Fixed JI123-4711"), new MockEntry("Fixed foo_bar-4710"), new MockEntry("Fixed FoO_bAr-4711"), new MockEntry("Fixed something.\nJFoO_bAr_MULTI-4718"), new MockEntry("TR-123: foo"), new MockEntry("[ABC-42] hallo"), new MockEntry("#123: this one must not match"), new MockEntry("ABC-: this one must also not match"), new MockEntry("ABC-: \n\nABC-127:\nthis one should match"), new MockEntry("ABC-: \n\nABC-128:\nthis one should match"), new MockEntry("ABC-: \n\nXYZ-10:\nXYZ-20 this one too"), new MockEntry("Fixed DOT-4."), new MockEntry("Fixed DOT-5. Did it right this time"));
  ...
  @Test public void onGetActionItInvokesServiceMethod() throws Exception {
  ...
  @Test public void getActionIdReturnsNullWhenServiceReturnsNull() throws Exception {
  ...
  @Test public void getActionIdIteratesOverAllActionsEvenOneOfNamesIsNull() throws Exception {
    ...
    verify(action2, times(2)).getName();
  }

  @Test public void getActionIdReturnsNullWhenNullWorkflowUsed() throws Exception {
  ...
  @Test public void getActionIdReturnsIdWhenFoundIgnorecaseWorkflow() throws Exception {
  ...
  @Test public void addCommentsOnNonEmptyWorkflowAndNonEmptyComment() throws Exception {
    ...
    when(mockSession.getActionIdForIssue(any(), eq(NON_EMPTY_WORKFLOW_LOWERCASE))).thenReturn(Integer.valueOf(randomNumeric(5)));
...
<<<<<<< /usr/src/app/output/jenkinsci/jira-plugin/474f5432d56bf2a59273c767db40d6a3d5806ff0/src/test/java/hudson/plugins/jira/ChangingWorkflowTest.java/left.java
    when(site).progressMatchingIssues(anyString(), anyString(), anyString(), any(PrintStream.class))
=======
    thenCallRealMethod()
>>>>>>> /usr/src/app/output/jenkinsci/jira-plugin/474f5432d56bf2a59273c767db40d6a3d5806ff0/src/test/java/hudson/plugins/jira/ChangingWorkflowTest.java/right.java
    ;
    site.progressMatchingIssues(ISSUE_JQL, NON_EMPTY_WORKFLOW_LOWERCASE, NON_EMPTY_COMMENT, mock(PrintStream.class));
  ...
  @Test public void addCommentsOnNullWorkflowAndNonEmptyComment() throws Exception {
    FieldSetter.setField(site, JiraSite.class.getDeclaredField("jiraSession"), mockSession);
    when(mockSession.getIssuesFromJqlSearch(anyString())).thenReturn(Arrays.asList(mock(Issue.class)));
    when(site.progressMatchingIssues(anyString(), any(), anyString(), any(PrintStream.class))).thenCallRealMethod();
    site.progressMatchingIssues(ISSUE_JQL, "Workflow", NON_EMPTY_COMMENT, mock(PrintStream.class));
    verify(mockSession, times(1)).addComment(any(), eq(NON_EMPTY_COMMENT), isNull(), isNull());
  }

  @Test public void dontAddCommentsOnNullWorkflowAndNullComment() throws IOException, TimeoutException {
