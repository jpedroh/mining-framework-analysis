...
public class DiskUsageCalculationTest extends TestCase {
  /**
     ...
     * see @testReschedule()
     */
  @Test public void testScheduledExecutionTime() throws Exception {
  ...
  @Test public void testGetRecurrencePeriod() {
    GregorianCalendar calendar = new GregorianCalendar();
    int minute = calendar.get(Calendar.MINUTE);
    minute = minute + 2;
  ...
  /**
     ...
     */
  @Test public void testReschedule() throws Exception {
    GregorianCalendar calendar = new GregorianCalendar();
    calendar.add(Calendar.MINUTE, 10);
    int minute = calendar.get(Calendar.MINUTE);
    TestDiskUsageCalculation calculation = (TestDiskUsageCalculation) new TestDiskUsageCalculation(minute + " * * * *", true).getNewInstance();
    Timer.get().schedule(calculation, calculation.getRecurrencePeriod(), TimeUnit.MILLISECONDS);
    calendar.add(Calendar.MINUTE, 10);
    minute = calendar.get(Calendar.MINUTE);
    calculation.setCron(minute + " * * * *");
    calculation.reschedule();
  ...
  @Test public void testTaskIsScheduledOnlyOneTimesPerMinute() throws Exception {
    ...
    while ((seconds > 55 && seconds < 59) || seconds == 0) {
      if (firstLoop) {
