...
public class ExtendedEmailPublisherDescriptorJCasCTest {
  ...
  @Test @ConfiguredWithCode(value = "configuration-as-code.yml") public void shouldValidatedJCasCConfiguration() throws Exception {
    final ExtendedEmailPublisherDescriptor descriptor = ExtensionList.lookupSingleton(ExtendedEmailPublisherDescriptor.class);
  ...
  @Test @ConfiguredWithCode(value = "configuration-as-code-with-triggers.yml") public void shouldBeAbleToConfigureTriggers() throws Exception {
    final ExtendedEmailPublisherDescriptor descriptor = ExtensionList.lookupSingleton(ExtendedEmailPublisherDescriptor.class);
    assertNotNull(descriptor);
    final List<String> expectedTriggers = Arrays.asList(RegressionTrigger.class.getName(), AbortedTrigger.class.getName(), FixedTrigger.class.getName());
    MatcherAssert.assertThat(descriptor.getDefaultTriggerIds(), Matchers.contains(expectedTriggers.stream().map(Matchers::equalTo).collect(Collectors.toList())));
  }
}