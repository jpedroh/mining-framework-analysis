 ...
 */
@SuppressWarnings(value = { "deprecation" }) public final class ByteList implements Comparable, CharSequence, Serializable {
  ...
  @Deprecated public Encoding encoding = ASCIIEncoding.INSTANCE;
  ...
  /**
     ...
     */
  @Deprecated public ByteList(ByteList wrap, boolean copy) {
  ...
  public boolean startsWith(ByteList other, int toffset) {
    if (realSize == 0 || this.realSize < other.realSize + toffset) {
      return false;
    }
    byte[] ta = bytes;
  ...
  public boolean equal(ByteList other) {
    ...
    if ((last = realSize) == other.realSize) {
      ...
      for (first = -1; --last > first && buf[begin + last] == otherBuf[other.begin + last] && ++first < last && buf[begin + first] == otherBuf[other.begin + first]; ) {
        ;
      }
      return first >= last;
    }
  ...
  public boolean sample_equals(Object other) {
    ...
    if (other instanceof ByteList) {
      ...
      if ((size = realSize) == b.realSize) {
        for (buf = bytes, first = -1, last = (size + 1) & ~1; (last -= 2) >= 0 && buf[begin + last] == b.bytes[b.begin + last] && (first += 2) < size && buf[begin + first] == b.bytes[b.begin + first]; ) {
          ;
        }
  ...
  @Override public int hashCode() {
    ...
    while (index < end) {
      key = ((key << 16) + (key << 6) - key) + (int) (bytes[index++]);
    }
    key = key + (key >> 5);
    return hash = key;
  }
