...
public class ASMExtractor implements Extractor {
  ...
  private void processClass(String className, URLClassLoader loader) throws IOException {
    ...
    if (in == null) {
      in = new FileInputStream(new File(className).getAbsoluteFile());
    }
    ...
    try {
      ...
      if (resultFile.isDirectory()) {
        if (internalPath.length > 1) {
          throw new IOException("Incomplete Path");
        }
        processDir(resultFile);
      } else {
        if (resultFile.getAbsolutePath().endsWith(".war") || resultFile.getAbsolutePath().endsWith(".jar")) {
          if (internalPath.length == 1) {
            processEar(resultFile.getAbsolutePath(), "");
          } else {
            processEar(resultFile.getAbsolutePath(), Arrays.copyOfRange(internalPath, 1, internalPath.length));
          }
        } else {
      ...
      if (file.isDirectory()) {
        ...
        if (file.getAbsolutePath().endsWith(".jar")) {
          ...
          if (file.getAbsolutePath().endsWith(".class")) {
            ...
            if (file.getAbsolutePath().endsWith(".ear") || file.getAbsolutePath().endsWith(".war")) {
  ...
  @Override public void processClass(String classFilePath) throws IOException {
    URLClassLoader classLoader = new URLClassLoader(new URL[] { new File(classFilePath).toURI().toURL() });
