...
public class EnderPlayer extends EnderEntity implements CommandSender, Player {
  ...
  @Override public void onSpawn() {
    ...
    this.inventoryHandler.tryPickup(new ItemStack(BlockId.DIAMOND_SWORD.getId(), (byte) 1, (short) 0));
  ...
  @Override public Packet[] getSpawnPackets() {
    ...
    if (this.getInventory().getItemInHand() != null) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 0, this.getInventory().getItemInHand()));
    }
    if (handler.getArmor().get(0) != null) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 4, handler.getArmor().get(0)));
    }
    if (handler.getArmor().get(1) != null) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 3, handler.getArmor().get(0)));
    }
    if (handler.getArmor().get(2) != null) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 2, handler.getArmor().get(0)));
    }
    if (handler.getArmor().get(3) != null) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 1, handler.getArmor().get(0)));
    }
  ...
  public void broadcastEquipment(EquipmentUpdateType type) {
    ...
    if ((type == EquipmentUpdateType.HELMET_CHANGE || type == EquipmentUpdateType.ALL)) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 4, handler.get(0)));
    }
    if ((type == EquipmentUpdateType.CHESTPLATE_CHANGE || type == EquipmentUpdateType.ALL)) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 3, handler.get(0)));
    }
    if ((type == EquipmentUpdateType.LEGGINGS_CHANGE || type == EquipmentUpdateType.ALL)) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 2, handler.get(0)));
    }
    if ((type == EquipmentUpdateType.BOOTS_CHANGE || type == EquipmentUpdateType.ALL)) {
      toSend.add(new PacketOutEntityEquipment(this.getEntityId(), (short) 1, handler.get(0)));
    }
  ...
  @Override public void updatePlayers(Set<EnderPlayer> onlinePlayers) {
  ...
  @Override public void teleport(EnderEntity entity) {
  ...
  @Override public boolean damage(float damage) {
    if (damage <= 0) {
  ...
  @Override public void kick(Message reason) {
  ...
  @Override public void sendMessage(Message message, ChatPosition position) {
  ...
  @Override public void setAllowFlight(boolean allowFlight) {
  ...
  @Override public int getExperienceLevel() {
    return this.clientSettings.experience;
  }

  @Override public void setExperience(int experience) {
    ...
    this.networkManager.sendPacket(new PacketOutSetExperience(0F, this.clientSettings.experience, this.clientSettings.experience));
  }

  @Override public void setExperienceLevel(int experienceLevel) {
    if (Main.getInstance().callEvent(new PlayerExpChangeEvent(this, this.getExperience(), experienceLevel))) {
      return;
    }
    this.clientSettings.experience = experienceLevel;
    this.networkManager.sendPacket(new PacketOutSetExperience(0F, this.clientSettings.experience, this.clientSettings.experience));
  }

  @Override public boolean canSprint() {
  ...
  @Override public void playSound(String soundName, int volume, int pitch) {
  ...
  @Override public void playParticle(Particle particle, Location location, float xOffset, float yOffset, float zOffset, float data, int amount) {
    this.networkManager.sendPacket(new PacketOutPlayParticle(particle.getId(), location, xOffset, yOffset, zOffset, data, amount));
  }

  @Override public void sendBlockUpdate(Location loc, BlockId blockId, byte dataValue) {
  ...
  @Override public void awardAchievment(String name) {
    this.getNetworkManager().sendPacket(new PacketOutStatistics(name, 1));
  }

  @Override public void updateStatistic(String name, int value) {
    this.getNetworkManager().sendPacket(new PacketOutStatistics(name, value));
  }

  @Override public void forceChat(String rawMessage) {
    this.onPlayerChat(rawMessage);
  }

  @Override public boolean isInAir() {
    return !isOnGround;
  }

  @Override public void setTabListHeader(Message header) {
  ...
  @Override public void setTabListFooter(Message footer) {
  ...
  @Override public void setTabListHeaderAndFooter(Message header, Message footer) {
    this.clientSettings.tabListHeader = header;
    this.clientSettings.tabListFooter = footer;
    this.getNetworkManager().sendPacket(new PacketOutPlayerListHeaderFooter(header, footer));
  }

  @Override public Message getTabListHeader() {
    return this.clientSettings.tabListHeader;
  }

  @Override public Message getTabListFooter() {
  ...
  @Override public void displayWelcomeTitle(Message title, Message subtitle) {
  ...
  @Override public Iterator<Location> getLineOfSight(double range, int blocksPerLocation) {
  ...
  @Override public Location getTargetBlock(double range) {
    ...
    if (isInBetween(yaw, -360, -270)) {
      ...
      if (isInBetween(yaw, -270, -180)) {
        yaw *= -1;
        yaw -= 180;
        yaw = 90 - yaw;
        watchingEast = true;
      } else {
        if (isInBetween(yaw, -180, -90)) {
          yaw *= -1;
          yaw -= 90;
          yaw = 90 - yaw;
          watchingSouth = true;
        } else {
          if (isInBetween(yaw, -90, 0)) {
            yaw *= -1;
            yaw = 90 - yaw;
            watchingWest = true;
          } else {
            if (isInBetween(yaw, 0, 90)) {
              watchingNorth = true;
            } else {
              if (isInBetween(yaw, 90, 180)) {
                yaw -= 90;
                watchingEast = true;
              } else {
                if (isInBetween(yaw, 180, 270)) {
                  yaw -= 180;
                  watchingSouth = true;
                } else {
                  if (isInBetween(yaw, 270, 360)) {
                    yaw -= 270;
                    watchingWest = true;
                  }
                }
