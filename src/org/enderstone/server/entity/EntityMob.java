...
public abstract class EntityMob extends EnderEntity implements Mob {
  ...
  @Override public void teleport(Location loc) {
    ...
    for (EnderPlayer ep : onlinePlayers) {
      if (ep.getLocation().isInRange(40, this.getLocation(), false) && !ep.canSeeEntity.contains(this)) {
        ...
        if (!ep.getLocation().isInRange(40, this.getLocation(), false) && ep.canSeeEntity.contains(this)) {
          ...
          if (ep.canSeeEntity.contains(this) && !Main.getInstance().getWorld(ep).entities.contains(this)) {
            ep.canSeeEntity.remove(this);
            ep.getNetworkManager().sendPacket(new PacketOutEntityDestroy(new Integer[] { this.getEntityId() }));
          }
        }
  ...
  @Override protected void onHealthUpdate(float newHealth, float lastHealth) {
    if (newHealth <= 0) {
  ...
  @Override public EnderWorld getWorld() {
    return this.world;
  }
  ...
  @Override public PathNavigator getNavigator() {
    return navigator;
  }
