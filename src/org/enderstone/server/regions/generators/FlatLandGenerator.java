...
public class FlatLandGenerator extends ChunkGenerator {
  @Override public BlockId[][] generateExtBlockSections(EnderWorld world, Random random, int x, int z, ChunkGenerator.BiomeGrid biomes) {
  ...
  @Override public List<BlockPopulator> getDefaultPopulators(EnderWorld world) {
    List<BlockPopulator> p = super.getDefaultPopulators(world);
    p.add(new BlockPopulator() {
      @Override public void populate(EnderWorld world, Random random, EnderChunk source) {
