...
public class MinecraftServerCodex extends ByteToMessageCodec<Packet> {
  ...
  public MinecraftServerCodex(NetworkManager manager, Map<InetAddress, Long> connectionThrottlingMap, long maxConnectionThrottle) {
    super(Packet.class);
  ...
  @Override public boolean acceptOutboundMessage(Object msg) throws Exception {
    return msg instanceof Packet;
  }

  @Override protected void encode(ChannelHandlerContext ctx, Packet packet, ByteBuf buf) throws IOException {
    packet.writeFully(new PacketDataWrapper(manager, buf));
  }

  @Override protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws DecodeException {
            ...
            switch (packet.getNextState()) {
              ...
              if (this.connectionThrottlingMap.containsKey(address)) {
                ...
                if (System.currentTimeMillis() - lastLogin < maxConnectionThrottle) {
                  ctx.write(new PacketOutLoginPlayerDisconnect(new SimpleMessage("Connection throtled!"))).addListener(ChannelFutureListener.CLOSE);
                  return;
                }
              }
