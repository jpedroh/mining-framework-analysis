...
public class InventoryHandler {
  ...
  private final InventoryListener listener = new InventoryListener() {
    @Override public void onSlotChange(DefaultInventory inv, int slot, ItemStack oldStack, ItemStack newStack) {
    ...
    @Override public void onPropertyChange(DefaultInventory inv, short property, short oldValue, short newValue) {
      if (inv != activeInventory && inv != equimentInventory) {
        inv.removeListener(this);
        EnderLogger.warn("Removing stale inventory listener from: " + inv);
        return;
    ...
    @Override public void closeInventory(DefaultInventory inv) {
      if (inv != activeInventory && inv != equimentInventory) {
        inv.removeListener(this);
        EnderLogger.warn("Removing stale inventory listener from: " + inv);
        return;
  ...
  public void openInventory(Inventory inv) {
    ...
    if (inv == equimentInventory) {
      ...
      if (inv.getType() == Inventory.InventoryType.PLAYER_INVENTORY) {
