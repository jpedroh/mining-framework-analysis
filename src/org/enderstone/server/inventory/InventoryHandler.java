...
public class InventoryHandler {
  ...
  protected boolean swapItems(List<ItemStack> target, int targetIndex, List<ItemStack> destination, int destionationIndex) {
    ...
    if (s1 == null ? s2.equals(s1) : s1.equals(s2)) {
      return true;
    }
  ...
  public void recievePacket(PacketInClickWindow packet) {
    ...
    if (!correctTransaction) {
      player.sendMessage(new SimpleMessage("Unable to process inventory action: " + packet.toString()));
    }
  ...
  public boolean handleInventoryClick(PacketInClickWindow packet) {
    ...
    switch (mode) {
      ...
      case 2:
      {
        ...
        this.activeInventory.onItemClick(true, null, slot, false, new MergedList.Builder<ItemStack>().addList(0, this.equimentInventory.getHotbar(), targetSlot, 1).build());
      }
      return true;
      case 3:
      {
  ...
  public void openInventory(HalfInventory inv) {
    ...
    this.player.networkManager.sendPacket(new PacketOutOpenWindow(this.nextWindowId, inventory.getType(), inventory.getTitle(), (byte) (inventory.getType().getPacketSize() == -1 ? inv.getSize() : inventory.getType().getPacketSize()), 0));
