...
public class UCT extends MDPSolver implements Planner, QFunction {
  ...
  /**
	 ...
	 */
  @Override public GreedyQPolicy planFromState(State initialState) {
    foundGoal = false;
    treeSize = 1;
    numVisits = 0;
  ...
  @Override public List<QValue> getQs(State s) {
    if (this.root == null) {
  ...
  @Override public QValue getQ(State s, AbstractGroundedAction a) {
    if (this.root == null) {
      this.planFromState(s);
    }
    HashableState sh = this.hashingFactory.hashState(s);
  ...
  @Override public double value(State s) {
    return QFunction.QFunctionHelper.getOptimalValue(this, s, this.tf);
  }

  @Override public void resetSolver() {
