...
public class ApprenticeshipLearning {
  ...
  /**
	 ...
	 */
  public static double[] estimateFeatureExpectation(EpisodeAnalysis episodeAnalysis, StateToFeatureVectorGenerator featureFunctions, Double gamma) {
    return ApprenticeshipLearning.estimateFeatureExpectation(Arrays.asList(episodeAnalysis), featureFunctions, gamma);
  }
  ...
  /**
	 ...
	 * @return The Feature Expectations generated (double array that matches the length of the featureMapping)
	 */
  public static double[] estimateFeatureExpectation(List<EpisodeAnalysis> episodes, StateToFeatureVectorGenerator featureFunctions, Double gamma) {
  ...
  /**
	 ...
	 */
  public static RewardFunction generateRewardFunction(StateToFeatureVectorGenerator featureFunctions, FeatureWeights featureWeights) {
    ...
    return new RewardFunction() {
      @Override public double reward(State state, GroundedAction a, State sprime) {
  ...
  private static Policy maxMarginMethod(ApprenticeshipLearningRequest request) {
    ...
    double[] expertExpectations = ApprenticeshipLearning.estimateFeatureExpectation(expertEpisodes, featureFunctions, request.getGamma());
    EpisodeAnalysis episodeAnalysis = policy.evaluateBehavior(request.getStartStateGenerator().generateState(), new UniformCostRF(), maximumExpertEpisodeLength);
    double[] featureExpectations = ApprenticeshipLearning.estimateFeatureExpectation(episodeAnalysis, featureFunctions, request.getGamma());
    ...
    for (int i = 0; i < maxIterations; ++i) {
      ...
      RewardFunction rewardFunction = ApprenticeshipLearning.generateRewardFunction(featureFunctions, featureWeights);
      planner.resetSolver();
      ...
      for (int j = 0; j < policyCount; ++j) {
        evaluatedEpisodes.add(policy.evaluateBehavior(request.getStartStateGenerator().generateState(), rewardFunction, maximumExpertEpisodeLength));
      }
      featureExpectations = ApprenticeshipLearning.estimateFeatureExpectation(evaluatedEpisodes, featureFunctions, request.getGamma());
      featureExpectationsHistory.add(featureExpectations);
    }
    request.setTHistory(tHistory);
    return policy;
  ...
  private static Policy projectionMethod(ApprenticeshipLearningRequest request) {
    ...
    for (int j = 0; j < request.getPolicyCount(); ++j) {
      sampleEpisodes.add(policy.evaluateBehavior(request.getStartStateGenerator().generateState(), new UniformCostRF(), maximumExpertEpisodeLength));
    }
    double[] curFE = ApprenticeshipLearning.estimateFeatureExpectation(sampleEpisodes, featureFunctions, request.getGamma());
  ...
  /**
	 ...
	 */
  private static FeatureWeights solveFeatureWeights(double[] expertExpectations, List<double[]> featureExpectations) {
  ...
  /**
	 ...
	 */
  private static double[] projectExpertFE(double[] expertFE, double[] lastFE, double[] lastProjFE) {
  ...
  public static class RandomPolicy extends Policy {
    ...
    @Override public AbstractGroundedAction getAction(State s) {
    ...
    @Override public List<ActionProb> getActionDistributionForState(State s) {
      HashableState hashableState = this.hashFactory.hashState(s);
      if (!this.stateActionDistributionMapping.containsKey(hashableState)) {
        this.addNewDistributionForState(s);
      }
    ...
    @Override public boolean isStochastic() {
      return true;
    }

    @Override public boolean isDefinedFor(State s) {
      return true;
    }
  }
}