...
public class GridGameStandardMechanics extends JointActionModel {
  ...
  @Override public List<TransitionProbability> transitionProbsFor(State s, JointAction ja) {
  ...
  @Override protected State actionHelper(State s, JointAction ja) {
    List<GroundedSGAgentAction> gsas = ja.getActionList();
    List<Location2> previousLocations = new ArrayList<GridGameStandardMechanics.Location2>();
    List<Location2> noopLocations = new ArrayList<GridGameStandardMechanics.Location2>();
    for (GroundedSGAgentAction gsa : gsas) {
  ...
  protected Location2 attemptedDelta(String actionName) {
    if (actionName.equals(GridGame.ACTIONNORTH)) {
      ...
      if (actionName.equals(GridGame.ACTIONSOUTH)) {
        return new Location2(0, -1);
      } else {
        if (actionName.equals(GridGame.ACTIONEAST)) {
          return new Location2(1, 0);
        } else {
          if (actionName.equals(GridGame.ACTIONWEST)) {
            return new Location2(-1, 0);
          } else {
            if (actionName.equals(GridGame.ACTIONNOOP)) {
