...
public class GreqlCodeGenerator extends CodeGenerator implements ManagableArtifact {
  ...
  public GreqlCodeGenerator(GreqlQuery query, Schema datagraphSchema, String packageName, String classname) {
    ...
    scope = new Scope();
    evaluator = new GreqlEvaluatorImpl(query, null, new GreqlEnvironmentAdapter());
  ...
  /**
	 ...
	 */
  public static void generateCode(String queryString, Schema datagraphSchema, String classname, String path) {
    GreqlQuery query = GreqlQuery.createQuery(queryString, new DefaultOptimizer(new DefaultOptimizerInfo(datagraphSchema)));
    ...
    GreqlCodeGenerator greqlcodeGen = new GreqlCodeGenerator(query, datagraphSchema, packageName, simpleName);
    try {
  ...
  /**
	 ...
	 *            the fully qualified name of the class to be generated
	 */
  public static Class<ExecutableQuery> generateCode(String queryString, Schema datagraphSchema, String classname) {
    GreqlQuery query = GreqlQuery.createQuery(queryString, new DefaultOptimizer(new DefaultOptimizerInfo(datagraphSchema)));
    String simpleName = classname;
    String packageName = "";
    if (classname.contains(".")) {
  ...
  @Override public CodeBlock createBody() {
    ...
    methodExecute.add("public synchronized Object execute(de.uni_koblenz.jgralab.Graph graph, GreqlEnvironment boundVariables) {");
    ...
    for (int i = 0; i < acceptedTypesNumber; i++) {
      clearVars.add("acceptedTypeCollection_" + i + " = " + "acceptedTypeCollection_" + i + ".bindToSchema(graph.getSchema());");
      clearVars.add("acceptedType_" + i + " = acceptedTypeCollection_" + i + ".getTypeIdSet();");
    }
    for (int i = 0; i < acceptedIncidenceClassesNumber; i++) {
      clearVars.add("acceptedIncidenceClasses_" + i + " = " + "bindIncidenceClassesToSchema(graph.getSchema(), acceptedIncideceClassesEC_" + i + ", acceptedIncideceClassesIncidenceClass_" + i + ");");
    }
    methodExecute.add("}");
    mExec.addNoIndent(methodExecute);
    code.add(mExec);
    ...
    methodGetUsedVariables.add("\tjava.util.Set<String> usedVariables = new java.util.HashSet<String>();");
    ...
    methodGetStoredVariables.add("\tjava.util.Set<String> storedVariables = new java.util.HashSet<String>();");
    for (String usedVariable : query.getStoredVariables()) {
      methodGetStoredVariables.add("\tstoredVariables.add(\"" + usedVariable + "\");");
    }
  ...
  private String createCodeForGreqlExpression(GreqlExpression rootExpr) {
    ...
    for (IsBoundVarOf inc : rootExpr.getIsBoundVarOfIncidences(EdgeDirection.IN)) {
      ...
      list.add(new CodeSnippet("Object " + var.get_name() + " = boundVariables.getVariable(\"" + var.get_name() + "\");"));
      list.add(new CodeSnippet("if (" + var.get_name() + " == null) {"));
      list.add(new CodeSnippet("\tthrow new UndefinedVariableException(\"" + var.get_name() + "\");"));
    ...
    list.add(new CodeSnippet("Object " + resultVariable + " = " + createCodeForExpression(queryExpr) + ";"));
    for (IsIdOfStoreClause storeInc : rootExpr.getIsIdOfStoreClauseIncidences(EdgeDirection.IN)) {
      ...
      list.add(new CodeSnippet("boundVariables.setVariable(\"" + ident.get_name() + "\"," + resultVariable + ");"));
    }
  ...
  private String createCodeForSubgraphRestrictedExpression(SubgraphRestrictedExpression subgraphExpr) {
    SubgraphDefinition subgraphDefinition = subgraphExpr.get_subgraphDefinition();
    Expression expression = (Expression) subgraphExpr.getFirstIsExpressionOnSubgraphIncidence(EdgeDirection.IN).getThat();
    CodeList result = null;
    if (subgraphDefinition.isInstanceOf(ExpressionDefinedSubgraph.VC)) {
      result = new CodeList();
      result.add(new CodeSnippet(createCodeForExpressionDefinedSubraph((ExpressionDefinedSubgraph) subgraphDefinition)));
    }
    if (result != null) {
      result.add(new CodeSnippet("Object result = " + createCodeForExpression(expression) + ";"));
      ...
      return "UnsupportedElement: " + subgraphExpr.getClass().getSimpleName();
    }
  }

  private String createCodeForExpressionDefinedSubraph(ExpressionDefinedSubgraph subgraphDefinition) {
    Expression subgraphDefiningExpression = subgraphDefinition.get_definingExpression();
    addImports("de.uni_koblenz.jgralab.graphmarker.SubGraphMarker");
    CodeSnippet subgraphSnippet = new CodeSnippet();
    subgraphSnippet.add("SubGraphMarker subGraphMarker = (SubGraphMarker) " + createCodeForExpression(subgraphDefiningExpression) + ";");
    subgraphSnippet.add("\t\tdatagraph.setTraversalContext(subGraphMarker);");
  ...
  private String createInitializerForTypeCollection(Iterable<IsTypeRestrOfExpression> iterable) {
    ...
    addStaticField("de.uni_koblenz.jgralab.greql.types.TypeCollection", fieldNameTC, "de.uni_koblenz.jgralab.greql.types.TypeCollection.empty()");
    if (iterable.iterator().hasNext()) {
      StringBuilder sb = new StringBuilder(fieldNameTC).append(" = ").append(fieldNameTC);
      for (IsTypeRestrOfExpression itroe : iterable) {
        TypeId typeId = itroe.getAlpha();
        GraphElementClass<?, ?> gec = evaluator.getGraphElementClass(typeId.get_name());
        if (gec == null) {
          ((GreqlQueryImpl) query).getVertexEvaluator(itroe.getOmega()).evaluate(evaluator);
          gec = evaluator.getGraphElementClass(typeId.get_name());
        }
        sb.append(".with(\"").append(gec.getQualifiedName()).append("\", ").append(typeId.is_type()).append(", ").append(typeId.is_excluded()).append(")");
      }
      sb.append(";");
  ...
  private String createInitializerForTypeCollection(TypeCollection typeCollection) {
    ...
    if (!typeCollection.isEmpty()) {
      ...
      for (GraphElementClass<?, ?> gec : schema.getGraphClass().getGraphElementClasses()) {
        sb.append(".with(\"").append(gec.getQualifiedName()).append("\", true, ").append(!typeCollection.acceptsType(gec)).append(")");
      }
      sb.append(";");
      addStaticInitializer(sb.toString());
    }
  ...
  private String createInitializerForIncidenceTypeCollection(Set<IncidenceClass> incidenceClasses) {
    String fieldNameBS = "acceptedIncidenceClasses_" + acceptedIncidenceClassesNumber;
    addStaticField("java.util.BitSet", fieldNameBS, "null");
    String edgeClasses = "acceptedIncideceClassesEC_" + acceptedIncidenceClassesNumber;
    addStaticField("String[]", edgeClasses, "new String[" + incidenceClasses.size() + "]");
    String fromOrTo = "acceptedIncideceClassesIncidenceClass_" + acceptedIncidenceClassesNumber++;
    addStaticField("boolean[]", fromOrTo, "new boolean[" + incidenceClasses.size() + "]");
    if (!incidenceClasses.isEmpty()) {
      int index = 0;
      for (IncidenceClass ic : incidenceClasses) {
        addStaticInitializer(edgeClasses + "[" + index + "] = \"" + ic.getEdgeClass().getQualifiedName() + "\";");
        addStaticInitializer(fromOrTo + "[" + index + "] = " + (ic.getEdgeClass().getFrom() == ic ? "true" : "false") + ";");
      }
    }
    return fieldNameBS;
  }
  ...
  private String createCodeForEdgeSetExpression(EdgeSetExpression setExpr) {
    ...
    CodeList list = new CodeList();
    String acceptedTypesField = createInitializerForTypeCollection(setExpr.getIsTypeRestrOfExpressionIncidences(EdgeDirection.IN));
    CodeSnippet createEdgeSetSnippet = new CodeSnippet();
    createEdgeSetSnippet.add("org.pcollections.PCollection<Edge> edgeSet = JGraLab.set();");
  ...
  private String createCodeForVertexSetExpression(VertexSetExpression setExpr) {
    ...
    createVertexSetSnippet.add("org.pcollections.PCollection<de.uni_koblenz.jgralab.Vertex> vertexSet = de.uni_koblenz.jgralab.JGraLab.set();");
    createVertexSetSnippet.add("for (de.uni_koblenz.jgralab.Vertex e : datagraph.vertices()) {");
  ...
  private String createCodeForListRangeConstruction(ListRangeConstruction listConstr) {
    ...
    listSnippet.add("org.pcollections.PCollection<Object> list = JGraLab.vector();");
    Expression startExpr = (Expression) listConstr.getFirstIsFirstValueOfIncidence(EdgeDirection.IN).getThat();
    int startValue = (Integer) ((GreqlQueryImpl) query).getVertexEvaluator(startExpr).getResult(evaluator);
    Expression endExpr = (Expression) listConstr.getFirstIsLastValueOfIncidence(EdgeDirection.IN).getThat();
    int endValue = (Integer) ((GreqlQueryImpl) query).getVertexEvaluator(endExpr).getResult(evaluator);
    listSnippet.add("int startRange = " + createCodeForExpression(startExpr) + ";");
    listSnippet.add("int endRange = " + createCodeForExpression(endExpr) + ";");
    listSnippet.add("for (int i = startRange; i " + (startValue <= endValue ? "<=" : ">=") + " endRange; " + (startValue <= endValue ? "i++" : "i--") + ") {");
  ...
  private String createCodeForSetConstruction(SetConstruction setConstr) {
    ...
    CodeList list = new CodeList();
    CodeSnippet listSnippet = new CodeSnippet();
    listSnippet.add("org.pcollections.PCollection<Object> list = JGraLab.set();");
    boolean isEmpty = true;
    StringBuilder builder = new StringBuilder("list = list");
  ...
  private String createCodeForMapConstruction(MapConstruction setConstr) {
    ...
    CodeList list = new CodeList();
    CodeSnippet listSnippet = new CodeSnippet();
    listSnippet.add("org.pcollections.PMap<Object, Object> list = JGraLab.map();");
    boolean isEmpty = true;
    StringBuilder builder = new StringBuilder("list = list");
    IsKeyExprOfConstruction isKey = setConstr.getFirstIsKeyExprOfConstructionIncidence(EdgeDirection.IN);
    IsValueExprOfConstruction isValue = setConstr.getFirstIsValueExprOfConstructionIncidence(EdgeDirection.IN);
    while (isKey != null) {
      assert isValue != null;
      isEmpty = false;
      Expression key = (Expression) isKey.getThat();
      Expression value = (Expression) isValue.getThat();
      builder.append(".plus(" + createCodeForExpression(key) + ", " + createCodeForExpression(value) + ")");
      isKey = isKey.getNextIsKeyExprOfConstructionIncidence(EdgeDirection.IN);
      isValue = isValue.getNextIsValueExprOfConstructionIncidence(EdgeDirection.IN);
    }
    assert isValue == null;
    builder.append(";");
    if (!isEmpty) {
  ...
  private String createCodeForTupleConstruction(TupleConstruction tupleConstr) {
    ...
    CodeList list = new CodeList();
    CodeSnippet listSnippet = new CodeSnippet();
    listSnippet.add("org.pcollections.PCollection<Object> list = Tuple.empty();");
    boolean isEmpty = true;
    StringBuilder builder = new StringBuilder("list = list");
  ...
  private String createCodeForComprehension(Comprehension compr) {
    ...
    if (compr instanceof ListComprehension) {
      if (compr.getFirstIsTableHeaderOfIncidence(EdgeDirection.IN) == null) {
        initSnippet.add("org.pcollections.PCollection<Object> result = JGraLab.vector();");
      } else {
        initSnippet.add("de.uni_koblenz.jgralab.greql.types.Table<Object> result = de.uni_koblenz.jgralab.greql.types.Table.empty();");
      }
    }
    if (compr instanceof SetComprehension) {
      initSnippet.add("org.pcollections.PCollection<Object> result = JGraLab.set();");
    }
    if (compr instanceof MapComprehension) {
      initSnippet.add("org.pcollections.PMap<Object, Object> result = JGraLab.map();");
    }
    Expression maxCount = compr.get_maxCount();
    boolean hasMaxCount = maxCount != null;
    if (hasMaxCount) {
      initSnippet.add("int maxCount = (Integer) " + createCodeForExpression(maxCount) + ";");
    }
    IsTableHeaderOf isTableHeaderOf = compr.getFirstIsTableHeaderOfIncidence(EdgeDirection.IN);
    if (isTableHeaderOf != null) {
      initSnippet.add("org.pcollections.PVector<String> header = JGraLab.vector();");
      while (isTableHeaderOf != null) {
        initSnippet.add("header = header.plus(" + createCodeForExpression(isTableHeaderOf.getAlpha()) + ");");
    ...
    scope.blockBegin();
    CodeList varIterationList = createCodeForVariableIterationOfComprehension(methodBody, hasMaxCount, compr, null);
    ...
    if (compr instanceof MapComprehension) {
      ...
      iteratedExprSnip.add("result = result.plus(" + createCodeForExpression(keyExpr) + ", " + createCodeForExpression(valueExpr) + ");");
    } else {
  ...
  public CodeList createCodeForVariableIterationOfComprehension(CodeList methodBody, boolean hasMaxCount, Comprehension compr, Set<Variable> neededVariables) {
    Declaration decl = (Declaration) compr.getFirstIsCompDeclOfIncidence(EdgeDirection.IN).getThat();
    ...
    for (IsSimpleDeclOf simpleDeclInc : decl.getIsSimpleDeclOfIncidences(EdgeDirection.IN)) {
      ...
      for (IsDeclaredVarOf declaredVarInc : simpleDecl.getIsDeclaredVarOfIncidences(EdgeDirection.IN)) {
        ...
        if (!isDomainCreated) {
          ...
          simpleDeclSnippet.add("@SuppressWarnings(\"unchecked\")", "org.pcollections.PCollection<Object> #simpleDeclDomainName# = (org.pcollections.PCollection<Object>) " + createCodeForExpression(domain) + ";");
        }
        ...
        VariableEvaluator<? extends Variable> vertexEval = (VariableEvaluator<? extends Variable>) ((GreqlQueryImpl) query).getVertexEvaluator(var);
        List<VertexEvaluator<? extends Expression>> dependingExpressions = vertexEval.calculateDependingExpressions();
  ...
  private String createCodeForQuantifiedExpression(QuantifiedExpression quantExpr) {
    ...
    for (IsSimpleDeclOf simpleDeclInc : decl.getIsSimpleDeclOfIncidences(EdgeDirection.IN)) {
      ...
      simpleDeclSnippet.add("@SuppressWarnings(\"unchecked\")", tabs + "org.pcollections.PCollection<Object> domain_#simpleDeclNum# = (org.pcollections.PCollection<Object>) " + createCodeForExpression(domain) + ";");
    ...
    CodeSnippet iteratedExprSnip = new CodeSnippet();
    iteratedExprSnip.add(tabs + getVariableName(Integer.toString(resultDefinition.getId())) + " = null;");
    iteratedExprSnip.add(tabs + "Object currentResult = " + createCodeForExpression(resultDefinition) + ";");
    switch (quantifier.get_type()) {
      case FORALL:
      iteratedExprSnip.add(tabs + "if ((currentResult instanceof Boolean) && ! (Boolean) currentResult) return false;");
      break;
      case EXISTS:
      iteratedExprSnip.add(tabs + "if (!(currentResult instanceof Boolean) || (Boolean) currentResult) return true;");
      break;
      case EXISTSONE:
      iteratedExprSnip.add(tabs + "if (!(currentResult instanceof Boolean) || (Boolean) currentResult) {");
  ...
  private String createCodeForFunctionApplication(FunctionApplication funApp) {
    ...
    if (funApp.getFirstIsTypeExprOfFunctionIncidence(EdgeDirection.IN) != null) {
      ...
      sb.append("TypeCollection arg_").append(argNumber).append(" = TypeCollection.empty()");
      for (IsTypeExprOfFunction itroe : funApp.getIsTypeExprOfFunctionIncidences(EdgeDirection.IN)) {
      ...
      sb.append("arg_").append(argNumber).append(" = arg_").append(argNumber++).append(".bindToSchema(datagraph.getSchema());");
      list.add(new CodeSnippet(sb.toString()));
    }
    ...
    for (Method m : methods) {
      if (m.getName() == "evaluate") {
        ...
        boolean needsGraphArgument = paramTypes.length > 1 && paramTypes[0] == Graph.class;
        ...
        if (paramTypes.length == argNumber) {
          ...
          argBuilder.append("\tObject result = " + functionStaticFieldName + ".evaluate(");
          if (needsGraphArgument) {
          ...
          for (int i = 0; i < paramTypes.length; i++) {
            ...
            TypeVariable<?>[] typeParameters = paramTypes[i].getTypeParameters();
            needsRawTypesWarning |= typeParameters.length > 0;
            needsUncheckedWarning |= (typeParameters.length > 0 && paramTypes[i] != java.lang.Enum.class && paramTypes[i] != AttributedElement.class);
            argBuilder.append(delim + cast + "arg_" + i);
            delim = ", ";
          }
          argBuilder.append(");");
          if (needsRawTypesWarning) {
            checkSnippet.add("\t@SuppressWarnings({ " + (needsUncheckedWarning ? "\"unchecked\", " : "") + "\"rawtypes\" })");
          }
    ...
    list.add(new CodeSnippet("throw new RuntimeException(\"Given arguments don\'t match an available GReQL function." + " If you have added a function, you need to recompile the GReQL query for the function to be available.\");"));
  ...
  private String createCodeForForwardVertexSet(ForwardVertexSet fws) {
    ...
    PathDescriptionEvaluator<? extends PathDescription> pathDescrEval = (PathDescriptionEvaluator<? extends PathDescription>) ((GreqlQueryImpl) query).getVertexEvaluator(pathDescr);
  ...
  private String createCodeForBackwardVertexSet(BackwardVertexSet fws) {
    PathDescription pathDescr = (PathDescription) fws.getFirstIsPathOfIncidence(EdgeDirection.IN).getThat();
    PathDescriptionEvaluator<? extends PathDescription> pathDescrEval = (PathDescriptionEvaluator<? extends PathDescription>) ((GreqlQueryImpl) query).getVertexEvaluator(pathDescr);
    DFA dfa = NFA.revertNFA((NFA) pathDescrEval.getResult(evaluator)).getDFA();
  ...
  private String createCodeForReachableVertices(FunctionApplication funApp) {
    ...
    for (IsArgumentOf iao : funApp.getIsArgumentOfIncidences(EdgeDirection.IN)) {
      if (startElementExpr == null) {
        startElementExpr = (Expression) iao.getThat();
      } else {
        if (iao.getThat() instanceof PathDescription) {
    ...
    PathDescriptionEvaluator<? extends PathDescription> pathDescrEval = (PathDescriptionEvaluator<? extends PathDescription>) ((GreqlQueryImpl) query).getVertexEvaluator(pathDescr);
    DFA dfa = ((NFA) pathDescrEval.getResult(evaluator)).getDFA();
    return createCodeForForwarOrBackwardVertexSet(dfa, startElementExpr, funApp);
  }

  private String createCodeForPathExistence(PathExistence queryExpr) {
    Expression startExpr = (Expression) queryExpr.getFirstIsStartExprOfIncidence(EdgeDirection.IN).getThat();
    Expression targetExpr = (Expression) queryExpr.getFirstIsTargetExprOfIncidence(EdgeDirection.IN).getThat();
    PathDescription pathDescr = (PathDescription) queryExpr.getFirstIsPathOfIncidence(EdgeDirection.IN).getThat();
    PathDescriptionEvaluator<? extends PathDescription> pathDescrEval = (PathDescriptionEvaluator<? extends PathDescription>) ((GreqlQueryImpl) query).getVertexEvaluator(pathDescr);
    DFA dfa = ((NFA) pathDescrEval.getResult(evaluator)).getDFA();
    return createCodeForForwarOrBackwardVertexSetOrPathExcistence(dfa, startExpr, targetExpr, queryExpr);
  }
  ...
  private String createCodeForIsReachable(FunctionApplication funApp) {
    ...
    PathDescriptionEvaluator<? extends PathDescription> pathDescrEval = (PathDescriptionEvaluator<? extends PathDescription>) ((GreqlQueryImpl) query).getVertexEvaluator(pathDescr);
    DFA dfa = ((NFA) pathDescrEval.getResult(evaluator)).getDFA();
    return createCodeForForwarOrBackwardVertexSetOrPathExcistence(dfa, startExpr, targetExpr, funApp);
  }

  private String createCodeForForwarOrBackwardVertexSet(DFA dfa, Expression startElementExpr, GreqlVertex syntaxGraphVertex) {
    return createCodeForForwarOrBackwardVertexSetOrPathExcistence(dfa, startElementExpr, null, syntaxGraphVertex);
  }

  private String createCodeForForwarOrBackwardVertexSetOrPathExcistence(DFA dfa, Expression startElementExpr, Expression targetElementExpr, GreqlVertex syntaxGraphVertex) {
    CodeList list = new CodeList();
    ...
    initSnippet.add("@SuppressWarnings(\"unchecked\")", "java.util.HashSet<Vertex>[] markedElements = new java.util.HashSet[#stateCount#];");
    initSnippet.setVariable("stateCount", Integer.toString(dfa.stateList.size()));
    initSnippet.add("for (int i=0; i<#stateCount#;i++) {");
    initSnippet.add("\tmarkedElements[i] = new java.util.HashSet<Vertex>(100);");
    initSnippet.add("}");
    initSnippet.add("java.util.BitSet finalStates = new java.util.BitSet();");
    ...
    if (targetElementExpr != null) {
      initSnippet.add("Vertex target = (Vertex)" + createCodeForExpression(targetElementExpr) + ";");
    }
    ...
    initSnippet.add("queue.put((Vertex) element, " + dfa.initialState.number + ");");
    ...
    if (targetElementExpr == null) {
      ...
      initSnippet.add("\tif (finalStates.get(stateNumber) && element == target){");
    ...
    for (State curState : dfa.stateList) {
      ...
      for (Transition curTrans : curState.outTransitions) {
        ...
        transitionCodeList.add(createCodeForTransition(curTrans, FunctionType.OTHER), 2);
  ...
  private CodeList createAddToPathSystemQueueSnippet(Transition t) {
    ...
    addSnippet.add("PathSystemMarkerEntry newEntry = ExecutablePathSystemHelper.markVertex(marker,");
  ...
  private String createCodeForPathSystem(DFA dfa, Expression startElementExpr, GreqlVertex syntaxGraphVertex) {
    CodeList list = new CodeList();
    addImports("de.uni_koblenz.jgralab.*");
    ...
    CodeSnippet initSnippet = new CodeSnippet();
    list.add(initSnippet);
    initSnippet.add("Vertex rootVertex = (Vertex)" + createCodeForExpression(startElementExpr) + ";");
    initSnippet.add("Vertex element = rootVertex;");
    initSnippet.add("Vertex nextElement;");
    initSnippet.add("java.util.Queue<PathSystemMarkerEntry> queue = new java.util.LinkedList<PathSystemMarkerEntry>();");
    ...
    initSnippet.add("PathSystemMarkerEntry currentEntry = ", "\tExecutablePathSystemHelper.markVertex(marker, element, #initialStateNumber#, #initialStateFinal#, null, null, 0, 0);");
    ...
    for (State curState : dfa.stateList) {
      ...
      for (Transition curTrans : curState.outTransitions) {
        ...
        transitionCodeList.add(createCodeForTransition(curTrans, FunctionType.PATH_SYSTEM), 2);
        transitionCodeList.add(new CodeSnippet("\t\t} //finished checking transitions of state " + curTrans.endState.number));
      }
      stateCodeList.add(new CodeSnippet("\t\tbreak;//break case block"));
    }
    ...
    finalSnippet.add("\t} //end of iterating incident edges ");
    finalSnippet.add("} //end of processing queue");
    finalSnippet.add("return ExecutablePathSystemHelper.createPathSystemFromMarkings(marker, (Vertex)rootVertex, finalEntries);");
    list.add(finalSnippet);
    return createMethod(list, syntaxGraphVertex);
  }
  ...
  private CodeBlock createAddToSliceQueueSnippet(Transition t) {
    ...
    list.setVariable("endStateFinal", Boolean.toString(t.endState.isFinal));
    CodeSnippet addSnippet = new CodeSnippet();
    addSnippet.add("if (!ExecutableSliceHelper.isMarked(marker, nextElement, #endStateNumber#)) {");
    addSnippet.add("\tqueue.add(new PathSystemMarkerEntry(nextElement, element, #traversedEdge#, #endStateNumber#, #endStateFinal#, currentEntry.stateNumber, 0));");
    addSnippet.add("}");
    addSnippet.add("ExecutableSliceHelper.markVertex(marker, nextElement, #endStateNumber#, #endStateFinal#, element, #traversedEdge#, currentEntry.stateNumber, 0);");
    list.add(addSnippet);
    return list;
  }
  ...
  private String createCodeForSlice(DFA dfa, Expression rootExpr, FunctionApplication funApp) {
    CodeList list = new CodeList();
    addImports("de.uni_koblenz.jgralab.*");
    ...
    CodeSnippet initSnippet = new CodeSnippet();
    list.add(initSnippet);
    initSnippet.add("Object rootResult = " + createCodeForExpression(rootExpr) + ";");
    initSnippet.add("@SuppressWarnings(\"unchecked\")");
    initSnippet.add("org.pcollections.PSet<Vertex> rootVertices = (org.pcollections.PSet<Vertex>) (rootResult instanceof Vertex ? JGraLab.set().plus(rootResult) : rootResult);");
    initSnippet.add("Set<Vertex> sliCritVertices = new HashSet<Vertex>();");
    initSnippet.add("sliCritVertices.addAll(rootVertices);");
    initSnippet.add("java.util.List<GraphMarker<Map<Edge, PathSystemMarkerEntry>>> marker = new ArrayList<GraphMarker<Map<Edge, PathSystemMarkerEntry>>>(" + dfa.stateList.size() + ");");
    initSnippet.add("for (int i = 0; i < " + dfa.stateList.size() + "; i++) {");
    initSnippet.add("\tmarker.add(new GraphMarker<Map<Edge, PathSystemMarkerEntry>>(datagraph));");
    initSnippet.add("}");
    initSnippet.add("java.util.List<Vertex> finalVertices = new ArrayList<Vertex>();");
    ...
    initSnippet.add("\tcurrentEntry = new PathSystemMarkerEntry(v, null, null, " + dfa.initialState.number + ", " + dfa.initialState.isFinal + ", 0, 0);");
    initSnippet.add("\tqueue.offer(currentEntry);");
    initSnippet.add("\tExecutableSliceHelper.markVertex(marker, v, #initialStateNumber#, #initialStateFinal#, null, null, 0, 0);");
    initSnippet.add("}");
    initSnippet.add("while (!queue.isEmpty()) {");
    initSnippet.add("\tcurrentEntry = queue.poll();");
    initSnippet.add("\telement = currentEntry.vertex;");
    ...
    for (State curState : dfa.stateList) {
      ...
      for (Transition curTrans : curState.outTransitions) {
        ...
        transitionCodeList.add(new CodeSnippet("\t\tif (!ExecutableSliceHelper.isMarked(marker, nextElement, " + curTrans.endState.number + ", inc)) {//checking all transitions of state " + curTrans.endState.number));
        transitionCodeList.add(createCodeForTransition(curTrans, FunctionType.SLICE), 2);
        transitionCodeList.add(new CodeSnippet("\t\t} //finished checking transitions of state " + curTrans.endState.number));
      }
      stateCodeList.add(new CodeSnippet("\t\tbreak;//break case block"));
    }
    ...
    finalSnippet.add("\t} //end of iterating incident edges ");
    finalSnippet.add("} //end of processing queue");
    finalSnippet.add("return ExecutableSliceHelper.createSliceFromMarkings(datagraph, sliCritVertices, finalVertices, marker);");
    list.add(finalSnippet);
    return createMethod(list, funApp);
  }
  ...
  /**
	 ...
	 */
  private CodeBlock createCodeForTransition(Transition trans, FunctionType functionType) {
    if (trans instanceof EdgeTransition) {
      return createCodeForEdgeTransition((EdgeTransition) trans, functionType);
    }
    if (trans instanceof SimpleTransition) {
      return createCodeForSimpleTransition((SimpleTransition) trans, functionType);
    }
    if (trans instanceof AggregationTransition) {
      return createCodeForAggregationTransition((AggregationTransition) trans, functionType);
    }
    if (trans instanceof BoolExpressionTransition) {
      return createCodeForBooleanExpressionTransition((BoolExpressionTransition) trans, functionType);
    }
    if (trans instanceof IntermediateVertexTransition) {
      return createCodeForIntermediateVertexTransition((IntermediateVertexTransition) trans, functionType);
    }
    if (trans instanceof VertexTypeRestrictionTransition) {
      return createCodeForVertexTypeRestrictionTransition((VertexTypeRestrictionTransition) trans, functionType);
    }
    return new CodeSnippet("FAILURE: TRANSITION TYPE IS UNKNOWN TO GREQL CODE GENERATOR " + trans.getClass().getSimpleName());
  }

  private CodeBlock createCodeForSimpleTransition(SimpleTransition trans, FunctionType functionType) {
    return createCodeForSimpleOrEdgeTransition(trans, functionType, null);
  }

  private CodeBlock createCodeForEdgeTransition(EdgeTransition trans, FunctionType functionType) {
    CodeList curr = new CodeList();
    VertexEvaluator<?> allowedEdgeEvaluator = trans.getAllowedEdgeEvaluator();
    CodeBlock content = createCodeForSimpleOrEdgeTransition(trans, functionType, curr);
    if (allowedEdgeEvaluator != null) {
      curr.add(new CodeSnippet("Edge allowedEdge = (Edge) " + createCodeForExpression((Expression) allowedEdgeEvaluator.getVertex()) + ";"));
      curr.add(new CodeSnippet("if (inc.getNormalEdge() == allowedEdge.getNormalEdge()){"));
      curr.add(content, 1);
  ...
  private CodeBlock createCodeForSimpleOrEdgeTransition(SimpleTransition trans, FunctionType functionType, CodeBlock edgeTest) {
    ...
    if (functionType == FunctionType.PATH_SYSTEM) {
      ...
      if (functionType == FunctionType.SLICE) {
  ...
  private CodeBlock createCodeForAggregationTransition(AggregationTransition trans, FunctionType functionType) {
    CodeList resultList = new CodeList();
    CodeList curr = resultList;
  ...
  private CodeList createTypeCollectionCheck(CodeList curr, TypeCollection typeCollection) {
    if (typeCollection != null) {
      ...
      curr.add(new CodeSnippet("if (" + fieldName + ".get(((de.uni_koblenz.jgralab.schema.GraphElementClass<?,?>)inc.getAttributedElementClass()).getGraphElementClassIdInSchema())) { //check type collection"));
      CodeList body = new CodeList();
      curr.add(body);
  ...
  private CodeList createPredicateCheck(CodeList curr, VertexEvaluator<? extends Expression> predicateEval) {
    if (predicateEval != null) {
      createThisLiterals();
      curr.add(new CodeSnippet("setThisEdge(inc);"));
      curr.add(new CodeSnippet("setThisVertex(element);"));
      curr.add(new CodeSnippet("if ((Boolean) " + createCodeForExpression(predicateEval.getVertex()) + ") { //begin check predicate"));
      CodeList body = new CodeList();
      curr.add(body);
  ...
  private CodeBlock createCodeForVertexTypeRestrictionTransition(VertexTypeRestrictionTransition trans, FunctionType functionType) {
    CodeList resultList = new CodeList();
    CodeList curr = resultList;
    TypeCollection typeCollection = trans.getAcceptedVertexTypes();
    if (typeCollection != null) {
      String fieldName = createInitializerForTypeCollection(typeCollection);
      curr.add(new CodeSnippet("if (" + fieldName + ".get(((de.uni_koblenz.jgralab.schema.GraphElementClass<?,?>)nextElement.getAttributedElementClass()).getGraphElementClassIdInSchema())) {//test for VertexTypeRestriction"));
      CodeList body = new CodeList();
      curr.add(body);
  ...
  private CodeBlock createCodeForIntermediateVertexTransition(IntermediateVertexTransition trans, FunctionType functionType) {
    CodeList curr = new CodeList();
    ...
    if (intermediateVertexEval != null) {
      ...
      predicateSnippet.add("boolean transitionWorks = tempRes == element;");
      predicateSnippet.add("if(!transitionWorks) {");
      predicateSnippet.add("\tif(tempRes instanceof org.pcollections.PCollection) {");
      predicateSnippet.add("\t\t@SuppressWarnings(\"unchecked\")");
      predicateSnippet.add("\t\torg.pcollections.PCollection<Object> tmpList = (org.pcollections.PCollection<Object>) tempRes;");
      predicateSnippet.add("\t\ttransitionWorks = tmpList.contains(element);");
      predicateSnippet.add("\t}");
      predicateSnippet.add("}");
      predicateSnippet.add("if(transitionWorks) { //test of intermediate vertex transition");
  ...
  private CodeBlock createCodeForBooleanExpressionTransition(BoolExpressionTransition trans, FunctionType functionType) {
    CodeList curr = new CodeList();
    CodeList resultList = curr;
    VertexEvaluator<? extends Expression> predicateEval = trans.getBooleanExpressionEvaluator();
    if (predicateEval != null) {
      createThisVertex();
      curr.add(new CodeSnippet("setThisVertex(element);"));
      curr.add(new CodeSnippet("if ((Boolean) " + createCodeForExpression(predicateEval.getVertex()) + ") {"));
      CodeList body = new CodeList();
      curr.add(body);
      curr.add(new CodeSnippet("}"));
      curr = body;
  ...
  private String createMethod(CodeList methodBody, GreqlVertex vertex) {
    String comment = "// " + GreqlSerializer.serializeVertex(vertex).replace("\n", " ");
    ...
    String methodName = "evaluationMethod_" + vertex.getId() + "_" + uniqueMethodId++;
  ...
  protected void createSetterForThisLiteral(ThisLiteral lit, String edgeOrVertex) {
    ...
    if (lit != null) {
      VariableEvaluator<? extends Variable> vertexEval = (VariableEvaluator<? extends Variable>) ((GreqlQueryImpl) query).getVertexEvaluator(lit);
      List<VertexEvaluator<? extends Expression>> dependingExpressions = vertexEval.calculateDependingExpressions();
      for (VertexEvaluator<? extends Expression> ve : dependingExpressions) {
        Vertex currentV = ve.getVertex();
  ...
  @Override public void createFiles(String pathPrefix) throws GraphIOException {
  ...
  @SuppressWarnings(value = { "unchecked" }) public Class<ExecutableQuery> compile() {
    ...
    try {
      ...
      return (Class<ExecutableQuery>) Class.forName(packageName + "." + this.classname, true, schemaClassManager);
  ...
  @Override protected CodeBlock createPackageDeclaration() {
    return new CodeSnippet("package " + packageName + ";");
  }

  @Override public String getManagedName() {
