...
public class GreqlCodeGenerator extends CodeGenerator implements ManagableArtifact {
  ...
  public static void generateCode(String queryString, Schema datagraphSchema, String classname, String path) {
    GreqlQuery query = GreqlQuery.createQuery(queryString, true, new DefaultOptimizerInfo(datagraphSchema));
  ...
  private String createCodeForGreqlExpression(GreqlExpression rootExpr) {
    ((GreqlExpressionEvaluator) ((GreqlQueryImpl) query).getVertexEvaluator(rootExpr)).handleImportedTypes(schema);
  ...
  private String createInitializerForTypeCollection(TypeCollection typeCollection) {
    ...
    addStaticField("java.util.BitSet", fieldName, "new java.util.BitSet(" + schema.getGraphElementClassCount() + ")");
    if (typeCollection.isEmpty()) {
      addStaticInitializer(fieldName + ".set(0, " + fieldName + ".size() - 1, true);");
    } else {
      addStaticInitializer("for (int b: new int[] " + typeCollection.getTypeIdSet() + ") { " + fieldName + ".set(b); }");
    }
    return fieldName;
  }
  ...
  private String createInitializerForIncidenceTypeCollection(Set<IncidenceClass> incidenceClasses) {
    String fieldName = "acceptedType_" + acceptedTypesNumber++;
    int numberOfTypesInSchema = schema.getGraphClass().getEdgeClasses().size() * 2;
  ...
  private String createCodeForEdgeSetExpression(EdgeSetExpression setExpr) {
    ...
    for (IsTypeRestrOfExpression inc : setExpr.getIsTypeRestrOfExpressionIncidences(EdgeDirection.IN)) {
      ...
      typeCol = typeCol.combine((TypeCollection) ((GreqlQueryImpl) query).getVertexEvaluator(typeId).getResult(evaluator));
    }
  ...
  private String createCodeForQuantifiedExpression(QuantifiedExpression quantExpr) {
    ...
    for (IsSimpleDeclOf simpleDeclInc : decl.getIsSimpleDeclOfIncidences(EdgeDirection.IN)) {
      ...
      simpleDeclSnippet.add(tabs + "org.pcollections.PCollection<Object> domain_#simpleDeclNum# = (org.pcollections.PCollection<Object>) " + createCodeForExpression(domain) + ";");
  ...
  private String createCodeForFunctionApplication(FunctionApplication funApp) {
    ...
    if (funId.get_name().equals("slice")) {
      throw new RuntimeException("Code generation for function slice is not yet implemented.");
    }
    ...
    for (Method m : methods) {
      if (m.getName() == "evaluate") {
        ...
        if (paramTypes.length == argNumber) {
          ...
          for (int i = 0; i < paramTypes.length; i++) {
            String cast = "(" + getCanonicalNameWithTypeParams(paramTypes[i]) + ")";
  ...
  private CodeBlock createCodeForEdgeTransition(EdgeTransition trans, boolean pathSystem) {
    ...
    CodeBlock content = createCodeForSimpleOrEdgeTransition(trans, pathSystem, curr);
