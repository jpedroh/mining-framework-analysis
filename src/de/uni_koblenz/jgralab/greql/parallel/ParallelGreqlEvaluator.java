...
public class ParallelGreqlEvaluator {
  ...
  public class TaskHandle implements Comparable<TaskHandle> {
    ...
    @Override public String toString() {
      return "TaskHandle " + seq + " prio " + priority + " (use: " + getUsedVariables() + ", store:" + getStoredVariables() + ")";
    }
    ...
    @Override public int compareTo(TaskHandle other) {
  ...
  public EvaluationEnvironment evaluate(boolean adjustPriorityValues) {
    return evaluate(null, new GreqlEnvironmentAdapter(), adjustPriorityValues);
  }
  ...
  public EvaluationEnvironment evaluate(Graph datagraph, boolean adjustPriorityValues) {
    return evaluate(datagraph, new GreqlEnvironmentAdapter(), adjustPriorityValues, false);
  }
  ...
  public EvaluationEnvironment evaluate(Graph datagraph, GreqlEnvironment greqlEnvironment, boolean adjustPriorityValues) {
    return evaluate(datagraph, greqlEnvironment, adjustPriorityValues, false);
  }
  ...
  private EvaluationEnvironment evaluate(Graph datagraph, GreqlEnvironment greqlEnvironment, boolean adjustPriorityValues, boolean sequentially) {
    ...
    if (!sequentially) {
      waitForTerminationTask = new FutureTask<Object>(new Callable<Object>() {
        @Override public Object call() throws Exception {
  ...
  /**
	 ...
	 */
  private void adjustPriorities(final EvaluationEnvironment evaluationEnvironment) {
  ...
  private Set<TaskHandle> createEvaluationTasks(EvaluationEnvironment evaluationEnvironment) {
    synchronized (dependencyGraph) {
      ...
      if (evaluationEnvironment.sequentially) {
        ...
        for (TaskHandle handle : dependencyGraph.getNodesInTopologicalOrder()) {
  ...
  private void scheduleNext(EvaluationEnvironment environment, TaskHandle finishedTask) {
    synchronized (dependencyGraph) {
      ...
      for (TaskHandle succ : dependencyGraph.getDirectSuccessors(finishedTask)) {
