...
public class GreqlSerializer {
    ...
    if (v instanceof Declaration) {
      ...
      if (v instanceof Definition) {
        ...
        if (v instanceof Direction) {
          ...
          if (v instanceof EdgeRestriction) {
            serializeEdgeRestriction((EdgeRestriction) v);
          } else {
            if (v instanceof Greql2Expression) {
              serializeGreql2Expression((Greql2Expression) v);
            } else {
              if (v instanceof Quantifier) {
                serializeQuantifier((Quantifier) v);
              } else {
                if (v instanceof RecordElement) {
  ...
  private void serializeQuantifier(Quantifier v) {
    switch (v.get_type()) {
      ...
      break;
      default:
      throw new RuntimeException("FIXME: Unhandled QuantificationType: " + v.get_type());
    }
  }
  ...
  private void serializeGreql2Expression(Greql2Expression greql2Expression) {
    Iterable<? extends Variable> boundVars = greql2Expression.get_boundVar();
  ...
  private void serializeTupleConstruction(TupleConstruction exp, boolean implicit) {
    if (!implicit) {
      sb.append("tup(");
    }
    boolean first = true;
  ...
  private void serializePathExpression(PathExpression exp) {
    if (exp instanceof BackwardVertexSet) {
      ...
      if (exp instanceof ForwardVertexSet) {
        serializeForwardVertexSet((ForwardVertexSet) exp);
      } else {
        if (exp instanceof PathExistence) {
  ...
  private void serializePrimaryPathDescription(PrimaryPathDescription exp) {
    if (exp instanceof EdgePathDescription) {
      ...
      if (exp instanceof SimplePathDescription) {
        serializeSimplePathDescription((SimplePathDescription) exp);
      } else {
        if (exp instanceof AggregationPathDescription) {
  ...
  private void serializeElementSetExpression(ElementSetExpression exp) {
    if (exp instanceof VertexSetExpression) {
      ...
      if (exp instanceof EdgeSetExpression) {
        sb.append("E");
      } else {
  ...
  private void serializeComprehension(Comprehension exp) {
    ...
    if (exp instanceof SetComprehension) {
      ...
      if (exp instanceof ListComprehension) {
        ...
        if (exp instanceof TableComprehension) {
  ...
  private void serializeLiteral(Literal exp) {
    if (exp instanceof BoolLiteral) {
      ...
      if (exp instanceof IntLiteral) {
        ...
        if (exp instanceof UndefinedLiteral) {
          sb.append("undefined");
        } else {
          if (exp instanceof DoubleLiteral) {
            ...
            if (exp instanceof StringLiteral) {
              ...
              if (exp instanceof ThisEdge) {
                sb.append("thisEdge");
              } else {
                if (exp instanceof ThisVertex) {
