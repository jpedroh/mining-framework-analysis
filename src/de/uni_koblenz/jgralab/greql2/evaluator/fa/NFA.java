...
public class NFA extends FiniteAutomaton {
  ...
  public static NFA createIteratedPathDescriptionNFA(NFA iteratedNFA, boolean optional) {
    ...
    Transition t = new EpsilonTransition(newFinalState, iteratedNFA.initialState);
  ...
  public static NFA createAlternativePathDescriptionNFA(List<NFA> nfaList) {
    ...
    for (int i = 0; i < nfaList.size(); i++) {
      NFA nextNFA = nfaList.get(i);
      Transition t = new EpsilonTransition(resultNFA.initialState, nextNFA.initialState);
  ...
  public static NFA createOptionalPathDescriptionNFA(NFA optionalNFA) {
    Transition t = new EpsilonTransition(optionalNFA.initialState, optionalNFA.finalStates.get(0));
  ...
  /**
	 ...
	 */
  public static NFA createEdgePathDescriptionNFA(Transition.AllowedEdgeDirection dir, TypeCollection typeCollection, Set<String> roles, VertexEvaluator edgeEval, VertexEvaluator predicateEvaluator, GraphMarker<VertexEvaluator> marker) {
    NFA nfa = new NFA();
    nfa.transitionList.clear();
    nfa.initialState.outTransitions.clear();
    nfa.finalStates.get(0).inTransitions.clear();
    SimpleTransition t = new EdgeTransition(nfa.initialState, nfa.finalStates.get(0), dir, typeCollection, roles, edgeEval, predicateEvaluator, marker);
    nfa.transitionList.add(t);
    nfa.updateStateAttributes();
    return nfa;
  }
  ...
  /**
	 ...
	 * EdgeRestrictions (RoleId, TypeId) are modeled in the Transition.
	 */
  public static NFA createSimplePathDescriptionNFA(Transition.AllowedEdgeDirection dir, TypeCollection typeCollection, Set<String> roles, VertexEvaluator predicateEvaluator, GraphMarker<VertexEvaluator> marker) {
    NFA nfa = new NFA();
    nfa.transitionList.clear();
    nfa.initialState.outTransitions.clear();
    nfa.finalStates.get(0).inTransitions.clear();
    SimpleTransition t = new SimpleTransition(nfa.initialState, nfa.finalStates.get(0), dir, typeCollection, roles, predicateEvaluator, marker);
    nfa.transitionList.add(t);
    nfa.updateStateAttributes();
    return nfa;
  }
  ...
  /**
	 ...
	 * EdgeRestrictions (RoleId, TypeId) are modeled in the Transition.
	 */
  public static NFA createAggregationPathDescriptionNFA(boolean aggregateFrom, TypeCollection typeCollection, Set<String> roles, VertexEvaluator predicateEvaluator, GraphMarker<VertexEvaluator> marker) {
    NFA nfa = new NFA();
    nfa.transitionList.clear();
    nfa.initialState.outTransitions.clear();
    nfa.finalStates.get(0).inTransitions.clear();
    AggregationTransition t = new AggregationTransition(nfa.initialState, nfa.finalStates.get(0), aggregateFrom, typeCollection, roles, predicateEvaluator, marker);
    nfa.transitionList.add(t);
    nfa.updateStateAttributes();
    return nfa;
  }
  ...
  public static void addGoalBooleanRestriction(NFA nfa, VertexEvaluator boolEval, GraphMarker<VertexEvaluator> marker) {
    ...
    nfa.stateList.add(restrictedFinalState);
    nfa.finalStates.add(restrictedFinalState);
    BoolExpressionTransition trans = new BoolExpressionTransition(newEndState, restrictedFinalState, boolEval, marker);
    nfa.transitionList.add(trans);
  }
  ...
  public static void addStartBooleanRestriction(NFA nfa, VertexEvaluator boolEval, GraphMarker<VertexEvaluator> marker) {
    ...
    nfa.stateList.add(newInitialState);
    BoolExpressionTransition trans = new BoolExpressionTransition(newInitialState, nfa.initialState, boolEval, marker);
    nfa.transitionList.add(trans);
    nfa.initialState = newInitialState;
  }
