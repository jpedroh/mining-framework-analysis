...
VertexEvaluator
>>>>>>>
 >  dependingExpressions ;   private final  long  estimatedAssignments =  Long . MIN_VALUE ;   public void setValue  (  Object variableValue ,  InternalGreqlEvaluator evaluator )  {  if  (  dependingExpressions == null )  {   dependingExpressions =  calculateDependingExpressions  ( ) ; }   int  size =  dependingExpressions . size  ( ) ;  for (   int  i = 0 ;  i < size ;  i ++ )  {    dependingExpressions . get  ( i ) . clear  ( evaluator ) ; }   evaluator . setLocalEvaluationResult  ( vertex , variableValue ) ; }   public Object getValue  (  InternalGreqlEvaluator evaluator )  {  return  evaluator . getLocalEvaluationResult  ( vertex ) ; }   public VariableEvaluator  (  V vertex ,  Query query )  {  super  ( vertex , query ) ; }    @ Override public Object evaluate  (  InternalGreqlEvaluator evaluator )  {  return  getValue  ( evaluator ) ; }    @ Override public Object getResult  (  InternalGreqlEvaluator evaluator )  {  return  getValue  ( evaluator ) ; }    @ Override public  Set  < Variable > getNeededVariables  ( )  {  if  (  neededVariables == null )  {   neededVariables =  new  HashSet  < Variable >  ( ) ;   neededVariables . add  ( vertex ) ; }  return neededVariables ; }    @ Override public  Set  < Variable > getDefinedVariables  ( )  {  if  (  definedVariables == null )  {   definedVariables =  new  HashSet  < Variable >  ( ) ; }  return definedVariables ; }    @ SuppressWarnings  ( "unchecked" ) protected  List  < 
<<<<<<<
 VertexEvaluator  <  ? extends Expression >
=======
VertexEvaluator
>>>>>>>
 > calculateDependingExpressions  ( )  {   Queue  < Greql2Vertex >  queue =  new  LinkedList  < Greql2Vertex >  ( ) ;   List  < 
<<<<<<<
 VertexEvaluator  <  ? extends Expression >
=======
VertexEvaluator
...
getMark
>>>>>>>
  ( currentVertex ) ;  if  (      (  eval != null ) &&  (  !  dependingEvaluators . contains  ( eval ) ) &&  (  !  (  eval instanceof PathDescriptionEvaluator ) ) &&  (  !  (  eval instanceof DeclarationEvaluator ) ) &&  (  !  (  eval instanceof SimpleDeclarationEvaluator ) ) )  {   dependingEvaluators . add  ( 
<<<<<<<
 (  VertexEvaluator  <  ? extends Expression > ) eval
=======
eval
>>>>>>>
 ) ; }  Greql2Aggregation  currentEdge =  currentVertex . getFirstGreql2AggregationIncidence  (  EdgeDirection . OUT ) ;  while  (  currentEdge != null )  {  Greql2Vertex  nextVertex =  ( Greql2Vertex )  currentEdge . getThat  ( ) ;  if  (  !  forbiddenVertices . contains  ( nextVertex ) )  {   queue . add  ( nextVertex ) ; }   currentEdge =  currentEdge . getNextGreql2AggregationIncidence  (  EdgeDirection . OUT ) ; } }  return dependingEvaluators ; }    @ Override public void calculateNeededAndDefinedVariables  ( )  { } 
<<<<<<<
=======
...
>>>>>>>
   public void setValue  (  Object variableValue2 )  {  if  (  dependingExpressions == null )  {   dependingExpressions =  calculateDependingExpressions  ( ) ; }   int  size =  dependingExpressions . size  ( ) ;  for (   int  i = 0 ;  i < size ;  i ++ )  {    dependingExpressions . get  ( i ) . clear  ( ) ; }   variableValue = variableValue2 ; } 
<<<<<<<
=======
  public Object getValue  ( )  {  return variableValue ; }
>>>>>>>
 
<<<<<<<
=======
   @ Override public Object evaluate  ( )  {  return variableValue ; }
>>>>>>>
    @ Override public Object getResult  ( )  {  return variableValue ; } }