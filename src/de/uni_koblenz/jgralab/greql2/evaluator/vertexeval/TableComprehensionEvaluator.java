...
public class TableComprehensionEvaluator extends VertexEvaluator {
  ...
  private void initialize() {
    Declaration d = (Declaration) vertex.getFirstIsCompDeclOfIncidence(EdgeDirection.IN).getAlpha();
    ...
    Expression columnHeader = (Expression) vertex.getFirstIsColumnHeaderExprOfIncidence(EdgeDirection.IN).getAlpha();
    columnHeaderEval = vertexEvalMarker.getMark(columnHeader);
    Expression rowHeader = (Expression) vertex.getFirstIsRowHeaderExprOfIncidence(EdgeDirection.IN).getAlpha();
    rowHeaderEval = vertexEvalMarker.getMark(rowHeader);
    Expression resultDef = (Expression) vertex.getFirstIsCompResultDefOfIncidence(EdgeDirection.IN).getAlpha();
  ...
  /**
	 ...
	 */
  public TableComprehensionEvaluator(TableComprehension vertex, GreqlEvaluator eval) {
    super(eval);
    this.vertex = vertex;
  }
  ...
  @Override public Object evaluate() {
    ...
    if (tHeader != null) {
      VertexEvaluator theval = vertexEvalMarker.getMark(tHeader.getAlpha());
  ...
  @Override public VertexCosts calculateSubtreeEvaluationCosts(GraphSize graphSize) {
    return this.greqlEvaluator.getCostModel().calculateCostsTableComprehension(this, graphSize);
  }

  @Override public long calculateEstimatedCardinality(GraphSize graphSize) {
    return greqlEvaluator.getCostModel().calculateCardinalityTableComprehension(this, graphSize);
  }
}