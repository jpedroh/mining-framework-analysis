 ...
 * 
 */
public class SimplePathDescriptionEvaluator extends PrimaryPathDescriptionEvaluator {
  public SimplePathDescriptionEvaluator(SimplePathDescription vertex, GreqlEvaluator eval) {
    super(vertex, eval);
  }
  ...
  @Override public NFA evaluate() {
    ...
    createdNFA = NFA.createSimplePathDescriptionNFA(getEdgeDirection(vertex), typeCollection, getEdgeRoles(edgeRestEval), predicateEvaluator, vertexEvalMarker);
    return createdNFA;
  }

  @Override public VertexCosts calculateSubtreeEvaluationCosts(GraphSize graphSize) {
    return this.greqlEvaluator.getCostModel().calculateCostsSimplePathDescription(this, graphSize);
  }
}