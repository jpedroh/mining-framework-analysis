...
public class MapConstructionEvaluator extends VertexEvaluator<MapConstruction> {
  ...
  @Override public Object evaluate(InternalGreqlEvaluator evaluator) {
    ...
    for (IsKeyExprOfConstruction e : vertex.getIsKeyExprOfConstructionIncidences(EdgeDirection.IN)) {
      Expression exp = e.getAlpha();
      VertexEvaluator<? extends Expression> expEval = query.getVertexEvaluator(exp);
      keys = keys.plus(expEval.getResult(evaluator));
    }
    PVector<Object> values = JGraLab.vector();
    for (IsValueExprOfConstruction e : vertex.getIsValueExprOfConstructionIncidences(EdgeDirection.IN)) {
      Expression exp = e.getAlpha();
      VertexEvaluator<? extends Expression> expEval = query.getVertexEvaluator(exp);
