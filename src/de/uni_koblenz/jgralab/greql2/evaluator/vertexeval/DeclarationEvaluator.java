...
public class DeclarationEvaluator extends VertexEvaluator {
  ...
  @Override public VariableDeclarationLayer evaluate() {
    ArrayList<VertexEvaluator> constraintList = new ArrayList<VertexEvaluator>();
    for (IsConstraintOf consInc : vertex.getIsConstraintOfIncidences(EdgeDirection.IN)) {
    ...
    for (IsSimpleDeclOf inc : vertex.getIsSimpleDeclOfIncidences(EdgeDirection.IN)) {
      ...
      SimpleDeclarationEvaluator simpleDeclEval = (SimpleDeclarationEvaluator) vertexEvalMarker.getMark(simpleDecl);
      @SuppressWarnings(value = { "unchecked" }) PVector<VariableDeclaration> resultCollection = (PVector<VariableDeclaration>) simpleDeclEval.getResult();
  ...
  @Override public VertexCosts calculateSubtreeEvaluationCosts(GraphSize graphSize) {
    return this.greqlEvaluator.getCostModel().calculateCostsDeclaration(this, graphSize);
  }
  ...
  @Override public long calculateEstimatedCardinality(GraphSize graphSize) {
    return greqlEvaluator.getCostModel().calculateCardinalityDeclaration(this, graphSize);
  }
}