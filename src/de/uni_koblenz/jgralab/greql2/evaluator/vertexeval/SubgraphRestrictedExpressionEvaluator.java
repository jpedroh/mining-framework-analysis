...
public class SubgraphRestrictedExpressionEvaluator extends VertexEvaluator<SubgraphRestrictedExpression> {
  ...
  public SubgraphRestrictedExpressionEvaluator(SubgraphRestrictedExpression vertex, QueryImpl query) {
    super(vertex, query);
  }
  ...
  @Override public Object evaluate(InternalGreqlEvaluator evaluator) {
    if (subgraphDefinitionEval == null) {
      ...
      subgraphDefinitionEval = (SubgraphDefinitionEvaluator<?>) query.getVertexEvaluator(defVertex);
    }
    TraversalContext subgraph = (TraversalContext) subgraphDefinitionEval.getResult(evaluator);
    if (exprEval == null) {
