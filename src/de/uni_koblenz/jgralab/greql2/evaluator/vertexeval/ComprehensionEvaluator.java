...
<<<<<<< /usr/src/app/output/jgralab/jgralab/55c5691ba6b104f745af5ca7f98479dba901c8bb/src/de/uni_koblenz/jgralab/greql2/evaluator/vertexeval/ComprehensionEvaluator.java/left.java
public abstract class ComprehensionEvaluator<V extends Comprehension> extends VertexEvaluator<V> {
  ...
  protected abstract PCollection<Object> getResultDatastructure(InternalGreqlEvaluator evaluator);

  protected final VertexEvaluator<? extends Expression> getResultDefinitionEvaluator() {
    if (resultDefinitionEvaluator == null) {
      Expression resultDefinition = (Expression) getVertex().getFirstIsCompResultDefOfIncidence(EdgeDirection.IN).getAlpha();
      resultDefinitionEvaluator = query.getVertexEvaluator(resultDefinition);
    }
  ...
  protected final VariableDeclarationLayer getVariableDeclationLayer(InternalGreqlEvaluator evaluator) {
    if (varDeclLayer == null) {
      Declaration d = (Declaration) getVertex().getFirstIsCompDeclOfIncidence(EdgeDirection.IN).getAlpha();
      DeclarationEvaluator declEval = (DeclarationEvaluator) query.getVertexEvaluator(d);
      varDeclLayer = (VariableDeclarationLayer) declEval.getResult(evaluator);
    }
    return varDeclLayer;
  }
  ...
  @Override public Object evaluate(InternalGreqlEvaluator evaluator) {
    if (getVertex().get_maxCount() != null) {
      VertexEvaluator<? extends Expression> maxCountEval = query.getVertexEvaluator(getVertex().get_maxCount());
    ...
    declLayer.reset();
    while (declLayer.iterate(evaluator) && (resultCollection.size() < maxCount)) {
...
public abstract class ComprehensionEvaluator extends VertexEvaluator {
  ...
  @Override public abstract Comprehension getVertex();
  ...
  protected final VertexEvaluator getResultDefinitionEvaluator() {
    if (resultDefinitionEvaluator == null) {
      Expression resultDefinition = (Expression) getVertex().getFirstIsCompResultDefOfIncidence(EdgeDirection.IN).getAlpha();
      resultDefinitionEvaluator = vertexEvalMarker.getMark(resultDefinition);
    }
    return resultDefinitionEvaluator;
  }
  ...
  protected final VariableDeclarationLayer getVariableDeclationLayer() {
    if (varDeclLayer == null) {
      Declaration d = (Declaration) getVertex().getFirstIsCompDeclOfIncidence(EdgeDirection.IN).getAlpha();
      DeclarationEvaluator declEval = (DeclarationEvaluator) vertexEvalMarker.getMark(d);
  ...
  protected final void initializeMaxCount() {
    if (getVertex().get_maxCount() != null) {
      VertexEvaluator maxCountEval = vertexEvalMarker.getMark(getVertex().get_maxCount());
...
>>>>>>> /usr/src/app/output/jgralab/jgralab/55c5691ba6b104f745af5ca7f98479dba901c8bb/src/de/uni_koblenz/jgralab/greql2/evaluator/vertexeval/ComprehensionEvaluator.java/right.java
