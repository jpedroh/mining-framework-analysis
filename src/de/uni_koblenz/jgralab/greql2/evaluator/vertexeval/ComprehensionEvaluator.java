...
public abstract class ComprehensionEvaluator<V extends Comprehension> extends VertexEvaluator<V> {
  ...
  protected final VertexEvaluator<? extends Expression> getResultDefinitionEvaluator() {
    if (resultDefinitionEvaluator == null) {
      Expression resultDefinition = getVertex().getFirstIsCompResultDefOfIncidence(EdgeDirection.IN).getAlpha();
  ...
  protected final VariableDeclarationLayer getVariableDeclationLayer(InternalGreqlEvaluator evaluator) {
    if (varDeclLayer == null) {
      Declaration d = getVertex().getFirstIsCompDeclOfIncidence(EdgeDirection.IN).getAlpha();
