...
public class ListRangeConstructionEvaluator extends VertexEvaluator<ListRangeConstruction> {
  /**
	 ...
	 */
  public ListRangeConstructionEvaluator(ListRangeConstruction vertex, QueryImpl query) {
    super(vertex, query);
  }
  ...
  private void getEvals() {
    Expression firstElementExpression = vertex.getFirstIsFirstValueOfIncidence(EdgeDirection.IN).getAlpha();
    Expression lastElementExpression = vertex.getFirstIsLastValueOfIncidence(EdgeDirection.IN).getAlpha();
    firstElementEvaluator = query.getVertexEvaluator(firstElementExpression);
