...
public abstract class VertexEvaluator<V extends Greql2Vertex> {
  ...
  /**
	 ...
	 */
  public static <V extends Greql2Vertex> VertexEvaluator<V> createVertexEvaluator(V vertex, QueryImpl query) {
    ...
    try {
      Class<?> argsClass[] = new Class[] { Class.forName(fullClassName), QueryImpl.class };
      Class<?> evalClass = Class.forName(evalName);
      Constructor<?> constructor = evalClass.getConstructor(argsClass);
      @SuppressWarnings(value = { "unchecked" }) VertexEvaluator<V> vertexEval = (VertexEvaluator<V>) constructor.newInstance(vertex, query);
      return vertexEval;
