...
<<<<<<< /usr/src/app/output/jgralab/jgralab/693df1d429d23b9f14b883047743fb3c1d785f91/src/de/uni_koblenz/jgralab/greql2/evaluator/GreqlEvaluator.java/left.java
public interface GreqlEvaluator {
  public Object getResult();

  public <T extends java.lang.Object> T getSingleResult();

  public <T extends java.lang.Object> PVector<T> getResultList();

  public <K extends java.lang.Object, V extends java.lang.Object> PMap<K, V> getResultMap();

  public <T extends java.lang.Object> POrderedSet<T> getResultSet();

  public Object evaluate(QueryImpl query, Graph datagraph, Map<String, Object> variables, ProgressFunction progressFunction);
}
=======
/**
...
public class GreqlEvaluator {
  public static void main(String[] args) throws FileNotFoundException, IOException, GraphIOException {
    if ((args.length < 1) || (args.length > 2)) {
      System.err.println("Usage: java GreqlEvaluator <query> [<graphfile>]");
      System.exit(1);
    }
    ...
    if (args.length == 2) {
      datagraph = GraphIO.loadGraphFromFile(args[1], new ConsoleProgressFunction("Loading"));
    }
  ...
  private static Logger logger = Logger.getLogger(GreqlEvaluator.class.getName());
  ...
  /**
	 ...
	 */
  public static synchronized PSet<Vertex> getVertexIndex(Graph graph, String queryPart) {
  ...
  /**
	 ...
	 */
  public static synchronized void addVertexIndex(Graph graph, String queryPart, PSet<Vertex> vertexSet) {
    SoftReference<GraphIndex> ref = graphIndizes.get(graph);
  ...
  /**
	 ...
	 */
  protected static synchronized void addOptimizedSyntaxGraph(String queryString, SyntaxGraphEntry entry) {
    SoftReference<List<SyntaxGraphEntry>> ref = optimizedGraphs.get(queryString);
    ...
    optimizedGraphs.put(queryString, new SoftReference<List<SyntaxGraphEntry>>(entryList));
  }

  /**
	 * gets an unlocked syntaxgraph out of the optimzedGraph map and locks it
	 */
  protected static synchronized SyntaxGraphEntry getOptimizedSyntaxGraph(String queryString, Optimizer optimizer, CostModel costModel) {
    SoftReference<List<SyntaxGraphEntry>> ref = optimizedGraphs.get(queryString);
    List<SyntaxGraphEntry> entryList = null;
    if (ref != null) {
      entryList = ref.get();
    ...
    for (SyntaxGraphEntry entry : entryList) {
      if (entry.getCostModel().isEquivalent(costModel)) {
        ...
        if (((opt != null) && opt.isEquivalent(optimizer)) || ((opt == null) && (optimizer == null))) {
          if (entry.lock()) {
            return entry;
          }
        }
  ...
  /**
	 ...
	 */
  public static synchronized void loadOptimizedSyntaxGraphs() throws GraphIOException {
    if (!(optimizedSyntaxGraphsDirectory.exists() && optimizedSyntaxGraphsDirectory.canRead() && optimizedSyntaxGraphsDirectory.canExecute())) {
      throw new GraphIOException(optimizedSyntaxGraphsDirectory.getPath() + " is not accessible.  Does it really exist and are the permissions ok?");
    }
    for (File syntaxGraphFile : optimizedSyntaxGraphsDirectory.listFiles(TGFilenameFilter.instance())) {
      logger.info("Loading SyntaxGraphEntry \"" + syntaxGraphFile.getPath() + "\".");
  ...
  public static synchronized void saveOptimizedSyntaxGraphs() {
    for (SoftReference<List<SyntaxGraphEntry>> ref : optimizedGraphs.values()) {
  ...
  public void setSubQuery(String name, String greqlQuery) {
    ...
    if (!name.matches("^\\w+$")) {
      throw new GreqlException("Invalid subquery name \'" + name + "\'. Only word chars are allowed.");
    }
    if (FunLib.contains(name)) {
      throw new GreqlException("The subquery \'" + name + "\' would shadow a GReQL function!");
    }
    Set<String> definedSubQueries = subQueryMap.keySet();
    HashSet<String> subQueryNames = new HashSet<String>(definedSubQueries.size() + 1);
    ...
    for (FunctionApplication fa : subQueryGraph.getFunctionApplicationVertices()) {
      if (name.equals(fa.get_functionId().get_name())) {
        throw new GreqlException("The subquery \'" + name + "\' is recursive.  That\'s not allowed!");
      }
    }
  ...
  @SuppressWarnings(value = { "unchecked" }) public <T extends java.lang.Object> T getSingleResult() {
    return (T) result;
  }

  @SuppressWarnings(value = { "unchecked" }) public <T extends java.lang.Object> PVector<T> getResultList() {
    return (PVector<T>) result;
  }

  @SuppressWarnings(value = { "unchecked" }) public <K extends java.lang.Object, V extends java.lang.Object> PMap<K, V> getResultMap() {
    return (PMap<K, V>) result;
  }

  @SuppressWarnings(value = { "unchecked" }) public <T extends java.lang.Object> POrderedSet<T> getResultSet() {
  ...
  /**
	 ...
	 */
  public GreqlEvaluator(String query, Graph datagraph, Map<String, Object> variables, ProgressFunction progressFunction) {
    this(datagraph, variables, progressFunction);
    setQuery(query);
  }

  private GreqlEvaluator(Graph datagraph, Map<String, Object> variables, ProgressFunction progressFunction) {
  ...
  /**
	 ...
	 *            a Map<String, JValue> of bound variables
	 */
  public GreqlEvaluator(String query, Graph datagraph, Map<String, Object> variables) {
  ...
  /**
	 ...
	 *            instance display a progress bar etc.
	 */
  public GreqlEvaluator(File queryFile, Graph datagraph, Map<String, Object> variables, ProgressFunction progressFunction) throws FileNotFoundException, IOException {
    this(datagraph, variables, progressFunction);
  ...
  /**
	 ...
	 *            a Map<String, JValue> of bound variables
	 */
  public GreqlEvaluator(File queryFile, Graph datagraph, Map<String, Object> variables) throws FileNotFoundException, IOException {
  ...
  private void weaveInSubQueries() {
    ...
    while (subqueryCall != null) {
      TGMerge tgm = new TGMerge(queryGraph, subQueryMap.get(subqueryCall.get_functionId().get_name()));
      tgm.merge();
  ...
  private FunctionApplication findSubQueryCall() {
    for (FunctionApplication fa : queryGraph.getFunctionApplicationVertices()) {
  ...
  private void weaveInSubquery(FunctionApplication fa) {
    ...
    if (args.size() != boundVars.size()) {
      throw new GreqlException("Subquery call to \'" + name + "\' has " + args.size() + " arguments, but the subquery definition has " + boundVars.size() + " formal parameters!");
    }
  ...
  public void createVertexEvaluators() {
    ...
    while (currentVertex != null) {
      VertexEvaluator vertexEval = VertexEvaluator.createVertexEvaluator(currentVertex, this);
      if (vertexEval != null) {
  ...
  private void resetVertexEvaluators() {
    Vertex currentVertex = queryGraph.getFirstVertex();
    while (currentVertex != null) {
      VertexEvaluator vertexEval = vertexEvalGraphMarker.getMark(currentVertex);
      if (vertexEval != null) {
  ...
  public void createOptimizedSyntaxGraph() throws OptimizerException {
    ...
    if (useSavedOptimizedSyntaxGraph && optimizedGraphs.containsKey(queryString)) {
      syntaxGraphEntry = getOptimizedSyntaxGraph(queryString, optimizer, costModel);
    ...
    if (DEBUG_OPTIMIZATION) {
      ...
      try {
        queryGraph.save(name + "tg", new ConsoleProgressFunction("Saving broken GReQL graph:"));
    ...
    syntaxGraphEntry = new SyntaxGraphEntry(queryString, queryGraph, optimizer, costModel, true);
  ...
  public boolean startEvaluation() throws OptimizerException {
    ...
    if (optimize) {
      ...
      if (DEBUG_OPTIMIZATION) {
        ...
        try {
          queryGraph.save(name + "tg", new ConsoleProgressFunction("Saving"));
          printGraphAsDot(queryGraph, true, name + "dot");
        } catch (GraphIOException e) {
          e.printStackTrace();
        }
        System.out.println("Saved optimized query graph to " + name + "tg/dot.");
        System.out.println("#########################################################");
      }
    } else {
      parseQuery(queryString);
    ...
    if (queryGraph == null) {
      throw new GreqlException("Query graph is null. Evaluation impossible.");
    }
    if (queryGraph.getVCount() <= 1) {
      result = Undefined.UNDEFINED;
      return true;
    }
    VertexEvaluator greql2ExpEval = vertexEvalGraphMarker.getMark(queryGraph.getFirstGreql2Expression());
    if (progressFunction != null) {
      estimatedInterpretationSteps = greql2ExpEval.getInitialSubtreeEvaluationCosts(new GraphSize(datagraph));
    ...
    plainEvaluationTime = System.currentTimeMillis() - plainEvaluationStartTime;
  ...
  private void printGraphAsDot(Graph graph, boolean reversedEdges, String outputFilename) {
    try {
      Class<?> tg2DotClass = Class.forName("de.uni_koblenz.jgralab.utilities.tg2dot.Tg2Dot");
      Method printMethod = tg2DotClass.getMethod("convertGraph", Graph.class, String.class, boolean.class);
      printMethod.invoke(tg2DotClass, new Object[] { graph, outputFilename, reversedEdges });
  ...
  public void printEvaluationTimes() {
    logger.info("Overall evaluation took " + overallEvaluationTime / 1000d + " seconds.\n" + " --> parsing time         : " + parseTime / 1000d + "\n --> optimization time    : " + optimizationTime / 1000d + "\n --> plain evaluation time: " + plainEvaluationTime / 1000d + "\n" + (progressFunction != null ? "Estimated evaluation costs: " + estimatedInterpretationSteps : ""));
  }
  ...
  public static void setOptimizedSyntaxGraphsDirectory(File optimizedSyntaxGraphsDirectory) {
  ...
  public void setUseSavedOptimizedSyntaxGraph(boolean useSavedOptimizedSyntaxGraph) {
    this.useSavedOptimizedSyntaxGraph = useSavedOptimizedSyntaxGraph;
  }
}
>>>>>>> /usr/src/app/output/jgralab/jgralab/693df1d429d23b9f14b883047743fb3c1d785f91/src/de/uni_koblenz/jgralab/greql2/evaluator/GreqlEvaluator.java/right.java
