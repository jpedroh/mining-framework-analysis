...
public class EarlySelectionOptimizer extends OptimizerBase {
  ...
  private boolean runOptimization() throws OptimizerException {
    ...
    Collections.sort(simpleDeclsWithMovableExpressions, new Comparator<SimpleDeclaration>() {
      @Override public int compare(SimpleDeclaration sd1, SimpleDeclaration sd2) {
        Declaration decl1 = (Declaration) sd1.getFirstIsSimpleDeclOfIncidence().getOmega();
        Declaration decl2 = (Declaration) sd2.getFirstIsSimpleDeclOfIncidence().getOmega();
    ...
    for (SimpleDeclaration sd : simpleDeclsWithMovableExpressions) {
      Declaration parentDecl = (Declaration) sd.getFirstIsSimpleDeclOfIncidence().getOmega();
  ...
  private void movePredicatesToMultiVarSimpleDeclaration(SimpleDeclaration origSD, Set<Expression> predicates, Set<Variable> varsDeclaredByOrigSD) throws OptimizerException {
    ...
    Declaration parentDeclOfOrigSD = (Declaration) origSD.getFirstIsSimpleDeclOfIncidence(EdgeDirection.OUT).getOmega();
    ...
    ((InternalEdge) origSD.getFirstIsSimpleDeclOfIncidence(EdgeDirection.OUT)).setOmega(newInnerDecl);
  ...
  private void movePredicatesToOneVarSimpleDeclaration(SimpleDeclaration origSD, Set<Expression> predicates, Set<Variable> varsDeclaredByOrigSD) throws OptimizerException {
    ...
    logger.finer(optimizerHeaderString() + "(M1) Performing early selection transformation for " + origSD + " declaring variable " + var + " (" + var.get_name() + ") with predicates " + predicates);
    ...
    ((InternalEdge) origSD.getFirstIsTypeExprOfIncidence(EdgeDirection.IN)).setOmega(newInnerSD);
    ...
    for (Expression exp : predicates) {
      removeExpressionFromOriginalConstraint(exp, (Declaration) origSD.getFirstIsSimpleDeclOfIncidence().getOmega());
    }
  }
  ...
  private HashMap<SimpleDeclaration, Set<Expression>> collectMovableExpressions(Expression exp) {
    ...
    if ((exp instanceof FunctionApplication) && OptimizerUtility.isAnd((FunctionApplication) exp)) {
      ...
      while (isArg != null) {
        for (Entry<SimpleDeclaration, Set<Expression>> entry : collectMovableExpressions((Expression) isArg.getAlpha()).entrySet()) {
    ...
    if (sd != null) {
      Declaration parent = (Declaration) sd.getFirstIsSimpleDeclOfIncidence(EdgeDirection.OUT).getOmega();
  ...
  private boolean existsForwardPathExcludingOtherTargetClassVertices(Edge edge, Vertex target) {
    ...
    if (omega.getAttributedElementClass().getM1Class() == target.getAttributedElementClass().getM1Class()) {
      return false;
    }
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "unchecked" }) private Vertex copySubgraph(Vertex origVertex, Greql2 graph, Set<Variable> variablesToBeCopied, HashMap<Variable, Variable> copiedVarMap) {
    ...
    Class<? extends Vertex> vertexClass = (Class<? extends Vertex>) origVertex.getAttributedElementClass().getM1Class();
    ...
    while (origEdge != null) {
      ...
      Class<? extends Edge> edgeClass = (Class<? extends Edge>) origEdge.getAttributedElementClass().getM1Class();
