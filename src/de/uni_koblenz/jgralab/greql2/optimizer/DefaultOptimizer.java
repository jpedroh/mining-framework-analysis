...
public class DefaultOptimizer extends OptimizerBase {
  ...
  public boolean optimize(GreqlEvaluator eval, Greql2 syntaxgraph) throws OptimizerException {
    ...
    int noOfRuns = 1;
    while (cso.optimize(eval, syntaxgraph) | txfao.optimize(eval, syntaxgraph) | cso.optimize(eval, syntaxgraph) | mco.optimize(eval, syntaxgraph) | pe2dpeo.optimize(eval, syntaxgraph) | eso.optimize(eval, syntaxgraph) | cso.optimize(eval, syntaxgraph) | vdoo.optimize(eval, syntaxgraph) | cso.optimize(eval, syntaxgraph) | peo.optimize(eval, syntaxgraph) | ceo.optimize(eval, syntaxgraph) | cso.optimize(eval, syntaxgraph) | msdo.optimize(eval, syntaxgraph)) {
  ...
  @SuppressWarnings(value = { "unused" }) private void printCosts(GreqlEvaluator eval, Greql2 syntaxgraph) {
    ...
    long estimatedInterpretationSteps = greql2ExpEval.getCurrentSubtreeEvaluationCosts(graphSize);
