...
public class DefaultOptimizer extends Optimizer {
  private static Logger logger = JGraLab.getLogger(DefaultOptimizer.class.getPackage().getName());

  /**
	 * Print the text representation of the optimized query after optimization.
	 */
  private static boolean debugOptimization = Boolean.parseBoolean(System.getProperty("greqlDebugOptimization", "false"));
  ...
  public static void optimizeQuery(Greql2Graph g) {
    if (debugOptimization) {
      ...
      try {
        g.save(name + ".tg", new ConsoleProgressFunction("Saving broken GReQL graph:"));
  ...
  @Override protected String optimizerHeaderString() {
    return "### " + this.getClass().getSimpleName() + ": ";
  }

  private static void printGraphAsDot(Graph graph, boolean reversedEdges, String outputFilename) {
    try {
  ...
  @Override protected boolean isEquivalent(Optimizer optimizer) {
  ...
  @Override public boolean optimize(Greql2Graph syntaxgraph) throws OptimizerException {
    ...
    int noOfRuns = 1;
    while (cso.optimize(eval, syntaxgraph) | txfao.optimize(eval, syntaxgraph) | cso.optimize(syntaxgraph) | mco.optimize(syntaxgraph) | pe2dpeo.optimize(syntaxgraph) | eso.optimize(syntaxgraph) | cso.optimize(syntaxgraph) | vdoo.optimize(syntaxgraph) | cso.optimize(syntaxgraph) | peo.optimize(syntaxgraph) | ceo.optimize(syntaxgraph) | cso.optimize(syntaxgraph) | msdo.optimize(syntaxgraph)) {
      ...
      logger.fine(optimizerHeaderString() + "starts a new iteration (" + noOfRuns + ")...");
    }
    logger.fine(optimizerHeaderString() + " finished after " + noOfRuns + " iterations.");
    return aTransformationWasDone;
  }

  @SuppressWarnings(value = { "unused" }) private void printCosts(GreqlEvaluator eval, Greql2Graph syntaxgraph) {
    ...
    GraphMarker<VertexEvaluator> marker = eval.getVertexEvaluatorGraphMarker();
    ...
    VertexEvaluator greql2ExpEval = marker.getMark(syntaxgraph.getFirstGreql2Expression());
