...
public abstract class Formula {
  ...
  private static Formula createFormulaFromExpressionInternal(GreqlEvaluator eval, Expression exp) {
    ...
    if (exp instanceof FunctionApplication) {
      ...
      if (OptimizerUtility.isAnd(funApp)) {
        ...
        Expression rightArg = (Expression) inc.getNextIsArgumentOfIncidence(EdgeDirection.IN).getAlpha();
  ...
  private boolean isFunApp(Vertex exp, String functionName) {
    if (exp instanceof FunctionApplication) {
      FunctionApplication funApp = (FunctionApplication) exp;
      return ((FunctionId) funApp.getFirstIsFunctionIdOfIncidence().getAlpha()).get_name().equals(functionName);
    }
    return false;
  }
