...
>>>>>>> /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/greql2/optimizer/condexp/Not.java/right.java
  ...
  @Override protected Formula calculateReplacementFormula(Expression exp, Literal literal) {
    return new Not(greqlEvaluator, formula.calculateReplacementFormula(exp, literal));
  }
  ...
  @Override public Formula simplify() {
    ...
    if (f instanceof And) {
      ...
      return new Or(greqlEvaluator, new Not(greqlEvaluator, left), new Not(greqlEvaluator, right)).simplify();
    }
    if (f instanceof Or) {
      Or or = (Or) f;
      Formula left = or.leftHandSide;
      Formula right = or.rightHandSide;
      return new And(greqlEvaluator, new Not(greqlEvaluator, left), new Not(greqlEvaluator, right)).simplify();
    }
