...
public class PathExistenceToDirectedPathExpressionOptimizer extends OptimizerBase {
  private static Logger logger = JGraLab.getLogger(PathExistenceToDirectedPathExpressionOptimizer.class.getPackage().getName());
  ...
  private boolean tryOptimizePathExistence(PathExistence pe) {
    ...
    if ((ibvoStart != null) && (ibvoTarget != null)) {
      ...
      if ((ibvoStart != null) && (ibvoTarget == null)) {
        anchorVar = start;
        sdsVar = target;
        SimpleDeclaration targetSD = (SimpleDeclaration) target.getFirstIsDeclaredVarOfIncidence().getOmega();
        ...
        if ((ibvoStart == null) && (ibvoTarget != null)) {
          anchorVar = target;
          sdsVar = start;
          SimpleDeclaration startSD = (SimpleDeclaration) start.getFirstIsDeclaredVarOfIncidence().getOmega();
    ...
    if (!isConstraintAndTopLevelConjunction(pe, (Declaration) sd.getFirstIsSimpleDeclOfIncidence().getOmega())) {
      logger.finer(optimizerHeaderString() + pe + " cannot be optimized, cause it\'s not in an constraint conjunction...");
      return false;
    }
    ...
    PathExpression directedVS = createForwardOrBackwardVertexSet(forward, anchorVar, path, null);
  ...
  private boolean isOptimizablePathDescription(PathDescription pd) {
    ...
    for (IsPathDescriptionOf ipdo : pd.getIsPathDescriptionOfIncidences(EdgeDirection.IN)) {
      boolean isOptimizable = isOptimizablePathDescription((PathDescription) ipdo.getThat());
      if (!isOptimizable) {
        return false;
      }
    }
  ...
  private boolean optimizeVertexSetExpression(PathExistence pe, SimpleDeclaration sd, Variable otherVar, VertexSetExpression vse) {
    ...
    PathExpression newPE = createForwardOrBackwardVertexSet(forward, otherVar, (PathDescription) pe.get_path(), vse.get_typeRestr());
    ...
    logger.finer(optimizerHeaderString() + "Created " + newPE + " as optimization...");
    return true;
  }
