...
public class VariableDeclarationOrderOptimizer extends OptimizerBase {
  private static Logger logger = JGraLab.getLogger(VariableDeclarationOrderOptimizer.class.getPackage().getName());
  ...
  @Override public boolean optimize(GreqlEvaluator eval, Greql2 syntaxgraph) throws OptimizerException {
    ...
    for (Declaration decl : syntaxgraph.getDeclarationVertices()) {
      ...
      for (Variable var : varsOfDecl) {
        units.add(new VariableDeclarationOrderUnit(var, decl, marker, graphSize));
      }
      unitsList.add(units);
    }
    ...
    for (List<VariableDeclarationOrderUnit> units : unitsList) {
      ...
      if (!varDeclOrderAfter.equals(varDeclOrderBefore)) {
        ...
        logger.finer(optimizerHeaderString() + "New order of declarations in " + declaringDecl);
        int i = 0;
        for (VariableDeclarationOrderUnit unit : units) {
          ...
          marker.mark(newSD, new SimpleDeclarationEvaluator(newSD, eval));
        }
      }
    }
  ...
  private List<Variable> collectVariablesInDeclarationOrder(Declaration decl) {
    ...
    for (IsSimpleDeclOf isSD : decl.getIsSimpleDeclOfIncidences()) {
      for (IsDeclaredVarOf isVar : ((SimpleDeclaration) isSD.getAlpha()).getIsDeclaredVarOfIncidences()) {
  ...
  private Set<Variable> collectVariablesDeclaredBy(Declaration decl) {
    ...
    for (IsSimpleDeclOf inc : decl.getIsSimpleDeclOfIncidences(EdgeDirection.IN)) {
      vars.addAll(OptimizerUtility.collectVariablesDeclaredBy((SimpleDeclaration) inc.getAlpha()));
    }
    return vars;
  }
}