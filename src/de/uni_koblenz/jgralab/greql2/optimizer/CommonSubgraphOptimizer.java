...
public class CommonSubgraphOptimizer extends OptimizerBase {
  private static Logger logger = JGraLab.getLogger(CommonSubgraphOptimizer.class.getPackage().getName());
  ...
  /**
	 ...
	 */
  private void mergeVertices(Greql2Vertex lowerVertex, Greql2Vertex higherVertex) {
    if (!(lowerVertex instanceof PathDescription)) {
      anOptimizationWasDone = true;
      logger.finer(optimizerHeaderString() + "Merging " + lowerVertex + " and " + higherVertex + ".");
  ...
  /**
	 ...
	 *            another {@link Greql2Vertex}
	 */
  private void mergeSourcePositionsBelow(Greql2Vertex lowerVertex, Greql2Vertex higherVertex) {
    Greql2Aggregation gal = lowerVertex.getFirstGreql2AggregationIncidence(EdgeDirection.IN);
    Greql2Aggregation gah = higherVertex.getFirstGreql2AggregationIncidence(EdgeDirection.IN);
    while ((gal != null) && (gah != null)) {
      OptimizerUtility.mergeSourcePositions(gah, gal);
      mergeSourcePositionsBelow((Greql2Vertex) gal.getAlpha(), (Greql2Vertex) gah.getAlpha());
