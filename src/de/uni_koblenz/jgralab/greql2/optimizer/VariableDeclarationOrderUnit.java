...
public class VariableDeclarationOrderUnit implements Comparable<VariableDeclarationOrderUnit> {
  ...
  /**
	 ...
	 */
  VariableDeclarationOrderUnit(Variable var, Declaration declaringDecl, GraphMarker<VertexEvaluator> marker, GraphSize graphSize) {
    this.variable = var;
    this.declaringDeclaration = declaringDecl;
    this.vertexEvalMarker = marker;
    this.graphSize = graphSize;
    this.simpleDeclarationOfVariable = (SimpleDeclaration) this.variable.getFirstIsDeclaredVarOfIncidence(EdgeDirection.OUT).getOmega();
    this.typeExpressionOfVariable = (Expression) this.simpleDeclarationOfVariable.getFirstIsTypeExprOfIncidence(EdgeDirection.IN).getAlpha();
  ...
  private void addDependendVertices(Vertex vertex) {
    if ((vertex == simpleDeclarationOfVariable) || (vertex == declaringDeclaration)) {
      return;
    }
    if (variable != vertex) {
  ...
  @Override public String toString() {
    return dependentVertices.size() + " Expressions depend on " + variable + " (" + variable.get_name() + ") resulting in costs of " + getVariableValueChangeCosts() + " on value changes.";
  }

  @Override public int compareTo(VariableDeclarationOrderUnit o) {
    if (this == o) {
      return 0;
    }
  ...
  private long calculateTypeExpressionCardinality() {
    VertexEvaluator veval = vertexEvalMarker.getMark(typeExpressionOfVariable);
