 ...
 * 
 */
public class PathExistenceOptimizer extends Optimizer {
  private static Logger logger = JGraLab.getLogger(PathExistenceOptimizer.class.getPackage().getName());
  ...
  @Override protected boolean optimize(Greql2Graph syntaxgraph, CostModel costModel) throws OptimizerException {
...
<<<<<<< /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/greql2/optimizer/PathExistenceOptimizer.java/left.java
    pe.getFirstIsStartExprOfIncidence(EdgeDirection.IN).getAlpha()
=======
    (Expression) pe.getFirstIsStartExprOfIncidence(EdgeDirection.IN).getAlpha()
...
<<<<<<< /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/greql2/optimizer/PathExistenceOptimizer.java/left.java
    pe.getFirstIsTargetExprOfIncidence(EdgeDirection.IN).getAlpha()
=======
    (Expression) pe.getFirstIsTargetExprOfIncidence(EdgeDirection.IN).getAlpha()
>>>>>>> /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/greql2/optimizer/PathExistenceOptimizer.java/right.java
    ;
    ...
    if (startExpVars.isEmpty() || (!targetExpVars.isEmpty() && isDeclaredBefore(startExpVars.last(), targetExpVars.last()))) {
      replacePathExistenceWithContainsFunApp(pe, startExp, targetExp, true);
    } else {
      if (targetExpVars.isEmpty() || (!startExpVars.isEmpty() && isDeclaredBefore(targetExpVars.last(), startExpVars.last()))) {
        replacePathExistenceWithContainsFunApp(pe, targetExp, startExp, false);
      }
    }
  }
  ...
  private void replacePathExistenceWithContainsFunApp(PathExistence pe, Expression startOrTargetExp, Expression otherExp, boolean forward) {
    logger.finer(optimizerHeaderString() + "Replacing " + pe + " with a contains FunctionApplication using a " + ((forward) ? "Forward" : "Backward") + "VertexSet.");
    ...
    FunctionId containsId = OptimizerUtility.findOrCreateFunctionId("contains", syntaxgraph);
...
=======
    (Expression) pe.getFirstIsPathOfIncidence(EdgeDirection.IN).getAlpha()
>>>>>>> /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/greql2/optimizer/PathExistenceOptimizer.java/right.java
    , vertexSet);
