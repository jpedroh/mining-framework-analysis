...
public class PathExistenceOptimizer extends OptimizerBase {
  ...
  @Override public boolean optimize(QueryImpl query) throws OptimizerException {
  ...
  private void maybeTransformPathExistence(PathExistence pe) {
    Expression startExp = (Expression) pe.getFirstIsStartExprOfIncidence(EdgeDirection.IN).getAlpha();
    Expression targetExp = (Expression) pe.getFirstIsTargetExprOfIncidence(EdgeDirection.IN).getAlpha();
  ...
  private void replacePathExistenceWithContainsFunApp(PathExistence pe, Expression startOrTargetExp, Expression otherExp, boolean forward) {
    ...
    syntaxgraph.createIsPathOf((Expression) pe.getFirstIsPathOfIncidence(EdgeDirection.IN).getAlpha(), vertexSet);
