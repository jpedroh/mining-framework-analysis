...
public class MergeConstraintsOptimizer extends Optimizer {
  private static Logger logger = JGraLab.getLogger(MergeConstraintsOptimizer.class.getPackage().getName());
  ...
  @Override protected boolean optimize(Greql2Graph syntaxgraph, CostModel costModel) {
    ...
    for (Declaration decl : declarations) {
      ...
      if (constraintEdges.size() > 1) {
        ...
        Expression singleConstraint = createConjunction(constraintEdges, syntaxgraph);
        logger.finer(optimizerHeaderString() + "Merging constraints on edges " + constraintEdges + " into conjunction " + singleConstraint + ".");
  ...
  /**
	 ...
	 */
  public Expression createConjunction(List<IsConstraintOf> constraintEdges, Greql2Graph syntaxgraph) {
    if (constraintEdges.size() == 1) {
    ...
    syntaxgraph.createIsArgumentOf((Expression) constraintEdges.get(0).getAlpha(), funApp);
    syntaxgraph.createIsArgumentOf(createConjunction(constraintEdges.subList(1, constraintEdges.size()), syntaxgraph), funApp);
    return funApp;
  }
}