...
@NeedsEvaluatorArgument public class Slice extends Function {
  public Slice() {
    super(1000, 1, 1.0);
  }
  ...
  protected boolean markVertex(Vertex v, State s, Vertex parentVertex, Edge e, State ps, int d) {
    ...
    GraphMarker<Map<Edge, PathSystemMarkerEntry>> currentMarker = marker.get(s.number);
  ...
  /**
	 ...
	 * @return
	 */
  private de.uni_koblenz.jgralab.greql2.types.Slice createSliceFromMarkings(Graph graph, Set<Vertex> sliCritVertices, List<Vertex> leaves) {
    de.uni_koblenz.jgralab.greql2.types.Slice slice = new de.uni_koblenz.jgralab.greql2.types.Slice(graph);
    Map<Edge, PathSystemMarkerEntry> sliCritVertexMarkerMap;
    PathSystemMarkerEntry sliCritVertexMarker;
    GraphMarker<Map<Edge, PathSystemMarkerEntry>> startStateMarker = marker.get(0);
    for (Vertex v : sliCritVertices) {
      sliCritVertexMarkerMap = startStateMarker.getMark(v);
      sliCritVertexMarker = sliCritVertexMarkerMap.get(null);
      slice.addSlicingCriterionVertex(v, sliCritVertexMarker.state.number, sliCritVertexMarker.state.isFinal);
    }
          ...
          for (PathSystemMarkerEntry currentMarker : currentGraphMarker.getMark(leaf).values()) {
            ...
            while (!queue.isEmpty()) {
              ...
              for (PathSystemMarkerEntry marker : getMarkersWithState(currentVertex, currentStateMarker.getMark(currentVertex)).values()) {
                ...
                slice.addVertex(currentVertex, marker.state.number, marker.edgeToParentVertex, marker.parentVertex, parentStateNumber, marker.state.isFinal);
