...
@NeedsEvaluatorArgument public class PathSystem extends Function {
  @Description(params = { "startVertex", "fa" }, description = "Returns a path system with the given root vertex, which is structured according to the given path description.", categories = Category.PATHS_AND_PATHSYSTEMS_AND_SLICES) public PathSystem() {
    super(1000, 1, 1.0);
  }
  ...
  public de.uni_koblenz.jgralab.greql2.types.PathSystem evaluate(InternalGreqlEvaluator evaluator, Vertex startVertex, DFA dfa) {
    @SuppressWarnings(value = { "unchecked" }) GraphMarker<PathSystemMarkerEntry>[] marker = new GraphMarker[dfa.stateList.size()];
  ...
  private Set<PathSystemMarkerEntry> markVerticesOfPathSystem(InternalGreqlEvaluator evaluator, GraphMarker<PathSystemMarkerEntry>[] marker, Vertex startVertex, DFA dfa) {
    ...
    PathSystemMarkerEntry currentEntry = markVertex(marker, startVertex, dfa.initialState, null, null, null, 0);
  ...
  /**
	 ...
	 * @return
	 */
  private de.uni_koblenz.jgralab.greql2.types.PathSystem createPathSystemFromMarkings(GraphMarker<PathSystemMarkerEntry>[] marker, Vertex rootVertex, Set<PathSystemMarkerEntry> leafEntries) {
    de.uni_koblenz.jgralab.greql2.types.PathSystem pathSystem = new de.uni_koblenz.jgralab.greql2.types.PathSystem(rootVertex.getGraph());
    PathSystemMarkerEntry rootMarker = marker[0].getMark(rootVertex);
    pathSystem.setRootVertex(rootVertex, rootMarker.state.number, rootMarker.state.isFinal);
    for (PathSystemMarkerEntry currentMarker : leafEntries) {
      ...
      while (currentVertex != null) {
        ...
        pathSystem.addVertex(currentVertex, currentMarker.state.number, currentMarker.edgeToParentVertex, currentMarker.parentVertex, parentStateNumber, currentMarker.distanceToRoot, currentMarker.state.isFinal);
        currentVertex = currentMarker.parentVertex;
        currentMarker = getMarkerWithState(marker, currentVertex, currentMarker.parentState);
      }
    }
    pathSystem.finish();
    return pathSystem;
  ...
  @Override public long getEstimatedCosts(ArrayList<Long> inElements) {
    return 1000;
  }
  ...
  @Override public long getEstimatedCardinality(int inElements) {
    return 1;
  }
}