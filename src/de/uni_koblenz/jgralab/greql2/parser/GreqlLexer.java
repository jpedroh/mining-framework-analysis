...
public class GreqlLexer {
  @SuppressWarnings(value = { "serial" }) protected static final Map<TokenTypes, String> fixedTokens = Collections.unmodifiableMap(new EnumMap<TokenTypes, String>(TokenTypes.class) {
    {
  ...
  public Token getNextToken() {
    ...
    if (recognizedTokenType == null) {
      ...
      if ((separator == '\"') || (separator == '\'')) {
        ...
        while ((position < query.length()) && (query.charAt(position) != separator)) {
          if (query.charAt(position) == '\\') {
            if (position == query.length()) {
              throw new ParsingException("String started at position " + start + " but is not closed in query", query.substring(start, position), start, position - start, query);
            }
            if ((query.charAt(position + 1) == separator) || (query.charAt(position + 1) == '\\')) {
              position++;
            }
          }
        ...
        if ((position >= query.length()) || (query.charAt(position) != separator)) {
          throw new ParsingException("String started at position " + start + " but is not closed in query", sb.toString(), start, position - start, query);
        }
        recognizedTokenType = TokenTypes.STRING;
        recognizedToken = new ComplexToken(recognizedTokenType, start, position, sb.toString());
        position++;
      } else {
        StringBuilder nextPossibleToken = new StringBuilder();
        int start = position;
        while ((query.length() > position) && (!isSeparator(query.charAt(position)))) {
        ...
        if (tokenText.equals("thisVertex")) {
          ...
          if (tokenText.equals("thisEdge")) {
            recognizedToken = new ComplexToken(TokenTypes.THISEDGE, start, position - start, tokenText);
          } else {
            if (tokenText.equals(fixedTokens.get(TokenTypes.POS_INFINITY)) || tokenText.equals(fixedTokens.get(TokenTypes.NEG_INFINITY)) || tokenText.equals(fixedTokens.get(TokenTypes.NOT_A_NUMBER))) {
              recognizedToken = matchDoubleConstantToken(start, position - start, tokenText);
            } else {
              if (startsWithNumber(tokenText)) {
    ...
    if (recognizedToken == null) {
      throw new ParsingException("Error while scanning query at position", null, position, position, query);
    }
    ...
    if (text.charAt(0) == '0') {
      if (text.length() == 1) {
        ...
        if ((text.charAt(1) == 'x') || (text.charAt(1) == 'X')) {
          ...
          try {
            ...
            throw new ParsingException("Not a valid hex number", text, start, end - start, query);
          }
        } else {
          ...
          try {
            ...
            throw new ParsingException("Not a valid octal number", text, start, end - start, query);
          }
        }
      }
    } else {
      switch (text.charAt(text.length() - 1)) {
        ...
        try {
          ...
          throw new ParsingException("Not a valid integer number", text, start, end - start, query);
        }
      }
    }
  ...
  private final void skipWs() {
    do {
      ...
      if ((position < (query.length() - 2)) && (query.substring(position, position + 2).equals("//"))) {
        position++;
      ...
      if ((position < (query.length() - 4)) && (query.substring(position, position + 2).equals("/*"))) {
        position++;
        while ((position < (query.length() - 1)) && (query.substring(position, position + 2).equals("*/"))) {
          position++;
        }
    ...
    } while(((position < query.length()) && (isWs(query.charAt(position)))) || ((position < (query.length() - 2)) && (query.substring(position, position + 2).equals("//"))) || ((position < (query.length() - 4)) && (query.substring(position, position + 2).equals("/*"))));
  }
