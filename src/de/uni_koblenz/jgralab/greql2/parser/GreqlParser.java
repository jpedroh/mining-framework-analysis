...
public class GreqlParser extends ParserHelper {
    ...
    if (inPredicateMode()) {
      if (positionOfTokenAfterRule == -1) {
        ...
        if (positionOfTokenAfterRule == -2) {
          ...
          current = positionOfTokenAfterRule;
          return -1;
        }
      }
    }
  ...
  protected final boolean isFunctionName(String ident) {
    return ((subQueryNames != null) && subQueryNames.contains(ident)) || FunLib.contains(ident);
  }
  ...
  @Override protected void debug(String s) {
  ...
  protected final PVector<SourcePosition> createSourcePositionList(int startOffset) {
    ...
    return list.plus(new SourcePosition(getCurrentOffset() - startOffset, startOffset));
  }
  ...
  private final ValueConstruction createPartsOfValueConstruction(List<VertexPosition<Expression>> expressions, ValueConstruction parent) {
    return (ValueConstruction) createMultipleEdgesToParent(expressions, parent, Greql2Schema.instance().ec_IsPartOf);
  }
  ...
  private final Vertex createMultipleEdgesToParent(List<VertexPosition<Expression>> expressions, Vertex parent, EdgeClass edgeClass) {
    if (expressions != null) {
      for (VertexPosition<? extends Vertex> expr : expressions) {
        Greql2Aggregation edge = (Greql2Aggregation) graph.createEdge(edgeClass, expr.node, parent);
        edge.set_sourcePositions(createSourcePositionList(expr.length, expr.offset));
      }
    }
    return parent;
  }
  ...
  @Override protected boolean inPredicateMode() {
  ...
  private final void fail(String msg) {
    ...
    ParsingException ex = new ParsingException(msg + " " + lookAhead(0), tokenText, offset, length, query);
  ...
  private final static boolean isValidSimpleName(String s) {
    ...
    char[] chars = s.toCharArray();
    if (!Character.isLetter(chars[0]) || !Character.isUpperCase(chars[0]) || (chars[0] > 127)) {
      return false;
    }
    for (int i = 1; i < chars.length; i++) {
      if (chars[i] > 127) {
  ...
  private final List<VertexPosition<Variable>> parseVariableList() {
    ...
    int offset = getCurrentOffset();
    vlist.add(new VertexPosition<Variable>(parseVariable(true), getLength(offset), offset));
  ...
  private final Quantifier parseQuantifier() {
    ...
    if (tryMatch(TokenTypes.FORALL)) {
      ...
      if (tryMatch(TokenTypes.EXISTS_ONE)) {
        type = QuantificationType.EXISTSONE;
      } else {
        if (tryMatch(TokenTypes.EXISTS)) {
    ...
    if (lookAhead(0) == TokenTypes.LET) {
      ...
      if (!inPredicateMode() && !defList.isEmpty()) {
        ...
        for (VertexPosition<Definition> def : defList) {
          ...
          definitionOf.set_sourcePositions(createSourcePositionList(def.length, def.offset));
        }
      }
      duringParsingvariableSymbolTable.blockEnd();
      return result;
  ...
  private final Definition parseDefinition() {
    ...
    if (!inPredicateMode()) {
      ...
      varOf.set_sourcePositions(createSourcePositionList(lengthVar, offsetVar));
      IsExprOf exprOf = graph.createIsExprOf(expr, definition);
      exprOf.set_sourcePositions(createSourcePositionList(lengthExpr, offsetExpr));
      return definition;
    }
    return null;
  }
  ...
  private final Expression parseConditionalExpression() {
    ...
    if (tryMatch(TokenTypes.QUESTION)) {
      ...
      if (!inPredicateMode()) {
        ...
        conditionOf.set_sourcePositions(createSourcePositionList(lengthExpr, offsetExpr));
        IsTrueExprOf trueExprOf = graph.createIsTrueExprOf(trueExpr, condExpr);
        trueExprOf.set_sourcePositions(createSourcePositionList(lengthTrueExpr, offsetTrueExpr));
        IsFalseExprOf falseExprOf = graph.createIsFalseExprOf(falseExpr, condExpr);
        falseExprOf.set_sourcePositions(createSourcePositionList(lengthFalseExpr, offsetFalseExpr));
        result = condExpr;
      }
    }
    return result;
  ...
  private final Expression parseEqualityExpression() {
    ...
    if (tryMatch(TokenTypes.EQUAL)) {
      ...
      if (tryMatch(TokenTypes.NOT_EQUAL)) {
  ...
  private final Expression parseRelationalExpression() {
    ...
    if (tryMatch(TokenTypes.L_T)) {
      ...
      if (tryMatch(TokenTypes.LE)) {
        name = "leEqual";
      } else {
        if (tryMatch(TokenTypes.GE)) {
          name = "grEqual";
        } else {
          if (tryMatch(TokenTypes.G_T)) {
            name = "grThan";
          } else {
            if (tryMatch(TokenTypes.MATCH)) {
              name = "reMatch";
            }
          }
        }
  ...
  private final Expression parseAdditiveExpression() {
    ...
    do {
      ...
      if (tryMatch(TokenTypes.PLUS)) {
        ...
        if (tryMatch(TokenTypes.MINUS)) {
          name = "sub";
        } else {
          if (tryMatch(TokenTypes.PLUSPLUS)) {
            name = "concat";
          }
        }
      }
  ...
  private final Expression parseMultiplicativeExpression() {
    ...
    do {
      ...
      if (tryMatch(TokenTypes.STAR)) {
        ...
        if (tryMatch(TokenTypes.MOD)) {
          name = "mod";
        } else {
          if (tryMatch(TokenTypes.DIV)) {
            name = "div";
          }
        }
      }
  ...
  private final Expression parseUnaryExpression() {
    FunctionConstruct construct = null;
    if ((lookAhead(0) == TokenTypes.NOT) || (lookAhead(0) == TokenTypes.MINUS)) {
      construct = new FunctionConstruct();
  ...
  private final Expression parsePrimaryExpression() {
    ...
    if (((lookAhead(0) == TokenTypes.IDENTIFIER) || (lookAhead(0) == TokenTypes.AND) || (lookAhead(0) == TokenTypes.NOT) || (lookAhead(0) == TokenTypes.XOR) || (lookAhead(0) == TokenTypes.OR)) && ((lookAhead(1) == TokenTypes.LCURLY) || (lookAhead(1) == TokenTypes.LPAREN))) {
      predicateStart();
      try {
  ...
  private final PathDescription parseAltPathDescription() {
    ...
    if (tryMatch(TokenTypes.BOR)) {
      ...
      if (!inPredicateMode()) {
        part1 = addPathElement(Greql2Schema.instance().vc_AlternativePathDescription, Greql2Schema.instance().ec_IsAlternativePathOf, null, part1, offsetPart1, lengthPart1, part2, offsetPart2, lengthPart2);
      }
    }
  ...
  private final PathDescription parseIntermediateVertexPathDescription() {
    ...
    if (predicateEnd()) {
      ...
      if (!inPredicateMode()) {
        result = (IntermediateVertexPathDescription) addPathElement(Greql2Schema.instance().vc_IntermediateVertexPathDescription, Greql2Schema.instance().ec_IsSubPathOf, null, part1, offsetPart1, lengthPart1, part2, offsetPart2, lengthPart2);
        IsIntermediateVertexOf intermediateVertexOf = graph.createIsIntermediateVertexOf(restrExpr, result);
        intermediateVertexOf.set_sourcePositions(createSourcePositionList(lengthExpr, offsetExpr));
      }
      return result;
    }
    return part1;
  ...
  private final PathDescription parseSequentialPathDescription() {
    ...
    if (predicateEnd()) {
      ...
      int lengthPart2 = getLength(offsetPart2);
      if (!inPredicateMode()) {
        return addPathElement(Greql2Schema.instance().vc_SequentialPathDescription, Greql2Schema.instance().ec_IsSequenceElementOf, null, part1, offsetPart1, lengthPart1, part2, offsetPart2, lengthPart2);
      } else {
        return null;
      }
    }
    ...
    if (!inPredicateMode()) {
      ...
      if (typeIds != null) {
        for (VertexPosition<? extends TypeOrRoleId> t : typeIds) {
          ...
          startRestrOf.set_sourcePositions(createSourcePositionList(t.length, t.offset));
        }
      }
    }
    return pathDescr;
  ...
  private final PathDescription parseIteratedOrTransposedPathDescription() {
    ...
    if ((lookAhead(0) == TokenTypes.STAR) || (lookAhead(0) == TokenTypes.PLUS) || (lookAhead(0) == TokenTypes.TRANSPOSED) || (lookAhead(0) == TokenTypes.CARET)) {
  ...
  private final PathDescription parseIteration(PathDescription iteratedPath, int offsetPath, int lengthPath) {
    ...
    if (iteration != null) {
      if (!inPredicateMode()) {
        ...
        iteratedPathOf.set_sourcePositions(createSourcePositionList(lengthPath, offsetPath));
        result = ipd;
      }
    } else {
      if (tryMatch(TokenTypes.TRANSPOSED)) {
        if (!inPredicateMode()) {
          ...
          transposedPathOf.set_sourcePositions(createSourcePositionList(lengthPath, offsetPath));
          result = tpd;
        }
      } else {
        if (tryMatch(TokenTypes.CARET)) {
          ...
          if (!inPredicateMode()) {
            ...
            exponentiatedPathOf.set_sourcePositions(createSourcePositionList(lengthPath, offsetPath));
            IsExponentOf exponentOf = graph.createIsExponentOf((IntLiteral) ie, epd);
            exponentOf.set_sourcePositions(createSourcePositionList(lengthExpr, offsetExpr));
            result = epd;
          }
        } else {
    ...
    if ((lookAhead(0) == TokenTypes.STAR) || (lookAhead(0) == TokenTypes.PLUS) || (lookAhead(0) == TokenTypes.CARET)) {
  ...
  private final PathDescription parsePrimaryPathDescription() {
    ...
    if ((lookAhead(0) == TokenTypes.OUTAGGREGATION) || (lookAhead(0) == TokenTypes.INAGGREGATION)) {
      return parseAggregationPathDescription();
    }
    if ((lookAhead(0) == TokenTypes.RARROW) || (lookAhead(0) == TokenTypes.LARROW) || (lookAhead(0) == TokenTypes.ARROW)) {
    ...
    if (tryMatch(TokenTypes.LBRACK)) {
      ...
      if (!inPredicateMode()) {
        ...
        optionalPathOf.set_sourcePositions(createSourcePositionList(length, offset));
        return optPathDescr;
      }
      return null;
    }
  ...
  private final PrimaryPathDescription parseAggregationPathDescription() {
    ...
    if (!inPredicateMode()) {
      AggregationPathDescription result = graph.createAggregationPathDescription();
  ...
  private final FunctionApplication parseFunctionApplication() {
    ...
    if (((lookAhead(0) == TokenTypes.IDENTIFIER) || (lookAhead(0) == TokenTypes.AND) || (lookAhead(0) == TokenTypes.NOT) || (lookAhead(0) == TokenTypes.XOR) || (lookAhead(0) == TokenTypes.OR)) && isFunctionName(lookAhead.getValue()) && ((lookAhead(1) == TokenTypes.LCURLY) || (lookAhead(1) == TokenTypes.LPAREN))) {
      int offset = getCurrentOffset();
      String name = lookAhead.getValue();
      match();
      int length = getLength(offset);
      ...
      if (!inPredicateMode()) {
        ...
        IsFunctionIdOf functionIdOf = graph.createIsFunctionIdOf(functionId, funApp);
        functionIdOf.set_sourcePositions(createSourcePositionList(length, offset));
        if (typeIds != null) {
          for (VertexPosition<TypeId> t : typeIds) {
            IsTypeExprOf typeOf = graph.createIsTypeExprOfFunction(t.node, funApp);
            typeOf.set_sourcePositions(createSourcePositionList(t.length, t.offset));
          }
        }
        if (expressions != null) {
          for (VertexPosition<Expression> ex : expressions) {
            IsArgumentOf argOf = graph.createIsArgumentOf(ex.node, funApp);
            argOf.set_sourcePositions(createSourcePositionList(ex.length, ex.offset));
          }
        }
        return funApp;
      }
  ...
  private final Expression parseValueConstruction() {
    if (lookAhead(0) != null) {
      switch (lookAhead(0)) {
        ...
        match(TokenTypes.RPAREN);
        if (!inPredicateMode()) {
          return createPartsOfValueConstruction(expressions, graph.createSetConstruction());
        } else {
          return null;
        }
        case TUP:
        ...
        match(TokenTypes.RPAREN);
        if (!inPredicateMode()) {
          return createPartsOfValueConstruction(expressions, graph.createTupleConstruction());
        } else {
          return null;
        }
      }
  ...
  private final MapConstruction parseMapConstruction() {
    ...
    if (!inPredicateMode()) {
      IsKeyExprOfConstruction keyEdge = graph.createIsKeyExprOfConstruction(keyExpr, mapConstr);
      keyEdge.set_sourcePositions(createSourcePositionList(lengthKey, offsetKey));
      IsValueExprOfConstruction valueEdge = graph.createIsValueExprOfConstruction(valueExpr, mapConstr);
      valueEdge.set_sourcePositions(createSourcePositionList(lengthValue, offsetValue));
    }
    while (tryMatch(TokenTypes.COMMA)) {
  ...
  private final ValueConstruction parseListConstruction() {
    ...
    if (tryMatch(TokenTypes.DOTDOT)) {
      ...
      if (!inPredicateMode()) {
        ...
        IsFirstValueOf firstValueOf = graph.createIsFirstValueOf(startExpr, (ListRangeConstruction) result);
        firstValueOf.set_sourcePositions(createSourcePositionList(lengthStart, offsetStart));
        IsLastValueOf lastValueOf = graph.createIsLastValueOf(endExpr, (ListRangeConstruction) result);
        lastValueOf.set_sourcePositions(createSourcePositionList(lengthEnd, offsetEnd));
      }
    } else {
      ...
      if (!inPredicateMode()) {
        ...
        result = createPartsOfValueConstruction(allExpressions, graph.createListConstruction());
      }
    }
    match(TokenTypes.RPAREN);
    return result;
  ...
  private final ValueConstruction parseRecordConstruction() {
    ...
    do {
      ...
      int length = getLength(offset);
      elements.add(new VertexPosition<RecordElement>(recElem, length, offset));
    } while(tryMatch(TokenTypes.COMMA));
    match(TokenTypes.RPAREN);
    if (!inPredicateMode()) {
      ...
      if (elements != null) {
        for (VertexPosition<RecordElement> expr : elements) {
          IsRecordElementOf exprOf = graph.createIsRecordElementOf(expr.node, valueConstr);
          exprOf.set_sourcePositions(createSourcePositionList(expr.length, expr.offset));
        }
      }
      return valueConstr;
    }
  ...
  private final RecordElement parseRecordElement() {
    ...
    if (!inPredicateMode()) {
      ...
      recIdOf.set_sourcePositions(createSourcePositionList(lengthRecId, offsetRecId));
  ...
  private final Declaration parseQuantifiedDeclaration() {
    ...
    if (!inPredicateMode()) {
      declaration = (Declaration) createMultipleEdgesToParent(declarations, graph.createDeclaration(), Greql2Schema.instance().ec_IsSimpleDeclOf, false);
    }
    while (tryMatch(TokenTypes.COMMA)) {
      ...
      if (!inPredicateMode()) {
        ...
        constraintOf.set_sourcePositions(createSourcePositionList(lengthConstraint, offsetConstraint));
      }
      predicateStart();
      try {
      ...
      if (predicateEnd()) {
        ...
        if (!inPredicateMode()) {
          createMultipleEdgesToParent(declarations, declaration, Greql2Schema.instance().ec_IsSimpleDeclOf, false);
        }
      }
    }
    return declaration;
  ...
  private final SimpleDeclaration parseSimpleDeclaration() {
    ...
    int length = getLength(offset);
    if (!inPredicateMode()) {
      SimpleDeclaration simpleDecl = (SimpleDeclaration) createMultipleEdgesToParent(variables, graph.createSimpleDeclaration(), Greql2Schema.instance().ec_IsDeclaredVarOf, "");
      IsTypeExprOf typeExprOf = graph.createIsTypeExprOfDeclaration(expr, simpleDecl);
      typeExprOf.set_sourcePositions(createSourcePositionList(length, offset));
      return simpleDecl;
    }
    return null;
  }
  ...
  private final Expression parseRangeExpression() {
    ...
    if (tryMatch(TokenTypes.LCURLY)) {
      if (!tryMatch(TokenTypes.RCURLY)) {
        ...
        match(TokenTypes.RCURLY);
        if (!inPredicateMode()) {
          createMultipleEdgesToParent(typeIds, expr, Greql2Schema.instance().ec_IsTypeRestrOfExpression, 0);
        }
      }
    }
    return expr;
  ...
  @SuppressWarnings(value = { "unchecked" }) private final EdgeRestriction parseEdgeRestriction() {
    List<VertexPosition<TypeId>> typeIds = null;
    ...
    if (!inPredicateMode()) {
      ...
      if (typeIds != null) {
        for (VertexPosition<TypeId> type : typeIds) {
          ...
          typeIdOf.set_sourcePositions(createSourcePositionList(type.length, type.offset));
        }
      }
      if (roleIds != null) {
        for (VertexPosition<RoleId> role : roleIds) {
          IsRoleIdOf roleIdOf = graph.createIsRoleIdOf(role.node, er);
          roleIdOf.set_sourcePositions(createSourcePositionList(role.length, role.offset));
        }
      }
      if (predicate != null) {
    ...
    do {
      ...
      if (!inPredicateMode()) {
        if (listCompr == null) {
          ...
          IsCompResultDefOf e = graph.createIsCompResultDefOf(tupConstr, listCompr);
          e.set_sourcePositions(createSourcePositionList(getLength(offset), offset));
        }
        IsPartOf partOf = graph.createIsPartOf(expr, tupConstr);
        partOf.set_sourcePositions(createSourcePositionList(lengthExpr, offsetExpr));
        if (hasLabel) {
          IsTableHeaderOf tableHeaderOf = graph.createIsTableHeaderOf(asExpr, listCompr);
          tableHeaderOf.set_sourcePositions(createSourcePositionList(lengthAsExpr, offsetAsExpr));
        }
      }
    } while(tryMatch(TokenTypes.COMMA));
  ...
  private final Comprehension parseReportClause() {
    ...
    if (map) {
      if (!inPredicateMode()) {
        ...
        IsKeyExprOfComprehension keyEdge = graph.createIsKeyExprOfComprehension(reportList.get(0).node, (MapComprehension) comprehension);
        IsValueExprOfComprehension valueEdge = graph.createIsValueExprOfComprehension(reportList.get(1).node, (MapComprehension) comprehension);
        keyEdge.set_sourcePositions(createSourcePositionList(reportList.get(0).length, reportList.get(0).offset));
        valueEdge.set_sourcePositions(createSourcePositionList(reportList.get(1).length, reportList.get(1).offset));
      }
    } else {
      if (vartable) {
        if (!inPredicateMode()) {
          ...
          IsColumnHeaderExprOf cHeaderE = graph.createIsColumnHeaderExprOf(reportList.get(0).node, (TableComprehension) comprehension);
          cHeaderE.set_sourcePositions(createSourcePositionList(reportList.get(0).length, reportList.get(0).offset));
          IsRowHeaderExprOf rHeaderE = graph.createIsRowHeaderExprOf(reportList.get(1).node, (TableComprehension) comprehension);
          rHeaderE.set_sourcePositions(createSourcePositionList(reportList.get(1).length, reportList.get(1).offset));
          e = graph.createIsCompResultDefOf(reportList.get(2).node, comprehension);
          e.set_sourcePositions(createSourcePositionList(reportList.get(2).length, reportList.get(2).offset));
          if (reportList.size() == 4) {
            IsTableHeaderOf tHeaderE = graph.createIsTableHeaderOf(reportList.get(3).node, (ComprehensionWithTableHeader) comprehension);
            tHeaderE.set_sourcePositions(createSourcePositionList(reportList.get(3).length, reportList.get(3).offset));
          }
        }
      } else {
        if (!inPredicateMode()) {
          if (reportList.size() > 1) {
            TupleConstruction tupConstr = (TupleConstruction) createMultipleEdgesToParent(reportList, graph.createTupleConstruction(), Greql2Schema.instance().ec_IsPartOf);
            e = graph.createIsCompResultDefOf(tupConstr, comprehension);
          } else {
            e = graph.createIsCompResultDefOf(reportList.get(0).node, comprehension);
          }
  ...
  private final Comprehension parseFWRExpression() {
    ...
    if (!inPredicateMode()) {
      ...
      comprDeclOf.set_sourcePositions(createSourcePositionList(lengthDecl, offsetDecl));
    }
  ...
  private final Expression parsePathExpression() {
    ...
    if (predicateEnd()) {
      ...
      if (predicateEnd()) {
        ...
        expr = parseValueAccess();
        int lengthArg1 = getLength(offsetArg1);
        if (lookAhead(0) == TokenTypes.SMILEY) {
          expr = parseRegPathOrPathSystem(expr, offsetArg1, lengthArg1);
        } else {
          predicateStart();
          try {
            parseAltPathDescription();
          ...
          if (predicateEnd()) {
            expr = parseRegPathExistenceOrForwardVertexSet(expr, offsetArg1, lengthArg1);
          }
        }
      } else {
  ...
  private final Expression parseRegPathExistenceOrForwardVertexSet(Expression expr, int offsetArg1, int lengthArg1) {
    ...
    if (predicateEnd()) {
      restrExpr = parseValueAccess();
      if (!inPredicateMode()) {
        int lengthExpr = getLength(offsetExpr);
        PathExistence pe = graph.createPathExistence();
        IsStartExprOf startVertexOf = graph.createIsStartExprOf(expr, pe);
        startVertexOf.set_sourcePositions(createSourcePositionList(lengthArg1, offsetArg1));
        IsTargetExprOf targetVertexOf = graph.createIsTargetExprOf(restrExpr, pe);
        targetVertexOf.set_sourcePositions(createSourcePositionList(lengthExpr, offsetExpr));
        IsPathOf pathOf = graph.createIsPathOf(pathDescr, pe);
        pathOf.set_sourcePositions(createSourcePositionList(lengthPathDescr, offsetPathDescr));
        return pe;
      }
      return null;
    } else {
  ...
  private final Expression parseRegPathOrPathSystem(Expression arg1, int offsetArg1, int lengthArg1) {
    ...
    if (!inPredicateMode()) {
      ...
      Expression result = createFunctionIdAndArgumentOf(funId, offsetOperator1, 3, arg1, offsetArg1, lengthArg1, pathDescr, offsetPathDescr, lengthPathDescr, true);
      if (isPath) {
        result = createFunctionIdAndArgumentOf(funId, offsetOperator1, 3, result, offsetArg1, -offsetArg1 + offsetOperator2 + 3, restrExpr, offsetExpr, lengthExpr, true);
      }
      return result;
    }
    return null;
  ...
  private final Expression parseRegBackwardVertexSetOrPathSystem() {
    ...
    if (!inPredicateMode()) {
      if (isPathSystem) {
        ...
        return createFunctionIdAndArgumentOf(f, offsetOperator, 3, pathDescr, offsetPathDescr, lengthPathDescr, restrExpr, offsetExpr, lengthExpr, true);
      } else {
  ...
  private final Expression parseNumericLiteral() {
    ...
    if ((lookAhead(0) == TokenTypes.HEXLITERAL) || (lookAhead(0) == TokenTypes.OCTLITERAL)) {
      if (((LongToken) lookAhead).getNumber().intValue() == ((LongToken) lookAhead).getNumber().longValue()) {
        IntLiteral literal = null;
        if (!inPredicateMode()) {
          literal = graph.createIntLiteral();
          literal.set_intValue(((LongToken) lookAhead).getNumber().intValue());
        }
        match();
        return literal;
      } else {
        ...
        if (!inPredicateMode()) {
          ...
          literal.set_longValue(((LongToken) lookAhead).getNumber());
        }
        match();
        return literal;
      }
    ...
    if ((lookAhead(0) == TokenTypes.INTLITERAL)) {
      ...
      if (lookAhead(0) == TokenTypes.DOT) {
        ...
        match();
        if ((lookAhead(0) == TokenTypes.INTLITERAL) || (lookAhead(0) == TokenTypes.OCTLITERAL)) {
