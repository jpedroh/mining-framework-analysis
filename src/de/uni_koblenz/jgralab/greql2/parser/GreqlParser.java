...
public class GreqlParser extends ParserHelper {
  ...
  private final Vertex createMultipleEdgesToParent(List<VertexPosition<Expression>> expressions, Vertex parent, Class<? extends Edge> edgeClass) {
    if (expressions != null) {
  ...
  private final Vertex createMultipleEdgesToParent(List<VertexPosition<TypeId>> expressions, Vertex parent, Class<? extends Edge> edgeClass, int i) {
    if (expressions != null) {
      for (VertexPosition<? extends Vertex> expr : expressions) {
        Greql2Aggregation edge = (Greql2Aggregation) graph.createEdge(edgeClass, expr.node, parent);
        edge.set_sourcePositions(createSourcePositionList(expr.length, expr.offset));
  ...
  private final Vertex createMultipleEdgesToParent(List<VertexPosition<SimpleDeclaration>> expressions, Vertex parent, Class<? extends Edge> edgeClass, boolean b) {
    if (expressions != null) {
      for (VertexPosition<? extends Vertex> expr : expressions) {
        Greql2Aggregation edge = (Greql2Aggregation) graph.createEdge(edgeClass, expr.node, parent);
        edge.set_sourcePositions(createSourcePositionList(expr.length, expr.offset));
  ...
  private final Vertex createMultipleEdgesToParent(List<VertexPosition<Variable>> expressions, Vertex parent, Class<? extends Edge> edgeClass, String s) {
    if (expressions != null) {
      for (VertexPosition<? extends Vertex> expr : expressions) {
        Greql2Aggregation edge = (Greql2Aggregation) graph.createEdge(edgeClass, expr.node, parent);
        edge.set_sourcePositions(createSourcePositionList(expr.length, expr.offset));
  ...
  private final PathDescription parseIntermediateVertexPathDescription() {
    ...
    if (predicateEnd()) {
      ...
      if (!inPredicateMode()) {
        result = (IntermediateVertexPathDescription) addPathElement(IntermediateVertexPathDescription.class, IsSubPathOf.class, null, part1, offsetPart1, lengthPart1, part2, offsetPart2, lengthPart2);
  ...
  private final PathDescription parseSequentialPathDescription() {
    ...
    if (predicateEnd()) {
      ...
      int lengthPart2 = getLength(offsetPart2);
      if (!inPredicateMode()) {
        return addPathElement(SequentialPathDescription.class, IsSequenceElementOf.class, null, part1, offsetPart1, lengthPart1, part2, offsetPart2, lengthPart2);
      } else {
        return null;
      }
    }
  ...
  private final Declaration parseQuantifiedDeclaration() {
    ...
    if (!inPredicateMode()) {
      declaration = (Declaration) createMultipleEdgesToParent(declarations, graph.createDeclaration(), IsSimpleDeclOf.class, false);
    }
    while (tryMatch(TokenTypes.COMMA)) {
      ...
      if (predicateEnd()) {
        ...
        if (!inPredicateMode()) {
          createMultipleEdgesToParent(declarations, declaration, IsSimpleDeclOf.class, false);
        }
      }
    }
    if (tryMatch(TokenTypes.IN)) {
      ...
      if (!inPredicateMode()) {
        ...
        subgraphOf.set_sourcePositions(createSourcePositionList(lengthSubgraph, offsetSubgraph));
      }
    }
    return declaration;
  }
  ...
  private final SimpleDeclaration parseSimpleDeclaration() {
    ...
    int length = getLength(offset);
    if (!inPredicateMode()) {
      SimpleDeclaration simpleDecl = (SimpleDeclaration) createMultipleEdgesToParent(variables, graph.createSimpleDeclaration(), IsDeclaredVarOf.class, "");
  ...
  private final Expression parseRangeExpression() {
    ...
    if (tryMatch(TokenTypes.LCURLY)) {
      if (!tryMatch(TokenTypes.RCURLY)) {
        ...
        match(TokenTypes.RCURLY);
        if (!inPredicateMode()) {
          createMultipleEdgesToParent(typeIds, expr, IsTypeRestrOf.class, 0);
        }
      }
    }
    return expr;
  ...
  private final Comprehension parseLabeledReportList() {
    ...
    if (!inPredicateMode() && (tupConstr.getDegree(EdgeDirection.IN) == 1)) {
      ...
      InternalEdge e2 = (InternalEdge) tupConstr.getFirstIncidence(EdgeDirection.OUT);
      e2.setAlpha(v);
      tupConstr.delete();
    }
    return listCompr;
