...
public abstract class ParserHelper {
  ...
  private void replaceDefinitionExpressions() throws DuplicateVariableException, UndefinedVariableException {
    ...
    for (DefinitionExpression exp : list) {
      ...
      for (Definition definition : defList) {
        ...
        isVarOf.delete();
        isExprOf.delete();
        InternalEdge e = (InternalEdge) variable.getFirstIncidence(EdgeDirection.OUT);
        while (e != null) {
          e.setAlpha(expr);
          e = (InternalEdge) variable.getFirstIncidence(EdgeDirection.OUT);
        }
        variable.delete();
      }
      Expression boundExpr = (Expression) exp.getFirstIsBoundExprOfIncidence(EdgeDirection.IN).getAlpha();
  ...
  private void mergeVariables(Vertex v, boolean separateScope) throws DuplicateVariableException, UndefinedVariableException {
    if (v instanceof DefinitionExpression) {
      ...
      if (v instanceof Comprehension) {
        mergeVariablesInComprehension((Comprehension) v, separateScope);
      } else {
        if (v instanceof QuantifiedExpression) {
          ...
          if (v instanceof Greql2Expression) {
            ...
            if (v instanceof ThisLiteral) {
              ...
              if (v instanceof Variable) {
                ...
                if (var != null) {
                  if (var != v) {
                    InternalEdge inc = (InternalEdge) v.getFirstIncidence(EdgeDirection.OUT);
                    inc.setAlpha(var);
                    if (v.getDegree() <= 0) {
                      v.delete();
                    }
                  ...
                  Greql2Aggregation e = (Greql2Aggregation) v.getFirstIncidence(EdgeDirection.OUT);
  ...
  /**
	 ...
	 */
  protected final void mergeVariablesInGreql2Expression(Greql2Expression root) throws DuplicateVariableException, UndefinedVariableException {
  ...
  private void mergeVariablesInDefinitionExpression(DefinitionExpression v, boolean separateScope) throws DuplicateVariableException, UndefinedVariableException {
    ...
    for (IsDefinitionOf currentEdge : v.getIsDefinitionOfIncidences(EdgeDirection.IN)) {
      ...
      Variable variable = (Variable) definition.getFirstIsVarOfIncidence(EdgeDirection.IN).getAlpha();
      afterParsingvariableSymbolTable.insert(variable.get_name(), variable);
    }
    Edge isBoundExprOf = v.getFirstIsBoundExprOfDefinitionIncidence(EdgeDirection.IN);
    mergeVariables(isBoundExprOf.getAlpha(), false);
    for (IsDefinitionOf currentEdge : v.getIsDefinitionOfIncidences(EdgeDirection.IN)) {
      Definition definition = (Definition) currentEdge.getAlpha();
      Expression expr = (Expression) definition.getFirstIsExprOfIncidence(EdgeDirection.IN).getAlpha();
  ...
  private void mergeVariablesInDeclaration(Declaration v) throws DuplicateVariableException, UndefinedVariableException {
    ...
    for (IsSimpleDeclOf currentEdge : v.getIsSimpleDeclOfIncidences(EdgeDirection.IN)) {
      SimpleDeclaration simpleDecl = (SimpleDeclaration) currentEdge.getAlpha();
      Expression expr = (Expression) simpleDecl.getFirstIsTypeExprOfIncidence(EdgeDirection.IN).getAlpha();
      mergeVariables(expr, true);
    }
  ...
  private void mergeVariablesInQuantifiedExpression(QuantifiedExpression v, boolean separateScope) throws DuplicateVariableException, UndefinedVariableException {
    ...
    IsBoundExprOfQuantifier isBoundExprOfQuantifier = v.getFirstIsBoundExprOfQuantifierIncidence(EdgeDirection.IN);
  ...
  private void mergeVariablesInComprehension(Comprehension v, boolean separateScope) throws DuplicateVariableException, UndefinedVariableException {
    ...
    if (isCompResultDefOf != null) {
      ...
      if (v instanceof ListComprehension) {
        IsTableHeaderOf isTableHeaderOf = v.getFirstIsTableHeaderOfIncidence(EdgeDirection.IN);
      ...
      if (v instanceof TableComprehension) {
        ...
        IsColumnHeaderExprOf ch = tc.getFirstIsColumnHeaderExprOfIncidence(EdgeDirection.IN);
        mergeVariables(ch.getAlpha(), true);
        IsRowHeaderExprOf rh = tc.getFirstIsRowHeaderExprOfIncidence(EdgeDirection.IN);
        mergeVariables(rh.getAlpha(), true);
        IsTableHeaderOf th = tc.getFirstIsTableHeaderOfIncidence(EdgeDirection.IN);
        if (th != null) {
