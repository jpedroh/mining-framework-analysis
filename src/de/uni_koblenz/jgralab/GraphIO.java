...
public class GraphIO {
  ...
  /**
	 ...
	 *             not be loaded
	 */
  public static Graph loadGraphFromFile(String filename, Schema schema, ProgressFunction pf, ImplementationType implementationType) throws GraphIOException {
  ...
  private static void close(Closeable stream) throws GraphIOException {
    try {
      ...
      throw new GraphIOException("Exception while closing the stream.", ex);
    }
  }
  ...
  public static Graph loadGraphFromStream(InputStream in, Schema schema, ProgressFunction pf, ImplementationType implementationType) throws GraphIOException {
    try {
      ...
      Class<?> schemaClass = Class.forName(schemaQName, true, M1ClassManager.instance(schemaQName));
    ...
    for (Entry<String, List<GraphElementClassData>> gcElements : edgeClassBuffer.entrySet()) {
      for (GraphElementClassData eData : gcElements.getValue()) {
        ...
        if (!(aec instanceof EdgeClass)) {
          throw new GraphIOException("Expected EdgeClass \'" + eData.getQualifiedName() + "\', but it\'s a " + aec.getM1Class().getSimpleName());
        }
