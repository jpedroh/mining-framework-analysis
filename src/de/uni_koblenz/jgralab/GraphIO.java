...
public class GraphIO {
  ...
  public static void saveGraphToStream(Graph graph, DataOutputStream out, ProgressFunction pf) throws GraphIOException {
    try {
      if (hasTemporaryElements(graph)) {
        throw new GraphIOException("Savin graph " + graph + " is not possible. " + "It contains temporary graph elements.");
      }
      GraphIO io = new GraphIO();
      io.TGOut = out;
  ...
  public static void saveGraphToStream(BooleanGraphMarker subGraph, DataOutputStream out, ProgressFunction pf) throws GraphIOException {
    try {
      if (hasTemporaryElements(subGraph, subGraph.getGraph())) {
        throw new GraphIOException("Savin sub graph " + subGraph + " of " + subGraph.getGraph() + " is not possible. " + "It contains temporary graph elements.");
      }
      GraphIO io = new GraphIO();
      io.TGOut = out;
  ...
  private void addAttributes(List<AttributeData> attributesData, AttributedElementClass<?, ?> aec) throws GraphIOException {
    for (AttributeData ad : attributesData) {
      aec.createAttribute(ad.name, attrDomain(ad.domainDescription), ad.defaultValue);
    }
  }
      ...
      for (GraphElementClassData vData : gcElements.getValue()) {
        ...
        if (aec instanceof VertexClass) {
          for (String superClassName : vData.directSuperClasses) {
            superClass = GECsearch.get(aec).getVertexClass(superClassName);
            if (superClass == null) {
    ...
    for (Entry<String, List<GraphElementClassData>> gcElements : edgeClassBuffer.entrySet()) {
      for (GraphElementClassData eData : gcElements.getValue()) {
        ...
        for (String superClassName : eData.directSuperClasses) {
          superClass = GECsearch.get(aec).getEdgeClass(superClassName);
          if (superClass == null) {
