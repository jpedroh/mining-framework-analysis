...
public class GraphIO {
  ...
  private void enumDomains() {
    ...
    for (EnumDomainData enumDomainData : this.enumDomainBuffer) {
      ...
      domain = this.schema.createEnumDomain(qName, enumDomainData.enumConstants);
  ...
  private void recordDomains() throws GraphIOException, SchemaException {
    ...
    for (RecordDomainData recordDomainData : this.recordDomainBuffer) {
      ...
      domain = this.schema.createRecordDomain(qName, this.getComponents(recordDomainData.components));
      this.domains.put(qName, domain);
    }
  }
  ...
  private List<RecordComponent> getComponents(List<ComponentData> componentsData) throws GraphIOException {
    ...
    for (ComponentData ad : componentsData) {
      RecordComponent c = new RecordComponent(ad.name, this.attrDomain(ad.domainDescription));
      result.add(c);
    }
    return result;
  }
  ...
  protected void addAttributes(List<AttributeData> attributesData, AttributedElementClass aec) throws GraphIOException {
  ...
  private void parseGraphElementClass(String gcName) throws GraphIOException, SchemaException {
    ...
    if (this.lookAhead.equals("VertexClass")) {
      ...
      if (this.lookAhead.equals("EdgeClass")) {
        this.match();
        String[] qn = this.matchQualifiedName(true);
        graphElementClassData.packageName = qn[0];
        graphElementClassData.simpleName = qn[1];
  ...
  private void buildVertexClassHierarchy() throws GraphIOException, SchemaException {
    ...
    for (Entry<String, List<GraphElementClassData>> gcElements : this.vertexClassBuffer.entrySet()) {
      for (GraphElementClassData vData : gcElements.getValue()) {
        aec = this.schema.getAttributedElementClass(vData.getQualifiedName());
        if (aec == null) {
        ...
        if (aec instanceof VertexClass) {
          for (String superClassName : vData.directSuperClasses) {
            superClass = (VertexClass) this.GECsearch.get(aec).getGraphElementClass(superClassName);
            if (superClass == null) {
  ...
  private void buildEdgeClassHierarchy() throws GraphIOException, SchemaException {
    ...
    for (Entry<String, List<GraphElementClassData>> gcElements : this.edgeClassBuffer.entrySet()) {
      for (GraphElementClassData eData : gcElements.getValue()) {
        aec = this.schema.getAttributedElementClass(eData.getQualifiedName());
        if (aec == null) {
        ...
        for (String superClassName : eData.directSuperClasses) {
          superClass = (EdgeClass) this.GECsearch.get(aec).getGraphElementClass(superClassName);
          if (superClass == null) {
  ...
  public final String[] matchQualifiedName(boolean isUpperCase) throws GraphIOException {
    String c = this.lookAhead.indexOf('.') >= 0 ? this.lookAhead : this.toQNameString(this.currentPackageName, this.lookAhead);
  ...
  private GraphBaseImpl graph(ProgressFunction pf, ImplementationType implementationType) throws GraphIOException {
    ...
    assert !this.gcName.contains(".") && isValidIdentifier(this.gcName) : "illegal characters in graph class \'" + this.gcName + "\'";
    ...
    try {
      ...
      throw new GraphIOException("can\'t create graph for class \'" + this.gcName + "\'", e);
    }
  ...
  private void vertexDesc(Graph graph, ImplementationType implementationType) throws GraphIOException {
    ...
    try {
      if (createMethod == null) {
        createMethod = this.schema.getVertexCreateMethod(vcName, implementationType);
  ...
  private void edgeDesc(Graph graph, ImplementationType implementationType) throws GraphIOException {
    ...
    try {
      if (createMethod == null) {
        createMethod = this.schema.getEdgeCreateMethod(ecName, implementationType);
      ...
      throw new GraphIOException("can\'t create edge " + eId + " from " + this.edgeOut[eId] + " to " + this.edgeIn[eId], e);
    }
        ...
        for (ComponentData comp : rd.components) {
          for (String componentDomain : comp.domainDescription) {
            ...
            for (RecordDomainData orderedRd : orderedRdList) {
              String qName = this.toQNameString(orderedRd.packageName, orderedRd.simpleName);
            ...
            for (EnumDomainData ed : this.enumDomainBuffer) {
              String qName = this.toQNameString(ed.packageName, ed.simpleName);
              if (componentDomain.equals(qName)) {
                componentDomsInOrderedList = true;
                break;
              }
            ...
            if (!componentDomsInOrderedList) {
              ...
              for (RecordDomainData rd2 : this.recordDomainBuffer) {
                String qName = this.toQNameString(rd2.packageName, rd2.simpleName);
