...
public class MatchReplace extends InPlaceTransformation {
  ...
  public static MatchReplace parseAndCreate(ExecuteTransformation et) {
    TemplateGraph replaceGraph = TemplateGraphParser.parse(et.match(TokenTypes.DOMAIN_SPECIFIC).value);
  ...
  @Override protected Integer transform() {
  ...
  private void verifyNoIncidentalDeletion(Vertex v) {
    for (Edge e : v.incidences()) {
      if (preservables.contains(e.getNormalEdge()) || !matchedEdges.contains(e.getNormalEdge())) {
        throw new GReTLException(context, "Prevervable or non-matched edge " + e + " from " + e.getAlpha() + " to " + e.getOmega() + " would be deleted by deleting " + v + "\ndeleted vertex = " + v + ",\nmatchedVertices = " + matchedVertices + ",\nmatchedEdges = " + matchedEdges + ",\npreservables = " + preservables + ",\nquery = " + semanticExpression);
      }
    }
  }
  ...
  private void createAndUpdateEdges() {
    for (CreateEdge ce : replaceGraph.getCreateEdgeEdges()) {
      ...
      if ((ce.get_typeName() == null) && (arch != null) && (arch instanceof Edge)) {
        InternalEdge e = 
<<<<<<< /usr/src/app/output/jgralab/jgralab/ba28dc3943bf31e37f3beb96450dac330ebebd47/src/de/uni_koblenz/jgralab/gretl/MatchReplace.java/left.java
        (InternalEdge) arch.toEdge().getNormalEdge()
=======
        ((Edge) arch).getNormalEdge()
>>>>>>> /usr/src/app/output/jgralab/jgralab/ba28dc3943bf31e37f3beb96450dac330ebebd47/src/de/uni_koblenz/jgralab/gretl/MatchReplace.java/right.java
        ;
        preservables.add(e);
        allModifiedElements.add(e);
        e.setAlpha(startVertex);
        e.setOmega(endVertex);
        setAttributeValues(e, arch, ce.get_attributes(), ce.is_copyAttributeValues());
      } else {
        ...
        setAttributeValues(newEdge, arch, ce.get_attributes(), ce.is_copyAttributeValues());
      }
    }
  }
  ...
  private void createAndUpdateVertices() {
    for (CreateVertex cv : replaceGraph.getCreateVertexVertices()) {
      ...
      if ((cv.get_typeName() == null) && (arch != null) && (arch instanceof Vertex)) {
        ...
        setAttributeValues(v, arch, cv.get_attributes(), cv.is_copyAttributeValues());
      } else {
        ...
        setAttributeValues(newVertex, arch, cv.get_attributes(), cv.is_copyAttributeValues());
      }
    }
  }
  ...
  private Edge createEdge(CreateEdge e, Vertex startVertex, Vertex endVertex) {
    ...
    if (e.get_typeName() != null) {
      ...
      ec = getSingleEdgeClassBetween((VertexClass) startVertex.getAttributedElementClass(), (VertexClass) endVertex.getAttributedElementClass());
    }
    return context.getTargetGraph().createEdge(ec.getM1Class(), startVertex, endVertex);
  }
  ...
  private EdgeClass getSingleEdgeClassBetween(VertexClass from, VertexClass to) {
    ...
    if (possibles.size() != 1) {
      throw new GReTLException(context, "Cannot find exactly one EdgeClass between " + from.getQualifiedName() + " and " + to.getQualifiedName() + ", but there were these possibilities: " + possibles + ".");
    }
  ...
  private void setAttributeValues(GraphElement ge, Object arch, Map<String, String> attrMap, boolean copy) {
    ...
    if (copy && ((arch == null) || !(arch instanceof AttributedElement))) {
      throw new GReTLException(context, "Should copy attribute values, but the archetype \'" + arch + "\' is no AttributedElement.");
    }
    if (copy) {
      ...
      for (Attribute attr : geClass.getAttributeList()) {
        ...
        if (aeClass.containsAttribute(attrName) && (aeClass.getAttribute(attrName).getDomain() == attr.getDomain())) {
          ge.setAttribute(attrName, ae.getAttribute(attrName));
        }
      }
    }
    for (Entry<String, String> e : attrMap.entrySet()) {
      ge.setAttribute(e.getKey(), context.evaluateGReQLQuery(e.getValue()));
    }
  }
