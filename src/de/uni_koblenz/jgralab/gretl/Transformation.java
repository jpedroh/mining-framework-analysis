 ...
 *
 */
public abstract class Transformation<T extends java.lang.Object> {
  @Retention(value = RetentionPolicy.RUNTIME) @Target(value = { ElementType.METHOD }) protected @interface After {
  }

  @Retention(value = RetentionPolicy.RUNTIME) @Target(value = { ElementType.METHOD }) protected @interface Before {
  }
  ...
  private final void invokeHooks(Class<? extends Annotation> annotationClass) {
    for (Class<?> cls = getClass(); Transformation.class.isAssignableFrom(cls); cls = cls.getSuperclass()) {
      for (Method method : cls.getDeclaredMethods()) {
        if (method.isAnnotationPresent(annotationClass)) {
          try {
            ...
            log.severe("Couldn\'t run @" + annotationClass.getSimpleName() + " method " + method.getName() + " of " + cls.getSimpleName() + ".");
            e.printStackTrace();
          }
        }
      }
  ...
  protected static Logger log = JGraLab.getLogger(Transformation.class.getPackage().getName());
  ...
  public final T execute() {
    ...
    if (context.outermost) {
      ...
      if (context.getTargetSchema() == null) {
        ...
        log.info("Target Schema exists. " + "Skipping schema creation phase...");
      }
      ...
      if (context.targetGraph == null) {
        ...
        if (context.targetGraph.getSchema().getQualifiedName().equals(context.targetSchema.getQualifiedName())) {
          log.info("Using a preset target graph...");
        } else {
          throw new GReTLException(context, "Preset target graph has wrong schema \'" + context.targetGraph.getSchema().getQualifiedName() + "\'. Expected was \'" + context.targetSchema.getQualifiedName() + "\'.");
        }
      }
      log.info("SCHEMA Phase took " + ((System.currentTimeMillis() - startTime) + "ms."));
      ...
      log.info("GRAPH Phase took " + ((System.currentTimeMillis() - startTime) + "ms."));
    } else {
      result = transform();
      ...
      if (DEBUG_EXECUTION && ((tg.getVCount() + tg.getECount()) < GReTLRunner.MAX_VISUALIZATION_SIZE)) {
        try {
          ...
          Tg2Dot.convertGraph(context.getTargetGraph(), "__debug_" + (EXECUTION_STEP++) + "_" + name + ".pdf", DEBUG_REVERSE_EDGES, GraphVizOutputFormat.PDF, (EdgeClass[]) null);
        } catch (IOException e) {
          e.printStackTrace();
        }
      }
  ...
  protected final VertexClass vc(String qualifiedName) {
    VertexClass vc = context.targetSchema.getGraphClass().getVertexClass(qualifiedName);
    if (vc == null) {
  ...
  protected final EdgeClass ec(String qualifiedName) {
    EdgeClass ec = context.targetSchema.getGraphClass().getEdgeClass(qualifiedName);
    if (ec == null) {
      throw new GReTLException(context, "There\'s no target EdgeClass \'" + qualifiedName + "\'.");
    }
    return ec;
  }
  ...
  protected final AttributedElementClass<?, ?> aec(String qualifiedName) {
    AttributedElementClass<?, ?> aec = context.targetSchema.getAttributedElementClass(qualifiedName);
    if (aec == null) {
      throw new GReTLException(context, "There\'s no target AttributedElementClass \'" + qualifiedName + "\'.");
    }
    return aec;
  }
  ...
  protected final GraphElementClass<?, ?> gec(String qualifiedName) {
    GraphElementClass<?, ?> gec = context.targetSchema.getGraphClass().getGraphElementClass(qualifiedName);
    if (gec == null) {
      throw new GReTLException(context, "There\'s no target GraphElementClass \'" + qualifiedName + "\'.");
    }
    return gec;
  }
  ...
  protected final Attribute attr(String qualifiedName) {
    ...
    AttributedElementClass<?, ?> aec = context.getTargetSchema().getAttributedElementClass(className);
    if (aec == null) {
      throw new GReTLException(context, "There\'s no target AttributedElementClass \'" + className + "\'.");
    }
    String attrName = qualifiedName.substring(lastDot + 1);
    Attribute attr = aec.getAttribute(attrName);
    if (attr == null) {
      throw new GReTLException(context, "There\'s no target Attribute \'" + attrName + "\' in AttributedElementClass \'" + className + "\'.");
    }
    return attr;
  }
  ...
  protected Domain domain(String domain) {
    ...
    if (d == null) {
      throw new GReTLException(context, "There\'s no target Domain \'" + domain + "\'.");
    }
    return d;
  }
}