...
public class GReTLRunner {
  ...
  static {
    ...
    new QueryImpl("from x: list(1..10) with x = 5 reportSet x end").evaluate();
  ...
  public void exec(String[] args) throws GraphIOException, IOException {
    ...
    if (cli.getArgs().length == 0) {
      ...
      for (String in : cli.getArgs()) {
        Graph inGraph = GraphIO.loadGraphFromFile(in, ImplementationType.GENERIC, new ConsoleProgressFunction("Loading"));
        if (cli.hasOption('u')) {
  ...
  private void saveTargetGraph(Graph outGraph, String outFileName, CommandLine cli) throws GraphIOException, IOException {
    GraphIO.saveGraphToFile(outGraph, outFileName, new ConsoleProgressFunction("Saving"));
    if (cli.hasOption('z')) {
      if ((outGraph.getVCount() + outGraph.getECount()) > MAX_VISUALIZATION_SIZE) {
        ...
        Tg2Dot.convertGraph(outGraph, pdf, cli.hasOption('r'), GraphVizOutputFormat.PDF, (EdgeClass[]) null);
      }
    }
  }
