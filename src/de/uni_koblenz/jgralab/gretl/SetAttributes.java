...
public class SetAttributes extends Transformation<PMap<AttributedElement<?, ?>, Object>> {
  ...
  public SetAttributes(final Context c, final Attribute attr, final PMap<Object, Object> archetypeValueMap) {
    super(c);
    attribute = attr;
    archetype2valueMap = archetypeValueMap;
  }

  public SetAttributes(final Context c, final Attribute attr, final String semanticExpression) {
    super(c);
    attribute = attr;
  ...
  @Override protected PMap<AttributedElement<?, ?>, Object> transform() {
    ...
    for (Object archetype : archetype2valueMap.keySet()) {
      AttributedElement<?, ?> image = context.getImg(attribute.getAttributedElementClass()).get(archetype);
      if (image == null) {
        String qname = attribute.getAttributedElementClass().getQualifiedName();
        throw new GReTLException(context, "The source graph element \'" + archetype + "\' has no image in " + Context.toGReTLVarNotation(qname, GReTLVariableType.IMG) + " yet, so no attribute \'" + attribute.getName() + "\' can be created!");
      }
  ...
  private Object convertToAttributeValue(Object val) {
    ...
    if (dom instanceof RecordDomain) {
      return context.getTargetGraph().createRecord((RecordDomain) dom, ((Record) val).toPMap());
    }
    return result;
  }
}