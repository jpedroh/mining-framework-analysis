...
public class TgSchema2Java {
  ...
  @Deprecated public void setTransactionSupportOnly() {
    System.err.println("Warning: this call is deprecated, use option \"-i\" instead.");
    config.setStandardSupport(false);
    config.setTransactionSupport(true);
  }

  @Deprecated public void setStandardSupportOnly() {
    System.err.println("Warning: this call is deprecated, use option \"-i\" instead.");
  ...
  private boolean isExistingSchema(Schema schema) {
    ...
    requiredFilePaths.add(commitPath + File.separator + schema.getPathName() + File.separator + "impl" + File.separator + schema.getName() + "Factory.java");
    for (Domain d : schema.getDomains().values()) {
      ...
      if (d.toString().startsWith("Enum") || d.toString().startsWith("Record")) {
        requiredFilePaths.add(commitPath + File.separator + schemaPath + File.separator + pathName + d.getSimpleName() + ".java");
      }
    }
    ...
    if (!gc.isAbstract()) {
      ...
      requiredFilePaths.add(commitPath + File.separator + schemaPath + File.separator + "impl" + File.separator + pathName + gc.getSimpleName() + "Impl.java");
    }
    for (VertexClass vc : schema.getGraphClass().getVertexClasses()) {
      requiredFilePaths.add(commitPath + File.separator + schemaPath + File.separator + vc.getFileName() + ".java");
      if (!vc.isAbstract()) {
        pathName = vc.getPathName();
        if (pathName != "") {
          pathName = pathName.concat(File.separator);
        }
        requiredFilePaths.add(commitPath + File.separator + schemaPath + File.separator + "impl" + File.separator + pathName + vc.
<<<<<<< /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/utilities/tgschema2java/TgSchema2Java.java/left.java
        getFileName()
=======
        getSimpleName()
>>>>>>> /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/utilities/tgschema2java/TgSchema2Java.java/right.java
         + "Impl.java");
      }
    }
    for (EdgeClass ec : schema.getGraphClass().getEdgeClasses()) {
      requiredFilePaths.add(commitPath + File.separator + schemaPath + File.separator + ec.getFileName() + ".java");
      if (!ec.isAbstract()) {
        pathName = ec.getPathName();
        if (pathName != "") {
          pathName = pathName.concat(File.separator);
        }
        requiredFilePaths.add(commitPath + File.separator + schemaPath + File.separator + "impl" + File.separator + pathName + ec.getSimpleName() + "Impl.java");
        requiredFilePaths.add(commitPath + File.separator + schemaPath + File.separator + "impl" + File.separator + pathName + "Reversed" + ec.
<<<<<<< /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/utilities/tgschema2java/TgSchema2Java.java/left.java
        getFileName()
=======
        getSimpleName()
>>>>>>> /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/utilities/tgschema2java/TgSchema2Java.java/right.java
  ...
  private static CommandLine processCommandLineOptions(String[] args) {
    ...
    Option standard = new Option("so", "standard-support-only", false, "(optional): Create standard support code only");
    standard.setRequired(false);
    oh.addOption(standard);
    Option transactions = new Option("to", "transaction-support-only", false, "(optional): Create transaction support code only");
    ...
    oh.addOption(path);
    Option implementation = new Option("i", "implementationMode", true, "(optional): explicitly specify the implementation modes that should be generated. By default only the standard support will be activated.");
  ...
  public void setImplementationMode(String value) {
    ...
    if (values.length > 0) {
      ...
      throw new IllegalArgumentException("No implementation mode specified.");
    }
    for (String v : values) {
      v = v.trim();
      if (v.equals("transaction")) {
        ...
        if (v.equals("standard")) {
          ...
          if (v.equals("database")) {
            setDatabaseSupport(true);
          } else {
            throw new IllegalArgumentException("Illegal value for implementation mode: " + v + "\nOnly \"transaction\",\"standard\" and \"database\" are supported.");
          }
        }
      }
    }
