...
public class Tg2SiDiff extends Tg2Whatever {
  ...
  @Override protected void graphStart(PrintStream out) {
    ...
    out.println("<Graph name=\"" + graph.getId() + "\" root=\"" + getRootVertexId() + "\">");
  }
  ...
  @Override public void graphEnd(PrintStream out) {
    out.println("</Graph>");
    out.print("</Document>");
  }
  ...
  @Override protected void printEdge(PrintStream out, Edge e) {
    ...
    if (PRINT_EDGES_AS_NODES) {
      out.print("<Node type=\"" + cls.getQualifiedName() + "\" id=\"edge" + e.getId() + "\">\n");
      if (cls.getAttributeCount() > 0) {
        printAttributes(out, e);
      }
      out.print("</Node>\n");
      out.print("<Edge type=\"" + cls.getQualifiedName() + "From\" src=\"vertex" + alpha.getId() + "\" tar=\"edge" + e.getId() + "\" nesting=\"" + aggregateFrom + "\"/>\n");
      out.print("<Edge type=\"" + cls.getQualifiedName() + "To\" src=\"edge" + e.getId() + "\" tar=\"vertex" + omega.getId() + "\" nesting=\"" + aggregateTo + "\"/>\n");
    } else {
      out.print("<Edge type=\"" + cls.getQualifiedName() + "\" src=\"vertex" + alpha.getId() + "\" tar=\"vertex" + omega.getId() + "\" nesting=\"" + aggregateFrom + "\"/>\n");
    }
  }
  ...
  private void printAttributes(PrintStream out, AttributedElement<?, ?> elem) {
    ...
    for (Attribute attr : cls.getAttributeList()) {
      ...
      out.print("    <Attribute name=\"" + attr.getName() + "\" value=\"" + attributeValue + "\"/>\n");
    }
  }
