 ...
 * 
 */
@WorkInProgress(description = "untested, suspected problems with record/enum domains and schema extraction") public class Tg2GXL extends Tg2Whatever {
  ...
  @Override protected void graphStart(PrintStream out) {
    ...
    if (printSchema) {
      out.println("<graph id=\"" + uniqueGraphClassName + "Graph\" edgeids=\"true\" edgemode=\"directed\" hypergraph=\"false\">");
      out.println("<type xlink:href=\"" + gxlMetaSchema + "#gxl-1.0\" xlink:type=\"simple\"/>");
    } else {
      out.println("<graph id=\"" + graph.getId() + "\" edgeids=\"true\" edgemode=\"directed\" hypergraph=\"false\">");
      out.println("<type xlink:href=\"" + schemaGraphOutputName + "#" + graph.getGraphClass().getQualifiedName() + "\" xlink:type=\"simple\"/>");
    }
  }
  ...
  /**
	 ...
	 */
  @Override protected void printVertex(PrintStream out, Vertex v) {
    ...
    if (printSchema && !(v instanceof Schema)) {
      ...
      out.println("<type xlink:href=\"" + gxlMetaSchema + "#" + grUML2GXL.get(elemClass.getQualifiedName()) + "\" xlink:type=\"simple\"/>");
      printAttributes(out, v);
      out.println("</node>");
    } else {
      if (!printSchema) {
        out.println("<node id=\"v:" + v.getId() + "\">");
        out.println("<type xlink:href=\"" + schemaGraphOutputName + "#" + elemClass.getQualifiedName() + "\" xlink:type=\"simple\"/>");
        printAttributes(out, v);
        out.println("</node>");
      }
    }
  ...
  /**
	 ...
	 *            the processed edge
	 */
  @Override protected void printEdge(PrintStream out, Edge e) {
    EdgeClass elemClass = e.getAttributedElementClass();
    if (printSchema && !(e instanceof de.uni_koblenz.jgralab.grumlschema.structure.DefinesGraphClass) && !(e instanceof HasRecordDomainComponent)) {
      ...
      if (e.getThis() instanceof de.uni_koblenz.jgralab.grumlschema.structure.AttributedElementClass) {
        thisVertex = ((de.uni_koblenz.jgralab.grumlschema.structure.AttributedElementClass) e.getThis()).get_qualifiedName();
      }
      if (e.getThat() instanceof de.uni_koblenz.jgralab.grumlschema.structure.AttributedElementClass) {
        thatVertex = ((de.uni_koblenz.jgralab.grumlschema.structure.AttributedElementClass) e.getThat()).get_qualifiedName();
      }
      out.println("<edge id=\"e:" + e.getId() + "\" to=\"" + thatVertex + "\" from=\"" + thisVertex + "\">");
      out.println("<type xlink:href=\"" + gxlMetaSchema + "#" + grUML2GXL.get(elemClass.getQualifiedName()) + "\" xlink:type=\"simple\"/>");
      ...
      if (!printSchema) {
        ...
        out.println("<edge id=\"e:" + e.getId() + "\" to=\"v:" + e.getThat().getId() + "\" from=\"v:" + e.getThis().getId() + "\" toorder=\" " + toOrder + "\" fromorder=\" " + fromOrder + "\">");
        out.println("<type xlink:href=\"" + schemaGraphOutputName + "#" + elemClass.getQualifiedName() + "\" xlink:type=\"simple\"/>");
        printAttributes(out, e);
        out.println("</edge>");
      }
  ...
  private void printAttributes(PrintStream out, AttributedElement<?, ?> elem) {
    for (Attribute attr : elem.getAttributedElementClass().getAttributeList()) {
  ...
  private void printComposite(PrintStream out, Domain dom, Object val) {
    if (!dom.isComposite()) {
      ...
      if (dom instanceof RecordDomain) {
        ...
        out.println("</string>");
        Collection<RecordComponent> components = ((RecordDomain) dom).getComponents();
        for (RecordComponent component : components) {
          ...
          out.println("</string>");
          try {
            printComposite(out, component.getDomain(), val.getClass().getField(component.getName()).get(val));
          } catch (Exception e) {
          }
          out.println("</tup>");
        }
  ...
  @Override public void convert() throws IOException {
    ...
    uniqueGraphClassName = graph.getSchema().getGraphClass().getQualifiedName();
    super.convert();
    setOutputFile(schemaGraphOutputName);
    setGraph(new Schema2SchemaGraph().convert2SchemaGraph(graph.getSchema()));
    printSchema = true;
    super.convert();
  }
  ...
  /**
	 ...
	 */
  @Override protected void getOptions(String[] args) {
    ...
    if (comLine.hasOption("g")) {
      try {
        ...
        System.err.println("Couldn\'t load graph in file \'" + graphName + "\': " + e.getMessage());
        if (e.getCause() != null) {
    ...
    if (comLine.hasOption("o")) {
      ...
      schemaGraphOutputName = graphOutputName.substring(0, graphOutputName.length() - 4) + "Schema.gxl";
    }
  }
