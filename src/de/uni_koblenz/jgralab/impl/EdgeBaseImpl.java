 ...
 */
public abstract class EdgeBaseImpl extends IncidenceImpl implements Edge, InternalEdge {
  ...
  @Override public int compareTo(AttributedElement<EdgeClass, Edge> a) {
    assert a != null;
  ...
  @Override public void setAlpha(Vertex alpha) {
    ...
    if (!graph.isLoading() && (graph.getECARuleManagerIfThere() != null)) {
      graph.getECARuleManagerIfThere().fireBeforeChangeAlphaOfEdgeEvents(this, oldAlpha, alphaBase);
    }
    if (alphaBase == oldAlpha) {
      return;
    }
    if (!((VertexClass) alphaBase.getAttributedElementClass()).isValidFromFor((EdgeClass) getAttributedElementClass())) {
  ...
  @Override public void setOmega(Vertex omega) {
    ...
    if (!graph.isLoading() && (graph.getECARuleManagerIfThere() != null)) {
      graph.getECARuleManagerIfThere().fireBeforeChangeOmegaOfEdgeEvents(this, oldOmgea, omegaBase);
    }
    if (omegaBase == oldOmgea) {
      return;
    }
    if (!((VertexClass) omegaBase.getAttributedElementClass()).isValidToFor((EdgeClass) getAttributedElementClass())) {
