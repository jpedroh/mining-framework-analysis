...
public class TransactionImpl implements Transaction {
  ...
  protected void removeNonReferencedPersistentValues() {
    ...
    TransactionImpl oldestTransaction = (TransactionImpl) transactionManager.getOldestTransaction();
    if (oldestTransaction == this && transactionManager.getTransactions().size() > 1) {
      oldestTransaction = (TransactionImpl) transactionManager.getTransactions().get(1);
    }
    ...
    if (changedDuringCommit != null) {
      for (VersionedDataObjectImpl<?> versionedDataObject : changedDuringCommit) {
        if (oldestTransaction == this && transactionManager.getTransactions().size() == 1) {
          maxVersionNumber = versionedDataObject.getLatestPersistentVersion();
        }
        versionedDataObject.removePersistentValues(maxVersionNumber);
        versionedDataObject.removeAllTemporaryValues(this);
        long minRange = 0;
        TransactionImpl realOldestTransaction = (TransactionImpl) transactionManager.getOldestTransaction();
        ...
        synchronized (transactionManager.transactionList) {
          if (transactionManager.transactionList.size() > 1) {
            for (int i = 0; i < transactionManager.transactionList.size() - 1; i++) {
              TransactionImpl t1 = (TransactionImpl) transactionManager.getTransactions().get(i);
              TransactionImpl t2 = (TransactionImpl) transactionManager.getTransactions().get(i + 1);
              minRange = t1.persistentVersionAtBot;
              maxRange = t2.persistentVersionAtBot;
              if (minRange > maxRange) {
                throw new GraphException("This should not happen. The transactions in transaction list should be sorted asc by persistentVersionAtBot.");
              }
              versionedDataObject.removePersistentValues(minRange, maxRange);
            }
          }
  ...
  @Override public List<Savepoint> getSavepoints() {
    if (thread != Thread.currentThread()) {
      throw new GraphException("Transaction is not active in current thread.");
    }
    if (savepointList == null) {
    ...
    synchronized (savepointList) {
      List<Savepoint> savepointList = new ArrayList<Savepoint>(this.savepointList);
      return savepointList;
    }
  }

  @Override public TransactionState getState() {
    return state;
  }
  ...
  @Override public void restoreSavepoint(Savepoint savepoint) throws InvalidSavepointException {
    ...
    if (savepointList == null || !savepointList.contains(savepoint) || savepoint.getTransaction() != this) {
  ...
  protected void removeInvalidSavepoints() {
    ...
    for (int i = savepointList.indexOf(latestRestoredSavepoint) + 1; i < savepointList.size(); i++) {
      if (logger != null) {
        logger.finest("--> remove sp id=" + savepointList.get(i).getID());
      }
      savepointList.remove(i);
    }
    if (temporaryVersionMap != null) {
      Set<VersionedDataObject<?>> versionedDataObjects = new HashSet<VersionedDataObject<?>>(temporaryVersionMap.keySet());
      for (VersionedDataObject<?> versionedDataObject : versionedDataObjects) {
        versionedDataObject.removeTemporaryValues(latestRestoredSavepoint.versionAtSavepoint, this);
  ...
  @Override public void removeSavepoint(Savepoint savepoint) {
    if (logger != null) {
      logger.fine("tx id=" + id + ", sp id=" + savepoint.getID());
    }
    if (thread != Thread.currentThread()) {
    ...
    if (savepoint.getTransaction() != this) {
      throw new GraphException("This savepoint doesn\'t belong to this transaction.");
    }
    if (savepointList != null && !savepointList.isEmpty()) {
      synchronized (savepointList) {
        int position = savepointList.indexOf(savepoint);
        savepointList.remove(savepoint);
        if (savepointList.isEmpty() && latestRestoredSavepoint != savepoint) {
          for (Entry<VersionedDataObject<?>, SortedMap<Long, Object>> entries : temporaryVersionMap.entrySet()) {
            VersionedDataObject<?> versionedDataObject = entries.getKey();
            SortedMap<Long, Object> versionsMap = entries.getValue();
            temporaryValueMap.put(versionedDataObject, versionsMap.get(versionsMap.lastKey()));
            temporaryVersionMap.remove(versionedDataObject);
          }
          temporaryVersionMap = null;
        } else {
  ...
  @Override public boolean isValid() {
    return getState() != TransactionState.ABORTED && getState() != TransactionState.COMMITTED;
  }
  ...
  public Set<VersionedDataObject<?>> getRemainingVersionedDataObjects() {
    ...
    if (temporaryValueMap != null && temporaryValueMap.size() > 0) {
      versionedDataObjects = new HashSet<VersionedDataObject<?>>(temporaryValueMap.keySet());
    }
    if (temporaryVersionMap != null && temporaryVersionMap.size() > 0) {
      versionedDataObjects = new HashSet<VersionedDataObject<?>>(temporaryVersionMap.keySet());
    }
    if (versionedDataObjects.size() > 0) {
      ...
      if (changedIncidences != null) {
        for (Map<IncidenceImpl, Map<ListPosition, Boolean>> map : changedIncidences.values()) {
      ...
      if (changedAttributes != null) {
        for (Entry<AttributedElement, Set<VersionedDataObject<?>>> entry : changedAttributes.entrySet()) {
