...
public abstract class ReversedEdgeImpl extends de.uni_koblenz.jgralab.impl.ReversedEdgeBaseImpl implements VersionedIncidence {
  ...
  @Override public void setIncidentVertex(Vertex v) {
    if (graph.isLoading()) {
      incidentVertex = new VersionedReferenceImpl<VertexBaseImpl>(normalEdge, (VertexBaseImpl) v);
    } else {
      if (incidentVertex == null) {
  ...
  @Override public void setNextIncidenceInternal(InternalEdge nextIncidence) {
    if (graph.isLoading()) {
      this.nextIncidence = new VersionedReferenceImpl<IncidenceImpl>(normalEdge, (IncidenceImpl) nextIncidence);
    } else {
      ...
      if (this.nextIncidence == null) {
        this.nextIncidence = new VersionedReferenceImpl<IncidenceImpl>(normalEdge);
      }
  ...
  @Override public void setPrevIncidenceInternal(InternalEdge prevIncidence) {
    if (graph.isLoading()) {
      this.prevIncidence = new VersionedReferenceImpl<IncidenceImpl>(normalEdge, (IncidenceImpl) prevIncidence);
    } else {
      TransactionImpl transaction = (TransactionImpl) graph.getCurrentTransaction();
      if (transaction == null) {
        throw new GraphException("Current transaction is null.");
      ...
      if (this.prevIncidence == null) {
        this.prevIncidence = new VersionedReferenceImpl<IncidenceImpl>(normalEdge);
      }
