...
public abstract class VertexBaseImpl extends GraphElementImpl<VertexClass, Vertex> implements Vertex, InternalVertex {
  ...
  @Override public Edge getFirstIncidence(EdgeClass anEdgeClass) {
    ...
    assert isValid();
    return getFirstIncidence(anEdgeClass.getSchemaClass(), EdgeDirection.INOUT);
  }
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public <T extends Vertex> POrderedSet<T> reachableVertices(Class<T> returnType, PathElement... pathElements) {
    ...
    for (int i = 0; i < pathElements.length; i++) {
      ...
      while (vx != null) {
        for (Edge e : vx.incidences(t.edgeClass, t.edgeDirection)) {
          if (!t.strictType || (t.strictType && (t.edgeClass == e.getSchemaClass()))) {
