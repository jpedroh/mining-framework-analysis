 ...
 */
public abstract class VertexBaseImpl extends GraphElementImpl implements Vertex, InternalVertex {
  /**
	 * @param id
  ...
  @Override public int getDegree(Class<? extends Edge> ec, EdgeDirection orientation) {
    assert ec != null;
    assert isValid();
    int degree = 0;
  ...
  @Override public String toString() {
    assert isValid();
    return "v" + getId() + ": " + getAttributedElementClass().getQualifiedName();
  }
  ...
  @Override public <T extends Vertex> POrderedSet<T> reachableVertices(String pathDescription, Class<T> vertexType) {
    return graph.reachableVertices(this, pathDescription, vertexType);
  }

  @SuppressWarnings(value = { "unchecked" }) public <T extends Vertex> POrderedSet<T> reachableVertices(Class<T> returnType, PathElement... pathElements) {
    ...
    for (int i = 0; i < pathElements.length; i++) {
      ...
      while (vx != null) {
        for (Edge e : vx.incidences(t.edgeClass, t.edgeDirection)) {
          if (!t.strictType || (t.strictType && (t.edgeClass == e.getM1Class()))) {
