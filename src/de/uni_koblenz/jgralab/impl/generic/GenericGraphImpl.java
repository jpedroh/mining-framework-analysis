...
public class GenericGraphImpl extends GraphImpl {
  ...
  /**
	 ...
	 */
  public static Graph createGraph(GraphClass type, String id, int vmax, int emax) {
  ...
  @Override public void readAttributeValueFromString(String attributeName, String value) throws GraphIOException, NoSuchAttributeException {
    ...
    attributes[i] = type.getAttribute(attributeName).getDomain().parseGenericAttribute(GraphIO.createStringReader(value, getSchema()));
  }

  @Override public void readAttributeValues(GraphIO io) throws GraphIOException {
    for (Attribute a : type.getAttributeList()) {
      attributes[type.getAttributeIndex(a.getName())] = a.getDomain().parseGenericAttribute(io);
    }
  }
  ...
  @Override public void writeAttributeValues(GraphIO io) throws IOException, GraphIOException {
    for (Attribute a : type.getAttributeList()) {
      a.getDomain().serializeGenericAttribute(io, getAttribute(a.getName()));
    }
  }

  @SuppressWarnings(value = { "unchecked" }) @Override public <T extends java.lang.Object> T getAttribute(String name) {
  ...
  @Override public <T extends java.lang.Object> void setAttribute(String name, T data) {
    ...
    if (type.getAttribute(name).getDomain().isConformGenericValue(data)) {
      ...
      throw new ClassCastException("Expected " + ((d instanceof RecordDomain) ? RecordImpl.class.getName() : d.getJavaAttributeImplementationTypeName(d.getPackageName())) + " object, but received " + data.getClass().getName() + " object instead");
    }
  }
  ...
  @Override public Vertex getFirstVertex(VertexClass vertexClass) {
    ...
    if (v.getAttributedElementClass().equals(vertexClass) || v.getAttributedElementClass().getAllSuperClasses().contains(vertexClass)) {
      return v;
    }
  ...
  @Override public Edge getFirstEdge(EdgeClass edgeClass) {
    ...
    if (e.getAttributedElementClass().equals(edgeClass) || e.getAttributedElementClass().getAllSuperClasses().contains(edgeClass)) {
      return e;
    }
    return e.getNextEdge(edgeClass);
  }

  @Override public Iterable<Vertex> vertices(VertexClass vc) {
    return new VertexIterable<Vertex>(this, vc);
  }

  @Override public Iterable<Edge> edges(EdgeClass ec) {
  ...
  static void initializeGenericAttributeValues(AttributedElement<?, ?> ae) {
    for (Attribute attr : ae.getAttributedElementClass().getAttributeList()) {
      if ((attr.getDefaultValueAsString() != null) && !attr.getDefaultValueAsString().isEmpty()) {
        ...
        ae.setAttribute(attr.getName(), genericAttributeDefaultValue(attr.getDomain()));
      }
    }
  }
