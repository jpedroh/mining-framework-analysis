...
<<<<<<< /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/codegenerator/VertexCodeGenerator.java/left.java
  ...
  private CodeBlock createValidEdgeSets(VertexClass vc) {
    ...
    CodeList code = new CodeList();
    code.setVariable("vcQualifiedName", schemaRootPackageName + ".impl." + vc.getQualifiedName());
    code.setVariable("vcCamelName", camelCase(vc.getUniqueName()));
    CodeSnippet s = new CodeSnippet(true);
    ...
    for (EdgeClass ec : vc.getValidFromEdgeClasses()) {
      ...
      line.setVariable("edgeClassQualifiedName", schemaRootPackageName + "." + ec.getQualifiedName());
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  @Override protected CodeBlock createAttributedElementClassConstant() {
    return new CodeSnippet(true, "public static final #jgSchemaPackage#.#schemaElementClass# VC" + " = #schemaPackageName#.#schemaName#.instance().#schemaVariableName#;");
  }
  ...
  private CodeBlock createGetEdgeForRolenameMethod() {
    ...
    for (EdgeClass ec : vc.getValidFromEdgeClasses()) {
      if (!ec.getTo().getRolename().isEmpty()) {
        ...
        code.setVariable("edgeclass", schemaRootPackageName + "." + ec.getQualifiedName());
