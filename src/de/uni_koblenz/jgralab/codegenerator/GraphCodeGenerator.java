...
public class GraphCodeGenerator extends AttributedElementCodeGenerator<GraphClass, Graph> {
  ...
  @Override protected CodeBlock createBody() {
    ...
    if (currentCycle.isStdOrDbImplOrTransImpl()) {
      ...
      code.add(new CodeSnippet("\n\tprotected GreqlEvaluator greqlEvaluator;\n", "@Override", "public synchronized <T extends Vertex> POrderedSet<T> reachableVertices(Vertex startVertex, String pathDescription, Class<T> vertexType) {", "\tif (greqlEvaluator == null) {", "\t\tgreqlEvaluator = new GreqlEvaluator((String) null, this, null);", "\t}", "\tgreqlEvaluator.setVariable(\"v\", startVertex);", "\tgreqlEvaluator.setQuery(\"using v: v \" + pathDescription);", "\tgreqlEvaluator.startEvaluation();", "\treturn greqlEvaluator.getResultSet();", "}"));
    }
...
<<<<<<< /usr/src/app/output/jgralab/jgralab/518fc01e6bec91cd0a597ff873ef2e91169448b9/src/de/uni_koblenz/jgralab/codegenerator/GraphCodeGenerator.java/left.java
      if (vertex.isDefaultGraphElementClass()) {
        ...
        if (vertex instanceof TemporaryVertexClassImpl) {
          continue;
        }
      }
=======
