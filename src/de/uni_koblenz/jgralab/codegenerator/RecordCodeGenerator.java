...
public class RecordCodeGenerator extends CodeGenerator {
  ...
  private CodeBlock createGraphIOConstructor() {
    ...
    for (RecordComponent rc : recordDomain.getComponents()) {
      if (currentCycle.isTransImpl()) {
        code.add(rc.getDomain().getTransactionReadMethod(schemaRootPackageName, "tmp_" + rc.getName(), "io"));
        CodeSnippet cs = new CodeSnippet("set_#key#((#typeName#) tmp_#key#);");
        cs.setVariable("key", rc.getName());
        cs.setVariable("typeName", rc.getDomain().getTransactionJavaAttributeImplementationTypeName(schemaRootPackageName));
        code.add(cs);
      } else {
        code.add(rc.getDomain().getReadMethod(schemaRootPackageName, "_" + rc.getName(), "io"));
      }
    }
    code.add(new CodeSnippet("io.match(\")\");"));
  ...
  private CodeBlock createFieldConstructor() {
    ...
    for (RecordComponent rc : recordDomain.getComponents()) {
      sb.append(delim);
      delim = ", ";
      sb.append(rc.getDomain().getJavaAttributeImplementationTypeName(schemaRootPackageName));
      sb.append(" _");
  ...
  private CodeBlock createMapConstructor() {
    ...
    code.add(new CodeSnippet("assert componentValues.size() == " + recordDomain.getComponents().size() + ";"));
    for (RecordComponent rc : recordDomain.getComponents()) {
      if (rc.getDomain().isComposite() && (suppressUnchecked.size() <= 1)) {
        suppressUnchecked.add("@SuppressWarnings(\"unchecked\")");
      }
      CodeBlock assign = new CodeSnippet("assert componentValues.containsKey(\"#name#\");", "_#name# = (#cls#)componentValues.get(\"#name#\");");
      assign.setVariable("name", rc.getName());
      assign.setVariable("cls", rc.getDomain().getJavaClassName(schemaRootPackageName));
      code.add(assign);
    }
    code.addNoIndent(new CodeSnippet("}"));
    return code;
  ...
  private CodeBlock createToPMapMethod() {
    ...
    for (RecordComponent rc : recordDomain.getComponents()) {
      CodeBlock assign = new CodeSnippet("m = m.plus(\"#name#\", _#name#);");
      assign.setVariable("name", rc.getName());
      code.add(assign);
    }
  ...
  private CodeBlock createEqualsMethod() {
    ...
    code.add(new CodeSnippet("if (o instanceof #jgPackage#.Record) {", "\t#jgPackage#.Record rec = (#jgPackage#.Record) o;", "\tif (rec.size() != " + recordDomain.getComponents().size() + ") {", "\t\treturn false;", "\t}", "\ttry {"));
    for (RecordComponent rc : recordDomain.getComponents()) {
      CodeSnippet codeSnippet = new CodeSnippet("\t\tif (!rec.getComponent(\"#name#\").equals(_#name#)) {", "\t\t\treturn false;", "\t\t}");
      codeSnippet.setVariable("name", rc.getName());
      code.add(codeSnippet);
    }
  ...
  @Override protected CodeBlock createHeader() {
    ...
    addImports("java.util.Collections", "java.util.List", "java.util.ArrayList");
    code.add("public class #simpleClassName# implements de.uni_koblenz.jgralab.Record {");
    code.add("\tprivate static List<String> componentNames = new ArrayList<String>(" + recordDomain.getComponents().size() + ");", "", "\tstatic {");
    for (RecordComponent rc : recordDomain.getComponents()) {
    ...
    code.add("", "\t@Override", "public int size() {", "\treturn " + recordDomain.getComponents().size() + ";", "}");
    return code;
  }
  ...
  protected CodeBlock createGetterMethods() {
    ...
    for (RecordComponent rc : recordDomain.getComponents()) {
      ...
      getterCode.setVariable("type", rc.getDomain().getJavaAttributeImplementationTypeName(schemaRootPackageName));
      getterCode.setVariable("ctype", rc.getDomain().getJavaAttributeImplementationTypeName(schemaRootPackageName));
  ...
  private CodeBlock createGenericGetter() {
    ...
    for (RecordComponent rc : recordDomain.getComponents()) {
      CodeBlock assign = null;
      if (currentCycle.isTransImpl()) {
        assign = new CodeSnippet("if (name.equals(\"#name#\")) {", "\treturn #isOrGet#_#name#();", "}");
      } else {
        assign = new CodeSnippet("if (name.equals(\"#name#\")) {", "\treturn _#name#;", "}");
      }
      assign.setVariable("name", rc.getName());
      assign.setVariable("cname", rc.getDomain().getJavaClassName(schemaRootPackageName));
  ...
  private CodeBlock createWriteComponentsMethod() {
    ...
    for (RecordComponent rc : recordDomain.getComponents()) {
      code.add(rc.getDomain().getWriteMethod(schemaRootPackageName, "_" + rc.getName(), "io"));
    }
