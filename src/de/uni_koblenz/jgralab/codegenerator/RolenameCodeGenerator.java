...
public class RolenameCodeGenerator {
  ...
  private CodeBlock createRemoveAdjacenceSnippet(String rolename, EdgeClass edgeClass, VertexClass definingVertexClass, EdgeDirection dir, boolean createClass) {
    ...
    code.setVariable("edgeClassName", schemaRootPackageName + edgeClass.getQualifiedName());
    code.setVariable("dir", "EdgeDirection." + dir.toString());
    code.setVariable("definingVertexClassName", schemaRootPackageName + definingVertexClass.getQualifiedName());
    if (!createClass) {
      ...
      code.add("@Override", "public boolean remove_#rolename#(#definingVertexClassName# vertex) {", "\tboolean elementRemoved = false;", "\t#edgeClassName# edge = (#edgeClassName#) getFirstIncidence(#edgeClassName#.class, #dir#);", "\twhile (edge != null) {", "\t\t#edgeClassName# next = (#edgeClassName#) edge.getNextIncidence(#edgeClassName#.class, #dir#);", "\t\tif (edge.getThat().equals(vertex)) {" + "\t\t\tedge.delete();", "\t\t\telementRemoved = true;", "\t\t}", "\t\tedge = next;", "\t}", "\treturn elementRemoved;", "}");
    }
    return code;
  }
  ...
  private CodeBlock createRemoveAllAdjacencesSnippet(String rolename, EdgeClass edgeClass, VertexClass otherVertexClass, EdgeDirection dir, boolean createClass) {
    ...
    code.setVariable("edgeClassName", schemaRootPackageName + edgeClass.getQualifiedName());
    code.setVariable("dir", "EdgeDirection." + dir.toString());
    code.setVariable("vertexClassName", schemaRootPackageName + otherVertexClass.getQualifiedName());
    if (!createClass) {
      ...
      code.add("@Override", "public java.util.List<? extends #vertexClassName#> remove_#rolename#() {", "\tjava.util.List<#vertexClassName#> adjacences = new java.util.ArrayList<#vertexClassName#>();", "\t#edgeClassName# edge = (#edgeClassName#) getFirstIncidence(#edgeClassName#.class, #dir#);", "\twhile (edge != null) {", "\t\t#edgeClassName# next = (#edgeClassName#) edge.getNextIncidence(#edgeClassName#.class, #dir#);", "\t\tadjacences.add((#vertexClassName#) edge.getThat());", "\t\tedge.delete();", "\t\tedge = next;", "\t}", "\treturn adjacences;", "}");
    }
    return code;
  }

  private CodeBlock createGetAdjacencesSnippet(IncidenceClass incClass, VertexClass allowedVertexClass, EdgeDirection dir, boolean createClass) {
    CodeSnippet code = new CodeSnippet();
    code.setVariable("rolename", incClass.getRolename());
    code.setVariable("edgeClassName", schemaRootPackageName + incClass.getEdgeClass().getQualifiedName());
    code.setVariable("dir", "EdgeDirection." + dir.toString());
    code.setVariable("vertexClassName", schemaRootPackageName + allowedVertexClass.getQualifiedName());
    if (incClass.getMax() == 1) {
      if (!createClass) {
        ...
        code.add("@Override", "public #vertexClassName# get_#rolename#() {", "\t#edgeClassName# edge = (#edgeClassName#) getFirstIncidence(#edgeClassName#.class, #dir#);", "\tif (edge != null) {", "\t\treturn (#vertexClassName#) edge.getThat();", "\t}", "\treturn null;", "}");
      }
    } else {
      if (!createClass) {
        code.add("/**", " * @return an Iterable of all vertices adjacent to this one with the rolename \'#rolename#\'", " *         (connected with a <code>#edgeClassName#</code> edge).", " */", "public Iterable<? extends #vertexClassName#> get_#rolename#();");
      } else {
        code.add("@Override", "public Iterable<? extends #vertexClassName#> get_#rolename#() {", "\treturn new de.uni_koblenz.jgralab.impl.NeighbourIterable<#edgeClassName#, #vertexClassName#>(this, #edgeClassName#.class, #dir#);", "}");
      }
    }
    return code;
  }
  ...
  private CodeBlock createAddRolenameSnippet(String rolename, EdgeClass edgeClass, VertexClass definingVertexClass, VertexClass allowedVertexClass, EdgeDirection dir, boolean createClass) {
    ...
    code.setVariable("graphClassName", schemaRootPackageName + edgeClass.getGraphClass().getQualifiedName());
    code.setVariable("definingVertexClassName", schemaRootPackageName + definingVertexClass.getQualifiedName());
    code.setVariable("allowedVertexClassName", schemaRootPackageName + allowedVertexClass.getQualifiedName());
    code.setVariable("thisVertexClassName", schemaRootPackageName + vertexClass.getQualifiedName());
    if (dir == EdgeDirection.OUT) {
      code.setVariable("alpha", "this");
      code.setVariable("alphaVertexClassName", schemaRootPackageName + vertexClass.getQualifiedName());
      code.setVariable("omega", "vertex");
      code.setVariable("omegaVertexClassName", schemaRootPackageName + allowedVertexClass.getQualifiedName());
    } else {
      code.setVariable("alpha", "vertex");
      code.setVariable("alphaVertexClassName", schemaRootPackageName + allowedVertexClass.getQualifiedName());
      code.setVariable("omega", "this");
      code.setVariable("omegaVertexClassName", schemaRootPackageName + vertexClass.getQualifiedName());
    }
    if (!createClass) {
      ...
      if (definingVertexClass != allowedVertexClass) {
        code.add("\tif (!(vertex instanceof #allowedVertexClassName#)) {", "\t\tthrow new de.uni_koblenz.jgralab.GraphException(\"The rolename #rolename# was redefined at the vertex class #thisVertexClassName#. Only vertices of #allowedVertexClassName# are allowed.\"); ", "\t}");
      }
      code.add(
<<<<<<< /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/codegenerator/RolenameCodeGenerator.java/left.java
...
>>>>>>> /usr/src/app/output/jgralab/jgralab/1502a3525248376410ba747530bf6ef93a0656f2/src/de/uni_koblenz/jgralab/codegenerator/RolenameCodeGenerator.java/right.java
      , "}");
    }
    return code;
  }
  ...
  public CodeBlock createRolenameMethods(boolean createClass) {
    ...
    for (IncidenceClass ic : validFarICs) {
      ...
      for (IncidenceClass redefinedIC : ic.getRedefinedIncidenceClasses()) {
        list.add(createMethodsForOneIncidenceClass(ic, redefinedIC, createClass));
      }
    }
    return list;
  }
  ...
  private CodeList createMethodsForOneIncidenceClass(IncidenceClass allowedIncidenceClass, IncidenceClass definingIncidenceClass, boolean createClass) {
    ...
    if (!rolename.isEmpty()) {
      ...
      list.addNoIndent(createAddRolenameSnippet(rolename, ec, vc, allowedVC, dir, createClass));
      list.addNoIndent(createRemoveAllAdjacencesSnippet(rolename, ec, allowedVC, dir, createClass));
      list.addNoIndent(createRemoveAdjacenceSnippet(rolename, ec, vc, dir, createClass));
