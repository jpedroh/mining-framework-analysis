...
public class AttributedElementCodeGenerator extends CodeGenerator {
  ...
  protected AttributedElementCodeGenerator(AttributedElementClass<?, ?> attributedElementClass, String schemaRootPackageName, CodeGeneratorConfiguration config) {
    ...
    rootBlock.setVariable("javaClassName", schemaRootPackageName + "." + aec.getQualifiedName());
    rootBlock.setVariable("qualifiedImplClassName", schemaRootPackageName + ".impl." + (config.hasTransactionSupport() ? "trans" : "std") + aec.getQualifiedName() + "Impl");
    ...
    for (AttributedElementClass<?, ?> superClass : attributedElementClass.getDirectSuperClasses()) {
...
<<<<<<< /usr/src/app/output/jgralab/jgralab/4aec874ec88fb13e499510a38c517beb31eaec51/src/de/uni_koblenz/jgralab/codegenerator/AttributedElementCodeGenerator.java/left.java
...
>>>>>>> /usr/src/app/output/jgralab/jgralab/4aec874ec88fb13e499510a38c517beb31eaec51/src/de/uni_koblenz/jgralab/codegenerator/AttributedElementCodeGenerator.java/right.java
     + " = #schemaPackageName#.#schemaName#.instance().#schemaVariableName#;");
  }

  @Override protected CodeBlock createHeader() {
    CodeSnippet code = new CodeSnippet(true);
    code.setVariable("classOrInterface", currentCycle.isStdOrDbImplOrTransImpl() ? " class" : " interface");
    ...
    code.setVariable("extends", currentCycle.isStdOrDbImplOrTransImpl() ? " extends #baseClassName#" : "");
    StringBuffer buf = new StringBuffer();
    if (interfaces.size() > 0) {
      String delim = currentCycle.isStdOrDbImplOrTransImpl() ? " implements " : " extends ";
...
=======
    "@Override"
>>>>>>> /usr/src/app/output/jgralab/jgralab/4aec874ec88fb13e499510a38c517beb31eaec51/src/de/uni_koblenz/jgralab/codegenerator/AttributedElementCodeGenerator.java/right.java
    , "public final #jgSchemaPackage#.#schemaElementClass# getAttributedElementClass() {", "\treturn #javaClassName#.ATTRIBUTED_ELEMENT_CLASS;", "}");
  }
  ...
  protected CodeBlock createGenericGetter(Set<Attribute> attrSet) {
    ...
    for (Attribute attr : attrSet) {
      ...
      if (attr.getDomain().isPrimitive()) {
        s.setVariable("attributeClassName", " (" + attr.getDomain().getJavaClassName(schemaRootPackageName) + ")");
      } else {
  ...
  protected CodeBlock createGenericSetter(Set<Attribute> attrSet) {
    ...
    for (Attribute attr : attrSet) {
      if (attr.getDomain().isComposite() && !(attr.getDomain() instanceof RecordDomain)) {
    ...
    for (Attribute attr : attrSet) {
      ...
      if (attr.getDomain().isComposite()) {
        s.setVariable("attributeClassName", attr.getDomain().getJavaAttributeImplementationTypeName(schemaRootPackageName));
      } else {
        s.setVariable("attributeClassName", attr.getDomain().getJavaClassName(schemaRootPackageName));
      }
  ...
  protected CodeBlock createGetter(Attribute attr) {
    ...
    switch (currentCycle) {
      ...
      code.setVariable("ttype", attr.getDomain().getTransactionJavaAttributeImplementationTypeName(schemaRootPackageName));
      code.add("public #type# #isOrGet#_#name#() {");
      addCheckValidityCode(code);
      code.add("\tif (_#name# == null)", "\t\treturn #initValue#;", "\t#ttype# value = _#name#.getValidValue(#theGraph#.getCurrentTransaction());");
  ...
  protected void addCheckValidityCode(CodeSnippet code) {
    code.add("\tif (!isValid())", "\t\tthrow new #jgPackage#.GraphException(\"Cannot access attribute \'#name#\', because \" + this + \" isn\'t valid in current transaction.\");");
  }
  ...
  protected CodeBlock createSetter(Attribute attr) {
    ...
    switch (currentCycle) {
      ...
      case TRANSIMPL:
      code.setVariable("ttype", attr.getDomain().getTransactionJavaAttributeImplementationTypeName(schemaRootPackageName));
      code.setVariable("vclass", attr.getDomain().getVersionedClass(schemaRootPackageName));
      code.setVariable("initLoading", "new #vclass#(this, _#name#, \"#name#\");");
      code.setVariable("init", "new #vclass#(this, null, \"#name#\");");
      code.add("public void set_#name#(#type# _#name#) {");
      addCheckValidityCode(code);
      code.add("\tif (#theGraph#.isLoading()) {", "\t\tthis._#name# = #initLoading#", "\t}", "\tif (this._#name# == null) {", "\t\tthis._#name# = #init#", "\t}", "\tthis._#name#.setValidValue((#ttype#) _#name#, #theGraph#.getCurrentTransaction());", "\tattributeChanged(this._#name#);", "\tgraphModified();", "}");
      break;
    }
    return code;
  }
  ...
  protected CodeBlock createReadAttributesFromStringMethod(Set<Attribute> attrSet) {
    CodeList code = new CodeList();
    addImports("#jgPackage#.GraphIO", "#jgPackage#.GraphIOException", "#jgPackage#.NoSuchAttributeException");
    ...
    if (attrSet != null) {
      for (Attribute attribute : attrSet) {
        ...
        if (currentCycle.isTransImpl()) {
          ...
          a.add(readBlock);
          a.add(attribute.getDomain().getReadMethod(schemaRootPackageName, "tmpVar", "io"));
          a.addNoIndent(new CodeSnippet("\t#setterName#(tmpVar);", "\treturn;", "}"));
        }
        if (currentCycle.isStdImpl() || currentCycle.isDbImpl()) {
          a.add(attribute.getDomain().getReadMethod(schemaRootPackageName, "_" + attribute.getName(), "io"));
  ...
  /**
	 ...
	 * @return
	 */
  protected CodeBlock createWriteAttributeToStringMethod(Set<Attribute> attrSet) {
    CodeList code = new CodeList();
    addImports("#jgPackage#.GraphIO", "#jgPackage#.GraphIOException", "#jgPackage#.NoSuchAttributeException");
    ...
    if (attrSet != null) {
      for (Attribute attribute : attrSet) {
        ...
        if (currentCycle.isTransImpl()) {
          a.add(attribute.getDomain().getTransactionWriteMethod(schemaRootPackageName, "_" + attribute.getName(), "io"));
        }
        if (currentCycle.isStdImpl() || currentCycle.isDbImpl()) {
          a.add(attribute.getDomain().getWriteMethod(schemaRootPackageName, "_" + attribute.getName(), "io"));
        }
  ...
  protected CodeBlock createReadAttributesMethod(SortedSet<Attribute> attrSet) {
    ...
    if (attrSet != null) {
      for (Attribute attribute : attrSet) {
        ...
        if (currentCycle.isStdImpl() || currentCycle.isDbImpl()) {
          code.add(attribute.getDomain().getReadMethod(schemaRootPackageName, "_" + attribute.getName(), "io"));
        }
        if (currentCycle.isTransImpl()) {
          code.add(attribute.getDomain().getTransactionReadMethod(schemaRootPackageName, "_" + attribute.getName(), "io"));
        }
  ...
  protected CodeBlock createWriteAttributesMethod(Set<Attribute> attrSet) {
    CodeList code = new CodeList();
    addImports("#jgPackage#.GraphIO", "#jgPackage#.GraphIOException", "java.io.IOException");
    ...
    if ((attrSet != null) && !attrSet.isEmpty()) {
      ...
      for (Attribute attribute : attrSet) {
        if (currentCycle.isStdImpl() || currentCycle.isDbImpl()) {
          code.add(attribute.getDomain().getWriteMethod(schemaRootPackageName, "_" + attribute.getName(), "io"));
        }
        if (currentCycle.isTransImpl()) {
          code.add(attribute.getDomain().getTransactionWriteMethod(schemaRootPackageName, "_" + attribute.getName(), "io"));
        }
      }
    }
    code.addNoIndent(new CodeSnippet("}"));
  ...
  /**
	 ...
	 * @return
	 */
  protected CodeBlock createGetVersionedAttributesMethod(SortedSet<Attribute> attributeList) {
    CodeList code = new CodeList();
    if (currentCycle.isTransImpl()) {
      CodeSnippet codeSnippet = new CodeSnippet();
      codeSnippet.add("public java.util.Set<#jgTransPackage#.VersionedDataObject<?>> attributes() {");
      codeSnippet.add("\tjava.util.Set<#jgTransPackage#.VersionedDataObject<?>> attributes = " + "new java.util.HashSet<#jgTransPackage#.VersionedDataObject<?>>();");
