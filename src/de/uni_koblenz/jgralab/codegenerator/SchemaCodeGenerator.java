...
public class SchemaCodeGenerator extends CodeGenerator {
  ...
  private CodeBlock createGraphFactoryMethod() {
    addImports("#jgPackage#.Graph", "#jgPackage#.ProgressFunction", "#jgPackage#.GraphIO", "#jgImplDbPackage#.GraphDatabaseException", "#jgImplDbPackage#.GraphDatabase", "#jgPackage#.GraphIOException", "#jgPackage#.ImplementationType", "#jgPackage#.GraphFactory");
    if (config.hasDatabaseSupport()) {
      addImports("#jgPackage#.GraphException", "#jgImplPackage#.GraphFactoryImpl");
    }
    CodeSnippet code = new CodeSnippet(true, "/**", " * Creates a new #gcName# graph.", "*/", "public #gcName# create#gcCamelName#(ImplementationType implType) {", "\treturn create#gcCamelName#(implType,null,1000,1000);", "}", "", "/**", " * Creates a new #gcName# graph with initial vertex and edge counts <code>vMax</code>, <code>eMax</code>.", " *", " * @param vMax initial vertex count", " * @param eMax initial edge count", "*/", "public #gcName# create#gcCamelName#(ImplementationType implType, String id, int vMax, int eMax) {", "\tswitch(implType){", ((config.hasStandardSupport()) ? "\tcase STANDARD:\n" + "\t\t\tGraphFactory stdFactory = new #schemaImplStdPackage#.#gcCamelName#FactoryImpl(); \n" + "\t\t\t#gcCamelName# stdGraph = (#gcCamelName#) stdFactory.createGraph(id, vMax, eMax); \n" + "\t\t\treturn stdGraph;\n" : ""), ((config.hasTransactionSupport()) ? "\tcase TRANSACTION:\n" + "\t\t\tGraphFactory transFactory = new #schemaImplTransPackage#.#gcCamelName#FactoryImpl(); \n" + "\t\t\t#gcCamelName# transGraph = (#gcCamelName#) transFactory.createGraph(id, vMax, eMax); \n" + "\t\t\treturn transGraph;\n" : ""), "\t}", "\tthrow new UnsupportedOperationException(\"No \"+implType+\" support compiled.\");", "}", "", "/**", " * Creates a new #gcName# graph in a database with given <code>id</code>.", " *", " * @param id Identifier of new graph", " * @param graphDatabase Database which should contain graph", " */", "public #gcName# create#gcCamelName#(String id, GraphDatabase graphDatabase) throws GraphDatabaseException {", (config.hasDatabaseSupport() ? "\treturn create#gcCamelName#(id, 100, 100, graphDatabase);" : "\tthrow new UnsupportedOperationException(\"No database support compiled.\");"), "}", "", "/**", " * Creates a new #gcName# graph in a database with given <code>id</code>.", " *", " * @param id Identifier of new graph", " * @param vMax Maximum initial count of vertices that can be held in graph.", " * @param eMax Maximum initial count of edges that can be held in graph.", " * @param graphDatabase Database which should contain graph", " */", "public #gcName# create#gcCamelName#(String id, int vMax, int eMax, GraphDatabase graphDatabase) throws GraphDatabaseException{", (config.hasDatabaseSupport() ? "\tGraphFactoryImpl graphFactory = new #schemaImplDbPackage#.#gcCamelName#FactoryImpl();\n" + "\t\tgraphFactory.setGraphDatabase(graphDatabase);\n\t" + "\tGraph graph = graphFactory.createGraph(id, vMax, eMax);\n\t\tif(!graphDatabase.containsGraph(id)){\n\t\t\tgraphDatabase.insert((#jgImplDbPackage#.GraphImpl)graph);\n\t\t\treturn (#gcCamelName#)graph;\n\t\t}\n\t\telse\n\t\t\tthrow new GraphException(\"Graph with identifier \" + id + \" already exists in database.\");" : "\tthrow new UnsupportedOperationException(\"No database support compiled.\");"), "}", "", "/**", " * Loads a #gcName# graph from the file <code>filename</code>.", " *", " * @param filename the name of the file", " * @return the loaded #gcName#", " * @throws GraphIOException if the graph cannot be loaded", " */", "public #gcName# load#gcCamelName#(ImplementationType implType, String filename) throws GraphIOException {", "\treturn load#gcCamelName#(implType, filename, null);", "}", "", "/**", " * Loads a #gcName# graph from the file <code>filename</code>.", " *", " * @param filename the name of the file", " * @param pf a progress function to monitor graph loading", " * @return the loaded #gcName#", " * @throws GraphIOException if the graph cannot be loaded", " */", "public #gcName# load#gcCamelName#(ImplementationType implType, String filename, ProgressFunction pf) throws GraphIOException {", "\tswitch(implType){", ((config.hasStandardSupport()) ? "\tcase STANDARD:\n\t" + "\t\tGraph stdGraph = GraphIO.loadGraphFromFileWithStandardSupport(filename, this, pf);\n\t" + "\t\tif (!(stdGraph instanceof #gcName#)) {\n\t" + "\t\t\tthrow new GraphIOException(\"Graph in file \'\" + filename + \"\' is not an instance of GraphClass #gcName#\");\n\t" + "\t\t}\n\t" + "\t\treturn (#gcName#) stdGraph;\n" : ""), ((config.hasTransactionSupport()) ? "\tcase TRANSACTION:\n\t" + "\t\tGraph transGraph = GraphIO.loadGraphFromFileWithTransactionSupport(filename, this, pf);\n\t" + "\t\tif (!(transGraph instanceof #gcName#)) {\n\t" + "\t\t\tthrow new GraphIOException(\"Graph in file \'\" + filename + \"\' is not an instance of GraphClass #gcName#\");\n\t" + "\t\t}\n\t" + "\t\treturn (#gcName#) transGraph;\n" : ""), "\t}", "\tthrow new UnsupportedOperationException(\"No \"+implType+\" support compiled.\");", "}");
  ...
  private CodeBlock createEdgeClass(EdgeClass ec) {
    ...
    code.addNoIndent(new CodeSnippet(true, "{", "\t#ecType# #aecVariable# = #schemaVariable# = #gcVariable#.create#ecType#(\"#ecName#\",", "\t\t#fromPart#,", "\t\t#toPart#);", "\t#aecVariable#.setAbstract(#ecAbstract#);"));
    ...
    for (String redefinedFromRole : ec.getFrom().getRedefinedRoles()) {
      CodeSnippet s = new CodeSnippet("#aecVariable#.getFrom().addRedefinedRole(\"#redefinedFromRole#\");");
      s.setVariable("redefinedFromRole", redefinedFromRole);
      code.add(s);
    }
    for (String redefinedToRole : ec.getTo().getRedefinedRoles()) {
      CodeSnippet s = new CodeSnippet("#aecVariable#.getTo().addRedefinedRole(\"#redefinedToRole#\");");
  ...
  private CodeBlock createVertexClass(VertexClass vc) {
    ...
    code.addNoIndent(new CodeSnippet(true, "{", "\tVertexClass #aecVariable# = #schemaVariable# = #gcVariable#.createVertexClass(\"#vcName#\");", "\t#aecVariable#.setAbstract(#vcAbstract#);"));
