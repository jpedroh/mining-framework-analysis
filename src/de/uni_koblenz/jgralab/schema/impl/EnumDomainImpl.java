...
public class EnumDomainImpl extends DomainImpl implements EnumDomain {
  ...
  @Override public void addConst(String aConst) {
    SchemaImpl s = (SchemaImpl) getSchema();
    s.assertNotFinished();
    if (constants.contains(aConst)) {
      throw new SchemaException("Try to add duplicate constant \'" + aConst + "\' to EnumDomain" + getQualifiedName());
    }
  ...
  @Override public PVector<String> getConsts() {
    return constants;
  }
  ...
  @Override public CodeBlock getReadMethod(String schemaPrefix, String variableName, String graphIoVariableName) {
    return new CodeSnippet(variableName + " = " + getJavaAttributeImplementationTypeName(schemaPrefix) + ".valueOfPermitNull(" + graphIoVariableName + ".matchEnumConstant());");
  }
  ...
  @Override public String toString() {
    StringBuilder output = new StringBuilder("domain Enum " + getQualifiedName() + " (");
    String delim = "";
    int count = 0;
  ...
  @Override public CodeBlock getTransactionReadMethod(String schemaPrefix, String variableName, String graphIoVariableName) {
    return new CodeSnippet(getJavaAttributeImplementationTypeName(schemaPrefix) + " " + variableName + " = " + getJavaAttributeImplementationTypeName(schemaPrefix) + ".valueOfPermitNull(" + graphIoVariableName + ".matchEnumConstant());");
  }
  ...
  @Override public CodeBlock getTransactionWriteMethod(String schemaRootPackagePrefix, String variableName, String graphIoVariableName) {
    return getWriteMethod(schemaRootPackagePrefix, "get" + CodeGenerator.camelCase(variableName) + "()", graphIoVariableName);
  }
  ...
  @Override public void delete() {
    ...
    if (!attributes.isEmpty()) {
      throw new SchemaException("Cannot delete enum domain that is still used by attributes: " + attributes);
    }
