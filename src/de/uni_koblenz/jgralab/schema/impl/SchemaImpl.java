...
public class SchemaImpl implements Schema, ManagableArtifact {
  ...
  void setGraphClass(GraphClassImpl gc) {
    if (graphClass != null) {
      throw new SchemaException("A GraphClass named \'" + graphClass.getQualifiedName() + "\' already exists in this Schema!");
    }
    graphClass = gc;
  }
  ...
  @Override public int getGraphElementClassCount() {
    return graphElementClassCount;
  }
  ...
  @Override public int getIncidenceClassCount() {
    return incidenceClassCount;
  }
  ...
  @Override public GraphElementClass<?, ?> getGraphElementClassById(int id) {
    if (this.finished) {
