...
public class SchemaImpl implements Schema {
  ...
  void addNamedElement(NamedElement namedElement) {
    ...
    if (this.duplicateSimpleNames.containsKey(aec.getSimpleName())) {
      AttributedElementClass other = this.duplicateSimpleNames.get(aec.getSimpleName());
      if (other != null) {
  ...
  private Vector<InMemoryJavaSourceFile> createClasses(CodeGeneratorConfiguration config) {
    ...
    for (VertexClass vertexClass : this.graphClass.getVertexClasses()) {
      VertexCodeGenerator codeGen = new VertexCodeGenerator(vertexClass, this.packagePrefix, config);
      javaSources.addAll(codeGen.createJavaSources());
    }
    for (EdgeClass edgeClass : this.graphClass.getEdgeClasses()) {
      CodeGenerator codeGen = new EdgeCodeGenerator(edgeClass, this.packagePrefix, config);
      javaSources.addAll(codeGen.createJavaSources());
  ...
  @Override public Vector<InMemoryJavaSourceFile> commit(CodeGeneratorConfiguration config) {
    ...
    Vector<InMemoryJavaSourceFile> javaSources = new Vector<InMemoryJavaSourceFile>();
    CodeGenerator schemaCodeGenerator = new SchemaCodeGenerator(this, this.packagePrefix, config);
    javaSources.addAll(schemaCodeGenerator.createJavaSources());
    CodeGenerator factoryCodeGenerator = new GraphFactoryGenerator(this, this.packagePrefix, config);
  ...
  protected Package createPackageWithParents(String qn) {
    ...
    assert (currentParent.getQualifiedName().isEmpty() ? currentParent == this.defaultPackage : true) : "The parent package of package \'" + pkgSimpleName + "\' is empty, but not the default package.";
...
=======
  @Override public List<EdgeClass> getEdgeClassesInTopologicalOrder() {
    ...
    edgeClassSet.addAll(this.graphClass.getEdgeClasses());
    topologicalOrderList.add(this.defaultEdgeClass);
...
=======
  @Override public List<VertexClass> getVertexClassesInTopologicalOrder() {
    ...
    vertexClassSet.addAll(this.graphClass.getVertexClasses());
    topologicalOrderList.add(this.defaultVertexClass);
...
<<<<<<< Unknown file: This is a bug in JDime.
=======
  @Override public void setGraphFactory(GraphFactory factory) {
...
>>>>>>> /usr/src/app/output/jgralab/jgralab/6b4faa42c0de2bdb921de9423bda798251cc7903/src/de/uni_koblenz/jgralab/schema/impl/SchemaImpl.java/right.java
  ...
  void setGraphClass(GraphClass gc) {
    if (this.graphClass != null) {
      throw new SchemaException("There already is a GraphClass named: " + this.graphClass.getQualifiedName() + "in the Schema!");
    }
    this.graphClass = gc;
  }
  ...
  @Override public GenericGraphImpl createGenericGraph() {
    return null;
  }
  ...
  @Override public Graph createGraph(ImplementationType implementationType, String id, int vCount, int eCount) {
    ...
    try {
      if (implementationType != ImplementationType.GENERIC) {
        return (Graph) graphCreateMethod.invoke(null, 
