...
public class GraphClassImpl extends AttributedElementClassImpl<GraphClass, Graph> implements GraphClass {
  Map<String, VertexClass> vertexClasses = new HashMap<String, VertexClass>();

  DirectedAcyclicGraph<VertexClass> vertexClassDag = new DirectedAcyclicGraph<VertexClass>(true);

  Map<String, EdgeClass> edgeClasses = new HashMap<String, EdgeClass>();

  DirectedAcyclicGraph<EdgeClass> edgeClassDag = new DirectedAcyclicGraph<EdgeClass>(true);
  ...
  @Override public final VertexClass getDefaultVertexClass() {
  ...
  @Override public final EdgeClass getDefaultEdgeClass() {
  ...
  @Override public final EdgeClass createEdgeClass(String qualifiedName, VertexClass from, int fromMin, int fromMax, String fromRoleName, AggregationKind aggrFrom, VertexClass to, int toMin, int toMax, String toRoleName, AggregationKind aggrTo) {
    assertNotFinished();
  ...
  @Override public final VertexClass createVertexClass(String qualifiedName) {
    assertNotFinished();
    String[] qn = SchemaImpl.splitQualifiedName(qualifiedName);
    PackageImpl parent = (PackageImpl) ((SchemaImpl) getSchema()).createPackageWithParents(qn[0]);
  ...
  @Override public final GraphElementClass<?, ?> getGraphElementClass(String qn) {
  ...
  @Override public final List<GraphElementClass<?, ?>> getGraphElementClasses() {
    List<GraphElementClass<?, ?>> l = new ArrayList<GraphElementClass<?, ?>>(vertexClasses.values());
    l.addAll(edgeClasses.values());
    return l;
  }

  @Override public final List<EdgeClass> getEdgeClasses() {
  ...
  @Override public final List<VertexClass> getVertexClasses() {
    PVector<VertexClass> vec = vertexClassDag.getNodesInTopologicalOrder();
    assert vec.get(0) == defaultVertexClass;
    return vec.subList(1, vec.size());
  }

  @Override public final VertexClass getVertexClass(String qn) {
    return vertexClasses.get(qn);
  }

  @Override public final EdgeClass getEdgeClass(String qn) {
    return edgeClasses.get(qn);
  }

  @Override public final int getEdgeClassCount() {
    return edgeClasses.size();
  }

  @Override public final int getVertexClassCount() {
    return vertexClasses.size();
  }

  @Override protected final void finish() {
    assertNotFinished();
  ...
  @Override public final boolean hasOwnAttributes() {
    return hasAttributes();
  }

  @Override public final Attribute getOwnAttribute(String name) {
    return getAttribute(name);
  }

  @Override public final int getOwnAttributeCount() {
    return getAttributeCount();
  }

  @Override public final List<Attribute> getOwnAttributeList() {
  ...
  @Override protected final void reopen() {
    for (VertexClass vc : vertexClassDag.getNodesInTopologicalOrder()) {
  ...
  @Override protected void deleteAttribute(AttributeImpl attr) {
