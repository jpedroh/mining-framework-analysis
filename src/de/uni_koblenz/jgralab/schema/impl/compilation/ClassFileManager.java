...
public class ClassFileManager extends ForwardingJavaFileManager<JavaFileManager> {
  ...
  @Override public JavaFileObject getJavaFileForOutput(Location location, String className, Kind kind, FileObject sibling) {
    if (logger != null) {
      logger.fine("(" + location + ", " + className + ", " + kind + ", " + sibling + ")");
    }
    InMemoryClassFile cfa = new InMemoryClassFile(className);
    SchemaClassManager.instance(qualifiedSchemaName).putSchemaClass(className, cfa);
    return cfa;
  }
  ...
  @Override public Iterable<JavaFileObject> list(Location location, String packageName, Set<Kind> kinds, boolean recurse) throws IOException {
    if (logger != null) {
      logger.fine("(" + location + ", " + packageName + ", " + kinds + ", " + recurse + ")");
    }
    EclipseAdapter ea = JGraLab.getEclipseAdapter();
    if ((ea == null) || !((location.getName().equals("CLASS_PATH") && kinds.contains(Kind.CLASS)))) {
