...
public class SetDomainImpl extends CollectionDomainImpl implements SetDomain {
  protected SetDomainImpl(Schema schema, Domain aBaseDomain) {
    super(SETDOMAIN_NAME + "<" + aBaseDomain.getTGTypeName(schema.getDefaultPackage()) + ">", (PackageImpl) schema.getDefaultPackage(), aBaseDomain);
  }
  ...
  private void internalGetReadMethod(CodeList code, String schemaRootPackagePrefix, String variableName, String graphIoVariableName) {
    ...
    code.add(this.getBaseDomain().getReadMethod(schemaRootPackagePrefix, "$" + variableName + "Element", graphIoVariableName), 1);
  ...
  @Override public String getTransactionJavaAttributeImplementationTypeName(String schemaRootPackagePrefix) {
    return SETDOMAIN_TYPE + "<" + this.baseDomain.getTransactionJavaClassName(schemaRootPackagePrefix) + ">";
  }
