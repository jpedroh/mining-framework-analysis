...
public abstract class AttributedElementClassImpl<SC extends AttributedElementClass<SC, IC>, IC extends AttributedElement<SC, IC>> extends NamedElementImpl implements AttributedElementClass<SC, IC> {
  ...
  protected Attribute createAttribute(Attribute anAttribute) {
    ...
    if (containsAttribute(anAttribute.getName())) {
      throw new SchemaException("Duplicate attribute \'" + anAttribute.getName() + "\' in AttributedElementClass \'" + getQualifiedName() + "\'");
    }
  ...
  @Override public Attribute createAttribute(String name, Domain domain, String defaultValueAsString) {
    return createAttribute(new AttributeImpl(name, domain, this, defaultValueAsString));
  }

  @Override public Attribute createAttribute(String name, Domain domain) {
  ...
  @Override public PSet<Constraint> getConstraints() {
    return constraints;
  }

  @SuppressWarnings(value = { "unchecked" }) @Override public Class<IC> getSchemaClass() {
    if (schemaClass == null) {
      String schemaClassName = schema.getPackagePrefix() + "." + getQualifiedName();
      try {
        schemaClass = (Class<IC>) Class.forName(schemaClassName, true, SchemaClassManager.instance(schema.getQualifiedName()));
  ...
  @SuppressWarnings(value = { "unchecked" }) public Class<IC> getSchemaImplementationClass(ImplementationType implType) {
    if (isAbstract()) {
      throw new SchemaClassAccessException("Can\'t get (generated) schema implementation class. AttributedElementClass \'" + getQualifiedName() + "\' is abstract!");
    }
    if (schemaImplementationClass == null) {
