...
public class EdgeNormalizer {
  ...
  public static void main(String[] args) throws NumberFormatException, IOException {
    ...
    IOHelper.strongLog("normalizing edges from " + Config.get().edgeInput + " and storing updated edges at " + Config.get().normalizedEdges);
    double time = ngn.normalize(Config.get().edgeInput, Config.get().normalizedEdges);
    IOHelper.strongLog("time for normalizing edges from " + Config.get().edgeInput + " : " + time);
  }
    ...
    for (int relType = 1; relType < 5; relType++) {
      ...
      for (File file : this.files) {
        ...
        while ((this.line = this.reader.readLine()) != null) {
          ...
          this.edgeCount = Long.parseLong(this.lineSplit[this.lineSplit.length - 1].substring(1));
          if (!this.outgoingEdges.containsKey(this.lineSplit[0])) {
            this.outgoingEdges.put(this.lineSplit[0], this.edgeCount);
          } else {
            this.outgoingEdges.put(this.lineSplit[0], this.outgoingEdges.get(this.lineSplit[0]) + this.edgeCount);
          }
        }
        this.reader.close();
        this.reader = IOHelper.openReadFile(file.getAbsolutePath());
        this.writer = IOHelper.openWriteFile(this.outputPathWithRelType + file.getName(), 32 * 1024 * 1024);
        while ((this.line = this.reader.readLine()) != null) {
          this.lineSplit = this.line.split("\t");
          if (this.lineSplit.length < 3) {
            continue;
          ...
          this.edgeCount = Long.parseLong(this.lineSplit[this.lineSplit.length - 1].substring(1));
          if (this.outgoingEdges.containsKey(this.lineSplit[0])) {
            this.writer.write(this.lineSplit[0] + "\t" + this.lineSplit[1] + "\t#" + (double) this.edgeCount / this.outgoingEdges.get(this.lineSplit[0]) + "\n");
          } else {
            IOHelper.strongLog("no edge count for:" + this.lineSplit[0] + " in file: " + file.getName());
          }
        }
        this.reader.close();
        this.writer.close();
