...
public class NGramBuilder {
  ...
  public static void run(String trainingPath, String trainingFile) {
    ...
    long endTime = 0;
    long sek = 0;
    String[] letters = countMostFrequentStartingLetters(62, trainingFile, trainingPath);
    ...
    for (int n = Config.get().nGramLength; n >= 1; n--) {
      ...
      if (Config.get().createSecondLevelNGramChunks) {
        ngcc.createSecondLevelChunks(trainingPath + Config.get().nGramsNotAggregatedPath + "/" + n, "." + n + "gc");
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish creating detailed ngram chunks");
      }
      if (Config.get().aggregateNGramChunks) {
        Aggregator a = new Aggregator();
        a.aggregateNGrams(trainingPath + Config.get().nGramsNotAggregatedPath + "/" + n, "." + n + "gc", 3);
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
      ...
      if (Config.get().sortNGrams) {
        Sorter s = new Sorter();
        s.sort(trainingPath + Config.get().nGramsNotAggregatedPath + "/" + n, "." + n + "ga");
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish sorting aggregated ngrams");
      }
      if (Config.get().generateNGramDistribution) {
        NGramDistribution ngd = new NGramDistribution();
        ngd.countDistribution(trainingPath + Config.get().nGramsNotAggregatedPath + "/" + n, "." + n + "gs");
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish creating distribution for ngrams");
      }
      new File(trainingPath + Config.get().typologyEdgesPathNotAggregated + "/" + (n - 1)).mkdirs();
      if (Config.get().createTypologyEdgeChunks) {
        TypologyChunkCreator tcc = new TypologyChunkCreator(letters);
        tcc.createTypoelogyEdgeChunks(trainingFile, trainingPath + Config.get().typologyEdgesPathNotAggregated, n - 1);
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish creating typology edge chunks of type: " + (n - 1));
      }
      if (Config.get().createSecondLevelTypologyEdgeChunks) {
        TypologyChunkCreator tcc = new TypologyChunkCreator(letters);
        tcc.createSecondLevelChunks(trainingPath + Config.get().typologyEdgesPathNotAggregated + "/" + (n - 1), "." + (n - 1) + "ec");
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish putting typology edge chunks of type: " + (n - 1) + " into smaller chunks");
      }
      if (Config.get().aggregateTypologyEdgeChunks) {
        Aggregator a = new Aggregator();
        a.aggregateNGrams(trainingPath + Config.get().typologyEdgesPathNotAggregated + "/" + (n - 1), "." + (n - 1) + "ec", 3);
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish aggregating ngrams");
      }
      if (Config.get().sortTypologyEdges) {
        Sorter s = new Sorter();
        s.sort(trainingPath + Config.get().typologyEdgesPathNotAggregated + "/" + (n - 1), "." + (n - 1) + "ea");
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish sorting aggregated ngrams");
      }
      if (Config.get().generateTypologyEdgeDistribution) {
        NGramDistribution ngd = new NGramDistribution();
        ngd.countDistribution(trainingPath + Config.get().typologyEdgesPathNotAggregated + "/" + (n - 1), "." + (n - 1) + "es");
        endTime = System.currentTimeMillis();
        sek = (endTime - startTime) / 1000;
        IOHelper.strongLog(sek + " seconds to: finnish creating distribution for ngrams");
      }
    ...
    if (Config.get().normalizeNGrams) {
      try {
        String input = trainingPath + Config.get().nGramsNotAggregatedPath + "/";
        String output = trainingPath + Config.get().nGramsNotAggregatedPath + "Normalized/";
        new File(output).mkdirs();
        NGramNormalizer ngn = new NGramNormalizer();
        IOHelper.strongLog("normalizing ngrams from " + input + " and storing updated ngrams at " + output);
        double time = ngn.normalize(input, output);
        IOHelper.strongLog("time for normalizing ngrams from " + input + " : " + time);
  ...
  private static String[] countMostFrequentStartingLetters(int k, String trainingFile, String trainingPath) {
    IOHelper.log("Counting the " + k + " most frequent letters in: " + trainingFile);
    String[] result = new String[k];
    try {
      IOHelper.log("Check if most frequent letters have been counted");
      File file = new File(trainingPath + Config.get().nGramsNotAggregatedPath + "mostFrequentLetters.txt");
      if (file.exists()) {
    ...
    BufferedReader br = IOHelper.openReadFile(trainingFile);
    String line = "";
    int cnt = 0;
