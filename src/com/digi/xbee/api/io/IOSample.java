...
public class IOSample {
  ...
  private void parseRawIOSample() {
    int dataIndex = 3;
    digitalHSBMask = ioSamplePayload[1] & 0x01;
    digitalLSBMask = ioSamplePayload[2] & 0xFF;
    digitalMask = (digitalHSBMask << 8) + digitalLSBMask;
    analogMask = ((ioSamplePayload[1] << 8) + ioSamplePayload[2]) & 0x7E00;
    if (digitalMask > 0) {
  ...
  private void parseIOSample() {
    int dataIndex = 4;
    digitalHSBMask = ioSamplePayload[1] & 0x7F;
    digitalLSBMask = ioSamplePayload[2] & 0xFF;
    digitalMask = (digitalHSBMask << 8) + digitalLSBMask;
    analogMask = ioSamplePayload[3] & 0xBF;
    if (digitalMask > 0) {
