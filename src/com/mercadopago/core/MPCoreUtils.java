  ...
  private static StringBuilder generateIdempotentKey(StringBuilder sb, Object object) throws MPException {
    ...
    for (Field field : declaredFields) {
      if (field.getAnnotation(Idempotent.class) != null) {
        ...
        if (value != null) {
          if (value.getClass().getCanonicalName().indexOf("com.mercadopago.resources.datastructures.") == 0 && !value.getClass().getName().contains("$")) {
            sb = generateIdempotentKey(sb, value);
          } else {
            if (value.getClass().getCanonicalName() == "java.util.ArrayList") {
