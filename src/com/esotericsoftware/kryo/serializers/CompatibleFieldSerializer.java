 ...
 * @author Nathan Sweet */
public class CompatibleFieldSerializer<T extends java.lang.Object> extends FieldSerializer<T> {
    ...
    if (config != null && !config.extendedFieldNames) {
      ...
      for (int i = 0, n = fields.length; i < n; i++) {
        ...
        if (!hashSet.add(field.getName())) {
          if (WARN) {
            warn("Detected duplicate field " + field.getName() + " in class hierarchy " + field.getDeclaringClass() + ". Consider enabling FieldSerializerConfig.extendedFieldNames");
          }
          break;
        }
      }
  ...
  @Override public void write(Kryo kryo, Output output, T object) {
    ...
    if (!context.containsKey(this)) {
      if (TRACE) {
        trace("kryo", "Write fields for class: " + type.getName());
      }
  ...
  @Override public T read(Kryo kryo, Input input, Class<? extends T> type) {
    int pop = pushTypeVariables();
    ...
    for (int i = 0, n = fields.length; i < n; i++) {
      ...
      if (readUnknownTagData) {
        ...
        if (cachedField == null) {
          if (TRACE) {
            trace("kryo", "Read unknown data, type: " + className(valueClass) + pos(input.position()));
          }
          try {
            kryo.readObject(fieldInput, valueClass);
          } catch (KryoException ex) {
            String message = "Unable to read unknown data, type: " + className(valueClass) + " (" + getType().getName() + "#" + cachedField + ")";
            if (!chunked) {
              throw new KryoException(message, ex);
            }
            if (DEBUG) {
        ...
        if (cachedField.valueClass != null && !Util.isAssignableTo(valueClass, cachedField.field.getType())) {
          String message = "Read type is incompatible with the field type: " + className(valueClass) + " -> " + className(cachedField.valueClass) + " (" + getType().getName() + "#" + cachedField + ")";
          if (!chunked) {
  ...
  private CachedField[] readFields(Kryo kryo, Input input) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (TRACE) {
        trace("kryo", "Read field name: " + names[i]);
      }
    }
    ...
    if (length < binarySearchThreshold) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (TRACE) {
          trace("kryo", "Unknown field will be skipped: " + schemaName);
        }
      }
    } else {
  ...
  public static class CompatibleFieldSerializerConfig extends FieldSerializerConfig {
    ...
    @Override public CompatibleFieldSerializerConfig clone() {
    ...
    public void setChunkedEncoding(boolean chunked) {
      ...
      if (TRACE) {
        trace("kryo", "CompatibleFieldSerializerConfig setChunked: " + chunked);
      }
    }
    ...
    @Override public void setExtendedFieldNames(boolean extendedFieldNames) {
      this.extendedFieldNames = extendedFieldNames;
      if (TRACE) {
        trace("kryo", "CompatibleFieldSerializerConfig extendedFieldNames: " + extendedFieldNames);
      }
    }
    ...
    public void setChunkSize(int chunkSize) {
      ...
      if (TRACE) {
        trace("kryo", "CompatibleFieldSerializerConfig setChunkSize: " + chunkSize);
      }
    }
