...
class CachedFields implements Comparator<CachedField> {
  ...
  public void rebuild() {
    if (serializer.type.isInterface()) {
    ...
    if (fields.length != newFields.size()) {
      fields = new CachedField[newFields.size()];
    }
    newFields.toArray(fields);
    Arrays.sort(fields, this);
    if (copyFields.length != newCopyFields.size()) {
      copyFields = new CachedField[newCopyFields.size()];
    }
  ...
  private void addField(Field field, boolean asm, ArrayList<CachedField> fields, ArrayList<CachedField> copyFields) {
    ...
    if (!field.isAccessible()) {
      ...
      try {
        ...
        if (DEBUG) {
          debug("kryo", "Unable to set field as accessible: " + field);
        }
        return;
      }
    }
    Optional optional = field.getAnnotation(Optional.class);
    if (optional != null && !serializer.kryo.getContext().containsKey(optional.value())) {
      return;
    }
    ...
    if (asm && !Modifier.isFinal(modifiers) && Modifier.isPublic(modifiers) && Modifier.isPublic(fieldClass.getModifiers())) {
      try {
        ...
        if (DEBUG) {
          debug("kryo", "Unable to use ReflectASM.", ex);
        }
      }
    }
    ...
    if (cachedField instanceof ReflectField) {
      ...
      if (serializer.kryo.isFinal(fieldClass) || config.fixedFieldTypes) {
  ...
  private CachedField newUnsafeField(Field field, Class fieldClass, GenericType genericType) {
    ...
    if (fieldClass == String.class && (!serializer.kryo.getReferences() || !serializer.kryo.getReferenceResolver().useReferences(String.class))) {
  ...
  private CachedField newAsmField(Field field, Class fieldClass, GenericType genericType) {
    if (fieldClass.isPrimitive()) {
      if (fieldClass == int.class) {
        return new IntAsmField(field);
      }
      if (fieldClass == float.class) {
        return new FloatAsmField(field);
      }
      if (fieldClass == boolean.class) {
        return new BooleanAsmField(field);
      }
      if (fieldClass == long.class) {
        return new LongAsmField(field);
      }
      if (fieldClass == double.class) {
        return new DoubleAsmField(field);
      }
      if (fieldClass == short.class) {
        return new ShortAsmField(field);
      }
      if (fieldClass == char.class) {
        return new CharAsmField(field);
      }
      if (fieldClass == byte.class) {
        return new ByteAsmField(field);
      }
    }
    if (fieldClass == String.class && (!serializer.kryo.getReferences() || !serializer.kryo.getReferenceResolver().useReferences(String.class))) {
  ...
  private CachedField newReflectField(Field field, Class fieldClass, GenericType genericType) {
    if (fieldClass.isPrimitive()) {
      if (fieldClass == int.class) {
        return new IntReflectField(field);
      }
      if (fieldClass == float.class) {
        return new FloatReflectField(field);
      }
      if (fieldClass == boolean.class) {
        return new BooleanReflectField(field);
      }
      if (fieldClass == long.class) {
        return new LongReflectField(field);
      }
      if (fieldClass == double.class) {
        return new DoubleReflectField(field);
      }
      if (fieldClass == short.class) {
        return new ShortReflectField(field);
      }
      if (fieldClass == char.class) {
        return new CharReflectField(field);
      }
      if (fieldClass == byte.class) {
        return new ByteReflectField(field);
      }
    }
  ...
  private void applyAnnotations(CachedField cachedField) {
    ...
    if (field.isAnnotationPresent(CollectionSerializer.BindCollection.class)) {
      ...
      if (!Collection.class.isAssignableFrom(field.getType())) {
        throw new KryoException("@BindCollection can only be used with a field implementing Collection: " + className(field.getType()));
      }
      ...
      Serializer elementSerializer = newSerializer(elementClass, annotation.elementSerializer(), annotation.elementSerializerFactory());
      CollectionSerializer serializer = new CollectionSerializer();
      serializer.setElementsCanBeNull(annotation.elementsCanBeNull());
      if (elementClass != null) {
        serializer.setElementClass(elementClass);
      }
      if (elementSerializer != null) {
        serializer.setElementSerializer(elementSerializer);
      }
      cachedField.setSerializer(serializer);
    }
    if (field.isAnnotationPresent(MapSerializer.BindMap.class)) {
      ...
      Serializer valueSerializer = newSerializer(valueClass, annotation.valueSerializer(), annotation.valueSerializerFactory());
      ...
      if (keyClass != null) {
        serializer.setKeyClass(keyClass);
      }
      if (keySerializer != null) {
        serializer.setKeySerializer(keySerializer);
      }
      if (valueClass != null) {
        serializer.setValueClass(valueClass);
      }
      if (valueSerializer != null) {
        serializer.setValueSerializer(valueSerializer);
      }
      cachedField.setSerializer(serializer);
    }
  }
  ...
  private Serializer newSerializer(Class valueClass, Class serializerClass, Class factoryClass) {
    ...
    if (factoryClass == null && serializerClass != null) {
      factoryClass = ReflectionSerializerFactory.class;
    }
