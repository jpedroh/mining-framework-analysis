 ...
 * @author Nathan Sweet */
public class TaggedFieldSerializer<T extends java.lang.Object> extends FieldSerializer<T> {
    ...
    for (int i = 0; i < fieldCount; i++) {
      ...
      if (readUnknownTagData) {
        ...
        try {
          ...
          String message = "Unable to read unknown tag " + tag + " data (unknown type). (" + getType().getName() + "#" + cachedField + ")";
          if (!chunked) {
        ...
        if (cachedField == null) {
          if (TRACE) {
            trace("kryo", "Read unknown tag " + tag + " data, type: " + className(valueClass));
          }
          try {
            kryo.readObject(fieldInput, valueClass);
          } catch (KryoException ex) {
            String message = "Unable to read unknown tag " + tag + " data, type: " + className(valueClass) + " (" + getType().getName() + "#" + cachedField + ")";
            if (!chunked) {
              throw new KryoException(message, ex);
            }
            if (DEBUG) {
        ...
        if (cachedField == null) {
          ...
          if (TRACE) {
            trace("kryo", "Skip unknown field tag: " + tag);
          }
          inputChunked.nextChunk();
          continue;
        }
  ...
  @Retention(value = RetentionPolicy.RUNTIME) @Target(value = { ElementType.FIELD }) public @interface Tag {
    int value();
  }
  ...
  public static class TaggedFieldSerializerConfig extends FieldSerializerConfig {
    ...
    public void setChunkedEncoding(boolean chunked) {
      ...
      if (TRACE) {
        trace("kryo", "TaggedFieldSerializerConfig setChunked: " + chunked);
      }
    }
    ...
    public void setChunkSize(int chunkSize) {
      ...
      if (TRACE) {
        trace("kryo", "TaggedFieldSerializerConfig setChunkSize: " + chunkSize);
      }
    }
