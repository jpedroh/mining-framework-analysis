...
public class FieldSerializer<T extends java.lang.Object> extends Serializer<T> implements Comparator<FieldSerializer.CachedField> {
  final Kryo kryo;
...

  {
    ...
    varIntsEnabled = true;
    if (TRACE) {
      trace("kryo", "Optimize ints: " + varIntsEnabled);
    }
  }
  ...
  public FieldSerializer(Kryo kryo, Class type) {
    ...
    if (!this.useAsmEnabled && !unsafeAvailable) {
      ...
      if (TRACE) {
        trace("kryo", "sun.misc.Unsafe is unavailable, using ASM.");
      }
    }
  ...
  protected void rebuildCachedFields() {
    if (TRACE && generics != null) {
      trace("kryo", "Generic type parameters: " + Arrays.toString(generics));
    }
    if (type.isInterface()) {
      fields = new CachedField[0];
      return;
    }
  ...
  protected void processAnnotatedFields() {
    for (int i = 0, n = fields.length; i < n; i++) {
      ...
      if (field.isAnnotationPresent(CollectionSerializer.BindKryoSerializer.class) && field.isAnnotationPresent(MapSerializer.BindKryoSerializer.class)) {
      }
      if (field.isAnnotationPresent(CollectionSerializer.BindKryoSerializer.class)) {
        if (fields[i].serializer != null) {
          throw new RuntimeException("CollectionSerialier.BindKryoSerializer cannot be used with field " + fields[i].getField().getDeclaringClass().getName() + "." + fields[i].getField().getName() + ", because it has a serializer already.");
        }
        CollectionSerializer.BindKryoSerializer annotation = field.getAnnotation(CollectionSerializer.BindKryoSerializer.class);
        if (Collection.class.isAssignableFrom(fields[i].field.getType())) {
          ...
          Serializer elementSerializer = (elementSerializerClass == null) ? null : ReflectionSerializerFactory.makeSerializer(this.getKryo(), elementSerializerClass, field.getClass());
          ...
          throw new RuntimeException("CollectionSerialier.BindKryoSerializer should be used only with fields implementing java.util.Collection, but field " + fields[i].getField().getDeclaringClass().getName() + "." + fields[i].getField().getName() + " does not implement it.");
        }
      }
      if (field.isAnnotationPresent(MapSerializer.BindKryoSerializer.class)) {
        if (fields[i].serializer != null) {
          throw new RuntimeException("MapSerialier.BindKryoSerializer cannot be used with field " + fields[i].getField().getDeclaringClass().getName() + "." + fields[i].getField().getName() + ", because it has a serializer already.");
        }
        ...
        if (Map.class.isAssignableFrom(fields[i].field.getType())) {
          ...
          Serializer valueSerializer = (valueSerializerClass == null) ? null : ReflectionSerializerFactory.makeSerializer(this.getKryo(), valueSerializerClass, field.getClass());
          Serializer keySerializer = (keySerializerClass == null) ? null : ReflectionSerializerFactory.makeSerializer(this.getKryo(), keySerializerClass, field.getClass());
          ...
          throw new RuntimeException("MapSerialier.BindKryoSerializer should be used only with fields implementing java.util.Map, but field " + fields[i].getField().getDeclaringClass().getName() + "." + fields[i].getField().getName() + " does not implement it.");
        }
      }
    }
  }
  ...
  private List<Field> buildValidFields(boolean transientFields, List<Field> allFields, ObjectMap context, IntArray useAsm) {
    ...
    for (int i = 0, n = allFields.size(); i < n; i++) {
      ...
      if (Modifier.isTransient(modifiers) != transientFields) {
        continue;
      }
      if (Modifier.isStatic(modifiers)) {
        continue;
      }
      if (field.isSynthetic() && ignoreSyntheticFields) {
        continue;
      }
  ...
  CachedField newCachedField(Field field, int fieldIndex, int accessIndex) {
    ...
    if (fieldGenericType == fieldClass[0]) {
      if (TRACE) {
        trace("kryo", "Field " + field.getName() + ": " + fieldClass[0]);
      }
    ...
    if (kryo.isFinal(fieldClass[0]) || fixedFieldTypes) {
    ...
    if (accessIndex != -1) {
      ...
      if (!useAsmEnabled) {
        ...
        if (fieldGenerics != null) {
          ...
          if (TRACE) {
            trace("kryo", "Field generics: " + Arrays.toString(cachedFieldGenerics));
          }
        }
      }
    }
  ...
  public void setFieldsCanBeNull(boolean fieldsCanBeNull) {
    ...
    if (TRACE) {
      trace("kryo", "setFieldsCanBeNull: " + fieldsCanBeNull);
    }
    rebuildCachedFields();
  }
  ...
  public void setFieldsAsAccessible(boolean setFieldsAsAccessible) {
    ...
    if (TRACE) {
      trace("kryo", "setFieldsAsAccessible: " + setFieldsAsAccessible);
    }
    rebuildCachedFields();
  }
  ...
  public void setIgnoreSyntheticFields(boolean ignoreSyntheticFields) {
    ...
    if (TRACE) {
      trace("kryo", "setIgnoreSyntheticFields: " + ignoreSyntheticFields);
    }
    rebuildCachedFields();
  }
  ...
  public void setFixedFieldTypes(boolean fixedFieldTypes) {
    ...
    if (TRACE) {
      trace("kryo", "setFixedFieldTypes: " + fixedFieldTypes);
    }
    rebuildCachedFields();
  }
  ...
  public void setUseAsm(boolean setUseAsm) {
    ...
    if (TRACE) {
      trace("kryo", "setUseAsm: " + setUseAsm);
    }
    rebuildCachedFields();
  }
  ...
  public void write(Kryo kryo, Output output, T object) {
    if (TRACE) {
      trace("kryo", "FieldSerializer.write fields of class: " + object.getClass().getName());
    }
  ...
  public static abstract class CachedField<X extends java.lang.Object> {
    Field field;
  ...
  @Retention(value = RetentionPolicy.RUNTIME) @Target(value = { ElementType.FIELD }) static public @interface Optional {
    public String value();
  }

  @Retention(value = RetentionPolicy.RUNTIME) @Target(value = { ElementType.FIELD }) public @interface BindKryoSerializer {
    @SuppressWarnings(value = { "rawtypes" }) Class<? extends Serializer> value();
  }
}