...
public class FieldSerializer<T extends java.lang.Object> extends Serializer<T> {
  ...
  static <T extends java.lang.Object> T invokeCanonicalConstructor(Class<T> type, CachedField[] fields, Object[] values) {
    final Class<?>[] objects = Arrays.stream(fields).sorted(Comparator.comparing((f) -> f.index)).map((f) -> f.field.getType()).toArray(Class[]::new);
    return invokeCanonicalConstructor(type, objects, values);
  }

  static <T extends java.lang.Object> T invokeCanonicalConstructor(Class<T> type, Class<?>[] paramTypes, Object[] args) {
    try {
