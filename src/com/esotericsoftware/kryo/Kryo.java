...
public class Kryo {
  ...
  boolean writeReferenceOrNull(Output output, Object object, boolean mayBeNull) {
    ...
    if (id != -1) {
      if (DEBUG) {
        debug("kryo", "Write object reference " + id + ": " + string(object));
      }
    ...
    if (TRACE) {
      trace("kryo", "Write initial object reference " + id + ": " + string(object));
    }
    return false;
  }
  ...
  int readReferenceOrNull(Input input, Class type, boolean mayBeNull) {
    ...
    if (id == NOT_NULL) {
      ...
      if (TRACE) {
        trace("kryo", "Read initial object reference " + id + ": " + className(type));
      }
    ...
    if (DEBUG) {
      debug("kryo", "Read object reference " + id + ": " + string(readObject));
    }
    return REF;
  }
  ...
  public void pushGenericsScope(Class type, Generics generics) {
    if (TRACE) {
      trace("kryo", "Settting a new generics scope for class " + type.getName() + ": " + generics);
    }
