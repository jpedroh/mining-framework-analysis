...
public class Z implements MouseListener, MouseWheelListener, MouseMotionListener, KeyListener, Runnable {
  ...
  public void activateGoUp() {
    ...
    if (selectedNode.getNodeType() == ZNodeType.PACKAGE) {
      ...
      if (selectedNode.getNodeType() == ZNodeType.MODULE) {
        pFile = pFile.getParentFile();
      }
    }
  ...
  ZNode createNewZ(final Point point, final ZNodeType type) {
    ...
    if (type == ZNodeType.DEPENDENCY) {
      new Thread(new Runnable() {
        @Override public void run() {
          ZNode dep = createNewZNode(point, type, new FakeDependencySelector().get());
          if (dep != null) {
  ...
  private ZNode createNewZNode(Point point, ZNodeType type, String name) {
    ...
    if (type == ZNodeType.METHOD) {
      zNode.setParentFile(new File(selectedNode.getParentFile(), selectedNode.getName() + "." + selectedNode.getExtension()));
  ...
  @Override public void mouseReleased(MouseEvent e) {
    ...
    if (point1 != null && point2 != null) {
      if (draggedNode == null) {
        ...
        if (e.getPoint().y >= display.getHeight() - 40) {
  ...
  @Override public void run() {
    ...
    if (getState() == State.SELECTING) {
      ...
      if (getState() == State.ANIMATING) {
        ...
        if (getState() == State.TIME_TRAVEL && aniCount.get() * diffsMap.getLogSize() / 1000 > (aniCount.get() - 1) * diffsMap.getLogSize() / 1000) {
          synchronized (zNodes) {
...
<<<<<<< /usr/src/app/output/adamd/z/da778dcb9cf35110cc3b35d9c4c26a66238d3450/src/com/adamldavis/z/Z.java/left.java
    runAsThread(new Runnable() {
      @Override public void run() {
        display.doLayout();
        display.getContentPane().doLayout();
...
=======
    start()
>>>>>>> /usr/src/app/output/adamd/z/da778dcb9cf35110cc3b35d9c4c26a66238d3450/src/com/adamldavis/z/Z.java/right.java
    ;
  }
  ...
  private void sortNodes() {
    Collections.sort(selectedNode.getSubmodules(), new Comparator<ZNode>() {
      @Override public int compare(ZNode node1, ZNode node2) {
        switch (order) {
          case ALPHA:
  ...
  private void updateSubLocations(ZNode node, boolean immediate, Point2D loc) {
    ...
    final float factor = selectedNode == node ? (1.3f + logSize(node.getSubmodules().size()) / 2.2f) : 1.2f;
    int size = Math.round(sizeMap.get(node) * factor);
    final float xRatio = display.getWidth() / display.getHeight();
    Map<ZNode, Point2D> map = new PixelZNodePositioner(center, new Dimension((int) (size * xRatio), size), new DirectionZNodePositioner(direction, makeNodePositioner())).getNewPositions(node);
