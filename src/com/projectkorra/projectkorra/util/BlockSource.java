...
public class BlockSource {
  ...
  public static void update(Player player, ClickType clickType) {
    ...
    if (coreAbil instanceof WaterAbility) {
      ...
      if (coreAbil instanceof EarthAbility) {
  ...
  public static BlockSourceInformation getBlockSourceInformation(Player player, BlockSourceType sourceType, ClickType clickType) {
    if (!playerSources.containsKey(player)) {
      ...
      if (!playerSources.get(player).containsKey(sourceType)) {
        ...
        if (!playerSources.get(player).get(sourceType).containsKey(clickType)) {
          return null;
        }
      }
    }
...
<<<<<<< /usr/src/app/output/projectkorra/projectkorra/d0b17cacf5a4067954c3a63ffaf45ca733cf2bdb/src/com/projectkorra/projectkorra/util/BlockSource.java/left.java
      WaterAbility.isBendableWaterTempBlock(info.getBlock())
=======
      PhaseChange.getFrozenBlocksAsTempBlock().contains(TempBlock.get(info.getBlock()))
...
<<<<<<< /usr/src/app/output/projectkorra/projectkorra/d0b17cacf5a4067954c3a63ffaf45ca733cf2bdb/src/com/projectkorra/projectkorra/util/BlockSource.java/left.java
      WaterAbility.isBendableWaterTempBlock(sourceBlock)
=======
      PhaseChange.getFrozenBlocksAsTempBlock().contains(TempBlock.get(sourceBlock))
>>>>>>> /usr/src/app/output/projectkorra/projectkorra/d0b17cacf5a4067954c3a63ffaf45ca733cf2bdb/src/com/projectkorra/projectkorra/util/BlockSource.java/right.java
      ) {
        return null;
      }
  ...
  private static boolean isStillAValidSource(BlockSourceInformation info, double range, ClickType clickType) {
    if (info == null || info.getBlock() == null) {
      ...
      if (info.getClickType() != clickType) {
        ...
        if (!info.getPlayer().getWorld().equals(info.getBlock().getWorld())) {
          return false;
        } else {
          if (Math.abs(info.getPlayer().getLocation().distance(info.getBlock().getLocation())) > range) {
            return false;
          } else {
            if (info.getSourceType() == BlockSourceType.WATER && !WaterAbility.isWaterbendable(info.getPlayer(), null, info.getBlock())) {
              return false;
            } else {
              if (info.getSourceType() == BlockSourceType.ICE && !WaterAbility.isIcebendable(info.getPlayer(), info.getBlock().getType(), false)) {
                return false;
              } else {
                if (info.getSourceType() == BlockSourceType.PLANT && (!WaterAbility.isPlant(info.getBlock()) || !WaterAbility.isWaterbendable(info.getPlayer(), null, info.getBlock()))) {
                  return false;
                } else {
                  if (info.getSourceType() == BlockSourceType.EARTH && !EarthAbility.isEarthbendable(info.getPlayer(), info.getBlock())) {
                    return false;
                  } else {
                    if (info.getSourceType() == BlockSourceType.METAL && (!EarthAbility.isMetal(info.getBlock()) || !EarthAbility.isEarthbendable(info.getPlayer(), info.getBlock()))) {
                      return false;
                    } else {
                      if (info.getSourceType() == BlockSourceType.LAVA && (!EarthAbility.isLava(info.getBlock()) || !EarthAbility.isLavabendable(info.getPlayer(), info.getBlock()))) {
                        return false;
                      }
                    }
                  }
