...
public class WhoCommand extends PKCommand {
  ...
  private void whoPlayer(final CommandSender sender, final String playerName) {
    @SuppressWarnings(value = { "deprecation" }) final OfflinePlayer player = Bukkit.getOfflinePlayer(playerName);
    ...
    if (bPlayer != null) {
      sender.sendMessage(
<<<<<<< /usr/src/app/output/projectkorra/projectkorra/74cd671247364b968d6693f48b8a118e6e9e4efb/src/com/projectkorra/projectkorra/command/WhoCommand.java/left.java
      player.getName()
=======
      ChatColor.BOLD + player.getName()
>>>>>>> /usr/src/app/output/projectkorra/projectkorra/74cd671247364b968d6693f48b8a118e6e9e4efb/src/com/projectkorra/projectkorra/command/WhoCommand.java/right.java
       + (!player.isOnline() ? ChatColor.RESET + " (Offline)" : ""));
      ...
      if (GeneralMethods.hasRPG()) {
        if (RPGMethods.isCurrentAvatar(player.getUniqueId())) {
          ...
          if (RPGMethods.hasBeenAvatar(player.getUniqueId())) {
  ...
  @Override protected List<String> getTabCompletion(CommandSender sender, List<String> args) {
