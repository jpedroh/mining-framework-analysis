...
public abstract class EarthAbility extends ElementalAbility {
        ...
        if (isTransparent(affectedblock)) {
          ...
          if (up) {
            ...
            if (topblock.getType() != Material.AIR) {
              ...
              if (!affectedblock.isLiquid() && affectedblock.getType() != Material.AIR) {
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "deprecation" }) public static void addTempAirBlock(Block block) {
    Information info;
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "deprecation" }) public static Block getEarthSourceBlock(Player player, String abilityName, double range) {
    ...
    if (bPlayer == null) {
      ...
      if (isEarthbendable(testBlock.getType(), true, true, true)) {
        ...
        if (!isTransparent(player, testBlock)) {
          return null;
        }
      }
    }
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "deprecation" }) public static Block getLavaSourceBlock(Player player, String abilityName, double range) {
    Location location = player.getEyeLocation();
    Vector vector = location.getDirection().clone().normalize();
    for (double i = 0; i <= range; i++) {
      Block block = location.clone().add(vector.clone().multiply(i)).getBlock();
  ...
  @SuppressWarnings(value = { "deprecation" }) public static Block getTargetEarthBlock(Player player, int range) {
  ...
  public static boolean isEarthbendable(Player player, String abilityName, Block block) {
    ...
    if (bPlayer == null || !isEarthbendable(block.getType(), true, true, true) || PREVENT_EARTHBENDING.contains(block) || GeneralMethods.isRegionProtectedFromBuild(player, abilityName, block.getLocation())) {
      ...
      if (isMetal(block) && !bPlayer.canMetalbend()) {
        return false;
      } else {
        if (isSand(block) && !bPlayer.canSandbend()) {
          return false;
        } else {
          if (isLava(block) && !bPlayer.canLavabend()) {
            return false;
          }
        }
      }
  ...
  @SuppressWarnings(value = { "deprecation" }) public static boolean isLavabendable(Player player, Block block) {
    byte full = 0x0;
    if (TempBlock.isTempBlock(block)) {
  ...
  @SuppressWarnings(value = { "deprecation" }) public static void moveEarthBlock(Block source, Block target) {
    Information info;
    ...
    if (info.getState().getType() == Material.SAND) {
      ...
      if (info.getState().getType() == Material.GRAVEL) {
  ...
  @SuppressWarnings(value = { "deprecation" }) public static boolean revertBlock(Block block) {
    byte full = 0x0;
    if (!isEarthRevertOn()) {
      MOVED_EARTH.remove(block);
      return false;
