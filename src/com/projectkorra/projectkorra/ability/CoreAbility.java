...
public abstract class CoreAbility implements Ability {
      ...
      for (CoreAbility abil : setAbils) {
        if (abil instanceof PassiveAbility) {
          ...
          if (bPlayer == null || !abil.getPlayer().isOnline()) {
            ...
            if (!bPlayer.canBendPassive(abil.getElement())) {
              abil.remove();
              continue;
            } else {
              if (!bPlayer.canUsePassive(abil.getElement())) {
                continue;
              }
            }
          }
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "unchecked" }) public static <T extends CoreAbility> Collection<T> getAbilities(Class<T> clazz) {
  ...
  /**
	 ...
	 * @return a Collection of real instances
	 */
  @SuppressWarnings(value = { "unchecked" }) public static <T extends CoreAbility> Collection<T> getAbilities(Player player, Class<T> clazz) {
      ...
      for (final ClassInfo info : ClassPath.from(loader).getAllClasses()) {
        ...
        try {
          ...
          if (ability == null || ability.getName() == null) {
            ...
            if (!ability.isEnabled() && !disabled.contains(ability.getName())) {
  ...
  @Override public boolean isHiddenAbility() {
    return hidden;
  }
