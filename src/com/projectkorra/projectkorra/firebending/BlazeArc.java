...
public class BlazeArc extends FireAbility {
    ...
    if (!bPlayer.canBendIgnoreBindsCooldowns(this)) {
      ...
      if (System.currentTimeMillis() - time >= interval) {
        ...
        if (isIgnitable(player, block)) {
          ...
          if (isIgnitable(player, block.getRelative(BlockFace.DOWN))) {
            ignite(block.getRelative(BlockFace.DOWN));
            location = block.getRelative(BlockFace.DOWN).getLocation();
          } else {
            if (isIgnitable(player, block.getRelative(BlockFace.UP))) {
  ...
  @SuppressWarnings(value = { "deprecation" }) public static void removeBlock(Block block) {
