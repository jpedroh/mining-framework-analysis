...
public class IceSpikeBlast extends IceAbility {
  ...
  public IceSpikeBlast(Player player) {
    ...
    if (sourceBlock == null) {
      ...
      if (GeneralMethods.isRegionProtectedFromBuild(this, sourceBlock.getLocation())) {
        return;
      } else {
        prepare(sourceBlock);
      }
  ...
  @Override public void progress() {
    if (!bPlayer.canBendIgnoreBindsCooldowns(this)) {
      ...
      if (player.getEyeLocation().distanceSquared(location) >= range * range) {
        ...
        if (!getName().contains(bPlayer.getBoundAbilityName()) && prepared) {
          remove();
          return;
        }
      }
  ...
  private static void block(Player player) {
    for (IceSpikeBlast iceSpike : getAbilities(IceSpikeBlast.class)) {
      if (iceSpike.player.equals(player)) {
        ...
        if (!iceSpike.location.getWorld().equals(player.getWorld())) {
          continue;
        } else {
  ...
  @SuppressWarnings(value = { "deprecation" }) private static void waterBottle(Player player) {
