...
public class WaterManipulation extends WaterAbility {
  ...
  /**
	 ...
	 */
  @Deprecated public static boolean annihilateBlasts(Location location, double radius, Player player) {
    boolean broke = false;
    ...
    for (WaterManipulation manip : getAbilities(WaterManipulation.class)) {
      if (!manip.location.getWorld().equals(player.getWorld())) {
        ...
        if (!manip.progressing) {
          ...
          if (manip.getPlayer().equals(player)) {
            continue;
          } else {
            if (GeneralMethods.isRegionProtectedFromBuild(manip, manip.location)) {
              continue;
            }
          }
        }
  ...
  public static boolean canFlowFromTo(Block from, Block to) {
    if (AFFECTED_BLOCKS.containsKey(to) || AFFECTED_BLOCKS.containsKey(from)) {
      ...
      if (WaterSpout.getAffectedBlocks().containsKey(to) || WaterSpout.getAffectedBlocks().containsKey(from)) {
        return false;
      } else {
        if (SurgeWall.getAffectedBlocks().containsKey(to) || SurgeWall.getAffectedBlocks().containsKey(from)) {
          return false;
        } else {
          if (SurgeWall.getWallBlocks().containsKey(to) || SurgeWall.getWallBlocks().containsKey(from)) {
            return false;
          } else {
            if (SurgeWave.isBlockWave(to) || SurgeWave.isBlockWave(from)) {
              return false;
            } else {
              if (isAdjacentToFrozenBlock(to) || isAdjacentToFrozenBlock(from)) {
                return false;
              }
            }
          }
  ...
  public static boolean canPhysicsChange(Block block) {
    if (AFFECTED_BLOCKS.containsKey(block)) {
      ...
      if (WaterSpout.getAffectedBlocks().containsKey(block)) {
        ...
        if (SurgeWall.getAffectedBlocks().containsKey(block)) {
          return false;
        } else {
          if (SurgeWall.getWallBlocks().containsKey(block)) {
            return false;
          } else {
            if (SurgeWave.isBlockWave(block)) {
              return false;
            } else {
...
<<<<<<< /usr/src/app/output/projectkorra/projectkorra/d0b17cacf5a4067954c3a63ffaf45ca733cf2bdb/src/com/projectkorra/projectkorra/waterbending/WaterManipulation.java/left.java
              WaterAbility.isBendableWaterTempBlock(block)
=======
              PhaseChange.getFrozenBlocksAsTempBlock().contains(TempBlock.get(block))
