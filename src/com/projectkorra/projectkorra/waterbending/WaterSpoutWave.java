  ...
  @Override public void progress() {
    ...
    if (type != AbilityType.RELEASE) {
      if (!player.hasPermission("bending.ability.WaterSpout.Wave")) {
        ...
        if (!getName().contains(bPlayer.getBoundAbilityName())) {
          remove();
          return;
        }
      }
    ...
    if (type == AbilityType.CLICK) {
      if (origin == null) {
        ...
        if (!isWaterbendable(block) || GeneralMethods.isRegionProtectedFromBuild(this, origin)) {
          ...
          if (iceOnly && !(isIcebendable(block) || isSnow(block))) {
            remove();
            return;
          }
        }
      ...
      if (type == AbilityType.SHIFT) {
        ...
        if (animation == AnimateState.RISE && location != null) {
          ...
          if (animation == AnimateState.TOWARD_PLAYER) {
            revertBlocks();
  ...
  public static boolean wasBrokenFor(Player player, Block block) {
    ...
    if (!waves.isEmpty()) {
      ...
      if (wave.origin == null) {
        ...
        if (wave.origin.getBlock().equals(block)) {
          return true;
        }
      }
    }
