...
public class SurgeWall extends WaterAbility {
  ...
  public SurgeWall(Player player) {
    ...
    if (bPlayer.isOnCooldown("SurgeWall")) {
      ...
      if (wall == null && WaterReturn.hasWaterBottle(player)) {
  ...
  @SuppressWarnings(value = { "deprecation" }) public void moveWater() {
    if (sourceBlock != null) {
  ...
  @Override public void progress() {
    ...
    if (System.currentTimeMillis() - time >= interval) {
      ...
      if (!progressing && !matchesName) {
        ...
        if (progressing && (!player.isSneaking() || !matchesName)) {
          remove();
          return;
        } else {
          if (!progressing) {
          ...
          for (double angle = 0; angle < 360; angle += 10) {
            ...
            if (GeneralMethods.isRegionProtectedFromBuild(this, block.getLocation())) {
              ...
              if (WALL_BLOCKS.containsKey(block)) {
                ...
                if (!blocks.contains(block) && (block.getType() == Material.AIR || block.getType() == Material.FIRE || isWaterbendable(block)) && isTransparent(block)) {
  ...
  @Override public boolean allowBreakPlants() {
    return false;
  }
