...
public class AirBlast extends AirAbility implements Attributable {
  ...
  private static void playOriginEffect(Player player) {
    ...
    if (bPlayer == null || player.isDead() || !player.isOnline()) {
      ...
      if (!origin.getWorld().equals(player.getWorld())) {
        ...
        if (!bPlayer.canBendIgnoreCooldowns(getAbility("AirBlast"))) {
          ORIGINS.remove(player);
          return;
        } else {
          if (origin.distanceSquared(player.getEyeLocation()) > getSelectRange() * getSelectRange()) {
            ORIGINS.remove(player);
            return;
          }
        }
  ...
  private void affect(Entity entity) {
    ...
    if (!isUser || isFromOtherOrigin) {
      ...
      if (source != null && (this.damage > 0 && entity instanceof LivingEntity && !entity.equals(player) && !affectedEntities.contains(entity))) {
        ...
        if (source == null && (damage > 0 && entity instanceof LivingEntity && !entity.equals(player) && !affectedEntities.contains(entity))) {
  ...
  @SuppressWarnings(value = { "deprecation" }) @Override public void progress() {
    ...
    for (Block testblock : GeneralMethods.getBlocksAroundPoint(location, radius)) {
      ...
      if ((block.getType() == Material.LEVER) && !affectedLevers.contains(block) && canFlickLevers) {
        ...
        if ((block.getType() == Material.STONE_BUTTON) && !affectedLevers.contains(block) && canPressButtons) {
          ...
          if ((block.getType() == Material.WOOD_BUTTON) && !affectedLevers.contains(block) && canPressButtons) {
  ...
  /**
	 ...
	 */
  @Deprecated public static boolean removeAirBlastsAroundPoint(Location location, double radius) {
  ...
  @Override public void registerAttributes() {
