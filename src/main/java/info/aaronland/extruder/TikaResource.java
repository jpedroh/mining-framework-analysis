...
@Path(value = "/tika") @Produces(value = { MediaType.TEXT_HTML + "; charset=UTF-8", MediaType.APPLICATION_JSON }) public class TikaResource {
  private static final Logger LOGGER = LoggerFactory.getLogger(TikaResource.class);

  @GET public Response extrudeThisUrl(@QueryParam(value = "url") String uri) {
    URL url;
    Document doc;
    DocumentView view;
    try {
  ...
  @POST @Consumes(value = MediaType.MULTIPART_FORM_DATA) public Response extrudeThisFile(FormDataMultiPart formParams) {
