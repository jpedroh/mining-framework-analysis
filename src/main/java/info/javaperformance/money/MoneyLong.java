...
class MoneyLong extends AbstractMoney {
  ...
  public Money add(final MoneyLong other) {
    ...
    if (m_precision == other.m_precision) {
      ...
      if (m_precision > other.m_precision) {
        ...
        if (mult / multiplier != other.m_units) {
  ...
  @Override protected int compareTo(final MoneyLong other) {
    if (m_precision == other.m_precision) {
  ...
  public Money multiply(final double multiplier) {
    final double unscaledRes = m_units * multiplier;
    ...
    return MoneyFactory.fromBigDecimal(toBigDecimal().multiply(new BigDecimal(multiplier, MathContext.DECIMAL64), MathContext.DECIMAL64));
  }
