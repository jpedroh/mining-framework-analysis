...
public class LumenTypeVisitor implements ASTVisitor {
  ...
  @Override public void visitExpression(Expression expr) {
  ...
  private void handleRoot(Expression expr) {
    ...
    if (expr instanceof StaticField) {
      ...
      if (expr instanceof StaticMethodCall) {
        ...
        call.setRef(new MethodReference.Static(call.getResolvedType().getInternalName(), methodName, methodType));
      }
    }
  }
  ...
  private void handleMethod(MethodCallExpr call, ClassData cls, String className) {
    ...
    if (method == null) {
      throw new RuntimeException("Unknown method for types");
    }
