...
public class Parser {
  public static final String OBJECT_CLASS_INTERNAL_NAME = org.objectweb.asm.Type.getType(Object.class).getInternalName();
  ...
  public ProgramNode parseMain() {
    ...
    while (token.getType() != EOF) {
      if (token.getType() == IMPORT_KEYWORD) {
        ...
        if (token.getType() == CLASS_KEYWORD || token.hasAttribute(Attribute.ACC_MOD)) {
          if (classNode != null) {
            throw new RuntimeException("Previous class definition found.");
          }
          Modifier mod = Modifier.PUBLIC;
          if (token.getType() != CLASS_KEYWORD) {
            mod = token.getType().toModifier();
            next().expectType(CLASS_KEYWORD);
          }
  ...
  private IfStmt parseIfStatement() {
    ...
    while (accept(ELSE_KEYWORD)) {
      if (accept(IF_KEYWORD)) {
        IfStmt.ElseIf elseIf = new IfStmt.ElseIf(parseExpression(), parseBody());
        elseIfs.add(elseIf);
      } else {
        if (elseBody != null) {
          throw new RuntimeException("There is already an else branch");
        }
        elseBody = parseBody();
      }
    }
  ...
  private Expression parseRelational() {
    ...
    Type peek = peek().getType();
    if (peek == LT || peek == GT || peek == LTE || peek == GTE || peek == INSTANCEOF_KEYWORD || peek == NOT_INSTANCEOF_KEYWORD) {
      next();
      if (peek == INSTANCEOF_KEYWORD) {
        String type = next().expectType(IDENTIFIER).getContent();
        return new InstanceofExpr(left, type);
      } else {
        if (peek == NOT_INSTANCEOF_KEYWORD) {
          String type = next().expectType(IDENTIFIER).getContent();
    ...
    if (token instanceof StringToken) {
      ...
      if (token.getType() == Type.NUMBER) {
        ...
        if (token.getType() == Type.TRUE) {
          ...
          if (token.getType() == Type.FALSE) {
            next();
            expr = new FalseExpr();
          } else {
            if (token.getType() == Type.NULL) {
              next();
              expr = new NullExpr();
            } else {
              if (token.getType() == Type.L_PAREN) {
                next();
