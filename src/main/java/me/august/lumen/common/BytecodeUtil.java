...
public final class BytecodeUtil implements Opcodes {
  public static final String[] LUMEN_PRIMITIVES = { "int", "byte", "boolean", "bool", "long", "char", "double", "float", "void" };

  public static final Set<String> LUMEN_PRIMITIVES_SET = new HashSet<>(Arrays.asList(LUMEN_PRIMITIVES));
  ...
  @Deprecated public static String[] splitTypes(String desc) {
  ...
  @Deprecated public static String toType(Class<?> cls) {
    switch (cls.getName()) {
      case "boolean":
      return "Z";
      case "byte":
  ...
  static {
    Class[] classes = new Class[] { Byte.class, Character.class, Short.class, Integer.class, Long.class, Float.class, Double.class };
  ...
  public static boolean isIntegral(Type type) {
    return (type.getSort() >= Type.BYTE && type.getSort() <= Type.INT) || type.getSort() == Type.LONG;
  }
  ...
  public static boolean canWidenTo(Type orig, Type target) {
    if (!isNumeric(orig) || !isNumeric(target)) {
      return false;
    }
  ...
  public static void concatStringsBytecode(MethodVisitor method, BuildContext ctx, Expression... exprs) {
    ...
    method.visitMethodInsn(INVOKESPECIAL, SB_TYPE.getInternalName(), "<init>", Type.getMethodType(Type.VOID_TYPE).getDescriptor(), false);
    for (Expression expr : exprs) {
      Type type = Type.getMethodType(SB_TYPE, expr.expressionType());
      expr.generate(method, ctx);
      method.visitMethodInsn(INVOKEVIRTUAL, SB_TYPE.getInternalName(), "append", type.getDescriptor(), false);
    }
    method.visitMethodInsn(INVOKEVIRTUAL, SB_TYPE.getInternalName(), "toString", Type.getMethodType(STRING_TYPE).getDescriptor(), false);
  }
