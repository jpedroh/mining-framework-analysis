...
public abstract class VestApplication extends Application {
  ...
  public VestApplication() {
    TypeParser typeParser = TypeParser.newBuilder().unregisterParser(Object.class).unregisterParser(File.class).build();
  ...
  public <T extends java.lang.Object> T getSingleton(Class<? extends T> type) {
      ...
      try {
        for (JavaFileObject javaObject : helper.find(packageName)) {
          try {
            ...
            final String clazzName = clazzUriString.substring(clazzUriString.lastIndexOf(folderLocationName), clazzUriString.lastIndexOf('.')).replaceAll("/", ".");
            ...
            for (Annotation annotation : annotations) {
              if (annotation.annotationType().equals(Path.class)) {
                ...
                if (annotation.annotationType().equals(Provider.class)) {
                  ...
                  if (annotation.annotationType().equals(Priority.class)) {
  ...
  private void addProvider(Integer priority, Class<?> providerClass) {
    priority = (Integer) ObjectUtils.defaultIfNull(priority, DEFAULT_PROVIDER_PRIORITY);
  ...
  /**
	 ...
	 */
  public <T extends java.lang.Object> List<T> getProviders(Class<T> clazz) {
      ...
      for (Entry<Integer, List<Class<?>>> providersEntry : providerClasses.entrySet()) {
        for (Class<?> providerClass : providersEntry.getValue()) {
          if (clazz.isAssignableFrom(providerClass)) {
            try {
              @SuppressWarnings(value = { "unchecked" }) T newInstance = (T) providerClass.newInstance();
    ...
    @SuppressWarnings(value = { "unchecked" }) List<T> providers = (List<T>) providersCache.get(clazz);
    return providers;
  }
