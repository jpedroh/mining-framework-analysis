...
public class DjService {
  ...
  private void moveLegacySongsJsonToDb() {
    ...
    logger.info("Found old-style song queue files at \'" + queueHistoryFilePath + "\' and \'" + unplayedSongsFilePath + "\'. We\'re adding them to a database and then renaming the files to end in .old");
    try {
  ...
  public synchronized void irc_volume(String sender, String newVolStr) {
    ...
    if ("up".equalsIgnoreCase(newVolStr)) {
      ...
      if ("down".equalsIgnoreCase(newVolStr)) {
        newVolume = oldVolume - 10;
      } else {
        try {
  ...
  public synchronized void irc_songRequest(String sender, String requestStr) {
    ...
    if (requestStr.trim().charAt(0) == '/') {
      requestStr = requestStr.replaceFirst("/", "");
    }
  ...
  private void appendSongReportEntry(StringBuilder sb, SongEntry song) {
    sb.append(song.buildYoutubeUrl()).append(" \"").append(song.getTitle()).append("\", requested by ").append(song.getUser()).append(", id: ").append(song.getRequestId());
  }
  ...
  @Nullable public synchronized SongEntry nextSong() {
    if (currentSong != null) {
