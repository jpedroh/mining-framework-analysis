...
public final class TestResult extends MetaTabulatedResult {
  ...
  public void parse(long buildTime, File baseDir, String[] reportFiles) throws IOException {
    ...
    if (!parsed) {
      ...
      throw new AbortException(String.format("Test reports were found but none of them are new. Did tests run? %n" + "For example, %s is %s old%n", f, Util.getTimeSpanString(buildTime - f.lastModified())));
    }
  }
  ...
  private void parsePossiblyEmpty(File reportFile) throws IOException {
    if (reportFile.length() == 0) {
      ...
      sr.addCase(new CaseResult(sr, "[empty]", "Test report file " + reportFile.getAbsolutePath() + " was length 0", null));
      add(sr);
    } else {
      parse(reportFile);
    }
