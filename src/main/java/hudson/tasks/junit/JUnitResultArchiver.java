...
public class JUnitResultArchiver extends Recorder implements SimpleBuildStep, JUnitTask {
  ...
  private static TestResult parse(@Nonnull JUnitTask task, @CheckForNull String nodeId, List<String> enclosingBlocks, String expandedTestResults, Run<?, ?> run, @Nonnull FilePath workspace, Launcher launcher, TaskListener listener) throws IOException, InterruptedException {
...
<<<<<<< /usr/src/app/output/jenkinsci/junit-plugin/cfcac8f0c9b228340fa43da976db9470237659fb/src/main/java/hudson/tasks/junit/JUnitResultArchiver.java/left.java
...
>>>>>>> /usr/src/app/output/jenkinsci/junit-plugin/cfcac8f0c9b228340fa43da976db9470237659fb/src/main/java/hudson/tasks/junit/JUnitResultArchiver.java/right.java
  }

  public static TestResultAction parseAndAttach(@Nonnull JUnitTask task, @CheckForNull String nodeId, List<String> enclosingBlocks, Run build, FilePath workspace, Launcher launcher, TaskListener listener) throws InterruptedException, IOException {
  ...
  @Extension public static class DescriptorImpl extends BuildStepDescriptor<Publisher> {
    ...
    public FormValidation doCheckHealthScaleFactor(@QueryParameter double value) {
      ...
      return FormValidation.ok(Messages.JUnitResultArchiver_HealthScaleFactorAnalysis(1, (int) (100.0 - Math.max(0.0, Math.min(100.0, 1 * value))), 5, (int) (100.0 - Math.max(0.0, Math.min(100.0, 5 * value)))));
    }
  }
}