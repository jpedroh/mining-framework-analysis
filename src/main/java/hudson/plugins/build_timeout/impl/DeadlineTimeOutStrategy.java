...
public class DeadlineTimeOutStrategy extends BuildTimeOutStrategy {
  ...
  @DataBoundConstructor public DeadlineTimeOutStrategy(String deadlineTime, int deadlineToleranceInMinutes) {
    ...
    this.deadlineToleranceInMinutes = deadlineToleranceInMinutes <= MINIMUM_DEADLINE_TOLERANCE_IN_MINUTES ? MINIMUM_DEADLINE_TOLERANCE_IN_MINUTES : deadlineToleranceInMinutes;
  }

  @Override public long getTimeOut(@NonNull AbstractBuild<?, ?> build, @NonNull BuildListener listener) throws InterruptedException, MacroEvaluationException, IOException, IllegalArgumentException {
    ...
    if (deadlineTimestamp.compareTo(now) <= 0 && deadlineTimestampWithTolerance.compareTo(now) > 0) {
      listener.getLogger().println(Messages.DeadlineTimeOutStrategy_ImmediatelyTerminate(expandedDeadlineTime, deadlineToleranceInMinutes));
      return 0;
    }
  ...
  @Extension public static class DescriptorImpl extends BuildTimeOutStrategyDescriptor {
    ...
    @Override public boolean isApplicableAsBuildStep() {
      return true;
    }
  }
}