...
public class Publisher extends Recorder implements SimpleBuildStep {
  ...
  /**
    * {@inheritDoc}
    */
  @Override public BuildStepDescriptor<hudson.tasks.Publisher> getDescriptor() {
    return DESCRIPTOR;
  }
  ...
  /**
    * {@inheritDoc}
    */
  @Override public void perform(Run<?, ?> build, FilePath workspace, Launcher launcher, final TaskListener listener) throws InterruptedException, IOException {
      ...
      if (failureOnFailedTestConfig && results.getFailedConfigCount() > 0) {
        ...
        if (thresholdMode == 1) {
          if (results.getFailCount() > failedFails) {
            ...
            if (results.getSkipCount() > failedSkips) {
              ...
              if (results.getFailCount() > unstableFails) {
                ...
                if (results.getSkipCount() > unstableSkips) {
          ...
          if (thresholdMode == 2) {
            ...
            if (failedPercent > failedFails) {
              ...
              if (skipPercent > failedSkips) {
                ...
                if (failedPercent > unstableFails) {
  ...
  public static final class DescriptorImpl extends BuildStepDescriptor<hudson.tasks.Publisher> {
    ...
    @Override public hudson.tasks.Publisher newInstance(@NonNull StaplerRequest req, JSONObject formData) throws FormException {
    ...
    private FormValidation validate(String value) {
      try {
        ...
        if (val > 100) {
          return FormValidation.ok("NOTE: value greater than 100 only make sense when Threshold Mode " + "is set to \'Number of Tests\'");
        }
    ...
    @POST public FormValidation doCheckUnstableSkips(@QueryParameter String value) {
