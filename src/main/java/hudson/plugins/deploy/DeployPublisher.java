...
public class DeployPublisher extends Publisher implements SimpleBuildStep, Serializable {
  ...
  @DataBoundConstructor public DeployPublisher(List<ContainerAdapter> adapters, String war, String contextPath, boolean onFailure) {
  ...
  @Override @Deprecated public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws InterruptedException, IOException {
    perform(false, build, build.getWorkspace(), launcher, listener);
    return true;
  }

  @Override public void perform(@Nonnull Run<?, ?> run, @Nonnull FilePath workspace, @Nonnull Launcher launcher, @Nonnull TaskListener listener) throws InterruptedException, IOException {
    perform(true, run, workspace, launcher, listener);
  }

  private @SuppressFBWarnings(value = "NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE") void perform(boolean fromWorkFlow, @Nonnull Run<?, ?> run, @Nonnull FilePath workspace, @Nonnull Launcher launcher, @Nonnull TaskListener listener) throws InterruptedException, IOException {
  ...
  @Extension public static final class DescriptorImpl extends Descriptor<Publisher> {
