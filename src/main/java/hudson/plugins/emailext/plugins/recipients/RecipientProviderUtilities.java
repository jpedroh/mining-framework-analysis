...
public final class RecipientProviderUtilities {
      ...
      if (run instanceof AbstractBuild<?, ?>) {
        ...
        try {
          ...
          if (List.class.isAssignableFrom(getChangeSets.getReturnType())) {
            @SuppressWarnings(value = { "unchecked" }) List<ChangeLogSet<ChangeLogSet.Entry>> sets = (List<ChangeLogSet<ChangeLogSet.Entry>>) getChangeSets.invoke(run);
  ...
  @SuppressWarnings(value = { "deprecated" }) private static User getByLegacyUserCause(Run<?, ?> run) {
    try {
  ...
  @Deprecated public static void addUsers(final Set<User> users, final TaskListener listener, final EnvVars env, final Set<InternetAddress> to, final Set<InternetAddress> cc, final Set<InternetAddress> bcc, final IDebug debug) {
    addUsers(users, listener, null, env, to, cc, bcc, debug);
  }

  public static void addUsers(final Set<User> users, final ExtendedEmailPublisherContext context, final EnvVars env, final Set<InternetAddress> to, final Set<InternetAddress> cc, final Set<InternetAddress> bcc, final IDebug debug) {
  ...
  private static void addUsers(final Set<User> users, final TaskListener listener, @CheckForNull Run<?, ?> run, final EnvVars env, final Set<InternetAddress> to, final Set<InternetAddress> cc, final Set<InternetAddress> bcc, final IDebug debug) {
          ...
          if (Jenkins.getActiveInstance().isUseSecurity()) {
            try {
              ...
              if (SEND_TO_UNKNOWN_USERS) {
                ...
                listener.getLogger().printf("\' which your security realm does not recognize; you may need" + " changes in your SCM plugin%n");
                continue;
              }
            }
          }
