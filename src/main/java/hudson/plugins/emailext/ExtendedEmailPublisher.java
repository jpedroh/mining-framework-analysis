...
public class ExtendedEmailPublisher extends Notifier implements MatrixAggregatable {
  ...
  @NonNull @Override public Collection<? extends Action> getProjectActions(AbstractProject<?, ?> project) {
  ...
  @Override public boolean prebuild(AbstractBuild<?, ?> build, BuildListener listener) {
  ...
  @SuppressFBWarnings(value = "REC_CATCH_EXCEPTION") boolean sendMail(ExtendedEmailPublisherContext context) {
    try {
      ...
      if (!mailAccount.isValid()) {
        if (!mailAccount.isFromAddressValid()) {
          ...
          if (mailAccount.isDefaultAccount()) {
            ...
            debug(context.getListener().getLogger(), "Additional account has invalid from address " + mailAccount.getAddress());
          }
        } else {
          if (!mailAccount.isSmtpServerValid()) {
            ...
            if (mailAccount.isDefaultAccount()) {
              ...
              debug(context.getListener().getLogger(), "Additional account " + mailAccount.getAddress() + " has invalid SMTP server");
            }
          }
        }
        return false;
          ...
          while (true) {
            try {
              ...
              if (e.getNextException() != null && (e.getNextException() instanceof SocketException || e.getNextException() instanceof ConnectException)) {
                ...
                while (next != null) {
                  debug(context.getListener().getLogger(), "Next " + next.getClass().getSimpleName() + " message: " + next.getMessage());
  ...
  @Restricted(value = NoExternalUse.class) MailAccount getMailAccount(ExtendedEmailPublisherContext context) throws AddressException {
    ExtendedEmailPublisherDescriptor descriptor = getDescriptor();
    if (StringUtils.isBlank(from)) {
      debug(context.getListener().getLogger(), "Sending mail from default account using System Admin e-mail address");
    ...
    for (MailAccount addAccount : descriptor.getAddAccounts()) {
      ...
      if (!addAccount.isValid()) {
        if (!addAccount.isFromAddressValid()) {
          debug(context.getListener().getLogger(), "Ignoring additional account " + "with invalid from address " + addAccount.getAddress());
        } else {
          if (!addAccount.isSmtpServerValid()) {
            debug(context.getListener().getLogger(), "Ignoring additional account " + addAccount.getAddress() + " with invalid SMTP server");
          }
        }
        continue;
      }
      if (!addAccount.getAddress().equalsIgnoreCase(fromAddress.getAddress())) {
        debug(context.getListener().getLogger(), "Ignoring valid additional account " + addAccount.getAddress() + " because it is not a match for " + from);
        continue;
      }
  ...
  private MimeMessage createMail(ExtendedEmailPublisherContext context, InternetAddress fromAddress, Session session) throws MessagingException, UnsupportedEncodingException {
    ExtendedEmailPublisherDescriptor descriptor = getDescriptor();
  ...
  @Override public ExtendedEmailPublisherDescriptor getDescriptor() {
  ...
  public MatrixAggregator createAggregator(MatrixBuild matrixbuild, Launcher launcher, BuildListener buildlistener) {
    return new MatrixAggregator(matrixbuild, launcher, buildlistener) {
      @Override public boolean endBuild() {
      ...
      @Override public boolean startBuild() {
        LOGGER.log(Level.FINER, "end build of {0}", this.build.getDisplayName());
        if (getMatrixTriggerMode().forParent) {
  ...
  public Object readResolve() {
    if (Jenkins.get().isUseSecurity() && (!StringUtils.isBlank(this.postsendScript) || !StringUtils.isBlank(this.presendScript))) {
