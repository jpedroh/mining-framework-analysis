...
public class CreateTriggerParser {
  /**
     ...
     */
  public static void parse(final PgDatabase database, final String statement, final boolean ignoreSlonyTriggers) {
    ...
    if (parser.expectOptional("BEFORE")) {
      ...
      if (parser.expectOptional("AFTER")) {
    ...
    while (true) {
      if (!first && !parser.expectOptional("OR")) {
        ...
        if (parser.expectOptional("INSERT")) {
          ...
          if (parser.expectOptional("UPDATE")) {
            ...
            if (parser.expectOptional("DELETE")) {
              trigger.setOnDelete(true);
            } else {
              if (parser.expectOptional("TRUNCATE")) {
    ...
    if (parser.expectOptional("FOR")) {
      ...
      if (parser.expectOptional("ROW")) {
        ...
        if (parser.expectOptional("STATEMENT")) {
    ...
    final boolean ignoreSlonyTrigger = ignoreSlonyTriggers && ("_slony_logtrigger".equals(trigger.getName()) || "_slony_denyaccess".equals(trigger.getName()));
    if (!ignoreSlonyTrigger) {
      final PgSchema schema = database.getSchema(ParserUtils.getSchemaName(relationName, database));
  ...
  public static void parseDisable(final PgDatabase database, final String statement) {
    final Parser parser = new Parser(statement);
    ...
    final PgSchema schema = database.getSchema(ParserUtils.getSchemaName(tableName, database));
