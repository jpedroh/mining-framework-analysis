  ...
  private boolean canRemoveSocket(BlockConnector socket) {
    ...
    for (int i = 0; i < total; i++) {
      ...
      if (conn == socket) {
        ...
        if (conn.getPositionType().equals(socket.getPositionType()) && conn.isExpandable() == socket.isExpandable() && conn.initKind().equals(socket.initKind()) && conn.getExpandGroup().equals(socket.getExpandGroup())) {
          if (first == -1) {
            first = i;
          } else {
            return true;
      ...
      for (int j = 0; j < stubChildren.getLength(); j++) {
        ...
        if (infoNode.getNodeName().equals("StubParentName")) {
          ...
          if (infoNode.getNodeName().equals("StubParentGenus")) {
            ...
            if (infoNode.getNodeName().equals("Block")) {
              blockNode = infoNode;
            }
          }
        }
                ...
                if (child.getNodeName().equals("AfterBlockId")) {
                  ...
                  if (child.getNodeName().equals("Plug")) {
                    ...
                    if (child.getNodeName().equals("Sockets")) {
                      ...
                      if (child.getNodeName().equals("LangSpecProperties")) {
