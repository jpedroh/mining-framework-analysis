...
public class KeyInputMap {
  ...
  protected static void enableDefaultKeyMapping(boolean enable) {
    ...
    char[] key = { KeyEvent.VK_0, KeyEvent.VK_1, KeyEvent.VK_2, KeyEvent.VK_3, KeyEvent.VK_4, KeyEvent.VK_5, KeyEvent.VK_6, KeyEvent.VK_7, KeyEvent.VK_8, KeyEvent.VK_9, '+', '>', '<', '*', '/', '=', '\"', 'x', 'X', '|', '!', '&', '^' };
    String[][] value = { { "number", "0" }, { "number", "1" }, { "number", "2" }, { "number", "3" }, { "number", "4" }, { "number", "5" }, { "number", "6" }, { "number", "7" }, { "number", "8" }, { "number", "9" }, { "sum", null }, { "greaterthan", null }, { "lessthan", null }, { "product", null }, { "quotient", null }, { "equals", null }, { "string", null }, { "product", null }, { "product", null }, { "or", null }, { "not", null }, { "and", null }, { "power", null } };
  ...
  public static void processKeyChar(Workspace workspace, KeyEvent key) {
    ...
    if (KeyInputMap.DEFAULT_ENABLED) {
      ...
      for (Character keyChar : KeyInputMap.defaultInputMap.keySet()) {
        if (keyChar.equals(key.getKeyChar())) {
          workspace.getTypeBlockManager().automateBlockInsertion(workspace, KeyInputMap.defaultInputMap.get(keyChar)[0], KeyInputMap.defaultInputMap.get(keyChar)[1]);
          return;
        }
      }
    }
    for (Character keyChar : KeyInputMap.customInputMap.keySet()) {
      if (keyChar.equals(key.getKeyChar())) {
        workspace.getTypeBlockManager().automateBlockInsertion(workspace, KeyInputMap.customInputMap.get(keyChar)[0], KeyInputMap.customInputMap.get(keyChar)[1]);
        return;
      }
    }
