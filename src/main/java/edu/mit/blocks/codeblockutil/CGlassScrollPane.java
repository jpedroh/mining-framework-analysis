...
public class CGlassScrollPane extends CScrollPane implements KeyListener {
  ...
  public CGlassScrollPane(JComponent view) {
    this(view, ScrollPolicy.VERTICAL_BAR_ALWAYS, ScrollPolicy.HORIZONTAL_BAR_ALWAYS);
  }
  ...
  public CGlassScrollPane(JComponent view, int thumbWidth, Color thumbColor, Color trackColor) {
    this(view, ScrollPolicy.VERTICAL_BAR_ALWAYS, ScrollPolicy.HORIZONTAL_BAR_ALWAYS, thumbWidth, thumbColor, trackColor);
  }
  ...
  /**
     ...
     */
  public CGlassScrollPane(JComponent view, ScrollPolicy verticalPolicy, ScrollPolicy horizontalPolicy, int thumbWidth, Color thumbColor, Color trackColor) {
    super();
    this.setLayout(null);
    ...
    scrollviewport = new JScrollPane(view, ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER) {
    ...
    verticalbar = new GlassVerticalBar(thumbWidth, thumbColor, trackColor, scrollviewport.getVerticalScrollBar().getModel(), verticalPolicy);
    horizontalbar = new GlassHorizontalBar(thumbWidth, thumbColor, trackColor, scrollviewport.getHorizontalScrollBar().getModel());
  ...
  public void repositionComponents() {
    ...
    if (this.vpolicy.equals(ScrollPolicy.VERTICAL_BAR_ALWAYS) || this.vpolicy.equals(ScrollPolicy.VERTICAL_BAR_AS_NEEDED)) {
      verticalbar.setBounds(this.getWidth() - this.thumbWidth, 7, this.thumbWidth, this.getHeight() - this.thumbWidth - 7);
    } else {
      verticalbar.setBounds(this.getWidth() - this.thumbWidth, 7, 0, 0);
    }
    if (this.hpolicy.equals(ScrollPolicy.HORIZONTAL_BAR_ALWAYS)) {
      horizontalbar.setBounds(7, this.getHeight() - this.thumbWidth, this.getWidth() - this.thumbWidth - 7, this.thumbWidth);
    } else {
      if (this.hpolicy.equals(ScrollPolicy.HORIZONTAL_BAR_AS_NEEDED)) {
        if (this.getWidth() < this.scrollviewport.getViewport().getView().getWidth() - 2) {
          horizontalbar.setBounds(7, this.getHeight() - this.thumbWidth, this.getWidth() - this.thumbWidth - 7, this.thumbWidth);
        } else {
          horizontalbar.setBounds(7, this.getHeight() - this.thumbWidth, 0, 0);
        }
      }
    }
    this.revalidate();
  ...
  /**
     ...
     */
  @Override public BoundedRangeModel getVerticalModel() {
  ...
  /**
     ...
     */
  @Override public BoundedRangeModel getHorizontalModel() {
  ...
  /**
     ...
     */
  @Override public void setScrollingUnit(int x) {
  ...
  @Override public void mouseWheelMoved(MouseWheelEvent e) {
    if (e.isShiftDown()) {
      scrollviewport.getHorizontalScrollBar().getModel().setValue(scrollviewport.getHorizontalScrollBar().getModel().getValue() + e.getUnitsToScroll() * e.getScrollAmount() * SCROLLINGUNIT);
      horizontalbar.repaint();
    } else {
      scrollviewport.getVerticalScrollBar().getModel().setValue(scrollviewport.getVerticalScrollBar().getModel().getValue() + e.getUnitsToScroll() * e.getScrollAmount() * SCROLLINGUNIT);
      verticalbar.repaint();
    }
  }
...
class GlassVerticalBar extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
  ...
  @Override public void paint(Graphics g) {
    ...
    if (!vpolicy.equals(CScrollPane.ScrollPolicy.VERTICAL_BAR_AS_NEEDED) || (modelrange.getMaximum() > this.getHeight() + 7 + this.getWidth())) {
      ...
      g2.setPaint(new GradientPaint(0, 0, this.thumbColor, this.getWidth() + 10, 0, Color.black, true));
