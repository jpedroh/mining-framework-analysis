...
@NotThreadSafe public class DataPrep {
  ...
  public static class SpanAndParseIdx {
    ...
    @Override public boolean equals(Object other) {
      if (other == null || !(other instanceof SpanAndParseIdx)) {
        return false;
      }
  ...
  public DataPrep(List<String> tagLines, List<String> feLines, String spanFilename, Map<String, Integer> featureIndex) throws IOException {
    this.featureIndex = featureIndex;
    new FileOutputStream(new File(spanFilename), false).close();
  ...
  public List<int[][]> getTrainData(String feline, List<SpanAndParseIdx> candidateSpans, Sentence sentence, String spanFilename) throws IOException {
    ...
    for (int i = 0; i < dataPoint.getNumOvertFrameElementFillers(); i++) {
      ...
      allFeaturesAndSpanLines.add(getFeaturesForOneArgument(dataPoint, frame, frameElement, goldSpan, candidateSpans));
    }
  ...
  Pair<List<int[]>, List<String>> getFeaturesForOneArgument(DataPointWithFrameElements dp, String frame, String fe, Range0Based goldSpan, List<SpanAndParseIdx> candidateSpans) {
    final List<int[]> features = Lists.newArrayList();
    final List<String> spanLines = Lists.newArrayList();
    spanLines.add(Joiner.on("\t").join(dp.getSentenceNum(), fe, frame, dp.getTargetTokenIdxs()[0], dp.getTargetTokenIdxs()[dp.getTargetTokenIdxs().length - 1], feIndex));
    final List<SpanAndParseIdx> goldFirst = Lists.newArrayListWithCapacity(candidateSpans.size());
  ...
  int[] getFeatureIndexes(DataPointWithFrameElements dataPoint, String frame, String fe, Range0Based span, DependencyParse parse) {
    final Set<String> featureSet = new FeatureExtractor().extractFeatures(dataPoint, frame, fe, span, parse).elementSet();
