...
public class FrameIdentificationGoldTargets {
  public static void main(String[] args) throws IOException, ClassNotFoundException {
    ...
    ArrayList<String> inputForFrameId = getGoldSeg(segs, start, end);
    ArrayList<String> idResult = new ArrayList<String>();
    final Pair<IdFeatureExtractor, TObjectDoubleHashMap<String>> extractorAndParams = FrameIdentificationRelease.parseParamFile(options.idParamFile.get());
    ...
    if (usegraph) {
      ...
      idModel = new GraphBasedFrameIdentifier(featureExtractor, frameMap.keySet(), cMap, paramList, sg);
    } else {
      idModel = new FastFrameIdentifier(featureExtractor, paramList, frameMap.keySet(), cMap);
    }
    System.out.println("Start Time:" + (new Date()));
    for (String input : inputForFrameId) {
      String[] toks = input.split("\t");
      int sentNum = Integer.parseInt(toks[2]);
      ...
      idResult.add(1 + "\t" + bestFrame + "\t" + split[0] + "\t" + toks[1] + "\t" + split[1] + "\t" + sentCount);
