...
public class PrepareFullAnnotationJson {
  ...
  /**
	 ...
	 */
  public static void writeJsonForPredictions(String testFEPredictionsFile, String testTokenizedFile, String outputFile) throws Exception {
  ...
  public static void writeJsonForPredictions(Reader tokenizedInput, Reader frameElementsInput, Writer output) throws IOException {
    final List<String> tokenizedLines = readLines(tokenizedInput);
    final Multimap<Integer, RankedScoredRoleAssignment> predictions = parseRoleAssignments(readLines(frameElementsInput));
  ...
  private static void writeJson(Multimap<Integer, RankedScoredRoleAssignment> predictions, List<String> tokenizedLines, Writer output) throws IOException {
  ...
  /**
	 ...
	 */
  public static SemaforParseResult getSemaforParse(Collection<RankedScoredRoleAssignment> rankedScoredRoleAssignments, List<String> tokens) {
    final ArrayList<Frame> frames = Lists.newArrayList();
    final ImmutableListMultimap<Range0Based, RankedScoredRoleAssignment> predictionsByFrame = Multimaps.index(rankedScoredRoleAssignments, getTargetSpan);
  ...
  private static NamedSpanSet makeSpan(int start, int end, String name, List<String> tokens) {
    final ImmutableList<Span> spans = ImmutableList.of(new Span(start, end, Joiner.on(" ").join(tokens.subList(start, end))));
