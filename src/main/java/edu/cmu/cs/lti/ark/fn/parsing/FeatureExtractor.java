...
public class FeatureExtractor {
  ...
  /**
	 ...
	 */
  protected static void conjoinAndAdd(String featureName, String frameAndRoleName, String roleName, ConjoinLevel level, Multiset<String> featureMap) {
    switch (level) {
  ...
  public Multiset<String> extractFeatures(DataPointWithFrameElements dp, String frameName, String roleName, final Range0Based fillerSpanRange, DependencyParse parse) {
...
<<<<<<< /usr/src/app/output/sammthomson/semafor/1b62f9ce9b3c32ea3e1c737df5c9acf3c23d27e1/src/main/java/edu/cmu/cs/lti/ark/fn/parsing/FeatureExtractor.java/left.java
    fillerSpanRange.equals(EMPTY_SPAN())
=======
    isEmptySpan(fillerSpanRange)
>>>>>>> /usr/src/app/output/sammthomson/semafor/1b62f9ce9b3c32ea3e1c737df5c9acf3c23d27e1/src/main/java/edu/cmu/cs/lti/ark/fn/parsing/FeatureExtractor.java/right.java
    ...
    conjoinAndAdd(overtness, frameAndRoleName, roleName, FRAME_AND_ROLE_NAME, featureMap);
    ...
    conjoinAndAdd(nullness + "NCHILDREN_" + tgtChildren.size(), frameAndRoleName, roleName, FRAME_AND_ROLE_NAME, featureMap);
    String dsubcat = "";
    ...
    if (!isEmpty) {
      ...
      final List<Pair<String, DependencyParse>> targetToFillerPath = DependencyParse.getPath(targetHeadNode, fillerHeadNode);
      ...
      if (isOverlap(spanStart, spanEnd, targetStart, targetEnd)) {
        ...
        if (targetTokenNums.length > 1) {
          if ((spanStart < targetStart && spanEnd < targetEnd) || (spanStart > targetStart && spanEnd > targetEnd)) {
      ...
      if (isClosedClass(nodes[startNode].getPOS())) {
        conjoinAndAdd("w[0]pos[0]_" + nodes[startNode].getWord() + " " + nodes[startNode].getPOS(), frameAndRoleName, roleName, FRAME_AND_ROLE_NAME, featureMap);
      }
      ...
      if (endNode - startNode > 0) {
        if (isClosedClass(nodes[startNode + 1].getPOS())) {
          conjoinAndAdd("w[1]pos[1]_" + nodes[startNode + 1].getWord() + " " + nodes[startNode + 1].getPOS(), frameAndRoleName, roleName, FRAME_AND_ROLE_NAME, featureMap);
        }
        ...
        if (endNode - startNode > 1) {
          if (isClosedClass(nodes[endNode].getPOS())) {
            conjoinAndAdd("w[-1]pos[-1]_" + nodes[endNode].getWord() + " " + nodes[endNode].getPOS() + "_", frameAndRoleName, roleName, FRAME_AND_ROLE_NAME, featureMap);
          }
          conjoinAndAdd("dep[-1]_" + nodes[endNode].getLabelType() + "_", frameAndRoleName, roleName, FRAME_AND_ROLE_NAME, featureMap);
        }
      }
  ...
  public static boolean isContentPOS(String pos) {
    return (pos.startsWith("N") || pos.startsWith("V") || pos.startsWith("A") || pos.startsWith("R"));
  }
  ...
  /**
	 ...
	 */
  private void extractChildPOSFeatures(Multiset<String> featureMap, DataPointWithFrameElements dp, DependencyParse[] nodes, final Range0Based fillerSpanRange, String frameAndRoleName, String roleName) {
