...
public class Semafor {
  ...
  public static void main(String[] args) throws Exception {
    ...
    semafor.runParser(Files.newReaderSupplier(inputFile, Charsets.UTF_8), Files.newWriterSupplier(outputFile, Charsets.UTF_8), numThreads);
  }

  public Semafor(Set<String> allRelatedWords, FEDict frameElementsForFrame, RoteSegmenter segmenter, GraphBasedFrameIdentifier idModel, Decoding decoder, Map<String, Integer> argIdFeatureIndex) {
  ...
  public static Semafor getSemaforInstance(String modelDirectory) throws IOException, ClassNotFoundException, URISyntaxException {
  ...
  /**
	 ...
	 */
  public void runParser(final InputSupplier<? extends Readable> inputSupplier, final OutputSupplier<? extends Writer> outputSupplier, final int numThreads) throws IOException, InterruptedException {
    final BlockingQueue<Future<Optional<SemaforParseResult>>> results = Queues.newLinkedBlockingDeque(5 * numThreads);
  ...
  public SemaforParseResult predictArguments(Sentence sentence, List<Pair<List<Integer>, String>> idResults) throws IOException {
  ...
  public List<String> predictArgumentLines(Sentence sentence, List<String> idResult, int kBest) throws IOException {
    ...
    for (String feLine : idResult) {
      ...
      final List<Range0Based> spans = spanPruner.candidateSpans(sentence, new Range0Based(targetStartTokenIdx, targetEndTokenIdx));
      ...
      for (String frameElement : frameElements) {
        ...
        for (Range0Based span : spans) {
          final Multiset<String> featureSet = featureExtractor.extractFeatures(dataPoint, frame, frameElement, span, parse);
      ...
      frameFeaturesList.add(new FrameFeatures(frame, targetStartTokenIdx, targetEndTokenIdx, frameElements, featuresAndSpanByArgument));
    }
