...
public class TrainBatch {
  ...
  public static void main(String[] args) throws Exception {
    ...
    final int numThreads = options.numThreads.present() ? options.numThreads.get() : Runtime.getRuntime().availableProcessors();
    final TrainBatch tbm = new TrainBatch(options.alphabetFile.get(), options.eventsFile.get(), options.modelFile.get(), options.reg.get(), options.l2Strength.get(), restartFile.equals("null") ? Optional.<String>absent() : Optional.of(restartFile), numThreads, options.usePartialCredit.get(), options.costMultiple.present() ? (float) options.costMultiple.get() : DEFAULT_COST_MULTIPLE);
    tbm.trainModel();
  }

  public TrainBatch(String alphabetFile, String eventsDir, String modelFile, String reg, double lambda, Optional<String> restartFile, int numThreads, boolean usePartialCreditCosts, float costMultiple) throws IOException {
  ...
  private Pair<Double, double[]> getValuesAndGradientsThreaded(final double[] currentParams) {
    ...
    double negLogLikelihood = -logLikelihood;
    timesEquals(gradients, -1.0);
  ...
  private double processBatch(Iterable<Integer> targetIdxs, double[] currentParams, double[] tGradients) throws Exception {
    ...
    for (int targetIdx : targetIdxs) {
      ...
      logLikelihood += addLogLossAndGradientForExample(getFeaturesForTarget(targetIdx), currentParams, tGradients);
      if (targetIdx % 100 == 0) {
  ...
  private double addLogLossAndGradientForExample(FeaturesAndCost[] featuresByFrame, double[] currentParams, double[] gradient) {
