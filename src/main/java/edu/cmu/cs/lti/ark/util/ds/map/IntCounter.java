 ...
 */
@NotThreadSafe public class IntCounter<T extends java.lang.Object> implements ICounter<T, Integer>, java.io.Serializable {
  ...
  @SuppressWarnings(value = { "unchecked" }) public Integer get(Object key) {
    return getT((T) key);
  }
  ...
  public void incrementAllBy(int delta) {
    for (TObjectIntIterator<T> iter = getIterator(); iter.hasNext(); ) {
      iter.advance();
  ...
  public void incrementAllBy(IntCounter<? extends T> counts) {
    for (TObjectIntIterator<? extends T> iter = counts.getIterator(); iter.hasNext(); ) {
      iter.advance();
  ...
  /** 
	 ...
	 */
  public static <T extends java.lang.Object> IntCounter<T> or(Collection<IntCounter<? extends T>> counters) {
    IntCounter<T> result = new IntCounter<T>();
    for (IntCounter<? extends T> counter : counters) {
      for (TObjectIntIterator<? extends T> iter = counter.getIterator(); iter.hasNext(); ) {
        iter.advance();
        if (iter.value() != 0) {
  ...
  public IntCounter<T> andWith(IntCounter<T> that) {
    IntCounter<T> result = new IntCounter<T>();
    for (TObjectIntIterator<T> iter = this.getIterator(); iter.hasNext(); ) {
      iter.advance();
  ...
  @SuppressWarnings(value = { "unchecked" }) public Set<T> keySet() {
  ...
  @SuppressWarnings(value = { "unchecked" }) public boolean containsKey(Object key) {
  ...
  @Override public Set<java.util.Map.Entry<T, Integer>> entrySet() {
  ...
  @Override public void putAll(Map<? extends T, ? extends Integer> m) {
