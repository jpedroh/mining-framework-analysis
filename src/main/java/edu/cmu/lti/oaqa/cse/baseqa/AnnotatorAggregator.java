...
public class AnnotatorAggregator extends AbstractLoggedComponent {
  ...
  private static String FormatErrorMsg = "Each entry in " + ElementNames + " must have exactly one element in the form: " + " -" + InheritKeyword + ": <name>";
   ...
   */
  @Override public final void process(JCas jcas) throws AnalysisEngineProcessException {
    super.process(jcas);
    try {
  ...
  private void ReadElements(UimaContext context) throws ResourceInitializationException {
    ...
    if (elementNames != null) {
      ...
      if (elementNames instanceof String) {
        ...
        try {
          @SuppressWarnings(value = { "unchecked" }) List<Map<String, String>> elems = (List<Map<String, String>>) obj;
        ...
        throw new ResourceInitializationException(new Exception("Parameter: " + ElementNames + " should be a string"));
      }
    } else {
      throw new ResourceInitializationException(new Exception("Missing parameter: " + ElementNames));
    }
  }
  ...
  AnalysisEngine CreateAnnotator(Map<String, String> DescMap) throws ResourceInitializationException {
    ...
    try {
      ...
      Class<? extends JCasAnnotator_ImplBase> aClass = BaseExperimentBuilder.loadFromClassOrInherit(handle, JCasAnnotator_ImplBase.class, tuples);
