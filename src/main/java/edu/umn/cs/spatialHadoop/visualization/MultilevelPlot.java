...
public class MultilevelPlot {
  ...
  public static class FlatPartitionMap extends Mapper<Rectangle, Iterable<? extends Shape>, LongWritable, Canvas> {
    ...
    @Override protected void map(Rectangle partition, Iterable<? extends Shape> shapes, Context context) throws IOException, InterruptedException {
      if (smooth) {
  ...
  public static class FlatPartitionReduce extends Reducer<LongWritable, Canvas, LongWritable, Canvas> {
    ...
    @Override protected void reduce(LongWritable tileID, Iterable<Canvas> interLayers, Context context) throws IOException, InterruptedException {
  ...
  public static class PyramidPartitionMap extends Mapper<Rectangle, Iterable<? extends Shape>, LongWritable, Shape> {
    ...
    @Override protected void map(Rectangle partition, Iterable<? extends Shape> shapes, Context context) throws IOException, InterruptedException {
      ...
      for (Shape shape : shapes) {
        ...
        for (int z = subPyramid.maximumLevel; z >= subPyramid.minimumLevel; z -= maxLevelsPerReducer) {
  ...
  public static class PyramidPartitionReduce extends Reducer<LongWritable, Shape, LongWritable, Canvas> {
    ...
    @Override protected void reduce(LongWritable tileID, Iterable<Shape> shapes, Context context) throws IOException, InterruptedException {
  ...
  private static Job plotMapReduce(Path[] inFiles, Path outFile, Class<? extends Plotter> plotterClass, OperationsParams params) throws IOException, InterruptedException, ClassNotFoundException {
    Plotter plotter;
    try {
    ...
    if (partitionTechnique.equalsIgnoreCase("flat")) {
      ...
      if (partitionTechnique.equalsIgnoreCase("pyramid")) {
  ...
  private static void plotLocal(Path[] inFiles, final Path outPath, final Class<? extends Plotter> plotterClass, final OperationsParams params) throws IOException, InterruptedException, ClassNotFoundException {
    ...
    final Rectangle inputMBR = params.get("mbr") != null ? params.getShape("mbr").getMBR() : FileMBR.fileMBR(inFiles, mbrParams);
    ...
    try {
      ...
      SubPyramid subPyramid = new SubPyramid(inputMBR, minLevel, maxLevel, 0, 0, 1 << maxLevel, 1 << maxLevel);
      ...
      for (InputSplit split : splits) {
        ...
        if (reader instanceof SpatialRecordReader3) {
          ...
          if (reader instanceof HDFRecordReader) {
      ...
      Parallel.forEach(entries.length, new RunnableRange<Object>() {
        @Override public Object run(int i1, int i2) {
          ...
          try {
            ...
            for (int i = i1; i < i2; i++) {
              ...
              DataOutputStream outFile = output ? outFS.create(imagePath) : new DataOutputStream(new NullOutputStream());
   ...
   */
  public static void createTiles(Iterable<? extends Shape> shapes, SubPyramid subPyramid, int tileWidth, int tileHeight, Plotter plotter, Map<Long, Canvas> tiles) {
  ...
  public static Job plot(Path[] inPaths, Path outPath, Class<? extends Plotter> plotterClass, OperationsParams params) throws IOException, InterruptedException, ClassNotFoundException {
      ...
      Thread memThread = new Thread(new Thread() {
        @Override public void run() {
          ...
          while (true) {
            ...
            runtime.gc();
            LOG.info("Memory usage: " + ((runtime.totalMemory() - runtime.freeMemory()) / (1024 * 1024 * 1024)) + "GB.");
          }
        }
      });
