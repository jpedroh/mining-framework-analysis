 ...
 *
 */
@OperationMetadata(shortName = "gplot", description = "Plots a file to an image") public class GeometricPlot {
  public static class GeometricRasterizer extends Plotter {
    ...
    @Override public Canvas createCanvas(int width, int height, Rectangle mbr) {
      ImageCanvas imageCanvas = new ImageCanvas(mbr, width, height);
      imageCanvas.setColor(strokeColor);
      return imageCanvas;
    }

    @Override public void plot(Canvas canvasLayer, Shape shape) {
      ImageCanvas imgLayer = (ImageCanvas) canvasLayer;
      imgLayer.drawShape(shape);
    }

    @Override public Class<? extends Canvas> getCanvasClass() {
      return ImageCanvas.class;
    }

    @Override public void merge(Canvas finalLayer, Canvas intermediateLayer) {
      ((ImageCanvas) finalLayer).mergeWith((ImageCanvas) intermediateLayer);
    }

    @Override public void writeImage(Canvas layer, DataOutputStream out, boolean vflip) throws IOException {
   ...
   */
  public static Job plot(Path[] inFiles, Path outFile, OperationsParams params) throws IOException, InterruptedException, ClassNotFoundException {
   ...
   * @throws InterruptedException
   */
  public static BufferedImage combineImages(Configuration conf, Path[] files, boolean includeBoundaries, int width, int height) throws IOException, InterruptedException {
    ...
    for (Path file : files) {
      ...
      if (fs.getFileStatus(file).isDir()) {
        ...
        graphics.drawImage(image, (int) mbr.x1, (int) mbr.y1, (int) mbr.getWidth(), (int) mbr.getHeight(), null);
        graphics.dispose();
