...
public class ImmutableConciseSet {
  ...
  public static ImmutableConciseSet intersection(Iterator<ImmutableConciseSet> sets) {
    ...
    while (sets.hasNext()) {
      final UnmodifiableIterator<ImmutableConciseSet> partialIter = Iterators.singletonIterator(partialResults);
      partialResults = doIntersection(Iterators.<ImmutableConciseSet>concat(partialIter, Iterators.limit(sets, CHUNK_SIZE)));
    }
    return partialResults;
  }
  ...
  private static void addAndCompact(IntList set, int wordToAdd) {
    ...
    if (ConciseSetUtils.isAllOnesLiteral(last)) {
      if (ConciseSetUtils.isAllOnesLiteral(wordToAdd)) {
        ...
        if (ConciseSetUtils.isOneSequence(wordToAdd) && ConciseSetUtils.getFlippedBit(wordToAdd) == -1) {
          newWord = wordToAdd + 1;
        }
      }
    } else {
      if (ConciseSetUtils.isOneSequence(last)) {
        if (ConciseSetUtils.isAllOnesLiteral(wordToAdd)) {
          newWord = last + 1;
        } else {
          if (ConciseSetUtils.isOneSequence(wordToAdd) && ConciseSetUtils.getFlippedBit(wordToAdd) == -1) {
        ...
        if (ConciseSetUtils.isAllZerosLiteral(last)) {
          if (ConciseSetUtils.isAllZerosLiteral(wordToAdd)) {
            newWord = 0x00000001;
          } else {
            if (ConciseSetUtils.isZeroSequence(wordToAdd) && ConciseSetUtils.getFlippedBit(wordToAdd) == -1) {
              newWord = wordToAdd + 1;
            }
          }
        } else {
          if (ConciseSetUtils.isZeroSequence(last)) {
            if (ConciseSetUtils.isAllZerosLiteral(wordToAdd)) {
              newWord = last + 1;
            } else {
              if (ConciseSetUtils.isZeroSequence(wordToAdd) && ConciseSetUtils.getFlippedBit(wordToAdd) == -1) {
            ...
            if (ConciseSetUtils.isLiteralWithSingleOneBit(last)) {
              ...
              if (ConciseSetUtils.isLiteralWithSingleZeroBit(last)) {
                int position = Integer.numberOfTrailingZeros(~last) + 1;
                if (ConciseSetUtils.isAllOnesLiteral(wordToAdd)) {
  ...
  private static ImmutableConciseSet doUnion(Iterator<ImmutableConciseSet> sets) {
    ...
    MinMaxPriorityQueue<WordHolder> theQ = MinMaxPriorityQueue.orderedBy(new Comparator<WordHolder>() {
      @Override public int compare(WordHolder h1, WordHolder h2) {
        int w1 = h1.getWord();
        int w2 = h2.getWord();
  ...
  public IntSet.IntIterator iterator() {
    if (isEmpty()) {
      return new IntSet.IntIterator() {
        @Override public void skipAllBefore(int element) {
        }
        ...
        @Override public IntSet.IntIterator clone() {
          throw new UnsupportedOperationException();
        }
      };
    }
  ...
  private class BitIterator implements IntSet.IntIterator {
    ...
    @Override public IntSet.IntIterator clone() {
      return new BitIterator((ConciseSetUtils.LiteralAndZeroFillExpander) litExp.clone(), (ConciseSetUtils.OneFillExpander) oneExp.clone(), exp.clone(), nextIndex, nextOffset);
    }
