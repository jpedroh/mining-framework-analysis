...
class KQSelector extends java.nio.channels.spi.AbstractSelector {
  ...
  @Override public Set<SelectionKey> keys() {
  ...
  @Override public Set<SelectionKey> selectedKeys() {
    return selected;
  }
  ...
  private int poll(long timeout) {
    ...
    try {
      ...
      if (DEBUG) {
        System.err.println("kevent returned " + nready + " events ready");
      }
    }  finally {
      end();
    }
    ...
    synchronized (regLock) {
      for (int i = 0; i < nready; ++i) {
        ...
        if (d != null) {
          ...
          if (DEBUG) {
            System.err.printf("fd=%d filt=0x%x\n", d.fd, filt);
          }
  ...
  private void handleChangedKey(Descriptor changed) {
    synchronized (regLock) {
      ...
      for (Integer filt : new Integer[] { EVFILT_READ, EVFILT_WRITE }) {
        ...
        if (DEBUG) {
          System.err.printf("Updating fd %d filt=0x%x flags=0x%x\n", changed.fd, filt, flags);
        }
        if (flags != 0) {
