...
public class UnixSocketChannel extends AbstractNativeSocketChannel {
  ...
  static UnixSocketAddress getpeername(int sockfd) {
    ...
    if (len.getValue() == addr.getHeaderLength()) {
      addr.setPath("");
    }
    return remote;
  }
  ...
  static UnixSocketAddress getsockname(int sockfd) {
    ...
    if (len.getValue() == headerLength) {
      ...
      if (len.getValue() < maxLength) {
        String path = addr.getPath();
        int newLength = len.getValue() - headerLength;
        if (newLength < path.length()) {
          addr.setPath(path.substring(0, len.getValue() - headerLength));
        }
      }
    }
    return remote;
  ...
  @Override public <T extends java.lang.Object> T getOption(java.net.SocketOption<T> name) throws IOException {
    throw new UnsupportedOperationException("getOption is not supported");
  }

  @Override public Set<java.net.SocketOption<?>> supportedOptions() {
  ...
  @Override public <T extends java.lang.Object> SocketChannel setOption(java.net.SocketOption<T> name, T value) throws IOException {
