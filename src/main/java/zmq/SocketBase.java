...
public abstract class SocketBase extends Own implements IPollEvents, Pipe.IPipeEvents {
  ...
  private void check_protocol(String protocol_) {
    ...
    if ((protocol_.equals("pgm") || protocol_.equals("epgm")) && options.type != ZMQ.ZMQ_PUB && options.type != ZMQ.ZMQ_SUB && options.type != ZMQ.ZMQ_XPUB && options.type != ZMQ.ZMQ_XSUB) {
  ...
  public boolean connect(String addr_) {
    ...
    if (protocol.equals("tcp")) {
      ...
      paddr.resolved().resolve(address, options.ipv4only != 0 ? true : false);
    } else {
      if (protocol.equals("ipc")) {
  ...
  @Override protected void process_stop() {
    stop_monitor();
    ctx_terminated = true;
  }

  @Override protected void process_bind(Pipe pipe_) {
    attach_pipe(pipe_);
  }

  @Override protected void process_term(int linger_) {
  ...
  @Override protected void process_destroy() {
    destroyed = true;
  }
  ...
  @Override public void out_event() {
    throw new UnsupportedOperationException();
  }

  @Override public void connect_event() {
    throw new UnsupportedOperationException();
  }

  @Override public void accept_event() {
    throw new UnsupportedOperationException();
  }

  @Override public void timer_event(int id_) {
    throw new UnsupportedOperationException();
  }
  ...
  @Override public void read_activated(Pipe pipe_) {
    xread_activated(pipe_);
  }

  @Override public void write_activated(Pipe pipe_) {
    xwrite_activated(pipe_);
  }

  @Override public void hiccuped(Pipe pipe_) {
  ...
  @Override public void terminated(Pipe pipe_) {
    xterminated(pipe_);
    Iterator<Entry<String, Pipe>> it = inprocs.entrySet().iterator();
    while (it.hasNext()) {
