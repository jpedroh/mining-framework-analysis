...
public class DefaultColumnFamily extends ColumnFamily implements ColumnFamilyPersonality {
  ...
  public void init() throws IOException {
    File sstableDirectory = SsTableStreamWriter.pathToSsTableDataDirectory(keyspace.getConfiguration(), columnFamilyMetadata);
    ...
    while ((t = r.getNextToken()) != null) {
      ...
      for (Entry<AtomKey, AtomValue> col : x.entrySet()) {
        if (col.getKey() instanceof RowTombstoneKey) {
          ...
          if (col.getKey() instanceof ColumnKey) {
            ...
            if (col.getValue() instanceof ColumnValue) {
              ...
              if (col.getValue() instanceof TombstoneValue) {
  ...
  @Deprecated public Memtable getMemtable() {
    return memtable.get();
  }

  @Deprecated public void setMemtable(Memtable memtable) {
  ...
  public static AtomValue applyRules(AtomValue lastValue, AtomValue thisValue) {
    ...
    if (thisValue instanceof TombstoneValue && thisValue.getTime() >= lastValue.getTime()) {
      return thisValue;
    }
    if (lastValue instanceof TombstoneValue && lastValue.getTime() >= thisValue.getTime()) {
      return lastValue;
    }
  ...
  public void put(String rowkey, String column, String value, long time) {
    try {
      memtable.get().getCommitLog().write(keyspace.getKeyspaceMetadata().getPartitioner().partition(rowkey), new ColumnKey(column), value, time, 0);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  ...
  void considerFlush() {
    ...
    if (columnFamilyMetadata.getFlushNumberOfRowKeys() != 0 && now.size() >= columnFamilyMetadata.getFlushNumberOfRowKeys()) {
      CommitLog commitLog = new CommitLog(this);
      try {
        commitLog.open();
      } catch (FileNotFoundException e) {
