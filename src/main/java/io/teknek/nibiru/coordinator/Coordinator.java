...
public class Coordinator {
  ...
  public Coordinator(Server server) {
    this.server = server;
    metaDataCoordinator = new MetaDataCoordinator(this, server.getConfiguration(), server.getMetaDataManager(), server.getClusterMembership(), server.getServerId());
  ...
  public Response handleStreamRequest(Message m) {
    ...
    Store store = this.server.getKeyspaces().get(m.getPayload().get("keyspace")).getStores().get(m.getPayload().get("store"));
    ...
    if (DirectSsTableWriter.OPEN.equals(m.getPayload().get("type"))) {
      ...
      if (DirectSsTableWriter.CLOSE.equals(m.getPayload().get("type"))) {
        w.close((String) m.getPayload().get("id"));
        return new Response();
      } else {
        if (DirectSsTableWriter.WRITE.equals(m.getPayload().get("type"))) {
          ...
          w.write(om.convertValue(m.getPayload().get("token"), Token.class), mp, (String) m.getPayload().get("id"));
          return new Response();
        }
      }
    }
  ...
  public List<Destination> destinationsForToken(Token token, Keyspace keyspace) {
    return keyspace.getKeyspaceMetaData().getRouter().routesTo(server.getServerId(), keyspace, server.getClusterMembership(), token);
  }
  ...
  public Response handle(Message message) {
    ...
    if (ColumnFamilyPersonality.PERSONALITY.equals(message.getPersonality())) {
      ...
      Response response = eventualCoordinator.handleMessage(token, message, destinations, timeoutInMs, destinationLocal, action, merger, getHinterForMessage(message, columnFamily));
      if (!response.containsKey("exception")) {
        response = triggerManager.executeTriggers(message, response, keyspace, columnFamily, timeoutInMs, requestStart);
      }
      return response;
    } else {
      if (KeyValuePersonality.KEY_VALUE_PERSONALITY.equals(message.getPersonality())) {
        ...
        Response response = eventualCoordinator.handleMessage(token, message, destinations, timeoutInMs, destinationLocal, action, merger, null);
