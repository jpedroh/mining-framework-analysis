 ...
 * @author Mark Hiner
 */
public abstract class AbstractArrayLoader<A extends java.lang.Object> implements SCIFIOArrayLoader<A> {
  private int index = 0;
  ...
  @Parameter private ImgUtilityService imgUtilityService;
  ...
  public AbstractArrayLoader(final Reader reader, final ImageRegion subRegion) {
    ...
    final Type<?> inputType = imgUtilityService.makeType(reader.getMetadata().get(0).getPixelType());
  ...
  @Override public ColorTable loadTable(final int imageIndex, final int planeIndex) throws FormatException, IOException {
    ColorTable ct = getTable(imageIndex, planeIndex);
    if (ct == null && !loadedTable()[imageIndex][planeIndex]) {
      final long[] planeMin = new long[reader.getMetadata().get(imageIndex).getAxesPlanar().size()];
  ...
  @Override public A loadArray(final Interval bounds) {
    synchronized (reader) {
  ...
  /**
	 ...
	 */
  private void read(final A data, final Interval bounds, final Range[] npRanges, final long[] npIndices) throws FormatException, IOException {
  ...
  /**
	 ...
	 */
  private void read(final A data, Plane tmpPlane, final Interval bounds, final Range[] npRanges, final long[] npIndices, final int depth, int planeCount) throws FormatException, IOException {
    if (depth < npRanges.length) {
      ...
      for (int i = 0; i < npRanges[npPosition].size(); i++) {
        ...
        read(data, tmpPlane, bounds, npRanges, npIndices, depth + 1, planeCount);
        planeCount++;
      }
    } else {
      if (inSubregion(npIndices)) {
        final int planeIndex = (int) FormatTools.positionToRaster(0, reader, npIndices);
  ...
  private boolean[][] loadedTable() {
    if (loadedTable == null) {
      ...
      loadedTable = new boolean[m.getImageCount()][(int) m.get(0).getPlaneCount()];
    }
    return loadedTable;
  }
  ...
  /**
	 ...
	 */
  private void addTable(final int imageIndex, final int planeIndex, final ColorTable colorTable) {
  ...
  private boolean inSubregion(final long[] npIndices) {
    ...
    if (subRegion != null) {
      int index = 0;
      for (final CalibratedAxis axis : reader.getMetadata().get(0).getAxesNonPlanar()) {
        inSubregion = inSubregion && inRange(subRegion.getRange(axis.type()), npIndices[index++]);
      }
    }
    return inSubregion;
  }
