...
public class HtmlReporter<E extends Enum<E>> extends Reporter<E> {
  ...
  @Override public void report(final StatisticsPeekHolder<E> statisticsPeekHolder) {
    try {
  ...
  @Override public void summarize(final StatisticsHolder<E> statisticsHolder) {
    ...
    try {
      for (Enum<E> result : results) {
        ...
        sb.append("reportPercentiles(\'").append(getPercentilesFilename(result.name()).substring(0, getPercentilesFilename(result.name()).length() - 4)).append("\', \'Reponse Time percentiles for ").append(result.name()).append("\', \'" + mean + "\', \'" + maxValue).append("\');").append(CRLF);
      }
  ...
  private String toCsv(GcStatsCollector.GcStats gcStats) {
    ...
    return String.valueOf(formatTimestampInNano(jvmStartTime + gcStats.getStartTimestamp())) + "," + gcStats.getDuration() + "," + gcStats.getAction() + "," + gcStats.getCause() + "," + gcStats.getName();
  }
