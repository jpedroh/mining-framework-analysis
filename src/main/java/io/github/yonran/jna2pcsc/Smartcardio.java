...
>>>>>>> /usr/src/app/output/jnasmartcardio/jnasmartcardio/d2203a3c4b2373e013348cd1f2c167a8343ee344/src/main/java/io/github/yonran/jna2pcsc/Smartcardio.java/right.java
    <String, SCardReaderState, Integer> 
<<<<<<< /usr/src/app/output/jnasmartcardio/jnasmartcardio/d2203a3c4b2373e013348cd1f2c167a8343ee344/src/main/java/io/github/yonran/jna2pcsc/Smartcardio.java/left.java
    stateMap
=======
    knownReaders
    ...
    @Override public List<CardTerminal> list(State state) throws CardException {
      ...
      if (state == State.CARD_REMOVAL || state == State.CARD_INSERTION) {
        ...
        for (int i = 0; i < knownReaders.size(); i++) {
          ...
          boolean cardInserted = !wasPresent && isPresent || oldCounter < newCounter;
          boolean cardRemoved = wasPresent && !isPresent || !isPresent && oldCounter < newCounter || oldCounter + 1 < newCounter;
          boolean shouldAdd = state == State.CARD_INSERTION && cardInserted || state == State.CARD_REMOVAL && cardRemoved;
          if (shouldAdd) {
...
>>>>>>> /usr/src/app/output/jnasmartcardio/jnasmartcardio/d2203a3c4b2373e013348cd1f2c167a8343ee344/src/main/java/io/github/yonran/jna2pcsc/Smartcardio.java/right.java
      ...
      if ("T=0".equals(protocol)) {
        ...
        if ("T=1".equals(protocol)) {
          ...
          if ("*".equals(protocol)) {
            ...
            if ("T=CL".equals(protocol)) {
              dwPreferredProtocols = 0;
            } else {
              throw new IllegalArgumentException("Protocol should be one of T=0, T=1, *, T=CL. Got " + protocol);
            }
          }
        }
      }
  ...
  public static class JnaCard extends Card {
    ...
    /**
		 ...
		 */
    @Override public byte[] transmitControlCommand(int controlCode, byte[] arg1) throws CardException {
  ...
  public static class JnaCardChannel extends CardChannel {
    ...
    @Override public int getChannelNumber() {
      return channel & 0xff;
    }
    ...
    private ByteBuffer transmitImpl(byte[] copy, ByteBuffer response) throws JnaPCSCException {
      ...
      if (convertToShortApdus && isExtendedApdu && originalLc == 0) {
        ...
        if (convertToShortApdus && isExtendedApdu && 0 < originalLc && originalLc <= 255) {
          System.arraycopy(copy, 7, copy, 5, originalLc);
          command.limit(5 + originalLc);
        } else {
          if (convertToShortApdus && isExtendedApdu && 256 <= originalLc) {
            System.arraycopy(copy, 7, copy, 5, originalLc);
            command.limit(5 + originalLc);
            throw new IllegalArgumentException("Can\'t transmit big bodies with T=0. (you\'ll need to do your own envelope)");
          } else {
            if (convertToShortApdus && !isExtendedApdu && originalLc > 0 && originalLe > 0) {
              command.limit(5 + originalLc);
            }
          }
        }
      ...
      while (true) {
        if (isResponseFinal) {
          ...
          if (response == null) {
            ...
            if (response != null && ignoreLeWhenAllocating) {
    ...
    static byte getClassByte(byte origCla, int channelNumber) {
      ...
      if (0 <= channelNumber && channelNumber <= 3) {
        ...
        if (0x04 <= channelNumber && channelNumber <= 0x13) {
