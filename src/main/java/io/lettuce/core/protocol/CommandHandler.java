...
public class CommandHandler extends ChannelDuplexHandler implements HasQueuedCommands {
  ...
  @Override public Collection<RedisCommand<?, ?, ?>> drainQueue() {
  ...
  private static <T extends java.lang.Object> List<T> drainCommands(Queue<T> source) {
  ...
  private void validateWrite(int commands) {
    if (usesBoundedQueues()) {
      ...
      if (stack.size() + commands > allowedRequestQueueSize) {
        throw new RedisException("Internal stack size exceeded: " + clientOptions.getRequestQueueSize() + ". Commands are not accepted until the stack size drops.");
      }
    }
  }
  ...
  private String logPrefix() {
    ...
    String buffer = "[" + ChannelLogDescriptor.logDescriptor(channel) + ", epid=" + endpoint.getId() + ", chid=0x" + getCommandHandlerId() + ']';
  ...
  class BackpressureSource implements DemandAware.Source {
    @Override public void requestMore() {
  ...
  static class AddToStack implements GenericFutureListener<Future<Void>> {
    ...
    /**
         ...
         * @return
         */
    @SuppressWarnings(value = { "unchecked" }) static AddToStack newInstance(ArrayDeque<?> stack, RedisCommand<?, ?, ?> command) {
    ...
    @SuppressWarnings(value = { "unchecked" }) @Override public void operationComplete(Future<Void> future) {
      try {
        if (future.isSuccess()) {
          stack.add(command);
        }
