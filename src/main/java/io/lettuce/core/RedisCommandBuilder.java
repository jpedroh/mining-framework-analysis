 ...
 */
@SuppressWarnings(value = { "unchecked", "varargs" }) class RedisCommandBuilder<K extends java.lang.Object, V extends java.lang.Object> extends BaseRedisCommandBuilder<K, V> {
  ...
  <T extends java.lang.Object> Command<K, V, T> eval(byte[] script, ScriptOutputType type, K... keys) {
  ...
  <T extends java.lang.Object> Command<K, V, T> eval(byte[] script, ScriptOutputType type, K[] keys, V... values) {
    LettuceAssert.notNull(script, "Script " + MUST_NOT_BE_NULL);
    LettuceAssert.notNull(type, "ScriptOutputType " + MUST_NOT_BE_NULL);
    LettuceAssert.notNull(keys, "Keys " + MUST_NOT_BE_NULL);
  ...
  <T extends java.lang.Object> Command<K, V, T> evalsha(String digest, ScriptOutputType type, K... keys) {
  ...
  <T extends java.lang.Object> Command<K, V, T> evalsha(String digest, ScriptOutputType type, K[] keys, V... values) {
    LettuceAssert.notNull(digest, "Digest " + MUST_NOT_BE_NULL);
    LettuceAssert.notEmpty(digest, "Digest " + MUST_NOT_BE_EMPTY);
    LettuceAssert.notNull(type, "ScriptOutputType " + MUST_NOT_BE_NULL);
    LettuceAssert.notNull(keys, "Keys " + MUST_NOT_BE_NULL);
  ...
  Command<K, V, List<GeoWithin<V>>> georadius(CommandType commandType, K key, double longitude, double latitude, double distance, String unit, GeoArgs geoArgs) {
    ...
    CommandArgs<K, V> args = new CommandArgs<>(codec).addKey(key).add(longitude).add(latitude).add(distance).add(unit);
    geoArgs.build(args);
    return createCommand(commandType, new GeoWithinListOutput<>(codec, geoArgs.isWithDistance(), geoArgs.isWithHash(), geoArgs.isWithCoordinates()), args);
  }
  ...
  Command<K, V, List<GeoWithin<V>>> geosearch(K key, GeoSearch.GeoRef<K> reference, GeoSearch.GeoPredicate predicate, GeoArgs geoArgs) {
    ...
    predicate.build(args);
    geoArgs.build(args);
    return createCommand(GEOSEARCH, new GeoWithinListOutput<>(codec, geoArgs.isWithDistance(), geoArgs.isWithHash(), geoArgs.isWithCoordinates()), args);
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) Command<K, V, String> pfmerge(K destkey, K sourcekey, K... moreSourceKeys) {
  ...
  @SuppressWarnings(value = { "unchecked" }) Command<K, V, String> pfmerge(K destkey, K... sourcekeys) {
    LettuceAssert.notNull(destkey, "Destkey " + MUST_NOT_BE_NULL);
  ...
  @SuppressWarnings(value = { "unchecked", "rawtypes" }) Command<K, V, Map<K, Long>> pubsubNumsub(K... pattern) {
    LettuceAssert.notNull(pattern, "Pattern " + MUST_NOT_BE_NULL);
  ...
  public Command<K, V, List<StreamMessage<K, V>>> xclaim(K key, Consumer<K> consumer, XClaimArgs xClaimArgs, String[] messageIds) {
    ...
    CommandArgs<K, V> args = new CommandArgs<>(codec).addKey(key).addKey(consumer.group).addKey(consumer.name).add(xClaimArgs.minIdleTime);
    for (String messageId : messageIds) {
      args.add(messageId);
    }
  ...
  public Command<K, V, String> xgroupCreate(StreamOffset<K> offset, K group, XGroupCreateArgs commandArgs) {
    ...
    LettuceAssert.notNull(group, "Group " + MUST_NOT_BE_NULL);
    CommandArgs<K, V> args = new CommandArgs<>(codec).add(CREATE).addKey(offset.getName()).addKey(group).add(offset.getOffset());
    if (commandArgs != null) {
  ...
  public Command<K, V, Boolean> xgroupCreateconsumer(K key, Consumer<K> consumer) {
    notNullKey(key);
    LettuceAssert.notNull(consumer, "Consumer " + MUST_NOT_BE_NULL);
    CommandArgs<K, V> args = new CommandArgs<>(codec).add("CREATECONSUMER").addKey(key).addKey(consumer.getGroup()).addKey(consumer.getName());
  ...
  public Command<K, V, Long> xgroupDelconsumer(K key, Consumer<K> consumer) {
    notNullKey(key);
    LettuceAssert.notNull(consumer, "Consumer " + MUST_NOT_BE_NULL);
    CommandArgs<K, V> args = new CommandArgs<>(codec).add("DELCONSUMER").addKey(key).addKey(consumer.getGroup()).addKey(consumer.getName());
  ...
  public Command<K, V, String> xgroupSetid(StreamOffset<K> offset, K group) {
    LettuceAssert.notNull(offset, "StreamOffset " + MUST_NOT_BE_NULL);
    LettuceAssert.notNull(group, "Group " + MUST_NOT_BE_NULL);
    CommandArgs<K, V> args = new CommandArgs<>(codec).add("SETID").addKey(offset.getName()).addKey(group).add(offset.getOffset());
    return createCommand(XGROUP, new StatusOutput<>(codec), args);
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) Command<K, V, Long> zadd(K key, ZAddArgs zAddArgs, Object... scoresAndValues) {
    notNullKey(key);
