 ...
 */
@Extension public class Root implements RootAction {
  ...
  public HttpResponse doSearch() {
    ...
    List<SearchResult> searchResults = getAll().stream().filter((sample) -> sample.getDisplayName().toLowerCase().contains(query.toLowerCase())).limit(5).map((sample) -> {
      SearchResult searchResult = new SearchResult();
      searchResult.setIcon(IconSet.getSymbol(sample.getIconFileName().substring(7), "", "", "", "design-library", ""));
      searchResult.setTitle(sample.getDisplayName());
      searchResult.setUrl(Jenkins.get().getRootUrl() + "design-library/" + sample.getUrlName());
      searchResult.setChildren(Collections.singletonList(new SearchResult("Tropic Morning News", "Hello world", "")));
      return searchResult;
