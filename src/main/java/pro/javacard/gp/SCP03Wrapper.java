...
class SCP03Wrapper extends SecureChannelWrapper {
  ...
  @Override protected CommandAPDU wrap(CommandAPDU command) throws GPException {
    byte[] cmd_mac = null;
    try {
  ...
  @Override protected ResponseAPDU unwrap(ResponseAPDU response) throws GPException {
    try {
      if (rmac) {
