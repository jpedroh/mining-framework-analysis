...
public class Processor {
   ...
   */
  public Processor(Configuration config) throws CorruptIndexException, IOException {
    this(config, true);
  }
   ...
   * existing data.
   */
  public Processor(Configuration config, boolean overwrite) throws CorruptIndexException, IOException {
    this.config = config;
   ...
   * them in batches, notifying the listeners throughout.
   */
  public void deduplicate(Collection<DataSource> sources, int batch_size) throws IOException {
  ...
  public void link() throws IOException {
    link(config.getDataSources(1), config.getDataSources(2), DEFAULT_BATCH_SIZE);
  }
   ...
   * records within each group.
   */
  public void link(Collection<DataSource> sources1, Collection<DataSource> sources2, int batch_size) throws IOException {
   ...
   * @throws IOException
   */
  public void buildIndex(Collection<DataSource> sources, int batch_size) throws CorruptIndexException, IOException {
    int count = 0;
    for (DataSource source : sources) {
      source.setLogger(logger);
      RecordIterator it2 = source.getRecords();
  ...
  private void match(Record record) throws IOException {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Matching record " + PrintMatchListener.toString(record) + " found " + candidates.size() + " candidates");
    }
    for (Record candidate : candidates) {
      ...
      if (prob > config.getThreshold()) {
        ...
        if (prob > config.getMaybeThreshold()) {
  ...
  boolean matchRL(Record record) throws IOException {
    ...
    if (best != null) {
      if (max > config.getThreshold()) {
        ...
        if (max > config.getMaybeThreshold()) {
      ...
      for (String v1 : vs1) {
        ...
        for (String v2 : vs2) {
          ...
          try {
            ...
            throw new RuntimeException("Comparison of values \'" + v1 + "\' and " + "\'" + v2 + "\' failed", e);
          }
        }
      }
