...
public class ConfigLoader {
   ...
   * @since 1.3
   */
  public static Configuration loadFromString(String config) throws IOException, SAXException {
    ConfigurationImpl cfg = new ConfigurationImpl();
    XMLReader parser = XMLReaderFactory.createXMLReader();
  ...
  private static class ConfigHandler extends DefaultHandler {
      ...
      if (keepers.contains(localName)) {
        ...
        if (localName.equals("property")) {
          ...
          if (attributes.getValue("lookup") != null) {
            lookup = (Property.Lookup) ObjectUtils.getEnumConstantByName(Property.Lookup.class, attributes.getValue("lookup").toUpperCase());
          }
        } else {
          if (localName.equals("csv")) {
            ...
            if (localName.equals("jdbc")) {
              ...
              if (localName.equals("jndi")) {
                datasource = new JNDIDataSource();
                currentobj = datasource;
              } else {
                if (localName.equals("sparql")) {
                  datasource = new SparqlDataSource();
                  currentobj = datasource;
                } else {
                  if (localName.equals("ntriples")) {
                    ...
                    if (localName.equals("data-source")) {
                      ...
                      if (localName.equals("column")) {
                        if (!(datasource instanceof ColumnarDataSource)) {
                          throw new DukeConfigException("Column inside data source which " + "does not support it: " + datasource);
                        }
                        ...
                        if (localName.equals("param")) {
                          ...
                          if (currentobj == null) {
                            throw new DukeConfigException("Trying to set parameter " + param + " but no current object");
                          }
                          if (param.equals("input-file") && path != null && !value.startsWith("/")) {
    ...
    public void endElement(String uri, String localName, String qName) {
      if (localName.equals("threshold")) {
        ...
        if (localName.equals("maybe-threshold")) {
            ...
            if (localName.equals("property")) {
              ...
              if (localName.equals("low")) {
                ...
                if (localName.equals("high")) {
                  ...
                  if (localName.equals("comparator")) {
                    comparator = (Comparator) objects.get(content.toString());
                    if (comparator == null) {
                      comparator = (Comparator) instantiate(content.toString());
                    }
                  } else {
                    if (localName.equals("csv") || localName.equals("jdbc") || localName.equals("jndi") || localName.equals("ntriples") || localName.equals("sparql") || localName.equals("data-source")) {
  ...
  private static Object instantiate(String classname) {
    try {
      ...
      throw new DukeConfigException("Couldn\'t instantiate class " + classname + ": " + e);
    }
  }
}