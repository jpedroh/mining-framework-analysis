...
public class TrackMateGUIController implements ActionListener {
  ...
  public TrackMateGUIController(final TrackMate trackmate) {
    ...
    this.displaySettingsListener = new DisplaySettingsListener() {
      @Override public void displaySettingsChanged(final DisplaySettingsEvent event) {
  ...
  protected Collection<WizardPanelDescriptor> createDescriptors() {
    ...
    startDialoDescriptor = new StartDialogDescriptor(this) {
      @Override public void aboutToHidePanel() {
        super.aboutToHidePanel();
        SomeDialogDescriptor.file = null;
      }

      @Override public void displayingPanel() {
    ...
    spotFilterDescriptor.addActionListener(new ActionListener() {
      @Override public void actionPerformed(final ActionEvent event) {
        if (event == spotFilterDescriptor.getComponent().COLOR_FEATURE_CHANGED) {
          ...
          @SuppressWarnings(value = { "unchecked" }) final FeatureColorGenerator<Spot> oldValue = (FeatureColorGenerator<Spot>) guimodel.getDisplaySettings().get(KEY_SPOT_COLORING);
    ...
    configureViewsDescriptor.getComponent().addActionListener(new ActionListener() {
      @Override public void actionPerformed(final ActionEvent event) {
        if (event == configureViewsDescriptor.getComponent().TRACK_SCHEME_BUTTON_PRESSED) {
          ...
          if (event == configureViewsDescriptor.getComponent().DO_ANALYSIS_BUTTON_PRESSED) {
            launchDoAnalysis(false);
          } else {
            if (event == configureViewsDescriptor.getComponent().DO_ANALYSIS_BUTTON_WITH_SHIFT_PRESSED) {
  ...
  protected WizardPanelDescriptor nextDescriptor(final WizardPanelDescriptor currentDescriptor) {
    if (currentDescriptor == startDialoDescriptor) {
      ...
      if (currentDescriptor == detectorChoiceDescriptor) {
        return detectorConfigurationDescriptor;
      } else {
        if (currentDescriptor == detectorConfigurationDescriptor) {
          ...
          if (currentDescriptor == detectionDescriptor) {
            return initFilterDescriptor;
          } else {
            if (currentDescriptor == initFilterDescriptor) {
              return viewChoiceDescriptor;
            } else {
              if (currentDescriptor == viewChoiceDescriptor) {
                return spotFilterDescriptor;
              } else {
                if (currentDescriptor == spotFilterDescriptor) {
                  ...
                  if (currentDescriptor == trackerChoiceDescriptor) {
                    ...
                    if (currentDescriptor == trackerConfigurationDescriptor) {
                      ...
                      if (currentDescriptor == trackingDescriptor) {
                        return trackFilterDescriptor;
                      } else {
                        if (currentDescriptor == trackFilterDescriptor) {
                          return configureViewsDescriptor;
                        } else {
                          if (currentDescriptor == configureViewsDescriptor) {
                            return grapherDescriptor;
                          } else {
                            if (currentDescriptor == grapherDescriptor) {
                              return actionChooserDescriptor;
                            } else {
                              if (currentDescriptor == actionChooserDescriptor) {
                                return null;
                              } else {
  ...
  protected void init() {
    ...
    gui.getLogger().log("Please note that TrackMate is available through Fiji, and is based on a publication. " + "If you use it successfully for your research please be so kind to cite our work:\n");
  ...
  @Override public void actionPerformed(final ActionEvent event) {
    ...
    if (event == gui.NEXT_BUTTON_PRESSED && guimodel.actionFlag) {
      ...
      if (event == gui.PREVIOUS_BUTTON_PRESSED && guimodel.actionFlag) {
        previous();
      } else {
        if (event == gui.LOAD_BUTTON_PRESSED && guimodel.actionFlag) {
          ...
          if (event == gui.SAVE_BUTTON_PRESSED && guimodel.actionFlag) {
            guimodel.actionFlag = false;
            gui.jButtonNext.setText("Resume");
            disableButtonsAndStoreState();
            ...
            if ((event == gui.NEXT_BUTTON_PRESSED || event == gui.PREVIOUS_BUTTON_PRESSED || event == gui.LOAD_BUTTON_PRESSED || event == gui.SAVE_BUTTON_PRESSED) && !guimodel.actionFlag) {
              guimodel.actionFlag = true;
              gui.show(guimodel.previousDescriptor);
              gui.jButtonNext.setText("Next");
              restoreButtonsState();
            } else {
              if (event == gui.LOG_BUTTON_PRESSED) {
                ...
                if (event == gui.DISPLAY_CONFIG_BUTTON_PRESSED) {
  ...
  private void save() {
    guimodel.previousDescriptor = guimodel.currentDescriptor;
    if (guimodel.currentDescriptor.equals(trackerConfigurationDescriptor) || guimodel.currentDescriptor.equals(detectorConfigurationDescriptor)) {
