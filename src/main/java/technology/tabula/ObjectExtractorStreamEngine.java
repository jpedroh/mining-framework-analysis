...
class ObjectExtractorStreamEngine extends PDFGraphicsStreamEngine {
  ...
  @Override public void drawImage(PDImage arg0) throws IOException {
  }
  ...
  @Override public void fillAndStrokePath(int arg0) throws IOException {
    strokeOrFillPath(true);
  }

  @Override public void fillPath(int arg0) throws IOException {
    strokeOrFillPath(true);
  }
  ...
  @Override public void shadingFill(COSName arg0) throws IOException {
  }
  ...
  private void strokeOrFillPath(boolean isFill) {
    ...
    while (!pi.isDone()) {
      ...
      if (currentSegment != PathIterator.SEG_LINETO && currentSegment != PathIterator.SEG_CLOSE && currentSegment != PathIterator.SEG_MOVETO) {
        path.reset();
        return;
      }
      pi.next();
    ...
    while (!pi.isDone()) {
      ...
      switch (currentSegment) {
        ...
        line = pc.compare(start_pos, end_pos) == -1 ? new Line2D.Float(start_pos, end_pos) : new Line2D.Float(end_pos, start_pos);
        ...
        line = pc.compare(end_pos, last_move) == -1 ? new Line2D.Float(end_pos, last_move) : new Line2D.Float(last_move, end_pos);
        if (line.intersects(this.currentClippingPath())) {
          Ruling r = new Ruling(line.getP1(), line.getP2()).intersect(this.currentClippingPath());
          if (r.length() > 0.01) {
            this.rulings.add(r);
