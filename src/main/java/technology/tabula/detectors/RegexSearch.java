...
public class RegexSearch {
  public static ArrayList<UpdatesOnResize> checkSearchesOnFilterResize(PDDocument file, FilteredArea filterArea, RegexSearch[] currentRegexSearches) {
  ...
  private static class UpdatesOnResize {
    RegexSearch updatedRegexSearch;

    ArrayList<MatchingArea> areasAdded;

    ArrayList<MatchingArea> areasRemoved;

    Boolean overlapsAnotherSearch;

    public UpdatesOnResize(RegexSearch regexSearch, ArrayList<MatchingArea> newAreasToAdd, ArrayList<MatchingArea> oldAreasToRemove, Boolean resizeEventCausedAnOverlap) {
  ...
  public RegexSearch(String regexBeforeTable, String includeRegexBeforeTable, String regexAfterTable, String includeRegexAfterTable, PDDocument document, FilteredArea areaToFilter) {
    this(regexBeforeTable, Boolean.valueOf(includeRegexBeforeTable), regexAfterTable, Boolean.valueOf(includeRegexAfterTable), document, areaToFilter);
  }

  public RegexSearch(String regexBeforeTable, Boolean includeRegexBeforeTable, String regexAfterTable, Boolean includeRegexAfterTable, PDDocument document, FilteredArea filterArea) {
  ...
  private class TableArea {
    ...
    public Integer getHeight() {
      return Math.round(_area.height);
    }

    public Integer getTop() {
      return Math.round(_area.getTop());
    }

    public Integer getLeft() {
      return Math.round(_area.getLeft());
    }

    public Integer getWidth() {
      return Math.round(_area.width);
    }
  }
  ...
  private static class MatchingArea extends HashMap<Integer, LinkedList<TableArea>> {
    ...
    public MatchingArea(Integer startPageNum, Integer endPageNum) {
      ...
      try {
        ...
        FileWriter fw = new FileWriter("./" + "ExtractionLog_" + timeStamp + ".txt", true);
  ...
  public String getPatternBefore() {
    return new String(_regexBeforeTable.toString());
  }

  public String getPatternAfter() {
    return new String(_regexAfterTable.toString());
  }
  ...
  private ArrayList<MatchingArea> detectMatchingAreas(PDDocument document, FilteredArea areaToFilter) {
    ...
    for (Integer currentPage = 1; currentPage <= totalNumPages; currentPage++) {
      ...
      if (areaToFilter != null) {
        height = Math.round(page.height - areaToFilter.getHeaderHeightScale() * page.height - areaToFilter.getFooterHeightScale() * page.height);
      }
      height -= top;
      ArrayList<TextElement> pageTextElements = (ArrayList<TextElement>) page.getText(new Rectangle(top, 0, page.width, height));
      ...
      while (beforeTableMatches.find(startMatchingAt) || afterTableMatches.find(startMatchingAt)) {
        ...
        if ((lastTableUnderDetection._pageBeginMatch.get() == INIT) || (lastTableUnderDetection._pageEndMatch.get() == INIT)) {
          ...
          if (lastTableUnderDetection._pageEndMatch.get() < lastTableUnderDetection._pageBeginMatch.get()) {
            tableUnderDetection = lastTableUnderDetection;
          } else {
            if (lastTableUnderDetection._pageEndCoord.getY() < lastTableUnderDetection._pageBeginCoord.getY() && (lastTableUnderDetection._pageEndMatch.get() == lastTableUnderDetection._pageBeginMatch.get())) {
              tableUnderDetection = lastTableUnderDetection;
            } else {
        ...
        if (bothMatchesEncountered) {
          if (beforeTableMatchLoc.intValue() == afterTableMatchLoc.intValue()) {
            ...
            offsetScale = (beginNotFoundYet) ? ((_includeRegexBeforeTable) ? SignOfOffset.NEGATIVE_BUFFER : SignOfOffset.POSITIVE_NO_BUFFER) : ((_includeRegexAfterTable) ? SignOfOffset.POSITIVE_WITH_BUFFER : SignOfOffset.NONE);
            ...
            offsetScale = (beginLocFoundFirst) ? ((_includeRegexBeforeTable) ? SignOfOffset.NEGATIVE_BUFFER : SignOfOffset.POSITIVE_NO_BUFFER) : ((_includeRegexAfterTable) ? SignOfOffset.POSITIVE_WITH_BUFFER : SignOfOffset.NONE);
          ...
          offsetScale = (beginLocNotFound) ? ((_includeRegexAfterTable) ? SignOfOffset.POSITIVE_WITH_BUFFER : SignOfOffset.NONE) : ((_includeRegexBeforeTable) ? SignOfOffset.NEGATIVE_BUFFER : SignOfOffset.POSITIVE_NO_BUFFER);
    ...
    if ((lastPotMatch._pageBeginMatch.get() == INIT) || (lastPotMatch._pageEndMatch.get() == INIT)) {
      ...
      if ((lastPotMatch._pageEndMatch.get() < lastPotMatch._pageBeginMatch.get())) {
        potentialMatches.removeLast();
      } else {
        if ((lastPotMatch._pageEndMatch.get() == lastPotMatch._pageBeginMatch.get()) && (lastPotMatch._pageEndCoord.getY() < lastPotMatch._pageBeginCoord.getY())) {
          potentialMatches.removeLast();
        }
      }
    }
  ...
  private ArrayList<MatchingArea> calculateMatchingAreas(LinkedList<DetectionData> foundMatches, PDDocument document, FilteredArea areaToFilter) {
    ...
    while (!foundMatches.isEmpty()) {
      ...
      if (foundTable._pageBeginMatch.get() == foundTable._pageEndMatch.get()) {
        ...
        Float footer_height = (areaToFilter == null) ? (float) 0 : areaToFilter.getFooterHeightScale() * currentPage.height;
        Float height = currentPage.height - foundTable._pageBeginCoord.y - footer_height;
        tableSubArea.add(new TableArea(currentPage.getPageNumber(), new Rectangle(foundTable._pageBeginCoord.y, 0, currentPage.width, height)));
        ...
        for (Integer iter = currentPage.getPageNumber() + 1; iter < foundTable._pageEndMatch.get(); iter++) {
          ...
          Integer subAreaTop = (areaToFilter != null) ? Math.round(areaToFilter.getHeaderHeightScale() * currentPage.height) : (int) Math.round(0.5 * (currentPage.getTextBounds().getMinY()));
          ...
          tableSubArea.add(new TableArea(currentPage.getPageNumber(), new Rectangle(subAreaTop, 0, currentPage.width, currentPage.height - (subAreaTop + areaToFilter.getFooterHeightScale()))));
          matchingArea.put(currentPage.getPageNumber(), tableSubArea);
        }
        currentPage = oe.extract(foundTable._pageEndMatch.get());
        tableSubArea = new LinkedList<>();
        Integer top = (areaToFilter != null) ? Math.round(areaToFilter.getHeaderHeightScale() * currentPage.height) : (int) Math.round(0.5 * (currentPage.getTextBounds().getMinY()));
