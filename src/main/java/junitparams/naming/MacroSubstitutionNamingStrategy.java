...
public class MacroSubstitutionNamingStrategy implements TestCaseNamingStrategy {
  ...
  @Override public String getTestCaseName(int parametersIndex, Object parameters) {
  ...
  private String lookupMacroValue(String macro, int parametersIndex, Object parameters) {
    ...
    switch (Macro.parse(macroKey)) {
      case INDEX:
      return String.valueOf(parametersIndex);
      case PARAMS:
      return Utils.stringify(parameters);
      case METHOD:
      return method.name();
      default:
      return substituteDynamicMacro(macro, macroKey, parameters);
    }
  }
  ...
  private String getMacroKey(String macro) {
    return macro.substring(MACRO_START.length(), macro.length() - MACRO_END.length()).toUpperCase(Locale.ENGLISH);
  }
  ...
  private enum Macro {
    ...
    private static final HashSet<String> macros = new HashSet<String>(Arrays.asList(Macro.INDEX.toString(), Macro.PARAMS.toString(), Macro.METHOD.toString()));
  }
}