...
public class InvokeParameterisedMethod extends Statement {
  ...
  private Object createObjectOfExpectedTypeBasedOnParams(Object[] paramset, Class<?>[] typesOfParameters) {
    ...
    try {
      ...
      throw new IllegalStateException("While trying to create object of class " + testMethod.getMethod().getParameterTypes()[0] + " could not find constructor with arguments matching (type-wise) the ones given in parameters.", e);
    }
    return resultParam;
  }
  ...
  private boolean testMethodParamsHasVarargs(Object[] columns, Class<?>[] expectedParameterTypes) {
    ...
    return expectedParameterTypes.length <= columns.length && expectedParameterTypes[paramLen - 1].isArray() && expectedParameterTypes[paramLen - 1].getComponentType().equals(columns[paramLen - 1].getClass());
  }

  private Object[] castAllParametersToProperTypes(Object[] columns, Class<?>[] expectedParameterTypes, Annotation[][] parameterAnnotations) throws ConversionFailedException {
  ...
  @SuppressWarnings(value = { "unchecked" }) private Object castParameterDirectly(Object object, Class clazz) {
    ...
    throw new IllegalArgumentException("Parameter type (" + clazz.getName() + ") cannot be handled! Only primitive types and Strings can be" + " used" + ".");
  }
