...
public class TestMethod {
  ...
  public TestMethod(FrameworkMethod method, TestClass testClass) {
    ...
    if (parametersAnnotation != null && fileParametersAnnotation != null) {
      throw new IllegalArgumentException("Both @Parameters and @FileParameters exist on " + frameworkMethod.getName() + ". Remove one of them!");
    }
  }
  ...
  Description describe() {
    if (isNotIgnored() && !describeFlat()) {
      ...
      for (int i = 0; i < params.length; i++) {
        ...
        parametrised.addChild(Description.createTestDescription(testClass(), Utils.stringify(paramSet, i) + " (" + name() + ")", annotations()));
      }
      return parametrised;
    } else {
  ...
  private String defaultMethodName() {
    String methodName;
    methodName = "parametersFor" + frameworkMethod.getName().substring(0, 1).toUpperCase() + frameworkMethod.getName().substring(1);
    return methodName;
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private Object[] invokeParamsProvidingMethod(Class<?> testClass, Method provideMethod) {
    try {
