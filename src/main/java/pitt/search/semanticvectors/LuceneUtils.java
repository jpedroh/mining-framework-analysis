...
public class LuceneUtils {
  ...
  public LuceneUtils(FlagConfig flagConfig) throws IOException {
    ...
    this.compositeReader = DirectoryReader.open(FSDirectory.open(FileSystems.getDefault().getPath(flagConfig.luceneindexpath())));
  ...
  public String getExternalDocId(int docID) throws IOException {
    if (flagConfig.docidfield().equals("luceneID")) {
      return docID + "";
    }
  ...
  public float getGlobalTermWeight(Term term) {
    ...
    VerbatimLogger.severe("Unrecognized termweight option: " + flagConfig.termweight() + ". Returning 1.\n");
    return 1;
  }
  ...
  public boolean termFilter(Term term) {
    return termFilter(term, flagConfig.contentsfields(), flagConfig.minfrequency(), flagConfig.maxfrequency(), flagConfig.maxnonalphabetchars(), flagConfig.filteroutnumbers(), flagConfig.mintermlength());
  }
