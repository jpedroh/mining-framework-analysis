...
public class BinaryVector implements Vector {
   ...
   */
  @SuppressWarnings(value = { "unchecked" }) public BinaryVector copy() {
  ...
  public String toString() {
    ...
    if (isSparse) {
      ...
      for (int x = 0; x < DEBUG_PRINT_LENGTH; x++) {
        debugString.append(bitSet.get(x) ? "1 " : "0 ");
      }
      ...
      for (int y = 0; y < votingRecord.size(); y++) {
        for (int x = 0; x < DEBUG_PRINT_LENGTH; x++) {
          debugString.append(votingRecord.get(y).get(x) ? "1 " : "0 ");
        }
        debugString.append("\n");
      }
      ...
      for (int x = 0; x < votingRecord.size(); x++) {
        for (int y = 0; y < DEBUG_PRINT_LENGTH; y++) {
          if (votingRecord.get(x).get(y)) {
            actualvals[y] += Math.pow(2, x);
          }
        }
      }
      for (int x = 0; x < DEBUG_PRINT_LENGTH; x++) {
      ...
      this.normalize();
      for (int x = 0; x < DEBUG_PRINT_LENGTH; x++) {
        debugString.append(bitSet.get(x) + " ");
      }
      debugString.append("\n");
      debugString.append("\nCardinality " + bitSet.cardinality() + "\n");
  ...
  @Override public boolean isZeroVector() {
    if (isSparse) {
  ...
  @Override public BinaryVector generateRandomVector(int dimension, int numEntries, Random random) {
    if (dimension % 64 != 0) {
      throw new IllegalArgumentException("Dimension should be a multiple of 64: " + dimension + " will lead to trouble!");
    }
  ...
  @Override public double measureOverlap(Vector other) {
  ...
  @Override public synchronized void superpose(Vector other, double weight, int[] permutation) {
    IncompatibleVectorsException.checkVectorsCompatible(this, other);
    if (weight == 0d) {
      return;
    }
    ...
    if (permutation != null) {
      if (permutation.length != dimension / 64) {
        throw new IllegalArgumentException("Binary vector of dimension " + dimension + " must have permutation of length " + dimension / 64 + " not " + permutation.length);
      }
  ...
  protected synchronized void superposeBitSetFromRowFloor(FixedBitSet incomingBitSet, int rowfloor) {
    ...
    for (int x = rowfloor + 1; x < votingRecord.size(); x++) {
      tempSet.andNot(votingRecord.get(x - 1));
  ...
  protected synchronized FixedBitSet concludeVote() {
    if (votingRecord.size() == 0 || votingRecord.size() == 1 && votingRecord.get(0).cardinality() == 0) {
      return new FixedBitSet(dimension);
    } else {
      return concludeVote(totalNumberOfVotes.get());
    }
  }
  ...
  protected synchronized FixedBitSet concludeVote(int target) {
    ...
    if (even) {
      ...
      while (q != DocIdSetIterator.NO_MORE_DOCS) {
        ...
        if (q + 1 >= tempSet.length()) {
          q = DocIdSetIterator.NO_MORE_DOCS;
        } else {
          q = tempSet.nextSetBit(q + 1);
        }
      }
  ...
  protected synchronized FixedBitSet concludeVote(int target, int row_ceiling) {
    ...
    if (row_floor >= votingRecord.size()) {
      return new FixedBitSet(dimension);
    }
  ...
  @Override public void bind(Vector other) {
    IncompatibleVectorsException.checkVectorsCompatible(this, other);
    if (!BINARY_BINDING_WITH_PERMUTE) {
      BinaryVector binaryOther = (BinaryVector) other;
      this.bitSet.xor(binaryOther.bitSet);
  ...
  @Override public void release(Vector other) {
    if (!BINARY_BINDING_WITH_PERMUTE) {
      bind(other);
    } else {
      bind(other, -1);
  ...
  @Override public synchronized void normalize() {
  ...
  @Override public void writeToLuceneStream(IndexOutput outputStream) {
    if (isSparse) {
      elementalToSemantic();
    }
  ...
  @Override public void readFromLuceneStream(IndexInput inputStream) {
  ...
  @Override public void readFromString(String input) {
    if (input.length() != dimension) {
      throw new IllegalArgumentException("Found " + (input.length()) + " possible coordinates: " + "expected " + dimension);
    }
    for (int i = 0; i < dimension; ++i) {
      if (input.charAt(i) == '1') {
        bitSet.set(i);
  ...
  public void permute(int[] permutation) {
    if (permutation.length != getDimension() / 64) {
      throw new IllegalArgumentException("Binary vector of dimension " + getDimension() + " must have permutation of length " + getDimension() / 64 + " not " + permutation.length);
    }
