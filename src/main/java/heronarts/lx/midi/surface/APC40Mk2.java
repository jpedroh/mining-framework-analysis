...
<<<<<<< /usr/src/app/output/heronarts/lx/198881dec136d9149da48e998f9c96ff27ef87b5/src/main/java/heronarts/lx/midi/surface/APC40Mk2.java/left.java
  private final LXMixerEngine.Listener mixerEngineListener = new LXMixerEngine.Listener() {
    @Override public void channelRemoved(LXMixerEngine mixer, LXAbstractChannel channel) {
      unregisterChannel(channel);
      sendChannels();
    }

    @Override public void channelMoved(LXMixerEngine mixer, LXAbstractChannel channel) {
      sendChannels();
    }

    @Override public void channelAdded(LXMixerEngine mixer, LXAbstractChannel channel) {
      sendChannels();
      registerChannel(channel);
    }
  };
=======
  public static final int RAINBOW_HUE_COLUMNS = 72;
>>>>>>> /usr/src/app/output/heronarts/lx/198881dec136d9149da48e998f9c96ff27ef87b5/src/main/java/heronarts/lx/midi/surface/APC40Mk2.java/right.java
  ...
  private boolean deviceLockOn;

  private Integer colorClipboard;

  private LXDynamicColor focusColor;
    ...
    void resend() {
      for (int i = 0; i < this.knobs.length; ++i) {
        ...
        if (parameter != null) {
          ...
          double normalized = (parameter instanceof CompoundParameter) ? ((CompoundParameter) parameter).getBaseNormalized() : parameter.getNormalized();
        ...
        if ((this.effect != null) && (parameter == this.effect.enabled)) {
          ...
          for (int i = 0; i < this.knobs.length; ++i) {
            if (parameter == this.knobs[i]) {
              double normalized = (parameter instanceof CompoundParameter) ? ((CompoundParameter) parameter).getBaseNormalized() : this.knobs[i].getNormalized();
              sendControlChange(0, DEVICE_KNOB + i, (int) (normalized * 127));
              break;
            }
          }
  ...
  private class ChannelListener implements LXChannel.Listener, LXBus.ClipListener, LXParameterListener {
      ...
      if (this.channel instanceof LXChannel) {
        ...
        if (p == c.focusedPattern) {
          ...
          if (focusedPatternIndex < channelSurfaceIndex) {
            ...
            if (focusedPatternIndex >= channelSurfaceIndex + CLIP_LAUNCH_ROWS) {
    ...
    @Override public void effectAdded(LXBus channel, LXEffect effect) {
    }

    @Override public void effectRemoved(LXBus channel, LXEffect effect) {
    }

    @Override public void effectMoved(LXBus channel, LXEffect effect) {
    }

    @Override public void indexChanged(LXAbstractChannel channel) {
    }

    @Override public void groupChanged(LXChannel channel, LXGroup group) {
    }

    @Override public void patternAdded(LXChannel channel, LXPattern pattern) {
      sendChannelPatterns(channel.getIndex(), channel);
    }

    @Override public void patternRemoved(LXChannel channel, LXPattern pattern) {
      sendChannelPatterns(channel.getIndex(), channel);
    }

    @Override public void patternMoved(LXChannel channel, LXPattern pattern) {
      sendChannelPatterns(channel.getIndex(), channel);
    }

    @Override public void patternWillChange(LXChannel channel, LXPattern pattern, LXPattern nextPattern) {
      sendChannelPatterns(channel.getIndex(), channel);
    }

    @Override public void patternDidChange(LXChannel channel, LXPattern pattern) {
      sendChannelPatterns(channel.getIndex(), channel);
    }

    @Override public void clipAdded(LXBus bus, LXClip clip) {
      clip.running.addListener(this);
      sendClip(this.channel.getIndex(), this.channel, clip.getIndex(), clip);
    }

    @Override public void clipRemoved(LXBus bus, LXClip clip) {
      clip.running.removeListener(this);
      sendChannelClips(this.channel.getIndex(), this.channel);
    }
  }
  ...
  @Override protected void onEnable(boolean on) {
    if (on) {
  ...
  @Override protected void onReconnect() {
  ...
  private void setApcMode(byte mode) {
    this.output.sendSysex(new byte[] { (byte) 0xf0, 0x47, 0x00, 0x29, 0x60, 0x00, 0x04, mode, 0x09, 0x03, 0x01, (byte) 0xf7 });
  }
  ...
  private void sendChannelPatterns(int index, LXAbstractChannel channelBus) {
    ...
    if (channelBus instanceof LXChannel) {
      ...
      for (int y = 0; y < CLIP_LAUNCH_ROWS; ++y) {
        sendNoteOn(LED_MODE_PRIMARY, CLIP_LAUNCH + CLIP_LAUNCH_COLUMNS * (CLIP_LAUNCH_ROWS - 1 - y) + index, LED_OFF);
      }
    }
  }
  ...
  private void sendClip(int channelIndex, LXAbstractChannel channel, int clipIndex, LXClip clip) {
    if (this.bankOn || this.deviceLockOn || channelIndex >= CLIP_LAUNCH_COLUMNS || clipIndex >= CLIP_LAUNCH_ROWS) {
      return;
    }
    int color = LED_OFF;
    int mode = LED_MODE_PRIMARY;
  ...
  private void sendSwatch(int index) {
    if (!this.deviceLockOn || (index >= PALETTE_SWATCH_COLUMNS)) {
      return;
    }
  ...
  private void noteReceived(MidiNote note, boolean on) {
    ...
    if (pitch >= SCENE_LAUNCH && pitch <= SCENE_LAUNCH_MAX && !this.deviceLockOn) {
  ...
  @Override public void noteOnReceived(MidiNoteOn note) {
    noteReceived(note, true);
  }

  @Override public void noteOffReceived(MidiNote note) {
    noteReceived(note, false);
  }

  @Override public void controlChangeReceived(MidiControlChange cc) {
    int number = cc.getCC();
    switch (number) {
      case TEMPO:
      if (this.deviceLockOn) {
