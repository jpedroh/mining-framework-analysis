...
public class MapItemManager implements Listener {
  ...
  public static ItemStack createMapItem(int mapID, String text, boolean isMapPart, boolean goldTitle) {
    ItemStack mapItem;
    if (goldTitle) {
      mapItem = new ItemStackBuilder(Material.FILLED_MAP).title(ChatColor.GOLD, text).hideAllAttributes().item();
    } else {
      mapItem = new ItemStackBuilder(Material.FILLED_MAP).title(text).hideAllAttributes().item();
    }
  ...
  private static void onItemFrameRemove(ItemFrame frame, Player player, EntityDamageByEntityEvent event) {
    ...
    frame.setItem(new ItemStackBuilder(item).title(getMapTitle(item)).hideAllAttributes().item());
  }

  @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public static void onEntityDamage(EntityDamageByEntityEvent event) {
  ...
  @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public static void onEntityInteract(PlayerInteractEntityEvent event) {
