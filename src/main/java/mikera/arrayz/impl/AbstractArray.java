 ...
 */
public abstract class AbstractArray<T extends java.lang.Object> implements INDArray, Iterable<T> {
  ...
  @Override public void multiply(double d) {
    int n = sliceCount();
    for (int i = 0; i < n; i++) {
      slice(i).multiply(d);
    }
  ...
  @Override public void setElements(double[] values, int offset) {
  ...
  @Override public INDArray ensureMutable() {
  ...
  @Override public INDArray innerProduct(double a) {
  ...
  @Override public INDArray innerProduct(AScalar s) {
    return innerProduct(s.get());
  }

  @Override public INDArray innerProduct(AVector a) {
    return innerProduct((INDArray) a);
  }

  @Override public INDArray outerProduct(INDArray a) {
  ...
  @Override public INDArray getTranspose() {
    return getTransposeCopy();
  }

  @Override public INDArray getTransposeView() {
    throw new UnsupportedOperationException();
  }

  @Override public INDArray getTransposeCopy() {
  ...
  @Override public void scaleAdd(double factor, double constant) {
    multiply(factor);
    add(constant);
  }
  ...
  @Override public INDArray clone() {
    return Arrayz.create(this);
  }

  @Override public INDArray copy() {
    if (!isMutable()) {
      return this;
    }
    return clone();
  ...
  @Override public INDArray scaleCopy(double d) {
    INDArray r = clone();
    r.scale(d);
    return r;
  }

  @Override public INDArray negateCopy() {
    INDArray r = clone();
    r.negate();
    return r;
  }

  @Override public boolean equals(INDArray a) {
    int dims = dimensionality();
  ...
  @Override public boolean equalsArray(double[] data, int offset) {
    ...
    if (dims == 0) {
      ...
      if (dims == 1) {
        ...
        for (int i = 0; i < sc; i++) {
          if (!slice(i).equalsArray(data, offset + i * skip)) {
            return false;
          }
        }
      }
  ...
  @Override public void add(double a) {
    int dims = dimensionality();
    if (dims == 0) {
      set(a + get());
    } else {
  ...
  @Override public void addAt(int i, double v) {
  ...
  @Override public void sub(double a) {
    add(-a);
  }
  ...
  @Override public boolean elementsEqual(double value) {
    ...
    int n = sliceCount();
    for (int i = 0; i < n; i++) {
      if (!slice(i).elementsEqual(value)) {
        return false;
      }
    }
    return true;
  ...
  @Override public double elementSquaredSum() {
    if (dimensionality() == 0) {
      double value = get();
      return value * value;
    }
  ...
  @Override public INDArray reshape(int... targetShape) {
    return Arrayz.createFromVector(asVector(), targetShape);
  }

  @Override public INDArray reorder(int dim, int[] order) {
    int dims = dimensionality();
    if ((dim < 0) || (dim >= dims)) {
  ...
  @Override public INDArray reorder(int[] order) {
    return reorder(0, order);
  }

  @Override public List<?> getSlices() {
    int n = sliceCount();
  ...
  @Override public List<?> getSlices(int dimension) {
  ...
  @Override public INDArray subArray(int[] offsets, int[] shape) {
    int n = dimensionality();
    if (offsets.length != n) {
  ...
  @Override public INDArray mutable() {
    if (isFullyMutable()) {
      return this;
    }
    return clone();
  ...
  @Override public final INDArray mutableClone() {
    return clone();
  }

  @Override public INDArray sparse() {
    int dims = dimensionality();
    if (dims == 0) {
      return this;
    }
  ...
  @Override public INDArray sparseClone() {
    int dims = dimensionality();
    if (dims == 0) {
      return this;
    }
    ...
    if (dims == 2) {
      if (this instanceof AMatrix) {
        return Matrixx.createSparseRows((AMatrix) this);
      }
  ...
  @Override public INDArray broadcastCloneLike(INDArray target) {
    INDArray r = this;
    if (r.dimensionality() < target.dimensionality()) {
