...
public abstract class ASparseIndexedVector extends ASparseVector {
  ...
  @Override public boolean isRangeZero(int start, int length) {
  ...
  @Override public void multiply(double factor) {
    double[] data = internalData();
  ...
  @Override public double magnitudeSquared() {
  ...
  @Override public AVector innerProduct(AMatrix a) {
  ...
  @Override public final double dotProduct(AVector v) {
    if (v instanceof ADenseArrayVector) {
      return dotProduct((ADenseArrayVector) v);
    }
    if (v instanceof ASparseVector) {
      return dotProduct((ASparseVector) v);
    }
    double result = 0.0;
    double[] data = internalData();
  ...
  @Override public int[] nonZeroIndices() {
  ...
  @Override public void addToArray(int offset, double[] array, int arrayOffset, int length) {
  ...
  @Override public void addToArray(double[] dest, int offset) {
    double[] data = internalData();
    int[] ixs = internalIndexArray();
    int dataLength = data.length;
  ...
  @Override public void addToArray(double[] dest, int offset, int stride) {
    double[] data = internalData();
    int[] ixs = internalIndexArray();
    for (int i = 0; i < data.length; i++) {
  ...
  @Override public void addMultipleToArray(double factor, int offset, double[] array, int arrayOffset, int length) {
  ...
  @Override public void addProductToArray(double factor, int offset, AVector other, int otherOffset, double[] array, int arrayOffset, int length) {
  ...
  @Override public void addProductToArray(double factor, int offset, ADenseArrayVector other, int otherOffset, double[] array, int arrayOffset, int length) {
    assert (offset >= 0);
    assert (offset + length <= length());
  ...
  @Override public boolean equalsArray(double[] ds, int offset) {
    double[] data = internalData();
    int[] ixs = internalIndexArray();
    int n = data.length;
    if (n == 0) {
      return DoubleArrays.isZero(ds, offset, length);
    }
    int di = 0;
    int i = 0;
    while (di < n) {
  ...
  @Override public final void getElements(double[] array, int offset) {
