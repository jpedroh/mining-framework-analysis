...
public class SparseIndexedVector extends ASparseIndexedVector {
  ...
  @Override public int nonSparseElementCount() {
    return data.length;
  }
  ...
  @Override public AVector innerProduct(AMatrix m) {
  ...
  @Override public void add(AVector v) {
  ...
  @Override public void add(ADenseArrayVector v) {
    includeIndices(v);
    for (int i = 0; i < data.length; i++) {
      data[i] += v.unsafeGet(index.get(i));
    }
  ...
  @Override public void add(ASparseVector v) {
  ...
  @Override public void sub(AVector v) {
    if (v instanceof ASparseVector) {
      sub((ASparseVector) v);
      return;
    }
  ...
  @Override public void add(double[] src, int offset) {
  ...
  @Override public void multiply(AVector v) {
  ...
  @Override public void multiply(double[] array, int offset) {
    double[] data = this.data;
    int[] ixs = index.data;
    for (int i = 0; i < data.length; i++) {
  ...
  @Override public double maxAbsElement() {
    double[] data = this.data;
    double result = 0.0;
    for (int i = 0; i < data.length; i++) {
  ...
  @Override public int maxElementIndex() {
    double[] data = this.data;
    if (data.length == 0) {
      return 0;
    }
  ...
  @Override public int maxAbsElementIndex() {
    double[] data = this.data;
    if (data.length == 0) {
      return 0;
    }
  ...
  @Override public int minElementIndex() {
    double[] data = this.data;
    if (data.length == 0) {
      return 0;
    }
  ...
  @Override public double unsafeGet(int i) {
    int ip = index.indexPosition(i);
    if (ip < 0) {
      return 0.0;
    }
  ...
  @Override public void setElements(int pos, double[] array, int offset, int length) {
  ...
  @Override public void set(AVector v) {
    checkSameLength(v);
    if (v instanceof ADenseArrayVector) {
  ...
  @Override public void set(ADenseArrayVector v) {
    checkSameLength(v);
    setElements(v.getArray(), v.getArrayOffset());
  }

  @Override public void set(int i, double value) {
    checkIndex(i);
    int ip = index.indexPosition(i);
    if (ip < 0) {
      if (value == 0.0) {
  ...
  @Override public ASparseVector roundToZero(double precision) {
  ...
  @Override public void addAt(int i, double value) {
    if (value == 0.0) {
      return;
    }
    int ip = index.indexPosition(i);
  ...
  @Override public Vector nonSparseValues() {
    return Vector.wrap(data);
  }

  @Override public Index nonSparseIndex() {
    return index;
  }

  @Override public boolean includesIndex(int i) {
  ...
  @Override public SparseIndexedVector clone() {
    return exactClone();
  }
  ...
  @Override public SparseIndexedVector sparseClone() {
    return exactClone();
  }

  @Override public SparseIndexedVector exactClone() {
  ...
  @Override double[] internalData() {
    return data;
  }

  @Override Index internalIndex() {
    return index;
  }
}