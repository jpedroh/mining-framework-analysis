...
public abstract class AMatrix extends AbstractArray<AVector> implements IMatrix {
  ...
  @Override public final double get(int row) {
  ...
  @Override public void set(int row, double value) {
    throw new VectorzException("1D get not supported on matrix!");
  }
  ...
  @Override public void fill(double value) {
    int rc = rowCount();
    int cc = columnCount();
  ...
  @Override public void clamp(double min, double max) {
    int len = rowCount();
    for (int i = 0; i < len; i++) {
  ...
  @Override public void pow(double exponent) {
    int len = rowCount();
    for (int i = 0; i < len; i++) {
  ...
  @Override public void set(int[] indexes, double value) {
    if (indexes.length == 2) {
  ...
  @Override public long elementCount() {
    return ((long) rowCount()) * columnCount();
  }

  @Override public final AVector slice(int row) {
    return getRowView(row);
  }

  @Override public AVector slice(int dimension, int index) {
  ...
  @Override public int sliceCount() {
    return rowCount();
  }

  @Override public List<AVector> getSlices() {
  ...
  @Override public List<INDArray> getSlices(int dimension) {
    if ((dimension < 0) || (dimension >= 2)) {
  ...
  @Override public List<INDArray> getSliceViews() {
  ...
  @Override public INDArray join(INDArray a, int dimension) {
  ...
  @Override public int[] getShapeClone() {
    return new int[] { rowCount(), columnCount() };
  }

  @Override public int getShape(int dim) {
    if (dim == 0) {
      return rowCount();
    } else {
      if (dim == 1) {
  ...
  @Override public long[] getLongShape() {
    return new long[] { rowCount(), columnCount() };
  }

  @Override public double get(int... indexes) {
  ...
  public boolean isIdentity() {
    ...
    for (int i = 0; i < rc; i++) {
      for (int j = 0; j < cc; j++) {
        ...
        if (!(this.unsafeGet(i, j) == expected)) {
          return false;
        }
      }
    }
  ...
  public boolean isOrthogonal() {
    return isSquare() && getTranspose().innerProduct(this).epsilonEquals(IdentityMatrix.create(columnCount()));
  }
  ...
  @Override public INDArray reshape(int... dimensions) {
  ...
  @Override public AMatrix reorder(int[] order) {
    return reorder(0, order);
  }

  @Override public AMatrix reorder(int dim, int[] order) {
  ...
  @Override public AMatrix subArray(int[] offsets, int[] shape) {
    if (offsets.length != 2) {
  ...
  @Override public INDArray rotateView(int dimension, int shift) {
  ...
  @Override public AVector transform(AVector source) {
  ...
  @Override public void transform(AVector source, AVector dest) {
  ...
  @Override public void transformInPlace(AVector v) {
  ...
  @Override public void set(INDArray a) {
    if (a instanceof AMatrix) {
      set((AMatrix) a);
      return;
    }
  ...
  @Override public void setElements(double[] values, int offset, int length) {
  ...
  @Override public void getElements(double[] dest, int offset) {
    int rc = this.rowCount();
    int cc = this.columnCount();
    for (int i = 0; i < rc; i++) {
  ...
  @Override public void copyTo(double[] arr) {
    getElements(arr, 0);
  }

  @Override public void setElements(double[] values) {
    setElements(values, 0, values.length);
  }

  @Override public abstract boolean isFullyMutable();

  @Override public boolean isMutable() {
    return isFullyMutable();
  }

  @Override public boolean isElementConstrained() {
    return false;
  }

  @Override public AMatrix clone() {
    return Matrixx.deepCopy(this);
  }

  @Override public AMatrix copy() {
    if (isMutable()) {
      return clone();
    }
    return this;
  ...
  @Override public AMatrix sparseClone() {
    return Matrixx.createSparse(this);
  }

  @Override public AMatrix ensureMutable() {
    if (isFullyMutable() && !isView()) {
      return this;
    }
    return clone();
  }
  ...
  private double calcDeterminant(int[] inds, int offset) {
    ...
    double det = unsafeGet(offset, inds[offset]) * calcDeterminant(inds, offset + 1);
    for (int i = 1; i < (rc - offset); i++) {
      IntArrays.swap(inds, offset, offset + i);
      det -= unsafeGet(offset, inds[offset]) * calcDeterminant(inds, offset + 1);
      IntArrays.swap(inds, offset, offset + i);
    }
    return det;
  }
  ...
  /**
	 ...
	 * @return
	 */
  @Override public AMatrix getTranspose() {
  ...
  /**
	 ...
	 */
  @Override public AMatrix getTransposeView() {
  ...
  /**
	 ...
	 */
  @Override public AMatrix getTransposeCopy() {
  ...
  @Override public void sub(double d) {
    add(-d);
  }

  @Override public final void scaleAdd(double factor, double constant) {
    multiply(factor);
    add(constant);
  }
  ...
  @Override public Iterator<Double> elementIterator() {
  ...
  @Override public long nonZeroCount() {
    long result = 0;
    int rc = rowCount();
    int cc = columnCount();
    for (int i = 0; i < rc; i++) {
      for (int j = 0; j < cc; j++) {
        if (unsafeGet(i, j) != 0.0) {
          result++;
        }
      }
    }
  ...
  @Override public void reciprocal() {
    int sc = rowCount();
    for (int i = 0; i < sc; i++) {
  ...
  @Override public void addToArray(double[] data, int offset) {
    int cc = columnCount();
    int rc = rowCount();
    for (int i = 0; i < rc; i++) {
  ...
  @Override public boolean epsilonEquals(INDArray a) {
    return epsilonEquals(a, Vectorz.TEST_EPSILON);
  }

  @Override public boolean epsilonEquals(INDArray a, double epsilon) {
    if (a instanceof AMatrix) {
    ...
    if (a.dimensionality() != 2) {
      ...
      for (int i = 0; i < sc; i++) {
        AVector s = getRow(i);
        if (!s.epsilonEquals(a.slice(i), epsilon)) {
          return false;
        }
      }
      return true;
  ...
  public boolean epsilonEquals(AMatrix a, double epsilon) {
    ...
    for (int i = 0; i < sc; i++) {
      AVector s = getRow(i);
      if (!s.epsilonEquals(a.getRow(i), epsilon)) {
        return false;
      }
    }
    return true;
  ...
  @Override public boolean equalsArray(double[] data, int offset) {
    int rc = rowCount();
    int cc = columnCount();
    if (this instanceof IFastRows) {
      for (int i = 0; i < rc; i++) {
        if (!getRow(i).equalsArray(data, offset + i * cc)) {
          return false;
        }
      }
    } else {
  ...
  @Override public boolean elementsEqual(double value) {
    int rc = rowCount();
    if (this instanceof IFastRows) {
      for (int i = 0; i < rc; i++) {
        if (!getRow(i).elementsEqual(value)) {
          return false;
        }
      }
    } else {
  ...
  public boolean equalsTranspose(AMatrix a) {
    ...
    for (int i = 0; i < rc; i++) {
      if (!getRow(i).equals(a.getColumn(i))) {
        return false;
      }
    }
    return true;
  ...
  @Override public boolean equals(INDArray v) {
    if (v instanceof AMatrix) {
      return equals((AMatrix) v);
    }
  ...
  public boolean epsilonEquals(AMatrix a) {
    ...
    if ((this instanceof IFastRows) && (a instanceof IFastRows)) {
      for (int i = 0; i < rc; i++) {
        if (!getRow(i).epsilonEquals(a.getRow(i))) {
          return false;
        }
      }
    } else {
  ...
  public boolean equals(AAffineTransform a) {
    return a.getTranslation().isIdentity() && this.equals(a.getMatrix());
  }

  protected boolean equalsByRows(AMatrix m) {
    int rc = rowCount();
    for (int i = 0; i < rc; i++) {
      if (!getRow(i).equals(m.getRow(i))) {
        return false;
      }
    }
    return true;
  ...
  /**
	 ...
	 * @return
	 */
  @Override public AVector asVector() {
    int rc = rowCount();
    if (rc == 0) {
    ...
    int cc = columnCount();
    if (cc == 1) {
      return getColumnView(0);
    }
    return new MatrixAsVector(this);
  }

  @Override public List<Double> asElementList() {
  ...
  @Override public AVector innerProduct(AVector v) {
  ...
  @Override public AMatrix innerProduct(AScalar s) {
    return innerProduct(s.get());
  }

  @Override public AMatrix innerProduct(double d) {
    AMatrix r = clone();
    r.scale(d);
    return r;
  }
  ...
  public AMatrix transposeInnerProduct(AMatrix s) {
    if (s instanceof Matrix) {
      return transposeInnerProduct((Matrix) s);
    }
    if (isSparse()) {
  ...
  @Override public AMatrix inverse() {
  ...
  @Override public void toDoubleBuffer(DoubleBuffer dest) {
    int n = rowCount();
    for (int i = 0; i < n; i++) {
  ...
  @Override public INDArray[] toSliceArray() {
    int n = sliceCount();
  ...
  @Override public double[] asDoubleArray() {
    return null;
  }

  @Override public void applyOp(Op op) {
    int rc = rowCount();
    for (int i = 0; i < rc; i++) {
  ...
  @Override public void applyOp(IOperator op) {
    if (op instanceof Op) {
      applyOp((Op) op);
      return;
    }
  ...
  @Override public void add(INDArray a) {
    if (a instanceof AMatrix) {
      add((AMatrix) a);
    } else {
      if (a instanceof AVector) {
  ...
  @Override public void multiply(INDArray a) {
    if (a instanceof AMatrix) {
  ...
  @Override public void divide(INDArray a) {
    if (a instanceof AMatrix) {
  ...
  @Override public void divide(double factor) {
    multiply(1.0 / factor);
  }

  @Override public void sub(INDArray a) {
    if (a instanceof AMatrix) {
      sub((AMatrix) a);
    } else {
      if (a instanceof AVector) {
  ...
  public INDArray broadcast(int... targetShape) {
    ...
    if (tdims < 2) {
      ...
      if (2 == tdims) {
        if (rowCount() == targetShape[0] && columnCount() == targetShape[1]) {
          return this;
        }
        throw new IllegalArgumentException(ErrorMessages.incompatibleBroadcast(this, targetShape));
      } else {
  ...
  @Override public INDArray broadcastLike(INDArray target) {
  ...
  @Override public INDArray broadcastCloneLike(INDArray target) {
    INDArray r = this;
  ...
  @Override public boolean isSameShape(INDArray a) {
    if (a instanceof AMatrix) {
      return isSameShape((AMatrix) a);
    }
    if (a.dimensionality() != 2) {
      return false;
    }
    for (int i = 0; i < 2; i++) {
      if (getShape(i) != a.getShape(i)) {
        return false;
      }
    }
    return true;
  ...
  public boolean isSymmetric() {
    ...
    for (int i = 0; i < rc; i++) {
      for (int j = i + 1; j < cc; j++) {
        if (unsafeGet(i, j) != unsafeGet(j, i)) {
          return false;
        }
      }
    }
  ...
  /**
	 ...
	 * @return
	 */
  @Override public AVector getBand(int band) {
  ...
  @Override public abstract AMatrix exactClone();

  @Override public INDArray immutable() {
    if (!isMutable()) {
      return this;
    }
  ...
  @Override public AMatrix mutable() {
    if (isFullyMutable()) {
      return this;
    }
    return clone();
  ...
  @Override public AMatrix sparse() {
  ...
  @Override public INDArray dense() {
  ...
  @Override public INDArray addCopy(INDArray a) {
    if (a instanceof AMatrix) {
  ...
  @Override public AMatrix addCopy(AMatrix a) {
    AMatrix m = this.clone();
    m.add(a);
    return m;
  }
