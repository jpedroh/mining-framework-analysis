...
public class SparseColumnMatrix extends ASparseRCMatrix implements ISparse, IFastColumns {
  ...
  public static SparseColumnMatrix create(AMatrix source) {
    ...
    for (int i = 0; i < cc; i++) {
      ...
      if (!(col.isZero())) {
        data.put(i, Vectorz.createSparse(col));
      }
    }
  ...
  @Override public void unsafeSet(int row, int column, double value) {
  ...
  @Override public AVector getColumn(int i) {
    if ((i < 0) || (i >= cols)) {
  ...
  @Override public AVector getColumnView(int i) {
    return ensureMutableColumn(i);
  }

  @Override public boolean isLowerTriangular() {
    int cc = columnCount();
    for (int i = 1; i < cc; i++) {
  ...
  @Override public void replaceColumn(int i, AVector col) {
    if ((i < 0) || (i >= cols)) {
      throw new IndexOutOfBoundsException(ErrorMessages.invalidSlice(this, 1, i));
    }
    if (col.length() != rows) {
      throw new IllegalArgumentException(ErrorMessages.incompatibleShape(col));
    }
    data.put(i, col);
  }

  @Override public void copyColumnTo(int i, double[] data, int offset) {
    getColumn(i).getElements(data, offset);
  }

  @Override public void copyRowTo(int row, double[] data, int offset) {
  ...
  @Override public SparseRowMatrix getTransposeView() {
  ...
  @Override public Matrix toMatrixTranspose() {
  ...
  @Override public AMatrix transposeInnerProduct(AMatrix a) {
    return getTranspose().innerProduct(a);
  }

  @Override public SparseColumnMatrix exactClone() {
  ...
  @Override public SparseColumnMatrix sparse() {
    return this;
  }
