...
public class SparseRowMatrix extends ASparseRCMatrix implements ISparse, IFastRows {
  ...
  @Override protected int lineLength() {
    return cols;
  }
  ...
  @Override public double unsafeGet(int row, int column) {
    return getRow(row).unsafeGet(column);
  }

  @Override public void unsafeSet(int i, int j, double value) {
  ...
  @Override public void set(AMatrix a) {
    checkSameShape(a);
  ...
  @Override public void setRow(int i, AVector v) {
    data[i] = v.copy();
  }

  @Override public void addAt(int i, int j, double d) {
    if (d == 0.0) {
      return;
    }
    AVector v = unsafeGetVec(i);
  ...
  @Override public void addToArray(double[] targetData, int offset) {
    for (int i = 0; i < rows; ++i) {
      AVector v = unsafeGetVec(i);
      if (v != null) {
        v.addToArray(targetData, offset + cols * i);
      }
    }
  }
  ...
  @Override public List<AVector> getColumns() {
  ...
  @Override public AVector getRow(int i) {
    if ((i < 0) || (i >= rows)) {
  ...
  @Override public AVector getRowView(int i) {
    return ensureMutableRow(i);
  }

  @Override public boolean isUpperTriangular() {
    int rc = rowCount();
    for (int i = 1; i < rc; i++) {
  ...
  @Override public void replaceRow(int i, AVector vec) {
    if ((i < 0) || (i >= rows)) {
      throw new IndexOutOfBoundsException(ErrorMessages.invalidSlice(this, 0, i));
    }
    if (vec.length() != cols) {
  ...
  @Override public void add(AMatrix a) {
    checkSameShape(a);
    int rc = rowCount();
    for (int i = 0; i < rc; i++) {
  ...
  @Override public void copyRowTo(int i, double[] data, int offset) {
  ...
  @Override public void copyColumnTo(int col, double[] targetData, int offset) {
    for (int i = 0; i < rows; ++i) {
      AVector v = unsafeGetVec(i);
  ...
  @Override public SparseColumnMatrix getTransposeView() {
    return SparseColumnMatrix.wrap(data, cols, rows);
  }

  @Override public AMatrix multiplyCopy(double a) {
    int n = lineCount();
  ...
  @Override public AVector innerProduct(AVector a) {
    return transform(a);
  }

  @Override public AVector transform(AVector a) {
  ...
  @Override public AMatrix innerProduct(AMatrix a) {
  ...
  @Override public SparseRowMatrix exactClone() {
  ...
  @Override public void validate() {
    super.validate();
    for (int i = 0; i < rows; i++) {
      if (getRow(i).length() != cols) {
        throw new VectorzException("Invalid column count at row: " + i);
      }
    }
  }

  @Override public boolean equals(AMatrix m) {
    if (m == this) {
      return true;
    }
    if (!isSameShape(m)) {
