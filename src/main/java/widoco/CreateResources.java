...
public class CreateResources {
  ...
  public static void generateDocumentation(String outFolder, Configuration c, File lodeResources) throws Exception {
    ...
    if (c.isIncludeIntroduction()) {
      intro = createIntroductionSection(folderOut + File.separator + "sections", lode.getNamespaceDeclarations(), c, languageFile);
    }
    if (c.isIncludeOverview()) {
      overview = createOverviewSection(folderOut + File.separator + "sections", c, lode.getClassList(), lode.getPropertyList(), lode.getDataPropList(), lode.getAnnotationPropList(), lode.getNamedIndividualList(), languageFile);
    }
  ...
  private static String createChangeLog(String path, Configuration c, Properties lang) {
    ...
    try {
      ...
      if (!c.isIncludeAllSectionsInOneDocument()) {
        saveDocument(path + File.separator + "changelog-" + c.getCurrentLanguage() + ".html", textToWrite, c);
      }
  ...
  private static String createAbstractSection(String path, Configuration c, Properties languageFile) {
    ...
    if ((c.getAbstractPath() != null) && (!"".equals(c.getAbstractPath()))) {
      ...
      if (!c.isIncludeAllSectionsInOneDocument()) {
        saveDocument(path + File.separator + "abstract-" + c.getCurrentLanguage() + ".html", textToWrite, c);
      }
    }
    return textToWrite;
  }

  private static String createIntroductionSection(String path, HashMap<String, String> nsDecl, Configuration c, Properties lang) {
    String textToWrite;
  ...
  private static String createOverviewSection(String path, Configuration c, String classesList, String propList, String dataPropList, String annotationProps, String namedIndividuals, Properties lang) {
    String textToWrite = "";
  ...
  private static String createDescriptionSection(String path, Configuration c, Properties lang) {
    ...
    if ((c.getDescriptionPath() != null) && (!"".equals(c.getDescriptionPath()))) {
      ...
      if (!c.isIncludeAllSectionsInOneDocument()) {
        saveDocument(path + File.separator + "description-" + c.getCurrentLanguage() + ".html", textToWrite, c);
      }
    }
    return textToWrite;
  }

  private static String createCrossReferenceSection(String path, LODEParser lodeParser, Configuration c, Properties lang) {
    String textToWrite = Constants.getCrossReferenceSectionTitleAndPlaceHolder(c, lang);
    String classesList = lodeParser.getClassList(), propList = lodeParser.getPropertyList(), dataPropList = lodeParser.getDataPropList(), annotationPropList = lodeParser.getAnnotationPropList(), namedIndividualList = lodeParser.getNamedIndividualList();
  ...
  private static String createReferencesSection(String path, Configuration c, Properties lang) {
    ...
    if ((c.getReferencesPath() != null) && (!"".equals(c.getReferencesPath()))) {
      ...
      if (!c.isIncludeAllSectionsInOneDocument()) {
        saveDocument(path + File.separator + "references-" + c.getCurrentLanguage() + ".html", textToWrite, c);
      }
    }
    return textToWrite;
  }
  ...
  private static void createUnifiedIndexDocument(String abstractText, String introText, String overviewText, String descriptionText, String crossrefText, String refText, String changeLogText, String path, Configuration c, LODEParser l, Properties lang) {
    String textToWrite = Constants.getUnifiedIndexDocument("resources", c, l, lang, abstractText, introText, overviewText, descriptionText, refText, changeLogText, crossrefText);
    saveDocument(path + File.separator + "index-" + c.getCurrentLanguage() + ".html", textToWrite, c);
  }
