...
public class XSD2OWLMapper {
  ...
  private OntClass convertSimpleType(XSSimpleType simple, String parentURI) {
    ...
    if (simple.isGlobal()) {
      if (NS.equals(XSD.getURI())) {
        OntClass dataType = ontology.createOntResource(OntClass.class, RDFS.Datatype, parentURI + Constants.DATATYPE_SUFFIX);
        ...
        if (parentURI != null) {
          OntClass datatype = ontology.createOntResource(OntClass.class, RDFS.Datatype, parentURI + Constants.DATATYPE_SUFFIX);
          Resource onDatatype = ontology.getEnumeratedClass(URI);
          if (onDatatype != null) {
  ...
  private OntClass convertEnumeration(EnumeratedClass enumClass, XSSimpleType simple, SimpleTypeRestriction facets) {
    ...
    if (baseNS.equals(XSD.getURI())) {
      enumClass.addSuperClass(ontology.createAllValuesFromRestriction(null, hasValue, XSDUtil.getXSDResource(base.getName())));
    ...
    for (int i = 0, length = facets.enumeration.length; i < length; i++) {
      String memberURI = enumClass.getURI() + "_" + facets.enumeration[i].replace('%', '_').replace(' ', '_').replace('[', '_').replace(']', '_');
  ...
  private OntClass convertRestriction(XSSimpleType simple, String URI) {
    ...
    if (baseType.getTargetNamespace().equals(XSDDatatype.XSD)) {
      ...
      if (baseFacets.enumeration != null) {
  ...
  private OntClass convertComplexType(XSComplexType complex, String parentURI) {
    ...
    if (baseType.isSimpleType()) {
      if (ontology.getOntResource(baseURI) != null && !baseType.getTargetNamespace().equals(XSDDatatype.XSD)) {
        ...
        if (baseType.getTargetNamespace().equals(XSDDatatype.XSD)) {
          complexClass.addSuperClass(ontology.createAllValuesFromRestriction(null, hasValue, XSDUtil.getXSDResource(baseType.getName())));
        } else {
          if (ontology.getOntResource(baseURI + Constants.DATATYPE_SUFFIX) != null) {
            complexClass.addSuperClass(ontology.createAllValuesFromRestriction(null, hasValue, ontology.getOntResource(baseURI + Constants.DATATYPE_SUFFIX)));
          }
        }
      ...
      if (baseType.isComplexType()) {
        if (complex.getDerivationMethod() == XSType.EXTENSION) {
          ...
          if (complex.getDerivationMethod() == XSType.RESTRICTION && baseURI.equals("http://www.w3.org/2001/XMLSchema#anyType")) {
  ...
  private void convertAttribute(XSAttributeUse attributeUse, OntClass complexClass) {
    ...
    if (NS.equals(XSDDatatype.XSD)) {
      if (attribute.getType().getName().equals("IDREFS") || attribute.getType().getName().equals("ENTITIES") || attribute.getType().getName().equals("NMTOKENS")) {
        return;
      }
    }
    ...
    if (NS.equals(XSDDatatype.XSD)) {
      ontology.createAllValuesFromRestriction(null, prop, XSDUtil.getXSDResource(attribute.getType().getName())).addSubClass(complexClass);
    } else {
      if (ontology.getOntResource(URI + Constants.DATATYPE_SUFFIX) != null) {
        ontology.createAllValuesFromRestriction(null, prop, ontology.getOntResource(URI + Constants.DATATYPE_SUFFIX)).addSubClass(complexClass);
      } else {
        if (ontology.getOntResource(URI) != null) {
        ...
        if (element.getType().isSimpleType()) {
          ...
          if (element.getType().getTargetNamespace().equals(XSDDatatype.XSD)) {
            if (element.getType().getName().equals("anyType")) {
              ...
              parent.addSuperClass(ontology.createAllValuesFromRestriction(null, prop, XSDUtil.getXSDResource(element.getType().getName())));
            }
          } else {
            parent.addSuperClass(ontology.createAllValuesFromRestriction(null, prop, ontology.getResource(getURI(element.getType()) + Constants.DATATYPE_SUFFIX)));
          ...
          if (element.getType().isComplexType()) {
            ...
            if (element.getType().getTargetNamespace().equals(XSDDatatype.XSD)) {
              ...
              if (element.getType().isGlobal()) {
                parent.addSuperClass(ontology.createAllValuesFromRestriction(null, prop, ontology.createResource(getURI(element.getType()))));
              } else {
  ...
  private RDFList getFacetList(SimpleTypeRestriction facets, XSRestrictionSimpleType restriction) {
    ...
    if (facets.maxInclusive != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "maxInclusive"), facets.maxInclusive, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.minInclusive != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "minInclusive"), facets.minInclusive, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.maxExclusive != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "maxExclusive"), facets.maxExclusive, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.minExclusive != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "minExclusive"), facets.minExclusive, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.length != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "length"), facets.length, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.maxLength != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "maxLength"), facets.maxLength, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.minLength != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "minLength"), facets.minLength, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.pattern != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "pattern"), facets.pattern, XSDDatatype.XSDstring));
    }
    if (facets.totalDigits != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "totalDigits"), facets.totalDigits, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.fractionDigits != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "fractionDigits"), facets.fractionDigits, XSDUtil.getXSDDatatype(name)));
    }
    if (facets.whiteSpace != null) {
      list = list.cons(ontology.createResource().addProperty(ontology.createProperty(XSD.getURI() + "whiteSpace"), facets.whiteSpace, XSDDatatype.XSDstring));
    }
    return list;
  }
