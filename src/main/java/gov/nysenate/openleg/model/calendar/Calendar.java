...
public class Calendar extends BaseLegislativeContent {
  ...
  public Set<CalendarEntryListId> getEntryListIds() {
    ...
    supplementalMap.values().stream().map(CalendarSupplemental::getCalendarSupplementalId).map(CalendarEntryListId::new).forEach(entryListIds::add);
    activeListMap.values().stream().map(CalendarActiveList::getCalendarActiveListId).map(CalendarEntryListId::new).forEach(entryListIds::add);
    return entryListIds;
  }
  ...
  @Override public boolean equals(Object obj) {
    ...
    return Objects.equals(this.id, other.id) && Objects.equals(this.supplementalMap, other.supplementalMap) && Objects.equals(this.activeListMap, other.activeListMap) && Objects.equals(this.publishedDateTime, other.publishedDateTime);
  }
  ...
  public List<CalendarEntryListId> getCalendarEntryListIds() {
    ...
    calendarEntryListIds.addAll(this.getSupplementalMap().values().stream().map(CalendarSupplemental::getCalendarSupplementalId).map(CalendarSupplementalId::toCalendarEntryListId).collect(Collectors.toList()));
    calendarEntryListIds.addAll(this.getActiveListMap().values().stream().map(CalendarActiveList::getCalendarActiveListId).map(CalendarActiveListId::toCalendarEntryListId).collect(Collectors.toList()));
