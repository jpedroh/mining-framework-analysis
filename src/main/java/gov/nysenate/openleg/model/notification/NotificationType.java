...
public enum NotificationType {
  ...
  private static final ImmutableSet<NotificationType> ALL_NOTIFICATION_TYPES = ImmutableSet.copyOf(NotificationType.values());

  @SuppressWarnings(value = { "IncompleteCopyConstructor" }) NotificationType(NotificationType parent) {
    this.parent = parent;
    if (parent != null) {
  ...
  public static Set<NotificationType> getCoverage(NotificationType type) {
    ...
    type.children.stream().map(NotificationType::getCoverage).forEach(result::addAll);
    return result;
  }
