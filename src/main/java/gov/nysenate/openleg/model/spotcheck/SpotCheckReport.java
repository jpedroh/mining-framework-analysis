 ...
 * @param <ContentKey>
 */
public class SpotCheckReport<ContentKey extends java.lang.Object> {
  /** Auto increment id */
  private int id;
  ...
  public long getOpenMismatchCount(boolean ignored) {
    return observations.values().stream().map((obs) -> obs.getMismatches().values().stream().filter((mismatch) -> !mismatch.isIgnored() ^ ignored).filter((mismatch) -> mismatch.getState() == MismatchState.OPEN).count()).reduce(0L, (a, b) -> a + b);
  }
  ...
  public Map<MismatchState, Long> getMismatchStatusCounts(boolean ignored) {
    if (observations != null) {
      ...
      observations.values().stream().flatMap((e) -> e.getMismatchStatusCounts(ignored).entrySet().stream()).forEach((e) -> counts.merge(e.getKey(), e.getValue(), Long::sum));
      return counts;
    }
  ...
  public Map<SpotCheckMismatchType, Map<MismatchState, Long>> getMismatchTypeStatusCounts(boolean ignored) {
    if (observations != null) {
      ...
      observations.values().stream().flatMap((e) -> e.getMismatchStatusTypes(ignored).entrySet().stream()).forEach((e) -> {
  ...
  public Map<MismatchState, Map<SpotCheckMismatchType, Long>> getMismatchStatusTypeCounts(boolean ignored) {
    if (observations != null) {
      ...
      observations.values().stream().flatMap((obs) -> obs.getMismatchStatusTypes(ignored).entrySet().stream()).forEach((entry) -> {
        long currentValue = Optional.ofNullable(countTable.get(entry.getValue(), entry.getKey())).orElse(0l);
  ...
  public Map<SpotCheckMismatchType, Long> getMismatchTypeCounts(boolean ignored) {
    if (observations != null) {
      ...
      observations.values().stream().flatMap((e) -> e.getMismatchTypes(ignored).stream()).forEach((e) -> counts.merge(e, 1L, (a, b) -> a + 1L));
      return counts;
    }
    throw new IllegalStateException("The observations on this report have not yet been set.");
  }
