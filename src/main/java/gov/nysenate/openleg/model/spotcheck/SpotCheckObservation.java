 ...
 */
public class SpotCheckObservation<ContentKey extends java.lang.Object> {
  ...
  /**
     ...
     */
  public static <ContentKey extends java.lang.Object> SpotCheckObservation<ContentKey> getRefMissingObs(SpotCheckReferenceId referenceId, ContentKey key) {
  ...
  /**
     ...
     * @return {@link SpotCheckObservation}
     */
  public static <ContentKey extends java.lang.Object> SpotCheckObservation<ContentKey> getObserveDataMissingObs(SpotCheckReferenceId referenceId, ContentKey key) {
    SpotCheckObservation<ContentKey> obs = new SpotCheckObservation<>(referenceId, key);
  ...
  public Map<MismatchState, Long> getMismatchStatusCounts(boolean ignored) {
    if (mismatches != null) {
      return mismatches.values().stream().filter((mismatch) -> !mismatch.isIgnored() ^ ignored).collect(Collectors.groupingBy(SpotCheckMismatch::getState, Collectors.counting()));
    } else {
  ...
  public Map<SpotCheckMismatchType, MismatchState> getMismatchStatusTypes(boolean ignored) {
    if (mismatches != null) {
      return mismatches.values().stream().filter((mismatch) -> !mismatch.isIgnored() ^ ignored).collect(Collectors.toMap(SpotCheckMismatch::getMismatchType, SpotCheckMismatch::getState));
    } else {
      throw new IllegalStateException("Collection of mismatches is null");
    }
  }
  ...
  public Set<SpotCheckMismatchType> getMismatchTypes(boolean ignored) {
    if (mismatches != null) {
      return mismatches.values().stream().filter((mismatch) -> !mismatch.isIgnored() ^ ignored).map(SpotCheckMismatch::getMismatchType).collect(Collectors.toSet());
    } else {
      throw new IllegalStateException("Collection of mismatches is null");
    }
  }
  ...
  private void validateMismatch(SpotCheckMismatch mismatch) {
    ...
    if (!referenceType.checkedMismatchTypes().contains(mismatch.getMismatchType())) {
      throw new IllegalArgumentException(mismatch.getMismatchType() + " mismatches cannot be reported in " + referenceType + " observations.");
    }
  }
