...
public class AgendaCommAddendumView implements ViewObject {
  ...
  public AgendaCommAddendumView(String addendumId, LocalDateTime modDateTime, AgendaInfoCommittee infoComm, AgendaVoteCommittee voteComm, BillDataService billDataService) {
    this.addendumId = addendumId;
    if (infoComm != null) {
      this.modifiedDateTime = modDateTime;
      this.meeting = new AgendaMeetingView(infoComm.getChair(), infoComm.getLocation(), infoComm.getMeetingDateTime(), infoComm.getNotes());
      this.bills = ListView.of(infoComm.getItems().stream().map((i) -> new AgendaItemView(i, billDataService)).collect(toList()));
  ...
  @JsonIgnore public CommitteeAgendaAddendumId getCommitteeAgendaAddendumId() {
