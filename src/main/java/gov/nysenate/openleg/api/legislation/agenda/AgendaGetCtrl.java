...
@RestController @RequestMapping(value = BASE_API_PATH + "/agendas", method = RequestMethod.GET) public class AgendaGetCtrl extends BaseCtrl {
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{year:\\d{4}}") public BaseResponse getAgendas(@PathVariable int year) {
    List<AgendaId> agendaIds = agendaData.getAgendaIds(year, SortOrder.ASC);
    return ListViewResponse.of(agendaIds.stream().map((aid) -> new AgendaSummaryView(agendaData.getAgenda(aid))).collect(Collectors.toList()), agendaIds.size(), LimitOffset.ALL);
  }
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{year:[\\d]{4}}/{agendaNo}") public BaseResponse getAgenda(@PathVariable int year, @PathVariable int agendaNo) {
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{weekOf:\\d{4}-\\d{2}-\\d{2}}") public ViewObjectResponse<AgendaView> getAgenda(@PathVariable String weekOf) {
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{year:[\\d]{4}}/{agendaNo}/{commName}") public BaseResponse getAgenda(@PathVariable int year, @PathVariable int agendaNo, @PathVariable String commName) {
    Agenda agenda = agendaData.getAgenda(new AgendaId(agendaNo, year));
    ...
    if (agenda.hasCommittee(committeeId)) {
      ...
      return new ViewObjectErrorResponse(ErrorCode.INVALID_ARGUMENTS, commName + " is not contained within agenda " + agendaNo + " (" + year + ")");
    }
  }
  ...
  /**
     ...
     */
  @RequestMapping(value = "/meetings/{from}/{to}") public BaseResponse getAgendaMeetings(@PathVariable String from, @PathVariable String to) throws SearchException {
    ...
    results.getResults().forEach((r) -> {
      ...
      agendaData.getAgenda(commAgendaId.getAgendaId()).getAgendaInfoAddenda().values().forEach((addn) -> {
        if (addn.getCommitteeInfoMap().containsKey(commAgendaId.getCommitteeId())) {
          meetingViews.add(new AgendaMeetingDetailView(commAgendaId, addn.getCommittee(commAgendaId.getCommitteeId()), addn.getId(), addn.getWeekOf()));
        }
      });
    });
    return DateRangeListViewResponse.of(meetingViews, getClosedRange(fromDateTime, toDateTime, "from", "to"), meetingViews.size(), LimitOffset.ALL);
  }

  /** --- Exception Handlers --- */
  @ExceptionHandler(value = AgendaNotFoundEx.class) @ResponseStatus(value = HttpStatus.NOT_FOUND) public ViewObjectErrorResponse agendaNotFoundHandler(AgendaNotFoundEx ex) {
