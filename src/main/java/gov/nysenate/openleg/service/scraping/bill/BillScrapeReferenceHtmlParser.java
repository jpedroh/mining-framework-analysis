 ...
 */
@Service public class BillScrapeReferenceHtmlParser {
  ...
  public String parseMemo(Document doc) {
    Element memoElement = doc.select("pre:last-of-type").first();
    if (memoElement != null) {
  ...
  private SortedSetMultimap<BillVoteCode, String> parseVote(Element voteTable) {
    ...
    for (int i = 0; i < entries.size(); i = i + 2) {
      votes.put(BillVoteCode.getValue(elementText(entries.get(i))), elementText(entries.get(i + 1)));
    }
    return votes;
  }
