...
@Service public class SpotcheckRunService {
  ...
  private <T extends java.lang.Object> void runReport(SpotCheckReportService<T> reportService, Range<LocalDateTime> reportRange) {
    ...
    try {
      ...
      int notesCutoff = 140;
      logger.info("Saving {} report. obs: {} mm: {}({}ig.) notes: {}", report.getReferenceType(), report.getObservedCount(), report.getOpenMismatchCount(false), report.getOpenMismatchCount(true), StringUtils.abbreviate(report.getNotes(), notesCutoff));
