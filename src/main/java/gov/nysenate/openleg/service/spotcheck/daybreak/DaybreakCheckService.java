...
@Service(value = "daybreak") public class DaybreakCheckService extends BaseSpotCheckService<BaseBillId, Bill, DaybreakBill> {
  private static final Logger logger = LoggerFactory.getLogger(DaybreakCheckService.class);

  @Autowired protected DaybreakDao daybreakDao;

  /** {@inheritDoc}
     *  Just use the latest daybreak files we have. */
  @Override public SpotCheckObservation<BaseBillId> check(Bill bill) throws ReferenceDataNotFoundEx {
    return check(bill, DateUtils.LONG_AGO.atStartOfDay(), LocalDateTime.now());
  }

  /** {@inheritDoc} */
  @Override public SpotCheckObservation<BaseBillId> check(Bill bill, LocalDateTime start, LocalDateTime end) throws ReferenceDataNotFoundEx {
    if (bill == null) {
  ...
  /** {@inheritDoc} */
  @Override public SpotCheckObservation<BaseBillId> check(Bill bill, DaybreakBill daybreakBill) {
  ...
  private void checkBillActions(Bill bill, DaybreakBill daybreakBill, SpotCheckObservation<BaseBillId> obsrv) {
    if (daybreakBill.getActions() != null && !daybreakBill.getActions().equals(bill.getActions())) {
      if (!daybreakBill.getActions().isEmpty()) {
        ...
        if (StringUtils.containsIgnoreCase(lastAction.getText(), "SUBSTITUTED BY") && bill.getActions().containsAll(daybreakBill.getActions())) {
          return;
        }
      }
  ...
  private List<String> prepareShortnames(Collection<String> shortnames) {
    return shortnames.stream().map((sn) -> sn.replaceAll("[\\(\\)]+", "")).map(StringUtils::upperCase).sorted().collect(toList());
  }
  ...
  private void checkMultiSponsors(Bill bill, DaybreakBill daybreakBill, SpotCheckObservation<BaseBillId> obsrv) {
    Optional<BillAmendment> activeAmendmentOpt = bill.hasActiveAmendment() ? Optional.of(bill.getActiveAmendment()) : Optional.empty();
    List<String> dataMuSponsors = activeAmendmentOpt.map(BillAmendment::getMultiSponsors).orElse(Collections.emptyList()).stream().map(SessionMember::getLbdcShortName).collect(collectingAndThen(toList(), this::prepareShortnames));
  ...
  private void checkCoSponsors(Bill bill, DaybreakBill daybreakBill, SpotCheckObservation<BaseBillId> obsrv) {
    Optional<BillAmendment> activeAmendmentOpt = bill.hasActiveAmendment() ? Optional.of(bill.getActiveAmendment()) : Optional.empty();
    List<String> dataCoSponsors = activeAmendmentOpt.map(BillAmendment::getCoSponsors).orElse(Collections.emptyList()).stream().map(SessionMember::getLbdcShortName).collect(collectingAndThen(toList(), this::prepareShortnames));
  ...
  private void checkBillLawAndSummary(Bill bill, DaybreakBill daybreakBill, SpotCheckObservation<BaseBillId> obsrv) {
    String billLawSummary = (Strings.nullToEmpty(bill.getAmendment(bill.getActiveVersion()).getLaw()) + " " + Strings.nullToEmpty(bill.getSummary())).trim();
  ...
  private String actionsListString(List<BillAction> actions) {
    String actionsStr = "";
    if (actions != null) {
      actionsStr = actions.stream().map((a) -> a.getChamber() + " - " + DateUtils.LRS_ACTIONS_DATE.format(a.getDate()) + " " + a.getText()).collect(joining("\n"));
    }
  ...
  private String publishedVersionsString(Bill bill) {
    Set<Version> publishedVersionSet = bill.getAmendPublishStatusMap().entrySet().stream().filter((entry) -> entry.getValue().isPublished()).map(Map.Entry::getKey).collect(toSet());
  ...
  private String publishedVersionsString(Set<Version> publishedVersions) {
    return publishedVersions.stream().sorted().map(Version::name).collect(joining(" "));
  }
}