 ...
 * Bill retrieval APIs
 */
@RestController @RequestMapping(value = BASE_API_PATH + "/bills", method = RequestMethod.GET, produces = APPLICATION_JSON_VALUE) public class BillGetCtrl extends BaseCtrl {
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{sessionYear:[\\d]{4}}") public BaseResponse getBills(@PathVariable int sessionYear, @RequestParam(defaultValue = "publishedDateTime:asc") String sort, @RequestParam(defaultValue = "false") boolean full, @RequestParam(defaultValue = "false") boolean idsOnly, WebRequest webRequest) throws SearchException {
    LimitOffset limOff = getLimitOffset(webRequest, 50);
    SearchResults<BaseBillId> results = billSearch.searchBills(SessionYear.of(sessionYear), sort, limOff);
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{sessionYear:[\\d]{4}}/{printNo}") public BaseResponse getBill(@PathVariable int sessionYear, @PathVariable String printNo, WebRequest request) {
    ...
    switch (level) {
      ...
      break;
      case WITH_REFS:
      viewObject = new DetailBillView(billData.getBill(baseBillId), billData);
      break;
      case NO_FULLTEXT:
      ...
      default:
      viewObject = new BillView(billData.getBill(baseBillId));
    }
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{sessionYear:[\\d]{4}}/{printNo}.pdf") public ResponseEntity<byte[]> getBillPdf(@PathVariable int sessionYear, @PathVariable String printNo) throws Exception {
  ...
  /**
     ...
     */
  @RequestMapping(value = "/{sessionYear:[\\d]{4}}/{printNo}/diff/{version1}/{version2}") public BaseResponse getBillDiff(@PathVariable int sessionYear, @PathVariable String printNo, @PathVariable String version1, @PathVariable String version2) {
    ...
    return new ViewObjectResponse<>(new BillDiffView(new BaseBillIdView(baseBillId), amend1.getVersion().toString(), amend2.getVersion().toString(), prettyHtml));
  }

  /** --- Exception Handlers --- */
  @ExceptionHandler(value = BillNotFoundEx.class) @ResponseStatus(value = HttpStatus.NOT_FOUND) public ViewObjectErrorResponse billNotFoundHandler(BillNotFoundEx ex) {
    return new ViewObjectErrorResponse(ErrorCode.BILL_NOT_FOUND, new BillIdView(ex.getBillId()));
  }

  @ExceptionHandler(value = BillAmendNotFoundEx.class) @ResponseStatus(value = HttpStatus.NOT_FOUND) public ViewObjectErrorResponse billAmendNotFoundHandler(BillAmendNotFoundEx ex) {
