 ...
 */
@Service public class CachedLawDataService extends CachingService<LawVersionId, LawTree> implements LawDataService {
  ...
  @Override public Map<LawVersionId, LawTree> initialEntries() {
    return lawDataDao.getLawInfos().stream().map((lawInfo) -> lawDataDao.getLawTree(lawInfo.getLawId(), LocalDate.now())).collect(Collectors.toMap(LawTree::getLawVersionId, (lawTree) -> lawTree));
  }

  /** {@inheritDoc} */
  @Override public List<LawInfo> getLawInfos() {
    return lawDataDao.getLawInfos().stream().sorted().collect(toList());
  }

  /** {@inheritDoc} */
  @Override public LawTree getLawTree(String lawId, LocalDate endPublishedDate) throws LawTreeNotFoundEx {
    if (lawId == null) {
  ...
  /** {@inheritDoc} */
  @Override public LawDocInfo getLawDocInfo(String documentId, LocalDate endPublishedDate) throws LawDocumentNotFoundEx {
    if (documentId == null || documentId.length() < 4) {
      throw new IllegalArgumentException("Document id cannot be less than 4 characters");
    }
    Optional<LawTreeNode> node = getLawTree(documentId.substring(0, 3), endPublishedDate).find(documentId);
    if (node.isPresent()) {
  ...
  /** {@inheritDoc} */
  @Override public LawDocument getLawDocument(String documentId, LocalDate endPublishedDate) throws LawDocumentNotFoundEx {
    if (documentId == null) {
      throw new IllegalArgumentException("Supplied documentId cannot be null");
    }
  ...
  /** {@inheritDoc} */
  @Override public Map<String, LawDocument> getLawDocuments(String lawId, LocalDate endPublishedDate) {
    if (lawId == null) {
      throw new IllegalArgumentException("Supplied lawId cannot be null");
    }
    if (endPublishedDate == null) {
  ...
  /** {@inheritDoc} */
  @Override public Set<RepealedLawDocId> getRepealedLawDocs(Range<LocalDate> dateRange) {
    return new HashSet<>(lawDataDao.getRepealedLaws(dateRange));
  }

  /** {@inheritDoc} */
  @Override public void saveLawTree(LawFile lawFile, LawTree lawTree) {
    if (lawTree == null) {
  ...
  /** {@inheritDoc} */
  @Override public void saveLawDocument(LawFile lawFile, LawDocument lawDocument) {
    if (lawDocument == null) {
