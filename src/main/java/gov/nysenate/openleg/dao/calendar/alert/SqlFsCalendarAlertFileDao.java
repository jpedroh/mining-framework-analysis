...
@Repository public class SqlFsCalendarAlertFileDao extends SqlBaseDao {
  private static final Logger logger = LoggerFactory.getLogger(SqlFsCalendarAlertFileDao.class);

  private static final Pattern calendarAlertFilePattern = Pattern.compile("^(floor_cal|active_list)_alert-\\d{4}-\\d+(-\\d+)?[A-Z]?-\\d{8}T\\d{6}.html$");
  ...
  public List<CalendarAlertFile> getIncomingCalendarAlerts() throws IOException {
    List<File> files = FileIOUtils.safeListFiles(incomingCalendarAlertDir, null, false).stream().filter((file) -> calendarAlertFilePattern.matcher(file.getName()).matches()).collect(Collectors.toList());
  ...
  protected class CalendarAlertFileRowMapper implements RowMapper<CalendarAlertFile> {
    @Override public CalendarAlertFile mapRow(ResultSet rs, int rowNum) throws SQLException {
