...
@Repository public class SqlBillDao extends SqlBaseDao implements BillDao {
  ...
  /**
     * {@inheritDoc}
     */
  @Override public Bill getBill(BillId billId) {
    ...
    for (BillAmendment amendment : billAmendments) {
      final ImmutableParams amendParams = baseParams.add(new MapSqlParameterSource("version", amendment.getVersion().getValue()));
  ...
  /**
     * {@inheritDoc}
     */
  @Override public BillInfo getBillInfo(BillId billId) throws DataAccessException {
  ...
  /**
     * {@inheritDoc}
     */
  @Override public void applyText(Bill strippedBill) throws DataAccessException {
  ...
  /**
     ...
     */
  @Override public void updateBill(Bill bill, SobiFragment sobiFragment) {
  ...
  /**
     * {@inheritDoc}
     */
  @Override public List<BaseBillId> getBillIds(SessionYear sessionYear, LimitOffset limOff, SortOrder billIdSort) throws DataAccessException {
    ...
    return jdbcNamed.query(SqlBillQuery.SELECT_BILL_IDS_BY_SESSION.getSql(schema(), orderBy, limOff), params, (rs, row) -> new BaseBillId(rs.getString("bill_print_no"), rs.getInt("bill_session_year")));
  }

  /**
     * {@inheritDoc}
     */
  @Override public int getBillCount() throws DataAccessException {
  ...
  /**
     * {@inheritDoc}
     */
  @Override public int getBillCount(SessionYear sessionYear) throws DataAccessException {
    ImmutableParams params = ImmutableParams.from(new MapSqlParameterSource("sessionYear", sessionYear.getYear()));
    return jdbcNamed.queryForObject(SqlBillQuery.SELECT_COUNT_ALL_BILLS_IN_SESSION.getSql(schema()), params, (rs, row) -> rs.getInt("total"));
  }

  /**
     * {@inheritDoc}
     */
  @Override public String getAlternateBillPdfUrl(BillId billId) {
    SqlParameterSource params = getBillIdParams(billId);
    return jdbcNamed.queryForObject(SqlBillQuery.SELECT_ALTERNATE_PDF_URL.getSql(schema()), params, (rs, row) -> rs.getString("url_path"));
  }

  /**
     * {@inheritDoc}
     */
  @Override public Range<SessionYear> activeSessionRange() {
    if (getBillCount() == 0) {
      throw new EmptyResultDataAccessException("No active session range since there are " + "no bills in the database!", 1);
    }
    return jdbc.queryForObject(SqlBillQuery.ACTIVE_SESSION_YEARS.getSql(schema()), (rs, row) -> Range.closed(SessionYear.of(rs.getInt("min")), SessionYear.of(rs.getInt("max"))));
  }
  ...
  public BillId getDirectPrevVersion(ImmutableParams baseParams) {
    List<BillId> billIds = jdbcNamed.query(SqlBillQuery.SELECT_BILL_PREVIOUS_VERSIONS.getSql(schema(), new OrderBy("bill_session_year", SortOrder.DESC)), baseParams, new BillPreviousVersionRowMapper());
    if (billIds.size() == 0) {
      return null;
    }
  ...
  public Set<BillId> getAllPreviousVersions(ImmutableParams baseParams) {
    ...
    return new TreeSet<>(jdbcNamed.query(SqlBillQuery.SELECT_ALL_BILL_PREVIOUS_VERSIONS.getSql(schema(), orderBy, LimitOffset.ALL), baseParams, new BillPreviousVersionRowMapper()));
  }
  ...
  public BillSponsor getBillSponsor(ImmutableParams baseParams) {
    try {
      return jdbcNamed.queryForObject(SqlBillQuery.SELECT_BILL_SPONSOR.getSql(schema()), baseParams, new BillSponsorRowMapper(memberService));
  ...
  public List<SessionMember> getAdditionalSponsors(ImmutableParams baseParams) {
    try {
      OrderBy orderBy = new OrderBy("sequence_no", SortOrder.ASC);
      return jdbcNamed.query(SqlBillQuery.SELECT_ADDTL_BILL_SPONSORS.getSql(schema(), orderBy, LimitOffset.ALL), baseParams, new BillMemberRowMapper(memberService));
    } catch (EmptyResultDataAccessException ex) {
  ...
  public LinkedList<BillStatus> getBillMilestones(ImmutableParams baseParams) {
    ...
    return new LinkedList<>(jdbcNamed.query(SqlBillQuery.GET_BILL_MILESTONES.getSql(schema(), orderBy, LimitOffset.ALL), baseParams, (rs, rowNum) -> {
  ...
  public List<CommitteeAgendaId> getCommitteeAgendas(ImmutableParams baseParams) {
    ...
    return jdbcNamed.query(SqlBillQuery.SELECT_COMM_AGENDA_IDS.getSql(schema(), orderBy, LimitOffset.ALL), baseParams, (rs, rowNum) -> new CommitteeAgendaId(new AgendaId(rs.getInt("agenda_no"), rs.getInt("year")), new CommitteeId(Chamber.SENATE, rs.getString("committee_name"))));
  }
  ...
  public List<CalendarId> getCalendars(ImmutableParams baseParams) {
    ...
    return jdbcNamed.query(SqlBillQuery.SELECT_CALENDAR_IDS.getSql(schema(), orderBy, LimitOffset.ALL), baseParams, (rs, rowNum) -> {
  ...
  protected void updateBillSameAs(BillAmendment amendment, SobiFragment sobiFragment, ImmutableParams amendParams) {
    ...
    if (!existingSameAs.equals(amendment.getSameAs())) {
      ...
      newSameAs.removeAll(existingSameAs);
      existingSameAs.removeAll(amendment.getSameAs());
  ...
  protected void updateActions(Bill bill, SobiFragment sobiFragment, ImmutableParams billParams) {
    ...
    newBillActions.removeAll(existingBillActions);
    existingBillActions.removeAll(bill.getActions());
  ...
  protected void updateBillCommittees(Bill bill, SobiFragment sobiFragment, ImmutableParams billParams) {
    ...
    if (!existingComms.equals(bill.getPastCommittees())) {
      ...
      newComms.removeAll(existingComms);
      existingComms.removeAll(bill.getPastCommittees());
  ...
  protected void updateBillMilestones(Bill bill, SobiFragment sobiFragment, ImmutableParams billParams) {
    ...
    if (!existingMilestones.equals(newMilestones)) {
      ...
      for (BillStatus status : newMilestones) {
        jdbcNamed.update(SqlBillQuery.INSERT_BILL_MILESTONE.getSql(schema()), getMilestoneParams(bill, status, rank++, sobiFragment));
      }
    }
  }
  ...
  protected void updateBillAmendPublishStatus(Bill bill, SobiFragment sobiFragment, ImmutableParams billParams) {
    ...
    diff.entriesOnlyOnLeft().forEach((version, pubStatus) -> {
      if (!pubStatus.isOverride() && pubStatus.isPublished()) {
        LocalDateTime dateTime = (sobiFragment != null) ? sobiFragment.getPublishedDateTime() : LocalDateTime.now();
  ...
  protected void updateBillCosponsor(BillAmendment billAmendment, SobiFragment sobiFragment, ImmutableParams amendParams) {
    ...
    List<Integer> newCoSponsorIds = billAmendment.getCoSponsors().stream().map(SessionMember::getSessionMemberId).collect(Collectors.toList());
    ...
    diff.entriesDiffering().forEach((smid, ordinal) -> {
      ImmutableParams cspParams = ImmutableParams.from(getCoMultiSponsorParams(billAmendment, smid, ordinal.rightValue(), sobiFragment));
      jdbcNamed.update(SqlBillQuery.UPDATE_BILL_COSPONSOR.getSql(schema()), cspParams);
    });
    diff.entriesOnlyOnRight().forEach((smid, ordinal) -> {
      ImmutableParams cspParams = ImmutableParams.from(getCoMultiSponsorParams(billAmendment, smid, ordinal, sobiFragment));
  ...
  protected void updateBillMultiSponsor(BillAmendment billAmendment, SobiFragment sobiFragment, ImmutableParams amendParams) {
    ...
    List<Integer> newMultiSponsorIds = billAmendment.getMultiSponsors().stream().map(SessionMember::getSessionMemberId).collect(Collectors.toList());
    ...
    diff.entriesDiffering().forEach((smid, ordinal) -> {
      ImmutableParams mspParams = ImmutableParams.from(getCoMultiSponsorParams(billAmendment, smid, ordinal.rightValue(), sobiFragment));
      jdbcNamed.update(SqlBillQuery.UPDATE_BILL_MULTISPONSOR.getSql(schema()), mspParams);
    });
    diff.entriesOnlyOnRight().forEach((smid, ordinal) -> {
      ImmutableParams mspParams = ImmutableParams.from(getCoMultiSponsorParams(billAmendment, smid, ordinal, sobiFragment));
  ...
  private List<Integer> getCoSponsorIds(SqlParameterSource params) {
    return jdbcNamed.query(SqlBillQuery.SELECT_BILL_COSPONSORS.getSql(schema()), params, (rs, rowNum) -> rs.getInt("session_member_id"));
  }

  private List<Integer> getMultiSponsorIds(SqlParameterSource params) {
    return jdbcNamed.query(SqlBillQuery.SELECT_BILL_MULTISPONSORS.getSql(schema()), params, (rs, rowNum) -> rs.getInt("session_member_id"));
  }

  private static class BillRowMapper implements RowMapper<Bill> {
    @Override public Bill mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      if (rs.getString("status") != null) {
        BillStatus status = new BillStatus(BillStatusType.valueOf(rs.getString("status")), rs.getDate("status_date").toLocalDate());
        status.setCommitteeId(getCommitteeIdFromRs(rs));
  ...
  private static class BillAmendmentRowMapper implements RowMapper<BillAmendment> {
    @Override public BillAmendment mapRow(ResultSet rs, int rowNum) throws SQLException {
  ...
  private static class BillAmendPublishStatusHandler implements RowCallbackHandler {
    ...
    @Override public void processRow(ResultSet rs) throws SQLException {
      PublishStatus pubStatus = new PublishStatus(rs.getBoolean("published"), getLocalDateTimeFromRs(rs, "effect_date_time"), rs.getBoolean("override"), rs.getString("notes"));
  ...
  private static class BillActionRowMapper implements RowMapper<BillAction> {
    @Override public BillAction mapRow(ResultSet rs, int rowNum) throws SQLException {
      BillAction billAction = new BillAction();
      billAction.setBillId(new BillId(rs.getString("bill_print_no"), rs.getInt("bill_session_year"), rs.getString("bill_amend_version")));
  ...
  private static class BillSameAsRowMapper implements RowMapper<BillId> {
    @Override public BillId mapRow(ResultSet rs, int rowNum) throws SQLException {
      return new BillId(rs.getString("same_as_bill_print_no"), rs.getInt("same_as_session_year"), rs.getString("same_as_amend_version"));
    }
  }

  private static class BillPreviousVersionRowMapper implements RowMapper<BillId> {
    @Override public BillId mapRow(ResultSet rs, int rowNum) throws SQLException {
      return new BillId(rs.getString("prev_bill_print_no"), rs.getInt("prev_bill_session_year"), rs.getString("prev_amend_version"));
    }
  }
  ...
  private static class BillSponsorRowMapper implements RowMapper<BillSponsor> {
    ...
    @Override public BillSponsor mapRow(ResultSet rs, int rowNum) throws SQLException {
  ...
  private static class BillMemberRowMapper implements RowMapper<SessionMember> {
    ...
    @Override public SessionMember mapRow(ResultSet rs, int rowNum) throws SQLException {
      int sessionMemberId = rs.getInt("session_member_id");
      try {
  ...
  private static class BillCommitteeRowMapper implements RowMapper<CommitteeVersionId> {
    @Override public CommitteeVersionId mapRow(ResultSet rs, int rowNum) throws SQLException {
  ...
  public ImmutableParams getBaseParams(BillId billId) {
    return ImmutableParams.from(new MapSqlParameterSource().addValue("printNo", billId.getBasePrintNo()).addValue("sessionYear", billId.getSession().getYear()));
  }

  public ImmutableParams getBillIdParams(BillId billId) {
    return ImmutableParams.from(new MapSqlParameterSource().addValue("printNo", billId.getBasePrintNo()).addValue("sessionYear", billId.getSession().getYear()).addValue("version", billId.getVersion().getValue()));
  }
  ...
  private static MapSqlParameterSource getBillAmendmentParams(BillAmendment amendment, SobiFragment fragment) {
    ...
    params.addValue("sponsorMemo", amendment.getMemo()).addValue("actClause", amendment.getActClause()).addValue("fullText", amendment.getFullText()).addValue("stricken", amendment.isStricken()).addValue("lawSection", amendment.getLawSection()).addValue("lawCode", amendment.getLaw()).addValue("uniBill", amendment.isUniBill());
    addLastFragmentParam(fragment, params);
    return params;
  }

  private static MapSqlParameterSource getBillPublishStatusParams(Bill bill, Version version, PublishStatus pubStatus, SobiFragment fragment) {
    MapSqlParameterSource params = new MapSqlParameterSource();
    addBillIdParams(bill, params);
  ...
  private static MapSqlParameterSource getBillSameAsParams(BillAmendment billAmendment, BillId sameAs, SobiFragment fragment) {
    ...
    params.addValue("sameAsPrintNo", sameAs.getBasePrintNo()).addValue("sameAsSessionYear", sameAs.getSession().getYear()).addValue("sameAsVersion", sameAs.getVersion().getValue());
    addLastFragmentParam(fragment, params);
    return params;
  }
  ...
  private static MapSqlParameterSource getBillPrevVersionParams(Bill bill, SobiFragment fragment) {
    MapSqlParameterSource params = new MapSqlParameterSource();
    addBillIdParams(bill, params);
    params.addValue("prevPrintNo", bill.getDirectPreviousVersion().getBasePrintNo()).addValue("prevSessionYear", bill.getDirectPreviousVersion().getSession().getYear()).addValue("prevVersion", bill.getDirectPreviousVersion().getVersion().getValue());
    addLastFragmentParam(fragment, params);
    return params;
  }
  ...
  private static MapSqlParameterSource getBillSponsorParams(Bill bill, SobiFragment fragment) {
    ...
    addBillIdParams(bill, params);
    params.addValue("sessionMemberId", (hasMember) ? billSponsor.getMember().getSessionMemberId() : null).addValue("budgetBill", (billSponsor != null && billSponsor.isBudget())).addValue("rulesSponsor", (billSponsor != null && billSponsor.isRules()));
    addLastFragmentParam(fragment, params);
    return params;
  }
  ...
  private static MapSqlParameterSource getMilestoneParams(Bill bill, BillStatus status, int rank, SobiFragment fragment) {
    MapSqlParameterSource params = new MapSqlParameterSource();
    addBillIdParams(bill, params);
    params.addValue("status", status.getStatusType().name()).addValue("rank", rank).addValue("actionSequenceNo", status.getActionSequenceNo()).addValue("date", toDate(status.getActionDate())).addValue("committeeName", (status.getCommitteeId() != null ? status.getCommitteeId().getName() : null)).addValue("committeeChamber", (status.getCommitteeId() != null ? status.getCommitteeId().getChamber().asSqlEnum() : null)).addValue("calNo", status.getCalendarNo());
    addLastFragmentParam(fragment, params);
    return params;
  }

  private static MapSqlParameterSource getCoMultiSponsorParams(BillAmendment billAmendment, int sessionMemberId, int sequenceNo, SobiFragment fragment) {
    MapSqlParameterSource params = new MapSqlParameterSource();
    addBillIdParams(billAmendment, params);
    params.addValue("sessionMemberId", sessionMemberId).addValue("sequenceNo", sequenceNo);
    addLastFragmentParam(fragment, params);
    return params;
  }

  private static MapSqlParameterSource getBillVoteInfoParams(BillAmendment billAmendment, BillVote billVote, SobiFragment fragment) {
    MapSqlParameterSource params = new MapSqlParameterSource();
    addBillIdParams(billAmendment, params);
    params.addValue("voteDate", toDate(billVote.getVoteDate())).addValue("voteType", billVote.getVoteType().name().toLowerCase()).addValue("sequenceNo", billVote.getSequenceNo()).addValue("committeeName", (billVote.getCommitteeId() != null) ? billVote.getCommitteeId().getName() : null).addValue("committeeChamber", (billVote.getCommitteeId() != null) ? billVote.getCommitteeId().getChamber().asSqlEnum() : null);
    addModPubDateParams(billVote.getModifiedDateTime(), billVote.getPublishedDateTime(), params);
    addLastFragmentParam(fragment, params);
    return params;
  }

  private static MapSqlParameterSource getBillCommitteeParams(Bill bill, CommitteeVersionId committee, SobiFragment fragment) {
    MapSqlParameterSource params = new MapSqlParameterSource();
    addBillIdParams(bill, params);
    params.addValue("committeeName", committee.getName()).addValue("committeeChamber", committee.getChamber().asSqlEnum()).addValue("actionDate", toDate(committee.getReferenceDate()));
    addLastFragmentParam(fragment, params);
    return params;
  }
  ...
  private static void addBillIdParams(Bill bill, MapSqlParameterSource params) {
    params.addValue("printNo", bill.getBasePrintNo()).addValue("sessionYear", bill.getSession().getYear());
  }
  ...
  private static void addBillIdParams(BillAmendment billAmendment, MapSqlParameterSource params) {
    params.addValue("printNo", billAmendment.getBasePrintNo()).addValue("sessionYear", billAmendment.getSession().getYear()).addValue("version", billAmendment.getVersion().getValue());
  }
