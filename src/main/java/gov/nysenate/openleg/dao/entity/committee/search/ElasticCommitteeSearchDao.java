...
@Repository public class ElasticCommitteeSearchDao extends ElasticBaseDao implements CommitteeSearchDao {
  ...
  private static final Pattern committeeSearchIdPattern = Pattern.compile("(SENATE|ASSEMBLY)-([A-z, ]*)-(.*)");

  @Autowired CommitteeDataService committeeDataService;

  @Override public SearchResults<CommitteeVersionId> searchCommittees(QueryBuilder query, QueryBuilder filter, List<SortBuilder> sort, LimitOffset limitOffset) {
  ...
  @Override public void updateCommitteeIndex(CommitteeSessionId committeeSessionId) {
    BulkRequestBuilder bulkRequest = searchClient.prepareBulk();
    committeeHistoryIndexBulkAdd(committeeSessionId, bulkRequest);
    bulkRequest.execute().actionGet();
  }

  @Override public void updateCommitteeIndexBulk(Collection<CommitteeSessionId> sessionIds) {
    if (sessionIds.isEmpty()) {
      return;
    }
    BulkRequestBuilder bulkRequest = searchClient.prepareBulk();
    sessionIds.stream().peek(this::deleteCommitteeFromIndex).forEach((sessionId) -> committeeHistoryIndexBulkAdd(sessionId, bulkRequest));
    bulkRequest.execute().actionGet();
  }

  @Override public void deleteCommitteeFromIndex(CommitteeSessionId committeeSessionId) {
  ...
  @Override protected List<String> getIndices() {
  ...
  private BulkRequestBuilder getCommitteeDeleteRequest(CommitteeSessionId committeeSessionId) {
    SearchResults<CommitteeVersionId> searchResults = searchCommittees(getCommitteeSessionQuery(committeeSessionId), null, Collections.emptyList(), LimitOffset.ALL);
    BulkRequestBuilder bulkDeleteRequest = searchClient.prepareBulk();
    searchResults.getResults().stream().map(SearchResult::getResult).map(this::getCommitteeVersionDeleteRequest).forEach(bulkDeleteRequest::add);
  ...
  private BoolQueryBuilder getCommitteeSessionQuery(CommitteeSessionId committeeSessionId) {
    return QueryBuilders.boolQuery().must(QueryBuilders.matchAllQuery()).filter(QueryBuilders.termQuery("chamber", committeeSessionId.getChamber().toString().toLowerCase())).filter(QueryBuilders.termQuery("name", committeeSessionId.getName().toLowerCase()));
  }
  ...
  private void committeeHistoryIndexBulkAdd(CommitteeSessionId committeeSessionId, BulkRequestBuilder bulkRequest) {
    try {
      committeeDataService.getCommitteeHistory(committeeSessionId).stream().map(this::getCommitteeVersionIndexRequest).forEach(bulkRequest::add);
  ...
  protected DeleteRequestBuilder getCommitteeVersionDeleteRequest(CommitteeVersionId committeeVersionId) {
    return searchClient.prepareDelete(committeeSearchIndexName, Integer.toString(committeeVersionId.getSession().getYear()), generateCommitteeVersionSearchId(committeeVersionId));
  }
  ...
  private IndexRequestBuilder getCommitteeVersionIndexRequest(Committee committee) {
    return searchClient.prepareIndex(committeeSearchIndexName, Integer.toString(committee.getSession().getYear()), generateCommitteeVersionSearchId(committee.getVersionId())).setSource(OutputUtils.toJson(new CommitteeView(committee)));
  }

  private String generateCommitteeVersionSearchId(CommitteeVersionId committeeVersionId) {
    return generateCommitteeSearchId(committeeVersionId) + "-" + committeeVersionId.getReferenceDate();
  }

  private String generateCommitteeSearchId(CommitteeId committeeId) {
    return committeeId.getChamber() + "-" + committeeId.getName();
  }
  ...
  private CommitteeVersionId getCommitteeVersionId(SearchHit hit) {
    ...
    return new CommitteeVersionId(Chamber.getValue(versionIdMatcher.group(1)), versionIdMatcher.group(2), SessionYear.of(Integer.parseInt(hit.getType())), LocalDateTime.parse(versionIdMatcher.group(3), DateTimeFormatter.ISO_DATE_TIME));
  }
}