...
public abstract class SqlBaseDao {
  ...
  protected String schema() {
    if (environment == null) {
      throw new IllegalStateException("The environment has not been initialized. Cannot perform SQL queries " + "since we can\'t determine which database schema to operate on.");
    }
  ...
  public static String toHstoreString(Map<String, String> hstoreMap) {
    return hstoreMap.entrySet().stream().map((kv) -> kv.getKey() + "=>" + (kv.getValue() == null ? "NULL" : StringUtils.isEmpty(kv.getValue()) ? "\"\"" : kv.getValue().replaceAll("([,=> ])", "\\\\$1").replaceAll("\'", "\'\'"))).collect(Collectors.joining(","));
  }
  ...
  protected String getDateColumnForUpdateType(UpdateType updateType) {
    ...
    if (updateType.equals(UpdateType.PROCESSED_DATE)) {
      ...
      if (updateType.equals(UpdateType.PUBLISHED_DATE)) {
  ...
  public static LocalDateTime getLocalDateTimeFromSobiFragmentId(String fragmentId) {
    if (fragmentId != null && !fragmentId.isEmpty()) {
      ...
      if (parts.size() == 4) {
        try {
          return LocalDateTime.parse(parts.get(1).substring(1) + parts.get(2).substring(1), DateTimeFormatter.ofPattern("yyMMddHHmmss"));
  ...
  public static Duration getDurationFromRs(ResultSet rs, String column) throws SQLException {
    PGInterval interval = (PGInterval) rs.getObject(column);
    return interval != null ? Duration.ofMillis((long) (interval.getSeconds() * 1000) + interval.getMinutes() * 1000 * 60 + interval.getHours() * 1000 * 60 * 60 + interval.getDays() * 1000 * 60 * 60 * 24) : null;
  }
