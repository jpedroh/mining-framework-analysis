...
public abstract class ElasticBaseDao {
  ...
  public void createIndices() {
    getIndices().stream().filter((index) -> !indicesExist(index)).forEach(this::createIndex);
  }
  ...
  /**
     ...
     */
  protected <T extends java.lang.Object> SearchResults<T> search(String indexName, QueryBuilder query, QueryBuilder postFilter, List<SortBuilder<?>> sort, LimitOffset limitOffset, Function<SearchHit, T> hitMapper) throws ElasticsearchException {
    return search(indexName, query, postFilter, null, null, sort, limitOffset, false, hitMapper);
  }
  ...
  /**
     ...
     */
  protected <T extends java.lang.Object> SearchResults<T> search(String indexName, QueryBuilder query, QueryBuilder postFilter, List<HighlightBuilder.Field> highlightedFields, RescorerBuilder<?> rescorer, List<SortBuilder<?>> sort, LimitOffset limitOffset, boolean fetchSource, Function<SearchHit, T> hitMapper) throws ElasticsearchException {
    if (indexIsEmpty(indexName)) {
      return SearchResults.empty();
    }
    SearchRequest searchRequest = getSearchRequest(indexName, query, postFilter, highlightedFields, rescorer, sort, limitOffset, fetchSource);
  ...
  /**
     ...
     * @return Optional<T></T>
     */
  protected <T extends java.lang.Object> Optional<T> getRequest(String index, String id, Function<GetResponse, T> responseMapper) {
  ...
  protected IndexRequest getJsonIndexRequest(String indexName, String id, Object object) {
    return new IndexRequest(indexName).id(id).source(OutputUtils.toElasticsearchJson(object), XContentType.JSON);
  }
  ...
  protected void safeBulkRequestExecute(BulkRequest bulkRequest) {
    ...
    for (BulkRequest subRequest : bulkRequests) {
      try {
        logger.debug("Making bulk request: {} bytes", StringUtils.leftPad(Long.toString(subRequest.estimatedSizeInBytes()), 9));
  ...
  /**
     ...
     */
  protected static final ImmutableMap<String, Object> searchableKeywordMapping = ImmutableMap.of("type", "keyword", "fields", ImmutableMap.of("text", ImmutableMap.of("type", "text")));

  protected static final ImmutableMap<String, Object> basicTimeMapping = ImmutableMap.of("type", "date", "format", "hour_minute");
  ...
  private void createIndex(String indexName) {
    try {
      CreateIndexRequest createIndexRequest = new CreateIndexRequest(indexName).settings(getIndexSettings());
  ...
  /**
     ...
     * @return SearchRequest
     */
  private SearchRequest getSearchRequest(String indexName, QueryBuilder query, QueryBuilder postFilter, List<HighlightBuilder.Field> highlightedFields, RescorerBuilder<?> rescorer, List<SortBuilder<?>> sort, LimitOffset limitOffset, boolean fetchSource) throws ElasticsearchException {
    limitOffset = adjustLimitOffset(limitOffset);
    SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder().query(query).from(limitOffset.getOffsetStart() - 1).size((limitOffset.hasLimit()) ? limitOffset.getLimit() : Integer.MAX_VALUE).minScore(0.05f).trackTotalHits(true).fetchSource(new FetchSourceContext(fetchSource));
    ...
    SearchRequest searchRequest = Requests.searchRequest(indexName).source(searchSourceBuilder).searchType(SearchType.QUERY_THEN_FETCH);
  ...
  /**
     ...
     */
  private <R extends java.lang.Object> SearchResults<R> getSearchResults(SearchResponse response, LimitOffset limitOffset, Function<SearchHit, R> hitMapper) {
    limitOffset = adjustLimitOffset(limitOffset);
    List<SearchResult<R>> resultList = new ArrayList<>();
    for (SearchHit hit : response.getHits().getHits()) {
      SearchResult<R> result = new SearchResult<>(hitMapper.apply(hit), (!Float.isNaN(hit.getScore())) ? BigDecimal.valueOf(hit.getScore()) : BigDecimal.ONE, hit.getHighlightFields());
  ...
  private LimitOffset adjustLimitOffset(LimitOffset limitOffset) {
    ...
    if (limitOffset.getOffsetEnd() > maxResultWindow) {
      throw new InvalidSearchParamException("LimitOffset with offset end of " + limitOffset.getOffsetEnd() + " extends past allowed result window of " + maxResultWindow);
    }
    return limitOffset;
  }
  ...
  private boolean indexIsEmpty(String indexName) {
    try {
      InputStream responseStream = searchClient.getLowLevelClient().performRequest(new Request("GET", COUNT_API + indexName + "?v")).getEntity().getContent();
  ...
  private List<BulkRequest> splitBulkRequest(BulkRequest bulkRequest) {
    ...
    while (!requestQueue.isEmpty()) {
      ...
      while (!requestQueue.isEmpty() && openBulk.estimatedSizeInBytes() < desiredBulkRequestSize) {
        ...
        if (nextDoc instanceof IndexRequest) {
          ...
          if (openBulk.numberOfActions() > 0 && openBulk.estimatedSizeInBytes() + length > desiredBulkRequestSize) {
            break;
          }
        }
    ...
    logger.debug("Large elasticsearch bulk request ({}) will be broken into {} smaller bulk requests", FileUtils.byteCountToDisplaySize(totalSize), bulkRequests.size());
    return bulkRequests;
  }
}