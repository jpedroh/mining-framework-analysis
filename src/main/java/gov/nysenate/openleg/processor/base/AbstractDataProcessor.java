...
public abstract class AbstractDataProcessor {
  ...
  protected DataProcessUnit createProcessUnit(SobiFragment sobiFragment) {
    return new DataProcessUnit("SOBI-" + sobiFragment.getType().name(), sobiFragment.getFragmentId(), LocalDateTime.now(), DataProcessAction.INGEST);
  }
  ...
  protected void flushBillUpdates() {
    if (billIngestCache.getSize() > 0) {
      ...
      billIngestCache.getCurrentCache().forEach((entry) -> billDataService.saveBill(entry.getLeft(), entry.getRight(), false));
      logger.debug("Broadcasting bill updates...");
      List<Bill> bills = billIngestCache.getCurrentCache().stream().map((entry) -> entry.getLeft()).collect(Collectors.toList());
  ...
  public List<SessionMember> getSessionMember(String sponsors, SessionYear session, Chamber chamber) {
    List<String> shortNames = Lists.newArrayList(Splitter.on(",").omitEmptyStrings().trimResults().splitToList(sponsors.toUpperCase()));
  ...
  protected void flushAgendaUpdates() {
    if (agendaIngestCache.getSize() > 0) {
      ...
      agendaIngestCache.getCurrentCache().forEach((entry) -> agendaDataService.saveAgenda(entry.getLeft(), entry.getRight(), false));
      List<Agenda> agendas = agendaIngestCache.getCurrentCache().stream().map((entry) -> entry.getLeft()).collect(Collectors.toList());
  ...
  protected void flushCalendarUpdates() {
    if (calendarIngestCache.getSize() > 0) {
      ...
      calendarIngestCache.getCurrentCache().forEach((entry) -> calendarDataService.saveCalendar(entry.getLeft(), entry.getRight(), false));
      List<Calendar> calendars = calendarIngestCache.getCurrentCache().stream().map((entry) -> entry.getLeft()).collect(Collectors.toList());
  ...
  protected void syncUniBillText(BillAmendment billAmendment, SobiFragment sobiFragment) {
    billAmendment.getSameAs().forEach((uniBillId) -> {
      ...
      if (billAmendment.getBillType().getChamber().equals(Chamber.SENATE)) {
        ...
        if (!uniBillAmend.getFullText().isEmpty()) {
          billAmendment.setFullText(uniBillAmend.getFullText());
          updatedBillId = billAmendment.getBaseBillId();
        }
      }
      if (updatedBillId != null) {
        eventBus.post(new BillFieldUpdateEvent(LocalDateTime.now(), updatedBillId, BillUpdateField.FULLTEXT));
      }
    });
  }
