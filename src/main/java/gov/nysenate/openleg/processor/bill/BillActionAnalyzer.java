...
public class BillActionAnalyzer {
  ...
  private static final Pattern committeeEventTextPattern = Pattern.compile("(REFERRED|COMMITTED|RECOMMIT) TO ([A-Z, ]*[A-Z]+)\\s?([0-9]+[A-Z]?)?");
  ...
  private static final List<Pattern> publishBillEventPatterns = Arrays.asList(Pattern.compile("PRINT NUMBER " + simpleBillRegex), Pattern.compile("AMEND(?:ED)? (?:ON THIRD READING )?(?:\\(T\\) )?" + simpleBillRegex), Pattern.compile("AMEND(?:ED)? (?:\\(T\\) )?AND RECOMMIT(?:TED)? TO RULES " + simpleBillRegex));

  /** Patterns for bill actions indicating that the specified version should be the new active version. */
  private static final List<Pattern> amendmentRestoreBillEventPatterns = Arrays.asList(Pattern.compile("AMEND(?:ED)? BY RESTORING TO PREVIOUS PRINT " + simpleBillRegex), Pattern.compile("AMEND(?:ED)? BY RESTORING TO ORIGINAL PRINT " + simpleBillRegex));

  private static final List<BillStatusType> senateMilestones = Arrays.asList(IN_SENATE_COMM, SENATE_FLOOR, PASSED_SENATE, IN_ASSEMBLY_COMM, ASSEMBLY_FLOOR, PASSED_ASSEMBLY, DELIVERED_TO_GOV, SIGNED_BY_GOV, VETOED);

  private static final List<BillStatusType> assemblyMilestones = Arrays.asList(IN_ASSEMBLY_COMM, ASSEMBLY_FLOOR, PASSED_ASSEMBLY, IN_SENATE_COMM, SENATE_FLOOR, PASSED_SENATE, DELIVERED_TO_GOV, SIGNED_BY_GOV, VETOED);
  ...
  protected void updatePublishStatus(BillAction action) {
    ...
    for (Pattern pattern : publishBillEventPatterns) {
      ...
      if (matcher.find()) {
        ...
        PublishStatus status = new PublishStatus(true, action.getDate().atStartOfDay(), false, action.getText());
  ...
  protected void updateBillStatus(BillAction action) {
    ...
    if (billId.getBillType().isResolution() && adoptedPattern.matcher(text).find()) {
      ...
      if (committeeMatcher.find()) {
        this.currentCommittee = new CommitteeVersionId(action.getChamber(), committeeMatcher.group(2), action.getBillId().getSession(), action.getDate().atStartOfDay());
        this.pastCommittees.add(this.currentCommittee);
        currStatus = new BillStatus((action.getChamber().equals(Chamber.SENATE)) ? IN_SENATE_COMM : IN_ASSEMBLY_COMM, action.getDate());
        currStatus.setCommitteeId(this.currentCommittee);
      } else {
        if (floorEventPattern.matcher(text).find()) {
          this.currentCommittee = null;
          currStatus = new BillStatus((action.getChamber().equals(Chamber.SENATE)) ? SENATE_FLOOR : ASSEMBLY_FLOOR, action.getDate());
          ...
          if (calMatcher.find()) {
            ...
            if (calNoTable.contains(year, action.getChamber())) {
              currStatus.setCalendarNo(calNoTable.get(year, action.getChamber()));
            }
          }
        } else {
          if (passedHouseMatcher.find()) {
            Chamber chamber = Chamber.getValue(passedHouseMatcher.group(1));
            currStatus = new BillStatus((chamber.equals(Chamber.SENATE)) ? PASSED_SENATE : PASSED_ASSEMBLY, action.getDate());
          } else {
            if (deliveredGovPattern.matcher(text).find()) {
              ...
              if (signedMatcher.find()) {
                ...
                if (vetoedPattern.matcher(text).find() || pocketVetoPattern.matcher(text).find()) {
                  if (!text.contains("LINE")) {
                    currStatus = new BillStatus(VETOED, action.getDate());
                  }
                } else {
                  if (text.contains("ENACTING CLAUSE STRICKEN")) {
                    currStatus = new BillStatus(STRICKEN, action.getDate());
                    this.stricken = true;
                  } else {
                    if (text.trim().equals("LOST")) {
      ...
      for (BillStatusType milestoneType : milestoneTypes) {
        for (BillStatus status : statusList) {
          if (status.getActionSequenceNo() <= lastSequenceNo) {
            ...
            if (status.getStatusType().equals(milestoneType)) {
