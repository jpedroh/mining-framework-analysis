...
public class SubmissionRepoImpl extends AbstractWeaverRepoImpl<Submission, SubmissionRepo> implements SubmissionRepoCustom {
  ...
  @Value(value = "${app.url}") private String url;

  @Autowired private SubmissionRepo submissionRepo;

  @Autowired private FieldPredicateRepo fieldPredicateRepo;

  @Autowired private FieldValueRepo fieldValueRepo;

  @Autowired private SubmissionWorkflowStepRepo submissionWorkflowStepRepo;

  @Autowired private SubmissionListColumnRepo submissionListColumnRepo;

  @Autowired private InputTypeRepo inputTypeRepo;
  ...
  @Value(value = "${app.document.path:private/}") private String documentPath;

  public SubmissionRepoImpl(DataSource dataSource) {
    jdbcTemplate = new JdbcTemplate(dataSource);
  }

  @Override @Transactional public Submission create(User submitter, Organization organization, SubmissionStatus startingStatus, Credentials credentials, List<CustomActionDefinition> customActions) throws OrganizationDoesNotAcceptSubmissionsException {
  ...
  @Override public Submission update(Submission submission) {
  ...
  @Override public void delete(Submission submission) {
  ...
  @Override public Submission updateStatus(Submission submission, SubmissionStatus submissionStatus, User user) {
  ...
  @Override public List<Submission> batchDynamicSubmissionQuery(NamedSearchFilterGroup activeFilter, List<SubmissionListColumn> submissionListColums) {
  ...
  @Override public Page<Submission> pageableDynamicSubmissionQuery(NamedSearchFilterGroup activeFilter, List<SubmissionListColumn> submissionListColums, Pageable pageable) throws ExecutionException {
  ...
  private QueryStrings craftDynamicSubmissionQuery(NamedSearchFilterGroup activeFilter, List<SubmissionListColumn> submissionListColums, Pageable pageable) {
    ...
    Collections.sort(allSubmissionListColumns, new Comparator<>() {
      @Override public int compare(SubmissionListColumn svc1, SubmissionListColumn svc2) {
          ...
          for (String filterString : submissionListColumn.getFilters()) {
            ...
            switch (submissionListColumn.getInputType().getName()) {
              ...
              if (filterString.contains("|")) {
                ...
                sqlBuilder.append("CAST(pfv").append(n).append(".value AS DATE) BETWEEN CAST(\'").append(dates[0]).append("\' AS DATE) AND CAST(\'").append(dates[1]).append("\' AS DATE)");
                sqlCountBuilder.append("CAST(fv.value AS DATE) BETWEEN CAST(\'").append(dates[0]).append("\' AS DATE) AND CAST(\'").append(dates[1]).append("\' AS DATE)");
              } else {
      ...
      if (sqlCountWherePredicate.size() > 0) {
        ...
        sqlCountWherePredicate.forEach((id, filter) -> {
          if (filter.length() > 0) {
            ...
            sqlCountSelectBuilder.append("\n(fv.field_predicate_id = ").append(id).append(" AND (").append(filter).append(")) AND");
          }
        });
      } else {
        sqlCountSelectBuilder.append("\nWHERE");
  ...
  public void setColumnOrderingForMonthYearDateFormat(Sort sort, StringBuilder sqlSelectBuilder, StringBuilder sqlOrderBysBuilder, String table) {
    if (sort == Sort.ASC || sort == Sort.DESC) {
      sqlSelectBuilder.append(table).append(".value,").append(" CAST(REPLACE(").append(table).append(".value, \' \', \' 1, \') AS DATE) AS").append(table).append("_date,");
      sqlOrderBysBuilder.append(table).append("_date ").append(sort.name()).append(",");
    }
  }

  @Override protected String getChannel() {
  ...
  private StringBuilder buildSubmissionDateFieldString(String column, String filter) {
    if (filter.contains("|")) {
      ...
      return new StringBuilder().append("s.").append(column).append(" BETWEEN CAST(\'").append(dates[0]).append("\' AS DATE) AND CAST(\'").append(dates[1]).append("\' AS DATE)");
    }
    return new StringBuilder().append("s.").append(column).append(" = CAST(\'").append(filter).append("\' AS DATE)");
  }
