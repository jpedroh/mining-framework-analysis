...
public final class FastMoney implements MonetaryAmount, Comparable<MonetaryAmount>, Serializable {
  ...
  @Override public FastMoney add(MonetaryAmount amount) {
  ...
  @Override public FastMoney divide(Number divisor) {
  ...
  @Override public FastMoney[] divideAndRemainder(Number divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
  ...
  @Override public FastMoney divideToIntegralValue(Number divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
      return new FastMoney(0L, getCurrency());
    }
    checkNumber(divisor);
  ...
  @Override public FastMoney multiply(Number multiplicand) {
    NumberVerifier.checkNoInfinityOrNaN(multiplicand);
    checkNumber(multiplicand);
    if (isOne(multiplicand)) {
      return this;
    }
    return new FastMoney(Math.multiplyExact(this.number, getInternalNumber(multiplicand, false)) / 100000L, getCurrency());
  }

  @Override public FastMoney negate() {
    return new FastMoney(Math.multiplyExact(this.number, -1), getCurrency());
  }

  @Override public FastMoney plus() {
    return this;
  }

  @Override public FastMoney subtract(MonetaryAmount subtrahend) {
    checkAmountParameter(subtrahend);
    if (subtrahend.isZero()) {
      return this;
    }
    return new FastMoney(Math.subtractExact(this.number, getInternalNumber(subtrahend.getNumber(), false)), getCurrency());
  }

  @Override public FastMoney remainder(Number divisor) {
    checkNumber(divisor);
    if (isOne(divisor)) {
  ...
  @Override public FastMoney scaleByPowerOfTen(int power) {
  ...
  @Override public FastMoney with(MonetaryOperator operator) {
  ...
  private static final ToStringMonetaryAmountFormat DEFAULT_FORMATTER = ToStringMonetaryAmountFormat.of(ToStringMonetaryAmountFormatStyle.FAST_MONEY);

  private BigDecimal getBigDecimal() {
    return BigDecimal.valueOf(this.number).movePointLeft(SCALE);
  }

  @Override public FastMoney multiply(double multiplicand) {
    NumberVerifier.checkNoInfinityOrNaN(multiplicand);
    if (multiplicand == 1.0) {
      return this;
    }
  ...
  @Override public FastMoney divide(long divisor) {
    if (divisor == 1L) {
      return this;
    }
  ...
  @Override public FastMoney divide(double divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
      return new FastMoney(0L, getCurrency());
    }
    if (divisor == 1.0d) {
  ...
  @Override public FastMoney remainder(long divisor) {
    return remainder(BigDecimal.valueOf(divisor));
  }

  @Override public FastMoney remainder(double divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
      return new FastMoney(0L, getCurrency());
    }
  ...
  @Override public FastMoney[] divideAndRemainder(long divisor) {
    return divideAndRemainder(BigDecimal.valueOf(divisor));
  }

  @Override public FastMoney[] divideAndRemainder(double divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
      FastMoney zero = new FastMoney(0L, getCurrency());
      return new FastMoney[] { zero, zero };
    } else {
      if (divisor == Double.NaN) {
  ...
  @Override public FastMoney stripTrailingZeros() {
    return this;
  }

  @Override public FastMoney multiply(long multiplicand) {
    if (multiplicand == 1) {
      return this;
    }
    if (multiplicand == 0) {
  ...
  @Override public FastMoney divideToIntegralValue(long divisor) {
    if (divisor == 1) {
      return this;
    }
  ...
  @Override public FastMoney divideToIntegralValue(double divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
      return new FastMoney(0L, getCurrency());
    }
    if (divisor == 1.0) {
  ...
  @Override public MonetaryAmountFactory<FastMoney> getFactory() {
