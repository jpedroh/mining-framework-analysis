...
public final class RoundedMoney implements MonetaryAmount, Comparable<MonetaryAmount>, Serializable {
  ...
  RoundedMoney(Number number, CurrencyUnit currency, MathContext mathContext) {
    ...
    this.monetaryContext = DEFAULT_MONETARY_CONTEXT.toBuilder().set("MonetaryRounding", rounding).set(mathContext).build();
  ...
  public static RoundedMoney of(Number number, CurrencyUnit currency, MonetaryContext monetaryContext) {
    return new RoundedMoney(number, currency, DEFAULT_MONETARY_CONTEXT.toBuilder().importContext(monetaryContext).build(), null);
  }
  ...
  public static RoundedMoney of(CurrencyUnit currency, Number number, MonetaryContext monetaryContext, MonetaryOperator rounding) {
    return new RoundedMoney(number, currency, DEFAULT_MONETARY_CONTEXT.toBuilder().importContext(monetaryContext).build(), rounding);
  }
  ...
  public static RoundedMoney of(Number number, String currencyCode) {
    return new RoundedMoney(number, Monetary.getCurrency(currencyCode), Monetary.getDefaultRounding());
  }
  ...
  public static RoundedMoney of(Number number, String currencyCode, MonetaryContext monetaryContext) {
    return new RoundedMoney(number, Monetary.getCurrency(currencyCode), DEFAULT_MONETARY_CONTEXT.toBuilder().importContext(monetaryContext).build(), null);
  }
  ...
  public static RoundedMoney of(String currencyCode, Number number, MonetaryContext monetaryContext, MonetaryOperator rounding) {
    return new RoundedMoney(number, Monetary.getCurrency(currencyCode), DEFAULT_MONETARY_CONTEXT.toBuilder().importContext(monetaryContext).build(), rounding);
  }
  ...
  /**
     ...
     */
  @Override public MonetaryContext getContext() {
    return this.monetaryContext;
  }

  @Override public RoundedMoney abs() {
  ...
  @Override public RoundedMoney add(MonetaryAmount amount) {
    MoneyUtils.checkAmountParameter(amount, this.currency);
    if (amount.isZero()) {
      return this;
    }
    return new RoundedMoney(this.number.add(amount.getNumber().numberValue(BigDecimal.class)), this.currency, this.rounding).with(rounding);
  }

  @Override public RoundedMoney divide(Number divisor) {
    BigDecimal bd = MoneyUtils.getBigDecimal(divisor);
    if (isOne(bd)) {
      return this;
    }
    BigDecimal dec = this.number.divide(bd, Optional.ofNullable(this.monetaryContext.get(RoundingMode.class)).orElse(RoundingMode.HALF_EVEN));
    return new RoundedMoney(dec, this.currency, this.rounding).with(rounding);
  }

  @Override public RoundedMoney[] divideAndRemainder(Number divisor) {
    BigDecimal bd = MoneyUtils.getBigDecimal(divisor);
    if (isOne(bd)) {
      return new RoundedMoney[] { this, new RoundedMoney(0L, getCurrency(), this.rounding) };
    }
    BigDecimal[] dec = this.number.divideAndRemainder(MoneyUtils.getBigDecimal(divisor), Optional.ofNullable(this.monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64));
    return new RoundedMoney[] { new RoundedMoney(dec[0], this.currency, this.rounding), new RoundedMoney(dec[1], this.currency, this.rounding).with(rounding) };
  }

  @Override public RoundedMoney divideToIntegralValue(Number divisor) {
    BigDecimal dec = this.number.divideToIntegralValue(MoneyUtils.getBigDecimal(divisor), Optional.ofNullable(this.monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64));
  ...
  @Override public RoundedMoney multiply(Number multiplicand) {
    ...
    BigDecimal dec = this.number.multiply(bd, Optional.ofNullable(this.monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64));
    return new RoundedMoney(dec, this.currency, this.rounding).with(rounding);
  }

  @Override public RoundedMoney negate() {
    return new RoundedMoney(this.number.negate(Optional.ofNullable(this.monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64)), this.currency, this.rounding);
  }

  @Override public RoundedMoney plus() {
    return this;
  }

  @Override public RoundedMoney subtract(MonetaryAmount amount) {
    MoneyUtils.checkAmountParameter(amount, this.currency);
    if (amount.isZero()) {
      return this;
    }
    return new RoundedMoney(this.number.subtract(amount.getNumber().numberValue(BigDecimal.class), Optional.ofNullable(this.monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64)), this.currency, this.rounding);
  }

  public RoundedMoney pow(int n) {
    return new RoundedMoney(this.number.pow(n, Optional.ofNullable(this.monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64)), this.currency, this.rounding).with(rounding);
  }
  ...
  @Override public RoundedMoney remainder(Number divisor) {
    return new RoundedMoney(this.number.remainder(MoneyUtils.getBigDecimal(divisor), Optional.ofNullable(this.monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64)), this.currency, this.rounding);
  }

  @Override public RoundedMoney scaleByPowerOfTen(int power) {
  ...
  @Override public boolean isPositiveOrZero() {
    return signum() >= 0;
  }
  ...
  @Override public boolean isNegativeOrZero() {
    return signum() <= 0;
  }
  ...
  @Override public boolean isLessThan(MonetaryAmount amount) {
    MoneyUtils.checkAmountParameter(amount, this.currency);
    return number.stripTrailingZeros().compareTo(amount.getNumber().numberValue(BigDecimal.class).stripTrailingZeros()) < 0;
  }

  @Override public boolean isLessThanOrEqualTo(MonetaryAmount amount) {
    MoneyUtils.checkAmountParameter(amount, this.currency);
    return number.stripTrailingZeros().compareTo(amount.getNumber().numberValue(BigDecimal.class).stripTrailingZeros()) <= 0;
  }

  @Override public boolean isGreaterThan(MonetaryAmount amount) {
    MoneyUtils.checkAmountParameter(amount, this.currency);
    return number.stripTrailingZeros().compareTo(amount.getNumber().numberValue(BigDecimal.class).stripTrailingZeros()) > 0;
  }

  @Override public boolean isGreaterThanOrEqualTo(MonetaryAmount amount) {
    MoneyUtils.checkAmountParameter(amount, this.currency);
    return number.stripTrailingZeros().compareTo(amount.getNumber().numberValue(BigDecimal.class).stripTrailingZeros()) >= 0;
  }

  @Override public boolean isEqualTo(MonetaryAmount amount) {
    MoneyUtils.checkAmountParameter(amount, this.currency);
    return number.stripTrailingZeros().compareTo(amount.getNumber().numberValue(BigDecimal.class).stripTrailingZeros()) == 0;
  }
  ...
  @Override public RoundedMoney with(MonetaryOperator operator) {
  ...
  private static final ToStringMonetaryAmountFormat DEFAULT_FORMATTER = ToStringMonetaryAmountFormat.of(ToStringMonetaryAmountFormatStyle.ROUNDED_MONEY);

  @Override public <T extends java.lang.Object> T query(MonetaryQuery<T> query) {
  ...
  @SuppressWarnings(value = { "unchecked" }) public <T extends java.lang.Object> T asType(Class<T> type) {
  ...
  public <T extends java.lang.Object> T asType(Class<T> type, MonetaryOperator adjuster) {
  ...
  @Override public boolean equals(Object obj) {
    ...
    if (obj instanceof RoundedMoney) {
      ...
      return Objects.equals(currency, other.currency) && Objects.equals(asNumberStripped(), other.asNumberStripped());
    }
    return false;
  }

  @Override public int compareTo(MonetaryAmount o) {
  ...
  @Override public NumberValue getNumber() {
  ...
  @Override public RoundedMoney multiply(long multiplicand) {
    if (multiplicand == 1L) {
      return this;
    }
  ...
  @Override public RoundedMoney multiply(double multiplicand) {
  ...
  @Override public RoundedMoney divide(long divisor) {
    if (divisor == 1L) {
      return this;
    }
  ...
  @Override public RoundedMoney divide(double divisor) {
  ...
  @Override public RoundedMoney remainder(long divisor) {
    return remainder(MoneyUtils.getBigDecimal(divisor));
  }

  @Override public RoundedMoney remainder(double divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
      return new RoundedMoney(0L, getCurrency(), this.monetaryContext, this.rounding);
    }
    return remainder(MoneyUtils.getBigDecimal(divisor));
  ...
  @Override public RoundedMoney[] divideAndRemainder(long divisor) {
    return divideAndRemainder(MoneyUtils.getBigDecimal(divisor));
  }

  @Override public RoundedMoney[] divideAndRemainder(double divisor) {
    if (NumberVerifier.isInfinityAndNotNaN(divisor)) {
  ...
  @Override public RoundedMoney stripTrailingZeros() {
    if (isZero()) {
  ...
  @Override public RoundedMoney divideToIntegralValue(long divisor) {
    return divideToIntegralValue(MoneyUtils.getBigDecimal(divisor));
  }

  @Override public RoundedMoney divideToIntegralValue(double divisor) {
    return divideToIntegralValue(MoneyUtils.getBigDecimal(divisor));
  }

  @Override public MonetaryAmountFactory<RoundedMoney> getFactory() {
