...
public class Scheduler implements org.apache.mesos.Scheduler, Runnable {
  ...
  private static final String NAME_NODE_TASK_STR = NAME_NODE_ID + "." + NAME_NODE_ID + "." + NAME_NODE_EXECUTOR_ID;
  ...
  private void launchInitialNameNodes(SchedulerDriver driver, Collection<Offer> offers) {
    for (int i = 0; i < TOTAL_NAME_NODES; i++) {
      if (offers.size() > 0) {
        ...
        pendingOffers.remove(offer.getId());
        launchNode(driver, offer, NAME_NODE_ID, Arrays.asList(NAME_NODE_ID, ZKFC_NODE_ID, JOURNAL_NODE_ID), NAME_NODE_EXECUTOR_ID);
      }
    }
  }
