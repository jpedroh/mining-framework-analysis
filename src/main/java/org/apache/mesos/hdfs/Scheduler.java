...
public class Scheduler implements org.apache.mesos.Scheduler, Runnable {
    ...
    for (Offer offer : offers) {
      if (acceptedOffer) {
        ...
        switch (liveState.getCurrentAcquisitionPhase()) {
          ...
          break;
          case START_NAME_NODES:
          if (dnsResolver.journalNodesResolvable() && tryToLaunchNameNode(driver, offer)) {
            acceptedOffer = true;
          } else {
            driver.declineOffer(offer.getId());
          }
  ...
  private boolean offerNotEnoughResources(Offer offer, double cpus, int mem) {
    for (Resource offerResource : offer.getResourcesList()) {
      if (offerResource.getName().equals("cpus") && cpus > offerResource.getScalar().getValue()) {
        return true;
      }
      if (offerResource.getName().equals("mem") && mem > offerResource.getScalar().getValue()) {
        return true;
      }
    }
    return false;
