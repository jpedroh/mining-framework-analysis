...
public class Scheduler implements org.apache.mesos.Scheduler, Runnable {
    ...
    if (isTerminalState(status)) {
      ...
      if (isRunningState(status)) {
        ...
        switch (liveState.getCurrentAcquisitionPhase()) {
          ...
          log.info(String.format("Reconcile timeout complete is: %b", liveState.reconciliationComplete()));
  ...
  @Override public void run() {
    FrameworkInfo.Builder frameworkInfo = FrameworkInfo.newBuilder().setName("HDFS " + conf.getClusterName()).setFailoverTimeout(conf.getFailoverTimeout()).setUser(conf.getHdfsUser()).setRole(conf.getHdfsRole()).setCheckpoint(true);
    try {
  ...
  private boolean isTerminalState(TaskStatus taskStatus) {
    return (taskStatus.getState().equals(TaskState.TASK_FAILED) || taskStatus.getState().equals(TaskState.TASK_FINISHED) || taskStatus.getState().equals(TaskState.TASK_KILLED) || taskStatus.getState().equals(TaskState.TASK_LOST));
  }
  ...
  private void correctCurrentPhase() {
    if (liveState.getJournalNodeSize() < conf.getJournalNodeCount()) {
      ...
      if (liveState.getNameNodeSize() == 0) {
        liveState.transitionTo(AcquisitionPhase.NAME_NODE_1);
      } else {
