...
public class Scheduler implements org.apache.mesos.Scheduler, Runnable {
  ...
  private void launchInitialNameNodes(SchedulerDriver driver, Collection<Offer> offers) {
    for (int i = 0; i < HDFSConstants.TOTAL_NAME_NODES; i++) {
      if (offers.size() > 0) {
        Offer offer = offers.iterator().next();
        pendingOffers.remove(offer.getId());
        launchNode(driver, offer, HDFSConstants.NAME_NODE_ID, Arrays.asList(HDFSConstants.NAME_NODE_ID, HDFSConstants.ZKFC_NODE_ID, HDFSConstants.JOURNAL_NODE_ID), HDFSConstants.NAME_NODE_EXECUTOR_ID);
      }
    }
  }
  ...
  @Override public void statusUpdate(SchedulerDriver driver, TaskStatus status) {
    log.info(String.format("Received status update for taskId=%s state=%s message=\'%s\' stagingTasks.size=%d", status.getTaskId().getValue(), status.getState().toString(), status.getMessage(), liveState.getStagingTasks().size()));
    if (status.getState().equals(TaskState.TASK_FAILED) || status.getState().equals(TaskState.TASK_FINISHED) || status.getState().equals(TaskState.TASK_KILLED) || status.getState().equals(TaskState.TASK_LOST)) {
      ...
      if (status.getState().equals(TaskState.TASK_RUNNING)) {
        ...
        if (status.getTaskId().getValue().contains(HDFSConstants.NAME_NODE_TASKID)) {
          ...
          if (status.getTaskId().getValue().contains(HDFSConstants.JOURNAL_NODE_ID) && (liveState.getJournalNodes().size() == (HDFSConstants.TOTAL_NAME_NODES + conf.getJournalNodeCount()))) {
            for (TaskID taskId : liveState.getStagingTasks()) {
              if (taskId.getValue().contains(HDFSConstants.NAME_NODE_TASKID)) {
  ...
  private void sendMessageTo(SchedulerDriver driver, TaskID taskId, String message) {
    ...
    postfix = postfix.substring(postfix.indexOf(".") + 1, postfix.length());
    driver.sendFrameworkMessage(ExecutorID.newBuilder().setValue("executor." + postfix).build(), SlaveID.newBuilder().setValue(slaveId).build(), message.getBytes());
  }
}