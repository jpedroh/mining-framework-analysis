...
public class Scheduler implements org.apache.mesos.Scheduler, Runnable {
  ...
  @Inject public Scheduler(SchedulerConf conf) {
    this.conf = conf;
    initClusterState();
    try {
  ...
  private void initClusterState() {
    ...
    ZooKeeperState zkState = new ZooKeeperState(conf.getStateZkServers(), conf.getStateZkTimeout(), TimeUnit.MILLISECONDS, "/hdfs-mesos/" + conf.getClusterName());
  ...
  private void launchInitialNameNodes(SchedulerDriver driver, Collection<Offer> offers) {
    if (offers.size() > 0) {
      ...
      pendingOffers.remove(offer.getId());
      launchNode(driver, offer, NAME_NODE_ID, Arrays.asList(NAME_NODE_ID, ZKFC_NODE_ID, JOURNAL_NODE_ID), PRIMARY_NAME_NODE_EXECUTOR_ID);
    }
    if (offers.size() > 0) {
      Offer offer = offers.iterator().next();
      pendingOffers.remove(offer.getId());
      launchNode(driver, offer, NAME_NODE_ID, Arrays.asList(NAME_NODE_ID, ZKFC_NODE_ID, JOURNAL_NODE_ID), SECONDARY_NAME_NODE_EXECUTOR_ID);
    }
  }

  @Override synchronized public void resourceOffers(SchedulerDriver driver, List<Offer> offers) {
    ...
    if (clusterState.getNameNodes().size() == 0 && clusterState.getJournalNodes().size() == 0) {
      ...
      for (Offer offer : incomingOffers) {
        if (nameNodes < TOTAL_NAME_NODES && clusterState.notInDfsHosts(offer.getSlaveId().getValue())) {
          nameNodes++;
          pendingOffers.put(offer.getId(), offer);
        } else {
          if (journalNodes < conf.getJournalNodeCount() && clusterState.notInDfsHosts(offer.getSlaveId().getValue())) {
            journalNodes++;
            pendingOffers.put(offer.getId(), offer);
          } else {
            driver.declineOffer(offer.getId());
      ...
      log.info(String.format("Currently have %d pending offers, with journalnodes=%d and namenodes=%d", pendingOffers.size(), journalNodes, nameNodes));
  ...
  @Override public void statusUpdate(SchedulerDriver driver, TaskStatus status) {
    log.info(String.format("Received status update for taskId=%s state=%s message=\'%s\' stagingTasks.size=%d", status.getTaskId().getValue(), status.getState().toString(), status.getMessage(), stagingTasks.size()));
    ClusterState clusterState = ClusterState.getInstance();
    ...
    if (status.getState().equals(TaskState.TASK_FAILED) || status.getState().equals(TaskState.TASK_FINISHED) || status.getState().equals(TaskState.TASK_KILLED) || status.getState().equals(TaskState.TASK_LOST)) {
      stagingTasks.remove(status.getTaskId());
      clusterState.removeTask(status);
    } else {
      if (status.getState().equals(TaskState.TASK_RUNNING)) {
        stagingTasks.remove(status.getTaskId());
        ...
        if (dfsTask.type == DfsTask.Type.NN) {
          ...
          if (dfsTask.type == DfsTask.Type.JN) {
            journalNodesRunning++;
  ...
  private void sendMessageTo(SchedulerDriver driver, TaskID taskId, String message) {
    ...
    postfix = postfix.substring(postfix.indexOf(".") + 1, postfix.length());
    driver.sendFrameworkMessage(ExecutorID.newBuilder().setValue("executor." + postfix).build(), SlaveID.newBuilder().setValue(dfsTask.slaveId).build(), message.getBytes());
  }
