...
public class Debugger extends ParserVisitor implements JexlInfo.Detail {
  ...
  protected Object acceptStatement(JexlNode child, Object data) {
    ...
    if (!(child instanceof ASTJexlScript || child instanceof ASTBlock || child instanceof ASTIfStatement || child instanceof ASTForeachStatement || child instanceof ASTWhileStatement || child instanceof ASTAnnotation)) {
      builder.append(';');
      if (indent > 0) {
        builder.append('\n');
      } else {
  ...
  @Override protected Object visit(ASTMethodNode node, Object data) {
    int num = node.jjtGetNumChildren();
    if (num == 2) {
      accept(node.jjtGetChild(0), data);
      accept(node.jjtGetChild(1), data);
