...
public final class Introspector {
  private static class CacheMiss {
    ...
    @SuppressWarnings(value = { "unused" }) public CacheMiss() {
    }
  }
  ...
  public Method getMethod(final Class<?> c, final MethodKey key) {
    try {
      ...
      if (logger != null && xambiguous.isSevere() && logger.isInfoEnabled()) {
        logger.info("ambiguous method invocation: " + c.getName() + "." + key.debugString(), xambiguous);
      }
      return null;
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (logger != null && logger.isDebugEnabled()) {
          logger.debug("unable to find class: " + cname + "." + key.debugString(), xnotfound);
        }
        ctor = null;
      } catch (final MethodKey.AmbiguousException xambiguous) {
        if (logger != null && xambiguous.isSevere() && logger.isInfoEnabled()) {
          logger.info("ambiguous constructor invocation: " + cname + "." + key.debugString(), xambiguous);
        }
        ctor = null;
      }
      return ctor;
