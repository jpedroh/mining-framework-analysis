...
public final class POP3ExportMbox {
      ...
      if (file != null) {
        ...
        if (mbox.isDirectory()) {
          ...
          for (int i = 1; i <= count; i++) {
            try (OutputStreamWriter fw = new OutputStreamWriter(new FileOutputStream(new File(mbox, i + ".eml")), StandardCharsets.ISO_8859_1)) {
              writeFile(pop3, fw, i);
            }
          }
        } else {
  ...
  private static void writeMbox(final POP3Client pop3, final OutputStreamWriter fw, final int i) throws IOException {
    final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("EEE MMM dd HH:mm:ss yyyy");
    final String replyTo = "MAILER-DAEMON";
