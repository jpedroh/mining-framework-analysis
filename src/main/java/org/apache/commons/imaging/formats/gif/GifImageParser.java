...
public class GifImageParser extends AbstractImageParser<GifImagingParameters> implements XmpEmbeddable<GifImagingParameters> {
  ...
  private static final byte[] XMP_APPLICATION_ID_AND_AUTH_CODE = { 0x58, 0x4D, 0x50, 0x20, 0x44, 0x61, 0x74, 0x61, 0x58, 0x4D, 0x50 };
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) private <T extends GifBlock> List<T> findAllBlocks(final List<GifBlock> blocks, final int code) {
  ...
  @Override public BufferedImage getBufferedImage(final ByteSource byteSource, final GifImagingParameters params) throws ImagingException, IOException {
    final GifImageContents imageContents = readFile(byteSource, false);
    final GifHeaderInfo ghi = imageContents.gifHeaderInfo;
    if (ghi == null) {
      throw new ImagingException("GIF: Couldn\'t read Header");
  ...
  private BufferedImage getBufferedImage(final GifHeaderInfo headerInfo, final GifImageData imageData, final byte[] globalColorTable) throws ImagingException {
  ...
  @Override public GifImagingParameters getDefaultParameters() {
  ...
  @Override public byte[] getIccProfileBytes(final ByteSource byteSource, final GifImagingParameters params) throws ImagingException, IOException {
    return null;
  }

  @Override public ImageInfo getImageInfo(final ByteSource byteSource, final GifImagingParameters params) throws ImagingException, IOException {
  ...
  @Override public Dimension getImageSize(final ByteSource byteSource, final GifImagingParameters params) throws ImagingException, IOException {
    final GifImageContents blocks = readFile(byteSource, false);
    final GifHeaderInfo bhi = blocks.gifHeaderInfo;
    if (bhi == null) {
      throw new ImagingException("GIF: Couldn\'t read Header");
  ...
  @Override public ImageMetadata getMetadata(final ByteSource byteSource, final GifImagingParameters params) throws ImagingException, IOException {
    boolean stopReadingBeforeImageData = true;
    if (params != null) {
      stopReadingBeforeImageData = params.getStopReadingBeforeImageData();
    }
  ...
  /**
     ...
     */
  @Override public String getXmpXml(final ByteSource byteSource, final XmpImagingParameters<GifImagingParameters> params) throws ImagingException, IOException {
    try (InputStream is = byteSource.getInputStream()) {
      ...
      for (final GifBlock block : blocks) {
        ...
        final String xml = new String(blockBytes, XMP_APPLICATION_ID_AND_AUTH_CODE.length, blockBytes.length - (XMP_APPLICATION_ID_AND_AUTH_CODE.length + gifMagicTrailer.length), StandardCharsets.UTF_8);
        result.add(xml);
      }
      if (result.isEmpty()) {
        return null;
  ...
  private List<GifBlock> readBlocks(final GifHeaderInfo ghi, final InputStream is, final boolean stopBeforeImageData, final FormatCompliance formatCompliance) throws ImagingException, IOException {
  ...
  private GifImageContents readFile(final ByteSource byteSource, final boolean stopBeforeImageData, final FormatCompliance formatCompliance) throws ImagingException, IOException {
    try (InputStream is = byteSource.getInputStream()) {
  ...
  private ImageDescriptor readImageDescriptor(final GifHeaderInfo ghi, final int blockCode, final InputStream is, final boolean stopBeforeImageData, final FormatCompliance formatCompliance) throws ImagingException, IOException {
  ...
  @Override public void writeImage(final BufferedImage src, final OutputStream os, GifImagingParameters params) throws ImagingException, IOException {
    if (params == null) {
    ...
    try (BinaryOutputStream bos = BinaryOutputStream.littleEndian(os)) {
      ...
      }
      {
        ...
        bos.write((byte) 0xf9);
        bos.write((byte) 4);
        final int packedFields = hasAlpha ? 1 : 0;
      ...
      }
      {
        ...
        bos.write2Bytes(height);
        {
          ...
          if (localColorTableFlag) {
            packedFields = (LOCAL_COLOR_TABLE_FLAG_MASK | (interlaceFlag ? INTERLACE_FLAG_MASK : 0) | (sortFlag ? SORT_FLAG_MASK : 0) | (7 & sizeOfLocalColorTable));
          } else {
