    ...
    try {
      ...
      for (PropertyDescriptor aPda : pda) {
        if (aPda.getReadMethod() != null && !OgnlRuntime.isMethodCallable(aPda.getReadMethod())) {
          aPda.setReadMethod(findClosestMatchingMethod(targetClass, aPda.getReadMethod(), aPda.getName(), aPda.getPropertyType(), true));
        }
        if (aPda.getWriteMethod() != null && !OgnlRuntime.isMethodCallable(aPda.getWriteMethod())) {
          aPda.setWriteMethod(findClosestMatchingMethod(targetClass, aPda.getWriteMethod(), aPda.getName(), aPda.getPropertyType(), false));
        }
  ...
  private static void findObjectIndexedPropertyDescriptors(Class<?> targetClass, Map<String, PropertyDescriptor> intoMap) throws OgnlException {
    ...
    for (Map.Entry<String, List<Method>> entry : allMethods.entrySet()) {
      ...
      if (indexMethodCheck(methods)) {
        ...
        if (((isSet = methodName.startsWith(OgnlRuntime.SET_PREFIX)) || (isGet = methodName.startsWith(OgnlRuntime.GET_PREFIX))) && (methodName.length() > 3)) {
    ...
    for (Map.Entry<String, List<Method>> entry : pairs.entrySet()) {
      ...
      List<Method> methods = entry.getValue();
      if (methods.size() == 2) {
        Method method1 = methods.get(0), method2 = methods.get(1), setMethod = (method1.getParameterTypes().length == 2) ? method1 : method2, getMethod = (setMethod == method1) ? method2 : method1;
