...
@Path(value = "/queries") public class RQuery extends RBase {
  protected static final Log l4j = LogFactory.getLog(RQuery.class.getName());

  @GET @Produces(value = "text/html") public Viewable list(@QueryParam(value = "crontabId") Integer crontabId, @QueryParam(value = "queryName") String queryName, @QueryParam(value = "extra") String extra, @QueryParam(value = "page") @DefaultValue(value = "1") int page, @QueryParam(value = "pageSize") @DefaultValue(value = "20") int pageSize) {
  ...
  @GET @Path(value = "{id}") @Produces(value = "text/html") public Viewable info(@PathParam(value = "id") Integer id) {
    QueryStore qs = QueryStore.getInstance();
    ...
    if (query.getCpuTime() != null) {
      request.setAttribute("cpuTime", Utilities.formatMsecToStr(query.getCpuTime()));
    }
    if (query.getTotalTime() != null) {
      request.setAttribute("totalTime", Utilities.formatMsecToStr(query.getTotalTime()));
    }
    if (query.getCpuTime() != null && query.getTotalTime() != null && query.getCpuTime() > query.getTotalTime()) {
      request.setAttribute("savedTime", Utilities.formatMsecToStr(Math.abs(query.getCpuTime() - query.getTotalTime())));
    }
    return new Viewable("/query/info.vm");
  }

  @GET @Path(value = "create") @Produces(value = "text/html;charset=ISO-8859-1") public Viewable create(@QueryParam(value = "queryId") Integer queryId) {
    if (queryId != null) {
  ...
  /**
     ...
     * @return
     */
  @POST @Path(value = "create/{mode}") @Produces(value = { "text/html;charset=ISO-8859-1", MediaType.APPLICATION_JSON }) public Response create(@FormParam(value = "query") String query, @FormParam(value = "name") String name, @FormParam(value = "callback") String callback, @PathParam(value = "mode") String mode) {
    ...
    if (is_api) {
      ...
      throw new WebApplicationException(Response.seeOther(URI.create("queries/" + mquery.getId())).build());
    }
  }

  @GET @Path(value = "{id}/result") @Produces(value = "text/html") public Viewable result(@PathParam(value = "id") Integer id, @QueryParam(value = "raw") @DefaultValue(value = "false") boolean raw) {
    ...
    try {
      org.apache.hadoop.fs.Path rPath = new org.apache.hadoop.fs.Path(query.getResultLocation());
      ...
      for (FileStatus _fs : fss) {
        ...
        if (!fs.getFileStatus(_fsPath).isDir()) {
          BufferedReader bf = new BufferedReader(new InputStreamReader(fs.open(_fsPath), "UTF-8"));
          if (raw) {
            ...
            while ((temp = bf.readLine()) != null) {
              writer.println(temp.replace('\u0001', '\t'));
            }
          } else {
            while ((temp = bf.readLine()) != null && readedLines < 100) {
              partialResult.add(temp.replace('\u0001', '\t'));
              readedLines++;
            }
            if (readedLines >= 100) {
              break;
