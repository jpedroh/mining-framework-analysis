...
public class QueryRunner implements Job, Running {
  protected static final Log l4j = LogFactory.getLog(QueryRunner.class.getName());
  ...
  protected ArrayList<String> queryToCmds(MQuery query) {
    ...
    Pattern setTimePattern = Pattern.compile("set(\\s+)date(\\s)*=(\\s)*(\\+|\\-)?\\s*(\\d+)\\s*(day|hour)(s?)", Pattern.CASE_INSENSITIVE);
    ...
    if (safeQuery.contains("hiveconf")) {
      ...
      if ("day".equalsIgnoreCase(timeDiffUnit)) {
        ...
        if ("hour".equalsIgnoreCase(timeDiffUnit)) {
  ...
  private void callback() {
    ...
    if (callback != null && !"".equals(callback)) {
      try {
        ...
        String postData = "id=" + URLEncoder.encode(this.mquery.getId().toString(), "UTF-8") + "&status=" + URLEncoder.encode(this.mquery.getStatus().toString(), "UTF-8") + "&error_code=" + errorCode + "&error_msg=" + errorMsg + "&result_location=" + URLEncoder.encode(this.mquery.getResultLocation(), "UTF-8") + "&result_location_url=" + URLEncoder.encode("/hwi/query_result.jsp?action=download&id=" + this.mquery.getId(), "UTF-8");
