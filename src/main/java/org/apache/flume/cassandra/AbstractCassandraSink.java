...
public abstract class AbstractCassandraSink extends AbstractSink implements Configurable {
  ...
  @Override public void configure(final Context config) {
    ...
    configuration.setDefaultReadConsistencyLevel(ConsistencyLevel.valueOf(config.getString("read_consistency", "CL_ONE")));
    configuration.setDefaultWriteConsistencyLevel(ConsistencyLevel.valueOf(config.getString("write_consistency", "CL_ONE")));
    ...
    try {
      Class<? extends ColumnFamilyChooser> chooserClass = (Class<? extends ColumnFamilyChooser>) Class.forName(chooserName);
