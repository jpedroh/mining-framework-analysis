...
public final class Index {
  ...
  private class IndexUpdateTask extends TimerTask {
    ...
    private void updateDocument(final IndexWriter writer, final String dbname, final JSONObject obj) throws IOException {
  ...
  public String query(final String dbname, final String query, final String sort_fields, final boolean ascending, final int skip, final int limit, final boolean include_docs, final boolean debug) throws IOException, ParseException {
    ...
    if (debug) {
      ...
      builder.append("<dt>search duration</dt><dd>" + DurationFormatUtils.formatDurationHMS(search_duration / 1000000) + "</dd>");
      builder.append("<dt>total duration</dt><dd>" + DurationFormatUtils.formatDurationHMS(total_duration / 1000000) + "</dd>");
  ...
  private Query visit(final Query result) {
    if (result instanceof BooleanQuery) {
      ...
      if (result instanceof ConstantScoreRangeQuery) {
        ...
        if (isNumericOrNull(rq.getLowerVal()) && isNumericOrNull(rq.getUpperVal())) {
          return new ConstantScoreRangeQuery(rq.getField(), encodeNumber(rq.getLowerVal()), encodeNumber(rq.getUpperVal()), rq.includesLower(), rq.includesUpper());
        }
        return result;
      }
    }
