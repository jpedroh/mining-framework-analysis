 ...
 */
@Path(value = "/") @Singleton @Produces(value = "image/png") public class MooseResource {
  ...
  public void initializeIfRequired() {
    if (this.mooseOverlay == null) {
      ...
      this.imageCache = CacheBuilder.<MooseRequest, Optional<BufferedImage>>newBuilder().maximumSize(20).expireAfterWrite(1, TimeUnit.DAYS).build(new MoosificatorCacheLoader());
    }
  }

  @GET @Path(value = "antler") public Response antlerificate(@QueryParam(value = "image") String sourceImage) {
    return processRequest(MooseRequest.newBuilder().withRequestType(MooseRequest.RequestType.ANTLER).withOriginalImageUrl(sourceImage));
  }

  @GET @Path(value = "moose") public Response moosificate(@QueryParam(value = "image") String sourceImage, @QueryParam(value = "debug") String debug) {
    return processRequest(MooseRequest.newBuilder().withRequestType(MooseRequest.RequestType.MOOSE).withOriginalImageUrl(sourceImage).withDebug(debug));
  }

  @GET @Path(value = "moose/{name}") public Response moosificateByName(@PathParam(value = "name") String name, @QueryParam(value = "image") String sourceImage) {
    return processRequest(MooseRequest.newBuilder().withRequestType(MooseRequest.RequestType.NAMED).withOriginalImageUrl(sourceImage).withOverlayImageName(name));
  }

  @GET @Path(value = "remoose") public Response remoosificate(@QueryParam(value = "image") String sourceImage, @QueryParam(value = "overlayImage") String overlayImageUrl) {
    return processRequest(MooseRequest.newBuilder().withRequestType(MooseRequest.RequestType.MOOSE).withOriginalImageUrl(sourceImage).withOverlayImageUrl(overlayImageUrl));
  }
  ...
  public class MoosificatorCacheLoader extends CacheLoader<MooseRequest, Optional<BufferedImage>> {
    @Override public Optional<BufferedImage> load(MooseRequest mooseRequest) throws Exception {
      try {
        return Optional.of(moosificateImage(mooseRequest));
      } catch (Throwable e) {
        MooseLogger.logEventForErrorMoosificating(mooseRequest, e);
        MooseLogger.getLogger().log(Level.WARNING, format("Error generating image for url [%s]", mooseRequest.getOriginalImageUrl().toExternalForm()), e);
  ...
  private BufferedImage moosificateImage(MooseRequest mooseRequest) throws jjil.core.Error, IOException {
    ...
    if (rectangles.isEmpty()) {
      ...
      g.drawImage(this.noFaceFoundExceptionOverlay, (int) ((canvasWidth - overlayWidth) / 2.f), (int) ((canvasHeight - overlayHeight) / 2.), overlayWidth, overlayHeight, null);
    } else {
      ...
      for (Rect rectangle : uniqueRectangles) {
        if (mooseRequest.isDebug()) {
          ...
          g.drawImage(this.mooseOverlay, (int) effectiveLeft, (int) effectiveTop, (int) effectiveWidth, (int) effectiveHeight, null);
        }
      }
    }
    return combined;
  ...
  public class AreaComparator implements Comparator<Rect> {
    @Override public int compare(Rect o1, Rect o2) {
