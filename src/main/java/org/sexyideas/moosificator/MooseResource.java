 ...
 */
@Path(value = "moose") @Singleton public class MooseResource {
  ...
  @GET @Produces(value = "image/png") public Response moosificate(@QueryParam(value = "image") String sourceImage, @QueryParam(value = "debug") String debug) {
    ...
    try {
      MooseRequest mooseRequest = MooseRequest.newBuilder().withOriginalImageUrl(imageUrl).withDebug(debug).build();
  ...
  private void logEventForMooseRetrieval(URL sourceImage) {
    try {
      ...
      LOGGER.log(Level.WARNING, format("Error storing event for retrieval of event for source image [%s]", sourceImage), e);
    }
  }
  ...
  private void logEventForErrorMoosificating(MooseRequest mooseRequest, Throwable exception) {
    try {
      ...
      LOGGER.log(Level.WARNING, format("Error storing event for retrieval of event for source image [%s]", mooseRequest.getOriginalImageUrl()), e);
    }
  }
  ...
  public class MoosificatorCacheLoader extends CacheLoader<MooseRequest, Optional<BufferedImage>> {
    @Override public Optional<BufferedImage> load(MooseRequest mooseRequest) throws Exception {
      try {
        ...
        LOGGER.log(Level.WARNING, format("Error generating image for url [%s]", mooseRequest.getOriginalImageUrl().toExternalForm()), e);
