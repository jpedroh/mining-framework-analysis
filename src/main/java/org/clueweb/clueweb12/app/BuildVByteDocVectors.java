...
public class BuildVByteDocVectors extends Configured implements Tool {
  ...
  @SuppressWarnings(value = { "static-access" }) public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("num").hasArg().withDescription("number of reducers").create(REDUCERS_OPTION));
    ...
    if (!cmdline.hasOption(INPUT_OPTION) || !cmdline.hasOption(OUTPUT_OPTION) || !cmdline.hasOption(DICTIONARY_OPTION) || !cmdline.hasOption(PREPROCESSING)) {
      HelpFormatter formatter = new HelpFormatter();
      formatter.printHelp(this.getClass().getName(), options);
      ToolRunner.printGenericCommandUsage(System.out);
      return -1;
  ...
  public static void main(String[] args) throws Exception {
    LOG.info("Running " + BuildVByteDocVectors.class.getCanonicalName() + " with args " + Arrays.toString(args));
