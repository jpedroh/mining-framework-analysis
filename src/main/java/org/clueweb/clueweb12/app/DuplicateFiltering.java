...
public class DuplicateFiltering extends Configured implements Tool {
  ...
  private static class MyPartitioner extends Partitioner<PairOfIntString, FloatArrayWritable> {
    @Override public int getPartition(PairOfIntString arg0, FloatArrayWritable arg1, int numPartitions) {
      return arg0.getLeftElement() % numPartitions;
    }
  }

  private static class MyMapper extends Mapper<Text, IntArrayWritable, PairOfIntString, FloatArrayWritable> {
  ...
  private static class MyReducer extends Reducer<PairOfIntString, FloatArrayWritable, NullWritable, Text> {
    private int topk;
    ...
    private static final HashMap<Integer, HashMap<Integer, HashMap<Integer, Float>>> termWeightsPerQuery = Maps.newHashMap();
    ...
    @Override public void reduce(PairOfIntString key, Iterable<FloatArrayWritable> values, Context context) throws IOException, InterruptedException {
  ...
  @SuppressWarnings(value = { "static-access", "deprecation" }) public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("float [0-1]").hasArg().withDescription("cosine similarity threshold").create(SIM_THRESHOLD));
    CommandLine cmdline;
    ...
    if (!cmdline.hasOption(DOCVECTOR_OPTION) || !cmdline.hasOption(OUTPUT_OPTION) || !cmdline.hasOption(DICTIONARY_OPTION) || !cmdline.hasOption(TREC_RESULT_FILE) || !cmdline.hasOption(SIM_THRESHOLD) || !cmdline.hasOption(TOPK)) {
      HelpFormatter formatter = new HelpFormatter();
      formatter.printHelp(this.getClass().getName(), options);
      ToolRunner.printGenericCommandUsage(System.out);
      return -1;
  ...
  public static void main(String[] args) throws Exception {
    LOG.info("Running " + DuplicateFiltering.class.getCanonicalName() + " with args " + Arrays.toString(args));
