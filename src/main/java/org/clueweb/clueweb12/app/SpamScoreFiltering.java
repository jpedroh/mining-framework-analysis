...
public class SpamScoreFiltering extends Configured implements Tool {
  ...
  private static class MyReducer extends Reducer<PairOfInts, Text, NullWritable, Text> {
    ...
    @Override
    public void reduce(PairOfInts key, Iterable<Text> values, Context context) throws IOException,
        InterruptedException {
  ...
  public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("path").hasArg()
        .withDescription("spam scores folder").create(SPAM_SCORES_FOLDER));
    ...
    if (!cmdline.hasOption(OUTPUT_OPTION) || !cmdline.hasOption(TREC_RESULT_FILE)
        || !cmdline.hasOption(SPAM_THRESHOLD) || !cmdline.hasOption(SPAM_SCORES_FOLDER)) {
      HelpFormatter formatter = new HelpFormatter();
      formatter.printHelp(this.getClass().getName(), options);
      ToolRunner.printGenericCommandUsage(System.out);
      return -1;
  ...
  public static void main(String[] args) throws Exception {
    LOG.info("Running " + SpamScoreFiltering.class.getCanonicalName() + " with args "
