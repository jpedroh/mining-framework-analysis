...
public class ComputeTermStatistics extends Configured implements Tool {
  ...
  private static final int MAX_DOC_LENGTH = 512 * 1024;
  ...
  private static class MyMapper extends Mapper<LongWritable, ClueWeb12WarcRecord, Text, PairOfIntLong> {
    ...
    @Override public void map(LongWritable key, ClueWeb12WarcRecord doc, Context context) throws IOException, InterruptedException {
  ...
  private static class MyCombiner extends Reducer<Text, PairOfIntLong, Text, PairOfIntLong> {
    ...
    @Override public void reduce(Text key, Iterable<PairOfIntLong> values, Context context) throws IOException, InterruptedException {
      int df = 0;
      long cf = 0;
  ...
  private static class MyReducer extends Reducer<Text, PairOfIntLong, Text, PairOfIntLong> {
    ...
    @Override public void setup(Reducer<Text, PairOfIntLong, Text, PairOfIntLong>.Context context) {
  ...
  @SuppressWarnings(value = { "static-access" }) public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("path").hasArg().withDescription("input path").create(INPUT_OPTION));
    options.addOption(OptionBuilder.withArgName("path").hasArg().withDescription("output path").create(OUTPUT_OPTION));
    options.addOption(OptionBuilder.withArgName("num").hasArg().withDescription("minimum df").create(DF_MIN_OPTION));
    ...
    if (!cmdline.hasOption(INPUT_OPTION) || !cmdline.hasOption(OUTPUT_OPTION) || !cmdline.hasOption(PREPROCESSING)) {
      HelpFormatter formatter = new HelpFormatter();
      formatter.printHelp(this.getClass().getName(), options);
      ToolRunner.printGenericCommandUsage(System.out);
      return -1;
...
<<<<<<< /usr/src/app/output/lintool/clueweb/850bda6ab8a72c664b915023c522133b365447d9/src/main/java/org/clueweb/clueweb12/app/ComputeTermStatistics.java/left.java
    conf.set(PREPROCESSING, preprocessing)
=======
    getConf().set(PREPROCESSING, preprocessing)
>>>>>>> /usr/src/app/output/lintool/clueweb/850bda6ab8a72c664b915023c522133b365447d9/src/main/java/org/clueweb/clueweb12/app/ComputeTermStatistics.java/right.java
    ;
  ...
  public static void main(String[] args) throws Exception {
    LOG.info("Running " + ComputeTermStatistics.class.getCanonicalName() + " with args " + Arrays.toString(args));
