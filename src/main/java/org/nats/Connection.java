...
public class Connection {
  ...
  public static final int INIT_BUFFER_SIZE = 1 * 1024 * 1024;

  public static final int MAX_BUFFER_SIZE = 16 * 1024 * 1024;

  public static final long REALLOCATION_THRESHOLD = 5 * 1000;
  ...
  protected static void init(Properties popts) {
    ...
    if (!popts.containsKey("max_reconnect_attempts")) {
      popts.put("max_reconnect_attempts", new Integer(DEFAULT_MAX_RECONNECT_ATTEMPTS));
    }
    if (!popts.containsKey("reconnect_time_wait")) {
      popts.put("reconnect_time_wait", new Integer(DEFAULT_RECONNECT_TIME_WAIT));
    }
    ...
    if (System.getenv("NATS_DEBUG") != null) {
      popts.put("debug", new Boolean(System.getenv("NATS_DEBUG")));
    }
    ...
    if (System.getenv("NATS_FAST_PRODUCER") != null) {
      popts.put("fast_producer", new Boolean(System.getenv("NATS_FAST_PRODUCER")));
    }
    ...
    if (System.getenv("NATS_MAX_RECONNECT_ATTEMPTS") != null) {
      popts.put("max_reconnect_attempts", Integer.parseInt(System.getenv("NATS_MAX_RECONNECT_ATTEMPTS")));
    }
    if (System.getenv("NATS_MAX_RECONNECT_TIME_WAIT") != null) {
      popts.put("max_reconnect_time_wait", Integer.parseInt(System.getenv("NATS_MAX_RECONNECT_TIME_WAIT")));
    }
  }
  ...
  public String createInbox() {
    ...
    return "_INBOX." + hexRand(0x0010000, rand) + hexRand(0x0010000, rand) + hexRand(0x0010000, rand) + hexRand(0x0010000, rand) + hexRand(0x0010000, rand) + hexRand(0x0100000, rand);
  }
    ...
    for ( ; ; ) {
      try {
        synchronized (sendBuffer) {
          ...
          if (priority || (sendBuffer.position() > MAX_PENDING_SIZE)) {
            flushPending();
          }
        }
        break;
            ...
            if ((pos = readNextOp(receiveBuffer)) == 0) {
              if (comp(buf, MSG, 3)) {
                ...
                if (comp(buf, PONG, 4)) {
                  ...
                  if (comp(buf, PING, 4)) {
                    sendCommand(PONG_RESPONSE, PONG_RESPONSE_LEN, true);
                  } else {
                    if (comp(buf, ERR, 4)) {
