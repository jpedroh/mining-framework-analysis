...
public class JSONCompare {
    ...
    if (expectedValue.getClass().isAssignableFrom(actualValue.getClass())) {
      ...
      if (isNull(expectedValue)) {
        ...
        if (isNull(actualValue)) {
          ...
          result.fail("Values of " + fullKey + " have different types: expected " + classToType(expectedValue) + ", but got " + classToType(actualValue));
        }
      }
    }
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private static void compareJSONArray(String key, JSONArray expected, JSONArray actual, JSONCompareMode mode, JSONCompareResult result) throws JSONException {
      ...
      if (allSimpleValues(expected)) {
        ...
        for (Object o : expectedCount.keySet()) {
          if (!actualCount.containsKey(o)) {
            ...
            if (actualCount.get(o) != expectedCount.get(o)) {
      ...
      for (int j = 0; j < expected.length(); ++j) {
        ...
        if (actualElement instanceof JSONObject) {
          ...
          if (actualElement instanceof JSONArray) {
            ...
            if (actualElement.equals(expected.get(j))) {
              matched.add(j);
              matchFound = true;
              break;
            }
