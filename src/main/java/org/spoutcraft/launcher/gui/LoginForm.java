...
public class LoginForm extends JFrame implements ActionListener, DownloadListener, KeyListener, WindowListener {
  ...
  public void drawCharacter(String url, int x, int y, List<JButton> buttons) {
    ...
    try {
      ...
      buttons.add(drawCropped(originalImage, type, 40, 8, 48, 16, x - 4, y - 5, 8));
      buttons.add(drawCropped(originalImage, type, 8, 8, 16, 16, x, y, 7));
      buttons.add(drawCropped(originalImage, type, 20, 20, 28, 32, x, y + 56, 7));
      buttons.add(drawCropped(originalImage, type, 44, 20, 48, 32, x - 28, y + 56, 7));
      buttons.add(drawCropped(originalImage, type, 44, 20, 48, 32, x + 56, y + 56, 7, true));
      buttons.add(drawCropped(originalImage, type, 4, 20, 8, 32, x, y + 140, 7));
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if (event.getSource() == loginSkin1 || event.getSource() == loginSkin2) {
      ...
      if (loginSkin1Image.contains(event.getSource())) {
        eventId = "Login";
        this.usernameField.setSelectedItem(loginSkin1.getText());
      } else {
        if (loginSkin2Image.contains(event.getSource())) {
          eventId = "Login";
  ...
  public void runGame() {
    ...
    if (result == LauncherFrame.SUCCESSFUL_LAUNCH) {
      ...
      if (result == LauncherFrame.ERROR_IN_LAUNCH) {
        loginButton.setEnabled(true);
        optionsButton.setEnabled(true);
        loginSkin1.setEnabled(true);
        loginSkin2.setEnabled(true);
