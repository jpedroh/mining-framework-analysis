...
public class OptionDialog extends JDialog implements ActionListener {
  ...
  public OptionDialog() {
    ...
    if (SettingsUtil.isRecommendedBuild()) {
      ...
      if (SettingsUtil.isDevelopmentBuild()) {
    ...
    gl_contentPanel.setHorizontalGroup(gl_contentPanel.createParallelGroup(Alignment.LEADING.index()).add(gl_contentPanel.createSequentialGroup().addContainerGap().add(gl_contentPanel.createParallelGroup(Alignment.LEADING.index()).add(gl_contentPanel.createSequentialGroup().add(selectBuild).addPreferredGap(LayoutStyle.RELATED).add(buildsCombo, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)).add(devBuilds).add(recBuilds).add(customBuilds).add(fastLoginCheckbox).add(alwaysUpdateCheckbox).add(clipboardCheckbox).add(backupCheckbox).add(retryLoginCheckbox).add(latestLWJGLCheckbox).add(clearCache).add(buildInfo).add(gl_contentPanel.createSequentialGroup().add(lblMemoryToAllocate).addPreferredGap(LayoutStyle.RELATED).add(memoryCombo, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE))).addContainerGap(27, Short.MAX_VALUE)));
    ...
    gl_contentPanel.setVerticalGroup(gl_contentPanel.createParallelGroup(Alignment.LEADING.index()).add(gl_contentPanel.createSequentialGroup().add(gl_contentPanel.createParallelGroup(Alignment.BASELINE.index()).add(selectBuild).add(buildsCombo, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)).add(devBuilds).add(recBuilds).add(customBuilds).add(fastLoginCheckbox).add(alwaysUpdateCheckbox).add(retryLoginCheckbox).addPreferredGap(LayoutStyle.RELATED).add(clipboardCheckbox).add(backupCheckbox).add(latestLWJGLCheckbox).addPreferredGap(LayoutStyle.RELATED).add(gl_contentPanel.createParallelGroup(Alignment.BASELINE.index()).add(memoryCombo, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE).add(lblMemoryToAllocate)).addPreferredGap(LayoutStyle.RELATED).add(clearCache).addPreferredGap(LayoutStyle.RELATED).add(buildInfo).addContainerGap(316, Short.MAX_VALUE)));
    ...
    if (id.equals("OK")) {
      ...
      if (id.equals("Cancel")) {
        ...
        if (id.equals("Clear Cache")) {
          ...
          if (id.equals(customBuilds.getText()) || id.equals(devBuilds.getText()) || id.equals(recBuilds.getText())) {
            updateBuildsCombo();
          }
        }
      }
