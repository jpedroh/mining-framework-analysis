...
public class GameUpdater {
  ...
  public boolean updateBC(Boolean force) throws Exception {
    ...
    if (!new File(this.updateDir.getPath() + File.separator + "minecraft.jar").exists()) {
      downloadFile(baseURL + "minecraft.jar?user=" + user + "&ticket=" + downloadTicket, this.updateDir + File.separator + "minecraft.jar");
    }
  ...
  @SuppressWarnings(value = { "unused" }) private void extractPack(String in, String out) throws Exception {
    File f = new File(in);
    if (!f.exists()) {
      return;
    }
  ...
  public Boolean mcUpdateAvailible(File versionFile) throws Exception {
    ...
    if (this.latestVersion > currentVersion) {
      return true;
    }
    return false;
  }
  ...
  @SuppressWarnings(value = { "unchecked", "rawtypes" }) public boolean canPlayOffline() {
    try {
