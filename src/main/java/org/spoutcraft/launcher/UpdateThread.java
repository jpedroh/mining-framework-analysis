...
public class UpdateThread extends Thread {
  ...
  @SuppressWarnings(value = { "unchecked" }) private void updateAssets(Map<String, Object> assets, File directory) {
    directory.mkdirs();
  ...
  private void cleanLogs() {
    ...
    if (logDirectory.exists() && logDirectory.isDirectory()) {
      for (File log : logDirectory.listFiles()) {
        ...
        date[2] = date[2].substring(0, date[2].length() - 4);
        try {
  ...
  private class DownloadListenerWrapper implements DownloadListener {
    ...
    @Override public void stateChanged(String fileName, float progress) {
