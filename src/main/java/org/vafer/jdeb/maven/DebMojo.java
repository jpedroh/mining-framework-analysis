 ...
 */
@Mojo(name = "jdeb", defaultPhase = LifecyclePhase.PACKAGE) public class DebMojo extends AbstractMojo {
  @Component private MavenProjectHelper projectHelper;

  @Component(hint = "jdeb-sec") private SecDispatcher secDispatcher;
  ...
  /**
     ...
     */
  @Parameter(defaultValue = "[[buildDir]]/[[artifactId]]_[[version]]_all.[[extension]]") private String deb;

  /**
     * Explicitly defines the path to the control directory. At least the
     * control file is mandatory.
     */
  @Parameter(defaultValue = "[[baseDir]]/src/deb/control") private String controlDir;

  /**
     * Explicitly define the file to read the changes from.
     */
  @Parameter(defaultValue = "[[baseDir]]/CHANGES.txt") private String changesIn;

  /**
     * Explicitly define the file where to write the changes to.
     */
  @Parameter(defaultValue = "[[buildDir]]/[[artifactId]]_[[version]]_all.changes") private String changesOut;

  /**
     * Explicitly define the file where to write the changes of the changes input to.
     */
  @Parameter(defaultValue = "[[baseDir]]/CHANGES.txt") private String changesSave;

  /**
     * The compression method used for the data file (none, gzip, bzip2 or xz)
     */
  @Parameter(defaultValue = "gzip") private String compression;

  /**
     * Boolean option whether to attach the artifact to the project
     */
  @Parameter(defaultValue = "true") private String attach;
  ...
  /**
     ...
     */
  @Parameter(defaultValue = "/opt/[[artifactId]]") private String installDir;

  /**
     * The type of attached artifact
     */
  @Parameter(defaultValue = "deb") private String type;
  ...
  /**
     * The build directory
     */
  @Parameter(property = "project.build.directory", required = true, readonly = true) private File buildDirectory;
  ...
  /**
     ...
     */
  @Parameter(defaultValue = "SNAPSHOT") private String snapshotEnv;
  ...
  @Parameter(defaultValue = "true") private boolean skipPOMs;

  @Parameter(defaultValue = "false") private boolean skipSubmodules;

  /**
     * @deprecated
     */
  @Parameter(defaultValue = "true") private boolean submodules;

  /**
     * If signPackage is true then a origin signature will be placed
     * in the generated package.
     */
  @Parameter(defaultValue = "false") private boolean signPackage;

  /**
     * Defines which utility is used to verify the signed package
     */
  @Parameter(defaultValue = "debsig-verify") private String signMethod;

  /**
     * Defines the role to sign with
     */
  @Parameter(defaultValue = "origin") private String signRole;

  /**
     * The keyring to use for signing operations.
     */
  @Parameter private String keyring;

  /**
     * The key to use for signing operations.
     */
  @Parameter private String key;

  /**
     * The passphrase to use for signing operations.
     */
  @Parameter private String passphrase;

  /**
     * The prefix to use when reading signing variables
     * from settings.
     */
  @Parameter(defaultValue = "jdeb.") private String signCfgPrefix;

  /**
     * The settings.
     */
  @Parameter(defaultValue = "${settings}") private Settings settings;
  ...
  protected VariableResolver initializeVariableResolver(Map<String, String> variables) {
    @SuppressWarnings(value = { "unchecked" }) final Map<String, String> projectProperties = Map.class.cast(getProject().getProperties());
    @SuppressWarnings(value = { "unchecked" }) final Map<String, String> systemProperties = Map.class.cast(System.getProperties());
  ...
  public void execute() throws MojoExecutionException {
    ...
    if (dataProducers.isEmpty()) {
      if (hasMainArtifact()) {
        ...
        @SuppressWarnings(value = { "unchecked" }) final Set<Artifact> projectArtifacts = project.getArtifacts();
        for (Artifact artifact : projectArtifacts) {
          artifacts.add(artifact);
        }
        @SuppressWarnings(value = { "unchecked" }) final List<Artifact> attachedArtifacts = project.getAttachedArtifacts();
        ...
        for (Artifact artifact : artifacts) {
          ...
          if (file != null) {
            dataProducers.add(new DataProducer() {
              @Override public void produce(final DataConsumer receiver) {
                try {
  ...
  private void initializeSignProperties() {
    ...
    Map<String, String> properties = readPropertiesFromActiveProfiles(signCfgPrefix, KEY, KEYRING, PASSPHRASE);
  ...
  /**
     ...
     */
  public Map<String, String> readPropertiesFromActiveProfiles(final String prefix, final String... properties) {
    if (settings == null) {
