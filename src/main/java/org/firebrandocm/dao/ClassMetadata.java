 ...
 */
public class ClassMetadata<T extends java.lang.Object> {
  ...
  protected void processElement(Field element, String propertyName, Class<?> type) throws ClassNotFoundException, IntrospectionException {
    if (valid(element, propertyName, type)) {
      if (element.isAnnotationPresent(Embedded.class)) {
        ...
        if (element.isAnnotationPresent(Mapped.class)) {
          processMappedEntity(type, element, propertyName);
        } else {
          if (element.isAnnotationPresent(MappedCollection.class)) {
            processMappedCollection(type, element, propertyName);
          } else {
            if (element.isAnnotationPresent(CounterIncrease.class)) {
  ...
  protected boolean valid(Field element, String name, Class<?> type) {
    return !name.equals(CLASS_PROPERTY) && !(element.isAnnotationPresent(Transient.class)) && !name.equals(keyProperty) && !type.getName().equals(PersistenceFactory.class.getName()) && !Modifier.isStatic(element.getModifiers());
  }
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) public T createProxy() {
    T instance;
    try {
