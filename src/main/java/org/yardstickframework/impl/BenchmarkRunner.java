...
public class BenchmarkRunner {
  ...
  @SuppressWarnings(value = { "FieldAccessedSynchronizedAndUnsynchronized" }) private Thread buildingThread;
  ...
  private void startBuildingThread() {
    ...
    buildingThread = new Thread() {
      @SuppressWarnings(value = { "BusyWait" }) @Override public void run() {
        try {
      ...
      try {
        ...
        for (int i = 0; i < drivers.length; i++) {
          try {
            ...
            errorHelp(cfg, "Failed to gracefully stop driver [driver=" + drivers[i] + ", err=" + e.getMessage() + ']', e);
          }
          try {
            probeSets[i].stop();
          } catch (Exception e) {
            errorHelp(cfg, "Failed to gracefully stop probe set [probe=" + probeSets[i] + ", err=" + e.getMessage() + ']', e);
          }
        }
      } catch (Exception e) {
