...
public class JFreeChartGraphPlotter {
  ...
  /**
     ...
     * @throws Exception If failed.
     */
  private static void processFilesPerProbe(Map<String, List<File>> res, File folderToWrite, JFreeChartGraphPlotterArguments args, JFreeChartGenerationMode mode) throws Exception {
    Map<String, List<JFreeChartPlotInfo>> infoMap = new HashMap<>();
      ...
      for (String line; (line = br.readLine()) != null; ) {
        ...
        if (data.isEmpty()) {
          ...
          for (int i = 0; i < plotNum; i++) {
            ...
            String yAxisLabel = metaInfo == null || i + 1 >= metaInfo.length ? "" : metaInfo[i + 1].replace("\"", "");
