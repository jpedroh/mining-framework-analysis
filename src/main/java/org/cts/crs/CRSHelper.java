...
public class CRSHelper {
  ...
  public static CoordinateReferenceSystem createCoordinateReferenceSystem(Identifier identifier, Map<String, String> parameters) {
    ...
    if (geodeticDatum == null) {
      LOGGER.warn("No datum definition. Cannot create the" + "CoordinateReferenceSystem");
      return null;
    }
    GeodeticCRS crs;
    ...
    if (sproj.equals(ProjValueParameters.GEOCENT)) {
      ...
      CoordinateSystem cs = new CoordinateSystem(new Axis[] { Axis.X, Axis.Y, Axis.Z }, new Unit[] { unit, unit, unit });
      crs = new GeocentricCRS(identifier, geodeticDatum, cs);
    } else {
      if (sproj.equals(ProjValueParameters.LONGLAT)) {
        Unit unit = Unit.DEGREE;
        if (stometer != null) {
          unit = new Unit(Quantity.LENGTH, "", Double.parseDouble(sunit), "");
        }
        CoordinateSystem cs = new CoordinateSystem(new Axis[] { Axis.LONGITUDE, Axis.LATITUDE, Axis.HEIGHT }, new Unit[] { unit, unit, Unit.METER });
        crs = new Geographic3DCRS(identifier, geodeticDatum, cs);
      } else {
        Projection proj = getProjection(sproj, geodeticDatum.getEllipsoid(), parameters);
        if (null != proj) {
          crs = new ProjectedCRS(identifier, geodeticDatum, proj);
        } else {
  ...
  public static void setDefaultWGS84Parameters(GeodeticDatum gd, Map<String, String> param) {
    ...
    if (identity) {
      ...
      if (translation) {
        ...
        op = SevenParameterTransformation.createBursaWolfTransformation(bwp[0], bwp[1], bwp[2], bwp[3], bwp[4], bwp[5], bwp[6]);
      }
    }
    if (op != null) {
  ...
  public static PrimeMeridian getPrimeMeridian(Map<String, String> param) {
    ...
    if (null != pmName) {
      if (pmName.equals("greenwich")) {
        ...
        if (pmName.equals("paris")) {
          return PrimeMeridian.PARIS;
        } else {
          if (pmName.equals("lisbon")) {
            return PrimeMeridian.LISBON;
          } else {
            if (pmName.equals("bogota")) {
              return PrimeMeridian.BOGOTA;
            } else {
              if (pmName.equals("madrid")) {
                return PrimeMeridian.MADRID;
              } else {
                if (pmName.equals("rome")) {
                  return PrimeMeridian.ROME;
                } else {
                  if (pmName.equals("bern")) {
                    return PrimeMeridian.BERN;
                  } else {
                    if (pmName.equals("jakarta")) {
                      return PrimeMeridian.JAKARTA;
                    } else {
                      if (pmName.equals("ferro")) {
                        return PrimeMeridian.FERRO;
                      } else {
                        if (pmName.equals("brussels")) {
                          return PrimeMeridian.BRUSSELS;
                        } else {
                          if (pmName.equals("stockholm")) {
                            return PrimeMeridian.STOCKHOLM;
                          } else {
                            if (pmName.equals("athens")) {
                              return PrimeMeridian.ATHENS;
                            } else {
                              if (pmName.equals("oslo")) {
                                return PrimeMeridian.OSLO;
                              } else {
                                try {
                                  double pmdd = Double.parseDouble(pmName);
                                  return PrimeMeridian.createPrimeMeridianFromDDLongitude(new Identifier(PrimeMeridian.class, Identifiable.UNKNOWN), pmdd);
  ...
  public static boolean isDatumSupported(String datumName) {
    if (null != datumName) {
      if (datumName.equals(GeodeticDatum.WGS84.getName())) {
        ...
        if (datumName.equals(GeodeticDatum.ED50.getName())) {
          return true;
        } else {
          if (datumName.equals(GeodeticDatum.NTF.getName())) {
            return true;
          } else {
            if (datumName.equals(GeodeticDatum.NTF_PARIS.getName())) {
              return true;
            } else {
              if (datumName.equals(GeodeticDatum.RGF93.getName())) {
                return true;
              }
            }
          }
  ...
  public static GeodeticDatum getDatum(Map<String, String> param) {
    ...
    if (null != datumName) {
      if (datumName.equalsIgnoreCase(GeodeticDatum.WGS84.getShortName())) {
        ...
        if (datumName.equalsIgnoreCase(GeodeticDatum.ED50.getShortName())) {
          return GeodeticDatum.ED50;
        } else {
          if (datumName.equalsIgnoreCase(GeodeticDatum.NTF.getShortName())) {
            return GeodeticDatum.NTF;
          } else {
            if (datumName.equalsIgnoreCase(GeodeticDatum.NTF_PARIS.getShortName())) {
              return GeodeticDatum.NTF_PARIS;
            } else {
              if (datumName.equalsIgnoreCase(GeodeticDatum.RGF93.getShortName())) {
      ...
      for (String grid : grids) {
        if (!grid.equals("null")) {
          ...
          if (grid.equals("@null")) {
            ...
            try {
              NTv2GridShiftTransformation gt = new NTv2GridShiftTransformation(GridShift.class.getResource(grid).toURI().toURL());
  ...
  public static Ellipsoid getEllipsoid(Map<String, String> param) {
    ...
    if (null != ellipsoidName) {
      if (ellipsoidName.equals("GRS80")) {
        ...
        if (ellipsoidName.equals("WGS_1984") || (ellipsoidName.equals("WGS 84"))) {
          return Ellipsoid.WGS84;
        } else {
          if (ellipsoidName.equals("International_1924") || ellipsoidName.equals("intl")) {
            return Ellipsoid.INTERNATIONAL1924;
          } else {
            if (ellipsoidName.equals("clrk66")) {
              return Ellipsoid.CLARKE1866;
            } else {
              if (ellipsoidName.equals("clrk80")) {
                return Ellipsoid.CLARKE1880ARC;
              } else {
                if (ellipsoidName.equals("Clarke_1880_IGN") || ellipsoidName.equals("Clarke 1880 (IGN)") || ellipsoidName.equals("Clarke_1880")) {
                  return Ellipsoid.CLARKE1880IGN;
                } else {
                  if (ellipsoidName.equals("Clarke_1880_RGS") || ellipsoidName.equals("Clarke 1880 (RGS)")) {
                    return Ellipsoid.CLARKE1880RGS;
                  } else {
                    if (ellipsoidName.equals("sphere")) {
                      return Ellipsoid.SPHERE;
                    } else {
                      if (ellipsoidName.equals("bessel")) {
                        return Ellipsoid.BESSEL1841;
                      } else {
                        if (ellipsoidName.equals("krass")) {
                          return Ellipsoid.KRASSOWSKI;
                        } else {
                          if (ellipsoidName.equals("evrstSS")) {
                            return Ellipsoid.EVERESTSS;
                          } else {
                            if (ellipsoidName.equals("GRS67")) {
                              return Ellipsoid.GRS67;
                            } else {
                              if (ellipsoidName.equals("aust_SA")) {
  ...
  /**
         ...
         * @return a Projection
         */
  public static Projection getProjection(String projectionName, Ellipsoid ell, Map<String, String> param) {
