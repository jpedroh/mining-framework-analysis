...
public class CRSHelper {
  ...
  public static CoordinateReferenceSystem createCoordinateReferenceSystem(Identifier identifier, Map<String, String> parameters) {
    ...
    if (sproj.equals(ProjValueParameters.GEOCENT)) {
      ...
      if (sproj.equals(ProjValueParameters.LONGLAT)) {
        CoordinateSystem cs = new CoordinateSystem(new Axis[] { Axis.LONGITUDE, Axis.LATITUDE, Axis.HEIGHT }, new Unit[] { Unit.DEGREE, Unit.DEGREE, Unit.METER });
    ...
    if (null != pmName) {
      ...
      if (pm == null) {
        try {
          ...
          pm = PrimeMeridian.createPrimeMeridianFromDDLongitude(new Identifier(PrimeMeridian.class, Identifiable.UNKNOWN), pmdd);
          ...
          if (grid.equals("@null")) {
            ...
            if (grid.equals("ntf_r93.gsb")) {
              if (crs.getDatum().equals(GeodeticDatum.NTF)) {
                crs.addGridTransformation(GeodeticDatum.RGF93, new CoordinateOperationSequence(new Identifier(CoordinateOperation.class, "NTF" + " to " + "RGF93"), new LongitudeRotation(GeodeticDatum.NTF.getPrimeMeridian().getLongitudeFromGreenwichInRadians() - GeodeticDatum.RGF93.getPrimeMeridian().getLongitudeFromGreenwichInRadians()), new Geographic2Geocentric(GeodeticDatum.NTF.getEllipsoid()), new FrenchGeocentricNTF2RGF(), new Geocentric2Geographic(GeodeticDatum.RGF93.getEllipsoid())));
              } else {
                if (crs.getDatum().equals(GeodeticDatum.NTF_PARIS)) {
                  crs.addGridTransformation(GeodeticDatum.RGF93, new CoordinateOperationSequence(new Identifier(CoordinateOperation.class, "NTF" + " to " + "RGF93"), GeodeticDatum.NTF_PARIS.getCoordinateOperations(GeodeticDatum.NTF).get(0), new LongitudeRotation(GeodeticDatum.NTF.getPrimeMeridian().getLongitudeFromGreenwichInRadians() - GeodeticDatum.RGF93.getPrimeMeridian().getLongitudeFromGreenwichInRadians()), new Geographic2Geocentric(GeodeticDatum.NTF.getEllipsoid()), new FrenchGeocentricNTF2RGF(), new Geocentric2Geographic(GeodeticDatum.RGF93.getEllipsoid())));
                }
              }
            }
            try {
  ...
  /**
         ...
         * @return a Projection
         */
  private static Projection getProjection(String projectionName, Ellipsoid ell, Map<String, String> param) {
