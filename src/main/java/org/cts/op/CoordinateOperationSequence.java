...
public class CoordinateOperationSequence extends AbstractCoordinateOperation {
  ...
  @Override public CoordinateOperation inverse() throws NonInvertibleOperationException {
    CoordinateOperation[] inverse_sequence = new CoordinateOperation[sequence.length];
    for (int i = 0; i < sequence.length; i++) {
      inverse_sequence[sequence.length - i - 1] = sequence[i].inverse();
    }
    return new CoordinateOperationSequence(getIdentifier(), inverse_sequence, precision);
  }
