...
public class Utils2D {
  ...
  public static Location calculateBoardPlacementLocation(BoardLocation bl, Location placementLocation) {
    ...
    Location l = new Location(LengthUnit.Millimeters, p.getX(), p.getY(), boardLocation.getZ(), angle + placementLocation.getRotation());
  ...
  public static Location calculateBoardPlacementLocationInverse(BoardLocation bl, Location placementLocation) {
    ...
    Point2D p = new Point2D.Double(placementLocation.getX(), placementLocation.getY());
    p = tx.transform(p, null);
    Location l = new Location(LengthUnit.Millimeters, bl.getSide() == Side.Bottom ? -p.getX() : p.getX(), p.getY(), 0., angle + placementLocation.getRotation());
    l = l.convertToUnits(placementUnits);
    return l;
  }
  ...
  public static Location calculateBoardLocation(BoardLocation boardLocation, Placement placementA, Placement placementB, Location observedLocationA, Location observedLocationB) {
    ...
    location = location.derive(null, null, boardLocation.getLocation().convertToUnits(location.getUnits()).getZ(), angle);
    return location;
  }
  ...
  public static AffineTransform deriveAffineTransform(Location source1, Location source2, Location source3, Location dest1, Location dest2, Location dest3) {
    ...
    return deriveAffineTransform(source1.getX(), source1.getY(), source2.getX(), source2.getY(), source3.getX(), source3.getY(), dest1.getX(), dest1.getY(), dest2.getX(), dest2.getY(), dest3.getX(), dest3.getY());
  }

  public static AffineTransform deriveAffineTransform(Location source1, Location source2, Location dest1, Location dest2) {
    source1 = source1.convertToUnits(LengthUnit.Millimeters);
    source2 = source2.convertToUnits(LengthUnit.Millimeters);
    dest1 = dest1.convertToUnits(LengthUnit.Millimeters);
    dest2 = dest2.convertToUnits(LengthUnit.Millimeters);
    return deriveAffineTransform(source1.getX(), source1.getY(), source2.getX(), source2.getY(), dest1.getX(), dest1.getY(), dest2.getX(), dest2.getY());
  }

  public static AffineTransform deriveAffineTransform(double sourceX1, double sourceY1, double sourceX2, double sourceY2, double sourceX3, double sourceY3, double destX1, double destY1, double destX2, double destY2, double destX3, double destY3) {
    RealMatrix source = MatrixUtils.createRealMatrix(new double[][] { { sourceX1, sourceX2, sourceX3 }, { sourceY1, sourceY2, sourceY3 }, { 1, 1, 1 } });
    RealMatrix dest = MatrixUtils.createRealMatrix(new double[][] { { destX1, destX2, destX3 }, { destY1, destY2, destY3 } });
  ...
  public static AffineTransform deriveAffineTransform(double sourceX1, double sourceY1, double sourceX2, double sourceY2, double destX1, double destY1, double destX2, double destY2) {
  ...
  public static AffineTransform deriveAffineTransform(List<Location> source, List<Location> destination) {
    ...
    if (n > 2) {
      ...
      double scaleFactor = Math.hypot(dHat.getEntry(0, 0) - dHat.getEntry(0, 1), dHat.getEntry(1, 0) - dHat.getEntry(1, 1)) / Math.hypot(sHat.getEntry(0, 0) - sHat.getEntry(0, 1), sHat.getEntry(1, 0) - sHat.getEntry(1, 1));
    ...
    return new AffineTransform(linearTransform.getEntry(0, 0), linearTransform.getEntry(1, 0), linearTransform.getEntry(0, 1), linearTransform.getEntry(1, 1), translation.getEntry(0, 0), translation.getEntry(1, 0));
  }
