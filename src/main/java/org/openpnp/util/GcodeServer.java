...
public class GcodeServer extends Thread {
  ...
  static final String firmware = "FIRMWARE_NAME:GcodeServer, FIRMWARE_URL:http%3A//openpnp.org, X-SOURCE_CODE_URL:https%3A//github.com/openpnp/openpnp, FIRMWARE_VERSION:" + Main.getVersion() + ", " + "X-FIRMWARE_BUILD_DATE:Oct 23 2020 00:00:00";
  ...
  class Worker extends Thread {
        ...
        if (this.code == null) {
          if (letter == 'T') {
            ...
            if (letter == 'O') {
              ...
              if (letter == 'M' && getNumberIntegral() / 100 == 1) {
    ...
    public List<GcodeWord> finalizeGcodeWord(GcodeWord currentWord, List<GcodeWord> commandWords) throws Exception {
      if (currentWord != null) {
    ...
    private void simulateGcode(List<GcodeWord> commandWords) throws Exception {
      if (!commandWords.isEmpty()) {
          ...
          if (axis instanceof ControllerAxis) {
            ...
            if (controllerAxis.getDriver() == getDriver()) {
              ...
              if (letter.isEmpty() || letter.length() > 1) {
                if (axis.getName().equals("x") || axis.getName().equals("y") || axis.getName().toLowerCase().substring(0, 1).equals("z")) {
                  controllerAxis.setLetter(axis.getName().toUpperCase().substring(0, 1));
                } else {
                  if (axis.getName().equals("C") || axis.getName().equals("C1")) {
                    controllerAxis.setLetter("A");
                  } else {
                    if (axis.getName().equals("C2")) {
        ...
        if (fWord != null) {
          feedRate = fWord.getNumberDouble() * unit / 60;
        }
        ...
        if (m114Word != null) {
          ...
          if (m114Word.getNumberFraction() == 0) {
            ...
            if (m114Word.getNumberFraction() == 1) {
        ...
        long dwellMilliseconds = (motionPlan.isEmpty() ? 0 : (int) Math.max(0, (motionPlan.lastKey() - NanosecondTime.getRuntimeSeconds()) * 1000));
        ...
        if (g0Word != null || g1Word != null || g28Word != null || (hWord != null && hWord.isDollar())) {
          ...
          if (g28Word != null || hWord != null && hWord.isDollar()) {
            ...
            homingOffsets = new AxesLocation(machine, getDriver(), (axis) -> (axis.getType() == Axis.Type.X ? homingError.getLengthX() : axis.getType() == Axis.Type.Y ? homingError.getLengthY() : null));
            speed = 0.5;
          ...
          Motion motion = new Motion(null, machineLocation, axesLocation, speed, feedRate, acceleration, jerk, (g0Word != null ? MotionOption.UncoordinatedMotion.flag() : 0));
  ...
  public Motion getMomentaryMotion(double time) {
    ...
    if (entry1 != null) {
      ...
      Motion motion = new Motion(null, getMachineLocation(), getMachineLocation(), 1.0, MotionOption.Stillstand);
