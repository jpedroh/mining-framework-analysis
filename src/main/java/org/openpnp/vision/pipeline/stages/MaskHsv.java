...
@Stage(description = "Mask color from an image based on the HSV color space. Pixels that fall between (hueMin, saturationMin, valueMin) and (hueMax, saturationMax, valueMax) are set to black in the output image. This stage expects the input to be in HSV_FULL format, so you should do a ConvertColor with Bgr2HsvFull before this stage and ConvertColor Hsv2BgrFull after. These are not applied internally as to not complicate the use of multiple instances of this stage in series. Note that this stage can be used with any 3 channel, 8 bit per channel color space. The order of the filtered channels is hue, saturation, value, but you can use these ranges for other channels.") public class MaskHsv extends CvStage {
  @Attribute @Property(description = "First hue to be masked.  Note hues range from 0 to 255 (inclusive) but in a circular fashion so that 255 is directly adjacent to 0 (as 359 degrees is adjacent to 0 degrees).  To mask hues that cross the 255-0 boundary, set hueMin greater than hueMax.  As a rough guide, yellows fall in the range 21 to 64, greens 64 to 107, cyans 107 to 149, blues 149 to 192, magentas 192 to 235, and reds 235 to 21.") private int hueMin = 31;


<<<<<<< /usr/src/app/output/openpnp/openpnp/f1d9c747ac3d7f0376b56e1c81f4da617f396e98/src/main/java/org/openpnp/vision/pipeline/stages/MaskHsv.java/left.java
  @Attribute(required = false) @Property(description = "If set, the mask is returned directly as a grayscale image with the masked area black, the unmasked white. Otherwise the masked area is blackened in the source image.") private boolean binaryMask = false;
=======
  @Attribute(required = false) @Property(description = "Sets each channel\'s min and max limits such that the pixels falling in and around the channel\'s histogram peak bin are masked.  The number of histogram bins that get masked is controlled by the value of the fractionToMask parameter.") private Boolean auto = false;
>>>>>>> /usr/src/app/output/openpnp/openpnp/f1d9c747ac3d7f0376b56e1c81f4da617f396e98/src/main/java/org/openpnp/vision/pipeline/stages/MaskHsv.java/right.java


  @Attribute(required = false) @Property(description = "When the auto flag is set, the min and max limits are set so that the fraction of pixels in the image that get masked is approximately equal to this value.  The pixels with the most commonly occurring colors are masked first.   Valid range is from 0.0 to 1.0 (inclusive).") private double fractionToMask = 0.0;

  @Attribute @Property(description = "Last hue to be masked.  Note hues range from 0 to 255 (inclusive) but in a circular fashion so that 255 is directly adjacent to 0 (as 359 degrees is adjacent to 0 degrees).  To mask hues that cross the 255-0 boundary, set hueMin greater than hueMax.  As a rough guide, yellows fall in the range 21 to 64, greens 64 to 107, cyans 107 to 149, blues 149 to 192, magentas 192 to 235, and reds 235 to 21.") private int hueMax = 116;

  @Attribute @Property(description = "Minimum saturation to be masked.  Note saturations range from 0 to 255 (inclusive). Setting saturationMin greater than saturationMax will result in no pixels being masked.") private int saturationMin = 0;

  @Attribute @Property(description = "Maximum saturation to be masked.  Note saturations range from 0 to 255 (inclusive). Setting saturationMax less than saturationMin will result in no pixels being masked.") private int saturationMax = 255;

  @Attribute @Property(description = "Minimum value to be masked.  Note values range from 0 to 255 (inclusive). Setting valueMin greater than valueMax will result in no pixels being masked.") private int valueMin = 0;

  @Attribute @Property(description = "Maximum value to be masked.  Note values range from 0 to 255 (inclusive). Setting valueMax less than valueMin will result in no pixels being masked.") private int valueMax = 255;

  @Attribute(required = false) @Property(description = "Inverts the selection of pixels to mask.") private Boolean invert;
  ...
  @Override public Result process(CvPipeline pipeline) throws Exception {
    ...
    if (auto) {
      ...
      while (amountMasked < amountToMask) {
        if (upDown) {
          endIdx = (endIdx + 1) % numberOfBins;
          currentLevel = hist.get(endIdx, 0)[0];
          amountMasked = amountMasked + currentLevel;
        } else {
          startIdx = (startIdx - 1 + numberOfBins) % numberOfBins;
