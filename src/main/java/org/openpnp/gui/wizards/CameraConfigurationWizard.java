...
@SuppressWarnings(value = { "serial" }) public class CameraConfigurationWizard extends AbstractConfigurationWizard {
  ...
  private static String downLookingCalibrationInstructions = "<html>\r\n\r\n" + "Note: Calibrating Units Per Pixel at two different heights is not manditory but " + "does enable additional features. If Dual Height calibration is not desired, " + "calibrate the Primary Units Per Pixel at <b>exactly</b> the desired working " + "height using the instructions below and then copy those settings to the Secondary " + "Units Per Pixel fields.\r\n<ol>\r\n" + "<li>Select a rectangular object with a known width and length that will fit within " + "the camera\'s field-of-view. Graphing paper is a good, easy choice for this. Enter " + "the object\'s width and length into the Width and Length fields above.\r\n" + "<li>Place the object on the table and align it square with the camera\'s reticle.\r\n" + "<li>Select the Primary (or Secondary) Units Per Pixel to calibrate using the Cal " + "Select radio buttons.  Note that the Primary Units Per Pixel should be calibrated " + "near the same Z height as the top surface of the circuit board(s) to be populated " + "and the Secondary at least as high as the top of the tallest part that may ever be " + "placed.\r\n" + "<li>Jog the nozzle over the object and then down so that it is just touching the " + "object.  Press Capture Z.  If calibrating the Primary Units Per Pixel, a dialog " + "will pop-up asking if the measurement location height should be used as the " + "default working height for the camera.  Answer Yes or No.  (If No, be sure to " + "manually populate the Default Working Height field after Units Per Pixel " + "calibration is complete.)\r\n" + "<li>Jog the camera to where it is centered over the object.\r\n" + "<li>Press Measure and use the camera selection rectangle to measure the object.  " + "If the object is not in perfect focus, use the middle of the blurry edges for the " + "measurement.  Press Confirm when finished.\r\n" + "<li>The calculated units per pixel values will be inserted into the X and Y " + "fields.\r\n" + "<li>Place a spacer (about as thick as the tallest part that may ever be expected to " + "be placed) under the object (or for machines that can physically move the top " + "camera in Z, jog the camera up or down by about that much) and repeat steps 3 " + "though 7 for the Secondary Units Per Pixel. Note that if the object now appears " + "too big to fit within the camera\'s field-of-view, a smaller object can be used for " + "this as long as the Width and Length fields are updated to match its size.\r\n" + "</ol>\r\n</html>";

  private static String upLookingCalibrationInstructions = "<html>\r\n\r\n" + "Note: Calibrating Units Per Pixel at two different heights is not manditory but it " + "does enable additional features. If Dual Height calibration is not desired, " + "calibrate the Primary Units Per Pixel per the instructions below and then copy " + "those settings to the Secondary Units Per Pixel fields.\r\n<ol>\r\n" + "<li>Select an object with a known width, length, and thickness that will fit within " + "the camera\'s field-of-view. Enter the object\'s width, length, and thickness into " + "the Width, Length, and Thickness fields above.\r\n" + "<li>Place the object square on the table and using a nozzle, pick up the object.\r\n" + "<li>Select the Primary (or Secondary) Units Per Pixel to calibrate using the Cal " + "Select radio buttons. Note that the Primary Units Per Pixel is calibrated at the " + "up looking camera\'s location while the Secondary should be calibrated at a higher " + "location.\r\n" + "<li>WARNING - if the nozzle is not already over the up looking camera\'s position, " + "the next step will automatically move the nozzle to that position.\r\n" + "<li>Press Measure and use the camera selection rectangle to measure the object.  If " + "necessary, use the jog panel to rotate the nozzle so that the object is square with " + "the selection rectangle.  If the object is not in perfect focus, use the middle of " + "the blurry edges for the measurement.  Press Confirm when finished.\r\n" + "<li>The calculated units per pixel values will be inserted into the X and Y " + "fields.\r\n" + "<li>Jog the nozzle up by about the thickness of the tallest part that may be " + "expected to be placed and repeat steps 3 though 6 for the Secondary Units Per " + "Pixel.\r\n" + "</ol>\r\n</html>";
  ...
  public CameraConfigurationWizard(AbstractBroadcastingCamera camera) {
    ...
    panelDefaultWorkingPlane.setBorder(new TitledBorder(null, "Default Working Height", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
    contentPanel.add(panelDefaultWorkingPlane);
    panelDefaultWorkingPlane.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(57dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC }));
    ...
    textFieldDefaultZ.setToolTipText("<html>" + "This is the Z level at which objects in the camera view are assumed<br>" + "to be if their true height is unknown.  Generally this should be set<br>" + "to the Z height of the working surface of the circuit board(s) that<br>" + "are to be populated.</html>");
    ...
    panelUpp.setBorder(new TitledBorder(null, "Units Per Pixel", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
    panelUpp.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(99dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.UNRELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC }));
    lblX = new JLabel("X");
    ...
    textFieldPrimaryUppHeightAboveCamera.setToolTipText("<html>" + "This is the height above the camera at which the Primary Units Per Pixel<br>" + "is calibrated.</html>");
    ...
    textFieldSecondaryUppHeightAboveCamera.setToolTipText("<html>" + "This is the height above the camera at which the Secondary Units Per Pixel<br>" + "is calibrated. If dual-height calibration is not to be used, set this to the<br>" + "same height as the Primary Units Per Pixel.</html>");
    ...
    panelCal.setBorder(new TitledBorder(null, "Calibration Tool", TitledBorder.LEADING, TitledBorder.TOP, null, null));
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      if (rdbtnPrimaryUpp.isSelected() || rdbtnSecondaryUpp.isSelected()) {
        if ((Camera.Looking) lookingCb.getSelectedItem() == Camera.Looking.Up) {
          ...
          if (!nozzleLocation.getLengthX().equals(desiredNozzleLocation.getLengthX()) || !nozzleLocation.getLengthY().equals(desiredNozzleLocation.getLengthY()) || !nozzleLocation.getLengthZ().equals(desiredNozzleLocation.getLengthZ()) || nozzleLocation.getRotation() != desiredNozzleLocation.getRotation()) {
          ...
          if (rdbtnPrimaryUpp.isSelected()) {
            textFieldDefaultZ.setText(String.format(Locale.US, zFormat, measurementLocation.getZ()));
          }
        }
        ...
        JOptionPane.showMessageDialog(null, "Select either the Primary or the Secondary Units Per Pixel to calibrate.", "Info", JOptionPane.INFORMATION_MESSAGE, null);
      }
    }
  };
  ...
  private Action confirmMeasureAction = new AbstractAction("Confirm") {
    @Override public void actionPerformed(ActionEvent arg0) {
      ...
      if (rdbtnPrimaryUpp.isSelected()) {
        textFieldPrimaryUppX.setText(String.format(Locale.US, uppFormat, (width / Math.abs(selection.width))));
        textFieldPrimaryUppY.setText(String.format(Locale.US, uppFormat, (height / Math.abs(selection.height))));
      } else {
        textFieldSecondaryUppX.setText(String.format(Locale.US, uppFormat, (width / Math.abs(selection.width))));
        textFieldSecondaryUppY.setText(String.format(Locale.US, uppFormat, (height / Math.abs(selection.height))));
      }
      double heightAboveCamera = ((AbstractCamera) camera).getHeightAboveCamera(measurementLocation).getValue();
      if (rdbtnPrimaryUpp.isSelected()) {
        textFieldPrimaryUppHeightAboveCamera.setText(String.format(Locale.US, zFormat, heightAboveCamera));
      } else {
        textFieldSecondaryUppHeightAboveCamera.setText(String.format(Locale.US, zFormat, heightAboveCamera));
      }
    }
  };
