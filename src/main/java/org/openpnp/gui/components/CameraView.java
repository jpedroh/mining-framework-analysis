...
@SuppressWarnings(value = { "serial" }) public class CameraView extends JComponent implements CameraListener {
  ...
  @Override public void frameReceived(BufferedImage img) {
    if (cameraViewFilter != null) {
      img = cameraViewFilter.filterCameraImage(camera, img);
    }
    BufferedImage oldFrame = lastFrame;
    lastFrame = img;
    if (oldFrame == null || (oldFrame.getWidth() != img.getWidth() || oldFrame.getHeight() != img.getHeight() || camera.getUnitsPerPixel() != lastUnitsPerPixel)) {
  ...
  @Override protected synchronized void paintComponent(Graphics g) {
    ...
    if (image != null) {
      ...
      for (Reticle reticle : reticles.values()) {
        reticle.draw(g2d, camera.getUnitsPerPixel().getUnits(), scaledUnitsPerPixelX, scaledUnitsPerPixelY, ins.left + (width / 2), ins.top + (height / 2), scaledWidth, scaledHeight, c);
      }
      if (text != null) {
  ...
  private void paintSelection(Graphics2D g2d) {
    ...
    g2d.setStroke(new BasicStroke(1f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, selectionDashProfile, selectionDashPhase));
    g2d.drawRect(rx, ry, rw, rh);
  ...
  private HandlePosition getSelectionHandleAtPosition(int x, int y) {
    ...
    if (isWithinHandle(x, y, rx, ry)) {
      ...
      if (isWithinHandle(x, y, rx2, ry)) {
        return HandlePosition.NE;
      } else {
        if (isWithinHandle(x, y, rx, ry2)) {
          return HandlePosition.SW;
        } else {
          if (isWithinHandle(x, y, rx2, ry2)) {
            return HandlePosition.SE;
          } else {
            if (isWithinHandle(x, y, rxc, ry)) {
              return HandlePosition.N;
            } else {
              if (isWithinHandle(x, y, rx2, ryc)) {
                return HandlePosition.E;
              } else {
                if (isWithinHandle(x, y, rxc, ry2)) {
                  return HandlePosition.S;
                } else {
                  if (isWithinHandle(x, y, rx, ryc)) {
  ...
  private static HandlePosition getOpposingHandle(Rectangle r, HandlePosition handlePosition) {
    if (r.getWidth() < 0 && r.getHeight() < 0) {
      if (handlePosition == HandlePosition.NW) {
        ...
        if (handlePosition == HandlePosition.NE) {
          return HandlePosition.SW;
        } else {
          if (handlePosition == HandlePosition.SE) {
            return HandlePosition.NW;
          } else {
          ...
          if (handlePosition == HandlePosition.NE) {
            ...
            if (handlePosition == HandlePosition.SE) {
              ...
              if (handlePosition == HandlePosition.SW) {
                ...
                if (handlePosition == HandlePosition.E) {
                  return HandlePosition.W;
                } else {
                  if (handlePosition == HandlePosition.W) {
                    return HandlePosition.E;
                  }
                }
              }
            ...
            if (handlePosition == HandlePosition.SE) {
              ...
              if (handlePosition == HandlePosition.NW) {
                ...
                if (handlePosition == HandlePosition.NE) {
                  ...
                  if (handlePosition == HandlePosition.S) {
                    return HandlePosition.N;
                  } else {
                    if (handlePosition == HandlePosition.N) {
                      return HandlePosition.S;
                    }
                  }
                }
        ...
        if (selectionMode == SelectionMode.Resizing) {
          ...
          if (selectionMode == null && selection != null) {
            ...
            if (handlePosition != null) {
              ...
              if (selectionScaled.contains(x, y)) {
                setCursor(Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR));
              } else {
  ...
  private void fireActionEvent(MouseEvent e) {
    ...
    CameraViewActionEvent action = new CameraViewActionEvent(CameraView.this, e.getX(), e.getY(), e.getX() * scaledUnitsPerPixelX, e.getY() * scaledUnitsPerPixelY, location);
  ...
  private void beginSelection(MouseEvent e) {
    if (selectionMode == null) {
      ...
      if (handlePosition != null) {
        ...
        if (selection != null && selectionScaled.contains(x, y)) {
  ...
  private void continueSelection(MouseEvent e) {
    ...
    if (selectionMode == SelectionMode.Resizing) {
      ...
      if (selectionActiveHandle == HandlePosition.NW) {
        ...
        if (selectionActiveHandle == HandlePosition.NE) {
          setScaledSelection(rx, y, x - rx, (rh - (y - ry)));
        } else {
          if (selectionActiveHandle == HandlePosition.N) {
            setScaledSelection(rx, y, rw, (rh - (y - ry)));
          } else {
            if (selectionActiveHandle == HandlePosition.E) {
              setScaledSelection(rx, ry, rw + (x - (rx + rw)), rh);
            } else {
              if (selectionActiveHandle == HandlePosition.SE) {
                setScaledSelection(rx, ry, rw + (x - (rx + rw)), rh + (y - (ry + rh)));
              } else {
                if (selectionActiveHandle == HandlePosition.S) {
                  setScaledSelection(rx, ry, rw, rh + (y - (ry + rh)));
                } else {
                  if (selectionActiveHandle == HandlePosition.SW) {
                    setScaledSelection(x, ry, (rw - (x - rx)), rh + (y - (ry + rh)));
                  } else {
                    if (selectionActiveHandle == HandlePosition.W) {
      ...
      if (selectionMode == SelectionMode.Moving) {
        setScaledSelection(x - selectionStartX, y - selectionStartY, selectionScaled.width, selectionScaled.height);
      } else {
        if (selectionMode == SelectionMode.Creating) {
  ...
  public CameraViewSelectionTextDelegate pixelsAndUnitsTextSelectionDelegate = new CameraViewSelectionTextDelegate() {
    @Override public String getSelectionText(CameraView cameraView) {
      ...
      String text = String.format(Locale.US, "%dpx, %dpx\n%2.3f%s, %2.3f%s", (int) selection.getWidth(), (int) selection.getHeight(), widthInUnits, camera.getUnitsPerPixel().getUnits().getShortName(), heightInUnits, camera.getUnitsPerPixel().getUnits().getShortName());
      return text;
    }
  };
}