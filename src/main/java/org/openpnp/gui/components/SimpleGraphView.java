...
@SuppressWarnings(value = { "serial" }) public class SimpleGraphView extends JComponent implements MouseMotionListener, MouseListener {
  ...
  @Override public synchronized void paintComponent(Graphics g) {
    ...
    int fontLineHeight = dfm.getAscent() + 1;
        ...
        if (min != null && max != null && (max.y - min.y) > 0.0 && (max.x - min.x) > 0.0) {
          ...
          if (dataScale.getColor() != null) {
            ...
            if (yMouse != null) {
              ...
              drawYIndicator(g2d, dfm, fontAscent, w, min, max, yOrigin, yScale, yUnit, y, dataScale.getColor());
            } else {
              double maxWidth = 0;
              ...
              if (xAxis != null) {
                ...
                if (size >= 2) {
                  ...
                  for (i = 1; i < size - 1; i++) {
                    ...
                    if (showMarkerOnly || cosine < 0.99 || Math.abs(xfPlot[i] - xPlot[s - 1]) > 12 || Math.abs(yfPlot[i] - yPlot[s - 1]) > 1.5) {
                  ...
                  if (selectedX != null) {
                    drawYIndicator(g2d, dfm, fontAscent, w, min, max, yOrigin, yScale, yUnit, dataRow.getInterpolated(selectedX), dataRow.getColor());
                  }
                }
              }
            }
  ...
  protected void drawYIndicator(Graphics2D g2d, FontMetrics dfm, int fontAscent, int w, Point2D.Double min, Point2D.Double max, double yOrigin, double yScale, double yUnit, Double y, Color color) {
    if (y == null) {
      return;
    }
    if (y < min.y) {
