...
public abstract class CalibrateCameraProcess {
  ...
  public CalibrateCameraProcess(MainFrame mainFrame, CameraView cameraView, List<Location> calibrationLocations, ArrayList<Integer> detectionDiameters, boolean automatic) throws Exception {
    ...
    if (detectionDiameters.size() != calibrationLocations.size()) {
      throw new Exception("Number of detection diameters does not equal the number of " + "calibration heights.");
    }
  ...
  /**
     ...
     * @throws Exception 
     */
  protected abstract void processRawCalibrationData(double[][][] testPattern3dPoints, double[][][] testPatternImagePoints, Size size, double mirrored, double apparentMotionDirection) throws Exception;
  ...
  private boolean initialAction() {
    ...
    testPatternZ = calibrationLocations.get(calibrationHeightIndex).convertToUnits(LengthUnit.Millimeters).getZ();
    if (isHeadMountedCamera) {
      movable = camera;
      maskDiameter = initialMaskDiameter;
      resultImage = null;
      showCircle(new org.opencv.core.Point(imageCenterPoint.getX(), imageCenterPoint.getY()), (int) (props.centeringDiameterFraction * maskDiameter / 2), Color.GREEN);
      ...
      setInstructionsAndProceedAction("Using the jog controls on the Machine Controls panel, jog the camera so that the calibration fiducial at Z = %s is approximately centered in the green circle.  Click Next when ready to proceed.", () -> requestOperatorToAdjustDiameterAction(), calibrationLocations.get(calibrationHeightIndex).getLengthZ().toString());
    } else {
      ...
      setInstructionsAndProceedAction("Select a nozzle and load it with the smallest available nozzle tip. Click Next when ready to proceed.", () -> requestOperatorToCenterNozzleTipAction());
    }
    return true;
  }
  ...
  private boolean requestOperatorToAdjustDiameterAction() {
    ...
    pipeline.setProperty("DetectCircularSymmetry.center", new org.openpnp.model.Point(expectedPoint.getX(), expectedPoint.getY()));
    if (detectionDiameters.get(calibrationHeightIndex) != null) {
      ...
      if (isHeadMountedCamera) {
        ...
        try {
          diameter = ((ReferenceNozzleTip) nozzle.getNozzleTip()).getCalibration().getCalibrationTipDiameter().divide(camera.getUnitsPerPixel(calibrationLocations.get(calibrationHeightIndex).getLengthZ()).getLengthX());
        } catch (Exception e) {
        }
    ...
    maskDiameter = 2 * (int) Math.min(expectedPoint.getX(), Math.min(pixelsX - expectedPoint.getX(), Math.min(expectedPoint.getY(), Math.min(pixelsY - expectedPoint.getY(), Math.max(maskDiameter / 2, advCal.getFiducialDiameter())))));
    swingWorker = new SwingWorker<Void, String>() {
      @Override protected Void doInBackground() throws Exception {
        while (!isCancelled()) {
          ...
          pipeline.setProperty("DetectCircularSymmetry.diameter", advCal.getFiducialDiameter());
          List<KeyPoint> keyPoints = null;
          try {
            pipeline.process();
            keyPoints = pipeline.getExpectedResult(VisionUtils.PIPELINE_RESULTS_NAME).getExpectedListModel(KeyPoint.class, new Exception("Calibration rig fiducial not found."));
          ...
          if (!isCancelled()) {
            if (keyPoints != null) {
              ...
              showCircle(new org.opencv.core.Point((pixelsX - 1) / 2.0, (pixelsY - 1) / 2.0), advCal.getFiducialDiameter() / 2, circleColor);
            }
          }
        }
        return null;
    ...
    setInstructionsAndProceedAction("Use the mouse scroll wheel to zoom in on the fiducial/nozzle tip and then adjust the Detection Diameter spinner until the red circle turns green with a + at its center and is sized to just fit the fiducial/nozzle tip with the + centered on the fiducial/nozzle tip. When ready, click Next to begin the automated calibration collection sequence.", () -> fiducialDiameterIsSetAction());
    swingWorker.execute();
    return true;
  }
  ...
  private boolean estimateUnitsPerPixelAction() {
    ...
    centralLocation = movable.getLocation().convertToUnits(LengthUnit.Millimeters).derive(null, null, movableZ, 0.0);
    ...
    swingWorker = new SwingWorker<Void, String>() {
      @Override protected Void doInBackground() throws Exception {
        ...
        try {
          if (movable instanceof Nozzle && ((Nozzle) movable).getRotationMode() == RotationMode.LimitedArticulation && angleIncrement < 360) {
        ...
        advCal.updateApproximateCameraF(cameraWalker.getEstimatedMillimetersPerPixel(), testPatternZ);
      ...
      @Override protected void process(List<String> chunksOfStatus) {
    ...
    swingWorker = new SwingWorker<Void, String>() {
      ...
      @Override protected Void doInBackground() throws Exception {
        ...
        for (int iLine = 0; iLine < numberOfLines; iLine++) {
          publish(String.format("Initializing radial line %d of %d at calibration Z coordinate %d of %d", iLine + 1, numberOfLines, calibrationHeightIndex + 1, numberOfCalibrationHeights));
          randomIdx.add(iLine);
          double lineAngle = iLine * 2 * Math.PI / numberOfLines;
          double[] unitVector = new double[] { Math.cos(lineAngle), Math.sin(lineAngle) };
          double scaling = advCal.getTestPatternFillFraction() * Math.min((pixelsX - detectionDiameters.get(calibrationHeightIndex)) / (2 * Math.abs(unitVector[0])), (pixelsY - detectionDiameters.get(calibrationHeightIndex)) / (2 * Math.abs(unitVector[1])));
          Location startLocation = centralLocation.derive(null, null, movable.getLocation().getLengthZ().convertToUnits(LengthUnit.Millimeters).getValue(), movable.getLocation().getRotation()).subtract(new Location(LengthUnit.Millimeters, 5 * advCal.getTrialStep().multiply(unitVector[0]).convertToUnits(LengthUnit.Millimeters).getValue(), 5 * advCal.getTrialStep().multiply(unitVector[1]).convertToUnits(LengthUnit.Millimeters).getValue(), 0, 0));
          try {
            machine.execute(() -> {
          ...
          try {
            lineWalkers.get(iLine).walkToPoint(centralLocation.derive(null, null, null, movable.getLocation().convertToUnits(centralLocation.getUnits()).getRotation()), imageCenterPoint, new Point(scaling * unitVector[0] + pixelsX / 2, scaling * unitVector[1] + pixelsY / 2));
        ...
        while (!done) {
          ...
          for (int iLine = 0; iLine < numberOfLines; iLine++) {
            publish(String.format("Collecting calibration points along radial line %d of %d, radial step %d, at calibration Z coordinate %d of %d", iLine + 1, numberOfLines, loopCount + 1, calibrationHeightIndex + 1, numberOfCalibrationHeights));
            try {
      ...
      @Override protected void done() {
        ...
        if (calibrationHeightIndex >= numberOfCalibrationHeights) {
          mainFrame.showInstructions("Camera Calibration Instructions/Status", "<html><body>" + "Processing calibration data - this may take a few seconds." + "</body></html>", true, false, "Next", cancelActionListener, proceedActionListener);
          ...
          for (int tpIdx = 0; tpIdx < testPattern3dPointsList.size(); tpIdx++) {
            ...
            for (int ptIdx = 0; ptIdx < tp.size(); ptIdx++) {
              ...
              testPattern3dPointsArray[tpIdx][ptIdx] = new double[] { mirrored * apparentMotionDirection * pt[0], apparentMotionDirection * pt[1], calibrationLocations.get(tpIdx).convertToUnits(LengthUnit.Millimeters).getZ() };
            }
          }
          ...
          try {
            processRawCalibrationData(testPattern3dPointsArray, testPatternImagePointsArray, new Size(pixelsX, pixelsY), mirrored, apparentMotionDirection);
          } catch (Exception ex) {
            MessageBoxes.errorBox(MainFrame.get(), "Error", ex);
          } finally {
            cleanUpWhenDone();
  ...
  protected boolean repeatAction() {
    ...
    if (isHeadMountedCamera) {
      ...
      try {
        machine.execute(() -> {
          nozzle.moveTo(centralLocation.convertToUnits(LengthUnit.Millimeters).derive(null, null, movableZ, (double) angle));
          return null;
        });
      } catch (Exception ex) {
        MessageBoxes.errorBox(MainFrame.get(), "Error", ex);
  ...
  private boolean requestOperatorToCenterNozzleTipAction() {
    ...
    setInstructionsAndProceedAction("Using the jog controls on the Machine Controls panel, jog the nozzle tip so that it is approximately in the center of the green circle. When ready, click Next to lower/raise the nozzle tip to the calibration height.", () -> captureCentralLocationAction());
    return true;
  }
  ...
  private boolean requestOperatorToVerifyNozzleTipIsCenteredAction() {
    ...
    setInstructionsAndProceedAction("Using the jog controls on the Machine Controls panel, rotate the nozzle tip through 360 degrees and verify it stays within the green circle. If necessary, jog it in X and/or Y so that it remains within the circle when it is rotated. Click Next when ready.", () -> captureVerifiedCentralLocationAction());
    return true;
  }
  ...
  private Point findAveragedFiducialPoint(Point expectedPoint) {
    ...
    do {
      ...
      if (count < numberOfAngles) {
        ...
        final Location moveLocation = movable.getLocation().derive(null, null, null, movable.getLocation().getRotation() + angleIncrement);
        try {
          machine.execute(() -> {
  ...
  private Point findCalibrationRigFiducialPoint(Point expectedPoint) {
    ...
    maskDiameter = 2 * (int) Math.min(expectedPoint.getX(), Math.min(pixelsX - expectedPoint.getX(), Math.min(expectedPoint.getY(), Math.min(pixelsY - expectedPoint.getY(), maskDiameter / 2))));
    pipeline.setProperty("MaskCircle.diameter", maskDiameter);
    ...
    while ((keypoints == null) && (attempts < 3)) {
      ...
      if (keypoints != null) {
        ...
        if (enclosingDiameter > props.maskGrowthThresholdFactor * maskDiameter) {
          ...
          if (enclosingDiameter < props.maskShrinkThresholdfactor * maskDiameter) {
  ...
  private void displayStatus(String status, Object... statusArguments) {
    ...
    Logger.trace(str);
    mainFrame.showInstructions("Camera Calibration Instructions/Status", "<html><body>" + str + "</body></html>", true, false, "Next", cancelActionListener, proceedActionListener);
  }
  ...
  /**
     ...
     * instructions string
     */
  private void setInstructionsAndProceedAction(String instructions, Thrunnable proceedAction, Object... instructionArguments) {
    String str = String.format(instructions, instructionArguments);
    Logger.trace(str);
    mainFrame.showInstructions("Camera Calibration Instructions/Status", "<html><body>" + str + "</body></html>", true, true, "Next", cancelActionListener, proceedActionListener);
  ...
  public static class CameraCalibrationProcessProperties {
    ...
    public int defaultDetectionDiameter = 25;
