...
public class MachineControlsPanel extends JPanel {
  ...
  public MachineControlsPanel(Configuration configuration, JobPanel jobPanel) {
    setBorder(new TitledBorder(null, Translations.getString("MachineControls.Label"), TitledBorder.LEADING, TitledBorder.TOP, null, null));
  ...
  public void updateDros() {
    ...
    z = l.getZ();
    c = l.getRotation();
    MainFrame.get().getDroLabel().setText(String.format("X:%-9s Y:%-9s Z:%-9s C:%-9s", String.format(Locale.US, configuration.getLengthDisplayFormat(), x), String.format(Locale.US, configuration.getLengthDisplayFormat(), y), String.format(Locale.US, configuration.getLengthDisplayFormat(), z), String.format(Locale.US, configuration.getLengthDisplayFormat(), c)));
  }

  private void createUi() {
    setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));
    JXCollapsiblePane collapsePane = new JXCollapsiblePane();
    JButton collapseButton = new JButton(collapsePane.getActionMap().get(JXCollapsiblePane.TOGGLE_ACTION));
    collapseButton.setBorderPainted(false);
    collapseButton.setHideActionText(true);
    collapseButton.setText("");
    Action collapseAction = collapseButton.getAction();
    collapseAction.putValue(JXCollapsiblePane.COLLAPSE_ICON, UIManager.getIcon("Tree.expandedIcon"));
    collapseAction.putValue(JXCollapsiblePane.EXPAND_ICON, UIManager.getIcon("Tree.collapsedIcon"));
    jogControlsPanel = new JogControlsPanel(configuration, this);
    JPanel panel = new JPanel();
    add(panel);
    panel.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("default:grow") }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC }));
  ...
  @SuppressWarnings(value = { "serial" }) public Action startStopMachineAction = new AbstractAction(Translations.getString("MachineControls.Action.Stop"), Icons.powerOn) {
    {
  ...
  @SuppressWarnings(value = { "serial" }) public Action targetToolAction = new AbstractAction(null, Icons.centerTool) {
    @Override public void actionPerformed(ActionEvent arg0) {
      UiUtils.submitUiMachineTask(() -> {
  ...
  @SuppressWarnings(value = { "serial" }) public Action targetCameraAction = new AbstractAction(null, Icons.centerCamera) {
    @Override public void actionPerformed(ActionEvent arg0) {
      UiUtils.submitUiMachineTask(() -> {
        HeadMountable tool = getSelectedTool();
        Camera camera = tool.getHead().getDefaultCamera();
  ...
  private void updateStartStopButton(boolean enabled) {
    startStopMachineAction.putValue(Action.NAME, enabled ? Translations.getString("MachineControls.Action.Stop") : Translations.getString("MachineControls.Action.Start"));
    startStopMachineAction.putValue(Action.SMALL_ICON, enabled ? Icons.powerOff : Icons.powerOn);
  }
  ...
  private MachineListener machineListener = new MachineListener.Adapter() {
    ...
    @Override public void machineEnableFailed(Machine machine, String reason) {
      updateStartStopButton(machine.isEnabled());
    }
    ...
    @Override public void machineDisableFailed(Machine machine, String reason) {
      updateStartStopButton(machine.isEnabled());
    }
  };

  private ConfigurationListener configurationListener = new ConfigurationListener.Adapter() {
    @Override public void configurationComplete(Configuration configuration) {
