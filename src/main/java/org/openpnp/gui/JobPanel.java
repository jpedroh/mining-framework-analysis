...
@SuppressWarnings(value = { "serial" }) public class JobPanel extends JPanel {
  @SuppressWarnings(value = { "unused" }) private static final Logger logger = LoggerFactory.getLogger(JobPanel.class);
  ...
  public JobPanel(Configuration configuration, MainFrame frame, MachineControlsPanel machineControlsPanel) {
    ...
    boardLocationSelectionActionGroup = new ActionGroup(removeBoardAction, captureCameraBoardLocationAction, captureToolBoardLocationAction, newPlacementAction, moveCameraToBoardLocationAction, moveToolToBoardLocationAction, twoPointLocateBoardLocationAction, fiducialCheckAction);
    boardLocationSelectionActionGroup.setEnabled(false);
    placementSelectionActionGroup = new ActionGroup(removePlacementAction, editPlacementFeederAction);
    placementSelectionActionGroup.setEnabled(false);
    placementCaptureAndPositionActionGroup = new ActionGroup(captureCameraPlacementLocation, captureToolPlacementLocation, moveCameraToPlacementLocation, moveToolToPlacementLocation);
    ...
    placementsTable.setDefaultEditor(Type.class, new DefaultCellEditor(typesComboBox));
    ...
    placementsTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
      @Override public void valueChanged(ListSelectionEvent e) {
        ...
        placementCaptureAndPositionActionGroup.setEnabled(getSelectedPlacement() != null && getSelectedPlacement().getSide() == getSelectedBoardLocation().getSide());
    ...
    placementsTable.addMouseListener(new MouseAdapter() {
      public void mouseClicked(MouseEvent mouseEvent) {
        ...
        int row = placementsTable.rowAtPoint(new Point(mouseEvent.getX(), mouseEvent.getY()));
        int col = placementsTable.columnAtPoint(new Point(mouseEvent.getX(), mouseEvent.getY()));
    ...
    pnlPlacements.setBorder(new TitledBorder(null, "Placements", TitledBorder.LEADING, TitledBorder.TOP, null, null));
  ...
  public final Action 
<<<<<<< /usr/src/app/output/openpnp/openpnp/99835c7c58242d6b0cdfba3211ed2f60169fdda7/src/main/java/org/openpnp/gui/JobPanel.java/left.java
  fiducialCheckAction = new AbstractAction() {
    {
      putValue(SMALL_ICON, Icons.fiducialCheck);
      putValue(NAME, "Fiducial Check");
      putValue(SHORT_DESCRIPTION, "Perform a fiducial check for the board and update it\'s location and rotation.");
    }
...
=======
  editPlacementFeederAction = new AbstractAction() {
    {
...
>>>>>>> /usr/src/app/output/openpnp/openpnp/99835c7c58242d6b0cdfba3211ed2f60169fdda7/src/main/java/org/openpnp/gui/JobPanel.java/right.java
  ;
  ...
  public final Action newPlacementAction = new AbstractAction() {
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      if (Configuration.get().getParts().size() == 0) {
        MessageBoxes.errorBox(getTopLevelAncestor(), "Error", "There are currently no parts defined in the system. Please create at least one part before creating a placement.");
        return;
      }
      BoardLocation boardLocation = getSelectedBoardLocation();
  ...
  public final Action removePlacementAction = new AbstractAction() {
    {
      ...
      putValue(SHORT_DESCRIPTION, "Remove the currently selected placement.");
    }
  ...
  public final Action moveCameraToPlacementLocation = new AbstractAction() {
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      Location placementLocation = Utils2D.calculateBoardPlacementLocation(getSelectedBoardLocation().getLocation(), getSelectedBoardLocation().getSide(), getSelectedPlacement().getLocation());
      final Camera camera = MainFrame.cameraPanel.getSelectedCamera();
      if (camera.getHead() == null) {
        MessageBoxes.errorBox(getTopLevelAncestor(), "Move Error", "Camera is not movable.");
        return;
  ...
  public final Action captureCameraPlacementLocation = new AbstractAction() {
    {
      ...
      putValue(SHORT_DESCRIPTION, "Set the placement\'s location to the camera\'s current position.");
    }
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      ...
      Location placementLocation = Utils2D.calculateBoardPlacementLocation(getSelectedBoardLocation().getLocation(), getSelectedPlacement().getSide(), MainFrame.cameraPanel.getSelectedCameraLocation().invert(true, true, true, true));
  ...
  public final Action captureToolPlacementLocation = new AbstractAction() {
    {
      ...
      putValue(SHORT_DESCRIPTION, "Set the placement\'s location to the tool\'s current position.");
    }

    @Override public void actionPerformed(ActionEvent arg0) {
      Nozzle nozzle = MainFrame.machineControlsPanel.getSelectedNozzle();
      Location placementLocation = Utils2D.calculateBoardPlacementLocation(getSelectedBoardLocation().getLocation(), getSelectedPlacement().getSide(), nozzle.getLocation().invert(true, true, true, true));
      getSelectedPlacement().setLocation(placementLocation.invert(true, true, true, true));
      placementsTable.repaint();
    }
  };
  ...
  private final JobProcessorListener jobProcessorListener = new JobProcessorListener.Adapter() {
    @Override public void jobStateChanged(JobState state) {
      updateJobActions();
    }

    @Override public void jobLoaded(Job job) {
      placementsTableModel.setBoard(null);
    ...
    @Override public void jobEncounteredError(JobError error, String description) {
  ...
  private final JobProcessorDelegate jobProcessorDelegate = new JobProcessorDelegate() {
    @Override public PickRetryAction partPickFailed(BoardLocation board, Part part, Feeder feeder) {
  ...
  static class StatusRenderer extends DefaultTableCellRenderer {
    public void setValue(Object value) {
      ...
      if (status == Status.Ready) {
        ...
        if (status == Status.MissingFeeder) {
