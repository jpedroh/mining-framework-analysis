...
@SuppressWarnings(value = { "serial" }) public class PackageVisionPanel extends JPanel {
  ...
  public PackageVisionPanel(Package pkg) {
    ...
    propertiesPanel.setBorder(new TitledBorder(null, Translations.getString("PackageVisionPanel.SettingsPanel.Boprder.title"), TitledBorder.LEADING, TitledBorder.TOP, null));
    propertiesPanel.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("default:grow"), FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, RowSpec.decode("default:grow"), FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC }));
    ...
    JLabel lblBodyWidth = new JLabel(Translations.getString("PackageVisionPanel.SettingsPanel.BodyWidthLabel.text"));
    ...
    JLabel lblBodyHeight = new JLabel(Translations.getString("" + "PackageVisionPanel.SettingsPanel.BodyLengthLabel.text"));
    ...
    tablePanel.setBorder(new TitledBorder(null, Translations.getString("PackageVisionPanel.PadsPanel.Border.title"), TitledBorder.LEADING, TitledBorder.TOP, null, null));
  ...
  public final Action newAction = new AbstractAction() {
    {
      ...
      putValue(SHORT_DESCRIPTION, Translations.getString("PackageVisionPanel.PadsPanel.Action.NewPad.Description"));
    }
  ...
  public final Action deleteAction = new AbstractAction() {
    {
      ...
      putValue(SHORT_DESCRIPTION, Translations.getString("PackageVisionPanel.PadsPanel.Action.DeletePad.Description"));
    }

    @Override public void actionPerformed(ActionEvent arg0) {
      int ret = JOptionPane.showConfirmDialog(getTopLevelAncestor(), Translations.getString("DialogMessages.ConfirmDelete.text") + " " + getSelectedPad().getName() + "?", Translations.getString("DialogMessages.ConfirmDelete.title") + " " + getSelectedPad().getName() + "?", JOptionPane.YES_NO_OPTION);
    ...
    UiUtils.messageBoxOnException(() -> {
      try {
        if (!footprint.getPads().isEmpty()) {
          int ret = JOptionPane.showConfirmDialog(getTopLevelAncestor(), "Are you sure you want to delete all existing pads?", "Delete all pads?", JOptionPane.YES_NO_OPTION);
          if (ret != JOptionPane.YES_OPTION) {
            return;
          }
