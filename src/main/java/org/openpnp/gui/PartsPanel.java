...
@SuppressWarnings(value = { "serial" }) public class PartsPanel extends JPanel implements WizardContainer {
  ...
  public PartsPanel(Configuration configuration, Frame frame) {
    ...
    table.setDefaultEditor(org.openpnp.model.Package.class, new DefaultCellEditor(packagesCombo));
    table.setDefaultRenderer(org.openpnp.model.Package.class, new IdentifiableTableCellRenderer<org.openpnp.model.Package>());
    ...
    JComboBox<FiducialVisionSettings> fiducialVisionCombo = new JComboBox<>(new VisionSettingsComboBoxModel(FiducialVisionSettings.class));
  ...
  public final Action newPartAction = new AbstractAction() {
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      if (Configuration.get().getPackages().size() == 0) {
        MessageBoxes.errorBox(getTopLevelAncestor(), "Error", "There are currently no packages defined in the system. Please create at least one package before creating a part.");
        return;
      }
      String id;
  ...
  public final Action deletePartAction = new AbstractAction() {
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      ...
      int ret = JOptionPane.showConfirmDialog(getTopLevelAncestor(), Translations.getString("DialogMessages.ConfirmDelete.text") + " " + formattedIds + "?", Translations.getString("DialogMessages.ConfirmDelete.title") + " " + selections.size() + " " + Translations.getString("CommonWords.parts") + "?", JOptionPane.YES_NO_OPTION);
  ...
  public final Action pastePartToClipboardAction = new AbstractAction() {
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      String id;
      while ((id = JOptionPane.showInputDialog(frame, "Please enter an ID for the pasted part.")) != null) {
        if (configuration.getPart(id) == null) {
          break;
        }
        MessageBoxes.errorBox(frame, "Error", "Part ID " + id + " already exists.");
  ...
  public void firePartSelectionChanged() {
    ...
    if (selectedPart != null) {
      tabbedPane.add(Translations.getString("PartsPanel.SettingsTab.title"), new JScrollPane(new PartSettingsPanel(selectedPart)));
      ...
      if (mainFrame.getTabs().getSelectedComponent() == mainFrame.getPartsTab() && Configuration.get().getTablesLinked() == TablesLinked.Linked) {
