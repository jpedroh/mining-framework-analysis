...
@SuppressWarnings(value = { "serial" }) public class PackagesPanel extends JPanel implements WizardContainer {
  ...
  public PackagesPanel(Configuration configuration, Frame frame) {
    ...
    JLabel lblSearch = new JLabel(Translations.getString("PackagesPanel.SearchLabel.text"));
    ...
    JComboBox<BottomVisionSettings> bottomVisionCombo = new JComboBox<>(new VisionSettingsComboBoxModel(BottomVisionSettings.class));
    bottomVisionCombo.setMaximumRowCount(20);
    bottomVisionCombo.setRenderer(new NamedListCellRenderer<>());
    table.setDefaultEditor(BottomVisionSettings.class, new DefaultCellEditor(bottomVisionCombo));
    JComboBox<BottomVisionSettings> fiducialVisionCombo = new JComboBox<>(new VisionSettingsComboBoxModel(FiducialVisionSettings.class));
    fiducialVisionCombo.setMaximumRowCount(20);
    fiducialVisionCombo.setRenderer(new NamedListCellRenderer<>());
    table.setDefaultEditor(FiducialVisionSettings.class, new DefaultCellEditor(fiducialVisionCombo));
    table.setDefaultRenderer(AbstractVisionSettings.class, new NamedTableCellRenderer<AbstractVisionSettings>());
  ...
  public final Action newPackageAction = new AbstractAction() {
    {
      ...
      putValue(NAME, Translations.getString("PackagesPanel.Action.NewPackage"));
      putValue(SHORT_DESCRIPTION, Translations.getString("PackagesPanel.Action.NewPackage.Description"));
    }
  ...
  public final Action deletePackageAction = new AbstractAction() {
    {
      ...
      putValue(NAME, Translations.getString("PackagesPanel.Action.DeletePackage"));
      putValue(SHORT_DESCRIPTION, Translations.getString("PackagesPanel.Action.DeletePackage.Description"));
    }
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      ...
      for (Package pkg : selections) {
        for (Part part : Configuration.get().getParts()) {
          if (part.getPackage() == pkg) {
            MessageBoxes.errorBox(getTopLevelAncestor(), Translations.getString("CommonWords.error"), pkg.getId() + " " + Translations.getString("CommonPhrases.cannotBeDeletedUsedBy") + " " + part.getId());
            return;
          }
        }
      }
      ...
      int ret = JOptionPane.showConfirmDialog(getTopLevelAncestor(), Translations.getString("DialogMessages.ConfirmDelete.text") + " " + formattedIds + "?", Translations.getString("DialogMessages.ConfirmDelete.title") + selections.size() + " " + Translations.getString("CommonWords.packages") + "?", JOptionPane.YES_NO_OPTION);
  ...
  public final Action copyPackageToClipboardAction = new AbstractAction() {
    {
      ...
      putValue(NAME, Translations.getString("PackagesPanel.Action.CopyPackage"));
      putValue(SHORT_DESCRIPTION, Translations.getString("PackagesPanel.Action.CopyPackage.Description"));
    }
  ...
  public final Action pastePackageToClipboardAction = new AbstractAction() {
    {
      ...
      putValue(NAME, Translations.getString("PackagesPanel.Action.PastePackage"));
      putValue(SHORT_DESCRIPTION, Translations.getString("PackagesPanel.Action.PastePackage.Description"));
    }

    @Override public void actionPerformed(ActionEvent arg0) {
      String id;
      while ((id = JOptionPane.showInputDialog(frame, "Please enter an ID for the pasted package.")) != null) {
        if (configuration.getPackage(id) == null) {
          break;
        }
        MessageBoxes.errorBox(frame, "Error", "Package ID " + id + " already exists.");
  ...
  public void firePackageSelectionChanged() {
    ...
    if (selectedPackage != null) {
      tabbedPane.add(Translations.getString("PackagesPanel.NozzleTipsTab.title"), new PackageNozzleTipsPanel(selectedPackage));
      tabbedPane.add("Settings", new JScrollPane(new PackageSettingsPanel(selectedPackage)));
      tabbedPane.add(
<<<<<<< /usr/src/app/output/openpnp/openpnp/f9a8fe8740a0c227349eb465f22314b826ed7d2f/src/main/java/org/openpnp/gui/PackagesPanel.java/left.java
      "Footprint"
=======
      Translations.getString("PackagesPanel.VisionTab.title")
>>>>>>> /usr/src/app/output/openpnp/openpnp/f9a8fe8740a0c227349eb465f22314b826ed7d2f/src/main/java/org/openpnp/gui/PackagesPanel.java/right.java
      , new JScrollPane(new PackageVisionPanel(selectedPackage)));
      tabbedPane.add(
<<<<<<< /usr/src/app/output/openpnp/openpnp/f9a8fe8740a0c227349eb465f22314b826ed7d2f/src/main/java/org/openpnp/gui/PackagesPanel.java/left.java
      "Vision Compositing"
=======
      Translations.getString("PackagesPanel.SettingsTab.title")
>>>>>>> /usr/src/app/output/openpnp/openpnp/f9a8fe8740a0c227349eb465f22314b826ed7d2f/src/main/java/org/openpnp/gui/PackagesPanel.java/right.java
      ...
      if (selectedTab != -1 && tabbedPane.getTabCount() > selectedTab) {
        tabbedPane.setSelectedIndex(selectedTab);
      }
      MainFrame mainFrame = MainFrame.get();
      if (mainFrame.getTabs().getSelectedComponent() == mainFrame.getPackagesTab() && Configuration.get().getTablesLinked() == TablesLinked.Linked) {
