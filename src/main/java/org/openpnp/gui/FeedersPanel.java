  ...
  protected static Nozzle getCompatibleNozzleAndTip(Feeder feeder, boolean allowNozzleTipChange) throws Exception {
    ...
    if (nozzle.getNozzleTip() == null || !packag.getCompatibleNozzleTips().contains(nozzle.getNozzleTip())) {
      ...
      if (nozzle.getNozzleTip() == null) {
        throw new Exception("No nozzle tip loaded on nozzle " + nozzle.getName() + ". " + "You may want to enable automatic nozzle tip change on manual pick on the Nozzle / Tool Changer.");
      } else {
        if (!resolved) {
          throw new Exception("Loaded nozzle tip " + nozzle.getNozzleTip().getName() + " is not compatible with package " + packag.getId() + ". " + (allowNozzleTipChange ? "You may want to enable automatic nozzle tip change on manual pick on the Nozzle / Tool Changer." : ""));
        }
      }
    }
    return nozzle;
