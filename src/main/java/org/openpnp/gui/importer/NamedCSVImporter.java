...
@SuppressWarnings(value = { "serial" }) public class NamedCSVImporter implements BoardImporter {
  ...
  private static final String Xs[] = { "X", "x", "X (mm)", "x (mm)", "Ref X", "ref x", "PosX", "Ref-X(mm)", "Ref-X(mil)" };
  ...
  static private int units_mils_x = 0, units_mils_y = 0, units_mils_height = 0;
  ...
  private static boolean checkCSV(String[] str) {
    if ((Ref = checkCSV(Refs, str)) != -1 && (Val = checkCSV(Vals, str)) != -1 && (Pack = checkCSV(Packs, str)) != -1 && (X = checkCSV(Xs, str)) != -1 && (Y = checkCSV(Ys, str)) != -1 && (Rot = checkCSV(Rots, str)) != -1) {
      HT = checkCSV(Heights, str);
      TB = checkCSV(TBs, str);
      Len = Ref <= Len ? Len : Ref;
      Len = Val <= Len ? Len : Val;
  ...
  private static boolean checkLine(String str) throws Exception {
    ...
    if (str.indexOf("val") == -1 && str.indexOf("Val") == -1 && str.indexOf("Comment") == -1 && str.indexOf("comment") == -1) {
      return false;
    }
    if (str.indexOf("ootprint") == -1 && str.indexOf("ackage") == -1 && str.indexOf("attern") == -1) {
      return false;
    }
    if (str.indexOf("Designator") == -1 && str.indexOf("designator") == -1 && str.indexOf("Part") == -1 && str.indexOf("part") == -1 && str.indexOf("Component") == -1 && str.indexOf("component") == -1 && str.indexOf("RefDes") == -1 && str.indexOf("Ref") == -1) {
      return false;
    }
    String as[], at[][];
  ...
  private static List<Placement> parseFile(File file, boolean createMissingParts, boolean updateHeights) throws Exception {
    ...
    for (String as[]; (as = csvParser.getLine()) != null; ) {
      if (as.length <= Len) {
        ...
        double placementX = Double.parseDouble(as[X].replace(",", ".").replace(" ", "").replace("mm", "").replace("mil", ""));
        if (units_mils_x == 1) {
          placementX = placementX * 0.0254;
        }
        double placementY = Double.parseDouble(as[Y].replace(",", ".").replace(" ", "").replace("mm", "").replace("mil", ""));
        if (units_mils_y == 1) {
          placementY = placementY * 0.0254;
        }
        double heightZ = 0.0;
        if (HT != -1) {
          heightZ = Double.parseDouble(as[HT].replace(",", ".").replace(" ", "").replace("mm", "").replace("mil", ""));
  ...
  class Dlg extends JDialog {
    ...
    public Dlg(Frame parent) {
      ...
      panel_1.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, RowSpec.decode("default:grow") }));
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        fileDialog.setFilenameFilter(new FilenameFilter() {
          @Override public boolean accept(File dir, String name) {
            return false || name.toLowerCase().endsWith(".csv") || name.toLowerCase().endsWith(".txt") || name.toLowerCase().endsWith(".dat");
          }
        });
    ...
    private class SwingAction_2 extends AbstractAction {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          if (topFile.exists()) {
            placements.addAll(parseFile(topFile, chckbxCreateMissingParts.isSelected(), chckbxUpdatePartHeight.isSelected()));
          }
        } catch (Exception e1) {
