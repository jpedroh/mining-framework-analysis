...
@SuppressWarnings(value = { "serial" }) public class NamedCSVImporter implements BoardImporter {
  ...
  private static boolean checkCSV(String[] str) {
    ...
    if ((Ref = checkCSV(Refs, str)) != -1 && (Val = checkCSV(Vals, str)) != -1 && (Pack = checkCSV(Packs, str)) != -1 && (X = checkCSV(Xs, str)) != -1 && (Y = checkCSV(Ys, str)) != -1 && (Rot = checkCSV(Rots, str)) != -1 && (TB = checkCSV(TBs, str)) != -2) {
      Len = Ref <= Len ? Len : Ref;
      Len = Val <= Len ? Len : Val;
  ...
  class Dlg extends JDialog {
    ...
    public Dlg(Frame parent) {
      ...
      panel_1.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC }));
