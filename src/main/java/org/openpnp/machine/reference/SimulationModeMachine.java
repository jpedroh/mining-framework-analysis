...
public class SimulationModeMachine extends ReferenceMachine {
  ...
  @Attribute(required = false) private SimulationMode simulationMode = SimulationMode.Off;

  /**
     * The simulated non-squareness is applied to what the simulated cameras see.
     * Works on the ImageCamera.
     */
  @Attribute(required = false) private double simulatedNonSquarenessFactor = 0.0;

  /**
     * Simulated runout on nozzle tips (currently all noozle tips get the same).
     * Works on the SimulatedUpCamera.
     */
  @Element(required = false) private Length simulatedRunout = new Length(0, LengthUnit.Millimeters);

  @Attribute(required = false) private double simulatedRunoutPhase = 30;

  /**
     * Simulated camera noise (number of sparks per frame) to test camera settle. 
     * Works on ImageCamera and SimulatedUpCamera.
     */
  @Attribute(required = false) private int simulatedCameraNoise = 0;

  /**
     * Simulated camera lag [s] to test camera settle. 
     * Works on ImageCamera and SimulatedUpCamera.
     */
  @Attribute(required = false) private double simulatedCameraLag = 0;

  /**
     * Simulated vibration to test camera settle. Initial max. amplitude.
     */
  @Attribute(required = false) private double simulatedVibrationAmplitude = 0;

  /**
     * Simulated vibration to test camera settle. Duration in seconds to ~1%.
     */
  @Attribute(required = false) private double simulatedVibrationDuration = 0.2;

  /**
     * Simulated homing error. Introduces an initial location error that Visual Homing needs to correct.
     * Works on ImageCamera.
     */
  @Element(required = false) private Location homingError = new Location(LengthUnit.Millimeters);

  /**
     * Checks Picks/Places by visually locating the tape pocket/solder lands in the ImageCamera.
     * Throws errors instead.
     */
  @Element(required = false) private boolean pickAndPlaceChecking = false;
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return Collect.concat(super.getPropertySheets(), new PropertySheet[] { new PropertySheetWizardAdapter(new SimulationModeMachineConfigurationWizard(this), "Simulation Mode") });
  }
  ...
  public void setMachineTableZ(Length machineTableZ) {
    for (Feeder feeder : getFeeders()) {
      if (feeder instanceof ReferenceFeeder) {
        ((ReferenceFeeder) feeder).setLocation(((ReferenceFeeder) feeder).getLocation().derive(null, null, machineTableZ.convertToUnits(((ReferenceFeeder) feeder).getLocation().getUnits()).getValue(), null));
      }
      if (feeder instanceof ReferenceStripFeeder) {
        ((ReferenceStripFeeder) feeder).setReferenceHoleLocation(((ReferenceStripFeeder) feeder).getReferenceHoleLocation().derive(null, null, machineTableZ.convertToUnits(((ReferenceStripFeeder) feeder).getReferenceHoleLocation().getUnits()).getValue(), null));
        ((ReferenceStripFeeder) feeder).setLastHoleLocation(((ReferenceStripFeeder) feeder).getLastHoleLocation().derive(null, null, machineTableZ.convertToUnits(((ReferenceStripFeeder) feeder).getLastHoleLocation().getUnits()).getValue(), null));
      }
    }
    for (BoardLocation boardLocation : MainFrame.get().getJobTab().getJob().getBoardLocations()) {
      boardLocation.setLocation(boardLocation.getLocation().derive(null, null, machineTableZ.convertToUnits(boardLocation.getLocation().getUnits()).getValue(), null));
    }
    for (Camera camera : getCameras()) {
      if (camera instanceof ReferenceCamera) {
        ((ReferenceCamera) camera).setHeadOffsets(((ReferenceCamera) camera).getHeadOffsets().derive(null, null, machineTableZ.convertToUnits(((ReferenceCamera) camera).getHeadOffsets().getUnits()).getValue(), null));
      }
    }
  }
        ...
        if (actuator.getHead() != null) {
          ...
          if (camera instanceof ImageCamera) {
            for (Nozzle nozzle : actuator.getHead().getNozzles()) {
              if (nozzle instanceof ReferenceNozzle && ((ReferenceNozzle) nozzle).getVacuumActuator() == actuator) {
                  ...
                  if (location.getLinearDistanceTo(machine.getDiscardLocation()) > 4.0) {
                    if ((Boolean) value == true) {
                      if (!((ImageCamera) camera).isPickLocation(location, nozzle)) {
                        throw new Exception("Nozzle " + nozzle.getName() + " part " + nozzle.getPart().getId() + " pick location not recognized.");
                      }
                    } else {
                      if (!((ImageCamera) camera).isPlaceLocation(location, nozzle)) {
                        throw new Exception("Nozzle " + nozzle.getName() + " part " + nozzle.getPart().getId() + " place location not recognized.");
                      }
                    }
                  }
                }
  ...
  public static Location getSimulatedPhysicalLocation(HeadMountable hm, Looking looking) {
    ...
    if (machine == null || machine.getSimulationMode() == SimulationMode.Off) {
      if (Configuration.get().getMachine() instanceof ReferenceMachine) {
        ...
        Motion momentary = ((ReferenceMachine) Configuration.get().getMachine()).getMotionPlanner().getMomentaryMotion(cameraTime);
      ...
      try {
        double lag = 0;
        if (looking != null && machine.getSimulationMode().isDynamicallyImperfectMachine()) {
        ...
        if (machine.getSimulationMode().isDynamicallyImperfectMachine()) {
          ...
          if (amplitude != 0) {
            double vibrationDuration = machine.getSimulatedVibrationDuration();
            double vibrationEigen = 13.313;
            ...
            if (xIntegral != 0 || yIntegral != 0) {
              axesLocation = axesLocation.add(new AxesLocation((a, b) -> (b), new AxesLocation(xAxis, -xIntegral), new AxesLocation(yAxis, -yIntegral)));
            }
          }
        }
        if (hm instanceof Nozzle && machine.getSimulationMode().isDynamicallyImperfectMachine()) {
          double runout = machine.getSimulatedRunout().convertToUnits(AxesLocation.getUnits()).getValue();
          double rotation = axesLocation.getCoordinate(mappedAxes.getAxis(Axis.Type.Rotation));
          Location runoutVector = new Location(AxesLocation.getUnits(), runout, 0, 0, 0).rotateXy(rotation - machine.getSimulatedRunoutPhase());
        ...
        if (machine.getSimulationMode().isImperfectMachine()) {
          ...
          axesLocation = axesLocation.add(new AxesLocation(mappedAxes.getAxis(Axis.Type.X), machine.getSimulatedNonSquarenessFactor() * y));
        }
        location = hm.toTransformed(axesLocation, LocationOption.SuppressStaticCompensation, LocationOption.SuppressDynamicCompensation);
        location = hm.toHeadMountableLocation(location, LocationOption.SuppressStaticCompensation, LocationOption.SuppressDynamicCompensation);
      } catch (Exception e) {
        Logger.error(e);
      }
    }
  ...
  public static AxesLocation getMomentaryVector(SimulationModeMachine machine, double cameraTime, BiFunction<Motion, Double, AxesLocation> function) throws Exception {
    Motion momentary = machine.getMotionPlanner().getMomentaryMotion(cameraTime);
    ...
    for (Driver driver : machine.getDrivers()) {
      if (driver instanceof GcodeDriver) {
        ...
        if (comms instanceof SimulatedCommunications) {
          GcodeServer server = ((SimulatedCommunications) comms).getGcodeServer();
          momentary = server.getMomentaryMotion(cameraTime);
  ...
  @Override public void fireMachineActuatorActivity(Actuator actuator) {
  ...
  public static void simulateCameraExposure(Camera camera, Graphics2D gFrame, int width, int height) {
    SimulationModeMachine machine = getSimulationModeMachine();
    if (machine != null && machine.getSimulationMode().isDynamicallyImperfectMachine()) {
