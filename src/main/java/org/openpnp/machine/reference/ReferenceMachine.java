...
public class ReferenceMachine extends AbstractMachine {
  @Deprecated @Element(required = false) private Driver driver = null;

  @Element(required = false) protected PnpJobProcessor pnpJobProcessor = new ReferencePnpJobProcessor();

  @Element(required = false) protected FiducialLocator fiducialLocator = new ReferenceFiducialLocator();

  @Element(required = false) protected MotionPlanner motionPlanner = new NullMotionPlanner();

  @Element(required = false) private boolean homeAfterEnabled = false;

  @Element(required = false) private boolean parkAfterHomed = false;

  @Attribute(required = false) private boolean autoToolSelect = true;

  @Attribute(required = false) private boolean safeZPark = true;

  @Element(required = false) private Length unsafeZRoamingDistance = new Length(10, LengthUnit.Millimeters);

  @Element(required = false) private boolean poolScriptingEngines = false;

  @Element(required = false) private Solutions solutions = new Solutions();

  @Deprecated @ElementList(required = false) Set<String> dismissedSolutions = null;
  ...
  @Override public MotionPlanner getMotionPlanner() {
  ...
  @Override public boolean isAutoToolSelect() {
  ...
  @Override public boolean isSafeZPark() {
    return safeZPark;
  }
  ...
  @Override public boolean isParkAfterHomed() {
  ...
  @Override public Length getUnsafeZRoamingDistance() {
  ...
  @Override public boolean isPoolScriptingEngines() {
  ...
  @Override public PropertySheetHolder[] getChildPropertySheetHolders() {
    ...
    children.add(new AxesPropertySheetHolder(this, Translations.getString("ReferenceMachine.PropertySheetHolder.Axes.title"), getAxes(), null));
    children.add(new SignalersPropertySheetHolder(this, Translations.getString("ReferenceMachine.PropertySheetHolder.Signalers.title"), getSignalers(), null));
    children.add(new SimplePropertySheetHolder(Translations.getString("ReferenceMachine.PropertySheetHolder.Feeders.title"), getFeeders()));
    children.add(new SimplePropertySheetHolder(Translations.getString("ReferenceMachine.PropertySheetHolder.Heads.title"), getHeads()));
    children.add(new NozzleTipsPropertySheetHolder(Translations.getString("ReferenceMachine.PropertySheetHolder.NozzleTips.title"), getNozzleTips(), null));
    children.add(new CamerasPropertySheetHolder(null, Translations.getString("ReferenceMachine.PropertySheetHolder.Cameras.title"), getCameras(), null));
    children.add(new ActuatorsPropertySheetHolder(null, Translations.getString("ReferenceMachine.PropertySheetHolder.Actuators.title"), getActuators(), null));
    children.add(new DriversPropertySheetHolder(this, Translations.getString("ReferenceMachine.PropertySheetHolder.Drivers.title"), getDrivers(), null));
    children.add(new SimplePropertySheetHolder(Translations.getString("ReferenceMachine.PropertySheetHolder.JobProcessors.title"), Arrays.asList(getPnpJobProcessor())));
    ...
    children.add(new SimplePropertySheetHolder(Translations.getString("ReferenceMachine.PropertySheetHolder.Vision.title"), vision));
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return Collect.concat(new PropertySheet[] { new PropertySheetWizardAdapter(getConfigurationWizard()) }, getMotionPlanner().getPropertySheets());
  }
  ...
  @Override public List<Class<? extends Axis>> getCompatibleAxisClasses() {
  ...
  @Override public List<Class<? extends Feeder>> getCompatibleFeederClasses() {
  ...
  @Override public List<Class<? extends Camera>> getCompatibleCameraClasses() {
  ...
  @Override public List<Class<? extends Nozzle>> getCompatibleNozzleClasses() {
  ...
  @Override public List<Class<? extends Actuator>> getCompatibleActuatorClasses() {
  ...
  @Override public List<Class<? extends Signaler>> getCompatibleSignalerClasses() {
  ...
  @Override public List<Class<? extends Driver>> getCompatibleDriverClasses() {
  ...
  @Override public List<Class<? extends MotionPlanner>> getCompatibleMotionPlannerClasses() {
  ...
  @Override public FiducialLocator getFiducialLocator() {
    return fiducialLocator;
  }

  @Override public PnpJobProcessor getPnpJobProcessor() {
  ...
  @Override public boolean isHomed() {
    return this.isHomed;
  }

  @Override public void setHomed(boolean isHomed) {
  ...
  @Element(required = false) private VisionSolutions visualSolutions = new VisionSolutions();

  public VisionSolutions getVisionSolutions() {
    return visualSolutions;
  }

  @Element(required = false) private CalibrationSolutions calibrationSolutions = new CalibrationSolutions();
  ...
  @Override public void findIssues(Solutions solutions) {
    ...
    if (solutions.isTargeting(Milestone.Advanced)) {
      if (getMotionPlanner() instanceof NullMotionPlanner) {
        solutions.add(new Solutions.Issue(this, "Advanced Motion Planner not set. Accept or Dismiss to continue.", "Change to ReferenceAdvancedMotionPlanner", Solutions.Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/Motion-Planner#choosing-a-motion-planner") {
      ...
      if (!(getMotionPlanner() instanceof NullMotionPlanner)) {
        solutions.add(new Solutions.Issue(this, "Advanced motion planner set. Revert to a simpler, safer planner.", "Change to NullMotionPlanner", Solutions.Severity.Information, "https://github.com/openpnp/openpnp/wiki/Motion-Planner#choosing-a-motion-planner") {
          final MotionPlanner oldMotionPlanner = ReferenceMachine.this.getMotionPlanner();

          @Override public boolean isUnhandled() {
            return false;
          }

          @Override public String getExtendedDescription() {
            return "<html><span color=\"red\">CAUTION:</span> This is a troubleshooting option, offered to remove the ReferenceAdvancedMotionPlanner " + "if it causes problems, or if you don\'t want it after all. Going back to the plain NullPlanner will lose you all the " + "advanced configuration.</html>";
          }
    ...
    if (solutions.isTargeting(Milestone.Basics) && !isAutoToolSelect()) {
      solutions.add(new Solutions.Issue(this, "OpenPnP can often automatically select the right tool for you in Machine Controls.", "Enable Auto tool select.", Solutions.Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration:-Machine-Setup#configuration") {
        @Override public void setState(Solutions.State state) throws Exception {
