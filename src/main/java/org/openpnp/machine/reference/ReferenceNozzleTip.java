...
public class ReferenceNozzleTip extends AbstractNozzleTip {
  ...
  @Attribute(required = false) private double placeBlowOffLevel;
  ...
  @Element(required = false) private Location touchLocation = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Length visionCalibrationZAdjust = new Length(0, LengthUnit.Millimeters);
  ...
  @Attribute(required = false) private VisionCalibration visionCalibration = VisionCalibration.None;
  ...
  @Attribute(required = false) VisionCalibrationTrigger visionCalibrationTrigger = VisionCalibrationTrigger.Manual;

  @Element(required = false) private Length visionTemplateDimensionX = new Length(10, LengthUnit.Millimeters);

  @Element(required = false) private Length visionTemplateDimensionY = new Length(10, LengthUnit.Millimeters);

  @Attribute(required = false) private double visionMatchMinimumScore = 0.2;

  private Double visionMatchLastScore = null;

  @Element(required = false) private Length visionTemplateTolerance = new Length(4, LengthUnit.Millimeters);

  @Attribute(required = false) private int visionCalibrationMaxPasses = 3;

  @Element(required = false) private Length visionCalibrationTolerance = new Length(0.7, LengthUnit.Millimeters);

  @Element(required = false) private TemplateImage visionTemplateImageEmpty = null;

  @Element(required = false) private TemplateImage visionTemplateImageOccupied = null;

  @Element(required = false) private Location visionCalibrationOffset;
  ...
  @Attribute(required = false) private ZCalibrationTrigger zCalibrationTrigger = ZCalibrationTrigger.Manual;

  @Attribute(required = false) private boolean zCalibrationFailHoming = true;

  @Attribute(required = false) private boolean templateNozzleTip = false;

  @Attribute(required = false) private boolean templateLocked = false;

  @Element(required = false) private Length minPartDiameter = new Length(0, LengthUnit.Millimeters);

  @Element(required = false) private Length maxPartDiameter = new Length(20, LengthUnit.Millimeters);

  @Element(required = false) private Length maxPartHeight = new Length(5, LengthUnit.Millimeters);

  @Element(required = false) protected Length maxPickTolerance = new Length(1, LengthUnit.Millimeters);
  ...
  @Element(required = false) VacuumMeasurementMethod methodPartOn = null;

  @Element(required = false) boolean partOnCheckAfterPick = true;

  @Element(required = false) boolean partOnCheckAlign = true;

  @Element(required = false) boolean partOnCheckBeforePlace = true;

  @Attribute(required = false) private boolean establishPartOnLevel;

  @Element(required = false) private double vacuumLevelPartOnLow;

  @Element(required = false) private double vacuumLevelPartOnHigh;

  @Element(required = false) private double vacuumDifferencePartOnLow;

  @Element(required = false) private double vacuumDifferencePartOnHigh;

  @Element(required = false) VacuumMeasurementMethod methodPartOff = null;

  @Attribute(required = false) private boolean establishPartOffLevel;

  @Element(required = false) boolean partOffCheckAfterPlace = true;

  @Element(required = false) boolean partOffCheckBeforePick = true;

  @Element(required = false) private double vacuumLevelPartOffLow;

  @Element(required = false) private double vacuumLevelPartOffHigh;

  @Attribute(required = false) private int partOffProbingMilliseconds;

  @Attribute(required = false) private int partOffDwellMilliseconds;

  @Element(required = false) private double vacuumDifferencePartOffLow;

  @Element(required = false) private double vacuumDifferencePartOffHigh;

  @Element(required = false) private Length diameterLow = new Length(0, LengthUnit.Millimeters);

  @Attribute(required = false) private boolean isPushAndDragAllowed = false;

  @Element(required = false) protected String changerActuatorPostStepOne;

  @Element(required = false) protected String changerActuatorPostStepTwo;

  @Element(required = false) protected String changerActuatorPostStepThree;
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(getConfigurationWizard()), new PropertySheetWizardAdapter(new ReferenceNozzleTipPartDetectionWizard(this), Translations.getString("ReferenceNozzleTip.PartDetection.tab.title")), new PropertySheetWizardAdapter(new ReferenceNozzleTipToolChangerWizard(this), Translations.getString("ReferenceNozzleTip.ToolChanger.tab.title")), new PropertySheetWizardAdapter(new ReferenceNozzleTipCalibrationWizard(this), Translations.getString("ReferenceNozzleTip.Calibration.tab.title")) };
  }

  @Override public Length getMaxPartHeight() {
  ...
  @Override public Length getMaxPartDiameter() {
  ...
  @Override public Length getMinPartDiameter() {
  ...
  @Override public Length getMaxPickTolerance() {
  ...
  @Override public ReferenceNozzle getNozzleWhereLoaded() {
  ...
  public void setTemplateNozzleTip(boolean templateNozzleTip) {
    ...
    if (templateNozzleTip && !oldValue) {
      for (NozzleTip nt : Configuration.get().getMachine().getNozzleTips()) {
        if (nt != this && nt instanceof ReferenceNozzleTip && ((ReferenceNozzleTip) nt).isTemplateNozzleTip()) {
  ...
  public void setVacuumLevelPartOnReading(Double vacuumLevelPartOnReading) {
    ...
    if (!(oldValue == null && vacuumLevelPartOnReading == null)) {
  ...
  public void setVacuumDifferencePartOnReading(Double vacuumDifferencePartOnReading) {
    ...
    if (!(oldValue == null && vacuumDifferencePartOnReading == null)) {
  ...
  public void setVacuumLevelPartOffReading(Double vacuumLevelPartOffReading) {
    ...
    if (!(oldValue == null && vacuumLevelPartOffReading == null)) {
  ...
  public void setVacuumDifferencePartOffReading(Double vacuumDifferencePartOffReading) {
    ...
    if (!(oldValue == null && vacuumDifferencePartOffReading == null)) {
  ...
  public void setVacuumPartOnGraph(SimpleGraph vacuumPartOnGraph) {
    ...
    if (!(oldValue == null && vacuumPartOnGraph == null)) {
      firePropertyChange("vacuumPartOnGraph", null, vacuumPartOnGraph);
    }
  }
  ...
  public void setVacuumPartOffGraph(SimpleGraph vacuumPartOffGraph) {
    ...
    if (!(oldValue == null && vacuumPartOffGraph == null)) {
      firePropertyChange("vacuumPartOffGraph", null, vacuumPartOffGraph);
    }
  }
  ...
  @Override public boolean isPushAndDragAllowed() {
  ...
  public static ReferenceNozzleTip getTemplateNozzleTip() throws Exception {
    for (NozzleTip nt : Configuration.get().getMachine().getNozzleTips()) {
      if (nt instanceof ReferenceNozzleTip && ((ReferenceNozzleTip) nt).isTemplateNozzleTip()) {
  ...
  public void assignNozzleTipChangerSettings(ReferenceNozzleTip templateNozzleTip, boolean changerLocations, boolean zCalibration, boolean visionCalibration) {
    if (getChangerStartLocation().getLinearDistanceTo(Location.origin) != 0 && !isTemplateLocked()) {
      if (changerLocations) {
        Location offsets = getChangerStartLocation().subtract(templateNozzleTip.getChangerStartLocation());
  ...
  public Action deleteAction = new AbstractAction("Delete Nozzle Tip") {
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      int ret = JOptionPane.showConfirmDialog(MainFrame.get(), Translations.getString("DialogMessages.ConfirmDelete.text") + " " + getName() + "?", Translations.getString("DialogMessages.ConfirmDelete.title") + " " + getName() + "?", JOptionPane.YES_NO_OPTION);
  ...
  public void ensureVisionCalibration(boolean nozzleTipChange) throws Exception {
    ...
    if (location != null) {
      location = location.add(new Location(getVisionCalibrationZAdjust().getUnits(), 0, 0, getVisionCalibrationZAdjust().getValue(), 0));
      if (visionCalibrationOffset == null || (getVisionCalibrationTrigger() == VisionCalibrationTrigger.NozzleTipChange && nozzleTipChange)) {
        ...
        int width = ((int) Math.ceil(getVisionTemplateDimensionX().add(visionTemplateTolerance).divide(upp.getLengthX()))) & ~1;
        int height = ((int) Math.ceil(getVisionTemplateDimensionY().add(visionTemplateTolerance).divide(upp.getLengthY()))) & ~1;
        ...
        try {
          ...
          for (int pass = 0; pass < visionCalibrationMaxPasses; ++pass) {
            ...
            cameraImage = ImageUtils.convertBufferedImage(cameraImage, templateEmpty.getType());
            ...
            Imgproc.matchTemplate(cameraCropMat, templateMatEmpty, resultEmptyMat, Imgproc.TM_CCOEFF_NORMED);
            ...
            Imgproc.matchTemplate(cameraCropMat, templateMatOccupied, resultOccupiedMat, Imgproc.TM_CCOEFF_NORMED);
            ...
            Logger.trace(String.format("Nozzle tip %s changer slot empty template at pixel offset %f, %f, score %f", getName(), emptyMatch.maxLoc.x, emptyMatch.maxLoc.y, emptyMatch.maxVal));
            Logger.trace(String.format("Nozzle tip %s changer slot occupied template at pixel offset %f, %f, score %f", getName(), occupiedMatch.maxLoc.x, occupiedMatch.maxLoc.y, occupiedMatch.maxVal));
            if (Math.max(emptyMatch.maxVal, occupiedMatch.maxVal) < visionMatchMinimumScore) {
              throw new Exception(String.format("Nozzle tip %s changer slot vision calibration failed. Score %f lower than minmum %f.", getName(), Math.max(emptyMatch.maxVal, occupiedMatch.maxVal), visionMatchMinimumScore));
            }
            boolean foundOccupied = (emptyMatch.maxVal < occupiedMatch.maxVal);
            if (foundOccupied != shouldBeOccupied) {
              throw new Exception(String.format("Nozzle tip %s changer slot was expected %s, but found %s.", getName(), shouldBeOccupied ? "occupied" : "empty", foundOccupied ? "occupied" : "empty"));
            }
            ...
            if (lastOffset.getLinearLengthTo(Location.origin).compareTo(visionCalibrationTolerance) < 0) {
              break;
            }
            if (location.getLinearLengthTo(nominalLocation).compareTo(visionTemplateTolerance) > 0) {
          ...
          Logger.trace(String.format("Nozzle tip %s changer slot calibration offset %s, %s, distance %s", getName(), visionCalibrationOffset.getLengthX(), visionCalibrationOffset.getLengthY(), visionCalibrationOffset.getLinearLengthTo(Location.origin)));
        }  finally {
