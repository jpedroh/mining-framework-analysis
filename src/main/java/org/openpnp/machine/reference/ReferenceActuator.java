...
public class ReferenceActuator extends AbstractActuator implements ReferenceHeadMountable {
  ...
  @Attribute(required = false) protected MachineStateActuation enabledActuation = MachineStateActuation.AssumeUnknown;

  @Attribute(required = false) protected MachineStateActuation homedActuation = MachineStateActuation.LeaveAsIs;

  @Attribute(required = false) protected MachineStateActuation disabledActuation = MachineStateActuation.LeaveAsIs;
    ...
    Configuration.get().addListener(new ConfigurationListener.Adapter() {
      @Override public void configurationLoaded(Configuration configuration) throws Exception {
        Configuration.get().getMachine().addListener(new MachineListener.Adapter() {
          ...
          @Override public void machineHomed(Machine machine, boolean isHomed) {
            if (isHomed) {
              actuateMachineState(machine, getHomedActuation(), true);
            }
          }

          @Override public void machineAboutToBeDisabled(Machine machine, String reason) {
  ...
  @Element(required = false) private ReferenceActuatorProfiles actuatorProfiles;
  ...
  @Override public Object getLastActuationValue() {
  ...
  @Override public void setValueType(ActuatorValueType valueType) {
  ...
  @Override public void actuate(String value) throws Exception {
    if (isCoordinatedBeforeActuate()) {
      coordinateWithMachine(false);
    }
    Logger.debug("{}.actuate({})", getName(), value);
  ...
  @Override public void actuateProfile(String name) throws Exception {
    if (getActuatorProfiles() != null) {
      setLastActuationValue(getActuatorProfiles().actuate(this, name));
    }
  }

  @Override public void actuateProfile(boolean on) throws Exception {
    if (getActuatorProfiles() != null) {
  ...
  @Override public <T extends java.lang.Object> String read(T parameter) throws Exception {
    if (isCoordinatedBeforeRead()) {
      coordinateWithMachine(false);
    }
