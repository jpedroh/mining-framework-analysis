...
public class ReferenceHead extends AbstractHead {
  ...
  public void moveTo(ReferenceHeadMountable hm, Location location, double speed) throws Exception {
    if (isSoftLimitsEnabled()) {
      ...
      if (cameraLocation.getX() < minLocation.getX() || cameraLocation.getX() > maxLocation.getX() || cameraLocation.getY() < minLocation.getY() || cameraLocation.getY() > maxLocation.getY()) {
        throw new Exception(String.format("Can\'t move %s to %s, outside of soft limits on head %s.", hm.getName(), location, getName()));
      }
    }
