...
public class ReferenceNozzle extends AbstractNozzle implements ReferenceHeadMountable {
  ...
  @Deprecated @Attribute(required = false) protected Boolean invertVacuumSenseLogic = null;
  ...
  @Override public void loadNozzleTip(NozzleTip nozzleTip) throws Exception {
    ...
    if (this.nozzleTip.getCalibration().isRecalibrateOnNozzleTipChangeNeeded()) {
      ...
      if (this.nozzleTip.getCalibration().isRecalibrateOnNozzleTipChangeInJobNeeded()) {
  ...
  @Override public Location getLocation() {
    ...
    ReferenceNozzleTip calibrationNozzleTip = getCalibrationNozzleTip();
    if (calibrationNozzleTip != null && calibrationNozzleTip.getCalibration().isCalibrated()) {
      Location offset = calibrationNozzleTip.getCalibration().getCalibratedOffset(location.getRotation());
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(getConfigurationWizard()), new PropertySheetWizardAdapter(new ReferenceNozzlePartDetectionWizard(this), "Part Detection"), new PropertySheetWizardAdapter(new ReferenceNozzleCameraOffsetWizard(this), "Offset Wizard") };
  }
