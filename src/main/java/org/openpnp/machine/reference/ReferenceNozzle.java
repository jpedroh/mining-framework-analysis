...
public class ReferenceNozzle extends AbstractNozzle implements ReferenceHeadMountable {
  ...
  @Element(required = false) protected String zSafeActuatorName = "";

  @Element(required = false) protected String zSafeActuatorValue = "";
  ...
  @Override public void moveToSafeZ(double speed) throws Exception {
    ...
    if (this.zSafeActuatorName.length() > 0 && this.zSafeActuatorValue.length() > 0) {
      ...
      if (zSafeActuator == null) {
        throw new Exception(String.format("No Actuator found with name %s on Head %s", zSafeActuatorName, this.getName()));
      }
      String zProbeValue = zSafeActuator.read();
      if (!zProbeValue.equals(zSafeActuatorValue)) {
        throw new Exception(String.format("Safe Z Actuator %s says Nozzle %s is not at safe Z (actuator is reading %s and not %s)", zSafeActuatorName, this.getName(), zProbeValue, zSafeActuatorValue));
      }
    }
  }
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(getConfigurationWizard()), new PropertySheetWizardAdapter(new ReferenceNozzleCompatibleNozzleTipsWizard(this), "Nozzle Tips"), new PropertySheetWizardAdapter(new ReferenceNozzlePartDetectionWizard(this), "Part Detection"), new PropertySheetWizardAdapter(new ReferenceNozzleToolChangerWizard(this), "Tool Changer"), new PropertySheetWizardAdapter(new ReferenceNozzleCameraOffsetWizard(this), "Offset Wizard") };
  }
  ...
  @Override public boolean isPartDetectionEnabled() {
  ...
  @Override public void purge() throws Exception {
    getDriver().place(this);
  }
