...
public class ReferenceNozzle extends AbstractNozzle implements ReferenceHeadMountable {
  ...
  @Element(required = false) protected Length safeZ = new Length(0, LengthUnit.Millimeters);
  ...
  /**
     ...
     */
  @Deprecated @Element(required = false) protected String vacuumSenseActuatorName;
  ...
  /**
     ...
     */
  @Attribute(required = false) private boolean limitRotation = true;
  ...
  @Override public void moveTo(Location location, double speed) throws Exception {
    ...
    if (limitRotation) {
      location = location.derive(null, null, null, Utils2D.normalizeAngle180(location.getRotation()));
    } else {
      location = location.derive(null, null, null, currentLocation.getRotation() + Utils2D.normalizeAngle180(location.getRotation() - currentLocation.getRotation()));
    }
    ReferenceNozzleTip calibrationNozzleTip = getCalibrationNozzleTip();
    if (calibrationNozzleTip != null && calibrationNozzleTip.getCalibration().isCalibrated(this)) {
  ...
  @Override public void moveTo(Location location) throws Exception {
  ...
  @Override public boolean isPartOnEnabled(Nozzle.PartOnStep step) {
    if ((step == PartOnStep.AfterPick && getNozzleTip().isPartOnCheckAfterPick()) || (step == PartOnStep.Align && getNozzleTip().isPartOnCheckAlign()) || (step == PartOnStep.BeforePlace && getNozzleTip().isPartOnCheckBeforePlace())) {
      return isVaccumActuatorEnabled() && (getNozzleTip().getMethodPartOn() != VacuumMeasurementMethod.None);
    }
    return false;
  }
  ...
  @Override public boolean isPartOffEnabled(Nozzle.PartOffStep step) {
    if ((step == PartOffStep.AfterPlace && getNozzleTip().isPartOffCheckAfterPlace()) || (step == PartOffStep.BeforePick && getNozzleTip().isPartOffCheckBeforePick())) {
      return isVaccumActuatorEnabled() && (getNozzleTip().getMethodPartOff() != VacuumMeasurementMethod.None);
    }
    return false;
  }
  ...
  protected void establishPickVacuumLevel(int milliseconds) throws Exception {
    ...
    if (vacuumGraph != null) {
      ...
      vacuumGraph.getRow(ReferenceNozzleTip.BOOLEAN, ReferenceNozzleTip.VALVE_ON).recordDataPoint(System.currentTimeMillis(), 1);
      nt.setVacuumPartOnGraph(vacuumGraph);
  ...
  protected void establishPlaceVacuumLevel(int milliseconds) throws Exception {
    ...
    if (vacuumGraph != null) {
      ...
      vacuumGraph.getRow(ReferenceNozzleTip.BOOLEAN, ReferenceNozzleTip.VALVE_ON).recordDataPoint(System.currentTimeMillis(), 0);
      nt.setVacuumPartOffGraph(vacuumGraph);
  ...
  protected double readPartOffVacuumLevel(int milliseconds) throws Exception {
    ...
    if (vacuumGraph != null) {
      long timeout = System.currentTimeMillis() + milliseconds;
      SimpleGraph.DataRow valveData = vacuumGraph.getRow(ReferenceNozzleTip.BOOLEAN, ReferenceNozzleTip.VALVE_ON);
  ...
  @Override public boolean isPartOn() throws Exception {
    ...
    SimpleGraph vacuumGraph = nt.getVacuumPartOnGraph();
    if (vacuumGraph != null) {
      vacuumGraph.getRow(ReferenceNozzleTip.PRESSURE, ReferenceNozzleTip.VACUUM).recordDataPoint(System.currentTimeMillis(), vacuumLevel);
      vacuumGraph.getRow(ReferenceNozzleTip.BOOLEAN, ReferenceNozzleTip.VALVE_ON).recordDataPoint(System.currentTimeMillis(), 1);
    }
    if (nt.getMethodPartOn().isDifferenceMethod()) {
