...
public class ReferenceNozzle extends AbstractNozzle implements ReferenceHeadMountable {
  ...
  @Element(required = false) private String vacuumSenseActuatorName;

  @Element(required = false) private String vacuumActuatorName;

  @Element(required = false) private String blowOffActuatorName;

  @Attribute(required = false) private boolean blowOffClosingValve = true;
  ...
  @Override public void applyConfiguration(Configuration configuration) {
  ...
  @Persist protected void persist() {
  ...
  public void adjustHeadOffsetsDependencies(Location headOffsetsOld, Location headOffsetsNew) {
    ...
    if (offsetsDiff.isInitialized() && headOffsetsNew.isInitialized() && head != null) {
      if (manualNozzleTipChangeLocation.isInitialized()) {
        ...
        Logger.info("Set manual nozzle tip change location for " + getName() + " to " + manualNozzleTipChangeLocation + " (previously " + oldLocation + ")");
      }
  ...
  @Override public void place() throws Exception {
    ...
    if (getPart() != null && getPart().getPackage().getPlaceBlowOffLevel() != 0) {
  ...
  @Override public Location toHeadMountableLocation(Location location, Location currentLocation, LocationOption... options) {
    ...
    if (!Arrays.asList(options).contains(LocationOption.SuppressDynamicCompensation)) {
      ...
      if (rotationModeOffset != null) {
        location = location.addWithRotation(new Location(location.getUnits(), 0, 0, 0, rotationModeOffset));
      }
    }
    return location;
  }
  ...
  @Override public void loadNozzleTip(NozzleTip nozzleTip) throws Exception {
    ...
    if (!nt.isUnloadedNozzleTipStandin()) {
      if (!changerEnabled) {
        ...
        throw new JobProcessorException(this, "Task interrupted: Please perform a manual nozzle tip " + nt.getName() + " load on nozzle " + getName() + " now. " + "You can then resume/restart the interrupted task.", true);
      }
    }
  ...
  @Override public void unloadNozzleTip() throws Exception {
    ...
    if (!changerEnabled) {
      waitForCompletion(CompletionType.WaitForStillstand);
      throw new JobProcessorException(this, "Task interrupted: Please perform a manual nozzle tip " + nt.getName() + " unload from nozzle " + getName() + " now. " + "You can then resume/restart the interrupted task.", true);
    }
    ensureZCalibrated(true);
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(getConfigurationWizard()), new PropertySheetWizardAdapter(new ReferenceNozzleCompatibleNozzleTipsWizard(this), Translations.getString("ReferenceNozzle.PropertySheetHolder.NozzleTips.title")), new PropertySheetWizardAdapter(new ReferenceNozzleVacuumWizard(this), Translations.getString("ReferenceNozzle.PropertySheetHolder.Vacuum.title")), new PropertySheetWizardAdapter(new ReferenceNozzleToolChangerWizard(this), Translations.getString("ReferenceNozzle.PropertySheetHolder.ToolChanger.title")), new PropertySheetWizardAdapter(new ReferenceNozzleCameraOffsetWizard(this), Translations.getString("ReferenceNozzle.PropertySheetHolder.OffsetWizard.title")) };
  }
  ...
  @Override public void findIssues(Solutions solutions) {
    ...
    try {
      if (solutions.isTargeting(Milestone.Basics)) {
        ActuatorSolutions.findActuateIssues(solutions, this, getVacuumActuator(), "vacuum valve", "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration%3A-Vacuum-Setup");
        if (getBlowOffActuator() != null) {
          AbstractActuator.suggestValueType(getBlowOffActuator(), ActuatorValueType.Double);
          ActuatorSolutions.findActuateIssues(solutions, this, getBlowOffActuator(), "blow off", "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration%3A-Vacuum-Setup");
        }
        ...
        for (NozzleTip tip : Configuration.get().getMachine().getNozzleTips()) {
          if (tip instanceof ReferenceNozzleTip) {
            ...
            if (referenceNozzleTip.getMethodPartOn() != VacuumMeasurementMethod.None || referenceNozzleTip.getMethodPartOff() != VacuumMeasurementMethod.None) {
              needsSensing = true;
              break;
            }
          }
        ...
        if (needsSensing) {
          ActuatorSolutions.findActuatorReadIssues(solutions, this, getVacuumSenseActuator(), "vacuum sensing", "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration%3A-Vacuum-Sensing#actuator-setup");
        }
      }
    } catch (Exception e) {
      Logger.warn(e);
