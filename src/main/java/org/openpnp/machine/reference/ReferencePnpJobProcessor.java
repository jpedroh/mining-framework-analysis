...
@Root public class ReferencePnpJobProcessor extends AbstractPnpJobProcessor {
  ...
  @Attribute(required = false) protected boolean parkWhenComplete = false;
  ...
  public ReferencePnpJobProcessor() {
    ...
    fsm.add(State.PreFlight, Message.Next, State.FiducialCheck, this::doPreFlight, Message.Next);
  ...
  public synchronized boolean next() throws Exception {
    ...
    if (fsm.getState() == State.Stopped) {
      ...
      if (fsm.getState() == State.Plan && isJobComplete()) {
    ...
    for (BoardLocation boardLocation : job.getBoardLocations()) {
      ...
      for (Placement placement : boardLocation.getBoard().getPlacements()) {
        if (idlist.contains(placement.getId())) {
          throw new Exception(String.format("This board contains at least one duplicate ID entry: %s ", placement.getId()));
        } else {
      ...
      for (Placement placement : boardLocation.getBoard().getPlacements()) {
        ...
        if (placement.getPart() == null) {
          throw new Exception(String.format("Part not found for board %s, placement %s.", boardLocation.getBoard().getName(), placement.getId()));
        }
        if (placement.getPart().getHeight().getValue() <= 0D) {
          throw new Exception(String.format("Part height for %s must be greater than 0.", placement.getPart().getId()));
        }
  ...
  protected void doPlan() throws Exception {
    ...
    List<JobPlacement> jobPlacements = getPendingJobPlacements().stream().sorted(Comparator.comparing(JobPlacement::getPartHeight)).collect(Collectors.toList());
  ...
  protected void doFeedAndPick() throws Exception {
    for (PlannedPlacement plannedPlacement : plannedPlacements) {
      ...
      if (!plannedPlacement.fed) {
        ...
        while (true) {
          ...
          try {
            ...
            if (lastError != null) {
              throw new Exception(String.format("Unable to feed %s. Feeder %s: %s.", part.getId(), lastErrorFeeder.getName(), lastError.getMessage()), lastError);
            } else {
          ...
          try {
            retry(1 + feeder.getRetryCount(), () -> {
              fireTextStatus("Feeding %s from %s for %s.", part.getId(), feeder.getName(), placement.getId());
              Logger.debug("Attempt Feed {} from {} with {}.", new Object[] { part, feeder, nozzle });
              feeder.feed(nozzle);
              Logger.debug("Fed {} from {} with {}.", new Object[] { part, feeder, nozzle });
            });
            break;
          } catch (Exception e) {
            Logger.debug("Feed {} from {} with {} failed!", new Object[] { part, feeder, nozzle });
      ...
      fireTextStatus("Picking %s from %s for %s.", part.getId(), feeder.getName(), placement.getId());
      ++totalPartsPlaced;
      nozzle.pick(part);
      nozzle.moveToSafeZ();
