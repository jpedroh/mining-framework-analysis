...
@SuppressWarnings(value = { "serial" }) public class BottomVisionSettingsConfigurationWizard extends AbstractConfigurationWizard {
  ...
  public BottomVisionSettingsConfigurationWizard(BottomVisionSettings visionSettings, PartSettingsHolder settingsHolder) {
  ...
  private void createUi() {
    ...
    panelAssignedTo.setLayout(new FormLayout(new ColumnSpec[] { ColumnSpec.decode("min(70dlu;default):grow") }, new RowSpec[] { FormSpecs.DEFAULT_ROWSPEC }));
    usedIn = new JLabel("None");
    panelAssignedTo.add(usedIn, "1, 1, fill, fill");
    JLabel lblAssignedTo = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.AssignedToLabel.text"));
    panel.add(lblAssignedTo, "2, 4");
    JLabel lblSettings = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.ManageSettingsLabel.text"));
    ...
    if (settingsHolder != null && bottomVision.getParentHolder(settingsHolder) != null) {
      btnSpecializeSetting.setText(Translations.getString("BottomVisionSettingsConfigurationWizard.SpecializeSettingsButton.SpecializeForText") + " " + settingsHolder.getShortName());
      btnSpecializeSetting.setToolTipText(Translations.getString("BottomVisionSettingsConfigurationWizard.SpecializeSettingsButton.toolTipText") + " " + settingsHolder.getClass().getSimpleName() + " " + settingsHolder.getShortName());
    } else {
      if (settingsHolder != null) {
        btnSpecializeSetting.setText(Translations.getString("BottomVisionSettingsConfigurationWizard.SpecializeSettingsButton.OptimizeText"));
        btnSpecializeSetting.setToolTipText("<html>Optimize the Bottom Vision Settings and their assignments:<br/>" + "<ul>" + "<li>Consolidate duplicate settings.</li>" + "<li>Remove unused settings.</li>" + "<li>Configure the most common Part settings as inherited Package settings.</li>" + "<li>Remove assignments where the same settings would be inherited anyway.</li>" + "</ul>" + "</html>");
      } else {
        btnSpecializeSetting.setEnabled(false);
        btnSpecializeSetting.setText(Translations.getString("BottomVisionSettingsConfigurationWizard.SpecializeSettingsButton.SpecializeText"));
    ...
    btnSpecializeSetting.addActionListener((e) -> {
      ...
      UiUtils.messageBoxOnException(() -> {
        if (settingsHolder != null && bottomVision.getParentHolder(settingsHolder) != null) {
          if (visionSettings.getUsedBottomVisionIn().size() == 1 && visionSettings.getUsedBottomVisionIn().get(0) == settingsHolder) {
    ...
    btnGeneralizeSettings = new JButton(Translations.getString("BottomVisionSettingsConfigurationWizard.GeneralizeButton.text"));
    ...
    if (settingsHolder == null || settingsHolder instanceof Part) {
      ...
      btnGeneralizeSettings.setText(Translations.getString("BottomVisionSettingsConfigurationWizard.GeneralizeButton.GeneralizeFor.text") + settingsHolder.getShortName());
      btnGeneralizeSettings.setToolTipText("<html>Generalize these Bottom Vision Settings for all the " + subjects + " with the " + settingsHolder.getClass().getSimpleName() + " " + settingsHolder.getShortName() + ".<br/>" + "This will unassign any special Bottom Vision Settings on " + subjects + " and delete those<br/>" + "Bottom Vision Settings that are no longer used elsewhere.</html>");
    }
    JButton resetButton = new JButton(Translations.getString("BottomVisionSettingsConfigurationWizard.ResetButton.text"));
    resetButton.addActionListener((e) -> {
      UiUtils.messageBoxOnException(() -> {
        int result = JOptionPane.showConfirmDialog(getTopLevelAncestor(), "This will reset the bottom vision settings with to the default settings. Are you sure??", null, JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (result == JOptionPane.YES_OPTION) {
          ...
          if (bottomVision.getBottomVisionSettings() == visionSettings) {
            BottomVisionSettings stockVisionSettings = (BottomVisionSettings) Configuration.get().getVisionSettings(AbstractVisionSettings.STOCK_BOTTOM_ID);
    ...
    JLabel lblEnabled = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.EnabledLabel.text"));
    panel.add(lblEnabled, "2, 8");
    enabledCheckbox = new JCheckBox("");
    panel.add(enabledCheckbox, "4, 8");
    JLabel lblPrerotate = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.PreRotateLabel.text"));
    ...
    JLabel lblMaxRotation = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.RotationLabel.text"));
    panel.add(lblMaxRotation, "6, 10, right, default");
    comboBoxMaxRotation = new JComboBox(ReferenceBottomVision.MaxRotation.values());
    comboBoxMaxRotation.setToolTipText(Translations.getString("BottomVisionSettingsConfigurationWizard.ComboMaxRotation.toolTipText"));
    panel.add(comboBoxMaxRotation, "8, 10, fill, default");
    JLabel lblPartCheckType = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.PartSizeCheckLabel.text"));
    ...
    JLabel lblPartSizeTolerance = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.PartSizeToleranceLabel.text"));
    ...
    pipelinePanel = new PipelineControls() {
      @Override public void configurePipeline(CvPipeline pipeline, Map<String, Object> pipelineParameterAssignments, boolean edit) throws Exception {
        UiUtils.messageBoxOnException(() -> {
          if (edit) {
            applyAction.actionPerformed(null);
          }
      ...
      @Override public void resetPipeline() throws Exception {
        int result = JOptionPane.showConfirmDialog(getTopLevelAncestor(), "This will replace the Pipeline with the default. Are you sure??", null, JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (result == JOptionPane.YES_OPTION) {
          UiUtils.messageBoxOnException(() -> {
            applyAction.actionPerformed(null);
            ReferenceBottomVision bottomVision = ReferenceBottomVision.getDefault();
    ...
    panelAlign.setBorder(new TitledBorder(null, Translations.getString("BottomVisionSettingsConfigurationWizard.PanelAlign.Border.title"), TitledBorder.LEADING, TitledBorder.TOP, null, null));
    panelAlign.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("right:max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, new ColumnSpec(ColumnSpec.FILL, Sizes.bounded(Sizes.DEFAULT, Sizes.constant("50dlu", true), Sizes.constant("70dlu", true)), 1), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("default:grow") }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC }));
    JLabel lblTestPlacementAngle = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelAlign.TestPlacementAngleLabel.text"));
    ...
    JButton btnTestAlighment = new JButton(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelAlign.TestAlignmentButton.text"));
    panelAlign.add(btnTestAlighment, "6, 2");
    chckbxCenterAfterTest = new JCheckBox(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelAlign.CenterAfterTestChkbox.text"));
    panelAlign.add(chckbxCenterAfterTest, "8, 2");
    chckbxCenterAfterTest.setToolTipText(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelAlign.CenterAfterTestChkbox.toolTipText"));
    ...
    panelDetectOffset.setBorder(new TitledBorder(null, Translations.getString("BottomVisionSettingsConfigurationWizard.PanelDetectOffset.Border.title"), TitledBorder.LEADING, TitledBorder.TOP, null, null));
    panelDetectOffset.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("right:max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("default:grow") }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC }));
    JLabel lblX = new JLabel("X");
    panelDetectOffset.add(lblX, "4, 2, center, default");
    JLabel lblY = new JLabel("Y");
    panelDetectOffset.add(lblY, "6, 2, center, default");
    JLabel lblVisionCenterOffset = new JLabel(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelDetectOffset.VisionCenterOffsetLabel.text"));
    panelDetectOffset.add(lblVisionCenterOffset, "2, 4");
    lblVisionCenterOffset.setToolTipText(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelDetectOffset.VisionCenterOffsetLabel.toolTipText"));
    ...
    JButton btnAutoVisionCenterOffset = new JButton(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelDetectOffset.AutoVisionCenterOffsetButton.text"));
    panelDetectOffset.add(btnAutoVisionCenterOffset, "8, 4");
    btnAutoVisionCenterOffset.setToolTipText(Translations.getString("BottomVisionSettingsConfigurationWizard.PanelDetectOffset.AutoVisionCenterOffsetButton.toolTipText"));
  ...
  @Override public void createBindings() {
    ...
    bind(UpdateStrategy.READ, visionSettings, "usedBottomVisionIn", usedIn, "text", new AbstractVisionSettings.ListConverter(true, settingsHolder));
  ...
  private void createPanel() {
    panel = new JPanel();
    panel.setBorder(new TitledBorder(null, Translations.getString("BottomVisionSettingsConfigurationWizard.GeneralPanel.Border.title"), TitledBorder.LEADING, TitledBorder.TOP, null, null));
    contentPanel.add(panel);
    panel.setLayout(new FormLayout(new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("right:max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(70dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(50dlu;default)"), FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("default:grow") }, new RowSpec[] { FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, FormSpecs.DEFAULT_ROWSPEC, FormSpecs.RELATED_GAP_ROWSPEC, RowSpec.decode("default:grow") }));
  }
  ...
  private void pipelineConfiguration(CvPipeline pipeline, Map<String, Object> pipelineParameterAssignments, boolean edit) throws Exception {
    ...
    double angle = new DoubleConverter(Configuration.get().getLengthDisplayFormat()).convertReverse(testAlignmentAngle.getText());
    ...
    if (part != null) {
      ...
      if (settingsHolder instanceof Part) {
        ...
        if (settingsHolder instanceof Package) {
          pkg = (Package) settingsHolder;
        } else {
          if (MainFrame.get().getPartsTab().getSelectedPart() != null) {
            part = MainFrame.get().getPartsTab().getSelectedPart();
            pkg = part.getPackage();
          } else {
            if (MainFrame.get().getPackagesTab().getSelectedPackage() != null) {
    ...
    if (pkg == null) {
      throw new Exception("A package must be designated to configure the pipeline. " + "Please pick a part with selected nozzle " + nozzle.getName() + ". " + "Alternatively, you can select a single part or package on the Parts or " + "Packages tab for a \"dry-run\" with empty nozzle (like after a failed pick).");
    }
    ...
    if (!pkg.getCompatibleNozzleTips().contains(nt)) {
      throw new Exception("Nozzle tip " + nt.getName() + " loaded on selected nozzle " + nozzle.getName() + " is not compatible with package " + pkg.getId() + ".");
    }
    Location location = bottomVision.getCameraLocationAtPartHeight(part, camera, nozzle, angle);
    bottomVision.preparePipeline(pipeline, pipelineParameterAssignments, camera, pkg, nozzle, nt, location, location, visionSettings);
    if (edit) {
      pipelinePanel.openPipelineEditor("Bottom Vision Pipeline", pipeline, "move nozzle " + nozzle.getName() + " to the camera alignment location before editing the pipeline", nozzle, location);
    }
  }
  ...
  public void alignAndCenter(ReferenceBottomVision bottomVision, Nozzle nozzle, double angle, boolean centerAfterTest) throws Exception {
    PartAlignment.PartAlignmentOffset alignmentOffset = VisionUtils.findPartAlignmentOffsets(bottomVision, nozzle.getPart(), null, new Location(LengthUnit.Millimeters, 0, 0, 0, angle), nozzle);
    Location offsets = alignmentOffset.getLocation();
    if (!centerAfterTest) {
      return;
    }
    Location cameraLocation = bottomVision.getCameraLocationAtPartHeight(nozzle.getPart(), VisionUtils.getBottomVisionCamera(), nozzle, angle);
  ...
  public Nozzle getNozzleWithPart() throws Exception {
    ...
    if (settingsHolder instanceof Part && part != this.settingsHolder) {
      throw new Exception("Wrong part " + part.getId() + " on Nozzle " + nozzle.getName());
    } else {
      if (settingsHolder instanceof Package && part.getPackage() != this.settingsHolder) {
    ...
    if (bottomVision.getInheritedVisionSettings(part) != visionSettings) {
      throw new Exception("Present Bottom Vision Settings are not effective for part " + part.getId() + " on Nozzle " + nozzle.getName() + ". " + "Assign to Package of Part.");
    }
    return nozzle;
  }
