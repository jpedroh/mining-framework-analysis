...
public class ReferenceBottomVision extends AbstractPartAlignment {
  @Deprecated @Element(required = false) protected CvPipeline pipeline;

  @Attribute(required = false) protected boolean enabled = false;

  @Attribute(required = false) protected boolean preRotate = false;

  @Attribute(required = false) protected int maxVisionPasses = 3;

  @Element(required = false) protected Length maxLinearOffset = new Length(1, LengthUnit.Millimeters);

  @Attribute(required = false) protected double maxAngularOffset = 10;

  @Attribute(required = false) protected double testAlignmentAngle = 0.0;

  @Element(required = false) protected Length maxSearchDistance = new Length(2, LengthUnit.Millimeters);

  @Deprecated @ElementMap(required = false) protected Map<String, PartSettings> partSettingsByPartId = null;
  ...
  @Override public PartAlignmentOffset findOffsets(Part part, BoardLocation boardLocation, Location placementLocation, Nozzle nozzle) throws Exception {
    ...
    if ((bottomVisionSettings.getPreRotateUsage() == PreRotateUsage.Default && preRotate) || (bottomVisionSettings.getPreRotateUsage() == PreRotateUsage.AlwaysOn)) {
  ...
  public Location getCameraLocationAtPartHeight(Part part, Camera camera, Nozzle nozzle, double angle) throws Exception {
    if (part.isPartHeightUnknown()) {
      if (camera.getFocusProvider() != null && nozzle.getNozzleTip() instanceof ReferenceNozzleTip) {
        ReferenceNozzleTip nt = (ReferenceNozzleTip) nozzle.getNozzleTip();
        Location location1 = camera.getLocation(nozzle).derive(null, null, null, angle);
        Location location0 = location1.add(new Location(nt.getMaxPartHeight().getUnits(), 0, 0, nt.getMaxPartHeight().getValue(), 0));
    ...
    return camera.getLocation(nozzle).add(new Location(part.getHeight().getUnits(), 0.0, 0.0, part.getHeight().getValue(), 0.0)).derive(null, null, null, angle);
  }

  private PartAlignmentOffset findOffsetsPreRotate(Part part, BoardLocation boardLocation, Location placementLocation, Nozzle nozzle, Camera camera, BottomVisionSettings bottomVisionSettings) throws Exception {
    double wantedAngle = placementLocation.getRotation();
    if (boardLocation != null) {
      wantedAngle = Utils2D.calculateBoardPlacementLocation(boardLocation, placementLocation).getRotation();
    }
    ...
    try (CvPipeline pipeline = bottomVisionSettings.getCvPipeline()) {
      ...
      for (int pass = 0; ; ) {
        ...
        offsets = offsets.rotateXy(-angleOffset).derive(null, null, null, angleOffset);
        ...
        rect.points(corners);
        Location corner = VisionUtils.getPixelCenterOffsets(camera, corners[0].x, corners[0].y).convertToUnits(maxLinearOffset.getUnits());
        Location cornerWithAngularOffset = corner.rotateXy(angleOffset);
        if (!partSizeCheck(part, bottomVisionSettings, rect, camera)) {
          throw new Exception(String.format("ReferenceBottomVision (%s): Incorrect part size.", part.getId()));
        } else {
          if (center.getLinearDistanceTo(offsets) > getMaxLinearOffset().getValue()) {
            Logger.debug("Offsets too large {} : center offset {} > {}", offsets, center.getLinearDistanceTo(offsets), getMaxLinearOffset().getValue());
          } else {
            if (corner.getLinearDistanceTo(cornerWithAngularOffset) > getMaxLinearOffset().getValue()) {
              Logger.debug("Offsets too large {} : corner offset {} > {}", offsets, corner.getLinearDistanceTo(cornerWithAngularOffset), getMaxLinearOffset().getValue());
            } else {
              if (Math.abs(angleOffset) > getMaxAngularOffset()) {
                Logger.debug("Offsets too large {} : angle offset {} > {}", offsets, Math.abs(angleOffset), getMaxAngularOffset());
              } else {
                break;
              }
            }
  ...
  private PartAlignmentOffset findOffsetsPostRotate(Part part, BoardLocation boardLocation, Location placementLocation, Nozzle nozzle, Camera camera, BottomVisionSettings bottomVisionSettings) throws Exception {
...
>>>>>>> /usr/src/app/output/openpnp/openpnp/5f47cac410b0b91c98034f026b69c9c955f79e4c/src/main/java/org/openpnp/machine/reference/vision/ReferenceBottomVision.java/right.java
    ...
    if (measuredSize.width > pxMaxWidth) {
      ...
      if (measuredSize.width < pxMinWidth) {
        Logger.debug("Package pixel width {} : limit {} : measured {}", pxWidth, pxMinWidth, measuredSize.width);
        return false;
      } else {
        if (measuredSize.height > pxMaxHeight) {
          Logger.debug("Package pixel height {} : limit {} : measured {}", pxHeight, pxMaxHeight, measuredSize.height);
          return false;
        } else {
          if (measuredSize.height < pxMinHeight) {
            Logger.debug("Package pixel height {} : limit {} : measured {}", pxHeight, pxMinHeight, measuredSize.height);
            return false;
          }
        }
      }
  ...
  private static void displayResult(CvPipeline pipeline, Part part, Location offsets, Camera camera, Nozzle nozzle) {
    ...
    if (mainFrame != null) {
      try {
        ...
        mainFrame.getCameraViews().getCameraView(camera).showFilteredImage(OpenCvUtils.toBufferedImage(pipeline.getWorkingImage()), s, 1500);
  ...
  private RotatedRect processPipelineAndGetResult(CvPipeline pipeline, Camera camera, Part part, Nozzle nozzle, Location wantedLocation, BottomVisionSettings bottomVisionSettings) throws Exception {
    ...
    if (result == null) {
      throw new Exception(String.format("ReferenceBottomVision (%s): Pipeline error. Pipeline must contain a result named \'%s\'.", part.getId(), VisionUtils.PIPELINE_RESULTS_NAME));
    }
    if (result.model == null) {
      throw new Exception(String.format("ReferenceBottomVision (%s): No result found.", part.getId()));
    }
    if (!(result.model instanceof RotatedRect)) {
      throw new Exception(String.format("ReferenceBottomVision (%s): Incorrect pipeline result type (%s). Expected RotatedRect.", part.getId(), result.model.getClass().getSimpleName()));
    }
    return (RotatedRect) result.model;
  }

  @Override public boolean canHandle(PartSettingsHolder settingsHolder, boolean allowDisabled) {
    ...
    if (visionSettings != null) {
      ...
      if (!allowDisabled) {
        Logger.trace("{}.canHandle({}) => {}, {}", this.getClass().getSimpleName(), settingsHolder == null ? "" : settingsHolder.getId(), visionSettings, isEnabled ? "enabled" : "disabled");
      }
  ...
  @Override public void setBottomVisionSettings(BottomVisionSettings visionSettings) {
    if (visionSettings == null) {
      return;
    }
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(new ReferenceBottomVisionConfigurationWizard(this)), new PropertySheetWizardAdapter(new BottomVisionSettingsConfigurationWizard(getBottomVisionSettings(), this)) };
  }
  ...
  @Deprecated @Root public static class PartSettings extends AbstractModelObject {
    @Deprecated @Attribute protected boolean enabled = true;

    @Deprecated @Attribute(required = false) protected PreRotateUsage preRotateUsage = PreRotateUsage.Default;

    @Deprecated @Attribute(required = false) protected PartSizeCheckMethod checkPartSizeMethod = PartSizeCheckMethod.Disabled;

    @Deprecated @Attribute(required = false) protected int checkSizeTolerancePercent = 20;

    @Deprecated @Attribute(required = false) protected MaxRotation maxRotation = MaxRotation.Adjust;

    @Deprecated @Element(required = false) protected Location visionOffset = new Location(LengthUnit.Millimeters);

    @Deprecated @Element protected CvPipeline pipeline;

    @Deprecated public PartSettings() {
    }
    ...
    @Deprecated public PreRotateUsage getPreRotateUsage() {
      return preRotateUsage;
    }

    @Deprecated public CvPipeline getPipeline() {
      return pipeline;
    }

    @Deprecated public void setPipeline(CvPipeline pipeline) {
      this.pipeline = pipeline;
    }

    @Deprecated public MaxRotation getMaxRotation() {
      return maxRotation;
    }
  ...
  public void optimizeVisionSettings(Configuration configuration) {
    ...
    for (AbstractVisionSettings visionSettings : configuration.getVisionSettings()) {
      if (visionSettings instanceof BottomVisionSettings) {
        ...
        if (firstVisionSettings == null) {
          ...
          if (visionSettings != defaultVisionSettings && !visionSettings.isStockSetting()) {
            ...
            if (visionSettings.getUsedBottomVisionIn().size() == 0) {
              if (firstVisionSettings != defaultVisionSettings && !firstVisionSettings.isStockSetting()) {
        ...
        for (Part part : configuration.getParts()) {
          if (part.getPackage() == pkg) {
            if (part.getBottomVisionSettings() == mostFrequentVisionSettings) {
              ...
              if (part.getBottomVisionSettings() == null && packageVisionSettings != mostFrequentVisionSettings) {
                part.setBottomVisionSettings(packageVisionSettings);
              }
            }
          }
        }
        if (mostFrequentVisionSettings != defaultVisionSettings && !mostFrequentVisionSettings.isStockSetting() && !mostFrequentVisionSettings.getName().isEmpty() && mostFrequentVisionSettings.getUsedBottomVisionIn().size() == 1) {
    ...
    for (AbstractVisionSettings visionSettings : configuration.getVisionSettings()) {
      if (visionSettings instanceof BottomVisionSettings) {
        ...
        if (!visionSettings.isStockSetting() && visionSettings != defaultVisionSettings && usedIn.isEmpty()) {
          configuration.removeVisionSettings(visionSettings);
        } else {
