...
public class ReferenceBottomVision implements PartAlignment {
  ...
  @Override public PartAlignmentOffset findOffsets(Part part, BoardLocation boardLocation, Location placementLocation, Nozzle nozzle) throws Exception {
    ...
    if (preRotate) {
      ...
      MovableUtils.moveToLocationAtSafeZ(nozzle, camera.getLocation().add(new Location(part.getHeight().getUnits(), 0.0, 0.0, part.getHeight().getValue(), 0.0)).derive(null, null, null, angle));
      ...
      nozzle.moveTo(new Location(LengthUnit.Millimeters, Double.NaN, Double.NaN, Double.NaN, placementAngle + angle), nozzle.getHead().getMachine().getSpeed());
      pipeline.process();
      if (!((pipeline.getResult("result")).model instanceof RotatedRect)) {
        throw new Exception("Bottom vision alignment failed for part " + part.getId() + " on nozzle " + nozzle.getName() + ". No result found.");
      }
      ...
      Location offsets = VisionUtils.getPixelCenterOffsets(camera, rect.center.x, rect.center.y).derive(null, null, null, Double.NaN);
