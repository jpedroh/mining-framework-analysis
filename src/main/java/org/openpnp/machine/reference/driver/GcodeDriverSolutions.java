        ...
        Solutions.Issue issue = new Solutions.Issue(gcodeDriver, "Use the GcodeAsyncDriver for advanced features. Accept or Dismiss to continue.", "Convert to GcodeAsyncDriver.", Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver") {
          @Override public void setState(Solutions.State state) throws Exception {
            if (state == Solutions.State.Solved) {
              ...
              if (getState() == Solutions.State.Solved) {
...
=======
        if (gcodeDriver.getFirmwareProperty("FIRMWARE_NAME", "").contains("Duet")) {
          isDuet = true;
          String firmwareVersion = gcodeDriver.getFirmwareProperty("FIRMWARE_VERSION", "0.0");
          Integer major = null;
          Integer minor = null;
          try {
            Matcher matcher = Pattern.compile("(?<major>-?\\d+)\\.(?<minor>-?\\d+).*").matcher(firmwareVersion);
            matcher.matches();
          ...
          if (major == null || minor == null || major < 3 || (major == 3 && minor < 3)) {
            issues.add(new Solutions.PlainIssue(gcodeDriver, "Duet3D firmware was improved for OpenPnP, please use version 3.3beta or newer. Current version is " + firmwareVersion, "Get the new version through the linked web page.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Motion-Controller-Firmwares#duet"));
          }
          if (gcodeDriver.getConfiguredAxes() != null) {
            try {
              Matcher matcher = Pattern.compile(".*\\s(?<axes>-?\\d+)\\saxes\\svisible.*").matcher(gcodeDriver.getConfiguredAxes());
              matcher.matches();
              firmwareAxesCount = Integer.parseUnsignedInt(matcher.group("axes"));
            } catch (NumberFormatException e) {
            }
            if (gcodeDriver.getConfiguredAxes().contains("(r)")) {
              issues.add(new Solutions.PlainIssue(gcodeDriver, "Axes should be configured as linear in feedrate calculations on the Duet controller. See the linked web page.", "Use the M584 S0 option in your config.g file.", Severity.Error, "https://duet3d.dozuki.com/Wiki/Gcode#Section_M584_Set_drive_mapping"));
            } else {
              firmwarePrimaryAxesCount = firmwareAxesCount;
            }
          }
        } else {
          if (gcodeDriver.getFirmwareProperty("FIRMWARE_NAME", "").contains("Marlin")) {
            isMarlin = true;
            firmwareAxesCount = Integer.valueOf(gcodeDriver.getFirmwareProperty("AXIS_COUNT", "0"));
            if (firmwareAxesCount > 3) {
              firmwarePrimaryAxesCount = firmwareAxesCount;
            } else {
              issues.add(new Solutions.PlainIssue(gcodeDriver, "There is a better Marlin firmware available. " + gcodeDriver.getDetectedFirmware(), "Please upgrade to the special PnP version. See info link.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Motion-Controller-Firmwares#marlin-20"));
            }
          } else {
            if (gcodeDriver.getFirmwareProperty("FIRMWARE_NAME", "").contains("TinyG")) {
              isTinyG = true;
            } else {
              if (gcodeDriver.getDetectedFirmware().contains("GcodeServer")) {
              } else {
                issues.add(new Solutions.PlainIssue(gcodeDriver, "Unknown firmware. " + gcodeDriver.getDetectedFirmware(), "Check out firmwares known to be well supported. See info link.", Severity.Warning, "https://github.com/openpnp/openpnp/wiki/Motion-Controller-Firmwares"));
              }
            }
          }
        }
>>>>>>> /usr/src/app/output/openpnp/openpnp/a9f99068197956007021fea9951f899a66d1865c/src/main/java/org/openpnp/machine/reference/driver/GcodeDriverSolutions.java/right.java
    ...
    if (solutions.isTargeting(Milestone.Connect)) {
      if (gcodeDriver.getCommunicationsType() == CommunicationsType.tcp && gcodeDriver.getIpAddress().contentEquals("GcodeServer")) {
        solutions.add(new Solutions.PlainIssue(gcodeDriver, "Connect the driver to your controller.", "Choose the right communications type and port/address settings.", Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/GcodeDriver#connection"));
      }
      ...
      if (gcodeDriver.getDetectedFirmware() == null) {
        solutions.add(new Solutions.Issue(gcodeDriver, "Firmware was not dected (" + (machine.isEnabled() ? (gcodeDriver.isSpeakingGcode() ? "failure, check log" : "controller may not speak Gcode") : "machine is disabled") + ").", "Retry the detection by connecting to the controller or assume a generic controller.", Severity.Fundamental, "https://www.reprap.org/wiki/G-code#M115:_Get_Firmware_Version_and_Capabilities") {
          @Override public Solutions.Issue.Choice[] getChoices() {
            return new Solutions.Issue.Choice[] { new Solutions.Issue.Choice(true, "<html><h3>Detect the firmware automatically</h3>" + "<p>The M115 command must be supported by the firmware. Make sure the controller " + "is connected to the computer and accept the solution to perform the detection.</p><br/>" + "<p>This might take a while!</p><br/>" + "<p>If the formware is known by OpenPnP, it will be able to automatically generate G-code " + "configuration for you." + "</html>", Icons.powerOn), new Solutions.Issue.Choice(false, "<html><h3>Assume a generic G-code controller</h3>" + "<p>When the M115 command is not supported by your controller you can let " + "OpenPnP propose a generic G-code configuration.</p><br/>" + "<p>You will likely need to hand-tune the G-code configuration to work with your controller.</p>" + "</html>", Icons.powerOff) };
          }
        ...
        if (gcodeDriver.getFirmwareProperty("FIRMWARE_NAME", "").contains("Smoothieware")) {
          ...
          if (firmwarePrimaryAxesCount == firmwareAxesCount) {
            ...
            solutions.add(new Solutions.PlainIssue(gcodeDriver, "There is a better Smoothieware firmware available. " + gcodeDriver.getDetectedFirmware(), "Please upgrade to the special PnP version. See info link.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Motion-Controller-Firmwares#smoothieware"));
          }
        } else {
          if (gcodeDriver.getFirmwareProperty("FIRMWARE_NAME", "").contains("Duet")) {
            isDuet = true;
            ...
            if (major == null || minor == null || major < 3 || (major == 3 && minor < 3)) {
              solutions.add(new Solutions.PlainIssue(gcodeDriver, "Duet3D firmware was improved for OpenPnP, please use version 3.3beta or newer. Current version is " + firmwareVersion, "Get the new version through the linked web page.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Motion-Controller-Firmwares#duet"));
            }
            if (gcodeDriver.getConfiguredAxes() != null) {
              try {
                Matcher matcher = Pattern.compile(".*\\s(?<axes>-?\\d+)\\saxes\\svisible.*").matcher(gcodeDriver.getConfiguredAxes());
              ...
              if (gcodeDriver.getConfiguredAxes().contains("(r)")) {
                solutions.add(new Solutions.PlainIssue(gcodeDriver, "Axes should be configured as linear in feedrate calculations on the Duet controller. See the linked web page.", "Use the M584 S0 option in your config.g file.", Severity.Error, "https://duet3d.dozuki.com/Wiki/Gcode#Section_M584_Set_drive_mapping"));
              } else {
                firmwarePrimaryAxesCount = firmwareAxesCount;
              }
            }
          } else {
            if (gcodeDriver.getDetectedFirmware().contains("Marlin")) {
              isMarlin = true;
              firmwareAxesCount = Integer.valueOf(gcodeDriver.getFirmwareProperty("AXIS_COUNT", "0"));
              if (firmwareAxesCount > 3) {
                firmwarePrimaryAxesCount = firmwareAxesCount;
              } else {
                solutions.add(new Solutions.PlainIssue(gcodeDriver, "There is a better Marlin firmware available. " + gcodeDriver.getDetectedFirmware(), "Please upgrade to the special PnP version. See info link.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Motion-Controller-Firmwares#marlin-20"));
              }
            } else {
              if (gcodeDriver.getDetectedFirmware().contains("TinyG")) {
                isTinyG = true;
              } else {
                if (gcodeDriver.getDetectedFirmware().contains("GcodeServer")) {
                } else {
                  solutions.add(new Solutions.PlainIssue(gcodeDriver, "Unknown firmware. " + gcodeDriver.getDetectedFirmware(), "Check out firmwares known to be well supported. See info link.", Severity.Warning, "https://github.com/openpnp/openpnp/wiki/Motion-Controller-Firmwares"));
                }
              }
            }
          }
        ...
        if (gcodeDriver.communicationsType == CommunicationsType.serial && gcodeDriver.serial != null) {
          ...
          if (oldFlowControl != newFlowControl) {
            solutions.add(new Solutions.Issue(gcodeDriver, "Change of serial port Flow Control recommended.", "Set " + newFlowControl.name() + " on serial port.", (gcodeDriver.serial.getFlowControl() == FlowControl.Off || isTinyG ? Severity.Warning : Severity.Suggestion), "https://en.wikipedia.org/wiki/Flow_control_(data)#Hardware_flow_control") {
              @Override public void setState(Solutions.State state) throws Exception {
                gcodeDriver.serial.setFlowControl((state == Solutions.State.Solved) ? newFlowControl : oldFlowControl);
                super.setState(state);
              }
            });
          }
      ...
      if (gcodeDriver.isConnectionKeepAlive()) {
        solutions.add(new Solutions.Issue(gcodeDriver, "Use Keep-Alive only when necessary. It may cause hard to diagnose problems.", "Disable Connection Keep-Alive.", Severity.Warning, null) {
          @Override public void setState(Solutions.State state) throws Exception {
      ...
      if (solutions.isTargeting(Milestone.Basics)) {
        if (gcodeDriver instanceof GcodeAsyncDriver) {
          ...
          if (confirmationFlowControlRecommended != confirmationFlowControl) {
            solutions.add(new Solutions.Issue(gcodeDriver, (confirmationFlowControl ? "Disable Confirmation Flow Control for full asynchronous operation." : "Enable Confirmation Flow Control" + (hasAxes ? "" : ", controller has no axes") + (isTinyG ? ", TinyG detected" : "") + "."), (confirmationFlowControl ? "Disable Confirmation Flow Control." : "Enable Confirmation Flow Control."), Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#advanced-settings") {
              @Override public void setState(Solutions.State state) throws Exception {
                ((GcodeAsyncDriver) gcodeDriver).setConfirmationFlowControl(confirmationFlowControl ^ (state == Solutions.State.Solved));
                super.setState(state);
              }
            });
          }
        ...
        if (hasAxes) {
          if (gcodeDriver.isSupportingPreMove()) {
            solutions.add(new Solutions.Issue(gcodeDriver, "Disallow Pre-Move Commands for advanced features. Accept or Dismiss to continue.", "Disable Allow Letter Pre-Move Commands.", Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/Advanced-Motion-Control#migration-from-a-previous-version") {
              @Override public void setState(Solutions.State state) throws Exception {
            ...
            if (!gcodeDriver.isUsingLetterVariables()) {
              solutions.add(new Solutions.Issue(gcodeDriver, "Use Axis Letter Variables for simpler use and advanced features.", "Enable Letter Variables.", Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/Advanced-Motion-Control#migration-from-a-previous-version") {
                @Override public void setState(Solutions.State state) throws Exception {
          ...
          if (solutions.isTargeting(Milestone.Advanced)) {
            ...
            final MotionControlType newMotionControlType = isTinyG ? MotionControlType.SimpleSCurve : MotionControlType.ModeratedConstantAcceleration;
            if (gcodeDriver.getMotionControlType().isUnpredictable() || (isTinyG && newMotionControlType != oldMotionControlType)) {
              solutions.add(new Solutions.Issue(gcodeDriver, (isTinyG ? "Choose " + newMotionControlType.name() + " for proper TinyG operation." : "Choose an advanced Motion Control Type for your controller type."), "Set to " + newMotionControlType.name() + ".", (isTinyG ? Severity.Error : Severity.Suggestion), "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#gcodedriver-new-settings") {
                @Override public void setState(Solutions.State state) throws Exception {
                  gcodeDriver.setMotionControlType((state == Solutions.State.Solved) ? newMotionControlType : oldMotionControlType);
                  super.setState(state);
                }
              });
            } else {
              if (gcodeDriver.getMaxFeedRate() > 0) {
                solutions.add(new Solutions.Issue(gcodeDriver, "Axis velocity limited by driver Maximum Feed Rate. ", "Remove driver Maximum Feed Rate.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#gcodedriver-new-settings") {
            ...
            if (oldMotionControlType != newMotionControlType) {
              solutions.add(new Solutions.Issue(gcodeDriver, "Choose the simplest Motion Control Type for the first basic setup.", "Set to " + newMotionControlType.name() + ".", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#gcodedriver-new-settings") {
                @Override public void setState(Solutions.State state) throws Exception {
                  gcodeDriver.setMotionControlType((state == Solutions.State.Solved) ? newMotionControlType : oldMotionControlType);
                  super.setState(state);
                }
        ...
        if (gcodeDriver.isSpeakingGcode()) {
          if (solutions.isTargeting(Milestone.Advanced)) {
            if (!gcodeDriver.isCompressGcode()) {
              solutions.add(new Solutions.Issue(gcodeDriver, "Compress Gcode for superior communications speed.", "Enable Compress Gcode.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#gcodedriver-new-settings") {
                @Override public void setState(Solutions.State state) throws Exception {
            ...
            if (!gcodeDriver.isRemoveComments()) {
              solutions.add(new Solutions.Issue(gcodeDriver, "Remove Gcode comments for superior communications speed.", "Enable Remove Comments.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#gcodedriver-new-settings") {
                @Override public void setState(Solutions.State state) throws Exception {
            ...
            if (gcodeDriver.isCompressGcode()) {
              solutions.add(new Solutions.Issue(gcodeDriver, "Disable G-code compression for trouble-free operation with incompatible controllers.", "Disable Compress G-code.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#gcodedriver-new-settings") {
                @Override public void setState(Solutions.State state) throws Exception {
                  gcodeDriver.setCompressGcode((state != Solutions.State.Solved));
                  super.setState(state);
                }
            ...
            if (gcodeDriver.isRemoveComments()) {
              solutions.add(new Solutions.Issue(gcodeDriver, "Keep G-code comments for better debugging.", "Disable Remove Comments.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#gcodedriver-new-settings") {
                @Override public void setState(Solutions.State state) throws Exception {
                  gcodeDriver.setRemoveComments((state != Solutions.State.Solved));
                  super.setState(state);
                }
            ...
            if (axis instanceof ReferenceControllerAxis) {
              if (firmwarePrimaryAxesCount != null && firmwarePrimaryAxesCount == firmwareAxesCount) {
                if (axis.isRotationalOnController()) {
                  ...
                  solutions.add(new Solutions.Issue(axis, "Axis should be treated as a linear for detected firmware (all-primary axes mode).", (oldInvertLinearRotational ? "Disable" : "Enable") + " Switch Linear \u2194 Rotational.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Machine-Axes#controller-settings") {
                    @Override public void setState(Solutions.State state) throws Exception {
                      ((ReferenceControllerAxis) axis).setInvertLinearRotational((state == Solutions.State.Solved) ^ oldInvertLinearRotational);
                      super.setState(state);
                    }
                  });
                }
              } else {
                if ("XYZABC".contains(axis.getLetter())) {
                  boolean rotational = "ABC".contains(axis.getLetter());
                  if (rotational ^ axis.isRotationalOnController()) {
                    final boolean oldInvertLinearRotational = ((ReferenceControllerAxis) axis).isInvertLinearRotational();
                    solutions.add(new Solutions.Issue(axis, "Axis should be treated as " + (rotational ? "rotational" : "linear") + " according to its letter.", (oldInvertLinearRotational ? "Disable" : "Enable") + " Switch Linear \u2194 Rotational.", Severity.Warning, "https://github.com/openpnp/openpnp/wiki/Machine-Axes#controller-settings-rotational-axis") {
                      @Override public void setState(Solutions.State state) throws Exception {
                        ((ReferenceControllerAxis) axis).setInvertLinearRotational((state == Solutions.State.Solved) ^ oldInvertLinearRotational);
                        super.setState(state);
                      }
        ...
        if (lettersOk) {
          for (CommandType commandType : gcodeDriver.isSpeakingGcode() ? CommandType.values() : new CommandType[] { CommandType.CONNECT_COMMAND }) {
            ...
            switch (commandType) {
              ...
              if (command == null) {
                ...
                if (isTinyG) {
                  ...
                  if (command.contains("$ex=2")) {
                    ...
                    if (command.contains("$ex=0")) {
                      commandBuilt = command.replace("$ex=0", "$ex=1");
                    } else {
                      if (!command.contains("$ex=1")) {
                        commandBuilt = "$ex=1\n" + command;
                      }
                    }
                  }
              ...
              if (command == null && gcodeDriver.getCommand(null, CommandType.MOVE_TO_COMPLETE_REGEX) == null) {
              ...
              if (hasAxes) {
                ...
                commandBuilt += ".*";
                if (gcodeDriver.getReportedAxes() != null && !gcodeDriver.getReportedAxes().matches(commandBuilt)) {
            ...
            suggestGcodeCommand(gcodeDriver, null, solutions, commandType, commandBuilt, commandModified, disallowHeadMountables);
          }
        }
        for (Command command : gcodeDriver.commands) {
          ...
          Command readCommand = gcodeDriver.getExactCommand(command.headMountableId, CommandType.ACTUATOR_READ_COMMAND);
          if (readCommand == null) {
            continue;
          }
          ...
          solutions.add(new Solutions.Issue(gcodeDriver, "Both ACTUATOR_READ_COMMAND and ACTUATOR_READ_WITH_DOUBLE_COMMAND are set for " + headMountable + " but the latter is deprecated", "Accept to replace ACTUATOR_READ_COMMAND with ACTUATOR_READ_WITH_DOUBLE_COMMAND. Dismiss to remove read with double.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/GcodeDriver%3A-Command-Reference#actuator_read_command") {
            @Override public void setState(Solutions.State state) throws Exception {
              ...
              if (state.equals(Solutions.State.Solved)) {
                ...
                if (state.equals(Solutions.State.Dismissed)) {
  ...
  /**
     ...
     */
  public static void suggestGcodeCommand(GcodeDriver gcodeDriver, HeadMountable headMountable, Solutions solutions, CommandType commandType, String suggestedCommand, boolean commandModified, boolean disallowHeadMountables) {
      ...
      for (Head head : Configuration.get().getMachine().getHeads()) {
        for (HeadMountable hm : head.getHeadMountables()) {
          ...
          if (commandHeadMountable != null) {
            solutions.add(new Solutions.Issue(gcodeDriver, hm.getClass().getSimpleName() + " " + hm.getName() + " " + commandType + " obsolete with Letter Variables.", "Remove the command.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Advanced-Motion-Control#migration-from-a-previous-version") {
              @Override public void setState(Solutions.State state) throws Exception {
  ...
  public static void convertToAsync(GcodeDriver gcodeDriver) throws Exception {
    ...
    gcodeDriverSerialized.replace(gcodeDriver.getClass().getCanonicalName(), GcodeAsyncDriver.class.getCanonicalName());
  ...
  public static void convertToPlain(GcodeAsyncDriver asyncDriver) throws Exception {
    ...
    String gcodeDriverSerialized = sw.toString();
    gcodeDriverSerialized.replace(asyncDriver.getClass().getCanonicalName(), GcodeAsyncDriver.class.getCanonicalName());
