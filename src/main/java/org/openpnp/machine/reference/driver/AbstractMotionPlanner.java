...
public abstract class AbstractMotionPlanner extends AbstractModelObject implements MotionPlanner, PropertySheetHolder {
  @Attribute(required = false) private double maximumPlanHistory = 60;
  ...
  @Override public synchronized void home() throws Exception {
  ...
  @Override public synchronized void setGlobalOffsets(AxesLocation axesLocation) throws Exception {
    executeMotionPlan(CompletionType.CommandStillstand);
    AxesLocation driverLocation = new AxesLocation(axesLocation.getControllerAxes(), (a) -> axesLocation.getLengthCoordinate(a).add(lastDirectionalBacklashOffset.getLengthCoordinate(a)));
  ...
  @Override public void moveTo(HeadMountable hm, AxesLocation axesLocation, double speed, MotionOption... options) throws Exception {
    axesLocation = limitAxesLocation(axesLocation, false);
    AxesLocation currentLocation = new AxesLocation(getMachine());
    AxesLocation newLocation = currentLocation.put(axesLocation);
    ...
    newLocation = currentLocation.put(axesLocation);
  ...
  /**
     ...
     * @throws Exception
     */
  protected void interlockMotionAcrossDrivers(HeadMountable hm, AxesLocation currentLocation, AxesLocation newLocation) throws Exception {
    AxesLocation segment = currentLocation.motionSegmentTo(newLocation);
    List<Driver> drivers = segment.getAxesDrivers(getMachine());
    if (drivers.size() > 0) {
      if (lastPlannedDrivers.size() > 0 && (drivers.size() > 1 || lastPlannedDrivers.size() > 1 || (drivers.size() == 1 && lastPlannedDrivers.size() == 1 && drivers.get(0) != lastPlannedDrivers.get(0)))) {
  ...
  /**
     ...
     * @return
     */
  protected synchronized AxesLocation createBacklashCompensatedMotion(HeadMountable hm, double speed, AxesLocation currentLocation, AxesLocation newLocation, MotionOption... options) {
    ...
    for (ControllerAxis axis : currentLocation.getControllerAxes()) {
      if (segment.contains(axis)) {
        if (axis instanceof ReferenceControllerAxis) {
          ...
          if (backlashOffset.getValue() != 0) {
            if (refAxis.getBacklashCompensationMethod().isDirectionalMethod()) {
              ...
              Length effectiveBacklashOffset = Math.signum(axisSegment.getValue()) == Math.signum(backlashOffset.getValue()) ? backlashOffset : new Length(0, LengthUnit.Millimeters);
              ...
              if (refAxis.getBacklashCompensationMethod() == BacklashCompensationMethod.DirectionalSneakUp) {
                ...
                backlashCompensatedNewLocation = backlashCompensatedNewLocation.subtract(new AxesLocation(refAxis, sneakOffset));
                ...
                if (Math.signum(displacement.getCoordinate(refAxis)) != Math.signum(axisSegment.getValue())) {
                  backlashCompensatedNewLocation = backlashCompensatedNewLocation.put(new AxesLocation(refAxis, backlashCompensatedCurrentLocation.getLengthCoordinate(refAxis)));
                }
                needsExtraBacklashMove = true;
                backlashCompensatedSpeed = Math.min(backlashCompensatedSpeed, refAxis.getBacklashSpeedFactor());
              } else {
                backlashCompensatedNewLocation = backlashCompensatedNewLocation.add(effectiveBacklashAxisOffset);
              }
              newLocation = newLocation.add(effectiveBacklashAxisOffset);
              lastDirectionalBacklashOffset = lastDirectionalBacklashOffset.put(effectiveBacklashAxisOffset);
            } else {
              lastDirectionalBacklashOffset = lastDirectionalBacklashOffset.put(new AxesLocation(refAxis, 0));
              if (!Motion.MotionOption.SpeedOverPrecision.isSetIn(optionFlags)) {
                if (refAxis.getBacklashCompensationMethod() == BacklashCompensationMethod.OneSidedPositioning || (refAxis.getBacklashCompensationMethod() == BacklashCompensationMethod.OneSidedOptimizedPositioning && Math.signum(segment.getCoordinate(refAxis)) == Math.signum(backlashOffset.getValue()))) {
                  backlashCompensatedNewLocation = backlashCompensatedNewLocation.add(new AxesLocation(axis, backlashOffset));
                  needsExtraBacklashMove = true;
                  backlashCompensatedSpeed = Math.min(backlashCompensatedSpeed, refAxis.getBacklashSpeedFactor());
                }
              }
        ...
        backlashCompensatedNewLocation = backlashCompensatedNewLocation.add(new AxesLocation(axis, lastDirectionalBacklashOffset.getLengthCoordinate(axis)));
        newLocation = newLocation.add(new AxesLocation(axis, lastDirectionalBacklashOffset.getLengthCoordinate(axis)));
      }
    }
    if (needsExtraBacklashMove) {
      addMotion(hm, speed, backlashCompensatedCurrentLocation, backlashCompensatedNewLocation, optionFlags);
      addMotion(hm, backlashCompensatedSpeed, backlashCompensatedNewLocation, newLocation, optionFlags);
    } else {
      addMotion(hm, speed, backlashCompensatedCurrentLocation, newLocation, optionFlags);
    }
    return newLocation;
  }

  protected Motion addMotion(HeadMountable hm, double speed, AxesLocation location0, AxesLocation location1, int options) {
    Motion motionCommand = new Motion(hm, location0, location1, speed, options);
  ...
  /**
     ...
     * @throws Exception
     */
  protected boolean executeMoveTo(ReferenceMachine machine, ReferenceHeadMountable hm, Motion plannedMotion, boolean firstAfterCoordination) throws Exception {
  ...
  /**
     ...
     * @throws Exception
     */
  protected AxesLocation limitAxesLocation(AxesLocation axesLocation, boolean silent) throws Exception {
        ...
        if (refAxis.getType() == Axis.Type.Rotation) {
          if (refAxis.isWrapAroundRotation()) {
            ...
            if (refAxis.isLimitRotation()) {
              axesLocation = axesLocation.put(new AxesLocation(refAxis, Utils2D.normalizeAngle180(axesLocation.getCoordinate(refAxis))));
            }
          }
        } else {
          ...
          if (refAxis.isSoftLimitLowEnabled()) {
            ...
            if (coordinate.getValue() < limit.getValue() && !refAxis.coordinatesMatch(limit, coordinate)) {
              ...
              throw new Exception(String.format("Can\'t move %s to %s, lower than soft limit %s.", refAxis.getName(), coordinate, limit));
            }
          }
          if (refAxis.isSoftLimitHighEnabled()) {
            ...
            if (coordinate.getValue() > limit.getValue() && !refAxis.coordinatesMatch(limit, coordinate)) {
              ...
              throw new Exception(String.format("Can\'t move %s to %s, higher than soft limit %s.", refAxis.getName(), coordinate, limit));
            }
          }
        }
      }
  ...
  @Override public boolean isValidLocation(AxesLocation axesLocation) {
    try {
  ...
  @Override public synchronized Motion getMomentaryMotion(double time) {
    ...
    if (entry1 != null) {
      ...
      Motion motion = new Motion(null, currentLocation, currentLocation, 1.0, MotionOption.Stillstand);
  ...
  @Override public void waitForCompletion(HeadMountable hm, CompletionType completionType) throws Exception {
    ...
    if (completionType.isEnforcingStillstand()) {
      ...
      AxesLocation reportedLocation = new AxesLocation(getMachine().getAxes(), (axis) -> ((axis instanceof ControllerAxis) ? ((ControllerAxis) axis).getDriverLengthCoordinate().subtract(lastDirectionalBacklashOffset.getLengthCoordinate(axis)) : null));
      AxesLocation currentLocation = new AxesLocation(reportedLocation.getControllerAxes(), (axis) -> axis.getLengthCoordinate());
      ...
      if (axis instanceof ReferenceControllerAxis) {
        ...
        if (refAxis.isLimitRotation() && refAxis.isWrapAroundRotation()) {
          ...
          if (anglePresent != angleWrappedAround) {
            refAxis.getDriver().setGlobalOffsets(getMachine(), new AxesLocation(refAxis, angleWrappedAround));
            refAxis.setLengthCoordinate(refAxis.getDriverLengthCoordinate().subtract(lastDirectionalBacklashOffset.getLengthCoordinate(refAxis)));
          }
        }
      }
    }
  ...
  protected void waitForDriverCompletion(HeadMountable hm, CompletionType completionType) throws Exception {
    ReferenceMachine machine = getMachine();
    AxesLocation mappedAxes = (hm != null ? hm.getMappedAxes(machine) : new AxesLocation(machine));
  ...
  @Override public synchronized void clearMotionPlanOlderThan(double time) {
