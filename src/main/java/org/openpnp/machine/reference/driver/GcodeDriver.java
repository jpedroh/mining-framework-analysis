...
@Root public class GcodeDriver extends AbstractReferenceDriver implements Named {
  public enum CommandType {
    ...
    ENABLE_COMMAND,
    DISABLE_COMMAND,
    @Deprecated POST_VISION_HOME_COMMAND,
    HOME_COMMAND("Id", "Name"),
    HOME_COMPLETE_REGEX,
    SET_GLOBAL_OFFSETS_COMMAND("Id", "Name", "X", "Y", "Z", "Rotation"),
    GET_POSITION_COMMAND,
    @Deprecated PUMP_ON_COMMAND,
    @Deprecated PUMP_OFF_COMMAND,
    MOVE_TO_COMMAND(true, "Id", "Name", "FeedRate", "X", "Y", "Z", "Rotation"),
    MOVE_TO_COMPLETE_COMMAND(true),
    MOVE_TO_COMPLETE_REGEX(true),
    @Deprecated PICK_COMMAND(true, "Id", "Name", "VacuumLevelPartOn", "VacuumLevelPartOff"),
    @Deprecated PLACE_COMMAND(true, "Id", "Name"),
    ...
    @Deprecated ACTUATOR_READ_WITH_DOUBLE_COMMAND(true, "Id", "Name", "Index", "DoubleValue", "IntegerValue"),
  ...
  public static class Command {
    @Attribute(required = false) public String headMountableId;

    @Attribute(required = true) public CommandType type;

    @ElementList(required = false, inline = true, entry = "text", data = true) public ArrayList<String> commands = new ArrayList<>();
  ...
  @Attribute(required = false) protected LengthUnit units = LengthUnit.Millimeters;

  @Attribute(required = false) protected int maxFeedRate = 1000;

  @Deprecated @Attribute(required = false) protected double backlashOffsetX = -1;

  @Deprecated @Attribute(required = false) protected double backlashOffsetY = -1;

  @Deprecated @Attribute(required = false) protected double backlashOffsetZ = 0;

  @Deprecated @Attribute(required = false) protected double backlashOffsetR = 0;

  @Deprecated @Attribute(required = false) protected double nonSquarenessFactor = 0;

  @Deprecated @Attribute(required = false) protected double backlashFeedRateFactor = 0.1;

  @Attribute(required = false) protected int timeoutMilliseconds = 5000;

  @Attribute(required = false) protected int connectWaitTimeMilliseconds = 3000;

  @Attribute(required = false) protected int dollarWaitTimeMilliseconds = 50;

  @Deprecated @Attribute(required = false) protected boolean visualHomingEnabled = true;

  @Attribute(required = false) protected boolean backslashEscapedCharactersEnabled = false;

  @Attribute(required = false) protected boolean removeComments;

  @Attribute(required = false) protected boolean compressGcode;

  @Attribute(required = false) protected boolean loggingGcode;

  @Deprecated @Element(required = false) protected Location homingFiducialLocation = new Location(LengthUnit.Millimeters);

  @Attribute(required = false) boolean supportingPreMove = false;

  @Attribute(required = false) boolean usingLetterVariables = true;

  @Attribute(required = false) int infinityTimeoutMilliseconds = 60000;

  @Element(required = false, data = true) String detectedFirmware = null;

  @Element(required = false, data = true) String reportedAxes = null;

  @Element(required = false, data = true) String configuredAxes = null;

  @ElementList(required = false, inline = true) public ArrayList<Command> commands = new ArrayList<>();

  @Deprecated @ElementList(required = false) protected List<GcodeDriver> subDrivers = null;

  @Deprecated @ElementList(required = false) protected List<Axis> axes = null;
  ...
  @Override public void createDefaults() throws Exception {
  ...
  @Override public void setEnabled(boolean enabled) throws Exception {
  ...
  @Override public void home(Machine machine) throws Exception {
    ...
    if (isUsingLetterVariables()) {
      AxesLocation axesHomeLocation = new AxesLocation(machine, (axis) -> (axis.getHomeCoordinate()));
  ...
  @Override public void setGlobalOffsets(Machine machine, AxesLocation axesLocation) throws Exception {
    ...
    if (command != null) {
      ...
      if (postVisionHomeCommand != null && axisX != null && axisY != null) {
        postVisionHomeCommand = substituteVariable(postVisionHomeCommand, "X", axesLocation.getCoordinate(axisX, getUnits()));
        postVisionHomeCommand = substituteVariable(postVisionHomeCommand, "Y", axesLocation.getCoordinate(axisY, getUnits()));
  ...
  @Override public AxesLocation getReportedLocation(long timeout) throws Exception {
  ...
  public Command getExactCommand(String headMountableId, CommandType type) {
    for (Command c : commands) {
      ...
      if (c.headMountableId != null && c.headMountableId.equals(headMountableId)) {
        ...
        if (c.headMountableId == null && headMountableId == null) {
          return c;
        }
      }
    }
  ...
  @Override public void moveTo(HeadMountable hm, MoveToCommand move) throws Exception {
    ...
    if (command == null) {
      ...
      if (isSupportingPreMove()) {
        throw new Exception(getName() + " MOVE_TO_COMMAND missing for " + hm.getClass().getSimpleName() + " " + hm.getName() + ", please set the command in the driver (no automatic support by Issues & Solutions due to driver Allow Pre-Move Commands option).");
      } else {
    ...
    if (hasVariable(command, "BacklashFeedRate")) {
      throw new Exception(getName() + " configuration upgrade needed: Please remove the extra backlash compensation move from your MOVE_TO_COMMAND. " + "Backlash compensation is now done outside of the drivers and configured on the axes.");
    }
    ...
    AxesLocation mappedAxes = (this.usingLetterVariables ? allAxesLocation : hm.getMappedAxes(machine)).drivenBy(this);
    ...
    for (String variable : getAxisVariables(machine)) {
      ...
      if (axis != null) {
        ...
        command = substituteVariable(command, variable + "JerkMupm3", axisJerk > 4.63 ? axisJerk * 1e-6 * Math.pow(60, 3) : null);
        axis.setDriverCoordinate(coordinate);
      } else {
        command = substituteVariable(command, variable, null);
  ...
  @Override public boolean isMotionPending() {
  ...
  @Override public void waitForCompletion(HeadMountable hm, CompletionType completionType) throws Exception {
  ...
  @Override public void actuate(Actuator actuator, boolean on) throws Exception {
  ...
  @Override public void actuate(Actuator actuator, double value) throws Exception {
  ...
  @Override public void actuate(Actuator actuator, String value) throws Exception {
  ...
  @Override public String actuatorRead(Actuator actuator, Object parameter) throws Exception {
  ...
  @Override public String actuatorRead(Actuator actuator) throws Exception {
  ...
  protected long getTimeoutAtMachineSpeed() {
    return timeoutMilliseconds == -1 ? timeoutMilliseconds : Math.round(timeoutMilliseconds / Math.max(0.05, Configuration.get().getMachine().getSpeed()));
  }
  ...
  protected Line waitForConfirmation(String command, long timeout) throws Exception {
  ...
  @FunctionalInterface public interface TimeoutAction {
    List<Line> apply(List<Line> responses) throws Exception;
  }

  public List<Line> receiveResponses(String regex, long timeout, TimeoutAction timeoutAction) throws Exception {
    if (timeout == -1) {
      timeout = infinityTimeoutMilliseconds;
    }
  ...
  protected boolean processPositionReport(Line line) {
    ...
    Matcher matcher = Pattern.compile(regex).matcher(line.getLine());
    matcher.matches();
    ...
    if (motionPending) {
      Logger.warn("Position report cannot be processed when motion might still be pending. Missing Machine Coordination on Actuators?", position);
    } else {
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(super.getConfigurationWizard()), new PropertySheetWizardAdapter(new GcodeDriverSettings(this), Translations.getString("GCodeDriver.GCodeDriverSettings.title")), new PropertySheetWizardAdapter(new GcodeDriverGcodes(this), Translations.getString("GCodeDriver.GCode.title")), new PropertySheetWizardAdapter(new GcodeDriverConsole(this), Translations.getString("GCodeDriver.Console.title")) };
  }

  @Override public LengthUnit getUnits() {
    return units;
  }
  ...
  @Override public Length getFeedRatePerSecond() {
  ...
  @Override public boolean isSupportingPreMove() {
  ...
  public List<String> getReportedAxesLetters() {
    ...
    while (m.find()) {
      ...
      if (!reportedLetters.contains(letter) && (!letter.equals("E") || reportedLetters.contains("A"))) {
  ...
  @Deprecated public Axis getLegacyAxis(HeadMountable hm, Type type) {
    if (axes != null) {
      for (Axis axis : axes) {
        if (axis.type == type && (axis.headMountableIds.contains("*") || axis.headMountableIds.contains(hm.getId()))) {
          return axis;
        }
      }
    }
  ...
  @Deprecated protected ReferenceControllerAxis migrateAxis(ReferenceMachine machine, Axis legacyAxis) throws Exception {
    ...
    switch (axis.getType()) {
      case X:
      axis.setBacklashCompensationMethod(backlashOffsetX != 0 ? BacklashCompensationMethod.OneSidedPositioning : BacklashCompensationMethod.None);
      ...
      break;
      case Y:
      axis.setBacklashCompensationMethod(backlashOffsetY != 0 ? BacklashCompensationMethod.OneSidedPositioning : BacklashCompensationMethod.None);
      ...
      break;
      case Z:
      axis.setBacklashCompensationMethod(backlashOffsetZ != 0 ? BacklashCompensationMethod.OneSidedPositioning : BacklashCompensationMethod.None);
      axis.setBacklashOffset(new Length(backlashOffsetZ, getUnits()));
      axis.setBacklashSpeedFactor(backlashFeedRateFactor);
      break;
      case Rotation:
      axis.setBacklashCompensationMethod(backlashOffsetR != 0 ? BacklashCompensationMethod.OneSidedPositioning : BacklashCompensationMethod.None);
  ...
  @Deprecated protected AbstractTransformedAxis migrateAxis(ReferenceMachine machine, ReferenceControllerAxis inputAxis, AxisTransform legacyAxis) throws Exception {
    if (legacyAxis instanceof CamTransform) {
      ...
      if (legacyAxis instanceof NegatingTransform) {
        ...
        if (legacyAxis instanceof ScalingTransform) {
          ReferenceMappedAxis axisTransform = new ReferenceMappedAxis();
          ...
          if (legacyAxis instanceof OffsetTransform) {
  ...
  @Deprecated @Override public void migrateDriver(Machine machine) throws Exception {
    machine.addDriver(this);
    if (machine instanceof ReferenceMachine) {
      this.usingLetterVariables = false;
      if (machine.getDrivers().size() > 1 && getName().equals("GcodeDriver")) {
      ...
      for (Head head : machine.getHeads()) {
        if (visualHomingEnabled) {
          ...
          homingFiducialLocation = homingFiducialLocation.subtract(new Location(homingFiducialLocation.getUnits(), homingFiducialLocation.getY() * nonSquarenessFactor, 0, 0, 0));
        ...
        for (Actuator actuator : head.getActuators()) {
          if (getCommand(actuator, CommandType.ACTUATE_BOOLEAN_COMMAND) != null || getCommand(actuator, CommandType.ACTUATE_DOUBLE_COMMAND) != null || getCommand(actuator, CommandType.ACTUATOR_READ_COMMAND) != null || getCommand(actuator, CommandType.ACTUATOR_READ_WITH_DOUBLE_COMMAND) != null || getCommand(actuator, CommandType.ACTUATOR_READ_REGEX) != null) {
  ...
  @Deprecated protected void migrateAssignAxis(Axis legacyAxis, AbstractAxis axis, HeadMountable hm) {
    ...
    if (legacyAxis.transform != null) {
      if (legacyAxis.transform instanceof CamTransform) {
        ...
        if (legacyAxis.transform instanceof NegatingTransform) {
    ...
    if (getLegacyAxis(hm, Type.Rotation) == legacyAxis) {
      ...
      if (hm instanceof ReferenceNozzle && assignAxis instanceof ReferenceControllerAxis) {
        ((ReferenceControllerAxis) assignAxis).setLimitRotation(((ReferenceNozzle) hm).isLimitRotation());
      }
    }
  }

  @Deprecated private static class Axis {
    @Attribute private String name;

    @Attribute private Type type;

    @Attribute(required = false) private double homeCoordinate = 0;

    @ElementList(required = false) private Set<String> headMountableIds = new HashSet<String>();

    @Element(required = false) private AxisTransform transform;

    @Element(required = false, data = true) private String preMoveCommand;

    private Axis() {
    }
  }

  @Deprecated public interface AxisTransform {
  }

  @Deprecated public static class NegatingTransform implements AxisTransform {
    @Element private String negatedHeadMountableId;
  }

  @Deprecated public static class CamTransform implements AxisTransform {
    @Element private String negatedHeadMountableId;

    @Attribute(required = false) private double camRadius = 24;

    @Attribute(required = false) private double camWheelRadius = 9.5;

    @Attribute(required = false) private double camWheelGap = 2;
  }

  @Deprecated public static class OffsetTransform implements AxisTransform {
    @ElementMap(required = false) HashMap<String, Double> offsetsByHeadMountableId = new HashMap<>();

    public OffsetTransform() {
    }
  }

  @Deprecated public static class ScalingTransform implements AxisTransform {
    @Attribute(required = false) private double scaleFactor = 1;
  }
}