...
@Root public class GcodeDriver extends AbstractReferenceDriver implements Named, Runnable {
  ...
  @Attribute(required = false) protected double backlashOffsetX = -1;

  @Attribute(required = false) protected double backlashOffsetY = -1;

  @Attribute(required = false) protected double nonSquarenessFactor = 0;

  @Attribute(required = false) protected double backlashFeedRateFactor = 0.1;
  ...
  @Attribute(required = false) protected boolean visualHomingEnabled = true;

  @Element(required = false) protected Location homingFiducialLocation = new Location(LengthUnit.Millimeters);

  @ElementList(required = false, inline = true) public ArrayList<Command> commands = new ArrayList<>();

  @ElementList(required = false) protected List<GcodeDriver> subDrivers = new ArrayList<>();

  @ElementList(required = false) protected List<Axis> axes = new ArrayList<>();

  @Attribute(required = false) protected String name = "GcodeDriver";
  ...
  @Override public void dispense(ReferencePasteDispenser dispenser, Location startLocation, Location endLocation, long dispenseTimeMilliseconds) throws Exception {
  ...
  @Override public void home(ReferenceHead head) throws Exception {
    ...
    if (visualHomingEnabled) {
      ...
      if (homePart != null) {
        Configuration.get().getMachine().getFiducialLocator().getHomeFiducialLocation(homingFiducialLocation, homePart);
        if (xAxis != null) {
  ...
  public Axis getAxis(HeadMountable hm, Axis.Type type) {
    for (Axis axis : axes) {
      if (axis.getType() == type && (axis.getHeadMountableIds().contains("*") || axis.getHeadMountableIds().contains(hm.getId()))) {
        return axis;
      }
    }
    return null;
  ...
  @Override public Location getLocation(ReferenceHeadMountable hm) {
    ...
    Location location = new Location(units, xAxis == null ? 0 : xAxis.getTransformedCoordinate(hm), yAxis == null ? 0 : yAxis.getTransformedCoordinate(hm), zAxis == null ? 0 : zAxis.getTransformedCoordinate(hm), rotationAxis == null ? 0 : rotationAxis.getTransformedCoordinate(hm)).add(hm.getHeadOffsets());
    return location;
  }

  @Override public void moveTo(ReferenceHeadMountable hm, Location location, double speed) throws Exception {
    ...
    if (xAxis != null || yAxis != null || zAxis != null || rotationAxis != null) {
      ...
      if (includeX) {
        ...
        command = substituteVariable(command, "BacklashOffsetX", x + backlashOffsetX + nonSquarenessFactor * y);
        ...
        command = substituteVariable(command, "BacklashOffsetX", null);
      }
      if (includeY) {
        command = substituteVariable(command, "Y", y);
        command = substituteVariable(command, "YF", y);
        command = substituteVariable(command, "BacklashOffsetY", y + backlashOffsetY);
        ...
        command = substituteVariable(command, "BacklashOffsetY", null);
      }
      if (includeZ) {
  ...
  @Override public void pick(ReferenceNozzle nozzle) throws Exception {
  ...
  @Override public void place(ReferenceNozzle nozzle) throws Exception {
    ReferenceNozzleTip nt = nozzle.getNozzleTip();
  ...
  public List<String> sendCommand(String command, long timeout) throws Exception {
    ...
    while (System.currentTimeMillis() - t < timeout) {
      String response = responseQueue.poll(timeout - (System.currentTimeMillis() - t), TimeUnit.MILLISECONDS);
      if (response == null) {
        continue;
      }
    ...
    responseQueue.drainTo(responses);
    Logger.debug("sendCommand({} {}, {}) => {}", new Object[] { getCommunications().getConnectionName(), command, timeout == Long.MAX_VALUE ? -1 : timeout, responses });
    return responses;
  }
  ...
  private boolean processPositionReport(String line) {
    ...
    Matcher matcher = Pattern.compile(getCommand(null, CommandType.POSITION_REPORT_REGEX)).matcher(line);
    matcher.matches();
    for (Axis axis : axes) {
      try {
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(new GcodeDriverGcodes(this), "Gcode"), new PropertySheetWizardAdapter(new GcodeDriverSettings(this), "General Settings"), new PropertySheetWizardAdapter(new GcodeDriverConsole(this), "Console"), new PropertySheetWizardAdapter(super.getConfigurationWizard(), "Communications") };
  }
  ...
  public Action deleteSubDriverAction = new AbstractAction() {
    ...
    @Override public void actionPerformed(ActionEvent arg0) {
      int ret = JOptionPane.showConfirmDialog(MainFrame.get(), "Are you sure you want to delete the selected sub-driver?", "Delete Sub-Driver?", JOptionPane.YES_NO_OPTION);
  ...
  public static class NegatingTransform implements AxisTransform {
    ...
    @Override public double toTransformed(Axis axis, HeadMountable hm, double rawCoordinate) {
    ...
    @Override public double toRaw(Axis axis, HeadMountable hm, double transformedCoordinate) {
