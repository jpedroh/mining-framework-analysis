...
public class ReferencePushPullFeeder extends ReferenceFeeder {
  @Attribute(required = false) protected Double rotationInFeeder = new Double(0.0);

  @Attribute(required = false) protected boolean normalizePickLocation = true;

  @Attribute(required = false) protected boolean snapToAxis = true;

  @Element(required = false) protected Location hole1Location = new Location(LengthUnit.Millimeters);

  @Element(required = false) protected Location hole2Location = new Location(LengthUnit.Millimeters);

  @Attribute(required = false) protected boolean usedAsTemplate = false;

  @Element protected Location feedStartLocation = new Location(LengthUnit.Millimeters);

  @Element(required = false) protected Location feedMid1Location = new Location(LengthUnit.Millimeters);

  @Element(required = false) protected Location feedMid2Location = new Location(LengthUnit.Millimeters);

  @Element(required = false) protected Location feedMid3Location = new Location(LengthUnit.Millimeters);

  @Element protected Location feedEndLocation = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Length partPitch = new Length(4, LengthUnit.Millimeters);

  @Element(required = false) private Length feedPitch = new Length(4, LengthUnit.Millimeters);

  @Attribute(required = false) private long feedMultiplier = 1;

  @Element(required = false) protected double feedSpeedPush1 = 1.0;

  @Attribute(required = false) protected double feedSpeedPush2 = 1.0;

  @Attribute(required = false) protected double feedSpeedPush3 = 1.0;

  @Attribute(required = false) protected double feedSpeedPushEnd = 1.0;

  @Attribute(required = false) protected double feedSpeedPull3 = 1.0;

  @Attribute(required = false) protected double feedSpeedPull2 = 1.0;

  @Attribute(required = false) protected double feedSpeedPull1 = 1.0;

  @Attribute(required = false) protected double feedSpeedPull0 = 1.0;

  @Attribute(required = false) protected boolean includedPush1 = false;

  @Attribute(required = false) protected boolean includedPush2 = false;

  @Attribute(required = false) protected boolean includedPush3 = false;

  @Attribute(required = false) protected boolean includedPushEnd = true;

  @Attribute(required = false) protected boolean includedMulti0 = true;

  @Attribute(required = false) protected boolean includedMulti1 = false;

  @Attribute(required = false) protected boolean includedMulti2 = false;

  @Attribute(required = false) protected boolean includedMulti3 = false;

  @Attribute(required = false) protected boolean includedMultiEnd = true;

  @Attribute(required = false) protected boolean includedPull0 = true;

  @Attribute(required = false) protected boolean includedPull1 = false;

  @Attribute(required = false) protected boolean includedPull2 = false;

  @Attribute(required = false) protected boolean includedPull3 = false;

  @Attribute(required = false) protected String actuatorName;

  @Attribute(required = false) protected String peelOffActuatorName;

  @Attribute(required = false) private long feedCount = 0;

  @Element(required = false) private CvPipeline pipeline = createDefaultPipeline();

  @Attribute(required = false) protected String ocrFontName = "Liberation Mono";

  @Attribute(required = false) protected double ocrFontSizePt = 7.0;

  @Element(required = false) protected RegionOfInterest ocrRegion = null;
  ...
  @Attribute(required = false) protected OcrWrongPartAction ocrWrongPartAction = OcrWrongPartAction.SwapOrCreate;

  @Attribute(required = false) protected boolean ocrDiscoverOnJobStart = true;

  @Attribute(required = false) protected boolean ocrStopAfterWrongPart = false;

  @Element(required = false) private Length precisionWanted = new Length(0.1, LengthUnit.Millimeters);

  @Attribute(required = false) private int calibrationCount = 0;

  @Element(required = false) private Length sumOfErrors = new Length(0, LengthUnit.Millimeters);

  @Element(required = false) private Length sumOfErrorSquares = new Length(0, LengthUnit.Millimeters);

  @Attribute(required = false) private double calibrationToleranceMm = 1.95;

  @Attribute(required = false) private double sprocketHoleToleranceMm = 0.6;

  @Attribute(required = false) private double rowLocationToleranceMm = 4.0;

  @Attribute(required = false) private double rowZLocationToleranceMm = 1.0;

  @Attribute(required = false) private int calibrateMaxPasses = 3;

  @Attribute(required = false) private double calibrateToleranceMm = 0.3;

  @Attribute(required = false) private int calibrateMinStatistic = 2;
  ...
  @Attribute(required = false) protected CalibrationTrigger calibrationTrigger = CalibrationTrigger.UntilConfident;
    ...
    Configuration.get().addListener(new ConfigurationListener.Adapter() {
      @Override public void configurationComplete(Configuration configuration) throws Exception {
        Configuration.get().getMachine().addListener(new MachineListener.Adapter() {
          @Override public void machineHeadActivity(Machine machine, Head head) {
            checkHomedState(machine);
          }

          @Override public void machineEnabled(Machine machine) {
            checkHomedState(machine);
          }
        });
      }
  ...
  public void assertCalibrated(boolean tapeFeed) throws Exception {
    ...
    if ((visionOffset == null && calibrationTrigger != CalibrationTrigger.None) || (tapeFeed && calibrationTrigger == CalibrationTrigger.UntilConfident && !isPrecisionSufficient()) || (tapeFeed && calibrationTrigger == CalibrationTrigger.OnEachTapeFeed)) {
  ...
  public boolean isPrecisionSufficient() {
    if (calibrationCount < calibrateMinStatistic) {
      ...
      if (getPrecisionConfidenceLimit().divide(getPrecisionWanted()) > 1.0) {
        return false;
      }
    }
    return true;
  ...
  @Override public void feed(Nozzle nozzle) throws Exception {
    ...
    if (getFeedCount() % getPartsPerFeedCycle() == 0) {
      assertCalibrated(false);
      Location feedStartLocation = getFeedStartLocation().derive(actuator.getLocation(), false, false, false, true).subtractWithRotation(getVisionOffset());
      Location feedMid1Location = getFeedMid1Location().derive(actuator.getLocation(), false, false, false, true).subtractWithRotation(getVisionOffset());
      Location feedMid2Location = getFeedMid2Location().derive(actuator.getLocation(), false, false, false, true).subtractWithRotation(getVisionOffset());
      Location feedMid3Location = getFeedMid3Location().derive(actuator.getLocation(), false, false, false, true).subtractWithRotation(getVisionOffset());
      Location feedEndLocation = getFeedEndLocation().derive(actuator.getLocation(), false, false, false, true).subtractWithRotation(getVisionOffset());
  ...
  public void ensureCameraZ(Camera camera) throws Exception {
    if (camera.isUnitsPerPixelAtZCalibrated() && !getLocation().getLengthZ().isInitialized()) {
  ...
  public void setActuatorName(String actuatorName) {
    ...
    firePropertyChange("actuatorName", oldValue == null ? "" : oldValue, actuatorName == null ? "" : actuatorName);
  }
  ...
  public void setPeelOffActuatorName(String actuatorName) {
    ...
    firePropertyChange("peelOffActuatorName", oldValue == null ? "" : oldValue, actuatorName == null ? "" : actuatorName);
  }
  ...
  public void addCalibrationError(Length error) {
    ...
    setCalibrationCount(getCalibrationCount() + 1);
  }

  public Length getPrecisionAverage() {
    return calibrationCount > 0 ? sumOfErrors.multiply(1.0 / calibrationCount) : new Length(0, LengthUnit.Millimeters);
  }
  ...
  public Length getPartsToSprocketHoleDistance() {
    return new Length(getLocation().getLinearDistanceToLineSegment(getHole1Location(), getHole2Location()), getLocation().getUnits());
  }
  ...
  public Location getPickLocation(long partInCycle, Location visionOffset) {
    ...
    Location feederLocation = new Location(partPitch.getUnits(), partPitch.multiply((double) offsetPitches).getValue(), 0, 0, getRotationInFeeder());
  ...
  public Location getNominalVisionLocation() throws Exception {
    if (!(hole1Location.isInitialized() && hole2Location.isInitialized())) {
      ...
      return getHole1Location().add(getHole2Location()).multiply(0.5).deriveLengths(null, null, getLocation().getLengthZ(), getLocation().getRotation() + getRotationInFeeder());
    }
  }
  ...
  public CvPipeline getCvPipeline(Camera camera, boolean clone, boolean performOcr, boolean autoSetup) {
    try {
      ...
      if (autoSetup) {
        ...
        range = camera.getWidth() > camera.getHeight() ? upp.getLengthY().multiply(camera.getHeight() / 2) : upp.getLengthX().multiply(camera.getWidth() / 2);
      } else {
        range = getHole1Location().getLinearLengthTo(getHole2Location()).multiply(0.5).add(new Length(sprocketHolePitchMm, LengthUnit.Millimeters));
      }
  ...
  private static CvPipeline createDefaultPipeline() {
    try {
      String xml = IOUtils.toString(BlindsFeeder.class.getResource("ReferencePushPullFeeder-DefaultPipeline.xml"));
  ...
  public class FindFeatures {
    ...
    private void drawOcrText(Mat mat, Color color) {
      if (detectedOcrModel != null) {
        Imgproc.putText(mat, detectedOcrModel.getText(), new org.opencv.core.Point(20, mat.rows() - 20), Imgproc.FONT_HERSHEY_PLAIN, 3, FluentCv.colorToScalar(Color.black), 6, 0, false);
        Imgproc.putText(mat, detectedOcrModel.getText(), new org.opencv.core.Point(20, mat.rows() - 20), Imgproc.FONT_HERSHEY_PLAIN, 3, FluentCv.colorToScalar(color), 2, 0, false);
      }
    }
    ...
    private void drawPartNumbers(Mat mat, Color color) {
      ...
      Size size = Imgproc.getTextSize(String.valueOf(getPartsPerFeedCycle()), Imgproc.FONT_HERSHEY_PLAIN, fontScale, 2, baseLine);
      Location textSizeMm = camera.getUnitsPerPixelAtZ().multiply(size.width, size.height, 0., 0.).convertToUnits(LengthUnit.Millimeters);
      ...
      if (textSizePitchCount < 0.75) {
        ...
        if (textSizePitchCount < 1.5) {
          step = 2;
        } else {
          if (textSizePitchCount < 4) {
            step = 5;
          } else {
            return;
          }
      ...
      for (int i = step; i <= getPartsPerFeedCycle(); i += step) {
        ...
        Location partLocation = getPickLocation(i, calibratedVisionOffset).convertToUnits(LengthUnit.Millimeters);
        ...
        textLocation = transformFeederToMachineLocation(textLocation, calibratedVisionOffset).convertToUnits(LengthUnit.Millimeters);
        ...
        if (p.x > 0 && p.x < camera.getWidth() && p.y > 0 && p.y < camera.getHeight()) {
          ...
          Imgproc.putText(mat, text, new org.opencv.core.Point(p.x + alignX, p.y + alignY), Imgproc.FONT_HERSHEY_PLAIN, fontScale, FluentCv.colorToScalar(color), 2, 0, false);
        }
      }
    }
    ...
    public FindFeatures invoke() throws Exception {
      ...
      try {
        ...
        if (calibrationTrigger == CalibrationTrigger.None) {
          ...
          final double sprocketHoleToPartMinMm = 3.5;
          final double sprocketHoleToPartGridMm = 2;
          ...
          resultsList = pipeline.getExpectedResult(VisionUtils.PIPELINE_RESULTS_NAME).getExpectedModel(List.class);
          ...
          for (Object result : resultsList) {
            if ((result) instanceof Result.Circle) {
              ...
              if ((result) instanceof RotatedRect) {
                ...
                if (Math.abs(rect.size.width * mmScale.getX() - sprocketHoleDiameterMm) < sprocketHoleToleranceMm && Math.abs(rect.size.height * mmScale.getX() - sprocketHoleDiameterMm) < sprocketHoleToleranceMm) {
          ...
          if (bestLine != null) {
            ...
            Collections.sort(holes, new Comparator<Result.Circle>() {
              @Override public int compare(Result.Circle o1, Result.Circle o2) {
            ...
            if (autoSetupMode == FindFeaturesMode.FromPickLocationGetHoles) {
              ...
              calibratedHole1Location = VisionUtils.getPixelLocation(camera, holes.get(0).x, holes.get(0).y).convertToUnits(LengthUnit.Millimeters);
              calibratedHole2Location = VisionUtils.getPixelLocation(camera, holes.get(1).x, holes.get(1).y).convertToUnits(LengthUnit.Millimeters);
              ...
              if (calibratedHole1Location.unitVectorTo(calibratedHole2Location).dotProduct(bestUnitVector).getValue() < 0.0) {
              ...
              calibratedPickLocation = camera.getLocation().derive(getLocation(), false, false, true, false).derive(null, null, null, angleTape);
            } else {
              for (Result.Circle hole : holes) {
                ...
                if (dist1Mm < calibrationToleranceMm && dist1Mm < dist2Mm) {
                  ...
                  if (dist2Mm < calibrationToleranceMm && dist2Mm < dist1Mm) {
              ...
              if (calibratedHole1Location == null || calibratedHole2Location == null) {
                ...
                if (snapToAxis) {
                  if (Math.abs(bestUnitVector.getX()) > Math.abs(bestUnitVector.getY()) * 5) {
                    ...
                    if (Math.abs(bestUnitVector.getY()) > Math.abs(bestUnitVector.getX()) * 5) {
                ...
                if (autoSetupMode == FindFeaturesMode.CalibrateHoles) {
                  ...
                  Location relativePickLocation = pickLocation.subtract(getHole1Location());
                  relativePickLocation = relativePickLocation.rotateXy(-pickLocation.getRotation()).derive(null, null, null, 0.0);
                  if (normalizePickLocation) {
                    relativePickLocation = new Location(LengthUnit.Millimeters, Math.round(relativePickLocation.getX() / partPitchMinMm) * partPitchMinMm, -sprocketHoleToPartMinMm + Math.round((relativePickLocation.getY() + sprocketHoleToPartMinMm) / sprocketHoleToPartGridMm) * sprocketHoleToPartGridMm, 0, 0);
                  }
                  calibratedPickLocation = calibratedHole1Location.add(relativePickLocation.rotateXy(angleTape)).derive(null, null, pickLocation.getZ(), angleTape);
                }
              }
            }
            if (calibratedHole1Location != null && calibratedPickLocation != null) {
              calibratedVisionOffset = getLocation().subtractWithRotation(calibratedPickLocation).derive(null, null, 0.0, null);
              Logger.debug("calibrated vision offset is: " + calibratedVisionOffset + ", length is: " + calibratedVisionOffset.getLinearLengthTo(Location.origin));
        ...
        if (showResultMilliseconds > 0) {
          ...
          resultMat.release();
          MainFrame.get().getCameraViews().getCameraView(camera).showFilteredImage(showResult, showResultMilliseconds);
        }
  ...
  public void autoSetup() throws Exception {
    ...
    try (CvPipeline pipeline = getCvPipeline(camera, true, true, true)) {
      pipeline.process();
      FindFeatures feature = new FindFeatures(camera, pipeline, 2000, FindFeaturesMode.FromPickLocationGetHoles).invoke();
  ...
  public Length getTapeWidth() {
    Location hole1Location = transformMachineToFeederLocation(getHole1Location(), null).convertToUnits(LengthUnit.Millimeters);
    final double partToSprocketHoleHalfTapeWidthDiffMm = 0.5;
  ...
  protected boolean isOnSameRowLocation(ReferencePushPullFeeder feederTemplate) {
    Location delta = getLocation().convertToUnits(LengthUnit.Millimeters).subtract(feederTemplate.getLocation());
  ...
  static boolean compatiblePartPackages(Part part1, Part part2) {
    ...
    if ((package1.getTapeSpecification() != null && !package1.getTapeSpecification().isEmpty()) && package1.getTapeSpecification().equals(package2.getTapeSpecification())) {
      return true;
    }
    return false;
  }
  ...
  public ReferencePushPullFeeder getTemplateFeeder(Part compatiblePart) {
    ...
    Collections.sort(list, new Comparator<ReferencePushPullFeeder>() {
      @Override public int compare(ReferencePushPullFeeder feeder1, ReferencePushPullFeeder feeder2) {
        int diff;
        ...
        return new Double(feeder1.getLocation().convertToUnits(LengthUnit.Millimeters).getLinearDistanceTo(getLocation())).compareTo(feeder2.getLocation().convertToUnits(LengthUnit.Millimeters).getLinearDistanceTo(getLocation()));
      }
    });
  ...
  public String getCloneTemplateStatus() {
    ...
    if (isUsedAsTemplate()) {
      ...
      if (templateFeeder != null) {
        ...
        if (part != null && compatiblePartPackages(part, templateFeeder.getPart())) {
          ...
          status += " selected as partial match.<br/>" + "<strong style=\"color:red\">Tape & reel specifications/packages are incompatible, settings must be reviewed.</strong>";
        }
      } else {
  ...
  public void smartClone(Part compatiblePart, boolean cloneLocationSettings, boolean cloneTapeSettings, boolean clonePushPullSettings, boolean cloneVisionSettings) throws Exception {
    ...
    cloneFeederSettings(cloneLocationSettings, cloneTapeSettings, clonePushPullSettings, cloneVisionSettings, templateFeeder);
  }

  public void cloneFeederSettings(boolean cloneLocationSettings, boolean cloneTapeSettings, boolean clonePushPullSettings, boolean cloneVisionSettings, ReferencePushPullFeeder templateFeeder) throws CloneNotSupportedException {
  ...
  protected void setFeederLocation(Location newTransform, boolean primary, boolean pushPull, boolean vision, ReferencePushPullFeeder templateFeeder) {
    ...
    if (vision) {
      if (templateFeeder.getOcrRegion() != null) {
        setOcrRegion(templateFeeder.getOcrRegion().rotateXy(newTransform.getRotation() - oldTransform.getRotation()));
      }
    }
    if (primary) {
  ...
  protected void triggerOcrAction(SimpleOcr.OcrModel ocrModel, OcrWrongPartAction ocrAction, boolean ocrStop, StringBuilder report) throws Exception {
    ...
    if (currentPart == null) {
      ...
      if (ocrPart != null && ocrPart != currentPart) {
        Logger.trace("OCR detected wrong part in slot of feeder " + getName() + ", current part " + currentPart.getId() + " != OCR part " + ocrPart.getId());
        ReferencePushPullFeeder otherFeeder = null;
        for (ReferencePushPullFeeder feeder : getAllPushPullFeeders()) {
          if (feeder.getPart() == ocrPart) {
            otherFeeder = feeder;
            Logger.trace("other feeder " + feeder.getName() + " has OCR detected part " + ocrPart.getId());
            break;
          }
        }
        if (ocrAction == OcrWrongPartAction.SwapFeeders) {
          if (otherFeeder == null) {
            throw new Exception("OCR detected part " + ocrPart.getId() + " in slot of feeder " + getName() + " is not present in any other feeder. Cannot swap out feeders.");
          }
        ...
        if (ocrAction == OcrWrongPartAction.ChangePart) {
          ...
          if (ocrAction == OcrWrongPartAction.ChangePartAndClone) {
            setOcrDetectedPart(ocrPart, true);
          }
        }
        if (ocrStop) {
          throw new Exception("OCR detected different part in feeder " + getName() + ", current part " + currentPart.getId() + " vs. OCR part " + ocrPart.getId() + ". Action performed: " + ocrAction.toString() + ". Please review.");
        } else {
          if (report != null) {
            report.append("<p>Feeder " + getName() + ": current part " + currentPart.getId() + ", OCR part " + ocrPart.getId() + ". Action performed: " + ocrAction.toString() + ".</p>");
          }
        }
      }
    }
  ...
  public ReferencePushPullFeeder createNewAtLocation(Location newLocation, Part part, ReferencePushPullFeeder templateFeeder) throws Exception {
  ...
  public ReferencePushPullFeeder createNewInRow() throws Exception {
    double bestDistanceMm = Double.MAX_VALUE;
    ReferencePushPullFeeder closestFeeder = null;
    for (ReferencePushPullFeeder feeder : getAllPushPullFeeders()) {
      if (feeder != this && feeder.getPart() != null) {
        double distanceMm = feeder.getLocation().convertToUnits(LengthUnit.Millimeters).getLinearDistanceTo(this.getLocation());
        if (closestFeeder == null || distanceMm < bestDistanceMm) {
          bestDistanceMm = distanceMm;
          closestFeeder = feeder;
        }
      }
    }
    Location rowUnit = transformFeederToMachineLocation(new Location(LengthUnit.Millimeters, 0, -getTapeWidth().convertToUnits(LengthUnit.Millimeters).getValue() - 8.0, 0, 0), null).subtract(getLocation());
    if (closestFeeder != null) {
      ...
      if (isSnapToAxis()) {
        if (Math.abs(rowUnit.getX()) > rowLocationToleranceMm && Math.abs(rowUnit.getY()) <= rowLocationToleranceMm) {
          ...
          if (Math.abs(rowUnit.getY()) > rowLocationToleranceMm && Math.abs(rowUnit.getX()) <= rowLocationToleranceMm) {
  ...
  @Override public Location getJobPreparationLocation() {
  ...
  @Override public void prepareForJob(boolean visit) throws Exception {
  ...
  public void performOcrOnFeederList(List<ReferencePushPullFeeder> ocrFeederList, OcrWrongPartAction ocrAction, boolean ocrStop, StringBuilder report) throws Exception {
    List<Location> feederLocationList = new ArrayList<>();
    for (ReferencePushPullFeeder feeder : ocrFeederList) {
      feederLocationList.add(feeder.getPickLocation(0, null).convertToUnits(LengthUnit.Millimeters));
    }
    TravellingSalesman<Location> tsm = new TravellingSalesman<>(feederLocationList, new TravellingSalesman.Locator<Location>() {
      @Override public Location getLocation(Location locatable) {
        return locatable;
      }
    }, getCamera().getLocation(), null);
    tsm.solve();
    for (Location location : tsm.getTravel()) {
      for (ReferencePushPullFeeder ocrFeeder : ocrFeederList) {
        if (location.getLinearDistanceTo(ocrFeeder.getPickLocation(0, null)) < calibrationToleranceMm) {
          ocrFeeder.performOcr(ocrAction != null ? ocrAction : ocrFeeder.getOcrWrongPartAction(), ocrAction != null ? ocrStop : ocrFeeder.isOcrStopAfterWrongPart(), report);
          break;
        }
      }
    }
  ...
  protected void performVisionOperations(Camera camera, CvPipeline pipeline, boolean storeHoles, boolean storePickLocation, boolean storeVisionOffset, OcrWrongPartAction ocrAction, boolean ocrStop, StringBuilder report) throws Exception {
    ...
    for (int i = 0; i < calibrateMaxPasses; i++) {
      Location midPoint = runningHole1Location.add(runningHole2Location).multiply(0.5, 0.5, 0, 0).derive(camera.getLocation(), false, false, true, false).derive(null, null, null, runningPickLocation.getRotation() + getRotationInFeeder());
      ...
      pipeline.process();
      FindFeatures feature = new FindFeatures(camera, pipeline, 2000, FindFeaturesMode.CalibrateHoles).invoke();
      ...
      Logger.trace("new vision offset " + feature.calibratedVisionOffset + " vs. previous vision offset " + runningVisionOffset + " results in error " + error + " at the (farthest) pick location");
      if (storeHoles) {
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(getConfigurationWizard(), "Configuration"), new PropertySheetWizardAdapter(new ReferencePushPullMotionConfigurationWizard(this), "Push-Pull Motion") };
  }
