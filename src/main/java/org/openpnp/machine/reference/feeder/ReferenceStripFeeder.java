...
public class ReferenceStripFeeder extends ReferenceFeeder {
  ...
  @Element(required = false) private Location referenceHoleLocation = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Location lastHoleLocation = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Length partPitch = new Length(4, LengthUnit.Millimeters);

  @Element(required = false) private Length tapeWidth = new Length(8, LengthUnit.Millimeters);

  @Attribute(required = false) private TapeType tapeType = TapeType.WhitePaper;
  ...
  @Attribute(required = false) private int maxFeedCount = 0;
  ...
  @Override public Location getPickLocation() throws Exception {
    ...
    Location l = Utils2D.getPointAlongLine(lineLocations[0], lineLocations[1], new Length((feedCount - 1) * partPitchAdjusted, partPitch.getUnits()));
  ...
  private void updateVisionOffsets(Nozzle nozzle) throws Exception {
    ...
    if (partPitch.convertToUnits(LengthUnit.Millimeters).getValue() < 4) {
      expectedLocation = Utils2D.getPointAlongLine(lineLocations[0], lineLocations[1], holePitch.multiply((feedCount - 1) / 2));
    } else {
      expectedLocation = Utils2D.getPointAlongLine(lineLocations[0], lineLocations[1], partPitch.multiply(feedCount - 1));
    }
  ...
  private static CvPipeline createDefaultPipeline() {
    try {
      String xml = IOUtils.toString(ReferenceStripFeeder.class.getResource("ReferenceStripFeeder-DefaultPipeline.xml"));
