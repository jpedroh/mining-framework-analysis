...
public class BlindsFeeder extends ReferenceFeeder {
  ...
  public class FindFeatures {
    ...
    public FindFeatures invoke() throws Exception {
      ...
      try {
        ...
        if (ocrStageResult != null && pipeline.getProperty("alphabet") instanceof String && !((String) pipeline.getProperty("alphabet")).isEmpty()) {
  ...
  public void actuateCover(Nozzle preferredNozzle, boolean openState, boolean loadNozzleTipIfNeeded, boolean restoreNozzleTip) throws Exception {
    if (coverType == CoverType.NoCover) {
      ...
      assertCalibration();
      if (coverType == CoverType.BlindsCover) {
        Length feederX0 = (openState ^ isBlindsCoverAlignmentReversed() ? edgeOpenDistance.multiply(-1.0).subtract(sprocketPitch.multiply(0.5)).subtract(nozzleTipDiameter.multiply(0.5)) : edgeClosedDistance.add(tapeLength).add(sprocketPitch.multiply(0.5)).add(nozzleTipDiameter.multiply(0.5))).convertToUnits(location.getUnits());
