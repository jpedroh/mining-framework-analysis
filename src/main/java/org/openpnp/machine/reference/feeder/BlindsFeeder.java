...
public class BlindsFeeder extends ReferenceFeeder {
  @Element(required = false) private Location fiducial1Location = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Location fiducial2Location = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Location fiducial3Location = new Location(LengthUnit.Millimeters);

  @Attribute(required = false) private boolean normalize = true;

  @Element(required = false) private Length tapeLength = new Length(0, LengthUnit.Millimeters);

  @Element(required = false) private Length feederExtent = new Length(0, LengthUnit.Millimeters);

  @Element(required = false) private Length pocketCenterline = new Length(0, LengthUnit.Millimeters);

  @Element(required = false) private Length pocketPitch = new Length(0, LengthUnit.Millimeters);

  @Element(required = false) private Length pocketSize = new Length(0, LengthUnit.Millimeters);

  @Element(required = false) private CvPipeline pipeline = createDefaultPipeline();

  @Attribute(required = false) private boolean visionEnabled = true;

  @Attribute private int feedCount = 0;

  @Attribute(required = false) private int feederNo = 0;

  @Attribute(required = false) private int feedersTotal = 0;

  @Attribute(required = false) private String feederGroupName = defaultGroupName;

  @Attribute(required = false) private int pocketCount = 0;

  @Attribute(required = false) private int firstPocket = 1;

  @Attribute(required = false) private int lastPocket = 0;

  @Element(required = false) private Length sprocketPitch = new Length(4, LengthUnit.Millimeters);

  @Element(required = false) private Length edgeOpenDistance = new Length(2, LengthUnit.Millimeters);

  @Element(required = false) private Length edgeClosedDistance = new Length(2, LengthUnit.Millimeters);

  @Element(required = false) private Length pushZOffset = new Length(0.25, LengthUnit.Millimeters);

  @Attribute(required = false) private double pushSpeed = 0.1;
  ...
  @Attribute(required = false) private OcrAction ocrAction = OcrAction.None;

  @Element(required = false) private Length ocrMargin = new Length(20, LengthUnit.Millimeters);

  @Attribute(required = false) private String ocrFontName = "Liberation Mono";

  @Attribute(required = false) private double ocrFontSizePt = 7.0;
  ...
  @Attribute(required = false) private OcrTextOrientation ocrTextOrientation = OcrTextOrientation.AwayFromTape;

  @Attribute(required = false) private int fidLocMaxPasses = 3;

  @Attribute(required = false) private double fidLocToleranceMm = 0.5;

  @Attribute(required = false) private double pocketPosToleranceMm = 0.1;

  /**
     * Maximum label size (derived from the fact that even 8mm tape labels must fit).
     */
  @Attribute(required = false) private double maxLabelSizeMm = 10.0;

  /**
     * Offset of the tape center from the pocket center according to EIA 481 (derived).
     */
  @Attribute(required = false) private double tapeCenterOffsetMm = -1.0625;
  ...
  private void recalculateGeometry() {
    if (pocketPitch.getValue() > 0.0) {
      ...
      setPocketCount((int) (Math.floor(tapeLength.divide(pocketPitch))) + (isSmallPitch ? 1 : 0));
      double pitchRelativePosition = (coverType == CoverType.BlindsCover ? 0.25 : 0.5);
      Length pocketAlign = sprocketPitch.multiply(Math.round(pocketPitch.multiply(pitchRelativePosition).divide(sprocketPitch) - 0.001));
      setPocketDistance(sprocketPitch.multiply(isSmallPitch ? 0.0 : 0.5).add(pocketAlign));
    }
  }
  ...
  public Location getUncalibratedPickLocation(double pocketNumber) {
    ...
    Location feederLocation = new Location(location.getUnits(), feederX.getValue(), feederY.getValue(), location.getZ(), getPickRotationInTape());
  ...
  public boolean isCoverOpenChecked() throws Exception {
    ...
    if (Double.isNaN(coverPosition.getValue())) {
      ...
      Logger.debug("[BlindsFeeder.isCoverOpenChecked] pocketPosition: {}, pocketDistance {}, error {}", coverPosition, pocketDistance, coverPosition.subtract(pocketDistance));
    }
    return isCoverOpen();
  }
  ...
  public void feed(Nozzle nozzle) throws Exception {
    ...
    if (coverType == CoverType.BlindsCover) {
      if (coverActuation == CoverActuation.CheckOpen) {
        ...
        if (coverActuation == CoverActuation.OpenOnFirstUse || coverActuation == CoverActuation.OpenOnJobStart) {
          if (!isCoverOpen()) {
  ...
  /**
     ...
     */
  @Override public boolean canTakeBackPart() {
    if (getFeedCount() > 0) {
      return true;
    } else {
      return false;
  ...
  @Override public void takeBackPart(Nozzle nozzle) throws Exception {
  ...
  private void setupOcr(Camera camera, CvPipeline pipeline, OcrAction ocrAction) {
    if (ocrAction != OcrAction.None) {
      ...
      pipeline.setProperty("SimpleOcr.alphabet", "");
    }
  }
  ...
  public Location[] getOcrRegionCornerLocations(Length feederY) {
    ...
    double feederY0Mm = feederYMm + tapeCenterOffsetMm - maxLabelSizeMm * 0.5;
    double feederY1Mm = feederYMm + tapeCenterOffsetMm + maxLabelSizeMm * 0.5;
    double edgePositionMm;
    if (getOcrMargin().getValue() < 0) {
      edgePositionMm = getTapeLength().add(getEdgeClosedDistance()).convertToUnits(LengthUnit.Millimeters).getValue() + ocrMarginMm;
    } else {
      edgePositionMm = -getEdgeOpenDistance().convertToUnits(LengthUnit.Millimeters).getValue();
    }
    Location[] feederLocations;
    if (getOcrTextOrientation() == OcrTextOrientation.AwayFromTape) {
      feederLocations = new Location[] { new Location(LengthUnit.Millimeters, edgePositionMm, feederY0Mm, 0, 0), new Location(LengthUnit.Millimeters, edgePositionMm - ocrMarginMm, feederY0Mm, 0, 0), new Location(LengthUnit.Millimeters, edgePositionMm, feederY1Mm, 0, 0) };
    } else {
      feederLocations = new Location[] { new Location(LengthUnit.Millimeters, edgePositionMm - ocrMarginMm, feederY1Mm, 0, 0), new Location(LengthUnit.Millimeters, edgePositionMm, feederY1Mm, 0, 0), new Location(LengthUnit.Millimeters, edgePositionMm - ocrMarginMm, feederY0Mm, 0, 0) };
    }
  ...
  public class FindFeatures {
    ...
    public FindFeatures invoke() throws Exception {
      ...
      try {
        results = pipeline.getExpectedResult(VisionUtils.PIPELINE_RESULTS_NAME).getExpectedListModel(RotatedRect.class, null);
        Result ocrStageResult = pipeline.getResult("OCR");
        if (ocrStageResult != null && pipeline.getProperty("SimpleOcr.alphabet") instanceof String && !((String) pipeline.getProperty("SimpleOcr.alphabet")).isEmpty()) {
        ...
        boolean angleTolerant = !(getFiducial1Location().isInitialized() && getFiducial2Location().isInitialized());
        boolean positionTolerant = angleTolerant;
        ...
        for (RotatedRect result : results) {
          ...
          for (int i = 0; i < 4; i++) {
            if (points[i].x <= 2 || points[i].x >= camera.getWidth() - 2 || points[i].y <= 2 || points[i].y >= camera.getHeight() - 2) {
              isAtMargin = true;
            }
            i++;
          }
          if (!isAtMargin) {
            Location center = transformMachineToFeederLocation(VisionUtils.getPixelLocation(camera, result.center.x, result.center.y)).convertToUnits(LengthUnit.Millimeters);
            ...
            if (positionTolerant || cameraFeederLocation.getLinearDistanceTo(center) < positionTolerance) {
              if (angleTolerant || Math.abs(Utils2D.angleNorm(angle - 45)) < angleTolerance) {
                if (mmSize.getX() > fidMin && mmSize.getX() < fidMax && mmSize.getY() > fidMin && mmSize.getY() < fidMax && mmSize.getX() / mmSize.getY() < fidAspect && mmSize.getY() / mmSize.getX() < fidAspect) {
                  fiducials.add(result);
                  Logger.debug("accepted fiducal candidate: result {}, mmSize {}", result, mmSize);
                } else {
                  Logger.debug("dismissed fiducal candidate: result {}, mmSize {}", result, mmSize);
                }
              } else {
                Logger.debug("dismissed fiducal candidate: result {}, angle {}", result, angle);
              }
            } else {
              Logger.debug("dismissed fiducal candidate: result {}, center {}", result, center);
            }
            if (positionTolerant || Math.abs(cameraFeederY - center.getY()) < positionTolerance) {
              if (positionTolerant || Math.abs(cameraFeederX - center.getX()) < pocketRange) {
                if (angleTolerant || Math.abs(Utils2D.angleNorm(angle - 0)) < angleTolerance) {
                  if (mmSize.getX() > blindMin && mmSize.getX() < blindMax && mmSize.getY() > blindMin && mmSize.getY() < blindMax && mmSize.getX() / mmSize.getY() < blindAspect && mmSize.getY() / mmSize.getX() < blindAspect) {
                    if (!positionTolerant) {
                      for (org.opencv.core.Point point : points) {
                        Location corner = transformMachineToFeederLocation(VisionUtils.getPixelLocation(camera, point.x, point.y)).convertToUnits(LengthUnit.Millimeters);
                    ...
                    blinds.add(result);
                    Logger.debug("accepted pocket candidate: result {}, mmSize {}", result, mmSize);
                  } else {
                    Logger.debug("dismissed pocket candidate: result {}, mmSize {}", result, mmSize);
                  }
                } else {
                  Logger.debug("dismissed pocket candidate: result {}, angle {}", result, angle);
                }
              } else {
                Logger.debug("dismissed pocket candidate: result {}, X {}", result, center.getX());
              }
            } else {
              Logger.debug("dismissed pocket candidate: result {}, Y {}", result, center.getY());
            }
          }
        }
        Collections.sort(fiducials, new Comparator<RotatedRect>() {
          @Override public int compare(RotatedRect o1, RotatedRect o2) {
        ...
        Logger.debug("histogram in Y: pocketCenterlineMm {}, pocketSizeMm {}", pocketCenterlineMm, pocketSizeMm);
        lines = new ArrayList<>();
        ...
        for (RotatedRect rect : blinds) {
          Location location = transformMachineToFeederLocation(VisionUtils.getPixelLocation(camera, rect.center.x, rect.center.y)).convertToUnits(LengthUnit.Millimeters);
          if (previous != null) {
        ...
        Logger.debug("histogram in pitch: pocketPitchMm {}", pocketPitchMm);
        double pocketPitchPosMm = (getPocketPitch().getValue() != 0. ? getPocketPitch().convertToUnits(LengthUnit.Millimeters).getValue() : pocketPitchMm);
        ...
        Logger.debug("histogram position: pocketPositionMm {} using pitch {}", pocketPositionMm, pocketPitchPosMm);
  ...
  public void ensureCameraZ(Camera camera) throws Exception {
    if (camera.isUnitsPerPixelAtZCalibrated() && !getLocation().getLengthZ().isInitialized()) {
      throw new Exception("Feeder " + getName() + ": Please set the Part Z first, it is required to determine " + "the true scale of the camera view for accurate computer vision.");
    }
    ...
    if (currentPart == null) {
      ...
      if (ocrPart != null && ocrPart != currentPart) {
        ...
        if (ocrAction == OcrAction.ChangePart) {
          ...
          if (ocrAction == OcrAction.CheckCorrect) {
            throw new Exception("OCR detected wrong part in slot of feeder " + getName() + ", current part " + currentPart.getId() + " != OCR part " + ocrPart.getId());
          }
        }
      } else {
        if (ocrPart == null) {
          if (ocrAction == OcrAction.ChangePart || ocrAction == OcrAction.CheckCorrect) {
            throw new Exception("OCR could not detect part in slot of feeder " + getName() + ", current part " + currentPart.getId());
          }
        }
      }
    }
  ...
  public void calibrateCoverEdges() throws Exception {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      MovableUtils.moveToLocationAtSafeZ(camera, cameraOpenPosition);
      findCoverPosition(camera);
      Length offsetOpen = getCoverPosition().subtract(wantedOpenPosition).convertToUnits(LengthUnit.Millimeters);
      ...
      setEdgeOpenDistance(getEdgeOpenDistance().add(offsetOpen.multiply(factor)));
      actuateCover(false);
      MovableUtils.moveToLocationAtSafeZ(camera, cameraClosedPosition);
      findCoverPosition(camera);
      Length offsetClosed = getCoverPosition().subtract(wantedClosedPosition).convertToUnits(LengthUnit.Millimeters);
      ...
      setEdgeClosedDistance(getEdgeClosedDistance().subtract(offsetClosed.multiply(factor)));
      if (Math.abs(offsetOpen.getValue()) < pocketPosToleranceMm * 0.5 && Math.abs(offsetClosed.getValue()) < pocketPosToleranceMm * 0.5) {
        break;
      }
    }
  }
  ...
  private Location locateFiducial(Camera camera, Location location) throws Exception {
    ...
    try (CvPipeline pipeline = getCvPipeline(camera, true, OcrAction.None)) {
      for (int i = 0; i < fidLocMaxPasses; i++) {
        ...
        Logger.debug("bestFiducialLocation: {}, mmDistance {}", bestFiducialLocation, mmDistance);
  ...
  @Attribute(required = false) private CoverType coverType = CoverType.BlindsCover;

  @Attribute(required = false) private CoverActuation coverActuation = CoverActuation.OpenOnJobStart;

  public static List<BlindsFeeder> getFeedersWithCoverToActuate(List<Feeder> feederPool, CoverActuation[] coverActuations, boolean openState) {
  ...
  public static void actuateAllFeederCovers(Nozzle preferredNozzle, boolean openState) throws Exception {
    List<BlindsFeeder> feederList = getFeedersWithCoverToActuate(Configuration.get().getMachine().getFeeders(), new CoverActuation[] { CoverActuation.Manual, CoverActuation.CheckOpen, CoverActuation.OpenOnFirstUse, CoverActuation.OpenOnJobStart }, openState);
  ...
  public static void actuateListedFeederCovers(Nozzle preferredNozzle, List<BlindsFeeder> feederList, boolean openState, boolean restoreNozzleTip) throws Exception {
    ...
    TravellingSalesman<BlindsFeeder> tsm = new TravellingSalesman<>(feederList, new TravellingSalesman.Locator<BlindsFeeder>() {
      @Override public Location getLocation(BlindsFeeder locatable) {
        return locatable.getUncalibratedPickLocation(openState ? 0 : locatable.getPocketCount() + 1);
      }
    }, nozzleAndTipForPushing.getNozzle().getLocation(), null);
    tsm.solve();
  ...
  @Override public Location getJobPreparationLocation() {
    if ((getOcrAction() != OcrAction.None || getCoverActuation() == CoverActuation.OpenOnJobStart) && !isCoverOpen()) {
  ...
  @Override public void prepareForJob(boolean visit) throws Exception {
    ...
    if (visit) {
      ...
      if (getCoverActuation() == CoverActuation.OpenOnJobStart && !isCoverOpen()) {
  ...
  public void actuateCover(Nozzle preferredNozzle, boolean openState, boolean loadNozzleTipIfNeeded, boolean restoreNozzleTip) throws Exception {
    if (coverType == CoverType.NoCover) {
      ...
      assertCalibration();
      if (coverType == CoverType.BlindsCover) {
        Length feederX0 = (openState ^ isBlindsCoverAlignmentReversed() ? edgeOpenDistance.multiply(-1.0).subtract(pocketPitch).multiply(0.5).subtract(sprocketPitch.multiply(0.5)).subtract(nozzleTipDiameter.multiply(0.5)) : edgeClosedDistance.add(tapeLength).add(pocketPitch).multiply(0.5).add(sprocketPitch.multiply(0.5)).add(nozzleTipDiameter.multiply(0.5))).convertToUnits(location.getUnits());
        Length feederX1 = (openState ^ isBlindsCoverAlignmentReversed() ? edgeOpenDistance.multiply(-1.0).subtract(nozzleTipDiameter.multiply(0.5)) : edgeClosedDistance.add(tapeLength).add(nozzleTipDiameter.multiply(0.5))).convertToUnits(location.getUnits());
        Length feederY = pocketCenterline.convertToUnits(location.getUnits());
        ...
        if (coverType == CoverType.PushCover && openState) {
          ...
          Length feederX0 = (getFeedCount() == 0 || !isCoverOpen() ? pocketPitch.multiply(-0.5).subtract(nozzleTipDiameter.multiply(1)) : coverPosition.subtract(pocketPitch.multiply(0.5)).subtract(nozzleTipDiameter.multiply(1))).convertToUnits(location.getUnits());
          Length feederX1 = pickFeederLocation.getLengthX().add(pocketPitch.multiply(0.5)).subtract(nozzleTipDiameter.multiply(0.5)).convertToUnits(location.getUnits());
          Length feederY = pickFeederLocation.getLengthY().convertToUnits(location.getUnits());
  ...
  private boolean updateFeederToMachineTransform() {
    ...
    if (!(fiducial1Location.isInitialized() && fiducial2Location.isInitialized() && distance > 1 * mm)) {
    ...
    Location ref = (fiducial3Location.convertToUnits(origin.getUnits()).getLinearDistanceTo(fiducial2Location) < fiducial3Location.convertToUnits(origin.getUnits()).getLinearDistanceTo(fiducial1Location) ? fiducial2Location : fiducial1Location).convertToUnits(origin.getUnits());
    ...
    tx = new AffineTransform(axisX.getX(), axisX.getY(), axisY.getX(), axisY.getY(), origin.getX(), origin.getY());
  ...
  public Location transformFeederToMachineLocation(Location feederLocation) {
    ...
    return new Location(fiducial1Location.getUnits(), ptDst.getX(), ptDst.getY(), feederLocation.getZ(), feederLocation.getRotation() + rotation);
  }
  ...
  public Location transformMachineToFeederLocation(Location machineLocation) {
    ...
    return new Location(fiducial1Location.getUnits(), ptDst.getX(), ptDst.getY(), machineLocation.getZ(), machineLocation.getRotation() + rotation);
  }
  ...
  public static List<BlindsFeeder> getConnectedFeedersByLocation(Location location, boolean fiducial1MatchOnly) {
    ...
    Collections.sort(list, new Comparator<BlindsFeeder>() {
      @Override public int compare(BlindsFeeder feeder1, BlindsFeeder feeder2) {
        return new Double(feeder1.getPocketCenterline().getValue()).compareTo(feeder2.getPocketCenterline().convertToUnits(feeder1.getPocketCenterline().getUnits()).getValue());
      }
    });
    return list;
  }
  ...
  public void setFiducial1Location(Location fiducial1Location) {
    ...
    if (!oldValue.equals(fiducial1Location)) {
      ...
      if (!(oldValue.isInitialized() || fiducial2Location.isInitialized() || fiducial3Location.isInitialized())) {
        updateFromConnectedFeeder(fiducial1Location, true);
      } else {
        this.updateConnectedFeedersFromThis(oldValue, true);
        if (oldValue.isInitialized() && oldValue.convertToUnits(LengthUnit.Millimeters).getLinearDistanceTo(fiducial1Location) > 2) {
    ...
    if (!oldValue.equals(fiducial2Location)) {
      ...
      if (oldValue.isInitialized() && oldValue.convertToUnits(LengthUnit.Millimeters).getLinearDistanceTo(fiducial1Location) > 2) {
        if (fiducial3Location.isInitialized() && fiducial1Location.isInitialized()) {
          ...
          if (fiducial1Location.getLinearDistanceTo(fiducial3Location) < oldValue.getLinearDistanceTo(fiducial3Location)) {
  ...
  public void setCoverPosition(Length coverPosition) {
    ...
    if (!(oldValue.equals(coverPosition) || Double.isNaN(oldValue.getValue()) == Double.isNaN(coverPosition.getValue()))) {
  ...
  public void setFeederGroupName(String newFeederGroupName) {
    ...
    boolean singleCanJoinNamedGroup = (!locationIsNull && feederGroupNames.contains(proposedGroupName) && (connected_feeders.size() <= 1));
    boolean canLeaveGroupForDefault = (!locationIsNull && feederGroupNames.contains(oldName) && proposedIsDefault);
    if (canJoinNamedGroup) {
      BlindsFeeder copyFeeder = feedersWithNewGroupName.get(0);
      this.updateFromConnectedFeeder(copyFeeder);
    } else {
      if (singleCanJoinNamedGroup) {
        BlindsFeeder copyFeeder = feedersWithNewGroupName.get(0);
        this.updateFromConnectedFeeder(copyFeeder);
      } else {
        if (canRenameGroup) {
  ...
  @Override public PropertySheet[] getPropertySheets() {
    return new PropertySheet[] { new PropertySheetWizardAdapter(getConfigurationWizard(), "Configuration"), new PropertySheetWizardAdapter(new BlindsFeederArrayConfigurationWizard(this), "Feeder Array") };
  }
  ...
  private static CvPipeline createDefaultPipeline() {
    try {
      String xml = IOUtils.toString(BlindsFeeder.class.getResource("BlindsFeeder-DefaultPipeline.xml"));
