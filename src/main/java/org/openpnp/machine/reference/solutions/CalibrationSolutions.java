...
public class CalibrationSolutions implements Solutions.Subject {
  ...
  public void calibrateAxisBacklash(ReferenceHead head, ReferenceCamera camera, HeadMountable movable, ReferenceControllerAxis axis, Length acceptableTolerance) throws Exception {
    ...
    stepTestGraph.getRow(ERROR, LIMIT0).setColor(new Color(0, 0x80, 0));
    stepTestGraph.getRow(ERROR, LIMIT1).setColor(new Color(0, 0x80, 0));
    ...
    for (double stepPos = -stepTestMm / 2; stepPos < stepTestMm / 2; stepPos += stepMm) {
      ...
      if (stepLocation0 != null) {
        ...
        stepTestGraph.getRow(ERROR, LIMIT0).recordDataPoint(step, -toleranceUnits);
        stepTestGraph.getRow(ERROR, LIMIT1).recordDataPoint(step, toleranceUnits);
      } else {
      ...
      for (double distance : backlashProbingDistances) {
        for (int reverse = 0; reverse <= 1; reverse++) {
          ...
          if (pass == 0) {
            ...
            distanceGraph.getRow(TIME, TIME + reverse).recordDataPoint(distance, t1 - t0);
          }
          ...
          Location effective0 = machine.getVisionSolutions().getDetectedLocation(camera, movable, location, fiducialDiameter, passTitle + distanceOutput + " \u25ba", false);
          if (pass == 0) {
            if (reverse == 0) {
              MovableUtils.moveToLocationAtSafeZ(movable, displacedAxisLocation(movable, axis, location, backlashTestMoveLargeMm * mmAxis, distance > backlashTestMoveMm));
            }
          ...
          Location effective1 = machine.getVisionSolutions().getDetectedLocation(camera, movable, location, fiducialDiameter, passTitle + distanceOutput + " \u25c4", false);
          ...
          if (pass == 0) {
            distanceGraph.getRow(SCALE, BACKLASH + reverse).recordDataPoint(distance, errorUnits);
          } else {
            distanceGraph.getRow(SCALE, OVERSHOOT + reverse).recordDataPoint(distance, (maxBacklash - errorUnits) / 2);
          }
    ...
    for (double speed : backlashProbingSpeeds) {
      ...
      for (int pass = 0; pass < backlashCalibrationPasses; pass++) {
        MovableUtils.moveToLocationAtSafeZ(movable, displacedAxisLocation(movable, axis, location, -backlashTestMoveLargeMm * mmAxis, false));
        movable.moveTo(location);
        Location effective0 = machine.getVisionSolutions().getDetectedLocation(camera, movable, location, fiducialDiameter, "Backlash at Speed " + speed + "\u00d7 \u25ba", false);
        MovableUtils.moveToLocationAtSafeZ(movable, displacedAxisLocation(movable, axis, location, backlashTestMoveLargeMm * mmAxis, false));
        movable.moveTo(location);
        Location effective1 = machine.getVisionSolutions().getDetectedLocation(camera, movable, location, fiducialDiameter, "Backlash at Speed " + speed + "\u00d7 \u25c4", false);
        double mmError = effective1.subtract(effective0).dotProduct(unit).getValue();
        if (movable == camera) {
          mmError = -mmError;
        }
