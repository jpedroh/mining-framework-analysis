  ...
  @Override public void findIssues(Solutions solutions) {
    ...
    if (camera != null) {
      ...
      if (isDefaultHead && solutions.isTargeting(Milestone.Welcome)) {
        solutions.add(new Solutions.Issue(head, Translations.getString("HeadSolutions.Issue.CreateNozzles"), Translations.getString("HeadSolutions.Solution.CreateNozzles"), Solutions.Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration%3A-Nozzle-Setup") {
          {
            ...
            if (nozzleSolution == null) {
              ...
              for (Nozzle nozzle : head.getNozzles()) {
                if (nozzle.getAxisZ() instanceof ReferenceCamCounterClockwiseAxis) {
                  ...
                  if (nozzle.getAxisZ() instanceof ReferenceMappedAxis) {
                    nozzleSolution = NozzleSolution.DualNegated;
                  } else {
                    if (nozzle.getAxisZ() instanceof ReferenceControllerAxis) {
                      multiplier++;
                    }
                  }
                }
          ...
          @Override public void setState(Solutions.State state) throws Exception {
            if (state == State.Solved) {
              if (solutions.confirm(
<<<<<<< /usr/src/app/output/openpnp/openpnp/f9a8fe8740a0c227349eb465f22314b826ed7d2f/src/main/java/org/openpnp/machine/reference/solutions/HeadSolutions.java/left.java
              "<html>" + "<p>Accepting this solution may change your machine configuration fundamentally.<br/>" + "The following will happen:<p/>" + "<ol>" + "<li>The current machine configuration is saved (same as File/Save Configuration).</li>" + "<li>The new solution overwrites your existing nozzle and axis configuration.</li>" + "<li>As far as nozzles, actuators and axes remain the same type and count, their detail configuration is preserved (except for names).</li>" + "<li>A nozzle solution can be applied multiple times, you can revisit and expand it.</li>" + "<li><span color=\"red\">Caution:</span> Reopen will not restore the previous configuration, only enable a fresh choice.<br/>" + "If you want to restore previous configuration you must restore the saved configuration manually.</li>" + "</ol>" + "<br/>" + "Are you sure?</html>"
=======
              Translations.getString("HeadSolutions.Issue.CreateNozzles.Confirm")
>>>>>>> /usr/src/app/output/openpnp/openpnp/f9a8fe8740a0c227349eb465f22314b826ed7d2f/src/main/java/org/openpnp/machine/reference/solutions/HeadSolutions.java/right.java
              , true)) {
          ...
          @Override public Solutions.Issue.CustomProperty[] getProperties() {
            return new Solutions.Issue.CustomProperty[] { new Solutions.Issue.IntegerProperty(Translations.getString("HeadSolutions.Solution.CreateNozzles.NumberOfNozzlesLabel.text"), Translations.getString("HeadSolutions.Solution.CreateNozzles.NumberOfNozzlesLabel.toolTipText"), 1, 8) {
          ...
          @Override public Solutions.Issue.Choice[] getChoices() {
            return new Solutions.Issue.Choice[] { new Solutions.Issue.Choice(NozzleSolution.Standalone, Translations.getString("HeadSolutions.Solution.CreateNozzles.Choice.0"), Icons.nozzleSingle), new Solutions.Issue.Choice(NozzleSolution.DualNegated, Translations.getString("HeadSolutions.Solution.CreateNozzles.Choice.1"), Icons.nozzleDualNeg), new Solutions.Issue.Choice(NozzleSolution.DualCam, Translations.getString("HeadSolutions.Solution.CreateNozzles.Choice.2"), Icons.nozzleDualCam) };
          }
        });
      }
      if (solutions.isTargeting(Milestone.Basics)) {
        ...
        ActuatorSolutions.findActuateIssues(solutions, head, head.getPumpActuator(), "pump control", "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration%3A-Vacuum-Setup#pump-control-setup");
        if (head.getzProbeActuator() != null) {
          ActuatorSolutions.findActuatorReadIssues(solutions, head, head.getzProbeActuator(), "Z probe", "https://github.com/openpnp/openpnp/wiki/Z-Probing");
        }
      }
    }
  }
  ...
  protected void perNozzleSolutions(Solutions solutions, Nozzle nozzle) {
    if (nozzle.getAxisZ() == null) {
      solutions.add(new Solutions.PlainIssue(nozzle, "Nozzle " + nozzle.getName() + " does not have a Z axis assigned.", "Please assign a proper Z axis. You might need to create one first.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Mapping-Axes"));
    }
    if (nozzle.getAxisRotation() == null) {
      solutions.add(new Solutions.PlainIssue(nozzle, "Nozzle " + nozzle.getName() + " does not have a Rotation axis assigned.", "Please assign a proper Rotation axis. You might need to create one first.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Mapping-Axes"));
    }
    if (nozzle.getAxisZ() != null && nozzle.getAxisRotation() != null) {
      ...
      for (Nozzle nozzle2 : head.getNozzles()) {
        ...
        if (nozzle2.getAxisZ() == nozzle.getAxisZ()) {
          solutions.add(new Solutions.PlainIssue(nozzle, "Nozzles " + nozzle2.getName() + " and " + nozzle.getName() + " have the same Z axis assigned.", "Please assign a different Z axis.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Mapping-Axes"));
        }
        if (nozzle2.getAxisRotation() == nozzle.getAxisRotation()) {
          solutions.add(new Solutions.PlainIssue(nozzle, "Nozzles " + nozzle2.getName() + " and " + nozzle.getName() + " have the same Rotation axis assigned.", "It is OK to share rotation axes. If intentional, just dismiss this issue. Otherwise assign a different Rotation axis.", Severity.Information, "https://github.com/openpnp/openpnp/wiki/Mapping-Axes"));
        }
      }
    }
  }

  protected void sameAxisAssignedIssue(Solutions solutions, Camera camera, HeadMountable hm, Axis axis, Axis.Type type) {
    solutions.add(new Solutions.Issue(head, "Camera " + camera.getName() + " and " + hm.getName() + " have the same " + type + " axis " + axis.getName() + " assigned.", "Unassign the " + type + " axis " + axis.getName() + " from the camera " + camera.getName() + ". Later you can press Find Issues & Solutions " + "again to get a new Solution for a virtual axis replacement.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Machine-Axes#referencevirtualaxis") {
      @Override public void setState(Solutions.State state) throws Exception {
        ((AbstractHeadMountable) camera).setAxis(((AbstractAxis) (state == State.Solved ? null : axis)), type);
        super.setState(state);
      }
    });
  }
  ...
  protected void addMissingAxisIssue(Solutions solutions, final Camera camera, Axis.Type type) {
    ...
    solutions.add(new Solutions.Issue(camera, (isXY ? "Missing " + type.name() + " axis assignment. Assign one to continue." : "Virtual " + type.name() + " axis recommended for camera " + camera.getName() + "."), (axis == null ? "Create and assign a " + type.name() + " axis manually." : "Assign " + (isNew ? "new " : "existing ") + axis.getClass().getSimpleName() + " " + axis.getName() + " as " + type.name() + "."), isXY ? Severity.Fundamental : Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/Mapping-Axes") {
      @Override public void setState(Solutions.State state) throws Exception {
        ((AbstractHeadMountable) camera).setAxis(((AbstractAxis) (state == State.Solved ? axis : null)), type);
        if (isNew) {
          try {
            if (state == State.Solved) {
      ...
      @Override public boolean canBeAccepted() {
        return axis != null;
      }
    });
  }

  protected void addInconsistentAxisIssue(Solutions solutions, final Camera camera, HeadMountable hm, Axis.Type type) {
    final Axis oldAxis = hm.getAxis(type);
    if ((hm instanceof Nozzle || oldAxis != null) && oldAxis != camera.getAxis(type)) {
      solutions.add(new Solutions.Issue(hm, "Inconsistent " + type.name() + " axis assignment " + (oldAxis != null ? oldAxis.getName() : "null") + " (not the same as default camera " + camera.getName() + ").", "Assign " + camera.getAxis(type).getName() + " as the " + type.name() + " axis.", (hm instanceof Nozzle) ? Severity.Error : Severity.Warning, "https://github.com/openpnp/openpnp/wiki/Mapping-Axes") {
        @Override public void setState(Solutions.State state) throws Exception {
          ((AbstractHeadMountable) hm).setAxis(((AbstractAxis) (state == State.Solved ? camera.getAxis(type) : oldAxis)), type);
          super.setState(state);
        }
      });
    }
  ...
  private synchronized void createNozzleSolution(Camera camera, NozzleSolution nozzleSolution, int nozzleSolutionsMultiplier) throws Exception {
  ...
  public void assignVacuumActuators(AbstractNozzle n, LinkedHashSet<Actuator> valveActuators, LinkedHashSet<Actuator> senseActuators, String suffix) throws Exception {
    if (n instanceof ReferenceNozzle) {
      ...
      if (vacuumSenseActuator.getDriver() instanceof GcodeDriver) {
        ...
        if (driver.getCommand(vacuumSenseActuator, CommandType.ACTUATE_BOOLEAN_COMMAND) != null && driver.getCommand(vacuumValveActuator, CommandType.ACTUATE_BOOLEAN_COMMAND) == null) {
  ...
  private AbstractNozzle reuseOrCreateNozzle(Camera camera, LinkedHashSet<AbstractNozzle> nozzles, String i) throws Exception {
  ...
  private <T extends Axis, C extends T> T reuseOrCreateAxis(Camera camera, LinkedHashSet<T> axes, Class<C> cls, Axis.Type type, String i) throws Exception {
    T axis = null;
    for (T a : axes) {
      axis = a;
      break;
    ...
    if (axis == null) {
      ...
      if (axis instanceof AbstractControllerAxis && camera.getAxisX() instanceof AbstractControllerAxis) {
  ...
  private Actuator reuseOrCreateActuator(LinkedHashSet<Actuator> actuators, String i) throws Exception {
