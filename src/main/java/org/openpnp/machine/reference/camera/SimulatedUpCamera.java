...
@Root public class SimulatedUpCamera extends ReferenceCamera {
  @Attribute(required = false) protected int width = 640;

  @Attribute(required = false) protected int height = 480;

  @Attribute(required = false) private boolean simulateFocalBlur;

  @Element(required = false) private Location errorOffsets = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Location simulatedLocation;

  @Element(required = false) private Location simulatedUnitsPerPixel;

  @Attribute(required = false) private boolean simulatedFlipped;
  ...
  @Override public BufferedImage internalCapture() {
    if (!ensureOpen()) {
      return null;
    }
    ...
    Location phySize = getSimulatedUnitsPerPixel().convertToUnits(LengthUnit.Millimeters).multiply(width, height, 0, 0);
    ...
    Rectangle2D.Double phyBounds = new Rectangle2D.Double(location.getX() - phyWidth, location.getY() - phyHeight, phyWidth * 2, phyHeight * 2);
  ...
  private void drawNozzle(Graphics2D gView, Nozzle nozzle, Location l) {
    ...
    if (fillShape(g, new Ellipse2D.Double(-0.5, -0.5, 1, 1), new Color(0, 220, 0), unitsPerPixel, offsets, false)) {
      ...
      Location partUndersideLocation = l.subtract(new Location(LengthUnit.Millimeters, 0, 0, partHeightMm, 0));
      ...
      if (frame != null) {
        blurObjectIntoView(gView, frame, nozzle, partUndersideLocation);
        g.dispose();
      }
    } else {
      if (frame != null) {
  ...
  protected void blurObjectIntoView(Graphics2D gView, BufferedImage frame, Nozzle nozzle, Location l) {
    ...
    double radius = Math.min(distanceMm * bokeh, 5);
    ConvolveOp op = null;
    if (radius > 0.01) {
  ...
  private boolean fillShape(Graphics2D g, Shape shape, Color color, Location unitsPerPixel, Location offsets, boolean addError) {
    ...
    if (perspective <= 2) {
      ...
      Color colorShade = new Color((int) Math.min(255, color.getRed() / shade), (int) Math.min(255, color.getGreen() / shade), (int) Math.min(255, color.getBlue() / shade));
  ...
  @Override public void findIssues(Solutions solutions) {
    ...
    if (solutions.isTargeting(Milestone.Connect)) {
      solutions.add(new Solutions.Issue(this, Translations.getString("SimulatedUpCamera.Issue"), Translations.getString("SimulatedUpCamera.Solution"), Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/OpenPnpCaptureCamera") {
