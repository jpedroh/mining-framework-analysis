...
public class ImageCamera extends ReferenceCamera {
  @Element private String sourceUri = "classpath://samples/pnp-test/pnp-test.png";

  @Attribute(required = false) private int width = 640;

  @Attribute(required = false) private int height = 480;

  @Element(required = false) private Location imageUnitsPerPixel;

  @Attribute(required = false) private double simulatedRotation = 0;

  @Attribute(required = false) private double simulatedScale = 1.0;

  @Attribute(required = false) private double simulatedDistortion = 0.0;

  @Attribute(required = false) private double simulatedYRotation = 0.0;

  @Attribute(required = false) private boolean simulatedFlipped = false;

  @Element(required = false) private Length focalLength = new Length(6, LengthUnit.Millimeters);

  @Element(required = false) private Length sensorDiagonal = new Length(4.4, LengthUnit.Millimeters);

  @Element(required = false) private Location primaryFiducial = new Location(LengthUnit.Millimeters);

  @Element(required = false) private Location secondaryFiducial = new Location(LengthUnit.Millimeters);
  ...
  /**
     ...
     */
  @Attribute(required = false) private double pickLocationToleranceMm = 0.1;

  @Attribute(required = false) private double pickLocationMinimumScore = 0.87;

  @Attribute(required = false) private double placeLocationToleranceMm = 0.1;

  @Attribute(required = false) private double placeLocationMinimumScore = 0.58;

  @Attribute(required = false) boolean filterTestImageVision = true;

  @Deprecated @Attribute(required = false) private boolean subPixelRendering = true;
  ...
  @Override public synchronized void open() throws Exception {
  ...
  @Override public synchronized BufferedImage internalCapture() {
    if (!ensureOpen()) {
      return null;
    }
  ...
  protected BufferedImage locationCapture(Location location, int width, int height, boolean simulation) {
    ...
    gFrame.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
  ...
  protected void drawFiducial(Graphics2D gFrame, int cameraWidth, int cameraHeight, Location uppDefault, Location upp, Location fiducial) {
    ...
    double dia = Math.sqrt(Math.pow(pt1.getX() - pt0.getX(), 2) + Math.pow(pt1.getY() - pt0.getY(), 2));
  ...
  public boolean isPickLocation(Location physicalLocation, Nozzle nozzle) throws Exception {
    return isPartLocation(physicalLocation, nozzle.getPart(), true, Color.black, Color.black, Color.white, pickLocationMinimumScore, pickLocationToleranceMm);
  }
  ...
  public boolean isPlaceLocation(Location physicalLocation, Nozzle nozzle) throws Exception {
    return isPartLocation(physicalLocation, nozzle.getPart(), false, Color.white, Color.black, null, placeLocationMinimumScore, placeLocationToleranceMm);
  }

  protected boolean isPartLocation(Location physicalLocation, Part part, boolean topView, Color padsColor, Color bodyColor, Color backgroundColor, double minScore, double maxDistanceMm) throws Exception {
    Mat mat = null;
    ...
    try {
      ...
      BufferedImage template = OpenCvUtils.createFootprintTemplate(this, footprint, physicalLocation.getRotation(), topView, padsColor, bodyColor, backgroundColor, 1.5, 8);
      ...
      if (bestMatch.score < minScore) {
        Logger.error("No good PnP location recognized in target area at location " + physicalLocation + ". Best match score " + bestMatch.score + " does not meet " + minScore + ", distance " + bestDistance);
        return false;
      }
      if (bestDistance > maxDistanceMm + pixelTolerance) {
        Logger.error("PnP location distance " + bestDistance + "mm greater than allowed " + maxDistanceMm + "mm + (" + pixelTolerance + "mm px tol), match " + bestMatch + ", at location " + physicalLocation);
        return false;
      }
      Logger.debug("PnP location distance " + bestDistance + "mm from best match score " + bestMatch.score);
      return true;
    }  finally {
      if (mat != null) {
        mat.release();
  ...
  @Override public void findIssues(Solutions solutions) {
    ...
    if (solutions.isTargeting(Milestone.Connect)) {
      solutions.add(new Solutions.Issue(this, Translations.getString("ImageCamera.Issue"), Translations.getString("ImageCamera.Solution"), Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/OpenPnpCaptureCamera") {
