...
public class ContactProbeNozzle extends ReferenceNozzle {
  ...
  @Attribute(required = false) private boolean discardProbing = false;

  @Element(required = false) private Length calibrationOffsetZ = null;

  @Element(required = false) private Length unloadedCalibrationOffsetZ = null;

  @Attribute(required = false) private double maxZOffsetMm = 2.0;

  @ElementMap(required = false) private HashMap<String, Length> probedFeederHeightOffsets = new HashMap<>();

  @ElementMap(required = false) private HashMap<String, Length> probedPartHeightOffsets = new HashMap<>();
  ...
  @Override public void moveToPlacementLocation(Location placementLocation, Part part) throws Exception {
    Length partHeight = ((part == null && nozzleTip != null) ? nozzleTip.getMaxPartHeight() : getSafePartHeight(part));
  ...
  @Override public void ensureZCalibrated(boolean assumeNozzleTipLoaded) throws Exception {
    ...
    if (assumeNozzleTipLoaded || !nt.getzCalibrationTrigger().isPerNozzleTip()) {
        ...
        if (getContactProbeActuator() instanceof AbstractActuator) {
          ...
          if (getAxisZ() instanceof ControllerAxis) {
            ...
            if (driver != null && driver != oldDriver) {
              solutions.add(new Solutions.Issue(this, "Z driver " + driver.getName() + " not same as actuator " + contactProbeActuator.getName() + " driver " + (oldDriver == null ? "(unassigned)" : oldDriver.getName()) + ".", "Assign driver " + driver.getName() + " to actuator " + contactProbeActuator.getName() + ".", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration%3A-Actuators#adding-actuators") {
                @Override public void setState(Solutions.State state) throws Exception {
          ...
          if (contactProbeActuator.getDriver() instanceof GcodeDriver) {
            ...
            if (suggestedCommand != null) {
              GcodeDriverSolutions.suggestGcodeCommand(gcodeDriver, contactProbeActuator, solutions, GcodeDriver.CommandType.ACTUATE_BOOLEAN_COMMAND, suggestedCommand, false, false);
            } else {
  ...
  public static void addConversionIssue(Solutions solutions, final ReferenceNozzle nozzle) {
    if (solutions.isTargeting(Milestone.Advanced)) {
      if (!(nozzle instanceof ContactProbeNozzle)) {
        solutions.add(new Solutions.Issue(nozzle, "The nozzle can be replaced with a ContactProbeNozzle to support various probing features.", "Replace with ContactProbeNozzle.", Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/Contact-Probing-Nozzle") {
          @Override public void setState(Solutions.State state) throws Exception {
      ...
      if (nozzle instanceof ContactProbeNozzle) {
        solutions.add(new Solutions.Issue(nozzle, "Converting the ContactProbeNozzle back to a plain ReferenceNozzle may simplify the machine setup.", "Replace with ReferenceNozzle.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/Contact-Probing-Nozzle") {
  ...
  public static ReferenceNozzle convertToReferenceNozzle(ContactProbeNozzle nozzle) throws Exception {
    ...
    serOut.write(nozzle, sw);
    String serialized = sw.toString();
    serialized.replace(nozzle.getClass().getCanonicalName(), ReferenceNozzle.class.getCanonicalName());
