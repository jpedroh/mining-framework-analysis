...
public class ContactProbeNozzle extends ReferenceNozzle {
  ...
  @Element(required = false) private String contactProbeActuatorName = "";

  private boolean isDisabled;

  @Element(required = false) private Length contactProbeStartOffsetZ = new Length(1, LengthUnit.Millimeters);

  @Element(required = false) private Length contactProbeDepthZ = new Length(2, LengthUnit.Millimeters);

  @Element(required = false) private Length sniffleIncrementZ = new Length(0.1, LengthUnit.Millimeters);

  @Attribute(required = false) private long sniffleDwellTime = 250;

  @Element(required = false) private Length contactProbeAdjustZ = new Length(0, LengthUnit.Millimeters);
  ...
  @Attribute(required = false) private ContactProbeMethod contactProbeMethod = ContactProbeMethod.ContactSenseActuator;
  ...
  @Attribute(required = false) private ContactProbeTrigger feederHeightProbing = ContactProbeTrigger.EachTime;

  @Attribute(required = false) private ContactProbeTrigger partHeightProbing = ContactProbeTrigger.EachTime;

  @Element(required = false) private Length calibrationOffsetZ = null;

  @Attribute(required = false) private double maxZOffsetMm = 2.0;

  @ElementMap(required = false) private HashMap<Feeder, Length> probedFeederHeightOffsets = new HashMap<>();

  @ElementMap(required = false) private HashMap<Part, Length> probedPartHeightOffsets = new HashMap<>();

  private ReferenceNozzleTip zCalibratedNozzleTip;

  @Override public PropertySheet[] getPropertySheets() {
    return Collect.concat(super.getPropertySheets(), new PropertySheet[] { new PropertySheetWizardAdapter(new ContactProbeNozzleWizard(this), "Contact Probe") });
  }
  ...
  @Override public void ensureZCalibrated() throws Exception {
  ...
  public static void referenceAllTouchLocationsZ() throws Exception {
    ...
    for (NozzleTip nt : Configuration.get().getMachine().getNozzleTips()) {
      if (nt != templateNozzleTip && nt instanceof ReferenceNozzleTip) {
        Location touchLocation = ((ReferenceNozzleTip) nt).getTouchLocation();
        if (touchLocation.getLinearDistanceTo(Location.origin) != 0 && !((ReferenceNozzleTip) nt).isUnloadedNozzleTipStandin()) {
...
<<<<<<< /usr/src/app/output/openpnp/openpnp/a9f99068197956007021fea9951f899a66d1865c/src/main/java/org/openpnp/machine/reference/ContactProbeNozzle.java/left.java
      ...
      try {
        if (getContactProbeActuator() instanceof AbstractActuator) {
          if (!getContactProbeActuator().isCoordinatedAfterActuate()) {
            solutions.add(new Solutions.Issue(getContactProbeActuator(), "Contact probe actuator needs machine coordination after actuation.", "Enable After Actuation machine coordination.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Motion-Planner#actuator-machine-coordination") {
    ...
    try {
      if (getContactProbeActuator() instanceof AbstractActuator) {
        ...
        if (!contactProbeActuator.isCoordinatedAfterActuate()) {
          issues.add(new Solutions.Issue(contactProbeActuator, "Contact probe actuator needs machine coordination after actuation.", "Enable After Actuation machine coordination.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/Motion-Planner#actuator-machine-coordination") {
            @Override public void setState(Solutions.State state) throws Exception {
        ...
        if (getAxisZ() instanceof ControllerAxis) {
          ...
          if (driver instanceof GcodeAsyncDriver) {
            if (!((GcodeAsyncDriver) driver).isReportedLocationConfirmation()) {
              issues.add(new Solutions.Issue(this, "Z driver " + driver.getName() + " must use Location Confirmation for the probe actuator to work.", "Enable Location Confirmation.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#advanced-settings") {
                @Override public void setState(Solutions.State state) throws Exception {
                  if (confirmStateChange(state)) {
                    ((GcodeAsyncDriver) driver).setReportedLocationConfirmation((state == Solutions.State.Solved));
                    super.setState(state);
            ...
            issues.add(new Solutions.PlainIssue(this, "Z driver " + driver.getName() + " must support Location Confirmation for the Z probe actuator to work.", "Only the GcodeAsyncDriver currently supports it.", Severity.Error, "https://github.com/openpnp/openpnp/wiki/GcodeAsyncDriver#advanced-settings"));
          }
        }
...
>>>>>>> /usr/src/app/output/openpnp/openpnp/a9f99068197956007021fea9951f899a66d1865c/src/main/java/org/openpnp/machine/reference/ContactProbeNozzle.java/right.java
  }
  ...
  protected boolean isPartHeightSensingAvailable(Part part, NozzleTip nozzleTip) {
    ...
    if (part != null && AbstractPnpJobProcessor.findPartAligner(machine, part) != null) {
      ...
      return (getContactProbeMethod().isPlacementCompatible());
    }
  }

  @Override protected boolean isNozzleTipAndPartCompatible(NozzleTip nt, Part part) {
  ...
  public static void addConversionIssue(List<Issue> issues, final ReferenceNozzle nozzle) {
    if (!(nozzle instanceof ContactProbeNozzle)) {
      issues.add(new Solutions.Issue(nozzle, "The nozzle can be replaced with a ContactProbeNozzle to support various probing features.", "Replace with ContactProbeNozzle.", Severity.Fundamental, "https://github.com/openpnp/openpnp/wiki/Contact-Probing-Nozzle") {
        @Override public void setState(Solutions.State state) throws Exception {
          if (confirmStateChange(state)) {
  ...
  public static ContactProbeNozzle convertToContactProbe(ReferenceNozzle nozzle) throws Exception {
    ...
    serialized.replace(nozzle.getClass().getCanonicalName(), ContactProbeNozzle.class.getCanonicalName());
