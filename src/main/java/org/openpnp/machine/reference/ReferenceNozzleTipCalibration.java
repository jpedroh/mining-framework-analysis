...
@Root public class ReferenceNozzleTipCalibration extends AbstractModelObject {
  ...
  public static class TableBasedRunoutCompensation implements ReferenceNozzleTipCalibration.RunoutCompensation {
    @Element(required = false) List<Location> nozzleTipMeasuredLocations;
    ...
    @Override public Location getOffset(double angle) {
    ...
    @Override public Location getCameraOffset() {
  ...
  public static class ModelBasedRunoutCompensation implements ReferenceNozzleTipCalibration.RunoutCompensation {
    ...
    @Attribute(required = false) protected double centerX = 0;

    @Attribute(required = false) protected double centerY = 0;

    @Attribute(required = false) protected double radius = 0;

    @Attribute(required = false) protected double phaseShift;
    ...
    public ModelBasedRunoutCompensation(List<Location> nozzleTipMeasuredLocations) {
      this.nozzleTipMeasuredLocations = nozzleTipMeasuredLocations;
      this.units = nozzleTipMeasuredLocations.size() > 0 ? nozzleTipMeasuredLocations.get(0).getUnits() : LengthUnit.Millimeters;
  ...
  @Attribute(required = false) private int allowMisdetections = 0;

  @Attribute(required = false) private double angleStart = -180;

  @Attribute(required = false) private double angleStop = 180;

  @Attribute(required = false) private double excenterRatio = 0.25;

  @Attribute(required = false) private boolean enabled;

  private boolean calibrating;

  @Element(required = false) private RunoutCompensation runoutCompensation = null;
  ...
  @Attribute(required = false) private ReferenceNozzleTipCalibration.RunoutCompensationAlgorithm runoutCompensationAlgorithm = RunoutCompensationAlgorithm.Model;

  @Attribute(required = false) private RecalibrationTrigger recalibrationTrigger = RecalibrationTrigger.NozzleTipChangeInJob;

  @Attribute(required = false) @Deprecated private Double offsetThreshold = 0.0;

  @Element(required = false) private Length offsetThresholdLength = new Length(0.5, LengthUnit.Millimeters);

  @Element(required = false) private Length calibrationZOffset = new Length(0.0, LengthUnit.Millimeters);
...
<<<<<<< /usr/src/app/output/openpnp/openpnp/14992da6ab44f34a81b4cab67ed34628e22a13e2/src/main/java/org/openpnp/machine/reference/ReferenceNozzleTipCalibration.java/left.java
    true
=======
    !(homing || Configuration.get().getMachine().isHomed())
>>>>>>> /usr/src/app/output/openpnp/openpnp/14992da6ab44f34a81b4cab67ed34628e22a13e2/src/main/java/org/openpnp/machine/reference/ReferenceNozzleTipCalibration.java/right.java
      ...
      throw new Exception(
<<<<<<< /usr/src/app/output/openpnp/openpnp/14992da6ab44f34a81b4cab67ed34628e22a13e2/src/main/java/org/openpnp/machine/reference/ReferenceNozzleTipCalibration.java/left.java
      "Calibration is broken in this version. Please downgrade if you require calibration."
=======
      "Machine not yet homed, nozzle tip calibration request aborted"
>>>>>>> /usr/src/app/output/openpnp/openpnp/14992da6ab44f34a81b4cab67ed34628e22a13e2/src/main/java/org/openpnp/machine/reference/ReferenceNozzleTipCalibration.java/right.java
      );
    }
...
<<<<<<< /usr/src/app/output/openpnp/openpnp/14992da6ab44f34a81b4cab67ed34628e22a13e2/src/main/java/org/openpnp/machine/reference/ReferenceNozzleTipCalibration.java/left.java
    null
=======
    (ReferenceNozzle) nozzleTip.getParentNozzle()
>>>>>>> /usr/src/app/output/openpnp/openpnp/14992da6ab44f34a81b4cab67ed34628e22a13e2/src/main/java/org/openpnp/machine/reference/ReferenceNozzleTipCalibration.java/right.java
    ;
    ...
    Location measureBaseLocation = cameraLocation.derive(null, null, null, 0d).add(new Location(this.calibrationZOffset.getUnits(), 0, 0, this.calibrationZOffset.getValue(), 0));
    try {
      calibrating = true;
      ...
      if (!calibrateCamera) {
        ...
        excenter = VisionUtils.getPixelCenterOffsets(camera, camera.getWidth() / 2 + Math.min(camera.getWidth(), camera.getHeight()) * excenterRatio, camera.getHeight() / 2);
      }
      ...
      for (int i = 0; i <= angleSubdivisions; i++) {
        ...
        Location measureLocation = measureBaseLocation.derive(null, null, null, measureAngle).add(excenter.rotateXy(measureAngle));
      ...
      if (!calibrateCamera) {
        if (this.runoutCompensationAlgorithm == RunoutCompensationAlgorithm.Model) {
          ...
          if (this.runoutCompensationAlgorithm == RunoutCompensationAlgorithm.ModelNoOffset) {
            ...
            if (this.runoutCompensationAlgorithm == RunoutCompensationAlgorithm.ModelCameraOffset) {
        ...
        Location newCameraPosition = referenceCamera.getHeadOffsets().subtract(cameraCompensation.getAxisOffset());
        Logger.debug("[nozzleTipCalibration]applying axis offset to bottom camera position: {} - {} = {}", referenceCamera.getHeadOffsets(), cameraCompensation.getAxisOffset(), newCameraPosition);
        ...
        Logger.debug("[nozzleTipCalibration]applying angle offset to bottom camera rotation: {} - {} = {}", referenceCamera.getRotation(), cameraCompensation.getPhaseShift(), newCameraAngle);
  ...
  private Location findCircle(Location measureLocation) throws Exception {
    ...
    try (CvPipeline pipeline = getPipeline()) {
      ...
      MainFrame.get().getCameraViews().getCameraView(camera).showFilteredImage(OpenCvUtils.toBufferedImage(pipeline.getWorkingImage()), 1000);
      ...
      while (locationsIterator.hasNext()) {
        ...
        Location measureLocationRelative = measureLocation.convertToUnits(location.getUnits()).subtract(camera.getLocation());
  ...
  public boolean isRecalibrateOnNozzleTipChangeNeeded() {
    return (recalibrationTrigger == RecalibrationTrigger.NozzleTipChange) || (recalibrationTrigger == RecalibrationTrigger.MachineHome && !isCalibrated());
  }

  public boolean isRecalibrateOnHomeNeeded() {
    return recalibrationTrigger == RecalibrationTrigger.NozzleTipChange || recalibrationTrigger == RecalibrationTrigger.MachineHome;
  }
