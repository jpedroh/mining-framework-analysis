...
public abstract class ReferenceCamera extends AbstractBroadcastingCamera implements ReferenceHeadMountable {
  ...
  @Attribute(required = false) private int captureTryCount = 4;

  @Attribute(required = false) private int captureTryTimeoutMs = 2000;

  @Element(required = false) private Location headOffsets = new Location(LengthUnit.Millimeters);

  @Attribute(required = false) protected double rotation = 0;

  @Attribute(required = false) protected boolean flipX = false;

  @Attribute(required = false) protected boolean flipY = false;

  @Deprecated @Element(required = false) protected Length safeZ = null;

  @Attribute(required = false) protected int offsetX = 0;

  @Attribute(required = false) protected int offsetY = 0;

  @Attribute(required = false) protected int cropWidth = 0;

  @Attribute(required = false) protected int cropHeight = 0;

  @Attribute(required = false) protected int scaleWidth = 0;

  @Attribute(required = false) protected int scaleHeight = 0;

  @Attribute(required = false) protected double redBalance = 1.0;

  @Attribute(required = false) protected double greenBalance = 1.0;

  @Attribute(required = false) protected double blueBalance = 1.0;

  @Attribute(required = false) protected double redGamma = 1.0;

  @Attribute(required = false) protected double greenGamma = 1.0;

  @Attribute(required = false) protected double blueGamma = 1.0;

  @Attribute(required = false) protected boolean deinterlace;

  @Element(required = false) private LensCalibrationParams calibration = new LensCalibrationParams();


<<<<<<< /usr/src/app/output/openpnp/openpnp/e42b7c30e360a47d4c2afa496c089935cb0bc69d/src/main/java/org/openpnp/machine/reference/ReferenceCamera.java/left.java
  @Element(required = false) private AdvancedCalibration advancedCalibration = new AdvancedCalibration();
=======
  @Element(required = false) private double[] redColorMap;
>>>>>>> /usr/src/app/output/openpnp/openpnp/e42b7c30e360a47d4c2afa496c089935cb0bc69d/src/main/java/org/openpnp/machine/reference/ReferenceCamera.java/right.java


  @Attribute(required = false) private String lightActuatorId;

  @Attribute(required = false) private boolean allowMachineActuators = false;

  @Attribute(required = false) private FocusSensingMethod focusSensingMethod = FocusSensingMethod.None;

  @Element(required = false) protected FocusProvider focusProvider = new AutoFocusProvider();

  @Attribute(required = false) private double whiteBalanceLeadFractile = 0.8;

  @Attribute(required = false) private double whiteBalanceClipFractile = 0.99;

  @Element(required = false) private double[] greenColorMap;

  @Element(required = false) private double[] blueColorMap;
  ...
  /**
     ...
     */
  @Override public BufferedImage captureTransformed() {
  ...
  /**
     ...
     * applied.
     */
  @Override public BufferedImage captureRaw() {
    return safeInternalCapture();
  }

  @Override public boolean hasNewFrame() {
    return isOpen();
  }
  ...
  @Override public Actuator getLightActuator() {
  ...
  @Override public FocusProvider getFocusProvider() {
...
>>>>>>> /usr/src/app/output/openpnp/openpnp/e42b7c30e360a47d4c2afa496c089935cb0bc69d/src/main/java/org/openpnp/machine/reference/ReferenceCamera.java/right.java
        ...
        if (isDeinterlaced() || isCropped() || isCalibrating() || isUndistorted() || isScaled() || isRotated() || isOffset() || isFlipped() || isWhiteBalanced()) {
          Mat mat = OpenCvUtils.toMat(image);
          mat = deinterlace(mat);
          mat = crop(mat);
          mat = calibrate(mat);
  ...
  @Override public Location getUnitsPerPixel(Length viewingPlaneZ) {
  ...
  public void autoAdjustWhiteBalanceMapped(int levels) throws Exception {
    ...
    for (int y = 0; y < image.getHeight(); y++) {
      for (int x = 0; x < image.getWidth(); x++) {
        ...
        int g = (rgb >> 8) & 0xff;
        int b = (rgb >> 0) & 0xff;
        int lum = ((r >= clip || g >= clip || b >= clip) ? 255 : (int) (amplify * (r + g + b) / 3));
        if (lum >= 1) {
    ...
    for (int ch = 0; ch < 3; ch++) {
      ...
      for (int level = 1; level < levels; level++) {
        if (level == levels) {
          ...
          if (colorMap[ch][level - 1] > colorMap[ch][level]) {
            if (level + 1 >= levels) {
              colorMap[ch][level] = (colorMap[ch][level - 1] * 3 - colorMap[ch][level - 2]) / 2;
            } else {
  ...
  private Mat crop(Mat mat) {
    if (isCropped()) {
      ...
      Rect roi = new Rect((int) ((mat.size().width / 2) - (cw / 2)), (int) ((mat.size().height / 2) - (ch / 2)), cw, ch);
  ...
  @Override public void findIssues(Solutions solutions) {
    ...
    if (solutions.isTargeting(Milestone.Vision)) {
      if (getLooking() == Looking.Up && isFlipX() == isFlipY() && !(this instanceof SimulatedUpCamera)) {
        solutions.add(new Solutions.PlainIssue(this, "An up-looking camera should usually mirror the image.", "Enable either Flip X or Flip Y (but not both) in the camera\'s Image Transforms.", Severity.Warning, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration:-General-Camera-Setup#set-rotation-and-transforms"));
      }
      final double previewFps = getPreviewFps();
      if (previewFps > 15) {
        solutions.add(new Solutions.Issue(this, "A high Preview FPS value might create undue CPU load.", "Set to 5 FPS.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration:-General-Camera-Setup#general-configuration") {
      ...
      if (!isSuspendPreviewInTasks()) {
        solutions.add(new Solutions.Issue(this, "It is recommended to suspend camera preview during machine tasks / Jobs.", "Enable Suspend during tasks.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration:-General-Camera-Setup#general-configuration") {
          @Override public void setState(Solutions.State state) throws Exception {
      ...
      if (!isAutoVisible()) {
        solutions.add(new Solutions.Issue(this, "In single camera preview OpenPnP can automatically switch the camera for you.", "Enable Auto Camera View.", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration:-General-Camera-Setup#general-configuration") {
          @Override public void setState(Solutions.State state) throws Exception {
      ...
      if (view != null) {
        ...
        if (renderingQuality.ordinal() < RenderingQuality.High.ordinal()) {
          solutions.add(new Solutions.Issue(this, "The preview rendering quality can be improved.", "Set to Rendering Quality to High (right click the Camera View to see other options).", Severity.Suggestion, "https://github.com/openpnp/openpnp/wiki/Setup-and-Calibration:-General-Camera-Setup#camera-view-configuration") {
            @Override public void setState(Solutions.State state) throws Exception {
