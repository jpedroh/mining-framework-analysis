...
public class Location {
  @Attribute private LengthUnit units;

  @Attribute(required = false) private double x;

  @Attribute(required = false) private double y;

  @Attribute(required = false) private double z;
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unused" }) private Location() {
    this(null);
  }
  ...
  public Location convertToUnits(LengthUnit units) {
    Location location = new Location(units, new Length(x, this.units).convertToUnits(units).getValue(), new Length(y, this.units).convertToUnits(units).getValue(), new Length(z, this.units).convertToUnits(units).getValue(), rotation);
    return location;
  }
  ...
  public Location invert(boolean x, boolean y, boolean z, boolean rotation) {
    return new Location(getUnits(), getX() * (x ? -1 : 1), getY() * (y ? -1 : 1), getZ() * (z ? -1 : 1), getRotation() * (rotation ? -1 : 1));
  }
  ...
  public Location derive(Double x, Double y, Double z, Double rotation) {
    return new Location(units, x == null ? this.x : x, y == null ? this.y : y, z == null ? this.z : z, rotation == null ? this.rotation : rotation);
  }
  ...
  public Location derive(Location location, boolean x, boolean y, boolean z, boolean rotation) {
    ...
    return new Location(units, x ? location.x : this.x, y ? location.y : this.y, z ? location.z : this.z, rotation ? location.rotation : this.rotation);
  }
  ...
  public Location rotateXy(double angle) {
    ...
    return new Location(getUnits(), getX() * Math.cos(angle) - getY() * Math.sin(angle), getX() * Math.sin(angle) + getY() * Math.cos(angle), getZ(), getRotation());
  }
