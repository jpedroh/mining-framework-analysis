...
public class ScriptFileWatcher {
  ...
  private void copyExampleScripts() {
    ...
    examplesDir.mkdirs();
    String[] exampleScripts = new String[] { "JavaScript/Call_Java.js", "JavaScript/Hello_World.js", "JavaScript/Move_Machine.js", "JavaScript/Pipeline.js", "JavaScript/Print_Scripting_Info.js", "JavaScript/QrCodeXout.js", "JavaScript/Reset_Strip_Feeders.js", "JavaScript/Utility.js", "Python/call_java.py", "Python/move_machine.py", "Python/print_hallo_openpnp.py", "Python/print_methods_vars.py", "Python/print_nozzle_info.py", "Python/print_scripting_info.py", "Python/use_module.py", "Python/utility.py" };
    for (String name : exampleScripts) {
      try {
        ...
        FileUtils.copyURLToFile(ClassLoader.getSystemResource("scripts/Examples/" + name), file);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  public void setMenu(JMenu menu) {
    ...
    AbstractAction btnClearPool = new AbstractAction(Translations.getString("Scripting.Action.ClearScriptingEnginePool")) {
      {
  ...
  private void watchDirectory(File directory) {
    try {
      directory.toPath().register(fileWatcher, StandardWatchEventKinds.ENTRY_CREATE, StandardWatchEventKinds.ENTRY_DELETE, StandardWatchEventKinds.ENTRY_MODIFY);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  private synchronized void synchronizeMenu(JMenu menu, File directory) {
    ...
    Set<String> itemNames = getScriptMenuItems(menu).stream().map(JMenuItem::getText).collect(Collectors.toSet());
    ...
    itemNames = getScriptMenuItems(menu).stream().map(JMenuItem::getText).collect(Collectors.toSet());
  ...
  private void addSorted(JMenu menu, JMenuItem item) {
    ...
    for (int i = 0; i < menu.getItemCount(); i++) {
      ...
      if (existingItem == null || item.getText().toLowerCase().compareTo(existingItem.getText().toLowerCase()) <= 0) {
        menu.insert(item, i);
        return;
      }
    }
