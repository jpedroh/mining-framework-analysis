 ...
 * @since 1.0
 */
@Mojo(name = "exec", threadSafe = true, requiresDependencyResolution = ResolutionScope.TEST) public class ExecMojo extends AbstractExecMojo {
  /**
  ...
  /**
     ...
     * @since 1.1-beta-2
     */
  @Parameter private Map<String, String> environmentVariables = new HashMap<String, String>();
  ...
  /**
     ...
     * @since 1.4.0
     */
  @Parameter private File environmentScript = null;
  ...
  /**
     ...
     */
  @Parameter(property = "exec.asyncDestroyOnShutdown", defaultValue = "true") private boolean asyncDestroyOnShutdown = true;
  ...
  public void execute() throws MojoExecutionException {
    ...
    try {
      ...
      if (hasCommandlineArgs()) {
        ...
        if (!StringUtils.isEmpty(argsProp)) {
      ...
      try {
        ...
        if (outputFile != null) {
          ...
          if (useMavenLogger) {
            ...
            try (OutputStream out = new LineRedirectOutputStream(mavenOutRedirect); OutputStream err = new LineRedirectOutputStream(mavenErrRedirect)) {
  ...
  private void handleAdditionalArgs(List<String> commandArguments) throws MojoExecutionException, IOException {
    ...
    for (int i = 0; i < arguments.size(); i++) {
      ...
      if (specialArg != null) {
        if (isLongClassPathArgument(specialArg) && argument instanceof Classpath) {
          ...
          if (isLongModulePathArgument(specialArg) && argument instanceof Modulepath) {
      ...
      if (argument instanceof Classpath) {
        ...
        if (argument instanceof Modulepath) {
          ...
          if ((argument instanceof String) && (isLongModulePathArgument((String) argument) || isLongClassPathArgument((String) argument))) {
  ...
  private List<Artifact> filterArtifacts(List<Artifact> artifacts, Collection<String> dependencies) {
    ...
    filter.add(new IncludesArtifactFilter(new ArrayList<String>(dependencies)));
  ...
  protected int executeCommandLine(Executor exec, final CommandLine commandLine, Map<String, String> enviro, final PumpStreamHandler psh) throws ExecuteException, IOException {
