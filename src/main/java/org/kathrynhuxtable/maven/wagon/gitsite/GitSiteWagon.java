...
public class GitSiteWagon extends AbstractWagon {
  ...
  private void checkIn(ScmProvider scmProvider, ScmRepository scmRepository, String msg) throws ScmException {
    ...
    ScmResult result = (CheckInScmResult) executeCommand((GitExeScmProvider) scmProvider, new GitSiteCheckInCommand(), scmRepository.getProviderRepository(), new ScmFileSet(checkoutDirectory), parameters);
  ...
  private String checkOut(ScmProvider scmProvider, ScmRepository scmRepository, String targetName, Resource resource) throws TransferFailedException {
    ...
    try {
      ...
      CheckOutScmResult ret = (CheckOutScmResult) executeCommand((GitExeScmProvider) scmProvider, new GitSiteCheckOutCommand(), scmRepository.getProviderRepository(), new ScmFileSet(new File(checkoutDirectory, "")), parameters);
      checkScmResult(ret);
    } catch (ScmException e) {
      fireTransferError(resource, e, TransferEvent.REQUEST_PUT);
    ...
    while (!stack.isEmpty()) {
      ...
      if (!newDir.mkdirs()) {
        throw new TransferFailedException("Failed to create directory " + newDir.getAbsolutePath() + "; parent should exist: " + checkoutDirectory);
      }
      try {
    ...
    if (scmFile.isDirectory()) {
      ...
      for (int i = 0; i < files.length; i++) {
        if (reservedScmFile != null && !reservedScmFile.equals(files[i].getName())) {
          addedFiles += addFiles(scmProvider, scmRepository, basedir, (scmFilePath.length() == 0 ? "" : scmFilePath + "/") + files[i].getName());
        }
      }
    }
    return addedFiles;
  ...
  public void connect(Repository repository, AuthenticationInfo authenticationInfo, ProxyInfoProvider proxyInfoProvider) throws ConnectionException, AuthenticationException {
    ...
    if (url.startsWith("gitsite:")) {
      ...
      if (index > -1) {
        ...
        if (siteBranch.endsWith("/")) {
          siteBranch = siteBranch.substring(0, siteBranch.length() - 1);
        }
      } else {
        siteBranch = "gh-pages";
      }
  ...
  private void putResource(File sourceDirectory, String destinationDirectory) throws TransferFailedException {
    ...
    try {
      ...
      if (!fileAlreadyInScm || scmFile.isDirectory()) {
        int addedFiles = addFiles(scmProvider, scmRepository, newCheckoutDirectory, sourceDirectory.isDirectory() ? "" : scmFile.getName());
  ...
  /**
     ...
     * @return the files.
     */
  @SuppressWarnings(value = { "unchecked" }) private List<ScmFile> getListScmResultFiles(ListScmResult result) {
