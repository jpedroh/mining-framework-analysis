...
public class StandardRepresentation implements Representation {
  ...
  @VisibleForTesting public static int getMaxLengthForSingleLineDescription() {
  ...
  @VisibleForTesting public static int getMaxElementsForPrinting() {
  ...
  @Override public String toStringOf(Object object) {
    ...
    if (object instanceof CompletableFuture) {
      return toStringOf((CompletableFuture<?>) object);
    }
    if (isArray(object)) {
    ...
    if (object instanceof Tuple) {
      return toStringOf((Tuple) object);
    }
    if (object instanceof MapEntry) {
      return toStringOf((MapEntry<?, ?>) object);
    }
    if (object instanceof Method) {
      return ((Method) object).toGenericString();
    }
    if (object instanceof InsertDelta<?>) {
      return toStringOf((InsertDelta<?>) object);
    }
    if (object instanceof ChangeDelta<?>) {
      return toStringOf((ChangeDelta<?>) object);
    }
    if (object instanceof DeleteDelta<?>) {
      return toStringOf((DeleteDelta<?>) object);
    }
    if (object instanceof ByteBuffer) {
      return toStringOf((ByteBuffer) object);
    }
    return object == null ? null : fallbackToStringOf(object);
  }

  @SuppressWarnings(value = { "unchecked" }) protected <T extends java.lang.Object> String customFormat(T object) {
    if (object == null) {
      return null;
    }
  ...
  @Override public String unambiguousToStringOf(Object obj) {
    return obj == null ? null : String.format("%s (%s@%s)", toStringOf(obj), obj.getClass().isAnonymousClass() ? obj.getClass().getName() : obj.getClass().getSimpleName(), toHexString(System.identityHashCode(obj)));
  }
  ...
  protected String toStringOf(Comparator<?> comparator) {
    ...
    if (comparatorSimpleClassName.length() == 0) {
      return quote("anonymous comparator class");
    }
  ...
  protected String toStringOf(ComparatorBasedComparisonStrategy comparatorBasedComparisonStrategy) {
    ...
    return comparatorDescription == null ? toStringOf(comparatorBasedComparisonStrategy.getComparator()) : quote(comparatorDescription);
  }
  ...
  protected String toStringOf(ByteBuffer b) {
    ...
    int arrowLocation = b.position() * (2 + separator.length());
  ...
  protected String toStringOf(AtomicMarkableReference<?> atomicMarkableReference) {
    return String.format("AtomicMarkableReference[marked=%s, reference=%s]", atomicMarkableReference.isMarked(), toStringOf(atomicMarkableReference.getReference()));
  }

  protected String toStringOf(AtomicStampedReference<?> atomicStampedReference) {
    return String.format("AtomicStampedReference[stamp=%s, reference=%s]", atomicStampedReference.getStamp(), toStringOf(atomicStampedReference.getReference()));
  }

  private String toStringOf(ChangeDelta<?> changeDelta) {
    return String.format("Changed content at line %s:%nexpecting:%n  %s%nbut was:%n  %s%n", changeDelta.lineNumber(), formatLines(changeDelta.getOriginal().getLines()), formatLines(changeDelta.getRevised().getLines()));
  }

  private String toStringOf(DeleteDelta<?> deleteDelta) {
    return String.format("Missing content at line %s:%n  %s%n", deleteDelta.lineNumber(), formatLines(deleteDelta.getOriginal().getLines()));
  }

  private String toStringOf(InsertDelta<?> insertDelta) {
    return String.format("Extra content at line %s:%n  %s%n", insertDelta.lineNumber(), formatLines(insertDelta.getRevised().getLines()));
  }
  ...
  protected String smartFormat(Object[] iterable) {
    ...
    return doesDescriptionFitOnSingleLine(singleLineDescription) ? singleLineDescription : multiLineFormat(iterable, alreadyFormatted);
  }
  ...
  protected String format(Object[] array, String elementSeparator, String indentation, Set<Object[]> alreadyFormatted) {
    ...
    while (true) {
      ...
      if (!isArray(element)) {
        desc.append(element == null ? NULL : toStringOf(element));
      } else {
        if (isArrayTypePrimitive(element)) {
          desc.append(formatPrimitiveArray(element));
        } else {
          if (alreadyFormatted.contains(element)) {
            desc.append("(this array)");
          } else {
            desc.append(format((Object[]) element, elementSeparator, indentation, alreadyFormatted));
          }
        }
      }
      if (i == array.length - 1) {
