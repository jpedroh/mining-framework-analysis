...
public class BaseAssertionGenerator implements AssertionGenerator, AssertionsEntryPointGenerator {
  ...
  static final String 
<<<<<<< /usr/src/app/output/joel-costigliola/assertj-assertions-generator/e9c59be58d0f5f8598375f4031f5f06ac0f1cdcc/src/main/java/org/assertj/assertions/generator/BaseAssertionGenerator.java/left.java
  DEFAULT_HAS_ASSERTION_TEMPLATE_FOR_REAL_NUMBER = "has_assertion_template_for_real_number.txt"
=======
  DEFAULT_CUSTOM_HIERARCHICAL_ASSERTION_CLASS_TEMPLATE = "custom_hierarchical_assertion_class_template.txt"
>>>>>>> /usr/src/app/output/joel-costigliola/assertj-assertions-generator/e9c59be58d0f5f8598375f4031f5f06ac0f1cdcc/src/main/java/org/assertj/assertions/generator/BaseAssertionGenerator.java/right.java
  ;
  ...
  private Template 
<<<<<<< /usr/src/app/output/joel-costigliola/assertj-assertions-generator/e9c59be58d0f5f8598375f4031f5f06ac0f1cdcc/src/main/java/org/assertj/assertions/generator/BaseAssertionGenerator.java/left.java
  hasAssertionTemplateForRealNumber
=======
  hierarchicalClassAssertionTemplate
>>>>>>> /usr/src/app/output/joel-costigliola/assertj-assertions-generator/e9c59be58d0f5f8598375f4031f5f06ac0f1cdcc/src/main/java/org/assertj/assertions/generator/BaseAssertionGenerator.java/right.java
  ;
  ...
  @Override public File generateCustomAssertionFor(ClassDescription classDescription) throws IOException {
  ...
  @Override public File[] generateHierarchicalCustomAssertionFor(ClassDescription classDescription, Set<Class<?>> allClasses) throws IOException {
    ...
    final String abstractFileName = abstractAssertClassNameOf(classDescription) + ".java";
    retval[0] = createFile(assertionFileContent[0], abstractFileName, targetDirectory);
    retval[1] = createFile(assertionFileContent[1], fileName, targetDirectory);
    return retval;
  }

  @Override public String[] generateHierarchicalCustomAssertionContentFor(ClassDescription classDescription, Set<Class<?>> allClasses) throws IOException {
    ...
    retval[0] = fillClassTemplate(assertionFileContentBuilder.toString(), classDescription, allClasses, false);
    retval[1] = fillClassTemplate(concreteAssertionFileContentBuilder.toString(), classDescription, null, true);
    return retval;
  }

  private String fillClassTemplate(String template, ClassDescription classDescription, Set<Class<?>> allClasses, boolean concrete) {
    ...
    final Class<?> superType = classDescription.getSuperType();
    final TypeName superTypeName = new TypeName(superType);
    final TypeName superAssertionClass;
    if (allClasses == null || !allClasses.contains(superType)) {
    ...
    final String selfType, myself, customAssertionClass;
    if (concrete) {
      customAssertionClass = assertClassNameOf(classDescription);
      selfType = customAssertionClass;
      myself = "this";
    } else {
      customAssertionClass = abstractAssertClassNameOf(classDescription);
      selfType = "S";
      myself = "myself";
    }
    return template.replaceAll(PACKAGE_REGEXP, classDescription.getPackageName()).replaceAll(CUSTOM_ASSERTION_CLASS_REGEXP, customAssertionClass).replaceAll(SUPER_ASSERTION_CLASS_REGEXP, superAssertionClass.getSimpleNameWithOuterClassNotSeparatedByDots()).replaceAll(CLASS_TO_ASSERT_REGEXP, classDescription.getClassNameWithOuterClass()).replaceAll(SELF_TYPE_REGEXP, selfType).replaceAll(MYSELF_REGEXP, myself).replace(IMPORTS, listImports(imports, classDescription.getPackageName()));
  }

  @Override public String generateCustomAssertionContentFor(ClassDescription classDescription) throws IOException {
    ...
    assertionFileContentBuilder.append(LINE_SEPARATOR).append("}").append(LINE_SEPARATOR);
    return fillClassTemplate(assertionFileContentBuilder.toString(), classDescription, null, true);
  }
  ...
  @Override public String generateAssertionsEntryPointClassContentFor(final Set<ClassDescription> classDescriptionSet, AssertionsEntryPointType assertionsEntryPointType, String entryPointClassPackage) {
    if (noClassDescriptionsGiven(classDescriptionSet)) {
      return "";
    }
    ...
    return generateAssertionsEntryPointClassContent(classDescriptionSet, assertionsEntryPointClassTemplate, assertionEntryPointMethodTemplate, entryPointClassPackage);
  }
  ...
  @Override public File generateAssertionsEntryPointClassFor(final Set<ClassDescription> classDescriptionSet, AssertionsEntryPointType assertionsEntryPointType, String entryPointClassPackage) throws IOException {
    if (noClassDescriptionsGiven(classDescriptionSet)) {
      return null;
    }
    ...
    return createAssertionsFileFor(classDescriptionSet, assertionsEntryPointFileContent, assertionsEntryPointType.getFileName(), entryPointClassPackage);
  }

  private String generateAssertionsEntryPointClassContent(final Set<ClassDescription> classDescriptionSet, final Template entryPointAssertionsClassTemplate, final Template entryPointAssertionMethodTemplate, String entryPointClassPackage) {
    ...
    entryPointAssertionsClassContent = entryPointAssertionsClassContent.replace(IMPORTS, softAssertionsImportsContent);
    String allEntryPointsAssertionContent = generateAssertionEntryPointMethodsFor(classDescriptionSet, entryPointAssertionMethodTemplate);
    entryPointAssertionsClassContent = entryPointAssertionsClassContent.replaceAll(ALL_ASSERTIONS_ENTRY_POINTS_REGEXP, allEntryPointsAssertionContent);
   ...
   */
  private File createAssertionsFileFor(final Set<ClassDescription> classDescriptionSet, final String fileContent, final String fileName, final String assertionsClassPackage) throws IOException {
  ...
  private String generateAssertionEntryPointMethodsFor(final Set<ClassDescription> classDescriptionSet, Template assertionEntryPointMethodTemplate) {
    SortedSet<ClassDescription> sortedClassDescriptionSet = new TreeSet<ClassDescription>(new Comparator<ClassDescription>() {
      @Override public int compare(final ClassDescription cd1, final ClassDescription cd2) {
    ...
    for (ClassDescription classDescription : sortedClassDescriptionSet) {
      ...
      assertionEntryPointMethodContent = assertionEntryPointMethodContent.replaceAll(CUSTOM_ASSERTION_CLASS_REGEXP, assertClassNameOf(classDescription));
      assertionEntryPointMethodContent = assertionEntryPointMethodContent.replaceAll(CLASS_TO_ASSERT_REGEXP, classDescription.getClassNameWithOuterClass());
  ...
  private String generateEntryPointsAssertionsImportFor(final Set<ClassDescription> classDescriptionSet, final String entryPointAssertionsClassPackage) {
  ...
  private String assertionContentFor(DataDescription field) {
    ...
    assertionContent = assertionContent.replace("get${Property}()", "${property}").replace("is${Property}()", "${property}");
    assertionContent = remove(assertionContent, "${throws}");
    assertionContent = remove(assertionContent, "${throws_javadoc}");
    return assertionContent.replaceAll(PROPERTY_WITH_UPPERCASE_FIRST_CHAR_REGEXP, capitalize(field.getName())).replaceAll(PROPERTY_TYPE_REGEXP, field.getTypeName()).replaceAll(PROPERTY_WITH_LOWERCASE_FIRST_CHAR_REGEXP, field.getName());
  }
  ...
  private String baseAssertionContentFor(DataDescription fieldOrProperty) {
    ...
    if (fieldOrProperty.isBooleanType()) {
      ...
      if (fieldOrProperty.isIterableType()) {
        assertionContent = hasIterableElementsAssertionTemplate.getContent().replaceAll(ELEMENT_TYPE_REGEXP, fieldOrProperty.getElementTypeName());
      } else {
        if (fieldOrProperty.isArrayType()) {
          assertionContent = hasArrayElementsAssertionTemplate.getContent().replaceAll(ELEMENT_TYPE_REGEXP, fieldOrProperty.getElementTypeName());
        } else {
          if (fieldOrProperty.isRealNumberType()) {
            assertionContent = hasAssertionTemplateForRealNumber.getContent();
          } else {
            if (fieldOrProperty.isPrimitiveType()) {
  ...
  private static void checkTemplateParameter(Template assertionClassTemplate, Type templateType) {
    ...
    if (templateType != assertionClassTemplate.getType()) {
      throw new IllegalArgumentException("Expecting a Template type to be \'" + templateType + "\' but was \'" + assertionClassTemplate.getType() + "\'");
    }
