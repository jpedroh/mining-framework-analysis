...
public class ClassToClassDescriptionConverter implements ClassDescriptionConverter<Class<?>> {
  ...
  @VisibleForTesting protected Set<GetterDescription> getterDescriptionsOf(Class<?> clazz) {
    for (Method getter : getterMethodsOf(clazz)) {
      if (isGetDeclaringClassEnumGetter(getter, clazz)) {
        continue;
      }
  ...
  @VisibleForTesting protected Set<GetterDescription> declaredGetterDescriptionsOf(Class<?> clazz) {
  ...
  @VisibleForTesting protected Set<FieldDescription> declaredFieldDescriptionsOf(Class<?> clazz) {
    return doFieldDescriptionsOf(declaredPublicFieldsOf(clazz));
  }

  @VisibleForTesting protected Set<FieldDescription> fieldDescriptionsOf(Class<?> clazz) {
  ...
  private TypeDescription getTypeDescription(Member member) {
    ...
    if (isArray(type)) {
      return buildArrayTypeDescription(type);
    }
    if (isIterable(type)) {
    ...
    for (Member getter : getterMethodsAndNonStaticPublicFieldsOf(clazz)) {
      ...
      if (isArray(propertyType)) {
        ...
        if (isIterable(propertyType)) {
          ...
          if (hasParameterizedType(getter)) {
  ...
  private Set<TypeName> removeTypeNamesInPackage(Set<TypeName> typeNamesToImport, String packageName) {
    ...
    for (TypeName typeName : typeNamesToImport) {
      if (!typeName.getPackageName().equals(packageName)) {
