...
public class NewsletterService {
  ...
  @Autowired private transient SubscriptionService subscriptionService;

  @Autowired private transient HtmlExternalizationService htmlExternalizationService;

  @Autowired private transient HttpClientService httpClientService;

  @Autowired private transient MailService mailService;

  @Autowired private transient RenderService renderService;

  @Autowired private transient JahiaSitesService siteService;

  @Autowired private transient JahiaUserManagerService userService;
  ...
  public boolean sendIssueToSubscribers(final JCRNodeWrapper node, final RenderContext renderContext, final Map<String, String> newsletterVersions) throws Exception {
          ...
          for (Subscription subscription : l.getData()) {
            ...
            if (confirmationKey == null) {
              try {
                ...
                logger.warn("Unable to store the confirmation key for the subscription " + subscription.getSubscriber(), e);
              }
            } else {
              letterContext.getRequest().setAttribute("org.jahia.modules.newsletter.unsubscribeLink", UnsubscribeAction.generateUnsubscribeLink(target, confirmationKey, renderContext.getRequest()));
            }
            sendIssue(letterContext, node, subscription.getEmail(), letterUser, "html", language, "live", newsletterVersions);
          }
    ...
    if (publicationService != null) {
      publicationService.publish(Collections.singletonList(node.getIdentifier()), node.getSession().getWorkspace().getName(), Constants.LIVE_WORKSPACE, Collections.singletonList(""));
    }
    logger.info("The content of the node {} was sent as a newsletter in {} ms", node.getPath(), System.currentTimeMillis() - timer);
    return true;
  }
  ...
  public boolean sendIssue(final RenderContext renderContext, final JCRNodeWrapper node, final String email, final JahiaUser user, final String type, final Locale locale, final String workspace, final Map<String, String> newsletterVersions) throws RepositoryException {
      ...
      JCRTemplate.getInstance().doExecute(user, workspace, locale, new JCRCallback<String>() {
        public String doInJCR(JCRSessionWrapper session) throws RepositoryException {
          ...
          try {
            ...
            @SuppressWarnings(value = { "rawtypes" }) Enumeration attributeNames = renderContext.getRequest().getAttributeNames();
    ...
    try {
      ...
      if (siteNode.isNodeType("jmix:newsletterSender")) {
        String newMailSender = siteNode.getPropertyAsString("newsletterMailSender");
        if ((newMailSender != null) && !"".equals(newMailSender.trim())) {
  ...
  public List<JCRNodeWrapper> getSiteNewsletters(JCRSiteNode site, String orderBy, boolean orderAscending, JCRSessionWrapper session) {
    ...
    try {
      ...
      q.append("select * from [" + JNT_NEWSLETTER + "] where isdescendantnode([").append(site.getPath()).append("])");
      if (orderBy != null) {
    ...
    if (logger.isDebugEnabled()) {
      logger.info("Site newsletters search took " + (System.currentTimeMillis() - timer) + " ms. Returning " + newsletters.size() + " newsletter(s)");
    }
    return newsletters;
  }
  ...
  public List<JCRNodeWrapper> getNewsletterIssues(String targetNewsletter, String orderBy, boolean orderAscending, JCRSessionWrapper session) {
    ...
    try {
      ...
      StringBuilder q = new StringBuilder();
      q.append("select * from [" + JNT_NEWSLETTER_ISSUE + "] where isdescendantnode([").append(target.getPath()).append("])");
      if (orderBy != null) {
        q.append(" order by [").append(orderBy).append("]").append(orderAscending ? "asc" : "desc");
      }
      Query query = queryManager.createQuery(q.toString(), Query.JCR_SQL2);
    ...
    if (logger.isDebugEnabled()) {
      logger.info("Newsletter issues search took " + (System.currentTimeMillis() - timer) + " ms. Returning " + issues.size() + " issue(s)");
    }
    return issues;
  }
  ...
  public JCRNodeWrapper getNewslettersRootNode(JCRSiteNode site, JCRSessionWrapper session) {
    StringBuilder q = new StringBuilder();
    q.append("select * from [" + JNT_NEWSLETTERS + "] where isdescendantnode([").append(site.getPath()).append("])");
    try {
      QueryManager queryManager = session.getWorkspace().getQueryManager();
      if (queryManager == null) {
        logger.error("Unable to obtain QueryManager instance");
