...
public class UnsubscribeAction extends Action {
  ...
  @Autowired private transient JahiaUserManagerService userManagerService;

  public static String generateUnsubscribeLink(JCRNodeWrapper newsletterNode, String confirmationKey, HttpServletRequest req) throws RepositoryException {
    return req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort() + Jahia.getContextPath() + Render.getRenderServletPath() + "/live/" + newsletterNode.getLanguage() + newsletterNode.getPath() + ".confirm.do?key=" + confirmationKey + "&exec=rem";
  }
        ...
        try {
          ...
          if (email != null) {
            ...
            if (subscription == null) {
              ...
              if (sendConfirmationMail(session, email, node, subscription, resource, req)) {
            ...
            if (subscription == null) {
              ...
              if (forceConfirmationForRegisteredUsers && sendConfirmationMail(session, userNode.getPropertyAsString("j:email"), node, subscription, resource, req)) {
                return new ActionResult(SC_OK, null, new JSONObject("{\"status\":\"mail-sent\"}"));
              }
            }
            subscriptionService.cancel(subscription.getIdentifier(), session);
  ...
  private boolean sendConfirmationMail(JCRSessionWrapper session, String email, JCRNodeWrapper node, JCRNodeWrapper subscription, Resource resource, HttpServletRequest req) throws RepositoryException, JSONException {
