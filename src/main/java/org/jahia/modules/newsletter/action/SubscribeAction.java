...
public class SubscribeAction extends Action {
  ...
  @Autowired private JahiaUserManagerService userManagerService;

  public ActionResult doExecute(final HttpServletRequest req, final RenderContext renderContext, final Resource resource, JCRSessionWrapper session, final Map<String, List<String>> parameters, URLResolver urlResolver) throws Exception {
        ...
        try {
          ...
          if (email != null) {
            if (email.length() == 0 || !MailService.isValidEmailAddress(email, false)) {
              logger.warn("Invalid e-mail address \'{}\' provided for subscription to {}." + " Ignoring subscription request.", email, node.getPath());
            ...
            if (subscription != null) {
              ...
              if (sendConfirmationMail(session, email, node, newSubscriptionNode, resource.getLocale(), req)) {
                return new ActionResult(SC_OK, null, new JSONObject("{\"status\":\"mail-sent\"}"));
              }
            }
          } else {
            ...
            if (subscriptionService.getSubscription(node.getIdentifier(), user.getUserKey(), session) != null) {
              return new ActionResult(SC_OK, null, new JSONObject("{\"status\":\"already-subscribed\"}"));
            } else {
              ...
              if (forceConfirmationForRegisteredUsers) {
                if (sendConfirmationMail(session, userNode.getPropertyAsString("j:email"), node, newSubscriptionNode, resource.getLocale(), req)) {
                  return new ActionResult(SC_OK, null, new JSONObject("{\"status\":\"mail-sent\"}"));
                }
              } else {
  ...
  private boolean sendConfirmationMail(JCRSessionWrapper session, String email, JCRNodeWrapper node, JCRNodeWrapper newSubscriptionNode, final Locale locale, HttpServletRequest req) throws RepositoryException, JSONException {
    if (mailConfirmationTemplate != null) {
      ...
      bindings.put("confirmationlink", req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort() + Jahia.getContextPath() + Render.getRenderServletPath() + "/live/" + node.getLanguage() + node.getPath() + ".confirm.do?key=" + confirmationKey + "&exec=add");
      try {
        ...
        mailService.sendMessageWithTemplate(mailConfirmationTemplate, bindings, email, mailSender, null, null, locale, modulePackageNameToUse);
