...
public class LDAPUserGroupProvider extends BaseUserGroupProvider {
  ...
  @Override public JahiaUser getUser(String name) throws UserNotFoundException {
  ...
  @Override public JahiaGroup getGroup(String name) throws GroupNotFoundException {
  ...
  @Override public List<Member> getGroupMembers(String groupName) {
  ...
  @Override public List<String> getMembership(final Member member) {
    ...
    List<String> memberships = ldapTemplateWrapper.execute(new BaseLdapActionCallback<List<String>>(getExternalUserGroupService(), getKey()) {
      @Override public List<String> doInLdap(LdapTemplate ldapTemplate) {
        return ldapTemplate.search(applyPredefinedGroupFilter(query().base(groupConfig.getSearchName()).attributes(groupConfig.getSearchAttribute()).where(OBJECTCLASS_ATTRIBUTE).is(groupConfig.getSearchObjectclass()).and(groupConfig.getMembersAttribute()).like(dn)), new AttributesMapper<String>() {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Query getMembership for {} / {} dn={} in {} ms", new Object[] { member.getName(), groupConfig.getSearchAttribute(), dn, System.currentTimeMillis() - startTime });
    }
    if (memberships == null) {
  ...
  @Override public List<String> searchUsers(final Properties searchCriteria, long offset, long limit) {
    ...
    ldapTemplateWrapper.execute(new BaseLdapActionCallback<Object>(getExternalUserGroupService(), getKey()) {
      @Override public Object doInLdap(LdapTemplate ldapTemplate) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Search users for criteria {} using filter {} done in {} ms. Found {} entries.", new Object[] { searchCriteria, query.filter(), System.currentTimeMillis() - startTime, names.size() });
    }
    return names.subList(Math.min((int) offset, names.size()), limit < 0 ? names.size() : Math.min((int) (offset + limit), names.size()));
  }

  @Override public List<String> searchGroups(Properties searchCriteria, long offset, long limit) {
  ...
  @Override public boolean verifyPassword(String userName, String userPassword) {
  ...
  @Override public boolean isAvailable() throws RepositoryException {
    long startTime = System.currentTimeMillis();
    ...
    boolean available = ldapTemplateWrapper.execute(new BaseLdapActionCallback<Boolean>(getExternalUserGroupService(), getKey()) {
      @Override public Boolean doInLdap(LdapTemplate ldapTemplate) {
        ldapTemplate.search(buildUserQuery(new Properties()), new NameClassPairCallbackHandler() {
          @Override public void handleNameClassPair(NameClassPair nameClassPair) throws NamingException {
          }
        });
        return true;
      }
  ...
  private List<String> searchGroups(final Properties searchCriteria, boolean isDynamics) {
    ...
    List<String> names = searchNameClassPairCallbackHandler.getNames();
    if (logger.isDebugEnabled()) {
      logger.debug("Search groups for criteria {} using filter {} done in {} ms. Found {} entries.", new Object[] { searchCriteria, query.filter(), System.currentTimeMillis() - startTime, names.size() });
    }
    return names;
  }
  ...
  private List<Member> loadMembersFromUrl(String url) {
    try {
      ...
      if ("one".equalsIgnoreCase(ldapURL.getScope())) {
        ...
        if ("base".equalsIgnoreCase(ldapURL.getScope())) {
      ...
      ldapTemplateWrapper.execute(new BaseLdapActionCallback<Object>(getExternalUserGroupService(), getKey()) {
        @Override public Object doInLdap(LdapTemplate ldapTemplate) {
          ldapTemplate.search(query().base(ldapURL.getDN()).attributes(attrs.toArray(new String[attrs.size()])).searchScope(searchScope).filter(ldapURL.getFilter()), nameClassPairCallbackHandler);
          return null;
        }
      });
  ...
  private List<Member> loadMembersFromDN(final String groupDN) {
    ...
    NamingEnumeration<?> members = ldapTemplateWrapper.execute(new BaseLdapActionCallback<NamingEnumeration<?>>(getExternalUserGroupService(), getKey()) {
      @Override public NamingEnumeration<?> doInLdap(LdapTemplate ldapTemplate) {
        if (groupConfig.getAdRangeStep() > 0) {
          ...
          return ldapTemplate.lookup(groupName, new String[] { groupConfig.getMembersAttribute() }, new AttributesMapper<NamingEnumeration<?>>() {
            @Override public NamingEnumeration<?> mapFromAttributes(Attributes attributes) throws NamingException {
  ...
  private LDAPUserCacheEntry getUserCacheEntry(final String userName, boolean cache) {
    ...
    boolean validLdapCall = ldapTemplateWrapper.execute(new BaseLdapActionCallback<Boolean>(getExternalUserGroupService(), getKey()) {
      @Override public Boolean doInLdap(LdapTemplate ldapTemplate) {
        ldapTemplate.search(applyPredefinedUserFilter(query().base(userConfig.getUidSearchName()).attributes(userAttrs.toArray(new String[userAttrs.size()])).where(OBJECTCLASS_ATTRIBUTE).is(userConfig.getSearchObjectclass()).and(userConfig.getUidSearchAttribute()).is(decode(userName)), true), nameClassPairCallbackHandler);
        return true;
      }
  ...
  private LDAPGroupCacheEntry getGroupCacheEntryByName(final String name, boolean cache, final boolean isDynamic) throws Exception {
    ...
    boolean validLdapCall = ldapTemplateWrapper.execute(new BaseLdapActionCallback<Boolean>(getExternalUserGroupService(), getKey()) {
      @Override public Boolean doInLdap(LdapTemplate ldapTemplate) {
        ldapTemplate.search(applyPredefinedGroupFilter(query().base(groupConfig.getSearchName()).attributes(groupAttrs.toArray(new String[groupAttrs.size()])).where(OBJECTCLASS_ATTRIBUTE).is(isDynamic ? groupConfig.getDynamicSearchObjectclass() : groupConfig.getSearchObjectclass()).and(groupConfig.getSearchAttribute()).is(decode(name))), nameClassPairCallbackHandler);
        return true;
      }
  ...
  private LDAPGroupCacheEntry getGroupCacheEntryByDN(final String dn, boolean cache, final boolean isDynamic) {
    ...
    ldapTemplateWrapper.execute(new BaseLdapActionCallback<Object>(getExternalUserGroupService(), getKey()) {
      @Override public Object doInLdap(LdapTemplate ldapTemplate) {
        ldapTemplate.search(applyPredefinedGroupFilter(query().base(dn).attributes(groupAttrs.toArray(new String[groupAttrs.size()])).searchScope(SearchScope.OBJECT).where(OBJECTCLASS_ATTRIBUTE).is(isDynamic ? groupConfig.getDynamicSearchObjectclass() : groupConfig.getSearchObjectclass())), nameClassPairCallbackHandler);
        return null;
      }
    });
    if (logger.isDebugEnabled()) {
  ...
  private LDAPUserCacheEntry getUserCacheEntryByDN(final String dn, boolean cache) {
    ...
    ldapTemplateWrapper.execute(new BaseLdapActionCallback<Object>(getExternalUserGroupService(), getKey()) {
      @Override public Object doInLdap(LdapTemplate ldapTemplate) {
        ldapTemplate.search(applyPredefinedUserFilter(query().base(dn).attributes(userAttrs.toArray(new String[userAttrs.size()])).searchScope(SearchScope.OBJECT).where(OBJECTCLASS_ATTRIBUTE).is(userConfig.getSearchObjectclass()), true), nameClassPairCallbackHandler);
        return null;
      }
    });
    if (logger.isDebugEnabled()) {
    ...
    @Override public void handleNameClassPair(NameClassPair nameClassPair) throws NamingException {
      if (nameClassPair instanceof SearchResult) {
        ...
        if (objectclasses.contains(userConfig.getSearchObjectclass())) {
          ...
          if (objectclasses.contains(groupConfig.getSearchObjectclass())) {
            isUser = false;
          } else {
            if (groupConfig.isDynamicEnabled() && objectclasses.contains(groupConfig.getDynamicSearchObjectclass())) {
              isUser = false;
              isDynamic = true;
            }
          }
        ...
        if (commonUserAttrs.contains(userConfig.getUidSearchAttribute()) && commonUserAttrs.size() > commonGroupAttrs.size()) {
          ...
          if (commonGroupAttrs.contains(groupConfig.getSearchAttribute())) {
  ...
  private LDAPUserCacheEntry attributesToUserCacheEntry(Attributes attrs, LDAPUserCacheEntry userCacheEntry) throws NamingException {
    ...
    if (uidAttr == null) {
      logger.warn("LDAP user entry is missing the required {} attribute. Skipping user. Available attributes: {}", userConfig.getUidSearchAttribute(), attrs);
      return null;
    }
  ...
  private ContainerCriteria buildUserQuery(Properties searchCriteria) {
    ...
    ContainerCriteria query = query().base(userConfig.getUidSearchName()).attributes(attributesToRetrieve.toArray(new String[attributesToRetrieve.size()])).countLimit((int) userConfig.getSearchCountlimit()).where(OBJECTCLASS_ATTRIBUTE).is(StringUtils.defaultString(userConfig.getSearchObjectclass(), "*"));
  ...
  private ContainerCriteria buildGroupQuery(Properties searchCriteria, boolean isDynamic) {
    ...
    ContainerCriteria query = query().base(groupConfig.getSearchName()).attributes(attributesToRetrieve.toArray(new String[attributesToRetrieve.size()])).countLimit((int) groupConfig.getSearchCountlimit()).where(OBJECTCLASS_ATTRIBUTE).is(isDynamic ? groupConfig.getDynamicSearchObjectclass() : groupConfig.getSearchObjectclass());
  ...
  private Properties mapJahiaPropertiesToLDAP(Properties searchCriteria, Map<String, String> configProperties) {
    ...
    for (Map.Entry<Object, Object> entry : searchCriteria.entrySet()) {
      if (configProperties.containsKey(entry.getKey())) {
        ...
        if (!entry.getKey().equals("*") && !entry.getKey().equals(JahiaUserManagerService.MULTI_CRITERIA_SEARCH_OPERATION)) {
          return null;
        }
      }
    }
  ...
  private Boolean guessUserOrGroupFromDN(String dn) throws InvalidNameException {
    ...
    if (memberLdapName.startsWith(new LdapName(userConfig.getUidSearchName()))) {
      ...
      if (memberLdapName.startsWith(new LdapName(groupConfig.getSearchName()))) {
  ...
  @Override protected String getSiteKey() {
  ...
  @Override public boolean supportsGroups() {
  ...
  public abstract class BaseLdapActionCallback<T extends java.lang.Object> implements LdapTemplateCallback<T> {
    ...
    @Override public T onError(Exception e) {
