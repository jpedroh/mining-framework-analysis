...
public abstract class AbstractHpiMojo extends AbstractJenkinsMojo {
  /**
     ...
     */
  @Parameter(defaultValue = "${project.build.directory}") protected String outputDirectory;
  ...
  /**
     ...
     */
  @Parameter(defaultValue = "${project.name}", readonly = true) protected String pluginName;
  ...
  /**
     ...
     */
  @Parameter(defaultValue = "${plugin.version.description}") protected String pluginVersionDescription;
  ...
  /**
     ...
     * @since 2.4
     */
  @Parameter(defaultValue = "true") protected boolean failOnVersionOverrideToDifferentRelease = true;

  /**
     * The directory where the webapp is built.
     */
  @Parameter(defaultValue = "${project.build.directory}/${project.build.finalName}") private File webappDirectory;
  ...
  /**
     ...
     */
  @Parameter private Resource[] webResources;

  @Parameter(defaultValue = "${project.build.filters}") private List<String> filters;

  /**
     * The path to the context.xml file to use.
     */
  @Parameter(defaultValue = "${maven.war.containerConfigXML}") private File containerConfigXML;

  /**
     * Directory to unpack dependent WARs into if needed
     */
  @Parameter(defaultValue = "${project.build.directory}/war/work") private File workDirectory;
  ...
  /**
     ...
     */
  @Parameter(alias = "excludes") private String warSourceExcludes;

  /**
     * The comma separated list of tokens to include when doing
     * a war overlay.
     */
  @Parameter(defaultValue = "**") private String dependentWarIncludes;
  ...
  public void copyResourcesWithFiltering(List<Resource> resources, File webappDirectory) throws MavenFilteringException {
    MavenResourcesExecution mavenResourcesExecution = new MavenResourcesExecution(resources, webappDirectory, project, StandardCharsets.UTF_8.name(), filters, Collections.emptyList(), session);
    ...
    if (!sourceDirectory.equals(webappDirectory)) {
      ...
      if (warSourceDirectory.exists()) {
        ...
        for (String fileName : fileNames) {
          FileUtils.copyFileIfModified(new File(sourceDirectory, fileName), new File(webappDirectory, fileName));
        }
      }
    }
  }
  ...
  private File unpackWarToTempDirectory(MavenArtifact artifact) throws MojoExecutionException {
    ...
    if (!Files.isDirectory(tempLocation.toPath())) {
      ...
      if (artifact.getFile().lastModified() > tempLocation.lastModified()) {
        process = true;
      }
    }
    if (process) {
...
<<<<<<< /usr/src/app/output/jenkinsci/maven-hpi-plugin/a5d3499521e6cd61f208e2337137f4fe4f90ac40/src/main/java/org/jenkinsci/maven/plugins/hpi/AbstractHpiMojo.java/left.java
    return v;
=======
    try {
...
>>>>>>> /usr/src/app/output/jenkinsci/maven-hpi-plugin/a5d3499521e6cd61f208e2337137f4fe4f90ac40/src/main/java/org/jenkinsci/maven/plugins/hpi/AbstractHpiMojo.java/right.java
  }
  ...
  /**
     ...
     */
  @CheckForNull protected Boolean isSupportDynamicLoading() throws IOException {
    try (URLClassLoader cl = new URLClassLoader(new URL[] { new File(project.getBuild().getOutputDirectory()).toURI().toURL() }, getClass().getClassLoader())) {
