 ...
 */
@Mojo(name = "run", requiresDependencyResolution = ResolutionScope.TEST) @Execute(phase = LifecyclePhase.COMPILE) public class RunMojo extends AbstractJettyMojo {
  ...
  /**
     ...
     * </p>
     */
  @Parameter(property = "webAppFile") private File webAppFile;

  /**
     * Path to {@code $JENKINS_HOME}. The launched Jenkins will use this directory as the workspace.
     */
  @Parameter(property = "hudsonHome", defaultValue = "${HUDSON_HOME}") private File hudsonHome;

  /**
     * Path to {@code $JENKINS_HOME}. The launched Jenkins will use this directory as the workspace.
     */
  @Parameter(property = "jenkinsHome", defaultValue = "${JENKINS_HOME}") private File jenkinsHome;
  ...
  /**
     ...
     * running Jenkins.
     */
  @Parameter(defaultValue = "test") protected String dependencyResolution;

  /**
     * Single directory for extra files to include in the WAR.
     */
  @Parameter(defaultValue = "${basedir}/src/main/webapp") protected File warSourceDirectory;
  ...
  /**
     ...
     * If connectors are configured in the Mojo, that'll take precedence.
     */
  @Parameter(property = "host", defaultValue = "localhost") protected String defaultHost;
  ...
  /**
     ...
     */
  @Parameter(property = "wildcardDNS") protected String wildcardDNS;

  /**
     * If true, the context will be restarted after a line feed on
     * the input console. Enabled by default.
     */
  @Parameter(property = "jetty.consoleForceReload", defaultValue = "true") protected boolean consoleForceReload;
  ...
  /**
     * @since 1.94
     */
  @Parameter protected boolean pluginFirstClassLoader = false;
  ...
  /**
     ...
     * @since 1.98
     */
  @Parameter private Map<String, String> loggers;
  ...
  /**
     ...
     */
  @Deprecated @Parameter private String minimumJavaVersion;
  ...
  /**
     ...
     */
  @Deprecated @Parameter(readonly = true, required = true, defaultValue = "/${project.artifactId}") protected String contextPath;

  @Component protected PluginWorkspaceMap pluginWorkspaceMap;
  ...
  @Override public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    Artifacts jenkinsArtifacts = Artifacts.of(getProject()).groupIdIs("org.jenkins-ci.main", "org.jvnet.hudson.main").artifactIdIsNot("remoting");
    if (webAppFile == null) {
  ...
  private boolean hasSameGavAsProject(Artifact a) {
    return getProject().getGroupId().equals(a.getGroupId()) && getProject().getArtifactId().equals(a.getArtifactId()) && getProject().getVersion().equals(a.getVersion());
  }
  ...
  @Override public void configureWebApplication() throws Exception {
  ...
  @Override public void configureScanner() throws MojoExecutionException {
    ...
    scannerListeners.add(new Scanner.BulkListener() {
      @Override public void filesChanged(List<String> changes) {
        try {
  ...
  @Override public void restartWebApp(boolean reconfigureScanner) throws Exception {
  ...
  @Override protected void startConsoleScanner() throws Exception {
  ...
  @Override public void checkPomConfiguration() throws MojoExecutionException {
  }

  @Override public void finishConfigurationBeforeStart() throws Exception {
    ...
    try {
      WebAppClassLoader wacl = new WebAppClassLoader(new JettyAndServletApiOnlyClassLoader(ClassLoader.getPlatformClassLoader(), getClass().getClassLoader()), wac) {
        ...
        @Override public void addJars(org.eclipse.jetty.util.resource.Resource lib) {
          super.addJars(lib);
        }
      };
  ...
  @Override public void startJetty() throws MojoExecutionException {
  ...
  protected Set<Artifact> resolveDependencies(String scope) throws MojoExecutionException {
    try {
      DependencyResolutionRequest request = new DefaultDependencyResolutionRequest(getProject(), session.getRepositorySession());
      ...
      if (result.getDependencyGraph() != null && !result.getDependencyGraph().getChildren().isEmpty()) {
        RepositoryUtils.toArtifacts(artifacts, result.getDependencyGraph().getChildren(), List.of(getProject().getArtifact().getId()), request.getResolutionFilter());
      }
      return artifacts;
