...
public class IrcBotImpl extends PircBot {
  ...
  private String getSummary(String number) throws ServiceException, IOException {
    ...
    return String.format("%s:%s (%s) %s", issue.getKey(), issue.getSummary(), findStatus(svc, token, issue.getStatus()).getName(), "http://jenkins-ci.org/issue/" + number);
  }
