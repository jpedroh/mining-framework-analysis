...
public class IrcListener extends ListenerAdapter {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) private final Map<String, Long> recentIssues = Collections.<String, Long>synchronizedMap(new HashMap<String, Long>(10));
  ...
  @Override public void onMessage(MessageEvent e) {
    ...
    if (!IrcBotConfig.getChannels().contains(channel.getName())) {
      return;
    }
  ...
  /**
     ...
     */
  @SuppressFBWarnings(value = "DM_EXIT", justification = "Intentionally restarting the app") private void restart(Channel channel, User sender) {
  ...
  private boolean createComponent(Channel channel, User sender, String subcomponent, String owner) {
    ...
    OutputChannel out = channel.send();
    out.message(String.format("Adding a new JIRA subcomponent %s to the %s project, owned by %s", subcomponent, IrcBotConfig.JIRA_DEFAULT_PROJECT, owner));
    boolean result = false;
    JiraRestClient client = null;
    try {
      client = JiraHelper.createJiraClient();
      final ComponentRestClient componentClient = client.getComponentClient();
      final Promise<Component> createComponent = componentClient.createComponent(IrcBotConfig.JIRA_DEFAULT_PROJECT, new ComponentInput(subcomponent, "subcomponent", owner, AssigneeType.COMPONENT_LEAD));
  ...
  private void renameComponent(Channel channel, User sender, String oldName, String newName) {
    ...
    try {
      ...
      Promise<Component> updateComponent = componentClient.updateComponent(component.getSelf(), new ComponentInput(newName, null, null, null));
  ...
  /**
     ...
     */
  private void setDefaultAssignee(Channel channel, User sender, List<String> subcomponents, @CheckForNull String owner) {
    if (!isSenderAuthorized(channel, sender)) {
      insufficientPermissionError(channel);
      return;
    }
    ...
    try {
      ...
      for (String subcomponent : subcomponents) {
        try {
          ...
          Promise<Component> updateComponent = client.getComponentClient().updateComponent(component.getSelf(), new ComponentInput(null, null, owner != null ? owner : "", AssigneeType.COMPONENT_LEAD));
          JiraHelper.wait(updateComponent);
          out.message(owner != null ? String.format("Default assignee set to %s for %s", owner, subcomponent) : "Default assignee has been removed for " + subcomponent);
  ...
  private void setComponentDescription(Channel channel, User sender, String componentName, @CheckForNull String description) {
    ...
    try {
      ...
      Promise<Component> updateComponent = client.getComponentClient().updateComponent(component.getSelf(), new ComponentInput(null, description != null ? description : "", null, null));
      JiraHelper.wait(updateComponent);
  ...
  private static GHTeam getOrCreateRepoLocalTeam(OutputChannel out, GitHub github, GHOrganization org, GHRepository r, List<String> githubUsers) throws IOException {
    ...
    if (t == null) {
      ...
      if (!githubUsers.isEmpty()) {
        maintainers = githubUsers.stream().filter((user) -> isMemberOfOrg(github, org, user)).collect(Collectors.toList());
  ...
  private static List<String> collectGroups(Matcher m, int startingGroup) {
    List<String> items = new ArrayList<>(Arrays.asList(m.group(startingGroup).split("\\s*,\\s*")));
    return items;
  }

  public static void main(String[] args) throws Exception {
    Configuration.Builder builder = new Configuration.Builder().setName(IrcBotConfig.NAME).addServer(IrcBotConfig.SERVER, 6667).setAutoReconnect(true).addListener(new IrcListener(new File("unknown-commands.txt")));
    for (String channel : IrcBotConfig.getChannels()) {
      builder.addAutoJoinChannel(channel);
    }
    if (args.length > 0) {
      builder.setCapEnabled(true).addCapHandler(new SASLCapHandler(IrcBotConfig.NAME, args[0]));
    }
