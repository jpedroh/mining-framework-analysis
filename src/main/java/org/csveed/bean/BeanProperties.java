...
public class BeanProperties implements Iterable<BeanProperty> {
  ...
  @SuppressWarnings(value = { "unchecked" }) private void addProperty(PropertyDescriptor propertyDescriptor, Field field) {
  ...
  public void setLocalizedNumber(String propertyName, Locale locale) {
    ...
    if (numberClass == null) {
      throw new CsvException(new GeneralError("Property " + beanClass.getName() + "." + propertyName + " is not a java.lang.Number"));
    }
  ...
  @SuppressWarnings(value = { "StatementWithEmptyBody" }) protected void removeFromColumnIndex(BeanProperty property) {
    while (indexToProperty.values().remove(property)) {
      ;
    }
  }

  @SuppressWarnings(value = { "StatementWithEmptyBody" }) protected void removeFromColumnName(BeanProperty property) {
    while (nameToProperty.values().remove(property)) {
      ;
    }
  }

  @SuppressWarnings(value = { "StatementWithEmptyBody" }) public void ignoreProperty(String propertyName) {
  ...
  protected BeanProperty get(String propertyName) {
    ...
    throw new CsvException(new GeneralError("Property does not exist: " + beanClass.getName() + "." + propertyName));
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override public Iterator<BeanProperty> iterator() {
