 ...
 * @since 1.3
 */
@FacesComponent(value = CommandScript.COMPONENT_TYPE) @ResourceDependency(library = "javax.faces", name = "jsf.js", target = "head") public class CommandScript extends UICommand {
    ...
    if (getChildCount() > 0) {
      for (UIComponent child : getChildren()) {
        if (child instanceof UIParameter) {
          ...
          if (!isEmpty(name)) {
            writer.append("o[").append(Json.encode(name)).append("]=").append(Json.encode(param.getValue())).append(";");
          }
        }
      }
    }
=======
    for (ParamHolder param : Components.getParams(this)) {
      writer.append("o[").append(Json.encode(param.getName())).append("]=").append(Json.encode(param.getValue())).append(";");
    }
  ...
  protected String resolveClientIds(FacesContext context, String relativeClientIds) {
    ...
    for (String relativeClientId : relativeClientIds.split("\\s+")) {
      ...
      if (relativeClientId.charAt(0) == '@') {
        ...
        if (found == null) {
          throw new IllegalArgumentException(String.format(ERROR_UNKNOWN_CLIENTID, relativeClientId, getNamingContainer().getClientId(context)));
        }
