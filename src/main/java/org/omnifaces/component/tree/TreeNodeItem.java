 ...
 */
@FacesComponent(value = TreeNodeItem.COMPONENT_TYPE) public class TreeNodeItem extends TreeFamily {
  ...
  /**
	 ...
	 */
  @Override @SuppressWarnings(value = { "rawtypes", "unchecked" }) protected void process(final FacesContext context, final PhaseId phaseId) {
    if (getChildCount() == 0) {
      return;
    }
    process(context, new Callback.ReturningWithArgument<Void, Tree>() {
      @Override public Void invoke(Tree tree) {
  ...
  /**
	 ...
	 * @see Tree#setCurrentModelNode(FacesContext, TreeModel)
	 */
  @Override @SuppressWarnings(value = { "rawtypes", "unchecked" }) public boolean visitTree(final VisitContext context, final VisitCallback callback) {
    ...
    return process(context.getFacesContext(), new Callback.ReturningWithArgument<Boolean, Tree>() {
      @Override public Boolean invoke(Tree tree) {
        if (tree.getCurrentModelNode() != null) {
          for (TreeModel childModelNode : (Iterable<TreeModel>) tree.getCurrentModelNode()) {
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "rawtypes" }) private <R extends java.lang.Object> R process(FacesContext context, Callback.ReturningWithArgument<R, Tree> callback) {
