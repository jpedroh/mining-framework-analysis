...
public class RedisCache<K extends java.lang.Object, V extends java.lang.Object> implements Cache<K, V> {
  ...
  /**
	 ...
	 */
  public RedisCache(IRedisManager cache, RedisSerializer keySerializer, RedisSerializer valueSerializer, String prefix) {
    this(cache, keySerializer, valueSerializer);
    this.keyPrefix = prefix;
  }

  @Override public V get(K key) throws CacheException {
  ...
  @Override public V put(K key, V value) throws CacheException {
  ...
  @Override public V remove(K key) throws CacheException {
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override public Set<K> keys() {
    Set<byte[]> keys = null;
    try {
      keys = redisManager.keys(keySerializer.serialize(this.keyPrefix + "*"));
    } catch (SerializationException e) {
