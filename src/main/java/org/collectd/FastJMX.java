...
public class FastJMX implements CollectdConfigInterface, CollectdInitInterface, CollectdReadInterface, CollectdShutdownInterface, NotificationListener {
  ...
  private static List<AttributePermutation> collectablePermutations = Collections.synchronizedList(new ArrayList<AttributePermutation>(100));
  ...
  public int config(final OConfigItem ci) {
    ...
    for (OConfigItem pluginChild : ci.getChildren()) {
      if ("maxthreads".equalsIgnoreCase(pluginChild.getKey())) {
        ...
        if ("collectinternal".equalsIgnoreCase(pluginChild.getKey())) {
          collectInternal = getConfigBoolean(pluginChild);
        } else {
          if ("forceLoggingTo".equalsIgnoreCase(pluginChild.getKey())) {
            ...
            if ("loglevel".equalsIgnoreCase(pluginChild.getKey())) {
              ...
              if ("mbean".equalsIgnoreCase(pluginChild.getKey()) || "mxbean".equalsIgnoreCase(pluginChild.getKey()) || "bean".equalsIgnoreCase(pluginChild.getKey())) {
                ...
                for (OConfigItem beanChild : pluginChild.getChildren()) {
                  if ("objectname".equalsIgnoreCase(beanChild.getKey())) {
                    ...
                    if ("instanceprefix".equalsIgnoreCase(beanChild.getKey())) {
                      beanInstancePrefix = getConfigString(beanChild);
                    } else {
                      if ("instancefrom".equalsIgnoreCase(beanChild.getKey())) {
                        beanInstanceFrom.add(getConfigString(beanChild));
                      } else {
                        if ("value".equalsIgnoreCase(beanChild.getKey())) {
                          ...
                          for (OConfigItem valueChild : beanChild.getChildren()) {
                            if ("attribute".equalsIgnoreCase(valueChild.getKey())) {
                              ...
                              if ("type".equalsIgnoreCase(valueChild.getKey())) {
                                valueDs = Collectd.getDS(getConfigString(valueChild));
                              } else {
                                if ("instanceprefix".equalsIgnoreCase(valueChild.getKey())) {
                                  valueInstancePrefix = getConfigString(valueChild);
                                } else {
                                  if ("instancefrom".equalsIgnoreCase(valueChild.getKey())) {
                                    valueInstanceFrom.add(getConfigString(valueChild));
                                  } else {
                                    if ("table".equalsIgnoreCase(valueChild.getKey()) || "composite".equalsIgnoreCase(valueChild.getKey())) {
                                      composite = getConfigBoolean(valueChild);
                                    } else {
                                      if ("pluginname".equalsIgnoreCase(valueChild.getKey())) {
                          ...
                          beanAttributes.add(new Attribute(valueAttributes, pluginName, valueDs, valueInstancePrefix, valueInstanceFrom, composite, beanAlias, matchObjectName, beanInstancePrefix, beanInstanceFrom));
                          if (valueDs.getDataSources().size() == valueAttributes.size()) {
                            attributes.addAll(beanAttributes);
                          } else {
                            logger.severe("The data set for bean \'" + beanAlias + "\' of type \'" + valueDs.getType() + "\' has " + valueDs.getDataSources().size() + " data sources, but there were " + valueAttributes.size() + " attributes configured. This bean will not be collected!");
                          }
                        }
                      }
                    }
                ...
                if ("connection".equalsIgnoreCase(pluginChild.getKey())) {
                  ...
                  for (OConfigItem connectionChild : pluginChild.getChildren()) {
                    if ("user".equalsIgnoreCase(connectionChild.getKey()) || "username".equalsIgnoreCase(connectionChild.getKey())) {
                      ...
                      if ("password".equalsIgnoreCase(connectionChild.getKey())) {
                        password = getConfigString(connectionChild);
                      } else {
                        if ("instanceprefix".equalsIgnoreCase(connectionChild.getKey())) {
                          connectionInstancePrefix = getConfigString(connectionChild);
                        } else {
                          if ("serviceurl".equalsIgnoreCase(connectionChild.getKey())) {
                            ...
                            if ("collect".equalsIgnoreCase(connectionChild.getKey())) {
                              beanAliases.add(getConfigString(connectionChild).toLowerCase());
                            } else {
                              if ("includeportinhostname".equalsIgnoreCase(connectionChild.getKey())) {
                                hostnamePort = getConfigBoolean(connectionChild);
                              } else {
                                if ("ttl".equalsIgnoreCase("ttl")) {
                                  ttl = getConfigNumber(connectionChild, ttl).longValue();
                                } else {
                                  if ("synchronous".equalsIgnoreCase("synchronous")) {
                    ...
                    if (!beanAliases.isEmpty()) {
                      if (hostName == null) {
                        ...
                        if ((hostName == null || "".equals(hostName)) && rawUrl.lastIndexOf("://") > rawUrl.lastIndexOf(":///")) {
                          try {
                            JMXServiceURL shortUrl = new JMXServiceURL(rawUrl.substring(0, rawUrl.indexOf(":///")) + rawUrl.substring(rawUrl.lastIndexOf("://")));
  ...
  private void createPermutations(final Connection connection) {
    ...
    for (Attribute attrib : attributes) {
      if (connection.getBeanAliases().contains(attrib.getBeanAlias())) {
        ...
        try {
          Set<ObjectName> instances = connection.getServerConnection().queryNames(attrib.getObjectName(), null);
          if (logger.isLoggable(Level.FINE)) {
