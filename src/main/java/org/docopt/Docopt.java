...
public final class Docopt {
  /**
	 ...
	 * </pre>
	 */
  private static List<Option> parseLong(final Tokens tokens, final List<Option> options) {
    String $long;
    String eq;
    String value;
    {
    ...
    if (similar.size() > 1) {
      ...
      throw tokens.error("%s is not a unique prefix: %s?", $long, join(", ", u));
    }
    Option o;
    if (similar.size() < 1) {
    ...
    } else {
      {
        ...
        o = new Option(u.getShort(), u.getLong(), u.getArgCount(), u.getValue());
      }
      if (o.getArgCount() == 0) {
        if (value != null) {
  ...
  /**
	 ...
	 * </pre>
	 */
  private static List<Option> parseShorts(final Tokens tokens, final List<Option> options) {
  ...
  private static Required parsePattern(final String source, final List<Option> options) {
  ...
  /**
	 ...
	 * </pre>
	 */
  private static List<? extends Pattern> parseExpr(final Tokens tokens, final List<Option> options) {
    List<Pattern> seq = parseSeq(tokens, options);
    if (!"|".equals(tokens.current())) {
      return seq;
    }
    final List<Pattern> result = (seq.size() > 1) ? list((Pattern) new Required(seq)) : seq;
  ...
  /**
	 ...
	 * </pre>
	 */
  private static List<Pattern> parseSeq(final Tokens tokens, final List<Option> options) {
  ...
  /**
	 ...
	 * </pre>
	 */
  private static List<? extends Pattern> parseAtom(final Tokens tokens, final List<Option> options) {
  ...
  /**
	 ...
	 * </pre>
	 */
  private static List<LeafPattern> parseArgv(final Tokens tokens, final List<Option> options, final boolean optionsFirst) {
  ...
  private static List<String> parseSection(final String name, final String source) {
    {
  ...
  private static void extras(final boolean help, final String version, final List<? extends LeafPattern> options, final String doc) {
    boolean u;
    {
      u = false;
      if (help) {
  ...
  private Map<String, Object> doParse(final List<String> argv) {
    final List<LeafPattern> $argv = parseArgv(Tokens.withExitException(argv), list(options), optionsFirst);
    final Set<Pattern> patternOptions = set(pattern.flat(Option.class));
    for (final Pattern optionsShortcut : pattern.flat(OptionsShortcut.class)) {
      {
        final List<Pattern> u = ((BranchPattern) optionsShortcut).getChildren();
        u.clear();
  ...
  /**
	 ...
	 */
  public Map<String, Object> parse(final List<String> argv) throws DocoptExitException {
    try {
      return doParse(argv);
      ...
      @SuppressWarnings(value = { "resource" }) final PrintStream ps = (e.getExitCode() == 0) ? out : err;
      if (ps != null) {
