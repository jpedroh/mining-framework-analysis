...
public class JDBCNodeStore implements NodeStore {
    ...
    try {
      if (subscription.getSubscription().equals(Subscriptions.none)) {
        ...
        if (rows == 0) {
          ...
          addStatement.setString(5, subscription.getInviter().toBareJID());
          addStatement.executeUpdate();
          addStatement.close();
        }
        t.commit();
  ...
  @Override public NodeSubscription getUserSubscription(String nodeId, JID user) throws NodeStoreException {
    ...
    try {
      ...
      java.sql.ResultSet rs = selectStatement.executeQuery();
      if (rs.next()) {
        subscription = new NodeSubscriptionImpl(nodeId, new JID(rs.getString(2)), new JID(rs.getString(3)), Subscriptions.valueOf(rs.getString(4)), rs.getTimestamp(5), rs.getString(6));
      } else {
  ...
  @Override public ResultSet<NodeSubscription> getUserSubscriptions(final JID user) throws NodeStoreException {
    ...
    try {
      ...
      while (rs.next()) {
        NodeSubscriptionImpl nodeSub = new NodeSubscriptionImpl(rs.getString(1), new JID(rs.getString(2)), new JID(rs.getString(3)), Subscriptions.valueOf(rs.getString(4)), rs.getTimestamp(5), rs.getString(6));
        result.add(nodeSub);
      }
