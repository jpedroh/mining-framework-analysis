...
public class JDBCNodeStore implements NodeStore {
  ...
  @Override public CloseableIterator<NodeItem> getRecentItems(JID user, Date since, int maxPerNode, int limit, String afterItemId, String node) throws NodeStoreException {
    PreparedStatement stmt = null;
    try {
      if (null != afterItemId) {
      ...
      stmt = conn.prepareStatement("SELECT * FROM (" + StringUtils.join(queryParts, " UNION ALL ") + ") AS recentItemsQuery ORDER BY \"updated\" DESC LIMIT ?;");
      int index = 1;
      for (Object parameter : parameters) {
        stmt.setObject(index, parameter);
        ++index;
