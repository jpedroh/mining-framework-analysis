...
public class RetractItemProcessor extends AbstractMessageProcessor {
  public RetractItemProcessor(BlockingQueue<Packet> outQueue, Properties configuration, ChannelManager channelManager) {
    super(channelManager, configuration, outQueue);
  }

  @Override public void process(Message packet) throws Exception {
    message = packet;
  ...
  private void handleItem() throws NodeStoreException {
    deleteItem(message.getElement().element("event").element("items").element("retract").attributeValue("id"));
  }
