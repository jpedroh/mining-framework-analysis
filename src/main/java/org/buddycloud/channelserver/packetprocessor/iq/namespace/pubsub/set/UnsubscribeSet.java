...
public class UnsubscribeSet extends PubSubElementProcessorAbstract {
  ...
  @Override public void process(Element elm, JID actorJID, IQ reqIQ, Element rsm) throws Exception {
    ...
    NodeAffiliation existingAffiliation = channelManager.getUserAffiliation(node, unsubscribingJid);
    String fromJID = request.getFrom().toBareJID();
    if (false == unsubscribingJid.equals(existingSubscription.getUser())) {
      createExtendedErrorReply(PacketError.Type.auth, PacketError.Condition.forbidden, CAN_NOT_UNSUBSCRIBE_ANOTHER_USER, JabberPubsub.NS_BUDDYCLOUD);
      outQueue.put(response);
      return;
    }
    if ((Affiliations.owner == existingAffiliation.getAffiliation()) && (channelManager.getNodeOwners(node).size() < 2)) {
      createExtendedErrorReply(PacketError.Type.cancel, PacketError.Condition.not_allowed, MUST_HAVE_ONE_OWNER, JabberPubsub.NS_BUDDYCLOUD);
      outQueue.put(response);
      return;
    }
    ...
    if (false == Affiliations.outcast.toString().equals(existingAffiliation.getAffiliation().toString())) {
