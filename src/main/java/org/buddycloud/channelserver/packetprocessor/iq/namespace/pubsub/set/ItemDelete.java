...
public class ItemDelete extends PubSubElementProcessorAbstract {
  ...
  @Override public void process(Element elm, JID actor, IQ reqIQ, Element rsm) throws InterruptedException, NodeStoreException {
    element = elm;
    request = reqIQ;
  ...
  private void sendNotifications(String node, GlobalItemID itemId) throws NodeStoreException {
    try {
      ...
      for (NodeSubscription subscription : subscriptions) {
        logger.debug("Subscription [node: " + subscription.getNodeId() + ", listener: " + subscription.getListener() + ", subscription: " + subscription.getSubscription() + "]");
  ...
  private boolean itemIdProvided() {
    String id = request.getElement().element("pubsub").element("retract").element("item").attributeValue("id");
