...
public class Publish extends PubSubElementProcessorAbstract {
  ...
  private void saveNodeItem() throws NodeStoreException {
    channelManager.addNodeItem(new NodeItemImpl(node, id, new Date(), entry.asXML(), inReplyTo));
  }
  ...
  private void extractItemDetails() throws InterruptedException {
    ...
    if (false == GlobalItemIDImpl.isGlobalId(globalItemId)) {
      globalItemId = new GlobalItemIDImpl(request.getTo(), node, globalItemId).toString();
    }
  ...
  private boolean userCanPost() throws NodeStoreException, InterruptedException {
    Subscriptions possibleExistingSubscription = channelManager.getUserSubscription(node, publishersJID).getSubscription();
    Affiliations possibleExistingAffiliation = channelManager.getUserAffiliation(node, publishersJID).getAffiliation();
    if ((false == possibleExistingSubscription.equals(Subscriptions.subscribed)) || (false == possibleExistingAffiliation.in(Affiliations.moderator, Affiliations.owner, Affiliations.publisher))) {
      response.setType(Type.error);
