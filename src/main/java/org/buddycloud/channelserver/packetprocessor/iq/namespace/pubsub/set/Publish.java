...
public class Publish extends PubSubElementProcessorAbstract {
  ...
  private boolean userCanPost() throws NodeStoreException, InterruptedException {
    ...
    if ((!membership.getSubscription().equals(Subscriptions.subscribed)) || (!membership.getAffiliation().in(Affiliations.moderator, Affiliations.owner, Affiliations.publisher))) {
      response.setType(Type.error);
