...
public class Publish extends PubSubElementProcessorAbstract {
  ...
  private boolean userCanPost() throws NodeStoreException, InterruptedException {
    ...
    if ((false == membership.getSubscription().equals(Subscriptions.subscribed)) || (false == membership.getAffiliation().in(Affiliations.moderator, Affiliations.owner, Affiliations.publisher))) {
      response.setType(Type.error);
