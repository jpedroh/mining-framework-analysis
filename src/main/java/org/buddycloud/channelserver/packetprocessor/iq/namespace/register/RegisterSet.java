...
public class RegisterSet implements PacketProcessor<IQ> {
  ...
  @SuppressWarnings(value = { "unused" }) private void userAlreadyRegistered() throws InterruptedException {
  ...
  private void autosubscribeToChannels(final JID from) {
    ...
    for (JID channel : channels) {
      ...
      try {
        ...
        if (configuration.getBooleanProperty(Configuration.CONFIGURATION_CHANNELS_AUTOSUBSCRIBE_AUTOAPPROVE, false) && Configuration.getInstance().isLocalJID(channel) && AccessModels.authorize.toString().equals(channelManager.getNodeConfValue(channelNodeId, Conf.ACCESS_MODEL))) {
