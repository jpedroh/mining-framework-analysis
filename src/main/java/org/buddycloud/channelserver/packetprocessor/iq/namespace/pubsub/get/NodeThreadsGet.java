...
public class NodeThreadsGet extends PubSubElementProcessorAbstract {
  ...
  private boolean userCanViewNode() throws NodeStoreException {
    ...
    if (nodeViewAcl.canViewNode(node, channelManager.getNodeMembership(node, actor), getNodeAccessModel(nodeConfiguration), Configuration.getInstance().isLocalJID(actor))) {
      return true;
    }
