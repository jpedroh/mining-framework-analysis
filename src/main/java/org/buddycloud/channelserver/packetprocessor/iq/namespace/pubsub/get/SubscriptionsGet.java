...
public class SubscriptionsGet implements PubSubElementProcessor {
  ...
  private boolean getNodeMemberships(Element subscriptions) throws NodeStoreException, InterruptedException {
    ...
    if ((null != requestIq.getElement().element("pubsub").element("set")) && (0 == cur.size()) && (false == Configuration.getInstance().isLocalNode(node))) {
      makeRemoteRequest(new JID(node.split("/")[2]).getDomain());
      return false;
    }
    ...
    for (NodeMembership ns : cur) {
      ...
      subscription.addAttribute("node", ns.getNodeId()).addAttribute("subscription", ns.getSubscription().toString()).addAttribute("jid", ns.getUser().toBareJID());
  ...
  private boolean getUserMemberships(Element subscriptions) throws NodeStoreException, InterruptedException {
    ...
    if ((null != requestIq.getElement().element("pubsub").element("set")) && (0 == cur.size()) && (false == Configuration.getInstance().isLocalJID(actorJid))) {
