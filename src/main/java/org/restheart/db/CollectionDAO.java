...
class CollectionDAO {
  ...
  DBCursor getCollectionDBCursor(DBCollection coll, Deque<String> sortBy, Deque<String> filters) throws JSONParseException {
    ...
    if (sortBy == null || sortBy.isEmpty()) {
      ...
      sortBy.stream().forEach((s) -> {
        String _s = s.trim();
        _s = _s.replaceAll("_lastupdated_on", "_etag");
        if (_s.startsWith("-")) {
  ...
  ArrayList<DBObject> getCollectionData(DBCollection coll, int page, int pagesize, Deque<String> sortBy, Deque<String> filters, DBCursorPool.EAGER_CURSOR_ALLOCATION_POLICY eager, boolean detectOids) throws JSONParseException {
