...
class IndexDAO {
  ...
  List<BsonDocument> getCollectionIndexes(String dbName, String collName) {
    ...
    ListIndexesIterable<Document> indexes = client.getDatabase(dbName).getCollection(collName, BsonDocument.class).listIndexes();
  ...
  int deleteIndex(String dbName, String collection, String indexId) {
    client.getDatabase(dbName).getCollection(collection).dropIndex(indexId);
