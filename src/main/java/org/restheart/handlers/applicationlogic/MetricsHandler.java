...
public class MetricsHandler extends PipedHttpHandler {
  @VisibleForTesting Configuration configuration = Bootstrapper.getConfiguration();

  @VisibleForTesting SharedMetricRegistryProxy metrics = new SharedMetricRegistryProxy();
  ...
  MetricRegistry getMetricsRegistry(RequestContext context, METRICS_GATHERING_LEVEL metricsLevel) {
    switch (metricsLevel) {
      ...
      return metrics.registry();
      case DATABASE:
      return metrics.registry(context.getDBName());
      case COLLECTION:
      return metrics.registry(context.getDBName(), context.getCollectionName());
      default:
      return null;
    }
  }
  ...
  @Override public void handleRequest(HttpServerExchange exchange, RequestContext context) throws Exception {
    ...
    if (registry != null) {
      if (context.getMethod() == METHOD.GET) {
        ...
        ResponseType responseType = Optional.ofNullable(ResponseType.forQueryParameter(representationFormatParameters == null ? null : representationFormatParameters.getFirst())).orElseGet(() -> ResponseType.forAcceptHeader(exchange.getRequestHeaders().getFirst(Headers.ACCEPT)));
        if (responseType != null) {
          exchange.setStatusCode(HttpStatus.SC_OK);
          responseType.writeTo(exchange, metricsLevelForRequest, registry);
          exchange.endExchange();
        } else {
          String acceptableTypes = Arrays.stream(ResponseType.values()).map(ResponseType::getContentType).map((x) -> "\'" + x + "\'").collect(Collectors.joining(","));
          ResponseHelper.endExchangeWithMessage(exchange, context, HttpStatus.SC_NOT_ACCEPTABLE, "not acceptable, acceptable content types are: " + acceptableTypes);
  ...
  @VisibleForTesting enum ResponseType {
    JSON("application/json") {
      @Override public String generateResponse(METRICS_GATHERING_LEVEL metricsLevel, MetricRegistry registry) throws IOException {
        BsonDocument document = MetricsJsonGenerator.generateMetricsBson(registry, TimeUnit.SECONDS, TimeUnit.MILLISECONDS);
        return document.toJson(JsonWriterSettings.builder().outputMode(JsonMode.RELAXED).indent(true).build());
      }
    },
    ...
    @VisibleForTesting static String calculateMediaRange(String contentType) {
    ...
    public boolean isAcceptableFor(AcceptHeaderEntry entry) {
      return entry.contentType.equalsIgnoreCase("*/*") || (entry.contentType.equalsIgnoreCase(contentType) && (entry.specialization == null || entry.specialization.equalsIgnoreCase(specialization))) || entry.contentType.equalsIgnoreCase(mediaRange);
    }
    ...
    static class AcceptHeaderEntry {
      ...
      @Override public String toString() {
        return "AcceptHeaderEntry{" + "contentType=\'" + contentType + '\'' + ", specialization=\'" + specialization + '\'' + ", qValue=" + qValue + '}';
      }
    }

    static class AcceptHeaderEntryComparator implements Comparator<AcceptHeaderEntry>, Serializable {
      @Override public int compare(AcceptHeaderEntry one, AcceptHeaderEntry two) {
    ...
    public static ResponseType forAcceptHeader(String acceptHeader) {
      ...
      return Arrays.stream(acceptHeader.split(",")).map(String::trim).map(AcceptHeaderEntry::of).filter(Objects::nonNull).sorted(new AcceptHeaderEntryComparator()).flatMap((x) -> Arrays.stream(ResponseType.values()).filter((rt) -> rt.isAcceptableFor(x))).findFirst().orElse(null);
    }

    public static ResponseType forQueryParameter(String rep) {
      if (REPRESENTATION_FORMAT.SHAL.name().equalsIgnoreCase(rep) || REPRESENTATION_FORMAT.PLAIN_JSON.name().equalsIgnoreCase(rep) || REPRESENTATION_FORMAT.PJ.name().equalsIgnoreCase(rep) || REPRESENTATION_FORMAT.STANDARD.name().equalsIgnoreCase(rep) || REPRESENTATION_FORMAT.S.name().equalsIgnoreCase(rep)) {
