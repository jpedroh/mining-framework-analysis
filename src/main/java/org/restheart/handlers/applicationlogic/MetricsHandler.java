...
public class MetricsHandler extends PipedHttpHandler {
  ...
  @Override public void handleRequest(HttpServerExchange exchange, RequestContext context) throws Exception {
    ...
    if (registry != null) {
      if (context.getMethod() == METHOD.GET) {
        ResponseType responseType = Optional.ofNullable(ResponseType.forQueryParameter(exchange.getQueryParameters().get(REPRESENTATION_FORMAT_KEY) == null ? null : exchange.getQueryParameters().get(REPRESENTATION_FORMAT_KEY).getFirst())).orElseGet(() -> ResponseType.forAcceptHeader(exchange.getRequestHeaders().getFirst(Headers.ACCEPT)));
  ...
  @VisibleForTesting enum ResponseType {
    JSON("application/json") {
      @Override public String generateResponse(MetricRegistry registry) throws IOException {
