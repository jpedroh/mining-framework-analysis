...
public class PostBucketHandler extends PipedHttpHandler {
  ...
  @Override public void handleRequest(HttpServerExchange exchange, RequestContext context) throws Exception {
    ...
    if (!_metadata.isDocument()) {
      ResponseHelper.endExchangeWithMessage(exchange, context, HttpStatus.SC_NOT_ACCEPTABLE, "data cannot be an array");
      next(exchange, context);
      return;
    }
    ...
    try {
      if (context.getFilePath() != null) {
        result = gridFsDAO.createFile(getDatabase(), context.getDBName(), context.getCollectionName(), metadata, context.getFilePath());
      } else {
        ResponseHelper.endExchangeWithMessage(exchange, context, HttpStatus.SC_BAD_REQUEST, "POST file request is in a bad format");
        next(exchange, context);
        return;
      }
      ...
      ResponseHelper.endExchangeWithMessage(exchange, context, HttpStatus.SC_NOT_IMPLEMENTED, errMsg);
      next(exchange, context);
      return;
    }
    context.setDbOperationResult(result);
    exchange.getResponseHeaders().add(HttpString.tryFromString("Location"), RepUtils.getReferenceLink(context, URLUtils.getRemappedRequestURL(exchange), result.getNewId()));
