...
abstract public class ElementBox extends Box {
  ...
  /**
     ...
     */
  @Override public boolean collapsesSpaces() {
  ...
  /**
     ...
     */
  @Override public boolean preservesLineBreaks() {
  ...
  /**
     ...
     */
  @Override public boolean allowsWrapping() {
    return (whitespace == ElementBox.WHITESPACE_NORMAL || whitespace == ElementBox.WHITESPACE_PRE_WRAP || whitespace == ElementBox.WHITESPACE_PRE_LINE);
  }
  ...
  public int totalWidth() {
    return emargin.left + border.left + padding.left + content.width + padding.right + border.right + emargin.right;
  }

  public int getAvailableContentWidth() {
    return availwidth - emargin.left - border.left - padding.left - padding.right - border.right - emargin.right;
  }

  @Override public Rectangle getMinimalAbsoluteBounds() {
  ...
  @Override public boolean affectsDisplay() {
    boolean ret = !isEmpty();
  ...
  public Rectangle getAbsoluteBackgroundBounds() {
    return new Rectangle(absbounds.x + margin.left + border.left, absbounds.y + margin.top + border.top, content.width + padding.left + padding.right, content.height + padding.top + padding.bottom);
  }
  ...
  public Rectangle getAbsoluteBorderBounds() {
    return new Rectangle(absbounds.x + margin.left, absbounds.y + margin.top, content.width + padding.left + padding.right + border.left + border.right, content.height + padding.top + padding.bottom + border.top + border.bottom);
  }
  ...
  @Override public void drawExtent(Graphics2D g) {
  ...
  protected void loadBasicStyle() {
    ...
    if (display == null) {
      display = CSSProperty.Display.INLINE;
    }
    ...
    if (display == ElementBox.DISPLAY_NONE) {
      ...
      if (position == BlockBox.POS_ABSOLUTE || position == BlockBox.POS_FIXED) {
        floating = BlockBox.FLOAT_NONE;
      }
    }
    if (floating != BlockBox.FLOAT_NONE || position == BlockBox.POS_ABSOLUTE || position == BlockBox.POS_FIXED || isRootElement()) {
      if (display == DISPLAY_INLINE_TABLE) {
        ...
        if (display == DISPLAY_INLINE || display == DISPLAY_RUN_IN || display == DISPLAY_TABLE_ROW_GROUP || display == DISPLAY_TABLE_COLUMN || display == DISPLAY_TABLE_COLUMN_GROUP || display == DISPLAY_TABLE_HEADER_GROUP || display == DISPLAY_TABLE_FOOTER_GROUP || display == DISPLAY_TABLE_ROW || display == DISPLAY_TABLE_CELL || display == DISPLAY_TABLE_CAPTION || display == DISPLAY_INLINE_BLOCK) {
  ...
  /**
     ...
     */
  @Override protected void updateStackingContexts() {
