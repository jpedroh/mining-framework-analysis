 ...
 */
@Slf4j public class LogUnitServer implements IServer {
  /** The options map. */
  ...
  @Data class FileHandle {
    ...
    @Getter(lazy = true) private final MappedByteBuffer byteBuffer = getMappedBuffer();
  ...
  /**
     ...
     */
  @Getter long contiguousHead;

  @Getter long contiguousTail;
  ...
  @Data class Address implements Comparable<Address> {
  ...
  /**
     ...
     */
  private final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(1, new ThreadFactoryBuilder().setDaemon(true).setNameFormat("LogUnit-Maintenance-%d").build());

  public LogUnitServer(Map<String, Object> opts) {
    this.opts = opts;
    maxCacheSize = Utils.parseLong(opts.get("--max-cache"));
    if ((Boolean) opts.get("--memory")) {
      log.warn("Log unit opened in-memory mode (Maximum size={}). " + "This should be run for testing purposes only. " + "If you exceed the maximum size of the unit, old entries will be AUTOMATICALLY trimmed. " + "The unit WILL LOSE ALL DATA if it exits.", Utils.convertToByteStringRepresentation(maxCacheSize));
      reset();
    } else {
    ...
    reset();
    scheduler.scheduleAtFixedRate(this::compactTail, Utils.getOption(opts, "--compact", Long.class, 60L), Utils.getOption(opts, "--compact", Long.class, 60L), TimeUnit.SECONDS);
    gcThread = new Thread(this::runGC);
    gcThread.start();
  }

  @Data static class LogFileHeader {
  ...
  /** Write the header for a Corfu log file.
     ...
     * @throws IOException
     */
  public void writeHeader(FileChannel fc, AtomicLong pointer, int version, long flags) throws IOException {
  ...
  /** Read the header for a Corfu log file.
     ...
     * @throws IOException
     */
  public LogFileHeader readHeader(FileChannel fc) throws IOException {
  ...
  /** Write a log entry to a file.
     ...
     */
  public void writeEntry(FileHandle fh, long address, LogUnitEntry entry) throws IOException {
  ...
  /** Find a log entry in a file.
     ...
     */
  public LogUnitEntry readEntry(FileHandle fh, long address) throws IOException {
    ...
    while (o.hasRemaining()) {
      ...
      if (addr != address) {
        ...
        return new LogUnitEntry(Unpooled.wrappedBuffer(dBuf), LogUnitMetadataMsg.mapFromBuffer(mBuf), false, true);
      }
    }
    return null;
  }
  ...
  public FileHandle getChannelForAddress(long address) {
    return channelMap.computeIfAbsent(address / 10000, (a) -> {
      ...
      try {
        FileChannel fc = FileChannel.open(FileSystems.getDefault().getPath(filePath), EnumSet.of(StandardOpenOption.READ, StandardOpenOption.WRITE, StandardOpenOption.CREATE, StandardOpenOption.SPARSE));
        FileLock fl = fc.lock();
  ...
  @Override public void handleMessage(CorfuMsg msg, ChannelHandlerContext ctx, IServerRouter r) {
    switch (msg.getMsgType()) {
      ...
      case TRIM:
      {
        ...
        trimMap.compute(m.getStreamID(), (key, prev) -> prev == null ? m.getPrefix() : Math.max(prev, m.getPrefix()));
  ...
  @Override public void reset() {
    ...
    if (dataCache != null) {
      dataCache.asMap().values().parallelStream().map((m) -> m.buffer.release());
    }
    dataCache = Caffeine.newBuilder().<Long, LogUnitEntry>weigher((k, v) -> v.buffer == null ? 1 : v.buffer.readableBytes()).maximumWeight(maxCacheSize).removalListener(this::handleEviction).writer(new CacheWriter<Long, LogUnitEntry>() {
      @Override public void write(Long address, LogUnitEntry entry) {
        ...
        if (!entry.isPersisted && prefix != null) {
          try {
      ...
      @Override public void delete(Long aLong, LogUnitEntry logUnitEntry, RemovalCause removalCause) {
      }
    }).build(this::handleRetrieval);
    streamCache = Caffeine.newBuilder().maximumSize(Utils.getOption(opts, "--stream-cache", Long.class, 5L)).writer(new CacheWriter<UUID, RangeSet<Long>>() {
      @Override public void write(UUID streamID, RangeSet<Long> entry) {
        if (prefix != null) {
          try {
            ...
            b.writeInt(rs.size());
            for (Range<Long> r : rs) {
              Serializers.getSerializer(Serializers.SerializerType.JAVA).serialize(r, b);
            }
            com.google.common.io.Files.write(b.array(), new File(prefix + File.pathSeparator + "stream" + streamID.toString()));
          } catch (IOException ie) {
      ...
      @Override public void delete(UUID streamID, RangeSet<Long> entry, RemovalCause removalCause) {
      }
    }).build(this::handleStreamRetrieval);
    trimMap = new ConcurrentHashMap<>();
  }

  @SuppressWarnings(value = { "unchecked" }) public RangeSet<Long> handleStreamRetrieval(UUID stream) {
    if (prefix != null) {
      Path p = FileSystems.getDefault().getPath(prefix + File.pathSeparator + "stream" + stream.toString());
      try {
        if (Files.exists(p)) {
          ...
          for (int i = 0; i < ranges; i++) {
            Range r = (Range) Serializers.getSerializer(Serializers.SerializerType.JAVA).deserialize(b, null);
            rs.add(r);
          }
        }
      } catch (IOException ie) {
  ...
  public void read(CorfuRangeMsg msg, ChannelHandlerContext ctx, IServerRouter r) {
    ...
    e.entrySet().parallelStream().forEach((rv) -> o.put(rv.getKey(), new LogUnitReadResponseMsg(rv.getValue())));
  ...
  public void runGC() {
    ...
    val retry = IRetry.build(IntervalAndSentinelRetry.class, this::handleGC).setOptions((x) -> x.setSentinelReference(running)).setOptions((x) -> x.setRetryInterval(60_000));
    gcRetry = (IntervalAndSentinelRetry) retry;
    retry.runForever();
  }

  @SuppressWarnings(value = { "unchecked" }) public boolean handleGC() {
  ...
  @Override public void shutdown() {
    ...
    if (channelMap != null) {
      channelMap.entrySet().parallelStream().forEach((f) -> {
        try {
