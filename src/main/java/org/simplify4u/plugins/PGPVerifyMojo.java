...
@Mojo(name = "check", requiresProject = true, requiresDependencyResolution = ResolutionScope.TEST, defaultPhase = LifecyclePhase.VALIDATE, threadSafe = true) public class PGPVerifyMojo extends AbstractMojo {
  ...
  /**
     ...
     * @since 1.8.0
     */
  @Parameter(property = "pgpverify.proxyName") private String proxyName;

  @Parameter(property = "mavenSettings", defaultValue = "${settings}") private Settings settings;
  ...
  /**
     ...
     * @since 1.7.0
     */
  @Parameter(property = "pgpverify.keyserversLoadBalance", defaultValue = "true") private boolean pgpKeyServerLoadBalance;
  ...
  private void initCache() throws MojoFailureException {
    List<String> keyServerList = Arrays.stream(KEY_SERVERS_SPLIT_PATTERN.split(pgpKeyServer)).map(String::trim).filter((s) -> s.length() > 0).collect(Collectors.toList());
    try {
  ...
  private boolean verifyPGPSignature(Artifact artifact, Artifact ascArtifact) throws MojoFailureException {
    ...
    try {
      ...
      if (!keysMap.isValidKey(artifact, publicKey, publicKeyRing)) {
        String msg = String.format("%s = %s", ArtifactUtils.key(artifact), PublicKeyUtils.fingerprintForMaster(publicKey, publicKeyRing));
      ...
      getLog().debug("signature.KeyAlgorithm: " + pgpSignature.getKeyAlgorithm() + " signature.hashAlgorithm: " + pgpSignature.getHashAlgorithm());
      ...
      if (e.getCause() instanceof PGPKeyNotFound && keysMap.isKeyMissing(artifact)) {
        final String logMessage = String.format("%s PGP Key not found on server, consistent with keys map.", artifact.getId());
        if (quiet) {
  ...
  Proxy getMavenProxy() {
    if (settings != null) {
      ...
      if (proxies != null && !proxies.isEmpty()) {
        if (proxyName != null) {
          return proxies.stream().filter((proxy) -> proxyName.equalsIgnoreCase(proxy.getId())).findFirst().orElse(null);
        } else {
  ...
  private boolean verifySignatureStatus(boolean signatureStatus, Artifact artifact, PGPPublicKey publicKey, PGPPublicKeyRing publicKeyRing) {
    if (signatureStatus) {
      ...
      if (keysMap.isBrokenSignature(artifact)) {
        String logMessage = String.format("%s PGP Signature is broken, consistent with keys map.", artifact.getId());
        if (quiet) {
          getLog().debug(logMessage);
        } else {
          getLog().info(logMessage);
        ...
        getLog().error(String.format(PGP_VERIFICATION_RESULT_FORMAT, artifact.getId(), "INVALID", PublicKeyUtils.keyIdDescription(publicKey, publicKeyRing), PublicKeyUtils.getUserIDs(publicKey, publicKeyRing)));
        return false;
      }
    }
  }
