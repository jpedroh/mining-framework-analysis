 ...
 */
@Component(role = KeysMap.class, instantiationStrategy = "per-lookup") public class KeysMap {
  @Requirement private ResourceManager resourceManager;
  ...
  public void load(Log log, String locale) throws ResourceNotFoundException, IOException {
    ...
    if (locale != null && !locale.trim().isEmpty()) {
      try (InputStream inputStream = resourceManager.getResourceAsInputStream(locale)) {
  ...
  public boolean isNoSignature(Artifact artifact) {
    return keysMapList.stream().filter((artifactInfo) -> artifactInfo.isMatch(artifact)).anyMatch(ArtifactInfo::isNoSignature);
  }
  ...
  public boolean isBrokenSignature(Artifact artifact) {
    return keysMapList.stream().filter((artifactInfo) -> artifactInfo.isMatch(artifact)).anyMatch(ArtifactInfo::isBrokenSignature);
  }
  ...
  public boolean isKeyMissing(Artifact artifact) {
    return keysMapList.stream().filter((artifactInfo) -> artifactInfo.isMatch(artifact)).anyMatch(ArtifactInfo::isKeyMissing);
  }
  ...
  public boolean isValidKey(Artifact artifact, PGPPublicKey key, PGPPublicKeyRing keyRing) {
    ...
    return keysMapList.stream().filter((artifactInfo) -> artifactInfo.isMatch(artifact)).anyMatch((artifactInfo) -> artifactInfo.isKeyMatch(key, keyRing));
  }
