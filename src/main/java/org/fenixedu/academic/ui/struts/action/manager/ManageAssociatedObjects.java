...
@StrutsFunctionality(app = ManagerSystemManagementApp.class, path = "manage-associated-objects", titleKey = "title.manage.associated.objects") @Mapping(path = "/manageAssociatedObjects", module = "manager") @Forwards(value = { @Forward(name = "list", path = "/manager/listAssociatedObjects.jsp"), @Forward(name = "createDepartment", path = "/manager/createDepartment.jsp"), @Forward(name = "createEmptyDegree", path = "/manager/createEmptyDegree.jsp"), @Forward(name = "createScientificArea", path = "/manager/createScientificArea.jsp"), @Forward(name = "editDepartment", path = "/manager/editDepartment.jsp"), @Forward(name = "createCompetenceCourseGroup", path = "/manager/createCompetenceCourseGroup.jsp"), @Forward(name = "associatePersonUnit", path = "/manager/associatePersonUnit.jsp"), @Forward(name = "createAcademicOffice", path = "/manager/createAcademicOffice.jsp"), @Forward(name = "createDegreeType", path = "/manager/createDegreeType.jsp") }) public class ManageAssociatedObjects extends FenixDispatchAction {
  ...
  @EntryPoint public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
  ...
  @Atomic private void createDepartment(AssociatedObjectsBean bean) {
    ...
    Unit departmentParent = Bennu.getInstance().getInstitutionUnit().getSubUnits().stream().filter((x) -> x instanceof AggregateUnit).filter((x) -> ((AggregateUnit) x).getName().equals("Departments")).findAny().orElse(Bennu.getInstance().getInstitutionUnit());
  ...
  @Atomic private void createAcademicOffice(AssociatedObjectsBean bean) {
    ...
    Unit.createNewUnit(office.getName(), null, null, null, new YearMonthDay(), null, servicesParent, AccountabilityType.readByType(AccountabilityTypeEnum.ADMINISTRATIVE_STRUCTURE), null, UnitClassification.CENTRAL_ORG, office, false, bean.getBuilding());
  }
  ...
  @Atomic(mode = TxMode.WRITE) private void createEmptyDegree(AssociatedObjectsBean bean) {
  ...
  @Atomic(mode = TxMode.WRITE) private void createScientificArea(AssociatedObjectsBean bean) {
    ScientificAreaUnit.createNewInternalScientificArea(bean.getNameLS(), null, null, bean.getCode(), new YearMonthDay(), null, bean.getDepartment().getDepartmentUnit(), AccountabilityType.readByType(AccountabilityTypeEnum.ORGANIZATIONAL_STRUCTURE), null, null, false, null);
  }
  ...
  @Atomic(mode = TxMode.WRITE) private void createCompetenceCourseGroup(AssociatedObjectsBean bean) {
    CompetenceCourseGroupUnit.createNewInternalCompetenceCourseGroupUnit(bean.getNameLS(), null, null, bean.getCode(), new YearMonthDay(), null, bean.getScientificAreaUnit(), AccountabilityType.readByType(AccountabilityTypeEnum.ORGANIZATIONAL_STRUCTURE), null, null, false, null);
  }
