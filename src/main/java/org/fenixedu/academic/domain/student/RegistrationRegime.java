...
public class RegistrationRegime extends RegistrationRegime_Base {
  ...
  static public Comparator<RegistrationRegime> COMPARATOR_BY_EXECUTION_YEAR = new Comparator<RegistrationRegime>() {
    @Override public int compare(RegistrationRegime o1, RegistrationRegime o2) {
  ...
  public RegistrationRegime(final Registration registration, final ExecutionYear executionYear, final RegistrationRegimeType type) {
    this();
  ...
  private void checkParameters(final Registration registration, final ExecutionYear executionYear, final RegistrationRegimeType type) {
  ...
  private void checkEctsCredits(final Registration registration, final ExecutionYear executionYear) {
    ...
    if (enroledEctsCredits > MaximumNumberOfCreditsForEnrolmentPeriod.MAXIMUM_NUMBER_OF_CREDITS) {
      throw new DomainException("error.RegistrationRegime.semester.has.more.ects.than.maximum.allowed", String.valueOf(enroledEctsCredits), executionYear.getQualifiedName(), String.valueOf(MaximumNumberOfCreditsForEnrolmentPeriod.MAXIMUM_NUMBER_OF_CREDITS));
    }
  }
  ...
  private void clearPartialRegimeEvents() {
    ...
    studentCurricularPlan.getGratuityEvent(getExecutionYear(), EnrolmentGratuityEvent.class).filter((e) -> e.getEventType() == EventType.PARTIAL_REGIME_ENROLMENT_GRATUITY && e.canBeCanceled()).forEach((enrolmentGratuityEvent) -> {
