...
public class RegistryDiplomaRequest extends RegistryDiplomaRequest_Base implements IRegistryDiplomaRequest, IRectorateSubmissionBatchDocumentEntry {
  ...
  @Override protected void checkParameters(DocumentRequestCreateBean bean) {
  ...
  @Override final public String getDescription() {
    return Joiner.on(" : ").join(super.getDescription(getAcademicServiceRequestType()), getProgramConclusion().getName().getContent());
  }

  @Override public DocumentRequestType getDocumentRequestType() {
    return DocumentRequestType.REGISTRY_DIPLOMA_REQUEST;
  }

  @Override public AcademicServiceRequestType getAcademicServiceRequestType() {
    return AcademicServiceRequestType.REGISTRY_DIPLOMA_REQUEST;
  }

  @Override public String getDocumentTemplateKey() {
  ...
  @Override public EventType getEventType() {
    final SetView<EventType> eventTypesToUse = Sets.intersection(getPossibleEventTypes(), getProgramConclusion().getEventTypes().getTypes());
  ...
  @Override public boolean hasPersonalInfo() {
    return true;
  }

  @Override protected void internalChangeState(AcademicServiceRequestBean academicServiceRequestBean) {
  ...
  @Override public boolean isAvailableForTransitedRegistrations() {
    return false;
  }

  @Override public boolean isPagedDocument() {
    return false;
  }

  @Override public boolean isPayedUponCreation() {
    return true;
  }

  @Override public boolean isPossibleToSendToOtherEntity() {
    return true;
  }

  @Override public boolean isManagedWithRectorateSubmissionBatch() {
    return true;
  }

  @Override public boolean isToPrint() {
    return false;
  }

  @Override public CycleType getRequestedCycle() {
    return getProgramConclusion().groupFor(getRegistration()).filter(CurriculumGroup::isCycleCurriculumGroup).map((cg) -> ((CycleCurriculumGroup) cg).getCycleType()).orElse(null);
  }

  @Override public LocalDate getConclusionDate() {
    return getProgramConclusion().groupFor(getRegistration()).map(CurriculumGroup::getConclusionProcess).map(ConclusionProcess::getConclusionDate).orElse(null);
  }

  @Override public ExecutionYear getConclusionYear() {
    return getProgramConclusion().groupFor(getRegistration()).map(CurriculumGroup::getConclusionProcess).map(ConclusionProcess::getConclusionYear).orElse(null);
  }

  @Override public String getGraduateTitle(Locale locale) {
    return getProgramConclusion().groupFor(getRegistration()).map((cg) -> cg.getDegreeModule().getGraduateTitle(getConclusionYear(), getLanguage())).orElse(null);
  }

  @Override public String getFinalAverage(final Locale locale) {
    return getRegistration().getFinalGrade(getProgramConclusion()).getValue();
  }

  @Override public String getQualifiedAverageGrade(final Locale locale) {
  ...
  @Override public String getProgrammeTypeDescription() {
    return getDegreeType().getName().getContent();
  }

  @Override public String getViewStudentProgrammeLink() {
    return "/student.do?method=visualizeRegistration&amp;registrationID=" + getRegistration().getExternalId();
  }

  @Override public String getReceivedActionLink() {
  ...
  @Override public boolean isProgrammeLinkVisible() {
    return getRegistration().isAllowedToManageRegistration();
  }

  @Override public String getDegreeName(ExecutionYear year) {
