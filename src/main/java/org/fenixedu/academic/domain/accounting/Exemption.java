...
public abstract class Exemption extends Exemption_Base {
  ...
  private void checkParameters(final Event event, final ExemptionJustification exemptionJustification) {
    ...
    if (!operationsAfter.isEmpty()) {
      throw new DomainException("error.accounting.Exemption.cannot.create.operations.after", operationsAfter.stream().collect(Collectors.joining(",")));
    }
  }
  ...
  @Override public void setResponsible(Person responsible) {
    throw new DomainException("error.accounting.Exemption.cannot.modify.responsible");
  }

  @Override public void setWhenCreated(DateTime whenCreated) {
    throw new DomainException("error.accounting.Exemption.cannot.modify.whenCreated");
  }

  @Override public void setExemptionJustification(ExemptionJustification exemptionJustification) {
