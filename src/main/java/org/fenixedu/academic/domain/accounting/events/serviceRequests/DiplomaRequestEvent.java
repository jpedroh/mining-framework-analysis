...
abstract public class DiplomaRequestEvent extends DiplomaRequestEvent_Base {
  ...
  @Override final public LabelFormatter getDescriptionForEntryType(final EntryType entryType) {
  ...
  private void fillDescription(final LabelFormatter labelFormatter) {
    ...
    if (request instanceof DiplomaRequest) {
      if (((DiplomaRequest) request).getRequestedCycle() != null) {
        labelFormatter.appendLabel(((DiplomaRequest) request).getRequestedCycle().getQualifiedName(), Bundle.ENUMERATION).appendLabel(" ").appendLabel("label.of", Bundle.APPLICATION).appendLabel(" ");
      }
    } else {
      labelFormatter.appendLabel(" ");
    }
  ...
  @Override public PostingRule getPostingRule() {
