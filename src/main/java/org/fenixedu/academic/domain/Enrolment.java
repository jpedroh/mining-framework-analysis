...
public class Enrolment extends Enrolment_Base implements IEnrolment {
  static final public Comparator<Enrolment> REVERSE_COMPARATOR_BY_EXECUTION_PERIOD_AND_ID = new Comparator<Enrolment>() {
    @Override public int compare(Enrolment o1, Enrolment o2) {
  ...
  static final public Comparator<Enrolment> COMPARATOR_BY_STUDENT_NUMBER = new Comparator<Enrolment>() {
    @Override public int compare(final Enrolment e1, final Enrolment e2) {
  ...
  @Override final public boolean isEnrolment() {
    return true;
  }
  ...
  @Override final public boolean isExternalEnrolment() {
    return false;
  }

  @Override final public boolean isPropaedeutic() {
    return super.isPropaedeutic();
  }

  @Override public boolean isExtraCurricular() {
    return super.isExtraCurricular();
  }

  @Override @Deprecated public Boolean getIsExtraCurricular() {
    return isExtraCurricular();
  }

  @Override @Deprecated public void setIsExtraCurricular(Boolean isExtraCurricular) {
  ...
  protected void initializeAsNew(StudentCurricularPlan studentCurricularPlan, CurriculumGroup curriculumGroup, CurricularCourse curricularCourse, ExecutionSemester executionSemester, EnrollmentCondition enrolmentCondition, String createdBy) {
    initializeAsNewWithoutEnrolmentEvaluation(studentCurricularPlan, curriculumGroup, curricularCourse, executionSemester, enrolmentCondition, createdBy);
  ...
  protected void initializeAsNew(StudentCurricularPlan studentCurricularPlan, CurricularCourse curricularCourse, ExecutionSemester executionSemester, EnrollmentCondition enrolmentCondition, String createdBy) {
    initializeAsNewWithoutEnrolmentEvaluation(studentCurricularPlan, curricularCourse, executionSemester, enrolmentCondition, createdBy);
    createEnrolmentEvaluationWithoutGrade();
  }
  ...
  final public Optional<EnrolmentEvaluation> getEnrolmentEvaluationBySeasonAndState(final EnrolmentEvaluationState state, final EvaluationSeason season) {
  ...
  protected void createEnrolmentEvaluationWithoutGrade() {
    boolean existing = getEnrolmentEvaluationBySeason(EvaluationConfiguration.getInstance().getDefaultEvaluationSeason()).filter((e) -> e.getGrade().equals(null)).findAny().isPresent();
    if (!existing) {
  ...
  final public EnrolmentEvaluation createEnrolmentEvaluationForImprovement(final Person person, final ExecutionSemester executionSemester) {
    final EnrolmentEvaluation enrolmentEvaluation = new EnrolmentEvaluation(this, EvaluationSeason.readImprovementSeason(), EnrolmentEvaluationState.TEMPORARY_OBJ, person, executionSemester);
  ...
  private void createAttendForImprovement(final ExecutionSemester executionSemester) {
    final Registration registration = getRegistration();
    ExecutionCourse currentExecutionCourse = (ExecutionCourse) CollectionUtils.find(getCurricularCourse().getAssociatedExecutionCoursesSet(), new Predicate() {
      @Override final public boolean evaluate(Object arg0) {
        ExecutionCourse executionCourse = (ExecutionCourse) arg0;
        if (executionCourse.getExecutionPeriod() == executionSemester && executionCourse.getEntryPhase() == EntryPhase.FIRST_PHASE) {
          return true;
        }
        return false;
      }
  ...
  final public boolean isImprovementForExecutionCourse(ExecutionCourse executionCourse) {
    return getExecutionPeriod() != executionCourse.getExecutionPeriod() && getCurricularCourse().getAssociatedExecutionCoursesSet().contains(executionCourse);
  }
  ...
  final public boolean isImprovingInExecutionPeriodFollowingApproval(final ExecutionSemester improvementExecutionPeriod) {
    ...
    for (ExecutionSemester executionSemester = enrolmentNextExecutionPeriod; executionSemester != null && executionSemester != improvementExecutionPeriod; executionSemester = executionSemester.getNextExecutionPeriod()) {
  ...
  final public EnrolmentEvaluation createSpecialSeasonEvaluation(final Person person) {
    if (!getEvaluationSeason().isSpecial() && !isApproved()) {
      ...
      return new EnrolmentEvaluation(this, EvaluationSeason.readSpecialSeason(), EnrolmentEvaluationState.TEMPORARY_OBJ, person);
    } else {
  ...
  final public void deleteSpecialSeasonEvaluation() {
    if (getEvaluationSeason().isSpecial() && hasSpecialSeason()) {
      ...
      setEvaluationSeason(EvaluationConfiguration.getInstance().getDefaultEvaluationSeason());
      getEnrolmentEvaluationBySeasonAndState(EnrolmentEvaluationState.TEMPORARY_OBJ, EvaluationSeason.readSpecialSeason()).ifPresent((ee) -> {
        if (ee.getSpecialSeasonEnrolmentEvent() != null) {
          ee.getSpecialSeasonEnrolmentEvent().cancel(Authenticate.getUser().getPerson(), BundleUtil.getString(Bundle.ACADEMIC, "enrolmentEvaluation.cancel.event.disenrolled"));
          ee.setEnrolmentEvaluationState(EnrolmentEvaluationState.ANNULED_OBJ);
        } else {
          ee.delete();
        }
      });
      getEnrolmentEvaluationBySeasonAndState(EnrolmentEvaluationState.FINAL_OBJ, EvaluationSeason.readSpecialSeason()).ifPresent((e) -> setEnrollmentState(e.getEnrollmentStateByGrade()));
    } else {
      throw new DomainException("error.invalid.enrolment.state");
    }
  }
  ...
  final public boolean hasSpecialSeason() {
    return getEnrolmentEvaluationBySeason(EvaluationSeason.readSpecialSeason()).anyMatch((ee) -> !ee.getEnrolmentEvaluationState().equals(EnrolmentEvaluationState.ANNULED_OBJ));
  }
  ...
  @Override final public boolean isApproved() {
    if (isAnnulled()) {
      return false;
    }
    final EnrolmentEvaluation latestEnrolmentEvaluation = getFinalEnrolmentEvaluation();
  ...
  @Override public boolean isApproved(final CurricularCourse curricularCourse, final ExecutionSemester executionSemester) {
  ...
  @Override public final ConclusionValue isConcluded(ExecutionYear executionYear) {
  ...
  @Override public YearMonthDay calculateConclusionDate() {
    ...
    return EvaluationConfiguration.getInstance().getEnrolmentEvaluationForConclusionDate(this).map(EnrolmentEvaluation::getExamDateYearMonthDay).orElse(null);
  }

  @Override @SuppressWarnings(value = { "unchecked" }) final public Curriculum getCurriculum(final DateTime when, final ExecutionYear year) {
    if (wasCreated(when) && (year == null || getExecutionYear().isBefore(year)) && isApproved() && !isPropaedeutic() && !isExtraCurricular()) {
      return new Curriculum(this, year, Collections.singleton((ICurriculumEntry) this), Collections.EMPTY_SET, Collections.singleton((ICurriculumEntry) this));
    }
    return Curriculum.createEmpty(this, year);
  }

  @Override final public Grade getGrade() {
  ...
  @Override public Grade getEctsGrade(final StudentCurricularPlan scp, final DateTime processingDate) {
    final EnrolmentEvaluation enrolmentEvaluation = getFinalEnrolmentEvaluation();
  ...
  @Override public EctsConversionTable getEctsConversionTable(final StudentCurricularPlan scp, final DateTime processingDate) {
    final EnrolmentEvaluation enrolmentEvaluation = getFinalEnrolmentEvaluation();
  ...
  @Override final public String getGradeValue() {
    return getGrade().getValue();
  }

  @Override final public Integer getFinalGrade() {
  ...
  @Override final public boolean isEnroled() {
    return this.getEnrollmentState() == EnrollmentState.ENROLLED;
  }

  @Deprecated final public boolean isEnrolmentStateApproved() {
    return this.getEnrollmentState() == EnrollmentState.APROVED;
  }

  @Deprecated final public boolean isEnrolmentStateNotApproved() {
    return this.getEnrollmentState() == EnrollmentState.NOT_APROVED;
  }

  @Deprecated final public boolean isEnrolmentStateNotEvaluated() {
  ...
  @Override final public void setDegreeModule(DegreeModule degreeModule) {
    super.setDegreeModule(degreeModule);
    resetIsFirstTimeEnrolment();
  }

  @Override final public void setEnrollmentState(EnrollmentState enrollmentState) {
    super.setEnrollmentState(enrollmentState);
    resetIsFirstTimeEnrolment();
  }

  @Override final public void setExecutionPeriod(ExecutionSemester executionSemester) {
    super.setExecutionPeriod(executionSemester);
    resetIsFirstTimeEnrolment();
  }

  @Override final public void setStudentCurricularPlan(StudentCurricularPlan studentCurricularPlan) {
  ...
  @Override protected void createCurriculumLineLog(final EnrolmentAction action) {
    new EnrolmentLog(action, getRegistration(), getCurricularCourse(), getExecutionPeriod(), getCurrentUser());
  }

  @Override public StringBuilder print(String tabs) {
  ...
  final public EnrolmentEvaluation addNewEnrolmentEvaluation(EnrolmentEvaluationState enrolmentEvaluationState, EvaluationSeason evaluationSeason, Person responsibleFor, String gradeValue, Date availableDate, Date examDate, ExecutionSemester executionSemester, final GradeScale gradeScale) {
    final Grade grade = Grade.createGrade(gradeValue, gradeScale != null ? gradeScale : getGradeScale());
    final EnrolmentEvaluation enrolmentEvaluation = new EnrolmentEvaluation(this, enrolmentEvaluationState, evaluationSeason, responsibleFor, grade, availableDate, examDate, new DateTime());
  ...
  final public EnrolmentEvaluation addNewEnrolmentEvaluation(EnrolmentEvaluationState enrolmentEvaluationState, EvaluationSeason evaluationSeason, Person responsibleFor, String gradeValue, Date availableDate, Date examDate, ExecutionSemester executionSemester, String bookReference, String page, GradeScale gradeScale) {
    EnrolmentEvaluation enrolmentEvaluation = addNewEnrolmentEvaluation(enrolmentEvaluationState, evaluationSeason, responsibleFor, gradeValue, availableDate, examDate, executionSemester, gradeScale);
  ...
  final public boolean hasAssociatedMarkSheetOrFinalGrade(EvaluationSeason season) {
    for (final EnrolmentEvaluation enrolmentEvaluation : this.getEvaluationsSet()) {
      if (enrolmentEvaluation.getEvaluationSeason().equals(season) && (enrolmentEvaluation.getMarkSheet() != null || enrolmentEvaluation.isFinal())) {
        return true;
      }
    }
    return false;
  ...
  final public List<EnrolmentEvaluation> getConfirmedEvaluations(EvaluationSeason season) {
    ...
    for (EnrolmentEvaluation evaluation : this.getEvaluationsSet()) {
      if (evaluation.getMarkSheet() != null && evaluation.getMarkSheet().getEvaluationSeason().equals(season) && evaluation.getMarkSheet().isConfirmed()) {
  ...
  final public EnrolmentEvaluation getImprovementEvaluation() {
    return getEnrolmentEvaluationBySeasonAndState(EnrolmentEvaluationState.TEMPORARY_OBJ, EvaluationSeason.readImprovementSeason()).orElse(null);
  }

  @Override final public List<Enrolment> getEnrolments() {
    return Collections.singletonList(this);
  }

  @Override final public boolean hasAnyEnrolments() {
    return true;
  }

  @Override final public StudentCurricularPlan getStudentCurricularPlan() {
    return getCurriculumGroup() != null ? getCurriculumGroup().getStudentCurricularPlan() : super.getStudentCurricularPlan();
  }

  @Override public boolean isEnroledInExecutionPeriod(final CurricularCourse curricularCourse, final ExecutionSemester executionSemester) {
    return this.getCurricularCourse() == curricularCourse && isValid(executionSemester);
  }

  @Override public boolean isValid(final ExecutionSemester executionSemester) {
    return getExecutionPeriod() == executionSemester || (getExecutionPeriod().getExecutionYear() == executionSemester.getExecutionYear() && getCurricularCourse().isAnual());
  }
  ...
  @Override final public boolean hasEnrolmentWithEnroledState(final CurricularCourse curricularCourse, final ExecutionSemester executionSemester) {
  ...
  @Override final public String getEnrolmentTypeName() {
  ...
  @Override final public Double getWeigth() {
    return isExtraCurricular() || isPropaedeutic() ? Double.valueOf(0) : getWeigthForCurriculum().doubleValue();
  }

  @Override final public BigDecimal getWeigthForCurriculum() {
    final Double d;
  ...
  /**
     ...
     * @return
     */
  @Deprecated final public double getCredits() {
    return getEctsCredits();
  }

  @Override final public Double getEctsCredits() {
    return isExtraCurricular() || isPropaedeutic() ? Double.valueOf(0d) : getEctsCreditsForCurriculum().doubleValue();
  }

  @Override final public BigDecimal getEctsCreditsForCurriculum() {
    return BigDecimal.valueOf(getCurricularCourse().getEctsCredits(getExecutionPeriod()));
  }

  @Override final public Double getAprovedEctsCredits() {
    return isApproved() ? getEctsCredits() : Double.valueOf(0d);
  }

  @Override final public Double getCreditsConcluded(ExecutionYear executionYear) {
    return executionYear == null || getExecutionYear().isBeforeOrEquals(executionYear) ? getAprovedEctsCredits() : 0d;
  }

  @Override final public Double getEnroledEctsCredits(final ExecutionSemester executionSemester) {
    return isValid(executionSemester) && isEnroled() ? getEctsCredits() : Double.valueOf(0d);
  }

  @Override final public Enrolment findEnrolmentFor(final CurricularCourse curricularCourse, final ExecutionSemester executionSemester) {
    return isEnroledInExecutionPeriod(curricularCourse, executionSemester) ? this : null;
  }

  @Override final public Enrolment getApprovedEnrolment(final CurricularCourse curricularCourse) {
    return isApproved(curricularCourse) ? this : null;
  }

  @Override public Set<IDegreeModuleToEvaluate> getDegreeModulesToEvaluate(final ExecutionSemester executionSemester) {
  ...
  @Override final public double getAccumulatedEctsCredits(final ExecutionSemester executionSemester) {
    return isBolonhaDegree() && !parentAllowAccumulatedEctsCredits() ? 0d : getStudentCurricularPlan().getAccumulatedEctsCredits(executionSemester, getCurricularCourse());
  }
  ...
  final public boolean isSpecialSeasonEnroled(final ExecutionYear executionYear) {
    return isSpecialSeason() && getExecutionPeriod().getExecutionYear() == executionYear && getTempSpecialSeasonEvaluation() != null;
  }
  ...
  private EnrolmentEvaluation getTempSpecialSeasonEvaluation() {
    return getEnrolmentEvaluationBySeasonAndState(EnrolmentEvaluationState.TEMPORARY_OBJ, EvaluationSeason.readSpecialSeason()).orElse(null);
  }
  ...
  @Override final public Collection<Enrolment> getSpecialSeasonEnrolments(final ExecutionYear executionYear) {
    return getExecutionPeriod().getExecutionYear() == executionYear && isSpecialSeason() ? Collections.singleton(this) : Collections.emptySet();
  }

  @Override final public Collection<Enrolment> getSpecialSeasonEnrolments(final ExecutionSemester executionSemester) {
  ...
  /**
     ...
     */
  @Override final public Thesis getThesis() {
    ...
    switch (theses.size()) {
      ...
      SortedSet<Thesis> sortedTheses = new TreeSet<Thesis>(new Comparator<Thesis>() {
        @Override public int compare(Thesis o1, Thesis o2) {
  ...
  @Override final public Unit getAcademicUnit() {
    return Bennu.getInstance().getInstitutionUnit();
  }

  @Override final public String getCode() {
  ...
  @Override public boolean hasEnrolment(ExecutionSemester executionSemester) {
    return isValid(executionSemester);
  }

  @Override public boolean hasEnrolment(ExecutionYear executionYear) {
    return isValid(executionYear);
  }

  @Override public boolean isEnroledInSpecialSeason(final ExecutionSemester executionSemester) {
    return isValid(executionSemester) && hasSpecialSeason();
  }

  @Override public boolean isEnroledInSpecialSeason(ExecutionYear executionYear) {
    return isValid(executionYear) && hasSpecialSeason();
  }

  @Override public int getNumberOfAllApprovedEnrolments(ExecutionSemester executionSemester) {
  ...
  public boolean canBeSubmittedForOldMarkSheet(EvaluationSeason season) {
    ...
    for (EnrolmentEvaluation enrolmentEvaluation : getEvaluationsSet()) {
      if (enrolmentEvaluation.getEvaluationSeason().equals(season) && enrolmentEvaluation.getMarkSheet() == null && (enrolmentEvaluation.isTemporary() || (enrolmentEvaluation.isNotEvaluated() && enrolmentEvaluation.getExamDateYearMonthDay() == null))) {
        return true;
      }
    }
    return false;
  ...
  @Override public boolean isAnual() {
    final CurricularCourse curricularCourse = getCurricularCourse();
  ...
  @Override public String getModuleTypeName() {
    return BundleUtil.getString(Bundle.ENUMERATION, this.getClass().getName());
  }

  @ConsistencyPredicate public boolean checkThesisMultiplicity() {
