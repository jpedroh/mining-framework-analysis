...
@GroupOperator(value = "thesisReaders") public class ThesisReadersGroup extends FenixGroup {
  private static final long serialVersionUID = -784604571687620343L;

  @GroupArgument private Thesis thesis;
  ...
  @Override public String[] getPresentationNameKeyArgs() {
  ...
  private Set<User> getThesisMembers() {
    Set<User> members = thesis.getParticipationsSet().stream().filter((p) -> p.getPerson() != null).map((p) -> p.getPerson().getUser()).collect(Collectors.toSet());
