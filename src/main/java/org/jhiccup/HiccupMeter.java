...
public class HiccupMeter extends Thread {
    ...
    public HiccupMeterConfiguration(final String[] args) {
      try {
        ...
        if (attachToProcess) {
          ...
          agentArgs = "-d " + startDelayMs + " -i " + reportingIntervalMs + ((startTimeAtZero) ? " -0" : "") + " -r " + resolutionMs;
        ...
        if (launchControlProcess) {
          ...
          controlProcessCommand = System.getProperty("java.home") + File.separator + "bin" + File.separator + "java" + " -cp " + System.getProperty("java.class.path") + " -Dorg.jhiccup.avoidRecursion=true" + " " + HiccupMeter.class.getCanonicalName() + " -l " + controlProcessLogFileName + " -i " + reportingIntervalMs + " -d " + startDelayMs + ((startTimeAtZero) ? " -0" : "") + " -r " + resolutionMs + " -terminateWithStdInput";
        }
        if (resolutionMs < 0) {
        ...
        String validArgs = "\"[-v] [-c] [-o] [-0] [-p pidOfProcessToAttachTo] [-j jHiccupJarFileName] " + "[-i reportingIntervalMs] [-h] [-t runTimeMs] [-d startDelayMs] " + "[-l logFileName] [-r resolutionMs] [-terminateWithStdInput] [-f inputFileName]\"\n";
  ...
  class InputRecorder extends HiccupRecorder {
    ...
    long processInputLine(final Scanner scanner, final Histogram histogram) {
      if (scanner.hasNextLine()) {
        try {
          final long timeMsec = (long) scanner.nextDouble();
          final long hiccupTimeUsec = (long) (scanner.nextDouble() * 1000);
    ...
    @Override public long getCurrentTimeMsec(final long nextReportingTime) throws InterruptedException {
      return processInputLine(scanner, histogram);
    }

    @Override public Histogram swapHistogram(final Histogram replacementHistogram) {
  ...
  static boolean canDoOverwritingRenames = true;

  void outputHistogramFile(final Histogram histogram, final boolean suppressRunTimeExceptions) {
    try {
      if (config.logFileName != null) {
        final PrintStream histogramLog = new PrintStream(new FileOutputStream(config.logFileName + ".hgrm.tmp"), false);
        histogramLog.println("jHiccup histogram report, " + new Date() + " :\n--------------------\n");
        histogram.getHistogramData().outputPercentileDistribution(histogramLog, config.histogramDumpTicksPerHalf, config.outputValueUnitRatio);
        histogramLog.close();
        ...
        if (!renameSucceess) {
          if (canDoOverwritingRenames) {
            ...
            if (renameSucceess) {
              canDoOverwritingRenames = false;
            }
          }
          if (!renameSucceess) {
            log.println("Failed to rename histogram file from " + fromFile.getName() + " to " + toFile.getName());
          }
        }
      }
    } catch (FileNotFoundException e) {
  ...
  @Override public void run() {
    final Histogram accumulatedHistogram = new Histogram(config.highestTrackableValue, config.numberOfSignificantValueDigits);
    ...
    final Histogram initialHistogram = new Histogram(config.highestTrackableValue, config.numberOfSignificantValueDigits);
    ...
    try {
      ...
      if (config.inputFileName == null) {
        ...
        log.println("#[Sampling start time: " + new Date() + ", (uptime at sampling start: " + (ManagementFactory.getRuntimeMXBean().getUptime() / 1000.0) + " seconds)]");
      ...
      if (config.logFormatCsv) {
        log.println("\"Time\",\"Int_Count\",\"Int_50%\",\"Int_90%\",\"Int_Max\",\"Total_Count\"," + "\"Total_50%\",\"Total_90%\",\"Total_99%\",\"Total_99.9%\",\"Total_99.99%\",\"Total_Max\"");
      } else {
      ...
      Histogram latestHistogram = new Histogram(config.highestTrackableValue, config.numberOfSignificantValueDigits);
      while ((now > 0) && ((config.runTimeMs == 0) || (config.runTimeMs > now - startTime))) {
        now = hiccupRecorder.getCurrentTimeMsec(nextReportingTime);
        if (now > nextReportingTime) {
          ...
          if (latestHistogram.getHistogramData().getTotalCount() > 0) {
            ...
            log.format(Locale.US, logFormat, (now - reportingStartTime) / 1000.0, latestHistogramData.getTotalCount(), latestHistogramData.getValueAtPercentile(50.0) / 1000.0, latestHistogramData.getValueAtPercentile(90.0) / 1000.0, latestHistogramData.getMaxValue() / 1000.0, accumulatedHistogramData.getTotalCount(), accumulatedHistogramData.getValueAtPercentile(50.0) / 1000.0, accumulatedHistogramData.getValueAtPercentile(90.0) / 1000.0, accumulatedHistogramData.getValueAtPercentile(99.0) / 1000.0, accumulatedHistogramData.getValueAtPercentile(99.9) / 1000.0, accumulatedHistogramData.getValueAtPercentile(99.99) / 1000.0, accumulatedHistogramData.getMaxValue() / 1000.0);
            outputHistogramFile(accumulatedHistogram, ((now - startTime) < config.timeDelayMsBeforeThrowingHistogramFileExceptions));
          }
        }
      }
    } catch (InterruptedException e) {
