...
public final class Symmetry010Date extends AbstractDate implements ChronoLocalDate, Serializable {
  ...
  static Symmetry010Date create(int prolepticYear, int month, int dayOfMonth) {
    ...
    if (dayOfMonth > DAYS_IN_MONTH) {
      if (month == MONTHS_IN_YEAR) {
        ...
        if (((month % 3 == 2) && dayOfMonth > DAYS_IN_MONTH_LONG) || (month % 3 != 2)) {
  ...
  @Override int getAlignedDayOfWeekInMonth() {
    return getDayOfWeek();
  }
  ...
  @Override int getDayOfWeek() {
    return ((dayOfYear - 1) % DAYS_IN_WEEK) + 1;
  }

  long getProlepticWeek() {
    return prolepticYear * WEEKS_IN_YEAR + Symmetry010Chronology.getLeapYearsBefore(prolepticYear) + ((dayOfYear - 1) / DAYS_IN_WEEK) - 1;
  }
  ...
  @Override Symmetry010Date resolvePrevious(int newYear, int newMonth, int dayOfMonth) {
  ...
  /**
     ...
     */
  @Override public Symmetry010Chronology getChronology() {
    return INSTANCE;
  }
  ...
  @Override public int lengthOfMonth() {
    return this.isLeapYear() && this.month == MONTHS_IN_YEAR ? DAYS_IN_MONTH + 7 : this.month % 3 == 2 ? DAYS_IN_MONTH_LONG : DAYS_IN_MONTH;
  }
  ...
  @Override public Symmetry010Date with(TemporalAdjuster adjuster) {
    return (Symmetry010Date) adjuster.adjustInto(this);
  }

  @Override public Symmetry010Date with(TemporalField field, long newValue) {
    if (field instanceof ChronoField) {
      if (newValue == 0) {
        return this;
      }
  ...
  @Override Symmetry010Date withDayOfYear(int value) {
    return ofYearDay(prolepticYear, value);
  }

  @Override public Symmetry010Date plus(TemporalAmount amount) {
    return (Symmetry010Date) amount.addTo(this);
  }

  @Override public Symmetry010Date plus(long amountToAdd, TemporalUnit unit) {
    return (Symmetry010Date) super.plus(amountToAdd, unit);
  }

  @Override public Symmetry010Date minus(TemporalAmount amount) {
    return (Symmetry010Date) amount.subtractFrom(this);
  }

  @Override public Symmetry010Date minus(long amountToSubtract, TemporalUnit unit) {
    return (Symmetry010Date) super.minus(amountToSubtract, unit);
  }

  @Override @SuppressWarnings(value = { "unchecked" }) public ChronoLocalDateTime<Symmetry010Date> atTime(LocalTime localTime) {
  ...
  @Override public ChronoPeriod until(ChronoLocalDate endDateExclusive) {
  ...
  @Override long weeksUntil(AbstractDate end) {
  ...
  @Override long monthsUntil(AbstractDate end) {
  ...
  @Override public long toEpochDay() {
    long epochDay = (long) (this.prolepticYear - 1) * DAYS_IN_YEAR + Symmetry010Chronology.getLeapYearsBefore(this.prolepticYear) * DAYS_IN_WEEK + this.dayOfYear - DAYS_0001_TO_1970 - 1;
    return epochDay;
  }
  ...
  @Override public String toString() {
    ...
    return buf.append(getChronology().toString()).append(' ').append(getEra()).append(' ').append(getYearOfEra()).append(this.month < 10 && this.month > 0 ? "/0" : '/').append(this.month).append(this.day < 10 ? "/0" : '/').append(this.day).toString();
  }
}