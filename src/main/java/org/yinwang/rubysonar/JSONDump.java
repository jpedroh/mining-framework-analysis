...
public class JSONDump {
  ...
  private static void writeSymJson(@NotNull Binding binding, JsonGenerator json) throws IOException {
    ...
    boolean isExported = !(Binding.Kind.VARIABLE == binding.kind || Binding.Kind.PARAMETER == binding.kind || Binding.Kind.SCOPE == binding.kind || Binding.Kind.ATTRIBUTE == binding.kind || (name != null && (name.length() == 0 || name.startsWith("lambda%"))));
  ...
  private static void writeDocJson(Binding binding, Analyzer idx, JsonGenerator json) throws Exception {
    ...
    if (!seenDocs.contains(path)) {
      ...
      if (binding.node instanceof org.yinwang.rubysonar.ast.Class) {
        ...
        if (binding.node instanceof Function) {
  ...
  private static void graph(List<String> srcpath, String[] inclpaths, OutputStream symOut, OutputStream refOut, OutputStream docOut) throws Exception {
