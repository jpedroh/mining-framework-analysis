...
public class ReindexingService extends AbstractLifecycleComponent<ReindexingService> {
  ...
  @Inject public ReindexingService(final Settings settings, final Client client, final ThreadPool threadPool) {
    super(settings);
    this.client = client;
  ...
  public String[] getNames() {
    return reindexingListenerMap.keySet().toArray(new String[reindexingListenerMap.size()]);
  }

  public void delete(final String name) {
    final ReindexingListener reindexingListener = reindexingListenerMap.remove(name);
    if (reindexingListener != null) {
      reindexingListener.interrupt();
    } else {
      throw new ReindexingException("Reindexing process " + name + " is not found.");
    }
  }
  ...
  public String execute(final Params params, final BytesReference content, final ActionListener<Void> listener) {
    ...
    final ReindexingListener reindexingListener = new ReindexingListener(url, toIndex, toType, scroll, listener);
    final SearchRequestBuilder builder = client.prepareSearch(fromIndex).setScroll(scroll).addFields(fields);
    if (fromType != null && fromType.trim().length() > 0) {
      builder.setTypes(fromType.split(","));
    }
    if (content == null || content.length() == 0) {
      builder.setQuery(QueryBuilders.matchAllQuery()).setSize(Integer.parseInt(params.param("size", "1000")));
    } else {
  ...
  private class ReindexingListener implements ActionListener<SearchResponse> {
    ...
    /**
         ...
         * @param response
         */
    @Override public void onResponse(final SearchResponse response) {
      if (interrupted.get()) {
    ...
    private void sendToLocalCluster(final String scrollId, final SearchHit[] hits) {
      ...
      for (final SearchHit hit : hits) {
        IndexRequestBuilder builder = client.prepareIndex(toIndex, toType != null ? toType : hit.getType(), hit.getId()).setSource(hit.getSource());
      ...
      bulkRequest.execute(new ActionListener<BulkResponse>() {
        @Override public void onResponse(final BulkResponse bulkResponse) {
          if (bulkResponse.hasFailures()) {
            throw new ReindexingException(bulkResponse.buildFailureMessage());
          }
          client.prepareSearchScroll(scrollId).setScroll(scroll).execute(ReindexingListener.this);
        }
    ...
    private void sendToRemoteCluster(final String scrollId, final SearchHit[] hits) {
      try {
        Curl.post(url + "_bulk").onConnect(new ConnectionBuilder() {
          @Override public void onConnect(CurlRequest curlRequest, HttpURLConnection connection) {
            connection.setDoOutput(true);
            try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(connection.getOutputStream(), curlRequest.encoding()))) {
          ...
          @Override public void onResponse(HttpURLConnection con) {
            try {
              int responseCode = con.getResponseCode();
              if (responseCode == 200) {
                client.prepareSearchScroll(scrollId).setScroll(scroll).execute(ReindexingListener.this);
              } else {
                throw new ReindexingException("The response code from " + url + " is");
              }
            } catch (Exception e) {
              ReindexingListener.this.onFailure(e);
            }
      ...
      if (scrollId != null) {
        client.prepareClearScroll().addScrollId(scrollId).execute(new ActionListener<ClearScrollResponse>() {
          ...
          @Override public void onFailure(Throwable e) {
            if (logger.isDebugEnabled()) {
              logger.debug("Failed to stop reindexing for " + toIndex + ".", e);
            }
          }
        });
      }
