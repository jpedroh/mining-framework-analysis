...
public class RestDataAction extends BaseRestHandler {
  ...
  public RestDataAction(final Settings settings, final RestController restController) {
  ...
  @Override public List<Route> routes() {
    return unmodifiableList(asList(new Route(GET, "/_data"), new Route(POST, "/_data"), new Route(GET, "/{index}/_data"), new Route(POST, "/{index}/_data")));
  }

  public RestChannelConsumer prepareRequest(final RestRequest request, final NodeClient client) throws IOException {
    SearchRequest searchRequest = new SearchRequest();
    request.withContentOrSourceParamParserOrNull((parser) -> RestSearchAction.parseSearchRequest(searchRequest, request, parser, client.getNamedWriteableRegistry(), (size) -> searchRequest.source().size(size)));
    ...
    return (channel) -> client.search(searchRequest, new SearchResponseListener(channel, file, limitBytes, dataContent));
  }
  ...
  private ContentType getContentType(final RestRequest request) {
    final String contentType = request.param("format", request.header("Content-Type"));
        ...
        if ("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet".equals(contentType) || "xlsx".equalsIgnoreCase(contentType)) {
          ...
          if ("text/javascript".equals(contentType) || "application/json".equals(contentType) || "json".equalsIgnoreCase(contentType)) {
            ...
            if ("application/list+json".equals(contentType) || "jsonlist".equals(contentType)) {
              ...
              if ("application/geo+json".equals(contentType) || "application/geojson".equals(contentType) || "geojson".equals(contentType)) {
  ...
  class SearchResponseListener implements ActionListener<SearchResponse> {
    ...
    SearchResponseListener(final RestChannel channel, final String file, final long limit, final DataContent dataContent) {
      ...
      if (!Strings.isNullOrEmpty(file)) {
        ...
        if (parentFile != null && !parentFile.isDirectory()) {
          throw new ElasticsearchException("Cannot create/access " + outputFile.getAbsolutePath());
        }
      }
      this.limit = limit;
    }
    ...
    @Override public void onResponse(final SearchResponse response) {
      try {
        ...
        dataContent.write(outputFile, response, channel, new ActionListener<Void>() {
          @Override public void onResponse(final Void response) {
            try {
              if (useLocalFile) {
                sendResponse(dataContent.getRequest(), channel, outputFile.getAbsolutePath());
              } else {
    ...
    private void writeResponse(final RestRequest request, final RestChannel channel, final File outputFile, final long limit, final DataContent dataContent) {
      ...
      try (FileInputStream fis = new FileInputStream(outputFile)) {
        ...
        final BytesRestResponse response = new BytesRestResponse(RestStatus.OK, contentType.contentType(), out.toByteArray());
