...
public class RedisRiver extends AbstractRiverComponent implements River {
  ...
  @Inject public RedisRiver(RiverName riverName, RiverSettings settings, Client client) {
    ...
    if (logger.isInfoEnabled()) {
      logger.info("Configured Redis connection {}:{}/{} DB={} bulkSize={} bulkTimeout={}", redisHost, redisPort, redisKey, redisDB, bulkSize, bulkTimeout);
    }
  }

  @Override public void start() {
    if (logger.isInfoEnabled()) {
      logger.info("Starting Redis River stream");
    }
    try {
    ...
    if (redisMode.equalsIgnoreCase("list")) {
      ...
      if (redisMode.equalsIgnoreCase("pubsub")) {
  ...
  @Override public void close() {
    if (logger.isInfoEnabled()) {
      logger.info("Closing down redis river");
    }
    closed = true;
    if (thread != null) {
      thread.interrupt();
  ...
  private class RedisPubSubRunner implements Runnable {
    ...
    @Override public void run() {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("Is about to subscribe to [{}]", redisKey);
      }
    ...
    private class RiverListener extends JedisPubSub {
      private void queueMessage(String message) {
        try {
          if (logger.isDebugEnabled()) {
            logger.debug("About to add a message...");
          }
          byte[] data = message.getBytes();
          currentRequest.add(data, 0, data.length, false);
          if (logger.isDebugEnabled()) {
            logger.debug("Current size" + currentRequest.numberOfActions());
          }
  ...
  private class RedisListRunner implements Runnable {
    ...
    private void loop() {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("Blocking on queue pop...");
        }
      ...
      if (response != null) {
        try {
          if (logger.isDebugEnabled()) {
            logger.debug("Popped from queue: {}", response);
          }
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("Nothing popped. Timed out.");
        }
        processBulkIfNeeded(true);
      }
    ...
    private void processBulkIfNeeded(Boolean force) {
      ...
      if (actionCount != 0 && (actionCount > bulkSize || force == true)) {
        try {
          if (logger.isDebugEnabled()) {
            logger.debug("Executing bulk request: actionCount={} bulkSize={} force={}", actionCount, bulkSize, force);
          }
          BulkResponse response = currentRequest.execute().actionGet();
          if (response.hasFailures()) {
            logger.error("failed to execute" + response.buildFailureMessage());
