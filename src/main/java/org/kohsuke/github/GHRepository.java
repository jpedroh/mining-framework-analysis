...
@SuppressWarnings(value = { "UnusedDeclaration" }) @SuppressFBWarnings(value = { "UWF_UNWRITTEN_PUBLIC_OR_PROTECTED_FIELD", "UWF_UNWRITTEN_FIELD", "NP_UNWRITTEN_FIELD" }, justification = "JSON API") public class GHRepository extends GHObject {
  ...
  public PagedIterable<GHDeployment> listDeployments(String sha, String ref, String task, String environment) {
    ...
    return root.retrieve().asPagedIterable(deploymentsUrl, GHDeployment[].class, (item) -> item.wrap(GHRepository.this));
  }
  ...
  public List<GHIssue> getIssues(GHIssueState state, GHMilestone milestone) throws IOException {
    return Arrays.asList(GHIssue.wrap(root.retrieve().with("state", state).with("milestone", milestone == null ? "none" : "" + milestone.getNumber()).to(getApiTailUrl("issues"), GHIssue[].class), this));
  }
  ...
  public PagedIterable<GHIssue> listIssues(final GHIssueState state) {
    return root.retrieve().with("state", state).asPagedIterable(getApiTailUrl("issues"), GHIssue[].class, (item) -> item.wrap(GHRepository.this));
  }
  ...
  public GHRef createRef(String name, String sha) throws IOException {
    return new Requester(root).with("ref", name).with("sha", sha).method("POST").to(getApiTailUrl("git/refs"), GHRef.class).wrap(root);
  }
  ...
  public PagedIterable<GHRelease> listReleases() throws IOException {
    return root.retrieve().asPagedIterable(getApiTailUrl("releases"), GHRelease[].class, (item) -> item.wrap(GHRepository.this));
  }

  public PagedIterable<GHTag> listTags() throws IOException {
    return root.retrieve().asPagedIterable(getApiTailUrl("tags"), GHTag[].class, (item) -> item.wrap(GHRepository.this));
  }
  ...
  public void setEmailServiceHook(String address) throws IOException {
    ...
    new Requester(root).method("POST").with("name", "email").with("config", config).with("active", true).to(getApiTailUrl("hooks"));
  }
  ...
  public PagedIterable<GHRepository> listForks(final ForkSort sort) {
    return root.retrieve().with("sort", sort).asPagedIterable(getApiTailUrl("forks"), GHRepository[].class, (item) -> item.wrap(root));
  }
  ...
  public GHPullRequest getPullRequest(int i) throws IOException {
    return root.retrieve().withPreview(SHADOW_CAT).to(getApiTailUrl("pulls/" + i), GHPullRequest.class).wrapUp(this);
  }
  ...
  public GHPullRequest createPullRequest(String title, String head, String base, String body, boolean maintainerCanModify, boolean draft) throws IOException {
    return new Requester(root).withPreview(SHADOW_CAT).with("title", title).with("head", head).with("base", base).with("body", body).with("maintainer_can_modify", maintainerCanModify).with("draft", draft).to(getApiTailUrl("pulls"), GHPullRequest.class).wrapUp(this);
  }
  ...
  public PagedIterable<GHCommit> listCommits() {
    return root.retrieve().asPagedIterable(String.format("/repos/%s/%s/commits", getOwnerName(), name), GHCommit[].class, (item) -> item.wrapUp(GHRepository.this));
  }
  ...
  public PagedIterable<GHCommitComment> listCommitComments() {
    return root.retrieve().asPagedIterable(String.format("/repos/%s/%s/comments", getOwnerName(), name), GHCommitComment[].class, (item) -> item.wrap(GHRepository.this));
  }
  ...
  public PagedIterable<GHCommitStatus> listCommitStatuses(final String sha1) throws IOException {
    return root.retrieve().asPagedIterable(String.format("/repos/%s/%s/statuses/%s", getOwnerName(), name, sha1), GHCommitStatus[].class, (item) -> item.wrapUp(root));
  }
  ...
  public GHCommitStatus createCommitStatus(String sha1, GHCommitState state, String targetUrl, String description, String context) throws IOException {
    return new Requester(root).with("state", state).with("target_url", targetUrl).with("description", description).with("context", context).to(String.format("/repos/%s/%s/statuses/%s", getOwnerName(), this.name, sha1), GHCommitStatus.class).wrapUp(root);
  }
  ...
  public PagedIterable<GHEventInfo> listEvents() throws IOException {
    return root.retrieve().asPagedIterable(String.format("/repos/%s/%s/events", getOwnerName(), name), GHEventInfo[].class, (item) -> item.wrapUp(root));
  }
  ...
  public PagedIterable<GHLabel> listLabels() throws IOException {
    return root.retrieve().withPreview(SYMMETRA).asPagedIterable(getApiTailUrl("labels"), GHLabel[].class, (item) -> item.wrapUp(GHRepository.this));
  }
  ...
  /**
     ...
     * @throws IOException
     */
  @Preview @Deprecated public GHLabel createLabel(String name, String color, String description) throws IOException {
    return root.retrieve().method("POST").withPreview(SYMMETRA).with("name", name).with("color", color).with("description", description).to(getApiTailUrl("labels"), GHLabel.class).wrapUp(this);
  }
  ...
  public PagedIterable<GHInvitation> listInvitations() {
    return root.retrieve().asPagedIterable(String.format("/repos/%s/%s/invitations", getOwnerName(), name), GHInvitation[].class, (item) -> item.wrapUp(root));
  }
  ...
  public PagedIterable<GHStargazer> listStargazers2() {
    return root.retrieve().withPreview("application/vnd.github.v3.star+json").asPagedIterable(getApiTailUrl("stargazers"), GHStargazer[].class, (item) -> item.wrapUp(GHRepository.this));
  }
  ...
  /**
     ...
     */
  @SuppressFBWarnings(value = "DMI_COLLECTION_OF_URLS", justification = "It causes a performance degradation, but we have already exposed it to the API") public Set<URL> getPostCommitHooks() {
  ...
  /**
     ...
     */
  @SuppressFBWarnings(value = "DMI_COLLECTION_OF_URLS", justification = "It causes a performance degradation, but we have already exposed it to the API") @SkipFromToString private final Set<URL> postCommitHooks = new AbstractSet<URL>() {
  ...
  public PagedIterable<GHMilestone> listMilestones(final GHIssueState state) {
    return root.retrieve().with("state", state).asPagedIterable(getApiTailUrl("milestones"), GHMilestone[].class, (item) -> item.wrap(GHRepository.this));
  }
  ...
  public GHDeployKey addDeployKey(String title, String key) throws IOException {
    return new Requester(root).with("title", title).with("key", key).method("POST").to(getApiTailUrl("keys"), GHDeployKey.class).wrap(this);
  }

  public List<GHDeployKey> getDeployKeys() throws IOException {
    List<GHDeployKey> list = new ArrayList<GHDeployKey>(Arrays.asList(root.retrieve().to(getApiTailUrl("keys"), GHDeployKey[].class)));
  ...
  public GHSubscription subscribe(boolean subscribed, boolean ignored) throws IOException {
    return new Requester(root).with("subscribed", subscribed).with("ignored", ignored).method("PUT").to(getApiTailUrl("subscription"), GHSubscription.class).wrapUp(this);
  }
  ...
  public PagedIterable<Contributor> listContributors() throws IOException {
    return root.retrieve().asPagedIterable(getApiTailUrl("contributors"), Contributor[].class, (item) -> item.wrapUp(root));
  }
  ...
  public GHProject createProject(String name, String body) throws IOException {
    return root.retrieve().method("POST").withPreview(INERTIA).with("name", name).with("body", body).to(getApiTailUrl("projects"), GHProject.class).wrap(this);
  }
  ...
  public PagedIterable<GHProject> listProjects(final GHProject.ProjectStateFilter status) throws IOException {
    return root.retrieve().withPreview(INERTIA).with("state", status).asPagedIterable(getApiTailUrl("projects"), GHProject[].class, (item) -> item.wrap(GHRepository.this));
  }
  ...
  public Reader renderMarkdown(String text, MarkdownMode mode) throws IOException {
    return new InputStreamReader(new Requester(root).with("text", text).with("mode", mode == null ? null : mode.toString()).with("context", getFullName()).asStream("/markdown"), "UTF-8");
  }
  ...
  public PagedIterable<GHIssueEvent> listIssueEvents() throws IOException {
    return root.retrieve().asPagedIterable(getApiTailUrl("issues/events"), GHIssueEvent[].class, (item) -> item.wrapUp(root));
  }
