...
public class MessageQueueFactoryImpl implements MessageQueueFactory {
  private static final class DefaultMessageQueue extends ConcurrentLinkedQueue<Message> implements MessageQueue {
    /**  */
    ...
    @Override public Message createMessage(final MessageSource source, final Message old, final Request<?> _request, final ExceptionHandler handler, final ResponseProcessor<?> rp) {
      return new Message(source, old, _request, handler, rp);
    }

    /** Is the queue empty? */
    @Override public boolean isNonEmpty() {
    ...
    /**
         *
         */
    @Override public void offer(final Message e, final boolean local) {
      if (local) {
        final int length = localQueue.length;
        if (count == length) {
          final Message[] newLocalQueue = new Message[1 + length * 2];
          if (head <= tail) {
            System.arraycopy(localQueue, head, newLocalQueue, 0, count);
            head = 0;
            tail = count;
          } else {
            System.arraycopy(localQueue, head, newLocalQueue, 0, length - head);
            System.arraycopy(localQueue, 0, newLocalQueue, tail - 1, tail);
          }
  ...
  @Override public MessageQueue createMessageQueue() {
