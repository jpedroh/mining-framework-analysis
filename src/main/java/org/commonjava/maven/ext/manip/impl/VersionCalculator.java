 ...
 * @author jdcasey
 */
@Component(role = VersionCalculator.class) public class VersionCalculator {
  ...
  @Requirement protected GalleyAPIWrapper readerWrapper;
  ...
  /**
     ...
     */
  public Map<String, String> calculateVersioningChanges(final Collection<Project> projects, final ManipulationSession session) throws ManipulationException {
    ...
    for (final Project project : projects) {
      ...
      final VersionCalculation modifiedVersion = calculate(project.getGroupId(), project.getArtifactId(), originalVersion, session);
    ...
    for (final Map.Entry<String, VersionCalculation> entry : calculationsByGA.entrySet()) {
      final String modifiedVersion = entry.getValue().renderVersion();
  ...
  /**
     ...
     * @throws ManipulationException
     */
  protected VersionCalculation calculate(final String groupId, final String artifactId, final String version, final ManipulationSession session) throws ManipulationException {
    ...
    if (staticSuffix != null) {
      calculateStatic(vc, baseVersion, snapshot, staticSuffix, groupId, artifactId, originalVersion, state, session);
    } else {
      if (incrementalSuffix != null) {
        calculateIncremental(vc, baseVersion, snapshot, incrementalSuffix, groupId, artifactId, originalVersion, state, session);
      }
    }
  ...
  private String calculateOSGiBase(String version) {
    ...
    logger.debug("Matched group " + match.group(1) + " and " + match.group(match.groupCount()) + " and modified baseVersion is " + result);
    return result.toString();
  }

  private void calculateStatic(final VersionCalculation vc, final String baseVersion, final boolean snapshot, final String suffix, final String groupId, final String artifactId, final String originalVersion, final VersioningState state, final ManipulationSession session) {
  ...
  private String trimBaseVersion(final VersionCalculation vc, String baseVersion, final String suffixBase) {
    ...
    if (idx > 1) {
      ...
      logger.debug("Trimmed version (without pre-existing suffix): " + baseVersion + " with base-version separator: " + baseSep);
    }
  ...
  private void calculateIncremental(final VersionCalculation vc, String baseVersion, final boolean snapshot, final String incrementalSerialSuffix, final String groupId, final String artifactId, final String originalVersion, final VersioningState state, final ManipulationSession session) throws ManipulationException {
    logger.debug("Using incremental suffix: " + incrementalSerialSuffix);
    String suffixBase = incrementalSerialSuffix;
    String sep = "-";
    final Matcher suffixMatcher = Pattern.compile(SERIAL_SUFFIX_PATTERN).matcher(incrementalSerialSuffix);
    if (suffixMatcher.matches()) {
    ...
    for (final String version : versionCandidates) {
      ...
      if (candidateSuffixMatcher.find()) {
        ...
        if (!trimmedBaseVersion.equals(trimmedBase)) {
          logger.debug("Ignoring irrelevant version: \'" + version + "\' (\'" + base + "\' doesn\'t match on base-version: \'" + baseVersion + "\').");
          continue;
        }
  ...
  /**
     ...
     */
  private Set<String> getMetadataVersions(final String groupId, final String artifactId, final ManipulationSession session) throws ManipulationException {
    logger.debug("Reading available versions from repository metadata for: " + groupId + ":" + artifactId);
    try {
      final MavenMetadataView metadataView = readerWrapper.readMetadataView(new ProjectRef(groupId, artifactId));
