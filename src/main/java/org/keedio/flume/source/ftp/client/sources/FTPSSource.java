...
public class FTPSSource extends KeedioSource<FTPFile> {
  ...
  @Override public List<FTPFile> listElements(String dir) throws IOException {
    FTPFile[] subFiles = getFtpsClient().listFiles(dir);
    return Arrays.asList(subFiles);
  }

  @Override public InputStream getInputStream(FTPFile file) throws IOException {
    if (isFlushLines()) {
  ...
  @Override public String getObjectName(FTPFile file) {
    return file.getName();
  }

  @Override public long getModifiedTime(FTPFile file) {
    return file.getTimestamp().getTimeInMillis();
  }

  @Override public boolean isDirectory(FTPFile file) {
    return file.isDirectory();
  }

  @Override public boolean isFile(FTPFile file) {
    return file.isFile();
  }
  ...
  @Override public long getObjectSize(FTPFile file) {
    return file.getSize();
  }

  @Override public boolean isLink(FTPFile file) {
    return file.isSymbolicLink();
  }

  @Override public String getLink(FTPFile file) {
    return file.getLink();
  }
  ...
  @Override public List<FTPFile> listElements(String dirToList, KeedioFileFilter filter) throws IOException {
