...
public class InstagramApi {
  ...
  public String getAuthorizationUrl(OAuthConfig config) {
    ...
    if (config.hasScope()) {
      return String.format(Constants.SCOPED_AUTHORIZE_URL, config.getApiKey(), formURLEncode(config.getCallback()), config.getScope());
    } else {
  ...
  public AccessTokenExtractor getAccessTokenExtractor() {
    return new AccessTokenExtractor() {
      @Override public Token extract(String response) {
