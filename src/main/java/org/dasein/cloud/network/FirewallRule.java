 ...
 * @since 2010.08
 */
@SuppressWarnings(value = { "UnusedDeclaration" }) public class FirewallRule implements Comparable<FirewallRule> {
  ...
  static public @Nullable FirewallRule parseId(@Nonnull String id) {
    ...
    if (parts[i].equalsIgnoreCase("DENY")) {
      ...
      if (parts[i].equalsIgnoreCase("ALLOW")) {
        i++;
      }
    }
    if (parts.length < i + 1) {
  ...
  @Override public int compareTo(FirewallRule other) {
    if (other == null) {
      return -1;
    }
    if (other == this) {
