 ...
 * @since 2013.01
 */
public final class Cache<T extends java.lang.Object> {
  static private final HashMap<String, Cache<?>> caches = new HashMap<String, Cache<?>>();

  static private class CacheEntry<T extends java.lang.Object> {
  ...
  /**
     ...
     */
  static public @Nonnull <X extends java.lang.Object> Cache<X> getInstance(@Nonnull CloudProvider provider, @Nonnull String name, @Nonnull Class<X> typeClass, @Nonnull CacheLevel level) {
  ...
  /**
     ...
     * @return a cache containing the context-sensitive cached items
     */
  static public @Nonnull <X extends java.lang.Object> Cache<X> getInstance(@Nonnull CloudProvider provider, @Nonnull String name, @Nonnull Class<X> typeClass, @Nonnull CacheLevel level, @Nonnegative TimePeriod<?> timeout) {
    Cache<X> c;
  ...
  private Cache(CacheLevel level, TimePeriod<?> timeout) {
    switch (level) {
      case CLOUD:
      cloudCache = new HashMap<String, CacheEntry<T>>();
      break;
      case CLOUD_ACCOUNT:
      cloudAccountCache = new HashMap<String, Map<String, CacheEntry<T>>>();
      break;
      case REGION:
      regionCache = new HashMap<String, Map<String, CacheEntry<T>>>();
      break;
      case REGION_ACCOUNT:
      regionAccountCache = new HashMap<String, Map<String, Map<String, CacheEntry<T>>>>();
      break;
    }
  ...
  public void clear() {
    synchronized (this) {
      if (cloudCache != null) {
        ...
        if (cloudAccountCache != null) {
          cloudAccountCache.clear();
        } else {
          if (regionCache != null) {
            regionCache.clear();
          } else {
            if (regionAccountCache != null) {
