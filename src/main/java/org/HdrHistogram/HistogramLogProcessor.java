...
public class HistogramLogProcessor extends Thread {
  ...
  private static class HistogramLogProcessorConfiguration {
    ...
    long movingWindowLengthInMsec = 60000;
    ...
    HistogramLogProcessorConfiguration(final String[] args) {
      ...
      try {
        ...
        final String validArgs = "\"[-csv] [-v] [-i inputFileName] [-o outputFileName] [-tag tag] " + "[-start rangeStartTimeSec] [-end rangeEndTimeSec] " + "[-outputValueUnitRatio r] [-correctLogWithKnownCoordinatedOmission i] [-listtags]";
    ...
    try {
      ...
      if (histogram != null) {
        if (config.expectedDoubleInterval != null) {
          ...
          if (config.expectedLongInterval != null) {
            if (histogram instanceof Histogram) {
  ...
  @Override public void run() {
    ...
    try {
      if (config.outputFileName != null) {
        ...
        if (config.movingWindow) {
          ...
          try {
            ...
            outputTimeRange(movingWindowLog, "Moving window log for " + config.movingWindowPercentileToReport + " percentile");
          } catch (FileNotFoundException ex) {
      ...
      Histogram accumulatedRegularHistogram = logUsesDoubleHistograms ? new Histogram(3) : ((Histogram) intervalHistogram).copy();
      ...
      EncodableHistogram movingWindowSumHistogram = logUsesDoubleHistograms ? new DoubleHistogram(3) : new Histogram(3);
      while (intervalHistogram != null) {
        ...
        if (config.movingWindow) {
          ...
          while ((movingWindowQueue.peek() != null) && (movingWindowQueue.peek().getEndTimeStamp() <= windowCutOffTimeStamp)) {
