...
public abstract class AbstractHistogram extends AbstractHistogramBase implements ValueRecorder, Serializable {
  ...
  private void resetMinNonZeroValue(final long minNonZeroValue) {
    final long internalValue = minNonZeroValue & ~unitMagnitudeMask;
  ...
  /**
     ...
     *                                       integer between 0 and 5.
     */
  protected AbstractHistogram(final long lowestDiscernibleValue, final long highestTrackableValue, final int numberOfSignificantValueDigits) {
  ...
  @SuppressWarnings(value = { "deprecation" }) private void init(final long lowestDiscernibleValue, final long highestTrackableValue, final int numberOfSignificantValueDigits, final double integerToDoubleValueConversionRatio, final int normalizingIndexOffset) {
  ...
  private void recordValueWithCountAndExpectedInterval(final long value, final long count, final long expectedIntervalBetweenValueSamples) throws ArrayIndexOutOfBoundsException {
    recordCountAtValue(count, value);
  ...
  private void recordSingleValueWithExpectedInterval(final long value, final long expectedIntervalBetweenValueSamples) throws ArrayIndexOutOfBoundsException {
    recordSingleValue(value);
    if (expectedIntervalBetweenValueSamples <= 0) {
      return;
    }
  ...
  /**
     ...
     *                                           than expectedIntervalBetweenValueSamples
     */
  public void copyIntoCorrectedForCoordinatedOmission(final AbstractHistogram targetHistogram, final long expectedIntervalBetweenValueSamples) {
    targetHistogram.reset();
  ...
  public void add(final AbstractHistogram otherHistogram) throws ArrayIndexOutOfBoundsException {
    ...
    if ((bucketCount == otherHistogram.bucketCount) && (subBucketCount == otherHistogram.subBucketCount) && (unitMagnitude == otherHistogram.unitMagnitude) && (getNormalizingIndexOffset() == otherHistogram.getNormalizingIndexOffset()) && !(otherHistogram instanceof ConcurrentHistogram)) {
  ...
  /**
     ...
     *
     */
  public void subtract(final AbstractHistogram otherHistogram) throws ArrayIndexOutOfBoundsException, IllegalArgumentException {
    if (highestEquivalentValue(otherHistogram.getMaxValue()) > highestEquivalentValue(valueFromIndex(this.countsArrayLength - 1))) {
  ...
  /**
     ...
     */
  public void addWhileCorrectingForCoordinatedOmission(final AbstractHistogram otherHistogram, final long expectedIntervalBetweenValueSamples) {
 ...
 */
  ...
  /**
     ...
     */
  @Override public void setStartTimeStamp(final long timeStampMsec) {
  ...
  /**
     ...
     */
  @Override public long getEndTimeStamp() {
  ...
  /**
     ...
     * @param timeStampMsec the value to set the time stamp to, [by convention] in msec since the epoch.
     */
  @Override public void setEndTimeStamp(final long timeStampMsec) {
  ...
  public double getMean() {
    ...
    while (recordedValuesIterator.hasNext()) {
      ...
      totalValue += medianEquivalentValue(iterationValue.getValueIteratedTo()) * (double) iterationValue.getCountAtValueIteratedTo();
    }
  ...
  public long getValueAtPercentile(final double percentile) {
    double requestedPercentile = Math.min(Math.max(Math.nextAfter(percentile, Double.NEGATIVE_INFINITY), 0.0D), 100.0D);
  ...
  public class LogarithmicBucketValues implements Iterable<HistogramIterationValue> {
    ...
    private LogarithmicBucketValues(final AbstractHistogram histogram, final long valueUnitsInFirstBucket, final double logBase) {
      this.histogram = histogram;
  ...
  private int getEncodingCookie() {
    return encodingCookieBase | 0x10;
  }

  private int getCompressedEncodingCookie() {
    return compressedEncodingCookieBase | 0x10;
  }
  ...
  static <T extends AbstractHistogram> T decodeFromByteBuffer(final ByteBuffer buffer, final Class<T> histogramClass, final long minBarForHighestTrackableValue) {
    try {
  ...
  private static <T extends AbstractHistogram> T decodeFromByteBuffer(final ByteBuffer buffer, final Class<T> histogramClass, final long minBarForHighestTrackableValue, final Inflater decompressor) throws DataFormatException {
    ...
    if ((getCookieBase(cookie) == encodingCookieBase) || (getCookieBase(cookie) == V1EncodingCookieBase)) {
      ...
      if (getCookieBase(cookie) == V0EncodingCookieBase) {
        numberOfSignificantValueDigits = buffer.getInt();
        lowestTrackableUnitValue = buffer.getLong();
        highestTrackableValue = buffer.getLong();
        buffer.getLong();
    ...
    T histogram;
    try {
      @SuppressWarnings(value = { "unchecked" }) Constructor<T> constructor = histogramClass.getConstructor(constructorArgsTypes);
  ...
  static <T extends AbstractHistogram> T decodeFromCompressedByteBuffer(final ByteBuffer buffer, final Class<T> histogramClass, final long minBarForHighestTrackableValue) throws DataFormatException {
    ...
    if ((getCookieBase(cookie) == compressedEncodingCookieBase) || (getCookieBase(cookie) == V1CompressedEncodingCookieBase)) {
      ...
      if (getCookieBase(cookie) == V0CompressedEncodingCookieBase) {
  ...
  int normalizeIndex(int index, int normalizingIndexOffset, int arrayLength) {
    ...
    if (normalizedIndex < 0) {
      ...
      if (normalizedIndex >= arrayLength) {
        normalizedIndex -= arrayLength;
      }
    }
