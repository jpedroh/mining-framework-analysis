...
public class DoubleRecorder implements DoubleValueRecorder {
  ...
  /**
     ...
     *                                       and separation. Must be a non-negative integer between 0 and 5.
     */
  public DoubleRecorder(final long highestToLowestValueRatio, final int numberOfSignificantValueDigits) {
    activeHistogram = new InternalConcurrentDoubleHistogram(instanceId, highestToLowestValueRatio, numberOfSignificantValueDigits);
    inactiveHistogram = null;
    activeHistogram.setStartTimeStamp(System.currentTimeMillis());
  }
  ...
  /**
     ...
     */
  @Override public void recordValue(final double value) {
  ...
  /**
     ...
     * @throws ArrayIndexOutOfBoundsException (may throw) if value is exceeds highestTrackableValue
     */
  @Override public void recordValueWithCount(final double value, final long count) throws ArrayIndexOutOfBoundsException {
    long criticalValueAtEnter = recordingPhaser.writerCriticalSectionEnter();
    try {
  ...
  /**
     ...
     * @throws ArrayIndexOutOfBoundsException (may throw) if value is exceeds highestTrackableValue
     */
  @Override public void recordValueWithExpectedInterval(final double value, final double expectedIntervalBetweenValueSamples) throws ArrayIndexOutOfBoundsException {
    long criticalValueAtEnter = recordingPhaser.writerCriticalSectionEnter();
    try {
  ...
  private void validateFitAsReplacementHistogram(DoubleHistogram replacementHistogram, boolean enforeContainingInstance) {
    ...
    if (replacementHistogram == null) {
      ...
      if ((replacementHistogram instanceof InternalConcurrentDoubleHistogram) && ((!enforeContainingInstance) || (((InternalConcurrentDoubleHistogram) replacementHistogram).containingInstanceId == activeHistogram.containingInstanceId))) {
        bad = false;
      }
    }
    if (bad) {
