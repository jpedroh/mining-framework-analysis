...
public class tarql extends CmdGeneral {
  ...
  static {
    ...
    try {
      ...
      date = (String) mf.getMainAttributes().getValue("Built-Date").replaceFirst("(\\d\\d\\d\\d)(\\d\\d)(\\d\\d)-(\\d\\d)(\\d\\d)", "$1-$2-$3T$4:$5:00Z");
    } catch (Exception ex) {
    }
    VERSION = version;
    BUILD_DATE = date;
...
<<<<<<< /usr/src/app/output/tarql/tarql/0644473c2bd9194e86eed5131e12cc580cf2956b/src/main/java/org/deri/tarql/tarql.java/left.java
  jsonFormatArg = new ArgDecl(false, "json_format")
=======
  jsonLDArg = new ArgDecl(false, "jsonld")
...
=======
    "Write JSON-LD instead of Turtle"
>>>>>>> /usr/src/app/output/tarql/tarql/0644473c2bd9194e86eed5131e12cc580cf2956b/src/main/java/org/deri/tarql/tarql.java/right.java
    );
  ...
  @Override protected void processModulesAndArgs() {
  ...
  @Override protected void exec() {
    initLogging();
    try {
      TarqlQuery q = baseIRI == null ? new TarqlParser(queryFile).getResult() : new TarqlParser(queryFile, baseIRI).getResult();
      if (testQuery) {
        q.makeTest();
      }
      if (stdin) {
        processResults(TarqlQueryExecutionFactory.create(q, InputStreamSource.fromStdin(), options));
      } else {
        if (csvFiles.isEmpty()) {
      ...
      if (resultTripleIterator.hasNext()) {
        ...
        if (writeNTriples) {
          ...
          if (writeJSONLD) {
            ...
            writer.writeTurtle(q.getPrologue().getBaseURI(), q.getPrologue().getPrefixMapping(), writeBase);
          }
        }
      }
  ...
  private void processResults(TarqlQueryExecution ex) throws IOException {
    ...
    if (ex.getFirstQuery().isSelectType()) {
      ...
      if (ex.getFirstQuery().isAskType()) {
        System.out.println(ResultSetFormatter.asText(ex.execSelect()));
      } else {
        if (ex.getFirstQuery().isConstructType()) {
