...
public abstract class JDBC3DatabaseMetaData extends org.sqlite.core.CoreDatabaseMetaData {
  ...
  public ResultSet getBestRowIdentifier(String c, String s, String t, int scope, boolean n) throws SQLException {
    if (getBestRowIdentifier == null) {
      getBestRowIdentifier = conn.prepareStatement("select null as SCOPE, null as COLUMN_NAME, " + "null as DATA_TYPE, null as TYPE_NAME, null as COLUMN_SIZE, " + "null as BUFFER_LENGTH, null as DECIMAL_DIGITS, null as PSEUDO_COLUMN limit 0;");
    }
  ...
  public ResultSet getColumnPrivileges(String c, String s, String t, String colPat) throws SQLException {
    if (getColumnPrivileges == null) {
      getColumnPrivileges = conn.prepareStatement("select null as TABLE_CAT, null as TABLE_SCHEM, " + "null as TABLE_NAME, null as COLUMN_NAME, null as GRANTOR, null as GRANTEE, " + "null as PRIVILEGE, null as IS_GRANTABLE limit 0;");
    }
        ...
        try {
          ...
          for (int i = 0; rscol.next(); i++) {
            ...
            if (TYPE_INTEGER.matcher(colType).find()) {
              ...
              if (TYPE_VARCHAR.matcher(colType).find()) {
                colJavaType = Types.VARCHAR;
              } else {
                if (TYPE_FLOAT.matcher(colType).find()) {
  ...
  public ResultSet getPrimaryKeys(String c, String s, String table) throws SQLException {
    ...
    for (int i = 0; i < columns.length; i++) {
      ...
      sql.append("select ").append(pkName).append(" as pk, \'").append(escape(columns[i].trim())).append("\' as cn, ").append(i + 1).append(" as ks");
    }
        ...
        try {
          ...
          for (Iterator iterator = fkNames.iterator(); iterator.hasNext(); ) {
            ...
            for (int j = 0; j < foreignKey.getColumnMappingCount(); j++) {
              ...
              exportedKeysQuery.append(count > 0 ? " union all select " : "select ").append(Integer.toString(keySeq)).append(" as ks, \'").append(escape(tbl)).append("\' as fkt, \'").append(escape(FKColName)).append("\' as fcn, \'").append(escape(PKColName)).append("\' as pcn, \'").append(escape(pkName)).append("\' as pkn, ").append(RULE_MAP.get(foreignKey.getOnUpdate())).append(" as ur, ").append(RULE_MAP.get(foreignKey.getOnDelete())).append(" as dr, ");
  ...
  private StringBuilder appendDummyForeignKeyList(StringBuilder sql) {
    sql.append("select -1 as ks, \'\' as ptn, \'\' as fcn, \'\' as pcn, ").append(DatabaseMetaData.importedKeyNoAction).append(" as ur, ").append(DatabaseMetaData.importedKeyNoAction).append(" as dr, ").append(" \'\' as fkn, ").append(" \'\' as pkn ").append(") limit 0;");
    return sql;
  }
    ...
    if (indexList.size() == 0) {
      ...
      while (indexIterator.hasNext()) {
        ...
        while (rs.next()) {
          ...
          sqlRow.append("select ").append(Integer.toString(1 - (Integer) currentIndex.get(1))).append(" as un,\'").append(escape(indexName)).append("\' as n,").append(Integer.toString(rs.getInt(1) + 1)).append(" as op,");
          if (colName == null) {
            sqlRow.append("null");
          } else {
  ...
  public ResultSet getProcedureColumns(String c, String s, String p, String colPat) throws SQLException {
    if (getProcedureColumns == null) {
      getProcedureColumns = conn.prepareStatement("select null as PROCEDURE_CAT, " + "null as PROCEDURE_SCHEM, null as PROCEDURE_NAME, null as COLUMN_NAME, " + "null as COLUMN_TYPE, null as DATA_TYPE, null as TYPE_NAME, null as PRECISION, " + "null as LENGTH, null as SCALE, null as RADIX, null as NULLABLE, " + "null as REMARKS limit 0;");
    }
  ...
  public ResultSet getProcedures(String c, String s, String p) throws SQLException {
    if (getProcedures == null) {
      getProcedures = conn.prepareStatement("select null as PROCEDURE_CAT, null as PROCEDURE_SCHEM, " + "null as PROCEDURE_NAME, null as UNDEF1, null as UNDEF2, null as UNDEF3, " + "null as REMARKS, null as PROCEDURE_TYPE limit 0;");
    }
  ...
  public ResultSet getSuperTypes(String c, String s, String t) throws SQLException {
    if (getSuperTypes == null) {
      getSuperTypes = conn.prepareStatement("select null as TYPE_CAT, null as TYPE_SCHEM, " + "null as TYPE_NAME, null as SUPERTYPE_CAT, null as SUPERTYPE_SCHEM, " + "null as SUPERTYPE_NAME limit 0;");
    }
  ...
  public ResultSet getTablePrivileges(String c, String s, String t) throws SQLException {
    if (getTablePrivileges == null) {
      getTablePrivileges = conn.prepareStatement("select  null as TABLE_CAT, " + "null as TABLE_SCHEM, null as TABLE_NAME, null as GRANTOR, null " + "GRANTEE,  null as PRIVILEGE, null as IS_GRANTABLE limit 0;");
    }
  ...
  public ResultSet getTypeInfo() throws SQLException {
    if (getTypeInfo == null) {
      getTypeInfo = conn.prepareStatement("select " + "tn as TYPE_NAME, " + "dt as DATA_TYPE, " + "0 as PRECISION, " + "null as LITERAL_PREFIX, " + "null as LITERAL_SUFFIX, " + "null as CREATE_PARAMS, " + DatabaseMetaData.typeNullable + " as NULLABLE, " + "1 as CASE_SENSITIVE, " + DatabaseMetaData.typeSearchable + " as SEARCHABLE, " + "0 as UNSIGNED_ATTRIBUTE, " + "0 as FIXED_PREC_SCALE, " + "0 as AUTO_INCREMENT, " + "null as LOCAL_TYPE_NAME, " + "0 as MINIMUM_SCALE, " + "0 as MAXIMUM_SCALE, " + "0 as SQL_DATA_TYPE, " + "0 as SQL_DATETIME_SUB, " + "10 as NUM_PREC_RADIX from (" + "    select \'BLOB\' as tn, " + Types.BLOB + " as dt union" + "    select \'NULL\' as tn, " + Types.NULL + " as dt union" + "    select \'REAL\' as tn, " + Types.REAL + " as dt union" + "    select \'TEXT\' as tn, " + Types.VARCHAR + " as dt union" + "    select \'INTEGER\' as tn, " + Types.INTEGER + " as dt" + ") order by TYPE_NAME;");
    }
  ...
  public ResultSet getUDTs(String c, String s, String t, int[] types) throws SQLException {
    if (getUDTs == null) {
      getUDTs = conn.prepareStatement("select  null as TYPE_CAT, null as TYPE_SCHEM, " + "null as TYPE_NAME,  null as CLASS_NAME,  null as DATA_TYPE, null as REMARKS, " + "null as BASE_TYPE " + "limit 0;");
    }
  ...
  public ResultSet getVersionColumns(String c, String s, String t) throws SQLException {
    if (getVersionColumns == null) {
      getVersionColumns = conn.prepareStatement("select null as SCOPE, null as COLUMN_NAME, " + "null as DATA_TYPE, null as TYPE_NAME, null as COLUMN_SIZE, " + "null as BUFFER_LENGTH, null as DECIMAL_DIGITS, null as PSEUDO_COLUMN limit 0;");
    }
  ...
  /**
     ...
     */
  protected final static Pattern PK_UNNAMED_PATTERN = Pattern.compile(".*PRIMARY\\s+KEY\\s*\\((.*?,+.*?)\\).*", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);

  /**
     * Pattern used to extract a named primary key.
     */
  protected final static Pattern PK_NAMED_PATTERN = Pattern.compile(".*CONSTRAINT\\s*(.*?)\\s*PRIMARY\\s+KEY\\s*\\((.*?)\\).*", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
  ...
  class ImportedKeyFinder {
    /**
         * Pattern used to extract a named primary key.
         */
    private final Pattern FK_NAMED_PATTERN = Pattern.compile("CONSTRAINT\\s*([A-Za-z_][A-Za-z\\d_]*)?\\s*FOREIGN\\s+KEY\\s*\\((.*?)\\)", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
    ...
    private List<String> getForeignKeyNames(String tbl) throws SQLException {
      ...
      try {
        ...
        rs = stat2.executeQuery("select sql from sqlite_master where" + " lower(name) = lower(\'" + escape(tbl) + "\')");
        if (rs.next()) {
