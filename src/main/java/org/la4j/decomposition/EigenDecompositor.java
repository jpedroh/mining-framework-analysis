...
public class EigenDecompositor implements MatrixDecompositor {
  /**
     ...
     * @return { P, D }
     */
  @Override public Matrix[] decompose(Matrix matrix, Factory factory) {
  ...
  private int findMax(Vector vector, int exl) {
    ...
    for (int i = 0; i < vector.length(); i++) {
      if (i != exl && Math.abs(vector.get(result)) < Math.abs(vector.get(i))) {
        result = i;
      }
    }
    return result;
          ...
          for (int k = m; k <= n - 1; k++) {
            ...
            if (s != 0) {
              ...
              for (int j = k; j < nn; j++) {
                ...
                if (notlast) {
                  p = p + r * H.get(k + 2, j);
                  H.update(k + 2, j, Matrices.asMinusFunction(p * z));
                }
              ...
              for (int i = 0; i <= Math.min(n, k + 3); i++) {
                ...
                if (notlast) {
                  p = p + z * H.get(i, k + 2);
                  H.update(i, k + 2, Matrices.asMinusFunction(p * r));
                }
              ...
              for (int i = low; i <= high; i++) {
                ...
                if (notlast) {
                  p = p + z * V.get(i, k + 2);
                  V.update(i, k + 2, Matrices.asMinusFunction(p * r));
                }
      ...
      if (q == 0) {
        ...
        if (q < 0) {
          ...
          if (Math.abs(H.get(n, n - 1)) > Math.abs(H.get(n - 1, n))) {
            ...
            double cdiv[] = cdiv(0.0, -H.get(n - 1, n), H.get(n - 1, n - 1) - p, q);
          ...
          for (int i = n - 2; i >= 0; i--) {
            ...
            if (e.get(i) < 0.0) {
              ...
              if (e.get(i) == 0) {
                ...
                x = H.get(i, i + 1);
                y = H.get(i + 1, i);
                vr = (d.get(i) - p) * (d.get(i) - p) + e.get(i) * e.get(i) - q * q;
                vi = (d.get(i) - p) * 2.0 * q;
                if (vr == 0.0 & vi == 0.0) {
                  vr = eps * norm * (Math.abs(w) + Math.abs(q) + Math.abs(x) + Math.abs(y) + Math.abs(z));
                }
                double cdiv[] = cdiv(x * r - z * ra + q * sa, x * s - z * sa - q * ra, vr, vi);
                H.set(i, n - 1, cdiv[0]);
                H.set(i, n, cdiv[1]);
                if (Math.abs(x) > (Math.abs(z) + Math.abs(q))) {
                  ...
                  cdiv = cdiv(-r - y * H.get(i, n - 1), -s - y * H.get(i, n), z, q);
