...
public abstract class AbstractVector implements Vector {
  ...
  @Override public Vector add(double value) {
    return add(value, factory);
  }

  @Override public Vector add(double value, Factory factory) {
  ...
  @Override public void addInPlace(double value) {
    for (int i = 0; i < length; i++) {
      update(i, Vectors.asPlusFunction(value));
    }
  }

  @Override public Vector add(Vector vector) {
    return add(vector, factory);
  }

  @Override public Vector add(Vector vector, Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public void addInPlace(Vector vector) {
    ensureArgumentIsNotNull(vector, "vector");
    ensureVectorIsSimilar(vector);
    pipeTo(VectorOperations.inPlaceVectorToVectorAddition(), vector);
  }

  @Override public Vector multiply(double value) {
    return multiply(value, factory);
  }

  @Override public Vector multiply(double value, Factory factory) {
    ensureFactoryIsNotNull(factory);
    return pipeTo(VectorOperations.ooPlaceVectorByValueMultiplication(value, factory));
  }

  @Override public void multiplyInPlace(double value) {
    pipeTo(VectorOperations.inPlaceVectorByValueMultiplication(value));
  }

  @Override public Vector hadamardProduct(Vector vector) {
    return hadamardProduct(vector, factory);
  }

  @Override public Vector hadamardProduct(Vector vector, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(vector, "vector");
    ensureVectorIsSimilar(vector);
    Vector result = blank(factory);
  ...
  @Override public Vector multiply(Matrix matrix) {
    return multiply(matrix, factory);
  }

  @Override public Vector multiply(Matrix matrix, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(matrix, "matrix");
    if (length != matrix.rows()) {
      fail("Wrong matrix dimensions: " + matrix.rows() + "x" + matrix.columns() + ". Should be: " + length + "x_.");
    }
  ...
  @Override public Vector subtract(double value) {
    return subtract(value, factory);
  }

  @Override public Vector subtract(double value, Factory factory) {
    return add(-value, factory);
  }

  @Override public Vector subtract(Vector vector) {
    return subtract(vector, factory);
  }

  @Override public Vector subtract(Vector vector, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(vector, "vector");
    ensureVectorIsSimilar(vector);
    Vector result = blank(factory);
  ...
  @Override public Vector divide(double value) {
    return divide(value, factory);
  }

  @Override public Vector divide(double value, Factory factory) {
  ...
  private static final class SparseDot implements VectorProcedure {
    ...
    @Override public void apply(int i, double value) {
      result += other.get(i) * value;
    }
  }

  @Override public double innerProduct(Vector vector) {
    ensureArgumentIsNotNull(vector, "vector");
    ensureVectorIsSimilar(vector);
    if (vector instanceof SparseVector) {
      ...
      if (this instanceof SparseVector) {
        return vector.innerProduct(this);
      }
    }
    return pipeTo(VectorOperations.ooPlaceInnerProduct(), vector);
  }

  @Override public Matrix outerProduct(Vector vector) {
    return outerProduct(vector, factory);
  }

  @Override public Matrix outerProduct(Vector vector, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(vector, "vector");
  ...
  @Override public Vector blank() {
    return blank(factory);
  }

  @Override public Vector blank(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Vector copy(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Vector resize(int length) {
    return resize(length, factory);
  }

  @Override public Vector resize(int length, Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Vector shuffle() {
    return shuffle(factory);
  }

  @Override public Vector shuffle(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Vector sliceLeft(int until) {
    return slice(0, until, factory);
  }

  @Override public Vector sliceLeft(int until, Factory factory) {
    return slice(0, until, factory);
  }

  @Override public Vector sliceRight(int from) {
    return slice(from, length, factory);
  }

  @Override public Vector sliceRight(int from, Factory factory) {
    return slice(from, length, factory);
  }

  @Override public Vector slice(int from, int until) {
    return slice(from, until, factory);
  }

  @Override public Vector slice(int from, int until, Factory factory) {
    ensureFactoryIsNotNull(factory);
    if (until - from < 0) {
  ...
  @Override public Vector select(int[] indices) {
    return select(indices, factory);
  }

  @Override public Vector select(int[] indices, Factory factory) {
  ...
  @Override public void each(VectorProcedure procedure) {
    for (int i = 0; i < length; i++) {
  ...
  @Override public Vector transform(VectorFunction function) {
    return transform(function, factory);
  }

  @Override public Vector transform(VectorFunction function, Factory factory) {
    Vector result = blank(factory);
    for (int i = 0; i < length; i++) {
  ...
  @Override public Vector transform(int i, VectorFunction function) {
    return transform(i, function, factory);
  }

  @Override public Vector transform(int i, VectorFunction function, Factory factory) {
    Vector result = copy(factory);
    result.set(i, function.evaluate(i, get(i)));
    return result;
  }

  @Override public void update(VectorFunction function) {
    for (int i = 0; i < length; i++) {
      set(i, function.evaluate(i, get(i)));
    }
  }

  @Override public void update(int i, VectorFunction function) {
    set(i, function.evaluate(i, get(i)));
  }

  @Override public double fold(VectorAccumulator accumulator) {
    for (int i = 0; i < length; i++) {
  ...
  @Override public boolean is(VectorPredicate predicate) {
    boolean result = true;
    for (int i = 0; i < length; i++) {
  ...
  @Override public boolean non(VectorPredicate predicate) {
    return !is(predicate);
  }

  @Override public Matrix toRowMatrix() {
    return toRowMatrix(factory);
  }

  @Override public Matrix toRowMatrix(Factory factory) {
    Matrix result = factory.createMatrix(1, length);
    result.setRow(0, this);
    return result;
  }

  @Override public Matrix toColumnMatrix() {
    return toColumnMatrix(factory);
  }

  @Override public Matrix toColumnMatrix(Factory factory) {
  ...
  @Override public String mkString(NumberFormat formatter, String delimiter) {
