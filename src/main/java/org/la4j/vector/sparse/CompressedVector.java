...
public class CompressedVector extends SparseVector {
  ...
  @Override public double getOrElse(int i, double defaultValue) {
  ...
  @Override public void swapElements(int i, int j) {
    if (i == j) {
      return;
    }
  ...
  @Override public Vector copyOfLength(int length) {
  ...
  @Override public void eachNonZero(VectorProcedure procedure) {
    for (int i = 0; i < cardinality; i++) {
      procedure.apply(indices[i], values[i]);
    }
  }

  @Override public void updateAt(int i, VectorFunction function) {
    int k = searchForIndex(i);
    if (k < cardinality && indices[k] == i) {
  ...
  @Override public boolean nonZeroAt(int i) {
    int k = searchForIndex(i);
    return k < cardinality && indices[k] == i;
  }

  @Override public <T extends Vector> T to(VectorFactory<T> factory) {
  ...
  @Override public Vector blankOfLength(int length) {
  ...
  @Override public VectorIterator nonZeroIterator() {
