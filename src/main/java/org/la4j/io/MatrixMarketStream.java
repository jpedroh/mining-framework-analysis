...
public class MatrixMarketStream extends AbstractStream implements MatrixStream, VectorStream {
  ...
  private static class SparseMatrixWriteProcedure implements MatrixProcedure {
    ...
    @Override public void apply(int i, int j, double value) {
      try {
        writer.write((i + 1) + " " + (j + 1) + " " + String.format(Locale.US, "%.12f", value));
        writer.newLine();
      } catch (IOException expected) {
        throw new IllegalArgumentException("Can\'t write with writer.");
      }
  ...
  @Override public Vector readVector() throws IOException {
    return readVector(null);
  }

  @Override public Vector readVector(Factory factory) throws IOException {
  ...
  @Override public void writeVector(Vector vector) throws IOException {
  ...
  @Override public Matrix readMatrix(Factory factory) throws IOException {
    ensureReaderInitialized();
  ...
  @Override public Matrix readMatrix() throws IOException {
    return readMatrix(null);
  }

  @Override public void writeMatrix(Matrix matrix) throws IOException {
    ensureWriterInitialized();
  ...
  private void writeSparseMatrix(SparseMatrix matrix) throws IOException {
    ...
    writer.newLine();
    writer.write(matrix.rows() + " " + matrix.columns() + " " + matrix.cardinality());
    writer.newLine();
  ...
  private Vector parseVector(Factory factory) throws IOException {
    ...
    if (token.equals("array")) {
      return parseDenseVector(factory == null ? Vectors.DEFAULT_DENSE_FACTORY : factory);
    } else {
      if (token.equals("coordinate")) {
        return parseSparseVector(factory == null ? Vectors.DEFAULT_SPARSE_FACTORY : factory);
      } else {
  ...
  private Matrix parseMatrix(Factory factory) throws IOException {
    ...
    ensureNext("general");
    if (token.equals("array")) {
      return parseDenseMatrix(factory == null ? Matrices.DEFAULT_DENSE_FACTORY : factory);
    } else {
      if (token.equals("coordinate")) {
        return parseSparseMatrix(factory == null ? Matrices.DEFAULT_SPARSE_FACTORY : factory);
      } else {
        throw new IOException("Unexpected token at stream: \"" + token + "\".");
      }
    }
