...
public abstract class AbstractMatrix implements Matrix {
  ...
  @Override public void assign(double value) {
    update(Matrices.asConstFunction(value));
  }

  @Override public void assignRow(int i, double value) {
    setRow(i, value);
  }

  @Override public void assignColumn(int j, double value) {
    setColumn(j, value);
  }

  @Override public void setRow(int i, double value) {
    updateRow(i, Matrices.asConstFunction(value));
  }

  @Override public void setColumn(int j, double value) {
  ...
  @Override public Vector getRow(int i, Factory factory) {
  ...
  @Override public Vector getColumn(int j) {
    return getColumn(j, factory);
  }

  @Override public Vector getColumn(int j, Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public void setRow(int i, Vector row) {
  ...
  @Override public void setColumn(int j, Vector column) {
  ...
  @Override public Matrix removeRow(int i) {
    return removeRow(i, factory);
  }

  @Override public Matrix removeRow(int i, Factory factory) {
    if (i >= rows || i < 0) {
  ...
  @Override public Matrix removeColumn(int j) {
    return removeColumn(j, factory);
  }

  @Override public Matrix removeColumn(int j, Factory factory) {
    if (j >= columns || j < 0) {
  ...
  @Override public Matrix removeFirstRow() {
    return removeFirstRow(factory);
  }

  @Override public Matrix removeFirstRow(Factory factory) {
    return removeRow(0, factory);
  }

  @Override public Matrix removeFirstColumn() {
    return removeFirstColumn(factory);
  }

  @Override public Matrix removeFirstColumn(Factory factory) {
    return removeColumn(0, factory);
  }

  @Override public Matrix removeLastRow() {
    return removeLastRow(factory);
  }

  @Override public Matrix removeLastRow(Factory factory) {
    return removeRow(rows - 1, factory);
  }

  @Override public Matrix removeLastColumn() {
    return removeLastColumn(factory);
  }

  @Override public Matrix removeLastColumn(Factory factory) {
    return removeColumn(columns - 1, factory);
  }

  @Override public void swapRows(int i, int j) {
    if (i != j) {
      Vector ii = getRow(i);
      Vector jj = getRow(j);
      setRow(i, jj);
  ...
  @Override public void swapColumns(int i, int j) {
    if (i != j) {
  ...
  @Override public Matrix transpose(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Matrix rotate() {
    return rotate(factory);
  }

  @Override public Matrix rotate(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Matrix power(int n) {
    return power(n, factory);
  }

  @Override public Matrix power(int n, Factory factory) {
    if (n < 0) {
  ...
  @Override public Matrix multiply(double value) {
    return multiply(value, factory);
  }

  @Override public Matrix multiply(double value, Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Vector multiply(Vector vector) {
    return multiply(vector, factory);
  }

  @Override public Vector multiply(Vector vector, Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Matrix multiply(Matrix matrix) {
    return multiply(matrix, factory);
  }

  @Override public Matrix multiply(Matrix matrix, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(matrix, "matrix");
    if (columns != matrix.rows()) {
      fail("Wrong matrix dimensions: " + matrix.rows() + "x" + matrix.columns() + ". Should be: " + columns + "x_.");
    }
  ...
  @Override public Matrix multiplyByItsTranspose() {
    return multiplyByItsTranspose(factory);
  }

  @Override public Matrix multiplyByItsTranspose(Factory factory) {
    ensureFactoryIsNotNull(factory);
    if (rows != columns) {
  ...
  @Override public Matrix subtract(double value) {
    return subtract(value, factory);
  }

  @Override public Matrix subtract(double value, Factory factory) {
    return add(-value, factory);
  }

  @Override public Matrix subtract(Matrix matrix) {
    return subtract(matrix, factory);
  }

  @Override public Matrix subtract(Matrix matrix, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(matrix, "matrix");
    if (rows != matrix.rows() || columns != matrix.columns()) {
      fail("Wrong matrix dimensions: " + matrix.rows() + "x" + matrix.columns() + ". Should be: " + rows + "x" + columns + ".");
    }
    Matrix result = blank(factory);
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  @Override public Matrix add(double value) {
    return add(value, factory);
  }

  @Override public Matrix add(double value, Factory factory) {
    ensureFactoryIsNotNull(factory);
    Matrix result = blank(factory);
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  @Override public Matrix add(Matrix matrix) {
    return add(matrix, factory);
  }

  @Override public Matrix add(Matrix matrix, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(matrix, "matrix");
    if (rows != matrix.rows() || columns != matrix.columns()) {
      fail("Wrong matrix dimensions: " + matrix.rows() + "x" + matrix.columns() + ". Should be: " + rows + "x" + columns + ".");
  ...
  @Override public Matrix insert(Matrix matrix) {
    return insert(matrix, 0, 0, 0, 0, matrix.rows(), matrix.columns());
  }

  @Override public Matrix insert(Matrix matrix, int rows, int columns) {
    return insert(matrix, 0, 0, 0, 0, rows, columns);
  }

  @Override public Matrix insert(Matrix matrix, int destRow, int destCol, int rows, int columns) {
    return insert(matrix, 0, 0, destRow, destCol, rows, columns);
  }

  @Override public Matrix insert(Matrix matrix, int srcRow, int srcCol, int destRow, int destCol, int rows, int columns) {
    ensureArgumentIsNotNull(matrix, "matrix");
    if (rows < 0 || columns < 0) {
    ...
    if (destRow + rows > this.columns || destCol + columns > this.rows) {
      fail("Out of bounds: Cannot add " + rows + " rows and " + columns + " cols at " + destRow + ", " + destCol + " in a " + this.rows + "x" + this.columns + " matrix.");
    }
    if (srcRow + rows > matrix.rows() || srcCol + columns > matrix.columns()) {
      fail("Out of bounds: Cannot get " + rows + " rows and " + columns + " cols at " + srcRow + ", " + srcCol + " from a " + matrix.rows() + "x" + matrix.columns() + " matrix.");
    }
  ...
  @Override public Matrix divide(double value) {
    return divide(value, factory);
  }

  @Override public Matrix divide(double value, Factory factory) {
    return multiply(1.0 / value, factory);
  }

  @Override public Matrix kroneckerProduct(Matrix matrix) {
    return kroneckerProduct(matrix, factory);
  }

  @Override public Matrix kroneckerProduct(Matrix matrix, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(matrix, "matrix");
  ...
  @Override public double diagonalProduct() {
  ...
  @Override public Matrix hadamardProduct(Matrix matrix) {
    return hadamardProduct(matrix, factory);
  }

  @Override public Matrix hadamardProduct(Matrix matrix, Factory factory) {
    ensureFactoryIsNotNull(factory);
    ensureArgumentIsNotNull(matrix, "matrix");
  ...
  @Override public Matrix blank() {
    return blank(factory);
  }

  @Override public Matrix blank(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Matrix copy(Factory factory) {
    ensureFactoryIsNotNull(factory);
    return factory.createMatrix(this);
  }

  @Override public Matrix resize(int rows, int columns) {
    return resize(rows, columns, factory);
  }

  @Override public Matrix resizeRows(int rows) {
    return resize(rows, columns, factory);
  }

  @Override public Matrix resizeRows(int rows, Factory factory) {
    return resize(rows, columns, factory);
  }

  @Override public Matrix resizeColumns(int columns) {
    return resize(rows, columns, factory);
  }

  @Override public Matrix resizeColumns(int columns, Factory factory) {
    return resize(rows, columns, factory);
  }

  @Override public Matrix resize(int rows, int columns, Factory factory) {
    ensureFactoryIsNotNull(factory);
    Matrix result = factory.createMatrix(rows, columns);
  ...
  @Override public Matrix shuffle() {
    return shuffle(factory);
  }

  @Override public Matrix shuffle(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Matrix slice(int fromRow, int fromColumn, int untilRow, int untilColumn) {
    return slice(fromRow, fromColumn, untilRow, untilColumn, factory);
  }

  @Override public Matrix slice(int fromRow, int fromColumn, int untilRow, int untilColumn, Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Matrix sliceTopLeft(int untilRow, int untilColumn) {
    return slice(0, 0, untilRow, untilColumn, factory);
  }

  @Override public Matrix sliceTopLeft(int untilRow, int untilColumn, Factory factory) {
    return slice(0, 0, untilRow, untilColumn, factory);
  }

  @Override public Matrix sliceBottomRight(int fromRow, int fromColumn) {
    return slice(fromRow, fromColumn, rows, columns, factory);
  }

  @Override public Matrix sliceBottomRight(int fromRow, int fromColumn, Factory factory) {
    return slice(fromRow, fromColumn, rows, columns, factory);
  }

  @Override public Matrix select(int[] rowIndices, int[] columnIndices) {
    return select(rowIndices, columnIndices, factory);
  }

  @Override public Matrix select(int[] rowIndices, int[] columnIndices, Factory factory) {
  ...
  @Override public Factory factory() {
    return factory;
  }

  @Override public void each(MatrixProcedure procedure) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  @Override public void eachInRow(int i, MatrixProcedure procedure) {
    for (int j = 0; j < columns; j++) {
      procedure.apply(i, j, get(i, j));
    }
  }

  @Override public void eachInRow(int i, VectorProcedure procedure) {
    for (int j = 0; j < columns; j++) {
      procedure.apply(j, get(i, j));
    }
  }

  @Override public void eachInColumn(int j, MatrixProcedure procedure) {
    for (int i = 0; i < rows; i++) {
      procedure.apply(i, j, get(i, j));
    }
  }

  @Override public void eachInColumn(int j, VectorProcedure procedure) {
    for (int i = 0; i < rows; i++) {
  ...
  @Override public double maxInRow(int i) {
    return foldRow(i, Matrices.mkMaxAccumulator());
  }

  @Override public double minInRow(int i) {
    return foldRow(i, Matrices.mkMinAccumulator());
  }

  @Override public double maxInColumn(int j) {
    return foldColumn(j, Matrices.mkMaxAccumulator());
  }

  @Override public double minInColumn(int j) {
    return foldColumn(j, Matrices.mkMinAccumulator());
  }

  @Override public Matrix transform(MatrixFunction function) {
    return transform(function, factory);
  }

  @Override public Matrix transform(MatrixFunction function, Factory factory) {
    Matrix result = blank(factory);
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  @Override public Matrix transform(int i, int j, MatrixFunction function) {
    return transform(i, j, function, factory);
  }

  @Override public Matrix transform(int i, int j, MatrixFunction function, Factory factory) {
    Matrix result = copy(factory);
    result.set(i, j, function.evaluate(i, j, result.get(i, j)));
    return result;
  }

  @Override public Matrix transformRow(int i, MatrixFunction function) {
    return transformRow(i, function, factory);
  }

  @Override public Matrix transformRow(int i, VectorFunction function) {
    return transformRow(i, function, factory);
  }

  @Override public Matrix transformRow(int i, MatrixFunction function, Factory factory) {
    Matrix result = copy(factory);
    for (int j = 0; j < columns; j++) {
      result.set(i, j, function.evaluate(i, j, result.get(i, j)));
    }
  ...
  @Override public Matrix transformRow(int i, VectorFunction function, Factory factory) {
    Matrix result = copy(factory);
    for (int j = 0; j < columns; j++) {
  ...
  @Override public Matrix transformColumn(int j, MatrixFunction function) {
    return transformColumn(j, function, factory);
  }

  @Override public Matrix transformColumn(int j, VectorFunction function) {
    return transformColumn(j, function, factory);
  }

  @Override public Matrix transformColumn(int j, MatrixFunction function, Factory factory) {
    Matrix result = copy(factory);
    for (int i = 0; i < rows; i++) {
      result.set(i, j, function.evaluate(i, j, result.get(i, j)));
    }
  ...
  @Override public Matrix transformColumn(int j, VectorFunction function, Factory factory) {
    Matrix result = copy(factory);
    for (int i = 0; i < rows; i++) {
  ...
  @Override public void update(MatrixFunction function) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  @Override public void update(int i, int j, MatrixFunction function) {
    set(i, j, function.evaluate(i, j, get(i, j)));
  }

  @Override public void updateRow(int i, MatrixFunction function) {
    for (int j = 0; j < columns; j++) {
      update(i, j, function);
    }
  }

  @Override public void updateRow(int i, VectorFunction function) {
  ...
  @Override public void updateColumn(int j, MatrixFunction function) {
    for (int i = 0; i < rows; i++) {
      update(i, j, function);
    }
  }

  @Override public void updateColumn(int j, VectorFunction function) {
  ...
  @Override public double fold(MatrixAccumulator accumulator) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  @Override public double foldRow(int i, MatrixAccumulator accumulator) {
    for (int j = 0; j < columns; j++) {
      accumulator.update(i, j, get(i, j));
    }
    return accumulator.accumulate();
  ...
  @Override public double foldRow(int i, VectorAccumulator accumulator) {
    for (int j = 0; j < columns; j++) {
  ...
  @Override public Vector foldRows(MatrixAccumulator accumulator) {
    Vector result = factory.createVector(rows);
    for (int i = 0; i < rows; i++) {
  ...
  @Override public Vector foldRows(VectorAccumulator accumulator) {
    Vector result = factory.createVector(rows);
    for (int i = 0; i < rows; i++) {
      result.set(i, foldRow(i, accumulator));
    }
  ...
  @Override public double foldColumn(int j, MatrixAccumulator accumulator) {
    for (int i = 0; i < rows; i++) {
      accumulator.update(i, j, get(i, j));
    }
    return accumulator.accumulate();
  ...
  @Override public double foldColumn(int j, VectorAccumulator accumulator) {
    for (int i = 0; i < rows; i++) {
  ...
  @Override public Vector foldColumns(MatrixAccumulator accumulator) {
    Vector result = factory.createVector(columns);
  ...
  @Override public Vector foldColumns(VectorAccumulator accumulator) {
    Vector result = factory.createVector(columns);
    for (int i = 0; i < columns; i++) {
      result.set(i, foldColumn(i, accumulator));
    }
  ...
  @Override public boolean is(MatrixPredicate predicate) {
  ...
  @Override public boolean is(AdvancedMatrixPredicate predicate) {
    return predicate.test(this);
  }

  @Override public boolean non(MatrixPredicate predicate) {
    return !is(predicate);
  }

  @Override public boolean non(AdvancedMatrixPredicate predicate) {
    return !is(predicate);
  }

  @Override public Vector toRowVector() {
    return toRowVector(factory);
  }

  @Override public Vector toRowVector(Factory factory) {
    return getRow(0, factory);
  }

  @Override public Vector toColumnVector() {
    return toColumnVector(factory);
  }

  @Override public Vector toColumnVector(Factory factory) {
    return getColumn(0, factory);
  }

  @Override public LinearSystemSolver withSolver(LinearAlgebra.SolverFactory factory) {
    return factory.create(this);
  }

  @Override public MatrixInverter withInverter(LinearAlgebra.InverterFactory factory) {
    return factory.create(this);
  }

  @Override public MatrixDecompositor withDecompositor(LinearAlgebra.DecompositorFactory factory) {
    return factory.create(this);
  }
  ...
  @Override public boolean equals(Matrix matrix, double precision) {
    if (rows != matrix.rows() || columns != matrix.columns()) {
      return false;
    }
    boolean result = true;
  ...
  @Override public String mkString(NumberFormat formatter) {
    return mkString(formatter, DEFAULT_ROWS_DELIMITER, DEFAULT_COLUMNS_DELIMITER);
  }

  @Override public String mkString(String rowsDelimiter, String columnsDelimiter) {
    return mkString(DEFAULT_FORMATTER, rowsDelimiter, columnsDelimiter);
  }

  @Override public String mkString(NumberFormat formatter, String rowsDelimiter, String columnsDelimiter) {
