...
public abstract class AbstractMatrix implements Matrix {
  ...
  @Override public int rank() {
    ...
    for (int k = 0; k < min; k++) {
      ...
      for (int i = k + 1; i < rows; i++) {
        for (int j = k; j < columns; j++) {
          copy.update(i, j, Matrices.asMinusFunction(copy.get(k, j) * copy.get(i, k)));
        }
      }
    ...
    int result = 0;
    while (result < min && Math.abs(copy.get(result, result)) > Matrices.EPS) {
      result++;
    }
    return result;
  }
  ...
  public Matrix power(int n, Factory factory) {
    if (n < 0) {
      throw new IllegalArgumentException("The exponent has to be larger than 0.");
    }
  ...
  @Override public Matrix hadamardProduct(Matrix matrix, Factory factory) {
    ...
    if ((columns != matrix.columns()) || (rows != matrix.rows())) {
      throw new IllegalArgumentException("Matrices dimensions are not equal: " + matrix.rows() + "x" + matrix.columns() + " not equal to " + rows + "x" + columns);
    }
  ...
  @Override public Matrix triangularize() {
    return triangularize(factory);
  }

  @Override public Matrix triangularize(Factory factory) {
    ensureFactoryIsNotNull(factory);
  ...
  @Override public Matrix[] decompose(MatrixDecompositor decompositor) {
    return decompose(decompositor, factory);
  }

  @Override public Matrix[] decompose(MatrixDecompositor decompositor, Factory factory) {
    return decompositor.decompose(this, factory);
  }

  @Override public Matrix inverse(MatrixInvertor invertor) {
    return inverse(invertor, factory);
  }

  @Override public Matrix inverse(MatrixInvertor invertor, Factory factory) {
  ...
  @Override public Matrix sliceBottomRight(int fromRow, int fromColumn, Factory fac) {
    return slice(fromRow, fromColumn, rows, columns, factory);
  }
  ...
  @Override public void eachNonZero(MatrixProcedure procedure) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  @Override public void eachNonZeroInRow(int i, MatrixProcedure procedure) {
    for (int j = 0; j < columns; j++) {
      if (Math.abs(get(i, j)) > Matrices.EPS) {
        procedure.apply(i, j, get(i, j));
      }
  ...
  @Override public void eachNonZeroInColumn(int j, MatrixProcedure procedure) {
    for (int i = 0; i < rows; i++) {
      if (Math.abs(get(i, j)) > Matrices.EPS) {
        procedure.apply(i, j, get(i, j));
      }
  ...
  @Override public boolean is(MatrixPredicate predicate) {
    boolean result = (predicate instanceof AdvancedMatrixPredicate) ? ((AdvancedMatrixPredicate) predicate).test(rows, columns) : rows > 0 && columns > 0;
  ...
  @Override public String toString() {
    ...
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        ...
        int size = String.valueOf((long) value).length() + precision + (value < 0 && value > -1.0 ? 1 : 0) + 2;
    ...
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        sb.append(String.format("%" + Integer.toString(formats[j]) + "." + precision + "f", get(i, j)));
      }
      sb.append("\n");
    }
    return sb.toString();
