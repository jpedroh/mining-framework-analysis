...
public abstract class Matrix implements Iterable<Double> {
  ...
  public static Matrix fromMatrixMarket(String mm) {
    ...
    if ("coordinate".equals(format)) {
      ...
      Matrix result = "row-major".equals(majority) ? RowMajorSparseMatrix.zero(rows, columns, cardinality) : ColumnMajorSparseMatrix.zero(rows, columns, cardinality);
  ...
  /**
     ...
     */
  public abstract <T extends java.lang.Object> T apply(MatrixOperation<T> operation);
  ...
  /**
     ...
     * @return the result of an operation applied to this matrix
     */
  public abstract <T extends java.lang.Object> T apply(MatrixMatrixOperation<T> operation, Matrix that);
  ...
  /**
     ...
     * @return the result of an operation applied to this matrix
     */
  public abstract <T extends java.lang.Object> T apply(MatrixVectorOperation<T> operation, Vector that);
  ...
  public Matrix insert(Matrix that, int srcRow, int srcColumn, int destRow, int destColumn, int rows, int columns) {
    ...
    if (destRow + rows > this.rows || destColumn + columns > this.columns) {
      fail("Out of bounds: Cannot add " + rows + " rows and " + columns + " cols at " + destRow + ", " + destColumn + " in a " + this.rows + "x" + this.columns + " matrix.");
    }
    if (srcRow + rows > that.rows || srcColumn + columns > that.columns) {
      fail("Out of bounds: Cannot get " + rows + " rows and " + columns + " cols at " + srcRow + ", " + srcColumn + " from a " + that.rows + "x" + that.columns + " matrix.");
    }
    Matrix result = copy();
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
  ...
  /**
     ...
     */
  @Override public MatrixIterator iterator() {
