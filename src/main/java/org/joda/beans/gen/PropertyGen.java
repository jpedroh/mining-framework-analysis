...
class PropertyGen {
  ...
  private boolean parseFinal(List<String> content) {
    ...
    if (parts[0].equals("final") || parts[1].equals("final") || (parts.length >= 3 && parts[2].equals("final"))) {
      return true;
    }
    return false;
  }
    ...
    if (commentEnd.equals("*/")) {
      ...
      if (startCommentIndex < annotationIndex - 1) {
        for (int i = startCommentIndex; i < annotationIndex - 1; i++) {
          ...
          if (commentLine.startsWith("@return") == false && commentLine.startsWith("@param") == false && commentLine.startsWith("@throws") == false && commentLine.startsWith("@exception") == false) {
      ...
      if (commentEnd.startsWith("/**") && commentEnd.endsWith("*/")) {
  ...
  List<String> generateMetaPropertyConstant() {
    ...
    if (data.isBeanGenericType()) {
      ...
      list.add("\t\tprivate final MetaProperty<" + propertyType() + "> " + metaFieldName() + " = (DirectMetaProperty) DirectMetaProperty.of" + readWrite() + "(");
      list.add("\t\t\t\tthis, \"" + data.getPropertyName() + "\", " + data.getBean().getTypeRaw() + ".class, " + actualType() + ");");
    } else {
      ...
      list.add("\t\tprivate final MetaProperty<" + propertyType + "> " + metaFieldName() + " = DirectMetaProperty.of" + readWrite() + "(");
      list.add("\t\t\t\tthis, \"" + data.getPropertyName() + "\", " + data.getBean().getTypeRaw() + ".class, " + actualType() + ");");
    }
    return list;
  }
  ...
  List<String> generateBuilderSetMethod() {
    ...
    list.add("\t\tpublic Builder" + data.getBean().getTypeGenericName(true) + " " + data.getPropertyName() + "(" + getBuilderType() + " " + data.getPropertyName() + ") {");
    if (data.isValidated()) {
