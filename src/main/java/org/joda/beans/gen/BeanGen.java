...
class BeanGen {
  ...
  private void generateFactory() {
    if (data.isFactoryRequired()) {
      ...
      if (nonDerived.isEmpty()) {
        ...
        if (data.isTypeGeneric()) {
          addLine(1, "public static " + data.getTypeGeneric(true) + " " + data.getTypeNoExtends() + " " + data.getFactoryName() + "(");
        } else {
          addLine(1, "public static " + data.getTypeNoExtends() + " " + data.getFactoryName() + "(");
        }
        for (int i = 0; i < nonDerived.size(); i++) {
  ...
  private void generateImmutableBuilderMethod() {
    if (data.isConstructable() && data.isBuilderGenerated()) {
      ...
      addLine(1, " */");
      if (data.isTypeGeneric()) {
        addLine(1, data.getEffectiveBuilderScope() + "static " + data.getTypeGeneric(true) + " " + data.getEffectiveBeanBuilderName() + data.getTypeGenericName(true) + " builder() {");
      } else {
  ...
  private void generateBuilderBasedConstructor() {
    if (data.getConstructorStyle() == CONSTRUCTOR_BY_BUILDER && data.getImmutableConstructor() == CONSTRUCTOR_NONE && ((data.isMutable() && data.isBuilderScopeVisible()) || data.isImmutable())) {
      List<PropertyGen> nonDerived = nonDerivedProperties();
      ...
      for (PropertyGen prop : properties) {
        if (prop.getData().isValidated()) {
          addLine(2, prop.getData().getValidationMethodName() + "(builder." + prop.generateBuilderFieldName() + ", \"" + prop.getData().getPropertyName() + "\");");
        }
      }
      if (data.isImmutable()) {
        for (int i = 0; i < nonDerived.size(); i++) {
  ...
  private void generateArgBasedConstructor() {
    if (data.getConstructorStyle() == CONSTRUCTOR_BY_ARGS && data.getImmutableConstructor() == CONSTRUCTOR_NONE && ((data.isMutable() && (data.isBuilderScopeVisible() || data.isBeanStyleLight())) || data.isImmutable())) {
      ...
      if (nonDerived.size() == 0) {
        ...
        if (!(data.isMutable() && data.isBeanStyleLight())) {
          for (PropertyGen prop : properties) {
            if (prop.getData().isValidated()) {
              addLine(2, prop.getData().getValidationMethodName() + "(" + prop.getData().getPropertyName() + ", \"" + prop.getData().getPropertyName() + "\");");
            }
          }
        }
        for (int i = 0; i < nonDerived.size(); i++) {
  ...
  private void generateMeta() {
    if (data.isBeanStyleLightOrMinimal()) {
      ...
      addLine(1, " */");
      boolean genericProps = data.getProperties().stream().filter((p) -> p.isGeneric()).findAny().isPresent();
      ...
      if (data.isBeanStyleLight()) {
        ...
        if (nonDerived.isEmpty()) {
          ...
          if (specialInit) {
            for (int i = 0; i < nonDerived.size(); i++) {
              addLine(5, nonDerived.get(i).generateInit() + joinComma(i, nonDerived, ")" + (hasAliases ? ";" : "")));
            }
          } else {
          ...
          for (int i = 0; i < aliases.size(); i++) {
            ...
            addLine(5, ".withAlias(\"" + prop.getData().getAlias() + "\", \"" + prop.getData().getPropertyName() + "\")" + join(i, aliases, "", ";"));
          }
        }
      } else {
        ...
        if (nonDerived.isEmpty()) {
          ...
          if (data.isImmutable()) {
            for (int i = 0; i < nonDerived.size(); i++) {
              addLine(5, nonDerived.get(i).generateLambdaGetter() + joinComma(i, nonDerived, ")" + (hasAliases ? ";" : "")));
            }
          } else {
            ...
            for (int i = 0; i < nonDerived.size(); i++) {
              addLine(7, nonDerived.get(i).generateLambdaSetter() + joinComma(i, nonDerived, "))" + (hasAliases ? ";" : "")));
            }
          }
          for (int i = 0; i < aliases.size(); i++) {
            PropertyGen prop = aliases.get(i);
  ...
  private void generateMetaForGenericType() {
    ...
    if (data.getTypeGenericCount() == 1) {
      addLine(1, "public static <R" + data.getTypeGenericExtends(0, typeNames) + "> " + data.getTypeRaw() + ".Meta<R> meta" + data.getTypeRaw() + "(Class<R> cls) {");
    } else {
      if (data.getTypeGenericCount() == 2) {
        addLine(1, "public static <R" + data.getTypeGenericExtends(0, typeNames) + ", S" + data.getTypeGenericExtends(1, typeNames) + "> " + data.getTypeRaw() + ".Meta<R, S> meta" + data.getTypeRaw() + "(Class<R> cls1, Class<S> cls2) {");
      } else {
        if (data.getTypeGenericCount() == 3) {
          addLine(1, "public static <R" + data.getTypeGenericExtends(0, typeNames) + ", S" + data.getTypeGenericExtends(1, typeNames) + ", T" + data.getTypeGenericExtends(2, typeNames) + "> " + data.getTypeRaw() + ".Meta<R, S, T> meta" + data.getTypeRaw() + "(Class<R> cls1, Class<S> cls2, Class<T> cls3) {");
        }
      }
    }
    addLine(2, "return " + data.getTypeRaw() + ".Meta.INSTANCE;");
  ...
  private void generateMetaBean() {
    if (data.isMetaScopePrivate() || data.isBeanStyleMinimal()) {
      ...
      addLine(1, "@Override");
      addLine(1, "public " + data.getTypeRaw() + ".Meta" + data.getTypeGenericName(true) + " metaBean() {");
      addLine(2, "return " + data.getTypeRaw() + ".Meta.INSTANCE;");
      addLine(1, "}");
      addBlankLine();
    }
  ...
  private void generateClone() {
    if (data.isSkipCloneGeneration() || data.isManualClone() || (data.isRootClass() == false && data.isConstructable() == false)) {
      return;
    }
    addLine(1, "@Override");
    if (data.isImmutable()) {
  ...
  private void generateEquals() {
    ...
    if (nonDerived.size() == 0) {
      ...
      for (int i = 0; i < nonDerived.size(); i++) {
        ...
        addLine(0, (i == 0 ? "\t\t\treturn " : "\t\t\t\t\t") + equals + (data.isSubClass() || i < nonDerived.size() - 1 ? " &&" : ";"));
      }
      if (data.isSubClass()) {
  ...
  private void generateToString() {
    ...
    if (data.isRootClass() && data.isTypeFinal()) {
      ...
      if (props.size() > 0) {
        ...
        for (int i = 0; i < props.size(); i++) {
          ...
          addLine(2, "buf.append(\"" + prop.getData().getPropertyName() + "\").append(\'=\')" + join(i, props, ".append(" + getter + ").append(\',\').append(\' \');", ".append(JodaBeanUtils.toString(" + getter + "));"));
        }
      }
    ...
    for (int i = 0; i < props.size(); i++) {
      ...
      String getter = toStringFieldAccessor(prop);
      data.ensureImport(JodaBeanUtils.class);
      addLine(2, "buf.append(\"" + prop.getData().getPropertyName() + "\").append(\'=\').append(JodaBeanUtils.toString(" + getter + ")).append(\',\').append(\' \');");
    }
    addLine(1, "}");
    addBlankLine();
  }
  ...
  private void generateMetaClass() {
    ...
    if (data.isTypeGeneric()) {
      addLine(1, data.getEffectiveMetaScope() + "static " + finalType + "class Meta" + data.getTypeGeneric(true) + " extends " + superMeta + " {");
    } else {
      addLine(1, data.getEffectiveMetaScope() + "static " + finalType + "class Meta extends " + superMeta + " {");
    }
    addLine(2, "/**");
  ...
  private void generateMetaBuilder() {
    ...
    if (data.isImmutable() && data.isEffectiveBuilderScopeVisible() == false) {
      ...
      if (data.isImmutable() || (data.isMutable() && data.isBuilderScopeVisible())) {
  ...
  private void generateMetaSetPropertyValue() {
    ...
    for (PropertyData prop : data.getProperties()) {
      generics |= (prop.getStyle().isWritable() && ((prop.isGeneric() && prop.isGenericWildcardParamType() == false) || data.isTypeGeneric()));
    }
    if (generics) {
  ...
  private void generateMetaValidate() {
    ...
    if (data.isValidated()) {
      for (PropertyGen prop : properties) {
        if (prop.getData().isValidated()) {
          addLine(3, prop.getData().getValidationMethodName() + "(((" + data.getTypeWildcard() + ") bean)." + prop.getData().getFieldName() + ", \"" + prop.getData().getPropertyName() + "\");");
        }
      }
    }
    if (data.isSubClass()) {
  ...
  private void generateBuilderClass() {
    ...
    if (data.isConstructable()) {
      addLine(1, data.getEffectiveBuilderScope() + "static " + finalType + "class Builder" + data.getTypeGeneric(true) + " extends " + superBuilder + " {");
    } else {
      addLine(1, data.getEffectiveBuilderScope() + "abstract static " + finalType + "class Builder" + data.getTypeGeneric(true) + " extends " + superBuilder + " {");
    }
    if (nonDerived.size() > 0) {
      addBlankLine();
  ...
  private void generateBuilderSet() {
    List<PropertyGen> nonDerived = nonDerivedProperties();
    boolean generics = data.getProperties().stream().filter((p) -> p.isGeneric() && p.isGenericWildcardParamType() == false).findAny().isPresent();
    if (generics) {
      addLine(2, "@SuppressWarnings(\"unchecked\")");
    }
    addLine(2, "@Override");
  ...
  private void generateBuilderToString() {
    ...
    for (int i = 0; i < nonDerived.size(); i++) {
      ...
      String getter = nonDerived.get(i).generateBuilderFieldName();
      data.ensureImport(JodaBeanUtils.class);
      addLine(3, "buf.append(\"" + prop.getData().getPropertyName() + "\").append(\'=\').append(JodaBeanUtils.toString(" + getter + ")).append(\',\').append(\' \');");
    }
    addLine(2, "}");
    addBlankLine();
  }
