...
public class HttpServer implements EventHandler {
  ...
  public HttpServer(Object requestCallback, boolean noKeepAlive, IOLoop loop, boolean xHeaders) throws Exception {
    ...
    logger.info("Thread poll fixed in " + Runtime.getRuntime().availableProcessors() + " threads");
  ...
  public void listen(int port) throws Exception {
    ...
    this.getLoop().addHandler(serverSocketChannel, this, serverSocketChannel.validOps());
    this.getLoop().start();
  }
  ...
  /**
	 ...
	 */
  @Override public void handleEvents(SelectionKey key) throws Exception {
    int accepted = 0;
    while (true) {
      accepted++;
      try {
        ...
        new HttpConnection(stream, clientChannel.socket().getInetAddress(), requestCallback, noKeepAlive, xHeaders);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  public static class HttpConnection implements StreamHandler {
    ...
    public HttpConnection(IOStream stream, InetAddress inetAddress, Object requestCallback, boolean noKeepAlive, boolean xHeaders) throws Exception {
      this.stream = stream;
    ...
    @Override public void execute(String data) {
      try {
        ...
        if (!version.startsWith("HTTP/")) {
          throw new RuntimeException("Malformed HTTP version in HTTP Request-Line");
        }
        HttpHeaders headers = HttpHeaders.parse(data.substring(eol + 1, data.length() - 1));
        httpRequest = new HttpRequest(method, uri, version, headers, address.getHostAddress(), this);
        int contentLength = 0;
        Integer.valueOf(headers.get("Content-Lenght", "0"));
        if (contentLength > 0 && contentLength > stream.getMaxBufferSize()) {
          throw new RuntimeException("Content-Length too long");
        }
        if (contentLength > 0 && headers.get("Expect", "").equals("100-continue")) {
  ...
  public static class HttpRequest {
    ...
    public HttpRequest(String method, String uri, String version, HttpHeaders headers, String remoteIp, HttpConnection connection) {
      this.method = method;
      this.uri = uri;
