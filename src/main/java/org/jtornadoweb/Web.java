...
public class Web {
  public static class RequestHandler {
    /**
		 ...
		 * methods.
		 */
    private static final String[] SUPPORTED_METHODS = { "GET", "HEAD", "POST", "DELETE", "PUT" };
    ...
    protected void setHeader(String name, String value) {
      ...
      if (!value.equals(saveValue)) {
        throw new IllegalArgumentException("Unsafe header value " + value);
      }
    ...
    /**
		 ...
		 * @return
		 */
    protected String getArgument(String name, String defaultValue, boolean trim) {
    ...
    protected List<String> getArguments(String name, boolean trim) {
      ...
      for (String s : request.arguments.get(name)) {
        _validValues.add(unicode(s.replaceAll("[\u0000-\b\u000e-\u001f]", " ")));
      }
      return _validValues;
    }
    ...
    /**
		 ...
		 * @param expiresDays
		 */
    protected void setCookie(String name, String value, String domain, Calendar expires, String path, Integer expiresDays) {
      name = utf8(name);
      value = utf8(value);
      if (name.concat(value).matches(".*[\u0000- ].*")) {
        throw new IllegalArgumentException(String.format("Invalid cookie %s : %s", name, value));
      }
    ...
    private String generateHeaders(boolean includeFooters) {
      return "HTTP/1.1 200 OK\r\nContent-Length: " + writeBuffer.getBytes().length + "\r\n\r\n";
    }
  }
  ...
  public static class Application implements RequestCallback {
    ...
    /**
		 ...
		 */
    @Override public void execute(HttpRequest request) {
      String path = request.path;
      RequestHandler handler = null;
      for (Map.Entry<Pattern, Class<? extends RequestHandler>> entry : handlers.entrySet()) {
        if (entry.getKey().matcher(path).matches()) {
          try {
            handler = (RequestHandler) entry.getValue().newInstance();
