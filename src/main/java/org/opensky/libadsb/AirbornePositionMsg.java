...
public class AirbornePositionMsg extends ExtendedSquitter {
  ...
  public AirbornePositionMsg(String raw_message) throws Exception {
    super(raw_message);
    assert getFormatTypeCode() == 0 || (getFormatTypeCode() >= 9 && getFormatTypeCode() <= 18) || (getFormatTypeCode() >= 20 && getFormatTypeCode() <= 22) : "This is not a position message! Wrong format type code (" + getFormatTypeCode() + ").";
    byte[] msg = getMessage();
  ...
  public String getSurveillanceStatusDescription() {
    String[] desc = { "No condition information", "Permanent alert (emergency condition)", "Temporary alert (change in Mode A identity code oter than emergency condition)", "SPI condition" };
  ...
  public double[] getGlobalPosition(AirbornePositionMsg other) throws Exception {
    assert tools.areEqual(other.getIcao24(), getIcao24()) : String.format("Transmitter of other message (%s) not equal to this (%s).", tools.toHexString(other.getIcao24()), tools.toHexString(this.getIcao24()));
    ...
    if (NL(Rlat0) != NL(Rlat1)) {
      throw new org.opensky.libadsb.exceptions.PositionsIncompatibleError("The two given position straddle a transition latitude " + "and cannot be decoded. Wait for positions where they are equal.");
    }
    double Dlon0 = 360.0 / Math.max(1.0, NL(Rlat0));
    double Dlon1 = 360.0 / Math.max(1.0, NL(Rlat1) - 1);
    double NL_helper = NL(isOddFormat() ? Rlat1 : Rlat0);
  ...
  public double[] getLocalPosition(double ref_lat, double ref_lon) throws Exception {
    ...
    double m = Math.floor(ref_lon / Dlon) + Math.floor(0.5 + (mod(ref_lon, Dlon)) / Dlon - (float) getCPREncodedLongitude() / ((double) (1 << 17)));
  ...
  public String toString() {
    return super.toString() + "\n" + "Position:\n" + "\tFormat:\t\t" + (isOddFormat() ? "odd" : "even") + "\tHas position:\t" + (hasPosition() ? "yes" : "no") + "\tHas altitude:\t" + (hasAltitude() ? "yes" : "no");
  }
}