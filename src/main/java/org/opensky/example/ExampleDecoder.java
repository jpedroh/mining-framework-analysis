  ...
  public void decodeMsg(double timestamp, String raw) throws Exception {
    ...
    if (tools.isZero(msg.getParity()) || msg.checkParity()) {
      ...
      switch (msg.getType()) {
        ...
        System.out.println("[" + icao24 + "]: Airspeed is " + (airspeed.hasAirspeedInfo() ? airspeed.getAirspeed() + " m/s" : "unkown"));
        if (airspeed.hasHeadingInfo()) {
          System.out.println("          Heading is " + (airspeed.hasHeadingInfo() ? airspeed.getHeading() + "\u00b0" : "unkown"));
        }
        if (airspeed.hasVerticalRateInfo()) {
          System.out.println("          Vertical rate is " + (airspeed.hasVerticalRateInfo() ? airspeed.getVerticalRate() + " m/s" : "unkown"));
        }
        break;
      ...
      if (msg.getDownlinkFormat() != 17) {
        switch (msg.getType()) {
          ...
          System.out.println("[" + icao24 + "]: Altitude is " + acas.getAltitude() + " and ACAS is " + (acas.hasOperatingACAS() ? "operating." : "not operating."));
          ...
          System.out.println("[" + icao24 + "]: Altitude is " + long_acas.getAltitude() + " and ACAS is " + (long_acas.hasOperatingACAS() ? "operating." : "not operating."));
