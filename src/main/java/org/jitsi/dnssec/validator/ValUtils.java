...
public class ValUtils {
  ...
  public static ResponseClassification classifyResponse(Message request, SMessage m) {
    ...
    if (m.getSectionRRsets(Section.AUTHORITY).size() == 0 && m.getSectionRRsets(Section.ANSWER).size() == 1 && m.getRcode() == Rcode.NOERROR && m.getSectionRRsets(Section.ANSWER).get(0).getType() == Type.NS && !m.getSectionRRsets(Section.ANSWER).get(0).getName().equals(request.getQuestion().getName())) {
      return ResponseClassification.REFERRAL;
    }
  ...
  public KeyEntry verifyNewDNSKEYs(SRRset dnskeyRrset, SRRset dsRrset, long badKeyTTL) {
    ...
    for (Iterator<?> i = dsRrset.rrs(); i.hasNext(); ) {
      ...
      DNSKEY:
      for (Iterator<?> j = dnskeyRrset.rrs(); j.hasNext(); ) {
  ...
  public static Name rrsetWildcard(RRset rrset) {
    @SuppressWarnings(value = { "unchecked" }) Iterator<RRSIGRecord> it = (Iterator<RRSIGRecord>) rrset.sigs();
