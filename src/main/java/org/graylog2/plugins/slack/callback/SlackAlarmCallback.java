...
public class SlackAlarmCallback extends SlackPluginBase implements AlarmCallback {
  ...
  @Override public void initialize(final Configuration config) throws AlarmCallbackConfigurationException {
  ...
  @Override public void call(Stream stream, AlertCondition.CheckResult result) throws AlarmCallbackException {
    final SlackClient client = new SlackClient(configuration);
    SlackMessage message = new SlackMessage(configuration.getString(CK_COLOR), configuration.getString(CK_ICON_EMOJI), configuration.getString(CK_ICON_URL), buildMessage(stream, result), configuration.getString(CK_USER_NAME), configuration.getString(CK_CHANNEL), configuration.getBoolean(CK_LINK_NAMES));
    ...
    if (!customFields.isEmpty()) {
      ...
      for (MessageSummary messageSummary : result.getMatchingMessages()) {
        String value = Arrays.stream(fields).map((field) -> Optional.ofNullable(messageSummary.getField(field)).map(Object::toString).orElse("???")).collect(Collectors.joining(DELIMITER));
  ...
  @Override public void checkConfiguration() throws ConfigurationException {
  }
