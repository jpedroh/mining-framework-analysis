...
public class GedcomParser {
  ...
  /**
     ...
     */
  private boolean g55() {
    return gedcom != null && gedcom.header != null && gedcom.header.gedcomVersion != null && SupportedVersion.V5_5.equals(gedcom.header.gedcomVersion.versionNumber);
  }
    ...
    for (StringTree ch : st.children) {
      if (Tag.PAGE.equals(ch.tag)) {
        ...
        if (Tag.EVENT.equals(ch.tag)) {
          ...
          if (Tag.DATA_FOR_CITATION.equals(ch.tag)) {
            CitationData d = new CitationData();
            cws.data.add(d);
            loadCitationData(ch, d);
          } else {
            if (Tag.QUALITY.equals(ch.tag)) {
              cws.certainty = new StringWithCustomTags(ch);
            } else {
              if (Tag.NOTE.equals(ch.tag)) {
                loadNote(ch, cws.notes);
              } else {
                if (Tag.OBJECT_MULTIMEDIA.equals(ch.tag)) {
  ...
  private void loadCorporation(StringTree st, Corporation corporation) {
    ...
    for (StringTree ch : st.children) {
      if (Tag.ADDRESS.equals(ch.tag)) {
        ...
        if (Tag.PHONE.equals(ch.tag)) {
          corporation.phoneNumbers.add(new StringWithCustomTags(ch));
        } else {
          if (Tag.WEB_ADDRESS.equals(ch.tag)) {
            corporation.wwwUrls.add(new StringWithCustomTags(ch));
            if (g55()) {
              warnings.add("GEDCOM version is 5.5 but WWW URL was specified for the corporation in the source system on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
            }
          } else {
            if (Tag.FAX.equals(ch.tag)) {
              corporation.faxNumbers.add(new StringWithCustomTags(ch));
              if (g55()) {
                warnings.add("GEDCOM version is 5.5 but fax number was specified for the corporation in the source system on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
              }
            } else {
              if (Tag.EMAIL.equals(ch.tag)) {
                corporation.emails.add(new StringWithCustomTags(ch));
                if (g55()) {
                  warnings.add("GEDCOM version is 5.5 but emails was specified for the corporation in the source system on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                }
              } else {
  ...
  private void loadFamily(StringTree st) {
    ...
    for (StringTree ch : st.children) {
      if (Tag.HUSBAND.equals(ch.tag)) {
        ...
        if (Tag.WIFE.equals(ch.tag)) {
          f.wife = getIndividual(ch.value);
        } else {
          if (Tag.CHILD.equals(ch.tag)) {
            f.children.add(getIndividual(ch.value));
          } else {
            if (Tag.NUM_CHILDREN.equals(ch.tag)) {
              ...
              if (Tag.SOURCE.equals(ch.tag)) {
                ...
                if (Tag.OBJECT_MULTIMEDIA.equals(ch.tag)) {
                  ...
                  if (Tag.RECORD_ID_NUMBER.equals(ch.tag)) {
                    f.automatedRecordId = new StringWithCustomTags(ch);
                  } else {
                    if (Tag.CHANGED_DATETIME.equals(ch.tag)) {
                      ...
                      if (Tag.NOTE.equals(ch.tag)) {
                        ...
                        if (Tag.RESTRICTION.equals(ch.tag)) {
              ...
              if (Tag.NOTE.equals(ch.tag)) {
                ...
                if (Tag.SOURCE.equals(ch.tag)) {
                  ...
                  if (Tag.RESTRICTION.equals(ch.tag)) {
                    ...
                    if (g55()) {
                      warnings.add("GEDCOM version is 5.5 but restriction notice was specified for family event on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                    }
                  } else {
                    if (Tag.RELIGION.equals(ch.tag)) {
                      e.religiousAffiliation = new StringWithCustomTags(ch);
                      if (g55()) {
                        warnings.add("GEDCOM version is 5.5 but religious affiliation was specified for family event on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                      }
                    } else {
                      if (Tag.AGE.equals(ch.tag)) {
                        e.age = new StringWithCustomTags(ch);
                      } else {
                        if (Tag.CAUSE.equals(ch.tag)) {
                            ...
                            if (Tag.AGENCY.equals(ch.tag)) {
                              ...
                              if (Tag.PHONE.equals(ch.tag)) {
                                ...
                                if (Tag.WEB_ADDRESS.equals(ch.tag)) {
                                  ...
                                  if (g55()) {
                                    warnings.add("GEDCOM version is 5.5 but WWW URL was specified for " + e.type + " family event on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                                  }
                                } else {
                                  if (Tag.FAX.equals(ch.tag)) {
                                    e.faxNumbers.add(new StringWithCustomTags(ch));
                                    if (g55()) {
                                      warnings.add("GEDCOM version is 5.5 but fax number was specified for " + e.type + " family event on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                                    }
                                  } else {
                                    if (Tag.EMAIL.equals(ch.tag)) {
                                      e.emails.add(new StringWithCustomTags(ch));
                                      if (g55()) {
                                        warnings.add("GEDCOM version is 5.5 but email was specified for " + e.type + " family event on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                                      }
                                    } else {
                                      if (Tag.HUSBAND.equals(ch.tag)) {
  ...
  private void loadFamilyWhereChild(StringTree st, List<FamilyChild> familiesWhereChild) {
    ...
    for (StringTree ch : st.children) {
      if (Tag.NOTE.equals(ch.tag)) {
        ...
        if (Tag.PEDIGREE.equals(ch.tag)) {
          fc.pedigree = new StringWithCustomTags(ch);
        } else {
          if (Tag.ADOPTION.equals(ch.tag)) {
            fc.adoptedBy = AdoptedByWhichParent.valueOf(ch.value);
          } else {
            if (Tag.STATUS.equals(ch.tag)) {
              fc.status = new StringWithCustomTags(ch);
              if (g55()) {
                warnings.add("GEDCOM version is 5.5 but status was specified for child-to-family link on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
              }
            } else {
              unknownTag(ch, fc);
            }
  ...
  private void loadFileReferences(Multimedia m, StringTree st) {
    ...
    if (g55()) {
      if (fileTagCount > 1) {
        warnings.add("GEDCOM version is 5.5, but multiple files referenced in multimedia reference on line " + st.lineNum + ", which is only allowed in 5.5.1. " + "Data will be loaded, but cannot be written back out unless the GEDCOM version is changed to 5.5.1");
      }
      if (formTagCount == 0) {
        warnings.add("GEDCOM version is 5.5, but there is not a FORM tag in the multimedia link on line " + st.lineNum + ", a scenario which is only allowed in 5.5.1. " + "Data will be loaded, but cannot be written back out unless the GEDCOM version is changed to 5.5.1");
      }
    }
    if (formTagCount > 1) {
      errors.add("Multiple FORM tags were found for a multimedia file reference at line " + st.lineNum + " - this is not compliant with any GEDCOM standard - data not loaded");
      return;
    }
      ...
      if (Tag.FILE.equals(ch.tag)) {
        ...
        if (Tag.TITLE.equals(ch.tag)) {
          ...
          if (Tag.NOTE.equals(ch.tag)) {
            loadNote(ch, m.notes);
            if (!g55()) {
              warnings.add("Gedcom version was 5.5.1, but a NOTE was found on a multimedia link on line " + ch.lineNum + ", which is no longer supported. " + "Data will be loaded, but cannot be written back out unless the GEDCOM version is changed to 5.5");
            }
          } else {
            unknownTag(ch, m);
          }
  ...
  private void loadHeader(StringTree st) {
    ...
    for (StringTree ch : st.children) {
      if (Tag.SOURCE.equals(ch.tag)) {
        ...
        if (Tag.DESTINATION.equals(ch.tag)) {
          ...
          if (Tag.DATE.equals(ch.tag)) {
            ...
            if (Tag.CHARACTER_SET.equals(ch.tag)) {
              ...
              if (Tag.SUBMITTER.equals(ch.tag)) {
                ...
                if (Tag.FILE.equals(ch.tag)) {
                  ...
                  if (Tag.GEDCOM_VERSION.equals(ch.tag)) {
                    header.gedcomVersion = new GedcomVersion();
                    loadGedcomVersion(ch, header.gedcomVersion);
                  } else {
                    if (Tag.COPYRIGHT.equals(ch.tag)) {
                      loadMultiLinesOfText(ch, header.copyrightData, header);
                      if (g55() && header.copyrightData.size() > 1) {
                        warnings.add("GEDCOM version is 5.5, but multiple lines of copyright data were specified, which is only allowed in GEDCOM 5.5.1. " + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                      }
                    } else {
                      if (Tag.SUBMISSION.equals(ch.tag)) {
                        if (header.submission == null) {
                          header.submission = gedcom.submission;
                        }
                      } else {
                        if (Tag.LANGUAGE.equals(ch.tag)) {
                                  ...
                                  if (Tag.OBJECT_MULTIMEDIA.equals(ch.tag)) {
                                    ...
                                    if (Tag.RESTRICTION.equals(ch.tag)) {
                                      ...
                                      if (Tag.SOURCE.equals(ch.tag)) {
                                        ...
                                        if (Tag.ALIAS.equals(ch.tag)) {
                                          i.aliases.add(new StringWithCustomTags(ch));
                                        } else {
                                          if (Tag.FAMILY_WHERE_SPOUSE.equals(ch.tag)) {
                                            loadFamilyWhereSpouse(ch, i.familiesWhereSpouse);
                                          } else {
                                            if (Tag.FAMILY_WHERE_CHILD.equals(ch.tag)) {
                                              loadFamilyWhereChild(ch, i.familiesWhereChild);
                                            } else {
                                              if (Tag.ASSOCIATION.equals(ch.tag)) {
                                                loadAssociation(ch, i.associations);
                                              } else {
                                                if (Tag.ANCESTOR_INTEREST.equals(ch.tag)) {
                                                  i.ancestorInterest.add(getSubmitter(ch.value));
                                                } else {
                                                  if (Tag.DESCENDANT_INTEREST.equals(ch.tag)) {
                                                    i.descendantInterest.add(getSubmitter(ch.value));
                                                  } else {
                                                    if (Tag.ANCESTRAL_FILE_NUMBER.equals(ch.tag)) {
                                ...
                                if (Tag.WEB_ADDRESS.equals(ch.tag)) {
                                  ...
                                  if (Tag.FAX.equals(ch.tag)) {
                                    ...
                                    if (Tag.EMAIL.equals(ch.tag)) {
                                      e.emails.add(new StringWithCustomTags(ch));
                                      if (g55()) {
                                        warnings.add("GEDCOM version is 5.5 but email was specified on " + e.type + " event on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                                      }
                                    } else {
                                      if (Tag.CONCATENATION.equals(ch.tag)) {
                                        if (e.description == null) {
  ...
  private void loadMultimediaRecord(StringTree st) {
    ...
    if (fileTagCount > 0) {
      if (g55()) {
        warnings.add("GEDCOM version was 5.5, but a 5.5.1-style multimedia record was found at line " + st.lineNum + ". " + "Data will be loaded, but might have problems being written until the version is for the data is changed to 5.5.1");
      }
      loadMultimediaRecord551(st);
    } else {
      if (!g55()) {
        warnings.add("GEDCOM version is 5.5.1, but a 5.5-style multimedia record was found at line " + st.lineNum + ". " + "Data will be loaded, but might have problems being written until the version is for the data is changed to 5.5.1");
      }
        ...
        if (Tag.TITLE.equals(ch.tag)) {
          ...
          if (Tag.NOTE.equals(ch.tag)) {
            ...
            if (Tag.SOURCE.equals(ch.tag)) {
              ...
              if (Tag.BLOB.equals(ch.tag)) {
                loadMultiLinesOfText(ch, m.blob, m);
                if (!g55()) {
                  warnings.add("GEDCOM version is 5.5.1, but a BLOB tag was found at line " + ch.lineNum + ". " + "Data will be loaded but will not be writeable unless GEDCOM version is changed to 5.5.1");
                }
              } else {
                if (Tag.OBJECT_MULTIMEDIA.equals(ch.tag)) {
                  ...
                  if (!g55()) {
                    warnings.add("GEDCOM version is 5.5.1, but a chained OBJE tag was found at line " + ch.lineNum + ". " + "Data will be loaded but will not be writeable unless GEDCOM version is changed to 5.5.1");
                  }
                } else {
                  if (Tag.REFERENCE.equals(ch.tag)) {
                    UserReference u = new UserReference();
          ...
          if (Tag.NOTE.equals(ch.tag)) {
            ...
            if (Tag.CONCATENATION.equals(ch.tag)) {
              ...
              if (Tag.CONTINUATION.equals(ch.tag)) {
                ...
                if (Tag.ROMANIZED.equals(ch.tag)) {
                  if (g55()) {
                    warnings.add("GEDCOM version is 5.5 but a romanized variation was specified on a place on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                  }
                  ...
                  if (Tag.PHONETIC.equals(ch.tag)) {
                    if (g55()) {
                      warnings.add("GEDCOM version is 5.5 but a phonetic variation was specified on a place on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                    }
                    NameVariation nv = new NameVariation();
                    ...
                    if (Tag.MAP.equals(ch.tag)) {
                      if (g55()) {
                        warnings.add("GEDCOM version is 5.5 but a map coordinate was specified on a place on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                      }
                      for (StringTree gch : ch.children) {
                        if (Tag.LATITUDE.equals(gch.tag)) {
                          place.latitude = new StringWithCustomTags(gch);
                        } else {
                          if (Tag.LONGITUDE.equals(gch.tag)) {
        ...
        if (Tag.ADDRESS.equals(ch.tag)) {
          ...
          if (Tag.PHONE.equals(ch.tag)) {
            ...
            if (Tag.WEB_ADDRESS.equals(ch.tag)) {
              ...
              if (g55()) {
                warnings.add("GEDCOM version is 5.5 but WWW URL was specified on repository " + r.xref + " on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
              }
            } else {
              if (Tag.FAX.equals(ch.tag)) {
                r.faxNumbers.add(new StringWithCustomTags(ch));
                if (g55()) {
                  warnings.add("GEDCOM version is 5.5 but fax was specified on repository " + r.xref + " on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                }
              } else {
                if (Tag.EMAIL.equals(ch.tag)) {
                  r.emails.add(new StringWithCustomTags(ch));
                  if (g55()) {
                    warnings.add("GEDCOM version is 5.5 but email was specified on repository " + r.xref + " on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                  }
                } else {
                  if (Tag.NOTE.equals(ch.tag)) {
                    loadNote(ch, r.notes);
  ...
  private RepositoryCitation loadRepositoryCitation(StringTree st) {
    ...
    for (StringTree ch : st.children) {
      if (Tag.NOTE.equals(ch.tag)) {
        ...
        if (Tag.CALL_NUMBER.equals(ch.tag)) {
    ...
    for (StringTree ch : st.children) {
      if (Tag.HEADER.equals(ch.tag)) {
        ...
        if (Tag.SUBMITTER.equals(ch.tag)) {
          ...
          if (Tag.INDIVIDUAL.equals(ch.tag)) {
            loadIndividual(ch);
          } else {
            if (Tag.SUBMISSION.equals(ch.tag)) {
              loadSubmission(ch);
              ...
              if (Tag.NOTE.equals(ch.tag)) {
                ...
                if (Tag.FAMILY.equals(ch.tag)) {
                  loadFamily(ch);
                } else {
                  if (Tag.TRAILER.equals(ch.tag)) {
                    gedcom.trailer = new Trailer();
                  } else {
                    if (Tag.SOURCE.equals(ch.tag)) {
                      loadSource(ch);
                    } else {
                      if (Tag.REPOSITORY.equals(ch.tag)) {
                        loadRepository(ch);
                      } else {
                        if (Tag.OBJECT_MULTIMEDIA.equals(ch.tag)) {
    ...
    for (StringTree ch : st.children) {
      if (Tag.VERSION.equals(ch.tag)) {
        ...
        if (Tag.NAME.equals(ch.tag)) {
          ...
          if (Tag.CORPORATION.equals(ch.tag)) {
        ...
        if (Tag.ADDRESS.equals(ch.tag)) {
          ...
          if (Tag.PHONE.equals(ch.tag)) {
            ...
            if (Tag.WEB_ADDRESS.equals(ch.tag)) {
              ...
              if (g55()) {
                warnings.add("GEDCOM version is 5.5 but WWW URL number was specified on submitter on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
              }
            } else {
              if (Tag.FAX.equals(ch.tag)) {
                submitter.faxNumbers.add(new StringWithCustomTags(ch));
                if (g55()) {
                  warnings.add("GEDCOM version is 5.5 but fax number was specified on submitter on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                }
              } else {
                if (Tag.EMAIL.equals(ch.tag)) {
                  submitter.emails.add(new StringWithCustomTags(ch));
                  if (g55()) {
                    warnings.add("GEDCOM version is 5.5 but email was specified on submitter on line " + ch.lineNum + ", which is a GEDCOM 5.5.1 feature." + "  Data loaded but cannot be re-written unless GEDCOM version changes.");
                  }
                } else {
                  if (Tag.LANGUAGE.equals(ch.tag)) {
