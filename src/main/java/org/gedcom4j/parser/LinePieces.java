...
class LinePieces {
  ...
  private void processLevel() throws GedcomParserException {
    try {
      ...
      throw new GedcomParserException("Line " + lineNum + " does not begin with a 1 or 2 digit number for the level followed by a space: " + new String(chars), e);
    } catch (IndexOutOfBoundsException e) {
      throw new GedcomParserException("Line " + lineNum + " does not begin with a 1 or 2 digit number for the level followed by a space: " + new String(chars), e);
    }
    if (level < 0 || level > 99) {
      throw new GedcomParserException("Line " + lineNum + " does not begin with a 1 or 2 digit number for the level followed by a space: " + new String(chars));
    }
  }
  ...
  private void processTag() throws GedcomParserException {
    ...
    if (t.length() > 0) {
      ...
      throw new GedcomParserException("All GEDCOM lines are required to have a tag value, but no tag could be found on line " + lineNum);
    }
  }
