...
public class EncFSCrypto {
  ...
  private static void cipherInit(Key key, Mac mac, int opMode, Cipher cipher, byte[] iv, byte[] ivSeed) throws InvalidAlgorithmParameterException {
    try {
  ...
  public static void cipherInit(EncFSVolume volume, int opMode, Cipher cipher, byte[] ivSeed) throws InvalidAlgorithmParameterException {
  ...
  public static byte[] streamDecode(Cipher cipher, Mac mac, Key key, byte[] iv, byte[] ivSeed, byte[] data) throws EncFSUnsupportedException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
  ...
  public static byte[] streamDecode(EncFSVolume volume, byte[] ivSeed, byte[] data) throws EncFSUnsupportedException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
    return streamDecode(volume.getStreamCipher(), volume.getMac(), volume.getKey(), volume.getIV(), ivSeed, data);
  }

  public static byte[] streamEncode(EncFSVolume volume, byte[] ivSeed, byte[] data) throws EncFSUnsupportedException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
    return streamEncode(volume.getStreamCipher(), volume.getMac(), volume.getKey(), volume.getIV(), ivSeed, data);
  }

  public static byte[] streamEncode(Cipher cipher, Mac mac, Key key, byte[] iv, byte[] ivSeed, byte[] data) throws EncFSUnsupportedException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
    byte[] ivSeedPlusOne;
    if (ivSeed.length == 4) {
      ivSeedPlusOne = EncFSUtil.intToByteArray(EncFSUtil.byteArrayToInt(ivSeed) + 1);
    } else {
  ...
  public static byte[] derivePasswordKey(EncFSConfig config, String password) throws EncFSInvalidConfigException, EncFSUnsupportedException {
    ...
    KeySpec ks = new PBEKeySpec(password.toCharArray(), cipherSaltData, config.getIterationCount(), config.getVolumeKeySize() + EncFSVolume.ENCFS_VOLUME_IV_LENGTH * 8);
  ...
  public static byte[] decryptVolumeKey(EncFSConfig config, byte[] pbkdf2Data) throws EncFSChecksumException, EncFSInvalidConfigException, EncFSCorruptDataException, EncFSUnsupportedException {
    ...
    byte[] passIvData = Arrays.copyOfRange(pbkdf2Data, keySizeInBytes, keySizeInBytes + EncFSVolume.ENCFS_VOLUME_IV_LENGTH);
  ...
  public static String encodeName(EncFSVolume volume, String fileName, String volumePath) throws EncFSCorruptDataException, EncFSChecksumException {
    Cipher cipher;
