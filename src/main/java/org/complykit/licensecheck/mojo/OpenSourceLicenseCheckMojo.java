 ...
 * @author michael.rice
 */
@Mojo(name = "os-check", requiresDependencyResolution = ResolutionScope.COMPILE_PLUS_RUNTIME) public class OpenSourceLicenseCheckMojo extends AbstractMojo {
  ...
  /**
     ...
     * For more, visit http://blog.sonatype.com/people/2011/01/how-to-use-aether-in-maven-plugins/
     */
  @Parameter(defaultValue = "${repositorySystemSession}", readonly = true) private RepositorySystemSession repoSession;

  /**
     * This is the project's remote repositories that can be used for resolving plugins and their dependencies
     */
  @Parameter(defaultValue = "${project.remoteProjectRepositories}", readonly = true) private List<RemoteRepository> remoteRepos;

  /**
     * This is the maximum number of parents to search through (in case there's a malformed pom).
     */
  @Parameter(property = "os-check.recursion-limit", defaultValue = "12") private int maxSearchDepth;
  ...
  /**
     ...
     */
  @Parameter(property = "os-check.excludes") private String[] excludes;
  ...
  /**
     ...
     */
  @Parameter(property = "os-check.excludesRegex") private String[] excludesRegex;

  @Parameter(property = "os-check.excludesNoLicense") private boolean excludeNoLicense;
  ...
  /**
     ...
     */
  @Parameter(property = "os-check.blacklist") private String[] blacklist;
  ...
  /**
     ...
     * &lt;param&gt;gpl-3.0&lt;/param&gt; &lt;/blacklist&gt; &lt;/configuration&gt;
     */
  @Parameter(property = "os-check.whitelist") private String[] whitelist;
  ...
  /**
     ...
     */
  @Parameter(property = "os-check.excludedScopes") private String[] excludedScopes;
  ...
  @Override public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    final Map<Artifact, String> licenses = new 
<<<<<<< /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/left.java
    HashMap<>
=======
    HashMap<Artifact, String>
>>>>>>> /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/right.java
    ();
    ...
    for (final Artifact artifact : artifacts) {
      if (!artifactIsOnExcludeList(excludeSet, excludePatternSet, excludedScopesSet, artifact)) {
        ...
        if (code == null || code.equals("")) {
          ...
          if (!blacklistSet.isEmpty() && isContained(blacklistSet, code)) {
            buildFails = true;
            code += " IS ON YOUR BLACKLIST";
          } else {
            if (!whitelistSet.isEmpty() && !isContained(whitelistSet, code)) {
              buildFails = true;
...
>>>>>>> /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/right.java
  ...
  @NotNull Set<String> getAsLowerCaseSetWithoutNull(@Nullable final String[] src) {
    return Arrays.stream(src != null ? src : new String[0]).filter(Objects::nonNull).map(OpenSourceLicenseCheckMojo::lowerCaseString).collect(Collectors.toSet());
  }

  @Nullable private Pattern compilePattern(@NotNull String pattern) {
    try {
  ...
  @NotNull private Set<Pattern> getAsPatternSet(@Nullable final String[] src) {
    return Arrays.stream(src != null ? src : new String[0]).unordered().filter(Objects::nonNull).distinct().map(this::compilePattern).filter(Objects::nonNull).collect(Collectors.toSet());
  }

  @NotNull private String toCoordinates(@NotNull final Artifact artifact) {
    return String.format("%s:%s:%s", artifact.getGroupId(), artifact.getArtifactId(), artifact.getVersion());
  }

  @Nullable private String recurseForLicenseName(@NotNull final Artifact artifact, final int currentDepth) throws IOException, MojoFailureException, XmlPullParserException {
...
>>>>>>> /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/right.java
    ...
    final Model 
<<<<<<< /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/left.java
    pom = readPomContents(artifact)
=======
    model = readPomContents(pomFile)
>>>>>>> /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/right.java
    ;
    if (model == null) {
      return null;
      ...
      if (parentArtifactCoords != null) {
        ...
        if (parent != null) {
          if (currentDepth >= maxSearchDepth) {
            throw new MaximumRecursionDepthReachedException(String.format("The maximum recursion depth of %d has been reached when handling transitive dependencies. Fix your project or extend it.", maxSearchDepth));
          }
  ...
  @NotNull private Model readPomContents(
<<<<<<< /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/left.java
  @NotNull final Artifact artifact
=======
  final File pomFile
>>>>>>> /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/right.java
  ) throws IOException, MojoFailureException, XmlPullParserException {
    if (!pomFile.exists()) {
      return null;
    }
    ...
    try {
      ...
      if (!pathFile.exists()) {
        ...
        if (artifact.getFile().exists()) {
          ...
          if (artifact.getFile().toString().toLowerCase().endsWith("pom.xml")) {
            getLog().debug(String.format("File %s is a pom, using that!", artifact.getFile()));
            reader = newReader(artifact.getFile(), UTF_8);
          } else {
            if (artifact.getFile().toString().toLowerCase().endsWith(".jar")) {
              getLog().debug(String.format("File %s is a jar, trying to find pom inside!", artifact.getFile()));
              jarFs = FileSystems.newFileSystem(artifact.getFile().toPath(), null);
              Path jarPom = jarFs.getPath(String.format("META-INF/maven/%s/%s/pom.xml", artifact.getGroupId(), artifact.getArtifactId()));
...
>>>>>>> /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/right.java
  ...
  /**
     ...
     * @return
     */
  @Nullable private String extractLicenseName(
<<<<<<< /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/left.java
...
<<<<<<< /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/left.java
  /**
     ...
     * @return
     */
  @Nullable private String extractParentCoords(@NotNull final String raw) {
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  /**
     ...
     */
  @Nullable private Artifact retrieveArtifact(final Parent parent) {
    final ArtifactRequest request = new ArtifactRequest();
...
<<<<<<< /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/left.java
      try {
        ...
        request.setArtifact(new DefaultArtifact(art.getGroupId(), art.getArtifactId(), "pom", art.getVersion()));
        result = repoSystem.resolveArtifact(repoSession, request);
      } catch (@NotNull final ArtifactResolutionException ex) {
        getLog().error(String.format("Could not resolve parent artifact (%s): ", coordinates), ex);
      }
=======
...
>>>>>>> /usr/src/app/output/mrice/license-check/df4be32c0a82130293069c4cb3c69fa9f176241d/src/main/java/org/complykit/licensecheck/mojo/OpenSourceLicenseCheckMojo.java/right.java
  ...
  @Nullable String convertLicenseNameToCode(@Nullable final String licenseName) {
    ...
    return descriptors.stream().sequential().filter((licenseDescriptor) -> licenseDescriptor.getMatcher(licenseName).find()).findFirst().map(LicenseDescriptor::getCode).orElse(null);
  }
  ...
  private void loadDescriptors() {
    ...
    for (final String line : lines) {
      ...
      final LicenseDescriptor descriptor = new LicenseDescriptor(columns[LICENSE_CODE_COLUMN], columns[LICENSE_NAME_COLUMN], columns[LICENSE_REGEX_COLUMN]);
  ...
  /**
     ...
     * @return
     */
  private boolean artifactIsOnExcludeList(final Set<String> excludeSet, @NotNull final Set<Pattern> patternSet, @NotNull final Set<String> excludedScopes, @NotNull final Artifact artifact) {
    return excludedScopes.contains(artifact.getScope()) || isExcludedTemplate(excludeSet, patternSet, toCoordinates(artifact));
  }

  private boolean isExcludedTemplate(final Set<String> excludeSet, @NotNull final Set<Pattern> patternSet, @NotNull final String template) {
