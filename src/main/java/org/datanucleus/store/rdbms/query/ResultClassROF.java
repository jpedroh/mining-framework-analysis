      ...
      if (resultFieldValues.length == 1 && resultFieldValues[0] != null && resultClass.isAssignableFrom(resultFieldValues[0].getClass())) {
        ...
        if (constructionDefined) {
          if (resultClassArgConstructor != null) {
            ...
            if (resultClassDefaultConstructor != null) {
              Object obj = null;
              try {
        ...
        if (!constructionDefined) {
          ...
          if (ctr != null) {
            ...
            for (int i = 0; i < resultFieldValues.length; i++) {
              ...
              if (resultClassMemberSetters[i] == null) {
                String setMethodName = (field != null) ? "set" + fieldName.substring(0, 1).toUpperCase() + field.getName().substring(1) : "set" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1);
    ...
    if (newMap.getNumberOfConstructorArgMappings() == 0) {
      ...
      for (int i = 0; i < numArgs; i++) {
        ...
        if (obj instanceof StatementMappingIndex) {
          ...
          if (obj instanceof StatementNewObjectMapping) {
      ...
      if (!field.isSynthetic() && !field.getName().startsWith(ec.getMetaDataManager().getEnhancedMethodNamePrefix())) {
        ...
        if (currValue != null) {
          if (currValue.getName().equals(field.getName())) {
            NucleusLogger.QUERY.info("Result column=" + field.getName().toUpperCase() + " is already mapped to \"" + currValue.toString() + "\"" + " but result class also has \"" + field.toString() + "\"; this latter field will not be set.");
          } else {
