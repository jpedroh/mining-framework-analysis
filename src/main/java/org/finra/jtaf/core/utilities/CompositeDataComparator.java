...
public class CompositeDataComparator {
  ...
  public void compareMap(String title, String element, Map<?, ?> expComp, Map<?, ?> actualComp) {
    ...
    for (Iterator<?> iter = keySet.iterator(); iter.hasNext(); ) {
      ...
      if (!actualComp.containsKey(obj)) {
        reportError(prefix + " Couldn\'t find Element [" + obj + "] in Actual Data");
      }
  ...
  public void compareList(String title, String element, List<?> expList, List<?> actualList) {
    String prefix = getPrefix(title, element);
  ...
  private void assessAndCompare(String title, String element, Object expected, Object actual) {
    String prefix = getPrefix(title, element);
    if (expected == null && actual == null) {
      ...
      if (expected == null || actual == null) {
        ...
        reportError(prefix + " didn\'t match. Expected Value[" + expected + "] and Actual Value [" + actual + "].");
      } else {
          ...
          if (expected instanceof List) {
            ...
            if (expected instanceof Timestamp) {
              ...
              if (!(actual instanceof Timestamp)) {
                reportError(prefix + " Expected value [" + expTimestamp + "] and Actual Value [" + actual + "].");
              } else {
              ...
              if (expected instanceof String[]) {
                ...
                if (e.length != a.length) {
                  reportError(prefix + " Length of string array didn\'t match. Expected [" + e.length + "] and Actual [" + a.length + "].");
                }
                ...
                if (expected instanceof String) {
                  ...
                  if (expected instanceof Boolean && actual instanceof String) {
                    try {
                      ...
                      if ("y".equalsIgnoreCase((String) actual) || "true".equalsIgnoreCase((String) actual)) {
                    ...
                    if (expected instanceof Integer && actual instanceof BigDecimal) {
  ...
  private boolean compareSize(String comp, List<?> expSessions, List<?> actualSessions) {
    ...
    if (expSessions.size() != actualSessions.size()) {
      reportError(comp + "didn\'t match; Expected List size: " + expSessions.size() + " -- Actual List size: " + actualSessions.size());
      equal = false;
    }
    return equal;
  }
