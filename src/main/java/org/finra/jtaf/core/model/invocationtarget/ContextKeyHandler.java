...
public class ContextKeyHandler {
  ...
  /**
     ...
     * @return -
     */
  @SuppressWarnings(value = { "unchecked" }) public Object replaceContextKey(Object value) {
    resolveCount++;
        ...
        if (beginIndex >= 0) {
          ...
          if (endIndex > 0) {
            ...
            if (getContext().getObject(runTimeKeyName) != null) {
              resultString.append(getContext().getObject(runTimeKeyName) + resultSplitted[i].substring(endIndex + 1));
            } else {
              if (InvocationTarget.getGlobalContext().get(runTimeKeyName) != null) {
                resultString.append(InvocationTarget.getGlobalContext().get(runTimeKeyName) + resultSplitted[i].substring(endIndex + 1));
              } else {
                throw new ContextKeyNotFoundException("Oops! Processing \'" + value + "\' key fails! Can\'t find run time parameter \'" + runTimeKeyName + "\'! You have put it to global content before...");
              }
            }
          }
        }
