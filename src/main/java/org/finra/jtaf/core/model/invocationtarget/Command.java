...
public abstract class Command extends InvocationTarget {
  ...
  private Object processRandomGenerator(String params) {
    ...
    for (String paramsSplittedCurrent : paramsSplitted) {
      ...
      if (paramsCurrent.length == 2) {
        if (paramsCurrent[0].equalsIgnoreCase("method")) {
          ...
          if (paramsCurrent[0].equalsIgnoreCase("length")) {
            length = paramsCurrent[1].replace("\'", "");
          } else {
            if (paramsCurrent[0].equalsIgnoreCase("lengthMin")) {
              lengthMin = paramsCurrent[1].replace("\'", "");
            } else {
              if (paramsCurrent[0].equalsIgnoreCase("lengthMax")) {
                lengthMax = paramsCurrent[1].replace("\'", "");
              } else {
                if (paramsCurrent[0].equalsIgnoreCase("min")) {
                  min = paramsCurrent[1].replace("\'", "");
                } else {
                  if (paramsCurrent[0].equalsIgnoreCase("max")) {
                    max = paramsCurrent[1].replace("\'", "");
                  } else {
                    if (paramsCurrent[0].equalsIgnoreCase("regexp")) {
                      regexp = paramsCurrent[1].replace("\'", "");
                    } else {
                      if (paramsCurrent[0].equalsIgnoreCase("saveToGlobalContextAs")) {
                        saveToGlobalContextAs = paramsCurrent[1].replace("\'", "");
                      } else {
                        logger.fatal("Oops! Can\'t parse \'$randomGenerator\' command parameters. Unknown parameter \'" + paramsCurrent[0] + "\'! This command supports these parameters: String method, String length, String lengthMin, String lengthMax, String min, String max, String regexp, String saveToGlobalContextAs. Example: $randomGenerator(method=\'string\' length=\'10\'). Fix your testscript, please!");
                      }
                    }
                  }
                }
    ...
    String result = RandomGenerator.generate(method, length, lengthMin, lengthMax, min, max, regexp);
  ...
  protected Object getRequiredObject(String key) {
    ...
    if (param == null) {
      throw new IllegalArgumentException(getName() + " : The parameter with key [" + key + "] is not set.");
    }
    return param;
  }
  ...
  protected final int getRequiredInteger(String attributeName) {
    ...
    if (value == null) {
      throw new NullPointerException(this.getName() + ": missing required integer attribute \'" + attributeName + "\'");
    }
  ...
  protected final float getRequiredFloat(String attributeName) {
    String value = (String) this.getOptionalObject(attributeName);
    if (value == null) {
      throw new NullPointerException(this.getName() + ": missing required float attribute \'" + attributeName + "\'");
    }
  ...
  protected final String getRequiredString(String attributeName) {
    String value = (String) this.getOptionalObject(attributeName);
    if (value == null) {
      throw new NullPointerException(this.getName() + ": missing required String attribute \'" + attributeName + "\'");
    }
    return value;
  }
  ...
  protected final boolean getRequiredBoolean(String attributeName) {
    ...
    if (temp == null) {
      throw new NullPointerException(this.getName() + ": missing required boolean attribute \'" + attributeName + "\'");
    }
    temp = temp.trim();
    if (temp.equalsIgnoreCase("true")) {
      ...
      if (temp.equalsIgnoreCase("false")) {
        ...
        throw new IllegalArgumentException(this.getName() + ": attribute \'" + attributeName + "\' must be either \'true\' or \'false\'");
      }
    }
  }
  ...
  protected final boolean getBooleanOrDefault(String attributeName, boolean defaultValue) {
    ...
    if (temp.equalsIgnoreCase("true")) {
      ...
      if (temp.equalsIgnoreCase("false")) {
        ...
        throw new IllegalArgumentException(this.getName() + ": attribute \'" + attributeName + "\' must be either \'true\' or \'false\' (or left unspecified)");
      }
    }
  }
