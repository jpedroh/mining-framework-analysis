...
public class Interpreter {
  ...
  /**
	 ...
	 * 
	 * @param testRunnerPlugins
	 *            => the list of test level plugins
	 */
  ...
  /**
	 ...
	 * 
	 * @param commandRunnerPlugins
	 *            => the list of command level plugins
	 */
  ...
  public final void visitInvocation(Invocation invocation) throws Throwable {
    ...
    if (this.commandRegistry.containsInvocationTarget(invocation.getTargetName())) {
      final InvocationTarget target = this.commandRegistry.getInvocationTarget(invocation.getTargetName());
  ...
  public InvocationTarget getInvocationTarget(Invocation invocation) throws Throwable {
    ...
    InvocationTarget mappedInvocationTarget = this.commandRegistry.getInvocationTarget(invocation.getTargetName());
    if (mappedInvocationTarget == null) {
      throw new IllegalArgumentException("Invocation " + invocation.getTargetName() + " not found");
    }
    if (mappedInvocationTarget instanceof Function) {
      target = mappedInvocationTarget;
    } else {
      Class<?> targetClass = automationClassLoader.loadClass(mappedInvocationTarget.getClass().getName());
      if (InvocationTarget.class.isAssignableFrom(targetClass)) {
        Constructor<?> constructor = targetClass.getConstructor(String.class);
        target = (InvocationTarget) constructor.newInstance(invocation.getTargetName());
  ...
  private void executeTestFinishPlugins() {
    if (testRunnerPlugins != null) {
      for (ITestRunnerPlugin testPlugin : testRunnerPlugins) {
        try {
          testPlugin.handleTestFinish(new TestRunnerPluginContext(context.getTestScript(), new TestResult(this.getTestStepDetails(), testStatus), context));
        } catch (Throwable e) {
          logger.error(e);
        }
      }
  ...
  /**
	 ...
	 * 
	 * @param test
	 *            => the current testscript in context
	 */
  private void executeTestStartPlugins(TestScript test) {
    try {
      if (testRunnerPlugins != null) {
        for (ITestRunnerPlugin testPlugin : testRunnerPlugins) {
          testPlugin.handleTestStart(new TestRunnerPluginContext(test, new TestResult(this.getTestStepDetails(), testStatus), context));
        }
      }
    } catch (Throwable th) {
      logger.error(th);
    ...
    try {
      if (commandRunnerPlugins != null) {
        for (ICommandRunnerPlugin commandPlugin : commandRunnerPlugins) {
          commandPlugin.handleCommandAfter(new CommandRunnerPluginContext(new TestStepsDetails(command.getName(), command.getUsage()), context));
        }
      }
    } catch (Throwable th) {
      logger.error(th);
  ...
  private void executeCommandStartPlugins(Command command) {
    try {
      if (commandRunnerPlugins != null) {
        for (ICommandRunnerPlugin commandPlugin : commandRunnerPlugins) {
          commandPlugin.handleCommandBefore(new CommandRunnerPluginContext(new TestStepsDetails(command.getName(), command.getUsage()), context));
        }
      }
    } catch (Throwable th) {
      logger.error(th);
