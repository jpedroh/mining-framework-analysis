...
<<<<<<< /usr/src/app/output/doctau/garbagecat/4af1d9c4fc71246add443b99af9cc47e5464aa89/src/main/java/org/eclipselabs/garbagecat/service/GcManager.java/left.java
      entangledLogLines = new ArrayList<String>()
=======
      savedLogLines = new ArrayList<String>()
...
>>>>>>> /usr/src/app/output/doctau/garbagecat/4af1d9c4fc71246add443b99af9cc47e5464aa89/src/main/java/org/eclipselabs/garbagecat/service/GcManager.java/right.java
  ...
  /**
     ...
     * @return
     */
  private String getPreprocessedLogEntry(String currentLogLine, String priorLogLine, String nextLogLine, Date jvmStartDate, List<String> savedLogLines) {
    ...
    if (!JdkUtil.discardLogLine(currentLogLine)) {
      ...
      if (UnloadingClassPreprocessAction.match(currentLogLine)) {
        ...
        if (CmsConcurrentModeFailurePreprocessAction.match(currentLogLine, priorLogLine, nextLogLine)) {
          ...
          if (GcTimeLimitExceededPreprocessAction.match(currentLogLine, priorLogLine)) {
            GcTimeLimitExceededPreprocessAction action = new GcTimeLimitExceededPreprocessAction(currentLogLine);
            preprocessedLogLine = action.getLogEntry();
          } else {
            if (PrintHeapAtGcPreprocessAction.match(currentLogLine, priorLogLine)) {
              PrintHeapAtGcPreprocessAction action = new PrintHeapAtGcPreprocessAction(currentLogLine);
              if (action.getLogEntry() != null) {
                preprocessedLogLine = action.getLogEntry();
              }
            } else {
              if (PrintTenuringDistributionPreprocessAction.match(currentLogLine)) {
                PrintTenuringDistributionPreprocessAction action = new PrintTenuringDistributionPreprocessAction(currentLogLine);
                if (action.getLogEntry() != null) {
                  preprocessedLogLine = action.getLogEntry();
                }
              } else {
                if (ParNewCmsConcurrentPreprocessAction.match(currentLogLine, priorLogLine, nextLogLine)) {
                  ParNewCmsConcurrentPreprocessAction action = new ParNewCmsConcurrentPreprocessAction(currentLogLine);
                  if (action.getLogEntry() != null) {
                    preprocessedLogLine = action.getLogEntry();
                  }
                } else {
                  if (ApplicationConcurrentTimePreprocessAction.match(currentLogLine, priorLogLine)) {
                    ApplicationConcurrentTimePreprocessAction action = new ApplicationConcurrentTimePreprocessAction(currentLogLine);
                    if (action.getLogEntry() != null) {
                      preprocessedLogLine = action.getLogEntry();
                    }
                  } else {
                    if (ApplicationStoppedTimePreprocessAction.match(currentLogLine, priorLogLine)) {
                      ApplicationStoppedTimePreprocessAction action = new ApplicationStoppedTimePreprocessAction(currentLogLine);
                      if (action.getLogEntry() != null) {
                        preprocessedLogLine = action.getLogEntry();
                      }
                    } else {
                      if (G1PrintGcDetailsPreprocessAction.match(currentLogLine)) {
                        G1PrintGcDetailsPreprocessAction action = new G1PrintGcDetailsPreprocessAction(priorLogLine, currentLogLine, nextLogLine, savedLogLines);
                        if (action.getLogEntry() != null) {
                          preprocessedLogLine = action.getLogEntry();
                        }
                      } else {
      ...
      while (logLine != null) {
        ...
        if (event instanceof BlockingEvent) {
          ...
          if (event instanceof TriggerData) {
            ...
            JdkUtil.TriggerType triggerType = JdkUtil.determineTriggerType(((TriggerData) event).getTrigger());
            if (!triggerTypes.contains(triggerType)) {
              triggerTypes.add(triggerType);
            }
          }
        } else {
          if (event instanceof ApplicationStoppedTimeEvent) {
            jvmDao.addStoppedTimeEvent((ApplicationStoppedTimeEvent) event);
          } else {
            if (event instanceof HeaderCommandLineFlagsEvent) {
              jvmDao.setOptions(((HeaderCommandLineFlagsEvent) event).getJvmOptions());
            } else {
              if (event instanceof HeaderMemoryEvent) {
                jvmDao.setMemory(((HeaderMemoryEvent) event).getLogEntry());
              } else {
                if (event instanceof HeaderVersionEvent) {
