...
public abstract class CommonResourceTest extends LdpTest {
  ...
  @BeforeClass(alwaysRun = true) public void determineOptions() {
  ...
  @Parameters(value = "auth") public CommonResourceTest(@Optional String auth) throws IOException {
  ...
  @Override protected RequestSpecification buildBaseRequestSpecification() {
    if (auth == null) {
...
<<<<<<< /usr/src/app/output/w3c/ldp-testsuite/8ebaa9b27acbc6165adaa9c0e3f6a4ab49e40bed/src/main/java/org/w3/ldp/testsuite/test/CommonResourceTest.java/left.java
  @Test(groups = { MUST }, description = "LDP servers MUST at least be" + " HTTP/1.1 conformant servers [HTTP11].") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-gen-http", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_EXTENSION, comment = "testIsHttp11Server covers only part of the specification requirement. " + "testIsHttp11Manual covers the rest.") public void testIsHttp11Server() {
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  @Test(groups = { MUST, MANUAL }, description = "LDP servers MUST at least be" + " HTTP/1.1 conformant servers [HTTP11].") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-gen-http", testMethod = METHOD.MANUAL, approval = STATUS.WG_APPROVED, comment = "testIsHttp11Manual covers only part of the specification requirement. " + "testIsHttp11Server covers the rest.") public void testIsHttp11Manual() throws URISyntaxException {
    throw new SkipNotTestableException();
  }

  @Test(groups = { MUST }, description = "LDP server responses MUST use entity tags " + "(either weak or strong ones) as response " + "ETag header values.") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-gen-etags", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_PENDING, comment = "testETagHeadersGet covers only part of the specification requirement. " + "testETagHeadersHead covers the rest.") public void testETagHeadersGet() {
    buildBaseRequestSpecification().expect().statusCode(isSuccessful()).header(ETAG, isValidEntityTag()).when().get(getResourceUri());
  }

  @Test(groups = { MUST }, description = "LDP server responses MUST use entity tags " + "(either weak or strong ones) as response " + "ETag header values.") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-gen-etags", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED, comment = "testETagHeadersHead covers only part of the specification requirement. " + "testETagHeadersGet covers the rest.") public void testETagHeadersHead() {
    buildBaseRequestSpecification().expect().statusCode(isSuccessful()).header(ETAG, isValidEntityTag()).when().head(getResourceUri());
  }

  @Test(groups = { MUST }, description = "LDP servers exposing LDPRs MUST advertise " + "their LDP support by exposing a HTTP Link header " + "with a target URI of http://www.w3.org/ns/ldp#Resource, " + "and a link relation type of type (that is, rel=\'type\') " + "in all responses to requests made to the LDPR\'s " + "HTTP Request-URI.") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-gen-linktypehdr", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED) public void testLdpLinkHeader() {
    Response response = buildBaseRequestSpecification().expect().statusCode(isSuccessful()).when().get(getResourceUri());
    assertTrue(containsLinkHeader(LDP.Resource.stringValue(), LINK_REL_TYPE, response), "4.2.1.4 LDP servers exposing LDPRs must advertise their LDP support by exposing a HTTP Link header " + "with a target URI of http://www.w3.org/ns/ldp#Resource, and a link relation type of type (that is, " + "rel=\'type\') in all responses to requests made to the LDPR\'s HTTP Request-URI. Actual: " + response.getHeader(LINK));
  }

  @Test(groups = { MUST }, description = "LDP servers MUST support the HTTP GET Method for LDPRs") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-get-must", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED) public void testGetResource() {
    ...
    buildBaseRequestSpecification().expect().statusCode(isSuccessful()).when().get(getResourceUri());
  }

  @Test(groups = { MUST }, description = "LDP servers MUST support the HTTP response headers " + "defined in section 4.2.8 HTTP OPTIONS. ") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-get-options", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED) public void testGetResponseHeaders() {
  ...
  @Test(groups = { SHOULD }, description = "LDP clients SHOULD use the HTTP If-Match header and HTTP ETags " + "to ensure it isn\u2019t modifying a resource that has changed since the " + "client last retrieved its representation. LDP servers SHOULD require " + "the HTTP If-Match header and HTTP ETags to detect collisions.") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-put-precond", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED, comment = "testPutRequiresIfMatch covers only part of the specification requirement. " + "testConditionFailedStatusCode, testPreconditionRequiredStatusCode " + "and testPutBadETag covers the rest.") public void testPutRequiresIfMatch() throws URISyntaxException {
  ...
  @Test(groups = { MUST }, description = "LDP servers MUST respond with status code 412 " + "(Condition Failed) if ETags fail to match when there " + "are no other errors with the request [HTTP11]. LDP " + "servers that require conditional requests MUST respond " + "with status code 428 (Precondition Required) when the " + "absence of a precondition is the only reason for rejecting " + "the request [RFC6585].") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-put-precond", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED, comment = "testPreconditionRequiredStatusCode covers only part of the specification requirement. " + "testConditionFailedStatusCode,  testPutBadETag" + "and testPutRequiresIfMatch covers the rest.") public void testPreconditionRequiredStatusCode() {
    skipIfMethodNotAllowed(HttpMethod.PUT);
    String resourceUri = getResourceUri();
  ...
  @Test(groups = { MUST }, description = "LDP servers MUST respond with status code 412 " + "(Condition Failed) if ETags fail to match when there " + "are no other errors with the request [HTTP11]. LDP " + "servers that require conditional requests MUST respond " + "with status code 428 (Precondition Required) when the " + "absence of a precondition is the only reason for rejecting " + "the request [RFC6585].") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-put-precond", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED, comment = "testPutBadETag covers only part of the specification requirement. " + "testConditionFailedStatusCode, testPreconditionRequiredStatusCode " + "and testPutRequiresIfMatch covers the rest.") public void testPutBadETag() {
    skipIfMethodNotAllowed(HttpMethod.PUT);
    String resourceUri = getResourceUri();
    Response response = buildBaseRequestSpecification().expect().statusCode(isSuccessful()).header(ETAG, isValidEntityTag()).when().get(resourceUri);
  ...
  @Test(groups = { MUST }, description = "LDP servers MUST support the HTTP HEAD method. ") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-head-must", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED) public void testHead() {
  ...
  @Test(groups = { MUST }, description = "LDP servers that support PATCH MUST include an " + "Accept-Patch HTTP response header [RFC5789] on HTTP " + "OPTIONS requests, listing patch document media type(s) " + "supported by the server. ") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-patch-acceptpatch", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED) public void testAcceptPatchHeader() {
    if (supports(HttpMethod.PATCH)) {
      buildBaseRequestSpecification().expect().statusCode(isSuccessful()).header(ACCEPT_PATCH, notNullValue()).when().options(getResourceUri());
    }
  }

  @Test(groups = { MUST }, description = "LDP servers MUST support the HTTP OPTIONS method. ") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-options-must", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED) public void testOptions() {
    buildBaseRequestSpecification().expect().statusCode(isSuccessful()).when().options(getResourceUri());
  }

  @Test(groups = { MUST }, description = "LDP servers MUST indicate their support for HTTP Methods " + "by responding to a HTTP OPTIONS request on the LDPR\u2019s URL " + "with the HTTP Method tokens in the HTTP response header Allow. ") @SpecTest(specRefUri = LdpTestSuite.SPEC_URI + "#ldpr-options-allow", testMethod = METHOD.AUTOMATED, approval = STATUS.WG_APPROVED) public void testOptionsAllowHeader() {
    buildBaseRequestSpecification().expect().statusCode(isSuccessful()).header(ALLOW, notNullValue()).when().options(getResourceUri());
  }
