...
public class LdpEarlTestManifest extends AbstractEarlReporter {
  private static final Property declaredInClass = ResourceFactory.createProperty(LDP.LDPT_NAMESPACE + "declaredInClass");

  private static final Property declaredTestCase = ResourceFactory.createProperty(LDP.LDPT_NAMESPACE + "declaredTestCase");

  private static final Property conformanceLevel = ResourceFactory.createProperty(LDP.LDPT_NAMESPACE + "conformanceLevel");

  /**
	 * A link to the published Javadoc for the test case (which in turn links to
	 * the source code).
	 */
  private static final Property documentation = ResourceFactory.createProperty(LDP.LDPT_NAMESPACE + "documentation");
  ...
  /**
	 ...
	 *      earl:mode</a>
	 */
  private static final Property testMethod = ResourceFactory.createProperty(LDP.LDPT_NAMESPACE + "testMethod");

  /**
	 * @see SpecTest.METHOD#AUTOMATED
	 */
  private static final Resource automated = ResourceFactory.createResource(LDP.LDPT_NAMESPACE + "automated");

  /**
	 * @see SpecTest.METHOD#MANUAL
	 */
  private static final Resource manual = ResourceFactory.createResource(LDP.LDPT_NAMESPACE + "manual");

  /**
	 * @see SpecTest.METHOD#CLIENT_ONLY
	 */
  private static final Resource clientOnly = ResourceFactory.createResource(LDP.LDPT_NAMESPACE + "clientOnly");

  /**
	 * @see SpecTest.METHOD#NOT_IMPLEMENTED
	 */
  private static final Resource notImplemented = ResourceFactory.createResource(LDP.LDPT_NAMESPACE + "notImplemented");

  /**
	 * @see SpecTest.steps
	 */
  private static final Property steps = ResourceFactory.createProperty(LDP.LDPT_NAMESPACE + "steps");
  ...
  private void writeManifest(ArrayList<Resource> testcases, String localName, String label, String description) {
    ...
    Resource manifest = model.createResource(LDP.LDPT_NAMESPACE + localName + "Manifest", TestManifest.Manifest);
  ...
  private <T extends java.lang.Object> void writeInfo(Class<T> testClass, String title, String description) {
    ...
    writeManifest(conformanceClasses.get(LdpTestCaseReporter.MUST), title + "-MUST", title + " (MUST)", description + " MUST conformance tests.");
    writeManifest(conformanceClasses.get(LdpTestCaseReporter.SHOULD), title + "-SHOULD", title + " (SHOULD)", description + " SHOULD conformance tests.");
    writeManifest(conformanceClasses.get(LdpTestCaseReporter.MAY), title + "-MAY", title + " (MAY)", description + " MAY conformance tests.");
    writeManifest(conformanceClasses.get(LdpTestCaseReporter.OTHER), title + "-OTHER", title + " (OTHER)", description + " No official conformance status.");
  }
  ...
  private Resource generateInformation(Method method, String className, ArrayList<ArrayList<Resource>> conformanceClasses) {
    ...
    if (method.getAnnotation(SpecTest.class) != null && method.getAnnotation(Test.class) != null) {
      ...
      if (testLdp.testMethod() == METHOD.CLIENT_ONLY) {
        System.err.println("Wrongly received CLIENT_ONLY test for " + testCaseName + ". Client-only tests should be defined in separate RDF manifest file.");
      }
      ...
      testCaseResource.addProperty(documentation, model.createResource(ReportUtils.getJavadocLink(method)));
