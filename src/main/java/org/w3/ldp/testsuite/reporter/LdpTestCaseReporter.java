...
public class LdpTestCaseReporter {
  ...
  private static HashMap<String, String> readyToBeApproved = new HashMap<String, String>();

  private static HashMap<String, String> needCode = new HashMap<String, String>();

  private static final HashMap<String, String> implmColor = new HashMap<String, String>();
  ...
  private static void makeReport() throws IOException {
    ...
    if (commit != null) {
      html.div().write("Test Suite Revision: ").a(href("https://github.com/w3c/ldp-testsuite/commit/" + commit)).content(commit)._div();
    }
  ...
  private static void createSummaryReport() throws IOException {
    ...
    html.ul().li().span(style(writeBlockStyle(statusColor, "approved")))._span().b().write(approved + " ")._b().write("WG Approved")._li();
    html.li().span(style(writeBlockStyle(statusColor, "pending")))._span().b().write(pending + " ")._b().write("Incomplete")._li();
    html.li().span(style(writeBlockStyle(statusColor, "extends")))._span().b().write(extended + " ")._b().write("Extension")._li();
    html.li().span(style(writeBlockStyle(statusColor, "deprecated")))._span().b().write(deprecated + " ")._b().write("No longer valid")._li();
    html.li().span(style(writeBlockStyle(statusColor, "clarify")))._span().b().write(clarification + " ")._b().write("Needs to be clarified with WG")._li();
    html._ul();
    html.br().b().a(href("#tobeapproved")).write(readyToBeApproved.size() + " Ready for WG Approval")._a()._b();
    html.br();
    ...
    html._td();
    html.td();
    html.b().write(automated + " / " + totalTests)._b().write(" of Total Tests Automated").br();
    html.b().write(automated + " / " + (automated + needCode.size()))._b().write(" of Tests Possible to Automate");
    html.ul();
    html.li().b().write("" + refURI.size())._b().write(" Requirements Covered")._li();
    ...
    html.li().span(style(writeBlockStyle(implmColor, "automated")))._span().b().write(implemented + " ")._b().write("Requirements Automated")._li();
    html.ul();
    html.li().b().write(auto[MUST] + " / " + mustTotal)._b().write(" MUST")._li();
    html.li().b().write(auto[SHOULD] + " / " + shouldTotal)._b().write(" SHOULD")._li();
    html.li().b().write(auto[MAY] + " / " + mayTotal)._b().write(" MAY")._li();
    html._ul();
    html._ul();
    html._td();
    html.td();
    html.b().write((needCode.size() + clients.size() + manuals.size()) + " ")._b().write("of the Unimplemented Tests");
    html.ul();
    html.li().b().write(needCode.size() + " ")._b().write("of the Tests ").a(href("#needCode")).write("Yet to be Coded")._a()._li();
    html.li().span(style(writeBlockStyle(implmColor, "client")))._span().b().write(clients.size() + " ")._b().write("of the Total are ").a(href("#clientTests")).write("Client-Based Tests")._a()._li();
    html.li().span(style(writeBlockStyle(implmColor, "manual")))._span().b().write(manuals.size() + " ")._b().write("of the Total must be ").a(href("#manualTests")).write("Tested Manually")._a()._li();
    html._ul();
    html.write("From the Total, ");
    html.ul();
    int unimplemented = getTotal(unimplmnt);
    html.li().span(style(writeBlockStyle(implmColor, "unimplemented")))._span().b().write(unimplemented + " ")._b().write("Requirements not Implemented")._li();
    html.ul();
  ...
  private static void generateListOfTestCases() throws IOException {
    ...
    if (readyToBeApproved.size() == 0) {
      ...
      html.p().write("For details of test cases, ").a(href("https://github.com/w3c/ldp-testsuite")).write("see source in GitHub")._a()._p();
      html.ul();
    ...
    if (needCode.size() == 0) {
      html.b().write("No test cases that need to be Implemented")._b().br();
    } else {
      html.ul();
    ...
    toTop();
    html.h2().a(id("manualTests")).content("Tests that Must be Tested Manually")._h2();
    generateList(manuals);
    html.h2().a(id("clientTests")).content("Client-Based Test Cases")._h2();
    generateList(clients);
  }

  private static <T extends java.lang.Object> void writeTestTables(Class<T> testClass) throws IOException {
    html.b().a(href("#" + testClass.getCanonicalName())).content(testClass.getCanonicalName())._b();
    html.br();
  ...
  private static <T extends java.lang.Object> void writeTestClassTable(Class<T> classType) throws IOException {
    html.span(class_("chartStart"));
  ...
  private static <T extends java.lang.Object> void acquireTestInfo(Class<T> classType) throws IOException {
    int total = 0, must = 0, should = 0, may = 0;
    int[] autoReq = { 0, 0, 0 }, unimReq = { 0, 0, 0 }, clientReq = { 0, 0, 0 }, manReq = { 0, 0, 0 };
    int[] apprReq = { 0, 0, 0 }, pendReq = { 0, 0, 0 }, extReq = { 0, 0, 0 }, depreReq = { 0, 0, 0 }, clariReq = { 0, 0, 0 };
    Method[] methods = classType.getDeclaredMethods();
    for (Method method : methods) {
      if (method.isAnnotationPresent(SpecTest.class) && method.isAnnotationPresent(Test.class)) {
        total++;
  ...
  private static void writeStatusGraph(String type, int[] apprReq, int[] pendReq, int[] extReq, int[] depreReq, int[] clariReq) throws IOException {
    graphs.write("<script>");
    graphs.write("Event.observe(window, \'load\', function() {");
    graphs.write("var " + type + "_statusbar" + " = new Grafico.StackedBarGraph($(\'" + type + "_statusbar\'),{");
  ...
  private static void writeImplementationGraph(String classType, int[] autoReq, int[] unimReq, int[] clientReq, int[] manReq) {
    graphs.write("<script>");
    graphs.write("Event.observe(window, \'load\', function() {");
    graphs.write("var " + classType + "_implmtbar" + " = new Grafico.StackedBarGraph($(\'" + classType + "_implmtbar\'), {");
  ...
  private static <T extends java.lang.Object> void acquireTestCases(Class<T> classType) throws IOException {
    ...
    for (Method method : declaredMethods) {
      if (method.isAnnotationPresent(Test.class)) {
        if (!initialRead) {
          ...
          html.li().b().a(id(method.getName())).write(method.getName() + ": ")._a()._b();
          generateInformation(method, name);
          html._li();
        }
      }
  ...
  private static void generateInformation(Method method, String name) throws IOException {
    ...
    if (method.getAnnotation(SpecTest.class) != null && method.getAnnotation(Test.class) != null) {
      ...
      if (!initialRead) {
        ...
        html.br().b().write("Reference URI: ")._b().a(href(testLdp.specRefUri())).write(testLdp.specRefUri())._a();
        html.br().b().write("Groups: ")._b().write(Arrays.toString(test.groups()));
        html.br().b().write("Status: ")._b().write(testLdp.approval().toString());
        html.br().b().write("Test Case Implementation: ")._b().write("" + testLdp.testMethod());
        html.br().b().write("Enabled: ")._b().write("" + test.enabled());
        html._div();
        toTestClass(name);
      }
    }
  ...
  private static void toTestClass(String name) throws IOException {
    html.p(class_("totest")).a(href("#" + name)).content("Back to Main Test Class")._p();
  }

  private static void writeCss() throws IOException {
    html.style().write(StringResource.get("testCaseStyle.css"), NO_ESCAPE)._style();
  }
  ...
  private static void createWriter(String directory, String output) {
    ...
    try {
      writer = new PrettyWriter(new FileWriter(directory + "/ldp-testsuite-coverage-report.html"));
      writer.write(output);
    } catch (IOException e) {
