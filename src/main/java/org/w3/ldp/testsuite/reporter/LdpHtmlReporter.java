...
public class LdpHtmlReporter implements IReporter {
  ...
  public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
    try {
      for (ISuite suite : suites) {
        ...
        html.div(class_("barChart").id("overallChart1"))._div();
        writePassFailLegend();
        html._span();
        html.span(class_("chartStart"));
  ...
  private void writeCss() throws IOException {
    html.style().write(StringResource.get("reportStyle.css"), NO_ESCAPE)._style();
  }
  ...
  private void createWriter(String directory, String output, String title) {
    ...
    try {
      writer = new BufferedWriter(new FileWriter(directory + "/" + title + "-execution-report.html"));
      writer.write(output);
    } catch (IOException e) {
    } finally {
      try {
  ...
  private void generateOverallSummaryReport(List<ISuite> suites, String id) throws IOException {
    ...
    for (ISuite suite : suites) {
      ...
      for (ISuiteResult sr : suiteResults.values()) {
        ITestContext tc = sr.getTestContext();
        Iterator<ITestResult> passedResults = tc.getPassedTests().getAllResults().iterator();
        ...
        Iterator<ITestResult> failedResults = tc.getFailedTests().getAllResults().iterator();
        ...
        Iterator<ITestResult> skippedResults = tc.getSkippedTests().getAllResults().iterator();
  ...
  private void generateSummaryTableStart(Date date, String suiteName) throws IOException {
  ...
  private void displayGroupsInfo(List<ISuite> suite) throws IOException {
    for (ISuite testSuite : suite) {
      ...
      for (ISuiteResult results : tests.values()) {
        ...
        generateList(included, "Included Groups for " + testSuite.getName());
        generateList(excluded, "Excluded Groups for " + testSuite.getName());
      }
    }
  }
  ...
  private void generateList(String[] list, String title) throws IOException {
    ...
    if (list.length == 0) {
      html.div(style("padding-left:2em")).i().content("No groups of this type found in the test suite")._div();
    } else {
      html.ul();
  ...
  private void makeMethodSummaryTable(IResultMap tests, String title) throws IOException {
    ...
    for (ITestResult result : tests.getAllResults()) {
      ...
      if (method.getConstructorOrMethod().getMethod().getAnnotation(SpecTest.class).testMethod().equals(METHOD.INDIRECT)) {
        ...
        html.td().a(href("#" + method.getTestClass().getName() + "_" + method.getMethodName())).write(method.getMethodName(), NO_ESCAPE)._a()._td();
        html.td().content(Arrays.toString(method.getGroups()));
        html.td().content(method.getTestClass().getName());
        html.td().content((method.getDescription() != null ? method.getDescription() : "No Description found"));
        html._tr();
      }
    }
    html._table();
      ...
      if (result.size() > 0) {
        if (result.contains("failed")) {
          ...
          if (result.contains("passed") && !result.contains("failed")) {
            ...
            if (result.contains("skipped") && !result.contains("failed") && !result.contains("passed")) {
  ...
  private void generateDetail(IResultMap tests) throws IOException {
    for (ITestResult m : tests.getAllResults()) {
      ...
      html.h2().a(id(m.getTestClass().getName() + "_" + method.getMethodName())).write(m.getTestClass().getName() + ": " + method.getMethodName())._a()._h2();
      getAdditionalInfo(m, method);
      html.p(class_("indented")).b().write("Description: ")._b().write((method.getDescription() != null ? method.getDescription() : "No description for this test method found"))._p();
      String groups = "";
      for (String group : method.getGroups()) {
        groups += group + " ";
      }
      html.p(class_("indented")).b().write("Requirement Level: ")._b().write(groups)._p();
      toTop();
    }
  }

  private void getAdditionalInfo(ITestResult m, ITestNGMethod method) throws IOException {
    ...
    if (m.getParameters() != null && m.getParameters().length != 0) {
      ...
      html.p(class_("indented")).b().write("Parameters: ")._b().write(parameters)._p();
    }
    if (m.getMethod().getConstructorOrMethod().getMethod().getAnnotation(SpecTest.class) != null) {
      SpecTest testLdp = m.getMethod().getConstructorOrMethod().getMethod().getAnnotation(SpecTest.class);
      if (!testLdp.comment().equals("")) {
        html.p(class_("note")).b().write("NOTE: ")._b().write(testLdp.comment())._p();
      }
...
>>>>>>> /usr/src/app/output/w3c/ldp-testsuite/a7515238ae8139c8d6b2ee7e042183294ca3869f/src/main/java/org/w3/ldp/testsuite/reporter/LdpHtmlReporter.java/right.java
      ...
      html.p(class_("indented")).b().write("Reference URI: ")._b().a(href(testLdp.specRefUri())).write(testLdp.specRefUri())._a()._p();
    }
  }
  ...
  private void createSkipExceptionTable(Throwable thrown) throws IOException {
    html.table(class_("indented"));
    html.tr(class_("center")).th(class_("Skipped")).content("[SKIPPED TEST]")._tr();
  ...
  private void createThrownTable(Throwable thrown) throws IOException {
    html.table(class_("indented"));
    html.tr(class_("center")).th(class_("Failed")).content("[FAILED TEST]")._tr();
  ...
  private void writeOverallBarChart() throws IOException {
    ...
    graphs.write("{ passed: [ " + mustPass + ", " + shouldPass + ", " + mayPass + " ],");
    graphs.write("failed: [" + mustFailed + ", " + shouldFailed + ", " + mayFailed + " ], ");
    ...
    graphs.write("datalabels: { passed: [ \"" + mustPass + " Passed\", \"" + shouldPass + " Passed\", \"" + mayPass + " Passed\"],");
    graphs.write("failed: [ \"" + mustFailed + " Failed\" , \"" + shouldFailed + " Failed\" , \"" + mayFailed + " Failed\" ],");
    graphs.write("skipped: [ \"" + mustSkip + " Skipped\", \"" + shouldSkip + " Skipped\", \"" + maySkip + " Skipped\" ]");
    graphs.write("},");
    graphs.write("}); });");
    graphs.write("</script>");
  }
  ...
  private void writeTestClassLegend() throws IOException {
    ...
    while (label.hasNext() && getColor != colors.size()) {
      ...
      html.write("<rect width=\"15\" height=\"15\" x=\"0\" y=\"" + rectY + "\" style=\"fill:" + colors.get(getColor) + "\"/>", NO_ESCAPE);
      html.write("<text x=\"20\" y=\"" + textY + "\" fill=\"black\">" + className + "</text>", NO_ESCAPE);
      textY += 20;
      rectY += 20;
      getColor++;
    }
  ...
  private void writeChartLabels(HashMap<String, Integer> passClasses, HashMap<String, Integer> failClasses, HashMap<String, Integer> skipClasses) {
    graphs.write("datalabels: {");
    Set<String> names = passClasses.keySet();
    Iterator<String> label = names.iterator();
    while (label.hasNext()) {
      String className = label.next();
      graphs.write(className + ": [ \"" + passClasses.get(className) + " " + className + "\", \"" + failClasses.get(className) + " " + className + "\", \"" + skipClasses.get(className) + " " + className + "\" ]");
      if (label.hasNext()) {
        graphs.write(",");
      }
    }
  ...
  private void writeChartValues(HashMap<String, Integer> passClasses, HashMap<String, Integer> failClasses, HashMap<String, Integer> skipClasses) {
    graphs.write("{");
    Set<String> names = passClasses.keySet();
    Iterator<String> label = names.iterator();
    while (label.hasNext()) {
      String className = label.next();
      graphs.write(className + ": [" + passClasses.get(className) + ", " + failClasses.get(className) + ", " + skipClasses.get(className) + " ]");
      if (label.hasNext()) {
        graphs.write(",");
      }
    }
