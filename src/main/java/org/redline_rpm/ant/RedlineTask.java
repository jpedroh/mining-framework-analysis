...
public class RedlineTask extends Task {
  ...
  @Override public void execute() {
    ...
    try {
      ...
      for (Provides provision : moreProvides) {
        builder.addProvides(provision.getName(), provision.getVersion());
      }
      for (Conflicts conflict : conflicts) {
        builder.addConflicts(conflict.getName(), conflict.getComparison(), conflict.getVersion());
      }
      for (Obsoletes obsoletion : obsoletes) {
        builder.addObsoletes(obsoletion.getName(), obsoletion.getComparison(), obsoletion.getVersion());
      }
  ...
  public void addObseletes(Obsoletes obsoletion) {
