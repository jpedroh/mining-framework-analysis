...
public class ExtractingParseObserver implements ParseObserver {
  ...
  protected static String jsOnClickUrl1PatString = "(?i)^(?:javascript:)?(?:(?:window|top|document|self|parent)\\.)?location(?:\\.href)?\\s*=\\s*(\'|&#39;)([^\'\"]{3,256})\\1$";

  protected static String jsOnClickUrl2PatString = "(?i)^(?:javascript:)?(?:window|parent)\\.open\\(([\'\"]|&#39;)([^\"\']{3,256}?)\\1[,)]";

  protected static Pattern[] jsOnClickUrlPatterns = { Pattern.compile(jsOnClickUrl1PatString), Pattern.compile(jsOnClickUrl2PatString) };
