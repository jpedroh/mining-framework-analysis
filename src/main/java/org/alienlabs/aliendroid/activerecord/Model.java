...
abstract public class Model {
  ...
  public void load(final Integer id) {
    final Cursor cursor = getHelper().getReadableDatabase().query(getTableName(), Reflection.getNonStaticDeclaredFieldsNames(this.getClass()), "_id=?", new String[] { id.toString() }, null, null, null);
  ...
  private static String getType(final Field field) {
    ...
    if (cls.equals("string") || cls.equals("char") || cls.equals("character")) {
      ...
      if (cls.equals("integer") || cls.equals("int") || cls.equals("long") || cls.equals("short") || cls.equals("byte") || cls.equals("boolean")) {
        type = "INTEGER";
      } else {
        if (cls.equals("double") || cls.equals("float")) {
          type = "REAL";
        } else {
          if (cls.equals("date")) {
            type = "DATE";
