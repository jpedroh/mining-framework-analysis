...
public class GraphitePlainTextTcpOutputWriter extends AbstractOutputWriter implements OutputWriter {
  ...
  @Override public void postConstruct(Map<String, String> settings) {
    super.postConstruct(settings);
    graphiteServerHostAndPort = new HostAndPort(getString(settings, SETTING_HOST), getInt(settings, SETTING_PORT, SETTING_PORT_DEFAULT_VALUE));
    ...
    logger.log(getInfoLevel(), "GraphitePlainTextTcpOutputWriter is configured with " + graphiteServerHostAndPort + ", metricPathPrefix=" + messageBuilder.getPrefix() + ", socketConnectTimeoutInMillis=" + socketConnectTimeoutInMillis);
  }
  ...
  @Override public void writeQueryResult(@Nonnull String metricName, @Nullable String type, @Nullable Object value) throws IOException {
  ...
  private void ensureGraphiteConnection() throws IOException {
    ...
    try {
      socketIsValid = socket != null && socket.isConnected() && socket.isBound() && !socket.isClosed() && !socket.isInputShutdown() && !socket.isOutputShutdown();
    } catch (Exception e) {
      socketIsValid = false;
    }
    if (!socketIsValid) {
      ...
      try {
        ...
        socket.connect(new InetSocketAddress(graphiteServerHostAndPort.getHost(), graphiteServerHostAndPort.getPort()), socketConnectTimeoutInMillis);
      } catch (IOException e) {
  ...
  @Override public String toString() {
    return "GraphitePlainTextTcpOutputWriter{" + ", " + graphiteServerHostAndPort + ", metricPathPrefix=\'" + messageBuilder.getPrefix() + '\'' + '}';
  }
