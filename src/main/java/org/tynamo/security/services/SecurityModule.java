 ...
 *
 */
@Security public class SecurityModule {
  ...
  @SuppressWarnings(value = { "rawtypes", "unchecked" }) public static RememberMeManager buildRememberMeManager(Serializer serializer, Logger logger, @Symbol(value = SymbolConstants.HMAC_PASSPHRASE) String hmacPassphrase, @Symbol(value = SecuritySymbols.REMEMBERME_CIPHERKERY) String rememberMeCipherKey) throws UnsupportedEncodingException {
    ...
    if (cipherKey.length <= 0) {
      if (hmacPassphrase.isEmpty()) {
        logger.error("Neither symbol \'" + SecuritySymbols.REMEMBERME_CIPHERKERY + "\' nor  \'" + SymbolConstants.HMAC_PASSPHRASE + "\' is set. Using a random value as the cipher key for encrypting rememberMe information. Cookies will be invalidated when the JVM is restarted");
        return rememberMeManager;
      }
      logger.warn("Symbol \'" + SecuritySymbols.REMEMBERME_CIPHERKERY + "\' is not set, using \'" + SymbolConstants.HMAC_PASSPHRASE + "\' as the cipher. Beware that changing the value will invalidate rememberMe cookies");
  ...
  public void contributeApplicationInitializer(OrderedConfiguration<ApplicationInitializerFilter> configuration, final ComponentClassResolver componentClassResolver, final ClassInterceptorsCache classInterceptorsCache) {
    configuration.add("SecurityApplicationInitializerFilter", new ApplicationInitializerFilter() {
      @Override public void initializeApplication(Context context, ApplicationInitializer initializer) {
  ...
  @SuppressWarnings(value = { "rawtypes" }) @Contribute(value = Serializer.class) public static void addSafePrincipalTypes(Configuration<Class> configuration) {
  ...
  @Contribute(value = ComponentClassTransformWorker2.class) public static void addTransformWorkers(OrderedConfiguration<ComponentClassTransformWorker2> configuration) {
  ...
  /**
	 ...
	 */
  @Match(value = "*") @Order(value = "before:*") public static void adviseSecurityAssert(MethodAdviceReceiver receiver, final @Core Environment environment) {
    ...
    for (Method method : serviceInterface.getMethods()) {
      ...
      for (final SecurityInterceptor interceptor : interceptors) {
        MethodAdvice advice = new MethodAdvice() {
          @Override public void advise(MethodInvocation invocation) {
  ...
  @SuppressWarnings(value = { "rawtypes" }) public void contributeRequestExceptionHandler(MappedConfiguration<Class, Object> configuration) {
  ...
  @Contribute(value = HttpServletRequestFilter.class) @Security public static void defaultSecurity(Configuration<SecurityFilterChain> configuration, SecurityFilterChainFactory factory) {
