 ...
 */
public class PersistentTreeMap<K extends java.lang.Object, V extends java.lang.Object> extends AbstractUnmodMap<K, V> implements ImSortedMap<K, V>, Serializable {
  static class Box<E extends java.lang.Object> {
    E val;
  ...
  /**
     ...
     any null Entries.
     */
  public static <K extends Comparable<K>, V extends java.lang.Object> PersistentTreeMap<K, V> of(Iterable<Map.Entry<K, V>> es) {
    if (es == null) {
      return empty();
    }
  ...
  /**
     ...
     */
  public static <K extends java.lang.Object, V extends java.lang.Object> PersistentTreeMap<K, V> ofComp(Comparator<? super K> comp, Iterable<Map.Entry<K, V>> kvPairs) {
    if (kvPairs == null) {
  ...
  /**
     ...
     be removed.
     */
  @SuppressWarnings(value = { "unchecked" }) public static <K extends Comparable<K>, V extends java.lang.Object> PersistentTreeMap<K, V> empty() {
    return (PersistentTreeMap<K, V>) EMPTY;
  }

  /** Returns a new empty PersistentTreeMap that will use the specified comparator. */
  public static <K extends java.lang.Object, V extends java.lang.Object> PersistentTreeMap<K, V> empty(Comparator<? super K> c) {
    return new PersistentTreeMap<>(c, null, 0);
  }

  static final class KeyComparator<T extends java.lang.Object> implements Comparator<Map.Entry<T, ?>>, Serializable {
  ...
  @SuppressWarnings(value = { "unchecked" }) private static <K extends java.lang.Object, V extends java.lang.Object> Node<K, V> append(Node<? extends K, ? extends V> left, Node<? extends K, ? extends V> right) {
    if (left == null) {
        ...
        if (left instanceof PersistentTreeMap.Red) {
          if (right instanceof PersistentTreeMap.Red) {
            ...
            if (app instanceof PersistentTreeMap.Red) {
              return red(app.getKey(), app.getValue(), red(left.getKey(), left.getValue(), left.left(), app.left()), red(right.getKey(), right.getValue(), app.right(), right.right()));
            } else {
              return red(left.getKey(), left.getValue(), left.left(), red(right.getKey(), right.getValue(), app, right.right()));
            }
          } else {
          ...
          if (right instanceof PersistentTreeMap.Red) {
            ...
            Node<K, V> app = append(left.right(), right.left());
            if (app instanceof PersistentTreeMap.Red) {
              return red(app.getKey(), app.getValue(), black(left.getKey(), left.getValue(), left.left(), app.left()), black(right.getKey(), right.getValue(), app.right(), right.right()));
            } else {
              return balanceLeftDel(left.getKey(), left.getValue(), left.left(), black(right.getKey(), right.getValue(), app, right.right()));
            }
          }
        }
      }
  ...
  private static <K extends java.lang.Object, V extends java.lang.Object, K1 extends K, V1 extends V> Node<K, V> balanceLeftDel(K1 key, V1 val, Node<? extends K, ? extends V> del, Node<? extends K, ? extends V> right) {
    if (del instanceof PersistentTreeMap.Red) {
      return red(key, val, del.blacken(), right);
    } else {
      if (right instanceof PersistentTreeMap.Black) {
        return rightBalance(key, val, del, right.redden());
      } else {
        if (right instanceof PersistentTreeMap.Red && right.left() instanceof PersistentTreeMap.Black) {
          return red(right.left().getKey(), right.left().getValue(), black(key, val, del, right.left().left()), rightBalance(right.getKey(), right.getValue(), right.left().right(), right.right().redden()));
        } else {
  ...
  private static <K extends java.lang.Object, V extends java.lang.Object, K1 extends K, V1 extends V> Node<K, V> balanceRightDel(K1 key, V1 val, Node<? extends K, ? extends V> left, Node<? extends K, ? extends V> del) {
    if (del instanceof PersistentTreeMap.Red) {
      return red(key, val, left, del.blacken());
    } else {
      if (left instanceof PersistentTreeMap.Black) {
        return leftBalance(key, val, left.redden(), del);
      } else {
        if (left instanceof PersistentTreeMap.Red && left.right() instanceof PersistentTreeMap.Black) {
          return red(left.right().getKey(), left.right().getValue(), leftBalance(left.getKey(), left.getValue(), left.left().redden(), left.right().left()), black(key, val, left.right().right(), del));
        } else {
          throw new UnsupportedOperationException("Invariant violation");
        }
      }
  ...
  private static <K extends java.lang.Object, V extends java.lang.Object, K1 extends K, V1 extends V> Node<K, V> leftBalance(K1 key, V1 val, Node<? extends K, ? extends V> ins, Node<? extends K, ? extends V> right) {
    if (ins instanceof PersistentTreeMap.Red && ins.left() instanceof PersistentTreeMap.Red) {
      return red(ins.getKey(), ins.getValue(), ins.left().blacken(), black(key, val, ins.right(), right));
    } else {
      if (ins instanceof PersistentTreeMap.Red && ins.right() instanceof PersistentTreeMap.Red) {
        return red(ins.right().getKey(), ins.right().getValue(), black(ins.getKey(), ins.getValue(), ins.left(), ins.right().left()), black(key, val, ins.right().right(), right));
      } else {
  ...
  private static <K extends java.lang.Object, V extends java.lang.Object, K1 extends K, V1 extends V> Node<K, V> rightBalance(K1 key, V1 val, Node<? extends K, ? extends V> left, Node<? extends K, ? extends V> ins) {
    if (ins instanceof PersistentTreeMap.Red && ins.right() instanceof PersistentTreeMap.Red) {
      return red(ins.getKey(), ins.getValue(), black(key, val, left, ins.left()), ins.right().blacken());
    } else {
      if (ins instanceof PersistentTreeMap.Red && ins.left() instanceof PersistentTreeMap.Red) {
        return red(ins.left().getKey(), ins.left().getValue(), black(key, val, left, ins.left().left()), black(ins.getKey(), ins.getValue(), ins.left().right(), ins.right()));
      } else {
  ...
  private Node<K, V> replace(Node<K, V> t, K key, V val) {
    int c = comp.compare(key, t.getKey());
    return t.replace(t.getKey(), c == 0 ? val : t.getValue(), c < 0 ? replace(t.left(), key, val) : t.left(), c > 0 ? replace(t.right(), key, val) : t.right());
  }

  @SuppressWarnings(value = { "unchecked", "RedundantCast", "Convert2Diamond" }) private static <K extends java.lang.Object, V extends java.lang.Object, K1 extends K, V1 extends V> Red<K, V> red(K1 key, V1 val, Node<? extends K, ? extends V> left, Node<? extends K, ? extends V> right) {
  ...
  @SuppressWarnings(value = { "unchecked", "RedundantCast", "Convert2Diamond" }) private static <K extends java.lang.Object, V extends java.lang.Object, K1 extends K, V1 extends V> Black<K, V> black(K1 key, V1 val, Node<? extends K, ? extends V> left, Node<? extends K, ? extends V> right) {
    if (left == null && right == null) {
  ...
  private static abstract class Node<K extends java.lang.Object, V extends java.lang.Object> extends Tuple2<K, V> {
    Node(K key, V val) {
      super(key, val);
    }
    ...
    @SuppressWarnings(value = { "UnusedDeclaration" }) abstract Node<K, V> removeLeft(Node<K, V> del);

    @SuppressWarnings(value = { "UnusedDeclaration" }) abstract Node<K, V> removeRight(Node<K, V> del);
    ...
    Node<K, V> balanceLeft(Node<K, V> parent) {
      return black(parent.get_1(), parent.get_2(), this, parent.right());
    }

    Node<K, V> balanceRight(Node<K, V> parent) {
      return black(parent.get_1(), parent.get_2(), parent.left(), this);
    }

    abstract Node<K, V> replace(K key, V val, Node<K, V> left, Node<K, V> right);

    @Override public String toString() {
      return stringify(get_1()) + "=" + stringify(get_2());
    }
  }

  private static class Black<K extends java.lang.Object, V extends java.lang.Object> extends Node<K, V> {
    Black(K key, V val) {
      super(key, val);
    }

    @Override Node<K, V> addLeft(Node<K, V> ins) {
      return ins.balanceLeft(this);
    }

    @Override Node<K, V> addRight(Node<K, V> ins) {
      return ins.balanceRight(this);
    }

    @Override Node<K, V> removeLeft(Node<K, V> del) {
      return balanceLeftDel(get_1(), get_2(), del, right());
    }

    @Override Node<K, V> removeRight(Node<K, V> del) {
      return balanceRightDel(get_1(), get_2(), left(), del);
    }

    @Override Node<K, V> blacken() {
      return this;
    }

    @Override Node<K, V> redden() {
      return new Red<>(get_1(), get_2());
    }

    @Override Node<K, V> replace(K key, V val, Node<K, V> left, Node<K, V> right) {
      return black(key, val, left, right);
    }
  }

  private static class BlackBranch<K extends java.lang.Object, V extends java.lang.Object> extends Black<K, V> {
    ...
    @Override public Node<K, V> left() {
      return left;
    }

    @Override public Node<K, V> right() {
      return right;
    }

    @Override Node<K, V> redden() {
      return new RedBranch<>(get_1(), get_2(), left, right);
    }
  }

  private static class Red<K extends java.lang.Object, V extends java.lang.Object> extends Node<K, V> {
    Red(K key, V val) {
      super(key, val);
    }

    @Override Node<K, V> addLeft(Node<K, V> ins) {
      return red(get_1(), get_2(), ins, right());
    }

    @Override Node<K, V> addRight(Node<K, V> ins) {
      return red(get_1(), get_2(), left(), ins);
    }

    @Override Node<K, V> removeLeft(Node<K, V> del) {
      return red(get_1(), get_2(), del, right());
    }

    @Override Node<K, V> removeRight(Node<K, V> del) {
      return red(get_1(), get_2(), left(), del);
    }

    @Override Node<K, V> blacken() {
      return new Black<>(get_1(), get_2());
    }
  ...
  private static class RedBranch<K extends java.lang.Object, V extends java.lang.Object> extends Red<K, V> {
    final transient Node<K, V> left;
    ...
    @Override Node<K, V> balanceLeft(Node<K, V> parent) {
      if (left instanceof PersistentTreeMap.Red) {
        return red(get_1(), get_2(), left.blacken(), black(parent.getKey(), parent.getValue(), right, parent.right()));
      } else {
        if (right instanceof PersistentTreeMap.Red) {
          return red(right.getKey(), right.getValue(), black(get_1(), get_2(), left, right.left()), black(parent.getKey(), parent.getValue(), right.right(), parent.right()));
        } else {
    ...
    @Override Node<K, V> balanceRight(Node<K, V> parent) {
      if (right instanceof PersistentTreeMap.Red) {
        return red(get_1(), get_2(), black(parent.getKey(), parent.getValue(), parent.left(), left), right.blacken());
      } else {
        if (left instanceof PersistentTreeMap.Red) {
          return red(left.getKey(), left.getValue(), black(parent.getKey(), parent.getValue(), parent.left(), left.left()), black(get_1(), get_2(), left.right(), right));
        } else {
    ...
    @Override Node<K, V> blacken() {
      return new BlackBranch<>(get_1(), get_2(), left, right);
    }
  }

  private static class NodeIterator<K extends java.lang.Object, V extends java.lang.Object> implements UnmodSortedIterator<UnEntry<K, V>> {
    ...
    @SuppressWarnings(value = { "unchecked" }) @Override public UnmodMap.UnEntry<K, V> next() {
      Node<K, V> t = stack.pop();
      push(asc ? t.right() : t.left());
      return Tuple2.Companion.of(t);
    }
  }
}