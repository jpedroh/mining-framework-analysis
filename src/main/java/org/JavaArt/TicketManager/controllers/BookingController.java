...
public class BookingController {
    ...
    public String bookingGet(Model model) throws SQLException {
        ...
        if ((events != null) && (events.size() > 0)) {
            ...
            //init set to fix crush for add button
...
//                model.addAttribute("seatsMap", seatsMap);
    ...
    public String bookingSetSectors(@RequestParam(value = "eventId", required = true)
    ...
    public String bookingSetRow(@RequestParam(value = "sectorId", required = true)
    int sectorId, Model model) throws SQLException {
    ...
    public String bookingSetSeat(@RequestParam(value = "row", required = true)
    int row, @ModelAttribute
    Sector sector, Model model) throws SQLException {
//        Map<Integer, Boolean> seatsMap = new TreeMap<>();
...
//        }
        ...
        for (int i = 1; i <= sector.getMaxSeats(); i++) {
            if (ticketService.isPlaceFree(sector, row, i)) {
                seatsMap.add(i);
            }
        }
    ...
    public String bookingAddTicket(@ModelAttribute("row")
    int row, @RequestParam(value = "seats", required = false)
    int[] seats, @ModelAttribute
    Sector sector, SessionStatus status, Model model) throws SQLException {
        if (seats == null) {
