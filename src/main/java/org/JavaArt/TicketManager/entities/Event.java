...
@Entity @Table(name = "event") public class Event implements Comparable<Event> {
  ...
  @Column private int BookingTimeOut;

  @Column(name = "isDeleted") private boolean isDeleted;

  @Column(name = "TimeStamp") private Date timeStamp = new Date();

  @JoinColumn(name = "operator_id") @ManyToOne(cascade = CascadeType.ALL) Operator operator;
  ...
  @Override @NotNull public int compareTo(Event event) {
