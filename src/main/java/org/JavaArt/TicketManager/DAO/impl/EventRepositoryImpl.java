...
@Repository public class EventRepositoryImpl implements EventRepository {
  ...
  @Override public Event getEventById(int id) {
    Session session = null;
    Event event = null;
    try {
      session = HibernateUtil.getSessionFactory().openSession();
  ...
  @Override public List<Event> getAllEvents() {
    ...
    try {
      session = HibernateUtil.getSessionFactory().openSession();
      events = session.createCriteria(Event.class).add(Restrictions.eq("isDeleted", new Boolean("false"))).addOrder(Order.asc("date")).list();
    } catch (Exception e) {
      JOptionPane.showMessageDialog(null, e.getMessage(), "Error I/O", JOptionPane.OK_OPTION);
    } finally {
      if (session != null && session.isOpen()) {
  ...
  @Override public List<Event> getFutureEvents() {
    Session session = null;
  ...
  @Override public List<Event> getFutureBookableEvents() {
    Session session = null;
    List<Event> events = new ArrayList<Event>();
    try {
      session = HibernateUtil.getSessionFactory().openSession();
  ...
  @Override public Event getEventByDate(Date date) {
    Session session = null;
    Event event = null;
    try {
      session = HibernateUtil.getSessionFactory().openSession();
      event = (Event) session.createCriteria(Event.class).add(Restrictions.eq("date", date));
    } catch (Exception e) {
      JOptionPane.showMessageDialog(null, e.getMessage(), "Error I/O", JOptionPane.OK_OPTION);
    } finally {
      if (session != null && session.isOpen()) {
