...
public class JettyAggregatedRunMojo extends AbstractEmbeddedJettyMojo {
  ...
  @Override public void deployWebApplications() throws Exception {
    scanners.clear();
    ...
    for (MavenProject subProject : session.getProjects()) {
      if ("war".equals(subProject.getPackaging())) {
        ...
        List<File> files = scanBuilder.setupScannerFiles(webAppConfig, Arrays.asList(subProject.getFile()), Collections.<String>emptyList());
  ...
  private void configureWarArtifactsForExtraContextHandlers() throws Exception {
  ...
  private MavenProject getLocalDownstreamProjectForDependency(final Artifact artifact, final MavenProject topProject) {
    ...
    for (MavenProject mavenProject : downstreamProjects) {
      if (mavenProject.getPackaging().equals("jar")) {
        if (artifact.getArtifactId().equals(mavenProject.getArtifactId()) && artifact.getGroupId().equals(mavenProject.getGroupId()) && artifact.getVersion().equals(mavenProject.getVersion())) {
          return mavenProject;
        }
      }
    }
  ...
  @Override public void restartWebApplications(boolean reconfigureScanner) throws Exception {
