...
public class SurefireParser {
  ...
  public Resource getUnitTestResource(String classname) {
    ...
    InputFile inputFile = fileSystem.inputFile(fileSystem.predicates().or(fileSystem.predicates().matchesPathPattern("**/" + fileName), fileSystem.predicates().matchesPathPattern("**/" + fileName.replace("_", "+"))));
    if (inputFile == null) {
      return null;
    }
