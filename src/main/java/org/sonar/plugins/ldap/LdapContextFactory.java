...
public class LdapContextFactory {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(LdapContextFactory.class);
	...
	public LdapContextFactory(Settings settings, String ldapIndex) {
		this.authentication = StringUtils.defaultString(
				settings.getString(ldapIndex + ".authentication"),
				DEFAULT_AUTHENTICATION);
		this.factory = StringUtils.defaultString(
				settings.getString(ldapIndex + ".contextFactoryClass"),
				DEFAULT_FACTORY);
	...
	private InitialDirContext createInitialDirContext(String principal,
			String credentials, boolean pooling) throws NamingException {
		return new InitialLdapContext(getEnvironment(principal, credentials,
				pooling), null);
	}

	private Properties getEnvironment(String principal, String credentials,
			boolean pooling) {
...
>>>>>>> /usr/src/app/output/sonarcommunity/sonar-ldap/245133355f897caada1272aff129ffe56d3ad4a2/src/main/java/org/sonar/plugins/ldap/LdapContextFactory.java/right.java
	...
	public String toString() {
		return Objects.toStringHelper(this).add("url", providerUrl)
				.add("authentication", authentication).add("factory", factory)
				.add("bindDn", username).add("realm", realm).toString();
	}
