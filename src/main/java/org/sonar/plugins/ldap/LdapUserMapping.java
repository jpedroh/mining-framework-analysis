...
public class LdapUserMapping {
	...
	public LdapUserMapping(Settings settings, String ldapIndex) {
		...
		String objectClass = settings
				.getString(ldapIndex + ".user.objectClass");
		String loginAttribute = settings.getString(ldapIndex
				+ ".user.loginAttribute");

		this.baseDn = usersBaseDn;
		this.realNameAttribute = StringUtils.defaultString(
				settings.getString(ldapIndex + ".user.realNameAttribute"),
				DEFAULT_NAME_ATTRIBUTE);
		this.emailAttribute = StringUtils.defaultString(
				settings.getString(ldapIndex + ".user.emailAttribute"),
				DEFAULT_EMAIL_ATTRIBUTE);

		String req;
		if (StringUtils.isNotBlank(objectClass)
				|| StringUtils.isNotBlank(loginAttribute)) {
			objectClass = StringUtils.defaultString(objectClass,
					DEFAULT_OBJECT_CLASS);
			loginAttribute = StringUtils.defaultString(loginAttribute,
					DEFAULT_LOGIN_ATTRIBUTE);
			req = "(&(objectClass=" + objectClass + ")(" + loginAttribute
					+ "={login}))";
			...
			LoggerFactory
					...
					.warn("Properties 'ldap.user.objectClass' and 'ldap.user.loginAttribute' are deprecated"
							+ " and should be replaced by single property 'ldap.user.request' with value: "
							+ req);
		} else {
			req = StringUtils.defaultString(
					settings.getString(ldapIndex + ".user.request"),
					DEFAULT_REQUEST);
		}
	...
	public LdapSearch createSearch(LdapContextFactory contextFactory,
		...
		return new LdapSearch(contextFactory).setBaseDn(getBaseDn())
				.setRequest(getRequest()).setParameters(username);
	}
...
>>>>>>> /usr/src/app/output/sonarcommunity/sonar-ldap/245133355f897caada1272aff129ffe56d3ad4a2/src/main/java/org/sonar/plugins/ldap/LdapUserMapping.java/right.java
	...
	public String toString() {
		return Objects.toStringHelper(this).add("baseDn", getBaseDn())
				.add("request", getRequest())
				.add("realNameAttribute", getRealNameAttribute())
				.add("emailAttribute", getEmailAttribute()).toString();
	}
