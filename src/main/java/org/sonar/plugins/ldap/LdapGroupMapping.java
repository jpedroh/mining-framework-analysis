...
public class LdapGroupMapping {
  ...
  public LdapGroupMapping(Settings settings, String ldapIndex) {
    ...
    if (StringUtils.isNotBlank(objectClass) || StringUtils.isNotBlank(memberAttribute)) {
      ...
      LoggerFactory.getLogger(LdapGroupMapping.class).warn("Properties \'ldap.group.objectClass\' and \'ldap.group.memberAttribute\' are deprecated" + " and should be replaced by single property \'ldap.group.request\' with value: " + req);
    } else {
  ...
  @Override public String toString() {
    return Objects.toStringHelper(this).add("baseDn", getBaseDn()).add("idAttribute", getIdAttribute()).add("requiredUserAttributes", Arrays.toString(getRequiredUserAttributes())).add("request", getRequest()).toString();
  }
}