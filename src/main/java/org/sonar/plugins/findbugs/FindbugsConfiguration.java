...
@ScannerSide public class FindbugsConfiguration implements Startable {
  ...
  public void initializeFindbugsProject(Project findbugsProject) throws IOException {
    ...
    if (classFilesToAnalyze.isEmpty()) {
      ...
      if (!isAllowUncompiledCode() && hasSourceFiles()) {
        throw new IllegalStateException(format("One (sub)project contains Java source files that are not compiled (%s).", fileSystem.baseDir().getPath()));
      }
    }
    for (File classToAnalyze : classFilesToAnalyze) {
      String absolutePath = classToAnalyze.getCanonicalPath();
  ...
  private boolean hasSourceFiles() {
    ...
    return fileSystem.hasFiles(pred.and(pred.hasType(Type.MAIN), pred.or(FindbugsPlugin.getSupportedLanguagesFilePredicate(pred)), pred.not(pred.matchesPathPattern("**/package-info.java")), pred.not(pred.matchesPathPattern("**/module-info.java")), pred.not(pred.matchesPathPattern("**/*.jsp")), pred.not(pred.matchesPathPattern("**/*.scala"))));
  }
    ...
    while (!dirs.isEmpty()) {
      ...
      for (File f : dirPoll.listFiles()) {
        if (f.isDirectory()) {
          ...
          if (isPrecompiledJspClassFile(f)) {
            allFiles.add(f);
          }
        }
      }
   ...
   */
  @SuppressWarnings(value = { "RV_RETURN_VALUE_IGNORED_BAD_PRACTICE" }) @Override public void stop() {
    if (jsr305Lib != null) {
      jsr305Lib.delete();
    }
    if (annotationsLib != null) {
