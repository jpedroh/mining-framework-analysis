...
public class SwitchProcessor {
  ...
  public List<TocEntry> processSwitches(WordprocessingMLPackage wordMLPackage, List<P> pList, List<SwitchInterface> switchesList, Map<P, ResultTriple> pNumbersMap) {
    ...
    for (P p : pList) {
      ...
      if (proceed) {
        ...
        if (entry.getEntryValue() == null || entry.getEntryValue().size() == 0) {
