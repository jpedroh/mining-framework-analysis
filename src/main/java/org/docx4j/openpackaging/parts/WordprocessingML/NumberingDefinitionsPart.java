...
public final class NumberingDefinitionsPart extends JaxbXmlPartXPathAware<Numbering> {
  ...
  public void init() {
    setContentType(new org.docx4j.openpackaging.contenttype.ContentType(org.docx4j.openpackaging.contenttype.ContentTypes.WORDPROCESSINGML_NUMBERING));
  ...
  public HashMap<String, AbstractListNumberingDefinition> getAbstractListDefinitions() {
    if (abstractListDefinitions == null) {
      initialiseMaps();
    }
  ...
  public HashMap<String, ListNumberingDefinition> getInstanceListDefinitions() {
    if (instanceListDefinitions == null) {
      initialiseMaps();
    }
  ...
  public void initialiseMaps() {
    ...
    abstractListDefinitions = new HashMap<String, AbstractListNumberingDefinition>(numbering.getAbstractNum().size());
    instanceListDefinitions = new HashMap<String, ListNumberingDefinition>(numbering.getNum().size());
    ...
    for (Numbering.Num numNode : numbering.getNum()) {
      ListNumberingDefinition listDef = new ListNumberingDefinition(numNode, abstractListDefinitions);
  ...
  public void resolveLinkedAbstractNum(StyleDefinitionsPart sdp) {
    ...
    for (Numbering.AbstractNum abstractNum : getJaxbElement().getAbstractNum()) {
      if (abstractNum.getNumStyleLink() == null) {
        continue;
      }
  ...
  public Numbering unmarshalDefaultNumbering() throws JAXBException {
    ...
    try {
      is = ResourceUtils.getResourceViaProperty("docx4j.openpackaging.parts.WordprocessingML.NumberingDefinitionsPart.DefaultNumbering", "org/docx4j/openpackaging/parts/WordprocessingML/numbering.xml");
    } catch (IOException e) {
      e.printStackTrace();
    }
    return unmarshal(is);
