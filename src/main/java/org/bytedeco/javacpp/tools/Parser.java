...
public class Parser {
    ...
    for (Token token = tokens.get(); !token.match(Token.EOF); token = tokens.next()) {
      if (token.match("::")) {
        ...
        if (token.match('<')) {
          ...
          if (token.match(Token.CONST, Token.CONSTEXPR)) {
                    ...
                    if (token.match(Token.OPERATOR)) {
                      ...
                      if (token.match(Token.FRIEND)) {
                        ...
                        if (token.match(Token.VIRTUAL)) {
                          ...
                          if (token.match(Token.ENUM, Token.EXPLICIT, Token.EXTERN, Token.INLINE, Token.CLASS, Token.INTERFACE, Token.__INTERFACE, Token.NAMESPACE, Token.STRUCT, Token.UNION, Token.TYPEDEF, Token.TYPENAME, Token.USING)) {
                            continue;
                          } else {
                            if (token.match((Object[]) infoMap.getFirst("basic/types").cppTypes) && (type.cppName.length() == 0 || type.simple)) {
  ...
  Declarator declarator(Context context, String defaultName, int infoNumber, boolean useDefaults, int varNumber, boolean arrayAsPointer, boolean pointerAsArray) throws ParserException {
    ...
    if (!valueType) {
      if (dcl.indirections == 0 && !dcl.reference) {
        ...
        if (dcl.indirections == 0 && dcl.reference) {
          ...
          if (dcl.indirections == 1 && dcl.reference) {
            type.annotations += "@ByPtrRef ";
          } else {
            if (dcl.indirections == 2 && !dcl.reference && infoNumber >= 0) {
      ...
      if (indirections2 == 0 && !typedef) {
        ...
        if (info != null && info.pointerTypes.length > 0) {
          ...
          if (typedef) {
            ...
            if (dcl.parameters.signature.length() > 0) {
    ...
    if (dcl.cppName != null) {
      ...
      if (namespace >= 0 && context.namespace != null && context.namespace.startsWith(localName.substring(0, namespace - 2))) {
    ...
    for (int n = 0; n < Integer.MAX_VALUE; n++) {
      ...
      if (metadcl == null || metadcl.indices == 0 || dcl.indices == 0) {
        ...
        if (metadcl != null && metadcl.cppName.length() > 0) {
          decl.text += metadcl.indices == 0 ? "@Name(\"" + metadcl.cppName + "." + shortName + "\") " : "@Name({\"" + metadcl.cppName + "\", \"." + shortName + "\"}) ";
  ...
  boolean group(Context context, DeclarationList declList) throws ParserException {
      ...
      if (typedef) {
        for (Token prevToken = null, token = tokens.get(); !token.match(Token.EOF); prevToken = token, token = tokens.next()) {
          if (token.match(';')) {
            ...
            if ((prevToken == null || prevToken.match('}', ',')) && token.match(Token.IDENTIFIER)) {
    ...
    boolean implicitConstructor = true, defaultConstructor = false, intConstructor = false, pointerConstructor = false, abstractClass = info != null && info.purify && !ctx.virtualize, havePureConst = false, haveVariables = false;
  ...
  public File parse(File outputDirectory, String[] classPath, Class cls) throws IOException, ParserException {
    ...
    text += "public class " + target.substring(n + 1) + " extends " + (clsHelpers.size() > 0 ? clsHelpers.get(0) : cls.getCanonicalName()) + " {\n" + "    static { Loader.load(); }\n";
