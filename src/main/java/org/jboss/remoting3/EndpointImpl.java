...
final class EndpointImpl extends AbstractHandleableCloseable<Endpoint> implements Endpoint {
  ...
  @SuppressWarnings(value = { "unused" }) private volatile int resourceCount = 0;
  ...
  @SuppressWarnings(value = { "unused" }) private final OptionMap optionMap;
  ...
  void executorUntick(Object opened) {
    ...
    if (log.isTraceEnabled()) {
      log.tracef(
<<<<<<< /usr/src/app/output/jboss-remoting/jboss-remoting/dd7f776d66ed721f4e2f6237137d69faa3d1b703/src/main/java/org/jboss/remoting3/EndpointImpl.java/left.java
      "Allocated tick to %d of %s (opened %s)"
=======
      "Allocated tick to %08x of %s (opened %s)"
>>>>>>> /usr/src/app/output/jboss-remoting/jboss-remoting/dd7f776d66ed721f4e2f6237137d69faa3d1b703/src/main/java/org/jboss/remoting3/EndpointImpl.java/right.java
      , Integer.valueOf(old + 1), this, opened);
    }
  }
  ...
  public IoFuture<Connection> connect(final URI destination) throws IOException {
    ...
    if (uriUserName != null) {
      builder.set(RemotingOptions.AUTHORIZE_ID, uriUserName);
    }
    if (uriUserRealm != null) {
      builder.set(RemotingOptions.AUTH_REALM, uriUserRealm);
    }
  ...
  public IoFuture<Connection> connect(final URI destination, final OptionMap connectOptions, final String userName, final String realmName, final char[] password, final XnioSsl xnioSsl) throws IOException {
    ...
    if (actualUserName != null) {
      builder.set(RemotingOptions.AUTHORIZE_ID, actualUserName);
    }
    if (actualUserRealm != null) {
      builder.set(RemotingOptions.AUTH_REALM, actualUserRealm);
    }
  ...
  public IoFuture<Connection> connect(final String protocol, final SocketAddress bindAddress, final SocketAddress destination, final OptionMap connectOptions, String userName, String realmName, final char[] password, final XnioSsl xnioSsl) throws IOException {
    final OptionMap.Builder builder = OptionMap.builder().addAll(connectOptions);
    if (userName != null) {
      builder.set(RemotingOptions.AUTHORIZE_ID, userName);
    } else {
      userName = optionMap.get(RemotingOptions.AUTHORIZE_ID);
    }
    if (realmName != null) {
      builder.set(RemotingOptions.AUTH_REALM, realmName);
    } else {
      realmName = optionMap.get(RemotingOptions.AUTH_REALM);
    }
    final OptionMap finalMap = builder.getMap();
  ...
  public <T extends java.lang.Object> T getConnectionProviderInterface(final String uriScheme, final Class<T> expectedType) throws UnknownURISchemeException, ClassCastException {
    final SecurityManager sm = System.getSecurityManager();
    if (sm != null) {
  ...
  private class MapRegistration<T extends java.lang.Object> extends AbstractHandleableCloseable<Registration> implements Registration {
  ...
  final class LocalConnectionContext implements ConnectionHandlerContext {
    ...
    @Deprecated public OpenListener getServiceOpenListener(final String serviceType) {
    ...
    public void accept(final ConnectionHandlerFactory connectionHandlerFactory) {
      synchronized (connectionLock) {
        ...
        try {
          ...
          if (!ok) {
            closeTick1("a failed inbound connection");
          }
        }
      }
    }
