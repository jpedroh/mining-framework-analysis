...
final class HttpUpgradeConnectionProvider extends RemoteConnectionProvider {
  ...
  @Override protected IoFuture<ConnectedStreamChannel> createConnection(final SocketAddress bindAddress, final SocketAddress dst, final OptionMap connectOptions, final ChannelListener<ConnectedStreamChannel> openListener) {
    ...
    }, null, connectOptions, new RemotingHandshakeChecker(secKey)).addNotifier(new IoFuture.Notifier<StreamConnection, Object>() {
      @Override public void notify(final IoFuture<? extends StreamConnection> ioFuture, final Object attachment) {
  ...
  @Override protected IoFuture<ConnectedSslStreamChannel> createSslConnection(final SocketAddress bindAddress, final InetSocketAddress destination, final OptionMap options, final XnioSsl xnioSsl, final ChannelListener<ConnectedStreamChannel> openListener) {
    final URI uri;
    try {
    ...
    HttpUpgrade.performUpgrade(getXnioWorker(), xnioSsl, (InetSocketAddress) bindAddress, uri, headers, new ChannelListener<SslConnection>() {
      @Override public void handleEvent(final SslConnection channel) {
    ...
    }, null, modifiedOptions, new RemotingHandshakeChecker(secKey)).addNotifier(new IoFuture.Notifier<SslConnection, Object>() {
      @Override public void notify(final IoFuture<? extends SslConnection> ioFuture, final Object attachment) {
        if (ioFuture.getStatus() == IoFuture.Status.FAILED) {
          future.setException(ioFuture.getException());
        }
      }
  ...
  private class RemotingHandshakeChecker implements HandshakeChecker {
    ...
    @Override public void checkHandshake(final Map<String, String> headers) throws IOException {
  ...
  final class ProviderInterface implements ExternalConnectionProvider {
    @Override public ConnectionAdaptor createConnectionAdaptor(final OptionMap optionMap, final ServerAuthenticationProvider authenticationProvider) throws IOException {
  ...
  private final class ConnectionAdaptorImpl implements ExternalConnectionProvider.ConnectionAdaptor {
    ...
    @Override public void adapt(final ConnectedStreamChannel channel) {
      ...
      if (RemoteConnectionProvider.LEAK_DEBUGGING) {
        messageBufferPool = new DebuggingBufferPool(messageBufferPool);
      }
    ...
    static {
      ...
      try {
        PrivilegedExceptionAction<Constructor<String>> runnable = new PrivilegedExceptionAction<Constructor<String>>() {
          @Override public Constructor<String> run() throws Exception {
