...
final class ServiceControllerImpl<S extends java.lang.Object> implements ServiceController<S>, Dependent {
  ...
  /**
     ...
     * initiated.
     */
  @SuppressWarnings(value = { "VolatileLongOrDoubleField" }) private volatile long lifecycleTime;
  ...
  private Transition getTransition() {
    ...
    switch (state) {
      case DOWN:
      {
        if (mode == ServiceController.Mode.REMOVE) {
          ...
          if (mode == ServiceController.Mode.NEVER) {
            ...
            if (shouldStart() && (mode != Mode.PASSIVE || stoppingDependencies == 0)) {
      ...
      case WAITING:
      {
        if (((mode != Mode.ON_DEMAND && mode != Mode.LAZY) || demandedByCount > 0) && (mode != Mode.PASSIVE || stoppingDependencies == 0)) {
  ...
  @Override public void immediateDependencyAvailable(ServiceName dependencyName) {
  ...
  @Override public void immediateDependencyUnavailable(ServiceName dependencyName) {
    final ArrayList<Runnable> tasks;
    synchronized (this) {
      final boolean leavingRestState = isStableRestState();
  ...
  @Override public void transitiveDependencyAvailable() {
    final ArrayList<Runnable> tasks;
    synchronized (this) {
      final boolean leavingRestState = isStableRestState();
  ...
  @Override public void transitiveDependencyUnavailable() {
    final ArrayList<Runnable> tasks;
    synchronized (this) {
      final boolean leavingRestState = isStableRestState();
  ...
  @Override public void immediateDependencyUp() {
    final ArrayList<Runnable> tasks;
    synchronized (this) {
      final boolean leavingRestState = isStableRestState();
      if (--stoppingDependencies != 0) {
  ...
  @Override public void immediateDependencyDown() {
    final ArrayList<Runnable> tasks;
    synchronized (this) {
      final boolean leavingRestState = isStableRestState();
  ...
  @Override public void dependencyFailed() {
    final ArrayList<Runnable> tasks;
    synchronized (this) {
      final boolean leavingRestState = isStableRestState();
  ...
  @Override public void dependencyFailureCleared() {
    final ArrayList<Runnable> tasks;
    synchronized (this) {
      final boolean leavingRestState = isStableRestState();
  ...
  @Override public synchronized Set<ServiceName> getImmediateUnavailableDependencies() {
  ...
  ServiceStatus getStatus() {
    synchronized (this) {
      ...
      return new ServiceStatus(parentName, name, aliases, serviceClass, mode.name(), state.getState().name(), state.name(), dependencyNames, failCount != 0, startException != null ? startException.toString() : null, !immediateUnavailableDependencies.isEmpty() || transitiveUnavailableDepCount != 0);
    }
  }
  ...
  private Dependent[][] getDependents() {
    ...
    if (aliasRegistrations.length == 0) {
      synchronized (dependentSet) {
        return new Dependent[][] { dependentSet.toScatteredArray(NO_DEPENDENTS), children.toScatteredArray(NO_DEPENDENTS) };
      }
    }
  ...
  private static <T extends java.lang.Object> void doInject(final ValueInjection<T> injection) {
    ...
    public void asynchronous() throws IllegalStateException {
      synchronized (ServiceControllerImpl.this) {
        if (state == ContextState.SYNC) {
          ...
          if (state == ContextState.SYNC_ASYNC_COMPLETE) {
            state = ContextState.COMPLETE;
          } else {
            if (state == ContextState.SYNC_ASYNC_FAILED) {
              state = ContextState.FAILED;
            } else {
              if (state == ContextState.ASYNC) {
                throw new IllegalStateException(ILLEGAL_CONTROLLER_STATE);
  ...
  private final class ChildServiceTarget extends ServiceTargetImpl {
    ...
    <T extends java.lang.Object> ServiceController<T> install(final ServiceBuilderImpl<T> serviceBuilder) throws ServiceRegistryException {
      if (!valid) {
    ...
    protected <T extends java.lang.Object> ServiceBuilder<T> createServiceBuilder(final ServiceName name, final Value<? extends Service<T>> value, final ServiceControllerImpl<?> parent) throws IllegalArgumentException {
      return super.createServiceBuilder(name, value, ServiceControllerImpl.this);
    }

    @Override public ServiceTarget subTarget() {
