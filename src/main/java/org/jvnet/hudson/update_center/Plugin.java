...
public class Plugin {
  ...
  @VisibleForTesting static String requireTopLevelUrl(String scmUrl) {
    if (scmUrl == null) {
      return null;
    }
  ...
  private String _getScmUrl() {
    try {
      ...
      if (scm == null) {
        ...
        if (parent != null) {
          try {
            File parentPomFile = latest.repository.resolve(new ArtifactInfo("", parent.element("groupId").getTextTrim(), parent.element("artifactId").getTextTrim(), parent.element("version").getTextTrim(), ""), "pom", null);
  ...
  public List<String> getLabels() throws IOException {
    ...
    if (scm != null && scm.contains("https://github.com/")) {
      ...
      if (parts.length >= 2) {
        unsanitizedLabels.addAll(Arrays.asList(GitHubSource.getInstance().getRepositoryTopics(parts[0], parts[1]).toArray(new String[0])));
      }
  ...
  @VisibleForTesting static String simplifyPluginName(String name) {
