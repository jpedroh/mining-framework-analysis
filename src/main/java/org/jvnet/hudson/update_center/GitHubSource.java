...
<<<<<<< /usr/src/app/output/jenkinsci/backend-update-center2/e2a2e33ea822cf3999e642f26bf9d1b17be958de/src/main/java/org/jvnet/hudson/update_center/GitHubSource.java/left.java
        getRepositoryData("jenkinsci")
=======
        getOrganizationTopics("jenkinsci")
>>>>>>> /usr/src/app/output/jenkinsci/backend-update-center2/e2a2e33ea822cf3999e642f26bf9d1b17be958de/src/main/java/org/jvnet/hudson/update_center/GitHubSource.java/right.java
  ...
  private void getRepositoryData(String organization) throws IOException {
    ...
    Cache cache = new Cache(GITHUB_API_CACHE, 20L * 1024 * 1024);
    ...
    while (hasNextPage) {
      ...
      jsonObject.put("query", String.format("{\n" + "  organization(login: %s) {\n" + "    repositories(first: 100, after: %s) {\n" + "      pageInfo {\n" + "        startCursor\n" + "        hasNextPage\n" + "        endCursor\n" + "      }\n" + "      edges {\n" + "        node {\n" + "          name\n" + "          hasIssuesEnabled\n" + "          repositoryTopics(first:100) {\n" + "            edges {\n" + "              node {\n" + "                topic {\n" + "                  name\n" + "                }\n" + "              }\n" + "            }\n" + "          }\n" + "        }\n" + "      }\n" + "    }\n" + "  }\n" + "}\n", "\"" + organization.replace("\"", "\\\"") + "\"", endCursor == null ? "null" : "\"" + endCursor.replace("\"", "\\\"") + "\""));
      System.err.println(String.format("Retrieving GitHub topics with end token... %s", endCursor));
      Request request = new Request.Builder().url(this.getGraphqlUrl()).post(RequestBody.create(MediaType.parse("application/json; charset=utf-8"), jsonObject.toString())).build();
      ...
      for (Object repository : repositories.getJSONArray("edges")) {
        ...
        for (Object repositoryTopic : node.getJSONObject("repositoryTopics").getJSONArray("edges")) {
          this.topicNames.get(name).add(((JSONObject) repositoryTopic).getJSONObject("node").getJSONObject("topic").getString("name"));
        }
      }
    }
  }
  ...
  protected Map<String, List<String>> getOrganizationTopics(String organization) throws IOException {
    ...
    while (hasNextPage) {
      ...
      for (Object repository : repositories.getJSONArray("edges")) {
        ...
        for (Object repositoryTopic : node.getJSONObject("repositoryTopics").getJSONArray("edges")) {
          this.topicNames.get(organization + "/" + name).add(((JSONObject) repositoryTopic).getJSONObject("node").getJSONObject("topic").getString("name"));
        }
      }
    }
    return this.topicNames;
