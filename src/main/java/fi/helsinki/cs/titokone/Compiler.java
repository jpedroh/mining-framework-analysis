...
public class Compiler {
  ...
  public Application getApplication() throws IllegalStateException {
    if (compileFinished) {
      ...
      if (!defStdin.equals("")) {
        st.addDefinition("stdin", defStdin);
      }
      if (!defStdout.equals("")) {
        st.addDefinition("stdout", defStdout);
      }
  ...
  private CompileInfo firstRoundProcess(String line) throws TTK91CompileException {
    ...
    try {
      ...
      if (invalidLabels.containsKey(lineTemp[0])) {
        if (!(lineTemp[1].equalsIgnoreCase("def") && (lineTemp[0].equalsIgnoreCase("stdin") || lineTemp[0].equalsIgnoreCase("stdout")))) {
      ...
      if (!validLabelName(lineTemp[0])) {
        ...
        if (lineTemp[1].equalsIgnoreCase("dc")) {
          ...
          if (lineTemp[2].trim().length() > 0) {
            try {
              ...
              comment = new Message("Invalid value for a " + "DC.").toString();
              throw new TTK91CompileException(comment);
            }
        ...
        if (lineTemp[1].equalsIgnoreCase("equ")) {
          if (symbols.containsKey(lineTemp[0])) {
            ...
            symbolTable.set(symbols.get(lineTemp[0]).intValue(), symbolTableEntry.clone());
          } else {
          ...
          try {
            ...
            comment = new Message("Invalid value for an " + "EQU.").toString();
            throw new TTK91CompileException(comment);
          }
          if (intValue < MININT || intValue > MAXINT) {
            comment = new Message("Invalid value for an " + "EQU.").toString();
          ...
          comment = new Message("Variable {0} defined as {1}.", commentParameters).toString();
        ...
        if (lineTemp[1].equals("ds")) {
          ...
          comment = new Message("Found variable {0}.", lineTemp[0]).toString();
          compileDebugger.setComment(comment);
        }
        ...
        if (lineTemp[1].equalsIgnoreCase("def")) {
          if (lineTemp[0].equals("stdin") || lineTemp[0].equals("stdout")) {
            ...
            commentParameters[1] = lineTemp[2];
            comment = new Message("{0} defined as {1}.", commentParameters).toString();
            compileDebugger.setComment(comment);
          } else {
            comment = new Message("Invalid DEF " + "operation.").toString();
            throw new TTK91CompileException(comment);
          }
        }
      }
      ...
      if (lineTemp[1].equals("")) {
        ...
        if (!lineTemp[0].equals("")) {
          ...
          if (invalidLabels.containsKey(lineTemp[0])) {
            ...
            compileDebugger.foundLabel(lineTemp[0], code.size() - 1);
          }
        }
        try {
          ...
          if (symbolicInterpreter.getRegisterId(lineTemp[4]) == -1) {
            ...
            if (!symbols.containsKey(lineTemp[4])) {
              if (invalidLabels.get(lineTemp[4]) == null) {
                ...
                symbols.put(lineTemp[4], new Integer(symbolTable.size()));
                symbolTableEntry[0] = lineTemp[4];
                symbolTableEntry[1] = String.valueOf(invalidLabels.get(lineTemp[4]));
                symbolTable.add(symbolTableEntry.clone());
              }
            }
          }
        ...
        if (variableUsed && labelFound) {
          ...
          comment = new Message("Found label {0} and variable " + "{1}.", commentParameters).toString();
          compileDebugger.setComment(comment);
        } else {
          if (variableUsed) {
  ...
  private CompileInfo initializeSecondRound() {
    ...
    String[] lineTemp;
    int dataAreaSize = 0;
    compileDebugger.setStatusMessage(new Message("Initializing the second  round of compilation.").toString());
    for (int i = 0; i < symbolTable.size(); ++i) {
  ...
  private CompileInfo secondRoundProcess(String line) throws TTK91CompileException {
    compileDebugger.setStatusMessage(new Message("Second round of compilation.").toString());
    String address = "";
    int lineAsBinary;
    String comment;
    ...
    lineAsBinary = symbolicInterpreter.stringToBinary(lineTemp[1], lineTemp[2], lineTemp[3], address, lineTemp[5]);
  ...
  public String[] parseLine(String symbolicOpcode) throws TTK91CompileException {
    ...
    if (symbolicInterpreter.getOpcode(wordTemp) == -1) {
      if (!validLabelName(wordTemp)) {
        comment = new Message("Compilation failed: {0}", new Message("invalid label {0}.", wordTemp).toString()).toString();
        throw new TTK91CompileException(comment);
      }
      label = wordTemp;
      ++lineAsArrayIndex;
    ...
    if (lineAsArrayIndex < lineAsArray.length) {
      ...
      if (symbolicInterpreter.getOpcode(opcode) < 0) {
        comment = new Message("Compilation failed: {0}", new Message("invalid opcode {0}.", opcode).toString()).toString();
        throw new TTK91CompileException(comment);
      }
    } else {
      comment = new Message("Compilation failed: {0}", new Message("invalid opcode {0}.", opcode).toString()).toString();
    ...
    if (lineAsArrayIndex < lineAsArray.length) {
      if (lineAsArray[lineAsArrayIndex].indexOf("(") != -1) {
        if (lineAsArray[lineAsArrayIndex].indexOf(")") < lineAsArray[lineAsArrayIndex].indexOf("(")) {
          comment = new Message("Compilation failed: {0}", new Message("second register expected.").toString()).toString();
          throw new TTK91CompileException(comment);
        } else {
          if (spaceBetweenMemorymodeAndAddress) {
            address = lineAsArray[lineAsArrayIndex].substring(0, lineAsArray[lineAsArrayIndex].indexOf("("));
          } else {
            address = lineAsArray[lineAsArrayIndex].substring(addressingMode.length(), lineAsArray[lineAsArrayIndex].indexOf("("));
          }
          if (address.isEmpty()) {
            address = "0";
          }
          secondRegister = lineAsArray[lineAsArrayIndex].substring(lineAsArray[lineAsArrayIndex].indexOf("(") + 1, lineAsArray[lineAsArrayIndex].indexOf(")"));
          if (symbolicInterpreter.getRegisterId(secondRegister) == -1) {
            comment = new Message("Compilation failed: {0}", new Message("invalid register {0}.", secondRegister).toString()).toString();
            throw new TTK91CompileException(comment);
          }
        }
      } else {
        ...
        if (lineAsArrayIndex + 1 < lineAsArray.length) {
          ++lineAsArrayIndex;
          if (lineAsArray[lineAsArrayIndex].indexOf("(") == -1 || lineAsArray[lineAsArrayIndex].indexOf(")") == -1) {
            comment = new Message("Compilation failed: {0}", new Message("second register expected.").toString()).toString();
            throw new TTK91CompileException(comment);
          }
          if (lineAsArray[lineAsArrayIndex].indexOf(")") < lineAsArray[lineAsArrayIndex].indexOf("(")) {
    ...
    if (lineAsArrayIndex < lineAsArray.length) {
      comment = new Message("Compilation failed: {0}", new Message("end of line expected.").toString()).toString();
      throw new TTK91CompileException(comment);
    }
    if (opcode.length() > 0) {
      if (opcode.charAt(0) == 'j' || opcode.charAt(0) == 'J' || opcode.toLowerCase().equals("not")) {
        if (opcode.toLowerCase().matches("j" + "n?" + "((neg)|(zer)|(pos))") || opcode.toLowerCase().equals("not")) {
          if (firstRegister.equals("")) {
            comment = new Message("Compilation failed: {0}", new Message("first register expected.").toString()).toString();
            throw new TTK91CompileException(comment);
          }
        }
        if (((!opcode.toLowerCase().equals("not")) && (addressingMode.equals("=") || address.equals("")))) {
          comment = new Message("Compilation failed: {0}", new Message("address expected.").toString()).toString();
          throw new TTK91CompileException(comment);
        }
      } else {
        if (opcode.equalsIgnoreCase("nop")) {
          ...
          if (opcode.equalsIgnoreCase("pop")) {
            if (addressingMode.equals("@") || addressingMode.equals("=") || !address.equals("")) {
              comment = new Message("Compilation failed: {0}", new Message("invalid argument.").toString()).toString();
              throw new TTK91CompileException(comment);
            }
          } else {
            if (opcode.equalsIgnoreCase("pushr") || opcode.equalsIgnoreCase("popr")) {
              if (firstRegister.equals("")) {
                comment = new Message("Compilation failed: {0}", new Message("first register expected.").toString()).toString();
                throw new TTK91CompileException(comment);
              }
            } else {
              if (firstRegister.equals("") || (address.equals("") && secondRegister.equals(""))) {
                comment = new Message("Compilation failed: {0}", new Message("address or 1 register expected.").toString()).toString();
                throw new TTK91CompileException(comment);
              }
            }
          }
    ...
    if (!address.equals("")) {
      ...
      if (isANumber) {
        if (address.length() > String.valueOf(ADDRESSMIN).length() || Integer.parseInt(address) > ADDRESSMAX || Integer.parseInt(address) < ADDRESSMIN) {
          comment = new Message("Compilation failed: {0}", new Message("invalid address value.").toString()).toString();
          throw new TTK91CompileException(comment);
        }
      }
    }
    ...
    if (opcode.equalsIgnoreCase("store") && address.equals("") && !addressingMode.equals("@")) {
      comment = new Message("Compilation failed: {0}", new Message("address expected.").toString()).toString();
      throw new TTK91CompileException(comment);
    }
    if (opcode.equalsIgnoreCase("store") && addressingMode.equals("=")) {
      comment = new Message("Compilation failed: {0}", new Message("invalid addressing mode {0}.", "=").toString()).toString();
      throw new TTK91CompileException(comment);
    }
    if (opcode.equals("") && (!label.equals("") || !firstRegister.equals("") || !addressingMode.equals("") || !address.equals("") || !secondRegister.equals(""))) {
      comment = new Message("Compilation failed: {0}", new Message("opcode missing.", "=").toString()).toString();
      throw new TTK91CompileException(comment);
    }
    parsedLine = new String[6];
  ...
  public String[] parseCompilerCommandLine(String line) throws TTK91CompileException {
    ...
    if (fieldEnd != -1) {
      line = line.substring(0, fieldEnd);
    }
    line = line.trim();
    fieldEnd = line.indexOf(" ");
    if (fieldEnd == -1) {
    ...
    if (nextToCheck < line.length()) {
      ...
      if (opcode.equals("def")) {
        if (value.length() < 1) {
          comment = new Message("Compilation failed: {0}", new Message("invalid value {0}.", value).toString()).toString();
          throw new TTK91CompileException(comment);
        }
      } else {
        if (value.length() > 0) {
          try {
            ...
            if (opcode.equalsIgnoreCase("ds") && intValue < 1) {
              comment = new Message("Compilation failed: {0}", new Message("invalid value for a DS {0}.", value).toString()).toString();
              throw new TTK91CompileException(comment);
            }
          } catch (NumberFormatException e) {
            comment = new Message("Compilation failed: {0}", new Message("invalid value {0}.", value).toString()).toString();
      ...
      comment = new Message("Compilation failed: {0}", new Message("value expected.").toString()).toString();
      throw new TTK91CompileException(comment);
    }
