...
public class Weka_Segmentation implements PlugIn {
                ...
                if (e.getSource() == probabilityButton) {
                  ...
                  if (e.getSource() == plotButton) {
                    ...
                    if (e.getSource() == applyButton) {
                      ...
                      if (e.getSource() == loadClassifierButton) {
                        loadClassifier();
                        win.updateButtonsEnabling();
                      } else {
                        if (e.getSource() == saveClassifierButton) {
      ...
      if (null != sliceSelector) {
        ...
        KeyListener keyListener = new KeyListener() {
          ...
          @Override public void keyReleased(final KeyEvent e) {
            exec.submit(new Runnable() {
              public void run() {
                if (e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == KeyEvent.VK_RIGHT || e.getKeyCode() == KeyEvent.VK_LESS || e.getKeyCode() == KeyEvent.VK_GREATER || e.getKeyCode() == KeyEvent.VK_COMMA || e.getKeyCode() == KeyEvent.VK_PERIOD) {
                  displayImage.killRoi();
                  updateExampleLists();
                  drawExamples();
                  if (showColorOverlay) {
  ...
  void listSelected(final ItemEvent e, final int i) {
    ...
    for (int j = 0; j < wekaSegmentation.getNumOfClasses(); j++) {
      if (j == i) {
        final Roi newRoi = wekaSegmentation.getExamples(i, displayImage.getCurrentSlice()).get(exampleList[i].getSelectedIndex());
  ...
  void updateClassColor(final int i) {
    Color newColor = JColorChooser.showDialog(addExampleButton[i], "Choose Color", colors[i]);
    if (null != newColor) {
      colors[i] = newColor;
      exampleList[i].setForeground(colors[i]);
  ...
  void deleteSelected(final ActionEvent e) {
    for (int i = 0; i < wekaSegmentation.getNumOfClasses(); i++) {
      if (e.getSource() == exampleList[i]) {
        ...
        if (displayImage.getRoi().equals(wekaSegmentation.getExamples(i, displayImage.getCurrentSlice()).get(index))) {
          displayImage.killRoi();
        }
        wekaSegmentation.deleteExample(i, displayImage.getCurrentSlice(), index);
        exampleList[i].remove(index);
        String[] arg = new String[] { Integer.toString(i), Integer.toString(displayImage.getCurrentSlice()), Integer.toString(index) };
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "deprecation" }) void runStopTraining(final String command) {
      ...
      Thread newTask = new Thread() {
        public void run() {
          ...
          try {
            ...
            err.printStackTrace();
            IJ.log("ERROR: plugin run out of memory. Please, " + "use a smaller input image or fewer features.");
          } finally {
            trainingFlag = false;
  ...
  void showProbabilityImage() {
    ...
    if (null != probImage) {
      probImage.setDimensions(numOfClasses, displayImage.getNSlices(), displayImage.getNFrames());
  ...
  void plotResult() {
    ...
    if (null == data) {
      IJ.error("Error in plot result", "No data available yet to plot results: you need to trace\n" + "some training samples or load data from file.");
      win.updateButtonsEnabling();
      return;
    }
  ...
  public void applyClassifierToTestData() {
    ...
    if (Prefs.useFileChooser) {
      ...
      final FileDialog fd = new FileDialog(parent, "Select file(s) to classify", FileDialog.LOAD);
      fd.setDirectory(dir);
    ...
    if (imageFiles.length >= 3) {
      ...
      if (decision == JOptionPane.YES_OPTION) {
        final DirectoryChooser dc = new DirectoryChooser("Select folder to store results");
      ...
      public void run() {
        for (int i = numThread; i < imageFiles.length; i += numThreads) {
          ...
          if (testImage.getNSlices() == 1 && isProcessing3D) {
            IJ.log("Error: " + file.getPath() + " is a 2D image but Trainable Weka Segmentation " + "is working in 3D.");
            IJ.error("Wrong image dimensions: " + file.getPath() + " is a 2D image but " + "Trainable Weka Segmentation is working in 3D.");
            return;
          }
          ...
          if (null == segmentation) {
            IJ.log("Error: " + file.getName() + "could not be " + "classified!");
            return;
          }
          if (!probabilityMaps) {
  ...
  public boolean showSettingsDialog() {
    ...
    gd.addButton("Save feature stack", new SaveFeatureStackButtonListener("Select location to save feature stack", wekaSegmentation));
    ...
    if (featuresChanged) {
      ...
      if (!wekaSegmentation.getFeatureStackArray().isEmpty() && wekaSegmentation.getFeatureStackArray().getReferenceSliceIndex() != -1) {
    ...
    if (iw instanceof CustomWindow) {
      ...
      if (null == classifiedImage) {
        if (win.trainingComplete) {
          ...
          IJ.log("Result image could not be created: " + " you need to train or load a classifier first.");
          return;
        }
      }
      final ImagePlus resultImage = classifiedImage.duplicate();
  ...
  public static void applyClassifier(String dir, String fileName, String showResultsFlag, String storeResultsFlag, String probabilityMapsFlag, String storeDir) {
    ...
    if (iw instanceof CustomWindow) {
      ...
      if (null == testImage) {
        IJ.log("Error: " + dir + File.separator + fileName + " could not be opened");
        return;
      }
  ...
  public static void saveFeatureStack(String dir, String fileWithExt) {
    ...
    if (iw instanceof CustomWindow) {
      ...
      for (int i = 0; i < featureStackArray.getSize(); i++) {
        final String fileName = dir + fileWithExt.substring(0, fileWithExt.length() - 4) + String.format("%04d", (i + 1)) + ".tif";
  ...
  public static void setFeature(String feature) {
    ...
    if (iw instanceof CustomWindow) {
      ...
      for (int i = 0; i < enabledFeatures.length; i++) {
        final String availableFeature = isProcessing3D ? FeatureStack3D.availableFeatures[i] : FeatureStack.availableFeatures[i];
  ...
  public static void disableMissingFeatures(final GenericDialog gd) {
    if (!isImageScienceAvailable()) {
      IJ.log("Warning: ImageScience library unavailable. " + "Some training features will be disabled.");
      @SuppressWarnings(value = { "unchecked" }) final Vector<Checkbox> v = gd.getCheckboxes();
