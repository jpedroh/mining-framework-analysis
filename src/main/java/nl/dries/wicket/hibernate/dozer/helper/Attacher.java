...
public class Attacher {
  ...
  protected Object attach(SimplePropertyDefinition def) {
    SessionImplementor sessionImpl = (SessionImplementor) sessionFinder.getHibernateSession(def.getHibernateProperty().getEntityClass());
  ...
  protected Object attach(CollectionPropertyDefinition def) {
    SessionImplementor sessionImpl = (SessionImplementor) sessionFinder.getHibernateSession(def.getOwner().getClass());
    ...
    collection.setSnapshot(identifier, def.getRole(), null);
    ...
    if (!persistenceContext.containsEntity(key)) {
      ...
      TypeHelper.deepCopy(values, ownPersister.getPropertyTypes(), ownPersister.getPropertyUpdateability(), values, sessionImpl);
      Object version = Versioning.getVersion(values, ownPersister);
      persistenceContext.addEntity(def.getOwner(), (ownPersister.isMutable() ? Status.MANAGED : Status.READ_ONLY), values, key, version, LockMode.NONE, true, ownPersister, false, true);
    }
  ...
  /**
	 ...
	 */
  protected CollectionPersister getCollectionPersister(CollectionPropertyDefinition def, SessionImplementor sessionImpl) {
    SessionFactoryImplementor factory = sessionImpl.getFactory();
