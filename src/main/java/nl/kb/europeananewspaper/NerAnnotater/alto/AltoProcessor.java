  ...
  public static void handlePotentialAltoFile(final URL potentialAltoFilename, final String mimeType, final Locale lang, final ResultHandler[] handler) throws IOException {
    if ("text/xml".equalsIgnoreCase(mimeType) || potentialAltoFilename.getFile().endsWith(".xml")) {
      try {
        ...
        Document doc = Jsoup.parse(potentialAltoFilename.openStream(), "UTF-8", "", Parser.xmlParser());
        Elements elementsByTag = doc.getElementsByTag("alto");
        if (elementsByTag.isEmpty()) {
          System.err.println("Does not seem to be a ALTO file: " + potentialAltoFilename.toExternalForm());
          return;
        }
        ...
        @SuppressWarnings(value = { "unchecked" }) CRFClassifier<CoreMap> classifier = (CRFClassifier<CoreMap>) NERClassifiers.getCRFClassifierForLanguage(lang);
          ...
          for (CoreMap label : classify) {
            if (label.get(HyphenatedLineBreak.class) != null) {
              ...
              if (!label.get(AnswerAnnotation.class).equals("O")) {
                classified += 1;
                for (ResultHandler h : handler) {
                  h.addToken(label.get(AltoStringID.class), label.get(OriginalContent.class), label.get(TextAnnotation.class), label.get(AnswerAnnotation.class), label.get(ContinuationAltoStringID.class));
                }
              } else {
                for (ResultHandler h : handler) {
                  h.addToken(label.get(AltoStringID.class), label.get(OriginalContent.class), label.get(TextAnnotation.class), null, label.get(ContinuationAltoStringID.class));
                }
              }
            }
          }
        ...
        System.out.println();
        System.out.println("Statistics: " + classified + " out of " + totalNumberOfWords + " (" + ((double) classified / (double) totalNumberOfWords) + ") classified");
        System.out.println("Total millisecs: " + (System.currentTimeMillis() - startTime));
      } catch (IOException e) {
