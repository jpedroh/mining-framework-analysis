...
public class App {
	...
	public static void main(final String[] args) throws ClassCastException, ClassNotFoundException, InterruptedException {
		...
		options.addOption(OptionBuilder.withLongOpt("export").withDescription("use FORMAT for export: log (Default), csv, html, db, alto.\n Multiple formats:\" -f html -f csv\"").hasArgs().withArgName("FORMAT").withType(java.lang.String.class).create("f"));
		options.addOption(OptionBuilder.withLongOpt("language").withDescription("use two-letter ISO-CODE for language selection: en, de, nl ....").hasArg().withArgName("ISO-CODE").withType(java.lang.String.class).create("l"));
		options.addOption(OptionBuilder.withLongOpt("container").withDescription("which FORMAT are the container input files: mets (Default), didl").hasArg().withArgName("FORMAT").withType(java.lang.String.class).create("c"));
		options.addOption(OptionBuilder.withLongOpt("nthreads").withDescription("maximum number of threads to be used for processing. Default 8 ").hasArg().withArgName("THREADS").withType(java.lang.Integer.class).create("n"));
		...
		options.addOption(OptionBuilder.withLongOpt("output-directory").withDescription("output DIRECTORY for result files. Default ./output").hasArg().withArgName("DIRECTORY").withType(java.lang.String.class).create("d"));
		try {
