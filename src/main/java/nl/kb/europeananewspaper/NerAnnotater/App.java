...
public class App {
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "static-access" }) public static void main(final String[] args) throws ClassCastException, ClassNotFoundException, InterruptedException {
    CommandLineParser parser = new PosixParser();
    Options options = new Options();
    options.addOption(OptionBuilder.withLongOpt("export").withDescription("use FORMAT for export: log (Default), csv, html, db, alto.\n Multiple formats:\" -f html -f csv\"").hasArgs().withArgName("FORMAT").withType(String.class).create("f"));
    options.addOption(OptionBuilder.withLongOpt("language").withDescription("use two-letter ISO-CODE for language selection: en, de, nl ....").hasArg().withArgName("ISO-CODE").withType(String.class).create("l"));
    options.addOption(OptionBuilder.withLongOpt("container").withDescription("which FORMAT are the container input files: mets (Default), didl, alto").hasArg().withArgName("FORMAT").withType(String.class).create("c"));
    options.addOption(OptionBuilder.withLongOpt("nthreads").withDescription("maximum number of threads to be used for processing. Default 8 ").hasArg().withArgName("THREADS").withType(Integer.class).create("n"));
    options.addOption(OptionBuilder.withLongOpt("models").withDescription("models for languages. Ex. -m de=/path/to/file/model_de.gz -m nl=/path/to/file/model_nl.gz").hasArgs().withArgName("language=filename").withValueSeparator().create("m"));
    options.addOption(OptionBuilder.withLongOpt("output-directory").withDescription("output DIRECTORY for result files. Default ./output").hasArg().withArgName("DIRECTORY").withType(String.class).create("d"));
    try {
      ...
      if (line.getOptionValue("c") != null) {
        ...
        if (line.getOptionValue("c").equals("didl")) {
          ...
          if (line.getOptionValue("c").equals("alto")) {
            processor = AltoLocalProcessor.INSTANCE;
          } else {
            if (line.getOptionValue("c").equals("mets")) {
      ...
      for (String key : results.keySet()) {
        try {
          ...
          System.err.println("Something went wrong during the parsing of container file " + key + " . Cause: " + e.getCause().getMessage());
          e.printStackTrace();
          withErrors += 1;
          errors = true;
        }
