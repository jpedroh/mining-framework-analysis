...
public class TextElementsExtractor {
      ...
      for (Element token : tokens) {
        ...
        for (Element textLineToken : textLineTokens) {
          if (textLineToken.tagName().equalsIgnoreCase("string")) {
            newBlock.add(getWordToLabel(textLineToken, firstSegmentAfterHyphenation));
            firstSegmentAfterHyphenation = false;
          } else {
            if (textLineToken.tagName().equalsIgnoreCase("hyp")) {
              hyphenatedEnd = true;
  ...
  private static CoreLabel getWordToLabel(Element token, Boolean wordSegmentAfterHyphenation) {
    ...
    if (wordSegmentAfterHyphenation) {
      ...
      if (nextSibling != null && nextSibling.tagName().equalsIgnoreCase("hyp")) {
  ...
  public static Element findAltoElementByStringID(Document altoDocument, String id) {
    ...
    if (split.length != 9) {
      logger.warning("id does not seem to have the right format. Has " + split.length + " instead of 9 parameters");
      return null;
    }
    Elements textlines = altoDocument.select("[HPOS=" + split[0] + "]").select("[VPOS=" + split[1] + "]").select("[HEIGHT=" + split[2] + "]").select("[WIDTH=" + split[3] + "]");
