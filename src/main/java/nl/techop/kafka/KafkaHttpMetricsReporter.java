...
public class KafkaHttpMetricsReporter implements KafkaMetricsReporter, KafkaHttpMetricsReporterMBean {
  ...
  @Override public void init(VerifiableProperties verifiableProperties) {
    if (!initialized) {
  ...
  @Override public synchronized void startReporter(long pollingPeriodSecs) {
  ...
  @Override public synchronized void stopReporter() {
    if (initialized && running) {
      metricsServer.stop();
    }
  }

  @Override public String getMBeanName() {
