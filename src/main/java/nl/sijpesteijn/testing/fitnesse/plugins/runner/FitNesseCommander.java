...
public class FitNesseCommander {
  ...
  public final void start() throws MafiaException {
    ...
    final String command = "java" + getJVMArguments(commanderConfig.getJvmArguments()) + " -cp " + commanderConfig.getClasspathString() + " fitnesseMain.FitNesseMain -p " + commanderConfig.getFitNessePort() + " -d " + commanderConfig.getWikiRoot() + " -r " + commanderConfig.getNameRootPage() + " -e " + commanderConfig.getRetainDays() + logArgument + authArgument + updatePreventsArgument;
  ...
  public final void stop() throws MafiaException {
    String authArgument = "";
    if (commanderConfig.getFitNesseAuthenticate() != null && commanderConfig.getFitNesseAuthenticate().contains(":")) {
      String userPasswordArgument = commanderConfig.getFitNesseAuthenticate().replaceFirst(":", " ");
      authArgument = " -c " + userPasswordArgument;
    }
    final String command = "java -cp " + commanderConfig.getClasspathString() + " fitnesse.Shutdown -p " + commanderConfig.getFitNessePort() + authArgument;
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "PMD" }) private void waitForProcess() throws InterruptedException {
    while (true) {
      try {
        process.exitValue();
        return;
      ...
      if (inputMonitor.isFinished()) {
        if (errorMonitor.getBuffer().toString().trim().endsWith("Unpacking new version of FitNesse resources. Please be patient.")) {
