 ...
 */
@Mojo(name = "test", defaultPhase = LifecyclePhase.INTEGRATION_TEST) public class FitNesseRunnerMojo extends AbstractStartFitNesseMojo {
  /**
     * Skip the running of test. Default false.
     */
  @Parameter(property = "mafia.test.skip", defaultValue = "false") private boolean skip;

  /**
     * Start a separate fitnesse instance for testing or test on a remote server. (Specified by protocol & host).
     */
  @Parameter(property = "startServer", defaultValue = "true") private boolean startServer;

  /**
     * The port number on which FitNesse is running the tests.
     */
  @Parameter(property = "fitNesseRunPort", defaultValue = "9091") private int fitNesseRunPort;

  /**
     * The directory where the Fitnesse reports have been generated.
     */
  @Parameter(property = "testResultsDirectory", defaultValue = "mafiaTestResults") private String testResultsDirectory;

  /**
     * List of tests to be run.
     */
  @Parameter(property = "tests") private List<String> tests;

  /**
     * List of suites to be run.
     */
  @Parameter(property = "suites") private List<String> suites;

  /**
     * Name of the suite page name.
     */
  @Parameter(property = "suitePageName") private String suitePageName;

  /**
     * Suite filter to run in the specified suite (=suitePageName).
     */
  @Parameter(property = "suiteFilter") private String suiteFilter;

  /**
     * If true, the mojo will stop when it encountered an ignored error message.
     */
  @Parameter(property = "stopTestsOnIgnore", defaultValue = "false") private boolean stopTestsOnIgnore;

  /**
     * If true, the mojo will stop when it encountered an exception error
     * message.
     */
  @Parameter(property = "stopTestsOnException", defaultValue = "true") private boolean stopTestsOnException;

  /**
     * If true, the mojo will stop when it encountered a wrong error message.
     */
  @Parameter(property = "stopTestsOnWrong", defaultValue = "true") private boolean stopTestsOnWrong;
  ...
  /**
     ...
     */
  @Component(role = ResultStore.class) private ResultStore resultStore;
  ...
  @Override public final void execute() throws MojoExecutionException, MojoFailureException {
    ...
    if (!skip) {
      ...
      try {
        ...
        final TestCaller testCaller = new URLTestCaller(fitNesseRunPort, "http", "localhost", new File(outputDirectory), resultStore);
        final Date startDate = new Date();
        final FitNesseTestRunner runner = new FitNesseTestRunner(testCaller, stopTestsOnIgnore, stopTestsOnException, stopTestsOnWrong, getLog());
  ...
  private void startCommander() throws MojoFailureException, MojoExecutionException {
    commander = new FitNesseCommander(getCommanderConfig(getJvmDependencies(), getJvmArguments(), 0, fitNesseRunPort, getFitNesseAuthenticate()));
    try {
      commander.start();
  ...
  /**
     ...
     */
  private void saveTestSummariesAndWriteProperties(final Map<String, MafiaTestSummary> testSummaries, final Date runDate) throws MafiaException {
  ...
  @Override public final String toString() {
    return super.toString() + ", FitNesseRunPort: " + fitNesseRunPort + ", Test results directory: " + testResultsDirectory + ", Tests: " + tests + ", Suites: " + suites + ", Suite page name: " + suitePageName + ", Suite filter: " + suiteFilter + ", Stop tests on ignore: " + stopTestsOnIgnore + ", Stop tests on exception: " + stopTestsOnException + ", Stop tests on wrong: " + stopTestsOnWrong;
  }
}