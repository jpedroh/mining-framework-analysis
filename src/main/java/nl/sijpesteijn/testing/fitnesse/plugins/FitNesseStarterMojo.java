 ...
 */
@Mojo(name = "start") public class FitNesseStarterMojo extends AbstractStartFitNesseMojo {
  /**
     * {@inheritDoc}
     */
  @Override public final void execute() throws MojoExecutionException, MojoFailureException {
    getLog().debug(toString());
    final FitNesseCommander commander = new FitNesseCommander(getCommanderConfig(getJvmDependencies(), getJvmArguments(), getRetainDays(), getFitNessePort(), getFitNesseAuthenticate()));
    try {
      commander.start();
    ...
    if (commander.hasError()) {
      ...
      throw new MojoExecutionException("Could not start FitNesse: (error output: " + commander.getErrorOutput() + ", output: " + commander.getOutput() + ")");
    } else {
  ...
  @Override public final String toString() {
    return super.toString() + ", Retain days: " + getRetainDays() + ", JVM arguments: " + getJvmArguments() + ", JVM dependencies: " + getJvmDependencies();
  }
}