...
public class A {
  ...
  public void run(String[] args) throws Exception {
    ...
    try {
      ...
      if (cmdLine.hasOption(CMD_AMQP)) {
        ...
        if (cmdLine.hasOption(CMD_ARTEMIS_CORE)) {
          protocol = Protocol.ArtemisCore;
        }
      }
      connect(cmdLine.getOptionValue(CMD_BROKER, "tcp://localhost:61616"), cmdLine.getOptionValue(CMD_USER), cmdLine.getOptionValue(CMD_PASS), protocol, cmdLine.getOptionValue(CMD_JNDI, ""), cmdLine.hasOption(CMD_NO_TRANSACTION_SUPPORT));
  ...
  protected void executeCommandLine(CommandLine cmdLine) throws Exception {
    if (cmdLine.hasOption(CMD_GET)) {
      ...
      if (cmdLine.hasOption(CMD_PUT)) {
        executePut(cmdLine);
      } else {
        if (cmdLine.hasOption(CMD_COPY_QUEUE)) {
          executeCopy(cmdLine);
        } else {
          if (cmdLine.hasOption(CMD_MOVE_QUEUE)) {
            executeMove(cmdLine);
          } else {
            if (cmdLine.hasOption(CMD_LIST_QUEUES)) {
              executeListQueues(cmdLine);
            } else {
              if (cmdLine.hasOption(CMD_READ_FOLDER)) {
                executeReadFolder(cmdLine);
              } else {
                if (cmdLine.hasOption(CMD_WRITE_DUMP)) {
                  executeWriteDump(cmdLine);
                } else {
                  if (cmdLine.hasOption(CMD_RESTORE_DUMP)) {
  ...
  protected void executeMove(CommandLine cmdLine) throws JMSException, UnsupportedEncodingException, ScriptException, IOException {
    ...
    Queue q = moveSession.createQueue(cmdLine.getOptionValue(CMD_MOVE_QUEUE));
  ...
  protected void executeCopy(CommandLine cmdLine) throws JMSException, ScriptException, IOException {
    ...
    Queue q = sess.createQueue(cmdLine.getOptionValue(CMD_COPY_QUEUE));
    ...
    int count = Integer.parseInt(cmdLine.getOptionValue(CMD_COUNT, DEFAULT_COUNT_ALL));
    int i = 0, j = 0;
    @SuppressWarnings(value = { "unchecked" }) Enumeration<Message> en = qb.getEnumeration();
  ...
  protected void connect(String url, String user, String password, Protocol protocol, String jndi, boolean noTransactionSupport) throws Exception {
    if (StringUtils.isBlank(jndi)) {
      ...
      try {
        ...
        if (propertiesStream == null) {
          propertiesStream = FileUtils.openInputStream(new File(jndi));
        }
  ...
  protected void executeReadFolder(final CommandLine cmdLine) throws IOException, JMSException, ScriptException {
    ...
    if (indexOfPathFilterSeparator > 0) {
    ...
    do {
      Collection<File> files = FileUtils.listFiles(directory, fileFilters, null);
      for (File file : files) {
  ...
  protected void putData(final String data, final CommandLine cmdLine) throws IOException, JMSException, ScriptException {
    ...
    if (cmdLine.hasOption("c")) {
      ...
      if (cmdLine.hasOption(CMD_BATCH_FILE)) {
        if (!useScript) {
  ...
  protected Message createMessageFromInput(final String data, String type, String encoding) throws JMSException, UnsupportedEncodingException, IOException, JsonParseException, JsonMappingException {
    Message outMsg = null;
  ...
  protected Message createMessageFromFile(final String data, String type, String encoding) throws IOException, JMSException, UnsupportedEncodingException, JsonParseException, JsonMappingException {
    Message outMsg = null;
  ...
  protected void executeBrowse(final CommandLine cmdLine) throws JMSException, IOException {
    ...
    @SuppressWarnings(value = { "rawtypes" }) final Enumeration en = qb.getEnumeration();
    int count = Integer.parseInt(cmdLine.getOptionValue(CMD_COUNT, DEFAULT_COUNT_ALL));
    int i = 0;
    ...
    if (msg instanceof TextMessage) {
      ...
      if (msg instanceof BytesMessage) {
        ...
        if (msg instanceof MapMessage) {
          ...
          @SuppressWarnings(value = { "unchecked" }) Enumeration<String> keys = mapMsg.getMapNames();
          output("Payload:");
  ...
  protected String advisoryDataStructureToString(final DataStructure dataStructure) throws JMSException {
    if (dataStructure != null) {
      switch (dataStructure.getDataStructureType()) {
        ...
        return "brokerId: " + brokerInfo.getBrokerId() + " brokerName: " + brokerInfo.getBrokerName() + " brokerURL: " + brokerInfo.getBrokerURL();
        default:
        return null;
      }
    } else {
  ...
  protected void displayRemoveInfo(final RemoveInfo removeInfo, final String startAdvisoryMsg) {
    switch (removeInfo.getObjectId().getDataStructureType()) {
      ...
      output("Removed producer " + startAdvisoryMsg + pi.getProducerId().getConnectionId() + " that produced to destination: " + pi.getDestination().toString());
      break;
      case CommandTypes.CONSUMER_INFO:
      ConsumerInfo ci = (ConsumerInfo) removeInfo.getObjectId();
      output("Removed consumer " + startAdvisoryMsg + ci.getConsumerId().getConnectionId() + " that consumed destination: " + ci.getDestination().toString());
      break;
      case CommandTypes.CONNECTION_INFO:
  ...
  protected void outputProperties(Message msg) throws JMSException {
    ...
    @SuppressWarnings(value = { "unchecked" }) Enumeration<String> en = msg.getPropertyNames();
  ...
  protected Options createOptions() {
    ...
    opts.addOption(CMD_NO_TRANSACTION_SUPPORT, "no-transaction-support", false, "Set to disable transactions if not supported by platform. " + "I.e. Azure Service Bus. When set to false, the Move option is NOT atomic.");
    opts.addOption(CMD_READ_FOLDER, "read-folder", true, "Read files in folder and put to queue. Sent files are deleted! Specify path and a filename." + " Wildcards are supported \'*\' and \'?\'. If no path is given, current directory is assumed.");
    @SuppressWarnings(value = { "static-access" }) Option property = OptionBuilder.withArgName("property=value").hasArgs(2).withValueSeparator().withDescription("use value for given String property. Can be used several times.").create(CMD_SET_HEADER);
    opts.addOption(property);
    @SuppressWarnings(value = { "static-access" }) Option longProperty = OptionBuilder.withArgName("property=value").hasArgs(2).withValueSeparator().withDescription("use value for given Long property. Can be used several times.").create(CMD_SET_LONG_HEADER);
    opts.addOption(longProperty);
    @SuppressWarnings(value = { "static-access" }) Option booleanProperty = OptionBuilder.withArgName("property=value").hasArgs(2).withValueSeparator().withDescription("use value for given Boolean property. Can be used several times.").create(CMD_SET_BOOLEAN_HEADER);
    opts.addOption(booleanProperty);
    @SuppressWarnings(value = { "static-access" }) Option intProperty = OptionBuilder.withArgName("property=value").hasArgs(2).withValueSeparator().withDescription("use value for given Integer property. Can be used several times.").create(CMD_SET_INT_HEADER);
    opts.addOption(intProperty);
    opts.addOption(CMD_WRITE_DUMP, "write-dump", true, "Write a dump of messages to a file. " + "Will preserve metadata and type. Can  be used with transformation option");
    opts.addOption(CMD_RESTORE_DUMP, "restore-dump", true, "Restore a dump of messages in a file," + "created with -" + CMD_WRITE_DUMP + ". Can be used with transformation option.");
    opts.addOption(CMD_TRANSFORM_SCRIPT, "transform-script", true, "JavaScript code (or @path/to/file.js). " + "Used to transform messages with the dump options. Access message in JavaScript by msg.JMSType = \'foobar\';");
    opts.addOption(CMD_VERSION, "version", false, "Show version of A");
    opts.addOption(CMD_JMS_TYPE, "jms-type", true, "Sets JMSType header");
    opts.addOption(CMD_BATCH_FILE, "batch-file", true, "Line separated batch file. Used with -p to produce one message per line in file. " + "Used together with Script where each batch line can be accessed with variable \'entry\' ");
    return opts;
  }
