...
public class ConsoleReader {
  ...
  public ConsoleReader(final InputStream in, final OutputStream out, final InputStream bindings, final Terminal term) throws IOException {
    this.in = in;
  ...
  /**
     ...
     */
  public ConsoleReader(final InputStream in, final Writer out, final InputStream bindings, final Terminal term) throws IOException {
    this.in = in;
    this.out = out;
    this.terminal = term != null ? term : TerminalFactory.get();
    this.keyBindings = loadKeyBindings(bindings);
...
<<<<<<< /usr/src/app/output/jline/jline2/5869eaa93a5099bb1bb4f1929a40d602b3e3624f/src/main/java/jline/console/ConsoleReader.java/left.java
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private void drawBuffer(final int clear) throws IOException {
    ...
    if (terminal.hasWeirdWrap()) {
      ...
      if (getCursorPosition() == width && buf.cursor == buf.length() && clear == 0) {
        print(32);
        print(13);
      }
    }
  ...
  private void clearAhead(final int num, int delta) throws IOException {
    ...
    if (terminal.isAnsiSupported()) {
      ...
      if (delta > 0 && terminal.hasWeirdWrap() && screenCursorCol % width == 0) {
        printAnsiSequence("B");
 ...
 *                      was pressed).
  ...
  /**
     ...
     */
  private int autoprintThreshold = Integer.getInteger(JLINE_COMPLETION_THRESHOLD, 100);
