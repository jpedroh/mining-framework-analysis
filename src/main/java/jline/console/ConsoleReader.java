...
public class ConsoleReader {
  ...
  private int 
<<<<<<< /usr/src/app/output/jline/jline2/5acfe5945383a620910a2d78eb131af86efe7022/src/main/java/jline/console/ConsoleReader.java/left.java
  promptLen
=======
  parenBlinkTimeout = 500
>>>>>>> /usr/src/app/output/jline/jline2/5acfe5945383a620910a2d78eb131af86efe7022/src/main/java/jline/console/ConsoleReader.java/right.java
  ;
  ...
  public ConsoleReader(final @Nullable String appName, final InputStream in, final OutputStream out, final @Nullable Terminal term) throws IOException {
    ...
    setInput(in);
    this.inputrcUrl = Urls.create(Configuration.getString(JLINE_INPUTRC, Urls.create(new File(Configuration.getUserHome(), INPUT_RC)).toExternalForm()));
  ...
  private boolean viEndWord(int count) throws IOException {
    ...
    int end = buf.buffer.length();
    for (int i = 0; pos < end && i < count; i++) {
      if (pos < (end - 1) && !isDelimiter(buf.buffer.charAt(pos)) && isDelimiter(buf.buffer.charAt(pos + 1))) {
        ++pos;
      }
      while (pos < end && isDelimiter(buf.buffer.charAt(pos))) {
        ++pos;
  ...
  public int moveCursor(final int num) throws IOException {
    ...
    if ((buf.cursor + where) < 0) {
      ...
      if ((buf.cursor + where) > buf.buffer.length()) {
      ...
      while (true) {
        ...
        if (state != State.SEARCH) {
          ...
          if (o instanceof Operation) {
            ...
            if (state == State.VI_CHANGE_TO || state == State.VI_YANK_TO || state == State.VI_DELETE_TO) {
            ...
            switch (op) {
              ...
              if (searchTerm.length() > 0) {
                ...
                printSearchStatus(searchTerm.toString(), searchIndex > -1 ? history.get(searchIndex).toString() : "");
              } else {
                searchIndex = -1;
                printSearchStatus("", "");
              }
              ...
              case VI_CHAR_SEARCH:
              {
                int searchChar = (c != ';' && c != ',') ? (pushBackChar.isEmpty() ? readCharacter() : pushBackChar.pop()) : 0;
              ...
              break;
              case VI_CHANGE_CHAR:
              success = viChangeChar(count, pushBackChar.isEmpty() ? readCharacter() : pushBackChar.pop());
              break;
            ...
            if (origState != State.NORMAL) {
              if (origState == State.VI_DELETE_TO) {
                ...
                if (origState == State.VI_CHANGE_TO) {
                  success = viDeleteTo(cursorStart, buf.cursor);
                  consoleKeys.setKeyMap(KeyMap.VI_INSERT);
                } else {
                  if (origState == State.VI_YANK_TO) {
