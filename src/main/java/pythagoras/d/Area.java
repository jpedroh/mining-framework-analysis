...
public class Area implements IShape, Cloneable {
  ...
  @Override public boolean contains(double x, double y, double width, double height) {
  ...
  @Override public PathIterator getPathIterator(AffineTransform t, double flatness) {
  ...
  private void addCurvePolygon(Area area) {
    CurveCrossingHelper crossHelper = new CurveCrossingHelper(new double[][] { coords, area.coords }, new int[] { coordsSize, area.coordsSize }, new int[][] { rules, area.rules }, new int[] { rulesSize, area.rulesSize }, new int[][] { offsets, area.offsets });
  ...
  private void addPolygon(Area area) {
    CrossingHelper crossHelper = new CrossingHelper(new double[][] { coords, area.coords }, new int[] { coordsSize, area.coordsSize });
    IntersectPoint[] intersectPoints = crossHelper.findCrossing();
    if (intersectPoints.length == 0) {
      if (area.contains(getBounds())) {
        copy(area, this);
  ...
  private void subtractPolygon(Area area) {
    ...
    do {
      ...
      resultOffsets[resultRulesPos++] = resultCoordPos - 2;
      int curIndex = point.getEndIndex(true);
      if ((curIndex < 0) || (area.isVertex(coords[2 * curIndex], coords[2 * curIndex + 1]) && crossHelper.containsPoint(new double[] { coords[2 * curIndex], coords[2 * curIndex + 1] }) && (coords[2 * curIndex] != point.getX() || coords[2 * curIndex + 1] != point.getY()))) {
        isCurrentArea = !isCurrentArea;
      } else {
        if (area.containsExact(coords[2 * curIndex], coords[2 * curIndex + 1]) > 0) {
          isCurrentArea = false;
  ...
  private int includeCoordsAndRules(int offset, int length, int[] rules, int[] offsets, int[] resultRules, int[] resultOffsets, double[] resultCoords, double[] coords, int resultRulesPos, int resultCoordPos, IntersectPoint point, boolean isCurrentArea, boolean way, int operation) {
    ...
    for (int i = offset; i < length; i++) {
      ...
      switch (rules[i]) {
        ...
        coefs = new double[] { coords[index - 2], coords[index - 1], coords[index], coords[index + 1], coords[index + 2], coords[index + 3], coords[index + 4], coords[index + 5] };
        isLeft = CrossingHelper.compare(coords[index - 2], coords[index - 1], point.getX(), point.getY()) > 0;
  ...
  private class AreaPathIterator implements PathIterator {
    ...
    @Override @SuppressWarnings(value = { "fallthrough" }) public int currentSegment(double[] c) {
      if (isDone()) {
