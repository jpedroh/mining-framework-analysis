...
public class Area implements IShape, Cloneable {
  ...
  public boolean isRectangular() {
    return (isPolygonal) && (rulesSize <= 5) && (coordsSize <= 8) && (coords[1] == coords[3]) && (coords[7] == coords[5]) && (coords[0] == coords[6]) && (coords[2] == coords[4]);
  }
  ...
  @Override public boolean contains(float x, float y, float width, float height) {
  ...
  @Override public boolean contains(IPoint p) {
    return contains(p.getX(), p.getY());
  }

  @Override public boolean contains(IRectangle r) {
  ...
  @Override public boolean intersects(IRectangle r) {
  ...
  @Override public Rectangle getBounds(Rectangle target) {
  ...
  private void addCurvePolygon(Area area) {
    CurveCrossingHelper crossHelper = new CurveCrossingHelper(new float[][] { coords, area.coords }, new int[] { coordsSize, area.coordsSize }, new int[][] { rules, area.rules }, new int[] { rulesSize, area.rulesSize }, new int[][] { offsets, area.offsets });
    ...
    do {
      ...
      if (curIndex < 0) {
        ...
        if (area.containsExact(coords[2 * curIndex], coords[2 * curIndex + 1]) > 0) {
          isCurrentArea = false;
        } else {
          isCurrentArea = true;
        }
      ...
      if ((point.getRuleIndex(isCurrentArea) > nextPoint.getRuleIndex(isCurrentArea))) {
        ...
        resultCoordPos = includeCoordsAndRules(offset + 1, rulesSize, rules, offsets, resultRules, resultOffsets, resultCoords, coords, resultRulesPos, resultCoordPos, point, isCurrentArea, false, 0);
      ...
      resultCoordPos = includeCoordsAndRules(offset, length, rules, offsets, resultRules, resultOffsets, resultCoords, coords, resultRulesPos, resultCoordPos, point, isCurrentArea, true, 0);
  ...
  private void addPolygon(Area area) {
    CrossingHelper crossHelper = new CrossingHelper(new float[][] { coords, area.coords }, new int[] { coordsSize, area.coordsSize });
    IntersectPoint[] intersectPoints = crossHelper.findCrossing();
    if (intersectPoints.length == 0) {
      if (area.contains(getBounds())) {
        copy(area, this);
  ...
  private void intersectCurvePolygon(Area area) {
    ...
    do {
      ...
      if (point.getRuleIndex(isCurrentArea) > nextPoint.getRuleIndex(isCurrentArea)) {
        int rulesSize = (isCurrentArea) ? this.rulesSize : area.rulesSize;
        resultCoordPos = includeCoordsAndRules(offset + 1, rulesSize, rules, offsets, resultRules, resultOffsets, resultCoords, coords, resultRulesPos, resultCoordPos, point, isCurrentArea, false, 1);
        resultRulesPos += rulesSize - offset - 1;
        offset = 1;
        isCopyUntilZero = true;
      }
      ...
      if ((length == offset) && (nextPoint.getRule(isCurrentArea) != PathIterator.SEG_LINETO) && (nextPoint.getRule(isCurrentArea) != PathIterator.SEG_CLOSE) && (point.getRule(isCurrentArea) != PathIterator.SEG_LINETO) && (point.getRule(isCurrentArea) != PathIterator.SEG_CLOSE)) {
        isCopyUntilZero = true;
        length++;
      }
      resultCoordPos = includeCoordsAndRules(offset, length, rules, offsets, resultRules, resultOffsets, resultCoords, coords, resultRulesPos, resultCoordPos, nextPoint, isCurrentArea, true, 1);
  ...
  private void subtractCurvePolygon(Area area) {
    ...
    do {
      ...
      IntersectPoint nextPoint = (isCurrentArea) ? getNextIntersectPoint(intersectPoints, point, isCurrentArea) : getPrevIntersectPoint(intersectPoints, point, isCurrentArea);
      float[] coords = (isCurrentArea) ? this.coords : area.coords;
      int[] offsets = (isCurrentArea) ? this.offsets : area.offsets;
      int[] rules = (isCurrentArea) ? this.rules : area.rules;
      ...
      if (((isCurrentArea) && (point.getRuleIndex(isCurrentArea) > nextPoint.getRuleIndex(isCurrentArea))) || ((!isCurrentArea) && (nextPoint.getRuleIndex(isCurrentArea) > nextPoint.getRuleIndex(isCurrentArea)))) {
        int rulesSize = (isCurrentArea) ? this.rulesSize : area.rulesSize;
        resultCoordPos = includeCoordsAndRules(offset + 1, rulesSize, rules, offsets, resultRules, resultOffsets, resultCoords, coords, resultRulesPos, resultCoordPos, point, isCurrentArea, false, 2);
        resultRulesPos += rulesSize - offset - 1;
        offset = 1;
        isCopyUntilZero = true;
      }
      ...
      resultCoordPos = includeCoordsAndRules(offset, length, rules, offsets, resultRules, resultOffsets, resultCoords, coords, resultRulesPos, resultCoordPos, point, isCurrentArea, true, 2);
  ...
  private void subtractPolygon(Area area) {
    ...
    float[] resultCoords = new float[2 * (coordsSize + area.coordsSize + intersectPoints.length)];
    ...
    do {
      ...
      resultOffsets[resultRulesPos++] = resultCoordPos - 2;
      int curIndex = point.getEndIndex(true);
      if ((curIndex < 0) || (area.isVertex(coords[2 * curIndex], coords[2 * curIndex + 1]) && crossHelper.containsPoint(new float[] { coords[2 * curIndex], coords[2 * curIndex + 1] }) && (coords[2 * curIndex] != point.getX() || coords[2 * curIndex + 1] != point.getY()))) {
        isCurrentArea = !isCurrentArea;
      } else {
        if (area.containsExact(coords[2 * curIndex], coords[2 * curIndex + 1]) > 0) {
          isCurrentArea = false;
      ...
      IntersectPoint nextPoint = (isCurrentArea) ? getNextIntersectPoint(intersectPoints, point, isCurrentArea) : getPrevIntersectPoint(intersectPoints, point, isCurrentArea);
      float[] coords = (isCurrentArea) ? this.coords : area.coords;
      int offset = (isCurrentArea) ? 2 * point.getEndIndex(isCurrentArea) : 2 * nextPoint.getEndIndex(isCurrentArea);
      ...
      if (offset >= 0) {
        int length = (isCurrentArea) ? 2 * nextPoint.getBegIndex(isCurrentArea) - offset + 2 : 2 * point.getBegIndex(isCurrentArea) - offset + 2;
        if (isCurrentArea) {
          System.arraycopy(coords, offset, resultCoords, resultCoordPos, length);
        } else {
          float[] temp = new float[length];
    ...
    for (IntersectPoint point : iPoints) {
      ...
      if (begIndex >= 0) {
        if (firstIsectPoint == null) {
          ...
          if (begIndex < firstIsectPoint.getBegIndex(isCurrentArea)) {
            firstIsectPoint = point;
          }
        }
      }
      if (endIndex <= begIndex) {
        if (nextIsectPoint == null) {
          ...
          if (begIndex < nextIsectPoint.getBegIndex(isCurrentArea)) {
            nextIsectPoint = point;
          }
        }
      }
    ...
    for (IntersectPoint point : iPoints) {
      ...
      if (endIndex >= 0) {
        if (firstIsectPoint == null) {
          ...
          if (endIndex < firstIsectPoint.getEndIndex(isCurrentArea)) {
            firstIsectPoint = point;
          }
        }
      }
      if (endIndex <= begIndex) {
        if (predIsectPoint == null) {
          ...
          if (endIndex > predIsectPoint.getEndIndex(isCurrentArea)) {
            predIsectPoint = point;
          }
        }
      }
  ...
  private int includeCoordsAndRules(int offset, int length, int[] rules, int[] offsets, int[] resultRules, int[] resultOffsets, float[] resultCoords, float[] coords, int resultRulesPos, int resultCoordPos, IntersectPoint point, boolean isCurrentArea, boolean way, int operation) {
    ...
    for (int i = offset; i < length; i++) {
      ...
      switch (rules[i]) {
        ...
        float[] coefs = new float[] { coords[index - 2], coords[index - 1], coords[index], coords[index + 1], coords[index + 2], coords[index + 3] };
        ...
        coefs = new float[] { coords[index - 2], coords[index - 1], coords[index], coords[index + 1], coords[index + 2], coords[index + 3], coords[index + 4], coords[index + 5] };
        isLeft = CrossingHelper.compare(coords[index - 2], coords[index - 1], point.getX(), point.getY()) > 0;
  ...
  private class AreaPathIterator implements PathIterator {
    ...
    @Override @SuppressWarnings(value = { "fallthrough" }) public int currentSegment(float[] c) {
      if (isDone()) {
