...
public class XPathProcessor extends AbstractRMLProcessor {
  ...
  @Override public void execute(final SesameDataSet dataset, final TriplesMap map, final RMLPerformer performer, String fileName) {
    try {
      this.map = map;
      ...
      event.setListener(new InstantEvaluationListener() {
        @Override public void onNodeHit(Expression expression, NodeItem nodeItem) {
          Node node = (Node) nodeItem.xml;
          performer.perform(node, dataset, map);
        }

        @Override public void finishedNodeSet(Expression expression) {
        }

        @Override public void onResult(Expression expression, Object result) {
        }
      });
  ...
  @Override public void execute_node(SesameDataSet dataset, String expression, TriplesMap parentTriplesMap, RMLPerformer performer, Object node) {
  ...
  @Override public List<String> extractValueFromNode(Object node, String expression) {
