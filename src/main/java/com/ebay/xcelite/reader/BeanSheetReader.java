 ...
 * created Sep 9, 2013
 */
public class BeanSheetReader<T extends java.lang.Object> extends AbstractSheetReader<T> {
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  @SneakyThrows @Override public T fillObject(Row row) {
  ...
  @SuppressWarnings(value = { "unchecked" }) private Field getField(Class<?> aClass, String name) {
    return ReflectionUtils.getAllFields(aClass, withName(name)).iterator().next();
  }

  @Override @SneakyThrows T getNewObject() {
  ...
  /**
     ...
     */
  @Override protected void validateColumns() {
    if (anyColumn != null) {
      return;
    }
  ...
  private static boolean isColumnInIgnoreList(Field anyColumnField, String columnName) {
    ...
    Set<String> ignoreCols = Arrays.stream(annotation.ignoreCols()).collect(Collectors.toSet());
  ...
  @Override protected void buildHeader(Row row) {
  ...
  private class ColumnsMapper {
    ...
    Set<String> getDeclaredHeaderNames() {
      return columnsMap.values().stream().map(
<<<<<<< /usr/src/app/output/ebay/xcelite/40e1d22a1ff208cd99f014eac25fe2a51f1cc6df/src/main/java/com/ebay/xcelite/reader/BeanSheetReader.java/left.java
      Col::getName
=======
      (c) -> c.getName()
