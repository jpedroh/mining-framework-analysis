 ...
 * @since 1.0
 */
public class BeanSheetWriter<T extends java.lang.Object> extends AbstractSheetWriter<T> {
  ...
  @Override public void write(Collection<T> data) {
  ...
  /**
     ...
     * @since 1.0
     */
  @SuppressWarnings(value = { "unchecked" }) @SneakyThrows @Override public void writeRow(T data, Row excelRow, int rowIndex) {
    int i = 0;
    for (Col col : columns) {
  ...
  @SuppressWarnings(value = { "unchecked" }) @SneakyThrows private void writeData(Collection<T> data) {
...
<<<<<<< /usr/src/app/output/ebay/xcelite/66cde34b01ea5ac3a5db132e6f11a8038db98a84/src/main/java/com/ebay/xcelite/writer/BeanSheetWriter.java/left.java
      excelRow = sheet.getNativeSheet().createRow(rowIndex)
=======
      row = sheet.getNativeSheet().createRow(rowIndex++)
>>>>>>> /usr/src/app/output/ebay/xcelite/66cde34b01ea5ac3a5db132e6f11a8038db98a84/src/main/java/com/ebay/xcelite/writer/BeanSheetWriter.java/right.java
  ...
  @SuppressWarnings(value = { "unchecked" }) @SneakyThrows private void writeToCell(Cell cell, Col col, Object fieldValueObj) {
    ...
    if (col.getDataFormat() != null) {
      cell.setCellStyle(CellStylesBank.get(sheet.getNativeSheet().getWorkbook()).getCustomDataFormatStyle(col.getDataFormat()));
    }
  ...
  @SuppressWarnings(value = { "unchecked" }) @SneakyThrows private void appendAnyColumns(T t, Set<Col> columnToAdd) {
  ...
  private void checkHasThrowPolicyMustThrow(Object fieldValueObj, Col col) {
    if ((null == fieldValueObj) && (options.getMissingCellPolicy().equals(MissingCellPolicy.THROW))) {
      throw new PolicyViolationException("Null property found and " + "MissingCellPolicy.THROW active. Object index: " + rowIndex + ", property name" + col.getFieldName());
    }
  }
}