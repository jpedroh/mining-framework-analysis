...
public class PlotterSettingsPanel extends JPanel {
  ...
  public PlotterSettingsPanel(PlotterSettings settings) {
    ...
    if (index < 0 || index >= machineStyles.size()) {
      index = 0;
    }
    ...
    interior1.add(zMotorType = new SelectOneOfMany("zMotorType", Translator.get("PlotterSettings.zMotorType"), new String[] { Translator.get("PlotterSettings.zMotorType.servo"), Translator.get("PlotterSettings.zMotorType.stepper") }, settings.getZMotorType() - 1));
