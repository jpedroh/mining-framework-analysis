...
public class Player extends Mob {
  ...
  @Override public void aiStep() {
    if (settings.HackType == 0 || !(HackState.fly || HackState.speed || HackState.noclip) && input.canMove) {
      inventory.tick();
      oBob = bob;
            ...
            if (bool2) {
              ...
              if (i != 0) {
                ...
                if (onGround) {
                  ...
                  if (HacksEnabled && !input.fall && k != 0 && jumpCount < 3) {
                    yd = 0.5F;
                    input.fall = true;
                    jumpCount += 1;
                  }
  ...
  @Override public void awardKillScore(Entity var1, int score) {
    this.score += score;
  }

  @Override public void bindTexture(TextureManager textureManager) {
    if (newTexture != null) {
    ...
    if (isInteger(modelName)) {
      ...
      if (!modelName.startsWith("humanoid")) {
  ...
  @Override public void die(Entity killedBy) {
    setSize(0.2F, 0.2F);
    this.setPos(x, y, z);
    yd = 0.1F;
    if (killedBy != null) {
  ...
  @Override public void hurt(Entity entity, int amount) {
    if (!level.creativeMode) {
      super.hurt(entity, amount);
    }
  }

  @Override public boolean isCreativeModeAllowed() {
    return true;
  }

  @Override public boolean isShootable() {
    return true;
  }
  ...
  @Override public void render(TextureManager textureManager, float var2) {
  ...
  @Override public void renderModel(TextureManager var1, float var2, float var3, float var4, float var5, float var6, float var7) {
    if (isInteger(modelName)) {
      try {
        GL11.glEnable(GL11.GL_ALPHA_TEST);
        GL11.glEnable(GL11.GL_BLEND);
