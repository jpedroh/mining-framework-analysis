...
public final class Minecraft implements Runnable {
          ...
          if (level != null && player != null && guiIsTransparent) {
            ...
            for (Entity entity : nearbyEntities) {
              if (entity.isPickable()) {
                var74 = 0.1F;
                MovingObjectPosition var78 = entity.boundingBox.grow(var74, var74, var74).clip(newPlayerVector, vec3D);
                if (var78 != null) {
            ...
            GL11.glViewport(0, 0, width, height);
            float viewDistanceFactor = 1F - (float) Math.pow(1F / (4 - settings.viewDistance), 0.25D);
            ...
            int blockTypeAroundHead = level.getTile((int) this.player.x, (int) (this.player.y + 0.12F), (int) this.player.z);
            ...
            for (int pass = 0; pass < 2; ++pass) {
              if (!particleManager.particles[pass].isEmpty()) {
                ...
                for (int i = 0; i < particleManager.particles[pass].size(); ++i) {
                  ((Particle) particleManager.particles[pass].get(i)).render(shapeRenderer, delta, var123, var69, newYRot, var117, var32);
                }
                shapeRenderer.end();
              }
            }
            ...
            if (selected != null) {
              ...
              if (levelRenderer.cracks > 0F) {
                ...
                for (int side = 0; side < 6; ++side) {
                  blockAroundHead.renderSide(shapeRenderer, var102.x, var102.y, var102.z, side, 240 + (int) (levelRenderer.cracks * 10F));
                }
                shapeRenderer.end();
  ...
  private void tick() {
    ...
    for (int i = 0; i < texManager.animations.size(); ++i) {
      ...
      GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, texFX.textureId % 16 << 4, texFX.textureId / 16 << 4, 16, 16, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, texManager.textureBuffer);
    }
      ...
      if (!networkManager.isConnected()) {
        ...
        if (networkManager.successful && networkManager.netHandler.connected) {
          ...
          try {
            ...
            for (int packetsReceived = 0; packetsReceived < NetworkManager.MAX_PACKETS_PER_TICK && networkHandler.in.position() > 0; packetsReceived++) {
        ...
        if (networkManager.levelLoaded) {
          ...
          networkManager.netHandler.send(PacketType.POSITION_ROTATION, packetHandler.canSendHeldBlock ? player.inventory.getSelected() : -1, playerXUnits, playerYUnits, playerZUnits, playerYRotation, playerXRotation);
        }
      }
    }
    if (isLoadingMap) {
