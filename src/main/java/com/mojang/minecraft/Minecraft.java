...
public final class Minecraft implements Runnable {
  ...
  private void logSystemInfo() {
    LogUtil.logInfo(String.format("GPU Vendor: %s | Renderer: %s | OpenGL version: %s", GL11.glGetString(GL11.GL_VENDOR), GL11.glGetString(GL11.GL_RENDERER), GL11.glGetString(GL11.GL_VERSION)));
  }
      ...
      if (!isConnecting) {
        ...
        if (!isConnecting) {
          ...
          if (level != null && player != null && guiIsTransparent) {
            ...
            Vec3D vec3D = newPlayerVector.add(var34 * reachDistance, (var33 * reachDistance) - modelCache.getModel(player.getModelName()).headOffset, var87 * reachDistance);
            ...
            vec3D = newPlayerVector.add(var34 * reachDistance, var33 * reachDistance, var87 * reachDistance);
            renderer.entity = null;
            List<Entity> nearbyEntities = level.blockMap.getEntities(player, player.boundingBox.expand(var34 * reachDistance, var33 * reachDistance, var87 * reachDistance));
            final float growFactor = 0.1F;
            float closestDist = 0F;
            for (Entity entity : nearbyEntities) {
              if (entity.isPickable()) {
                MovingObjectPosition var78 = entity.boundingBox.grow(growFactor, growFactor, growFactor).clip(newPlayerVector, vec3D);
                if (var78 != null) {
            ...
            GL11.glViewport(0, 0, width, height);
            float viewDistanceFactor = 1F - (float) Math.pow(1F / (settings.viewDistance + 1), 0.25D);
            ...
            Collections.sort(levelRenderer.chunksToUpdate, new ChunkDirtyDistanceComparator(player));
            ...
            if (chunkUpdates > 0) {
              ...
              if (settings.framerateLimit > 0) {
                ...
                if (timer.lastFrameDuration > 1 / minDesiredFramerate) {
                  ...
                  if (renderer.everBackedOffFromChunkUpdates) {
                    ...
                    for (int z = playerZ - 1; z <= playerZ + 1; ++z) {
                      ...
                      if (var104 != 0 && Block.blocks[var104].isSolid()) {
                        ...
                        for (int side = 0; side < 6; ++side) {
                          Block.blocks[var104].renderInside(shapeRenderer, x, y, z, side);
                        }
                        shapeRenderer.end();
            ...
            float var117 = -(newYRot = -MathHelper.sin(player.yRot * (float) Math.PI / 180F)) * MathHelper.sin(player.xRot * (float) Math.PI / 180F);
            ...
            for (int pass = 0; pass < 2; ++pass) {
              ...
              if (!particles.isEmpty()) {
                ...
                for (Particle particle : particles) {
                  particle.render(shapeRenderer, delta, var123, var69, newYRot, var117, var32);
                }
                shapeRenderer.end();
              }
            }
            ...
            if (selected != null) {
              ...
              if (levelRenderer.cracks > 0F) {
                ...
                GL11.glTranslatef(selected.x + blockXAverage, selected.y + blockYAverage, selected.z + blockZAverage);
                ...
                for (int side = 0; side < 6; ++side) {
                  blockAroundHead.renderSide(shapeRenderer, selected.x, selected.y, selected.z, side, 240 + (int) (levelRenderer.cracks * 10F));
                }
                shapeRenderer.end();
              ...
              if (pointedBlockType > 0) {
                AABB aabb = Block.blocks[pointedBlockType].getSelectionBox(selected.x, selected.y, selected.z).grow(0.002F, 0.002F, 0.002F);
            ...
            renderer.updateFog();
            GL11.glEnable(GL11.GL_TEXTURE_2D);
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, levelRenderer.textureManager.load(Textures.MAP_EDGE));
            ...
            if (heldBlock.block != null) {
              ...
              if (settings.thirdPersonMode == ThirdPersonMode.NONE && canRenderGUI) {
                GL11.glBindTexture(GL11.GL_TEXTURE_2D, heldBlock.minecraft.textureManager.load(Textures.TERRAIN));
  ...
  private void tick() {
    ...
    if (level != null && player != null) {
      ...
      int heldBlockId = player.inventory.getSelected();
      ...
      if (renderer.minecraft.isRaining) {
        ...
        for (int i = 0; i < 50; ++i) {
          ...
          if (groundLevel <= playerY + 4 && groundLevel >= playerY - 4) {
            ...
            particleManager.spawnParticle(new WaterDropParticle(level, raindropBlockX + offsetX, groundLevel + 0.1F, raindropBlockY + offsetZ));
          }
        }
      }
  ...
  private void renderAnimatedTextures() {
    ...
    for (int i = 0; i < textureManager.animations.size(); ++i) {
      ...
      GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, texFX.textureId % 16 << 4, texFX.textureId / 16 << 4, 16, 16, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, textureManager.textureBuffer);
    }
  }
  ...
  private void doNetworking() {
    try {
      if (!networkManager.handshakeSent) {
        networkManager.send(PacketType.IDENTIFICATION, Constants.PROTOCOL_VERSION, session.username, session.mppass, (int) Constants.CLIENT_TYPE);
        networkManager.handshakeSent = true;
      }
      do {
        networkManager.channel.read(networkManager.in);
        for (int packetsReceived = 0; packetsReceived < NetworkManager.MAX_PACKETS_PER_TICK && networkManager.in.position() > 0; packetsReceived++) {
      ...
      networkManager.send(PacketType.POSITION_ROTATION, networkManager.isExtEnabled(ProtocolExtension.HELD_BLOCK) ? player.inventory.getSelected() : -1, playerXUnits, playerYUnits, playerZUnits, playerYRotation, playerXRotation);
    } catch (Exception ex) {
      LogUtil.logWarning("Error in network handling code.", ex);
      setCurrentScreen(new ErrorScreen("Disconnected!", "You\'ve lost connection to the server"));
      isConnecting = false;
