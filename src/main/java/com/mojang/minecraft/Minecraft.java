...
public final class Minecraft implements Runnable {
  ...
  public static boolean doesUrlExistAndIsImage(URL url) {
    try {
      ...
      boolean result = (con.getResponseCode() == HttpURLConnection.HTTP_OK) && con.getContentType().contains("image");
      con.disconnect();
      return result;
    } catch (Exception ex) {
  ...
  /**
	 ...
	 */
  public Minecraft(Canvas canvas, MinecraftApplet applet, int width, int height, boolean fullscreen, boolean isApplet) {
    this.applet = applet;
    this.canvas = canvas;
    this.width = width;
    this.height = height;
  ...
  public final void generateLevel(int scale) {
    ...
    Level newLevel = new LevelGenerator(progressBar).generate(username, 128 << scale, 128 << scale, 64);
  ...
  private void onMouseClick(int button) {
    ...
    if (selected.hasEntity) {
      ...
      boolean isAirOrLiquid = (block == null || block == Block.WATER || block == Block.STATIONARY_WATER || block == Block.LAVA || block == Block.STATIONARY_LAVA);
      if (!isAirOrLiquid || (aabb != null && (!(!player.boundingBox.intersects(aabb) && level.isFree(aabb))))) {
        return;
      }
  ...
  private void initialize() throws Exception {
    ...
    if (settings.lastUsedTexturePack != null) {
      File texturePack = new File(getMinecraftDirectory(), "texturepacks/" + settings.lastUsedTexturePack);
    ...
    if (server != null && session != null) {
      networkManager = new NetworkManager(this, server, port, session.username, session.mppass);
    }
  }
  ...
  @Override public final void run() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(null, ex.toString(), "Failed to start ClassiCube", JOptionPane.ERROR_MESSAGE);
      return;
    }
  ...
  private void onFrame() {
    ...
    if (!Display.isFullscreen() && (canvas.getWidth() != Display.getDisplayMode().getWidth() || canvas.getHeight() != Display.getDisplayMode().getHeight())) {
    ...
    try {
      ...
      long actualNow = System.nanoTime() / 1000000L;
      ...
      if (!isOnline) {
        ...
        if (renderer.minecraft.hasMouse) {
          ...
          if (renderer.minecraft.isLevelLoaded) {
            if (renderer.minecraft.canvas != null) {
              ...
              Mouse.setCursorPosition(renderer.minecraft.width / 2, renderer.minecraft.height / 2);
            }
          } else {
            var81 = Mouse.getDX();
            var86 = Mouse.getDY();
        ...
        if (!renderer.minecraft.isOnline) {
          ...
          if (renderer.minecraft.level != null && player != null) {
            ...
            Vec3D vec3D = var31.add(var34 * reachDistance, var33 * reachDistance, var87 * reachDistance);
            renderer.minecraft.selected = renderer.minecraft.level.clip(var31, vec3D);
            var74 = reachDistance;
            if (renderer.minecraft.selected != null) {
              var74 = renderer.minecraft.selected.vec.distance(renderer.getPlayerVector(timer.delta));
            }
            ...
            vec3D = var31.add(var34 * reachDistance, var33 * reachDistance, var87 * reachDistance);
            renderer.entity = null;
            List<Entity> var37 = renderer.minecraft.level.blockMap.getEntities(player, player.boundingBox.expand(var34 * reachDistance, var33 * reachDistance, var87 * reachDistance));
            float var35 = 0F;
            for (i = 0; i < var37.size(); ++i) {
              Entity var88 = var37.get(i);
              if (var88.isPickable()) {
                var74 = 0.1F;
                MovingObjectPosition var78 = var88.boundingBox.grow(var74, var74, var74).clip(var31, vec3D);
                if (var78 != null) {
            ...
            float viewDistanceFactor = 1F - (float) (Math.pow((1F / (4 - renderer.minecraft.settings.viewDistance)), 0.25D));
            ...
            Block block = Block.blocks[level.getTile((int) this.player.x, (int) (this.player.y + 0.12F), (int) this.player.z)];
            ...
            GLU.gluPerspective(var69, (float) renderer.minecraft.width / (float) renderer.minecraft.height, 0.05F, renderer.fogEnd);
            GL11.glMatrixMode(GL11.GL_MODELVIEW);
            GL11.glLoadIdentity();
            ...
            if (renderer.minecraft.selected != null && settings.thirdPersonMode == 2) {
              cameraDistance = -(renderer.minecraft.selected.vec.distance(renderer.getPlayerVector(timer.delta)) - 0.51F);
            ...
            Collections.sort(renderer.minecraft.levelRenderer.chunks, new ChunkDirtyDistanceComparator(player));
                  ...
                  for (int var38 = var110 - 1; var38 <= var110 + 1; ++var38) {
                    ...
                    if (var104 != 0 && Block.blocks[var104].isSolid()) {
                      ...
                      for (var114 = 0; var114 < 6; ++var114) {
                        Block.blocks[var104].renderInside(shapeRenderer, var122, var98, var105, var114);
                      }
                      shapeRenderer.end();
            ...
            float var117 = -(var30 = -MathHelper.sin(player.yRot * (float) Math.PI / 180F)) * MathHelper.sin(player.xRot * (float) Math.PI / 180F);
            ...
            for (var83 = 0; var83 < 2; ++var83) {
              if (!particleManager.particles[var83].isEmpty()) {
                ...
                for (i = 0; i < particleManager.particles[var83].size(); ++i) {
                  ((Particle) particleManager.particles[var83].get(i)).render(shapeRenderer, delta, var29, var69, var30, var117, var32);
                }
                shapeRenderer.end();
              }
            }
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, levelRenderer.textureManager.load("/rock.png"));
            GL11.glEnable(GL11.GL_TEXTURE_2D);
            GL11.glCallList(levelRenderer.listId);
            renderer.updateFog();
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, levelRenderer.textureManager.load("/clouds.png"));
            ...
            if (settings.showClouds) {
              ...
              for (x = -2048; x < levelRenderer.level.width + 2048; x += 512) {
                for (y = -2048; y < levelRenderer.level.length + 2048; y += 512) {
                  shapeRenderer.vertexUV(x, cloudLevel, y + 512, x * unknownCloud + cloudTickOffset, (y + 512) * unknownCloud);
                  shapeRenderer.vertexUV(x + 512, cloudLevel, y + 512, (x + 512) * unknownCloud + cloudTickOffset, (y + 512) * unknownCloud);
                  shapeRenderer.vertexUV(x + 512, cloudLevel, y, (x + 512) * unknownCloud + cloudTickOffset, y * unknownCloud);
                  shapeRenderer.vertexUV(x, cloudLevel, y, x * unknownCloud + cloudTickOffset, y * unknownCloud);
                  shapeRenderer.vertexUV(x, cloudLevel, y, x * unknownCloud + cloudTickOffset, y * unknownCloud);
                  shapeRenderer.vertexUV(x + 512, cloudLevel, y, (x + 512) * unknownCloud + cloudTickOffset, y * unknownCloud);
                  shapeRenderer.vertexUV(x + 512, cloudLevel, y + 512, (x + 512) * unknownCloud + cloudTickOffset, (y + 512) * unknownCloud);
                  shapeRenderer.vertexUV(x, cloudLevel, y + 512, x * unknownCloud + cloudTickOffset, (y + 512) * unknownCloud);
            ...
            if (renderer.minecraft.selected != null) {
              ...
              GL11.glColor4f(1F, 1F, 1F, (MathHelper.sin(System.currentTimeMillis() / 100F) * 0.2F + 0.4F) * 0.5F);
              if (levelRenderer.cracks > 0F) {
                ...
                GL11.glTranslatef(var102.x + blockXAverage, var102.y + blockYAverage, var102.z + blockZAverage);
                ...
                for (i = 0; i < 6; ++i) {
                  block.renderSide(shapeRenderer, var102.x, var102.y, var102.z, i, 240 + (int) (levelRenderer.cracks * 10F));
                }
                shapeRenderer.end();
              ...
              if (var104 > 0) {
                AABB aabb = Block.blocks[var104].getSelectionBox(selectedBlock.x, selectedBlock.y, selectedBlock.z).grow(0.002F, 0.002F, 0.002F);
            ...
            GL11.glEnable(GL11.GL_TEXTURE_2D);
            GL11.glEnable(GL11.GL_BLEND);
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, levelRenderer.textureManager.load("/water.png"));
            ...
            if (renderer.minecraft.isRaining || renderer.minecraft.isSnowing) {
              ...
              if (renderer.minecraft.isRaining) {
                GL11.glBindTexture(GL11.GL_TEXTURE_2D, renderer.minecraft.textureManager.load("/rain.png"));
              } else {
                if (renderer.minecraft.isSnowing) {
                  GL11.glBindTexture(GL11.GL_TEXTURE_2D, renderer.minecraft.textureManager.load("/snow.png"));
                  speed = 0.2F;
                }
              }
              for (var110 = var104 - 5; var110 <= var104 + 5; ++var110) {
                for (var122 = var114 - 5; var122 <= var114 + 5; ++var122) {
                  ...
                  if (var86 != var125) {
                    var74 = ((renderer.levelTicks + var110 * 3121 + var122 * 418711) % 32 + delta) / 32F * speed;
                    ...
                    shapeRenderer.begin();
                    shapeRenderer.vertexUV(var110, var86, var122, 0F, var86 * 2F / 8F + var74 * 2F);
                    shapeRenderer.vertexUV(var110 + 1, var86, var122 + 1, 2F, var86 * 2F / 8F + var74 * 2F);
                    shapeRenderer.vertexUV(var110 + 1, var125, var122 + 1, 2F, var125 * 2F / 8F + var74 * 2F);
                    shapeRenderer.vertexUV(var110, var125, var122, 0F, var125 * 2F / 8F + var74 * 2F);
                    shapeRenderer.vertexUV(var110, var86, var122 + 1, 0F, var86 * 2F / 8F + var74 * 2F);
                    shapeRenderer.vertexUV(var110 + 1, var86, var122, 2F, var86 * 2F / 8F + var74 * 2F);
                    shapeRenderer.vertexUV(var110 + 1, var125, var122, 2F, var125 * 2F / 8F + var74 * 2F);
                    shapeRenderer.vertexUV(var110, var125, var122 + 1, 0F, var125 * 2F / 8F + var74 * 2F);
                    shapeRenderer.end();
                  }
                }
              }
            ...
            if (!isSinglePlayer && networkManager != null && networkManager.players != null && networkManager.players.size() > 0) {
              if ((settings.ShowNames == 2 || settings.ShowNames == 3) && this.player.userType >= 100) {
                for (int n = 0; n < networkManager.players.values().size(); n++) {
                  NetworkPlayer np = (NetworkPlayer) networkManager.players.values().toArray()[n];
                  if (np != null) {
            ...
            var117 = renderer.heldBlock.lastPos + (heldBlock.pos - heldBlock.lastPos) * delta;
            ...
            if (heldBlock.moving) {
              var33 = MathHelper.sin((var74 = (heldBlock.offset + delta) / 7F) * var74 * (float) Math.PI);
              GL11.glRotatef(MathHelper.sin(MathHelper.sqrt(var74) * (float) Math.PI) * 80F, 0F, 1F, 0F);
              GL11.glRotatef(-var33 * 20F, 1F, 0F, 0F);
            }
            ColorCache color = heldBlock.minecraft.level.getBrightnessColor((int) player.x, (int) player.y, (int) player.z);
            GL11.glColor4f(color.R, color.G, color.B, 1F);
            if (heldBlock.block != null) {
              GL11.glScalef(0.4F, 0.4F, 0.4F);
              GL11.glTranslatef(-0.5F, -0.5F, -0.5F);
              if (settings.thirdPersonMode == 0 && canRenderGUI) {
                GL11.glBindTexture(GL11.GL_TEXTURE_2D, heldBlock.minecraft.textureManager.load("/terrain.png"));
            ...
            renderer.minecraft.hud.render(timer.delta, renderer.minecraft.currentScreen != null, mouseX, mouseY);
          } else {
            GL11.glViewport(0, 0, renderer.minecraft.width, renderer.minecraft.height);
  ...
  public final void setLevel(Level theLevel) {
    if (applet == null || !applet.getDocumentBase().getHost().equalsIgnoreCase("minecraft.net") && !applet.getDocumentBase().getHost().equalsIgnoreCase("www.minecraft.net") || !applet.getCodeBase().getHost().equalsIgnoreCase("minecraft.net") && !applet.getCodeBase().getHost().equalsIgnoreCase("www.minecraft.net")) {
      theLevel = null;
    }
    level = theLevel;
  ...
  public void takeAndSaveScreenshot(int width, int height) {
    try {
      ...
      ComponentColorModel colorComp = new ComponentColorModel(colorSpace, bitsPerPixel, false, false, 3, DataBuffer.TYPE_BYTE);
      WritableRaster raster = Raster.createInterleavedRaster(new DataBufferByte(pixels, pixels.length), width, height, width * 3, 3, colOffsets, null);
      ...
      String serverName = ProgressBarDisplay.title.toLowerCase().contains("connecting..") ? "" : ProgressBarDisplay.title;
      if (isSinglePlayer) {
  ...
  private void tick() {
    ...
    for (i = 0; i < texManager.animations.size(); ++i) {
      ...
      GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, texFX.textureId % 16 << 4, texFX.textureId / 16 << 4, 16, 16, 6408, 5121, texManager.textureBuffer);
    }
    int var4;
    int var40;
    int var46;
              ...
              while (networkHandler.in.position() > 0 && var4++ != 100) {
                ...
                if (networkHandler.netManager.successful) {
                  if (packetType == PacketType.EXT_INFO) {
                    ...
                    LogUtil.logInfo("Connecting to AppName: " + AppName + " with extension count: " + ExtensionCount);
                    receivedExtensionLength = ExtensionCount;
                    Constants.SERVER_SUPPORTED_EXTENSIONS.clear();
                  } else {
                    if (packetType == PacketType.EXT_ENTRY) {
                      ...
                      if (receivedExtensionLength == Constants.SERVER_SUPPORTED_EXTENSIONS.size()) {
                        ...
                        for (int j = 0; j < PacketType.packets.length - 1; j++) {
                          if (PacketType.packets[j] != null && !PacketType.packets[j].extName.equals("")) {
                            temp.add(new ExtData(PacketType.packets[j].extName, PacketType.packets[j].Version));
                          }
                        }
                      ...
                      if (packetType == PacketType.SELECTION_CUBOID) {
                        ...
                        SelectionBoxData data = new SelectionBoxData(ID, Name, new ColorCache(r / 255F, g / 255F, b / 255F, a / 255F), new CustomAABB(X1, Y1, Z1, X2, Y2, Z2));
                        selectionBoxes.add(data);
                      } else {
                        if (packetType == PacketType.REMOVE_SELECTION_CUBOID) {
                          byte ID = (Byte) packetParams[0];
                          ...
                          if (packetType == PacketType.ENV_SET_COLOR) {
                            ...
                            if (packetType == PacketType.ENV_SET_MAP_APPEARANCE) {
                              ...
                              if (packetType == PacketType.CLICK_DISTANCE) {
                                short Distance = (Short) packetParams[0];
                              } else {
                                if (packetType == PacketType.HOLDTHIS) {
                                  ...
                                  if (packetType == PacketType.SET_TEXT_HOTKEY) {
                                    ...
                                    if (packetType == PacketType.EXT_ADD_PLAYER_NAME) {
                                      ...
                                      if (packetType == PacketType.EXT_ADD_ENTITY) {
                                        ...
                                        if (packetType == PacketType.EXT_REMOVE_PLAYER_NAME) {
                                          Short NameId = (Short) packetParams[0];
                                          ...
                                          if (packetType == PacketType.CUSTOM_BLOCK_SUPPORT_LEVEL) {
                                            ...
                                            networkManager.netHandler.send(PacketType.CUSTOM_BLOCK_SUPPORT_LEVEL, Constants.CUSTOM_BLOCK_SUPPORT_LEVEL);
                                            SessionData.setAllowedBlocks(SupportLevel);
                                          } else {
                                            if (packetType == PacketType.SET_BLOCK_PERMISSIONS) {
                                              ...
                                              if (packetType == PacketType.CHANGE_MODEL) {
                                                byte PlayerID = (Byte) packetParams[0];
                                                ...
                                                if (packetType == PacketType.ENV_SET_WEATHER_TYPE) {
...
<<<<<<< /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/left.java
                                                    networkManager.minecraft.player.userType = 100
=======
...
>>>>>>> /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/right.java
                                                    ...
                                                    if (packetType == PacketType.LEVEL_INIT) {
                                                      ...
                                                      if (packetType == PacketType.LEVEL_DATA) {
                                                        ...
                                                        if (packetType == PacketType.LEVEL_FINALIZE) {
                                                          try {
                                                            networkManager.levelData.close();
                                                          } catch (IOException ex) {
                                                            LogUtil.logError("Error receiving level data.", ex);
                                                          }
                                                          byte[] decompressedStream = LevelLoader.decompress(new ByteArrayInputStream(networkManager.levelData.toByteArray()));
                                                          ...
                                                          if (packetType == PacketType.BLOCK_CHANGE) {
                                                            if (networkManager.minecraft.level != null) {
                                                              networkManager.minecraft.level.netSetTile((Short) packetParams[0], (Short) packetParams[1], (Short) packetParams[2], (Byte) packetParams[3]);
                                                            }
                                                          } else {
                                                            byte var9;
                                                            String var34;
                                                            ...
                                                            if (packetType == PacketType.SPAWN_PLAYER) {
                                                              ...
                                                              if (var5 >= 0) {
                                                                ...
                                                                var33 = new NetworkPlayer(networkManager.minecraft, var34, var36, var47, var10005, var58 * 360 / 256F, var9 * 360 / 256F);
                                                                networkManager.players.put(var5, var33);
                                                                networkManager.minecraft.level.addEntity(var33);
                                                              } else {
                                                                networkManager.minecraft.level.setSpawnPos(var36 / 32, var47 / 32, var10005 / 32, var9 * 320 / 256);
                                                                networkManager.minecraft.player.moveTo(var36 / 32F, var47 / 32F, var10005 / 32F, var9 * 360 / 256F, var58 * 360 / 256F);
                                                              }
                                                            } else {
                                                              byte var53;
...
<<<<<<< /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/left.java
                                                                  if (!settings.TPDisabled) {
                                                                    networkManager.minecraft.player.moveTo(var38 / 32.0F, var36 / 32.0F, var47 / 32.0F, var53 * 360 / 256.0F, var9 * 360 / 256.0F);
                                                                  }
=======
                                                                  networkManager.minecraft.player.moveTo(var66 / 32F, var36 / 32F, var47 / 32F, var53 * 360 / 256F, var9 * 360 / 256F);
>>>>>>> /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/right.java
                                                                } else {
                                                                  var53 = (byte) (var53 + 128);
                                                                  var36 = (short) (var36 - 22);
                                                                  if ((networkPlayer = networkManager.players.get(Byte.valueOf(var5))) != null) {
                                                                    networkPlayer.teleport(var66, var36, var47, var9 * 360 / 256F, var53 * 360 / 256F);
                                                                  }
                                                                }
                                                              } else {
                                                                byte var37;
...
<<<<<<< /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/left.java
                                                                      var53 = (byte) (var53 + 128)
=======
                                                                      networkPlayerInstance.queue(var37, var44, var49, var9 * 360 / 256F, var53 * 360 / 256F)
>>>>>>> /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/right.java
                                                                      ;
                                                                      if ((networkPlayer = networkManager.players.get(Byte.valueOf(playerID))) != null) {
                                                                        networkPlayer.queue(var37, var44, var49, var53 * 360 / 256.0F, var9 * 360 / 256.0F);
                                                                      }
                                                                    }
                                                                  }
                                                                } else {
                                                                  if (packetType == PacketType.ROTATION_UPDATE) {
                                                                    byte playerID = (Byte) packetParams[0];
                                                                    var37 = (Byte) packetParams[1];
                                                                    var44 = (Byte) packetParams[2];
                                                                    if (playerID >= 0) {
...
>>>>>>> /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/right.java
                                                                    ...
                                                                    if (packetType == PacketType.POSITION_UPDATE) {
                                                                      byte playerID = (Byte) packetParams[0];
                                                                      NetworkPlayer networkPlayerInstance = networkManager.players.get(Byte.valueOf(playerID));
...
>>>>>>> /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/right.java
                                                                      ...
                                                                      if (packetType == PacketType.DESPAWN_PLAYER) {
                                                                        byte playerID = (Byte) packetParams[0];
                                                                        ...
                                                                        if (packetType == PacketType.CHAT_MESSAGE) {
                                                                          ...
                                                                          if (packetType == PacketType.DISCONNECT) {
                                                                            ...
                                                                            networkManager.minecraft.setCurrentScreen(new ErrorScreen("Connection lost", (String) packetParams[0]));
                                                                          } else {
                                                                            if (packetType == PacketType.UPDATE_PLAYER_TYPE) {
...
>>>>>>> /usr/src/app/output/andrewphorn/classicube-client/268948793e799134b9a023699c3f118f381d8447/src/main/java/com/mojang/minecraft/Minecraft.java/right.java
        ...
        if (networkManager.levelLoaded) {
          ...
          var20.netHandler.send(PacketType.POSITION_ROTATION, canSendHeldBlock ? player.inventory.getSelected() : -1, var24, var4, var40, var46, var45);
        }
      }
    }
    if (isLoadingMap) {
        ...
        while (Mouse.next()) {
          ...
          if (currentScreen == null) {
            if (!hasMouse && Mouse.getEventButtonState()) {
              ...
              if (Mouse.getEventButton() == 2 && Mouse.getEventButtonState() && selected != null) {
                var16 = level.getTile(selected.x, selected.y, selected.z);
                player.inventory.grabTexture(var16, gamemode instanceof CreativeGameMode);
              }
            }
          }
          if (currentScreen != null) {
        ...
        while (Keyboard.next()) {
          ...
          if (Keyboard.getEventKeyState()) {
            ...
            if (currentScreen == null) {
              ...
              if (gamemode instanceof CreativeGameMode) {
                if (HackState.Noclip || HackState.Fly || HackState.Speed) {
                  ...
                  if (Keyboard.getEventKey() == settings.saveLocationKey.key) {
                    level.setSpawnPos((int) player.x, (int) player.y, (int) player.z, player.yRot);
                    player.resetPos();
                  }
                }
              }
              ...
              if (Keyboard.getEventKey() == 53 && networkManager != null && networkManager.isConnected()) {
                player.releaseAllKeys();
              ...
              if (settings.HacksEnabled) {
                if (settings.HackType == 0) {
                  if (Keyboard.getEventKey() == Keyboard.KEY_X) {
                    if (HackState.Noclip || HackState.Noclip && player.userType >= 100) {
              ...
              if (Keyboard.getEventKey() == 15 && gamemode instanceof SurvivalGameMode && player.arrows > 0) {
                level.addEntity(new Arrow(level, player, player.x, player.y, player.z, player.yRot, player.xRot, 1.2F));
                --player.arrows;
              }
              if (Keyboard.getEventKey() == settings.inventoryKey.key) {
                gamemode.openInventory();
              }
              if (Keyboard.getEventKey() == settings.chatKey.key && networkManager != null && networkManager.isConnected()) {
                player.releaseAllKeys();
            ...
            if (Keyboard.getEventKey() == settings.toggleFogKey.key) {
              settings.toggleSetting(4, !Keyboard.isKeyDown(42) && !Keyboard.isKeyDown(54) ? 1 : -1);
            }
          }
        }
        if (currentScreen == null) {
      ...
      if (renderer.minecraft.isRaining) {
        ...
        for (i = 0; i < 50; ++i) {
          ...
          if (var57 <= var46 + 4 && var57 >= var46 - 4) {
            ...
            renderer.minecraft.particleManager.spawnParticle(new WaterDropParticle(var32, var60 + var56, var57 + 0.1F, var52 + var62));
          }
        }
      }
      ++levelRenderer.ticks;
    ...
    if (currentScreen instanceof BlockSelectScreen) {
      ...
      if (currentScreen == null) {
        while (Mouse.next()) {
          if (currentScreen == null) {
            ...
            if (!hasMouse && Mouse.getEventButtonState()) {
              ...
              if (Mouse.getEventButton() == MB_MIDDLE && Mouse.getEventButtonState() && selected != null) {
        ...
        if (!gamemode.instantBreak && punchingCooldown <= 0) {
          if ((currentScreen == null) && Mouse.isButtonDown(MB_LEFT) && hasMouse && selected != null && !selected.hasEntity) {
