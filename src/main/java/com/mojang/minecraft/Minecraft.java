...
public final class Minecraft implements Runnable {
		...
		try {
			...
			if (gamemode instanceof CreativeGameMode) {
			    if ((var11 = var1.levelIo
				    .load((new FileInputStream(new File(mcDir,
					    "levelc.dat"))))) != null) {
				var1.setLevel(var11);
			    }
	...
	try {
		...
		if (this.waiting) {
			...
			double var95;
			var24 = ((var95 = var20 / 1000.0D) - var63.lastHR)
				* var63.adjustment;
			var63.lastHR = var95;
			if (var24 < 0.0D) {
			    var24 = 0.0D;
			...
			var63.elapsedDelta = (float) (var63.elapsedDelta + var24
				* var63.speed * var63.tps);
				...
				if (renderer.minecraft.level != null) {
					...
					for (int i = 0; i < this.selectionBoxes
					    ...
					    CustomAABB bounds = this.selectionBoxes
						    .get(i).Bounds;
					    ColorCache color = this.selectionBoxes
						    .get(i).Color;
					    GL11.glLineWidth(2);
					    ...
					    sr.begin();
					    sr.vertex(bounds.x0, bounds.y0,
						    bounds.z1);
					    // Bottom Right
					    sr.vertex(bounds.x1, bounds.y0,
						    bounds.z1);
					    // Top Right
					    sr.vertex(bounds.x1, bounds.y1,
						    bounds.z1);
					    // Top Left
					    sr.vertex(bounds.x0, bounds.y1,
						    bounds.z1);
