...
public class Reflector {
  ...
  @SuppressWarnings(value = { "unchecked" }) static private <T extends Throwable> void sneakyThrow0(final Throwable t) throws T {
    throw (T) t;
  }

  private static String noMethodReport(final String methodName, final Object target, final Object[] args) {
  ...
  static Object invokeMatchingMethod(final String methodName, final List methods, final Object target, final Object[] args) {
    Method m = null;
   ...
   */
  public static ConcurrentHashMap<Class<?>, Reference<Map<String, PropertyFoundResult>>> cachedProperties = new ConcurrentHashMap<Class<?>, Reference<Map<String, PropertyFoundResult>>>();

  /**
   * instance fields setter caching.
   */
  public static ConcurrentHashMap<Class<?>, Reference<Map<String, PropertyFoundResult>>> cachedSettters = new ConcurrentHashMap<Class<?>, Reference<Map<String, PropertyFoundResult>>>();
   ...
   */
  public static ConcurrentHashMap<Class<?>, Reference<Map<String, PropertyFoundResult>>> cachedMethods = new ConcurrentHashMap<Class<?>, Reference<Map<String, PropertyFoundResult>>>();
  ...
  public static Object fastGetProperty(final Object obj, final String name, final PropertyType type) {
    ...
    try {
      ...
      if (result.handle != null) {
        Object ret = type == PropertyType.StaticField ? result.handle.invoke() : result.handle.invoke(obj);
        if (result.isBooleanType && !(ret instanceof Boolean)) {
          putDummyHandle(name, results);
          return throwNoSuchPropertyException("Property `" + name + "` not found in java bean: " + obj);
        }
        return ret;
      } else {
        if (type == PropertyType.StaticMethod) {
  ...
  private static PropertyFoundResult retrieveStaticFieldHandle(final Map<String, PropertyFoundResult> results, final Class<?> clazz, final String name) throws IllegalAccessException, NoSuchFieldException {
  ...
  private static PropertyFoundResult retrieveGetterHandle(final Map<String, PropertyFoundResult> results, final Class<?> clazz, final String name) throws IllegalAccessException {
    PropertyFoundResult result;
  ...
  private static PropertyFoundResult retrieveSetterHandle(final Map<String, PropertyFoundResult> results, final Class<?> clazz, final String name) throws IllegalAccessException {
    PropertyFoundResult result;
  ...
  private static void putDummyHandle(final String name, final Map<String, PropertyFoundResult> handles) {
    handles.put(name, new PropertyFoundResult(null, false));
  }

  private static Map<String, PropertyFoundResult> getClassPropertyResults(final ConcurrentHashMap<Class<?>, Reference<Map<String, PropertyFoundResult>>> cache, final ReferenceQueue<Map<String, PropertyFoundResult>> rq, final Class<?> clazz) {
  ...
  public static List<Method> getStaticMethods(final Class<?> c, final String methodName) {
    ...
    for (Method method : c.getMethods()) {
      ...
      if (Modifier.isStatic(modifiers) && Modifier.isPublic(modifiers) && methodName.equals(method.getName())) {
        ret.add(method);
      }
    }
    return ret;
  ...
  static <K extends java.lang.Object, V extends java.lang.Object> void clearCache(final ReferenceQueue<V> rq, final ConcurrentHashMap<K, Reference<V>> cache) {
    if (rq.poll() != null) {
      while (rq.poll() != null) {
        ;
      }
  ...
  public static Object invokeStaticMethod(final Class<?> c, final String methodName, final List<Method> methods, final Object[] args) {
    return invokeMatchingMethod(methodName, methods, null, args);
  }

  public static Object invokeInstanceMethod(final Class<?> c, final String methodName, final Object target, final List<Method> methods, final Object[] args) {
  ...
  private static Set<Class<?>> longClasses = asSet(Long.class, long.class, Integer.class, int.class, Byte.class, byte.class, Short.class, short.class, Byte.class, byte.class);

  private static Set<Class<?>> doubleClasses = asSet(Double.class, double.class, Float.class, float.class);
  ...
  @SuppressWarnings(value = { "unchecked" }) public static Object getProperty(final Object target, final String name) {
    final String[] names = Constants.SPLIT_PAT.split(name);
    return fastGetProperty(name, names, null, (target instanceof Map) ? Target.withEnv((Map) target) : Target.withObject(target), false, 0, names.length);
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) public static Object fastGetProperty(final String name, final String[] names, final Map<String, Object> env, final Target target, final boolean tryResolveStaticMethod, final int offset, final int len) {
    ...
    for (int i = offset; i < max; i++) {
      ...
      if (target.innerClazz != null) {
        ...
        if (tryResolveStaticMethod && instance.isFeatureEnabled(Feature.StaticMethods) && names.length == 2) {
          ...
          if (instance.isFeatureEnabled(Feature.StaticFields)) {
  ...
  public static Map<String, List<Method>> findMethodsFromClass(final Class<?> clazz, final boolean isStatic) {
