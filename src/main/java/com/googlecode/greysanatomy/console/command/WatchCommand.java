...
@RiscCmd(named = "watch", sort = 4, desc = "The call context information buried point observation methods.", eg = { "watch -b org\\.apache\\.commons\\.lang\\.StringUtils isEmpty p.params[0]", "watch -f org\\.apache\\.commons\\.lang\\.StringUtils isEmpty p.returnObj", "watch -bf .*StringUtils isEmpty p.params[0]", "watch .*StringUtils isEmpty p.params[0]" }) public class WatchCommand extends Command {
  ...
  @RiscIndexArg(index = 2, name = "express", description = "expression, write by javascript. use \'p.\' before express", description2 = "" + " \n" + "For example\n" + "    : p.params[0]\n" + "    : p.params[0]+p.params[1]\n" + "    : p.returnObj\n" + "    : p.throwExp\n" + "    : p.target.targetThis.getClass()\n" + " \n" + "The structure of \'p\'\n" + "    p.\n" + "    \\+- params[0..n] : the parameters of methods\n" + "    \\+- returnObj    : the return object of methods\n" + "    \\+- throwExp     : the throw exception of methods\n" + "    \\+- target\n" + "         \\+- targetThis  : the object entity\n" + "         \\+- targetClass : the object\'s class") private String expression;

  @RiscNamedArg(named = "s", description = "is watch on success") private boolean isSuccess = false;

  @RiscNamedArg(named = "b", description = "is watch on before") private boolean isBefore = true;

  @RiscNamedArg(named = "f", description = "is watch on finish") private boolean isFinish = false;

  @RiscNamedArg(named = "e", description = "is watch on exception") private boolean isException = false;

  @Override public Action getAction() {
    return new Action() {
      @Override public void action(final ConsoleServer consoleServer, Info info, final Sender sender) throws Throwable {
...
<<<<<<< /usr/src/app/output/oldmanpushcart/greys-anatomy/26f8fde516d364d9aadbda98ad78f6c4255470e3/src/main/java/com/googlecode/greysanatomy/console/command/WatchCommand.java/left.java
            if (watchPoint == WatchPointEnum.before && isBefore) {
              try {
...
>>>>>>> /usr/src/app/output/oldmanpushcart/greys-anatomy/26f8fde516d364d9aadbda98ad78f6c4255470e3/src/main/java/com/googlecode/greysanatomy/console/command/WatchCommand.java/right.java
          }

          @Override public void onFinish(Advice p) {

<<<<<<< /usr/src/app/output/oldmanpushcart/greys-anatomy/26f8fde516d364d9aadbda98ad78f6c4255470e3/src/main/java/com/googlecode/greysanatomy/console/command/WatchCommand.java/left.java
            if (watchPoint == WatchPointEnum.finish || isFinish) {
              try {
                invoke.invokeFunction("printWatch", p, sender);
              } catch (Exception e) {
              }
...
>>>>>>> /usr/src/app/output/oldmanpushcart/greys-anatomy/26f8fde516d364d9aadbda98ad78f6c4255470e3/src/main/java/com/googlecode/greysanatomy/console/command/WatchCommand.java/right.java
          ...
          @Override public void onException(Advice p) {
            if (isException) {
              try {
                invoke.invokeFunction("printWatch", p, sender);
              } catch (Exception e) {
          ...
          @Override public void onSuccess(Advice p) {
            if (isSuccess) {
              try {
                invoke.invokeFunction("printWatch", p, sender);
              } catch (Exception e) {
