...
@RiscCmd(named = "profiler", sort = 6, desc = "The call stack output buried point method for rendering path of.", eg = { "profiler -c 50 org\\.apache\\.commons\\..* .* org\\.apache\\.commons\\.lang\\.StringUtils isEmpty", "profiler -c 50 org\\.apache\\.commons\\..* .* .*StringUtils isEmpty" }) public class ProfilerCommand extends Command {
  @RiscIndexArg(index = 0, name = "rendering-class-regex", description = "regex match of rendering classpath.classname") private String classRegex;

  @RiscIndexArg(index = 1, name = "rendering-method-regex", description = "regex match of rendering methodname") private String methodRegex;

  @RiscIndexArg(index = 2, name = "class-regex", description = "regex match of classpath.classname") private String probeClassRegex;

  @RiscIndexArg(index = 3, name = "method-regex", description = "regex match of methodname") private String probeMethodRegex;

  @RiscNamedArg(named = "c", hasValue = true, description = "the cost limit for output") private long cost;
      ...
      @Override public void action(final ConsoleServer consoleServer, Info info, final Sender sender) throws Throwable {
        ...
        final AdviceListenerAdapter advice = new AdviceListenerAdapter() {
          ...
          @Override public void onFinish(Advice p) {
            ...
            if (0 == deep.get()) {
              ...
              if (cost >= ProfilerCommand.this.cost) {
                final StringBuilder dumpSB = new StringBuilder().append("Thread Info:").append(Thread.currentThread().getName()).append("\n").append(dump).append("\n\n");
          ...
          private boolean isEntered(Advice p) {
            ...
            final String cmKey = new StringBuilder().append(p.getTarget().getTargetClassName()).append("#").append(p.getTarget().getTargetBehaviorName()).toString();
            if (cmCache.containsKey(cmKey)) {
              return cmCache.get(cmKey);
            } else {
              final boolean isProbe = p.getTarget().getTargetClassName().matches(probeClassRegex) && p.getTarget().getTargetBehaviorName().matches(probeMethodRegex);
        ...
        final TransformResult result = transform(inst, classRegex, methodRegex, advice, info, new GreysAnatomyClassFileTransformer.Progress() {
          ...
          @Override public void progress(int index, int total) {
            if (total > 0) {
              final int step;
              if (total < 500) {
                step = 10;
