 ...
 * @author vlinux
 */
@RiscCmd(named = "js", sort = 3, desc = "Let Greys use the JavaScript enhancement.", eg = { "js -f /tmp/debug.js org\\.apache\\.commons\\.lang\\.StringUtils isEmpty", "js -f /tmp/debug.js .*StringUtils isEmpty" }) public class JavaScriptCommand extends Command {
  ...
  @RiscNamedArg(named = "f", hasValue = true, description = "the file of javascript") private File scriptFile;
  ...
  @Override public Action getAction() {
    return new Action() {
      @Override public void action(final ConsoleServer consoleServer, final Info info, final Sender sender) throws Throwable {
        if (!scriptFile.isFile() || !scriptFile.exists() || !scriptFile.canRead()) {
        ...
        final TransformResult result = transform(inst, classRegex, methodRegex, new AdviceListenerAdapter() {
          @Override public void onBefore(final Advice p) {
            try {
          ...
          @Override public void onSuccess(final Advice p) {
            try {
          ...
          @Override public void onException(final Advice p) {
            try {
          ...
          @Override public void onFinish(final Advice p) {
            try {
