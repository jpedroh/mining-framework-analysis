...
public class IteratorAggregation {
  ...
  public static IteratingRLW not(final IteratingRLW x) {
    return new IteratingRLW() {
      ...
      @Override public long getLiteralWordAt(int index) {
        return ~x.getLiteralWordAt(index);
      }

      @Override public int getNumberOfLiteralWords() {
        return x.getNumberOfLiteralWords();
      }

      @Override public boolean getRunningBit() {
      ...
      @Override public long getRunningLength() {
        return x.getRunningLength();
      }

      @Override public void discardFirstWords(long y) {
        x.discardFirstWords(y);
      }

      @Override public IteratingRLW clone() throws CloneNotSupportedException {
  ...
  /**
	 ...
	 *            the iterator
	 */
  protected static void dischargeAsEmpty(final BitmapStorage container, final IteratingRLW i) {
    while (i.size() > 0) {
  ...
  static void andToContainer(final BitmapStorage container, int desiredrlwcount, final IteratingRLW rlwi, IteratingRLW rlwj) {
  ...
  static void andToContainer(final BitmapStorage container, final IteratingRLW rlwi, IteratingRLW rlwj) {
  ...
  /**
	 ...
	 */
  public static void xorToContainer(final BitmapStorage container, int desiredrlwcount, final IteratingRLW rlwi, final IteratingRLW rlwj) {
    while ((rlwi.size() > 0) && (rlwj.size() > 0) && (desiredrlwcount-- > 0)) {
      while ((rlwi.getRunningLength() > 0) || (rlwj.getRunningLength() > 0)) {
        final boolean i_is_prey = rlwi.getRunningLength() < rlwj.getRunningLength();
        final IteratingRLW prey = i_is_prey ? rlwi : rlwj;
...
class BufferedORIterator implements CloneableIterator<EWAHIterator> {
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override public BufferedXORIterator clone() throws CloneNotSupportedException {
  ...
  @Override public EWAHIterator next() {
    this.buffer.clear();
    long effective = 0;
...
class BufferedAndIterator implements CloneableIterator<EWAHIterator> {
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override public BufferedAndIterator clone() throws CloneNotSupportedException {
