...
public class ManufacturerSpecificCommandClass extends CommandClass {
  ...
  @Override public void onApplicationCommand(NodeContext context, byte[] ccb, int startIndex) {
  ...
  ProductInfo parseManufacturerSpecificData(byte[] ccb, int startIndex) throws CommandClassParseException {
    if (ccb.length >= startIndex + 7) {
      return ProductRegistry.lookupProduct(ByteUtil.convertTwoBytesToInt(ccb[startIndex + 2], ccb[startIndex + 3]), ByteUtil.convertTwoBytesToInt(ccb[startIndex + 4], ccb[startIndex + 5]), ByteUtil.convertTwoBytesToInt(ccb[startIndex + 6], ccb[startIndex + 7]));
    } else {
      throw new CommandClassParseException("Manufacturer data is too short");
    }
  }

  @Override public int queueStartupMessages(NodeContext context, byte nodeId) {
    if (productInfo == null) {
  ...
  @Override public Map<String, Object> restore(PersistenceContext ctx, byte nodeId) {
    Map<String, Object> map = super.restore(ctx, nodeId);
    productInfo = ProductRegistry.lookupProduct((Integer) map.get("manufacturerId"), (Integer) map.get("productTypeId"), (Integer) map.get("productId"));
    logger.debug("Restored manufacturer information: {}", productInfo);
    return map;
  }

  @Override public Map<String, Object> save(PersistenceContext ctx, byte nodeId) {
  ...
  @Override public String toString() {
    return "ManufacturerSpecificCommandClass{" + "version=" + getVersion() + ", productInfo=" + productInfo + '}';
  }
}