...
public class Consumer implements Runnable {
  ...
  private Connection createRabbitMQConnection(Config config) throws IOException {
    ...
    config = config.withRecoveryPolicy(RecoveryPolicies.recoverAlways()).withRetryPolicy(new RetryPolicy().withMaxAttempts(200).withInterval(Duration.seconds(1)).withMaxDuration(Duration.minutes(5)));
    ConnectionOptions options = new ConnectionOptions().withHost(hostname).withPort(port).withVirtualHost(virtualHost).withUsername(username).withPassword(password);
    if (sslEnabled) {
      try {
