...
public abstract class UploadService extends StoragePluginService<UploadServiceData> {
  ...
  private ExecutorService executorService = new ThreadPoolExecutor(UPLOAD_THREAD_COUNT, UPLOAD_THREAD_COUNT, KEEP_ALIVE_TIME, TimeUnit.SECONDS, new LinkedBlockingDeque<Runnable>());
  ...
  class FileUploadThread implements Runnable {
    ...
    @Override public void run() {
      try {
        ...
        azureBlob = new AzureBlob(uploadItem.getShareName(), uploadItem.getFileUrl(), uploadedFileHash, filePath.length(), Constants.FILE_STORAGE, getServiceData().getCredentialsId());
        filesUploaded.addAndGet(1);
        azureBlobs.add(azureBlob);
      } catch (WAStorageException | InterruptedException | IOException e) {
        LOGGER.log(Level.SEVERE, e.getMessage(), e);
        final String message = Messages.AzureStorageBuilder_download_err(getServiceData().getStorageAccountInfo().getStorageAccName()) + ":" + e.getMessage();
  ...
  protected static class UploadResult implements Serializable {
    ...
    /**
         ...
         */
    public UploadResult(int statusCode, String responseBody, String fileHash, String name, String url, long byteSize, String storageType, long startTime, long endTime) {
  ...
  static final class UploadOnSlave extends MasterToSlaveFileCallable<List<UploadResult>> {
    ...
    @Override public List<UploadResult> invoke(File f, VirtualChannel channel) throws IOException, InterruptedException {
      ExecutorService agentExecutorService = new ThreadPoolExecutor(UPLOAD_THREAD_COUNT, UPLOAD_THREAD_COUNT, KEEP_ALIVE_TIME, TimeUnit.SECONDS, new LinkedBlockingDeque<>());
      List<Future<UploadResult>> futures = new ArrayList<>();
      for (UploadObject uploadObject : uploadObjects) {
        Future<UploadResult> future = agentExecutorService.submit(new UploadThread(proxy, uploadObject));
        futures.add(future);
      }
  ...
  protected void updateAzureBlobs(List<UploadResult> results, List<AzureBlob> azureBlobs) throws WAStorageException {
    for (UploadResult result : results) {
      if (result.getStatusCode() == HttpStatus.SC_CREATED) {
        ...
        AzureBlob azureBlob = new AzureBlob(result.getName(), result.getUrl(), result.getFileHash(), result.getByteSize(), result.getStorageType(), serviceData.getCredentialsId());
        filesUploaded.addAndGet(1);
        azureBlobs.add(azureBlob);
  ...
  protected String generateWriteSASURL(StorageAccountInfo storageAccountInfo, String fileName, String storageType, String name) throws Exception {
    if (storageType.equalsIgnoreCase(Constants.BLOB_STORAGE)) {
      ...
      if (storageType.equalsIgnoreCase(Constants.FILE_STORAGE)) {
  ...
  static class UploadThread implements Callable<UploadResult> {
    ...
    private BlobServiceClient getBlobServiceClient() {
      return new BlobServiceClientBuilder().credential(new AzureSasCredential(uploadObject.getSas())).httpClient(HttpClientRetriever.get(proxyConfiguration)).endpoint(uploadObject.getUrl()).buildClient();
    }

    @Override public UploadResult call() {
      ...
      BlobClient blockBlobClient = containerClient.getBlobClient(uploadObject.getName());
      long startTime = System.currentTimeMillis();
      BlobUploadFromFileOptions options = new BlobUploadFromFileOptions(file.getAbsolutePath()).setHeaders(getBlobHttpHeaders()).setMetadata(uploadObject.getMetadata());
      Response<BlockBlobItem> block = blockBlobClient.uploadFromFileWithResponse(options, null, Context.NONE);
      ...
      return new UploadResult(block.getStatusCode(), null, new String(md5, StandardCharsets.UTF_8), uploadObject.getName(), uploadObject.getUrl(), length, uploadObject.getStorageType(), startTime, endTime);
    }
  ...
  @Override public final int execute() throws WAStorageException {
        ...
        if (fileName != null && fileName.contains("::")) {
          ...
          if (fileName.length() > embVPSepIndex + 1) {
            ...
            if (StringUtils.isBlank(embeddedVP)) {
              ...
              if (!embeddedVP.endsWith(Constants.FWD_SLASH)) {
    ...
    if (serviceData.getUploadType() != UploadType.ZIP && filesUploaded.get() != filesNeedUpload) {
      throw new WAStorageException(String.format("Only %d/%d files are successfully uploaded.", filesUploaded.get(), filesNeedUpload));
    }
  ...
  protected String uploadCloudFile(ShareFileClient fileClient, FilePath localPath) throws WAStorageException {
    long startTime = System.currentTimeMillis();
