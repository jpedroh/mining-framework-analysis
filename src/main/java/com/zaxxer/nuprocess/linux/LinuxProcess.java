...
public class LinuxProcess extends BasePosixProcess {
  ...
  private void prepareProcess(List<String> command, String[] environment, Path cwd) {
    ...
    createPipes();
    int[] child_fds = new int[]{ stdinWidow, stdoutWidow, stderrWidow };
    if (JVM_MAJOR_VERSION >= 10) {
      ...
      pid = // used on Linux
      /* redirectErrorStream */
      com.zaxxer.nuprocess.internal.LibJava8.Java_java_lang_UNIXProcess_forkAndExec(JNIEnv.CURRENT, this, LaunchMechanism.VFORK.ordinal() + 1, toCString(System.getProperty("java.home") + "/lib/jspawnhelper"), toCString(cmdarray[0]), argBlock, args.length, envBlock, environment.length, cwd != null ? toCString(cwd.toString()) : null, child_fds, ((byte) (0)));
    }
  }
