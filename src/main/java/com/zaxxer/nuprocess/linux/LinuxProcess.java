...
public class LinuxProcess extends BasePosixProcess {
  ...
  @SuppressWarnings(value = { "unused" }) private enum LaunchMechanism {
    FORK,
    POSIX_SPAWN,
    VFORK
  }
  ...
  @Override public NuProcess start(List<String> command, String[] environment, Path cwd) {
    callPreStart();
    try {
...
=======
      if (JVM_MAJOR_VERSION >= 10) {
        pid = com.zaxxer.nuprocess.internal.LibJava10.Java_java_lang_ProcessImpl_forkAndExec(JNIEnv.CURRENT, this, LaunchMechanism.VFORK.ordinal() + 1, toCString(System.getProperty("java.home") + "/lib/jspawnhelper"), toCString(cmdarray[0]), argBlock, args.length, envBlock, environment.length, (cwd != null ? toCString(cwd.toString()) : null), child_fds, (byte) 0);
      } else {
        pid = com.zaxxer.nuprocess.internal.LibJava8.Java_java_lang_UNIXProcess_forkAndExec(JNIEnv.CURRENT, this, LaunchMechanism.VFORK.ordinal() + 1, toCString(System.getProperty("java.home") + "/lib/jspawnhelper"), toCString(cmdarray[0]), argBlock, args.length, envBlock, environment.length, (cwd != null ? toCString(cwd.toString()) : null), child_fds, (byte) 0);
      }
>>>>>>> /usr/src/app/output/brettwooldridge/nuprocess/f2f0b7eab313c7df153843f68dc79075cb7126ee/src/main/java/com/zaxxer/nuprocess/linux/LinuxProcess.java/right.java
  ...
  @Override public void run(List<String> command, String[] environment, Path cwd) {
    callPreStart();
    try {
      prepareProcess(command, environment, cwd);
      if (pid == -1) {
  ...
  @Override protected boolean checkLaunch() {
