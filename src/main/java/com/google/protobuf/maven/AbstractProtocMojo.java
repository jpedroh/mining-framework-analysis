    ...
    if (protoSourceRoot.exists()) {
      try {
        ...
        if (protoFiles.isEmpty()) {
          ...
          if (!hasDelta(protoFiles)) {
            getLog().info("Skipping compilation because build context has no changes.");
            attachFiles();
          } else {

<<<<<<< /usr/src/app/output/dtrott/maven-protoc-plugin/281edcc995e2f5511028659d923d7fe3d64cb2f1/src/main/java/com/google/protobuf/maven/AbstractProtocMojo.java/left.java
            if (checkStaleness && checkFilesUpToDate(protoFiles, outputFiles)) {
              getLog().info("Skipping compilation because target directory newer than sources.");
              attachFiles();
            } else {
              ImmutableSet<File> derivedProtoPathElements = makeProtoPathFromJars(temporaryProtoFileDirectory, getDependencyArtifactFiles());
              ...
              Toolchain tc = toolchainManager.getToolchainFromBuildContext("protobuf", session);
              if (tc != null) {
                getLog().info("Toolchain in protoc-plugin: " + tc);
                if (protocExecutable != null) {
                  getLog().warn("Toolchains are ignored, \'protocExecutable\' parameter is set to " + protocExecutable);
                } else {
                  protocExecutable = tc.findTool("protoc");
                }
              }
              if (protocExecutable == null) {
              ...
              final Protoc.Builder protocBuilder = new Protoc.Builder(protocExecutable, outputDirectory).addProtoPathElement(protoSourceRoot).addProtoPathElements(derivedProtoPathElements).addProtoPathElements(asList(additionalProtoPathElements)).addProtoFiles(protoFiles);
...
=======
            {
              ...
              final Protoc.Builder protocBuilder = new Protoc.Builder(protocExecutable).addProtoPathElement(protoSourceRoot).addProtoPathElements(derivedProtoPathElements).addProtoPathElements(asList(additionalProtoPathElements)).addProtoFiles(protoFiles);
              addProtocBuilderParameters(protocBuilder);
              final Protoc protoc = protocBuilder.build();
              if (getLog().isDebugEnabled()) {
                getLog().debug("Proto source root:");
...
>>>>>>> /usr/src/app/output/dtrott/maven-protoc-plugin/281edcc995e2f5511028659d923d7fe3d64cb2f1/src/main/java/com/google/protobuf/maven/AbstractProtocMojo.java/right.java
      ...
      getLog().info(format("%s does not exist. Review the configuration or consider disabling the plugin.", protoSourceRoot));
    }
  }
      ...
      if (classpathElementFile.isFile() && classpathElementFile.canRead() && !classpathElementFile.getName().endsWith(".xml")) {
        ...
        while (jarEntries.hasMoreElements()) {
          ...
          if (jarEntryName.endsWith(PROTO_FILE_SUFFIX)) {
            final File jarDirectory = new File(temporaryProtoFileDirectory, truncatePath(classpathJar.getName()));
  ...
  protected ImmutableSet<File> findProtoFilesInDirectory(File directory) throws IOException {
    ...
    final List<File> protoFilesInDirectory = getFiles(directory, joiner.join(includes), joiner.join(excludes));
