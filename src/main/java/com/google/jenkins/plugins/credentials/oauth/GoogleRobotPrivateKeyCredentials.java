...
@NameWith(value = GoogleRobotNameProvider.class, priority = 50) public final class GoogleRobotPrivateKeyCredentials extends GoogleRobotCredentials {
   ...
   * @param module The module for instantiating dependent objects, or null.
   */
  @DataBoundConstructor public GoogleRobotPrivateKeyCredentials(@CheckForNull CredentialsScope scope, String id, String projectId, ServiceAccountConfig serviceAccountConfig, @Nullable GoogleRobotCredentialsModule module) throws Exception {
    super(scope, id, projectId, module);
    this.serviceAccountConfig = serviceAccountConfig;
  }

  @SuppressWarnings(value = { "deprecation" }) @SuppressFBWarnings(value = "RCN_REDUNDANT_NULLCHECK_OF_NONNULL_VALUE", justification = "for migrating older credentials that did not have a separate id field, and would really " + "have a null id when attempted to deserialize. readResolve overwrites these nulls") public Object readResolve() throws Exception {
    ...
    return new GoogleRobotPrivateKeyCredentials(getCredentialsScope() == null ? CredentialsScope.GLOBAL : getCredentialsScope(), getId() == null ? getProjectId() : getId(), getProjectId(), serviceAccountConfig, getModule());
  }
  ...
  @SuppressWarnings(value = { "deprecation" }) private String getClientEmailFromSecretsFile() throws MissingSecretsFileException, SecretsFileNotFoundException, InvalidSecretsFileException {
    if (secretsFile == null) {
      throw new MissingSecretsFileException();
    }
    LegacyJsonKey legacyJsonKey;
    try {
      legacyJsonKey = LegacyJsonKey.load(getModule().getJsonFactory(), new FileInputStream(secretsFile));
  ...
  public static List<ServiceAccountConfig.Descriptor> getServiceAccountConfigDescriptors() {
    ...
    return ImmutableList.of((ServiceAccountConfig.Descriptor) instance.getDescriptorOrDie(JsonServiceAccountConfig.class), (ServiceAccountConfig.Descriptor) instance.getDescriptorOrDie(P12ServiceAccountConfig.class));
  }

  @NonNull @Override public String getUsername() throws KeyTypeNotSetException, AccountIdNotSetException {
    if (serviceAccountConfig == null) {
  ...
  @Override public GoogleCredential getGoogleCredential(GoogleOAuth2ScopeRequirement requirement) throws KeyTypeNotSetException, AccountIdNotSetException, PrivateKeyNotSetException {
    if (serviceAccountConfig == null) {
      throw new KeyTypeNotSetException();
    }
    ...
    return new GoogleCredential.Builder().setTransport(getModule().getHttpTransport()).setJsonFactory(getModule().getJsonFactory()).setServiceAccountScopes(requirement.getScopes()).setServiceAccountId(serviceAccountConfig.getAccountId()).setServiceAccountPrivateKey(serviceAccountConfig.getPrivateKey()).build();
  }
  ...
  @Extension public static class Descriptor extends AbstractGoogleRobotCredentialsDescriptor {
    public Descriptor() {
      this(new GoogleRobotCredentialsModule());
    }

    @VisibleForTesting Descriptor(GoogleRobotCredentialsModule module) {
