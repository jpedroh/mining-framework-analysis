...
public class CopyFromHelper {
    ...
    // ENTRIES(propertyName,interface.class)
    private final Map<String, Class<?>> baseInterfaceMap;

    // ENTRIES(interface.class,implementation.class)
                ...
                for (final PropertyDescriptor pd : pds) {
                    ...
                    (pReadMethod.getParameterTypes().length == 0))// filter
                     && // getter
                                                                        ...
                                                                        // take
                                                                        // parameters
                    baseInterfaceMap.containsKey(propertyName)) {
                        // only
                        // copies
                        // properties
                        // defined
    ...
    private <T> T doCopy(T value, final Class<?> baseInterface) throws Exception {
        if (value != null) {
            ...
            if (vClass.isArray()) {
                ...
                // because Date it is not inmutable
