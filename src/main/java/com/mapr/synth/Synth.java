...
public class Synth {
  ...
  public static void main(String[] args) throws IOException, CmdLineException, InterruptedException, ExecutionException {
    ...
    try {
      ...
      System.err.println("Usage: " + "[ -count <number>G|M|K ] " + "-schema schema-file " + "[-quote DOUBLE_QUOTE|BACK_SLASH|OPTIMISTIC] " + 
...
>>>>>>> /usr/src/app/output/tdunning/log-synth/83316ef849229d87b24ce5d0d6d54f1f76764759/src/main/java/com/mapr/synth/Synth.java/right.java
    ...
    Preconditions.checkArgument(opts.threads > 0 && opts.threads <= 2000, "Must have at least one thread and no more than 2000");
    Preconditions.checkArgument(opts.format == Format.TEMPLATE && opts.template != null, "Please specify a template file");
    if (opts.threads > 1) {
      Preconditions.checkArgument(!"-".equals(opts.output), "If more than on thread is used, you have to use -output to set the output directory");
    }
    ...
    if (!"-".equals(opts.output)) {
      ...
      Preconditions.checkArgument(outputDir.exists() && outputDir.isDirectory(), String.format("Couldn\'t create directory %s", opts.output));
    }
    if (opts.schema == null) {
    ...
    Preconditions.checkState(total == opts.count, String.format("Expected to generate %d lines of output, but actually generated %d", opts.count, total));
    pool.shutdownNow();
    blinker.shutdownNow();
    finalRun.set(true);
    sideLog.close();
  ...
  private static class ReportingWorker implements Callable<Integer> {
    ...
    @Override public Integer call() throws Exception {
      if ("-".equals(opts.output)) {
        ...
        try (PrintStream out = new PrintStream(Files.newOutputStream(outputPath, StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING))) {
    ...
    public class ThreadReport {
      ...
      public ThreadReport() {
        while (true) {
          ...
          if (lastWall.compareAndSet(oldWall, wall) && lastThreadTime.compareAndSet(oldThread, thread) && lastRowCount.compareAndSet(oldRowCount, rowCount)) {
            return;
          }
        }
      }
  ...
  private static class Options {
    @Option(name = "-output") String output = "-";

    @Option(name = "-threads") int threads = 1;

    @Option(name = "-count", handler = SizeParser.class) int count = 1000;

    @Option(name = "-schema", required = false) File schema;

    @Option(name = "-template", required = false) File template;

    @Option(name = "-format") Format format = Format.CSV;

    @Option(name = "-quote") Quote quote = Quote.DOUBLE_QUOTE;
