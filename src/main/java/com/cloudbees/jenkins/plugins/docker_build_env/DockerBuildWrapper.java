...
public class DockerBuildWrapper extends BuildWrapper {
  ...
  @DataBoundConstructor public DockerBuildWrapper(DockerImageSelector selector, String dockerInstallation, DockerServerEndpoint dockerHost, String dockerRegistryCredentials, boolean verbose, boolean privileged, List<Volume> volumes, String group, String command, boolean forcePull, String net, String memory, String cpu, String filterEnvVariables, boolean noCache) {
  ...
  @Override public Launcher decorateLauncher(final AbstractBuild build, final Launcher launcher, final BuildListener listener) throws IOException, InterruptedException, Run.RunnerAbortedException {
  ...
  private String startBuildContainer(BuiltInContainer runInContainer, AbstractBuild build, BuildListener listener) throws IOException {
    try {
      ...
      return runInContainer.getDocker().runDetached(runInContainer.image, workdir, runInContainer.getVolumes(build), runInContainer.getPortsMap(), links, environment, build.getSensitiveBuildVariables(), net, memory, cpu, command);
    } catch (InterruptedException e) {
      throw new RuntimeException("Interrupted");
    }
  }
