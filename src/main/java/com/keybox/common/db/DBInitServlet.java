 ...
 */
@WebServlet(name = "DBInitServlet", urlPatterns = { "/config" }, loadOnStartup = 1) public class DBInitServlet extends javax.servlet.http.HttpServlet {
  /**
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    try {
      ...
      if (rs == null || !rs.next()) {
        resetSSHKey = true;
        statement.executeUpdate("create table if not exists users (" + "id INTEGER PRIMARY KEY AUTO_INCREMENT," + "first_nm varchar, " + "last_nm varchar, " + "email varchar, " + "username varchar not null, " + "password varchar, " + "auth_token varchar, " + "enabled boolean not null default true, " + "pwreset boolean not null default false, " + "auth_type varchar not null default \'" + Auth.AUTH_BASIC + "\', " + "user_type varchar not null default \'" + Auth.ADMINISTRATOR + "\', " + "salt varchar, " + "otp_secret varchar)");
        ...
        statement.executeUpdate("create table if not exists system (" + "id INTEGER PRIMARY KEY AUTO_INCREMENT, " + "display_nm varchar not null, " + "user varchar not null, " + "host varchar not null, " + "port INTEGER not null, " + "authorized_keys varchar not null, " + "status_cd varchar not null default \'INITIAL\', " + "enabled boolean not null default true, " + "instance_id varchar DEFAULT \'---\', " + "region varchar DEFAULT \'---\'," + ")");
        statement.executeUpdate("create table if not exists profiles (" + "id INTEGER PRIMARY KEY AUTO_INCREMENT, " + "nm varchar not null, " + "tag varchar not null, " + "desc varchar not null)");
        ...
        statement.executeUpdate("create table if not exists application_key (" + "id INTEGER PRIMARY KEY AUTO_INCREMENT, " + "keyname varchar not null, " + "public_key varchar not null, " + "private_key varchar not null, " + "passphrase varchar," + "initialKey boolean DEFAULT FALSE, " + "user_id INTEGER DEFAULT null, " + "type varchar not null, " + "fingerprint_id INTEGER, " + "enabled boolean not null default true, " + "create_dt timestamp not null default CURRENT_TIMESTAMP()," + "ec2_region varchar DEFAULT \'NO_EC2_REGION\', " + "foreign key (user_id) references users(id), " + "foreign key (fingerprint_id) references fingerprint(id))");
        statement.executeUpdate("create table if not exists application_key_system (" + "system_id INTEGER, " + "application_key_id INTEGER, " + "active boolean, " + "foreign key (system_id) references system(id) ON DELETE CASCADE, " + "foreign key (application_key_id) references application_key(id) ON DELETE CASCADE)");
        ...
        statement.executeUpdate("create table if not exists public_keys (" + "id INTEGER PRIMARY KEY AUTO_INCREMENT, " + "key_nm varchar not null, " + "type varchar, " + "fingerprint_id INTEGER, " + "public_key varchar, " + "enabled boolean not null default true, " + "create_dt timestamp not null default CURRENT_TIMESTAMP(), " + "user_id INTEGER, " + "profile_id INTEGER, " + "foreign key (profile_id) references profiles(id) on delete cascade, " + "foreign key (user_id) references users(id) on delete cascade, " + "foreign key (fingerprint_id) references fingerprint(id) on delete cascade)");
