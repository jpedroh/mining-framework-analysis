...
public class DBInitServlet extends javax.servlet.http.HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			if (rs == null || !rs.next()) {
				resetSSHKey = true;
				statement.executeUpdate("create table if not exists users (id INTEGER PRIMARY KEY AUTO_INCREMENT, first_nm varchar, last_nm varchar, email varchar, username varchar not null, password varchar, auth_token varchar, enabled boolean not null default true, user_type varchar not null default '" + Auth.ADMINISTRATOR + "', salt varchar, otp_secret varchar, use_otp boolean default true)");
...
<<<<<<< /usr/src/app/output/bastillion-io/bastillion/e6ae3604ccbbaf42827e6eb4749121a250b95571/src/main/java/com/keybox/common/db/DBInitServlet.java/left.java
				statement.executeUpdate("create table if not exists public_keys (id INTEGER PRIMARY KEY AUTO_INCREMENT, key_nm varchar not null, public_key varchar, key_tp varchar, key_fp varchar, user_id INTEGER, profile_id INTEGER, foreign key (profile_id) references profiles(id) on delete cascade, foreign key (user_id) references users(id) on delete cascade)");
||||||| /usr/src/app/output/bastillion-io/bastillion/e6ae3604ccbbaf42827e6eb4749121a250b95571/src/main/java/com/keybox/common/db/DBInitServlet.java/base.java
				statement.executeUpdate("create table if not exists public_keys (id INTEGER PRIMARY KEY AUTO_INCREMENT, key_nm varchar not null, public_key varchar, user_id INTEGER, profile_id INTEGER, foreign key (profile_id) references profiles(id) on delete cascade, foreign key (user_id) references users(id) on delete cascade)");
=======
...
>>>>>>> /usr/src/app/output/bastillion-io/bastillion/e6ae3604ccbbaf42827e6eb4749121a250b95571/src/main/java/com/keybox/common/db/DBInitServlet.java/right.java
