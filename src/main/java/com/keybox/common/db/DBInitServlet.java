	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			if (rs == null || !rs.next()) {
				...
				statement.executeUpdate("create table if not exists application_key (id INTEGER PRIMARY KEY AUTO_INCREMENT, keyname varchar not null, public_key varchar not null, private_key varchar not null, passphrase varchar,"
						+ "initialKey boolean DEFAULT FALSE, user_id INTEGER DEFAULT null, type varchar not null, fingerprint_id INTEGER, enabled boolean not null default true, create_dt timestamp not null default CURRENT_TIMESTAMP(),"
						+ "foreign key (user_id) references users(id),foreign key (fingerprint_id) references fingerprint(id))");
