 ...
 */
@SuppressWarnings(value = { "unchecked" }) public class AuthKeysAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {
  ...
  @Action(value = "/manage/enablePublicKey", results = { @Result(name = "success", location = "/manage/view_keys.jsp") }) public String enablePublicKey() {
  ...
  @Action(value = "/manage/disablePublicKey", results = { @Result(name = "success", location = "/manage/view_keys.jsp") }) public String disablePublicKey() {
    publicKey = PublicKeyDB.getPublicKey(publicKey.getId());
  ...
  @Action(value = "/manage/viewKeys", results = { @Result(name = "success", location = "/manage/view_keys.jsp") }) public String manageViewKeys() {
    profileList = ProfileDB.getAllProfiles();
    userList = UserDB.getUserSet(new SortedSet(SessionAuditDB.SORT_BY_USERNAME)).getItemList();
  ...
  @Action(value = "/admin/viewKeys", results = { @Result(name = "success", location = "/admin/view_keys.jsp") }) public String adminViewKeys() {
  ...
  @Action(value = "/admin/savePublicKey", results = { @Result(name = "input", location = "/admin/view_keys.jsp"), @Result(name = "success", location = "/admin/viewKeys.action?sortedSet.orderByDirection=${sortedSet.orderByDirection}&sortedSet.orderByField=${sortedSet.orderByField}&keyNm=${publicKey.keyNm}", type = "redirect") }) public String savePublicKeys() {
    Long userId = AuthUtil.getUserId(servletRequest.getSession());
    String userType = AuthUtil.getUserType(servletRequest.getSession());
  ...
  @Action(value = "/admin/deletePublicKey", results = { @Result(name = "input", location = "/admin/view_keys.jsp"), @Result(name = "success", location = "/admin/viewKeys.action?sortedSet.orderByDirection=${sortedSet.orderByDirection}&sortedSet.orderByField=${sortedSet.orderByField}", type = "redirect") }) public String deletePublicKey() {
    if (publicKey.getId() != null) {
      publicKey = PublicKeyDB.getPublicKey(publicKey.getId());
  ...
  @Action(value = "/admin/downloadPvtKey") public String downloadPvtKey() {
...
>>>>>>> /usr/src/app/output/bastillion-io/bastillion/9963fe0cc7668e7d305e6dc3ab82ec5d0fef8701/src/main/java/com/keybox/manage/action/AuthKeysAction.java/right.java
  ...
  public void validateSavePublicKeys() {
    Long userId = AuthUtil.getUserId(servletRequest.getSession());
    if (publicKey == null || publicKey.getKeyNm() == null || publicKey.getKeyNm().trim().equals("")) {
    ...
    if (publicKey != null) {
      if (existingKeyId != null) {
        ...
        if ("true".equals(AppConfig.getProperty("forceUserKeyGeneration"))) {
          if (publicKey.getPassphrase() == null || publicKey.getPassphrase().trim().equals("")) {
            addFieldError("publicKey.passphrase", "Required");
          } else {
            if (publicKey.getPassphraseConfirm() == null || publicKey.getPassphraseConfirm().trim().equals("")) {
              addFieldError("publicKey.passphraseConfirm", "Required");
            } else {
              if (!publicKey.getPassphrase().equals(publicKey.getPassphraseConfirm())) {
                addActionError("Passphrases do not match");
              } else {
                if (!PasswordUtil.isValid(publicKey.getPassphrase())) {
      ...
      if (publicKey.getPublicKey() == null || publicKey.getPublicKey().trim().equals("")) {
        ...
        if (SSHUtil.getFingerprint(publicKey.getPublicKey()) == null || SSHUtil.getKeyType(publicKey.getPublicKey()) == null) {
          addFieldError("publicKey.publicKey", "Invalid");
        } else {
          if (PublicKeyDB.isKeyDisabled(SSHUtil.getFingerprint(publicKey.getPublicKey()))) {
            addActionError("This key has been disabled. Please generate and set a new public key.");
            addFieldError("publicKey.publicKey", "Invalid");
          } else {
            if (PublicKeyDB.isKeyRegistered(userId, publicKey)) {
              addActionError("This key has already been registered under selected profile.");
              addFieldError("publicKey.publicKey", "Invalid");
            } else {
              if (PublicKeyDB.isKeyExists(userId, publicKey)) {
