  ...
  public static HostSystem authAndAddPubKey(HostSystem hostSystem, String passphrase, String password) {
    ...
    try {
      ...
      if (e.getMessage().toLowerCase().contains("userauth fail")) {
        ...
        if (e.getMessage().toLowerCase().contains("auth fail") || e.getMessage().toLowerCase().contains("auth cancel")) {
          hostSystem.setStatusCd(HostSystem.AUTH_FAIL_STATUS);
        } else {
          if (e.getMessage().toLowerCase().contains("unknownhostexception")) {
        ...
        if (keyPair != null) {
          ...
          if (KeyPair.DSA == type) {
            ...
            if (KeyPair.RSA == type) {
              ...
              if (KeyPair.ECDSA == type) {
                keyType = "ECDSA";
              } else {
                if (KeyPair.UNKNOWN == type) {
                  keyType = "UNKNOWN";
                } else {
                  if (KeyPair.ERROR == type) {
                    keyType = "ERROR";
                  }
                }
              }
