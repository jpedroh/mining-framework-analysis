...
public class SystemDB {
  ...
  public static List<HostSystem> getAllSystemsWhereApplicationKeyOlderThan(Integer days) {
    ...
    try {
      con = DBUtils.getConn();
      PreparedStatement stmt = con.prepareStatement("SELECT sys.ID FROM SYSTEM sys " + "JOIN APPLICATION_KEY_SYSTEM appkey_sys on appkey_sys.system_id = sys.id " + "JOIN APPLICATION_KEY appkey on appkey_sys.application_key_id = appkey.id " + "WHERE appkey_sys.active = true " + "AND appkey.initialkey = false " + "AND appkey.create_dt < ? ");
  ...
  private static HostSystem transformerEC2InstanzToHostSystem(Instance instance, String ec2Region) {
    ...
    if (!"true".equals(AppConfig.getProperty("useEC2PvtDNS")) && StringUtils.isNotEmpty(instance.getPublicDnsName())) {
      ...
      if (!"true".equals(AppConfig.getProperty("useEC2PvtDNS")) && StringUtils.isNotEmpty(instance.getPublicIpAddress())) {
