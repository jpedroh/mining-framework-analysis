...
public class PSTAttachment extends PSTObject {
  /**
     ...
     */
  PSTAttachment(final PSTFile theFile, final PSTTableBC table, final HashMap<Integer, PSTDescriptorItem> localDescriptorItems) {
  ...
  public InputStream getFileInputStream() throws IOException, PSTException {
    ...
    if (null == attachmentDataObject) {
      ...
      if (attachmentDataObject.isExternalValueReference) {
        final PSTDescriptorItem descriptorItemNested = this.localDescriptorItems.get(attachmentDataObject.entryValueReference);
