 ...
 *
 */
@Immutable public class CsvReader {
  /**
  ...
  public static Table read(String tableName, ColumnType[] types, boolean header, char columnSeparator, InputStream stream) throws IOException {
   ...
   * @throws IOException
   */
  public static Table headerOnly(String name, ColumnType[] types, boolean header, char columnSeparator, InputStream stream) throws IOException {
    BufferedReader streamReader = new BufferedReader(new InputStreamReader(stream));
    Table table;
    try (CSVReader reader = new CSVReader(streamReader, columnSeparator, '\"')) {
      String[] nextLine;
   ...
   * @throws IOException
   */
  public static Table headerOnly(ColumnType[] types, boolean header, char columnSeparator, String fileName) throws IOException {
    InputStream stream = new FileInputStream(fileName);
  ...
  @VisibleForTesting static ColumnType[] detectColumnTypes(String file, boolean header, char delimiter) throws IOException {
  ...
  private static ColumnType detectType(List<String> valuesList) {
    ColumnType[] typeArray = { LOCAL_DATE_TIME, LOCAL_TIME, LOCAL_DATE, BOOLEAN, SHORT_INT, INTEGER, LONG_INT, FLOAT };
  ...
  private static java.util.function.Predicate<String> isBoolean = (s) -> TypeUtils.TRUE_STRINGS_FOR_DETECTION.contains(s) || TypeUtils.FALSE_STRINGS_FOR_DETECTION.contains(s);

  private static Predicate<String> isLong = new Predicate<String>() {
    @Override public boolean test(@Nullable String s) {
      try {
        Long.parseLong(s);
        return true;
