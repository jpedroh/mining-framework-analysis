...
public class S3River extends AbstractRiverComponent implements River {
  ...
  @Inject @SuppressWarnings(value = { "unchecked" }) protected S3River(RiverName riverName, RiverSettings settings, Client client, ThreadPool threadPool) throws Exception {
    ...
    if (settings.settings().containsKey("amazon-s3")) {
      ...
      feedDefinition = new S3RiverFeedDefinition(feedname, bucket, pathPrefix, downloadHost, updateRate, Arrays.asList(includes), Arrays.asList(excludes), accessKey, secretKey, jsonSupport, indexedCharsRatio);
    } else {
    ...
    try {
      ...
      logger.error("Exception while connecting Amazon S3 user bucket. " + "Either access key, secret key or bucket name are incorrect");
      throw ase;
    }
    ...
    threadPool.generic().execute(new Runnable() {
      @Override public void run() {
        ...
        bulkProcessor = BulkProcessor.builder(client, new BulkProcessor.Listener() {
          ...
          @Override public void afterBulk(long id, BulkRequest request, BulkResponse response) {
          ...
          @Override public void afterBulk(long id, BulkRequest request, Throwable throwable) {
            logger.warn("Error executing bulk", throwable);
          }
        }).setBulkActions(bulkSize).build();
        feedThread = EsExecutors.daemonThreadFactory(settings.globalSettings(), "fs_slurper").newThread(new S3Scanner(feedDefinition));
        feedThread.start();
  ...
  private void pushMapping(String index, String type, XContentBuilder xcontent) throws Exception {
    ...
    if (!mappingExist) {
      ...
      if (xcontent != null) {
        ...
        PutMappingResponse response = client.admin().indices().preparePutMapping(index).setType(type).setSource(xcontent).execute().actionGet();
  ...
  private class S3Scanner implements Runnable {
    ...
    private boolean isStarted() {
      ...
      try {
        if (!isStartedGetResponse.isExists()) {
          XContentBuilder xb = jsonBuilder().startObject().startObject("amazon-s3").field("feedname", feedDefinition.getFeedname()).field("status", "STARTED").endObject().endObject();
    ...
    @SuppressWarnings(value = { "unchecked" }) private Long getLastScanTimeFromRiver(String lastScanTimeField) {
      Long result = null;
      try {
        client.admin().indices().prepareRefresh("_river").execute().actionGet();
    ...
    private List<String> getAlreadyIndexFileIds() {
      ...
      SearchResponse response = client.prepareSearch(indexName).setSearchType(SearchType.QUERY_AND_FETCH).setTypes(typeName).setFrom(0).setSize(5000).execute().actionGet();
...
>>>>>>> /usr/src/app/output/lbroudoux/es-amazon-s3-river/2aa45336dcc877155d7df5063071f37a86368271/src/main/java/com/github/lbroudoux/elasticsearch/river/s3/river/S3River.java/right.java
            ...
            String parsedContent = TikaHolder.tika().parseToString(new BytesStreamInput(fileContent, false), fileMetadata, indexedChars);
            ...
            esIndex(indexName, typeName, fileId, jsonBuilder().startObject().field(S3RiverUtil.DOC_FIELD_TITLE, summary.getKey().substring(summary.getKey().lastIndexOf('/') + 1)).field(S3RiverUtil.DOC_FIELD_MODIFIED_DATE, summary.getLastModified().getTime()).field(S3RiverUtil.DOC_FIELD_SOURCE_URL, s3.getDownloadUrl(summary, feedDefinition)).field(S3RiverUtil.DOC_FIELD_METADATA, s3.getS3UserMetadata(summary.getKey())).startObject("file").field("_name", summary.getKey().substring(summary.getKey().lastIndexOf('/') + 1)).field("title", summary.getKey().substring(summary.getKey().lastIndexOf('/') + 1)).field("file", parsedContent).field("metadata", fileMetadataMap).endObject().endObject());
            return fileId;
          }
        }
      } catch (Exception e) {
    ...
    private void updateRiver(String lastScanTimeField, Long lastScanTime) throws Exception {
      ...
      XContentBuilder xb = jsonBuilder().startObject().startObject("amazon-s3").field("feedname", feedDefinition.getFeedname()).field(lastScanTimeField, lastScanTime).endObject().endObject();
