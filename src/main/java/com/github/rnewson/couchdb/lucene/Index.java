    ...
    private synchronized void updateIndex() throws IOException {
      ...
      try {
          ...
          try {
            do {
              ...
              if (Arrays.binarySearch(dbnames, term.text()) < 0) {
                Utils.LOG.info("Database \'" + term.text() + "\' has been deleted, removing all documents from index.");
        ...
        if (commit) {
          ...
          final IndexReader reader = IndexReader.open(dir);
          try {
            Utils.LOG.info("Committed changes to index (" + reader.numDocs() + " documents in index, " + reader.numDeletedDocs() + " deletes).");
          }  finally {
            reader.close();
          }
        } else {
