...
public final class DatabaseIndexer implements Runnable, ResponseHandler<Void> {
  private class IndexState {
    ...
    public synchronized IndexReader borrowReader(final boolean staleOk) throws IOException {
    ...
    public IndexSearcher borrowSearcher(final boolean staleOk) throws IOException {
  ...
  public void admin(final HttpServletRequest req, final HttpServletResponse resp) throws IOException {
  ...
  public void info(final HttpServletRequest req, final HttpServletResponse resp) throws IOException {
    final IndexState state = getState(req, resp);
    if (state == null) {
      return;
    }
    ...
    for (final Entry<View, IndexState> entry : states.entrySet()) {
      ...
      if (state.pending_seq > getUpdateSequence(state.writer)) {
        ...
        if (!state.writerDirty) {
          logger.debug("Forcing additional document as nothing else was indexed since last commit.");
  ...
  private Document forceDocument() {
    ...
    result.add(new Field("_cl", uuid.toString(), Store.NO, Index.NOT_ANALYZED_NO_NORMS));
    return result;
  }
  ...
  private boolean getBooleanParameter(final HttpServletRequest req, final String parameterName) {
    return Boolean.parseBoolean(req.getParameter(parameterName));
  }

  private int getIntParameter(final HttpServletRequest req, final String parameterName, final int defaultValue) {
    final String result = req.getParameter(parameterName);
    return result != null ? Integer.parseInt(result) : defaultValue;
  }

  private IndexState getState(final HttpServletRequest req, final HttpServletResponse resp) throws IOException {
