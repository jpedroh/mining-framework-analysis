...
public final class DatabaseIndexer implements Runnable, ResponseHandler<Void> {
  private class IndexState {
    ...
    public IndexState(final DocumentConverter converter, final IndexWriter writer, final Analyzer analyzer, final Database database, final View view) {
    ...
    public synchronized DirectoryReader borrowReader(final boolean staleOk) throws IOException, JSONException {
    ...
    public IndexSearcher borrowSearcher(final boolean staleOk) throws IOException, JSONException {
  ...
  public static File viewDir(final File root, final UUID uuid, final String digest, final boolean mkdirs) throws IOException {
  ...
  public void admin(final HttpServletRequest req, final HttpServletResponse resp) throws IOException, JSONException {
  ...
  public void info(final HttpServletRequest req, final HttpServletResponse resp) throws IOException, JSONException {
    final IndexState state = getState(req, resp);
    if (state == null) {
      return;
    }
  ...
  public void search(final HttpServletRequest req, final HttpServletResponse resp) throws IOException, JSONException {
    final IndexState state = getState(req, resp);
    if (state == null) {
      return;
    }
    ...
    try {
      ...
      for (final String queryString : getQueryStrings(req)) {
        ...
        final Operator operator = "and".equalsIgnoreCase(req.getParameter("default_operator")) ? Operator.AND : Operator.OR;
        ...
        if (getBooleanParameter(req, "rewrite")) {
          ...
          final int highlight_length = max(getIntParameter(req, "highlight_length", 18), 18);
          ...
          if (include_docs && fetch_ids.length > 0) {
            ...
            for (int j = 0; j < max; j++) {
              ...
              final JSONObject row = doc == null ? new JSONObject("{\"error\":\"not_found\"}") : doc.asJson();
  ...
  private static boolean getBooleanParameter(final HttpServletRequest req, final String parameterName) {
    return Boolean.parseBoolean(req.getParameter(parameterName));
  }

  private static int getIntParameter(final HttpServletRequest req, final String parameterName, final int defaultValue) {
    final String result = req.getParameter(parameterName);
    return result != null ? Integer.parseInt(result) : defaultValue;
  }

  private IndexState getState(final HttpServletRequest req, final HttpServletResponse resp) throws IOException, JSONException {
  ...
  private IndexWriter newWriter(final Directory dir) throws IOException {
    ...
    config.setRAMBufferSizeMB(ini.getDouble("lucene.ramBufferSizeMB", IndexWriterConfig.DEFAULT_RAM_BUFFER_SIZE_MB));
    return new IndexWriter(dir, config);
  }

  private File viewDir(final View view, final boolean mkdirs) throws IOException {
