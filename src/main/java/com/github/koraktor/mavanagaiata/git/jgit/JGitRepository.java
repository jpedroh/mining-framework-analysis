...
public class JGitRepository extends AbstractGitRepository {
  ...
  public String getAbbreviatedCommitId(GitCommit commit) throws GitRepositoryException {
    try {
      ...
      return this.repository.getObjectDatabase().newReader().abbreviate(rawCommit).name();
    } catch (IOException e) {
      throw new GitRepositoryException(String.format("Commit \"%s\" could not be abbreviated.", this.getHeadObject().getName()), e);
    }
  }
