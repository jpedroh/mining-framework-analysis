...
public class J2KReadState {
  ...
  public Raster getTile(int tileX, int tileY, WritableRaster raster) throws IOException {
    ...
    if (noTransform) {
      ...
      int cTileWidth;
      int cTileHeight;
      if (raster != null && (this.resolution < hd.getDecoderSpecs().dls.getMin()) || stepX != 1 || stepY != 1) {
        ...
        tOffx = ictransf.getCompULX(0) - (ictransf.getImgULX() + ictransf.getCompSubsX(0) - 1) / ictransf.getCompSubsX(0) + destinationRegion.x;
        tOffy = ictransf.getCompULY(0) - (ictransf.getImgULY() + ictransf.getCompSubsY(0) - 1) / ictransf.getCompSubsY(0) + destinationRegion.y;
      ...
      for (int l = 0; l < cTileHeight; l++) {
        ...
        for (int i = 0; i < numBands; i++) {
          ...
          if (ImageUtil.isBinary(sampleModel)) {
            ...
            ImageUtil.setUnpackedBinaryData(bytebuf, raster, new Rectangle(tOffx, tOffy + l, cTileWidth, 1));
          } else {
            for (int j = cTileWidth - 1; j >= 0; j--) {
              int tmp = (data[k1--] >> fracBit) + lS;
  ...
  public BufferedImage readBufferedImage() throws IOException {
    ...
    if (image == null) {
      ...
      raster = Raster.createWritableRaster(sampleModel.createCompatibleSampleModel(destinationRegion.x + destinationRegion.width, destinationRegion.y + destinationRegion.height), new Point(0, 0));
  ...
  private void initializeRead(int imageIndex, J2KImageReadParamJava param, J2KMetadata metadata) {
    try {
      ...
      if (resolution != minResLevels || stepX != 1 || stepY != 1) {
        sourceRegion = J2KImageReader.getReducedRect(sourceRegion, minResLevels, resolution, stepX, stepY);
      }
      destinationRegion = (Rectangle) sourceRegion.clone();
      J2KImageReader.computeRegionsWrapper(param, false, this.width, this.height, param.getDestination(), sourceRegion, destinationRegion);
      ...
      if (tileWidth * (1 << (minResLevels - resolution)) * stepX > hd.getNomTileWidth()) {
        tileStepX = (tileWidth * (1 << (minResLevels - resolution)) * stepX + hd.getNomTileWidth() - 1) / hd.getNomTileWidth();
      } else {
        tileStepX = 1;
      }
      if (tileHeight * (1 << (minResLevels - resolution)) * stepY > hd.getNomTileHeight()) {
        tileStepY = (tileHeight * (1 << (minResLevels - resolution)) * stepY + hd.getNomTileHeight() - 1) / hd.getNomTileHeight();
      } else {
        tileStepY = 1;
      }
      ...
      try {
        ...
        breader = BitstreamReaderAgent.createInstance(in, hd, j2krparam, decSpec, logJJ2000Messages, hi);
      } catch (IOException e) {
      ...
      J2KImageReader.checkReadParamBandSettingsWrapper(param, hd.getNumComps(), destinationBands.length);
  ...
  private Raster readSubsampledRaster(WritableRaster raster) throws IOException {
    if (raster == null) {
      raster = Raster.createWritableRaster(sampleModel.createCompatibleSampleModel(destinationRegion.x + destinationRegion.width, destinationRegion.y + destinationRegion.height), new Point(destinationRegion.x, destinationRegion.y));
    }
    int pixbuf[] = null;
    boolean prog = false;
    Point nT = ictransf.getNumTiles(null);
  ...
  public SampleModel getSampleModel() {
    ...
    if (nComp == 1 && (maxDepth == 1 || maxDepth == 2 || maxDepth == 4)) {
      sampleModel = new MultiPixelPackedSampleModel(DataBuffer.TYPE_BYTE, realWidth, realHeight, maxDepth);
    } else {
      if (maxDepth <= 8) {
        sampleModel = new PixelInterleavedSampleModel(DataBuffer.TYPE_BYTE, realWidth, realHeight, nComp, realWidth * nComp, bandOffsets);
      } else {
        if (maxDepth <= 16) {
          sampleModel = new PixelInterleavedSampleModel(isSigned ? DataBuffer.TYPE_SHORT : DataBuffer.TYPE_USHORT, realWidth, realHeight, nComp, realWidth * nComp, bandOffsets);
        } else {
          if (maxDepth <= 32) {
            sampleModel = new PixelInterleavedSampleModel(DataBuffer.TYPE_INT, realWidth, realHeight, nComp, realWidth * nComp, bandOffsets);
          } else {
    ...
    if (hi.siz.csiz <= 4) {
      ...
      if (type != -1) {
        if (hi.siz.csiz == 1 && (maxBitDepth == 1 || maxBitDepth == 2 || maxBitDepth == 4)) {
          ...
          colorModel = new ComponentColorModel(cs, bitsPerComponent, hasAlpha, false, hasAlpha ? Transparency.TRANSLUCENT : Transparency.OPAQUE, type);
        }
        return colorModel;
      }
    }
