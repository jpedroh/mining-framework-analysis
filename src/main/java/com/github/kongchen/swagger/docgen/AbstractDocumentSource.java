...
public abstract class AbstractDocumentSource {
  ...
  private Swagger doFilter(Swagger swagger) throws GenerateException {
    ...
    return new SpecFilter().filter(swagger, filter, new HashMap<String, List<String>>(), new HashMap<String, String>(), new HashMap<String, List<String>>());
  }
  ...
  private void initHandlebars(Handlebars handlebars) {
    handlebars.registerHelper("ifeq", new Helper<String>() {
      @Override public CharSequence apply(String value, Options options) throws IOException {
