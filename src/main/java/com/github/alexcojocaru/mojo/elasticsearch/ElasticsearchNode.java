...
public class ElasticsearchNode {
  ...
  public ElasticsearchNode(Settings settings) throws MojoExecutionException {
    settings = Settings.settingsBuilder().put("index.number_of_shards", 1).put("index.number_of_replicas", 0).put("network.host", "127.0.0.1").put("discovery.zen.ping.timeout", "3ms").put("discovery.zen.ping.multicast.enabled", false).put("http.cors.enabled", true).put(settings).build();
  ...
  /**
     ...
     * @return an instance of an ElasticsearchNode
     */
  public static ElasticsearchNode start(String dataPath, int httpPort, int tcpPort) throws MojoExecutionException {
    String homePath = new File(dataPath).getParent();
    Settings settings = Settings.settingsBuilder().put("cluster.name", "test").put("action.auto_create_index", false).put("transport.tcp.port", tcpPort).put("http.port", httpPort).put("path.data", dataPath).put("path.home", homePath).build();
