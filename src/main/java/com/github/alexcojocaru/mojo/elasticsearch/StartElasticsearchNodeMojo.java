...
public class StartElasticsearchNodeMojo extends AbstractElasticsearchNodeMojo {
  ...
  @Override public void execute() throws MojoExecutionException {
    ...
    Settings.Builder builder = Settings.settingsBuilder().put("cluster.name", clusterName).put("action.auto_create_index", autoCreateIndex).put("transport.tcp.port", tcpPort).put("http.port", httpPort).put("path.home", outputDirectory.getAbsolutePath()).put("path.data", dataDirectory.getAbsolutePath()).put("path.logs", logsDirectory.getAbsolutePath());
  ...
  /**
     ...
     */
  private File prepareDirectory(File parentDir, String dirname, String purpose) throws MojoExecutionException {
    ...
    if (dir.exists()) {
      ...
      try {
        ...
        throw new MojoExecutionException("Cannot delete the existing Elasticsearch " + purpose + " " + dir.getAbsolutePath(), e);
      }
    }
    if (!dir.mkdirs()) {
      throw new MojoExecutionException("Cannot create the Elasticsearch " + purpose + " " + dir.getAbsolutePath());
    }
    return dir;
  }
}