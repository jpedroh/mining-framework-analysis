...
public class PropertyWrapper extends SchemaWrapper {
  ...
  public PropertyWrapper(CustomSchemaWrapper ownerSchemaWrapper, Set<ManagedReference> managedReferences, Method method, Field field) {
    ...
    if (getAccessibleObject().getAnnotation(SchemaIgnore.class) != null) {
      ...
      if (getReferenceType() == ReferenceType.BACKWARD) {
        ...
        if (ownerSchemaWrapper.getJavaType() == propertyType) {
  ...
  @Override public String getDollarSchema() {
  ...
  @Override public boolean isEnumWrapper() {
    return schemaWrapper.isEnumWrapper();
  }

  @Override public boolean isSimpleWrapper() {
    return schemaWrapper.isSimpleWrapper();
  }

  @Override public boolean isCustomWrapper() {
    return schemaWrapper.isCustomWrapper();
  }

  @Override public boolean isRefWrapper() {
    return schemaWrapper.isRefWrapper();
  }

  @Override public boolean isArrayWrapper() {
    return schemaWrapper.isArrayWrapper();
  }

  @Override public boolean isNullWrapper() {
    return schemaWrapper.isNullWrapper();
  }

  @Override public boolean isEmptyWrapper() {
    return schemaWrapper.isEmptyWrapper();
  }

  @Override public <T extends SchemaWrapper> T cast() {
  ...
  @Override protected ObjectNode getNode() {
    return schemaWrapper.getNode();
  }

  @Override protected void processNullable() {
  ...
  @Override protected String getNodeTextValue(JsonNode node) {
    return schemaWrapper.getNodeTextValue(node);
  }

  @Override protected void setType(String type) {
    schemaWrapper.setType(type);
  }

  private String processPropertyName() {
    String name = (field == null) ? firstToLowerCase(method.getName().replace("get", "")) : field.getName();
    return name;
  }
