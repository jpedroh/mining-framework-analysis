 ...
 */
@Cmd(name = "tt", sort = 5, summary = "TimeTunnel the method call.", eg = { "tt -t *StringUtils isEmpty", "tt -l", "tt -D", "tt -i 1000 -w params[0]", "tt -i 1000 -d", "tt -i 1000" }) public class TimeTunnelCommand implements Command {
  private static final Map<Integer, TimeFragment> timeFragmentMap = new LinkedHashMap<Integer, TimeFragment>();

  private static final AtomicInteger sequence = new AtomicInteger(1000);

  @NamedArg(name = "t", summary = "record the method call to time fragment.") private boolean isTimeTunnel = false;

  @IndexArg(index = 0, isRequired = false, name = "class-pattern", summary = "pattern matching of classpath.classname") private String classPattern;

  @IndexArg(index = 1, isRequired = false, name = "method-pattern", summary = "pattern matching of method name") private String methodPattern;

  @NamedArg(name = "l", summary = "list all the time fragments.") private boolean isList = false;

  @NamedArg(name = "D", summary = "delete all time fragments.") private boolean isDeleteAll = false;

  @NamedArg(name = "i", hasValue = true, summary = "appoint the index of time fragment. If use only, show the time fragment detail.") private Integer index;

  @NamedArg(name = "x", hasValue = true, summary = "expend level of object. Default level-0") private Integer expend;

  @NamedArg(name = "w", hasValue = true, summary = "watch-express, watch the time fragment by groovy-express, like params[0], returnObj, throwExp and so on.", description = "" + " \n" + "For example\n" + "    : params[0]\n" + "    : params[0]+params[1]\n" + "    : returnObj\n" + "    : throwExp\n" + "    : target\n" + "    : clazz\n" + "    : method\n" + " \n" + "The structure of \'advice\'\n" + "          target : the object entity\n" + "           clazz : the object\'s class\n" + "          method : the constructor or method\n" + "    params[0..n] : the parameters of methods\n" + "       returnObj : the return object of methods\n" + "        throwExp : the throw exception of methods\n" + " \n") private String watchExpress = EMPTY;

  @NamedArg(name = "s", hasValue = true, summary = "search-express, searching the time fragments by groovy-express") private String searchExpress = EMPTY;

  @NamedArg(name = "p", summary = "rePlay the time fragment of method called.") private boolean isPlay = false;

  @NamedArg(name = "d", summary = "delete the index time fragment") private boolean isDelete = false;

  @NamedArg(name = "S", summary = "including sub class") private boolean isIncludeSub = false;

  @NamedArg(name = "E", summary = "enable the regex pattern matching") private boolean isRegEx = false;
  ...
  private void checkArguments() {
    ...
    if (null == index && !isTimeTunnel && !isDelete && !isDeleteAll && isBlank(watchExpress) && !isList && isBlank(searchExpress) && !isPlay) {
  ...
  private static final int[] TABLE_COL_WIDTH = new int[] { 8, 20, 8, 8, 15, 30, 30 };

  private static final String[] TABLE_COL_TITLE = new String[] { "INDEX", "TIMESTAMP", "IS-RET", "IS-EXP", "OBJECT", "CLASS", "METHOD" };

  private GetEnhancerAction doTimeTunnel() {
    final Matcher classNameMatcher = isRegEx ? new RegexMatcher(classPattern) : new WildcardMatcher(classPattern);
    final Matcher methodNameMatcher = isRegEx ? new RegexMatcher(methodPattern) : new WildcardMatcher(methodPattern);
    return new GetEnhancerAction() {
      @Override public GetEnhancer action(Session session, Instrumentation inst, final Sender sender) throws Throwable {
        return new GetEnhancer() {
          @Override public Matcher getClassNameMatcher() {
            return classNameMatcher;
          }

          @Override public Matcher getMethodNameMatcher() {
            return methodNameMatcher;
          }

          @Override public boolean isIncludeSub() {
            return isIncludeSub;
          }

          @Override public AdviceListener getAdviceListener() {
            return new ReflectAdviceListenerAdapter() {
              volatile boolean isFirst = true;

              @Override public void afterReturning(ClassLoader loader, Class<?> clazz, GaMethod method, Object target, Object[] args, Object returnObject) throws Throwable {
                afterFinishing(newForAfterRetuning(loader, clazz, method, target, args, returnObject));
              }

              @Override public void afterThrowing(ClassLoader loader, Class<?> clazz, GaMethod method, Object target, Object[] args, Throwable throwable) {
                afterFinishing(newForAfterThrowing(loader, clazz, method, target, args, throwable));
              }
  ...
  private RowAction doList() {
    return new RowAction() {
      @Override public RowAffect action(Session session, Instrumentation inst, Sender sender) throws Throwable {
  ...
  private RowAction doSearch() {
    return new RowAction() {
      @Override public RowAffect action(Session session, Instrumentation inst, Sender sender) throws Throwable {
        ...
        if (hasWatchExpress()) {
          final TableView view = new TableView(new ColumnDefine[] { new ColumnDefine(RIGHT), new ColumnDefine(LEFT) }).hasBorder(true).padding(1).addRow("INDEX", "SEARCH-RESULT");
          for (Map.Entry<Integer, TimeFragment> entry : matchingTimeSegmentMap.entrySet()) {
            ...
            view.addRow(entry.getKey(), isNeedExpend() ? new ObjectView(value, expend).draw() : value);
          }
  ...
  private RowAction doPlay() {
    return new RowAction() {
      @Override public RowAffect action(Session session, Instrumentation inst, Sender sender) throws Throwable {
        ...
        final String objectAddress = advice.getTarget() == null ? "NULL" : "0x" + toHexString(advice.getTarget().hashCode());
        final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        final TableView view = new TableView(new ColumnDefine[] { new ColumnDefine(RIGHT), new ColumnDefine(50, true, LEFT) }).hasBorder(true).padding(1).addRow("RE-INDEX", index).addRow("GMT-REPLAY", sdf.format(new Date())).addRow("OBJECT", objectAddress).addRow("CLASS", className).addRow("METHOD", methodName);
  ...
  private TableView createTableView() {
    return new TableView(new ColumnDefine[] { new ColumnDefine(TABLE_COL_WIDTH[0], false, RIGHT), new ColumnDefine(TABLE_COL_WIDTH[1], false, RIGHT), new ColumnDefine(TABLE_COL_WIDTH[2], false, RIGHT), new ColumnDefine(TABLE_COL_WIDTH[3], false, RIGHT), new ColumnDefine(TABLE_COL_WIDTH[4], false, RIGHT), new ColumnDefine(TABLE_COL_WIDTH[5], false, RIGHT), new ColumnDefine(TABLE_COL_WIDTH[6], false, RIGHT) }).hasBorder(true).padding(1);
  }

  private TableView fillTableTitle(TableView tableView) {
    return tableView.addRow(TABLE_COL_TITLE[0], TABLE_COL_TITLE[1], TABLE_COL_TITLE[2], TABLE_COL_TITLE[3], TABLE_COL_TITLE[4], TABLE_COL_TITLE[5], TABLE_COL_TITLE[6]);
  }
  ...
  private TableView fillTableRow(TableView tableView, int index, TimeFragment tf) {
    final Advice advice = tf.getAdvice();
    return tableView.addRow(index, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(tf.getGmtCreate()), advice.isAfterReturning(), advice.isAfterThrowing(), advice.getTarget() == null ? "NULL" : "0x" + toHexString(advice.getTarget().hashCode()), substringAfterLast("." + advice.getClazz().getName(), "."), advice.getMethod().getName());
  }
  ...
  private RowAction doShow() {
    return new RowAction() {
      @Override public RowAffect action(Session session, Instrumentation inst, Sender sender) throws Throwable {
        ...
        final String objectAddress = advice.getTarget() == null ? "NULL" : "0x" + toHexString(advice.getTarget().hashCode());
        final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        final TableView view = new TableView(new ColumnDefine[] { new ColumnDefine(RIGHT), new ColumnDefine(100, false, LEFT) }).hasBorder(true).padding(1).addRow("INDEX", index).addRow("GMT-CREATE", sdf.format(tf.getGmtCreate())).addRow("OBJECT", objectAddress).addRow("CLASS", className).addRow("METHOD", methodName).addRow("IS-RETURN", advice.isAfterReturning()).addRow("IS-EXCEPTION", advice.isAfterThrowing());
        if (null != advice.getParams()) {
          int paramIndex = 0;
          for (Object param : advice.getParams()) {
            if (isNeedExpend()) {
              ...
              if (null != index) {
                ...
                if (hasSearchExpress()) {
                  ...
                  action = new SilentAction() {
                    @Override public void action(Session session, Instrumentation inst, Sender sender) throws Throwable {
