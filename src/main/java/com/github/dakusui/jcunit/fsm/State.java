 ...
 */
public interface State<SUT extends java.lang.Object> extends StateChecker<SUT>, Serializable {
  /**
  ...
  class Base<SUT extends java.lang.Object> implements State<SUT> {
    ...
    @Override public boolean check(SUT sut) {
      return stateSpec.check(sut);
    }

    @SuppressWarnings(value = { "unchecked" }) @Override public Expectation<SUT> expectation(Action<SUT> action, Args args) {
      Method m = Checks.checknotnull(actionMethods.get(action.id()), "Unknown action \'%s\' was given.", action);
      Checks.checktest(Expectation.class.isAssignableFrom(m.getReturnType()), "Method \'%s/%s\' of \'%s\' must return an \'%s\' object (but \'%s\' was returned).", m.getName(), m.getParameterTypes().length, m.getDeclaringClass().getCanonicalName(), Expectation.class.getCanonicalName(), m.getReturnType().getCanonicalName());
      return buildExpectation(m, args);
    }

    @SuppressWarnings(value = { "unchecked" }) @Override public <SPEC extends FsmSpec<SUT>> SPEC spec() {
      return (SPEC) this.stateSpec;
    }

    @SuppressWarnings(value = { "unchecked" }) private Expectation<SUT> buildExpectation(Method m, Args args) {
      Object[] argsToMethod = Utils.concatenate(new Object[] { new Expectation.Builder<>(this.fsmName, fsm) }, args.values());
      try {
        return (Expectation<SUT>) m.invoke(stateSpec, argsToMethod);
      } catch (IllegalArgumentException e) {
        throw Checks.wraptesterror(e, "Wrong types: \'%s/%s\' of \'%s\' can\'t be executed with %s", m.getName(), m.getParameterTypes().length, m.getDeclaringClass(), Arrays.toString(argsToMethod));
      } catch (IllegalAccessException e) {
        throw Checks.fail();
      } catch (InvocationTargetException e) {
        throw Checks.wraptesterror(e, "Method \'%s/%s\' of \'%s\' must always succeed and return an object of \'%s\'.", m.getName(), args.values().length, stateSpec.getClass().getCanonicalName(), Expectation.class.getCanonicalName());
      }
    }
