 ...
 */
public interface FiniteStateMachine<SUT extends java.lang.Object> {
  ...
  static <SUT extends java.lang.Object> FiniteStateMachine<SUT> create(String fsmName, Class<? extends FsmSpec<SUT>> specClass) {
    return new Impl<>(fsmName, specClass);
  }

  class Impl<SUT extends java.lang.Object> implements FiniteStateMachine<SUT> {
    ...
    public Impl(String fsmName, Class<? extends FsmSpec<SUT>> specClass) {
      ...
      for (Map.Entry<String, Method> each : actionMethods.entrySet()) {
        ...
        if (m.getParameterTypes().length > 1) {
          ...
          Checks.checktest(f != null, "A parameter field \'%s\' referred to by \'%s\' is not found in \'%s\'", paramsFieldName, each.getKey(), specClass);
        } else {
          Checks.checktest(ActionSpec.DEFAULT_PARAMS_SPEC.equals(paramsFieldName), "An action without parameters but Expectation.Builder must not have \'parametersSpec\' attribute.");
        }
      ...
      Checks.checktest(initialState != null, "A state whose name is \'I\'(annotated with %s) couldn\'t be found in \'%s\'", StateSpec.class.getSimpleName(), specClass.getCanonicalName());
      this.states = Collections.unmodifiableList(states);
      this.initialState = initialState;
    }

    @Override public State<SUT> initialState() {
      return this.initialState;
    }

    @Override public List<State<SUT>> states() {
      return this.states;
    }

    @Override public List<Action<SUT>> actions() {
      return this.actions;
    }
     ...
     */
    @SuppressWarnings(value = { "unchecked" }) private Action<SUT> createAction(final Method actionMethod, final Field paramsField) {
    ...
    private Parameters getParamsFactors(Field field) {
      ...
      Checks.checktest((((Parameters) ret).values()).size() > 0, "The field \'%s\' of \'%s\' must be assigned Object[][] value whose length is larget than 0.", field.getName(), field.getType().getCanonicalName());
    ...
    private Field validateParamsField(Field fsmField) {
      ...
      Checks.checktest(Modifier.isPublic(m) && Modifier.isStatic(m) && Modifier.isFinal(m) && Parameters.class.isAssignableFrom(fsmField.getType()), "Field \'%s\' of \'%s\' must be public, static, final, and of %s.", ret.getName(), ret.getType().getSimpleName(), Parameters.class.getSimpleName());
      return ret;
    }
    ...
    @SuppressWarnings(value = { "unchecked" }) private FsmSpec<SUT> getStateSpecValue(Field field) {
    ...
    private Field validateStateSpecField(Field fsmField) {
      Field ret = Checks.checknotnull(fsmField);
      int m = ret.getModifiers();
      Checks.checktest(Modifier.isPublic(m) && Modifier.isStatic(m) && Modifier.isFinal(m) && fsmField.getType().isAssignableFrom(fsmField.getType()), "Field \'%s\' of \'%s\' must be public, static, final, and of \'%s\'.", ret.getName(), ret.getType().getCanonicalName(), fsmField.getType().getCanonicalName());
      return ret;
    }

    static String generateMethodId(Method m) {
      return Checks.checknotnull(m).getName() + "/" + StringUtils.join(",", asList(m.getParameterTypes()).subList(1, m.getParameterTypes().length).stream().map(Class::getCanonicalName).toArray());
    }
  }
}