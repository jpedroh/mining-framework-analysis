...
public class IpoGplus extends Generator.Base {
  ...
  @Override public List<Tuple> generateCore() {
    ...
    List<Tuple> ts = streamAllPossibleTuples(allFactors.subList(0, t), t).filter(isAllowedTuple(allFactors, allConstraints)).collect(toList());
    if (ts.isEmpty()) {
      return emptyList();
    }
    ...
    for (int i = t + 1; i <= n; i++) {
      ...
      for (Tuple σ : new LinkedList<>(π)) {
        if (ts.stream().anyMatch(σ::isSubtupleOf)) {
          ...
          List<Tuple> work = ts;
          Tuple chosenTest = streamIncompleteTestsToCoverGivenTuple(ts, σ).map((tuple) -> new Tuple.Builder().putAll(removeDontCares(tuple)).putAll(σ).build()).filter(isAllowedTuple(allFactors, allConstraints)).findFirst().orElseGet(() -> {
            Tuple ret = createTupleFrom(FactorUtils.toFactorNames(processedFactors), σ);
            work.add(ret);
            return ret;
          });
          chosenTest.putAll(σ);
      ...
      ts = ts.stream().map(replaceDontCareValuesWithActualLevels(allFactors, allConstraints)).collect(toList());
    }
    return ts;
  }
  ...
  public static Function<Tuple, Tuple> replaceDontCareValuesWithActualLevels(final List<Factor> allFactors, List<Constraint> allConstraints) {
    return new Function<Tuple, Tuple>() {
      ...
      @Override public Tuple apply(Tuple in) {
        ...
        return new Tuple.Builder().putAll(in).putAll(chooseAssignment(streamAssignmentsForDontCaresUnderConstraints(in, allFactors, allConstraints), i++).orElseThrow(() -> TestDefinitionException.impossibleConstraint(allConstraints))).build();
      }
