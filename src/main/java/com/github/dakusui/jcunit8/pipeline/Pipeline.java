...
public interface Pipeline {
  ...
  class Standard implements Pipeline {
    @Override public TestSuite execute(Config config, ParameterSpace parameterSpace, TestScenario testScenario) {
    ...
    public TestSuite generateTestSuite(Config config, ParameterSpace parameterSpace, TestScenario testScenario) {
      ...
      if (config.getRequirement().generateNegativeTests()) {
        builder = builder.addAllToNegativeTuples(negativeTestGenerator(config.getRequirement().generateNegativeTests(), toFactorSpaceForNegativeTestGeneration(parameterSpace), regularTestTuples, config.getRequirement().seeds(), config.getRequirement()).generate());
      }
      return builder.build();
    }

    private void validateSeeds(List<Tuple> seeds, ParameterSpace parameterSpace) {
      List<Function<Tuple, String>> checks = asList((Tuple tuple) -> !parameterSpace.getParameterNames().containsAll(tuple.keySet()) ? String.format("Unknown parameter(s) were found: %s in tuple: %s", new LinkedList<String>() {
        {
          addAll(tuple.keySet());
          removeAll(parameterSpace.getParameterNames());
        }
      }, tuple) : null, (Tuple tuple) -> !tuple.keySet().containsAll(parameterSpace.getParameterNames()) ? String.format("Parameter(s) were not found: %s in tuple: %s", new LinkedList<String>() {
        {
          addAll(parameterSpace.getParameterNames());
          removeAll(tuple.keySet());
        }
      }, tuple) : null);
      List<String> errors = seeds.stream().flatMap((seed) -> checks.stream().map((each) -> each.apply(seed))).filter(Objects::nonNull).collect(toList());
      if (!errors.isEmpty()) {
        throw new InvalidTestException(String.format("Error(s) are found in seeds: %s", errors));
      }
    }

    public ParameterSpace preprocess(Config config, ParameterSpace parameterSpace) {
      return new ParameterSpace.Builder().addAllParameters(parameterSpace.getParameterNames().stream().map((String parameterName) -> toSimpleParameterIfNecessary(config, parameterSpace.getParameter(parameterName), parameterSpace.getConstraints())).collect(toList())).addAllConstraints(parameterSpace.getConstraints()).build();
    }

    public SchemafulTupleSet engine(Config config, ParameterSpace parameterSpace) {
      return config.partitioner().apply(config.encoder().apply(parameterSpace)).stream().map(config.optimizer()).filter((Predicate<FactorSpace>) (factorSpace) -> !factorSpace.getFactors().isEmpty()).map(config.generator(parameterSpace, config.getRequirement())).reduce(config.joiner()).map((SchemafulTupleSet tuples) -> new SchemafulTupleSet.Builder(parameterSpace.getParameterNames()).addAll(tuples.stream().map((Tuple tuple) -> {
        ...
        return builder.build();
      }).collect(toList())).build()).orElseThrow(TestDefinitionException::noParameterFound);
    }
    ...
    private FactorSpace toFactorSpaceForNegativeTestGeneration(ParameterSpace parameterSpace) {
      ...
      return FactorSpace.create(parameterSpace.getParameterNames().stream().map((String s) -> {
        Parameter<Object> parameter = parameterSpace.getParameter(s);
        return Factor.create(s, parameter.getKnownValues().toArray());
      }).collect(toList()), new ArrayList<>(parameterSpace.getConstraints()));
    }
    ...
    private Generator negativeTestGenerator(boolean generateNegativeTests, FactorSpace factorSpace, List<Tuple> tuplesForRegularTests, List<Tuple> encodedSeeds, Requirement requirement) {
      return generateNegativeTests ? new Negative(tuplesForRegularTests, encodedSeeds, factorSpace, requirement) : new Passthrough(tuplesForRegularTests, factorSpace, requirement);
    }
    ...
    private Parameter<?> toSimpleParameterIfNecessary(Config config, Parameter<?> parameter, List<Constraint> constraints) {
      if (!(parameter instanceof Parameter.Simple) && isInvolvedByAnyConstraint(parameter, constraints)) {
        List<Object> values = Stream.concat(parameter.getKnownValues().stream(), engine(config, new ParameterSpace.Builder().addParameter(parameter).build()).stream().map((tuple) -> tuple.get(parameter.getName()))).collect(toList());
        return Parameter.Simple.Factory.of(Utils.unique(
<<<<<<< /usr/src/app/output/dakusui/jcunit/462dfb4cb47c28e87efb37d8632033bd845ed3d9/src/main/java/com/github/dakusui/jcunit8/pipeline/Pipeline.java/left.java
        Stream.concat(parameter.getKnownValues().stream(), engine(config, new ParameterSpace.Builder().addParameter(parameter).build()).stream().map((tuple) -> tuple.get(parameter.getName()))).collect(toList())
=======
        values
>>>>>>> /usr/src/app/output/dakusui/jcunit/462dfb4cb47c28e87efb37d8632033bd845ed3d9/src/main/java/com/github/dakusui/jcunit8/pipeline/Pipeline.java/right.java
