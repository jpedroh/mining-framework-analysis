 ...
 * A pipeline object.
 */
@SuppressWarnings(value = { "unchecked" }) public interface Pipeline {
  TestSuite execute(Config config, ParameterSpace parameterSpace);

  class Standard implements Pipeline {
    @Override public TestSuite execute(Config config, ParameterSpace parameterSpace) {
    ...
    @SuppressWarnings(value = { "unchecked" }) private Parameter toSimpleParameterIfNecessary(Config config, Parameter parameter, List<Constraint> constraints) {
      if (!(parameter instanceof Parameter.Simple) && isInvolvedByAnyConstraint(parameter, constraints)) {
        return Parameter.Simple.Factory.of(Utils.unique(Stream.<Object>concat(parameter.getKnownValues().stream(), engine(config, new ParameterSpace.Builder().addParameter(parameter).build()).stream().map((tuple) -> tuple.get(parameter.getName()))).collect(toList()))).create(parameter.getName());
      }
      return parameter;
    }
