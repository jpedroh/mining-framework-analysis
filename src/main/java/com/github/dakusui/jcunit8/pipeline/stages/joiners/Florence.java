...
@SuppressWarnings(value = { "NonAsciiCharacters" }) public class Florence extends Joiner.Base {
    ...
    for (int i = 0; i < rhs.width(); i++) {
      ...
      try {
        ...
        while (!π.isEmpty()) {
          ...
          try {
            long max = tuplesRemovedFromπ < 0 ? new Combinator<>(involvedFactors, t).size() : tuplesRemovedFromπ;
            Tuple n = session.chooseBestCombination(π, lhs, rhs, involvedFactors, max);
  ...
  private void removeAlreadyCoveredTuples(TupleSet π, Collection<Tuple> ts, int t) {
    ts.forEach((tuple) -> π.removeAll(TupleUtils.subtuplesOf(tuple, t)));
  }
  ...
  private static class Session {
    ...
    Tuple chooseBestCombination(TupleSet π, SchemafulTupleSet lhs, SchemafulTupleSet rhs, List<String> involvedFactors, long max) {
      ...
      return lhs.stream().map((tuple) -> new Entry(tuple, simplify(rhs, involvedFactors))).max(comparingInt((o) -> countOverlappingTuples(o.tuple, π))).map((entry) -> Utils.max(entry.candidates.stream(), max, (t) -> (long) countTuplesCoveredBy(t, entry.tuple, π)).map((chosenFromCandidates) -> connect(entry.tuple, chosenFromCandidates)).orElseGet(() -> {
