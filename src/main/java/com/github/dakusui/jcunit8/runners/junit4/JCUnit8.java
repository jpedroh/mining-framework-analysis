...
public class JCUnit8 extends org.junit.runners.Parameterized {
  ...
  public JCUnit8(Class<?> klass) throws Throwable {
    ...
    this.testSuite = buildTestSuite(configFactory.create(), buildParameterSpace(new ArrayList<>(buildParameterMap(parameterSpaceDefinition).values()), NodeUtils.allTestPredicates(createParameterSpaceDefinitionTestClass()).values().stream().filter((each) -> each instanceof Constraint).map(Constraint.class::cast).collect(toList())));
  ...
  private TestClassValidator[] createValidatorsFor(TestClass parameterSpaceDefinitionClass) {
    return new TestClassValidator[] { new AnnotationsValidator(), new PublicClassValidator(), new TestClassValidator() {
      @Override public List<Exception> validateTestClass(TestClass testClass) {
      ...
      private void validateFromAnnotationsAreReferencingExistingParameterSourceMethods(TestClass testClass, List<Exception> errors) {
        testClass.getAnnotatedMethods(Test.class).forEach((frameworkMethod) -> Stream.of(frameworkMethod.getMethod().getParameterAnnotations()).forEach((Annotation[] annotations) -> Stream.of(annotations).filter((Annotation annotation) -> annotation instanceof From).forEach((Annotation annotation) -> {
          List<FrameworkMethod> methods = parameterSpaceDefinitionClass.getAnnotatedMethods(ParameterSource.class).stream().filter((FrameworkMethod each) -> Objects.equals(each.getName(), From.class.cast(annotation).value())).collect(toList());
          if (methods.isEmpty()) {
            errors.add(new Exception(format("A method \'%s\' annotated with \'%s\' is not defined in \'%s\'", From.class.cast(annotation).value(), ParameterSource.class.getSimpleName(), parameterSpaceDefinitionClass.getJavaClass().getCanonicalName())));
          }
        })));
      }
    } };
  ...
  private TestClass createParameterSpaceDefinitionTestClass() {
    ...
    return Objects.equals(parameterSpaceClass, ConfigureWith.DEFAULT_INSTANCE.parameterSpace()) ? this.getTestClass() : new TestClass(parameterSpaceClass);
  }
  ...
  private List<Runner> createRunners() {
    ...
    return this.testSuite.stream().map((Function<TestCase, Runner>) (tupleTestCase) -> {
      try {
  ...
  private static SortedMap<String, com.github.dakusui.jcunit8.factorspace.Parameter> buildParameterMap(TestClass parameterSpaceDefinitionTestClass) {
    return new TreeMap<String, com.github.dakusui.jcunit8.factorspace.Parameter>() {
      {
        parameterSpaceDefinitionTestClass.getAnnotatedMethods(ParameterSource.class).forEach((frameworkMethod) -> put(frameworkMethod.getName(), buildParameterFactoryCreatorFrom(frameworkMethod).apply(Utils.createInstanceOf(parameterSpaceDefinitionTestClass)).create(frameworkMethod.getName())));
      }
    };
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private static <A extends Annotation> List<A> getParameterAnnotationsFrom(FrameworkMethod method, Class<A> annotationClass) {
   ...
   */
  @Ignore @SuppressWarnings(value = { "unused", "WeakerAccess" }) public static void noMatchingTestMethodIsFoundForThisTestCase() {
  }
  ...
  private class MyBlockJUnit4ClassRunner extends BlockJUnit4ClassRunner {
    ...
    @Override public List<FrameworkMethod> getChildren() {
      try {
    ...
    @Override protected Statement methodInvoker(final FrameworkMethod method, final Object test) {
      return new InvokeMethod(method, test) {
        @Override public void evaluate() throws Throwable {
          Object[] args = validateArguments(method, method.getMethod().getParameterTypes(), getParameterAnnotationsFrom(method, From.class).stream().map(From::value).map((s) -> tupleTestCase.get().get(s)).collect(toList()).toArray());
          method.invokeExplosively(test, args);
        }
      };
    }

    @SuppressWarnings(value = { "unchecked" }) private Object[] validateArguments(FrameworkMethod method, Class[] parameterClasses, Object[] argumentValues) {
    ...
    String composeErrorMessageForTypeMismatch(Object argumentValue, FrameworkMethod method, int parameterIndex) {
      return String.format("\'%s\' is not compatible with parameter %s of \'%s(%s)\'", argumentValue, parameterIndex, method.getName(), Arrays.stream(method.getMethod().getParameterTypes()).map(Class::getSimpleName).collect(joining(",")));
    }

    @Override public Object createTest() {
    ...
    private boolean shouldInvoke(FrameworkMethod each, Tuple tuple) {
      ...
      return NodeUtils.buildPredicate(each.getAnnotation(Given.class).value(), createParameterSpaceDefinitionTestClass()).test(tuple);
    }
