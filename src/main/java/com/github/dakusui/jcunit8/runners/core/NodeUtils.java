...
<<<<<<< /usr/src/app/output/dakusui/jcunit/ca5f167b3c86b6b2e90f8b1fe0425f65a6748713/src/main/java/com/github/dakusui/jcunit8/runners/core/NodeUtils.java/left.java
  public static <T extends java.lang.Object> Predicate<T> buildPredicate(String[] values, TestClass parameterSpaceDefinitionClass) {
    class Builder implements Node.Visitor {
      ...
      @SuppressWarnings(value = { "unchecked" }) @Override public void visitLeaf(Node.Leaf leaf) {
        result = (Predicate<T>) lookupTestPredicate(leaf.id()).orElseThrow(FrameworkException::unexpectedByDesign);
      }

      @Override public void visitAnd(Node.And and) {
        and.children().forEach((node) -> {
      ...
      @Override public void visitOr(Node.Or or) {
        or.children().forEach((node) -> {
          Predicate<T> previous = result;
          node.accept(this);
          if (previous != null) {
      ...
      @Override public void visitNot(Node.Not not) {
        not.target().accept(this);
        result = result.negate();
      }

      private Optional<TestPredicate> lookupTestPredicate(String name) {
        return this.predicates.containsKey(name) ? Optional.of(this.predicates.get(name)) : Optional.empty();
      }
    }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  public static TestPredicate buildPredicate(String[] values, TestClass parameterSpaceDefinitionClass) {
    class Builder implements Node.Visitor {
      ...
      @Override public void visitLeaf(Node.Leaf leaf) {
    ...
    Builder builder = new Builder();
    parse(values).accept(builder);
    return TestPredicate.of(builder.involvedKeys.stream().collect(toList()), builder.result);
  }

  public static List<String> allLeaves(String[] values) {
    return new LinkedList<String>() {
      {
        parse(values).accept(new Node.Visitor.Base() {
          @Override public void visitLeaf(Node.Leaf leaf) {
            add(leaf.id());
          }
        });
  ...
  public static SortedMap<String, TestPredicate> allTestPredicates(TestClass parameterSpaceDefinitionClass) {
    ...
    return new TreeMap<>(parameterSpaceDefinitionClass.getAnnotatedMethods(Condition.class).stream().collect(toMap(FrameworkMethod::getName, (frameworkMethod) -> createTestPredicate(testObject, frameworkMethod))));
  }

  public static TestPredicate createTestPredicate(Object testObject, FrameworkMethod method) {
    List<String> involvedKeys = Stream.of(method.getMethod().getParameterAnnotations()).map((annotations) -> Stream.of(annotations).filter((annotation) -> annotation instanceof From).map(From.class::cast).map(From::value).findFirst().<FrameworkException>orElseThrow(FrameworkException::unexpectedByDesign)).collect(toList());
    Predicate<Tuple> predicate = (tuple) -> {
      try {
        return (boolean) method.invokeExplosively(testObject, involvedKeys.stream().map(tuple::get).toArray());
      } catch (Throwable e) {
        throw unexpectedByDesign(e);
      }
    };
    return method.getAnnotation(Condition.class).constraint() ? Constraint.create(predicate, involvedKeys) : new TestPredicate() {
      @Override public boolean test(Tuple tuple) {
        return predicate.test(tuple);
      }

      @Override public List<String> involvedKeys() {
        return involvedKeys;
      }
    };
  }
  ...
  public static Node parse(String[] values) {
    return new Node.Or.Impl(Stream.of(values).map(NodeUtils::parseLine).collect(toList()));
  }

  public static Node parseLine(String value) {
    return new Node.And.Impl(Stream.of(value.split("&&")).map((s) -> s.startsWith("!") ? new Node.Not.Impl(new Node.Leaf.Impl(s.substring(1))) : new Node.Leaf.Impl(s)).collect(toList()));
  }
}