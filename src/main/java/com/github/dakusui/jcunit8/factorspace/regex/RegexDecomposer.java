...
public class RegexDecomposer extends RegexTranslator {
  ...
  private List<Constraint> buildConstraints(List<? extends Factor> factors) {
    ...
    for (final Factor each : factors) {
      final List<String> referrers = getReferringFactors(each, factors).stream().map(Factor::getName).collect(Collectors.toList());
      ...
      ret.add(new Constraint() {
        @Override public boolean test(Tuple in) {
        ...
        @SuppressWarnings(value = { "unchecked" }) boolean isReferencedBy(Object referrerValue) {
          return ((List<Object>) requireNonNull(referrerValue)).stream().anyMatch((Object each) -> each instanceof Reference && ((Reference) each).key.equals(referee));
        }
      });
    }
    return ret;
