 ...
 * @author Richard Kerr
 */
@Mojo(name = "nar-process-libraries", defaultPhase = LifecyclePhase.PROCESS_CLASSES, requiresProject = true) public class NarProcessLibraries extends AbstractCompileMojo {
  /**
     * List of commands to execute
     *
     * @parameter default-value=""
     */
  @Parameter private List<ProcessLibraryCommand> commands;
  ...
  @Override public void narExecute() throws MojoFailureException, MojoExecutionException {
    ...
    for (Library library : getLibraries()) {
      ...
      if (type.equalsIgnoreCase(Library.EXECUTABLE)) {
        File outDir = getLayout().getBinDirectory(getTargetDirectory(), getMavenProject().getArtifactId(), getMavenProject().getVersion(), getAOL().toString());
        outFile = new File(outDir, getOutput(false));
      } else {
        File outDir = getLayout().getLibDirectory(getTargetDirectory(), getMavenProject().getArtifactId(), getMavenProject().getVersion(), getAOL().toString(), type);
