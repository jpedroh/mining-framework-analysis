...
public abstract class CommandLineCompiler extends AbstractCompiler {
  ...
  protected CommandLineCompiler(final String command, final String identifierArg, final String[] sourceExtensions, final String[] headerExtensions, final String outputSuffix, final boolean libtool, final CommandLineCompiler libtoolCompiler, final boolean newEnvironment, final Environment env) {
  ...
  abstract protected void addImpliedArgs(Vector<String> args, boolean debug, boolean multithreaded, boolean exceptions, LinkType linkType, Boolean rtti, OptimizationEnum optimization);
   ...
   */
  protected void addIncludes(final String baseDirPath, final File[] includeDirs, final Vector<String> args, final Vector<String> relativeArgs, final StringBuffer includePathId, final boolean isSystem) {
   ...
   *
   */
  public void compile(final CCTask task, final File outputDir, final String[] sourceFiles, String[] args, final String[] endArgs, final boolean relentless, final CommandLineCompilerConfiguration config, final ProgressMonitor monitor) throws BuildException {
    ...
    for (int sourceIndex = 0; sourceIndex < sourceFiles.length; ) {
      ...
      for (firstFileNextExec = sourceIndex; firstFileNextExec < sourceFiles.length && firstFileNextExec - sourceIndex < maxInputFilesPerCommand; firstFileNextExec++) {
  ...
  @Override protected CompilerConfiguration createConfiguration(final CCTask task, final LinkType linkType, final ProcessorDef[] baseDefs, final CompilerDef specificDef, final TargetDef targetPlatform, final VersionInfo versionInfo) {
  ...
  @Override public String getIdentifier() {
    if (this.identifier == null) {
      if (this.identifierArg == null) {
        this.identifier = getIdentifier(new String[] { this.command }, this.command);
      } else {
        this.identifier = getIdentifier(new String[] { this.command, this.identifierArg }, this.command);
      }
    }
