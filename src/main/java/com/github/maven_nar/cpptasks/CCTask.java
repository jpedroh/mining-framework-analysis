...
public class CCTask extends Task {
  ...
  public static Map getTargetsToBuildByConfiguration(Map targets) {
    ...
    while (targetEnum.hasNext()) {
      ...
      if (target.getRebuild()) {
        Vector targetsForSameConfig = (Vector) targetsByConfig.get(target.getConfiguration());
  ...
  protected int checkForChangedIncludeFiles(Map targets) {
    ...
    if (potentialTargets > 0) {
      ...
      while (targetEnum.hasNext()) {
        TargetInfo target = (TargetInfo) targetEnum.next();
        if (!target.getRebuild()) {
          if (dependencyTable.needsRebuild(this, target, dependencyDepth)) {
            target.mustRebuild();
          }
        }
      }
  ...
  protected LinkerConfiguration collectExplicitObjectFiles(Vector objectFiles, Vector sysObjectFiles, VersionInfo versionInfo) {
    ...
    for (int i = 0; i < _linkers.size(); i++) {
      ...
      if (currentLinkerDef.isActive()) {
        ...
        if (selectedLinker != null) {
          linkerConfig = currentLinkerDef.createConfiguration(this, linkType, linkerDef, targetPlatform, versionInfo);
    ...
    if (linkerConfig == null) {
      ...
      selectedLinker = (Linker) linkerDef.getProcessor().getLinker(linkType);
      objCollector = new ObjectFileCollector(selectedLinker, objectFiles);
      sysLibraryCollector = new SystemLibraryCollector(selectedLinker, sysLibraries);
    }
  ...
  public void execute() throws BuildException {
    ...
    LinkerConfiguration linkerConfig = collectExplicitObjectFiles(objectFiles, sysObjectFiles, versionInfo);
    ...
    if (projects.size() > 0) {
      ...
      for (int i = 0; i < _compilers.size(); i++) {
        CompilerDef currentCompilerDef = (CompilerDef) _compilers.elementAt(i);
    ...
    if (rebuildCount > 0) {
      ...
      while (targetVectorEnum.hasNext()) {
        Vector targetsForConfig = (Vector) targetVectorEnum.next();
      ...
      for (int i = 0; i < targetVectors.length; i++) {
        ...
        for (int j = 0; j < cores.length; j++) {
          cores[j] = new Core(this, j, config, _objDir, sourceFiles[j], relentless, monitor);
    ...
    if (dependencyDepth >= 0) {
      throw new BuildException("All files at depth " + Integer.toString(dependencyDepth) + " from changes successfully compiled.\n" + "Remove or change dependencyDepth to -1 to perform full compilation.");
    }
    if (linkTarget != null) {
  ...
  class Core extends Thread {
    ...
    Core(CCTask task, int coreNo, CompilerConfiguration config, File objDir, List set, boolean relentless, CCTaskProgressMonitor monitor) {
      super("Core " + coreNo);
      this.task = task;
      this.config = config;
      this.objDir = objDir;
  ...
  protected TargetInfo getLinkTarget(LinkerConfiguration linkerConfig, Vector objectFiles, Vector sysObjectFiles, Map compileTargets, VersionInfo versionInfo) {
    ...
    while (compileTargetsIterator.hasNext()) {
      TargetInfo compileTarget = (TargetInfo) compileTargetsIterator.next();
  ...
  /**
     ...
     *  
     */
  private Map getTargets(LinkerConfiguration linkerConfig, Vector objectFiles, VersionInfo versionInfo, File outputFile) {
      ...
      if (currentCompilerDef.isActive()) {
        ...
        if (precompileDef != null) {
          ...
          if (configs != null && configs.length == 2) {
            TargetMatcher matcher = new TargetMatcher(this, _objDir, new ProcessorConfiguration[] { configs[0] }, linkerConfig, objectFiles, targets, versionInfo);
        ...
        if (currentCompilerDef.hasFileSets()) {
          TargetMatcher matcher = new TargetMatcher(this, _objDir, localConfigs, linkerConfig, objectFiles, targets, versionInfo);
          currentCompilerDef.visitFiles(matcher);
        }
        biddingProcessors.addElement(config);
      }
    }
    ProcessorConfiguration config = compilerDef.createConfiguration(this, linkType, null, targetPlatform, versionInfo);
    biddingProcessors.addElement(config);
    ProcessorConfiguration[] bidders = new ProcessorConfiguration[biddingProcessors.size()];
    biddingProcessors.copyInto(bidders);
    TargetMatcher matcher = new TargetMatcher(this, _objDir, bidders, linkerConfig, objectFiles, targets, versionInfo);
    compilerDef.visitFiles(matcher);
    if (outputFile != null && versionInfo != null) {
      boolean isDebug = linkerConfig.isDebug();
      try {
        linkerConfig.getLinker().addVersionFiles(versionInfo, linkType, outputFile, isDebug, _objDir, matcher);
      } catch (IOException ex) {
