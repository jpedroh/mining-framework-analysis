...
public abstract class AbstractDependencyMojo extends AbstractNarMojo {
  /**
	 ...
	 * @readonly
	 */
  @Parameter(defaultValue = "${localRepository}", required = true, readonly = true) private ArtifactRepository localRepository;
  ...
  /**
     ...
     * @required
     */
  @Component(role = org.codehaus.plexus.archiver.manager.ArchiverManager.class) protected ArchiverManager archiverManager;
  ...
  public final NarInfo getNarInfo(Artifact dependency) throws MojoExecutionException {
    ...
    if (dependency.getFile().isDirectory()) {
      ...
      return new NarInfo(dependency.getGroupId(), dependency.getArtifactId(), dependency.getBaseVersion(), getLog(), dependency.getFile());
    }
    File file = new File(getLocalRepository().getBasedir(), getLocalRepository().pathOf(dependency));
    if (!file.exists()) {
  ...
  public final List<AttachedNarArtifact> getAllAttachedNarArtifacts(List<NarArtifact> narArtifacts) throws MojoExecutionException, MojoFailureException {
    ...
    for (Iterator<NarArtifact> i = narArtifacts.iterator(); i.hasNext(); ) {
      ...
      String binding = getBinding(dependency);
      artifactList.addAll(getAttachedNarArtifacts(dependency, getAOL(), binding));
  ...
  public final void unpackAttachedNars(List<AttachedNarArtifact> dependencies) throws MojoExecutionException, MojoFailureException {
    ...
    for (Iterator i = dependencies.iterator(); i.hasNext(); ) {
      ...
      File file = getNarManager().getNarFile(dependency);
  ...
  public void narExecute() throws MojoFailureException, MojoExecutionException {
    ...
    List<AttachedNarArtifact> dependencies = getAllAttachedNarArtifacts(narArtifacts);
