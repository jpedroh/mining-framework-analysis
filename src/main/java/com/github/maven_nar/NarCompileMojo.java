...
@Mojo(name = "nar-compile", defaultPhase = LifecyclePhase.COMPILE, requiresProject = true, requiresDependencyResolution = ResolutionScope.COMPILE) public class NarCompileMojo extends AbstractCompileMojo {
  ...
  private void createLibrary(final Project antProject, final Library library) throws MojoExecutionException, MojoFailureException {
    ...
    final LinkerDef linkerDefinition = getLinker().getLinker(this, antProject, getOS(), getAOL().getKey() + ".linker.", type);
    ...
    if (getRuntime(getAOL()).equals("dynamic") && getOS().equals(OS.WINDOWS) && getLinker().getName(null, null).equals("msvc") && !getLinker().getVersion(this).startsWith("6.")) {
      ...
      if (libType.equals(Library.JNI) || libType.equals(Library.SHARED)) {
        ...
        final int result = NarUtil.runCommand("cmd", new String[] { "/C", "mt.exe", "/manifest", manifest, "/outputresource:" + dll + ";#2" }, null, env, getLog());
        if (result != 0) {
          throw new MojoFailureException("MT.EXE failed with exit code: " + result);
        }
      } else {
        if (libType.equals(Library.EXECUTABLE)) {
          final String exe = outFile.getPath() + ".exe";
          final String manifest = exe + ".manifest";
          final int result = NarUtil.runCommand("cmd", new String[] { "/C", "mt.exe", "/manifest", manifest, "/outputresource:" + exe + ";#1" }, null, env, getLog());
          if (result != 0) {
            throw new MojoFailureException("MT.EXE failed with exit code: " + result);
          }
        }
   ...
   */
  @Override protected List<Artifact> getArtifacts() {
    try {
