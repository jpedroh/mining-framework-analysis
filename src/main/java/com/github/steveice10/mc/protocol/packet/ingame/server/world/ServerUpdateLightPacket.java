...
@Data @With @Setter(value = AccessLevel.NONE) @NoArgsConstructor(access = AccessLevel.PRIVATE) public class ServerUpdateLightPacket implements Packet {
  ...
  @Override public void read(NetInput in) throws IOException {
    ...
    for (int i = 0; i < this.blockLight.length; i++) {
      if ((blockLightMask[0] & 1 << i) != 0) {
        ...
        if ((emptyBlockLightMask[0] & 1 << i) != 0) {
  ...
  @Override public void write(NetOutput out) throws IOException {
  ...
  @Override public boolean isPriority() {
    return false;
  }
}