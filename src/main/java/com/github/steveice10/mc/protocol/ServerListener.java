...
public class ServerListener extends SessionAdapter {
  ...
  @Override public void connected(ConnectedEvent event) {
  ...
  @Override public void packetReceived(PacketReceivedEvent event) {
    ...
    if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
      if (event.getPacket() instanceof ServerboundHelloPacket) {
        ...
        if (event.getPacket() instanceof ServerboundKeyPacket) {
    ...
    if (protocol.getSubProtocol() == SubProtocol.STATUS) {
      if (event.getPacket() instanceof ServerboundStatusRequestPacket) {
        ...
        if (builder == null) {
          builder = (session) -> new ServerStatusInfo(VersionInfo.CURRENT, new PlayerInfo(0, 20, new GameProfile[0]), Component.text("A Minecraft Server"), null);
        }
        ServerStatusInfo info = builder.buildInfo(event.getSession());
        event.getSession().send(new ClientboundStatusResponsePacket(info));
      } else {
        if (event.getPacket() instanceof ServerboundPingRequestPacket) {
  ...
  @Override public void packetSent(PacketSentEvent event) {
  ...
  @Override public void disconnecting(DisconnectingEvent event) {
    MinecraftProtocol protocol = (MinecraftProtocol) event.getSession().getPacketProtocol();
    if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
