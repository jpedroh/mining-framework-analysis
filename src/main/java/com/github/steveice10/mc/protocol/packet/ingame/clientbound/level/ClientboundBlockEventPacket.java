...
@Data @With @AllArgsConstructor public class ClientboundBlockEventPacket implements MinecraftPacket {
    ...
    if (this.blockId == NOTE_BLOCK) {
      ...
      if (this.blockId == STICKY_PISTON || this.blockId == PISTON) {
        ...
        if (this.blockId == MOB_SPAWNER) {
          ...
          if (this.blockId == CHEST || this.blockId == ENDER_CHEST || this.blockId == TRAPPED_CHEST || (this.blockId >= SHULKER_BOX_LOWER && this.blockId <= SHULKER_BOX_HIGHER)) {
  ...
  @Override public void serialize(ByteBuf out, MinecraftCodecHelper helper) {
    int val = 0;
    int type = 0;
