 ...
 */
@AllArgsConstructor public class ClientListener extends SessionAdapter {
    ...
    if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
      if (event.getPacket() instanceof ClientboundHelloPacket) {
        ...
        if (event.getPacket() instanceof ClientboundGameProfilePacket) {
          ...
          if (event.getPacket() instanceof ClientboundLoginDisconnectPacket) {
      ...
      if (protocol.getSubProtocol() == SubProtocol.STATUS) {
        if (event.getPacket() instanceof ClientboundStatusResponsePacket) {
          ...
          if (event.getPacket() instanceof ClientboundPongResponsePacket) {
  ...
  @Override public void connected(ConnectedEvent event) {
    if (this.targetSubProtocol == SubProtocol.LOGIN) {
      ...
      if (this.targetSubProtocol == SubProtocol.STATUS) {
