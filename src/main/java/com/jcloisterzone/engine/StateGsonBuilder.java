...
>>>>>>> /usr/src/app/output/farin/jcloisterzone/692611d036c53048b40f0ae2e2dfc355fc6857d5/src/main/java/com/jcloisterzone/engine/StateGsonBuilder.java/right.java
    .class, new 
<<<<<<< /usr/src/app/output/farin/jcloisterzone/692611d036c53048b40f0ae2e2dfc355fc6857d5/src/main/java/com/jcloisterzone/engine/StateGsonBuilder.java/left.java
    BardsLuteActionSerializer
=======
    SelectFeatureActionSerializer
>>>>>>> /usr/src/app/output/farin/jcloisterzone/692611d036c53048b40f0ae2e2dfc355fc6857d5/src/main/java/com/jcloisterzone/engine/StateGsonBuilder.java/right.java
    ());
  ...
  public JsonElement serializeTokens(GameState root, JsonSerializationContext context) {
    ...
    if (littleBuildings != null) {
      ...
      littleBuildings.forEach((pos, lb) -> {
        if (lb == LittleBuilding.LB_SHED) {
          shed.add(context.serialize(pos));
        }
        if (lb == LittleBuilding.LB_HOUSE) {
          house.add(context.serialize(pos));
        }
        if (lb == LittleBuilding.LB_TOWER) {
          tower.add(context.serialize(pos));
        }
      });
      if (shed.size() > 0) {
  ...
  private class TilePackSerializer implements JsonSerializer<TilePack> {
    @Override public JsonElement serialize(TilePack state, Type type, JsonSerializationContext jsonSerializationContext) {
      JsonObject json = new JsonObject();
  ...
  private class ActionsStateSerializer implements JsonSerializer<ActionsState> {
    @Override public JsonElement serialize(ActionsState state, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class ActionSerializer implements JsonSerializer<AbstractPlayerAction<?>> {
    ...
    @Override public JsonElement serialize(AbstractPlayerAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class SelectFeatureActionSerializer implements JsonSerializer<SelectFeatureAction> {
    @Override public JsonElement serialize(SelectFeatureAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class TilePlacementActionSerializer implements JsonSerializer<TilePlacementAction> {
    @Override public JsonElement serialize(TilePlacementAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
      ...
      action.getOptions().groupBy(PlacementOption::getPosition).forEach((pos, group) -> {
        ...
        group.map(PlacementOption::getRotation).toArray().sorted().forEach((rot) -> {
          rotations.add(rotationToPrimitive(rot));
        });
  ...
  private class MeepleActionSerializer implements JsonSerializer<MeepleAction> {
    @Override public JsonElement serialize(MeepleAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class NeutralFigureActionSerializer implements JsonSerializer<NeutralFigureAction> {
    @Override public JsonElement serialize(NeutralFigureAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class FerriesActionSerializer implements JsonSerializer<FerriesAction> {
    @Override public JsonElement serialize(FerriesAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class MoveDragonActionSerializer implements JsonSerializer<MoveDragonAction> {
    @Override public JsonElement serialize(MoveDragonAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class FairyNextToActionSerializer implements JsonSerializer<FairyNextToAction> {
    @Override public JsonElement serialize(FairyNextToAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class FairyOnTileActionSerializer implements JsonSerializer<FairyOnTileAction> {
    @Override public JsonElement serialize(FairyOnTileAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class ReturnMeepleActionSerializer implements JsonSerializer<ReturnMeepleAction> {
    @Override public JsonElement serialize(ReturnMeepleAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class CaptureFollowerActionSerializer implements JsonSerializer<CaptureFollowerAction> {
    @Override public JsonElement serialize(CaptureFollowerAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class TowerPieceActionSerializer implements JsonSerializer<TowerPieceAction> {
    @Override public JsonElement serialize(TowerPieceAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class SelectPrisonerToExchangeActionSerializer implements JsonSerializer<SelectPrisonerToExchangeAction> {
    @Override public JsonElement serialize(SelectPrisonerToExchangeAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class CornCircleSelectDeployOrRemoveActionSerializer implements JsonSerializer<CornCircleSelectDeployOrRemoveAction> {
    @Override public JsonElement serialize(CornCircleSelectDeployOrRemoveAction action, Type type, JsonSerializationContext jsonSerializationContext) {
      JsonObject json = new JsonObject();
  ...
  private class FlockActionSerializer implements JsonSerializer<FlockAction> {
    @Override public JsonElement serialize(FlockAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class TunnelActionSerializer implements JsonSerializer<TunnelAction> {
    @Override public JsonElement serialize(TunnelAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class GoldPieceActionSerializer implements JsonSerializer<GoldPieceAction> {
    @Override public JsonElement serialize(GoldPieceAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class LittleBuildingActionSerializer implements JsonSerializer<LittleBuildingAction> {
    @Override public JsonElement serialize(LittleBuildingAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
  ...
  private class BardsLuteActionSerializer implements JsonSerializer<BardsLuteAction> {
    @Override public JsonElement serialize(BardsLuteAction action, Type type, JsonSerializationContext context) {
      JsonObject json = new JsonObject();
