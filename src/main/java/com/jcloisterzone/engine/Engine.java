...
public class Engine implements Runnable {
  @Argument(alias = "p", description = "Use socket connection on given port instead of stdin stdout") private static Integer port;

  @Argument(alias = "r", description = "Rerun engine on game end.") private static Boolean reload = false;
  ...
  private Map<Class<? extends Meeple>, Integer> addMeeples(Map<Class<? extends Meeple>, Integer> meeples, GameSetupMessage setupMsg, String key, Class<? extends Meeple> cls) {
  ...
  private Set<Class<? extends Capability<?>>> addCapabilities(Set<Class<? extends Capability<?>>> capabilities, GameSetupMessage setupMsg, String key, Class<? extends Capability<?>> cls) {
  ...
  private GameSetup createSetupFromMessage(GameSetupMessage setupMsg) {
    ...
    capabilities = addCapabilities(capabilities, setupMsg, 
<<<<<<< /usr/src/app/output/farin/jcloisterzone/692611d036c53048b40f0ae2e2dfc355fc6857d5/src/main/java/com/jcloisterzone/engine/Engine.java/left.java
    "bards-lute"
=======
    "robbers-son"
>>>>>>> /usr/src/app/output/farin/jcloisterzone/692611d036c53048b40f0ae2e2dfc355fc6857d5/src/main/java/com/jcloisterzone/engine/Engine.java/right.java
    , 
<<<<<<< /usr/src/app/output/farin/jcloisterzone/692611d036c53048b40f0ae2e2dfc355fc6857d5/src/main/java/com/jcloisterzone/engine/Engine.java/left.java
    BardsLuteCapability
=======
    RobbersSonCapability
>>>>>>> /usr/src/app/output/farin/jcloisterzone/692611d036c53048b40f0ae2e2dfc355fc6857d5/src/main/java/com/jcloisterzone/engine/Engine.java/right.java
    .class);
    ...
    GameSetup gameSetup = new GameSetup(HashMap.ofAll(setupMsg.getSets()), HashMap.ofAll(setupMsg.getElements()), meeples, capabilities, rules, io.vavr.collection.List.ofAll(setupMsg.getStart()));
    return gameSetup;
  }
  ...
  @Override public void run() {
    ...
    while (!gameIsOver) {
      ...
      if (msg instanceof ReplayableMessage) {
        ...
        boolean undoAllowed = (!(msg instanceof RandomChangingMessage) || ((RandomChangingMessage) msg).getRandom() == null) && newActivePlayer != null && newActivePlayer.equals(oldActivePlayer) && !(msg instanceof DeployMeepleMessage && ((DeployMeepleMessage) msg).getMeepleId().contains("shepherd")) && !(msg instanceof MoveNeutralFigureMessage && ((MoveNeutralFigureMessage) msg).getFigureId().contains("dragon"));
        if (undoAllowed) {
          game.markUndo();
        } else {
          game.clearUndo();
