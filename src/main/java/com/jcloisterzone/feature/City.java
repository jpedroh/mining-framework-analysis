...
public class City extends CompletableFeature<City> implements ModifiedFeature<City> {
  ...
  public City(List<FeaturePointer> places, Set<Edge> openEdges, Set<FeaturePointer> neighboring, Set<Tuple2<ShortEdge, FeaturePointer>> multiEdges, Map<FeatureModifier<?>, Object> modifiers) {
    super(places, openEdges, neighboring);
    this.multiEdges = multiEdges;
    this.modifiers = modifiers;
  }

  @Override public Map<FeatureModifier<?>, Object> getModifiers() {
    return modifiers;
  }

  @Override public City setModifiers(Map<FeatureModifier<?>, Object> modifiers) {
  ...
  @Override public City merge(City city) {
    assert city != this;
    return new City(mergePlaces(city), mergeEdges(city), mergeNeighboring(city), mergeMultiEdges(city), mergeModifiers(city));
  }

  @Override public City closeEdge(Edge edge) {
    return new City(places, openEdges.remove(edge), neighboring, multiEdges.filter((me) -> !me._1.equals(edge)), modifiers);
  }

  @Override public City setOpenEdges(Set<Edge> openEdges) {
    return new City(places, openEdges, neighboring, multiEdges, modifiers);
  }

  @Override public Feature placeOnBoard(Position pos, Rotation rot) {
    return new City(placeOnBoardPlaces(pos, rot), placeOnBoardEdges(pos, rot), placeOnBoardNeighboring(pos, rot), placeOnBoardMultiEdges(pos, rot), modifiers);
  }
  ...
  @Override public City setNeighboring(Set<FeaturePointer> neighboring) {
  ...
  @Override public PointsExpression getStructurePoints(GameState state, boolean completed) {
  ...
  @Override public PointsExpression getPoints(GameState state) {
