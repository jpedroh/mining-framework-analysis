...
public class TileBuilder {
  ...
  private java.util.List<Tuple3<ShortEdge, Location, FeaturePointer>> multiEdges;
  ...
  private void processFieldElement(Element e) {
    ...
    if (e.hasAttribute("city")) {
      ...
      adjoiningCities = HashSet.ofAll(citiesLocs.map((loc) -> new FeaturePointer(Position.ZERO, City.class, loc)));
    } else {
  ...
  public static Set<Edge> initOpenEdges(Stream<Location> sides) {
    return HashSet.ofAll(sides.filter(Location::isEdge).map((loc) -> new Edge(Position.ZERO, loc)));
  }
}