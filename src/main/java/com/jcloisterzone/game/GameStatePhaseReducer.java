...
public class GameStatePhaseReducer implements Function2<GameState, Message, GameState> {
  ...
  public GameStatePhaseReducer(GameSetup setup, double initialRandom) {
    ...
    if (setup.contains(CountCapability.class)) {
      endChain = new CocFinalScoringPhase(randomGenerator, endChain);
    }
    if (setup.contains(AbbeyCapability.class)) {
      endChain = abbeyEndGamePhase = new AbbeyEndGamePhase(randomGenerator, endChain);
    }
    next = cleanUpTurnPhase = new CleanUpTurnPhase(randomGenerator, null);
    if (setup.contains(BazaarCapability.class)) {
      next = new BazaarPhase(randomGenerator, next);
    }
    if (setup.getBooleanRule(Rule.ESCAPE)) {
      next = new EscapePhase(randomGenerator, next);
    }
    next = cleanUpTurnPartPhase = new CleanUpTurnPartPhase(randomGenerator, next);
    if (setup.contains(CornCircleCapability.class)) {
      next = new CornCirclePhase(randomGenerator, next);
    }
    ...
    if (setup.contains(CountCapability.class)) {
      next = new CocFollowerPhase(randomGenerator, next);
    }
    if (setup.contains(BigTopCapability.class)) {
      next = new BigTopPhase(randomGenerator, next);
    }
    if (setup.contains(WagonCapability.class)) {
      next = new WagonPhase(randomGenerator, next);
    }
    next = new ScoringPhase(randomGenerator, next);
    if (setup.contains(CountCapability.class)) {
      next = new CocScoringPhase(randomGenerator, next);
    }
    next = new CommitActionPhase(randomGenerator, next);
    if (setup.contains(CastleCapability.class)) {
      next = new CastlePhase(randomGenerator, next);
    }
    if (setup.contains(DragonCapability.class) && !"after-scoring".equals(setup.getStringRule(Rule.DRAGON_MOVEMENT))) {
      next = new DragonPhase(randomGenerator, next);
    }
    ...
    if (setup.contains(RussianPromosTrapCapability.class)) {
      next = new RussianPromosTrapPhase(randomGenerator, next);
    }
    if (setup.contains(TunnelCapability.class)) {
      next = new TunnelPhase(randomGenerator, next);
    }
    if (setup.contains(PhantomCapability.class)) {
      next = new PhantomPhase(randomGenerator, next);
    }
    if (setup.contains(RussianPromosTrapCapability.class)) {
      next = new RussianPromosTrapPhase(randomGenerator, next);
    }
    next = actionPhase = new ActionPhase(randomGenerator, next);
    if (setup.contains(BardsLuteCapability.class)) {
      next = new BardsLutePhase(randomGenerator, next);
    }
    if (setup.contains(MageAndWitchCapability.class)) {
      next = new MageAndWitchPhase(randomGenerator, next);
    }
    if (setup.contains(GoldminesCapability.class)) {
      next = new GoldPiecePhase(randomGenerator, next);
    }
    ...
    if (setup.contains(FairyCapability.class)) {
      next = new FairyPhase(randomGenerator, next);
    }
    cleanUpTurnPhase.setDefaultNext(next);
  ...
  @Override public GameState apply(GameState state, Message message) {
