...
public class CocFollowerPhase extends Phase {
  ...
  @Override public StepResult enter(GameState state) {
    Stream<ScoreEvent> events = Stream.ofAll(state.getCurrentTurnPartEvents()).filter(Predicates.instanceOf(ScoreEvent.class)).map((ev) -> (ScoreEvent) ev).filter((ev) -> ev.isLandscapeSource());
    ...
    Vector<Class<? extends Meeple>> meepleTypes = Vector.of(SmallFollower.class, BigFollower.class, Phantom.class, Wagon.class, Mayor.class, Ringmaster.class);
    ...
    Stream<Tuple2<FeaturePointer, Feature>> quarters = state.getTileFeatures2(quarterPos).filter((t) -> t._1.getLocation().isCityOfCarcassonneQuarter() && (marketAllowed || t._1.getLocation() != Location.QUARTER_MARKET));
    GameState _state = state;
    Vector<PlayerAction<?>> actions = availMeeples.map((meeple) -> {
      Set<FeaturePointer> locations = quarters.filter((t) -> meeple.isDeploymentAllowed(_state, t._1, (Quarter) t._2) == DeploymentCheckResult.OK).map((t) -> t._1).toSet();
