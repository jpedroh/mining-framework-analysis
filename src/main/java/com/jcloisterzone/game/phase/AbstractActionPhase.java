...
public abstract class AbstractActionPhase extends Phase {
  ...
  private Stream<Tuple2<FeaturePointer, Structure>> getAvailableStructures(GameState state, Stream<PlacedTile> tiles, Set<Position> allowCompletedOn) {
    return tiles.flatMap((tile) -> {
      ...
      if (isCurrentTile && tile.getTile().hasModifier(BardsLuteCapability.BARDS_LUTE)) {
        Location placedTokenLocation = state.getCapabilityModel(BardsLuteCapability.class).filter((t) -> t._1.getPosition().equals(tile.getPosition()) && t._2).map((t) -> t._1).get(0).getLocation();
  ...
  private Set<FeaturePointer> getMeepleAvailableStructures(GameState state, Meeple meeple, Stream<Tuple2<FeaturePointer, Structure>> structures, boolean includeOccupied) {
        ...
        if (struct instanceof Road) {
          ...
          if (road.isLabyrinth(state)) {
            ...
            if (initial.isLabyrinth(state)) {
              ...
              Set<FeaturePointer> segment = road.findSegmentBorderedBy(state, t._1, (fp) -> {
                Road r = (Road) state.getPlacedTile(fp.getPosition()).getInitialFeaturePartOf(fp.getLocation())._2;
                return r.isLabyrinth(state);
              }).toSet();
              boolean segmentIsEmpty = Stream.ofAll(state.getDeployedMeeples()).filter((x) -> segment.contains(x._2)).isEmpty();
              if (segmentIsEmpty) {
                return true;
              }
            }
    ...
    return structures.filter((t) -> meeple.isDeploymentAllowed(state, t._1, t._2) == DeploymentCheckResult.OK).map((t) -> t._1).toSet();
  }
  ...
  protected Vector<PlayerAction<?>> prepareMeepleActions(GameState state, Vector<Class<? extends Meeple>> meepleTypes) {
    ...
    Vector<PlayerAction<?>> actions = availMeeples.map((meeple) -> {
      Set<FeaturePointer> locations = getMeepleAvailableStructures(state, meeple, meeple instanceof Special ? specialMeepleStructures : regularMeepleStructures, false);
  ...
  @Override @PhaseMessageHandler public StepResult handlePayRansom(GameState state, PayRansomMessage msg) {
    state = (new PayRansom(msg.getMeepleId())).apply(state);
    return enter(state);
  }

  @PhaseMessageHandler public StepResult handleDeployMeeple(GameState state, DeployMeepleMessage msg) {
    ...
    if (!fp.getFeature().equals(Tower.class) && placedTile.getTile().hasModifier(PortalCapability.MAGIC_PORTAL) && !fp.getPosition().equals(placedTile.getPosition())) {
  ...
  public StepResult handleDeployFlier(GameState state, DeployMeepleMessage msg) {
    ...
    state = state.appendEvent(new FlierRollEvent(PlayEventMeta.createWithActivePlayer(state), placedTile.getPosition(), distance));
