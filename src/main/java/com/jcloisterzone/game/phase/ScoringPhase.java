...
public class ScoringPhase extends Phase {
  ...
  private void collectClosedByFerries(GameState state) {
    for (Tuple2<Position, Tuple2<Location, Location>> t : state.getCapabilityModel(FerriesCapability.class).getMovedFerries()) {
      ...
      Location to = t._2._2;
      List<FeaturePointer> affected = from.subtract(to).splitToSides().map((loc) -> new FeaturePointer(t._1, Road.class, loc));
  ...
  @Override public StepResult enter(GameState state) {
    ...
    collectCompletedOnAdjacentEdges(state, pos);
    ...
    if (state.getCapabilities().contains(TunnelCapability.class)) {
      ...
      List<Feature> tunnelModified = state.getCurrentTurnEvents().filter(Predicates.instanceOf(TokenPlacedEvent.class)).map((ev) -> (TokenPlacedEvent) ev).filter((ev) -> ev.getToken() instanceof Tunnel).map((ev) -> _state.getFeature((FeaturePointer) ev.getPointer()));
      assert tunnelModified.size() <= 1;
      for (Feature road : tunnelModified) {
        collectCompleted(state, (Completable) road);
      }
    }
    Set<Position> neighbourPositions = state.getAdjacentAndDiagonalTiles2(pos).map((pt) -> pt._2.getPosition()).toSet();
    ...
    if (state.getCapabilities().contains(BarnCapability.class)) {
      ...
      GameState _state = state;
      for (Field field : state.getTileFeatures2(pos).map(Tuple2::_2).filter((f) -> f != placedBarnField).filter(Predicates.instanceOf(Field.class)).map((f) -> (Field) f).filter((f) -> f.getSpecialMeeples(_state).find(Predicates.instanceOf(Barn.class)).isDefined())) {
    ...
    if (!deployedWagonsBefore.isEmpty()) {
      ...
      Queue<Tuple2<Wagon, FeaturePointer>> model = state.getPlayers().getPlayersBeginWith(state.getTurnPlayer()).flatMap((p) -> returnedVagons.filter((w) -> w.getPlayer().equals(p))).map((w) -> new Tuple2<>(w, deployedWagonsBefore.get(w).get())).toQueue();
  ...
  private Map<Wagon, FeaturePointer> getDeployedWagons(GameState state) {
    return state.getDeployedMeeples().filter((m, fp) -> m instanceof Wagon).mapKeys((m) -> (Wagon) m);
  }
