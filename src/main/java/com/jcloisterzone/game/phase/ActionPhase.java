...
public class ActionPhase extends AbstractActionPhase {
  ...
  @Override public StepResult enter(GameState state) {
    Player player = state.getTurnPlayer();
    Vector<Class<? extends Meeple>> meepleTypes = Vector.of(SmallFollower.class, BigFollower.class, Phantom.class, Abbot.class, Wagon.class, Mayor.class, Builder.class, Pig.class, Shepherd.class, Ringmaster.class);
    ...
    if (state.getCapabilities().contains(PrincessCapability.class) && "must".equals(state.getStringRule(Rule.PRINCESS_ACTION))) {
      ReturnMeepleAction princessAction = (ReturnMeepleAction) state.getPlayerActions().getActions().find((a) -> a instanceof ReturnMeepleAction && ((ReturnMeepleAction) a).getSource() == ReturnMeepleSource.PRINCESS).getOrNull();
  ...
  @PhaseMessageHandler public StepResult handleMoveNeutralFigure(GameState state, MoveNeutralFigureMessage msg) {
  ...
  @PhaseMessageHandler public StepResult handleReturnMeeple(GameState state, ReturnMeepleMessage msg) {
    MeeplePointer ptr = msg.getPointer();
    Meeple meeple = state.getDeployedMeeples().find((m) -> ptr.match(m._1)).map((t) -> t._1).getOrElseThrow(() -> new IllegalArgumentException("Pointer doesn\'t match any meeple"));
    Monastic assignAbbotScore = null;
    switch (msg.getSource()) {
      case PRINCESS:
      case ROBBERS_SON:
      ReturnMeepleAction princessAction = (ReturnMeepleAction) state.getPlayerActions().getActions().find((a) -> a instanceof ReturnMeepleAction && ((ReturnMeepleAction) a).getSource() == msg.getSource()).getOrElseThrow(() -> new IllegalArgumentException("Return meeple is not allowed"));
  ...
  @PhaseMessageHandler public StepResult handleScoreAcrobatsMessage(GameState state, ScoreAcrobatsMessage msg) {
    FeaturePointer fp = msg.getPointer();
    state.getPlayerActions().getActions().find((a) -> a instanceof ScoreAcrobatsAction && ((ScoreAcrobatsAction) a).getOptions().contains(fp)).getOrElseThrow(() -> new IllegalArgumentException("Invalid SCORE_ACROBATS"));
  ...
  private StepResult handlePlaceTower(GameState state, PlaceTokenMessage msg) {
    ...
    state = state.appendEvent(new TokenPlacedEvent(PlayEventMeta.createWithActivePlayer(state), TowerToken.TOWER_PIECE, ptr));
    state = clearActions(state);
  ...
  @PhaseMessageHandler public StepResult handlePlaceToken(GameState state, PlaceTokenMessage msg) {
