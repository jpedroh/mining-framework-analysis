 ...
 */
@Immutable @EqualsAndHashCode(of = "base") @ToString(of = "base") @Loggable(value = Loggable.DEBUG) @SuppressWarnings(value = { "PMD.TooManyMethods" }) public final class JdkRequest implements Request {
  /**
     * The wire to use.
  ...
  private static final Wire WIRE = new Wire() {
    @Override public Response send(final Request req, final String home, final String method, final Collection<Map.Entry<String, String>> headers, final InputStream content, final int connect, final int read, final SSLContext sslcontext) throws IOException {
      ...
      try {
        conn = this.createHttpUrlConnection(raw, connect, read, method, headers, sslcontext);
        if (method.equals(Request.POST) || method.equals(Request.PUT) || method.equals(Request.PATCH)) {
        ...
        return new DefaultResponse(req, conn.getResponseCode(), conn.getResponseMessage(), this.headers(conn.getHeaderFields()), this.body(conn));
    ...
    private HttpURLConnection createHttpUrlConnection(final URLConnection raw, final int connect, final int read, final String method, final Collection<Map.Entry<String, String>> headers, final SSLContext sslcontext) throws IOException {
    ...
    /**
         ...
         */
    private void writeFully(final InputStream content, final OutputStream output) throws IOException {
    ...
    /**
         ...
         * @return Headers
         */
    private Array<Map.Entry<String, String>> headers(final Map<String, List<String>> fields) {
