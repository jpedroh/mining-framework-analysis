 ...
 */
@Immutable @EqualsAndHashCode(of = { "base", "code", "phrase", "hdrs", "content" }) @Loggable(value = Loggable.DEBUG) @SuppressWarnings(value = { "PMD.TooManyMethods" }) public final class FakeRequest implements Request {
  /**
  ...
  /**
     ...
     */
  public FakeRequest(final int status, final String reason, final Collection<Map.Entry<String, String>> headers, final byte[] body) {
    this.code = status;
    this.phrase = reason;
    this.hdrs = new Array<>(headers);
    this.content = body.clone();
    this.base = new BaseRequest(new Wire() {
      @Override public Response send(final Request req, final String home, final String method, final Collection<Map.Entry<String, String>> headers, final InputStream text, final int connect, final int read, final SSLContext sslcontext) {
