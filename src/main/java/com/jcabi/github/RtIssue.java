 ...
 */
@Immutable @Loggable(value = Loggable.DEBUG) @EqualsAndHashCode(of = { "request", "owner", "num" }) @SuppressWarnings(value = { "PMD.TooManyMethods" }) final class RtIssue implements Issue {
  /**
     * API entry point.
     */
  ...
  @Override public Iterable<Event> events() {
    return new RtPagination<Event>(this.request.uri().path("/events").back(), new RtValuePagination.Mapping<Event, JsonObject>() {
      @Override public Event map(final JsonObject object) {
        return new RtEvent(RtIssue.this.entry, RtIssue.this.owner, object.getInt("id"));
      }
    });
  }
  ...
  @Override public int compareTo(final Issue issue) {
