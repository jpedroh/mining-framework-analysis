 ...
 * @version $Id$
 */
@Immutable @ToString @EqualsAndHashCode(of = { "origin", "threshold" }) public final class CarefulWire implements Wire {
  /**
     * Original wire.
     */
  ...
  @Override @NotNull(message = "response can\'t be NULL") public Response send(@NotNull(message = "req can\'t be NULL") final Request req, @NotNull(message = "home can\'t be NULL") final String home, @NotNull(message = "method can\'t be NULL") final String method, @NotNull(message = "headers can\'t be NULL") final Collection<Map.Entry<String, String>> headers, @NotNull(message = "content can\'t be NULL") final InputStream content) throws IOException {
    ...
    final int remaining = Integer.parseInt(resp.headers().get("X-RateLimit-Remaining").get(0));
    if (remaining < this.threshold) {
      final long reset = Long.parseLong(resp.headers().get("X-RateLimit-Reset").get(0));
