 ...
 * @since 0.8
 */
@Immutable @Loggable(value = Loggable.DEBUG) @EqualsAndHashCode(of = { "entry", "request", "owner" }) public final class RtContents implements Contents {
  /**
     * API entry point.
     */
  ...
  @Override public Content create(@NotNull(message = "path can\'t be NULL") final String path, @NotNull(message = "message can\'t be NULL") final String message, @NotNull(message = "content can\'t be NULL") final String content) throws IOException {
    final JsonStructure json = Json.createObjectBuilder().add("message", message).add("content", content).build();
    return new RtContent(this.entry, this.owner, this.request.method(Request.PUT).body().set(json).back().fetch().as(RestResponse.class).assertStatus(HttpURLConnection.HTTP_CREATED).as(JsonResponse.class).json().readObject().getJsonObject("content").getString("path"));
  }
