 ...
 */
@Immutable @Loggable(value = Loggable.DEBUG) @ToString @EqualsAndHashCode(of = { "storage", "self" }) final class MkUser implements User {
  /**
     * Storage.
     */
  ...
  /**
     ...
     */
  MkUser(@NotNull(message = "stg can\'t be NULL") final MkStorage stg, @NotNull(message = "login can\'t be NULL") final String login) throws IOException {
    this.storage = stg;
    this.self = login;
    this.storage.apply(new Directives().xpath(String.format("/github/users[not(user[login=\'%s\'])]", login)).add("user").add("login").set(login));
  }
  ...
  @Override @NotNull(message = "orgs is never NULL") public Organizations organizations() {
    try {
  ...
  @Override @NotNull(message = "public keys is never NULL") public PublicKeys keys() {
    try {
