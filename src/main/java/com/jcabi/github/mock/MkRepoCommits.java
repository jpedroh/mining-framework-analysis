 ...
 */
@Immutable @Loggable(value = Loggable.DEBUG) @ToString @EqualsAndHashCode(of = { "storage", "self", "coords" }) final class MkRepoCommits implements RepoCommits {
  /**
     * Storage.
     */
  ...
  /**
     ...
     */
  MkRepoCommits(@NotNull(message = "stg can\'t be NULL") final MkStorage stg, @NotNull(message = "login can\'t be NULL") final String login, @NotNull(message = "repo can\'t be NULL") final Coordinates repo) throws IOException {
    this.storage = stg;
    this.self = login;
    this.coords = repo;
  ...
  @Override @NotNull(message = "Iterable of commits can\'t be NULL") public Iterable<RepoCommit> iterate() {
    return new MkIterable<RepoCommit>(this.storage, String.format("%s/commit", this.xpath()), new MkIterable.Mapping<RepoCommit>() {
      @Override public RepoCommit map(final XML xml) {
        return MkRepoCommits.this.get(xml.xpath("sha/text()").get(0));
      }
    });
  }

  @Override @NotNull(message = "repocommit can\'t be NULL") public RepoCommit get(@NotNull(message = "sha shouldn\'t be NULL") final String sha) {
    return new MkRepoCommit(new MkRepo(this.storage, this.self, this.coords), sha);
  }

  @Override @NotNull(message = "comparison is never NULL") public CommitsComparison compare(@NotNull(message = "base can\'t be NULL") final String base, @NotNull(message = "head can\'t be NULL") final String head) {
    return new MkCommitsComparison(this.storage, this.self, this.coords);
  }

  @Override @NotNull(message = "diff is never NULL") public String diff(@NotNull(message = "base should not be NULL") final String base, @NotNull(message = "head should not be NULL") final String head) throws IOException {
    throw new UnsupportedOperationException("MkRepoCommits#diff()");
  }

  @Override @NotNull(message = "patch is never NULL") public String patch(@NotNull(message = "base shouldn\'t be NULL") final String base, @NotNull(message = "head shouldn\'t be NULL") final String head) throws IOException {
