...
@Immutable public interface MkStorage {
  /**
     ...
     */
  @NotNull(message = "xml is never NULL") XML xml() throws IOException;
  ...
  /**
     ...
     *  storage is locked by another thread.
     */
  void apply(@NotNull(message = "dirs can\'t be NULL") Iterable<Directive> dirs) throws IOException;
  ...
  @Immutable @EqualsAndHashCode(of = "name") @Loggable(value = Loggable.DEBUG) final class InFile implements MkStorage {
    /**
         * File name.
         */
    ...
    /**
         ...
         * @throws IOException If there is any I/O problem
         */
    public InFile(@NotNull(message = "file can\'t be NULL") final File file) throws IOException {
    ...
    @Override @NotNull(message = "XML is never NULL") public XML xml() throws IOException {
      if (this.lock.isLocked() && !this.lock.isHeldByCurrentThread()) {
        throw new ConcurrentModificationException("lock should be taken before method call");
      }
      this.lock.lock();
      try {
    ...
    @Override public void apply(@NotNull(message = "dirs cannot be NULL") final Iterable<Directive> dirs) throws IOException {
      if (this.lock.isLocked() && !this.lock.isHeldByCurrentThread()) {
        throw new ConcurrentModificationException("lock should be taken before method call");
      }
      this.lock.lock();
      try {
        FileUtils.write(new File(this.name), new XMLDocument(new Xembler(dirs).apply(this.xml().node())).toString(), Charsets.UTF_8);
