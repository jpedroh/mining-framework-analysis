...
public class Mustache {
  public static class Compiler {
    ...
    public boolean isFalsey(Object value) {
      return ((emptyStringIsFalse && isEmptyCharSequence(formatter.format(value))) || (zeroIsFalse && (value instanceof Number) && ((Number) value).longValue() == 0));
    }
  ...
  public interface Collector {
    ...
    <K extends java.lang.Object, V extends java.lang.Object> Map<K, V> createFetcherCache();
  }
  ...
  protected static Template.Segment[] trim(Template.Segment[] segs, boolean top) {
    for (int ii = 0, ll = segs.length; ii < ll; ii++) {
      ...
      if (seg instanceof BlockSegment) {
        ...
        if (seg instanceof FauxSegment || seg instanceof IncludedTemplateSegment) {
    ...
    protected void parseChar(char c) {
      switch (state) {
        ...
        if (c == delims.end1) {
          ...
          if (c == delims.start1 && text.length() > 0 && text.charAt(0) != '!') {
            restoreStartTag(text, delims);
            accum.addTextSegment(text);
            tagStartColumn = column;
            if (delims.start2 == NO_CHAR) {
          ...
          if (text.charAt(0) == '=') {
            ...
            if (delims.isStaches() && text.charAt(0) == delims.start1) {
              ...
              if (end3 != '}') {
                ...
                throw new MustacheParseException("Invalid triple-mustache tag: {{" + text + "}}" + got, line);
              }
              text.replace(0, 1, "&");
            }
  ...
  protected static class StringSegment extends Template.Segment {
    ...
    public StringSegment trimTrailBlank() {
      return _trailBlank == -1 ? this : new StringSegment(_text.substring(0, _trailBlank), _leadBlank, -1);
    }
    ...
    private static int blankPos(String text, boolean leading, boolean first) {
      int len = text.length();
      for (int ii = leading ? 0 : len - 1, ll = leading ? len : -1, dd = leading ? 1 : -1; ii != ll; ii += dd) {
  ...
  protected static class VariableSegment extends NamedSegment {
    ...
    @Override public void execute(Template tmpl, Template.Context ctx, Writer out) {
      ...
      if (value == null) {
        String msg = Template.isThisName(_name) ? "Resolved \'.\' to null (which is disallowed), on line " + _line : "No key, method or field with name \'" + _name + "\' on line " + _line;
  ...
  protected static class SectionSegment extends BlockSegment {
    ...
    @Override public void execute(Template tmpl, Template.Context ctx, Writer out) {
      Object value = tmpl.getSectionValue(ctx, _name, _line);
    ...
    @Override public void execute(Template tmpl, Template.Context ctx, Writer out) {
      ...
      if (iter != null) {
        ...
        if (value instanceof Boolean) {
          ...
          if (value instanceof InvertibleLambda) {
            try {
