 ...
 */
public abstract class RetryJob<V extends java.lang.Object> implements Runnable {
  ...
  protected void logFailure(AsyncRetryContext nextRetryContext, long duration) {
    log.trace("Giving up after {} retries, last run took: {}ms, last exception: ", context.getRetryCount(), duration, nextRetryContext.getLastThrowable());
  }
  ...
  protected void logRetry(AsyncRetryContext nextRetryContext, long delay, long duration) {
    ...
    log.trace("Retry {} failed after {}ms, scheduled next retry in {}ms ({})", context.getRetryCount(), duration, delay, nextRunDate, nextRetryContext.getLastThrowable());
  }
