 ...
 */
@CommonsLog public class PrefetchGetRecordsCache implements GetRecordsCache {
...
>>>>>>> /usr/src/app/output/awslabs/amazon-kinesis-client/81c13d2a3566428a51973caa7cd3069f81bf57de/src/main/java/com/amazonaws/services/kinesis/clientlibrary/lib/worker/PrefetchGetRecordsCache.java/right.java
  ...
  public PrefetchGetRecordsCache(final int maxPendingProcessRecordsInput, final int maxByteSize, final int maxRecordsCount, final int maxRecordsPerCall, @NonNull final GetRecordsRetrievalStrategy getRecordsRetrievalStrategy, @NonNull final ExecutorService executorService, @NonNull final IMetricsFactory metricsFactory, long idleMillisBetweenCalls) {
  ...
  @Override public ProcessRecordsInput getNextResult() {
    if (executorService.isShutdown()) {
  ...
  @Override public GetRecordsRetrievalStrategy getGetRecordsRetrievalStrategy() {
      ...
      while (!isShutdown) {
        ...
        if (prefetchCounters.shouldGetNewRecords()) {
          try {
            ...
            ProcessRecordsInput processRecordsInput = new ProcessRecordsInput().withRecords(getRecordsResult.getRecords()).withMillisBehindLatest(getRecordsResult.getMillisBehindLatest()).withCacheEntryTime(lastSuccessfulCall);
