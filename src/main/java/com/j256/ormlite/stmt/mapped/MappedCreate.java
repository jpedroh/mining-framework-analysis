 ...
 * @author graywatson
 */
public class MappedCreate<T extends java.lang.Object, ID extends java.lang.Object> extends BaseMappedStatement<T, ID> {
    ...
    try {
      ...
      if (rowC > 0) {
        if (versionDefaultValue != null) {
          argFieldTypes[versionFieldTypeIndex].assignField(connectionSource, data, versionDefaultValue, false, null);
        }
        if (keyHolder != null) {
  ...
  public static <T extends java.lang.Object, ID extends java.lang.Object> MappedCreate<T, ID> build(Dao<T, ID> dao, TableInfo<T, ID> tableInfo) {
  ...
  private void assignIdValue(T data, Number val, String label, ObjectCache objectCache) throws SQLException {
    ...
    if (logger.isLevelEnabled(Level.DEBUG)) {
      logger.debug("assigned id \'{}\' from {} to \'{}\' in {} object", new Object[] { val, label, idField.getFieldName(), dataClassName });
    }
  }
  ...
  private static class KeyHolder implements GeneratedKeyHolder {
    ...
    @Override public void addKey(Number key) throws SQLException {
      if (this.key == null) {
        this.key = key;
      } else {
