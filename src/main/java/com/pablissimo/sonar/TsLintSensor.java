    ...
    for (File file : fileSystem.files(this.filePredicates.hasLanguage(TypeScriptLanguage.LANGUAGE_EXTENSION))) {
      ...
      if (issues != null) {
        for (TsLintIssue issue : issues) {
          issuable.addIssue(issuable.newIssueBuilder().line(issue.getStartPosition().getLine()).message(issue.getFailure()).ruleKey(RuleKey.of(TsRulesDefinition.REPOSITORY_NAME, issue.getRuleName())).build());
        }
      }
    }
  }
