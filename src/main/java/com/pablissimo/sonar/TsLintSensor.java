...
public class TsLintSensor implements Sensor {
  ...
  @Override public void describe(SensorDescriptor desc) {
    desc.name("Linting sensor for TypeScript files").onlyOnLanguage(TypeScriptLanguage.LANGUAGE_KEY);
  }
  ...
  @Override public void execute(SensorContext ctx) {
    ...
    for (Map.Entry<String, List<TsLintIssue>> kvp : issues.entrySet()) {
      ...
      for (TsLintIssue issue : batchIssues) {
        ...
        NewIssue newIssue = ctx.newIssue().forRule(RuleKey.of(TsRulesDefinition.REPOSITORY_NAME, ruleName));
        NewIssueLocation newIssueLocation = newIssue.newLocation().on(inputFile).message(issue.getFailure()).at(inputFile.selectLine(issue.getStartPosition().getLine() + 1));
