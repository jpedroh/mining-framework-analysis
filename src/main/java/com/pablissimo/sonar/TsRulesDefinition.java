...
public class TsRulesDefinition implements RulesDefinition {
  ...
  public static final TsLintRule TSLINT_UNKNOWN_RULE = new TsLintRule("tslint-issue", Severity.MAJOR, "tslint issues that are not yet known to the plugin", "No description for TsLint rule");
  ...
  public static void loadRules(InputStream stream, List<TsLintRule> rulesCollection) {
    ...
    for (String propKey : properties.stringPropertyNames()) {
      ...
      if (debtRemediationFunction != null) {
        ...
        tsRule = new TsLintRule(ruleId, ruleSeverity, ruleName, ruleDescription, debtRemediationFunctionEnum, debtRemediationScalar, debtRemediationOffset, debtType);
      }
      if (tsRule == null) {
        tsRule = new TsLintRule(ruleId, ruleSeverity, ruleName, ruleDescription);
      }
      rulesCollection.add(tsRule);
    }
    Collections.sort(rulesCollection, new Comparator<TsLintRule>() {
      @Override public int compare(TsLintRule r1, TsLintRule r2) {
  ...
  private void createRule(NewRepository repository, TsLintRule tsRule) {
    NewRule sonarRule = repository.createRule(tsRule.key).setName(tsRule.name).setSeverity(tsRule.severity).setHtmlDescription(tsRule.htmlDescription).setStatus(RuleStatus.READY);
  ...
  public void define(Context context) {
    NewRepository repository = context.createRepository(REPOSITORY_NAME, TypeScriptLanguage.LANGUAGE_KEY).setName("TsLint Analyzer");
