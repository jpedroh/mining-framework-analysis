...
public class ApiParserImpl implements ApiParser {
  ...
  public ApiParserImpl(List<String> baseControllerPackage, List<String> baseModelPackage, String basePath, String servletPath, String apiVersion, List<String> ignorableAnnotations, boolean ignoreUnusedPathVariables) {
  ...
  public Documentation getResourceListing(Map<String, Documentation> documentationMap) {
    ...
    for (String key : documentationMap.keySet()) {
      ...
      String docPath = servletPath + "/doc";
      if (!key.startsWith("/")) {
        docPath = docPath + "/";
      }
  ...
  private void createDocumentationSchemas(Documentation documentation) {
    for (String modelPackage : modelPackages) {
      Reflections reflections = new Reflections(new ConfigurationBuilder().filterInputsBy(new FilterBuilder().include(FilterBuilder.prefix(modelPackage))).setUrls(ClasspathHelper.forPackage(modelPackage)).setScanners(new SubTypesScanner(false), new ResourcesScanner()));
