 ...
 * @since 4.0
 */
@ChannelHandler.Sharable public class StatefulRedisClusterConnectionImpl<K extends java.lang.Object, V extends java.lang.Object> extends RedisChannelHandler<K, V> implements StatefulRedisClusterConnection<K, V> {
  ...
  @Override public RedisAdvancedClusterCommands<K, V> sync() {
    if (sync == null) {
      InvocationHandler h = syncInvocationHandler();
      sync = (RedisAdvancedClusterCommands) Proxy.newProxyInstance(AbstractRedisClient.class.getClassLoader(), new Class<?>[] { RedisAdvancedClusterConnection.class, RedisAdvancedClusterCommands.class }, h);
    }
    return sync;
  }
  ...
  @Override public RedisAdvancedClusterAsyncCommands<K, V> async() {
  ...
  @Override public RedisAdvancedClusterReactiveCommands<K, V> reactive() {
  ...
  @Override public StatefulRedisConnection<K, V> getConnection(String nodeId) {
    RedisURI redisURI = lookup(nodeId);
    if (redisURI == null) {
      throw new RedisException("NodeId " + nodeId + " does not belong to the cluster");
    }
    StatefulRedisConnection<K, V> connection = getClusterDistributionChannelWriter().getClusterConnectionProvider().getConnection(ClusterConnectionProvider.Intent.WRITE, nodeId);
    return connection;
  }

  @Override public StatefulRedisConnection<K, V> getConnection(String host, int port) {
    StatefulRedisConnection<K, V> connection = getClusterDistributionChannelWriter().getClusterConnectionProvider().getConnection(ClusterConnectionProvider.Intent.WRITE, host, port);
    return connection;
  }
  ...
  @Override public <T extends java.lang.Object, C extends RedisCommand<K, V, T>> C dispatch(C cmd) {
  ...
  private <T extends java.lang.Object> RedisCommand<K, V, T> attachOnComplete(RedisCommand<K, V, T> command, Consumer<T> consumer) {
  ...
  private static class ClusterFutureSyncInvocationHandler<K extends java.lang.Object, V extends java.lang.Object> extends AbstractInvocationHandler {
