...
public class SslConnectionBuilder extends ConnectionBuilder {
  ...
  @Override protected List<ChannelHandler> buildHandlers() {
  ...
  @Override public RedisChannelInitializer build() {
    final List<ChannelHandler> channelHandlers = buildHandlers();
    return new SslChannelInitializer(clientOptions().isPingBeforeActivateConnection(), channelHandlers, redisURI, clientResources().eventBus());
  }
