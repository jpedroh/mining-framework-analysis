 ...
 * @since 3.0
 */
@SuppressWarnings(value = { "serial" }) public class RoleParser {
  ...
  protected static final Map<String, RedisSlaveInstance.State> SLAVE_STATE_MAPPING = ImmutableMap.of("connect", RedisSlaveInstance.State.CONNECT, "connected", RedisSlaveInstance.State.CONNECTED, "connecting", RedisSlaveInstance.State.CONNECTING, "sync", RedisSlaveInstance.State.SYNC);
  ...
  public static RedisInstance parse(List<?> roleOutput) {
    ...
    checkArgument(roleOutput.get(0) instanceof String && ROLE_MAPPING.containsKey(roleOutput.get(0)), "First role element must be a string (any of " + ROLE_MAPPING.keySet() + ")");
