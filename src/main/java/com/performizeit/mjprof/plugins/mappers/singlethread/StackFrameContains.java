...
@Plugin(name = "frame", params = { @Param }, description = "Eliminates stack frames from all stacks which do not contain string.") public class StackFrameContains extends SingleThreadMapperBase {
  ...
  @Override public ThreadInfo map(ThreadInfo stck) {
    ...
    p.filter(new ProfileNodeFilter() {
      @Override public boolean accept(SFNode node, int level, Object context) {
