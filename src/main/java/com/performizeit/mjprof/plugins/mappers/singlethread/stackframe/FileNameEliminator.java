...
@SuppressWarnings(value = { "unused" }) @Plugin(name = "-fn", params = {  }, description = "Eliminates file name and line from stack frames") public class FileNameEliminator extends SingleThreadMapperBase {
  public FileNameEliminator() {
  }

  @Override public ThreadInfo map(ThreadInfo stck) {
    HashMap<String, Object> mtd = stck.cloneMetaData();
    Profile jss = (Profile) stck.getVal("stack");
    jss.visit(new ProfileVisitor() {
      @Override public void visit(SFNode sf, int level) {
        if (sf.getStackFrame() == null) {
          return;
        }
