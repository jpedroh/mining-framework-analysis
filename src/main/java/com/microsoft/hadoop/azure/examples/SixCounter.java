 ...
 */
public class SixCounter extends Configured implements Tool {
  ...
  private static void populate(CloudTable t) throws Exception {
    ...
    for (int partition = 0; partition < NUM_PARTITIONS; partition++) {
      ...
      for (int row = 0; row < NUM_ROWS_PER_PARTITION; row++) {
        batch.add(TableOperation.insert(newEntity("p" + partition, "r" + row, rand.nextInt(100))));
        rowsInBatch++;
  ...
  public static class SixFinderMapper extends Mapper<Text, WritableEntity, IntWritable, LongWritable> {
    private static final IntWritable SIX = new IntWritable(6);

    private static final LongWritable ONE = new LongWritable(1);

    @Override protected void map(Text key, WritableEntity value, Context context) throws IOException, InterruptedException {
  ...
  public static class TallyReducer extends Reducer<IntWritable, LongWritable, IntWritable, LongWritable> {
    private final LongWritable sum = new LongWritable();

    @Override protected void reduce(IntWritable key, Iterable<LongWritable> values, Context context) throws IOException, InterruptedException {
      long currentSum = 0;
  ...
  private Job configureJob(CloudTable t, Path outputPath) throws IOException {
    ...
    AzureTableConfiguration.configureInputTable(job.getConfiguration(), t.getName(), accountUri, accountKey);
  ...
  private CloudTable createTableReference(String[] args) throws Exception {
    ...
    accountUri = new URI(String.format("http://%s.table.core.windows.net/", accountName));
