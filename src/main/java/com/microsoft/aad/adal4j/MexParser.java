...
class MexParser {
  ...
  static BindingPolicy getWsTrustEndpointFromMexEndpoint(String metadataEndpoint, Proxy proxy, SSLSocketFactory sslSocketFactory) throws Exception {
    String mexResponse = HttpHelper.executeHttpGet(log, metadataEndpoint, proxy, sslSocketFactory);
  ...
  private static String checkPolicy(XPath xPath, Node node) throws XPathExpressionException {
    ...
    if (transportBindingNodes.getLength() > 0 && id != null) {
      ...
      log.debug("potential policy did not match required transport binding: " + id.getNodeValue());
    }
    return policyId;
  }
}