...
class AuthenticationAuthority {
  ...
  private final static String[] TRUSTED_HOST_LIST = { "login.windows.net", "login.chinacloudapi.cn", "login-us.microsoftonline.com", "login.microsoftonline.de", "login.microsoftonline.com", "login.microsoftonline.us" };

  private final 
<<<<<<< /usr/src/app/output/msopentech/azure-activedirectory-library-for-java/c2385c17f056dc66330d9568d2cae775c813aa18/src/main/java/com/microsoft/aad/adal4j/AuthenticationAuthority.java/left.java
  String
=======
  Logger
>>>>>>> /usr/src/app/output/msopentech/azure-activedirectory-library-for-java/c2385c17f056dc66330d9568d2cae775c813aa18/src/main/java/com/microsoft/aad/adal4j/AuthenticationAuthority.java/right.java
   
<<<<<<< /usr/src/app/output/msopentech/azure-activedirectory-library-for-java/c2385c17f056dc66330d9568d2cae775c813aa18/src/main/java/com/microsoft/aad/adal4j/AuthenticationAuthority.java/left.java
  devicecodeEndpointFormat = "https://%s/{tenant}" + DEVICE_CODE_ENDPOINT
=======
  piiLog = LoggerFactory.getLogger(LogHelper.PII_LOGGER_PREFIX + this.getClass())
>>>>>>> /usr/src/app/output/msopentech/azure-activedirectory-library-for-java/c2385c17f056dc66330d9568d2cae775c813aa18/src/main/java/com/microsoft/aad/adal4j/AuthenticationAuthority.java/right.java
  ;
  ...
  private final String instanceDiscoveryEndpointFormat = "https://%s/" + DISCOVERY_ENDPOINT;
  ...
  AuthenticationAuthority(final URL authorityUrl, final boolean validateAuthority) {
  ...
  void doInstanceDiscovery(final Map<String, String> headers, final Proxy proxy, final SSLSocketFactory sslSocketFactory) throws Exception {
    if (!instanceDiscoveryCompleted) {
      if (!doStaticInstanceDiscovery()) {
        if (validateAuthority && !doDynamicInstanceDiscovery(headers, proxy, sslSocketFactory)) {
          throw new AuthenticationException(AuthenticationErrorMessage.AUTHORITY_NOT_IN_VALID_LIST);
        }
      }
      String msg = LogHelper.createMessage("Instance discovery was successful", headers.get(ClientDataHttpHeaders.CORRELATION_ID_HEADER_NAME));
      log.info(msg);
      piiLog.info(msg);
  ...
  boolean doDynamicInstanceDiscovery(final Map<String, String> headers, final Proxy proxy, final SSLSocketFactory sslSocketFactory) throws Exception {
    final String json = HttpHelper.executeHttpGet(log, piiLog, instanceDiscoveryEndpoint, headers, proxy, sslSocketFactory);
  ...
  void setupAuthorityProperties() {
    ...
    final String path = this.authorityUrl.getPath().substring(1).toLowerCase();
    final String tenant = path.substring(0, path.indexOf("/")).toLowerCase();
    this.host = host;
  ...
  void validateAuthorityUrl() {
    if (authorityType != AuthorityType.AAD && validateAuthority) {
      throw new IllegalArgumentException(AuthenticationErrorMessage.UNSUPPORTED_AUTHORITY_VALIDATION);
    }
    if (!this.authorityUrl.getProtocol().equalsIgnoreCase("https")) {
      throw new IllegalArgumentException(AuthenticationErrorMessage.AUTHORITY_URI_INSECURE);
    }
  ...
  void createInstanceDiscoveryEndpoint(final String tenant) {
    this.instanceDiscoveryEndpoint += "?api-version=1.0&authorization_endpoint=" + AUTHORIZE_ENDPOINT_TEMPLATE;
    this.instanceDiscoveryEndpoint = this.instanceDiscoveryEndpoint.replace("{host}", host);
