...
class AdalTokenRequest {
  ...
  AdalTokenRequest(final URL uri, final ClientAuthentication clientAuth, final AdalGrant authzGrant, final Map<String, String> headerMap, final Proxy proxy, final SSLSocketFactory sslSocketFactory) {
  ...
  AuthenticationResult executeOAuthRequestAndProcessResponse() throws ParseException, AuthenticationException, SerializeException, IOException, java.text.ParseException {
    ...
    if (httpResponse.getStatusCode() == HTTPResponse.SC_OK) {
      ...
      if (errorResponse != null && errorResponse.isDeviceCodeError()) {
        ...
        DeviceCodeException deviceCodeException = new DeviceCodeException(errorObject.getCode(), errorObject.getDescription());
        throw new AuthenticationException("Device Code Error", deviceCodeException);
      }
      final AdalAccessTokenResponse response = AdalAccessTokenResponse.parseHttpResponse(httpResponse);
      ...
      if (HTTPResponse.SC_BAD_REQUEST == errorResponse.getErrorObject().getHTTPStatusCode() && "interaction_required".equals(errorResponse.getErrorObject().getCode())) {
        throw new AdalClaimsChallengeException(errorResponse.toJSONObject().toJSONString(), getClaims(httpResponse.getContent()));
      } else {
  ...
  AdalOAuthRequest toOAuthRequest() throws SerializeException {
    ...
    final AdalOAuthRequest httpRequest = new AdalOAuthRequest(HTTPRequest.Method.POST, this.uri, headerMap, this.proxy, this.sslSocketFactory);
