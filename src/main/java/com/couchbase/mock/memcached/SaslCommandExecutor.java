...
public class SaslCommandExecutor implements CommandExecutor {
  ...
  @Override public BinaryResponse execute(BinaryCommand cmd, MemcachedServer server, MemcachedConnection client) throws ProtocolException {
  ...
  private void createSaslServer(BinaryCommand cmd, MemcachedServer server, MemcachedConnection client) throws ProtocolException {
    try {
      ...
      saslServer = Sasl.createSaslServer(mechanism, PROTOCOL_COUCHBASE, serverName, null, new SaslCallbackHandler(username, password));
