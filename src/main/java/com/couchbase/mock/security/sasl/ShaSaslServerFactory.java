...
public class ShaSaslServerFactory implements SaslServerFactory {
  ...
  @Override public SaslServer createSaslServer(String mechanism, String protocol, String serverName, Map<String, ?> props, CallbackHandler cbh) throws SaslException {
  ...
  private int getDigestSize(String mechanism) {
    ...
    if (mechanism.equals(SCRAM_SHA512)) {
      ...
      if (mechanism.equals(SCRAM_SHA256)) {
        sha = 256;
      } else {
        if (mechanism.equals(SCRAM_SHA1)) {
          sha = 1;
        }
      }
    }
  ...
  @Override public String[] getMechanismNames(Map<String, ?> props) {
