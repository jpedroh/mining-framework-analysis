...
public abstract class BaseESReducer implements Reducer<Text, Text, NullWritable, Text> {
  ...
  public static enum JOB_COUNTER {
    TIME_SPENT_INDEXING_MS,
    TIME_SPENT_FLUSHING_MS,
    TIME_SPENT_MERGING_MS,
    TIME_SPENT_SNAPSHOTTING_MS,
    TIME_SPENT_TRANSPORTING_SNAPSHOT_MS,
    INDEXING_DOC_FAIL,
    INDEX_DOC_CREATED,
    INDEX_DOC_NOT_CREATED
  }
  ...
  private void init(String index) {
    ...
    ESEmbededContainer.Builder builder = new ESEmbededContainer.Builder().withNodeName("embededESTempLoaderNode" + partition).withWorkingDir(esWorkingDir).withClusterName("bulkLoadPartition:" + partition).withSnapshotWorkingLocation(snapshotWorkingLocation).withSnapshotRepoName(snapshotRepoName);
    ...
    esEmbededContainer.getNode().client().admin().indices().prepareCreate(index).setSettings(
<<<<<<< /usr/src/app/output/mypurecloud/elasticsearch-lambda/9ad6ef3cc4463ad5e7c71ae057264ad3ba7b2f86/src/main/java/com/inin/analytics/elasticsearch/BaseESReducer.java/left.java
    builder().put("index.number_of_replicas", 0)
=======
    settingsBuilder().put("index.number_of_replicas", 0).put(IndexMetaData.SETTING_NUMBER_OF_SHARDS, shardConfig.getShardsForIndex(index))
>>>>>>> /usr/src/app/output/mypurecloud/elasticsearch-lambda/9ad6ef3cc4463ad5e7c71ae057264ad3ba7b2f86/src/main/java/com/inin/analytics/elasticsearch/BaseESReducer.java/right.java
    ).get();
  }
  ...
  @Override public void reduce(Text docMetaData, Iterator<Text> documentPayloads, OutputCollector<NullWritable, Text> output, final Reporter reporter) throws IOException {
