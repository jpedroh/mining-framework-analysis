...
public class SQSMessageConsumerPrefetch implements Runnable, PrefetchManager {
  ...
  @Override public SQSMessageConsumer getMessageConsumer() {
  ...
  protected List<Message> getMessages(int batchSize, int waitTimeSeconds) throws JMSException {
    assert batchSize > 0;
    Builder receiveMessageRequestBuilder = ReceiveMessageRequest.builder().queueUrl(queueUrl).maxNumberOfMessages(batchSize).attributeNamesWithStrings(ALL).messageAttributeNames(ALL).waitTimeSeconds(waitTimeSeconds);
  ...
  protected javax.jms.Message convertToJMSMessage(Message message) throws JMSException {
    MessageAttributeValue messageTypeAttribute = message.messageAttributes().get(SQSMessage.JMS_SQS_MESSAGE_TYPE);
    ...
    if (messageTypeAttribute == null) {
      ...
      if (SQSMessage.BYTE_MESSAGE_TYPE.equals(messageType)) {
        try {
          ...
          LOG.warn("MessageReceiptHandle - " + message.receiptHandle() + "cannot be serialized to BytesMessage", e);
          throw e;
        }
      } else {
        if (SQSMessage.OBJECT_MESSAGE_TYPE.equals(messageType)) {
          jmsMessage = new SQSObjectMessage(acknowledger, queueUrl, message);
        } else {
          if (SQSMessage.TEXT_MESSAGE_TYPE.equals(messageType)) {
            jmsMessage = new SQSTextMessage(acknowledger, queueUrl, message);
          } else {
    ...
    MessageAttributeValue replyToQueueNameAttribute = message.messageAttributes().get(SQSMessage.JMS_SQS_REPLY_TO_QUEUE_NAME);
    MessageAttributeValue replyToQueueUrlAttribute = message.messageAttributes().get(SQSMessage.JMS_SQS_REPLY_TO_QUEUE_URL);
    ...
    MessageAttributeValue correlationIdAttribute = message.messageAttributes().get(SQSMessage.JMS_SQS_CORRELATION_ID);
  ...
  private long getJMSTimestamp(Message message) {
    Map<String, String> systemAttributes = message.
...
>>>>>>> /usr/src/app/output/awslabs/amazon-sqs-java-messaging-lib/016338972c443427e3b9f46c5e00f254a10c7095/src/main/java/com/amazon/sqs/javamessaging/SQSMessageConsumerPrefetch.java/right.java
  ...
  @Override public void messageDispatched() {
    synchronized (stateLock) {
      messagesPrefetched--;
      messagesRequested--;
  ...
  @Override public void messageListenerReady() {
    synchronized (stateLock) {
    ...
    synchronized (stateLock) {
      ...
      try {
        if (!messageQueue.isEmpty()) {
          ...
          long waitTime = 0;
          while (messageQueue.isEmpty() && !isClosed() && (timeout == 0 || (waitTime = getWaitTime(timeout, startTime)) > 0)) {
            try {
