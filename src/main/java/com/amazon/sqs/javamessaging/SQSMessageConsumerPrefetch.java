...
public class SQSMessageConsumerPrefetch implements Runnable, PrefetchManager {
  ...
  protected List<Message> getMessages(int batchSize, int waitTimeSeconds) throws JMSException {
    assert batchSize > 0;
    ReceiveMessageRequest receiveMessageRequest = new ReceiveMessageRequest(queueUrl).withMaxNumberOfMessages(batchSize).withAttributeNames(ALL).withMessageAttributeNames(ALL).withWaitTimeSeconds(waitTimeSeconds);
  ...
  protected javax.jms.Message convertToJMSMessage(Message message) throws JMSException {
    MessageAttributeValue messageTypeAttribute = message.getMessageAttributes().get(SQSMessage.JMS_SQS_MESSAGE_TYPE);
    ...
    MessageAttributeValue replyToQueueNameAttribute = message.getMessageAttributes().get(SQSMessage.JMS_SQS_REPLY_TO_QUEUE_NAME);
    MessageAttributeValue replyToQueueUrlAttribute = message.getMessageAttributes().get(SQSMessage.JMS_SQS_REPLY_TO_QUEUE_URL);
    ...
    MessageAttributeValue correlationIdAttribute = message.getMessageAttributes().get(SQSMessage.JMS_SQS_CORRELATION_ID);
