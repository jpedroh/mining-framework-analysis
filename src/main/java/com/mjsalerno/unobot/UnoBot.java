...
public class UnoBot extends ListenerAdapter {
  ...
  @Override public void onMessage(MessageEvent event) throws Exception {
    ...
    if (tokens[0].equalsIgnoreCase(this.token + "info")) {
      ...
      if (tokens[0].equalsIgnoreCase(this.token + "unohelp")) {
        ...
        if (tokens[0].equalsIgnoreCase(this.token + "joinc") && isBotOp(sender)) {
          bot.sendIRC().joinChannel(tokens[1]);
        } else {
          if (tokens[0].equalsIgnoreCase(this.token + "update") && this.isBotOp(sender) && this.updateScript != null) {
            try {
            ...
            if (tokens[0].equalsIgnoreCase(this.token + "part") && isBotOp(sender)) {
              Channel chan = bot.getUserChannelDao().getChannel(tokens[1]);
              chan.send().part();
            } else {
              if (tokens[0].equalsIgnoreCase(this.token + "quit") && isBotOp(sender)) {
                bot.sendIRC().quitServer();
                System.exit(0);
              } else {
                if (tokens[0].equalsIgnoreCase(this.token + "resetsb") && isBotOp(sender)) {
                  try {
                    resetScoreBoard();
    ...
    if (tokens[0].equalsIgnoreCase(this.token + "messages") && messagesEnabled) {
      ...
      if (tokens[0].equalsIgnoreCase(this.token + "tell") && messagesEnabled) {
        ...
        try {
          ...
          bot.sendIRC().message(channel, ERR_SAVE_MSG + "data to a file since there was a file not found exception");
        } catch (IOException ex) {
          bot.sendIRC().message(channel, ERR_SAVE_MSG + "data to a file");
        }
      } else {
        if (tokens[0].equalsIgnoreCase(this.token + "score")) {
          if (!this.sb.isEmpty()) {
            try {
              printScore(channel);
            } catch (FileNotFoundException ex) {
          ...
          if (tokens[0].equalsIgnoreCase(this.token + "ai") && !gameUp) {
            if (!botAI) {
              Configuration configuration2;
              Configuration.Builder configBuilder = new Configuration.Builder().setName(unoAINick).setLogin(unoAINick).setRealName(bot.getNick()).setAutoReconnect(true).setAutoNickChange(true).setCapEnabled(true).setMessageDelay(4000).addServers(bot.getConfiguration().getServers()).addAutoJoinChannel(channel).setSocketFactory(usingSSL ? new UtilSSLSocketFactory().trustAllCertificates() : SocketFactory.getDefault()).setSocketTimeout(130 * 1000).setVersion("mIRC v7.32 Khaled Mardam-Bey");
            ...
            if (tokens[0].equalsIgnoreCase(this.token + "uno")) {
              if (gameUp) {
                ...
                while (gameUp) {
                  ...
                  if (tokens[0].equalsIgnoreCase(this.token + "join") && gameUp) {
                    ...
                    if ((tokens[0].equalsIgnoreCase(this.token + "endgame") && gameUp) && (isBotOp(sender) || sender.equals(gameStarter))) {
                      if (delt) {
                        stopTimer();
                        deck.clear();
                        delt = false;
                      ...
                      if (tokens[0].equalsIgnoreCase(this.token + "leave")) {
                        ...
                        if (tokens[0].equalsIgnoreCase(this.token + "unocount") && delt) {
                          bot.sendIRC().message(channel, players.countCards());
                        } else {
                          if (tokens[0].equalsIgnoreCase(this.token + "players") && gameUp) {
                            printPlayers(channel);
                          } else {
                            if ((tokens[0].equalsIgnoreCase(this.token + "deal")) && !delt && gameUp && ((sender.equals(gameStarter)) || (isBotOp(sender)))) {
                              ...
                              if ((tokens[0].equalsIgnoreCase(this.token + "what")) && (delt)) {
                                bot.sendIRC().message(channel, TOP_CARD + deck.topCard().toIRCString());
                                bot.sendIRC().message(channel, players.at().getName() + UR_TURN);
                              } else {
                                if ((tokens[0].equalsIgnoreCase(this.token + "wait")) && delt && (sender.equals(players.at().getName()))) {
                                  stopTimer();
                                  bot.sendIRC().message(channel, players.at().getName() + " stopped their turn timer.");
                                } else {
                                  if ((tokens[0].equalsIgnoreCase(this.token + "draw")) && delt && (sender.equals(players.at().getName()))) {
                                    if (!drew) {
                                      if (attack) {
                                      ...
                                      bot.sendIRC().message(channel, "Sorry " + sender + " but you already " + "drew a card. If you still have no card to play then " + "pass by typing !pass");
                                    }
                                  } else {
                                    if ((tokens[0].equalsIgnoreCase(this.token + "pass")) && delt && (sender.equals(players.at().getName()))) {
                                      if (drew) {
                                        stopTimer();
                                      ...
                                      if ((tokens[0].equalsIgnoreCase(this.token + "showcards") || tokens[0].equalsIgnoreCase(this.token + "hand")) && delt) {
                                        bot.sendIRC().notice(sender, showCards(players.get(sender)));
                                      } else {
                                        if (tokens[0].equalsIgnoreCase(this.token + "rank")) {
                                          for (int i = 0; i < this.sb.size(); i++) {
                                            this.bot.sendIRC().message(channel, sb.playerRankToString(i));
                                          }
                                        } else {
                                          if ((tokens[0].equalsIgnoreCase(this.token + "play") || tokens[0].equalsIgnoreCase(this.token + "p")) && delt && gameUp && (sender.equals(players.at().getName()))) {
                                            Card card = null;
                                            if (tokens.length >= 3) {
                                                ...
                                                if (deck.isPlayable(card)) {
                                                  ...
                                                  if ((card.color.equals(Card.Color.WILD))) {
                                                    ...
                                                    if (tokens[2].equalsIgnoreCase("R") || tokens[2].equalsIgnoreCase("RED")) {
                                                      ...
                                                      if (tokens[2].equalsIgnoreCase("B") || tokens[2].equalsIgnoreCase("BLUE")) {
                                                        coler += "BLUE";
                                                      } else {
                                                        if (tokens[2].equalsIgnoreCase("G") || tokens[2].equalsIgnoreCase("GREEN")) {
                                                          coler += "GREEN";
                                                        } else {
                                                          if (tokens[2].equalsIgnoreCase("Y") || tokens[2].equalsIgnoreCase("YELLOW")) {
                                                            coler += "YELLOW";
                                                          } else {
                                                    ...
                                                    if (card.face.equals(Card.Face.S)) {
                                                      ...
                                                      if (card.face.equals(Card.Face.R)) {
                                                        ...
                                                        if (card.face.equals(Card.Face.D2)) {
                                                          player.play(card, deck);
  ...
  @Override public void onJoin(JoinEvent event) throws Exception {
    ...
    if (gameUp && channel.equals(gameChannel)) {
      ...
      if ((bot.getNick().equals(sender)) && this.currChannel == null) {
  ...
  @Override public void onUserList(UserListEvent event) throws Exception {
    String channel = event.getChannel().getName();
    if (messagesEnabled) {
  ...
  @Override public void onNickChange(NickChangeEvent event) throws Exception {
  ...
  @Override public void onPrivateMessage(PrivateMessageEvent event) throws Exception {
    String sender = event.getUser().getNick();
