...
public class TestManager {
  ...
  public void setPostTestPauseInSeconds(String postTestPauseInSeconds) {
    ...
    try {
      ...
      LOGGER.error("Error parsing value {} of <postTestPauseInSeconds>, will default to 0", postTestPauseInSeconds, ex);
    }
    if (null == testPause) {
      testPause = 0l;
    }
  ...
  public List<String> executeTests() throws MojoExecutionException {
    ...
    for (String file : tests) {
      if (generateReports) {
        thisTestArgs.setReportsDirectory(reportDirectory + "/" + FilenameUtils.getBaseName(file) + "_" + sdf.print(new DateTime()));
      }
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "ResultOfMethodCallIgnored" }) private String executeSingleTest(File test, JMeterArgumentsArray testArgs) throws MojoExecutionException {
    LOGGER.info(" ");
