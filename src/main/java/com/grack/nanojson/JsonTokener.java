...
final class JsonTokener {
  ...
  JsonTokener(InputStream stm) throws JsonParserException {
    final InputStream buffered = (stm instanceof BufferedInputStream || stm instanceof ByteArrayInputStream) ? stm : new BufferedInputStream(stm);
    buffered.mark(4);
    try {
      Charset charset;
              ...
              if (digit >= '0' && digit <= '9') {
                ...
                if (digit >= 'A' && digit <= 'F') {
                  ...
                  if (digit >= 'a' && digit <= 'f') {
                    ...
                    throw createParseException(null, "Expected unicode hex escape character: " + (char) digit + " (" + digit + ")", false);
                  }
                }
              }
            }
  ...
  @SuppressWarnings(value = { "fallthrough" }) private void consumeTokenStringUtf8Char(char c) throws JsonParserException {
    ensureBuffer(5);
    switch (c & 0xf0) {
      case 0x80:
      case 0x90:
      ...
      utf8adjust += 2;
      if ((c >= '\ud800' && c <= '\udbff') || (c >= '\udc00' && c <= '\udfff')) {
        throw createParseException(null, "Illegal UTF-8 codepoint: 0x" + Integer.toHexString(c), false);
      }
      reusableBuffer.append(c);
      break;
      case 0xf0:
      ...
      switch ((c & 0xc) >> 2) {
        case 0:
        case 1:
        reusableBuffer.appendCodePoint((c & 7) << 18 | (buffer[index++] & 0x3f) << 12 | (buffer[index++] & 0x3f) << 6 | (buffer[index++] & 0x3f));
        utf8adjust += 3;
        break;
        case 2:
        int codepoint = (c & 3) << 24 | (buffer[index++] & 0x3f) << 18 | (buffer[index++] & 0x3f) << 12 | (buffer[index++] & 0x3f) << 6 | (buffer[index++] & 0x3f);
        throw createParseException(null, "Unable to represent codepoint 0x" + Integer.toHexString(codepoint) + " in a Java string", false);
        case 3:
        codepoint = (c & 1) << 30 | (buffer[index++] & 0x3f) << 24 | (buffer[index++] & 0x3f) << 18 | (buffer[index++] & 0x3f) << 12 | (buffer[index++] & 0x3f) << 6 | (buffer[index++] & 0x3f);
        throw createParseException(null, "Unable to represent codepoint 0x" + Integer.toHexString(codepoint) + " in a Java string", false);
        default:
        assert false : "Impossible";
      }
  ...
  JsonParserException createParseException(Exception e, String message, boolean tokenPos) {
    if (tokenPos) {
      return new JsonParserException(e, message + " on line " + linePos + ", char " + tokenCharPos, linePos, tokenCharPos, tokenCharOffset);
    } else {
      int charPos = Math.max(1, index + charOffset - rowPos - utf8adjust);
      return new JsonParserException(e, message + " on line " + linePos + ", char " + charPos, linePos, charPos, index + charOffset);
    }
  }
}