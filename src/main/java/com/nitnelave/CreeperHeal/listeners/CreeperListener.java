...
public class CreeperListener implements Listener {
  /**
     ...
     */
  @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onEntityExplode(EntityExplodeEvent event) {
  ...
  /**
     ...
     *            The EntityExplode event.
     */
  @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onBlockExplode(BlockExplodeEvent event) {
  ...
  /**
     ...
     */
  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true) public void onHangingBreak(HangingBreakEvent event) {
  ...
  /**
     ...
     */
  @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true) public void onEntityChangeBlock(EntityChangeBlockEvent event) {
    CreeperLog.debug("Entity change block event");
    if (event.getEntityType() == EntityType.SILVERFISH && CreeperTag.INFESTED_BLOCKS.isTagged(event.getTo()) && CreeperConfig.getBool(CfgVal.REPLACE_SILVERFISH_BLOCKS)) {
  ...
  /**
     ...
     */
  @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true) public void onEntityDamagedByEntity(EntityDamageByEntityEvent event) {
    CreeperLog.debug("Entity damaged by entity event");
    WorldConfig world = CreeperConfig.getWorld(event.getEntity().getWorld());
    if (event.getEntityType() == EntityType.ARMOR_STAND && event.getCause() == EntityDamageEvent.DamageCause.ENTITY_EXPLOSION && world.shouldReplace(event.getDamager())) {
  ...
  /**
     ...
     */
  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true) public void onEntityBreakDoor(EntityBreakDoorEvent event) {
    WorldConfig world = CreeperConfig.getWorld(event.getBlock().getWorld());
