...
public class CreeperExplosion {
  ...
  public void addBlocks(List<Block> blocks, Location newLoc) {
    timer = new ReplacementTimer(new Date(new Date().getTime() + 1000 * CreeperConfig.getInt(CfgVal.WAIT_BEFORE_HEAL)), world.isRepairTimed());
    loc = new Location(loc.getWorld(), (locWeight * loc.getX() + newLoc.getX()) / (locWeight + 1), (locWeight * loc.getY() + newLoc.getY()) / (locWeight + 1), (locWeight * loc.getZ() + newLoc.getZ()) / (locWeight + 1));
    locWeight++;
    checked.clear();
    recordBlocks(blocks);
...
>>>>>>> /usr/src/app/output/nitnelave/creeperheal/a8f5cf9a61b44a1c73353cfe0ba1bc9e341fd402/src/main/java/com/nitnelave/CreeperHeal/block/CreeperExplosion.java/right.java
  ...
  private boolean isObsidianLike(Material m, boolean table) {
    return m == Material.OBSIDIAN || (table && (m == Material.ENCHANTING_TABLE || m == Material.ENDER_CHEST));
  }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  public void recordBlock(Block block) {
    ...
    if ((CreeperConfig.getBool(CfgVal.PREVENT_CHAIN_REACTION) && creeperBlock.getType().equals(Material.TNT)) || world.isProtected(creeperBlock.getBlock())) {
...
<<<<<<< /usr/src/app/output/nitnelave/creeperheal/a8f5cf9a61b44a1c73353cfe0ba1bc9e341fd402/src/main/java/com/nitnelave/CreeperHeal/block/CreeperExplosion.java/left.java
  ...
  public void record(Replaceable replaceable) {
    ...
    if ((CreeperConfig.getBool(CfgVal.PREVENT_CHAIN_REACTION) && replaceable.getType().equals(Material.TNT)) || world.isProtected(replaceable.getBlock())) {
      ToReplaceList.addToReplace(creeperBlock);
      replaceable.remove();
      return;
    }
