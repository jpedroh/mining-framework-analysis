...
public class LZFInputStream extends InputStream {
  ...
  public LZFInputStream(final ChunkDecoder decoder, final InputStream in) throws IOException {
  ...
  public LZFInputStream(final ChunkDecoder decoder, final InputStream in, boolean fullReads) throws IOException {
    super();
    _decoder = decoder;
  ...
  @Override public int read(final byte[] buffer, int offset, int length) throws IOException {
    if (length < 1) {
      return 0;
    }
    if (!readyBuffer()) {
