...
public class RecurlyClient {
  ...
  private <T extends java.lang.Object> T callRecurly(final AsyncHttpClient.BoundRequestBuilder builder, @Nullable final Class<T> clazz) throws IOException, ExecutionException, InterruptedException {
    final Response response = builder.addHeader("Authorization", "Basic " + key).addHeader("Accept", "application/xml").addHeader("Content-Type", "application/xml; charset=utf-8").addHeader(HttpHeaders.USER_AGENT, userAgent).setBodyEncoding("UTF-8").execute().get();
