...
public class RecurlyClient {
  ...
  public Redemption getCouponRedemptionByInvoice(final Integer invoiceNumber) {
    return doGET(Invoices.INVOICES_RESOURCE + "/" + invoiceNumber + Redemption.REDEMPTION_RESOURCE, Redemption.class);
  }
  ...
  public Redemptions getCouponRedemptionsByInvoice(final Integer invoiceNumber) {
    return doGET(Invoices.INVOICES_RESOURCE + "/" + invoiceNumber + Redemption.REDEMPTION_RESOURCE, Redemptions.class);
  }
  ...
  private byte[] callRecurlySafeGetPdf(String url) {
    ...
    try {
      response = builderCommon(client.prepareGet(url)).addHeader("Accept", "application/pdf").addHeader("Content-Type", "application/pdf").execute().get();
  ...
  private <T extends java.lang.Object> T callRecurlySafeXml(final AsyncHttpClient.BoundRequestBuilder builder, @Nullable final Class<T> clazz) {
    try {
  ...
  private <T extends java.lang.Object> T callRecurlyXml(final AsyncHttpClient.BoundRequestBuilder builder, @Nullable final Class<T> clazz) throws IOException, ExecutionException, InterruptedException {
    final Response response = builderCommon(builder).addHeader("Accept", "application/xml").addHeader("Content-Type", "application/xml; charset=utf-8").execute().get();
...
=======
  private <T extends java.lang.Object> T callRecurly(final AsyncHttpClient.BoundRequestBuilder builder, @Nullable final Class<T> clazz) throws IOException, ExecutionException, InterruptedException {
    final Response response = builder.addHeader("Authorization", "Basic " + key).addHeader("Accept", "application/xml").addHeader("Content-Type", "application/xml; charset=utf-8").addHeader("X-Api-Version", RECURLY_API_VERSION).addHeader(HttpHeaders.USER_AGENT, userAgent).setBodyEncoding("UTF-8").execute().get();
    final InputStream in = response.getResponseBodyAsStream();
    try {
      final String payload = convertStreamToString(in);
      if (debug()) {
...
>>>>>>> /usr/src/app/output/killbilling/recurly-java-library/d2c10050284b90a252edefbba709a016c5b9b453/src/main/java/com/ning/billing/recurly/RecurlyClient.java/right.java
