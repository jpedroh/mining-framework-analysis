...
public class SiteCrawler {
  ...
  public String getCrawlProgress() {
    ...
    sb.append(progress.scheduledForDownload).append(" scheduled for download. ");
    sb.append(progress.scheduledForProcessing).append(" scheduled for processing. ");
  ...
  private boolean shouldContinueCrawling() {
    ...
    if (shortCircuitAfter != 0 && visitedCounter.get() > shortCircuitAfter) {
      logger.info("A shortcircuit was set (at {}) and has been triggered after {} visited pages. Stopping this crawl for that reason.", shortCircuitAfter, visitedCounter.get());
      logger.warn("If you see a shortcircuit message (this one) in a production environment/build, " + "it is likely that somebody forgot to remove a debug \".setShortCircuit\" call. " + "Please report this if found.");
      return false;
    }
