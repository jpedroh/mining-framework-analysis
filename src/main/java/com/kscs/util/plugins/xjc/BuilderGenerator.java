...
class BuilderGenerator {
      ...
      if (typeInfo instanceof CElementInfo) {
        ...
        if (refs.size() == 1) {
          try {
            ...
            throw new RuntimeException(String.format("Unexpected type %s for tagRef %s", refs.get(0).getClass().getCanonicalName(), tagRef.getTagName()));
          }
        } else {
          throw new RuntimeException(String.format("Expecting one ref type for tagRef %s, found %s", tagRef.getTagName(), refs.size()));
        }
      } else {
        throw new RuntimeException(String.format("Unexpected type %s for tagRef %s", typeInfo.getClass().getCanonicalName(), tagRef.getTagName()));
      }
    }
    return type;
  }
  ...
  private void generateSingularChoiceProperty(final JBlock initBody, final JVar productParam, final PropertyOutline propertyOutline) {
    JFieldVar superTypeBuilderField = generateSingularChoiceSuperTypeProperty(initBody, productParam, propertyOutline).orElseThrow(() -> new RuntimeException(String.format("Expecting to have a builderField for property %s", propertyOutline.getFieldName())));
    for (final PropertyOutline.TagRef typeInfo : propertyOutline.getChoiceProperties()) {
      ...
      if (childBuilderOutline == null) {
        throw new RuntimeException(String.format("Don\'t think we should ever come in here, fieldName: %s", propertyOutline.getFieldName()));
      } else {
        ...
        if (this.implement) {
          ...
          JVar childBuilder = withBuilderMethod.body().decl(JMod.FINAL, builderWithMethodReturnType, fieldName + this.settings.getBuilderFieldSuffix(), JExpr._new(builderFieldElementType).arg(JExpr._this()).arg(JExpr._null()).arg(JExpr.FALSE));
          withBuilderMethod.body().assign(JExpr._this().ref(superTypeBuilderField), childBuilder);
  ...
  private void generateAddMethods(final PropertyOutline propertyOutline, final QName elementName, final JType jType) {
  ...
  private void overrideAddMethods(final PropertyOutline propertyOutline, final QName elementName, final JType elementType) {
  ...
  private void generateCollectionProperty(final JBlock initBody, final JVar productParam, final PropertyOutline propertyOutline, final JClass elementType) {
    ...
    if (this.implement) {
      ...
      final JExpression builderCreationExpression = childBuilderOutline == null ? JExpr._new(this.pluginContext.buildableClass).arg(jForEach.var()) : JExpr._new(childBuilderType).arg(JExpr._this()).arg(jForEach.var()).arg(this.settings.isCopyAlways() ? JExpr.TRUE : JExpr.FALSE);
  ...
  private Optional<JFieldVar> generateSingularChoiceSuperTypeProperty(final JBlock initBody, final JVar productParam, final PropertyOutline propertyOutline) {
    ...
    if (childBuilderOutline == null) {
      ...
      if (this.implement) {
        ...
        withMethod.body().assign(JExpr._this().ref(builderField), nullSafe(param, JExpr._new(this.pluginContext.buildableClass).arg(param)));
        withMethod.body()._return(JExpr._this());
        initBody.assign(productParam.ref(fieldName), nullSafe(JExpr._this().ref(builderField), this.pluginContext.castOnDemand(fieldType, JExpr._this().ref(builderField).invoke(this.settings.getBuildMethodName()))));
      }
    } else {
      throw new RuntimeException(String.format("Don\'t think we should ever come in here, fieldName: %s", propertyOutline.getFieldName()));
    }
        ...
        if (superPropertyOutline.getChoiceProperties().size() > 1) {
          for (final PropertyOutline.TagRef tagRef : propertyOutline.getChoiceProperties()) {
            ...
            try {
              ...
              pluginContext.errorHandler.warning(new SAXParseException("Encountered unsupported child type \"" + tagRef.getTypeInfo() + "\" in choice children collection. Unable to generate choice expansion.", this.pluginContext.outline.getModel().getLocator(), e));
            }
          }
        }
  ...
  final void generateCopyOfBuilderMethods() {
    if (this.implement) {
      ...
      builderCopyOfValueMethod.body().add(JExpr.invoke(paramOtherValue, PluginContext.COPY_TO_METHOD_NAME).arg(JExpr._this()))._return(JExpr._this());
                  ...
                  if (this.pluginContext.partialCopyableInterface.isAssignableFrom(fieldClass)) {
                    ...
                    if (this.pluginContext.copyableInterface.isAssignableFrom(fieldClass)) {
                      ...
                      if (this.pluginContext.cloneableInterface.isAssignableFrom(fieldClass)) {
                        ...
                        if (this.pluginContext.buildableInterface.isAssignableFrom(fieldClass)) {
                          currentBlock.assign(targetField, nullSafe(sourceRef, JExpr._new(this.pluginContext.buildableClass).arg(sourceRef)));
                        } else {
                          if (fieldOutline.getChoiceProperties().size() > 1) {
                            currentBlock.assign(targetField, nullSafe(sourceRef, JExpr._new(this.pluginContext.buildableClass).arg(sourceRef)));
                          } else {
  ...
  private void generateAddMethodJavadoc(final JMethod method, final JVar param) {
    ...
    method.javadoc().append(MessageFormat.format(this.resources.getString("comment.addMethod"), propertyName)).addParam(param).append(MessageFormat.format(this.resources.getString("comment.addMethod.param"), propertyName));
  }

  private void generateWithMethodJavadoc(final JMethod method, final JVar param) {
    final String propertyName = param.name();
    method.javadoc().append(MessageFormat.format(this.resources.getString("comment.withMethod"), propertyName)).addParam(param).append(MessageFormat.format(this.resources.getString("comment.withMethod.param"), propertyName));
  }
  ...
  private void generateBuilderMethodJavadoc(final JMethod method, final String methodPrefix, final String propertyName) {
    ...
    method.javadoc().append(MessageFormat.format(this.resources.getString("comment." + methodPrefix + "BuilderMethod"), propertyName, endMethodClassName)).addReturn().append(MessageFormat.format(this.resources.getString("comment." + methodPrefix + "BuilderMethod.return"), propertyName, endMethodClassName));
  }
