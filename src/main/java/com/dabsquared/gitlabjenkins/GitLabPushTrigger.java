...
<<<<<<< /usr/src/app/output/dabsquared/gitlab-plugin/387517997c283739cbe1f4c0de882e6be6dd8b12/src/main/java/com/dabsquared/gitlabjenkins/GitLabPushTrigger.java/left.java
  private boolean triggerOnlyIfNewCommitsPushed = false;
=======
  private String labelsThatForcesBuildIfAdded = "";
>>>>>>> /usr/src/app/output/dabsquared/gitlab-plugin/387517997c283739cbe1f4c0de882e6be6dd8b12/src/main/java/com/dabsquared/gitlabjenkins/GitLabPushTrigger.java/right.java
  ...
  @Initializer(after = InitMilestone.JOB_LOADED) public static void migrateJobs() throws IOException {
    ...
    if (!oldConfig.jobsMigrated) {
      ...
      gitLabConfig.getConnections().add(new GitLabConnection(oldConfig.gitlabHostUrl, oldConfig.gitlabHostUrl, oldConfig.gitlabApiToken, "autodetect", oldConfig.ignoreCertificateErrors, 10, 10));
  ...
  @Override public boolean getTriggerOnMergeRequest() {
    return triggerOnMergeRequest;
  }

  @Override public boolean isTriggerOnlyIfNewCommitsPushed() {
    return triggerOnlyIfNewCommitsPushed;
  }

  @Override public boolean isTriggerOnAcceptedMergeRequest() {
    return triggerOnAcceptedMergeRequest;
  }

  @Override public boolean isTriggerOnApprovedMergeRequest() {
    return triggerOnApprovedMergeRequest;
  }

  @Override public boolean isTriggerOnClosedMergeRequest() {
  ...
  @Override public TriggerOpenMergeRequest getTriggerOpenMergeRequestOnPush() {
  ...
  @Override public boolean isSkipWorkInProgressMergeRequest() {
    return skipWorkInProgressMergeRequest;
  }

  @Override public String getLabelsThatForcesBuildIfAdded() {
  ...
  @Override public boolean getCancelPendingBuildsOnUpdate() {
  ...
  private void initializeBranchFilter() {
    branchFilter = BranchFilterFactory.newBranchFilter(branchFilterConfig().withIncludeBranchesSpec(includeBranchesSpec).withExcludeBranchesSpec(excludeBranchesSpec).withSourceBranchRegex(sourceBranchRegex).withTargetBranchRegex(targetBranchRegex).build(branchFilterType));
  }
  ...
  @Extension @Symbol(value = "gitlab") public static class DescriptorImpl extends TriggerDescriptor {
    ...
    @Override public boolean isApplicable(Item item) {
      return item instanceof Job && SCMTriggerItems.asSCMTriggerItem(item) != null && item instanceof ParameterizedJobMixIn.ParameterizedJob;
    }
    ...
    private StringBuilder retrieveProjectUrl(Job<?, ?> project) {
      return new StringBuilder().append(Jenkins.getInstance().getRootUrl()).append(GitLabWebHook.WEBHOOK_URL).append(retrieveParentUrl(project)).append('/').append(Util.rawEncode(project.getName()));
    }
    ...
    public ListBoxModel doFillTriggerOpenMergeRequestOnPushItems(@QueryParameter String triggerOpenMergeRequestOnPush) {
      return new ListBoxModel(new Option("Never", "never", triggerOpenMergeRequestOnPush.matches("never")), new Option("On push to source branch", "source", triggerOpenMergeRequestOnPush.matches("source")), new Option("On push to source or target branch", "both", triggerOpenMergeRequestOnPush.matches("both")));
    }
