...
public class PlayerProfile {
  ...
  private final Map<PrimarySkillType, Integer> skills = new EnumMap<>(PrimarySkillType.class);

  private final Map<PrimarySkillType, Float> skillsXp = new EnumMap<>(PrimarySkillType.class);

  private final Map<SuperAbilityType, Integer> abilityDATS = new EnumMap<>(SuperAbilityType.class);

  private final Map<UniqueDataType, Integer> uniquePlayerData = new EnumMap<>(UniqueDataType.class);
  ...
  @Deprecated public PlayerProfile(String playerName) {
    this(playerName, null, 0);
  }

  @Deprecated public PlayerProfile(String playerName, UUID uuid) {
    this(playerName, uuid, 0);
  }

  @Deprecated public PlayerProfile(String playerName, int startingLevel) {
  ...
  public PlayerProfile(String playerName, @Nullable UUID uuid, int startingLevel) {
    ...
    uniquePlayerData.put(UniqueDataType.CHIMAERA_WING_DATS, 0);
    lastLogin = System.currentTimeMillis();
  }

  @Deprecated public PlayerProfile(@NotNull String playerName, boolean isLoaded, int startingLvl) {
  ...
  @Deprecated public void scheduleSyncSaveDelay() {
  ...
  public void save(boolean useSync) {
    ...
    if (changed) {
      ...
      if (saveAttempts > 0) {
        mcMMO.p.getLogger().severe("Attempted to save profile for player " + getPlayerName() + " resulted in failure. " + saveAttempts + " have been made so far.");
      }
      if (saveAttempts < 10) {
        saveAttempts++;
