  ...
  public static void processLevelUpBroadcasting(@NotNull McMMOPlayer mmoPlayer, @NotNull PrimarySkillType primarySkillType, int level) {
    ...
    if (mcMMO.p.getGeneralConfig().shouldLevelUpBroadcasts()) {
      ...
      if (remainder == 0) {
        ...
        HoverEvent<Component> levelMilestoneHover = Component.text(mmoPlayer.getPlayer().getName()).append(Component.newline()).append(Component.text(LocalDate.now().toString())).append(Component.newline()).append(Component.text(mcMMO.p.getSkillTools().getLocalizedSkillName(primarySkillType) + " reached level " + level)).color(TextColor.fromHexString(HEX_BEIGE_COLOR)).asHoverEvent();
        ...
        mcMMO.p.getFoliaLib().getImpl().runNextTick((t) -> audience.sendMessage(component));
      }
    }
  }
  ...
  public static void processPowerLevelUpBroadcasting(@NotNull McMMOPlayer mmoPlayer, int powerLevel) {
    ...
    if (mcMMO.p.getGeneralConfig().shouldPowerLevelUpBroadcasts()) {
      ...
      if (remainder == 0) {
        ...
        HoverEvent<Component> levelMilestoneHover = Component.text(mmoPlayer.getPlayer().getName()).append(Component.newline()).append(Component.text(LocalDate.now().toString())).append(Component.newline()).append(Component.text("Power level has reached " + powerLevel)).color(TextColor.fromHexString(HEX_BEIGE_COLOR)).asHoverEvent();
