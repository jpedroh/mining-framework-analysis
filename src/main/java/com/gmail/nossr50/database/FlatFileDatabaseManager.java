...
public final class FlatFileDatabaseManager implements DatabaseManager {
  ...
  public static final int DATA_ENTRY_COUNT = COOLDOWN_TRIDENTS + 1;
  ...
  @Override public void cleanupUser(UUID uuid) {
  }
    ...
    synchronized (fileWritingLock) {
      try {
        ...
        while ((line = in.readLine()) != null) {
          ...
          if (!(uuid != null && splitData[UUID_INDEX].equalsIgnoreCase(uuid.toString())) && !splitData[USERNAME_INDEX].equalsIgnoreCase(playerName)) {
            writer.append(line).append("\r\n");
          } else {
        ...
        while ((line = in.readLine()) != null) {
          ...
          try {
            ...
            if (fromDataUUID.equals(uuid)) {
              ...
              if (!matchingName) {
                logger.warning("When loading user: " + playerName + " with UUID of (" + uuid.toString() + ") we found a mismatched name, the name in the DB will be replaced (DB name: " + dbPlayerName + ")");
