...
public class ListRecordsHandler extends VerbHandler<ListRecordsType> {
  ...
  @Override public ListRecordsType handle(OAIParameters parameters) throws OAIException, HandlerException {
    ...
    if (!parameters.hasSet()) {
      if (parameters.hasFrom() && !parameters.hasUntil()) {
        result = itemRepositoryHelper.getItems(context, token.getOffset(), length, parameters.getMetadataPrefix(), parameters.getFrom());
      } else {
        if (!parameters.hasFrom() && parameters.hasUntil()) {
          result = itemRepositoryHelper.getItemsUntil(context, token.getOffset(), length, parameters.getMetadataPrefix(), parameters.getUntil());
        } else {
          if (parameters.hasFrom() && parameters.hasUntil()) {
            result = itemRepositoryHelper.getItems(context, token.getOffset(), length, parameters.getMetadataPrefix(), parameters.getFrom(), parameters.getUntil());
          } else {
            result = itemRepositoryHelper.getItems(context, token.getOffset(), length, parameters.getMetadataPrefix());
          }
        }
      }
    } else {
      ...
      if (parameters.hasFrom() && !parameters.hasUntil()) {
        result = itemRepositoryHelper.getItems(context, token.getOffset(), length, parameters.getMetadataPrefix(), parameters.getSet(), parameters.getFrom());
      } else {
        if (!parameters.hasFrom() && parameters.hasUntil()) {
          result = itemRepositoryHelper.getItemsUntil(context, token.getOffset(), length, parameters.getMetadataPrefix(), parameters.getSet(), parameters.getUntil());
        } else {
          if (parameters.hasFrom() && parameters.hasUntil()) {
            result = itemRepositoryHelper.getItems(context, token.getOffset(), length, parameters.getMetadataPrefix(), parameters.getSet(), parameters.getFrom(), parameters.getUntil());
          } else {
            result = itemRepositoryHelper.getItems(context, token.getOffset(), length, parameters.getMetadataPrefix(), parameters.getSet());
          }
        }
      }
    }
    ...
    if (!item.isDeleted()) {
      ...
      try {
        if (context.getTransformer().hasXslTemplates()) {
          metadata = new MetadataType(itemHelperWrap.toPipeline(true).apply(context.getTransformer().getXslTemplates().getValue()).apply(format.getXsltTemplates()).getTransformed());
        } else {
          metadata = new MetadataType(itemHelperWrap.toPipeline(true).apply(format.getXsltTemplates()).getTransformed());
        }
