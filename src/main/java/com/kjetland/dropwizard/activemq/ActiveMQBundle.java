...
public class ActiveMQBundle implements ConfiguredBundle<ActiveMQConfigHolder>, Managed, ActiveMQSenderFactory {
  ...
  @Override public void run(ActiveMQConfigHolder configuration, Environment environment) throws Exception {
    ...
    environment.healthChecks().register("ActiveMQ", new ActiveMQHealthCheck(realConnectionFactory, configuration.getActiveMQ().healthCheckMillisecondsToWait));
  ...
  public <T extends java.lang.Object> void registerReceiver(String destination, ActiveMQReceiver<T> receiver, Class<? extends T> clazz, final boolean ackMessageOnException) {
    ActiveMQReceiverHandler<T> handler = null;
    handler = new ActiveMQReceiverHandler<T>(destination, connectionFactory, receiver, clazz, objectMapper, (message, exception) -> {
  ...
  private <T extends java.lang.Object> void internalRegisterReceiver(String destination, ActiveMQReceiverHandler<T> handler) {
  ...
  public <T extends java.lang.Object> void registerReceiver(String destination, ActiveMQReceiver<T> receiver, Class<? extends T> clazz, ActiveMQExceptionHandler exceptionHandler) {
    ActiveMQReceiverHandler<T> handler = null;
    handler = new ActiveMQReceiverHandler<T>(destination, connectionFactory, receiver, clazz, objectMapper, exceptionHandler, shutdownWaitInSeconds);
    internalRegisterReceiver(destination, handler);
  }
}