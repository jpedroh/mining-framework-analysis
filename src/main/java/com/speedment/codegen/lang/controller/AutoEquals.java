...
public class AutoEquals<T extends Fieldable<T> & Methodable<T> & Nameable<T>> implements Consumer<T> {
  @Override public void accept(T t) {
    ...
    if (!hasMethod(t, "equals", 1)) {
      ...
      t.add(new MethodImpl("equals", Default.BOOLEAN_PRIMITIVE).add(Default.OVERRIDE).public_().add(new FieldImpl("other", Default.OBJECT)).add("if (other == null) return false;" + nl()).add("if (!getClass().equals(other.getClass())) " + block("return false;") + nl()).add("final " + type + " o = (" + type + ") other;").add(t.getFields().stream().map((f) -> compare(t, f)).collect(Collectors.joining(nl() + " && ", "return (", ");"))));
    }
    if (!hasMethod(t, "hashCode", 0)) {
      t.add(new MethodImpl("hashCode", Default.INT_PRIMITIVE).add(Default.OVERRIDE).public_().add("int hash = 7;").add(t.getFields().stream().map((f) -> hash(f)).collect(Collectors.joining(nl()))).add("return hash;"));
    }
  }
  ...
  private String hash(Field f) {
    ...
    switch (f.getType().getName()) {
      ...
      default:
      return prefix + "this." + f.getName() + ".hashCode();";
    }
  }
