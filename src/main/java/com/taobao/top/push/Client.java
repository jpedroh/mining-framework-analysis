...
public class Client {
  ...
  private Map<String, Object, Object> state;

  public Client(LoggerFactory factory, Object id) {
    this(factory, id, null, null);
  }

  public Client(LoggerFactory factory, Object id, MessageStateHandler messageStateHandler, ClientStateHandler clientStateHandler) {
    ...
    this.state = new ConcurrentHashMap<String, Object, Object>();
  }
  ...
  public Map<String, Object, Object> getState() {
    return this.state;
  }
  ...
  public void flush(CancellationToken token, int count) {
    ...
    if (temp > 0 && this.logger.isInfoEnabled()) {
      this.logger.info("flush %s messages to client#%s, cost %sms, totalSendMessageCount=%s", temp, this.getId(), System.currentTimeMillis() - begin, this.totalSendMessageCount);
    }
  }
  ...
  public synchronized void disconnect(String reasonText) {
    ...
    for (int i = 0; i < size; i++) {
      try {
        ...
        this.logger.info("client#%s disconnect a connection from %s: %s", this.getId(), connection.getOrigin(), reasonText);
  ...
  protected synchronized void AddConnection(ClientConnection conn) {
    ...
    this.logger.info("client#%s add new connection from %s", this.getId(), conn.getOrigin());
    this.onConnect(conn);
  }

  protected synchronized void RemoveConnection(ClientConnection conn) {
    this.connections.remove(conn);
    this.logger.info("client#%s remove a connection from %s", this.getId(), conn.getOrigin());
  ...
  protected boolean SendMessage(CancellationToken token, Object message, Queue<ClientConnection> connectionQueue) {
    while (true) {
      ...
      if (connection == null) {
        this.logger.info(String.format("client#%s no valid connection, drop message: %s", this.getId(), message));
      ...
      if (!connection.isOpen()) {
        ...
        if (this.logger.isInfoEnabled()) {
          this.logger.info("connection#%s[%s] is closed, remove it", connection.getId(), connection.getOrigin());
        }
        continue;
      }
      SendStatus status;
      try {
        ...
        this.logger.error(String.format("send message error to %s[%s]: %s", connection.getId(), connection.getOrigin(), message), e);
        continue;
      }
      switch (status) {
        case SENT:
