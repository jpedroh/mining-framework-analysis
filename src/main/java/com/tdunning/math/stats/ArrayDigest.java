...
public class ArrayDigest extends AbstractTDigest {
  ...
  @Override public void add(double x, int w) {
    checkValue(x);
    Index start = floor(x);
    if (start == null) {
      start = ceiling(x);
  ...
  @Override public void compress(GroupTree other) {
  ...
  @Override public int centroidCount() {
    return centroidCount;
  }

  @Override public Iterable<? extends Centroid> centroids() {
  ...
  @Override public double compression() {
    return compression;
  }
  ...
  /**
     ...
     */
  @Override public int smallByteSize() {
    int bound = byteSize();
  ...
  /**
     ...
     */
  @Override public void asBytes(ByteBuffer buf) {
  ...
  @Override public void asSmallBytes(ByteBuffer buf) {
  ...
  public static ArrayDigest fromBytes(ByteBuffer buf) {
    ...
    if (encoding == VERBOSE_ENCODING || encoding == VERBOSE_ARRAY_DIGEST) {
      ...
      if (encoding == SMALL_ENCODING || encoding == SMALL_ARRAY_DIGEST) {
        double compression = buf.getDouble();
        int pageSize = 32;
  ...
  private Iterable<Index> inclusiveTail(final Index start) {
    return new Iterable<Index>() {
      @Override public Iterator<Index> iterator() {
  ...
  @Override void add(double x, int w, Centroid base) {
