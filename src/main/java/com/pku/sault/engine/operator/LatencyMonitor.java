...
=======
      if (newestId - oldestId < queueSize) {
        return false;
      }
>>>>>>> /usr/src/app/output/taotaotheripper/sault/c42a266c8aca381fb541ecf5cabf1ad9594f7026/src/main/java/com/pku/sault/engine/operator/LatencyMonitor.java/right.java
  ...
  LatencyMonitor(List<Pair<ActorRef, ActorRef>> targetPorts, Bolt bolt) {
    ...
    timer = getContext().system().scheduler().schedule(Duration.Zero(), Duration.create(probePeriod, TimeUnit.MILLISECONDS), getSelf(), Msg.TICK, getContext().dispatcher(), getSelf());
    logger.info("Latency Monitor Started");
  }

  private Procedure<Object> SPLITTING = new Procedure<Object>() {
    @Override public void apply(Object msg) {
