...
class RouteTree implements Serializable {
  ...
  boolean isSiblingAvailable(int lowerBound) {
    ...
    return (siblingLowerBound >= MIN_VALUE && siblingLowerBound <= MAX_VALUE && !isSiblingSplit(siblingLowerBound, lowerBound));
  }
  ...
  ActorRef sibling(int lowerBound) {
    assert (isValidLowerBound(lowerBound));
    int siblingLowerBound = siblingNode(lowerBound);
    if (siblingLowerBound < MIN_VALUE || siblingLowerBound > MAX_VALUE || isSiblingSplit(siblingLowerBound, lowerBound)) {
