...
public class MstepParallel extends RecursiveAction {
  ...
  protected void computeDirectly() {
    ...
    for (int j = start; j < start + length; j++) {
      ...
      try {
        optimizer.minimize(itemLogLikelihood, initialValue, true, false, 500, 1);
        param = optimizer.getParameters();
        if (optimizer.getTerminationCode() > 3) {
          codeCount[0]++;
        }
      ...
      if (irm[j].getType() == IrmType.L3 || irm[j].getType() == IrmType.L4) {
        if (nPar == 4) {
          ...
          irm[j].setProposalGuessing(Math.min(1.000, Math.max(param[2], 0.001)));
          irm[j].setProposalSlipping(Math.max(0.60, Math.min(param[3], 0.999)));
        } else {
          if (nPar == 3) {
            if (param[0] < 0) {
              codeCount[1]++;
