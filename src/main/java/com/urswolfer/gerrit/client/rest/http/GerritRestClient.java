...
public class GerritRestClient implements RestClient {
  ...
  @Override public JsonElement getRequest(String path) throws RestApiException {
    return requestJson(path, null, HttpVerb.GET);
  }

  @Override public JsonElement postRequest(String path) throws RestApiException {
    return postRequest(path, null);
  }

  @Override public JsonElement postRequest(String path, String requestBody) throws RestApiException {
    return requestJson(path, requestBody, HttpVerb.POST);
  }

  @Override public JsonElement putRequest(String path) throws RestApiException {
    return putRequest(path, null);
  }

  @Override public JsonElement putRequest(String path, String requestBody) throws RestApiException {
    return requestJson(path, requestBody, HttpVerb.PUT);
  }

  @Override public JsonElement deleteRequest(String path) throws RestApiException {
    return requestJson(path, null, HttpVerb.DELETE);
  }

  @Override public JsonElement requestJson(String path, String requestBody, HttpVerb verb) throws RestApiException {
    try {
  ...
  @Override public HttpResponse requestRest(String path, String requestBody, HttpVerb verb) throws IOException, HttpStatusException {
  ...
  @Override public HttpResponse request(String path, String requestBody, HttpVerb verb, Header... headers) throws IOException, HttpStatusException {
  ...
  private Optional<Cookie> findCookie(final String cookieName) {
    ...
    return Iterables.tryFind(cookies, new Predicate<Cookie>() {
      @Override public boolean apply(Cookie cookie) {
  ...
  private HttpClientBuilder getHttpClient(HttpContext httpContext) {
    ...
    RequestConfig.Builder requestConfig = RequestConfig.custom().setConnectTimeout(CONNECTION_TIMEOUT_MS).setSocketTimeout(CONNECTION_TIMEOUT_MS).setConnectionRequestTimeout(CONNECTION_TIMEOUT_MS);
  ...
  private void throwHttpStatusException(HttpResponse response) throws IOException, HttpStatusException {
    ...
    String message = String.format("Request not successful. Message: %s. Status-Code: %s. Content: %s.", statusLine.getReasonPhrase(), statusLine.getStatusCode(), body);
