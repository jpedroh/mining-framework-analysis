...
public abstract class InfiniteScrollListView<T extends java.lang.Object> extends WebMarkupContainer {
  ...
  @SuppressWarnings(value = { "serial" }) @Override protected void onInitialize() {
    super.onInitialize();
    ...
    add(downBehavior = new AbstractDefaultAjaxBehavior() {
      ...
      @Override protected void respond(AjaxRequestTarget target) {
        target.appendJavaScript("InfiniteScroll.getFromContainer(\'" + InfiniteScrollListView.this.getMarkupId() + "\')" + ".scrollUpTo(\'" + getItemMarkupId(InfiniteScrollListView.this.getModel().getObject().get(Math.min(InfiniteScrollListView.this.getModel().getViewSize() - 1, InfiniteScrollListView.this.getModel().getObject().size() - 1))) + "\')");
  ...
  @SuppressWarnings(value = { "unchecked" }) public PageableListModel<List<T>> getModel() {
