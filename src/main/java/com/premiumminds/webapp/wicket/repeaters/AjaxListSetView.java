...
public abstract class AjaxListSetView<T extends Serializable> extends AbstractRepeater2 {
  ...
  @Override protected Iterator<? extends Component> renderIterator() {
    return IteratorUtils.transformedIterator(getList().iterator(), new Transformer<T, ListSetItem<?>>() {
      @Override public ListSetItem<?> transform(T el) {
  ...
  @Override protected void onPopulate() {
  ...
  @SuppressWarnings(value = { "unchecked" }) private ListOrderedSet<T> getList() {
  ...
  private void updateAjax(AjaxRequestTarget target) {
    ...
    for (ListSetItem<T> item : newItems) {
      String script = String.format("var item = document.createElement(\'%s\'); " + "item.id = \'%s\'; " + "var container = Wicket.$(\'%s\'); " + "$(container).append(item); ", markupTag, item.getMarkupId(), getMarkupId());
  ...
  public static class AjaxListener implements IListener {
    @Override public void onBeforeRespond(Map<String, Component> map, AjaxRequestTarget target) {
    ...
    @Override public void onAfterRespond(Map<String, Component> map, IJavaScriptResponse response) {
    }
