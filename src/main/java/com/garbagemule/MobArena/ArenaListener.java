...
public class ArenaListener {
  ...
  private boolean softRestore, softRestoreDrops, protect;

  private boolean monsterExp, monsterInfight, monsterTrack, pvpOn, pvpEnabled = false, foodRegen, lockFoodLevel, useClassChests;

  private boolean allowTeleport, canShare, autoIgniteTNT;

  private Set<Player> banned;


<<<<<<< /usr/src/app/output/garbagemule/mobarena/bad2700bdab561919b585cad9810fde8e939bbfc/src/main/java/com/garbagemule/MobArena/ArenaListener.java/left.java
  private HashMap<Integer, Player> lastKnownPlayerTargets;
=======
  private int autoIgniteFuse;
>>>>>>> /usr/src/app/output/garbagemule/mobarena/bad2700bdab561919b585cad9810fde8e939bbfc/src/main/java/com/garbagemule/MobArena/ArenaListener.java/right.java
  ...
  public ArenaListener(Arena arena, MobArena plugin) {
    ...
    this.excludeFromRetargeting = EnumSet.of(EntityType.ELDER_GUARDIAN, EntityType.GUARDIAN);
  ...
  public void onEntityExplode(EntityExplodeEvent event) {
    ...
    for (Block b : event.blockList()) {
      ...
      if (state.getData() instanceof Door && ((Door) state.getData()).isTopHalf()) {
        ...
        if (state.getData() instanceof Bed && ((Bed) state.getData()).isHeadOfBed()) {
        ...
        if (state instanceof Sign) {
          ...
          if (state.getData() instanceof Bed) {
            ...
            if (state.getData() instanceof Door) {
              ...
              if (state.getData() instanceof Attachable || state.getData() instanceof Redstone) {
                r = new RepairableAttachable(state);
              } else {
                r = new RepairableBlock(state);
              }
  ...
  public void onEntityDeath(EntityDeathEvent event) {
    if (event instanceof PlayerDeathEvent) {
      ...
      if (monsters.hasPet(event.getEntity())) {
        monsters.removePet(event.getEntity());
      } else {
        if (monsters.removeMonster(event.getEntity())) {
          onMonsterDeath(event);
        } else {
          if (monsters.removeMount(event.getEntity())) {
            onMountDeath(event);
          } else {
            if (monsters.removeGolem(event.getEntity())) {
  ...
  public void onEntityDamage(EntityDamageEvent event) {
    ...
    MABoss boss = (damagee instanceof LivingEntity) ? monsters.getBoss((LivingEntity) damagee) : null;
    if (arena.hasPet(damagee)) {
      ...
      if (damagee instanceof ArmorStand) {
        ...
        if (damagee instanceof AbstractHorse && monsters.hasMount(damagee)) {
          onMountDamage(event, (Horse) damagee, damager);
        } else {
          if (damagee instanceof Player) {
            onPlayerDamage(event, (Player) damagee, damager);
          } else {
            if (damagee instanceof Snowman && event.getCause() == DamageCause.MELTING) {
              ...
              if (boss != null) {
                ...
                if (monsters.getMonsters().contains(damagee)) {
                  onMonsterDamage(event, damagee, damager);
                } else {
                  if (monsters.getGolems().contains(damagee)) {
  ...
  private void onMonsterDamage(EntityDamageEvent event, Entity monster, Entity damager) {
    if (damager instanceof Player) {
      ...
      if (arena.hasPet(damager)) {
        ...
        if (monsters.getMonsters().contains(damager)) {
          if (!monsterInfight) {
            event.setCancelled(true);
          }
        }
  ...
  @SuppressWarnings(value = { "SuspiciousMethodCalls" }) private boolean isArenaPlayer(Entity entity) {
    return arena.getPlayersInArena().contains(entity);
  }

  @SuppressWarnings(value = { "SuspiciousMethodCalls" }) private boolean isArenaMonster(Entity entity) {
