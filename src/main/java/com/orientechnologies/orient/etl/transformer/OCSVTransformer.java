...
public class OCSVTransformer extends OAbstractTransformer {
  ...
  @Override public void configure(final OETLProcessor iProcessor, final ODocument iConfiguration, final OCommandContext iContext) {
  ...
  private Object determineTheType(String fieldStringValue) {
    Object fieldValue;
    if ((fieldValue = transformToDate(fieldStringValue)) == null) {
      if ((fieldValue = transformToNumeric(fieldStringValue)) == null) {
  ...
  public String getCellContent(String iValue) {
    ...
    if (stringCharacter != null && iValue.length() > 1 && (iValue.charAt(0) == stringCharacter && iValue.charAt(iValue.length() - 1) == stringCharacter)) {
