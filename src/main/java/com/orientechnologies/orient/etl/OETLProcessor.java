...
public class OETLProcessor {
   ...
   *          Execution Context
   */
  public OETLProcessor(final List<OBlock> iBeginBlocks, final OSource iSource, final OExtractor iExtractor, final List<OTransformer> iTransformers, final OLoader iLoader, final List<OBlock> iEndBlocks, final OCommandContext iContext) {
  ...
  public OETLProcessor parse(final ODocument cfg, final OCommandContext iContext) {
    return parse((Collection<ODocument>) cfg.field("begin"), (ODocument) cfg.field("source"), (ODocument) cfg.field("extractor"), (Collection<ODocument>) cfg.field("transformers"), (ODocument) cfg.field("loader"), (Collection<ODocument>) cfg.field("end"), iContext);
  }
  ...
  protected Class getClassByName(final OETLComponent iComponent, final String iClassName) {
    ...
    if (iClassName.equals("ODocument")) {
      ...
      if (iClassName.equals("String")) {
        inClass = String.class;
      } else {
        if (iClassName.equals("Object")) {
          inClass = Object.class;
        } else {
          if (iClassName.equals("OrientVertex")) {
            inClass = OrientVertex.class;
          } else {
            if (iClassName.equals("OrientEdge")) {
  ...
  protected void checkTypeCompatibility(final OETLComponent iCurrentComponent, final OETLComponent iLastComponent) {
    ...
    try {
      ...
      throw new OConfigurationException("Error on checking compatibility between components \'" + iLastComponent.getName() + "\' and \'" + iCurrentComponent.getName() + "\'", e);
    }
