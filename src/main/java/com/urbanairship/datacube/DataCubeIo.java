...
public class DataCubeIo<T extends Op> {
  ...
  public DataCubeIo(DataCube<T> cube, DbHarness<T> db, int batchSize, long maxBatchAgeMs, SyncLevel syncLevel) {
  ...
  /**
     ...
     */
  public DataCubeIo(DataCube<T> cube, DbHarness<T> db, int batchSize, long maxBatchAgeMs, SyncLevel syncLevel, String metricsScope) {
    this.cube = cube;
    this.db = db;
    this.batchSize = batchSize;
    this.maxBatchAgeMs = maxBatchAgeMs;
    this.syncLevel = syncLevel;
    writesMeter = Metrics.newMeter(DataCubeIo.class, "writes", metricsScope, "writes", TimeUnit.SECONDS);
    asyncQueueBackoffMeter = Metrics.newMeter(DataCubeIo.class, "backoffMeter", metricsScope, "fullQueueExceptions", TimeUnit.SECONDS);
    runBatchMeter = Metrics.newMeter(DataCubeIo.class, "runBatchMeter", metricsScope, "batches", TimeUnit.SECONDS);
    ageFlushes = Metrics.newMeter(DataCubeIo.class, "flushesDueToAge", metricsScope, "flushes", TimeUnit.SECONDS);
    sizeFlushes = Metrics.newMeter(DataCubeIo.class, "flushesDueToSize", metricsScope, "flushes", TimeUnit.SECONDS);
    this.asyncErrorMonitorExecutor = new ThreadPoolExecutor(Integer.MAX_VALUE, Integer.MAX_VALUE, 1, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new NamedThreadFactory("DataCubeIo async DB watcher"));
    ...
    switch (syncLevel) {
      ...
      synchronized (lock) {
        if (batchInProgress.getMap().isEmpty()) {
          ...
          batchFlushDeadlineMs = nowTimeMs + maxBatchAgeMs;
        ...
        if (batchInProgress.getMap().size() >= batchSize) {
          ...
          if (System.currentTimeMillis() >= batchFlushDeadlineMs) {
  ...
  public void writeSync(T op, WriteBuilder at) throws IOException, InterruptedException {
    if (syncLevel == SyncLevel.BATCH_ASYNC) {
      throw new IllegalArgumentException("You can\'t use WriteSync for this cube with " + "SyncLevel " + syncLevel);
    }
    ...
    if (optFuture.isPresent()) {
      try {
        ...
        if (flushException instanceof IOException) {
          ...
          if (flushException instanceof InterruptedException) {
            throw (InterruptedException) flushException;
          } else {
            if (flushException instanceof RuntimeException) {
