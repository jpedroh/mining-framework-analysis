...
public class HBaseDbHarness<T extends Op> implements DbHarness<T> {
  ...
  private final static Function<Map<byte[], byte[]>, Void> NOP = new Function<Map<byte[], byte[]>, Void>() {
    @Nullable @Override public Void apply(@Nullable Map<byte[], byte[]> map) {
      return null;
    }
  };
  ...
  public HBaseDbHarness(HTablePool pool, byte[] uniqueCubeName, byte[] tableName, byte[] cf, Deserializer<T> deserializer, IdService idService, CommitType commitType) throws IOException {
    this(pool, uniqueCubeName, tableName, cf, deserializer, idService, commitType, NOP, 5, 5, 10, null);
  }

  public HBaseDbHarness(HTablePool pool, byte[] uniqueCubeName, byte[] tableName, byte[] cf, Deserializer<T> deserializer, IdService idService, CommitType commitType, int numFlushThreads, int numIoeTries, int numCasTries, String metricsScope) throws IOException {
    this(pool, uniqueCubeName, tableName, cf, deserializer, idService, commitType, NOP, numFlushThreads, numIoeTries, numCasTries, metricsScope);
  }

  public HBaseDbHarness(HTablePool pool, byte[] uniqueCubeName, byte[] tableName, byte[] cf, Deserializer<T> deserializer, IdService idService, CommitType commitType, Function<Map<byte[], byte[]>, Void> onFlush, int numFlushThreads, int numIoeTries, int numCasTries, String metricsScope) throws IOException {
    this.pool = pool;
    ...
    flushSuccessTimer = Metrics.newTimer(HBaseDbHarness.class, "successfulBatchFlush", metricsScope, TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
    flushFailTimer = Metrics.newTimer(HBaseDbHarness.class, "failedBatchFlush", metricsScope, TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
    singleWriteTimer = Metrics.newTimer(HBaseDbHarness.class, "singleWrites", metricsScope, TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
    incrementSize = Metrics.newHistogram(HBaseDbHarness.class, "incrementSize", metricsScope, true);
    casTries = Metrics.newHistogram(HBaseDbHarness.class, "casTries", metricsScope, true);
    casRetriesExhausted = Metrics.newCounter(HBaseDbHarness.class, "casRetriesExhausted", metricsScope);
    ...
    this.flushExecutor = new ThreadPoolExecutor(numFlushThreads, numFlushThreads, 1, TimeUnit.MINUTES, workQueue, new NamedThreadFactory("HBase DB flusher " + cubeName));
  ...
  @Override public Optional<T> get(Address c) throws IOException, InterruptedException {
    ...
    if (result == null || result.isEmpty()) {
      if (log.isDebugEnabled()) {
        log.debug("Returning absent for cube:" + Arrays.toString(uniqueCubeName) + " for address:" + c + " key " + Base64.encodeBase64String(rowKey));
      }
      return Optional.absent();
    } else {
      T deserialized = deserializer.fromBytes(result.value());
      if (log.isDebugEnabled()) {
        log.debug("Returning value for cube:" + Arrays.toString(uniqueCubeName) + " address:" + c + ": " + " key " + Base64.encodeBase64String(rowKey) + ": " + deserialized);
      }
    ...
    @Override public Object call() throws Exception {
      ...
      try {
        for (int attempt = 0; attempt < numIoeTries; attempt++) {
          try {
            flushBatch(batch);
            afterExecute.afterExecute(null);
            return null;
          } catch (IOException e) {
            lastIOException = e;
            log.error("IOException in worker thread flushing to HBase on attempt " + attempt + "/" + numIoeTries + ", will retry", e);
            Thread.sleep(500);
          }
        }
      } catch (Exception e) {
  ...
  @SuppressWarnings(value = { "unchecked" }) private byte[] readCombineCas(byte[] rowKey, T newOp) throws IOException {
    Get get = new Get(rowKey);
    ...
    throw new IOException("Exhausted retries doing checkAndPut after " + numCasTries + " tries");
  }

  @Override public void set(Address c, T op) throws IOException, InterruptedException {
    final byte[] rowKey = ArrayUtils.addAll(uniqueCubeName, c.toKey(idService));
    overwrite(rowKey, op);
  }
  ...
  private void flushBatch(Batch<T> batch) throws IOException, InterruptedException {
    ...
    try {
      for (Map.Entry<Address, T> entry : batchMap.entrySet()) {
        ...
        if (log.isDebugEnabled()) {
          log.debug("Succesfully wrote cube:" + Arrays.toString(uniqueCubeName) + " address:" + address);
        }
  ...
  @Override public List<Optional<T>> multiGet(List<Address> addresses) throws IOException {
    ...
    for (Result result : results) {
      if (result == null || result.isEmpty()) {
        if (log.isDebugEnabled()) {
          ...
          log.debug("Returning absent for cube:" + Arrays.toString(uniqueCubeName) + " for address:" + address + " key " + Base64.encodeBase64String(rowKey));
        }
        ...
        if (log.isDebugEnabled()) {
          Address address = addresses.get(idx);
          byte[] rowKey = rowKeys.get(idx);
          log.debug("Returning value for cube:" + Arrays.toString(uniqueCubeName) + " address:" + address + ": " + " key " + Base64.encodeBase64String(rowKey) + ": " + deserialized);
        }
