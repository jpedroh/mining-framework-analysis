...
public class MapDbHarness<T extends Op> implements DbHarness<T> {
  ...
  public MapDbHarness(ConcurrentMap<BoxedByteArray, byte[]> map, Deserializer<T> deserializer, CommitType commitType, IdService idService) {
    this.map = map;
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) @Override public Future<?> runBatchAsync(Batch<T> batch, AfterExecute<T> afterExecute) {
    for (Map.Entry<Address, T> entry : batch.getMap().entrySet()) {
      ...
      if (commitType == CommitType.READ_COMBINE_CAS) {
        ...
        do {
          ...
          if (oldBytes.isPresent()) {
            ...
            if (log.isDebugEnabled()) {
              log.debug("Combined " + oldOp + " and " + opFromBatch + " into " + newOp);
            }
          } else {
            newOp = opFromBatch;
          }
          if (oldBytes.isPresent()) {
            if (map.replace(mapKey, oldBytes.get(), newOp.serialize())) {
              if (log.isDebugEnabled()) {
                log.debug("Successful CAS overwrite at key " + Hex.encodeHexString(mapKey.bytes) + " with " + Hex.encodeHexString(newOp.serialize()));
              }
              break;
            }
          } else {
            if (map.putIfAbsent(mapKey, newOp.serialize()) == null) {
              if (log.isDebugEnabled()) {
                log.debug("Successful CAS insert without existing key for key " + Hex.encodeHexString(mapKey.bytes) + " with " + Hex.encodeHexString(newOp.serialize()));
              }
              break;
            }
          }
        ...
        if (commitType == CommitType.OVERWRITE) {
  ...
  @Override public Optional<T> get(Address address) throws IOException, InterruptedException {
  ...
  private Optional<byte[]> getRaw(Address address) throws InterruptedException {
    ...
    if (log.isDebugEnabled()) {
      log.debug("getRaw for key " + Hex.encodeHexString(mapKey) + " returned " + Arrays.toString(bytes));
    }
    if (bytes == null) {
      return Optional.absent();
    } else {
  ...
  @Override public List<Optional<T>> multiGet(List<Address> addresses) throws IOException, InterruptedException {
