...
class ExecutionTimeBuilder {
  ...
  protected ExecutionTime build() {
    ...
    return new ExecutionTime(cronDefinition, yearsValueGenerator, daysOfWeekCronField, daysOfMonthCronField, daysOfYearCronField, months, hours, minutes, seconds);
  }
  ...
  private TimeNode timeNodeAlways(CronFieldName name, int lower, int higher) {
    return new TimeNode(FieldValueGeneratorFactory.forCronField(new CronField(name, always(), getConstraint(name))).generateCandidates(lower, higher));
  }
  ...
  private void validate(CronFieldName name, CronField cronField) {
    ...
    if (!name.equals(cronField.getField())) {
      throw new IllegalArgumentException(String.format("Invalid argument! Expected CronField instance for field %s but found %s", cronField.getField(), name));
    }
  }
