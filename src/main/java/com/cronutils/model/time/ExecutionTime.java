...
public class ExecutionTime {
  ...
  @VisibleForTesting ExecutionTime(CronDefinition cronDefinition, FieldValueGenerator yearsValueGenerator, CronField daysOfWeekCronField, CronField daysOfMonthCronField, CronField daysOfYearCronField, TimeNode months, TimeNode hours, TimeNode minutes, TimeNode seconds) {
  ...
  private ExecutionTimeResult potentialNextClosestMatch(ZonedDateTime date) throws NoSuchValueException {
    ...
    if (!minutes.getValues().contains(date.getMinute())) {
      ...
      if (nearestValue.getShifts() > 0) {
        newDate = ZonedDateTime.ofLocal(LocalDateTime.of(date.getYear(), date.getMonthValue(), date.getDayOfMonth(), date.getHour(), 0, 0), date.getZone(), date.getOffset()).plusHours(nearestValue.getShifts());
        return new ExecutionTimeResult(newDate, false);
      }
      if (nearestValue.getValue() < date.getMinute()) {
        date = date.plusHours(1);
      }
      newDate = ZonedDateTime.ofLocal(LocalDateTime.of(date.getYear(), date.getMonthValue(), date.getDayOfMonth(), date.getHour(), nextMinutes, lowestSecond), date.getZone(), date.getOffset());
    ...
    if (!seconds.getValues().contains(date.getSecond())) {
      ...
      if (nearestValue.getShifts() > 0) {
        newDate = ZonedDateTime.ofLocal(LocalDateTime.of(date.getYear(), date.getMonthValue(), date.getDayOfMonth(), date.getHour(), date.getMinute(), 0), date.getZone(), date.getOffset()).plusMinutes(nearestValue.getShifts());
        return new ExecutionTimeResult(newDate, false);
      }
      if (nearestValue.getValue() < date.getSecond()) {
        date = date.plusMinutes(1);
      }
      newDate = ZonedDateTime.ofLocal(LocalDateTime.of(date.getYear(), date.getMonthValue(), date.getDayOfMonth(), date.getHour(), date.getMinute(), nextSeconds), date.getZone(), date.getOffset());
      return new ExecutionTimeResult(newDate, true);
    }
  ...
  private TimeNode generateDayCandidatesUsingDoW(ZonedDateTime reference, WeekDay mondayDoWValue) {
    LocalDate date = LocalDate.of(reference.getYear(), reference.getMonthValue(), 1);
    int lengthOfMonth = date.lengthOfMonth();
    Set<Integer> candidates = new HashSet<>(createDayOfWeekValueGeneratorInstance(daysOfWeekCronField, reference.getYear(), reference.getMonthValue(), mondayDoWValue).generateCandidates(1, lengthOfMonth));
    List<Integer> candidatesList = new ArrayList<>(candidates);
    Collections.sort(candidatesList);
    return new TimeNode(candidatesList);
  }
