...
public class FileTarget extends TargetHandler implements Runnable {
  ...
  private static final ExecutorService finderPool = MoreExecutors.getExitingExecutorService(new ThreadPoolExecutor(finderThreads, finderThreads, 0L, TimeUnit.MILLISECONDS, finderQueue, new ThreadFactoryBuilder().setNameFormat("finder-%d").build()), 1, TimeUnit.SECONDS);
  ...
  @Override public void setContext(MeshyServer master, ChannelState state, int session) {
  ...
  @Override public void receive(int length, ChannelBuffer buffer) throws Exception {
  ...
  @Override public void receiveComplete() throws IOException {
    try {
    ...
    try {
      ...
      if (remote) {
        ...
        fileSource = new FileSource(getChannelMaster(), MeshyConstants.LINK_NAMED, paths.toArray(new String[paths.size()])) {
          ...
          @Override public void init(int session, int targetHandler, Set<Channel> group) {
            if (forwardMetaData) {
          ...
          @Override public void receiveComplete(ChannelState state, int completedSession) throws Exception {
  ...
  private final LinkedHashMap<String, String> pathStrings = new LinkedHashMap<String, String>() {
    @Override public boolean removeEldestEntry(Map.Entry<String, String> entry) {
