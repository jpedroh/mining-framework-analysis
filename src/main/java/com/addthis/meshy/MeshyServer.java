...
public class MeshyServer extends Meshy {
  ...
  public MeshyServer(final int port, final File rootDir, @Nullable String[] netif, final MeshyServerGroup group) throws IOException {
    super();
    this.group = group;
    ...
    if ((netif == null) || (netif.length == 0)) {
      ...
      for (String net : netif) {
        ...
        for (InterfaceAddress addr : nicif.getInterfaceAddresses()) {
          ...
          ServerSocketChannel serverChannel = (ServerSocketChannel) bootstrap.bind(new InetSocketAddress(inAddr, port)).syncUninterruptibly().channel();
  ...
  @Override protected void channelConnected(Channel channel, ChannelState channelState) {
  ...
  @Override protected void channelClosed(Channel channel, ChannelState channelState) {
    ...
    synchronized (connectedChannels) {
      for (ChannelState channelState : connectedChannels) {
        if (newUuid.equals(channelState.getName())) {
          log.info("rejecting peerage for {} @ {} (to {} @ {}) because uuid matches existing {} for: {}", peerState.getName(), peerState.getChannelRemoteAddress(), newUuid, newAddr, channelState, this);
          return false;
        }
        if (newAddr.equals(channelState.getRemoteAddress())) {
          log.info("rejecting peerage for {} @ {} (to {} @ {}) because address matches existing {} for: {}", peerState.getName(), peerState.getChannelRemoteAddress(), newUuid, newAddr, channelState, this);
          return false;
        }
      }
      log.info("promoting {} @ {} to named server peer as {} @ {} for: {}", peerState.getName(), peerState.getChannelRemoteAddress(), newUuid, newAddr, this);
  ...
  private void startAutoMesh(final int port, final int timeout) {
    Thread t = new Thread(new AutoMeshTask(this, group, timeout, port), "AutoMesh Peer Listener (port: " + port + ")");
    t.setDaemon(true);
    t.start();
  }

  @Override public String toString() {
    return MoreObjects.toStringHelper(this).add("serverPort", serverPort).add("serverUuid", serverUuid).add("channelCount", getChannelCount()).add("peeredCount", getServerPeerCount()).toString();
  }
}