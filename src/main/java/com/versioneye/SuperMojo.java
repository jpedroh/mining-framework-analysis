...
public class SuperMojo extends AbstractMojo {
  ...
  @Component protected RepositorySystem system;

  @Parameter(defaultValue = "${project}") protected MavenProject project;

  @Parameter(defaultValue = "${repositorySystemSession}") protected RepositorySystemSession session;

  @Parameter(defaultValue = "${project.remoteProjectRepositories}") protected List<RemoteRepository> repos;

  @Parameter(defaultValue = "${basedir}", property = "basedir", required = true) protected File projectDirectory;

  @Parameter(defaultValue = "${project.build.directory}", property = "outputDir", required = true) protected File outputDirectory;

  @Parameter(defaultValue = "${user.home}") protected File homeDirectory;

  @Parameter(property = "baseUrl", defaultValue = "https://www.versioneye.com") protected String baseUrl;

  @Parameter(property = "apiPath", defaultValue = "/api/v2") protected String apiPath;

  @Parameter(property = "apiKey") protected String apiKey;
  ...
  protected String fetchApiKey() throws Exception {
    ...
    if (apiKey == null || apiKey.isEmpty()) {
      throw new MojoExecutionException("com.versioneye.properties found but without an API Key! " + "Read the instructions at https://github.com/com.versioneye/versioneye_maven_plugin");
    }
    return apiKey;
  }
  ...
  protected Properties fetchHomeProperties() throws Exception {
    ...
    if (!file.exists()) {
      throw new MojoExecutionException(propertiesPath + " is missing! Read the instructions at " + "https://github.com/com.versioneye/versioneye_maven_plugin");
    }
    PropertiesUtils propertiesUtils = new PropertiesUtils();
