...
class ProxySourceFactory<Proxy extends java.lang.Object> {
  ...
  public String create(Class<?> originalClass) throws FinalCanNotBeExtendedException {
    ...
    builder = klass(generatedClassName).inPackage(generatedPackageName).modifier(klass.getModifiers() & ~(Modifier.STATIC | Modifier.PROTECTED)).parent(klass).interfaces(ProxySetter.class).add(field(originalClass, PROXY_OBJECT_FIELD_NAME).initNull()).add(field(MethodInterceptor.class, INTERCEPTOR_FIELD_NAME).initNull()).add(method("void", "set" + PROXY_OBJECT_FIELD_NAME).modifier(Modifier.PUBLIC).arguments(argument(Object.class, PROXY_OBJECT_FIELD_NAME)).command("this." + PROXY_OBJECT_FIELD_NAME + " = (" + originalClass.getCanonicalName() + ")" + PROXY_OBJECT_FIELD_NAME)).add(method("void", "set" + INTERCEPTOR_FIELD_NAME).modifier(Modifier.PUBLIC).arguments(argument(MethodInterceptor.class, INTERCEPTOR_FIELD_NAME)).command("this." + INTERCEPTOR_FIELD_NAME + " = " + INTERCEPTOR_FIELD_NAME));
    for (Constructor<?> constructor : klass.getConstructors()) {
      Type[] types = constructor.getGenericParameterTypes();
      builder.add(constructor(builder).arguments(getArguments(types)).command("super(" + getCommaSeparatedArgumentLists(types.length) + ")"));
    }
  ...
  private void assertClassIsNotFinal() throws FinalCanNotBeExtendedException {
    if ((klass.getModifiers() & Modifier.FINAL) > 0) {
      throw new FinalCanNotBeExtendedException("The class \'" + klass.getCanonicalName() + "\' is final. Proxy can not be created for final classes");
    }
  }
  ...
  private void appendMethodSource(Method method) {
    if ((method.getModifiers() & Modifier.FINAL) == 0) {
      boolean intercept = callbackFilter == null || callbackFilter.accept(method);
      if (intercept) {
  ...
  private static String calculateMethodProxyFieldName(Method method) {
    ...
    for (Class<?> parameterClazz : method.getParameterTypes()) {
      parameterTypes.append('_').append(parameterClazz.getName().replace('.', '_'));
    }
    return method.getName() + parameterTypes.toString() + "_MethodProxyInstance";
  }

  private void createProxyClassField(Method method) {
    JSC field = field(calculateMethodProxyFieldName(method)).returnType(MethodProxy.class).modifier(Modifier.PRIVATE).initValue("null");
    builder.add(field);
  }
  ...
  private String createMethodProxy(Method method) {
    JSC invoke = method("invoke").modifier(Modifier.PUBLIC).arguments(argument(Object.class, "obj"), argument((new Object[0]).getClass(), "args")).exceptions("Throwable").returnType(Object.class);
  ...
  private void createProxyMethod(Method method, boolean intercept) {
    final String returnType = Generics.typeToString(method.getGenericReturnType());
    ...
    if (intercept) {
      ...
      sb.append("if( null == ").append(methodProxyFieldName).append("){").append(methodProxyFieldName).append(" = new com.javax0.djcproxy.MethodProxy() {").append(createMethodProxy(method)).append("};}");
      if (!"void".equals(returnType)) {
        sb.append("return (" + method.getReturnType().getCanonicalName() + ")");
      }
      sb.append(INTERCEPTOR_FIELD_NAME + ".intercept(\n" + PROXY_OBJECT_FIELD_NAME + ", ");
      sb.append(PROXY_OBJECT_FIELD_NAME + ".getClass().getMethod(\"" + method.getName() + "\", ");
    ...
    builder.add(method(returnType, name).annotation("@Override").modifier(method.getModifiers() & ~(Modifier.NATIVE | Modifier.ABSTRACT)).arguments(arguments).commandBlock(sb.toString()).exceptions(method.getExceptionTypes()));
  }
}