...
public class LicenseVerifierReport extends AbstractLicenseVerifierPlugIn implements MavenReport {
  ...
  protected ResourceBundle getBundle(Locale locale) {
    return ResourceBundle.getBundle("mlvp", locale, this.getClass().getClassLoader());
  }
  ...
  private void doGenerateItemReport(ResourceBundle bundle, Sink sink) {
    ...
    for (LicenseInformation item : licenseData.getAllWithoutExcluded()) {
      sink.tableRow();
      cell(sink, item.getArtifact().getId());
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    try {
      ...
      getLog().error("An error has occurred in " + getName(Locale.ENGLISH) + " report generation:" + e.getMessage(), e);
