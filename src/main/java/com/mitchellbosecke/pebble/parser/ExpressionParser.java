...
public class ExpressionParser {
  ...
  private Expression<?> parseExpression(int minPrecedence) throws ParserException {
    ...
    while (isBinary(token) && binaryOperators.get(token.getValue()).getPrecedence() >= minPrecedence) {
      ...
      if (FilterExpression.class.equals(operator.getNodeClass())) {
        ...
        if (PositiveTestExpression.class.equals(operator.getNodeClass()) || NegativeTestExpression.class.equals(operator.getNodeClass())) {
  ...
  private Expression<?> subparseExpression() throws ParserException {
    ...
    switch (token.getType()) {
      ...
      break;
      default:
      throw new ParserException(null, String.format("Unexpected token \"%s\" of value \"%s\"", token.getType().toString(), token.getValue()), token.getLineNumber(), stream.getFilename());
    }
  ...
  private Expression<?> parseStringExpression() throws ParserException {
    ...
    while (true) {
      if (nextCanBeString && stream.current().test(Token.Type.STRING)) {
        ...
        if (stream.current().test(Token.Type.STRING_INTERPOLATION_START)) {
  ...
  @SuppressWarnings(value = { "unchecked" }) private Expression<?> parseTernaryExpression(Expression<?> expression) throws ParserException {
