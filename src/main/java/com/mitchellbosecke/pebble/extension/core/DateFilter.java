...
>>>>>>> /usr/src/app/output/mbosecke/pebble/1d6c48cea436f9c099a0ca56ba3cbad12c5d8db5/src/main/java/com/mitchellbosecke/pebble/extension/core/DateFilter.java/right.java
  ...
  private Object applyTemporal(final TemporalAccessor input, PebbleTemplate self, final Locale locale, int lineNumber, final String format) throws PebbleException {
    final DateTimeFormatter formatter = format != null ? DateTimeFormatter.ofPattern(format, locale) : DateTimeFormatter.ISO_DATE_TIME;
    try {
      return new SafeString(formatter.format(input));
    } catch (DateTimeException dte) {
      throw new PebbleException(dte, String.format("Could not parse the string \'%1\' into a date.", input.toString()), lineNumber, self.getName());
    }
  }
}