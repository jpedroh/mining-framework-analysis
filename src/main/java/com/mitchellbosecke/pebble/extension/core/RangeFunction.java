...
public class RangeFunction implements Function {
  ...
  @Override public Object execute(Map<String, Object> args, PebbleTemplate self, EvaluationContext context, int lineNumber) throws PebbleException {
    ...
    if (increment == null) {
      ...
      if (!(increment instanceof Number)) {
        throw new PebbleException(null, "The increment of the range function must be a number " + increment, lineNumber, self.getName());
      }
    }
    ...
    if (start instanceof Number && end instanceof Number) {
      ...
      if (incrementNum > 0) {
        ...
        if (incrementNum < 0) {
          ...
          throw new PebbleException(null, "The increment of the range function must be different than 0", lineNumber, self.getName());
        }
      }
    } else {
      if (start instanceof String && end instanceof String) {
        ...
        if (startStr.length() != 1 || endStr.length() != 1) {
          throw new PebbleException(null, "Arguments of range function must be of type Number or String with " + "a length of 1", lineNumber, self.getName());
        }
