...
public final class LanguageDetectorImpl implements LanguageDetector {
  ...
  @NotNull private final NgramFrequencyData ngramFrequencyData;

  /**
     * User-defined language priorities, in the same order as {@code langlist}.
     */
  @Nullable private final double[] priorMap;
  ...
  @Override public Optional<LdLocale> detect(CharSequence text) {
  ...
  @Override public List<DetectedLanguage> getProbabilities(CharSequence text) {
  ...
  /**
     ...
     */
  @Nullable private double[] detectBlock(CharSequence text) {
  ...
  private double[] detectBlockShortText(Map<String, Integer> ngrams) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("==> " + sortProbability(prob));
    }
    return prob;
  }
      ...
      for (int i = 0; i < ITERATION_LIMIT; i++) {
        ...
        if (i % 5 == 0) {
          ...
          if (logger.isTraceEnabled()) {
            logger.trace("> " + sortProbability(prob));
          }
        }
      }
  ...
  private boolean updateLangProb(@NotNull double[] prob, @NotNull String ngram, int count, double alpha) {
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(ngram + "(" + Util.unicodeEncode(ngram) + "):" + Util.wordProbToString(langProbMap, ngramFrequencyData.getLanguageList()));
    }
    ...
    if (ngram.length() > 1) {
      if (prefixFactor != 1.0 && ngram.charAt(0) == ' ') {
        ...
        if (suffixFactor != 1.0 && ngram.charAt(ngram.length() - 1) == ' ') {
          weight *= suffixFactor;
        }
      }
    }
  ...
  /**
     ...
     */
  @NotNull private List<DetectedLanguage> sortProbability(double[] prob) {
    ...
    list.sort(new Comparator<DetectedLanguage>() {
