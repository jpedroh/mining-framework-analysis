...
public class Game extends StateBasedGame {
  ...
  @Override public void initStatesList(GameContainer container) throws SlickException {
    ...
    DuneTerrainFactory terrainFactory = new DuneTerrainFactory(new Theme(imageRepository.loadAndCache("sheet_terrain.png"), TILE_SIZE));
    container.setShowFPS(SHOW_FPS);
    container.setVSync(VSYNC);
    Shroud shroud = new Shroud(imageRepository.loadAndCache("shroud_edges.png"), TILE_SIZE);
    EntitiesData entitiesData = new EntitiesDataReader().fromRulesIni();
    ScenarioFactory scenarioFactory;
    if (StringUtils.isEmpty(this.mapFileName)) {
      scenarioFactory = new RandomMapScenarioFactory(shroud, terrainFactory, entitiesData);
    } else {
      scenarioFactory = new IniScenarioFactory(shroud, terrainFactory, entitiesData, mapFileName);
    }
    PlayingState playingState = new PlayingState(container, imageRepository, scenarioFactory);
  ...
  public static void main(String[] args) {
    ...
    Bootstrap.runAsApplication(new Game(GAME_TITLE, mapFileName), SCREEN_WIDTH, SCREEN_HEIGHT, FULLSCREEN);
  }
}