...
<<<<<<< /usr/src/app/output/bedatadriven/jackson-datatype-jts/1416f1af0698abdf85fed9076cf00896f176f3a8/src/main/java/com/bedatadriven/jackson/datatype/jts/GeometryDeserializer.java/left.java
    if (gf == null) {
...
=======
    this.gf = gf;
>>>>>>> /usr/src/app/output/bedatadriven/jackson-datatype-jts/1416f1af0698abdf85fed9076cf00896f176f3a8/src/main/java/com/bedatadriven/jackson/datatype/jts/GeometryDeserializer.java/right.java
  }

  @Override public Geometry deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
  ...
  private Geometry parseGeometry(JsonNode root) throws JsonMappingException {
    ...
    if (POINT.equals(typeName)) {
      ...
      if (MULTI_POINT.equals(typeName)) {
        return parseMultiPoint(root);
      } else {
        if (LINE_STRING.equals(typeName)) {
          return parseLineString(root);
        } else {
          if (MULTI_LINE_STRING.equals(typeName)) {
            return parseMultiLineStrings(root);
          } else {
            if (POLYGON.equals(typeName)) {
              return parsePolygon(root);
            } else {
              if (MULTI_POLYGON.equals(typeName)) {
                return parseMultiPolygon(root);
              } else {
                if (GEOMETRY_COLLECTION.equals(typeName)) {
