...
public final class HttpEventCollectorLoggingHandler extends Handler {
  ...
  public HttpEventCollectorLoggingHandler() {
    ...
    metadata.put(MetadataTags.HOST, getConfigurationProperty(MetadataTags.HOST, null));
    metadata.put(MetadataTags.INDEX, getConfigurationProperty(MetadataTags.INDEX, null));
    metadata.put(MetadataTags.SOURCE, getConfigurationProperty(MetadataTags.SOURCE, null));
    metadata.put(MetadataTags.SOURCETYPE, getConfigurationProperty(MetadataTags.SOURCETYPE, null));
    metadata.put(MetadataTags.MESSAGEFORMAT, getConfigurationProperty(MetadataTags.MESSAGEFORMAT, null));
    ...
    HttpEventCollectorSender.TimeoutSettings timeoutSettings = new HttpEventCollectorSender.TimeoutSettings(getConfigurationNumericProperty(ConnectTimeoutConfTag, HttpEventCollectorSender.TimeoutSettings.DEFAULT_CONNECT_TIMEOUT), getConfigurationNumericProperty(CallTimeoutConfTag, HttpEventCollectorSender.TimeoutSettings.DEFAULT_CALL_TIMEOUT), getConfigurationNumericProperty(ReadTimeoutConfTag, HttpEventCollectorSender.TimeoutSettings.DEFAULT_READ_TIMEOUT), getConfigurationNumericProperty(WriteTimeoutConfTag, HttpEventCollectorSender.TimeoutSettings.DEFAULT_WRITE_TIMEOUT), getConfigurationNumericProperty(TerminationTimeoutConfTag, HttpEventCollectorSender.TimeoutSettings.DEFAULT_TERMINATION_TIMEOUT));
  ...
  @Override public void publish(LogRecord record) {
    this.sender.send(record.getMillis(), record.getLevel().toString(), record.getMessage(), includeLoggerName ? record.getLoggerName() : null, includeThreadName ? String.format(Locale.US, "%d", record.getThreadID()) : null, null, (!includeException || record.getThrown() == null) ? null : record.getThrown().getMessage(), null);
  }
  ...
  private String getConfigurationProperty(final String property, final String defaultValue) {
    String value = LogManager.getLogManager().getProperty(getClass().getName() + '.' + property);
    if (value == null) {
      value = defaultValue;
    }
    return value;
  ...
  private long getConfigurationNumericProperty(final String property, long defaultValue) {
    return Integer.parseInt(getConfigurationProperty(property, String.format("%d", defaultValue)));
  }

  private boolean getConfigurationBooleanProperty(final String property, boolean defaultValue) {
    return Boolean.parseBoolean(getConfigurationProperty(property, String.valueOf(defaultValue)));
  }
}