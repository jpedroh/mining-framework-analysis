...
public class HttpEventCollectorLogbackAppender<E extends java.lang.Object> extends AppenderBase<E> {
  ...
  private void sendEvent(ILoggingEvent event) {
    ...
    if (this.started) {
      this.sender.send(event.getTimeStamp(), event.getLevel().toString(), _layout.doLayout((E) event), _includeLoggerName ? event.getLoggerName() : null, _includeThreadName ? event.getThreadName() : null, _includeMDC ? event.getMDCPropertyMap() : null, (!_includeException || event.getThrowableProxy() == null) ? null : event.getThrowableProxy().getMessage(), c.convert(event));
    }
  }
