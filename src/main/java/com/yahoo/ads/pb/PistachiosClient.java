...
public class PistachiosClient {
  ...
  static {
    ...
    try {
      ...
      setZkRegistryData(ConfigurationManager.getConfiguration().getString("Pistachio.Processor.JarPath"), ConfigurationManager.getConfiguration().getString("Pistachio.Processor.ClassName"), ProcessorRegistry.processorRegistryPath, zk);
      setZkRegistryData(ConfigurationManager.getConfiguration().getString("Pistachio.StoreCallback.JarPath"), ConfigurationManager.getConfiguration().getString("Pistachio.StoreCallback.ClassName"), StoreCallbackRegistry.storeCallbackPath, zk);
      setZkRegistryData(ConfigurationManager.getConfiguration().getString("Pistachio.LookupCallback.JarPath"), ConfigurationManager.getConfiguration().getString("Pistachio.LookupCallback.ClassName"), LookupCallbackRegistry.lookupCallbackPath, zk);
    } catch (Exception e) {
  ...
  private class RetryWaiter {
    ...
    public RetryWaiter(Meter failureMeter0) {
      ...
      backoff = (new ExponentialBackOff.Builder()).setInitialIntervalMillis(initialIntervalMillis).setMaxElapsedTimeMillis(maxElapsedTimeMillis).setMaxIntervalMillis(maxIntervalMillis).build();
    }
   ...
   * @exception       Exception other errors indicating failure
   */
  public boolean delete(byte[] key) throws MasterNotFoundException, ConnectionBrokenException {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      if (args.length == 2 && args[0].equals("lookup")) {
        ...
        if (args.length == 3 && args[0].equals("store")) {
          id = args[1];
          store = true;
          value = args[2];
          client.store(id.getBytes(), value.getBytes(), true);
        } else {
          if (args.length == 3 && args[0].equals("processbatch")) {
            id = (args[1]);
            store = true;
            value = args[2];
            ...
            if (args.length == 2 && args[0].equals("delete")) {
              ...
              if (args.length == 2 && args[0].equals("iterate")) {
                ...
                if (args.length == 3 && args[0].equals("jump")) {
                  String partition = args[1];
                  PistachioIterator iterator = client.iterator(Long.parseLong(partition));
                  ...
                  if (args.length == 2 && args[0].equalsIgnoreCase("multilookup")) {
                    ...
                    for (Map.Entry<byte[], byte[]> entry : ret.entrySet()) {
                      System.out.println("client.lookup(" + DefaultDataInterpreter.getDataInterpreter().interpretId(entry.getKey()) + ")" + DefaultDataInterpreter.getDataInterpreter().interpretData(entry.getValue()));
                    }
                  } else {
                    if (args.length == 3 && args[0].equalsIgnoreCase("multiprocess")) {
                      String[] tokens = args[1].split(",");
                      value = args[2];
                      ...
                      for (Map.Entry<byte[], Boolean> entry : ret.entrySet()) {
                        System.out.println("client.lookup(" + DefaultDataInterpreter.getDataInterpreter().interpretId(entry.getKey()) + ") " + (entry.getValue() ? "Succeed" : "Failure"));
                      }
                    } else {
