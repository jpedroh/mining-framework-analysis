...
public class BulkConnection {
  ...
  private JobInfo createOrUpdateJob(JobInfo job, String endpoint) throws AsyncApiException {
    return createOrUpdateJob(job, endpoint, job.getContentType() == null ? ContentType.XML : job.getContentType());
  }
  ...
  static void parseAndThrowException(InputStream in, ContentType type) throws AsyncApiException {
    try {
      ...
      if (type == ContentType.XML || type == ContentType.ZIP_XML || type == ContentType.CSV || type == ContentType.ZIP_CSV) {
        ...
        if (type == ContentType.JSON || type == ContentType.ZIP_JSON) {
  ...
  private BatchInfo createBatchFromStreamImpl(JobInfo jobInfo, InputStream input, boolean isZip) throws AsyncApiException {
    try {
      ...
      if (!transport.isSuccessful()) {
        parseAndThrowException(result, jobInfo.getContentType());
      }
  ...
  public BatchInfo createBatchWithInputStreamAttachments(JobInfo jobInfo, InputStream batchContent, Map<String, InputStream> attachments) throws AsyncApiException {
    ...
    try {
      ...
      endpoint = endpoint + "job/" + jobInfo.getId() + "/batch";
      Transport transport = config.createTransport();
      ZipOutputStream zipOut = new ZipOutputStream(transport.connect(endpoint, getHeaders(getContentTypeString(jobInfo.getContentType(), true)), false));
      try {
  ...
  private InputStream doHttpGet(URL url) throws IOException, AsyncApiException {
    ...
    if (!success) {
      ...
      if (connection.getContentType().contains(XML_CONTENT_TYPE)) {
        ...
        if (connection.getContentType().contains(JSON_CONTENT_TYPE)) {
  ...
  /**
     ...
     */
  static <T extends java.lang.Object> T deserializeJsonToObject(InputStream in, Class<T> tmpClass) throws IOException, ConnectionException {
    ObjectMapper mapper = new ObjectMapper();
