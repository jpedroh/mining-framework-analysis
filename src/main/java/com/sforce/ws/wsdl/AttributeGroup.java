      ...
      parser.addPostParseProcessor(new WsdlParser.PostParseProcessor() {
        @Override public void postParse() throws ConnectionException {
          ...
          if (referencedGroup != null) {
            ...
            throw new ConnectionException("attributeGroup ref \'" + ref + "\' could not be resolved at: " + positionDescription);
          }
        }
      });
    } else {
      if (name == null) {
        throw new WsdlParseException("attributeGroup name can not be null at: " + parser.getPositionDescription());
      }
      if ("".equals(name)) {
        throw new WsdlParseException("attributeGroup name can not be empty at: " + parser.getPositionDescription());
      }
    }
