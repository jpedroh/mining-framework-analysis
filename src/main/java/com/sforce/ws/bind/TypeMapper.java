...
public class TypeMapper {
  ...
  private static HashMap<String, QName> getJavaXmlMapping() {
    ...
    map.put("[B", new QName(Constants.SCHEMA_NS, "base64Binary"));
  ...
  public boolean isSObject(String namespace, String name) {
    if ("sObject".equals(name) && (SfdcApiType.Partner.getSobjectNamespace().equals(namespace) || SfdcApiType.CrossInstance.getSobjectNamespace().equals(namespace) || SfdcApiType.Internal.getSobjectNamespace().equals(namespace) || SfdcApiType.ClientSync.getSobjectNamespace().equals(namespace) || SfdcApiType.SyncApi.getSobjectNamespace().equals(namespace))) {
      return true;
    }
    return false;
  }
                  ...
                  if ("dateTime".equals(localType)) {
                    ...
                    if ("boolean".equals(localType)) {
                      ...
                      if ("base64Binary".equals(localType)) {
                        ...
                        if ("stringArray".equals(localType)) {
  ...
  @SuppressWarnings(value = { "unchecked" }) Object readSingle(XmlInputStream in, TypeInfo typeInfo, Class type) throws IOException, ConnectionException {
    if (isXsiNilTrue(in)) {
      consumeStartTag(in);
      consumeEndTag(in);
      return null;
  ...
  private Class<?> getJavaType(QName qName) throws ConnectionException {
    ...
    if (c == null) {
      ...
      if (type == null) {
        type = NameMapper.getPackageName(qName.getNamespaceURI(), packagePrefix) + "." + NameMapper.getClassName(qName.getLocalPart());
      }
      c = load(type);
