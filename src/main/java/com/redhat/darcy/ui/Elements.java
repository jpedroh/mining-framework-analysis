...
public abstract class Elements {
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) public static <T extends Element> T element(Class<T> type, Locator locator) {
    if (!type.isInterface()) {
    ...
    return (T) Proxy.newProxyInstance(Elements.class.getClassLoader(), new Class[] { type, HasElementContext.class, WrapsElement.class }, invocationHandler);
  }
  ...
  /**
     ...
     * implementation is registered for that class.
     */
  @SuppressWarnings(value = { "unchecked" }) public static <T extends Element> List<T> elements(Class<T> type, Locator locator) {
    if (!type.isInterface()) {
      throw new IllegalArgumentException("Element type must be an interface, was: " + type);
    }
    InvocationHandler invocationHandler = new ElementListHandler(type, locator);
    return (List<T>) Proxy.newProxyInstance(Elements.class.getClassLoader(), new Class[] { List.class, HasElementContext.class }, invocationHandler);
  }
