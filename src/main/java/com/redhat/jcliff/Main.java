 ...
 */
public class Main {
  private static final String VERSION = "2.12.4";
                  ...
                  if (args[i].startsWith("--pre=")) {
                    ...
                    if (args[i].equals("--noop")) {
                      ...
                      if (args[i].equals("--json")) {
                        ...
                        if (args[i].startsWith("--ruledir=")) {
                          ruleDir = args[i].substring("--ruledir=".length());
                        } else {
                          if (args[i].equals("--nobatch")) {
                            batch = false;
                          } else {
                            if (args[i].startsWith("--timeout=")) {
                              ...
                              if (args[i].startsWith("--reconnect-delay=")) {
                                reconnectDelay = args[i].substring("--reconnect-delay=".length());
                              } else {
                                if (args[i].startsWith("--waitport=")) {
    ...
    if (!files.isEmpty()) {
      ...
      try {
        ...
        if (!noDeployments) {
          ...
          Map<String, Set<String>> replaceList = deployment.findDeploymentsToReplace(allRequestedDeployments, currentDeployments);
          if (replaceList != null && replaceList.size() > 0) {
            for (Set<String> x : replaceList.values()) {
              currentDeploymentNames.removeAll(x);
            }
          }
          String[] names = deployment.getNewDeployments(allRequestedDeployments, currentDeploymentNames);
          for (String x : names) {
