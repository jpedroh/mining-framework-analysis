...
public class RebuildAction implements Action {
  ...
  /**
     * Rebuild Descriptor.
     */
  @Extension public static final RebuildDescriptor DESCRIPTOR = new RebuildDescriptor();
  ...
  /**
     ...
     */
  public void nonParameterizedRebuild(Run currentBuild, StaplerResponse response) throws IOException {
  ...
  private ParameterValue cloneParameter(ParameterValue oldValue, String newValue) {
    if (oldValue instanceof StringParameterValue) {
      ...
      if (oldValue instanceof BooleanParameterValue) {
        return new BooleanParameterValue(oldValue.getName(), Boolean.parseBoolean(newValue), oldValue.getDescription());
      } else {
  ...
  public RebuildParameterPage getRebuildParameterPage(ParameterValue value) {
    ...
    if (getClass().getResource(String.format("/%s/%s.jelly", getClass().getCanonicalName().replace('.', '/'), value.getClass().getSimpleName())) != null) {
      return new RebuildParameterPage(getClass(), String.format("%s.jelly", value.getClass().getSimpleName()));
    }
    return null;
  }
}