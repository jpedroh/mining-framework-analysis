...
public abstract class AbstractModelObject implements IModelObject {
  ...
  public static abstract class JsonUtil<T extends java.lang.Object> implements IModelObject.IJsonUtil<T> {
    ...
    public T[] createModelObjectArray(JsonArray jsonArray) {
      @SuppressWarnings(value = { "unchecked" }) T[] array = (T[]) Array.newInstance(new TypeToken<T>(getClass()) { }.getRawType(), jsonArray.size());
    ...
    public <X extends java.lang.Object> X[] createModelObjectArray(JsonArray jsonArray, TypeToken<X> typeToken) {
      @SuppressWarnings(value = { "unchecked" }) X[] array = (X[]) Array.newInstance(typeToken.getRawType(), jsonArray.size());
      for (int i = 0; i < jsonArray.size(); i++) {
        JsonElement jsonElement = jsonArray.get(i);
        JsonObject jsonObject = jsonElement.getAsJsonObject();
    ...
    public Paging<T> createModelObjectPaging(JsonObject jsonObject) {
      ...
      return new Paging.Builder<T>().setHref(jsonObject.get("href").getAsString()).setItems(createModelObjectArray(jsonObject.getAsJsonArray("items"))).setLimit(jsonObject.get("limit").getAsInt()).setNext((jsonObject.get("next") instanceof JsonNull) ? null : jsonObject.get("next").getAsString()).setOffset(jsonObject.get("offset").getAsInt()).setPrevious((jsonObject.get("previous") instanceof JsonNull) ? null : jsonObject.get("previous").getAsString()).setTotal(jsonObject.get("total").getAsInt()).build();
    }
    ...
    public PagingCursorbased<T> createModelObjectPagingCursorbased(JsonObject jsonObject) {
      Type type = new TypeToken<T>(getClass()) { }.getType();
      return new PagingCursorbased.Builder<T>().setHref(jsonObject.get("href").getAsString()).setItems(createModelObjectArray(jsonObject.getAsJsonArray("items"))).setLimit(jsonObject.get("limit").getAsInt()).setNext((jsonObject.get("next") instanceof JsonNull) ? null : jsonObject.get("next").getAsString()).setCursors(new Cursor.JsonUtil().createModelObjectArray(jsonObject.get("cursors").getAsJsonArray())).setTotal(jsonObject.get("total").getAsInt()).build();
    }
