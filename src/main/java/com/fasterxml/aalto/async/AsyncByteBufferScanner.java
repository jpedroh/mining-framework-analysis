...
public class AsyncByteBufferScanner extends AsyncByteScanner implements AsyncByteBufferFeeder {
  ...
  @Override protected final byte _currentByte() throws XMLStreamException {
  ...
  @Override protected final byte _prevByte() throws XMLStreamException {
  ...
  @Override protected final boolean handleDTDInternalSubset(boolean init) throws XMLStreamException {
    char[] outputBuffer;
    int outPtr;
    if (init) {
  ...
  @Override public final boolean needMoreInput() {
    return (_inputPtr >= _inputEnd) && !_endOfInput;
  }

  @Override public void feedInput(ByteBuffer buffer) throws XMLStreamException {
    if (_inputPtr < _inputEnd) {
  ...
  @Override public int nextFromTree() throws XMLStreamException {
    ...
    if (_nextEvent == EVENT_INCOMPLETE) {
      ...
      if (_state == STATE_TREE_SEEN_LT) {
        ...
        if (_state == STATE_TREE_SEEN_AMP) {
          return handleEntityStartingToken();
        } else {
          if (_state == STATE_TREE_NAMED_ENTITY_START) {
            return handleNamedEntityStartingToken();
          } else {
            if (_state == STATE_TREE_NUMERIC_ENTITY_START) {
  ...
  @Override protected int handlePI() throws XMLStreamException {
      ...
      switch (_state) {
        ...
        case STATE_PI_AFTER_TARGET:
        {
          ...
          if (b == BYTE_SPACE || b == BYTE_CR || b == BYTE_LF || b == BYTE_TAB) {
            ...
            _textBuilder.resetWithEmpty();
            if ((_inputPtr + 1) < _inputEnd && _inputBuffer.get(_inputPtr) == BYTE_QMARK && _inputBuffer.get(_inputPtr + 1) == BYTE_GT) {
              _inputPtr += 2;
              break main_loop;
            }
  ...
  @Override protected final int handleComment() throws XMLStreamException {
  ...
  private int handleEndElementStart() throws XMLStreamException {
    ...
    for (int qix = 0; qix < size; ++qix) {
      int ptr = _inputPtr;
      int q = (buf.get(ptr) << 24) | ((buf.get(ptr + 1) & 0xFF) << 16) | ((buf.get(ptr + 2) & 0xFF) << 8) | ((buf.get(ptr + 3) & 0xFF));
      _inputPtr += 4;
  ...
  private int handleEndElement() throws XMLStreamException {
    if (_state == STATE_DEFAULT) {
      ...
      final int quadSize = elemName.sizeInQuads() - 1;
      for ( ; _quadCount < quadSize; ++_quadCount) {
  ...
  @Override protected final int startCharacters(byte b) throws XMLStreamException {
    dummy_loop:
    do {
      int c = (int) b & 0xFF;
  ...
  protected int handleEntityInCharacters() throws XMLStreamException {
    ...
    if ((ptr + 3) <= _inputEnd) {
      ...
      if (b == BYTE_a) {
        ...
        if (b == BYTE_m) {
          if ((ptr + 1) < _inputPtr && _inputBuffer.get(ptr) == BYTE_p && _inputBuffer.get(ptr + 1) == BYTE_SEMICOLON) {
            _inputPtr = ptr + 2;
            return INT_AMP;
          }
        } else {
          if (b == BYTE_p) {
            if ((ptr + 2) < _inputPtr && _inputBuffer.get(ptr) == BYTE_o && _inputBuffer.get(ptr + 1) == BYTE_s && _inputBuffer.get(ptr + 2) == BYTE_SEMICOLON) {
              _inputPtr = ptr + 3;
              return INT_APOS;
            }
          }
        ...
        if (b == BYTE_g) {
          ...
          if (b == BYTE_l) {
            ...
            if (b == BYTE_q) {
              if ((ptr + 3) < _inputPtr && _inputBuffer.get(ptr) == BYTE_u && _inputBuffer.get(ptr + 1) == BYTE_o && _inputBuffer.get(ptr + 2) == BYTE_t && _inputBuffer.get(ptr + 3) == BYTE_SEMICOLON) {
                _inputPtr = ptr + 4;
                return INT_APOS;
              }
            }
  ...
  /**
     ...
     */
  @Override protected boolean skipCoalescedText() throws XMLStreamException {
    throwInternal();
    return false;
  }
  ...
  /**
     ...
     */
  @Override protected boolean handleAttrValue() throws XMLStreamException {
    if (_pendingInput != 0) {
  ...
  private final boolean handleAttrValuePending() throws XMLStreamException {
    ...
    if (_pendingInput == PENDING_STATE_ATTR_VALUE_AMP) {
      ...
      if (_pendingInput == PENDING_STATE_ATTR_VALUE_AMP_HASH) {
        if (_inputBuffer.get(_inputPtr) == BYTE_x) {
          _pendingInput = PENDING_STATE_ATTR_VALUE_AMP_HASH_X;
          ++_inputPtr;
          if (_inputPtr >= _inputEnd) {
        ...
        if (_pendingInput == PENDING_STATE_ATTR_VALUE_AMP_HASH_X) {
          ch = handleHexEntityInAttribute(true);
        } else {
          if (_pendingInput == PENDING_STATE_ATTR_VALUE_ENTITY_NAME) {
            ...
            if (_pendingInput == PENDING_STATE_ATTR_VALUE_DEC_DIGIT) {
              ch = handleDecEntityInAttribute(false);
            } else {
              if (_pendingInput == PENDING_STATE_ATTR_VALUE_HEX_DIGIT) {
  ...
  @Override protected final PName parseNewName(byte b) throws XMLStreamException {
    int q = b & 0xFF;
    if (q < INT_A) {
  ...
  @Override protected final PName parsePName() throws XMLStreamException {
    ...
    while (true) {
      ...
      if (_quadCount == 0) {
        ...
        if (_quadCount >= _quadBuffer.length) {
  ...
  /**
     ...
     */
  @Override protected final boolean handlePartialCR() {
    if (_pendingInput != PENDING_STATE_CR) {
      throwInternal();
    }
    if (_inputPtr >= _inputEnd) {
