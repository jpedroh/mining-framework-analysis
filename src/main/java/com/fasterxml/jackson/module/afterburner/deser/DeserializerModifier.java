...
public class DeserializerModifier extends BeanDeserializerModifier {
  ...
  @Override public BeanDeserializerBuilder updateBuilder(DeserializationConfig config, BeanDescription beanDesc, BeanDeserializerBuilder builder) {
    ...
    List<OptimizedSettableBeanProperty<?>> newProps = findOptimizableProperties(config, collector, builder.getProperties());
    if (!newProps.isEmpty()) {
      BeanPropertyMutator baseMutator = collector.buildMutator(_classLoader);
      for (OptimizedSettableBeanProperty<?> prop : newProps) {
        BeanPropertyMutator mut = baseMutator.with(prop.getOriginalProperty(), prop.getOptimizedIndex());
  ...
  /**
     ...
     */
  protected boolean isDefaultDeserializer(DeserializationConfig config, JsonDeserializer<?> ser) {
