...
public class DateTimeDeserializer extends JodaDateDeserializerBase<ReadableInstant> {
  ...
  @SuppressWarnings(value = { "unchecked" }) public static <T extends ReadableInstant> ValueDeserializer<T> forType(Class<T> cls) {
    return (ValueDeserializer<T>) new DateTimeDeserializer(cls, FormatConfig.DEFAULT_DATETIME_PARSER);
  }

  @Override public JodaDateDeserializerBase<?> withFormat(JacksonJodaDateFormat format) {
    return new DateTimeDeserializer(_valueClass, format);
  }

  @Override public ReadableInstant deserialize(JsonParser p, DeserializationContext ctxt) throws JacksonException {
  ...
  protected ReadableInstant _fromString(final JsonParser p, final DeserializationContext ctxt, String value) throws JacksonException {
    value = value.trim();
    if (value.isEmpty()) {
    ...
    if (ix > 0) {
      ...
      DateTime result = _format.createParser(ctxt).withZone(tz).parseDateTime(value);
