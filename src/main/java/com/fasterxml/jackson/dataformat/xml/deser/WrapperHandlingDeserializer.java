...
public class WrapperHandlingDeserializer extends DelegatingDeserializer {
  ...
  @Override protected ValueDeserializer<?> newDelegatingInstance(ValueDeserializer<?> newDelegatee0) {
  ...
  @Override public Object deserialize(JsonParser p, DeserializationContext ctxt) throws JacksonException {
    _configureParser(p);
    return _delegatee.deserialize(p, ctxt);
  }

  @SuppressWarnings(value = { "unchecked" }) @Override public Object deserialize(JsonParser p, DeserializationContext ctxt, Object intoValue) throws JacksonException {
    _configureParser(p);
    return ((ValueDeserializer<Object>) _delegatee).deserialize(p, ctxt, intoValue);
  }

  @Override public Object deserializeWithType(JsonParser p, DeserializationContext ctxt, TypeDeserializer typeDeserializer) throws JacksonException {
    _configureParser(p);
    return _delegatee.deserializeWithType(p, ctxt, typeDeserializer);
  }

  @SuppressWarnings(value = { "resource" }) protected final void _configureParser(JsonParser p) throws JacksonException {
    while (p instanceof JsonParserDelegate) {
      p = ((JsonParserDelegate) p).delegate();
    }
    if ((p instanceof FromXmlParser) && (_namesToWrap != null)) {
      JsonToken t = p.currentToken();
      if (t == JsonToken.START_OBJECT || t == JsonToken.START_ARRAY || t == JsonToken.PROPERTY_NAME) {
