...
public abstract class ParserBase extends ParserMinimalBase {
  ...
  @Override public JsonParser enable(Feature f) {
    _features |= f.getMask();
  ...
  @Override public JsonParser disable(Feature f) {
    _features &= ~f.getMask();
    if (f == Feature.STRICT_DUPLICATE_DETECTION) {
  ...
  @Override @Deprecated public JsonParser setFeatureMask(int newMask) {
  ...
  @Override public JsonParser overrideStdFeatures(int values, int mask) {
  ...
  @Override public JsonReadContext getParsingContext() {
  ...
  @SuppressWarnings(value = { "resource" }) @Override public byte[] getBinaryValue(Base64Variant variant) throws IOException {
  ...
  protected final JsonToken reset(boolean negative, int intLen, int fractLen, int expLen) throws JsonParseException {
  ...
  protected final JsonToken resetInt(boolean negative, int intLen) throws JsonParseException {
    try {
  ...
  protected final JsonToken resetFloat(boolean negative, int intLen, int fractLen, int expLen) throws JsonParseException {
    try {
  ...
  protected final JsonToken resetAsNaN(String valueStr, double value) throws JsonParseException {
  ...
  @Override public boolean isNaN() throws IOException {
  ...
  @Override public Number getNumberValueExact() throws IOException {
    if (_currToken == JsonToken.VALUE_NUMBER_INT) {
      if (_numTypesValid == NR_UNKNOWN) {
        _parseNumericValue(NR_UNKNOWN);
      }
  ...
  @Override public LazyNumber getLazyNumber() throws IOException {
    if (_numTypesValid == NR_UNKNOWN) {
      _parseNumericValue(NR_UNKNOWN);
    }
    if (_currToken == JsonToken.VALUE_NUMBER_INT) {
      ...
      if ((_numTypesValid & NR_BIGINT) != 0) {
        ...
        return new LazyBigInteger(_numberString, isEnabled(StreamReadFeature.USE_FAST_BIG_NUMBER_PARSER));
      }
      _throwInternal();
    }
    if ((_numTypesValid & NR_BIGDECIMAL) != 0) {
      ...
      return new LazyBigDecimal(_numberString, isEnabled(StreamReadFeature.USE_FAST_BIG_NUMBER_PARSER));
    }
    if ((_numTypesValid & NR_FLOAT) != 0) {
  ...
  protected void _reportMismatchedEndMarker(int actCh, char expCh) throws JsonParseException {
    ...
    _reportError(String.format("Unexpected close marker \'%s\': expected \'%c\' (for %s starting at %s)", (char) actCh, expCh, ctxt.typeDesc(), ctxt.startLocation(_contentReference())));
  }

  @SuppressWarnings(value = { "deprecation" }) protected char _handleUnrecognizedCharacterEscape(char ch) throws JsonProcessingException {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "deprecation" }) protected void _throwUnquotedSpace(int i, String ctxtDesc) throws JsonParseException {
  ...
  /**
     ...
     * @since 2.10
     */
  @SuppressWarnings(value = { "deprecation" }) protected String _validJsonValueList() throws IOException {
