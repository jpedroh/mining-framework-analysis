...
public final class ByteSourceJsonBootstrapper {
  ...
  @SuppressWarnings(value = { "resource" }) public Reader constructReader() throws IOException {
  ...
  private boolean checkUTF32(int quad) throws IOException {
    if ((quad >> 8) == 0) {
      ...
      if ((quad & 0x00FFFFFF) == 0) {
        _bigEndian = false;
      } else {
        if ((quad & ~0x00FF0000) == 0) {
          reportWeirdUCS4("3412");
        } else {
          if ((quad & ~0x0000FF00) == 0) {
            reportWeirdUCS4("2143");
          } else {
            return false;
          }
