...
public class ReaderBasedJsonParser extends ParserBase {
  ...
  @Override public int releaseBuffered(Writer w) throws IOException {
  ...
  /**
     ...
     */
  @Override public final String getText() throws IOException {
  ...
  @Override public final char[] getTextCharacters() throws IOException {
    if (_currToken != null) {
  ...
  @Override public final int getTextLength() throws IOException {
    if (_currToken != null) {
      switch (_currToken.id()) {
        case ID_FIELD_NAME:
  ...
  @Override public final int getTextOffset() throws IOException {
    if (_currToken != null) {
      switch (_currToken.id()) {
        case ID_FIELD_NAME:
        return 0;
  ...
  @Override public boolean nextFieldName(SerializableString sstr) throws IOException {
    _numTypesValid = NR_UNKNOWN;
    if (_currToken == JsonToken.FIELD_NAME) {
      _nextAfterName();
      return false;
  ...
  @Override public final String nextTextValue() throws IOException {
    if (_currToken == JsonToken.FIELD_NAME) {
      _nameCopied = false;
      JsonToken t = _nextToken;
      _nextToken = null;
  ...
  @Override public final int nextIntValue(int defaultValue) throws IOException {
    if (_currToken == JsonToken.FIELD_NAME) {
      _nameCopied = false;
      JsonToken t = _nextToken;
      _nextToken = null;
  ...
  @Override public final long nextLongValue(long defaultValue) throws IOException {
    if (_currToken == JsonToken.FIELD_NAME) {
      _nameCopied = false;
      JsonToken t = _nextToken;
      _nextToken = null;
  ...
  @Override public final Boolean nextBooleanValue() throws IOException {
    if (_currToken == JsonToken.FIELD_NAME) {
      _nameCopied = false;
      JsonToken t = _nextToken;
      _nextToken = null;
  ...
  @Override protected final void _finishString() throws IOException {
    int ptr = _inputPtr;
    final int inputLen = _inputEnd;
    if (ptr < inputLen) {
      final int[] codes = _icLatin1;
  ...
  @Override public JsonLocation getTokenLocation() {
    ...
    if (_currToken == JsonToken.FIELD_NAME) {
      ...
      return new JsonLocation(src, -1L, total, _nameStartRow, _nameStartCol);
    }
    return new JsonLocation(src, -1L, _tokenInputTotal - 1, _tokenInputRow, _tokenInputCol);
  }
