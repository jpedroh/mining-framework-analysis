...
public final class BigDecimalParser {
  ...
  public static BigDecimal parse(final char[] chars, final int off, final int len) {
    try {
      ...
      if (len <= MAX_CHARS_TO_REPORT) {
        ...
        stringToReport = new String(Arrays.copyOfRange(chars, off, MAX_CHARS_TO_REPORT)) + "(truncated, full length is " + chars.length + " chars)";
      }
  ...
  public static BigDecimal parseWithFastParser(final char[] ch, final int off, final int len) {
    try {
      ...
      final String reportNum = len <= MAX_CHARS_TO_REPORT ? new String(ch, off, len) : new String(ch, off, MAX_CHARS_TO_REPORT) + " [truncated]";
      throw new NumberFormatException("Value \"" + reportNum + "\" can not be represented as `java.math.BigDecimal`, reason: " + nfe.getMessage());
    }
  }
  ...
  private static BigDecimal toBigDecimalRec(final char[] chars, final int off, final int len, final int scale, final int splitLen) {
    if (len > splitLen) {
      int mid = len / 2;
