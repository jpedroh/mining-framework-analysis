...
class InPlaceApplyProcessor implements JsonPatchProcessor {
  ...
  private JsonNode getParentNode(List<String> fromPath, Operation forOp) {
    ...
    if (node == null) {
      error(forOp, "noSuchPath in source, path provided: " + getArrayNodeRepresentation(fromPath));
    }
    return node;
  }
  ...
  private static String getArrayNodeRepresentation(List<String> path) {
    return Joiner.on('/').appendTo(new StringBuilder().append('/'), Iterables.transform(path, EncodePathFunction.getInstance())).toString();
  }
}