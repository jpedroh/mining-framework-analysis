...
public class CassandraPersistor extends BusModBase implements Handler<Message<JsonObject>> {
  ...
  /**
	 ...
	 * @param message
	 */
  @SuppressWarnings(value = { "unchecked" }) private void prepare(Message<JsonObject> message) {
  ...
  protected void raw(Message<JsonObject> message) {
    ...
    try {
      ...
      sendError(message, "[Cassandra Persistor] Could not create query statement from " + rawMessage + "!", e);
      return;
    }
      ...
      if (columnValue instanceof String) {
        ...
        if (columnValue instanceof Boolean) {
          ...
          if (columnValue instanceof Date) {
            ...
            if (columnValue instanceof UUID) {
  ...
  /**
	 * Set the Cassandra connection session and forward that to the prepared statement cache for PreparedStatement
	 * creation.
	 * 
	 * @param session
	 */
