...
class CompositeTimedMethodInterceptor extends AbstractMetricMethodInterceptor<CompositeTimed, CompositeTimer> implements Ordered {
  ...
  @Override protected Object invoke(final MethodInvocation invocation, final CompositeTimer metric, final CompositeTimed annotation) throws Throwable {
  ...
  @Override protected CompositeTimer buildMetric(final MetricRegistry metricRegistry, final String metricName, final CompositeTimed annotation) {
  ...
  @Override protected String buildMetricName(final Class<?> targetClass, final Method method, final CompositeTimed annotation) {
  ...
  static AdviceFactory adviceFactory(final MetricRegistry metricRegistry) {
    return new AdviceFactory() {
      @Override public Advice getAdvice(Object bean, Class<?> targetClass) {
