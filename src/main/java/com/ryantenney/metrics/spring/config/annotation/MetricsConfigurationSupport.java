...
public class MetricsConfigurationSupport implements ImportAware {
  ...
  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor exceptionMeteredAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.exceptionMetered(getMetricRegistry(), proxyConfig);
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor meteredAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.metered(getMetricRegistry(), proxyConfig);
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor timedAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.timed(getMetricRegistry(), proxyConfig);
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor countedAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.counted(getMetricRegistry(), proxyConfig);
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor gaugeFieldAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.gaugeField(getMetricRegistry());
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor gaugeMethodAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.gaugeMethod(getMetricRegistry());
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor cachedGaugeAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.cachedGauge(getMetricRegistry());
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor metricAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.metric(getMetricRegistry());
  }

  @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor healthCheckBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.healthCheck(getHealthCheckRegistry());
  }

  @Deprecated @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor legacyCountedAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.legacyCounted(getMetricRegistry(), proxyConfig);
  }

  @Deprecated @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor legacyCachedGaugeAnnotationBeanPostProcessor() {
    return MetricsBeanPostProcessorFactory.legacyCachedGauge(getMetricRegistry());
  }

  @Deprecated @Bean @Role(value = BeanDefinition.ROLE_INFRASTRUCTURE) public BeanPostProcessor legacyMetricAnnotationBeanPostProcessor() {
