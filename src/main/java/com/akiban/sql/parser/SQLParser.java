...
public class SQLParser implements SQLParserContext {
  ...
  private Set<SQLParserFeature> features = EnumSet.of(SQLParserFeature.GROUPING, SQLParserFeature.DIV_OPERATOR, SQLParserFeature.MYSQL_HINTS, SQLParserFeature.MYSQL_INTERVAL, SQLParserFeature.UNSIGNED, SQLParserFeature.INFIX_MOD);
  ...
  public StatementNode parseStatement(String sqlText) throws StandardException {
    ...
    try {
      ...
      throw new SQLParserException(standardizeEol(ex.getMessage()), ex, tokenErrorPosition(ex.currentToken, sqlText));
    } catch (TokenMgrError ex) {
      parser = null;
      if (ex.errorCode == TokenMgrError.LEXICAL_ERROR) {
        throw new SQLParserException(ex.getMessage(), ex, lineColumnErrorPosition(ex.errorLine, ex.errorColumn, sqlText));
      } else {
