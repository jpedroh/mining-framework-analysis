...
<<<<<<< /usr/src/app/output/brunoribeiro/sql-parser/101b5461bd04057ce934915a66975c7f373918af/src/main/java/com/akiban/sql/unparser/NodeToString.java/left.java
      SPECIAL_INDEX_FUNC_NODE
=======
      ADD_INDEX_CONSTRAINT_NODE
>>>>>>> /usr/src/app/output/brunoribeiro/sql-parser/101b5461bd04057ce934915a66975c7f373918af/src/main/java/com/akiban/sql/unparser/NodeToString.java/right.java
      ...
      return 
<<<<<<< /usr/src/app/output/brunoribeiro/sql-parser/101b5461bd04057ce934915a66975c7f373918af/src/main/java/com/akiban/sql/unparser/NodeToString.java/left.java
      zorderFuncNode((SpecialIndexFuncNode) node)
=======
      indexConstraint((AddIndexConstraintDefinitionNode) node)
>>>>>>> /usr/src/app/output/brunoribeiro/sql-parser/101b5461bd04057ce934915a66975c7f373918af/src/main/java/com/akiban/sql/unparser/NodeToString.java/right.java
      ;
      default:
  ...
  protected String indexConstraint(AddIndexConstraintDefinitionNode node) throws StandardException {
    ...
    builder.append('(').append(indexColumnList(node.getIndexColumnList())).append(')');
  ...
  protected String renameNode(RenameNode node) throws StandardException {
    if (node.isAlterTable()) {
      return "ALTER TABLE " + toString(node.getObjectName()) + "RENAME COLUMN " + node.getOldObjectName() + " TO " + node.getNewObjectName();
    } else {
      if (node.getRenameType() == RenameNode.RenameType.INDEX) {
        if (node.getObjectName() == null) {
          return node.statementToString() + " " + node.getOldObjectName() + " TO " + node.getNewObjectName();
        } else {
          return node.statementToString() + " " + toString(node.getObjectName()) + "." + node.getOldObjectName() + " TO " + node.getNewObjectName();
        }
      } else {
        return node.statementToString() + " " + toString(node.getObjectName()) + " TO " + toString(node.getNewTableName());
      }
    }
  }
  ...
  protected String unionNode(UnionNode node) throws StandardException {
    return toString(node.getLeftResultSet()) + " UNION " + toString(node.getRightResultSet());
  }
  ...
  protected String likeEscapeOperatorNode(LikeEscapeOperatorNode node) throws StandardException {
    String like = maybeParens(node.getReceiver()) + " " + node.getOperator().toUpperCase() + " " + maybeParens(node.getLeftOperand());
  ...
  protected String inListOperatorNode(InListOperatorNode node) throws StandardException {
    return maybeParens(node.getLeftOperand()) + " IN (" + toString(node.getRightOperandList()) + ")";
  }
  ...
  protected String betweenOperatorNode(BetweenOperatorNode node) throws StandardException {
    return maybeParens(node.getLeftOperand()) + " BETWEEN " + maybeParens(node.getRightOperandList().get(0)) + " AND " + maybeParens(node.getRightOperandList().get(1));
  }
  ...
  protected String coalesceFunctionNode(CoalesceFunctionNode node) throws StandardException {
    return functionCall(node.getFunctionName(), node.getArgumentsList());
  }
  ...
  protected String prefixUnary(UnaryOperatorNode node) throws StandardException {
    return node.getOperator().toUpperCase() + " " + maybeParens(node.getOperand());
  }

  protected String suffixUnary(UnaryOperatorNode node) throws StandardException {
    return maybeParens(node.getOperand()) + " " + node.getOperator().toUpperCase();
  }

  protected String functionUnary(UnaryOperatorNode node) throws StandardException {
    return node.getOperator().toUpperCase() + "(" + toString(node.getOperand()) + ")";
  }

  protected String infixBinary(BinaryOperatorNode node) throws StandardException {
    return maybeParens(node.getLeftOperand()) + " " + node.getOperator().toUpperCase() + " " + maybeParens(node.getRightOperand());
  }
  ...
  protected String castNode(CastNode node) throws StandardException {
    return "CAST(" + toString(node.getCastOperand()) + " AS " + node.getType().toString() + ")";
  }
  ...
  protected String methodCallNode(MethodCallNode node) throws StandardException {
  ...
  protected String zorderFuncNode(SpecialIndexFuncNode node) {
    return node.getFunctionType() + "(" + node.get(0).getColumnName() + ", " + node.get(1).getColumnName() + ")";
  }
}