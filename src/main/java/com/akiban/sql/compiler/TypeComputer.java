...
public class TypeComputer implements Visitor {
  ...
  protected DataTypeDescriptor unaryLogicalOperatorNode(UnaryLogicalOperatorNode node) throws StandardException {
    ...
    if ((type != null) && !type.getTypeId().isBooleanTypeId()) {
      ...
      operand = (ValueNode) node.getNodeFactory().getNode(NodeTypes.CAST_NODE, operand, type, node.getParserContext());
  ...
  protected DataTypeDescriptor binaryLogicalOperatorNode(BinaryLogicalOperatorNode node) throws StandardException {
    ...
    if ((leftType != null) && !leftType.getTypeId().isBooleanTypeId()) {
      ...
      leftOperand = (ValueNode) node.getNodeFactory().getNode(NodeTypes.CAST_NODE, leftOperand, leftType, node.getParserContext());
    ...
    if ((rightType != null) && !rightType.getTypeId().isBooleanTypeId()) {
      ...
      rightOperand = (ValueNode) node.getNodeFactory().getNode(NodeTypes.CAST_NODE, rightOperand, rightType, node.getParserContext());
  ...
  protected DataTypeDescriptor binaryArithmeticOperatorNode(BinaryArithmeticOperatorNode node) throws StandardException {
    ...
    if (isParameterOrUntypedNull(leftOperand) && (rightType != null)) {
      ...
      if (isParameterOrUntypedNull(rightOperand) && (leftType != null)) {
  ...
  protected DataTypeDescriptor betweenOperatorNode(BetweenOperatorNode node) throws StandardException {
    ...
    if (!leftTypeId.isStringTypeId()) {
      if ((lowType != null) && lowType.getTypeId().isStringTypeId()) {
        lowOperand = (ValueNode) node.getNodeFactory().getNode(NodeTypes.CAST_NODE, lowOperand, leftType.getNullabilityType(lowType.isNullable()), node.getParserContext());
        rightOperands.set(0, lowOperand);
      }
      if ((highType != null) && highType.getTypeId().isStringTypeId()) {
        highOperand = (ValueNode) node.getNodeFactory().getNode(NodeTypes.CAST_NODE, highOperand, leftType.getNullabilityType(highType.isNullable()), node.getParserContext());
  ...
  protected DataTypeDescriptor concatenationOperatorNode(ConcatenationOperatorNode node) throws StandardException {
    ...
    return new DataTypeDescriptor(TypeId.VARCHAR_ID, leftType.isNullable() || rightType.isNullable(), leftType.getMaximumWidth() + rightType.getMaximumWidth());
  }
    ...
    if (result != null) {
      for (int i = 0; i < nodeList.size(); i++) {
        ...
        if (isParameterOrUntypedNull(node)) {
          ...
          if (addDominantCast(result, node.getType())) {
            node = (ValueNode) node.getNodeFactory().getNode(NodeTypes.CAST_NODE, node, result.getNullabilityType(node.getType().isNullable()), node.getParserContext());
            nodeList.set(i, node);
          }
        }
      }
