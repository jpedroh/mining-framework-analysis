...
public class ClueConfiguration {
  ...
  private static <T extends java.lang.Object> T getInstance(String className, T defaultInstance) {
    try {
      if (className == null) {
  ...
  private ClueConfiguration(Properties config) {
    analyzerQuery = getInstance(config.getProperty(ANALYZER_QUERY_PARAM), new StandardAnalyzer(Version.LUCENE_46));
    dirBuilder = getInstance(config.getProperty(DIRECTORY_BUILDER_PARAM), new DefaultDirectoryBuilder());
    queryBuilder = getInstance(config.getProperty(QUERY_BUILDER_PARAM), new DefaultQueryBuilder());
    indexReaderFactory = getInstance(config.getProperty(INDEX_READER_FACTORY_PARAM), new DefaultIndexReaderFactory());
