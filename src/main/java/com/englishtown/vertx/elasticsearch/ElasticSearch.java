...
public class ElasticSearch extends BusModBase implements Handler<Message<JsonObject>> {
  ...
  @Inject public ElasticSearch(TransportClientFactory clientFactory, ElasticSearchConfigurator configurator) {
  ...
  public void doIndex(final Message<JsonObject> message) {
    ...
    client.prepareIndex(index, type, id).setSource(source.encode()).execute(new ActionListener<IndexResponse>() {
      @Override public void onResponse(IndexResponse indexResponse) {
        JsonObject reply = new JsonObject().putString(CONST_INDEX, indexResponse.getIndex()).putString(CONST_TYPE, indexResponse.getType()).putString(CONST_ID, indexResponse.getId()).putNumber(CONST_VERSION, indexResponse.getVersion());
  ...
  public void doScroll(final Message<JsonObject> message) {
    ...
    client.prepareSearchScroll(scrollId).setScroll(scroll).execute(new ActionListener<SearchResponse>() {
      @Override public void onResponse(SearchResponse searchResponse) {
        handleActionResponse(searchResponse, message);
      }
  ...
  public void doDelete(final Message<JsonObject> message) {
    ...
    client.prepareDelete(index, type, id).execute(new ActionListener<DeleteResponse>() {
      @Override public void onResponse(DeleteResponse deleteResponse) {
        JsonObject reply = new JsonObject().putString(CONST_INDEX, deleteResponse.getIndex()).putString(CONST_TYPE, deleteResponse.getType()).putString(CONST_ID, deleteResponse.getId()).putNumber(CONST_VERSION, deleteResponse.getVersion());
        sendOK(message, reply);
      }
