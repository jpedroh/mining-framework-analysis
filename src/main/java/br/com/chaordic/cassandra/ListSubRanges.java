...
public class ListSubRanges {
  private static final String DEFAULT_NUMBER_OF_PARTITIONS = "32768";
  ...
  static {
    options = new Options();
    options.addOption("st", START_TOKEN_OPT, true, "Calculate subranges of the range with this start token.");
    options.addOption("et", END_TOKEN_OPT, true, "Calculate subranges of the range with this end token.");
    options.addOption("l", CASSANDRA_LISTEN_ADDRESS_OPT, true, "Node listen address (if different from broadcast address). (default: nodeIpAddress)");
    options.addOption("pr", PARTITIONER_RANGE_OPT, false, "Only consider the first range returned by the partitioner.");
    options.addOption("n", PARTITIONS_OPT, true, "Number of partitions per subsplit. (default 32K)");
    options.addOption("o", OMIT_HEADER_OPT, false, "Number of partitions per subsplit.");
    options.addOption("u", USERNAME, false, "Username to authenticate with.");
    options.addOption("p", PASSWORD, false, "Password to authenticate with.");
  }
  ...
  private static void retrieveAndPrintSubsplits(String columnFamily, int keysPerSplit, String startToken, String endToken, Cassandra.Client client) throws InvalidRequestException, TException {
  ...
  private static void initClient(String username, String password) throws InvalidRequestException, TException, AuthenticationException, AuthorizationException {
