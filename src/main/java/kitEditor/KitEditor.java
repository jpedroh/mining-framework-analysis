...
public class KitEditor extends JFrame implements SamplePicker.Listener {
  ...
  private void setListeners() {
    ...
    previousBankButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) previousBankAction.getValue(Action.ACCELERATOR_KEY), "previousBankAction");
    ...
    nextBankButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) nextBankAction.getValue(Action.ACCELERATOR_KEY), "nextBankAction");
    ...
    for (Sample s : samples[selectedBank]) {
      if (s != null) {
        if (halfSpeed.isSelected()) {
          s.setTrim((int) Math.ceil(s.getTrim() / 2.0));
        } else {
  ...
  private void addMenu() {
    ...
    edit.addMenuListener(new MenuListener() {
      @Override public void menuSelected(MenuEvent e) {
        trimAll.setEnabled(firstFreeSampleSlot() != 0 && firstFreeSampleSlot() != 1);
        pasteSampleMenuItem.setEnabled(firstFreeSampleSlot() >= 0 && firstFreeSampleSlot() < 15 && clipboard[0] != null);
      }
    ...
    new FileDrop(contentPane, (files) -> {
      for (File file : files) {
        ...
        if (fileName.endsWith(".wav")) {
          if (romImage == null) {
            JOptionPane.showMessageDialog(contentPane, "Open .gb file before adding samples.", "Can\'t add sample!", JOptionPane.ERROR_MESSAGE);
            continue;
          }
          addSample(file);
        } else {
          if (fileName.endsWith(".kit")) {
            ...
            JOptionPane.showMessageDialog(contentPane, "Unknown file type!", "File error", JOptionPane.ERROR_MESSAGE);
            return;
          }
        }
      }
  ...
  private byte[] getNibbles(int index) {
    ...
    System.arraycopy(romImage, (romBank - 1) * RomUtilities.BANK_SIZE + start, arr, 0, stop - start);
    if (isBankSwizzled()) {
      unSwizzle(arr);
    }
    return arr;
  ...
  @Override public void playSample() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(this, e.getMessage(), "Audio error", JOptionPane.INFORMATION_MESSAGE);
      e.printStackTrace();
    }
    updateSampleView();
  }
  ...
  private void saveKitSettings(File kitFile) throws IOException {
    ...
    try (FileWriter fileWriter = new FileWriter(kitSettingsFile)) {
      for (Sample s : samples[selectedBank]) {
        ...
        fileWriter.write(s.getFile().getAbsolutePath() + "|" + s.getVolumeDb() + "|" + s.getTrim() + "|" + s.getPitchSemitones() + "\n");
      }
    }
  }
  ...
  private void loadKit(File kitFile) {
    ...
    try {
      ...
      if (buf[0] != 0x60 || buf[1] != 0x40) {
        JOptionPane.showMessageDialog(this, "Malformed kit file!", "File error", JOptionPane.ERROR_MESSAGE);
        return;
      }
      for (byte aBuf : buf) {
  ...
  private void loadKitSettings(File kitFile) throws IOException, UnsupportedAudioFileException {
    ...
    try (BufferedReader fileReader = new BufferedReader(new FileReader(kitSettingsFile))) {
      for (int i = 0; i < MAX_SAMPLES; ++i) {
        ...
        samples[selectedBank][i] = Sample.createFromWav(sampleFile, dither.isSelected(), halfSpeed.isSelected(), volume, trim, pitch);
      }
    }
  }
  ...
  private void addSample(File wavFile) {
    if (firstFreeSampleSlot() == -1) {
      JOptionPane.showMessageDialog(contentPane, "Can\'t add sample, kit is full!", "Kit full", JOptionPane.ERROR_MESSAGE);
      return;
    }
    ...
    try {
      ...
      if (bytesFreeAfterAdd < 0) {
        ...
        sample.setTrim(trim);
        sample.reload(halfSpeed.isSelected());
        JOptionPane.showMessageDialog(this, "Trimmed sample to fit.", "Kit full!", JOptionPane.INFORMATION_MESSAGE);
      }
    } catch (Exception e) {
      showFileErrorMessage(e);
      return;
  ...
  private void dropSample() {
    ...
    for (int indexIt = 0; indexIt < indices.size(); ++indexIt) {
      ...
      if (14 - index >= 0) {
        System.arraycopy(samples[selectedBank], index + 1, samples[selectedBank], index, 14 - index);
      }
  ...
  private void updateButtonStates() {
    ...
    saveRomButton.setEnabled(!kitTooBig() && (!Arrays.equals(document.romImage(), romImage) || document.isRomDirty()));
  ...
  private void trimAllSamples() {
    ...
    if (trimmableSamples < 1) {
      JOptionPane.showMessageDialog(this, "No trimmable samples", "No samples to trim", JOptionPane.INFORMATION_MESSAGE);
      return;
    }
    ...
    int maxEqualSampleLength = trimmableSamples > sampleCount ? equalSampleLength + addLength / (trimmableSamples - sampleCount) : equalSampleLength;
    for (int sampleIt = 0; sampleIt < MAX_SAMPLES; ++sampleIt) {
      Sample sample = samples[selectedBank][sampleIt];
      if (sample != null && sample.canAdjustVolume()) {
        int trim = sample.untrimmedLengthInBytes() > maxEqualSampleLength ? (int) Math.round((sample.untrimmedLengthInBytes() - maxEqualSampleLength) / 16.0) : 0;
        sample.setTrim(trim);
      }
    }
    ...
    compileKit();
    updateRomView();
    JOptionPane.showMessageDialog(this, "Trimmed all samples to fit.", "Done", JOptionPane.INFORMATION_MESSAGE);
  }
...
>>>>>>> /usr/src/app/output/jkotlinski/lsdpatch/739c6593d8e03cf19f345d56a96db2886f48adbf/src/main/java/kitEditor/KitEditor.java/right.java
  ...
  @Override public void dupeSample() {
    int index = samplePicker.getSelectedIndex();
    duplicateSample(samples[selectedBank][index]);
  }

  @Override public void deleteSample() {
    dropSample();
  }

  @Override public void replaceSample() {
  ...
  @Override public void renameSample(String s) {
    renameSample(samplePicker.getSelectedIndex(), s);
    updateRomView();
  }

  @Override public void copySample() {
    ArrayList<Integer> indices = samplePicker.getSelectedIndices();
    for (int index = 0; index < clipboard.length; ++index) {
