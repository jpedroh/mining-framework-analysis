...
@Mojo(name = "create-graph") public class JMeterPluginsMojo extends AbstractMojo {
  ...
  @Parameter String inputFilePattern;

  @Parameter List<Graph> graphs;

  @Parameter(defaultValue = "${project.build.directory}/jmeter") File workingDirectory;

  @Component MavenProject mavenProject;

  @Component MavenSession mavenSession;
  ...
  @Override public void execute() throws MojoExecutionException, MojoFailureException {
      ...
      try {
        if (JMETER_CONFIG_ARTIFACT_NAME.equals(artifact.getArtifactId())) {
          ...
          while (entries.hasMoreElements()) {
            ...
            if (!entry.isDirectory() && entry.getName().startsWith("bin") && entry.getName().endsWith(".properties")) {
          ...
          if (isJMeterDependency(artifact)) {
            ...
            if (isJMeterPluginsDependency(artifact)) {
    ...
    for (File source : getInputFiles()) {
      ...
      for (Graph graph : graphs) {
        ...
        try {
          executeMojo(plugin(groupId("org.codehaus.mojo"), artifactId("exec-maven-plugin"), version("1.2.1")), goal("exec"), configuration(element(name("executable"), "java"), element(name("workingDirectory"), binDir.getAbsolutePath()), element(name("arguments"), array(argument("-Dlog_file="), argument("-classpath", libDir.getAbsolutePath() + File.separator + "*" + File.pathSeparator + libExtDir.getAbsolutePath() + File.separator + "*"), argument("kg.apc.cmd.UniversalRunner"), argument("--tool", "Reporter"), argument("--input-jtl", source.getAbsolutePath()), argument("--plugin-type", graph.pluginType), (outputResultFile.endsWith(".csv")) ? argument("--generate-csv") : array(argument("--width", String.valueOf(graph.width), graph.width != null), argument("--height", String.valueOf(graph.height), graph.height != null), argument("--graph-per-row", graph.graphPerRow, !StringUtils.isBlank(graph.graphPerRow)), argument("--granulation", String.valueOf(graph.granulation), graph.granulation != null), argument("--prevent-outliers", graph.preventOutliers, !StringUtils.isBlank(graph.preventOutliers)), argument("--line-weight", graph.lineWeight, !StringUtils.isBlank(graph.lineWeight)), argument("--generate-png")), argument(outputResultFile)))), executionEnvironment(mavenProject, mavenSession, pluginManager));
      ...
      argList.add(element(name("argument"), libDir.getAbsolutePath() + File.separator + "*" + File.pathSeparator + libExtDir.getAbsolutePath() + File.separator + "*"));
...
=======
      try {
        executeMojo(plugin(groupId("org.codehaus.mojo"), artifactId("exec-maven-plugin"), version("1.2.1")), goal("exec"), configuration(element(name("executable"), "java"), element(name("workingDirectory"), binDir.getAbsolutePath()), element(name("arguments"), argList.toArray(new Element[0]))), executionEnvironment(mavenProject, mavenSession, pluginManager));
      } catch (Throwable throwable) {
        throw new RuntimeException(throwable);
      }
...
>>>>>>> /usr/src/app/output/codecentric/jmeter-graph-maven-plugin/9d653ec06f567a6b301d02ba565f3d9023294c3c/src/main/java/de/codecentric/jmeter/JMeterPluginsMojo.java/right.java
    ...
    @Override public String toString() {
      return 
<<<<<<< /usr/src/app/output/codecentric/jmeter-graph-maven-plugin/9d653ec06f567a6b301d02ba565f3d9023294c3c/src/main/java/de/codecentric/jmeter/JMeterPluginsMojo.java/left.java
      "Graph{" + "pluginType=\'" + pluginType + '\'' + ", width=" + width
=======
