...
public class SerializerHandler {
  ...
  private SerializerHandler(int flags) {
    this.flags = flags;
    string = new StringSerializer((flags & XMLRPCClient.FLAGS_NO_STRING_ENCODE) == 0, (flags & XMLRPCClient.FLAGS_NO_STRING_DECODE) == 0);
  }
      ...
      if (TYPE_STRING.equals(type)) {
        ...
        if (TYPE_BOOLEAN.equals(type)) {
          ...
          if (TYPE_DOUBLE.equals(type)) {
            ...
            if (TYPE_INT.equals(type) || TYPE_INT2.equals(type)) {
              s = integer;
            } else {
