...
public class ZencoderClient implements IZencoderClient {
  ...
  private void completeJobInfo(ZencoderJob job, Document response) {
    try {
      NodeList outputs = (NodeList) xPath.evaluate("/api-response/job/outputs", response, XPathConstants.NODESET);
  ...
  @Override public Document createJob(ZencoderJob job) throws ZencoderErrorResponseException {
    Document data;
    try {
      data = job.createXML();
      if (data == null) {
...
>>>>>>> /usr/src/app/output/bitzeche/zencoder-java/fbdc46df52f91095d127c9df2b2223207aa84dc1/src/main/java/de/bitzeche/video/transcoding/zencoder/ZencoderClient.java/right.java
  ...
  public ZencoderNotificationJobState jobProgress(int id) {
    ...
    String url = zencoderAPIBaseUrl + "jobs/" + id + "/progress.xml?api_key=" + zencoderAPIKey;
    ...
    try {
      stateString = (String) xPath.evaluate("/api-response/state", response, XPathConstants.STRING);
  ...
  public boolean cancelJob(int id) {
    String url = zencoderAPIBaseUrl + "jobs/" + id + "/cancel.json?api_key=" + zencoderAPIKey;
    ClientResponse res = sendPutRequest(url);
    int responseStatus = res.getStatus();
    if (responseStatus == 200 || responseStatus == 204) {
      return true;
  ...
  @Deprecated public boolean deleteJob(int id) {
