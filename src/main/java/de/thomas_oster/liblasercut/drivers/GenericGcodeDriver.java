...
public class GenericGcodeDriver extends LaserCutter {
  ...
  public String getPostHttpUploadGcode() {
    return postHttpUploadGcode;
  }

  public void setPostHttpUploadGcode(String postHttpUploadGcode) {
    this.postHttpUploadGcode = postHttpUploadGcode;
  }
    ...
    if (uploadMethod == null || uploadMethod.length() == 0) {
      if (getHost() != null && getHost().length() > 0) {
        ...
        if (getComport() != null && !getComport().equals("")) {
          ...
          if (getHttpUploadUrl() != null && getHttpUploadUrl().length() > 0) {
  ...
  @Override public LaserProperty getLaserPropertyForRaster3dPart() {
    return new FloatPowerSpeedFocusProperty();
  }
    ...
    if (UPLOAD_METHOD_IP.equals(uploadMethod)) {
      ...
      if (UPLOAD_METHOD_SERIAL.equals(uploadMethod)) {
        ...
        if (UPLOAD_METHOD_HTTP.equals(uploadMethod)) {
          ...
          if (UPLOAD_METHOD_FILE.equals(uploadMethod)) {
            ...
            if (UPLOAD_METHOD_OCTOPRINT.equals(uploadMethod)) {
  ...
  @Override public void saveJob(OutputStream fileOutputStream, LaserJob job) throws IllegalJobException, Exception {
    this.currentPower = -1;
    this.currentSpeed = -1;
    checkJob(job);
  ...
  @Override public boolean canEstimateJobDuration() {
    return true;
  }

  @Override public int estimateJobDuration(LaserJob job) {
   ...
   */
  @Override public double getRasterPadding() {
   ...
   */
  @Override public boolean getRasterPaddingAllowOutsideMachineSpace() {
  ...
  private static final String[] SETTINGS_LIST = new String[] { SETTING_UPLOAD_METHOD, SETTING_BAUDRATE, SETTING_BEDWIDTH, SETTING_BEDHEIGHT, SETTING_COMPORT, SETTING_FLIP_X, SETTING_FLIP_Y, SETTING_HOST, SETTING_HTTP_UPLOAD_URL, SETTING_AUTOPLAY, SETTING_POST_HTTP_UPLOAD_GCODE, SETTING_IDENTIFICATION_STRING, SETTING_INIT_DELAY, SETTING_LINEEND, SETTING_MAX_SPEED, SETTING_TRAVEL_SPEED, SETTING_SPINDLE_MAX, SETTING_BLANK_LASER_DURING_RAPIDS, SETTING_PRE_JOB_GCODE, SETTING_POST_JOB_GCODE, SETTING_RESOLUTIONS, SETTING_WAIT_FOR_OK, SETTING_SERIAL_TIMEOUT, SETTING_FILE_EXPORT_PATH, SETTING_USE_BIDIRECTIONAL_RASTERING, SETTING_RASTER_PADDING, SETTING_RASTER_PADDING_ALLOW_OUTSIDE_MACHINE_SPACE, SETTING_API_KEY, SETTING_GCODE_DIGITS, SETTING_SCODE_DIGITS };
        ...
        if (SETTING_BEDWIDTH.equals(attribute)) {
          ...
          if (SETTING_BEDHEIGHT.equals(attribute)) {
            ...
            if (SETTING_COMPORT.equals(attribute)) {
              ...
              if (SETTING_FLIP_X.equals(attribute)) {
                return this.isFlipXaxis();
              } else {
                if (SETTING_FLIP_Y.equals(attribute)) {
                  return this.isFlipYaxis();
                } else {
                  if (SETTING_HTTP_UPLOAD_URL.equals(attribute)) {
                    return this.getHttpUploadUrl();
                  } else {
                    if (SETTING_AUTOPLAY.equals(attribute)) {
                      return this.isAutoPlay();
                    } else {
                      if (SETTING_POST_HTTP_UPLOAD_GCODE.equals(attribute)) {
                        return this.getPostHttpUploadGcode();
                      } else {
                        if (SETTING_IDENTIFICATION_STRING.equals(attribute)) {
                          return this.getIdentificationLine();
                        } else {
                          if (SETTING_INIT_DELAY.equals(attribute)) {
                            return this.getInitDelay();
                          } else {
                            if (SETTING_LINEEND.equals(attribute)) {
                              return this.getLineend();
                            } else {
                              if (SETTING_MAX_SPEED.equals(attribute)) {
                                return this.getMax_speed();
                              } else {
                                if (SETTING_TRAVEL_SPEED.equals(attribute)) {
                                  return this.getTravel_speed();
                                } else {
                                  if (SETTING_PRE_JOB_GCODE.equals(attribute)) {
                                    return this.getPreJobGcode();
                                  } else {
                                    if (SETTING_POST_JOB_GCODE.equals(attribute)) {
                                      return this.getPostJobGcode();
                                    } else {
                                      if (SETTING_RESOLUTIONS.equals(attribute)) {
                                        return this.getSupportedResolutions();
                                      } else {
                                        if (SETTING_WAIT_FOR_OK.equals(attribute)) {
                                          return this.isWaitForOKafterEachLine();
                                        } else {
                                          if (SETTING_SERIAL_TIMEOUT.equals(attribute)) {
                                            return this.getSerialTimeout();
                                          } else {
                                            if (SETTING_BLANK_LASER_DURING_RAPIDS.equals(attribute)) {
                                              return this.getBlankLaserDuringRapids();
                                            } else {
                                              if (SETTING_FILE_EXPORT_PATH.equals(attribute)) {
                                                return this.getExportPath();
                                              } else {
                                                if (SETTING_USE_BIDIRECTIONAL_RASTERING.equals(attribute)) {
                                                  return this.getUseBidirectionalRastering();
                                                } else {
                                                  if (SETTING_SPINDLE_MAX.equals(attribute)) {
                                                    return this.getSpindleMax();
                                                  } else {
                                                    if (SETTING_UPLOAD_METHOD.equals(attribute)) {
                                                      return this.getUploadMethod();
                                                    } else {
                                                      if (SETTING_RASTER_PADDING.equals(attribute)) {
                                                        return this.getRasterPadding();
                                                      } else {
                                                        if (SETTING_RASTER_PADDING_ALLOW_OUTSIDE_MACHINE_SPACE.equals(attribute)) {
                                                          return this.getRasterPaddingAllowOutsideMachineSpace();
                                                        } else {
                                                          if (
<<<<<<< /usr/src/app/output/t-oster/liblasercut/d12d1ac8f333e21225100ad2e06c7d5743dc4523/src/main/java/de/thomas_oster/liblasercut/drivers/GenericGcodeDriver.java/left.java
                                                          SETTING_API_KEY
=======
                                                          SETTING_GCODE_DIGITS
>>>>>>> /usr/src/app/output/t-oster/liblasercut/d12d1ac8f333e21225100ad2e06c7d5743dc4523/src/main/java/de/thomas_oster/liblasercut/drivers/GenericGcodeDriver.java/right.java
                                                          .equals(attribute)) {
                                                            return this.
<<<<<<< /usr/src/app/output/t-oster/liblasercut/d12d1ac8f333e21225100ad2e06c7d5743dc4523/src/main/java/de/thomas_oster/liblasercut/drivers/GenericGcodeDriver.java/left.java
                                                            getApiKey()
...
>>>>>>> /usr/src/app/output/t-oster/liblasercut/d12d1ac8f333e21225100ad2e06c7d5743dc4523/src/main/java/de/thomas_oster/liblasercut/drivers/GenericGcodeDriver.java/right.java
                                                              ...
                                                              if (SETTING_SCODE_DIGITS.equals(attribute)) {
...
>>>>>>> /usr/src/app/output/t-oster/liblasercut/d12d1ac8f333e21225100ad2e06c7d5743dc4523/src/main/java/de/thomas_oster/liblasercut/drivers/GenericGcodeDriver.java/right.java
   ...
   */
  @Override protected void setKeysMissingFromDeserialization() {
    if (this.spindleMax <= 0.0) {
      this.spindleMax = 1.0;
    }
  }

  @Override public GenericGcodeDriver clone() {
