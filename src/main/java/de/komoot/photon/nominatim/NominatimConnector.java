...
@Slf4j public class NominatimConnector {
  ...
  private final RowMapper<NominatimResult> osmlineRowMapper = new RowMapper<NominatimResult>() {
    @Override public NominatimResult mapRow(ResultSet rs, int rownum) throws SQLException {
      ...
      PhotonDoc doc = new PhotonDoc(rs.getLong("place_id"), "W", rs.getLong("osm_id"), "place", "house_number", Collections.<String, String>emptyMap(), (String) null, Collections.<String, String>emptyMap(), Collections.<String, String>emptyMap(), (Envelope) null, rs.getLong("parent_place_id"), 0d, CountryCode.getByCode(rs.getString("country_code")), (Point) null, 0, 30);
  ...
  private final RowMapper<NominatimResult> placeRowMapper = new RowMapper<NominatimResult>() {
    @Override public NominatimResult mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      PhotonDoc doc = new PhotonDoc(rs.getLong("place_id"), rs.getString("osm_type"), rs.getLong("osm_id"), rs.getString("class"), rs.getString("type"), DBUtils.getMap(rs, "name"), (String) null, DBUtils.getMap(rs, "address"), DBUtils.getMap(rs, "extratags"), envelope, rs.getLong("parent_place_id"), importance, CountryCode.getByCode(rs.getString("country_code")), (Point) DBUtils.extractGeometry(rs, "centroid"), rs.getLong("linked_place_id"), rs.getInt("rank_address"));
      doc.setPostcode(rs.getString("postcode"));
      doc.setCountry(getCountryNames(rs.getString("country_code")));
      NominatimResult result = new NominatimResult(doc);
  ...
  List<AddressRow> getAddresses(PhotonDoc doc) {
    RowMapper<AddressRow> rowMapper = new RowMapper<AddressRow>() {
      @Override public AddressRow mapRow(ResultSet rs, int rowNum) throws SQLException {
        return new AddressRow(rs.getLong("place_id"), DBUtils.getMap(rs, "name"), rs.getString("class"), rs.getString("type"), rs.getInt("rank_address"));
      }
    };
  ...
  public void readEntireDatabase(String... countryCodes) {
    ...
    template.query("SELECT " + selectColsPlaceX + " FROM placex " + " WHERE linked_place_id IS NULL AND centroid IS NOT NULL " + andCountryCodeStr + " ORDER BY geometry_sector; ", new RowCallbackHandler() {
      @Override public void processRow(ResultSet rs) throws SQLException {
    ...
    template.query("SELECT place_id, osm_id, parent_place_id, startnumber, endnumber, interpolationtype, postcode, country_code, linegeo " + " FROM location_property_osmline " + whereCountryCodeStr + " ORDER BY geometry_sector; ", new RowCallbackHandler() {
      @Override public void processRow(ResultSet rs) throws SQLException {
  ...
  public List<PhotonDoc> readDocument(long osmId, char osmType) {
    return template.query("SELECT " + selectColsPlaceX + " FROM placex WHERE osm_id = ? AND osm_type = ?; ", new Object[] { osmId, osmType }, new RowMapper<PhotonDoc>() {
      @Override public PhotonDoc mapRow(ResultSet resultSet, int i) throws SQLException {
