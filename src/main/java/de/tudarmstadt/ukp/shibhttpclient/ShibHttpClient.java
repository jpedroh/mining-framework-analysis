...
public class ShibHttpClient implements HttpClient {
  ...
  public ShibHttpClient(String aIdpUrl, String aUsername, String aPassword, boolean aAnyCert) {
    ...
    client = HttpClients.custom().setConnectionManager(connMgr).setRoutePlanner(sdrp).setDefaultRequestConfig(globalRequestConfig).setDefaultCookieStore(cookieStore).addInterceptorFirst(preProcessor).addInterceptorFirst(postProcessor).build();
  ...
  public final class HttpRequestPostprocessor implements HttpResponseInterceptor {
    @Override public void process(HttpResponse res, HttpContext ctx) throws HttpException, IOException {
      ...
      if (res.getFirstHeader(HEADER_CONTENT_TYPE) != null) {
        ContentType contentType = ContentType.parse(res.getFirstHeader(HEADER_CONTENT_TYPE).getValue());
