...
public class ShibHttpClient implements HttpClient {
  ...
  public ShibHttpClient(String aIdpUrl, String aUsername, String aPassword, HttpHost aProxy, boolean anyCert, boolean transparentAuth) {
    ...
    if (anyCert) {
      try {
        ...
        Registry<ConnectionSocketFactory> socketFactoryRegistry = RegistryBuilder.<ConnectionSocketFactory>create().register("http", new PlainConnectionSocketFactory()).register("https", new SSLConnectionSocketFactory(builder.build(), SSLConnectionSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER)).build();
    ...
    HttpClientBuilder customClient = HttpClients.custom().setConnectionManager(connMgr).setDefaultRequestConfig(globalRequestConfig).setDefaultCookieStore(cookieStore).addInterceptorFirst(new HttpRequestPreprocessor());
    if (transparentAuth) {
  ...
  @Override public HttpResponse execute(HttpUriRequest aRequest) throws IOException, ClientProtocolException {
    return client.execute(aRequest);
  }

  @Override public HttpResponse execute(HttpUriRequest aRequest, HttpContext aContext) throws IOException, ClientProtocolException {
    return client.execute(aRequest, aContext);
  }

  @Override public HttpResponse execute(HttpHost aTarget, HttpRequest aRequest) throws IOException, ClientProtocolException {
    return client.execute(aTarget, aRequest);
  }

  @Override public HttpResponse execute(HttpHost aTarget, HttpRequest aRequest, HttpContext aContext) throws IOException, ClientProtocolException {
    return client.execute(aTarget, aRequest, aContext);
  }

  @Override public <T extends java.lang.Object> T execute(HttpUriRequest aRequest, ResponseHandler<? extends T> aResponseHandler) throws IOException, ClientProtocolException {
    return client.execute(aRequest, aResponseHandler);
  }

  @Override public <T extends java.lang.Object> T execute(HttpUriRequest aRequest, ResponseHandler<? extends T> aResponseHandler, HttpContext aContext) throws IOException, ClientProtocolException {
    return client.execute(aRequest, aResponseHandler, aContext);
  }

  @Override public <T extends java.lang.Object> T execute(HttpHost aTarget, HttpRequest aRequest, ResponseHandler<? extends T> aResponseHandler) throws IOException, ClientProtocolException {
    return client.execute(aTarget, aRequest, aResponseHandler);
  }

  @Override public <T extends java.lang.Object> T execute(HttpHost aTarget, HttpRequest aRequest, ResponseHandler<? extends T> aResponseHandler, HttpContext aContext) throws IOException, ClientProtocolException {
  ...
  private final class HttpRequestPreprocessor implements HttpRequestInterceptor {
    @Override public void process(final HttpRequest req, final HttpContext ctx) throws HttpException, IOException {
      req.addHeader(HttpHeaders.ACCEPT, MIME_TYPE_PAOS);
      req.addHeader(HEADER_PAOS, "ver=\"" + SAMLConstants.PAOS_NS + "\";\"" + SAMLConstants.SAML20ECP_NS + "\"");
      HttpRequest r = req;
      ...
      if (!REDIRECTABLE.contains(r.getRequestLine().getMethod()) && r.getParams().isParameterFalse(AUTH_IN_PROGRESS)) {
        log.trace("Unredirectable request [" + r.getRequestLine().getMethod() + "], trying to knock first at " + r.getRequestLine().getUri());
  ...
  public final class HttpRequestPostprocessor implements HttpResponseInterceptor {
    @Override public void process(HttpResponse res, HttpContext ctx) throws HttpException, IOException {
      ...
      Envelope initialLoginSoapResponse = (Envelope) unmarshallMessage(parserPool, paosResponse.getEntity().getContent());
      RelayState relayState = null;
      if (!initialLoginSoapResponse.getHeader().getUnknownXMLObjects(RelayState.DEFAULT_ELEMENT_NAME).isEmpty()) {
        relayState = (RelayState) initialLoginSoapResponse.getHeader().getUnknownXMLObjects(RelayState.DEFAULT_ELEMENT_NAME).get(0);
        relayState.detach();
      ...
      Envelope idpLoginSoapResponse = (Envelope) unmarshallMessage(parserPool, idpLoginResponse.getEntity().getContent());
      EntityUtils.consume(idpLoginResponse.getEntity());
      String assertionConsumerServiceURL = ((Response) idpLoginSoapResponse.getHeader().getUnknownXMLObjects(Response.DEFAULT_ELEMENT_NAME).get(0)).getAssertionConsumerServiceURL();
      log.debug("assertionConsumerServiceURL: " + assertionConsumerServiceURL);
      List<XMLObject> responses = idpLoginSoapResponse.getBody().getUnknownXMLObjects(org.opensaml.saml2.core.Response.DEFAULT_ELEMENT_NAME);
      ...
      if (spLoginResponse.getStatusLine().getStatusCode() == 302 && !REDIRECTABLE.contains(originalRequest.getRequestLine().getMethod())) {
        EntityUtils.consume(spLoginResponse.getEntity());
        throw new NonRepeatableRequestException("Request of type [" + originalRequest.getRequestLine().getMethod() + "] cannot be redirected");
      }
