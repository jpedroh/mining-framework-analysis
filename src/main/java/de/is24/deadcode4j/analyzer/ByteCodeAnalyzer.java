 ...
 * @since 1.3
 */
@SuppressWarnings(value = { "PMD.TooManyStaticImports" }) public abstract class ByteCodeAnalyzer extends AnalyzerAdapter {
  /**
     ...
     * @since 1.4
     */
  @Nonnull @SuppressWarnings(value = { "unchecked" }) protected static Iterable<Annotation> getAnnotations(@Nonnull CtClass clazz, ElementType... elementTypes) {
    List<ElementType> types = asList(elementTypes);
    List<AttributeInfo> attributes = newArrayList();
    if (clazz.getName().endsWith("package-info") && types.contains(ElementType.PACKAGE) || !clazz.getName().endsWith("package-info") && types.contains(ElementType.TYPE)) {
  ...
  @Override public final void doAnalysis(@Nonnull CodeContext codeContext, @Nonnull File file) {
  ...
  /**
     ...
     * @since 1.6
     */
  @Nonnull protected final ClassPool getClassPool(@Nonnull CodeContext codeContext) {
