 ...
 * @since 1.4
 */
@SuppressWarnings(value = { "PMD.TooManyStaticImports" }) public final class HibernateAnnotationsAnalyzer extends ByteCodeAnalyzer implements Analyzer {
  ...
  private void reportTypeUsage(CodeContext codeContext) {
    for (Map.Entry<String, Set<String>> typeUsage : this.typeUsages.entrySet()) {
      ...
      if (classDefiningType != null) {
        ...
        if (codeContext.getAnalyzedCode().getAnalyzedClasses().contains(typeName)) {
          dependee = typeName;
        }
      }
      if (dependee != null) {
