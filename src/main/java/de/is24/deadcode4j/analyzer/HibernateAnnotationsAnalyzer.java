 ...
 * @since 1.4
 */
@SuppressWarnings(value = { "PMD.TooManyStaticImports" }) public final class HibernateAnnotationsAnalyzer extends ByteCodeAnalyzer {
  ...
  @Nonnull private static Iterable<Annotation> getAnnotations(@Nonnull CtClass clazz, @Nonnull final String typeName, ElementType... elementTypes) {
    return filter(getAnnotations(clazz, elementTypes), new Predicate<Annotation>() {
      @Override public boolean apply(@Nullable Annotation annotation) {
        return annotation != null && typeName.equals(annotation.getTypeName());
      }
    });
  }

  @Nullable private static String getStringFrom(@Nonnull Annotation annotation, @Nonnull String memberName) {
    MemberValue memberValue = annotation.getMemberValue(memberName);
    if (memberValue == null) {
      return null;
    }
    checkState(StringMemberValue.class.isInstance(memberValue), "The member [" + memberName + "] is no StringMemberValue!");
    return StringMemberValue.class.cast(memberValue).getValue();
  }

  @Nonnull private static String getMandatoryStringFrom(@Nonnull Annotation annotation, @Nonnull String memberName) {
  ...
  @Nonnull private static Iterable<Annotation> getAnnotationsFrom(@Nonnull Annotation annotation, @Nonnull String memberName) {
    MemberValue memberValue = annotation.getMemberValue(memberName);
    if (memberValue == null) {
      return emptyList();
    }
    checkState(ArrayMemberValue.class.isInstance(memberValue), "The member [" + memberName + "] is no ArrayMemberValue!");
    ...
    return filter(transform(asList(nestedMembers), new Function<MemberValue, Annotation>() {
      @Override public Annotation apply(@Nullable MemberValue memberValue) {
        return memberValue == null ? null : AnnotationMemberValue.class.cast(memberValue).getValue();
      }
    }), notNull());
  }

  @Override protected void analyzeClass(@Nonnull CodeContext codeContext, @Nonnull CtClass clazz) {
  ...
  @Override public void finishAnalysis(@Nonnull CodeContext codeContext) {
  ...
  private void processGenericGenerator(CodeContext codeContext, CtClass clazz, Annotation annotation) {
    String className = clazz.getName();
    Optional<String> resolvedStrategyClass = classPoolAccessorFor(codeContext).resolveClass(getMandatoryStringFrom(annotation, "strategy"));
  ...
  @Nonnull private Map<String, String> getAllGeneratorDefinitions(@Nonnull CodeContext codeContext) {
    IntermediateResultMap<String, String> resultMap = resultMapFrom(codeContext, getClass().getName() + "|generatorDefinitions");
    if (resultMap == null) {
  ...
  @Nonnull private Map<String, String> getAllTypeDefinitions(@Nonnull CodeContext codeContext) {
  ...
  @Nonnull private Map<String, Set<String>> getExistingGeneratorUsages(@Nonnull CodeContext codeContext) {
    IntermediateResultMap<String, Set<String>> resultMap = resultMapFrom(codeContext, getClass().getName() + "|generatorUsages");
    return resultMap != null ? resultMap.getMap() : Collections.<String, Set<String>>emptyMap();
  }

  @Nonnull private Map<String, Set<String>> getExistingTypeUsages(@Nonnull CodeContext codeContext) {
