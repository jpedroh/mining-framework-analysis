 ...
 * @since 1.6
 */
@SuppressWarnings(value = { "PMD.TooManyStaticImports" }) public class TypeErasureAnalyzer extends AnalyzerAdapter {
  @Nonnull private static ClassOrInterfaceType getFirstNode(@Nonnull ClassOrInterfaceType fieldAccessExpr) {
    ClassOrInterfaceType scope = fieldAccessExpr.getScope();
    return scope == null ? fieldAccessExpr : getFirstNode(scope);
  }

  @Nonnull private static String getQualifier(@Nonnull ClassOrInterfaceType classOrInterfaceType) {
  ...
  @Nonnull private static String getTypeName(@Nonnull Node node) {
    ...
    for ( ; ; ) {
      if (TypeDeclaration.class.isInstance(node)) {
        ...
        if (CompilationUnit.class.isInstance(node)) {
  ...
  @Nonnull private static StringBuilder prepend(@Nonnull NameExpr nameExpr, @Nonnull StringBuilder buffy) {
    for ( ; ; ) {
      if (buffy.length() > 0) {
        buffy.insert(0, '.');
      }
  ...
  private void analyzeCompilationUnit(@Nonnull final AnalysisContext analysisContext, @Nonnull final CompilationUnit compilationUnit) {
    compilationUnit.accept(new VoidVisitorAdapter<Void>() {
      ...
      private void resolveTypeReference(ClassOrInterfaceType referencedType) {
        @SuppressWarnings(value = { "unchecked" }) Optional<String> resolvedClass = or(resolveFullyQualifiedClass(), resolveInnerType(), resolveImport(), resolvePackageType(), resolveAsteriskImports(), resolveJavaLangType()).apply(referencedType);
      ...
      private Optional<String> resolveInnerReference(@Nonnull ClassOrInterfaceType firstQualifier, @Nonnull Iterable<? extends BodyDeclaration> bodyDeclarations) {
      ...
      @Nonnull private String resolveReferencedType(@Nonnull ClassOrInterfaceType firstQualifier, @Nonnull TypeDeclaration typeDeclaration) {
...
>>>>>>> /usr/src/app/output/immobilienscout24/deadcode4j/9371e6be648e58efbcd2b581637be4d5b80092c4/src/main/java/de/is24/deadcode4j/analyzer/TypeErasureAnalyzer.java/right.java
}