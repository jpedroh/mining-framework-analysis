...
public class DeadCodeFinder {
  ...
  @Nonnull public DeadCode findDeadCode(@Nonnull Iterable<Module> modules) {
    AnalyzedCode analyzedCode = analyzeCode(modules);
    return computeDeadCode(analyzedCode);
  }

  @Nonnull private AnalyzedCode analyzeCode(@Nonnull Iterable<Module> modules) {
    List<AnalyzedCode> analyzedCode = newArrayList();
    for (Module module : modules) {

<<<<<<< /usr/src/app/output/immobilienscout24/deadcode4j/69518ebbfef40f661ff6ab30ea9ab758a81509c7/src/main/java/de/is24/deadcode4j/DeadCodeFinder.java/left.java
...
>>>>>>> /usr/src/app/output/immobilienscout24/deadcode4j/69518ebbfef40f661ff6ab30ea9ab758a81509c7/src/main/java/de/is24/deadcode4j/DeadCodeFinder.java/right.java
  ...
  @Nonnull private DeadCode computeDeadCode(@Nonnull AnalyzedCode analyzedCode) {
  ...
  private void analyzeRepository(@Nonnull CodeContext codeContext, @Nonnull Repository repository) {
    RepositoryAnalyzer repositoryAnalyzer = new RepositoryAnalyzer(repository.getFileFilter(), this.analyzers, codeContext);
    try {
  ...
  @Nonnull Collection<String> determineDeadClasses(@Nonnull AnalyzedCode analyzedCode) {
  ...
  private static class RepositoryAnalyzer extends DirectoryWalker<Void> {
    ...
    @Override protected void handleEnd(Collection<Void> results) {
      for (Analyzer analyzer : this.analyzers) {
