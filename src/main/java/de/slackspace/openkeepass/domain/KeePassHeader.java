...
public class KeePassHeader {
  ...
  public void checkVersionSupport(byte[] keepassFile) throws IOException {
    ...
    if (signaturePart1 == DATABASE_V2_FILE_SIGNATURE_1_INT && signaturePart2 == DATABASE_V2_FILE_SIGNATURE_2_INT) {
      ...
      if (signaturePart1 == OLD_DATABASE_V1_FILE_SIGNATURE_1_INT && signaturePart2 == OLD_DATABASE_V1_FILE_SIGNATURE_2_INT) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "resource" }) public void read(byte[] keepassFile) throws IOException {
    SafeInputStream inputStream = new SafeInputStream(new BufferedInputStream(new ByteArrayInputStream(keepassFile)));
    inputStream.skipSafe(VERSION_SIGNATURE_LENGTH);
    while (true) {
      try {
  ...
  private static byte[] getEndOfHeader() {
    try {
      ByteArrayOutputStream stream = new ByteArrayOutputStream();
      stream.write(0);
      stream.write(4);
  ...
  private static ByteBuffer wrapInBuffer(byte[] value) {
    ByteBuffer buffer = ByteBuffer.wrap(value);
    buffer.order(ByteOrder.LITTLE_ENDIAN);
    return buffer;
  }

  private static byte[] wrapInBuffer(int value) {
  ...
  private static byte[] wrapInBuffer(long value) {
