...
public abstract class NeedleTestcase {
  ...
  private void initMethodInjection(final Object instance) {
    ...
    for (final Method method : methods) {
      ...
      final InjectionTargetInformationFactory injectionTargetInformationFactory = new InjectionTargetInformationFactory() {
        @Override public InjectionTargetInformation create(final Class<?> parameterType, final int parameterIndex) {
  ...
  private Object[] createArguments(final Class<?>[] parameterTypes, final InjectionTargetInformationFactory injectionTargetInformationFactory) {
    final Object[] arguments = new Object[parameterTypes.length];
    for (int i = 0; i < parameterTypes.length; i++) {
      final InjectionTargetInformation injectionTargetInformation = injectionTargetInformationFactory.create(parameterTypes[i], i);
  ...
  private Object setInstanceIfNotNull(final Field field, final ObjectUnderTest objectUnderTestAnnotation, final Object test) throws Exception {
    ...
    if (instance == null) {
      final Class<?> implementation = objectUnderTestAnnotation.implementation() != Void.class ? objectUnderTestAnnotation.implementation() : field.getType();
  ...
  private Object createInstanceByNoArgConstructor(final Class<?> implementation) throws ObjectUnderTestInstantiationException {
    try {
  ...
  private Object getInstanceByConstructorInjection(final Class<?> implementation) throws ObjectUnderTestInstantiationException {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) public <X extends java.lang.Object> X getInjectedObject(final Object key) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) public <X extends MockProvider> X getMockProvider() {
  ...
  private Entry<Object, Object> inject(final InjectionTargetInformation injectionTargetInformation) {
    for (final Collection<InjectionProvider<?>> collection : configuration.getInjectionProvider()) {
      final Entry<Object, Object> injection = configuration.handleInjectionProvider(collection, injectionTargetInformation);
      if (injection != null) {
        final Object injectionKey = injection.getKey();
        final Object injectionValue = context.getInjectedObject(injectionKey) == null ? injection.getValue() : context.getInjectedObject(injectionKey);
