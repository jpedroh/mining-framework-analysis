...
public abstract class CoapMessage implements Cloneable {
  ...
  protected CoapMessage(CoapMessage coapMessage) throws InvalidHeaderException {
    this(new Header(coapMessage.getHeader()), new OptionList(coapMessage.getOptionList()), coapMessage.getPayload());
  }
  ...
  public byte[] getToken() {
    try {
      return optionList.getOption(TOKEN).get(0).getValue();
  ...
  /**
     ...
     */
  public void setBlockOption(OptionName optionName, long blockNumber, boolean isLastBlock, Blocksize blocksize) throws InvalidOptionException, ToManyOptionsException {
  ...
  public boolean isLastBlock(OptionName optionName) throws InvalidOptionException {
    if (optionName != BLOCK_1 && optionName != BLOCK_2) {
      String msg = "Option " + optionName + " is not a block option and as such does not contain \'isLastBlock\' field.";
      InvalidOptionException e = new InvalidOptionException(optionName, msg);
      log.error(msg, e);
      throw e;
    }
  ...
  public long getBlockNumber(OptionName optionName) throws InvalidOptionException {
    if (optionName != BLOCK_1 && optionName != BLOCK_2) {
      String msg = "Option " + optionName + " is not a block option and as such does not contain \'blocknumber\' field.";
      InvalidOptionException e = new InvalidOptionException(optionName, msg);
      log.error(msg, e);
      throw e;
    }
  ...
  public boolean equals(Object obj) {
    ...
    return this.getHeader().getVersion() == msg.getHeader().getVersion() && this.getHeader().getMsgType().number == msg.getHeader().getMsgType().number && this.getHeader().getCode().number == msg.getHeader().getCode().number && this.getHeader().getMsgID() == msg.getHeader().getMsgID() && optionList.equals(msg.getOptionList()) && payload.equals(msg.getPayload());
  }

  @Override public CoapMessage clone() throws CloneNotSupportedException {
