...
public abstract class Option {
  ...
  public static Collection<Option> createTargetURIOptions(URI uri) throws URISyntaxException, InvalidOptionException {
    ...
    if (scheme == null) {
      String msg = "[Option] URI must be absolute and " + "scheme must be either \"coap\" or \"coaps\" but is " + scheme;
    ...
    if (port > 0 && port != OptionRegistry.COAP_PORT_DEFAULT) {
      ...
      log.debug("URI-Port is either empty or default (= " + OptionRegistry.COAP_PORT_DEFAULT + ") and thus not added as option.");
    }
  ...
  public static Collection<Option> createProxyUriOptions(URI uri) throws InvalidOptionException, URISyntaxException {
    ...
    while (startPos < encodedUri.length) {
      int endPos = Math.min(startPos + OptionRegistry.getMaxLength(OptionName.PROXY_URI), encodedUri.length);
      proxyOptions.add(new StringOption(OptionName.PROXY_URI, Tools.getByteArrayRange(encodedUri, startPos, endPos)));
      startPos = endPos;
    }
    return proxyOptions;
  }
  ...
  public static Option createOption(OptionName optionName, byte[] value) throws InvalidOptionException {
    ...
    switch (OptionRegistry.getOptionType(optionName)) {
      ...
      return new EmptyOption(optionName);
      default:
      throw new InvalidOptionException(optionName, "Type of option number " + optionName.number + " is unknown.");
    }
  }
  ...
  private static Collection<Option> createSeparatedOptions(OptionName optionName, String seperator, String value) throws InvalidOptionException {
    ...
    for (String part : parts) {
      ...
      log.debug("" + optionName + " option instance for part \'" + part + "\' successfully created " + "(but not yet added to the option list!)");
    }
    return options;
  }
