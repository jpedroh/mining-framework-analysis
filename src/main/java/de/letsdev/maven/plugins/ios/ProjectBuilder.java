...
public class ProjectBuilder {
  ...
  public static void build(final Map<String, String> properties, MavenProject mavenProject, final List<FileReplacement> fileReplacements, final List<String> xcodeBuildParameters, final XcodeExportOptions xcodeExportOptions, final StringReplacementConfig stringReplacements, List<String> targetDependencies) throws IOSException, IOException {
    ...
    try {
      ...
      if (Utils.isiOSFramework(mavenProject, properties) || Utils.isMacOSFramework(properties)) {
        ...
        File appTargetPath = new File(targetDirectory + File.separator + properties.get(Utils.PLUGIN_PROPERTIES.CONFIGURATION.toString()) + "-" + Utils.SDK_IPHONE_OS + "/" + properties.get(Utils.PLUGIN_PROPERTIES.TARGET.toString()) + "." + Utils.PLUGIN_SUFFIX.APP);
        File newAppTargetPath = new File(targetDirectory + File.separator + properties.get(Utils.PLUGIN_PROPERTIES.CONFIGURATION.toString()) + "-" + Utils.SDK_IPHONE_OS + "/" + properties.get(Utils.PLUGIN_PROPERTIES.APP_NAME.toString()) + "." + Utils.PLUGIN_SUFFIX.APP);
        File ipaBasePath = new File(targetDirectory + File.separator + properties.get(Utils.PLUGIN_PROPERTIES.CONFIGURATION.toString()) + "-" + Utils.SDK_IPHONE_OS);
        ...
        File dsymTargetPath = new File(targetDirectory + File.separator + properties.get(Utils.PLUGIN_PROPERTIES.CONFIGURATION.toString()) + "-" + Utils.SDK_IPHONE_OS + "/" + properties.get(Utils.PLUGIN_PROPERTIES.TARGET.toString()) + "." + Utils.PLUGIN_SUFFIX.APP_DSYM);
        File newDsymTargetPath = new File(targetDirectory + File.separator + properties.get(Utils.PLUGIN_PROPERTIES.CONFIGURATION.toString()) + "-" + Utils.SDK_IPHONE_OS + "/" + properties.get(Utils.PLUGIN_PROPERTIES.APP_NAME.toString()) + "." + Utils.PLUGIN_SUFFIX.APP_DSYM);
  ...
  private static void removeSimulatorArchitectures(File rootDirectory) {
    try {
      ...
      outputStream.close();
      ProcessBuilder processBuilder = new ProcessBuilder("sh", tempFile.getAbsoluteFile().toString(), rootDirectory.getAbsolutePath());
  ...
  private static void codeSignBeforeXcode6(Map<String, String> properties, File workDirectory, File newAppTargetPath, File ipaTargetPath, File ipaTmpDir) throws IOSException {
    ProcessBuilder processBuilderCodeSign = new ProcessBuilder("xcrun", "--no-cache", "-sdk", properties.get(Utils.PLUGIN_PROPERTIES.SDK.toString()), "PackageApplication", "-v", newAppTargetPath.toString(), "-o", ipaTargetPath.toString(), "--sign", properties.get(Utils.PLUGIN_PROPERTIES.CODE_SIGN_IDENTITY.toString()));
    processBuilderCodeSign.directory(workDirectory);
    processBuilderCodeSign.environment().put("TMPDIR", ipaTmpDir.getAbsolutePath());
  ...
  private static void buildXcodeProject(MavenProject mavenProject, Map<String, String> properties, File workDirectory, File targetDirectory, String projectName, boolean shouldUseIphoneSimulatorSDK, List<String> xcodeBuildParameters) throws IOSException {
    ...
    if (Utils.shouldCodeSign(mavenProject, properties) && properties.containsKey(Utils.PLUGIN_PROPERTIES.PROVISIONING_PROFILE_UUID.toString()) && !properties.get(Utils.PLUGIN_PROPERTIES.PROVISIONING_PROFILE_UUID.toString()).equals("")) {
      buildParameters.add("PROVISIONING_PROFILE=\"" + properties.get(Utils.PLUGIN_PROPERTIES.PROVISIONING_PROFILE_UUID.toString()) + "\"");
    }
    if (Utils.shouldCodeSign(mavenProject, properties) && properties.containsKey(Utils.PLUGIN_PROPERTIES.PROVISIONING_PROFILE_SPECIFIER.toString()) && !properties.get(Utils.PLUGIN_PROPERTIES.PROVISIONING_PROFILE_SPECIFIER.toString()).equals("")) {
    ...
    if (!Utils.shouldBuildXCArchive(mavenProject, properties) && target != null) {
    ...
    if (properties.containsKey(Utils.PLUGIN_PROPERTIES.DERIVED_DATA_PATH.toString())) {
      buildParameters.add("-derivedDataPath " + properties.get(Utils.PLUGIN_PROPERTIES.DERIVED_DATA_PATH.toString()));
    } else {
      buildParameters.add("CONFIGURATION_BUILD_DIR=\"" + targetDirectory.getAbsolutePath() + "/" + properties.get(Utils.PLUGIN_PROPERTIES.CONFIGURATION.toString()) + "-" + Utils.SDK_IPHONE_OS + "\"");
    }
  ...
  private static void mergeFrameworkProducts(File targetWorkDirectoryIphone, File targetWorkDirectoryIphoneSimulator, String appName, String frameworkName) throws IOSException {
    ...
    final String iphoneSimulatorFrameworkProductPath = targetWorkDirectoryIphoneSimulator.toString() + "/" + frameworkName;
    ...
    ProcessBuilder processBuilder = new ProcessBuilder("sh", tempFile.getAbsoluteFile().toString(), iphoneosFrameworkProductPath, iphoneSimulatorFrameworkProductPath, mergedFrameworkPath, appName);
  ...
  private static void exportProductArchive(File archiveFile, File targetPath, String productName) throws IOSException {
    try {
      ...
      outputStream.close();
      ProcessBuilder processBuilder = new ProcessBuilder("sh", tempFile.getAbsoluteFile().toString(), archiveFile.toString(), productPath, productName, targetPath.toString(), productTargetPath);
  ...
  private static void writeDeployPlistFile(MavenProject mavenProject, File targetDirectory, String deployPlistName, final Map<String, String> properties) throws IOSException {
    try {
      ...
      outputStream.close();
      ProcessBuilder processBuilder = new ProcessBuilder("sh", tempFile.getAbsoluteFile().toString(), deployPlistName, ipaLocation, iconLocation, displayName, bundleIdentifier, bundleVersion, buildNumber);
