...
public class LdapLoginSource implements ILoginSource, Runnable {
  ...
  @SuppressWarnings(value = { "rawtypes" }) private void read() throws Exception {
    Exception ex = null;
    synchronized (fLock) {
  ...
  @SuppressWarnings(value = { "rawtypes" }) private void readData(NamingEnumeration ldapContents) throws Exception {
  ...
  @Override public List<String> getUserPasswords() {
    if (!fInitialized) {
      try {
        read();
      } catch (Exception e) {
