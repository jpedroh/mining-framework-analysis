...
public class CellHSSFImpl implements Cell {
  ...
  public String getValue() {
    if (theCell.getCellType() == HSSFCell.CELL_TYPE_BLANK) {
      ...
      if (theCell.getCellType() == HSSFCell.CELL_TYPE_BOOLEAN) {
        return Boolean.toString(theCell.getBooleanCellValue());
      } else {
        if (theCell.getCellType() == HSSFCell.CELL_TYPE_ERROR) {
          return "<ERROR?>";
        } else {
          if (theCell.getCellType() == HSSFCell.CELL_TYPE_FORMULA) {
            return theCell.getCellFormula();
          } else {
            if (theCell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
              return Double.toString(theCell.getNumericCellValue());
            } else {
              if (theCell.getCellType() == HSSFCell.CELL_TYPE_STRING) {
  ...
  @Override public Color getBackgroundFill() {
    HSSFCellStyle cellStyle = theCell.getCellStyle();
    if (cellStyle == null) {
      return Color.WHITE;
    }
  ...
  @Override public void setBackgroundFill(Color colour) {
  ...
  public int getAlignment() {
    ...
    if (hssfAlignment == HSSFCellStyle.ALIGN_LEFT) {
      ...
      if (hssfAlignment == HSSFCellStyle.ALIGN_CENTER) {
        return SwingConstants.CENTER;
      } else {
        if (hssfAlignment == HSSFCellStyle.ALIGN_RIGHT) {
