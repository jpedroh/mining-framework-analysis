 ...
 *
 */
@ThreadSafe @Immutable public class PodamFactoryImpl implements PodamFactory {
  ...
  /** Application logger */
  private static final Logger LOG = LoggerFactory.getLogger(PodamFactoryImpl.class.getName());
  ...
  /**
	 * {@inheritDoc}
	 */
  @Override public <T extends java.lang.Object> T manufacturePojo(Class<T> pojoClass) {
  ...
  /**
	 * {@inheritDoc}
	 */
  @Override public <T extends java.lang.Object> T manufacturePojo(Class<T> pojoClass, Type... genericTypeArgs) {
  ...
  /**
	 * {@inheritDoc}
	 */
  @Override public DataProviderStrategy getStrategy() {
    return strategy;
  }
  ...
  /**
	 ...
	 */
  private Type[] fillTypeArgMap(final Map<String, Type> typeArgsMap, final Class<?> pojoClass, final Type[] genericTypeArgs) {
    final TypeVariable<?>[] typeParameters = pojoClass.getTypeParameters();
    if (typeParameters.length > genericTypeArgs.length) {
      String msg = pojoClass.getCanonicalName() + " is missing generic type arguments, expected " + typeParameters.length + " found " + genericTypeArgs.length + ". Returning null";
    ...
    if (constructors.length == 0 || Modifier.isAbstract(clazz.getModifiers())) {
      ...
      try {
        ...
        if (genericTypeArgsExtra != null) {
          LOG.warn(String.format("Lost %d generic type arguments", genericTypeArgsExtra.length));
        }
      } catch (IllegalStateException e) {
        LOG.error("An error occurred while filling the type argument in the map", e);
        return null;
      }
      ...
      for (Method candidateConstructor : declaredMethods) {
        if (!Modifier.isStatic(candidateConstructor.getModifiers()) || !candidateConstructor.getReturnType().equals(clazz) || retValue != null) {
          continue;
        }
        ...
        if (parameterTypes.length == 0) {
          ...
          for (Type paramType : parameterTypes) {
            ...
            Class<?> parameterType = resolveGenericParameter(paramType, typeArgsMap, methodGenericTypeArgs);
            ...
            if (Collection.class.isAssignableFrom(parameterType)) {
              ...
              int nbrElements = strategy.getNumberOfCollectionElements(elementType);
              ...
              for (int i = 0; i < nbrElements; i++) {
                Object attributeValue = manufactureAttributeValue(clazz, pojos, elementType, annotations, attributeName);
              ...
              if (Map.class.isAssignableFrom(parameterType)) {
                ...
                int nbrElements = strategy.getNumberOfCollectionElements(valueClass);
                for (Annotation annotation : annotations) {
                  if (annotation.annotationType().equals(PodamCollection.class)) {
                    PodamCollection ann = (PodamCollection) annotation;
                    nbrElements = ann.nbrElements();
                ...
                for (int i = 0; i < nbrElements; i++) {
                  Object keyValue = manufactureAttributeValue(clazz, pojos, keyClass, annotations, attributeName);
                  Object elementValue = manufactureAttributeValue(clazz, pojos, valueClass, annotations, attributeName);
                  mapType.put(keyValue, elementValue);
                }
                parameterValues[idx] = mapType;
              } else {
                parameterValues[idx] = manufactureAttributeValue(clazz, pojos, parameterType, annotations, attributeName, typeArgsMap, genericTypeArgs);
              }
            }
            idx++;
          }
        ...
        try {
          ...
          LOG.info("PODAM could not create an instance for constructor: " + candidateConstructor + ". Will try another one...", t);
        }
      }
    } else {
      ...
      for (Constructor<?> constructor : constructors) {
        try {
          Object[] constructorArgs = getParameterValuesForConstructor(constructor, pojoClass, pojos, genericTypeArgs);
          retValue = constructor.newInstance(constructorArgs);
          LOG.info("For class: " + clazz.getName() + " a valid constructor: " + constructor + " was found. PODAM will use it to create an instance.");
          break;
        } catch (Exception t) {
          LOG.info("Couldn\'t create attribute with constructor: " + constructor + ". Will check if other constructors are available", t);
        }
      }
    }
    if (retValue == null) {
      LOG.warn("For class: {} PODAM could not possibly create a value." + " This attribute will be returned as null.", clazz);
    }
    return retValue;
  }
  ...
  private Class<?> resolveGenericParameter(Type paramType, Map<String, Type> typeArgsMap, AtomicReference<Type[]> methodGenericTypeArgs) {
    ...
    if (paramType instanceof TypeVariable<?>) {
      ...
      if (type != null) {
        parameterType = resolveGenericParameter(type, typeArgsMap, methodGenericTypeArgs);
      } else {
        LOG.warn("Unrecognized type {}. Will use Object intead", typeVariable);
      ...
      if (paramType instanceof ParameterizedType) {
        ...
        if (paramType instanceof WildcardType) {
          ...
          if (bounds != null && bounds.length > 0) {
            ...
            parameterType = resolveGenericParameter(bounds[0], typeArgsMap, methodGenericTypeArgs);
          } else {
            LOG.warn("Unrecognized type" + wType.toString() + ". Will use Object intead");
            parameterType = Object.class;
          }
        } else {
          if (paramType instanceof Class) {
            ...
            LOG.warn("Unrecognized type {}. Will use Object intead", paramType.getClass().getSimpleName());
            parameterType = Object.class;
          }
        }
      }
    ...
    for (Annotation annotation : annotations) {
      if (PodamByteValue.class.isAssignableFrom(annotation.getClass())) {
        ...
        if (null != numValueStr && !"".equals(numValueStr)) {
          ...
          retValue = strategy.getByteInRange(minValue, maxValue, attributeMetadata);
        }
        break;
      }
    }
    ...
    for (Annotation annotation : annotations) {
      if (PodamShortValue.class.isAssignableFrom(annotation.getClass())) {
        ...
        if (null != numValueStr && !"".equals(numValueStr)) {
          ...
          retValue = strategy.getShortInRange(minValue, maxValue, attributeMetadata);
        }
        break;
      }
    }
    ...
    for (Annotation annotation : annotations) {
      if (PodamCharValue.class.isAssignableFrom(annotation.getClass())) {
        ...
        if (charValue != ' ') {
          ...
          retValue = strategy.getCharacterInRange(minValue, maxValue, attributeMetadata);
        }
        break;
      }
    }
    ...
    for (Annotation annotation : annotations) {
      if (PodamIntValue.class.isAssignableFrom(annotation.getClass())) {
        ...
        if (null != numValueStr && !"".equals(numValueStr)) {
          ...
          retValue = strategy.getIntegerInRange(minValue, maxValue, attributeMetadata);
        }
        break;
      }
    }
    ...
    for (Annotation annotation : annotations) {
      if (PodamFloatValue.class.isAssignableFrom(annotation.getClass())) {
        ...
        if (null != numValueStr && !"".equals(numValueStr)) {
          ...
          retValue = strategy.getFloatInRange(minValue, maxValue, attributeMetadata);
        }
        break;
      }
    }
    ...
    for (Annotation annotation : annotations) {
      if (PodamDoubleValue.class.isAssignableFrom(annotation.getClass())) {
        ...
        if (null != numValueStr && !"".equals(numValueStr)) {
          ...
          retValue = strategy.getDoubleInRange(minValue, maxValue, attributeMetadata);
        }
        break;
      }
    }
    ...
    for (Annotation annotation : annotations) {
      if (PodamLongValue.class.isAssignableFrom(annotation.getClass())) {
        ...
        if (null != numValueStr && !"".equals(numValueStr)) {
          ...
          retValue = strategy.getLongInRange(minValue, maxValue, attributeMetadata);
        }
        break;
      }
    }
  ...
  private Object resolveWrapperValue(Class<?> candidateWrapperClass, List<Annotation> annotations, AttributeMetadata attributeMetadata) {
    ...
    if (candidateWrapperClass.equals(Integer.class)) {
      ...
      if (candidateWrapperClass.equals(Long.class)) {
        if (!annotations.isEmpty()) {
          retValue = getLongValueWithinRange(annotations, attributeMetadata);
        }
        if (retValue == null) {
        ...
        if (candidateWrapperClass.equals(Float.class)) {
          if (!annotations.isEmpty()) {
            retValue = getFloatValueWithinRange(annotations, attributeMetadata);
          }
          if (retValue == null) {
          ...
          if (candidateWrapperClass.equals(Double.class)) {
            if (!annotations.isEmpty()) {
              retValue = getDoubleValueWithinRange(annotations, attributeMetadata);
            }
            if (retValue == null) {
            ...
            if (candidateWrapperClass.equals(Boolean.class)) {
              if (!annotations.isEmpty()) {
                retValue = getBooleanValueForAnnotation(annotations);
              }
              if (retValue == null) {
              ...
              if (candidateWrapperClass.equals(Byte.class)) {
                if (!annotations.isEmpty()) {
                  retValue = getByteValueWithinRange(annotations, attributeMetadata);
                }
                if (retValue == null) {
                ...
                if (candidateWrapperClass.equals(Short.class)) {
                  ...
                  if (candidateWrapperClass.equals(Character.class)) {
                    if (!annotations.isEmpty()) {
                      retValue = getCharacterValueWithinRange(annotations, attributeMetadata);
                    }
                    if (retValue == null) {
  ...
  /**
	 ...
	 *             If it was not possible to create a class from a string
	 */
  @SuppressWarnings(value = { UNCHECKED_STR, RAWTYPES_STR }) private <T extends java.lang.Object> T resolvePojoWithoutSetters(Class<T> pojoClass, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    T retValue = null;
    Constructor<?>[] constructors = pojoClass.getConstructors();
    if (constructors.length == 0) {
      retValue = (T) createNewInstanceForClassWithoutConstructors(pojoClass, pojos, pojoClass);
    } else {
      strategy.sort(constructors);
      for (Constructor<?> constructor : constructors) {
        Object[] parameterValues = getParameterValuesForConstructor(constructor, pojoClass, pojos, genericTypeArgs);
        try {
          ...
          if (retValue instanceof Collection && ((Collection) retValue).isEmpty()) {
            LOG.info("We could create an instance with constructor: " + constructor + ", but collection is empty" + ". Will try with another one.");
          } else {
            if (retValue instanceof Map && ((Map) retValue).isEmpty()) {
              LOG.info("We could create an instance with constructor: " + constructor + ", but map is empty" + ". Will try with another one.");
            } else {
          ...
          LOG.info("We couldn\'t create an instance for pojo: " + pojoClass + " for constructor: " + constructor + ". Will try with another one.", t);
        }
      }
      if (retValue == null) {
        LOG.warn("For class: {} PODAM could not possibly create a value." + " This attribute will be returned as null.", pojoClass);
      }
    }
    return retValue;
  }
  ...
  /**
	 ...
	 *             setting its state
	 */
  @SuppressWarnings(value = { UNCHECKED_STR }) private <T extends java.lang.Object> T manufacturePojoInternal(Class<T> pojoClass, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    T retValue = null;
    ...
    if (pojoClass.isInterface() || Modifier.isAbstract(pojoClass.getModifiers())) {
      ...
      if (!specificClass.equals(pojoClass)) {
        ...
        if (Modifier.isAbstract(pojoClass.getModifiers())) {
          return (T) createNewInstanceForClassWithoutConstructors(pojoClass, pojos, pojoClass, genericTypeArgs);
        } else {
          LOG.warn("Cannot instantiate an interface {}." + " Returning null.", pojoClass);
        }
      }
    }
    ...
    try {
      ...
      if (constructors == null || constructors.length == 0) {
        LOG.warn("No public constructors were found. " + "We\'ll look for a default, non-public constructor. ");
        ...
        retValue = resolvePojoWithoutSetters(pojoClass, pojos, genericTypeArgs);
      }
    } catch (SecurityException e) {
      throw new PodamMockeryException("Security exception while applying introspection.", e);
    } catch (NoSuchMethodException e1) {
      LOG.info("No default (public or non-public) constructors were found. " + "Also no other public constructors were found. " + "Your last hope is that we find a non-public, non-default constructor.", e1);
      ...
      Object[] parameterValuesForConstructor = getParameterValuesForConstructor(constructors[0], pojoClass, pojos, genericTypeArgs);
    ...
    for (Method setter : classInfo.getClassSetters()) {
      ...
      if (null != attributeStrategyAnnotation) {
        ...
        if (setter.getGenericParameterTypes()[0] instanceof ParameterizedType) {
          ...
          if (setter.getGenericParameterTypes()[0] instanceof TypeVariable) {
        ...
        setterArg = manufactureAttributeValue(pojoClass, pojos, attributeType, pojoAttributeAnnotations, attributeName, typeArgsMap, typeArguments);
      }
      if (setterArg != null) {
        int modifiers = setter.getModifiers();
        if (!Modifier.isPublic(modifiers)) {
          LOG.warn("The setter: {} is not accessible.Setting it to accessible. " + "However this is a security hack and your code should really adhere to Javabean standards.", setter.getName());
          setter.setAccessible(true);
        }
        setter.invoke(retValue, setterArg);
      } else {
        LOG.warn("Couldn\'t find a suitable value for attribute: {}" + ". This POJO attribute will be left to null.", attributeType);
      }
    }
    return retValue;
  }
  ...
  private Object manufactureAttributeValue(Class<?> pojoClass, Map<Class<?>, Integer> pojos, Class<?> attributeType, List<Annotation> annotations, String attributeName, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    return manufactureAttributeValue(pojoClass, pojos, attributeType, annotations, attributeName, nullTypeArgsMap, genericTypeArgs);
  }
  ...
  /**
	 ...
	 *
	 */
  @SuppressWarnings(value = { RAWTYPES_STR }) private Object manufactureAttributeValue(Class<?> pojoClass, Map<Class<?>, Integer> pojos, Class<?> attributeType, List<Annotation> annotations, String attributeName, Map<String, Type> typeArgsMap, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    if (realAttributeType.isPrimitive()) {
      ...
      if (isWrapper(realAttributeType)) {
        attributeValue = resolveWrapperValue(realAttributeType, annotations, attributeMetadata);
      } else {
        if (realAttributeType.equals(String.class)) {
          attributeValue = resolveStringValue(annotations, attributeMetadata);
        } else {
          if (realAttributeType.getName().startsWith("[")) {
            attributeValue = resolveArrayElementValue(realAttributeType, pojos, annotations, pojoClass, attributeName, typeArgsMap);
          } else {
            if (Collection.class.isAssignableFrom(realAttributeType)) {
              attributeValue = resolveCollectionValueWhenCollectionIsPojoAttribute(pojoClass, pojos, realAttributeType, attributeName, annotations, typeArgsMap, genericTypeArgs);
            } else {
              if (Map.class.isAssignableFrom(realAttributeType)) {
                attributeValue = resolveMapValueWhenMapIsPojoAttribute(pojoClass, pojos, realAttributeType, attributeName, annotations, typeArgsMap, genericTypeArgs);
              } else {
                if (realAttributeType.isEnum()) {
                  ...
                  if (depth <= strategy.getMaxDepth(pojoClass)) {
                    ...
                    if (realAttributeType.getName().startsWith("java.") || realAttributeType.getName().startsWith("javax.")) {
                      attributeValue = createNewInstanceForClassWithoutConstructors(pojoClass, pojos, realAttributeType, genericTypeArgs);
                    } else {
                      attributeValue = this.manufacturePojoInternal(realAttributeType, pojos, genericTypeArgs);
                    }
                    pojos.put(realAttributeType, depth);
                  } else {
                    LOG.warn("Loop in {} production detected. Returning null.", realAttributeType);
                    attributeValue = null;
                  }
                }
              }
  ...
  private String resolveStringValue(List<Annotation> annotations, AttributeMetadata attributeMetadata) throws InstantiationException, IllegalAccessException {
    ...
    if (annotations == null || annotations.isEmpty()) {
      ...
      for (Annotation annotation : annotations) {
        if (!PodamStringValue.class.isAssignableFrom(annotation.getClass())) {
          continue;
        }
  ...
  private Field getField(Class<?> pojoClass, String attributeName) {
    ...
    while (clazz != null) {
      try {
        ...
        LOG.info("A field could not be found for attribute name: " + attributeName, e);
        clazz = clazz.getSuperclass();
      }
    }
    return field;
  ...
  private PodamStrategyValue containsAttributeStrategyAnnotation(List<Annotation> annotations) {
    ...
    for (Annotation annotation : annotations) {
      if (PodamStrategyValue.class.isAssignableFrom(annotation.getClass())) {
  ...
  private boolean isWrapper(Class<?> candidateWrapperClass) {
    return candidateWrapperClass.equals(Byte.class) ? true : candidateWrapperClass.equals(Boolean.class) ? true : candidateWrapperClass.equals(Character.class) ? true : candidateWrapperClass.equals(Short.class) ? true : candidateWrapperClass.equals(Integer.class) ? true : candidateWrapperClass.equals(Long.class) ? true : candidateWrapperClass.equals(Float.class) ? true : candidateWrapperClass.equals(Double.class) ? true : false;
  }
  ...
  private List<Annotation> retrieveFieldAnnotations(Class<?> clazz, Method setter) {
    ...
    while (workClass != null) {
      try {
        ...
        LOG.info("A field for " + attributeName + " could not be found", e);
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { UNCHECKED_STR }) private Collection<? super Object> resolveCollectionValueWhenCollectionIsPojoAttribute(Class<?> pojoClass, Map<Class<?>, Integer> pojos, Class<?> collectionType, String attributeName, List<Annotation> annotations, Map<String, Type> typeArgsMap, Type... genericTypeArgs) {
    ...
    try {
      ...
      LOG.info("The name was empty or we couldn\'t call an empty constructor. Creating an empty collection.", e);
      retValue = resolveCollectionType(collectionType);
    }
    try {
      ...
      if (genericTypeArgs == null || genericTypeArgs.length == 0) {
        ...
        typeClass = resolveGenericParameter(actualTypeArgument, typeArgsMap, elementGenericTypeArgs);
      }
      fillCollection(pojoClass, pojos, attributeName, annotations, retValue, typeClass, elementGenericTypeArgs.get());
    } catch (SecurityException e) {
    ...
    for (int i = 0; i < nbrElements; i++) {
      if (null != elementStrategy && ObjectStrategy.class.isAssignableFrom(elementStrategy.getClass()) && Object.class.equals(collectionElementType)) {
        ...
        if (null != elementStrategy && !ObjectStrategy.class.isAssignableFrom(elementStrategy.getClass())) {
          ...
          Object strategyValue = returnAttributeDataStrategyValue(collectionElementType, elementStrategy);
          collection.add(strategyValue);
        } else {
          collection.add(manufactureAttributeValue(pojoClass, pojos, collectionElementType, annotations, attributeName, genericTypeArgs));
        }
      }
    }
  }
  ...
  private Map<? super Object, ? super Object> resolveMapValueWhenMapIsPojoAttribute(Class<?> pojoClass, Map<Class<?>, Integer> pojos, Class<?> attributeType, String attributeName, List<Annotation> annotations, Map<String, Type> typeArgsMap, Type... genericTypeArgs) {
    ...
    try {
      ...
      field.setAccessible(true);
      @SuppressWarnings(value = { UNCHECKED_STR }) Map<? super Object, ? super Object> coll = (Map<? super Object, ? super Object>) field.get(newInstance);
      if (null != coll) {
        retValue = coll;
      } else {
      ...
      LOG.info("The name was empty of we couldn\'t call an empty constructor. Creating an empty Map", e);
      retValue = resolveMapType(attributeType);
    }
    try {
      Class<?> keyClass = null;
      Class<?> elementClass = null;
      AtomicReference<Type[]> keyGenericTypeArgs = new AtomicReference<Type[]>(new Type[] {  });
      AtomicReference<Type[]> elementGenericTypeArgs = new AtomicReference<Type[]>(new Type[] {  });
      if (genericTypeArgs == null || genericTypeArgs.length == 0) {
  ...
  private void fillMap(MapArguments mapArguments) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    int nbrElements = strategy.getNumberOfCollectionElements(mapArguments.getElementClass());
    if (null != collectionAnnotation) {
      nbrElements = collectionAnnotation.nbrElements();
  ...
  private Object getMapKeyOrElementValue(MapKeyOrElementsArguments keyOrElementsArguments) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    if (null != keyOrElementsArguments.getElementStrategy() && ObjectStrategy.class.isAssignableFrom(keyOrElementsArguments.getElementStrategy().getClass()) && Object.class.equals(keyOrElementsArguments.getKeyOrValueType())) {
      ...
      if (null != keyOrElementsArguments.getElementStrategy() && !ObjectStrategy.class.isAssignableFrom(keyOrElementsArguments.getElementStrategy().getClass())) {
        LOG.debug("Map key or value will be filled using the following strategy: " + keyOrElementsArguments.getElementStrategy());
        retValue = returnAttributeDataStrategyValue(keyOrElementsArguments.getKeyOrValueType(), keyOrElementsArguments.getElementStrategy());
      } else {
        retValue = manufactureAttributeValue(keyOrElementsArguments.getPojoClass(), keyOrElementsArguments.getPojos(), keyOrElementsArguments.getKeyOrValueType(), keyOrElementsArguments.getAnnotations(), keyOrElementsArguments.getAttributeName(), keyOrElementsArguments.getGenericTypeArgs());
      }
    }
    return retValue;
  }
  ...
  private Object resolveArrayElementValue(Class<?> attributeType, Map<Class<?>, Integer> pojos, List<Annotation> annotations, Class<?> pojoClass, String attributeName, Map<String, Type> typeArgsMap) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    if (null != attributeName) {
      try {
        final Type genericType = pojoClass.getDeclaredField(attributeName).getGenericType();
        if (genericType instanceof GenericArrayType) {
          final Type type = ((GenericArrayType) genericType).getGenericComponentType();
          if (type instanceof TypeVariable<?>) {
            final Type typeVarType = typeArgsMap.get(((TypeVariable<?>) type).getName());
            componentType = resolveGenericParameter(typeVarType, typeArgsMap, genericTypeArgs);
          }
        }
      } catch (NoSuchFieldException e) {
        LOG.info("Cannot get the declared field type for field " + attributeName + " of class " + pojoClass.getName(), e);
      }
    }
    PodamCollection collectionAnnotation = null;
    AttributeStrategy<?> elementStrategy = null;
    ...
    for (int i = 0; i < nbrElements; i++) {
      if (null != elementStrategy && ObjectStrategy.class.isAssignableFrom(collectionAnnotation.collectionElementStrategy()) && Object.class.equals(componentType)) {
        ...
        if (null != elementStrategy && !ObjectStrategy.class.isAssignableFrom(collectionAnnotation.collectionElementStrategy())) {
          LOG.debug("Array elements will be filled using the following strategy: " + elementStrategy);
          arrayElement = returnAttributeDataStrategyValue(componentType, elementStrategy);
        } else {
          arrayElement = manufactureAttributeValue(pojoClass, pojos, componentType, annotations, attributeName, typeArgsMap, genericTypeArgs.get());
        }
      }
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { RAWTYPES_STR, UNCHECKED_STR }) private Collection<? super Object> resolveCollectionType(Class<?> collectionType) {
    Collection<? super Object> retValue = null;
  ...
  /**
	 ...
	 *
	 */
  @SuppressWarnings(value = { UNCHECKED_STR, RAWTYPES_STR }) private Map<? super Object, ? super Object> resolveMapType(Class<?> attributeType) {
    Map<? super Object, ? super Object> retValue = null;
    if (SortedMap.class.isAssignableFrom(attributeType)) {
      retValue = new TreeMap();
    } else {
      if (ConcurrentMap.class.isAssignableFrom(attributeType)) {
  ...
  private Object[] getParameterValuesForConstructor(Constructor<?> constructor, Class<?> pojoClass, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    genericTypeArgsExtra = fillTypeArgMap(typeArgsMap, pojoClass, genericTypeArgs);
    ...
    for (Class<?> parameterType : parameterTypes) {
      ...
      if (Collection.class.isAssignableFrom(parameterType)) {
        ...
        AtomicReference<Type[]> collectionGenericTypeArgs = new AtomicReference<Type[]>(new Type[] {  });
        if (type instanceof ParameterizedType) {
        ...
        fillCollection(pojoClass, pojos, attributeName, annotations, collection, collectionElementType, genericTypeArgsAll);
        ...
        if (Map.class.isAssignableFrom(parameterType)) {
          ...
          parameterValues[idx] = manufactureAttributeValue(pojoClass, pojos, parameterType, annotations, attributeName, genericTypeArgs);
        }
      }
      idx++;
    }
  ...
  private Object returnAttributeDataStrategyValue(Class<?> attributeType, AttributeStrategy<?> attributeStrategy) throws InstantiationException, IllegalAccessException {
    ...
    try {
      attributeStrategyMethod = attributeStrategy.getClass().getMethod(PodamConstants.PODAM_ATTRIBUTE_STRATEGY_METHOD_NAME, new Class<?>[] {  });
