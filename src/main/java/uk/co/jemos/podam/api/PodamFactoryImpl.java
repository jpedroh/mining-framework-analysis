...
@ThreadSafe @Immutable public class PodamFactoryImpl implements PodamFactory {
  ...
  /**
	 * {@inheritDoc}
	 */
  @Override public <T extends java.lang.Object> T manufacturePojoWithFullData(Class<T> pojoClass, Type... genericTypeArgs) {
    ...
    if (getStrategy() instanceof AbstractRandomDataProviderStrategy) {
      ...
      throw new IllegalStateException("manufacturePojoWithFullData can" + " only be used with strategies derived from" + " AbstractRandomDataProviderStrategy");
    }
    ConstructorAdaptiveComparator constructorComparator;
    if (sortingStrategy.getConstructorComparator() instanceof ConstructorAdaptiveComparator) {
      constructorComparator = (ConstructorAdaptiveComparator) sortingStrategy.getConstructorComparator();
    } else {
      throw new IllegalStateException("manufacturePojoWithFullData can" + " only be used with constructor comparators derived from" + " ConstructorAdaptiveComparator");
    }
  ...
  private Type[] fillTypeArgMap(final Map<String, Type> typeArgsMap, final Class<?> pojoClass, final Type[] genericTypeArgs) {
    ...
    if (typeParameters.size() > genericTypeArgs.length) {
      String msg = pojoClass.getCanonicalName() + " is missing generic type arguments, expected " + typeParameters + " found " + Arrays.toString(genericTypeArgs);
  ...
  private Object instantiatePojoWithoutConstructors(Class<?> pojoClass, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    for (Method candidateConstructor : declaredMethods) {
      ...
      try {
        ...
        LOG.debug("PODAM could not create an instance for constructor: " + candidateConstructor + ". Will try another one...", t);
      }
    }
    if (retValue == null) {
    ...
    if (retValue == null) {
      LOG.warn("For class {} PODAM could not possibly create" + " a value. It will be returned as null.", pojoClass);
    }
    return retValue;
  }
  ...
  private Class<?> resolveGenericParameter(Type paramType, Map<String, Type> typeArgsMap, AtomicReference<Type[]> methodGenericTypeArgs) {
    ...
    if (parameterType == null) {
      LOG.warn("Unrecognized type {}. Will use Object instead", paramType);
              ...
              if (primitiveClass.equals(byte.class)) {
                ...
                if (primitiveClass.equals(short.class)) {
                  ...
                  if (primitiveClass.equals(char.class)) {
                    ...
                    throw new IllegalArgumentException(String.format("%s is unsupported primitive type", primitiveClass));
                  }
                }
              }
            }
  ...
  /**
	 ...
	 *             If it was not possible to create a class from a string
	 */
  @SuppressWarnings(value = { UNCHECKED_STR }) private <T extends java.lang.Object> T instantiatePojo(Class<T> pojoClass, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws SecurityException {
    T retValue = null;
    Constructor<?>[] constructors = pojoClass.getConstructors();
    if (constructors.length == 0 || Modifier.isAbstract(pojoClass.getModifiers())) {
      try {
        retValue = (T) instantiatePojoWithoutConstructors(pojoClass, pojos, genericTypeArgs);
      } catch (Exception e) {
        LOG.debug("We couldn\'t create an instance for pojo: " + pojoClass + " with factory methods, will " + " try non-public constructors.", e);
      }
      if (retValue == null) {
    ...
    if (retValue == null && constructors.length > 0) {
      ...
      for (Constructor<?> constructor : constructors) {
        try {
          ...
          LOG.debug("We couldn\'t create an instance for pojo: {} with" + " constructor: {}. Will try with another one.", pojoClass, constructor, e);
        }
      }
    }
    if (retValue == null) {
  ...
  @SuppressWarnings(value = { UNCHECKED_STR }) private <T extends java.lang.Object> T manufacturePojoInternal(Class<T> pojoClass, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    if (pojoClass.isPrimitive()) {
      ...
      String noName = null;
      return (T) resolvePrimitiveValue(pojoClass, annotations, new AttributeMetadata(noName, pojoClass, annotations, pojoClass));
    }
    ...
    ClassInfo classInfo = PodamUtils.getClassInfo(pojoClass, strategy.getExcludedAnnotations());
    ...
    for (Method setter : classInfo.getClassSetters()) {
      ...
      if (parameterTypes.length != 1) {
        LOG.warn("Skipping setter with non-single arguments {}", setter);
        continue;
      }
      ...
      if (null != attributeStrategyAnnotation) {
        ...
        if (genericTypeArgsExtra != null) {
          LOG.warn("Lost generic type arguments {}", Arrays.toString(genericTypeArgsExtra));
        }
        ...
        AtomicReference<Type[]> typeGenericTypeArgs = new AtomicReference<Type[]>(NO_TYPES);
        for (int i = 0; i < typeArguments.length; i++) {
          if (typeArguments[i] instanceof TypeVariable) {
            Class<?> resolvedType = resolveGenericParameter(typeArguments[i], typeArgsMap, typeGenericTypeArgs);
            if (!Collection.class.isAssignableFrom(resolvedType) && !Map.class.isAssignableFrom(resolvedType)) {
              typeArguments[i] = resolvedType;
            }
          }
        }
        setterArg = manufactureAttributeValue(retValue, pojos, attributeType, setter.getGenericParameterTypes()[0], pojoAttributeAnnotations, attributeName, typeArgsMap, typeArguments);
        if (null == setterArg) {
      ...
      if (setterArg != null) {
        try {
          ...
          LOG.warn("{} is not accessible. Setting it to accessible." + " However this is a security hack and your code" + " should really adhere to JavaBeans standards.", setter.toString());
  ...
  /**
	 ...
	 *
	 */
  private Object manufactureParameterValue(Map<Class<?>, Integer> pojos, Class<?> parameterType, Type genericType, List<Annotation> annotations, Map<String, Type> typeArgsMap, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    String attributeName = null;
    return manufactureAttributeValue(Object.class, pojos, parameterType, genericType, annotations, attributeName, typeArgsMap, genericTypeArgs);
  }
  ...
  private Object manufactureAttributeValue(Object pojo, Map<Class<?>, Integer> pojos, Class<?> attributeType, Type genericAttributeType, List<Annotation> annotations, String attributeName, Map<String, Type> typeArgsMap, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
            ...
            if (Collection.class.isAssignableFrom(realAttributeType)) {
              ...
              if (Map.class.isAssignableFrom(realAttributeType)) {
                ...
                if (realAttributeType.isEnum()) {
                  ...
                  if (Type.class.isAssignableFrom(realAttributeType)) {
                    if (genericTypeArgs.length > 0 && genericTypeArgs[0] != null) {
                      ...
                      LOG.error("{} is missing generic type argument, supplied {} {}", genericAttributeType, typeArgsMap, Arrays.toString(genericTypeArgs));
                    }
                  }
                }
              }
    ...
    if (attributeValue == null) {
      ...
      if (depth <= strategy.getMaxDepth(pojo.getClass())) {
        ...
        LOG.warn("Loop in {} production detected.", realAttributeType);
  ...
  /**
	 ...
	 */
  private <T extends java.lang.Object> T getDefaultFieldValue(Object pojo, String attributeName) {
    T retValue = null;
    try {
      Field field = getField(pojo.getClass(), attributeName);
      if (field != null) {
        field.setAccessible(true);
        @SuppressWarnings(value = { UNCHECKED_STR }) T t = (T) field.get(pojo);
        retValue = t;
      } else {
        LOG.info("The field {}[{}] didn\'t exist.", pojo.getClass(), attributeName);
      }
    } catch (Exception e) {
      LOG.warn("We couldn\'t get default value for {}[{}]", pojo.getClass(), attributeName, e);
    }
    return retValue;
  }
  ...
  private Field getField(Class<?> pojoClass, String attributeName) {
    ...
    if (field == null) {
      LOG.warn("A field could not be found for attribute \'{}[{}]\'", pojoClass, attributeName);
    }
    return field;
  }
  ...
  /**
	 ...
	 */
  private Collection<? super Object> resolveCollectionValueWhenCollectionIsPojoAttribute(Object pojo, Map<Class<?>, Integer> pojos, Class<?> collectionType, String attributeName, List<Annotation> annotations, Map<String, Type> typeArgsMap, Type... genericTypeArgs) {
    ...
    try {
      ...
      fillCollection(pojos, annotations, retValue, typeClass, elementGenericTypeArgs.get());
    } catch (SecurityException e) {
  ...
  private void fillCollection(Collection<? super Object> collection, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    Type[] genericTypeArgsExtra = fillTypeArgMap(typeArgsMap, collectionClass, genericTypeArgs);
    ...
    while (typeParams.length < 1) {
      ...
      collectionClass = resolveGenericParameter(type, typeArgsMap, elementGenericTypeArgs);
      typeParams = elementGenericTypeArgs.get();
    }
    Class<?> elementTypeClass = resolveGenericParameter(typeParams[0], typeArgsMap, elementGenericTypeArgs);
    fillCollection(pojos, Arrays.asList(annotations), collection, elementTypeClass, elementGenericTypeArgs.get());
  }
  ...
  /**
	 ...
	 *
	 */
  private void fillCollection(Map<Class<?>, Integer> pojos, List<Annotation> annotations, Collection<? super Object> collection, Class<?> collectionElementType, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    if (null != collectionAnnotation) {
      ...
      nbrElements = strategy.getNumberOfCollectionElements(collectionElementType);
    }
    try {
      ...
      for (int i = collection.size(); i < nbrElements; i++) {
        ...
        if (null != elementStrategy && ObjectStrategy.class.isAssignableFrom(elementStrategy.getClass()) && Object.class.equals(collectionElementType)) {
          ...
          if (null != elementStrategy && !ObjectStrategy.class.isAssignableFrom(elementStrategy.getClass())) {
            ...
            element = returnAttributeDataStrategyValue(collectionElementType, elementStrategy);
          } else {
            Map<String, Type> nullTypeArgsMap = new HashMap<String, Type>();
            element = manufactureParameterValue(pojos, collectionElementType, collectionElementType, annotations, nullTypeArgsMap, genericTypeArgs);
          }
        }
  ...
  /**
	 ...
	 */
  private Map<? super Object, ? super Object> resolveMapValueWhenMapIsPojoAttribute(Object pojo, Map<Class<?>, Integer> pojos, Class<?> attributeType, String attributeName, List<Annotation> annotations, Map<String, Type> typeArgsMap, Type... genericTypeArgs) {
  ...
  private void fillMap(Map<? super Object, ? super Object> map, Map<Class<?>, Integer> pojos, Type... genericTypeArgs) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    AtomicReference<Type[]> keyGenericTypeArgs = new AtomicReference<Type[]>(NO_TYPES);
    Class<?> keyClass = resolveGenericParameter(typeParams[0], typeArgsMap, keyGenericTypeArgs);
    Class<?> elementClass = resolveGenericParameter(typeParams[1], typeArgsMap, elementGenericTypeArgs);
    MapArguments mapArguments = new MapArguments();
    mapArguments.setPojos(pojos);
  ...
  private void fillMap(MapArguments mapArguments) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    if (null != collectionAnnotation) {
      ...
      nbrElements = strategy.getNumberOfCollectionElements(mapArguments.getElementClass());
    }
    ...
    try {
      ...
      for (int i = map.size(); i < nbrElements; i++) {
        ...
        @SuppressWarnings(value = { UNCHECKED_STR }) Class<? extends Map<?, ?>> mapType = (Class<? extends Map<?, ?>>) mapArguments.getMapToBeFilled().getClass();
  ...
  private Object getMapKeyOrElementValue(MapKeyOrElementsArguments keyOrElementsArguments) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    ...
    if (null != keyOrElementsArguments.getElementStrategy() && ObjectStrategy.class.isAssignableFrom(keyOrElementsArguments.getElementStrategy().getClass()) && Object.class.equals(keyOrElementsArguments.getKeyOrValueType())) {
      ...
      if (null != keyOrElementsArguments.getElementStrategy() && !ObjectStrategy.class.isAssignableFrom(keyOrElementsArguments.getElementStrategy().getClass())) {
        ...
        Map<String, Type> nullTypeArgsMap = new HashMap<String, Type>();
        retValue = manufactureParameterValue(keyOrElementsArguments.getPojos(), keyOrElementsArguments.getKeyOrValueType(), keyOrElementsArguments.getKeyOrValueType(), keyOrElementsArguments.getAnnotations(), nullTypeArgsMap, keyOrElementsArguments.getGenericTypeArgs());
      }
    }
    return retValue;
  }
  ...
  /**
	 ...
	 *             If it was not possible to create a class from a string
	 */
  private Object resolveArrayElementValue(Class<?> attributeType, Type genericType, Map<Class<?>, Integer> pojos, List<Annotation> annotations, Object pojo, String attributeName, Map<String, Type> typeArgsMap) throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
    Class<?> componentType = null;
    AtomicReference<Type[]> genericTypeArgs = new AtomicReference<Type[]>(NO_TYPES);
    if (genericType instanceof GenericArrayType) {
      ...
      if (genericComponentType instanceof TypeVariable) {
        ...
        componentType = resolveGenericParameter(resolvedType, typeArgsMap, genericTypeArgs);
      }
    }
    ...
    for (int i = 0; i < nbrElements; i++) {
      if (null != elementStrategy && ObjectStrategy.class.isAssignableFrom(collectionAnnotation.collectionElementStrategy()) && Object.class.equals(componentType)) {
        ...
        if (null != elementStrategy && !ObjectStrategy.class.isAssignableFrom(collectionAnnotation.collectionElementStrategy())) {
          ...
          arrayElement = manufactureAttributeValue(pojo, pojos, componentType, genericType, annotations, attributeName, typeArgsMap, genericTypeArgs.get());
        }
      }
  ...
  /**
	 ...
	 */
  private Collection<? super Object> resolveCollectionType(Class<?> collectionType, Collection<? super Object> defaultValue) {
    Collection<? super Object> retValue = null;
  ...
  /**
	 ...
	 *
	 */
  private Map<? super Object, ? super Object> resolveMapType(Class<?> mapType, Map<? super Object, ? super Object> defaultValue) {
    Map<? super Object, ? super Object> retValue = null;
    if (null != defaultValue && (defaultValue.getClass().getModifiers() & Modifier.PRIVATE) == 0) {
      retValue = defaultValue;
    } else {
    ...
    if (Collection.class.isAssignableFrom(parameterType)) {
      ...
      if (collection != null) {
        ...
        if (genericType instanceof ParameterizedType) {
          ...
          LOG.warn("Collection parameter {} type is non-generic." + "We will assume a Collection<Object> for you.", genericType);
      ...
      if (Map.class.isAssignableFrom(parameterType)) {
        ...
        if (map != null) {
          ...
          if (genericType instanceof ParameterizedType) {
            ...
            LOG.warn("Map parameter {} type is non-generic." + "We will assume a Map<Object,Object> for you.", genericType);
            keyClass = Object.class;
            elementClass = Object.class;
          }
