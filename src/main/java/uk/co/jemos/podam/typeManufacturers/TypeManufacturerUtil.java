...
public abstract class TypeManufacturerUtil {
    ...
    while (iter.hasNext()) {
      ...
      if (Proxy.isProxyClass(annotationClass)) {
        ...
        if (interfaces.length == 1) {
          @SuppressWarnings(value = { "unchecked" }) Class<? extends Annotation> tmp = (Class<? extends Annotation>) interfaces[0];
          annotationClass = tmp;
        }
      }
  ...
  /**
     ...
     */
  public static Type[] fillTypeArgMap(final Map<String, Type> typeArgsMap, final Class<?> pojoClass, final Type[] genericTypeArgs) {
  ...
  public static Type[] mergeActualAndSuppliedGenericTypes(Class<?> attributeType, Type genericAttributeType, Type[] suppliedTypes, Map<String, Type> typeArgsMap) {
    ...
    if (genericAttributeType instanceof ParameterizedType) {
      ...
      if (genericAttributeType instanceof WildcardType) {
    ...
    for (int i = 0; i < actualTypes.length; i++) {
      ...
      if (actualTypes[i] instanceof TypeVariable) {
        ...
        if (actualTypes[i] instanceof WildcardType) {
          AtomicReference<Type[]> methodGenericTypeArgs = new AtomicReference<Type[]>(PodamConstants.NO_TYPES);
      ...
      if ((type == null) && (genericTypes != null)) {
        if (genericTypes[i] instanceof Class) {
          ...
          if (genericTypes[i] instanceof WildcardType) {
            AtomicReference<Type[]> methodGenericTypeArgs = new AtomicReference<Type[]>(PodamConstants.NO_TYPES);
            type = resolveGenericParameter(genericTypes[i], typeArgsMap, methodGenericTypeArgs);
          } else {
  ...
  public static Object returnAttributeDataStrategyValue(Class<?> attributeType, AttributeStrategy<?> attributeStrategy) throws IllegalArgumentException {
    ...
    if (retValue != null) {
      Class<?> desiredType = attributeType.isPrimitive() ? PodamUtils.primitiveToBoxedType(attributeType) : attributeType;
