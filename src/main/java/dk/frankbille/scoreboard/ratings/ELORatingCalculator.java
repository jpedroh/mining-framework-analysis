...
public class ELORatingCalculator implements RatingCalculator {
  ...
  @Override public void setGames(List<Game> games) {
    players.clear();
    gamePlayers.clear();
  ...
  @Override public ELOPlayerRating getPlayerRating(long playerId) {
  ...
  private void setRatingChange(GameTeam team, double ratingChange) {
    ...
    for (Player player : team.getTeam().getPlayers()) {
      ...
      addGamePlayerRating(team.getGame().getId(), player.getId(), playerRating.getRating(), ratingChange);
  ...
  @Override public GamePlayerRating getGamePlayerRating(long gameId, long playerId) {
    return gamePlayers.get(gameId + "-" + playerId);
  }

  @Override public GameRating getGameRatingChange(Long gameId) {
