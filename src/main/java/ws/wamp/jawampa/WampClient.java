...
public class WampClient {
  ...
  WampClient(URI routerUri, String realm, WampRoles[] roles, boolean useStrictUriValidation, boolean closeClientOnErrors, WampClientChannelFactory channelFactory, int nrReconnects, int reconnectInterval, String authId, List<ClientSideAuthentication> authMethods) {
  ...
  /**
     ...
     * @return An observable that can be used to subscribe on the topic.
     */
  public <T extends java.lang.Object> Observable<T> makeSubscription(final String topic, SubscriptionFlags flags, final Class<T> eventClass) {
      ...
      @Override public void call(final Subscriber<? super PubSubData> subscriber) {
        try {
          if (flags == SubscriptionFlags.Exact) {
            ...
            if (flags == SubscriptionFlags.Prefix) {
              UriValidator.validatePrefix(topic, useStrictUriValidation);
            } else {
              if (flags == SubscriptionFlags.Wildcard) {
