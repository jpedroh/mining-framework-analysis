...
public class Linear extends AbstractLinear {
  ...
  public Linear() {
    ...
    VALUE_BACKGROUND_TRACK_FRACTIONS = new float[] { 0.0f, 0.48f, 0.49f, 1.0f };
    ...
    VALUE_BORDER_FRACTIONS = new float[] { 0.0f, 0.48f, 0.49f, 1.0f };
    ...
    VALUE_FOREGROUND_FRACTIONS = new float[] { 0.0f, 1.0f };
    VALUE_FOREGROUND_COLORS = new Color[] { new Color(1.0f, 1.0f, 1.0f, 0.7f), new Color(1.0f, 1.0f, 1.0f, 0.05f) };
  ...
  private void drawValue(final Graphics2D G2, final int WIDTH, final int HEIGHT, final double PIXEL_SCALE) {
    ...
    final double MAX_POS_OF_BAR;
    final double MIN_POS_OF_BAR;
    final double FULL_SIZE;
    final double VALUE_SIZE;
    final double START_OF_VALUE;
    final double SIZE_FACTOR;
    if (VERTICAL) {
      MAX_POS_OF_BAR = HEIGHT * 0.12864077669902912;
      MIN_POS_OF_BAR = HEIGHT * 0.8567961165048543;
      ...
      MAX_POS_OF_BAR = WIDTH * 0.8567961165048543;
      MIN_POS_OF_BAR = WIDTH * 0.14285714285714285;
    ...
    final Color[] VALUE_BACKGROUND_TRACK_COLORS = { UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.0470588235f), UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.1450980392f), UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.1490196078f), UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.0470588235f) };
    ...
    final Color[] VALUE_BORDER_COLORS = { UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.2980392157f), UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.6862745098f), UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.6980392157f), UTIL.setAlpha(getBackgroundColor().LABEL_COLOR, 0.4f) };
    ...
    final float[] VALUE_BACKGROUND_FRACTIONS = { 0.0f, 0.99f, 1.0f };
    Color[] valueBackgroundColors;
    if (getValueColor() != ColorDef.CUSTOM) {
      valueBackgroundColors = new Color[] { getValueColor().MEDIUM, getValueColor().LIGHT, getValueColor().LIGHT };
    } else {
      valueBackgroundColors = new Color[] { getCustomValueColorObject().MEDIUM, getCustomValueColorObject().LIGHT, getCustomValueColorObject().LIGHT };
    }
    if (isSectionsVisible()) {
      for (Section section : getSections()) {
        if (Double.compare(getValue(), section.getStart()) >= 0 && Double.compare(getValue(), section.getStop()) <= 0) {
          valueBackgroundColors = new Color[] { isTransparentSectionsEnabled() ? section.getTransparentColor() : section.getColor(), isTransparentSectionsEnabled() ? section.getTransparentColor() : section.getColor(), isTransparentSectionsEnabled() ? section.getTransparentColor() : section.getColor() };
  ...
  @Override public boolean isTickmarksVisible() {
    return getModel().isTickmarksVisible();
  }

  @Override public void setTickmarksVisible(final boolean TICKMARKS_VISIBLE) {
  ...
  @Override public Paint createCustomLcdBackgroundPaint(final Color[] LCD_COLORS) {
    ...
    final Color[] FOREGROUND_COLORS = { LCD_COLORS[0], LCD_COLORS[1], LCD_COLORS[2], LCD_COLORS[3], LCD_COLORS[4] };
