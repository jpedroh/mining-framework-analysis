...
public class Neo4jGraphDatabase extends GraphDatabaseBase<Iterator<Node>, Iterator<Relationship>, Node, Relationship> {
  ...
  @Override public void open() {
    ...
    try (Transaction tx = beginUnforcedTransaction()) {
      try {
  ...
  @Override public void massiveModeLoading(File dataPath) {
    ...
    neo4jMassiveInsertion.createGraph(dataPath, 0);
  }
  ...
  @Override public void shortestPath(Node n1, Integer i) {
  ...
  @Override public Set<Integer> getNodesFromNodeCommunity(int nodeCommunity) {
    ...
    try (Transaction tx = beginUnforcedTransaction()) {
      try {
        ResourceIterable<Node> iter = neo4jGraph.findNodesByLabelAndProperty(NODE_LABEL, NODE_COMMUNITY, nodeCommunity);
        for (Node n : iter) {
          String nodeIdString = (String) (n.getProperty(NODE_ID));
          nodes.add(Integer.valueOf(nodeIdString));
        }
  ...
  @Override public double getEdgesInsideCommunity(int nodeCommunity, int communityNodes) {
    double edges = 0;
    try (Transaction tx = beginUnforcedTransaction()) {
      try {
        ResourceIterable<Node> nodes = neo4jGraph.findNodesByLabelAndProperty(NODE_LABEL, NODE_COMMUNITY, nodeCommunity);
  ...
  @Override public void moveNode(int nodeCommunity, int toCommunity) {
    try (Transaction tx = beginUnforcedTransaction()) {
      try {
        ResourceIterable<Node> fromIter = neo4jGraph.findNodesByLabelAndProperty(NODE_LABEL, NODE_COMMUNITY, nodeCommunity);
  ...
  @Override public int getCommunity(int nodeCommunity) {
    ...
    try (Transaction tx = beginUnforcedTransaction()) {
      try {
        Node node = neo4jGraph.findNodesByLabelAndProperty(NODE_LABEL, NODE_COMMUNITY, nodeCommunity).iterator().next();
  ...
  @Override public Iterator<Node> getVertexIterator() {
    return GlobalGraphOperations.at(neo4jGraph).getAllNodes().iterator();
  }

  @Override public Iterator<Relationship> getNeighborsOfVertex(Node v) {
    return v.getRelationships(Neo4jGraphDatabase.RelTypes.SIMILAR, Direction.BOTH).iterator();
  }

  @Override public void cleanupVertexIterator(Iterator<Node> it) {
  }

  @Override public Node getOtherVertexFromEdge(Relationship r, Node n) {
    return r.getOtherNode(n);
  }

  @Override public Iterator<Relationship> getAllEdges() {
    return GlobalGraphOperations.at(neo4jGraph).getAllRelationships().iterator();
  }

  @Override public Node getSrcVertexFromEdge(Relationship edge) {
    return edge.getStartNode();
  }

  @Override public Node getDestVertexFromEdge(Relationship edge) {
    return edge.getEndNode();
  }

  @Override public boolean edgeIteratorHasNext(Iterator<Relationship> it) {
    return it.hasNext();
  }

  @Override public Relationship nextEdge(Iterator<Relationship> it) {
    return it.next();
  }

  @Override public void cleanupEdgeIterator(Iterator<Relationship> it) {
  }

  @Override public boolean vertexIteratorHasNext(Iterator<Node> it) {
    return it.hasNext();
  }

  @Override public Node nextVertex(Iterator<Node> it) {
    return it.next();
  }

  @Override public Node getVertex(Integer i) {
    return neo4jGraph.findNodesByLabelAndProperty(Neo4jGraphDatabase.NODE_LABEL, NODE_ID, i).iterator().next();
  }
}