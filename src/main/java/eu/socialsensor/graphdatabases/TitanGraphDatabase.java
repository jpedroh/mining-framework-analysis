...
public class TitanGraphDatabase extends GraphDatabaseBase<Iterator<Vertex>, Iterator<Edge>, Vertex, Edge> {
  ...
  public TitanGraphDatabase(GraphDatabaseType type, BenchmarkConfiguration config, File dbStorageDirectory) {
    ...
    if (!GraphDatabaseType.TITAN_FLAVORS.contains(type)) {
      throw new IllegalArgumentException(String.format("The graph database %s is not a Titan database.", type == null ? "null" : type.name()));
    }
  }
  ...
  private static final Configuration generateBaseTitanConfiguration(GraphDatabaseType type, File dbPath, boolean batchLoading, BenchmarkConfiguration bench) {
    if (!GraphDatabaseType.TITAN_FLAVORS.contains(type)) {
      throw new IllegalArgumentException("must provide a Titan database type but got " + (type == null ? "null" : type.name()));
    }
    if (dbPath == null) {
  ...
  private static final TitanGraph buildTitanGraph(GraphDatabaseType type, File dbPath, BenchmarkConfiguration bench, boolean batchLoading) {
    ...
    if (GraphDatabaseType.TITAN_CASSANDRA == type) {
      ...
      if (GraphDatabaseType.TITAN_CASSANDRA_EMBEDDED == type) {
        storage.addProperty("cassandra-config-dir", "configuration/cassandra.yaml");
        storage.addProperty("transactions", Boolean.toString(batchLoading));
      } else {
        if (GraphDatabaseType.TITAN_DYNAMODB == type) {
          ...
          if (GraphDatabaseType.TITAN_DYNAMODB == type && bench.getDynamodbEndpoint() != null && !bench.getDynamodbEndpoint().isEmpty()) {
          ...
          if (bench.getDynamodbCredentialsFqClassName() != null && !bench.getDynamodbCredentialsFqClassName().isEmpty()) {
          ...
          if (bench.getDynamodbCredentialsCtorArguments() != null) {
            credentials.addProperty(Constants.DYNAMODB_CREDENTIALS_CONSTRUCTOR_ARGS.getName(), bench.getDynamodbCredentialsCtorArguments());
          }
    ...
    if (type == GraphDatabaseType.TITAN_DYNAMODB && config.getDynamodbPrecreateTables()) {
      ...
      for (String store : Constants.REQUIRED_BACKEND_STORES) {
        ...
        if (BackendDataModel.MULTI == config.getDynamodbDataModel()) {
          requests.add(DynamoDBStore.createTableRequest(tableName, rcu, wcu));
        } else {
          if (BackendDataModel.SINGLE == config.getDynamodbDataModel()) {
            requests.add(DynamoDBSingleRowStore.createTableRequest(tableName, rcu, wcu));
          }
        }
      }
      final AmazonDynamoDB client = new AmazonDynamoDBClient(Client.createAWSCredentialsProvider(config.getDynamodbCredentialsFqClassName(), config.getDynamodbCredentialsCtorArguments() == null ? null : config.getDynamodbCredentialsCtorArguments().split(",")));
  ...
  @Override public void createGraphForSingleLoad() {
    open();
    createSchema();
  }

  @Override public void createGraphForMassiveLoad() {
    open(true);
    createSchema();
    batchGraph = new BatchGraph<TitanGraph>(titanGraph, VertexIDType.NUMBER, 100000);
    batchGraph.setVertexIdKey(NODE_ID);
    batchGraph.setLoadingFromScratch(true);
  }

  @Override public void massiveModeLoading(File dataPath) {
    Insertion titanMassiveInsertion = new TitanMassiveInsertion(this.batchGraph, type);
    titanMassiveInsertion.createGraph(dataPath, 0);
  }

  @Override public void singleModeLoading(File dataPath, File resultsPath, int scenarioNumber) {
  ...
  @Override public void delete() {
    titanGraph = buildTitanGraph(type, dbStorageDirectory, config, false);
    try {
      titanGraph.shutdown();
    } catch (IOError e) {
      System.err.println("Failed to shutdown titan graph: " + e.getMessage());
  ...
  @Override public void shutdownMassiveGraph() {
    if (titanGraph == null) {
      return;
    }
    try {
  ...
  @Override public void shortestPath(final Vertex fromNode, Integer node) {
    final Vertex v2 = titanGraph.getVertices(NODE_ID, node).iterator().next();
    @SuppressWarnings(value = { "rawtypes" }) final GremlinPipeline<String, List> pathPipe = new GremlinPipeline<String, List>(fromNode).as(SIMILAR).out(SIMILAR).loop(SIMILAR, new PipeFunction<LoopBundle<Vertex>, Boolean>() {
  ...
  @Override public Set<Integer> getNeighborsIds(int nodeId) {
  ...
  @Override public double getNodeWeight(int nodeId) {
    Vertex vertex = titanGraph.getVertices(NODE_ID, nodeId).iterator().next();
  ...
  @Override public void initCommunityProperty() {
  ...
  @Override public Set<Integer> getCommunitiesConnectedToNodeCommunities(int nodeCommunities) {
  ...
  @Override public Set<Integer> getNodesFromCommunity(int community) {
  ...
  @Override public Set<Integer> getNodesFromNodeCommunity(int nodeCommunity) {
    Set<Integer> nodes = new HashSet<Integer>();
  ...
  @Override public double getEdgesInsideCommunity(int vertexCommunity, int communityVertices) {
    double edges = 0;
  ...
  @Override public double getCommunityWeight(int community) {
  ...
  @Override public double getNodeCommunityWeight(int nodeCommunity) {
  ...
  @Override public void moveNode(int nodeCommunity, int toCommunity) {
  ...
  @Override public double getGraphWeightSum() {
    Iterable<Edge> edges = titanGraph.getEdges();
    return (double) Iterables.size(edges);
  }

  @Override public int reInitializeCommunities() {
  ...
  @Override public int getCommunity(int nodeCommunity) {
  ...
  @Override public int getCommunityFromNode(int nodeId) {
    Vertex vertex = titanGraph.getVertices(NODE_ID, nodeId).iterator().next();
    return vertex.getProperty(COMMUNITY);
  }

  @Override public int getCommunitySize(int community) {
  ...
  @Override public Map<Integer, List<Integer>> mapCommunities(int numberOfCommunities) {
  ...
  @Override public boolean nodeExists(int nodeId) {
    Iterable<Vertex> iter = titanGraph.getVertices(NODE_ID, nodeId);
    return iter.iterator().hasNext();
  }

  @Override public Iterator<Vertex> getVertexIterator() {
    return titanGraph.getVertices().iterator();
  }

  @Override public Iterator<Edge> getNeighborsOfVertex(Vertex v) {
    return v.getEdges(Direction.BOTH, SIMILAR).iterator();
  }

  @Override public void cleanupVertexIterator(Iterator<Vertex> it) {
    return;
  }

  @Override public Vertex getOtherVertexFromEdge(Edge edge, Vertex oneVertex) {
    return edge.getVertex(Direction.IN).equals(oneVertex) ? edge.getVertex(Direction.OUT) : edge.getVertex(Direction.IN);
  }

  @Override public Iterator<Edge> getAllEdges() {
    return titanGraph.getEdges().iterator();
  }

  @Override public Vertex getSrcVertexFromEdge(Edge edge) {
    return edge.getVertex(Direction.IN);
  }

  @Override public Vertex getDestVertexFromEdge(Edge edge) {
    return edge.getVertex(Direction.OUT);
  }

  @Override public boolean edgeIteratorHasNext(Iterator<Edge> it) {
    return it.hasNext();
  }

  @Override public Edge nextEdge(Iterator<Edge> it) {
    return it.next();
  }

  @Override public void cleanupEdgeIterator(Iterator<Edge> it) {
  }

  @Override public boolean vertexIteratorHasNext(Iterator<Vertex> it) {
    return it.hasNext();
  }

  @Override public Vertex nextVertex(Iterator<Vertex> it) {
    return it.next();
  }

  @Override public Vertex getVertex(Integer i) {
