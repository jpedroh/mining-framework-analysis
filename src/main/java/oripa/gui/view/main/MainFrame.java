...
public class MainFrame extends JFrame implements ComponentListener, WindowListener {
  ...
  private final JMenuItem menuItemRepeatCopy = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.ARRAY_COPY_ID));
  ...
  private final JMenuItem menuItemDeleteSelectedLines = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.DELETE_SELECTED_ID));
  ...
  public MainFrame() {
    ...
    var mainScreen = new PainterScreen(actionHolder, viewContext, paintContext, document);
    ...
    try {
      uiPanel = new UIPanel(stateManager, screenUpdater, actionHolder, viewContext, paintContext, document, setting, screenSetting);
      uiPanel.setChildFrameManager(childFrameManager);
    } catch (RuntimeException ex) {
      logger.error("UI panel construction failed", ex);
      Dialogs.showErrorDialog(this, resourceHolder.getString(ResourceKey.ERROR, StringID.Error.DEFAULT_TITLE_ID), ex);
      System.exit(1);
    }
    ...
    copyDialogOpener = new CopyDialogOpener(new ArrayCopyDialogFactory(), new CircleCopyDialogFactory(), () -> dialogService.showNoSelectionMessageForArrayCopy(this), () -> dialogService.showNoSelectionMessageForCircleCopy(this));
    ...
    IntStream.range(0, Constants.MRUFILE_NUM).forEach((i) -> MRUFilesMenuItem[i] = new JMenuItem());
  ...
  private void addActionListenersToComponents() {
    ...
    menuItemUnSelectAll.addActionListener(new UnselectAllItemsActionListener(actionHolder, paintContext, statePopper, screenUpdater));
  ...
  private void saveFileWithModelCheck(final CreasePatternFileTypeKey type) {
    try {
      dataFileAccess.saveFileWithModelCheck(document, fileHistory.getLastDirectory(), filterSelector.getFilter(type), this, () -> dialogService.showModelBuildFailureDialog(this) == JOptionPane.OK_OPTION);
    } catch (IOException e) {
  ...
  private String loadFile(final String filePath) {
    ...
    try {
      return dataFileAccess.loadFile(filePath, fileHistory.getLastPath(), this, filterSelector.getLoadables()).map((doc) -> {
        document.set(doc);
        var property = document.getProperty();
        uiPanel.setEstimationResultColors(convertCodeToColor(property.extractFrontColorCode()), convertCodeToColor(property.extractBackColorCode()));
