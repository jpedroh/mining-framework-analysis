...
public class MainFrame extends JFrame implements ComponentListener, WindowListener {
  ...
  private final JMenu menuFile = new JMenu(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.FILE_ID));

  private final DocFilterSelector filterSelector = new DocFilterSelector();

  private final JMenu menuEdit = new JMenu(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.EDIT_ID));

  private final Doc document = new Doc();

  private final JMenu menuHelp = new JMenu(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.HELP_ID));

  private final PaintContextFactory contextFactory = new PaintContextFactory();

  private final JMenuItem menuItemClear = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.NEW_ID));

  private final PaintContextInterface paintContext = contextFactory.createContext();

  private final JMenuItem menuItemOpen = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.OPEN_ID));

  private final MouseActionHolder actionHolder = new MouseActionHolder();

  private final JMenuItem menuItemSave = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.SAVE_ID));

  private final ButtonFactory buttonFactory;

  private final JMenuItem menuItemSaveAs = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.SAVE_AS_ID));

  private final RepeatCopyDialog arrayCopyDialog;

  private final JMenuItem menuItemSaveAsImage = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.SAVE_AS_IMAGE_ID));

  private final CircleCopyDialog circleCopyDialog;

  private final JMenuItem menuItemExportFOLD = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.EXPORT_FOLD_ID));

  public static JLabel hintLabel = new JLabel();

  private final JMenuItem menuItemExportDXF = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.EXPORT_DXF_ID));

  private final JMenuItem menuItemExportCP = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.EXPORT_CP_ID));

  private final JMenuItem menuItemExportSVG = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.EXPORT_SVG_ID));

  private final JMenuItem menuItemExit = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.EXIT_ID));

  private final JMenuItem[] MRUFilesMenuItem = new JMenuItem[Config.MRUFILE_NUM];

  private JMenuItem menuItemChangeOutline;

  private final JMenuItem menuItemProperty = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.PROPERTY_ID));
  ...
  private final JMenuItem menuItemUndo = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.UNDO_ID));

  private final JMenuItem menuItemRedo = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.REDO_ID));

  private final JMenuItem menuItemAbout = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.ABOUT_ID));

  private final JMenuItem menuItemRepeatCopy = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.
...
>>>>>>> /usr/src/app/output/oripa/oripa/c7a747856cefa97fa45aa389c08f0eabb471efff/src/main/java/oripa/view/main/MainFrame.java/right.java
  ...
  private final JMenuItem menuItemCircleCopy = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.CIRCLE_COPY_ID));

  private final JMenuItem menuItemUnSelectAll = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.UNSELECT_ALL_ID));

  private final JMenuItem menuItemDeleteSelectedLines = new JMenuItem(resourceHolder.getString(ResourceKey.LABEL, StringID.Main.
<<<<<<< /usr/src/app/output/oripa/oripa/c7a747856cefa97fa45aa389c08f0eabb471efff/src/main/java/oripa/view/main/MainFrame.java/left.java
  DELETE_SELECTED_LINES_ID
=======
  DELETE_SELECTED_ID
>>>>>>> /usr/src/app/output/oripa/oripa/c7a747856cefa97fa45aa389c08f0eabb471efff/src/main/java/oripa/view/main/MainFrame.java/right.java
  ));

  private final IniFileAccess iniFileAccess = new IniFileAccess(new InitDataFileReader(), new InitDataFileWriter());
  ...
  public MainFrame() {
    ...
    try {
      uiPanel = new UIPanel(stateManager, screenUpdater, actionHolder, paintContext, document, setting, screenSetting);
    ...
    JScrollPane uiScroll = new JScrollPane(uiPanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
    uiScroll.setPreferredSize(new Dimension(uiPanel.getPreferredSize().width + 25, uiPanel.getPreferredSize().height));
    uiScroll.setAlignmentX(JPanel.LEFT_ALIGNMENT);
    var stateFactory = new PaintBoundStateFactory(stateManager, setting, uiPanel.getUIPanelSetting(), originHolder);
    buttonFactory = new PaintActionButtonFactory(stateFactory, paintContext, actionHolder, screenUpdater);
    ...
    IntStream.range(0, Config.MRUFILE_NUM).forEach((i) -> MRUFilesMenuItem[i] = new JMenuItem());
...
>>>>>>> /usr/src/app/output/oripa/oripa/c7a747856cefa97fa45aa389c08f0eabb471efff/src/main/java/oripa/view/main/MainFrame.java/right.java
  ...
  private void createPaintMenuItems() {
    menuItemChangeOutline = (JMenuItem) buttonFactory.create(this, JMenuItem.class, StringID.EDIT_CONTOUR_ID, null);
    menuItemSelectAll = (JMenuItem) buttonFactory.create(this, JMenuItem.class, StringID.SELECT_ALL_LINE_ID, null);
    menuItemCopyAndPaste = (JMenuItem) buttonFactory.create(this, JMenuItem.class, StringID.COPY_PASTE_ID, null);
    menuItemCutAndPaste = (JMenuItem) buttonFactory.create(this, JMenuItem.class, StringID.CUT_PASTE_ID, null);
  }
  ...
  private void addActionListenersToComponents() {
    ...
    menuItemSave.addActionListener((e) -> {
      ...
      if (CreasePatternFileTypeKey.OPX.extensionsMatch(filePath)) {
        ...
        if (CreasePatternFileTypeKey.FOLD.extensionsMatch(filePath)) {
    ...
    menuItemExportFOLD.addActionListener((e) -> {
      ...
      saveFile(lastDirectory, document.getDataFileName(), filterSelector.getFilter(CreasePatternFileTypeKey.FOLD));
    });
    menuItemSaveAsImage.addActionListener((e) -> {
      String lastDirectory = fileHistory.getLastDirectory();
      saveFile(lastDirectory, document.getDataFileName(), filterSelector.getFilter(CreasePatternFileTypeKey.PICT));
    });
    ...
    menuItemAbout.addActionListener((e) -> JOptionPane.showMessageDialog(this, resourceHolder.getString(ResourceKey.APP_INFO, StringID.AppInfo.ABOUT_THIS_ID), resourceHolder.getString(ResourceKey.LABEL, StringID.Main.TITLE_ID), JOptionPane.INFORMATION_MESSAGE));
    ...
    menuItemSelectAll.addActionListener(new SelectAllLineActionListener(paintContext));
    menuItemSelectAll.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A, InputEvent.CTRL_DOWN_MASK));
    menuItemUnSelectAll.addActionListener(new UnselectAllLinesActionListener(paintContext, screenUpdater));
    menuItemUnSelectAll.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0));
    menuItemDeleteSelectedLines.addActionListener(new DeleteSelectedLinesActionListener(paintContext, screenUpdater));
    menuItemDeleteSelectedLines.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0));
    menuItemCopyAndPaste.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.CTRL_DOWN_MASK));
    menuItemCutAndPaste.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_X, InputEvent.CTRL_DOWN_MASK));
  ...
  private void setProjectSavingAction(final CreasePatternFileTypeKey fileType) {
    filterSelector.getFilter(fileType).setSavingAction(new AbstractSavingAction<Doc>() {
      @Override public boolean save(final Doc data) {
        try {
...
<<<<<<< /usr/src/app/output/oripa/oripa/c7a747856cefa97fa45aa389c08f0eabb471efff/src/main/java/oripa/view/main/MainFrame.java/left.java
  private void openFileFromMRUFileMenuItem(final ActionEvent e) {
    ...
    try {
      ...
      showErrorDialog(resourceHolder.getString(ResourceKey.LABEL, "Error_FileLoadFailed"), ex);
    }
    screenUpdater.updateScreen();
  }
=======
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private void loadFileFromMRUFileMenuItem(final ActionEvent e) {
    ...
    try {
      ...
      Dialogs.showErrorDialog(this, resourceHolder.getString(ResourceKey.ERROR, StringID.Error.LOAD_FAILED_ID), ex);
    }
    screenUpdater.updateScreen();
  }
  ...
  private void saveAnyTypeUsingGUI() {
    String lastDirectory = fileHistory.getLastDirectory();
    String path = saveFile(lastDirectory, document.getDataFileName(), filterSelector.getSavables());
    updateMenu(path);
    updateTitleText();
  }
  ...
  private void clear() {
    ...
    paintContextModification.setCreasePatternToPaintContext(document.getCreasePattern(), paintContext);
  ...
  private void showPropertyDialog() {
    ...
    dialog.setLocation((int) (rec.getCenterX() - dialog.getWidth() / 2), (int) (rec.getCenterY() - dialog.getHeight() / 2));
  ...
  private void showArrayCopyDialog() {
    ...
    if (painter.countSelectedLines() == 0) {
      JOptionPane.showMessageDialog(this, resourceHolder.getString(ResourceKey.WARNING, StringID.Warning.NO_SELECTION_ID), resourceHolder.getString(ResourceKey.WARNING, StringID.Warning.ARRAY_COPY_TITLE_ID), JOptionPane.WARNING_MESSAGE);
    } else {
  ...
  private void showCircleCopyDialog() {
    Painter painter = paintContext.getPainter();
    if (painter.countSelectedLines() == 0) {
      JOptionPane.showMessageDialog(this, resourceHolder.getString(ResourceKey.WARNING, StringID.Warning.NO_SELECTION_ID), resourceHolder.getString(ResourceKey.WARNING, StringID.Warning.CIRCLE_COPY_TITLE_ID), JOptionPane.WARNING_MESSAGE);
    } else {
  ...
  private void updateTitleText() {
    ...
    if (document.getDataFilePath().isEmpty()) {
      fileName = resourceHolder.getString(ResourceKey.
<<<<<<< /usr/src/app/output/oripa/oripa/c7a747856cefa97fa45aa389c08f0eabb471efff/src/main/java/oripa/view/main/MainFrame.java/left.java
      LABEL
=======
      DEFAULT
>>>>>>> /usr/src/app/output/oripa/oripa/c7a747856cefa97fa45aa389c08f0eabb471efff/src/main/java/oripa/view/main/MainFrame.java/right.java
    ...
    setTitle(fileName + " - " + resourceHolder.getString(ResourceKey.LABEL, StringID.Main.TITLE_ID));
  }
  ...
  private void saveProjectFile(final Doc doc, final String filePath, final CreasePatternFileTypeKey fileType) {
    try {
      ...
      Dialogs.showErrorDialog(this, resourceHolder.getString(ResourceKey.ERROR, StringID.Error.SAVE_FAILED_ID), e);
    }
  ...
  @SafeVarargs private String saveFile(final String directory, final String fileName, final FileAccessSupportFilter<Doc>... filters) {
    try {
      return dataFileAccess.saveFile(document, directory, fileName, this, filters).map((path) -> {
        paintContext.creasePatternUndo().clearChanged();
        return path;
  ...
  private String loadFile(final String filePath) {
    childFrameManager.closeAllChildrenRecursively(this);
    try {
      return dataFileAccess.loadFile(filePath, fileHistory.getLastPath(), this, filterSelector.getLoadables()).map((doc) -> {
        document.set(doc);
        screenSetting.setGridVisible(false);
        paintContextModification.setCreasePatternToPaintContext(document.getCreasePattern(), paintContext);
        return document.getDataFilePath();
      }).orElse(null);
    } catch (FileVersionError | IllegalArgumentException | WrongDataFormatException | IOException e) {
      logger.error("failed to load", e);
      Dialogs.showErrorDialog(this, resourceHolder.getString(ResourceKey.ERROR, StringID.Error.LOAD_FAILED_ID), e);
      return document.getDataFilePath();
    }
  }
  ...
  private void saveIniFile() {
    try {
      ...
      Dialogs.showErrorDialog(this, resourceHolder.getString(ResourceKey.ERROR, StringID.Error.SAVE_INI_FAILED_ID), e);
    }
  }
  ...
  @Override public void componentResized(final ComponentEvent arg0) {
  }

  @Override public void componentMoved(final ComponentEvent arg0) {
  }

  @Override public void componentShown(final ComponentEvent arg0) {
  }

  @Override public void componentHidden(final ComponentEvent arg0) {
  }
  ...
  @Override public void windowClosing(final WindowEvent arg0) {
    if (paintContext.creasePatternUndo().changeExists()) {
      int selected = JOptionPane.showConfirmDialog(this, "The crease pattern has been modified. Would you like to save?", "Comfirm to save", JOptionPane.YES_NO_OPTION);
      if (selected == JOptionPane.YES_OPTION) {
        document.setCreasePattern(paintContext.getCreasePattern());
        String path = saveFile(fileHistory.getLastDirectory(), document.getDataFileName(), filterSelector.getSavables());
        if (path == null) {
        }
      }
    }
