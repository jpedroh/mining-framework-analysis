 ...
 */
@WebSocket(maxTextMessageSize = 256 * 1024 * 1024) public class ServiceSocket {
  ...
  @OnWebSocketFrame public void onFrame(Frame frame) {
    synchronized (parent) {
      ...
      if (responseExpression == null || responseExpression.matcher(frameTxt).find()) {
        ...
        if (!disconnectPattern.isEmpty() && disconnectExpression.matcher(frameTxt).find()) {
          logMessage.append("; matched connection close pattern").append("\n");
          closeLatch.countDown();
          close(StatusCode.NORMAL, "JMeter closed session.");
        } else {
  ...
  @OnWebSocketConnect public void onOpen(Session session) {
