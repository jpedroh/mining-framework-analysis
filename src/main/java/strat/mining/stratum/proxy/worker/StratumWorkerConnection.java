...
public class StratumWorkerConnection extends StratumConnection implements WorkerConnection {
  ...
  @Override protected void onParsingError(String line, Throwable throwable) {
    LOGGER.error("Parsing error on worker connection {}. Failed to parse line {}.", getConnectionName(), line, throwable);
  }

  @Override protected void onDisconnectWithError(Throwable cause) {
    manager.onWorkerDisconnection(this, cause);
  }

  @Override protected void onNotify(MiningNotifyNotification notify) {
  }

  @Override protected void onSetDifficulty(MiningSetDifficultyNotification setDifficulty) {
  }

  @Override protected void onSetExtranonce(MiningSetExtranonceNotification setExtranonce) {
  }

  @Override protected void onClientReconnect(ClientReconnectNotification clientReconnect) {
  }

  @Override protected void onAuthorizeRequest(MiningAuthorizeRequest request) {
  ...
  @Override protected void onSubscribeRequest(MiningSubscribeRequest request) {
    ...
    if (error == null) {
      try {
        ...
        LOGGER.error("Too many connections on pool {} for the connection {}. Sending error and close the connection.", pool.getName(), getConnectionName(), e);
        error = new JsonRpcError();
        error.setCode(JsonRpcError.ErrorCode.UNKNOWN.getCode());
  ...
  @Override protected void onSubmitRequest(MiningSubmitRequest request) {
  ...
  @Override protected void onExtranonceSubscribeRequest(MiningExtranonceSubscribeRequest request) {
  ...
  @Override protected void onGetVersionRequest(ClientGetVersionRequest request) {
  ...
  public void onPoolSubmitResponse(MiningSubmitRequest workerRequest, MiningSubmitResponse poolResponse) {
    ...
    if (poolResponse.getIsAccepted() != null && poolResponse.getIsAccepted()) {
      LOGGER.info("Accepted share (diff: {}) from {}@{} on {}. Yeah !!!!", difficultyString, workerRequest.getWorkerName(), getConnectionName(), pool.getName());
    } else {
      LOGGER.info("REJECTED share (diff: {}) from {}@{} on {}. Booo !!!!. Error: {}", difficultyString, workerRequest.getWorkerName(), getConnectionName(), pool.getName(), poolResponse.getJsonError());
    }
  ...
  @Override protected void onAuthorizeResponse(MiningAuthorizeRequest request, MiningAuthorizeResponse response) {
    LOGGER.warn("Worker {} send an Authorize response. This should not happen.", getConnectionName());
  }

  @Override protected void onSubscribeResponse(MiningSubscribeRequest request, MiningSubscribeResponse response) {
    LOGGER.warn("Worker {} send a Subscribe response. This should not happen.", getConnectionName());
  }

  @Override protected void onSubmitResponse(MiningSubmitRequest request, MiningSubmitResponse response) {
    LOGGER.warn("Worker {} send a Submit response. This should not happen.", getConnectionName());
  }

  @Override protected void onExtranonceSubscribeResponse(MiningExtranonceSubscribeRequest request, MiningExtranonceSubscribeResponse response) {
    LOGGER.warn("Worker {} send an Extranonce subscribe response. This should not happen.", getConnectionName());
  }

  @Override protected void onGetVersionResponse(ClientGetVersionRequest request, ClientGetVersionResponse response) {
  }
  ...
  public void rebindToPool(Pool newPool) throws TooManyWorkersException, ChangeExtranonceNotSupportedException {
    if (isSetExtranonceNotificationSupported) {
      LOGGER.info("Rebind connection {} from pool {} to pool {} with setExtranonce notification.", getConnectionName(), pool.getName(), newPool.getName());
  ...
  @Override public Map<String, String> getAuthorizedWorkers() {
  ...
  @Override public double getAcceptedHashrate() {
    return workerHashrateDelegator.getAcceptedHashrate();
  }

  @Override public double getRejectedHashrate() {
    return workerHashrateDelegator.getRejectedHashrate();
  }

  @Override public void updateShareLists(Share share, boolean isAccepted) {
    workerHashrateDelegator.updateShareLists(share, isAccepted);
  }

  @Override public void setSamplingHashesPeriod(Integer samplingHashesPeriod) {
    workerHashrateDelegator.setSamplingHashesPeriod(samplingHashesPeriod);
  }

  @Override public Date getActiveSince() {
  ...
  @Override public void onPoolDifficultyChanged(MiningSetDifficultyNotification notification) {
    if (logRealShareDifficulty) {
  ...
  @Override public void onPoolNotify(MiningNotifyNotification notification) {
    if (logRealShareDifficulty) {
  ...
  private void updateBlockHeader(MiningNotifyNotification notification) {
    ...
    if (pool != null) {
      currentHeader = new GetworkJobTemplate(notification.getJobId(), notification.getBitcoinVersion(), notification.getPreviousHash(), notification.getCurrentNTime(), notification.getNetworkDifficultyBits(), notification.getMerkleBranches(), notification.getCoinbase1(), notification.getCoinbase2(), getPool().getExtranonce1() + extranonce1Tail);
