 ...
 */
@SuppressWarnings(value = { "unchecked" }) @Extension public class ParametersSlicer extends UnorderedStringSlicer<Job> {
  ...
  public static class ParametersSliceSpec extends UnorderedStringSlicerSpec<Job> {
    ...
    @Override public boolean isIndexUsed(int count) {
      return true;
    }

    @Override public boolean isValueTrimmed() {
      return false;
    }

    @Override public String getValueIndex(Job item, int index) {
      List<ParameterItem> pitems = getParameterItems(item);
      return pitems.get(index).name;
    }

    @Override public int getValueIndex(Job item, String indexName) {
      List<ParameterItem> pitems = getParameterItems(item);
