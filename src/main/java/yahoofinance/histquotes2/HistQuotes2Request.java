...
public class HistQuotes2Request {
    ...
    private HistoricalQuote parseCSVLine(String line) {
        ...
        return new HistoricalQuote(this.symbol, CalendarUtil.parseHistDate(data[0])).setOpen(BigDecimalUtil.getBigDecimal(data[1])).setLow(BigDecimalUtil.getBigDecimal(data[3])).setHigh(BigDecimalUtil.getBigDecimal(data[2])).setClose(BigDecimalUtil.getBigDecimal(data[4])).setAdjClose(PrimitiveTypesConvertUtils.getBigDecimal(data[6])).setVolume(BigDecimalUtil.getLong(data[5]));
    }
}