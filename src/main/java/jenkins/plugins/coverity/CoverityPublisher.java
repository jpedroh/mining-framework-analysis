        ...
        if (analysisVersionXml.isFile()) {
          try {
            ...
            if (version.compareTo(CoverityVersion.MINIMUM_SUPPORTED_VERSION) < 0) {
              return FormValidation.error("\"Coverity Static Analysis\" version " + version.toString() + " detected. " + "The minimum supported version is " + CoverityVersion.MINIMUM_SUPPORTED_VERSION.getEffectiveVersion().toString());
            }
          } catch (InterruptedException e) {
      ...
      try {
        if (cimStream.isValid()) {
          ...
          if (defectFilters != null) {
            defectFilters.invertCheckers(allCheckers, toStrings(cimStreamDescriptor.doFillClassificationDefectFilterItems(cimInstance)), toStrings(cimStreamDescriptor.doFillActionDefectFilterItems(cimInstance)), toStrings(cimStreamDescriptor.doFillSeveritiesDefectFilterItems(cimInstance)), toStrings(cimStreamDescriptor.doFillComponentDefectFilterItems(cimInstance, cimStream.getStream())));
          }
        }
