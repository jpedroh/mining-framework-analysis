...
public class CoverityPublisher extends Recorder {
  ...
  @DataBoundConstructor public CoverityPublisher(CIMStream cimStream, InvocationAssistance invocationAssistance, boolean failBuild, boolean unstable, boolean keepIntDir, boolean skipFetchingDefects, boolean hideChart, TaOptionBlock taOptionBlock, ScmOptionBlock scmOptionBlock) {
  ...
  @Extension public static class DescriptorImpl extends BuildStepDescriptor<Publisher> {
    @Deprecated private String home;
      ...
      if (StringUtils.isNotEmpty(home)) {
        ...
        if (installations.length > 0) {
          for (CoverityToolInstallation installation : installations) {
            if (CoverityToolInstallation.DEFAULT_NAME.equalsIgnoreCase(installation.getName()) || home.equals(installation.getHome())) {
              defaultExists = true;
            }
          }
        }
    ...
    @Deprecated public String getHome() {
      return home;
    }

    @Deprecated public void setHome(String home) {
      this.home = home;
    }
    ...
    @SuppressWarnings(value = { "deprecation" }) public String getHome(Node node, EnvVars environment, TaskListener listener) {
    ...
    /**
         ...
         */
    public FormValidation doCheckInstance(@QueryParameter String host, @QueryParameter int port, @QueryParameter String user, @QueryParameter String password, @QueryParameter boolean useSSL, @QueryParameter int dataPort, @QueryParameter String credentialId) {
        ...
        if (analysisDir.exists()) {
          if (analysisVersionXml.isFile()) {
            ...
            if (version.compareTo(CoverityVersion.MINIMUM_SUPPORTED_VERSION) < 0) {
              return FormValidation.error("Analysis version " + version.toString() + " detected. " + "The minimum supported version is " + CoverityVersion.MINIMUM_SUPPORTED_VERSION.toString());
            }
    ...
    public FormValidation doCheckHome(@QueryParameter String home) {
      if (StringUtils.isNotEmpty(home)) {
        return FormValidation.warning("Static Analysis Location is deprecated in Coverity plugin version 1.10 and later. " + "Please use the Coverity Static Analysis Tools global configuration instead.");
      }
    ...
    @Override public Publisher newInstance(@CheckForNull StaplerRequest req, @Nonnull JSONObject formData) throws FormException {
      ...
      if (json != null && !json.isNullObject()) {
        ...
        if (cimStreamDescriptor != null) {
          if (StringUtils.isEmpty(cimStream.getInstance()) || StringUtils.isEmpty(cimStream.getProject()) || StringUtils.isEmpty(cimStream.getStream())) {
            ...
            if (defectFilters != null) {
              try {
                cimStream.getDefectFilters().initializeFilter(allCheckers, toStrings(cimStreamDescriptor.doFillClassificationDefectFilterItems(cimStream.getInstance())), toStrings(cimStreamDescriptor.doFillActionDefectFilterItems(cimStream.getInstance())), toStrings(cimStreamDescriptor.doFillSeveritiesDefectFilterItems(cimStream.getInstance())), toStrings(cimStreamDescriptor.doFillComponentDefectFilterItems(cimStream.getInstance(), cimStream.getStream())), toStrings(cimStreamDescriptor.doFillImpactDefectFilterItems(cimStream.getInstance())));
    ...
    @JavaScriptMethod public void doLoadProjectsForInstance(StaplerRequest req, StaplerResponse rsp) throws ServletException, IOException {
      JSONObject json = getJSONClassObject(req.getSubmittedForm(), getId());
      if (json != null && !json.isNullObject()) {
        CoverityPublisher publisher = req.bindJSON(CoverityPublisher.class, json);
        CIMStream cimStream = publisher.getCimStream();
        if (cimStream != null) {
          ...
          final List<String> projects = cimInstance != null ? new ArrayList<>(CimCache.getInstance().getProjects(cimInstance)) : new ArrayList<String>();
    ...
    @JavaScriptMethod public void doLoadStreamsForProject(StaplerRequest req, StaplerResponse rsp) throws ServletException, IOException {
      JSONObject json = getJSONClassObject(req.getSubmittedForm(), getId());
      if (json != null && !json.isNullObject()) {
        CoverityPublisher publisher = req.bindJSON(CoverityPublisher.class, json);
        CIMStream cimStream = publisher.getCimStream();
        if (cimStream != null) {
          CIMInstance cimInstance = getInstance(publisher);
          final List<String> streams = cimInstance != null ? new ArrayList<>(CimCache.getInstance().getStreams(cimInstance, cimStream.getProject())) : new ArrayList<String>();
    ...
    public ListBoxModel doFillCredentialIdItems(@AncestorInPath Jenkins context) {
      return new StandardListBoxModel().withEmptySelection().withMatching(CredentialsMatchers.anyOf(CredentialsMatchers.instanceOf(StandardUsernamePasswordCredentials.class)), CredentialsProvider.lookupCredentials(StandardUsernamePasswordCredentials.class, context, ACL.SYSTEM, new ArrayList<DomainRequirement>()));
    }
