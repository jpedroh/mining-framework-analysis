...
public class CoverityLauncher extends Launcher {
  ...
  @Override public Proc launch(ProcStarter starter) throws IOException {
    ...
    if (!isCoverityBuildStepEnabled || (isCoverityBuildStepEnabled && CoverityLauncherDecorator.CoverityBuildStep.get())) {
      ...
      if (invocationAssistance != null && invocationAssistance.getSaOverride() != null) {
        home = new CoverityToolInstallation(CoverityToolInstallation.GLOBAL_OVERRIDE_NAME, CoverityUtils.evaluateEnvVars(invocationAssistance.getSaOverride(), envVars, invocationAssistance.getUseAdvancedParser())).forEnvironment(envVars).getHome();
      }
