...
public class CoverityViewResultsPublisher extends Recorder implements SimpleBuildStep {
  ...
  @DataBoundConstructor public CoverityViewResultsPublisher(String connectInstance, String connectView, String projectId) {
  ...
  @Override public void perform(@Nonnull Run<?, ?> run, @Nonnull FilePath workspace, @Nonnull Launcher launcher, @Nonnull TaskListener listener) throws IOException {
    final PrintStream logger = listener.getLogger();
    logger.println("[Coverity] Publish Coverity View Results { " + "connectInstance:\'" + connectInstance + "\', " + "projectId:\'" + projectId + "\', " + "connectView:\'" + connectView + "}");
    ...
    if (StringUtils.isEmpty(projectId) || StringUtils.isEmpty(connectView)) {
      logger.println("[Coverity] Coverity Connect project and view are required. But was Project: \'" + projectId + "\' View: \'" + connectView + "\'");
      throw new AbortException("Coverity Connect project and view are required. But was Project: \'" + projectId + "\' View: \'" + connectView + "\'");
    }
    try {
      ...
      if (abortPipeline && buildAction.getDefects().size() > 0) {
        ...
        if (failPipeline && buildAction.getDefects().size() > 0) {
          ...
          if (unstable && buildAction.getDefects().size() > 0) {
  ...
  @Override public CoverityViewResultsDescriptor getDescriptor() {
  ...
  @DataBoundSetter public void setFailPipeline(boolean failPipeline) {
