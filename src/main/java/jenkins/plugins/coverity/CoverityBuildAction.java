...
public class CoverityBuildAction implements LastBuildAction {
  ...
  public String getURL(CoverityDefect defect) throws IOException, CovRemoteServiceException_Exception {
    ...
    return String.format(header + "://%s:%d/sourcebrowser.htm?projectId=%s&mergedDefectId=%d", instance.getHost(), instance.getPort(), instance.getProjectKey(projectId), defect.getCid());
  }
  ...
  @Override public Collection<? extends Action> getProjectActions() {
    if (build != null) {
      ...
      if (parent instanceof AbstractProject) {
        ...
        if (coverityPublisher != null && coverityPublisher.getHideChart()) {
