...
public class WebServiceFactory {
  ...
  protected DefectService createDefectService(CIMInstance cimInstance) throws MalformedURLException {
    ...
    DefectServiceService defectServiceService = new DefectServiceService(url, new QName(COVERITY_V9_NAMESPACE, "DefectServiceService"));
      ...
      synchronized (this) {
        if (connectionResponse.isConnected()) {
          if (serviceType == WebServiceType.ConfigurationService) {
            ...
            if (serviceType == WebServiceType.DefectService) {
              if (!defectServiceMap.containsKey(cimInstance)) {
  ...
  protected ConfigurationService createConfigurationService(CIMInstance cimInstance) throws MalformedURLException {
    ...
    ConfigurationServiceService configurationServiceService = new ConfigurationServiceService(url, new QName(COVERITY_V9_NAMESPACE, "ConfigurationServiceService"));
    ClassLoader cl = Thread.currentThread().getContextClassLoader();
    try {
  ...
  public ViewsService getViewService(CIMInstance instance) throws MalformedURLException, NoSuchAlgorithmException, KeyManagementException {
    ...
    if (instance.isUseSSL()) {
      ...
      restClient = ClientBuilder.newBuilder().sslContext(sslContext).hostnameVerifier(HttpsURLConnection.getDefaultHostnameVerifier()).build();
    } else {
      restClient = ClientBuilder.newClient();
    }
    HttpAuthenticationFeature httpAuthenticationFeature = HttpAuthenticationFeature.basic(instance.getCoverityUser(), instance.getCoverityPassword());
  ...
  private void attachAuthenticationHandler(BindingProvider service, CIMInstance cimInstance) {
    service.getBinding().setHandlerChain(Arrays.<Handler>asList(new ClientAuthenticationHandlerWSS(cimInstance.getCoverityUser(), cimInstance.getCoverityPassword())));
  }
  ...
  public static class CheckWsResponse {
    ...
    @Override public String toString() {
      return "Check Coverity Web Service Response: {" + " Code=" + responseCode + ", Message=\"" + responseMessage + "\"" + ", URL=" + serviceUrl + '}';
    }
