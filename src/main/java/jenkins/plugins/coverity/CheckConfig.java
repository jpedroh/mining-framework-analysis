...
public class CheckConfig extends AbstractDescribableImpl<CheckConfig> {
        ...
        for (Status s : status) {
          if (s instanceof StreamStatus) {
            ...
            if (!ss.getVersion().compareToAnalysis(analysisVersion)) {
              newStatus.add(new Status(false, "Connect instance " + ss.getStream().toPrettyString() + " (version " + ss.getVersion() + "|" + ss.getVersion().getEffectiveVersion() + ") is incompatible with analysis version " + analysisVersion));
            }
          }
        }
  ...
  public static NodeStatus checkNode(CoverityPublisher publisher, AbstractBuild<?, ?> build, Launcher launcher, TaskListener listener) {
    ...
    try {
      ...
      final TaskListener listen = listener;
      CoverityVersion version = getVersion(homePath, listen);
      if (version.compareTo(CoverityVersion.MINIMUM_SUPPORTED_VERSION) < 0) {
        return new NodeStatus(false, "\"Coverity Static Analysis\" version " + version.toString() + " is not supported. " + "The minimum supported version is " + CoverityVersion.MINIMUM_SUPPORTED_VERSION.getEffectiveVersion().toString(), node, version);
      }
    ...
    try {
      ...
      try {
        if (connectorParser.beta != null) {
          return new CoverityVersion(Integer.parseInt(connectorParser.major), Integer.parseInt(connectorParser.minor), Integer.parseInt(connectorParser.revision), Integer.parseInt(connectorParser.beta));
        } else {
          return new CoverityVersion(Integer.parseInt(connectorParser.major), Integer.parseInt(connectorParser.minor), Integer.parseInt(connectorParser.revision));
        }
  ...
  @Extension public static class DescriptorImpl extends Descriptor<CheckConfig> {
