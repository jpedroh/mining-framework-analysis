...
public final class SubparsersImpl implements Subparsers {
  ...
  @Override public SubparserImpl addParser(String command) {
    return addParser(command, true, mainParser_.getPrefixChars());
  }

  @Override public SubparserImpl addParser(String command, boolean addHelp) {
    return addParser(command, addHelp, mainParser_.getPrefixChars());
  }

  @Override public SubparserImpl addParser(String command, boolean addHelp, String prefixChars) {
    ...
    SubparserImpl parser = new SubparserImpl(mainParser_.getConfig().forSubparser(addHelp, prefixChars), command, mainParser_);
    parsers_.put(command, parser);
    return parser;
  }

  @Override public SubparsersImpl dest(String dest) {
    dest_ = TextHelper.nonNull(dest);
    return this;
  }

  @Override public SubparsersImpl help(String help) {
    help_ = TextHelper.nonNull(help);
    return this;
  }

  @Override public SubparsersImpl title(String title) {
  ...
  @Override public SubparsersImpl description(String description) {
  ...
  @Override public SubparsersImpl metavar(String metavar) {
  ...
  private SubparserImpl resolveNextSubparser(ParseState state, String command) throws ArgumentParserException {
    ...
    if (ap == null) {
      List<String> cand = TextHelper.findPrefix(parsers_.keySet(), command);
      int size = cand.size();
      if (size == 1) {
        ...
        if (size > 1) {
          ...
          throw new ArgumentParserException(String.format(TextHelper.LOCALE_ROOT, localize("ambiguousCommandError"), command, TextHelper.concat(cand, 0, ", ")), mainParser_);
        }
      }
    }
    return ap;
  ...
  public void parseArg(ParseState state, Map<String, Object> opts) throws ArgumentParserException {
    ...
    if (ap == null) {
      ...
      throw new UnrecognizedCommandException(String.format(TextHelper.LOCALE_ROOT, localize("invalidChoiceError"), state.getArg(), sb.toString()), mainParser_, state.getArg());
    } else {
      ++state.index;
  ...
  public void printSubparserHelp(PrintWriter writer, int format_width) {
    ...
    for (Map.Entry<String, SubparserImpl> entry : parsers_.entrySet()) {
      if (entry.getValue().getHelpControl() != FeatureControl.SUPPRESS && entry.getKey().equals(entry.getValue().getCommand())) {
