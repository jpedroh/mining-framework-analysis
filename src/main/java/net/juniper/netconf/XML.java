  ...
  public String findValue(List<String> list) {
    ...
    for (int k = 0; k < list.size(); k++) {
      ...
      if (!nextElementName.contains("~")) {
        try {
          NodeList nextElementList = nextElement.getElementsByTagName(nextElementName);
          String n2nString = null;
          if (k < list.size() - 1) {
            n2nString = list.get(k + 1);
          }
          if (n2nString != null && n2nString.contains("~")) {
            String n2nText = n2nString.substring(n2nString.indexOf("~") + 1);
            String n2nElementName = n2nString.substring(0, n2nString.indexOf("~"));
            for (int i = 0; i < nextElementList.getLength(); i++) {
              nextElement = (Element) nextElementList.item(i);
              Element n2nElement = (Element) nextElement.getElementsByTagName(n2nElementName).item(0);
              String text = n2nElement.getFirstChild().getNodeValue();
              text = trim(text);
