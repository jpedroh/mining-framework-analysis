...
class AuthContextImpl extends AuthContext {
  ...
  class MessageFactoryImpl extends MessageFactory {
    ...
    DHCommitMessage getDHCommitMessage() throws OtrException {
      DHCommitMessage message = new DHCommitMessage(getSession().getProtocolVersion(), getLocalDHPublicKeyHash(), getLocalDHPublicKeyEncrypted());
    ...
    DHKeyMessage getDHKeyMessage() throws OtrException {
      DHKeyMessage dhKeyMessage = new DHKeyMessage(getSession().getProtocolVersion(), (DHPublicKey) getLocalDHKeyPair().getPublic());
    ...
    RevealSignatureMessage getRevealSignatureMessage() throws OtrException {
      try {
        SignatureM m = new SignatureM((DHPublicKey) getLocalDHKeyPair().getPublic(), getRemoteDHPublicKey(), getLocalLongTermKeyPair().getPublic(), getLocalDHKeyPairID());
        ...
        SignatureX mysteriousX = new SignatureX(getLocalLongTermKeyPair().getPublic(), getLocalDHKeyPairID(), signature);
        byte[] xEncrypted = otrCryptoEngine.aesEncrypt(getC(), null, SerializationUtils.toByteArray(mysteriousX));
        byte[] tmp = SerializationUtils.writeData(xEncrypted);
        byte[] xEncryptedHash = otrCryptoEngine.sha256Hmac160(tmp, getM2());
        RevealSignatureMessage revealSignatureMessage = new RevealSignatureMessage(getSession().getProtocolVersion(), xEncrypted, xEncryptedHash, getR());
        revealSignatureMessage.senderInstanceTag = getSession().getSenderInstanceTag().getValue();
        revealSignatureMessage.receiverInstanceTag = getSession().getReceiverInstanceTag().getValue();
    ...
    SignatureMessage getSignatureMessage() throws OtrException {
      ...
      try {
        ...
        SignatureMessage signatureMessage = new SignatureMessage(getSession().getProtocolVersion(), xEncrypted, xEncryptedHash);
  ...
  private void handleSignatureMessage(SignatureMessage m) throws OtrException {
    ...
    logger.finest(sessionID.getAccountID() + " received a signature message from " + sessionID.getUserID() + " through " + sessionID.getProtocolName() + ".");
    if (m.protocolVersion == OTRv.TWO && !session.getSessionPolicy().getAllowV2()) {
      ...
      if (m.protocolVersion == OTRv.THREE && !session.getSessionPolicy().getAllowV3()) {
        logger.finest("If ALLOW_V3 is not set, ignore this message.");
        return;
      } else {
        if (m.protocolVersion == OTRv.THREE && session.getSenderInstanceTag().getValue() != m.receiverInstanceTag) {
          logger.finest("Received a Signature Message with receiver instance tag" + " that is different from ours, ignore this message");
          return;
        }
      }
    }
  ...
  private void handleRevealSignatureMessage(RevealSignatureMessage m) throws OtrException {
    Session session = getSession();
    SessionID sessionID = session.getSessionID();
    logger.finest(sessionID.getAccountID() + " received a reveal signature message from " + sessionID.getUserID() + " through " + sessionID.getProtocolName() + ".");
    if (m.protocolVersion == OTRv.TWO && !session.getSessionPolicy().getAllowV2()) {
      logger.finest("If ALLOW_V2 is not set, ignore this message.");
      return;
    } else {
      if (m.protocolVersion == OTRv.THREE && !session.getSessionPolicy().getAllowV3()) {
        ...
        if (m.protocolVersion == OTRv.THREE && session.getSenderInstanceTag().getValue() != m.receiverInstanceTag) {
          logger.finest("Received a Reveal Signature Message with receiver instance tag" + " that is different from ours, ignore this message");
          return;
        }
      }
    }
  ...
  private void handleDHKeyMessage(DHKeyMessage m) throws OtrException {
    Session session = getSession();
    SessionID sessionID = session.getSessionID();
    logger.finest(sessionID.getAccountID() + " received a D-H key message from " + sessionID.getUserID() + " through " + sessionID.getProtocolName() + ".");
    if (m.protocolVersion == OTRv.TWO && !session.getSessionPolicy().getAllowV2()) {
      logger.finest("If ALLOW_V2 is not set, ignore this message.");
      return;
    } else {
      if (m.protocolVersion == OTRv.THREE && !session.getSessionPolicy().getAllowV3()) {
        ...
        if (m.protocolVersion == OTRv.THREE && session.getSenderInstanceTag().getValue() != m.receiverInstanceTag) {
          logger.finest("Received a D-H Key Message with receiver instance tag" + " that is different from ours, ignore this message");
          return;
        }
      }
    }
  ...
  private void handleDHCommitMessage(DHCommitMessage m) throws OtrException {
    Session session = getSession();
    SessionID sessionID = session.getSessionID();
    logger.finest(sessionID.getAccountID() + " received a D-H commit message from " + sessionID.getUserID() + " through " + sessionID.getProtocolName() + ".");
    if (m.protocolVersion == OTRv.TWO && !session.getSessionPolicy().getAllowV2()) {
      ...
      if (m.protocolVersion == OTRv.THREE && !session.getSessionPolicy().getAllowV3()) {
        ...
        if (m.protocolVersion == OTRv.THREE && session.getSenderInstanceTag().getValue() != m.receiverInstanceTag && m.receiverInstanceTag != 0) {
          logger.finest("Received a D-H commit message with receiver instance tag " + "that is different from ours, ignore this message.");
          return;
        }
      }
    }
