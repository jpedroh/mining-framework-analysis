...
public class SessionImpl implements Session {
  ...
  private SessionImpl(SessionID sessionID, OtrEngineHost listener, InstanceTag senderTag, InstanceTag receiverTag) {
    this.setSessionID(sessionID);
    this.setHost(listener);
    this.sessionStatus = SessionStatus.PLAINTEXT;
    this.offerStatus = OfferStatus.idle;
    ...
    if (m instanceof AbstractEncodedMessage && isMasterSession) {
      ...
      if (encodedM.protocolVersion == OTRv.THREE) {
        if (encodedM.receiverInstanceTag != this.getSenderInstanceTag().getValue()) {
          if (!(encodedM.messageType == AbstractEncodedMessage.MESSAGE_DH_COMMIT && encodedM.receiverInstanceTag == 0)) {
        ...
        if (encodedM.senderInstanceTag != this.getReceiverInstanceTag().getValue() && this.getReceiverInstanceTag().getValue() != 0) {
          ...
          synchronized (slaveSessions) {
            if (!slaveSessions.containsKey(newReceiverTag)) {
              final SessionImpl session = new SessionImpl(sessionID, getHost(), getSenderInstanceTag(), newReceiverTag);
  ...
  private void handleQueryMessage(QueryMessage queryMessage) throws OtrException {
    logger.finest(getSessionID().getAccountID() + " received a query message from " + getSessionID().getUserID() + " throught " + getSessionID().getProtocolName() + ".");
    OtrPolicy policy = getSessionPolicy();
    if (queryMessage.versions.contains(OTRv.THREE) && policy.getAllowV3()) {
      ...
      if (queryMessage.versions.contains(OTRv.TWO) && policy.getAllowV2()) {
        ...
        if (queryMessage.versions.contains(OTRv.ONE) && policy.getAllowV1()) {
  ...
  private void handleErrorMessage(ErrorMessage errorMessage) throws OtrException {
    logger.finest(getSessionID().getAccountID() + " received an error message from " + getSessionID().getUserID() + " throught " + getSessionID().getUserID() + ".");
  ...
  private String handleDataMessage(DataMessage data) throws OtrException {
    ...
    switch (this.getSessionStatus()) {
      ...
      if (matchingKeys == null) {
        ...
        injectMessage(new ErrorMessage(AbstractMessage.MESSAGE_ERROR, getHost().getReplyForUnreadableMessage(getSessionID())));
        return null;
      }
  ...
  private String handlePlainTextMessage(PlainTextMessage plainTextMessage) throws OtrException {
    logger.finest(getSessionID().getAccountID() + " received a plaintext message from " + getSessionID().getUserID() + " throught " + getSessionID().getProtocolName() + ".");
    OtrPolicy policy = getSessionPolicy();
    ...
    if (versions == null || versions.size() < 1) {
      ...
      if (policy.getWhitespaceStartAKE()) {
        ...
        if (plainTextMessage.versions.contains(OTRv.THREE) && policy.getAllowV3()) {
          ...
          if (plainTextMessage.versions.contains(OTRv.TWO) && policy.getAllowV2()) {
            ...
            if (plainTextMessage.versions.contains(1) && policy.getAllowV1()) {
  ...
  public String transformSending(String msgText, List<TLV> tlvs) throws OtrException {
    ...
    switch (this.getSessionStatus()) {
      ...
      case ENCRYPTED:
      logger.finest(getSessionID().getAccountID() + " sends an encrypted message to " + getSessionID().getUserID() + " throught " + getSessionID().getProtocolName() + ".");
      ...
      MysteriousT t = new MysteriousT(this.protocolVersion, getSenderInstanceTag().getValue(), getReceiverInstanceTag().getValue(), 0, senderKeyID, receipientKeyID, nextDH, ctr, encryptedMsg);
