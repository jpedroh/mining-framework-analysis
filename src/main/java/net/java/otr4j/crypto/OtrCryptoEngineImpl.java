...
public class OtrCryptoEngineImpl implements OtrCryptoEngine {
  ...
  @Override public KeyPair generateDSAKeyPair() {
    try {
  ...
  @Override public KeyPair generateDHKeyPair() {
    try {
  ...
  @Override public DHPublicKey getDHPublicKey(BigInteger mpi) throws OtrCryptoException {
  ...
  @Override public byte[] sha256Hmac(byte[] b, byte[] key) throws OtrCryptoException {
    return this.sha256Hmac(b, key, 0);
  }

  @Override public byte[] sha256Hmac(byte[] b, byte[] key, int length) throws OtrCryptoException {
  ...
  @Override public byte[] sha1Hmac(byte[] b, byte[] key, int length) throws OtrCryptoException {
    try {
  ...
  @Override public byte[] sha256Hmac160(byte[] b, byte[] key) throws OtrCryptoException {
    return sha256Hmac(b, key, 20);
  }

  @Override public byte[] sha256Hash(byte[] b) {
    try {
  ...
  @Override public byte[] sha1Hash(byte[] b) {
    try {
  ...
  /**
	 ...
	 */
  @Override public byte[] aesDecrypt(byte[] key, byte[] ctr, byte[] b) throws OtrCryptoException {
    try {
  ...
  @Override public byte[] aesEncrypt(byte[] key, byte[] ctr, byte[] b) throws OtrCryptoException {
    try {
  ...
  @Override public BigInteger generateSecret(PrivateKey privKey, PublicKey pubKey) throws OtrCryptoException {
    try {
  ...
  @Override public byte[] sign(byte[] b, PrivateKey privatekey) throws OtrCryptoException {
  ...
  @Override public boolean verify(byte[] b, PublicKey pubKey, byte[] rs) throws OtrCryptoException {
  ...
  private byte[] bytesModQ(final BigInteger q, final byte[] data) {
    return data.length == DSA_RAW_DATA_LENGTH_BYTES ? data : Util.asUnsignedByteArray(DSA_RAW_DATA_LENGTH_BYTES, new BigInteger(1, data).mod(q));
  }

  @Override public String getFingerprint(PublicKey pubKey) throws OtrCryptoException {
    byte[] b = getFingerprintRaw(pubKey);
    return SerializationUtils.byteArrayToHexString(b);
  }

  @Override public byte[] getFingerprintRaw(PublicKey pubKey) throws OtrCryptoException {
    byte[] b;
    try {
