...
public abstract class AbstractRedisStorage<T extends JedisCommands> {
  ...
  protected final int TRIGGER_LOCK_TIMEOUT = 10 * 60 * 1000;
  ...
  protected boolean applyMisfire(OperableTrigger trigger, T jedis) throws JobPersistenceException {
    ...
    if (nextFireTime == null || nextFireTime.getTime() > misfireTime || trigger.getMisfireInstruction() == Trigger.MISFIRE_INSTRUCTION_IGNORE_MISFIRE_POLICY) {
      return false;
    }
    ...
    if (trigger.getNextFireTime() == null) {
      ...
      if (nextFireTime.equals(trigger.getNextFireTime())) {
        return false;
      }
    }
    return true;
  ...
  protected boolean isBlockedJob(String jobHashKey, T jedis) {
    ...
    return jedis.sismember(redisSchema.blockedJobsSet(), jobHashKey) && isActiveInstance(jedis.get(redisSchema.jobBlockedKey(jobKey)), jedis);
  }
