...
public class OAReplyProvider extends IQProvider<IQ> {
  ...
  @Override public IQ parse(XmlPullParser parser, int initialDepth) throws Exception {
    ...
    if (!replyParser.validResponseCode(statusCode)) {
      throw new HarmonyProtocolException(format("Got error response [%s]: %s", statusCode, attrs.get("errorstring")));
    }
