...
abstract class AbstractChecksumMojo extends AbstractMojo {
  ...
  /**
     ...
     * @since 1.0
     */
  @Parameter protected List<String> algorithms = Arrays.asList(Constants.DEFAULT_EXECUTION_ALGORITHMS);
  ...
  /**
     ...
     * @since 1.0
     */
  @Parameter(property = "encoding", defaultValue = "${project.build.sourceEncoding}") protected String encoding = Constants.DEFAULT_ENCODING;
  ...
  /**
     ...
     * @since 1.3
     */
  @Parameter(property = "attachChecksums", defaultValue = "false") protected boolean attachChecksums;
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    if (isIndividualFiles()) {
      ...
      if (StringUtils.isNotEmpty(getIndividualFilesOutputDirectory())) {
        outputDirectory = FileUtils.resolveFile(new File(project.getBuild().getDirectory()), getIndividualFilesOutputDirectory());
      }
      execution.addTarget(new OneHashPerFileTarget(encoding, outputDirectory, createArtifactListeners()));
    }
    if (isCsvSummary()) {
      execution.addTarget(new CsvSummaryFileTarget(FileUtils.resolveFile(new File(project.getBuild().getDirectory()), getCsvSummaryFile()), encoding, createArtifactListeners()));
    }
    if (isXmlSummary()) {
      execution.addTarget(new XmlSummaryFileTarget(FileUtils.resolveFile(new File(project.getBuild().getDirectory()), getXmlSummaryFile()), encoding, createArtifactListeners()));
    }
    if (isShasumSummary()) {
      execution.addTarget(new ShasumSummaryFileTarget(FileUtils.resolveFile(new File(project.getBuild().getDirectory()), getShasumSummaryFile())));
    }
    try {
      execution.run();
