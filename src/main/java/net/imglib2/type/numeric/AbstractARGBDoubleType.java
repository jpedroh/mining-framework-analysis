...
abstract public class AbstractARGBDoubleType<T extends AbstractARGBDoubleType<T>> implements NumericType<T> {
  ...
  @Override public void add(final T c) {
    set(getA() + c.getA(), getR() + c.getR(), getG() + c.getG(), getB() + c.getB());
  }
  ...
  @Override public void pow(final T c) {
    set(Math.pow(getA(), c.getA()), Math.pow(getR(), c.getR()), Math.pow(getG(), c.getG()), Math.pow(getB(), c.getB()));
  }

  @Override public void pow(final double power) {
    set(Math.pow(getA(), power), Math.pow(getR(), power), Math.pow(getG(), power), Math.pow(getB(), power));
  }
  ...
  @Override public void mul(final float c) {
    set(getA() * c, getR() * c, getG() * c, getB() * c);
  }
  ...
  @Override public boolean valueEquals(T t) {
