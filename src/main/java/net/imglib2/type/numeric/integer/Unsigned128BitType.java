 ...
 * the math methods defined in the superinterface {@link NumericType} are
 * implemented using {@link BigInteger} and {@link BigDecimal}. This class is
 * not {@link Thread}-safe; do a {@link #copy()} first to operate on a different
 * {@link Thread}.
 *
...
public class Unsigned128BitType extends AbstractIntegerType<Unsigned128BitType> implements NativeType<Unsigned128BitType> {
  ...
  @Override public NativeTypeFactory<Unsigned128BitType, LongAccess> getNativeTypeFactory() {
    return typeFactory;
  }
  ...
  /**
	 ...
	 * {@link BigInteger#toByteArray()}. Only the last 16 bytes are read, if
	 * there are more.
	 */
  ...
  @Override public double getRealDouble() {
    ...
    return UnsignedLongType.unsignedLongToDouble(lower) + UnsignedLongType.unsignedLongToDouble(upper) * Math.pow(2, 64);
  }
  ...
  @Override public void setReal(float real) {
    setReal((double) real);
  }

  @Override public void setReal(double real) {
    real = Math.floor(real + 0.5);
    final double base = Math.pow(2, 64);
    double upper = Math.floor(real / base);
    double lower = real - base * upper;
    set(UnsignedLongType.doubleToUnsignedLong(lower), UnsignedLongType.doubleToUnsignedLong(upper));
  }
  ...
  /**
	 * The true maximum value, unlike {@link #getMaxValue()} which cannot
	 * represent it in a {@code double}.
	 */
  ...
  /**
	 * Implemented using {@link BigDecimal#multiply(BigDecimal)} and
  ...
  @Override public void pow(final Unsigned128BitType t) {
