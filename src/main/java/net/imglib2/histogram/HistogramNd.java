...
public class HistogramNd<T extends java.lang.Object> implements Img<LongType> {
  ...
  @SuppressWarnings(value = { "synthetic-access" }) private final Incrementer incrementer = new Incrementer();

  @SuppressWarnings(value = { "synthetic-access" }) private final Decrementer decrementer = new Decrementer();
  ...
  /**
	 ...
	 */
  @Override public void dimensions(final long[] dims) {
    distrib.dimensions(dims);
  }

  @Override public RandomAccess<LongType> randomAccess() {
    return distrib.randomAccess();
  }

  @Override public RandomAccess<LongType> randomAccess(final Interval interval) {
  ...
  @Override public void min(final long[] min) {
    distrib.min(min);
  }

  @Override public void min(final Positionable min) {
    distrib.min(min);
  }
  ...
  @Override public void max(final long[] max) {
    distrib.max(max);
  }

  @Override public void max(final Positionable max) {
    distrib.max(max);
  }
  ...
  @Override public void realMin(final double[] min) {
    distrib.realMin(min);
  }

  @Override public void realMin(final RealPositionable min) {
    distrib.realMin(min);
  }
  ...
  @Override public void realMax(final double[] max) {
    distrib.realMax(max);
  }

  @Override public void realMax(final RealPositionable max) {
    distrib.realMax(max);
  }

  @Override public Cursor<LongType> cursor() {
    return distrib.cursor();
  }

  @Override public Cursor<LongType> localizingCursor() {
  ...
  @Override public LongType firstElement() {
  ...
  @Override public Iterator<LongType> iterator() {
    return distrib.iterator();
  }

  @Override public ImgFactory<LongType> factory() {
    return distrib.factory();
  }

  @Override public HistogramNd<T> copy() {
  ...
  private class Decrementer implements Counter {
    @SuppressWarnings(value = { "synthetic-access" }) @Override public void count(final long[] position, final boolean ignored) {
      if (ignored) {
        ignoredCount--;
      } else {
