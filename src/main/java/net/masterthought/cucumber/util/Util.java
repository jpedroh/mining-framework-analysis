...
public final class Util {
  ...
  private static final PeriodFormatter TIME_FORMATTER = new PeriodFormatterBuilder().appendDays().appendSuffix("d").appendSeparator(" ").appendHours().appendSuffix("h").appendSeparator(" ").appendMinutes().appendSuffix("m").appendSeparator(" ").appendSeconds().appendSuffix("s").appendSeparator(" ").minimumPrintedDigits(3).appendMillis().appendSuffix("ms").toFormatter();
      ...
      if (element.isScenario() && (!element.getStatus().isPassed())) {
        ...
        for (Step step : scenario.getSteps()) {
          if (step.getResult().getStatus().equals(Status.FAILED)) {
            ...
            String[] info = { scenario.getName(), step.getName(), step.getId(), errorMessage == null ? "Error message not found." : errorMessage };
