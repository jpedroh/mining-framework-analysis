...
public class ServerTask extends AbstractTask {
              ...
              if ("stop".equals(operation)) {
                ...
                if ("status".equals(operation)) {
                  ...
                  if ("debug".equals(operation)) {
                    ...
                    if ("package".equals(operation)) {
                      doPackage();
                    } else {
                      if ("dump".equals(operation)) {
                        doDump();
                      } else {
                        if ("javadump".equals(operation)) {
                          doJavaDump();
                        } else {
  ...
  private void validateServerStarted(File outputFile, long startTimeout) throws Exception {
    ...
    log("Waiting up to " + (startTimeout / 1000) + " seconds for server confirmation:  " + START_MESSAGE_CODE.toString() + " to be found in " + outputFile);
    try {
