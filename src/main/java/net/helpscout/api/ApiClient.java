...
public class ApiClient {
  ...
  public void updateConversation(Conversation conversation) throws ApiException {
    GsonBuilder builder = new GsonBuilder().setDateFormat("yyyy-MM-dd\'T\'HH:mm:ss\'Z\'").registerTypeAdapter(ThreadState.class, new ThreadStateAdapter()).registerTypeAdapter(Status.class, new StatusAdapter()).registerTypeAdapter(CustomFieldResponse.class, new CustomFieldResponseAdapter());
  ...
  private void setThreadProperties(ConversationThread thread) {
    ...
    if (theThread.getClass().isAssignableFrom(BaseLineItem.class)) {
      ...
      if (theThread.getClass().isAssignableFrom(Message.class)) {
        thread.setType(ThreadType.Message);
      } else {
        if (theThread.getClass().isAssignableFrom(Note.class)) {
          thread.setType(ThreadType.Note);
        } else {
          if (theThread.getClass().isAssignableFrom(Customer.class)) {
            thread.setType(ThreadType.Customer);
          } else {
            if (theThread.getClass().isAssignableFrom(ForwardParent.class)) {
              thread.setType(ThreadType.ForwardParent);
            } else {
              if (theThread.getClass().isAssignableFrom(ForwardChild.class)) {
                thread.setType(ThreadType.ForwardChild);
              } else {
                if (theThread.getClass().isAssignableFrom(Chat.class)) {
...
=======
  @SneakyThrows private String setParams(String url, Map<String, String> params) {
...
>>>>>>> /usr/src/app/output/helpscout/helpscout-api-java/e2841d19c8084fd9eea601c54cbce98896fd770a/src/main/java/net/helpscout/api/ApiClient.java/right.java


  private <T extends java.lang.Object> T getObject(String url, Class<T> clazzType) throws ApiException {
    String json = doGet(url, HTTP_STATUS_OK);
    return Parser.getInstance().getObject(json, clazzType);
  }

  private <T extends java.lang.Object> T getItem(String url, Class<T> clazzType, int expectedCode) throws ApiException {
  ...
  private <T extends java.lang.Object> Page<T> getPage(String url, Class<T> clazzType, int expectedCode) throws ApiException {
    return getPage(url, null, clazzType, expectedCode);
  }

  private <T extends java.lang.Object> Page<T> getPage(String url, Class<T> clazzType, String wrapperObjectName) throws ApiException {
    String json = doGet(url, HTTP_STATUS_OK);
  ...
  private <T extends java.lang.Object> Page<T> getPage(String url, Map<String, String> params, Class<T> clazzType, int expectedCode) throws ApiException {
  ...
  private <T extends java.lang.Object> T doPost(String url, String requestBody, int expectedCode, ResultExtractor<T> extractor) throws ApiException {
