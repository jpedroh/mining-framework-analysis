...
public class Plate extends Photo {
  ...
  private static int horizontalDetectionType = Configurator.getConfigurator().getIntProperty("platehorizontalgraph_detectionType");

  private Plate plateCopy;
  ...
  public Vector<Char> getChars() {
    ...
    for (int i = 0; i < peaks.size(); i++) {
      ...
      out.add(new Char(getImage().getSubimage(p.getLeft(), 0, p.getDiff(), getImage().getHeight()), this.plateCopy.getImage().getSubimage(p.getLeft(), 0, p.getDiff(), getImage().getHeight()), new PositionInPlate(p.getLeft(), p.getRight())));
    }
    return out;
  }

  @Override public Plate clone() throws CloneNotSupportedException {
    super.clone();
  ...
  @Override public void verticalEdgeDetector(BufferedImage source) {
    float[] matrix = { -1, 0, 1 };
