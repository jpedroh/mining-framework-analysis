...
public class MagicMatcher implements Cloneable {
  ...
  public boolean isValid() {
    ...
    if ((type != null) && !type.equals("") && (comparator != '\u0000') && ((comparator == '=') || (comparator == '!') || (comparator == '>') || (comparator == '<')) && (description != null) && !description.equals("") && (test != null) && !test.equals("")) {
      return true;
    }
    return false;
  }
  ...
  public MagicMatch test(File f, boolean onlyMimeMatch) throws IOException, UnsupportedTypeException {
    ...
    try {
      ...
      if (type.equals("byte")) {
        ...
        if (type.equals("short") || type.equals("leshort") || type.equals("beshort")) {
          length = 4;
        } else {
          if (type.equals("long") || type.equals("lelong") || type.equals("belong")) {
            length = 8;
          } else {
      ...
      if (testInternal(buf)) {
        ...
        if ((onlyMimeMatch == false) && (subMatchers != null) && (subMatchers.size() > 0)) {
          log.debug("test(File): testing " + subMatchers.size() + " submatches for \'" + description + "\'");
  ...
  private boolean testString(ByteBuffer data) {
    ...
    for (i = 0; i < t.length; i++) {
      log.debug("testing byte \'" + b[i] + "\' from \'" + new String(data.array()) + "\' against byte \'" + t[i] + "\' from \'" + new String(test.array()) + "\'");
      if (t[i] != b[i]) {
        diff = true;
        break;
      }
  ...
  private boolean testLong(ByteBuffer data) {
    ...
    log.debug("testLong(): testing \'" + Long.toHexString(val) + "\' against \'" + test + "\' => \'" + Long.toHexString(tst) + "\'");
    switch (comparator) {
      case '=':
      return val == tst;
      case '!':
