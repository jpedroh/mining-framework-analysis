...
public class VanillaBytes<U> extends AbstractBytes<U> implements Byteable<Bytes<U>, U> , Comparable<CharSequence> {
    @java.lang.Deprecated
    public VanillaBytes(@NotNull
    BytesStore bytesStore) throws IllegalStateException, IllegalArgumentException {
        this(bytesStore, bytesStore.writePosition(), bytesStore.writeLimit());
    }

    protected VanillaBytes(@NotNull
    BytesStore bytesStore, long writePosition, long writeLimit) throws IllegalStateException, IllegalArgumentException {
    ...
    final BytesStore<Bytes<U>, U> byteStore, final long offset, final long length) throws IllegalStateException, IllegalArgumentException, BufferUnderflowException {
    ...
    private void setBytesStore(@NotNull
    BytesStore<Bytes<U>, U> bytesStore) throws IllegalStateException, IllegalArgumentException {
    ...
    @NotNull
    @Override
    public Bytes<U> bytesForRead() throws IllegalStateException {
        throwExceptionIfReleased();
        try {
            return isClear() ? new VanillaBytes<>(bytesStore, writePosition(), bytesStore.writeLimit()) : new SubBytes<>(bytesStore, readPosition(), readLimit());
        } catch (java.lang.IllegalArgumentException | BufferUnderflowException e) {
            throw new AssertionError(e);
        }
    }

    @Override
    public boolean isEqual(@Nullable
    String other) throws IllegalStateException {
        if ((other == null) || (other.length() != readRemaining())) {
            return false;
        }
    ...
    RandomDataInput bytes, long offset, long length) throws BufferOverflowException, BufferUnderflowException, IllegalStateException, IllegalArgumentException {
    ...
    public void write(long position, @NotNull
    CharSequence str, int offset, int length) throws BufferOverflowException, IllegalArgumentException, ArithmeticException, IllegalStateException, BufferUnderflowException {
        requireNonNull(str);
        ensureCapacity(writePosition() + length);
    ...
    @Override
    @NotNull
    public VanillaBytes append(@NotNull
    CharSequence str, int start, int end) throws IndexOutOfBoundsException {
    ...
    @NotNull
    @Override
    public VanillaBytes appendUtf8(@NotNull
    CharSequence str) throws BufferOverflowException {
        requireNonNull(str);
        ReportUnoptimised.reportOnce();
        try {
            if (isDirectMemory()) {
    ...
    CharSequence cs) throws BufferOverflowException, BufferUnderflowException, IndexOutOfBoundsException, IllegalStateException {
        requireNonNull(cs);
    ...
    @NotNull
    public Bytes<U> append8bit(@NotNull
    BytesStore bs) throws BufferOverflowException, BufferUnderflowException, IllegalStateException {
    ...
    @Override
    public Bytes<U> write(@NotNull
    BytesStore bytes, long offset, long length) throws BufferOverflowException, BufferUnderflowException, IllegalStateException, IllegalArgumentException {
        requireNonNull(bytes);
        ensureCapacity(writePosition() + length);
    ...
    @NotNull
    public Bytes<U> append8bit(@NotNull
    String cs) throws BufferOverflowException, IllegalStateException {
        requireNonNull(cs);
        if (isDirectMemory()) {
    ...
    @NotNull
    private Bytes<U> append8bitNBS_S(@NotNull
    String s) throws BufferOverflowException, IllegalStateException {
        int length = s.length();
    ...
    public String toString() {
        ...
        reserve(this);
        try {
            return bytesStore instanceof NativeBytesStore ? toString2(((NativeBytesStore) (bytesStore))) : toString0();
    ...
    private String toString2(@NotNull
    NativeBytesStore bytesStore) {
        @Nullable
        final Memory memory = bytesStore.memory;
        int length = ((int) (Math.min(Bytes.MAX_HEAP_CAPACITY, realReadRemaining())));
        @NotNull
    ...
    @NotNull
    protected Bytes<U> append8bit0(@NotNull
    CharSequence cs) throws BufferOverflowException, IllegalStateException {
    ...
    @Override
    public boolean equalBytes(@NotNull
    BytesStore bytesStore, long length) throws BufferUnderflowException, IllegalStateException {
    ...
    public void read8Bit(char[] chars, int length) throws BufferUnderflowException, IllegalStateException {
        ...
        // TODO: protected?
    @Override
    public int byteCheckSum(int start, int end) throws IORuntimeException, BufferUnderflowException {
        byte b = 0;
            // the below cast is safe as should only be called from net.openhft.chronicle.bytes.AbstractBytes.byteCheckSum(long, long)
        @Nullable
        final NativeBytesStore bytesStore = ((NativeBytesStore) (bytesStore()));
        @Nullable
        final Memory memory = bytesStore.memory;
        assert memory != null;
        ...
        for (; i < (len - 3); i += 4) {
            b += ((memory.readByte(addr + i) + memory.readByte((addr + i) + 1)) + memory.readByte((addr + i) + 2)) + memory.readByte((addr + i) + 3);
        }
        for (; i < len; i++) {
    ...
    char[] chars, int offset, int length) throws BufferOverflowException, IllegalStateException, BufferUnderflowException, IllegalArgumentException {
