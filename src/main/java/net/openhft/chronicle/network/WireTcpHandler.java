...
public abstract class WireTcpHandler<T extends NetworkContext> implements TcpHandler, NetworkContextManager<T> {
  ...
  @NotNull protected Wire outWire;
  ...
  @NotNull private Wire inWire;

  private boolean recreateWire;

  @Nullable private WireType wireType;
  ...
  @Override public void process(@NotNull Bytes in, @NotNull Bytes out) {
    if (closed) {
      return;
    }
  ...
  private void logYaml(long start) {
    if (YamlLogging.showServerReads() && !inWire.bytes().isEmpty()) {
      String s = Wires.fromSizePrefixedBlobs(inWire.bytes(), start, inWire.bytes().readLimit());
  ...
  /**
     ...
     */
  protected abstract void onRead(@NotNull DocumentContext in, @NotNull WireOut out);
