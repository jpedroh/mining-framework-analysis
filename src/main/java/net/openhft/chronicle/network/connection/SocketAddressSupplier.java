...
public class SocketAddressSupplier implements Supplier<SocketAddress> {
  ...
  @Nullable private RemoteAddressSupplier current;
  ...
  /**
     ...
     */
  @NotNull public static SocketAddressSupplier uri(String connectURI) {
    return new SocketAddressSupplier(new String[] { connectURI }, "");
  }

  @NotNull public List<RemoteAddressSupplier> all() {
  ...
  @Nullable @Override public InetSocketAddress get() {
  ...
  @NotNull public String log(RemoteAddressSupplier current) {
    if (current == null) {
      return "(none)";
    }
