...
class TcpEventHandler implements EventHandler, Closeable {
  public static final int TCP_BUFFER = Integer.getInteger("TcpEventHandler.tcpBufferSize", 8 << 20);
  ...
  @NotNull private final SocketChannel sc;
  ...
  @NotNull private final WriteEventHandler writeEventHandler;

  @NotNull private final NetworkLog readLog, writeLog;

  int oneInTen;

  @Nullable private ByteBuffer inBB = ByteBuffer.allocateDirect(CAPACITY);

  @Nullable private Bytes inBBB;

  @Nullable private ByteBuffer outBB = ByteBuffer.allocateDirect(CAPACITY);

  @Nullable private Bytes outBBB;

  private long lastTickReadTime = Time.tickTime(), lastHeartBeatTick = lastTickReadTime + 1000;

  public TcpEventHandler(@NotNull SocketChannel sc, @NotNull TcpHandler handler, @NotNull final SessionDetailsProvider sessionDetails, boolean unchecked, long heartBeatIntervalTicks, long heartBeatTimeoutTicks) throws IOException {
