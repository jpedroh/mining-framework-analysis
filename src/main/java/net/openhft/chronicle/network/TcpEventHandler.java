...
public class TcpEventHandler<T extends NetworkContext<T>> extends AbstractCloseable implements EventHandler, TcpEventHandlerManager<T> {
  ...
  @NotNull private final ChronicleSocketChannel sc;

  private final String scToString;

  @NotNull private final T nc;

  @NotNull private final NetworkLog readLog, writeLog;

  @NotNull private final Bytes<ByteBuffer> inBBB;

  @NotNull private final Bytes<ByteBuffer> outBBB;
  ...
  @Nullable private volatile TcpHandler<T> tcpHandler;
  ...
  @Override public void eventLoop(final EventLoop eventLoop) {
  ...
  @Override public void resetUsedByThread() {
  ...
  @Override public String toString() {
    return "TcpEventHandler{" + "sc=" + scToString + ", " + "tcpHandler=" + tcpHandler + ", " + "closed=" + isClosed() + '}';
  }
  ...
  @NotNull public HandlerPriority singleThreadedPriority() {
    return HandlerPriority.MEDIUM;
  }

  @Nullable public TcpHandler<T> tcpHandler() {
    return tcpHandler;
  }

  @Override public void tcpHandler(final TcpHandler<T> tcpHandler) {
  ...
  @PackageLocal boolean invokeHandler() throws IOException {
    Jvm.safepoint();
    boolean busy = false;
    ...
    if (inBBB.readRemaining() == 0) {
      ...
      if (inBBB.readPosition() > TCP_BUFFER / 4) {
        compactBuffer();
        busy = true;
      }
    }
  ...
  private void handleIOE(@NotNull final IOException e, final boolean clientIntentionallyClosed, @Nullable final HeartbeatListener heartbeatListener) {
    try {
  ...
  @PackageLocal boolean tryWrite(final ByteBuffer outBB) throws IOException {
  ...
  @FunctionalInterface public interface SocketReader {
    /**
  ...
  public static final class DefaultSocketReader implements SocketReader {
    @Override public int read(@NotNull final ChronicleSocketChannel socketChannel, @NotNull final Bytes<ByteBuffer> bytes) throws IOException {
  ...
  public static class Factory<T extends NetworkContext<T>> implements MarshallableFunction<T, TcpEventHandler<T>> {
    ...
    @NotNull @Override public TcpEventHandler<T> apply(@NotNull final T nc) {
  ...
  private static final class ThreadLogTypeElapsedRecord {
    ...
    public ThreadLogTypeElapsedRecord(@NotNull final LogType logType, final long elapsedNs) {
  ...
  private final class StatusMonitorEventHandler implements EventHandler {
    ...
    @Override public boolean action() throws InvalidEventHandlerException {
      ...
      if (!logs.isEmpty()) {
        ...
        while ((msg = logs.poll()) != null) {
          ...
          messageBuilder.append("Non blocking ").append(className).append(" ").append(msg.logType.label()).append(" took ").append(msg.elapsedNs / 1000).append(" us");
      ...
      if (now > lastMonitor + (MONITOR_POLL_EVERY_SEC * 1000)) {
        ...
        if (networkStatsListener != null && !networkStatsListener.isClosed()) {
          if (lastMonitor == 0) {
            ...
            networkStatsListener.onNetworkStats(bytesWriteCount.get() / MONITOR_POLL_EVERY_SEC, bytesReadCount.get() / MONITOR_POLL_EVERY_SEC, socketPollCount.get() / MONITOR_POLL_EVERY_SEC);
    ...
    @Override public @NotNull HandlerPriority priority() {
