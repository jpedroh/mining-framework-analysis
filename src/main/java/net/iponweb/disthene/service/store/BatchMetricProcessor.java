      ...
      Thread flusher = flusherFactory.newThread(new Runnable() {
        @Override public void run() {
          ...
          while (true) {
            ...
            if (metric != null) {
              batch.add(statement.bind(metric.getRollup() * metric.getPeriod(), Collections.singletonList(metric.getValue()), metric.getTenant(), metric.getRollup(), metric.getPeriod(), metric.getPath(), metric.getUnixTimestamp()));
              currentBatchSize++;
