...
public class ResponseFormatter {
  public static FullHttpResponse formatResponse(List<TimeSeries> timeSeriesList, RenderParameters parameters) throws UnsupportedOperationException, LogarithmicScaleNotAllowed {
    ...
    switch (parameters.getFormat()) {
      ...
      case RAW:
      return formatResponseAsRaw(filtered);
      case CSV:
      return formatResponseAsCSV(filtered, parameters);
      case PNG:
      return formatResponseAsPng(filtered, parameters);
      case GRAPHPLOT_JSON:
      return formatResponseAsGraphplotJson(filtered, parameters);
      default:
  ...
  private static FullHttpResponse formatResponseAsCSV(List<TimeSeries> timeSeriesList, RenderParameters renderParameters) {
    ...
    FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.OK, Unpooled.wrappedBuffer(responseString.getBytes()));
  ...
  private static FullHttpResponse formatResponseAsPng(List<TimeSeries> timeSeriesList, RenderParameters renderParameters) throws LogarithmicScaleNotAllowed {
    FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.OK, Unpooled.wrappedBuffer(Graph.getInstance(renderParameters.getImageParameters().getGraphType(), renderParameters, timeSeriesList).drawGraph()));
