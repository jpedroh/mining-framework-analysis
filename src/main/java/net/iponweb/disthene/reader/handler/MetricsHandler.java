...
public class MetricsHandler implements DistheneReaderHandler {
  ...
  @Override public FullHttpResponse handle(HttpRequest request) throws ParameterParsingException, ExecutionException, InterruptedException, TooMuchDataExpectedException, IOException {
    MetricsParameters parameters = parse(request);
    logger.debug("Got request: " + parameters);
    FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.OK, Unpooled.wrappedBuffer(metricService.getMetricsAsJson(parameters.getTenant(), parameters.getPath(), parameters.getFrom(), parameters.getTo()).getBytes()));
  ...
  private MetricsParameters parse(HttpRequest request) throws ParameterParsingException {
    ...
    if (request.method().equals(HttpMethod.GET)) {
      ...
      if (request.method().equals(HttpMethod.POST)) {
        MetricsParameters parameters = new Gson().fromJson(((HttpContent) request).content().toString(Charset.defaultCharset()), MetricsParameters.class);
  ...
  private static class MetricsParameters {
    ...
    @Override public String toString() {
      return "MetricsParameters{" + "tenant=\'" + tenant + '\'' + ", path=" + path + ", from=" + from + ", to=" + to + '}';
    }
  }
}