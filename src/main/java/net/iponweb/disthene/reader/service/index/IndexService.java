...
public class IndexService {
  ...
  public IndexService(IndexConfiguration indexConfiguration) {
    ...
    Settings settings = ImmutableSettings.settingsBuilder().put("cluster.name", indexConfiguration.getName()).build();
  ...
  public Map<String, String> getPaths(String tenant, List<String> wildcards) throws TooMuchDataExpectedException {
    ...
    SearchResponse response = client.prepareSearch(indexConfiguration.getIndex()).setScroll(new TimeValue(indexConfiguration.getTimeout())).setSize(indexConfiguration.getScroll()).setQuery(QueryBuilders.filteredQuery(QueryBuilders.regexpQuery("path", regEx), FilterBuilders.termFilter("tenant", tenant))).addField("path").addField("origin").execute().actionGet();
...
>>>>>>> /usr/src/app/output/einsamhauer/disthene-reader/28a8ab2adb083fdca0b512255193aeb06dc1726a/src/main/java/net/iponweb/disthene/reader/service/index/IndexService.java/right.java
    ...
    while (response.getHits().getHits().length > 0) {
      ...
      response = client.prepareSearchScroll(response.getScrollId()).setScroll(new TimeValue(indexConfiguration.getTimeout())).execute().actionGet();
    }
    return result;
  }
  ...
  public String getPathsAsJsonArray(String tenant, String wildcard) throws TooMuchDataExpectedException {
    ...
    SearchResponse response = client.prepareSearch(indexConfiguration.getIndex()).setScroll(new TimeValue(indexConfiguration.getTimeout())).setSize(indexConfiguration.getScroll()).setQuery(QueryBuilders.filteredQuery(QueryBuilders.regexpQuery("path", regEx), FilterBuilders.termFilter("tenant", tenant))).execute().actionGet();
    if (response.getHits().totalHits() > indexConfiguration.getMaxPaths()) {
      logger.debug("Total number of paths exceeds the limit: " + response.getHits().totalHits());
      throw new TooMuchDataExpectedException("Total number of paths exceeds the limit: " + response.getHits().totalHits() + " (the limit is " + indexConfiguration.getMaxPaths() + ")");
    }
  ...
  public String getSearchPathsAsString(String tenant, String regEx, int limit) {
    SearchResponse response = client.prepareSearch(indexConfiguration.getIndex()).setScroll(new TimeValue(indexConfiguration.getTimeout())).setSize(limit).setQuery(QueryBuilders.filteredQuery(QueryBuilders.regexpQuery("path", regEx), FilterBuilders.termFilter("tenant", tenant))).addField("path").execute().actionGet();
    List<String> paths = new ArrayList<>();
    for (SearchHit hit : response.getHits()) {
