...
>>>>>>> /usr/src/app/output/einsamhauer/disthene/8c23ce22f44dfa88b1093a590f68cddbb7856da7/src/main/java/net/iponweb/disthene/Disthene.java/right.java
  ...
  private void run() {
    try {
      ...
      bus = new MBassador<>(new BusConfiguration().addFeature(Feature.SyncPubSub.Default()).addFeature(Feature.AsynchronousHandlerInvocation.Default()).addFeature(Feature.AsynchronousMessageDispatch.Default()).addPublicationErrorHandler((error) -> logger.error(error)));
...
=======
      @SuppressWarnings(value = { "unused" }) MetricService metricService = new MetricService(bus, blacklistService);
>>>>>>> /usr/src/app/output/einsamhauer/disthene/8c23ce22f44dfa88b1093a590f68cddbb7856da7/src/main/java/net/iponweb/disthene/Disthene.java/right.java
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      new Disthene(commandLine.getOptionValue("c", DEFAULT_CONFIG_LOCATION), commandLine.getOptionValue("b", DEFAULT_BLACKLIST_LOCATION), commandLine.getOptionValue("w", DEFAULT_WHITELIST_LOCATION), commandLine.getOptionValue("a", DEFAULT_AGGREGATION_CONFIG_LOCATION)).run();
  ...
  private class SighupSignalHandler implements SignalHandler {
    @Override public void handle(Signal signal) {
      ...
      try {
        ...
        logger.debug("Reloaded allowed tenants: {allowAll=" + distheneConfiguration.getCarbon().isAllowAll() + ", tenants=" + distheneConfiguration.getCarbon().getAuthorizedTenants());
      } catch (Exception e) {
