...
=======
    if (req.getType().equals(ClanRequest.START_WAR)) {
      ...
      if (req.getType().equals(ClanRequest.END_WAR)) {
        Clan clan = req.getClan();
        Clan war = plugin.getClanManager().getClan(req.getTarget());
        ClanPlayer cp = req.getRequester();
        if (war != null && clan != null) {
        ...
        if (req.getType().equals(ClanRequest.CREATE_ALLY)) {
          ...
          if (req.getType().equals(ClanRequest.BREAK_RIVALRY)) {
            Clan clan = req.getClan();
            ...
            if (req.votingFinished()) {
              ...
              if (req.getType().equals(ClanRequest.DEMOTE)) {
                ...
                if (req.getType().equals(ClanRequest.PROMOTE)) {
                  Clan clan = req.getClan();
                  ...
                  if (req.getType().equals(ClanRequest.DISBAND)) {
                    Clan clan = req.getClan();
                    if (denies.isEmpty()) {
...
>>>>>>> /usr/src/app/output/phaed420/simpleclans/a30dbb49985fd1da095e49ef9afe95b8857d0377/src/main/java/net/sacredlabyrinth/phaed/simpleclans/managers/RequestManager.java/right.java
  ...
  private void processDemote(Request req, Clan requesterClan, UUID targetPlayer, List<String> denies) {
    ...
    if (denies.isEmpty()) {
      requesterClan.addBb(plugin.getLang("leaders"), ChatColor.AQUA + MessageFormat.format(plugin.getLang("demoted.back.to.member"), demotedName));
      requesterClan.demote(targetPlayer);
    } else {
      String deniers = Helper.capitalize(Helper.toMessage(Helper.toArray(denies), ", "));
      requesterClan.leaderAnnounce(ChatColor.RED + MessageFormat.format(plugin.getLang("denied.demotion"), deniers, demotedName));
    }
  }

  private void processBreakRivalry(Clan requesterClan, ClanPlayer requesterCp, Clan targetClan, List<String> accepts, List<String> denies) {
  ...
  private void processCreateAlly(Clan requesterClan, ClanPlayer requesterCp, Clan targetClan, List<String> accepts, List<String> denies) {
    if (targetClan != null && requesterClan != null) {
      if (!accepts.isEmpty()) {
  ...
  private void processEndWar(Clan requesterClan, ClanPlayer requesterCp, Clan targetClan, List<String> accepts, List<String> denies) {
  ...
  private void processStartWar(Clan requesterClan, ClanPlayer requesterCp, Clan targetClan, List<String> accepts, List<String> denies) {
    if (requesterClan != null && targetClan != null) {
      if (!accepts.isEmpty()) {
