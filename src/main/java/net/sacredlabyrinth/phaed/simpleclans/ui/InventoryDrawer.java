...
>>>>>>> /usr/src/app/output/phaed420/simpleclans/2244703564dec7ffa4bb40114daae24341080275/src/main/java/net/sacredlabyrinth/phaed/simpleclans/ui/InventoryDrawer.java/right.java
  ...
  @NotNull private static String getSafeTitle(@NotNull SCFrame frame) {
  ...
  @NotNull private static Inventory prepareInventory(@NotNull SCFrame frame) {
    ...
    if (plugin.getSettingsManager().isDebugging()) {
      plugin.getLogger().log(Level.INFO, String.format("It took %s millisecond(s) to load the frame %s for %s", System.currentTimeMillis() - start, frame.getTitle(), frame.getViewer().getName()));
    }
    return inventory;
  }

  @Deprecated public static void update(@NotNull SCFrame frame) {
