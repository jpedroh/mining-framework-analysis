...
public class LeaderboardFrame extends SCFrame {
  ...
  @Override public void createComponents() {
    ...
    for (int i = paginator.getMinIndex(); paginator.isValidIndex(i); i++) {
      ...
      SCComponent c = new SCComponentImpl(lang("gui.leaderboard.player.title", i + 1, cp.getName()), Arrays.asList(cp.getClan() == null ? lang("gui.playerdetails.player.lore.noclan") : lang("gui.playerdetails.player.lore.clan", cp.getClan().getColorTag(), cp.getClan().getName()), lang("gui.playerdetails.player.lore.kdr", Helper.formatKDR(cp.getKDR())), lang("gui.playerdetails.player.lore.last.seen", cp.getLastSeenString())), Material.SKULL_ITEM, (byte) 3, slot);
