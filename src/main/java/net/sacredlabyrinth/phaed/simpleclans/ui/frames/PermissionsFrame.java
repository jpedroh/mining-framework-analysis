...
public class PermissionsFrame extends SCFrame {
  ...
  @Override public void createComponents() {
    ...
    SCComponent add = new SCComponentImpl(lang("gui.permissions.add.title"), null, Material.WOOL, 4);
    ...
    for (int i = paginator.getMinIndex(); paginator.isValidIndex(i); i++) {
      ...
      SCComponent c = new SCComponentImpl(lang("gui.permissions.permission.title", permission), Collections.singletonList(lang("gui.permissions.permission.lore")), Material.PAPER, slot);
      c.setListener(ClickType.RIGHT, () -> InventoryController.runSubcommand(getViewer(), String.format("rank permissions %s remove %s", rank.getName(), permission), true));
