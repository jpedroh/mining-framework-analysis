...
public final class CallbackMethodConfigureScanner {
  ...
  public void scanMethod() throws VerificationException {
    if (klass.isInterface() && null == klass.getAnnotation(LifecycleMeta.class)) {
      return;
    }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  public boolean onMethodFound(Method method, int inheritanceLevel) throws VerificationException {
    ...
    configurePostStateChange(mw, method.getAnnotation(PostStateChange.class));
    configureOnEvent(method, method.getAnnotation(OnEvent.class));
    if (stateMachineObjectBuilderImpl.hasLifecycleOverrideAnnotation(method)) {
      lifecycleOverridenCallbackDefinitionSet.add(method.getName());
    }
    return false;
  }

  private void configureCallbacks(final MethodWrapper methodWrapper, final Callbacks callbacks) throws VerificationException {
    if (null == callbacks) {
      return;
    }
    ...
    for (final OnEvent item : callbacks.onEvent()) {
      configureOnEvent(method, item);
    }
  }

  private void configurePreStateChange(final MethodWrapper methodWrapper, final PreStateChange preStateChange) throws VerificationException {
    ...
    if (isRelationalCallback(observableName, observableClass)) {
      ...
      final StateMachineObject<?> callBackEventSourceContainer = this.stateMachineObjectBuilderImpl.getRegistry().loadStateMachineObject(actualObservableClass);
      ...
      final Readable<?> accessor = evaluateAccessor(mappedBy, actualObservableClass);
      configurePreStateChangeRelationalCallbackObjects(methodWrapper, from, to, callBackEventSourceContainer, accessor);
    } else {
      configurePreStateChangeNonRelationalCallbackObjects(methodWrapper, from, to);
    }
  }

  private boolean isMappedByInvalid(final String mappedBy, final Class<?> actualObservableClass) {
    return null == mappedBy || CallbackConsts.NULL_STR.equalsIgnoreCase(mappedBy) || null == convertRelationKey(actualObservableClass, mappedBy);
  }

  private Class<?> evaluateActualObservableClassOfPreStateChange(final Method method, final String observableName, final Class<?> observableClass) throws VerificationException {
    ...
    if (!CallbackConsts.NULL_STR.equals(observableName) && Null.class != observableClass) {
      ...
      if (!observableClass.isAssignableFrom(observableClassViaObservaleName)) {
        throw this.stateMachineObjectBuilderImpl.newVerificationException(this.stateMachineObjectBuilderImpl.getDottedPath(), SyntaxErrors.PRE_STATE_CHANGE_OBSERVABLE_NAME_MISMATCH_OBSERVABLE_CLASS, observableName, observableClass, method);
      }
  ...
  private boolean isRelationalCallback(final String observableName, final Class<?> observableClass) {
    return !CallbackConsts.NULL_STR.equals(observableName) || Null.class != observableClass;
  }

  private void configurePostStateChange(final MethodWrapper methodWrapper, final PostStateChange postStateChange) throws VerificationException {
    ...
    if (isRelationalCallback(observableName, observableClass)) {
      final Class<?> actualObservableClass = evaluateActualObservableClassOfPostStateChange(methodWrapper.getMethod(), observableName, observableClass);
      validateMappedby(methodWrapper.getMethod(), mappedBy, actualObservableClass, SyntaxErrors.POST_STATE_CHANGE_MAPPEDBY_INVALID);
      final StateMachineObject<?> callBackEventSourceContainer = this.stateMachineObjectBuilderImpl.getRegistry().loadStateMachineObject(actualObservableClass);
      if (AnyState.class != from) {
        verifyFromState(methodWrapper.getMethod(), from, callBackEventSourceContainer.getMetaType(), SyntaxErrors.POST_STATE_CHANGE_FROM_STATE_IS_INVALID);
      }
      if (AnyState.class != to) {
        verifyPostToState(methodWrapper.getMethod(), to, callBackEventSourceContainer.getMetaType());
      }
      final Readable<?> accessor = evaluateAccessor(mappedBy, actualObservableClass);
      configurePostStateChangeCallbackObjectsWithRelational(methodWrapper, from, to, callBackEventSourceContainer, accessor);
    } else {
      configurePostStateChangeNonRelationalCallbackObjects(methodWrapper, from, to);
    }
  }
  ...
  private void configureOnEvent(final Method method, final OnEvent onEvent) throws VerificationException {
    ...
    if (isRelationalCallback(observableName, observableClass)) {
      final Class<?> actualObservableClass = evaluateActualObservableClassOfOnEvent(method, observableName, observableClass);
      validateMappedby(method, mappedBy, actualObservableClass, SyntaxErrors.ON_EVENT_MAPPEDBY_INVALID);
      final StateMachineObject<?> callBackEventSourceContainer = this.stateMachineObjectBuilderImpl.getRegistry().loadStateMachineObject(actualObservableClass);
      if (AnyEvent.class != eventClass) {
        verifyEvent(method, eventClass, callBackEventSourceContainer.getMetaType(), SyntaxErrors.ON_EVENT_EVENT_IS_INVALID);
      }
      final Readable<?> accessor = evaluateAccessor(mappedBy, actualObservableClass);
  ...
  private void configureOnEventNonRelationalCallbackObjects(Method method, Class<?> eventClass) {
  ...
  private void configureOnEventRelationalCallbackObjects(Method method, Class<?> eventClass, StateMachineObject<?> callBackEventSourceContainer, Readable<?> accessor) {
  ...
  private void verifyEvent(Method method, Class<?> eventClass, StateMachineMetadata metaType, String onEventEventIsInvalid) throws VerificationException {
    if (null == metaType.getEvent(eventClass)) {
      throw this.stateMachineObjectBuilderImpl.newVerificationException(metaType.getDottedPath(), SyntaxErrors.ON_EVENT_EVENT_IS_INVALID, eventClass, method, metaType.getPrimaryKey());
    }
  }

  private Class<?> evaluateActualObservableClassOfOnEvent(Method method, String observableName, Class<?> observableClass) throws VerificationException {
    Class<?> actualObservableClass = null;
    if (!CallbackConsts.NULL_STR.equals(observableName) && Null.class != observableClass) {
      verifyObservableClass(method, observableClass, SyntaxErrors.ON_EVENT_OBSERVABLE_CLASS_INVALID);
      final Class<?> observableClassViaObservaleName = verifyObservableName(method, observableName, SyntaxErrors.ON_EVENT_RELATION_INVALID);
      if (!observableClass.isAssignableFrom(observableClassViaObservaleName)) {
        throw this.stateMachineObjectBuilderImpl.newVerificationException(this.stateMachineObjectBuilderImpl.getDottedPath(), SyntaxErrors.ON_EVENT_OBSERVABLE_NAME_MISMATCH_OBSERVABLE_CLASS, observableName, observableClass, method);
      }
      actualObservableClass = observableClass;
    } else {
      if (CallbackConsts.NULL_STR.equals(observableName) && Null.class != observableClass) {
        ...
        if (!CallbackConsts.NULL_STR.equals(observableName) && Null.class == observableClass) {
          actualObservableClass = verifyObservableName(method, observableName, SyntaxErrors.ON_EVENT_RELATION_INVALID);
        }
      }
    }
    return actualObservableClass;
  ...
  private void validateMappedby(final Method method, final String mappedBy, final Class<?> actualObservableClass, final String errorCode) throws VerificationException {
    final StateMachineObject<?> callBackEventSourceContainer = this.stateMachineObjectBuilderImpl.getRegistry().loadStateMachineObject(actualObservableClass);
    if (isMappedByInvalid(mappedBy, actualObservableClass)) {
      throw this.stateMachineObjectBuilderImpl.newVerificationException(callBackEventSourceContainer.getDottedPath(), errorCode, mappedBy, method, actualObservableClass);
    }
  }

  private Class<?> evaluateActualObservableClassOfPostStateChange(Method method, final String observableName, final Class<?> observableClass) throws VerificationException {
    Class<?> actualObservableClass = null;
    if (!CallbackConsts.NULL_STR.equals(observableName) && Null.class != observableClass) {
      ...
      if (!observableClass.isAssignableFrom(observableClassViaObservaleName)) {
        throw this.stateMachineObjectBuilderImpl.newVerificationException(this.stateMachineObjectBuilderImpl.getDottedPath(), SyntaxErrors.POST_STATE_CHANGE_OBSERVABLE_NAME_MISMATCH_OBSERVABLE_CLASS, observableName, observableClass, method);
      }
      actualObservableClass = observableClass;
    } else {
      if (CallbackConsts.NULL_STR.equals(observableName) && Null.class != observableClass) {
  ...
  private Class<?> verifyObservableName(Method method, final String observableName, String errorCode) throws VerificationException {
    ...
    if (null == actualObservableClass) {
      throw this.stateMachineObjectBuilderImpl.newVerificationException(this.stateMachineObjectBuilderImpl.getDottedPath(), errorCode, observableName, method, klass);
    }
    return actualObservableClass;
  }
  ...
  private void verifyObservableClass(Method method, final Class<?> observableClass, String errorCode) throws VerificationException {
    try {
      ...
      if (e.getVerificationFailureSet().iterator().next().getErrorCode().equals(SyntaxErrors.REGISTERED_META_ERROR)) {
        throw this.stateMachineObjectBuilderImpl.newVerificationException(this.stateMachineObjectBuilderImpl.getDottedPath(), errorCode, observableClass, method);
      }
    }
  }
  ...
  private void verifyPostToState(final Method method, Class<?> to, final StateMachineMetadata metaType) throws VerificationException {
    if (null == metaType.getState(to)) {
      throw this.stateMachineObjectBuilderImpl.newVerificationException(metaType.getDottedPath(), SyntaxErrors.POST_STATE_CHANGE_TO_STATE_IS_INVALID, to, method, metaType.getPrimaryKey());
    }
  }
  ...
  private void verifyPreToState(Method method, Class<?> to, StateMachineMetadata stateMachineMetadata) throws VerificationException {
    if (null == stateMachineMetadata.getState(to)) {
      throw this.stateMachineObjectBuilderImpl.newVerificationException(stateMachineMetadata.getDottedPath(), SyntaxErrors.PRE_STATE_CHANGE_TO_STATE_IS_INVALID, to, method, stateMachineMetadata.getPrimaryKey());
    }
  ...
  private void verifyPreToStatePostEvaluate(Method method, Class<?> toStateClass, StateMachineMetadata stateMachineMetadata) throws VerificationException {
    for (final EventMetadata event : stateMachineMetadata.getState(toStateClass).getPossibleReachingEvents()) {
      if (event.isConditional() && event.postValidate()) {
        throw this.stateMachineObjectBuilderImpl.newVerificationException(stateMachineMetadata.getDottedPath(), SyntaxErrors.PRE_STATE_CHANGE_TO_POST_EVALUATE_STATE_IS_INVALID, toStateClass, method, event.getDottedPath());
      }
    }
  }
  ...
  private void verifyFromState(Method method, Class<?> stateClass, StateMachineMetadata metaType, String errorCode) throws VerificationException {
    if (null == metaType.getState(stateClass)) {
      throw this.stateMachineObjectBuilderImpl.newVerificationException(metaType.getDottedPath(), errorCode, stateClass, method, metaType.getPrimaryKey());
    }
  }

  private Readable<?> evaluateAccessor(String mappedBy, Class<?> observableClass) {
  ...
  private void configurePreStateChangeRelationalCallbackObjects(final MethodWrapper method, final Class<?> from, final Class<?> to, final StateMachineObject<?> callBackEventSourceContainer, final Readable<?> accessor) {
  ...
  private void configurePostStateChangeCallbackObjectsWithRelational(MethodWrapper method, Class<?> from, Class<?> to, final StateMachineObject<?> callBackEventSourceContainer, final Readable<?> accessor) {
    RelationalCallbackObject item = null;
    if (focusFromAndToState(from, to)) {
      method.setGeneralize(CallbackObject.SPECIFIC);
      item = new RelationalCallbackObject(from.getSimpleName(), to.getSimpleName(), method, accessor);
  ...
  private void configurePreStateChangeNonRelationalCallbackObjects(final MethodWrapper methodWrapper, final Class<?> from, final Class<?> to) {
    CallbackObject item = null;
    if (focusFromAndToState(from, to)) {
      methodWrapper.setGeneralize(CallbackObject.SPECIFIC);
      item = new CallbackObject(from.getSimpleName(), to.getSimpleName(), methodWrapper);
      ...
      if (focusToState(from, to)) {
        ...
        item = new CallbackObject(AnyState.class.getSimpleName(), to.getSimpleName(), methodWrapper);
        ...
        if (focusFromState(from, to)) {
          ...
          item = new CallbackObject(from.getSimpleName(), AnyState.class.getSimpleName(), methodWrapper);
          ...
          item = new CallbackObject(AnyState.class.getSimpleName(), AnyState.class.getSimpleName(), methodWrapper);
  ...
  private void configurePostStateChangeNonRelationalCallbackObjects(final MethodWrapper methodWrapper, Class<?> from, Class<?> to) {
    CallbackObject item = null;
    if (focusFromAndToState(from, to)) {
      methodWrapper.setGeneralize(CallbackObject.SPECIFIC);
      item = new CallbackObject(from.getSimpleName(), to.getSimpleName(), methodWrapper);
  ...
  private boolean isCallbackMethod(Method method) {
    if (hasAnnotation(method, Callbacks.class) || hasAnnotation(method, PreStateChange.class) || hasAnnotation(method, PostStateChange.class) || hasAnnotation(method, OnEvent.class)) {
      return true;
    }
    return false;
  }
