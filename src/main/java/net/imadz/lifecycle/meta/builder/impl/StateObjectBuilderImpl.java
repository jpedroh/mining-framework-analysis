...
public class StateObjectBuilderImpl<S extends java.lang.Object> extends ObjectBuilderBase<StateObject<S>, StateMachineObject<S>, StateMetadata> implements StateObjectBuilder<S> {
  ...
  @Override public StateObjectBuilder<S> build(Class<?> klass, StateMachineObject<S> parent) throws VerificationException {
    super.build(klass, parent);
    return this;
  }

  @Override public void verifyValidWhile(Object target, RelationConstraintMetadata[] relationMetadataArray, final Object relatedTarget, UnlockableStack stack) {
    try {
      ...
      if (!found) {
        ...
        throw new LifecycleException(getClass(), LifecycleCommonErrors.BUNDLE, LifecycleCommonErrors.STATE_INVALID, target, this.getMetaType().getSimpleName(), relatedTarget, relatedStateName, Arrays.toString(validRelationStates.toArray(new String[0])));
      } else {
  ...
  private StateMachineObject<?> findRelatedStateMachineWithRelatedTarget(RelationConstraintMetadata[] relationMetadataArray, final Object relatedTarget) throws VerificationException {
  ...
  @Override public void verifyInboundWhileAndLockRelatedObjects(Object eventKey, Object target, String nextState, RelationConstraintMetadata[] relationMetadataArray, Object relatedTarget, UnlockableStack stack) {
    try {
      final StateMachineObject<?> relatedStateMachineObject = findRelatedStateMachineWithRelatedTarget(relationMetadataArray, relatedTarget);
      lockRelatedObject(relatedTarget, stack, relatedStateMachineObject);
      ...
      if (!find) {
        ...
        throw new LifecycleException(getClass(), LifecycleCommonErrors.BUNDLE, LifecycleCommonErrors.VIOLATE_INBOUND_WHILE_RELATION_CONSTRAINT, eventKey, nextState, target, relatedTarget, relatedEvaluateState, Arrays.toString(validRelationStates.toArray(new String[0])));
      } else {
        relatedStateMachineObject.validateValidWhiles(relatedTarget, stack);
      }
    } catch (VerificationException e) {
  ...
  @Override public void invokeFromPreStateChangeCallbacks(LifecycleContext<?, S> callbackContext) {
  ...
  @Override public void invokeToPreStateChangeCallbacks(LifecycleContext<?, S> callbackContext) {
  ...
  @Override public void invokeFromPostStateChangeCallbacks(CallbackObject cbo, LifecycleContext<?, S> callbackContext) {
    if (postFromStateChangeCallbacksMap.containsKey(callbackContext.getFromStateName())) {
      cbo.doCallback(callbackContext);
    }
  }

  @Override public void invokeToPostStateChangeCallbacks(CallbackObject cbo, LifecycleContext<?, S> callbackContext) {
  ...
  @Override public void addPreToCallbackObject(Class<?> to, final CallbackObject callbackObject) {
  ...
  @Override public void addPreFromCallbackObject(Class<?> from, final CallbackObject callbackObject) {
  ...
  @Override public void addPostToCallbackObject(Class<?> to, final CallbackObject item) {
    final String toStateClassName = to.getSimpleName();
  ...
  @Override public void addPostFromCallbackObject(Class<?> from, final CallbackObject item) {
    final String fromStateClassName = from.getSimpleName();
  ...
  @Override public List<CallbackObject> getPostFromCallbackObjects(final String stateName) {
    return this.postFromStateChangeCallbacksMap.get(stateName);
  }

  @Override public List<CallbackObject> getPostToCallbackObjects(final String stateName) {
