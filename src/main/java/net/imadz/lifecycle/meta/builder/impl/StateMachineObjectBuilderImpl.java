...
public class StateMachineObjectBuilderImpl<S extends java.lang.Object> extends ObjectBuilderBase<StateMachineObject<S>, StateMachineObject<S>, StateMachineMetadata> implements StateMachineObjectBuilder<S> {
  ...
  @SuppressWarnings(value = { "rawtypes" }) private final KeyedList<StateObject> stateObjectList = new KeyedList<StateObject>();
  ...
  @SuppressWarnings(value = { "unused" }) private RelationObject parentRelationObject;
  ...
  @Override public void addCommonPostStateChangeCallbackObject(CallbackObject item) {
    this.commonPostStateChangeCallbackObjects.add(item);
  }

  @Override public void addCommonPreStateChangeCallbackObject(CallbackObject item) {
  ...
  @Override public void addSpecificPostStateChangeCallbackObject(CallbackObject item) {
    this.specificPostStateChangeCallbackObjects.add(item);
  }

  @Override public void addSpecificPreStateChangeCallbackObject(CallbackObject item) {
    this.specificPreStateChangeCallbackObjects.add(item);
  }

  @Override public StateMachineObjectBuilder<S> build(Class<?> klass, StateMachineObject<S> parent) throws VerificationException {
    setPrimaryKey(klass);
    addKey(klass);
  ...
  private void configureConditions(final Class<?> klass) {
    MethodScanner.scanMethodsOnClasses(klass, new MethodScanCallback() {
      @Override public boolean onMethodFound(Method method) {
  ...
  @SuppressWarnings(value = { "unchecked", "rawtypes" }) private void configureFieldStateAccessor(Field stateField) {
    if (String.class.equals(stateField.getType())) {
      ...
      try {
        ...
        this.stateAccessor = new ConverterAccessor(stateConverter, new StateFieldAccessor(stateField));
  ...
  @SuppressWarnings(value = { "rawtypes", "unchecked" }) private void configurePropertyAccessor(Class<?> klass, Method getter) {
    ...
    if (String.class.equals(getter.getReturnType())) {
      this.stateAccessor = new StatePropertyAccessor<String>(getter, setter);
    } else {
      try {
        this.stateConverter = (StateConverter<S>) getter.getAnnotation(Converter.class).value().newInstance();
        this.stateAccessor = new ConverterAccessor(stateConverter, new StatePropertyAccessor(getter, setter));
      } catch (InstantiationException e) {
        throw new IllegalStateException(e);
      } catch (IllegalAccessException e) {
        throw new IllegalStateException(e);
  ...
  private void configureRelationObjectsFromField(final Class<?> klass, final ArrayList<RelationMetadata> extendedRelationMetadata) throws VerificationException {
    ...
    for (Class<?> clazz = klass; clazz != Object.class; clazz = clazz.getSuperclass()) {
      for (Field field : clazz.getDeclaredFields()) {
        ...
        final StateMachineMetadata relatedStateMachine = getMetaType().getRegistry().loadStateMachineMetadata(clazz.getAnnotation(LifecycleMeta.class).value());
  ...
  private Setter<?> configureSetter(Class<?> klass, Method getter, final String setterName) {
  ...
  private void configureEventObject(final Class<?> klass, final Method method, final EventMetadata eventMetadata) throws VerificationException {
    final EventObjectBuilderImpl eventObjectBuilder = new EventObjectBuilderImpl(this, method, eventMetadata);
  ...
  @SuppressWarnings(value = { "unchecked" }) private ConditionalEvent<Object> createConditionalEvent(final FunctionMetadata functionMetadata) throws InstantiationException, IllegalAccessException {
    return (ConditionalEvent<Object>) functionMetadata.getEvent().getJudgerClass().newInstance();
  }

  @Override public void doInterceptAfter(LifecycleInterceptContext context) {
  ...
  @Override public void doInterceptBefore(LifecycleInterceptContext context) {
    doLock(context);
  ...
  @Override public void doInterceptException(LifecycleInterceptContext lifecycleContext) {
  ...
  private Object evaluateJudgeable(Object target, final EventMetadata eventMetadata) throws IllegalAccessException, InvocationTargetException {
    final ConditionObject conditionObject = getConditionObject(eventMetadata.getConditionClass());
  ...
  private String evaluateNextState(Object target, Object eventKey) {
    ...
    final FunctionMetadata functionMetadata = state.getMetaType().getFunctionMetadata(eventKey);
    requireFunctionNotNull(eventKey, state, functionMetadata);
    if (isConditional(functionMetadata)) {
      return evaluateNextStateWithConditionalEvent(target, functionMetadata);
    } else {
  ...
  private String evaluateNextStateWithConditionalEvent(Object target, final FunctionMetadata functionMetadata) {
    try {
      final ConditionalEvent<Object> conditionalEvent = createConditionalEvent(functionMetadata);
      final Class<?> nextStateClass = conditionalEvent.doConditionJudge(evaluateJudgeable(target, functionMetadata.getEvent()));
  ...
  @Override public String evaluateState(Object target) {
  ...
  private Method findDefaultStateGetterMethod(Class<?> klass) throws VerificationException {
    final StateIndicatorDefaultMethodScanner scanner = new StateIndicatorDefaultMethodScanner();
    MethodScanner.scanMethodsOnClasses(klass, scanner);
    return scanner.getDefaultMethod();
  }

  private Field findFieldWith(Class<?> klass, Class<? extends Annotation> aClass) throws VerificationException {
    if (klass.isInterface()) {
      return null;
    }
  ...
  private void fireLifecycleEvents(StateMachineObject<?> stateMachine, LifecycleInterceptContext context) {
  ...
  private Readable<?> getEvaluator(Object relationKey) {
    if (relationObjectList.containsKey(relationKey)) {
      return (Readable<?>) relationObjectList.get(relationKey).getEvaluator();
    }
    throw new IllegalStateException("The evaluate is not found, which should not happen. Check the verifyRelationsAllBeCoveraged method with key:" + relationKey);
  }

  @Override public LifecycleLockStrategy getLifecycleLockStrategy() {
  ...
  private Object getRelationInstance(Object contextTarget, final HashMap<Class<?>, Object> relationsInMethodParameters, final Entry<String, List<RelationConstraintMetadata>> relationMetadataEntry) {
    ...
    if (null == relationObject) {
      Readable<?> evaluator = getEvaluator(relationMetadataEntry.getValue().get(0).getRelationMetadata().getPrimaryKey());
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override public StateObject<S> getState(Object stateKey) {
    return (StateObject<S>) stateObjectList.get(stateKey);
  }

  @SuppressWarnings(value = { "unchecked" }) private S getStateRawTypeValue(String stateName) {
  ...
  private EventMetadata[] getEventsToState(StateMetadata state) {
    ...
    for (final StateMetadata stateMetadata : getMetaType().getAllStates()) {
      for (final EventMetadata eventMetadata : stateMetadata.getPossibleLeavingEvents()) {
  ...
  private StateMetadata handleCompositeStateMachineLinkage(StateMetadata nextState) {
    if (nextState.isCompositeState()) {
      ...
      if (nextState.getStateMachine().isComposite() && nextState.isFinal()) {
  ...
  private boolean hasOnlyOneStateCandidate(StateMachineObject<?> stateMachine, LifecycleInterceptContext context) {
  ...
  private void invokeCommonPostStateChangeCallbacks(CallbackObject cbo, LifecycleContext<?, S> callbackContext) {
  ...
  private void invokeSpecificPostStateChangeCallbacks(CallbackObject cbo, LifecycleContext<?, S> callbackContext) {
  ...
  @Override public boolean isLockEnabled() {
  ...
  private boolean isEventIn(StateMetadata state, EventMetadata eventMetadata) {
    for (final StateMetadata stateMetadata : getMetaType().getAllStates()) {
      for (FunctionMetadata item : stateMetadata.getDeclaredFunctionMetadata()) {
        if (item.getEvent().getDottedPath().equals(eventMetadata.getDottedPath())) {
  ...
  private boolean nextStateCanBeEvaluatedBeforeTranstion(StateMachineObject<?> stateMachine, LifecycleInterceptContext context) {
    if (hasOnlyOneStateCandidate(stateMachine, context)) {
      return true;
    } else {
      if (evaluateConditionBeforeEvent(context.getEventKey())) {
        return true;
      }
    }
    return false;
  ...
  private void performCallbacksAfterStateChange(LifecycleInterceptContext context) {
  ...
  private void performCallbacksBeforeStateChange(LifecycleInterceptContext context) {
  ...
  private void performPostStateChangeCallback(LifecycleInterceptContext context) {
    ...
    final LinkedList<CallbackObject> sortedCallbackObjects = getSortedCallbackObjects(fromStateName, toStateName);
    doCallbackObjects(callbackContext, sortedCallbackObjects, fromStateName, toStateName);
  }

  private void doCallbackObjects(final LifecycleContext<?, S> callbackContext, final LinkedList<CallbackObject> allCallbacks, final String fromStateName, final String toStateName) {
    for (CallbackObject cbo : allCallbacks) {
      if (cbo.getGeneralize() == CallbackObject.SPECIFIC) {
        ...
        if (cbo.getGeneralize() == CallbackObject.FROM) {
          getState(fromStateName).invokeFromPostStateChangeCallbacks(cbo, callbackContext);
        } else {
          if (cbo.getGeneralize() == CallbackObject.TO) {
            getState(toStateName).invokeToPostStateChangeCallbacks(cbo, callbackContext);
          } else {
  ...
  private LinkedList<CallbackObject> getSortedCallbackObjects(final String fromState, final String toState) {
  ...
  private void requireFunctionNotNull(Object eventKey, final StateObject<S> state, final FunctionMetadata functionMetadata) {
    if (null == functionMetadata) {
      throw new IllegalArgumentException("Invalid Key or Key not registered: " + eventKey + " while searching function metadata from state: " + state);
    }
  }
  ...
  private void validateInboundConstrantBeforeMethodInvocation(LifecycleInterceptContext context, final StateMachineObject<?> stateMachine) {
  ...
  private void validateInboundWhiles(LifecycleInterceptContext context) {
    ...
    final String nextState = evaluateNextState(target, context.getEventKey());
    ...
    for (final Entry<String, List<RelationConstraintMetadata>> relationMetadataEntry : mergeRelations(nextStateMetadata.getInboundWhiles()).entrySet()) {
      ...
      if (null != relationTarget) {
        getState(state.getDottedPath()).verifyInboundWhileAndLockRelatedObjects(context.getEventKey(), target, nextState, relationMetadataEntry.getValue().toArray(new RelationConstraintMetadata[0]), relationTarget, context);
      } else {
  ...
  private void validateRequiredRelationExist(final StateMetadata state, final Entry<String, List<RelationConstraintMetadata>> relationMetadataEntry, String errorCode) {
    for (final RelationConstraintMetadata item : relationMetadataEntry.getValue()) {
      if (!item.isNullable()) {
        throw new LifecycleException(getClass(), LifecycleCommonErrors.BUNDLE, errorCode, item.getPrimaryKey(), "nullable = " + item.isNullable(), state.getPrimaryKey());
      }
    }
  }
  ...
  private void validateEvent(LifecycleInterceptContext context) {
    ...
    final EventMetadata event = validateEvent(context.getTarget(), context.getFromState(), context.getEventKey());
    context.setEventType(event.getType());
    context.setEventName(event.getDottedPath().getName());
  }

  private EventMetadata validateEvent(final Object target, final String fromState, final Object eventKey) {
    final StateMetadata stateMetadata = this.getMetaType().getState(fromState);
    if (!stateMetadata.isEventValid(eventKey)) {
      throw new LifecycleException(getClass(), "lifecycle_common", LifecycleCommonErrors.ILLEGAL_EVENT_ON_STATE, eventKey, fromState, target);
    } else {
      return stateMetadata.getEvent(eventKey);
    }
  }

  @Override public void validateValidWhiles(final Object target, final UnlockableStack stack) {
    final StateMetadata state = getMetaType().getState(evaluateState(target));
    final HashMap<String, List<RelationConstraintMetadata>> mergedRelations = mergeRelations(state.getValidWhiles());
  ...
  private void verifyAllEventsCoverage(Class<?> klass, VerificationFailureSet failureSet) {
  ...
  private void verifyConditionBeCovered(Class<?> klass, final ConditionMetadata conditionMetadata) throws VerificationException {
    ...
    if (!scanner.isCovered()) {
      throw newVerificationException(getDottedPath(), SyntaxErrors.LM_CONDITION_NOT_COVERED, klass, getMetaType().getDottedPath(), conditionMetadata.getDottedPath());
    }
  }
  ...
  private boolean verifyCustomizedStateIndicatorGetter(Class<?> klass) throws VerificationException {
    final Method specifiedGetter = findCustomizedStateIndicatorGetter(klass);
    if (null != specifiedGetter) {
  ...
  private boolean verifyDefaultStateGetterMethod(Class<?> klass) throws VerificationException {
    final Method defaultGetter = findDefaultStateGetterMethod(klass);
  ...
  @Override public void verifyMetaData(VerificationFailureSet verificationSet) {
  }

  private void verifyRelationBeCovered(Class<?> klass, final RelationConstraintMetadata relation, final EventMetadata event) throws VerificationException {
    ...
    for (final Method method : eventMethods) {
      ...
      throw new VerificationException(newVerificationFailure(getDottedPath(), SyntaxErrors.LM_RELATION_NOT_BE_CONCRETED, method.getName(), klass.getName(), relation.getDottedPath().getName(), relation.getParent().getDottedPath()));
    }
  }
    ...
    for (Field field : klass.getDeclaredFields()) {
      ...
      if (Null.class != relationClass) {
        if (!getMetaType().hasRelation(relationClass)) {
          throw new VerificationException(newVerificationFailure(getDottedPath(), SyntaxErrors.LM_REFERENCE_INVALID_RELATION_INSTANCE, klass.getName(), relationClass.getName(), getMetaType().getDottedPath().getAbsoluteName()));
        }
      } else {
  ...
  private void verifyStateIndicatorConverter(AnnotatedElement getter, Class<?> stateType) throws VerificationException {
    ...
    verifyStateIndicatorConverterDeclaredCorrectTypeParameter(stateType, getterDeclaringClass, converterMeta);
  }

  private void verifyStateIndicatorConverterDeclaredCorrectTypeParameter(Class<?> stateType, final Class<?> getterDeclaringClass, final Converter converterMeta) throws VerificationException {
    for (Type type : converterMeta.value().getGenericInterfaces()) {
      if (type instanceof ParameterizedType) {
        ...
        if (implementsStateConverter(pType) && !matchedWithStateType(stateType, pType)) {
          throw newVerificationException(getDottedPath(), SyntaxErrors.STATE_INDICATOR_CONVERTER_INVALID, getterDeclaringClass, stateType, converterMeta.value(), pType.getActualTypeArguments()[0]);
        }
      }
    }
  }
  ...
  private boolean verifyStateIndicatorField(Class<?> klass) throws VerificationException {
  ...
  private void verifyEventBeCovered(Class<?> klass, final EventMetadata eventMetadata, final VerificationFailureSet failureSet) {
    CoverageVerifier coverage = new CoverageVerifier(this, eventMetadata, failureSet);
    MethodScanner.scanMethodsOnClasses(klass, coverage);
    if (coverage.notCovered()) {
      failureSet.add(newVerificationFailure(eventMetadata.getDottedPath().getAbsoluteName(), SyntaxErrors.LM_EVENT_NOT_CONCRETED_IN_LM, eventMetadata.getDottedPath().getName(), getMetaType().getDottedPath().getAbsoluteName(), klass.getSimpleName()));
    }
  }

  private void verifyEventMethod(Method method, VerificationFailureSet failureSet) {
    ...
    if (Null.class == event.value()) {
      eventMetadata = verifyEventMethodDefaultStyle(method, failureSet, eventMetadata);
    } else {
      eventMetadata = verifyEventMethodWithEventClassKey(method, failureSet, event, eventMetadata);
    }
  ...
  private EventMetadata verifyEventMethodDefaultStyle(Method method, VerificationFailureSet failureSet, EventMetadata eventMetadata) {
    if (!getMetaType().hasEvent(StringUtil.toUppercaseFirstCharacter(method.getName()))) {
      failureSet.add(newVerificationFailure(getMethodDottedPath(method), SyntaxErrors.LM_METHOD_NAME_INVALID, getMetaType().getDottedPath(), method.getName(), method.getDeclaringClass().getName()));
    } else {
      eventMetadata = getMetaType().getEvent(StringUtil.toUppercaseFirstCharacter(method.getName()));
    }
  ...
  private void verifyEventMethods(Class<?> klass) throws VerificationException {
    final VerificationFailureSet failureSet = new VerificationFailureSet();
  ...
  private void verifyEventMethodsValidity(final Class<?> klass, final VerificationFailureSet failureSet) {
    MethodScanner.scanMethodsOnClasses(klass, new MethodScanCallback() {
      @Override public boolean onMethodFound(Method method) {
  ...
  private EventMetadata verifyEventMethodWithEventClassKey(Method method, VerificationFailureSet failureSet, final Event event, EventMetadata eventMetadata) {
    if (!getMetaType().hasEvent(event.value())) {
      failureSet.add(newVerificationFailure(getMethodDottedPath(method), SyntaxErrors.LM_EVENT_METHOD_WITH_INVALID_EVENT_REFERENCE, event, method.getName(), method.getDeclaringClass().getName(), getMetaType().getDottedPath()));
    } else {
  ...
  @Override public Entry<StateAccessible<String>, String> getInitialState() {
  ...
  @Override public void addCommonOnEventCallbackObject(EventCallbackObject item) {
    this.commonEventCallbackObjects.add(item);
  }

  @Override public EventObject getEvent(Object eventKey) {
