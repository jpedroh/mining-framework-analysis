...
public class Issue extends Resource {
  public static final class FluentCreateComposed {
    ...
    public IssueFields createNewIssue() {
      ...
      issueFields.field(Field.PROJECT, project).field(Field.ISSUE_TYPE, issueType);
    ...
    public Results execute() throws JiraException {
      ...
      if (!(result instanceof JSONObject) || !((JSONObject) result).containsKey("issues") || !(((JSONObject) result).get("issues") instanceof JSONArray)) {
  ...
  private static class IssueIterator implements Iterator<Issue> {
    ...
    private List<Issue> getNextIssues() throws JiraException {
      ...
      JSON result = null;
      try {
        URI searchUri = createSearchURI(restclient, jql, includedFields, expandFields, maxResults, startAt);
        result = restclient.get(searchUri);
      } catch (Exception ex) {
        throw new JiraException("Failed to search issues", ex);
      }
  ...
  public static JSONObject getCreateMetadata(RestClient restclient, String project, String issueType) throws JiraException {
    ...
    try {
      ...
      URI createuri = restclient.buildURI(getBaseUri() + "issue/createmeta", params);
  ...
  public static FluentCreate create(RestClient restclient, String project, String issueType) throws JiraException {
    FluentCreate fc = new FluentCreate(restclient, getCreateMetadata(restclient, project, issueType));
    return fc.field(Field.PROJECT, project).field(Field.ISSUE_TYPE, issueType);
  }

  public static FluentCreateComposed createBulk(RestClient restclient, JSONObject createmetadata, String project, String issueType) throws JiraException {
    return new FluentCreateComposed(restclient, createmetadata, project, issueType);
  }
  ...
  public FluentCreate createSubtask() throws JiraException {
    return Issue.create(restclient, getProject().getKey(), "Sub-task").field(Field.PARENT, getKey());
  }
  ...
  /**
     ...
     */
  protected static URI createSearchURI(RestClient restclient, String jql, String includedFields, String expandFields, Integer maxResults, Integer startAt) throws URISyntaxException {
    Map<String, String> queryParams = new HashMap<String, String>();
  ...
  public void addWatcher(String username) throws JiraException {
    try {
      ...
      throw new JiraException("Failed to add watcher (" + username + ") to issue " + key, ex);
    }
  }
  ...
  public void deleteWatcher(String username) throws JiraException {
    try {
      ...
      URI uri = restclient.buildURI(getRestUri(key) + "/watchers", connectionParams);
      restclient.delete(uri);
    } catch (Exception ex) {
      throw new JiraException("Failed to remove watch (" + username + ") from issue " + key, ex);
    }
  }
  ...
  public List<RemoteLink> getRemoteLinks() throws JiraException {
    ...
    try {
      ...
      throw new JiraException("Failed to get remote links for issue " + key, ex);
    }
  ...
  public List<WorkLog> getAllWorkLogs() throws JiraException {
    ...
    try {
      ...
      throw new JiraException("Failed to get worklog for issue " + key, ex);
    }
