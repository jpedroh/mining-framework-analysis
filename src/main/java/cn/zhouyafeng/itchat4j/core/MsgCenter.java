...
public class MsgCenter {
  ...
  public static JSONArray produceMsg(JSONArray msgList) {
    ...
    for (int i = 0; i < msgList.size(); i++) {
      ...
      if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_TEXT.getCode()) {
        ...
        if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_IMAGE.getCode() || m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_EMOTICON.getCode()) {
          m.put("Type", MsgTypeEnum.PIC.getType());
        } else {
          if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_VOICE.getCode()) {
            m.put("Type", MsgTypeEnum.VOICE.getType());
          } else {
            if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_VERIFYMSG.getCode()) {
            } else {
              if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_SHARECARD.getCode()) {
                m.put("Type", MsgTypeEnum.NAMECARD.getType());
              } else {
                if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_VIDEO.getCode() || m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_MICROVIDEO.getCode()) {
                  m.put("Type", MsgTypeEnum.VIEDO.getType());
                } else {
                  if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_APP.getCode()) {
                  } else {
                    if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_STATUSNOTIFY.getCode()) {
                    } else {
                      if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_SYS.getCode()) {
                        m.put("Type", MsgTypeEnum.SYS.getType());
                      } else {
                        if (m.getInteger("MsgType") == MsgCodeEnum.MSGTYPE_RECALLED.getCode()) {
                        } else {
