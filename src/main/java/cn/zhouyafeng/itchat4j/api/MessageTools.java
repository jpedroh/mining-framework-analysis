...
public class MessageTools {
  ...
  public static JSONArray produceMsg(JSONArray msgList) {
    ...
    for (int i = 0; i < msgList.size(); i++) {
      ...
      if (m.getInteger("MsgType") == MsgType.MSGTYPE_TEXT) {
        ...
        if (m.getInteger("MsgType") == MsgType.MSGTYPE_IMAGE || m.getInteger("MsgType") == MsgType.MSGTYPE_EMOTICON) {
          m.put("Type", MsgType.PIC);
        } else {
          if (m.getInteger("MsgType") == MsgType.MSGTYPE_VOICE) {
            m.put("Type", MsgType.VOICE);
          } else {
            if (m.getInteger("MsgType") == 37) {
            } else {
              if (m.getInteger("MsgType") == 42) {
                m.put("Type", MsgType.NAMECARD);
              } else {
                if (m.getInteger("MsgType") == MsgType.MSGTYPE_VIDEO || m.getInteger("MsgType") == MsgType.MSGTYPE_MICROVIDEO) {
                  m.put("Type", MsgType.VIEDO);
                } else {
                  if (m.getInteger("MsgType") == 49) {
                  } else {
                    if (m.getInteger("MsgType") == 51) {
                    } else {
                      if (m.getInteger("MsgType") == 10000) {
                      } else {
                        if (m.getInteger("MsgType") == 10002) {
                        } else {
  ...
  public static void sendRawMsg(int msgType, String content, String toUserName) {
    ...
    @SuppressWarnings(value = { "unchecked" }) Map<String, Map<String, String>> baseRequestMap = (Map<String, Map<String, String>>) core.getLoginInfo().get("baseRequest");
