...
public class LoginServiceImpl implements ILoginService {
  ...
  @Override public boolean getQR(String qrPath) {
  ...
  @Override public boolean webWxInit() {
    core.setAlive(true);
  ...
  @Override public void wxStatusNotify() {
  ...
  @Override public void webWxGetContact() {
        ...
        if ((o.getInteger("VerifyFlag") & 8) != 0) {
          ...
          if (Config.API_SPECIAL_USER.contains(o.getString("UserName"))) {
            ...
            if (o.getString("UserName").indexOf("@@") != -1) {
              ...
              if (o.getString("UserName").equals(core.getUserSelf().getString("UserName"))) {
  ...
  @Override public void WebWxBatchGetContact() {
    ...
    try {
      ...
      for (int i = 0; i < contactList.size(); i++) {
        if (contactList.getJSONObject(i).getString("UserName").indexOf("@@") > -1) {
          ...
          core.getGroupMemeberMap().put(contactList.getJSONObject(i).getString("UserName"), contactList.getJSONObject(i).getJSONArray("MemberList"));
        }
      }
    } catch (Exception e) {
      LOG.info(e.getMessage());
  ...
  private JSONObject webWxSync() {
    ...
    String url = String.format(URLEnum.WEB_WX_SYNC_URL.getUrl(), core.getLoginInfo().get(StorageLoginInfoEnum.url.getKey()), core.getLoginInfo().get(StorageLoginInfoEnum.wxsid.getKey()), core.getLoginInfo().get(StorageLoginInfoEnum.skey.getKey()), core.getLoginInfo().get(StorageLoginInfoEnum.pass_ticket.getKey()));
    Map<String, Object> paramMap = core.getParamMap();
