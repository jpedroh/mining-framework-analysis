...
public class WorkerCallable implements Callable<Integer> {
  ...
  public void buildLocalTypeTopicCounts() {
    ...
    for (int doc = startDoc; doc < data.size() && doc < startDoc + numDocs; doc++) {
      ...
      for (int position = 0; position < tokens.size(); position++) {
        ...
        @Var int index = 0;
        @Var int currentTopic = currentTypeTopicCounts[index] & topicMask;
        int currentValue;
  ...
  protected int sampleTopicsForOneDoc(FeatureSequence tokenSequence, FeatureSequence topicSequence, boolean readjustTopicsAndStats) {
    ...
    @Var int[] currentTypeTopicCounts;
    @Var int type;
    @Var int oldTopic;
    @Var int newTopic;
    ...
    @Var int nonZeroTopics = denseIndex;
    @Var double topicBetaMass = 0.0;
    ...
    for (int position = 0; position < docLength; position++) {
      ...
      @Var int index = 0;
      @Var int currentTopic;
      @Var int currentValue;
      @Var boolean alreadyDecremented = (oldTopic == ParallelTopicModel.UNASSIGNED_TOPIC);
      topicTermMass = 0.0;
      ...
      @Var double sample = random.nextUniform() * (smoothingOnlyMass + topicBetaMass + topicTermMass);
      ...
      if (sample < topicTermMass) {
        ...
        index = 0;
        while (currentTypeTopicCounts[index] > 0 && (currentTypeTopicCounts[index] & topicMask) != newTopic) {
          index++;
          if (index == currentTypeTopicCounts.length) {
      ...
      if (newTopic == -1) {
        System.err.println("WorkerCallable sampling error: " + origSample + " " + sample + " " + smoothingOnlyMass + " " + topicBetaMass + " " + topicTermMass);
