        ...
        if (ot == ObjectParameters.PCEP_OBJECT_TYPE_P2MP_ENDPOINTS_DATAPATHID) {
          ...
          if (ot == ObjectParameters.PCEP_OBJECT_TYPE_P2MP_ENDPOINTS_IPV4) {
            ...
            if (ot == ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_IPV4) {
              ...
              if (ot == ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_UNNUMBERED) {
                try {
                ...
                if (ot == ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_MAC) {
                  try {
                  ...
                  if (ot == ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_MAC_NOT_UNICAST) {
                    try {
                    ...
                    if (ot == ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_IPV6) {
                      try {
                      ...
                      if (ot == ObjectParameters.PCEP_OBJECT_TYPE_GENERALIZED_ENDPOINTS) {
                        ...
                        if (ot == ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_DATAPATH_ID) {
                          try {
...
<<<<<<< /usr/src/app/output/telefonicaid/netphony-network-protocols/cfab35baf55c9496446ba24b76345d8e52145511/src/es/tid/pce/pcep/constructs/PCEPIntiatedLSP.java/left.java
        log.warn("Just one SRP, one LSP and one END-POINTS, object found, no more")
=======
        len += endPoint.getLength()
>>>>>>> /usr/src/app/output/telefonicaid/netphony-network-protocols/cfab35baf55c9496446ba24b76345d8e52145511/src/es/tid/pce/pcep/constructs/PCEPIntiatedLSP.java/right.java
        ;
        if (offset >= max_offset) {
          this.setLength(len);
          log.warn("Just one SRP, one LSP and one END-POINTS, object found, no more");
    ...
    if (PCEPObject.getObjectClass(bytes, offset) == ObjectParameters.PCEP_OBJECT_CLASS_ERO) {
      ...
      if (PCEPObject.getObjectClass(bytes, offset) == ObjectParameters.PCEP_OBJECT_CLASS_SR_ERO) {
        try {
    ...
    if (oc == ObjectParameters.PCEP_OBJECT_CLASS_BANDWIDTH) {
      if (ot == ObjectParameters.PCEP_OBJECT_TYPE_BANDWIDTH_REQUEST) {
        ...
        if (ot == ObjectParameters.PCEP_OBJECT_TYPE_BANDWIDTH_EXISTING_TE_LSP) {
          try {
          ...
          if (ot == ObjectParameters.PCEP_OBJECT_TYPE_BANDWIDTH_GEN_BW_REQUEST) {
            try {
            ...
            if (ot == ObjectParameters.PCEP_OBJECT_TYPE_BANDWIDTH_GEN_BW_EXISTING_TE_LSP) {
              try {
                bandwidth = new BandwidthRequested(bytes, offset);
              } catch (MalformedPCEPObjectException e) {
                log.warn("Malformed BANDWIDTH Object found");
