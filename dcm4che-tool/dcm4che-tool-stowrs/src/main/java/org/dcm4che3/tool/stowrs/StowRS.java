...
public class StowRS {
  ...
  @SuppressWarnings(value = { "static-access" }) private static CommandLine parseCommandLine(String[] args) throws ParseException {
    Options opts = new Options();
    opts.addOption(Option.builder("s").numberOfArgs(2).argName("[seq.]attr=value").desc(rb.getString("set")).build());
    opts.addOption(Option.builder("f").hasArg().argName("file").longOpt("file").desc(rb.getString("file")).build());
    ...
    opts.addOption(Option.builder("t").hasArg().argName("type").longOpt("type").desc(rb.getString("type")).build());
    ...
    opts.addOption(Option.builder("a").longOpt("accept").hasArg().desc(rb.getString("accept")).build());
    ...
    opts.addOption(Option.builder("l").longOpt("limit").hasArg().desc(rb.getString("limit")).build());
    opts.addOption(Option.builder("H").hasArg().argName("httpHeader:value").desc(rb.getString("httpHeader")).build());
    opts.addOption(Option.builder().hasArg().argName("suffix").desc(rb.getString("uid-suffix")).longOpt("uid-suffix").build());
    OptionGroup group = new OptionGroup();
    group.addOption(Option.builder("u").hasArg().argName("user:password").longOpt("user").desc(rb.getString("user")).build());
  ...
  @SuppressWarnings(value = { "unchecked" }) public static void main(String[] args) {
    long t1;
    try {
  ...
  private static void logSentPerChunk(StowChunk stowChunk, long t1) {
    ...
    System.out.println(MessageFormat.format(rb.getString(limit == 0 ? "sentNoLimit" : "sent"), stowChunk.sent, mb, s, mb / s));
  }
  ...
  private static void logSent(StowRS stowRS, long t1) {
    ...
    System.out.println(MessageFormat.format(rb.getString("sentAll"), stowRS.filesSent, mb, s, mb / s));
  }
  ...
  private void scan(List<String> files) {
    ...
    System.out.println(MessageFormat.format(rb.getString("scanned"), filesScanned, (t2 - t1) / 1000F, (t2 - t1) / filesScanned));
  }
  ...
  private void doNecessaryChecks(CommandLine cl, List<String> files) throws Exception {
    ...
    authorization = cl.hasOption("u") ? basicAuth(cl.getOptionValue("u")) : cl.hasOption("bearer") ? "Bearer " + cl.getOptionValue("bearer") : null;
  ...
  enum FileContentType {
    PDF(UID.EncapsulatedPDFStorage, Tag.EncapsulatedDocument, MediaTypes.APPLICATION_PDF, "encapsulatedPDFMetadata.xml"),
    CDA(UID.EncapsulatedCDAStorage, Tag.EncapsulatedDocument, MediaType.TEXT_XML, "encapsulatedCDAMetadata.xml"),
    SLA(UID.EncapsulatedSTLStorage, Tag.EncapsulatedDocument, MediaTypes.APPLICATION_SLA, "encapsulatedSTLMetadata.xml"),
    STL(UID.EncapsulatedSTLStorage, Tag.EncapsulatedDocument, MediaTypes.MODEL_STL, "encapsulatedSTLMetadata.xml"),
    STL_BINARY(UID.EncapsulatedSTLStorage, Tag.EncapsulatedDocument, MediaTypes.MODEL_X_STL_BINARY, "encapsulatedSTLMetadata.xml"),
    MTL(UID.EncapsulatedMTLStorage, Tag.EncapsulatedDocument, MediaTypes.MODEL_MTL, "encapsulatedMTLMetadata.xml"),
    OBJ(UID.EncapsulatedOBJStorage, Tag.EncapsulatedDocument, MediaTypes.MODEL_OBJ, "encapsulatedOBJMetadata.xml"),
    GENOZIP(UID.PrivateDcm4cheEncapsulatedGenozipStorage, Tag.EncapsulatedDocument, MediaTypes.APPLICATION_VND_GENOZIP, "encapsulatedGenozipMetadata.xml"),
    VCF_BZIP2(UID.PrivateDcm4cheEncapsulatedBzip2VCFStorage, Tag.EncapsulatedDocument, MediaTypes.APPLICATION_PRS_VCFBZIP2, "encapsulatedVCFBzip2Metadata.xml"),
    DOC_BZIP2(UID.PrivateDcm4cheEncapsulatedBzip2DocumentStorage, Tag.EncapsulatedDocument, MediaTypes.APPLICATION_X_BZIP2, "encapsulatedDocumentBzip2Metadata.xml"),
    JPEG(vlPhotographicImage ? UID.VLPhotographicImageStorage : UID.SecondaryCaptureImageStorage, Tag.PixelData, MediaTypes.IMAGE_JPEG, vlPhotographicImage ? "vlPhotographicImageMetadata.xml" : "secondaryCaptureImageMetadata.xml"),
    JP2(vlPhotographicImage ? UID.VLPhotographicImageStorage : UID.SecondaryCaptureImageStorage, Tag.PixelData, MediaTypes.IMAGE_JP2, vlPhotographicImage ? "vlPhotographicImageMetadata.xml" : "secondaryCaptureImageMetadata.xml"),
    PNG(vlPhotographicImage ? UID.VLPhotographicImageStorage : UID.SecondaryCaptureImageStorage, Tag.PixelData, MediaTypes.IMAGE_PNG, vlPhotographicImage ? "vlPhotographicImageMetadata.xml" : "secondaryCaptureImageMetadata.xml"),
    GIF(videoPhotographicImage ? UID.VideoPhotographicImageStorage : vlPhotographicImage ? UID.VLPhotographicImageStorage : UID.SecondaryCaptureImageStorage, Tag.PixelData, MediaTypes.IMAGE_GIF, vlPhotographicImage || videoPhotographicImage ? "vlPhotographicImageMetadata.xml" : "secondaryCaptureImageMetadata.xml"),
    MPEG(UID.VideoPhotographicImageStorage, Tag.PixelData, MediaTypes.VIDEO_MPEG, "vlPhotographicImageMetadata.xml"),
    MP4(UID.VideoPhotographicImageStorage, Tag.PixelData, MediaTypes.VIDEO_MP4, "vlPhotographicImageMetadata.xml"),
    QUICKTIME(UID.VideoPhotographicImageStorage, Tag.PixelData, MediaTypes.VIDEO_QUICKTIME, "vlPhotographicImageMetadata.xml")
    ;
  ...
  private enum CompressedPixelData {
    JPEG() {
      @Override void parse(SeekableByteChannel channel) throws IOException {
    ...
    static CompressedPixelData valueOf() {
      return bulkdataFileContentType == FileContentType.JP2 ? JPEG : bulkdataFileContentType == FileContentType.QUICKTIME ? MP4 : valueOf(bulkdataFileContentType.name());
    }
  }
  ...
  private Map<String, String> requestProperties(String[] httpHeaders) {
    ...
    requestProperties.put("Content-Type", MediaTypes.MULTIPART_RELATED + "; type=\"" + requestContentType + "\"; boundary=" + boundary);
  ...
  private void stow(final HttpURLConnection connection, StowChunk stowChunk) throws Exception {
    ...
    try {
      ...
      out.write(("\r\n--" + boundary + "--\r\n").getBytes());
      out.flush();
      logIncoming(connection.getResponseCode(), connection.getResponseMessage(), connection.getHeaderFields(), connection.getInputStream());
  ...
  private Path updateAttrs(Path path) {
    ...
    try {
      ...
      try (DicomOutputStream dos = new DicomOutputStream(new BufferedOutputStream(new FileOutputStream(tmpFile)), fmi != null ? UID.ExplicitVRLittleEndian : tsuid != null ? tsuid : UID.ImplicitVRLittleEndian)) {
  ...
  private void writeMetadataAndBulkData(OutputStream out, List<String> files, final Attributes staticMetadata, StowChunk stowChunk) throws Exception {
  ...
  private void writeXMLMetadataAndBulkdata(final OutputStream out, List<String> files, final Attributes staticMetadata, StowChunk stowChunk) throws Exception {
    if (files.isEmpty()) {
    ...
    if (fileCount.incrementAndGet() > 1) {
      if (fileContentTypeFromCL == null) {
        ...
        if (!firstBulkdataFileContentType.equals(bulkdataFileContentType)) {
          LOG.info(MessageFormat.format(rb.getString("ignore-non-matching-file-content-type"), path, bulkdataFileContentType, firstBulkdataFileContentType));
          return true;
        }
      } else {
  ...
  interface StowRSFileConsumer<Path extends java.lang.Object> {
    void accept(Path path) throws IOException;
  }

  interface StowRSFileFunction<Path extends java.lang.Object> {
