...
public class GtfsReaderTest {
  @Test public void testAllFields() throws IOException {
    MockGtfs gtfs = MockGtfs.create();
    gtfs.putLines("block.txt", "block_seq_num,block_var_num,block_route_num,block_run_num", "4237385,1,599,1");
    gtfs.putLines("agency.txt", "agency_id,agency_name,agency_url,agency_timezone,agency_lang,agency_phone,agency_fare_url,agency_email", "1,Agency,http://agency.gov/,America/Los_Angeles,en,555-1234,http://agency.gov/fares,agency@email.com");
    gtfs.putLines("levels.txt", "level_id,level_index,level_name", "L1,-1.3,Level One");
    gtfs.putLines("stops.txt", "stop_id,stop_name,stop_lat,stop_lon,stop_desc,stop_code,stop_direction,location_type,parent_station," + "stop_url,wheelchair_boarding,zone_id,stop_timezone,vehicle_type,platform_code,level_id", "S1,Stop,47.0,-122.0,description,123,N,1,1234,http://agency.gov/stop,1,Z,America/New_York,2,9 3/4,L1");
    ...
    gtfs.putLines("trips.txt", "route_id,service_id,trip_id,trip_headsign,trip_short_name,direction_id,block_id,shape_id,route_short_name," + "trip_bikes_allowed,bikes_allowed,wheelchair_accessible,peak_offpeak", "R1,WEEK,T1,head-sign,short-name,1,B1,SHP1,10X,1,2,1,3");
    ...
    gtfs.putLines("feed_info.txt", "feed_publisher_name,feed_publisher_url,feed_lang,feed_start_date,feed_end_date,feed_version,feed_contact_email,feed_contact_url", "Test,http://agency.gov/,en,20120110,20120217,2.0,agency@email.com,http://agency.gov/");
    gtfs.putLines("pathways.txt", "pathway_id,pathway_mode,is_bidirectional,from_stop_id,to_stop_id,traversal_time", "P1,1,1,S1,S1,60");
    gtfs.putLines("ridership.txt", "total_boardings,total_alightings,ridership_start_date,ridership_end_date,monday,tuesday,wednesday,thursday,friday,saturday,sunday,agency_id,route_id,trip_id,stop_id,average_load", "0,0,20180101,20180601,1,1,1,1,1,0,0,1,1,34741338,538,15.2", "0,0,20180101,20180601,1,1,1,1,1,0,0,1,1,34741338,558,14.4", "0,0,20180101,20180601,1,1,1,1,1,0,0,1,1,34741339,2010,1.3");
  ...
  @Test public void testIslandTransit() throws IOException {
    String agencyId = "26";
  ...
  @Test public void testCaltrain() throws IOException, ParseException {
  ...
  @Test public void testBart() throws IOException, ParseException {
  ...
  @Test public void testIntern() throws IOException, ParseException {
    File resourcePath;
    String agencyId;
    GtfsDao entityStore;
  ...
  @Test public void testTestAgency() throws IOException {
  ...
  @Test public void testUtf8() throws IOException, ParseException, InterruptedException {
    MockGtfs mockGtfs = MockGtfs.create();
    mockGtfs.putDefaultStopTimes();
    mockGtfs.putFile("routes.txt", new File("src/test/resources/org/onebusaway/gtfs/utf8-routes.txt"));
    String agencyId = "1";
  ...
  @Test public void testBom() throws IOException, ParseException, InterruptedException {
    MockGtfs mockGtfs = MockGtfs.create();
    mockGtfs.putDefaultStopTimes();
  ...
  @Test public void testAgency() throws CsvEntityIOException, IOException {
  ...
  @Test public void testFrequency() throws CsvEntityIOException, IOException {
    GtfsReader reader = new GtfsReader();
  ...
  @Test public void turlockFaresV2() throws CsvEntityIOException, IOException {
  ...
  @Test public void mdotMetroFaresV2() throws CsvEntityIOException, IOException {
    String agencyId = "1";
  ...
  @Test public void testFaresV2Distance() throws IOException {
    MockGtfs gtfs = MockGtfs.create();
    gtfs.putMinimal();
    gtfs.putLines("fare_products.txt", "fare_product_id, amount, currency", "" + "fare_1,5,EUR");
    gtfs.putLines("fare_leg_rules.txt", "network_id,min_distance,max_distance,distance_type,fare_product_id", "bus,0,3,1,fare_1");
    GtfsRelationalDao dao = processFeed(gtfs.getPath(), "1", false);
  ...
  @Test public void testFeedInfo() throws CsvEntityIOException, IOException {
    GtfsReader reader = new GtfsReader();
    StringBuilder b = new StringBuilder();
  ...
  @Test public void testDefaultAgencyForRoutes() throws IOException {
    MockGtfs gtfs = MockGtfs.create();
    gtfs.putAgencies(1);
    gtfs.putRoutes(1);
  ...
  @Test public void testCsvParser() throws CsvEntityIOException, IOException {
    GtfsReader reader = new GtfsReader();
    reader.setDefaultAgencyId("1");
   ...
   * Private Methods
   ****/
  public static GtfsRelationalDao processFeed(File resourcePath, String agencyId, boolean internStrings) throws IOException {
    GtfsReader reader = new GtfsReader();
  ...
  private ShapePoint getShapePoint(Iterable<ShapePoint> shapePoints, AgencyAndId shapeId, int sequence) {
