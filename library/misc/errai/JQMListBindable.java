 ...
 */
public class JQMListBindable<M extends java.lang.Object> extends JQMList implements HasValue<List<M>>, BindableListChangeHandler<M> {
  ...
  public static void forEachListItem(List<? extends ComplexPanel> uiItems, JQMListItemHandler handler) {
    if (handler == null || uiItems == null || uiItems.isEmpty()) {
      return;
    }
    for (ComplexPanel i : uiItems) {
      if (i instanceof JQMListItem) {
        ...
        if (i instanceof JQMListDivider) {
  ...
  public interface Renderer<M extends java.lang.Object> {
    ...
    /**
         ...
         */
    List<? extends ComplexPanel> itemChanged(JQMListBindable<M> list, int dataIndex, M oldItem, List<? extends ComplexPanel> oldUiItems, boolean oldItemDeleted, M newItem, List<? extends ComplexPanel> newUiItems);
  ...
  public static abstract class BaseRenderer<M extends java.lang.Object> implements Renderer<M> {
    @Override public List<? extends ComplexPanel> insertItem(JQMListBindable<M> list, int dataIndex, M item) {
      return addItem(list, item);
    }

    @Override public List<? extends ComplexPanel> itemChanged(JQMListBindable<M> list, int dataIndex, M oldItem, List<? extends ComplexPanel> oldUiItems, boolean oldItemDeleted, M newItem, List<? extends ComplexPanel> newUiItems) {
      if (oldItemDeleted) {
        removeItem(list, oldItem, oldUiItems);
      }
      if (newUiItems != null) {
        removeItem(list, newItem, newUiItems);
      }
      return insertItem(list, dataIndex, newItem);
    }

    @Override public void onBeforeRefresh(JQMListBindable<M> list) {
    }

    @Override public void onAfterRefresh(JQMListBindable<M> list) {
    }
  }

  public static abstract class ListRenderer<M extends java.lang.Object> extends BaseRenderer<M> {
    ...
    private List<? extends ComplexPanel> addItemIntern(final JQMListBindable<M> list, M item, final int position) {
      removeEmptyMsg(list);
      JQMListItem li = createListItem(item);
      if (li != null) {
        if (position >= 0) {
          list.addItem(position, li);
        } else {
          list.appendItem(li);
        }
        ...
        forEachListItem(lst, new JQMListItemHandler() {
          ...
          @Override public void onListItem(JQMListItem item) {
            if (position >= 0) {
              list.addItem(j++, item);
            } else {
          ...
          @Override public void onDivider(JQMListDivider divider) {
            if (position >= 0) {
              list.addDivider(j++, divider);
            } else {
              list.appendDivider(divider);
            }
          }
        });
        return lst;
    ...
    @Override public List<? extends ComplexPanel> addItem(final JQMListBindable<M> list, M item) {
    ...
    @Override public void removeItem(final JQMListBindable<M> list, M item, List<? extends ComplexPanel> uiItems) {
  ...
  public static class Ordered<M extends java.lang.Object> extends JQMListBindable<M> {
    public Ordered() {
      super(true);
    }
  }

  public static class Unordered<M extends java.lang.Object> extends JQMListBindable<M> {
    public Unordered() {
      super(false);
    }
  }
  ...
  public void setDataItems(List<M> items) {
    ...
    if (items == null) {
      ...
      this.dataItems = items instanceof BindableListWrapper ? (BindableListWrapper<M>) items : new BindableListWrapper<M>(items);
      if (changed) {
        this.dataItems.addChangeHandler(this);
      }
    }
    addDataItems();
  }
  ...
  private void dataItemChanged(final int index, final M oldItem, final M newItem) {
    ...
    if (oldItem != newItem && !oldInData) {
      dataToUI.remove(oldItem);
    }
  ...
  public int getUiIndex(M dataItem) {
    ...
    if (item instanceof JQMListItem) {
      ...
      if (item instanceof JQMListDivider) {
  ...
  @Override public HandlerRegistration addValueChangeHandler(ValueChangeHandler<List<M>> handler) {
  ...
  @Override public List<M> getValue() {
    return dataItems;
  }

  @Override public void setValue(List<M> value) {
    setValue(value, false);
  }

  @Override public void setValue(List<M> value, boolean fireEvents) {
  ...
  @Override public void onItemAdded(List<M> oldList, M item) {
    addDataItem(item);
    doRefresh();
  }

  @Override public void onItemAddedAt(List<M> oldList, int index, M item) {
    insertDataItem(index, item);
    doRefresh();
  }

  @Override public void onItemsAdded(List<M> oldList, Collection<? extends M> items) {
    for (M m : items) {
      addDataItem(m);
    }
    doRefresh();
  ...
  @Override public void onItemsAddedAt(List<M> oldList, int index, Collection<? extends M> item) {
  ...
  @Override public void onItemsCleared(List<M> oldList) {
    clear();
    doRefresh();
  }

  @Override public void onItemRemovedAt(List<M> oldList, int index) {
    removeDataItem(oldList.get(index));
    doRefresh();
  }

  @Override public void onItemsRemovedAt(List<M> oldList, List<Integer> indexes) {
  ...
  @Override public void onItemChanged(List<M> oldList, int index, M item) {
