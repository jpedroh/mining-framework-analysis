...
=======
 *  Binds model's List&lt;M> with UI list items generated by some Renderer implementation.
>>>>>>> RIGHT
 */
public class JQMListBindable<M> extends JQMList implements HasValue<List<M>> , BindableListChangeHandler<M> {
    public interface JQMListItemHandler {
        public abstract void onDivider(JQMListDivider divider);

        public abstract void onListItem(JQMListItem item);
    }
...
=======
         *  1. data item's content changed, i.e. oldItem == newItem
         *  2. new data item is replacing old one
         *  3. sort() is called, so data items positions are exchanging (no real remove/add to the list)
>>>>>>> RIGHT
         *
...
>>>>>>> RIGHT
         ...
         */
        public abstract List<? extends ComplexPanel> itemChanged(JQMListBindable<M> list, int dataIndex, M oldItem, List<? extends ComplexPanel> oldUiItems, boolean oldItemDeleted, M newItem, List<? extends ComplexPanel> newUiItems);
    ...
    public static abstract class BaseRenderer<M> implements Renderer<M> {
        ...
        @Override
        public List<? extends ComplexPanel> itemChanged(JQMListBindable<M> list, int dataIndex, M oldItem, List<? extends ComplexPanel> oldUiItems, boolean oldItemDeleted, M newItem, List<? extends ComplexPanel> newUiItems) {
            if (oldItemDeleted) {
...
=======
         * Stable ui index means that there are no manual sort() or items exchange operations
>>>>>>> RIGHT
