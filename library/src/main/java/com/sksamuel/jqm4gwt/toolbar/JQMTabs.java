...
public class JQMTabs extends JQMWidget {
  ...
  @UiChild(tagname = "button") public void addHeader(final JQMButton button) {
    if (list != null) {
  ...
  @UiChild(tagname = "listitem") public void addHeader(JQMListItem item) {
    if (navbar != null) {
      throw new IllegalArgumentException(ERROR_HEADER);
    }
    if (list == null) {
  ...
  @UiChild(tagname = "content") public void addContent(final Widget widget) {
      ...
      for (int i = 0; i < navbar.getButtonCount(); i++) {
        ...
        if (isSelected(li)) {
          ...
          for (int j = 0; j < navbar.getButtonCount(); j++) {
            ...
            if (JQMCommon.isBtnActive(b)) {
              JQMCommon.setBtnActive(b, false);
            }
          }
          break;
        }
          ...
          for (int i = 0; i < items.size(); i++) {
            ...
            if (isSelected(li.getElement())) {
              ...
              for (int j = 0; j < items.size(); j++) {
                ...
                if (liTmp.isActiveHighlight()) {
                  liTmp.setActiveHighlight(false);
                }
              }
              break;
            }
      ...
      if (list != null) {
        ...
        if (items != null) {
          for (int j = 0; j < items.size(); j++) {
            ...
            if (li.isActiveHighlight()) {
              li.setActiveHighlight(false);
            }
          }
        }
      }
    ...
    if (list != null) {
      ...
      for (int i = 0; i < items.size(); i++) {
        ...
        for (String id : ids) {
          ...
          if (!byHrefs && id.equals(li.getElement().getId()) || byHrefs && id.equals(hrefToId(li.getHref()))) {
            found.put(id, li);
            break;
          }
        }
      ...
      if (navbar != null) {
        for (int i = 0; i < navbar.getButtonCount(); i++) {
          ...
          for (String id : ids) {
            ...
            if (!byHrefs && id.equals(li.getId()) || byHrefs && id.equals(hrefToId(btn.getHref()))) {
              found.put(id, btn);
              break;
            }
          }
  ...
  public List<Widget> getContents() {
    ...
    for (int i = 0; i < flow.getWidgetCount(); i++) {
      ...
      if (rslt == null) {
        rslt = new ArrayList<Widget>(flow.getWidgetCount());
      }
      rslt.add(w);
    }
    return rslt;
  ...
  protected void execOnActivate(String newTabId, String oldTabId, String newPanelId, String oldPanelId) {
    List<Widget> headers = findHeaders(false, newTabId, oldTabId);
    if (headers == null) {
      headers = findHeaders(true, newPanelId, oldPanelId);
    }
    ...
    JQMTabsEvent.fire(this, TabsState.ACTIVATE, headers.get(0), headers.get(1), contents.get(0), contents.get(1));
  }

  protected boolean execOnBeforeActivate(String newTabId, String oldTabId, String newPanelId, String oldPanelId) {
    List<Widget> headers = findHeaders(false, newTabId, oldTabId);
    if (headers == null) {
      headers = findHeaders(true, newPanelId, oldPanelId);
    }
    ...
    try {
      JQMTabsEvent.fire(this, TabsState.BEFORE_ACTIVATE, headers.get(0), headers.get(1), contents.get(0), contents.get(1));
    } catch (Exception e) {
      return false;
    }
    ...
    if (activeTabId != null || activePanId != null) {
      if (list != null) {
        ...
        for (int i = 0; i < items.size(); i++) {
          ...
          if (li.isActiveHighlight() != act) {
            li.setActiveHighlight(act);
          }
        }
      } else {
        if (navbar != null) {
          for (int i = 0; i < navbar.getButtonCount(); i++) {
            ...
            if (JQMCommon.isBtnActive(btn) != act) {
              JQMCommon.setBtnActive(btn, act);
            }
          }
        }
      }
  ...
  public void setCollapseAll(boolean value) {
    if (value) {
      JQMCommon.setAttribute(this, DATA_ACTIVE, "false");
    } else {
      JQMCommon.setAttribute(this, DATA_ACTIVE, null);
    }
  }
