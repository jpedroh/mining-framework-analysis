...
public class JQMNavBar extends JQMWidget implements HasFixedPosition {
  ...
  @UiChild(tagname = "button") public void add(final JQMButton button) {
    if (button == null) {
      return;
    }
    if (buttons == null) {
      buttons = new ArrayList<JQMButton>();
    }
    buttons.add(button);
  ...
  @UiChild(limit = 1, tagname = "buttonActive") public void addActive(final JQMButton button) {
    if (button == null) {
      return;
    }
    add(button);
  ...
  /**
     ...
     */
  @UiChild(limit = 1, tagname = "buttonActivePersist") public void addActivePersist(final JQMButton button) {
    if (button == null) {
      return;
    }
    addActive(button);
  ...
  @Override public void setFixed(boolean fixed) {
    if (fixed) {
  ...
  public void setIconPos(IconPos pos) {
    if (pos == null) {
      ...
      for (int i = 0; i < buttons.size(); i++) {
        ...
        if (p == null || oldPos == null || p.equals(oldPos)) {
          btn.setIconPos(pos);
        }
      }
    }
