...
public class JQMPage extends JQMContainer implements HasFullScreen<JQMPage> {
  ...
  private int hideFixedToolbarsIfVirtualKeyboard = 0;
  ...
  /**
     ...
     */
  @Override public JQMContainer withContainerId() {
  ...
  @Override public void add(Widget widget) {
    if (widget instanceof HasJqmHeader) {
      ...
      if (widget instanceof HasJqmFooter) {
  ...
  protected void doPageBeforeShow(Element prevPage) {
    ...
    if (isDialog()) {
      if (transparent && prevPage != null) {
        ...
        if (content != null) {
          content.addStyleName(UI_BODY_INHERIT);
        }
        Element dlgContain = JQMCommon.findChild(getElement(), UI_DIALOG_CONTAIN);
        if (dlgContain != null) {
          dlgContain.addClassName(UI_BODY_INHERIT);
        }
      } else {
        transparentPrevPage = null;
        transparentPrevPageClearCache = false;
        if (content != null) {
          content.removeStyleName(UI_BODY_INHERIT);
        }
        Element dlgContain = JQMCommon.findChild(getElement(), UI_DIALOG_CONTAIN);
        if (dlgContain != null) {
          dlgContain.removeClassName(UI_BODY_INHERIT);
        }
      }
    }
  }
  ...
  protected void doPageShow(Element prevPage) {
    ...
    if (contentCentered || pseudoFixedToolbars || contentHeightPercent > 0 || hideFixedToolbarsIfContentAreaPercentBelow > 0 || hideFixedToolbarsIfVirtualKeyboard > 0) {
  ...
  private void initOrientationChange() {
    if (orientationChangeInitialized) {
      return;
    }
    ...
    this.addJQMEventHandler(JQMComponentEvents.ORIENTATIONCHANGE, new JQMOrientationChangeHandler() {
      @Override public void onEvent(JQMEvent<?> event) {
        ...
        if (JQMOrientationChangeHandler.LANDSCAPE.equals(s)) {
          ...
          if (JQMOrientationChangeHandler.PORTRAIT.equals(s)) {
  ...
  @Override public void setFullScreen(boolean fs) {
    if (fs) {
  ...
  @Override public JQMPage withFullScreen(boolean fs) {
    setFullScreen(fs);
    return this;
  }
  ...
  private void hideFixedToolbars(int headerH, int footerH) {
    final JQMHeader header = getHeader();
    if (header != null && (header.isFixed() || pseudoFixedToolbars) && !header.getElement().hasClassName(JQM4GWT_FIXED_HIDDEN)) {
      Element headerElt = header.getElement();
      if (header.isFixed()) {
    ...
    final JQMFooter footer = getFooter();
    if (footer != null && (footer.isFixed() || pseudoFixedToolbars) && !footer.getElement().hasClassName(JQM4GWT_FIXED_HIDDEN)) {
      Element footerElt = footer.getElement();
      if (footer.isFixed()) {
  ...
  private boolean isFixedToolbarsHidden() {
    final JQMHeader header = getHeader();
    final JQMFooter footer = getFooter();
    return (header != null && header.getElement().hasClassName(JQM4GWT_FIXED_HIDDEN)) || (footer != null && footer.getElement().hasClassName(JQM4GWT_FIXED_HIDDEN));
  }

  private void processFixedToolbars() {
    if (!isVisible() || header == null && footer == null) {
      return;
    }
    ...
    if (hideFixedToolbarsIfVirtualKeyboard > 0 && initialWindowHeight - windowH >= hideFixedToolbarsIfVirtualKeyboard) {
  ...
  public void centerContent() {
    ...
    if (contentCentered) {
      ...
      boolean needCentering = (header == null && footer == null) || (isFixedHeader && isFixedFooter) || (isFixedHeader && footer == null) || (isFixedFooter && header == null);
      if (needCentering && !isFixedToolbarsHidden()) {
        contentH = content.getOffsetHeight();
        if (contentH > 0) {
          if (header != null) {
            headerH = header.getOffsetHeight();
          }
          if (footer != null) {
    ...
    if (marginTop <= 0) {
      ...
      content.getElement().getStyle().setMarginTop(marginTop, Unit.PX);
    }
    if (footer != null) {
      ...
      if (footerMarginTop <= 0) {
        footer.getElement().getStyle().clearMarginTop();
      } else {
        footer.getElement().getStyle().setMarginTop(footerMarginTop, Unit.PX);
      }
    }
  }
  ...
  public void setDlgTransparent(boolean transparent) {
    ...
    if (this.transparent) {
      addStyleName(JQM4GWT_DLG_TRANSPARENT);
    } else {
      removeStyleName(JQM4GWT_DLG_TRANSPARENT);
    }
  }
  ...
  public void restoreRolePage() {
    ...
    if (h != null) {
      ...
      if (btn != null && "#".equals(JQMCommon.getAttribute(btn, "href")) && (DataIcon.DELETE == JQMCommon.getIcon(btn) || DataIcon.DELETE == JQMCommon.getStyleIcon(btn))) {
  ...
  public void openDialog() {
    if (JQMCommon.isDataDialog(getElement())) {
      ...
      JQMContext.changePage(this, true);
    }
  }
  ...
  public void closeDialog() {
    if (isDialog()) {
      ...
      if (Mobile.DATA_ROLE_DIALOG.equals(JQMCommon.getDataRole(this))) {
