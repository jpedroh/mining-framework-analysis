...
class ModelRepresenter extends Representer {
  ...
  private class RepresentXpp3Dom implements Represent {
    ...
    public Node representData(Object data) {
      return representMapping(Tag.MAP, toMap((Xpp3Dom) data), DumperOptions.FlowStyle.
...
>>>>>>> /usr/src/app/output/takari/maven-polyglot/746ef5141b10b1281a83864bd72ff5b78816762b/polyglot-yaml/src/main/java/org/sonatype/maven/polyglot/yaml/ModelRepresenter.java/right.java
    ...
    private Map<String, Object> toMap(Xpp3Dom node) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if ("reportPlugins".equals(childName)) {
          ...
          if (childNameLength > 3 && childName.endsWith("ies")) {
            singularName = childName.substring(0, childNameLength - 3);
          } else {
            if (childNameLength > 1 && childName.endsWith("s")) {
  ...
  @Override protected Set<Property> getProperties(Class<? extends Object> type) {
    try {
