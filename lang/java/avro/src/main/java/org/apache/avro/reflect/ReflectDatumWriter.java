 ...
 * via Java reflection.
 */
public class ReflectDatumWriter<T extends java.lang.Object> extends SpecificDatumWriter<T> {
  ...
  @Override public void write(T datum, Encoder out) throws IOException {
  ...
  @Override protected void writeRecord(Schema schema, Object datum, Encoder out) throws IOException {
    if (((ReflectData) getData()).isResolvingCircularRefs()) {
      if (CIRCULAR_REFS.get().containsKey(datum)) {
        datum = CIRCULAR_REFS.get().get(datum);
      } else {
        CIRCULAR_REFS.get().put(datum, new CircularRef(CIRCULAR_REFS.get().size()));
      }
    }
    super.writeRecord(schema, datum, out);
  }

  @Override protected int resolveUnion(Schema union, Object datum) {
    if (((ReflectData) getData()).isResolvingCircularRefs() && CIRCULAR_REFS.get().containsKey(datum)) {
      datum = CIRCULAR_REFS.get().get(datum);
    }
   ...
   * representations.*/
  @Override protected void writeArray(Schema schema, Object datum, Encoder out) throws IOException {
  ...
  @Override protected void writeBytes(Object datum, Encoder out) throws IOException {
  ...
  @Override protected void write(Schema schema, Object datum, Encoder out) throws IOException {
    if (datum instanceof Byte) {
      ...
      if (datum instanceof Short) {
        ...
        if (datum instanceof Character) {
          datum = (int) (char) (Character) datum;
        } else {
          if (datum instanceof Map && ReflectData.isNonStringMapSchema(schema)) {
  ...
  @Override protected void writeField(Object record, Field f, Encoder out, Object state) throws IOException {
    if (state != null) {
