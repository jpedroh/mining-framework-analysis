...
public class TestSchemaCompatibility {
  ...
  @Test public void testValidateArrayWriterSchema() throws Exception {
    ...
    final SchemaCompatibility.SchemaPairCompatibility invalidResult = new SchemaCompatibility.SchemaPairCompatibility(SchemaCompatibility.SchemaCompatibilityResult.incompatible(SchemaIncompatibilityType.TYPE_MISMATCH, invalidReader, STRING_ARRAY_SCHEMA, "reader type: MAP not compatible with writer type: ARRAY", asList("")), invalidReader, STRING_ARRAY_SCHEMA, String.format("Data encoded using writer schema:%n%s%n" + "will or may fail to decode using reader schema:%n%s%n", STRING_ARRAY_SCHEMA.toString(true), invalidReader.toString(true)));
  ...
  public static void validateIncompatibleSchemas(Schema reader, Schema writer, SchemaIncompatibilityType incompatibility, String message, String location) {
    validateIncompatibleSchemas(reader, writer, asList(incompatibility), asList(message), asList(location));
  }
  ...
  @Test public void testBytesDecimalWithSameScaleAndPrecision() {
    final SchemaPairCompatibility result = checkReaderWriterCompatibility(BYTES_DECIMAL_3_3, BYTES_DECIMAL_3_3);
  ...
  @Test public void testFixedDecimalWithSameScaleAndPrecision() {
    final SchemaPairCompatibility result = checkReaderWriterCompatibility(FIXED_DECIMAL_3_3, FIXED_DECIMAL_3_3);
    assertEquals(SchemaCompatibilityType.COMPATIBLE, result.getType());
  }
}