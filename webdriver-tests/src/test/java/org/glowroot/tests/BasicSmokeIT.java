...
public class BasicSmokeIT extends WebDriverIT {
  @BeforeAll public static void setUp() throws Exception {
    String content = httpGet("http://localhost:" + getUiPort() + "/backend/config/transaction?agent-id=" + agentId);
    JsonNode responseNode = new ObjectMapper().readTree(content);
    String version = responseNode.get("config").get("version").asText();
    httpPost("http://localhost:" + getUiPort() + "/backend/config/transaction?agent-id=" + agentId, "{\"slowThresholdMillis\":0,\"profilingIntervalMillis\":10," + "\"captureThreadStats\":false,\"version\":\"" + version + "\"}");
    for (int i = 0; i < 3; i++) {
    ...
    while (stopwatch.elapsed(SECONDS) < 30) {
      ...
      content = httpGet("http://localhost:" + getUiPort() + "/backend/transaction/summaries?agent-rollup-id=" + agentId + "&transaction-type=Web&from=" + from + "&to=" + to + "&sort-order=total-time&limit=10");
  ...
  @Test public void shouldCheckTransactionPages() throws Exception {
    App app = app();
  ...
  @Test public void shouldCheckNonActiveTraceModalPages() throws Exception {
    App app = app();
    app.open();
    clickPartialLinkWithWait("Slow traces");
    String url = "http://localhost:" + getUiPort() + "/backend/transaction/points" + "?transaction-type=Web" + "&from=0" + "&to=" + Long.MAX_VALUE + "&duration-millis-low=0" + "&headline-comparator=begins" + "&headline=" + "&error-message-comparator=begins" + "&error-message=" + "&user-comparator=begins" + "&user=" + "&attribute-name=" + "&attribute-value-comparator=begins" + "&attribute-value=" + "&limit=500" + "&agent-rollup-id=" + agentId;
    ...
    if (WebDriverSetup.useCentral) {
      driver.get(app.getBaseUrl() + "/transaction/traces?agent-id=" + agentId + "&transaction-type=Web&modal-agent-id=" + agentId + "&modal-trace-id=" + traceId);
    } else {
  ...
  @Test public void shouldCheckActiveTraceModalPages() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
  ...
  @Test public void shouldCheckErrorsPages() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
    app.open();
  ...
  @Test public void shouldCheckJvmPages() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
    JvmSidebar jvmSidebar = new JvmSidebar(driver);
    app.open();
    ...
    if (!WebDriverSetup.useCentral) {
      ...
      String heapDumpFileName = Utils.getWithWait(driver, xpath("//div[@ng-if=\'heapDumpResponse\']//table//tr[1]/td[2]")).getText();
    ...
    List<WebElement> elements = new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfAllElementsLocatedBy(className("gt-mbean-unexpanded-content")));
  ...
  @Test public void shouldRunReportTransactionAverage() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
  ...
  @Test public void shouldRunReportTransactionPercentile() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
    AdhocPage adhocPage = new AdhocPage(driver);
    app.open();
  ...
  @Test public void shouldRunReportTransactionCount() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
    AdhocPage adhocPage = new AdhocPage(driver);
    app.open();
  ...
  @Test public void shouldRunReportErrorRate() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
    AdhocPage adhocPage = new AdhocPage(driver);
    app.open();
  ...
  @Test public void shouldRunReportErrorCount() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
    AdhocPage adhocPage = new AdhocPage(driver);
    app.open();
  ...
  @Test public void shouldRunReportGauge() throws Exception {
    App app = app();
    GlobalNavbar globalNavbar = globalNavbar();
    AdhocPage adhocPage = new AdhocPage(driver);
    app.open();
    ...
    adhocPage.getMetricSelect().selectByValue("string:gauge:java.lang:type=Memory:HeapMemoryUsage.used");
    adhocPage.clickRunReportButton();
    waitFor(xpath("//div[@ng-if=\'showChart\']"));
  }

  @Test public void shouldCheckLogPage() throws Exception {
    httpGet("http://localhost:" + getUiPort() + "/log");
  }

  @Test public void shouldCheckHealthCheckPage() throws Exception {
    httpGet("http://localhost:" + getUiPort() + "/health");
  }

  @Test public void shouldCheckCassandraWriteTotals() throws Exception {
    Assumptions.assumeTrue(WebDriverSetup.useCentral);
    httpGet("http://localhost:" + getUiPort() + "/backend/admin/cassandra-write-totals?limit=10");
  }
  ...
  private void clickAcross() throws InterruptedException {
    ...
    click(xpath("//button[@ng-click=\'refresh()\']"));
    new WebDriverWait(driver, 30).until(ExpectedConditions.textToBePresentInElementLocated(className("gt-profile"), "JdbcServlet"));
  ...
  private void clickAroundInTraceModal(String traceId, boolean active) throws Exception {
    ...
    if (WebDriverSetup.useCentral) {
      download = "http://localhost:" + getUiPort() + "/export/trace?agent-id=" + agentId + "&trace-id=" + traceId + urlSuffix;
    } else {
