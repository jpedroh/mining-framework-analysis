...
public class AccidentNotificationBolt extends BaseRichBolt {
  ...
  @Override public void prepare(@SuppressWarnings(value = { "rawtypes" }) Map stormConf, TopologyContext context, OutputCollector collector) {
  ...
  @Override public void execute(Tuple tuple) {
    if (tuple.getSourceStreamId().equals(TopologyControl.POS_REPORTS_STREAM_ID)) {
      if (!tuple.contains(TopologyControl.POS_REPORT_FIELD_NAME)) {
        throw new RuntimeException(String.format("tuple without pos report submitted on stream \'%s\'", TopologyControl.POS_REPORTS_STREAM_ID));
      }
      ...
      for (short i = 0; i <= 4; i++) {
        ...
        SegmentIdentifier accidentSegment = new SegmentIdentifier(pos.getXWay(), segmentToCheck, pos.getDirection());
      ...
      if (tuple.getSourceStreamId().equals(TopologyControl.ACCIDENT_INFO_STREAM_ID)) {
        if (!tuple.contains(TopologyControl.ACCIDENT_INFO_FIELD_NAME)) {
          throw new RuntimeException(String.format("tuple without accident submitted on stream", TopologyControl.ACCIDENT_INFO_STREAM_ID));
        }
        this.updateAccidents(tuple);
      } else {
        throw new RuntimeException(String.format("Errornous stream subscription. Please report a bug at %s", Helper.ISSUE_REPORT_URL));
      }
    }
