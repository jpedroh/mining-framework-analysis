...
public abstract class AbstractGeneratorMojo extends AbstractMojo {
  ...
  @Component protected BuildContext buildContext;
  ...
  /**
     ...
     * @since 3.3
     */
  @Parameter protected List<String> packagingTypes = Arrays.asList("maven-plugin");
  ...
  @Override public void execute() throws MojoExecutionException {
    ...
    if (!"maven-plugin".equalsIgnoreCase(project.getArtifactId()) && project.getArtifactId().toLowerCase().startsWith("maven-") && project.getArtifactId().toLowerCase().endsWith("-plugin") && !"org.apache.maven.plugins".equals(project.getGroupId())) {
      getLog().error(LS + LS + "Artifact Ids of the format maven-___-plugin are reserved for" + LS + "plugins in the Group Id org.apache.maven.plugins" + LS + "Please change your artifactId to the format ___-maven-plugin" + LS + "In the future this error will break the build." + LS + LS);
    }
    ...
    if (goalPrefix == null) {
      ...
      if (!goalPrefix.equals(defaultGoalPrefix)) {
        getLog().warn(LS + LS + "Goal prefix is specified as: \'" + goalPrefix + "\'. " + "Maven currently expects it to be \'" + defaultGoalPrefix + "\'." + LS);
      }
    }
    ...
    try {
      ...
      throw new MojoExecutionException("The API of the mojo scanner is not compatible with this plugin version." + " Please check the plugin dependencies configured in the POM and ensure the versions match.", e);
    }
  }
  ...
  private Set<Artifact> mavenDependenciesNotInProvidedScope() {
    ...
    for (Artifact dependency : project.getArtifacts()) {
      if ("org.apache.maven".equals(dependency.getGroupId()) && dependency.getArtifactId().startsWith("maven-") && !dependency.getArtifactId().equals("maven-archiver") && !dependency.getArtifactId().equals("maven-jxr") && !Artifact.SCOPE_PROVIDED.equals(dependency.getScope())) {
