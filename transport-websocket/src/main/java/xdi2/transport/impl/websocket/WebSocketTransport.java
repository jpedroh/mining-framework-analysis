...
public class WebSocketTransport extends AbstractTransport<WebSocketTransportRequest, WebSocketTransportResponse> {
  ...
  @Override public void shutdown() throws Exception {
    ...
    if (exs.size() > 1) {
      throw new Exception("Multiple exceptions while shutting down: " + exs);
    }
    if (exs.size() > 0) {
      throw exs.get(0);
    }
  ...
  @Override public void execute(WebSocketTransportRequest request, WebSocketTransportResponse response) throws IOException {
    if (log.isInfoEnabled()) {
      log.info("Incoming message to " + request.getRequestPath() + ". Subprotocol: " + request.getNegotiatedSubprotocol());
    }
    try {
  ...
  public void registerSession(Session session, XDIArc toPeerRootXDIArc) {
    if (log.isDebugEnabled()) {
      log.debug("Registering session " + session.getId() + " for TO peer root " + toPeerRootXDIArc);
    }
  ...
  public Session findSession(XDIArc toPeerRootXDIArc) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Found session " + (session == null ? null : session.getId()) + " for TO peer root " + toPeerRootXDIArc);
    }
    return session;
  }
