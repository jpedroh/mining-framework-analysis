 ...
 * Time: 09:18
 */
@RunWith(value = SpringJUnit4ClassRunner.class) @ContextConfiguration(classes = WorkerManagerTest.Configurator.class) public class WorkerManagerTest {
  @Autowired private WorkerManager workerManager;

  @Autowired private WorkerNodeService workerNodeService;

  @Autowired private WorkerRecoveryManager workerRecoveryManager;
  ...
  @After public void shutdownWorker() throws Exception {
  ...
  @Test public void startUpWrongVersion() throws Exception {
    workerManager.onApplicationEvent(mock(ContextClosedEvent.class));
    assertThat(workerManager.isUp()).isFalse();
    reset(workerNodeService);
  ...
  @Test public void startUpWithFailure() throws Exception {
    workerManager.onApplicationEvent(mock(ContextClosedEvent.class));
    assertThat(workerManager.isUp()).isFalse();
    reset(workerNodeService);
    doThrow(new RuntimeException("try 1")).doThrow(new RuntimeException("try 2")).doThrow(new RuntimeException("try 3")).doReturn("1").when(workerNodeService).up(CREDENTIAL_UUID, "version", "123");
    workerManager.onApplicationEvent(mock(ContextRefreshedEvent.class));
    Thread.sleep(2000L);
  ...
  @Test(timeout = 10000) public void testKeepAliveFailTriggerRecovery() {
  ...
  @Configuration static class Configurator {
    @Bean SynchronizationManager synchronizationManager() {
    ...
    @Bean Integer numberOfExecutionThreads() {
      return 2;
    }

    @Bean Double pollingMemoryRatio() {
      return 0.2D;
    }

    @Bean Long initStartUpSleep() {
      return 10L;
    }

    @Bean Long maxStartUpSleep() {
      return 100L;
    }

    @Bean WorkerConfigurationUtils workerConfigurationUtils() {
    ...
    @Bean WorkerStateUpdateService workerStateUpdateService() {
      return mock(WorkerStateUpdateService.class);
    }

    @Bean Integer inBufferCapacity() {
      return 20;
    }

    @Bean WorkerVersionService workerVersionService() {
