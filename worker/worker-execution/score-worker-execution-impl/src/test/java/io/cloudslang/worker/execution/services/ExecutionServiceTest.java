 ...
 * Time: 14:11
 */
@RunWith(value = SpringJUnit4ClassRunner.class) @ContextConfiguration @DirtiesContext(classMode = ClassMode.AFTER_EACH_TEST_METHOD) public class ExecutionServiceTest {
  ...
  @Autowired @Qualifier(value = "executionService") private ExecutionServiceImpl executionService;

  @Autowired @Qualifier(value = "timeoutExecutionService") private ExecutionServiceImpl timeoutExecutionService;
  ...
  @Test public void handlePausedFlow_NotPausedExecutionTest() throws InterruptedException {
  ...
  @Test public void handlePausedFlow_UserPausedTest() throws InterruptedException {
  ...
  @Test public void handlePausedFlow_UserPausedParentTest() throws InterruptedException {
    final Long executionId = 111L;
  ...
  @Test public void handleCancelledFlowsTest() {
  ...
  @Test public void handlePausedFlow_sequentialOperationReached() throws InterruptedException {
  ...
  @Test public void executeStepTest() throws InterruptedException {
    ExecutionStep executionStep = new ExecutionStep(EXECUTION_STEP_1_ID);
    ...
    assertEquals(0, exe.getPosition().longValue());
    assertTrue(exe.getSystemContext().hasStepErrorKey());
  }

  @Test public void executeStepTestWithEnabledTimeoutGoesWellWithoutExceptionOrTimeout() throws InterruptedException {
    ExecutionStep executionStep = new ExecutionStep(EXECUTION_STEP_1_ID);
    Map<String, Serializable> actionData = new HashMap<>();
    actionData.put("actionType", "content");
    executionStep.setActionData(actionData);
    executionStep.setAction(CONTENT_EXEC_CONTROL_ACTION_METADATA);
    when(reflectionAdapter.executeControlAction(eq(CONTENT_EXEC_CONTROL_ACTION_METADATA), any(ReadonlyStepActionDataAccessor.class))).thenReturn(null);
    Execution exe = new Execution(0L, 0L, new HashMap<String, String>());
  ...
  @Test public void executeStepTestWithEnabledTimeoutWithExceptionDuringCallable() throws InterruptedException {
    ExecutionStep executionStep = new ExecutionStep(EXECUTION_STEP_1_ID);
    Map<String, Serializable> actionData = new HashMap<>();
    actionData.put("actionType", "content");
    executionStep.setActionData(actionData);
    ...
    assertEquals(0, exe.getPosition().longValue());
    assertTrue(exe.getSystemContext().hasStepErrorKey());
    assertThat(exe.getSystemContext().getStepErrorKey(), containsString("ABC message"));
  }

  @Test public void executeNavigationTest() throws InterruptedException {
    ExecutionStep executionStep = new ExecutionStep(EXECUTION_STEP_1_ID);
  ...
  @Test public void postExecutionSettingsTest() {
  ...
  @Configuration static class ConfigurationForTest {
    @Bean public EventBus getEventBus() {
      return mock(EventBus.class);
    }

    @Bean(name = "consumptionFastEventBus") public FastEventBus getFastEventBus() {
      return mock(FastEventBus.class);
    }

    @Bean public ExecutionServiceImpl executionService() {
      return new ExecutionServiceImpl();
    }

    @Bean public ExecutionServiceImpl timeoutExecutionService() {
    ...
    @Bean public WorkerConfigurationService getWorkerConfigurationService() {
    ...
    @Bean public ReflectionAdapter getReflectionAdapter() {
    ...
    @Bean public WorkerDbSupportService getWorkerDbSupportService() {
      return mock(WorkerDbSupportService.class);
    }

    @Bean public PauseResumeService getPauseResumeService() {
      return mock(PauseResumeService.class);
    }

    @Bean public CancelExecutionService getCancelExecutionService() {
      return mock(CancelExecutionService.class);
    }

    @Bean public WorkerRecoveryManager workerRecoveryManager() {
      return mock(WorkerRecoveryManager.class);
    }

    @Bean public RobotAvailabilityService robotAvailabilityService() {
      return mock(RobotAvailabilityService.class);
    }

    @Bean public AplsLicensingService aplsLicensingService() {
