...
public final class ExecutionServiceImpl implements ExecutionService {
  ...
  @Autowired private PauseResumeService pauseService;

  @Autowired private ReflectionAdapter reflectionAdapter;

  @Autowired private WorkerDbSupportService workerDbSupportService;

  @Autowired private WorkerConfigurationService workerConfigurationService;

  @Autowired private EventBus eventBus;

  @Autowired private RobotAvailabilityService robotAvailabilityService;

  private static final int DEFAULT_PLATFORM_LEVEL_OPERATION_TIMEOUT_IN_SECONDS = 24 * 60 * 60;

  private static final int DEFAULT_PLATFORM_LEVEL_WAIT_PERIOD_FOR_TIMEOUT_IN_SECONDS = 5 * 60;
  ...
  public ExecutionServiceImpl() {
    this.operationTimeoutMillis = getSafeIntProperty("execution.operationTimeoutInSeconds", DEFAULT_PLATFORM_LEVEL_OPERATION_TIMEOUT_IN_SECONDS) * 1000L;
    this.waitPeriodForTimeoutMillis = getSafeIntProperty("execution.waitPeriodForTimeoutInSeconds", DEFAULT_PLATFORM_LEVEL_WAIT_PERIOD_FOR_TIMEOUT_IN_SECONDS) * 1000L;
    this.waitPauseForTimeoutMillis = getSafeLongProperty("execution.waitPauseForTimeoutInMillis", DEFAULT_PLATFORM_LEVEL_WAIT_PAUSE_FOR_TIMEOUT_IN_MILLIS);
  ...
  @Override public Execution execute(Execution execution) throws InterruptedException {
    try {
      if (handleCancelledFlow(execution) || (!isDebuggerMode(execution.getSystemContext()) && handlePausedFlow(execution))) {
      ...
      if ((!execution.getSystemContext().hasStepErrorKey()) && (currStep.getActionData().get(ACTION_TYPE) != null) && currStep.getActionData().get(ACTION_TYPE).toString().equalsIgnoreCase(SEQUENTIAL)) {
        return null;
      }
  ...
  @Override public void pauseSequentialExecution(Execution execution) throws InterruptedException {
    final PauseReason pauseReason = robotAvailabilityService.isRobotAvailable(execution.getSystemContext().getRobotGroupName()).size() > 0 ? PENDING_ROBOT : NO_ROBOTS_IN_GROUP;
    pauseFlow(execution, pauseReason);
  }

  @Override public void postExecutionWork(Execution execution) throws InterruptedException {
  ...
  private Execution doWaitForCancel(Execution execution) throws InterruptedException, TimeoutException {
    int iterations = ((int) (waitPeriodForTimeoutMillis / waitPauseForTimeoutMillis)) + 1;
  ...
  @Override public List<Execution> executeSplitForNonBlockAndParallel(Execution execution) throws InterruptedException {
    try {
      ExecutionStep currStep = loadExecutionStep(execution);
  ...
  @Override public List<Execution> executeSplitForMi(Execution execution, String splitUuid, int nrOfAlreadyCreatedBranches) throws InterruptedException {
    try {
      ExecutionStep currStep = loadExecutionStep(execution);
      if (!isDebuggerMode(execution.getSystemContext()) && handlePausedFlow(execution)) {
        return null;
      ...
      dumpBusEvents(execution);
      List<StartBranchDataContainer> newBranches = execution.getSystemContext().removeBranchesData();
      List<Execution> newExecutions = createChildExecutionsForMi(execution.getExecutionId(), newBranches, splitUuid, nrOfAlreadyCreatedBranches);
  ...
  private void failFlowIfSplitStepFailed(Execution execution) throws InterruptedException {
    if (execution.getSystemContext().hasStepErrorKey()) {
      ...
      execution.setPosition(null);
      try {
        createErrorEvent(exception, "Error occurred during split step ", SCORE_STEP_SPLIT_ERROR, execution.getSystemContext());
  ...
  @Override public boolean isSplitStep(Execution execution) {
    ExecutionStep currStep = loadExecutionStep(execution);
    return currStep.isSplitStep();
  }

  protected boolean handleCancelledFlow(Execution execution) {
    if (workerConfigurationService.isExecutionCancelled(execution.getExecutionId()) || (execution.getSystemContext().getFlowTerminationType() == ExecutionStatus.CANCELED)) {
  ...
  private void pauseFlow(Execution execution, PauseReason reason) throws InterruptedException {
    ...
    if (!isDebuggerMode(execution.getSystemContext()) && reason.equals(PauseReason.USER_PAUSED)) {
      ...
      if (reason == NO_ROBOTS_IN_GROUP || reason == PENDING_ROBOT) {
        pauseService.pauseExecution(executionId, branchId, reason);
      }
    }
  ...
  private PauseReason findPauseReason(Long executionId, String branchId) {
    if (workerConfigurationService.isExecutionPaused(executionId, branchId)) {
      ...
      if (branchId != null && workerConfigurationService.isExecutionPaused(executionId, null)) {
    ...
    try {
      ...
      if (enableNewTimeoutMechanism && isContentOperationStep(action)) {
        ...
        if ((startTime != null) && (timeoutMins != null)) {
          ...
          SandboxExecutionRunnable<Object> sandboxExecutionRunnable = new SandboxExecutionRunnable<>(currentThread().getContextClassLoader(), operationCallable);
          ...
          if (dynamicTimeout == -1L) {
            String timeoutErrorMessageBeforeStep = String.format("Timeout (%d minutes) exceeded for execution id %s having start time %s (current time %s) before executing step %s", timeoutMins, valueOf(execution.getExecutionId()), valueOf(startTime), valueOf(now), valueOf(currStep.getExecStepId()));
          ...
          if (operationExecutionThread.isAlive()) {
            String timeoutErrorMessageDuringStep = String.format("Timeout (%d minutes) exceeded for execution id %s having start time %s (current time %s) when running step %s", timeoutMins, valueOf(execution.getExecutionId()), valueOf(startTime), valueOf(System.currentTimeMillis()), valueOf(currStep.getExecStepId()));
  ...
  private boolean isContentOperationStep(ControlActionMetadata action) {
    return (action != null) && StringUtils.equals(action.getMethodName(), EXECUTE_CONTENT_ACTION) && endsWith(action.getClassName(), EXECUTE_CONTENT_ACTION_CLASSNAME);
  }
