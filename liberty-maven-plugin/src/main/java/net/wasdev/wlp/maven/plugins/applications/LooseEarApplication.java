...
public class LooseEarApplication extends LooseApplication {
  ...
  public void addApplicationXmlFile() throws Exception {
    ...
    if (path != null && !path.isEmpty()) {
      ...
      if (getPluginConfiguration(project, "org.apache.maven.plugins", "maven-ear-plugin", "generateApplicationXml") == null || getPluginConfiguration(project, "org.apache.maven.plugins", "maven-ear-plugin", "generateApplicationXml").equals("true")) {
      ...
      if (val != null) {
        ...
        if (modules != null) {
          for (int i = 0; i < modules.length; i++) {
            if (groupId.equals(getConfigValue(modules[i].getChild("groupId"))) && artifactId.equals(getConfigValue(modules[i].getChild("artifactId")))) {
  ...
  public String getEarFileNameMapping() {
    String fileNameMapping = getPluginConfiguration(project, "org.apache.maven.plugins", "maven-ear-plugin", "fileNameMapping");
  ...
  public boolean isEarDependency(Artifact artifact) {
    ...
    for (Artifact dep : deps) {
      if (("compile".equals(artifact.getScope()) || "runtime".equals(artifact.getScope())) && "jar".equals(dep.getType()) && artifact.getGroupId().equals(dep.getGroupId()) && artifact.getArtifactId().equals(dep.getArtifactId()) && artifact.getVersion().equals(dep.getVersion())) {
        return true;
      }
    }
    return false;
