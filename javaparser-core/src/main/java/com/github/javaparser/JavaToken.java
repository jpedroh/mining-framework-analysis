...
public class JavaToken {
  ...
  JavaToken(Token token, List<JavaToken> tokens) {
    ...
    if (token.kind == GeneratedJavaParserConstants.GT) {
      ...
      if (token.kind == GeneratedJavaParserConstants.RSIGNEDSHIFT) {
  ...
  public JavaToken(int kind) {
    ...
    if (TokenTypes.isEndOfLineToken(kind)) {
      ...
      if (TokenTypes.isWhitespace(kind)) {
        content = " ";
      }
    }
    this.kind = kind;
  ...
  @Generated(value = "com.github.javaparser.generator.core.other.TokenKindGenerator") public enum Kind {
    EOF(0),
    SPACE(1),
    WINDOWS_EOL(2),
    UNIX_EOL(3),
