...
class CommentsInserter {
  ...
  void insertComments(Node node, TreeSet<Comment> commentsToAttribute) {
    ...
    for (Node child : children) {
      ...
      commentsInsideChild.addAll(commentsToAttribute.stream().filter((c) -> c.getRange().isPresent()).filter((c) -> PositionUtils.nodeContains(child, c, configuration.isDoNotConsiderAnnotationsAsNodeStartForCodeAttribution())).collect(Collectors.toList()));
    ...
    PositionUtils.sortByBeginPosition(childrenAndComments, configuration.isDoNotConsiderAnnotationsAsNodeStartForCodeAttribution());
  ...
  private boolean attributeLineCommentToNodeOrChild(Node node, LineComment lineComment) {
    ...
    if (node.getBegin().get().line == lineComment.getBegin().get().line && !node.hasComment()) {
