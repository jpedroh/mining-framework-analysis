...
public class CommandSend extends Command implements TabExecutor {
  ...
  protected static class SendCallback {
    ...
    public void lastEntryDone() {
      ...
      for (Map.Entry<ServerConnectRequest.Result, List<String>> entry : results.entrySet()) {
        ...
        serverText.event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(Joiner.on(", ").join(serverNames)).color(ChatColor.YELLOW).append((rem > 0) ? " and " + rem + " more" : "", ComponentBuilder.FormatRetention.ALL).create()));
        serverText.append(ProxyServer.getInstance().getTranslation("command_send_server_title", CaseFormat.UPPER_UNDERSCORE.converterTo(CaseFormat.UPPER_CAMEL).convert(entry.getKey().name())), ComponentBuilder.FormatRetention.EVENTS);
        serverText.append(TextComponent.fromLegacyText(ProxyServer.getInstance().getTranslation("command_send_counter", entry.getValue().size())), ComponentBuilder.FormatRetention.EVENTS);
  ...
  @Override public void execute(CommandSender sender, String[] args) {
    ...
    sender.sendMessage(ProxyServer.getInstance().getTranslation("command_send_attempting", (targets.size() == 1) ? targets.get(0).getName() : targets.size() + " players", server.getName()));
...
=======
    targets.stream().map((player) -> ServerConnectRequest.builder().target(server).reason(ServerConnectEvent.Reason.COMMAND).callback(new SendCallback.Entry(callback, player, server)).build()).forEach((request) -> targets.forEach((player) -> player.connect(request)));
