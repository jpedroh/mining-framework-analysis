...
@Configuration @ComponentScan(basePackages = "com.findwise.hydra.admin.rest") public class AppConfig {
  ...
  @Bean @Autowired public MongoConnector connector(DatabaseConfig config) {
    return new MongoConnector(config);
  }

  @Bean public DatabaseConfig databaseConfig() {
  ...
  @Bean(name = "jsonpCallbackFilter") public static JsonpCallbackFilter jsonpCallback() {
  ...
  @Bean public PropertyPlaceholderConfigurer properties() {
    ...
    final Resource[] resources = new Resource[] { new ClassPathResource(PROPERTIES_FILE), new FileSystemResource(propertiesFile) };
  ...
  @Bean public ConfigurationService<MongoType> configurationService(MongoConnector connector) {
    return new ConfigurationService<MongoType>(connector);
  }

  @Bean public DocumentsService<MongoType> documentsService(MongoConnector connector) {
    return new DocumentsService<MongoType>(connector);
  }

  @Bean public StagesService<MongoType> stagesService(MongoConnector connector) {
