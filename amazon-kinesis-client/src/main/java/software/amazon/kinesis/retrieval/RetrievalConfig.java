 ...
 */
@Getter @Setter @ToString @EqualsAndHashCode @Accessors(fluent = true) public class RetrievalConfig {
  /**
  ...
  /**
     ...
     */
  @Deprecated private InitialPositionInStreamExtended initialPositionInStreamExtended = InitialPositionInStreamExtended.newInitialPosition(InitialPositionInStream.LATEST);
  ...
  public RetrievalConfig(@NonNull KinesisAsyncClient kinesisAsyncClient, @NonNull String streamName, @NonNull String applicationName) {
    this(kinesisAsyncClient, new SingleStreamTracker(streamName, kinesisAsyncClient.serviceClientConfiguration().region()), applicationName);
  }

  public RetrievalConfig(@NonNull KinesisAsyncClient kinesisAsyncClient, @NonNull StreamTracker streamTracker, @NonNull String applicationName) {
    this.kinesisClient = kinesisAsyncClient;
    this.streamTracker = streamTracker;
    this.applicationName = applicationName;
    this.appStreamTracker = DeprecationUtils.convert(streamTracker, (singleStreamTracker) -> singleStreamTracker.streamConfigList().get(0));
  }
