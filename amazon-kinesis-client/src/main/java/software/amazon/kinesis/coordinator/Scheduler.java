...
@Getter @Accessors(fluent = true) @Slf4j public class Scheduler implements Runnable {
  ...
  @VisibleForTesting protected Scheduler(@NonNull final CheckpointConfig checkpointConfig, @NonNull final CoordinatorConfig coordinatorConfig, @NonNull final LeaseManagementConfig leaseManagementConfig, @NonNull final LifecycleConfig lifecycleConfig, @NonNull final MetricsConfig metricsConfig, @NonNull final ProcessorConfig processorConfig, @NonNull final RetrievalConfig retrievalConfig, @NonNull final DiagnosticEventFactory diagnosticEventFactory) {
    ...
    this.currentStreamConfigMap = this.retrievalConfig.appStreamTracker().map((multiStreamTracker) -> {
    ...
    this.shardSyncTaskManagerProvider = (streamConfig) -> this.leaseManagementConfig.leaseManagementFactory(leaseSerializer, isMultiStreamMode).createShardSyncTaskManager(this.metricsFactory, streamConfig);
  ...
  @VisibleForTesting Set<StreamIdentifier> checkAndSyncStreamShardsAndLeases() throws DependencyException, ProvisionedThroughputException, InvalidStateException {
    ...
    if (shouldSyncStreamsNow()) {
      ...
      try {
        ...
        newStreamConfigMap.putAll(multiStreamTracker.streamConfigList().stream().collect(Collectors.toMap((sc) -> sc.streamIdentifier(), (sc) -> sc)));
        ...
        if (formerStreamsLeasesDeletionStrategy.leaseDeletionType() == StreamsLeasesDeletionType.FORMER_STREAMS_AUTO_DETECTION_DEFERRED_DELETION) {
          ...
          if (formerStreamsLeasesDeletionStrategy.leaseDeletionType() == StreamsLeasesDeletionType.PROVIDED_STREAMS_DEFERRED_DELETION) {
            Optional.ofNullable(formerStreamsLeasesDeletionStrategy.streamIdentifiersForLeaseCleanup()).ifPresent((streamIdentifiers) -> streamIdentifiers.stream().forEach((streamIdentifier) -> enqueueStreamLeaseDeletionOperation.accept(streamIdentifier)));
          } else {
        ...
        final Map<Boolean, Set<StreamIdentifier>> staleStreamIdDeletionDecisionMap = staleStreamDeletionMap.keySet().stream().collect(Collectors.partitioningBy((streamIdentifier) -> newStreamConfigMap.containsKey(streamIdentifier), Collectors.toSet()));
  ...
  private void syncStreamsFromLeaseTableOnAppInit(List<MultiStreamLease> leases) {
    final Set<StreamIdentifier> streamIdentifiers = leases.stream().map((lease) -> StreamIdentifier.multiStreamInstance(lease.streamIdentifier())).collect(Collectors.toSet());
