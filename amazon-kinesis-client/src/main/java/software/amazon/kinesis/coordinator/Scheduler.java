...
@Getter @Accessors(fluent = true) @Slf4j public class Scheduler implements Runnable {
  ...
  @Getter(value = AccessLevel.NONE) private boolean shouldSyncLeases = true;
  ...
  @VisibleForTesting protected Scheduler(@NonNull final CheckpointConfig checkpointConfig, @NonNull final CoordinatorConfig coordinatorConfig, @NonNull final LeaseManagementConfig leaseManagementConfig, @NonNull final LifecycleConfig lifecycleConfig, @NonNull final MetricsConfig metricsConfig, @NonNull final ProcessorConfig processorConfig, @NonNull final RetrievalConfig retrievalConfig, @NonNull final DiagnosticEventFactory diagnosticEventFactory) {
    ...
    this.shardSyncTaskManagerProvider = (streamConfig) -> this.leaseManagementConfig.leaseManagementFactory(leaseSerializer, isMultiStreamMode).createShardSyncTaskManager(this.metricsFactory, streamConfig, this.deletedStreamListProvider);
    ...
    this.leaderElectedPeriodicShardSyncManager = new PeriodicShardSyncManager(leaseManagementConfig.workerIdentifier(), leaderDecider, leaseRefresher, currentStreamConfigMap, shardSyncTaskManagerProvider, isMultiStreamMode, metricsFactory, leaseManagementConfig.leasesRecoveryAuditorExecutionFrequencyMillis(), leaseManagementConfig.leasesRecoveryAuditorInconsistencyConfidenceThreshold());
    this.leaseCleanupManager = this.leaseManagementConfig.leaseManagementFactory(leaseSerializer, isMultiStreamMode).createLeaseCleanupManager(metricsFactory);
  ...
  @VisibleForTesting boolean shouldInitiateLeaseSync() throws InterruptedException, DependencyException, ProvisionedThroughputException, InvalidStateException {
  ...
  @VisibleForTesting void runProcessLoop() {
    try {
      ...
      for (ShardInfo shardInfo : getShardInfoForAssignments()) {
        ShardConsumer shardConsumer = createOrGetShardConsumer(shardInfo, processorConfig.shardRecordProcessorFactory(), leaseCleanupManager);
      ...
      log.error("Worker.run caught exception, sleeping for {} milli seconds!", shardConsumerDispatchPollIntervalMillis, e);
      try {
        Thread.sleep(shardConsumerDispatchPollIntervalMillis);
  ...
  /**
     ...
     */
  @VisibleForTesting Set<StreamIdentifier> checkAndSyncStreamShardsAndLeases() throws DependencyException, ProvisionedThroughputException, InvalidStateException {
    ...
    if (shouldSyncStreamsNow()) {
      ...
      try {
        final Map<StreamIdentifier, StreamConfig> newStreamConfigMap = streamTracker.streamConfigList().stream().collect(Collectors.toMap(StreamConfig::streamIdentifier, Function.identity()));
        ...
        if (formerStreamsLeasesDeletionStrategy.leaseDeletionType() == FORMER_STREAMS_AUTO_DETECTION_DEFERRED_DELETION) {
          ...
          if (formerStreamsLeasesDeletionStrategy.leaseDeletionType() == StreamsLeasesDeletionType.PROVIDED_STREAMS_DEFERRED_DELETION) {
            Optional.ofNullable(formerStreamsLeasesDeletionStrategy.streamIdentifiersForLeaseCleanup()).ifPresent((streamIdentifiers) -> streamIdentifiers.forEach(enqueueStreamLeaseDeletionOperation));
          } else {
            ...
            while (currentSetOfStreamsIter.hasNext()) {
              ...
              if (!newStreamConfigMap.containsKey(streamIdentifier)) {
                if (SHOULD_DO_LEASE_SYNC_FOR_OLD_STREAMS) {
                  log.info("Found old/deleted stream : {}. Triggering shard sync. Removing from tracked active streams.", streamIdentifier);
                  ShardSyncTaskManager shardSyncTaskManager = createOrGetShardSyncTaskManager(currentStreamConfigMap.get(streamIdentifier));
                  shardSyncTaskManager.submitShardSyncTask();
                } else {
                  log.info("Found old/deleted stream : {}. Removing from tracked active streams, but not cleaning up leases," + " as part of this workflow", streamIdentifier);
                }
        ...
        final Map<Boolean, Set<StreamIdentifier>> staleStreamIdDeletionDecisionMap = staleStreamDeletionMap.keySet().stream().collect(Collectors.partitioningBy(newStreamConfigMap::containsKey, Collectors.toSet()));
        final Set<StreamIdentifier> staleStreamIdsToBeDeleted = staleStreamIdDeletionDecisionMap.get(false).stream().filter((streamIdentifier) -> Duration.between(staleStreamDeletionMap.get(streamIdentifier), Instant.now()).toMillis() >= waitPeriodToDeleteOldStreams.toMillis()).collect(Collectors.toSet());
        final Set<StreamIdentifier> deletedStreamSet = this.deletedStreamListProvider.purgeAllDeletedStream().stream().filter((streamIdentifier) -> !newStreamConfigMap.containsKey(streamIdentifier)).collect(Collectors.toSet());
        ...
        if (!staleStreamDeletionMap.isEmpty()) {
          log.warn("Streams enqueued for deletion for lease table cleanup along with their scheduled time for deletion: {} ", staleStreamDeletionMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, (entry) -> entry.getValue().plus(waitPeriodToDeleteOldStreams))));
        }
        ...
        MetricsUtil.addCount(metricsScope, PENDING_STREAMS_DELETION_COUNT, staleStreamDeletionMap.size(), MetricsLevel.SUMMARY);
        MetricsUtil.addCount(metricsScope, NON_EXISTING_STREAM_DELETE_COUNT, deletedStreamSet.size(), MetricsLevel.SUMMARY);
  ...
  @VisibleForTesting void syncStreamsFromLeaseTableOnAppInit(List<MultiStreamLease> leases) {
    leases.stream().map((lease) -> StreamIdentifier.multiStreamInstance(lease.streamIdentifier())).filter((streamIdentifier) -> !currentStreamConfigMap.containsKey(streamIdentifier)).forEach((streamIdentifier) -> {
  ...
  private Set<StreamIdentifier> deleteMultiStreamLeases(Set<StreamIdentifier> streamIdentifiers) throws DependencyException, ProvisionedThroughputException, InvalidStateException {
    ...
    final Map<String, List<MultiStreamLease>> streamIdToShardsMap = leases.stream().collect(Collectors.groupingBy(MultiStreamLease::streamIdentifier, Collectors.toCollection(ArrayList::new)));
    for (StreamIdentifier streamIdentifier : streamIdentifiers) {
    ...
    if (leases != null) {
      for (MultiStreamLease lease : leases) {
        try {
          ...
          log.error("Unable to delete stale stream lease {}. Skipping further deletions for this stream. Will retry later.", lease.leaseKey(), e);
          return false;
        }
      }
    }
  ...
  /**
     ...
     */
  ShardConsumer createOrGetShardConsumer(@NonNull final ShardInfo shardInfo, @NonNull final ShardRecordProcessorFactory shardRecordProcessorFactory, @NonNull final LeaseCleanupManager leaseCleanupManager) {
    ShardConsumer consumer = shardInfoShardConsumerMap.get(shardInfo);
  ...
  protected ShardConsumer buildConsumer(@NonNull final ShardInfo shardInfo, @NonNull final ShardRecordProcessorFactory shardRecordProcessorFactory, @NonNull final LeaseCleanupManager leaseCleanupManager) {
