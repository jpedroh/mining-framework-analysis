 ...
 */
@Slf4j @KinesisClientInternalApi public class HierarchicalShardSyncer {
  /**
     ...
     */
  public synchronized void checkAndCreateLeaseForNewShards(@NonNull final ShardDetector shardDetector, final LeaseRefresher leaseRefresher, final InitialPositionInStreamExtended initialPosition, final boolean cleanupLeasesOfCompletedShards, final boolean ignoreUnexpectedChildShards, final MetricsScope scope) throws DependencyException, InvalidStateException, ProvisionedThroughputException, KinesisClientLibIOException {
    final List<Shard> 
<<<<<<< /usr/src/app/output/awslabs/amazon-kinesis-client/4a5a42f8359123d49e1752a7a488564c578671a9/amazon-kinesis-client/src/main/java/software/amazon/kinesis/leases/HierarchicalShardSyncer.java/left.java
    shards = getShardList(shardDetector)
=======
    latestShards = getShardList(shardDetector)
>>>>>>> /usr/src/app/output/awslabs/amazon-kinesis-client/4a5a42f8359123d49e1752a7a488564c578671a9/amazon-kinesis-client/src/main/java/software/amazon/kinesis/leases/HierarchicalShardSyncer.java/right.java
    ;
    checkAndCreateLeaseForNewShards(shards, shardDetector, leaseRefresher, initialPosition, cleanupLeasesOfCompletedShards, ignoreUnexpectedChildShards, scope, latestShards);
  }

  public synchronized void checkAndCreateLeaseForNewShards(List<Shard> shards, @NonNull final ShardDetector shardDetector, final LeaseRefresher leaseRefresher, final InitialPositionInStreamExtended initialPosition, final boolean cleanupLeasesOfCompletedShards, final boolean ignoreUnexpectedChildShards, final MetricsScope scope, List<Shard> latestShards) throws DependencyException, InvalidStateException, ProvisionedThroughputException, KinesisClientLibIOException {
...
>>>>>>> /usr/src/app/output/awslabs/amazon-kinesis-client/4a5a42f8359123d49e1752a7a488564c578671a9/amazon-kinesis-client/src/main/java/software/amazon/kinesis/leases/HierarchicalShardSyncer.java/right.java
    ...
    final Map<String, Set<String>> shardIdToChildShardIdsMap = constructShardIdToChildShardIdsMap(shardIdToShardMap);
  ...
  /** Helper method to detect a race condition between fetching the shards via paginated DescribeStream calls
     ...
     * @throws KinesisClientLibIOException
     */
  private static void assertAllParentShardsAreClosed(final Set<String> inconsistentShardIds) throws KinesisClientLibIOException {
  ...
  /**
     ...
     */
  private static Set<String> findInconsistentShardIds(final Map<String, Set<String>> shardIdToChildShardIdsMap, final Map<String, Shard> shardIdToShardMap) {
    return shardIdToChildShardIdsMap.entrySet().stream().filter((entry) -> entry.getKey() == null || shardIdToShardMap.get(entry.getKey()).sequenceNumberRange().endingSequenceNumber() == null).flatMap((entry) -> shardIdToChildShardIdsMap.get(entry.getKey()).stream()).collect(Collectors.toSet());
  }
  ...
  /**
     ...
     * @throws KinesisClientLibIOException
     */
  synchronized void assertClosedShardsAreCoveredOrAbsent(final Map<String, Shard> shardIdToShardMap, final Map<String, Set<String>> shardIdToChildShardIdsMap, final Set<String> shardIdsOfClosedShards) throws KinesisClientLibIOException {
    final String exceptionMessageSuffix = "This can happen if we constructed the list of shards " + " while a reshard operation was in progress.";
  ...
  private synchronized void assertHashRangeOfClosedShardIsCovered(final Shard closedShard, final Map<String, Shard> shardIdToShardMap, final Set<String> childShardIds) throws KinesisClientLibIOException {
  ...
  static Map<String, Set<String>> constructShardIdToChildShardIdsMap(final Map<String, Shard> shardIdToShardMap) {
    ...
    for (final Map.Entry<String, Shard> entry : shardIdToShardMap.entrySet()) {
      ...
      if (parentShardId != null && shardIdToShardMap.containsKey(parentShardId)) {
        final Set<String> childShardIds = shardIdToChildShardIdsMap.computeIfAbsent(parentShardId, (key) -> new HashSet<>());
      ...
      if (adjacentParentShardId != null && shardIdToShardMap.containsKey(adjacentParentShardId)) {
        final Set<String> childShardIds = shardIdToChildShardIdsMap.computeIfAbsent(adjacentParentShardId, (key) -> new HashSet<>());
        childShardIds.add(shardId);
      }
    }
  ...
  /**
     ...
     */
  static List<Lease> determineNewLeasesToCreate(final List<Shard> shards, final List<Lease> currentLeases, final InitialPositionInStreamExtended initialPosition, final Set<String> inconsistentShardIds) {
    ...
    final Set<String> shardIdsOfCurrentLeases = currentLeases.stream().peek((lease) -> log.debug("Existing lease: {}", lease)).map(Lease::leaseKey).collect(Collectors.toSet());
    ...
    for (Shard shard : openShards) {
      ...
      if (shardIdsOfCurrentLeases.contains(shardId)) {
        ...
        if (inconsistentShardIds.contains(shardId)) {
          ...
          final boolean isDescendant = checkIfDescendantAndAddNewLeasesForAncestors(shardId, initialPosition, shardIdsOfCurrentLeases, shardIdToShardMapOfAllKinesisShards, shardIdToNewLeaseMap, memoizationContext);
    ...
    final Comparator<Lease> startingSequenceNumberComparator = new StartingSequenceNumberAndShardIdBasedComparator(shardIdToShardMapOfAllKinesisShards);
  ...
  /**
     ...
     * Note: Package level access only for testing purposes.
     */
  static List<Lease> determineNewLeasesToCreate(final List<Shard> shards, final List<Lease> currentLeases, final InitialPositionInStreamExtended initialPosition) {
  ...
  /**
     ...
     */
  static boolean checkIfDescendantAndAddNewLeasesForAncestors(final String shardId, final InitialPositionInStreamExtended initialPosition, final Set<String> shardIdsOfCurrentLeases, final Map<String, Shard> shardIdToShardMapOfAllKinesisShards, final Map<String, Lease> shardIdToLeaseMapOfNewShards, final Map<String, Boolean> memoizationContext) {
    ...
    if (shardId != null && shardIdToShardMapOfAllKinesisShards.containsKey(shardId)) {
      if (shardIdsOfCurrentLeases.contains(shardId)) {
        ...
        for (String parentShardId : parentShardIds) {
          if (checkIfDescendantAndAddNewLeasesForAncestors(parentShardId, initialPosition, shardIdsOfCurrentLeases, shardIdToShardMapOfAllKinesisShards, shardIdToLeaseMapOfNewShards, memoizationContext)) {
            isDescendant = true;
  ...
  /**
     ...
     */
  static Set<String> getParentShardIds(final Shard shard, final Map<String, Shard> shardIdToShardMapOfAllKinesisShards) {
  ...
  /**
     ...
     * @throws DependencyException
     */
  private static void cleanupGarbageLeases(@NonNull final ShardDetector shardDetector, final List<Shard> shards, final List<Lease> trackedLeases, final LeaseRefresher leaseRefresher) throws KinesisClientLibIOException, DependencyException, InvalidStateException, ProvisionedThroughputException {
    final Set<String> kinesisShards = shards.stream().map(Shard::shardId).collect(Collectors.toSet());
    final List<Lease> garbageLeases = trackedLeases.stream().filter((lease) -> isCandidateForCleanup(lease, kinesisShards)).collect(Collectors.toList());
    if (!CollectionUtils.isNullOrEmpty(garbageLeases)) {
      log.info("Found {} candidate leases for cleanup. Refreshing list of" + " Kinesis shards to pick up recent/latest shards", garbageLeases.size());
      final Set<String> currentKinesisShardIds = getShardList(shardDetector).stream().map(Shard::shardId).collect(Collectors.toSet());
  ...
  /**
     ...
     */
  static boolean isCandidateForCleanup(final Lease lease, final Set<String> currentKinesisShardIds) throws KinesisClientLibIOException {
    ...
    if (currentKinesisShardIds.contains(lease.leaseKey())) {
      ...
      for (String parentShardId : parentShardIds) {
        if (currentKinesisShardIds.contains(parentShardId)) {
          final String message = String.format("Parent shard %s exists but not the child shard %s", parentShardId, lease.leaseKey());
          log.info(message);
  ...
  /**
     ...
     * @throws KinesisClientLibIOException
     */
  private synchronized void cleanupLeasesOfFinishedShards(final Collection<Lease> currentLeases, final Map<String, Shard> shardIdToShardMap, final Map<String, Set<String>> shardIdToChildShardIdsMap, final List<Lease> trackedLeases, final LeaseRefresher leaseRefresher) throws DependencyException, InvalidStateException, ProvisionedThroughputException, KinesisClientLibIOException {
    final List<Lease> leasesOfClosedShards = currentLeases.stream().filter((lease) -> lease.checkpoint().equals(ExtendedSequenceNumber.SHARD_END)).collect(Collectors.toList());
    final Set<String> shardIdsOfClosedShards = leasesOfClosedShards.stream().map(Lease::leaseKey).collect(Collectors.toSet());
    if (!CollectionUtils.isNullOrEmpty(leasesOfClosedShards)) {
      ...
      Comparator<? super Lease> startingSequenceNumberComparator = new StartingSequenceNumberAndShardIdBasedComparator(shardIdToShardMap);
      leasesOfClosedShards.sort(startingSequenceNumberComparator);
      final Map<String, Lease> trackedLeaseMap = trackedLeases.stream().collect(Collectors.toMap(Lease::leaseKey, Function.identity()));
  ...
  /** 
     ...
     * @throws DependencyException 
     */
  synchronized void cleanupLeaseForClosedShard(final String closedShardId, final Set<String> childShardIds, final Map<String, Lease> trackedLeases, final LeaseRefresher leaseRefresher) throws DependencyException, InvalidStateException, ProvisionedThroughputException {
    final Lease leaseForClosedShard = trackedLeases.get(closedShardId);
    final List<Lease> childShardLeases = childShardIds.stream().map(trackedLeases::get).filter(Objects::nonNull).collect(Collectors.toList());
    if (leaseForClosedShard != null && leaseForClosedShard.checkpoint().equals(ExtendedSequenceNumber.SHARD_END) && childShardLeases.size() == childShardIds.size()) {
      ...
      if (okayToDelete) {
        log.info("Deleting lease for shard {} as it has been completely processed and processing of child " + "shards has begun.", leaseForClosedShard.leaseKey());
  ...
  static List<Shard> getOpenShards(final List<Shard> allShards) {
    return allShards.stream().filter((shard) -> shard.sequenceNumberRange().endingSequenceNumber() == null).peek((shard) -> log.debug("Found open shard: {}", shard.shardId())).collect(Collectors.toList());
  }
  ...
  private static ExtendedSequenceNumber convertToCheckpoint(final InitialPositionInStreamExtended position) {
    ...
    if (position.getInitialPositionInStream().equals(InitialPositionInStream.TRIM_HORIZON)) {
      ...
      if (position.getInitialPositionInStream().equals(InitialPositionInStream.LATEST)) {
        checkpoint = ExtendedSequenceNumber.LATEST;
      } else {
        if (position.getInitialPositionInStream().equals(InitialPositionInStream.AT_TIMESTAMP)) {
  ...
  @RequiredArgsConstructor private static class StartingSequenceNumberAndShardIdBasedComparator implements Comparator<Lease>, Serializable {
    ...
    /**
         ...
         * {@inheritDoc}
         */
    @Override public int compare(final Lease lease1, final Lease lease2) {
      int result = 0;
