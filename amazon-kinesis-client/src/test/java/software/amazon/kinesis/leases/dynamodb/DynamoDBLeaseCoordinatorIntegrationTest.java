...
@RunWith(value = MockitoJUnitRunner.class) public class DynamoDBLeaseCoordinatorIntegrationTest extends LeaseIntegrationTest {
  ...
  @Before public void setup() throws ProvisionedThroughputException, DependencyException, InvalidStateException {
    coordinator = new DynamoDBLeaseCoordinator(leaseRefresher, WORKER_ID, LEASE_DURATION_MILLIS, EPSILON_MILLIS, LEASE_TAKER_INTERVAL_MILLIS, MAX_LEASES_FOR_WORKER, MAX_LEASES_TO_STEAL_AT_ONE_TIME, MAX_LEASE_RENEWER_THREAD_COUNT, INITIAL_LEASE_TABLE_READ_CAPACITY, INITIAL_LEASE_TABLE_WRITE_CAPACITY, metricsFactory);
  ...
  /**
     ...
     */
  @Test public void testUpdateCheckpoint() throws Exception {
  ...
  /**
     ...
     */
  @Test public void testGetAllAssignments() throws Exception {
    TestHarnessBuilder builder = new TestHarnessBuilder();
    Map<String, Lease> addedLeases = builder.withLease("1", WORKER_ID).withLease("2", WORKER_ID).withLease("3", WORKER_ID).withLease("4", WORKER_ID).withLease("5", WORKER_ID).build();
    coordinator.runLeaseTaker();
  ...
  /**
     ...
     */
  @Test public void testUpdateCheckpointLeaseUpdated() throws LeasingException {
    TestHarnessBuilder builder = new TestHarnessBuilder();
    builder.withLease(leaseKey, null).build();
    coordinator.runLeaseTaker();
    coordinator.runLeaseRenewer();
  ...
  /**
     ...
     */
  @Test public void testUpdateCheckpointBadConcurrencyToken() throws LeasingException {
    TestHarnessBuilder builder = new TestHarnessBuilder();
    builder.withLease(leaseKey, null).build();
    coordinator.runLeaseTaker();
    coordinator.runLeaseRenewer();
