...
@Slf4j public class LeaseCoordinatorExerciser {
  ...
  public static void main(String[] args) throws DependencyException, InvalidStateException, ProvisionedThroughputException {
    ...
    LeaseRefresher leaseRefresher = new DynamoDBLeaseRefresher("nagl_ShardProgress", dynamoDBClient, new DynamoDBLeaseSerializer(), true, TableCreatorCallback.NOOP_TABLE_CREATOR_CALLBACK);
    ...
    CloudWatchAsyncClient client = CloudWatchAsyncClient.builder().credentialsProvider(DefaultCredentialsProvider.create()).build();
    CloudWatchMetricsFactory metricsFactory = new CloudWatchMetricsFactory(client, "testNamespace", 30 * 1000, 1000, METRICS_LEVEL, MetricsConfig.METRICS_DIMENSIONS_ALL, FLUSH_SIZE);
    ...
    for (int i = 0; i < numCoordinators; i++) {
      ...
      LeaseCoordinator coord = new DynamoDBLeaseCoordinator(leaseRefresher, workerIdentifier, leaseDurationMillis, epsilonMillis, LEASE_TAKER_INTERVAL_MILLIS, MAX_LEASES_FOR_WORKER, MAX_LEASES_TO_STEAL_AT_ONE_TIME, MAX_LEASE_RENEWER_THREAD_COUNT, INITIAL_LEASE_TABLE_READ_CAPACITY, INITIAL_LEASE_TABLE_WRITE_CAPACITY, metricsFactory);
        ...
        while (true) {
          for (LeaseCoordinator coord : coordinators) {
            ...
            Collections.sort(asgn, new Comparator<Lease>() {
              @Override public int compare(final Lease arg0, final Lease arg1) {
            ...
            for (Lease lease : asgn) {
              ...
              builder.append(String.format("<font color=\"%s\">%03d</font>", String.format("#00%02x00", greenNess), Integer.parseInt(leaseKey))).append(" ");
            }
            builder.append("</html>");
