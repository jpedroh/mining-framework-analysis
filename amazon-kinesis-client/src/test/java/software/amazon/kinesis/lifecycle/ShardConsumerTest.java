 ...
 */
@RunWith(value = MockitoJUnitRunner.class) @Slf4j public class ShardConsumerTest {
  ...
  @Mock private ConsumerState blockedOnParentsState;

  @Mock private ConsumerTask blockedOnParentsTask;

  @Mock private ConsumerState initialState;

  @Mock private ConsumerTask initializeTask;

  @Mock private ConsumerState processingState;

  @Mock private ConsumerTask processingTask;

  @Mock private ConsumerState shutdownState;

  @Mock private ConsumerTask shutdownTask;

  @Mock private TaskResult initializeTaskResult;

  @Mock private TaskResult processingTaskResult;

  @Mock private TaskResult blockOnParentsTaskResult;

  @Mock private ConsumerState shutdownCompleteState;

  @Mock private ShardConsumerArgument shardConsumerArgument;

  @Mock private ConsumerState shutdownRequestedState;

  @Mock private ConsumerTask shutdownRequestedTask;

  @Mock private ConsumerState shutdownRequestedAwaitState;
  ...
  @Before public void before() {
    ...
    ThreadFactory factory = new ThreadFactoryBuilder().setNameFormat("test-" + testName.getMethodName() + "-%04d").setDaemon(true).build();
    executorService = new ThreadPoolExecutor(4, 4, 1, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), factory);
    processRecordsInput = ProcessRecordsInput.builder().isAtShardEnd(false).cacheEntryTime(Instant.now()).millisBehindLatest(1000L).records(Collections.emptyList()).build();
    initialTaskInput = TaskExecutionListenerInput.builder().shardInfo(shardInfo).taskType(TaskType.INITIALIZE).build();
    processTaskInput = TaskExecutionListenerInput.builder().shardInfo(shardInfo).taskType(TaskType.PROCESS).build();
    shutdownRequestedTaskInput = TaskExecutionListenerInput.builder().shardInfo(shardInfo).taskType(TaskType.SHUTDOWN_NOTIFICATION).build();
    shutdownRequestedAwaitTaskInput = TaskExecutionListenerInput.builder().shardInfo(shardInfo).taskType(TaskType.SHUTDOWN_COMPLETE).build();
    shutdownTaskInput = TaskExecutionListenerInput.builder().shardInfo(shardInfo).taskType(TaskType.SHUTDOWN).build();
  }
  ...
  private class TestPublisher implements RecordsPublisher {
    ...
    @Override public void start(ExtendedSequenceNumber extendedSequenceNumber, InitialPositionInStreamExtended initialPositionInStreamExtended) {
    }
  ...
  @Test(timeout = 1000L) public void testLeaseLossIsNonBlocking() throws Exception {
    CyclicBarrier taskCallBarrier = new CyclicBarrier(2);
  ...
  @Test public void testDataArrivesAfterProcessing2() throws Exception {
    CyclicBarrier taskCallBarrier = new CyclicBarrier(2);
    mockSuccessfulInitialize(null);
    mockSuccessfulProcessing(taskCallBarrier);
    mockSuccessfulShutdown(null);
  ...
  @Test @Ignore public final void testInitializationStateUponFailure() throws Exception {
  ...
  /**
     ...
     */
  @Test public final void testSuccessfulConsumerStateTransition() {
    ExecutorService directExecutorService = spy(executorService);
    doAnswer(this::directlyExecuteRunnable).when(directExecutorService).execute(any());
  ...
  /**
     ...
     */
  @Test public final void testConsumerNotTransitionsToProcessingWhenInitializationFails() {
    ...
    mockSuccessfulInitializeWithFailureTransition();
    mockSuccessfulProcessing(null);
    doAnswer(this::directlyExecuteRunnable).doThrow(new RejectedExecutionException()).when(failingService).execute(any());
  ...
  /**
     ...
     */
  @Test public final void testConsumerTransitionsToProcessingWithIntermittentInitializationFailures() {
    ExecutorService failingService = spy(executorService);
    final ShardConsumer consumer = createShardConsumer(failingService, blockedOnParentsState);
    mockSuccessfulUnblockOnParents();
    mockSuccessfulInitializeWithFailureTransition();
    mockSuccessfulProcessing(null);
    doAnswer(this::directlyExecuteRunnable).doThrow(new RejectedExecutionException()).doThrow(new RejectedExecutionException()).doThrow(new RejectedExecutionException()).doAnswer(this::directlyExecuteRunnable).when(failingService).execute(any());
  ...
  /**
     ...
     */
  @Test public final void testConsumerNotTransitionsToInitializingWhenWaitingOnParentsFails() {
    ExecutorService failingService = spy(executorService);
    final ShardConsumer consumer = createShardConsumer(failingService, blockedOnParentsState);
  ...
  /**
     ...
     */
  @Test(expected = RejectedExecutionException.class) public final void testInitializationStateUponSubmissionFailure() throws Exception {
  ...
  @Test public void testErrorThrowableInInitialization() throws Exception {
    final ShardConsumer consumer = createShardConsumer(recordsPublisher);
  ...
  @Test public void testRequestedShutdownWhileQuiet() throws Exception {
    CyclicBarrier taskBarrier = new CyclicBarrier(2);
    TestPublisher cache = new TestPublisher();
    final ShardConsumer consumer = createShardConsumer(cache);
    mockSuccessfulInitialize(null);
    ...
    when(shutdownRequestedState.shutdownTransition(eq(ShutdownReason.REQUESTED))).thenReturn(shutdownRequestedAwaitState);
    ...
    when(shutdownRequestedAwaitState.shutdownTransition(eq(ShutdownReason.REQUESTED))).thenReturn(shutdownRequestedState);
  ...
  @Test public void testExceptionInProcessingStopsRequests() throws Exception {
    TestPublisher cache = new TestPublisher();
    ShardConsumer consumer = new ShardConsumer(cache, executorService, shardInfo, Optional.of(1L), shardConsumerArgument, initialState, Function.identity(), 1, taskExecutionListener, 0);
    mockSuccessfulInitialize(null);
    mockSuccessfulProcessing(null);
    CyclicBarrier taskCallBarrier = new CyclicBarrier(2);
  ...
  @Test public void testLongRunningTasks() throws Exception {
    TestPublisher cache = new TestPublisher();
    ShardConsumer consumer = new ShardConsumer(cache, executorService, shardInfo, Optional.of(1L), shardConsumerArgument, initialState, Function.identity(), 1, taskExecutionListener, 0);
  ...
  @Test public void testLeaseEvictedOnShutdownWhenEnabled() throws Exception {
    CyclicBarrier taskBarrier = new CyclicBarrier(2);
    TestPublisher cache = new TestPublisher();
    ShardConsumer consumer = new ShardConsumer(cache, executorService, shardInfo, logWarningForTaskAfterMillis, shardConsumerArgument, initialState, (t) -> t, 1, taskExecutionListener, 0);
    mockSuccessfulInitialize(null);
    mockSuccessfulProcessing(taskBarrier);
  ...
  @Test public void testLeaseNotEvictedOnShutdownByDefault() throws Exception {
    CyclicBarrier taskBarrier = new CyclicBarrier(2);
    TestPublisher cache = new TestPublisher();
    ShardConsumer consumer = new ShardConsumer(cache, executorService, shardInfo, logWarningForTaskAfterMillis, shardConsumerArgument, initialState, (t) -> t, 1, taskExecutionListener, 0);
    mockSuccessfulInitialize(null);
  ...
  private ShardConsumer createShardConsumer(final RecordsPublisher publisher, final ExecutorService executorService, final ConsumerState state) {
    return new ShardConsumer(publisher, executorService, shardInfo, logWarningForTaskAfterMillis, shardConsumerArgument, state, Function.identity(), 1, taskExecutionListener, 0);
  }
}