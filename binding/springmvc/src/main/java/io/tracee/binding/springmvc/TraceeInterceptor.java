...
public final class TraceeInterceptor implements HandlerInterceptor {
  ...
  @Override public boolean preHandle(final HttpServletRequest request, final HttpServletResponse response, final Object o) throws Exception {
    ...
    if (configuration.shouldProcessContext(IncomingRequest)) {
      ...
      @SuppressWarnings(value = { "unchecked" }) final Enumeration<String> headers = request.getHeaders(incomingHeaderName);
  ...
  @Override public void postHandle(HttpServletRequest request, HttpServletResponse response, Object o, ModelAndView modelAndView) throws Exception {
    final TraceeFilterConfiguration configuration = backend.getConfiguration(profileName);
