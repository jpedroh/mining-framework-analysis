...
public final class TraceeServletRequestListener implements ServletRequestListener {
  ...
  @Override public void requestDestroyed(final ServletRequestEvent sre) {
    backend.clear();
  }

  @Override public void requestInitialized(final ServletRequestEvent sre) {
  ...
  private void httpRequestInitialized(final HttpServletRequest request) {
    ...
    if (configuration.shouldProcessContext(IncomingRequest)) {
      ...
      @SuppressWarnings(value = { "unchecked" }) final Enumeration<String> headers = request.getHeaders(HTTP_HEADER_NAME);
