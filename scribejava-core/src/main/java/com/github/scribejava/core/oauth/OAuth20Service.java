...
public class OAuth20Service extends OAuthService {
  ...
  protected Future<OAuth2AccessToken> sendAccessTokenRequestAsync(OAuthRequest request, OAuthAsyncRequestCallback<OAuth2AccessToken> callback) {
    ...
    return execute(request, callback, new OAuthRequest.ResponseConverter<OAuth2AccessToken>() {
      @Override public OAuth2AccessToken convert(Response response) throws IOException {
  ...
  protected OAuthRequest createAccessTokenRequest(AccessTokenRequestParams params) {
    ...
    if (isDebug()) {
      log("created access token request with body params [%s], query string params [%s]", request.getBodyParams().asFormUrlEncodedString(), request.getQueryStringParams().asFormUrlEncodedString());
    }
    return request;
  }
  ...
  protected OAuthRequest createRefreshTokenRequest(String refreshToken, String scope) {
    ...
    if (isDebug()) {
      log("created refresh token request with body params [%s], query string params [%s]", request.getBodyParams().asFormUrlEncodedString(), request.getQueryStringParams().asFormUrlEncodedString());
    }
    return request;
  }
  ...
  public Future<OAuth2AccessToken> getAccessTokenPasswordGrantAsync(String username, String password) {
    return getAccessTokenPasswordGrantAsync(username, password, (OAuthAsyncRequestCallback<OAuth2AccessToken>) null);
  }
  ...
  protected OAuthRequest createAccessTokenPasswordGrantRequest(String username, String password, String scope) {
    ...
    api.getClientAuthentication().addClientAuthentication(request, getApiKey(), getApiSecret());
    if (isDebug()) {
      log("created access token password grant request with body params [%s], query string params [%s]", request.getBodyParams().asFormUrlEncodedString(), request.getQueryStringParams().asFormUrlEncodedString());
    }
    return request;
  }
  ...
  protected OAuthRequest createAccessTokenClientCredentialsGrantRequest(String scope) {
    ...
    if (isDebug()) {
      log("created access token client credentials grant request with body params [%s], query string params [%s]", request.getBodyParams().asFormUrlEncodedString(), request.getQueryStringParams().asFormUrlEncodedString());
    }
    return request;
  }
  ...
  protected OAuthRequest createRevokeTokenRequest(String tokenToRevoke, TokenTypeHint tokenTypeHint) {
    ...
    if (isDebug()) {
      log("created revoke token request with body params [%s], query string params [%s]", request.getBodyParams().asFormUrlEncodedString(), request.getQueryStringParams().asFormUrlEncodedString());
    }
    return request;
  }
  ...
  public Future<Void> revokeToken(String tokenToRevoke, OAuthAsyncRequestCallback<Void> callback, TokenTypeHint tokenTypeHint) {
    ...
    return execute(request, callback, new OAuthRequest.ResponseConverter<Void>() {
      @Override public Void convert(Response response) throws IOException {
        checkForErrorRevokeToken(response);
        response.close();
        return null;
      }
  ...
  public DeviceCode getDeviceCode() throws InterruptedException, ExecutionException, IOException {
    ...
    try (Response response = execute(request)) {
      ...
      if (response.getCode() == 200) {
        ...
        return new DeviceCode(extractRequiredParameter(n, "device_code", body).textValue(), extractRequiredParameter(n, "user_code", body).textValue(), extractRequiredParameter(n, "verification_uri", body).textValue(), n.path("interval").asInt(5), extractRequiredParameter(n, "expires_in", body).intValue());
      } else {
  ...
  /**
     ...
     * @see #getDeviceCode()
     */
  public OAuth2AccessToken getAccessTokenDeviceCodeGrant(DeviceCode deviceCode) throws IOException, InterruptedException, ExecutionException {
  ...
  /**
     ...
     * @see #getDeviceCode()
     */
  public OAuth2AccessToken pollDeviceAccessToken(DeviceCode deviceCode) throws InterruptedException, ExecutionException, IOException {
