...
public class OAuth10aService extends OAuthService<OAuth1AccessToken> {
  ...
  public final Future<OAuth1RequestToken> getRequestTokenAsync(OAuthAsyncRequestCallback<OAuth1RequestToken> callback) {
    ...
    return execute(request, callback, new OAuthRequest.ResponseConverter<OAuth1RequestToken>() {
      @Override public OAuth1RequestToken convert(Response response) throws IOException {
  ...
  public final Future<OAuth1AccessToken> getAccessTokenAsync(OAuth1RequestToken requestToken, String oauthVerifier, OAuthAsyncRequestCallback<OAuth1AccessToken> callback) {
    ...
    return execute(request, callback, new OAuthRequest.ResponseConverter<OAuth1AccessToken>() {
      @Override public OAuth1AccessToken convert(Response response) throws IOException {
  ...
  @Override public void signRequest(OAuth1AccessToken token, OAuthRequest request) {
    final OAuthConfig config = getConfig();
  ...
  protected void appendSignature(OAuthRequest request) {
    final OAuthConfig config = getConfig();
    @SuppressWarnings(value = { "deprecation" }) final OAuth1SignatureType signatureType = config.getSignatureType() == null ? api.getSignatureType() : config.getSignatureType();
