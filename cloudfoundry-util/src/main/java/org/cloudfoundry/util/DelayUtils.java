...
public final class DelayUtils {
  ...
  public static Function<Flux<Long>, Publisher<?>> fixed(Duration duration) {
    return (iterations) -> iterations.flatMap((iteration) -> Mono.delay(duration).doOnSubscribe(logDelay(duration)), 1);
  }
  ...
  public static Function<Flux<Long>, Publisher<?>> instant() {
    return (iterations) -> iterations.flatMap((iteration) -> Mono.just(0L).doOnSubscribe(logDelay(Duration.ZERO)), 1);
  }
  ...
  private static Flux<?> getDelay(Duration minimum, Duration maximum, Instant finish, Flux<Long> iterations) {
    return iterations.map((iteration) -> calculateDuration(minimum, maximum, iteration)).concatMap((delay) -> {
