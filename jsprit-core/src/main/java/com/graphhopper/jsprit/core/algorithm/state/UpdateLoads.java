...
class UpdateLoads implements ActivityVisitor, StateUpdater, InsertionStartsListener, JobInsertedListener {
  ...
  @Override public void informJobInserted(Job job2insert, VehicleRoute inRoute, double additionalCosts, double additionalTime) {
    if (job2insert instanceof Delivery) {
      ...
      stateManager.putTypedInternalRouteState(inRoute, InternalStates.LOAD_AT_BEGINNING, loadAtDepot.add(job2insert.getSize()));
    } else {
      if (job2insert instanceof Pickup || job2insert instanceof Service) {
        ...
        stateManager.putTypedInternalRouteState(inRoute, InternalStates.LOAD_AT_END, loadAtEnd.add(job2insert.getSize()));
      }
    }
  }
