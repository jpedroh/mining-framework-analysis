...
public final class ReturnedShipment extends Shipment {
  ...
  public static final class Builder extends Shipment.BuilderBase<ReturnedShipment, Builder> {
    ...
    @Override protected ReturnedShipment createInstance() {
  ...
  @Override protected void createActivities(JobBuilder<?, ?> builder) {
    ...
    list.addActivity(new PickupActivityNEW(this, ACTIVITY_NAME_PICKUP, shipmentBuilder.getPickupLocation(), shipmentBuilder.getPickupServiceTime(), shipmentBuilder.getCapacity(), shipmentBuilder.getPickupTimeWindows().getTimeWindows()));
    list.addActivity(new ExchangeActivityNEW(this, ACTIVITY_NAME_DELIVERY, shipmentBuilder.getDeliveryLocation(), shipmentBuilder.getDeliveryServiceTime(), shipmentBuilder.getBackhaulCapacity().subtract(shipmentBuilder.getCapacity()), shipmentBuilder.getDeliveryTimeWindows().getTimeWindows()));
    list.addActivity(new DeliveryActivityNEW(this, ACTIVITY_NAME_BACKHAUL, shipmentBuilder.getBackhaulLocation(), shipmentBuilder.getBackhaulServiceTime(), shipmentBuilder.getBackhaulCapacity(), shipmentBuilder.getBackhaulTimeWindows().getTimeWindows()));
    setActivities(list);
  }

  public PickupActivityNEW getPickupActivityTO_BE_RENAMED_LATER() {
    return (PickupActivityNEW) getActivityList().findByType(ACTIVITY_NAME_PICKUP).get();
  }

  public ExchangeActivityNEW getDeliveryActivityTO_BE_RENAMED_LATER() {
    return (ExchangeActivityNEW) getActivityList().findByType(ACTIVITY_NAME_DELIVERY).get();
  }

  public DeliveryActivityNEW getBackhaulActivityTO_BE_RENAMED_LATER() {
    return (DeliveryActivityNEW) getActivityList().findByType(ACTIVITY_NAME_BACKHAUL).get();
  }
}