...
class VehicleFleetManagerImpl implements VehicleFleetManager {
  ...
  @Override public Collection<Vehicle> getAvailableVehicles(Vehicle withoutThisType) {
    List<Vehicle> vehicles = new ArrayList<Vehicle>();
  ...
  @Override public void lock(Vehicle vehicle) {
  ...
  @Override public void unlock(Vehicle vehicle) {
    if (vehicles.isEmpty() || vehicle instanceof NoVehicle) {
      return;
    }
    if (vehicle == null) {
  ...
  @Override public boolean isLocked(Vehicle vehicle) {
  ...
  @Deprecated public int sizeOfLockedVehicles() {
