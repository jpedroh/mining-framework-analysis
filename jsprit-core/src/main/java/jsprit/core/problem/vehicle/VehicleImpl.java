  ...
  public static class Builder {
    ...
    public Builder setStartLocationId(String startLocationId) {
      if (startLocationId == null) {
        throw new IllegalArgumentException("startLocationId cannot be null");
      }
    ...
    public VehicleImpl build() {
      ...
      if (locationId == null && locationCoord == null) {
        throw new IllegalStateException("locationId and locationCoord is missing.");
      }
      if (locationCoord == null) {
        log.warn("locationCoord for vehicle " + id + " is missing.");
      }
      if (endLocationId == null && endLocationCoord != null) {
        endLocationId = endLocationCoord.toString();
      }
      if (endLocationId == null && endLocationCoord == null) {
        endLocationId = startLocationId;
        endLocationCoord = startLocationCoord;
      }
      if (!startLocationId.equals(endLocationId) && returnToDepot == false) {
        throw new IllegalStateException("this must not be. you specified both endLocationId and open-routes. this is contradictory. <br>" + "if you set endLocation, returnToDepot must be true. if returnToDepot is false, endLocationCoord must not be specified.");
      }
...
>>>>>>> /usr/src/app/output/jsprit/jsprit/9c4bd498c41594d2634c177e7309e0e365c58859/jsprit-core/src/main/java/jsprit/core/problem/vehicle/VehicleImpl.java/right.java
  ...
  @Override public String toString() {
    return "[id=" + id + "][type=" + type + "][startLocationId=" + startLocationId + "][startLocationCoord=" + startLocationCoord + "]" + "[endLocationId=" + endLocationId + "][endLocationCoord=" + endLocationCoord + "]" + "[isReturnToDepot=" + isReturnToDepot() + "]";
  }

  @Override public double getEarliestDeparture() {
    return earliestDeparture;
  }

  @Override public double getLatestArrival() {
  ...
  @Override public String getStartLocationId() {
    return this.startLocationId;
  }

  @Override public Coordinate getStartLocationCoordinate() {
    return this.startLocationCoord;
  }

  @Override public String getEndLocationId() {
    return this.endLocationId;
  }

  @Override public Coordinate getEndLocationCoordinate() {
  ...
  @Override public Skills getSkills() {
    return skills;
  }
}