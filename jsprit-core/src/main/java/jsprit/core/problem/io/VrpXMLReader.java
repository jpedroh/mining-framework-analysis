...
public class VrpXMLReader {
  ...
  static class DefaultServiceBuilderFactory implements ServiceBuilderFactory {
    @Override public jsprit.core.problem.job.Service.Builder createBuilder(String serviceType, String id, Integer size) {
      if (serviceType.equals("pickup")) {
        if (size != null) {
          return Pickup.Builder.newInstance(id).addSizeDimension(0, size);
        } else {
          return Pickup.Builder.newInstance(id);
        }
      } else {
        if (serviceType.equals("delivery")) {
          if (size != null) {
            return Delivery.Builder.newInstance(id).addSizeDimension(0, size);
          } else {
            return Delivery.Builder.newInstance(id);
          }
        } else {
          if (size != null) {
            return Service.Builder.newInstance(id).addSizeDimension(0, size);
          } else {
  ...
  @Deprecated interface JobConfigReader {
  ...
  @Deprecated public void addJobConfigReader(JobConfigReader reader) {
    jobConfigReaders.add(reader);
  }

  @Deprecated public void setTourActivityFactory(TourActivityFactory tourActivityFactory) {
  }

  @Deprecated public void setServiceBuilderFactory(ServiceBuilderFactory serviceBuilderFactory) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "UnusedDeclaration" }) public void setSchemaValidation(boolean schemaValidation) {
  ...
  private void readProblemType(XMLConfiguration vrpProblem) {
    ...
    if (fleetSize == null) {
      ...
      if (fleetSize.toUpperCase().equals(FleetSize.INFINITE.toString())) {
        vrpBuilder.setFleetSize(FleetSize.INFINITE);
      } else {
