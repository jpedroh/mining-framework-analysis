...
@SuppressWarnings(value = { "deprecation" }) public class VrpXMLReader {
  ...
  static class DefaultServiceBuilderFactory implements ServiceBuilderFactory {
    @Override public jsprit.core.problem.job.Service.Builder createBuilder(String serviceType, String id, int size) {
  ...
  static class ServiceConfigReader implements JobConfigReader {
    ...
    @Override public void readConfig(XMLConfiguration config) {
    }
  }
    ...
    for (HierarchicalConfiguration solutionConfig : solutionConfigs) {
      ...
      for (HierarchicalConfiguration routeConfig : routeConfigs) {
        ...
        for (HierarchicalConfiguration actConfig : actConfigs) {
          ...
          if (arrTimeS == null) {
            throw new IllegalStateException("act.arrTime is missing.");
          }
          String endTimeS = actConfig.getString("endTime");
          if (endTimeS == null) {
            throw new IllegalStateException("act.endTime is missing.");
          }
  ...
  private void readProblemType(XMLConfiguration vrpProblem) {
    ...
    if (fleetComposition == null) {
      vrpBuilder.setFleetComposition(FleetComposition.HOMOGENEOUS);
    } else {
      if (fleetComposition.toUpperCase().equals(FleetComposition.HETEROGENEOUS.toString())) {
  ...
  private static int getCap(HierarchicalConfiguration serviceConfig) {
    ...
    int cap = 0;
    if (capacityDemand != null) {
      cap = Integer.parseInt(capacityDemand);
    }
    return cap;
  }
  ...
  private void readVehiclesAndTheirTypes(XMLConfiguration vrpProblem) {
    ...
    for (HierarchicalConfiguration typeConfig : typeConfigs) {
      ...
      if (capacity == null) {
        throw new IllegalStateException("capacity is missing.");
      }
    ...
    for (HierarchicalConfiguration vehicleConfig : vehicleConfigs) {
      ...
      if (locationId == null) {
        throw new IllegalStateException("location.id is missing.");
      }
      ...
      if (endLocationId != null) {
        builder.setEndLocationId(endLocationId);
      }
