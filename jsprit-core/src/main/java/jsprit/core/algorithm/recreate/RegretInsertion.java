...
public class RegretInsertion extends AbstractInsertionStrategy {
  ...
  public static class DefaultScorer implements ScoringFunction {
    ...
    @Override public double score(InsertionData best, Job job) {
      double score;
    ...
    private double scoreShipment(InsertionData best, Job job) {
      ...
      double maxDepotDistance_1 = Math.max(getDistance(best.getSelectedVehicle().getStartLocation(), shipment.getPickupLocation()), getDistance(best.getSelectedVehicle().getStartLocation(), shipment.getDeliveryLocation()));
    ...
    private double scoreService(InsertionData best, Job job) {
      ...
      if (location != null) {
        maxDepotDistance = Math.max(getDistance(best.getSelectedVehicle().getStartLocation(), location), getDistance(best.getSelectedVehicle().getEndLocation(), location));
      }
      return Math.max(tw_param * (((Service) job).getTimeWindow().getEnd() - ((Service) job).getTimeWindow().getStart()), minTimeWindowScore) + depotDistance_param * maxDepotDistance;
    }
    ...
    for (VehicleRoute route : routes) {
      ...
      if (best == null) {
        ...
        if (iData.getInsertionCost() < best.getInsertionCost()) {
          ...
          if (secondBest == null || (iData.getInsertionCost() < secondBest.getInsertionCost())) {
            secondBest = iData;
          }
        }
      }
