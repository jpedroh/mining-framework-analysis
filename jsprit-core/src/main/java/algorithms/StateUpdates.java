...
class StateUpdates {
  ...
  static class UpdateCostsAtAllLevels implements ActivityVisitor {
    ...
    @Override public void visit(TourActivity act) {
  ...
  static class UpdateLoadsAtStartAndEndOfRouteWhenInsertionStarts implements InsertionStarts {
    ...
    @Override public void insertionStarts(VehicleRoute route) {
      int loadAtDepot = 0;
      int loadAtEnd = 0;
      for (Job j : route.getTourActivities().getJobs()) {
        if (j instanceof Delivery) {
          ...
          if (j instanceof Pickup || j instanceof Service) {
  ...
  static class UpdateLoadsAtStartAndEndOfRouteWhenJobHasBeenInserted implements JobInsertedListener {
    ...
    @Override public void informJobInserted(Job job2insert, VehicleRoute inRoute, double additionalCosts, double additionalTime) {
      if (job2insert instanceof Delivery) {
        ...
        if (job2insert instanceof Pickup || job2insert instanceof Service) {
  ...
  static class UpdateRouteStatesOnceTheRouteHasBeenChanged implements InsertionStartsListener, JobInsertedListener {
    ...
    @Override public void informJobInserted(Job job2insert, VehicleRoute inRoute, double additionalCosts, double additionalTime) {
      for (JobInsertedListener l : jobInsertionListeners) {
        l.informJobInserted(job2insert, inRoute, additionalCosts, additionalTime);
      }
