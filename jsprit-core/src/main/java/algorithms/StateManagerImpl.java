...
public class StateManagerImpl implements StateManager, IterationStartsListener, RuinListener, InsertionStartsListener, JobInsertedListener, InsertionEndsListener {
  ...
  private static class StatesImpl implements States {
    ...
    @Override public State getState(StateId key) {
  ...
  @Override public State getActivityState(TourActivity act, StateId stateId) {
  ...
  private State getDefaultActState(StateId stateId, TourActivity act) {
    if (stateId.equals(StateIdFactory.LOAD)) {
      return new StateImpl(0);
    }
    if (stateId.equals(StateIdFactory.COSTS)) {
      return new StateImpl(0);
    }
    if (stateId.equals(StateIdFactory.DURATION)) {
      return new StateImpl(0);
    }
    if (stateId.equals(StateIdFactory.EARLIEST_OPERATION_START_TIME)) {
      return new StateImpl(act.getTheoreticalEarliestOperationStartTime());
    }
    if (stateId.equals(StateIdFactory.LATEST_OPERATION_START_TIME)) {
      return new StateImpl(act.getTheoreticalLatestOperationStartTime());
    }
    if (stateId.equals(StateIdFactory.FUTURE_PICKS)) {
      return new StateImpl(0);
    }
    if (stateId.equals(StateIdFactory.PAST_DELIVERIES)) {
      return new StateImpl(0);
    }
    return null;
  }
  ...
  private State getDefaultRouteState(StateId stateId, VehicleRoute route) {
    ...
    if (stateId.equals(StateIdFactory.LOAD_AT_BEGINNING)) {
      return new StateImpl(0);
    }
    if (stateId.equals(StateIdFactory.COSTS)) {
      return new StateImpl(0);
  ...
  @Override public State getRouteState(VehicleRoute route, StateId stateId) {
