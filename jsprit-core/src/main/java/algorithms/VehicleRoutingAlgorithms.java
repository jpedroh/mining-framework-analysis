...
public class VehicleRoutingAlgorithms {
  ...
  @Deprecated public static VehicleRoutingAlgorithm readAndCreateAlgorithm(final VehicleRoutingProblem vrp, final XMLConfiguration config) {
  ...
  private static SearchStrategyModule buildModule(HierarchicalConfiguration moduleConfig, final VehicleRoutingProblem vrp, VehicleFleetManager vehicleFleetManager, final StateManagerImpl routeStates, Set<PrioritizedVRAListener> algorithmListeners, TypedMap definedClasses, ExecutorService executorService, int nuOfThreads, ConstraintManager constraintManager) {
    ...
    if (moduleName.equals("ruin_and_recreate")) {
      ...
      if (ruin_name.equals("randomRuin")) {
        ...
        if (ruin_name.equals("radialRuin")) {
          ...
          if (ruin_distance == null) {
            jobDistance = new JobDistanceAvgCosts(vrp.getTransportCosts());
          } else {
            if (ruin_distance.equals("euclidean")) {
              jobDistance = new EuclideanServiceDistance();
            } else {
              throw new IllegalStateException("does not know ruin.distance " + ruin_distance + ". either ommit ruin.distance then the " + "default is used or use \'euclidean\'");
            }
          }
    ...
    if (moduleName.equals("gendreau")) {
      ...
      if (ruinName == null) {
        throw new IllegalStateException("gendreau.ruin[@name] is missing. set it to \"radialRuin\" or \"randomRuin\"");
      }
      ...
      String insertionName = moduleConfig.getString("insertion[@name]");
      if (insertionName == null) {
        throw new IllegalStateException("gendreau.insertion[@name] is missing. set it to \"regretInsertion\" or \"bestInsertion\"");
      }
      String insertionId = moduleConfig.getString("insertion[@id]");
      if (insertionId == null) {
        insertionId = "noId";
