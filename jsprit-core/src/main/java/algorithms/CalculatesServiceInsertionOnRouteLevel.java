...
final class CalculatesServiceInsertionOnRouteLevel implements JobInsertionCalculator {
  ...
  private Neighborhood neighborhood = new Neighborhood() {
    @Override public boolean areNeighbors(String location1, String location2) {
      return true;
    }
  };
...
>>>>>>> /usr/src/app/output/jsprit/jsprit/c60ab8afd055cab1b8c7826b493fcae8b619e9c4/jsprit-core/src/main/java/algorithms/CalculatesServiceInsertionOnRouteLevel.java/right.java
  ...
  /**
	 ...
	 * 
	 */
  @Override public InsertionData calculate(final VehicleRoute currentRoute, final Job jobToInsert, final Vehicle newVehicle, double newVehicleDepartureTime, final Driver newDriver, final double best_known_insertion_costs) {
    if (jobToInsert == null) {
    ...
    for (TourActivity nextAct : tour.getActivities()) {
      if (neighborhood.areNeighbors(serviceAct2Insert.getLocationId(), prevAct.getLocationId()) && neighborhood.areNeighbors(serviceAct2Insert.getLocationId(), nextAct.getLocationId())) {
        ...
        path.add(prevAct);
        path.add(serviceAct2Insert);
        path.add(nextAct);
        if (nuOfActsForwardLooking > 0) {
          path.addAll(getForwardLookingPath(currentRoute, actIndex));
        }
  ...
  private Comparator<InsertionData> getComparator() {
    return new Comparator<InsertionData>() {
      @Override public int compare(InsertionData o1, InsertionData o2) {
