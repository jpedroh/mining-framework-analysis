...
public class BreakActivityTest {
  ...
  @Before public void doBefore() {
    Builder breakBuilder = new Break.Builder("service").setTimeWindow(TimeWindow.newInstance(1., 2.)).setServiceTime(3);
    service = breakBuilder.build();
    serviceActivity = BreakActivity.newInstance(service, breakBuilder);
    serviceActivity.setTheoreticalEarliestOperationStartTime(service.getActivity().getBreakTimeWindow().getStart());
    serviceActivity.setTheoreticalLatestOperationStartTime(service.getActivity().getBreakTimeWindow().getEnd());
  }
  ...
  @Test public void hasVariableLocationShouldBeTrue() {
  ...
  @Test public void whenTwoDeliveriesHaveTheSameUnderlyingJob_theyAreEqual() {
    ...
    ServiceActivity d1 = new ServiceActivity(s1, "s1", loc, 0d, SizeDimension.EMPTY, TimeWindows.ANY_TIME.getTimeWindows());
    ServiceActivity d2 = new ServiceActivity(s2, "s2", loc, 0d, SizeDimension.EMPTY, TimeWindows.ANY_TIME.getTimeWindows());
