...
public class VehicleDependentTraveledDistanceTest {
  ...
  @Before public void doBefore() {
    ...
    vrp = VehicleRoutingProblem.Builder.newInstance().setRoutingCost(new DefaultCosts(ManhattanDistanceCalculator.getInstance())).addVehicle(vehicle).addVehicle(vehicle2).addJob(d1).addJob(d2).addJob(s1).addJob(pickup).addJob(newDelivery).build();
    ...
    com.graphhopper.jsprit.core.algorithm.state.VehicleDependentTraveledDistance traveledDistance = new com.graphhopper.jsprit.core.algorithm.state.VehicleDependentTraveledDistance(new TransportDistance() {
      @Override public double getDistance(Location from, Location to, double departureTime, Vehicle vehicle) {
        return new DefaultCosts(ManhattanDistanceCalculator.getInstance()).getDistance(from, to, departureTime, vehicle);
      }
  ...
  @Test public void whenEndLocationIsSet_constraintShouldWork() {
    ...
    MaxDistanceConstraint maxDistanceConstraint = new MaxDistanceConstraint(new StateManager(vrp), traveledDistanceId, new TransportDistance() {
      @Override public double getDistance(Location from, Location to, double departureTime, Vehicle vehicle) {
    ...
    Assert.assertTrue(maxDistanceConstraint.fulfilled(context, new Start(vehicle.getStartLocation(), 0, Double.MAX_VALUE), vrp.getActivities(pickup).get(0), new End(vehicle.getEndLocation(), 0, Double.MAX_VALUE), 0).equals(HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED));
  }

  @Test public void insertNewInVehicleShouldFail() {
    MaxDistanceConstraint maxDistanceConstraint = new MaxDistanceConstraint(stateManager, traveledDistanceId, new TransportDistance() {
      @Override public double getDistance(Location from, Location to, double departureTime, Vehicle vehicle) {
        return vrp.getTransportCosts().getTransportTime(from, to, departureTime, null, vehicle);
      }
    }, maxDistanceMap);
  ...
  @Test public void distanceOfShipmentInRoute() {
  ...
  @Test public void distanceOfPickupInRoute() {
  ...
  @Test public void distanceOfPickupInRouteVehicle2() {
  ...
  @Test public void distanceToTravelShouldBeCorrect() {
    double total = stateManager.getRouteState(route, vehicle, traveledDistanceId, Double.class);
  ...
  @Test public void distanceToTravelShouldBeCorrectVehicle2() {
    double total = stateManager.getRouteState(route, vehicle2, traveledDistanceId, Double.class);
  ...
  @Test public void whenAddingDeliverShipment_constraintShouldWork() {
    ...
    Assert.assertTrue(maxDistanceConstraint.fulfilled(context, new Start(vehicle.getStartLocation(), 0, Double.MAX_VALUE), vrp.getActivities(shipment).get(0), new End(vehicle.getEndLocation(), 0, Double.MAX_VALUE), 0).equals(HardActivityConstraint.ConstraintsStatus.FULFILLED));
    ...
    Assert.assertTrue(maxDistanceConstraint.fulfilled(context, vrp.getActivities(shipment).get(0), vrp.getActivities(shipment).get(1), new End(vehicle.getEndLocation(), 0, Double.MAX_VALUE), 3).equals(HardActivityConstraint.ConstraintsStatus.FULFILLED));
  }
  ...
  @Test public void whenAddingDeliverShipmentWithVehDiffStartEndLocs_constraintShouldWork() {
    ...
    pickupContext.setInsertionIndex(0);
    context.setRelatedActivityContext(pickupContext);
    Assert.assertTrue(maxDistanceConstraint.fulfilled(context, vrp.getActivities(shipment).get(0), vrp.getActivities(shipment).get(1), new End(vehicle.getEndLocation(), 0, Double.MAX_VALUE), 1).equals(HardActivityConstraint.ConstraintsStatus.FULFILLED));
  }
}