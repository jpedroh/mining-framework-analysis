...
public class AnsiOutputStream extends FilterOutputStream {
  ...
  @Deprecated public static final byte[] REST_CODE = RESET_CODE;
  ...
  @Override public synchronized void write(int data) throws IOException {
    switch (state) {
      ...
      if (data == SECOND_ESC_CHAR) {
        ...
        if (data == SECOND_OSC_CHAR) {
          state = LOOKING_FOR_OSC_COMMAND;
        } else {
          if (data == SECOND_CHARSET0_CHAR) {
            options.add(new Integer('0'));
            state = LOOKING_FOR_CHARSET;
          } else {
            if (data == SECOND_CHARSET1_CHAR) {
  ...
  @Deprecated protected static final int ATTRIBUTE_NEGATIVE_Off = 27;
