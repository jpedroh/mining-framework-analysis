...
@Path(value = AtupApi.TEST_JOB_PATH) public class AtupTestJobResource {
  ...
  @Autowired private JobLaunchService jobLaunchService;

  public AtupTestJobResource() {
  }

  @POST @Consumes(value = MediaType.APPLICATION_JSON) public void createJob(@Context final HttpHeaders headers, final AtupTestJobInfo jobInfo) {
  ...
  @GET @Produces(value = MediaType.APPLICATION_JSON) public AtupTestJobListInfo retrieveRunningJobs() {
    return jobLaunchService.getJobs();
  }

  @POST @Path(value = "launch") public void launchJob() throws ExecutionException, InterruptedException {
