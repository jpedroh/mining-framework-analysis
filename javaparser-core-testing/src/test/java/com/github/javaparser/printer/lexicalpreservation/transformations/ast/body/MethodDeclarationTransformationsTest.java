...
class MethodDeclarationTransformationsTest extends AbstractLexicalPreservingTest {
  ...
  @Test void settingName() {
    MethodDeclaration it = consider("void A(){}");
    it.setName("B");
    assertTransformedToString("void B(){}", it);
  }

  @Disabled @Test void removingDuplicateJavaDocComment() {
  ...
  @Disabled @Test void replacingDuplicateJavaDocComment() {
    considerCode("public class MyClass {" + EOL + EOL + "  /**" + EOL + "   * Comment A" + EOL + "   */" + EOL + "  public void oneMethod() {" + EOL + "  }" + EOL + EOL + "  /**" + EOL + "   * Comment A" + EOL + "   */" + EOL + "  public void anotherMethod() {" + EOL + "  }" + EOL + "}" + EOL);
    MethodDeclaration methodDeclaration = cu.findAll(MethodDeclaration.class).get(1);
  ...
  @Disabled @Test void removingDuplicateComment() {
    considerCode("public class MyClass {" + EOL + EOL + "  /*" + EOL + "   * Comment A" + EOL + "   */" + EOL + "  public void oneMethod() {" + EOL + "  }" + EOL + EOL + "  /*" + EOL + "   * Comment A" + EOL + "   */" + EOL + "  public void anotherMethod() {" + EOL + "  }" + EOL + "}" + EOL);
    MethodDeclaration methodDeclaration = cu.findAll(MethodDeclaration.class).get(1);
    methodDeclaration.removeComment();
    String result = LexicalPreservingPrinter.print(cu.findCompilationUnit().get());
  ...
  @Test void removingModifiersWithExistingAnnotations() {
  ...
  @Test void removingOnlyParameter() {
  ...
  @Test void addingToExistingAnnotations() {
    considerCode("class X {" + EOL + "  @Test" + EOL + "  public void testCase() {" + EOL + "  }" + EOL + "}" + EOL);
  ...
  @Test void addingAnnotationsNoModifiers() {
  ...
  @Test void replacingAnnotations() {
  ...
  @Test void addingAnnotationsShort() {
  ...
  @Disabled @Test void removingAnnotationsWithSpaces() {
    considerCode("class X {" + EOL + "  @Override " + EOL + "  public void testCase() {" + EOL + "  }" + EOL + "}" + EOL);
    cu.getType(0).getMethods().get(0).getAnnotationByName("Override").get().remove();
    String result = LexicalPreservingPrinter.print(cu.findCompilationUnit().get());
    assertEqualsNoEol("class X {\n" + "  public void testCase() {\n" + "  }\n" + "}\n", result);
  ...
  @Test public void parseAndPrintAnonymousClassExpression() {
  ...
  @Test public void parseAndPrintAnonymousClassStatement() {
  ...
  @Test public void replaceBodyShouldNotBreakAnonymousClasses() {
