...
abstract public class KryoTestCase {
  ...
  public <T extends java.lang.Object> T roundTrip(int lengthNonGeneric, int lengthGeneric, T object1) {
    roundTrip(lengthNonGeneric, object1, false);
    return roundTrip(lengthGeneric, object1, true);
  }

  /** @param length Pass Integer.MIN_VALUE to disable checking the length. */
  public <T extends java.lang.Object> T roundTrip(int length, T object1, boolean useGenerics) {
    try {
  ...
  public <T extends java.lang.Object> T roundTrip(int length, T object1) {
    T object2 = roundTripWithBufferFactory(length, object1, new BufferFactory() {
      @Override public Output createOutput(OutputStream os) {
        return new Output(os);
      }

      @Override public Output createOutput(OutputStream os, int size) {
        return new Output(os, size);
      }

      @Override public Output createOutput(int size, int limit) {
        return new Output(size, limit);
      }

      @Override public Input createInput(InputStream os, int size) {
        return new Input(os, size);
      }

      @Override public Input createInput(byte[] buffer) {
  ...
  public <T extends java.lang.Object> T roundTripWithBufferFactory(int lengthNonGeneric, int lengthGeneric, T object1, BufferFactory sf) {
  ...
  public <T extends java.lang.Object> T roundTripWithBufferFactory(int length, T object1, BufferFactory sf, boolean useGenerics) {
    kryo.setOptimizedGenerics(useGenerics);
    try {
