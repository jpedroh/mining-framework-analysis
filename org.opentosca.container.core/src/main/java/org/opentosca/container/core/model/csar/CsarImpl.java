...
public class CsarImpl implements Csar {
  ...
  @NonNull private final CsarId id;
  ...
  private Optional<ServiceTemplateId> readEntryServiceTemplate(Path csarLocation) {
    ...
    return qname == null ? Optional.empty() : Optional.of(new ServiceTemplateId(QName.valueOf(qname)));
  }

  @Override public CsarId id() {
    return id;
  }

  @Override public List<TArtifactTemplate> artifactTemplates() {
  ...
  @Override public @NonNull Map<QName, TArtifactType> artifactTypesMap() {
    return this.artifactTypes;
  }

  @Override public @NonNull List<TServiceTemplate> serviceTemplates() {
    return wineryRepo.getAllDefinitionsChildIds(ServiceTemplateId.class).stream().map(wineryRepo::getElement).collect(Collectors.toList());
  }

  @Override public @NonNull List<TPolicyTemplate> policyTemplates() {
    return new ArrayList<>(this.policyTemplates.values());
  }

  @Override public TServiceTemplate entryServiceTemplate() {
  ...
  @Override public TDefinitions entryDefinitions() {
    return this.definitions.get(this.entryServiceTemplate.get().getQName());
  }

  @Override public @NonNull List<TDefinitions> definitions() {
    return Lists.newArrayList(this.definitions.values());
  }

  @Override public @NonNull List<TExportedOperation> exportedOperations() {
    return serviceTemplates().stream().map(TServiceTemplate::getBoundaryDefinitions).filter(Objects::nonNull).map(TBoundaryDefinitions::getInterfaces).flatMap(Collection::stream).map(TExportedInterface::getOperation).flatMap(Collection::stream).collect(Collectors.toList());
  }

  @Override public @NonNull List<TPlan> plans() {
    return Optional.ofNullable(entryServiceTemplate()).map(TServiceTemplate::getPlans).orElse(Collections.emptyList());
  }

  @Override @Nullable public Application selfserviceMetadata() {
  ...
  @Override public @NonNull List<TNodeType> nodeTypes() {
    return new ArrayList<>(this.nodeTypes.values());
  }

  @Override public @NonNull Map<QName, TNodeType> nodeTypesMap() {
    return this.nodeTypes;
  }

  @Override public @NonNull List<TNodeTypeImplementation> nodeTypeImplementations() {
    return new ArrayList<>(this.nodeTypeImplementations.values());
  }

  @Override public @NonNull Map<QName, TNodeTypeImplementation> nodeTypeImplementationsMap() {
    return this.nodeTypeImplementations;
  }

  @Override public @NonNull List<TRelationshipType> relationshipTypes() {
    return new ArrayList<>(this.relationshipTypes.values());
  }

  @Override public @NonNull List<TRelationshipTypeImplementation> relationshipTypeImplementations() {
    return new ArrayList<>(this.relationshipTypeImplementations.values());
  }

  @Override public TExtensibleElements queryRepository(@NonNull DefinitionsChildId id) {
    return wineryRepo.getElement(id);
  }

  @Override public @NonNull String description() {
    Application metadata = selfserviceMetadata();
    return metadata == null ? "" : metadata.getDescription();
  }

  @Override public void exportTo(@NonNull Path targetPath) throws IOException {
  ...
  @Override public @NonNull Path getSaveLocation() {
