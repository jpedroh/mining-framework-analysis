...
public class ODEEndpointUpdater {
  ...
  public boolean changeEndpoints(final List<File> processFiles, final CsarId csarId) {
    ...
    for (final QName portType : unchangedFiles.keySet()) {
      ...
      for (final File file : unchangedFiles.get(portType)) {
        LOG.warn("WSDL file {} which contained portType {} and couldn\'t be updated", file.toPath(), portType);
      }
    }
    return true;
  }
  ...
  private List<QName> getInvokedDeployXMLPorts(final File deployXML) throws JAXBException {
    ...
    final JAXBContext context = JAXBContext.newInstance("org.apache.ode.schemas.dd._2007._03", this.getClass().getClassLoader());
  ...
  /**
     ...
     */
  private Map<QName, List<File>> getWSDLtoChange(final List<QName> ports, final List<File> wsdlFiles) throws WSDLException {
    ...
    for (final QName port : ports) {
      ...
      for (final File wsdlFile : wsdlFiles) {
        LOG.debug("Checking if wsdl file {} contains portType {}", wsdlFile.getAbsolutePath(), port);
  ...
  private boolean checkIfPortIsInWsdlDef(final QName port, final Definition wsdlDef) {
    for (final Object serviceObj : wsdlDef.getServices().values()) {
      ...
      for (final Object portObj : service.getPorts().values()) {
        ...
        LOG.debug("Checking if port {} matches port with name {} and namespace {} ", port.toString(), name, namespace);
    ...
    for (final QName portType : map.keySet()) {
      ...
      for (final File wsdlFile : map.get(portType)) {
        if (!this.updateInvokedWSDLAddresses(portType, wsdlFile)) {
          LOG.error("Unable to update \'{}\' for porttype \'{}\'.", wsdlFile, portType.toString());
  ...
  private boolean changePortAddressWithEndpointDB(final Port port) {
    boolean changed = false;
    LOG.debug("Trying to match address element with available endpoints for port {} ", port.getName());
    for (final Object obj : port.getExtensibilityElements()) {
      final ExtensibilityElement element = (ExtensibilityElement) obj;
  ...
  private List<Endpoint> getEndpointsFromEndpointDB(final Port port) {
    ...
    if (endpointService != null) {
      LOG.debug("Fetching Endpoints for PortType {} ", port.getBinding().getPortType().getQName().toString());
      final List<Endpoint> temp = endpointService.getEndpoints();
      for (final Endpoint endpoint : temp) {
        if (endpoint.getPortType() != null && endpoint.getPortType().equals(port.getBinding().getPortType().getQName()) && endpoint.getManagingContainer().equals(Settings.OPENTOSCA_CONTAINER_HOSTNAME)) {
  ...
  private boolean changeAddress(final ExtensibilityElement element, final Endpoint endpoint) {
    if (element.getElementType().equals(SOAPConstants.Q_ELEM_SOAP_ADDRESS)) {
      LOG.debug("Changing the SOAP-Address Element inside for porttype {} ", endpoint.getPortType().toString());
      final SOAPAddress address = (SOAPAddress) element;
      address.setLocationURI(endpoint.getUri().toString());
    } else {
      if (element.getElementType().equals(HTTPConstants.Q_ELEM_HTTP_ADDRESS)) {
        LOG.debug("Changing the HTTP-Address Element inside for porttype {} ", endpoint.getPortType().toString());
