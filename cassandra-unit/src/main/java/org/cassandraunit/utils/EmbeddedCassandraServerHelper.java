...
public class EmbeddedCassandraServerHelper {
  ...
  private static void dropKeyspacesWithNativeDriver() {
    ...
    try (com.datastax.driver.core.Cluster cluster = com.datastax.driver.core.Cluster.builder().addContactPoint(host).withPort(port).build(); com.datastax.driver.core.Session session = cluster.connect()) {
  ...
  private static boolean isSystemKeyspaceName(String keyspaceName) {
    return INTERNAL_CASSANDRA_KEYSPACE.equals(keyspaceName) || INTERNAL_CASSANDRA_AUTH_KEYSPACE.equals(keyspaceName) || INTERNAL_CASSANDRA_TRACES_KEYSPACE.equals(keyspaceName);
  }
  ...
  private static void copy(String resource, String directory) throws IOException {
    ...
    try (InputStream is = EmbeddedCassandraServerHelper.class.getResourceAsStream(resource); OutputStream out = new FileOutputStream(file)) {
