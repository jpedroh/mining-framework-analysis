...
public class CsvExtractorFactoryTest extends BaseExtractorFactoryTest<StreamConnector, CsvExtractorFactory> {
  @Override protected CsvExtractorFactory createExtractorFactory() {
    return new CsvExtractorFactory();
  }

  @Override protected Class<StreamConnector> getConnectorType() {
    return StreamConnector.class;
  }

  @Override public void setUpExtractorModel() {
    super.setUpExtractorModel();
    getModel().setAttributes(new BaseRowAttribute(String.class, "k1", "k1", 0), new BaseRowAttribute(String.class, "k2", "k2", 1), new BaseRowAttribute(String.class, "k3", "k3", 2));
  }
  ...
  @Test public void testBasicIntegration() throws IOException {
    when(getConnectorMock().getInputStream(anyMap())).thenReturn(new ByteArrayInputStream("r1c1,r1c2,r1c3\nr2c1,r2c2,r2c3".getBytes()));
    RowReader reader = getExtractorFactory().createExtractor(getConnectorMock(), getModel()).getReader(null);
    doCheck(reader);
  }

  @Test public void testSetDelimiter() throws IOException {
    when(getConnectorMock().getInputStream(anyMap())).thenReturn(new ByteArrayInputStream("r1c1;r1c2;r1c3\nr2c1;r2c2;r2c3".getBytes()));
  ...
  @Test public void testSetReadFrom() throws IOException {
    when(getConnectorMock().getInputStream(anyMap())).thenReturn(new ByteArrayInputStream("k1,k2,k3\nr1c1,r1c2,r1c3\nr2c1,r2c2,r2c3".getBytes()));
