...
public class FulltextIndexingAgent extends Agent<String> {
  ...
  @Override public ReturnValue processTask(final Task<String> task) throws Throwable {
  ...
  @Override public Class getSupportedTaskType() {
    return FulltextIndexingTask.class;
  }

  @Override protected boolean canHandleMore() {
    return true;
  }
    ...
    try {
      if (indexable != null && !(indexable instanceof File && ((File) indexable).isTemplate())) {
        ...
        if (inputStream != null) {
          ...
          try (FulltextTokenizer tokenizer = new FulltextTokenizer(indexable.indexedWordMinLength(), indexable.indexedWordMaxLength())) {
