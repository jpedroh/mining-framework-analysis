...
public class StructrFtpFile extends AbstractStructrFtpFile {
  ...
  @Override public OutputStream createOutputStream(final long l) throws IOException {
    try (Tx tx = StructrApp.getInstance(securityContext).tx()) {
  ...
  @Override public InputStream createInputStream(final long l) throws IOException {
    try (Tx tx = StructrApp.getInstance(securityContext).tx()) {
  ...
  @Override public List<FtpFile> listFiles() {
    logger.info("listFiles()");
    return null;
  }

  @Override public Object getPhysicalFile() {
