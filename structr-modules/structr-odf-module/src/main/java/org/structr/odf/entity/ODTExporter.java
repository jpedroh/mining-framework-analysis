...
public interface ODTExporter extends ODFExporter {
  static class Impl {
    static {
      ...
      type.addMethod("exportAttributes").addParameter("ctx", SecurityContext.class.getName()).addParameter("uuid", String.class.getName()).setSource(ODTExporter.class.getName() + ".exportAttributes(this, uuid, ctx);").addException(FrameworkException.class.getName()).setDoExport(true);
    }
  }
      ...
      for (int i = 0; i < nodes.getLength(); i++) {
        ...
        if (nodeFieldValue != null) {
          if (nodeFieldValue instanceof String[]) {
            ...
            if (nodeFieldValue instanceof Collection) {
