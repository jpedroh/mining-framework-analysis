...
public class TestInterestMethods {
  ...
  private static final ByteBuffer codedInterest = toBuffer(new int[] { 0x05, 0x50, 0x07, 0x0A, 0x08, 0x03, 0x6E, 0x64, 0x6E, 0x08, 0x03, 0x61, 0x62, 0x63, 0x09, 0x38, 0x0D, 0x01, 0x04, 0x0E, 0x01, 0x06, 0x0F, 0x22, 0x1D, 0x20, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x10, 0x07, 0x08, 0x03, 0x61, 0x62, 0x63, 0x13, 0x00, 0x11, 0x01, 0x01, 0x12, 0x00, 0x0A, 0x04, 0x61, 0x62, 0x61, 0x62, 0x0C, 0x02, 0x75, 0x30, 1 });

  static String dump(Object s1) {
    return "" + s1;
  }

  static String dump(Object s1, Object s2) {
    return s1 + " " + s2;
  }

  private static ArrayList initialDump = new ArrayList(Arrays.asList(new Object[] { "name: /ndn/abc", "minSuffixComponents: 4", "maxSuffixComponents: 6", "keyLocator: KeyLocatorDigest: 000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f", "exclude: abc,*", "childSelector: 1", "mustBeFresh: true", "nonce: 61626162", "lifetimeMilliseconds: 30000" }));

  private static ArrayList dumpInterest(Interest interest) {
    ArrayList result = new ArrayList();
    result.add(dump("name:", interest.getName().toUri()));
    result.add(dump("minSuffixComponents:", interest.getMinSuffixComponents() >= 0 ? interest.getMinSuffixComponents() : "<none>"));
    result.add(dump("maxSuffixComponents:", interest.getMaxSuffixComponents() >= 0 ? interest.getMaxSuffixComponents() : "<none>"));
    if (interest.getKeyLocator().getType() != KeyLocatorType.NONE) {
      if (interest.getKeyLocator().getType() == KeyLocatorType.KEY_LOCATOR_DIGEST) {
        result.add(dump("keyLocator: KeyLocatorDigest:", interest.getKeyLocator().getKeyData().toHex()));
      } else {
        if (interest.getKeyLocator().getType() == KeyLocatorType.KEYNAME) {
          result.add(dump("keyLocator: KeyName:", interest.getKeyLocator().getKeyName().toUri()));
        } else {
    ...
    result.add(dump("exclude:", interest.getExclude().size() > 0 ? interest.getExclude().toUri() : "<none>"));
    result.add(dump("childSelector:", interest.getChildSelector() >= 0 ? interest.getChildSelector() : "<none>"));
    result.add(dump("mustBeFresh:", interest.getMustBeFresh() ? "true" : "false"));
    result.add(dump("nonce:", interest.getNonce().size() == 0 ? "<none>" : interest.getNonce().toHex()));
    result.add(dump("lifetimeMilliseconds:", interest.getInterestLifetimeMilliseconds() < 0 ? "<none>" : "" + (long) interest.getInterestLifetimeMilliseconds()));
    return result;
  }
  ...
  private static boolean interestDumpsEqual(ArrayList dump1, ArrayList dump2) {
    String prefix = "nonce:";
    return Arrays.equals(removeStartingWith(dump1, prefix).toArray(), removeStartingWith(dump2, prefix).toArray());
  }

  private static Interest createFreshInterest() {
    Interest freshInterest = new Interest(new Name("/ndn/abc")).setMustBeFresh(false).setMinSuffixComponents(4).setMaxSuffixComponents(6).setInterestLifetimeMilliseconds(30000).setChildSelector(1).setMustBeFresh(true);
    freshInterest.getKeyLocator().setType(KeyLocatorType.KEY_LOCATOR_DIGEST);
    freshInterest.getKeyLocator().setKeyData(new Blob(new int[] { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F }));
  ...
  @Test public void testRedecodeImplicitDigestExclude() {
    Interest interest = new Interest(new Name("/A"));
    interest.getExclude().appendComponent(new Name("/sha256digest=" + "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f").get(0));
    ...
    ArrayList redecodedDump = dumpInterest(reDecodedInterest);
    assertTrue("Re-decoded interest does not match original", interestDumpsEqual(dump, redecodedDump));
  }

  @Test public void testCreateFresh() {
  ...
  @Test public void testCopyConstructor() {
    ...
    assertTrue("Interest constructed as deep copy does not match original", interestDumpsEqual(dumpInterest(interest), dumpInterest(referenceInterest)));
  }

  @Test public void testEmptyNonce() {
    Interest freshInterest = createFreshInterest();
    assertTrue("Freshly created interest should not have a nonce", freshInterest.getNonce().isNull());
  }

  @Test public void testSetRemovesNonce() {
    assertFalse(referenceInterest.getNonce().isNull());
    Interest interest = new Interest(referenceInterest);
    interest.getExclude().clear();
    assertTrue("Interest should not have a nonce after changing fields", interest.getNonce().isNull());
  }

  @Test public void testRefreshNonce() {
    Interest interest = new Interest(referenceInterest);
    Blob oldNonce = interest.getNonce();
    assertEquals(4, oldNonce.size());
    interest.refreshNonce();
    assertEquals("The refreshed nonce should be the same size", oldNonce.size(), interest.getNonce().size());
    assertFalse("The refreshed nonce should be different", interest.getNonce().equals(oldNonce));
  }

  @Test public void testExcludeMatches() {
    ...
    assertFalse(component.toEscapedString() + " should not match " + exclude.toUri(), exclude.matches(component));
    component = new Name("%00%0F").get(0);
    assertTrue(component.toEscapedString() + " should match " + exclude.toUri(), exclude.matches(component));
    component = new Name("%00%21").get(0);
    assertFalse(component.toEscapedString() + " should not match " + exclude.toUri(), exclude.matches(component));
  }

  @Test public void testVerifyDigestSha256() throws SecurityException {
  ...
  @Test public void testMatchesData() throws EncodingException {
    Interest interest = new Interest(new Name("/A"));
    ...
    Interest interest7 = new Interest(new Name("/A/B/sha256digest=" + "54008e240a7eea2714a161dfddf0dd6ced223b3856e9da96792151e180f3b128"));
    assertEquals(true, interest7.matchesData(data7));
    Interest interest7b = new Interest(new Name("/A/B/%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00" + "%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00"));
  ...
  @Test public void testInterestFilterMatching() {
