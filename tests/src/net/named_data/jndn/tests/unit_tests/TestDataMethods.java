...
class CredentialStorage {
  public CredentialStorage() {
    ...
    defaultCertName_ = keyName.getSubName(0, keyName.size() - 1).append("KEY").append(keyName.get(-1)).append("ID-CERT").append("0");
    Name ecdsaKeyName = new Name("/testEcdsa/DSK-123");
    ecdsaCertName_ = ecdsaKeyName.getSubName(0, ecdsaKeyName.size() - 1).append("KEY").append(ecdsaKeyName.get(-1)).append("ID-CERT").append("0");
    try {
      identityStorage_.addKey(keyName, KeyType.RSA, new Blob(DEFAULT_RSA_PUBLIC_KEY_DER, false));
      privateKeyStorage_.setKeyPairForKeyName(keyName, KeyType.RSA, DEFAULT_RSA_PUBLIC_KEY_DER, DEFAULT_RSA_PRIVATE_KEY_DER);
      identityStorage_.addKey(ecdsaKeyName, KeyType.ECDSA, new Blob(DEFAULT_EC_PUBLIC_KEY_DER, false));
      privateKeyStorage_.setKeyPairForKeyName(ecdsaKeyName, KeyType.ECDSA, DEFAULT_EC_PUBLIC_KEY_DER, DEFAULT_EC_PRIVATE_KEY_DER);
  ...
  public void signData(Data data, Name certificateName) throws SecurityException {
  ...
  private final KeyChain keyChain_ = new KeyChain(new IdentityManager(identityStorage_, privateKeyStorage_), new SelfVerifyPolicyManager(identityStorage_));
...
public class TestDataMethods {
  ...
  private static final ByteBuffer codedData = toBuffer(new int[] { , 1 });

  private static final int experimentalSignatureType = 100;

  private static final ByteBuffer experimentalSignatureInfo = toBuffer(new int[] { 0x16, 0x08, 0x1B, 0x01, experimentalSignatureType, 0x81, 0x03, 1, 2, 3 });

  private static final ByteBuffer experimentalSignatureInfoNoSignatureType = toBuffer(new int[] { 0x16, 0x05, 0x81, 0x03, 1, 2, 3 });

  private static final ByteBuffer experimentalSignatureInfoBadTlv = toBuffer(new int[] { 0x16, 0x08, 0x1B, 0x01, experimentalSignatureType, 0x81, 0x10, 1, 2, 3 });
  ...
  private static ArrayList dumpData(Data data) {
    ...
    if (data.getMetaInfo().getType() != ContentType.BLOB) {
      result.add(dump("metaInfo.type:", data.getMetaInfo().getType() == ContentType.LINK ? "LINK" : (data.getMetaInfo().getType() == ContentType.KEY ? "KEY" : "unknown")));
    }
    result.add(dump("metaInfo.freshnessPeriod (milliseconds):", data.getMetaInfo().getFreshnessPeriod() >= 0 ? "" + (long) data.getMetaInfo().getFreshnessPeriod() : "<none>"));
    result.add(dump("metaInfo.finalBlockId:", data.getMetaInfo().getFinalBlockId().getValue().size() > 0 ? data.getMetaInfo().getFinalBlockId().toEscapedString() : "<none>"));
    if (data.getSignature() instanceof Sha256WithRsaSignature) {
      ...
      result.add(dump("signature.signature:", signature.getSignature().size() == 0 ? "<none>" : signature.getSignature().toHex()));
      if (signature.getKeyLocator().getType() != KeyLocatorType.NONE) {
        if (signature.getKeyLocator().getType() == KeyLocatorType.KEY_LOCATOR_DIGEST) {
          result.add(dump("signature.keyLocator: KeyLocatorDigest:", signature.getKeyLocator().getKeyData().toHex()));
        } else {
          if (signature.getKeyLocator().getType() == KeyLocatorType.KEYNAME) {
            result.add(dump("signature.keyLocator: KeyName:", signature.getKeyLocator().getKeyName().toUri()));
          } else {
  ...
  private static ArrayList initialDump = new ArrayList(Arrays.asList(new Object[] { "name: /ndn/abc", "content (raw): SUCCESS!", "content (hex): 5355434345535321", "metaInfo.freshnessPeriod (milliseconds): 5000", "metaInfo.finalBlockId: %00%09", "signature.signature: ", "signature.keyLocator: KeyName: /testname/KEY/DSK-123/ID-CERT" }));
  ...
  private static boolean dataDumpsEqual(ArrayList dump1, ArrayList dump2) {
  ...
  @Test public void testDump() {
    ...
    assertArrayEquals("Initial dump does not have expected format", dumpData(data).toArray(), initialDump.toArray());
  }
  ...
  @Test public void testEncodeDecode() {
    ...
    assertArrayEquals("Re-decoded data does not match original dump", initialDump.toArray(), dumpData(reDecodedData).toArray());
  }

  @Test public void testEmptySignature() {
    Data data = new Data();
    Sha256WithRsaSignature signature = (Sha256WithRsaSignature) data.getSignature();
    assertTrue("Key locator type on unsigned data should not be set", signature.getKeyLocator().getType() == KeyLocatorType.NONE);
    assertTrue("Non-empty signature on unsigned data", signature.getSignature().isNull());
  }

  @Test public void testCopyFields() {
    ...
    assertTrue("Freshly created data does not match original dump", dataDumpsEqual(freshDump, initialDump));
  }

  @Test public void testVerify() throws SecurityException {
  ...
  @Test public void testVerifyEcdsa() throws SecurityException {
    VerifyCounter counter = new VerifyCounter();
    try {
  ...
  @Test public void testGenericSignature() {
    GenericSignature signature = new GenericSignature();
    signature.setSignatureInfoEncoding(new Blob(experimentalSignatureInfo, false), -1);
    ...
    assertTrue(new Blob(experimentalSignatureInfo, false).equals(decodedSignature.getSignatureInfoEncoding()));
    assertTrue(signatureValue.equals(decodedSignature.getSignature()));
    signature = new GenericSignature();
    signature.setSignatureInfoEncoding(new Blob(experimentalSignatureInfoNoSignatureType, false), -1);
    signature.setSignature(signatureValue);
    freshData.setSignature(signature);
    ...
    signature = new GenericSignature();
    signature.setSignatureInfoEncoding(new Blob(experimentalSignatureInfoBadTlv, false), -1);
    signature.setSignature(signatureValue);
    freshData.setSignature(signature);
    gotError = true;
    try {
  ...
  @Test public void testFullName() throws EncodingException {
    Data data = new Data();
    try {
      data.wireDecode(codedData);
    } catch (EncodingException ex) {
    ...
    assertEquals("/ndn/abc/sha256digest=" + "96556d685dcb1af04be4ae57f0e7223457d4055ea9b3d07c0d337bef4a8b3ee9", data.getFullName().toUri());
