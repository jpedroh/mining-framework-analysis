...
public class TestDerEncodeDecode {
  ...
  private static final ByteBuffer PUBLIC_KEY = toBuffer(new int[] {  });
  ...
  @Before public void setUp() throws DerDecodingException, UnrecognizedKeyFormatException {
    ...
    cert.setName(new Name("/test/KEY/ksk-1457560485494/ID-CERT/%FD%00%00%01S%80H%E1%F3"));
  ...
  @Test public void testRepresentation() {
    assertEquals("Certificate dump does not have expected format", CERT_STRING, "" + toyCert);
  }

  @Test public void testEncodeDecode() throws DerDecodingException, DerEncodingException {
    toyCert.encode();
    ...
    assertEquals("Certificate representation changed after encoding", "" + toyCert, "" + decoded_cert);
  }

  @Test public void testExtension() throws DerDecodingException, DerEncodingException {
    ...
    CertificateExtension certExtension = new CertificateExtension(oidString, isCritical, extValueData);
    toyCert.encode();
    ...
    assertEquals("Wrong number of certificate extensions after decoding", 1, decodedCert.getExtensionList().size());
    CertificateExtension decodedExtension = (CertificateExtension) decodedCert.getExtensionList().get(0);
    assertEquals("Certificate extension has the wrong OID after decoding", oidString, "" + decodedExtension.getOid());
    assertEquals("Certificate extension has the wrong isCritical value after decoding", isCritical, decodedExtension.getIsCritical());
    DerNode parsedExtValue = DerNode.parse(decodedExtension.getValue().buf());
    List decodedExtValueRoot = parsedExtValue.getChildren();
    assertEquals("Wrong number of certificate extension value items after decoding", 3, decodedExtValueRoot.size());
    ...
    assertEquals("Wrong extension value name after decoding", name, "" + decodedName.toVal());
    assertEquals("Wrong extension value trust class after decoding", trustClass, (int) (Integer) decodedTrustClass.toVal());
    assertEquals("Wrong extension value trust level after decoding", trustLevel, (int) (Integer) decodedTrustLevel.toVal());
  }

  @Test public void testDecode() throws DerDecodingException {
    Data data = new Data(new Name("/tmp"));
    data.setContent(new Blob(REAL_CERT, false));
    Certificate realCert = new Certificate(data);
    assertEquals("Certificate representation changed after decoding", REAL_CERT_STRING, "" + realCert);
  }

  @Test public void testOid() throws DerEncodingException, DerDecodingException {
    String oidString = "1.2.840.113549.1.1.11";
    DerOid derOid = new DerOid(oidString);
    String expectedEncoding = "06092a864886f70d01010b";
    assertEquals("Incorrect OID encoding", expectedEncoding, derOid.encode().toHex());
    assertEquals("Incorrect decoded OID", oidString, derOid.toVal());
  }

  @Test public void testPrepareUnsignedCertificate() throws SecurityException, DerDecodingException {
    ...
    subjectDescriptions.add(new CertificateSubjectDescription(TEST_OID, "TEST NAME"));
    IdentityCertificate newCertificate = identityManager.prepareUnsignedIdentityCertificate(keyName, keyName.getPrefix(1), toyCertNotBefore, toyCertNotAfter, subjectDescriptions);
    newCertificate.setName(new Name(newCertificate.getName().getPrefix(-1).append(toyCert.getName().get(-1))));
    IdentityCertificate certificateCopy = new IdentityCertificate(newCertificate);
    assertEquals("Prepared unsigned certificate dump does not have the expected format", "" + toyCert, "" + certificateCopy);
  }
}