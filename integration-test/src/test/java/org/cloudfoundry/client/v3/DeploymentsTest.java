...
@IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_2_4) public final class DeploymentsTest extends AbstractIntegrationTest {
  ...
  @Test public void listFilterByApplication() throws Exception {
    String name = this.nameFactory.getApplicationName();
    Path path = new ClassPathResource("test-application.zip").getFile().toPath();
  ...
  @Test public void listFilterByState() throws Exception {
    String name = this.nameFactory.getApplicationName();
    Path path = new ClassPathResource("test-application.zip").getFile().toPath();
  ...
  private static Mono<String> createApplicationId(CloudFoundryOperations cloudFoundryOperations, String name, Path path) {
    return requestCreateApplication(cloudFoundryOperations, name, path).then(getApplicationId(cloudFoundryOperations, name));
  }
  ...
  private static Mono<String> createDeploymentId(CloudFoundryClient cloudFoundryClient, String applicationId, String dropletId) {
    return requestCreateDeployment(cloudFoundryClient, applicationId, dropletId).map(CreateDeploymentResponse::getId);
  }

  private static Mono<String> getApplicationId(CloudFoundryOperations cloudFoundryOperations, String name) {
    return cloudFoundryOperations.applications().get(GetApplicationRequest.builder().name(name).build()).map(ApplicationDetail::getId);
  }

  private static Mono<String> getDropletId(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return requestCurrentDroplet(cloudFoundryClient, applicationId).map(GetApplicationCurrentDropletResponse::getId);
  }
  ...
  private static Mono<Void> requestCreateApplication(CloudFoundryOperations cloudFoundryOperations, String name, Path path) {
    return cloudFoundryOperations.applications().push(PushApplicationRequest.builder().path(path).buildpack("staticfile_buildpack").diskQuota(256).healthCheckType(ApplicationHealthCheck.PORT).memory(64).name(name).noStart(false).build());
  }
  ...
  private static Mono<CreateDeploymentResponse> requestCreateDeployment(CloudFoundryClient cloudFoundryClient, String applicationId, String dropletId) {
    return cloudFoundryClient.deploymentsV3().create(CreateDeploymentRequest.builder().droplet(Relationship.builder().id(dropletId).build()).relationships(DeploymentRelationships.builder().app(ToOneRelationship.builder().data(Relationship.builder().id(applicationId).build()).build()).build()).build());
  }
  ...
  private static Mono<GetApplicationCurrentDropletResponse> requestCurrentDroplet(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return cloudFoundryClient.applicationsV3().getCurrentDroplet(GetApplicationCurrentDropletRequest.builder().applicationId(applicationId).build());
  }

  private static Flux<DeploymentResource> requestListDeployments(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return PaginationUtils.requestClientV3Resources((page) -> cloudFoundryClient.deploymentsV3().list(ListDeploymentsRequest.builder().applicationId(applicationId).page(page).build()));
  }
}