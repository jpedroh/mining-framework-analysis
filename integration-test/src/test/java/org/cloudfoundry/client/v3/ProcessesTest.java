...
@IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_2_0) public final class ProcessesTest extends AbstractIntegrationTest {
  ...
  private static Mono<GetApplicationProcessResponse> requestGetApplicationProcess(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return cloudFoundryClient.applicationsV3().getProcess(GetApplicationProcessRequest.builder().applicationId(applicationId).type("web").build());
  }

  private static Mono<GetProcessResponse> requestGetProcess(CloudFoundryClient cloudFoundryClient, String processId) {
    return cloudFoundryClient.processes().get(GetProcessRequest.builder().processId(processId).build());
  }
}