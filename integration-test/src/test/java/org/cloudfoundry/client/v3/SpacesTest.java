...
@IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_1_12) public final class SpacesTest extends AbstractIntegrationTest {
  @Autowired private CloudFoundryClient cloudFoundryClient;

  @Autowired private Mono<String> organizationId;

  @Test public void assignIsolationSegment() {
    String isolationSegmentName = this.nameFactory.getIsolationSegmentName();
    String spaceName = this.nameFactory.getSpaceName();
    this.organizationId.flatMap((organizationId) -> Mono.zip(createIsolationSegmentId(this.cloudFoundryClient, isolationSegmentName, organizationId), createSpaceId(this.cloudFoundryClient, organizationId, spaceName))).flatMap(function((isolationSegmentId, spaceId) -> Mono.zip(Mono.just(isolationSegmentId), this.cloudFoundryClient.spacesV3().assignIsolationSegment(AssignSpaceIsolationSegmentRequest.builder().data(Relationship.builder().id(isolationSegmentId).build()).spaceId(spaceId).build()).map((response) -> response.getData().getId())))).as(StepVerifier::create).consumeNextWith(tupleEquality()).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void create() {
    String spaceName = this.nameFactory.getSpaceName();
    this.organizationId.flatMap((organizationId) -> this.cloudFoundryClient.spacesV3().create(CreateSpaceRequest.builder().name(spaceName).relationships(SpaceRelationships.builder().organization(ToOneRelationship.builder().data(Relationship.builder().id(organizationId).build()).build()).build()).build())).then(requestListSpaces(this.cloudFoundryClient, spaceName).single()).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void getIsolationSegment() {
    String isolationSegmentName = this.nameFactory.getIsolationSegmentName();
    String spaceName = this.nameFactory.getSpaceName();
    this.organizationId.flatMap((organizationId) -> Mono.zip(createIsolationSegmentId(this.cloudFoundryClient, isolationSegmentName, organizationId), createSpaceId(this.cloudFoundryClient, organizationId, spaceName))).delayUntil(function((isolationSegmentId, spaceId) -> requestAssignIsolationSegment(this.cloudFoundryClient, isolationSegmentId, spaceId))).flatMap(function((isolationSegmentId, spaceId) -> Mono.zip(Mono.just(isolationSegmentId), this.cloudFoundryClient.spacesV3().getIsolationSegment(GetSpaceIsolationSegmentRequest.builder().spaceId(spaceId).build()).map((response) -> response.getData().getId())))).as(StepVerifier::create).consumeNextWith(tupleEquality()).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void list() {
    String spaceName = this.nameFactory.getSpaceName();
    this.organizationId.flatMap((organizationId) -> requestCreateSpace(this.cloudFoundryClient, organizationId, spaceName)).thenMany(PaginationUtils.requestClientV3Resources((page) -> this.cloudFoundryClient.spacesV3().list(ListSpacesRequest.builder().page(page).build()))).filter((resource) -> spaceName.equals(resource.getName())).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  private static Mono<String> createIsolationSegmentId(CloudFoundryClient cloudFoundryClient, String isolationSegmentName, String organizationId) {
    return requestCreateIsolationSegment(cloudFoundryClient, isolationSegmentName).map(CreateIsolationSegmentResponse::getId).delayUntil((isolationSegmentId) -> requestAddIsolationSegmentOrganizationEntitlement(cloudFoundryClient, isolationSegmentId, organizationId));
  }

  private static Mono<String> createSpaceId(CloudFoundryClient cloudFoundryClient, String organizationId, String spaceName) {
    return requestCreateSpace(cloudFoundryClient, organizationId, spaceName).map(CreateSpaceResponse::getId);
  }
  ...
  private static Mono<CreateSpaceResponse> requestCreateSpace(CloudFoundryClient cloudFoundryClient, String organizationId, String spaceName) {
    return cloudFoundryClient.spacesV3().create(CreateSpaceRequest.builder().name(spaceName).relationships(SpaceRelationships.builder().organization(ToOneRelationship.builder().data(Relationship.builder().id(organizationId).build()).build()).build()).build());
  }

  private static Flux<SpaceResource> requestListSpaces(CloudFoundryClient cloudFoundryClient, String spaceName) {
    return PaginationUtils.requestClientV3Resources((page) -> cloudFoundryClient.spacesV3().list(ListSpacesRequest.builder().name(spaceName).page(page).build()));
  }
}