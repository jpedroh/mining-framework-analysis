...
@IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_2_1) public final class ServiceInstancesTest extends AbstractIntegrationTest {
  ...
  private static Mono<String> getPlanId(CloudFoundryClient cloudFoundryClient, String serviceBrokerId, String serviceName) {
    return requestListServices(cloudFoundryClient, serviceBrokerId).filter((resource) -> serviceName.equals(resource.getEntity().getLabel())).single().map(ResourceUtils::getId).flatMapMany((serviceId) -> requestListServicePlans(cloudFoundryClient, serviceId)).single().map(ResourceUtils::getId);
  }
  ...
  private static Mono<ListSharedSpacesRelationshipResponse> requestListSharedSpacesRelationship(CloudFoundryClient cloudFoundryClient, String serviceInstanceId) {
    return cloudFoundryClient.serviceInstancesV3().listSharedSpacesRelationship(ListSharedSpacesRelationshipRequest.builder().serviceInstanceId(serviceInstanceId).build());
  }
  ...
  private static Mono<ShareServiceInstanceResponse> requestShareServiceInstance(CloudFoundryClient cloudFoundryClient, String newSpaceId, String serviceInstanceId) {
    return cloudFoundryClient.serviceInstancesV3().share(ShareServiceInstanceRequest.builder().data(Relationship.builder().id(newSpaceId).build()).serviceInstanceId(serviceInstanceId).build());
  }
}