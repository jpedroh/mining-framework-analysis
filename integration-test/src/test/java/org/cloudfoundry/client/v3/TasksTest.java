...
@IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_1_12) public final class TasksTest extends AbstractIntegrationTest {
  ...
  @Test public void listFilterByName() throws IOException {
    String applicationName = this.nameFactory.getApplicationName();
  ...
  @Test public void listFilterByState() throws IOException {
    String applicationName = this.nameFactory.getApplicationName();
  ...
  private static Mono<Void> createApplication(CloudFoundryOperations cloudFoundryOperations, String name) throws IOException {
    return cloudFoundryOperations.applications().push(PushApplicationRequest.builder().path(new ClassPathResource("test-application.zip").getFile().toPath()).buildpack("staticfile_buildpack").diskQuota(256).healthCheckType(ApplicationHealthCheck.PORT).memory(64).name(name).noStart(false).build());
  }
  ...
  private static Mono<String> getApplicationId(CloudFoundryClient cloudFoundryClient, String applicationName) {
    return PaginationUtils.requestClientV3Resources((page) -> cloudFoundryClient.applicationsV3().list(ListApplicationsRequest.builder().name(applicationName).build())).single().map(ApplicationResource::getId);
  }

  private static Mono<String> getDropletId(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return cloudFoundryClient.applicationsV3().getCurrentDroplet(GetApplicationCurrentDropletRequest.builder().applicationId(applicationId).build()).map(GetApplicationCurrentDropletResponse::getId);
  }

  private static Mono<String> getTaskName(CloudFoundryClient cloudFoundryClient, String taskId) {
    return cloudFoundryClient.tasks().get(GetTaskRequest.builder().taskId(taskId).build()).map(GetTaskResponse::getName);
  }

  private static Mono<CreateTaskResponse> requestCreateTask(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return cloudFoundryClient.tasks().create(CreateTaskRequest.builder().applicationId(applicationId).command("ls").diskInMb(64).memoryInMb(67).build());
  }

  private static Flux<TaskResource> requestListTasks(CloudFoundryClient cloudFoundryClient) {
    return PaginationUtils.requestClientV3Resources((page) -> cloudFoundryClient.tasks().list(ListTasksRequest.builder().page(page).build()));
  }
}