...
@IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_1_11) public final class IsolationSegmentsTest extends AbstractIntegrationTest {
  ...
  @Test public void listFilterByOrganizationId() {
    String isolationSegmentName = this.nameFactory.getIsolationSegmentName();
    String organizationName = this.nameFactory.getOrganizationName();
    createOrganizationId(this.cloudFoundryClient, organizationName).flatMap((organizationId) -> createEntitledIsolationSegmentId(this.cloudFoundryClient, isolationSegmentName, organizationId).thenReturn(organizationId)).flatMapMany((organizationId) -> PaginationUtils.requestClientV3Resources((page) -> this.cloudFoundryClient.isolationSegments().list(ListIsolationSegmentsRequest.builder().organizationId(organizationId).page(page).build()))).filter((resource) -> isolationSegmentName.equals(resource.getName())).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }
