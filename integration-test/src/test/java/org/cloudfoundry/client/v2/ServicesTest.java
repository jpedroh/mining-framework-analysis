...
public final class ServicesTest extends AbstractIntegrationTest {
  ...
  @Test public void deleteAsyncFalse() {
    ...
    String spaceName = this.nameFactory.getSpaceName();
    ServiceBrokerUtils.ServiceBrokerMetadata serviceBrokerMetadata = this.organizationId.flatMap((organizationId) -> createSpaceId(this.cloudFoundryClient, organizationId, spaceName)).flatMap((spaceId) -> createServiceBroker(this.cloudFoundryClient, this.nameFactory, planName, serviceBrokerName, serviceName, spaceId, true)).block(Duration.ofMinutes(5));
    getServiceId(this.cloudFoundryClient, serviceBrokerMetadata.serviceBrokerId).flatMapMany((serviceId) -> deleteServicePlans(this.cloudFoundryClient, serviceBrokerMetadata.serviceBrokerId, serviceId).thenMany(Mono.just(serviceId))).flatMap((serviceId) -> this.cloudFoundryClient.services().delete(DeleteServiceRequest.builder().async(false).serviceId(serviceId).build()).then(Mono.just(serviceBrokerMetadata.serviceBrokerId))).thenMany(requestListServices(this.cloudFoundryClient, serviceBrokerMetadata.serviceBrokerId)).as(StepVerifier::create).expectComplete().verify(Duration.ofMinutes(1));
    deleteServiceBroker(this.cloudFoundryClient, serviceBrokerMetadata.applicationMetadata.applicationId).block(Duration.ofMinutes(5));
  }
  ...
  @Test public void listServicePlansFilterByServiceInstanceIds() {
    ...
    this.serviceBrokerId.flatMap((serviceBrokerId) -> Mono.zip(getServiceId(this.cloudFoundryClient, serviceBrokerId), this.spaceId)).flatMap(function((serviceId, spaceId) -> Mono.zip(Mono.just(serviceId), createServiceInstanceId(this.cloudFoundryClient, serviceInstanceName, serviceId, spaceId)))).flatMapMany(function((serviceId, serviceInstanceId) -> PaginationUtils.requestClientV2Resources((page) -> this.cloudFoundryClient.services().listServicePlans(ListServiceServicePlansRequest.builder().serviceId(serviceId).serviceInstanceId(serviceInstanceId).page(page).build())))).map((response) -> response.getEntity().getName()).as(StepVerifier::create).expectNext(this.planName).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  private static Mono<String> getServiceId(CloudFoundryClient cloudFoundryClient, String serviceBrokerId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.services().list(ListServicesRequest.builder().page(page).build())).filter((resource) -> serviceBrokerId.equals(ResourceUtils.getEntity(resource).getServiceBrokerId())).single().map(ResourceUtils::getId);
  }
