...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  @Test public void listRoutesFilterByPort() {
    ...
    Mono.zip(getRouterGroupId(this.routingClient, DEFAULT_ROUTER_GROUP).flatMap((routerGroupId) -> createTcpDomainId(this.cloudFoundryClient, domainName, routerGroupId)), this.spaceId).flatMap(function((domainId, spaceId) -> Mono.zip(createUserProvidedServiceInstanceId(this.cloudFoundryClient, instanceName, spaceId), createRouteId(this.cloudFoundryClient, domainId, port, spaceId)))).flatMap(function((instanceId, routeId) -> requestAssociateRoute(this.cloudFoundryClient, instanceId, routeId).thenReturn(instanceId))).flatMapMany((instanceId) -> Mono.zip(Mono.just(instanceId), PaginationUtils.requestClientV2Resources((page) -> this.cloudFoundryClient.userProvidedServiceInstances().listRoutes(ListUserProvidedServiceInstanceRoutesRequest.builder().page(page).port(port.toString()).userProvidedServiceInstanceId(instanceId).build())).map((resource) -> ResourceUtils.getEntity(resource).getServiceInstanceId()).single())).as(StepVerifier::create).consumeNextWith(tupleEquality()).expectComplete().verify(Duration.ofMinutes(5));
  }
