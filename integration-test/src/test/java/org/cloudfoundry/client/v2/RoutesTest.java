...
public final class RoutesTest extends AbstractIntegrationTest {
  ...
  @Test public void associateApplication() {
  ...
  @Test public void exists() {
    ...
    Mono.zip(this.organizationId.flatMap((organizationId) -> createPrivateDomainId(this.cloudFoundryClient, domainName, organizationId)), this.spaceId).delayUntil(function((domainId, spaceId) -> requestCreateRoute(this.cloudFoundryClient, domainId, hostName, spaceId))).flatMap(function((domainId, spaceId) -> this.cloudFoundryClient.routes().exists(RouteExistsRequest.builder().domainId(domainId).host(hostName).build()))).as(StepVerifier::create).expectNext(true).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void existsDoesNotExist() {
    String domainName = this.nameFactory.getDomainName();
    String hostName1 = this.nameFactory.getHostName();
    String hostName2 = this.nameFactory.getHostName();
    Mono.zip(this.organizationId.flatMap((organizationId) -> createPrivateDomainId(this.cloudFoundryClient, domainName, organizationId)), this.spaceId).delayUntil(function((domainId, spaceId) -> this.cloudFoundryClient.routes().create(CreateRouteRequest.builder().domainId(domainId).host(hostName1).spaceId(spaceId).build()))).flatMap(function((domainId, spaceId) -> this.cloudFoundryClient.routes().exists(RouteExistsRequest.builder().domainId(domainId).host(hostName2).build()))).as(StepVerifier::create).expectNext(false).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  @Test public void listApplicationsFilterBySpaceId() {
    String applicationName = this.nameFactory.getApplicationName();
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void listFilterByDomainId() {
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void listMappingsFilterByApplicationId() {
    String applicationName = this.nameFactory.getApplicationName();
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void removeApplication() {
    String applicationName = this.nameFactory.getApplicationName();
    String domainName = this.nameFactory.getDomainName();
  ...
  private static Mono<AssociateRouteApplicationResponse> associateApplicationWithRoute(CloudFoundryClient cloudFoundryClient, String applicationId, String routeId) {
    return cloudFoundryClient.routes().associateApplication(AssociateRouteApplicationRequest.builder().applicationId(applicationId).routeId(routeId).build());
  }
  ...
  private static Mono<CreateRouteResponse> requestCreateRoute(CloudFoundryClient cloudFoundryClient, String domainId, String host, String spaceId) {
    return cloudFoundryClient.routes().create(CreateRouteRequest.builder().domainId(domainId).host(host).spaceId(spaceId).build());
  }
  ...
  private static Mono<GetRouteResponse> requestGetRoute(CloudFoundryClient cloudFoundryClient, String routeId) {
    return cloudFoundryClient.routes().get(GetRouteRequest.builder().routeId(routeId).build());
  }
}