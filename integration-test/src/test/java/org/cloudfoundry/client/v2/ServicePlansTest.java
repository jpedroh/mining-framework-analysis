...
public final class ServicePlansTest extends AbstractIntegrationTest {
  ...
  @Test public void listFilterByServiceBrokerId() {
    this.serviceBrokerId.flatMapMany((serviceBrokerId) -> PaginationUtils.requestClientV2Resources((page) -> this.cloudFoundryClient.servicePlans().list(ListServicePlansRequest.builder().page(page).serviceBrokerId(serviceBrokerId).build()))).map((response) -> ResourceUtils.getEntity(response).getName()).as(StepVerifier::create).expectNext(this.planName).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  @Test public void listFilterByServiceInstanceId() {
  ...
  @Test public void listServiceInstancesFilterBySpaceId() {
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
  ...
  @Test public void update() {
    ...
    String spaceName = this.nameFactory.getSpaceName();
    ServiceBrokerUtils.ServiceBrokerMetadata serviceBrokerMetadata = this.organizationId.flatMap((organizationId) -> createSpaceId(this.cloudFoundryClient, organizationId, spaceName)).flatMap((spaceId) -> createServiceBroker(this.cloudFoundryClient, this.nameFactory, planName, serviceBrokerName, serviceName, spaceId, true)).block(Duration.ofMinutes(5));
    getServicePlanId(this.cloudFoundryClient, serviceBrokerMetadata.serviceBrokerId).flatMap((servicePlanId) -> this.cloudFoundryClient.servicePlans().update(UpdateServicePlanRequest.builder().publiclyVisible(false).servicePlanId(servicePlanId).build())).thenMany(requestListServicePlans(this.cloudFoundryClient, serviceBrokerMetadata.serviceBrokerId)).filter((response) -> planName.equals(ResourceUtils.getEntity(response).getName())).map((response) -> ResourceUtils.getEntity(response).getPubliclyVisible()).as(StepVerifier::create).expectNext(false).expectComplete().verify(Duration.ofMinutes(5));
    deleteServiceBroker(this.cloudFoundryClient, serviceBrokerMetadata.applicationMetadata.applicationId).block(Duration.ofMinutes(5));
  }
  ...
  private static Mono<String> createServiceInstanceId(CloudFoundryClient cloudFoundryClient, String serviceInstanceName, String servicePlanId, String spaceId) {
    return requestCreateServiceInstance(cloudFoundryClient, serviceInstanceName, servicePlanId, spaceId).then(requestListServiceInstances(cloudFoundryClient, serviceInstanceName).single().map(ResourceUtils::getId));
  }
  ...
  private static Mono<String> getServiceId(CloudFoundryClient cloudFoundryClient, String serviceName) {
    return requestListServices(cloudFoundryClient, serviceName).single().map(ResourceUtils::getId);
  }

  private static Mono<String> getServicePlanId(CloudFoundryClient cloudFoundryClient, String serviceBrokerId) {
    return requestListServicePlans(cloudFoundryClient, serviceBrokerId).map(ResourceUtils::getId).single();
  }
  ...
  private static Mono<CreateApplicationResponse> requestCreateApplication(CloudFoundryClient cloudFoundryClient, String spaceId, String applicationName) {
    return cloudFoundryClient.applicationsV2().create(CreateApplicationRequest.builder().buildpack("https://github.com/cloudfoundry/java-buildpack.git").memory(64).name(applicationName).spaceId(spaceId).build());
  }
  ...
  private static Mono<CreateServiceInstanceResponse> requestCreateServiceInstance(CloudFoundryClient cloudFoundryClient, String serviceInstanceName, String servicePlanId, String spaceId) {
    return cloudFoundryClient.serviceInstances().create(CreateServiceInstanceRequest.builder().name(serviceInstanceName).servicePlanId(servicePlanId).spaceId(spaceId).build());
  }
  ...
  private static Flux<ServiceInstanceResource> requestListServiceInstances(CloudFoundryClient cloudFoundryClient, String serviceInstanceName) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.serviceInstances().list(ListServiceInstancesRequest.builder().name(serviceInstanceName).build()));
  }
  ...
  private static Flux<ServicePlanResource> requestListServicePlans(CloudFoundryClient cloudFoundryClient, String serviceBrokerId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.servicePlans().list(ListServicePlansRequest.builder().page(page).serviceBrokerId(serviceBrokerId).build()));
  }
