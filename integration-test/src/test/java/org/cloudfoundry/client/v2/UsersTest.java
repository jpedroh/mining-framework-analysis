...
public final class UsersTest extends AbstractIntegrationTest {
  ...
  @Test public void listSpacesFilterByOrganizationId() {
    String spaceName = this.nameFactory.getSpaceName();
    String userId = this.nameFactory.getUserId();
    this.organizationId.flatMap((organizationId) -> Mono.zip(Mono.just(organizationId), createSpaceId(this.cloudFoundryClient, organizationId, spaceName))).flatMap(function((organizationId, spaceId) -> requestCreateUser(this.cloudFoundryClient, spaceId, userId).then(requestAssociateSpace(this.cloudFoundryClient, spaceId, userId)).thenReturn(organizationId))).flatMapMany((organizationId) -> PaginationUtils.requestClientV2Resources((page) -> this.cloudFoundryClient.users().listSpaces(ListUserSpacesRequest.builder().organizationId(organizationId).page(page).userId(userId).build()))).map((resource) -> resource.getEntity().getName()).as(StepVerifier::create).expectNext(spaceName).expectComplete().verify(Duration.ofMinutes(5));
  }
