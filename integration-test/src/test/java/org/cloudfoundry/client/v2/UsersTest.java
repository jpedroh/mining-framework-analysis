...
public final class UsersTest extends AbstractIntegrationTest {
  ...
  @Test public void summary() {
    String organizationName = this.nameFactory.getOrganizationName();
    String userId = this.nameFactory.getUserId();
    createOrganizationId(this.cloudFoundryClient, organizationName).then((organizationId) -> requestCreateUser(this.cloudFoundryClient, userId).then(requestAssociateOrganization(this.cloudFoundryClient, organizationId, userId))).then(this.cloudFoundryClient.users().summary(SummaryUserRequest.builder().userId(userId).build())).map((response) -> ResourceUtils.getEntity(response).getOrganizations()).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }
