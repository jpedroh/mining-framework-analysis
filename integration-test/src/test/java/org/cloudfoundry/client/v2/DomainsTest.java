...
@SuppressWarnings(value = { "deprecation" }) public final class DomainsTest extends AbstractIntegrationTest {
  ...
  @Test public void create() {
    ...
    this.organizationId.flatMap((organizationId) -> Mono.zip(createDomainEntity(this.cloudFoundryClient, organizationId, domainName), Mono.just(organizationId))).as(StepVerifier::create).consumeNextWith(domainNameAndOrganizationIdEquality(domainName)).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  @Test public void deleteNotAsync() {
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void listDomainSpaces() {
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void listDomainSpacesFilterByApplicationId() {
  ...
  @Test public void listDomainSpacesFilterByDeveloperId() {
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void listDomainSpacesFilterByName() {
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void listDomainSpacesFilterByOrganizationId() {
    String domainName = this.nameFactory.getDomainName();
  ...
  @Test public void listFilterByOwningOrganizationId() {
    String domainName = this.nameFactory.getDomainName();
  ...
  private static Mono<DomainEntity> createDomainEntity(CloudFoundryClient cloudFoundryClient, String organizationId, String domainName) {
    return requestCreateDomain(cloudFoundryClient, organizationId, domainName).map(ResourceUtils::getEntity);
  }

  private static Mono<String> createDomainId(CloudFoundryClient cloudFoundryClient, String domainName, String organizationId) {
    return requestCreateDomain(cloudFoundryClient, organizationId, domainName).map(ResourceUtils::getId);
  }
  ...
  private static Mono<DomainEntity> getDomainEntity(CloudFoundryClient cloudFoundryClient, String domainId) {
    return requestGetDomain(cloudFoundryClient, domainId).map(ResourceUtils::getEntity);
  }

  private static Mono<String> getRouteId(CloudFoundryClient cloudFoundryClient, String domainId, String spaceId) {
    return requestCreateRoute(cloudFoundryClient, domainId, spaceId).map(ResourceUtils::getId);
  }

  private static Mono<String> getSpaceName(CloudFoundryClient cloudFoundryClient, String spaceId) {
    return requestSpace(cloudFoundryClient, spaceId).map(ResourceUtils::getEntity).map(SpaceEntity::getName);
  }
  ...
  private static Mono<AssociateRouteApplicationResponse> requestAssociateRouteApplication(CloudFoundryClient cloudFoundryClient, String applicationId, String routeId) {
    return cloudFoundryClient.routes().associateApplication(AssociateRouteApplicationRequest.builder().routeId(routeId).applicationId(applicationId).build());
  }
  ...
  private static Mono<AssociateSpaceDeveloperResponse> requestAssociateSpaceDeveloper(CloudFoundryClient cloudFoundryClient, String spaceId, String developerId) {
    return cloudFoundryClient.spaces().associateDeveloper(AssociateSpaceDeveloperRequest.builder().spaceId(spaceId).developerId(developerId).build());
  }
  ...
  private static Mono<CreateDomainResponse> requestCreateDomain(CloudFoundryClient cloudFoundryClient, String organizationId, String domainName) {
    return cloudFoundryClient.domains().create(CreateDomainRequest.builder().name(domainName).owningOrganizationId(organizationId).wildcard(true).build());
  }
  ...
  private static Mono<CreateRouteResponse> requestCreateRoute(CloudFoundryClient cloudFoundryClient, String domainId, String spaceId) {
    return cloudFoundryClient.routes().create(CreateRouteRequest.builder().domainId(domainId).spaceId(spaceId).build());
  }

  private static Mono<DeleteDomainResponse> requestDeleteDomain(CloudFoundryClient cloudFoundryClient, String domainId) {
    return cloudFoundryClient.domains().delete(DeleteDomainRequest.builder().async(true).domainId(domainId).build());
  }
  ...
  private static Mono<DeleteDomainResponse> requestDeleteDomainAsyncFalse(CloudFoundryClient cloudFoundryClient, String domainId) {
    return cloudFoundryClient.domains().delete(DeleteDomainRequest.builder().async(false).domainId(domainId).build());
  }

  private static Mono<GetDomainResponse> requestGetDomain(CloudFoundryClient cloudFoundryClient, String domainId) {
    return cloudFoundryClient.domains().get(GetDomainRequest.builder().domainId(domainId).build());
  }

  private static Flux<SpaceResource> requestListDomainSpaces(CloudFoundryClient cloudFoundryClient, String domainId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().listSpaces(ListDomainSpacesRequest.builder().domainId(domainId).page(page).build()));
  }
  ...
  private static Flux<SpaceResource> requestListDomainSpacesByApplicationId(CloudFoundryClient cloudFoundryClient, String applicationId, String domainId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().listSpaces(ListDomainSpacesRequest.builder().page(page).applicationId(applicationId).domainId(domainId).build()));
  }
  ...
  private static Flux<SpaceResource> requestListDomainSpacesByOrganizationId(CloudFoundryClient cloudFoundryClient, String domainId, String organizationId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().listSpaces(ListDomainSpacesRequest.builder().domainId(domainId).organizationId(organizationId).page(page).build()));
  }
  ...
  private static Flux<SpaceResource> requestListDomainSpacesBySpaceName(CloudFoundryClient cloudFoundryClient, String domainId, String spaceName) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().listSpaces(ListDomainSpacesRequest.builder().domainId(domainId).name(spaceName).page(page).build()));
  }

  private static Flux<DomainResource> requestListDomains(CloudFoundryClient cloudFoundryClient, String domainName) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().list(ListDomainsRequest.builder().name(domainName).page(page).build()));
  }

  private static Flux<DomainResource> requestListDomains(CloudFoundryClient cloudFoundryClient) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().list(ListDomainsRequest.builder().page(page).build()));
  }
  ...
  private static Flux<DomainResource> requestListDomainsByOwningOrganization(CloudFoundryClient cloudFoundryClient, String organizationId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().list(ListDomainsRequest.builder().owningOrganizationId(organizationId).page(page).build()));
  }
  ...
  private static Flux<SpaceResource> requestListSpaceDevelopers(CloudFoundryClient cloudFoundryClient, String domainId, String userId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.domains().listSpaces(ListDomainSpacesRequest.builder().page(page).developerId(userId).domainId(domainId).build()));
  }
