...
public final class ServiceKeysTest extends AbstractIntegrationTest {
  ...
  @SuppressWarnings(value = { "unchecked" }) @Test public void create() {
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
    String serviceKeyName = this.nameFactory.getServiceKeyName();
    Mono.zip(this.serviceBrokerId, this.spaceId).
<<<<<<< /usr/src/app/output/cloudfoundry/cf-java-client/fcc8e8fb113e3caadf856f92b566e5fef15ee52c/integration-test/src/test/java/org/cloudfoundry/client/v2/ServiceKeysTest.java/left.java
...
>>>>>>> /usr/src/app/output/cloudfoundry/cf-java-client/fcc8e8fb113e3caadf856f92b566e5fef15ee52c/integration-test/src/test/java/org/cloudfoundry/client/v2/ServiceKeysTest.java/right.java
  ...
  private static Mono<String> createServiceInstanceId(CloudFoundryClient cloudFoundryClient, String serviceBrokerId, String serviceInstanceName, String serviceName, String spaceId) {
    return getPlanId(cloudFoundryClient, serviceBrokerId, serviceName).flatMap((planId) -> requestCreateServiceInstance(cloudFoundryClient, planId, serviceInstanceName, spaceId)).map(ResourceUtils::getId);
  }
  ...
  private static Mono<CreateServiceInstanceResponse> requestCreateServiceInstance(CloudFoundryClient cloudFoundryClient, String planId, String serviceInstanceName, String spaceId) {
    return cloudFoundryClient.serviceInstances().create(CreateServiceInstanceRequest.builder().name(serviceInstanceName).servicePlanId(planId).spaceId(spaceId).build());
  }
  ...
  private static Mono<GetServiceKeyResponse> requestGetServiceKey(CloudFoundryClient cloudFoundryClient, String serviceKeyId) {
    return cloudFoundryClient.serviceKeys().get(GetServiceKeyRequest.builder().serviceKeyId(serviceKeyId).build());
  }

  private static Flux<ServiceKeyResource> requestListServiceKeys(CloudFoundryClient cloudFoundryClient) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.serviceKeys().list(ListServiceKeysRequest.builder().page(page).build()));
  }
  ...
  private static Flux<ServiceResource> requestListServices(CloudFoundryClient cloudFoundryClient, String serviceBrokerId, String serviceName) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.services().list(ListServicesRequest.builder().page(page).label(serviceName).serviceBrokerId(serviceBrokerId).build()));
  }
}