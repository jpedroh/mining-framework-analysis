...
public final class SharedDomainsTest extends AbstractIntegrationTest {
  ...
  @Test public void listFilterByName() {
    String domainName = this.nameFactory.getDomainName();
    getSharedDomainId(this.cloudFoundryClient, domainName).flatMap((sharedDomainId) -> Mono.zip(Mono.just(sharedDomainId), requestListSharedDomains(this.cloudFoundryClient, domainName).single().map(ResourceUtils::getId))).as(StepVerifier::create).consumeNextWith(tupleEquality()).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  private static Mono<GetSharedDomainResponse> getSharedDomainResource(CloudFoundryClient cloudFoundryClient, String sharedDomainId) {
    return cloudFoundryClient.sharedDomains().get(GetSharedDomainRequest.builder().sharedDomainId(sharedDomainId).build());
  }
  ...
  private static Mono<CreateSharedDomainResponse> requestCreateSharedDomain(CloudFoundryClient cloudFoundryClient, String sharedDomainName) {
    return cloudFoundryClient.sharedDomains().create(CreateSharedDomainRequest.builder().name(sharedDomainName).build());
  }
  ...
  private static Flux<SharedDomainResource> requestListSharedDomains(CloudFoundryClient cloudFoundryClient, String sharedDomainName) {
    ...
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.sharedDomains().list(requestBuilder.page(page).build()));
  }
}