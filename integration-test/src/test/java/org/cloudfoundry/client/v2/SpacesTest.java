...
public final class SpacesTest extends AbstractIntegrationTest {
  ...
  @Test public void listSecurityGroups() {
    String securityGroupName = this.nameFactory.getSecurityGroupName();
    String spaceName = this.nameFactory.getSpaceName();
    this.organizationId.flatMap((organizationId) -> Mono.zip(createSecurityGroupId(this.cloudFoundryClient, securityGroupName), createSpaceId(this.cloudFoundryClient, organizationId, spaceName))).flatMap(function((securityGroupId, spaceId) -> requestAssociateSpaceSecurityGroup(this.cloudFoundryClient, spaceId, securityGroupId).then(Mono.just(spaceId)))).flatMapMany((spaceId) -> PaginationUtils.requestClientV2Resources((page) -> this.cloudFoundryClient.spaces().listSecurityGroups(ListSpaceSecurityGroupsRequest.builder().page(page).spaceId(spaceId).build()))).filter((response) -> securityGroupName.equals(response.getEntity().getName())).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  @Ignore(value = "Await https://github.com/cloudfoundry/cloud_controller_ng/issues/855 to decide whether this parameter should exist") @Test public void listServiceInstancesFilterByOrganizationId() {
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
    String spaceName = this.nameFactory.getSpaceName();
    this.organizationId.flatMap((organizationId) -> Mono.zip(Mono.just(organizationId), this.serviceBrokerId, createSpaceId(this.cloudFoundryClient, organizationId, spaceName))).delayUntil(function((organizationId, serviceBrokerId, spaceId) -> createServiceInstanceId(this.cloudFoundryClient, serviceBrokerId, serviceInstanceName, spaceId))).flatMapMany(function((organizationId, ignore, spaceId) -> requestListServiceInstances(this.cloudFoundryClient, spaceId, (builder) -> builder.organizationId(organizationId)))).filter((resource) -> serviceInstanceName.equals(ResourceUtils.getEntity(resource).getName())).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  private static Mono<String> createServiceInstanceId(CloudFoundryClient cloudFoundryClient, String serviceBrokerId, String serviceInstanceName, String spaceId) {
    return getServicePlanId(cloudFoundryClient, serviceBrokerId).flatMap((planId) -> requestCreateServiceInstance(cloudFoundryClient, planId, serviceInstanceName, spaceId)).map(ResourceUtils::getId);
  }
  ...
  private static Mono<String> getServicePlanId(CloudFoundryClient cloudFoundryClient, String serviceBrokerId) {
    return requestListServices(cloudFoundryClient, serviceBrokerId).single().map(ResourceUtils::getId).flatMapMany((serviceId) -> requestListServicePlans(cloudFoundryClient, serviceId)).single().map(ResourceUtils::getId);
  }
  ...
  private static Flux<ServiceResource> requestListServices(CloudFoundryClient cloudFoundryClient, String serviceBrokerId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.services().list(ListServicesRequest.builder().page(page).serviceBrokerId(serviceBrokerId).build()));
  }
