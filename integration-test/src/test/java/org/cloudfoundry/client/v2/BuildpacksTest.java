...
public final class BuildpacksTest extends AbstractIntegrationTest {
  ...
  @Test public void uploadDirectory() throws IOException {
  ...
  private static Mono<BuildpackEntity> getBuildpackEntity(CloudFoundryClient cloudFoundryClient, String buildpackName) {
    return requestListBuildpacks(cloudFoundryClient, buildpackName).map(ResourceUtils::getEntity).singleOrEmpty();
  }
  ...
  private static Mono<CreateBuildpackResponse> requestCreateBuildpack(CloudFoundryClient cloudFoundryClient, String buildpackName) {
    return cloudFoundryClient.buildpacks().create(CreateBuildpackRequest.builder().enabled(false).locked(false).name(buildpackName).position(3).build());
  }

  private static Flux<BuildpackResource> requestListBuildpacks(CloudFoundryClient cloudFoundryClient, String buildpackName) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.buildpacks().list(ListBuildpacksRequest.builder().name(buildpackName).build()));
  }
}