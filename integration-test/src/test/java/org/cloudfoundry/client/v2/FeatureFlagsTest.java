...
public final class FeatureFlagsTest extends AbstractIntegrationTest {
  private static final List<String> coreFeatureFlagNameList = Arrays.asList("app_bits_upload", "app_scaling", "diego_docker", "private_domain_creation", "route_creation", "service_instance_creation", "set_roles_by_username", "unset_roles_by_username", "user_org_creation");

  @Autowired private CloudFoundryClient cloudFoundryClient;

  @Test public void getEach() {
    Flux.fromIterable(coreFeatureFlagNameList).flatMap((flagName) -> this.cloudFoundryClient.featureFlags().get(GetFeatureFlagRequest.builder().name(flagName).build()).map((response) -> Tuples.of(flagName, response))).collectList().as(StepVerifier::create).consumeNextWith((list) -> list.forEach(consumer((name, entity) -> assertThat(entity.getName()).isEqualTo(name)))).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void list() {
    this.cloudFoundryClient.featureFlags().list(ListFeatureFlagsRequest.builder().build()).as(StepVerifier::create).consumeNextWith((response) -> {
  ...
  @Test public void setAndResetEach() {
