...
public final class ServiceBindingsTest extends AbstractIntegrationTest {
  ...
  @Test public void create() {
    ...
    createServiceInstanceAndApplicationIds(this.spaceId, this.cloudFoundryClient, serviceInstanceName, applicationName).flatMap(function((serviceInstanceId, applicationId) -> Mono.zip(Mono.just(serviceInstanceId), Mono.just(applicationId), requestCreateServiceBinding(this.cloudFoundryClient, applicationId, serviceInstanceId)))).as(StepVerifier::create).consumeNextWith(serviceBindingEquality()).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void delete() {
    String applicationName = this.nameFactory.getApplicationName();
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
    createServiceInstanceAndApplicationIds(this.spaceId, this.cloudFoundryClient, serviceInstanceName, applicationName).flatMap(function((serviceInstanceId, applicationId) -> Mono.zip(Mono.just(serviceInstanceId), Mono.just(applicationId), createServiceBindingId(this.cloudFoundryClient, applicationId, serviceInstanceId)))).delayUntil(function((serviceInstanceId, applicationId, serviceBindingId) -> deleteServiceBinding(this.cloudFoundryClient, serviceBindingId))).flatMap(function((serviceInstanceId, applicationId, serviceBindingId) -> requestGetServiceBinding(this.cloudFoundryClient, serviceBindingId))).as(StepVerifier::create).consumeErrorWith((t) -> assertThat(t).isInstanceOf(ClientV2Exception.class).hasMessageMatching("CF-ServiceBindingNotFound\\([0-9]+\\): The service binding could not be found: .*")).verify(Duration.ofMinutes(5));
  }
  ...
  private static Mono<String> createApplicationId(CloudFoundryClient cloudFoundryClient, String spaceId, String applicationName) {
    return requestCreateApplication(cloudFoundryClient, spaceId, applicationName, null, null, null, null).map(ResourceUtils::getId);
  }
  ...
  private static Mono<Tuple2<String, String>> createServiceInstanceAndApplicationIds(Mono<String> spaceId, CloudFoundryClient cloudFoundryClient, String serviceInstance, String application) {
    return spaceId.flatMap((spaceId1) -> Mono.zip(createUserServiceInstanceId(cloudFoundryClient, spaceId1, serviceInstance), createApplicationId(cloudFoundryClient, spaceId1, application)));
  }
  ...
  private static Mono<Void> deleteServiceBinding(CloudFoundryClient cloudFoundryClient, String serviceBindingId) {
    return requestDeleteServiceBinding(cloudFoundryClient, serviceBindingId).flatMap((job) -> JobUtils.waitForCompletion(cloudFoundryClient, Duration.ofMinutes(5), job));
  }
  ...
  private static Mono<DeleteServiceBindingResponse> requestDeleteServiceBinding(CloudFoundryClient cloudFoundryClient, String serviceBindingId) {
    return cloudFoundryClient.serviceBindingsV2().delete(DeleteServiceBindingRequest.builder().serviceBindingId(serviceBindingId).build());
  }
  ...
  private static Mono<GetServiceBindingResponse> requestGetServiceBinding(CloudFoundryClient cloudFoundryClient, String serviceBindingId) {
    return cloudFoundryClient.serviceBindingsV2().get(GetServiceBindingRequest.builder().serviceBindingId(serviceBindingId).build());
  }
  ...
  private static Flux<ServiceBindingResource> requestListServiceBindings(CloudFoundryClient cloudFoundryClient, String applicationId, String serviceInstanceId) {
    ...
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.serviceBindingsV2().list(builder.page(page).build()));
  }
