...
public final class ServiceBrokerUtils {
  public static Mono<ServiceBrokerUtils.ServiceBrokerMetadata> createServiceBroker(CloudFoundryClient cloudFoundryClient, NameFactory nameFactory, String planName, String serviceBrokerName, String serviceName, String spaceId, Boolean spaceScoped) {
    ...
    return getSharedDomain(cloudFoundryClient).flatMap((domain) -> pushServiceBrokerApplication(cloudFoundryClient, application, domain, nameFactory, planName, serviceName, spaceId)).
<<<<<<< /usr/src/app/output/cloudfoundry/cf-java-client/c656f6aa5f42563e62c3f226b05615e9e95c3087/integration-test/src/test/java/org/cloudfoundry/ServiceBrokerUtils.java/left.java
    flatMap((applicationMetadata) -> requestCreateServiceBroker(cloudFoundryClient, applicationMetadata, serviceBrokerName, spaceScoped).delayUntil((response) -> makeServicePlanPubliclyVisible(cloudFoundryClient, serviceName, spaceScoped)).map((response) -> new ServiceBrokerMetadata(applicationMetadata, ResourceUtils.getId(response))))
=======
    then((applicationMetadata) -> requestCreateServiceBroker(cloudFoundryClient, applicationMetadata, serviceBrokerName, spaceScoped).then((response) -> Mono.when(makeServicePlanPubliclyVisible(cloudFoundryClient, serviceName, spaceScoped), makeServicePlanPubliclyVisible(cloudFoundryClient, serviceName + "-shareable", spaceScoped)).then(Mono.just(response))).map((response) -> new ServiceBrokerMetadata(applicationMetadata, ResourceUtils.getId(response))))
>>>>>>> /usr/src/app/output/cloudfoundry/cf-java-client/c656f6aa5f42563e62c3f226b05615e9e95c3087/integration-test/src/test/java/org/cloudfoundry/ServiceBrokerUtils.java/right.java
  ...
  public static Mono<Void> deleteServiceBroker(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return cloudFoundryClient.applicationsV2().delete(DeleteApplicationRequest.builder().applicationId(applicationId).build());
  }
  ...
  private static Mono<String> createRouteId(CloudFoundryClient cloudFoundryClient, String domainId, String spaceId, String hostName) {
    return requestCreateRoute(cloudFoundryClient, domainId, spaceId, hostName).map(ResourceUtils::getId);
  }
  ...
  private static Mono<String> getApplicationInstanceState(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return requestApplicationInstances(cloudFoundryClient, applicationId).flatMapMany((response) -> Flux.fromIterable(response.getInstances().values())).single().map(ApplicationInstanceInfo::getState);
  }

  private static Mono<String> getApplicationPackageState(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return requestGetApplication(cloudFoundryClient, applicationId).map((response) -> ResourceUtils.getEntity(response).getPackageState());
  }
  ...
  private static Mono<String> getServicePlanId(CloudFoundryClient cloudFoundryClient, String serviceId) {
    return requestListServicePlans(cloudFoundryClient, serviceId).single().map(ResourceUtils::getId);
  }

  private static Mono<SharedDomainResource> getSharedDomain(CloudFoundryClient cloudFoundryClient) {
    return requestListSharedDomains(cloudFoundryClient).next();
  }
  ...
  private static Mono<CreateApplicationResponse> requestCreateApplication(CloudFoundryClient cloudFoundryClient, String spaceId, String applicationName) {
    return cloudFoundryClient.applicationsV2().create(CreateApplicationRequest.builder().buildpack("https://github.com/cloudfoundry/java-buildpack.git").memory(768).name(applicationName).spaceId(spaceId).build());
  }
  ...
  private static Mono<CreateRouteResponse> requestCreateRoute(CloudFoundryClient cloudFoundryClient, String domainId, String spaceId, String hostName) {
    return cloudFoundryClient.routes().create(CreateRouteRequest.builder().domainId(domainId).host(hostName).spaceId(spaceId).build());
  }

  private static Mono<CreateServiceBrokerResponse> requestCreateServiceBroker(CloudFoundryClient cloudFoundryClient, ApplicationMetadata applicationMetadata, String serviceBrokerName, Boolean spaceScoped) {
  ...
  private static Flux<ServicePlanResource> requestListServicePlans(CloudFoundryClient cloudFoundryClient, String serviceId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.servicePlans().list(ListServicePlansRequest.builder().serviceId(serviceId).page(page).build()));
  }

  private static Flux<ServiceResource> requestListServices(CloudFoundryClient cloudFoundryClient, String serviceName) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.services().list(ListServicesRequest.builder().label(serviceName).build()));
  }
  ...
  private static Mono<UpdateApplicationResponse> requestUpdateApplication(CloudFoundryClient cloudFoundryClient, String applicationId, String planName, String serviceName, String state) {
    return cloudFoundryClient.applicationsV2().update(UpdateApplicationRequest.builder().applicationId(applicationId).environmentJson("SERVICE_NAME", serviceName).environmentJson("PLAN_NAME", planName).state(state).build());
  }
  ...
  private static Mono<UpdateServicePlanResponse> requestUpdateServicePlan(CloudFoundryClient cloudFoundryClient, String planId, Boolean visibility) {
    return cloudFoundryClient.servicePlans().update(UpdateServicePlanRequest.builder().servicePlanId(planId).publiclyVisible(visibility).build());
  }
  ...
  private static Mono<UploadApplicationResponse> requestUploadApplication(CloudFoundryClient cloudFoundryClient, String applicationId, Path application) {
    return cloudFoundryClient.applicationsV2().upload(UploadApplicationRequest.builder().application(application).applicationId(applicationId).async(true).build());
  }
