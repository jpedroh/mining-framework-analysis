...
public final class ServiceAdminTest extends AbstractIntegrationTest {
  ...
  @Test public void disableServiceAccess() {
  ...
  @Test public void disableServiceAccessSpecifyAll() {
    String planName = this.nameFactory.getPlanName();
    String serviceBrokerName = this.nameFactory.getServiceBrokerName();
    String serviceName = this.nameFactory.getServiceName();
    String spaceName = this.nameFactory.getSpaceName();
    ServiceBrokerUtils.ServiceBrokerMetadata serviceBrokerMetadata = this.organizationId.flatMap((organizationId) -> createSpaceId(this.cloudFoundryClient, organizationId, spaceName)).flatMap((spaceId) -> createServiceBroker(this.cloudFoundryClient, this.nameFactory, planName, serviceBrokerName, serviceName, spaceId, false)).block(Duration.ofMinutes(5));
    resetToEnabled(this.cloudFoundryOperations, serviceName).then(this.cloudFoundryOperations.serviceAdmin().disableServiceAccess(DisableServiceAccessRequest.builder().organizationName(this.organizationName).servicePlanName(planName).serviceName(serviceName).build())).thenMany(requestListServiceAccessSettings(this.cloudFoundryOperations, serviceName)).filter((response) -> serviceName.equals(response.getServiceName())).as(StepVerifier::create).expectNext(ServiceAccess.builder().access(Access.ALL).brokerName(serviceBrokerName).organizationName().planName(planName).serviceName(serviceName).build()).expectComplete().verify(Duration.ofMinutes(5));
    deleteServiceBroker(this.cloudFoundryClient, serviceBrokerMetadata.applicationMetadata.applicationId).block(Duration.ofMinutes(5));
  }

  @Test public void disableServiceAccessSpecifyOrganization() {
    String planName = this.nameFactory.getPlanName();
    String serviceBrokerName = this.nameFactory.getServiceBrokerName();
    String serviceName = this.nameFactory.getServiceName();
    String spaceName = this.nameFactory.getSpaceName();
  ...
  @Test public void disableServiceAccessSpecifyServicePlan() {
    String planName = this.nameFactory.getPlanName();
    String serviceBrokerName = this.nameFactory.getServiceBrokerName();
    String serviceName = this.nameFactory.getServiceName();
    String spaceName = this.nameFactory.getSpaceName();
  ...
  @Test public void enableServiceAccessSpecifyServicePlan() {
    String planName = this.nameFactory.getPlanName();
    String serviceBrokerName = this.nameFactory.getServiceBrokerName();
    String serviceName = this.nameFactory.getServiceName();
    String spaceName = this.nameFactory.getSpaceName();
  ...
  @Test public void listServiceAccessSettings() {
    resetToEnabled(this.cloudFoundryOperations, this.serviceName).thenMany(this.cloudFoundryOperations.serviceAdmin().listServiceAccessSettings(ListServiceAccessSettingsRequest.builder().build())).filter((response) -> this.serviceName.equals(response.getServiceName())).as(StepVerifier::create).expectNext(ServiceAccess.builder().access(Access.ALL).brokerName(this.serviceBrokerName).organizationName().planName(this.planName).serviceName(this.serviceName).build()).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void listServiceAccessSettingsSpecifyBroker() {
    resetToEnabled(this.cloudFoundryOperations, this.serviceName).thenMany(this.cloudFoundryOperations.serviceAdmin().listServiceAccessSettings(ListServiceAccessSettingsRequest.builder().brokerName(this.serviceBrokerName).build())).single().as(StepVerifier::create).expectNext(ServiceAccess.builder().access(Access.ALL).brokerName(this.serviceBrokerName).organizationName().planName(this.planName).serviceName(this.serviceName).build()).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void listServiceAccessSettingsSpecifyOrganization() {
  ...
  @Test public void listServiceAccessSettingsSpecifyService() {
  ...
  private static Flux<ServiceAccess> requestListServiceAccessSettings(CloudFoundryOperations cloudFoundryOperations, String serviceName) {
    return cloudFoundryOperations.serviceAdmin().listServiceAccessSettings(ListServiceAccessSettingsRequest.builder().serviceName(serviceName).build());
  }

  private static Mono<Void> resetToDisabled(CloudFoundryOperations cloudFoundryOperations, String serviceName) {
    return cloudFoundryOperations.serviceAdmin().disableServiceAccess(DisableServiceAccessRequest.builder().serviceName(serviceName).build());
  }

  private static Mono<Void> resetToEnabled(CloudFoundryOperations cloudFoundryOperations, String serviceName) {
    return cloudFoundryOperations.serviceAdmin().enableServiceAccess(EnableServiceAccessRequest.builder().serviceName(serviceName).build());
  }
}