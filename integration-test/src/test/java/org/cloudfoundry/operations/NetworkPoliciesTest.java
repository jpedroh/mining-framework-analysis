...
public final class NetworkPoliciesTest extends AbstractIntegrationTest {
  ...
  @IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_1_12) @Test public void add() throws TimeoutException, InterruptedException {
    ...
    this.spaceId.flatMapMany((spaceId) -> Mono.zip(createApplicationId(this.cloudFoundryClient, destinationApplicationName, spaceId), createApplicationId(this.cloudFoundryClient, sourceApplicationName, spaceId))).thenMany(this.cloudFoundryOperations.networkPolicies().add(AddNetworkPolicyRequest.builder().destination(destinationApplicationName).source(sourceApplicationName).startPort(port).build())).thenMany(requestListNetworkPolicies(this.cloudFoundryOperations, sourceApplicationName)).map(Policy::getDestination).as(StepVerifier::create).expectNext(destinationApplicationName).expectComplete().verify(Duration.ofMinutes(5));
  }

  @IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_1_12) @Test public void list() throws TimeoutException, InterruptedException {
    String destinationApplicationName = this.nameFactory.getApplicationName();
    String sourceApplicationName = this.nameFactory.getApplicationName();
    Integer port = this.nameFactory.getPort();
    this.spaceId.flatMapMany((spaceId) -> Mono.zip(createApplicationId(this.cloudFoundryClient, destinationApplicationName, spaceId), createApplicationId(this.cloudFoundryClient, sourceApplicationName, spaceId))).thenMany(requestAddNetworkPolicy(this.cloudFoundryOperations, destinationApplicationName, port, sourceApplicationName)).thenMany(this.cloudFoundryOperations.networkPolicies().list(ListNetworkPoliciesRequest.builder().source(sourceApplicationName).build())).map(Policy::getStartPort).as(StepVerifier::create).expectNext(port).expectComplete().verify(Duration.ofMinutes(5));
  }

  @IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_1_12) @Test public void remove() throws TimeoutException, InterruptedException {
    String destinationApplicationName = this.nameFactory.getApplicationName();
    String sourceApplicationName = this.nameFactory.getApplicationName();
    Integer port = this.nameFactory.getPort();
  ...
  private static Flux<Void> requestAddNetworkPolicy(CloudFoundryOperations cloudFoundryOperations, String destinationApplicationName, Integer port, String sourceApplicationName) {
    return cloudFoundryOperations.networkPolicies().add(AddNetworkPolicyRequest.builder().destination(destinationApplicationName).startPort(port).source(sourceApplicationName).build());
  }
  ...
  private static Flux<Policy> requestListNetworkPolicies(CloudFoundryOperations cloudFoundryOperations, String sourceApplicationName) {
    return cloudFoundryOperations.networkPolicies().list(ListNetworkPoliciesRequest.builder().source(sourceApplicationName).build());
  }
}