...
public final class ServicesTest extends AbstractIntegrationTest {
  ...
  @Test public void bindRoutePrivateDomain() {
  ...
  @Test public void getServiceKey() {
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
    String serviceKeyName = this.nameFactory.getServiceKeyName();
  ...
  @Test public void listServiceKey() {
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
    String serviceKeyName = this.nameFactory.getServiceKeyName();
  ...
  @Test public void listServiceOfferings() {
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
  ...
  @Test public void unbindService() throws IOException {
    String applicationName = this.nameFactory.getApplicationName();
    String serviceInstanceName = this.nameFactory.getServiceInstanceName();
  ...
  private static Mono<Void> requestBindRoute(CloudFoundryOperations cloudFoundryOperations, String domainName, String hostName, String path, String userProvidedServiceInstanceName) {
    return cloudFoundryOperations.services().bindRoute(BindRouteServiceInstanceRequest.builder().domainName(domainName).hostname(hostName).path(path).serviceInstanceName(userProvidedServiceInstanceName).build());
  }
  ...
  private static Mono<Void> requestBindService(CloudFoundryOperations cloudFoundryOperations, String applicationName, String serviceInstanceName) {
    return cloudFoundryOperations.services().bind(BindServiceInstanceRequest.builder().applicationName(applicationName).serviceInstanceName(serviceInstanceName).build());
  }
  ...
  private static Mono<Void> requestCreateApplication(CloudFoundryOperations cloudFoundryOperations, Path application, String name) {
    return cloudFoundryOperations.applications().push(PushApplicationRequest.builder().path(application).buildpack("staticfile_buildpack").diskQuota(512).healthCheckType(ApplicationHealthCheck.PORT).memory(64).name(name).noStart(true).build());
  }
  ...
  private static Mono<Void> requestCreateServiceInstance(CloudFoundryOperations cloudFoundryOperations, String planName, String serviceInstanceName, String serviceName) {
    return cloudFoundryOperations.services().createInstance(CreateServiceInstanceRequest.builder().planName(planName).serviceName(serviceName).serviceInstanceName(serviceInstanceName).build());
  }
  ...
  private static Mono<Void> requestCreateServiceKey(CloudFoundryOperations cloudFoundryOperations, String serviceKeyName, String serviceInstanceName) {
    return cloudFoundryOperations.services().createServiceKey(CreateServiceKeyRequest.builder().serviceInstanceName(serviceInstanceName).serviceKeyName(serviceKeyName).build());
  }
  ...
  private static Mono<Void> requestCreateUserProvidedServiceInstance(CloudFoundryOperations cloudFoundryOperations, String userProvidedServiceInstanceName) {
    return cloudFoundryOperations.services().createUserProvidedInstance(CreateUserProvidedServiceInstanceRequest.builder().name(userProvidedServiceInstanceName).routeServiceUrl("https://test.route.service.url").build());
  }
  ...
  private static Mono<ApplicationDetail> requestGetApplication(CloudFoundryOperations cloudFoundryOperations, String applicationName) {
    return cloudFoundryOperations.applications().get(GetApplicationRequest.builder().name(applicationName).build());
  }
  ...
  private static Mono<ServiceInstance> requestGetInstance(CloudFoundryOperations cloudFoundryOperations, String userProvidedServiceInstanceName) {
    return cloudFoundryOperations.services().getInstance(GetServiceInstanceRequest.builder().name(userProvidedServiceInstanceName).build());
  }
  ...
  private static Mono<ServiceKey> requestGetServiceKey(CloudFoundryOperations cloudFoundryOperations, String serviceInstanceName, String serviceKeyName) {
    return cloudFoundryOperations.services().getServiceKey(GetServiceKeyRequest.builder().serviceInstanceName(serviceInstanceName).serviceKeyName(serviceKeyName).build());
  }
  ...
  private static Flux<ServiceBindingResource> requestListServiceBindings(CloudFoundryClient cloudFoundryClient, String applicationId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.serviceBindingsV2().list(ListServiceBindingsRequest.builder().applicationId(applicationId).page(page).build()));
  }
  ...
  private static Flux<UserProvidedServiceInstanceResource> requestListUserProvidedServiceInstances(CloudFoundryClient cloudFoundryClient, String serviceInstanceName) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.userProvidedServiceInstances().list(ListUserProvidedServiceInstancesRequest.builder().name(serviceInstanceName).page(page).build()));
  }
}