...
public final class GroupsTest extends AbstractIntegrationTest {
  ...
  @Test public void addMemberUser() {
    String displayName = this.nameFactory.getGroupName();
    String userName = this.nameFactory.getUserName();
    Mono.zip(createGroupId(this.uaaClient, displayName), createUserId(this.uaaClient, userName)).flatMap(function((groupId, userId) -> Mono.zip(this.uaaClient.groups().addMember(AddMemberRequest.builder().groupId(groupId).origin(userName + "-origin").type(MemberType.USER).memberId(userId).build()), Mono.just(userId)))).as(StepVerifier::create).consumeNextWith(consumer((response, userId) -> {
  ...
  @Test public void checkMembership() {
    String displayName = this.nameFactory.getGroupName();
    String userName = this.nameFactory.getUserName();
    createUserId(this.uaaClient, userName).flatMap((userId) -> Mono.zip(createGroupIdWithMember(this.uaaClient, displayName, userId), Mono.just(userId))).flatMap(function((groupId, userId) -> Mono.zip(this.uaaClient.groups().checkMembership(CheckMembershipRequest.builder().groupId(groupId).memberId(userId).build()), Mono.just(userId)))).as(StepVerifier::create).consumeNextWith(consumer((response, userId) -> {
      assertThat(response.getMemberId()).isEqualTo(userId);
  ...
  @Test public void create() {
    String displayName = this.nameFactory.getGroupName();
    String userName = this.nameFactory.getUserName();
    createUserId(this.uaaClient, userName).flatMap((userId) -> this.uaaClient.groups().create(CreateGroupRequest.builder().displayName(displayName).member(MemberSummary.builder().memberId(userId).build()).build())).then(requestList(this.uaaClient).filter((resource) -> displayName.equals(resource.getDisplayName())).single()).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void delete() {
    String displayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, displayName).flatMap((groupId) -> this.uaaClient.groups().delete(DeleteGroupRequest.builder().groupId(groupId).build())).map(DeleteGroupResponse::getDisplayName).as(StepVerifier::create).expectNext(displayName).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void get() {
    String displayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, displayName).flatMap((groupId) -> this.uaaClient.groups().get(GetGroupRequest.builder().groupId(groupId).build()).map(GetGroupResponse::getDisplayName)).as(StepVerifier::create).expectNext(displayName).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void list() {
    String displayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, displayName).flatMap((groupId) -> PaginationUtils.requestUaaResources((startIndex) -> this.uaaClient.groups().list(ListGroupsRequest.builder().filter(String.format("id eq \"%s\"", groupId)).startIndex(startIndex).build())).single()).map(Group::getDisplayName).as(StepVerifier::create).expectNext(displayName).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void listExternalGroupMappings() {
    String displayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, displayName).flatMap((groupId) -> requestMapExternalGroupResponse(this.uaaClient, displayName, groupId)).thenMany(PaginationUtils.requestUaaResources((startIndex) -> this.uaaClient.groups().listExternalGroupMappings(ListExternalGroupMappingsRequest.builder().startIndex(startIndex).build())).filter((group) -> displayName.equals(group.getDisplayName()))).map(ExternalGroupResource::getExternalGroup).as(StepVerifier::create).expectNext(displayName + "-external-group").expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void listMembers() {
    String displayName = this.nameFactory.getGroupName();
    String userName = this.nameFactory.getUserName();
    createUserId(this.uaaClient, userName).flatMap((userId) -> Mono.zip(createGroupIdWithMember(this.uaaClient, displayName, userId), Mono.just(userId))).flatMap(function((groupId, userId) -> Mono.zip(this.uaaClient.groups().listMembers(ListMembersRequest.builder().groupId(groupId).returnEntities(false).build()).flatMapIterable(ListMembersResponse::getMembers).map(Member::getMemberId).single(), Mono.just(userId)))).as(StepVerifier::create).consumeNextWith(tupleEquality()).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void listMembersWithEntity() {
    String displayName = this.nameFactory.getGroupName();
    String userName = this.nameFactory.getUserName();
    createUserId(this.uaaClient, userName).flatMap((userId) -> createGroupIdWithMember(this.uaaClient, displayName, userId)).flatMapMany((groupId) -> this.uaaClient.groups().listMembers(ListMembersRequest.builder().groupId(groupId).returnEntities(true).build())).flatMapIterable(ListMembersResponse::getMembers).map(Member::getEntity).map(Optional::get).cast(UserEntity.class).single().map(UserEntity::getUserName).as(StepVerifier::create).expectNext(userName).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void mapExternalGroupMappings() {
    String displayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, displayName).flatMap((groupId) -> this.uaaClient.groups().mapExternalGroup(MapExternalGroupRequest.builder().externalGroup(displayName + "-external-group").groupId(groupId).origin(displayName + "-origin").build())).thenMany(requestListExternalGroupResource(this.uaaClient).filter((group) -> displayName.equals(group.getDisplayName()))).as(StepVerifier::create).consumeNextWith((resource) -> {
  ...
  @Test public void removeMember() {
    String displayName = this.nameFactory.getGroupName();
    String userName = this.nameFactory.getUserName();
    createUserId(this.uaaClient, userName).flatMap((userId) -> Mono.zip(createGroupIdWithMember(this.uaaClient, displayName, userId), Mono.just(userId))).flatMap(function((groupId, userId) -> Mono.zip(this.uaaClient.groups().removeMember(RemoveMemberRequest.builder().groupId(groupId).memberId(userId).build()).map(RemoveMemberResponse::getMemberId), Mono.just(userId)))).as(StepVerifier::create).consumeNextWith(tupleEquality()).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void unmapExternalGroupMappingsByGroupDisplayName() {
    String displayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, displayName).flatMap((groupId) -> requestMapExternalGroupResponse(this.uaaClient, displayName, groupId)).then(this.uaaClient.groups().unmapExternalGroupByGroupDisplayName(UnmapExternalGroupByGroupDisplayNameRequest.builder().groupDisplayName(displayName).externalGroup(displayName + "-external-group").origin(displayName + "-origin").build())).thenMany(requestListExternalGroupResources(this.uaaClient)).filter((resource) -> displayName.equals(resource.getDisplayName())).as(StepVerifier::create).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void unmapExternalGroupMappingsByGroupId() {
    String displayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, displayName).delayUntil((groupId) -> requestMapExternalGroupResponse(this.uaaClient, displayName, groupId)).flatMap((groupId) -> this.uaaClient.groups().unmapExternalGroupByGroupId(UnmapExternalGroupByGroupIdRequest.builder().groupId(groupId).externalGroup(displayName + "-external-group").origin(displayName + "-origin").build())).thenMany(requestListExternalGroupResources(this.uaaClient).filter((resource) -> displayName.equals(resource.getDisplayName()))).as(StepVerifier::create).expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void update() {
    String baseDisplayName = this.nameFactory.getGroupName();
    String newDisplayName = this.nameFactory.getGroupName();
    createGroupId(this.uaaClient, baseDisplayName).flatMap((groupId) -> this.uaaClient.groups().update(UpdateGroupRequest.builder().displayName(newDisplayName).groupId(groupId).version("*").build())).then(requestList(this.uaaClient).filter((resource) -> newDisplayName.equals(resource.getDisplayName())).single()).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  private static Mono<String> createGroupIdWithMember(UaaClient uaaClient, String displayName, String userId) {
    return requestCreateGroupWithMember(uaaClient, displayName, userId).map(CreateGroupResponse::getId);
  }
  ...
  private static Mono<CreateGroupResponse> requestCreateGroup(UaaClient uaaClient, String displayName) {
    return uaaClient.groups().create(CreateGroupRequest.builder().displayName(displayName).build());
  }
  ...
  private static Mono<CreateGroupResponse> requestCreateGroupWithMember(UaaClient uaaClient, String displayName, String userId) {
    return uaaClient.groups().create(CreateGroupRequest.builder().displayName(displayName).member(MemberSummary.builder().memberId(userId).origin("test-origin").type(MemberType.USER).build()).build());
  }

  private static Mono<CreateUserResponse> requestCreateUser(UaaClient uaaClient, String userName) {
    return uaaClient.users().create(CreateUserRequest.builder().email(Email.builder().value("test-email").primary(true).build()).name(Name.builder().familyName("test-family-name").givenName("test-given-name").build()).password("test-password").userName(userName).build());
  }

  private static Flux<Group> requestList(UaaClient uaaClient) {
    return PaginationUtils.requestUaaResources((startIndex) -> uaaClient.groups().list(ListGroupsRequest.builder().startIndex(startIndex).build()));
  }

  private static Flux<ExternalGroupResource> requestListExternalGroupResource(UaaClient uaaClient) {
    return PaginationUtils.requestUaaResources((startIndex) -> uaaClient.groups().listExternalGroupMappings(ListExternalGroupMappingsRequest.builder().startIndex(startIndex).build()));
  }
  ...
  private static Mono<MapExternalGroupResponse> requestMapExternalGroupResponse(UaaClient uaaClient, String displayName, String groupId) {
    return uaaClient.groups().mapExternalGroup(MapExternalGroupRequest.builder().externalGroup(displayName + "-external-group").groupId(groupId).origin(displayName + "-origin").build());
  }
}