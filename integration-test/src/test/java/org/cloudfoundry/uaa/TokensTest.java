...
public final class TokensTest extends AbstractIntegrationTest {
  ...
  @Test public void checkTokenNotAuthorized() {
    this.tokenProvider.getToken(this.connectionContext).flatMap((token) -> this.uaaClient.tokens().check(CheckTokenRequest.builder().token(token).clientId(this.clientId).clientSecret(this.clientSecret).scopes("password.write", "scim.userids").build())).as(StepVerifier::create).consumeErrorWith((t) -> assertThat(t).isInstanceOf(UaaException.class).hasMessage("access_denied: Access is denied")).verify(Duration.ofMinutes(5));
  }
