...
public final class ClientsTest extends AbstractIntegrationTest {
  ...
  @Test public void batchChangeSecret() {
    ...
    requestCreateClient(this.uaaClient, clientId1, clientSecret).then(requestCreateClient(this.uaaClient, clientId2, clientSecret)).then(this.uaaClient.clients().batchChangeSecret(BatchChangeSecretRequest.builder().changeSecrets(ChangeSecret.builder().clientId(clientId1).oldSecret(clientSecret).secret(newClientSecret1).build(), ChangeSecret.builder().clientId(clientId2).oldSecret(clientSecret).secret(newClientSecret2).build()).build())).flatMapIterable(BatchChangeSecretResponse::getClients).as(StepVerifier::create).expectNextCount(2).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  @Test public void create() {
    String clientId = this.nameFactory.getClientId();
    String clientSecret = this.nameFactory.getClientSecret();
    this.uaaClient.clients().create(CreateClientRequest.builder().approvalsDeleted(true).authorizedGrantType(PASSWORD).clientId(clientId).clientSecret(clientSecret).scopes("client.read", "client.write").tokenSalt("test-token-salt").build()).as(StepVerifier::create).consumeNextWith((response) -> {
      assertThat(response.getAuthorizedGrantTypes()).containsExactly(PASSWORD, REFRESH_TOKEN);
