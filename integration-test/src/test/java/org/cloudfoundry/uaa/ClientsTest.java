...
public final class ClientsTest extends AbstractIntegrationTest {
  ...
  @Test public void list() {
    String clientId = this.nameFactory.getClientId();
    String clientSecret = this.nameFactory.getClientSecret();
    requestCreateClient(this.uaaClient, clientId, clientSecret).thenMany(PaginationUtils.requestUaaResources((startIndex) -> this.uaaClient.clients().list(ListClientsRequest.builder().startIndex(startIndex).build()))).filter((client) -> clientId.equals(client.getClientId())).as(StepVerifier::create).expectNextCount(1).expectComplete().verify(Duration.ofMinutes(5));
  }
