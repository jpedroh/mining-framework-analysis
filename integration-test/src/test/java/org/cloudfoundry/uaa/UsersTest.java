...
public final class UsersTest extends AbstractIntegrationTest {
  ...
  @Test public void changePassword() throws TimeoutException, InterruptedException {
  ...
  @IfCloudFoundryVersion(greaterThanOrEqualTo = CloudFoundryVersion.PCF_1_10) @Test public void expirePassword() throws TimeoutException, InterruptedException {
    String userName = this.nameFactory.getUserName();
  ...
  @IfCloudFoundryVersion(equalTo = CloudFoundryVersion.PCF_1_9) @Test public void expirePassword19() throws TimeoutException, InterruptedException {
    String userName = this.nameFactory.getUserName();
  ...
  @Test public void getVerificationLink() throws TimeoutException, InterruptedException {
    String userName = this.nameFactory.getUserName();
  ...
  @Test public void invite() throws TimeoutException, InterruptedException {
    this.uaaClient.users().invite(InviteUsersRequest.builder().email("test@email.address").redirectUri("test-redirect-uri").build()).flatMapIterable(InviteUsersResponse::getNewInvites).single().as(StepVerifier::create).consumeNextWith((invite) -> {
  ...
  @Test public void lookup() throws TimeoutException, InterruptedException {
    String userName = this.nameFactory.getUserName();
  ...
  @Test public void userInfo() throws TimeoutException, InterruptedException {
    this.uaaClient.users().userInfo(UserInfoRequest.builder().build()).map(UserInfoResponse::getName).as(StepVerifier::create).expectNext("Test User").expectComplete().verify(Duration.ofMinutes(5));
  }

  @Test public void verifyUser() throws TimeoutException, InterruptedException {
    String userName = this.nameFactory.getUserName();
  ...
  private static Mono<CreateUserResponse> requestCreateUser(UaaClient uaaClient, String userName) {
    return uaaClient.users().create(CreateUserRequest.builder().email(Email.builder().value("test-email").primary(true).build()).name(Name.builder().familyName("test-family-name").givenName("test-given-name").build()).password("test-password").verified(false).userName(userName).build());
  }

  private static Mono<ListUsersResponse> requestListUsers(UaaClient uaaClient, String userId) {
    return uaaClient.users().list(ListUsersRequest.builder().filter(String.format("id eq \"%s\"", userId)).build());
  }
}