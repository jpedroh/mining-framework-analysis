...
public final class TcpRoutesTest extends AbstractIntegrationTest {
  ...
  @Autowired private NameFactory nameFactory;
  ...
  @Test public void create() {
    ...
    getRouterGroupId(this.routingClient, DEFAULT_ROUTER_GROUP).flatMap((routerGroupId) -> this.routingClient.tcpRoutes().create(CreateTcpRoutesRequest.builder().tcpRoute(TcpRouteConfiguration.builder().backendIp(backendIp).backendPort(backendPort).port(port).routerGroupId(routerGroupId).ttl(59).build()).build())).then(requestListTcpRoutes(this.routingClient)).flatMapIterable(ListTcpRoutesResponse::getTcpRoutes).filter((route) -> backendIp.equals(route.getBackendIp())).filter((route) -> backendPort.equals(route.getBackendPort())).map(TcpRoute::getPort).as(StepVerifier::create).expectNext(port).expectComplete().verify(Duration.ofMinutes(5));
  }
  ...
  private static Mono<CreateTcpRoutesResponse> createTcpRoute(RoutingClient routingClient, String backendIp, Integer backendPort, Integer port, String routerGroupId) {
    return routingClient.tcpRoutes().create(CreateTcpRoutesRequest.builder().tcpRoute(TcpRouteConfiguration.builder().backendIp(backendIp).backendPort(backendPort).port(port).routerGroupId(routerGroupId).ttl(59).build()).build());
  }
