...
final class CloudFoundryCleaner {
  ...
  private static Mono<UpdateSpaceResponse> removeSpaceMetadata(CloudFoundryClient cloudFoundryClient, SpaceResource space) {
    return cloudFoundryClient.spacesV3().get(GetSpaceRequest.builder().spaceId(ResourceUtils.getId(space)).build()).map(GetSpaceResponse::getMetadata).flatMap((metadata) -> {
      ...
      return requestUpdateSpace(cloudFoundryClient, annotations, labels, ResourceUtils.getId(space)).doOnError((t) -> LOGGER.error("Unable to remove metadata from {}", ResourceUtils.getEntity(space).getName()));
    });
  }
  ...
  private static Mono<UpdateSpaceResponse> requestUpdateSpace(CloudFoundryClient cloudFoundryClient, Map<String, String> annotations, Map<String, String> labels, String spaceId) {
    return cloudFoundryClient.spacesV3().update(UpdateSpaceRequest.builder().metadata(Metadata.builder().annotations(annotations).labels(labels).build()).spaceId(spaceId).build());
  }
}