...
final class CloudFoundryCleaner {
  ...
  void clean() {
    Flux.empty().thenMany(Mono.when(cleanServiceInstances(this.cloudFoundryClient, this.nameFactory), cleanUserProvidedServiceInstances(this.cloudFoundryClient, this.nameFactory))).thenMany(Mono.when(cleanBuildpacks(this.cloudFoundryClient, this.nameFactory), cleanClients(this.uaaClient, this.nameFactory), cleanFeatureFlags(this.cloudFoundryClient), cleanGroups(this.uaaClient, this.nameFactory), cleanIdentityProviders(this.uaaClient, this.nameFactory), cleanIdentityZones(this.uaaClient, this.nameFactory), cleanNetworkingPolicies(this.networkingClient, this.nameFactory, this.serverVersion), cleanRoutes(this.cloudFoundryClient, this.nameFactory), cleanSecurityGroups(this.cloudFoundryClient, this.nameFactory), cleanServiceBrokers(this.cloudFoundryClient, this.nameFactory), cleanSpaceQuotaDefinitions(this.cloudFoundryClient, this.nameFactory), cleanStacks(this.cloudFoundryClient, this.nameFactory), cleanUsers(this.cloudFoundryClient, this.nameFactory), cleanUsers(this.uaaClient, this.nameFactory))).thenMany(cleanApplicationsV2(this.cloudFoundryClient, this.nameFactory)).thenMany(Mono.when(cleanPrivateDomains(this.cloudFoundryClient, this.nameFactory), cleanSharedDomains(this.cloudFoundryClient, this.nameFactory), cleanSpaces(this.cloudFoundryClient, this.nameFactory))).thenMany(cleanOrganizations(this.cloudFoundryClient, this.nameFactory)).thenMany(cleanOrganizationQuotaDefinitions(this.cloudFoundryClient, this.nameFactory)).retry(5, (t) -> t instanceof SSLException).doOnSubscribe((s) -> LOGGER.debug(">> CLEANUP <<")).doOnComplete(() -> LOGGER.debug("<< CLEANUP >>")).then().block(Duration.ofMinutes(30));
  }
  ...
  private static Flux<Void> cleanUsers(CloudFoundryClient cloudFoundryClient, NameFactory nameFactory) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.users().list(org.cloudfoundry.client.v2.users.ListUsersRequest.builder().page(page).build())).filter((resource) -> isCleanable(nameFactory, resource)).map((resource) -> resource.getMetadata().getId()).flatMap((userId) -> cloudFoundryClient.users().delete(org.cloudfoundry.client.v2.users.DeleteUserRequest.builder().async(true).userId(userId).build()).doOnError((t) -> LOGGER.error("Unable to delete user {}", userId, t))).flatMap((job) -> JobUtils.waitForCompletion(cloudFoundryClient, Duration.ofMinutes(5), job));
  }
