...
final class CloudFoundryCleaner {
  ...
  void clean() {
    Flux.empty().thenMany(Mono.when(cleanServiceInstances(this.cloudFoundryClient, this.nameFactory, this.serverVersion), cleanUserProvidedServiceInstances(this.cloudFoundryClient, this.nameFactory))).thenMany(Mono.when(cleanBuildpacks(this.cloudFoundryClient, this.nameFactory), cleanClients(this.uaaClient, this.nameFactory), cleanFeatureFlags(this.cloudFoundryClient), cleanGroups(this.uaaClient, this.nameFactory), cleanIdentityProviders(this.uaaClient, this.nameFactory), cleanIdentityZones(this.uaaClient, this.nameFactory), cleanNetworkingPolicies(this.networkingClient, this.nameFactory, this.serverVersion), cleanRoutes(this.cloudFoundryClient, this.nameFactory), cleanSecurityGroups(this.cloudFoundryClient, this.nameFactory), cleanServiceBrokers(this.cloudFoundryClient, this.nameFactory), cleanSpaceQuotaDefinitions(this.cloudFoundryClient, this.nameFactory), cleanStacks(this.cloudFoundryClient, this.nameFactory), cleanUsers(this.cloudFoundryClient, this.nameFactory))).thenMany(Mono.when(cleanApplicationsV2(this.cloudFoundryClient, this.nameFactory), cleanUsers(this.uaaClient, this.nameFactory))).thenMany(Mono.when(cleanPrivateDomains(this.cloudFoundryClient, this.nameFactory), cleanSharedDomains(this.cloudFoundryClient, this.nameFactory), cleanSpaces(this.cloudFoundryClient, this.nameFactory))).thenMany(cleanOrganizations(this.cloudFoundryClient, this.nameFactory)).thenMany(cleanOrganizationQuotaDefinitions(this.cloudFoundryClient, this.nameFactory)).retry(5, (t) -> t instanceof SSLException).doOnSubscribe((s) -> LOGGER.debug(">> CLEANUP <<")).doOnComplete(() -> LOGGER.debug("<< CLEANUP >>")).then().block(Duration.ofMinutes(30));
  }
...
<<<<<<< /usr/src/app/output/cloudfoundry/cf-java-client/c656f6aa5f42563e62c3f226b05615e9e95c3087/integration-test/src/test/java/org/cloudfoundry/CloudFoundryCleaner.java/left.java
    delayUntil((serviceInstance) -> removeServiceKeys(cloudFoundryClient, serviceInstance)).flatMap((serviceInstance) -> cloudFoundryClient.serviceInstances().delete(DeleteServiceInstanceRequest.builder().async(true).serviceInstanceId(ResourceUtils.getId(serviceInstance)).build()).flatMap((response) -> {
...
=======
    flatMap((serviceInstance) -> removeServiceKeys(cloudFoundryClient, serviceInstance).thenMany(Flux.just(serviceInstance))).flatMap((serviceInstance) -> removeServiceShares(cloudFoundryClient, serviceInstance, serverVersion).thenMany(Flux.just(serviceInstance))).flatMap((serviceInstance) -> cloudFoundryClient.serviceInstances().delete(DeleteServiceInstanceRequest.builder().async(true).serviceInstanceId(ResourceUtils.getId(serviceInstance)).build()).then((response) -> {
      Object entity = response.getEntity();
      if (entity instanceof JobEntity) {
        return JobUtils.waitForCompletion(cloudFoundryClient, Duration.ofMinutes(5), (JobEntity) response.getEntity());
      } else {
...
>>>>>>> /usr/src/app/output/cloudfoundry/cf-java-client/c656f6aa5f42563e62c3f226b05615e9e95c3087/integration-test/src/test/java/org/cloudfoundry/CloudFoundryCleaner.java/right.java
  ...
  private static Flux<Void> removeApplicationServiceBindings(CloudFoundryClient cloudFoundryClient, ApplicationResource application) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.applicationsV2().listServiceBindings(ListApplicationServiceBindingsRequest.builder().page(page).applicationId(ResourceUtils.getId(application)).build())).flatMap((serviceBinding) -> requestRemoveServiceBinding(cloudFoundryClient, ResourceUtils.getId(application), ResourceUtils.getId(serviceBinding)).doOnError((t) -> LOGGER.error("Unable to remove service binding from {}", ResourceUtils.getEntity(application).getName(), t)));
  }
  ...
  private static Flux<Void> removeServiceShares(CloudFoundryClient cloudFoundryClient, ServiceInstanceResource serviceInstance, Version serverVersion) {
    return ifCfVersion(PCF_2_1, serverVersion, () -> cloudFoundryClient.serviceInstancesV3().listSharedSpacesRelationship(ListSharedSpacesRelationshipRequest.builder().serviceInstanceId(ResourceUtils.getId(serviceInstance)).build()).flatMapIterable(ListSharedSpacesRelationshipResponse::getData).map(Relationship::getId).flatMap((spaceId) -> requestUnshareServiceInstance(cloudFoundryClient, serviceInstance, spaceId).doOnError((t) -> LOGGER.error("Unable to remove service share from {}", ResourceUtils.getEntity(serviceInstance).getName(), t))));
  }
  ...
  private static Mono<Void> requestUnshareServiceInstance(CloudFoundryClient cloudFoundryClient, ServiceInstanceResource serviceInstance, String spaceId) {
    return cloudFoundryClient.serviceInstancesV3().unshare(UnshareServiceInstanceRequest.builder().serviceInstanceId(ResourceUtils.getId(serviceInstance)).spaceId(spaceId).build());
  }
}