...
public abstract class AbstractEmbedVaadinTomcat implements EmbedVaadinServer, Serializable {
  ...
  protected Context getContext() {
    if (context == null) {
      context = getTomcat().addContext(getConfig().getContextPath(), getConfig().getContextRootDirectory().getAbsolutePath());
    }
    return context;
  }
  ...
  protected <T extends VaadinServlet> Wrapper initializeVaadinServlet(T servlet) {
    final Wrapper wrapper = Tomcat.addServlet(getContext(), "vaadin", servlet);
