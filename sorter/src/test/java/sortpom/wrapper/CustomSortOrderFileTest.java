...
class CustomSortOrderFileTest {
  @Test void compareDefaultSortOrderFileToString() throws IOException, JDOMException {
  ...
  private String getToStringOnCustomSortOrderFile() throws IOException, JDOMException {
    PluginParameters pluginParameters = PluginParameters.builder().setPomFile(null).setFileOutput(false, ".bak", null, false).setEncoding("UTF-8").setFormatting("\r\n", true, true, true).setIndent(2, false, false).setSortOrder("src/test/resources/sortOrderFiles/with_newline_tags.xml", null).setSortEntities("scope,groupId,artifactId", "groupId,artifactId", true, true, true).build();
