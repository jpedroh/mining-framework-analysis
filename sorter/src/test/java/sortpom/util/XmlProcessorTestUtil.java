...
public class XmlProcessorTestUtil {
  ...
  private void setup(String inputFileName) throws Exception {
    PluginParameters pluginParameters = PluginParameters.builder().setPomFile(null).setFileOutput(false, ".bak", null, false).setEncoding("UTF-8").setFormatting(lineSeparator, expandEmptyElements, spaceBeforeCloseEmptyElement, keepBlankLines).setIndent(2, indentBlankLines, false).setSortOrder(predefinedSortOrder + ".xml", null).setSortEntities(
...
>>>>>>> /usr/src/app/output/ekryd/sortpom/bf7b1ecd4e2b72560fd8c05bcdd5cef33bf45f67/sorter/src/test/java/sortpom/util/XmlProcessorTestUtil.java/right.java
    , sortPlugins, sortProperties, sortModules, false).build();
    ...
    if (sortAlphabeticalOnly) {
      wrapperFactory = new WrapperFactory() {
        @Override public HierarchyRootWrapper createFromRootElement(final Element rootElement) {
          return new HierarchyRootWrapper(new AlphabeticalSortedWrapper(rootElement));
        }

        @SuppressWarnings(value = { "unchecked" }) @Override public <T extends Content> Wrapper<T> create(final T content) {
