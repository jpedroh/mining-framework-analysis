...
@RunWith(value = MockitoJUnitRunner.class) public class AbstractContextTest {
  @Mock private Session sessionCookie;

  @Mock private FlashScope flashCookie;

  @Mock private BodyParserEngineManager bodyParserEngineManager;

  @Mock private Route route;

  @Mock private Validation validation;

  @Mock private BodyParserEngine bodyParserEngine;

  @Mock private NinjaProperties ninjaProperties;

  private AbstractContextImpl abstractContext;

  @Before public void setUp() {
    abstractContext = new AbstractContextImpl(bodyParserEngineManager, flashCookie, ninjaProperties, sessionCookie, validation, null);
    abstractContext.init("", "/");
  }

  @Test public void getRemoteAddr() {
  ...
  @Test public void getRemoteAddrIgnoresXForwardHeader() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getRemoteAddrUsesXForwardHeader() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getRemoteAddrParsesXForwardedForIfMoreThanOneHostPresent() {
    AbstractContextImpl context = spy(abstractContext);
    when(ninjaProperties.getBooleanWithDefault(Context.NINJA_PROPERTIES_X_FORWARDED_FOR, false)).thenReturn(Boolean.TRUE);
    doReturn("1.1.1.1").when(context).getRealRemoteAddr();
  ...
  @Test public void getRemoteAddrUsesFallbackIfXForwardedForIsNotValidInetAddr() {
    AbstractContextImpl context = spy(abstractContext);
    when(ninjaProperties.getBooleanWithDefault(Context.NINJA_PROPERTIES_X_FORWARDED_FOR, false)).thenReturn(Boolean.TRUE);
    doReturn("1.1.1.1").when(context).getRealRemoteAddr();
  ...
  @Test public void addCookieViaResult() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void unsetCookieAddsCookieWithMaxAgeZero() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getPathParameter() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getPathParameterDecodingWorks() {
    AbstractContextImpl context = spy(abstractContext);
    Map<String, String> parameterMap = Maps.newHashMap();
  ...
  @Test public void getPathParameterAsInteger() {
    AbstractContextImpl context = spy(abstractContext);
    Map<String, String> parameterMap = Maps.newHashMap();
    parameterMap.put("parameter", "parameter");
    when(route.getPathParametersEncoded(Matchers.anyString())).thenReturn(parameterMap);
  ...
  @Test public void getParameterAsInteger() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getParameterAs() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void finalizeInAbstractContextSavesFlashSessionCookies() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getAcceptContentType() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getAcceptEncoding() {
    AbstractContextImpl context = spy(abstractContext);
  ...
  @Test public void getAcceptLanguage() {
    AbstractContextImpl context = spy(abstractContext);
    String language = "de";
  ...
  @Test public void getAcceptCharset() {
    AbstractContextImpl context = spy(abstractContext);
