...
public class RouteBuilderImpl implements RouteBuilder {
  ...
  @Inject public RouteBuilderImpl(NinjaProperties ninjaProperties) {
  ...
  @Override public void with(Class controllerClass, String controllerMethod) {
    this.functionalMethod = verifyControllerMethod(controllerClass, controllerMethod);
  }

  @Override @Deprecated public void with(MethodReference methodRef) {
    with(methodRef.getDeclaringClass(), methodRef.getMethodName());
  }

  @Override @Deprecated public void with(final Result result) {
    with(ControllerMethods.of(() -> result));
  }

  @Override public Void with(ControllerMethod controllerMethod) {
  ...
  @Override public RouteBuilder route(String uri) {
    this.uri = uri;
    return this;
  }
  ...
  /**
     ...
     */
  private Method verifyControllerMethod(Class<?> controllerClass, String controllerMethod) {
    try {
      ...
      log.error("Can not find Controller " + controllerClass.getName() + " and method " + controllerMethod);
  ...
  private FilterChain buildFilterChain(Injector injector, LinkedList<Class<? extends Filter>> filters) {
    if (filters.isEmpty()) {
      Provider<?> targetProvider = (targetObject.isPresent() ? Providers.of(targetObject.get()) : injector.getProvider(functionalMethod.getDeclaringClass()));
      ControllerMethodInvoker methodInvoker = ControllerMethodInvoker.build(functionalMethod, implementationMethod.orElse(functionalMethod), injector, ninjaProperties);
      return new FilterChainEnd(targetProvider, methodInvoker);
    } else {
      Class<? extends Filter> filter = filters.pop();
      return new FilterChainImpl(injector.getProvider(filter), buildFilterChain(injector, filters));
    }
  }
