...
class RouteBuilderImpl implements RouteBuilder {
  ...
  @Override public void with(Class controller, String controllerMethod) {
  ...
  public Route buildRoute(Injector injector) {
    ...
    if (controller != null) {
      if (controllerMethod == null) {
        throw new IllegalStateException(String.format("Route \'%s\' does not have a controller method", uri));
      }
    ...
    return new Route(httpMethod, uri, controller, controllerMethod, buildFilterChain(injector, filters, controller, controllerMethod));
  }

  private FilterChain buildFilterChain(Injector injector, LinkedList<Class<? extends Filter>> filters, Class<?> controller, Method controllerMethod) {
    if (filters.isEmpty()) {
      return new FilterChainEnd(injector.getProvider(controller), ControllerMethodInvoker.build(controllerMethod, injector));
    } else {
      Class<? extends Filter> filter = filters.pop();
      return new FilterChainImpl(injector.getProvider(filter), buildFilterChain(injector, filters, controller, controllerMethod));
    }
  }
