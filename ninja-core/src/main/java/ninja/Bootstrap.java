...
public class Bootstrap {
  ...
  public Bootstrap(NinjaPropertiesImpl ninjaProperties) {
    ...
    this.applicationModulesBasePackage = Optional.fromNullable(ninjaProperties.get(NinjaConstant.APPLICATION_MODULES_BASE_PACKAGE));
  }
  ...
  protected void configure() throws Exception {
    ...
    String applicationModuleClassName = resolveApplicationClassName(APPLICATION_GUICE_MODULE_CONVENTION_LOCATION);
    if (doesClassExist(applicationModuleClassName)) {
      ...
      try {
        applicationConfiguration = (AbstractModule) applicationModuleClass.getConstructor(NinjaProperties.class).newInstance(ninjaProperties);
      } catch (NoSuchMethodException e) {
        applicationConfiguration = (AbstractModule) applicationModuleClass.getConstructor().newInstance();
      }
      addModule(applicationConfiguration);
    }
    String applicationNinjaClassName = resolveApplicationClassName(NINJA_CONVENTION_LOCATION);
  ...
  public void initRoutes() throws Exception {
    String applicationRoutesClassName = resolveApplicationClassName(ROUTES_CONVENTION_LOCATION);
  ...
  private void initLogback() {
    try {
      ...
      logger.info("Logback is not on classpath (you are probably using slf4j-jdk14). I did not configure anything. It\'s up to you now...", exception);
    }
  }
  ...
  protected String resolveApplicationClassName(String classLocationAsDefinedByNinja) {
    if (applicationModulesBasePackage.isPresent()) {
      return new StringBuilder().append(applicationModulesBasePackage.get()).append('.').append(classLocationAsDefinedByNinja).toString();
    } else {
