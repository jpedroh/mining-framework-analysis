...
public class ContextImpl implements Context {
  ...
  @Inject public ContextImpl(BodyParserEngineManager bodyParserEngineManager, FlashCookie flashCookie, Logger logger, SessionCookie sessionCookie, TemplateEngineManager templateEngineManager) {
  ...
  @Override public Context template(String explicitTemplateName) {
    this.templateOverride = explicitTemplateName;
    return this;
  }

  @Override public Context status(HTTP_STATUS httpStatus) {
  ...
  @Override public Integer getPathParameterNumeric(String key) {
  ...
  @Override public Integer getParameterNumeric(String key, Integer defaultValue) {
    String parameter = getParameter(key);
    try {
      return Integer.parseInt(parameter);
    } catch (Exception e) {
  ...
  @Override public void addCookie(ninja.Cookie cookie) {
    httpServletResponse.addCookie(CookieHelper.convertNinjaCookieToServletCookie(cookie));
  }

  @Override public void unsetCookie(String name) {
  ...
  @Override public void render(Object object) {
  ...
  @Override public void renderHtml() {
    renderHtml(new HashMap<String, String>());
  }

  @Override public void renderHtml(Object object) {
    finalizeResponseHeaders(ContentTypes.TEXT_HTML);
    TemplateEngine templateEngine = templateEngineManager.getTemplateEngineForContentType(ContentTypes.TEXT_HTML);
    templateEngine.invoke(this, object);
  }

  @Override public void renderJson(Object object) {
    finalizeResponseHeaders(ContentTypes.APPLICATION_JSON);
    TemplateEngine templateEngine = templateEngineManager.getTemplateEngineForContentType(ContentTypes.APPLICATION_JSON);
    templateEngine.invoke(this, object);
  }
  ...
  private void setStatusOnResponse(HTTP_STATUS httpStatus) {
    if (httpStatus.equals(HTTP_STATUS.ok200)) {
      ...
      if (httpStatus.equals(HTTP_STATUS.notFound404)) {
        httpServletResponse.setStatus(404);
      } else {
        if (httpStatus.equals(HTTP_STATUS.forbidden403)) {
          httpServletResponse.setStatus(403);
        } else {
          if (httpStatus.equals(HTTP_STATUS.teapot418)) {
            httpServletResponse.setStatus(418);
          } else {
            if (httpStatus.equals(HTTP_STATUS.badRequest400)) {
              httpServletResponse.setStatus(400);
            } else {
              if (httpStatus.equals(HTTP_STATUS.noContent204)) {
                httpServletResponse.setStatus(204);
              } else {
                if (httpStatus.equals(HTTP_STATUS.created201)) {
  ...
  @Override public String getTemplateName(String suffix) {
    if (templateOverride == null) {
      ...
      return String.format("%sviews/%s/%s%s", parentControllerPackageAsPath, controller.getSimpleName(), route.getControllerMethod().getName(), suffix);
    }
    return templateOverride;
  }

  @Override public <T extends java.lang.Object> T parseBody(Class<T> classOfT) {
    BodyParserEngine bodyParserEngine = bodyParserEngineManager.getBodyParserEngineForContentType(ContentTypes.APPLICATION_JSON);
  ...
  @Override public FlashCookie getFlashCookie() {
    return flashCookie;
  }

  @Override public SessionCookie getSessionCookie() {
