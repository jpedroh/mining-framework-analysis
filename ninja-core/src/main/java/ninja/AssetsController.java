 ...
 * 
 */
@Singleton public class AssetsController {
  ...
  @Inject public AssetsController(MimeTypes mimeTypes) {
  ...
  public Result serve(Context context) {
    Object renderable = new Renderable() {
      @Override public void render(Context context, Result result) {
        String finalName = context.getHttpServletRequest().getRequestURI().replaceFirst(PUBLIC_PREFIX, "");
        InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream(ASSETS_PREFIX + finalName);
        if (inputStream == null) {
          ...
          try {
            ...
            ByteStreams.copy(this.getClass().getClassLoader().getResourceAsStream(ASSETS_PREFIX + finalName), context.getHttpServletResponse().getOutputStream());
