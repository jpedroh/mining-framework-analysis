...
public class HttpBasicAccessAuthenticatorTest {
  ...
  @Before public void setup() throws NoSuchAlgorithmException, InvalidKeyException, NoSuchPaddingException {
    ...
    basicAccessAuthenticator = new HttpBasicAccessAuthenticator(cookieEncryptor, Collections.singletonList(credentialsAuthenticator), APP_ID, sessionMaxTimeSeconds);
  ...
  @Test public void testValidCookie() throws ExecutionException, InterruptedException {
  ...
  @Test public void testInvalidCookie() throws ExecutionException, InterruptedException {
    setRequestCookie("not a cookie");
    assertNull(basicAccessAuthenticator.authenticate(request).get());
  }

  @Test public void testExpiredCookie() throws ExecutionException, InterruptedException {
  ...
  @Test public void testCookieWithWrongAppId() throws ExecutionException, InterruptedException {
  ...
  @Test public void testNoCookieAndNoCredentials() throws ExecutionException, InterruptedException {
    assertNull(basicAccessAuthenticator.authenticate(request).get());
  }

  @Test public void testInvalidCredentials() throws ExecutionException, InterruptedException {
    populateRequestWithCredentials("username", "expectedPassword");
    assertNull(basicAccessAuthenticator.authenticate(request).get());
  }

  @Test public void testValidCredentials() throws ExecutionException, InterruptedException {
    populateRequestWithCredentials(ROOT, ROOT);
    assertEquals(credentialsAuthenticator.getId(), basicAccessAuthenticator.authenticate(request).get());
  }

  @Test public void testValidCredentialsNonRootPath() throws ExecutionException, InterruptedException {
  ...
  private String createCookie(final String username, final DateTime cookieExpiredTime, final String appId, final String authenticatorId) {
    final AuthenticationCookie cookie = new AuthenticationCookie(username, cookieExpiredTime, appId, authenticatorId);
...
<<<<<<< /usr/src/app/output/outbrain/ob1k/3f7bdcaadc669ba2d14a424104ee4e09433edf88/ob1k-security/src/test/java/com/outbrain/ob1k/security/server/HttpBasicAccessAuthenticatorTest.java/left.java
    Base64.getEncoder().encodeToString(credentials.getBytes())
=======
    new String(Base64.getEncoder().encode(credentials.getBytes()))
>>>>>>> /usr/src/app/output/outbrain/ob1k/3f7bdcaadc669ba2d14a424104ee4e09433edf88/ob1k-security/src/test/java/com/outbrain/ob1k/security/server/HttpBasicAccessAuthenticatorTest.java/right.java
    ;
  ...
  private static class SpecificUserPasswordAuthenticator implements CredentialsAuthenticator<UserPasswordToken> {
    ...
    @Override public ComposableFuture<Boolean> authenticate(final Credentials<UserPasswordToken> credentials) {
