 ...
 */
@SuppressWarnings(value = { "unchecked", "WeakerAccess" }) public class MapDBContext implements DBContext {
  ...
  public static DBContext offlineInstance(String name) {
    DB db = DBMaker.fileDB(name).fileMmapEnableIfSupported().closeOnJvmShutdown().transactionEnable().fileDeleteAfterClose().make();
    return new MapDBContext(db);
  }

  @Override public <T extends java.lang.Object> List<T> getList(String name) {
    return (List<T>) db.<T>indexTreeList(name, Serializer.JAVA).createOrOpen();
  }

  @Override public <K extends java.lang.Object, V extends java.lang.Object> Map<K, V> getMap(String name) {
    return db.<K, V>hashMap(name, JAVA, JAVA).createOrOpen();
  }

  @Override public <T extends java.lang.Object> Set<T> getSet(String name) {
    return (Set<T>) db.<T>hashSet(name, JAVA).createOrOpen();
  }

  @Override public <T extends java.lang.Object> Var<T> getVar(String name) {
    return new MapDBVar<>((Atomic.Var<T>) db.atomicVar(name).createOrOpen());
  }

  @Override public String summary() {
    return stream(db.getAllNames().spliterator(), false).map(this::info).reduce(new StringJoiner("\n"), StringJoiner::add, StringJoiner::merge).toString();
  }

  @Override public Object backup() {
    Map<String, Object> collectedMap = localCopy();
    return writeAsString(collectedMap);
  }

  @Override public boolean recover(Object backup) {
  ...
  @Override public String info(String name) {
      ...
      if (struct instanceof Set) {
        ...
        if (struct instanceof List) {
          ...
          if (struct instanceof Map) {
            ...
            if (struct instanceof Atomic.Var) {
