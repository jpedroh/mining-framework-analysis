...
@SuppressWarnings(value = { "ConfusingArgumentToVarargsMethod", "UnusedReturnValue", "WeakerAccess", "unused", "ConstantConditions" }) public abstract class BaseAbilityBot extends DefaultAbsSender implements AbilityExtension {
  ...
  public void onUpdateReceived(Update update) {
    ...
    Stream.of(update).filter(this::checkGlobalFlags).filter(this::checkBlacklist).map(this::addUser).filter(this::filterReply).filter(this::hasUser).map(this::getAbility).filter(this::validateAbility).filter(this::checkPrivacy).filter(this::checkLocality).filter(this::checkInput).filter(this::checkMessageFlags).map(this::getContext).map(
<<<<<<< /usr/src/app/output/rubenlagus/telegrambots/28f60e302495a3f8a967a9cd5591396f39bba598/telegrambots-abilities/src/main/java/org/telegram/abilitybots/api/bot/BaseAbilityBot.java/left.java
    this::consumeUpdate
=======
    this::updateStats
>>>>>>> /usr/src/app/output/rubenlagus/telegrambots/28f60e302495a3f8a967a9cd5591396f39bba598/telegrambots-abilities/src/main/java/org/telegram/abilitybots/api/bot/BaseAbilityBot.java/right.java
  ...
  @NotNull Privacy getPrivacy(Update update, int id) {
