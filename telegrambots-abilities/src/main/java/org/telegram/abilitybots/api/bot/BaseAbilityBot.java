 ...
 */
@SuppressWarnings(value = { "ConfusingArgumentToVarargsMethod", "UnusedReturnValue", "WeakerAccess", "unused", "ConstantConditions" }) public abstract class BaseAbilityBot extends DefaultAbsSender implements AbilityExtension {
  ...
  public boolean isGroupAdmin(long chatId, long id) {
    ...
    return silent.execute(admins).orElse(new ArrayList<>()).stream().map((member) -> {
      final String status = member.getStatus();
      if (status.equals(ChatMemberOwner.STATUS) || status.equals(ChatMemberAdministrator.STATUS)) {
  ...
  private void registerAbilities() {
    try {
      extensions.addAll(stream(getClass().getMethods()).filter(checkReturnType(AbilityExtension.class)).map(returnExtension(this)).collect(Collectors.toList()));
      extensions.add(this);
      ...
      abilities = Stream.concat(defaultAbsStream, extensions.stream().flatMap((ext) -> stream(ext.getClass().getMethods()).filter(checkReturnType(Ability.class)).map(returnAbility(ext)))).collect(ImmutableMap::<String, Ability>builder, (b, a) -> b.put(a.name(), a), (b1, b2) -> b1.putAll(b2.build())).build();
      Stream<Reply> extensionReplies = extensions.stream().flatMap((ext) -> stream(ext.getClass().getMethods()).filter(checkReturnType(Reply.class)).map(returnReply(ext))).flatMap(Reply::stream);
      Stream<Reply> extensionCollectionReplies = extensions.stream().flatMap((extension) -> stream(extension.getClass().getMethods()).filter(checkReturnType(ReplyCollection.class)).map(returnReplyCollection(extension)).flatMap(ReplyCollection::stream));
      Stream<Reply> abilityReplies = abilities.values().stream().flatMap((ability) -> ability.replies().stream()).flatMap(Reply::stream);
      replies = Stream.of(abilityReplies, extensionReplies, extensionCollectionReplies).flatMap((replyStream) -> replyStream).collect(ImmutableList::<Reply>builder, Builder::add, (b1, b2) -> b1.addAll(b2.build())).build();
  ...
  private void initStats() {
    Set<String> enabledStats = Stream.concat(replies.stream().filter(Reply::statsEnabled).map(Reply::name), abilities.entrySet().stream().filter((entry) -> entry.getValue().statsEnabled()).map(Map.Entry::getKey)).collect(toSet());
    stats = db.getMap(STATS);
    Set<String> toBeRemoved = difference(stats.keySet(), enabledStats);
    toBeRemoved.forEach(stats::remove);
    enabledStats.forEach((abName) -> stats.computeIfAbsent(abName, (name) -> createStats(abName, 0)));
  }
  ...
  Trio<Update, Ability, String[]> getAbility(Update update) {
    ...
    if (allowContinuousText()) {
      String abName = abilities.keySet().stream().filter((name) -> msg.getText().startsWith(format("%s%s", getCommandPrefix(), name))).max(comparingInt(String::length)).orElse(DEFAULT);
      tokens = msg.getText().replaceFirst(getCommandPrefix() + abName, "").split(getCommandRegexSplit());
  ...
  private String stripBotUsername(String token) {
    return compile(format("@%s", botUsername), CASE_INSENSITIVE).matcher(token).replaceAll("");
  }
  ...
  boolean filterReply(Update update) {
    return replies.stream().filter((reply) -> runSilently(() -> reply.isOkFor(update), reply.name())).map((reply) -> runSilently(() -> {
  ...
  boolean runSilently(Callable<Boolean> callable, String name) {
    try {
      ...
      String msg = format("Reply [%s] failed to check for conditions. " + "Make sure you\'re safeguarding against all possible updates.", name);
