...
public enum PHPGrammar implements GrammarRuleKey {
  ...
  public static void lexical(LexerlessGrammarBuilder b) {
    b.rule(SPACING).is(b.skippedTrivia(b.regexp("[" + LexicalConstant.LINE_TERMINATOR + LexicalConstant.WHITESPACE + "]*+")), b.zeroOrMore(b.commentTrivia(b.regexp(LexicalConstant.COMMENT)), b.skippedTrivia(b.regexp("[" + LexicalConstant.LINE_TERMINATOR + LexicalConstant.WHITESPACE + "]*+")))).skip();
  ...
  public static void expression(LexerlessGrammarBuilder b) {
    ...
    b.rule(REFERENCE_VARIABLE).is(COMPOUND_VARIABLE, b.zeroOrMore(b.firstOf(DIMENSIONAL_OFFSET, b.sequence(LCURLYBRACE, EXPRESSION, RCURLYBRACE))));
    b.rule(VARIABLE_WITHOUT_OBJECTS).is(b.optional(SIMPLE_INDIRECT_REFERENCE), REFERENCE_VARIABLE);
    b.rule(COMPOUND_VARIABLE).is(b.firstOf(VAR_IDENTIFIER, b.sequence(DOLAR_LCURLY, EXPRESSION, RCURLYBRACE)));
    ...
    b.rule(VARIABLE_NAME).is(b.firstOf(IDENTIFIER, KEYWORDS, b.sequence(LCURLYBRACE, EXPRESSION, RCURLYBRACE)));
    b.rule(DIMENSIONAL_OFFSET).is(LBRACKET, b.optional(EXPRESSION), RBRACKET);
    b.rule(ALIAS_VARIABLE).is(AND, MEMBER_EXPRESSION);
    b.rule(MEMBER_EXPRESSION).is(PRIMARY_EXPRESSION, b.zeroOrMore(b.firstOf(OBJECT_MEMBER_ACCESS, CLASS_MEMBER_ACCESS, DIMENSIONAL_OFFSET, FUNCTION_CALL_PARAMETER_LIST)));
    b.rule(PRIMARY_EXPRESSION).is(b.firstOf(b.sequence(PHPKeyword.NAMESPACE, FULLY_QUALIFIED_NAME), CLASS_NAME, VARIABLE_WITHOUT_OBJECTS, IDENTIFIER, PARENTHESIS_EXPRESSION)).skip();
    ...
    b.rule(OBJECT_DIM_LIST).is(VARIABLE_NAME, b.zeroOrMore(b.firstOf(b.sequence(LCURLYBRACE, EXPRESSION, RCURLYBRACE), DIMENSIONAL_OFFSET)));
    ...
    b.rule(COMBINED_SCALAR).is(b.firstOf(b.sequence(ARRAY, LPARENTHESIS, b.optional(ARRAY_PAIR_LIST), RPARENTHESIS), b.sequence(LBRACKET, b.optional(ARRAY_PAIR_LIST), RBRACKET)));
    b.rule(ARRAY_PAIR_LIST).is(ARRAY_PAIR, b.zeroOrMore(COMMA, ARRAY_PAIR), b.optional(COMMA));
    b.rule(ARRAY_PAIR).is(b.firstOf(b.sequence(EXPRESSION, b.optional(DOUBLEARROW, b.firstOf(ALIAS_VARIABLE, EXPRESSION))), ALIAS_VARIABLE));
    b.rule(COMMON_SCALAR).is(b.firstOf(HEREDOC, NUMERIC_LITERAL, STRING_LITERAL, BOOLEAN_LITERAL, word(b, "NULL"), word(b, "__CLASS__"), word(b, "__FILE__"), word(b, "__DIR__"), word(b, "__FUNCTION__"), word(b, "__LINE__"), word(b, "__METHOD__"), word(b, "__NAMESPACE__"), word(b, "__TRAIT__")));
    b.rule(BOOLEAN_LITERAL).is(b.firstOf(word(b, "TRUE"), word(b, "FALSE")));
    b.rule(ENCAPS_LIST).is(b.oneOrMore(b.firstOf(ENCAPS_VAR, STRING_WITH_ENCAPS_VAR_CHARACTERS)));
    b.rule(ENCAPS_VAR).is(b.firstOf(SEMI_COMPLEX_ENCAPS_VARIABLE, SIMPLE_ENCAPS_VARIABLE, COMPLEX_ENCAPS_VARIABLE));
    b.rule(COMPLEX_ENCAPS_VARIABLE).is(LCURLYBRACE, b.next(DOLAR), EXPRESSION, RCURLYBRACE);
    b.rule(SEMI_COMPLEX_ENCAPS_VARIABLE).is(DOLAR_LCURLY, b.firstOf(EXPRESSION, SEMI_COMPLEX_RECOVERY_EXPRESSION), RCURLYBRACE);
    b.rule(SEMI_COMPLEX_RECOVERY_EXPRESSION).is(b.regexp("[^}]++"));
    b.rule(SIMPLE_ENCAPS_VARIABLE).is(VAR_IDENTIFIER, b.optional(b.firstOf(ENCAPS_DIMENSIONAL_OFFSET, ENCAPS_OBJECT_MEMBER_ACCESS)));
    ...
    b.rule(ENCAPS_VAR_OFFSET).is(b.firstOf(IDENTIFIER, NUMERIC_LITERAL, VAR_IDENTIFIER));
    ...
    b.rule(POSTFIX_EXPR).is(b.firstOf(COMBINED_SCALAR_OFFSET, FUNCTION_EXPRESSION, COMMON_SCALAR, MEMBER_EXPRESSION, NEW_EXPR, EXIT_EXPR, LIST_ASSIGNMENT_EXPR, INTERNAL_FUNCTION), b.optional(b.firstOf(INC, DEC, b.sequence(INSTANCEOF, MEMBER_EXPRESSION))));
    b.rule(FUNCTION_EXPRESSION).is(b.optional(STATIC), FUNCTION, b.optional(AND), LPARENTHESIS, b.optional(PARAMETER_LIST), RPARENTHESIS, b.optional(LEXICAL_VARS), BLOCK);
    ...
    b.rule(INTERNAL_FUNCTION).is(b.firstOf(b.sequence(ISSET, LPARENTHESIS, EXPRESSION, b.zeroOrMore(COMMA, EXPRESSION), RPARENTHESIS), b.sequence(EMPTY, LPARENTHESIS, EXPRESSION, RPARENTHESIS), b.sequence(INCLUDE_ONCE, EXPRESSION), b.sequence(INCLUDE, EXPRESSION), b.sequence(EVAL, LPARENTHESIS, EXPRESSION, RPARENTHESIS), b.sequence(REQUIRE_ONCE, EXPRESSION), b.sequence(REQUIRE, EXPRESSION), b.sequence(CLONE, EXPRESSION), b.sequence(PRINT, EXPRESSION)));
    b.rule(NEW_EXPR).is(NEW, MEMBER_EXPRESSION);
    b.rule(UNARY_EXPR).is(b.firstOf(b.sequence(b.firstOf(INC, DEC), POSTFIX_EXPR), b.sequence(b.firstOf(PLUS, MINUS, TILDA, BANG), UNARY_EXPR), b.sequence(PHPPunctuator.AT, POSTFIX_EXPR), b.sequence(CAST_TYPE, EXPRESSION), POSTFIX_EXPR)).skipIfOneChild();
    ...
    b.rule(ASSIGNMENT_EXPR).is(b.firstOf(b.sequence(MEMBER_EXPRESSION, EQU, AND, b.firstOf(NEW_EXPR, MEMBER_EXPRESSION)), b.sequence(CONDITIONAL_EXPR, ASSIGNMENT_OPERATOR, ASSIGNMENT_EXPR), CONDITIONAL_EXPR)).skipIfOneChild();
  ...
  public static void declaration(LexerlessGrammarBuilder b) {
    ...
    b.rule(USE_DECLARATION).is(b.firstOf(b.sequence(NAMESPACE_NAME, AS, IDENTIFIER), NAMESPACE_NAME));
    b.rule(CLASS_DECLARATION).is(CLASS_ENTRY_TYPE, IDENTIFIER, b.optional(EXTENDS_FROM), b.optional(IMPLEMENTS_LIST), LCURLYBRACE, b.zeroOrMore(CLASS_STATEMENT), RCURLYBRACE);
    b.rule(CLASS_ENTRY_TYPE).is(b.firstOf(b.sequence(b.optional(CLASS_TYPE), CLASS), TRAIT));
    ...
    b.rule(FULLY_QUALIFIED_CLASS_NAME).is(b.firstOf(b.sequence(NAMESPACE, FULLY_QUALIFIED_NAME), NAMESPACE_NAME));
    b.rule(INTERFACE_DECLARATION).is(INTERFACE, IDENTIFIER, b.optional(INTERFACE_EXTENDS_LIST), LCURLYBRACE, b.zeroOrMore(CLASS_STATEMENT), RCURLYBRACE);
    ...
    b.rule(METHOD_DECLARATION).is(b.zeroOrMore(MEMBER_MODIFIER), FUNCTION, b.optional(REFERENCE), IDENTIFIER, LPARENTHESIS, b.optional(PARAMETER_LIST), RPARENTHESIS, METHOD_BODY);
    ...
    b.rule(MEMBER_MODIFIER).is(b.firstOf(PUBLIC, PROTECTED, PRIVATE, STATIC, ABSTRACT, FINAL));
  }

  public static void statement(LexerlessGrammarBuilder b) {
    b.rule(TOP_STATEMENT_LIST).is(b.oneOrMore(TOP_STATEMENT));
    b.rule(TOP_STATEMENT).is(b.firstOf(CLASS_DECLARATION, FUNCTION_DECLARATION, INTERFACE_DECLARATION, NAMESPACE_STATEMENT, USE_STATEMENT, USE_FUNCTION_DECLARATION_STATEMENT, CONSTANT_DECLARATION, HALT_COMPILER_STATEMENT, STATEMENT));
    b.rule(NAMESPACE_NAME).is(b.firstOf(FULLY_QUALIFIED_NAME, QUALIFIED_NAME, UNQUALIFIED_NAME));
    ...
    b.rule(NAMESPACE_STATEMENT).is(b.firstOf(b.sequence(NAMESPACE, NAMESPACE_NAME, EOS), b.sequence(NAMESPACE, b.optional(NAMESPACE_NAME), LCURLYBRACE, b.optional(TOP_STATEMENT_LIST), RCURLYBRACE)));
    ...
    b.rule(FUNCTION_DECLARATION).is(FUNCTION, b.optional(REFERENCE), IDENTIFIER, LPARENTHESIS, b.optional(PARAMETER_LIST), RPARENTHESIS, BLOCK);
    b.rule(STATEMENT).is(b.firstOf(BLOCK, ALTERNATIVE_IF_STATEMENT, THROW_STATEMENT, IF_STATEMENT, WHILE_STATEMENT, DO_WHILE_STATEMENT, FOREACH_STATEMENT, FOR_STATEMENT, SWITCH_STATEMENT, BREAK_STATEMENT, CONTINUE_STATEMENT, RETURN_STATEMENT, EMPTY_STATEMENT, YIELD_STATEMENT, GLOBAL_STATEMENT, STATIC_STATEMENT, ECHO_STATEMENT, TRY_STATEMENT, DECLARE_STATEMENT, GOTO_STATEMENT, INLINE_HTML, UNSET_VARIABLE_STATEMENT, EXPRESSION_STATEMENT, LABEL));
    ...
    b.rule(ALTERNATIVE_IF_STATEMENT).is(IF, PARENTHESIS_EXPRESSION, COLON, b.optional(INNER_STATEMENT_LIST), b.optional(ALTERNATIVE_ELSEIF_LIST), b.optional(ALTERNATIVE_ELSE_CLAUSE), ENDIF, EOS);
    ...
    b.rule(FOR_STATEMENT).is(FOR, LPARENTHESIS, b.optional(FOR_EXRR), SEMICOLON, b.optional(FOR_EXRR), SEMICOLON, b.optional(FOR_EXRR), RPARENTHESIS, b.firstOf(ALTERNATIVE_FOR_STATEMENT, STATEMENT));
    ...
    b.rule(SWITCH_CASE_LIST).is(b.firstOf(b.sequence(LCURLYBRACE, b.optional(SEMICOLON), b.optional(CASE_LIST), RCURLYBRACE), b.sequence(COLON, b.optional(SEMICOLON), b.optional(CASE_LIST), ENDSWITCH, EOS)));
    ...
    b.rule(FOREACH_VARIABLE).is(b.firstOf(b.sequence(b.optional(AND), MEMBER_EXPRESSION), b.sequence(LIST, LPARENTHESIS, ASSIGNMENT_LIST, RPARENTHESIS)));
    b.rule(ALTERNATIVE_FOREACH_STATEMENT).is(COLON, b.optional(INNER_STATEMENT_LIST), ENDFOREACH, EOS);
    b.rule(DECLARE_STATEMENT).is(DECLARE, LPARENTHESIS, DECLARE_LIST, RPARENTHESIS, b.firstOf(ALTERNATIVE_DECLARE_STATEMENT, EOS, STATEMENT));
    ...
    b.rule(CLASS_STATEMENT).is(b.firstOf(METHOD_DECLARATION, CLASS_VARIABLE_DECLARATION, CLASS_CONSTANT_DECLARATION, TRAIT_USE_STATEMENT));
    b.rule(TRAIT_USE_STATEMENT).is(USE, INTERFACE_LIST, TRAIT_ADAPTATIONS);
    b.rule(TRAIT_ADAPTATIONS).is(b.firstOf(b.sequence(LCURLYBRACE, b.zeroOrMore(TRAIT_ADAPTATION_STATEMENT), RCURLYBRACE), EOS));
    ...
    b.rule(TRAIT_ALIAS).is(TRAIT_METHOD_REFERENCE, AS, b.firstOf(b.sequence(b.optional(MEMBER_MODIFIER), IDENTIFIER), MEMBER_MODIFIER));
    ...
    b.rule(INNER_STATEMENT_LIST).is(b.oneOrMore(b.firstOf(FUNCTION_DECLARATION, CLASS_DECLARATION, INTERFACE_DECLARATION, HALT_COMPILER_STATEMENT, STATEMENT)));
  }
  ...
  private static void keywords(LexerlessGrammarBuilder b) {
    ...
    b.rule(KEYWORDS).is(SPACING, b.firstOf(PHPKeyword.getKeywordValues()[0], PHPKeyword.getKeywordValues()[1], rest), b.nextNot(b.regexp(LexicalConstant.IDENTIFIER_PART)));
  }
