...
public class SMTPCommandTest {
  ...
  @Test public void authPlain() throws IOException, MessagingException, UserException {
    Socket smtpSocket;
    String hostAddress = greenMail.getSmtp().getBindTo();
    int port = greenMail.getSmtp().getPort();
    URLName smtpURL = new URLName(hostAddress);
    {
      ...
      try {
        ...
        smtpTransport.connect(smtpSocket);
        assertThat(smtpTransport.isConnected(), is(equalTo(true)));
        smtpTransport.issueCommand("AUTH PLAIN dGVzdAB0ZXN0AHRlc3RwYXNz", -1);
    ...
    }
    {
      ...
      try {
        ...
        smtpTransport.issueCommand("dGVzdAB0ZXN0AHRlc3RwYXNz", -1);
        assertThat(smtpTransport.getLastServerResponse(), equalToCompressingWhiteSpace(AuthCommand.AUTH_SUCCEDED));
      }  finally {
        smtpTransport.close();
      }
...
<<<<<<< /usr/src/app/output/greenmail-mail-test/greenmail/e152dc4b90c0ad478ee886f8b110c5b220d46b05/greenmail-core/src/test/java/com/icegreen/greenmail/test/commands/SMTPCommandTest.java/left.java
      equalToIgnoringWhiteSpace(smtpTransport.getLastServerResponse())
=======
      equalToCompressingWhiteSpace(smtpTransport.getLastServerResponse())
