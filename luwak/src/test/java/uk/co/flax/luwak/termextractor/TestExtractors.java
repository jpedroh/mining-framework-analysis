...
public class TestExtractors {
  ...
  @Test public void testRegexpExtractor() {
    ...
    assertThat(builder.collectTerms(new RegexpQuery(new Term("field", "super.*califragilistic")))).containsExactly(new QueryTerm("field", "califragilisticXX", QueryTerm.Type.CUSTOM, "WILDCARD"));
    assertThat(builder.collectTerms(new RegexpQuery(new Term("field", "hell.")))).containsExactly(new QueryTerm("field", "hellXX", QueryTerm.Type.CUSTOM, "WILDCARD"));
    assertThat(builder.collectTerms(new RegexpQuery(new Term("field", "hel?o")))).containsExactly(new QueryTerm("field", "heXX", QueryTerm.Type.CUSTOM, "WILDCARD"));
  }

  @Test @SuppressWarnings(value = { "deprecation" }) public void testRangeQueriesReturnAnyToken() {
  ...
  @Test public void testConstantScoreQueryExtractor() {
    BooleanQuery.Builder bq = new BooleanQuery.Builder();
    ...
    assertThat(treeBuilder.collectTerms(csqWithQuery)).containsExactly(new QueryTerm("f", "q1", QueryTerm.Type.EXACT));
  }

  @Test public void testPhraseQueryExtractor() {
    PhraseQuery.Builder pq = new PhraseQuery.Builder();
    pq.add(new Term("f", "hello"));
    pq.add(new Term("f", "encyclopedia"));
    assertThat(treeBuilder.collectTerms(pq.build())).containsOnly(new QueryTerm("f", "encyclopedia", QueryTerm.Type.EXACT));
  }

  @Test public void testBoostQueryExtractor() {
    BooleanQuery.Builder bq = new BooleanQuery.Builder();
    bq.add(new TermQuery(new Term("f", "q1")), BooleanClause.Occur.MUST);
    bq.add(new TermQuery(new Term("f", "q2")), BooleanClause.Occur.SHOULD);
