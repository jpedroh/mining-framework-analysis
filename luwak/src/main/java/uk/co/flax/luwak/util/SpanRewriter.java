...
public class SpanRewriter {
  ...
  public Query rewrite(Query in) throws RewriteException {
    ...
    if (in instanceof BoostQuery) {
      return rewrite(((BoostQuery) in).getQuery());
    }
  ...
  protected Query rewriteTermsQuery(TermsQuery query) throws RewriteException {
    ...
    try {
      ...
      for (Map.Entry<String, List<SpanTermQuery>> entry : spanQueries.entrySet()) {
        ...
        builder.add(new SpanOrQuery(termQueries.toArray(new SpanTermQuery[termQueries.size()])), BooleanClause.Occur.SHOULD);
      }
  ...
  protected Query rewritePhraseQuery(PhraseQuery query) {
    ...
    for (int i = 0; i < positions.length; i++) {
      if (positions[i] - positions[0] != i) {
        throw new IllegalArgumentException("Don\'t know how to rewrite PhraseQuery with holes or overlaps " + "(position must increase by 1 each time but found term " + terms[i - 1] + " at position " + positions[i - 1] + " followed by term " + terms[i] + " at position " + positions[i] + ")");
      }
