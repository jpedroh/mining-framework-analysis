...
public class Monitor implements Closeable {
  ...
  private class PresearcherQueryBuilder implements QueryIndex.QueryBuilder {
    ...
    @Override public Query buildQuery(QueryTermFilter termFilter) throws IOException {
  ...
  private static class StandardQueryCollector<T extends QueryMatch> implements QueryIndex.QueryCollector {
    ...
    @Override public void matchQuery(String id, QueryCacheEntry query, QueryIndex.DataValues dataValues) throws IOException {
      if (query == null) {
        return;
      }
      try {
  ...
  /**
     ...
     * @throws IOException on IO errors
     */
  public <T extends QueryMatch> PresearcherMatches<T> debug(final DocumentBatch docs, MatcherFactory<T> factory) throws IOException {
  ...
  private class PresearcherQueryCollector<T extends QueryMatch> extends StandardQueryCollector<T> {
    ...
    @Override public void matchQuery(final String id, QueryCacheEntry query, QueryIndex.DataValues dataValues) throws IOException {
      SpanCollector collector = new SpanCollector() {
        @Override public void collectLeaf(PostingsEnum postingsEnum, int position, Term term) throws IOException {
          matchingTerms.computeIfAbsent(id, (i) -> new StringBuilder()).append(" ").append(term.field()).append(":").append(term.bytes().utf8ToString());
        }
