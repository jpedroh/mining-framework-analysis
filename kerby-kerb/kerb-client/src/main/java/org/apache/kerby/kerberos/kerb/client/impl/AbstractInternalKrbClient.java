...
public abstract class AbstractInternalKrbClient implements InternalKrbClient {
  ...
  /**
     * {@inheritDoc}
     */
  @Override public KrbSetting getSetting() {
    return krbSetting;
  }
  ...
  /**
     * {@inheritDoc}
     */
  @Override public void init() throws KrbException {
  ...
  /**
     * {@inheritDoc}
     */
  @Override public TgtTicket requestTgtTicket(KOptions requestOptions) throws KrbException {
    AsRequest asRequest = null;
    if (requestOptions.contains(KrbOption.USE_PASSWD)) {
      asRequest = new AsRequestWithPasswd(context);
    } else {
      if (requestOptions.contains(KrbOption.USE_KEYTAB)) {
        ...
        if (requestOptions.contains(KrbOption.USE_PKINIT_ANONYMOUS)) {
          ...
          if (requestOptions.contains(KrbOption.USE_PKINIT)) {
            asRequest = new AsRequestWithCert(context);
          } else {
            if (requestOptions.contains(KrbOption.USE_TOKEN)) {
              asRequest = new AsRequestWithToken(context);
            } else {
              if (requestOptions.contains(KrbOption.TOKEN_USER_ID_TOKEN)) {
                asRequest = new AsRequestWithToken(context);
              }
            }
          }
  ...
  /**
     * {@inheritDoc}
     */
  @Override public ServiceTicket requestServiceTicket(KOptions requestOptions) throws KrbException {
    ...
    if (requestOptions.contains(KrbOption.TOKEN_USER_AC_TOKEN)) {
      ...
      if (requestOptions.contains(KrbOption.USE_TGT)) {
  ...
  protected abstract TgtTicket doRequestTgtTicket(AsRequest tgtTktReq) throws KrbException;

  protected abstract ServiceTicket doRequestServiceTicket(TgsRequest tgsRequest) throws KrbException;
