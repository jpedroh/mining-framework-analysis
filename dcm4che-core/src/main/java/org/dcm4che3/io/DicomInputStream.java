...
public class DicomInputStream extends FilterInputStream implements DicomInputHandler, BulkDataCreator {
  ...
  static final String VALUE_TOO_LARGE = "tag value too large, must be less than 2Gib";
  ...
  public static String toAttributePath(List<ItemPointer> itemPointers, int tag) {
    ...
    for (ItemPointer itemPointer : itemPointers) {
      sb.append('/').append(TagUtils.toHexString(itemPointer.sequenceTag)).append('/').append(itemPointer.itemIndex);
    }
  ...
  private DicomStreamException tagValueTooLargeException() {
    return new DicomStreamException(String.format("0x%s %s", TagUtils.toHexString(tag), VALUE_TOO_LARGE));
  }
      ...
      if (explicitVR) {
        ...
        if (vr == null) {
          ...
          if (!stopPredicate.test(this)) {
            LOG.warn("Unrecognized VR code: {}H for {} - treat as {}", TagUtils.shortToHexString(encodedVR), TagUtils.toString(tag), vr);
          }
        }
  ...
  /**
     ...
     */
  @Deprecated public Attributes readDataset(int len, int stopTag) throws IOException {
  ...
  /**
     ...
     * {@link #createWithLimit} or {@link #createWithLimitFromFileLength}.
     */
  @Deprecated public Attributes readDataset(int len, Predicate<DicomInputStream> stopPredicate) throws IOException {
  ...
  public void readAttributes(Attributes attrs, int len, Predicate<DicomInputStream> stopPredicate) throws IOException {
    ...
    while (undeflen || this.pos < endPos) {
      ...
      if (vr != null) {
        if (vr == VR.UN) {
          vr = tag == Tag.PurposeOfReferenceCodeSequence ? probeObservationClass() ? VR.CS : VR.SQ : ElementDictionary.vrOf(tag, attrs.getPrivateCreator(tag));
          if (vr == VR.UN && length == UNDEFINED_LENGTH) {
            vr = VR.SQ;
          }
        }
        excludeBulkData = includeBulkData == IncludeBulkData.NO && isBulkData(attrs);
        includeBulkDataURI = length != 0 && vr != VR.SQ && includeBulkData == IncludeBulkData.URI && isBulkData(attrs);
        handler.readValue(this, attrs);
      } else {
        skipAttribute(UNEXPECTED_ATTRIBUTE);
      }
  ...
  @Override public void readValue(DicomInputStream dis, Attributes attrs) throws IOException {
    checkIsThis(dis);
    if (excludeBulkData) {
      ...
      if (length == 0) {
        ...
        if (vr == VR.SQ) {
          ...
          if (length == UNDEFINED_LENGTH) {
            ...
            if (length == BulkData.MAGIC_LEN && super.in instanceof ObjectInputStream) {
              attrs.setValue(tag, vr, deserializeBulkData((ObjectInputStream) super.in));
            } else {
              if (includeBulkDataURI) {
  ...
  @Override public BulkData createBulkData(DicomInputStream dis) throws IOException {
    BulkData bulkData;
  ...
  private boolean isBulkData(Attributes attrs) {
    return bulkDataDescriptor.isBulkData(itemPointers, attrs.getPrivateCreator(tag), tag, vr, length);
  }

  @Override public void readValue(DicomInputStream dis, Sequence seq) throws IOException {
    checkIsThis(dis);
    if (length == 0) {
  ...
  @Override public void readValue(DicomInputStream dis, Fragments frags) throws IOException {
    checkIsThis(dis);
    if (excludeBulkData) {
      skipFully(length);
    } else {
  ...
  @Override public void startDataset(DicomInputStream dis) {
  }

  @Override public void endDataset(DicomInputStream dis) {
  }
  ...
  private void skipAttribute(String message) throws IOException {
    LOG.warn(message, new Object[] { TagUtils.toString(tag), length, tagPos });
    skipFully(length);
  }
  ...
  private void readSequence(int len, Attributes attrs, int sqtag) throws IOException {
    ...
    if (encodedVR == 0x554e && !probeExplicitVR()) {
      explicitVR = false;
      bigEndian = false;
    }
  ...
  public Attributes readItem() throws IOException {
    readHeader();
    if (tag != Tag.Item) {
      throw new IOException("Unexpected attribute " + TagUtils.toString(tag) + " #" + length + " @ " + pos);
    }
  ...
  public byte[] readValue() throws IOException {
    ...
    try {
      ...
      if (limitedStream && valLen > ((LimitedInputStream) in).getRemaining()) {
        throw new EOFException("Length " + valLen + " for tag " + TagUtils.toString(tag) + " @ " + tagPos + " exceeds remaining " + ((LimitedInputStream) in).getRemaining() + " (pos: " + pos + ")");
      }
      ...
      LOG.warn("IOException during read of {} #{} @ {}", TagUtils.toString(tag), length, tagPos, e);
      throw e;
    }
  }
  ...
  private void guessTransferSyntax(int preambleLength) throws IOException {
    ...
    if (rlen == b134.length) {
      if (b134[preambleLength] == 'D' && b134[preambleLength + 1] == 'I' && b134[preambleLength + 2] == 'C' && b134[preambleLength + 3] == 'M' && b134[preambleLength + 4] == 2 && b134[preambleLength + 5] == 0) {
  ...
  private boolean guessTransferSyntax(byte[] b132, int rlen, boolean bigEndian) throws DicomStreamException {
    ...
    if (ByteUtils.bytesToVR(b132, 4) == vr.code()) {
      this.tsuid = bigEndian ? UID.ExplicitVRBigEndian : UID.ExplicitVRLittleEndian;
