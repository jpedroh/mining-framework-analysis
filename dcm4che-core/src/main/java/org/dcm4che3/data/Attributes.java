...
public class Attributes implements Serializable {
  ...
  public static abstract class ItemPointerVisitor implements SequenceVisitor {
    ...
    @Override public void startItem(int sqTag, int itemIndex) {
  ...
  private ItemPointer[] itemPointers(int n) {
    ...
    itemPointers[itemPointers.length - n - 1] = new ItemPointer(parentSequencePrivateCreator, parentSequenceTag, itemIndex());
    return itemPointers;
  }
  ...
  private void decodeStringValuesUsingSpecificCharacterSet() {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (value instanceof Sequence) {
        ...
        if ((vr = vrs[i]).useSpecificCharacterSet()) {
          if (value instanceof byte[]) {
  ...
  private int creatorTagOf(String privateCreator, int tag, boolean reserve) {
    if (!TagUtils.isPrivateGroup(tag)) {
      throw new IllegalArgumentException(TagUtils.toString(tag) + " is not a private Data Element");
    }
    ...
    if ((creatorTag & 0xff00) != 0) {
      throw new IllegalStateException("No free block for Private Element " + TagUtils.toString(tag));
    }
  ...
  static Object loadBulkData(Object value) {
    try {
      return (value instanceof BulkData) ? ((BulkData) value).toBytes(null, ((BulkData) value).bigEndian()) : value;
    } catch (Exception e) {
  ...
  private long[] decodeISValue(int index) {
    ...
    if (value instanceof String) {
      ...
      for (int i = 0; i < ls.length; i++) {
        String s = ss[i];
        ls[i] = (s != null && !s.isEmpty()) ? StringUtils.parseIS(s) : Long.MIN_VALUE;
      }
    }
    values[index] = ls;
    return ls;
  ...
  public String privateCreatorOf(int tag) {
    return TagUtils.isPrivateTag(tag) ? privateCreatorAt(indexOf(TagUtils.creatorTagOf(tag))) : null;
  }

  private String privateCreatorAt(int index) {
    Object value;
    return (index < 0 || !vrs[index].isStringType() || (value = decodeStringValue(index)) == Value.NULL) ? null : VR.LO.toString(value, false, 0, null);
  }
  ...
  public byte[] getSafeBytes(String privateCreator, int tag) {
    try {
      ...
      LOG.info("Access " + TagUtils.toString(tag) + " throws i/o exception", e);
      return null;
    }
  }
  ...
  public Date getDate(String privateCreator, long tag, Date defVal, DatePrecision precision) {
    ...
    try {
      ...
      LOG.info("Invalid value of {} DA or {} TM", TagUtils.toString(daTag), TagUtils.toString(tmTag));
      return defVal;
    }
  }
  ...
  public Date[] getDates(String privateCreator, long tag, DatePrecisions precisions) {
    ...
    try {
      ...
      while (i < tm.length) {
        dates[i++] = VR.DT.toDate(da[i] + tm[i], tz, 0, false, null, precisions.precisions[i] = new DatePrecision());
      }
      while (i < da.length) {
        dates[i++] = VR.DA.toDate(da[i], tz, 0, false, null, precisions.precisions[i] = new DatePrecision());
      }
    } catch (IllegalArgumentException e) {
      LOG.info("Invalid value of {} DA or {} TM", TagUtils.toString(daTag), TagUtils.toString(tmTag));
  ...
  private DateRange toDateRange(String da, String tm) {
    ...
    DatePrecision precision = new DatePrecision();
    return new DateRange(darange[0] == null ? null : VR.DT.toDate(tmrange[0] == null ? darange[0] : darange[0] + tmrange[0], tz, 0, false, null, precision), darange[1] == null ? null : VR.DT.toDate(tmrange[1] == null ? darange[1] : darange[1] + tmrange[1], tz, 0, true, null, precision));
  }
  ...
  public void setTimezone(TimeZone tz) {
    ...
    if (tz.useDaylightTime()) {
      ...
      setString(Tag.TimezoneOffsetFromUTC, VR.SH, DateUtils.formatTimezoneOffsetFromUTC(tz));
    }
    this.tz = tz;
  }
  ...
  private void updateTimezone(TimeZone from, TimeZone to) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (val instanceof Sequence) {
        ...
        if (vrs[i] == VR.TM && tags[i] != Tag.PatientBirthTime || vrs[i] == VR.DT && tags[i] != Tag.ContextGroupVersion && tags[i] != Tag.ContextGroupLocalVersion) {
      ...
      if (tm instanceof String[]) {
        ...
        if (da instanceof String[]) {
          ...
          if (da == Value.NULL) {
            ...
            String dt = updateTimeZoneDT(from, to, (String) da + tms[0]);
        ...
        if (da instanceof String[]) {
          String[] das = (String[]) da;
          String dt = updateTimeZoneDT(from, to, das[0] + (String) tm);
          ...
          if (da == Value.NULL) {
            ...
            if (tmRange != null) {
              ...
              String dt = updateTimeZoneDT(from, to, (String) da + (String) tm);
              values[daIndex] = dt.substring(0, 8);
              values[tmIndex] = dt.substring(8);
            }
          }
  ...
  public String getPrivateCreator(int tag) {
    return TagUtils.isPrivateTag(tag) ? getString(TagUtils.creatorTagOf(tag), null) : null;
  }
  ...
  public Object remove(String privateCreator, int tag) {
    ...
    if (tag == Tag.SpecificCharacterSet) {
      ...
      if (tag == Tag.TimezoneOffsetFromUTC) {
  ...
  public boolean addSelected(Attributes other, String privateCreator, int tag) {
    ...
    if (!getSpecificCharacterSet().contains(other.getSpecificCharacterSet()) && containsNonASCIIStringValues(value, other.vrs[index])) {
      throw new IncompatibleSpecificCharacterSetException("Specific Character Sets " + Arrays.toString(getSpecificCharacterSet().toCodes()) + " and " + Arrays.toString(other.getSpecificCharacterSet().toCodes()) + " not compatible");
    }
    if (value instanceof Sequence) {
  ...
  private static boolean containsNonASCIIStringValues(Object val, VR vr) {
    if (val instanceof Sequence) {
      ...
      if (val != Value.NULL && vr.useSpecificCharacterSet()) {
        return true;
      }
    }
    return false;
  ...
  private boolean add(Attributes other, int[] include, int[] exclude, int fromIndex, int toIndex, Attributes selection, UpdatePolicy updatePolicy, boolean mergeOriginalAttributesSequence, boolean simulate, Attributes modified) {
    ...
    if (updatePolicy != UpdatePolicy.PRESERVE && !isEmpty() && !isSpecificCharacterSetCompatible(other, include, exclude, fromIndex, toIndex, selection, updatePolicy)) {
      throw new IncompatibleSpecificCharacterSetException("Specific Character Sets " + Arrays.toString(getSpecificCharacterSet().toCodes()) + " and " + Arrays.toString(other.getSpecificCharacterSet().toCodes()) + " not compatible");
    }
    ...
    for (int i = 0; i < otherSize; i++) {
      ...
      if (TagUtils.isPrivateCreator(tag) && (privateCreator = other.privateCreatorAt(i)) != null) {
        if ((selection == null || selection.creatorTagOf(privateCreator, tag, false) > 0) && !contains(tag) && (creatorTagOf(privateCreator, tag, false) < 0 || other.creatorTagOf(privateCreator, tag, false) != tag)) {
      ...
      if (TagUtils.isPrivateTag(tag)) {
        ...
        if (creatorTag != tmp) {
          ...
          privateCreator0 = privateCreator == null || privateCreator.equals(privateCreatorAt(indexOf(creatorTag))) ? null : privateCreator;
        }
      } else {
        creatorTag = 0;
        privateCreator = null;
          ...
          if (modified != null && !isEmpty(origValue) && !modified.contains(privateCreator, tag)) {
            if (origValue instanceof Sequence) {
              ...
              if (origValue instanceof Fragments) {
                ...
                modified.set(privateCreator, tag, vrs[j], toggleEndian(vrs[j], origValue, modifiedToggleEndian));
              }
            }
          }
        }
      ...
      if (!simulate) {
        if (value instanceof Sequence) {
          Sequence dest;
          if (mergeOriginalAttributesSequence && tag == Tag.OriginalAttributesSequence && (dest = getSequence(tag)) != null) {
            mergeOriginalAttributesSequence((Sequence) value, dest);
          } else {
            set(privateCreator0, tag, (Sequence) value, selection != null ? selection.getNestedDataset(privateCreator, tag) : null);
          }
        } else {
          if (value instanceof Fragments) {
  ...
  private boolean isSpecificCharacterSetCompatible(Attributes other, int[] include, int[] exclude, int fromIndex, int toIndex, Attributes selection, UpdatePolicy updatePolicy) {
    return isUpdateSpecificCharacterSet(other, include, exclude, fromIndex, toIndex, selection, updatePolicy) ? other.getSpecificCharacterSet().contains(getSpecificCharacterSet()) || !containsNonASCIIStringValues(null, null, 0, 0, null) : getSpecificCharacterSet().contains(other.getSpecificCharacterSet()) || !other.containsNonASCIIStringValues(include, exclude, fromIndex, toIndex, selection);
  }

  private boolean containsNonASCIIStringValues(int[] include, int[] exclude, int fromIndex, int toIndex, Attributes selection) {
    for (int i = 0; i < size; i++) {
      int tag = tags[i];
      Object val = values[i];
      if ((include == null || Arrays.binarySearch(include, fromIndex, toIndex, tag) >= 0) && (exclude == null || Arrays.binarySearch(exclude, fromIndex, toIndex, tag) < 0) && (selection == null || selection.contains(tag))) {
        if (val instanceof Sequence) {
          ...
          if (val != Value.NULL && vrs[i].useSpecificCharacterSet()) {
            return true;
          }
        }
      }
  ...
  private boolean isUpdateSpecificCharacterSet(Attributes other, int[] include, int[] exclude, int fromIndex, int toIndex, Attributes selection, UpdatePolicy updatePolicy) {
    ...
    return (include == null || Arrays.binarySearch(include, fromIndex, toIndex, Tag.SpecificCharacterSet) >= 0) && (exclude == null || Arrays.binarySearch(exclude, fromIndex, toIndex, Tag.SpecificCharacterSet) < 0) && (selection == null || selection.contains(Tag.SpecificCharacterSet));
  }
  ...
  private void mergeOriginalAttributesSequence(Sequence src, Sequence dest) {
    ...
    for (Attributes srcItem : src) {
      ...
      if (destItem != null && (destModified = destItem.getNestedDataset(Tag.ModifiedAttributesSequence)) != null) {
        try {
  ...
  private boolean equalValues(Attributes other, int index, int otherIndex) {
    ...
    if (vr.isStringType()) {
      ...
      return (v1 instanceof String[]) ? (v2 instanceof String[]) && (vr == VR.PN ? equalPNValues((String[]) v1, (String[]) v2) : Arrays.equals((String[]) v1, (String[]) v2)) : !(v2 instanceof String[]) && (vr == VR.PN ? equalPNValues(v1, v2) : v1.equals(v2));
    }
  ...
  private boolean equalPNValues(Object v1, Object v2) {
    return v1 == Value.NULL ? !containsPNValue(v2) : v2 == Value.NULL ? !containsPNValue(v1) : equalPNValues((String) v1, (String) v2);
  }
  ...
  private void set(String privateCreator, int tag, Sequence src, Attributes selection) {
    ...
    for (Attributes item : src) {
      dst.add(selection != null && !selection.isEmpty() ? new Attributes(item, bigEndian, selection) : new Attributes(item, bigEndian));
    }
  }
  ...
  public void writePostPixelDataTo(DicomOutputStream out) throws IOException {
  ...
  public boolean accept(Visitor visitor, boolean visitNestedDatasets) throws Exception {
    ...
    if (tags[0] < 0) {
      int index0 = -(1 + indexOf(0));
      return accept(visitor, visitNestedDatasets, index0, size) && accept(visitor, visitNestedDatasets, 0, index0);
    } else {
  ...
  private void checkInGroup(int i, int groupLengthTag) {
    ...
    if (TagUtils.groupLengthTagOf(tag) != groupLengthTag) {
      throw new IllegalStateException(TagUtils.toString(tag) + " does not belong to group (" + TagUtils.shortToHexString(TagUtils.groupNumber(groupLengthTag)) + ",eeee).");
    }
  }
  ...
  public Attributes createFileMetaInformation(String tsuid, boolean includeImplementationVersionName) {
    return createFileMetaInformation(getString(Tag.SOPInstanceUID, null), getString(Tag.SOPClassUID, null), tsuid, includeImplementationVersionName);
  }
  ...
  /**
     ...
     * @return created DICOM File Meta Information
     */
  public static Attributes createFileMetaInformation(String iuid, String cuid, String tsuid, boolean includeImplementationVersionName) {
  ...
  public boolean matches(Attributes keys, boolean ignorePNCase, boolean matchNoValue) {
    ...
    for (int i = 0; i < keysSize; i++) {
      ...
      if (keyVrs[i].isStringType()) {
        if (!matches(privateCreator, tag, keyVrs[i], ignorePNCase, matchNoValue, keys.getStrings(privateCreator, tag, null))) {
          return false;
        }
      } else {
  ...
  private boolean matches(String privateCreator, int tag, boolean ignorePNCase, boolean matchNoValue, Sequence keySeq) {
    int n = keySeq.size();
    if (n > 1) {
      LOG.info("Matching Key {} with VR: SQ contains {} Items - only consider first Item", TagUtils.toString(tag), n);
    }
  ...
  private void writeObject(ObjectOutputStream out) throws IOException {
    ...
    out.writeInt(size);
    @SuppressWarnings(value = { "resource" }) DicomOutputStream dout = new DicomOutputStream(out, bigEndian ? UID.ExplicitVRBigEndian : UID.ExplicitVRLittleEndian);
  ...
  private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
    ...
    init(in.readInt());
    @SuppressWarnings(value = { "resource" }) DicomInputStream din = new DicomInputStream(in, bigEndian ? UID.ExplicitVRBigEndian : UID.ExplicitVRLittleEndian);
  ...
  private void validate(DataElement el, ValidationResult result, Map<String, Boolean> processedConditions) {
  ...
  private IOD[] checkforMissingItems(int[] matchingItems, IOD[] itemIODs) {
    ...
    for (int i = 0; i < matchingItems.length; i++) {
      ...
      if (matchingItems[i] == 0 && itemIOD.getType() == DataElementType.TYPE_1) {
  ...
  private <T extends java.lang.Object> boolean isOneOf(Object val, T[] ss) {
    if (ss == null) {
      return true;
    }
    for (T s : ss) {
  ...
  private int creatorIndexOf(String privateCreator, int groupNumber) {
    if ((groupNumber & 1) == 0) {
      throw new IllegalArgumentException("(" + TagUtils.shortToHexString(groupNumber) + ",xxxx) is not a private Group");
    }
