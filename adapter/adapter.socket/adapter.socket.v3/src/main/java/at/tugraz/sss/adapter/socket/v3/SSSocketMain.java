...
public class SSSocketMain extends SSServImplA {
  ...
  public void start(String[] args) throws SSErr {
    try {
      ...
      if (SSCoreConf.instGet().getSss().use) {
        ...
        while (true) {
          new Thread(new SSSocket(server.accept())).start();
        }
      }
    } catch (Exception error) {
      SSServErrReg.regErrThrow(error);
  ...
  public class SSSocket implements Runnable {
    ...
    public SSSocket(final Socket clientSocket) throws UnsupportedEncodingException, IOException {
      this.clientSocket = clientSocket;
      this.inputStreamReader = new InputStreamReader(clientSocket.getInputStream(), SSEncodingU.utf8.toString());
      this.outputStreamWriter = new OutputStreamWriter(clientSocket.getOutputStream(), SSEncodingU.utf8.toString());
    ...
    @Override public void run() {
      ...
      try {
        ...
        par = new SSServPar(clientSocket, sqlCon, clientMsg);
