...
<<<<<<< /usr/src/app/output/lusito/gamedevweek/2b8368e72b1c86feb0f0c8df2780a326352dcdb2/gdx-kitten-rescue/src/main/java/de/hochschuletrier/gdw/ss14/ecs/components/CatPhysicsComponent.java/left.java
  Logger logger = LoggerFactory.getLogger(CatPhysicsComponent.class);
=======
  public short mask, category, group;
>>>>>>> /usr/src/app/output/lusito/gamedevweek/2b8368e72b1c86feb0f0c8df2780a326352dcdb2/gdx-kitten-rescue/src/main/java/de/hochschuletrier/gdw/ss14/ecs/components/CatPhysicsComponent.java/right.java
  ...
  /**
     ...
     */
  public CatPhysicsComponent(Vector2 position, float width, float height, float rotation, float friction, float restitution) {
  ...
  /**
     ...
     *            the restitution (elasticity) [0..1][low restitution is like rock - high like a ball]
     */
  public CatPhysicsComponent(Vector2 position, float width, float height, float rotation, float friction, float restitution, short mask, short category, short group) {
    if (height <= width) {
      throw new IllegalArgumentException("cat needs to be higher than fat");
    }
  ...
  @Override public void initPhysics(PhysixManager manager) {
    ...
    PhysixFixtureDef fixturedef = new PhysixFixtureDef(manager).density(1).friction(friction).restitution(restitution).mask(mask).category(category).groupIndex(group);
    physicsBody = new PhysixBodyDef(BodyType.DynamicBody, manager).position(position).fixedRotation(true).angle(rotation).create();
  ...
  @Override protected void beginContact(PhysixContact contact) {
    super.beginContact(contact);
    collisionListeners.forEach((l) -> l.fireBeginnCollision(contact));
  }

  @Override protected void endContact(PhysixContact contact) {
