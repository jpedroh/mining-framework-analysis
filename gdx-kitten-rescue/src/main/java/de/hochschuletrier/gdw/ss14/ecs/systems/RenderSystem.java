...
public class RenderSystem extends ECSystem {
  ...
  @Override public void render() {
    ...
    if (renderComponents == null || physicsComponents == null || renderComponents.length != arrayLength || physicsComponents.length != arrayLength) {
    ...
    for (Integer integer : entites) {
      ...
      if (renderCompo.texture != null) {
        ...
        DrawUtil.batch.draw(renderCompo.texture, physicsCompo.getPosition().x - (renderCompo.texture.getRegionWidth() / 2), physicsCompo.getPosition().y - (renderCompo.texture.getRegionHeight() / 2), renderCompo.texture.getRegionWidth() / 2, renderCompo.texture.getRegionHeight() / 2, renderCompo.texture.getRegionWidth(), renderCompo.texture.getRegionHeight(), 1f, 1f, (float) (physicsCompo.getRotation() * 180 / Math.PI));
        if (renderCompo.tintColor != null) {
          DrawUtil.batch.end();
