...
public class CatContactSystem extends ECSystem implements ICollisionListener {
  ...
  private void removeRayCast(PhysicsComponent a, PhysicsComponent b) {
    int i = 0;
    while (!(raycst_startPhys.get(i) == a && raycst_targetPhys.get(i) == b) && !(i > raycst_startPhys.size())) {
      i++;
    }
      ...
      if ((c = entityManager.getComponent(otherEntity, JumpablePropertyComponent.class)) != null) {
        switch (((JumpablePropertyComponent) c).type) {
          ...
          if ((d = entityManager.getComponent(myEntity, CatPropertyComponent.class)) != null) {
            ...
            SoundManager.performAction(CatStateEnum.FALL);
          }
          break;
          case waterpuddle:
          if (!isCatInZone) {
          ...
          if ((d = entityManager.getComponent(myEntity, CatPropertyComponent.class)) != null) {
            ...
            SoundManager.performAction(CatStateEnum.DIE);
          }
          break;
          case bloodpuddle:
          if (!isCatInZone) {
  ...
  @Override public void update(float delta) {
    for (int i = 0; i < raycst_startPhys.size(); ++i) {
      phyManager.getWorld().rayCast(rcp, raycst_startPhys.get(i).physicsBody.getPosition(), raycst_targetPhys.get(i).physicsBody.getPosition());
      boolean freeSight = false;
      for (RayCastPhysics rcst : rcp.collisions) {
        if (rcst.m_fixture.getBody().getUserData() != null && rcst.m_fixture.getBody().getUserData().equals("wall")) {
          break;
        }
        for (Fixture s : raycst_startPhys.get(i).physicsBody.getFixtureList()) {
          if ((s.getUserData() != null && s.getUserData().equals("sightcone")) || s != rcst.m_fixture) {
            continue;
          }
          freeSight = true;
        }
