...
public class CatContactSystem extends ECSystem implements ICollisionListener {
  ...
  @Override public void update(float delta) {
    if (raycst_startPhys.size() == 0) {
      return;
    }
    if (raycst_targetPhys.size() == 0) {
      return;
    }
    for (int i = 0; i < raycst_startPhys.size(); ++i) {
      ...
      for (RayCastPhysics rcst : rcp.collisions) {
        ...
        for (Fixture s : raycst_startPhys.get(i).physicsBody.getFixtureList()) {
          if (s.getUserData().equals("sightcone") || s != rcst.m_fixture) {
            continue;
          }
          freeSight = true;
        }
      ...
      if ((c = entityManager.getComponent(startEntity, EnemyComponent.class)) != null) {
        ...
        if ((c = entityManager.getComponent(startEntity, PlayerComponent.class)) != null) {
