  ...
  @Override public void update(float delta) {
    ...
    for (Integer entity : entities) {
      ...
      if (catPropertyComponent.getState() == CatStateEnum.IDLE || catPropertyComponent.getState() == CatStateEnum.WALK || catPropertyComponent.getState() == CatStateEnum.RUN) {
        ...
        float physicAngle = (float) Math.atan2(-(movementComponent.oldPositionVec.x - movementComponent.positionVec.x), movementComponent.oldPositionVec.y - movementComponent.positionVec.y);
        if (physicAngle < 0) {
          physicAngle *= -1;
        }
        ...
        if (distance >= 200 && laserPointerComponent.isVisible) {
          ...
          if (distance >= 100 && laserPointerComponent.isVisible) {
