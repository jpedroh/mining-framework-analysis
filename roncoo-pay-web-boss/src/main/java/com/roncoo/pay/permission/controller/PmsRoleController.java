...
@Controller @RequestMapping(value = "/pms/role") public class PmsRoleController extends BaseController {
  ...
  /**
	 ...
	 * @return
	 */
  @SuppressWarnings(value = { "unchecked" }) @RequiresPermissions(value = "pms:role:assignmenu") @RequestMapping(value = "/assignMenuUI") public String assignMenuUI(HttpServletRequest req, Model model, Long roleId) {
    PmsRole role = pmsRoleService.getDataById(roleId);
    if (role == null) {
      return operateError("\u65e0\u6cd5\u83b7\u53d6\u89d2\u8272\u4fe1\u606f", model);
    }
  ...
  /**
	 * 分配角色菜单
	 */
  @RequiresPermissions(value = "pms:role:assignmenu") @RequestMapping(value = "/assignMenu") public String assignMenu(HttpServletRequest req, Model model, @RequestParam(value = "roleId") Long roleId, DwzAjax dwz, @RequestParam(value = "selectVal") String selectVal) {
    try {
