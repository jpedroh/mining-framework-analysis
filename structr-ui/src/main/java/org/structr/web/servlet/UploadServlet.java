...
public class UploadServlet extends AbstractServletBase implements HttpServiceServlet {
  ...
  @Override protected void doPut(final HttpServletRequest request, final HttpServletResponse response) throws ServletException {
    try {
      assertInitialized();
    } catch (FrameworkException fex) {
      try {
  ...
  @Override protected void doOptions(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    setCustomResponseHeaders(response);
    try {
      assertInitialized();
      request.setCharacterEncoding("UTF-8");
  ...
  protected Gson getGson() {
    ...
    final GsonBuilder gsonBuilder = new GsonBuilder().setPrettyPrinting().serializeNulls().registerTypeAdapter(IJsonInput.class, jsonInputAdapter);
