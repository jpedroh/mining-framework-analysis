...
public class DirectoryWatchServiceTest extends StructrUiTest {
  ...
  @Parameters(value = "testDatabaseConnection") @BeforeClass(alwaysRun = true) @Override public void setup(@Optional String testDatabaseConnection) {
    Settings.Services.setValue("NodeService SchemaService HttpService DirectoryWatchService");
    super.setup(testDatabaseConnection);
  }

  @Test public void testMountedFolderInRoot() {
  ...
  @Test public void testNestedMountedFolder() {
  ...
  @Test public void testExternalChangesOfMountedDirectory() {
    Path root = null;
  ...
  @Test public void testDisableWatchKeyRegistration() {
    Path root = null;
    java.io.File file1 = null;
    java.io.File file2 = null;
    java.io.File file3 = null;
    try {
      ...
      try (Tx tx = app.tx()) {
        ...
        app.create(Folder.class, new NodeAttribute<>(Folder.name, "mounted3"), new NodeAttribute<>(StructrApp.key(Folder.class, "storageProvider"), "testMount"), new NodeAttribute<>(StructrApp.key(Folder.class, "mountWatchContents"), false));
        tx.success();
      } catch (FrameworkException fex) {
        fail("Unexpected exception.");
      }
  ...
  @Test public void testImageDataUploadToMountedFolder() {
    ...
    try (Tx tx = app.tx()) {
      final User tester = app.create(User.class, new NodeAttribute<>(StructrApp.key(User.class, "name"), "tester"), new NodeAttribute<>(StructrApp.key(User.class, "password"), "tester"));
      ...
      final Folder folder = app.create(Folder.class, new NodeAttribute<>(Folder.name, "mounted"), new NodeAttribute<>(StructrApp.key(Folder.class, "mountWatchContents"), false), new NodeAttribute<>(StructrApp.key(Folder.class, "storageProvider"), "testMount"));
    ...
    try (Tx tx = app.tx()) {
      ...
      if (grant != null) {
        ...
        app.create(ResourceAccess.class, new NodeAttribute<>(ResourceAccess.signature, "Image"), new NodeAttribute<>(ResourceAccess.flags, 4L), new NodeAttribute<>(ResourceAccess.visibleToAuthenticatedUsers, true));
      }
    ...
    RestAssured.given().contentType("application/json; charset=UTF-8").filter(ResponseLoggingFilter.logResponseIfStatusCodeIs(200)).filter(ResponseLoggingFilter.logResponseIfStatusCodeIs(201)).filter(ResponseLoggingFilter.logResponseIfStatusCodeIs(400)).filter(ResponseLoggingFilter.logResponseIfStatusCodeIs(404)).filter(ResponseLoggingFilter.logResponseIfStatusCodeIs(422)).filter(ResponseLoggingFilter.logResponseIfStatusCodeIs(500)).headers("X-User", "tester", "X-Password", "tester").body("{ name: \'test.png\', imageData: \'" + base64Data + "\' }").expect().statusCode(201).when().post("/Image");
    try (Tx tx = app.tx()) {
  ...
  private String readFile(final File file) throws IOException {
    try (InputStream is = StorageProviderFactory.getStorageProvider(file).getInputStream()) {
      return IOUtils.toString(is, "utf-8");
    }
  }
}