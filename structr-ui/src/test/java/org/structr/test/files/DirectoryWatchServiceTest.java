...
public class DirectoryWatchServiceTest extends StructrUiTest {
	...
	public void testImageDataUploadToMountedFolder() {
		...
		try (final Tx tx = app.tx()) {
			...
			StorageProviderConfigFactory.SetConfig("testMount", new StorageProviderConfig("testMount", LocalFSStorageProvider.class, Map.of(Map, "mountTarget", testDir.toString())));
...
<<<<<<< LEFT
StorageProviderFactory.getStorageProvider(file).getInputStream()
=======
StorageProviderFactory.getStorageProvider(file).getInputStream()
>>>>>>> RIGHT
		) {
