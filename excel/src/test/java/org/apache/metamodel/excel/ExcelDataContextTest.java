...
public class ExcelDataContextTest extends TestCase {
  ...
  public void testEmptyFileNoHeaderLine() throws Exception {
    DataContext dc = new ExcelDataContext(copyOf("src/test/resources/empty_file.xls"), new ExcelConfiguration(ExcelConfiguration.NO_COLUMN_NAME_LINE, false, false));
    assertEquals(1, dc.getDefaultSchema().getTableCount());
    Table table = dc.getDefaultSchema().getTables()[0];
    assertEquals("sheet", table.getName());
    assertEquals(0, table.getColumnCount());
  ...
  public void testUnexistingHeaderLine() throws Exception {
    DataContext dc = new ExcelDataContext(copyOf("src/test/resources/xls_people.xls"), new ExcelConfiguration(20, true, false));
    assertEquals(1, dc.getDefaultSchema().getTableCount());
    Table table = dc.getDefaultSchema().getTables()[0];
  ...
  public void testInvalidFormula() throws Exception {
    ...
    assertTrue(ds.next());
    row = ds.getRow();
    assertEquals("TismmerswerskisMFSTLandsmeers                                                          ", row.getValue(0).toString());
    assertTrue(ds.next());
    row = ds.getRow();
  ...
  public void testTicket99defect() throws Exception {
    ...
    DataContext dc = new ExcelDataContext(file);
    Schema schema = dc.getDefaultSchema();
    assertEquals("[Table[name=Sheet1,type=null,remarks=null], Table[name=Sheet2,type=null,remarks=null], Table[name=Sheet3,type=null,remarks=null]]", Arrays.toString(schema.getTables()));
    assertEquals(0, schema.getTableByName("Sheet2").getColumnCount());
    assertEquals(0, schema.getTableByName("Sheet3").getColumnCount());
    Table table = schema.getTableByName("Sheet1");
    assertEquals("[Column[name=Pkg No.,columnNumber=0,type=VARCHAR,nullable=true,nativeType=null,columnSize=null], " + "Column[name=Description,columnNumber=1,type=VARCHAR,nullable=true,nativeType=null,columnSize=null], " + "Column[name=Room,columnNumber=2,type=VARCHAR,nullable=true,nativeType=null,columnSize=null], " + "Column[name=Level,columnNumber=3,type=VARCHAR,nullable=true,nativeType=null,columnSize=null]]", Arrays.toString(table.getColumns()));
  }
  ...
  public void testInsertInto() throws Exception {
    ...
    dc.executeUpdate(new UpdateScript() {
      @Override public void run(UpdateCallback cb) {
        ...
        cb.insertInto(table).value("id", 1000).value(nameColumn, "pennywise the [clown]", clownStyle).value("gender", "male").value("age", 30, thirtyStyle).execute();
      }
    });
    ...
    assertEquals("font-style: italic;text-decoration: underline;text-align: center;color: rgb(0,0,255);", ds.getRow().getStyle(0).toCSS());
    assertFalse(ds.next());
  }
  ...
  private void runCreateTableTest(File file) {
    ...
    dc.executeUpdate(new UpdateScript() {
      @Override public void run(UpdateCallback cb) {
        Schema schema = dc.getDefaultSchema();
        Table table1 = cb.createTable(schema, "my_table_1").withColumn("foo").withColumn("bar").withColumn("baz").execute();
        assertEquals(1, schema.getTableCount());
        assertSame(table1.getSchema(), schema);
        assertSame(table1, schema.getTables()[0]);
        Table table2 = cb.createTable(schema, "my_table_2").withColumn("foo").withColumn("bar").withColumn("baz").execute();
    ...
    dc.executeUpdate(new UpdateScript() {
      @Override public void run(UpdateCallback cb) {
        cb.insertInto(dc.getTableByQualifiedLabel("my_table_1")).value("foo", 456.2).value("bar", "\u043f\u0430\u0440\u0444\u044e\u043c\u0435\u0440\u0438\u044f +\u0438 \u043a\u043e\u0441\u043c\u0435\u0442\u0438\u043a\u0430").value("baz", false).execute();
      }
    });
    dc.executeUpdate(new UpdateScript() {
      @Override public void run(UpdateCallback cb) {
        cb.insertInto("my_table_1").value("foo", 789).value("bar", DateUtils.get(2011, Month.JULY, 8)).value("baz", false).execute();
      }
    });
    ...
    assertEquals("1", MetaModelHelper.executeSingleRowQuery(dc, dc.query().from("my_table_1").selectCount().toQuery()).getValue(0).toString());
  ...
  public void testCustomColumnNames() throws Exception {
    ...
    final ExcelConfiguration configuration = new ExcelConfiguration(ExcelConfiguration.DEFAULT_COLUMN_NAME_LINE, new CustomColumnNamingStrategy(firstColumnName, secondColumnName, thirdColumnName), true, false);
    final DataContext dataContext = new ExcelDataContext(copyOf("src/test/resources/Spreadsheet2007.xlsx"), configuration);
