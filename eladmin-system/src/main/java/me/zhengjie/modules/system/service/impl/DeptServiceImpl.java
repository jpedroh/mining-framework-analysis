 ...
 * @date 2019-03-25
 */
@Service @RequiredArgsConstructor @CacheConfig(cacheNames = "dept") public class DeptServiceImpl implements DeptService {
  ...
  @Override public List<DeptDto> queryAll(DeptQueryCriteria criteria, Boolean isQuery) throws Exception {
  ...
  @Override @Cacheable(key = "\'id:\' + #p0") public DeptDto findById(Long id) {
    Dept dept = deptRepository.findById(id).orElseGet(Dept::new);
    ValidationUtil.isNull(dept.getId(), "Dept", "id", id);
    return deptMapper.toDto(dept);
  }

  @Override @Cacheable(key = "\'pid:\' + #p0") public List<Dept> findByPid(long pid) {
    return deptRepository.findByPid(pid);
  }

  @Override public Set<Dept> findByRoleId(Long id) {
    return deptRepository.findByRoleId(id);
  }

  @Override @Transactional(rollbackFor = Exception.class) public void create(Dept resources) {
  ...
  @Override @Transactional(rollbackFor = Exception.class) public void update(Dept resources) {
  ...
  @Override @Transactional(rollbackFor = Exception.class) public void delete(Set<DeptDto> deptDtos) {
  ...
  @Override public void download(List<DeptDto> deptDtos, HttpServletResponse response) throws IOException {
  ...
  @Override public Set<DeptDto> getDeleteDepts(List<Dept> menuList, Set<DeptDto> deptDtos) {
  ...
  @Override public List<Long> getDeptChildren(Long deptId, List<Dept> deptList) {
  ...
  @Override public List<DeptDto> getSuperior(DeptDto deptDto, List<Dept> depts) {
  ...
  @Override public Object buildTree(List<DeptDto> deptDtos) {
    ...
    for (DeptDto deptDTO : deptDtos) {
      ...
      if (isChild) {
        ...
        if (deptDTO.getPid() != null && !deptNames.contains(findById(deptDTO.getPid()).getName())) {
          depts.add(deptDTO);
        }
      }
    }
  ...
  @Override public void verification(Set<DeptDto> deptDtos) {
