...
class PredicateIndexArray implements PredicateIndex {
  ...
  @Override public long getNumOcurrences(int pred) {
    return bitmap.select1(pred) - bitmap.select1(pred - 1);
  }

  @Override public long getOccurrence(long base, long occ) {
  ...
  @Override public void generate(ProgressListener listener) {
    IntermediateListener iListener = new IntermediateListener(listener);
    StopWatch st = new StopWatch();
    @SuppressWarnings(value = { "resource" }) SequenceLog64 predCount = new SequenceLog64(BitUtil.log2(triples.getSeqY().getNumberOfElements()));
    long maxCount = 0;
  ...
  @Override public void mapIndex(CountInputStream input, File f, ProgressListener listener) throws IOException {
    bitmap = BitmapFactory.createBitmap(input);
    bitmap.load(input, null);
