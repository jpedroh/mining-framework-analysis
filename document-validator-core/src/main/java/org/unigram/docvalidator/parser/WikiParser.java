 ...
 */
@SuppressWarnings(value = { "ALL" }) public final class WikiParser extends BasicDocumentParser {
  /**
   * Constructor.
   */
  public WikiParser() {
              ...
              if (check(NUMBERED_LIST_PATTERN, line, head)) {
                ...
                if (check(BEGIN_COMMENT_PATTERN, line, head)) {
                  ...
                  if (line.equals("")) {
                    ...
                    String remainStr = appendSentencesIntoSection(lineNum, remain.append(line).toString(), currentSection);
  ...
  private void appendListElement(Section currentSection, LinePattern prevPattern, List<String> head, int lineNum) {
    ...
    currentSection.appendListElement(extractListLevel(head.get(0)), outputSentences);
    if (remainSentence != null && remainSentence.length() > 0) {
      outputSentences.add(new Sentence(remainSentence, lineNum));
    }
  }

  private Section appendSection(FileContent fileContent, Section currentSection, List<String> head, int lineNum) {
    ...
    if (!addChild(currentSection, tmpSection)) {
      LOG.warn("Failed to add parent for a Seciotn: " + tmpSection.getHeaderContents().next());
    }
  ...
  private String appendSentencesIntoSection(int lineNum, String line, Section currentSection) {
    List<Sentence> outputSentences = new ArrayList<Sentence>();
