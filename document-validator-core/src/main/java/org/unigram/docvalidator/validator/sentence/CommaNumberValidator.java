...
public class CommaNumberValidator implements SentenceValidator {
  ...
  public List<ValidationError> check(Sentence line) {
    ...
    if (maxCommaNum < commaCount) {
      result.add(new ValidationError("The number of comma is exceeds the maximum \"" + String.valueOf(commaCount) + "\".", line));
    }
    return result;
  }
  ...
  public boolean initialize(ValidatorConfiguration conf, CharacterTable characterTable) throws DocumentValidatorException {
    ...
    if (conf.getAttribute("max_comma_num") != null) {
      ...
      LOG.info("Maximum number of comma in one sentence is set to " + this.maxCommaNum);
    } else {
