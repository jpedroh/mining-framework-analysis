...
public class QuotationValidator implements SentenceValidator {
  public QuotationValidator() {
    ...
    leftSingleQuotationMark = new DVCharacter("LEFT_SINGLE_QUOTATION_MARK", "\u2018", "", true, false);
    rightSingleQuotationMark = new DVCharacter("RIGHT_SINGLE_QUOTATION_MARK", "\u2019", "", false, true);
    leftDoubleQuotationMark = new DVCharacter("LEFT_DOUBLE_QUOTATION_MARK", "\u201c", "", true, false);
    rightDoubleQuotationMark = new DVCharacter("RIGHT_DOUBLE_QUOTATION_MARK", "\u201d", "", false, true);
  ...
  @Override public List<ValidationError> check(Sentence sentence) {
    List<ValidationError> errors = new ArrayList<ValidationError>();
    List<ValidationError> result = this.checkQuotation(sentence, leftSingleQuotationMark, rightSingleQuotationMark);
    if (result != null) {
      errors.addAll(result);
    }
    errors.addAll(this.checkQuotation(sentence, leftDoubleQuotationMark, rightDoubleQuotationMark));
    return errors;
  }

  @Override public boolean initialize(ValidatorConfiguration conf, CharacterTable charTable) throws DocumentValidatorException {
    ...
    if (conf.getAttribute("use_ascii").equals("true")) {
      ...
      if (charTable.isContainCharacter("LEFT_SINGLE_QUOTATION_MARK")) {
        leftSingleQuotationMark = charTable.getCharacter("LEFT_SINGLE_QUOTATION_MARK");
      }
      if (charTable.isContainCharacter("RIGHT_SINGLE_QUOTATION_MARK")) {
        rightSingleQuotationMark = charTable.getCharacter("RIGHT_SINGLE_QUOTATION_MARK");
      }
      if (charTable.isContainCharacter("LEFT_DOUBLE_QUOTATION_MARK")) {
        leftSingleQuotationMark = charTable.getCharacter("LEFT_DOUBLE_QUOTATION_MARK");
      }
      if (charTable.isContainCharacter("RIGHT_DOUBLE_QUOTATION_MARK")) {
        rightSingleQuotationMark = charTable.getCharacter("RIGHT_DOUBLE_QUOTATION_MARK");
      }
    }
    return true;
  }

  private List<ValidationError> checkQuotation(Sentence sentence, DVCharacter leftQuotation, DVCharacter rightQuotation) {
    ...
    while (leftPosition >= 0 && rightPosition < sentenceString.length()) {
      ...
      if (leftPosition < 0) {
        rightPosition = this.getQuotePosition(sentenceString, rightQuotation.getValue(), rightPosition + 1);
      } else {
        rightPosition = this.getQuotePosition(sentenceString, rightQuotation.getValue(), leftPosition + 1);
      }
      if (leftPosition >= 0 && rightPosition < 0) {
        errors.add(new ValidationError("Right Quotation mark does not exist." + String.valueOf(sentence.content.length()), sentence));
        break;
      }
      if (leftPosition < 0 && rightPosition >= 0) {
        errors.add(new ValidationError("left Quotation mark does not exist." + String.valueOf(sentence.content.length()), sentence));
        break;
      }
      ...
      if (nextLeftPosition < rightPosition && nextLeftPosition > 0) {
        errors.add(new ValidationError("Twice Right Quotation marks in succession.", sentence));
      }
      if (nextRightPosition < leftPosition && nextRightPosition > 0) {
        errors.add(new ValidationError("Twice Left Quotation marks in succession.", sentence));
      }
      if (leftPosition > 0 && leftQuotation.isNeedBeforeSpace() && (sentenceString.charAt(leftPosition - 1) != ' ')) {
        errors.add(new ValidationError("Left quotation does not have space.", sentence));
      }
      if (rightPosition > 0 && rightPosition < sentenceString.length() - 1 && rightQuotation.isNeedAfterSpace() && (sentenceString.charAt(rightPosition + 1) != ' ' && sentenceString.charAt(rightPosition + 1) != this.period)) {
        errors.add(new ValidationError("Right quotation does not have space", sentence));
      }
    }
    return errors;
  }

  private int getQuotePosition(String sentenceStr, String quote, int startPosition) {
