...
public class SentenceLengthValidator implements SentenceValidator {
  @SuppressWarnings(value = { "WeakerAccess" }) public static final int DEFAULT_MAX_LENGTH = 30;

  public List<ValidationError> check(Sentence line) {
    List<ValidationError> result = new ArrayList<ValidationError>();
    if (line.content.length() > maxLength) {
      result.add(new ValidationError("The length of the line exceeds the maximum " + String.valueOf(line.content.length()) + ".", line));
    }
    return result;
  }
