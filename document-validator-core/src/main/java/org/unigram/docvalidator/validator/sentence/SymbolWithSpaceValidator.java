...
public class SymbolWithSpaceValidator implements SentenceValidator {
  ...
  public boolean initialize(ValidatorConfiguration validatorConf, CharacterTable characterConf) throws DocumentValidatorException {
  ...
  private ValidationError validateCharacter(Sentence sentence, String name) {
    ...
    if (position != -1) {
      if (position > 0 && character.isNeedBeforeSpace() && !Character.isWhitespace(sentenceStr.charAt(position - 1))) {
        return new ValidationError("Need white space before symbol (" + character.getName() + "): " + sentenceStr.charAt(position) + ".", sentence);
      } else {
        if (position < sentenceStr.length() - 1 && character.isNeedAfterSpace() && !Character.isWhitespace(sentenceStr.charAt(position + 1))) {
          return new ValidationError("Need white space after symbol (" + character.getName() + "): " + sentenceStr.charAt(position), sentence);
        }
      }
    }
    return null;
