...
public class SpaceBeginningOfSentenceValidator implements SentenceValidator {
  public List<ValidationError> check(Sentence sentence) {
    ...
    if (!sentence.isFirstSentence && content.length() > 0 && !String.valueOf(content.charAt(0)).equals(" ")) {
      result.add(new ValidationError("Space not exist the beginning of sentence.", sentence));
    }
    return result;
  }
