...
public class SectionLengthValidator extends SectionValidator {
  ...
  @Override protected List<ValidationError> check(Section section) {
    List<ValidationError> validationErrors = new ArrayList<ValidationError>();
    int sectionCharNumber = 0;
    for (Iterator<Paragraph> paraIterator = section.getParagraphs(); paraIterator.hasNext(); ) {
      Paragraph currentPraParagraph = paraIterator.next();
      for (Iterator<Sentence> sentenceIterator = currentPraParagraph.getSentences(); sentenceIterator.hasNext(); ) {
        Sentence sentence = sentenceIterator.next();
        sectionCharNumber += sentence.content.length();
      }
      if (sectionCharNumber > maxSectionCharNumber) {
        ValidationError error = new ValidationError("The number of the character exceeds the maximum \"" + String.valueOf(sectionCharNumber) + "\".", section.getHeaderContent(0));
  ...
  @Override public boolean loadConfiguration(ValidatorConfiguration conf, CharacterTable characterTable) {
    if (conf.getAttribute("max_char_number") == null) {
      ...
      this.maxSectionCharNumber = Integer.valueOf(conf.getAttribute("max_char_number"));
    }
    return true;
  }
