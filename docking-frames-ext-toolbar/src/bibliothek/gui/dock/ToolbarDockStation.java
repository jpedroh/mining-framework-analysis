 ...
 */
public class ToolbarDockStation extends AbstractDockableStation implements PositionedDockStation, OrientedDockStation, ToolbarInterface, ToolbarElementInterface {
  /**
  ...
  public PlaceholderMap getPlaceholders(final Map<Dockable, Integer> children) {
    ...
    return dockables.toMap(new PlaceholderListItemAdapter<Dockable, Dockable>() {
      @Override public ConvertedPlaceholderListItem convert(int index, Dockable dockable) {
  ...
  public void setPlaceholders(PlaceholderMap map, final Map<Integer, Dockable> children) {
    ...
    try {
      ...
      next.read(map, new PlaceholderListItemAdapter<Dockable, Dockable>() {
        ...
        @Override public Dockable convert(ConvertedPlaceholderListItem item) {
        ...
        @Override public void added(Dockable dockable) {
          try {
  ...
  @Override public StationDropOperation prepareDrop(int mouseX, int mouseY, int titleX, int titleY, boolean checkOverrideZone, Dockable dockable) {
  ...
  private void move(Dockable dockable, int indexWhereInsert) {
    System.out.println(this.toString() + "## move() ## ==> Index: " + indexWhereInsert);
    DockController controller = getController();
    try {
      if (controller != null) {
        controller.freezeLayout();
  ...
  @Override public <D extends Dockable & DockStation> boolean isInOverrideZone(int x, int y, D invoker, Dockable drop) {
    return false;
  }
  ...
  private void add(Dockable dockable, int index) {
    System.out.println(this.toString() + "## add(Dockable dockable, int index)##");
...
>>>>>>> /usr/src/app/output/benoker/dockingframes/ee7fa99567bdcb72ebca8df25a6b110b5ac2b93d/docking-frames-ext-toolbar/src/bibliothek/gui/dock/ToolbarDockStation.java/right.java
  ...
  private void insertAt(Dockable dockable, int index) {
    ...
    basePanel.getContentPane().setBounds(0, 0, basePanel.getContentPane().getPreferredSize().width, basePanel.getContentPane().getPreferredSize().height);
    basePanel.setPreferredSize(new Dimension(basePanel.getContentPane().getPreferredSize().width, basePanel.getContentPane().getPreferredSize().height));
...
=======
    switch (orientation) {
      case VERTICAL:
      this.basePanel.getContentPane().setLayout(new BoxLayout(basePanel.getContentPane(), BoxLayout.Y_AXIS));
      basePanel.getContentPane().setBounds(0, 0, basePanel.getContentPane().getPreferredSize().width, basePanel.getContentPane().getPreferredSize().height);
      basePanel.setPreferredSize(new Dimension(basePanel.getContentPane().getPreferredSize().width, basePanel.getContentPane().getPreferredSize().height));
      break;
      case HORIZONTAL:
      this.basePanel.getContentPane().setLayout(new BoxLayout(basePanel.getContentPane(), BoxLayout.X_AXIS));
      basePanel.getContentPane().setBounds(0, 0, basePanel.getContentPane().getPreferredSize().width, basePanel.getContentPane().getPreferredSize().height);
      basePanel.setPreferredSize(new Dimension(basePanel.getContentPane().getPreferredSize().width, basePanel.getContentPane().getPreferredSize().height));
      break;
    }
...
>>>>>>> /usr/src/app/output/benoker/dockingframes/ee7fa99567bdcb72ebca8df25a6b110b5ac2b93d/docking-frames-ext-toolbar/src/bibliothek/gui/dock/ToolbarDockStation.java/right.java
  ...
  protected class OverpaintablePanelBase extends OverpaintablePanel {
    ...
    /**
		 ...
		 */
    @SuppressWarnings(value = { "serial" }) private JPanel basePane = new SizeFixedPanel() {
      @Override public Dimension getPreferredSize() {
        Dimension titlePreferredSize = getTitlePane().getPreferredSize();
        Dimension contentPreferredSize = getContentPane().getPreferredSize();
        Dimension basePreferredSize = null;
        switch (ToolbarDockStation.this.getPosition()) {
          case NORTH:
          case SOUTH:
          basePreferredSize = new Dimension(contentPreferredSize.width + titlePreferredSize.width, contentPreferredSize.height);
          break;
          case WEST:
          case EAST:
          basePreferredSize = new Dimension(contentPreferredSize.width, titlePreferredSize.height + contentPreferredSize.height);
          break;
          case CENTER:
    ...
    public OverpaintablePanelBase() {
      basePane.setBorder(new CompoundBorder(new EtchedBorder(), new EmptyBorder(new Insets(5, 5, 5, 5))));
    ...
    @Override protected void paintOverlay(Graphics g) {
      ...
      if (indexBeneathMouse != null) {
        Rectangle rect = dockables.dockables().get(indexBeneathMouse).getComponent().getBounds();
        if (rect != null) {
      ...
      if (getDockables().size() != 0) {
        switch (getOrientation()) {
          ...
          for (Dockable dockable : getDockables()) {
            currentSize = dockable.getComponent().getPreferredSize();
            optimalSize.height += currentSize.height;
            if (currentSize.width > optimalSize.width) {
              optimalSize.width = currentSize.width;
            }
          }
          System.out.println("Computation... " + optimalSize.height + " / " + optimalSize.width);
          return optimalSize;
          case HORIZONTAL:
