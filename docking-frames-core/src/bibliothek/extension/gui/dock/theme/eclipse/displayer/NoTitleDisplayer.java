 ...
 */
@Deprecated @Todo(compatibility = Compatibility.BREAK_MINOR, priority = Priority.ENHANCEMENT, target = Version.VERSION_1_1_2, description = "remove this class") public class NoTitleDisplayer extends ConfiguredBackgroundPanel implements DockableDisplayer, InvisibleTabPane, BorderedComponent {
  ...
  public NoTitleDisplayer(DockStation station, Dockable dockable, TitleBar bar) {
    ...
    painter = new PropertyValue<TabPainter>(EclipseTheme.TAB_PAINTER) {
      @Override protected void valueChanged(TabPainter oldValue, TabPainter newValue) {
        updateFullBorder();
        updateInvisibleTab();
      }
    };
