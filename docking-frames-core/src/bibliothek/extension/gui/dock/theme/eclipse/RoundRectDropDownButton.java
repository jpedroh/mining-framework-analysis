 ...
 */
@ColorCodes(value = { "action.button.text", "action.button.text.disabled" }) public class RoundRectDropDownButton extends JComponent implements FocusAwareComponent {
  ...
  private void doPaintBackground(Graphics g) {
    ...
    if (model.isMousePressed()) {
      ...
      if (model.isSelected() || model.isMouseInside()) {
        border = Colors.diffMirror(background, 0.3);
