 ...
 */
@ColorCodes(value = { "stack.tab.border", "stack.tab.border.selected", "stack.tab.border.selected.focused", "stack.tab.border.selected.focuslost", "stack.tab.border.disabled", "stack.tab.top", "stack.tab.top.selected", "stack.tab.top.selected.focused", "stack.tab.top.selected.focuslost", "stack.tab.top.disabled", "stack.tab.bottom", "stack.tab.bottom.selected", "stack.tab.bottom.selected.focused", "stack.tab.bottom.selected.focuslost", "stack.tab.bottom.disabled", "stack.tab.text", "stack.tab.text.selected", "stack.tab.text.selected.focused", "stack.tab.text.selected.focuslost", "stack.tab.text.disabled", "stack.border" }) public abstract class BaseTabComponent extends ConfiguredBackgroundPanel implements TabComponent {
  ...
  public void bind() {
    if (buttons != null) {
      buttons.set(dockable, new EclipseDockActionSource(pane.getTheme(), dockable.getGlobalActionOffers(), dockable, true));
    }
  ...
  @Override public synchronized void removeMouseListener(MouseListener l) {
  ...
  @Override public synchronized void removeMouseMotionListener(MouseMotionListener l) {
  ...
  @Override public Dimension getPreferredSize() {
    ...
    if (orientation.isHorizontal()) {
      return new Dimension(labelSize.width + buttonSize.width + labelInsets.left + labelInsets.right + buttonInsets.left + buttonInsets.right, Math.max(labelSize.height + labelInsets.top + labelInsets.bottom, buttonSize.height + buttonInsets.top + buttonInsets.bottom));
    } else {
      return new Dimension(Math.max(labelSize.width + labelInsets.left + labelInsets.right, buttonSize.width + buttonInsets.left + buttonInsets.right), labelSize.height + buttonSize.height + labelInsets.top + labelInsets.bottom + buttonInsets.top + buttonInsets.bottom);
    }
  }
  ...
  private class BaseTabFont extends TabFont {
    ...
    @Override protected void changed(FontModifier oldValue, FontModifier newValue) {
      updateFont();
    }
  }
