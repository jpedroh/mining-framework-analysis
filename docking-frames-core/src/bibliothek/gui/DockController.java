...
public class DockController {
  ...
  protected final void initiate(DockControllerFactory factory, ControllerSetupCollection setup) {
    ...
    properties.set(SplitDockStation.MAXIMIZE_ACCELERATOR, KeyStroke.getKeyStroke(KeyEvent.VK_M, InputEvent.CTRL_DOWN_MASK));
    properties.set(DockFrontend.HIDE_ACCELERATOR, KeyStroke.getKeyStroke(KeyEvent.VK_F4, InputEvent.CTRL_DOWN_MASK));
    properties.set(DockableSelector.INIT_SELECTION, KeyStroke.getKeyStroke(KeyEvent.VK_E, KeyEvent.CTRL_DOWN_MASK | KeyEvent.SHIFT_DOWN_MASK));
  ...
  public void addRepresentative(DockElementRepresentative representative) {
    DockControllerRepresentativeListener[] listeners = componentToDockElementsListeners.toArray(new DockControllerRepresentativeListener[componentToDockElementsListeners.size()]);
  ...
  public void setAtLeastFocusedDockable(Dockable focusedDockable, Component component) {
    ...
    if (current == null) {
      ...
      if (!DockUtilities.isAncestor(focusedDockable, current)) {
        setFocusedDockable(focusedDockable, component, false);
      }
    }
  }
  ...
  protected DockTitleBindingListener[] dockTitleBindingListeners() {
    return dockTitleBindingListeners.toArray(new DockTitleBindingListener[dockTitleBindingListeners.size()]);
  }
  ...
  private class DockableSelectionObserver extends DockRegisterAdapter {
    ...
    private DockStationListener listener = new DockStationAdapter() {
      @Override public void dockableSelected(DockStation station, Dockable oldSelected, Dockable newSelected) {
        fireDockableSelected(station, oldSelected, newSelected);
      }
    };

    @Override public void dockStationRegistered(DockController controller, DockStation station) {
  ...
  private class DockTitleObserver extends DockRegisterAdapter implements DockTitleBindingListener {
    ...
    private DockableListener dockableListener = new DockableAdapter() {
      @Override public void titleBound(Dockable dockable, DockTitle title) {
        titles.add(title);
        title.bind();
      ...
      @Override public void titleUnbound(Dockable dockable, DockTitle title) {
        titles.remove(title);
        title.unbind();
    ...
    @Override public void dockableRegistering(DockController controller, Dockable dockable) {
