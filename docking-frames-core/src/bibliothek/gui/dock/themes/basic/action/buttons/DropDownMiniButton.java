...
public class DropDownMiniButton extends MiniButton<BasicDropDownButtonModel> {
  ...
  public DropDownMiniButton(BasicDropDownButtonHandler handler) {
    ...
    BasicDropDownButtonModel model = new BasicDropDownButtonModel(this, handler, handler) {
      ...
      @Override protected boolean inDropDownArea(int x, int y) {
  ...
  private void doPaintForeground(Graphics g) {
    ...
    if (getModel().getOrientation().isHorizontal()) {
      ...
      drop.paintIcon(this, g, (int) (factor * (insets.left + insets.right + iconWidth + dropWidth / 2) - dropWidth / 2), insets.top + (getHeight() - insets.top - insets.bottom - dropIcon.getIconHeight()) / 2);
    } else {
      ...
      drop.paintIcon(this, g, insets.left + (getWidth() - insets.left - insets.right - dropIcon.getIconWidth()) / 2, (int) (factor * (insets.top + insets.bottom + iconHeight + dropHeight / 2) - dropHeight / 2));
    }
