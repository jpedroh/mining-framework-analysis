...
public class AbstractDockTitle extends ConfiguredBackgroundPanel implements DockTitle {
  ...
  @Override public void setBackground(Color fg) {
  ...
  @Override public Dimension getPreferredSize() {
    ...
    if (orientation.isHorizontal()) {
      ...
      preferred = new Dimension(width + iconTextGap + insets.left + insets.right, height + insets.top + insets.bottom);
    } else {
      int width = 0;
      int height = 0;
      if (icon != null) {
      ...
      preferred = new Dimension(width + insets.left + insets.right, height + iconTextGap + insets.top + insets.bottom);
    }
