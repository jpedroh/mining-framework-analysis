...
public class ScreenDockStation extends AbstractDockStation {
  ...
  public static final PropertyKey<ScreenDockWindowFactory> WINDOW_FACTORY = new PropertyKey<ScreenDockWindowFactory>("ScreenDockStation.window_factory", new ConstantPropertyFactory<ScreenDockWindowFactory>(new DefaultScreenDockWindowFactory()), true);
  ...
  public static final PropertyKey<MagnetStrategy> MAGNET_STRATEGY = new PropertyKey<MagnetStrategy>("ScreenDockStation.magnet_strategy", new ConstantPropertyFactory<MagnetStrategy>(new DefaultMagnetStrategy()) {
...
<<<<<<< /usr/src/app/output/benoker/dockingframes/7792989b18d9aa22f844dd8a0ba8b56c8021d1e5/docking-frames-core/src/bibliothek/gui/dock/ScreenDockStation.java/left.java
...
>>>>>>> /usr/src/app/output/benoker/dockingframes/7792989b18d9aa22f844dd8a0ba8b56c8021d1e5/docking-frames-core/src/bibliothek/gui/dock/ScreenDockStation.java/right.java
  ...
  private PropertyValue<BoundaryRestriction> restriction = new PropertyValue<BoundaryRestriction>(ScreenDockStation.BOUNDARY_RESTRICTION) {
    @Override protected void valueChanged(BoundaryRestriction oldValue, BoundaryRestriction newValue) {
      checkWindowBoundaries();
    }
  };

  /** a factory used to create new windows for this station */
  private PropertyValue<ScreenDockWindowFactory> windowFactory = new PropertyValue<ScreenDockWindowFactory>(ScreenDockStation.WINDOW_FACTORY) {
    @Override protected void valueChanged(ScreenDockWindowFactory oldValue, ScreenDockWindowFactory newValue) {
      updateWindows(true);
    }
  };

  /** a strategy for telling {@link #windowFactory} how to create new windows */
  private PropertyValue<ScreenDockWindowConfiguration> windowConfiguration = new PropertyValue<ScreenDockWindowConfiguration>(ScreenDockStation.WINDOW_CONFIGURATION) {
    @Override protected void valueChanged(ScreenDockWindowConfiguration oldValue, ScreenDockWindowConfiguration newValue) {
      updateWindows(true);
    }
  };

  /** the current fullscreen strategy */
  private PropertyValue<ScreenDockFullscreenStrategy> fullscreenStrategy = new PropertyValue<ScreenDockFullscreenStrategy>(ScreenDockStation.FULL_SCREEN_STRATEGY) {
    @Override protected void valueChanged(ScreenDockFullscreenStrategy oldValue, ScreenDockFullscreenStrategy newValue) {
  ...
  private PropertyValue<ScreenDropSizeStrategy> dropSizeStrategy = new PropertyValue<ScreenDropSizeStrategy>(DROP_SIZE_STRATEGY) {
    @Override protected void valueChanged(ScreenDropSizeStrategy oldValue, ScreenDropSizeStrategy newValue) {
      if (oldValue != null) {
        oldValue.uninstall(ScreenDockStation.this);
      }
      if (newValue != null) {
  ...
  public PlaceholderMap getPlaceholders(final Map<Dockable, Integer> children) {
    ...
    return dockables.toMap(new PlaceholderListItemAdapter<Dockable, ScreenDockWindowHandle>() {
      @Override public ConvertedPlaceholderListItem convert(int index, ScreenDockWindowHandle dockable) {
  ...
  public void setPlaceholders(PlaceholderMap map, final Map<Integer, Dockable> children) {
    ...
    try {
      ...
      next.read(map, new PlaceholderListItemAdapter<Dockable, ScreenDockWindowHandle>() {
        ...
        @Override public ScreenDockWindowHandle convert(ConvertedPlaceholderListItem item) {
        ...
        @Override public void added(ScreenDockWindowHandle dockable) {
          try {
  ...
  private boolean checkDropInfo(DropInfo dropInfo) {
    if (dropInfo.combine != null) {
      if (!accept(dropInfo.dockable) || !dropInfo.dockable.accept(this, dropInfo.combine.getDockable()) || !dropInfo.combine.getDockable().accept(this, dropInfo.dockable) || !getController().getAcceptance().accept(this, dropInfo.combine.getDockable(), dropInfo.dockable)) {
        return false;
      }
    } else {
      if (!accept(dropInfo.dockable) || !dropInfo.dockable.accept(this) || !getController().getAcceptance().accept(this, dropInfo.dockable)) {
        return false;
      }
    }
    return true;
  ...
  private void combine(CombinerSource source, CombinerTarget target, DockableProperty property) {
    ...
    Dockable valid = combiner.combine(new CombinerSourceWrapper(source) {
      ...
      @Override public Dockable getOld() {
        return old;
      }
    }, target);
    if (property != null) {
