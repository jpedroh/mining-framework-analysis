...
>>>>>>> /usr/src/app/output/benoker/dockingframes/928dc5c2f90afc62f8a1b8219859401fffd65bca/docking-frames-core/src/bibliothek/gui/dock/station/split/Leaf.java/right.java
  ...
  @Override public void onSpanResize() {
    resetDisplayerBounds();
  }
  ...
  @Override public PutInfo getPut(int x, int y, double factorW, double factorH, Dockable drop) {
    ...
    if (x > bounds.x + sideSnapSize * bounds.width && x < bounds.x + bounds.width - sideSnapSize * bounds.width && y > bounds.y + sideSnapSize * bounds.height && y < bounds.y + bounds.height - sideSnapSize * bounds.height) {
      centered = true;
  ...
  @Override public boolean insert(SplitDockPathProperty property, int depth, Dockable dockable) {
    if (depth < property.size()) {
      ...
      if (node.getLocation() == SplitDockPathProperty.Location.LEFT || node.getLocation() == SplitDockPathProperty.Location.RIGHT) {
