...
public class StatementEvaluator extends DelegateExpressionEvaluator {
  @Override public Value caseAAlwaysStm(AAlwaysStm node, Context ctxt) throws AnalysisException {
  ...
  @Override public Value caseAAssignmentStm(AAssignmentStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    ...
    if (Settings.dialect == Dialect.VDM_RT && Properties.rt_log_instvarchanges) {
      ...
      if (self == null) {
        RTLogger.log(new RTExtendedTextMessage("InstVarChange -> instnm: \"" + node.getTarget().toString() + "\"" + " val: \"" + noquotes + "\"" + " objref: nil" + " id: " + BasicSchedulableThread.getThread(Thread.currentThread()).getId()));
      } else {
        RTLogger.log(new RTExtendedTextMessage("InstVarChange -> instnm: \"" + node.getTarget().toString() + "\"" + " val: \"" + noquotes + "\"" + " objref: " + self.objectReference + " id: " + BasicSchedulableThread.getThread(Thread.currentThread()).getId()));
      }
    }
    return new VoidValue();
  }

  @Override public Value caseAAtomicStm(AAtomicStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    ...
    try {
      ...
      for (int i = 0; i < size; i++) {
        ...
        target.set(node.getLocation(), values.get(i), ctxt);
  ...
  @Override public Value caseACallObjectStm(ACallObjectStm node, Context ctxt) throws AnalysisException {
    ...
    try {
      ...
      if (v == null) {
        VdmRuntimeError.abort(node.getField().getLocation(), 4035, "Object has no field: " + adjfield.getName(), ctxt);
      }
      v = v.deref();
  ...
  @Override public Value caseACallStm(ACallStm node, Context ctxt) throws AnalysisException {
    Breakpoint breakpoint = BreakpointManager.getBreakpoint(node);
    breakpoint.check(node.getLocation(), ctxt);
    boolean endstop = breakpoint.catchReturn(ctxt);
    try {
  ...
  @Override public Value caseACasesStm(ACasesStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
  ...
  @Override public Value caseAClassInvariantStm(AClassInvariantStm node, Context ctxt) throws AnalysisException {
  ...
  @Override public Value caseACyclesStm(ACyclesStm node, Context ctxt) throws AnalysisException {
  ...
  @Override public Value caseADurationStm(ADurationStm node, Context ctxt) throws AnalysisException {
    node.getLocation().hit();
  ...
  @Override public Value caseAElseIfStm(AElseIfStm node, Context ctxt) throws AnalysisException {
    return evalElseIf(node, node.getLocation(), node.getElseIf(), node.getThenStm(), ctxt);
  }

  @Override public Value caseAErrorStm(AErrorStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    return VdmRuntimeError.abort(node.getLocation(), 4036, "ERROR statement reached", ctxt);
  }

  @Override public Value caseAExitStm(AExitStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    Value v = null;
    ...
    throw new ExitException(v, node.getLocation(), ctxt);
  }

  @Override public Value caseAForAllStm(AForAllStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    try {
  ...
  @Override public Value caseAForIndexStm(AForIndexStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    try {
      ...
      if (bval == 0) {
        VdmRuntimeError.abort(node.getLocation(), 4038, "Loop, from " + fval + " to " + tval + " by " + bval + " will never terminate", ctxt);
      }
      for (long value = fval; bval > 0 && value <= tval || bval < 0 && value >= tval; value += bval) {
  ...
  @Override public Value caseAForPatternBindStm(AForPatternBindStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    try {
      ...
      if (node.getPatternBind().getPattern() != null) {
        for (Value val : values) {
          try {
            ...
            evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(node.getLocation().getModule()).getNamedValues(node.getPatternBind().getPattern(), val, ctxt));
            Value rv = node.getStatement().apply(VdmRuntime.getStatementEvaluator(), evalContext);
            if (!rv.isVoid()) {
              return rv;
            }
        ...
        if (node.getPatternBind().getBind() instanceof ASetBind) {
          ...
          for (Value val : values) {
            try {
              ...
              evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(node.getLocation().getModule()).getNamedValues(setbind.getPattern(), val, ctxt));
              Value rv = node.getStatement().apply(VdmRuntime.getStatementEvaluator(), evalContext);
              if (!rv.isVoid()) {
                return rv;
              }
          ...
          if (node.getPatternBind().getBind() instanceof ASeqBind) {
            ...
            for (Value val : values) {
              try {
                if (!seq.contains(val)) {
                  VdmRuntimeError.abort(node.getLocation(), 4039, "Seq bind does not contain value " + val, ctxt);
                }
                Context evalContext = new Context(ctxt.assistantFactory, node.getLocation(), "for seq bind", ctxt);
                evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(node.getLocation().getModule()).getNamedValues(seqbind.getPattern(), val, ctxt));
                Value rv = node.getStatement().apply(VdmRuntime.getStatementEvaluator(), evalContext);
                if (!rv.isVoid()) {
                  return rv;
                }
            ...
            for (Value val : values) {
              try {
                ...
                evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(node.getLocation().getModule()).getNamedValues(typebind.getPattern(), converted, ctxt));
                Value rv = node.getStatement().apply(VdmRuntime.getStatementEvaluator(), evalContext);
                if (!rv.isVoid()) {
                  return rv;
                }
  ...
  @Override public Value caseAIfStm(AIfStm node, Context ctxt) throws AnalysisException {
    return evalIf(node, node.getLocation(), node.getIfExp(), node.getThenStm(), node.getElseIf(), node.getElseStm(), ctxt);
  }

  @Override public Value caseALetBeStStm(ALetBeStStm node, Context ctxt) throws AnalysisException {
    return evalLetBeSt(node, node.getLocation(), node.getDef(), node.getSuchThat(), node.getStatement(), 4040, "statement", ctxt);
  }

  @Override public Value caseALetStm(ALetStm node, Context ctxt) throws AnalysisException {
    return evalLet(node, node.getLocation(), node.getLocalDefs(), node.getStatement(), "statement", ctxt);
  }

  @Override public Value caseANotYetSpecifiedStm(ANotYetSpecifiedStm node, Context ctxt) throws AnalysisException {
    return evalANotYetSpecified(node, node.getLocation(), 4041, "statement", ctxt);
  }

  @Override public Value caseAReturnStm(AReturnStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    if (node.getExpression() == null) {
  ...
  @Override public Value caseABlockSimpleBlockStm(ABlockSimpleBlockStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
  ...
  @Override public Value caseANonDeterministicSimpleBlockStm(ANonDeterministicSimpleBlockStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    return this.evalBlock(node, ctxt);
  }

  @Override public Value caseASkipStm(ASkipStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    return new VoidValue();
  }

  @Override public Value caseASpecificationStm(ASpecificationStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    return VdmRuntimeError.abort(node.getLocation(), 4047, "Cannot execute specification statement", ctxt);
  }

  @Override public Value caseAStartStm(AStartStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    try {
  ...
  @Override public Value caseAStopStm(AStopStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    try {
      Value value = node.getObj().apply(VdmRuntime.getStatementEvaluator(), ctxt);
      if (value.isType(SetValue.class)) {
  ...
  private void stop(ObjectValue target, ILexLocation location, Context ctxt) throws ValueException {
    ...
    if (target.getCPU() != ctxt.threadState.CPU) {
      throw new ContextException(4161, "Cannot stop object " + target.objectReference + " on CPU " + target.getCPU().getName() + " from CPU " + ctxt.threadState.CPU, location, ctxt);
    }
    ...
    if (count == 0) {
      throw new ContextException(4160, "Object #" + target.objectReference + " is not running a thread to stop", location, ctxt);
    }
  }

  @Override public Value caseASubclassResponsibilityStm(ASubclassResponsibilityStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    return VdmRuntimeError.abort(node.getLocation(), 4048, "\'is subclass responsibility\' statement reached", ctxt);
  }

  @Override public Value caseATixeStm(ATixeStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    Value rv = null;
    try {
      rv = node.getBody().apply(VdmRuntime.getStatementEvaluator(), ctxt);
      ...
      while (true) {
        ...
        try {
          for (ATixeStmtAlternative tsa : node.getTraps()) {
            rv = eval(tsa, node.getLocation(), exval, ctxt);
            if (rv != null) {
              return rv;
            }
          }
  ...
  @Override public Value caseATrapStm(ATrapStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    Value rv = null;
    try {
      rv = node.getBody().apply(VdmRuntime.getStatementEvaluator(), ctxt);
      ...
      try {
        if (node.getPatternBind().getPattern() != null) {
          ...
          evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(node.getLocation().getModule()).getNamedValues(node.getPatternBind().getPattern(), exval, ctxt));
          ...
          if (node.getPatternBind().getBind() instanceof ASetBind) {
            ...
            if (set.contains(exval)) {
              ...
              evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(node.getLocation().getModule()).getNamedValues(setbind.getPattern(), exval, ctxt));
              rv = node.getWith().apply(VdmRuntime.getStatementEvaluator(), evalContext);
            } else {
              throw e;
            }
            ...
            if (node.getPatternBind().getBind() instanceof ASeqBind) {
              ...
              if (seq.contains(exval)) {
                ...
                evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(node.getLocation().getModule()).getNamedValues(seqbind.getPattern(), exval, ctxt));
                rv = node.getWith().apply(VdmRuntime.getStatementEvaluator(), evalContext);
              } else {
                throw e;
              }
  ...
  @Override public Value caseAWhileStm(AWhileStm node, Context ctxt) throws AnalysisException {
    BreakpointManager.getBreakpoint(node).check(node.getLocation(), ctxt);
    try {
  ...
  @Override public Value caseAPeriodicStm(APeriodicStm node, Context ctxt) throws AnalysisException {
    ...
    for (int i = 0; i < node.getArgs().size(); i++) {
      ...
      try {
        ...
        VdmRuntimeError.abort(node.getLocation(), 4157, "Expecting +ive integer in periodic argument " + (i + 1) + ", was " + argval, ctxt);
      }
    }
    if (node.getPeriod() == 0) {
  ...
  @Override public Value caseASporadicStm(ASporadicStm node, Context ctxt) throws AnalysisException {
    ...
    for (PExp arg : node.getArgs()) {
      ...
      try {
        ...
        if (value < 0) {
          VdmRuntimeError.abort(node.getLocation(), 4157, "Expecting +ive integer in sporadic argument " + (i + 1) + ", was " + value, ctxt);
        }
        if (i == MINDELAY) {
        ...
        VdmRuntimeError.abort(node.getLocation(), 4157, "Expecting +ive integer in sporadic argument " + (i + 1) + ", was " + argval, ctxt);
      }
      i++;
    }
    return null;
  ...
  @Override public Value caseAIdentifierStateDesignator(AIdentifierStateDesignator node, Context ctxt) throws AnalysisException {
    return ctxt.lookup(node.getName().getExplicit(true));
  }

  @Override public Value caseAFieldStateDesignator(AFieldStateDesignator node, Context ctxt) throws AnalysisException {
    Value result = null;
    try {
      ...
      if (result instanceof ObjectValue && node.getObjectfield() != null) {
        ...
        if (result instanceof RecordValue) {
  ...
  @Override public Value caseAFieldObjectDesignator(AFieldObjectDesignator node, Context ctxt) throws AnalysisException {
    try {
  ...
  @Override public Value caseAMapSeqStateDesignator(AMapSeqStateDesignator node, Context ctxt) throws AnalysisException {
    Value result = null;
    try {
  ...
  @Override public Value caseAApplyObjectDesignator(AApplyObjectDesignator node, Context ctxt) throws AnalysisException {
    try {
  ...
  @Override public Value caseAAnnotatedStm(AAnnotatedStm node, Context ctxt) throws AnalysisException {
  ...
  @Override public Value caseAIdentifierObjectDesignator(AIdentifierObjectDesignator node, Context ctxt) throws AnalysisException {
    return node.getExpression().apply(VdmRuntime.getExpressionEvaluator(), ctxt);
  }

  @Override public Value caseANewObjectDesignator(ANewObjectDesignator node, Context ctxt) throws AnalysisException {
    return node.getExpression().apply(VdmRuntime.getExpressionEvaluator(), ctxt);
  }

  @Override public Value caseASelfObjectDesignator(ASelfObjectDesignator node, Context ctxt) throws AnalysisException {
    return ctxt.lookup(node.getSelf());
  }

  private Value evalBlock(SSimpleBlockStm node, Context ctxt) throws AnalysisException {
  ...
  public Value eval(ATixeStmtAlternative node, ILexLocation location, Value exval, Context ctxt) throws AnalysisException {
    Context evalContext = null;
    try {
      if (node.getPatternBind().getPattern() != null) {
        evalContext = new Context(ctxt.assistantFactory, location, "tixe pattern", ctxt);
        evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(location.getModule()).getNamedValues(node.getPatternBind().getPattern(), exval, ctxt));
      } else {
        if (node.getPatternBind().getBind() instanceof ASetBind) {
          ASetBind setbind = (ASetBind) node.getPatternBind().getBind();
          ValueSet set = setbind.getSet().apply(VdmRuntime.getStatementEvaluator(), ctxt).setValue(ctxt);
          if (set.contains(exval)) {
            ...
            evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(location.getModule()).getNamedValues(setbind.getPattern(), exval, ctxt));
          } else {
            evalContext = null;
          }
        } else {
          if (node.getPatternBind().getBind() instanceof ASeqBind) {
            ...
            if (seq.contains(exval)) {
              ...
              evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(location.getModule()).getNamedValues(seqbind.getPattern(), exval, ctxt));
            } else {
              evalContext = null;
            }
          } else {
            ...
            evalContext.putList(ctxt.assistantFactory.createPPatternAssistant(location.getModule()).getNamedValues(typebind.getPattern(), converted, ctxt));
          }
        }
      }
    } catch (ValueException ve) {
  ...
  private Value eval(ACaseAlternativeStm node, Value val, Context ctxt) throws AnalysisException {
  ...
  public void start(AStartStm node, ObjectValue target, OperationValue op, Context ctxt) throws AnalysisException {
    if (op.body instanceof APeriodicStm) {
      ...
      try {
        ...
        ps.apply(VdmRuntime.getStatementEvaluator(), pctxt);
      }  finally {
      ...
      if (op.body instanceof ASporadicStm) {
        RootContext global = ClassInterpreter.getInstance().initialContext;
