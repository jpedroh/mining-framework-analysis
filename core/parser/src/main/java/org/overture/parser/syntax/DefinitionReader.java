...
public class DefinitionReader extends SyntaxReader {
  ...
  public List<PDefinition> readDefinitions() throws ParserException, LexException {
    ...
    while (lastToken().isNot(VDMToken.EOF) && lastToken().isNot(VDMToken.END)) {
      switch (lastToken().type) {
        ...
        break;
        case TRACES:
        if (dialect == Dialect.VDM_SL && Settings.release != Release.VDM_10) {
  ...
  private AAccessSpecifierAccessSpecifier readAccessSpecifier(boolean asyncOK, boolean pureOK) throws LexException, ParserException {
    if (dialect == Dialect.VDM_SL) {
...
>>>>>>> /usr/src/app/output/overturetool/overture/bffba8bbaba72042e85c88e333720221b2d54250/core/parser/src/main/java/org/overture/parser/syntax/DefinitionReader.java/right.java
  ...
  private PDefinition readFunctionDefinition(NameScope scope) throws ParserException, LexException {
  ...
  private PDefinition readExplicitFunctionDefinition(LexIdentifierToken funcName, NameScope scope, List<ILexNameToken> typeParams) throws ParserException, LexException {
    nextToken();
    ...
    while (lastToken().is(VDMToken.BRA)) {
      if (nextToken().isNot(VDMToken.KET)) {
        ...
        parameters.add(new Vector<PPattern>());
        nextToken();
      }
    }
  ...
  private PDefinition readImplicitFunctionDefinition(LexIdentifierToken funcName, NameScope scope, List<ILexNameToken> typeParams) throws ParserException, LexException {
    nextToken();
  ...
  public PDefinition readLocalDefinition(NameScope scope) throws ParserException, LexException {
  ...
  public PDefinition readValueDefinition(NameScope scope) throws ParserException, LexException {
  ...
  private PDefinition readExplicitOperationDefinition(LexIdentifierToken funcName) throws ParserException, LexException {
    nextToken();
    ...
    if (nextToken().isNot(VDMToken.KET)) {
      ...
      parameters = new Vector<PPattern>();
      nextToken();
    }
  ...
  private PDefinition readImplicitOperationDefinition(LexIdentifierToken funcName) throws ParserException, LexException {
    nextToken();
    PatternReader pr = getPatternReader();
    TypeReader tr = getTypeReader();
    List<APatternListTypePair> parameterPatterns = new Vector<APatternListTypePair>();
  ...
  public ASpecificationStm readSpecification(ILexLocation location, boolean postMandatory) throws ParserException, LexException {
  ...
  private PTraceCoreDefinition readCoreTraceDefinition() throws ParserException, LexException {
    ...
    switch (token.type) {
      ...
      if (!(stmt instanceof ACallStm) && !(stmt instanceof ACallObjectStm)) {
