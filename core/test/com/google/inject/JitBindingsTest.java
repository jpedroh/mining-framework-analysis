...
public class JitBindingsTest extends TestCase {
  ...
  private String jitInParentFailed(TypeLiteral<?> clazz) {
    String shortName = "JitBindingsTest$" + clazz.getRawType().getSimpleName();
    return "Explicit bindings are required and " + shortName + " would be bound in a parent injector.";
  }

  private String inChildMessage(Class<?> clazz) {
    return "Unable to create binding for " + "JitBindingsTest$" + clazz.getSimpleName() + " because it was already configured on one or more child injectors or private modules";
  }
