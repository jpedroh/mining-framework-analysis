...
public class QueryPostprocessDataContextTest extends MetaModelTestCase {
  ...
  public void testAggregateQueryRegularWhereClause() throws Exception {
    MockDataContext dc = new MockDataContext("sch", "tab", "1");
    Table table = dc.getDefaultSchema().getTables()[0];
    assertSingleRowResult("Row[values=[3]]", dc.query().from(table).selectCount().where("baz").eq("world").execute());
  }
  ...
  public void testApplyFunctionToNullValues() throws Exception {
    QueryPostprocessDataContext dataContext = new QueryPostprocessDataContext() {
      @Override public DataSet materializeMainSchemaTable(Table table, Column[] columns, int maxRows) {
        if (table == table1) {
    ...
    DataSet dataSet = dataContext.query().from(TABLE_CONTRIBUTOR).select(FunctionType.SUM, COLUMN_CONTRIBUTOR_COUNTRY).select(COLUMN_CONTRIBUTOR_NAME).groupBy(COLUMN_CONTRIBUTOR_NAME).orderBy(COLUMN_CONTRIBUTOR_NAME).execute();
  ...
  public void testAggregateQueryWhereClauseExcludingAll() throws Exception {
    MockDataContext dc = new MockDataContext("sch", "tab", "1");
    assertSingleRowResult("Row[values=[0]]", dc.query().from("tab").selectCount().where("baz").eq("non_existing_value").execute());
  }
  ...
  public void testInformationSchema() throws Exception {
    ...
    assertEquals("[Table[name=tables,type=TABLE,remarks=null], Table[name=columns,type=TABLE,remarks=null], Table[name=relationships,type=TABLE,remarks=null]]", Arrays.toString(informationSchema.getTables()));
    ...
    assertEquals("[Column[name=primary_table,columnNumber=0,type=VARCHAR,nullable=false,nativeType=null,columnSize=null], " + "Column[name=primary_column,columnNumber=1,type=VARCHAR,nullable=false,nativeType=null,columnSize=null], " + "Column[name=foreign_table,columnNumber=2,type=VARCHAR,nullable=false,nativeType=null,columnSize=null], " + "Column[name=foreign_column,columnNumber=3,type=VARCHAR,nullable=false,nativeType=null,columnSize=null]]", Arrays.toString(relationshipsTable.getColumns()));
    ...
    Query q = new Query().select(tablesTable.getColumnByName("name")).select(columnsTable.getColumnByName("name")).select(columnsTable.getBooleanColumns()).from(joinFromItem);
    assertEquals("SELECT tables.name, columns.name, columns.nullable, columns.indexed " + "FROM information_schema.tables INNER JOIN information_schema.columns " + "ON tables.name = columns.table", q.toString());
  ...
  public void testOrderByWithoutSelecting() throws Exception {
    ...
    @SuppressWarnings(value = { "deprecation" }) TableModel tableModel = data.toTableModel();
  ...
  public void testGroupByWithoutSelecting() throws Exception {
    ...
    q.orderBy(roleColumn);
    assertEquals("SELECT SUM(r.project_id) FROM MetaModelSchema.role r GROUP BY r.name ORDER BY r.name ASC", q.toString());
    DataContext dc = getDataContext();
    DataSet data = dc.executeQuery(q);
    assertEquals(1, data.getSelectItems().length);
  ...
  public void testSimpleGroupBy() throws Exception {
    ...
    assertEquals("SELECT r.name, COUNT(*) AS c, SUM(r.contributor_id) FROM MetaModelSchema.role r WHERE r.name = \'founder\' GROUP BY r.name", q.toString());
    data = dc.executeQuery(q);
  ...
  public void testSimpleHaving() throws Exception {
    ...
    assertEquals("SELECT c.name, COUNT(c.contributor_id) AS my_count FROM MetaModelSchema.role c GROUP BY c.name HAVING COUNT(c.contributor_id) > 1 ORDER BY COUNT(c.contributor_id) ASC", q.toString());
  ...
  public void testHavingFunctionNotSelected() throws Exception {
    ...
    assertEquals("SELECT c.name FROM MetaModelSchema.role c GROUP BY c.name HAVING COUNT(c.contributor_id) > 3", q.toString());
    DataSet data = getDataContext().executeQuery(q);
    assertTrue(data.next());
  ...
  public void testCompiledQueryParameterInWhereClause() throws Exception {
    ...
    CompiledQuery compiledQuery = dc.query().from(table1).select("name").where(COLUMN_CONTRIBUTOR_COUNTRY).eq(param1).compile();
    try {
  ...
  public void testCompiledQueryParameterInSubQuery() throws Exception {
    ...
    final Query subQuery = dc.query().from(table1).select("name").where(COLUMN_CONTRIBUTOR_COUNTRY).eq(param1).toQuery();
    final FromItem subQueryFromItem = new FromItem(subQuery);
    final Query query = new Query().select(new SelectItem(subQuery.getSelectClause().getItem(0), subQueryFromItem)).from(subQueryFromItem);
  ...
  public void testCarthesianProductWithWhere() throws Exception {
    ...
    assertEquals("SELECT contributor.name, role.name FROM MetaModelSchema.contributor, MetaModelSchema.role WHERE contributor.contributor_id = role.contributor_id", q.toString());
    DataSet data = dc.executeQuery(q);
    assertEquals(2, data.getSelectItems().length);
    assertTrue(data.next());
  ...
  public void testSubSelectionAndInnerJoin() throws Exception {
    ...
    q.select(s2);
    q.from(fromItem);
    assertEquals("SELECT contributor.name, role.name FROM MetaModelSchema.contributor INNER JOIN MetaModelSchema.role ON contributor.contributor_id = role.contributor_id", q.toString());
    DataSet data = dc.executeQuery(q);
    assertEquals(2, data.getSelectItems().length);
    assertTrue(data.next());
    assertEquals("Row[values=[kasper, founder]]", data.getRow().toString());
  ...
  public void testSubquery() throws Exception {
    ...
    assertEquals("SELECT name AS e FROM (SELECT contributor.contributor_id, contributor.name, contributor.country FROM MetaModelSchema.contributor)", q2.toString());
    fromItem.setAlias("c");
    assertEquals("SELECT c.name AS e FROM (SELECT contributor.contributor_id, contributor.name, contributor.country FROM MetaModelSchema.contributor) c", q2.toString());
    DataContext dc = getDataContext();
    ...
    fromItem.setAlias("d");
    assertEquals("SELECT d.e AS f FROM (SELECT c.name AS e FROM (SELECT contributor.contributor_id, contributor.name, contributor.country FROM MetaModelSchema.contributor) c) d", q3.toString());
  ...
  public void testOrderBy() throws Exception {
    ...
    assertEquals("SELECT c.contributor_id, c.name, c.country FROM MetaModelSchema.contributor c ORDER BY c.country DESC, c.name ASC", q.toString());
    DataSet data = getDataContext().executeQuery(q);
    assertTrue(data.next());
  ...
  public void testExecuteCount() throws Exception {
    QueryPostprocessDataContext dc = new QueryPostprocessDataContext() {
      ...
      @Override protected Number executeCountQuery(Table table, List<FilterItem> whereItems, boolean functionApproximationAllowed) {
        return 1337;
      }
  ...
  public void testExecutePrimaryKeyLookupQuery() throws Exception {
    QueryPostprocessDataContext dc = new QueryPostprocessDataContext() {
      ...
      @Override protected Row executePrimaryKeyLookupQuery(Table table, List<SelectItem> selectItems, Column primaryKeyColumn, Object keyValue) {
  ...
  public void testQueryWithMultipleColumnsInExpression() {
    ...
    assertEquals("SELECT contributor.contributor_id, contributor.name, contributor.country, COUNT(*)" + " FROM MetaModelSchema.contributor", query3.toString());
  }
}