...
public class QueryParserTest extends TestCase {
  ...
  public void testQueryInLowerCase() throws Exception {
    Query q = MetaModelHelper.parseQuery(dc, "select a.foo as f from sch.tbl a inner join sch.tbl b on a.foo=b.foo order by a.foo asc");
    assertEquals("SELECT a.foo AS f FROM sch.tbl a INNER JOIN sch.tbl b ON a.foo = b.foo ORDER BY a.foo ASC", q.toSql());
  }
  ...
  public void testSelectEverythingFromJoin() throws Exception {
    ...
    assertEquals("SELECT a.foo, a.bar, a.baz, b.foo, b.bar, b.baz FROM sch.tbl a INNER JOIN sch.tbl b ON a.foo = b.foo", q.toSql());
    q = MetaModelHelper.parseQuery(dc, "SELECT a.foo, b.* FROM sch.tbl a INNER JOIN sch.tbl b ON a.foo = b.foo");
    assertEquals("SELECT a.foo, b.foo, b.bar, b.baz FROM sch.tbl a INNER JOIN sch.tbl b ON a.foo = b.foo", q.toSql());
  }
  ...
  public void testSimpleSelectFrom() throws Exception {
    ...
    assertNull("FROM item was an expression based item, which indicates it was not parsed", fromItem.getExpression());
    ...
    assertNull("SELECT item was an expression based item, which indicates it was not parsed", selectItem.getExpression());
  ...
  public void testCarthesianProduct() throws Exception {
    Query q = MetaModelHelper.parseQuery(dc, "  SELECT a.foo,b.bar FROM      sch.tbl a, sch.tbl b \t WHERE a.foo = b.foo");
  ...
  public void testJoin() throws Exception {
    Query q = MetaModelHelper.parseQuery(dc, "SELECT a.foo,b.bar FROM sch.tbl a INNER JOIN sch.tbl b ON a.foo = b.foo");
    assertEquals("SELECT a.foo, b.bar FROM sch.tbl a INNER JOIN sch.tbl b ON a.foo = b.foo", q.toSql());
    q = MetaModelHelper.parseQuery(dc, "SELECT COUNT(*) FROM sch.tbl a LEFT JOIN sch.tbl b ON a.foo = b.foo AND a.bar = b.baz");
  ...
  public void testSimpleSelectFromWhere() throws Exception {
    ...
    assertNull("WHERE item was an expression based item, which indicates it was not parsed", whereClause.getItem(0).getExpression());
    assertEquals(2, whereClause.getItemCount());
    assertNull("WHERE item was an expression based item, which indicates it was not parsed", whereClause.getItem(1).getExpression());
  ...
  public void testCoumpoundWhereClause() throws Exception {
    Query q = MetaModelHelper.parseQuery(dc, "SELECT foo FROM sch.tbl WHERE (bar = \'baz\' OR (baz > 5 AND baz < 7))");
  ...
  public void testWhereSomethingIsNull() throws Exception {
    ...
    assertNull("WHERE item was an expression based item, which indicates it was not parsed", q.getWhereClause().getItem(0).getExpression());
  ...
  public void testFullQuery() throws Exception {
    Query q = MetaModelHelper.parseQuery(dc, "SELECT foo, COUNT(* ), MAX( baz ) FROM sch.tbl WHERE bar = \'baz\' AND foo = bar AND baz > 5 " + "GROUP BY foo HAVING COUNT(*) > 2 ORDER BY foo LIMIT 20 OFFSET 10");
    assertEquals("SELECT tbl.foo, COUNT(*), MAX(tbl.baz) FROM sch.tbl WHERE tbl.bar = \'baz\' AND tbl.foo = tbl.bar AND tbl.baz > 5 " + "GROUP BY tbl.foo HAVING COUNT(*) > 2 ORDER BY tbl.foo ASC", q.toSql());
