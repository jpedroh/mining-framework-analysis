...
public class RowTest {
  ...
  @Test public void testGetDouble() throws IOException {
    ...
    while (row.hasNext()) {
      row.next();
      assertEquals(table.numberColumn(1).get(row.getRowNumber()), row.getDouble(1), 0.00001);
      assertEquals(table.numberColumn("approval").get(row.getRowNumber()), row.getDouble("approval"), 0.00001);
    }
  }

  @Test public void testGetInt() throws IOException {
    Table table = Table.read().csv("../data/bush.csv");
    Row row = new Row(table);
    while (row.hasNext()) {
      row.next();
