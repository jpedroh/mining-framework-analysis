...
public class DateColumnType extends AbstractColumnType<LocalDate> {
  ...
  public static final DateColumnType INSTANCE = new DateColumnType(Integer.MIN_VALUE, 4, "LOCAL_DATE", "Date");

  private DateColumnType(Comparable<?> missingValue, int byteSize, String name, String printerFriendlyName) {
    super(missingValue, byteSize, name, printerFriendlyName);
  }

  @Override public DateColumn create(String name) {
    return DateColumn.create(name);
  }

  @Override public StringParser<LocalDate> defaultParser() {
    return new DateStringParser(this);
  }

  @Override public StringParser<LocalDate> customParser(CsvReadOptions options) {
    return new DateStringParser(this, options);
  }

  @Override public void copy(IntArrayList rows, Column<LocalDate> oldColumn, Column<LocalDate> newColumn) {
  ...
  @Override public void copyFromRows(IntArrayList rows, Column<LocalDate> newColumn, Row row) {
    DateColumn newDate = (DateColumn) newColumn;
    for (int index : rows) {
      row.at(index);
  ...
  @Override public boolean compare(int rowNumber, Column temp, Column original) {
