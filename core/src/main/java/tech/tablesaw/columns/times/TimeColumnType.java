...
public class TimeColumnType extends AbstractColumnType<LocalTime> {
  ...
  @Override public TimeColumn create(String name) {
    return TimeColumn.create(name);
  }

  @Override public StringParser<LocalTime> defaultParser() {
    return new TimeStringParser(this);
  }

  @Override public StringParser<LocalTime> customParser(CsvReadOptions options) {
    return new TimeStringParser(this, options);
  }

  @Override public void copy(IntArrayList rows, Column<LocalTime> oldColumn, Column<LocalTime> newColumn) {
  ...
  @Override public void copyFromRows(IntArrayList rows, Column<LocalTime> newColumn, Row row) {
    TimeColumn newTime = (TimeColumn) newColumn;
    for (int index : rows) {
      row.at(index);
  ...
  @Override public boolean compare(int rowNumber, Column<LocalTime> temp, Column<LocalTime> original) {
