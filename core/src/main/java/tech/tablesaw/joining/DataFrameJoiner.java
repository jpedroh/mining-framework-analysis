...
>>>>>>> /usr/src/app/output/lwhite1/tablesaw/4c7711f268d8e23009c13d95619f1784a13e97b0/core/src/main/java/tech/tablesaw/joining/DataFrameJoiner.java/right.java
  ...
  public Table inner(boolean allowDuplicateColumnNames, Table... tables) {
    ...
    for (Table currT : tables) {
      joined = joinInternal(joined, currT, column.name(), false, allowDuplicateColumnNames, columnNames);
    }
    return joined;
  }
...
<<<<<<< /usr/src/app/output/lwhite1/tablesaw/4c7711f268d8e23009c13d95619f1784a13e97b0/core/src/main/java/tech/tablesaw/joining/DataFrameJoiner.java/left.java
            ...
            if (type instanceof IntColumnType) {
              ...
              if (type instanceof LongColumnType) {
                ...
                if (type instanceof ShortColumnType) {
                  ...
                  throw new IllegalArgumentException("Joining is supported on integer, string, and date-like columns. Column " + column.name() + " is of type " + column.type());
                }
              }
            }
          }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private void renameColumnsWithDuplicateNames(Table table1, Table table2, String... col2Names) {
    ...
    for (Column<?> table2Column : table2.columns()) {
      ...
      if (table1.columnNames().stream().anyMatch(columnName::equalsIgnoreCase) && !(list.stream().anyMatch(columnName::equalsIgnoreCase))) {
  ...
  public Table rightOuter(Table table2, boolean allowDuplicateColumnNames, String... col2Names) {
    ...
    for (String name : table.columnNames()) {
      try {
        ...
        System.out.println("NOTE: DataFrameJoiner.rightOuter(): skipping left table\'s column,\'" + name + "\', in favor of right table\'s matching column that was kept in join operation.");
      }
    }
  ...
  private Table emptyTableFromColumns(Table table1, Table table2, String... col2Names) {
    Column<?>[] cols = Streams.concat(table1.columns().stream(), table2.columns().stream().filter((c) -> !Arrays.asList(col2Names).stream().anyMatch(c.name()::equalsIgnoreCase))).map((col) -> col.emptyCopy(col.size())).toArray(Column[]::new);
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "rawtypes", "unchecked" }) private void withMissingLeftJoin(Table destination, Table table1) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "rawtypes", "unchecked" }) private void withMissingRightJoin(Table destination, List<CategoricalColumn<?>> joinColumns, Table table2) {
