...
@Immutable public class CsvReader {
  /**
     ...
     */
  private List<ColumnType> typeArray = Lists.newArrayList(LOCAL_DATE_TIME, LOCAL_TIME, LOCAL_DATE, BOOLEAN, INTEGER, FLOAT, DOUBLE, STRING);
  ...
  public Table read(CsvReadOptions options) throws IOException {
    byte[] bytes = options.reader() != null ? CharStreams.toString(options.reader()).getBytes() : null;
    ColumnType[] types;
    if (options.columnTypes() != null) {
      types = options.columnTypes();
    } else {
      try (InputStream detectTypesStream = options.reader() != null ? new ByteArrayInputStream(bytes) : new FileInputStream(options.file())) {
        types = detectColumnTypes(detectTypesStream, options);
      }
    }
    InputStream stream = options.reader() != null ? new ByteArrayInputStream(bytes) : new FileInputStream(options.file());
  ...
  public Table headerOnly(ColumnType[] types, boolean header, CsvReadOptions options, File file) throws IOException {
    ...
    try (BufferedReader streamReader = new BufferedReader(reader)) {
      ...
      String[] headerNames = header ? csvParser.parseNext() : makeColumnNames(types);
