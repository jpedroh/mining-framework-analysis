...
public class OpenDriveHandler {
  ...
  /**
     ...
     */
  public static boolean loadRoadNetwork(RoadNetwork roadNetwork, String filename) throws JAXBException, SAXException, IllegalArgumentException {
  ...
  private boolean create(String filename, OpenDRIVE openDriveNetwork, RoadNetwork roadNetwork) throws IllegalArgumentException {
    ...
    for (Controller controller : openDriveNetwork.getController()) {
      for (Control control : controller.getControl()) {
        if (signalIdsToController.put(control.getSignalId(), controller) != null) {
          throw new IllegalArgumentException("trafficlight id=" + control.getSignalId() + " is referenced more than once in xodr <controller> definitions.");
        }
      }
    }
  ...
  private RoadSegment createRoadSegment(LaneSectionType laneType, Road road, boolean hasPeer, RoadMapping roadMapping) {
    Preconditions.checkArgument(road.getLanes().getLaneSection().size() == 1, "cannot handle more than one laneSection in roadId=" + road.getId());
    LaneSection laneSection = road.getLanes().getLaneSection().get(0);
    Preconditions.checkArgument(laneType == Lanes.LaneSectionType.LEFT ? laneSection.isSetLeft() : laneSection.isSetRight(), "road=" + road.getId() + " has no " + laneType.toString() + " lane defined.");
    List<Lane> lanes = (laneType == Lanes.LaneSectionType.LEFT) ? laneSection.getLeft().getLane() : laneSection.getRight().getLane();
    Preconditions.checkArgument(lanes.size() > 0, "no lanes in laneSection=" + laneType.toString() + " on road=" + road.getId() + " defined.");
    final RoadSegment roadSegment = laneType.isReverseDirection() ? new RoadSegment(roadMapping.roadLength(), lanes.size(), new RoadMappingPeer(roadMapping), RoadSegmentDirection.BACKWARD) : new RoadSegment(roadMapping.roadLength(), lanes.size(), roadMapping, RoadSegmentDirection.FORWARD);
    ...
    if (road.isSetType() && !road.getType().isEmpty()) {
      if (road.getType().size() > 1) {
        LOG.error("Movsim considers only first entry of the road.type and ignores the others defined for road=" + road.getId());
      }
    ...
    if (road.isSetObjects()) {
      for (OpenDRIVE.Road.Objects.Object roadObject : road.getObjects().getObject()) {
        Preconditions.checkArgument(roadObject.isSetOrientation(), "no orientation set for xodr roadObject=" + roadObject.getId() + " (this is not required by the xodr-xsd but currently for consistent movsim input).");
        if (hasPeer && roadObject.isSetOrientation() && !laneType.idAppender().equals(roadObject.getOrientation())) {
          continue;
        }
        if (laneType.isReverseDirection()) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug("Transform road object position for backward link: roadObject={}, originalPosition={}, roadSegment position=" + roadObject.getS(), roadObject.getId(), originalS);
          }
        }
        ...
        if (roadObjectType.equals(RoadObject.XodrRoadObjectType.SPEEDLIMIT.xodrIdentifier())) {
          ...
          if (roadObject.isSetValidLength()) {
            if (roadObject.getValidLength() <= 0) {
              throw new IllegalArgumentException("validLength=" + roadObject.getValidLength() + " but movsim\'s speedlimit expects a nontrivial length > 0.");
            }
            double endPosition = roadObject.getS() + roadObject.getValidLength();
            if (endPosition > roadSegment.roadLength()) {
              throw new IllegalArgumentException("speedlimit validity range=" + roadObject.getValidLength() + " results in=" + endPosition + " which exceeds the roadlength of roadSegment=" + roadSegment.userId());
            }
            roadSegment.roadObjects().add(new SpeedLimit(endPosition, MovsimConstants.MAX_VEHICLE_SPEED, roadSegment));
          }
        } else {
    ...
    if (road.isSetSignals()) {
      for (Signal signal : road.getSignals().getSignal()) {
        ...
        if (hasPeer && !(signal.getOrientation().equals(LaneSectionType.LEFT.idAppender()) || signal.getOrientation().equals(LaneSectionType.RIGHT.idAppender()))) {
          throw new IllegalArgumentException("signal.orientation= \"" + signal.getOrientation() + "\" does not match the expected values (+,-).");
        }
        ...
        if (!added) {
          throw new IllegalArgumentException("trafficlight signal with id=" + signal.getId() + " is not unique in xodr network definition.");
        }
        Controller controller = signalIdsToController.get(signal.getId());
        if (controller == null) {
          throw new IllegalArgumentException("trafficlight signal with id=" + signal.getId() + " is not referenced in xodr <controller> definition.");
        }
        if (laneType.isReverseDirection()) {
          double originalS = signal.getS();
          signal.setS(roadSegment.roadLength() - originalS);
          LOG.debug("Transform signal position from reverse direction: signal={}, originalPosition={}, roadSegment position=" + signal.getS(), signal.getId(), originalS);
        }
    ...
    if (road.isSetObjects()) {
      for (OpenDRIVE.Road.Objects.Tunnel tunnel : road.getObjects().getTunnel()) {
        if (laneType.isReverseDirection()) {
          ...
          LOG.debug("Transform tunnel position for reverse direction: tunnel={}, originalPosition={}, roadSegment position=" + tunnel.getS(), tunnel.getId(), originalS);
        }
  ...
  private static RoadMapping createRoadMappings(Road road) throws IllegalArgumentException {
    ...
    Preconditions.checkState(firstLaneSection.isSetRight(), "expect always at least a <right> lanesection (development stage)");
  ...
  private static RoadSegment getRoadSegment(RoadNetwork roadNetwork, String roadId, LaneSectionType type) {
    ...
    if (roadSegment == null) {
      roadSegment = roadNetwork.findByUserId(roadId + (type == LaneSectionType.LEFT ? Lanes.LaneSectionType.LEFT.idAppender() : Lanes.LaneSectionType.RIGHT.idAppender()));
    }
    if (roadSegment == null) {
  ...
  private static RoadSegment getRoadSegment(RoadNetwork roadNetwork, String roadId, int lane) {
    return getRoadSegment(roadNetwork, roadId, (lane > 0 ? Lanes.LaneSectionType.LEFT : Lanes.LaneSectionType.RIGHT));
  }
  ...
  private static void checkLaneIndexConventions(LaneSectionType laneType, String roadId, List<Lane> lanes) {
    ...
    for (Lane lane : lanes) {
      ...
      if (lane.getId() == 0) {
        throw new IllegalArgumentException("usage of the laneIndex index={} for a normal lane in xodr. 0 is reserved for a <center> lane-section. roadId=" + roadId);
      }
      if (laneType == Lanes.LaneSectionType.LEFT && lane.getId() < 0) {
        throw new IllegalArgumentException("lane indices of a <laneSection><left> must be positive in roadId=" + roadId);
      }
  ...
  private static void setLaneType(int laneNumber, Lane lane, RoadSegment roadSegment) {
    ...
    if (lane.getType().equals(Lanes.Type.TRAFFIC.getOpenDriveIdentifier())) {
      ...
      if (lane.getType().equals(Lanes.Type.ENTRANCE.getOpenDriveIdentifier())) {
        ...
        if (lane.getType().equals(Lanes.Type.RESTRICTED.getOpenDriveIdentifier())) {
          roadSegment.setLaneType(laneNumber, Lanes.Type.RESTRICTED);
          Vehicle obstacle = new Vehicle(roadSegment.roadLength(), 0.0, laneNumber, 1.0, 1.0);
          obstacle.setType(Vehicle.Type.OBSTACLE);
          roadSegment.addObstacle(obstacle);
        } else {
          if (lane.getType().equals(Lanes.Type.EXIT.getOpenDriveIdentifier())) {
            roadSegment.setLaneType(laneNumber, Lanes.Type.EXIT);
          } else {
            if (lane.getType().equals(Lanes.Type.SHOULDER.getOpenDriveIdentifier())) {
  ...
  private static void joinByLanes(RoadNetwork roadNetwork, Road road) {
    ...
    if (laneSection.isSetRight()) {
      joinByLanes(roadNetwork, road, laneSection.getRight().getLane(), Lanes.LaneSectionType.RIGHT.isReverseDirection());
    }
  }
    ...
    for (Lane lane : lanes) {
      ...
      if (lane.getLink().isSetPredecessor()) {
        if (!hasRoadPredecessor(road)) {
          throw new IllegalArgumentException("predecessor lane link but no road link defined for road=" + road.getId());
        }
        ...
        if (isReverse) {
          Link.addLanePair(laneIdToLaneIndex(toLane), roadSegment, laneIdToLaneIndex(fromLane), sourceRoadSegment);
        } else {
      ...
      if (lane.getLink().isSetSuccessor()) {
        if (!hasRoadSuccessor(road)) {
          throw new IllegalArgumentException("successor lane link but no road link defined for road=" + road.getId());
        }
        ...
        if (isReverse) {
          Link.addLanePair(laneIdToLaneIndex(toLane), sinkRoadSegment, laneIdToLaneIndex(fromLane), roadSegment);
        } else {
  ...
  private static boolean hasRoadSuccessor(Road road) {
    return road.getLink().isSetSuccessor() && road.getLink().getSuccessor().getElementType().equals(RoadLinkElementType.ROAD.xodrIdentifier());
  }

  private static boolean hasRoadPredecessor(Road road) {
    return road.getLink().isSetPredecessor() && road.getLink().getPredecessor().getElementType().equals(RoadLinkElementType.ROAD.xodrIdentifier());
  }
    ...
    for (Junction junction : openDriveNetwork.getJunction()) {
      for (Connection connection : junction.getConnection()) {
        for (LaneLink laneLink : connection.getLaneLink()) {
          ...
          RoadSegment connectingRoadSegment = getRoadSegment(roadNetwork, connection.getConnectingRoad(), laneLink.getTo());
          final boolean isReverse = laneLink.getTo() > 0 || laneLink.getFrom() > 0;
          LOG.info("junction={}, road={}", junction.getId(), road.getId());
          LOG.info("incomingRS={}, connectingRoadSegment={}", incomingRoadSegment.userId(), connectingRoadSegment.userId());
          LOG.info("lanepair from={} to={}", laneLink.getFrom(), laneLink.getTo());
          LOG.info("isReverse={}, roadPredecessorIsJunction={}", isReverse, roadPredecessorIsJunction(junction, road));
          if (roadPredecessorIsJunction(junction, road)) {
            if (isReverse) {
              Link.addLanePair(laneIdToLaneIndex(laneLink.getTo()), connectingRoadSegment, laneIdToLaneIndex(laneLink.getFrom()), incomingRoadSegment);
            } else {
              Link.addLanePair(laneIdToLaneIndex(laneLink.getFrom()), incomingRoadSegment, laneIdToLaneIndex(laneLink.getTo()), connectingRoadSegment);
            }
          } else {
            if (roadSuccessorIsJunction(junction, road)) {
              if (isReverse) {
                Link.addLanePair(laneIdToLaneIndex(laneLink.getTo()), connectingRoadSegment, laneIdToLaneIndex(laneLink.getFrom()), incomingRoadSegment);
              } else {
                Link.addLanePair(laneIdToLaneIndex(laneLink.getFrom()), connectingRoadSegment, laneIdToLaneIndex(laneLink.getTo()), incomingRoadSegment);
              }
            } else {
  ...
  private static void checkIfAllLanesAreConnected(RoadNetwork roadNetwork) {
    ...
    for (RoadSegment roadSegment : roadNetwork) {
      ...
      for (LaneSegment laneSegment : roadSegment.laneSegments()) {
        if (laneSegment.sinkLaneSegment() == null && (laneSegment.type() != Lanes.Type.ENTRANCE && laneSegment.type() != Lanes.Type.RESTRICTED)) {
          LOG.error("no sinklane for lane={} on RoadSegment={}", laneSegment.lane(), laneSegment.roadSegment().userId());
          valid = false;
        }
      }
    }
  ...
  private static boolean roadSuccessorIsJunction(Junction junction, Road road) {
    return road.getLink().isSetSuccessor() && road.getLink().getSuccessor().getElementType().equals(RoadLinkElementType.JUNCTION.xodrIdentifier()) && road.getLink().getSuccessor().getElementId().equals(junction.getId());
  }

  private static boolean roadPredecessorIsJunction(Junction junction, Road road) {
    return road.getLink().isSetPredecessor() && road.getLink().getPredecessor().getElementType().equals(RoadLinkElementType.JUNCTION.xodrIdentifier()) && road.getLink().getPredecessor().getElementId().equals(junction.getId());
  }
