...
class FileFloatingCars extends FileOutputBase {
  ...
  private static final String outputHeading = COMMENT_CHAR + "     t[s],    roadId,      lane,      x[m], totalX[m],    v[m/s],  a[m/s^2],aModel[m/s^2], gap[m],   dv[m/s],distToTL[m],fuelFlow[ml/s],frontVehID,slope[rad]";
  ...
  static void writeHeader(PrintWriter writer, Vehicle vehicle, Route route) {
    ...
    writer.println(String.format("%s model category = %s", COMMENT_CHAR, vehicle.getLongitudinalModel().modelName().getCategory().toString()));
    writer.println(String.format("%s model name = %s (short name: %s)", COMMENT_CHAR, vehicle.getLongitudinalModel().modelName().getDetailedName(), vehicle.getLongitudinalModel().modelName().getShortName()));
    writer.println(String.format("%s physical vehicle length (in m) = %.2f", COMMENT_CHAR, vehicle.physicalQuantities().getLength()));
    writer.println(String.format("%s position x is defined by vehicle front (on the given road segment)", COMMENT_CHAR));
    writer.println(String.format("%s origin roadsegment id= %d, exit roadsegment id= %d (not set=%d)", COMMENT_CHAR, vehicle.originRoadSegmentId(), vehicle.exitRoadSegmentId(), Vehicle.ROAD_SEGMENT_ID_NOT_SET));
  ...
  static void writeData(double time, Vehicle veh, Vehicle frontVeh, PrintWriter writer) {
    ...
    writer.printf(outputFormat, time, veh.roadSegmentId(), veh.lane(), physicalQuantities.getFrontPosition(), physicalQuantities.totalTravelDistance(), physicalQuantities.getSpeed(), physicalQuantities.getAcc(), physicalQuantities.accModel(), physicalQuantities.getNetDistance(frontVeh), physicalQuantities.getRelSpeed(frontVeh), physicalQuantities.getxScale() * veh.getDistanceToTrafficlight(), 1000 * veh.getEnergyModel().getActualFuelFlowLiterPerS(), frontVeh == null ? -1 : frontVeh.getVehNumber(), veh.getSlope());
    writer.flush();
  }
}