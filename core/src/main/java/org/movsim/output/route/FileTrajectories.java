...
public class FileTrajectories extends FileOutputBase implements SimulationTimeStep {
  ...
  private static final String outputHeading = COMMENT_CHAR + "     t[s], lane,       x[m],     v[m/s],   a[m/s^2],     gap[m],    dv[m/s], label,           id,  roadId, originId, infoComment, absTime, xWithOffset[m]";
  ...
  public FileTrajectories(Trajectories traj, Route route) {
    ...
    LOG.info("interval for output: timeStart=" + (traj.isSetStartTime() ? traj.getStartTime() : "--") + ", timeEnd=" + (traj.isSetEndTime() ? traj.getEndTime() : "--"));
      ...
      for (LaneSegment laneSegment : roadSegment.laneSegments()) {
        for (final Vehicle vehicle : laneSegment) {
          ...
          if (!traj.isSetRandomFraction() || vehicle.getRandomFix() < traj.getRandomFraction()) {
            if (vehicle.getFrontPosition() >= positionIntervalStart && vehicle.getFrontPosition() <= positionIntervalEnd) {
  ...
  private void writeVehicleData(Vehicle me, double positionOnRoute, Vehicle frontVehicle, String formattedTime) {
    ...
    final double s = (frontVehicle == null || frontVehicle.type() == Vehicle.Type.OBSTACLE) ? 0 : me.getNetDistance(frontVehicle);
    final double dv = (frontVehicle == null || frontVehicle.type() == Vehicle.Type.OBSTACLE) ? 0 : me.getRelSpeed(frontVehicle);
    write(outputFormat, time, me.lane(), pos, me.getSpeed(), me.getAcc(), s, dv, me.getLabel(), me.getId(), me.roadSegmentId(), me.originRoadSegmentId(), formattedTime, pos + traj.getOffsetPosition(), me.getUserData().getString(","));
  }
}