...
public class ConsumptionOnRoute extends OutputOnRouteBase {
  ...
  public ConsumptionOnRoute(double simulationTimestep, ConsumptionCalculation fuelRouteInput, RoadNetwork roadNetwork, Route route, boolean writeOutput) {
  ...
  @Override public void timeStep(double dt, double simulationTime, long iterationCount) {
    ...
    instConsumptionEMA = (simulationTime == 0) ? instantaneousConsumptionRate : ExponentialMovingAverage.calc(instantaneousConsumptionRate, instConsumptionEMA, beta);
    if (fileWriter != null) {
