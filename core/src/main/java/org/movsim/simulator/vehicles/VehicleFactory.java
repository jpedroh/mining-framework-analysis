...
public final class VehicleFactory {
  ...
  public VehicleFactory(double simulationTimestep, VehiclePrototypes vehPrototypes, @Nullable Consumption consumption, Routing routing, @Nullable ServiceProviders serviceProviders) {
  ...
  public Vehicle create(VehicleType vehicleType, @Nullable Route route) {
    ...
    accelerationModel.setRelativeRandomizationV0(vehicleType.getRelativeV0Randomization(), vehicleType.getV0DistributionType());
    LaneChangeModel laneChangeModel = prototype.createLaneChangeModel();
    Vehicle vehicle = new Vehicle(prototype.getLabel(), accelerationModel, prototype.getConfiguration(), laneChangeModel);
  ...
  private void initialize(double simulationTimestep, List<VehiclePrototypeConfiguration> configurations, @Nullable Consumption consumption) {
    for (VehiclePrototypeConfiguration typeConfig : configurations) {
      if (vehiclePrototypes.containsKey(typeConfig.getLabel())) {
        throw new IllegalArgumentException("ambigous vehicle prototype definition: prototype with label=\"" + typeConfig.getLabel() + "\" already exists.");
      }
      ...
      if (typeConfig.isSetConsumptionModelName()) {
        ...
        if (!fuelModelFactory.hasModel(consumptionModelName)) {
          throw new IllegalArgumentException("cannot find vehicle\'s consumption model with label=\"" + consumptionModelName);
        }
