...
class Gipps extends LongitudinalModelBase {
  ...
  @Override public void setRelativeRandomizationV0(double relRandomizationFactor, DistributionTypeEnum distributionType) {
  }
  ...
  private double acc(double s, double v, double dv, double v0Local, double TLocal) {
    ...
    final double vSafe = -b * TLocal + Math.sqrt(b * b * TLocal * TLocal + vp * vp + 2 * b * Math.max(s - getMinimumGap(), 0.));
    final double vNew = Math.min(vSafe, Math.min(v + a * TLocal, v0Local));
    final double aWanted = (vNew - v) / TLocal;
    return aWanted;
  }

  @Override protected IModelParameterGipps getParameter() {
    return param;
  }
