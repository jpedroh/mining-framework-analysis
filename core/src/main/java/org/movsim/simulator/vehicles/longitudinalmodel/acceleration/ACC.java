...
class ACC extends LongitudinalModelBase {
  ...
  @Override protected IModelParameterACC getParameter() {
    return param;
  }
  ...
  private double acc(double s, double v, double dv, double aLead, double TLocal, double v0Local, double aLocal) {
    ...
    final double sstar = getMinimumGap() + Math.max(TLocal * v + param.getS1() * Math.sqrt((v + 0.00001) / v0Local) + 0.5 * v * dv / Math.sqrt(aLocal * param.getB()), 0.);
    final double z = sstar / Math.max(s, 0.01);
    final double accEmpty = (v <= v0Local) ? aLocal * (1 - Math.pow((v / v0Local), param.getDelta())) : -param.getB() * (1 - Math.pow((v0Local / v), aLocal * param.getDelta() / param.getB()));
    ...
    final double accACC_IIDM = (accIIDM > accCAH) ? accIIDM : (1 - param.getCoolness()) * accIIDM + param.getCoolness() * (accCAH + param.getB() * Math.tanh((accIIDM - accCAH) / param.getB()));
    return accACC_IIDM;
  }
}