...
public class Vehicle {
  ...
  private void updateBrakeLightStatus() {
    if (brakeLightOn) {
      ...
      if (accOld > -THRESHOLD_BRAKELIGHT_ON && acc < -THRESHOLD_BRAKELIGHT_ON && speed > 0.001) {
        brakeLightOn = true;
      }
    }
  }
  ...
  public void moveToNewRoadSegment(RoadSegment newRoadSegment, int newLane, double newRearPosition, double exitPos) {
    ...
    LOG.debug("move to new segment: rearPosOld={} --> new rearPosOld={}", frontPositionOld - getLength(), frontPositionOld - offsetPosition - getLength());
  ...
  @Override public String toString() {
    ...
    return "Vehicle [id=" + id + ", label=" + label + ", type=" + type + ", length=" + df.format(getLength()) + ", frontPosition=" + df.format(frontPosition) + ", frontPositionOld=" + df.format(frontPositionOld) + ", speed=" + df.format(speed) + ", accModel=" + df.format(accModel) + ", acc=" + df.format(acc) + ", accOld=" + df.format(accOld) + ", vehNumber=" + vehNumber + ", lane=" + lane + ", brakeLightOn=" + brakeLightOn + "]";
  }
