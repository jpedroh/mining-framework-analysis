 ...
 * 
 * <p>
 * Each vehicle has its own unique immutable id which is assigned when the
 * vehicle is created.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * A vehicle has the kinematic attributes of position, velocity and
 * acceleration. A vehicle's position is given by the position of the front of
 * the vehicle on the road segment and also by the vehicle's lane.
 * </p>
 * 
 * <p>
 * A vehicle possesses two intelligence modules:
 * <ul>
 * <li>a LongitudinalModel which determines its acceleration in the direction of
 * travel.</li>
 * <li>a LaneChangeModel which determines when it changes lanes.</li>
 * </ul>
 * </p>
 * <p>
 * Vehicles are quite frequently created and destroyed, so by design they have
 * few allocated properties.
 * </p>
 */
public class Vehicle {
	...
	public Vehicle(String label, LongitudinalModelBase longitudinalModel,
			VehiclePrototypeConfiguration vehInput,
	 ...
	 * Constructor.
	 */
	public Vehicle(double rearPosition, double speed, int lane, double length,
			double width) {
	...
	public void updateAcceleration(double dt, RoadSegment roadSegment,
			LaneSegment laneSegment, LaneSegment leftLaneSegment) {
		...
		acc = accModel = calcAccModel(laneSegment, leftLaneSegment,
	 ...
	 */
	private final double moderateAcceleration(double acc,
	...
	private double accelerationConsideringExit(RoadSegment roadSegment) {
		...
		LOG.debug("exitRoadSectionId={}, current roadSegment={}",
		...
		if (roadSegment.id() == exitRoadSegmentId) {
			LaneSegment firstExitLaneSegment = roadSegment
					.laneSegment(roadSegment.trafficLaneMax() + Lanes.TO_RIGHT);
			assert firstExitLaneSegment != null
					&& firstExitLaneSegment.type() == Lanes.Type.EXIT : "no exitLaneSegment="
					+ firstExitLaneSegment;
			Vehicle frontVehicle = firstExitLaneSegment.frontVehicle(this);
			accToVehicleInExitLane = longitudinalModel.calcAcc(this,
					frontVehicle);
			accToVehicleInExitLane = Math.max(accToVehicleInExitLane,
					-maxDeceleration);
			if (LOG.isDebugEnabled()) {
				LOG.debug(String
						.format("considering exit=%d: veh=%d, distance to front veh in exit lane=%.2f, speed=%.2f, calcAcc=%.2f, accLimit=%.2f",
								exitRoadSegmentId, getId(),
								getNetDistance(frontVehicle), getSpeed(),
								longitudinalModel.calcAcc(this, frontVehicle),
		...
		RoadSegment sinkRoadSegmentWithExit = roadSegment
				.sinkRoadSegmentPerId(exitRoadSegmentId);
		if (sinkRoadSegmentWithExit != null) {
			LaneSegment exitLaneSegment = sinkRoadSegmentWithExit
					.laneSegment(sinkRoadSegmentWithExit.trafficLaneMax()
							+ Lanes.TO_RIGHT);
			assert exitLaneSegment != null
					&& exitLaneSegment.type() == Lanes.Type.EXIT : "no exitLaneSegment="
					+ exitLaneSegment;
			Vehicle frontVehicle = exitLaneSegment.rearVehicle();
			if (frontVehicle != null) {
				double s = roadSegment.roadLength() - this.getFrontPosition()
						+ frontVehicle.getRearPosition();
				double dv = getSpeed() - frontVehicle.getSpeed();
				accToVehicleInExitLane = longitudinalModel.calcAccSimple(s,
						getSpeed(), dv);
				accToVehicleInExitLane = Math.max(accToVehicleInExitLane,
						-maxDeceleration);
				if (LOG.isDebugEnabled()) {
	...
	public double calcAccModel(LaneSegment laneSegment,
			LaneSegment leftLaneSegment) {
		return calcAccModel(laneSegment, leftLaneSegment, 1.0, 1.0, 1.0);
	}

	private double calcAccModel(LaneSegment laneSegment,
			LaneSegment leftLaneSegment, double alphaTLocal,
		...
		if (laneChangeModel != null && laneChangeModel.isInitialized()
				&& laneChangeModel.withEuropeanRules()) {
			acc = longitudinalModel.calcAccEur(laneChangeModel.vCritEurRules(),
					this, laneSegment, leftLaneSegment, alphaTLocal,
					alphaV0Local, alphaALocal);
		} else {
			acc = longitudinalModel.calcAcc(this, laneSegment, alphaTLocal,
					alphaV0Local, alphaALocal);
		}
	...
	public void updatePositionAndSpeed(double dt) {
		...
		if (longitudinalModel != null && longitudinalModel.isCA()) {
		...
		} else if (longitudinalModel != null
				&& (longitudinalModel.modelName() == ModelName.NEWELL)) {
			...
			final double advance = (acc * dt >= -speed) ? speed * dt + 0.5
					* acc * dt * dt : -0.5 * speed * speed / acc;
			frontPosition += advance;
			totalTravelDistance += advance;
			speed += dt * acc;
			if (speed < 0) {
		...
		if (fuelModel != null) {
			totalFuelUsedLiters += fuelModel.getFuelFlowInLiterPerS(speed, acc)
					* dt;
		}
	}
	...
	public boolean considerOvertakingViaPeer(double dt, RoadSegment roadSegment) {
		...
		if (!roadSegment.hasPeer() || roadSegment.laneCount() > 1
				|| lane() != Lanes.MOST_INNER_LANE || laneChangeModel == null
				|| !laneChangeModel.isInitialized() || inProcessOfLaneChange()) {
			return false;
		}
		...
		if (lcDecision == LaneChangeDecision.OVERTAKE_VIA_PEER) {
			...
			resetDelay(dt);
			LOG.debug("do overtaking lane change to={} into target lane={}",
					lcDecision, targetLane);
		}
	...
	public boolean considerFinishOvertaking(double dt, LaneSegment laneSegment) {
		...
		LaneChangeDecision lcDecision = laneChangeModel
				.finishOvertakingViaPeer(laneSegment);
		if (lcDecision == LaneChangeDecision.MANDATORY_TO_RIGHT) {
			setTargetLane(Lanes.MOST_INNER_LANE);
			resetDelay(dt);
			LOG.debug(
					"finish overtaking, turn from lane={} into target lane={}",
					laneOld, targetLane);
			return true;
		}
		return false;
	...
	public boolean considerLaneChange(double dt, RoadSegment roadSegment) {
		...
		if (laneChangeDirection != Lanes.NO_CHANGE) {
			...
			resetDelay(dt);
			LOG.debug("do lane change to={} into target lane={}",
					laneChangeDirection, targetLane);
			return true;
		}
		return false;
	}
	...
	private void considerRouteAlternatives(RoadSegment roadSegment) {
		...
		if (DecisionModel.doDiverge(beta, roadSegment, alternatives)) {
			exitRoadSegmentId = roadSegment.id();
	...
	public double getContinousLane() {
		if (inProcessOfLaneChange()) {
			final double fractionTimeLaneChange = Math.min(1, tLaneChangeDelay
					/ FINITE_LANE_CHANGE_TIME_S);
			return fractionTimeLaneChange * lane + (1 - fractionTimeLaneChange)
					* laneOld;
		}
		return lane();
	}
	...
	private void updateBrakeLightStatus() {
		...
		} else if (accOld > -THRESHOLD_BRAKELIGHT_ON
	...
	/**
	 * <p>
	 * Called when vehicle changes road segments (and possibly also lanes) at a
	 * link or junction.
	 * </p>
	 * <p>
	 * Although the change of lanes is immediate, <code>lane</code>,
	 * <code>prevLane</code> and <code>timeAtWhichLastChangedLanes</code> are
	 * used to interpolate this vehicle's lateral position and so give the
	 ...
	 * @param exitPos
	 */
	public void moveToNewRoadSegment(RoadSegment newRoadSegment, int newLane,
			double newRearPosition, double exitPos) {
		// distanceTravelledToStartOfRoadSegment += rearPosition - newRearPos;
		this.lane = newLane;
		this.laneOld = newLane;
		laneOld = Math.max(Lanes.MOST_INNER_LANE,
				Math.min(laneOld, newRoadSegment.laneCount()));
		double offsetPosition = getRearPosition() - newRearPosition;
		LOG.debug("move to new segment: rearPosOld={} --> new rearPosOld={}",
				frontPositionOld - length, frontPositionOld - offsetPosition
						- length);
	...
	private void updateRoute() {
		...
		if (route != null && routeIndex < route.size()) {
			...
			if (routeRoadSegment.id() != roadSegmentId) {
				LOG.warn("vehicle={} has left its route={}.", this,
						route.getName());
			...
			if (routeIndex < route.size()) {
				...
				if (routeRoadSegment.exitsOnto(nextRouteRoadSegment.id())) {
					...
					final RoadSegment nextNextRouteRoadSegment = route
							.get(routeIndex + 1);
					if (nextRouteRoadSegment.exitsOnto(nextNextRouteRoadSegment
							.id())) {
						// this vehicle needs to exit on this roadSegment
	...
	public String toString() {
		return "Vehicle [id=" + id + ", label=" + label + ", length=" + length
				+ ", frontPosition=" + frontPosition + ", frontPositionOld="
				+ frontPositionOld + ", speed=" + speed + ", accModel="
				+ accModel + ", acc=" + acc + ", accOld=" + accOld
				+ ", vehNumber=" + vehNumber + ", lane=" + lane + "]";
	}
