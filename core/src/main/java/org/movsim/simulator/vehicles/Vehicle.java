...
public class Vehicle {
  ...
  public Vehicle(String label, LongitudinalModelBase longitudinalModel, VehiclePrototypeConfiguration vehInput, @Nullable LaneChangeModel lcModel) {
  ...
  public void updateAcceleration(double dt, RoadSegment roadSegment, LaneSegment laneSegment, LaneSegment leftLaneSegment) {
    ...
    acc = Math.max(acc + accError, -maxDeceleration);
  }
  ...
  private double accelerationConsideringExit(RoadSegment roadSegment) {
    ...
    if (roadSegment.id() == exitRoadSegmentId) {
      ...
      assert firstExitLaneSegment != null && firstExitLaneSegment.type() == Lanes.Type.EXIT : "no exitLaneSegment=" + firstExitLaneSegment;
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(String.format("considering exit=%d: veh=%d, distance to front veh in exit lane=%.2f, speed=%.2f, calcAcc=%.2f, accLimit=%.2f", exitRoadSegmentId, getId(), getNetDistance(frontVehicle), getSpeed(), longitudinalModel.calcAcc(this, frontVehicle), accToVehicleInExitLane));
      }
      return accToVehicleInExitLane;
    }
    RoadSegment sinkRoadSegmentWithExit = roadSegment.sinkRoadSegmentPerId(exitRoadSegmentId);
    if (sinkRoadSegmentWithExit != null) {
      LaneSegment exitLaneSegment = sinkRoadSegmentWithExit.laneSegment(sinkRoadSegmentWithExit.trafficLaneMax() + Lanes.TO_RIGHT);
      assert exitLaneSegment != null && exitLaneSegment.type() == Lanes.Type.EXIT : "no exitLaneSegment=" + exitLaneSegment;
      ...
      if (frontVehicle != null) {
        ...
        accToVehicleInExitLane = Math.max(accToVehicleInExitLane, -maxDeceleration);
        if (LOG.isDebugEnabled()) {
          LOG.debug(String.format("considering exit=%d: veh=%d, distance to front veh in exit lane=%.2f, speed=%.2f, accLimit=%.2f", exitRoadSegmentId, getId(), getNetDistance(frontVehicle), getSpeed(), accToVehicleInExitLane));
        }
        return accToVehicleInExitLane;
      }
    }
  ...
  private double calcAccModel(LaneSegment laneSegment, LaneSegment leftLaneSegment, double alphaTLocal, double alphaV0Local, double alphaALocal) {
    if (longitudinalModel == null) {
      return 0.0;
    }
    double acc;
    if (laneChangeModel != null && laneChangeModel.isInitialized() && laneChangeModel.withEuropeanRules()) {
      acc = longitudinalModel.calcAccEur(laneChangeModel.vCritEurRules(), this, laneSegment, leftLaneSegment, alphaTLocal, alphaV0Local, alphaALocal);
    } else {
  ...
  public void updatePositionAndSpeed(double dt) {
    ...
    if (longitudinalModel != null && longitudinalModel.isCA()) {
      ...
      if (longitudinalModel != null && (longitudinalModel.modelName() == ModelName.NEWELL)) {
        if (speed < 0) {
          speed = 0;
        }
  ...
  public boolean considerOvertakingViaPeer(double dt, RoadSegment roadSegment) {
    ...
    if (!roadSegment.hasPeer() || roadSegment.laneCount() > 1 || lane() != Lanes.MOST_INNER_LANE || laneChangeModel == null || !laneChangeModel.isInitialized() || inProcessOfLaneChange()) {
      return false;
    }
  ...
  private void considerRouteAlternatives(RoadSegment roadSegment) {
    ...
    if (DecisionModel.doDiverge(beta, roadSegment, alternatives)) {
      ...
      if (roadSegment.laneType(roadSegment.laneCount()) != Lanes.Type.EXIT) {
        throw new IllegalArgumentException("end of VariableMessageSignDiversion lies on roadSegment " + roadSegment.userId() + " without exit lane!");
      }
    }
  }
  ...
  public void moveToNewRoadSegment(RoadSegment newRoadSegment, int newLane, double newRearPosition, double exitPos) {
    ...
    LOG.debug("move to new segment: rearPosOld={} --> new rearPosOld={}", frontPositionOld - length, frontPositionOld - offsetPosition - length);
  ...
  @Override public String toString() {
    return "Vehicle [id=" + id + ", label=" + label + ", length=" + length + ", frontPosition=" + frontPosition + ", frontPositionOld=" + frontPositionOld + ", speed=" + speed + ", accModel=" + accModel + ", acc=" + acc + ", accOld=" + accOld + ", vehNumber=" + vehNumber + ", lane=" + lane + "]";
  }
