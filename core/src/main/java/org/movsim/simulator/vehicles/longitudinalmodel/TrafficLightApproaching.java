...
public class TrafficLightApproaching {
  ...
  public void update(Vehicle vehicle, RoadSegment roadSegment) {
    ...
    LOG.debug("approaching non-green trafficlight: distanceToTrafficlight={}, trafficLight={}", distanceToTrafficlight, toString());
    Preconditions.checkArgument(distanceToTrafficlight >= 0, "trafficlight already passed, removal of passed lights not working! [check if vehicle has passed start-signal]");
    ...
    if (trafficLight.status() == TrafficLightStatus.GREEN_RED) {
      ...
      if (trafficLight.status() == TrafficLightStatus.RED) {
        ...
        if (accTrafficLight <= -maxDeceleration || minBrakeDist >= distanceToTrafficlight) {
          LOG.info(String.format("veh id=%d in dilemma zone is going to pass red light at distance=%.2fm due to physics (assuming user-defined max. possible braking=%.2fm/s^2!", vehicle.getId(), distanceToTrafficlight, maxDeceleration));
          considerTrafficLight = false;
        }
      }
    }
  ...
  private TrafficLight findNonGreenTrafficLight(Vehicle vehicle, RoadSegment roadSegment) {
    ...
    while (iterator.hasNext()) {
      ...
      if (trafficLight.status() != TrafficLightStatus.GREEN && trafficLight.status() != TrafficLightStatus.RED_GREEN) {
        return trafficLight;
      }
    }
    return null;
  ...
  private static double calcEffectiveFrontVehicleLengths(Vehicle me, TrafficLight trafficLight, double distanceToSecondTrafficlight) {
  ...
  @Override public String toString() {
    return "TrafficLightApproaching [trafficLights.size=" + trafficLights.size() + ", considerTrafficLight=" + considerTrafficLight + ", accTrafficLight=" + accTrafficLight + ", distanceToTrafficlight=" + distanceToTrafficlight + "]";
  }
