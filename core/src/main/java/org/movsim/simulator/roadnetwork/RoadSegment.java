...
public class RoadSegment extends DefaultWeightedEdge implements Iterable<Vehicle> {
  ...
  public final LaneSegment sourceLaneSegment(int lane) {
    Preconditions.checkArgument(lane >= Lanes.LANE1 && lane <= laneCount, "lane=" + lane + " not defined for roadId=" + userId());
  ...
  @CheckForNull public RoadSegment sinkRoadSegmentPerId(int exitRoadSegmentId) {
  ...
  public boolean exitsOnto(int exitRoadSegmentId) {
    for (final LaneSegment laneSegment : laneSegments) {
      if (laneSegment.type() == Lanes.Type.EXIT) {
        assert laneSegment.sinkLaneSegment() != null : "roadSegment=" + userId() + " with lane=" + laneSegment.lane() + " has no downstream connection.";
  ...
  public void updateSignalPointsAfterOutflowAndInflow(double simulationTime) {
    assert updateSignalPointsBeforeOutflowCalled;
    for (SignalPoint signalPoint : signalPoints) {
      signalPoint.registerPassingVehicles(simulationTime, iterator());
    }
      ...
      for (Iterator<Vehicle> vehIterator = laneSegment.iterator(); vehIterator.hasNext(); ) {
        ...
        if (vehicle.inProcessOfLaneChange()) {
          ...
          if (vehicle.considerLaneChange(dt, this)) {
            ...
            if (vehicle.considerOvertakingViaPeer(dt, this)) {
  ...
  private void checkFinishingOvertaking(double dt) {
    for (Iterator<Vehicle> vehIterator = overtakingSegment.iterator(); vehIterator.hasNext(); ) {
      ...
      if (vehicle.inProcessOfLaneChange()) {
        ...
        if (vehicle.considerFinishOvertaking(dt, laneSegment(Lanes.MOST_INNER_LANE))) {
    ...
    for (final LaneSegment laneSegment : laneSegments) {
      ...
      for (Vehicle vehicle : laneSegment) {
        ...
        if (netDistance < 0) {
          ...
          sb.append(String.format("Crash of Vehicle i=%d (vehId=%d) at x=%.4f ", index, vehicle.getId(), vehicle.getFrontPosition()));
          if (vehFront != null) {
          ...
          for (int j = Math.max(0, index - 8), M = laneSegment.vehicleCount(); j <= Math.min(index + 8, M - 1); j++) {
            ...
            sb.append(String.format("veh=%d, pos=%6.2f, speed=%4.2f, accModel=%4.3f, acc=%4.3f, length=%3.1f, lane=%d, nodeId=%d%n", j, veh.getFrontPosition(), veh.getSpeed(), veh.accModel(), veh.getAcc(), veh.getLength(), veh.lane(), veh.getId()));
          }
  ...
  @Override public String toString() {
    return "RoadSegment [nodeId=" + id + ", userId=" + userId + ", roadName=" + roadName + ", roadLength=" + roadLength + ", laneCount=" + laneCount + ", " + getOriginNode() + ", " + getDestinationNode() + "]";
  }
