...
public abstract class AbstractTrafficSource implements SimulationTimeStep {
  ...
  public interface RecordDataCallback {
    /**
         ...
         */
    public void recordData(double simulationTime, int laneEnter, double xEnter, double vEnter, double totalInflow, int enteringVehCounter, double nWait);
  }
  ...
  void recordData(double simulationTime, final double totalInflow) {
    if (recordDataCallback != null) {
      recordDataCallback.recordData(simulationTime, laneEnterLast, xEnterLast, vEnterLast, totalInflow, enteringVehCounter, nWait);
    }
  }
  ...
  @CheckForNull public TrafficCompositionGenerator getTrafficComposition() {
    return vehGenerator;
  }
  ...
  protected void calcApproximateInflow(double dt) {
    ...
    if (measuredTime > MEASURING_INTERVAL_S) {
      measuredInflow = measuredInflowCount / MEASURING_INTERVAL_S;
      measuredTime = 0.0;
      measuredInflowCount = 0;
      LOG.debug(String.format("source=%d with measured inflow Q=%.1f/h over all lanes and queue length %d of waiting vehicles", roadSegment.id(), measuredInflow * Units.INVS_TO_INVH, getQueueLength()));
    }
  }
}