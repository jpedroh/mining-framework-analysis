...
public class Simulator implements SimulationTimeStep, SimulationRun.CompletionCallback {
  ...
  public void initialize() throws JAXBException, SAXException {
    ...
    if (inputData.getScenario().getSimulation().isSetTimeOffset()) {
      DateTime dateTime = LocalDateTime.parse(inputData.getScenario().getSimulation().getTimeOffset(), DateTimeFormat.forPattern("YYYY-MM-dd\'T\'HH:mm:ssZ")).toDateTime(DateTimeZone.UTC);
    ...
    projectMetaData.setXodrNetworkFilename(inputData.getScenario().getNetworkFilename());
    ...
    vehicleFactory = new VehicleFactory(simulationInput.getTimestep(), inputData.getVehiclePrototypes(), inputData.getConsumption(), routing);
  ...
  private void matchRoadSegmentsAndRoadInput(List<Road> roads) {
    for (Road roadInput : roads) {
      ...
      RoadSegment roadSegment = Preconditions.checkNotNull(roadNetwork.findByUserId(roadInput.getId()), "cannot find roadId=\"" + roadInput.getId() + "\" in road network.");
    ...
    for (RoadSegment roadSegment : roadNetwork) {
      for (TrafficLight trafficLight : roadSegment.trafficLights()) {
        if (trafficLight.status() == null) {
          throw new IllegalArgumentException("trafficLight=" + trafficLight.signalId() + " on road=" + roadSegment.userId() + " hat not been initialized. Check movsim regulator input.");
        }
      }
    }
  }
  ...
  /**
     ...
     * @throws ParserConfigurationException
     */
  private static boolean parseOpenDriveXml(RoadNetwork roadNetwork, ProjectMetaData projectMetaData) throws JAXBException, SAXException {
  ...
  private void addInputToRoadSegment(RoadSegment roadSegment, Road roadInput) {
    TrafficCompositionGenerator composition = roadInput.isSetTrafficComposition() ? new TrafficCompositionGenerator(roadInput.getTrafficComposition(), vehicleFactory) : defaultTrafficComposition;
    ...
    if (roadInput.isSetTrafficSource()) {
      ...
      if (trafficSourceData.isSetInflow()) {
        ...
        if (trafficSourceData.isSetInflowFromFile()) {
          trafficSource = new TrafficSourceMicro(composition, roadSegment);
          MicroInflowFileReader reader = new MicroInflowFileReader(trafficSourceData.getInflowFromFile(), roadSegment.laneCount(), timeOffsetMillis, routing, (TrafficSourceMicro) trafficSource);
          reader.readData();
        }
      }
    ...
    if (roadInput.isSetFlowConservingInhomogeneities()) {
      for (org.movsim.autogen.Inhomogeneity inhomogeneity : roadInput.getFlowConservingInhomogeneities().getInhomogeneity()) {
    ...
    if (roadInput.isSetVariableMessageSignDiversions()) {
      for (org.movsim.autogen.VariableMessageSignDiversion diversion : roadInput.getVariableMessageSignDiversions().getVariableMessageSignDiversion()) {
        VariableMessageSignDiversion variableMessageSignDiversion = new VariableMessageSignDiversion(diversion.getPosition(), diversion.getValidLength(), roadSegment);
  ...
  private static void initialConditions(RoadSegment roadSegment, InitialConditions initialConditions, TrafficCompositionGenerator vehGenerator) {
    ...
    if (initialConditions.isSetMacroIC()) {
      ...
      if (initialConditions.isSetMicroIC()) {
  ...
  /**
     ...
     * @param vehGenerator
     */
  private static void setMacroInitialConditions(RoadSegment roadSegment, List<MacroIC> macroInitialConditions, TrafficCompositionGenerator vehGenerator) {
    ...
    for (LaneSegment laneSegment : roadSegment.laneSegments()) {
      ...
      double position = roadSegment.roadLength();
      while (position > 0) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(String.format("macroscopic init conditions from input: roadId=%s, x=%.3f, rho(x)=%.3f/km, speed=%.2fkm/h", roadSegment.id(), position, Units.INVM_TO_INVKM * rhoLocal, Units.MS_TO_KMH * speedInit));
        }
        if (rhoLocal <= 0) {
        ...
        final double gapToLeader = (leader == null) ? MovsimConstants.GAP_INFINITY : leader.getRearPosition() - position;
        if (LOG.isDebugEnabled()) {
  ...
  private static void setMicroInitialConditions(RoadSegment roadSegment, List<MicroIC> initialMicroConditions, TrafficCompositionGenerator vehGenerator) {
    ...
    for (final MicroIC ic : initialMicroConditions) {
      ...
      final Vehicle veh = (vehTypeFromFile.length() == 0) ? vehGenerator.createVehicle() : vehGenerator.createVehicle(vehTypeFromFile);
      ...
      if (lane < Lanes.MOST_INNER_LANE || lane > roadSegment.laneCount()) {
        throw new IllegalArgumentException("lane=" + lane + " given in initial condition does not exist for road=" + roadSegment.id() + " which has a laneCount of " + roadSegment.laneCount());
      }
      veh.setLane(lane);
      roadSegment.addVehicle(veh);
      LOG.info(String.format("set vehicle with label = %s on lane=%d with front at x=%.2f, speed=%.2f", veh.getLabel(), veh.lane(), veh.getFrontPosition(), veh.getSpeed()));
      if (veh.getLongitudinalModel().isCA()) {
        LOG.info(String.format("and for the CA in physical quantities: front position at x=%.2f, speed=%.2f", veh.physicalQuantities().getFrontPosition(), veh.physicalQuantities().getSpeed()));
      }
    }
  }
  ...
  public void reset() {
    ...
    if (inputData.getScenario().isSetOutputConfiguration()) {
      simOutput = new SimulationOutput(simulationRunnable.timeStep(), projectMetaData.isInstantaneousFileOutput(), inputData.getScenario().getOutputConfiguration(), roadNetwork, routing, vehicleFactory);
    }
    obstacleCount = roadNetwork.obstacleCount();
  }

  public void runToCompletion() {
    LOG.info("Simulator.run: start simulation at {} seconds of simulation project={}", simulationRunnable.simulationTime(), projectName);
    startTimeMillis = System.currentTimeMillis();
  ...
  @Override public void simulationComplete(double simulationTime) {
    LOG.info(String.format("Simulator.run: stop after time = %.2fs = %.2fh of simulation project=%s", simulationTime, simulationTime / 3600., projectName));
    ...
    if (LOG.isInfoEnabled()) {
      LOG.info(String.format("time elapsed=%d milliseconds --> simulation time warp = %.2f, time per 1000 update steps=%.3fs", elapsedTimeMillis, simulationTime / TimeUnit.MILLISECONDS.toSeconds(elapsedTimeMillis), (1000. * TimeUnit.MILLISECONDS.toSeconds(elapsedTimeMillis) / simulationRunnable.iterationCount())));
  ...
  @Override public void timeStep(double dt, double simulationTime, long iterationCount) {
    if (LOG.isInfoEnabled() && iterationCount % 1000 == 0) {
      ...
      LOG.info(String.format("Simulator.update :time = %.2fs = %.2fh, dt = %.2fs, vehicles=%d, projectName=%s", simulationTime, simulationTime / 3600, dt, numberOfVehicles, projectName));
    }
