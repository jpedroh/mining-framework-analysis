...
public class Simulator implements SimulationTimeStep, SimulationRun.CompletionCallback {
  ...
  private void matchRoadSegmentsAndRoadInput(List<Road> roads) {
    for (Road roadInput : roads) {
      ...
      RoadSegment roadSegment = Preconditions.checkNotNull(roadNetwork.findByUserId(roadInput.getId()), "cannot find roadId=" + roadInput.getId() + " from input in constructed roadNetwork. IGNORE DATA!!!");
...
>>>>>>> /usr/src/app/output/movsim/movsim/633cf8a7804a537c3b908eff2ec21dc4213d9b77/core/src/main/java/org/movsim/simulator/Simulator.java/right.java
  ...
  private void addInputToRoadSegment(RoadSegment roadSegment, Road roadInput) {
    ...
    if (roadInput.isSetFlowConservingInhomogeneities()) {
      roadSegment.setFlowConservingBottlenecks(new FlowConservingBottlenecks(roadInput.getFlowConservingInhomogeneities()));
    }
  ...
  private void createParkingSink(TrafficSink trafficSink, RoadSegment roadSegment) {
    ...
    RoadSegment sourceRoadSegment = Preconditions.checkNotNull(roadNetwork.findByUserId(parking.getSourceRoadId()), "cannot find roadSegment=" + parking.getSourceRoadId() + " specified as re-entrance from the road=" + roadSegment.id());
  ...
  @Override public void simulationComplete(double simulationTime) {
    ...
    LOG.info(String.format("time elapsed = %.3fs --> simulation time warp = %.2f, time per 1000 update steps=%.3fs", elapsedTime, simulationTime / elapsedTime, 1000 * elapsedTime / simulationRunnable.iterationCount()));
  }

  @Override public void timeStep(double dt, double simulationTime, long iterationCount) {
    if (iterationCount % 200 == 0) {
      if (LOG.isInfoEnabled()) {
        LOG.info(String.format("Simulator.update :time = %.2fs = %.2fh, dt = %.2fs, projectName=%s", simulationTime, simulationTime / 3600, dt, projectName));
      }
    }
