...
public class FilterItem extends BaseObject implements QueryItem, Cloneable, IRowFilter {
  ...
  /**
     ...
     */
  @Override public String toSql(boolean includeSchemaInColumnPaths) {
    if (_expression != null) {
      return _expression;
    }
    ...
    if (_childItems == null) {
      ...
      if (_operand == null && _operator == OperatorType.EQUALS_TO) {
        ...
        if (_operand == null && _operator == OperatorType.DIFFERENT_FROM) {
    ...
    if (_childItems == null) {
      ...
      if (operandValue == null) {
        if (_operator == OperatorType.DIFFERENT_FROM) {
          ...
          if (_operator == OperatorType.EQUALS_TO) {
            return (selectItemValue == null);
          } else {
            return false;
          }
  ...
  private boolean compare(Object selectItemValue, Object operandValue) {
    ...
    if (_operator == OperatorType.DIFFERENT_FROM) {
      ...
      if (_operator == OperatorType.EQUALS_TO) {
        ...
        if (_operator == OperatorType.GREATER_THAN) {
          return comparator.compare(selectItemValue, operandValue) > 0;
        } else {
          if (_operator == OperatorType.GREATER_THAN_OR_EQUAL) {
            return comparator.compare(selectItemValue, operandValue) >= 0;
          } else {
            if (_operator == OperatorType.LESS_THAN) {
              return comparator.compare(selectItemValue, operandValue) < 0;
            } else {
              if (_operator == OperatorType.LESS_THAN_OR_EQUAL) {
                return comparator.compare(selectItemValue, operandValue) <= 0;
              } else {
                if (_operator == OperatorType.LIKE) {
                  ...
                  if (_operator == OperatorType.NOT_LIKE) {
                    WildcardPattern matcher = new WildcardPattern((String) operandValue, '%');
                    return !matcher.matches((String) selectItemValue);
                  } else {
                    if (_operator == OperatorType.IN) {
                      Set<?> inValues = getInValues();
                      return inValues.contains(selectItemValue);
                    } else {
                      if (_operator == OperatorType.NOT_IN) {
                        Set<?> inValues = getInValues();
                        return !inValues.contains(selectItemValue);
                      } else {
  ...
  @Override protected FilterItem clone() {
  ...
  @Override protected void decorateIdentity(List<Object> identifiers) {
  ...
  @Override public boolean accept(Row row) {
    return evaluate(row);
  }
}