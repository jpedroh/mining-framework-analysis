...
public final class PropertiesUtils {
  ...
  /**
     ...
     */
  public static boolean updateProperties(ManipulationSession session, Set<Project> projects, boolean ignoreStrict, String key, String newValue) throws ManipulationException {
    ...
    for (final Project p : projects) {
      if (p.getModel().getProperties().containsKey(key)) {
        ...
        if (oldValue != null && oldValue.startsWith("${")) {
          if (!updateProperties(session, projects, ignoreStrict, oldValue.substring(2, oldValue.indexOf('}')), newValue)) {
          ...
          if (state.getStrict() && !ignoreStrict) {
            if (!checkStrictValue(session, oldValue, newValue)) {
              if (state.getFailOnStrictViolation()) {
                throw new ManipulationException("Replacing original property version {} with new version {} for {} violates the strict version-alignment rule!", oldValue, newValue, key);
              } else {
  ...
  public static boolean checkStrictValue(ManipulationSession session, String oldValue, String newValue) {
    ...
    if (state.getIncrementalSerialSuffix() != null && !state.getIncrementalSerialSuffix().isEmpty()) {
      ...
      if (state.getSuffix() != null && !state.getSuffix().isEmpty()) {
  ...
  /**
     ...
     * @throws ManipulationException
     */
  public static boolean cacheProperty(Map<String, String> versionPropertyUpdateMap, String oldVersion, String newVersion, Object originalType) throws ManipulationException {
    boolean result = false;
    if (oldVersion != null && oldVersion.startsWith("${")) {
      ...
      if (endIndex != oldVersion.length() - 1) {
        throw new ManipulationException("NYI : handling for versions (" + oldVersion + ") with multiple embedded properties is NYI. ");
      } else {
        if ("project.version".equals(oldProperty)) {
          ...
          logger.debug("For {} ; original version was a property mapping; caching new value for update {} -> {}", originalType, oldProperty, newVersion);
  ...
  /**
     ...
     * @throws ManipulationException
     */
  public static String resolveProperties(List<Project> projects, String version) throws ManipulationException {
    String result = version;
    if (version.startsWith("${")) {
      final int endIndex = version.indexOf('}');
      if (endIndex != version.length() - 1) {
        throw new ManipulationException("NYI : handling for versions (" + version + ") with multiple embedded properties is NYI. ");
      }
