...
public class XDIJSONPARSEWriter extends AbstractXDIWriter {
  ...
  @Override public Writer write(Graph graph, Writer writer) throws IOException {
    ...
    if (!this.isWriteImplied()) {
      statements = new SelectingNotImpliedStatementIterator(statements);
    }
    JsonArray gom = makeGom(statements);
    JsonWriter jsonWriter = new JsonWriter(writer);
    if (this.isWritePretty()) {
  ...
  private static JsonArray makeGom(XDIStatement statement) {
    ...
    if (statement.getObject() instanceof XDIAddress) {
      ...
      if (statement.getObject() instanceof XDIArc) {
  ...
  private static JsonElement makeGom(XDIAddress XDIaddress) {
    ...
    if (XDIaddress.getNumXDIArcs() == 1) {
      ...
      for (int i = 0; i < XDIaddress.getNumXDIArcs(); i++) {
        ((JsonArray) gom).add(makeGom(XDIaddress.getXDIArc(i)));
      }
    }
    return gom;
  }
