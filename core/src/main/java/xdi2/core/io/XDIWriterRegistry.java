...
public final class XDIWriterRegistry {
  ...
  private static String writerClassNames[] = { XDIJSONWriter.class.getName(), XDIDisplayWriter.class.getName(), XDIJSONTREEWriter.class.getName(), XDIJSONPARSEWriter.class.getName(), 
...
>>>>>>> Unknown file: This is a bug in JDime.
  , XDIKeyValueWriter.class.getName(), XDIHTMLWriter.class.getName() };
  ...
  static {
    ...
    for (Class<XDIWriter> writerClass : writerClasses) {
      ...
      if (format != null) {
        writerClassesByFormat.put(format, writerClass);
      }
      if (fileExtension != null) {
        writerClassesByFileExtension.put(fileExtension, writerClass);
      }
      if (mimeType != null) {
        writerClassesByMimeType.put(mimeType, writerClass);
      }
    }
    defaultWriterClass = writerClasses.get(0);
  }

  @SuppressWarnings(value = { "unchecked" }) private static Class<XDIWriter> forName(String writerClassName) throws ClassNotFoundException {
  ...
  public static XDIWriter forFormat(String format, Properties parameters) {
    if (format == null) {
      return XDIWriterRegistry.getDefault();
    }
