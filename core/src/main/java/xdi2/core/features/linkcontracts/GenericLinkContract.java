...
public class GenericLinkContract extends LinkContractBase {
  ...
  public static boolean isValid(XdiEntity xdiEntity) {
    if (xdiEntity instanceof XdiEntitySingleton) {
      if (!((XdiEntitySingleton) xdiEntity).getBaseArcXri().equals(XdiAbstractContext.getBaseArcXri(XDILinkContractConstants.XRI_SS_DO))) {
        return false;
      }
      if (getAuthorizingParty(xdiEntity.getXri()) == null) {
        return false;
      }
      if (getRequestingParty(xdiEntity.getXri()) == null) {
        return false;
      }
      return true;
    } else {
      if (xdiEntity instanceof XdiEntityMember) {
        if (!((XdiEntityMember) xdiEntity).getXdiCollection().getBaseArcXri().equals(XdiAbstractContext.getBaseArcXri(XDILinkContractConstants.XRI_SS_DO))) {
          return false;
        }
        if (getAuthorizingParty(xdiEntity.getXri()) == null) {
          return false;
  ...
  public static GenericLinkContract findGenericLinkContract(Graph graph, XDI3Segment authorizingParty, XDI3Segment requestingParty, XDI3Segment templateId, boolean create) {
    ...
    if (genericLinkContractContextNode == null) {
      return null;
    }
  ...
  public static XDI3Segment getTemplateId(XDI3Segment xri) {
    ...
    if (index2 < 0) {
      index2 = XDI3Util.indexOfXri(xri, XdiEntityCollection.createArcXri(XDILinkContractConstants.XRI_SS_DO));
    }
    if (index1 < 0 || index2 < 0 || index1 >= index2) {
      return null;
    }
