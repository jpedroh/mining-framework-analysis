...
public class XDIArc extends XDIIdentifier {
  ...
  static XDIArc fromComponents(String string, Character cs, boolean variable, boolean definition, boolean collection, boolean attribute, boolean immutable, boolean relative, String literal, XDIXRef xref) {
    if (string == null) {
      ...
      if (variable) {
        buffer.append(XDIConstants.XS_VARIABLE.charAt(0));
      }
      if (definition) {
        buffer.append(XDIConstants.XS_DEFINITION.charAt(0));
      }
      if (collection) {
      ...
      if (immutable) {
        buffer.append(XDIConstants.S_IMMUTABLE);
      }
      if (relative) {
        buffer.append(XDIConstants.S_RELATIVE);
      }
      if (literal != null) {
        buffer.append(literal);
      }
      if (xref != null) {
        buffer.append(xref.toString());
      }
      if (attribute) {
      ...
      if (definition) {
        buffer.append(XDIConstants.XS_DEFINITION.charAt(1));
      }
      if (variable) {
        buffer.append(XDIConstants.XS_VARIABLE.charAt(1));
      }
      string = buffer.toString();
    }
    if (cs == null && literal != null) {
      throw new IllegalArgumentException("If there is a literal, must have a context symbol: " + string);
    }
    if (literal != null && xref != null) {
      throw new IllegalArgumentException("Cannot have both literal and xref: " + string);
    }
