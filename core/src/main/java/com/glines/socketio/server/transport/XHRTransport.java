...
public abstract class XHRTransport extends AbstractHttpTransport {
  ...
  protected abstract class XHRSessionHelper implements SessionTransportHandler, ContinuationListener {
    ...
    @Override public void sendMessage(SocketIOFrame frame) throws SocketIOException {
      synchronized (this) {
        Log.debug("Session[" + session.getSessionId() + "]: " + "sendMessage(frame): [" + frame.getFrameType() + "]: " + frame.getData());
        if (is_open) {
    ...
    @Override public void sendMessage(String message) throws SocketIOException {
      Log.debug("Session[" + session.getSessionId() + "]: " + "sendMessage(String): " + message);
    ...
    @Override public void sendMessage(int messageType, String message) throws SocketIOException {
      synchronized (this) {
        Log.debug("Session[" + session.getSessionId() + "]: " + "sendMessage(int, String): [" + messageType + "]: " + message);
    ...
    @Override public void abort() {
      ...
      buffer.setListener(new TransportBuffer.BufferListener() {
        @Override public boolean onMessages(List<String> messages) {
          return false;
        }

        @Override public boolean onMessage(String message) {
          return false;
        }
      });
      buffer.clear();
  ...
  @Override protected SocketIOSession connect(HttpServletRequest request, HttpServletResponse response, Transport.InboundFactory inboundFactory, com.glines.socketio.server.SocketIOSession.Factory sessionFactory) throws IOException {
