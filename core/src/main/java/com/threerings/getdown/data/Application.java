...
public class Application {
  ...
  public Resource getJavaVMResource() {
    ...
    try {
      ...
      return new Resource(vmfile, remote, getLocalPath(vmfile), EnumSet.of(Resource.Attr.UNPACK, Resource.Attr.CLEAN));
    } catch (Exception e) {
  ...
  public URL getTrackingURL(String event) {
    try {
      ...
      return _trackingURL == null ? null : HostWhitelist.verify(new URL(_trackingURL + encodePath(event + suffix + ga)));
...
<<<<<<< /usr/src/app/output/threerings/getdown/ac499c1e3c4dc81fadfe3e64e617357892c5d0e8/core/src/main/java/com/threerings/getdown/data/Application.java/left.java
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  public Config initConfig(boolean checkPlatform) throws IOException {
    ...
    try {
      if (cfgfile.exists()) {
        ...
        if ((cfgfile = getLocalPath(CONFIG_FILE + "_old")).exists()) {
  ...
  public void initConfigTracking(Config config) {
    ...
    if (!StringUtil.isBlank(trackPcts)) {
      ...
      if (!StringUtil.isBlank(_trackingURL)) {
        _trackingPcts = new HashSet<>();
  ...
  public boolean haveValidJavaVersion() {
    ...
    try {
      ...
      log.info("Checking Java version", "current", version, "wantMin", _javaMinVersion, "wantMax", _javaMaxVersion);
      ...
      if (_javaExactVersionRequired) {
        if (version == _javaMinVersion) {
          ...
          log.warning("An exact Java VM version is required.", "current", version, "required", _javaMinVersion);
          return false;
        }
      }
      ...
      log.warning("Unable to parse VM version, hoping for the best", "error", re, "needed", _javaMinVersion);
      return true;
    }
  }
  ...
  public boolean verifyMetadata(StatusDisplay status) throws IOException {
    ...
    if (_version != -1) {
      ...
      if (_latest != null) {
        try (InputStream in = ConnectionUtil.open(proxy, _latest, 0, 0).getInputStream(); InputStreamReader reader = new InputStreamReader(in, UTF_8); BufferedReader bin = new BufferedReader(reader)) {
          for (String[] pair : Config.parsePairs(bin, Config.createOpts(false))) {
            if (pair[0].equals("version")) {
              ...
              if (fileVersion != -1 && _targetVersion > fileVersion) {
                try (FileOutputStream fos = new FileOutputStream(vfile); PrintStream out = new PrintStream(fos)) {
  ...
  public void verifyResources(ProgressObserver obs, int[] alreadyValid, Set<Resource> unpacked, Set<Resource> toInstall, Set<Resource> toDownload) throws InterruptedException {
    ...
    for (int ii = 0; ii < sizes.length; ii++) {
      ...
      exec.execute(new Runnable() {
        public void run() {
          verifyResource(rsrc, pagg.startElement(index), fAlreadyValid, unpackedAsync, toInstallAsync, toDownloadAsync);
          actions.add(new Runnable() {
            public void run() {
              completed[0] += 1;
            }
    ...
    log.info("Verified resources", "count", rsrcs.size(), "size", (totalSize / 1024) + "k", "duration", (complete - start) + "ms");
  }
  ...
  private void verifyResource(Resource rsrc, ProgressObserver obs, int[] alreadyValid, Set<Resource> unpacked, Set<Resource> toInstall, Set<Resource> toDownload) {
    ...
    try {
      ...
      log.info("Failure verifying resource. Requesting redownload...", "rsrc", rsrc, "error", e);
    } finally {
      obs.progress(100);
    }
    toDownload.add(rsrc);
