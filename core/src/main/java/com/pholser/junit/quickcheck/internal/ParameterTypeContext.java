...
public class ParameterTypeContext {
  ...
  public ParameterTypeContext(String parameterName, AnnotatedType parameterType, String declarerName) {
    this(parameterName, parameterType, declarerName, emptyMap());
  }
  ...
  public ParameterTypeContext(String parameterName, AnnotatedType parameterType, String declarerName, Map<String, Type> typeVariables) {
    this(parameterName, parameterType, declarerName, Types.forJavaLangReflectType(parameterType.getType(), toTokens(typeVariables)), toTokens(typeVariables));
  }
  ...
  public ParameterTypeContext(Class<?> clazz) {
    this(clazz.getTypeName(), FakeAnnotatedTypeFactory.makeFrom(clazz), clazz.getTypeName(), Types.forJavaLangReflectType(clazz), emptyMap());
  }

  private static Map<String, org.javaruntype.type.Type<?>> toTokens(Map<String, Type> typeVariables) {
    return typeVariables.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, (e) -> Types.forJavaLangReflectType(e.getValue())));
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private Generator<?> makeGenerator(Class<? extends Generator> generatorType) {
  ...
  private void ensureCorrectType(Generator<?> generator) {
    org.javaruntype.type.Type<?> parameterTypeToken = Types.forJavaLangReflectType(type(), typeVariables);
    for (Class<?> each : generator.types()) {
      if (!maybeWrap(parameterTypeToken.getRawClass()).isAssignableFrom(maybeWrap(each))) {
        throw new IllegalArgumentException(format(EXPLICIT_GENERATOR_TYPE_MISMATCH_MESSAGE, each, From.class.getName(), parameterName));
      }
    }
  }
  ...
  public ParameterTypeContext arrayComponentContext() {
    @SuppressWarnings(value = { "unchecked" }) org.javaruntype.type.Type<?> component = arrayComponentOf((org.javaruntype.type.Type<Object[]>) token);
    AnnotatedType annotatedComponent = annotatedArrayComponent(component);
    return new ParameterTypeContext(annotatedComponent.getType().getTypeName(), annotatedComponent, parameterType.getType().getTypeName(), component, typeVariables).annotate(annotatedComponent).allowMixedTypes(true);
  }

  private AnnotatedType annotatedArrayComponent(org.javaruntype.type.Type<?> component) {
    return parameterType instanceof AnnotatedArrayType ? ((AnnotatedArrayType) parameterType).getAnnotatedGenericComponentType() : FakeAnnotatedTypeFactory.makeFrom(component.getComponentClass());
  }
  ...
  public List<ParameterTypeContext> typeParameterContexts(SourceOfRandomness random) {
    ...
    for (int i = 0; i < typeParameters.size(); ++i) {
      ...
      AnnotatedType a = annotatedTypeParameters.size() > i ? annotatedTypeParameters.get(i) : zilch();
  ...
  private void addStandardTypeParameterContext(List<ParameterTypeContext> typeParameterContexts, TypeParameter<?> p, AnnotatedType a) {
    typeParameterContexts.add(new ParameterTypeContext(p.getType().getName(), a, annotatedType().getType().getTypeName(), p.getType(), typeVariables).allowMixedTypes(!(a instanceof TypeVariable)).annotate(a));
  }

  private void addWildcardTypeParameterContext(List<ParameterTypeContext> typeParameterContexts, AnnotatedType a) {
    typeParameterContexts.add(new ParameterTypeContext("Zilch", a, annotatedType().getType().getTypeName(), Types.forJavaLangReflectType(Zilch.class), typeVariables).allowMixedTypes(true).annotate(a));
  }
  ...
  private void addExtendsTypeParameterContext(List<ParameterTypeContext> typeParameterContexts, TypeParameter<?> p, AnnotatedType a) {
    typeParameterContexts.add(new ParameterTypeContext(p.getType().getName(), annotatedComponentTypes(a).get(0), annotatedType().getType().getTypeName(), p.getType(), typeVariables).allowMixedTypes(false).annotate(a));
  }
  ...
  private void addSuperTypeParameterContext(SourceOfRandomness random, List<ParameterTypeContext> typeParameterContexts, TypeParameter<?> p, AnnotatedType a) {
    ...
    typeParameterContexts.add(new ParameterTypeContext(p.getType().getName(), annotatedComponentTypes(a).get(0), annotatedType().getType().getTypeName(), choice, typeVariables).allowMixedTypes(false).annotate(a));
  }
