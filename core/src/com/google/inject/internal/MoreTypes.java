...
public class MoreTypes {
   ...
   */
  public static <T extends java.lang.Object> TypeLiteral<T> canonicalizeForKey(TypeLiteral<T> typeLiteral) {
    ...
    if (typeLiteral.getRawType() == javax.inject.Provider.class) {
      ...
      @SuppressWarnings(value = { "unchecked" }) TypeLiteral<T> guiceProviderType = (TypeLiteral<T>) TypeLiteral.get(Types.providerOf(parameterizedType.getActualTypeArguments()[0]));
      return guiceProviderType;
    }
    @SuppressWarnings(value = { "unchecked" }) TypeLiteral<T> wrappedPrimitives = (TypeLiteral<T>) PRIMITIVE_TO_WRAPPER.get(typeLiteral);
  ...
  public static class ParameterizedTypeImpl implements ParameterizedType, Serializable, CompositeType {
    ...
    public ParameterizedTypeImpl(Type ownerType, Type rawType, Type... typeArguments) {
      if (rawType instanceof Class<?>) {
        ...
        checkArgument(ownerType != null || rawTypeAsClass.getEnclosingClass() == null, "No owner type for enclosed %s", rawType);
        checkArgument(ownerType == null || rawTypeAsClass.getEnclosingClass() != null, "Owner type for unenclosed %s", rawType);
      }
  ...
  private static void checkNotPrimitive(Type type, String use) {
    checkArgument(!(type instanceof Class<?>) || !((Class) type).isPrimitive(), "Primitive types are not allowed in %s: %s", use, type);
  }
