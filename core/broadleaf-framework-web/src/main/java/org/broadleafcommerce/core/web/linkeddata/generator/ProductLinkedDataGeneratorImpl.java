...
@Service(value = "blProductLinkedDataGenerator") public class ProductLinkedDataGeneratorImpl extends AbstractLinkedDataGenerator {
  ...
  @Resource(name = "blRatingService") protected RatingService ratingService;
  ...
  protected JSONObject addProductData(final HttpServletRequest request, final Product product, final String url) throws JSONException {
    ...
    if (sku.isActive()) {
      if (sku.getInventoryType() != null) {
        if (sku.getInventoryType().equals(InventoryType.ALWAYS_AVAILABLE)) {
          ...
          if (sku.getInventoryType().equals(InventoryType.CHECK_QUANTITY) && sku.getQuantityAvailable() != null && sku.getQuantityAvailable() > 0) {
            purchasable = true;
          }
        }
      } else {
