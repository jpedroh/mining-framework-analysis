 ...
 */
@Deprecated @Component(value = "blOnePageCheckoutProcessor") @ConditionalOnTemplating public class OnePageCheckoutProcessor extends AbstractBroadleafVariableModifierProcessor {
  ...
  @Resource(name = "blFulfillmentPricingService") protected FulfillmentPricingService fulfillmentPricingService;

  @Resource(name = "blOrderToPaymentRequestDTOService") protected OrderToPaymentRequestDTOService orderToPaymentRequestDTOService;

  @Resource(name = "blCartStateService") protected CartStateService cartStateService;

  @Resource(name = "blCheckoutFormService") protected CheckoutFormService checkoutFormService;
  ...
  @Override public boolean useGlobalScope() {
    return false;
  }

  @Override public Map<String, Object> populateModelVariables(String tagName, Map<String, String> tagAttributes, BroadleafTemplateContext context) {
  ...
  protected void populateSectionViewStates(Map<String, Object> localVars) {
    ...
    if (orderContainsThirdPartyPayment || orderContainsUnconfirmedCreditCard) {
      ...
      if (orderTotalAfterAppliedPayments != null && orderTotalAfterAppliedPayments.isZero()) {
        showAllPaymentMethods = false;
      }
    }
    ...
    for (ListIterator<CheckoutSectionDTO> itr = drawnSections.listIterator(); itr.hasNext(); ) {
      ...
      if (CheckoutSectionViewType.ORDER_INFO.equals(section.getView()) && editOrderInfo) {
        ...
        if (CheckoutSectionViewType.BILLING_INFO.equals(section.getView()) && editBillingInfo) {
          ...
          if (CheckoutSectionViewType.SHIPPING_INFO.equals(section.getView()) && editShippingInfo) {
            section.setState(CheckoutSectionStateType.FORM);
          }
        }
      }
