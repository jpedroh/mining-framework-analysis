 ...
 * Date: 9/26/12
 */
@Service(value = "blUpdateCartService") public class UpdateCartServiceImpl implements UpdateCartService {
  ...
  @Resource(name = "blUpdateCartServiceExtensionManager") protected UpdateCartServiceExtensionManager extensionManager;

  @Autowired @Qualifier(value = "blOrderLockManager") protected OrderLockManager orderLockManager;

  @Override public boolean currencyHasChanged() {
    BroadleafCurrency currency = findActiveCurrency();
    if (getSavedCurrency() == null) {
      setSavedCurrency(currency);
    } else {
      if (getSavedCurrency() != currency) {
        return true;
      }
    }
    return false;
  ...
  @Override public UpdateCartResponse copyCartToCurrentContext(Order currentCart) {
  ...
  @Override public void validateAddToCartRequest(OrderItemRequestDTO itemRequest, Order cart) {
    if (extensionManager != null) {
      extensionManager.getProxy().validateAddToCartItem(itemRequest, cart);
    }
  }

  @Override public void updateAndValidateCart(Order cart) {
    if (extensionManager != null) {
      ...
      if (clearCart != null && clearCart.booleanValue()) {
        ...
        try {
          if (repriceCart != null && repriceCart.booleanValue()) {
            ...
            if (saveCart != null && saveCart.booleanValue()) {
              Object lockObject = null;
              try {
                lockObject = lockOrder(cart, lockObject);
  ...
  @Override public void setSavedCurrency(BroadleafCurrency savedCurrency) {
    this.savedCurrency = savedCurrency;
  }

  @Override public BroadleafCurrency getSavedCurrency() {
