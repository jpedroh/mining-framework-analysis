...
@Service(value = "blProductOptionValidationService") public class ProductOptionValidationServiceImpl implements ProductOptionValidationService {
  ...
  @Resource protected ProductOptionDao productOptionDao;

  @Autowired protected Environment environment;

  @Value(value = "${exploitProtection.xssEnabled:false}") protected boolean xssExploitProtectionEnabled;

  @Value(value = "${blc.site.enable.xssWrapper:false}") protected boolean siteXssWrapperEnabled;
  ...
  @Override public Boolean validate(ProductOption productOption, String value) {
    ...
    if (isRequiredAttributeNotProvided(productOption, value)) {
      ...
      if (requiresValidation(productOption, value) && !validateRegex(validationString, value)) {
        ...
        throw new ProductOptionValidationException(errorMessage, productOption.getErrorCode(), attributeName, value, validationString, errorMessage);
      }
    }
    return true;
  }
  ...
  @Override public boolean hasProductOptionValidationStrategy(ProductOption productOption) {
    return productOption.getProductOptionValidationStrategyType() != null;
  }

  @Override public boolean isSubmitType(ProductOption productOption) {
  ...
  @Override public boolean isAddOrNoneType(ProductOption productOption) {
    boolean hasStrategy = hasProductOptionValidationStrategy(productOption);
    return hasStrategy && productOption.getProductOptionValidationStrategyType().getRank() <= ADD_TYPE_RANK;
  }

  @Override public void validateWithoutException(ProductOption productOption, String attributeValue, ActivityMessages messages) {
    try {
  ...
  @Override public List<Long> findSkuIdsForProductOptionValues(Long productId, String attributeName, String attributeValue, List<Long> possibleSkuIds) {
