 ...
 * @author Jeff Fischer
 */
@Repository(value = "blCategoryDao") public class CategoryDaoImpl implements CategoryDao {
  @Value(value = "${query.dateResolution.category:10000}") protected Long currentDateResolution;
  ...
  @Resource(name = "blCategoryDaoExtensionManager") protected CategoryDaoExtensionManager extensionManager;
  ...
  @Override public List<Category> readCategoriesByIds(List<Long> categoryIds) {
    TypedQuery<Category> query = em.createQuery("select category from org.broadleafcommerce.core.catalog.domain.Category category " + "where category.id in :ids", Category.class);
    query.setParameter("ids", sandBoxHelper.mergeCloneIds(CategoryImpl.class, categoryIds.toArray(new Long[categoryIds.size()])));
    query.setHint(QueryHints.HINT_CACHEABLE, true);
    query.setHint(QueryHints.HINT_CACHE_REGION, "query.Catalog");
    return query.getResultList();
  }

  @Override public Category readCategoryByExternalId(@Nonnull String externalId) {
  ...
  @Override @Deprecated public Category readCategoryByName(String categoryName) {
  ...
  @Nonnull @Override public List<Category> readCategoriesByNames(List<String> names) {
    TypedQuery<Category> query = em.createQuery("select category from org.broadleafcommerce.core.catalog.domain.Category category " + "where category.name in :names", Category.class);
  ...
  @Override public Long readTotalCategoryCount() {
  ...
  @Override public List<Category> readAllSubCategories(Long id) {
    TypedQuery<Category> query = em.createNamedQuery("BC_READ_ALL_SUBCATEGORIES", Category.class);
  ...
  @Override public Category findCategoryByURI(String uri) {
    ...
    query.setHint(QueryHints.HINT_CACHEABLE, true);
    query.setHint(QueryHints.HINT_CACHE_REGION, "query.Catalog");
    @SuppressWarnings(value = { "unchecked" }) List<Category> results = query.getResultList();
  ...
  @Override public Long readCountAllActiveProductsByCategory(Category category) {
  ...
  @Override public List<CategoryProductXref> findXrefByCategoryWithDefaultReference(Long categoryId) {
    TypedQuery<CategoryProductXref> query = em.createQuery("SELECT xref FROM org.broadleafcommerce.core.catalog.domain.CategoryProductXrefImpl xref" + " WHERE xref.category.id = :categoryId AND xref.defaultReference=TRUE", CategoryProductXref.class);
