...
@Service(value = "blItemOfferProcessor") public class ItemOfferProcessorImpl extends OrderOfferProcessorImpl implements ItemOfferProcessor, ItemOfferMarkTargets {
  ...
  @Override public void filterItemLevelOffer(PromotableOrder order, List<PromotableCandidateItemOffer> qualifiedItemOffers, Offer offer) {
  ...
  @Override public void applyAllItemOffers(List<PromotableCandidateItemOffer> itemOffers, PromotableOrder order) {
    for (PromotableCandidateItemOffer itemOffer : itemOffers) {
      if (orderMeetsQualifyingSubtotalRequirements(order, itemOffer) && orderMeetsTargetSubtotalRequirements(order, itemOffer) && orderMeetsSubtotalRequirements(order, itemOffer)) {
  ...
  @Override public void filterOffers(PromotableOrder order, List<Offer> filteredOffers, List<PromotableCandidateOrderOffer> qualifiedOrderOffers, List<PromotableCandidateItemOffer> qualifiedItemOffers) {
    ...
    if (itemOffers.size() > 1) {
      for (PromotableCandidateItemOffer itemOffer : itemOffers) {
        ...
        for (PromotableOrderItemPriceDetail detail : order.getAllPromotableOrderItemPriceDetails()) {
          ...
          for (PromotionDiscount discount : detail.getPromotionDiscounts()) {
            ...
            if (useCalculatePercent(offer)) {
              ...
              if (hasQualifierAndQualifierRestricted(offer)) {
        ...
        if (usePercentOffValue(offer)) {
          ...
          if (useCalculatePercent(offer) || hasQualifierAndQualifierRestricted(offer)) {
  ...
  private boolean hasQualifierAndQualifierRestricted(Offer offer) {
    return !offer.getQualifyingItemCriteriaXref().isEmpty() && Objects.equals(offer.getOfferItemQualifierRuleType(), OfferItemRestrictionRuleType.NONE);
  }
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public void applyAndCompareOrderAndItemOffers(PromotableOrder order, List<PromotableCandidateOrderOffer> qualifiedOrderOffers, List<PromotableCandidateItemOffer> qualifiedItemOffers) {
