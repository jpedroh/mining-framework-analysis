...
@Repository(value = "blSearchFacetDao") public class SearchFacetDaoImpl implements SearchFacetDao {
  ...
  @Override public List<SearchFacet> readAllSearchFacets(FieldEntity entityType) {
    ...
    criteria.where(builder.equal(facet.get("showOnSearch").as(Boolean.class), true), builder.or(builder.isNull(archived.as(String.class)), builder.notEqual(archived.as(Character.class), 'Y')), facet.join("fieldType").join("indexField").join("field").get("entityType").as(String.class).in(entityType.getAllLookupTypes()));
  ...
  @Override public <T extends java.lang.Object> List<T> readDistinctValuesForField(String fieldName, Class<T> fieldValueClass) {
    CriteriaBuilder builder = em.getCriteriaBuilder();
    ...
    if (fieldName.contains("defaultSku.")) {
      ...
      if (fieldName.contains("productAttributes.")) {
  ...
  @Override public SearchFacet save(SearchFacet searchFacet) {
    return em.merge(searchFacet);
  }

  @Override public SearchFacet readSearchFacetForField(Field field) {
    CriteriaBuilder builder = em.getCriteriaBuilder();
    CriteriaQuery<SearchFacet> criteria = builder.createQuery(SearchFacet.class);
    Root<SearchFacetImpl> facet = criteria.from(SearchFacetImpl.class);
    criteria.select(facet);
  ...
  @Override public List<SearchFacetRange> readSearchFacetRangesForSearchFacet(SearchFacet searchFacet) {
    CriteriaBuilder builder = em.getCriteriaBuilder();
