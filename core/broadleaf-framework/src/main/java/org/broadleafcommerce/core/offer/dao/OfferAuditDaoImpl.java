...
@Repository(value = "blOfferAuditDao") public class OfferAuditDaoImpl implements OfferAuditDao {
  ...
  @Override public Long countUsesByCustomer(Order order, Long customerId, Long offerId) {
    return countUsesByCustomer(order, customerId, offerId, null);
  }

  @Override public Long countUsesByAccount(Order order, Long accountId, Long offerId) {
    return countUsesByAccount(order, accountId, offerId, null);
  }

  @Override public Long countUsesByAccount(Order order, Long accountId, Long offerId, Long minimumDaysPerUsage) {
    return countUsesByAccountOrCustomer(order, NULL_CUSTOMER_ID, accountId, offerId, minimumDaysPerUsage);
  }

  @Override public Long countUsesByCustomer(Order order, Long customerId, Long offerId, Long minimumDaysPerUsage) {
  ...
  protected Long countUsesByAccountOrCustomer(Order order, Long customerId, Long accountId, Long offerId, Long minimumDaysPerUsage) {
    ...
    restrictions.add(builder.and(customerOrAccountPredicate, builder.equal(root.get("offerId"), offerId), builder.or(builder.isNull(root.get("orderId")), builder.and(builder.notEqual(root.get("orderId"), getOrderId(order)), builder.notEqual(orderRoot.get("status"), OrderStatus.CANCELLED.getType()), builder.equal(orderRoot.get("id"), root.get("orderId")))), getOmsOrderPredicate(builder, orderRoot, parentOrder)));
  ...
  protected Predicate getOmsOrderPredicate(CriteriaBuilder builder, Root<OrderImpl> orderRoot, Join<Object, Object> parentOrder) {
    if (ModulePresentUtil.isPresent(BroadleafModuleRegistration.BroadleafModuleEnum.OMS)) {
      return builder.or(builder.isNull(orderRoot.get("embeddedOmsOrder").get("parentOrder")), builder.notEqual(parentOrder.get("status"), OrderStatus.CANCELLED.getType()));
    } else {
  ...
  @Deprecated @Override public Long countUsesByCustomer(Long customerId, Long offerId) {
    TypedQuery<Long> query = new TypedQueryBuilder<>(OfferAudit.class, "offerAudit").addRestriction("offerAudit.customerId", "=", customerId).addRestriction("offerAudit.offerId", "=", offerId).toCountQuery(em);
    return query.getSingleResult();
  }

  @Override public Long countOfferCodeUses(Order order, Long offerCodeId) {
    CriteriaBuilder builder = em.getCriteriaBuilder();
    CriteriaQuery<Long> criteria = builder.createQuery(Long.class);
    Root<OfferAuditImpl> root = criteria.from(OfferAuditImpl.class);
    Root<OrderImpl> orderRoot = criteria.from(OrderImpl.class);
  ...
  @Deprecated @Override public Long countOfferCodeUses(Long offerCodeId) {
    TypedQuery<Long> query = new TypedQueryBuilder<>(OfferAudit.class, "offerAudit").addRestriction("offerAudit.offerCodeId", "=", offerCodeId).toCountQuery(em);
    return query.getSingleResult();
  }

  @Override public List<OfferAudit> readOfferAuditsByOrderId(Long orderId) {
    TypedQuery<OfferAudit> query = new TypedQueryBuilder<>(OfferAudit.class, "offerAudit").addRestriction("offerAudit.orderId", "=", orderId).toQuery(em);
