 ...
 * @author apazzolini
 */
@Service(value = "blOrderService") @ManagedResource(objectName = "org.broadleafcommerce:name=OrderService", description = "Order Service", currencyTimeLimit = 15) public class OrderServiceImpl implements OrderService {
  private static final Log LOG = LogFactory.getLog(OrderServiceImpl.class);

  @Resource(name = "blOrderPaymentDao") protected OrderPaymentDao paymentDao;

  @Resource(name = "blOrderDao") protected OrderDao orderDao;

  @Resource(name = "blOfferDao") protected OfferDao offerDao;

  @Resource(name = "blNullOrderFactory") protected NullOrderFactory nullOrderFactory;

  @Resource(name = "blPricingService") protected PricingService pricingService;
  ...
  @Resource(name = "blSecureOrderPaymentService") protected SecureOrderPaymentService securePaymentInfoService;

  @Resource(name = "blMergeCartService") protected MergeCartService mergeCartService;

  @Resource(name = "blOrderServiceExtensionManager") protected OrderServiceExtensionManager extensionManager;

  @Resource(name = "blAddItemWorkflow") protected Processor addItemWorkflow;

  @Resource(name = "blUpdateProductOptionsForItemWorkflow") private Processor updateProductOptionsForItemWorkflow;

  @Resource(name = "blUpdateItemWorkflow") protected Processor updateItemWorkflow;

  @Resource(name = "blRemoveItemWorkflow") protected Processor removeItemWorkflow;

  @Resource(name = "blTransactionManager") protected PlatformTransactionManager transactionManager;

  @Value(value = "${pricing.retry.count.for.lock.failure}") protected int pricingRetryCountForLockFailure = 3;

  @Value(value = "${pricing.retry.wait.interval.for.lock.failure}") protected long pricingRetryWaitIntervalForLockFailure = 500L;
  ...
  /**
     ...
     */
  @Value(value = "${auto.flush.on.query.during.add.to.cart:false}") protected boolean autoFlushAddToCart = false;
  ...
  /**
     ...
     */
  @Value(value = "${auto.flush.on.query.during.update.cart:false}") protected boolean autoFlushUpdateCart = false;
  ...
  /**
     ...
     */
  @Value(value = "${auto.flush.on.query.during.remove.from.cart:false}") protected boolean autoFlushRemoveFromCart = false;
  ...
  /**
     ...
     */
  @Value(value = "${auto.flush.on.query.during.cart.pricing.save:false}") protected boolean autoFlushSaveCart = false;
  ...
  @Resource(name = "blOfferServiceExtensionManager") protected OfferServiceExtensionManager offerServiceExtensionManager;

  @Override @Transactional(value = "blTransactionManager") public Order createNewCartForCustomer(Customer customer) {
  ...
  @Override @Transactional(value = "blTransactionManager") public Order createNamedOrderForCustomer(String name, Customer customer) {
  ...
  @Override public List<Order> findOrdersByIds(List<Long> orderIds) {
    return orderDao.readOrdersByIds(orderIds);
  }

  @Override public Order findOrderById(Long orderId, boolean refresh) {
  ...
  @Override public List<Order> findOrdersByDateRange(Date startDate, Date endDate) {
    return orderDao.readOrdersByDateRange(startDate, endDate);
  }

  @Override public List<Order> findOrdersByDaysCount(Integer daysCount, Integer batchSize) {
    return orderDao.readOrdersOlderThanDaysCount(daysCount, batchSize);
  }

  @Override public List<Order> findOrdersForCustomersInDateRange(List<Long> customerIds, Date startDate, Date endDate) {
    return orderDao.readOrdersForCustomersInDateRange(customerIds, startDate, endDate);
  }

  @Override public List<OrderPayment> findPaymentsForOrder(Order order) {
    return paymentDao.readPaymentsForOrder(order);
  }

  @Override @Transactional(value = "blTransactionManager") public OrderPayment addPaymentToOrder(Order order, OrderPayment payment, Referenced securePaymentInfo) {
  ...
  @Override public Order save(Order order, boolean priceOrder, boolean repriceItems) throws PricingException {
    if (repriceItems) {
      order.updatePrices();
    }
  ...
  @Override @Transactional(value = "blTransactionManager") public void cancelOrder(Order order) {
    orderDao.delete(order);
  }

  @Override @Transactional(value = "blTransactionManager") public void deleteOrder(Order order) {
    orderMultishipOptionService.deleteAllOrderMultishipOptions(order);
    orderDao.delete(order);
  }

  @Override @Transactional(value = "blTransactionManager") public Order addOfferCode(Order order, OfferCode offerCode, boolean priceOrder) throws PricingException, OfferException {
  ...
  @Override @Transactional(value = "blTransactionManager") public Order addOfferCodes(Order order, List<OfferCode> offerCodes, boolean priceOrder) throws PricingException, OfferException {
    ...
    if (offerCodes != null && !offerCodes.isEmpty()) {
      for (OfferCode offerCode : offerCodes) {
        if (order.getAddedOfferCodes().contains(offerCode) || addedOffers.contains(offerCode.getOffer())) {
          ...
          if (!offerService.verifyMaxCustomerUsageThreshold(order, offerCode)) {
            ...
            if (!offerCode.isActive() || !offerCode.getOffer().isActive()) {
  ...
  @Override @Transactional(value = "blTransactionManager") public Order removeOfferCode(Order order, OfferCode offerCode, boolean priceOrder) throws PricingException {
  ...
  @Override @Transactional(value = "blTransactionManager") public Order removeAllOfferCodes(Order order, boolean priceOrder) throws PricingException {
    order.getAddedOfferCodes().clear();
    order = save(order, priceOrder);
    return order;
  }

  @Override @ManagedAttribute(description = "The delete empty named order after adding items to cart attribute", currencyTimeLimit = 15) public void setDeleteEmptyNamedOrders(boolean deleteEmptyNamedOrders) {
      ...
      for (int i = (order.getOrderItems().size() - 1); i >= 0; i--) {
        ...
        if (currentItem instanceof DiscreteOrderItem) {
          ...
          if (skuId != null) {
            ...
            if (productId != null && discreteItem.getProduct() != null && productId.equals(discreteItem.getProduct().getId())) {
              return discreteItem;
            }
          }
        } else {
          if (currentItem instanceof BundleOrderItem) {
            ...
            if (skuId != null) {
              ...
              if (productId != null && bundleItem.getProduct() != null && productId.equals(bundleItem.getProduct().getId())) {
                return bundleItem;
              }
            }
          }
  ...
  @Override @Transactional(value = "blTransactionManager") public Order confirmOrder(Order order) {
    return orderDao.submitOrder(order);
  }

  @Override @Transactional(value = "blTransactionManager") public Order addAllItemsFromNamedOrder(Order namedOrder, boolean priceOrder) throws RemoveFromCartException, AddToCartException {
    ...
    CollectionUtils.filter(items, new TypedPredicate<OrderItem>() {
      @Override public boolean eval(OrderItem orderItem) {
  ...
  @Override @Transactional(value = "blTransactionManager") public Order addItemFromNamedOrder(Order namedOrder, OrderItem item, boolean priceOrder) throws RemoveFromCartException, AddToCartException {
    Order cartOrder = orderDao.readCartForCustomer(namedOrder.getCustomer());
    if (cartOrder == null) {
      cartOrder = createNewCartForCustomer(namedOrder.getCustomer());
    }
  ...
  @Override @Transactional(value = "blTransactionManager") public Order addItemFromNamedOrder(Order namedOrder, OrderItem item, int quantity, boolean priceOrder) throws RemoveFromCartException, AddToCartException, UpdateCartException {
  ...
  @Override @Transactional(value = "blTransactionManager") public OrderItem addGiftWrapItemToOrder(Order order, GiftWrapOrderItemRequest itemRequest, boolean priceOrder) throws PricingException {
  ...
  @Override @Transactional(value = "blTransactionManager", rollbackFor = { AddToCartException.class }) public Order addItem(Long orderId, OrderItemRequestDTO orderItemRequestDTO, boolean priceOrder) throws AddToCartException {
    orderItemRequestDTO.setOverrideRetailPrice(null);
    orderItemRequestDTO.setOverrideSalePrice(null);
    return addItemWithPriceOverrides(orderId, orderItemRequestDTO, priceOrder);
  }

  @Override @Transactional(value = "blTransactionManager", rollbackFor = { AddToCartException.class }) public Order addItemWithPriceOverrides(Long orderId, OrderItemRequestDTO orderItemRequestDTO, boolean priceOrder) throws AddToCartException {
  ...
  @Override public int getTotalChildOrderItems(OrderItemRequestDTO orderItemRequestDTO) {
    int count = 1;
  ...
  @Override public void addChildItems(OrderItemRequestDTO orderItemRequestDTO, int numAdditionRequests, int currentAddition, ProcessContext<CartOperationRequest> context, List<ActivityMessageDTO> orderMessages) throws WorkflowException {
  ...
  @Override public void addDependentOrderItem(OrderItemRequestDTO parentOrderItemRequest, OrderItemRequestDTO dependentOrderItem) {
    ...
    Collections.sort(parentOrderItemRequest.getChildOrderItems(), new Comparator<OrderItemRequestDTO>() {
      @Override public int compare(OrderItemRequestDTO o1, OrderItemRequestDTO o2) {
  ...
  @Override @Transactional(value = "blTransactionManager", rollbackFor = { UpdateCartException.class, RemoveFromCartException.class }) public Order updateItemQuantity(Long orderId, OrderItemRequestDTO orderItemRequestDTO, boolean priceOrder) throws UpdateCartException, RemoveFromCartException {
    Order order = findOrderById(orderId);
    preValidateCartOperation(order);
  ...
  @Override @Transactional(value = "blTransactionManager", rollbackFor = { RemoveFromCartException.class }) public Order removeItem(Long orderId, Long orderItemId, boolean priceOrder) throws RemoveFromCartException {
  ...
  @Override @Transactional(value = "blTransactionManager", rollbackFor = { RemoveFromCartException.class }) public Order removeInactiveItems(Long orderId, boolean priceOrder) throws RemoveFromCartException {
    Order order = findOrderById(orderId);
    try {
  ...
  @Override @ManagedAttribute(description = "The move item from named order when adding to the cart attribute", currencyTimeLimit = 15) public boolean isMoveNamedOrderItems() {
    return moveNamedOrderItems;
  }

  @Override @ManagedAttribute(description = "The move item from named order when adding to the cart attribute", currencyTimeLimit = 15) public void setMoveNamedOrderItems(boolean moveNamedOrderItems) {
    this.moveNamedOrderItems = moveNamedOrderItems;
  }

  @Override @ManagedAttribute(description = "The delete empty named order after adding items to cart attribute", currencyTimeLimit = 15) public boolean isDeleteEmptyNamedOrders() {
    return deleteEmptyNamedOrders;
  }

  @Override @Transactional(value = "blTransactionManager") public void removeAllPaymentsFromOrder(Order order) {
    removePaymentsFromOrder(order, null);
  }

  @Override @Transactional(value = "blTransactionManager") public void removePaymentsFromOrder(Order order, PaymentType paymentInfoType) {
  ...
  @Override @Transactional(value = "blTransactionManager") public void removeCreditCardPaymentsFromOrder(Order order) {
    List<OrderPayment> infos = new ArrayList<OrderPayment>();
    for (OrderPayment paymentInfo : order.getPayments()) {
  ...
  @Override @Transactional(value = "blTransactionManager") public void removePaymentFromOrder(Order order, OrderPayment payment) {
  ...
  @Override @Transactional(value = "blTransactionManager", rollbackFor = { UpdateCartException.class }) public Order updateProductOptionsForItem(Long orderId, OrderItemRequestDTO orderItemRequestDTO, boolean priceOrder) throws UpdateCartException {
    try {
      CartOperationRequest cartOpRequest = new CartOperationRequest(findOrderById(orderId), orderItemRequestDTO, priceOrder);
  ...
  @Override public Order reloadOrder(Order order) {
  ...
  @Override @Transactional(value = "blTransactionManager") public boolean acquireLock(Order order) {
    return orderDao.acquireLock(order);
  }

  @Override public boolean releaseLock(Order order) {
    return orderDao.releaseLock(order);
  }

  @Override public void printOrder(Order order, Log log) {
  ...
  @Override public void preValidateCartOperation(Order cart) {
  ...
  @Override public void preValidateUpdateQuantityOperation(Order cart, OrderItemRequestDTO dto) {
    ExtensionResultHolder erh = new ExtensionResultHolder();
  ...
  @Override public void refresh(Order order) {
    orderDao.refresh(order);
  }

  @Override public Order findCartForCustomerWithEnhancements(Customer customer) {
  ...
  @Override public Order findCartForCustomerWithEnhancements(Customer customer, Order candidateOrder) {
    ExtensionResultHolder<Order> erh = new ExtensionResultHolder<Order>();
  ...
  @Override public List<Order> findOrdersByEmail(String email) {
    return orderDao.readOrdersByEmail(email);
  }

  @Override public List<Order> readBatchOrders(int start, int pageSize, List<OrderStatus> orderStatusList) {
    return orderDao.readBatchOrders(start, pageSize, orderStatusList);
  }

  @Override public Long readNumberOfOrders() {
