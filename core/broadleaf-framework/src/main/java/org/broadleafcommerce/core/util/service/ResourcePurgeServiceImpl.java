 ...
 * @author Jeff Fischer
 */
@Service(value = "blResourcePurgeService") public class ResourcePurgeServiceImpl implements ResourcePurgeService {
  private static final Log LOG = LogFactory.getLog(ResourcePurgeServiceImpl.class);

  private static final Long BATCH_SIZE = 50L;

  private static final Long PURGE_ERROR_CACHE_RETRY_SECONDS = System.currentTimeMillis() - 172800;
  ...
  @Resource(name = "blResourcePurgeDao") protected ResourcePurgeDao resourcePurgeDao;
  ...
  @Resource(name = "blDeleteStatementGenerator") protected DeleteStatementGenerator deleteStatementGenerator;

  @Autowired protected Environment env;

  @PersistenceContext(unitName = "blPU") protected EntityManager em;

  @Resource(name = "blResourcePurgeExtensionManager") protected ResourcePurgeExtensionManager extensionManager;

  @Override public void purgeCarts(final Map<String, String> config) {
    ...
    synchronized (cartPurgeErrors) {
      ...
      for (Order cart : carts) {
        TransactionStatus status = TransactionUtils.createTransaction("Cart Purge", TransactionDefinition.PROPAGATION_REQUIRED, transactionManager, false);
        try {
          deleteCart(cart);
  ...
  @Override public void notifyCarts(final Map<String, String> config) {
    if (LOG.isDebugEnabled()) {
      LOG.debug("Notifying carts of purge");
    }
    if (MapUtils.isEmpty(config)) {
      throw new IllegalArgumentException("Cannot notify carts of purge since there was no configuration provided. " + "In the absence of config params, all carts would be candidates for deletion.");
    }
    CartPurgeParams purgeParams = new CartPurgeParams(config).invoke();
    int batchCount = 0;
    Set<Long> failedCartIds = getCartsInErrorToIgnore(purgeParams);
  ...
  @Override public void purgeCustomers(final Map<String, String> config) {
    if (LOG.isDebugEnabled()) {
