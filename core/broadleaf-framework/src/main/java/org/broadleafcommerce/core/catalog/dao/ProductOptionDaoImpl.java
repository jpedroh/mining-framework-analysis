...
@Repository(value = "blProductOptionDao") public class ProductOptionDaoImpl implements ProductOptionDao {
  ...
  @Override public List<AssignedProductOptionDTO> findAssignedProductOptionsByProductId(Long productId) {
    ...
    criteria.select(builder.construct(AssignedProductOptionDTO.class, root.get("sku").get("product").get("id"), root.get("productOptionValue").get("productOption").get("attributeName"), root.get("productOptionValue"), root.get("sku")));
  ...
  @Override public List<AssignedProductOptionDTO> findAssignedProductOptionsByProduct(Product product) {
    return findAssignedProductOptionsByProductId(product.getId());
  }

  @Override public Long countAllowedValuesForProductOptionById(Long productOptionId) {
    CriteriaBuilder builder = em.getCriteriaBuilder();
  ...
  @Override public List<Long> readSkuIdsForProductOptionValues(Long productId, String attributeName, String attributeValue, List<Long> possibleSkuIds) {
  ...
  protected void attachArchivalConditionIfPossible(Class<?> clazz, Path<?> path, CriteriaBuilder cb, List<Predicate> predicates) {
    if (Status.class.isAssignableFrom(clazz)) {
      predicates.add(cb.or(cb.isNull(path.get("archiveStatus").get("archived")), cb.equal(path.get("archiveStatus").get("archived"), 'N')));
    }
  }

  @SuppressWarnings(value = { "unchecked" }) protected Predicate buildSkuDomainPredicate(CriteriaBuilder cb, Path fieldName, List<Long> possibleSkuIds) {
  ...
  @Override public Long countProductsUsingProductOptionById(Long productOptionId) {
    TypedQuery<Long> query = getProductIdsUsingProductOptionByIdQuery(productOptionId, true);
    query.setHint(QueryHints.HINT_CACHEABLE, true);
    return query.getSingleResult();
  }

  @Override public List<Long> findProductIdsUsingProductOptionById(Long productOptionId, int start, int pageSize) {
