 ...
 */
@Service(value = "blOrderToPaymentRequestDTOService") public class OrderToPaymentRequestDTOServiceImpl implements OrderToPaymentRequestDTOService {
  ...
  @Resource(name = "blFulfillmentGroupService") protected FulfillmentGroupService fgService;

  @Resource(name = "blPostLoaderDao") protected PostLoaderDao postLoaderDao;

  @Resource(name = "blPaymentRequestDTOService") protected PaymentRequestDTOService paymentRequestDTOService;

  @Override public PaymentRequestDTO translateOrder(Order order) {
  ...
  @Override public PaymentRequestDTO translatePaymentTransaction(Money transactionAmount, PaymentTransaction paymentTransaction) {
    return translatePaymentTransaction(transactionAmount, paymentTransaction, false);
  }

  @Override public PaymentRequestDTO translatePaymentTransaction(Money transactionAmount, PaymentTransaction paymentTransaction, boolean autoCalculateFinalPaymentTotals) {
  ...
  @Override public void populateTotals(Order order, PaymentRequestDTO requestDTO) {
  ...
  @Override public void populateCustomerInfo(Order order, PaymentRequestDTO requestDTO) {
  ...
  /**
     ...
     */
  @Override public void populateShipTo(Order order, PaymentRequestDTO requestDTO) {
  ...
  @Override public void populateBillTo(Order order, PaymentRequestDTO requestDTO) {
  ...
  protected void populateAddressInfo(final AddressDTO<PaymentRequestDTO> dto, final Address address) {
    ...
    dto.addressFirstName(name.firstName).addressLastName(name.lastName).addressCompanyName(address.getCompanyName()).addressLine1(address.getAddressLine1()).addressLine2(address.getAddressLine2()).addressCityLocality(address.getCity()).addressStateRegion(stateAbbr).addressPostalCode(address.getPostalCode()).addressCountryCode(countryAbbr).addressPhone(phone).addressCounty(address.getCounty()).addressEmail(address.getEmailAddress());
  }
  ...
  /**
     ...
     * @param requestDTO
     */
  @Override public void populateDefaultLineItemsAndSubtotal(Order order, PaymentRequestDTO requestDTO) {
  ...
  @Data public class NameResponse {
