...
public class TypeCheckerDefinitionVisitor extends AbstractTypeCheckVisitor {
  ...
  @Override public PType caseAAssignmentDefinition(AAssignmentDefinition node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAInstanceVariableDefinition(AInstanceVariableDefinition node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAClassInvariantDefinition(AClassInvariantDefinition node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAEqualsDefinition(AEqualsDefinition node, TypeCheckInfo question) throws AnalysisException {
    question.qualifiers = null;
    node.setExpType(node.getTest().apply(THIS, question));
    PPattern pattern = node.getPattern();
    if (pattern != null) {
      question.assistantFactory.createPPatternAssistant(question.fromModule).typeResolve(pattern, THIS, question);
      node.setDefs(question.assistantFactory.createPPatternAssistant(question.fromModule).getDefinitions(pattern, node.getExpType(), question.scope));
      node.setDefType(node.getExpType());
    } else {
      if (node.getTypebind() != null) {
        ...
        node.setDefType(typebind.getType());
        node.setDefs(question.assistantFactory.createPPatternAssistant(question.fromModule).getDefinitions(typebind.getPattern(), node.getDefType(), question.scope));
      } else {
        if (node.getSetbind() != null) {
          question.qualifiers = null;
          PType st = node.getSetbind().getSet().apply(THIS, question);
          if (!question.assistantFactory.createPTypeAssistant().isSet(st, question.fromModule)) {
            TypeCheckerErrors.report(3015, "Set bind is not a set type?", node.getLocation(), node);
            node.setDefType(node.getExpType());
          } else {
            PType setof = question.assistantFactory.createPTypeAssistant().getSet(st, question.fromModule).getSetof();
            ...
            node.setDefType(setof);
          }
          question.assistantFactory.createPPatternAssistant(question.fromModule).typeResolve(node.getSetbind().getPattern(), THIS, question);
          node.setDefs(question.assistantFactory.createPPatternAssistant(question.fromModule).getDefinitions(node.getSetbind().getPattern(), node.getDefType(), question.scope));
        } else {
          question.qualifiers = null;
          PType st = node.getSeqbind().getSeq().apply(THIS, question);
          if (!question.assistantFactory.createPTypeAssistant().isSeq(st, question.fromModule)) {
            TypeCheckerErrors.report(3015, "Seq bind is not a seq type?", node.getLocation(), node);
            node.setDefType(node.getExpType());
          } else {
            PType seqof = question.assistantFactory.createPTypeAssistant().getSeq(st, question.fromModule).getSeqof();
            ...
            node.setDefType(seqof);
          }
          question.assistantFactory.createPPatternAssistant(question.fromModule).typeResolve(node.getSeqbind().getPattern(), THIS, question);
          node.setDefs(question.assistantFactory.createPPatternAssistant(question.fromModule).getDefinitions(node.getSeqbind().getPattern(), node.getDefType(), question.scope));
        }
      }
    }
  ...
  @Override public PType caseAExplicitFunctionDefinition(AExplicitFunctionDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    ...
    if (question.env.isVDMPP() && !question.assistantFactory.createPAccessSpecifierAssistant().isStatic(node.getAccess())) {
    ...
    if (question.env.isVDMPP()) {
      ...
      if (assist.isPrivate(node.getAccess()) && node.getBody() instanceof ASubclassResponsibilityExp) {
    ...
    if (node.getMeasure() == null && node.getRecursive()) {
      ...
      if (node.getMeasure() instanceof AVariableExp) {
        ...
        if (question.env.isVDMPP()) {
          exp.getName().setTypeQualifier(params);
        }
        ...
        if (node.getMeasure() instanceof ANotYetSpecifiedExp) {
    ...
    if (!(node.getBody() instanceof ANotYetSpecifiedExp) && !(node.getBody() instanceof ASubclassResponsibilityExp) && !(node.getName().toString().startsWith("measure_"))) {
      local.unusedCheck();
    }
  ...
  @Override public PType caseAExternalDefinition(AExternalDefinition node, TypeCheckInfo question) {
    return null;
  }

  @Override public PType caseAImplicitFunctionDefinition(AImplicitFunctionDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    question.assistantFactory.getTypeComparator().checkComposeTypes(node.getType(), question.env, false);
    ...
    if (node.getBody() != null) {
      if (node.getClassDefinition() != null && !question.assistantFactory.createPAccessSpecifierAssistant().isStatic(node.getAccess())) {
        local.add(question.assistantFactory.createPDefinitionAssistant().getSelfDefinition(node));
      }
    ...
    if (!(node.getBody() instanceof ANotYetSpecifiedExp) && !(node.getBody() instanceof ASubclassResponsibilityExp)) {
      local.unusedCheck();
    }
    node.setType(node.getType());
    afterAnnotations(node.getAnnotations(), node, question);
  ...
  @Override public PType caseAExplicitOperationDefinition(AExplicitOperationDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    question.assistantFactory.getTypeComparator().checkComposeTypes(node.getType(), question.env, false);
    ...
    if (question.env.isVDMPP()) {
      ...
      if (node.getIsConstructor()) {
        if (question.assistantFactory.createPAccessSpecifierAssistant().isAsync(node.getAccess()) || question.assistantFactory.createPAccessSpecifierAssistant().isStatic(node.getAccess()) || node.getAccess().getPure()) {
          TypeCheckerErrors.report(3286, "Constructor cannot be \'async\', \'static\' or \'pure\'", node.getLocation(), node);
        }
        if (question.assistantFactory.createPTypeAssistant().isClass(((AOperationType) node.getType()).getResult(), question.env, question.fromModule)) {
          AClassType ctype = question.assistantFactory.createPTypeAssistant().getClassType(((AOperationType) node.getType()).getResult(), question.env, question.fromModule);
          if (ctype.getClassdef() != node.getClassDefinition()) {
            TypeCheckerErrors.report(3025, "Constructor operation must have return type " + node.getClassDefinition().getName().getName(), node.getLocation(), node);
          }
          if (!question.assistantFactory.getLexNameTokenAssistant().isEqual(ctype.getClassdef().getName(), node.getClassDefinition().getName())) {
            TypeCheckerErrors.report(3025, "Constructor operation must have return type " + node.getClassDefinition().getName().getName(), node.getLocation(), node);
          }
        } else {
          TypeCheckerErrors.report(3026, "Constructor operation must have return type " + node.getClassDefinition().getName().getName(), node.getLocation(), node);
        }
      }
    }
    List<QualifiedDefinition> qualified = new Vector<QualifiedDefinition>();
    ...
    if (node.getIsConstructor() && !question.assistantFactory.createPTypeAssistant().isType(node.getActualResult(), AVoidType.class) && !compatible || !node.getIsConstructor() && !compatible) {
      ...
      if (!node.getIsConstructor() && !question.assistantFactory.createPTypeAssistant().isUnknown(actualResult)) {
        if (question.assistantFactory.createPTypeAssistant().isVoid(((AOperationType) node.getType()).getResult()) && !question.assistantFactory.createPTypeAssistant().isVoid(actualResult)) {
          ...
          if (!question.assistantFactory.createPTypeAssistant().isVoid(((AOperationType) node.getType()).getResult()) && question.assistantFactory.createPTypeAssistant().hasVoid(actualResult)) {
    ...
    if (question.assistantFactory.createPAccessSpecifierAssistant().isAsync(node.getAccess()) && !question.assistantFactory.createPTypeAssistant().isType(((AOperationType) node.getType()).getResult(), AVoidType.class)) {
      TypeCheckerErrors.report(3293, "Asynchronous operation \'" + node.getName() + "\' cannot return a value", node.getLocation(), node);
    }
    if (node.getAccess().getPure() && question.assistantFactory.createPTypeAssistant().isType(((AOperationType) node.getType()).getResult(), AVoidType.class) && !question.assistantFactory.createPTypeAssistant().isUnknown(((AOperationType) node.getType()).getResult())) {
    ...
    if (question.env.isVDMPP()) {
      PAccessSpecifierAssistantTC assist = question.assistantFactory.createPAccessSpecifierAssistant();
      if (assist.isPrivate(node.getAccess()) && node.getBody() instanceof ASubclassResponsibilityStm) {
        TypeCheckerErrors.report(3329, "Abstract function/operation must be public or protected", node.getLocation(), node);
      }
    }
    if (!(node.getBody() instanceof ANotYetSpecifiedStm) && !(node.getBody() instanceof ASubclassResponsibilityStm)) {
      local.unusedCheck();
    }
    node.setType(node.getType());
    afterAnnotations(node.getAnnotations(), node, question);
  ...
  @Override public PType caseAImplicitOperationDefinition(AImplicitOperationDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    question.assistantFactory.getTypeComparator().checkComposeTypes(node.getType(), question.env, false);
    ...
    defs.addAll(question.assistantFactory.createPDefinitionAssistant().checkDuplicatePatterns(node, argdefs));
    if (node.getResult() != null) {
      defs.addAll(question.assistantFactory.createPPatternAssistant(question.fromModule).getDefinitions(node.getResult().getPattern(), ((AOperationType) node.getType()).getResult(), NameScope.STATE));
    }
      ...
      for (AExternalClause clause : node.getExternals()) {
        ...
        for (ILexNameToken exname : clause.getIdentifiers()) {
          ...
          if (sdef == null) {
            ...
            if (!(clause.getType() instanceof AUnknownType) && !question.assistantFactory.createPTypeAssistant().equals(sdef.getType(), clause.getType())) {
    ...
    if (node.getBody() != null) {
      ...
      if (node.getIsConstructor() && !question.assistantFactory.createPTypeAssistant().isType(node.getActualResult(), AVoidType.class) && !compatible || !node.getIsConstructor() && !compatible) {
        ...
        if (!node.getIsConstructor() && !question.assistantFactory.createPTypeAssistant().isUnknown(node.getActualResult())) {
          if (question.assistantFactory.createPTypeAssistant().isVoid(((AOperationType) node.getType()).getResult()) && !question.assistantFactory.createPTypeAssistant().isVoid(node.getActualResult())) {
            TypeCheckerErrors.report(3312, "Void operation returns non-void value", node.getLocation(), node);
            TypeCheckerErrors.detail2("Actual", node.getActualResult(), "Expected", ((AOperationType) node.getType()).getResult());
          } else {
            if (!question.assistantFactory.createPTypeAssistant().isVoid(((AOperationType) node.getType()).getResult()) && question.assistantFactory.createPTypeAssistant().hasVoid(node.getActualResult())) {
              TypeCheckerErrors.report(3313, "Operation returns void value", node.getLocation(), node);
              TypeCheckerErrors.detail2("Actual", node.getActualResult(), "Expected", ((AOperationType) node.getType()).getResult());
            }
          }
  ...
  @Override public PType caseAImportedDefinition(AImportedDefinition node, TypeCheckInfo question) throws AnalysisException {
    node.setType(node.getDef().apply(THIS, question));
    return node.getType();
  }

  @Override public PType caseAInheritedDefinition(AInheritedDefinition node, TypeCheckInfo question) throws AnalysisException {
    node.setType(node.getSuperdef().apply(THIS, question));
    return node.getType();
  }

  @Override public PType caseALocalDefinition(ALocalDefinition node, TypeCheckInfo question) {
  ...
  @Override public PType caseAMultiBindListDefinition(AMultiBindListDefinition node, TypeCheckInfo question) throws AnalysisException {
    if (node.getType() != null) {
      question.assistantFactory.getTypeComparator().checkComposeTypes(node.getType(), question.env, false);
    }
    List<PDefinition> defs = new Vector<PDefinition>();
  ...
  @Override public PType caseAMutexSyncDefinition(AMutexSyncDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    ...
    if (node.getOperations().isEmpty()) {
      for (PDefinition def : node.getClassDefinition().apply(question.assistantFactory.getDefinitionCollector())) {
        if (question.assistantFactory.createPDefinitionAssistant().isCallableOperation(def) && !def.getName().getName().equals(classdef.getName().getName())) {
      ...
      for (PDefinition def : definitions) {
        if (def.getName() != null && def.getName().matches(opname)) {
          ...
          if (!question.assistantFactory.createPDefinitionAssistant().isCallableOperation(def)) {
            TypeCheckerErrors.report(3038, opname + " is not an explicit operation", opname.getLocation(), opname);
          }
  ...
  @Override public PType caseANamedTraceDefinition(ANamedTraceDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    if (question.env.isVDMPP()) {
  ...
  @Override public PType caseAPerSyncDefinition(APerSyncDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    ...
    for (PDefinition def : definitions) {
      if (def.getName() != null && def.getName().matches(node.getOpname())) {
        opfound++;
        if (!question.assistantFactory.createPDefinitionAssistant().isCallableOperation(def)) {
          TypeCheckerErrors.report(3042, node.getOpname() + " is not an explicit operation", node.getOpname().getLocation(), node.getOpname());
        }
        ...
        if (def.getAccess().getPure()) {
          TypeCheckerErrors.report(3340, "Pure operation cannot have permission predicate", node.getOpname().getLocation(), node.getOpname());
        }
  ...
  @Override public PType caseARenamedDefinition(ARenamedDefinition node, TypeCheckInfo question) throws AnalysisException {
    node.setType(node.getDef().apply(THIS, question));
    return node.getType();
  }

  @Override public PType caseAStateDefinition(AStateDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    if (node.getPass() == Pass.TYPES) {
      ...
      if (node.getInvdef() != null) {
        ...
        question.assistantFactory.createPPatternAssistant(question.fromModule).typeResolve(node.getInvPattern(), THIS, question);
      }
      if (node.getInitdef() != null) {
        node.getInitdef().apply(THIS, question);
        question.assistantFactory.createPPatternAssistant(question.fromModule).typeResolve(node.getInitPattern(), THIS, question);
      }
    }
    afterAnnotations(node.getAnnotations(), node, question);
    return null;
  ...
  @Override public PType caseAThreadDefinition(AThreadDefinition node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseATypeDefinition(ATypeDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
  ...
  @Override public PType caseAUntypedDefinition(AUntypedDefinition node, TypeCheckInfo question) {
    assert false : "Can\'t type check untyped definition?";
    return null;
  }

  @Override public PType caseAValueDefinition(AValueDefinition node, TypeCheckInfo question) throws AnalysisException {
    checkAnnotations(node, question);
    beforeAnnotations(node.getAnnotations(), node, question);
    if (node.getType() != null) {
      question.assistantFactory.getTypeComparator().checkComposeTypes(node.getType(), question.env, false);
    ...
    if (expType instanceof AUnknownType) {
      node.setPass(Pass.FINAL);
    }
    if (expType instanceof AVoidType) {
      TypeCheckerErrors.report(3048, "Expression does not return a value", node.getExpression().getLocation(), node.getExpression());
    } else {
      if (type != null && !(type instanceof AUnknownType)) {
    ...
    if (base.isVDMPP() && type instanceof ANamedInvariantType) {
      ...
      if (typedef == null) {
        TypeCheckerErrors.report(2048, "Cannot find symbol " + named.getName().toString(), named.getLocation(), named);
        return node.getType();
      }
  ...
  @Override public PType caseALetDefBindingTraceDefinition(ALetDefBindingTraceDefinition node, TypeCheckInfo question) throws AnalysisException {
    return typeCheckLet(node, node.getLocalDefs(), node.getBody(), question);
  }

  @Override public PType caseALetBeStBindingTraceDefinition(ALetBeStBindingTraceDefinition node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseARepeatTraceDefinition(ARepeatTraceDefinition node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAConcurrentExpressionTraceCoreDefinition(AConcurrentExpressionTraceCoreDefinition node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseABracketedExpressionTraceCoreDefinition(ABracketedExpressionTraceCoreDefinition node, TypeCheckInfo question) throws AnalysisException {
    for (ATraceDefinitionTerm term : node.getTerms()) {
  ...
  @Override public PType caseAApplyExpressionTraceCoreDefinition(AApplyExpressionTraceCoreDefinition node, TypeCheckInfo question) throws AnalysisException {
    return node.getCallStatement().apply(THIS, question);
  }

  @Override public PType caseAEqRelation(AEqRelation node, TypeCheckInfo question) throws AnalysisException {
    return node.getRelDef().apply(THIS, question);
  }

  @Override public PType caseAOrdRelation(AOrdRelation node, TypeCheckInfo question) throws AnalysisException {
  ...
  public void typeCheck(List<PTraceDefinition> term, IQuestionAnswer<TypeCheckInfo, PType> rootVisitor, TypeCheckInfo question) throws AnalysisException {
    for (PTraceDefinition def : term) {
      def.apply(rootVisitor, question);
    }
  }

  public Collection<? extends PDefinition> getDefinitions(APatternListTypePair pltp, NameScope scope, ITypeCheckerAssistantFactory assistantFactory) {
    List<PDefinition> list = new Vector<PDefinition>();
    for (PPattern p : pltp.getPatterns()) {
      list.addAll(assistantFactory.createPPatternAssistant(null).getDefinitions(p, pltp.getType(), scope));
    }
    return list;
  }

  public void typeResolve(AExternalClause clause, IQuestionAnswer<TypeCheckInfo, PType> rootVisitor, TypeCheckInfo question) {
  ...
  private void setMeasureExp(TypeCheckInfo question, SFunctionDefinitionBase node, Environment base, Environment local, NameScope scope) throws AnalysisException {
    ...
    AFunctionType mtype = question.assistantFactory.createAFunctionTypeAssistant().getMeasureType((AFunctionType) node.getType(), isCurried, actual, question.fromModule);
    AExplicitFunctionDefinition def = AstFactory.newAExplicitFunctionDefinition(node.getMeasureName(), scope, (List<ILexNameToken>) node.getTypeParams().clone(), mtype, cpll, node.getMeasure(), null, null, false, null);
  ...
  private void setMeasureDef(TypeCheckInfo question, SFunctionDefinitionBase node, ILexNameToken mname, Environment base, NameScope scope) {
    ...
    if (mdef == null) {
      ...
      if (!(mdef instanceof AExplicitFunctionDefinition)) {
        ...
        if (mdef == node) {
          ...
          if (node.getTypeParams() == null && efd.getTypeParams() != null) {
            ...
            if (node.getTypeParams() != null && efd.getTypeParams() == null) {
              ...
              if (node.getTypeParams() != null && efd.getTypeParams() != null && !node.getTypeParams().equals(efd.getTypeParams())) {
          ...
          if (node.getTypeParams() != null && !node.getTypeParams().isEmpty() && exParams != null) {
            ...
            if (exParams != null && !question.assistantFactory.getTypeComparator().compatible(mtype.getParameters(), exParams)) {
              TypeCheckerErrors.report(3303, "Measure parameters different to function", node.getMeasure().getLocation(), node.getMeasure());
              TypeCheckerErrors.detail2(mname.toString(), mtype.getParameters(), node.getName().getName(), exParams);
            } else {
              if (exParams == null && !question.assistantFactory.getTypeComparator().compatible(mtype.getParameters(), ((AFunctionType) node.getType()).getParameters())) {
                TypeCheckerErrors.report(3303, "Measure parameters different to function", node.getMeasure().getLocation(), node.getMeasure());
  ...
  private void checkMeasure(TypeCheckInfo question, SFunctionDefinitionBase node, ILexNameToken iLexNameToken, PType result) {
    ...
    if (!assistant.isNumeric(result, question.fromModule)) {
      if (assistant.isProduct(result, question.fromModule)) {
        AProductType pt = assistant.getProduct(result, question.fromModule);
        for (PType t : pt.getTypes()) {
          if (!assistant.isNumeric(t, question.fromModule)) {
  ...
  private void checkAnnotations(PDefinition node, TypeCheckInfo question) throws AnalysisException {
    for (PAnnotation annotation : node.getAnnotations()) {
      annotation.apply(THIS, question);
    }
  }
}