...
public class ImplicitDefinitionFinder extends QuestionAdaptor<Environment> {
  ...
  @Override public void defaultSClassDefinition(SClassDefinition node, Environment question) throws AnalysisException {
    if (node instanceof ASystemClassDefinition) {
      ...
      for (PDefinition d : node.getDefinitions()) {
        if (d instanceof AInstanceVariableDefinition) {
          ...
          if (ivType instanceof AUnresolvedType && iv.getExpression() instanceof AUndefinedExp) {
            ...
            if (ivType instanceof AUnresolvedType && iv.getExpression() instanceof ANewExp) {
              AUnresolvedType ut = (AUnresolvedType) ivType;
              if (ut.getName().getFullName().equals("CPU")) {
                ...
                if (speed == 0) {
                  ...
                  if (speed > AstFactoryTC.CPU_MAX_FREQUENCY) {
          ...
          if (d instanceof AExplicitOperationDefinition) {
            ...
            if (!edef.getName().getName().equals(node.getName().getName()) || !edef.getParameterPatterns().isEmpty()) {
  ...
  @Override public void caseAClassInvariantDefinition(AClassInvariantDefinition node, Environment question) throws AnalysisException {
  }

  @Override public void caseAEqualsDefinition(AEqualsDefinition node, Environment question) throws AnalysisException {
  }

  @Override public void caseAExplicitFunctionDefinition(AExplicitFunctionDefinition node, Environment question) throws AnalysisException {
  ...
  @Override public void caseAExplicitOperationDefinition(AExplicitOperationDefinition node, Environment question) throws AnalysisException {
  ...
  @Override public void caseAImplicitFunctionDefinition(AImplicitFunctionDefinition node, Environment question) throws AnalysisException {
    if (node.getPrecondition() != null) {
  ...
  @Override public void caseAImplicitOperationDefinition(AImplicitOperationDefinition node, Environment question) throws AnalysisException {
    node.setState(findStateDefinition(question, node));
    if (node.getPrecondition() != null) {
  ...
  @Override public void caseAStateDefinition(AStateDefinition node, Environment question) throws AnalysisException {
  ...
  @Override public void caseAThreadDefinition(AThreadDefinition node, Environment question) throws AnalysisException {
    node.setOperationDef(getThreadDefinition(node));
  }

  @Override public void caseATypeDefinition(ATypeDefinition node, Environment question) throws AnalysisException {
    if (node.getInvPattern() != null) {
      node.setInvdef(getInvDefinition(node));
  ...
  @Override public void defaultPDefinition(PDefinition node, Environment question) throws AnalysisException {
    return;
  }
  ...
  private AExplicitFunctionDefinition getRelDef(PRelation node, ATypeDefinition typedef, ILexNameToken fname) {
    ...
    AFunctionType ftype = AstFactory.newAFunctionType(loc, false, ptypes, AstFactory.newABooleanBasicType(loc));
    AExplicitFunctionDefinition def = AstFactory.newAExplicitFunctionDefinition(fname, NameScope.GLOBAL, null, ftype, parameters, node.getRelExp(), null, null, true, null);
  ...
  private void setMinMax(AOrdRelation ordRelation, ATypeDefinition typeDef) {
    ...
    List<List<PPattern>> parameters = new Vector<List<PPattern>>();
    parameters.add(params);
    AIfExp maxBody = AstFactoryTC.newAIfExp(loc.clone(), AstFactoryTC.newALessEqualNumericBinaryExp(left.clone(), new LexToken(loc.clone(), VDMToken.LE), right.clone()), left.clone(), new LinkedList<>(), right.clone());
    PTypeList ptypes = getPTypes(typeDef);
    AFunctionType ftype = AstFactory.newAFunctionType(loc.clone(), false, ptypes, typeDef.getInvType().clone());
    AExplicitFunctionDefinition maxD = AstFactory.newAExplicitFunctionDefinition(typeDef.getName().getMaxName(loc), NameScope.GLOBAL, null, ftype, parameters, maxBody, null, null, true, null);
    ...
    AExplicitFunctionDefinition minD = AstFactory.newAExplicitFunctionDefinition(typeDef.getName().getMinName(loc), NameScope.GLOBAL, null, ftype, parameters2, minBody, null, null, true, null);
