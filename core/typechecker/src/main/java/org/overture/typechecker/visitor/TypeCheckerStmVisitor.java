...
public class TypeCheckerStmVisitor extends AbstractTypeCheckVisitor {
  public TypeCheckerStmVisitor(IQuestionAnswer<TypeCheckInfo, PType> typeCheckVisitor) {
    super(typeCheckVisitor);
  }

  @Override public PType caseAAlwaysStm(AAlwaysStm node, TypeCheckInfo question) throws AnalysisException {
    node.getAlways().apply(THIS, question);
    node.setType(node.getBody().apply(THIS, question));
    return node.getType();
  }

  @Override public PType caseAAnnotatedStm(AAnnotatedStm node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAAssignmentStm(AAssignmentStm node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAAtomicStm(AAtomicStm node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAForPatternBindStm(AForPatternBindStm node, TypeCheckInfo question) throws AnalysisException {
    ...
    if (question.assistantFactory.createPTypeAssistant().isSeq(stype, question.fromModule)) {
      node.setSeqType(question.assistantFactory.createPTypeAssistant().getSeq(stype, question.fromModule));
  ...
  @Override public PType defaultSSimpleBlockStm(SSimpleBlockStm node, TypeCheckInfo question) throws AnalysisException {
    ...
    for (PStm stmt : node.getStatements()) {
      ...
      if (notreached) {
        ...
        if (stype instanceof AUnionType) {
          ...
          if (stype instanceof AVoidType || stype instanceof AUnknownType) {
            notreached = false;
          }
        }
      }
    }
    if (last != null && (question.assistantFactory.createPTypeAssistant().isType(last, AVoidType.class) || question.assistantFactory.createPTypeAssistant().isUnknown(last))) {
  ...
  @Override public PType caseABlockSimpleBlockStm(ABlockSimpleBlockStm node, TypeCheckInfo question) throws AnalysisException {
    Environment local = question.env;
  ...
  @Override public PType caseACallObjectStm(ACallObjectStm node, TypeCheckInfo question) throws AnalysisException {
    PType dtype = node.getDesignator().apply(THIS, question);
    if (!question.assistantFactory.createPTypeAssistant().isClass(dtype, question.env, question.fromModule)) {
      TypeCheckerErrors.report(3207, "Object designator is not an object type", node.getLocation(), node);
      node.setType(AstFactory.newAUnknownType(node.getLocation()));
      return node.getType();
    }
    AClassType ctype = question.assistantFactory.createPTypeAssistant().getClassType(dtype, question.env, question.fromModule);
    ...
    if (self == classdef || question.assistantFactory.createPDefinitionAssistant().hasSupertype(self, question.assistantFactory.createPDefinitionAssistant().getType(classdef))) {
    ...
    if (Settings.dialect == Dialect.VDM_RT && node.getField().getModule().equals("CPU") && node.getField().getName().equals("deploy")) {
      ...
      if (Settings.dialect == Dialect.VDM_RT && node.getField().getModule().equals("CPU") && node.getField().getName().equals("setPriority")) {
        if (!(atypes.get(0) instanceof AOperationType)) {
          ...
          if (a1.getName().getModule().equals(a1.getName().getName())) {
        ...
        if (fdef == null) {
          ...
          if (question.assistantFactory.createPDefinitionAssistant().isStatic(fdef) && !question.env.isStatic()) {
          }
        }
      }
    }
    ...
    if (question.assistantFactory.createPTypeAssistant().isOperation(type, question.fromModule)) {
      AOperationType optype = question.assistantFactory.createPTypeAssistant().getOperation(type, question.fromModule);
      ...
      checkArgTypes(type, optype.getParameters(), atypes, question);
      ...
      if (question.assistantFactory.createPTypeAssistant().isFunction(type, question.fromModule)) {
        ...
        checkArgTypes(type, ftype.getParameters(), atypes, question);
  ...
  @Override public PType caseACallStm(ACallStm node, TypeCheckInfo question) throws AnalysisException {
    List<PType> atypes = getArgTypes(node.getArgs(), THIS, question);
  ...
  @Override public PType caseACaseAlternativeStm(ACaseAlternativeStm node, TypeCheckInfo question) throws AnalysisException {
    if (node.getDefs().size() == 0) {
      ...
      node.getDefs().addAll(question.assistantFactory.createPPatternAssistant(question.fromModule).getDefinitions(node.getPattern(), stm.getExp().getType(), NameScope.LOCAL));
    }
    question.assistantFactory.createPDefinitionListAssistant().typeCheck(node.getDefs(), THIS, question);
    if (!question.assistantFactory.createPPatternAssistant(question.fromModule).matches(node.getPattern(), node.getCtype())) {
      TypeCheckerErrors.report(3311, "Pattern cannot match", node.getPattern().getLocation(), node.getPattern());
    }
  ...
  @Override public PType caseACasesStm(ACasesStm node, TypeCheckInfo question) throws AnalysisException {
    ...
    for (ACaseAlternativeStm c : node.getCases()) {
      ...
      always = always || question.assistantFactory.createPPatternAssistant(question.fromModule).alwaysMatches(c.getPattern(), expType);
    }
  ...
  @Override public PType caseAClassInvariantStm(AClassInvariantStm node, TypeCheckInfo question) {
    node.setType(AstFactory.newABooleanBasicType(node.getLocation()));
    return node.getType();
  }

  @Override public PType caseACyclesStm(ACyclesStm node, TypeCheckInfo question) throws AnalysisException {
    PDefinition encl = question.env.getEnclosingDefinition();
  ...
  @Override public PType caseALetStm(ALetStm node, TypeCheckInfo question) throws AnalysisException {
    node.setType(typeCheckLet(node, node.getLocalDefs(), node.getStatement(), question));
    return node.getType();
  }

  @Override public PType caseADurationStm(ADurationStm node, TypeCheckInfo question) throws AnalysisException {
    PDefinition encl = question.env.getEnclosingDefinition();
    if (encl != null && encl.getAccess().getPure()) {
  ...
  @Override public PType caseAElseIfStm(AElseIfStm node, TypeCheckInfo question) throws AnalysisException {
    node.setType(typeCheckAElseIf(node, node.getLocation(), node.getElseIf(), node.getThenStm(), question));
    return node.getType();
  }

  @Override public PType caseAErrorStm(AErrorStm node, TypeCheckInfo question) {
    node.setType(AstFactory.newAUnknownType(node.getLocation()));
    return node.getType();
  }

  @Override public PType caseAExitStm(AExitStm node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseAForAllStm(AForAllStm node, TypeCheckInfo question) throws AnalysisException {
    node.setType(node.getSet().apply(THIS, question));
    question.assistantFactory.createPPatternAssistant(question.fromModule).typeResolve(node.getPattern(), THIS, question);
    if (question.assistantFactory.createPTypeAssistant().isSet(node.getType(), question.fromModule)) {
      SSetType st = question.assistantFactory.createPTypeAssistant().getSet(node.getType(), question.fromModule);
      List<PDefinition> defs = question.assistantFactory.createPPatternAssistant(question.fromModule).getDefinitions(node.getPattern(), st.getSetof(), NameScope.LOCAL);
  ...
  @Override public PType caseAForIndexStm(AForIndexStm node, TypeCheckInfo question) throws AnalysisException {
    PType ft = node.getFrom().apply(THIS, question);
    PType tt = node.getTo().apply(THIS, question);
    if (!question.assistantFactory.createPTypeAssistant().isNumeric(ft, question.fromModule)) {
      TypeCheckerErrors.report(3220, "From type is not numeric", node.getLocation(), node);
    }
    if (!question.assistantFactory.createPTypeAssistant().isNumeric(tt, question.fromModule)) {
      TypeCheckerErrors.report(3221, "To type is not numeric", node.getLocation(), node);
    }
    if (node.getBy() != null) {
      PType bt = node.getBy().apply(THIS, question);
      if (!question.assistantFactory.createPTypeAssistant().isNumeric(bt, question.fromModule)) {
  ...
  @Override public PType caseAIfStm(AIfStm node, TypeCheckInfo question) throws AnalysisException {
    node.setType(typeCheckIf(node.getLocation(), node.getIfExp(), node.getThenStm(), node.getElseIf(), node.getElseStm(), question));
    return node.getType();
  }

  @Override public PType caseALetBeStStm(ALetBeStStm node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseANonDeterministicSimpleBlockStm(ANonDeterministicSimpleBlockStm node, TypeCheckInfo question) throws AnalysisException {
    PDefinition encl = question.env.getEnclosingDefinition();
    if (encl != null && encl.getAccess().getPure()) {
      TypeCheckerErrors.report(3346, "Cannot use non-deterministic statement in pure operations", node.getLocation(), node);
    }
    PTypeSet rtypes = new PTypeSet(question.assistantFactory);
    int rcount = 0;
    for (PStm stmt : node.getStatements()) {
  ...
  @Override public PType caseANotYetSpecifiedStm(ANotYetSpecifiedStm node, TypeCheckInfo question) {
    node.setType(typeCheckANotYetSpecifiedExp(node, node.getLocation()));
    return node.getType();
  }

  @Override public PType caseAReturnStm(AReturnStm node, TypeCheckInfo question) throws AnalysisException {
    PDefinition encl = question.env.getEnclosingDefinition();
    ...
    if (encl instanceof AExplicitOperationDefinition) {
      ...
      if (encl instanceof AImplicitOperationDefinition) {
  ...
  @Override public PType caseASkipStm(ASkipStm node, TypeCheckInfo question) {
    node.setType(AstFactory.newAVoidType(node.getLocation()));
    return node.getType();
  }

  @Override public PType caseASpecificationStm(ASpecificationStm node, TypeCheckInfo question) throws AnalysisException {
    ...
    Environment local = new FlatEnvironment(question.assistantFactory, defs, question.env);
    if (node.getPrecondition() != null && !question.assistantFactory.createPTypeAssistant().isType(node.getPrecondition().apply(THIS, new TypeCheckInfo(question.assistantFactory, local, NameScope.NAMESANDSTATE)), ABooleanBasicType.class)) {
    ...
    if (node.getPostcondition() != null && !question.assistantFactory.createPTypeAssistant().isType(node.getPostcondition().apply(THIS, new TypeCheckInfo(question.assistantFactory, local, NameScope.NAMESANDANYSTATE)), ABooleanBasicType.class)) {
  ...
  @Override public PType caseATrapStm(ATrapStm node, TypeCheckInfo question) throws AnalysisException {
    PTypeSet rtypes = new PTypeSet(question.assistantFactory);
  ...
  @Override public PType caseAWhileStm(AWhileStm node, TypeCheckInfo question) throws AnalysisException {
    ...
    if (node.getExp() instanceof ABooleanConstExp && stype instanceof AUnionType) {
      ...
      if (boolLiteral.getValue().getValue()) {
  ...
  @Override public PType caseAPeriodicStm(APeriodicStm node, TypeCheckInfo question) throws AnalysisException {
    ...
    if (args.size() != 4) {
      ...
      for (PExp arg : args) {
        ...
        if (!question.assistantFactory.createPTypeAssistant().isNumeric(type, question.fromModule)) {
    ...
    if (opdef instanceof AExplicitOperationDefinition) {
      ...
      if (opdef instanceof AImplicitOperationDefinition) {
  ...
  @Override public PType caseASporadicStm(ASporadicStm node, TypeCheckInfo question) throws AnalysisException {
    List<PExp> args = node.getArgs();
    if (args.size() != 3) {
  ...
  @Override public PType caseAStartStm(AStartStm node, TypeCheckInfo question) throws AnalysisException {
    PDefinition encl = question.env.getEnclosingDefinition();
    if (encl != null && encl.getAccess().getPure()) {
    ...
    if (question.assistantFactory.createPTypeAssistant().isSet(type, question.fromModule)) {
      ...
      if (!question.assistantFactory.createPTypeAssistant().isClass(set.getSetof(), null, question.fromModule)) {
        TypeCheckerErrors.report(3235, "Expression is not a set of object references", node.getObj().getLocation(), node.getObj());
      } else {
        AClassType ctype = question.assistantFactory.createPTypeAssistant().getClassType(set.getSetof(), null, question.fromModule);
      ...
      if (question.assistantFactory.createPTypeAssistant().isClass(type, null, question.fromModule)) {
        AClassType ctype = question.assistantFactory.createPTypeAssistant().getClassType(type, null, question.fromModule);
        if (question.assistantFactory.createSClassDefinitionAssistant().findThread(ctype.getClassdef()) == null) {
  ...
  @Override public PType caseAStopStm(AStopStm node, TypeCheckInfo question) throws AnalysisException {
    PType type = node.getObj().apply(THIS, question);
    if (question.assistantFactory.createPTypeAssistant().isSet(type, question.fromModule)) {
      SSetType set = question.assistantFactory.createPTypeAssistant().getSet(type, question.fromModule);
      if (!question.assistantFactory.createPTypeAssistant().isClass(set.getSetof(), null, question.fromModule)) {
  ...
  @Override public PType caseASubclassResponsibilityStm(ASubclassResponsibilityStm node, TypeCheckInfo question) {
    node.setType(AstFactory.newAUnknownType(node.getLocation()));
    return node.getType();
  }

  @Override public PType caseATixeStm(ATixeStm node, TypeCheckInfo question) throws AnalysisException {
  ...
  @Override public PType caseATixeStmtAlternative(ATixeStmtAlternative node, TypeCheckInfo question) throws AnalysisException {
    node.getPatternBind().apply(THIS, new TypeCheckInfo(question.assistantFactory, question.env, question.scope));
    List<PDefinition> defs = getDefinitions(node.getPatternBind());
    question.assistantFactory.createPDefinitionListAssistant().typeCheck(defs, THIS, question);
    Environment local = new FlatCheckedEnvironment(question.assistantFactory, defs, question.env, question.scope);
  ...
  @Override public PType caseADefPatternBind(ADefPatternBind node, TypeCheckInfo question) throws AnalysisException {
    node.setDefs(null);
    ...
    if (bind != null) {
      if (bind instanceof ATypeBind) {
        ...
        SSetType settype = question.assistantFactory.createPTypeAssistant().getSet(setbind.getSet().apply(THIS, question), question.fromModule);
  ...
  public List<PType> getArgTypes(List<PExp> args, IQuestionAnswer<TypeCheckInfo, PType> rootVisitor, TypeCheckInfo question) throws AnalysisException {
  ...
  public PDefinition targetDefinition(PStateDesignator pStateDesignator, TypeCheckInfo question) {
  ...
  public void checkArgTypes(ACallStm node, PType type, List<PType> ptypes, List<PType> atypes, TypeCheckInfo question) {
    if (ptypes.size() != atypes.size()) {
