...
public class TypeCheckerExpVisitor extends AbstractTypeCheckVisitor {
  ...
  @Override public PType caseADomainResByBinaryExp(ADomainResByBinaryExp node, TypeCheckInfo question) throws AnalysisException {
    ...
    if (question.constraint != null && question.assistantFactory.createPTypeAssistant().isMap(question.constraint)) {
  ...
  @Override public PType caseAPlusPlusBinaryExp(APlusPlusBinaryExp node, TypeCheckInfo question) throws AnalysisException {
    ...
    boolean unique = !question.assistantFactory.createPTypeAssistant().isUnion(node.getLeft().getType()) && !question.assistantFactory.createPTypeAssistant().isUnion(node.getRight().getType());
    if (question.assistantFactory.createPTypeAssistant().isMap(node.getLeft().getType())) {
      if (!question.assistantFactory.createPTypeAssistant().isMap(node.getRight().getType())) {
  ...
  @Override public PType caseAProperSubsetBinaryExp(AProperSubsetBinaryExp node, TypeCheckInfo question) throws AnalysisException {
    ...
    if (question.assistantFactory.createPTypeAssistant().isSet(ltype) && question.assistantFactory.createPTypeAssistant().isSet(rtype) && !question.assistantFactory.getTypeComparator().compatible(ltype, rtype)) {
  ...
  @Override public PType caseAMkTypeExp(AMkTypeExp node, TypeCheckInfo question) throws AnalysisException {
    ...
    if (node.getRecordType().getOpaque() && !node.getLocation().getModule().equals(node.getRecordType().getLocation().getModule())) {
  ...
  @Override public PType caseASeqCompSeqExp(ASeqCompSeqExp node, TypeCheckInfo question) throws AnalysisException {
    ...
    def.apply(THIS, question.newConstraint(null));
    if (node.getSetBind() != null && (question.assistantFactory.createPPatternAssistant().getVariableNames(node.getSetBind().getPattern()).size() > 1 || !question.assistantFactory.createPTypeAssistant().isOrdered(question.assistantFactory.createPDefinitionAssistant().getType(def), node.getLocation()))) {
  ...
  @Override public PType caseASetEnumSetExp(ASetEnumSetExp node, TypeCheckInfo question) throws AnalysisException {
    ...
    List<PType> types = node.getTypes();
    TypeCheckInfo elemConstraint = question;
    if (question.constraint != null && question.assistantFactory.createPTypeAssistant().isSet(question.constraint)) {
  ...
  @Override public PType caseATupleExp(ATupleExp node, TypeCheckInfo question) throws AnalysisException {
    ...
    if (question.constraint != null && question.assistantFactory.createPTypeAssistant().isProduct(question.constraint)) {
  ...
  public PType sequenceApply(AApplyExp node, boolean isSimple, SSeqType seq, TypeCheckInfo question) {
    if (node.getArgs().size() != 1) {
      ...
      if (!question.assistantFactory.createPTypeAssistant().isNumeric(node.getArgtypes().get(0))) {
