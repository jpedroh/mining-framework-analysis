...
public class ModuleTypeChecker extends TypeChecker {
  ...
  /**
	 ...
	 */
  @Override public void typeCheck() {
    ...
    for (PDefinition d : checkDefs) {
      try {
        assistantFactory.createPDefinitionAssistant().typeResolve(d, tc, new TypeCheckInfo(assistantFactory, env).newModule(d.getLocation().getModule()));
        ...
        report(3431, te.getMessage(), null);
      }
    }
    Annotation.init();
    for (AModuleModules m : modules) {
