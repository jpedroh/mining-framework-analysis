...
public class POSTaggerFactoryTest {
  private static ObjectStream<POSSample> createSampleStream() throws IOException {
    ...
    try {
      sampleDataIn = new MarkableFileInputStreamFactory(new File(POSTaggerFactory.class.getResource("/AnnotatedSentences.txt").toURI().getPath()));
  ...
  private static POSModel trainPOSModel(ModelType type, POSTaggerFactory factory) throws IOException {
    return POSTaggerME.train("en", createSampleStream(), TrainingParameters.defaultParams(), factory);
  }

  @Test public void testPOSTaggerWithCustomFactory() throws Exception {
