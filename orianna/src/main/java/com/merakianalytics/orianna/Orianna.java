...
public abstract class Orianna {
  public static class Configuration {
    ...
    private static PipelineConfiguration getDefaultPipeline() {
      ...
      final Set<TransformerConfiguration> transformers = ImmutableSet.of(TransformerConfiguration.defaultConfiguration(ChampionMasteryTransformer.class), TransformerConfiguration.defaultConfiguration(ChampionTransformer.class), TransformerConfiguration.defaultConfiguration(LeagueTransformer.class), TransformerConfiguration.defaultConfiguration(MatchTransformer.class), TransformerConfiguration.defaultConfiguration(SpectatorTransformer.class), TransformerConfiguration.defaultConfiguration(StaticDataTransformer.class), TransformerConfiguration.defaultConfiguration(StatusTransformer.class), TransformerConfiguration.defaultConfiguration(SummonerTransformer.class), TransformerConfiguration.defaultConfiguration(ThirdPartyCodeTransformer.class));
      config.setTransformers(transformers);
      final List<PipelineElementConfiguration> elements = ImmutableList.of(PipelineElementConfiguration.defaultConfiguration(InMemoryCache.class), PipelineElementConfiguration.defaultConfiguration(GhostObjectSource.class), PipelineElementConfiguration.defaultConfiguration(ChampionGG.class), PipelineElementConfiguration.defaultConfiguration(DataDragon.class), PipelineElementConfiguration.defaultConfiguration(RiotAPI.class), PipelineElementConfiguration.defaultConfiguration(ImageDataSource.class));
  ...
  public static class Settings {
    ...
    private Supplier<DataPipeline> newPipelineSupplier() {
      return Suppliers.memoize(new Supplier<DataPipeline>() {
        @Override public DataPipeline get() {
    ...
    private Supplier<String> newVersionSupplier(final Platform platform) {
      ...
      if (configuration.getCurrentVersionExpiration().getPeriod() < 0) {
        ...
        if (configuration.getCurrentVersionExpiration().getPeriod() > 0) {
          supplier = Suppliers.memoizeWithExpiration(supplier, configuration.getCurrentVersionExpiration().getPeriod(), configuration.getCurrentVersionExpiration().getUnit());
        }
      }
      return supplier;
    }
  ...
  private static Settings defaultSettings() {
    ...
    if (configPath != null && !configPath.isEmpty()) {
      try {
        ...
        LOGGER.error("Failed to load environment-configured configuration from " + configPath + "! Using default configuration from resources instead.", e);
      }
    }
    try {
      return new Settings(getConfiguration(Resources.asCharSource(Resources.getResource("com/merakianalytics/orianna/default-orianna-config.json"), Charset.forName("UTF-8"))));
    } catch (final OriannaException e) {
