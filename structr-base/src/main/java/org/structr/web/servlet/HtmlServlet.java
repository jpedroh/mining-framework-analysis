...
public class HtmlServlet extends AbstractServletBase implements HttpServiceServlet {
  ...
  @Override protected void doHead(final HttpServletRequest request, final HttpServletResponse response) {
    final Authenticator auth = getConfig().getAuthenticator();
  ...
  @Override protected void doOptions(final HttpServletRequest request, final HttpServletResponse response) {
  ...
  private Page findPage(final SecurityContext securityContext, final String path, final EditMode edit) throws FrameworkException {
    ...
    final List<Page> possiblePages = StructrApp.getInstance(securityContext).nodeQuery(Page.class).or().notBlank(pathKey).and(pathKey, path).parent().or().blank(pathKey).and(nameKey, name).parent().sort(pathKey).getAsList();
  ...
  private List<Linkable> findPossibleEntryPointsByUuid(final SecurityContext securityContext, final HttpServletRequest request, final String uuid) throws FrameworkException {
    ...
    if (uuid.length() > 0) {
      logger.debug("Requested id: {}", uuid);
      final Query query = StructrApp.getInstance(securityContext).nodeQuery();
      query.and().or().andTypes(Page.class).andTypes(File.class).parent().and(GraphObject.id, uuid);
