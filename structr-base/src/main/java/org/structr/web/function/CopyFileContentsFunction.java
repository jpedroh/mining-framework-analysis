...
public class CopyFileContentsFunction extends UiAdvancedFunction {
  ...
  @Override public Object apply(final ActionContext ctx, final Object caller, Object[] sources) throws FrameworkException {
    try {
      ...
      if (toCopy instanceof File && toBeReplaced instanceof File) {
        ...
        try (InputStream is = StorageProviderFactory.getStorageProvider(nodeToCopy).getInputStream(); OutputStream os = StorageProviderFactory.getStorageProvider(nodeToBeReplaced).getOutputStream()) {
          IOUtils.copy(is, os);
  ...
  @Override public String usage(boolean inJavaScriptContext) {
