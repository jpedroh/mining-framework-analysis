...
public class Importer {
  ...
  private DOMNode createChildNodes(final Node startNode, final DOMNode parent, final Page page, final boolean removeHashAttribute, final int depth, final DOMNode suppliedRoot) throws FrameworkException {
    ...
    for (Node node : children) {
      ...
      if (StringUtils.isBlank(tag)) {
        ...
        if ("structr:template".equals(tag)) {
          ...
          if (src != null) {
            ...
            if (DeployCommand.isUuid(src)) {
              ...
              if (DeployCommand.endsWithUuid(src)) {
          ...
          if ("structr:shared-template".equals(tag)) {
            ...
            if ("structr:component".equals(tag)) {
              final String src = node.attr("src");
              if (src != null) {
      ...
      if (newNode != null) {
          ...
          if (!key.equals("text")) {
            ...
            if (key.startsWith("data-")) {
              ...
              if (linkable != null && "link".equals(tag) && "href".equals(key) && isLocal && !isActive && !isDeployment) {
                ...
                if (linkable != null && ("href".equals(key) || "src".equals(key)) && isLocal && !isActive && !isAnchor && !isStructrLib && !isDeployment) {
        ...
        if ("script".equals(tag)) {
          ...
          if (contentType == null) {
            ...
            if (contentType.equals("application/schema+json")) {
  ...
  private Linkable downloadFile(final String downloadAddress, final URL base) {
    ...
    try {
      ...
      if (fileNode == null) {
        ...
        try (FileInputStream is = new FileInputStream(tmpFile); OutputStream os = StorageProviderFactory.getStorageProvider(fileNode).getOutputStream()) {
          IOUtils.copy(is, os);
  ...
  private void processCssFileNode(final File fileNode, final URL base) throws IOException {
    ...
    try (InputStream is = fileNode.getInputStream()) {
