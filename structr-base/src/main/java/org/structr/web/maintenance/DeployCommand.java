...
public class DeployCommand extends NodeServiceCommand implements MaintenanceCommand {
  ...
  @Override public void execute(final Map<String, Object> parameters) throws FrameworkException {
  ...
  @Override public boolean requiresEnclosingTransaction() {
    return false;
  }

  @Override public boolean requiresFlushingOfCaches() {
    return false;
  }

  public Map<String, Object> readMetadataFileIntoMap(final Path metadataFile) {
    if (Files.exists(metadataFile)) {
      try (Reader reader = Files.newBufferedReader(metadataFile, Charset.forName("utf-8"))) {
  ...
  protected void doImport(final Map<String, Object> attributes) throws FrameworkException {
    ...
    try {
      ...
      if (!Files.exists(deploymentConfFile) && !Files.exists(preDeployConfFile) && !Files.exists(postDeployConfFile) && !Files.exists(grantsMetadataFile) && !Files.exists(corsSettingsMetadataFile) && !Files.exists(mailTemplatesMetadataFile) && !Files.exists(widgetsMetadataFile) && !Files.exists(localizationsMetadataFile) && !Files.exists(applicationConfigurationDataMetadataFile) && !Files.exists(filesMetadataFile) && !Files.exists(pagesMetadataFile) && !Files.exists(componentsMetadataFile) && !Files.exists(templatesMetadataFile) && !Files.exists(actionMappingMetadataFile) && !Files.exists(parameterMappingMetadataFile) && !Files.exists(sitesConfFile) && !Files.exists(schemaFolder)) {
      ...
      if (!acceptDeploymentExportVersion(deploymentConf)) {
        ...
        final String text = "The deployment export data currently being imported has been created with a newer version of Structr " + "which is not supported because of incompatible changes in the deployment format.\n" + "Current version: " + currentVersion + "\n" + "Export version:  " + exportVersion;
        final String htmlText = "The deployment export data currently being imported has been created with a newer version of Structr " + "which is not supported because of incompatible changes in the deployment format.<br><br><table>" + "<tr><th>Current version: </th><td>" + currentVersion + "</td></tr>" + "<tr><th>Export version: </th><td>" + exportVersion + "</td></tr>" + "</table>";
      ...
      if (!missingPrincipals.isEmpty()) {
        ...
        final String text = "The following user(s) and/or group(s) are missing for grants or node ownership during <b>deployment</b>.<br>" + "Because of these missing grants/ownerships, <b>the functionality is not identical to the export you just imported</b>!" + "<ul><li>" + String.join("</li><li>", missingPrincipals) + "</li></ul>" + "Consider adding these principals to your <a href=\"https://docs.structr.com/docs/fundamental-concepts#pre-deployconf\">pre-deploy.conf</a> and re-importing.";
      ...
      if (!missingSchemaFile.isEmpty()) {
        ...
        final String text = "The following schema methods/functions require file(s) to be present in the tree-based schema export.<br>" + "Because those files are missing, the functionality will not be available after importing!<br>" + "The most common cause is that someone forgot to add these files to the repository." + "<ul><li>" + String.join("</li><li>", missingSchemaFile) + "</li></ul>";
  ...
  private void exportResourceAccessGrants(final Path target) throws FrameworkException {
    ...
    if (!unreachableGrants.isEmpty()) {
      ...
      final String htmlText = "The ability to use group/user rights to grants has been added to improve flexibility. The following grants are inaccessible for any non-admin users:<br><br>" + unreachableGrants.stream().reduce("", (acc, signature) -> acc.concat("&nbsp;- ").concat(signature).concat("<br>")) + "<br>You can edit the visibility in the <a href=\"#security\">Security</a> area.";
  ...
  private void exportMailTemplates(final Path targetConf, final Path targetFolder) throws FrameworkException {
    ...
    mailTemplates.sort(new AbstractMapComparator<Object>() {
      @Override public String getKey(final Map<String, Object> map) {
  ...
  private void exportApplicationConfigurationData(final Path target) throws FrameworkException {
    ...
    writeSortedCompactJsonToFile(target, applicationConfigurationDataNodes, new AbstractMapComparator<Object>() {
      @Override public String getKey(Map<String, Object> map) {
  ...
  protected List<Map<String, Object>> readConfigList(final Path conf) {
    try (Reader reader = Files.newBufferedReader(conf, Charset.forName("utf-8"))) {
  ...
  private void importResourceAccessGrants(final List<Map<String, Object>> data) throws FrameworkException {
    ...
    try (Tx tx = app.tx()) {
      ...
      for (final Map<String, Object> entry : data) {
        if (!entry.containsKey("grantees") && !entry.containsKey("visibleToPublicUsers") && !entry.containsKey("visibleToAuthenticatedUsers")) {
          ...
          if (flags != 0) {
            ...
            final boolean hasAnyNonAuthFlags = ((flags & UiAuthenticator.NON_AUTH_USER_GET) == UiAuthenticator.NON_AUTH_USER_GET) || ((flags & UiAuthenticator.NON_AUTH_USER_PUT) == UiAuthenticator.NON_AUTH_USER_PUT) || ((flags & UiAuthenticator.NON_AUTH_USER_POST) == UiAuthenticator.NON_AUTH_USER_POST) || ((flags & UiAuthenticator.NON_AUTH_USER_DELETE) == UiAuthenticator.NON_AUTH_USER_DELETE) || ((flags & UiAuthenticator.NON_AUTH_USER_OPTIONS) == UiAuthenticator.NON_AUTH_USER_OPTIONS) || ((flags & UiAuthenticator.NON_AUTH_USER_HEAD) == UiAuthenticator.NON_AUTH_USER_HEAD) || ((flags & UiAuthenticator.NON_AUTH_USER_PATCH) == UiAuthenticator.NON_AUTH_USER_PATCH);
            final boolean hasAnyAuthFlags = ((flags & UiAuthenticator.AUTH_USER_GET) == UiAuthenticator.AUTH_USER_GET) || ((flags & UiAuthenticator.AUTH_USER_PUT) == UiAuthenticator.AUTH_USER_PUT) || ((flags & UiAuthenticator.AUTH_USER_POST) == UiAuthenticator.AUTH_USER_POST) || ((flags & UiAuthenticator.AUTH_USER_DELETE) == UiAuthenticator.AUTH_USER_DELETE) || ((flags & UiAuthenticator.AUTH_USER_OPTIONS) == UiAuthenticator.AUTH_USER_OPTIONS) || ((flags & UiAuthenticator.AUTH_USER_HEAD) == UiAuthenticator.AUTH_USER_HEAD) || ((flags & UiAuthenticator.AUTH_USER_PATCH) == UiAuthenticator.AUTH_USER_PATCH);
      ...
      if (isOldExport) {
        ...
        final String htmlText = "Configuration was auto-updated using this simple heuristic:<br>" + "&nbsp;- Grants with public access were set to <code>visibleToPublicUsers: true</code><br>" + "&nbsp;- Grants with authenticated access were set to <code>visibleToAuthenticatedUsers: true</code><br><br>" + "Please make any necessary changes in the <a href=\"#security\">Security</a> area as this may not suffice for your use case. The ability to use group/user rights to grants has been added to improve flexibility.";
        deferredLogTexts.add(text + "\n\n" + grantMessagesText);
  ...
  private void importHTMLContent(final App app, final Path source, final Path pagesMetadataFile, final Path componentsMetadataFile, final Path templatesMetadataFile, final Path sitesConfFile, final boolean extendExistingApp, final boolean relativeVisibility, final Map<DOMNode, PropertyMap> deferredNodesAndTheirProperties) throws FrameworkException {
    ...
    if (keysTotal > 0) {
      if (!relativeVisibility) {
        ...
        final String text = "The deployment export data currently being imported has been created with an older version of Structr (or deployment.conf is missing) " + "in which the visibility flags of DOM elements were exported depending on the flags of the containing page (or deployment.conf is missing).\n\n" + "***The data will be imported correctly, based on the old format.***\n\n" + "After this import has finished, you should **export again to the same location** so that the deployment export data will be upgraded to the most recent format.";
        final String htmlText = "The deployment export currently being imported has been created with an older version of Structr " + "in which the visibility flags of DOM elements were exported depending on the flags of the containing page (or deployment.conf is missing).<br><br>" + "<b>The data will be imported correctly, based on the old format.</b><br><br>" + "After this import has finished, you should <b>export again to the same location</b> so that the deployment export data will be upgraded to the most recent format.";
    ...
    if (Files.exists(schemaFolder)) {
      try {
        ...
        if (!Files.exists(schemaJsonFile)) {
          ...
          ctx.setDoTransactionNotifications(false);
          final App app = StructrApp.getInstance(ctx);
          try (FileReader reader = new FileReader(schemaJsonFile.toFile())) {
  ...
  protected void writeJsonToFile(final Path path, final Object data) {
    try (Writer fos = new OutputStreamWriter(new FileOutputStream(path.toFile()))) {
  ...
  protected void writeSortedCompactJsonToFile(final Path target, final List<Map<String, Object>> objects, final AbstractMapComparator<Object> sortComparator) {
    try (Writer fos = new OutputStreamWriter(new FileOutputStream(target.toFile()))) {
