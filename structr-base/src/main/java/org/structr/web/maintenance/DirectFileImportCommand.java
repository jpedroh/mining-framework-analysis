...
public class DirectFileImportCommand extends NodeServiceCommand implements MaintenanceCommand {
  ...
  @Override public void execute(final Map<String, Object> attributes) throws FrameworkException {
    ...
    if (sourcePath.contains(PathHelper.PATH_SEP)) {
      ...
      if (StringUtils.isNotBlank(folderPart)) {
        ...
        try {
          try (DirectoryStream<Path> stream = Files.newDirectoryStream(source, namePart)) {
    ...
    try (Tx tx = app.tx()) {
      ...
      if (attrs.isDirectory()) {
        ...
        if (attrs.isRegularFile()) {
          ...
          try (InputStream is = new FileInputStream(file.toFile()); OutputStream os = StorageProviderFactory.getStorageProvider(newFile).getOutputStream()) {
            IOUtils.copy(is, os);
          }
