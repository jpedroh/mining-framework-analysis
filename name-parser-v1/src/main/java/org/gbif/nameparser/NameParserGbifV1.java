...
public class NameParserGbifV1 implements NameParser {
  ...
  private static final Map<org.gbif.nameparser.api.NameType, NameType> NAME_TYPE_MAP = ImmutableMap.<org.gbif.nameparser.api.NameType, NameType>builder().put(org.gbif.nameparser.api.NameType.SCIENTIFIC, NameType.SCIENTIFIC).put(org.gbif.nameparser.api.NameType.VIRUS, NameType.VIRUS).put(org.gbif.nameparser.api.NameType.HYBRID_FORMULA, NameType.HYBRID).put(org.gbif.nameparser.api.NameType.INFORMAL, NameType.INFORMAL).put(org.gbif.nameparser.api.NameType.OTU, NameType.OTU).put(org.gbif.nameparser.api.NameType.PLACEHOLDER, NameType.PLACEHOLDER).put(org.gbif.nameparser.api.NameType.NO_NAME, NameType.NO_NAME).build();
  ...
  @Override public ParsedName parse(String s, @Nullable Rank rank) throws UnparsableException {
    try {
  ...
  @Override public ParsedName parse(String scientificName) throws UnparsableException {
  ...
  @Override public String parseToCanonical(String scientificName, @Nullable Rank rank) {
  ...
  private ParsedName convert(String scientificName, Rank rank, org.gbif.nameparser.api.ParsedName pn) throws UnparsableException {
    if ((!pn.getType().isParsable() && pn.getType() != org.gbif.nameparser.api.NameType.PLACEHOLDER) || pn.getState() == org.gbif.nameparser.api.ParsedName.State.NONE) {
    ...
    if (pn.getEpithetQualifier() != null && !pn.getEpithetQualifier().isEmpty()) {
      ...
      for (Map.Entry<NamePart, String> pq : pn.getEpithetQualifier().entrySet()) {
        ...
        sb.append(pq.getValue()).append(" ").append(pn.getEpithet(pq.getKey()));
      }
  ...
  public static NameType gbifNameType(org.gbif.nameparser.api.ParsedName pn) {
    ...
    if (pn.isDoubtful() && pn.getWarnings().contains(Warnings.BLACKLISTED_EPITHET)) {
      ...
      if (pn.isCandidatus()) {
        ...
        if (pn.getCode() == NomCode.CULTIVARS || pn.getCultivarEpithet() != null) {
          t = NameType.CULTIVAR;
        } else {
