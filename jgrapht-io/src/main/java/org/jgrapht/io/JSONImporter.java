 ...
 */
public class JSONImporter<V extends java.lang.Object, E extends java.lang.Object> extends AbstractBaseImporter<V, E> implements GraphImporter<V, E> {
  /**
  ...
  /**
     ...
     */
  @Override public void importGraph(Graph<V, E> graph, Reader input) throws ImportException {
    try {
  ...
  private class ThrowingErrorListener extends BaseErrorListener {
    @Override public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol, int line, int charPositionInLine, String msg, RecognitionException e) throws ParseCancellationException {
  ...
  private class CreateGraphJsonListener extends JsonBaseListener {
    ...
    public void updateGraph(Graph<V, E> graph) throws ImportException {
      ...
      for (PartialEdge pe : edges) {
        ...
        if (isWeighted) {
          ...
          if (weight != null) {
            AttributeType type = weight.getType();
            if (type.equals(AttributeType.FLOAT) || type.equals(AttributeType.DOUBLE)) {
              graph.setEdgeWeight(e, Double.parseDouble(weight.getValue()));
            }
          }
        }
      }
    ...
    @Override public void enterArray(JsonParser.ArrayContext ctx) {
      ...
      if (insideNodes && objectLevel == 1 && arrayLevel == 1) {
        ...
        if (insideEdges && objectLevel == 1 && arrayLevel == 1) {
