...
public class CodeGeneratorController extends CayenneController implements ObjEntityListener, EmbeddableListener, DataMapListener {
  ...
  public CodeGeneratorController(ProjectController projectController) {
    ...
    this.classes = new TreeSet<>(Comparator.comparing((ConfigurationNode o) -> o.acceptVisitor(TYPE_GETTER)).thenComparing((o) -> o.acceptVisitor(NAME_GETTER)));
  ...
  @Override public CodeGeneratorPane getView() {
    return view;
  }
  ...
  @SuppressWarnings(value = { "unused" }) public void generateAction() {
    ClassGenerationAction generator = new ToolsInjectorBuilder().addModule((binder) -> binder.bind(DataChannelMetaData.class).toInstance(projectController.getApplication().getMetaData())).create().getInstance(ClassGenerationActionFactory.class).createAction(cgenConfiguration);
    try {
  ...
  @SuppressWarnings(value = { "unused" }) public void addConfigAction() {
    String name = JOptionPane.showInputDialog(view, "Type the name for new cgenConfiguration", view.getConfigurationsComboBox().getSelectedItem());
  ...
  @SuppressWarnings(value = { "unused" }) public void editConfigAction() {
    String name = JOptionPane.showInputDialog(view, "Type the new name for cgenConfiguration", view.getConfigurationsComboBox().getSelectedItem());
    if (name != null) {
  ...
  @SuppressWarnings(value = { "unused" }) public void removeConfigAction() {
    int result = JOptionPane.showConfirmDialog(view, "Configuration will be remove\n" + "               Are you sure?", "Delete cgenConfiguration", JOptionPane.YES_NO_OPTION);
    if (result == 0) {
  ...
  public void setConfiguration(String selectedConfig) {
    ...
    addToSelectedEntities(dataMap.getObjEntities().stream().map(Entity::getName).collect(Collectors.toList()));
    addToSelectedEmbeddables(dataMap.getEmbeddables().stream().map(Embeddable::getClassName).collect(Collectors.toList()));
  }
  ...
  @Override public void objEntityChanged(EntityEvent e) {
  }

  @Override public void objEntityAdded(EntityEvent e) {
    addEntity(e.getEntity().getDataMap(), (ObjEntity) e.getEntity());
  }

  @Override public void objEntityRemoved(EntityEvent e) {
  ...
  @Override public void embeddableChanged(EmbeddableEvent e, DataMap map) {
  }

  @Override public void embeddableAdded(EmbeddableEvent e, DataMap map) {
    prepareClasses(map);
  ...
  @Override public void embeddableRemoved(EmbeddableEvent e, DataMap map) {
  ...
  @Override public void dataMapChanged(DataMapEvent e) {
  ...
  @Override public void dataMapAdded(DataMapEvent e) {
  }

  @Override public void dataMapRemoved(DataMapEvent e) {
  }
  ...
  public void onProjectSaved(ProjectSavedEvent e) {
    if (getStandardModeController() != null && getStandardModeController().getView() != null && cgenConfiguration != null) {
  ...
  private final Predicate<ConfigurationNode> defaultPredicate = (o) -> o.acceptVisitor(new BaseConfigurationNodeVisitor<Boolean>() {
    @Override public Boolean visitDataMap(DataMap dataMap) {
      return false;
    }

    @Override public Boolean visitObjEntity(ObjEntity entity) {
      return classesSelector.getProblem(entity.getName()) == null;
    }

    @Override public Boolean visitEmbeddable(Embeddable embeddable) {
  ...
  private static final ConfigurationNodeVisitor<Integer> TYPE_GETTER = new BaseConfigurationNodeVisitor<Integer>() {
    @Override public Integer visitDataMap(DataMap dataMap) {
      return 10;
    }

    @Override public Integer visitObjEntity(ObjEntity entity) {
      return 20;
    }

    @Override public Integer visitEmbeddable(Embeddable embeddable) {
      return 30;
    }
  };

  private static final ConfigurationNodeVisitor<String> NAME_GETTER = new BaseConfigurationNodeVisitor<String>() {
    @Override public String visitDataMap(DataMap dataMap) {
      return dataMap.getName();
    }

    @Override public String visitEmbeddable(Embeddable embeddable) {
      return embeddable.getClassName();
    }

    @Override public String visitObjEntity(ObjEntity entity) {
