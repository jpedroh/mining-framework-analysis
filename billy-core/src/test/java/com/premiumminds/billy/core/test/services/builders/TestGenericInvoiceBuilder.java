...
public class TestGenericInvoiceBuilder extends AbstractTest {
  ...
  @Test public void doTest() {
    MockGenericInvoiceEntity mock = this.createMockEntity(MockGenericInvoiceEntity.class, TestGenericInvoiceBuilder.INVOICE_YML);
    mock.setCurrency(Currency.getInstance("EUR"));
    Mockito.when(this.getInstance(DAOGenericInvoice.class).getEntityInstance()).thenReturn(new MockGenericInvoiceEntity());
    MockCustomerEntity mockCustomerEntity = this.createMockEntity(MockCustomerEntity.class, TestGenericInvoiceBuilder.CUSTOMER_YML);
    MockSupplierEntity mockSupplierEntity = this.createMockEntity(MockSupplierEntity.class, TestGenericInvoiceBuilder.SUPPLIER_YML);
    ...
    MockGenericInvoiceEntryEntity mockInvoice = this.createMockEntity(MockGenericInvoiceEntryEntity.class, TestGenericInvoiceBuilder.ENTRY_YML);
    Mockito.when(this.getInstance(DAOGenericInvoiceEntry.class).get(Matchers.any(UID.class))).thenReturn(mockInvoice);
    ...
    for (int i = 0; i < invoice.getEntries().size(); i++) {
      ...
      Assert.assertEquals(invoices.get(i).getUnitAmountWithoutTax(), mockInvoices.get(i).getUnitAmountWithoutTax());
      Assert.assertEquals(invoices.get(i).getUnitAmountWithTax(), mockInvoices.get(i).getUnitAmountWithTax());
      Assert.assertEquals(invoices.get(i).getUnitTaxAmount(), mockInvoices.get(i).getUnitTaxAmount());
