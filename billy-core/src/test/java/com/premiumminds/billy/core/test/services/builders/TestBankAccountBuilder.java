...
public class TestBankAccountBuilder extends AbstractTest {
  ...
  @Test public void doTest() {
    MockBankAccountEntity mockBankAccount = this.createMockEntity(MockBankAccountEntity.class, TestBankAccountBuilder.BANK_ACCOUNT_YML);
    Mockito.when(this.getInstance(DAOBankAccount.class).getEntityInstance()).thenReturn(new MockBankAccountEntity());
