...
public class TestBusinessBuilder extends AbstractTest {
  ...
  @Test public void doTest() {
    MockBusinessEntity mockBusiness = this.createMockEntity(MockBusinessEntity.class, TestBusinessBuilder.BUSINESS_YML);
    Mockito.when(this.getInstance(DAOBusiness.class).getEntityInstance()).thenReturn(new MockBusinessEntity());
    Mockito.when(this.getInstance(DAOContext.class).get(Matchers.any(UID.class))).thenReturn((ContextEntity) mockBusiness.getOperationalContext());
