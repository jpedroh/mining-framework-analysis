...
public class TestContextBuilder extends AbstractTest {
  ...
  @Test public void doTest() {
    ...
    Mockito.when(this.getInstance(DAOContext.class).get(Matchers.any(UID.class))).thenReturn((ContextEntity) mockContext.getParentContext());
    Context.Builder builder = this.getInstance(Context.Builder.class);
    builder.setDescription(mockContext.getDescription()).setName(mockContext.getName()).setParentContextUID(mockContext.getParentContext().getUID());
