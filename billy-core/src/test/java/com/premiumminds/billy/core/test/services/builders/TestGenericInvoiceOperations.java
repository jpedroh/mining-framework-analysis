...
public class TestGenericInvoiceOperations extends AbstractTest {
  ...
  @Before public void setUp() {
    this.mockInvoiceEntity = this.createMockEntity(MockGenericInvoiceEntity.class, TestGenericInvoiceOperations.INVOICE_YML);
    Mockito.when(this.getInstance(DAOGenericInvoice.class).getEntityInstance()).thenReturn(new MockGenericInvoiceEntity());
    Mockito.when(this.getInstance(DAOContext.class).isSubContext(Matchers.any(Context.class), Matchers.any(Context.class))).thenReturn(true);
    this.mockCustomerEntity = this.createMockEntity(MockCustomerEntity.class, TestGenericInvoiceOperations.CUSTOMER_YML);
    this.mockSupplierEntity = this.createMockEntity(MockSupplierEntity.class, TestGenericInvoiceOperations.SUPPLIER_YML);
    this.mockInvoiceEntity.setCustomer(this.mockCustomerEntity);
    Mockito.when(this.getInstance(DAOCustomer.class).get(Matchers.any(UID.class))).thenReturn(this.mockCustomerEntity);
    Mockito.when(this.getInstance(DAOSupplier.class).get(Matchers.any(UID.class))).thenReturn(this.mockSupplierEntity);
  }

  @Test public void simpleOperationsTest() {
    MockGenericInvoiceEntryEntity mockEntry = this.getMockEntryEntity(this.mockInvoiceEntity, this.testValue2);
    Mockito.when(this.getInstance(DAOGenericInvoiceEntry.class).get(Matchers.any(UID.class))).thenReturn(mockEntry);
  ...
  @Test public void operationsTest() {
    ...
    GenericInvoice invoice = builder.build();
    Assert.assertTrue(invoice != null);
    Assert.assertTrue(invoice.getAmountWithoutTax().compareTo(mockEntry.getAmountWithoutTax().add(mockEntry2.getAmountWithoutTax(), this.mc)) == 0);
    Assert.assertTrue(invoice.getAmountWithTax().compareTo(mockEntry.getAmountWithTax().add(mockEntry2.getAmountWithTax(), this.mc)) == 0);
    Assert.assertTrue(invoice.getTaxAmount().compareTo(mockEntry.getTaxAmount().add(mockEntry2.getTaxAmount(), this.mc)) == 0);
  }

  @Test public void manyEntriesTest() {
    ...
    GenericInvoice.Builder builder = this.getBuilder();
    for (int i = 0; i < 20; i++) {
      MockGenericInvoiceEntryEntity mockEntry = this.getMockEntryEntity(this.mockInvoiceEntity, new BigDecimal("5.977"));
  ...
  @Test public void testRoundingValues() {
    this.mockInvoiceEntity.getEntries().clear();
    MockGenericInvoiceEntryEntity mockEntry1 = this.getMockEntryEntity(this.mockInvoiceEntity, new BigDecimal("0.33"));
    MockGenericInvoiceEntryEntity mockEntry2 = this.getMockEntryEntity(this.mockInvoiceEntity, new BigDecimal("0.33"));
    this.mockInvoiceEntity.getEntries().add(mockEntry1);
    this.mockInvoiceEntity.getEntries().add(mockEntry2);
    Mockito.when(this.getInstance(DAOGenericInvoiceEntry.class).get(Matchers.any(UID.class))).thenReturn(mockEntry1);
    GenericInvoice.Builder builder = this.getBuilder();
    ...
    GenericInvoice invoice = builder.build();
    Assert.assertTrue(invoice.getTaxAmount().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).compareTo(invoice.getAmountWithTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).subtract(invoice.getAmountWithoutTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()))) == 0);
  }

  public MockGenericInvoiceEntryEntity getMockEntryEntity(MockGenericInvoiceEntity invoice, BigDecimal unitValue) {
    MockGenericInvoiceEntryEntity result = this.createMockEntity(MockGenericInvoiceEntryEntity.class, TestGenericInvoiceOperations.ENTRY_YML);
  ...
  public GenericInvoice.Builder getBuilder() {
    ...
    builder.setBatchId(this.mockInvoiceEntity.getBatchId()).setDate(this.mockInvoiceEntity.getDate()).setGeneralLedgerDate(this.mockInvoiceEntity.getGeneralLedgerDate()).setOfficeNumber(this.mockInvoiceEntity.getOfficeNumber()).setPaymentTerms(this.mockInvoiceEntity.getPaymentTerms()).setSelfBilled(this.mockInvoiceEntity.selfBilled).setSettlementDate(this.mockInvoiceEntity.getSettlementDate()).setSettlementDescription(this.mockInvoiceEntity.getSettlementDescription()).setSettlementDiscount(this.mockInvoiceEntity.getSettlementDiscount()).setSourceId(this.mockInvoiceEntity.getSourceId()).setTransactionId(this.mockInvoiceEntity.getTransactionId()).setCustomerUID(this.mockCustomerEntity.getUID()).setSupplierUID(this.mockSupplierEntity.getUID());
    return builder;
  }
}