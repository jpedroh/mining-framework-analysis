...
public class TestGenericInvoiceOperations extends AbstractTest {
  ...
  @BeforeEach public void setUp() {
    ...
    Mockito.when(this.getInstance(DAOContext.class).isSameOrSubContext(Mockito.any(Context.class), Mockito.any(Context.class))).thenReturn(true);
    ...
    Mockito.when(this.getInstance(DAOCustomer.class).get(Mockito.any(UID.class))).thenReturn(this.mockCustomerEntity);
    Mockito.when(this.getInstance(DAOSupplier.class).get(Mockito.any(UID.class))).thenReturn(this.mockSupplierEntity);
  }
  ...
  @Test public void operationsTest() {
    ...
    GenericInvoice invoice = builder.build();
    Assertions.assertTrue(invoice != null);
    Assertions.assertTrue(invoice.getAmountWithoutTax().compareTo(mockEntry.getAmountWithoutTax().add(mockEntry2.getAmountWithoutTax(), this.mc)) == 0);
    Assertions.assertTrue(invoice.getAmountWithTax().compareTo(mockEntry.getAmountWithTax().add(mockEntry2.getAmountWithTax(), this.mc)) == 0);
    Assertions.assertTrue(invoice.getTaxAmount().compareTo(mockEntry.getTaxAmount().add(mockEntry2.getTaxAmount(), this.mc)) == 0);
  }
  ...
  @Test public void testRoundingValues() {
    ...
    Mockito.when(this.getInstance(DAOGenericInvoiceEntry.class).get(Mockito.any(UID.class))).thenReturn(mockEntry1);
    GenericInvoice.Builder builder = this.getBuilder();
    ...
    GenericInvoice invoice = builder.build();
    Assertions.assertTrue(invoice.getTaxAmount().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).compareTo(invoice.getAmountWithTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()).subtract(invoice.getAmountWithoutTax().setScale(BillyMathContext.SCALE, this.mc.getRoundingMode()))) == 0);
  }
