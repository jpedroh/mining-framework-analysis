...
public class TestSupplierBuilder extends AbstractTest {
  ...
  @Test public void doTest() {
    MockSupplierEntity mockSupplier = this.createMockEntity(MockSupplierEntity.class, TestSupplierBuilder.SUPPLIER_YML);
    ...
    builder.addAddress(mockMainAddressBuilder).addAddress(mockShippingAddressBuilder).addAddress(mockBillingAddressBuilder).addContact(mockMainContactBuilder).addBankAccount(mockBankAccountBuilder1).setBillingAddress(mockBillingAddressBuilder).setMainAddress(mockMainAddressBuilder).setMainContact(mockMainContactBuilder).setName(mockSupplier.getName()).setSelfBillingAgreement(mockSupplier.hasSelfBillingAgreement()).setTaxRegistrationNumber(mockSupplier.getTaxRegistrationNumber(), null).setShippingAddress(mockShippingAddressBuilder);
