...
public class TestCustomerBuilder extends AbstractTest {
  ...
  @Test public void doTest() {
    MockCustomerEntity mockCustomer = this.createMockEntity(MockCustomerEntity.class, TestCustomerBuilder.CUSTOMER_YML);
    Mockito.when(this.getInstance(DAOCustomer.class).getEntityInstance()).thenReturn(new MockCustomerEntity());
    Mockito.when(this.getInstance(DAOContact.class).get(Matchers.any(UID.class))).thenReturn((ContactEntity) mockCustomer.getMainContact());
    ...
    builder.addBankAccount(mockBankAccountBuilder1).addBankAccount(mockBankAccountBuilder2).addAddress(mockMainAddressBuilder, true).addContact(mockMainContactBuilder).addContact(mockContactBuilder1).addContact(mockContactBuilder2).setBillingAddress(mockBillingAddressBuilder).setHasSelfBillingAgreement(mockCustomer.hasSelfBillingAgreement()).setName(mockCustomer.getName()).setShippingAddress(mockShippingAddressBuilder).setTaxRegistrationNumber(mockCustomer.getTaxRegistrationNumber(), null).setMainContactUID(mockCustomer.getMainContact().getUID());
