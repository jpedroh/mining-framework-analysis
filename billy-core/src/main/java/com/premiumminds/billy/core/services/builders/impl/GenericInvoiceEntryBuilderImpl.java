...
public class GenericInvoiceEntryBuilderImpl<TBuilder extends GenericInvoiceEntryBuilderImpl<TBuilder, TEntry, TDAOEntry, TDAOInvoice>, TEntry extends GenericInvoiceEntry, TDAOEntry extends AbstractDAOGenericInvoiceEntry<?>, TDAOInvoice extends AbstractDAOGenericInvoice<?>> extends AbstractBuilder<TBuilder, TEntry> implements GenericInvoiceEntryBuilder<TBuilder, TEntry> {
  ...
  @Inject public GenericInvoiceEntryBuilderImpl(TDAOEntry daoEntry, TDAOInvoice daoInvoice, DAOTax daoTax, DAOProduct daoProduct, DAOContext daoContext) {
    super(daoEntry);
  ...
  @Override @NotOnUpdate public <T extends ShippingPoint> TBuilder setShippingOrigin(Builder<T> originBuilder) {
    BillyValidator.notNull(originBuilder, GenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.entry_shipping_origin"));
    this.getTypeInstance().setShippingOrigin(originBuilder.build());
    return this.getBuilder();
  }

  @Override @NotOnUpdate public <T extends ShippingPoint> TBuilder setShippingDestination(Builder<T> destinationBuilder) {
    BillyValidator.notNull(destinationBuilder, GenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.entry_shipping_destination"));
    this.getTypeInstance().setShippingDestination(destinationBuilder.build());
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setProductUID(UID productUID) {
  ...
  @Override @NotOnUpdate public TBuilder setCurrency(Currency currency) {
    BillyValidator.notNull(currency, "field.currency");
    this.getTypeInstance().setCurrency(currency);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setQuantity(BigDecimal quantity) {
    Validate.isTrue(quantity.compareTo(BigDecimal.ZERO) > 0, "The quantity must be positive");
    this.getTypeInstance().setQuantity(quantity);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setUnitOfMeasure(String unit) {
    BillyValidator.notBlank(unit, "field.unit");
    this.getTypeInstance().setUnitOfMeasure(unit);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setTaxPointDate(Date date) {
  ...
  @Override @NotOnUpdate public TBuilder addDocumentReferenceUID(UID referenceUID) {
  ...
  @Override @NotOnUpdate public TBuilder setDescription(String description) {
  ...
  @Override @NotOnUpdate public TBuilder setShippingCostsAmount(BigDecimal amount) {
    BillyValidator.notNull(amount, GenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.shipping_costs_amount"));
    Validate.isTrue(amount.compareTo(BigDecimal.ZERO) >= 0);
    this.getTypeInstance().setShippingCostsAmount(amount);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setAmountType(AmountType type) {
    this.getTypeInstance().setAmountType(type);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setUnitAmount(AmountType type, BigDecimal amount) {
  ...
  @Override @NotOnUpdate public TBuilder setContextUID(UID uidContext) {
  ...
  @Override @NotOnUpdate public TBuilder setTaxExemptionReason(String exemptionReason) {
    BillyValidator.notBlank(exemptionReason, GenericInvoiceEntryBuilderImpl.LOCALIZER.getString("field.tax_exemption_reason"));
  ...
  protected void validateValues() throws ValidationException {
    ...
    if (e.getTaxes().isEmpty()) {
      throw new ValidationException(GenericInvoiceEntryBuilderImpl.LOCALIZER.getString("exception.invalid_taxes"));
    }
    e.setUnitDiscountAmount(BigDecimal.ZERO);
    if (e.getUnitAmountWithTax() != null) {
      ...
      for (Tax t : this.getTypeInstance().getTaxes()) {
        switch (t.getTaxRateType()) {
          ...
          break;
          case PERCENTAGE:
          unitAmountWithoutTax = e.getUnitAmountWithTax().divide(BigDecimal.ONE.add(t.getPercentageRateValue().divide(new BigDecimal("100"), mc), mc), mc);
          unitTaxAmount = unitTaxAmount.add(e.getUnitAmountWithTax().subtract(unitAmountWithoutTax, mc), mc);
          break;
          default:
          break;
        }
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override protected GenericInvoiceEntryEntity getTypeInstance() {
