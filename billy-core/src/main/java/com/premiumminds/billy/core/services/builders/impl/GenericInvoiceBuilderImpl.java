...
public class GenericInvoiceBuilderImpl<TBuilder extends GenericInvoiceBuilderImpl<TBuilder, TEntry, TDocument>, TEntry extends GenericInvoiceEntry, TDocument extends GenericInvoice> extends AbstractBuilder<TBuilder, TDocument> implements GenericInvoiceBuilder<TBuilder, TEntry, TDocument> {
  ...
  public <TDAO extends AbstractDAOGenericInvoice<? extends TDocument>> GenericInvoiceBuilderImpl(TDAO daoGenericInvoice, DAOBusiness daoBusiness, DAOCustomer daoCustomer, DAOSupplier daoSupplier) {
  ...
  @Override @NotOnUpdate @NotImplemented public TBuilder setCurrency(Currency currency) {
  ...
  @Override @NotOnUpdate public TBuilder setBusinessUID(UID businessUID) {
  ...
  @Override @NotOnUpdate public TBuilder setCustomerUID(UID customerUID) {
  ...
  @Override @NotOnUpdate public TBuilder setSupplierUID(UID supplier) {
  ...
  @Override @NotOnUpdate public TBuilder setOfficeNumber(String number) {
  ...
  @Override @NotOnUpdate public TBuilder setDate(Date date) {
    BillyValidator.notNull(date, GenericInvoiceBuilderImpl.LOCALIZER.getString("field.date"));
    this.getTypeInstance().setDate(date);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public <T extends ShippingPointEntity> TBuilder setShippingOrigin(Builder<T> originBuilder) {
  ...
  @Override @NotOnUpdate public <T extends ShippingPointEntity> TBuilder setShippingDestination(Builder<T> destinationBuilder) {
    Validate.notNull(destinationBuilder, GenericInvoiceBuilderImpl.LOCALIZER.getString("field.shipping_destination"));
    this.getTypeInstance().setShippingDestination(destinationBuilder.build());
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setPaymentTerms(String terms) {
  ...
  @Override @NotOnUpdate public TBuilder setSelfBilled(boolean selfBilled) {
    this.getTypeInstance().setSelfBilled(selfBilled);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setSourceId(String source) {
  ...
  @Override @NotOnUpdate public TBuilder setGeneralLedgerDate(Date date) {
  ...
  @Override @NotOnUpdate public TBuilder setBatchId(String id) {
    Validate.notBlank(id, GenericInvoiceBuilderImpl.LOCALIZER.getString("field.batch_id"));
    this.getTypeInstance().setBatchId(id);
    return this.getBuilder();
  }

  @Override @NotOnUpdate public TBuilder setTransactionId(String id) {
  ...
  @Override @NotOnUpdate public TBuilder addReceiptNumber(String number) {
    BillyValidator.notBlank(number, GenericInvoiceBuilderImpl.LOCALIZER.getString("field.receipt_number"));
    return this.getBuilder();
  }

  @Override @NotOnUpdate public <T extends GenericInvoiceEntry> TBuilder addEntry(Builder<T> entryBuilder) {
    BillyValidator.notNull(entryBuilder, GenericInvoiceBuilderImpl.LOCALIZER.getString("field.entry"));
    T entry = entryBuilder.build();
    Validate.isInstanceOf(GenericInvoiceEntryEntity.class, entry);
  ...
  @Override @NotOnUpdate public TBuilder setSettlementDescription(String description) {
  ...
  @Override @NotOnUpdate public TBuilder setSettlementDiscount(BigDecimal discount) {
    Validate.isTrue(discount == null || discount.compareTo(BigDecimal.ZERO) >= 0, GenericInvoiceBuilderImpl.LOCALIZER.getString("field.discount"));
    this.getTypeInstance().setSettlementDiscount(discount);
    return this.getBuilder();
  }

  @NotImplemented @Deprecated @Override public TBuilder setDiscounts(DiscountType type, BigDecimal... discounts) {
    return null;
  }

  @Override @NotOnUpdate public TBuilder setSettlementDate(Date date) {
  ...
  @Override @NotOnUpdate public <T extends Payment> TBuilder addPayment(Builder<T> paymentBuilder) {
  ...
  @Override public TBuilder setScale(int scale) {
    Validate.notNull(scale);
    this.getTypeInstance().setScale(scale);
    return this.getBuilder();
  }

  @NotOnUpdate @Override protected void validateInstance() throws ValidationException {
    GenericInvoiceEntity i = this.getTypeInstance();
  ...
  protected void validateValues() throws ValidationException {
    ...
    Validate.isTrue(i.getAmountWithTax().subtract(i.getTaxAmount(), mc).setScale(7, mc.getRoundingMode()).compareTo(i.getAmountWithoutTax().setScale(7, mc.getRoundingMode())) == 0, "The invoice values are invalid", i.getAmountWithTax(), i.getAmountWithoutTax(), i.getTaxAmount());
    Validate.isTrue(i.getAmountWithTax().compareTo(BigDecimal.ZERO) > 0 && i.getAmountWithoutTax().compareTo(BigDecimal.ZERO) >= 0 && i.getTaxAmount().compareTo(BigDecimal.ZERO) > 0, "The invoice values are lower than zero", i.getAmountWithTax(), i.getAmountWithoutTax(), i.getTaxAmount());
  }

  @Deprecated @NotImplemented protected void applyDiscounts() {
    GenericInvoiceEntity i = this.getTypeInstance();
    i.getSettlementDiscount();
  }

  @SuppressWarnings(value = { "unchecked" }) @Override protected GenericInvoiceEntity getTypeInstance() {
