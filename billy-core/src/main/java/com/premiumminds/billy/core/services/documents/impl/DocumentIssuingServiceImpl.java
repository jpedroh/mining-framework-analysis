...
public class DocumentIssuingServiceImpl implements DocumentIssuingService {
  ...
  private <T extends GenericInvoice> T issueDocument(Builder<T> documentBuilder, final IssuingParams parameters) throws DocumentIssuingException, SeriesUniqueCodeNotFilled, DocumentSeriesDoesNotExistException {
    final T document = documentBuilder.build();
    final Type[] types = document.getClass().getGenericInterfaces();
    for (Type type : types) {
      if (this.handlers.containsKey(type)) {
        @SuppressWarnings(value = { "unchecked" }) DocumentIssuingHandler<T, IssuingParams> handler = (DocumentIssuingHandler<T, IssuingParams>) this.handlers.get(type);
