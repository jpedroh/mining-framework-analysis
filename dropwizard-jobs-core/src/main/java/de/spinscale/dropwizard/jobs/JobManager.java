...
public class JobManager implements Managed {
  ...
  protected void scheduleAllJobsOnApplicationStop() throws SchedulerException {
    List<JobDetail> jobDetails = Arrays.stream(jobs).filter((job) -> job.getClass().isAnnotationPresent(OnApplicationStop.class)).map((job) -> JobBuilder.newJob(job.getClass()).build()).collect(Collectors.toList());
  ...
  protected void scheduleAllJobsWithOnAnnotation() throws SchedulerException {
    List<Class<? extends Job>> onJobClasses = Arrays.stream(this.jobs).filter((job) -> job.getClass().isAnnotationPresent(On.class)).map((job) -> job.getClass()).collect(Collectors.toList());
    ...
    for (Class<? extends org.quartz.Job> clazz : onJobClasses) {
      ...
      if (misfirePolicy == On.MisfirePolicy.IGNORE_MISFIRES) {
        ...
        if (misfirePolicy == On.MisfirePolicy.DO_NOTHING) {
          scheduleBuilder.withMisfireHandlingInstructionDoNothing();
        } else {
          if (misfirePolicy == On.MisfirePolicy.FIRE_AND_PROCEED) {
  ...
  protected void scheduleAllJobsWithEveryAnnotation() throws SchedulerException {
    List<Class<? extends Job>> everyJobClasses = Arrays.stream(this.jobs).filter((job) -> job.getClass().isAnnotationPresent(Every.class)).map((job) -> job.getClass()).collect(Collectors.toList());
    ...
    for (Class<? extends org.quartz.Job> clazz : everyJobClasses) {
      ...
      SimpleScheduleBuilder scheduleBuilder = SimpleScheduleBuilder.simpleSchedule().withIntervalInMilliseconds(milliSeconds);
      if (repeatCount > -1) {
      ...
      if (misfirePolicy == Every.MisfirePolicy.IGNORE_MISFIRES) {
        ...
        if (misfirePolicy == Every.MisfirePolicy.FIRE_NOW) {
          scheduleBuilder.withMisfireHandlingInstructionFireNow();
        } else {
          if (misfirePolicy == Every.MisfirePolicy.NOW_WITH_EXISTING_COUNT) {
            scheduleBuilder.withMisfireHandlingInstructionNowWithExistingCount();
          } else {
            if (misfirePolicy == Every.MisfirePolicy.NOW_WITH_REMAINING_COUNT) {
              scheduleBuilder.withMisfireHandlingInstructionNowWithRemainingCount();
            } else {
              if (misfirePolicy == Every.MisfirePolicy.NEXT_WITH_EXISTING_COUNT) {
                scheduleBuilder.withMisfireHandlingInstructionNextWithExistingCount();
              } else {
                if (misfirePolicy == Every.MisfirePolicy.NEXT_WITH_REMAINING_COUNT) {
      ...
      Trigger trigger = TriggerBuilder.newTrigger().withSchedule(scheduleBuilder).startAt(start.toDate()).withPriority(priority).build();
  ...
  @SuppressWarnings(value = { "unchecked" }) private String readDurationFromConfig(Every annotation, Class<? extends org.quartz.Job> clazz) {
  ...
  protected void scheduleAllJobsOnApplicationStart() throws SchedulerException {
    List<JobDetail> jobDetails = Arrays.stream(this.jobs).filter((job) -> job.getClass().isAnnotationPresent(OnApplicationStart.class)).map((job) -> JobBuilder.newJob(job.getClass()).build()).collect(Collectors.toList());
  ...
  private void logAllOnApplicationStopJobs() {
    ...
    Arrays.stream(this.jobs).filter((job) -> job.getClass().isAnnotationPresent(OnApplicationStop.class)).map((job) -> job.getClass()).forEach((clazz) -> log.info("   " + clazz.getCanonicalName()));
  }

  private void createOrUpdateJob(JobKey jobKey, Class<? extends org.quartz.Job> clazz, Trigger trigger, boolean requestsRecovery, boolean storeDurably) throws SchedulerException {
    JobBuilder jobBuilder = JobBuilder.newJob(clazz).withIdentity(jobKey).requestRecovery(requestsRecovery).storeDurably(storeDurably);
