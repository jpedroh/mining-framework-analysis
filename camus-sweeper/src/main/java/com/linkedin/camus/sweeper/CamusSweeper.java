...
public class CamusSweeper extends Configured implements Tool {
  ...
  public Map<FileStatus, String> findAllTopics(Path input, PathFilter filter, String topicSubdir, FileSystem fs) throws IOException {
  ...
  public void run() throws Exception {
    ...
    Map<FileStatus, String> topics = findAllTopics(fromLocationPath, new WhiteBlackListPathFilter(whitelist, blacklist, fileSystem.getFileStatus(fromLocationPath).getPath()), sourceSubdir, fileSystem);
    ...
    if (!errorMessages.isEmpty()) {
      for (SweeperError error : errorMessages) {
        System.err.println("Error occurred in " + error.getTopic() + " at " + error.getInputPath().toString() + " message " + error.getException().getMessage());
  ...
  protected void runCollectorForTopicDir(FileSystem fs, String topic, Path topicSourceDir, Path topicDestDir) throws Exception {
  ...
  protected class KafkaCollector {
    ...
    public void run() throws Exception {
      ...
      ((CamusSweeperJob) Class.forName(props.getProperty("camus.sweeper.io.configurer.class")).newInstance()).setLogger(log).configureJob(topicName, job);
  ...
  public static class WhiteBlackListPathFilter implements PathFilter {
    ...
    @Override public boolean accept(Path path) {
      ...
      return whitelist.matcher(fullName).matches() && !(blacklist.matcher(fullName).matches() || name.startsWith(".") || name.startsWith("_"));
    }
