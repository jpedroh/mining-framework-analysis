...
@SuppressWarnings(value = { "javadoc" }) public abstract class DistanceTest<K extends java.lang.Object> {
  ...
  private static <K extends java.lang.Object> boolean testCoincidence(Distance<K> distance, K a, K b) {
    return distance.distance(a, b) != 0.0f || a.equals(b);
  }

  private static <K extends java.lang.Object> void testNullPointerException(Distance<K> distance, K a, K b) {
    try {
  ...
  private static <K extends java.lang.Object> void testRange(Distance<K> metric, K a, K b) {
    float similarity = metric.distance(a, b);
    String message1 = String.format("Distance %s-%s %f must be non-negative", a, b, similarity);
    assertTrue(message1, 0.0f <= similarity);
  }

  private static <K extends java.lang.Object> void testReflexive(Distance<K> metric, K a, float delta) {
    assertEquals(0.0f, metric.distance(a, a), delta);
  }

  private static <K extends java.lang.Object> void testDistance(Distance<K> metric, K a, K b, float expected, float delta) {
    float similarity = metric.distance(a, b);
    String message = String.format("\"%s\" vs \"%s\"", a, b);
    assertEquals(message, expected, similarity, delta);
  }

  private static <K extends java.lang.Object> boolean testSubadditivity(Distance<K> metric, K a, K b, K c) {
  ...
  private static <K extends java.lang.Object> void testSymmetric(Distance<K> metric, K a, K b, float delta) {
    float distance = metric.distance(a, b);
    float distanceReversed = metric.distance(b, a);
    String message = String.format("Distance relation \"%s\" vs \"%s\" must be symmetric", a, b);
  ...
  @Test public final void distance() {
    for (TestCase<K> t : tests) {
