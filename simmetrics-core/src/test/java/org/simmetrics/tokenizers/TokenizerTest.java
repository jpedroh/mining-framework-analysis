...
@SuppressWarnings(value = { "javadoc" }) public abstract class TokenizerTest {
  ...
  private static void testTokens(String string, Collection<String> expected, Collection<String> actual) {
  ...
  @Test public final void containsEmptyTest() {
    for (T t : tests) {
      if (t.string().isEmpty()) {
        return;
      }
  ...
  @Test public final void shouldTokenizeToList() {
  ...
  @Test public final void tokenizeToListShouldThrowNullPointerException() {
    if (supportsTokenizeToList()) {
  ...
  @Test public final void shouldTokenizeToSet() {
  ...
  @Test public final void tokenizeToSetShouldThrowNullPointerException() {
    if (supportsTokenizeToSet()) {
      thrown.expect(NullPointerException.class);
    } else {
      thrown.expect(UnsupportedOperationException.class);
  ...
  @Test public final void shouldTokenizeToMutiset() {
    if (!supportsTokenizeToMultiset()) {
      thrown.expect(UnsupportedOperationException.class);
    }
    for (T t : tests) {
      testTokens(t.string(), t.tokensAsMultiset(), tokenizer.tokenizeToMultiset(t.string()));
    }
  }

  @Test public final void tokenizeToMultisetShouldThrowNullPointerException() {
    if (supportsTokenizeToMultiset()) {
      thrown.expect(NullPointerException.class);
    } else {
      thrown.expect(UnsupportedOperationException.class);
