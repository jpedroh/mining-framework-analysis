...
public class FurGuardiansGame extends BaseGame {
  ...
  protected final static String VERSION = "1.26.0";
  ...
  private final static List<String> animals = Arrays.asList("Bear", "Cat", "Dog", "Elephant", "Horse", "Koala", "Mouse", "Panda", "Rabbit", "Rhino", "Wolf", "Zebra");
  ...
  private final static List<String> initTips() {
    ...
    list.add("You can switch between full control and auto-run in Menu/Setup");
  ...
  protected final static String[][] ads = { { "Thank you", "for playing" }, { "Thank you", "for downloading" }, { "#gamedev", "#indiedev" }, { "Open", "source" }, { "You", "rock!" }, { "You\'re", "a superstar!" }, { "You\'re", "awesome!" }, { "Like the game?", "Tell a friend" }, { "Like the game?", "Rate it" }, { "Like the game?", "Write a review" }, { "Like the game?", "+1 it" }, { "Pandcorps has no", "marketing team" }, { "The Pandcorps", "ad budget is 0" }, { "You can follow", "@pandcorps" }, { "You can contact", getEmail() }, { "You can visit", "http://pandcorps.org" }, { "Find a bug?", "Tell Pandcorps" }, { "Have a suggestion?", "Tell Pandcorps" }, { "Trying to be", "fun for all" } };
  ...
  @Override protected final boolean isFullScreen() {
    return true;
  }

  @Override protected final void initEarliest() {
    loadConfig();
  }

  @Override protected final void init(final Panroom room) throws Exception {
  ...
  protected final static class PlatformScreen extends Panscreen {
    ...
    @Override protected final void load() throws Exception {
      level = true;
      Player.clearPower();
      loadLevel();
      fadeIn(room);
  ...
  private final static void buildGuy(final Img guy, final Img face, final Img[] tails, final Img eyes, final Img clothing, final Img clothingOverlay, final int y, final int t) {
    if (tails != null) {
  ...
  protected final static void reloadAnimalStrip(final PlayerContext pc, final boolean full) {
    ...
    if (needWing) {
      ...
      if (full && avatar.jumpMode == Player.JUMP_HIGH) {
  ...
  private final static Panimation createNorth(final Img[] maps, final int mi, final Img wing, final Img drgn, final Img[] clothingMap, final Img tailNorth, final Img faceNorth, final String pre, final String suf) {
  ...
  private final static void loadConstants() throws Exception {
    ...
    loaders.add(new Runnable() {
      @Override public final void run() {
        Coltil.set(allEnemies, Level.DROWID, new EnemyDefinition("Drowid", 1, null, true, 1));
      }
    });
    loaders.add(new Runnable() {
      @Override public final void run() {
        ...
        Coltil.set(allEnemies, Level.DROLOCK, drolock);
      }
    });
    loaders.add(new Runnable() {
      @Override public final void run() {
        Coltil.set(allEnemies, Level.HOB_TROLL, new EnemyDefinition("Hob-troll", 2, null, true));
      }
    });
    loaders.add(new Runnable() {
      @Override public final void run() {
        ...
        Coltil.set(allEnemies, Level.HOB_OGRE, new EnemyDefinition("Hob-ogre", 2, f, false));
        ...
        armoredImp.splatHandler = new BurstHandler() {
        ...
        thrownImp.stepHandler = new InteractionHandler() {
          @Override public final boolean onInteract(final Enemy enemy, final Player player) {
            ...
            if (hv == 0) {
              ...
              if (enemy.timer < 8 && Math.abs(hv) == 1) {
                enemy.timer++;
              } else {
                if (hv > 0) {
                  enemy.hv--;
        ...
        final EnemyDefinition blackBlob = new EnemyDefinition("Black Blob", 14, bbf, true, true, Enemy.DEFAULT_X, Enemy.DEFAULT_H);
        blackBlob.splatHandler = new BurstHandler() {
          @Override public final void onBurst(final CustomBurst burst) {
            final Enemy prev = (Enemy) burst.getContext();
            final BurstHandler h = new BurstHandler() {
              @Override public final void onBurst(final CustomBurst b) {
  ...
  protected final static Panimation getGemAnm(final Panmage img) {
    if (Panmage.contains(gemAnm, img)) {
      ...
      if (Panmage.contains(gemBlueAnm, img)) {
        return gemBlueAnm;
      }
    }
    return null;
  ...
  protected final static Panlayer addHud(final Panroom room, final boolean allowAuto, final boolean level) {
    ...
    if (level) {
      initTouchButtons(hud, allowAuto, false, hudGem);
    }
    return hud;
  }
  ...
  protected final static void addHud(final Panlayer hud, final PlayerContext pc, final int x, final int y, final boolean level, final boolean mult) {
    ...
    if (mult) {
      final CallSequence multSeq = new CallSequence() {
        @Override protected String call() {
    ...
    for (final Panctor actor : room.getActors()) {
      if (actor instanceof Wisp || actor instanceof Trio || actor instanceof Leg || actor instanceof Projectile) {
        ...
        if (actor instanceof Enemy) {
          ((Enemy) actor).flipAndFall(Player.VEL_BUMP);
        }
      }
    }
  ...
  @Override public boolean onPause() {
