...
public class AsynchronousMultiThreadedGeneticAlgorithm<S extends Solution<?>> extends Master<ParallelTask<S>, List<S>> {
  ...
  private void createWorkers(int numberOfCores, Problem<S> problem) {
    IntStream.range(0, numberOfCores).forEach((i) -> new Worker<>((task) -> {
      problem.evaluate(task.getContents());
      return ParallelTask.create(createTaskIdentifier(), task.getContents());
    }, pendingTaskQueue, completedTaskQueue).start());
  }
  ...
  @Override public List<ParallelTask<S>> createInitialTasks() {
  ...
  @Override public void submitInitialTasks(List<ParallelTask<S>> initialTaskList) {
  ...
  @Override public void processComputedTask(ParallelTask<S> task) {
    evaluations++;
  ...
  @Override public void submitTask(ParallelTask<S> task) {
    pendingTaskQueue.add(task);
  }

  @Override public ParallelTask<S> createNewTask() {
  ...
  @Override public boolean stoppingConditionIsNotMet() {
