...
public class JavaParserInterfaceDeclaration extends AbstractTypeDeclaration implements ResolvedInterfaceDeclaration, MethodResolutionCapability, MethodUsageResolutionCapability, SymbolResolutionCapability {
  ...
  @Override public List<ResolvedFieldDeclaration> getAllFields() {
    ...
    getAncestors().stream().filter((ancestor) -> ancestor.getTypeDeclaration().isPresent()).forEach((ancestor) -> ancestor.getTypeDeclaration().get().getAllFields().forEach((f) -> {
      fields.add(new ResolvedFieldDeclaration() {
        ...
        @Override public ResolvedTypeDeclaration declaringType() {
