...
public class JavassistMethodDeclaration implements ResolvedMethodDeclaration, TypeVariableResolutionCapability {
  ...
  @Override public ResolvedReferenceTypeDeclaration declaringType() {
    if (ctMethod.getDeclaringClass().isInterface()) {
      ...
      if (ctMethod.getDeclaringClass().isEnum()) {
  ...
  @Override public ResolvedParameterDeclaration getParam(int i) {
    try {
      ...
      if (ctMethod.getGenericSignature() != null) {
        ...
        return new JavassistParameterDeclaration(JavassistUtils.signatureTypeToType(signatureType, typeSolver, this), typeSolver, variadic, paramName.orElse(null));
      } else {
        return new JavassistParameterDeclaration(ctMethod.getParameterTypes()[i], typeSolver, variadic, paramName.orElse(null));
      }
    } catch (NotFoundException e) {
      throw new RuntimeException(e);
  ...
  @Override public Optional<MethodDeclaration> toAst() {
