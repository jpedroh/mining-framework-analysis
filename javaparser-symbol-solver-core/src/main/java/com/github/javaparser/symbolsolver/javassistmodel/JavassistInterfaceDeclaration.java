 ...
 */
public class JavassistInterfaceDeclaration extends AbstractTypeDeclaration implements ResolvedInterfaceDeclaration, MethodResolutionCapability, MethodUsageResolutionCapability {
  ...
  @Override public List<ResolvedReferenceType> getInterfacesExtended() {
    return Arrays.stream(ctClass.getClassFile().getInterfaces()).map((i) -> typeSolver.solveType(i)).map((i) -> new ReferenceTypeImpl(i, typeSolver)).collect(Collectors.toList());
  }
