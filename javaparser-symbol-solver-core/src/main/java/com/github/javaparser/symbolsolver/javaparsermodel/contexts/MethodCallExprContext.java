...
public class MethodCallExprContext extends AbstractJavaParserContext<MethodCallExpr> {
  ...
  @Override public Optional<MethodUsage> solveMethodAsUsage(String name, List<ResolvedType> argumentsTypes) {
    if (wrappedNode.getScope().isPresent()) {
  ...
  private MethodUsage resolveMethodTypeParameters(MethodUsage methodUsage, List<ResolvedType> actualParamTypes) {
    ...
    if (methodUsage.getDeclaration().hasVariadicParameter()) {
      if (actualParamTypes.size() == methodUsage.getDeclaration().getNumberOfParams()) {
        ...
        ResolvedType actualType = actualParamTypes.get(actualParamTypes.size() - 1).isArray() ? actualParamTypes.get(actualParamTypes.size() - 1).asArrayType().getComponentType() : actualParamTypes.get(actualParamTypes.size() - 1);
  ...
  private Optional<MethodUsage> solveMethodAsUsage(ResolvedTypeVariable tp, String name, List<ResolvedType> argumentsTypes, Context invokationContext) {
    ...
    if (bounds.isEmpty()) {
      bounds = Collections.singletonList(ResolvedTypeParameterDeclaration.Bound.extendsBound(JavaParserFacade.get(typeSolver).classToResolvedType(Object.class)));
    }
