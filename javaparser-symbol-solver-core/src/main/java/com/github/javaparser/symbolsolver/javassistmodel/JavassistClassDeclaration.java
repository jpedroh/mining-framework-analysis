...
public class JavassistClassDeclaration extends AbstractClassDeclaration implements MethodUsageResolutionCapability {
  ...
  @Override @Deprecated public SymbolReference<ResolvedMethodDeclaration> solveMethod(String name, List<ResolvedType> argumentsTypes, boolean staticOnly) {
    ...
    SymbolReference<ResolvedMethodDeclaration> superClassMethodRef = MethodResolutionLogic.solveMethodInType(getSuperClass().getTypeDeclaration(), name, argumentsTypes, staticOnly);
    ...
    for (ResolvedReferenceType interfaceRef : getInterfaces()) {
      SymbolReference<ResolvedMethodDeclaration> interfaceMethodRef = MethodResolutionLogic.solveMethodInType(interfaceRef.getTypeDeclaration(), name, argumentsTypes, staticOnly);
  ...
  @Override public boolean isAssignableBy(ResolvedType type) {
    ...
    try {
      if (this.ctClass.getSuperclass() != null && new JavassistClassDeclaration(this.ctClass.getSuperclass(), typeSolver).isAssignableBy(type)) {
        return true;
      }
  ...
  @Override public List<ResolvedReferenceType> getInterfaces() {
    try {
      if (ctClass.getGenericSignature() == null) {
        return Arrays.stream(ctClass.getClassFile().getInterfaces()).map((i) -> typeSolver.solveType(JavassistUtils.internalNameToCanonicalName(i))).map((i) -> new ReferenceTypeImpl(i, typeSolver)).collect(Collectors.toList());
      } else {
        SignatureAttribute.ClassSignature classSignature = SignatureAttribute.toClassSignature(ctClass.getGenericSignature());
        return Arrays.stream(classSignature.getInterfaces()).map((i) -> JavassistUtils.signatureTypeToType(i, typeSolver, this).asReferenceType()).collect(Collectors.toList());
      }
    } catch (BadBytecode e) {
      throw new RuntimeException(e);
    }
  ...
  @Override public com.github.javaparser.ast.Modifier.Keyword accessSpecifier() {
