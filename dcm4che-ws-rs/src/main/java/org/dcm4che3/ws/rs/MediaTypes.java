...
public class MediaTypes {
  ...
  public static String sopClassOf(MediaType bulkdataMediaType) {
    String type = bulkdataMediaType.getType().toLowerCase();
    return type.equals("image") ? UID.SecondaryCaptureImageStorage : type.equals("video") ? UID.VideoPhotographicImageStorage : equalsIgnoreParameters(bulkdataMediaType, APPLICATION_PDF_TYPE) ? UID.EncapsulatedPDFStorage : equalsIgnoreParameters(bulkdataMediaType, MediaType.APPLICATION_XML_TYPE) ? UID.EncapsulatedCDAStorage : isSTLType(bulkdataMediaType) ? UID.EncapsulatedSTLStorage : equalsIgnoreParameters(bulkdataMediaType, MODEL_OBJ_TYPE) ? UID.EncapsulatedOBJStorage : equalsIgnoreParameters(bulkdataMediaType, MODEL_MTL_TYPE) ? UID.EncapsulatedMTLStorage : null;
  }

  public static boolean isSTLType(MediaType mediaType) {
    return equalsIgnoreParameters(mediaType, MODEL_STL_TYPE) || equalsIgnoreParameters(mediaType, MODEL_X_STL_BINARY_TYPE) || equalsIgnoreParameters(mediaType, APPLICATION_SLA_TYPE);
  }
  ...
  public static String getTransferSyntax(MediaType type) {
    return type != null && equalsIgnoreParameters(APPLICATION_DICOM_TYPE, type) ? type.getParameters().get("transfer-syntax") : null;
  }
