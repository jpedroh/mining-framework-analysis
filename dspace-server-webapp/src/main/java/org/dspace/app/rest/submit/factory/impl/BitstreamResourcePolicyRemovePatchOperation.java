...
public class BitstreamResourcePolicyRemovePatchOperation extends RemovePatchOperation<AccessConditionDTO> {
  ...
  @Override void remove(Context context, HttpServletRequest currentRequest, InProgressSubmission source, String path, Object value) throws Exception {
    ...
    for (Bundle bb : bundle) {
      ...
      for (Bitstream b : bb.getBitstreams()) {
        if (idx == Integer.parseInt(bitstreamIdx)) {
          if (split.length == 3) {
            resourcePolicyService.removePolicies(context, b, ResourcePolicy.TYPE_CUSTOM);
          } else {
