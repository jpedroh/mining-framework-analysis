...
@Component public class ResourcePolicyAdminPermissionEvalutatorPlugin extends RestObjectPermissionEvaluatorPlugin {
  ...
  @Override public boolean hasDSpacePermission(Authentication authentication, Serializable targetId, String targetType, DSpaceRestPermission permission) {
    ...
    if (!DSpaceRestPermission.ADMIN.equals(restPermission) || !StringUtils.equalsIgnoreCase(targetType, RESOURCE_POLICY_PATCH)) {
      return false;
    }
    ...
    try {
      ...
      if (resourcePolicy == null) {
        throw new ResourceNotFoundException(ResourcePolicyRest.CATEGORY + "." + ResourcePolicyRest.NAME + " with id: " + resourcePolicyID + " not found");
      }
