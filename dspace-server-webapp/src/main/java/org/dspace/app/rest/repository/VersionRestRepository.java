 ...
 */
@Component(value = VersionRest.CATEGORY + "." + VersionRest.NAME) public class VersionRestRepository extends DSpaceRestRepository<VersionRest, Integer> implements ReloadableEntityObjectRepository<Version, Integer> {
  ...
  @Autowired private UriListHandlerService uriListHandlerService;

  @Autowired private WorkspaceItemService workspaceItemService;

  @SuppressWarnings(value = { "rawtypes" }) @Autowired(required = true) protected WorkflowItemService workflowItemService;

  @Override @PreAuthorize(value = "@versioningSecurity.isEnableVersioning() && hasPermission(#id, \'VERSION\', \'READ\')") public VersionRest findOne(Context context, Integer id) {
    try {
  ...
  @Override @PreAuthorize(value = "@versioningSecurity.isEnableVersioning() && hasAuthority(\'AUTHENTICATED\')") protected VersionRest createAndReturn(Context context, List<String> stringList) throws AuthorizeException, SQLException, RepositoryMethodNotImplementedException {
    ...
    if (hasEntityType && isBlockEntity) {
      throw new AuthorizeException("You are trying to create a new version for an entity" + " which is blocked by the configuration");
    }
    ...
    Version version = StringUtils.isNotBlank(summary) ? versioningService.createNewVersion(context, item, summary) : versioningService.createNewVersion(context, item);
    return converter.toRest(version, utils.obtainProjection());
  }

  @Override @PreAuthorize(value = "@versioningSecurity.isEnableVersioning() && hasPermission(#versionId, \'version\', \'ADMIN\')") protected void patch(Context context, HttpServletRequest request, String apiCategory, String model, Integer versionId, Patch patch) throws AuthorizeException, SQLException {
  ...
  @Override public Page<VersionRest> findAll(Context context, Pageable pageable) {
    throw new RepositoryMethodNotImplementedException("No implementation found; Method not allowed!", "");
  }

  @Override public Class<VersionRest> getDomainClass() {
    return VersionRest.class;
  }

  @Override public Version findDomainObjectByPk(Context context, Integer id) throws SQLException {
    return versioningService.getVersion(context, id);
  }

  @Override public Class<Integer> getPKClass() {
