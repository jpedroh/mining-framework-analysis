 ...
 */
@Component public class EPersonRestAuthenticationProvider implements AuthenticationProvider {
  ...
  @Autowired(required = false) private List<PostLoggedInAction> postLoggedInActions;
      ...
      try {
        ...
        if (implicitStatus == AuthenticationMethod.SUCCESS) {
          ...
          if (AuthenticationMethod.SUCCESS == authenticateResult) {
            log.info(LogHelper.getHeader(newContext, "login", "type=explicit"));
            output = createAuthentication(newContext);
            ...
            log.info(LogHelper.getHeader(newContext, "failed_login", "email=" + name + ", result=" + authenticateResult));
  ...
  public List<GrantedAuthority> getGrantedAuthorities(Context context) {
    ...
    if (eperson != null) {
      ...
      if (isAdmin) {
        ...
        if (authorizeService.isAccountManager(context)) {
