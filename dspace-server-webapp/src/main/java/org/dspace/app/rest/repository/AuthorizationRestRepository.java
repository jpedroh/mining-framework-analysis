 ...
 */
@Component(value = AuthorizationRest.CATEGORY + "." + AuthorizationRest.NAME) public class AuthorizationRestRepository extends DSpaceRestRepository<AuthorizationRest, String> {
  ...
  @Autowired private AuthorizationRestUtil authorizationRestUtil;
  ...
  @Override @PreAuthorize(value = "hasPermission(#id, \'authorization\', \'READ\')") public AuthorizationRest findOne(Context context, String id) {
  ...
  /**
     ...
     * @throws SQLException
     */
  @PreAuthorize(value = "#epersonUuid==null || hasPermission(#epersonUuid, \'EPERSON\', \'READ\')") @SearchRestMethod(name = "object") public Page<AuthorizationRest> findByObject(@Parameter(value = "uri", required = true) String uri, @Parameter(value = "eperson") UUID epersonUuid, @Parameter(value = "feature") String featureName, Pageable pageable) throws AuthorizeException, SQLException {
  ...
  @PreAuthorize(value = "#epersonUuid==null || hasPermission(#epersonUuid, \'EPERSON\', \'READ\')") @SearchRestMethod(name = "objects") public Page<AuthorizationRest> findByObjects(@Parameter(value = "uuid", required = true) List<String> uuidList, @Parameter(value = "type", required = true) String type, @Parameter(value = "eperson") UUID epersonUuid, @Parameter(value = "feature") List<String> featureNames, Pageable pageable) throws AuthorizeException, SQLException {
    Context context = obtainContext();
    EPerson currUser = context.getCurrentUser();
    EPerson user = getUserFromRequestParameter(context, epersonUuid);
    if (ObjectUtils.notEqual(currUser, user)) {
    ...
    List<Authorization> authorizations = findAuthorizationsByUUIDList(context, type, uuidList, user, featureNames);
    if (ObjectUtils.notEqual(currUser, user)) {
      context.restoreContextUser();
    }
    return converter.toRestPage(authorizations, 
...
>>>>>>> /usr/src/app/output/dspace/dspace/c65314db9d4f1df5539b4785a5b234ee3ab8a2a5/dspace-server-webapp/src/main/java/org/dspace/app/rest/repository/AuthorizationRestRepository.java/right.java
  ...
  private List<Authorization> findAuthorizationsByUUIDList(Context context, String type, List<String> uuidList, EPerson user, List<String> featureNames) {
    if (featureNames.isEmpty()) {
      return new ArrayList<>();
    }
    List<Authorization> authorizations = new ArrayList<>();
    List<BaseObjectRest> objects = uuidList.stream().map((uuid) -> utils.getBaseObjectRestFromTypeAndUUID(context, type, uuid)).collect(Collectors.toList());
    objects.forEach((object) -> featureNames.forEach((featureName) -> {
      try {
  ...
  private List<Authorization> findAuthorizationsForUri(Context context, EPerson user, String uri, String featureName) throws SQLException {
  ...
  private List<Authorization> authorizationsForObject(Context context, EPerson user, String featureName, BaseObjectRest obj) throws SQLException {
    if (obj == null) {
      return new ArrayList<>();
    }
  ...
  private List<Authorization> findByObjectAndFeature(Context context, EPerson user, BaseObjectRest obj, String featureName) throws SQLException {
  ...
  @Override public Class<AuthorizationRest> getDomainClass() {
    return AuthorizationRest.class;
  }

  @Override public Page<AuthorizationRest> findAll(Context context, Pageable pageable) {
