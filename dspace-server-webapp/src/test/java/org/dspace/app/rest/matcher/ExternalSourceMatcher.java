...
public class ExternalSourceMatcher {
  ...
  public static Matcher<? super Object> matchExternalSource(String id, String name, boolean hierarchical) {
    return allOf(hasJsonPath("$.id", is(id)), hasJsonPath("$.name", is(name)), hasJsonPath("$.hierarchical", is(hierarchical)), hasJsonPath("$._links.entries.href", is(REST_SERVER_URL + "integration/externalsources/" + name + "/entries")));
  }
}