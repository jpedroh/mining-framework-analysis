...
public class DeleteEPersonSubmitterIT extends AbstractControllerIntegrationTest {
  ...
  protected RequestItemAuthorExtractor requestItemAuthorExtractor = DSpaceServicesFactory.getInstance().getServiceManager().getServiceByName("org.dspace.app.requestitem.RequestItemAuthorExtractor", RequestItemAuthorExtractor.class);
  ...
  @Before @Override public void setUp() throws Exception {
    ...
    submitterForVersion1 = EPersonBuilder.createEPerson(context).withEmail("submitterForVersion1@example.org").build();
    submitterForVersion2 = EPersonBuilder.createEPerson(context).withEmail("submitterForVersion2@example.org").build();
  ...
  /**
     ...
     * @throws Exception
     */
  @Test public void testArchivedItemSubmitterDelete() throws Exception {
    context.turnOffAuthorisationSystem();
    Community parent = CommunityBuilder.createCommunity(context).build();
    Collection collection = CollectionBuilder.createCollection(context, parent).build();
    WorkspaceItem wsi = WorkspaceItemBuilder.createWorkspaceItem(context, collection).withSubmitter(submitter).withTitle("Test Item").withIssueDate("2019-03-06").withSubject("ExtraEntry").build();
  ...
  /**
     ...
     * @throws Exception
     */
  @Test public void testWIthdrawnItemSubmitterDelete() throws Exception {
    context.turnOffAuthorisationSystem();
    Community parent = CommunityBuilder.createCommunity(context).build();
    Collection collection = CollectionBuilder.createCollection(context, parent).build();
    WorkspaceItem wsi = WorkspaceItemBuilder.createWorkspaceItem(context, collection).withSubmitter(submitter).withTitle("Test Item").withIssueDate("2019-03-06").withSubject("ExtraEntry").build();
    ...
    getClient(token).perform(patch("/api/core/items/" + item.getID()).content(patchBodyToReinstate).contentType(MediaType.APPLICATION_JSON_PATCH_JSON)).andExpect(status().isOk()).andExpect(jsonPath("$.uuid", Matchers.is(item.getID().toString()))).andExpect(jsonPath("$.withdrawn", Matchers.is(false))).andExpect(jsonPath("$.inArchive", Matchers.is(true)));
    assertNull(retrieveItemSubmitter(item.getID()));
    getClient(token).perform(patch("/api/core/items/" + item.getID()).content(patchBodyToWithdraw).contentType(MediaType.APPLICATION_JSON_PATCH_JSON)).andExpect(status().isOk()).andExpect(jsonPath("$.uuid", Matchers.is(item.getID().toString()))).andExpect(jsonPath("$.withdrawn", Matchers.is(true))).andExpect(jsonPath("$.inArchive", Matchers.is(false)));
    assertNull(retrieveItemSubmitter(item.getID()));
  }

  @Test public void testVersionItemSubmitterDelete() throws Exception {
    String token = getAuthToken(admin.getEmail(), password);
    context.turnOffAuthorisationSystem();
    Community parent = CommunityBuilder.createCommunity(context).build();
    Collection collection = CollectionBuilder.createCollection(context, parent).build();
  ...
  @Test public void testWorkspaceItemSubmitterDelete() throws Exception {
    context.turnOffAuthorisationSystem();
    Community parent = CommunityBuilder.createCommunity(context).build();
    Collection collection = CollectionBuilder.createCollection(context, parent).build();
    WorkspaceItem wsi = WorkspaceItemBuilder.createWorkspaceItem(context, collection).withSubmitter(submitter).withTitle("Test Item").withIssueDate("2019-03-06").withSubject("ExtraEntry").build();
    String token = getAuthToken(admin.getEmail(), password);
    getClient(token).perform(get("/api/submission/workspaceitems/" + wsi.getID())).andExpect(status().isOk());
    assertDeletionOfEperson(submitter);
    getClient(token).perform(get("/api/submission/workspaceitems/" + wsi.getID())).andExpect(status().isNotFound());
  }

  @Test public void testWorkflowItemSubmitterDelete() throws Exception {
    context.turnOffAuthorisationSystem();
    Community parent = CommunityBuilder.createCommunity(context).build();
    Collection collection = CollectionBuilder.createCollection(context, parent).withWorkflowGroup(1, workflowUser).build();
    XmlWorkflowItem workflowItem = WorkflowItemBuilder.createWorkflowItem(context, collection).withSubmitter(submitter).withTitle("Test Item").withIssueDate("2019-03-06").withSubject("ExtraEntry").build();
    String token = getAuthToken(admin.getEmail(), password);
    getClient(token).perform(get("/api/workflow/workflowitems/" + workflowItem.getID())).andExpect(status().isOk());
    assertDeletionOfEperson(submitter);
    getClient(token).perform(get("/api/workflow/workflowitems/" + workflowItem.getID())).andExpect(status().isOk());
  }
  ...
  private void assertDeletionOfEperson(EPerson ePerson) throws SQLException {
    try {
      String token = getAuthToken(admin.getEmail(), password);
      getClient(token).perform(delete("/api/eperson/epersons/" + ePerson.getID())).andExpect(status().isNoContent());
    } catch (Exception ex) {
  ...
  private Item retrieveVersionItem(int id) throws Exception {
    ...
    String token = getAuthToken(admin.getEmail(), password);
    getClient(token).perform(get("/api/versioning/versions/" + id + "/item")).andExpect(status().isOk()).andDo((result) -> idRef.set(read(result.getResponse().getContentAsString(), "$.uuid")));
