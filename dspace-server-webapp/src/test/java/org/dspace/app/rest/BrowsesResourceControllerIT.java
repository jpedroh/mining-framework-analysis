...
public class BrowsesResourceControllerIT extends AbstractControllerIntegrationTest {
  ...
  @Test public void findBrowseBySubjectEntries() throws Exception {
    ...
    Item withdrawnItem1 = ItemBuilder.createItem(context, col2).withTitle("Withdrawn item 1").withIssueDate("2016-02-13").withAuthor("Smith, Maria").withAuthor("Doe, Jane").withSubject("AnotherTest").withSubject("TestingForMore").withSubject("ExtraEntry").withSubject("WithdrawnEntry").withdrawn().build();
    Item privateItem1 = ItemBuilder.createItem(context, col2).withTitle("Private item 1").withIssueDate("2016-02-13").withAuthor("Smith, Maria").withAuthor("Doe, Jane").withSubject("AnotherTest").withSubject("TestingForMore").withSubject("ExtraEntry").withSubject("PrivateEntry").makeUnDiscoverable().build();
  ...
  @Test public void findBrowseBySubjectItems() throws Exception {
    ...
    getClient().perform(get("/api/discover/browses/subject/items").param("filterValue", "PrivateEntry")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.totalElements", is(0))).andExpect(jsonPath("$.page.size", is(20)));
    getClient().perform(get("/api/discover/browses/subject/items").param("filterValue", "WithdrawnEntry")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.totalElements", is(0))).andExpect(jsonPath("$.page.size", is(20)));
  }

  @Test public void findBrowseBySubjectItemsWithScope() throws Exception {
    context.turnOffAuthorisationSystem();
    parentCommunity = CommunityBuilder.createCommunity(context).withName("Parent Community").build();
    Community child1 = CommunityBuilder.createSubCommunity(context, parentCommunity).withName("Sub Community").build();
    Collection col1 = CollectionBuilder.createCollection(context, child1).withName("Collection 1").build();
    ...
    Item privateItem1 = ItemBuilder.createItem(context, col2).withTitle("Private item 1").withIssueDate("2016-02-13").withAuthor("Smith, Maria").withAuthor("Doe, Jane").withSubject("AnotherTest").withSubject("TestingForMore").withSubject("ExtraEntry").withSubject("PrivateEntry").makeUnDiscoverable().build();
    context.restoreAuthSystemState();
    getClient().perform(get("/api/discover/browses/subject/items").param("scope", String.valueOf(col2.getID())).param("filterValue", "ExtraEntry")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.totalElements", is(0))).andExpect(jsonPath("$.page.size", is(20)));
    getClient().perform(get("/api/discover/browses/subject/items").param("scope", String.valueOf(col2.getID())).param("filterValue", "AnotherTest")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.totalElements", is(2))).andExpect(jsonPath("$.page.size", is(20))).andExpect(jsonPath("$._embedded.items", contains(ItemMatcher.matchItemWithTitleAndDateIssued(publicItem2, "Public item 2", "2016-02-13"), ItemMatcher.matchItemWithTitleAndDateIssued(publicItem3, "Public item 3", "2016-02-14"))));
    getClient().perform(get("/api/discover/browses/subject/items").param("scope", String.valueOf(col2.getID())).param("filterValue", "PrivateEntry")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.totalElements", is(0))).andExpect(jsonPath("$.page.size", is(20)));
    getClient().perform(get("/api/discover/browses/subject/items").param("scope", String.valueOf(col2.getID())).param("filterValue", "WithdrawnEntry")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.totalElements", is(0))).andExpect(jsonPath("$.page.size", is(20)));
  }
  ...
  @Test public void findBrowseByTitleItemsWithScope() throws Exception {
    context.turnOffAuthorisationSystem();
    parentCommunity = CommunityBuilder.createCommunity(context).withName("Parent Community").build();
    Community child1 = CommunityBuilder.createSubCommunity(context, parentCommunity).withName("Sub Community").build();
    Collection col1 = CollectionBuilder.createCollection(context, child1).withName("Collection 1").build();
    ...
    Item internalItem = ItemBuilder.createItem(context, col2).withTitle("Internal publication").withIssueDate("2016-09-19").withAuthor("Doe, John").withSubject("Unknown").withReaderGroup(internalGroup).build();
    context.restoreAuthSystemState();
    getClient().perform(get("/api/discover/browses/title/items").param("scope", String.valueOf(col2.getID())).param("sort", "title,desc")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.size", is(20))).andExpect(jsonPath("$.page.totalElements", is(1))).andExpect(jsonPath("$.page.totalPages", is(1))).andExpect(jsonPath("$.page.number", is(0))).andExpect(jsonPath("$._embedded.items", contains(ItemMatcher.matchItemWithTitleAndDateIssued(publicItem2, "Public item 2", "2016-02-13")))).andExpect(jsonPath("$._embedded.items[*].metadata", Matchers.allOf(not(matchMetadata("dc.title", "This is a private item")), not(matchMetadata("dc.title", "Internal publication")))));
  }
  ...
  @Test public void testPaginationBrowseByDateIssuedItems() throws Exception {
    ...
    Item withdrawnItem1 = ItemBuilder.createItem(context, col2).withTitle("Withdrawn item 1").withIssueDate("2016-02-13").withdrawn().build();
    Item privateItem1 = ItemBuilder.createItem(context, col2).withTitle("Private item 1").makeUnDiscoverable().build();
    context.restoreAuthSystemState();
  ...
  @Test public void testPaginationBrowseByDateIssuedItemsWithScope() throws Exception {
    context.turnOffAuthorisationSystem();
    parentCommunity = CommunityBuilder.createCommunity(context).withName("Parent Community").build();
    Community child1 = CommunityBuilder.createSubCommunity(context, parentCommunity).withName("Sub Community").build();
    Collection col1 = CollectionBuilder.createCollection(context, child1).withName("Collection 1").build();
    ...
    Item privateItem1 = ItemBuilder.createItem(context, col2).withTitle("Private item 1").makeUnDiscoverable().build();
    context.restoreAuthSystemState();
    getClient().perform(get("/api/discover/browses/dateissued/items").param("scope", String.valueOf(col2.getID())).param("sort", "title,asc").param("size", "5")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.size", is(5))).andExpect(jsonPath("$.page.totalElements", is(3))).andExpect(jsonPath("$.page.totalPages", is(1))).andExpect(jsonPath("$.page.number", is(0))).andExpect(jsonPath("$._embedded.items", contains(ItemMatcher.matchItemWithTitleAndDateIssued(item2, "Item 2", "2016-02-13"), ItemMatcher.matchItemWithTitleAndDateIssued(item4, "Item 4", "2016-02-11"), ItemMatcher.matchItemWithTitleAndDateIssued(item6, "Item 6", "2016-01-13"))));
  }
  ...
  @Test public void testBrowseByItemsStartsWith() throws Exception {
    ...
    getClient().perform(get("/api/discover/browses/title/items?startsWith=Blade Runner").param("scope", col1.getID().toString()).param("size", "2")).andExpect(status().isOk()).andExpect(content().contentType(contentType)).andExpect(jsonPath("$.page.totalElements", is(1))).andExpect(jsonPath("$.page.number", is(0))).andExpect(jsonPath("$._links.self.href", containsString("startsWith=Blade Runner"))).andExpect(jsonPath("$._embedded.items", contains(ItemMatcher.matchItemWithTitleAndDateIssued(item2, "Blade Runner", "1982-06-25"))));
  }
