 ...
 */
@SuppressWarnings(value = { "serial" }) public class RankingAndCrowdingSelection<S extends Solution<?>> implements SelectionOperator<List<S>, List<S>> {
  ...
  public List<S> execute(List<S> solutionList) throws JMetalException {
    if (null == solutionList) {
      ...
      if (solutionList.isEmpty()) {
        ...
        if (solutionList.size() < solutionsToSelect) {
          throw new JMetalException("The population size (" + solutionList.size() + ") is smaller than" + "the solutions to selected (" + solutionsToSelect + ")");
        }
      }
    }
