 ...
 */
@SuppressWarnings(value = { "serial" }) public class MergeSortNonDominatedSortRanking<S extends Solution<?>> implements Ranking<S> {
  ...
  @Override public Ranking<S> computeRanking(List<S> solutionSet) {
    ...
    SORT_INDEX = _m + 1;
    SOL_ID = _m;
    _work = new double[_n][SORT_INDEX + 1];
    _population = new double[_n][_m + 2];
    for (int i = 0; i < _n; i++) {
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private boolean merge_sort(double[][] src, double[][] dest, int low, int high, int obj, int toObj) {
    ...
    for (s = low, i = low, j = mid; s < high; s++) {
      if (j >= high) {
        ...
        if (i < mid && compare_lex(src[i], src[j], obj, toObj) <= 0) {
          dest[s] = src[i++];
        } else {
          dest[s] = src[j++];
        }
  ...
  private void sortRestOfObjectives() {
    ...
    for (int obj = 2; obj < _m; obj++) {
      if (merge_sort(_population, _work, 0, _n, obj, obj + 1)) {
        if (obj == lastObjective) {
          for (p = 0; p < _n; p++) {
            bsManager.computeSolutionRanking((int) _population[p][SORT_INDEX], (int) _population[p][SOL_ID]);
          }
        }
        continue;
      }
  ...
  public final int[] sort(double[][] populationData) {
    ...
    for (int[] duplicated : _duplicatedSolutions) {
      _ranking[duplicated[1]] = _ranking[duplicated[0]];
    }
    _n = _initialPopulationSize;
    return _ranking;
  }
  ...
  @Override public Comparator<S> getSolutionComparator() {
