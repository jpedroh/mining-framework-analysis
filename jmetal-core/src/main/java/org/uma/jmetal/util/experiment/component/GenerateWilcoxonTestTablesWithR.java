...
public class GenerateWilcoxonTestTablesWithR<Result> implements ExperimentComponent {
  ...
  private void printHeaderLatexCommands(String rFileName, String latexFileName) throws IOException {
    try (final FileWriter os = new FileWriter(rFileName, false)) {
      ...
      output = ((((((((((((((((((((((((("latexHeader <- function() {" + ("\n" + "  write(\"\\\\documentclass{article}\", \"")) + latexFileName) + APPEND_STRING) + "  write(\"\\\\title{StandardStudy}\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\usepackage{amssymb}\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\author{A.J.Nebro}\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\begin{document}\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\maketitle\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\section{Tables}\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\\", \"") + latexFileName) + APPEND_STRING) + "}") + "\n";
      os.write(output + "\n");
    }
  }

  private void printEndLatexCommands(String rFileName, String latexFileName) throws IOException {
    try (final FileWriter os = new FileWriter(rFileName, true)) {
  ...
  private void printTableHeader(GenericIndicator<?> indicator, String rFileName, String latexFileName) throws IOException {
    ...
    // Generate function latexTableHeader()
      String output = ((((((((((((((((((((((((((((("latexTableHeader <- function(problem, tabularString, latexTableFirstLine) {" + ("\n" + "  write(\"\\\\begin{table}\", \"")) + latexFileName) + APPEND_STRING) + latexTableCaption) + "\n") + latexTableLabel) + "\n") + "  write(\"\\\\centering\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\begin{scriptsize}\", \"") + latexFileName) + APPEND_STRING) + // "  write(\"\\\\begin{tabular}{" + latexTabularAlignment + "}\", \"" + texFile + "\", append=TRUE)" + "\n" +
      "  write(\"\\\\begin{tabular}{\", \"") + latexFileName) + APPEND_STRING) + "  write(tabularString, \"") + latexFileName) + APPEND_STRING) + "  write(\"}\", \"") + latexFileName) + APPEND_STRING) + // latexTableFirstLine +
      "  write(latexTableFirstLine, \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\hline \", \"") + latexFileName) + APPEND_STRING) + "}") + "\n";
      os.write(output + "\n");
    }
  }
  ...
  private void printTableTail(String rFileName, String latexFileName) throws IOException {
    ...
    try (final FileWriter os = new FileWriter(rFileName, true)) {
      String output = ((((((((((((("latexTableTail <- function() { " + ("\n" + "  write(\"\\\\hline\", \"")) + latexFileName) + APPEND_STRING) + "  write(\"\\\\end{tabular}\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\end{scriptsize}\", \"") + latexFileName) + APPEND_STRING) + "  write(\"\\\\end{table}\", \"") + latexFileName) + APPEND_STRING) + "}") + "\n";
      os.write(output + "\n");
    }
  }
  ...
  private void printGenerateMainScript(GenericIndicator<?> indicator, String rFileName, String latexFileName) throws IOException {
    try (final FileWriter os = new FileWriter(rFileName, true)) {
      ...
      os.write((output + "\n") + "\n");
      output = ((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((("# Step 3. Problem loop " + ("\n" + "latexTableHeader(\"")) + problemList) + "\", tabularString, latexTableFirstLine)") + "\n\n") + "indx = 0") + "\n") + "for (i in algorithmList) {") + "\n") + "  if (i != \"") + experiment.getAlgorithmList().get(experiment.getAlgorithmList().size() - 1).getAlgorithmTag()) + END_STATEMENT) + "    write(i , \"") + latexFileName) + APPEND_STRING) + "    write(\" & \", \"") + latexFileName) + APPEND_STRING) + "\n") + "    jndx = 0") + "\n") + "    for (j in algorithmList) {") + "\n") + "      for (problem in problemList) {") + "\n") + "        if (jndx != 0) {") + "\n") + "          if (i != j) {") + "\n") + "            printTableLine(indicator, i, j, indx, jndx, problem)") + "\n") + "          }") + "\n") + "          else {") + "\n") + "            write(\"  \", \"") + latexFileName) + APPEND_STRING) + "          } ") + "\n") + "          if (problem == \"") + experiment.getProblemList().get(experiment.getProblemList().size() - 1).getTag()) + END_STATEMENT) + "            if (j == \"") + experiment.getAlgorithmList().get(experiment.getAlgorithmList().size() - 1).getAlgorithmTag()) + END_STATEMENT) + "              write(\" \\\\\\\\ \", \"") + latexFileName) + APPEND_STRING) + "            } ") + "\n") + "            else {") + "\n") + "              write(\" & \", \"") + latexFileName) + APPEND_STRING) + "            }") + "\n") + "          }") + "\n") + "     else {") + "\n") + "    write("&", "") + latexFileName) + APPEND_STRING) + "     }") + "\n") + "        }") + "\n") + "      }") + "\n") + "      jndx = jndx + 1") + "\n") + END_JUMP_STRING) + "    indx = indx + 1") + "\n") + "  }") + "\n") + "} # for algorithm") + "\n") + "\n") + "  latexTableTail()") + "\n";
      os.write(output + "\n");
