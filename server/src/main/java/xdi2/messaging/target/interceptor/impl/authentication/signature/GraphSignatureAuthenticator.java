...
public class GraphSignatureAuthenticator extends PublicKeySignatureAuthenticator {
  ...
  @Override public void init(MessagingTarget messagingTarget, AuthenticationSignatureInterceptor authenticationSignatureInterceptor) throws Exception {
    super.init(messagingTarget, authenticationSignatureInterceptor);
    if (this.getSignatureGraph() == null && messagingTarget instanceof GraphMessagingTarget) {
      this.setSignatureGraph(((GraphMessagingTarget) messagingTarget).getGraph());
    }
    if (this.getSignatureGraph() == null) {
      throw new Xdi2MessagingException("No secret token graph.", null, null);
    }
  }

  @Override public PublicKey getPublicKey(Message message) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Sender peer root: " + senderPeerRoot);
    }
    ...
    try {
      ...
      if (log.isWarnEnabled()) {
        log.warn("Invalid RSA public key " + publicKeyString + ": " + ex.getMessage(), ex);
      }
      return null;
    }
    return publicKey;
