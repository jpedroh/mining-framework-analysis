...
public class PromotionResultsFragmentController extends BaseFormXmlController {
  ...
  public PromotionResultsFragmentController(SBuildServer buildServer, DeployableArtifactoryServers deployableServers) {
    this.buildServer = buildServer;
    this.deployableServers = deployableServers;
  }

  @Override protected ModelAndView doGet(@NotNull HttpServletRequest request, @NotNull HttpServletResponse response) {
    return null;
  }

  @Override protected void doPost(@NotNull HttpServletRequest request, @NotNull HttpServletResponse response, @NotNull Element xmlResponse) {
    ...
    try (ArtifactoryManager artifactoryManager = getArtifactoryManager(server, preferredDeployer.getUsername(), preferredDeployer.getPassword())) {
      ...
      addError(errors, "errorPromotion", "Failed to execute the promotion. Please review the TeamCity " + "server and Artifactory logs for further details.", xmlResponse);
    }
  }

  private void promoteBuild(@NotNull HttpServletRequest request, @NotNull Element xmlResponse, ActionErrors errors, SBuild build, Map<String, String> parameters, ArtifactoryManager artifactoryManager) throws IOException {
    PromotionBuilder promotionBuilder = new PromotionBuilder().status(PromotionTargetStatusType.valueOf(request.getParameter("targetStatus")).getStatusDisplayName()).comment(request.getParameter("comment")).ciUser(SessionUser.getUser(request).getUsername()).targetRepo(request.getParameter("promotionRepository")).dependencies(Boolean.parseBoolean(request.getParameter("includeDependencies"))).copy(Boolean.parseBoolean(request.getParameter("useCopy"))).dryRun(true);
  ...
  private boolean sendPromotionRequest(@NotNull Element xmlResponse, ActionErrors errors, SBuild build, Map<String, String> parameters, ArtifactoryManager artifactoryManager, PromotionBuilder promotionBuilder, boolean isDryRun) {
    if (isDryRun) {
  ...
  private void populateTargetRepos(@NotNull Element xmlResponse, String selectedUrlId, boolean overrideDeployerCredentials, CredentialsBean preferredDeployer, SProject project) {
    ...
    List<String> deployableRepos = deployableServers.getServerDeployableRepos(selectedUrlId, overrideDeployerCredentials, preferredDeployer.getUsername(), preferredDeployer.getPassword(), project);
  ...
  @NotNull private CredentialsBean getDeployerCredentialsBean(Map<String, String> parameters, ServerConfigBean server, boolean overrideDeployerCredentials) {
    String username = "";
    String password = "";
    ...
    return CredentialsHelper.getPreferredDeployingCredentials(server, overrideDeployerCredentials, username, password);
  }

  @Nullable private ServerConfigBean getServerConfigBean(@NotNull Element xmlResponse, ActionErrors errors, String selectedUrlId, SProject project) {
    ServerConfigBean server = deployableServers.getServerConfigById(selectedUrlId, project);
    if (server == null) {
      addError(errors, "errorPromotion", "Unable to perform any promotion operations: could not find an " + "Artifactory server associated with the configuration ID of \'" + selectedUrlId + "\'.", xmlResponse);
      return null;
    }
    return server;
  }

  @Nullable private String getSelectedUrlId(@NotNull Element xmlResponse, ActionErrors errors, Map<String, String> parameters) {
    String selectUrlIdParam = parameters.get(RunnerParameterKeys.URL_ID);
    if (StringUtils.isBlank(selectUrlIdParam)) {
      addError(errors, "errorDetails", "Unable to perform any promotion operations: could not find an " + "Artifactory server ID associated with the configuration of the selected build.", xmlResponse);
      return null;
    }
    return selectUrlIdParam;
  }

  @Nullable private SBuildType getSBuildType(@NotNull Element xmlResponse, ActionErrors errors, SBuild build) {
    SBuildType type = build.getBuildType();
    if (type == null) {
      addError(errors, "errorDetails", "The type of the selected build configuration could not be resolved.", xmlResponse);
      return null;
    }
    return type;
  }

  @Nullable private SBuild getSBuild(@NotNull HttpServletRequest request, @NotNull Element xmlResponse, ActionErrors errors) {
    SBuild build = BuildDataExtensionUtil.retrieveBuild(request, buildServer);
    if (build == null) {
      addError(errors, "errorDetails", "The selected build configuration could not be resolved: make sure " + "that the request sent to the promotion controller includes the \'buildId\' parameter.", xmlResponse);
      return null;
    }
    return build;
  }
