...
public class DeployableArtifactoryServers {
  ...
  public List<String> getServerDeployableRepos(String serverUrlId, boolean overrideDeployerCredentials, String username, String password, @Nullable SProject project) {
    List<ServerConfigBean> serverConfigs = configPersistenceManager.getConfiguredServers(project);
    for (ServerConfigBean serverConfig : serverConfigs) {
  ...
  public List<String> getServerLocalAndCacheRepos(String serverUrlId, boolean overrideDeployerCredentials, String username, String password, SProject project) {
    List<ServerConfigBean> serverConfigs = configPersistenceManager.getConfiguredServers(project);
    for (ServerConfigBean serverConfig : serverConfigs) {
      if (Objects.equals(serverUrlId, serverConfig.getId())) {
        CredentialsBean deployingCredentials = CredentialsHelper.getPreferredDeployingCredentials(serverConfig, overrideDeployerCredentials, username, password);
        try (ArtifactoryManager artifactoryManager = getArtifactoryManager(deployingCredentials, serverConfig)) {
          ...
          return Stream.concat(localRepos.stream(), remoteCacheRepos.stream()).collect(Collectors.toList());
        } catch (Exception e) {
          logException(e);
        }
      }
  ...
  public List<String> getServerResolvingRepos(String serverUrlId, boolean overrideDeployerCredentials, String username, String password, @Nullable SProject project) {
    List<ServerConfigBean> serverConfigs = configPersistenceManager.getConfiguredServers(project);
    for (ServerConfigBean serverConfig : serverConfigs) {
      if (Objects.equals(serverUrlId, serverConfig.getId())) {
  ...
  private ArtifactoryManager getArtifactoryManager(CredentialsBean credentials, ServerConfigBean serverConfig) {
    ArtifactoryManager artifactoryManager = new ArtifactoryManager(serverConfig.getUrl(), credentials.getUsername(), credentials.getPassword(), new TeamcityServerBuildInfoLog());
    artifactoryManager.setConnectionTimeout(serverConfig.getTimeout());
    ProxyInfo proxyInfo = ProxyInfo.getInfo();
    if (proxyInfo != null) {
      artifactoryManager.setProxyConfiguration(proxyInfo.getHost(), proxyInfo.getPort(), proxyInfo.getUsername(), proxyInfo.getPassword());
    }
  ...
  public boolean serverHasAddons(String serverUrlId, boolean overrideDeployerCredentials, String username, String password, @Nullable SProject project) {
    ...
    for (ServerConfigBean serverConfig : serverConfigs) {
      if (Objects.equals(serverUrlId, serverConfig.getId())) {
        CredentialsBean deployerCredentials = CredentialsHelper.getPreferredResolvingCredentials(serverConfig, overrideDeployerCredentials, username, password);
    ...
    for (ServerConfigBean serverConfig : serverConfigs) {
      if (Objects.equals(serverUrlId, serverConfig.getId())) {
        ...
        try (ArtifactoryManager artifactoryManager = getArtifactoryManager(deployerCredentials, serverConfig)) {
          ...
          boolean compatible = serverVersion.isAtLeast(new ArtifactoryVersion(ConstantValues.MINIMAL_ARTIFACTORY_VERSION));
