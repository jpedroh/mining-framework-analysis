...
public class ArtifactoryPolledBuildTrigger extends PolledBuildTrigger {
  ...
  public ArtifactoryPolledBuildTrigger(DeployableArtifactoryServers deployableArtifactoryServers, Multimap<String, BuildWatchedItem> watchedItems) {
    this.deployableArtifactoryServers = deployableArtifactoryServers;
    this.watchedItems = watchedItems;
  }

  @Override public int getPollInterval(@NotNull PolledTriggerContext context) {
  ...
  @Override public void triggerBuild(@NotNull PolledTriggerContext context) throws BuildTriggerException {
    BuildTriggerDescriptor descriptor = context.getTriggerDescriptor();
    Map<String, String> map = descriptor.getProperties();
        ...
        try {
          ...
          if (itemValue != itemLastModified && itemValue != 0) {
            if (itemLastModified != 0) {
              ...
              String message = String.format("Artifactory trigger has found changes on the watched " + "item \'%s\' for build \'%s\'. Last modified time was %s and is now %s.", itemUrl, triggerId, format.format(itemValue), itemLastModified);
          ...
          Loggers.SERVER.error("Error occurred while polling for changes for build \'" + triggerId + "\' on path \'" + serverConfig.getUrl() + "/" + itemUrl + "\': " + e.getMessage());
        }
      }
    }
    if (foundChange) {
  ...
  @NotNull private String getUniqueTriggerId(PolledTriggerContext context) {
