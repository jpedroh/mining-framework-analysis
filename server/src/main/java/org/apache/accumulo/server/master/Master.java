...
public class Master implements LiveTServerSet.Listener, TableObserver, CurrentState {
  ...
  volatile private SortedMap<TServerInstance, TabletServerStatus> tserverStatus = Collections.unmodifiableSortedMap(new TreeMap<TServerInstance, TabletServerStatus>());
  ...
  static final boolean transitionOK[][] = { { X, X, _, _, _, _, X }, { _, X, X, X, _, _, X }, { _, _, X, X, X, _, X }, { _, _, X, X, X, _, X }, { _, _, X, X, X, X, X }, { _, _, _, _, _, X, X }, { _, _, _, _, _, _, X } };
  ...
  private void upgradeZookeeper() {
    if (Accumulo.getAccumuloPersistentVersion(fs) == Constants.PREV_DATA_VERSION) {
      try {
        ...
        TablePropUtil.setTableProperty(Constants.METADATA_TABLE_ID, Property.TABLE_ITERATOR_PREFIX.getKey() + "majc.bulkLoadFilter", "20," + MetadataBulkLoadFilter.class.getName());
  ...
  private class MasterClientServiceHandler implements MasterClientService.Iface {
    ...
    @Override public long initiateFlush(TInfo tinfo, AuthInfo c, String tableId) throws ThriftSecurityException, ThriftTableOperationException, TException {
    ...
    @Override public void waitForFlush(TInfo tinfo, AuthInfo c, String tableId, ByteBuffer startRow, ByteBuffer endRow, long flushID, long maxLoops) throws ThriftSecurityException, ThriftTableOperationException, TException {
      security.canFlush(c, tableId);
    ...
    @Override public MasterMonitorInfo getMasterStats(TInfo info, AuthInfo credentials) throws ThriftSecurityException, TException {
    ...
    private void alterTableProperty(AuthInfo c, String tableName, String property, String value, TableOperation op) throws ThriftSecurityException, ThriftTableOperationException {
      ...
      try {
        if (value == null) {
          ...
          if (!TablePropUtil.setTableProperty(tableId, property, value)) {
    ...
    @Override public void removeTableProperty(TInfo info, AuthInfo credentials, String tableName, String property) throws ThriftSecurityException, ThriftTableOperationException, TException {
      alterTableProperty(credentials, tableName, property, null, TableOperation.REMOVE_PROPERTY);
    }

    @Override public void setTableProperty(TInfo info, AuthInfo credentials, String tableName, String property, String value) throws ThriftSecurityException, ThriftTableOperationException, TException {
      alterTableProperty(credentials, tableName, property, value, TableOperation.SET_PROPERTY);
    }

    @Override public void shutdown(TInfo info, AuthInfo c, boolean stopTabletServers) throws ThriftSecurityException, TException {
      security.canPerformSystemActions(c);
      Master.this.shutdown(stopTabletServers);
    }

    @Override public void shutdownTabletServer(TInfo info, AuthInfo c, String tabletServer, boolean force) throws ThriftSecurityException, TException {
      security.canPerformSystemActions(c);
    ...
    @Override public void reportSplitExtent(TInfo info, AuthInfo credentials, String serverName, TabletSplit split) throws TException {
    ...
    @Override public void reportTabletStatus(TInfo info, AuthInfo credentials, String serverName, TabletLoadState status, TKeyExtent ttablet) throws TException {
    ...
    @Override public void setMasterGoalState(TInfo info, AuthInfo c, MasterGoalState state) throws ThriftSecurityException, TException {
      security.canPerformSystemActions(c);
    ...
    private void updatePlugins(String property) {
      if (property.equals(Property.MASTER_TABLET_BALANCER.getKey())) {
        TabletBalancer balancer = createInstanceFromPropertyName(instance.getConfiguration(), Property.MASTER_TABLET_BALANCER, TabletBalancer.class, new DefaultLoadBalancer());
    ...
    @Override public void removeSystemProperty(TInfo info, AuthInfo c, String property) throws ThriftSecurityException, TException {
      security.canPerformSystemActions(c);
      try {
    ...
    @Override public void setSystemProperty(TInfo info, AuthInfo credentials, String property, String value) throws ThriftSecurityException, TException {
    ...
    @Override public long beginTableOperation(TInfo tinfo, AuthInfo credentials) throws ThriftSecurityException, TException {
      authenticate(credentials);
      return fate.startTransaction();
    }

    @Override public void executeTableOperation(TInfo tinfo, AuthInfo c, long opid, org.apache.accumulo.core.master.thrift.TableOperation op, List<ByteBuffer> arguments, Map<String, String> options, boolean autoCleanup) throws ThriftSecurityException, ThriftTableOperationException, TException {
      authenticate(c);
      switch (op) {
        case CREATE:
        {
          ...
          org.apache.accumulo.core.client.admin.TimeType timeType = org.apache.accumulo.core.client.admin.TimeType.valueOf(ByteBufferUtil.toString(arguments.get(1)));
    ...
    @Override public String waitForTableOperation(TInfo tinfo, AuthInfo credentials, long opid) throws ThriftSecurityException, ThriftTableOperationException, TException {
      authenticate(credentials);
    ...
    @Override public void finishTableOperation(TInfo tinfo, AuthInfo credentials, long opid) throws ThriftSecurityException, TException {
      authenticate(credentials);
      fate.delete(opid);
    }
  }
  ...
  public void setMergeState(MergeInfo info, MergeState state) throws IOException, KeeperException, InterruptedException {
    synchronized (mergeLock) {
      ...
      if (state.equals(MergeState.NONE)) {
        ...
        ZooReaderWriter.getInstance().putPersistentData(path, out.getData(), state.equals(MergeState.STARTED) ? ZooUtil.NodeExistsPolicy.FAIL : ZooUtil.NodeExistsPolicy.OVERWRITE);
      }
      mergeLock.notifyAll();
    }
  ...
  private void setMasterGoalState(MasterGoalState state) {
    try {
      ZooReaderWriter.getInstance().putPersistentData(ZooUtil.getRoot(instance) + Constants.ZMASTER_GOAL_STATE, state.name().getBytes(utf8), NodeExistsPolicy.OVERWRITE);
    } catch (Exception ex) {
  ...
  private class TabletGroupWatcher extends Daemon {
    ...
    private void deleteTablets(MergeInfo info) throws AccumuloException {
      ...
      try {
        ...
        Range deleteRange = new Range(KeyExtent.getMetadataEntry(range.getTableId(), start), false, KeyExtent.getMetadataEntry(range.getTableId(), range.getEndRow()), true);
        ...
        if (followingTablet != null) {
          ...
          MetadataTable.addTablet(new KeyExtent(range.getTableId(), null, range.getPrevEndRow()), Constants.DEFAULT_TABLET_LOCATION, SecurityConstants.getSystemCredentials(), timeType, masterLock);
        }
      } catch (Exception ex) {
  ...
  public void run() throws IOException, InterruptedException, KeeperException {
    ...
    final TabletStateStore stores[] = { new ZooTabletStateStore(new ZooStore(zroot)), new RootTabletStateStore(instance, systemAuths, this), new MetaDataStateStore(instance, systemAuths, this) };
    ...
    try {
      fate = new Fate<Master>(this, new org.apache.accumulo.fate.ZooStore<Master>(ZooUtil.getRoot(instance) + Constants.ZFATE, ZooReaderWriter.getRetryingInstance()), 4);
    ...
    clientService = TServerUtils.startServer(getSystemConfiguration(), Property.MASTER_CLIENTPORT, processor, "Master", "Master Client Service Handler", null, Property.MASTER_MINTHREADS, Property.MASTER_THREADCHECK).server;
  ...
  private void getMasterLock(final String zMasterLoc) throws KeeperException, InterruptedException {
    ...
    final String masterClientAddress = org.apache.accumulo.core.util.AddressUtil.toString(new InetSocketAddress(hostname, getSystemConfiguration().getPort(Property.MASTER_CLIENTPORT)));
  ...
  @Override public void stateChanged(String tableId, TableState state) {
    nextEvent.event("Table state in zookeeper changed for %s to %s", tableId, state);
  }

  @Override public void initialize(Map<String, TableState> tableIdToStateMap) {
  }

  @Override public void sessionExpired() {
  }

  @Override public Set<String> onlineTables() {
