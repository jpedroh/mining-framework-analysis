...
class FinishImportTable extends MasterRepo {
  ...
  @Override public long isReady(long tid, Master environment) throws Exception {
    return 0;
  }

  @Override public Repo<Master> call(long tid, Master env) throws Exception {
  ...
  @Override public String getReturn() {
    return tableInfo.tableId;
  }

  @Override public void undo(long tid, Master env) throws Exception {
  }
}
...
class MoveExportedFiles extends MasterRepo {
  ...
  @Override public Repo<Master> call(long tid, Master environment) throws Exception {
    try {
      ...
      for (String oldFileName : fileNameMappings.keySet()) {
        if (!fs.exists(new Path(tableInfo.exportDir, oldFileName))) {
          throw new ThriftTableOperationException(tableInfo.tableId, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "File referenced by exported table does not exists " + oldFileName);
        }
      }
      ...
      throw new ThriftTableOperationException(tableInfo.tableId, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Error renaming files " + ioe.getMessage());
    }
  }
}
...
class PopulateMetadataTable extends MasterRepo {
  ...
  @Override public Repo<Master> call(long tid, Master environment) throws Exception {
    ...
    try {
      FileSystem fs = environment.getFileSystem();
      mbw = HdfsZooInstance.getInstance().getConnector(SecurityConstants.getSystemCredentials()).createBatchWriter(Constants.METADATA_TABLE_NAME, new BatchWriterConfig());
      ...
      log.warn(ioe.getMessage(), ioe);
      throw new ThriftTableOperationException(tableInfo.tableId, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Error reading " + path + " " + ioe.getMessage());
    } finally {
      if (zis != null) {
        try {
          zis.close();
  ...
  @Override public void undo(long tid, Master environment) throws Exception {
...
class MapImportFileNames extends MasterRepo {
  ...
  @Override public Repo<Master> call(long tid, Master environment) throws Exception {
    ...
    try {
      ...
      log.warn(ioe.getMessage(), ioe);
      throw new ThriftTableOperationException(tableInfo.tableId, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Error writing mapping file " + path + " " + ioe.getMessage());
    } finally {
...
class ImportPopulateZookeeper extends MasterRepo {
  ...
  private Map<String, String> getExportedProps(FileSystem fs) throws Exception {
    ...
    try {
      ...
      throw new ThriftTableOperationException(tableInfo.tableId, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Error reading table props from " + path + " " + ioe.getMessage());
    }
  }
  ...
  @Override public Repo<Master> call(long tid, Master env) throws Exception {
    ...
    for (Entry<String, String> entry : getExportedProps(env.getFileSystem()).entrySet()) {
      if (!TablePropUtil.setTableProperty(tableInfo.tableId, entry.getKey(), entry.getValue())) {
        throw new ThriftTableOperationException(tableInfo.tableId, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Invalid table property " + entry.getKey());
      }
    }
  ...
  public void checkVersions(Master env) throws ThriftTableOperationException {
    ...
    try {
        ...
        if (zipEntry.getName().equals(Constants.EXPORT_INFO_FILE)) {
          ...
          while ((line = in.readLine()) != null) {
            ...
            if (sa[0].equals(ExportTable.EXPORT_VERSION_PROP)) {
              ...
              if (sa[0].equals(ExportTable.DATA_VERSION_PROP)) {
      ...
      if (exportVersion == null || exportVersion > ExportTable.VERSION) {
        throw new ThriftTableOperationException(null, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Incompatible export version " + exportVersion);
      }
      if (dataVersion == null || dataVersion > Constants.DATA_VERSION) {
        throw new ThriftTableOperationException(null, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Incompatible data version " + exportVersion);
      }
    } catch (IOException ioe) {
      log.warn(ioe.getMessage(), ioe);
      throw new ThriftTableOperationException(null, tableInfo.tableName, TableOperation.IMPORT, TableOperationExceptionType.OTHER, "Failed to read export metadata " + ioe.getMessage());
    } finally {
      if (zis != null) {
        try {
          zis.close();
