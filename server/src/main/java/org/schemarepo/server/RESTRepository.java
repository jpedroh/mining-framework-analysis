 ...
 */
@Singleton @Path(value = "/") public class RESTRepository {
   ...
   */
  @Inject public RESTRepository(Repository repo, JsonUtil jsonUtil) {
    this.repo = repo;
   ...
   */
  @GET public Response allSubjects(@HeaderParam(value = "Accept") String mediaType) {
    if (mediaType.equals(MediaType.APPLICATION_JSON)) {
      return Response.ok(jsonUtil.subjectsToJson(repo.subjects()), MediaType.APPLICATION_JSON).build();
    } else {
      return Response.ok(RepositoryUtil.subjectsToString(repo.subjects()), MediaType.TEXT_PLAIN).build();
    }
  }
   ...
   *         such subject
   */
  @GET @Path(value = "{subject}/all") public Response allSchemaEntries(@HeaderParam(value = "Accept") String mediaType, @PathParam(value = "subject") String subject) {
    Subject s = repo.lookup(subject);
    if (null == s) {
      throw new NotFoundException(MessageStrings.SUBJECT_DOES_NOT_EXIST_ERROR);
    }
    if (mediaType.equals(MediaType.APPLICATION_JSON)) {
      return Response.ok(jsonUtil.schemasToJson(s.allEntries()), MediaType.APPLICATION_JSON).build();
    } else {
      return Response.ok(RepositoryUtil.schemasToString(s.allEntries()), MediaType.TEXT_PLAIN).build();
    }
  }
