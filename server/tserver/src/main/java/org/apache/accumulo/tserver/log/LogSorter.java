...
public class LogSorter {
  ...
  class LogProcessor implements Processor {
    ...
    @Override public Processor newProcessor() {
      return new LogProcessor();
    }

    @Override public void process(String child, byte[] data) {
  ...
  @VisibleForTesting void writeBuffer(String destPath, List<Pair<LogFileKey, LogFileValue>> buffer, int part) throws IOException {
    ...
    try (var writer = FileOperations.getInstance().newWriterBuilder().forFile(UnreferencedTabletFile.of(fs, fullPath), fs, fs.getConf(), cryptoService).withTableConfiguration(sortedLogConf).build()) {
  ...
  public void startWatchingForRecoveryLogs(ThreadPoolExecutor distWorkQThreadPool) throws KeeperException, InterruptedException {
    ...
    new DistributedWorkQueue(context.getZooKeeperRoot() + Constants.ZRECOVERY, sortedLogConf, context).startProcessing(new LogProcessor(), this.threadPool);
  }
