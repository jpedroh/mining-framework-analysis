...
public class ServerAmpleImpl extends AmpleImpl implements Ample {
  ...
  @Override public Ample.TabletMutator mutateTablet(KeyExtent extent) {
  ...
  @Override public TabletsMutator mutateTablets() {
    ...
    try {
      context.getZooReaderWriter().mutateOrCreate(zpath, new byte[0], (currVal) -> {
        ...
        if (newJson.length() > 262_144) {
          log.warn("Root tablet deletion candidates stored in ZK at {} are getting large ({} bytes), is" + " Accumulo GC process running?  Large nodes may cause problems for Zookeeper!", zpath, newJson.length());
        }
  ...
  @Override public void putGcCandidates(TableId tableId, Collection<StoredTabletFile> candidates) {
  ...
  @Override public void putGcFileAndDirCandidates(TableId tableId, Collection<ReferenceFile> candidates) {
    if (DataLevel.of(tableId) == DataLevel.ROOT) {
      mutateRootGcCandidates((rgcc) -> rgcc.add(candidates.stream().map((reference) -> StoredTabletFile.of(URI.create(reference.getMetadataPath())))));
      return;
    }
    try (BatchWriter writer = context.createBatchWriter(DataLevel.of(tableId).metaTable())) {
  ...
  @Override public void addBulkLoadInProgressFlag(String path, long fateTxid) {
  ...
  @Override public void removeBulkLoadInProgressFlag(String path) {
    Mutation m = new Mutation(BlipSection.getRowPrefix() + path);
  ...
  @Override public void removeBulkLoadEntries(TableId tableId, long tid, Text firstSplit, Text lastSplit) {
    Preconditions.checkArgument(DataLevel.of(tableId) == DataLevel.USER);
    try (Scanner mscanner = new IsolatedScanner(context.createScanner(MetadataTable.NAME, Authorizations.EMPTY)); BatchWriter bw = context.createBatchWriter(MetadataTable.NAME)) {
  ...
  @Override public void deleteGcCandidates(DataLevel level, Collection<GcCandidate> candidates, GcCandidateType type) {
  ...
  @Override public Iterator<GcCandidate> getGcCandidates(DataLevel level) {
    if (level == DataLevel.ROOT) {
      ...
      if (level == DataLevel.METADATA || level == DataLevel.USER) {
        ...
        return scanner.stream().filter((entry) -> entry.getValue().equals(SkewedKeyValue.NAME)).map((entry) -> new GcCandidate(DeletesSection.decodeRow(entry.getKey().getRow().toString()), entry.getKey().getTimestamp())).iterator();
      } else {
  ...
  @Override public Mutation createDeleteMutation(ReferenceFile tabletFilePathToRemove) {
  ...
  @Override public void putExternalCompactionFinalStates(Collection<ExternalCompactionFinalState> finalStates) {
  ...
  @Override public Stream<ExternalCompactionFinalState> getExternalCompactionFinalStates() {
    Scanner scanner;
    try {
    ...
    return scanner.stream().map((e) -> ExternalCompactionFinalState.fromJson(ExternalCompactionId.of(e.getKey().getRowData().toString().substring(pLen)), e.getValue().toString()));
  }

  @Override public void deleteExternalCompactionFinalStates(Collection<ExternalCompactionId> statusesToDelete) {
    try (BatchWriter writer = context.createBatchWriter(DataLevel.USER.metaTable())) {
      String prefix = ExternalCompactionSection.getRowPrefix();
  ...
  @Override public void putScanServerFileReferences(Collection<ScanServerRefTabletFile> scanRefs) {
    try (BatchWriter writer = context.createBatchWriter(DataLevel.USER.metaTable())) {
      ...
      throw new IllegalStateException("Error inserting scan server file references into " + DataLevel.USER.metaTable(), e);
    }
  }

  @Override public Stream<ScanServerRefTabletFile> getScanServerFileReferences() {
    try {
      ...
      return StreamSupport.stream(scanner.spliterator(), false).map((e) -> new ScanServerRefTabletFile(e.getKey().getRowData().toString().substring(pLen), e.getKey().getColumnFamily(), e.getKey().getColumnQualifier()));
    } catch (TableNotFoundException e) {
      throw new IllegalStateException(DataLevel.USER.metaTable() + " not found!", e);
    }
  }

  @Override public void deleteScanServerFileReferences(String serverAddress, UUID scanServerLockUUID) {
    ...
    try (Scanner scanner = context.createScanner(DataLevel.USER.metaTable(), Authorizations.EMPTY)) {
      scanner.setRange(ScanServerFileReferenceSection.getRange());
      scanner.fetchColumn(new Text(serverAddress), new Text(scanServerLockUUID.toString()));
      int pLen = ScanServerFileReferenceSection.getRowPrefix().length();
      Set<ScanServerRefTabletFile> refsToDelete = StreamSupport.stream(scanner.spliterator(), false).map((e) -> new ScanServerRefTabletFile(e.getKey().getRowData().toString().substring(pLen), e.getKey().getColumnFamily(), e.getKey().getColumnQualifier())).collect(Collectors.toSet());
  ...
  @Override public void deleteScanServerFileReferences(Collection<ScanServerRefTabletFile> refsToDelete) {
    try (BatchWriter writer = context.createBatchWriter(DataLevel.USER.metaTable())) {
      String prefix = ScanServerFileReferenceSection.getRowPrefix();
