  ...
  public Optional<ActionReference> lookupAction(String category, String name) {
    ...
    if (key != null) {
      if (categoryActions.containsKey(key)) {
        return categoryActions.get(key).stream().filter((action) -> action.getId().contains(name)).findFirst();
      }
      return Optional.empty();
    }
    return categoryActions.values().stream().flatMap(Collection::stream).filter((action) -> action.getId().contains(name)).findFirst();
  }
