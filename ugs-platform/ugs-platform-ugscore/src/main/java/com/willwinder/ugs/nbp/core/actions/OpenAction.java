...
@ActionID(category = LocalizingService.OpenCategory, id = LocalizingService.OpenActionId) @ActionRegistration(iconBase = OpenAction.ICON_BASE, displayName = "resources.MessagesBundle#" + LocalizingService.OpenTitleKey, lazy = false) @ActionReferences(value = { @ActionReference(path = LocalizingService.OpenWindowPath, position = 10), @ActionReference(path = "Toolbars/File", position = 10), @ActionReference(path = "Shortcuts", name = "M-O") }) public final class OpenAction extends AbstractAction {
  ...
  public void openFile(File selectedFile) throws DataObjectNotFoundException {
    if (EditorUtils.closeOpenEditors()) {
      ...
      DataObject.find(FileUtil.toFileObject(selectedFile)).getLookup().lookup(OpenCookie.class).open();
    }
  }
