 ...
 */
@ActionID(category = LocalizingService.OutlineCategory, id = LocalizingService.OutlineActionId) @ActionRegistration(iconBase = OutlineAction.ICON_BASE, displayName = "resources.MessagesBundle#" + LocalizingService.OutlineTitleKey, lazy = false) @ActionReferences(value = { @ActionReference(path = LocalizingService.MENU_PROGRAM, position = 1200, separatorBefore = 1199) }) public final class OutlineAction extends AbstractAction implements UGSEventListener {
  ...
  @Override public boolean isEnabled() {
    return backend != null && backend.getControllerState() == ControllerState.IDLE && backend.getGcodeFile() != null;
  }
