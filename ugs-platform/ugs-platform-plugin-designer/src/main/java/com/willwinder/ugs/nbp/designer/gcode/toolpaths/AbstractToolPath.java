...
public abstract class AbstractToolPath implements PathGenerator {
  ...
  protected LinearRing pathToLinearRing(GcodePath gcodePath) {
    List<org.locationtech.jts.geom.Coordinate> coordinateList = gcodePath.getSegments().stream().map((segment) -> pointToCoordinate(segment.getPoint())).collect(Collectors.toList());
  ...
  protected List<NumericCoordinate> geometryToCoordinates(Geometry geometry) {
    ...
    return Arrays.stream(coordinates).map((c) -> new NumericCoordinate(c.getX(), c.getY(), c.getZ())).collect(Collectors.toList());
  }
