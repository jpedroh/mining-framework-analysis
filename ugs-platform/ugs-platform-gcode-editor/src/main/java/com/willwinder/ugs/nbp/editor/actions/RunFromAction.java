...
@ActionID(category = LocalizingService.RunFromCategory, id = LocalizingService.RunFromActionId) @ActionRegistration(iconBase = RunFromAction.ICON_BASE, displayName = "Run from...", lazy = false) @ActionReferences(value = { @ActionReference(path = LocalizingService.RunFromWindowPath, position = 1016) }) public final class RunFromAction extends CookieAction implements UGSEventListener {
  ...
  @Override protected void performAction(Node[] activatedNodes) {
    ...
    try {
      Integer result = Integer.parseInt(JOptionPane.showInputDialog(new JFrame(), "Enter a line number to start from.", "Run From Action", JOptionPane.QUESTION_MESSAGE));
