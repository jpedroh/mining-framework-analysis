...
public class ParamCoreSTBranchActionBuilder extends STBranchActionBuilder {
  ...
  @Override public String buildArgs(EAction a) {
    return IntStream.range(0, a.payload.elems.size()).mapToObj((i) -> ParamCoreSTApiGenConstants.GO_CROSS_RECEIVE_FUN_ARG + i + " *" + a.payload.elems.get(i)).collect(Collectors.joining(", "));
  }

  @Override public String buildBody(STStateChanApiBuilder api, EState curr, EAction a, EState succ) {
    String res = "ch, selected := <-s._" + a.mid + "_Chan\n" + "if !selected {\n" + "\treturn nil // select ignores nilchan\n" + "}\n";
