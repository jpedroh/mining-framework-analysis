...
public class ParamCoreSTBranchStateBuilder extends STBranchStateBuilder {
  ...
  @Override public String getPreamble(STStateChanApiBuilder api, EState s) {
    ...
    if (((GoJob) apigen.job).noCopy) {
      res += "label := " + sEpRecv + "Raw(" + sEpProto + "." + peer.getName() + ", " + foo.apply(g.start) + ", " + foo.apply(g.end) + ")\n" + "op := *label[0].(*string)\n";
    } else {
    ...
    if (!allEmpty) {
      res += "b := " + sEpRecv + (((GoJob) api.job).noCopy ? "Raw" : "") + "(" + sEpProto + "." + peer.getName() + ", " + foo.apply(g.start) + ", " + foo.apply(g.end) + ")\n";
    }
    res += "switch op {\n" + s.getActions().stream().map((a) -> {
      ...
      return "\tcase \"" + a.mid + "\":\n" + "\t\tch := make(chan *" + apigen.getStateChanName(s.getSuccessor(a)) + ", 1)\n" + "\t\tch <- " + apigen.getSuccStateChan(this.bb, s, s.getSuccessor(a), sEp) + "\n" + "\t\ts._" + a.mid + "_Chan <- ch\n" + "\t\t" + s.getActions().stream().filter((otheract) -> otheract.mid != a.mid).map((otheract) -> {
        return "close(s._" + otheract.mid + "_Chan)";
      }).collect(Collectors.joining("\n\t")) + "\n";
    }).collect(Collectors.joining("\n")) + "\n" + "\tdefault:\n" + "\t\t" + ParamCoreSTApiGenConstants.GO_IO_FUN_RECEIVER + "." + ParamCoreSTApiGenConstants.GO_SCHAN_ENDPOINT + "." + "Errors <- session.UnknownChoiceLabelError{Label: op}\n" + "\t}\n";
    if (!allEmpty) {
      res += "\ts.data <- b\n";
    }
    res += "}\n";
