...
public class ParamCoreSTReceiveActionBuilder extends STReceiveActionBuilder {
  @Override public String getActionName(STStateChanApiBuilder api, EAction a) {
    return ParamCoreSTApiGenConstants.GO_CROSS_RECEIVE_FUN_PREFIX + "_" + ParamCoreSTStateChanApiBuilder.getGeneratedParamRoleName(((ParamCoreEAction) a).getPeer()) + "_" + a.mid;
  }

  @Override public String buildArgs(EAction a) {
    return IntStream.range(0, a.payload.elems.size()).mapToObj((i) -> ParamCoreSTApiGenConstants.GO_CROSS_RECEIVE_FUN_ARG + i + " *" + a.payload.elems.get(i) + ", reduceFn" + i + " func(" + ParamCoreSTApiGenConstants.GO_CROSS_SEND_FUN_ARG + i + " []int) int").collect(Collectors.joining(", "));
  }
