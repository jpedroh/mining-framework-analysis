...
public class ExperimentalHighlighter implements Highlighter {
  ...
  @Override public HighlightField highlight(HighlighterContext context) {
    try {
  ...
  static class HighlightExecutionContext {
    ...
    <T extends java.lang.Object> T getOption(String key, T defaultValue) {
      @SuppressWarnings(value = { "unchecked" }) T value = (T) getOption(key);
    ...
    private void ensureWeigher() {
      ...
      weigher = new BasicQueryWeigher(new ElasticsearchQueryFlattener(maxExpandedTerms, phraseAsTerms, removeHighFrequencyTermsFromCommonTerms), infos, context.hitContext.topLevelReader(), context.query.originalQuery());
    ...
    /**
         ...
         */
    @SuppressWarnings(value = { "unchecked" }) private List<String> getRegexes() {
    ...
    private HitEnum buildLuceneRegexHitEnumForRegex(final AutomatonHitEnum.Factory factory, List<String> fieldValues, final boolean caseInsensitive) {
      ...
      if (fieldValues.size() == 1) {
        ...
        Iterator<HitEnumAndLength> hitEnumsFromStreams = Iterators.transform(fieldValues.iterator(), new Function<String, HitEnumAndLength>() {
          @Override public HitEnumAndLength apply(String fieldValue) {
            if (caseInsensitive) {
              fieldValue = fieldValue.toLowerCase(getLocale());
            }
    ...
    private SnippetWeigher buildSnippetWeigher() {
      ...
      try {
        @SuppressWarnings(value = { "unchecked" }) Map<String, Object> map = (Map<String, Object>) config;
        if (map.containsKey("sum")) {
          return new SumSnippetWeigher();
        }
        Object exponentialConfig = map.get("exponential");
        if (exponentialConfig != null) {
          @SuppressWarnings(value = { "unchecked" }) Map<String, Object> exponentialConfigMap = (Map<String, Object>) exponentialConfig;
    ...
    private List<FieldWrapper> buildFetchFields() {
      @SuppressWarnings(value = { "unchecked" }) List<String> fetchFields = (List<String>) getOption("fetch_fields");
      if (fetchFields == null) {
        return null;
      }
