...
@Rule(key = "S2755") public class XxeCheck extends PHPVisitorCheck {
  ...
  @Override public void visitFunctionCall(FunctionCallTree call) {
    ...
    if (callee.is(Kind.NAMESPACE_NAME) && "simplexml_load_string".equals(functionName)) {
      ...
      if (callee.is(Kind.OBJECT_MEMBER_ACCESS)) {
        if ("load".equals(functionName) || "loadxml".equals(functionName)) {
          ...
          if ("setparserproperty".equals(functionName)) {
            checkSetParserProperty(call);
          }
        }
      } else {
        if (callee.is(Kind.CLASS_MEMBER_ACCESS) && "Xml::build".equals(callee.toString())) {
  ...
  private void checkSimpleXmlOption(ExpressionTree optionValue, Tree treeToReport) {
    if (optionValue.is(Kind.NAMESPACE_NAME) && "LIBXML_NOENT".equals(((NamespaceNameTree) optionValue).unqualifiedName())) {
      ...
      if (optionValue.is(Kind.BITWISE_OR)) {
        ...
        if (optionValue.is(Kind.PARENTHESISED_EXPRESSION)) {
  ...
  private void checkXmlBuildOption(ExpressionTree valueTree, Tree primaryTree) {
    if (valueTree.is(Kind.VARIABLE_IDENTIFIER)) {
      ...
      if (valueTree.is(ARRAY)) {
        arrayValue((ArrayInitializerTree) valueTree, "loadEntities").ifPresent((x) -> raiseIssueIfTrue(x, primaryTree));
      }
    }
  }
