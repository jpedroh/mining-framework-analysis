...
public class DynamicDaoHelperImpl implements DynamicDaoHelper {
  ...
  public static Class<?> getNonProxyImplementationClassIfNecessary(Class<?> candidate) {
    ...
    if (ProxyFactory.isProxyClass(candidate)) {
      ...
      if (HibernateProxy.class.isAssignableFrom(candidate)) {
        if (!candidate.getName().contains(HIBERNATE_PROXY)) {
          throw new ProxyDetectionException(String.format("Cannot determine the original implementation class for " + "the Hibernate proxy. Expected to find the keyphrase (%s) in the proxy classname (%s).", HIBERNATE_PROXY, candidate.getName()));
        }
  ...
  @Override public Class<?>[] getAllPolymorphicEntitiesFromCeiling(Class<?> ceilingClass, boolean includeUnqualifiedPolymorphicEntities, boolean useCache) {
  ...
  @Override public Class<?>[] getUpDownInheritance(Class<?> testClass, boolean includeUnqualifiedPolymorphicEntities, boolean useCache) {
  ...
  @Override public boolean isExcludeClassFromPolymorphism(Class<?> clazz) {
    ...
    if (adminPresentationClass == null) {
      ...
      if (adminPresentationClass.excludeFromPolymorphism()) {
        return true;
      }
    }
    return false;
  ...
  @Override public Map<String, Object> getIdMetadata(Class<?> entityClass, EntityManager entityManager) {
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public List<String> getPropertyNames(Class<?> entityClass) {
    entityClass = getNonProxyImplementationClassIfNecessary(entityClass);
  ...
  @Override public Field getIdField(Class<?> clazz) {
