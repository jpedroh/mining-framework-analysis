 ...
 * @version 1.0
 */
@RunWith(value = Arquillian.class) @RunAsClient public class UcscResponseTest extends AbstractResponseTest {
  private static final String BEACON = "clinvar";

  @Override public void testAllRefsFound(URL url) throws JAXBException, MalformedURLException {
  }

  @Test public void testClinvar(@ArquillianResource URL url) throws JAXBException, MalformedURLException {
    String b = "clinvar";
    String[] query = { "1", "10042537", "T", null };
  ...
  @Test public void testUniprot(@ArquillianResource URL url) throws JAXBException, MalformedURLException {
    String b = "uniprot";
    String[] query = { "1", "977027", "T", null };
    BeaconResponseTo br = readBeaconResponse(url.toExternalForm() + getUrl(b, query));
    assertNotNull(br);
    assertTrue(beaconsMatch(br.getBeacon(), b));
    assertTrue(queriesMatch(br.getQuery(), query));
  ...
  @Test public void testLovd(@ArquillianResource URL url) throws JAXBException, MalformedURLException {
    String b = "lovd";
  ...
  @Test @Override public void testSpecificRefNotFound(@ArquillianResource URL url) throws JAXBException, MalformedURLException {
    String[] query = { "1", "10042537", "T", "hg38" };
  ...
  @Test @Override public void testInvalidRef(@ArquillianResource URL url) throws JAXBException, MalformedURLException {
    String[] query = { "1", "10042537", "T", "hg100" };
    BeaconResponseTo br = readBeaconResponse(url.toExternalForm() + getUrl(BEACON, query));
    assertNotNull(br);
    assertTrue(beaconsMatch(br.getBeacon(), BEACON));
  ...
  @Test @Override public void testRefConversion(@ArquillianResource URL url) throws JAXBException, MalformedURLException {
    String[] query = { "1", "10042537", "T", "grch37" };
    BeaconResponseTo br = readBeaconResponse(url.toExternalForm() + getUrl(BEACON, query));
    assertNotNull(br);
    assertTrue(beaconsMatch(br.getBeacon(), BEACON));
    assertTrue(getNonMachingFields(br.getQuery(), query).size() == 1);
