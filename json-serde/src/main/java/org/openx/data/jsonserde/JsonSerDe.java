...
public class JsonSerDe extends AbstractSerDe {
  ...
  @Override public void initialize(Configuration conf, Properties tbl) throws SerDeException {
    ...
    explicitNull = Boolean.parseBoolean(tbl.getProperty(PROP_EXPLICIT_NULL, "false"));
  }
  ...
  /**
     ...
     * @throws SerDeException 
     */
  @Override public Object deserialize(Writable w) throws SerDeException {
    Text rowText = (Text) w;
  ...
  private JSONObject serializeStruct(Object obj, StructObjectInspector soi, List<String> columnNames) {
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      try {
        if (null != data) {
          result.put(getSerializedFieldName(columnNames, i, sf), serializeField(data, sf.getFieldObjectInspector()));
        } else {
          if (explicitNull) {
  ...
  private JSONObject serializeMap(Object obj, MapObjectInspector moi) {
    ...
    for (Object k : m.keySet()) {
      try {
        jo.put(serializeField(k, moi.getMapKeyObjectInspector()).toString(), serializeField(m.get(k), moi.getMapValueObjectInspector()));
      } catch (JSONException ex) {
