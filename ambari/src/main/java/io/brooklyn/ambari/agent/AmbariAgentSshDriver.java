...
public class AmbariAgentSshDriver extends JavaSoftwareProcessSshDriver implements AmbariAgentDriver {
  ...
  @Override protected String getLogFileLocation() {
  ...
  @Override public AmbariAgentImpl getEntity() {
  ...
  @Override public void install() {
    ...
    ImmutableList<String> commands = ImmutableList.<String>builder().add(defaultAmbariInstallHelper.installAmbariRequirements(getMachine())).addAll(BashCommands.setHostname(fqdn)).add(installPackage("ambari-agent")).build();
  ...
  @Override public void customize() {
    ...
    newScript(CUSTOMIZING).body.append(sudo(format("mv %s %s", tmpConfigFileLoc, destinationConfigFile))).failOnNonZeroResultCode().execute();
  }
