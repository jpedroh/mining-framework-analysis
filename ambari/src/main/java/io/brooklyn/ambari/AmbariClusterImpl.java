...
public class AmbariClusterImpl extends BasicStartableImpl implements AmbariCluster {
  ...
  private Function<AmbariServer, String> mapAmbariServerToFQDN = new Function<AmbariServer, String>() {
    @Nullable @Override public String apply(@Nullable AmbariServer ambariServer) {
  ...
  @Override public void init() {
    ...
    for (EntitySpec<? extends ExtraService> entitySpec : getConfig(EXTRA_HADOOP_SERVICES)) {
      String bindTo = entitySpec.getFlags().containsKey(ExtraService.BIND_TO.getName()) ? String.valueOf(entitySpec.getFlags().get(ExtraService.BIND_TO.getName())) : ExtraService.BIND_TO.getDefaultValue();
      String serviceName = entitySpec.getFlags().containsKey(ExtraService.SERVICE_NAME.getName()) ? String.valueOf(entitySpec.getFlags().get(ExtraService.SERVICE_NAME.getName())) : ExtraService.SERVICE_NAME.getDefaultValue();
      List<String> componentNames = entitySpec.getFlags().containsKey(ExtraService.COMPONENT_NAMES.getName()) ? (List<String>) entitySpec.getFlags().get(ExtraService.COMPONENT_NAMES.getName()) : ExtraService.COMPONENT_NAMES.getDefaultValue();
      ...
      if (isHostGroupsDeployment) {
        Preconditions.checkNotNull(componentNames, "Please specify the list of components names (%s) for \"%s\" as this is a host groups based deployment.", ExtraService.COMPONENT_NAMES.getName(), entitySpec.getType().getName());
        Iterable<ExtraServiceComponentMap> componentMaps = transform(componentNames, componentToComponentMap(bindTo));
        if (componentMaps.iterator().hasNext()) {
          mapComponentsToHostGroups(componentMaps);
        }
      } else {
        Preconditions.checkNotNull(serviceName, "Please specify the service name (%s) for \"%s\" as this is a services based deployment.", ExtraService.SERVICE_NAME.getName(), entitySpec.getType().getName());
  ...
  private Function<String, ExtraServiceComponentMap> componentToComponentMap(final String defaultBindTo) {
    return new Function<String, ExtraServiceComponentMap>() {
      @Nullable @Override public ExtraServiceComponentMap apply(@Nullable String componentName) {
  ...
  @Override public Iterable<AmbariNode> getAmbariNodes() {
    return Entities.descendants(this, AmbariNode.class);
  }

  @Override public Iterable<AmbariAgent> getAmbariAgents() {
    return Entities.descendants(this, AmbariAgent.class);
  }

  @Override public Iterable<AmbariServer> getAmbariServers() {
    return Entities.descendants(this, AmbariServer.class);
  }

  @Override public AmbariServer getMasterAmbariServer() {
    return Iterables.getFirst(Entities.descendants(this, AmbariServer.class), null);
  }

  @Override public List<String> getExtraStackDefinitionsUrls() {
  ...
  static final class RegisteredHostEventListener implements SensorEventListener<List<String>> {
    ...
    @Override public void onEvent(SensorEvent<List<String>> event) {
  ...
  static final class ClusterStateEventListener implements SensorEventListener<String> {
    ...
    @Override public void onEvent(SensorEvent<String> sensorEvent) {
  ...
  @Override public void deployCluster() throws AmbariApiException, ExtraServiceException {
  ...
  @Override public void postDeployCluster() throws ExtraServiceException {
  ...
  private EntitySpec<? extends AmbariServer> createServerSpec(Object securityGroup) {
    EntitySpec<? extends AmbariServer> serverSpec = EntitySpec.create(getConfig(SERVER_SPEC)).configure(SoftwareProcess.SUGGESTED_VERSION, getConfig(AmbariCluster.SUGGESTED_VERSION)).configure(config().getBag().getAllConfig()).displayName("Ambari Server");
  ...
  private RecommendationWrapper getRecommendationWrapperFromAmbariHostGroups() {
    ...
    for (AmbariHostGroup ambariHostGroup : getHostGroups()) {
      HostGroup.Builder hostGroupBuilder = new HostGroup.Builder().setName(ambariHostGroup.getDisplayName()).addComponents(ambariHostGroup.getComponents());
      ...
      bindingsBuilder.addHostGroup(new HostGroup.Builder().setName(ambariHostGroup.getDisplayName()).addHosts(ambariHostGroup.getHostFQDNs()).build());
    }
    List<String> serverComponentsList = getConfig(AmbariCluster.SERVER_COMPONENTS);
    if (!serverComponentsList.isEmpty()) {
      HostGroup.Builder hostGroupBuilder = new HostGroup.Builder().setName(SERVER_HOST_GROUP).addComponents(serverComponentsList);
      ...
      bindingsBuilder.addHostGroup(new HostGroup.Builder().setName(SERVER_HOST_GROUP).addHosts(Lists.newArrayList(fqdns)).build());
    }
    return new RecommendationWrapper.Builder().setStack(new Stack.Builder().setName(getConfig(HADOOP_STACK_NAME)).setVersion(getConfig(HADOOP_STACK_VERSION)).build()).setRecommendation(new Recommendation.Builder().setBlueprint(blueprintBuilder.build()).setBindings(bindingsBuilder.build()).build()).build();
  }
  ...
  private RecommendationWrapper getRecommendationWrapperFromAmbariServer() {
    ...
    final RecommendationWrappers recommendationWrappers = getMasterAmbariServer().getRecommendations(getConfig(HADOOP_STACK_NAME), getConfig(HADOOP_STACK_VERSION), hosts, services);
    return recommendationWrappers.getRecommendationWrappers().size() > 0 ? recommendationWrappers.getRecommendationWrappers().get(0) : null;
  }
  ...
  private Task<List<?>> parallelListenerTask(final Function<ExtraService, ?> fn) {
    ...
    for (final ExtraService extraService : getExtraServices()) {
      Task<?> t = Tasks.builder().name(extraService.toString()).description("Invocation on " + extraService.toString()).body(new FunctionRunningCallable<ExtraService>(extraService, fn)).build();
      tasks.add(t);
    }
    return Tasks.parallel("Parallel invocation of " + fn + " on extra services", tasks);
  }

  @Nonnull private AmbariCluster getAmbariCluster() {
    return this;
  }

  @Nonnull private Iterable<ExtraService> getExtraServices() {
  ...
  @Nullable private AmbariAgent getAmbariAgentByFqdn(@Nonnull String fqdn) {
  ...
  private class PreClusterDeployFunction implements Function<ExtraService, Void> {
    @Override public Void apply(ExtraService extraService) {
  ...
  private void createClusterTopology() {
    for (int i = 0; i < getAttribute(EXPECTED_AGENTS); i++) {
      addChild(EntitySpec.create(AmbariHostGroup.class).configure(AmbariHostGroup.INITIAL_SIZE, 1).displayName(String.format("host-group-%d", (i + 1))));
    }
  }

  private <T extends java.lang.Object> T getRequiredConfig(ConfigKey<T> key) {
    return checkNotNull(getConfig(key), "config %s", key);
  }

  private static <T extends java.lang.Object, V extends java.lang.Object> void putIfNotPresent(Map<T, V> map, T key, V value) {
