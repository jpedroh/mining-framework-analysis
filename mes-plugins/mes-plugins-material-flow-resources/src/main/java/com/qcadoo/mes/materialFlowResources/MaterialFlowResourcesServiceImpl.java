...
@Service public class MaterialFlowResourcesServiceImpl implements MaterialFlowResourcesService {
  ...
  @Override public List<Entity> getWarehouseLocationsFromDB() {
    return getLocationDD().find().list().getEntities();
  }

  @Override public BigDecimal getResourcesQuantityForLocationAndProduct(final Entity location, final Entity product) {
    ...
    if (Objects.isNull(resources)) {
      ...
      for (Entity resource : resources) {
        resourcesQuantity = resourcesQuantity.add(resource.getDecimalField(ResourceFields.QUANTITY), numberService.getMathContext());
      }
      return resourcesQuantity;
    }
  }

  @Override public List<Entity> getResourcesForLocationAndProduct(final Entity location, final Entity product) {
    return getResourceDD().find().add(SearchRestrictions.belongsTo(ResourceFields.LOCATION, location)).add(SearchRestrictions.belongsTo(ResourceFields.PRODUCT, product)).addOrder(SearchOrders.asc(ResourceFields.TIME)).list().getEntities();
  }

  @Override public Map<Long, BigDecimal> getQuantitiesForProductsAndLocation(final List<Entity> products, final Entity location) {
    return getQuantitiesForProductsAndLocation(products, location, false);
  }

  @Override public Map<Long, BigDecimal> getQuantitiesForProductsAndLocation(final List<Entity> products, final Entity location, final boolean withoutBlockedForQualityControl) {
    return getQuantitiesForProductsAndLocation(products, location, withoutBlockedForQualityControl, ResourceStockDtoFields.AVAILABLE_QUANTITY);
  }

  @Override public Map<Long, BigDecimal> getQuantitiesForProductsAndLocation(final List<Entity> products, final Entity location, final boolean withoutBlockedForQualityControl, final String fieldName) {
    ...
    if (products.size() > 0) {
      ...
      resourceStocks.forEach((resourceStock) -> quantities.put(Long.valueOf(resourceStock.getIntegerField(ResourceStockDtoFields.PRODUCT_ID).intValue()), ResourceStockDtoFields.AVAILABLE_QUANTITY.equals(fieldName) ? resourceStock.getDecimalField(ResourceStockDtoFields.AVAILABLE_QUANTITY) : resourceStock.getDecimalField(ResourceStockDtoFields.QUANTITY)));
    }
    return quantities;
  }

  @Override public Map<Long, Map<Long, BigDecimal>> getQuantitiesForProductsAndLocations(final List<Entity> products, final List<Entity> locations) {
  ...
  @Override public List<QuantityDto> getQuantitiesForProductsAndLocationWMS(final List<String> productNumbers, final Long materialFlowLocationId) {
  ...
  @Override public List<ResourcesQuantityDto> getResourceQuantities(final Long storageLocationId, final String productNumber) {
  ...
  @Override public List<PalletNumberProductDTO> getProductsForPalletNumber(String palletNumber, List<String> userLocationNumbers) {
  ...
  @Override public List<SumOfProductsDto> getSumOfProducts(String productNumber, List<String> locationNumbers) {
  ...
  @Override public List<StorageLocationsForProductDto> getStoragesForProductNumber(String productNumber, List<String> locationNumbers) {
  ...
  @Override public ResourceDetailsDto getResourceDetails(String resourceNumber) {
    Map<String, Object> params = Maps.newHashMap();
...
<<<<<<< /usr/src/app/output/qcadoo/mes/f2bd4f55611e1699ec432c9a59deda4dea07d659/mes-plugins/mes-plugins-material-flow-resources/src/main/java/com/qcadoo/mes/materialFlowResources/MaterialFlowResourcesServiceImpl.java/left.java
      "dto.blockedForQualityControl as blockedForQualityControl "
=======
      "dto.qualityRating as qualityRating "
...
>>>>>>> /usr/src/app/output/qcadoo/mes/f2bd4f55611e1699ec432c9a59deda4dea07d659/mes-plugins/mes-plugins-material-flow-resources/src/main/java/com/qcadoo/mes/materialFlowResources/MaterialFlowResourcesServiceImpl.java/right.java
  ...
  @Override public List<ResourceToRepackDto> getResourceListByStorageLocationNumber(String storageLocationNumber, List<String> userLocations) {
    Map<String, Object> params = Maps.newHashMap();
  ...
  @Override public List<ResourceToRepackDto> getResourceListByPalletNumber(String palletNumber, List<String> userLocations) {
    Map<String, Object> params = Maps.newHashMap();
    List<ResourceToRepackDto> list = new ArrayList<>();
  ...
  @Override public List<ResourceToRepackDto> getResourceListByPalletAndLocationNumber(String palletNumber, String storageLocationNumber, List<String> userLocations) {
    Map<String, Object> params = Maps.newHashMap();
    List<ResourceToRepackDto> list = new ArrayList<>();
    if ((palletNumber != null || !palletNumber.isEmpty()) && (storageLocationNumber != null || !storageLocationNumber.isEmpty()) && !userLocations.isEmpty()) {
      StringBuilder prepareQuery = new StringBuilder();
      prepareQuery.append("SELECT ");
      prepareQuery.append("dto.location_id as locationId, ");
      prepareQuery.append("dto.number as resourceNumber, ");
  ...
  private DataDefinition getResourceStockDtoDD() {
    return dataDefinitionService.get(MaterialFlowResourcesConstants.PLUGIN_IDENTIFIER, MaterialFlowResourcesConstants.MODEL_RESOURCE_STOCK_DTO);
  }
}