...
@Repository public class DocumentPositionService {
  ...
  @Autowired private AttributePositionService attributePositionService;

  @Autowired private PluginManager pluginManager;

  public GridResponse<DocumentPositionDTO> findAll(final Long documentId, final String _sidx, final String _sord, final int page, final int perPage, final DocumentPositionDTO position, final Map<String, String> attributeFilters) {
    ...
    List<String> attrCloumns = columns.stream().filter((c) -> c.isChecked() && c.isForAttribute()).map(ColumnProperties::getName).collect(Collectors.toList());
    ...
    String queryRecords = String.format(query, "*", "ORDER BY " + orderBy + " " + sord) + String.format(" LIMIT %d OFFSET %d", perPage, perPage * (page - 1));
  ...
  public void update(final DocumentPositionDTO documentPositionVO) {
    ...
    attributePositionService.createOrUpdateAttributePositionValues(false, documentPositionVO.getId(), documentPositionVO.getAttrs());
    updateDocumentPositionsNumbers(documentPositionVO.getDocument());
  }
  ...
  private List<StorageLocationDTO> getStorageLocations(String preparedQuery, final String query, final Map<String, Object> paramMap) {
    if (Strings.isNullOrEmpty(query)) {
      ...
      return jdbcTemplate.query(preparedQuery, queryParameters, BeanPropertyRowMapper.newInstance(StorageLocationDTO.class));
    }
  }
  ...
  public DataResponse getStorageLocationsResponse(final String query, final String product, final String document) {
    ...
    if (Strings.isNullOrEmpty(product)) {
      preparedQuery = "SELECT id, number FROM materialflowresources_storagelocation WHERE number ilike :query " + "AND location_id IN (SELECT DISTINCT COALESCE(locationfrom_id, locationto_id) FROM materialflowresources_document WHERE id = :document) " + "AND active = true;";
    } else {
      preparedQuery = "SELECT id, number FROM materialflowresources_storagelocation WHERE number ilike :query " + "AND location_id IN (SELECT DISTINCT COALESCE(locationfrom_id, locationto_id) FROM materialflowresources_document WHERE id = :document) " + "AND (product_id IN (SELECT id FROM basic_product WHERE number LIKE :product) OR product_id IS NULL) " + "AND active = true;";
  ...
  public DataResponse getBatchesResponse(final String query, final String product) {
    if (StringUtils.isEmpty(product)) {
      ...
      Map<String, Object> paramMap = Maps.newHashMap();
      paramMap.put("product", product);
      preparedQuery = "SELECT _batch.id, _batch.number as number, p.number as product " + "FROM advancedgenealogy_batch _batch " + "LEFT JOIN basic_product p on p.id = _batch.product_id " + "WHERE p.number = :product AND _batch.number ilike :query AND _batch.active=true";
      MapSqlParameterSource queryParameters = new MapSqlParameterSource(paramMap).addValue("query", '%' + query + '%');
      preparedQuery = preparedQuery + " LIMIT " + DataProvider.MAX_RESULTS + ';';
      List<BatchDTO> entities = jdbcTemplate.query(preparedQuery, queryParameters, BeanPropertyRowMapper.newInstance(BatchDTO.class));
      return dataProvider.getDataResponse(query, preparedQuery, entities, paramMap);
    }
  }
  ...
  public Map<String, Object> getGridConfig(final Long documentId) {
    try {
      String query = "SELECT documentpositionparametersitem.*, attr.dataType as attributeDataType, attr.valueType as attributeValueType " + "FROM materialflowresources_documentpositionparametersitem documentpositionparametersitem " + "LEFT JOIN basic_attribute attr ON attr.id = documentpositionparametersitem.attribute_id  " + "WHERE attr IS NULL OR attr.active = TRUE  ORDER BY documentpositionparametersitem.ordering";
      List<ColumnProperties> columns = jdbcTemplate.query(query, Collections.emptyMap(), new BeanPropertyRowMapper(ColumnProperties.class));
  ...
  public ProductDTO getProductForProductNumber(final String number) {
    String _query = "SELECT product.id, product.number AS code, product.number, product.name, product.ean, product.globaltypeofmaterial, product.category " + "FROM basic_product product WHERE product.number = :number";
    List<ProductDTO> products = jdbcTemplate.query(_query, Collections.singletonMap("number", number), new BeanPropertyRowMapper(ProductDTO.class));
    if (products.size() == 1) {
  ...
  public void updateDocumentPositionsNumbers(final Long documentId) {
    String query = "SELECT p.*, p.document_id AS document, product.number AS product, product.unit, palletnumber.number AS palletnumber, " + "location.number AS storagelocationnumber\n" + "\tFROM materialflowresources_position p\n" + "\tLEFT JOIN basic_product product ON (p.product_id = product.id)\n" + "\tLEFT JOIN basic_palletnumber palletnumber ON (p.palletnumber_id = palletnumber.id)\n" + "\tLEFT JOIN materialflowresources_storagelocation location ON (p.storagelocation_id = location.id) WHERE p.document_id = :documentId ORDER BY p.id";
    List<DocumentPositionDTO> list = jdbcTemplate.query(query, Collections.singletonMap(DOCUMENT_ID, documentId), new BeanPropertyRowMapper(DocumentPositionDTO.class));
    int index = 1;
  ...
  private List<Map<String, Object>> getAvailableAdditionalUnitsByProduct(final Map<String, Object> units) {
    ...
    String query = "SELECT convItem.unitto, convItem.quantityto, convItem.quantityfrom, dictItem.isinteger FROM qcadoomodel_unitconversionitem convItem " + " JOIN qcadoomodel_dictionaryitem dictItem ON dictItem.name = convItem.unitto " + " WHERE convItem.product_id = :id";
  ...
  private boolean isGridReadOnly(final Long documentId) {
    ...
    if (pluginManager.isPluginEnabled(MOBILE_WMS)) {
      ...
      readOnly = readOnly || documentResult.get(DocumentFields.WMS) != null && (boolean) documentResult.get(DocumentFields.WMS) && documentResult.get(DocumentFields.EDIT_IN_WMS) != null && !(boolean) documentResult.get(DocumentFields.EDIT_IN_WMS);
    }
    return readOnly;
  }
  ...
  public StorageLocationDTO getStorageLocation(final String product, final String document) {
    ...
    String query = "SELECT dsl.storagelocation_id AS id, dsl.number AS number, p.name AS product, l.name AS location " + "FROM materialflowresources_defaultstoragelocationdto dsl " + "JOIN basic_product p ON p.id = dsl.product_id " + "JOIN materialflow_location l ON l.id = dsl.location_id " + "WHERE dsl.location_id IN " + "(SELECT DISTINCT COALESCE(locationfrom_id, locationto_id) AS location FROM materialflowresources_document WHERE id = :document) " + "AND dsl.active = true AND p.number = :product LIMIT 1;";
    Map<String, Object> filter = Maps.newHashMap();
    filter.put("product", product);
    filter.put("document", Integer.parseInt(document));
    List<StorageLocationDTO> locations = jdbcTemplate.query(query, filter, new BeanPropertyRowMapper(StorageLocationDTO.class));
    if (locations.size() == 1) {
  ...
  public ResourceDTO getResource(final Long document, final String product, final BigDecimal conversion, final Long batchId) {
    ...
    batches = batches.stream().filter((resource) -> resource.getAvailableQuantity().compareTo(BigDecimal.ZERO) > 0).collect(Collectors.toList());
    if (batches.isEmpty()) {
      return null;
    } else {
  ...
  public List<AbstractDTO> getResources(final Long document, final String additionalQuery, final String product, final BigDecimal conversion, boolean useBatch, Long batchId) {
  ...
  public DataResponse getResourcesResponse(final Long document, final String query, final String product, final BigDecimal conversion, final Long batchId, final boolean shouldCheckMaxResults) {
    if (Strings.isNullOrEmpty(product)) {
      return new DataResponse(Lists.newArrayList(), 0);
    }
    boolean useBatch = Objects.nonNull(batchId);
    ...
    ilikeValue = ilikeValue.replace("%%", "%");
    List<AbstractDTO> entities = getResources(document, ilikeValue, product, conversion, useBatch, batchId);
    if (entities.isEmpty()) {
  ...
  public ResourceDTO getResourceByNumber(final String resource) {
    ...
    if (batches.isEmpty()) {
      ...
      String attrBuilder = "SELECT att.number, resourceattributevalue.value " + "FROM materialflowresources_resourceattributevalue resourceattributevalue " + "LEFT JOIN materialflowresources_resource res ON res.id = resourceattributevalue.resource_id " + "LEFT JOIN basic_attribute att ON att.id = resourceattributevalue.attribute_id " + "WHERE res.id = :resourceId";
      List<AttributeDto> attributes = jdbcTemplate.query(attrBuilder, params, new BeanPropertyRowMapper(AttributeDto.class));
