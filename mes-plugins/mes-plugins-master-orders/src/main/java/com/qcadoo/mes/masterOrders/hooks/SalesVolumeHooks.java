...
@Service public class SalesVolumeHooks {
  @Autowired private NumberService numberService;

  @Autowired private MaterialFlowResourcesService materialFlowResourcesService;
  ...
  public boolean checkIfSalesVolumeAlreadyExists(final DataDefinition salesVolumeDD, final Entity salesVolume) {
    ...
    if (Objects.isNull(salesVolumeFromDB)) {
      ...
      salesVolume.addError(salesVolumeDD.getField(SalesVolumeFields.PRODUCT), "masterOrders.salesVolume.error.productAlreadyExists");
      return false;
    }
  }
