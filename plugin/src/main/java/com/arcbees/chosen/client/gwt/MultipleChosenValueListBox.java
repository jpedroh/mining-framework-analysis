...
public class MultipleChosenValueListBox<T extends java.lang.Object> extends BaseChosenValueListBox<T> implements HasValue<List<T>>, IsEditor<TakesValueEditor<List<T>>> {
  ...
  @Override public HandlerRegistration addValueChangeHandler(ValueChangeHandler<List<T>> handler) {
    return addHandler(handler, ValueChangeEvent.getType());
  }

  @Override public TakesValueEditor<List<T>> asEditor() {
    if (editor == null) {
  ...
  /**
     ...
     */
  @Override public void setValue(List<T> values) {
  ...
  @Override protected ChosenListBox createChosenListBox(ChosenOptions options) {
    return new ChosenListBox(true, options);
  }

  @Override protected void deselectValue(T value) {
    boolean removed = selectedValues.remove(value);
    Preconditions.checkState(removed, "Value was not previously selected");
    ValueChangeEvent.fire(this, getValue());
  }

  @Override protected void selectValue(T value) {
    setValue(Lists.newArrayList(value), true, false);
  }

  @Override protected void updateChosenListBox() {
  ...
  @Override protected void addItemToChosenListBox(T value) {
  ...
  private void checkValuesAcceptability(List<T> values) {
    ...
    if (!unacceptableValues.isEmpty()) {
      throw new IllegalStateException("These following values are not accepted by the component: " + unacceptableValues);
    }
  }
}