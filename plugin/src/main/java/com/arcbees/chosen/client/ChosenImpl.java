...
public class ChosenImpl {
  public static interface ChozenTemplate extends SafeHtmlTemplates {
    ...
    @Template(value = "<li class=\"{1}\" id=\"{0}\"><span>{2}</span><a href=\"javascript:void(0)\" class=\"{3}\" " + "rel=\"{4}\" data-chosen-value=\"{5}\"></a></li>") SafeHtml choice(String id, String searchChoiceClass, SafeHtml content, String searchChoiceCloseClass, String rel, String value);

    @Template(value = "<div id=\"{0}\" class=\"{1}\"></div>") SafeHtml container(String id, String cssClasses);

    @Template(value = "<ul class=\"{0}\"><li class=\"{1}\"><input type=\"text\" value=\"{2}\" class=\"{3}\" " + "autocomplete=\"off\" style=\"width:25px;\"/></li></ul><div class=\"{4}\" style=\"{6}\"><ul " + "class=\"{5}\"></ul></div>") SafeHtml contentMultiple(String chznChoicesClass, String chznSearchFieldClass, String defaultText, String defaultClass, String chznDropClass, String chznResultClass, SafeStyles offsets);

    @Template(value = "<a href=\"javascript:void(0)\" class=\"{0} {1}\"><span>{2}</span><div><b></b></div></a><div " + "class=\"{3}\" style=\"{6}\"><div class=\"{4}\"><input type=\"text\" autocomplete=\"off\" /></div><ul" + " class=\"{5}\"></ul></div>") SafeHtml contentSingle(String chznSingleClass, String chznDefaultClass, String defaultText, String dropClass, String chznSearchClass, String chznResultClass, SafeStyles offsets);

    @Template(value = "<li id=\"{0}\" class=\"{1}\">{2}</li>") SafeHtml group(String id, String groupResultClass, String content);

    @Template(value = "<li class=\"{0}\">{1}\"<span></span>\"</li>") SafeHtml noResults(String noResultsClass, String content);

    @Template(value = "<li id=\"{0}\" class=\"{1}\" style=\"{2}\">{3}</li>") SafeHtml option(String id, String groupResultClass, SafeStyles style, String content);

    @Template(value = "<li id=\"{0}\" class=\"{1}\" style=\"{2}\">{3}</li>") SafeHtml option(String id, String groupResultClass, SafeStyles safeStyles, SafeHtml htmlContent);
  }
  ...
  private static class ClientResultsFilter implements ResultsFilter {
    ...
    @Override public void filter(String searchText, ChosenImpl chosen, boolean isShowing) {
        ...
        if (item.isGroup()) {
          ...
          if (!(chosen.isMultiple() && option.isSelected())) {
            ...
            if (regex.test(optionContent)) {
              ...
              if (optionContent.indexOf(" ") >= 0 || optionContent.indexOf("[") == 0) {
...
<<<<<<< /usr/src/app/output/arcbees/gwtchosen/d5d56191cde632b6ce9f22b6853ca259916891ae/plugin/src/main/java/com/arcbees/chosen/client/ChosenImpl.java/left.java
  private static final int VERTICAL_OFFSET = -9000;
=======
  private List<String> selectedValues = new ArrayList<String>();
>>>>>>> /usr/src/app/output/arcbees/gwtchosen/d5d56191cde632b6ce9f22b6853ca259916891ae/plugin/src/main/java/com/arcbees/chosen/client/ChosenImpl.java/right.java
  ...
  private void choiceBuild(OptionItem option) {
    ...
    searchContainer.before(ChozenTemplate.templates.choice(choiceId, css.searchChoice(), html, css.searchChoiceClose(), "" + option.getArrayIndex(), option.getValue()).asString());
  ...
  private boolean choicesClick(Event e) {
    ...
    Element target = e.getEventTarget().cast();
    GQuery $e = $(target);
    if (activeField && !($e.hasClass(css.searchChoice()) || !$e.parents("." + css.searchChoice()).isEmpty()) && !resultsShowing) {
      resultsShow();
    }
    return false;
  }
  ...
  private boolean containerMouseDown(Event e) {
    ...
    if (!pendingDestroyClick && !targetCloseLink) {
      if (!activeField) {
        ...
        if (!isMultiple && !$e.isEmpty() && ($e.get(0) == selectedItem.get(0) || $e.parents("a." + css.chznSingle()).length() > 0)) {
          e.preventDefault();
          resultsToggle();
        }
      }
  ...
  private SafeHtml createOption(OptionItem item) {
    ...
    builder.append(fromSafeConstant("<option value=\'")).appendEscaped(item.getValue()).append(fromSafeConstant("\'"));
    if (item.isSelected()) {
  ...
  private void searchFieldScale() {
    ...
    StringBuilder styleBlock = new StringBuilder("position:absolute; " + (isRTL ? "right" : "left") + ": -1000px; top: -1000px; " + "visibility:hidden;");
    String[] styleToCopy = { "font-size", "font-style", "font-weight", "font-family", "line-height", "text-transform", "letter-spacing" };
  ...
  private void setDefaultText() {
    ...
    if (dataPlaceHolder != null && dataPlaceHolder.length() > 0) {
      ...
      if (isMultiple) {
        if (options.getPlaceholderTextMultiple() != null) {
          ...
          if (options.getPlaceholderText() != null) {
    ...
    if (dataNoResultsText != null && dataNoResultsText.length() > 0) {
      ...
      if (options.getNoResultsText() != null) {
  ...
  private void setDefaultValues() {
    ...
    allowSingleDeselect = options.isAllowSingleDeselect() && optionsList.getLength() > 0 && "".equals(optionsList.getItem(0).getText());
    choices = 0;
  ...
  private void setup() {
    ...
    GQuery containerTemp = $(ChozenTemplate.templates.container(containerId, cssClasses).asString()).width(fWidth);
    ...
    if (isMultiple) {
      containerTemp.html(ChozenTemplate.templates.contentMultiple(css.chznChoices(), css.searchField(), defaultText, css.defaultClass(), css.chznDrop(), css.chznResults(), ssb.toSafeStyles()).asString());
    } else {
      containerTemp.html(ChozenTemplate.templates.contentSingle(css.chznSingle(), css.chznDefault(), defaultText, css.chznDrop(), css.chznSearch(), css.chznResults(), ssb.toSafeStyles()).asString());
    }
    ...
    if (isMultiple) {
      ...
      int searchFieldWidth = ddWidth - getSideBorderPadding(searchContainer, isHidden) - getSideBorderPadding(searchField, isHidden);
  ...
  private void singleDeselectControlBuild() {
    if (allowSingleDeselect && selectedItem.find("abbr").isEmpty()) {
      selectedItem.find("span").first().after("<abbr class=\"" + css.searchChoiceClose() + "\"></abbr>");
    }
  }
  ...
  private void winnowResultsClear() {
    ...
    for (Element li : lis.elements()) {
      ...
      if ($li.hasClass(css.groupResult())) {
        ...
        if (!isMultiple || !$li.hasClass(css.resultSelected())) {
          resultActivate($li);
        }
      }
