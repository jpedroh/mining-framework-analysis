...
public class ChosenImpl {
  public interface ChozenTemplate extends SafeHtmlTemplates {
    ...
    @Template(value = "<li class=\"{1}\" id=\"{0}\"><span>{2}</span><a href=\"javascript:void(0)\" class=\"{3} {6}\" " + "rel=\"{4}\" data-chosen-value=\"{5}\"></a></li>") SafeHtml choice(String id, String searchChoiceClass, SafeHtml content, String searchChoiceCloseClass, String rel, String value, String iconCloseClass);
    ...
    @Template(value = "<a href=\"javascript:void(0)\" class=\"{0} {1}\"><span>{2}</span><div><b " + "class=\"{7}\"></b></div></a><div class=\"{3}\" style=\"{6}\"><div class=\"{4} {8}\">" + "<input type=\"text\" autocomplete=\"off\" /></div><ul class=\"{5}\"></ul></div>") SafeHtml contentSingle(String chznSingleClass, String chznDefaultClass, String defaultText, String dropClass, String chznSearchClass, String chznResultClass, SafeStyles offsets, String iconArrowClass, String iconSearchClass);
  ...
  private void choiceBuild(OptionItem option) {
    ...
    searchContainer.before(ChozenTemplate.templates.choice(choiceId, css.searchChoice(), html, css.searchChoiceClose(), "" + option.getArrayIndex(), option.getValue(), css.iconCross()).asString());
  ...
  private SafeHtml createOption(OptionItem item) {
    ...
    builder.append(fromTrustedString("<option value=\'")).appendEscaped(item.getValue()).append(fromTrustedString("\'"));
    if (item.isSelected()) {
  ...
  private void setup() {
    ...
    if (isMultiple) {
      ...
      containerTemp.html(ChozenTemplate.templates.contentSingle(css.chznSingle(), css.chznDefault(), defaultText, css.chznDrop(), css.chznSearch(), css.chznResults(), ssb.toSafeStyles(), css.iconArrow(), css.iconSearch()).asString());
    }
  ...
  private void singleDeselectControlBuild() {
    if (allowSingleDeselect && selectedItem.find("abbr").isEmpty()) {
      selectedItem.find("span").first().after("<abbr class=\"" + css.searchChoiceClose() + " " + css.iconCross() + "\"></abbr>");
    }
  }
