...
public abstract class ChosenImpl {
  ...
  protected void containerMouseDownImpl(Event e, GQuery element) {
    if (!activeField) {
      ...
      if (!element.isEmpty() && (element.get(0) == selectedItem.get(0) || element.parents("a." + css.chznSingle()).length() > 0)) {
        e.preventDefault();
        resultsToggle();
      }
    }
  ...
  void noResults(String terms) {
    GQuery noResults = $(ChosenTemplate.templates.noResults(css.noResults(), resultsNoneFound).asString());
  ...
  private SafeHtml resultAddOption(OptionItem option) {
    if (!option.isDisabled()) {
      ...
      if (option.getHtml() != null && !option.getHtml().trim().isEmpty()) {
        ...
        return ChosenTemplate.templates.option(option.getDomId(), classes.toString().trim(), safeStyles, option.getText());
      }
    }
    return null;
  }
  ...
  private void setup() {
    ...
    GQuery containerTemp = $(ChosenTemplate.templates.container(containerId, cssClasses).asString()).width(fWidth);
