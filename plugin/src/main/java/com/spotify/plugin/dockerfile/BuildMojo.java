...
@Mojo(name = "build", defaultPhase = LifecyclePhase.PACKAGE, requiresProject = true, threadSafe = true) public class BuildMojo extends AbstractDockerMojo {
  /**
   * Directory containing the the build context. This is typically the directory that contains
   * your Dockerfile.
   */
  @Parameter(defaultValue = "${project.basedir}", property = "dockerfile.contextDirectory", required = true) private File contextDirectory;


<<<<<<< /usr/src/app/output/spotify/dockerfile-maven/580f84d9277f80882d20aff38d6f065ed7206bf8/plugin/src/main/java/com/spotify/plugin/dockerfile/BuildMojo.java/left.java
  /**
   * Path to the Dockerfile to build. The specified file must reside withing the build context
   */
  @Parameter(property = "dockerfile.dockerfile", required = false) private File dockerfile;
=======
  @Parameter(property = "dockerfile.build.cacheFrom") private List<String> cacheFrom;
>>>>>>> /usr/src/app/output/spotify/dockerfile-maven/580f84d9277f80882d20aff38d6f065ed7206bf8/plugin/src/main/java/com/spotify/plugin/dockerfile/BuildMojo.java/right.java
   ...
   */
  @Parameter(property = "dockerfile.repository") private String repository;

  /**
   * The tag to apply when building the Dockerfile, which is appended to the repository.
   */
  @Parameter(property = "dockerfile.tag", defaultValue = "latest") private String tag;

  /**
   * Disables the build goal; it becomes a no-op.
   */
  @Parameter(property = "dockerfile.build.skip", defaultValue = "false") private boolean skipBuild;

  /**
   * Updates base images automatically.
   */
  @Parameter(property = "dockerfile.build.pullNewerImage", defaultValue = "true") private boolean pullNewerImage;

  /**
   * Do not use cache when building the image.
   */
  @Parameter(property = "dockerfile.build.noCache", defaultValue = "false") private boolean noCache;

  /**
   * Custom build arguments.
   */
  @Parameter(property = "dockerfile.buildArgs") private Map<String, String> buildArgs;

  @Parameter(property = "dockerfile.build.squash", defaultValue = "false") private boolean squash;

  @Override public void execute(DockerClient dockerClient) throws MojoExecutionException, MojoFailureException {
    ...
    final String imageId = buildImage(dockerClient, log, verbose, contextDirectory.toPath(), dockerfilePath, repository, tag, pullNewerImage, noCache, buildArgs, cacheFrom, squash);
    if (imageId == null) {
  ...
  @Nullable static String buildImage(@Nonnull DockerClient dockerClient, @Nonnull Log log, boolean verbose, @Nonnull Path contextDirectory, @Nullable Path dockerfile, @Nullable String repository, @Nonnull String tag, boolean pullNewerImage, boolean noCache, @Nullable Map<String, String> buildArgs, @Nullable List<String> cacheFrom, boolean squash) throws MojoExecutionException, MojoFailureException {
    ...
    if (dockerfile != null) {
      buildParameters.add(DockerClient.BuildParam.dockerfile(contextDirectory.relativize(dockerfile)));
    }
    ...
    if (cacheFrom != null) {
      ...
      for (String image : cacheFrom) {
        try {
          ...
          log.warn(MessageFormat.format("Image {0} not found, build will not use it for cache-from", image));
    ...
    final DockerClient.BuildParam[] buildParametersArray = buildParameters.toArray(new DockerClient.BuildParam[buildParameters.size()]);
    log.info("");
    try {
      if (repository != null) {
  ...
  private static void requireValidDockerFilePath(@Nonnull Log log, @Nonnull Path contextDirectory, @Nullable Path dockerfile) throws MojoFailureException {
    ...
    if (dockerfile == null && !Files.exists(contextDirectory.resolve("Dockerfile")) && !Files.exists(contextDirectory.resolve("dockerfile"))) {
    ...
    if (dockerfile != null) {
      ...
      if (!dockerfile.startsWith(contextDirectory)) {
        log.error("Dockerfile " + dockerfile + " is not a child of the context directory: " + contextDirectory);
