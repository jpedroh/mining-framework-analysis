...
public class JavaParserTest {
  @Test public void rangeOfAnnotationMemberDeclarationIsCorrect() {
  ...
  @Test public void rangeOfAnnotationMemberDeclarationWithArrayTypeIsCorrect() {
    String code = "@interface AD { String[] foo(); }";
    CompilationUnit cu = JavaParser.parse(code);
    AnnotationMemberDeclaration memberDeclaration = (AnnotationMemberDeclaration) cu.getAnnotationDeclarationByName("AD").get().getMember(0);
    assertEquals(true, memberDeclaration.getRange().isPresent());
  ...
  @Test public void rangeOfArrayCreationLevelWithExpressionIsCorrect() {
  ...
  @Test public void rangeOfArrayCreationLevelWithoutExpressionIsCorrect() {
    String code = "new int[][]";
    ArrayCreationExpr expression = JavaParser.parseExpression(code);
    Optional<Range> range;
    range = expression.getLevels().get(0).getRange();
  ...
  @Test public void parseErrorContainsLocation() {
  ...
  @Test public void parseIntersectionType() {
  ...
  @Test public void rangeOfIntersectionType() {
  ...
  @Test public void rangeOfCastNonIntersection() {
  ...
  @Test public void rangeOfLambda() {
    String code = "class A {" + EOL + "  Object f() {" + EOL + "    return (Comparator<Map.Entry<K, V>> & Serializable)(c1, c2) -> c1.getKey().compareTo(c2.getKey()); " + EOL + "}}";
    CompilationUnit cu = JavaParser.parse(code);
    MethodDeclaration methodDeclaration = (MethodDeclaration) cu.getClassByName("A").get().getMember(0);
    ReturnStmt returnStmt = (ReturnStmt) methodDeclaration.getBody().get().getStatement(0);
  ...
  @Test public void rangeOfLambdaBody() {
    String code = "class A {" + EOL + "  Object f() {" + EOL + "    return (Comparator<Map.Entry<K, V>> & Serializable)(c1, c2) -> c1.getKey().compareTo(c2.getKey()); " + EOL + "}}";
    CompilationUnit cu = JavaParser.parse(code);
    MethodDeclaration methodDeclaration = (MethodDeclaration) cu.getClassByName("A").get().getMember(0);
    ReturnStmt returnStmt = (ReturnStmt) methodDeclaration.getBody().get().getStatement(0);
  ...
  @Test public void testNotStoringTokens() {
  ...
  @Test(expected = ParseProblemException.class) public void trailingCodeIsAnError() {
    JavaParser.parseBlock("{} efijqoifjqefj");
  }

  @Test public void trailingWhitespaceIsIgnored() {
