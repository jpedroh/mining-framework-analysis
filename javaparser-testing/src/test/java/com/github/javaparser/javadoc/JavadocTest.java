...
public class JavadocTest {
  @Test public void toTextForEmptyJavadoc() {
    Javadoc javadoc = new Javadoc(new JavadocDescription());
    assertEquals("", javadoc.toText());
  }

  @Test public void toTextForJavadocWithTwoLinesOfJustDescription() {
  ...
  @Test public void toTextForJavadocWithTwoLinesOfJustDescriptionAndOneBlockTag() {
    Javadoc javadoc = new Javadoc(JavadocDescription.parseText("first line" + EOL + "second line"));
...
>>>>>>> /usr/src/app/output/javaparser/javaparser/2dfcfbf8118865639d2e2cafe1223d592804b470/javaparser-testing/src/test/java/com/github/javaparser/javadoc/JavadocTest.java/right.java
  ...
  @Test public void inlineTagsAreParsable() {
    String docText = "Returns the {@link TOFilename}s of all files that existed during the requested" + EOL + "{@link TOVersion}." + EOL + "" + EOL + "@param versionID the id of the {@link TOVersion}." + EOL + "@return the filenames" + EOL + "@throws InvalidIDException if the {@link IPersistence} doesn\'t recognize the given versionID." + EOL;
    String javadoc = JavaParser.parseJavadoc(docText).toText();
    assertTrue(javadoc.contains("{@link TOVersion}"));
  }

  @Test public void emptyLinesBetweenBlockTagsGetsFiltered() {
    String comment = " * The type of the Object to be mapped." + EOL + " * This interface maps the given Objects to existing ones in the database and" + EOL + " * saves them." + EOL + " * " + EOL + " * @author censored" + EOL + " * " + EOL + " * @param <T>" + EOL;
    Javadoc javadoc = JavaParser.parseJavadoc(comment);
    assertEquals(2, javadoc.getBlockTags().size());
  }

  @Test public void blockTagModificationWorks() {
    Javadoc javadoc = new Javadoc(new JavadocDescription());
  ...
  @Test public void descriptionModificationWorks() {
