...
public final class DefaultDomains implements Domains {
  ...
  @Override public Mono<Void> create(CreateDomainRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getOrganizationId(cloudFoundryClient, request.getOrganization()))).flatMap(function((cloudFoundryClient, organizationId) -> requestCreateDomain(cloudFoundryClient, request.getDomain(), organizationId))).then().transform(OperationsLogging.log("Create Domain")).checkpoint();
  }

  @Override public Mono<Void> createShared(CreateSharedDomainRequest request) {
    if (request.getRouterGroup() == null) {
      return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> requestCreateSharedDomain(cloudFoundryClient, request.getDomain(), null)).then().transform(OperationsLogging.log("Create Shared Domain")).checkpoint();
    } else {
  ...
  @Override public Flux<Domain> list() {
    return this.cloudFoundryClient.flatMapMany((cloudFoundryClient) -> requestListPrivateDomains(cloudFoundryClient).map(DefaultDomains::toDomain).mergeWith(requestListSharedDomains(cloudFoundryClient).map(DefaultDomains::toDomain))).transform(OperationsLogging.log("List Domains")).checkpoint();
  }

  @Override public Flux<RouterGroup> listRouterGroups() {
    return this.routingClient.flatMapMany(DefaultDomains::requestListRouterGroups).flatMapIterable(ListRouterGroupsResponse::getRouterGroups).map(DefaultDomains::toRouterGroup).transform(OperationsLogging.log("List Router Groups")).checkpoint();
  }

  @Override public Mono<Void> share(ShareDomainRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getPrivateDomainId(cloudFoundryClient, request.getDomain()), getOrganizationId(cloudFoundryClient, request.getOrganization()))).flatMap(function(DefaultDomains::requestAssociateOrganizationPrivateDomainRequest)).then().transform(OperationsLogging.log("Share Domain")).checkpoint();
  }

  @Override public Mono<Void> unshare(UnshareDomainRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getPrivateDomainId(cloudFoundryClient, request.getDomain()), getOrganizationId(cloudFoundryClient, request.getOrganization()))).flatMap(function(DefaultDomains::requestRemoveOrganizationPrivateDomainRequest)).transform(OperationsLogging.log("Unshare Domain")).checkpoint();
  }
  ...
  private static Mono<PrivateDomainResource> getPrivateDomain(CloudFoundryClient cloudFoundryClient, String domain) {
    return requestListPrivateDomains(cloudFoundryClient, domain).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Private domain %s does not exist", domain));
  }
  ...
  private static Mono<String> getRouterGroupId(RoutingClient routingClient, String routerGroup) {
    return requestListRouterGroups(routingClient).flatMapIterable(ListRouterGroupsResponse::getRouterGroups).filter((group) -> routerGroup.equals(group.getName())).single().map(org.cloudfoundry.routing.v1.routergroups.RouterGroup::getRouterGroupId);
  }
  ...
  private static Mono<CreateSharedDomainResponse> requestCreateSharedDomain(CloudFoundryClient cloudFoundryClient, String domain, String routerGroupId) {
    return cloudFoundryClient.sharedDomains().create(org.cloudfoundry.client.v2.shareddomains.CreateSharedDomainRequest.builder().name(domain).routerGroupId(routerGroupId).build());
  }
  ...
  private static Mono<ListRouterGroupsResponse> requestListRouterGroups(RoutingClient routingClient) {
    return routingClient.routerGroups().list(org.cloudfoundry.routing.v1.routergroups.ListRouterGroupsRequest.builder().build());
  }
  ...
  private static Domain toDomain(PrivateDomainResource resource) {
    ...
    return Domain.builder().id(ResourceUtils.getId(resource)).name(entity.getName()).status(Status.OWNED).build();
  }

  private static Domain toDomain(SharedDomainResource resource) {
    SharedDomainEntity entity = ResourceUtils.getEntity(resource);
    return Domain.builder().id(ResourceUtils.getId(resource)).name(entity.getName()).status(Status.SHARED).type(entity.getRouterGroupType()).build();
  }

  private static RouterGroup toRouterGroup(org.cloudfoundry.routing.v1.routergroups.RouterGroup group) {
    return RouterGroup.builder().id(group.getRouterGroupId()).name(group.getName()).type(group.getType()).build();
  }
}