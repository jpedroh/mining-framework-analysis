...
@Value.Immutable abstract class _DefaultCloudFoundryOperations implements CloudFoundryOperations {
  ...
  @Override @Value.Derived public UserAdmin userAdmin() {
  ...
  /**
     ...
     */
  @Nullable abstract CloudFoundryClient getCloudFoundryClient();

  @Value.Derived Mono<CloudFoundryClient> getCloudFoundryClientPublisher() {
    return Optional.ofNullable(getCloudFoundryClient()).map(Mono::just).orElse(Mono.error(new IllegalStateException("CloudFoundryClient must be set")));
  }

  /**
     * The {@link DopplerClient} to use for operations functionality
     */
  @Nullable abstract DopplerClient getDopplerClient();

  @Value.Derived Mono<DopplerClient> getDopplerClientPublisher() {
    return Optional.ofNullable(getDopplerClient()).map(Mono::just).orElse(Mono.error(new IllegalStateException("DopplerClient must be set")));
  }

  /**
     * The {@link NetworkingClient} to use for operations functionality
     */
  @Nullable abstract NetworkingClient getNetworkingClient();

  @Value.Derived Mono<NetworkingClient> getNetworkingClientPublisher() {
    return Optional.ofNullable(getNetworkingClient()).map(Mono::just).orElse(Mono.error(new IllegalStateException("NetworkingClient must be set")));
  }

  /**
     * The organization to target
     */
  @Nullable abstract String getOrganization();

  @Value.Derived Mono<String> getOrganizationId() {
    String organization = getOrganization();
    if (hasText(organization)) {
      Mono<String> cached = getOrganization(getCloudFoundryClientPublisher(), organization).map(ResourceUtils::getId);
      return getCacheDuration().map(cached::cache).orElseGet(cached::cache);
    } else {
  ...
  /**
     ...
     */
  @Nullable abstract RoutingClient getRoutingClient();

  @Value.Derived Mono<RoutingClient> getRoutingClientPublisher() {
    return Optional.ofNullable(getRoutingClient()).map(Mono::just).orElse(Mono.error(new IllegalStateException("RoutingClient must be set")));
  }

  /**
     * The space to target
     */
  @Nullable abstract String getSpace();

  @Value.Derived Mono<String> getSpaceId() {
    String space = getSpace();
    if (hasText(getSpace())) {
      Mono<String> cached = getOrganizationId().flatMap((organizationId) -> getSpace(getCloudFoundryClientPublisher(), organizationId, space)).map(ResourceUtils::getId);
      return getCacheDuration().map(cached::cache).orElseGet(cached::cache);
    } else {
  ...
  /**
     ...
     */
  @Nullable abstract UaaClient getUaaClient();

  @Value.Derived Mono<UaaClient> getUaaClientPublisher() {
    return Optional.ofNullable(getUaaClient()).map(Mono::just).orElse(Mono.error(new IllegalStateException("UaaClient must be set")));
  }

  @Value.Derived Mono<String> getUsername() {
    return getUaaClientPublisher().flatMap(UaaClient::getUsername);
  }
  ...
  private static Flux<OrganizationResource> requestOrganizations(Mono<CloudFoundryClient> cloudFoundryClientPublisher, String organization) {
    return cloudFoundryClientPublisher.flatMapMany((cloudFoundryClient) -> PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.organizations().list(ListOrganizationsRequest.builder().name(organization).page(page).build())));
  }
  ...
  private static Flux<SpaceResource> requestSpaces(Mono<CloudFoundryClient> cloudFoundryClientPublisher, String organizationId, String space) {
    return cloudFoundryClientPublisher.flatMapMany((cloudFoundryClient) -> PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.spaces().list(ListSpacesRequest.builder().organizationId(organizationId).name(space).page(page).build())));
  }
}