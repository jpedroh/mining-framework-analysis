...
public final class DefaultBuildpacks implements Buildpacks {
  ...
  @Override public Mono<Void> rename(RenameBuildpackRequest request) {
  ...
  @Override public Mono<Void> update(UpdateBuildpackRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(getBuildPackId(cloudFoundryClient, request.getName()), Mono.just(cloudFoundryClient))).flatMap(function((buildpackId, cloudFoundryClient) -> Mono.zip(requestUpdateBuildpack(cloudFoundryClient, buildpackId, request), uploadBuildpackBits(cloudFoundryClient, buildpackId, request)))).then().transform(OperationsLogging.log("Update Buildpack")).checkpoint();
  }

  private static Mono<Void> deleteBuildpack(CloudFoundryClient cloudFoundryClient, String buildpackId, Duration timeout) {
    return requestDeleteBuildpack(cloudFoundryClient, buildpackId).flatMap((job) -> JobUtils.waitForCompletion(cloudFoundryClient, timeout, job));
  }

  private static Mono<String> getBuildPackId(CloudFoundryClient cloudFoundryClient, String name) {
    return requestBuildpacks(cloudFoundryClient, name).singleOrEmpty().map(ResourceUtils::getId).switchIfEmpty(ExceptionUtils.illegalArgument("Buildpack %s not found", name));
  }
  ...
  private static Mono<DeleteBuildpackResponse> requestDeleteBuildpack(CloudFoundryClient cloudFoundryClient, String buildpackId) {
    return cloudFoundryClient.buildpacks().delete(org.cloudfoundry.client.v2.buildpacks.DeleteBuildpackRequest.builder().async(true).buildpackId(buildpackId).build());
  }
  ...
  private static Buildpack toBuildpackResource(BuildpackResource resource) {
    ...
    return Buildpack.builder().enabled(entity.getEnabled()).filename(entity.getFilename()).id(ResourceUtils.getId(resource)).locked(entity.getLocked()).name(entity.getName()).position(entity.getPosition()).build();
  }
  ...
  private static Mono<Void> uploadBuildpackBits(CloudFoundryClient cloudFoundryClient, String buildpackId, UpdateBuildpackRequest request) {
    if (request.getBuildpack() != null) {
      return requestUploadBuildpackBits(cloudFoundryClient, buildpackId, request.getBuildpack()).then(Mono.empty());
    }
    return Mono.empty();
  }
}