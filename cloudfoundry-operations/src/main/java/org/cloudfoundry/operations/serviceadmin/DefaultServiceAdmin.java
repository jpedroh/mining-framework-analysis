...
public final class DefaultServiceAdmin implements ServiceAdmin {
  ...
  @Override public Mono<Void> disableServiceAccess(DisableServiceAccessRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getServiceId(cloudFoundryClient, request.getServiceName()))).flatMap(function((cloudFoundryClient, serviceId) -> Mono.zip(Mono.just(cloudFoundryClient), getServicePlans(cloudFoundryClient, serviceId)))).flatMap(function((cloudFoundryClient, servicePlans) -> Mono.zip(updateServicePlanVisibilities(cloudFoundryClient, request, servicePlans), updateServicePlansPublicStatus(cloudFoundryClient, request, servicePlans)))).then().transform(OperationsLogging.log("Disable Service Access")).checkpoint();
  }

  @Override public Mono<Void> enableServiceAccess(EnableServiceAccessRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getServiceId(cloudFoundryClient, request.getServiceName()))).flatMap(function((cloudFoundryClient, serviceId) -> Mono.zip(Mono.just(cloudFoundryClient), getServicePlans(cloudFoundryClient, serviceId)))).flatMap(function((cloudFoundryClient, servicePlans) -> Mono.zip(updateServicePlanVisibilities(cloudFoundryClient, request, servicePlans), updateServicePlansPublicStatus(cloudFoundryClient, request, servicePlans)))).then().transform(OperationsLogging.log("Enable Service Access")).checkpoint();
  }
