...
public final class DefaultServiceAdmin implements ServiceAdmin {
  ...
  @Override public Mono<Void> create(CreateServiceBrokerRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.spaceId).flatMap(function((cloudFoundryClient, spaceId) -> requestCreateServiceBroker(cloudFoundryClient, request.getName(), request.getUrl(), request.getUsername(), request.getPassword(), request.getSpaceScoped(), spaceId))).then().transform(OperationsLogging.log("Create Service Broker")).checkpoint();
  }

  @Override public Mono<Void> delete(DeleteServiceBrokerRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getServiceBrokerId(cloudFoundryClient, request.getName()))).flatMap(function(DefaultServiceAdmin::requestDeleteServiceBroker)).transform(OperationsLogging.log("Delete Service Broker")).checkpoint();
  }

  @Override public Mono<Void> disableServiceAccess(DisableServiceAccessRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getServiceId(cloudFoundryClient, request.getServiceName()))).flatMap(function((cloudFoundryClient, serviceId) -> Mono.zip(Mono.just(cloudFoundryClient), getServicePlans(cloudFoundryClient, serviceId)))).flatMap(function((cloudFoundryClient, servicePlans) -> Mono.when(updateServicePlanVisibilities(cloudFoundryClient, request, servicePlans), updateServicePlansPublicStatus(cloudFoundryClient, request, servicePlans)))).then().transform(OperationsLogging.log("Disable Service Access")).checkpoint();
  }

  @Override public Mono<Void> enableServiceAccess(EnableServiceAccessRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), getServiceId(cloudFoundryClient, request.getServiceName()))).flatMap(function((cloudFoundryClient, serviceId) -> Mono.zip(Mono.just(cloudFoundryClient), getServicePlans(cloudFoundryClient, serviceId)))).flatMap(function((cloudFoundryClient, servicePlans) -> Mono.when(updateServicePlanVisibilities(cloudFoundryClient, request, servicePlans), updateServicePlansPublicStatus(cloudFoundryClient, request, servicePlans)))).then().transform(OperationsLogging.log("Enable Service Access")).checkpoint();
  }

  @Override public Flux<ServiceBroker> list() {
  ...
  @Override public Flux<ServiceAccess> listServiceAccessSettings(ListServiceAccessSettingsRequest request) {
    return this.cloudFoundryClient.flatMap((cloudFoundryClient) -> Mono.zip(Mono.just(cloudFoundryClient), listServiceBrokers(cloudFoundryClient), Mono.just(request), requestListServicePlanVisibilities(cloudFoundryClient))).flatMapMany(function(DefaultServiceAdmin::collectServiceAccessSettings)).transform(OperationsLogging.log("List Service Access Settings")).checkpoint();
  }

  @Override public Mono<Void> update(UpdateServiceBrokerRequest request) {
    return this.cloudFoundryClient.then((cloudFoundryClient) -> Mono.when(Mono.just(cloudFoundryClient), getServiceBrokerId(cloudFoundryClient, request.getName()))).then(function((cloudFoundryClient, serviceBrokerId) -> requestUpdateServiceBroker(cloudFoundryClient, request, serviceBrokerId))).then().transform(OperationsLogging.log("Update Service Broker")).checkpoint();
  }
  ...
  private static Flux<ServiceAccess> collectServiceAccessSettings(CloudFoundryClient cloudFoundryClient, List<ServiceBrokerResource> brokers, ListServiceAccessSettingsRequest request, List<ServicePlanVisibilityResource> visibilities) {
    List<String> brokerIds = brokers.stream().filter(isRequiredServiceBroker(request.getBrokerName())).map(ResourceUtils::getId).collect(Collectors.toList());
    if (brokerIds.size() == 0) {
      throw new IllegalArgumentException(String.format("Service Broker %s not found", request.getBrokerName()));
    }
    return Mono.when(validateOrganization(cloudFoundryClient, request.getOrganizationName()), validateService(cloudFoundryClient, request.getServiceName())).thenMany(requestListServices(cloudFoundryClient, brokerIds)).filter((service) -> isVisibleService(request, service)).collectList().flatMapMany((services) -> getServicePlans(cloudFoundryClient, services).map((servicePlan) -> Tuples.of(services, servicePlan))).flatMap(function((services, servicePlan) -> getOrganizationNames(cloudFoundryClient, servicePlan, visibilities).map((organizationNames) -> Tuples.of(organizationNames, services, servicePlan)))).filter(predicate((organizationNames, services, servicePlan) -> isVisibleOrganization(organizationNames, request.getOrganizationName(), servicePlan))).map(function((organizationNames, services, servicePlan) -> toServiceAccess(brokers, organizationNames, services, servicePlan)));
  }
  ...
  private static Mono<ServiceBrokerResource> getServiceBroker(CloudFoundryClient cloudFoundryClient, String serviceBrokerName) {
    return requestListServiceBrokers(cloudFoundryClient, serviceBrokerName).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Service Broker %s not found", serviceBrokerName));
  }

  private static Mono<String> getServiceBrokerId(CloudFoundryClient cloudFoundryClient, String serviceBrokerName) {
    return getServiceBroker(cloudFoundryClient, serviceBrokerName).map(ResourceUtils::getId);
  }

  private static Mono<String> getServiceId(CloudFoundryClient cloudFoundryClient, String serviceName) {
    return requestListServices(cloudFoundryClient, serviceName).singleOrEmpty().map(ResourceUtils::getId).switchIfEmpty(ExceptionUtils.illegalArgument("Service offering %s not found", serviceName));
  }

  private static Mono<List<ServicePlanResource>> getServicePlans(CloudFoundryClient cloudFoundryClient, String serviceId) {
    return requestListServicePlans(cloudFoundryClient, Collections.singletonList(serviceId)).collectList();
  }
  ...
  private static boolean isVisibleOrganization(List<String> organizationNames, String requiredOrganization, ServicePlanResource servicePlan) {
    return ResourceUtils.getEntity(servicePlan).getPubliclyVisible() || requiredOrganization == null || requiredOrganization.isEmpty() || organizationNames != null && organizationNames.contains(requiredOrganization);
  }

  private static boolean isVisibleService(ListServiceAccessSettingsRequest request, ServiceResource service) {
    return request.getServiceName() == null || request.getServiceName().isEmpty() || request.getServiceName().equals(ResourceUtils.getEntity(service).getLabel());
  }

  private static Mono<List<ServiceBrokerResource>> listServiceBrokers(CloudFoundryClient cloudFoundryClient) {
    return requestListServiceBrokers(cloudFoundryClient).switchIfEmpty(ExceptionUtils.illegalArgument("No Service Brokers found")).collectList();
  }

  private static Flux<String> listServicePlanVisibilityIds(CloudFoundryClient cloudFoundryClient, List<String> servicePlanIds) {
    return requestListServicePlanVisibilities(cloudFoundryClient, servicePlanIds).map(ResourceUtils::getId);
  }
  ...
  private static Flux<String> listServicePlanVisibilityIds(CloudFoundryClient cloudFoundryClient, String organizationId, List<String> servicePlanIds) {
    return requestListServicePlanVisibilities(cloudFoundryClient, organizationId, servicePlanIds).map(ResourceUtils::getId);
  }
  ...
  private static Mono<CreateServiceBrokerResponse> requestCreateServiceBroker(CloudFoundryClient cloudFoundryClient, String name, String url, String username, String password, Boolean isSpaceScoped, String spaceId) {
    return cloudFoundryClient.serviceBrokers().create(org.cloudfoundry.client.v2.servicebrokers.CreateServiceBrokerRequest.builder().name(name).brokerUrl(url).authenticationUsername(username).authenticationPassword(password).spaceId(Optional.ofNullable(isSpaceScoped).orElse(false) ? spaceId : null).build());
  }
  ...
  private static Mono<DeleteServicePlanVisibilityResponse> requestDeleteServicePlanVisibility(CloudFoundryClient cloudFoundryClient, String visibilityId) {
    return cloudFoundryClient.servicePlanVisibilities().delete(DeleteServicePlanVisibilityRequest.builder().async(true).servicePlanVisibilityId(visibilityId).build());
  }
  ...
  private static Flux<ServicePlanVisibilityResource> requestListServicePlanVisibilities(CloudFoundryClient cloudFoundryClient, List<String> servicePlanIds) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.servicePlanVisibilities().list(ListServicePlanVisibilitiesRequest.builder().page(page).servicePlanIds(servicePlanIds).build()));
  }
  ...
  private static Flux<ServicePlanVisibilityResource> requestListServicePlanVisibilities(CloudFoundryClient cloudFoundryClient, String organizationId, List<String> servicePlanIds) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.servicePlanVisibilities().list(ListServicePlanVisibilitiesRequest.builder().organizationId(organizationId).page(page).servicePlanIds(servicePlanIds).build()));
  }
  ...
  private static Flux<ServicePlanResource> requestListServicePlans(CloudFoundryClient cloudFoundryClient, List<String> services) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.servicePlans().list(ListServicePlansRequest.builder().page(page).serviceIds(services).build()));
  }

  private static Flux<ServiceResource> requestListServices(CloudFoundryClient cloudFoundryClient, List<String> brokerIds) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.services().list(ListServicesRequest.builder().page(page).serviceBrokerIds(brokerIds).build()));
  }
  ...
  private static Mono<UpdateServiceBrokerResponse> requestUpdateServiceBroker(CloudFoundryClient cloudFoundryClient, UpdateServiceBrokerRequest request, String serviceBrokerId) {
    return cloudFoundryClient.serviceBrokers().update(org.cloudfoundry.client.v2.servicebrokers.UpdateServiceBrokerRequest.builder().serviceBrokerId(serviceBrokerId).name(request.getName()).brokerUrl(request.getUrl()).authenticationUsername(request.getUsername()).authenticationPassword(request.getPassword()).build());
  }
  ...
  private static Mono<UpdateServicePlanResponse> requestUpdateServicePlanPublicStatus(CloudFoundryClient cloudFoundryClient, boolean publiclyVisible, String servicePlanId) {
    return cloudFoundryClient.servicePlans().update(UpdateServicePlanRequest.builder().publiclyVisible(publiclyVisible).servicePlanId(servicePlanId).build());
  }
  ...
  private static ServiceAccess toServiceAccess(List<ServiceBrokerResource> brokers, List<String> organizationNames, List<ServiceResource> services, ServicePlanResource servicePlan) {
    ...
    ServiceResource service = services.stream().filter((item) -> ResourceUtils.getId(item).equals(servicePlan.getEntity().getServiceId())).findFirst().orElseThrow(() -> new IllegalStateException(String.format("Unable to find service for %s", servicePlanName)));
    String serviceName = ResourceUtils.getEntity(service).getLabel();
    String brokerName = brokers.stream().filter((broker) -> ResourceUtils.getId(broker).equals(ResourceUtils.getEntity(service).getServiceBrokerId())).map((broker) -> ResourceUtils.getEntity(broker).getName()).findFirst().orElseThrow(() -> new IllegalStateException(String.format("Unable to find broker for %s", serviceName)));
  ...
  private static Mono<Void> updateServicePlanVisibilities(CloudFoundryClient cloudFoundryClient, EnableServiceAccessRequest request, List<ServicePlanResource> servicePlans) {
    List<String> servicePlanIds = servicePlans.stream().filter((servicePlan) -> isUpdateableServicePlan(request.getServicePlanName(), servicePlan)).map(ResourceUtils::getId).collect(Collectors.toList());
    if (request.getOrganizationName() != null && !request.getOrganizationName().isEmpty()) {
      return getOrganizationId(cloudFoundryClient, request.getOrganizationName()).flatMapMany((organizationId) -> Flux.fromIterable(servicePlanIds).flatMap((servicePlanId) -> requestCreateServicePlanVisibility(cloudFoundryClient, organizationId, servicePlanId))).then();
    } else {
      return listServicePlanVisibilityIds(cloudFoundryClient, servicePlanIds).flatMap((visibilityId) -> requestDeleteServicePlanVisibility(cloudFoundryClient, visibilityId).flatMap((job) -> JobUtils.waitForCompletion(cloudFoundryClient, request.getCompletionTimeout(), job))).then();
    }
  }
  ...
  private static Mono<Void> updateServicePlanVisibilities(CloudFoundryClient cloudFoundryClient, DisableServiceAccessRequest request, List<ServicePlanResource> servicePlans) {
    List<String> servicePlanIds = servicePlans.stream().filter((servicePlan) -> isUpdateableServicePlan(request.getServicePlanName(), servicePlan)).map(ResourceUtils::getId).collect(Collectors.toList());
    if (request.getOrganizationName() != null && !request.getOrganizationName().isEmpty()) {
      return getOrganizationId(cloudFoundryClient, request.getOrganizationName()).flatMap((organizationId) -> listServicePlanVisibilityIds(cloudFoundryClient, organizationId, servicePlanIds).flatMap((visibilityId) -> requestDeleteServicePlanVisibility(cloudFoundryClient, visibilityId).flatMap((job) -> JobUtils.waitForCompletion(cloudFoundryClient, request.getCompletionTimeout(), job))).then());
    } else {
      return listServicePlanVisibilityIds(cloudFoundryClient, servicePlanIds).flatMap((visibilityId) -> requestDeleteServicePlanVisibility(cloudFoundryClient, visibilityId).flatMap((job) -> JobUtils.waitForCompletion(cloudFoundryClient, request.getCompletionTimeout(), job))).then();
    }
  }
  ...
  private static Mono<Void> updateServicePlansPublicStatus(CloudFoundryClient cloudFoundryClient, DisableServiceAccessRequest request, List<ServicePlanResource> servicePlans) {
    ...
    return Flux.fromIterable(servicePlans).filter((servicePlan) -> ResourceUtils.getEntity(servicePlan).getPubliclyVisible()).flatMap((servicePlan) -> requestUpdateServicePlanPublicStatus(cloudFoundryClient, false, ResourceUtils.getId(servicePlan))).then();
  }
  ...
  private static Mono<Void> updateServicePlansPublicStatus(CloudFoundryClient cloudFoundryClient, EnableServiceAccessRequest request, List<ServicePlanResource> servicePlans) {
    ...
    return Flux.fromIterable(servicePlans).filter((servicePlan) -> !ResourceUtils.getEntity(servicePlan).getPubliclyVisible()).flatMap((servicePlan) -> requestUpdateServicePlanPublicStatus(cloudFoundryClient, true, ResourceUtils.getId(servicePlan))).then();
  }

  private static Mono<Void> validateOrganization(CloudFoundryClient cloudFoundryClient, String organizationName) {
    if (organizationName != null) {
      return requestListOrganizations(cloudFoundryClient, organizationName).switchIfEmpty(ExceptionUtils.illegalArgument("Organization %s not found", organizationName)).then();
    } else {
      return Mono.empty();
    }
  }
  ...
  private static Mono<Void> validateService(CloudFoundryClient cloudFoundryClient, String serviceName) {
    if (serviceName != null) {
      return requestListServices(cloudFoundryClient, serviceName).switchIfEmpty(ExceptionUtils.illegalArgument("Service %s not found", serviceName)).then();
    } else {
      return Mono.empty();
    }
  }
  ...
  private ServiceBroker toServiceBroker(ServiceBrokerResource resource) {
    ...
    return ServiceBroker.builder().id(ResourceUtils.getId(resource)).name(entity.getName()).url(entity.getBrokerUrl()).build();
  }
}