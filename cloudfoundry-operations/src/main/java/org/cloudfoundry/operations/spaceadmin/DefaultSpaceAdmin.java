...
public final class DefaultSpaceAdmin implements SpaceAdmin {
  ...
  @Override public Mono<SpaceQuota> get(GetSpaceQuotaRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> getSpaceQuotaDefinition(cloudFoundryClient, organizationId, request.getName()))).map(DefaultSpaceAdmin::toSpaceQuota).transform(OperationsLogging.log("Get Space Quota")).checkpoint();
  }

  @Override public Flux<SpaceQuota> listQuotas() {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMapMany(function(DefaultSpaceAdmin::requestSpaceQuotaDefinitions)).map(DefaultSpaceAdmin::toSpaceQuota).transform(OperationsLogging.log("List Space Quota")).checkpoint();
  }
  ...
  private static Mono<SpaceQuotaDefinitionResource> getSpaceQuotaDefinition(CloudFoundryClient cloudFoundryClient, String organizationId, String name) {
    return requestSpaceQuotaDefinitions(cloudFoundryClient, organizationId).filter((resource) -> name.equals(ResourceUtils.getEntity(resource).getName())).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Space Quota %s does not exist", name));
  }
