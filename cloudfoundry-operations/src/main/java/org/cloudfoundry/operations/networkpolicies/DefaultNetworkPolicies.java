...
public final class DefaultNetworkPolicies implements NetworkPolicies {
  ...
  @Override public Flux<Void> add(AddNetworkPolicyRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.networkingClient, this.spaceId).flatMapMany(function((cloudFoundryClient, networkingClient, spaceId) -> Mono.zip(Mono.just(networkingClient), getApplicationsByName(cloudFoundryClient, spaceId)))).flatMap(function((networkingClient, applications) -> requestAddPolicy(networkingClient, applications, request))).transform(OperationsLogging.log("Add Network Policy")).checkpoint();
  }

  @Override public Flux<Policy> list(ListNetworkPoliciesRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.networkingClient, this.spaceId).flatMapMany(function((cloudFoundryClient, networkingClient, spaceId) -> Mono.zip(getApplicationsById(cloudFoundryClient, spaceId), getPolicies(networkingClient)))).flatMap(function((applications, policies) -> toPolicy(applications, policies, request))).transform(OperationsLogging.log("List Network Policies")).checkpoint();
  }

  @Override public Flux<Void> remove(RemoveNetworkPolicyRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.networkingClient, this.spaceId).flatMapMany(function((cloudFoundryClient, networkingClient, spaceId) -> Mono.zip(Mono.just(networkingClient), getApplicationsByName(cloudFoundryClient, spaceId)))).flatMap(function((networkingClient, applications) -> requestRemovePolicy(networkingClient, applications, request))).transform(OperationsLogging.log("Remove Network Policy")).checkpoint();
  }

  private static Mono<Map<String, String>> getApplicationsById(CloudFoundryClient cloudFoundryClient, String spaceId) {
    return requestListApplications(cloudFoundryClient, spaceId).map((resource) -> Tuples.of(resource.getId(), resource.getName())).collectMap(function((id, name) -> id), function((id, name) -> name));
  }

  private static Mono<Map<String, String>> getApplicationsByName(CloudFoundryClient cloudFoundryClient, String spaceId) {
    return requestListApplications(cloudFoundryClient, spaceId).map((resource) -> Tuples.of(resource.getId(), resource.getName())).collectMap(function((id, name) -> name), function((id, name) -> id));
  }
  ...
  private static Flux<ApplicationResource> requestListApplications(CloudFoundryClient cloudFoundryClient, String spaceId) {
    return PaginationUtils.requestClientV3Resources((page) -> cloudFoundryClient.applicationsV3().list(ListApplicationsRequest.builder().page(page).spaceId(spaceId).build()));
  }
  ...
  private static Flux<Policy> toPolicy(Map<String, String> applications, List<org.cloudfoundry.networking.v1.policies.Policy> policies, ListNetworkPoliciesRequest request) {
    return Flux.fromIterable(policies).filter((policy) -> null != applications.get(policy.getSource().getId()) && null != applications.get(policy.getDestination().getId())).filter((policy) -> request.getSource() == null || request.getSource().equals(applications.get(policy.getSource().getId()))).map((policy) -> Policy.builder().destination(applications.get(policy.getDestination().getId())).endPort(policy.getDestination().getPorts().getEnd()).startPort(policy.getDestination().getPorts().getStart()).protocol(policy.getDestination().getProtocol()).source(applications.get(policy.getSource().getId())).build());
  }
}