...
public final class DefaultRoutes implements Routes {
  ...
  private static Mono<Optional<String>> getServiceName(CloudFoundryClient cloudFoundryClient, RouteEntity route) {
    return Mono.justOrEmpty(route.getServiceInstanceId()).flatMap((serviceInstanceId) -> getServiceInstanceName(cloudFoundryClient, serviceInstanceId, route.getSpaceId())).defaultIfEmpty(Optional.empty());
  }
