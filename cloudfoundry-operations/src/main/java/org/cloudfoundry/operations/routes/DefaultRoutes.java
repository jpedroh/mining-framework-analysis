...
public final class DefaultRoutes implements Routes {
  ...
  @Override public Flux<Route> list(ListRoutesRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getAllDomains(cloudFoundryClient, organizationId), getAllSpaces(cloudFoundryClient, organizationId)))).flatMapMany(function((cloudFoundryClient, domains, spaces) -> getRoutes(cloudFoundryClient, request, this.organizationId, this.spaceId).map((resource) -> Tuples.of(cloudFoundryClient, domains, resource, spaces)))).flatMap(function((cloudFoundryClient, domains, resource, spaces) -> Mono.zip(getApplicationNames(cloudFoundryClient, ResourceUtils.getId(resource)), getDomainName(domains, ResourceUtils.getEntity(resource).getDomainId()), Mono.just(resource), getSpaceName(spaces, ResourceUtils.getEntity(resource).getSpaceId())))).map(function(DefaultRoutes::toRoute)).transform(OperationsLogging.log("List Routes")).checkpoint();
  }
  ...
  private static Route toRoute(List<String> applications, String domain, RouteResource resource, String space) {
    ...
    return Route.builder().applications(applications).domain(domain).host(entity.getHost()).id(ResourceUtils.getId(resource)).path(entity.getPath()).space(space).build();
  }
