...
public final class DefaultRoutes implements Routes {
  ...
  @Override public Mono<Boolean> check(CheckRouteRequest request) {
  ...
  @Override public Mono<Integer> create(CreateRouteRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getSpaceId(cloudFoundryClient, organizationId, request.getSpace()), getDomainId(cloudFoundryClient, organizationId, request.getDomain())))).flatMap(function((cloudFoundryClient, spaceId, domainId) -> requestCreateRoute(cloudFoundryClient, domainId, request.getHost(), request.getPath(), request.getPort(), request.getRandomPort(), spaceId).map(ResourceUtils::getEntity).flatMap((routeEntity) -> Mono.justOrEmpty(routeEntity.getPort())))).transform(OperationsLogging.log("Create Route")).checkpoint();
  }

  @Override public Mono<Void> delete(DeleteRouteRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getDomainId(cloudFoundryClient, organizationId, request.getDomain())))).flatMap(function((cloudFoundryClient, domainId) -> Mono.zip(Mono.just(cloudFoundryClient), Mono.just(request.getCompletionTimeout()), getRouteId(cloudFoundryClient, request.getHost(), request.getDomain(), domainId, request.getPath(), request.getPort())))).flatMap(function(DefaultRoutes::deleteRoute)).transform(OperationsLogging.log("Delete Route")).checkpoint();
  }

  @Override public Mono<Void> deleteOrphanedRoutes(DeleteOrphanedRoutesRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.spaceId).flatMapMany(function((cloudFoundryClient, spaceId) -> requestSpaceRoutes(cloudFoundryClient, spaceId).filter((route) -> isRouteOrphan(ResourceUtils.getEntity(route))).map(ResourceUtils::getId).map((routeId) -> Tuples.of(cloudFoundryClient, routeId)))).flatMap(function((cloudFoundryClient, routeId) -> getApplications(cloudFoundryClient, routeId).map((applicationResources) -> Tuples.of(cloudFoundryClient, applicationResources, routeId)))).filter(predicate((cloudFoundryClient, applicationResources, routeId) -> isApplicationOrphan(applicationResources))).flatMap(function((cloudFoundryClient, applicationResources, routeId) -> deleteRoute(cloudFoundryClient, request.getCompletionTimeout(), routeId))).then().transform(OperationsLogging.log("Delete Orphaned Routes")).checkpoint();
  }

  @Override public Flux<Route> list(ListRoutesRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getAllDomains(cloudFoundryClient, organizationId), getAllSpaces(cloudFoundryClient, organizationId)))).flatMapMany(function((cloudFoundryClient, domains, spaces) -> getRoutes(cloudFoundryClient, request, this.organizationId, this.spaceId).map((route) -> Tuples.of(cloudFoundryClient, domains, route, spaces)))).flatMap(function((cloudFoundryClient, domains, route, spaces) -> Mono.
<<<<<<< /usr/src/app/output/cloudfoundry/cf-java-client/a9d35d542d88727dd302fb7a488891077bf28fb5/cloudfoundry-operations/src/main/java/org/cloudfoundry/operations/routes/DefaultRoutes.java/left.java
    zip(getApplicationNames(cloudFoundryClient, ResourceUtils.getId(resource)), getDomainName(domains, ResourceUtils.getEntity(resource).getDomainId()), Mono.just(resource), getSpaceName(spaces, ResourceUtils.getEntity(resource).getSpaceId()))
=======
    when(getApplicationNames(cloudFoundryClient, ResourceUtils.getId(route)), getDomainName(domains, ResourceUtils.getEntity(route).getDomainId()), Mono.just(route), getServiceName(cloudFoundryClient, ResourceUtils.getEntity(route)), getSpaceName(spaces, ResourceUtils.getEntity(route).getSpaceId()))
>>>>>>> /usr/src/app/output/cloudfoundry/cf-java-client/a9d35d542d88727dd302fb7a488891077bf28fb5/cloudfoundry-operations/src/main/java/org/cloudfoundry/operations/routes/DefaultRoutes.java/right.java
  ...
  private static Mono<List<String>> getApplicationNames(CloudFoundryClient cloudFoundryClient, String routeId) {
    return requestApplications(cloudFoundryClient, routeId).map((resource) -> ResourceUtils.getEntity(resource).getName()).collectList();
  }
  ...
  private static Mono<Resource<?>> getDomain(CloudFoundryClient cloudFoundryClient, String organizationId, String domain) {
    return getDomains(cloudFoundryClient, organizationId, domain).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Domain %s does not exist", domain));
  }

  private static Mono<String> getDomainId(CloudFoundryClient cloudFoundryClient, String organizationId, String domain) {
    return getDomain(cloudFoundryClient, organizationId, domain).map(ResourceUtils::getId);
  }
  ...
  private static Flux<Resource<?>> getDomains(CloudFoundryClient cloudFoundryClient, String organizationId, String domain) {
    return requestPrivateDomains(cloudFoundryClient, organizationId, domain).map((Function<PrivateDomainResource, Resource<?>>) (in) -> in).switchIfEmpty(requestSharedDomains(cloudFoundryClient, domain));
  }

  private static Mono<String> getOptionalDomainId(CloudFoundryClient cloudFoundryClient, String organizationId, String domain) {
    return getDomains(cloudFoundryClient, organizationId, domain).singleOrEmpty().map(ResourceUtils::getId);
  }

  private static Mono<AbstractRouteResource> getOrCreateRoute(CloudFoundryClient cloudFoundryClient, String organizationId, String spaceId, String domain, String host, String path, Integer port, Boolean randomPort) {
    if (randomPort != null) {
      return getDomainId(cloudFoundryClient, organizationId, domain).flatMap((domainId) -> requestCreateRoute(cloudFoundryClient, domainId, host, path, port, randomPort, spaceId));
    }
    return getDomainId(cloudFoundryClient, organizationId, domain).flatMap((domainId) -> getRoute(cloudFoundryClient, domainId, host, path, port).cast(AbstractRouteResource.class).switchIfEmpty(requestCreateRoute(cloudFoundryClient, domainId, host, path, port, randomPort, spaceId)));
  }
  ...
  private static Mono<RouteResource> getRoute(CloudFoundryClient cloudFoundryClient, String domainId, String domain, String host, String path, Integer port) {
    return getRoute(cloudFoundryClient, domainId, host, path, port).switchIfEmpty(ExceptionUtils.illegalArgument("Route for %s does not exist", domain));
  }
  ...
  private static Mono<RouteResource> getRoute(CloudFoundryClient cloudFoundryClient, String domainId, String host, String path, Integer port) {
    if (port != null) {
      return requestRoutes(cloudFoundryClient, domainId, null, null, port).singleOrEmpty();
    } else {
      return requestRoutes(cloudFoundryClient, domainId, host, path, port).filter((resource) -> isIdentical(nullSafe(host), ResourceUtils.getEntity(resource).getHost())).filter((resource) -> isIdentical(Optional.ofNullable(path).orElse(""), ResourceUtils.getEntity(resource).getPath())).singleOrEmpty();
    }
  }
  ...
  private static Mono<String> getRouteId(CloudFoundryClient cloudFoundryClient, String host, String domain, String domainId, String path, Integer port) {
    return getRoute(cloudFoundryClient, domainId, domain, host, path, port).map(ResourceUtils::getId);
  }
  ...
  private static Flux<RouteResource> getRoutes(CloudFoundryClient cloudFoundryClient, ListRoutesRequest request, Mono<String> organizationId, Mono<String> spaceId) {
    if (Level.ORGANIZATION == request.getLevel()) {
      return organizationId.flatMapMany((organizationId1) -> requestRoutes(cloudFoundryClient, (builder) -> builder.organizationId(organizationId1)));
    } else {
      return spaceId.flatMapMany((spaceId1) -> requestSpaceRoutes(cloudFoundryClient, spaceId1));
    }
  }
  ...
  private static Mono<Optional<String>> getServiceInstanceName(CloudFoundryClient cloudFoundryClient, String serviceInstanceId, String spaceId) {
    return requestListSpaceServiceInstances(cloudFoundryClient, spaceId).filter((resource) -> serviceInstanceId.equals(ResourceUtils.getId(resource))).single().map((resource) -> Optional.of(ResourceUtils.getEntity(resource).getName()));
  }

  private static Mono<Optional<String>> getServiceName(CloudFoundryClient cloudFoundryClient, RouteEntity route) {
    return Mono.justOrEmpty(route.getServiceInstanceId()).then((serviceInstanceId) -> getServiceInstanceName(cloudFoundryClient, serviceInstanceId, route.getSpaceId())).defaultIfEmpty(Optional.empty());
  }

  private static Mono<SpaceResource> getSpace(CloudFoundryClient cloudFoundryClient, String organizationId, String space) {
    return requestSpaces(cloudFoundryClient, organizationId, space).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Space %s does not exist", space));
  }

  private static Mono<String> getSpaceId(CloudFoundryClient cloudFoundryClient, String organizationId, String space) {
    return getSpace(cloudFoundryClient, organizationId, space).map(ResourceUtils::getId);
  }
  ...
  private static Mono<CreateRouteResponse> requestCreateRoute(CloudFoundryClient cloudFoundryClient, String domainId, String host, String path, Integer port, Boolean randomPort, String spaceId) {
    ...
    return cloudFoundryClient.routes().create(builder.domainId(domainId).spaceId(spaceId).build());
  }

  private static Mono<DeleteRouteResponse> requestDeleteRoute(CloudFoundryClient cloudFoundryClient, String routeId) {
    return cloudFoundryClient.routes().delete(org.cloudfoundry.client.v2.routes.DeleteRouteRequest.builder().async(true).routeId(routeId).build());
  }
  ...
  private static Flux<UnionServiceInstanceResource> requestListSpaceServiceInstances(CloudFoundryClient cloudFoundryClient, String spaceId) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.spaces().listServiceInstances(ListSpaceServiceInstancesRequest.builder().page(page).returnUserProvidedServiceInstances(true).spaceId(spaceId).build()));
  }
  ...
  private static Flux<RouteResource> requestRoutes(CloudFoundryClient cloudFoundryClient, String domainId, String host, String path, Integer port) {
    return requestRoutes(cloudFoundryClient, (builder) -> builder.domainId(domainId).hosts(Optional.ofNullable(host).map(Collections::singletonList).orElse(null)).paths(Optional.ofNullable(path).map(Collections::singletonList).orElse(null)).port(Optional.ofNullable(port).orElse(null)));
  }
