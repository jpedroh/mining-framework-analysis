...
public final class DefaultSpaces implements Spaces {
  ...
  @Override public Mono<Void> allowSsh(AllowSpaceSshRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getOrganizationSpaceIdWhere(cloudFoundryClient, organizationId, request.getName(), sshEnabled(false))))).flatMap(function((cloudFoundryClient, spaceId) -> requestUpdateSpaceSsh(cloudFoundryClient, spaceId, true))).then().transform(OperationsLogging.log("Allow Space SSH")).checkpoint();
  }

  @Override public Mono<Void> create(CreateSpaceRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.username).flatMap(function((cloudFoundryClient, username) -> Mono.zip(Mono.just(cloudFoundryClient), Mono.just(username), getOrganizationIdOrDefault(cloudFoundryClient, request.getOrganization(), this.organizationId)))).flatMap(function((cloudFoundryClient, username, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), Mono.just(username), Mono.just(organizationId), getOptionalSpaceQuotaId(cloudFoundryClient, organizationId, request.getSpaceQuota())))).flatMap(function((cloudFoundryClient, username, organizationId, spaceQuotaId) -> Mono.zip(Mono.just(cloudFoundryClient), Mono.just(organizationId), requestCreateSpace(cloudFoundryClient, organizationId, request.getName(), spaceQuotaId.orElse(null)).map(ResourceUtils::getId), Mono.just(username)))).delayUntil(function((cloudFoundryClient, organizationId, spaceId, username) -> requestAssociateOrganizationUserByUsername(cloudFoundryClient, organizationId, username))).flatMap(function((cloudFoundryClient, organizationId, spaceId, username) -> Mono.zip(requestAssociateSpaceManagerByUsername(cloudFoundryClient, spaceId, username), requestAssociateSpaceDeveloperByUsername(cloudFoundryClient, spaceId, username)))).then().transform(OperationsLogging.log("Create Space")).checkpoint();
  }

  @Override public Mono<Void> delete(DeleteSpaceRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), Mono.just(request.getCompletionTimeout()), getOrganizationSpaceId(cloudFoundryClient, organizationId, request.getName())))).flatMap(function(DefaultSpaces::deleteSpace)).transform(OperationsLogging.log("Delete Space")).checkpoint();
  }

  @Override public Mono<Void> disallowSsh(DisallowSpaceSshRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getOrganizationSpaceIdWhere(cloudFoundryClient, organizationId, request.getName(), sshEnabled(true))))).flatMap(function((cloudFoundryClient, spaceId) -> requestUpdateSpaceSsh(cloudFoundryClient, spaceId, false))).then().transform(OperationsLogging.log("Disallow Space SSH")).checkpoint();
  }

  @Override public Mono<SpaceDetail> get(GetSpaceRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getOrganizationSpace(cloudFoundryClient, organizationId, request.getName())))).flatMap(function((cloudFoundryClient, resource) -> getSpaceDetail(cloudFoundryClient, resource, request))).transform(OperationsLogging.log("Get Space")).checkpoint();
  }

  @Override public Flux<SpaceSummary> list() {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMapMany(function(DefaultSpaces::requestSpaces)).map(DefaultSpaces::toSpaceSummary).transform(OperationsLogging.log("List Spaces")).checkpoint();
  }

  @Override public Mono<Void> rename(RenameSpaceRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(Mono.just(cloudFoundryClient), getOrganizationSpaceId(cloudFoundryClient, organizationId, request.getName())))).flatMap(function((cloudFoundryClient, spaceId) -> requestUpdateSpace(cloudFoundryClient, spaceId, request.getNewName()))).then().transform(OperationsLogging.log("Rename Space")).checkpoint();
  }

  @Override public Mono<Boolean> sshAllowed(SpaceSshAllowedRequest request) {
    return Mono.zip(this.cloudFoundryClient, this.organizationId).flatMap(function((cloudFoundryClient, organizationId) -> getOrganizationSpace(cloudFoundryClient, organizationId, request.getName()))).map((resource) -> ResourceUtils.getEntity(resource).getAllowSsh()).transform(OperationsLogging.log("Is Space SSH Allowed")).checkpoint();
  }

  private static Mono<Void> deleteSpace(CloudFoundryClient cloudFoundryClient, Duration completionTimeout, String spaceId) {
    return requestDeleteSpace(cloudFoundryClient, spaceId).flatMap((job) -> JobUtils.waitForCompletion(cloudFoundryClient, completionTimeout, job));
  }

  private static Mono<List<String>> getApplicationNames(CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
    return requestSpaceApplications(cloudFoundryClient, ResourceUtils.getId(spaceResource)).map((applicationResource) -> ResourceUtils.getEntity(applicationResource).getName()).collectList();
  }

  private static Mono<List<String>> getDomainNames(CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
    return requestListPrivateDomains(cloudFoundryClient, spaceResource.getEntity().getOrganizationId()).map((resource) -> resource.getEntity().getName()).mergeWith(requestListSharedDomains(cloudFoundryClient).map((resource) -> resource.getEntity().getName())).collectList();
  }
  ...
  private static Mono<Optional<SpaceQuota>> getOptionalSpaceQuotaDefinition(CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
    ...
    return requestSpaceQuotaDefinition(cloudFoundryClient, spaceQuotaDefinitionId).map(DefaultSpaces::toSpaceQuotaDefinition).map(Optional::of);
  }
  ...
  private static Mono<Optional<String>> getOptionalSpaceQuotaId(CloudFoundryClient cloudFoundryClient, String organizationId, String spaceQuota) {
    if (spaceQuota == null) {
      ...
      return getSpaceQuota(cloudFoundryClient, organizationId, spaceQuota).map(ResourceUtils::getId).map(Optional::of);
    }
  }

  private static Mono<OrganizationResource> getOrganization(CloudFoundryClient cloudFoundryClient, String organization) {
    return requestOrganizations(cloudFoundryClient, organization).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Organization %s does not exist", organization));
  }
  ...
  private static Mono<String> getOrganizationIdOrDefault(CloudFoundryClient cloudFoundryClient, String organizationName, Mono<String> organizationId) {
    return Optional.ofNullable(organizationName).map((organization) -> getOrganizationId(cloudFoundryClient, organization)).orElse(organizationId);
  }

  private static Mono<String> getOrganizationName(CloudFoundryClient cloudFoundryClient, SpaceResource resource) {
    return requestOrganization(cloudFoundryClient, ResourceUtils.getEntity(resource).getOrganizationId()).map((response) -> ResourceUtils.getEntity(response).getName());
  }
  ...
  private static Mono<SpaceResource> getOrganizationSpace(CloudFoundryClient cloudFoundryClient, String organizationId, String space) {
    return requestOrganizationSpaces(cloudFoundryClient, organizationId, space).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Space %s does not exist", space));
  }
  ...
  private static Mono<String> getOrganizationSpaceId(CloudFoundryClient cloudFoundryClient, String organizationId, String space) {
    return getOrganizationSpace(cloudFoundryClient, organizationId, space).map(ResourceUtils::getId);
  }
  ...
  private static Mono<String> getOrganizationSpaceIdWhere(CloudFoundryClient cloudFoundryClient, String organizationId, String space, Predicate<SpaceResource> predicate) {
    return getOrganizationSpace(cloudFoundryClient, organizationId, space).filter(predicate).map(ResourceUtils::getId);
  }
  ...
  private static Mono<List<String>> getServiceNames(CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
    return requestSpaceServices(cloudFoundryClient, ResourceUtils.getId(spaceResource)).map((serviceResource) -> ResourceUtils.getEntity(serviceResource).getLabel()).collectList();
  }
  ...
  private static Mono<SpaceDetail> getSpaceDetail(CloudFoundryClient cloudFoundryClient, SpaceResource resource, GetSpaceRequest request) {
    return Mono.zip(getApplicationNames(cloudFoundryClient, resource), getDomainNames(cloudFoundryClient, resource), getOrganizationName(cloudFoundryClient, resource), getSecurityGroups(cloudFoundryClient, resource, Optional.ofNullable(request.getSecurityGroupRules()).orElse(false)), getServiceNames(cloudFoundryClient, resource), getOptionalSpaceQuotaDefinition(cloudFoundryClient, resource)).map(function((applications, domains, organization, securityGroups, services, spaceQuota) -> toSpaceDetail(applications, domains, organization, resource, securityGroups, services, spaceQuota)));
  }
  ...
  private static Mono<SpaceQuotaDefinitionResource> getSpaceQuota(CloudFoundryClient cloudFoundryClient, String organizationId, String spaceQuota) {
    return requestOrganizationSpaceQuotas(cloudFoundryClient, organizationId, spaceQuota).single().onErrorResume(NoSuchElementException.class, (t) -> ExceptionUtils.illegalArgument("Space quota definition %s does not exist", spaceQuota));
  }
  ...
  private static Mono<DeleteSpaceResponse> requestDeleteSpace(CloudFoundryClient cloudFoundryClient, String spaceId) {
    return cloudFoundryClient.spaces().delete(org.cloudfoundry.client.v2.spaces.DeleteSpaceRequest.builder().async(true).spaceId(spaceId).build());
  }
  ...
  private static Flux<SpaceQuotaDefinitionResource> requestOrganizationSpaceQuotas(CloudFoundryClient cloudFoundryClient, String organizationId, String spaceQuota) {
    return PaginationUtils.requestClientV2Resources((page) -> cloudFoundryClient.organizations().listSpaceQuotaDefinitions(ListOrganizationSpaceQuotaDefinitionsRequest.builder().page(page).organizationId(organizationId).build())).filter((resource) -> ResourceUtils.getEntity(resource).getName().equals(spaceQuota));
  }
  ...
  private static SpaceDetail toSpaceDetail(List<String> applications, List<String> domains, String organization, SpaceResource resource, List<SecurityGroupEntity> securityGroups, List<String> services, Optional<SpaceQuota> spaceQuota) {
    return SpaceDetail.builder().applications(applications).domains(domains).id(ResourceUtils.getId(resource)).name(ResourceUtils.getEntity(resource).getName()).organization(organization).securityGroups(toSpaceDetailSecurityGroups(securityGroups)).services(services).spaceQuota(spaceQuota).build();
  }
  ...
  private static List<SecurityGroup> toSpaceDetailSecurityGroups(List<SecurityGroupEntity> securityGroups) {
    return securityGroups.stream().map((entity) -> SecurityGroup.builder().name(entity.getName()).rules(toSpaceDetailSecurityGroupRules(entity.getRules())).build()).collect(Collectors.toList());
  }
  ...
  private static SpaceSummary toSpaceSummary(SpaceResource resource) {
    return SpaceSummary.builder().id(ResourceUtils.getId(resource)).name(ResourceUtils.getEntity(resource).getName()).build();
  }
}