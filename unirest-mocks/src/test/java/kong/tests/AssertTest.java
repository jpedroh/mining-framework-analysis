...
class AssertTest extends Base {
  @Test void canAssertANumberOfTimes() {
    Unirest.get(path).asEmpty();
    Unirest.get(path).asEmpty();
    Assert exp = client.assertThat(HttpMethod.GET, path);
    exp.wasInvokedTimes(2);
    assertException(() -> exp.wasInvokedTimes(1), "Incorrect number of invocations. Expected 1 got 2\n" + "GET http://basic");
    assertException(() -> exp.wasInvokedTimes(3), "Incorrect number of invocations. Expected 3 got 2\n" + "GET http://basic");
  }

  @Test void noExpectationsAtAll() {
    Unirest.get(path).asEmpty();
    client.verifyAll();
  }

  @Test void noExpectation() {
    client.expect(GET, otherPath);
    assertException(() -> client.verifyAll(), "A expectation was never invoked! GET http://other\n");
  }

  @Test void noInvocationHappened() {
    assertException(() -> client.assertThat(GET, path), "No Matching Invocation:: GET http://basic");
  }

  @Test void assertHeader() {
    Unirest.get(path).header("monster", "grover").asEmpty();
    Assert expect = client.assertThat(GET, path);
    expect.hadHeader("monster", "grover");
    assertException(() -> expect.hadHeader("monster", "oscar"), "No invocation found with header [monster: oscar]\nFound:\nmonster: grover");
  }

  @Test void canSetHeaderExpectationOnExpects() {
    client.expect(GET, path).header("monster", "grover");
    assertException(() -> client.verifyAll(), "A expectation was never invoked! GET http://basic\n" + "Headers:\n" + "monster: grover");
    Unirest.get(path).header("monster", "grover").asEmpty();
    client.verifyAll();
  }

  @Test public void expectAnyPath() {
    client.expect(HttpMethod.GET).thenReturn("woh");
    Unirest.get(path).asEmpty();
    client.verifyAll();
  }

  @Test void canExpectQueryParams() {
  ...
  @Test void canResetExpects() {
    client.expect(GET).thenReturn("HI");
    client.reset();
    assertEquals(null, Unirest.get(path).asString().getBody());
  }

  @Test void canBeStrictAndForbidAnythingWithoutAMatch() {
    client.defaultResponse().withStatus(400, "wtf").thenReturn("boo");
  ...
  @Test void canSetASupplierForTheReponseBody() {
  ...
  @Test void assertPostWithNoBody() {
