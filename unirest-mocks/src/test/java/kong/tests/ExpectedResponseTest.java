...
class ExpectedResponseTest extends Base {
  @Test void getRequestSummaryOnResponse() {
    client.expect(GET, "http://localhost/get/cheese/passed").queryString("params", "cheese").thenReturn();
    HttpRequestSummary sum = Unirest.get("http://localhost/get/{params}/passed").routeParam("params", "cheese").queryString("fruit", "apples").asEmpty().getRequestSummary();
  ...
  @Test void canExpectErrorsJustStatus() {
  ...
  @Test void simpleGetString() {
    client.expect(HttpMethod.GET, path).thenReturn("Hello World");
    assertEquals("Hello World", Unirest.get(path).asString().getBody());
  }

  @Test void simpleGetBytes() {
    client.expect(HttpMethod.GET, path).thenReturn("Hello World");
    byte[] body = Unirest.get(path).asBytes().getBody();
    assertEquals("Hello World", new String(body));
  }

  @Test void simpleJson() {
    client.expect(HttpMethod.GET, path).thenReturn("{\"fruit\": \"apple\"}");
    assertEquals("apple", Unirest.get(path).asJson().getBody().getObject().getString("fruit"));
  }

  @Test void setReturnAsJson() {
    client.expect(HttpMethod.GET, path).thenReturn(new JSONObject("{\"fruit\": \"apple\"}"));
    assertEquals("apple", Unirest.get(path).asJson().getBody().getObject().getString("fruit"));
  }

  @Test void canPassInAndReturnObjectsAsJson() {
    client.expect(HttpMethod.GET, path).thenReturn(new Pojo("apple"));
    Pojo pojo = Unirest.get(path).asObject(Pojo.class).getBody();
    assertEquals("apple", pojo.fruit);
  }

  @Test void willUseRequestObjectMapperIfSupplied() {
    client.expect(HttpMethod.GET, path).thenReturn(new Pojo("apple"));
    String pojo = Unirest.get(path).withObjectMapper(new DerpMapper()).asString().getBody();
    assertEquals("derp", pojo);
  }

  @Test void willUseConfigObjectMapperIfSupplied() {
    client.expect(HttpMethod.GET, path).thenReturn(new Pojo("apple"));
    Unirest.config().setObjectMapper(new DerpMapper());
    String pojo = Unirest.get(path).asString().getBody();
    assertEquals("derp", pojo);
  }

  @Test void canSetResponseHeaders() {
    client.expect(GET, path).thenReturn("foo").withHeader("monster", "grover");
  ...
  @Test void canReturnEmptyWithHeaders() {
    client.expect(GET, path).thenReturn().withHeader("monster", "grover");
    HttpResponse<String> rez = Unirest.get(path).asString();
    assertEquals(null, rez.getBody());
    assertEquals("grover", rez.getHeaders().getFirst("monster"));
  }

  @Test void shouldSerializeMockResponse() {
    MockClient mock = MockClient.register();
    mock.expect(HttpMethod.GET, "http://zombo.com").thenReturn(MockResponse.bad("Bad Request"));
  ...
  @Test void shouldSerializeJsonMockResponse() {
    MockClient mock = MockClient.register();
    mock.expect(HttpMethod.GET, "http://zombo.com").thenReturn(MockResponse.bad(new JSONObject(of("message", "howdy"))));
  ...
  @Test void shouldSerializeObjectMockResponse() {
    MockClient mock = MockClient.register();
    mock.expect(HttpMethod.GET, "http://zombo.com").thenReturn(MockResponse.bad(new Pojo("Pears")));
  ...
  private class DerpMapper implements ObjectMapper {
    @Override public <T extends java.lang.Object> T readValue(String value, Class<T> valueType) {
      return (T) value;
    }
