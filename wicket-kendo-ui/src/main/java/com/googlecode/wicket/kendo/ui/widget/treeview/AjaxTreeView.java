...
public class AjaxTreeView extends JQueryContainer implements ITreeViewListener {
  ...
  public void expandPath(AjaxRequestTarget target, String path) {
    ...
    target.appendJavaScript(JQueryUtils.trycatch(statement));
  }
  ...
  @Override public boolean isChangeEventEnabled() {
    return false;
  }

  @Override public boolean isExpandEventEnabled() {
    return false;
  }

  @Override public boolean isDropEventEnabled() {
    return false;
  }
  ...
  @Override public void onConfigure(JQueryBehavior behavior) {
    ...
    behavior.setOption("dataUrlField", Options.asString(TreeNodeFactory.URL_FIELD));
    behavior.setOption("dataTextField", Options.asString(TreeNodeFactory.TEXT_FIELD));
    if (this.templateBehavior != null) {
  ...
  @Override public void onExpand(AjaxRequestTarget target, int nodeId) {
  }

  @Override public void onChange(AjaxRequestTarget target, int nodeId, String nodePath) {
  }

  @Override public void onDrop(AjaxRequestTarget target, int nodeId, int parentId, String position) {
  }
  ...
  @Override public JQueryBehavior newWidgetBehavior(String selector) {
    return new AjaxTreeViewBehavior(selector, this.options, this) {
      ...
      @Override protected CharSequence getDataSourceUrl() {
        return AjaxTreeView.this.getCallbackUrl();
      }

      @Override protected void onConfigure(TreeViewDataSource dataSource) {
