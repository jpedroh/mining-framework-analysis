    ...
    if (!daoMethod.getName().startsWith("read")) {
      ...
      for (StackTraceElement stackTraceElement : stackTraceElements) {
        if (daoMethod.getDeclaringClass().getName().equals(stackTraceElement.getClassName()) && !daoMethod.getName().equals(stackTraceElement.getMethodName())) {
          throw new RuntimeException(Utils.formatMessage("Method {0}.{1}() was invoked from another method in the same " + "class - {2}.{3}(). DAO implementations should avoid this as it causes objects to be " + "indexed and cached twice per request.", daoMethod.getDeclaringClass().getSimpleName(), daoMethod.getName(), stackTraceElement.getClassName(), stackTraceElement.getMethodName()));
        }
      }
    }
  }
