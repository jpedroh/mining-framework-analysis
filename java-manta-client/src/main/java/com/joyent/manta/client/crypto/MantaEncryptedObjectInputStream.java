 ...
 * @since 3.0.0
 */
@SuppressWarnings(value = { "Duplicates" }) public class MantaEncryptedObjectInputStream extends MantaObjectInputStream {
  ...
  /**
     ...
     */
  public MantaEncryptedObjectInputStream(final MantaObjectInputStream backingStream, final SupportedCipherDetails cipherDetails, final SecretKey secretKey, final boolean authenticateCiphertext) {
  ...
  /**
     ...
     */
  public MantaEncryptedObjectInputStream(final MantaObjectInputStream backingStream, final SupportedCipherDetails cipherDetails, final SecretKey secretKey, final boolean authenticateCiphertext, final Long startPositionInclusive, final Long plaintextRangeLength) {
    super(backingStream);
    ...
    this.plaintextRangeLength = verifyAndConditionallyRecalculatePlaintextLength(plaintextRangeLength);
  ...
  /**
     ...
     */
  private Long verifyAndConditionallyRecalculatePlaintextLength(final Long aPlaintextRangeLength) {
    if (aPlaintextRangeLength == null) {
      return null;
    }
    final Long contentLength = super.getContentLength();
    Validate.notNull(contentLength, "Manta should always return a content-length");
    long ciphertextSizeCalculation = cipherDetails.ciphertextSize(aPlaintextRangeLength + initialBytesToSkip);
    if (ciphertextSizeCalculation > contentLength) {
      return HttpHelper.attemptToFindPlaintextSize(getResponse(), contentLength, cipherDetails);
    }
  ...
  private Mac findHmac() {
    ...
    if (hmacString == null) {
      String msg = String.format("HMAC header metadata [%s] was missing from object", MantaHttpHeaders.ENCRYPTION_HMAC_TYPE);
      MantaClientEncryptionException e = new MantaClientEncryptionException(msg);
      annotateException(e);
      throw e;
    }
    if (hmacString.isEmpty()) {
      String msg = String.format("HMAC header metadata [%s] was empty on object", MantaHttpHeaders.ENCRYPTION_HMAC_TYPE);
      MantaClientEncryptionException e = new MantaClientEncryptionException(msg);
      annotateException(e);
      throw e;
    }
  ...
  @Override public InputStream getBackingStream() {
  ...
  @Override public long skip(final long numberOfBytesToSkip) throws IOException {
    if (this.closed) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "EmptyStatement" }) private void readRemainingBytes() throws IOException {
