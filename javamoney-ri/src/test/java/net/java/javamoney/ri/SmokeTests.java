...
public class SmokeTests {
  ...
  @Test public void testCreateAmounts() {
    CurrencyUnit currency = Monetary.getCurrencyUnitProvider().get("ISO-4217", "CHF");
    MonetaryAmount amount1 = Monetary.getMonetaryAmountFactory().get(currency, 1.0d);
    MonetaryAmount amount2 = Monetary.getMonetaryAmountFactory().get(currency, 1.0d);
  ...
  @Test public void testCreateMoney() {
  ...
  @Test public void testGettingCurrenciesPerLocale() {
  ...
  @Test @Ignore public void testGettingParsers() {
    try {
      AmountParser parser = Monetary.getAmountParserFactory().getAmountParser(Locale.GERMANY);
      MonetaryAmount amount1 = parser.parse("CFH 123.45");
      StyleableAmountParser locParser = Monetary.getAmountParserFactory().getLocalizableAmountParser();
      MonetaryAmount amount2 = locParser.parse("CFH 123.45", Locale.CHINA);
    } catch (ParseException e) {
      logger.debug("Error", e);
    }
  }

  @Test @Ignore public void testGettingFormatters() {
    CurrencyUnit currency = Monetary.getCurrencyUnitProvider().get("ISO4217", "CHF");
    MonetaryAmount amount = Monetary.getMonetaryAmountFactory().get(currency, 1.0d);
    try {
      AmountFormatter formatter = Monetary.getAmountFormatterFactory().getAmountFormatter(Locale.GERMANY);
      String formatted = formatter.format(amount);
      assertEquals(1.0d, amount.doubleValue(), 0);
      StyleableAmountFormatter locFormatter = Monetary.getAmountFormatterFactory().getLocalizableAmountFormatter();
