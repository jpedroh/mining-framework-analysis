...
public class DispatcherHelper {
  ...
  /**
     ...
     */
  private static final Map<Class<?>, WeakReference<Method[]>> customValidations = Collections.synchronizedMap(new WeakHashMap<Class<?>, WeakReference<Method[]>>());
    ...
    return ctx.wrap(new Interceptor() {
      public Resolution intercept(ExecutionContext ctx) throws Exception {
        ...
        try {
          ...
          if (bean.getClass().isAnnotationPresent(RestActionBean.class)) {
            log.error("No handler method found for request with ActionBean [" + bean.getClass().getName() + "] and eventName [ " + eventName + "] and " + " request method [" + context.getRequest().getMethod() + "]");
            return new ErrorResolution(HttpServletResponse.SC_NOT_FOUND, "The requested handler method " + "(" + eventName + ") is not found on this RestActionBean (" + ctx.getActionBean().getClass().getSimpleName() + ")");
          } else {
            throw sse;
          }
        }
        if (handler == null) {
          if (bean.getClass().isAnnotationPresent(RestActionBean.class)) {
            ...
            throw new StripesServletException("No handler method found for request with ActionBean [" + bean.getClass().getName() + "] and eventName [ " + eventName + "] and " + " request method [" + context.getRequest().getMethod() + "]");
          }
        }
        log.debug("Resolved event: ", context.getEventName(), "; will invoke: ", bean.getClass().getSimpleName(), ".", handler.getName(), "() with ", "request method [", context.getRequest().getMethod(), "]");
  ...
  /**
     ...
     * otherwise.
     */
  public static Resolution doBindingAndValidation(final ExecutionContext ctx, final boolean validate) throws Exception {
    ...
    if (validations == null) {
      ...
      while (temp != null) {
        for (Method method : temp.getDeclaredMethods()) {
          ...
          if ((method.getAnnotation(ValidationMethod.class) != null) && ((args.length == 0) || (args.length == 1 && args[0].equals(ValidationErrors.class)))) {
    ...
    return ctx.wrap(new Interceptor() {
      public Resolution intercept(ExecutionContext ctx) throws Exception {
        ...
        if (!handler.getName().equalsIgnoreCase(requestMethod)) {
          ...
          if (!methodSupported) {
            String msg = "The handler method [" + handler.getName() + "] in ActionBean [" + bean.getClass().getName() + "] for eventName [ " + ctx.getActionBeanContext().getEventName() + "] does not support " + " the request method [" + requestMethod + "]";
            if (bean.getClass().isAnnotationPresent(RestActionBean.class)) {
              log.error(msg);
            ...
            log.debug("Invoking the handler method [" + handler.getName() + "] in ActionBean [" + bean.getClass().getName() + "] for eventName [" + ctx.getActionBeanContext().getEventName() + "] with request method [" + requestMethod + "]");
          }
        }
