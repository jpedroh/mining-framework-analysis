...
public class MockRoundtrip {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "unchecked" }) public <A extends ActionBean> A getActionBean(Class<A> type) {
  ...
  private static AnnotatedClassActionResolver getActionResolver(MockServletContext context) {
    for (Filter filter : context.getFilters()) {
      if (filter instanceof StripesFilter) {
        ActionResolver resolver = ((StripesFilter) filter).getInstanceConfiguration().getActionResolver();
  ...
  private static String getUrlBindingStub(Class<? extends ActionBean> clazz, MockServletContext context) {
