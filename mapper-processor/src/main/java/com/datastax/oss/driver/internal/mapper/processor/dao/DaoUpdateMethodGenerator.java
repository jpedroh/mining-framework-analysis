...
public class DaoUpdateMethodGenerator extends DaoMethodGenerator {
  ...
  protected Set<DaoReturnTypeKind> getSupportedReturnTypes() {
    return ImmutableSet.of(VOID, FUTURE_OF_VOID, RESULT_SET, BOUND_STATEMENT, FUTURE_OF_ASYNC_RESULT_SET, BOOLEAN, FUTURE_OF_BOOLEAN, REACTIVE_RESULT_SET);
  }

  @Override public Optional<MethodSpec> generate() {
    ...
    if (entityElement == null) {
      context.getMessager().error(methodElement, processedType, "%s methods must take the entity to update as the first parameter", Update.class.getSimpleName());
    ...
    methodBodyBuilder.addStatement("$T boundStatementBuilder = $L.boundStatementBuilder()", BoundStatementBuilder.class, statementName);
    ...
    if (customWhereClause.isEmpty()) {
      methodBodyBuilder.addStatement("$1L.set($2L, boundStatementBuilder, $3T.$4L)", helperFieldName, entityParameterName, NullSavingStrategy.class, nullSavingStrategy);
    } else {
      methodBodyBuilder.addStatement("$1T nullSavingStrategy = $1T.$2L", NullSavingStrategy.class, nullSavingStrategy);
      for (PropertyDefinition property : entityDefinition.getRegularColumns()) {
        GeneratedCodePatterns.setValue(property.getCqlName(), property.getType(), CodeBlock.of("$L.$L()", entityParameterName, property.getGetterName()), "boundStatementBuilder", methodBodyBuilder, enclosingClass, true);
      }
    }
    if (parameters.size() > 1) {
      List<? extends VariableElement> bindMarkers = parameters.subList(1, parameters.size());
      if (validateCqlNamesPresent(bindMarkers)) {
        GeneratedCodePatterns.bindParameters(bindMarkers, methodBodyBuilder, enclosingClass, context, false);
      } else {
        return Optional.empty();
      }
    }
    ...
    return Optional.of(GeneratedCodePatterns.override(methodElement, typeParameters).addCode(methodBody).build());
  }

  private void generatePrepareRequest(MethodSpec.Builder methodBuilder, String requestName, String helperFieldName) {
    Update annotation = methodElement.getAnnotation(Update.class);
    maybeAddWhereClause(methodBuilder, requestName, helperFieldName, annotation.customWhereClause());
  ...
  private void maybeAddWhereClause(MethodSpec.Builder methodBuilder, String requestName, String helperFieldName, String customWhereClause) {
    if (customWhereClause.isEmpty()) {
      methodBuilder.addCode("$[$1T $2L = $1T.newInstance((($4T)$3L.updateByPrimaryKey()", SimpleStatement.class, requestName, helperFieldName, DefaultUpdate.class);
    } else {
      methodBuilder.addCode("$[$1T $2L = $1T.newInstance((($5T)$3L.updateStart().whereRaw($4S)", SimpleStatement.class, requestName, helperFieldName, customWhereClause, DefaultUpdate.class);
    }
  }
  ...
  private void maybeAddIfClause(MethodSpec.Builder methodBuilder, Update annotation) {
    if (annotation.ifExists() && !annotation.customIfClause().isEmpty()) {
      context.getMessager().error(methodElement, processedType, "Invalid annotation parameters: %s cannot have both ifExists and customIfClause", Update.class.getSimpleName());
    }
