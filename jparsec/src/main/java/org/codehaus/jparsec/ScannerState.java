...
final class ScannerState extends ParseContext {
  ...
  final <T extends java.lang.Object> T run(Parser<T> parser) {
    if (!applyWithExceptionWrapped(parser)) {
      ParserException exception = new ParserException(renderError(), module, locator.locate(errorIndex()));
      throw exception;
    }
  ...
  private boolean applyWithExceptionWrapped(Parser<?> parser) {
    try {
      ...
      ParserException wrapper = new ParserException(e, null, module, locator.locate(getIndex()));
      throw wrapper;
    }
  }
}