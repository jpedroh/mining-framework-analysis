...
public class Game extends InputAdapter {
  ...
  private void generateWorldFromTileMap() {
    ...
    generator.generate(map, (Layer layer, TileInfo info) -> {
      return info.getBooleanProperty("Invulnerable", false) && info.getProperty("Type", "").equals("Floor");
    }, (Rectangle rect) -> {
      EntityCreator.createAndAddInvulnerableFloor(engine, physixSystem, rect, tileWidth, tileHeight);
    });
      ...
      for (int i = 0; i < map.getWidth(); i++) {
        for (int j = 0; j < map.getHeight(); j++) {
          if (tiles != null && tiles[i] != null && tiles[i][j] != null) {
            if (tiles[i][j].getIntProperty("Hitpoint", 0) != 0 && tiles[i][j].getProperty("Type", "").equals("Floor")) {
              EntityCreator.createAndAddVulnerableFloor(engine, physixSystem, i * map.getTileWidth() + 0.5f * map.getTileWidth(), j * map.getTileHeight() + 0.5f * map.getTileHeight(), map.getTileWidth(), map.getTileHeight());
            }
          }
        }
      }
