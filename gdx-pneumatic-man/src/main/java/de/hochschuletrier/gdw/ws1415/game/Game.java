...
public class Game extends InputAdapter {
  ...
  private final PooledEngine engine = new PooledEngine(GameConstants.ENTITY_POOL_INITIAL_SIZE, GameConstants.ENTITY_POOL_MAX_SIZE, GameConstants.COMPONENT_POOL_INITIAL_SIZE, GameConstants.COMPONENT_POOL_MAX_SIZE);

  private final PhysixSystem physixSystem = new PhysixSystem(GameConstants.BOX2D_SCALE, GameConstants.VELOCITY_ITERATIONS, GameConstants.POSITION_ITERATIONS, GameConstants.PRIORITY_PHYSIX);
...
<<<<<<< /usr/src/app/output/lusito/gamedevweek/d6b031e2569face0946671177f28d977474ef5c2/gdx-pneumatic-man/src/main/java/de/hochschuletrier/gdw/ws1415/game/Game.java/left.java
  private final AISystem aisystems = new AISystem(GameConstants.PRIORITY_PHYSIX + 1, physixSystem);
=======
  private final HashMap<TileSet, Texture> tilesetImages = new HashMap();
>>>>>>> /usr/src/app/output/lusito/gamedevweek/d6b031e2569face0946671177f28d977474ef5c2/gdx-pneumatic-man/src/main/java/de/hochschuletrier/gdw/ws1415/game/Game.java/right.java
  ...
  private void generateWorldFromTileMap() {
    ...
    generator.generate(map, (Layer layer, TileInfo info) -> info.getBooleanProperty("Invulnerable", false), (Rectangle rect) -> EntityCreator.createAndAddFloor(engine, physixSystem, rect, tileWidth, tileHeight));
            ...
            if (tiles[i] != null) {
              if (tiles[i][j] != null) {
                if (tiles[i][j].getIntProperty("Hitpoint", 0) != 0) {
                  EntityCreator.createAndAddFloor(engine, physixSystem, i * map.getTileWidth() + 0.5f * map.getTileWidth(), j * map.getTileHeight() + 0.5f * map.getTileHeight(), map.getTileWidth(), map.getTileHeight());
                }
              }
            }
          }
  ...
  public void createBall(float x, float y, float radius) {
    ...
    modifyComponent.schedule(() -> {
      ...
      PhysixBodyDef bodyDef = new PhysixBodyDef(BodyType.DynamicBody, physixSystem).position(x, y).fixedRotation(false);
      bodyComponent.init(bodyDef, physixSystem, entity);
      PhysixFixtureDef fixtureDef = new PhysixFixtureDef(physixSystem).density(5).friction(0.2f).restitution(0.4f).shapeCircle(radius);
