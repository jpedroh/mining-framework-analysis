...
public class autoAlgorithmWithWFGExperiments {
  ...
  static List<ExperimentAlgorithm<DoubleSolution, List<DoubleSolution>>> configureAlgorithmList(List<ExperimentProblem<DoubleSolution>> problemList) {
    ...
    for (int run = 0; run < INDEPENDENT_RUNS; run++) {
      ...
      for (ExperimentProblem<DoubleSolution> experimentProblem : problemList) {
        String[] parameters = ("--problemName " + experimentProblem.getProblem().getClass().getName() + " " + "--referenceFrontFileName " + experimentProblem.getReferenceFront() + " " + "--maximumNumberOfEvaluations 25000 " + "--algorithmResult externalArchive " + "--populationSizeWithArchive 75 " + "--populationSize 100 " + "--offspringPopulationSize 65 " + "--createInitialSolutions latinHypercubeSampling " + "--variation crossoverAndMutationVariation " + "--selection tournament " + "--selectionTournamentSize 7 " + "--rankingForSelection dominanceRanking " + "--densityEstimatorForSelection crowdingDistance " + "--crossover BLX_ALPHA " + "--crossoverProbability 0.9102 " + "--crossoverRepairStrategy bounds " + "--blxAlphaCrossoverAlphaValue 0.5804 " + "--sbxDistributionIndex 20.0 " + "--mutation uniform " + "--mutationProbability 0.0110 " + "--mutationRepairStrategy round " + "--uniformMutationPerturbation 0.8564 " + "--polynomialMutationDistributionIndex 20.0 " + "--externalArchive unboundedExternalArchive ").split("\\s+");
