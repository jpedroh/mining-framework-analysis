...
public class MOEADDEComponentBasedConfigurationExample extends AbstractAlgorithmRunner {
  public static void main(String[] args) throws FileNotFoundException {
    ...
    DifferentialCrossoverVariation variation = new DifferentialCrossoverVariation(1, crossover, mutation, subProblemIdGenerator);
    ...
    PopulationAndNeighborhoodMatingPoolSelection<DoubleSolution> selection = new PopulationAndNeighborhoodMatingPoolSelection<>(variation.getCrossover().getNumberOfRequiredParents(), subProblemIdGenerator, neighborhood, neighborhoodSelectionProbability, true);
    int maximumNumberOfReplacedSolutions = 2;
    AggregativeFunction aggregativeFunction = new Tschebyscheff();
    MOEADReplacement<DoubleSolution> replacement = new MOEADReplacement<>(selection, neighborhood, aggregativeFunction, subProblemIdGenerator, maximumNumberOfReplacedSolutions);
    Evaluation<DoubleSolution> evaluation = new SequentialEvaluation<>(problem);
    algorithm = new MOEADDE(evaluation, new RandomSolutionsCreation<>(problem, populationSize), new TerminationByEvaluations(150000), selection, variation, replacement);
    algorithm.run();
