 ...
 */
@SuppressWarnings(value = { "serial" }) public class MOEAD<S extends Solution<?>> extends ComponentBasedEvolutionaryAlgorithm<S> {
  /** Constructor */
  public MOEAD(Evaluation<S> evaluation, SolutionsCreation<S> initialPopulationCreation, Termination termination, RandomMatingPoolSelection<S> selection, CrossoverAndMutationVariation<S> variation, MOEADReplacement<S> replacement) {
    super("MOEAD", evaluation, initialPopulationCreation, termination, selection, variation, replacement);
  }
   ...
   * @param termination
   */
  public MOEAD(Problem<S> problem, int populationSize, MutationOperator<S> mutationOperator, CrossoverOperator<S> crossoverOperator, AggregativeFunction aggregativeFunction, double neighborhoodSelectionProbability, int maximumNumberOfReplacedSolutions, int neighborhoodSize, String weightVectorDirectory, Termination termination) {
    this.name = "MOEAD";
    ...
    this.selection = new PopulationAndNeighborhoodMatingPoolSelection<S>(variation.getMatingPoolSize(), subProblemIdGenerator, neighborhood, neighborhoodSelectionProbability, true);
    this.replacement = new MOEADReplacement<S>((PopulationAndNeighborhoodMatingPoolSelection<S>) selection, neighborhood, aggregativeFunction, subProblemIdGenerator, maximumNumberOfReplacedSolutions);
