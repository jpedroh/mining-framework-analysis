...
public class ClassUtils {
  ...
  private static ExcelContentProperty doGetExcelContentProperty(Class<?> clazz, Class<?> headClass, String fieldName, ConfigurationHolder configurationHolder) {
    ExcelContentProperty excelContentProperty = Optional.ofNullable(declaredFieldContentMap(clazz, configurationHolder)).map((map) -> map.get(fieldName)).orElse(null);
    ExcelContentProperty headExcelContentProperty = Optional.ofNullable(declaredFieldContentMap(headClass, configurationHolder)).map((map) -> map.get(fieldName)).orElse(null);
  ...
  private static FieldCache doDeclaredFields(Class<?> clazz, ConfigurationHolder configurationHolder) {
    ...
    boolean needIgnore = !CollectionUtils.isEmpty(writeHolder.excludeColumnFieldNames()) || !CollectionUtils.isEmpty(writeHolder.excludeColumnIndexes()) || !CollectionUtils.isEmpty(writeHolder.includeColumnFieldNames()) || !CollectionUtils.isEmpty(writeHolder.includeColumnIndexes());
    if (!needIgnore) {
      return fieldCache;
    }
  ...
  private static Map<Integer, FieldWrapper> buildSortedAllFieldMap(Map<Integer, List<FieldWrapper>> orderFieldMap, Map<Integer, FieldWrapper> indexFieldMap) {
    Map<Integer, FieldWrapper> sortedAllFieldMap = new HashMap<>((orderFieldMap.size() + indexFieldMap.size()) * 4 / 3 + 1);
  ...
  @Getter @Setter @EqualsAndHashCode @AllArgsConstructor public static class ContentPropertyKey {
    private Class<?> clazz;
  ...
  @Data public static class FieldCacheKey {
    private Class<?> clazz;
