...
public class ElasticSearchUtils {
  ...
  public static Map<String, ?> getMappingSource(final MutableTable table) {
    if (table.getColumnByName(FIELD_ID) == null) {
      final MutableColumn idColumn = new MutableColumn(FIELD_ID, ColumnType.STRING).setTable(table).setPrimaryKey(true);
  ...
  /**
     ...
     */
  public static QueryBuilder createQueryBuilderForSimpleWhere(final List<FilterItem> whereItems, final LogicalOperator logicalOperator) {
  ...
  private static QueryBuilder createQueryBuilderForOperator(final FilterItem filterItem, final Column column) {
    if (OperatorType.EQUALS_TO.equals(filterItem.getOperator())) {
      if (filterItem.getOperand() == null) {
        ...
        if (column.getType().isLiteral() && filterItem.getOperand().equals("")) {
  ...
  public static ColumnType getColumnTypeFromElasticSearchType(final String metaDataFieldType) {
    ...
    if (metaDataFieldType.startsWith("date")) {
      ...
      if (metaDataFieldType.equals("long")) {
        ...
        if (metaDataFieldType.equals("string")) {
          columnType = ColumnType.STRING;
        } else {
          if (metaDataFieldType.equals("float")) {
            columnType = ColumnType.FLOAT;
          } else {
            if (metaDataFieldType.equals("boolean")) {
              columnType = ColumnType.BOOLEAN;
            } else {
              if (metaDataFieldType.equals("double")) {
        ...
        if (sourceMap != null) {
          ...
          if (column.getType() == ColumnType.DATE) {
            ...
            if (column.getType() == ColumnType.MAP && value == null) {
              ...
              sourceMap.keySet().stream().filter((fieldName) -> fieldName.startsWith(column.getName() + ".")).forEach((fieldName) -> evaluateField(sourceMap, valueMap, fieldName, fieldName.substring(fieldName.indexOf('.') + 1)));
              if (!valueMap.isEmpty()) {
                values[i] = valueMap;
              }
            } else {
  ...
  private static void evaluateField(final Map<String, Object> sourceMap, final Map<String, Object> valueMap, final String sourceFieldName, final String subFieldName) {
    if (subFieldName.contains(".")) {
      @SuppressWarnings(value = { "unchecked" }) final Map<String, Object> nestedValueMap = (Map<String, Object>) valueMap.computeIfAbsent(subFieldName.substring(0, subFieldName.indexOf('.')), (key) -> new HashMap<>());
      evaluateField(sourceMap, nestedValueMap, sourceFieldName, subFieldName.substring(subFieldName.indexOf('.') + 1));
    } else {
