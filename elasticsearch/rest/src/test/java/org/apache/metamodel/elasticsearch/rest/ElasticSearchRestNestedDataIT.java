...
public class ElasticSearchRestNestedDataIT {
  ...
  @Before public void setUp() throws Exception {
    ...
    client = ElasticSearchRestUtil.createClient(new HttpHost(dockerHostAddress, ElasticSearchRestDataContextIT.DEFAULT_REST_CLIENT_PORT), null, null);
  ...
  @Test public void testNestedData() throws Exception {
  ...
  @Test public void testIndexOfDocumentWithDots() throws Exception {
    final String document = "{ \"user.fullname\": \"John Doe\", " + "\"user.address.street\": \"Main street 1\", " + "\"user.address.city\": \"Newville\", " + "\"message\": \"This is what I have to say.\" }";
    final IndexRequest indexRequest = new IndexRequest(INDEX_NAME).id("1");
  ...
  private void validateSchemaAndResults() {
    ...
    try (DataSet dataSet = dataContext.query().from(DEFAULT_TABLE_NAME).select("user").and("message").execute()) {
      ...
      @SuppressWarnings(value = { "rawtypes" }) final Map userValueMap = (Map) userValue;
      ...
      @SuppressWarnings(value = { "rawtypes" }) final Map addressValueMap = (Map) addressValue;
