...
public final class CheckstyleValidator implements ResourceValidator {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "PMD.ConstructorOnlyInitializesOrCallOtherConstructors" }) public CheckstyleValidator(final Environment env) {
    this.env = env;
  ...
  @Override @SuppressWarnings(value = { "PMD.AvoidInstantiatingObjectsInLoops" }) public Collection<Violation> validate(final Collection<File> files) {
    ...
    for (final AuditEvent event : events) {
      ...
      results.add(new Violation.Default(this.name(), check.substring(check.lastIndexOf('.') + 1), event.getFileName(), String.valueOf(event.getLine()), event.getMessage()));
    }
    return results;
  }
  ...
  private Configuration configuration() {
    final File cache = new File(this.env.tempdir(), "checkstyle/checkstyle.cache");
    ...
    final InputSource src = new InputSource(this.getClass().getResourceAsStream("checks.xml"));
    final Configuration config;
    try {
      config = ConfigurationLoader.loadConfiguration(src, new PropertiesExpander(props), ConfigurationLoader.IgnoredModulesOptions.OMIT);
    } catch (final CheckstyleException ex) {
