...
public class KafkaAvroMessageDecoder extends MessageDecoder<byte[], Record> {
  ...
  @Override public void init(Properties props, String topicName) {
    ...
    try {
      ...
      timestampFormat = props.getProperty(JsonStringMessageDecoder.CAMUS_MESSAGE_TIMESTAMP_FORMAT, JsonStringMessageDecoder.CAMUS_MESSAGE_TIMESTAMP_FORMAT);
    } catch (Exception e) {
  ...
  public CamusWrapper<Record> decode(byte[] payload) {
    try {
      ...
      DatumReader<Record> reader = (helper.getTargetSchema() == null) ? new GenericDatumReader<Record>(helper.getSchema()) : new GenericDatumReader<Record>(helper.getSchema(), helper.getTargetSchema());
  ...
  public class CamusAvroWrapper extends CamusWrapper<Record> {
    ...
    @Override public long getTimestamp() {
      ...
      if (header != null && header.get("time") != null) {
        ...
        if (super.getRecord().get("timestamp") != null) {
        ...
        if (timestampField != null) {
          ...
          if (eventTimestamp != null) {
            if ("unix_milliseconds".equals(timestampFormat)) {
              ...
              if ("unix_seconds".equals(timestampFormat)) {
                return getAsLong(eventTimestamp) * 1000;
              } else {
                if ("iso".equals(timestampFormat)) {
