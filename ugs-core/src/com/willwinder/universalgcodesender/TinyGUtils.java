...
public class TinyGUtils {
  ...
  public static String generateResetCoordinatesToZeroCommand(ControllerStatus controllerStatus, GcodeState gcodeState) {
    ...
    return "G10 L2 P" + offsetCode + " X" + Utils.formatter.format(machineCoord.get(Axis.X)) + " Y" + Utils.formatter.format(machineCoord.get(Axis.Y)) + " Z" + Utils.formatter.format(machineCoord.get(Axis.Z));
  }
  ...
  private static boolean hasNumericField(JsonObject statusResultObject, String fieldName) {
    return statusResultObject.has(fieldName) && !statusResultObject.get(fieldName).isJsonNull() && NUMBER_REGEX.matcher(statusResultObject.get(fieldName).getAsString()).matches();
  }
