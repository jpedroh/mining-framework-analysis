...
public class CommandProcessorList implements CommandProcessor, Iterable<CommandProcessor> {
  ...
  /**
     ...
     */
  @Override public List<String> processCommand(String command, final GcodeState initialState) throws GcodeParserException {
  ...
  @Override public Iterator<CommandProcessor> iterator() {
    return commandProcessors.iterator();
  }

  @Override public void forEach(Consumer<? super CommandProcessor> action) {
    commandProcessors.forEach(action);
  }

  @Override public Spliterator<CommandProcessor> spliterator() {
