...
public class GcodePreprocessorUtils {
  ...
  static public Position updatePointWithCommand(List<String> commandArgs, Position initial, boolean absoluteMode) {
    ...
    if (Double.isNaN(x) && Double.isNaN(y) && Double.isNaN(z) && Double.isNaN(a) && Double.isNaN(b) && Double.isNaN(c)) {
      return null;
    }
    ...
    for (int i = 0; i < command.length(); i++) {
      ...
      if (c == '(' && !readLineComment) {
        ...
        if (blockCommentDepth > 0 && c == ')') {
          ...
          if (c == ';' && !readLineComment && blockCommentDepth == 0) {
            if (sb.length() > 0) {
              l.add(sb.toString());
              sb = new StringBuilder();
            }
  ...
  /**
     ...
     */
  static public List<Position> generatePointsAlongArcBDring(final Position start, final Position end, final Position center, boolean clockwise, double radius, double minArcLengthMM, double arcSegmentLengthMM, PlaneFormatter plane) {
    double r = radius;
    if (r == 0) {
  ...
  /**
     ...
     * @param plane helper to select values for arcs across different planes
     */
  public static List<Position> generatePointsAlongArcBDring(final Position p1, final Position p2, final Position center, boolean isCw, double radius, double startAngle, double sweep, int numPoints, PlaneFormatter plane) {
