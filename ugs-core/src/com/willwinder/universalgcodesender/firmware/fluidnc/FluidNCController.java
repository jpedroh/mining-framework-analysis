...
public class FluidNCController implements IController, CommunicatorListener {
  ...
  public FluidNCController(ICommunicator communicator) {
    this.controllerStatus = ControllerStatusBuilder.newInstance().setState(ControllerState.DISCONNECTED).build();
  ...
  @Override public void resetCoordinateToZero(Axis axis) throws Exception {
    if (isCommOpen()) {
  ...
  @Override public void jogMachine(PartialPosition distance, double feedRate) throws Exception {
  ...
  private void setControllerState(ControllerState controllerState) {
    ControllerStatus newControllerStatus = ControllerStatusBuilder.newInstance(controllerStatus).setState(controllerState).build();
  ...
  @Override public void cancelJog() throws Exception {
    communicator.sendByteImmediately(GrblUtils.GRBL_JOG_CANCEL_COMMAND);
  }

  @Override public CommunicatorState getCommunicatorState() {
  ...
  @Override public GcodeCommand createCommand(String command) throws Exception {
    return new FluidNCCommand(command);
  }

  @Override public void sendCommandImmediately(GcodeCommand cmd) throws Exception {
    communicator.queueCommand(cmd);
    communicator.streamCommands();
  }

  @Override public void queueStream(IGcodeStreamReader reader) {
  ...
  @Override public void rawResponseListener(String response) {
    if (GrblUtils.isGrblStatusString(response)) {
      ...
      if (getActiveCommand().isPresent()) {
        ...
        if (FluidNCUtils.isWelcomeResponse(response)) {
          messageService.dispatchMessage(MessageType.VERBOSE, response + "\n");
          if (controllerStatus.getState() == ControllerState.CONNECTING) {
  ...
  @Override public IFileService getFileService() {
    return fileService;
  }
}