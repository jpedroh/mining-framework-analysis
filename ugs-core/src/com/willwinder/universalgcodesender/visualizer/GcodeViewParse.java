...
public class GcodeViewParse {
  ...
  /**
     ...
     */
  public List<LineSegment> toObjFromReader(IGcodeStreamReader reader, double arcSegmentLength) throws IOException, GcodeParserException {
    lines.clear();
...
<<<<<<< /usr/src/app/output/winder/universal-g-code-sender/cd6db300cec1859541cb3529678fefda18701dd2/ugs-core/src/com/willwinder/universalgcodesender/visualizer/GcodeViewParse.java/left.java
    UnitUtils.Units.getUnits(gp.getCurrentState().units)
=======
    gp.getCurrentState().getUnits()
        ...
        for (GcodeMeta meta : points) {
          if (meta.point != null) {
            ...
            if (start.getUnits() != UnitUtils.Units.MM && gp.getCurrentState().isMetric) {
              start = new Position(meta.point.point().x, meta.point.point().y, meta.point.point().z, gp.getCurrentState().isMetric ? UnitUtils.Units.MM : UnitUtils.Units.INCH);
            } else {
