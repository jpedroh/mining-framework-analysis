...
@PluginDescriptor(name = "Object Markers", description = "Enable marking of objects using the Shift key", tags = { "overlay", "objects", "mark", "marker" }, enabledByDefault = false) @Slf4j public class ObjectIndicatorsPlugin extends Plugin {
  ...
  @Getter(value = AccessLevel.PACKAGE) private final List<ColorTileObject> objects = new ArrayList<>();
  ...
  @Inject private ObjectIndicatorsConfig config;

  @Inject private Gson gson;

  @Provides ObjectIndicatorsConfig provideConfig(ConfigManager configManager) {
  ...
  @Subscribe public void onMenuEntryAdded(MenuEntryAdded event) {
    ...
    client.createMenuEntry(-1).setOption(objects.stream().anyMatch((o) -> o.getTileObject() == tileObject) ? UNMARK : MARK).setTarget(event.getTarget()).setParam0(event.getActionParam0()).setParam1(event.getActionParam1()).setIdentifier(event.getIdentifier()).setType(MenuAction.RUNELITE).onClick(this::markObject);
  }
  ...
  private void checkObjectPoints(TileObject object) {
    ...
    for (ObjectPoint objectPoint : objectPoints) {
      if (worldPoint.getRegionX() == objectPoint.getRegionX() && worldPoint.getRegionY() == objectPoint.getRegionY() && worldPoint.getPlane() == objectPoint.getZ() && objectPoint.getId() == object.getId()) {
        ...
        objects.add(new ColorTileObject(object, objectComposition, objectPoint.getName(), objectPoint.getColor(), objectPoint.getFillColor()));
        break;
      }
    }
  }
  ...
  private void markObject(ObjectComposition objectComposition, String name, final TileObject object) {
    ...
    final ObjectPoint point = new ObjectPoint(object.getId(), name, regionId, worldPoint.getRegionX(), worldPoint.getRegionY(), worldPoint.getPlane(), color, fillColor);
    ...
    if (objects.removeIf((o) -> o.getTileObject() == object)) {
      ...
      objects.add(new ColorTileObject(object, client.getObjectDefinition(object.getId()), name, color, fillColor));
  ...
  @Nullable private ObjectComposition getObjectComposition(int id) {
