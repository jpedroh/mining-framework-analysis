...
class ObjectIndicatorsOverlay extends Overlay {
  ...
  @Inject private ObjectIndicatorsOverlay(Client client, ObjectIndicatorsConfig config, ObjectIndicatorsPlugin plugin, ModelOutlineRenderer modelOutlineRenderer) {
    this.client = client;
    this.config = config;
    this.plugin = plugin;
  ...
  @Override public Dimension render(Graphics2D graphics) {
    ...
    for (ColorTileObject colorTileObject : plugin.getObjects()) {
      ...
      if (composition.getImpostorIds() != null) {
        ...
        if (composition == null || Strings.isNullOrEmpty(composition.getName()) || "null".equals(composition.getName()) || !composition.getName().equals(colorTileObject.getName())) {
          continue;
        }
      }
