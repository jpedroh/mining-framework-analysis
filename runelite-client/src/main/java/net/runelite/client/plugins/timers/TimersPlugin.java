...
@PluginDescriptor(name = "Timers", description = "Show various timers in an infobox", tags = { "combat", "items", "magic", "potions", "prayer", "overlay", "abyssal", "sire", "inferno", "fight", "caves", "cape", "timer", "tzhaar", "thieving", "pickpocket", "hunter", "impling", "puro" }) @Slf4j public class TimersPlugin extends Plugin {
  ...
  @Inject private TimersConfig config;

  @Inject private InfoBoxManager infoBoxManager;

  @Provides TimersConfig getConfig(ConfigManager configManager) {
  ...
  @Subscribe public void onVarbitChanged(VarbitChanged event) {
    ...
    if (event.getVarpId() == VarPlayer.POISON && config.showAntiPoison()) {
      ...
      updateVarTimer(ANTIPOISON, event.getValue(), (i) -> i >= 0 || i < VENOM_VALUE_CUTOFF, (i) -> nextPoisonTick - tickCount + Math.abs((i + 1) * POISON_TICK_LENGTH));
      updateVarTimer(ANTIVENOM, event.getValue(), (i) -> i >= VENOM_VALUE_CUTOFF, (i) -> nextPoisonTick - tickCount + Math.abs((i + 1 - VENOM_VALUE_CUTOFF) * POISON_TICK_LENGTH));
    }
    if ((event.getVarbitId() == Varbits.NMZ_OVERLOAD_REFRESHES_REMAINING || event.getVarbitId() == Varbits.COX_OVERLOAD_REFRESHES_REMAINING) && config.showOverload()) {
    ...
    if (event.getVarbitId() == Varbits.RUN_SLOWED_DEPLETION_ACTIVE || event.getVarbitId() == Varbits.STAMINA_EFFECT || event.getVarbitId() == Varbits.RING_OF_ENDURANCE_EFFECT) {
    ...
    if (event.getVarbitId() == Varbits.DIVINE_SUPER_DEFENCE && config.showDivine()) {
      if (client.getVarbitValue(Varbits.DIVINE_SUPER_COMBAT) > event.getValue() || client.getVarbitValue(Varbits.DIVINE_BASTION) > event.getValue() || client.getVarbitValue(Varbits.DIVINE_BATTLEMAGE) > event.getValue()) {
        return;
      }
  ...
  @Subscribe public void onChatMessage(ChatMessage event) {
    ...
    if (config.showGodWarsAltar() && message.equalsIgnoreCase(GOD_WARS_ALTAR_MESSAGE)) {
    ...
    if (config.showCannon()) {
      if (message.equals(CANNON_BASE_MESSAGE) || message.equals(CANNON_STAND_MESSAGE) || message.equals(CANNON_BARRELS_MESSAGE) || message.equals(CANNON_FURNACE_MESSAGE) || message.contains(CANNON_REPAIR_MESSAGE)) {
        ...
        if (message.equals(CANNON_BROKEN_MESSAGE)) {
          removeGameTimer(CANNON);
          TimerTimer cannonTimer = createGameTimer(CANNON_REPAIR);
          cannonTimer.setTooltip(cannonTimer.getTooltip() + " - World " + client.getWorld());
        } else {
          if (message.equals(CANNON_PICKUP_MESSAGE) || message.equals(CANNON_DESTROYED_MESSAGE)) {
            removeGameTimer(CANNON);
            removeGameTimer(CANNON_REPAIR);
          }
        }
        ...
        if (message.endsWith(WARD_OF_ARCEUUS_MESSAGE)) {
          ...
          if (message.contains(RESURRECT_THRALL_MESSAGE_START) && message.endsWith(RESURRECT_THRALL_MESSAGE_END)) {
            ...
            if (client.getVarbitValue(Varbits.COMBAT_ACHIEVEMENT_TIER_GRANDMASTER) == 2) {
              ...
              if (client.getVarbitValue(Varbits.COMBAT_ACHIEVEMENT_TIER_MASTER) == 2) {
                t += t / 2;
              }
            }
  ...
  @Subscribe public void onItemContainerChanged(ItemContainerChanged itemContainerChanged) {
    ...
    if (weapon == null || (weapon.getId() != ItemID.STAFF_OF_THE_DEAD && weapon.getId() != ItemID.TOXIC_STAFF_OF_THE_DEAD && weapon.getId() != ItemID.STAFF_OF_LIGHT && weapon.getId() != ItemID.TOXIC_STAFF_UNCHARGED)) {
      removeGameTimer(STAFF_OF_THE_DEAD);
    }
  }
  ...
  private void updateVarTimer(final GameTimer gameTimer, final int varValue, final IntPredicate removeTimerCheck, final IntUnaryOperator tickDuration) {
    ...
    if (removeTimerCheck.test(varValue)) {
      ...
      if (timer == null || ticks > timer.ticks) {
