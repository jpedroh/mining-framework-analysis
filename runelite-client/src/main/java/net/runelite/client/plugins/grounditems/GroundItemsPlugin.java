...
@PluginDescriptor(name = "Ground Items", description = "Highlight ground items and/or show price information", tags = { "grand", "exchange", "high", "alchemy", "prices", "highlight", "overlay", "lootbeam" }) public class GroundItemsPlugin extends Plugin {
  @Value static class PriceHighlight {
  ...
  @Inject private GroundItemMouseAdapter mouseAdapter;

  @Getter(value = AccessLevel.PACKAGE) @Setter(value = AccessLevel.PACKAGE) private Map.Entry<Rectangle, GroundItem> textBoxBounds;

  @Getter(value = AccessLevel.PACKAGE) @Setter(value = AccessLevel.PACKAGE) private Map.Entry<Rectangle, GroundItem> hiddenBoxBounds;

  @Getter(value = AccessLevel.PACKAGE) @Setter(value = AccessLevel.PACKAGE) private Map.Entry<Rectangle, GroundItem> highlightBoxBounds;

  @Getter(value = AccessLevel.PACKAGE) @Setter(value = AccessLevel.PACKAGE) private boolean hotKeyPressed;

  @Getter(value = AccessLevel.PACKAGE) @Setter(value = AccessLevel.PACKAGE) private boolean hideAll;

  private List<String> hiddenItemList = new CopyOnWriteArrayList<>();

  private List<String> highlightedItemsList = new CopyOnWriteArrayList<>();

  @Inject private GroundItemHotkeyListener hotkeyListener;
  ...
  @Inject private GroundItemsConfig config;

  @Inject private GroundItemsOverlay overlay;

  @Inject private Notifier notifier;

  @Inject private ScheduledExecutorService executor;

  @Getter private final Table<WorldPoint, Integer, GroundItem> collectedGroundItems = HashBasedTable.create();
  ...
  @Provides GroundItemsConfig provideConfig(ConfigManager configManager) {
  ...
  @Subscribe public void onGameStateChanged(final GameStateChanged event) {
  ...
  @Subscribe public void onNpcLootReceived(NpcLootReceived npcLootReceived) {
  ...
  private void lootReceived(Collection<ItemStack> items, LootType lootType) {
    ...
    items.stream().map(ItemStack::getLocation).map((l) -> WorldPoint.fromLocal(client, l)).distinct().forEach(this::handleLootbeam);
  }
  ...
  @Subscribe public void onMenuEntryAdded(MenuEntryAdded event) {
    ...
    if (type == MenuAction.GROUND_ITEM_FIRST_OPTION || type == MenuAction.GROUND_ITEM_SECOND_OPTION || type == MenuAction.GROUND_ITEM_THIRD_OPTION || type == MenuAction.GROUND_ITEM_FOURTH_OPTION || type == MenuAction.GROUND_ITEM_FIFTH_OPTION || type == MenuAction.WIDGET_TARGET_ON_GROUND_ITEM) {
      ...
      if ((config.itemHighlightMode() == ItemHighlightMode.MENU || config.itemHighlightMode() == ItemHighlightMode.BOTH) && (color != null && canBeRecolored && !color.equals(config.defaultColor()))) {
  ...
  Color getHidden(NamedQuantity item, int gePrice, int haPrice, boolean isTradeable) {
    ...
    return isExplicitHidden || (!isExplicitHighlight && canBeHidden && underGe && underHa) ? config.hiddenColor() : null;
  }
  ...
  private void notifyHighlightedItem(GroundItem item) {
    final boolean shouldNotifyHighlighted = config.notifyHighlightedDrops() && TRUE.equals(highlightedItems.getUnchecked(new NamedQuantity(item)));
    final boolean shouldNotifyTier = config.notifyTier() != HighlightTier.OFF && getValueByMode(item.getGePrice(), item.getHaPrice()) > config.notifyTier().getValueFromTier(config) && FALSE.equals(hiddenItems.getUnchecked(new NamedQuantity(item)));
    ...
    final StringBuilder notificationStringBuilder = new StringBuilder().append("You received a ").append(dropType).append(" drop: ").append(item.getName());
  ...
  @Subscribe public void onMenuOptionClicked(MenuOptionClicked menuOptionClicked) {
    if (menuOptionClicked.isItemOp() && menuOptionClicked.getMenuOption().equals("Drop")) {
      ...
      if (menuOptionClicked.getMenuAction() == MenuAction.WIDGET_TARGET_ON_GAME_OBJECT && client.getSelectedWidget().getId() == WidgetInfo.INVENTORY.getId()) {
  ...
  private void handleLootbeam(WorldPoint worldPoint) {
    ...
    for (GroundItem groundItem : groundItems) {
      ...
      if (config.showLootbeamForHighlighted() && TRUE.equals(highlightedItems.getUnchecked(item))) {
