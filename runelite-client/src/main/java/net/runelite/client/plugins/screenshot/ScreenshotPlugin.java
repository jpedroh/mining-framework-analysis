...
@PluginDescriptor(name = "Screenshot", description = "Enable the manual and automatic taking of screenshots", tags = { "external", "images", "imgur", "integration", "notifications" }) @Slf4j public class ScreenshotPlugin extends Plugin {
  ...
  @Inject private ScreenshotConfig config;

  @Inject private OverlayManager overlayManager;

  @Inject private ScreenshotOverlay screenshotOverlay;
  ...
  @Getter(value = AccessLevel.PACKAGE) private BufferedImage reportButton;
  ...
  @Provides ScreenshotConfig getConfig(ConfigManager configManager) {
  ...
  @Override protected void startUp() throws Exception {
    ...
    titleBarButton = NavigationButton.builder().tab(false).tooltip("Take screenshot").icon(iconImage).onClick(this::manualScreenshot).popup(ImmutableMap.<String, Runnable>builder().put("Open screenshot folder...", () -> {
...
<<<<<<< /usr/src/app/output/runelite/runelite/32dc12da8dda6d3eb70e9d44f2c9a1915c5dd444/runelite-client/src/main/java/net/runelite/client/plugins/screenshot/ScreenshotPlugin.java/left.java
        player != client.getLocalPlayer() && (player.isFriendsChatMember() || player.isFriend()) && config.screenshotFriendDeath() && player.getCanvasTilePoly() != null || player != client.getLocalPlayer() && player.isClanMember() && config.screenshotClanDeath() && player.getCanvasTilePoly() != null
=======
        player != client.getLocalPlayer() && ((player.isFriendsChatMember() || player.isFriend()) && config.screenshotFriendDeath() || (player.isClanMember() && config.screenshotClanDeath())) && player.getCanvasTilePoly() != null
>>>>>>> /usr/src/app/output/runelite/runelite/32dc12da8dda6d3eb70e9d44f2c9a1915c5dd444/runelite-client/src/main/java/net/runelite/client/plugins/screenshot/ScreenshotPlugin.java/right.java
  ...
  @Subscribe public void onPlayerLootReceived(final PlayerLootReceived playerLootReceived) {
  ...
  /**
	 ...
	 */
  @VisibleForTesting static String parseQuestCompletedWidget(final String text) {
  ...
  /**
	 ...
	 */
  @VisibleForTesting static String parseBAHighGambleWidget(final String text) {
  ...
  private void takeScreenshot(String fileName, String subDir, Image image) {
    BufferedImage screenshot = config.includeFrame() ? new BufferedImage(clientUi.getWidth(), clientUi.getHeight(), BufferedImage.TYPE_INT_ARGB) : new BufferedImage(image.getWidth(null), image.getHeight(null), BufferedImage.TYPE_INT_ARGB);
  ...
  private boolean isInsideGauntlet() {
    return this.client.isInInstancedRegion() && this.client.getMapRegions().length > 0 && (this.client.getMapRegions()[0] == GAUNTLET_REGION || this.client.getMapRegions()[0] == CORRUPTED_GAUNTLET_REGION);
  }

  @VisibleForTesting int getClueNumber() {
    return clueNumber;
  }

  @VisibleForTesting String getClueType() {
    return clueType;
  }

  @VisibleForTesting KillType getKillType() {
    return killType;
  }

  @VisibleForTesting int getKillCountNumber() {
