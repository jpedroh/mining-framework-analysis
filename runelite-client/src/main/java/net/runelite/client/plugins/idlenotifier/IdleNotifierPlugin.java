...
@PluginDescriptor(name = "Idle Notifier", description = "Send a notification when going idle, or when HP/Prayer reaches a threshold", tags = { "health", "hitpoints", "notifications", "prayer" }) public class IdleNotifierPlugin extends Plugin {
  private static final int LOGOUT_WARNING_MILLIS = (4 * 60 + 40) * 1000;

  private static final int COMBAT_WARNING_MILLIS = 19 * 60 * 1000;
  ...
  @Provides IdleNotifierConfig provideConfig(ConfigManager configManager) {
  ...
  @Subscribe public void onHitsplatApplied(HitsplatApplied event) {
    ...
    if (hitsplat.getHitsplatType() == Hitsplat.HitsplatType.DAMAGE_ME || hitsplat.getHitsplatType() == Hitsplat.HitsplatType.BLOCK_ME) {
  ...
  @Subscribe public void onGameTick(GameTick event) {
    ...
    if (client.getGameState() != GameState.LOGGED_IN || local == null || System.currentTimeMillis() - client.getMouseLastPressedMillis() < 1000 || client.getKeyboardIdleTicks() < 10) {
      resetTimers();
      return;
    }
  ...
  private boolean checkInteractionIdle(Duration waitDuration, Player local) {
    ...
    if (interact == null) {
      if (lastInteracting != null && Instant.now().compareTo(lastInteracting.plus(waitDuration)) >= 0 && lastCombatCountdown == 0) {
        lastInteract = null;
        lastInteracting = null;
        lastAnimation = IDLE;
        lastAnimating = null;
  ...
  private boolean checkMovementIdle(Duration waitDuration, Player local) {
    ...
    if (lastPosition.equals(position)) {
      if (notifyPosition && local.getAnimation() == IDLE && Instant.now().compareTo(lastMoving.plus(waitDuration)) >= 0) {
