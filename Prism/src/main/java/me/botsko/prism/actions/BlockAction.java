...
public class BlockAction extends GenericAction {
  ...
  @Override public ChangeResult applyRollback(Player player, PrismParameters parameters, boolean isPreview) {
  ...
  @Override public ChangeResult applyRestore(Player player, PrismParameters parameters, boolean isPreview) {
    final Block block = getWorld().getBlockAt(getLoc());
    if (getActionType().doesCreateBlock()) {
      return placeBlock(player, parameters, isPreview, block, false);
    } else {
  ...
  @Override public ChangeResult applyUndo(Player player, PrismParameters parameters, boolean isPreview) {
    final Block block = getWorld().getBlockAt(getLoc());
    return placeBlock(player, parameters, isPreview, block, false);
  }

  @Override public ChangeResult applyDeferred(Player player, PrismParameters parameters, boolean isPreview) {
    final Block block = getWorld().getBlockAt(getLoc());
  ...
  /**
     ...
     * @return ChangeResult.
     */
  private @NotNull ChangeResult handleApply(final Block block, final BlockState originalBlock, final PrismParameters parameters, final boolean cancelIfBadPlace) {
  ...
  private @NotNull ChangeResultImpl handleSkulls(final Block block, BlockActionData blockActionData, final BlockState originalBlock) {
    block.setType(getMaterial());
    BlockState state = block.getState();
