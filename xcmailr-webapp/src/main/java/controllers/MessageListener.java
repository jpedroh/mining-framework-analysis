 ...
 */
@Singleton public class MessageListener implements SimpleMessageListener {
  @Inject XCMailrConf xcmConfiguration;

  @Inject MailrMessageSenderFactory mailrSenderFactory;
  ...
  @Override public void deliver(String from, String recipient, InputStream data) {
    try {
      ...
      if (data.available() > xcmConfiguration.MAX_MAIL_SIZE) {
        log.error("Cancel mail processing due to restriction of mail size. Dropped mail: " + from + " => " + recipient + ", size: " + data.available() + " bytes");
        return;
      }
  ...
  private void persistMail(MBox mailBox, String from, MimeMessage mail, String rawMessage) throws MessagingException, IOException {
  ...
  private void createMtxAndAddToQueue(final int status, final String from, final String recipient, final String forwardTarget) {
    if (xcmConfiguration.MTX_MAX_AGE != 0) {
