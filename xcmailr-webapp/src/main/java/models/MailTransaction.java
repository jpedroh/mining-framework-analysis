 ...
 */
@Entity @Table(name = "mailtransactions") public class MailTransaction {
  @Id private Long id;
  ...
  public static List<MailTransaction> getForTarget(final String targetAddr) {
    List<MailTransaction> list = Ebean.find(MailTransaction.class).where().eq("targetaddr", targetAddr).orderBy("ts desc").findList();
    return list;
  }
  ...
  public static List<MailTransaction> getForRelay(final String relayAddr) {
    List<MailTransaction> list = Ebean.find(MailTransaction.class).where().eq("relayaddr", relayAddr).orderBy("ts desc").findList();
    return list;
  }
  ...
  public static List<MailTransaction> getForSource(final String sourceAddr) {
    List<MailTransaction> list = Ebean.find(MailTransaction.class).where().eq("sourceaddr", sourceAddr).orderBy("ts desc").findList();
    return list;
  }
