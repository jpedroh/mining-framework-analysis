...
public class EObjectDeserializer extends JsonDeserializer<EObject> implements ContextualDeserializer {
  ...
  @Override public EObject deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
    prepareContext(ctxt, null);
    return doDeserialize(jp, findRoot(ctxt), ctxt);
  }

  @Override public EObject deserialize(JsonParser jp, DeserializationContext ctxt, EObject intoValue) throws IOException {
    if (intoValue == null) {
      return null;
    }
    prepareContext(ctxt, null);
  ...
  private void readFeature(JsonParser jp, EObject current, String fieldName, DeserializationContext ctxt, Resource resource, ReferenceEntries entries) throws IOException {
    ...
    if (feature != null) {
      ...
      if (resource != null) {
        resource.getErrors().add(new JSONException("Unknown feature " + fieldName, jp.getCurrentLocation()));
      }
      jp.nextToken();
      jp.skipChildren();
    }
  ...
  @SuppressWarnings(value = { "unchecked" }) private void readContainment(JsonParser jp, EObject owner, DeserializationContext ctxt, EReference reference, Resource resource, ReferenceEntries entries) throws IOException {
  ...
  private void readAttribute(JsonParser jp, EObject owner, EAttribute attribute, Resource resource, DeserializationContext ctxt) throws IOException {
  ...
  private void readSingleAttribute(JsonParser jp, EObject owner, EAttribute attribute, Resource resource, EDataType dataType, DeserializationContext ctxt) throws IOException {
    final Class<?> type = dataType.getInstanceClass();
    Object value;
    if (type == null || dataType instanceof EEnum || EcorePackage.Literals.EJAVA_CLASS.equals(dataType) || EcorePackage.Literals.EJAVA_OBJECT.equals(dataType)) {
