...
public class PTSAFTFileGenerator {
  ...
  @Inject public PTSAFTFileGenerator(DAOPTCustomer daoCustomer, DAOPTSupplier daoSupplier, DAOPTProduct daoProduct, DAOPTTax daoPTTax, DAOPTRegionContext daoPTRegionContext, DAOPTInvoice daoPTInvoice, DAOPTSimpleInvoice daoPTSimpleInvoice, DAOPTReceiptInvoice daoPTReceiptInvoice, DAOPTCreditNote daoPTCreditNote, final DAOInvoiceSeries daoInvoiceSeries) {
    ...
    try {
      this.jaxbContext = JAXBContext.newInstance("com.premiumminds.billy.portugal.services.export.saftpt.v1_04_01.schema");
    ...
    try {
      return new TransactionWrapper<AuditFile>(daoPTInvoice) {
        @Override public AuditFile runTransaction() throws Exception {
          ...
          @SuppressWarnings(value = { "unchecked" }) List<PTCustomerEntity> customers = (List<PTCustomerEntity>) (List<?>) daoCustomer.getAllActiveCustomers();
          ...
          @SuppressWarnings(value = { "unchecked" }) List<PTSupplierEntity> suppliers = (List<PTSupplierEntity>) (List<?>) daoSupplier.getAllActiveSuppliers();
          ...
          @SuppressWarnings(value = { "unchecked" }) List<PTProductEntity> products = (List<PTProductEntity>) (List<?>) daoProduct.getAllActiveProducts();
          ...
          PTRegionContextEntity context = (PTRegionContextEntity) daoPTRegionContext.get(PTSAFTFileGenerator.this.config.getUID(Key.Context.Portugal.UUID));
          @SuppressWarnings(value = { "unchecked" }) List<PTTaxEntity> taxes = (List<PTTaxEntity>) (List<?>) daoPTTax.getTaxesForSAFTPT(context, null, null);
  ...
  private SourceDocuments generateSourceDocuments(List<PTInvoiceEntity> invoices, List<PTSimpleInvoiceEntity> simpleInvoices, List<PTReceiptInvoiceEntity> receiptInvoices, List<PTCreditNoteEntity> creditNotes) throws DatatypeConfigurationException, RequiredFieldNotFoundException, InvalidDocumentTypeException, InvalidDocumentStateException, InvalidTaxTypeException, InvalidTaxCodeException, InvalidPaymentMechanismException, InvalidInvoiceTypeException {
    ...
    totalCredit = processInvoices(simpleInvoices, salesInvoices, totalCredit);
    totalCredit = processInvoices(receiptInvoices, salesInvoices, totalCredit);
  ...
  private Invoice generateSAFTInvoice(PTGenericInvoiceEntity document) throws DatatypeConfigurationException, RequiredFieldNotFoundException, InvalidDocumentTypeException, InvalidDocumentStateException, InvalidInvoiceTypeException {
    ...
    InvoiceSeriesEntity invoiceSeries = this.daoInvoiceSeries.getSeries(document.getSeries(), document.getBusiness().getUID().toString(), LockModeType.NONE);
    StringBuilder atcudBuilder = invoiceSeries.getSeriesUniqueCode().map((s) -> new StringBuilder().append(s).append("-").append(document.getSeriesNumber())).orElse(new StringBuilder().append("0"));
    saftInv.setDocumentStatus(this.getDocumentStatus(document));
    saftInv.setInvoiceNo(validateString("InvoiceNo", document.getNumber(), MAX_LENGTH_60, true));
    saftInv.setATCUD(atcudBuilder.toString());
    saftInv.setInvoiceType(validateString("InvoiceType", getDocumentType(document), MAX_LENGTH_2, true));
    saftInv.setHash(validateString("Hash", document.getHash(), MAX_LENGTH_172, true));
    if (document.getHashControl() != null) {
      saftInv.setHashControl(validateString("HashControl", document.getHashControl(), MAX_LENGTH_40, false));
    }
    saftInv.setPeriod(validateInteger("Period", Integer.toString(getDateField(document.getDate(), Calendar.MONTH) + 1), MAX_LENGTH_2, true));
    saftInv.setInvoiceDate(formatDate(document.getDate()));
    saftInv.setSpecialRegimes(getSpecialRegimes(document.isSelfBilled(), document.isCashVATEndorser(), document.isThirdPartyBilled()));
    saftInv.setSourceID(validateString("InvoiceSourceID", document.getSourceId(), MAX_LENGTH_30, true));
    if (document.getEACCode() != null) {
      saftInv.setEACCode(validateString("EACCode", document.getEACCode(), MAX_LENGTH_5, false));
    }
    saftInv.setSystemEntryDate(formatDateTime(document.getCreateTimestamp()));
    UID customerUID = document.getCustomer().getUID();
    String customerID = customerUID.equals(this.config.getUID(Key.Customer.Generic.UUID)) ? "Consumidor final" : document.getCustomer().getID().toString();
  ...
  private SpecialRegimes getSpecialRegimes(Boolean selfBilled, Boolean cashVATEndorser, Boolean thirdPartyBilled) throws RequiredFieldNotFoundException {
    ...
    if (null != selfBilled) {
      rvalue.setSelfBillingIndicator(validateInteger("SelfBillingIndicator", selfBilled ? "1" : "0", MAX_LENGTH_1, true));
    } else {
      rvalue.setSelfBillingIndicator(0);
    }
    if (null != cashVATEndorser) {
      rvalue.setCashVATSchemeIndicator(validateInteger("CashVATSchemeIndicator", cashVATEndorser ? "1" : "0", MAX_LENGTH_1, true));
    } else {
      rvalue.setCashVATSchemeIndicator(0);
    }
    if (null != thirdPartyBilled) {
      rvalue.setThirdPartiesBillingIndicator(validateInteger("ThirdPartiesBillingIndicator", thirdPartyBilled ? "1" : "0", MAX_LENGTH_1, true));
    } else {
    ...
    for (PTGenericInvoiceEntryEntity entry : entries) {
      ...
      if (entry.getTaxes().size() > 0) {
        ...
        if ((tax.getTaxPercentage() != null && tax.getTaxPercentage().compareTo(BigDecimal.ZERO) == 0) || (tax.getTaxAmount() != null && tax.getTaxAmount().compareTo(BigDecimal.ZERO) == 0)) {
          ...
          line.setTaxExemptionCode(this.validateString("TaxExemptionCode", entry.getTaxExemptionCode(), this.MAX_LENGTH_60, true));
        }
      }
  ...
  private DocumentStatus getDocumentStatus(PTGenericInvoiceEntity document) throws InvalidDocumentStateException, DatatypeConfigurationException, RequiredFieldNotFoundException {
    ...
    if (document.getChangeReason() != null) {
      status.setReason(validateString("Reason", document.getChangeReason(), MAX_LENGTH_50, false));
    }
