 ...
 * @author zhangliang
 */
@Slf4j public class ZookeeperRegistryCenter implements CoordinatorRegistryCenter {
  @Getter(value = AccessLevel.PROTECTED) private ZookeeperConfiguration zkConfig;
  ...
  private void fillData() throws Exception {
    for (Entry<Object, Object> entry : loadLocalProperties().entrySet()) {
      ...
      if (null == client.checkExists().forPath(key)) {
        ...
        if (zkConfig.isOverwrite() || 0 == client.getData().forPath(key).length) {
  ...
  @Override public boolean isExisted(final String key) {
    try {
  ...
  @Override public void persist(final String key, final String value) {
    try {
      if (!isExisted(key)) {
  ...
  @Override public void persistEphemeral(final String key, final String value) {
    try {
      if (isExisted(key)) {
  ...
  @Override public Object getRawClient() {
    return client;
  }
  ...
  @Override public Object getRawCache(final String cachePath) {
