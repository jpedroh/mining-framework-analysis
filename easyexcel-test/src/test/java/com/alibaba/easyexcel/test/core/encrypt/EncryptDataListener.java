...
public class EncryptDataListener extends AnalysisEventListener<EncryptData> {
  ...
  @Override public void invoke(EncryptData data, AnalysisContext context) {
    list.add(data);
  }
  ...
  @Override public void doAfterAllAnalysed(AnalysisContext context) {
    ...
    Assert.assertEquals(context.readSheetHolder().getExcelReadHeadProperty().getHeadMap().get(0).getHeadNameList().get(0), "\u59d3\u540d");
