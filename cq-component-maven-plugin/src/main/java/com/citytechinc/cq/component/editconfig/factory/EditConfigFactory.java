...
public class EditConfigFactory {
  ...
  public static final EditConfig make(CtClass componentClass) throws InvalidComponentClassException, ClassNotFoundException {
    ...
    return new DefaultEditConfig(parameters, componentAnnotation.disableTargeting());
  }
  ...
  private static final EditConfigInPlaceEditing getInPlaceEditingForEditConfig(Component componentAnnotation) {
    if (!StringUtils.isEmpty(componentAnnotation.inPlaceEditingConfigPath()) || !StringUtils.isEmpty(componentAnnotation.inPlaceEditingConfigPath())) {
