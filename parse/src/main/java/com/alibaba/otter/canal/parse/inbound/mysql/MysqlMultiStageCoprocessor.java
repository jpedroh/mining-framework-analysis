...
public class MysqlMultiStageCoprocessor extends AbstractCanalLifeCycle implements MultiStageCoprocessor {
  ...
  @Override public void start() {
    ...
    this.parserExecutor = Executors.newFixedThreadPool(parserThreadCount, new NamedThreadFactory("MultiStageCoprocessor-Parser-" + destination));
    ...
    BatchEventProcessor<MessageEvent> simpleParserStage = new BatchEventProcessor<MessageEvent>(disruptorMsgBuffer, sequenceBarrier, new SimpleParserStage());
    ...
    WorkerPool<MessageEvent> workerPool = new WorkerPool<MessageEvent>(disruptorMsgBuffer, dmlParserSequenceBarrier, exceptionHandler, workHandlers);
    ...
    BatchEventProcessor<MessageEvent> sinkStoreStage = new BatchEventProcessor<MessageEvent>(disruptorMsgBuffer, sinkSequenceBarrier, new SinkStoreStage());
  ...
  private class SimpleParserStage implements EventHandler<MessageEvent>, LifecycleAware {
    ...
    public void onEvent(MessageEvent event, long sequence, boolean endOfBatch) throws Exception {
      try {
        ...
        if (StringUtils.isNotEmpty(event.getBinlogFileName()) && (context.getLogPosition() == null || !context.getLogPosition().getFileName().equals(event.getBinlogFileName()))) {
          if (context.getLogPosition() == null) {
            context.setLogPosition(new LogPosition(event.getBinlogFileName(), 0));
          } else {
            context.setLogPosition(new LogPosition(event.getBinlogFileName(), context.getLogPosition().getPosition()));
          }
        }
  ...
  private class DmlParserStage implements WorkHandler<MessageEvent>, LifecycleAware {
    @Override public void onEvent(MessageEvent event) throws Exception {
      try {
  ...
  class SimpleFatalExceptionHandler implements ExceptionHandler {
    @Override public void handleEventException(final Throwable ex, final long sequence, final Object event) {
    }

    @Override public void handleOnStartException(final Throwable ex) {
    }

    @Override public void handleOnShutdownException(final Throwable ex) {
    }
  }
