 ...
 */
@Mojo(name = "enforce", defaultPhase = LifecyclePhase.PROCESS_CLASSES, requiresDependencyResolution = ResolutionScope.COMPILE) public class EnforcerMojo extends AbstractMojo {
  /**
     * The maven project.
     */
  @Parameter(property = "project", readonly = true) protected MavenProject project;

  /**
     * If true, skip running the checker entirely.
     */
  @Parameter(property = "access-modifier-checker.skip", defaultValue = "false") private boolean skip = false;

  /**
     * If false, print warnings about violations but do not fail the build.
     */
  @Parameter(property = "access-modifier-checker.failOnError", defaultValue = "true") private boolean failOnError = true;
  ...
  @Override @SuppressFBWarnings(value = { "URLCONNECTION_SSRF_FD", "PATH_TRAVERSAL_IN", "DP_CREATE_CLASSLOADER_INSIDE_DO_PRIVILEGED" }) public void execute() throws MojoExecutionException, MojoFailureException {
    if (skip) {
    ...
    try {
      ...
      Checker checker = new Checker(new URLClassLoader(dependencies.toArray(new URL[0]), getClass().getClassLoader()), new ErrorListener() {
        @Override public void onError(Throwable t, Location loc, String msg) {
        ...
        @Override public void onWarning(Throwable t, Location loc, String msg) {
