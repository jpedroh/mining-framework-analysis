...
public class SearchResultsFeedModel implements Model {
  ...
  @Override public void init(Map initData) throws WebloggerException {
    ...
    if (weblogRequest instanceof WeblogFeedRequest) {
      ...
      throw new WebloggerException("weblogRequest is not a WeblogFeedRequest." + "  FeedModel only supports feed requests.");
    }
    ...
    String pagerUrl = urlStrategy.getWeblogFeedURL(weblog, feedRequest.getLocale(), feedRequest.getType(), feedRequest.getFormat(), null, null, null, false, true);
    if (feedRequest.getTerm() == null) {
      pager = new SearchResultsFeedPager(urlStrategy, pagerUrl, feedRequest.getPage(), feedRequest, results, false);
      return;
    }
    ...
    pager = new SearchResultsFeedPager(urlStrategy, pagerUrl, feedRequest.getPage(), feedRequest, results, (hits > (offset + limit)));
  }
  ...
  private void convertHitsToEntries(ScoreDoc[] hits, SearchOperation search) throws WebloggerException {
    ...
    try {
      ...
      for (int i = offset; i < offset + limit; i++) {
        ...
        if (!(websiteSpecificSearch && handle.equals(feedRequest.getWeblogHandle())) && doc.getField(FieldConstants.CATEGORY) != null) {
  ...
  public String getTerm() {
    ...
    return (query == null) ? "" : StringEscapeUtils.escapeXml11(Utilities.escapeHTML(query));
  }
