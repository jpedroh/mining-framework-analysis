...
public class FieldSerializerBenchmark {
  @Benchmark public void field(FieldSerializerState state) {
    state.roundTrip();
  }

  @Benchmark public void compatible(CompatibleState state) {
    state.roundTrip();
  }

  @Benchmark public void tagged(TaggedState state) {
    state.roundTrip();
  }

  @Benchmark public void version(VersionState state) {
    state.roundTrip();
  }

  @Benchmark public void custom(CustomState state) {
    state.roundTrip();
  }

  @Benchmark public void deserializeCollection(DeserializeCollectionStateDefault state) {
    state.roundTrip();
  }

  @State(value = Scope.Thread) static public abstract class BenchmarkState {
    @Param(value = { "true", "false" }) public boolean references;
    ...
    @Setup(value = Level.Trial) public void setup() {
      switch (objectType) {
        case sample:
  ...
  static public class CompatibleState extends BenchmarkState {
    @Param(value = { "true", "false" }) public boolean chunked;
  ...
  static public class DeserializeCollectionStateDefault extends ArrayClassResolverBenchmark.DeserializeCollectionState {
    @Override protected Kryo createKryo() {
      return new Kryo();
    }
  }
}