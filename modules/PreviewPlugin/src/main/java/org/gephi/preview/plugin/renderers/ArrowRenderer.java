 ...
 */
@ServiceProvider(service = Renderer.class, position = 200) public class ArrowRenderer implements Renderer {
  ...
  @Override public void render(final Item item, final RenderTarget target, final PreviewProperties properties) {
  ...
  @Override public CanvasSize getCanvasSize(final Item item, final PreviewProperties properties) {
    ...
    return properties.getBooleanValue(PreviewProperty.EDGE_CURVED) ? new CanvasSize() : new CanvasSize(minX, minY, maxX - minX, maxY - minY);
  }
  ...
  @Override public boolean needsItemBuilder(ItemBuilder itemBuilder, PreviewProperties properties) {
    return (itemBuilder instanceof EdgeBuilder || itemBuilder instanceof NodeBuilder) && showArrows(properties);
  }
  ...
  private class Helper {
    ...
    public Helper(final Item item, final PreviewProperties properties) {
      ...
      final float size = properties.getFloatValue(PreviewProperty.ARROW_SIZE) * weight.floatValue();
      float radius = -(properties.getFloatValue(PreviewProperty.EDGE_RADIUS) + (Float) targetItem.getData(NodeItem.SIZE) / 2f);
      if (radius > 0) {
        radius = 0;
      }
