 ...
 */
@ServiceProvider(service = Renderer.class, position = 100) public class EdgeRenderer implements Renderer {
  ...
  /**
     ...
     */
  @Deprecated public static final String BEZIER_CURVENESS = "edge.bezier-curveness";
  ...
  public static Color getColor(final Item item, final PreviewProperties properties) {
    ...
    final EdgeColor edgeColor = properties.getValue(PreviewProperty.EDGE_COLOR);
    final Color color = edgeColor.getColor(item.getData(EdgeItem.COLOR), sourceItem.getData(NodeItem.COLOR), targetItem.getData(NodeItem.COLOR));
  ...
  @Override public void preProcess(PreviewModel previewModel) {
    ...
    for (Item edge : edgeItems) {
      minWeight = Math.min(minWeight, edge.getData(EdgeItem.WEIGHT));
      maxWeight = Math.max(maxWeight, edge.getData(EdgeItem.WEIGHT));
    }
    ...
    if (rescaleWeight) {
      ...
      if (!Double.isInfinite(minWeight) && !Double.isInfinite(maxWeight) && !NumberUtils.equalsEpsilon(maxWeight, minWeight)) {
        for (final Item item : edgeItems) {
    ...
    for (final Item item : edgeItems) {
      if (!(Boolean) item.getData(EdgeItem.SELF_LOOP)) {
        final float edgeRadius = properties.getFloatValue(PreviewProperty.EDGE_RADIUS);
        ...
        final float targetRadius = -(edgeRadius + (Float) targetItem.getData(NodeItem.SIZE) / 2f + arrowRadiusSize);
        item.setData(TARGET_RADIUS, targetRadius);
        final Item sourceItem = item.getData(SOURCE);
        final float sourceRadius = -(edgeRadius + (Float) sourceItem.getData(NodeItem.SIZE) / 2f);
  ...
  @Override public void postProcess(PreviewModel previewModel, RenderTarget renderTarget, PreviewProperties properties) {
  }

  @Override public CanvasSize getCanvasSize(Item item, PreviewProperties properties) {
    if (isSelfLoopEdge(item)) {
  ...
  @Override public boolean needsItemBuilder(ItemBuilder itemBuilder, PreviewProperties properties) {
    return (itemBuilder instanceof EdgeBuilder || itemBuilder instanceof NodeBuilder) && showEdges(properties);
  }
  ...
  private boolean showEdges(PreviewProperties properties) {
    return properties.getBooleanValue(PreviewProperty.SHOW_EDGES) && !properties.getBooleanValue(PreviewProperty.MOVING);
  }
  ...
  private static class StraightEdgeRenderer {
    public void render(final Item item, final RenderTarget target, final PreviewProperties properties) {
      ...
      if (target instanceof G2DTarget) {
        ...
        graphics.setStroke(new BasicStroke(getThickness(item), BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));
    ...
    public CanvasSize getCanvasSize(final Item item, final PreviewProperties properties) {
      final Item sourceItem = item.getData(SOURCE);
      final Item targetItem = item.getData(TARGET);
    ...
    public void render(final Item item, final RenderTarget target, final PreviewProperties properties) {
      ...
      if (target instanceof G2DTarget) {
        ...
        if (target instanceof SVGTarget) {
          ...
          final Element edgeElem = svgTarget.createElement("path");
          edgeElem.setAttribute("class", String.format("%s %s", SVGUtils.idAsClassAttribute(((Node) h.sourceItem.getSource()).getId()), SVGUtils.idAsClassAttribute(((Node) h.targetItem.getSource()).getId())));
          String path = String.format(Locale.ENGLISH, "M %f,%f A %f,%f %d,%d %d,%f,%f", h.x1, h.y1, h.r, h.r, 0, 0, 1, h.x2, h.y2);
...
>>>>>>> /usr/src/app/output/gephi/gephi/e161809510d2c809c5d9c338bdf90fbe1c32a3ad/modules/PreviewPlugin/src/main/java/org/gephi/preview/plugin/renderers/EdgeRenderer.java/right.java
    ...
    public CanvasSize getCanvasSize(final Item item, final PreviewProperties properties) {
      ...
      final float maxX = Math.max(h.x1, h.x2);
      final float maxY = Math.max(h.y1, h.y2);
      return new CanvasSize(minX, minY, maxX - minX, maxY - minY);
    }
    ...
    private static class Helper {
      ...
      private Vector computeCtrlPoint(final Float x, final Float y, final Vector direction, final float factor, final Vector normalVector) {
    ...
    public void render(final Item item, final RenderTarget target, final PreviewProperties properties) {
      ...
      if (target instanceof G2DTarget) {
        ...
        if (target instanceof SVGTarget) {
          ...
          selfLoopElem.setAttribute("d", String.format(Locale.ENGLISH, "M %f,%f C %f,%f %f,%f %f,%f", h.x, h.y, h.v1.x, h.v1.y, h.v2.x, h.v2.y, h.x, h.y));
          ...
          selfLoopElem.setAttribute("stroke-width", Float.toString(getThickness(item) * svgTarget.getScaleRatio()));
