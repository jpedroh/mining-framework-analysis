        ...
        if ("body".equals(value)) {
          ...
          for (String key : keys) {
            if (key.startsWith("x-")) {
              ...
              if (!BODY_PARAMETER_KEYS.contains(key)) {
                result.extra(location, key, obj.get(key));
              }
            }
          }
    ...
    if (type != null) {
      ...
      for (String key : keys) {
        if (key.startsWith("x-")) {
          ...
          if (!SECURITY_SCHEME_KEYS.contains(key)) {
            result.extra(location, key, node.get(key));
          }
        }
      }
    ...
    if (node != null) {
      ...
      for (String key : keys) {
        if (key.startsWith("x-")) {
          ...
          if (!EXTERNAL_DOCS_KEYS.contains(key)) {
            result.extra(location + ".externalDocs", key, node.get(key));
          }
        }
      }
