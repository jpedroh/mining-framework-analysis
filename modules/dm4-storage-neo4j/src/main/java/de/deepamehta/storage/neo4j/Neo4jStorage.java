...
public class Neo4jStorage implements DeepaMehtaStorage {
  ...
  TopicModel buildTopic(Node topicNode) {
    return new TopicModel(topicNode.getId(), uri(topicNode), typeUri(topicNode), simpleValue(topicNode), null);
  }
  ...
  AssociationModel buildAssociation(Node assocNode) {
    ...
    return new AssociationModel(assocNode.getId(), uri(assocNode), typeUri(assocNode), roleModels.get(0), roleModels.get(1), simpleValue(assocNode), null);
  }
  ...
  private Node fetchTopicNode(long topicId) {
    return checkType(fetchNode(topicId), NodeType.TOPIC);
  }

  private Node fetchAssociationNode(long assocId) {
    return checkType(fetchNode(assocId), NodeType.ASSOC);
  }
  ...
  private List<RelatedTopicModel> buildRelatedTopics(List<AssociationModel> assocs, long playerId) {
    ...
    for (AssociationModel assoc : assocs) {
      relTopics.add(new RelatedTopicModel(fetchTopic(assoc.getOtherPlayerId(playerId)), assoc));
    }
    return relTopics;
  }
  ...
  private List<RelatedAssociationModel> buildRelatedAssociations(List<AssociationModel> assocs, long playerId) {
    ...
    for (AssociationModel assoc : assocs) {
      relAssocs.add(new RelatedAssociationModel(fetchAssociation(assoc.getOtherPlayerId(playerId)), assoc));
    }
    return relAssocs;
  }
