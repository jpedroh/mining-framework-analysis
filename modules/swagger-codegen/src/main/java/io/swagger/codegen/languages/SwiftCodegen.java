...
public class SwiftCodegen extends DefaultCodegen implements CodegenConfig {
  ...
  public SwiftCodegen() {
    ...
    cliOptions.add(new CliOption(UNWRAP_REQUIRED, "Treat \'required\' properties in response as non-optional " + "(which would crash the app if api returns null as opposed to required option specified in json schema"));
  ...
  @Override public CodegenProperty fromProperty(String name, Property p) {
    ...
    if (codegenProperty.isEnum) {
      ...
      codegenProperty.datatypeWithEnum = StringUtils.left(codegenProperty.datatypeWithEnum, codegenProperty.datatypeWithEnum.length() - "Enum".length());
  ...
  @Override public CodegenOperation fromOperation(String path, String httpMethod, Operation operation, Map<String, Model> definitions, Swagger swagger) {
    ...
    parameters = Lists.newArrayList(Iterators.filter(parameters.iterator(), new Predicate<Parameter>() {
      @Override public boolean apply(@Nullable Parameter parameter) {
