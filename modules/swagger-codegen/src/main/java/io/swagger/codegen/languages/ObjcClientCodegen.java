...
public class ObjcClientCodegen extends DefaultCodegen implements CodegenConfig {
  ...
  public ObjcClientCodegen() {
    ...
    foundationClasses = new HashSet<String>(Arrays.asList("NSNumber", "NSObject", "NSString", "NSDate", "NSData", "NSURL", "NSDictionary"));
    ...
    cliOptions.add(new CliOption(CLASS_PREFIX, "prefix for generated classes (convention: Abbreviation of pod name e.g. `HN` for `HackerNews`).`").defaultValue("SWG"));
    cliOptions.add(new CliOption(POD_NAME, "cocoapods package name (convention: CameCase).").defaultValue("SwaggerClient"));
    cliOptions.add(new CliOption(CodegenConstants.POD_VERSION, "cocoapods package version.").defaultValue("1.0.0"));
    ...
    cliOptions.add(new CliOption(GIT_REPO_URL, "URL for the git repo where this podspec should point to.").defaultValue("https://github.com/swagger-api/swagger-codegen"));
  ...
  @Override public String toInstantiationType(Property p) {
  ...
  @Override public boolean isDataTypeBinary(String dataType) {
    return dataType.toLowerCase().startsWith("nsdata");
  }

  @Override public String toModelName(String type) {
  ...
  public String toModelNameWithoutReservedWordCheck(String type) {
    ...
    if (typeMapping.keySet().contains(type) || foundationClasses.contains(type) || importMapping.values().contains(type) || defaultIncludes.contains(type) || languageSpecificPrimitives.contains(type)) {
  ...
  @Override protected void setNonArrayMapProperty(CodegenProperty property, String type) {
  ...
  @SuppressWarnings(value = { "static-method" }) public String escapeSpecialWord(String name) {
    return "var_" + name;
  }
    ...
    if ("NSString*".equalsIgnoreCase(type)) {
      ...
      if ("NSNumber*".equals(type)) {
        ...
        if ("BOOLEAN".equalsIgnoreCase(type) || "bool".equalsIgnoreCase(type)) {
          ...
          if ("NSURL*".equalsIgnoreCase(type)) {
            if (example == null) {
              example = "/path/to/file";
            }
            example = "[NSURL fileURLWithPath:@\"" + escapeText(example) + "\"]";
          } else {
            if ("NSDate*".equalsIgnoreCase(type)) {
              if (example == null) {
                example = "2013-10-20T19:20:30+01:00";
              }
              example = "@\"" + escapeText(example) + "\"";
            } else {
              if ("NSData".equalsIgnoreCase(type)) {
                example = "1234";
              } else {
