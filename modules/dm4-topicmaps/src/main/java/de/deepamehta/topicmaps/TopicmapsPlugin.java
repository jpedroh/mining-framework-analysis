...
@Path(value = "/topicmap") @Consumes(value = "application/json") @Produces(value = "application/json") public class TopicmapsPlugin extends PluginActivator implements TopicmapsService {
  ...
  @POST @Transactional @Override public Topic createTopicmap(@QueryParam(value = "name") String name, @QueryParam(value = "renderer_uri") String topicmapRendererUri, @QueryParam(value = "private") boolean isPrivate) {
  ...
  @POST @Path(value = "/{id}/topic/{topic_id}") @Transactional @Override public void addTopicToTopicmap(@PathParam(value = "id") final long topicmapId, @PathParam(value = "topic_id") final long topicId, final ViewProperties viewProps) {
    try {
  ...
  @POST @Path(value = "/{id}/association/{assoc_id}") @Transactional @Override public void addAssociationToTopicmap(@PathParam(value = "id") final long topicmapId, @PathParam(value = "assoc_id") final long assocId) {
    try {
      dm4.getAccessControl().runWithoutWorkspaceAssignment(new Callable<Void>() {
        @Override public Void call() {
  ...
  @PUT @Path(value = "/{id}/topic/{topic_id}") @Transactional @Override public void setViewProperties(@PathParam(value = "id") long topicmapId, @PathParam(value = "topic_id") long topicId, ViewProperties viewProps) {
    storeViewProperties(topicmapId, topicId, viewProps);
  }

  @PUT @Path(value = "/{id}/topic/{topic_id}/{x}/{y}") @Transactional @Override public void setTopicPosition(@PathParam(value = "id") long topicmapId, @PathParam(value = "topic_id") long topicId, @PathParam(value = "x") int x, @PathParam(value = "y") int y) {
    try {
  ...
  @PUT @Path(value = "/{id}/topic/{topic_id}/{visibility}") @Transactional @Override public void setTopicVisibility(@PathParam(value = "id") long topicmapId, @PathParam(value = "topic_id") long topicId, @PathParam(value = "visibility") boolean visibility) {
    storeViewProperties(topicmapId, topicId, new ViewProperties(visibility));
  }

  @DELETE @Path(value = "/{id}/association/{assoc_id}") @Transactional @Override public void removeAssociationFromTopicmap(@PathParam(value = "id") long topicmapId, @PathParam(value = "assoc_id") long assocId) {
    try {
  ...
  @PUT @Path(value = "/{id}") @Transactional @Override public void setClusterPosition(@PathParam(value = "id") long topicmapId, ClusterCoords coords) {
    for (ClusterCoords.Entry entry : coords) {
      setTopicPosition(topicmapId, entry.topicId, entry.x, entry.y);
    }
  }

  @PUT @Path(value = "/{id}/translation/{x}/{y}") @Transactional @Override public void setTopicmapTranslation(@PathParam(value = "id") long topicmapId, @PathParam(value = "x") int transX, @PathParam(value = "y") int transY) {
    try {
  ...
  private class Messenger {
    ...
    private void addTopicToTopicmap(long topicmapId, TopicViewModel topic) {
      try {
        messageToAllButOne(new JSONObject().put("type", "addTopicToTopicmap").put("args", new JSONObject().put("topicmapId", topicmapId).put("viewTopic", topic.toJSON())));
      } catch (Exception e) {
    ...
    private void addAssociationToTopicmap(long topicmapId, AssociationModel assoc) {
      try {
        messageToAllButOne(new JSONObject().put("type", "addAssocToTopicmap").put("args", new JSONObject().put("topicmapId", topicmapId).put("assoc", assoc.toJSON())));
      } catch (Exception e) {
    ...
    private void setTopicPosition(long topicmapId, long topicId, int x, int y) {
      try {
        messageToAllButOne(new JSONObject().put("type", "setTopicPosition").put("args", new JSONObject().put("topicmapId", topicmapId).put("topicId", topicId).put("pos", new JSONObject().put("x", x).put("y", y))));
      } catch (Exception e) {
