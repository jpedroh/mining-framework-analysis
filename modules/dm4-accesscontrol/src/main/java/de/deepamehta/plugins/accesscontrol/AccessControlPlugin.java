...
@Path(value = "/accesscontrol") @Consumes(value = "application/json") @Produces(value = "application/json") public class AccessControlPlugin extends PluginActivator implements AccessControlService, InitializePluginListener, SecurityContext, PostCreateTopicListener, PostCreateAssociationListener, PreSendTopicListener, PreSendAssociationListener, PreSendTopicTypeListener, PostInstallPluginListener, IntroduceTopicTypeListener, PluginServiceArrivedListener, PluginServiceGoneListener {
  private static final boolean READ_REQUIRES_LOGIN = Boolean.valueOf(System.getProperty("dm4.security.read_requires_login"));

  private static final boolean WRITE_REQUIRES_LOGIN = Boolean.valueOf(System.getProperty("dm4.security.write_requires_login"));
  ...
  @POST @Path(value = "/login") @Override public Topic login() {
    return getUsername();
  }

  @POST @Path(value = "/logout") @Produces(value = "text/plain") @Override public boolean logout() {
    request.getSession(false).invalidate();
    return READ_REQUIRES_LOGIN;
  }

  @GET @Path(value = "/user") @Override public Topic getUsername() {
    try {
  ...
  @GET @Path(value = "/topic/{topic_id}") @Override public Permissions getTopicPermissions(@PathParam(value = "topic_id") long topicId) {
    return createPermissions(hasPermission(getUsername(), Operation.WRITE, topicId));
  }

  @GET @Path(value = "/owner/{user_id}/{type_uri}") @Override public Topic getOwnedTopic(@PathParam(value = "user_id") long userId, @PathParam(value = "type_uri") String typeUri) {
    return null;
  }

  @POST @Path(value = "/topic/{topic_id}/owner/{user_id}") @Override public void setOwner(@PathParam(value = "topic_id") long topicId, @PathParam(value = "user_id") long userId) {
  ...
  @POST @Path(value = "/topic/{topic_id}/role/{role_uri}") @Override public void createACLEntry(@PathParam(value = "topic_id") long topicId, @PathParam(value = "role_uri") Role role, Permissions permissions) {
    createACLEntry(dms.getTopic(topicId, false, null), role, permissions);
  }

  @Override public void createACLEntry(DeepaMehtaObject object, Role role, Permissions permissions) {
  ...
  @POST @Path(value = "/user/{user_id}/{workspace_id}") @Override public void joinWorkspace(@PathParam(value = "workspace_id") long workspaceId, @PathParam(value = "user_id") long userId) {
    dms.createAssociation(new AssociationModel(WORKSPACE_MEMBERSHIP, new TopicRoleModel(userId, ROLE_TYPE_USER), new TopicRoleModel(workspaceId, ROLE_TYPE_WORKSPACE)), null);
  }

  @Override public void checkRequest(HttpServletRequest request) throws AccessControlException {
  ...
  private Topic fetchUserAccount(Topic username) {
    Topic userAccount = username.getRelatedTopic("dm4.core.composition", "dm4.core.part", "dm4.core.whole", "dm4.accesscontrol.user_account", true, false, null);
    if (userAccount == null) {
  ...
  @Override public void preSendTopic(Topic topic, ClientState clientState) {
    if (isPluginTopic(topic)) {
      enrichWithPermissions(topic, false);
      return;
    }
  ...
  @Override public void preSendAssociation(Association assoc, ClientState clientState) {
  ...
  @Override public void preSendTopicType(TopicType topicType, ClientState clientState) {
    if (isPluginType(topicType) || topicType.getUri().equals("dm4.core.meta_meta_type")) {
      enrichWithPermissions(topicType, false, false);
      return;
    }
    logger.fine("### Enriching topic type \"" + topicType.getUri() + "\" with its permissions");
    Topic username = getUsername();
    enrichWithPermissions(topicType, hasPermission(username, Operation.WRITE, topicType), hasPermission(username, Operation.CREATE, topicType));
  }

  @Override public void postInstallPlugin() {
  ...
  @Override public void pluginServiceArrived(PluginService service) {
  ...
  @Override public void pluginServiceGone(PluginService service) {
  ...
  private Topic createUserAccount(Credentials cred) {
    return dms.createTopic(new TopicModel("dm4.accesscontrol.user_account", new CompositeValue().put("dm4.accesscontrol.username", cred.username).put("dm4.accesscontrol.password", cred.password)), null);
  }
...
=======
  private void setCreator(Topic topic) {
    Topic username = getUsername();
    if (username == null) {
      logger.fine("Assigning a creator to " + info(topic) + " failed " + "(no user is logged in). Assigning user \"admin\" instead.");
      username = fetchAdminUser();
    }
    setCreator(topic, username.getId());
  }
>>>>>>> /usr/src/app/output/jri/deepamehta/6a7295cd1ff99f1f7658108bf10b3b1f4e40d639/modules/dm4-accesscontrol/src/main/java/de/deepamehta/plugins/accesscontrol/AccessControlPlugin.java/right.java
...
>>>>>>> /usr/src/app/output/jri/deepamehta/6a7295cd1ff99f1f7658108bf10b3b1f4e40d639/modules/dm4-accesscontrol/src/main/java/de/deepamehta/plugins/accesscontrol/AccessControlPlugin.java/right.java
  ...
  private void assignCreator(DeepaMehtaObject object) {
    Topic username = getUsername();
    if (username == null) {
      logger.fine("Assigning a creator to " + info(object) + " failed (no user is logged in). " + "The default user (\"admin\") is assigned instead.");
  ...
  private boolean hasPermission(Topic username, Operation operation, DeepaMehtaObject object) {
    ...
    for (RelatedTopic aclEntry : 
<<<<<<< /usr/src/app/output/jri/deepamehta/6a7295cd1ff99f1f7658108bf10b3b1f4e40d639/modules/dm4-accesscontrol/src/main/java/de/deepamehta/plugins/accesscontrol/AccessControlPlugin.java/left.java
    getACLEntries(object)
=======
    fetchACLEntries(topic)
>>>>>>> /usr/src/app/output/jri/deepamehta/6a7295cd1ff99f1f7658108bf10b3b1f4e40d639/modules/dm4-accesscontrol/src/main/java/de/deepamehta/plugins/accesscontrol/AccessControlPlugin.java/right.java
    ) {
  ...
  private boolean userOccupiesRole(DeepaMehtaObject object, Topic username, String roleUri) {
    ...
    if (roleUri.equals("dm4.accesscontrol.role_user")) {
      ...
      if (roleUri.equals("dm4.accesscontrol.role_member")) {
        ...
        if (roleUri.equals("dm4.accesscontrol.role_owner")) {
          if (userIsOwner(username, object)) {
            return true;
          }
        } else {
          if (roleUri.equals("dm4.accesscontrol.role_creator")) {
  ...
  private boolean userIsMember(Topic username, DeepaMehtaObject object) {
    Set<RelatedTopic> workspaces = wsService.getWorkspaces(object.getId());
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private Topic getCreator(DeepaMehtaObject object) {
    ...
    return creator.getRelatedTopic("dm4.core.aggregation", "dm4.core.whole", "dm4.core.part", "dm4.accesscontrol.username", false, false, null);
  }
  ...
  private Topic getOwner(DeepaMehtaObject object) {
    ...
    return owner.getRelatedTopic("dm4.core.aggregation", "dm4.core.whole", "dm4.core.part", "dm4.accesscontrol.username", false, false, null);
  }

  private boolean isMemberOfWorkspace(long userId, long workspaceId) {
    return dms.getAssociation(WORKSPACE_MEMBERSHIP, userId, workspaceId, ROLE_TYPE_USER, ROLE_TYPE_WORKSPACE, false, null) != null;
  }
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private TopicModel createCreatorModel(long usernameId) {
    return new TopicModel("dm4.accesscontrol.creator", new CompositeValue().putRef("dm4.accesscontrol.username", usernameId));
  }

  private TopicModel createOwnerModel(long userId) {
    return new TopicModel("dm4.accesscontrol.owner", new CompositeValue().putRef("dm4.accesscontrol.user_account", userId));
  }

  private TopicModel createAclEntryModel(Role role, Permissions permissions) {
    return new TopicModel("dm4.accesscontrol.acl_entry", new CompositeValue().putRef("dm4.accesscontrol.role", role.uri).put("dm4.accesscontrol.permission", permissions.asTopics()));
  }
