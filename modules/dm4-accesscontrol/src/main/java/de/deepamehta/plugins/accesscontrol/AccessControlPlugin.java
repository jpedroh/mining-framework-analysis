...
@Path(value = "/accesscontrol") @Consumes(value = "application/json") @Produces(value = "application/json") public class AccessControlPlugin extends PluginActivator implements AccessControlService, PostCreateTopicListener, PreSendTopicListener, PreSendTopicTypeListener, PostInstallPluginListener, IntroduceTopicTypeListener, PluginServiceArrivedListener, PluginServiceGoneListener {
  ...
  @GET @Path(value = "/user/{username}") @Override public Topic lookupUserAccount(@PathParam(value = "username") String username) {
    logger.info("username=\"" + username + "\"");
    return getUserAccount(username);
  }

  @GET @Path(value = "/user") @Override public Topic getUsername(@HeaderParam(value = "Cookie") ClientState clientState) {
    if (clientState == null) {
      return null;
    }
  ...
  @GET @Path(value = "/topic/{topic_id}") @Override public Permissions getTopicPermissions(@PathParam(value = "topic_id") long topicId, @HeaderParam(value = "Cookie") ClientState clientState) {
  ...
  @Override public void createACLEntry(Topic topic, Role role, Permissions permissions) {
  ...
  @Override public void preSendTopic(Topic topic, ClientState clientState) {
    ...
    Topic username = getUsername(clientState);
    enrichWithPermissions(topic, hasPermission(username, Operation.WRITE, topic));
  }
  ...
  private Topic createUserAccount(String username, String password) {
    return dms.createTopic(new TopicModel("dm4.accesscontrol.user_account", new CompositeValue().put("dm4.accesscontrol.username", username).put("dm4.accesscontrol.password", encryptPassword(password))), null);
  }
  ...
  private Topic getUserAccount(String username) {
    ...
    return userName.getRelatedTopic("dm4.core.composition", "dm4.core.part", "dm4.core.whole", "dm4.accesscontrol.user_account", true, false, null);
  }
  ...
  private void setCreator(Topic topic, ClientState clientState) {
    Topic username = getUsername(clientState);
    if (username == null) {
      logger.warning("Assigning a creator to " + info(topic) + " failed (no user is logged in). " + "Assigning user \"admin\" instead.");
  ...
  private TopicModel creatorModel(long usernameId) {
    return new TopicModel("dm4.accesscontrol.creator", new CompositeValue().put_ref("dm4.accesscontrol.username", usernameId));
  }

  private TopicModel ownerModel(long userId) {
    return new TopicModel("dm4.accesscontrol.owner", new CompositeValue().put_ref("dm4.accesscontrol.user_account", userId));
  }

  private TopicModel aclEntryModel(Role role, Permissions permissions) {
    return new TopicModel("dm4.accesscontrol.acl_entry", new CompositeValue().put_ref("dm4.accesscontrol.role", role.uri).put("dm4.accesscontrol.permission", permissions.asTopics()));
  }
