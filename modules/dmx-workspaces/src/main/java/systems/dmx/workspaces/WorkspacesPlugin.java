...
@Path(value = "/workspaces") @Consumes(value = "application/json") @Produces(value = "application/json") public class WorkspacesPlugin extends PluginActivator implements WorkspacesService, IntroduceTopicType, IntroduceAssocType, PostCreateTopic, PostCreateAssoc, PreDeleteTopic {
  ...
  @POST @Transactional @Override public Topic createWorkspace(@QueryParam(value = "name") final String name, @QueryParam(value = "uri") final String uri, @QueryParam(value = "sharing_mode_uri") final SharingMode sharingMode) {
    ...
    try {
      Topic workspace = dmx.getPrivilegedAccess().runWithoutWorkspaceAssignment(new Callable<Topic>() {
        @Override public Topic call() {
          ...
          Topic workspace = dmx.createTopic(mf.newTopicModel(uri, WORKSPACE, mf.newChildTopicsModel().set(WORKSPACE_NAME, name).setRef(SHARING_MODE, sharingMode.getUri())));
          Topic topicmap = topicmapsService.createTopicmap(TopicmapsService.DEFAULT_TOPICMAP_NAME, TopicmapsService.DEFAULT_TOPICMAP_TYPE_URI, null);
          dmx.getPrivilegedAccess().assignToWorkspace(topicmap, workspace.getId());
          return workspace;
        }
      });
      me.newWorkspace(workspace);
      return workspace;
    } catch (Exception e) {
  ...
  @PUT @Path(value = "/{workspace_id}/object/{object_id}") @Transactional public DirectivesResponse assignToWorkspace(@PathParam(value = "object_id") long objectId, @PathParam(value = "workspace_id") long workspaceId) {
    try {
  ...
  @Override public void assignToWorkspace(DMXObject object, long workspaceId) {
    try {
      checkAssignmentArgs(object, workspaceId);
      __assignToWorkspace(object, workspaceId);
    } catch (Exception e) {
  ...
  @Override public void assignTypeToWorkspace(DMXType type, long workspaceId) {
    try {
  ...
  @GET @Path(value = "/{id}/assocs") @Override public List<Assoc> getAssignedAssocs(@PathParam(value = "id") long workspaceId) {
  ...
  @GET @Path(value = "/{id}/assocs/{assoc_type_uri}") @Override public List<Assoc> getAssignedAssocs(@PathParam(value = "id") long workspaceId, @PathParam(value = "assoc_type_uri") String assocTypeUri) {
  ...
  @Override public void preInstall() {
    configService.registerConfigDefinition(new ConfigDefinition(ConfigTarget.TYPE_INSTANCES, "dmx.accesscontrol.username", mf.newTopicModel("dmx.workspaces.enabled_sharing_modes", mf.newChildTopicsModel().set("dmx.workspaces.private.enabled", SHARING_MODE_PRIVATE_ENABLED).set("dmx.workspaces.confidential.enabled", SHARING_MODE_CONFIDENTIAL_ENABLED).set("dmx.workspaces.collaborative.enabled", SHARING_MODE_COLLABORATIVE_ENABLED).set("dmx.workspaces.public.enabled", SHARING_MODE_PUBLIC_ENABLED).set("dmx.workspaces.common.enabled", SHARING_MODE_COMMON_ENABLED)), ConfigModificationRole.ADMIN));
  }
  ...
  /**
     ...
     *       type if its URI begins with "dmx."
     */
  @Override public void introduceAssocType(AssocType assocType) {
  ...
  @Override public void postCreateTopic(Topic topic) {
    ...
    _assignToWorkspace(topic, workspaceId, false);
  }

  /**
     * Assigns every created association to the current workspace.
     */
  @Override public void postCreateAssoc(Assoc assoc) {
    ...
    _assignToWorkspace(assoc, workspaceId, false);
  }
  ...
  private void __assignToWorkspace(DMXObject object, long workspaceId) {
    facetsService.updateFacet(object, WORKSPACE_FACET, mf.newFacetValueModel(WORKSPACE + "#" + WORKSPACE_ASSIGNMENT).setRef(workspaceId));
  ...
  private boolean isWorkspacePart(Topic topic) {
    ...
    return typeUri.equals(WORKSPACE_NAME) || typeUri.equals(WORKSPACE_DESCRIPTION);
  }
  ...
  private class Messenger {
    ...
    private void newWorkspace(Topic workspace) {
      try {
        sendToAllButOrigin(new JSONObject().put("type", "newWorkspace").put("args", new JSONObject().put("workspace", workspace.toJSON())));
      } catch (Exception e) {
