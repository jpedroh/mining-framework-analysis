...
class PluginManager {
  ...
  synchronized PluginImpl getPlugin(String pluginUri) {
    ...
    if (plugin == null) {
      throw new RuntimeException("Plugin \"" + pluginUri + "\" is not installed/activated. Activated plugins: " + activatedPlugins.keySet());
    }
    return plugin;
  }
  ...
  private boolean checkAllPluginsActivated() {
    ...
    logger.info("### Bundles total: " + bundles.length + ", DMX plugins: " + plugins + ", Activated: " + activated);
  ...
  private void removeFromActivatedPlugins(String pluginUri) {
    if (activatedPlugins.remove(pluginUri) == null) {
      throw new RuntimeException("Removing plugin \"" + pluginUri + "\" from pool of activated plugins failed: " + "not found in " + activatedPlugins);
    }
  }
