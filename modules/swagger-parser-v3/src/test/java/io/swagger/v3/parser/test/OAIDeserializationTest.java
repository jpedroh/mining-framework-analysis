...
public class OAIDeserializationTest {
  @Test public void testDeserializeSimpleDefinition() throws Exception {
  ...
  @Test public void testDeserializeYamlDefinitionMissingSchema_Issue1951() throws Exception {
    ...
    errorResponse.description("My error type").getContent().addMediaType("application/json", new MediaType().schema(new ComposedSchema().$ref("NotAddedYet").type("schema")));
    ...
    api.setPaths(new Paths().addPathItem("/thingy", new PathItem().post(new Operation().responses(resp).requestBody(new RequestBody().content(content)))));
  ...
  @Test public void testDeserializeYamlDefinition() throws Exception {
    OpenAPI api = new OpenAPI();
    api.setSpecVersion(SpecVersion.V30);
    api.setComponents(new Components());
