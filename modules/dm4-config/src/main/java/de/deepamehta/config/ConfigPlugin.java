...
@Path(value = "/config") @Produces(value = "application/json") public class ConfigPlugin extends PluginActivator implements ConfigService, PostCreateTopicListener {
  ...
  @GET @Path(value = "/{config_type_uri}/topic/{topic_id}") @Override public RelatedTopic getConfigTopic(@PathParam(value = "config_type_uri") String configTypeUri, @PathParam(value = "topic_id") long topicId) {
    return _getConfigTopic(configTypeUri, topicId);
  }

  @Override public void createConfigTopic(String configTypeUri, Topic topic) {
    _createConfigTopic(getApplicableConfigDefinition(topic, configTypeUri), topic);
  }

  @Override public void registerConfigDefinition(ConfigDefinition configDef) {
    try {
  ...
  @Override public void unregisterConfigDefinition(String configTypeUri) {
    try {
  ...
  @GET public ConfigDefinitions getConfigDefinitions() {
    try {
  ...
  private RelatedTopic _createConfigTopic(final ConfigDefinition configDef, final Topic topic) {
    ...
    try {
      ...
      return ac.runWithoutWorkspaceAssignment(new Callable<RelatedTopic>() {
        @Override public RelatedTopic call() {
          Topic configTopic = dm4.createTopic(configDef.getConfigValue(topic));
          dm4.createAssociation(mf.newAssociationModel(ASSOC_TYPE_CONFIGURATION, mf.newTopicRoleModel(topic.getId(), ROLE_TYPE_CONFIGURABLE), mf.newTopicRoleModel(configTopic.getId(), ROLE_TYPE_DEFAULT)));
  ...
  private ConfigDefinition getApplicableConfigDefinition(Topic topic, String configTypeUri) {
    ...
    ConfigDefinition configDef = findByConfigTypeUri(configDefs, configTypeUri);
    if (configDef == null) {
      throw new RuntimeException("For " + info(topic) + " no config definition for type \"" + configTypeUri + "\" registered");
    }
    return configDef;
  }
