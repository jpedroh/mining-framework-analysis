 ...
 */
@ServiceProvider(service = PreviewController.class) public class PreviewControllerImpl implements PreviewController {
  ...
  @Override public void refreshPreview() {
    refreshPreview(model.getWorkspace());
  }

  @Override public synchronized void refreshPreview(Workspace workspace) {
  ...
  @Override public void render(RenderTarget target, Workspace workspace) {
    PreviewModelImpl m = getModel(workspace);
    render(target, m.getManagedEnabledRenderers(), m);
  }

  @Override public void render(RenderTarget target, Renderer[] renderers) {
    render(target, renderers, getModel());
  }

  @Override public void render(RenderTarget target, Renderer[] renderers, Workspace workspace) {
  ...
  @Override public PreviewModelImpl getModel() {
    if (model == null) {
      ProjectController pc = Lookup.getDefault().lookup(ProjectController.class);
      if (pc.getCurrentWorkspace() != null) {
  ...
  @Override public synchronized PreviewModelImpl getModel(Workspace workspace) {
  ...
  @Override public RenderTarget getRenderTarget(String name) {
    return getRenderTarget(name, getModel());
  }

  @Override public RenderTarget getRenderTarget(String name, Workspace workspace) {
  ...
  @Override public Renderer[] getRegisteredRenderers() {
  ...
  @Override public boolean isAnyPluginRendererRegistered() {
  ...
  @Override public boolean sendMouseEvent(PreviewMouseEvent event) {
    return sendMouseEvent(event, Lookup.getDefault().lookup(ProjectController.class).getCurrentWorkspace());
  }

  @Override public boolean sendMouseEvent(PreviewMouseEvent event, Workspace workspace) {
    if (workspace == null) {
      return false;
    }
  ...
  @Override public void setGlobalCanvasSize(boolean globalCanvasSize) {
    if (model != null) {
