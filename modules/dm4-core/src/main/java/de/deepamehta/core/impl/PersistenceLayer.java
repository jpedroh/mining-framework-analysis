...
public final class PersistenceLayer extends StorageDecorator {
  ...
  void deleteTopicType(String topicTypeUri) {
    try {
      _getTopicType(topicTypeUri).delete();
    } catch (Exception e) {
  ...
  void deleteAssociationType(String assocTypeUri) {
    try {
      _getAssociationType(assocTypeUri).delete();
    } catch (Exception e) {
  ...
  <O extends java.lang.Object> List<O> checkReadAccessAndInstantiate(Iterable<? extends DeepaMehtaObjectModelImpl> models) {
  ...
  private void createType(TypeModelImpl model, String uriPrefix) {
    ...
    createTopic(typeTopic, uriPrefix);
