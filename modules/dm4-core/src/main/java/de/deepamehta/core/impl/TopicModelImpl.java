...
class TopicModelImpl extends DeepaMehtaObjectModelImpl implements TopicModel {
  ...
  @Override public TopicModel clone() {
    try {
  ...
  @Override TopicImpl instantiate() {
    return new TopicImpl(this, pl);
  }

  @Override final TopicModelImpl createModelWithChildTopics(ChildTopicsModel childTopics) {
    return mf.newTopicModel(typeUri, childTopics);
  }

  @Override final TopicTypeModelImpl getType() {
    ...
    try {
      ...
      for (AssociationDefinitionModel assocDef : getType().getAssocDefs()) {
        ...
        if (cardinalityUri.equals("dm4.core.one")) {
          childTopic = childTopics.getTopicOrNull(assocDefUri);
        } else {
          if (cardinalityUri.equals("dm4.core.many")) {
            List<RelatedTopicModelImpl> _childTopics = childTopics.getTopicsOrNull(assocDefUri);
  ...
  private AssociationModelImpl fetchInstantiation() {
    RelatedTopicModelImpl topicType = getRelatedTopic("dm4.core.instantiation", "dm4.core.instance", "dm4.core.type", "dm4.core.topic_type");
    if (topicType == null) {
