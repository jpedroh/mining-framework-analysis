...
public class PluginImpl implements Plugin, EventHandler {
    ...
    return new ServiceTracker(bundleContext, serviceInterface.getName(), null) {
      @Override public Object addingService(ServiceReference serviceRef) {
        ...
        try {
          ...
          logger.log(Level.SEVERE, "An error occurred while adding service " + serviceInterface.getName() + " to " + pluginContext + ":", e);
        }
        return service;
      }
      ...
      @Override public void removedService(ServiceReference ref, Object service) {
        try {
          ...
          logger.log(Level.SEVERE, "An error occurred while removing service " + serviceInterface.getName() + " from " + pluginContext + ":", e);
        }
      }
    };
  }
  ...
  private void removeService(Object service, Class serviceInterface) {
    if (service == dm4) {
      ...
      unpublishFileSystem();
      dm4.pluginManager.deactivatePlugin(this);
      setCoreService(null);
    } else {
  ...
  private Topic createPluginTopic() {
    return dm4.createTopic(mf.newTopicModel(pluginUri, "dm4.core.plugin", mf.newChildTopicsModel().put("dm4.core.plugin_name", pluginName()).put("dm4.core.plugin_symbolic_name", pluginUri).put("dm4.core.plugin_migration_nr", 0)));
  }
  ...
  private List<Class<?>> getProviderClasses() throws IOException {
    ...
    for (String className : scanPackage("/provider")) {
      ...
      if (clazz == null) {
        ...
        if (!dm4.wpService.isProviderClass(clazz)) {
          continue;
        }
      }
  ...
  @Override public void handleEvent(Event event) {
    ...
    try {
      ...
      logger.log(Level.SEVERE, "An error occurred while handling PLUGIN_ACTIVATED event from \"" + pluginUri + "\" for " + this + ":", e);
    }
  }
