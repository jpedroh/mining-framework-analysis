...
public class PluginImpl implements Plugin, EventHandler {
  ...
  public void publishDirectory(String directoryPath, String uriNamespace, SecurityHandler securityHandler) {
    try {
      ...
      directoryResource = webPublishingService.publishStaticResources(directoryPath, uriNamespace, securityHandler);
    } catch (Exception e) {
  ...
  @Override public InputStream getStaticResource(String name) {
    try {
  ...
  @Override public boolean hasStaticResource(String name) {
  ...
  private Properties readConfigFile() {
    try {
      ...
      if (!hasStaticResource(PLUGIN_CONFIG_FILE)) {
        logger.info("Reading config file \"" + PLUGIN_CONFIG_FILE + "\" for " + this + " ABORTED " + "-- file does not exist");
        return properties;
      }
  ...
  private void createPluginServiceTrackers() {
    ...
    logger.info("Tracking " + injectableServices.size() + " plugin services for " + this + " " + injectableServices);
    ...
    for (Field field : pluginContext.getClass().getDeclaredFields()) {
      if (field.isAnnotationPresent(Inject.class)) {
        ...
        if (!PluginService.class.isAssignableFrom(fieldType)) {
          throw new RuntimeException("Injected field \"" + field.getName() + "\" is not a plugin service. " + "Use @Inject only for plugin services");
        }
    ...
    return new ServiceTracker(bundleContext, serviceInterface.getName(), null) {
      @Override public Object addingService(ServiceReference serviceRef) {
        ...
        try {
          ...
          logger.log(Level.SEVERE, "Adding service " + serviceInterface.getName() + " to " + pluginContext + " failed", e);
        }
        return service;
      }
      ...
      @Override public void removedService(ServiceReference ref, Object service) {
        try {
          ...
          logger.log(Level.SEVERE, "Removing service " + serviceInterface.getName() + " from " + pluginContext + " failed", e);
        }
      }
    };
  }
  ...
  private void checkRequirementsForActivation() {
    if (dms != null && webPublishingService != null && eventService != null && pluginServicesAvailable() && dependenciesAvailable()) {
  ...
  private Topic createPluginTopic() {
    return dms.createTopic(new TopicModel(pluginUri, "dm4.core.plugin", new CompositeValueModel().put("dm4.core.plugin_name", pluginName()).put("dm4.core.plugin_symbolic_name", pluginUri).put("dm4.core.plugin_migration_nr", 0)));
  }

  private Topic fetchPluginTopic() {
    return dms.getTopic("uri", new SimpleValue(pluginUri), false);
  }
  ...
  @Override public void handleEvent(Event event) {
    ...
    try {
      ...
      logger.log(Level.SEVERE, "Handling PLUGIN_ACTIVATED event from \"" + pluginUri + "\" for " + this + " failed", e);
    }
  }
