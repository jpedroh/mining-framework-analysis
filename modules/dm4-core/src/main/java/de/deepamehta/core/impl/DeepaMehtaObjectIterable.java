...
class TopicIterable implements Iterable<Topic> {
  ...
  @Override public Iterator<Topic> iterator() {
    return topics;
  }
}
...
class AssociationIterable implements Iterable<Association> {
  ...
  @Override public Iterator<Association> iterator() {
    return assocs;
  }
}
...
class TopicIterator extends ObjectIterator<Topic, TopicModel> {
  ...
  @Override Iterator<TopicModel> fetchObjects() {
    return dms.storageDecorator.fetchAllTopics();
  }

  @Override Topic instantiateObject(TopicModel model) {
    return dms.instantiateTopic(model, true);
  }
}
...
class AssociationIterator extends ObjectIterator<Association, AssociationModel> {
  ...
  @Override Iterator<AssociationModel> fetchObjects() {
    return dms.storageDecorator.fetchAllAssociations();
  }

  @Override Association instantiateObject(AssociationModel model) {
    return dms.instantiateAssociation(model, true);
  }
}
