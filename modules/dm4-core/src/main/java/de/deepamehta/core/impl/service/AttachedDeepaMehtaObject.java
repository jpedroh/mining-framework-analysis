...
abstract class AttachedDeepaMehtaObject implements DeepaMehtaObject {
  ...
  @Override public AttachedRelatedTopic getRelatedTopic(String assocTypeUri, String myRoleTypeUri, String othersRoleTypeUri, String othersTopicTypeUri, boolean fetchComposite, boolean fetchRelatingComposite, ClientState clientState) {
    ResultSet<RelatedTopic> topics = getRelatedTopics(assocTypeUri, myRoleTypeUri, othersRoleTypeUri, othersTopicTypeUri, fetchComposite, fetchRelatingComposite, 0, clientState);
    switch (topics.getSize()) {
      ...
      default:
      throw new RuntimeException("Ambiguity: there are " + topics.getSize() + " related topics (object ID=" + getId() + ", assocTypeUri=\"" + assocTypeUri + "\", myRoleTypeUri=\"" + myRoleTypeUri + "\", " + "othersRoleTypeUri=\"" + othersRoleTypeUri + "\", othersTopicTypeUri=\"" + othersTopicTypeUri + "\")");
    }
  }
  ...
  @Override public ResultSet<RelatedTopic> getRelatedTopics(String assocTypeUri, String myRoleTypeUri, String othersRoleTypeUri, String othersTopicTypeUri, boolean fetchComposite, boolean fetchRelatingComposite, int maxResultSize, ClientState clientState) {
    ...
    return getRelatedTopics(assocTypeUris, myRoleTypeUri, othersRoleTypeUri, othersTopicTypeUri, fetchComposite, fetchRelatingComposite, maxResultSize, clientState);
  }
    ...
    try {
      ...
      for (AssociationDefinition assocDef : getType().getAssocDefs().values()) {
        ...
        if (cardinalityUri.equals("dm4.core.one")) {
          AttachedTopic childTopic = fetchChildTopic(assocDef, true);
          if (childTopic != null) {
  ...
  private void updateCompositeValue(CompositeValue newComp, ClientState clientState, Directives directives) {
    try {
      for (AssociationDefinition assocDef : getType().getAssocDefs().values()) {
        TopicModel valueTopic = newComp.getTopic(assocDef.getUri(), null);
        if (valueTopic == null) {
          continue;
        }
  ...
  @Override public void updateCompositeValue(AssociationDefinition assocDef, TopicModel valueTopic, ClientState clientState, Directives directives) {
    CompositeValue comp = getCompositeValue();
  ...
  private String buildLabel() {
    ...
    if (type.getDataTypeUri().equals("dm4.core.composite")) {
      ...
      for (String assocDefUri : type.getLabelConfig()) {
        AttachedDeepaMehtaObject childTopic = fetchChildTopic(assocDefUri, false);
        if (childTopic != null) {
  ...
  private String buildDefaultLabel() {
    ...
    if (type.getDataTypeUri().equals("dm4.core.composite")) {
      ...
      if (i.hasNext()) {
        ...
        AttachedDeepaMehtaObject childTopic = fetchChildTopic(assocDef, false);
        if (childTopic != null) {
  ...
  private void associateChildTopic(AssociationDefinition assocDef, long childTopicId) {
    dms.createAssociation(assocDef.getInstanceLevelAssocTypeUri(), getRoleModel(assocDef.getWholeRoleTypeUri()), new TopicRoleModel(childTopicId, assocDef.getPartRoleTypeUri()));
  }
