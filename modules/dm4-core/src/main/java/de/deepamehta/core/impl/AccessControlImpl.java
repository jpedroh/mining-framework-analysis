...
class AccessControlImpl implements AccessControl {
  ...
  @Override public boolean hasPermission(String username, Operation operation, long objectId) {
    String typeUri = null;
    try {
      ...
      throw new RuntimeException("Checking permission for object " + objectId + " failed (typeUri=\"" + typeUri + "\", " + userInfo(username) + ", operation=" + operation + ")", e);
    }
  }
  ...
  /**
     ...
     */
  @Override public boolean hasReadPermission(String username, long workspaceId) {
  ...
  /**
     ...
     * @param   workspaceId     the ID of the workspace that is relevant for the permission check. Is never -1.
     */
  @Override public boolean hasWritePermission(String username, long workspaceId) {
    SharingMode sharingMode = getSharingMode(workspaceId);
    switch (sharingMode) {
      case PRIVATE:
      return isOwner(username, workspaceId);
  ...
  @Override public Topic checkCredentials(Credentials cred) {
  ...
  @Override public void changePassword(Credentials cred) {
    try {
      ...
      userAccount.update(mf.newTopicModel(mf.newChildTopicsModel().put("dm4.accesscontrol.password", cred.password)));
    } catch (Exception e) {
      throw new RuntimeException("Changing password for user \"" + cred.username + "\" failed", e);
    }
  }

  @Override public Topic getUsernameTopic(String username) {
    TopicModelImpl usernameTopic = _getUsernameTopic(username);
    return usernameTopic != null ? usernameTopic.instantiate() : null;
  }

  @Override public Topic getPrivateWorkspace(String username) {
  ...
  @Override public boolean isMember(String username, long workspaceId) {
    try {
      ...
      AssociationModel membership = pl.fetchAssociation(TYPE_MEMBERSHIP, _getUsernameTopicOrThrow(username).getId(), workspaceId, "dm4.core.default", "dm4.core.default");
  ...
  @Override public String getCreator(long objectId) {
    return pl.hasProperty(objectId, PROP_CREATOR) ? (String) pl.fetchProperty(objectId, PROP_CREATOR) : null;
  }

  @Override public String getUsername(HttpServletRequest request) {
    try {
  ...
  @Override public Topic getUsernameTopic(HttpServletRequest request) {
  ...
  @Override public String username(HttpSession session) {
    return (String) session.getAttribute("username");
  }

  @Override public Topic getWorkspace(String uri) {
  ...
  @Override public long getDeepaMehtaWorkspaceId() {
    return getWorkspace(DEEPAMEHTA_WORKSPACE_URI).getId();
  }

  @Override public long getAdministrationWorkspaceId() {
    return getWorkspace(ADMINISTRATION_WORKSPACE_URI).getId();
  }

  @Override public long getSystemWorkspaceId() {
  ...
  @Override public long getAssignedWorkspaceId(long objectId) {
    try {
      long workspaceId = -1;
  ...
  @Override public void assignToWorkspace(DeepaMehtaObject object, long workspaceId) {
    try {
      pl.createAssociation("dm4.core.aggregation", object.getModel().createRoleModel("dm4.core.parent"), mf.newTopicRoleModel(workspaceId, "dm4.core.child"));
  ...
  @Override public boolean isWorkspaceAssignment(Association assoc) {
  ...
  @Override public <V extends java.lang.Object> V runWithoutWorkspaceAssignment(Callable<V> callable) throws Exception {
    return contextTracker.run(callable);
  }

  @Override public boolean workspaceAssignmentIsSuppressed() {
    return contextTracker.runsInTrackedContext();
  }

  @Override public void deleteAssociationMapcontext(Association assoc) {
    if (!assoc.getTypeUri().equals(ASSOCIATION_MAPCONTEXT)) {
      throw new RuntimeException("Association " + assoc.getId() + " not eligible for privileged deletion (" + assoc + ")");
    }
    ((AssociationImpl) assoc).getModel().delete();
  }

  @Override public RelatedTopic getConfigTopic(String configTypeUri, long topicId) {
    try {
      RelatedTopicModelImpl configTopic = pl.fetchTopicRelatedTopic(topicId, ASSOC_TYPE_CONFIGURATION, ROLE_TYPE_CONFIGURABLE, ROLE_TYPE_DEFAULT, configTypeUri);
      if (configTopic == null) {
  ...
  @Override public String getUsername(String emailAddress) {
    try {
  ...
  @Override public String getEmailAddress(String username) {
    try {
  ...
  @Override public boolean emailAddressExists(String emailAddress) {
  ...
  private boolean matches(TopicModel usernameTopic, String password) {
    String _password = getPasswordTopic(usernameTopic).getSimpleValue().toString();
  ...
  private TopicModelImpl _getUserAccount(TopicModel usernameTopic) {
    RelatedTopicModelImpl userAccount = pl.fetchTopicRelatedTopic(usernameTopic.getId(), "dm4.core.composition", "dm4.core.child", "dm4.core.parent", "dm4.accesscontrol.user_account");
    if (userAccount == null) {
  ...
  private TopicModel _getPasswordTopic(TopicModel userAccount) {
    RelatedTopicModel password = pl.fetchTopicRelatedTopic(userAccount.getId(), "dm4.core.composition", "dm4.core.parent", "dm4.core.child", "dm4.accesscontrol.password");
    if (password == null) {
  ...
  private boolean permissionIfNoWorkspaceIsAssigned(Operation operation, long objectId, String typeUri) {
    switch (operation) {
      case READ:
      logger.fine("Object " + objectId + " (typeUri=\"" + typeUri + "\") is not assigned to any workspace -- READ permission is granted");
      return true;
      case WRITE:
      logger.warning("Object " + objectId + " (typeUri=\"" + typeUri + "\") is not assigned to any workspace -- WRITE permission is refused");
      return false;
      default:
  ...
  private SharingMode getSharingMode(long workspaceId) {
    TopicModel sharingMode = pl.fetchTopicRelatedTopic(workspaceId, "dm4.core.aggregation", "dm4.core.parent", "dm4.core.child", "dm4.workspaces.sharing_mode");
    if (sharingMode == null) {
  ...
  private boolean isTopicmapPrivate(long topicmapId) {
    TopicModel privateFlag = pl.fetchTopicRelatedTopic(topicmapId, "dm4.core.composition", "dm4.core.parent", "dm4.core.child", "dm4.topicmaps.private");
    if (privateFlag == null) {
      return false;
    }
  ...
  private String _getUsername(String emailAddress) {
    ...
    for (TopicModelImpl emailAddressTopic : queryTopics(TYPE_EMAIL_ADDRESS, emailAddress)) {
      TopicModel usernameTopic = emailAddressTopic.getRelatedTopic(ASSOC_TYPE_USER_MAILBOX, "dm4.core.child", "dm4.core.parent", TYPE_USERNAME);
      if (usernameTopic != null) {
        if (username != null) {
  ...
  private String _getEmailAddress(String username) {
    TopicModel emailAddress = _getUsernameTopicOrThrow(username).getRelatedTopic(ASSOC_TYPE_USER_MAILBOX, "dm4.core.parent", "dm4.core.child", TYPE_EMAIL_ADDRESS);
