...
public class PythonClientExperimentalCodegen extends PythonClientCodegen {
    ...
    for (Map.Entry<String, Object> entry : objs.entrySet()) {
      ...
      for (Map<String, Object> mo : models) {
        ...
        if (cm.isEnum || cm.isAlias) {
          ...
          if (cm.isArrayModel && !modelProperty.isEnum && !modelProperty.hasValidation) {
            modelSchemasToRemove.put(cm.name, modelSchema);
          }
        }
      }
  ...
  /**
     ...
     */
  @Override public CodegenResponse fromResponse(String responseCode, ApiResponse response) {
  ...
  /**
     ...
     */
  @Override public void handleMethodResponse(Operation operation, Map<String, Schema> schemas, CodegenOperation op, ApiResponse methodResponse) {
  ...
  /**
     ...
     */
  @Override protected void handleMethodResponse(Operation operation, Map<String, Schema> schemas, CodegenOperation op, ApiResponse methodResponse, Map<String, String> importMappings) {
    ...
    if (responseSchema != null) {
      ...
      if (defaultResponse.isListContainer) {
        ...
        if (defaultResponse.isMapContainer) {
  ...
  @Override public void postProcessModelProperty(CodegenModel model, CodegenProperty p) {
    ...
    if (p.complexType != null && !languageSpecificPrimitives.contains(p.complexType)) {
      ...
      if (p.isListContainer && p.mostInnerItems.complexType != null && !languageSpecificPrimitives.contains(p.mostInnerItems.complexType)) {
