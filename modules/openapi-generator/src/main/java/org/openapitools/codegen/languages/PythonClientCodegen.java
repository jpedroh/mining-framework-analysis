...
public class PythonClientCodegen extends PythonLegacyClientCodegen {
  ...
  public PythonClientCodegen() {
    ...
    cliOptions.remove(4);
    cliOptions.add(new CliOption(CodegenConstants.PYTHON_ATTR_NONE_IF_UNSET, CodegenConstants.PYTHON_ATTR_NONE_IF_UNSET_DESC).defaultValue(Boolean.FALSE.toString()));
    cliOptions.add(new CliOption(CodegenConstants.INIT_REQUIRED_VARS, CodegenConstants.INIT_REQUIRED_VARS_DESC).defaultValue(Boolean.FALSE.toString()));
    CliOption disallowAdditionalPropertiesIfNotPresentOpt = CliOption.newBoolean(CodegenConstants.DISALLOW_ADDITIONAL_PROPERTIES_IF_NOT_PRESENT, CodegenConstants.DISALLOW_ADDITIONAL_PROPERTIES_IF_NOT_PRESENT_DESC).defaultValue(Boolean.FALSE.toString());
    ...
    disallowAdditionalPropertiesIfNotPresentOpts.put("true", "Keep the old (incorrect) behaviour that \'additionalProperties\' is set to false by default. NOTE: " + "this option breaks composition and will be removed in 6.0.0");
  ...
  @Override public void processOpts() {
    ...
    SupportingFile originalInitModel = supportingFiles.stream().filter((sf) -> sf.getTemplateFile().equals("__init__model.mustache")).reduce((a, b) -> {
    ...
    LOGGER.info("{} is hard coded to true in this generator. Alias models will only be generated if they contain validations or enums", CodegenConstants.GENERATE_ALIAS_AS_MODEL);
                ...
                if (ModelUtils.isObjectSchema(ref)) {
                  if (ref.getProperties() != null && !ref.getProperties().isEmpty()) {
                    ...
                    if (ModelUtils.hasValidation(ref)) {
                      ...
                      if (!getAllOfDescendants(simpleRef, openAPI).isEmpty()) {
                        return schema;
                      } else {
                        return unaliasSchema(allSchemas.get(ModelUtils.getSimpleRef(schema.get$ref())), usedImportMappings);
                      }
  ...
  @Override public String toDefaultValue(Schema p) {
    ...
    if (p.getDefault() != null) {
      ...
      if (p.getEnum() != null && p.getEnum().size() == 1) {
  ...
  @Override public void postProcessModelProperty(CodegenModel model, CodegenProperty p) {
  ...
  @Override public CodegenModel fromModel(String name, Schema sc) {
    ...
    if (sc.getDefault() == null && defaultValue == null) {
      ...
      if (sc.getDefault() != null) {
        ...
        if (defaultValue != null && cm.defaultValue == null) {
          cm.defaultValue = defaultValue;
          cm.hasRequired = false;
        }
      }
  ...
  @Override protected Schema getAdditionalProperties(Schema schema) {
