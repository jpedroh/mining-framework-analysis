...
public abstract class AbstractCSharpCodegen extends DefaultCodegen implements CodegenConfig {
  ...
  @Override protected void addParentFromContainer(CodegenModel model, Schema schema) {
  }
  ...
  @Override public void processOpts() {
    ...
    additionalProperties.put("lambdaCref", new Mustache.Lambda() {
      @Override public void execute(Template.Fragment fragment, Writer writer) throws IOException {
  ...
  @Override protected ImmutableMap.Builder<String, Lambda> addMustacheLambdas() {
  ...
  @Override public void postProcessModelProperty(CodegenModel model, CodegenProperty property) {
    ...
    if (property.isEnum && !property.vendorExtensions.containsKey(this.zeroBasedEnumVendorExtension)) {
      if (Boolean.TRUE.equals(this.zeroBasedEnums)) {
        ...
        if (!Boolean.FALSE.equals(this.zeroBasedEnums)) {
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public OperationsMap postProcessOperationsWithModels(OperationsMap objs, List<ModelMap> allModels) {
    super.postProcessOperationsWithModels(objs, allModels);
    Set<String> httpStatusesWithReturn = additionalProperties.get("x-http-statuses-with-return") instanceof Set<?> ? (Set<String>) additionalProperties.get("x-http-statuses-with-return") : new HashSet<String>();
  ...
  private CodegenModel getModelFromParameter(List<ModelMap> allModels, CodegenParameter parameter) {
    return parameter.isModel ? allModels.stream().map((m) -> m.getModel()).filter((m) -> m.getClassname().equals(parameter.dataType)).findFirst().orElse(null) : null;
  }
  ...
  public String escapeReservedWord(CodegenModel model, String name) {
    ...
    return name.equalsIgnoreCase(model.getClassname()) ? this.invalidNamePrefix + camelize(name) : name;
  }

  @Override public String escapeReservedWord(String name) {
    if (reservedWords().contains(name) || reservedWords().contains(name.toLowerCase(Locale.ROOT)) || reservedWords().contains(camelize(sanitizeName(name))) || isReservedWord(name) || name.matches("^\\d.*")) {
  ...
  @Override public String toEnumValue(String value, String datatype) {
    ...
    final String partiallyEscaped = value.replace("\n", "\\n").replace("\t", "\\t").replace("\r", "\\r").replaceAll("(?<!\\\\)\"", "\\\\\"");
  ...
  @Override public void setParameterExampleValue(CodegenParameter p) {
    ...
    if (example == null) {
      ...
      if (Boolean.TRUE.equals(p.isArray)) {
        ...
        if (Boolean.TRUE.equals(p.isModel)) {
  ...
  public void postProcessPattern(String pattern, Map<String, Object> vendorExtensions) {
    if (pattern != null) {
      ...
      int end = hasModifiers.matcher(pattern).find() ? pattern.lastIndexOf('/') : pattern.length() - 1;
