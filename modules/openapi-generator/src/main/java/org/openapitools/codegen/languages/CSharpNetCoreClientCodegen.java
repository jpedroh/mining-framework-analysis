...
@SuppressWarnings(value = { "Duplicates" }) public class CSharpNetCoreClientCodegen extends AbstractCSharpCodegen {
  ...
  @SuppressWarnings(value = { "hiding" }) private final Logger LOGGER = LoggerFactory.getLogger(CSharpClientCodegen.class);

  private static final List<FrameworkStrategy> frameworkStrategies = Arrays.asList(FrameworkStrategy.NETSTANDARD_1_3, FrameworkStrategy.NETSTANDARD_1_4, FrameworkStrategy.NETSTANDARD_1_5, FrameworkStrategy.NETSTANDARD_1_6, FrameworkStrategy.NETSTANDARD_2_0, FrameworkStrategy.NETSTANDARD_2_1, FrameworkStrategy.NETCOREAPP_3_1, FrameworkStrategy.NETFRAMEWORK_4_7, FrameworkStrategy.NETFRAMEWORK_4_8, FrameworkStrategy.NET_6_0, FrameworkStrategy.NET_7_0);
  ...
  public CSharpNetCoreClientCodegen() {
    super();
    modifyFeatureSet((features) -> features.includeDocumentationFeatures(DocumentationFeature.Readme).securityFeatures(EnumSet.of(SecurityFeature.OAuth2_Implicit, SecurityFeature.OAuth2_ClientCredentials, SecurityFeature.BasicAuth, SecurityFeature.ApiKey)).excludeGlobalFeatures(GlobalFeature.XMLStructureDefinitions, GlobalFeature.Callbacks, GlobalFeature.LinkObjects, GlobalFeature.ParameterStyling).includeSchemaSupportFeatures(SchemaSupportFeature.Polymorphism).includeParameterFeatures(ParameterFeature.Cookie).includeClientModificationFeatures(ClientModificationFeature.BasePath, ClientModificationFeature.UserAgent));
    ...
    addOption(DATE_FORMAT, "The default DateTime format.", this.dateFormat);
    addOption(DATETIME_FORMAT, "The default DateTime format.", this.dateTimeFormat);
    ...
    addSwitch(CodegenConstants.NULLABLE_REFERENCE_TYPES, CodegenConstants.NULLABLE_REFERENCE_TYPES_DESC + " Starting in .NET 6.0 the default is true.", this.nullReferenceTypesFlag);
    ...
    addSwitch(CodegenConstants.USE_DATETIME_OFFSET, CodegenConstants.USE_DATETIME_OFFSET_DESC, this.useDateTimeOffsetFlag);
    addSwitch(CodegenConstants.USE_COLLECTION, CodegenConstants.USE_COLLECTION_DESC, this.useCollection);
    addSwitch(CodegenConstants.RETURN_ICOLLECTION, CodegenConstants.RETURN_ICOLLECTION_DESC, this.returnICollection);
    addSwitch(CodegenConstants.OPTIONAL_METHOD_ARGUMENT, "C# Optional method argument, e.g. void square(int x=10) (.net 4.0+ only).", this.optionalMethodArgumentFlag);
    addSwitch(CodegenConstants.OPTIONAL_ASSEMBLY_INFO, CodegenConstants.OPTIONAL_ASSEMBLY_INFO_DESC, this.optionalAssemblyInfoFlag);
    addSwitch(CodegenConstants.OPTIONAL_EMIT_DEFAULT_VALUES, CodegenConstants.OPTIONAL_EMIT_DEFAULT_VALUES_DESC, this.optionalEmitDefaultValuesFlag);
    addSwitch(CodegenConstants.OPTIONAL_CONDITIONAL_SERIALIZATION, CodegenConstants.OPTIONAL_CONDITIONAL_SERIALIZATION_DESC, this.conditionalSerialization);
    addSwitch(CodegenConstants.OPTIONAL_PROJECT_FILE, CodegenConstants.OPTIONAL_PROJECT_FILE_DESC, this.optionalProjectFileFlag);
    ...
    addSwitch(CodegenConstants.NETCORE_PROJECT_FILE, CodegenConstants.NETCORE_PROJECT_FILE_DESC, this.netCoreProjectFileFlag);
    addSwitch(CodegenConstants.VALIDATABLE, CodegenConstants.VALIDATABLE_DESC, this.validatable);
    addSwitch(CodegenConstants.USE_ONEOF_DISCRIMINATOR_LOOKUP, CodegenConstants.USE_ONEOF_DISCRIMINATOR_LOOKUP_DESC, this.caseInsensitiveResponseHeaders);
    addSwitch(CodegenConstants.CASE_INSENSITIVE_RESPONSE_HEADERS, CodegenConstants.CASE_INSENSITIVE_RESPONSE_HEADERS_DESC, this.caseInsensitiveResponseHeaders);
    regexModifiers = new HashMap<>();
    regexModifiers.put('i', "IgnoreCase");
    regexModifiers.put('m', "Multiline");
    regexModifiers.put('s', "Singleline");
    regexModifiers.put('x', "IgnorePatternWhitespace");
    supportedLibraries.put(GENERICHOST, "HttpClient with Generic Host dependency injection (https://docs.microsoft.com/en-us/dotnet/core/extensions/generic-host) " + "(Experimental. Subject to breaking changes without notice.)");
    supportedLibraries.put(HTTPCLIENT, "HttpClient (https://docs.microsoft.com/en-us/dotnet/api/system.net.http.httpclient) " + "(Experimental. Subject to breaking changes without notice.)");
    supportedLibraries.put(UNITY_WEB_REQUEST, "UnityWebRequest (...) " + "(Experimental. Subject to breaking changes without notice.)");
  ...
  public static Comparator<CodegenProperty> propertyComparatorByNullable = new Comparator<CodegenProperty>() {
    @Override public int compare(CodegenProperty one, CodegenProperty another) {
      if (one.isNullable == another.isNullable) {
        ...
        if (Boolean.FALSE.equals(one.isNullable)) {
          return -1;
        } else {
          return 1;
        }
  ...
  public static Comparator<CodegenParameter> parameterComparatorByRequired = new Comparator<CodegenParameter>() {
    @Override public int compare(CodegenParameter one, CodegenParameter another) {
      if (one.required == another.required) {
        ...
        if (Boolean.TRUE.equals(one.required)) {
          return -1;
        } else {
          return 1;
        }
  ...
  @Override public String getNullableType(Schema p, String type) {
  ...
  @Override public Mustache.Compiler processCompiler(Mustache.Compiler compiler) {
  ...
  @Override public void processOpts() {
    ...
    if (!netStandard) {
      ...
      if (!additionalProperties.containsKey(CodegenConstants.NULLABLE_REFERENCE_TYPES) && !strategies.stream().anyMatch((s) -> s.equals(FrameworkStrategy.NETCOREAPP_3_1) || s.equals(FrameworkStrategy.NETFRAMEWORK_4_8) || s.equals(FrameworkStrategy.NETFRAMEWORK_4_7))) {
  ...
  public void addSupportingFiles(final String clientPackageDir, final String packageFolder, final AtomicReference<Boolean> excludeTests, final String testPackageFolder, final String testPackageName, final String modelPackageDir, final String authPackageDir) {
    ...
    supportingFiles.add(new SupportingFile("GlobalConfiguration.mustache", clientPackageDir, "GlobalConfiguration.cs"));
  ...
  public void addGenericHostSupportingFiles(final String clientPackageDir, final String packageDir, final AtomicReference<Boolean> excludeTests, final String testPackageDir, final String testPackageName, final String modelPackageDir) {
  ...
  @Override public void setPackageVersion(String packageVersion) {
  ...
  @Override public String toEnumVarName(String value, String datatype) {
    ...
    if (datatype.startsWith("int") || datatype.startsWith("uint") || datatype.startsWith("long") || datatype.startsWith("ulong") || datatype.startsWith("double") || datatype.startsWith("float")) {
  ...
  @SuppressWarnings(value = { "Duplicates" }) private static abstract class FrameworkStrategy {
    ...
    protected void configureAdditionalProperties(final Map<String, Object> properties) {
      ...
      if (properties.containsKey(SUPPORTS_UWP)) {
        LOGGER.warn(".NET {} generator does not support the UWP option. Use the csharp generator instead.", this.name);
  ...
  protected void configureAdditionalPropertiesForFrameworks(final Map<String, Object> properties, List<FrameworkStrategy> strategies) {
    properties.putIfAbsent(CodegenConstants.DOTNET_FRAMEWORK, strategies.stream().map((p) -> p.name).collect(Collectors.joining(";")));
  ...
  /**
     ...
     */
  @Override public String toInstantiationType(Schema schema) {
  ...
  private void fixInvalidPropertyName(CodegenModel model, CodegenProperty property) {
    if (property.name.equalsIgnoreCase(model.classname) || reservedWords().contains(property.name) || reservedWords().contains(camelize(sanitizeName(property.name), LOWERCASE_FIRST_LETTER))) {
      property.name = property.name + "Property";
    }
  }
  ...
  private void removePropertiesDeclaredInComposedClass(String className, List<CodegenModel> allModels, CodegenModel cm) {
    ...
    otherModel.readWriteVars.stream().filter((v) -> cm.readWriteVars.stream().anyMatch((cmV) -> cmV.baseName.equals(v.baseName))).collect(Collectors.toList()).forEach((v) -> {
  ...
  @Override protected void updateModelForObject(CodegenModel m, Schema schema) {
