...
public class DefaultCodegen implements CodegenConfig {
  ...
  static {
    ...
    sanitizedNameCache = Caffeine.newBuilder().maximumSize(cacheSize).expireAfterAccess(cacheExpiry, TimeUnit.SECONDS).ticker(Ticker.systemTicker()).build();
  ...
  @Override public List<CliOption> cliOptions() {
    return cliOptions;
  }
  ...
  @Override public void processOpts() {
    ...
    if (additionalProperties.containsKey(CodegenConstants.SORT_PARAMS_BY_REQUIRED_FLAG)) {
      this.setSortParamsByRequiredFlag(Boolean.valueOf(additionalProperties.get(CodegenConstants.SORT_PARAMS_BY_REQUIRED_FLAG).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.SORT_MODEL_PROPERTIES_BY_REQUIRED_FLAG)) {
      this.setSortModelPropertiesByRequiredFlag(Boolean.valueOf(additionalProperties.get(CodegenConstants.SORT_MODEL_PROPERTIES_BY_REQUIRED_FLAG).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.PREPEND_FORM_OR_BODY_PARAMETERS)) {
      this.setPrependFormOrBodyParameters(Boolean.valueOf(additionalProperties.get(CodegenConstants.PREPEND_FORM_OR_BODY_PARAMETERS).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.ENSURE_UNIQUE_PARAMS)) {
      this.setEnsureUniqueParams(Boolean.valueOf(additionalProperties.get(CodegenConstants.ENSURE_UNIQUE_PARAMS).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.ALLOW_UNICODE_IDENTIFIERS)) {
      this.setAllowUnicodeIdentifiers(Boolean.valueOf(additionalProperties.get(CodegenConstants.ALLOW_UNICODE_IDENTIFIERS).toString()));
    }
    ...
    if (additionalProperties.containsKey(CodegenConstants.REMOVE_OPERATION_ID_PREFIX)) {
      this.setRemoveOperationIdPrefix(Boolean.parseBoolean(additionalProperties.get(CodegenConstants.REMOVE_OPERATION_ID_PREFIX).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.REMOVE_OPERATION_ID_PREFIX_DELIMITER)) {
      this.setRemoveOperationIdPrefixDelimiter(additionalProperties.get(CodegenConstants.REMOVE_OPERATION_ID_PREFIX_DELIMITER).toString());
    }
    if (additionalProperties.containsKey(CodegenConstants.REMOVE_OPERATION_ID_PREFIX_COUNT)) {
      this.setRemoveOperationIdPrefixCount(Integer.parseInt(additionalProperties.get(CodegenConstants.REMOVE_OPERATION_ID_PREFIX_COUNT).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.SKIP_OPERATION_EXAMPLE)) {
      this.setSkipOperationExample(Boolean.parseBoolean(additionalProperties.get(CodegenConstants.SKIP_OPERATION_EXAMPLE).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.DOCEXTENSION)) {
      this.setDocExtension(String.valueOf(additionalProperties.get(CodegenConstants.DOCEXTENSION).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.ENABLE_POST_PROCESS_FILE)) {
      this.setEnablePostProcessFile(Boolean.parseBoolean(additionalProperties.get(CodegenConstants.ENABLE_POST_PROCESS_FILE).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.GENERATE_ALIAS_AS_MODEL)) {
      ModelUtils.setGenerateAliasAsModel(Boolean.parseBoolean(additionalProperties.get(CodegenConstants.GENERATE_ALIAS_AS_MODEL).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.REMOVE_ENUM_VALUE_PREFIX)) {
      this.setRemoveEnumValuePrefix(Boolean.parseBoolean(additionalProperties.get(CodegenConstants.REMOVE_ENUM_VALUE_PREFIX).toString()));
    }
    ...
    if (additionalProperties.containsKey(CodegenConstants.ENUM_UNKNOWN_DEFAULT_CASE)) {
      this.setEnumUnknownDefaultCase(Boolean.parseBoolean(additionalProperties.get(CodegenConstants.ENUM_UNKNOWN_DEFAULT_CASE).toString()));
    }
  }
  ...
  protected ImmutableMap.Builder<String, Lambda> addMustacheLambdas() {
    return new ImmutableMap.Builder<String, Mustache.Lambda>().put("lowercase", new LowercaseLambda().generator(this)).put("uppercase", new UppercaseLambda()).put("snakecase", new SnakecaseLambda()).put("titlecase", new TitlecaseLambda()).put("camelcase", new CamelCaseLambda(true).generator(this)).put("pascalcase", new CamelCaseLambda(false).generator(this)).put("indented", new IndentedLambda()).put("indented_8", new IndentedLambda(8, " ")).put("indented_12", new IndentedLambda(12, " ")).put("indented_16", new IndentedLambda(16, " "));
  }
  ...
  @Override @SuppressWarnings(value = { "static-method" }) public Map<String, ModelsMap> postProcessAllModels(Map<String, ModelsMap> objs) {
  ...
  private boolean codegenPropertyIsNew(CodegenModel model, CodegenProperty property) {
    return model.parentModel == null ? false : model.parentModel.allVars.stream().anyMatch((p) -> p.name.equals(property.name) && (p.dataType.equals(property.dataType) == false || p.datatypeWithEnum.equals(property.datatypeWithEnum) == false));
  }

  private boolean codegenPropertyIsInherited(CodegenModel model, CodegenProperty property) {
    return model.parentModel == null ? false : model.parentModel.allVars.stream().anyMatch((p) -> p.name.equals(property.name));
  }
  ...
  /**
     ...
     */
  @Override public Map<String, ModelsMap> updateAllModels(Map<String, ModelsMap> objs) {
  ...
  public void setCircularReferences(Map<String, CodegenModel> models) {
    final Map<String, List<CodegenProperty>> allVarsDependencyMap = models.entrySet().stream().collect(Collectors.toMap(Entry::getKey, (entry) -> getModelDependencies(entry.getValue().getAllVars())));
    models.keySet().forEach((name) -> setCircularReferencesOnProperties(name, allVarsDependencyMap));
    final Map<String, List<CodegenProperty>> varsDependencyMap = models.entrySet().stream().collect(Collectors.toMap(Entry::getKey, (entry) -> getModelDependencies(entry.getValue().getVars())));
    models.keySet().forEach((name) -> setCircularReferencesOnProperties(name, varsDependencyMap));
    final Map<String, List<CodegenProperty>> oneOfDependencyMap = models.entrySet().stream().collect(Collectors.toMap(Entry::getKey, (entry) -> getModelDependencies((entry.getValue().getComposedSchemas() != null && entry.getValue().getComposedSchemas().getOneOf() != null) ? entry.getValue().getComposedSchemas().getOneOf() : new ArrayList<CodegenProperty>())));
  ...
  private void setCircularReferencesOnProperties(final String root, final Map<String, List<CodegenProperty>> dependencyMap) {
    dependencyMap.getOrDefault(root, new ArrayList<>()).forEach((prop) -> {
      final List<String> unvisited = Collections.singletonList(prop.isContainer ? prop.items.dataType : prop.dataType);
      prop.isCircularReference = isCircularReference(root, new HashSet<>(), new ArrayList<>(unvisited), dependencyMap);
    });
  }

  private boolean isCircularReference(final String root, final Set<String> visited, final List<String> unvisited, final Map<String, List<CodegenProperty>> dependencyMap) {
    for (int i = 0; i < unvisited.size(); i++) {
      ...
      if (!visited.contains(next)) {
        ...
        dependencyMap.getOrDefault(next, new ArrayList<>()).forEach((prop) -> unvisited.add(prop.isContainer ? prop.items.dataType : prop.dataType));
        visited.add(next);
      }
    }
    return false;
  ...
  @Override @SuppressWarnings(value = { "static-method" }) public ModelsMap postProcessModels(ModelsMap objs) {
    return objs;
  }
  ...
  /**
     ...
     */
  @Override public void setOpenAPI(OpenAPI openAPI) {
  ...
  @Override @SuppressWarnings(value = { "static-method" }) public void postProcess() {
    System.out.println("################################################################################");
  ...
  @Override @SuppressWarnings(value = { "static-method" }) public OperationsMap postProcessOperationsWithModels(OperationsMap objs, List<ModelMap> allModels) {
    return objs;
  }

  @Override @SuppressWarnings(value = { "static-method" }) public Map<String, Object> postProcessSupportingFileData(Map<String, Object> objs) {
    return objs;
  }

  @Override @SuppressWarnings(value = { "unused" }) public void postProcessModelProperty(CodegenModel model, CodegenProperty property) {
  }

  @Override @SuppressWarnings(value = { "unused" }) public void postProcessParameter(CodegenParameter parameter) {
  }

  @Override @SuppressWarnings(value = { "unused" }) public void preprocessOpenAPI(OpenAPI openAPI) {
  ...
  @Override @SuppressWarnings(value = { "unused" }) public void processOpenAPI(OpenAPI openAPI) {
  }

  @Override @SuppressWarnings(value = { "unused" }) public Compiler processCompiler(Compiler compiler) {
    return compiler;
  }

  @Override @SuppressWarnings(value = { "unused" }) public TemplatingEngineAdapter processTemplatingEngine(TemplatingEngineAdapter templatingEngine) {
    return templatingEngine;
  }

  @Override @SuppressWarnings(value = { "static-method" }) public String escapeText(String input) {
    if (input == null) {
      return input;
    }
  ...
  /**
     ...
     */
  @Override public String escapeTextWhileAllowingNewLines(String input) {
    if (input == null) {
      return input;
    }
    return escapeUnsafeCharacters(StringEscapeUtils.unescapeJava(StringEscapeUtils.escapeJava(input).replace("\\/", "/")).replaceAll("[\\t]", " ").replace("\\", "\\\\").replace("\"", "\\\""));
  }

  @Override @SuppressWarnings(value = { "static-method" }) public String encodePath(String input) {
  ...
  @Override public Set<String> defaultIncludes() {
    return defaultIncludes;
  }

  @Override public Map<String, String> typeMapping() {
    return typeMapping;
  }

  @Override public Map<String, String> instantiationTypes() {
    return instantiationTypes;
  }

  @Override public Set<String> reservedWords() {
    return reservedWords;
  }

  @Override public Set<String> languageSpecificPrimitives() {
    return languageSpecificPrimitives;
  }

  @Override public Map<String, String> importMapping() {
    return importMapping;
  }

  @Override public Map<String, String> schemaMapping() {
    return schemaMapping;
  }

  @Override public Map<String, String> inlineSchemaNameMapping() {
    return inlineSchemaNameMapping;
  }

  @Override public Map<String, String> inlineSchemaNameDefault() {
    return inlineSchemaNameDefault;
  }

  @Override public Map<String, String> openapiNormalizer() {
    return openapiNormalizer;
  }

  @Override public String testPackage() {
    return testPackage;
  }

  @Override public String modelPackage() {
    return modelPackage;
  }

  @Override public String apiPackage() {
    return apiPackage;
  }

  @Override public String fileSuffix() {
    return fileSuffix;
  }

  @Override public String templateDir() {
    return templateDir;
  }

  @Override public String embeddedTemplateDir() {
  ...
  @Override public Map<String, String> apiDocTemplateFiles() {
    return apiDocTemplateFiles;
  }

  @Override public Map<String, String> modelDocTemplateFiles() {
    return modelDocTemplateFiles;
  }

  @Override public Map<String, String> reservedWordsMappings() {
    return reservedWordsMappings;
  }

  @Override public Map<String, String> apiTestTemplateFiles() {
    return apiTestTemplateFiles;
  }

  @Override public Map<String, String> modelTestTemplateFiles() {
    return modelTestTemplateFiles;
  }

  @Override public Map<String, String> apiTemplateFiles() {
    return apiTemplateFiles;
  }

  @Override public Map<String, String> modelTemplateFiles() {
  ...
  @Override public Map<String, Object> additionalProperties() {
    return additionalProperties;
  }

  @Override public Map<String, String> serverVariableOverrides() {
    return serverVariables;
  }

  @Override public Map<String, Object> vendorExtensions() {
    return vendorExtensions;
  }

  @Override public List<SupportingFile> supportingFiles() {
    return supportingFiles;
  }

  @Override public String outputFolder() {
    return outputFolder;
  }
  ...
  @Override public String getOutputDir() {
    return outputFolder();
  }

  @Override public String getInputSpec() {
    return inputSpec;
  }

  @Override public void setInputSpec(String inputSpec) {
    this.inputSpec = inputSpec;
  }

  @Override public String getFilesMetadataFilename() {
  ...
  @Override public String getVersionMetadataFilename() {
  ...
  /**
     ...
     */
  @Override public GeneratorMetadata getGeneratorMetadata() {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "static-method" }) public String toOperationId(String operationId) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "static-method" }) public String toEnumName(CodegenProperty property) {
  ...
  /**
     ...
     */
  @Override @SuppressWarnings(value = { "static-method" }) public String escapeReservedWord(String name) {
  ...
  /**
     ...
     * @return the fully-qualified "Model" name for import
     */
  @Override public String toModelImport(String name) {
  ...
  /**
     ...
     */
  @Override public Map<String, String> toModelImportMap(String name) {
  ...
  /**
     ...
     * @return the fully-qualified "Api" name for import
     */
  @Override public String toApiImport(String name) {
  ...
  public DefaultCodegen() {
    ...
    generatorMetadata = GeneratorMetadata.newBuilder().stability(Stability.STABLE).featureSet(DefaultFeatureSet).generationMessage(String.format(Locale.ROOT, "OpenAPI Generator: %s (%s)", getName(), codegenType.toValue())).build();
    defaultIncludes = new HashSet<>(Arrays.asList("double", "int", "long", "short", "char", "float", "String", "boolean", "Boolean", "Double", "Void", "Integer", "Long", "Float"));
    ...
    cliOptions.add(CliOption.newBoolean(CodegenConstants.SORT_PARAMS_BY_REQUIRED_FLAG, CodegenConstants.SORT_PARAMS_BY_REQUIRED_FLAG_DESC).defaultValue(Boolean.TRUE.toString()));
    cliOptions.add(CliOption.newBoolean(CodegenConstants.SORT_MODEL_PROPERTIES_BY_REQUIRED_FLAG, CodegenConstants.SORT_MODEL_PROPERTIES_BY_REQUIRED_FLAG_DESC).defaultValue(Boolean.TRUE.toString()));
    cliOptions.add(CliOption.newBoolean(CodegenConstants.ENSURE_UNIQUE_PARAMS, CodegenConstants.ENSURE_UNIQUE_PARAMS_DESC).defaultValue(Boolean.TRUE.toString()));
    cliOptions.add(CliOption.newBoolean(CodegenConstants.ALLOW_UNICODE_IDENTIFIERS, CodegenConstants.ALLOW_UNICODE_IDENTIFIERS_DESC).defaultValue(Boolean.FALSE.toString()));
    cliOptions.add(CliOption.newBoolean(CodegenConstants.PREPEND_FORM_OR_BODY_PARAMETERS, CodegenConstants.PREPEND_FORM_OR_BODY_PARAMETERS_DESC).defaultValue(Boolean.FALSE.toString()));
    ...
    CliOption disallowAdditionalPropertiesIfNotPresentOpt = CliOption.newBoolean(CodegenConstants.DISALLOW_ADDITIONAL_PROPERTIES_IF_NOT_PRESENT, CodegenConstants.DISALLOW_ADDITIONAL_PROPERTIES_IF_NOT_PRESENT_DESC).defaultValue(Boolean.TRUE.toString());
    ...
    CliOption enumUnknownDefaultCaseOpt = CliOption.newBoolean(CodegenConstants.ENUM_UNKNOWN_DEFAULT_CASE, CodegenConstants.ENUM_UNKNOWN_DEFAULT_CASE_DESC).defaultValue(Boolean.FALSE.toString());
  ...
  /**
     ...
     */
  @Override @SuppressWarnings(value = { "static-method" }) public String generateExamplePath(String path, Operation operation) {
        ...
        if (param instanceof QueryParameter) {
          ...
          if (qp.getStyle() != null) {
            ...
            if (Parameter.StyleEnum.FORM.equals(qp.getStyle())) {
              ...
              if (Parameter.StyleEnum.PIPEDELIMITED.equals(qp.getStyle())) {
                paramPart.append("|");
              } else {
                if (Parameter.StyleEnum.SPACEDELIMITED.equals(qp.getStyle())) {
                  paramPart.append("%20");
                } else {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "static-method" }) public String toDefaultValue(Schema schema) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "squid:S3923" }) private String getPropertyDefaultValue(Schema schema) {
  ...
  /**
     ...
     * @return string presentation of the default value of the property
     */
  @SuppressWarnings(value = { "static-method" }) public String toDefaultValueWithParam(String name, Schema schema) {
    return " = data." + name + ";";
  }
  ...
  /**
     ...
     **/
  @SuppressWarnings(value = { "static-method" }) public String getSchemaType(Schema schema) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "static-method" }) public String toAllOfName(List<String> names, ComposedSchema composedSchema) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "static-method" }) public String toAnyOfName(List<String> names, ComposedSchema composedSchema) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "static-method" }) public String toOneOfName(List<String> names, ComposedSchema composedSchema) {
    Map<String, Object> exts = composedSchema.getExtensions();
  ...
  @Override public Schema unaliasSchema(Schema schema) {
  ...
  private String getPrimitiveType(Schema schema) {
    if (schema == null) {
      ...
      if (typeMapping.containsKey(schema.getType() + "+" + schema.getFormat())) {
                                      ...
                                      if (isFreeFormObject(schema)) {
                                        ...
                                        if (schema.getProperties() != null && !schema.getProperties().isEmpty()) {
                                          ...
                                          if (ModelUtils.isAnyType(schema)) {
                                            ...
                                            if (StringUtils.isNotEmpty(schema.getType())) {
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "static-method" }) public String lowerCamelCase(String name) {
  ...
  /**
     ...
     */
  @Override @SuppressWarnings(value = { "static-method" }) public String getTypeDeclaration(String name) {
    return name;
  }
  ...
  /**
     ...
     */
  @Override public String getTypeDeclaration(Schema schema) {
    if (schema == null) {
  ...
  /**
     ...
     * @return getter name based on naming convention
     */
  @Override public String toGetter(String name) {
    return "get" + getterAndSetterCapitalize(name);
  }
  ...
  /**
     ...
     */
  @Override public String toSetter(String name) {
  ...
  private static class NamedSchema {
    ...
    @Override public boolean equals(Object o) {
      ...
      return Objects.equals(required, that.required) && Objects.equals(name, that.name) && Objects.equals(schema, that.schema) && Objects.equals(schemaIsFromAdditionalProperties, that.schemaIsFromAdditionalProperties);
    }
      ...
      for (Schema interfaceSchema : interfaces) {
        ...
        if (allDefinitions != null && refSchema != null) {
          if (allParents.contains(ref) && supportsMultipleInheritance) {
            ...
            if (parentName != null && parentName.equals(ref) && supportsInheritance) {
              addProperties(allProperties, allRequired, refSchema, new HashSet<>());
            } else {
  ...
  private HashMap<String, SchemaTestCase> extractSchemaTestCases(String refToTestCases) {
    ...
    for (Entry<String, LinkedHashMap<String, Object>> entry : testNameToTesCase.entrySet()) {
      ...
      SchemaTestCase testCase = new SchemaTestCase((String) testExample.getOrDefault("description", ""), new ObjectWithTypeBooleans(data), (boolean) testExample.get("valid"));
  ...
  /**
     ...
     */
  @Override public CodegenModel fromModel(String name, Schema schema) {
    Map<String, Schema> allDefinitions = ModelUtils.getSchemas(this.openAPI);
    CodegenModel m = CodegenModelFactory.newInstance(CodegenModelType.MODEL);
    if (schema.equals(trueSchema)) {
      m.setIsBooleanSchemaTrue(true);
    } else {
      if (schema.equals(falseSchema)) {
    ...
    if (ModelUtils.isComposedSchema(refSchema)) {
      ...
      if (composedSchema.getOneOf() != null && composedSchema.getOneOf().size() != 0) {
        ...
        for (Schema oneOf : composedSchema.getOneOf()) {
          ...
          if (thisCp == null) {
            LOGGER.warn("\'{}\' defines discriminator \'{}\', but the referenced OneOf schema \'{}\' is missing {}", composedSchemaName, discPropName, modelName, discPropName);
          }
          ...
          if (cp != thisCp) {
            LOGGER.warn("\'{}\' defines discriminator \'{}\', but the OneOf schema \'{}\' has a different {} definition than the prior OneOf schema\'s. Make sure the {} type and required values are the same", composedSchemaName, discPropName, modelName, discPropName, discPropName);
          }
        }
        return cp;
      }
      if (composedSchema.getAnyOf() != null && composedSchema.getAnyOf().size() != 0) {
        ...
        for (Schema anyOf : composedSchema.getAnyOf()) {
          ...
          if (thisCp == null) {
            LOGGER.warn("\'{}\' defines discriminator \'{}\', but the referenced AnyOf schema \'{}\' is missing {}", composedSchemaName, discPropName, modelName, discPropName);
          }
          if (cp != null && cp.dataType == null) {
            cp = thisCp;
            continue;
          ...
          if (cp != thisCp) {
            LOGGER.warn("\'{}\' defines discriminator \'{}\', but the AnyOf schema \'{}\' has a different {} definition than the prior AnyOf schema\'s. Make sure the {} type and required values are the same", composedSchemaName, discPropName, modelName, discPropName, discPropName);
          }
        }
        return cp;
      }
    ...
    if (ModelUtils.isComposedSchema(refSchema)) {
      ...
      if (composedSchema.getOneOf() != null && composedSchema.getOneOf().size() != 0) {
        ...
        if (discriminatorsPropNames.size() > 1) {
          LOGGER.warn("The oneOf schemas have conflicting discriminator property names. " + "oneOf schemas must have the same property name, but found " + String.join(", ", discriminatorsPropNames));
        }
      ...
      if (composedSchema.getAnyOf() != null && composedSchema.getAnyOf().size() != 0) {
        ...
        if (discriminatorsPropNames.size() > 1) {
          LOGGER.warn("The anyOf schemas have conflicting discriminator property names. " + "anyOf schemas must have the same property name, but found " + String.join(", ", discriminatorsPropNames));
        }
  ...
  protected List<MappedModel> getOneOfAnyOfDescendants(String composedSchemaName, String discPropName, ComposedSchema c, OpenAPI openAPI) {
    ...
    for (List<Schema> schemaList : listOLists) {
      ...
      for (Schema sc : schemaList) {
        ...
        if (ref == null) {
          LOGGER.warn("Invalid inline schema defined in oneOf/anyOf in \'{}\'. Per the OpenApi spec, for this case when a composed schema defines a discriminator, the oneOf/anyOf schemas must use $ref. Change this inline definition to a $ref definition", composedSchemaName);
        }
  ...
  protected CodegenDiscriminator createDiscriminator(String schemaName, Schema schema, OpenAPI openAPI) {
    ...
    if (schema.getProperties() != null && schema.getProperties().get(discPropName) instanceof StringSchema) {
            ...
            if (ModelUtils.isEmailSchema(p)) {
              ...
              if (ModelUtils.isDateSchema(p)) {
                ...
                if (ModelUtils.isDateTimeSchema(p)) {
                  ...
                  if (ModelUtils.isDecimalSchema(p)) {
  ...
  public CodegenProperty fromProperty(String name, Schema p, boolean required, boolean schemaIsFromAdditionalProperties) {
    ...
    if (ModelUtils.isIntegerSchema(p)) {
      ...
      if (ModelUtils.isBooleanSchema(p)) {
        ...
        if (ModelUtils.isFileSchema(p) && !ModelUtils.isStringSchema(p)) {
          property.isFile = true;
        } else {
    ...
    boolean isAnyTypeWithNothingElseSet = (ModelUtils.isAnyType(p) && (p.getProperties() == null || p.getProperties().isEmpty()) && !ModelUtils.isComposedSchema(p) && p.getAdditionalProperties() == null && p.getNot() == null && p.getEnum() == null);
  ...
  protected void handleMethodResponse(Operation operation, Map<String, Schema> schemas, CodegenOperation op, ApiResponse methodResponse, Map<String, String> schemaMappings) {
    ...
    if (responseSchema != null) {
      ...
      if (ModelUtils.isArraySchema(responseSchema)) {
        ...
        if (ModelUtils.isMapSchema(responseSchema)) {
      ...
      if (cm.isContainer) {
        ...
        if ("map".equals(cm.containerType)) {
          ...
          if ("list".equalsIgnoreCase(cm.containerType)) {
            op.isArray = true;
          } else {
            if ("array".equalsIgnoreCase(cm.containerType)) {
              op.isArray = true;
            } else {
              if ("set".equalsIgnoreCase(cm.containerType)) {
                op.isArray = true;
              }
            }
          }
  ...
  @Override public CodegenOperation fromOperation(String path, String httpMethod, Operation operation, List<Server> servers) {
    ...
    if (operation.getResponses() != null && !operation.getResponses().isEmpty()) {
      ...
      for (Map.Entry<String, ApiResponse> operationGetResponsesEntry : operation.getResponses().entrySet()) {
        ...
        if (!addSchemaImportsFromV3SpecLocations) {
          if (r.baseType != null && !defaultIncludes.contains(r.baseType) && !languageSpecificPrimitives.contains(r.baseType)) {
        ...
        if ((Boolean.TRUE.equals(r.is4xx) || Boolean.TRUE.equals(r.is5xx)) && Boolean.FALSE.equals(r.primitiveType) && Boolean.FALSE.equals(r.simpleType)) {
    ...
    if (parameters != null) {
      for (Parameter param : parameters) {
        ...
        if (param instanceof QueryParameter || "query".equalsIgnoreCase(param.getIn())) {
          ...
          if (param instanceof PathParameter || "path".equalsIgnoreCase(param.getIn())) {
            pathParams.add(p.copy());
          } else {
            if (param instanceof HeaderParameter || "header".equalsIgnoreCase(param.getIn())) {
              headerParams.add(p.copy());
            } else {
              if (param instanceof CookieParameter || "cookie".equalsIgnoreCase(param.getIn())) {
    ...
    if (sortParamsByRequiredFlag) {
      Collections.sort(allParams, new Comparator<CodegenParameter>() {
        @Override public int compare(CodegenParameter one, CodegenParameter another) {
          if (one.required == another.required) {
            return 0;
          } else {
            if (one.required) {
  ...
  public CodegenResponse fromResponse(String responseCode, ApiResponse response) {
    ...
    if (ModelUtils.isArraySchema(responseSchema)) {
      ...
      if (ModelUtils.isFileSchema(responseSchema) && !ModelUtils.isStringSchema(responseSchema)) {
        r.isFile = true;
      } else {
        if (ModelUtils.isStringSchema(responseSchema)) {
          if (ModelUtils.isEmailSchema(responseSchema)) {
            r.isEmail = true;
          } else {
            if (ModelUtils.isUUIDSchema(responseSchema)) {
              r.isUuid = true;
            } else {
              if (ModelUtils.isByteArraySchema(responseSchema)) {
                r.setIsString(false);
                r.isByteArray = true;
              } else {
                if (ModelUtils.isBinarySchema(responseSchema)) {
                  r.isFile = true;
                  r.isBinary = true;
                } else {
                  if (ModelUtils.isDateSchema(responseSchema)) {
                    r.setIsString(false);
                    r.isDate = true;
                  } else {
                    if (ModelUtils.isDateTimeSchema(responseSchema)) {
                      r.setIsString(false);
                      r.isDateTime = true;
                    } else {
                      if (ModelUtils.isDecimalSchema(responseSchema)) {
                        r.isDecimal = true;
                        r.setIsString(false);
                        r.isNumeric = true;
                      }
          ...
          if (ModelUtils.isIntegerSchema(responseSchema)) {
            ...
            if (ModelUtils.isNumberSchema(responseSchema)) {
              r.isNumeric = Boolean.TRUE;
              if (ModelUtils.isFloatSchema(responseSchema)) {
                r.isFloat = Boolean.TRUE;
              } else {
                if (ModelUtils.isDoubleSchema(responseSchema)) {
              ...
              if (ModelUtils.isTypeObjectSchema(responseSchema)) {
                ...
                if (ModelUtils.isAnyType(responseSchema)) {
                  addVarsRequiredVarsAdditionalProps(responseSchema, r);
                } else {
                  if (!ModelUtils.isBooleanSchema(responseSchema)) {
  ...
  public CodegenCallback fromCallback(String name, Callback callback, List<Server> servers) {
    ...
    callback.forEach((expression, pi) -> {
      ...
      Stream.of(Pair.of("get", pi.getGet()), Pair.of("head", pi.getHead()), Pair.of("put", pi.getPut()), Pair.of("post", pi.getPost()), Pair.of("delete", pi.getDelete()), Pair.of("patch", pi.getPatch()), Pair.of("options", pi.getOptions())).filter((p) -> p.getValue() != null).forEach((p) -> {
  ...
  protected void updateParameterForString(CodegenParameter codegenParameter, Schema parameterSchema) {
    if (ModelUtils.isEmailSchema(parameterSchema)) {
      ...
      if (ModelUtils.isUUIDSchema(parameterSchema)) {
        codegenParameter.isUuid = true;
      } else {
        if (ModelUtils.isByteArraySchema(parameterSchema)) {
          codegenParameter.setIsString(false);
          codegenParameter.isByteArray = true;
          codegenParameter.isPrimitiveType = true;
        } else {
          if (ModelUtils.isBinarySchema(parameterSchema)) {
            codegenParameter.isBinary = true;
            codegenParameter.isFile = true;
            codegenParameter.isPrimitiveType = true;
          } else {
            if (ModelUtils.isDateSchema(parameterSchema)) {
              codegenParameter.setIsString(false);
              codegenParameter.isDate = true;
              codegenParameter.isPrimitiveType = true;
            } else {
              if (ModelUtils.isDateTimeSchema(parameterSchema)) {
                codegenParameter.setIsString(false);
                codegenParameter.isDateTime = true;
                codegenParameter.isPrimitiveType = true;
              } else {
                if (ModelUtils.isDecimalSchema(parameterSchema)) {
                  codegenParameter.setIsString(false);
  ...
  public CodegenParameter fromParameter(Parameter parameter, Set<String> imports) {
    ...
    if (parameter instanceof QueryParameter || "query".equalsIgnoreCase(parameter.getIn())) {
      ...
      if (parameter instanceof PathParameter || "path".equalsIgnoreCase(parameter.getIn())) {
        ...
        if (parameter instanceof HeaderParameter || "header".equalsIgnoreCase(parameter.getIn())) {
          codegenParameter.isHeaderParam = true;
        } else {
          if (parameter instanceof CookieParameter || "cookie".equalsIgnoreCase(parameter.getIn())) {
    ...
    if (ModelUtils.isFileSchema(parameterSchema) && !ModelUtils.isStringSchema(parameterSchema)) {
      ...
      if (ModelUtils.isStringSchema(parameterSchema)) {
        ...
        if (ModelUtils.isBooleanSchema(parameterSchema)) {
          codegenParameter.isPrimitiveType = true;
        } else {
          if (ModelUtils.isNumberSchema(parameterSchema)) {
            codegenParameter.isPrimitiveType = true;
            if (ModelUtils.isFloatSchema(parameterSchema)) {
              codegenParameter.isFloat = true;
            } else {
              if (ModelUtils.isDoubleSchema(parameterSchema)) {
            ...
            if (ModelUtils.isIntegerSchema(parameterSchema)) {
              codegenParameter.isPrimitiveType = true;
              if (ModelUtils.isLongSchema(parameterSchema)) {
                codegenParameter.isLong = true;
              } else {
                codegenParameter.isInteger = true;
                if (ModelUtils.isShortSchema(parameterSchema)) {
              ...
              if (ModelUtils.isTypeObjectSchema(parameterSchema)) {
                if (ModelUtils.isMapSchema(parameterSchema)) {
                ...
                if (ModelUtils.isNullType(parameterSchema)) {
                  ;
                } else {
                  if (ModelUtils.isAnyType(parameterSchema)) {
                    if (ModelUtils.isMapSchema(parameterSchema)) {
                      updateParameterForMap(codegenParameter, parameterSchema, imports);
                    }
                    addVarsRequiredVarsAdditionalProps(parameterSchema, codegenParameter);
    ...
    if (codegenParameter.isQueryParam && codegenParameter.isDeepObject && loadDeepObjectIntoItems) {
      ...
      if (schema != null) {
        ...
        LOGGER.warn("No object schema found for deepObject parameter{} deepObject won\'t have specific properties", codegenParameter);
      }
    }
    finishUpdatingParameter(codegenParameter, parameter);
    return codegenParameter;
  ...
  /**
     ...
     */
  @Override @SuppressWarnings(value = { "static-method" }) public List<CodegenSecurity> fromSecurity(Map<String, SecurityScheme> securitySchemeMap) {
        ...
        if (SecurityScheme.Type.HTTP.equals(securityScheme.getType())) {
          ...
          if ("basic".equalsIgnoreCase(securityScheme.getScheme())) {
            ...
            if ("bearer".equalsIgnoreCase(securityScheme.getScheme())) {
              ...
              if ("signature".equalsIgnoreCase(securityScheme.getScheme())) {
    ...
    Collections.sort(codegenSecurities, new Comparator<CodegenSecurity>() {
      @Override public int compare(CodegenSecurity one, CodegenSecurity another) {
  ...
  protected boolean needToImport(String type) {
    return StringUtils.isNotBlank(type) && !defaultIncludes.contains(type) && !languageSpecificPrimitives.contains(type);
  }

  @SuppressWarnings(value = { "static-method" }) protected List<Map<String, Object>> toExamples(Map<String, Object> examples) {
    if (examples == null) {
      return null;
    }
  ...
  /**
     ...
     */
  @Override @SuppressWarnings(value = { "static-method" }) public void addOperationToGroup(String tag, String resourcePath, Operation operation, CodegenOperation co, Map<String, List<CodegenOperation>> operations) {
  ...
  protected void addVars(CodegenModel m, Map<String, Schema> properties, List<String> required, Map<String, Schema> allProperties, List<String> allRequired) {
    ...
    if (properties != null && !properties.isEmpty()) {
      m.hasVars = true;
      Set<String> mandatory = required == null ? Collections.emptySet() : new TreeSet<>(required);
    ...
    if (allProperties != null) {
      Set<String> allMandatory = allRequired == null ? Collections.emptySet() : new TreeSet<>(allRequired);
    ...
    for (Map.Entry<String, Schema> entry : properties.entrySet()) {
      ...
      if (prop == null) {
        ...
        if (cm != null && cm.allVars == vars && varsMap.keySet().contains(key)) {
          LOGGER.debug("The property `{}` already defined in the child model. Using the one from child.", key);
          cp = varsMap.get(key);
        } else {
  ...
  /**
     ...
     * @return camelized string
     */
  @SuppressWarnings(value = { "static-method" }) public String removeNonNameElementToCamelCase(String name) {
  ...
  @Override public String apiFilename(String templateName, String tag) {
    String suffix = apiTemplateFiles().get(templateName);
    return apiFileFolder() + File.separator + toApiFilename(tag) + suffix;
  }

  @Override public String modelFilename(String templateName, String modelName) {
  ...
  /**
     ...
     */
  @Override public String apiDocFilename(String templateName, String tag) {
  ...
  /**
     ...
     */
  @Override public String apiTestFilename(String templateName, String tag) {
    String suffix = apiTestTemplateFiles().get(templateName);
    return apiTestFileFolder() + File.separator + toApiTestFilename(tag) + suffix;
  }

  @Override public boolean shouldOverwrite(String filename) {
    return !(skipOverwrite && new File(filename).exists());
  }

  @Override public boolean isSkipOverwrite() {
    return skipOverwrite;
  }

  @Override public void setSkipOverwrite(boolean skipOverwrite) {
    this.skipOverwrite = skipOverwrite;
  }

  @Override public boolean isRemoveOperationIdPrefix() {
    return removeOperationIdPrefix;
  }

  @Override public boolean isSkipOperationExample() {
    return skipOperationExample;
  }

  @Override public void setRemoveOperationIdPrefix(boolean removeOperationIdPrefix) {
  ...
  @Override public void setSkipOperationExample(boolean skipOperationExample) {
    this.skipOperationExample = skipOperationExample;
  }

  @Override public boolean isHideGenerationTimestamp() {
    return hideGenerationTimestamp;
  }

  @Override public void setHideGenerationTimestamp(boolean hideGenerationTimestamp) {
  ...
  /**
     ...
     */
  @Override public Map<String, String> supportedLibraries() {
  ...
  /**
     ...
     */
  @Override public void setLibrary(String library) {
  ...
  /**
     ...
     */
  @Override public String getLibrary() {
    return library;
  }
  ...
  /**
     ...
     */
  @Override public void setGitHost(String gitHost) {
  ...
  /**
     ...
     * @return Git host
     */
  @Override public String getGitHost() {
    return gitHost;
  }
  ...
  /**
     ...
     */
  @Override public void setGitUserId(String gitUserId) {
  ...
  /**
     ...
     * @return Git user ID
     */
  @Override public String getGitUserId() {
    return gitUserId;
  }
  ...
  /**
     ...
     */
  @Override public void setGitRepoId(String gitRepoId) {
  ...
  /**
     ...
     * @return Git repo ID
     */
  @Override public String getGitRepoId() {
    return gitRepoId;
  }
  ...
  /**
     ...
     */
  @Override public void setReleaseNote(String releaseNote) {
  ...
  /**
     ...
     * @return Release note
     */
  @Override public String getReleaseNote() {
    return releaseNote;
  }
  ...
  /**
     ...
     */
  @Override public String getDocExtension() {
    return docExtension;
  }
  ...
  /**
     ...
     */
  @Override public void setDocExtension(String userDocExtension) {
  ...
  /**
     ...
     */
  @Override public void setHttpUserAgent(String httpUserAgent) {
  ...
  /**
     ...
     */
  @Override public String getHttpUserAgent() {
    return httpUserAgent;
  }

  @SuppressWarnings(value = { "static-method" }) protected CliOption buildLibraryCliOption(Map<String, String> supportedLibraries) {
  ...
  /**
     ...
     */
  @Override @SuppressWarnings(value = { "static-method" }) public String sanitizeName(String name) {
    return sanitizeName(name, "\\W");
  }

  @Override public void setTemplatingEngine(TemplatingEngineAdapter templatingEngine) {
    this.templatingEngine = templatingEngine;
  }

  @Override public TemplatingEngineAdapter getTemplatingEngine() {
  ...
  /**
     ...
     * @return sanitized string
     */
  @SuppressWarnings(value = { "static-method" }) public String sanitizeName(final String name, String removeCharRegEx, ArrayList<String> exceptionList) {
    if (name == null) {
    ...
    if (Boolean.TRUE.equals(property.isEmail) && Boolean.TRUE.equals(property.isString)) {
      ...
      if (Boolean.TRUE.equals(property.isUuid) && Boolean.TRUE.equals(property.isString)) {
        ...
        if (Boolean.TRUE.equals(property.isByteArray)) {
          ...
          if (Boolean.TRUE.equals(property.isBinary)) {
            ...
            if (Boolean.TRUE.equals(property.isString)) {
              ...
              if (Boolean.TRUE.equals(property.isBoolean)) {
                parameter.isBoolean = true;
                parameter.isPrimitiveType = true;
              } else {
                if (Boolean.TRUE.equals(property.isLong)) {
                  parameter.isLong = true;
                  parameter.isPrimitiveType = true;
                } else {
                  if (Boolean.TRUE.equals(property.isInteger)) {
                    parameter.isInteger = true;
                    parameter.isPrimitiveType = true;
                    if (Boolean.TRUE.equals(property.isShort)) {
                      parameter.isShort = true;
                    } else {
                      if (Boolean.TRUE.equals(property.isUnboundedInteger)) {
                        parameter.isUnboundedInteger = true;
                      }
                    }
                  } else {
                    if (Boolean.TRUE.equals(property.isDouble)) {
                      parameter.isDouble = true;
                      parameter.isPrimitiveType = true;
                    } else {
                      if (Boolean.TRUE.equals(property.isFloat)) {
                        parameter.isFloat = true;
                        parameter.isPrimitiveType = true;
                      } else {
                        if (Boolean.TRUE.equals(property.isDecimal)) {
                          parameter.isDecimal = true;
                          parameter.isPrimitiveType = true;
                        } else {
                          if (Boolean.TRUE.equals(property.isNumber)) {
                            parameter.isNumber = true;
                            parameter.isPrimitiveType = true;
                          } else {
                            if (Boolean.TRUE.equals(property.isDate)) {
                              parameter.isDate = true;
                              parameter.isPrimitiveType = true;
                            } else {
                              if (Boolean.TRUE.equals(property.isDateTime)) {
                                parameter.isDateTime = true;
                                parameter.isPrimitiveType = true;
                              } else {
                                if (Boolean.TRUE.equals(property.isFreeFormObject)) {
                                  parameter.isFreeFormObject = true;
                                } else {
                                  if (Boolean.TRUE.equals(property.isAnyType)) {
  ...
  public void updateCodegenPropertyEnum(CodegenProperty var) {
    ...
    Optional<Schema> referencedSchema = ModelUtils.getSchemas(openAPI).entrySet().stream().filter((entry) -> Objects.equals(varDataType, toModelName(entry.getKey()))).map(Map.Entry::getValue).findFirst();
  ...
  /**
     ...
     */
  @Override public String getIgnoreFilePathOverride() {
  ...
  /**
     ...
     */
  @Override public void setIgnoreFilePathOverride(final String ignoreFileOverride) {
  ...
  public boolean hasFormParameter(OpenAPI openAPI, Operation operation) {
    ...
    for (String consume : consumesInfo) {
      if (consume != null && (consume.toLowerCase(Locale.ROOT).startsWith("application/x-www-form-urlencoded") || consume.toLowerCase(Locale.ROOT).startsWith("multipart"))) {
        return true;
      }
    }
    return false;
    ...
    if (ModelUtils.isFileSchema(ps) && !ModelUtils.isStringSchema(ps)) {
      ...
      if (ModelUtils.isStringSchema(ps)) {
        if (ModelUtils.isEmailSchema(ps)) {
          ...
          if (ModelUtils.isUUIDSchema(ps)) {
            codegenParameter.isUuid = true;
          } else {
            if (ModelUtils.isByteArraySchema(ps)) {
              codegenParameter.setIsString(false);
              codegenParameter.isByteArray = true;
              codegenParameter.isPrimitiveType = true;
            } else {
              if (ModelUtils.isBinarySchema(ps)) {
                codegenParameter.isBinary = true;
                codegenParameter.isFile = true;
                codegenParameter.isPrimitiveType = true;
              } else {
                if (ModelUtils.isDateSchema(ps)) {
                  codegenParameter.setIsString(false);
                  codegenParameter.isDate = true;
                  codegenParameter.isPrimitiveType = true;
                } else {
                  if (ModelUtils.isDateTimeSchema(ps)) {
                    codegenParameter.setIsString(false);
                    codegenParameter.isDateTime = true;
                    codegenParameter.isPrimitiveType = true;
                  } else {
                    if (ModelUtils.isDecimalSchema(ps)) {
                      codegenParameter.setIsString(false);
                      codegenParameter.isDecimal = true;
                      codegenParameter.isPrimitiveType = true;
                    }
        ...
        if (ModelUtils.isBooleanSchema(ps)) {
          codegenParameter.isPrimitiveType = true;
        } else {
          if (ModelUtils.isNumberSchema(ps)) {
            codegenParameter.isPrimitiveType = true;
            if (ModelUtils.isFloatSchema(ps)) {
              codegenParameter.isFloat = true;
            } else {
              if (ModelUtils.isDoubleSchema(ps)) {
                codegenParameter.isDouble = true;
              }
            }
          } else {
            if (ModelUtils.isIntegerSchema(ps)) {
              codegenParameter.isPrimitiveType = true;
              if (ModelUtils.isLongSchema(ps)) {
                codegenParameter.isLong = true;
              } else {
                codegenParameter.isInteger = true;
                if (ModelUtils.isShortSchema(ps)) {
                  codegenParameter.isShort = true;
                } else {
                  ;
                }
              ...
              if (ModelUtils.isTypeObjectSchema(ps)) {
                if (ModelUtils.isFreeFormObject(openAPI, ps)) {
                  codegenParameter.isFreeFormObject = true;
                }
              } else {
                if (ModelUtils.isNullType(ps)) {
                  ;
                } else {
                  if (ModelUtils.isAnyType(ps)) {
                    ;
                  } else {
                    if (ModelUtils.isArraySchema(ps)) {
    ...
    for (Entry<String, MediaType> contentEntry : content.entrySet()) {
      ...
      if (mt.getEncoding() != null) {
        ...
        for (Entry<String, Encoding> encodingEntry : encMap.entrySet()) {
          ...
          CodegenEncoding ce = new CodegenEncoding(enc.getContentType(), headers, enc.getStyle().toString(), enc.getExplode() == null ? false : enc.getExplode().booleanValue(), enc.getAllowReserved() == null ? false : enc.getAllowReserved().booleanValue());
        ...
        if (schema.getAdditionalProperties() instanceof Boolean && Boolean.FALSE.equals(schema.getAdditionalProperties())) {
          ...
          if (supportsAdditionalPropertiesWithComposedSchema && !disallowAdditionalPropertiesIfNotPresent) {
            ...
            if (schema.getAdditionalProperties() == null) {
              ...
              if (schema.getAdditionalProperties() instanceof Boolean && Boolean.TRUE.equals(schema.getAdditionalProperties())) {
  ...
  protected void addVarsRequiredVarsAdditionalProps(Schema schema, IJsonSchemaValidationProperties property) {
    ...
    Set<String> mandatory = schema.getRequired() == null ? Collections.emptySet() : new TreeSet<>(schema.getRequired());
  ...
  @Override public List<CodegenServer> fromServers(List<Server> servers) {
    if (servers == null) {
      return Collections.emptyList();
    }
  ...
  @Override public List<CodegenServerVariable> fromServerVariables(Map<String, ServerVariable> variables) {
    if (variables == null) {
      return Collections.emptyList();
    }
  ...
  /**
     ...
     */
  @Override public boolean isEnablePostProcessFile() {
  ...
  /**
     ...
     */
  @Override public void setEnablePostProcessFile(boolean enablePostProcessFile) {
  ...
  /**
     ...
     */
  @Override public boolean isEnableMinimalUpdate() {
  ...
  /**
     ...
     */
  @Override public void setEnableMinimalUpdate(boolean enableMinimalUpdate) {
  ...
  /**
     ...
     */
  @Override public boolean isStrictSpecBehavior() {
  ...
  /**
     ...
     */
  @Override public void setStrictSpecBehavior(final boolean strictSpecBehavior) {
    this.strictSpecBehavior = strictSpecBehavior;
  }

  @Override public FeatureSet getFeatureSet() {
  ...
  /**
     ...
     */
  @Override public boolean isRemoveEnumValuePrefix() {
  ...
  /**
     ...
     */
  @Override public void setRemoveEnumValuePrefix(final boolean removeEnumValuePrefix) {
  ...
  protected void modifyFeatureSet(Consumer<FeatureSet.Builder> processor) {
    ...
    this.generatorMetadata = GeneratorMetadata.newBuilder(generatorMetadata).featureSet(builder.build()).build();
  }
  ...
  private static class SanitizeNameOptions {
    ...
    @Override public boolean equals(Object o) {
      ...
      return Objects.equals(getName(), that.getName()) && Objects.equals(getRemoveCharRegEx(), that.getRemoveCharRegEx()) && Objects.equals(getExceptions(), that.getExceptions());
    }
  ...
  public boolean isAnyTypeSchema(Schema schema) {
    ...
    if (schema.getClass().equals(Schema.class) && schema.get$ref() == null && schema.getType() == null && (schema.getProperties() == null || schema.getProperties().isEmpty()) && schema.getAdditionalProperties() == null && schema.getNot() == null && schema.getEnum() == null) {
      return true;
    }
    return false;
  }
  ...
  @Override public String defaultTemplatingEngine() {
    return "mustache";
  }
  ...
  @Override public String generatorLanguageVersion() {
    return null;
  }
  ...
  @Override public boolean getUseInlineModelResolver() {
    return true;
  }

  @Override public boolean getUseOpenAPINormalizer() {
    return true;
  }
