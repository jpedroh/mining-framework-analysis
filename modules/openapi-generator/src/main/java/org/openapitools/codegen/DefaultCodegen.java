...
public class DefaultCodegen implements CodegenConfig {
  ...
  public void processOpts() {
    ...
    if (additionalProperties.containsKey(CodegenConstants.ENABLE_POST_PROCESS_FILE)) {
      this.setEnablePostProcessFile(Boolean.valueOf(additionalProperties.get(CodegenConstants.ENABLE_POST_PROCESS_FILE).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.GENERATE_ALIAS_AS_MODEL)) {
      ModelUtils.setGenerateAliasAsModel(Boolean.valueOf(additionalProperties.get(CodegenConstants.GENERATE_ALIAS_AS_MODEL).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.REMOVE_ENUM_VALUE_PREFIX)) {
      this.setRemoveEnumValuePrefix(Boolean.valueOf(additionalProperties.get(CodegenConstants.REMOVE_ENUM_VALUE_PREFIX).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.LEGACY_DISCRIMINATOR_BEHAVIOR)) {
      this.setLegacyDiscriminatorBehavior(Boolean.valueOf(additionalProperties.get(CodegenConstants.LEGACY_DISCRIMINATOR_BEHAVIOR).toString()));
    }
    if (additionalProperties.containsKey(CodegenConstants.DISALLOW_ADDITIONAL_PROPERTIES_IF_NOT_PRESENT)) {
      this.setDisallowAdditionalPropertiesIfNotPresent(Boolean.valueOf(additionalProperties.get(CodegenConstants.DISALLOW_ADDITIONAL_PROPERTIES_IF_NOT_PRESENT).toString()));
    }
  }
  ...
  private void setCircularReferencesOnProperties(final String root, final Map<String, List<CodegenProperty>> dependencyMap) {
    dependencyMap.getOrDefault(root, new ArrayList<>()).stream().forEach((prop) -> {
  ...
  public DefaultCodegen() {
    ...
    disallowAdditionalPropertiesIfNotPresentOpts.put("true", "when the \'additionalProperties\' keyword is not present in a schema, " + "the value of \'additionalProperties\' is automatically set to false, i.e. no additional properties are allowed. " + "Note: this mode is not compliant with the JSON schema specification. " + "This is the original openapi-generator behavior.");
    ...
    if (ModelUtils.isComposedSchema(refSchema)) {
      ...
      if (composedSchema.getOneOf() != null && composedSchema.getOneOf().size() != 0) {
        ...
        if (discriminatorsPropNames.size() > 1) {
          throw new RuntimeException("The oneOf schemas have conflicting discriminator property names. " + "oneOf schemas must have the same property name, but found " + String.join(", ", discriminatorsPropNames));
        }
      ...
      if (composedSchema.getAnyOf() != null && composedSchema.getAnyOf().size() != 0) {
        ...
        if (discriminatorsPropNames.size() > 1) {
          throw new RuntimeException("The anyOf schemas have conflicting discriminator property names. " + "anyOf schemas must have the same property name, but found " + String.join(", ", discriminatorsPropNames));
        }
                  ...
                  if (isAnyTypeSchema(p)) {
                    ...
                    if (ModelUtils.isArraySchema(p)) {
                      ...
                      if (ModelUtils.isMapSchema(p)) {
                        Schema innerSchema = ModelUtils.unaliasSchema(this.openAPI, getAdditionalProperties(p), importMapping);
                        if (innerSchema == null) {
  ...
  public CodegenResponse fromResponse(String responseCode, ApiResponse response) {
    ...
    if (r.schema != null) {
      ...
      if (cp.isContainer) {
        ...
        r.isListContainer = "list".equalsIgnoreCase(cp.containerType) || "array".equalsIgnoreCase(cp.containerType) || "set".equalsIgnoreCase(cp.containerType);
      } else {
        r.simpleType = true;
      }
