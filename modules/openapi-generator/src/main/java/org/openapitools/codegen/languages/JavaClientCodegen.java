...
public class JavaClientCodegen extends AbstractJavaCodegen implements BeanValidationFeatures, PerformBeanValidationFeatures, GzipFeatures {
  ...
  @Override public void processOpts() {
    ...
    if (!additionalProperties.containsKey(MICROPROFILE_REST_CLIENT_VERSION)) {
      ...
      if (!mpRestClientVersions.containsKey(mpRestClientVersion)) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Version %s of MicroProfile Rest Client is not supported or incorrect. Supported versions are %s", mpRestClientVersion, String.join(", ", mpRestClientVersions.keySet())));
      }
    }
  ...
  @SuppressWarnings(value = { "unchecked" }) @Override public ModelsMap postProcessModels(ModelsMap objs) {
      ...
      for (ModelMap mo : models) {
        ...
        for (CodegenProperty var : cm.vars) {
          if (this.openApiNullable) {
            ...
            findByName(var.name, cm.readOnlyVars).ifPresent((p) -> p.getVendorExtensions().put("x-is-jackson-optional-nullable", isOptionalNullable));
          }
