...
public class ModelUtils {
  ...
  private static Schema getSchemaFromContent(Content content, String reqType) {
    ...
    for (String type : content.keySet()) {
      if (isTypeFormParam(type) && "form".equalsIgnoreCase(reqType)) {
        ...
        if (!isTypeFormParam(type) && "body".equalsIgnoreCase(reqType)) {
          MediaType media = content.get(type);
          return media.getSchema();
        }
      }
  ...
  public static Map<String, List<String>> getChildrenMap(OpenAPI openAPI) {
    Map<String, Schema> allSchemas = getSchemas(openAPI);
    Map<String, List<Entry<String, Schema>>> groupedByParent = allSchemas.entrySet().stream().filter((entry) -> isComposedSchema(entry.getValue())).collect(Collectors.groupingBy((entry) -> getParentName((ComposedSchema) entry.getValue(), allSchemas)));
  ...
  public static List<String> getAllParentsName(ComposedSchema composedSchema, Map<String, Schema> allSchemas, boolean includeAncestors) {
    ...
    if (names.size() == 0 && refedWithoutDiscriminator.size() == 1) {
      LOGGER.warn("[deprecated] inheritance without use of \'discriminator.propertyName\' is deprecated " + "and will be removed in a future release. Generating model for {}. Title: {}", composedSchema.getName(), composedSchema.getTitle());
  ...
  public static boolean isTypeFormParam(String contentType) {
    if ("application/x-www-form-urlencoded".equalsIgnoreCase(contentType) || "multipart/form-data".equalsIgnoreCase(contentType) || "multipart/related".equalsIgnoreCase(contentType)) {
      return true;
    } else {
      return false;
    }
