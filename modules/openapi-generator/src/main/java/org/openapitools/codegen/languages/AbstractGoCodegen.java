...
public abstract class AbstractGoCodegen extends DefaultCodegen implements CodegenConfig {
  ...
  public AbstractGoCodegen() {
    ...
    languageSpecificPrimitives = new HashSet<String>(Arrays.asList("string", "bool", "uint", "uint32", "uint64", "int", "int32", "int64", "float32", "float64", "complex64", "complex128", "rune", "byte", "map[string]interface{}", "interface{}"));
  ...
  protected boolean isReservedFilename(String name) {
    ...
    Set<String> reservedSuffixes = new HashSet<String>(Arrays.asList("test", "aix", "android", "darwin", "dragonfly", "freebsd", "illumos", "js", "linux", "netbsd", "openbsd", "plan9", "solaris", "windows", "386", "amd64", "arm", "arm64", "mips", "mips64", "mips64le", "mipsle", "ppc64", "ppc64le", "s390x", "wasm"));
  ...
  @Override public String toModelFilename(String name) {
    ...
    if (isReservedFilename(name)) {
      LOGGER.warn("{}.go with suffix (reserved word) cannot be used as filename. Renamed to {}_.go", name, name);
      name += "_";
    }
    return name;
  }
  ...
  public String toModel(String name, boolean doUnderscore) {
    ...
    if (name.matches("^\\d.*")) {
      LOGGER.warn("{} (model name starts with number) cannot be used as model name. Renamed to {}", name, "model_" + name);
      name = "model_" + name;
    }
    if (doUnderscore) {
  ...
  @Override public String toApiFilename(String name) {
    ...
    if (isReservedFilename(api)) {
      LOGGER.warn("{}.go with suffix (reserved word) cannot be used as filename. Renamed to {}_.go", name, api);
      api += "_";
    }
    apiName = api;
    return apiName;
  ...
  @Override public String getSchemaType(Schema p) {
    ...
    if (ref != null && !ref.isEmpty()) {
      ...
      if ("object".equals(openAPIType) && isAnyTypeSchema(p)) {
        type = "interface{}";
      } else {
        if (typeMapping.containsKey(openAPIType)) {
