...
public abstract class AbstractGoCodegen extends DefaultCodegen implements CodegenConfig {
  ...
  public AbstractGoCodegen() {
    ...
    languageSpecificPrimitives = new HashSet<String>(Arrays.asList("string", "bool", "uint", "uint32", "uint64", "int", "int32", "int64", "float32", "float64", "complex64", "complex128", "rune", "byte", "ObjectType", "AnyType"));
  ...
  @Override public String getSchemaType(Schema p) {
    ...
    if (ref != null && !ref.isEmpty()) {
      ...
      if ("object".equals(openAPIType) && ModelUtils.isAnyTypeSchema(p)) {
        type = "AnyType";
      } else {
        if (typeMapping.containsKey(openAPIType)) {
      ...
      for (CodegenParameter param : operation.allParams) {
        ...
        if (!param.required && usesOptionals) {
          ...
          if ("time.Time".equals(param.dataType)) {
            ...
            param.vendorExtensions.put("x-optionalDataType", optionalType);
