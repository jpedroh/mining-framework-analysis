...
@SuppressWarnings(value = { "rawtypes" }) public class DefaultGenerator extends AbstractGenerator implements Generator {
  ...
  @Override public boolean getEnableMinimalUpdate() {
...
>>>>>>> Unknown file: This is a bug in JDime.
   void configureGeneratorProperties() {
    ...
    contextPath = config.escapeText(url.getPath()).replaceAll("/$", "");
    ...
    for (String templateName : config.modelTestTemplateFiles().keySet()) {
      ...
      if (generateModelTests) {
        if (new File(filename).exists()) {
          ...
          if (dryRun) {
            dryRunStatusMap.put(filename, new DryRunStatus(java.nio.file.Paths.get(filename), DryRunStatus.State.SkippedOverwrite, "Test files never overwrite an existing file of the same name."));
          }
          continue;
        }
        ...
        if (dryRun) {
          dryRunStatusMap.put(filename, new DryRunStatus(java.nio.file.Paths.get(filename), DryRunStatus.State.Skipped, "Skipped by modelTests option supplied by user."));
        }
      }
    }
  }
  ...
  private void generateModelDocumentation(List<File> files, Map<String, Object> models, String modelName) throws IOException {
    for (String templateName : config.modelDocTemplateFiles().keySet()) {
      ...
      if (generateModelDocumentation) {
        ...
        if (dryRun) {
          dryRunStatusMap.put(filename, new DryRunStatus(java.nio.file.Paths.get(filename), DryRunStatus.State.Skipped, "Skipped by modelDocs option supplied by user."));
        }
      }
    }
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) protected void generateModels(List<File> files, List<Object> allModels, List<String> unusedModels) {
    if (!generateModels) {
    ...
    Boolean skipFormModel = GlobalSettings.getProperty(CodegenConstants.SKIP_FORM_MODEL) != null ? Boolean.valueOf(GlobalSettings.getProperty(CodegenConstants.SKIP_FORM_MODEL)) : getGeneratorPropertyDefaultSwitch(CodegenConstants.SKIP_FORM_MODEL, false);
        ...
        if (config.importMapping().containsKey(name)) {
          ...
          if (dryRun) {
            for (String templateName : config.modelTemplateFiles().keySet()) {
              ...
              dryRunStatusMap.put(filename, new DryRunStatus(java.nio.file.Paths.get(filename), DryRunStatus.State.Skipped, "Skipped prior to model processing due to import mapping conflict (either by user or by generator)."));
            }
          }
          continue;
        }
  ...
  @SuppressWarnings(value = { "unchecked" }) private void generateApis(List<File> files, List<Object> allOperations, List<Object> allModels) {
    if (!generateApis) {
      ...
      try {
        ...
        for (String templateName : config.apiTestTemplateFiles().keySet()) {
          ...
          if (generateApiTests) {
            ...
            if (dryRun) {
              dryRunStatusMap.put(filename, new DryRunStatus(apiTestFile.toPath(), DryRunStatus.State.Skipped, "Skipped by apiTests option supplied by user."));
            }
          }
        }
        for (String templateName : config.apiDocTemplateFiles().keySet()) {
          ...
          if (generateApiDocumentation) {
            ...
            if (dryRun) {
              dryRunStatusMap.put(filename, new DryRunStatus(apiDocFile.toPath(), DryRunStatus.State.Skipped, "Skipped by apiDocs option supplied by user."));
            }
          }
        }
      } catch (Exception e) {
  ...
  private void generateSupportingFiles(List<File> files, Map<String, Object> bundle) {
    ...
    for (SupportingFile support : config.supportingFiles()) {
      try {
        ...
        String outputFilename = new File(support.destinationFilename).isAbsolute() ? support.destinationFilename : outputFolder + File.separator + support.destinationFilename.replace('/', File.separatorChar);
        if (!config.shouldOverwrite(outputFilename)) {
          LOGGER.info("Skipped overwriting {}", outputFilename);
          if (dryRun) {
            Path skippedSupportingFile = java.nio.file.Paths.get(outputFilename);
            DryRunStatus status = new DryRunStatus(skippedSupportingFile, DryRunStatus.State.SkippedOverwrite);
          }
          continue;
        }
        String templateFile;
        ...
        if (!shouldGenerate) {
          if (dryRun) {
            Path skippedSupportingFile = java.nio.file.Paths.get(outputFilename);
            DryRunStatus status = new DryRunStatus(skippedSupportingFile, DryRunStatus.State.Skipped, "Skipped by supportingFiles option supplied by user.");
    ...
    if (generateMetadata && !ignoreFile.exists()) {
      ...
      if (generateMetadata && dryRun && ignoreFile.exists()) {
        ...
        if (!generateMetadata && dryRun) {
          dryRunStatusMap.put(ignoreFileNameTarget, new DryRunStatus(ignoreFile.toPath(), DryRunStatus.State.Skipped, "Skipped by generateMetadata option supplied by user"));
        }
      }
    }
  ...
  @SuppressWarnings(value = { "unchecked" }) private Map<String, Object> buildSupportFileBundle(List<Object> allOperations, List<Object> allModels) {
  ...
  @Override public String getFullTemplateContents(String templateName) {
  ...
  /**
     ...
     */
  @Override public Path getFullTemplatePath(String name) {
  ...
  @SuppressWarnings(value = { "unchecked" }) private Map<String, Object> processOperations(CodegenConfig config, String tag, List<CodegenOperation> ops, List<Object> allModels) {
    ...
    for (String nextImport : allImports) {
      ...
      if (mapping != null && !mappingSet.contains(mapping)) {
  ...
  /**
     ...
     */
  @Override public File writeToFile(String filename, byte[] contents) throws IOException {
    if (dryRun) {
      Path path = java.nio.file.Paths.get(filename);
