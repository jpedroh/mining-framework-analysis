...
public class SpringCodegen extends AbstractJavaCodegen implements BeanValidationFeatures, PerformBeanValidationFeatures, OptionalFeatures, SwaggerUIFeatures {
  ...
  public SpringCodegen() {
    ...
    cliOptions.add(CliOption.newBoolean(USE_SPRING_BOOT3, "Generate code and provide dependencies for use with Spring Boot 3.x. (Use jakarta instead of javax in imports).", useSwaggerUI));
  ...
  @Override public void processOpts() {
    ...
    if (!interfaceOnly) {
      ...
      if (SPRING_CLOUD_LIBRARY.equals(library)) {
        ...
        if (!reactive && !apiFirst) {
          if (DocumentationProvider.SPRINGDOC.equals(getDocumentationProvider())) {
            supportingFiles.add(new SupportingFile("springdocDocumentationConfig.mustache", (sourceFolder + File.separator + configPackage).replace(".", java.io.File.separator), "SpringDocConfiguration.java"));
          } else {
            if (DocumentationProvider.SPRINGFOX.equals(getDocumentationProvider())) {
      ...
      if (this.openapiDocketConfig && !SPRING_CLOUD_LIBRARY.equals(library) && !this.reactive && !this.apiFirst) {
  ...
  private void doDataTypeAssignment(String returnType, DataTypeAssigner dataTypeAssigner) {
    ...
    if (rt == null) {
      ...
      if (rt.startsWith("List") || rt.startsWith("java.util.List")) {
        ...
        if (rt.startsWith("Map") || rt.startsWith("java.util.Map")) {
          final int start = rt.indexOf("<");
          final int end = rt.lastIndexOf(">");
          if (start > 0 && end > 0) {
          ...
          if (rt.startsWith("Set") || rt.startsWith("java.util.Set")) {
            final int start = rt.indexOf("<");
            final int end = rt.lastIndexOf(">");
            if (start > 0 && end > 0) {
              dataTypeAssigner.setReturnType(rt.substring(start + 1, end).trim());
  ...
  public @Override void setUseBeanValidation(boolean useBeanValidation) {
    this.useBeanValidation = useBeanValidation;
  }

  public @Override void setPerformBeanValidation(boolean performBeanValidation) {
  ...
  @Override public void setUseSwaggerUI(boolean useSwaggerUI) {
