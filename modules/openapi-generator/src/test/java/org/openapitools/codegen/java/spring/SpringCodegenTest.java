...
public class SpringCodegenTest {
  ...
  @Test public void useBeanValidationTruePerformBeanValidationFalseJava8TrueJakartaeeTrueForFormatEmail() throws IOException {
  ...
  private void beanValidationForFormatEmail(boolean useBeanValidation, boolean performBeanValidation, boolean java8, boolean useJakarta, String contains, String notContains) throws IOException {
    ...
    JavaFileAssert javaFileAssert = JavaFileAssert.assertThat(files.get("PersonWithEmail.java")).printFileContent();
    if (useBeanValidation) {
      javaFileAssert.hasImports((useJakarta ? "jakarta" : "javax") + ".validation.constraints");
    }
    if (performBeanValidation) {
      javaFileAssert.hasImports("org.hibernate.validator.constraints");
    }
  ...
  @Test public void nonRequiredFieldShouldNotIncludeNotNullAnnotation_issue13365() throws IOException {
    ...
    javaFileAssert.isNot(new Condition<>((classfile) -> classfile.getImports().stream().map(NodeWithName::getNameAsString).anyMatch("javax.validation.constraints.NotNull"::equals), ""));
  }
  ...
  @Test public void requiredFieldShouldIncludeNotNullAnnotationWithBeanValidationTrue_issue13365() throws IOException {
    ...
    Map<String, File> files = generateFiles(codegen, "src/test/resources/bugs/issue_13365.yml");
    JavaFileAssert javaFileAssert = JavaFileAssert.assertThat(files.get("Person.java")).printFileContent();
    javaFileAssert.assertMethod("getName").assertMethodAnnotations().containsWithName("NotNull").containsWithName("Size").
