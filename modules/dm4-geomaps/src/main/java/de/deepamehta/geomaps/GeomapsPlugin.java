...
@Path(value = "/geomap") @Consumes(value = "application/json") @Produces(value = "application/json") public class GeomapsPlugin extends PluginActivator implements GeomapsService, PostCreateTopicListener, PostUpdateTopicListener, PreSendTopicListener {
  ...
  @GET @Path(value = "/topic/{id}") @Override public Topic getDomainTopic(@PathParam(value = "id") long geoCoordId) {
    try {
  ...
  @Override public GeoCoordinate getGeoCoordinate(Topic geoTopic) {
    try {
  ...
  @Override public GeoCoordinate geoCoordinate(Topic geoCoordTopic) {
  ...
  @PUT @Path(value = "/{id}/topic/{geo_coord_id}") @Override public void addCoordinateToGeomap(@PathParam(value = "id") long geomapId, @PathParam(value = "geo_coord_id") long geoCoordId) {
    logger.info("### Adding geo coordinate topic " + geoCoordId + " to geomap " + geomapId);
    AssociationModel model = mf.newAssociationModel("dm4.geomaps.geotopic_mapcontext", mf.newTopicRoleModel(geomapId, "dm4.core.default"), mf.newTopicRoleModel(geoCoordId, "dm4.topicmaps.topicmap_topic"));
  ...
  @PUT @Path(value = "/{id}/center/{lon}/{lat}/zoom/{zoom}") @Override public void setGeomapState(@PathParam(value = "id") long geomapId, @PathParam(value = "lon") double lon, @PathParam(value = "lat") double lat, @PathParam(value = "zoom") int zoom) {
    ChildTopicsModel geomapState = mf.newChildTopicsModel().put("dm4.topicmaps.state", mf.newChildTopicsModel().put("dm4.topicmaps.translation", mf.newChildTopicsModel().put("dm4.topicmaps.translation_x", lon).put("dm4.topicmaps.translation_y", lat)).put("dm4.topicmaps.zoom_level", zoom));
    dm4.updateTopic(mf.newTopicModel(geomapId, geomapState));
  }

  @GET @Path(value = "/distance") @Override public double getDistance(@QueryParam(value = "coord1") GeoCoordinate coord1, @QueryParam(value = "coord2") GeoCoordinate coord2) {
  ...
  @Override public <V extends java.lang.Object> V runWithoutGeocoding(Callable<V> callable) throws Exception {
  ...
  /**
     ...
     */
  @Override public void preSendTopic(Topic topic) {
  ...
  private void storeGeoCoordinate(Topic address, GeoCoordinate geoCoord) {
    try {
      ...
      facetsService.updateFacet(address, "dm4.geomaps.geo_coordinate_facet", mf.newFacetValueModel("dm4.geomaps.geo_coordinate").put(mf.newChildTopicsModel().put("dm4.geomaps.longitude", geoCoord.lon).put("dm4.geomaps.latitude", geoCoord.lat)));
    } catch (Exception e) {
  ...
  private boolean abortGeocodingByCookie(Topic address) {
    ...
    if (cookies.has(COOKIE_NO_GEOCODING)) {
      ...
      if (abort) {
        logger.info("Geocoding for Address topic " + address.getId() + " SUPPRESSED -- \"" + COOKIE_NO_GEOCODING + "\" cookie detected");
      }
    }
    return abort;
  }
  ...
  private boolean abortGeocodingByExcecutionContext(Topic address) {
    ...
    if (abort) {
      logger.info("Geocoding for Address topic " + address.getId() + " SUPPRESSED -- runWithoutGeocoding() " + "context detected");
    }
    return abort;
  }
