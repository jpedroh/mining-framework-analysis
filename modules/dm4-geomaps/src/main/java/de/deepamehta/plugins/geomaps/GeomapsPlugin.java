...
@Path(value = "/geomap") @Consumes(value = "application/json") @Produces(value = "application/json") public class GeomapsPlugin extends PluginActivator implements GeomapsService, PostCreateTopicListener, PostUpdateTopicListener, PreSendTopicListener {
  ...
  @GET @Path(value = "/{id}") @Override public Geomap getGeomap(@PathParam(value = "id") long geomapId) {
    return new Geomap(geomapId, dms);
  }

  @GET @Path(value = "/topic/{id}") @Override public Topic getGeoTopic(@PathParam(value = "id") long topicId, @HeaderParam(value = "Cookie") ClientState clientState) {
    try {
      Topic topic = dms.getTopic(topicId, true, clientState);
      RelatedTopic parentTopic;
      while ((parentTopic = topic.getRelatedTopic(null, "dm4.core.part", "dm4.core.whole", null, true, false, clientState)) != null) {
        topic = parentTopic;
      }
      return topic;
    } catch (Exception e) {
      throw new WebApplicationException(new RuntimeException("Finding the geo coordinate\'s parent topic failed " + "(topicId=" + topicId + ")", e));
    }
  }

  @GET @Path(value = "/{id}/topics") @Override public ResultSet<RelatedTopic> getGeomapTopics(@PathParam(value = "id") long geomapId) {
    return Geomap.fetchGeomapTopics(geomapId, dms);
  }

  @PUT @Path(value = "/{id}/topic/{topic_id}") @Override public void addTopicToGeomap(@PathParam(value = "id") long geomapId, @PathParam(value = "topic_id") long topicId) {
    logger.info("### Adding topic " + topicId + " to geomap " + geomapId);
    AssociationModel model = new AssociationModel("dm4.geomaps.geotopic_mapcontext", new TopicRoleModel(geomapId, "dm4.core.default"), new TopicRoleModel(topicId, "dm4.topicmaps.topicmap_topic"));
    Association refAssoc = dms.createAssociation(model, null);
  }

  @PUT @Path(value = "/{id}/center/{lon}/{lat}/zoom/{zoom}") @Override public void setGeomapState(@PathParam(value = "id") long geomapId, @PathParam(value = "lon") double lon, @PathParam(value = "lat") double lat, @PathParam(value = "zoom") int zoom) {
    CompositeValueModel geomapState = new CompositeValueModel().put("dm4.topicmaps.state", new CompositeValueModel().put("dm4.topicmaps.translation", new CompositeValueModel().put("dm4.topicmaps.translation_x", lon).put("dm4.topicmaps.translation_y", lat)).put("dm4.topicmaps.zoom_level", zoom));
  ...
  @Override @ConsumesService(value = { "de.deepamehta.plugins.topicmaps.service.TopicmapsService", "de.deepamehta.plugins.facets.service.FacetsService" }) public void serviceArrived(PluginService service) {
  ...
  @Override public void preSendTopic(Topic topic, ClientState clientState) {
    ...
    if (geoFacet != null) {
      ...
      logger.info("### Enriching address " + address.getId() + " with its geo facet ABORTED " + "-- no geo facet in DB");
    }
  }
  ...
  private void storeGeoFacet(Topic address, LonLat geoCoordinate, ClientState clientState, Directives directives) {
    try {
      ...
      TopicModel geoFacet = new TopicModel("dm4.geomaps.geo_coordinate", new CompositeValueModel().put("dm4.geomaps.longitude", geoCoordinate.lon).put("dm4.geomaps.latitude", geoCoordinate.lat));
  ...
  private class Address {
    ...
    @Override public boolean equals(Object o) {
      ...
      return street.equals(addr.street) && postalCode.equals(addr.postalCode) && city.equals(addr.city) && country.equals(addr.country);
    }
