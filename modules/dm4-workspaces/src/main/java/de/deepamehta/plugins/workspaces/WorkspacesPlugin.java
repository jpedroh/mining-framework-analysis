...
public class WorkspacesPlugin extends PluginActivator implements WorkspacesService, PostCreateTopicListener, PostInstallPluginListener {
  ...
  @Override public Topic createWorkspace(String name) {
    logger.info("Creating workspace \"" + name + "\"");
    CompositeValue comp = new CompositeValue().put("dm4.workspaces.name", name);
    return dms.createTopic(new TopicModel("dm4.workspaces.workspace", comp), null);
  }

  @Override public void assignTopic(long workspaceId, long topicId) {
    ...
    dms.createAssociation(assocModel, null);
  }

  @Override public void assignType(long workspaceId, long typeId) {
    checkWorkspaceId(workspaceId);
  ...
  @Override public Set<RelatedTopic> getWorkspaces(long typeId) {
    Topic typeTopic = dms.getTopic(typeId, false, null);
    return typeTopic.getRelatedTopics(WORKSPACE_TYPE, ROLE_TYPE_TYPE, null, "dm4.workspaces.workspace", false, false, 0, null).getItems();
  }
  ...
  @Override public void postCreateTopic(Topic topic, ClientState clientState, Directives directives) {
    long workspaceId = -1;
    try {
      if (topic.getTypeUri().equals("dm4.webclient.search") || topic.getTypeUri().equals("dm4.workspaces.workspace")) {
        logger.info("Assigning topic to a workspace ABORTED: searches and workspaces are not assigned (" + topic + ")");
        return;
      }
      if (clientState == null) {
        return;
      ...
      if (wsId == null) {
        logger.warning("Assigning " + topic + " to a workspace failed (current workspace is unknown " + "(no setting found in client context))");
        return;
      }
  ...
  private void checkWorkspaceId(long workspaceId) {
    String typeUri = dms.getTopic(workspaceId, false, null).getTypeUri();
    if (!typeUri.equals("dm4.workspaces.workspace")) {
      throw new IllegalArgumentException("Topic " + workspaceId + " is not a workspace (but of type \"" + typeUri + "\")");
    }
  }

  private boolean isSearchableUnit(TopicType topicType) {
    Boolean isSearchableUnit = (Boolean) topicType.getViewConfig("dm4.webclient.view_config", "dm4.webclient.is_searchable_unit");
    return isSearchableUnit != null ? isSearchableUnit.booleanValue() : false;
  }
}