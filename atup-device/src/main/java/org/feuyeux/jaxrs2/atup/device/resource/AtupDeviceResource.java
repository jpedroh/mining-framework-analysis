...
@Path(value = AtupApi.DEVICE_PATH) public class AtupDeviceResource {
  private final Logger log = LogManager.getLogger(AtupDeviceResource.class.getName());

  @Autowired private AtupDeviceService service;

  @Autowired private StationDetectService detectService;

  @POST @Produces(value = MediaType.APPLICATION_JSON) @Consumes(value = MediaType.APPLICATION_JSON) public AtupDevice createDevice(@Context final HttpHeaders headers, final AtupDevice deviceInfo) {
    try {
  ...
  @PUT @Produces(value = MediaType.APPLICATION_JSON) @Consumes(value = MediaType.APPLICATION_JSON) public AtupDevice updateDevice(@Context final HttpHeaders headers, final AtupDevice deviceInfo) {
    try {
      fillUser(headers, deviceInfo);
  ...
  @GET @Produces(value = MediaType.APPLICATION_JSON) public AtupDeviceListInfo getDevicesByUser(@Context final HttpHeaders headers) {
    final String userId = headers.getRequestHeader("Atup-User").get(0);
  ...
  @POST @Path(value = "detect") public void detect() throws ExecutionException, InterruptedException {
