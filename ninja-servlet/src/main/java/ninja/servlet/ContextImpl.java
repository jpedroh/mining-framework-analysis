...
public class ContextImpl implements Context.Impl {
  ...
  @Inject public ContextImpl(BodyParserEngineManager bodyParserEngineManager, FlashScope flashCookie, NinjaProperties ninjaProperties, ResultHandler resultHandler, Session sessionCookie, Validation validation, Injector injector) {
  ...
  @Override public String getPathParameter(String key) {
  ...
  @Override public String getPathParameterEncoded(String key) {
    return route.getPathParametersEncoded(getRequestPath()).get(key);
  }

  @Override public Integer getPathParameterAsInteger(String key) {
  ...
  @Override public List<String> getParameterValues(String name) {
    if (!formFieldsProcessed) {
      processFormFields();
    }
    if (formFieldsMap == null) {
  ...
  @Override public String getParameter(String key, String defaultValue) {
  ...
  @Override public Integer getParameterAsInteger(String key) {
    return getParameterAsInteger(key, null);
  }

  @Override public Integer getParameterAsInteger(String key, Integer defaultValue) {
    String parameter = getParameter(key);
    try {
      return Integer.parseInt(parameter);
    } catch (NumberFormatException e) {
  ...
  @Override public FileItem getParameterAsFileItem(String key) {
    if (!formFieldsProcessed) {
      processFormFields();
    }
  ...
  @Override public List<FileItem> getParameterAsFileItems(String key) {
    if (!formFieldsProcessed) {
      processFormFields();
    }
    if (fileFieldsMap == null) {
  ...
  @Override public Map<String, FileItem[]> getParameterFileItems() {
    if (!formFieldsProcessed) {
      processFormFields();
    }
  ...
  @Override public <T extends java.lang.Object> T getParameterAs(String key, Class<T> clazz) {
    return getParameterAs(key, clazz, null);
  }

  @Override public <T extends java.lang.Object> T getParameterAs(String key, Class<T> clazz, T defaultValue) {
    String parameter = getParameter(key);
    try {
  ...
  @Override public String getCookieValue(String name) {
    return CookieHelper.getCookieValue(name, httpServletRequest.getCookies());
  }

  @Override public <T extends java.lang.Object> T parseBody(Class<T> classOfT) {
    ...
    BodyParserEngine bodyParserEngine = bodyParserEngineManager.getBodyParserEngineForContentType(contentTypeOnly);
  ...
  @Override public FlashScope getFlashCookie() {
    return flashScope;
  }

  @Override public Session getSessionCookie() {
    return session;
  }

  @Override public FlashScope getFlashScope() {
    return flashScope;
  }
  ...
  @Override public boolean hasCookie(String cookieName) {
  ...
  @Override public void unsetCookie(Cookie cookie) {
    httpServletResponse.addCookie(CookieHelper.convertNinjaCookieToServletCookie(Cookie.builder(cookie).setMaxAge(0).build()));
  }

  @Deprecated @Override public String getRequestUri() {
  ...
  @Override public String getRemoteAddr() {
    boolean isUsageOfXForwardedHeaderEnabled = ninjaProperties.getBooleanWithDefault(Context.NINJA_PROPERTIES_X_FORWARDED_FOR, false);
    String remoteAddr;
  ...
  @Override public void handleAsync() {
    synchronized (asyncLock) {
      if (asyncStrategy == null) {
        asyncStrategy = AsyncStrategyFactoryHolder.getInstance(httpServletRequest).createStrategy(httpServletRequest, resultHandler);
        asyncStrategy.handleAsync();
      }
    }
  }

  @Override public void returnResultAsync(Result result) {
    synchronized (asyncLock) {
      handleAsync();
  ...
  @Override public void asyncRequestComplete() {
  ...
  /**
     ...
     */
  @Override public Result controllerReturned() {
    if (asyncStrategy != null) {
  ...
  @Override public ResponseStreams finalizeHeadersWithoutFlashAndSessionCookie(Result result) {
  ...
  @Override public String getAcceptEncoding() {
    return httpServletRequest.getHeader("accept-encoding");
  }

  @Override public String getAcceptLanguage() {
  ...
  @Override public FileItemIterator getFileItemIterator() {
  ...
  @Override public Validation getValidation() {
    return validation;
  }
  ...
  @Override public <T extends java.lang.Object> T getAttribute(String name, Class<T> clazz) {
  ...
  @Override public boolean isRequestJson() {
  ...
  @Override public boolean isRequestXml() {
    String contentType = getRequestContentType();
    if (contentType == null || contentType.isEmpty()) {
      return false;
    }
