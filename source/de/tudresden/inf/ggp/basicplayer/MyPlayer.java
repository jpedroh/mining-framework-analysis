...
public final class MyPlayer extends Player {
  static {
    StrategyFactory.getInstance().addDescription("AStarStrategy", SinglePlayerStrategy.class.getCanonicalName(), "Simulates games and chooses the best path.");
  }
  ...
  public void commandStart(Message msg) {
    ...
    Game runningGame = (Game) factory.createGame(GameFactory.JAVAPROVER, msg.getGameDescription());
    ...
    System.out.println("Casanova created the strategy " + strategy.getClass().getSimpleName() + ".");
    System.out.println("Casanova starts contemplate while doing yoga.");
    realMatch = createRealMatch(msg.getMatchId(), runningGame, strategy, msg.getRole(), startClock, playClock);
  ...
  public String commandPlay(Message msg) {
    checkMatchId(msg);
    if (msg.hasMoves()) {
      System.out.println("Moves from GameMaster: " + Arrays.toString(msg.getMoves()));
  ...
  public static void main(String[] args) {
    try {
      new PlayerServer(new MyPlayer(), PlayerServer.getOptions(args)).waitForExit();
