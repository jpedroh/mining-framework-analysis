...
@SuppressWarnings(value = { "deprecation" }) public final class PartitionTracker {
  ...
  public PartitionTracker(QueryPolicy policy, Statement stmt, int nodeCapacity, PartitionFilter filter) {
    this((Policy) policy, nodeCapacity, filter, (stmt.maxRecords > 0) ? stmt.maxRecords : policy.maxRecords);
  }
  ...
  private PartitionTracker(Policy policy, int nodeCapacity, PartitionFilter filter, long maxRecords) {
    if (!(filter.begin >= 0 && filter.begin < Node.PARTITIONS)) {
      throw new AerospikeException(ResultCode.PARAMETER_ERROR, "Invalid partition begin " + filter.begin + ". Valid range: 0-" + (Node.PARTITIONS - 1));
    }
    if (filter.count <= 0) {
    ...
    if (filter.begin + filter.count > Node.PARTITIONS) {
      throw new AerospikeException(ResultCode.PARAMETER_ERROR, "Invalid partition range (" + filter.begin + ',' + filter.count + ')');
    }
