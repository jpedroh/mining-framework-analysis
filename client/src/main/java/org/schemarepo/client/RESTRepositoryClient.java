...
public class RESTRepositoryClient implements RepositoryClient {
  ...
  @Inject public RESTRepositoryClient(@Named(value = Config.CLIENT_SERVER_URL) String url, @Named(value = Config.JSON_UTIL_IMPLEMENTATION) JsonUtil jsonUtil, @Named(value = Config.CLIENT_RETURN_NONE_ON_EXCEPTIONS) boolean returnNoneOnExceptions) {
    logger.info(format("Pointing to schema-repo server at %s", url));
    logger.info(format("Remote exceptions from GET requests will be %s", returnNoneOnExceptions ? "swallowed and an \'empty\' value returned" : "propagated to the caller"));
    this.webResource = Client.create().resource(url);
    this.returnNoneOnExceptions = returnNoneOnExceptions;
    this.jsonUtil = jsonUtil;
  }

  @Override public Subject register(String subject, SubjectConfig config) {
    Form form = new Form();
    ...
    String regSubjectName = webResource.path(subject).accept(MediaType.TEXT_PLAIN).type(MediaType.APPLICATION_FORM_URLENCODED).put(String.class, form);
    return new RESTSubject(regSubjectName);
  }

  @Override public Subject lookup(String subject) {
  ...
  @Override public Iterable<Subject> subjects() {
  ...
  private class RESTSubject extends Subject {
    ...
    @Override public SubjectConfig getConfig() {
    ...
    @Override public SchemaEntry register(String schema) throws SchemaValidationException {
      RepositoryUtil.validateSchemaOrSubject(schema);
      String path = getName() + "/register";
      return handleRegisterRequest(path, schema, false);
    }

    @Override public SchemaEntry registerIfLatest(String schema, SchemaEntry latest) throws SchemaValidationException {
      RepositoryUtil.validateSchemaOrSubject(schema);
    ...
    private SchemaEntry handleRegisterRequest(String path, String schema, boolean resourceNotFoundExpected) throws SchemaValidationException {
      ...
      try {
        String schemaId = webResource.path(path).accept(MediaType.TEXT_PLAIN).type(MediaType.TEXT_PLAIN_TYPE).put(String.class, schema);
    ...
    @Override public SchemaEntry lookupBySchema(String schema) {
      RepositoryUtil.validateSchemaOrSubject(schema);
      String path = getName() + "/schema";
      SchemaEntry schemaEntry = null;
      try {
        String schemaId = webResource.path(path).accept(MediaType.TEXT_PLAIN).type(MediaType.TEXT_PLAIN_TYPE).post(String.class, schema);
        schemaEntry = new SchemaEntry(schemaId, schema);
      } catch (RuntimeException e) {
    ...
    @Override public SchemaEntry lookupById(String schemaId) {
    ...
    @Override public SchemaEntry latest() {
    ...
    @Override public Iterable<SchemaEntry> allEntries() {
...
>>>>>>> /usr/src/app/output/schema-repo/schema-repo/f1a22946df99bc2bd9997867310bc9420c72adf2/client/src/main/java/org/schemarepo/client/RESTRepositoryClient.java/right.java
    ...
    @Override public boolean integralKeys() {
  ...
  private void handleException(Exception ex, String msg, boolean resourceNotFoundExpected) {
    final ClientResponse.Status status = ex instanceof UniformInterfaceException ? ((UniformInterfaceException) ex).getResponse().getClientResponseStatus() : null;
    if (status == NOT_FOUND && resourceNotFoundExpected || status == CONFLICT) {
      logger.debug(msg, ex);
    } else {
      if (returnNoneOnExceptions) {
