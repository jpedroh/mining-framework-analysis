...
public class XDIDiscoveryClient {
  ...
  public XDIDiscoveryClient(XDIClient<TransportMessagingResponse> registryXdiClient, DiscoveryCache discoveryCache) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Initializing discovery with client " + (registryXdiClient == null ? null : registryXdiClient.getClass().getSimpleName()) + " and cache " + (discoveryCache == null ? null : discoveryCache.getClass().getSimpleName()));
    }
  }
  ...
  public XDIDiscoveryResult discover(XDIAddress query, XDIAddress[] endpointUriTypes) throws Xdi2DiscoveryException, Xdi2ClientException {
    ...
    if (xdiDiscoveryResultRegistry == null) {
      if (log.isDebugEnabled()) {
        log.debug("No discovery result from registry for " + query);
      }
      return null;
    }
    if (xdiDiscoveryResultRegistry.getXdiEndpointUri() == null || xdiDiscoveryResultRegistry.getCloudNumber() == null) {
      if (log.isDebugEnabled()) {
        log.debug("No XDI endpoint URI or cloud number from registry for " + query);
      }
    ...
    if (xdiDiscoveryResultAuthority == null) {
      if (log.isDebugEnabled()) {
        log.debug("No discovery result from authority for " + query);
      }
      return xdiDiscoveryResultRegistry;
    }
  ...
  public XDIDiscoveryResult discoverFromRegistry(XDIAddress query, XDIAddress[] endpointUriTypes) throws Xdi2DiscoveryException, Xdi2ClientException {
    ...
    if (this.getDiscoveryCache() != null) {
      registryMessagingResponse = (TransportMessagingResponse) this.getDiscoveryCache().getRegistry(registryDiscoveryCacheKey);
    }
    MessageEnvelope registryMessageEnvelope = null;
    if (registryMessagingResponse != null) {
      if (log.isDebugEnabled()) {
        log.debug("Registry cache HIT: " + registryDiscoveryCacheKey);
      }
    } else {
      if (log.isDebugEnabled()) {
        log.debug("Registry cache MISS: " + registryDiscoveryCacheKey);
      }
      ...
      if (this.getDiscoveryCache() != null) {
        if (log.isDebugEnabled()) {
          log.debug("Registry cache PUT: " + registryDiscoveryCacheKey);
        }
    ...
    if (CloudNumber.isValid(query) && xdiDiscoveryResult.getCloudNumber() != null) {
      if (!xdiDiscoveryResult.getCloudNumber().getXDIAddress().equals(query)) {
        throw new Xdi2DiscoveryException("Queried cloud number " + query + " does not match discovered cloud number " + xdiDiscoveryResult.getCloudNumber().getXDIAddress());
      }
    }
    if (log.isDebugEnabled()) {
  ...
  public XDIDiscoveryResult discoverFromAuthority(URI xdiEndpointUri, CloudNumber cloudNumber, XDIAddress[] endpointUriTypes) throws Xdi2DiscoveryException, Xdi2ClientException {
    ...
    if (this.getDiscoveryCache() != null) {
      authorityMessagingResponse = (TransportMessagingResponse) this.getDiscoveryCache().getAuthority(authorityDiscoveryCacheKey);
    }
    MessageEnvelope authorityMessageEnvelope = null;
    if (authorityMessagingResponse != null) {
      if (log.isDebugEnabled()) {
        log.debug("Authority cache HIT: " + authorityDiscoveryCacheKey);
      }
    } else {
      if (log.isDebugEnabled()) {
        log.debug("Authority cache MISS: " + authorityDiscoveryCacheKey);
      }
      ...
      if (this.getDiscoveryCache() != null) {
        if (log.isDebugEnabled()) {
          log.debug("Authority cache PUT: " + authorityDiscoveryCacheKey);
        }
