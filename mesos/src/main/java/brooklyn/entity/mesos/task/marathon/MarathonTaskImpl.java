...
public class MarathonTaskImpl extends MesosTaskImpl implements MarathonTask {
  ...
  public static final CharMatcher TASK_CHARACTERS = CharMatcher.anyOf("_-/").or(CharMatcher.inRange('a', 'z')).or(CharMatcher.inRange('0', '9'));
  ...
  @Override public void connectSensors() {
    ...
    HttpFeed.Builder httpFeedBuilder = HttpFeed.builder().entity(this).period(500, TimeUnit.MILLISECONDS).baseUri(uri).credentialsIfNotNull(config().get(MesosCluster.MESOS_USERNAME), config().get(MesosCluster.MESOS_PASSWORD)).header("Accept", "application/json").poll(new HttpPollConfig<Boolean>(SERVICE_UP).onSuccess(Functionals.chain(HttpValueFunctions.jsonContents(), JsonFunctions.walk("tasks"), new Function<JsonElement, Boolean>() {
      @Override public Boolean apply(JsonElement input) {
        JsonArray tasks = input.getAsJsonArray();
        return tasks.size() == 1;
      }
    })).onFailureOrException(Functions.constant(Boolean.FALSE))).poll(new HttpPollConfig<Long>(TASK_STARTED_AT).onSuccess(Functionals.chain(HttpValueFunctions.jsonContents(), JsonFunctions.walk("tasks"), new Function<JsonElement, Long>() {
      @Override public Long apply(JsonElement input) {
        JsonArray tasks = input.getAsJsonArray();
    ...
    })).onFailureOrException(Functions.<Long>constant(-1L))).poll(new HttpPollConfig<Long>(TASK_STAGED_AT).onSuccess(Functionals.chain(HttpValueFunctions.jsonContents(), JsonFunctions.walk("tasks"), new Function<JsonElement, Long>() {
      @Override public Long apply(JsonElement input) {
        JsonArray tasks = input.getAsJsonArray();
        for (JsonElement each : tasks) {
    ...
    })).onFailureOrException(Functions.<Long>constant(-1L))).poll(new HttpPollConfig<String>(Attributes.HOSTNAME).onSuccess(Functionals.chain(HttpValueFunctions.jsonContents(), JsonFunctions.walk("tasks"), new Function<JsonElement, String>() {
    ...
    })).onFailureOrException(Functions.<String>constant(null))).poll(new HttpPollConfig<String>(Attributes.ADDRESS).onSuccess(Functionals.chain(HttpValueFunctions.jsonContents(), JsonFunctions.walk("tasks"), new Function<JsonElement, String>() {
      @Override public String apply(JsonElement input) {
        JsonArray tasks = input.getAsJsonArray();
        for (JsonElement each : tasks) {
          JsonElement host = each.getAsJsonObject().get("host");
  ...
  @Override public void start(Collection<? extends Location> locations) {
    ...
    try {
      LOG.debug("Starting task {} on {} with flags: {}", new Object[] { id, marathon, Joiner.on(",").withKeyValueSeparator("=").join(flags) });
  ...
  @Override public Entity getRunningEntity() {
    return sensors().get(ENTITY);
  }

  @Override public void setRunningEntity(Entity entity) {
  ...
  @Override public MarathonTaskLocation getDynamicLocation() {
  ...
  public Optional<JsonElement> getApplicationJson() {
    ...
    HttpToolResponse response = HttpTool.httpGet(MesosUtils.buildClient(framework), URI.create(uri), MutableMap.of(HttpHeaders.ACCEPT, "application/json"));
  ...
  @Override public MarathonTaskLocation createLocation(Map<String, ?> flags) {
    Entity entity = getRunningEntity();
    ...
    if (application.isPresent()) {
      JsonArray tasks = application.get().getAsJsonObject().get("app").getAsJsonObject().get("tasks").getAsJsonArray();
      for (JsonElement each : tasks) {
        ...
        if (ports != null && !ports.isJsonNull()) {
          ...
          if (array.size() > 0) {
            for (int i = 0; i < array.size(); i++) {
              ...
              if (containerPort == 22) {
                sensors().set(DockerAttributes.DOCKER_MAPPED_SSH_PORT, HostAndPort.fromParts(getHostname(), hostPort).toString());
              }
    ...
    LocationSpec<MarathonTaskLocation> spec = LocationSpec.create(MarathonTaskLocation.class).parent(getMarathonFramework().getDynamicLocation()).configure(flags).configure(DynamicLocation.OWNER, this).configure("entity", getRunningEntity()).configure(CloudLocationConfig.WAIT_FOR_SSHABLE, "false").configure(SshMachineLocation.DETECT_MACHINE_DETAILS, false).configure(SshMachineLocation.TCP_PORT_MAPPINGS, tcpMappings).displayName(getShortName());
    Boolean useSsh = config().get(DockerAttributes.DOCKER_USE_SSH);
    if (useSsh) {
      spec.configure(SshMachineLocation.SSH_HOST, getHostname()).configure(SshMachineLocation.SSH_PORT, getSshPort()).configure("address", getAddress()).configure(LocationConfigKeys.USER, "root").configure(LocationConfigKeys.PASSWORD, "p4ssw0rd").configure(SshTool.PROP_PASSWORD, "p4ssw0rd").configure(SshTool.PROP_HOST, getHostname()).configure(SshTool.PROP_PORT, getSshPort()).configure(LocationConfigKeys.PRIVATE_KEY_DATA, (String) null).configure(LocationConfigKeys.PRIVATE_KEY_FILE, (String) null);
    }
  ...
  @Override public MarathonFramework getMarathonFramework() {
