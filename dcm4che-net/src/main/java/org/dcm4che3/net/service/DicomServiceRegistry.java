...
public class DicomServiceRegistry implements DimseRQHandler {
  ...
  @Override public void onDimseRQ(Association as, PresentationContext pc, Dimse dimse, Attributes cmd, PDVInputStream data) throws IOException {
    try {
      lookupService(as, dimse, cmd).onDimseRQ(as, pc, dimse, cmd, data);
    } catch (DicomServiceApplicationException e) {
      Association.LOG.info("{}: processing {} failed. Caused by: {}", as, dimse.toString(cmd, pc.getPCID(), pc.getTransferSyntax()), e.getLocalizedMessage());
      rspForDimseRQException(as, pc, dimse, cmd, e);
    } catch (DicomServiceException e) {
      Association.LOG.info("{}: processing {} failed. Caused by:\t", as, dimse.toString(cmd, pc.getPCID(), pc.getTransferSyntax()), e);
      rspForDimseRQException(as, pc, dimse, cmd, e);
    }
  }
  ...
  @Override public void onClose(Association as) {
