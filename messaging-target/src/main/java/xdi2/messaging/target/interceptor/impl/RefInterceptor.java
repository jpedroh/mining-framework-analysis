...
public class RefInterceptor extends AbstractInterceptor<MessagingTarget> implements MessageEnvelopeInterceptor, OperationInterceptor, TargetInterceptor, Prototype<RefInterceptor> {
  ...
  @Override public RefInterceptor instanceFor(PrototypingContext prototypingContext) {
    return this;
  }

  @Override public void init(MessagingTarget messagingTarget) throws Exception {
  ...
  @Override public InterceptorResult before(MessageEnvelope messageEnvelope, ExecutionContext executionContext, ExecutionResult executionResult) throws Xdi2MessagingException {
    resetRefRepRelationsPerMessageEnvelope(executionContext);
    resetCompletedAddresses(executionContext);
    return InterceptorResult.DEFAULT;
  }

  @Override public InterceptorResult after(MessageEnvelope messageEnvelope, ExecutionContext executionContext, ExecutionResult executionResult) throws Xdi2MessagingException {
    return InterceptorResult.DEFAULT;
  }

  @Override public void exception(MessageEnvelope messageEnvelope, ExecutionContext executionContext, ExecutionResult executionResult, Exception ex) {
  }
  ...
  @Override public InterceptorResult after(Operation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
      ...
      for (Relation refRepRelation : refRepRelations) {
        ...
        for (XDIAddress completedAddress : getCompletedAddresses(executionContext)) {
          if (refRepRelation.getContextNode().getXDIAddress().equals(completedAddress)) {
            if (log.isDebugEnabled()) {
              log.debug("In message result: Skipping $ref/$rep relation " + refRepRelation + " because of already completed address " + completedAddress);
            }
        ...
        if (log.isDebugEnabled()) {
          log.debug("In message result: After $get feedback on $ref/$rep relation " + refRepRelation + " we now have: " + operationResultGraph);
        }
      }
    }
    ...
    while ((refRepRelation = popRefRepRelationPerOperation(executionContext)) != null) {
      ...
      if (log.isDebugEnabled()) {
        log.debug("In message result: We now have: " + operationResultGraph);
      }
    }
    return InterceptorResult.DEFAULT;
  }

  @Override public XDIAddress targetAddress(XDIAddress targetAddress, Operation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
    if (targetAddress.isLiteralNodeXDIAddress()) {
  ...
  @Override public XDIStatement targetStatement(XDIStatement targetStatement, Operation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
    boolean doFollowTargetSubject = true;
    if (XDIDictionaryConstants.XDI_ADD_REF.equals(targetStatement.getRelationXDIAddress())) {
      doFollowTargetSubject = false;
    }
    if (XDIDictionaryConstants.XDI_ADD_REP.equals(targetStatement.getRelationXDIAddress())) {
      doFollowTargetSubject = false;
    }
    if (XDIDictionaryConstants.XDI_ADD_IS_REF.equals(targetStatement.getRelationXDIAddress())) {
      doFollowTargetSubject = false;
    }
    if (XDIDictionaryConstants.XDI_ADD_IS_REP.equals(targetStatement.getRelationXDIAddress())) {
      doFollowTargetSubject = false;
    }
    boolean doFollowTargetObject = true;
    if (XDIDictionaryConstants.XDI_ADD_REF.equals(targetStatement.getRelationXDIAddress())) {
      doFollowTargetObject = false;
    }
    if (XDIDictionaryConstants.XDI_ADD_REP.equals(targetStatement.getRelationXDIAddress())) {
      doFollowTargetObject = false;
    }
    ...
    if (targetStatement.isRelationStatement() && XDIConstants.XDI_ADD_COMMON_VARIABLE.equals(targetStatement.getTargetXDIAddress())) {
      doFollowTargetObject = false;
    }
  ...
  private static XDIAddress followAllRefRepRelations(XDIAddress contextNodeXDIAddress, Operation operation, ExecutionContext executionContext) throws Xdi2MessagingException {
    ...
    while (true) {
      ...
      if (followedContextNodeXDIAddress == tempContextNodeXDIAddress) {
        break;
      }
      if (log.isDebugEnabled()) {
        log.debug("In message envelope: Followed " + tempContextNodeXDIAddress + " to " + followedContextNodeXDIAddress);
      }
    }
    ...
    while (!XDIConstants.XDI_ADD_ROOT.equals(contextNodeXDIAddress)) {
      ...
      if (refRelation != null) {
        ...
        if (referenceContextNode != null) {
          if (referenceContextNode.equals(refRelation.getContextNode())) {
            break;
          }
      ...
      if (repRelation != null) {
        ...
        if (replacementContextNode != null) {
          if (replacementContextNode.equals(repRelation.getContextNode())) {
            break;
          }
  ...
  private static Graph feedbackGetSourceOfRefRepRelation(ContextNode refRepContextNode, Operation operation, ExecutionContext executionContext) throws Xdi2MessagingException {
    if (log.isDebugEnabled()) {
      log.debug("Initiating $get feedback to get source of $ref/$rep relation: " + refRepContextNode);
    }
    ...
    if (Boolean.TRUE.equals(operation.getParameterBoolean(XDIMessagingConstants.XDI_ADD_OPERATION_PARAMETER_DEREF))) {
      feedbackOperation.setParameter(XDIMessagingConstants.XDI_ADD_OPERATION_PARAMETER_DEREF, Boolean.TRUE);
    }
    ...
    try {
      ...
      if (linkContractInterceptor != null) {
        linkContractInterceptor.setDisabledForMessage(feedbackMessage);
      }
    ...
    if (log.isDebugEnabled()) {
      log.debug("Completed $get feedback on source of $ref/$rep relation: " + refRepContextNode + ", execution result: " + feedbackExecutionResult);
    }
  ...
  private static Graph feedbackFindRefRepRelationsInContext(XDIAddress contextNodeXDIAddress, Operation operation, ExecutionContext executionContext) throws Xdi2MessagingException {
    if (log.isDebugEnabled()) {
      log.debug("Initiating $get feedback to find $ref/$rep relations in context: " + contextNodeXDIAddress);
    }
    AbstractMessagingTarget messagingTarget = (AbstractMessagingTarget) executionContext.getCurrentMessagingTarget();
    MessageEnvelope feedbackMessageEnvelope = new MessageEnvelope();
    ...
    try {
      ...
      if (refInterceptor != null) {
        refInterceptor.setDisabledForMessage(feedbackMessageRef);
      }
      if (refInterceptor != null) {
        refInterceptor.setDisabledForMessage(feedbackMessageRep);
      }
      LinkContractInterceptor linkContractInterceptor = messagingTarget.getInterceptors().getInterceptor(LinkContractInterceptor.class);
      if (linkContractInterceptor != null) {
        linkContractInterceptor.setDisabledForMessage(feedbackMessageRef);
      }
      if (linkContractInterceptor != null) {
        linkContractInterceptor.setDisabledForMessage(feedbackMessageRep);
      }
      ...
      if (messagePolicyInterceptor != null) {
        messagePolicyInterceptor.setDisabledForMessage(feedbackMessageRef);
      }
      if (messagePolicyInterceptor != null) {
        messagePolicyInterceptor.setDisabledForMessage(feedbackMessageRep);
      }
      messageAttributes = executionContext.getMessageAttributes();
      operationAttributes = executionContext.getOperationAttributes();
    ...
    feedbackExecutionResult.finish();
    if (log.isDebugEnabled()) {
      log.debug("Completed $get feedback to find $ref/$rep relations in context: " + contextNodeXDIAddress + ", execution result: " + feedbackExecutionResult);
    }
    return feedbackExecutionResult.getFinishedResultGraph();
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private static Map<XDIAddress, Relation[]> getRefRepRelationsPerMessageEnvelope(ExecutionContext executionContext) {
  ...
  private static Relation[] getRefRepRelationPerMessageEnvelope(ExecutionContext executionContext, XDIAddress contextNodeXDIAddress) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Get $ref/$rep relation for " + contextNodeXDIAddress + ": " + refRepRelation);
    }
  ...
  private static void setRefRepRelationPerMessageEnvelope(ExecutionContext executionContext, XDIAddress contextNodeXDIAddress, Relation[] refRepRelation) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Set $ref/$rep relation for " + contextNodeXDIAddress + ": " + refRepRelation);
    }
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private static Deque<Relation> getRefRepRelationsPerOperation(ExecutionContext executionContext) {
  ...
  private static Relation popRefRepRelationPerOperation(ExecutionContext executionContext) {
    ...
    if (refRepRelations.isEmpty()) {
      return null;
    }
    Relation refRepRelation = refRepRelations.pop();
    if (log.isDebugEnabled()) {
      log.debug("Popping $ref/$rep relation: " + refRepRelation);
    }
    return refRepRelation;
  }
  ...
  private static void pushRefRepRelationPerOperation(ExecutionContext executionContext, Relation refRepRelation) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Pushing $ref/$rep relation: " + refRepRelation);
    }
  }
  ...
  @SuppressWarnings(value = { "unchecked" }) private static Set<XDIAddress> getCompletedAddresses(ExecutionContext executionContext) {
  ...
  private static void addCompletedAddress(ExecutionContext executionContext, XDIAddress contextNodeXDIAddress) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Added completed address: " + contextNodeXDIAddress);
    }
  }
