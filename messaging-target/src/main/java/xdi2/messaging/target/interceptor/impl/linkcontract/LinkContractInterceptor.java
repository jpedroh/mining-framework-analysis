...
public class LinkContractInterceptor extends AbstractInterceptor<MessagingTarget> implements GraphAware, MessageInterceptor, OperationInterceptor, TargetInterceptor, Prototype<LinkContractInterceptor> {
  ...
  @Override public LinkContractInterceptor instanceFor(PrototypingContext prototypingContext) {
  ...
  @Override public void setGraph(Graph graph) {
    if (this.getLinkContractsGraph() == null) {
      this.setLinkContractsGraph(graph);
    }
  }
  ...
  @Override public InterceptorResult before(Message message, ExecutionContext executionContext, ExecutionResult executionResult) throws Xdi2MessagingException {
    ...
    if (linkContract == null) {
      throw new Xdi2MessagingException("Invalid link contract: " + linkContractXDIAddress, null, executionContext);
    }
    if (log.isDebugEnabled()) {
    ...
    if (policyRoot != null) {
      if (log.isDebugEnabled()) {
        log.debug("Link contract " + linkContract + " policy evaluated to " + policyRootResult);
      }
    }
    if (policyRootResult) {
    ...
    if (pushPolicyRoot != null) {
      if (log.isDebugEnabled()) {
        log.debug("Link contract " + linkContract + " push policy evaluated to " + pushPolicyRootResult);
      }
    }
  ...
  @Override public InterceptorResult before(Operation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
    ...
    if (pushFlag == null) {
      throw new Xdi2MessagingException("No push flag.", null, executionContext);
    }
    if (isConnect(operation)) {
      ...
      for (LinkContractTemplate linkContractTemplate : linkContractTemplates) {
        ...
        if (decideLinkContractPermission(operation.getOperationXDIAddress(), targetXDIAddress, linkContract)) {
          ...
          if (log.isDebugEnabled()) {
            log.debug("Authorization failed, because of missing " + operation.getOperationXDIAddress() + " permissions on target address " + targetXDIAddress);
          }
        }
  ...
  @Override public InterceptorResult after(Operation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
    return InterceptorResult.DEFAULT;
  }

  @Override public XDIAddress targetAddress(XDIAddress targetXDIAddress, Operation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
    LinkContract linkContract = getLinkContract(executionContext);
    if (linkContract == null) {
      throw new Xdi2MessagingException("No link contract.", null, executionContext);
    }
    ...
    if (Boolean.TRUE.equals(authorized)) {
      if (isSetOnDoAddress(targetXDIAddress, operation)) {
        ...
        if (decideLinkContractPermission(XDILinkContractConstants.XDI_ADD_SET_DO, doTargetAddress, linkContract)) {
          authorized = Boolean.TRUE;
          if (log.isDebugEnabled()) {
            log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_DO + " succeeded, because of " + XDILinkContractConstants.XDI_ADD_SET_DO + " permission on target address " + doTargetAddress);
          }
        } else {
          authorized = Boolean.FALSE;
          if (log.isDebugEnabled()) {
            log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_DO + " failed, because of missing " + XDILinkContractConstants.XDI_ADD_SET_DO + " permission on target address " + doTargetAddress);
          }
        }
      }
    }
  ...
  @Override public XDIStatement targetStatement(XDIStatement targetXDIStatement, Operation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
    LinkContract linkContract = getLinkContract(executionContext);
    if (linkContract == null) {
      throw new Xdi2MessagingException("No link contract.", null, executionContext);
    }
    Boolean pushRequired = getPushFlag(executionContext);
    if (pushRequired == null) {
      throw new Xdi2MessagingException("No push required flag.", null, executionContext);
    }
    ...
    if (decideLinkContractPermission(operation.getOperationXDIAddress(), targetXDIAddress, linkContract)) {
      ...
      if (decideLinkContractPermission(operation.getOperationXDIAddress(), targetXDIStatement, linkContract)) {
        authorized = Boolean.TRUE;
        if (log.isDebugEnabled()) {
          log.debug("Authorization succeeded, because of " + operation.getOperationXDIAddress() + " permission on target statement " + targetXDIStatement);
        }
      } else {
        authorized = Boolean.FALSE;
        if (log.isDebugEnabled()) {
          log.debug("Authorization failed, because of missing " + operation.getOperationXDIAddress() + " permissions on either target address " + targetXDIAddress + ", or target statement " + targetXDIStatement);
        }
      }
    }
    if (Boolean.TRUE.equals(authorized)) {
      if (isSetOnDoAddress(targetXDIAddress, operation)) {
        if (decideLinkContractPermission(XDILinkContractConstants.XDI_ADD_SET_DO, targetXDIAddress, linkContract)) {
          authorized = Boolean.TRUE;
          if (log.isDebugEnabled()) {
            log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_DO + " succeeded, because of " + XDILinkContractConstants.XDI_ADD_SET_DO + " permission on target address " + targetXDIAddress);
          }
        } else {
          if (decideLinkContractPermission(operation.getOperationXDIAddress(), targetXDIStatement, linkContract)) {
            authorized = Boolean.TRUE;
            if (log.isDebugEnabled()) {
              log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_DO + " succeeded, because of " + XDILinkContractConstants.XDI_ADD_SET_DO + " permission on target statement " + targetXDIStatement);
            }
          } else {
            authorized = Boolean.FALSE;
            if (log.isDebugEnabled()) {
              log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_DO + " failed, because of missing " + XDILinkContractConstants.XDI_ADD_SET_DO + " permission on target address " + targetXDIAddress);
            }
          }
        }
      }
    ...
    if (Boolean.TRUE.equals(authorized)) {
      if (isSetOnRefRepStatement(targetXDIStatement, operation)) {
        if (decideLinkContractPermission(XDILinkContractConstants.XDI_ADD_SET_REF, targetXDIStatement.getContextNodeXDIAddress(), linkContract) && decideLinkContractPermission(XDILinkContractConstants.XDI_ADD_SET_REF, targetXDIStatement.getTargetXDIAddress(), linkContract)) {
          authorized = Boolean.TRUE;
          if (log.isDebugEnabled()) {
            log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_REF + " succeeded, because of " + XDILinkContractConstants.XDI_ADD_SET_REF + " permission on target addresses " + targetXDIStatement.getContextNodeXDIAddress() + " and " + targetXDIStatement.getTargetXDIAddress());
          }
        } else {
          if (decideLinkContractPermission(XDILinkContractConstants.XDI_ADD_SET_REF, targetXDIStatement, linkContract)) {
            authorized = Boolean.TRUE;
            if (log.isDebugEnabled()) {
              log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_REF + " succeeded, because of " + XDILinkContractConstants.XDI_ADD_SET_REF + " permission on target statement " + targetXDIStatement);
            }
          } else {
            authorized = Boolean.FALSE;
            if (log.isDebugEnabled()) {
              log.debug("Authorization for " + XDILinkContractConstants.XDI_ADD_SET_REF + " failed, because of missing " + XDILinkContractConstants.XDI_ADD_SET_REF + " permissions on either target addresses " + targetXDIStatement.getContextNodeXDIAddress() + " and " + targetXDIStatement.getTargetXDIAddress() + ", or target statement " + targetXDIStatement);
            }
          }
        }
      }
  ...
  private static boolean isSetOnDoAddress(XDIAddress targetAddress, Operation operation) {
    ...
    if (XDIAddressUtil.indexOfXDIArc(targetAddress, XDILinkContractConstants.XDI_ARC_DO) != -1) {
      return true;
    }
    return false;
  }
  ...
  private static boolean isSetOnRefRepStatement(XDIStatement targetStatement, Operation operation) {
    ...
    if (XDIDictionaryConstants.XDI_ADD_REP.equals(targetStatement.getRelationXDIAddress())) {
      return true;
    }
    return false;
  }
  ...
  private static boolean decideLinkContractPermission(XDIAddress permissionAddress, XDIAddress contextNodeXDIAddress, LinkContract linkContract) {
    ...
    for (XDIAddress targetAddress : positiveIterator) {
      if (XDIAddressUtil.startsWithXDIAddress(contextNodeXDIAddress, targetAddress, false, true) != null) {
        ...
        if (positiveMatch > longestPositivePermission) {
          longestPositivePermission = positiveMatch;
        }
        if (log.isDebugEnabled()) {
          log.debug("Link contract " + linkContract + " allows " + permissionAddress + " on " + contextNodeXDIAddress);
        }
      }
    }
    ...
    for (XDIAddress targetAddress : negativeIterator) {
      if (XDIAddressUtil.startsWithXDIAddress(contextNodeXDIAddress, targetAddress, false, true) != null) {
        ...
        if (negativeMatch > longestNegativePermission) {
          longestNegativePermission = negativeMatch;
        }
        if (log.isDebugEnabled()) {
          log.debug("Link contract " + linkContract + " does not allow " + permissionAddress + " on " + contextNodeXDIAddress);
        }
      }
    }
    boolean decision = longestPositivePermission > longestNegativePermission;
    if (log.isDebugEnabled()) {
      log.debug("Link contract " + linkContract + " decision for " + permissionAddress + " on address " + contextNodeXDIAddress + ": " + decision);
    }
    return decision;
  }
  ...
  private static boolean decideLinkContractPermission(XDIAddress permissionAddress, XDIStatement XDIstatement, LinkContract linkContract) {
    boolean positivePermission = linkContract.hasPermissionTargetXDIStatement(permissionAddress, XDIstatement) || linkContract.hasPermissionTargetXDIStatement(XDILinkContractConstants.XDI_ADD_ALL, XDIstatement);
    if (log.isDebugEnabled()) {
      log.debug("Link contract " + linkContract + " allows " + permissionAddress + " on " + XDIstatement);
    }
    boolean negativePermission = linkContract.hasNegativePermissionTargetXDIStatement(permissionAddress, XDIstatement) || linkContract.hasNegativePermissionTargetXDIStatement(XDILinkContractConstants.XDI_ADD_ALL, XDIstatement);
    if (log.isDebugEnabled()) {
      log.debug("Link contract " + linkContract + " does not allow " + permissionAddress + " on " + XDIstatement);
    }
    boolean decision = positivePermission && !negativePermission;
    if (log.isDebugEnabled()) {
      log.debug("Link contract " + linkContract + " decision for " + permissionAddress + " on statement " + XDIstatement + ": " + decision);
    }
    return decision;
  }
...
>>>>>>> /usr/src/app/output/projectdanube/xdi2/18402a3963837f0348650eea9f7c3801c5a4f313/messaging-target/src/main/java/xdi2/messaging/target/interceptor/impl/linkcontract/LinkContractInterceptor.java/right.java
