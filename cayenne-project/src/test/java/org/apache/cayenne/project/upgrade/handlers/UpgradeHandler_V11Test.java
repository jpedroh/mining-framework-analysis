...
public class UpgradeHandler_V11Test extends BaseUpgradeHandlerTest {
  ...
  @Test public void testProjectDomUpgrade() throws Exception {
  ...
  @Test public void testDataMapDomUpgrade() throws Exception {
  ...
  @Test public void testCgenDomUpgrade() throws Exception {
    Document document = processDataMapDom("test-map-v10.map.xml");
    Element root = document.getDocumentElement();
    ...
    for (int i = 0; i < childNodes.getLength(); i++) {
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE) {
        switch (node.getNodeName()) {
          ...
          break;
          case "superTemplate":
          assertEquals("The template /org/apache/cayenne/project/upgrade/handlers was not found " + "during the project upgrade. Use the template editor in Cayenne modeler to set the template", node.getFirstChild().getNodeValue());
          break;
          case "embeddableSuperTemplate":
          assertEquals("The template ../../testWrongPath was not found during the project upgrade." + " Use the template editor in Cayenne modeler to set the template", node.getFirstChild().getNodeValue());
          break;
          case "dataMapSuperTemplate":
  ...
  @Test public void testModelUpgrade() {
