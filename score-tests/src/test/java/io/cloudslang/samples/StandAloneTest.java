 ...
 * Time: 14:42
 */
@RunWith(value = SpringJUnit4ClassRunner.class) @ContextConfiguration(locations = "classpath:/META-INF/spring/samples/schemaAllTestContext.xml") @DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_CLASS) public class StandAloneTest {
  ...
  @PostConstruct public void waitToWorkerToBeUp() throws InterruptedException {
    Thread.sleep(1000L);
  }

  @Test(timeout = 20000) public void baseStandAloneTest() {
  ...
  @Test(timeout = 20000) public void subFlowTest() {
    ...
    waitForAllEventsToArrive(2);
  }

  @Test(timeout = 20000) public void testParallelFlow() {
  ...
  @Test(timeout = 20000) public void useExecutionSessionDataTest() {
  ...
  @Test(timeout = 20000) public void shareSessionDataWithSubflowTest() {
    ...
    Map<String, Serializable> getRuntimeValues = new HashMap<>();
    getRuntimeValues.put("STEP_TYPE", "PARALLEL");
    TriggeringProperties triggeringProperties = TriggeringProperties.create(executionPlan).setDependencies(dependencies).setRuntimeValues(getRuntimeValues);
    registerEventListener(EventConstants.SCORE_FINISHED_EVENT, SessionDataActions.SESSION_BEFORE_PUT_DATA_EVENT, SessionDataActions.SESSION_GET_DATA_EVENT);
    score.trigger(triggeringProperties);
    waitForAllEventsToArrive(3);
    ScoreEvent sessionBeforePutEvent = getEventFromQueueByType(SessionDataActions.SESSION_BEFORE_PUT_DATA_EVENT);
  ...
  @Test(timeout = 20000) public void sessionDataNotSharedBetweenExecutions() {
  ...
  private void registerEventListener(String... eventTypes) {
    ...
    eventBus.subscribe(new ScoreEventListener() {
      @Override public void onEvent(ScoreEvent event) {
