 ...
 * Date: 1/21/14
 */
@RunWith(value = SpringJUnit4ClassRunner.class) @ContextConfiguration(classes = EngineTest.Context.class) @DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_CLASS) public class EngineTest {
  private static final int WORKER_FREE_MEMORY = 200000000;

  @Autowired private Score score;

  @Autowired private WorkerNodeService workerNodeService;

  @Autowired private QueueDispatcherService dispatcherService;

  @Test public void baseEngineTest() throws InterruptedException {
  ...
  private ExecutionPlan createExecutionPlan() {
    return new ExecutionPlan().setFlowUuid("flowUUID").addStep(new ExecutionStep(1L)).setBeginStep(1L);
  }

  @Configuration @EnableTransactionManagement @ImportResource(value = "META-INF/spring/schema/schemaEngineTestContext.xml") static class Context {
    ...
    @Bean(name = "entityManagerFactory") @DependsOn(value = "liquibase") LocalContainerEntityManagerFactoryBean emf(JpaVendorAdapter jpaVendorAdapter, Properties jpaProperties) {
    ...
    @Bean(name = "consumptionFastEventBus") FastEventBus fastEventBus() {
    ...
    @Bean public QueueConfigurationDataService queueConfigurationDataService() {
