 ...
 * Date: 1/21/14
 */
@RunWith(value = SpringJUnit4ClassRunner.class) @ContextConfiguration @DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_CLASS) public class WorkerTest {
  @Autowired private WorkerManager workerManager;

  @Test public void baseEngineTest() throws InterruptedException {
  }

  @Configuration @ImportResource(value = "classpath:META-INF/spring/schema/schemaWorkerTestContext.xml") static class Context {
    @Bean EngineVersionService versionService() {
      return mock(EngineVersionService.class);
    }

    @Bean OrchestratorDispatcherService orchestratorDispatcherService() {
      return mock(OrchestratorDispatcherService.class);
    }

    @Bean QueueDispatcherService queueDispatcherService() {
      return mock(QueueDispatcherService.class);
    }

    @Bean PauseResumeService pauseResumeService() {
    ...
    @Bean WorkerDbSupportService workerDbSupportService() {
    ...
    @Bean MergedConfigurationService mergedConfigurationService() {
    ...
    @Bean ScoreEventFactory scoreEventFactory() {
      return mock(ScoreEventFactory.class);
    }

    @Bean QueueStateIdGeneratorService queueStateIdGeneratorService() {
    ...
    @Bean SuspendedExecutionService suspendedExecution() {
      return mock(SuspendedExecutionService.class);
    }

    @Bean AplsLicensingService aplsLicensingService() {
      return mock(AplsLicensingService.class);
    }

    @Bean public WorkerQueueDetailsContainer workerQueueDetailsContainer() {
