 ...
 */
@RunWith(value = MockitoJUnitRunner.class) public class TilesMavenLifecycleParticipantTest {
  ...
  @Before public void setupParticipant() {
  ...
  @Test public void testGetArtifactFromCoordinates() {
  ...
  @Test public void testGetArtifactRequestFromArtifact() {
    Artifact artifact = participant.getArtifactFromCoordinates("dummy", "dummy", "1");
  ...
  @Test public void testResolveArtifact() throws ArtifactResolutionException, MojoExecutionException, IOException {
  ...
  @Test public void testMergeTile() throws MavenExecutionException, IOException, ArtifactResolutionException {
  ...
  private void mockRepositoryWithProvidedArtifact(Artifact artifact) throws ArtifactResolutionException {
    ...
    when(this.mockRepositorySystem.resolveArtifact(same(defaultRepositorySystemSession), argThat(new MatchesArtifact(artifact)))).thenReturn(expectedResult);
  }
  ...
  class MatchesArtifact extends ArgumentMatcher<ArtifactRequest> {
    ...
    @Override public boolean matches(Object oRequest) {
      ArtifactRequest artifactRequest = (ArtifactRequest) oRequest;
      Artifact theirArtifact = artifactRequest.getArtifact();
      return myArtifact.getGroupId().equals(theirArtifact.getGroupId()) && myArtifact.getArtifactId().equals(theirArtifact.getArtifactId()) && myArtifact.getVersion().equals(theirArtifact.getVersion());
    }
  }
}