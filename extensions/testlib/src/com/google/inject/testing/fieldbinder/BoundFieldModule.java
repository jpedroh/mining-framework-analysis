...
public final class BoundFieldModule implements Module {
    ...
    private TypeLiteral<?> getBoundType() {
      ...
      if (bindClass == Bind.class) {
        ...
        if (!naturalType.isPresent()) {
          throwBoundFieldException(field, "Non parameterized Provider fields must have an explicit " + "binding class via @Bind(to = Foo.class)");
        }
  ...
  private static boolean hasInject(Field field) {
    return field.isAnnotationPresent(javax.inject.Inject.class) || field.isAnnotationPresent(com.google.inject.Inject.class);
  }
  ...
  private Optional<BoundFieldInfo> getBoundFieldInfo(TypeLiteral<?> containingClassType, Field field) {
    ...
    return Optional.of(new BoundFieldInfo(field, bindAnnotation, containingClassType.getFieldType(field)));
  }
  ...
  private void bindField(final BoundFieldInfo fieldInfo) {
    if (fieldInfo.naturalType.isPresent()) {
      ...
      if (!boundRawType.isAssignableFrom(naturalRawType)) {
        throwBoundFieldException(fieldInfo.field, "Requested binding type \"%s\" is not assignable from field binding type \"%s\"", boundRawType.getName(), naturalRawType.getName());
      }
    }
    ...
    @SuppressWarnings(value = { "unchecked" }) AnnotatedBindingBuilder<Object> binderUnsafe = (AnnotatedBindingBuilder<Object>) binder;
    if (isTransparentProvider(fieldInfo.type.getRawType())) {
      if (fieldInfo.bindAnnotation.lazy()) {
        throwBoundFieldException(fieldInfo.field, "\'lazy\' is incompatible with Provider valued fields");
      }
      @SuppressWarnings(value = { "unchecked" }) Provider<?> fieldValueUnsafe = (Provider<?>) getFieldValue(fieldInfo);
  ...
  private Object getFieldValue(final BoundFieldInfo fieldInfo) {
    ...
    if (fieldValue == null) {
      throwBoundFieldException(fieldInfo.field, "Binding to null values is not allowed. " + "Use Providers.of(null) if this is your intended behavior.", fieldInfo.field.getName());
    }
    return fieldValue;
  }
