...
public class OptionalBinderTest extends TestCase {
  ...
  final TypeLiteral<?> javaOptionalOfString = HAS_JAVA_OPTIONAL ? OptionalBinder.javaOptionalOf(stringKey.getTypeLiteral()) : null;

  final TypeLiteral<Optional<Provider<String>>> optionalOfProviderString = new TypeLiteral<Optional<Provider<String>>>() { };

  final TypeLiteral<?> javaOptionalOfProviderString = HAS_JAVA_OPTIONAL ? OptionalBinder.javaOptionalOfProvider(stringKey.getTypeLiteral()) : null;

  final TypeLiteral<Optional<javax.inject.Provider<String>>> optionalOfJavaxProviderString = new TypeLiteral<Optional<javax.inject.Provider<String>>>() { };

  final TypeLiteral<?> javaOptionalOfJavaxProviderString = HAS_JAVA_OPTIONAL ? OptionalBinder.javaOptionalOfJavaxProvider(stringKey.getTypeLiteral()) : null;
  ...
  final TypeLiteral<?> javaOptionalOfInteger = HAS_JAVA_OPTIONAL ? OptionalBinder.javaOptionalOf(intKey.getTypeLiteral()) : null;

  final TypeLiteral<Optional<Provider<Integer>>> optionalOfProviderInteger = new TypeLiteral<Optional<Provider<Integer>>>() { };

  final TypeLiteral<?> javaOptionalOfProviderInteger = HAS_JAVA_OPTIONAL ? OptionalBinder.javaOptionalOfProvider(intKey.getTypeLiteral()) : null;

  final TypeLiteral<Optional<javax.inject.Provider<Integer>>> optionalOfJavaxProviderInteger = new TypeLiteral<Optional<javax.inject.Provider<Integer>>>() { };

  final TypeLiteral<?> javaOptionalOfJavaxProviderInteger = HAS_JAVA_OPTIONAL ? OptionalBinder.javaOptionalOfJavaxProvider(intKey.getTypeLiteral()) : null;
  ...
  public void testDifferentBindingsFail_defaults() {
    ...
    try {
      ...
      assertContains(ce.getMessage(), "1) A binding to java.lang.String annotated with @" + Default.class.getName() + " was already configured at " + module.getClass().getName() + ".configure(", "at " + module.getClass().getName() + ".configure(");
    }
  }
  ...
  public void testOptionalIsAppropriatelyLazy() throws Exception {
    Module module = new AbstractModule() {
      ...
      @Provides @Named(value = "foo") int provideInt() {
        return nextValue++;
      }
    };
    Injector injector = Guice.createInjector(module);
  ...
  @SuppressWarnings(value = { "rawtypes" }) private Set<String> recurseForDependencies(Injector injector, HasDependencies hasDependencies) {
    Set<String> elements = Sets.newHashSet();
  ...
  public void testModuleOverrideRepeatedInstalls_toKey() {
    ...
    Injector injector = Guice.createInjector(m, Modules.override(m).with(m));
    assertEquals("B", injector.getInstance(Key.get(String.class)));
    assertOptionalVisitor(stringKey, setOf(m, Modules.override(m).with(m)), VisitType.BOTH, 0, linked(aKey), linked(bKey), null);
  }
  ...
  public void testModuleOverrideRepeatedInstalls_toProviderInstance() {
    ...
    Injector injector = Guice.createInjector(m, Modules.override(m).with(m));
    assertEquals("B", injector.getInstance(Key.get(String.class)));
    assertOptionalVisitor(stringKey, setOf(m, Modules.override(m).with(m)), VisitType.BOTH, 0, providerInstance("A"), providerInstance("B"), null);
  }
  ...
  public void testModuleOverrideRepeatedInstalls_toProviderKey() {
    ...
    Injector injector = Guice.createInjector(m, Modules.override(m).with(m));
    assertEquals("B", injector.getInstance(Key.get(String.class)));
    assertOptionalVisitor(stringKey, setOf(m, Modules.override(m).with(m)), VisitType.BOTH, 0, providerKey(Key.get(AStringProvider.class)), providerKey(Key.get(BStringProvider.class)), null);
  }

  private static class StringGrabber {
    private final String string;

    @SuppressWarnings(value = { "unused" }) public StringGrabber(@Named(value = "A_string") String string) {
      this.string = string;
    }

    @SuppressWarnings(value = { "unused" }) public StringGrabber(@Named(value = "B_string") String string, int unused) {
      this.string = string;
    }
  ...
  public void testModuleOverrideRepeatedInstalls_toConstructor() {
    Module m = new AbstractModule() {
      @Override protected void configure() {
        ...
        OptionalBinder<StringGrabber> b = OptionalBinder.newOptionalBinder(binder(), StringGrabber.class);
        try {
          b.setDefault().toConstructor(StringGrabber.class.getConstructor(String.class));
          b.setBinding().toConstructor(StringGrabber.class.getConstructor(String.class, int.class));
  ...
  @BindingAnnotation @Retention(value = RetentionPolicy.RUNTIME) @Target(value = { ElementType.FIELD, ElementType.PARAMETER, ElementType.METHOD }) private static @interface Marker {
  }

  @Marker public void testMatchingMarkerAnnotations() throws Exception {
  ...
  @OptionalBinder.Actual(value = "foo") @OptionalBinder.Default(value = "foo") static class Dummy {
  }

  @SuppressWarnings(value = { "unchecked" }) private <V extends java.lang.Object> Set<V> setOf(V... elements) {
    return ImmutableSet.copyOf(elements);
  }

  @SuppressWarnings(value = { "unchecked" }) private <T extends java.lang.Object> Optional<T> toOptional(Object object) throws Exception {
    assertTrue("not a java.util.Optional: " + object.getClass(), JAVA_OPTIONAL_CLASS.isInstance(object));
