...
@RequiredArgsConstructor public class ServerConnector extends PacketHandler {
  ...
  @Override public void connected(ChannelWrapper channel) throws Exception {
    ...
    if (BungeeCord.getInstance().config.isIpForward() && user.getSocketAddress() instanceof InetSocketAddress) {
      String newHost = copiedHandshake.getHost() + "\u0000" + AddressUtil.sanitizeAddress(user.getAddress()) + "\u0000" + user.getUUID();
  ...
  @Override public void handle(LoginPayloadRequest loginPayloadRequest) {
