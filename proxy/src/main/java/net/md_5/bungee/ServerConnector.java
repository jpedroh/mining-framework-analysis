...
@RequiredArgsConstructor public class ServerConnector extends PacketHandler {
  ...
  @Override public void handle(Login login) throws Exception {
    ...
    if (user.isNeedLogin()) {
      ...
      Login modLogin = new Login(login.getEntityId(), login.getGameMode(), (byte) login.getDimension(), login.getDifficulty(), (byte) user.getPendingConnection().getListener().getTabListSize(), login.getLevelType(), login.getViewDistance(), login.isReducedDebugInfo());
