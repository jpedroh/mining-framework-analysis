  ...
  @Override public void handle(Login login) throws Exception {
    ...
    if (user.getServer() == null) {
      ...
      if (handshakeHandler != null && handshakeHandler.isServerForge()) {
        ...
        modLogin = new Login(login.getEntityId(), login.getGameMode(), (byte) login.getDimension(), login.getDifficulty(), (byte) user.getPendingConnection().getListener().getTabListSize(), login.getLevelType(), login.isReducedDebugInfo());
      }
