...
@RequiredArgsConstructor public class ServerConnector extends PacketHandler {
  ...
  @Getter private ForgeServerHandler handshakeHandler;
  ...
  @Override public void connected(ChannelWrapper channel) throws Exception {
    ...
    if (BungeeCord.getInstance().config.isIpForward()) {
      ...
      if (user.getForgeClientHandler().isFmlTokenInHandshake()) {
        ...
        newp[newp.length - 1] = new LoginResult.Property(ForgeConstants.EXTRA_DATA, user.getExtraDataInHandshake().replaceAll("\u0000", "\u0001"), "");
        properties = newp;
      }
      if (properties.length > 0) {
        newHost += "\u0000" + BungeeCord.getInstance().gson.toJson(properties);
      }
  ...
  @Override public void handle(Login login) throws Exception {
...
=======
      new Login(login.getEntityId(), login.getGameMode(), (byte) login.getDimension(), login.getSeed(), login.getDifficulty(), (byte) user.getPendingConnection().getListener().getTabListSize(), login.getLevelType(), login.getViewDistance(), login.isReducedDebugInfo(), login.isNormalRespawn())
>>>>>>> /usr/src/app/output/spigotmc/bungeecord/4d6f19c2f720fb28540471a93d5b7b99c15dfa71/proxy/src/main/java/net/md_5/bungee/ServerConnector.java/right.java
      ;
      if (handshakeHandler != null && handshakeHandler.isServerForge()) {
        modLogin = new Login(login.getEntityId(), login.getGameMode(), login.getDimension(), login.getDifficulty(), (byte) user.getPendingConnection().getListener().getTabListSize(), login.getLevelType(), login.getViewDistance(), login.isReducedDebugInfo());
      } else {
        modLogin = new Login(login.getEntityId(), login.getGameMode(), (byte) login.getDimension(), login.getDifficulty(), (byte) user.getPendingConnection().getListener().getTabListSize(), login.getLevelType(), login.getViewDistance(), login.isReducedDebugInfo());
      }
      ...
      for (Objective objective : serverScoreboard.getObjectives()) {
        user.unsafe().sendPacket(new ScoreboardObjective(objective.getName(), objective.getValue(), objective.getType() == null ? null : ScoreboardObjective.HealthDisplay.fromString(objective.getType()), (byte) 1));
      }
