...
@RequiredArgsConstructor public class DownstreamBridge extends PacketHandler {
  ...
  @Override public void exception(final Throwable t) throws Exception {
    ...
    if (def != null) {
      ...
      ServerConnectRequest connectRequest = ServerConnectRequest.builder().callback(callback).retry(false).reason(ServerConnectEvent.Reason.SERVER_DOWN_REDIRECT).target(def).build();
  ...
  @Override public void handle(KeepAlive alive) throws Exception {
    ...
    if (timeout <= 0 || server.getKeepAlives().size() < timeout / 50) {
