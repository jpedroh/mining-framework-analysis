...
@RequiredArgsConstructor public class InitialHandler extends PacketHandler implements PendingConnection {
  ...
  @Override public void handle(LegacyPing ping) throws Exception {
    ...
    ServerPing legacy = new ServerPing(new ServerPing.Protocol(bungee.getName() + " " + bungee.getGameVersion(), bungee.getProtocolVersion()), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCount(), null), new TextComponent(TextComponent.fromLegacyText(listener.getMotd())), (Favicon) null);
  ...
  @Override public void handle(StatusRequest statusRequest) throws Exception {
    ...
    if (forced != null && listener.isPingPassthrough()) {
      ...
      pingBack.done(new ServerPing(new ServerPing.Protocol(bungee.getName() + " " + bungee.getGameVersion(), protocol), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCount(), null), motd, BungeeCord.getInstance().config.getFaviconObject()), null);
    }
          ...
          @Override public void run() {
            if (!ch.isClosing()) {
              ...
              if (getVersion() >= ProtocolConstants.MINECRAFT_1_7_6) {
                ...
                unsafe.sendPacket(new LoginSuccess(getUUID(), getName()));
              }
