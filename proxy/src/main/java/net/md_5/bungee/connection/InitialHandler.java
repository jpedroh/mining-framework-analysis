...
public class InitialHandler extends PacketHandler implements PendingConnection {
    ...
    public void handle(PacketWrapper packet) throws Exception {
        //BotFilter ch.isClosed
        if ((packet.packet == null) && (!ch.isClosed())) {
            //BotFilter removed bump
    ...
    public void handle(LegacyPing ping) throws Exception {
        ...
        ServerPing legacy = //BotFilter
                //BotFilter
        new ServerPing(new ServerPing.Protocol(bungee.getCustomBungeeName(), bungee.getProtocolVersion()), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCountBF(true), null), new TextComponent(TextComponent.fromLegacyText(listener.getMotd())), ((Favicon) (null)));
    ...
    private ServerPing getPingInfo(String motd, int protocol) {
        return //BotFilter
                //BotFilter
                //BotFilter PingLimiter.handle() ? null :
        new ServerPing(new ServerPing.Protocol(bungee.getCustomBungeeName(), protocol), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCountBF(true), null), motd, PingLimiter.handle() ? null : BungeeCord.getInstance().config.getFaviconObject());
    }
    ...
    public void delayedHandleOfLoginRequset() {
        ...
        bungee.getPluginManager().callEvent(new PreLoginEvent(this, callback));
    }
    ...
    private void finish() {
        if (isOnlineMode()) {
            ...
            if (oldName != null) {
                // TODO See #1218
                // TODO: Cache this disconnect packet
