...
getInstance
>>>>>>>
  ( ) . add  (   getAddress  ( ) . getAddress  ( ) ) ;  break ;   case 2 :   thisState =  State . USERNAME ;   ch . setProtocol  (  Protocol . LOGIN ) ;  if  (  !   ProtocolConstants . SUPPORTED_VERSION_IDS . contains  (  handshake . getProtocolVersion  ( ) ) )  {  if  (   handshake . getProtocolVersion  ( ) >  bungee . getProtocolVersion  ( ) )  {   disconnect  (  bungee . getTranslation  ( "outdated_server" ) ) ; } else  {   disconnect  (  bungee . getTranslation  ( "outdated_client" ) ) ; }  return ; }  if  (  
<<<<<<<
ManyChecksUtils
=======
Utils
>>>>>>>
 . isManyChecks  (    getAddress  ( ) . getAddress  ( ) . 
<<<<<<<
getAddress
=======
getHostAddress
>>>>>>>
  ( ) , false , false ) )  {   PacketUtil . kickPlayer  (  KickType . MANYCHECKS ,  Protocol . LOGIN , ch ,  getVersion  ( ) ) ;   
<<<<<<<
 bungee . getLogger  ( )
=======
disconnect
>>>>>>>
 . log  (  Level . INFO , "[{0}] disconnected: Too many checks in 10 min" ,   
<<<<<<<
 getAddress  ( )
=======
Config
...
getErrorManyChecks
>>>>>>>
  ( ) ) ;  return ; }  ServerPingUtils  ping =   BotFilter . getInstance  ( ) . getServerPingUtils  ( ) ;  if  (   ping . needCheck  ( ) &&  ping . needKickOrRemove  (   getAddress  ( ) . getAddress  ( ) ) )  {   disconnect  (    Settings . IMP . SERVER_PING_CHECK . KICK_MESSAGE ) ;    bungee . getLogger  ( ) . log  (  Level . INFO , "[{0}] disconnected: The player did not ping the server" ,    getAddress  ( ) . getAddress  ( ) . getHostAddress  ( ) ) ;  return ; }  if  (    bungee . getConnectionThrottle  ( ) != null &&   bungee . getConnectionThrottle  ( ) . throttle  (   getAddress  ( ) . getAddress  ( ) ) )  {   PacketUtil . kickPlayer  (  KickType . THROTTLE ,  Protocol . LOGIN , ch ,  getVersion  ( ) ) ;    bungee . getLogger  ( ) . log  (  Level . INFO , "[{0}] disconnected: Connection is throttled" ,    getAddress  ( ) . getAddress  ( ) . getHostAddress  ( ) ) ; }  break ;   default :  throw  new IllegalArgumentException  (  "Cannot request protocol " +  handshake . getRequestedProtocol  ( ) ) ; } }    @ Override public void handle  (  LoginRequest loginRequest )  throws Exception  {   Preconditions . checkState  (  thisState ==  State . USERNAME , "Not expecting USERNAME" ) ;    this . loginRequest = loginRequest ;  if  (   getName  ( ) . contains  ( "." ) )  {   disconnect  (  bungee . getTranslation  ( "name_invalid" ) ) ;  return ; }  if  (    getName  ( ) . length  ( ) > 16 )  {   disconnect  (  bungee . getTranslation  ( "name_too_long" ) ) ;  return ; }   int  limit =    BungeeCord . getInstance  ( ) . config . getPlayerLimit  ( ) ;  if  (   limit > 0 &&   bungee . 
<<<<<<<
getOnlineCountBF
=======
getOnlineCountWithGG
>>>>>>>
  ( false ) > limit )  {   disconnect  (  bungee . getTranslation  ( "proxy_full" ) ) ;  return ; }  Config  config =  Config . getConfig  ( ) ;  InetAddress  address =   getAddress  ( ) . getAddress  ( ) ;  boolean  needCheck =  config . needCheck  (  getName  ( ) ,  address . getHostAddress  ( ) ) ;  
<<<<<<<
KickType
=======
ServerPingUtils
...
null
=======
 pingUtils . needKickAndRemove  ( address )
>>>>>>>
...
disconnect
>>>>>>>
 . log  (  Level . INFO ,  "[{0}] disconnected: " . concat  (  (   kickType ==  KickType . COUNTRY ? "Country is not allowed" : "Proxy detected" ) ) ,  
<<<<<<<
...
getConfig
>>>>>>>
  ( ) . needCheck  (  getName  ( ) ,    getAddress  ( ) . getAddress  ( ) . 
<<<<<<<
getAddress
=======
getHostAddress
...
>>>>>>>
 . setHandler  ( 
<<<<<<<
 new Connector  ( userCon )
=======
userCon
>>>>>>>
 ) ; } else  {   finishLogin  ( userCon ) ; } }   public void finishLogin  (  UserConnection userCon )  {   Callback  < LoginEvent >  complete =  new  Callback  < LoginEvent >  ( )  {    @ Override public void done  (  LoginEvent result ,  Throwable error )  {  if  (  result . isCancelled  ( ) )  {   disconnect  (  result . getCancelReasonComponents  ( ) ) ;  return ; }  if  (  ch . isClosed  ( ) )  {  return ; }  if  (    ch . getHandle  ( ) . eventLoop  ( ) . inEventLoop  ( ) )  {   finnalyFinishLogin  ( userCon ) ; } else  {     ch . getHandle  ( ) . eventLoop  ( ) . execute  (   ( ) ->  {  if  (  !  ch . isClosing  ( ) )  {   finnalyFinishLogin  ( userCon ) ; } } ) ; } } } ;    bungee . getPluginManager  ( ) . callEvent  (  new LoginEvent  (  InitialHandler . this , complete ) ) ; }   private void finnalyFinishLogin  (  UserConnection userCon )  {      ch . getHandle  ( ) . pipeline  ( ) . get  (  HandlerBoss . class ) . setHandler  (  new UpstreamBridge  ( bungee , userCon ) ) ;    bungee . getPluginManager  ( ) . callEvent  (  new PostLoginEvent  ( userCon ) ) ;  ServerInfo  server ;  if  (   bungee . getReconnectHandler  ( ) != null )  {   server =   bungee . getReconnectHandler  ( ) . getServer  ( userCon ) ; } else  {   server =  AbstractReconnectHandler . getForcedHost  (  InitialHandler . this ) ; }  if  (  server == null )  {   server =  bungee . getServerInfo  (  listener . getDefaultServer  ( ) ) ; }   userCon . connect  ( server , null , true ) ;   thisState =  State . FINISHED ; }    @ Override public void disconnect  (  String reason )  {   disconnect  (  TextComponent . fromLegacyText  ( reason ) ) ; }    @ Override public void disconnect  (   final BaseComponent ...  reason )  {  if  (   thisState !=  State . STATUS &&  thisState !=  State . PING )  {   ch . delayedClose  (  new Kick  (  ComponentSerializer . toString  ( reason ) ) ) ; } else  {   ch . close  ( ) ; } }    @ Override public void disconnect  (  BaseComponent reason )  {   disconnect  (  new BaseComponent  [ ]  { reason } ) ; }    @ Override public String getName  ( )  {  return   (  name != null ) ? name :   (  loginRequest == null ) ? null :  loginRequest . getData  ( ) ; }    @ Override public  int getVersion  ( )  {  return   (  handshake == null ) ?  - 1 :  handshake . getProtocolVersion  ( ) ; }    @ Override public InetSocketAddress getAddress  ( )  {  return  ch . getRemoteAddress  ( ) ; }    @ Override public Unsafe unsafe  ( )  {  return unsafe ; }    @ Override public void setOnlineMode  (  boolean onlineMode )  {   Preconditions . checkState  (  thisState ==  State . USERNAME , "Can only set online mode status whilst state is username" ) ;    this . onlineMode = onlineMode ; }    @ Override public void setUniqueId  (  UUID uuid )  {   Preconditions . checkState  (  thisState ==  State . USERNAME , "Can only set uuid while state is username" ) ;   Preconditions . checkState  (  ! onlineMode , "Can only set uuid when online mode is false" ) ;    this . uniqueId = uuid ; }    @ Override public String getUUID  ( )  {  return   uniqueId . toString  ( ) . replaceAll  ( "-" , "" ) ; }    @ Override public String toString  ( )  {  return   "[" +  (   (   getName  ( ) != null ) ?  getName  ( ) :  getAddress  ( ) ) + "] <-> InitialHandler" ; }    @ Override public boolean isConnected  ( )  {  return  !  ch . isClosed  ( ) ; }   private void callCheckEvent  (  UserConnection userCon )  {   Callback  < BotFilterCheckStartingEvent >  complete =  new  Callback  < BotFilterCheckStartingEvent >  ( )  {    @ Override public void done  (  BotFilterCheckStartingEvent result ,  Throwable error )  {  if  (  result . isCancelled  ( ) )  {   disconnect  (  result . getCancelReasonComponents  ( ) ) ;  return ; }  if  (  ch . isClosed  ( ) )  {  return ; }  if  (    ch . getHandle  ( ) . eventLoop  ( ) . inEventLoop  ( ) )  {      ch . getHandle  ( ) . pipeline  ( ) . get  (  HandlerBoss . class ) . setHandler  (  new BFConnector  ( userCon ) ) ; } else  {     ch . getHandle  ( ) . eventLoop  ( ) . execute  (   ( ) ->  {  if  (  !  ch . isClosing  ( ) )  {      ch . getHandle  ( ) . pipeline  ( ) . get  (  HandlerBoss . class ) . setHandler  (  new BFConnector  ( userCon ) ) ; } } ) ; } } } ;    bungee . getPluginManager  ( ) . callEvent  (  new BotFilterCheckStartingEvent  ( userCon , complete ) ) ; } }