...
@RequiredArgsConstructor public class InitialHandler extends PacketHandler implements PendingConnection {
  ...
  @Override public void handle(StatusRequest statusRequest) throws Exception {
    ...
    if (forced != null && listener.isPingPassthrough()) {
      ...
      pingBack.done(new ServerPing(new ServerPing.Protocol(bungee.getCustomBungeeName(), protocol), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCountBF(true), null), motd, PingLimiter.handle() ? null : BungeeCord.getInstance().config.getFaviconObject()), null);
    }
