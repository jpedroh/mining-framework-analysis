...
@RequiredArgsConstructor public class InitialHandler extends PacketHandler implements PendingConnection {
  ...
  private ServerPing getPingInfo(String motd, int protocol) {
    return new ServerPing(new ServerPing.Protocol(bungee.getName() + " " + bungee.getGameVersion(), protocol), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCount(), null), motd, BungeeCord.getInstance().config.getFaviconObject());
  }
...
>>>>>>> /usr/src/app/output/spigotmc/bungeecord/932348085a5332c09cb9f0a07f72e65592b3dbc6/proxy/src/main/java/net/md_5/bungee/connection/InitialHandler.java/right.java
  ...
  private void finish() {
    ...
    if (bungee.getBotFilter().needCheck(getName(), getAddress().getAddress())) {
      ...
      bungee.getBotFilter().saveUser(userCon.getName().toLowerCase(), IPUtils.getAddress(userCon));
