...
@RequiredArgsConstructor public class InitialHandler extends PacketHandler implements PendingConnection {
  ...
  @Override public void handle(LegacyPing ping) throws Exception {
    ...
    ServerPing legacy = new ServerPing(new ServerPing.Protocol("BotFilter 1.8-1.12.1 by vk.com Leymooo_s", bungee.getProtocolVersion()), new ServerPing.Players(listener.getMaxPlayers(), bungee.getFakeOnlineCountWithGG(), null), new TextComponent(TextComponent.fromLegacyText(listener.getMotd())), (Favicon) null);
  ...
  @Override public void handle(StatusRequest statusRequest) throws Exception {
    ...
    if (forced != null && listener.isPingPassthrough()) {
      ...
      pingBack.done(new ServerPing(new ServerPing.Protocol("BotFilter 1.8-1.12.1 by vk.com Leymooo_s", protocol), new ServerPing.Players(listener.getMaxPlayers(), bungee.getFakeOnlineCountWithGG(), null), motd, BungeeCord.getInstance().config.getFaviconObject()), null);
    }
  ...
  @Override public void handle(LoginRequest loginRequest) throws Exception {
    ...
    if (limit > 0 && bungee.getOnlineCountWithGG() > limit) {
