...
@RequiredArgsConstructor public class InitialHandler extends PacketHandler implements PendingConnection {
  ...
  @Getter private ChannelWrapper ch;
  ...
  @Override public void handle(LegacyPing ping) throws Exception {
    checkState(!this.legacy, "Multiple legacy pings");
    this.legacy = true;
    final boolean v1_5 = ping.isV1_5();
    ServerPing legacy = new ServerPing(new ServerPing.Protocol(bungee.getCustomBungeeName(), bungee.getProtocolVersion()), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCountBF(true), null), new TextComponent(TextComponent.fromLegacyText(listener.getMotd())), (Favicon) null);
  ...
  private ServerPing getPingInfo(String motd, int protocol) {
    return new ServerPing(new ServerPing.Protocol(bungee.getCustomBungeeName(), protocol), new ServerPing.Players(listener.getMaxPlayers(), bungee.getOnlineCountBF(true), null), motd, PingLimiter.handle() ? null : BungeeCord.getInstance().config.getFaviconObject());
  }

  @Override public void handle(StatusRequest statusRequest) throws Exception {
    checkState(thisState == State.STATUS, "Not expecting STATUS");
  ...
  @Override public void handle(PingPacket ping) throws Exception {
    checkState(thisState == State.PING, "Not expecting PING");
    unsafe.sendPacket(ping);
    disconnect("");
  }

  @Override public void handle(Handshake handshake) throws Exception {
    checkState(thisState == State.HANDSHAKE, "Not expecting HANDSHAKE");
    ...
    switch (handshake.getRequestedProtocol()) {
      ...
      bungee.getBotFilter().getServerPingUtils().add(getAddress().getAddress());
      break;
      case 2:
      ...
      break;
      default:
      throw new FastException("[" + ch.getHandle().remoteAddress() + "] Cannot request protocol " + handshake.getRequestedProtocol());
    }
  }

  @Override public void handle(LoginRequest loginRequest) throws Exception {
    checkState(thisState == State.USERNAME, "Not expecting USERNAME");
  ...
  @Override public void handle(final EncryptionResponse encryptResponse) throws Exception {
    checkState(thisState == State.ENCRYPT, "Not expecting ENCRYPT");
  ...
  private void finish() {
    ...
    UserConnection 
<<<<<<< /usr/src/app/output/spigotmc/bungeecord/c7bcae2002da3950aad27aaf29801308bebbff23/proxy/src/main/java/net/md_5/bungee/connection/InitialHandler.java/left.java
    userCon = new UserConnection(bungee, ch, getName(), InitialHandler.this)
=======
    complete = new Callback<LoginEvent>() {
...
>>>>>>> /usr/src/app/output/spigotmc/bungeecord/c7bcae2002da3950aad27aaf29801308bebbff23/proxy/src/main/java/net/md_5/bungee/connection/InitialHandler.java/right.java
    ;
    userCon.setCompressionThreshold(BungeeCord.getInstance().config.getCompressionThreshold());
    ...
    if (Settings.IMP.PROTECTION.ALWAYS_CHECK || bungee.getBotFilter().needCheck(getName(), getAddress().getAddress())) {
      ...
      bungee.getBotFilter().saveUser(userCon.getName().toLowerCase(), IPUtils.getAddress(userCon), false);
      finishLogin(userCon, sendLoginSuccess);
    }
  }
