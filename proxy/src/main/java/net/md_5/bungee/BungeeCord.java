...
>>>>>>> /usr/src/app/output/spigotmc/bungeecord/c2e4969e7aef8be88f061c92d879b20803cbc4b5/proxy/src/main/java/net/md_5/bungee/BungeeCord.java/right.java
  ...
  public void startListeners() {
    for (final ListenerInfo info : config.getListeners()) {
      ...
      new ServerBootstrap().channel(PipelineUtils.getServerChannel(info.getSocketAddress())).option(ChannelOption.SO_REUSEADDR, true).childAttr(PipelineUtils.LISTENER, info).childHandler(PipelineUtils.SERVER_CHILD).group(eventLoops).localAddress(info.getSocketAddress()).bind().addListener(listener);
