...
public class BungeeCord extends ProxyServer {
  ...
  public void startListeners() {
    for (final ListenerInfo info : config.getListeners()) {
      ...
      new ServerBootstrap().channel(PipelineUtils.getServerChannel()).option(ChannelOption.SO_REUSEADDR, true).childOption(ChannelOption.WRITE_BUFFER_HIGH_WATER_MARK, 1024 * 1024 * 10).childOption(ChannelOption.WRITE_BUFFER_LOW_WATER_MARK, 1024 * 1024 * 1).childAttr(PipelineUtils.LISTENER, info).childHandler(PipelineUtils.SERVER_CHILD).group(bossEventLoopGroup, workerEventLoopGroup).localAddress(info.getHost()).bind().addListener(listener);
