...
public class BungeeCord extends ProxyServer {
  ...
  @Override @SuppressFBWarnings(value = "RV_RETURN_VALUE_IGNORED_BAD_PRACTICE") public void start() throws Exception {
    ...
    String property = System.getProperty("bungeeName");
    customBungeeName = (property == null ? getName() : property) + " " + getGameVersion();
    new BotFilter();
  ...
  public void startListeners() {
    for (final ListenerInfo info : config.getListeners()) {
      ...
      new ServerBootstrap().channel(PipelineUtils.getServerChannel()).option(ChannelOption.SO_REUSEADDR, true).childAttr(PipelineUtils.LISTENER, info).childHandler(PipelineUtils.SERVER_CHILD).group(bossEventLoopGroup, workerEventLoopGroup).localAddress(info.getHost()).bind().addListener(listener);
