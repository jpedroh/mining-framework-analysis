...
@Getter public class Configuration implements ProxyConfig {
  ...
  public void load() {
    ...
    disabledCommands.remove("bungee");
    disabledCommands.remove("botfilter");
  ...
  public void loadServers(ConfigurationAdapter adapter, boolean load) {
    ...
    if (servers == null) {
      ...
      servers.values().stream().filter((oldServer) -> (!newServers.containsKey(oldServer.getName()) || !newServers.get(oldServer.getName()).equals(oldServer))).forEach((server) -> {
