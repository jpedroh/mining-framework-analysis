...
public class YamlConfig implements ConfigurationAdapter {
  @RequiredArgsConstructor private enum DefaultTabList {
    GLOBAL,
    GLOBAL_PING,
    SERVER
  }
  ...
  private static Pattern RANGE_MATCH = Pattern.compile("(.+)\\[(\\d+)-(\\d+)\\]");
  ...
  @Override public void load() {
    ...
    if (permissions.isEmpty()) {
      permissions.put("default", Arrays.asList(new String[] { "bungeecord.command.server", "bungeecord.command.list" }));
      permissions.put("admin", Arrays.asList(new String[] { "bungeecord.command.alert", "bungeecord.command.end", "bungeecord.command.ip", "bungeecord.command.reload" }));
    }
  ...
  private <T extends java.lang.Object> T get(String path, T def) {
    return get(path, def, config);
  }

  @SuppressWarnings(value = { "unchecked" }) private <T extends java.lang.Object> T get(String path, T def, Map submap) {
  ...
  @SuppressWarnings(value = { "unchecked" }) private void set(String path, Object val, Map submap) {
    int index = path.indexOf('.');
    if (index == -1) {
      if (val == null) {
        submap.remove(path);
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public Map<String, ServerInfo> getServers() {
    ...
    for (Map.Entry<String, Map<String, Object>> entry : base.entrySet()) {
      ...
      if (matcher.find()) {
        ...
        BungeeCord.getInstance().getLogger().log(Level.INFO, "Use range {0} for server {1}", new Object[] { "[" + lower + ".." + upper + "]", name });
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) @SuppressFBWarnings(value = "RCN_REDUNDANT_NULLCHECK_OF_NONNULL_VALUE") public Collection<ListenerInfo> getListeners() {
  ...
  @Override @SuppressWarnings(value = { "unchecked" }) public Collection<String> getGroups(String player) {
  ...
  @Override public Collection<?> getList(String path, Collection<?> def) {
    return get(path, def);
  }

  @Override @SuppressWarnings(value = { "unchecked" }) public Collection<String> getPermissions(String group) {
