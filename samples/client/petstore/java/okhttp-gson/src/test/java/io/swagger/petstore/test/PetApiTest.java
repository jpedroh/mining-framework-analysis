...
public class PetApiTest {
  ...
  @Test public void testCreateAndGetPetAsync() throws Exception {
    Pet pet = createRandomPet();
    api.addPet(pet);
    final Map<String, Object> result = new HashMap<String, Object>();
    api.getPetByIdAsync(pet.getId(), new ApiCallback<Pet>() {
      @Override public void onFailure(ApiException e, int statusCode, Map<String, List<String>> responseHeaders) {
        result.put("error", e.getMessage());
      }

      @Override public void onSuccess(Pet pet, int statusCode, Map<String, List<String>> responseHeaders) {
        result.put("pet", pet);
      }
    ...
    do {
      if (tryCount > maxTry) {
        fail("have not got result of getPetByIdAsync after 10 seconds");
      }
      Thread.sleep(1000);
      tryCount += 1;
      if (result.get("error") != null) {
        fail((String) result.get("error"));
      }
