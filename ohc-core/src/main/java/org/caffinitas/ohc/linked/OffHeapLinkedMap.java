...
final class OffHeapLinkedMap {
  ...
  long getEntry(KeyBuffer key, boolean reference, boolean updateLRU) {
    boolean wasFirst = lock();
    try {
      for (long hashEntryAdr = table.getFirst(key.hash()); hashEntryAdr != 0L; hashEntryAdr = HashEntries.getNext(hashEntryAdr)) {
  ...
  boolean putEntry(long newHashEntryAdr, long hash, long keyLen, long bytes, boolean ifAbsent, long expireAt, long oldValueAddr, long oldValueOffset, long oldValueLen) {
  ...
  void clear() {
    ...
    try {
      ...
      for (int p = 0; p < table.size(); p++) {
        for (long hashEntryAdr = table.getFirst(p); hashEntryAdr != 0L; hashEntryAdr = next) {
  ...
  private void removeEntry(long removeHashEntryAdr, boolean removeFromTimeouts) {
    ...
    try {
      ...
      long prevEntryAdr = 0L;
      for (long hashEntryAdr = table.getFirst(hash); hashEntryAdr != 0L; prevEntryAdr = hashEntryAdr, hashEntryAdr = HashEntries.getNext(hashEntryAdr)) {
  ...
  void removeEntry(KeyBuffer key) {
    ...
    try {
      long prevEntryAdr = 0L;
      for (long hashEntryAdr = table.getFirst(key.hash()); hashEntryAdr != 0L; prevEntryAdr = hashEntryAdr, hashEntryAdr = HashEntries.getNext(hashEntryAdr)) {
        if (notSameKey(key, hashEntryAdr)) {
          continue;
        }
        removeHashEntryAdr = hashEntryAdr;
  ...
  private static boolean notSameKey(KeyBuffer key, long hashEntryAdr) {
    ...
    return serKeyLen != key.size() || !HashEntries.compareKey(hashEntryAdr, key, serKeyLen);
  }
  ...
  private static boolean notSameKey(long newHashEntryAdr, long newHash, long newKeyLen, long hashEntryAdr) {
    ...
    long serKeyLen = HashEntries.getKeyLen(hashEntryAdr);
    return serKeyLen != newKeyLen || !HashEntries.compare(hashEntryAdr, Util.ENTRY_OFF_DATA, newHashEntryAdr, Util.ENTRY_OFF_DATA, serKeyLen);
  }
  ...
  private void rehash() {
    ...
    for (int part = 0; part < tableSize; part++) {
      for (long hashEntryAdr = tab.getFirst(part); hashEntryAdr != 0L; hashEntryAdr = next) {
        next = HashEntries.getNext(hashEntryAdr);
  ...
  void getEntryAddresses(int mapSegmentIndex, int nSegments, List<Long> hashEntryAdrs) {
    ...
    try {
      ...
      for ( ; nSegments-- > 0 && mapSegmentIndex < table.size(); mapSegmentIndex++) {
        for (long hashEntryAdr = table.getFirst(mapSegmentIndex); hashEntryAdr != 0L; hashEntryAdr = HashEntries.getNext(hashEntryAdr)) {
          long expireAt = HashEntries.getExpireAt(hashEntryAdr);
  ...
  private static final class Table {
    ...
    private void removeLinkInternal(long hash, long hashEntryAdr, long prevEntryAdr, long next) {
      ...
      if (head == hashEntryAdr) {
        ...
        if (prevEntryAdr != 0L) {
          if (prevEntryAdr == -1L) {
            for (long adr = head; adr != 0L; prevEntryAdr = adr, adr = HashEntries.getNext(adr)) {
              if (adr == hashEntryAdr) {
                break;
              }
            }
