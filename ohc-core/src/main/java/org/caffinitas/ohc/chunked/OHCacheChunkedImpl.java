...
public final class OHCacheChunkedImpl<K extends java.lang.Object, V extends java.lang.Object> implements OHCache<K, V> {
  ...
  private boolean putInternalVariable(K k, V v, boolean ifAbsent, V old) {
    int sz = Util.entryOffData(isFixedSize()) + (isFixedSize() ? fixedKeySize + fixedValueSize + (old != null ? fixedValueSize : 0) : keySize(k) + valueSize(v) + (old != null ? valueSize(old) : 0));
  ...
  public OHCacheStats stats() {
    ...
    return new OHCacheStats(hitCount(), missCount(), evictedEntries(), expiredEntries(), perSegmentSizes(), size(), capacity(), freeCapacity(), rehashes, putAddCount(), putReplaceCount(), putFailCount, removeCount(), Uns.getTotalAllocated(), 0L);
  }
  ...
  public String toString() {
    return getClass().getSimpleName() + "(capacity=" + capacity() + " ,segments=" + maps.length + " ,chunkSize=" + chunkSize + " ,fixedKeySize=" + fixedKeySize + " ,fixedValueSize=" + fixedValueSize + " ,maxEntrySize=" + maxEntrySize;
  }
}