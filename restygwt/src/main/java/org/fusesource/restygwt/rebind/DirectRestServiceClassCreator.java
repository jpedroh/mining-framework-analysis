...
public class DirectRestServiceClassCreator extends DirectRestBaseSourceCreator {
  ...
  @Override protected ClassSourceFileComposerFactory createComposerFactory() throws UnableToCompleteException {
    return createClassSourceComposerFactory(JavaSourceCategory.CLASS, null, new String[] { source.getParameterizedQualifiedSourceName(), CallbackAware.class.getCanonicalName(), RestServiceProxy.class.getCanonicalName() });
  }
  ...
  private void createDelegateRestServiceProxyMethods() {
    ...
    p("public final void setResource(" + resourceClass + " resource) {").i(1).p("((" + restServiceProxyClass + ")service).setResource(resource);").i(-1).p("}");
    p("public final " + resourceClass + " getResource() {").i(1).p("return ((" + restServiceProxyClass + ")service).getResource();").i(-1).p("}");
    p("public final void setDispatcher(" + dispatcherClass + " resource) {").i(1).p("((" + restServiceProxyClass + ")service).setDispatcher(resource);").i(-1).p("}");
    p("public final " + dispatcherClass + " getDispatcher() {").i(1).p("return ((" + restServiceProxyClass + ")service).getDispatcher();").i(-1).p("}");
  }
