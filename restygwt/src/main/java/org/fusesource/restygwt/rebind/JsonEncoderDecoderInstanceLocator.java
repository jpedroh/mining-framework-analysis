...
public class JsonEncoderDecoderInstanceLocator {
  ...
  @SuppressWarnings(value = { "unchecked" }) private void fillInCustomGenerators(GeneratorContext context, TreeLogger logger) {
    try {
  ...
  public String encodeExpression(JType type, String expression, Style style) throws UnableToCompleteException {
    return encodeDecodeExpression(type, expression, style, "encode", JSON_ENCODER_DECODER_CLASS + ".toJSON", JSON_ENCODER_DECODER_CLASS + ".toJSON", JSON_ENCODER_DECODER_CLASS + ".toJSON", JSON_ENCODER_DECODER_CLASS + ".toJSON");
  }

  public String decodeExpression(JType type, String expression, Style style) throws UnableToCompleteException {
    return encodeDecodeExpression(type, expression, style, "decode", JSON_ENCODER_DECODER_CLASS + ".toMap", JSON_ENCODER_DECODER_CLASS + ".toSet", JSON_ENCODER_DECODER_CLASS + ".toList", JSON_ENCODER_DECODER_CLASS + ".toArray");
  }

  private String encodeDecodeExpression(JType type, String expression, Style style, String encoderMethod, String mapMethod, String setMethod, String listMethod, String arrayMethod) throws UnableToCompleteException {
    ...
    if (isCollectionType(clazz)) {
      ...
      if (coders != null) {
        ...
        if (encoderDecoder != null && keyEncoderDecoder != null) {
          return mapMethod + "(" + expression + ", " + keyEncoderDecoder + ", " + encoderDecoder + ", " + JSON_CLASS + ".Style." + style.name() + ")";
        } else {
          if (encoderDecoder != null) {
    ...
    if (encoderDecoder != null) {
      ...
      return arrayMethod + "(" + expression + ", " + encoderDecoder + ", new " + type.isArray().getComponentType().getQualifiedSourceName() + "[" + JSON_ENCODER_DECODER_CLASS + ".getSize(" + expression + ")])";
    }
    error("Do not know how to encode/decode " + type);
    return null;
  }

  protected String[] isMapEncoderDecoder(JClassType clazz, JClassType[] types, Style style) throws UnableToCompleteException {
    ...
    if (isCollectionType(clazz)) {
      ...
      if (coders != null) {
        ...
        if (result != null && keyEncoderDecoder != null) {
          return JSON_NESTED_ENCODER_DECODER_CLASS + ".mapEncoderDecoder( " + keyEncoderDecoder + ", " + result + ", " + JSON_CLASS + ".Style." + style.name() + " )";
        } else {
          if (result != null) {
            return JSON_NESTED_ENCODER_DECODER_CLASS + ".mapEncoderDecoder( " + result + ", " + JSON_CLASS + ".Style." + style.name() + " )";
          }
        }
      }
  ...
  boolean isCollectionType(JClassType clazz) {
    return clazz != null && (clazz.isAssignableTo(SET_TYPE) || clazz.isAssignableTo(LIST_TYPE) || clazz.isAssignableTo(MAP_TYPE) || clazz.isAssignableTo(COLLECTION_TYPE));
  }
