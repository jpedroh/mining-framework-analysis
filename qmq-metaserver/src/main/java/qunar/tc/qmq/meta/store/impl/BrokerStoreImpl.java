...
public class BrokerStoreImpl implements BrokerStore {
  ...
  @Override public Optional<BrokerMeta> queryBroker(final String hostname, final int servePort) {
    final List<BrokerMeta> brokers = jdbcTemplate.query(QUERY_BROKER_SQL, MAPPER, hostname, servePort);
    return optionalBroker(brokers);
  }

  @Override public Optional<BrokerMeta> queryByRole(final String brokerGroup, final int role) {
  ...
  @Override public List<BrokerMeta> queryBrokers(final String groupName) {
    return jdbcTemplate.query(QUERY_BROKERS_SQL, MAPPER, groupName);
  }

  @Override public List<BrokerMeta> allBrokers() {
    return jdbcTemplate.query(ALL_BROKERS_SQL, MAPPER);
  }

  @Override public int insertBroker(final BrokerMeta broker) {
    return jdbcTemplate.update(INSERT_BROKER_SQL, broker.getGroup(), broker.getRole().getCode(), broker.getHostname(), broker.getIp(), broker.getServePort(), broker.getSyncPort());
  }

  @Override public int replaceBrokerByRole(final BrokerMeta oldBroker, final BrokerMeta newBroker) {
    return jdbcTemplate.update(REPLACE_BROKER_BY_ROLE_SQL, newBroker.getHostname(), newBroker.getIp(), newBroker.getServePort(), newBroker.getSyncPort(), oldBroker.getGroup(), oldBroker.getRole().getCode(), oldBroker.getHostname(), oldBroker.getServePort());
  }
}