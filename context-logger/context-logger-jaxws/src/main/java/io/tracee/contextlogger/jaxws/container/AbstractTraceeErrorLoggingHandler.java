...
public abstract class AbstractTraceeErrorLoggingHandler extends AbstractTraceeHandler {
  ...
  @Override public final boolean handleFault(SOAPMessageContext context) {
    SOAPMessage soapMessage = context.getMessage();
    TraceeContextLogger.createDefault().logJsonWithPrefixedMessage("TRACEE JAXWS ERROR CONTEXT LISTENER", ImplicitContext.COMMON, ImplicitContext.TRACEE, JaxWsWrapper.wrap(THREAD_LOCAL_SOAP_MESSAGE_STR.get(), convertSoapMessageAsString(soapMessage)));
    return true;
  }
  ...
  @Override public final Set<QName> getHeaders() {
    return null;
  }
}