...
public class MissingConstructorIntegrationTest {
    ...
    public void should_handle_missing_no_args_constructor_gently() {
        ...
        String result = TraceeContextLogger.create().config().enforceProfile(Profile.ENHANCED).apply().build()
                .createJson(TestBrokenImplicitContentDataProviderWithoutDefaultConstructor.class);
    ...
    public void should_wrap_with_external_wrappers_correctly_using_enhanced_profile() {
        ...
        String result = TraceeContextLogger.create().config().enforceProfile(Profile.ENHANCED).apply().build()
                .createJson(new BrokenCustomContextDataWrapperWithMissingNoargsConstructor());
