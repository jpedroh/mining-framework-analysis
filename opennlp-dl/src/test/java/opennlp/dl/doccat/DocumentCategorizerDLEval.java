...
public class DocumentCategorizerDLEval extends AbstactDLTest {
  @Test public void categorize() throws IOException, OrtException {
    ...
    final DocumentCategorizerDL documentCategorizerDL = new DocumentCategorizerDL(model, vocab, getCategories(), new AverageClassificationScoringStrategy(), new InferenceOptions());
    ...
    final double[] sortedResult = Arrays.stream(result).boxed().sorted(Collections.reverseOrder()).mapToDouble(Double::doubleValue).toArray();
    final double[] expected = new double[] { 0.3391093313694, 0.2611352801322937, 0.24420668184757233, 0.11939861625432968, 0.03615010157227516 };
    System.out.println("Actual:   " + Arrays.toString(sortedResult));
    System.out.println("Expected: " + Arrays.toString(expected));
    Assertions.assertArrayEquals("Arrays differ in value.", expected, sortedResult, 0.0);
    Assertions.assertEquals(5, result.length);
    final String category = documentCategorizerDL.getBestCategory(result);
    Assertions.assertEquals("bad", category);
  }

  @Disabled(value = "This test will should only be run if a GPU device is present.") @Test public void categorizeWithGpu() throws Exception {
    final File model = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.onnx");
    final File vocab = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.vocab");
  ...
  @Test public void categorizeWithInferenceOptions() throws Exception {
    ...
    final DocumentCategorizerDL documentCategorizerDL = new DocumentCategorizerDL(model, vocab, categories, new AverageClassificationScoringStrategy(), inferenceOptions);
    ...
    Assertions.assertArrayEquals("Arrays differ in value.", expected, result, 0.0);
    Assertions.assertEquals(2, result.length);
    final String category = documentCategorizerDL.getBestCategory(result);
    Assertions.assertEquals("negative", category);
  }

  @Test public void scoreMap() throws Exception {
    final File model = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.onnx");
    final File vocab = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.vocab");
    final DocumentCategorizerDL documentCategorizerDL = new DocumentCategorizerDL(model, vocab, getCategories(), new AverageClassificationScoringStrategy(), new InferenceOptions());
  ...
  @Test public void sortedScoreMap() throws IOException, OrtException {
    final File model = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.onnx");
    final File vocab = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.vocab");
    final DocumentCategorizerDL documentCategorizerDL = new DocumentCategorizerDL(model, vocab, getCategories(), new AverageClassificationScoringStrategy(), new InferenceOptions());
  ...
  @Test public void doccat() throws IOException, OrtException {
    final File model = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.onnx");
    final File vocab = new File(getOpennlpDataDir(), "onnx/doccat/nlptown_bert-base-multilingual-uncased-sentiment.vocab");
    final DocumentCategorizerDL documentCategorizerDL = new DocumentCategorizerDL(model, vocab, getCategories(), new AverageClassificationScoringStrategy(), new InferenceOptions());
